# 深度学习目标检测模型全面综述：Faster R-CNN、R-FCN和SSD - 知乎
# 
...
![](https://pic2.zhimg.com/v2-45efd1c50170bafef1f07156f53b9c5d_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='640' height='356'></svg>)
...
在[以前的文章中](https://link.zhihu.com/?target=http%3A//mp.weixin.qq.com/s%3F__biz%3DMzA3MzI4MjgzMw%3D%3D%26mid%3D2650729930%26idx%3D1%26sn%3D5f81864de64075de1a460e622baea239%26chksm%3D871b29b4b06ca0a23e2bef560abf09dad1b7d48d7b657cb2cd47f59573638be74278013ea913%26scene%3D21%23wechat_redirect) ，机器之心曾梳理了 Xception、Inception 和 ResNet 等基本网络的架构和背后的设计思路。在本文中，我们会对 Tensorflow 的目标检测模型 Faster R-CNN、R-FCN 以及 SSD 做同样的介绍。希望在结束本文的阅读之后，你可以了解到以下两点：
...
下图具体描绘了上述 3 个步骤：
![](https://pic3.zhimg.com/v2-0701991a5668ed59a741e7cdfb14b4a2_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='640' height='407'></svg>)
...
Fast R-CNN 模型结构示意图：
![](https://pic1.zhimg.com/v2-20b5a36fb53ef0f04f58d95719fa50bc_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='640' height='409'></svg>)
...
![](https://pic2.zhimg.com/v2-f03af24e1d800e8cb92dae6770b57c49_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='640' height='378'></svg>)
...
![](https://pic3.zhimg.com/v2-6df2ac557e574d8c9668317d17fcf9f6_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='640' height='574'></svg>)
...
以下是 R-FCN 的工作方式：
- 在输入图像上运行一个 CNN（本例中使用的是 ResNet）。
- 添加一个全卷积层，以生成位置敏感分数图的 score bank。这里应该有 k²(C+1) 个分数图，其中，k²代表切分一个目标的相关位置的数量（比如，3²代表一个 3x3 的空间网格），C+1 代表 C 个类外加一个背景。
...
- 一旦每个 k²子区域都具备每个类的「目标匹配」值，那么我们就可以对这些子区域求平均值，得到每个类的分数。
- 通过对剩下 C+1 个维度向量进行 softmax 回归，来对 RoI 进行分类。
...
![](https://pic1.zhimg.com/v2-753cd3360427f87069282722d5182a14_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='640' height='283'></svg>)
...
![](https://pic2.zhimg.com/v2-59bb570a810d0881c1ef9b361ef6a5a1_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='640' height='631'></svg>)
...
- 在一系列卷积层中传递这个图像，产生一系列大小不同的特征图（比如 10x10、6x6、3x3 等等。）
- 对每个这些特征图中的每个位置而言，都使用一个 3x3 的卷积滤波器（convolutional filter）来评估一小部分默认的边界框。这些默认边的界框本质上等价于 Faster R-CNN 的 anchor box。
- 对每个边界框都同时执行预测： a）边界框的偏移；b）分类的概率。
- 在训练期间，用这些基于 IoU（Intersection over Union，也被称为 Jaccard 相似系数）系数的预测边界框来匹配正确的边界框。被最佳预测的边界框将被标签为「正」，并且其它边界框的 IoU 大于 0.5。
...
![](https://pic2.zhimg.com/v2-43b59ed944f4f968dc3cb94751d6a989_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='640' height='184'></svg>)
如上所述，最终有可缩小尺寸的「额外特征层」。这些尺寸变化的特征图有助于捕捉不同大小的目标。例如，下面是一个正在执行的 SSD。
![](https://pic4.zhimg.com/v2-fa0b3b32a90356a2767b280b0c7ec213_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='640' height='239'></svg>)
...
原文链接：[https://medium.com/towards-data-science/deep-learning-for-object-detection-a-comprehensive-review-73930816d8d9](https://link.zhihu.com/?target=https%3A//medium.com/towards-data-science/deep-learning-for-object-detection-a-comprehensive-review-73930816d8d9)
