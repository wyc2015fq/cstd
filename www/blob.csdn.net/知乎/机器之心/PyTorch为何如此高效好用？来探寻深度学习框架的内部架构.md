# PyTorch为何如此高效好用？来探寻深度学习框架的内部架构 - 知乎
# 



选自blog.christianperone，作者：Christian S. Perone，机器之心编译，原文链接：[http://blog.christianperone.com/2018/03/pytorch-internal-architecture-tour/](https://link.zhihu.com/?target=http%3A//blog.christianperone.com/2018/03/pytorch-internal-architecture-tour/)
...
![](https://pic2.zhimg.com/v2-60a639d1a6a11b8cddf10a74e6630c8d_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1002' height='362'></svg>)
Python API 中有两个额外的宏，分别称为 Py_INCREF() 和 Py_DECREF()，可用于增加和减少 Python 对象的引用计数。多实体可以借用或拥有其他对象的引用（因此引用计数被增加），而只有当引用计数达到零，Python 才会自动删除那个对象的内存。想了解更多有关 Python C/++扩展的知识，请参见：[https://docs.python.org/3/extending/newtypes.html](https://link.zhihu.com/?target=https%3A//docs.python.org/3/extending/newtypes.html)。
...
*代码摘自（tensor_numpy.cpp：[https://github.com/pytorch/pytorch/blob/master/torch/csrc/utils/tensor_numpy.cpp#L88](https://link.zhihu.com/?target=https%3A//github.com/pytorch/pytorch/blob/master/torch/csrc/utils/tensor_numpy.cpp%23L88)）*
...
![](https://pic4.zhimg.com/v2-c232908ccaad253fc3b2d861d35dc7f3_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1034' height='234'></svg>)
...
*代码摘自（THAllocator.h：[https://github.com/pytorch/pytorch/blob/master/aten/src/TH/THAllocator.h#L16](https://link.zhihu.com/?target=https%3A//github.com/pytorch/pytorch/blob/master/aten/src/TH/THAllocator.h%23L16)）*
...
*代码摘自（THCAllocator.c：[https://github.com/pytorch/pytorch/blob/master/aten/src/THC/THCAllocator.c#L3](https://link.zhihu.com/?target=https%3A//github.com/pytorch/pytorch/blob/master/aten/src/THC/THCAllocator.c%23L3)）*
...
![](https://pic2.zhimg.com/v2-77237720119a1f4668c131c1eed2b1ad_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='800' height='244'></svg>)
...
现在让我们来看看 PyTorch 代码库最新的一些内容——DLPack（[https://github.com/dmlc/dlpack](https://link.zhihu.com/?target=https%3A//github.com/dmlc/dlpack)）。DLPack 是一个内存张量结构的开放标准，允许张量数据在框架之间交换。非常有趣的是，这种内存表示是标准化的——与大多数框架已经在使用的内存表示方法非常类似，这就允许我们可以在框架之间共享，且完全无需复制数据。鉴于目前我们还没有内部通信的工具，DLPack 是一个非常了不起的创造。
...
*代码来自 [https://github.com/dmlc/dlpack/blob/master/include/dlpack/dlpack.h](https://link.zhihu.com/?target=https%3A//github.com/dmlc/dlpack/blob/master/include/dlpack/dlpack.h)*
