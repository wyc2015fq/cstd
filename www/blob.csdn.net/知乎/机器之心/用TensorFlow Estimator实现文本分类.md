# 用TensorFlow Estimator实现文本分类 - 知乎
# 



选自[http://ruder.io](https://link.zhihu.com/?target=http%3A//ruder.io)，作者：Sebastian Ruder，机器之心编译。
...
- 第一部分重点讨论了预建评估器（[https://developers.googleblog.com/2017/09/introducing-tensorflow-datasets.html](https://link.zhihu.com/?target=https%3A//developers.googleblog.com/2017/09/introducing-tensorflow-datasets.html)）
- 第二部分讨论了特征列（[https://developers.googleblog.com/2017/11/introducing-tensorflow-feature-columns.html](https://link.zhihu.com/?target=https%3A//developers.googleblog.com/2017/11/introducing-tensorflow-feature-columns.html)）
- 第三部分讲解了如何创建一个自定义的评估器（[https://developers.googleblog.com/2017/12/creating-custom-estimators-in-tensorflow.html](https://link.zhihu.com/?target=https%3A//developers.googleblog.com/2017/12/creating-custom-estimators-in-tensorflow.html)）。

第四部分的内容将会建立在上述所有章节的基础上，我们将处理一系列不同的自然语言处理（NLP）问题。本文演示了如何使用自定义的 TensorFlow 评估器、嵌入技术及 tf.layers 模块（[https://www.tensorflow.org/api_docs/python/tf/layers](https://link.zhihu.com/?target=https%3A//www.tensorflow.org/api_docs/python/tf/layers)）来处理文本分类任务。在这篇文章中，我们会学习 word2vec 词嵌入和迁移学习技术，在有标签数据稀缺时获得更好的模型性能。

我们将展示相关的代码片段。这里是完整的 Jupyter Notebook 代码，你可以在本地或者 Google Colaboratory 上运行它。清晰的「.py」源文件可以通过以下链接获得：（[https://github.com/eisenjulian/nlp_estimator_tutorial/blob/master/nlp_estimators.py](https://link.zhihu.com/?target=https%3A//github.com/eisenjulian/nlp_estimator_tutorial/blob/master/nlp_estimators.py)）。
...
我们将使用的数据集是 IMDB 大规模电影评论数据集（[http://ai.stanford.edu/~amaas/data/sentiment/](https://link.zhihu.com/?target=http%3A//ai.stanford.edu/~amaas/data/sentiment/)），它包含 25,000 篇高度分化的电影评论作为训练数据，另有 25,000 篇作为测试数据。我们将使用这个数据集去训练一个能够预测一条评论是正面还是负面的二分类模型。
...
![](https://pic1.zhimg.com/v2-5b73607d69120920671dfee0970cd864_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='655' height='542'></svg>)
...
我们可以使用 TensorBoard 中的 t-SNE（[https://en.wikipedia.org/wiki/T-distributed_stochastic_neighbor_embedding](https://link.zhihu.com/?target=https%3A//en.wikipedia.org/wiki/T-distributed_stochastic_neighbor_embedding)）将我们 50 维的词向量方案可视化为 R^3。我们估计相似的词彼此距离会比较接近。这可以称为一种检验我们的模型权重并且发现意想不到的表现的有效方法。
![](https://pic3.zhimg.com/v2-2b1bfbacbcb47e925a844f5d5b0156de_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='988' height='690'></svg>)
**卷积**
...
![](https://pic3.zhimg.com/v2-e0a9cfdce23398431903bd745bea3d22_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='872' height='512'></svg>)
...
![](https://pic1.zhimg.com/v2-4e20898fe63c8b84940b895b29be4af4_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1044' height='103'></svg>)
...
每个神经元一次处理一个词嵌入，并且根据依赖于嵌入向量 x_t 和之前的状态 h_t-1 的可微的计算更新它的内部状态。为了更好地理解 LSTM 的工作原理，可以参考 Chris Olah 的博文（[https://colah.github.io/posts/2015-08-Understanding-LSTMs/](https://link.zhihu.com/?target=https%3A//colah.github.io/posts/2015-08-Understanding-LSTMs/)）。
![](https://pic2.zhimg.com/v2-cb41e5887ce568e6d1ba0efb3c13ca19_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='470'></svg>)来源：Understanding LSTM Networks by Chris Olah
完整的 LSTM 模型可以表示成下面的简单流程图：
![](https://pic2.zhimg.com/v2-2c2c903e61acbf9d1758aedda1c93171_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='696' height='234'></svg>)
...
![](https://pic4.zhimg.com/v2-969877e8c34152135c4dc8c382bd9f6b_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='655'></svg>)


![](https://pic4.zhimg.com/v2-cc55f5cba828cf29611ccd644dd2b653_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='467'></svg>)
...
更多细节请查看：
- 能在本地或 Colaboratory 上运行的 Jupyter notebook（[https://github.com/eisenjulian/nlp_estimator_tutorial/blob/master/nlp_estimators.ipynb](https://link.zhihu.com/?target=https%3A//github.com/eisenjulian/nlp_estimator_tutorial/blob/master/nlp_estimators.ipynb)）
- 本文使用的完整源代码（[https://github.com/eisenjulian/nlp_estimator_tutorial/blob/master/nlp_estimators.py](https://link.zhihu.com/?target=https%3A//github.com/eisenjulian/nlp_estimator_tutorial/blob/master/nlp_estimators.py)）
- TensorFlow 嵌入技术指南（[https://www.tensorflow.org/programmers_guide/embedding](https://link.zhihu.com/?target=https%3A//www.tensorflow.org/programmers_guide/embedding)）
- TensorFlow 词向量表示教程（[https://www.tensorflow.org/tutorials/word2vec](https://link.zhihu.com/?target=https%3A//www.tensorflow.org/tutorials/word2vec)）
- NLTK 的原始文本处理（[http://www.nltk.org/book/ch03.html](https://link.zhihu.com/?target=http%3A//www.nltk.org/book/ch03.html)）章节，讲述如何设计语言数据管道
