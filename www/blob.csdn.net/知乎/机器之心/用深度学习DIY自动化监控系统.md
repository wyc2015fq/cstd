...
**选自Medium，作者：Bharath Raj，机器之心编译。**
...
![](https://pic4.zhimg.com/v2-4dd13355ccf0dd38211cb408f903a24b_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='800' height='363'></svg>)
传统计算机视觉（CV）和深度学习相比于人类，在 ImageNet 数据集上随时间变化的错误率。（图源：[https://www.dsiac.org/resources/journals/dsiac/winter-2017-volume-4-number-1/real-time-situ-intelligent-video-analytics](https://link.zhihu.com/?target=https%3A//www.dsiac.org/resources/journals/dsiac/winter-2017-volume-4-number-1/real-time-situ-intelligent-video-analytics)）
...
![](https://pic2.zhimg.com/v2-0eb9d3331d23e3e698fc3f53c2ccb2d1_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='700' height='771'></svg>)
为了找到目标的确切位置，我们的算法应该检查图像的每一部分以找到一个类的存在。这比听起来更难。但自 2014 年以来，深度学习领域不断更新的研究已经引入了可以实时检测目标的复杂神经网络。
![](https://pic4.zhimg.com/v2-da393656c64a6631f1cc4c10fc3bff87_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='340'></svg>)看，仅在 2 年时间内，性能就增加了那么多！
有几种在内部使用不同方法的深度学习架构来实现相同的任务。最流行的变种是 Faster RCNN、YOLO 和 SSD 网络。
![](https://pic1.zhimg.com/v2-82f5a7b8a1c2007c0619070ffd9da244_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='643'></svg>)速度和准确率的权衡。更高的 mAP 和 更少的 GPU 运行时间是最优的
...
![](https://pic1.zhimg.com/v2-3f21eeaab113237f692a59403ab03e88_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='720'></svg>)典型的监控摄像机。（图片来自 Unsplash 的 Scott Webb）
...
![](https://pic2.zhimg.com/v2-e66626351f5f375efeb43fc8ffaf8c91_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='690' height='408'></svg>)内存消耗与 GPU 推断时间（毫秒）。大多数高性能模型都会消耗大量内存。（图源：https://arxiv.org/pdf/1611.10012.pdf）- 边缘处理
...
![](https://pic1.zhimg.com/v2-2b0f134330721836157aa1df2dde2660_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='563'></svg>)
不同目标检测器的 FPS 性能。（图源：[https://medium.com/@jonathan_hui/object-detection-speed-and-accuracy-comparison-faster-r-cnn-r-fcn-ssd-and-yolo-5425656ae359](https://link.zhihu.com/?target=https%3A//medium.com/%40jonathan_hui/object-detection-speed-and-accuracy-comparison-faster-r-cnn-r-fcn-ssd-and-yolo-5425656ae359)）
...
在本节中，我们将使用目标检测来检测一下如何识别行人。我们将使用 TensorFlow 目标检测 API 来构建我们的模块。我们会简要探讨如何设置 API，并训练其完成监控任务。详细解释见另一篇博文（[https://medium.freecodecamp.org/how-to-play-quidditch-using-the-tensorflow-object-detection-api-b0742b99065d](https://link.zhihu.com/?target=https%3A//medium.freecodecamp.org/how-to-play-quidditch-using-the-tensorflow-object-detection-api-b0742b99065d)）。
...
3. 推断
![](https://pic1.zhimg.com/v2-75292030e4ec4190e359373907a13a34_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='700' height='1048'></svg>)涉及训练目标检测模型的工作流程
...
![](https://pic3.zhimg.com/v2-c9ba1f95d218278585ed03638069c6d6_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='608'></svg>)从我们的数据集中提取带注释的图像
如前所述，摄像机中的图像质量可能较低。因此你必须训练你的模型适应这样的工作条件。一种巧妙的方法是使用数据扩充，在此有详细解释（[https://medium.com/nanonets/how-to-use-deep-learning-when-you-have-limited-data-part-2-data-augmentation-c26971dc8ced](https://link.zhihu.com/?target=https%3A//medium.com/nanonets/how-to-use-deep-learning-when-you-have-limited-data-part-2-data-augmentation-c26971dc8ced)）。本质上说，我们必须添加一些噪音来降低数据集中图片的质量。我们还可以尝试模糊和侵蚀效果。

为了目标检测任务，我们将使用 TownCentre 数据集。我们将使用视频的前 3600 帧进行训练和验证，剩下的 900 帧用来测试。你可以使用我 github repo 中的脚本来提取数据集。GitHub repo：[https://github.com/thatbrguy/Pedestrian-Detector](https://link.zhihu.com/?target=https%3A//github.com/thatbrguy/Pedestrian-Detector)。
...
克隆存储库（[https://github.com/thatbrguy/Pedestrian-Detector](https://link.zhihu.com/?target=https%3A//github.com/thatbrguy/Pedestrian-Detector)）。运行以下命令去安装需求包，编译一些 Protobuf 库并设置路径变量。
...
接着，你必须创建一个包含 XML 和图片文件名的文本文件。例如，如果你的数据集中有图像 img1.jpg、img2.jpg 以及 img1.xml、img2.xml，你的 trainval.txt 应如下所示：
...
![](https://pic3.zhimg.com/v2-a5474937aa7e68444e4d2daa93b8c1b2_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='650'></svg>)一堆在 MS COCO 数据集上的预训练模型
...
![](https://pic2.zhimg.com/v2-15511d25db22c20953c15b77f540d3cd_b.gif)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='350' height='197'></svg>)
**训练时间**

下图展示了训练每个模型 10 k 步（以小时为单位）所需的时间。这不包括超参数搜索所需的时间。
![](https://pic4.zhimg.com/v2-6250e236ca16177529ebeb57826263db_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1058' height='712'></svg>)
...
![](https://pic1.zhimg.com/v2-ade25f5f89c1ab6c221456630853db1c_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='800' height='397'></svg>)
...
![](https://pic2.zhimg.com/v2-800603fab5c4502e7d27662cf72e9bcd_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='800' height='397'></svg>)
...
![](https://pic3.zhimg.com/v2-a340b35206ed939329e88ad552425f72_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='800' height='537'></svg>)
...
![](https://pic4.zhimg.com/v2-3ef28d4059fc4e443b7bdc09c1b8b943_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='700' height='935'></svg>)使用 Nanonets 简化工作流程
早些时候，我曾提到像微型无人机这样的移动监控设备如何大大提高效率。我们可以使用类似 Raspberry Pi 的微控制器轻松地建造这样的无人机，同时我们可以使用 API 调用来执行推断。

开始使用 Nanonets API 进行目标检测是非常简单的，但若想要一篇讲解清楚的指南，你可以查看这篇博文（[https://medium.com/nanonets/how-to-easily-detect-objects-with-deep-learning-on-raspberrypi-225f29635c74](https://link.zhihu.com/?target=https%3A//medium.com/nanonets/how-to-easily-detect-objects-with-deep-learning-on-raspberrypi-225f29635c74)）。
...
[Pedestrian Detection_腾讯视频​v.qq.com![图标](https://pic3.zhimg.com/v2-a772a2982020f0c43d39432a93d041da_180x120.jpg)](https://link.zhihu.com/?target=https%3A//v.qq.com/x/page/k0771ddbo0r.html)
**自动化监控可靠吗？**
...
原文链接：[https://medium.com/nanonets/how-to-automate-surveillance-easily-with-deep-learning-4eb4fa0cd68d](https://link.zhihu.com/?target=https%3A//medium.com/nanonets/how-to-automate-surveillance-easily-with-deep-learning-4eb4fa0cd68d)
