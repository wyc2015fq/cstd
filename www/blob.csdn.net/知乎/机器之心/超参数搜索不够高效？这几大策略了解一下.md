# 超参数搜索不够高效？这几大策略了解一下 - 知乎
# 



选自 floydhub，作者：Alessio Gozzoli，机器之心编译。
...
![](https://pic1.zhimg.com/v2-be11ff8823a7dbdc54c2a6f785c4ca5c_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='809' height='206'></svg>)将超参数比作「旋钮」或「拨号盘」
或者：

> 超参数是在开始训练之前手动设置的具有预定值的训练变量。

我们可能会同意学习率和 Dropout 率是超参数，但模型设计变量呢？模型设计变量包括嵌入，层数，激活函数等。我们应该将这些变量视为超参数吗？
![](https://pic1.zhimg.com/v2-82972fcbc77851a034edca58499a3ffc_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='578'></svg>)模型设计变量+超参数→模型参数
...
![](https://pic3.zhimg.com/v2-7f719d5a82eef2fcfdcef57eeba2258e_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='582'></svg>)变量类别示例图
**下一个问题：搜索代价高昂**

我们已经知道，我们的目标是搜索超参数的最佳配置，但超参数搜索本质上是一个受计算能力、金钱和时间约束的迭代过程。
![](https://pic4.zhimg.com/v2-9c5e84e338e3afc0fcec47d122d34bd7_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='750' height='293'></svg>)超参数搜索周期
...
![](https://pic3.zhimg.com/v2-5283ad426e8e05d8891b320d59df3aae_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='891' height='645'></svg>)照看（babysitting）
...
*![](https://pic3.zhimg.com/v2-7d884fdd1472185644110b90026bd5d2_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1069' height='516'></svg>)两变量并发执行的网格搜索*
...
单击以下链接可在 FloydHub 上打开 Workspace：[https://www.floydhub.com/signup?source=run](https://link.zhihu.com/?target=https%3A//www.floydhub.com/signup%3Fsource%3Drun)。你可以使用工作区在完全配置的云服务器上运行以下代码（使用 Scikit-learn 和 Keras 进行网格搜索）。
...
几年前，Bergstra 和 Bengio 发表了一篇惊人的论文 ([http://www.jmlr.org/papers/volume13/bergstra12a/bergstra12a.pdf](https://link.zhihu.com/?target=http%3A//www.jmlr.org/papers/volume13/bergstra12a/bergstra12a.pdf))，证明了网格搜索的低效率。

网格搜索和随机搜索之间唯一真正的区别在于策略周期的第 1 步 - 随机搜索从配置空间中随机选取点。

让我们使用下面的图片（论文中提供）来展示研究员的证明。
![](https://pic2.zhimg.com/v2-7c3f18c2fffddf08b40e94ce38fb3c65_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='913' height='528'></svg>)网格搜索 vs 随机搜索
...
![](https://pic3.zhimg.com/v2-2b017099364b08344d70c309c0391992_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='839' height='504'></svg>)网格搜索和随机搜索的优缺点
现在试试随机搜索！

单击以下链接可在 FloydHub 上打开 Workspace：[https://www.floydhub.com/signup?source=run](https://link.zhihu.com/?target=https%3A//www.floydhub.com/signup%3Fsource%3Drun)。你可以使用工作区在完全配置的云服务器上运行以下代码（使用 Scikit-learn 和 Keras 进行随机搜索）。
...
![](https://pic2.zhimg.com/v2-23364ccd28b3fbb258fadb7ccb146c39_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='498' height='137'></svg>)常用的批大小和学习率的比例空间
例如，通常使用 2 的幂作为批大小的值，并在对数尺度上对学习率进行采样。
![](https://pic1.zhimg.com/v2-bfcb951ae9dd423ceb245d7b6b6f7af0_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='495' height='422'></svg>)放大！
...
![](https://pic3.zhimg.com/v2-ee445b605d8e375db22410f991c5a7f6_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='847' height='339'></svg>)贝叶斯优化工作流
...
我们来深入研究这个伟大教程 ([https://www.iro.umontreal.ca/~bengioy/cifar/NCAP2014-summerschool/slides/Ryan_adams_140814_bayesopt_ncap.pdf](https://link.zhihu.com/?target=https%3A//www.iro.umontreal.ca/~bengioy/cifar/NCAP2014-summerschool/slides/Ryan_adams_140814_bayesopt_ncap.pdf)) 提供的示例。
![](https://pic1.zhimg.com/v2-36513a33d80fe76d5c08be084b8d16d8_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='864' height='562'></svg>)有 2 个点的高斯过程的优化过程
...
![](https://pic3.zhimg.com/v2-c3deb90d82724fadc5eb54cb5e51336a_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='863' height='549'></svg>)3 点高斯过程
我们训练的模型越多，代理人对下一个有希望采样的点就越有信心。以下是模型经过 8 次训练后的图表：
![](https://pic3.zhimg.com/v2-1c563de1a7a3f3baf43fc459cac8a166_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='870' height='549'></svg>)8 点高斯过程
高斯过程属于基于序列模型的优化（SMBO）类别的算法。正如我们刚看到的，这些算法为开始搜索最佳超参数配置提供了非常好的基准。但是，跟所有工具一样，它们也有缺点：
- 根据定义，该过程是有顺序的
- 它只能处理数值参数
- 即使训练表现不佳，它也不提供任何停止训练的机制

请注意，我们只是简单地谈到了这个话题，如果你对细节部分以及如何扩展 SMBO 感兴趣，那么请看一下这篇论文 ([https://www.cs.ubc.ca/~hutter/papers/10-TR-SMAC.pdf)](https://link.zhihu.com/?target=https%3A//www.cs.ubc.ca/~hutter/papers/10-TR-SMAC.pdf)。
...
单击以下链接可在 FloydHub 上打开 Workspace：[https://www.floydhub.com/signup?source=run](https://link.zhihu.com/?target=https%3A//www.floydhub.com/signup%3Fsource%3Drun)。你可以使用工作区在完全配置的云服务器上运行以下代码（使用 Hyperas 进行贝叶斯优化（SMBO-TPE））。
...
![](https://pic2.zhimg.com/v2-c88658f1a4049e984e26cbeea51645fd_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1062' height='451'></svg>)总结
...
下面是最常用的终止训练标准的图表：
![](https://pic2.zhimg.com/v2-941a08c134c2d0e955ba04254a36202d_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='576' height='664'></svg>)终止标准
...
你很可能听说过谷歌的 AutoML，这是他们对神经架构搜索的品牌重塑。请记住，在本文开头，我们决定将模型设计组件合并到超参数变量中。那么，神经架构搜索是 AutoML 的子领域，旨在为给定任务找到最佳模型。关于这个主题的全面讨论需要一系列文章。幸运的是，来自 fast.ai 的 Rachel Thomas 博士做了一项了不起的工作，我们很乐意提供链接：[http://www.fast.ai/2018/07/12/auto-ml-1/](https://link.zhihu.com/?target=http%3A//www.fast.ai/2018/07/12/auto-ml-1/)。
...
![](https://pic4.zhimg.com/v2-8452baa9c1e98a6b037be04c9a26d0a7_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='491'></svg>)项目主页
...
![](https://pic1.zhimg.com/v2-f278f57e3f64fb699e7e336ddf167448_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='422'></svg>)训练指标
原文链接：[https://blog.floydhub.com/guide-to-hyperparameters-search-for-deep-learning-models/](https://link.zhihu.com/?target=https%3A//blog.floydhub.com/guide-to-hyperparameters-search-for-deep-learning-models/)
