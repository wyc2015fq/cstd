# TF官方博客：基于TensorFlow.js框架的浏览器实时姿态估计 - 知乎
# 



选自TensorFlow Blog，**机器之心编译，参与：王淑婷、路。**
...
模型 Demo：[https://storage.googleapis.com/tfjs-models/demos/posenet/camera.html](https://link.zhihu.com/?target=https%3A//storage.googleapis.com/tfjs-models/demos/posenet/camera.html)
![](https://pic3.zhimg.com/v2-536b5cbd7ec810cf66ec0aa381c9b566_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='540'></svg>)PoseNet 可以利用单姿态或多姿态算法检测图像和视频中的人物，而所有这些都可以在浏览器中实现。
...
简单而言，姿态估计分两个阶段进行：
- 将 RGB 图像作为输入馈送到卷积神经网络中。
- 利用单姿态或多姿态解码算法解码来解码模型输出中的姿态、姿态置信度得分、关键点位置和关键点置信度得分。

但这些关键词是什么意思呢？我们来看几个最重要的词：
- 姿态——在最高层次上，PoseNet 将返回一个姿态目标，其中包含检测到的每个人物的关键点列表和实例级置信度得分。
![](https://pic3.zhimg.com/v2-ea63d2f52665417396dec95d2c3c1de2_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='720'></svg>)PoseNet 返回检测到的每个人和每个姿态关键点的置信度值。图像来源：《Microsoft Coco: Common Objects in Context Dataset》（https://cocodataset.org ）- 姿态置信度得分——决定姿态估计的总体置信度，范围在 0.0~1.0 之间，可用来隐藏被认为不够明显的姿态。
- 关键点——人体姿态被估计的一部分，如鼻子、右耳、左膝、右脚等。它包含位置和关键点置信度得分。PoseNet 当前可以检测 17 个关键点，如下图所示：
![](https://pic3.zhimg.com/v2-e4b9dc07692881ba24dbfaa7d209fd02_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='720'></svg>)PoseNet 检测到的 17 个姿态关键点。- 关键点置信度得分——决定估计的关键点位置准确的置信度，范围在 0.0~1.0 之间，可用来隐藏被认为不够明显的关键点。
...
**第二步（上）：单人姿态估计**
![](https://pic2.zhimg.com/v2-1345038937a134bc8ebf5717cbe01965_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='513' height='513'></svg>)应用于图像的单人姿态估计算法示例。图像来源：《Microsoft Coco: Common Objects in Context Dataset》（https://cocodataset.org）
...
- 输出步幅——必须为 32、16 或 8。默认值为 16。从内部来看，该参数影响神经网络中层的高度和宽度。在较高层次上，它会影响姿态估计的准确率和速度。输出步幅的值越低，准确率越高，但速度越慢；输出步幅值越高，速度越快，但准确率越低。查看输出步幅对输出质量影响的最佳方法是使用单姿态估计算法 demo：[https://storage.googleapis.com/tfjs-models/demos/posenet/camera.html](https://link.zhihu.com/?target=https%3A//storage.googleapis.com/tfjs-models/demos/posenet/camera.html)。
...
**第二步（下）：多人姿态估计**
![](https://pic1.zhimg.com/v2-c986c78fbfd97d012fbeb9f92eb90e3c_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='513' height='513'></svg>)应用于图像的多人姿态估计算法示例。图像来源：「Microsoft Coco: Common Objects in Context Dataset」（https://cocodataset.org）
...
查看这些参数影响的最佳方法是使用多姿态估计 demo（[https://storage.googleapis.com/tfjs-models/demos/posenet/camera.html](https://link.zhihu.com/?target=https%3A//storage.googleapis.com/tfjs-models/demos/posenet/camera.html)）。
...
![](https://pic3.zhimg.com/v2-6d786430914a03302b517805e64eee62_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='720'></svg>)使用 PoseNet 的单人姿态检测器流程
...
![](https://pic3.zhimg.com/v2-d9002940309f28fa002611ff4d7445b6_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='720'></svg>)输出步幅决定了输出比输入图像缩小的程度。输出步幅越高，速度越快，但准确率越低。
...
![](https://pic3.zhimg.com/v2-c7ed42594bc55335f2d0efc52e83d0c2_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='1080' height='720'></svg>)PoseNet 返回的 17 个姿态关键点中每一个都与用于确定关键点确切位置的热图张量和偏移向量张量相关联。
