# 单级式目标检测方法概述：YOLO与SSD - 知乎
# 



**选自[http://jeremyjordan.me](https://link.zhihu.com/?target=http%3A//jeremyjordan.me)，作者：Jeremy Jordan，机器之心编译，参与：Panda。**
...
![](https://pic4.zhimg.com/v2-dc9b6ff497756c975144cd1b961f2cd3_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='900' height='295'></svg>)目标检测示例，来自 PASCAL VOC 数据集：http://host.robots.ox.ac.uk/pascal/VOC/
...
![](https://pic2.zhimg.com/v2-1443711b79d0f8c2e697b59c46701571_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='344'></svg>)
...
![](https://pic2.zhimg.com/v2-300f51c76613daad43625ffd85766c21_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='422'></svg>)
我们将这个 7x7 网格关联回原输入，以了解每个网格单元相对于原图像所代表的内容。
![](https://pic3.zhimg.com/v2-8f969ccfdc7fbe24f83bfca1656906ee_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='403'></svg>)
通过观察哪个网格单元包含我们的边界框标注的中心，我们还可以在粗略的（7x7）特征图中大致确定目标的位置。我们将把该网格单元指定为「负责」检测该特定目标的单元。
![](https://pic2.zhimg.com/v2-a755e7c78fa669478c5d6b053964a7b1_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='353'></svg>)
为了检测该目标，我们将添加另外一个卷积层并学习结合了所有 512 个特征图背景的核参数，以得到一个对应于包含目标的网格单元的激活。
![](https://pic4.zhimg.com/v2-72283ba0e30509ff6d8ca2ae854b4a3f_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='411'></svg>)
如果输入图像包含多个目标，那么在我们的网格上应该有多个激活，表示每个激活区域中都有一个目标。
![](https://pic2.zhimg.com/v2-c2434e2165631e43d9f97546794a00f9_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='218'></svg>)
...
![](https://pic3.zhimg.com/v2-e9eee974d7bce599c9470d02bb8790f6_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='302'></svg>)
为了清楚说明，下面展示了应用 5+C 个卷积过滤器的完整输出，为每个网格单元都得到一个边界框描述量。
![](https://pic2.zhimg.com/v2-08da86b8fb304dc06a4256936ae4a3a1_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='535'></svg>)
但是，某些图像可能有多个目标都「属于」同一网格单元。我们可以修改我们的层以得到 B(5+C) 个过滤器，这样我们就能为每个网格单元位置预测 B 个边界框了。
![](https://pic3.zhimg.com/v2-dd5fa900b70dfd41145f83893d8a6de6_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='250'></svg>)
可视化我们的 B(5+C) 过滤器的完整卷积输出，我们可以看到我们的模型总是能为给定图像得到固定数量的 N×N×B 预测。然后我们可以过滤我们的预测，仅考虑 pobj 超过某个定义的阈值的边界框。
![](https://pic4.zhimg.com/v2-50b13c4f5e3322a07531cfe7838dd5bf_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='459'></svg>)
由于我们的检测过程的卷积本质，多个目标可以并行的方式被检测到。但是，我们最终还会预测到没有找到任何目标的大量网格单元。尽管我们可以通过这些边界框的 pobj 分数滤除它们，但这会在预测得到的包含目标和不包含目标的边界框之间引入相当大的不平衡。
![](https://pic3.zhimg.com/v2-2e908c09bc6134e0997b7ba0fd53db26_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='462'></svg>)
...
![](https://pic3.zhimg.com/v2-f15fd499a2a0e7a762ae272e559f1626_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='399'></svg>)
...
![](https://pic2.zhimg.com/v2-77bc0422f370abf9eac6305835d56235_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='407'></svg>)注：尽管图中没有给出，但我们的预测网格中的每个单元都有这些锚框
我们不再直接预测边界框的尺寸，而是重新形式化了我们的任务，从而只需简单预测与我们的边界框先验尺寸的偏移量，这样我们就可以优化我们的预测边界框尺寸了。这种处理方式能让这一预测任务更容易学习。
![](https://pic1.zhimg.com/v2-1777484f66957a1683a2fd8c9f81e000_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='451'></svg>)
...
![](https://pic3.zhimg.com/v2-8ac3c31da18013923d3732572aa4790e_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='337'></svg>)我不喜欢这个奇怪的「从更高分辨率特征图的 skip 连接」思想
幸运的是，在第三个迭代版本中，这被改成了更标准的特征金字塔网络输出结构。使用这一方法，我们将在输出一个预测结果和上采样特征图（使用 skip 连接）之间交替。这允许预测能够利用网络更早期的更细粒度信息，这有助于检测图像中的小目标。
![](https://pic4.zhimg.com/v2-03bfca1d56c1b4cb767fcbf1587dc90b_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='333'></svg>)
...
![](https://pic1.zhimg.com/v2-7f5fc1cc4273c3a2304e19f2cc38f968_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='311'></svg>)
...
![](https://pic4.zhimg.com/v2-1bfac254c06ff720dba6357f401c3117_b.jpg)![](data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='793' height='477'></svg>)
...
- PASCAL VOC 2012 检测竞赛：[http://host.robots.ox.ac.uk/pascal/VOC/voc2012/index.html](https://link.zhihu.com/?target=http%3A//host.robots.ox.ac.uk/pascal/VOC/voc2012/index.html)
- COCO 2018 目标检测任务：[http://cocodataset.org/#detection-2018](https://link.zhihu.com/?target=http%3A//cocodataset.org/%23detection-2018)
- ImageNet 目标检测挑战赛：[https://www.kaggle.com/c/imagenet-object-detection-challenge](https://link.zhihu.com/?target=https%3A//www.kaggle.com/c/imagenet-object-detection-challenge)
- Google AI Open Images——目标检测：[https://www.kaggle.com/c/google-ai-open-images-object-detection-track](https://link.zhihu.com/?target=https%3A//www.kaggle.com/c/google-ai-open-images-object-detection-track)
- 视觉遇上无人机挑战：[http://www.aiskyeye.com/views/index](https://link.zhihu.com/?target=http%3A//www.aiskyeye.com/views/index)
...
- YOLOv3：[https://arxiv.org/abs/1804.02767](https://link.zhihu.com/?target=https%3A//arxiv.org/abs/1804.02767)
- SSD：[https://arxiv.org/abs/1512.02325](https://link.zhihu.com/?target=https%3A//arxiv.org/abs/1512.02325)
- DSSD：[https://arxiv.org/abs/1701.06659](https://link.zhihu.com/?target=https%3A//arxiv.org/abs/1701.06659)（本文中没有讨论，但值得一读）
- 用于密集目标检测的焦点损失：[https://arxiv.org/abs/1708.02002](https://link.zhihu.com/?target=https%3A//arxiv.org/abs/1708.02002)
- 卷积神经网络的有趣失败和 CoordConv 解决方案：[https://arxiv.org/abs/1807.03247](https://link.zhihu.com/?target=https%3A//arxiv.org/abs/1807.03247)（查看有关目标检测的相关章节）以及相关视频：[https://www.youtube.com/watch?v=8yFQc6elePA](https://link.zhihu.com/?target=https%3A//www.youtube.com/watch%3Fv%3D8yFQc6elePA)




**讲座**
- Stanford CS 231n：第 11 讲 | 检测和分割：[https://www.youtube.com/watch?v=nDPWywWRIRo&t=1967s](https://link.zhihu.com/?target=https%3A//www.youtube.com/watch%3Fv%3DnDPWywWRIRo%26t%3D1967s)




**标注数据的工具**
- CVAT：[https://github.com/opencv/cvat](https://link.zhihu.com/?target=https%3A//github.com/opencv/cvat)




原文链接：[https://www.jeremyjordan.me/object-detection-one-stage/](https://link.zhihu.com/?target=https%3A//www.jeremyjordan.me/object-detection-one-stage/)
