...
![](https://pic4.zhimg.com/v2-88a21459841156bc975958719378e1bf_b.jpg)
...
![](https://pic3.zhimg.com/v2-c0414641f482935dbed32f197aee705e_b.jpg)
...
![](https://pic3.zhimg.com/v2-006c90ac1d91e03792036c5a5c6c28ce_b.jpg)
这个划分方案对应的决策树如下图所示：
![](https://pic2.zhimg.com/v2-cf405f0dd902087e15badb92d9ecdd71_b.jpg)
以上图像来自SIGAI的云端实验室。
...
![](https://pic4.zhimg.com/v2-ba92be1ac213570facb92ec70b5d53a3_b.jpg)
其中Ni是第i类样本数，N为总样本数。根据这个概率值可以定义各种不纯度指标，下面分别介绍。

样本集的熵不纯度定义为：
![](https://pic4.zhimg.com/v2-5000dbbe07e68aa7028047eb4933521b_b.jpg)
熵是信息论中的一个重要概念，用来度量一组数据包含的信息量大小。当样本只属于某一类时熵最小，当样本均匀的分布于所有类中时熵最大。因此，如果能找到一个分裂让熵最小，这就是我们想要的最佳分裂。

样本集的Gini不纯度定义为：
![](https://pic2.zhimg.com/v2-b2189090a0663390a6101cfdb316493d_b.jpg)
当样本属于某一类时Gini不纯度的值最小，此时最小值为0；当样本均匀的分布与每一类时Gini不纯度的值最大。这源自于如下的数学结论，在下面的约束条件下：
![](https://pic3.zhimg.com/v2-e66270155f3f5b4fb0e7c7cbc0dc2c2e_b.jpg)
对于如下目标函数：
![](https://pic3.zhimg.com/v2-54186d30c1a4236ab76cbe87740e3ff2_b.jpg)
...
![](https://pic4.zhimg.com/v2-b78be12f84d60a37321ec622b956ccbf_b.jpg)
样本集的误分类不纯度定义为：
![](https://pic4.zhimg.com/v2-63958ea89510a6820a2120adc3845843_b.jpg)
...
![](https://pic2.zhimg.com/v2-285460ec58771b523765a81a4a8c9651_b.jpg)
其中G(Dl)是左子集的不纯度，G(Dr)是右子集的不纯度，N是总样本数，Nl是左子集的样本数，Nr是右子集的样本数。

如果采用Gini不纯度指标，将Gini不纯度的计算公式代入上式可以得到：
![](https://pic2.zhimg.com/v2-682f3bce1aa1d9b14b100306de5d0e21_b.jpg)
其中Nl,i是左子节点中第i类样本数；Nr,i是右子节点中第i类样本数。由于N是一个固定值，要让Gini不纯度最小化等价于让下面的值最大化：
![](https://pic4.zhimg.com/v2-fae7edafbdfc1cf17216c7aef2bf4993_b.jpg)
...
![](https://pic3.zhimg.com/v2-bc414774d771ecc9fca912de98d7e0be_b.jpg)
...
![](https://pic1.zhimg.com/v2-0204f076e6b33e8e39f7d3d46da48530_b.jpg)
...
![](https://pic4.zhimg.com/v2-3ebd662e813ac197d52d4b8dc9306ea7_b.jpg)
把均值的定义带入上式，得到：
![](https://pic3.zhimg.com/v2-5a67e1b8bdaa73b5d445dd1e6918c5fa_b.jpg)
根据样本集的回归误差，我们同样可以构造出分裂的回归误差。分裂的目标是最大程度的减小回归误差，因此把分裂的误差指标定义为分裂之前的回归误差减去分裂之后左右子树的回归误差：
![](https://pic2.zhimg.com/v2-282a6cbf2157760079f581e409674ab9_b.jpg)
将误差的计算公式代入上式，可以得到：
![](https://pic2.zhimg.com/v2-81ff2909ba29d13c9109ce532ea96711_b.jpg)
由于N和 ![-\frac{1}{N^{2}}(\sum_{i=1}^{N}{y_{2}})^{2}](https://www.zhihu.com/equation?tex=-%5Cfrac%7B1%7D%7BN%5E%7B2%7D%7D%28%5Csum_%7Bi%3D1%7D%5E%7BN%7D%7By_%7B2%7D%7D%29%5E%7B2%7D) 是常数，要让上式最大化等价于让下式最大化：
![](https://pic2.zhimg.com/v2-02d8079776632f3503fb838cb20e4c11_b.jpg)
...
![](https://pic2.zhimg.com/v2-7e77e756c2497fce0e4c850521f12709_b.jpg)
其中E(n)是节点n的错误率，E(nt)是以节点n为根的子树的错误率。 ![|n_{t}|](https://www.zhihu.com/equation?tex=%7Cn_%7Bt%7D%7C) 为子树的叶子节点数，即复杂度。a值是用树的复杂度归一化之后的错误率增加值，即将整个子树剪掉之后用一个叶子节点替代，相对于原来的子树错误率的增加值。该值越小，剪枝之后树的分类效果和剪枝之前越接近。上面的定义依赖于节点的错误率指标，下面对分类问题和回归问题介绍它的计算公式。对于分类问题，错误率定义为：
![](https://pic2.zhimg.com/v2-f7f43deb85712eae75aa04b68223a14d_b.jpg)
其中N是节点的总样本数，Ni是第i类样本数，这就是之前定义的误分类指标。对于回归问题，错误率为节点样本集的均方误差：
![](https://pic2.zhimg.com/v2-ba713756323b316954b286777ad516f9_b.jpg)
子树的错误率为树的所叶子节点错误率之和。计算出a值之后，剪掉该值最小的节点得到剪枝后的树，然后重复这种操作直到剩下根节点，由此得到一个决策树序列：
![](https://pic1.zhimg.com/v2-abb2071f200c042981e1063ba4499418_b.jpg)
...
![](https://pic3.zhimg.com/v2-229db99fe352856c901ace86872791d2_b.gif)
