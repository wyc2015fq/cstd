...
## **PDF全文链接：**[NAS（神经结构搜索综述）](https://link.zhihu.com/?target=http%3A//www.tensorinfinity.com/paper_136.html)
...
![](https://pic2.zhimg.com/v2-261f4e89d5c60e5d336052e7fc6d116d_b.png)
...
![](https://pic3.zhimg.com/v2-0e574807c1ef41d73e80d67eb6dbe5d2_b.png)
...
![](https://pic1.zhimg.com/v2-926c10c9ca9f9a5feff26ad0cd1b10f8_b.png)
...
![](https://pic2.zhimg.com/v2-b4040b08e683800e815365184d6f6025_b.png)
...
![](https://pic4.zhimg.com/v2-6d1a397e485909024f33a6989cbd1ccb_b.png)
...
![L(\theta )=E_{\Delta \sim p_{\theta (\cdot )}}\left [ R(\Delta ) \right ]](https://www.zhihu.com/equation?tex=L%28%5Ctheta+%29%3DE_%7B%5CDelta+%5Csim+p_%7B%5Ctheta+%28%5Ccdot+%29%7D%7D%5Cleft+%5B+R%28%5CDelta+%29+%5Cright+%5D)

其中 ![\theta ](https://www.zhihu.com/equation?tex=%5Ctheta+) 是控制器的参数， ![\Delta ](https://www.zhihu.com/equation?tex=%5CDelta+) 是子网络， ![P_{\theta }(\Delta )](https://www.zhihu.com/equation?tex=P_%7B%5Ctheta+%7D%28%5CDelta+%29) 是控制器输出的子网络所服从的概率分布， ![R(\Delta )](https://www.zhihu.com/equation?tex=R%28%5CDelta+%29) 是子网络在验证集上的精度值。直观的目标是某种结构的子网络准确率越高，则控制器生成该网络结构的概率越大。因此可以按照下式计算控制器的参数

![\bigtriangledown _{\theta }L(\theta )= \sum_{1}^{T}E_{p(a_{1:T};\theta )}\left [ \bigtriangledown _{\theta }\ln p(a_{t}\mid a_{t-1:1};\theta ) R\right ]](https://www.zhihu.com/equation?tex=%5Cbigtriangledown+_%7B%5Ctheta+%7DL%28%5Ctheta+%29%3D+%5Csum_%7B1%7D%5E%7BT%7DE_%7Bp%28a_%7B1%3AT%7D%3B%5Ctheta+%29%7D%5Cleft+%5B+%5Cbigtriangledown+_%7B%5Ctheta+%7D%5Cln+p%28a_%7Bt%7D%5Cmid+a_%7Bt-1%3A1%7D%3B%5Ctheta+%29+R%5Cright+%5D)

其中R为子网络的准确率，p为生成该子网络结构的概率。实现时使用采样来近似数学期望值

![\frac{1}{m}\sum_{k=1}^{m}\sum_{t=1}^{T}\bigtriangledown _{\theta }\ln p(a_{t}\mid a_{t-1:1};\theta )R_{k}](https://www.zhihu.com/equation?tex=%5Cfrac%7B1%7D%7Bm%7D%5Csum_%7Bk%3D1%7D%5E%7Bm%7D%5Csum_%7Bt%3D1%7D%5E%7BT%7D%5Cbigtriangledown+_%7B%5Ctheta+%7D%5Cln+p%28a_%7Bt%7D%5Cmid+a_%7Bt-1%3A1%7D%3B%5Ctheta+%29R_%7Bk%7D)

其中m为mini-batch的样本数，T为神经网络的层数。为了解决REINFORCE算法计算出的梯度值偏差问题，在计算梯度时减掉了均值b

![\frac{1}{m}\sum_{k=1}^{m}\sum_{t=1}^{T}\bigtriangledown _{\theta }\ln p(a_{t}\mid a_{t-1:1};\theta )(R_{k}-b)](https://www.zhihu.com/equation?tex=%5Cfrac%7B1%7D%7Bm%7D%5Csum_%7Bk%3D1%7D%5E%7Bm%7D%5Csum_%7Bt%3D1%7D%5E%7BT%7D%5Cbigtriangledown+_%7B%5Ctheta+%7D%5Cln+p%28a_%7Bt%7D%5Cmid+a_%7Bt-1%3A1%7D%3B%5Ctheta+%29%28R_%7Bk%7D-b%29)
...
![](https://pic3.zhimg.com/v2-3f15a91ff3431a49e87c4b515e326d46_b.png)
...
![](https://pic1.zhimg.com/v2-cb5e1634ec0e10ff10b3c64801f39a1c_b.png)
...
![](https://pic4.zhimg.com/v2-57e5e32c98acc4675afc6c45a79584d7_b.png)

下图是典型的普通块，这里B=4。

![](https://pic1.zhimg.com/v2-6c40aa28e50cf2d1fccf015c6a70bbfc_b.png)

下图是典型的约简块，同样的B=4，这里将4个临时结果拼接，形成 ![h_{t}](https://www.zhihu.com/equation?tex=h_%7Bt%7D) 作为本单元的输出值。

![](https://pic1.zhimg.com/v2-9457ff0a3ac4a0d9f262616f6d7ece08_b.png)
...
![](https://pic2.zhimg.com/v2-fb009a576bba7f77afd1b012672892a9_b.png)
...
![](https://pic3.zhimg.com/v2-d937276289ee331c8315dab46fac8e16_b.png)
该图有4个顶点，红色的边表示信息的流动，黑色的边无效即没有使用。右图为对应的循环神经网络单元，运算节点的编号与左图中图的顶点编号对应。节点1接收 ![x_{t}](https://www.zhihu.com/equation?tex=x_%7Bt%7D) 和 ![h_{t-1}](https://www.zhihu.com/equation?tex=h_%7Bt-1%7D) 作为输入，执行下面的运算

![h_{1}=tanh(x_{t}\cdot W^{(x)}+h_{t-1}\cdot W_{1}^{(h)})](https://www.zhihu.com/equation?tex=h_%7B1%7D%3Dtanh%28x_%7Bt%7D%5Ccdot+W%5E%7B%28x%29%7D%2Bh_%7Bt-1%7D%5Ccdot+W_%7B1%7D%5E%7B%28h%29%7D%29)

这里的激活函数选用tanh，权重矩阵为本节点的参数。节点2以节点1的输出值为输入，选择ReLU作为激活函数，执行下面的运算

![h_{2}=ReLU(h_{1}\cdot W_{2,1}^{(h)})](https://www.zhihu.com/equation?tex=h_%7B2%7D%3DReLU%28h_%7B1%7D%5Ccdot+W_%7B2%2C1%7D%5E%7B%28h%29%7D%29)

该节点同样有权重矩阵。节点3以节点2的输出值为输入，选择ReLU作为激活函数，执行下面的运算

![h_{3}=ReLU(h_{2}\cdot W_{3,2}^{(h)})](https://www.zhihu.com/equation?tex=h_%7B3%7D%3DReLU%28h_%7B2%7D%5Ccdot+W_%7B3%2C2%7D%5E%7B%28h%29%7D%29)

节点4以节点1的输出值作为输入，选择tanh作为激活函数，执行下面运算

![h_{4}=ReLU(h_{1}\cdot W_{4,1}^{(h)})](https://www.zhihu.com/equation?tex=h_%7B4%7D%3DReLU%28h_%7B1%7D%5Ccdot+W_%7B4%2C1%7D%5E%7B%28h%29%7D%29)

节点3和4没有后续节点，因此根据它们计算输出值。输出值为它们的均值

![h_{t}=(h_{3}+h_{4})/2](https://www.zhihu.com/equation?tex=h_%7Bt%7D%3D%28h_%7B3%7D%2Bh_%7B4%7D%29%2F2)

对于每个 ![j< l](https://www.zhihu.com/equation?tex=j%3C+l) 的节点对，都有一个独立的权重矩阵 ![W_{l,j}^{(h)}](https://www.zhihu.com/equation?tex=W_%7Bl%2Cj%7D%5E%7B%28h%29%7D) ，为每个节点l一旦确定其前驱节点j，则使用该矩阵。在ENAS中，所有循环单元共用一组相同的权重参数。

下面介绍控制器如何生成该网络结构。控制器在每次预测时需要做两个决策：确定以哪个节点的输出值作为输入即作为当前节点的前驱，为当前节点选用哪种激活函数。对于第1个节点，输入值是确定的，为 ![x_{t}](https://www.zhihu.com/equation?tex=x_%7Bt%7D) 和 ![h_{t-1}](https://www.zhihu.com/equation?tex=h_%7Bt-1%7D) ，控制器只用为其选择激活函数。接下来生成节点2，首先选择一个节点作为输入即作为节点2的前驱节点，然后为节点2选择激活函数。其他节点依次类推，这一过程如下图所示。

![](https://pic4.zhimg.com/v2-b0153571a7b252d0ebf1ca80445ee8d3_b.png)

假设循环神经网络的单元有N个节点，在生成第i个节点时，可以选择的前驱节点为[0,i-1]之间的整数，有i种情况，因此网络的拓扑结构有N!种。对于每种网络拓扑，每个节点的激活函数有4种选择，分别为tanh，sigmoid，identity，ReLU。因此可以搜索的网络结构总共有 ![4^{N}\times N!](https://www.zhihu.com/equation?tex=4%5E%7BN%7D%5Ctimes+N%21) 种情况。

下面介绍卷积神经网络的生成方式。下图给4个节点的卷积神经网络，同样的，红色的边表示有效边，黑色的未激活。这个图对应的网络结构如右图所示。

![](https://pic2.zhimg.com/v2-25b74be6615d7c472de50487c0729a01_b.png)
...
对于第k层，小于等于k-1的不同层都可以用来作为它的输入，因此有 ![2^{k-1}](https://www.zhihu.com/equation?tex=2%5E%7Bk-1%7D) 种连接关系。对于上图中的卷积神经网络，在k=4时选择{1,3}作为它的前驱，导致第1、3个层都与第4个层连接。这种做法可以形成任意的跨层连接。

在每个节点处允许的运算有6种情况，分别是： ![3\times 3](https://www.zhihu.com/equation?tex=3%5Ctimes+3) 卷积， ![5\times 5](https://www.zhihu.com/equation?tex=5%5Ctimes+5) 卷积， ![3\times 3](https://www.zhihu.com/equation?tex=3%5Ctimes+3) 深度可分离卷积， ![5\times 5](https://www.zhihu.com/equation?tex=5%5Ctimes+5) 深度可分离卷积， ![3\times 3](https://www.zhihu.com/equation?tex=3%5Ctimes+3) 均值池化， ![3\times 3](https://www.zhihu.com/equation?tex=3%5Ctimes+3) 最大值池化。与循环神经网络相同，每个节点处都有所有运算的参数，并被所有网络结构共享。生成卷积神经网络结构的过程如下图所示。

![](https://pic1.zhimg.com/v2-1b54654c641c486000a35ab5a1eb4abc_b.png)

如果一个卷积神经网络有L个层，则在第k层处有 ![2^{k-1}](https://www.zhihu.com/equation?tex=2%5E%7Bk-1%7D) 种连接关系，由于各个层之间的连接关系是单独确定即相互独立的，因此网络的连接关系及拓扑结构有

![2^{1+2+...+L-1}=2^{L(L-1)/2}](https://www.zhihu.com/equation?tex=2%5E%7B1%2B2%2B...%2BL-1%7D%3D2%5E%7BL%28L-1%29%2F2%7D)

种情况，对于每种网络结构，在每个层有6种可供选择的运算，因此所有可能的网络结构有

![6^{L}\times 2^{L(L-1)/2}](https://www.zhihu.com/equation?tex=6%5E%7BL%7D%5Ctimes+2%5E%7BL%28L-1%29%2F2%7D)

种情况。如果L=12，则所有可能的网络结构数为 ![1.6\times 10^{29}](https://www.zhihu.com/equation?tex=1.6%5Ctimes+10%5E%7B29%7D)。

除了生成整个卷积网络，还可以生成卷积网络额单元然后将其堆叠形成完整的网络，具体做法与NASNet类似，不再重复介绍。

生成网络结构之后，接下来的核心任务是训练子网络和控制器网络。假设控制器网络的参数为 ![\theta](https://www.zhihu.com/equation?tex=%5Ctheta) ，子网络的参数为W，后者被所有子网络共享。这两组参数交替训练，在每次迭代时分两个阶段，首先训练W，然后训练 ![\theta](https://www.zhihu.com/equation?tex=%5Ctheta) 。

第一阶段先固定住控制器的参数，控制器的输出策略为 ![\pi (m;\theta )](https://www.zhihu.com/equation?tex=%5Cpi+%28m%3B%5Ctheta+%29)，从中采样出网络结构。以交叉熵作为损失函数，计算损失函数对w的梯度并更新。第二阶段固定住w，用 REINFORCE算法更新控制器的参数 ![\theta](https://www.zhihu.com/equation?tex=%5Ctheta) ，最大化奖励值的数学期望

![E_{m-\pi (m;\theta )}\left [ R(m,w) \right ]](https://www.zhihu.com/equation?tex=E_%7Bm-%5Cpi+%28m%3B%5Ctheta+%29%7D%5Cleft+%5B+R%28m%2Cw%29+%5Cright+%5D)

奖励值 ![R(m,w)](https://www.zhihu.com/equation?tex=R%28m%2Cw%29) 为子网络m在验证集上的精度值。训练完成之后，根据控制器的策略 ![\pi (m;\theta )](https://www.zhihu.com/equation?tex=%5Cpi+%28m%3B%5Ctheta+%29) 采样出多个子网络并训练，计算它们在验证集上的表现，选择表现最好的网络作为最终生成的网络。
...
假设整个神经网络共有S级，级的编号为s=1,...,S 。第S级有 ![k_{S}](https://www.zhihu.com/equation?tex=k_%7BS%7D) 个节点，这些节点的编号为 ![k_{s}=1,...,K_{s}](https://www.zhihu.com/equation?tex=k_%7Bs%7D%3D1%2C...%2CK_%7Bs%7D) 。第s级的第 ![k_{s}](https://www.zhihu.com/equation?tex=k_%7Bs%7D) 个节点为  ![v_{s,k_{s}}](https://www.zhihu.com/equation?tex=v_%7Bs%2Ck_%7Bs%7D%7D) 。这些节点是有序的，只允许数据从低编号的节点流向高编号的节点以保证生成的是有向无环图。每一级编码的比特数为

![1+2+...+(K_{s}-1)=\frac{1}{2}K_{s}(K_{s}-1)](https://www.zhihu.com/equation?tex=1%2B2%2B...%2B%28K_%7Bs%7D-1%29%3D%5Cfrac%7B1%7D%7B2%7DK_%7Bs%7D%28K_%7Bs%7D-1%29)

节点 ![v_{s,k_{s}}](https://www.zhihu.com/equation?tex=v_%7Bs%2Ck_%7Bs%7D%7D) 与 ![v_{s,1},...v_{s,k_{s-1}}](https://www.zhihu.com/equation?tex=v_%7Bs%2C1%7D%2C...v_%7Bs%2Ck_%7Bs-1%7D%7D) 之间都可能有边连接，因此对于它需要 ![k_{s-1}](https://www.zhihu.com/equation?tex=k_%7Bs-1%7D) 个比特位。第1个节点不需要编码，因为没有编号更小的节点连接它；第2个节点 ![v_{s,2}](https://www.zhihu.com/equation?tex=v_%7Bs%2C2%7D) 可能有连接 ![(v_{s,1},v_{s,2})](https://www.zhihu.com/equation?tex=%28v_%7Bs%2C1%7D%2Cv_%7Bs%2C2%7D%29) ；第3个节点可能有连接 ![(v_{s,1},v_{s,3})](https://www.zhihu.com/equation?tex=%28v_%7Bs%2C1%7D%2Cv_%7Bs%2C3%7D%29) ，；其他以此类推。如果节点之间有边连接，则编码为1；否则为0。对于一个S级的网络，总编码位数即长度为

![L= \sum_{s=1}^{S}K_{s}(K_{s}-1)](https://www.zhihu.com/equation?tex=L%3D+%5Csum_%7Bs%3D1%7D%5E%7BS%7DK_%7Bs%7D%28K_%7Bs%7D-1%29)

下图是一个2级网络的编码结果。第1级有4个节点，第2级有5个节点。需要注意的是为了保证数据的流入和流出，有特定的节点（红色和绿色）充当输入与输出节点。

![](https://pic2.zhimg.com/v2-8156450f388b16ab16ba4e629410ade1_b.png)
...
对于编码长度为L的网络，所有可能的网络结构数为 ![2^{L}](https://www.zhihu.com/equation?tex=2%5E%7BL%7D) 。如果一个网络有3级即S=3，各级节点数为 ![(K_{1},K_{2},K_{3})](https://www.zhihu.com/equation?tex=%28K_%7B1%7D%2CK_%7B2%7D%2CK_%7B3%7D%29) 。则![L=19，2^{L}=524288](https://www.zhihu.com/equation?tex=L%3D19%EF%BC%8C2%5E%7BL%7D%3D524288) ，这个数据非常大，对于更深的网络，搜索空间就更大。

这种编码方式可以表示各种典型的网络拓扑结构，包括VGGNet，ResNet，DenseNet。这些网络的编码如下图所示。对于有些特殊的网络，如GoogLeNet的Inception模块，通过将基本块进行扩充，也可以支持。

![](https://pic2.zhimg.com/v2-87ee2161275e8465761a5b52767ceba1_b.png)
...
输入：参考数据集D，迭代次数T，每次迭代时的个体数N，变异概率 ![P_{M}](https://www.zhihu.com/equation?tex=P_%7BM%7D) ，交叉概率 ![P_{C}](https://www.zhihu.com/equation?tex=P_%7BC%7D) ，变异参数![q_{M}](https://www.zhihu.com/equation?tex=q_%7BM%7D) ，交叉参数 ![q_{C}](https://www.zhihu.com/equation?tex=q_%7BC%7D)

初始化：随机生成个体

![\left \{ {M_{0,n}} \right \}_{n=1}^{N}](https://www.zhihu.com/equation?tex=%5Cleft+%5C%7B+%7BM_%7B0%2Cn%7D%7D+%5Cright+%5C%7D_%7Bn%3D1%7D%5E%7BN%7D)
...
![\left \{ M_{t-1,n} \right \}_{n=1}^{N}](https://www.zhihu.com/equation?tex=%5Cleft+%5C%7B+M_%7Bt-1%2Cn%7D+%5Cright+%5C%7D_%7Bn%3D1%7D%5E%7BN%7D)
...
![\left \{ M_{t,2n-1},M_{t,2n} \right \}_{n=1}^{N/2}](https://www.zhihu.com/equation?tex=%5Cleft+%5C%7B+M_%7Bt%2C2n-1%7D%2CM_%7Bt%2C2n%7D+%5Cright+%5C%7D_%7Bn%3D1%7D%5E%7BN%2F2%7D)

用概率 ![P_{C}](https://www.zhihu.com/equation?tex=P_%7BC%7D)和参数 ![q_{C}](https://www.zhihu.com/equation?tex=q_%7BC%7D) 执行交叉

变异：对所有非交叉个体

![\left \{ M_{t,n} \right \}_{n=1}^{N}](https://www.zhihu.com/equation?tex=%5Cleft+%5C%7B+M_%7Bt%2Cn%7D+%5Cright+%5C%7D_%7Bn%3D1%7D%5E%7BN%7D)
...
首先为网络单元定义有向无环图。假设有向无环图有N个顶点，顶点 ![x_{i}](https://www.zhihu.com/equation?tex=x_%7Bi%7D) 为数据的潜在表示，如卷积神经网络的特征图像。图的边(i,j)为某种运算 ![o_{i,j}](https://www.zhihu.com/equation?tex=o_%7Bi%2Cj%7D) ，对 ![x_{i}](https://www.zhihu.com/equation?tex=x_%7Bi%7D) 进行变换，然后将变换结构送入顶点 ![x_{j}](https://www.zhihu.com/equation?tex=x_%7Bj%7D) 。典型的运算有卷积、池化、以及零运算，零运算在后面介绍。每个单元有2个输入节点，1个输出节点。对于卷积单元，输入节点为上两层的输出值；对于循环神经网络，输入节点为当前时刻的输入值以及上一时刻的状态值。因此卷积神经网络和循环神经网络可以用这种图统一表示。输出节点的值根据对所有中间节点进行约简运算（reduction operation，如concatenation）而得到。每个中间节点的值根据其所有前驱节点（编号更小的节点）计算得到，计算公式为

![x_{i}= \sum_{j< i}o_{(j,i)}(x_{j})](https://www.zhihu.com/equation?tex=x_%7Bi%7D%3D+%5Csum_%7Bj%3C+i%7Do_%7B%28j%2Ci%29%7D%28x_%7Bj%7D%29)

零运算是一种特殊的运算，它表示两个节点之间没有关系，即节点 ![x_{i}](https://www.zhihu.com/equation?tex=x_%7Bi%7D) 的值不会用来计算 ![x_{j}](https://www.zhihu.com/equation?tex=x_%7Bj%7D) 的值。下图是一个有向无环图表示。

![](https://pic1.zhimg.com/v2-ce4ed200115669dcd53a3c3a63be6c8c_b.png)

接下来对问题进行松弛，转化为连续优化问题。对于某一网络结构，顶点 ![x_{i}](https://www.zhihu.com/equation?tex=x_%7Bi%7D) 到顶点 ![x_{j}](https://www.zhihu.com/equation?tex=x_%7Bj%7D) 的运算是确定的。在这里将其概率化，即表示为各种运算的概率叠加。假设O为所有候选运算的集合， ![o(\cdot )](https://www.zhihu.com/equation?tex=o%28%5Ccdot+%29) 为作用于![x_{i}](https://www.zhihu.com/equation?tex=x_%7Bi%7D) 的某种运算。则节点![x_{j}](https://www.zhihu.com/equation?tex=x_%7Bj%7D) 的值是各种候选运算的结果的概率叠加，而使用每种运算的概率用softmax回归表示。计算公式为

![\bar{o}_{(i,j)}(x)=\sum_{o\in O}\frac{exp(\alpha _{o,(i,j)})}{\sum_{o\in O}exp(\alpha _{o,(i,j)})}o(x)](https://www.zhihu.com/equation?tex=%5Cbar%7Bo%7D_%7B%28i%2Cj%29%7D%28x%29%3D%5Csum_%7Bo%5Cin+O%7D%5Cfrac%7Bexp%28%5Calpha+_%7Bo%2C%28i%2Cj%29%7D%29%7D%7B%5Csum_%7Bo%5Cin+O%7Dexp%28%5Calpha+_%7Bo%2C%28i%2Cj%29%7D%29%7Do%28x%29)

其中 ![\alpha ](https://www.zhihu.com/equation?tex=%5Calpha+) 为系数值，用于计算使用每种运算的概率值。如果定义向量

![\alpha =[\alpha _{1(i,j)}...\alpha _{\left | O \right |,(i,j)}]](https://www.zhihu.com/equation?tex=%5Calpha+%3D%5B%5Calpha+_%7B1%28i%2Cj%29%7D...%5Calpha+_%7B%5Cleft+%7C+O+%5Cright+%7C%2C%28i%2Cj%29%7D%5D)

则网络结构搜索问题可以转换为求解这些向量。得到这些向量的值之后即可确定网络解结构，在每个顶点处选择概率最大的运算作为该节点的运算

![O_{i,j}=arg max_{o\in O}\alpha _{o,(i,j)}](https://www.zhihu.com/equation?tex=O_%7Bi%2Cj%7D%3Darg+max_%7Bo%5Cin+O%7D%5Calpha+_%7Bo%2C%28i%2Cj%29%7D)

这些向量可以看做是网络结构的编码表示。在将问题进行松弛之后，接下来的目标是同时求解网络结构 ![\alpha ](https://www.zhihu.com/equation?tex=%5Calpha+) 以及所有混合运算所对应的权重W。算法的目标是最优化验证集上的损失，采用梯度下降法。

假设 ![L_{train}](https://www.zhihu.com/equation?tex=L_%7Btrain%7D) 与 ![L_{val}](https://www.zhihu.com/equation?tex=L_%7Bval%7D) 分别为训练集与验证集上的损失，它们不仅与网络结构有关，还与网络权重参数有关。结构搜索的目标是寻找最优的结构 ![\alpha ^{*}](https://www.zhihu.com/equation?tex=%5Calpha+%5E%7B%2A%7D) 以最小化验证集损失 ![L_{val}(w^{*},\alpha ^{*})](https://www.zhihu.com/equation?tex=L_%7Bval%7D%28w%5E%7B%2A%7D%2C%5Calpha+%5E%7B%2A%7D%29) 。而由此结构所决定的最优权重 ![w^{*}](https://www.zhihu.com/equation?tex=w%5E%7B%2A%7D) 通过最小化训练集上的损失而确定

![w^{*}=arg min_{w}L_{train}(w,\alpha ^{*})](https://www.zhihu.com/equation?tex=w%5E%7B%2A%7D%3Darg+min_%7Bw%7DL_%7Btrain%7D%28w%2C%5Calpha+%5E%7B%2A%7D%29)

因此该最优化问题是一个双层优化问题， ![\alpha ](https://www.zhihu.com/equation?tex=%5Calpha+) 为上层优化变量，而w为下层优化变量，可以形式化的表述为

![min_{\alpha }L_{val}(w^{*}(\alpha ),\alpha )\\s.t \quad w^{*}(\alpha )=argmin_{w}L_{train}(w,\alpha )](https://www.zhihu.com/equation?tex=min_%7B%5Calpha+%7DL_%7Bval%7D%28w%5E%7B%2A%7D%28%5Calpha+%29%2C%5Calpha+%29%5C%5Cs.t+%5Cquad+w%5E%7B%2A%7D%28%5Calpha+%29%3Dargmin_%7Bw%7DL_%7Btrain%7D%28w%2C%5Calpha+%29)

精确求解上面的优化问题成本太高，只要 ![\alpha](https://www.zhihu.com/equation?tex=%5Calpha) 有任何改变，都需要求解内层优化问题得到 ![w^{*}(\alpha )](https://www.zhihu.com/equation?tex=w%5E%7B%2A%7D%28%5Calpha+%29) 。DARTS进行了近似求解，梯度下降时交替优化权重和结构。在第k次迭代时，根据当前结构 ![\alpha _{k-1}](https://www.zhihu.com/equation?tex=%5Calpha+_%7Bk-1%7D) ，通过将 ![w_{k-1}](https://www.zhihu.com/equation?tex=w_%7Bk-1%7D) 向着使得训练误差 ![L_{train}(w_{k-1},\alpha _{k-1})](https://www.zhihu.com/equation?tex=L_%7Btrain%7D%28w_%7Bk-1%7D%2C%5Calpha+_%7Bk-1%7D%29) 最小化的方向移动而得到 ![w_{k}](https://www.zhihu.com/equation?tex=w_%7Bk%7D) 。接下来固定住 ![w_{k}](https://www.zhihu.com/equation?tex=w_%7Bk%7D)，最小化在验证集上的损失而得到新的网络结构。这一步优化的目标为

![L_{val}\left ( w_{k}-\xi \bigtriangledown _{w}L_{train}(w_{k},\alpha _{k-1}),\alpha _{k-1} \right )](https://www.zhihu.com/equation?tex=L_%7Bval%7D%5Cleft+%28+w_%7Bk%7D-%5Cxi+%5Cbigtriangledown+_%7Bw%7DL_%7Btrain%7D%28w_%7Bk%7D%2C%5Calpha+_%7Bk-1%7D%29%2C%5Calpha+_%7Bk-1%7D+%5Cright+%29)

这里先对w进行了一次梯度下降迭代， ![\xi](https://www.zhihu.com/equation?tex=%5Cxi) 为学习率。根据复合函数的求导法则，上面的目标函数对 ![\alpha](https://www.zhihu.com/equation?tex=%5Calpha) 的梯度为

![\bigtriangledown _{\alpha }L_{val}(w,\alpha )-\xi \bigtriangledown _{\alpha ,w}^{2}L_{train}(w,\alpha )\bigtriangledown _{w}L_{val}(w,\alpha )](https://www.zhihu.com/equation?tex=%5Cbigtriangledown+_%7B%5Calpha+%7DL_%7Bval%7D%28w%2C%5Calpha+%29-%5Cxi+%5Cbigtriangledown+_%7B%5Calpha+%2Cw%7D%5E%7B2%7DL_%7Btrain%7D%28w%2C%5Calpha+%29%5Cbigtriangledown+_%7Bw%7DL_%7Bval%7D%28w%2C%5Calpha+%29)

其中

![w^{'}=w-\xi \bigtriangledown _{w}L_{train}(w,\alpha )](https://www.zhihu.com/equation?tex=w%5E%7B%27%7D%3Dw-%5Cxi+%5Cbigtriangledown+_%7Bw%7DL_%7Btrain%7D%28w%2C%5Calpha+%29)

上面梯度计算公式的第二项为矩阵与向量的乘法，为了加快计算速度，用有限差分公式进行了近似。完整的算法流程如下所示。

为每条边(i,j)根据其参数化的向量 ![\alpha _{i,j}](https://www.zhihu.com/equation?tex=%5Calpha+_%7Bi%2Cj%7D) 计算混合操作 ![\bar{o}_{i,j}](https://www.zhihu.com/equation?tex=%5Cbar%7Bo%7D_%7Bi%2Cj%7D)

while没有收敛，do

根据 ![\bigtriangledown _{w}L_{train}(w,\alpha )](https://www.zhihu.com/equation?tex=%5Cbigtriangledown+_%7Bw%7DL_%7Btrain%7D%28w%2C%5Calpha+%29) 更新权重w

根据 ![\bigtriangledown _{\alpha }L_{val}(w-\xi \bigtriangledown _{w}L_{train}(w,\alpha ),\alpha )](https://www.zhihu.com/equation?tex=%5Cbigtriangledown+_%7B%5Calpha+%7DL_%7Bval%7D%28w-%5Cxi+%5Cbigtriangledown+_%7Bw%7DL_%7Btrain%7D%28w%2C%5Calpha+%29%2C%5Calpha+%29) 更新结构 ![\alpha](https://www.zhihu.com/equation?tex=%5Calpha)

对每条边(i,j)用 ![o_{ij}=arg max_{o\in O}\alpha _{o,(i,j)}](https://www.zhihu.com/equation?tex=o_%7Bij%7D%3Darg+max_%7Bo%5Cin+O%7D%5Calpha+_%7Bo%2C%28i%2Cj%29%7D) 替换 ![\bar{o}_{i,j}](https://www.zhihu.com/equation?tex=%5Cbar%7Bo%7D_%7Bi%2Cj%7D)

下图为优化完成之后的网络结构，边的粗细代表这种运算的权重。在得到$$\alpha $$和w的值之后，可以根据下面的规则确定网络的结构。

![](https://pic1.zhimg.com/v2-a3640b1a46401eca2cdd82573b6582f8_b.png)

1.为每个内部节点保留k个最强的前驱，边的强大根据下面的公式计算

![max_{o\in O,o\neq zero}\frac{exp(\alpha _{o,(i,j)})}{\sum_{o\in O}exp(\alpha _{o,(i,j)})}](https://www.zhihu.com/equation?tex=max_%7Bo%5Cin+O%2Co%5Cneq+zero%7D%5Cfrac%7Bexp%28%5Calpha+_%7Bo%2C%28i%2Cj%29%7D%29%7D%7B%5Csum_%7Bo%5Cin+O%7Dexp%28%5Calpha+_%7Bo%2C%28i%2Cj%29%7D%29%7D)
