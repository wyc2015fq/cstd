...
有关于本文所涉及的网络压缩量化的相关知识配套有视频讲解，有兴趣或者需要深入了解学习的同学可以进入www.sigai.cn,按下图搜索相应学习视频。
![](https://pic1.zhimg.com/v2-0c19eb117588aad7bdb5ec44a68020c4_b.jpg)
***导言***
...
1.1 标准卷积
![](https://pic4.zhimg.com/v2-399614b034d4b25382b0f5e4c80107f7_b.jpg)图1标准卷积计算图
HxW表示输入特征图空间尺寸（如图1所示，H和W代表特征图的宽度和高度，输入和输出特征图尺寸不变），N是输入特征通道数，KxK表示卷积核尺寸，M表示输出卷积通道数，则标准卷积计算量是HWNK²M。
![](https://pic3.zhimg.com/v2-4acb7417537e845657901de022a9167a_b.jpg)图2标准卷积计算过程
...
![](https://pic2.zhimg.com/v2-2d777b5051846271ffce36228fdcc575_b.jpg)图3标准卷积：空间维度和通道维度示意图
1.2 Grouped Convolution

分组卷积是标准卷积的变体，其中输入特征通道被为G组(图4)，并且对于每个分组的信道独立地执行卷积，则分组卷积计算量是HWNK²M/G，为标准卷积计算量的1/G。

![](https://pic2.zhimg.com/v2-baf2aeff4dc500d4a6d257513c0b2b95_b.jpg)图 4分组卷积：空间维度和通道维度示意图
...
![](https://pic4.zhimg.com/v2-5c41e6327eeb662dc1c6f8c20c8badcb_b.jpg)图5 AlexNet网络架构
...
![](https://pic4.zhimg.com/v2-e1ba817f6ccfa8dec34ebd1ee5af8907_b.jpg)图6 depthwise卷积![](https://pic3.zhimg.com/v2-5e3d86dd8cdcbe300cd85f1f616440e2_b.jpg)图7 Depthwise卷积：空间维度和通道维度示意图
...
![](https://pic1.zhimg.com/v2-6183445ebd302f4358e3f27333edadec_b.jpg)图8 Pointwise卷积![](https://pic3.zhimg.com/v2-46e4dfaf83219c95d0ddec73a18ad38e_b.jpg)图9 Pointwise卷积：空间维度和通道维度示意图
1.5 Channel Shuffle
...
![](https://pic3.zhimg.com/v2-3868e1964596de344311df5596506726_b.jpg)图10 Channel shuffle：空间维度和通道维度示意图
...
![](https://pic1.zhimg.com/v2-67792efa04876858b66eca4a27334f9c_b.jpg)图13 shuffle V1 Block架构![](https://pic1.zhimg.com/v2-e24b5fd15f7966a02acf56bd45993ca8_b.jpg)图14 ShuffleNet V1 Block：空间维度和通道维度示意图
如所示，ShuffleNet block最重要的操作是channel shuffle layer，在两个分组卷积之间改变通道的顺序，channel shuffle实现分组卷积的信息交换机制。

ResNet bottleneck计算量：
![](https://pic3.zhimg.com/v2-8280d63f6f9c490b994c333962d434b6_b.jpg)
ShuffleNet stride=1计算量：
![](https://pic1.zhimg.com/v2-cea6eaab7f668b416d5a505c0ebe731c_b.jpg)
...
![](https://pic2.zhimg.com/v2-db5f38c980fa4659c0c60d615e9a5425_b.jpg)图15 Mobile V1, Mobile V2,ResNet架构对比
2.4 ShuffleNet V2
...
![](https://pic3.zhimg.com/v2-7713deb7bafcf3501da78a62fc87687e_b.jpg)图16 ShuffeNet V1 VS ShuffeNet V2架构
...
![](https://pic2.zhimg.com/v2-fe43b69af9e6d4171ac1bf9d9a2ec23d_b.jpg)图17 卷积运算汇总参考图
## 3、NAS与神经网络架构搜索
...
![](https://pic3.zhimg.com/v2-97403f439722b04efdc252633ac7ccbe_b.jpg)
...
![](https://pic3.zhimg.com/v2-aa25270781c4561d4450181e25156cce_b.jpg)图18 NasNet基本Cell
NasNet首先基于人类的一些先验知识：卷积运算类型、Cell连接方式，Cell内的多分支拓扑结构，这些是积木。NasNet的搜索算法就是搭积木的过程，不断尝试各种可行的架构，通过代理评价指标确定模型的性能，实现全局最优搜索。
![](https://pic4.zhimg.com/v2-c4c59e9e46d4040c5d4d6c237a051917_b.jpg)图19 NasNet分类任务基本架构![](https://pic2.zhimg.com/v2-d1c183dc7ff2de753c47570b57e92199_b.jpg)
...
![](https://pic3.zhimg.com/v2-0b2ae503ec82b0dde0785c3c4b0bcd4e_b.jpg)图20 MnasNet搜索空间
MnasNet的搜索空间与NasNet类似，但是搜索目标函数同时考虑准确率和速率：
![](https://pic2.zhimg.com/v2-9b772f2370da3049a2193bb2438ef019_b.jpg)
m 表示模型，ACC(m) 表示目标模型的准确率，LAT(m) 表示耗时，T 表示目标耗时。而论文提出一种策略，基于梯度的强化学习方法寻找帕雷托最优，同时对 ACC(m) 和 LAT(m) 帕雷托改善。
![](https://pic4.zhimg.com/v2-8e86a838fb22816d10af7c3a70acbc47_b.jpg)图21 MnasNet基本架构
...
![](https://pic4.zhimg.com/v2-1af485dcd194086fa49d3262ffc601d7_b.jpg)
图 20 AMC引擎示意图

2、搜索空间

AMC对每一层t定义了11个特征表示*st*的状态：
![](https://pic1.zhimg.com/v2-0ba7c942bc8e47bef1c691c56ac66d2c_b.jpg)
...
![](https://pic4.zhimg.com/v2-890ef5fc97693f101b63411c4263d097_b.jpg)
对于quality-critical的AI应用（例如Google Photos），AMC提出精度保证的压缩（accuracy-guaranteed compression），在实现最小尺寸模型的同时不损失精度。

![](https://pic1.zhimg.com/v2-2327b71aec64948572e74f98e7816be4_b.jpg)
3、结论
...
![](https://pic4.zhimg.com/v2-a90134df392c56ae0d403ea98ebde2c7_b.jpg)图21 PocketFlow框架示意图
...
![](https://pic2.zhimg.com/v2-cc6fc1694294a3d041c001890b812949_b.jpg)![](https://pic4.zhimg.com/v2-0aeb57b8715b9de0f095e4be4f230507_b.jpg)![](https://pic1.zhimg.com/v2-713ef0282f81eaa26949ff26ceb65178_b.jpg)![](https://pic1.zhimg.com/v2-572d9d08a335911534f53dcb1073fa64_b.jpg)图22 post-training模型量化实验目录
...
[9]  [https://github.com/tensorflow/tensorflow/blob/master/tensorflow/contrib/lite/tutorials/](https://link.zhihu.com/?target=https%3A//github.com/tensorflow/tensorflow/blob/master/tensorflow/contrib/lite/tutorials/)

[10]   让算法解放算法工程师----NAS综述，[https://zhuanlan.zhihu.com/p/44576620](https://zhuanlan.zhihu.com/p/44576620)
...
> SIGAI推荐：为了帮助大家更好地理解当前卷积神经网络的压缩和加速相关算法原理和实现细节，SIGAI邀请了深度模型压缩与优化领域的资深专家王博士为你全面、系统讲述各种相关算法的原理以及编码实现中的细节与技巧，避免实现论文的算法时踩坑。

1.网络压缩量化之剪枝技术原理详解，Filter-Level，Group-LEVEL，稀疏化
2.网络压缩量化之剪枝技术相关实践 
3.网络压缩量化之低秩估计原理详解，矩阵分解，张量分解
4.网络压缩量化之低秩估计相关实践
...
欢迎关注[http://www.sigai.cn](https://link.zhihu.com/?target=http%3A//www.sigai.cn)
