...
> **文章作者：Poll**
**责任编辑：张帆（东北大学系统工程硕士生）**
本篇文章转自**【博客园】的优秀文章：[Evolutionary Algorithm] 进化算法简介（原文链接网址：**[http://u6.gg/dFFyy](https://link.zhihu.com/?target=http%3A//u6.gg/dFFyy)**）**
...
![](https://pic2.zhimg.com/v2-be9f999cf6b71be72c51a58ddde738a1_b.jpg)
...
遗传算法的基本操作可以用下图来描述：
![](https://pic2.zhimg.com/v2-b6d941f9cd9a429845f6098fd49a2a05_b.jpg)
...
![](https://pic2.zhimg.com/v2-4e5574094b2856cfcf13b8028a1d4451_b.jpg)
...
x1 = decode_x1(pop(:,:));  %对x1 x2进行解码
x2 = decode_x2(pop(:,:end));
fitness = 21.5 + x1.*sin(*pi*x1) +x2.*sin(*pi*x2);%适应度函数
while it < generation
    ...
    x1 = decode_x1(pop(:,:));
    x2 = decode_x2(pop(:,:end));
    fitness = 21.5 + x1.*sin(*pi*x1) +x2.*sin(*pi*x2);%计算适应度
    [fit_best,index] = max(fitness);%本代中的最优目标值
 if fit_best >= maxdata
        maxdata = fit_best;
        verter = pop(index,:);
        x_1 = decode_x1(verter(:));
        x_2 = decode_x2(verter(:end));
    end
    num(it) = maxdata;
    it = it + ;
end
disp(sprintf('max_f=：%.6f',num(it-)));%输出最优解
...
![](https://pic4.zhimg.com/v2-b4a129586b90979d85e6ef055884a683_b.jpg)
