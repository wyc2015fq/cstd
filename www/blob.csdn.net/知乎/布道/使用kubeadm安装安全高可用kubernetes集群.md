# 使用kubeadm安装安全高可用kubernetes集群 - 知乎
# 



> **请关注微信公众号：AiDevOps，回复：kubeadm，获取本文使用kubeadm 1.9.1 所使用Google的**[http://gcr.io](https://link.zhihu.com/?target=http%3A//gcr.io)**镜像库（docker load 直接导入，即可使用，非常方便，快捷）和部署脚本下载链接。**
...
$ yum-config-manager \
    --add-repo \
[https://download.docker.com/linux/centos/docker-ce.repo](https://link.zhihu.com/?target=https%3A//download.docker.com/linux/centos/docker-ce.repo)
...
- 建议二进制方法提前部署好docker-compose，步骤参考后文
- 建议永久关闭selinux和swap以免后续问题
- 建议停止并关闭firewalld/iptables等防火墙
- 新的节点启动后记得改网络名 `hostnamectl set-hostname masterX`
- 节点之间要能互通内网环境稳定
- 安装中出了问题要看日志journalctl -n 10 ,运行中的日志查看`tail -f 10 /var/log/messages`
...
> 有以下需要注意的事项：
- 修改init.sh脚本在后面添加,如果二进制程序没可执行权限`chmod +x /usr/bin/kube*`
- cgroups驱动需要选择docker17.0x版本，就不需要去调整了，如果是1.1x版本的docker需要**手动修改**kubelet的启动文件里面的cgroups配置为`systemd` （修改位置`/etc/systemd/system/kubelet.service.d`） 与 docker info|grep Cg一致
- 提前修改默认的init 或者手动执行`sysctl -w net.ipv4.ip_forward=1` 不然第七行报错
...
- 然后访问[https://master1IP](https://link.zhihu.com/?target=https%3A//master1ip/):32000端口即可，在chrome下无法进入提示证书有误，更换firefox可以，提示说证书日期不对（待修复）
