# linux 下链接库的生成使用（转载） - h13 - 博客园
一、为什么要使用库文件
我们在实际编程工作中肯定会遇见这种情况：有几个项目里有一些函数模块的功能相同，
实现代码也相同，也是我们所说的重复代码。比如，非常多项目里都有一个用户验证的功能。
代码段如下：
...
把公共的（也就是能被多次复用的）目标代码从项目中分离出来，统一存放到库文件中，
项目要用到这些代码的时候，在编译或运行的时候从库文件中取得目标代码即可。库文件
又分两种：静态库和动态库。
二、静态库和动态库
    如果程式是在编译时加载库文件的，就是使用了静态库。如果是在运行时加载目标代码，
就成为动态库。换句话说，如果是使用静态库，则静态库代码在编译时就拷贝到了程式的代码段，
程式的体积会膨胀。如果使用动态库，则程式中只保留库文件的名字和函数名，在运行时去查找
库文件和函数体，程式的体积基本变化不大。
    静态库的原则是“以空间换时间”，增加程式体积，减少运行时间;
    动态库则是“以时间换空间”，增加了运行时间，但减少了程式本身的体积。
    下面我们就以实际例子来看看怎么使用这两种库.
三、静态库的编写和使用
1、概述
   静态库文件的扩展名一般为.a,其编写步骤非常简单。
⑴编写函数代码
⑵编译生成各目标文件
...
   ⑶在gcc 的-l参数后加上库文件名，不过要去掉lib和.a扩展名。
...
     库文件名以lib开头， 以.so 结尾。
        使用方式分为两种: 隐式调用和显示调用 
     隐式调用类似于静态库的使用，但需修改动态链接库的设置文件/etc/ld.so.conf;
    显示调用则是在主程式里使用dlopen、dlsym、dlerror、dlclose等系统函数。
    具体的调用方式会在 "五、动态库的调用" 中周详说明.
2、编写最简单的动态库文件
...
  return 0; 
  }
2、 编译测试程式,和静态库类似,要把头文件的路径加到-I参数里面
  gcc -I /root/exercise -o main.o -c main.c
  目前生成了一个main.o文件
3、连接生成测试程式 
   gcc -o main -L/root/exercise   main.o -ltest
...
    这个原因就是程式运行时并不知道动态库所在的路径，因此自然未找到。
    解决这个问题的办法有三种。见下节
六、使动态库被系统共享的三种办法
...
   /lib,/usr/lib及/etc/ld.so.conf文件内所列的一系列目录.
      实例：执行 
# cp libtest.so /lib 
# ldconfig 
     或: 
...
  此时再执行main，即可成功.
(2)将动态链接库所在目录名追加到动态链接库设置文件/etc/ld.so.conf中. 
# pwd >> /etc/ld.so.conf 
# ldconfig 
  此时再执行main，即可成功.
...
# ldconfig `pwd` 
     此时再执行main，即可成功.
      要注意,第三种方法虽然有效,但效果是暂时的,供程式测试还能,一旦再度运行ldconfig,
   则缓存文件内容可能改动,所需的动态链接库可能不被系统共享了.
...
可见使用静态库时，由于库已被编译成程式的一部分，因此ldd的输出中就只有用到的
系统库。
⑵用动态库隐式连接时的结果 
...
    /lib/ld-linux.so.2 => /lib/ld-linux.so.2 (0xb75eb000)
可见显式使用动态库时，程式中不再保存运行时打开动态库的信息，只保留用到的系统库的信息. 
   这个和使用静态库时的输出是类似的.
----------------------------------------------------------------------------------
**另外，注意- l参数好象应该加到输入文件名的后面，否则会报错。**就是这么一句,解决了我的问题,
原来要用
$ cc +DD64 -L/usr/local/lib -o foo foo.c -ldmalloc
$ ls
foo    foo.c
才行,必须把-l放在输入文件foo.c的后面
