# 如何编写linux下nand flash驱动（转） - h13 - 博客园
**[转：http://www.cnblogs.com/shenhaocn/archive/2011/03/10/1980310.html](http://www.cnblogs.com/shenhaocn/archive/2011/03/10/1980310.html)**
**1. 硬件特性：**
**【Flash的硬件实现机制】**
...
![](http://www.rosoo.net/Files/allimg/100429/11927_1.jpg.jpg)
...
![image](https://images.cnblogs.com/cnblogs_com/shenhaocn/201103/201103102324503839.png)
表1.Nand Flash 第3个ID的含义
**【Nand Flash的物理存储单元的阵列组织结构】**
Nand flash的内部组织结构，此处还是用图来解释，比较容易理解：
![](http://www.rosoo.net/Files/allimg/100429/11927_2.jpg.jpg)
...
所以，总结一下Flash的特殊性如下：
![image](https://images.cnblogs.com/cnblogs_com/shenhaocn/201103/201103102324539048.png)
...
![](http://www.rosoo.net/Files/allimg/100429/11927_3.jpg.jpg)
图3.Nand Flash引脚功能说明
...
![](http://www.rosoo.net/Files/allimg/100429/11927_4.jpg.jpg)
...
**【随机输出页内数据（Random Da****ta Output In a Page）】**
...
**version: 1.0 date:20090721 Author：crifan Mail:green-waste(At)163.com **
**【读（read）操作过程详解】**
...
下面是datasheet中的命令集合：
![](http://www.rosoo.net/Files/allimg/100429/11927_5.jpg.jpg)
...
![](http://www.rosoo.net/Files/allimg/100429/11927_6.jpg.jpg)
...
![](http://www.rosoo.net/Files/allimg/100429/11927_7.jpg.jpg)
...
除了网上最流行的这个解释之外：
[NAND和NOR的比较](http://www.rosoo.net/a/201004/9309.html)
再多说几句：
1.nor的成本相对高，具体读写数据时候，不容易出错。总体上，比较适合应用于存储少量的代码。
2.Nand flash相对成本低。使用中数据读写容易出错，所以一般都需要有对应的软件或者硬件的数据校验算法，统称为ECC。由于相对来说，容量大，价格便宜，因此适合用来存储大量的数据。其在[嵌入式](http://www.rosoo.net/a/embedded/)系统中的作用，相当于PC上的硬盘，用于存储大量数据。
所以，一个常见的应用组合就是，用小容量的Nor Flash存储启动代码，比如uboot，系统启动后,初始化对应的硬件，包括SDRAM等，然后将Nand Flash上的[Linux](http://www.rosoo.net/a/list_49_1.html) 内核读取到内存中，做好该做的事情后，就跳转到SDRAM中去执行内核了，然后内核解压（如果是压缩内核的话，否则就直接运行了）后，开始运行，在[Linux](http://www.rosoo.net/a/list_49_1.html)内核启动最后，去Nand Flash上，挂载根文件，比如jffs2，yaffs2等，挂载完成，运行初始化脚本，启动consle交互，才运行你通过console和内核交互。至此完成整个系统启动过程。
...
2. [芯片内执行(XIP, eXecute In Place)](http://hi.baidu.com/serial_story/blog/item/adb20a2a3f8ffe3c5243c1df.html):
...
**2. 软件方面**
如果想要在Linux下编写Nand Flash[驱动](http://www.rosoo.net/a/list_34_1.html)，那么就先要搞清楚Linux下，关于此部分的整个框架。弄明白，系统是如何管理你的nand flash的，以及，系统都帮你做了那些准备工作，而剩下的，[驱动](http://www.rosoo.net/a/list_34_1.html)底层实现部分，你要去实现哪些功能，才能使得硬件正常工作起来。
