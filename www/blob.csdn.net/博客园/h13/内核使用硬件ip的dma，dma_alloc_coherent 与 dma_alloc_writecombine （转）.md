# 内核使用硬件ip的dma，dma_alloc_coherent 与 dma_alloc_writecombine （转） - h13 - 博客园
...
由此，再去理解 LDD3上讲解的一致性 DMA映射
与流式 DMA
映射就比较容易了，一致性 DMA映射(dma_alloc_coherent )调用的是上面的函数， 由于关闭了 cache/buffer,性能自然比较低。
而流式则通过复杂的同步机制，没有付出性能的代价。
所以我们要尽量使用流式DMA来编程。
再去看看 mmc驱动里使用的分散/聚集映射，原来也是一种流式
 DMA映射。
【补充】
讲了那么多没用，总结下用法：
...
转自：[http://blog.csdn.net/lanmanck/article/details/4773175](http://blog.csdn.net/lanmanck/article/details/4773175)
