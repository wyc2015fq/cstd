# Boost.Asio基本原理（CSDN也有Markdown了，好开森） - DoubleLi - 博客园
...
`ip::address addr = ip::address::from_string("127.0.0.1");`- 1
...
`ip::tcp::endpoint ep( ip::address::from_string("127.0.0.1"), 80);`- 1
...
- 当socket连接到服务端时，*on_connect*被调用了，它会添加一个异步操作。
