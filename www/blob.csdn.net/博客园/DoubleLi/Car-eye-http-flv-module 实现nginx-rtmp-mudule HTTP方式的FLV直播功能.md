# Car-eye-http-flv-module 实现nginx-rtmp-mudule HTTP方式的FLV直播功能 - DoubleLi - 博客园
...
如果将模块编译为动态模块，那么[NGINX](http://nginx.org/)的版本号必须大于或者等于1.9.11。
...
RTMP默认端口为1935，如果使用了其他端口，必须指定`port=xxx`。
