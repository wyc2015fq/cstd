...
今天本来是烧写内核，结果一不小心把uboot也整不能用了，无奈之下只好重新烧个uboot，等都弄好以后，发现系统还是启动不了，原来是启动参数设置不对，于是找到了这篇文章，//是我添加的内容。



原文地址：[http://blog.chinaunix.net/u3/94312/showart_1923637.html](http://blog.chinaunix.net/u3/94312/showart_1923637.html)
...
    前面有说过bootcmd是自动启动时默认执行的一些命令，因此你可以在当前环境中定义各种不同配置，不同环境的参数设置，然后设置bootcmd为你经常使用的那种参数。//如果要输入分号，记得是这样：‘\;’，zImage用的是go addr，uImage用的是bootm addr，我是这样理解的。
...
//mini2400运行OpenWrt：setenv bootargs noinitrd root=/dev/mtdblock3 rootfstype=jffs2 console=ttySAC0,115200 师兄给我解释mtdblock3：0是uboot，1是uboot环境变量，2是内核，3是文件系统。至于为什么是ttySAC0而不是ttyS0，我也不自到。
