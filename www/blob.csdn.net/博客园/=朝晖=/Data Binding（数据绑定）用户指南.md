# Data Binding（数据绑定）用户指南 - =朝晖= - 博客园
# [Data Binding（数据绑定）用户指南](https://www.cnblogs.com/dhcn/p/7124908.html)
http://www.jianshu.com/p/b1df61a4df77
## 1）介绍
这篇文章介绍了如何使用Data Binding库来写声明的layouts文件，并且用最少的代码来绑定你的app逻辑和layouts文件。
Data Binding库不仅灵活而且广泛兼容- 它是一个support库，因此你可以在所有的[Android](http://lib.csdn.net/base/android)平台最低能到[android](http://lib.csdn.net/base/android)2.1（API等级7+）上使用它。
...
```
##### b）Data对象
假设你有一个**user**的plain-old [Java](http://lib.csdn.net/base/java)Object（POJO）：
```java
public class User {
...
## 4）深入Layout文件
##### a）Import
零个或多个`import`元素可能在`data`元素中使用。这些只用在你的layout文件中添加引用，就像在[Java](http://lib.csdn.net/base/java)中：
```
<data>
...
##### c）自定义Binding类名称
默认情况下，Binding类的命名是基于所述layout文件的名称，用大写开头，除去下划线（*）以及（*）后的第一个字母大写，然后添加“Binding”后缀。这个类将被放置在一个模块封装包里的`databinding`封装包下。例如，所述layout文件`contact_item.xml`将生成`ContactItemBinding`。如果模块包是`com.example.my.app`，那么它将被放置在`com.example.my.app.databinding`。
...
## 5）Data 对象
任何Plain old [java](http://lib.csdn.net/base/java)object（PO​​JO）可用于Data Binding，但修改POJO不会导致UI更新。Data Binding的真正能力是当数据变化时，可以通知给你的Data对象。有三种不同的数据变化通知机制：`Observable`对象、`ObservableFields`以及`observable collections`。
...
```
以上例子，开发者需要重命名setters是不太可能了，android[架构](http://lib.csdn.net/base/architecture)属性已经实现了。
##### c）自定义Setters
...
**（如有翻译有误或者不理解的地方，请指正）**
原文链接：http://www.jianshu.com/p/b1df61a4df77
