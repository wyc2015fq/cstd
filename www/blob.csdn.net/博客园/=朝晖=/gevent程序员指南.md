# gevent程序员指南 - =朝晖= - 博客园
# [gevent程序员指南](https://www.cnblogs.com/dhcn/p/7106424.html)
# gevent程序员指南
### 由Gevent社区编写
> gevent是一个基于[libev](http://software.schmorp.de/pkg/libev.html)的并发库。它为各种并发和网络相关的任务提供了整洁的API。
# 介绍
本指南假定读者有中级[Python](http://lib.csdn.net/base/python)水平，但不要求有其它更多的知识，不期待读者有 并发方面的知识。本指南的目标在于给予你需要的工具来开始使用gevent，帮助你 驯服现有的并发问题，并从今开始编写异步应用程序。
### 贡献者
...
# 核心部分
## Greenlets
在gevent中用到的主要模式是Greenlet, 它是以C扩展模块形式接入[python](http://lib.csdn.net/base/python)的轻量级协程。 Greenlet全部运行在主程序[操作系统](http://lib.csdn.net/base/operatingsystem)进程的内部，但它们被协作式地调度。
> 
在任何时刻，只有一个协程在运行。
...
![Greenlet Control Flow](http://xlambda.com/gevent-tutorial/flow.gif)
...
with Timeout(time_to_wait, TooLong):
    gevent.sleep(10)
```
另外，对各种Greenlet和[数据结构](http://lib.csdn.net/base/datastructure)相关的调用，gevent也提供了超时参数。 例如：
```python
import gevent
from gevent import Timeout
def wait():
...
例如，[Redis](http://lib.csdn.net/base/redis)的python绑定一般使用常规的tcp socket来与`[redis](http://lib.csdn.net/base/redis)-server`实例通信。 通过简单地调用`gevent.monkey.patch_all()`，可以使得redis的绑定协作式的调度 请求，与gevent栈的其它部分一起工作。
...
为了[测试](http://lib.csdn.net/base/softwaretest)基准，试用Apache Benchmark `ab`或浏览 [Benchmark of Python WSGI Servers](http://nichol.as/benchmark-of-python-web-servers) 来与其它server作对比。
```
...
```
## 聊天server
最后一个生动的例子，实现一个实时聊天室。运行这个例子需要 [Flask](http://flask.pocoo.org/) (你可以使用Django, Pyramid等，但不是必须的)。 对应的[JavaScript](http://lib.csdn.net/base/javascript)和HTML文件可以在 [这里](https://github.com/sdiehl/minichat)找到。
```python
