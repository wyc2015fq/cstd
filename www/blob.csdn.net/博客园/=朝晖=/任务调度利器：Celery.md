# 任务调度利器：Celery - =朝晖= - 博客园
# [任务调度利器：Celery](https://www.cnblogs.com/dhcn/p/7120712.html)
http://www.liaoxuefeng.com/article/00137760323922531a8582c08814fb09e9930cede45e3cc000
Celery是[Python](http://lib.csdn.net/base/python)开发的分布式任务调度模块，今天抽空看了一下，果然接口简单，开发容易，5分钟就写出了一个异步发送邮件的服务。
Celery本身不含消息服务，它使用第三方消息服务来传递任务，目前，Celery支持的消息服务有RabbitMQ、[Redis](http://lib.csdn.net/base/redis)甚至是[数据库](http://lib.csdn.net/base/mysql)，当然[redis](http://lib.csdn.net/base/redis)应该是最佳选择。
### 安装Celery
用pip或easy_install安装：
```
...
```
Celery默认设置就能满足基本要求。Worker以Pool模式启动，默认大小为CPU核心数量，缺省序列化机制是pickle，但可以指定为json。由于[python](http://lib.csdn.net/base/python)调用UNIX/[Linux](http://lib.csdn.net/base/linux)程序实在太容易，所以，用Celery作为异步任务框架非常合适。
