# A million requests per second with Python - =朝晖= - 博客园
# [A million requests per second with Python](https://www.cnblogs.com/dhcn/p/7130881.html)
https://medium.freecodecamp.com/million-requests-per-second-with-[Python](http://lib.csdn.net/base/python)-95c137af319
Is it possible to hit a million requests per second with [python](http://lib.csdn.net/base/python)? Probably not until recently.
...
Errata: As user @heppu points out, [go](http://lib.csdn.net/base/go)’s stdlib HTTP server can be 12% faster than this graph shows when written more carefully. Also there’s an awesome fasthttp server for Go that apparently is only 18% slower than Japronto in this particular benchmark. Awesome! For details see [https://github.com/squeaky-pl/japronto/pull/12](https://github.com/squeaky-pl/japronto/pull/12) and [https://github.com/squeaky-pl/japronto/pull/14](https://github.com/squeaky-pl/japronto/pull/14).
...
### Final words
All the techniques that I’ve mentioned here are not really specific to Python. They could be probably employed in other languages like Ruby, [JavaScript](http://lib.csdn.net/base/javascript) or even [PHP](http://lib.csdn.net/base/php). I’d be interested in doing such work, too, but this sadly will not happen unless somebody can fund it.
