# 理解HTTP幂等性 - =朝晖= - 博客园
# [理解HTTP幂等性](https://www.cnblogs.com/dhcn/p/7120706.html)
http://ju.outofmemory.cn/entry/153657
基于HTTP协议的Web API是时下最为流行的一种分布式服务提供方式。无论是在大型互联网应用还是企业级[架构](http://lib.csdn.net/base/architecture)中，我们都见到了越来越多的SOA或RESTful的Web API。为什么Web API如此流行呢？我认为很大程度上应归功于简单有效的HTTP协议。HTTP协议是一种分布式的面向资源的网络应用层协议，无论是服务器端提供Web服务，还是客户端消费Web服务都非常简单。再加上浏览器、[JavaScript](http://lib.csdn.net/base/javascript)、AJAX、JSON以及[HTML5](http://lib.csdn.net/base/html5)等技术和工具的发展，互联网应用架构设计表现出了从传统的[PHP](http://lib.csdn.net/base/php)、JSP、ASP[.NET](http://lib.csdn.net/base/dotnet)等服务器端动态网页向Web API + RIA（富互联网应用）过渡的趋势。Web API专注于提供业务服务，RIA专注于用户界面和交互设计，从此两个领域的分工更加明晰。在这种趋势下，Web API设计将成为服务器端程序员的必修课。然而，正如简单的[Java](http://lib.csdn.net/base/java)语言并不意味着高质量的Java程序，简单的HTTP协议也不意味着高质量的Web API。要想设计出高质量的Web API，还需要深入理解分布式系统及HTTP协议的特性。
## 幂等性定义
...
[理解HTTP幂等性](http://ju.outofmemory.cn/entry/153657)，首发于 [脑袋博客](http://ifelsend.com/blog)。
