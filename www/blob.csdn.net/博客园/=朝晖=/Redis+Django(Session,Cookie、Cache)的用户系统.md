# Redis+Django(Session,Cookie、Cache)的用户系统 - =朝晖= - 博客园
# [Redis+Django(Session,Cookie、Cache)的用户系统](https://www.cnblogs.com/dhcn/p/7117425.html)
转自 http://www.cnblogs.com/BeginMan/p/3890761.html
## 一.Django authentication
[django authentication](https://docs.djangoproject.com/en/1.5/topics/auth/) 提供了一个便利的user api接口,无论在py中 `request.user`,参见 [Request and response objects](https://docs.djangoproject.com/en/1.5/ref/request-response/) .还是模板中的 `{{user}}` 都能随时随地使用,如果从web开发角度来看,其实无非就是cookie与session的运用.
...
## 二.Session 与Cookie
![](http://blog.chedushi.com/wp-content/uploads/2014/10/FjEFry.gif)
图片来源: [http://blog.csdn.net/tzjly/article/details/6986268](http://blog.csdn.net/tzjly/article/details/6986268)
...
```
对COOKIES的设置通过 `HttpResponse` 对象的 `set_cookie` 来完成, [文档传送门](https://docs.djangoproject.com/en/1.5/ref/request-response/#django.http.HttpResponse.set_cookie)
...
思路参考: [http://www.cnblogs.com/ddyq/p/3151284.html](http://www.cnblogs.com/ddyq/p/3151284.html)
第三方库: `django-redis-sessions` , [github地址](https://github.com/martinrusev/django-redis-sessions) ,提供了Redis database backend for your sessions. 另一个第三方库 [Redis Django Cache Backend](https://github.com/sebleier/django-redis-cache) ,A cache backend for Django using the Redis datastructure server.用Redis存储缓存数据.
对此我们可以应用到自己的项目中,如上篇 [Redis key的设计模式 & django登陆](http://beginman.sinaapp.com/blog/75/) 提到的实例,那么接下来我们在此基础上 **实现session,cookie,Redis定制的用户系统** .
**注意要实现配置好Redis服务.**
### 1.安装&配置
...
```
至此我们的Redis For Django算是完成了,如果不太明白,还有一篇文章 [**Using Redis as Django’s session store and cache backend**](http://michal.karzynski.pl/blog/2013/07/14/using-redis-as-django-session-store-and-cache-backend/)可以借鉴.
...
[这里](https://github.com/BeginMan/RedisPoem) . 上篇我们实现了登陆的功能,接下来要实现用户系统功能.
## 1.思路
![](http://blog.chedushi.com/wp-content/uploads/2014/10/bA3qIzI.png)
## 2.程序实现
### 1.用户系统类
