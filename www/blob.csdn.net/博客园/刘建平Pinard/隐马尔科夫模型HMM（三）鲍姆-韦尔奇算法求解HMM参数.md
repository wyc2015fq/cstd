...
# 隐马尔科夫模型HMM（三）鲍姆-韦尔奇算法求解HMM参数 - 刘建平Pinard - 博客园
...
$$
利用我们在[隐马尔科夫模型HMM（二）前向后向算法评估观察序列概率](http://www.cnblogs.com/pinard/p/6955871.html)里第二节中前向概率的定义可得：
$$
...
$$
利用[隐马尔科夫模型HMM（二）前向后向算法评估观察序列概率](http://www.cnblogs.com/pinard/p/6955871.html)里第二节中前向概率的定义和第五节$\xi_t(i,j)$的定义可得们在M步$a_{ij}$的迭代公式为：
$$
...
$$
其中$I(o_t^{(d)}=v_k)$当且仅当$o_t^{(d)}=v_k$时为1，否则为0. 利用[隐马尔科夫模型HMM（二）前向后向算法评估观察序列概率](http://www.cnblogs.com/pinard/p/6955871.html)里第二节中前向概率的定义可得$b_{j}(o_t)$的最终表达式为：
$$
