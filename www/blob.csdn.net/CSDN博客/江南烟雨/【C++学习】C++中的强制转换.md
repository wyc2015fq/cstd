# 【C++学习】C++中的强制转换 - 江南烟雨 - CSDN博客
2013年06月28日 21:16:43[江南烟雨](https://me.csdn.net/xiajun07061225)阅读数：2335标签：[C++																[类																[对象](https://so.csdn.net/so/search/s.do?q=对象&t=blog)](https://so.csdn.net/so/search/s.do?q=类&t=blog)](https://so.csdn.net/so/search/s.do?q=C++&t=blog)
...
If pb points to an object of type B and not to thecomplete D class, then dynamic_cast will know enough to return zero. However,static_cast relies on the programmer’s assertion that pb points to an object oftype D and simply returns a pointer to that supposed
 D object. 
