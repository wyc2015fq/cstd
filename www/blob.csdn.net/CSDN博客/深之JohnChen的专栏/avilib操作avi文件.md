# avilib操作avi文件 - 深之JohnChen的专栏 - CSDN博客

2018年10月19日 15:58:41[byxdaz](https://me.csdn.net/byxdaz)阅读数：575


一、avilib介绍

avilib是一个使用c语言编写的库，可以很方便的对avi文件提取帧和生成avi视频文件。如下列出avilib库的代码。
...
//   nhb += MAX_INFO_STRLEN;
   ...
   if ( lseek(AVI->fdes,0,SEEK_SET)<0 ||
        avi_write(AVI->fdes,(char *)AVI_header,HEADERBYTES)!=HEADERBYTES ||
        (AVI->pos>0&&ftruncate(AVI->fdes,AVI->pos)<0 ))
   {
      AVI_errno = AVI_ERR_CLOSE;
      return -1;
   }
...
1、AVI_open_output_file，打开avi文件

2、AVI_set_video 设置视频参数

//AVI：为打开avi文件的avi_t;width:图像宽度；height：图像高度；fps：帧率；compressor：视频压缩类型，比如"MJPG","H264"
...
3、AVI_set_audio 设置音频参数

//AVI：为打开avi文件的avi_t;channels：音频通道；rate：采样率；bits：位宽；音频格式；mp3rate：mp3采样率频率，单位kbps
...
4、AVI_write_frame写视频数据

5、AVI_write_audio 写音频数据

6、AVI_close 关闭avi文件

读取avi文件

1、AVI_open_input_file 打开avi文件

2、AVI_video_width、AVI_video_height、AVI_frame_rate、AVI_video_compressor视频参数获取函数；

     AVI_audio_channels、AVI_audio_bits、AVI_audio_format音频获取参数等。

3、AVI_video_frames 获取所有帧数量

4、循环帧数量，读取每帧数据

 int frames = AVI_video_frames(...)

 int frame = 0;

 char * videobuf = new char[width*height*3];

 int audiobufsize = 2048*channels;

 char * audiobuf = new char[audiobufsize];
...
   long video_bytes = AVI_frame_size(avifile,frame);//获取视频大小

      long audio_bytes = AVI_audio_size(avifile,frame);//获取音频大小

      long actual_video_size = AVI_read_frame(avifile,(char *)videobuf,&iskeyframe);//读取视频帧数据

      AVI_read_audio(avifile, (char *)audiobuf,audio_bytes); //读取音频帧数据
...
AVI文件格式，参考[https://blog.csdn.net/qq_18854309/article/details/77621951?utm_source=blogxgwz0](https://blog.csdn.net/qq_18854309/article/details/77621951?utm_source=blogxgwz0)

avilib库下载 [https://github.com/woshinieao/avilib](https://github.com/woshinieao/avilib)
