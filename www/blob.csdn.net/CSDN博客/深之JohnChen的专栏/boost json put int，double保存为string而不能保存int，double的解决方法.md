# boost json put int，double保存为string而不能保存int，double的解决方法 - 深之JohnChen的专栏 - CSDN博客

2018年08月31日 19:31:14[byxdaz](https://me.csdn.net/byxdaz)阅读数：357


boost json put int，double保存为string而不能保存int，double的解决方法，主要修改write_json_helper函数。本文以boost 1.66.0版本为例，列出需要修改的地方。

修改boost\property_tree\json_parser\detail\write.hpp文件两个地方：
...
ptRoot.put<std::string>("deviceid","00001", my_string_translator<std::string>());
ptRoot.put<unsigned int>("alarmtype", 1);	
std::stringstream sstream;
boost::property_tree::write_json(sstream, ptRoot);
std::string strParams = sstream.str();
```
...
如果不修改boost的property_tree代码的话，拷贝boost文件中property_tree文件夹到本地工程，修改write.hpp文件。

修改本地property_tree/json_parser.hpp文件中的头引用部分，如下：
...
#include "json_parser/detail/write.hpp"

在工程中包含本地json_parser.hpp文件
