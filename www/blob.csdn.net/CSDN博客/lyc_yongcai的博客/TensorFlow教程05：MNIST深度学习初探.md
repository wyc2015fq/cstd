...
# TensorFlow教程05：MNIST深度学习初探 - lyc_yongcai的博客 - CSDN博客


2017年06月14日 21:29:22[刷街兜风](https://me.csdn.net/lyc_yongcai)阅读数：408
...
mnist = input_data.read_data_sets('MNIST_data', one_hot=True)
```
这里，mnist是一个轻量级的类。它以Numpy数组的形式存储着训练、校验和[测试](http://lib.csdn.net/base/softwaretest)数据集。同时提供了一个函数，用于在迭代中获得minibatch，后面我们将会用到。
...
```
**计算图**
为了在[Python](http://lib.csdn.net/base/python)中进行高效的数值计算，我们通常会使用像NumPy一类的库，将一些诸如矩阵乘法的耗时操作在[python](http://lib.csdn.net/base/python)环境的外部来计算，这些计算通常会通过其它语言并用更为高效的代码来实现。
...
我们已经定义好模型和训练用的损失函数，那么用TensorFlow进行训练就很简单了。因为TensorFlow知道整个计算图，它可以使用自动微分法找到对于各个变量的损失的梯度值。TensorFlow有大量内置的优化[算法](http://lib.csdn.net/base/datastructure)这个例子中，我们用最速下降法让交叉熵下降，步长为0.01.
**[python]**[
view plain](#)[
copy](#)
[print](#)[?](#)
...
copy](#)
[print](#)[?](#)
printaccuracy.eval(feed_dict={x: mnist.test.images, y_: mnist.test.labels})
