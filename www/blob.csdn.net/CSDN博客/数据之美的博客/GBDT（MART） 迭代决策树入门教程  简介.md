# GBDT（MART） 迭代决策树入门教程 | 简介 - 数据之美的博客 - CSDN博客
2017年09月05日 02:54:27[看穿数据之美](https://me.csdn.net/oppo62258801)阅读数：137
...
       GBDT(Gradient Boosting Decision Tree) 又叫 MART（Multiple Additive Regression Tree)，是一种迭代的决策树[算法](http://lib.csdn.net/base/datastructure)，该算法由多棵决策树组成，所有树的结论累加起来做最终答案。它在被提出之初就和SVM一起被认为是泛化能力（generalization)较强的算法。近些年更因为被用于搜索排序的[机器学习](http://lib.csdn.net/base/machinelearning)模型而引起大家关注。
...
答案是过拟合。过拟合是指为了让训练集精度更高，学到了很多”仅在训练集上成立的规律“，导致换一个数据集当前规律就不适用了。其实只要允许一棵树的叶子节点足够多，训练集总是能训练到100%准确率的（大不了最后一个叶子上只有一个instance)。在训练精度和实际精度（或[测试](http://lib.csdn.net/base/softwaretest)精度）之间，后者才是我们想要真正得到的。
...
**六、 [搜索引擎](http://lib.csdn.net/base/searchengine)排序应用 RankNet**
