# 机器学习算法与Python实践之k近邻（KNN） - 数据之美的博客 - CSDN博客
2017年05月06日 21:30:45[看穿数据之美](https://me.csdn.net/oppo62258801)阅读数：219
**[机器学习](http://lib.csdn.net/base/machinelearning)[算法](http://lib.csdn.net/base/datastructure)与[Python](http://lib.csdn.net/base/python)实践之（一）k近邻（KNN）**
...
       总的来说就是我们已经存在了一个带标签的[数据库](http://lib.csdn.net/base/mysql)，然后输入没有标签的新数据后，将新数据的每个特征与样本集中数据对应的特征进行比较，然后算法提取样本集中特征最相似（最近邻）的分类标签。一般来说，只选择样本数据库中前k个最相似的数据。最后，选择k个最相似数据中出现次数最多的分类。其算法描述如下：
...
**2.1、kNN基础实践**
       一般实现一个算法后，我们需要先用一个很小的数据库来[测试](http://lib.csdn.net/base/softwaretest)它的正确性，否则一下子给个[大数据](http://lib.csdn.net/base/hadoop)给它，它也很难消化，而且还不利于我们分析代码的有效性。
      首先，我们新建一个kNN.py脚本文件，文件里面包含两个函数，一个用来生成小数据库，一个实现kNN分类算法。代码如下：
**[python]**[view
 plain](http://blog.csdn.net/zouxy09/article/details/16955347#)[copy](http://blog.csdn.net/zouxy09/article/details/16955347#)
![在CODE上查看代码片](https://code.csdn.net/assets/CODE_ico.png)[](https://code.csdn.net/snippets/81906/fork)
- #########################################
...
**[python]**[view
 plain](http://blog.csdn.net/zouxy09/article/details/16955347#)[copy](http://blog.csdn.net/zouxy09/article/details/16955347#)
![在CODE上查看代码片](https://code.csdn.net/assets/CODE_ico.png)[](https://code.csdn.net/snippets/81906/fork)
- Your input is: [ 1.21.0] and classified to class:  A  
- Your input is: [ 0.10.3] and classified to class:  B  
**2.2、kNN进阶**
