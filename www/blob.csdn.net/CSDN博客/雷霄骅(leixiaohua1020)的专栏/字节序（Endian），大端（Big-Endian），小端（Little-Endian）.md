# 字节序（Endian），大端（Big-Endian），小端（Little-Endian） - 雷霄骅(leixiaohua1020)的专栏 - CSDN博客





2013年10月13日 17:56:00[雷霄骅](https://me.csdn.net/leixiaohua1020)阅读数：88
...
现代的计算机系统一般采用**字节**(Octet, 8 bit Byte)作为逻辑寻址单位。当物理单位的长度大于1个字节时，就要区分**字节顺序**(Byte Order, or[Endianness](http://en.wikipedia.org/wiki/Endianness))。常见的字节顺序有两种：**Big Endian**(High-byte first)和**Little Endian**(Low-byte first)，这就是表2.1中的**BE**和**LE**。Intel X86平台采用Little Endian，而PowerPC处理器则采用了Big Endian。举例来说，整型数字$**1234ABCD**存储的时候就会有两种方式：
|字节顺序|内存数据|备注|
|----|----|----|
...
![](http://www.cppblog.com/images/cppblog_com/tx7do/250px-Endianessmap_svg.png)
...
原文地址：[http://www.cppblog.com/tx7do/archive/2009/01/06/71276.html](http://www.cppblog.com/tx7do/archive/2009/01/06/71276.html)
