# RTMPdump（libRTMP） 源代码分析 9： 接收消息（Message）（接收视音频数据） - 雷霄骅(leixiaohua1020)的专栏 - CSDN博客





2013年10月23日 15:46:00[雷霄骅](https://me.csdn.net/leixiaohua1020)阅读数：30
...
前一篇文章分析了RTMPdump（libRTMP） 的发送消息（Message）方面的源代码：[RTMPdump（libRTMP） 源代码分析 8： 发送消息（Message）](http://blog.csdn.net/leixiaohua1020/article/details/12958747)
...
```
 ...
 这里有两个函数比较重要：RTMP_ReadPacket()以及RTMP_ClientPacket()。这两个函数中，前一个函数负责从网络上读取数据，后一个负责处理数据。这部分与建立RTMP连接的网络流（NetStream）的时候很相似，参考：[RTMPdump（libRTMP） 源代码分析 6： 建立一个流媒体连接 （NetStream部分 1）](http://blog.csdn.net/leixiaohua1020/article/details/12957877)
