# Non-standard calculus - 袁萌专栏 - CSDN博客
2018年10月12日 18:53:19[yuanmeng001](https://me.csdn.net/yuanmeng001)阅读数：84
...
1 History
2 Motivation
3 Keisler's textbook
4 Definition of derivative
5 Continuity
6 Uniform continuity
7 Compactness
8 Heine–Cantor theorem
9 Why is the squaring function not uniformly continuous?
10 Example: Dirichlet function
11 Limit
12 Limit of sequence
...
20 External links
History
The history of non-standard calculus began with the use of infinitely small quantities, called infinitesimals in calculus. The use of infinitesimals can be found the foundations of calculus independently developed by Gottfried Leibniz and Isaac Newton starting in the 1660s. John Wallis refined earlier techniques of indivisibles of Cavalieri and others by exploiting an infinitesimal quantity he denoted
1 ∞ {\displaystyle {\tfrac {1}{\infty }}}
 in area calculations, preparing the ground for integral calculus.[3] They drew on the work of such mathematicians as Pierre de Fermat, Isaac Barrow and René Descartes.
...
f ′ {\displaystyle f'}
 of the function
y = f ( x ) = x 2 {\displaystyle y=f(x)=x^{2}}
 at x, both approaches agree on the algebraic manipulations:
Δ y Δ x = ( x + Δ x ) 2 − x 2 Δ x = 2 x Δ x + ( Δ x ) 2 Δ x = 2 x + Δ x ≈ 2 x {\displaystyle {\frac {\Delta y}{\Delta x}}={\frac {(x+\Delta x)^{2}-x^{2}}{\Delta x}}={\frac {2x\Delta x+(\Delta x)^{2}}{\Delta x}}=2x+\Delta x\approx 2x}
This becomes a computation of the derivatives using the hyperreals if we interpret
Δ x {\displaystyle \Delta x}
 as an infinitesimal and let the symbol "
≈ {\displaystyle \approx }
" be the relation "is infinitely close to".
In order to make f ' a real-valued function, we must dispense with the final term
Δ x {\displaystyle \Delta x}
. In the standard approach using only real numbers, that is done by taking the limit as
Δ x {\displaystyle \Delta x}
 tends to zero. In the hyperreal approach, the quantity
Δ x {\displaystyle \Delta x}
...
Δ y / Δ x {\displaystyle \Delta y/\Delta x}
...
f ′ ( x ) = s t ( f ∗ ( x + ε ) − f ∗ ( x ) ε ) , {\displaystyle f'(x)=\mathrm {st} \left({\frac {f^{*}(x+\varepsilon )-f^{*}(x)}{\varepsilon }}\right),}
where st is the standard part function, yielding the real number infinitely close to the hyperreal argument of st, and
f ∗ {\displaystyle f^{*}}
...
Using the notation
≈ {\displaystyle \approx }
 for the relation of being infinitely close as above, the definition can be extended to arbitrary (standard or non-standard) points as follows:
A function f is microcontinuous at x if whenever
x ′ ≈ x {\displaystyle x'\approx x}
, one has
f ∗ ( x ′ ) ≈ f ∗ ( x ) {\displaystyle f^{*}(x')\approx f^{*}(x)}
...
x ≈ y {\displaystyle x\approx y}
 then
f ∗ ( x ) ≈ f ∗ ( y ) {\displaystyle f^{*}(x)\approx f^{*}(y)}
.
...
f ( x ) ≈ f ( c ) ≈ f ( y ) , {\displaystyle f(x)\approx f(c)\approx f(y),}
and therefore f(x) and f(y) are infinitely close, proving uniform continuity of f.
Why is the squaring function not uniformly continuous?[edit]
Let f(x) = x2 defined on
R {\displaystyle \mathbb {R} }
. Let
N ∈ R ∗ {\displaystyle N\in \mathbb {R} ^{*}}
 be an infinite hyperreal. The hyperreal number
N + 1 N {\displaystyle N+{\tfrac {1}{N}}}
 is infinitely close to N. Meanwhile, the difference
f ( N + 1 N ) − f ( N ) = N 2 + 2 + 1 N 2 − N 2 = 2 + 1 N 2 {\displaystyle f(N+{\tfrac {1}{N}})-f(N)=N^{2}+2+{\tfrac {1}{N^{2}}}-N^{2}=2+{\tfrac {1}{N^{2}}}}
...
Example: Dirichlet function[edit]
Consider the Dirichlet function
I Q ( x ) := { 1  if  x  is rational , 0  if  x  is irrational . {\displaystyle I_{Q}(x):={\begin{cases}1&{\text{ if }}x{\text{ is rational}},\\0&{\text{ if }}x{\text{ is irrational}}.\end{cases}}}
...
Limit of sequence[edit]
Given a sequence of real numbers
{ x n | n ∈ N } {\displaystyle \{x_{n}|n\in \mathbb {N} \}}
, if
L ∈ R {\displaystyle L\in \mathbb {R} }
 we say L is the limit of the sequence and write
L = lim n → ∞ x n {\displaystyle L=\lim _{n\to \infty }x_{n}}
...
L = lim n → ∞ x n ⟺ ∀ ϵ > 0 , ∃ N ∈ N , ∀ n ∈ N : n > N → | x n − L | < ϵ . {\displaystyle L=\lim _{n\to \infty }x_{n}\Longleftrightarrow \forall \epsilon >0\;,\exists N\in \mathbb {N} \;,\forall n\in \mathbb {N} :n>N\rightarrow |x_{n}-L|<\epsilon .}
...
c = s t ( x i 0 ) {\displaystyle c=\mathrm {st} (x_{i_{0}})}
...
Δ h f := st ⁡ [ ∗ f ] ( x + h ) − [ ∗ f ] ( x ) h {\displaystyle \Delta _{h}f:=\operatorname {st} {\frac {[{}^{*}\!f](x+h)-[{}^{*}\!f](x)}{h}}}
...
∑ k = 0 n − 1 f ( ξ k ) ( x k + 1 − x k ) {\displaystyle \sum _{k=0}^{n-1}f(\xi _{k})(x_{k+1}-x_{k})}
where
a = x 0 ≤ ξ 0 ≤ x 1 ≤ … x n − 1 ≤ ξ n − 1 ≤ x n = b . {\displaystyle a=x_{0}\leq \xi _{0}\leq x_{1}\leq \ldots x_{n-1}\leq \xi _{n-1}\leq x_{n}=b.}
We will call such a sequence of values a partition or mesh and
sup k ( x k + 1 − x k ) {\displaystyle \sup _{k}(x_{k+1}-x_{k})}
...
S M = st ⁡ ∑ k = 0 n − 1 [ ∗ f ] ( ξ k ) ( x k + 1 − x k ) {\displaystyle S_{M}=\operatorname {st} \sum _{k=0}^{n-1}[*f](\xi _{k})(x_{k+1}-x_{k})}
...
Then
| f ( b ) − f ( a ) | ≤ ∑ k = 1 N − 1 | f ( x k + 1 ) − f ( x k ) | ≤ ∑ k = 1 N − 1 { | f ′ ( x k ) | + ϵ k } | x k + 1 − x k | . {\displaystyle |f(b)-f(a)|\leq \sum _{k=1}^{N-1}|f(x_{k+1})-f(x_{k})|\leq \sum _{k=1}^{N-1}\left\{|f'(x_{k})|+\epsilon _{k}\right\}|x_{k+1}-x_{k}|.}
...
| f ( b ) − f ( a ) | ≤ ∑ k = 1 N − 1 ( M + ϵ ) ( x k + 1 − x k ) = M ( b − a ) + ϵ ( b − a ) {\displaystyle |f(b)-f(a)|\leq \sum _{k=1}^{N-1}(M+\epsilon )(x_{k+1}-x_{k})=M(b-a)+\epsilon (b-a)}
...
Hazewinkel, Michiel, ed. (2001) [1994], "Infinitesimal calculus", Encyclopedia of Mathematics, Springer Science+Business Media B.V. / Kluwer Academic Publishers, ISBN 978-1-55608-010-4
External links
...
Infinitesimals
History
AdequalityLeibniz's notationIntegral symbolCriticism of non-standard analysisThe AnalystThe Method of Mechanical TheoremsCavalieri's principleMethod of indivisibles
Related branches
Non-standard analysisNon-standard calculusInternal set theorySynthetic differential geometrySmooth infinitesimal analysisConstructive non-standard analysisInfinitesimal strain theory (physics)
Formalizations
DifferentialsHyperreal numbersDual numbersSurreal numbers
Individual concepts
Standard part functionTransfer principleHyperintegerIncrement theoremMonadInternal setLevi-Civita fieldHyperfinite setLaw of ContinuityOverspillMicrocontinuityTranscendental law of homogeneity
Scientists
Gottfried Wilhelm LeibnizAbraham RobinsonPierre de FermatAugustin-Louis CauchyLeonhard Euler
Textbooks
Analyse des Infiniment PetitsElementary Calculus Cours d'Analyse
Categories: Non-standard analysis Calculus Infinity
