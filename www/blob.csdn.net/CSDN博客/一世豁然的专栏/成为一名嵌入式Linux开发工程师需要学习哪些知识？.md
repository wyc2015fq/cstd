# 成为一名嵌入式Linux开发工程师需要学习哪些知识？ - 一世豁然的专栏 - CSDN博客





2015年04月29日 20:25:20[一世豁然](https://me.csdn.net/Explorer_day)阅读数：1132标签：[解决方案](https://so.csdn.net/so/search/s.do?q=解决方案&t=blog)
...
本博客转载于：[http://bbs.elecfans.com/jishu_426554_1_1.html](http://bbs.elecfans.com/jishu_426554_1_1.html)
...
Linux操作系统的概念、安装方法，详细了解Linux下的目录结构、基本命令、编辑器VI
，编译器GCC，调试器GDB和
Make 项目管理工具, Shell Makefile脚本编写等知识，嵌入式开发环境的搭建。


　　三：Linux系统编程

　　重点学习标准I/O库，Linux多任务编程中的多进程和多线程，以及进程间[通信](http://bbs.elecfans.com/zhuti_wireless_1.html)(pipe、FIFO、消息队列、共享内存、signal、信号量等)，同步与互斥对共享资源访问控制等重要知识，主要提升对Linux应用开发的理解和代码调试的能力。
...
　　通过基于[ARM](http://bbs.elecfans.com/zhuti_arm_1.html) Cortex-A8处理s5pv210了解[芯片](http://www.hqchip.com/?u=999)手册的基本阅读技巧，掌握s5pv210系统资源、时钟控制器、[电源](http://bbs.elecfans.com/zhuti_power_1.html)管理、异常中断控制器、nand
 flash控制器等模块，为底层平台搭建做好准备。Linux平台包括内核裁减、内核移植、交叉编译、GNU工具使用、内核调试、Bootloader介绍、制作与原理分析、根文件系统制作以及向内核中添加自己的模块，并在s5pv210实验平台上运行自己制作的Linux系统,集成部署Linux系统整个流程。同时了解Android操作系统开发流程。Android系统是基于Linux平台的开源操作系统，该平台由操作系统、中间件、用户界面和应用软件组成，是首个为移动终端打造的真正开放和完整的移动软件，目前它的应用不再局限于移动终端，还包括数据电视、机顶盒、PDA等消费类[电子](http://bbs.elecfans.com/)产品。
