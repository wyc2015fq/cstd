# TreeFrog Framework开发环境搭建 (C++ web framework, based on Qt) - baidu_33570760的博客 - CSDN博客
2017年06月20日 11:23:53[carman_风](https://me.csdn.net/baidu_33570760)阅读数：752
## 参考地址：http://treefrogframework.github.io/treefrog-framework/user-guide/ch/install/
## 安装
首先,我们需要先安装好Qt库.对于Windows和Mac OS X,从[Qt 网站](http://qt-project.org/downloads) 下载并安装它.对于Linux,你可以安装一个发布的包.如果是Ubuntu:安装Qt库和开发工具
```
...
```
### 安装说明
- 解压已 [下载](http://www.treefrogframework.org/ja/%E3%83%80%E3%82%A6%E3%83%B3%E3%83%AD%E3%83%BC%E3%83%89)的文件.以下命令应用到版本’x.x.x’.请对于的替换.
```
$ tar xvfz treefrog-x.x.x.tar.gz
```
- 运行build命令.对于Windows:请为*发行(releae)*和调试(debugging)*创建两种不同类型的二进制文件.打开Qt命令行窗口,然后使用以下命令Build.配置批处理文件(configure.bat)应该执行两次.
```
> cd treefrog-x.x.x
...
```
**说明:**
为了调试Treefrog框架本身,请使用*configure*选项.现在请执行命令:
```
./configure --enable-debug
```
...
```
(‘x.x.x’表示当前使用的版本)
![创建快捷方式](http://treefrogframework.github.io/treefrog-framework/assets/images/documentation/shortcut_ch.png)
设置快捷方式的名字为’Treefrog命令行窗口’.
![快捷方式名称](http://treefrogframework.github.io/treefrog-framework/assets/images/documentation/shortcut-name_ch.png)
### 配置选项
