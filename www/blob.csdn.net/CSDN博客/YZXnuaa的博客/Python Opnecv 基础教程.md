# Python Opnecv 基础教程 - YZXnuaa的博客 - CSDN博客
2018年02月01日 17:27:58[YZXnuaa](https://me.csdn.net/YZXnuaa)阅读数：810

本篇是前面两篇教程：[给深度学习入门者的Python快速教程 - 基础篇](https://zhuanlan.zhihu.com/p/24162430)
...
- flann：最近邻算法库，Fast Library for ApproximateNearest Neighbors，用于在多维空间进行聚类和检索，经常和关键点匹配搭配使用。
...
执行exe安装后，会在<安装目录>/build/python/2.7下发现一个叫cv2.pyd的文件，把这个文件拷贝到<Python目录>\Lib\site-packages下，就可以了。Windows下如果只想在Python中体验OpenCV还有个更简单的方法是加州大学尔湾分校（University ofCalifornia, Irvine）的Christoph Gohlke制作的Windows下的Python科学计算包网页，下载对应版本的wheel文件，然后通过pip安装：
[http://www.lfd.uci.edu/~gohlke/pythonlibs/#opencv](http://link.zhihu.com/?target=http%3A//www.lfd.uci.edu/%7Egohlke/pythonlibs/%23opencv)
...
```
执行这段代码得到如下的图像：
![](https://pic1.zhimg.com/80/v2-e89c8ea16a8e3b72fc8a0c4dcbcc0e7b_hd.jpg)
## 6.2.4视频功能
...
![](https://pic1.zhimg.com/80/v2-8643baac95bd135cd5ffac3b6d1a0c7a_hd.jpg)
...
![](https://pic4.zhimg.com/80/v2-8e2c7867c54bd1ef7d5afba1285acab8_hd.jpg)
...
## 6.3.3随机颜色和明暗
比起AlexNet论文里在PCA之后的主成分上做扰动的方法，本书用来实现随机的颜色以及明暗的方法相对简单很多，就是给HSV空间的每个通道，分别加上一个微小的扰动。其中对于色调，从-![\delta](http://www.zhihu.com/equation?tex=%5Cdelta)到![\delta](http://www.zhihu.com/equation?tex=%5Cdelta)之间按均匀采样，获取一个随机数![\hat \delta](http://www.zhihu.com/equation?tex=%5Chat+%5Cdelta)作为要扰动的值，然后新的像素值x’为原始像素值x
 +![\hat \delta](http://www.zhihu.com/equation?tex=%5Chat+%5Cdelta)；对于其他两个空间则是新像素值x’为原始像素值x的(1+![\hat \delta](http://www.zhihu.com/equation?tex=%5Chat+%5Cdelta))倍，从而实现色调，饱和度和明暗度的扰动。
...
![](https://pic3.zhimg.com/80/v2-dc6d55514e584fcc8a7b5bf8b20b136c_hd.jpg)
...
```
执行这段代码得到如下窗口：
![](https://pic1.zhimg.com/80/v2-1712545c1114f3f66f37121c02354144_hd.jpg)
...
![](https://pic1.zhimg.com/80/v2-8bf145d52acf931772ce598e8bbc777a_hd.jpg)
