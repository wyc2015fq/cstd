# 概率图模型学习笔记：HMM、MEMM、CRF - YZXnuaa的博客 - CSDN博客
2018年03月20日 15:21:47[YZXnuaa](https://me.csdn.net/YZXnuaa)阅读数：4697
作者：Scofield
...
## **二、Prerequisite  **
**   2.1 概率图**
**    2.1.1 概览        2.1.2 有向图 vs. 无向图        2.1.3 马尔科夫假设&马尔科夫性    2.2 判别式模型 vs. 生成式模型    2.3 序列建模**
## **三、HMM**
**  3.1 理解HMM    3.2 模型运行过程        3.2.1 学习过程        3.2.2 序列标注（解码）过程        3.2.3 序列概率过程**
## **四、MEMM**
**  4.1 理解MEMM    4.2 模型运行过程**
**     4.2.1 学习过程         4.2.2 序列标注（解码）过程         4.2.3 序列概率过程    4.3 标注偏置？ **
## **五、CRF**
**  5.1 理解CRF    5.2 模型运行过程        5.2.1 学习过程        5.2.2 解码过程        5.2.3 序列概率过程    5.3 CRF++分析    5.4 LSTM+CRF **
## **六、总结**
## **Referrence**
## **一、Preface**
...
![](https://pic1.zhimg.com/50/v2-714c1843f78b6aecdb0c57cdd08e1c6a_hd.jpg)![](https://pic1.zhimg.com/80/v2-714c1843f78b6aecdb0c57cdd08e1c6a_hd.jpg)
...
![](https://pic1.zhimg.com/50/v2-5b3f6b4a2d905297b7f73a89e92ee618_hd.jpg)![](https://pic1.zhimg.com/80/v2-5b3f6b4a2d905297b7f73a89e92ee618_hd.jpg)
应该这样表示他们的联合概率:
...
![](https://pic4.zhimg.com/50/v2-1d8faeb71d690d02e110c7cd1d39eed3_hd.jpg)![](https://pic4.zhimg.com/80/v2-1d8faeb71d690d02e110c7cd1d39eed3_hd.jpg)
如果一个graph太大，可以用因子分解将 ![P=(Y)](https://www.zhihu.com/equation?tex=P%3D%28Y%29) 写为若干个联合概率的乘积。咋分解呢，将一个图分为若干个“小团”，注意每个团必须是“最大团”（就是里面任何两个点连在了一块，具体……算了不解释，就是最大连通子图），则有：
...
所以判别式模型的特征总结如下：
- 对 ![P(Y|X)](https://www.zhihu.com/equation?tex=P%28Y%7CX%29) 建模
...
所以生成式总结下有如下特点：
- 对 ![P(X,Y)](https://www.zhihu.com/equation?tex=P%28X%2CY%29) 建模
- 这里我们主要讲分类问题，所以是要对每个label（ ![y_{i}](https://www.zhihu.com/equation?tex=y_%7Bi%7D+) ）都需要建模，最终选择最优概率的label为结果，所以没有什么判别边界。（对于序列标注问题，那只需要构件一个model）
- 中间生成联合分布，并可生成采样数据。
...
![](https://pic3.zhimg.com/50/v2-9dfdfb246a1e6922ff8835d7fdf45e05_hd.jpg)![](https://pic3.zhimg.com/80/v2-9dfdfb246a1e6922ff8835d7fdf45e05_hd.jpg)
## **2.3 序列建模**
为了号召零门槛理解，现在解释如何为序列问题建模。
![](https://pic4.zhimg.com/50/v2-5358abb09290b93a5642fd81702dfe41_hd.jpg)![](https://pic4.zhimg.com/80/v2-5358abb09290b93a5642fd81702dfe41_hd.jpg)
...
所以图看起来是这样的：
![](https://pic4.zhimg.com/50/v2-d4077c2dbd9899d8896751a28490c9c7_hd.jpg)![](https://pic4.zhimg.com/80/v2-d4077c2dbd9899d8896751a28490c9c7_hd.jpg)
...
下面来点高层次的理解：
- 根据概率图分类，可以看到HMM属于有向图，并且是生成式模型，直接对联合概率分布建模 ![P(O,I) = \sum_{t=1}^{T}P(O_{t} | O_{t-1})P(I_{t} | O_{t})](https://www.zhihu.com/equation?tex=P%28O%2CI%29+%3D+%5Csum_%7Bt%3D1%7D%5E%7BT%7DP%28O_%7Bt%7D+%7C+O_%7Bt-1%7D%29P%28I_%7Bt%7D+%7C+O_%7Bt%7D%29) (注意，这个公式不在模型运行的任何阶段能体现出来，只是我们都去这么来表示HMM是个生成式模型，他的联合概率 ![P(O,I)](https://www.zhihu.com/equation?tex=P%28O%2CI%29) 就是这么计算的)。
- 并且B中 ![b_{ij} = P(o_{t}|i_{t})](https://www.zhihu.com/equation?tex=b_%7Bij%7D+%3D+P%28o_%7Bt%7D%7Ci_%7Bt%7D%29) ，这意味着o对i有依赖性。
- 在A中， ![a_{ij} = P(i_{t+1}|i_{t})](https://www.zhihu.com/equation?tex=a_%7Bij%7D+%3D+P%28i_%7Bt%2B1%7D%7Ci_%7Bt%7D%29) ，也就是说只遵循了一阶马尔科夫假设，1-gram。试想，如果数据的依赖超过1-gram，那肯定HMM肯定是考虑不进去的。这一点限制了HMM的性能。
## **3.2 模型运行过程**
...
比如说，在代码里计算完了就是这样的： 
![](https://pic1.zhimg.com/50/v2-5343666b942f952f6c79c4fa5e2dfdd9_hd.jpg)![](https://pic1.zhimg.com/80/v2-5343666b942f952f6c79c4fa5e2dfdd9_hd.jpg)![](https://pic4.zhimg.com/50/v2-0d695539c785fe16cfcfff3b9bd0c164_hd.jpg)![](https://pic4.zhimg.com/80/v2-0d695539c785fe16cfcfff3b9bd0c164_hd.jpg)![](https://pic2.zhimg.com/50/v2-443053bd4342c71bf602ac15aa7c8731_hd.jpg)![](https://pic2.zhimg.com/80/v2-443053bd4342c71bf602ac15aa7c8731_hd.jpg)
**2. Baum-Welch(前向后向)**
...
![](https://pic1.zhimg.com/50/v2-71f1ea9abbab357f7d9bad1138ee7344_hd.jpg)![](https://pic1.zhimg.com/80/v2-71f1ea9abbab357f7d9bad1138ee7344_hd.jpg)
...
![](https://pic4.zhimg.com/50/v2-cb2cc25593fcaf06e682191d551ba03b_hd.jpg)![](https://pic4.zhimg.com/80/v2-cb2cc25593fcaf06e682191d551ba03b_hd.jpg)
MEMM需要两点注意：
- 与HMM的 ![o_{i}](https://www.zhihu.com/equation?tex=o_%7Bi%7D) 依赖 ![i_{i}](https://www.zhihu.com/equation?tex=i_%7Bi%7D) 不一样，MEMM当前隐藏状态 ![i_{i}](https://www.zhihu.com/equation?tex=i_%7Bi%7D) 应该是依赖当前时刻的观测节点 ![o_{i}](https://www.zhihu.com/equation?tex=o_%7Bi%7D) 和上一时刻的隐藏节点 ![i_{i-1}](https://www.zhihu.com/equation?tex=i_%7Bi-1%7D)
...
**1. 现象**
是从街货上烤过来的…… 
![](https://pic4.zhimg.com/50/v2-40f9945cdffb12cfec84bebc7b7e3be5_hd.jpg)![](https://pic4.zhimg.com/80/v2-40f9945cdffb12cfec84bebc7b7e3be5_hd.jpg)
...
![](https://pic3.zhimg.com/50/v2-c5e2e782e35f6412ed65e58cdda0964e_hd.jpg)![](https://pic3.zhimg.com/80/v2-c5e2e782e35f6412ed65e58cdda0964e_hd.jpg)
...
![](https://pic3.zhimg.com/50/v2-376fd85a490e161978130ddd759244d4_hd.jpg)![](https://pic3.zhimg.com/80/v2-376fd85a490e161978130ddd759244d4_hd.jpg)
