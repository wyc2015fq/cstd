# Tensorflow一些常用基本概念与函数（4） - YZXnuaa的博客 - CSDN博客
2018年03月30日 00:23:57[YZXnuaa](https://me.csdn.net/YZXnuaa)阅读数：131
...
- 1
- 2
然后，可以设置 一个用于记录全局训练步骤的单值。以及使用minimize()操作，该操作不仅可以优化更新训练的模型参数，也可以为全局步骤(global step)计数。与其他tensorflow操作类似，这些训练操作都需要在[tf.session](http://blog.csdn.net/lenbow/article/details/52181159)会话中进行
```
...
|class tf.train.AdadeltaOptimizer|使用[Adadelta算法](http://arxiv.org/abs/1212.5701)的Optimizer|
...
|class tf.train.AdagradOptimizer|使用[Adagrad算法](http://www.jmlr.org/papers/volume12/duchi11a/duchi11a.pdf)的Optimizer|
...
|class tf.train.MomentumOptimizer|使用[Momentum](http://www.cs.toronto.edu/~tijmen/csc321/slides/lecture_slides_lec6.pdf)算法的Optimizer|
...
|class tf.train.AdamOptimizer|使用[Adam 算法](http://arxiv.org/abs/1412.6980)的Optimizer|
...
|class tf.train.FtrlOptimizer|使用[FTRL 算法](https://www.eecs.tufts.edu/~dsculley/papers/ad-click-prediction.pdf)的Optimizer|
...
|class tf.train.RMSPropOptimizer|使用[RMSProp算法](http://www.cs.toronto.edu/~tijmen/csc321/slides/lecture_slides_lec6.pdf)的Optimizer|
...
▷ *[tf.train.AdamOptimizer](http://arxiv.org/pdf/1412.6980.pdf)*
Adam 的基本运行方式，首先初始化：
```
...
- 3
- 4
在[论文](http://arxiv.org/pdf/1412.6980.pdf)中的 section2 的末尾所描述了更新规则，该规则使用梯度g来更新变量：
```
t <- t + 1
...
查看[queue](http://blog.csdn.net/lenbow/article/details/52181159)中，queue相关的内容，了解tensorflow中队列的运行方式。
|操作|描述|
|----|----|
...
#class tf.train.Supervisor一个训练辅助器，用于checkpoints模型以及计算的summaries。该监视器只是一个小的外壳(wrapper),用于Coordinator, a Saver, 和a SessionManager周围tf.train.Supervisor.__init__(graph=None, ready_op=0, is_chief=True, init_op=0, init_feed_dict=None, local_init_op=0, logdir=None, 
...
|操作|描述|
|----|----|
|tf.scalar_summary(tags, values, collections=None, name=None)|输出一个标量值的summary协议buffertag的shape需要与values的相同，用来做summaries的tags，为字符串|
...
▶▶*将记录汇总写入文件中(Adding Summaries to Event Files)*|操作|描述|
|----|----|
...
|tf.train.LooperThread.loop(coord, timer_interval_secs, target, args=None, kwargs=None)|启动一个LooperThread，用于周期地调用某个函数调用函数target(args)|
...
[8] Tensorflow一些常用基本概念与函数（2） [http://blog.csdn.net/lenbow/article/details/52181159](http://blog.csdn.net/lenbow/article/details/52181159)
[9] Tensorflow一些常用基本概念与函数（3） [http://blog.csdn.net/lenbow/article/details/52213105](http://blog.csdn.net/lenbow/article/details/52213105)
