# ML二：NNSearch数据结构--二叉树 - wishchinYang的专栏 - CSDN博客
2014年01月18日 15:48:00[wishchin](https://me.csdn.net/wishchin)阅读数：3493
wiki百科：[http://zh.wikipedia.org/wiki/%E5%86%B3%E7%AD%96%E6%A0%91%E5%AD%A6%E4%B9%A0](http://zh.wikipedia.org/wiki/%E5%86%B3%E7%AD%96%E6%A0%91%E5%AD%A6%E4%B9%A0)
opencv学习笔记--二杈决策树：[http://blog.csdn.net/homechao/article/details/9061921](http://blog.csdn.net/homechao/article/details/9061921)
### （1）:从K近邻算法、距离度量谈到KD树、SIFT+BBF算法：[http://blog.csdn.net/v_july_v/article/details/8203674](http://blog.csdn.net/v_july_v/article/details/8203674)
### 前言
    前两日，在[微博](http://weibo.com/1580904460/z5AvZoes0)上说：“到今天为止，我至少亏欠了3篇文章待写：1、[KD树](http://blog.csdn.net/v_july_v/article/details/8203674)；2、神经网络；3、编程艺术第28章。你看到，blog内的文章与你于别处所见的任何都不同。于是，等啊等，等一台电脑，只好等待..”。得益于田，借了我一台电脑（借他电脑的时候，我连表示感谢，他说“能找到工作全靠你的博客，这点儿小忙还说，不地道”，有的时候，稍许感受到受人信任也是一种压力，愿我不辜负大家对我的信任），于是今天开始[Top
 10 Algorithms in Data Mining](http://blog.csdn.net/v_july_v/article/category/1061301)系列第三篇文章，即本文「从K近邻算法谈到KD树、SIFT+BBF算法」的创作。
...
    而关于[R树](http://blog.csdn.net/v_JULY_v/article/details/6530142)本blog内之前已有介绍(同时，关于基于R树的最近邻查找，还可以看下这篇文章：[http://blog.sina.com.cn/s/blog_72e1c7550101dsc3.html](http://blog.sina.com.cn/s/blog_72e1c7550101dsc3.html))，本文着重介绍k-d树。
...
- 确定：split域=x。具体是：6个数据点在x，y维度上的数据方差分别为39，28.63，所以在x轴上[方差](http://zh.wikipedia.org/zh/%E6%96%B9%E5%B7%AE)更大，故split域值为x；
...
- return kd_node;  
- }  
- staticvoid expand_kd_node_subtree( struct kd_node* kd_node )  
- {  
- /* base case: leaf node */
...
    也就是说，跟普通二叉树(包括如下图所示的[红黑树](http://blog.csdn.net/v_JULY_v/article/details/6105630))结点的删除是同样的思想：用被删除节点A的左子树的最右节点或者A的右子树的最左节点作为替代A的节点(比如，下图红黑树中，若要删除根结点26，第一步便是用23或28取代根结点26)。
> 
> 
...
    一般来讲，最临近搜索只需要检测几个叶子结点即可，如下图所示：![](file:///C:%5CUsers%5CLaoT%5CAppData%5CLocal%5CTemp%5CTempPic%5C%288F0M]G6_XTI%7EM4_RH%28T]EI.tmp)
> 
> 
> 
> 
...
    但是，如果当实例点的分布比较糟糕时，几乎要遍历所有的结点，如下所示：![](file:///C:%5CUsers%5CLaoT%5CAppData%5CLocal%5CTemp%5CTempPic%5CJ%7D%29%29%28$%7DKM8NF$AGDD%%60T29F.tmp)
> 
> 
> 
> 
...
如果不相交，向上回溯。
- 当**回退到根结点时，搜索结束**，最后的“当前最近点”即为x 的最近邻点。
...
    当然，如果你觉得上述论文还不够满足你胃口的话，这里有一大堆nearest neighbor algorithms相关的论文可供你看：[http://scholar.google.com.hk/scholar?q=nearest+neighbor+algorithms&btnG=&hl=zh-CN&as_sdt=0&as_vis=1](http://scholar.google.com.hk/scholar?q=nearest+neighbor+algorithms&btnG=&hl=zh-CN&as_sdt=0&as_vis=1)（其中，这篇可以看下：Spill-Trees，An
...
- [九、图像特征提取与匹配之SIFT算法](http://blog.csdn.net/v_JULY_v/article/details/6186942)      (sift算法系列五篇文章)
...
    不熟悉SIFT算法相关概念的可以看上述几篇文章，这里不再做赘述。与此同时，本文此部分也作为[十五个经典算法研究系列](http://blog.csdn.net/v_JULY_v/article/details/6305212)里SIFT算法的九之四续。
...
函数一的声明：
- externint sift_features( IplImage* img, struct feature** feat );  
函数一的实现：
...
函数二的声明：
- externint _sift_features( IplImage* img, struct feature** feat, int intvls,  
...
- elseif((*feat)[i].dense == 3) ++n3;  
- elseif((*feat)[i].dense == 2) ++n2;  
- elseif((*feat)[i].dense == 1) ++n1;  
- else                         ++n0;  
-     }  
- 
...
    上文中一直在讲最近邻问题，也就是说只找最近的那唯一一个邻居，但如果现实中需要我们找到k个最近的邻居。该如何做呢？对的，之前blog内曾相近阐述过[寻找最小的k个数](http://blog.csdn.net/v_JULY_v/article/details/6370650)的问题，显然，寻找k个最近邻与寻找最小的k个数的问题如出一辙。
...
- staticvoid restore_minpq_order( struct pq_node* pq_array, int i, int n )  
- {                                                          //0     //0
- struct pq_node tmp;  
- int l, r, min = i;  
- 
...
- staticstruct kd_node* explore_to_leaf( struct kd_node* kd_node, struct feature* feat,  
...
### 3.3、SIFT+BBF算法匹配效果
    之前试了下sift + KD + BBF算法，用两幅不同的图片做了下匹配（当然，运行结果显示是不匹配的），效果还不错：[http://weibo.com/1580904460/yDmzAEwcV#1348475194313](http://weibo.com/1580904460/yDmzAEwcV#1348475194313)。
> 
![](https://img-my.csdn.net/uploads/201209/24/1348475394_9590.jpg)
    “编译的过程中，直接用的VS2010 + opencv（并没下gsl）。2012.09.24”。....
    本文完整源码有pudn帐号的朋友可以前去这里下载：[http://www.pudn.com/downloads340/sourcecode/graph/texture_mapping/detail1486667.html](http://www.pudn.com/downloads340/sourcecode/graph/texture_mapping/detail1486667.html)（没有pudn账号的同学请加群：169056165，至群共享下载，验证信息：sift）。感谢诸位。
## 参考文献及推荐阅读
...
## 预告
    本文之后，待写的几篇文章罗列如下：
- [数据挖掘中所需的概率论与数理统计知识](http://blog.csdn.net/v_july_v/article/details/8308762)，估计年底前完成（更新：已经完成，点击左边文字链接即可打开）；
- 机器学习中相关的降维方法，如PCA/LDA等等，明年年初完成；
- 神经网络入门学习导论，明年1月内完成；
- 程序员编程艺术第二十八章，时间待定；
- ...
    欢迎期待。July、二零一二年十二月十七日。
更多[3](http://blog.csdn.net/v_july_v/article/details/8203674#)
- 上一篇：[九月十月百度人搜，阿里巴巴，腾讯华为笔试面试八十题(第331-410题)](http://blog.csdn.net/v_july_v/article/details/7974418)
- 下一篇：[数据挖掘中所需的概率论与数理统计知识、上](http://blog.csdn.net/v_july_v/article/details/8308762)
- 顶
64- 踩
2
查看评论
- 
51楼 [x_hh](http://blog.csdn.net/Xu_Haihua)2013-12-26 17:46发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/F/7/1/3_xu_haihua.jpg)6个数据点在x，y维度上的数据方差分别为39，28.63，这两个数据怎么算出来的，我看网上好多都是这个数据，今天还和人讨论着，就是算不出来，不知道是网上的数据错的还是我们的方法错了，大侠指点一下- 
50楼 [瓜瓜东西](http://blog.csdn.net/jintianhen1)2013-12-07 22:45发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/C/6/C/3_jintianhen1.jpg)虽然我看不到，但我知道楼主很牛掰- 
49楼 [zhanglibin_1222](http://blog.csdn.net/zhanglibin_1222)2013-12-07 11:02发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/D/F/3_zhanglibin_1222.jpg)支持，大力支持。- 
48楼 [一生爱花](http://blog.csdn.net/u012710853)2013-11-03 21:54发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/0/2/B/3_u012710853.jpg)求博主联系方式，有很多问题想请教！最好有QQ
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2013-11-04 13:43发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复u012710853：你好，咱们可以在微博私信上联系：http://weibo.com/julyweibo，多谢关注。- 
47楼 [thinker](http://blog.csdn.net/qq1184810369)2013-10-30 22:31发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/0/8/D/3_qq1184810369.jpg)
- If Dist（target[s]，back_point -> Node-data[s]） < Max_dist   //判断还需进入的子空间  
-    　　　　If target[s] <= back_point -> Node-data[s]    
-    　　　　　　Kd_point = back_point -> right；  //如果target位于左子空间，就应进入右子空间  
- else
...
46楼 [youlan9115](http://blog.csdn.net/u011378543)2013-10-25 22:38发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/A/0/5/3_u011378543.jpg)楼主好强大！我决定要把你的博文通通学习一遍~~~- 
45楼 [fengjianbang](http://blog.csdn.net/fengjianbang)2013-09-16 14:57发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/D/8/3/3_fengjianbang.jpg)楼主，您好，最近您这篇文章看了很多遍，但是一直有一个问题不能理解。就是kd树里面确定分割维的时候，你的那个方差值是怎么算出来的，还请赐教啊。- 
44楼 [xxc1605629895](http://blog.csdn.net/xxc1605629895)2013-09-11 09:56发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/4/4/3_xxc1605629895.jpg)大神，请问 k-d树查询算法中回溯的时候用到的 Max_dist 是什么呢？- 
43楼 [wzlang](http://blog.csdn.net/wzlang)2013-09-05 09:20发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/D/F/3_wzlang.jpg)博主您好，小弟使用KD-BBF算法用于SIFT时发现速度很慢，比穷举法要慢很多很多。- 
42楼 [catTom](http://blog.csdn.net/xishisugan)2013-08-26 02:20发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/3/3/E/3_xishisugan.jpg)看您把问题都研究挺深入的，可以写一下后缀数组吗？我对于后缀数组的计数排序实现很不明白- 
41楼 [JackDanny](http://blog.csdn.net/u011340930)2013-08-05 20:06发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/6/D/2/3_u011340930.jpg)很喜欢博主的文章，刚刚用豆约翰博客备份专家备份了您的全部博文。- 
40楼 [MATH_F](http://blog.csdn.net/u011585849)2013-08-03 00:50发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/6/E/F/3_u011585849.jpg)或者说 为什么kd树中删除的时候一会儿比较x 一会人比较y 有什么标准嘛 以及 那个示例好像不是kd树啊。。- 
39楼 [MATH_F](http://blog.csdn.net/u011585849)2013-08-03 00:19发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/6/E/F/3_u011585849.jpg)KD树的删除
它得替代节点要么是（a,b）左子树中的X坐标最大值的结点，要么是（a,b）右子树中x坐标最小值的结点
这个是不是写错了
右子树种Y最小吧 是嘛？- 
38楼 [孟小子](http://blog.csdn.net/meng_zy)2013-07-07 11:48发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/F/5/3/3_meng_zy.jpg)很喜欢博主的文章，刚刚用豆约翰博客备份专家备份了您的全部博文。- 
37楼 [见死不救No1](http://blog.csdn.net/renhuanzhu)2013-06-02 20:08发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/0/6/4/3_renhuanzhu.jpg)楼主真厉害，学习了！- 
36楼 [wz88423511](http://blog.csdn.net/wz88423511)2013-05-23 15:33发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/B/7/8/3_wz88423511.jpg)http://www.cs.umd.edu/class/spring2008/cmsc420/L19.kd-trees.pdf
你怎么看待不存在右子树的时候寻找左子树最大值的问题- 
35楼 [wz88423511](http://blog.csdn.net/wz88423511)2013-05-23 15:33发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/B/7/8/3_wz88423511.jpg)http://www.cs.umd.edu/class/spring2008/cmsc420/L19.kd-trees.pdf
文章中提到了关于右子树不存在的情况下寻找左子树中的最大值可能存在的问题，求赐教，你怎么看待这个问题- 
34楼 [wz88423511](http://blog.csdn.net/wz88423511)2013-05-23 15:31发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/B/7/8/3_wz88423511.jpg)http://www.cs.umd.edu/class/spring2008/cmsc420/L19.kd-trees.pdf
...
求指教 ，你怎么看待这个问题- 
33楼 [燎原](http://blog.csdn.net/gao1511)2013-05-03 10:43发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/4/1/D/3_gao1511.jpg)博主，您在K-D树如何构建的那个例子中，举例有6个点，(2,3)、(5,4)。。。它们在X,Y维度上的方差是怎样计算的啊？就是用X方向上的坐标值求得平均值，再按照方差的计算公式去求吗？可是我算出来的数据跟您的差别好大好大啊，还烦请博主详述一下吧。。。我真的是小白。。。- 
32楼 [奇异果](http://blog.csdn.net/liukang0618)2013-04-30 13:06发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/C/F/9/3_liukang0618.jpg)在选择用哪个维度切分的时候，是顺序循环选择，还是每次选择都选择方差最大的呢？如5维(a,b,c,d,e)，是abcdeabcdeab...这样选，还是每次都计算方差选最大的？
再有，允许相邻两次选择同一维度进行划分吗，如abb这种？
- 
Re: [wz88423511](http://blog.csdn.net/wz88423511)2013-05-23 15:35发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/B/7/8/3_wz88423511.jpg)回复liukang0618：采用方差最大。是为了找到点分布最分散的维度，这样可以平衡做右子树，所以可以连续对同一维度进行切割的。- 
31楼 [飞信天下](http://blog.csdn.net/google0802)2013-03-25 17:26发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/9/E/A/3_google0802.jpg)楼主，您好！我有个问题请教您哈！
...
- 
Re: [Day_plot](http://blog.csdn.net/Day_plot)2013-10-26 11:48发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/5/F/6/3_day_plot.jpg)回复google0802：按照Rob Hess的代码，其实他已经将kd-tree的构造改了，和博主介绍的不一样，改成了将所有的数据都设成了叶子节点。你用上面的例子构造一个kd-tree，打印出所有节点就知道了
- 
Re: [飞信天下](http://blog.csdn.net/google0802)2013-10-30 21:15发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/9/E/A/3_google0802.jpg)回复Day_plot：你的意思是说rob hess的代码的kd树把所有的数据当为叶子？我试一试打印所有叶子，谢谢你哈！- 
30楼 [daduzifufu](http://blog.csdn.net/daduzifufu)2013-01-16 21:46发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/3/1/D/3_daduzifufu.jpg)学习的近似误差, 学习的估计误差区别是什么?- 
29楼 [heroalone2012](http://blog.csdn.net/heroalone2012)2013-01-10 16:09发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/C/C/9/3_heroalone2012.jpg)博主你好，最近一直在学习你这篇博文，受益匪浅
但我有个弱弱的问题，希望博主指点。
...
最近被导师盯着做点云的k近邻，我导师说了，寒假前如果做不出，就不用回家过年了。。。。
跪求指点啊！！！- 
28楼 [visionfans](http://blog.csdn.net/visionfans)2012-12-28 00:15发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/8/E/5/3_visionfans.jpg)好文章，加油- 
27楼 [fengzhe0411](http://blog.csdn.net/fengzhe0411)2012-12-22 15:53发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/9/A/7/3_fengzhe0411.jpg)随着树的深度增加，循环的选取坐标轴，作为分割超平面的法向量。对于3-d tree来说，根节点选取x轴，根节点的孩子选取y轴，根节点的孙子选取z轴，根节点的曾孙子选取x轴，这样循环下去。
到底是按照方差最大的选取划分维度，还是按x，y，x循环- 
26楼 [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-12-17 11:56发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)预告
本文之后，待写的几篇文章罗列如下：
...
5、...
欢迎期待。July、二零一二年十二月十七日。- 
25楼 [swuxd](http://blog.csdn.net/swuxd)2012-12-11 20:29发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/8/D/B/3_swuxd.jpg)非常好评论一下
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-12-12 23:13发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复swuxd：thank you，文章2.6节、kd树近邻搜索算法的改进：BBF算法 刚做了再次修改，有何问题，欢迎各位读者不吝指出，谢谢。- 
24楼 [lvjun93](http://blog.csdn.net/lvjun93)2012-12-10 19:41发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/B/E/F/3_lvjun93.jpg)楼主，你在写完svm和knn之后，能否解释一下2012百度机器学习与数据挖掘校招的笔试题最后一题？？
...
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-12-11 00:28发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复lvjun93：建议你先去了解一下这几个算法：
...
23楼 [empty16](http://blog.csdn.net/empty16)2012-12-09 13:01发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/6/A/8/3_empty16.jpg)楼主博文观点思路很明确，学习目标也很清晰，目标坚定，道路清晰。给我们这些学习之人带来很多思路，讲解了很多好多算法，谢谢楼主- 
22楼 [lovingdiana](http://blog.csdn.net/lovingdiana)2012-12-05 15:39发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/8/1/1/3_lovingdiana.jpg)正好在弄SIFT算法和KD-TREE搜索算法，楼主的博文实在是对我帮助很大，谢谢哈！期待您更好的博文，不知道楼主有没有关于SIFT改进算法的研究？
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-12-05 19:29发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复lovingdiana：SIFT改进算法，以后有机会研究下:-)- 
21楼 [jAmEs_](http://blog.csdn.net/jAmEs_)2012-12-04 17:31发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/C/F/F/3_james_.jpg)很强大，要看需要慢慢消化- 
20楼 [maxiao001](http://blog.csdn.net/maxiao001)2012-12-01 20:51发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/9/F/1/3_maxiao001.jpg)楼主确实全程高能。- 
19楼 [nmcfyangyang](http://blog.csdn.net/nmcfyangyang)2012-12-01 18:46发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/2/2/0/3_nmcfyangyang.jpg)2.确定：Node-data = （7,2）。具体是：根据x维上的值将数据排序，6个数据的中值为7，所以Node-data域位数据点（7,2）。这样，该节点的分割超平面就是通过（7,2）并垂直于：split=x轴的直线x=7；
这段中，提到6个数据的中值是7，中值是中位数的意思吧？应该是6吧！？
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-12-01 22:03发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复nmcfyangyang：“6个二维数据点{(2,3)，(5,4)，(9,6)，(4,7)，(8,1)，(7,2)}，根据x维上的值将数据排序，6个数据的中值为7”
这里所谓的中值非中位数的意思，而是中间大小的值，6个数据点按照x维上的值排序后，是{(2,3)，(4,7)，(5,4)，(7,2)，(8,1)，(9,6)}，所以中值为点(7,2)中的7（不存在x值为6的点）。- 
18楼 [天才在左疯子在右](http://blog.csdn.net/fengzi2009F)2012-11-29 21:14发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/C/3/7/3_fengzi2009f.jpg)有点小深奥- 
17楼 [meiguang1028](http://blog.csdn.net/meiguang1028)2012-11-29 15:00发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/1/2/E/3_meiguang1028.jpg)最近也在做关于应用knn算法的研究，读了一下博主的文章，感觉把KNN算法介绍的非常通俗。但是我觉得可以再介绍一些nonparametric的内容，毕竟knn是nonparametric的一种方法，以及knn为何有效（也许这部分内容需要一些数学证明，为了通俗，所以博主省略了）。还有就是距离的选择应该是根据应用场合而定，个人感觉是这样的。所以觉得没有比较利用这么大的篇幅来介绍。最后想说的是，非常喜欢博主写的文章，以后多向博主学习，希望看到更多的技术文章。
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-29 20:26发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复meiguang1028：非常好的建议，谢谢！我也了解下nonparametric，你有相关的资料介绍么？
- 
Re: [meiguang1028](http://blog.csdn.net/meiguang1028)2012-11-30 13:19发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/1/2/E/3_meiguang1028.jpg)回复v_JULY_v：pattern classification, 很经典的一个教材，里头nonparametric章节介绍的比较详细。
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-30 14:23发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复meiguang1028：好！是模式分类那一本书吧，晚上回去一定好好看下- 
16楼 [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-28 21:10发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)一群友欲编译本文源码，但不知”宏”的定义，后知刚学编程。另一群友提示说，学编程要从基础一点一点开始，不要一上来就搞算法，偶强烈赞同。自己09年10月C、C++、数据结构，10年底算法，12年5月开始DM&ML，近期遂补数学。何谓基础，就是你我容易忽略的简单的初级的东西，少说也得花心思好好学3年。- 
15楼 [江南烟雨](http://blog.csdn.net/xiajun07061225)2012-11-27 16:07发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/C/0/4/3_xiajun07061225.jpg)膜拜一下~学习一下，算法很难，也很重要~明年找工作之前一定要把这里面的算法都自己实现一遍~~
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-27 18:28发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复xiajun07061225：期待你自己的实现~- 
14楼 [UESTC_HN_AY_GUOBO](http://blog.csdn.net/UESTC_HN_AY_GUOBO)2012-11-26 10:28发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/1/E/6/3_uestc_hn_ay_guobo.jpg)首先 谢谢博主的劳动
...
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-27 10:56发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复UESTC_HN_AY_GUOBO：谢谢。
“如果把空间数据进行了kdtree 或者 mtree的索引 ，然后使用knn 那么与直接使用knn算法在空间数据上 结果会有不同吗？”
我觉得你的问题等价于：索引有用么？- 
13楼 [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-26 00:19发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)全文第二次修订完毕，补充了kd树的插入，删除情况的图例分析。- 
12楼 [guaimaer](http://blog.csdn.net/guaimaer)2012-11-24 17:01发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/F/A/F/3_guaimaer.jpg)顶礼膜拜中- 
11楼 [mandycool](http://blog.csdn.net/mandycool)2012-11-24 10:48发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/7/B/6/3_mandycool.jpg)kd树做高维数据还是很困难，即使加入bbf，现在用的较多的是randomized kd-tree，lz也可添加介绍。将继续关注lz~~
- 
Re: [UESTC_HN_AY_GUOBO](http://blog.csdn.net/UESTC_HN_AY_GUOBO)2012-11-26 10:31发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/1/E/6/3_uestc_hn_ay_guobo.jpg)回复mandycool：现在主要是在kdtree的基础上有了mtree或者mvptree
其实关键还是pivot的选择
度量空间中算法还是怎么能减少 距离计算
使用度量空间的法则 过滤距离计算
- 
Re: [mandycool](http://blog.csdn.net/mandycool)2012-11-26 15:56发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/7/B/6/3_mandycool.jpg)回复UESTC_HN_AY_GUOBO：大致查了一下，mtree和mvp-tree是2000年前提出的吧，感觉是比较老的技术了；现在在kd树的基础上，有做基变换的方法，有做pca的方法；根据flann作者的说法，multi randomized kd-tree经过比较是比较高效的了；另外还有在每一层上做聚类的方法，据说效果也不错；如有误，欢迎指正
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-26 18:37发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复mandycool：有相关的资料链接或论文么？
- Re: [mandycool](http://blog.csdn.net/mandycool)2012-11-26 19:31发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/7/B/6/3_mandycool.jpg)回复v_JULY_v：FAST APPROXIMATE NEAREST NEIGHBORS
WITH AUTOMATIC ALGORITHM CONFIGURATION
- Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-26 19:39发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复mandycool：建议你写一篇blog记录下学习心得或体会哈
- Re: [mandycool](http://blog.csdn.net/mandycool)2012-11-26 19:47发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/7/B/6/3_mandycool.jpg)回复v_JULY_v：自己比较浮躁，没法写的像lz这么细致，还配图的；等做完毕设看能不能写点吧。。。
- Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-26 19:56发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复mandycool：好的，期待! 到时候向你文章学习
- Re: [](http://blog.csdn.net/mandycool)mandycool2012-11-26 21:07发表[](http://blog.csdn.net/v_july_v/article/details/8203674#reply)[回复]
[](http://blog.csdn.net/mandycool)![](http://avatar.csdn.net/7/B/6/3_mandycool.jpg)回复v_JULY_v：lz的博文写的很好，我才该多学习，哈哈- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-26 10:54发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复UESTC_HN_AY_GUOBO：针对你说的
“关键还是pivot的选择
度量空间中算法还是怎么能减少 距离计算
使用度量空间的法则 过滤距离计算”
你有何体会或使用心得么？
- 
Re: [mandycool](http://blog.csdn.net/mandycool)2012-11-26 19:41发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/7/B/6/3_mandycool.jpg)回复v_JULY_v：刚看了一下mvp-tree，是利用三角形不等式来缩小搜索区域的，不过mvp-tree的目标稍有不同，查询的是到query点的距离小于某个值r的点；另外作者test的数据集只有20维，不知道上百维以后效果如何
- Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-30 22:25发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复mandycool：补充了球树，VP树，MVP树的相关简介，资料还是有限，无法进一步深入..
- Re: [mandycool](http://blog.csdn.net/mandycool)2012-11-30 23:48发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/7/B/6/3_mandycool.jpg)回复v_JULY_v：Distance-based indexing for high-dimensional metric spaces
vp-tree和mvp-tree均可参见此篇，中文资料确实很少
- Re: [](http://blog.csdn.net/v_JULY_v)v_JULY_v2012-12-01 11:20发表[](http://blog.csdn.net/v_july_v/article/details/8203674#reply)[回复]
[](http://blog.csdn.net/v_JULY_v)![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复mandycool：网络还真不给力，半天下不下来，打不开- 
Re: [mandycool](http://blog.csdn.net/mandycool)2012-11-26 15:58发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/7/B/6/3_mandycool.jpg)回复v_JULY_v：减少距离计算的一个思路是做embedding，通过不等式排除掉一部分点- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-24 11:44发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复mandycool：好，非常感谢，我了解下randomized kd-tree !- 
10楼 [leifeng457327997](http://blog.csdn.net/leifeng457327997)2012-11-24 00:13发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/0/B/F/3_leifeng457327997.jpg)太深奥了- 
9楼 [Felven](http://blog.csdn.net/jj12345jj198999)2012-11-23 16:58发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/A/0/5/3_jj12345jj198999.jpg)最近上研究生学 模式识别，机器学习，人工智能，再加上自己在实验室帮老师搞数据挖掘，觉得这些技术很不错，学好绝对有很大帮助。
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-23 17:01发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复jj12345jj198999：研究生有机会在实验室里 做各种实验，真心幸福丫- 
8楼 [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-23 12:11发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)OK，本文基本修订完毕。
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-23 16:54发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复v_JULY_v：补充了本文完整源码的下载地址。- 
7楼 [无为Rookie_CEO](http://blog.csdn.net/Rookie_CEO)2012-11-22 13:07发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/7/4/3_rookie_ceo.jpg)顶一下- 
6楼 [syzcch](http://blog.csdn.net/syzcch)2012-11-21 13:07发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/8/A/8/3_syzcch.jpg)july 还是一如既往的给力
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-22 14:32发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复syzcch：多谢关注哦！- 
5楼 [SuperFC](http://blog.csdn.net/fengchaokobe)2012-11-21 09:27发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/9/0/5/3_fengchaokobe.jpg)KD树：统计、计算、挖掘！- 
4楼 [铭毅天下](http://blog.csdn.net/wojiushiwo987)2012-11-20 23:15发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/4/7/A/3_wojiushiwo987.jpg)引用“zhucegepp”的评论：博主本科？
去了什么公司？
貌似QQ，博主说过。。
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-21 00:16发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复wojiushiwo987：NO，:-)- 
3楼 [zhucegepp](http://blog.csdn.net/zhucegepp)2012-11-20 23:03发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/B/8/3/3_zhucegepp.jpg)博主本科？
去了什么公司？
- 
Re: [Mini校长](http://blog.csdn.net/somnl)2012-11-26 10:52发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/4/2/A/3_somnl.jpg)回复zhucegepp：我觉得可以去google，现在博主在上海- 
2楼 [potentialman](http://blog.csdn.net/potentialman)2012-11-20 22:53发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/7/7/5/3_potentialman.jpg)mark一下 明天考linux 考了再静心来看。
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-22 00:47发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复potentialman：文章补充了第一部分中的皮尔逊相关系数等距离度量表示法，不知你昨晚来看了没？- 
1楼 [zhangt02](http://blog.csdn.net/zhangt02)2012-11-20 22:01发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/5/7/3/3_zhangt02.jpg)之前看过局部特征不变向量那本书 你更加详细的阐述了KD树~~
- 
Re: [v_JULY_v](http://blog.csdn.net/v_JULY_v)2012-11-21 00:17发表[[回复]](http://blog.csdn.net/v_july_v/article/details/8203674#reply)
![](http://avatar.csdn.net/E/0/C/3_v_july_v.jpg)回复zhangt02：EN，文中若有任何问题，欢迎随时赐教 & 指正。
(2):决策树 Python代码：[http://blog.csdn.net/wyb_009/article/details/9191325](http://blog.csdn.net/wyb_009/article/details/9191325)
...
**[python]**[view plain](http://blog.csdn.net/wyb_009/article/details/9191325#)[copy](http://blog.csdn.net/wyb_009/article/details/9191325#)[print](http://blog.csdn.net/wyb_009/article/details/9191325#)[?](http://blog.csdn.net/wyb_009/article/details/9191325#)
- def entropy(dataset):  
...
数据如下：
**[plain]**[view plain](http://blog.csdn.net/wyb_009/article/details/9191325#)[copy](http://blog.csdn.net/wyb_009/article/details/9191325#)[print](http://blog.csdn.net/wyb_009/article/details/9191325#)[?](http://blog.csdn.net/wyb_009/article/details/9191325#)
...
我这里测试结果如下：
![](https://img-blog.csdn.net/20130627230124265?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvd3liXzAwOQ==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/Center)
眼镜男终于可以买到合适的眼镜啦。。。
所有代码黏在下面：
**[python]**[view plain](http://blog.csdn.net/wyb_009/article/details/9191325#)[copy](http://blog.csdn.net/wyb_009/article/details/9191325#)[print](http://blog.csdn.net/wyb_009/article/details/9191325#)[?](http://blog.csdn.net/wyb_009/article/details/9191325#)
- def entropy(dataset):  
