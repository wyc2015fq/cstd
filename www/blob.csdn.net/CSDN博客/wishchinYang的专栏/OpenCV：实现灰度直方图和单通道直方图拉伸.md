# OpenCV：实现灰度直方图和单通道直方图拉伸 - wishchinYang的专栏 - CSDN博客
2015年09月15日 15:24:14[wishchin](https://me.csdn.net/wishchin)阅读数：3725
       原文链接：[http://blog.csdn.net/xiaowei_cqu/article/details/7600666](http://blog.csdn.net/xiaowei_cqu/article/details/7600666)
       本文略有修改，如有疑问或者版权问题，请移步原作者或者告知本人。
直接贴代码：
```cpp
//计算直方图
	cv::MatND colorWish::getHist(const cv::Mat &image, int  histSize)
	{
		cv::Mat im;
		if (image.channels() == 3)
			cv::cvtColor(image, im, CV_RGB2GRAY, 0);
		else		im = image;
		float r[2];	r[0] = 0;	r[1] = 255;
	...
	//直方图拉伸
	cv::Mat colorWish::stretch(const cv::Mat &image, int minValue /* = 0 */)
	{
		int histSize = 256;
		const cv::Mat hist = getHist(image, histSize);
		int mini = 0;
...
OpenCV中用CvHistogram表示多维直方图（[CvHistogram](http://www.opencv.org.cn/index.php/Cv%E5%9B%BE%E5%83%8F%E5%A4%84%E7%90%86#CvHistogram)）：
```cpp
...
直方图的应用以后再讨论。
转载请注明出处：[http://blog.csdn.net/xiaowei_cqu/article/details/7600666](http://blog.csdn.net/xiaowei_cqu/article/details/7600666)
实验代码下载：[http://download.csdn.net/detail/xiaowei_cqu/4328881](http://download.csdn.net/detail/xiaowei_cqu/4328881)
Mat格式的参考这里：[http://blog.csdn.net/xiaowei_cqu/article/details/8833799](http://blog.csdn.net/xiaowei_cqu/article/details/8833799)
