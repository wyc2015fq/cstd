# 单台服务器并发TCP连接数到底可以有多少 - Likes的博客 - CSDN博客
2019年03月21日 21:09:44[Lailikes](https://me.csdn.net/songchuwang1868)阅读数：12标签：[网络																[最大连接数																[fd																[port](https://so.csdn.net/so/search/s.do?q=port&t=blog)](https://so.csdn.net/so/search/s.do?q=fd&t=blog)](https://so.csdn.net/so/search/s.do?q=最大连接数&t=blog)](https://so.csdn.net/so/search/s.do?q=网络&t=blog)
个人分类：[网络](https://blog.csdn.net/songchuwang1868/article/category/8235292)
## [http://www.cnblogs.com/zhangkele/articles/8955051.html](http://www.cnblogs.com/zhangkele/articles/8955051.html)
## 常识一：文件句柄限制
...
分析一下
**如何标识一个TCP连接：**
系统用一个**4四元组来唯一标识一个TCP连接：{***local ip, local port,remote ip,remote port***}**。
好吧，我们拿出《UNIX网络编程：卷一》第四章中对accept的讲解来看看概念性的东西，
第二个参数cliaddr代表了客户端的ip地址和端口号。而我们作为服务端实际只使用了bind时这一个端口，
说明端口号65535并不是并发量的限制。
**server最大tcp连接数：（good！！！！！！！！）**
...
因此**最大tcp连接为**：**客户端ip数×客户端port数**，对IPV4，不考虑ip地址分类等因素，**最大tcp连接数约为：*2的32次方（ip数）×2的16次方（port数），也就是server端单机最大tcp连接数约为2的48次方。***
## 本文小结
