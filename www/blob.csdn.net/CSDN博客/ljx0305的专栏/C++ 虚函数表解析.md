# C++ 虚函数表解析 - ljx0305的专栏 - CSDN博客
2008年02月13日 22:26:00[ljx0305](https://me.csdn.net/ljx0305)阅读数：686标签：[c++																[fun																[class																[iostream																[编译器																[语言](https://so.csdn.net/so/search/s.do?q=语言&t=blog)](https://so.csdn.net/so/search/s.do?q=编译器&t=blog)](https://so.csdn.net/so/search/s.do?q=iostream&t=blog)](https://so.csdn.net/so/search/s.do?q=class&t=blog)](https://so.csdn.net/so/search/s.do?q=fun&t=blog)](https://so.csdn.net/so/search/s.do?q=c++&t=blog)
...
Base b;
Fun pFun = NULL;
cout << "虚函数表地址：" << *(int*)(&b) << endl;   //原贴中此地方写的是cout << "虚函数表地址：" << (int*)(&b) << endl;这样是错的,现以修改正确
cout << "虚函数表 — 第一个函数地址：" << *(int*)*(int*)(&b) << endl;//原贴中此地方写的是cout << "虚函数表 — 第一个函数地址：" << (int*)*(int*)(&b) << endl;这样写是错的,现以修改正确
...
![](https://p-blog.csdn.net/images/p_blog_csdn_net/haoel/15190/o_vtable1.jpg)
<?xml:namespace prefix = v />
注意：在上面这个图中，我在虚函数表的最后多加了一个结点，这是虚函数表的结束结点，就像字符串的结束符“/0”一样，其标志了虚函数表的结束。这个结束标志的值在不同的编译器下是不同的。在WinXP+VS2003下，这个值是NULL。而在Ubuntu 7.10 + Linux 2.6.22 + GCC 4.1.3下，这个值是如果1，表示还有下一个虚函数表，如果值是0，表示是最后一个虚函数表。
下面，我将分别说明“无覆盖”和“有覆盖”时的虚函数表的样子。没有覆盖父类的虚函数是毫无意义的。我之所以要讲述没有覆盖的情况，主要目的是为了给一个对比。在比较之下，我们可以更加清楚地知道其内部的具体实现。
# 一般继承（无虚函数覆盖）
下面，再让我们来看看继承时的虚函数表是什么样的。假设有如下所示的一个继承关系：
![](https://p-blog.csdn.net/images/p_blog_csdn_net/haoel/15190/o_Drawing3.jpg)
...
![](https://p-blog.csdn.net/images/p_blog_csdn_net/haoel/15190/o_Drawing4.jpg)
...
![](https://p-blog.csdn.net/images/p_blog_csdn_net/haoel/15190/o_Drawing1.jpg)
对于子类实例中的虚函数表，是下面这个样子：
![](https://p-blog.csdn.net/images/p_blog_csdn_net/haoel/15190/o_vtable4.JPG)
我们可以看到：
1）每个父类都有自己的虚表。
...
![](https://p-blog.csdn.net/images/p_blog_csdn_net/haoel/15190/o_Drawing2.jpg)
下面是对于子类实例中的虚函数表的图：
...
![](https://p-blog.csdn.net/images/p_blog_csdn_net/haoel/15190/o_vtable_vc.JPG)
# 附录二：例程
...
更多文章请访问我的Blog: [http://blog.csdn.net/haoel](http://blog.csdn.net/haoel)
个人见解，对于有数据成员的类来说其虚表为
例
#include <iostream>
using namespace std;
class Base1 {
...
            //Base2's vtable
            //pFun = (Fun)*((int*)*(int*)((int*)&d+1)+0);
            pFun = (Fun)pVtab[2][0];   //注意这里pVtab[2][0]中的2 这个数是  由于Base1有一个整形的数据成员由于指针跟整形数据成员同为4个字节所以由pVtab[1][0]变成了 pVtab[2][0]
            pFun();
            //pFun = (Fun)*((int*)*(int*)((int*)&d+1)+1);
            pFun = (Fun)pVtab[2][1];
            pFun();
