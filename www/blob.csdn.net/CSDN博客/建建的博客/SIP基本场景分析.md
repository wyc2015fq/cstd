# SIP基本场景分析 - 建建的博客 - CSDN博客
2017年02月22日 13:20:57[纪建](https://me.csdn.net/u013898698)阅读数：487
个人分类：[SIP协议](https://blog.csdn.net/u013898698/article/category/6740258)
...
![wps_clip_image-377](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-377_thumb.png)
...
1．2 业务过程和协议流程
##### Ø 注册流程：
![wps_clip_image-1045](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-1045_thumb.png)
...
##### Ø 注销流程：
![wps_clip_image-1311](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-1311_thumb.png)
（1）终端向代理服务器发送register消息注销，其头中expire字段设置为0。
（2）代理服务器在收到后送回200OK响应，并将数据库中的用户有关消息注销。
##### Ø 基本呼叫建立过程：
![wps_clip_image-1406](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-1406_thumb.png)
...
##### Ø 正常呼叫释放过程：
![wps_clip_image-1897](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-1897_thumb.png)
（1） 正常呼叫
...
（5） 代理服务器转发响应信息200OK。
##### Ø 会话更改流程：
![wps_clip_image-2048](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-2048_thumb.png)
（1）用户代理服务端和代理客户端正常通话。
（2）用户代理服务端向用户代理客户端发送Invite信息，带有新的SDP协商信息。
（3）用户处理客户端回复200OK，并将协商后的SDP信息带回。
（4）用户代理服务端发送ACK给用户代理客户端进行确认。
##### Ø 被叫忙呼叫释放：
![wps_clip_image-2186](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-2186_thumb.png)
...
##### Ø 被叫无应答流程一：
![wps_clip_image-2458](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-2458_thumb.png)
（1）用户摘机发起一路呼叫，终端代理A向该区域代理服务器发起Invite请求；
（2）代理服务器向被叫终端代理B传送Invite请求。
（3）代理服务器向终端代理A发送呼叫处理中的应答信息：100Trying。
（4）终端代理B向代理服务器送呼叫处理中的应答信息：100Trying。
...
##### Ø 被叫无应答流程二：
![wps_clip_image-2935](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-2935_thumb.png)
（1）用户摘机发起一路呼叫，终端代理A向该区域代理服务器发起Invite请求；
（2）代理服务器向被叫终端代理B传送Invite请求。
（3）代理服务器向终端代理A发送呼叫处理中的应答信息：100Trying。
（4）终端代理B向代理服务器送呼叫处理中的应答信息：100Trying。
...
##### Ø 遇忙呼叫前转：
![wps_clip_image-3326](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-3326_thumb.png)
（1）用户摘机发起一路呼叫，终端代理A向该区域代理服务器发起Invite请求；
（2）代理服务器向被叫终端代理B传送Invite请求。
（3）代理服务器向终端代理A发送呼叫处理中的应答信息：100Trying。
（4）终端代理B向代理服务器送呼叫处理中的应答信息：100Trying。
...
##### Ø 无应答呼叫前转流程：
![wps_clip_image-3923](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-3923_thumb.png)
...
##### Ø 呼叫保持：
![wps_clip_image-4651](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-4651_thumb.png)
（1）用户摘机发起一路呼叫，终端代理A向该区域的代理服务器发起Invite请求；
（2）代理服务器通过认证/计费中心确认用户认证已通过后，检查请求消息中的Via头域中是否已包含其地址。若已包含，说明发生环回，返回指示错误的应答；若没有问题，代理服务器在请求消息的Via头域插入自身地址，并向Invite消息的To域所指示的被叫终端代理B传送Invite请求。
（3）代理服务器向终端代理A发送呼叫处理中的应答信息：100Trying。
（4）终端代理B向代理服务器送呼叫处理中的应答信息：100Trying。
...
##### Ø 呼叫等待：
![wps_clip_image-5343](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-5343_thumb.png)
（1）AB正常通话。
...
##### Ø 注册流程：
![wps_clip_image-6298](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-6298_thumb.png)
以上是REGISTER包。
我们可以看到在注册的时候，终端会向代理服务器59.64.135.22发送REGISTER请求注册。
![wps_clip_image-6365](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-6365_thumb.png)
以上是REGISTEROK包。服务器返回200 OK，表示注册成功。
##### Ø 基本呼叫建立过程：
![wps_clip_image-6413](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-6413_thumb.png)
以上是INVITE包。我们可以看到在发起呼叫初期，终端向825002发出Invite的呼叫请求。
![wps_clip_image-6465](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-6465_thumb.png)
以上是Trying包，说明终端825003正在试着连通服务器，进一步转接到825002.
由于设置了自动接听，所以此次通话没有振铃的包。
![wps_clip_image-6538](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-6538_thumb.png)
这是ACK包，代表确认信号。
##### Ø 正常呼叫释放过程：
![wps_clip_image-6566](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-6566_thumb.png)
以上是BYE包。这是825002挂断后服务器向825003发送的释放呼叫信号。
##### Ø 注销流程：
![wps_clip_image-6615](https://p-blog.csdn.net/images/p_blog_csdn_net/gnuhpc/598384/o_wps_clip_image-6615_thumb.png)
