# 最小生成树-Prim算法和Kruskal算法 - PeterBishop - CSDN博客





2018年08月16日 20:50:06[PeterBishop0](https://me.csdn.net/qq_40061421)阅读数：60
...
更为实际的情景是这样的情况，在某地分布着`N`个村庄，现在需要在`N`个村庄之间修路，每个村庄之前的距离不同，问怎么修最短的路，将各个村庄连接起来。

以上这些问题都可以归纳为最小生成树问题，用正式的表述方法描述为：给定一个无方向的带权图`G=(V, E)`，最小生成树为集合`T`, `T`是以最小代价连接`V`中所有顶点所用边`E`的最小集合。 集合`T`中的边能够形成一颗树，这是因为每个节点（除了根节点）都能向上找到它的一个父节点。
...
![这里写图片描述](http://www.2cto.com/uploadfile/Collfiles/20160330/20160330093229560.png)

2 选择顶点`a`后，顶点a置成`visit`（涂黑）,计算周围与它连接的点的距离：
![这里写图片描述](http://www.2cto.com/uploadfile/Collfiles/20160330/20160330093229561.png)

3 与之相连的点距离分别为`7`,`6`,`4`，选择`C`点距离最短，涂黑`C`，同时将这条边高亮加入最小生成树：
![这里写图片描述](http://www.2cto.com/uploadfile/Collfiles/20160330/20160330093229562.png)
...
![这里写图片描述](http://www.2cto.com/uploadfile/Collfiles/20160330/20160330093229563.png)

5 接下来很明显，`d`距离`b`最短，将`d`涂黑，`bd`高亮：
![这里写图片描述](http://www.2cto.com/uploadfile/Collfiles/20160330/20160330093229564.png)

6 `f`距离`d`为`7`，距离`b`为`4`，更新它的最短距离值是`4`，所以涂黑`f`，高亮`bf`：
![这里写图片描述](http://www.2cto.com/uploadfile/Collfiles/20160330/20160330093229565.png)

7 最后只有`e`了：
![这里写图片描述](http://www.2cto.com/uploadfile/Collfiles/20160330/20160330093229566.png)
...
|12345|`typedef struct _node{``    ``int``val;   ``//长度``    ``int``start; ``//边的起点``    ``int``end;   ``//边的终点``}Node;`|
|----|----|



3 继续找到第二短的边，将`c`, `d`再放入同一个集合里：
![](http://www.2cto.com/uploadfile/Collfiles/20160330/20160330093231569.png)

4 继续找，找到第三短的边`ab`，因为`a`,`e`已经在一个集合里，再将`b`加入：
![](http://www.2cto.com/uploadfile/Collfiles/20160330/20160330093231570.png)

5 继续找，找到`b`,`e`，因为`b`,`e`已经同属于一个集合，连起来的话就形成环了，所以边`be`不加入最小生成树：
![](http://www.2cto.com/uploadfile/Collfiles/20160330/20160330093232571.png)

6 再找，找到`bc`，因为`c`,`d`是一个集合的，`a`,`b`,`e`是一个集合，所以再合并这两个集合：
![](http://www.2cto.com/uploadfile/Collfiles/20160330/20160330093232572.png)
