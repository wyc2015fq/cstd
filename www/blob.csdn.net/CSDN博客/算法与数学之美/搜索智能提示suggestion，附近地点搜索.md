# 搜索智能提示suggestion，附近地点搜索 - 算法与数学之美 - CSDN博客
2017年10月29日 00:00:00[算法与数学之美](https://me.csdn.net/FnqTyr45)阅读数：646
搜索智能提示suggestion
**题目详情**：百度搜索框中，输入“北京”，搜索框下面会以北京为前缀，展示“北京爱情故事”、“北京公交”、“北京医院”等等搜索词，输入“结构之”，会提示“结构之法”，“结构之法 算法之道”等搜索词。
请问，如何设计此系统，使得空间和时间复杂度尽量低。
![640?wx_fmt=jpeg&wxfrom=5&wx_lazy=1](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkzgmfzVtZpjH9MNkI269jAibMJtvy96Zqh8uKvDQoyib07P1anNcUtM0ibC5mw83O5EyHJoCOdibCqNyQ/640?wx_fmt=jpeg&wxfrom=5&wx_lazy=1)
...
> 
直接上**Trie树**「Trie树的介绍见：从Trie树（字典树）谈到后缀树」 +  **TOP K**「hashmap+堆，hashmap+堆
 统计出如10个近似的热词，也就是说，只存与关键词近似的比如10个热词」
...
![0?wx_fmt=png](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkzgmfzVtZpjH9MNkI269jAibjTSm88nB4CW4Bn44bE0ZUDXnEZ4Fx10WpZpiaDibO2uWb31HMianhiaJXg/0?wx_fmt=png)
...
借用上面的图，当用户输入前缀a的时候，搜索框可能会展示以a为前缀的“abcd”，“abd”等关键词，再当用户输入前缀b的时候，搜索框下面可能会提示以b为前缀的“bcd”等关键词，如此，实现搜索引擎智能提示suggestion的第一个步骤便清晰了，即用trie树存储大量字符串，当前缀固定时，存储相对来说比较热的后缀。那又如何统计热词呢？请看下文步骤二、TOP
 K算法统计热词。
####  步骤二、TOP K算法统计热词
...
> 
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkzgmfzVtZpjH9MNkI269jAib98qHUbhI8VvibDGxrrpicFuwWIqnmpOia0AtpUuk2wia8B9dSNDG4GaiaGw/0?wx_fmt=jpeg)
...
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkzgmfzVtZpjH9MNkI269jAibZworDgS4aXRjGumiaiaeqQ6GGzqvWT0FTod7UAI3Ch0eD8jvtAaetXibg/0?wx_fmt=jpeg)
...
**“****寻找热门查询，300万个查询字符串中统计最热门的10个****查询**
...
- 
**堆**排序：第二步、借助堆这个数据结构，找出Top K，时间复杂度为N‘logK。即借助堆结构，我们可以在log量级的时间内查找和调整/移动。因此，维护一个K(该题目中是10)大小的小根堆，然后遍历300万的Query，分别和根元素进行对比。所以，我们最终的时间复杂度是：O（N） + N'
 * O（logK），（N为1000万，N’为300万）。
别忘了这篇文章中所述的堆排序思路：‘维护k个元素的最小堆，即用容量为k的最小堆存储最先遍历到的k个数，并假设它们即是最大的k个数，建堆费时O（k），并调整堆(费时O（logk）)后，有k1>k2>...kmin（kmin设为小顶堆中最小元素）。继续遍历数列，每次遍历一个元素x，与堆顶元素比较，若x>kmin，则更新堆（x入堆，用时logk），否则不更新堆。这样下来，总费时O（k*logk+（n-k）*logk）=O（n*logk）。此方法得益于在堆中，查找等各项操作时间复杂度均为logk。’--第三章续、Top
 K算法问题的实现。
...
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkzgmfzVtZpjH9MNkI269jAibtOGKU2vL1XV18uQzRnXTu9dKcLh1DiaPKNibF7ERCmMXD3icF1Ip6PrmQ/0?wx_fmt=jpeg)
...
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkzgmfzVtZpjH9MNkI269jAibDK7VAoyPibZ6pl7021ZWFnFaQJQWRX7K4Nw5s4RSgQYOtn8zvThTypQ/0?wx_fmt=jpeg)
### 解法一、R树二维搜索
...
![0?wx_fmt=png](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkzgmfzVtZpjH9MNkI269jAibuateeD5Pgia6I44hFqJU64YINph16OvqQkEZwDc9FxqVV4jPoKu2B1w/0?wx_fmt=png)
...
> 
![0?wx_fmt=jpeg](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkzgmfzVtZpjH9MNkI269jAibx7DpOwlOMcHKWmD13Ld7ctBCIGGlP2m96PDicma6xiaGyzibXSqDKHfNw/0?wx_fmt=jpeg)
我来详细解释一下这张图。
- 
...
> 
![0?wx_fmt=png](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkzgmfzVtZpjH9MNkI269jAibWAoBibP6gKm1uySOfC8kauRk908PaVKiajeLOwQFnjOxVcDBQgcHt3UQ/0?wx_fmt=png)
**一棵R树满足如下的性质：**
- 
...
> 
> 
![0?wx_fmt=png](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkzgmfzVtZpjH9MNkI269jAibiajFHVrCKEGsqEvnXPfe1kPLpAKKCUQp5ppPjazBl7IGVKh7NhwzCyw/0?wx_fmt=png)
下图描述的就是在二维空间中的叶子结点所要存储的信息。
> 
> 
> 
> 
![0?wx_fmt=png](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkzgmfzVtZpjH9MNkI269jAibibF14rzUjcMrPDSpLjpGibQdjnUhwmaCmOqDsicsUY58F7fk4icovPgwBA/0?wx_fmt=png)
...
![0?wx_fmt=png](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkzgmfzVtZpjH9MNkI269jAib7azLbALpOfEI2Uia6LoLQfca6wM3EWgoMA7AZd6icFvEm32mictybUFqg/0?wx_fmt=png)
> 
> 
> 
> 
...
![0?wx_fmt=png](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkzgmfzVtZpjH9MNkI269jAibF7kq0KjHHkQ5MtRBEWK0QZ3vicjxI9rYdFnVV41HU2caXzE5KbzK5xw/0?wx_fmt=png)
> 
...
![0?wx_fmt=png](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkzgmfzVtZpjH9MNkI269jAibcyYuoWdUYQ9ehzbB68jl7ianhgXdcRp7s7LjKW58HtS1RBM856Q4UcQ/0?wx_fmt=png)
> 
划分成四块后我们可以定义这四块的值，如下（左下为00，左上为01，右下为10，右上为11）：
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkzgmfzVtZpjH9MNkI269jAiboU0x22cvpmRzRNVfWcvgN8PmRvabpMwHvVXufpFObbm9VbUpB8QhlQ/0?wx_fmt=jpeg)
这样［4，6］点的geohash值目前为 00然后再将四个小块每一块进行切割，如下：
![0?wx_fmt=png](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkzgmfzVtZpjH9MNkI269jAibOuUWQ9qPE6uFfriakeaqtLGASibZMTpozMaq2ibxZ0zLffuyyIbPC2ZjQ/0?wx_fmt=png)
这时［4，6］点位于右上区域，右上的值为11，这样［4，6］点的geohash值变为：0011继续往下做两次切分：
![0?wx_fmt=png](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkzgmfzVtZpjH9MNkI269jAibkMZH8PuyiaFicprHqg0UxI45AOUia8hIqg4T3tYw5tbqHMGUpZgAkfW2Q/0?wx_fmt=png)
![0?wx_fmt=png](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkzgmfzVtZpjH9MNkI269jAibHByRzbJl27OMBwI36B6GwOKDlXce5IeV4agVib3t7o1Y05SialzelOrQ/0?wx_fmt=png)
...
![0?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkzgmfzVtZpjH9MNkI269jAibSorTW0q0fNQTyAevrxbDldj6L8jxFZFaTU6gKVmSxkk9kV6BSqS4KA/0?wx_fmt=png)
...
**稿件一经采用，我们将奉上稿酬。**
**投稿邮箱：math_alg@163.com****商务合作：微信号hengzi5809**
