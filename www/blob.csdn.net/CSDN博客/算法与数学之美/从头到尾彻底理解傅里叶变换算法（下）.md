# 从头到尾彻底理解傅里叶变换算法（下） - 算法与数学之美 - CSDN博客
2017年10月23日 00:00:00[算法与数学之美](https://me.csdn.net/FnqTyr45)阅读数：353
> 
作者：July、dznlong  
...
第三章、复数
第四章、复数形式离散傅立叶变换
前期回顾，在上一篇：[从头到尾彻底理解傅里叶变换算法（上）](http://mp.weixin.qq.com/s?__biz=MzA5ODUxOTA5Mg==&mid=2652556071&idx=1&sn=46ee5814741810a35ede8f24021dcd9a&chksm=8b7e3a7cbc09b36a8b893c121f78093099992eb23de2eb745e12c9f9a03b475772da91bf60f9&scene=21#wechat_redirect)里，我们讲了傅立叶变换的由来、和实数形式离散傅立叶变换（Real
 DFT）俩个问题，
...
![640?wx_fmt=jpeg&wxfrom=5&wx_lazy=1](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkwtSOny4SBunNDhTQXO7UiaLKicT5y0Grvl3TJ7Xge7N0uBkWnzIIBLmCqez0vyG9wfolckhD5IphcQ/640?wx_fmt=jpeg&wxfrom=5&wx_lazy=1)
其中h表示高度，g表示重力加速度(9.8m/s2)，v表示初速度，t表示时间。现在反过来，假如知道了高度，要求计算到这个高度所需要的时间，这时我们又可以通过下式来计算：
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkwtSOny4SBunNDhTQXO7UiaL3pgNRnlmZ1ztBpic6HbibX9QYL2kUTH95hJxPVVuUywAuibvgyFaSNhZg/0?wx_fmt=jpeg)
...
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkwtSOny4SBunNDhTQXO7UiaLzBuicC9nicad5bgDXbC4YtTFChU0cd46fV5C3J6PpRVe5e047bWe4Q3A/0?wx_fmt=jpeg)
...
            Re C = 3      Im C = -7 
       复数之间也可以进行加减乘除运算：
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkwtSOny4SBunNDhTQXO7UiaLdALhaQzHOAkqpibDg3fMZ3RA5byZmo2rIa8kdXGQ7DwODQ6ljs1k5pA/0?wx_fmt=jpeg)
...
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkwtSOny4SBunNDhTQXO7UiaL663PgQ6GHrHTOCuO32FDEU8x1qOl9EbOuP8vef5tMB1Wf9jWGHx2ZQ/0?wx_fmt=jpeg)
上图中的M即是数量积(magnitude)，表示从原点到坐标点的距离，θ是相位角(phase angle)，表示从X轴正方向到某个向量的夹角，下面四个式子是计算方法：
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkwtSOny4SBunNDhTQXO7UiaLXMicKRBcQGOEEeBxakWDe7YIWWQ4ZQUZEObSOuP1nHnW5mymmuyNPWg/0?wx_fmt=jpeg)
...
ejx = cos x + j sin x 
这个等式可以从下面的级数变换中得到证明：
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkwtSOny4SBunNDhTQXO7UiaLs2F3EtzdF8f9pxORpOwRNNjlxt59EfWQrz1ZT60ptHZIUDHnMndInA/0?wx_fmt=jpeg)
...
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkwtSOny4SBunNDhTQXO7UiaL1r7wLAzxkJKicKdGLnbSgcKXT2Dib20uh6wUuecFsk2dyhQtsx7GDmiaw/0?wx_fmt=jpeg)
**三、复数是数学分析中的一个工具**
...
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkwtSOny4SBunNDhTQXO7UiaLKXqRTSqmHoVBHE55S13tAzmt0SdzuC1SDLAk1icVpoQcZu2b3tUuPIw/0?wx_fmt=jpeg)
上式中余弦幅值A经变换生成a，正弦幅值B的相反数经变换生成b：A <=> a，B<=> -b，但要注意的是，这不是个等式，只是个替换形式而已。
对于极坐标形式可以按如下形式进行转换：
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkwtSOny4SBunNDhTQXO7UiaLjbaaibJic3pECqxc42DgaUuZ98icUYkhqWbPkP1ugNbo2HPtV5lVTI5ZQ/0?wx_fmt=jpeg)
上式中，M <=> M，θ<=>φ。
...
![0?wx_fmt=jpeg](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_jpg/951TjTgiabkwtSOny4SBunNDhTQXO7UiaLMfzHLrcTicEiboZZ2PKaeFwZZzIupGp1ibLuhbkKMYx5kMLJzBRSjiao7w/0?wx_fmt=jpeg)
...
![0?wx_fmt=png](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkwtSOny4SBunNDhTQXO7UiaLZqM6VgQFtBmVvaL7SlRQF1Fal8w3TsibDEah3uHDfd6mZoEw0VibDzBg/0?wx_fmt=png)
     这个式子很容易可以得到欧拉变换式子：
![0?wx_fmt=png](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkwtSOny4SBunNDhTQXO7UiaLmtyoia4qpJRKNy5nhsJxBcsP4wtlibvsIkXWAO6ib9zdVPyia41ExwicCzA/0?wx_fmt=png)
...
![0?wx_fmt=png](https://ss.csdn.net/p?http://mmbiz.qpic.cn/mmbiz_png/951TjTgiabkwtSOny4SBunNDhTQXO7UiaLn3QgxfcxRUwu9NK6LGOtPZPQLBYeuyLSEKIojvJicmvsN33HuUegNGw/0?wx_fmt=png)
...
从上面的分析中可以看出，实数傅立叶变换跟复数傅立叶变换，在进行逆变换时得到的结果是一样的，只不过是殊途同归吧。本文完。
**-------------****----**
**明明共同关注公众号，彼此却互不认识；**
**明明具有相同的爱好，却无缘相识；**
**有没有觉得这就是上帝给我们的一个bug！**
