# 【Python学习系列十八】基于scikit-learn库逻辑回归训练模型(delta比赛代码3) - 专注于数据挖掘算法研究和应用 - CSDN博客





2017年06月16日 10:42:27[fjssharpsword](https://me.csdn.net/fjssharpsword)阅读数：2702
...
def main():
    ...
    #模型训练
    label_X = train_label_df[['voucher_num','use_nums','use_ratio','voucher_ratio','avg_discount','avg_txn_amt']]
    ...
    #predicted_X=test_label_df[['voucher_num','use_nums','use_ratio','voucher_ratio','avg_discount','avg_txn_amt']]
    ...
    unlabel_X_Scale=unlabel_df[['voucher_num','use_nums','use_ratio','voucher_ratio','avg_discount','avg_txn_amt']]
    unlabel_X_Scale=preprocessing.scale(unlabel_X_Scale)#归一化
    unlabel_y=model.predict(unlabel_X_Scale)
    out_y=pd.DataFrame(unlabel_y.reshape(-1,1),columns=['label'])
    unlabel_X_new=unlabel_X.join(out_y,how='left')
    label_df=label_df.append(unlabel_X_new)#构成新的训练集
    #预训练结束    
    ...
    while (f1_score-f1_score_old)>0.0001 :#迭代收敛到f1-score不再提升
        ...
        model = LogisticRegression()#DecisionTreeClassifier()
        #if flag==0 :
        #    model = DecisionTreeClassifier()#逻辑回归，第一次预训练
        #else :
        ...
        if f1_score_old<f1_score :#为未标记样本打上标记，然后加入训练集
            ...
            outset["label"] = outset.apply(lambda x:  0 if x["prob"] <0.55 else 1, axis = 1)
