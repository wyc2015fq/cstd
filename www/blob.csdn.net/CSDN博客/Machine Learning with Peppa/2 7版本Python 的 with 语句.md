# 2.7版本Python 的 with 语句 - Machine Learning with Peppa - CSDN博客





2018年02月03日 22:09:16[刺客五六柒](https://me.csdn.net/qq_39521554)阅读数：89
...
with 语句是从 Python 2.5 开始引入的一种与异常处理相关的功能（2.5 版本中要通过 from __future__ import with_statement 导入后才可以使用），从 2.7 版本开始缺省可用（参考 [What's
 new in Python 2.6?](http://docs.python.org/release/2.6/whatsnew/2.6.html#pep-343-the-with-statement) 中 with 语句相关部分介绍）。with 语句适用于对资源进行访问的场合，确保不管使用过程中是否发生异常都会执行必要的“清理”操作，释放资源，比如文件使用后自动关闭、线程中锁的自动获取和释放等。
...
```
with
 context_expression [as target(s)]:
```
...
```
with
 open(r'somefileName') as somefile:
```
...
```
value
 = type(context_manager).__enter__(context_manager)
```
...
```
exc
 = True   # True 表示正常执行，即便有异常也忽略；False 表示重新抛出异常，需要对异常进行处理
```
...
```
if
 not exit(context_manager, *sys.exc_info()):
```
...
```
print
 '[Enter %s]: Allocate resource.' % self.tag
```
...
```
print
 '[Exit %s]: Exited without exception.' % self.tag
```
...
```
print
 '[Exit %s]: Exited with exception raised.' % self.tag
```
...
```
print
 '[with-body] Run with exception. Failed to finish statement-body!'
```
```
...
```
[Exit
 Normal]: Exited without exception.
```
```
...
```
[Enter
 With-Exception]: Allocate resource.
```
...
```
[Exit
 With-Exception]: Exited with exception raised.
```
...
可以自定义上下文管理器来对软件系统中的资源进行管理，比如数据库连接、共享资源的访问控制等。Python 在线文档 [Writing
 Context Managers](http://docs.python.org/release/2.6/whatsnew/2.6.html#module-contextlib) 提供了一个针对数据库连接进行管理的上下文管理器的简单范例。
...
```
Code
 before yield-statement executes in __enter__
```
...
```
Code
 after yield-statement executes in __exit__
```
