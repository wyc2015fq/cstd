# 一文详解什么是RNN（循环神经网络） - Machine Learning with Peppa - CSDN博客





2018年04月25日 18:21:04[刺客五六柒](https://me.csdn.net/qq_39521554)阅读数：421
...
RNN是一个序列到序列的模型，假设11xt−1,xt,xt+1是一个输入：“我是中国“，那么1ot−1,ot就应该对应”是”，”中国”这两个，预测下一个词最有可能是什么？就是1ot+1应该是”人”的概率比较大。
...
1.可以把St当作饮状态，捕捉了之前时间点上的信息。就像你去考研一样，考的时候记住了你能记住的所有信息。 
...
11111从前往后：St1→=f(U1→∗Xt+W1→∗St−1+b1→)



22212从后往前:St2→=f(U2∗Xt→+W2→∗St−1+b2→)



12输出：ot=softmax(V∗[St1→;St2→])
这里的12[St1→;St2→]做的是一个拼接，如果他们都是1000X1维的，拼接在一起就是1000X2维的了。
...
深层双向RNN就是基于这么一个想法，他的输入有两方面，第一就是前一时刻的隐藏层传过来的信息1h→t−1(i)，和当前时刻上一隐藏层传过来的信息111ht(i−1)=[h→t(i−1);h←t(i−1)]，包括前向和后向的。 
...
假设我们对3E3的W求偏导：它的损失首先来源于预测的输出3y^3，预测的输出又是来源于当前时刻的记忆3s3,当前的记忆又是来源于当前的输出和截止到上一时刻的记忆：332s3=tanh(Ux3+Ws2)

因此，根据链式法则可以有:



333333∂E3∂W=∂E3∂y^3∂y^3∂s3∂s3∂W
但是，你会发现，221s2=tanh(Ux2+Ws1)，也就是2s2里面的函数还包含了W，因此，这个链式法则还没到底，就像图上画的那样，所以真正的链式法则是这样的： 
...
1St=tanh(U∗Xt+W∗St−1)
我们把图像的特征加在里面，可以得到：


1St=tanh(U∗Xt+W∗St−1+V∗X)
