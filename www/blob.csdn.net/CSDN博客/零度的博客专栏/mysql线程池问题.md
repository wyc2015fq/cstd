# mysql线程池问题 - 零度的博客专栏 - CSDN博客
2017年03月02日 14:55:16[零度anngle](https://me.csdn.net/zmx729618)阅读数：439
**问题分析：**
没有引入thread pool前，[MySQL](http://lib.csdn.net/base/14)使用的是one
 thread per connection，一旦connection增加到一定程度，MySQL的性能将急剧下降甚至被压跨。引入thread pool后将会解决上述问题,同时会减少MySQL内部的线程数（节省内存）及频繁创建线程的开销（节省cpu)。
## **thread pool是如何工作的呢？**
...
4.据称阿里的数据库的thread_pool_stall_limit
