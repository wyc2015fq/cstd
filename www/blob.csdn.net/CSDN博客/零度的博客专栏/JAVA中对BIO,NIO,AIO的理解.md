# JAVA中对BIO,NIO,AIO的理解 - 零度的博客专栏 - CSDN博客
2016年07月05日 10:53:40[零度anngle](https://me.csdn.net/zmx729618)阅读数：450标签：[java																[bio																[nio																[aio](https://so.csdn.net/so/search/s.do?q=aio&t=blog)](https://so.csdn.net/so/search/s.do?q=nio&t=blog)](https://so.csdn.net/so/search/s.do?q=bio&t=blog)](https://so.csdn.net/so/search/s.do?q=java&t=blog)
...
7 什么是异步阻塞？ 
8 什么是异步非阻塞？ 
散仙不才，在查了一部分资料后，愿试着以通俗易懂的方式解释下这几个名词。如有不足之处，还望告知。 在弄清楚上面的几个问题之前，我们首先得明白什么是同步，异步，阻塞，非阻塞，只有这几个单个概念理解清楚了，然后在组合理解起来，就相对比较容易了。 
