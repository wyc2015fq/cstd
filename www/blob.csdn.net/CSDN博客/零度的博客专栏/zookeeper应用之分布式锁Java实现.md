# zookeeper应用之分布式锁Java实现 - 零度的博客专栏 - CSDN博客
2018年03月19日 17:14:56[零度anngle](https://me.csdn.net/zmx729618)阅读数：330标签：[zookeeper																[分布式锁](https://so.csdn.net/so/search/s.do?q=分布式锁&t=blog)](https://so.csdn.net/so/search/s.do?q=zookeeper&t=blog)
...
```
```java
) ## watch proceess WatchedEvent state:SyncConnected type:NodeDeleted
```
```java
