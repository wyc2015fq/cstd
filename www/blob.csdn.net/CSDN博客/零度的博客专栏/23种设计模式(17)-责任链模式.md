# 23种设计模式(17)-责任链模式 - 零度的博客专栏 - CSDN博客
2017年07月28日 10:05:44[零度anngle](https://me.csdn.net/zmx729618)阅读数：192
**责任链模式**
    一、 概念 
    二、 引子 
    三、 结构 
    四、 具体案例 
**一、概念**
责任链模式是一种对象的行为模式。在责任链模式里，很多对象由每一个对象对其下家的引用而连接起来形成一条链。请求在这个链上传递，直到 链上的某一个对象决定处理此请求。发出这个请求的客户端并不知道链上的哪一个对象最终处理这个请求，这使得系统可以在不影响客户端的情况下动态地重新组织 和分配责任。
**二、引子 从击鼓传花谈起**
...
![这里写图片描述](https://img-blog.csdn.net/20160204162813274)
击鼓传花便是责任链模式的应用。责任链可能是一条直线、一个环链或者一个树结构的一部分。
**三、结构**
　下面使用了一个责任链模式的最简单的实现。 
![这里写图片描述](https://img-blog.csdn.net/20160204162852219)
　　责任链模式涉及到的角色如下所示： 
...
![这里写图片描述](https://img-blog.csdn.net/20160204163204026)
**四、具体案例**
来考虑这样一个功能:申请聚餐费用的管理。 
...
![这里写图片描述](https://img-blog.csdn.net/20160204163744699)
源代码 
抽象处理者角色类
```java
