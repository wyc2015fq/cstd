# 使用Nginx反向代理后如何在web应用中获取用户IP - 零度的博客专栏 - CSDN博客
2018年03月01日 17:49:19[零度anngle](https://me.csdn.net/zmx729618)阅读数：114
### 一、问题背景
...
$proxy_add_x_forwarded_for变量包含客户端请求头中的"X-Forwarded-For"，与$remote_addr两部分，他们之间用逗号分开。举个例子，有一个web应用，在它之前通过了两个nginx转发，www.linuxidc.com 即用户访问该web通过两台nginx。
在第一台nginx中,使用： proxy_set_header     X-Forwarded-For   $proxy_add_x_forwarded_for;
现在的$proxy_add_x_forwarded_for变量的"X-Forwarded-For"部分是空的，所以只有$remote_addr，而$remote_addr的值是用户的ip，于是赋值以后，X-Forwarded-For变量的值就是用户的真实的ip地址了。
到了第二台nginx，同样使用：proxy_set_header     X-Forwarded-For   $proxy_add_x_forwarded_for;
...
原文链接：https://www.linuxidc.com/Linux/2012-06/63587.htm
