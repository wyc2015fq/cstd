# HttpClient通过RetryHandler实现超时重试 - 零度的博客专栏 - CSDN博客
2017年10月26日 13:44:51[零度anngle](https://me.csdn.net/zmx729618)阅读数：2377
> 
java项目中使用apache httpclient进行数据传输、访问，目前程序中涉及到需要callback操作，product需要被动的接收consume的处理状态，为了最大程度的能够callback成功因此consume在http调用出现问题（如：服务不可用、异常、超时）情况下需要进行重试（retry
 request），在这里我列举出我找到的retry方案，有些成功有些不成功。
...
可以实现，满足需求，这具有`HttpRequestRetryHandler`的所有优点，并且可以自定义retry时间的间隔，如下，是我定义的httpclient pool，
```java
public static CloseableHttpClient getHttpClient() {
