# 机器学习入门系列01，Introduction 简介 - 零度的博客专栏 - CSDN博客
2017年03月28日 11:26:04[零度anngle](https://me.csdn.net/zmx729618)阅读数：668
# 我们将要学习什么东东？
## 什么是机器学习？
...
### Regression（回归）是什么?
Regression 意思是，我们要找到这样一个函数 ，使得输出为
 scalar（标量，数值）
> 
...
上面讲过，[机器学习](http://lib.csdn.net/base/2)的第一个步骤就是确定一个函数集，可以选择Linear Model（线性模型），但是 Linear Model 比较有限。更多的则是 Non-lilnear Model（非线性模型）。比如深度学习就是
 Non-lilnear Model 。
举个例子，比如图像识别：
...
蓝色的都是代表Scenario（方案），红色部分指的是你要找的function的样子，可以在Semi-supervised Learning 的情况下做红色的task，也可以在 Transfer Learning下做红色task，就是右上角的4个虽然是小方块，但是 Supervised Learning 里面的 task 都可以在其他蓝色 scenario 中做。绿色的部分就是方法或者说模型，绿色的内容虽然写在 Classification 里面，但是可以放在 Regression 或者 Structured
 Learning 中。
