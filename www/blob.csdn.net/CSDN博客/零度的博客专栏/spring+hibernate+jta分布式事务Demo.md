# spring+hibernate+jta分布式事务Demo - 零度的博客专栏 - CSDN博客
2016年05月04日 15:45:01[零度anngle](https://me.csdn.net/zmx729618)阅读数：509
...
1、DAO接口、DAOImpl实现，Service接口、ServiceImpl实现都不用去动它，为使第一次测试时事务顺利提交，我们把第一部分中对于serviceImpl中insert方法中故意制造的困难去除。[](http://tkfiles.storage.msn.com/y1pR6nssK-Ax5xERzhjxKbS3AiMuFo8jGxDpEWBna5eynRmwSB9Qe8jOWNVyfxanKQxuEYhYh07kbQ?PARTNER=WRITER)
——————————————————————————————
...
——————————————————————————————————————————
2、spring是不能直接管理分布式事务的，需要管理的话就得借助jta，但是jta又不能运行在tomcat这个容器中，于是我们就要用到AtomikosTransactionsEssentials这个第三方jar包来提供jta支持。AtomikosTransactionsEssentials可以从[http://www.atomikos.com](http://www.atomikos.com/)[下载，当然我这里也提供这些类包的ftp](http://www.atomikos.com/home.html)下载（ftp下载，勾选连接到服务器，用户名: 
 momo 密码 momoko8443）。解压zip文件后，我们从dist文件夹中拷贝几个jar包复制到工程中的lib下。如果你不使用spring管理hibernate，则把hibernate的包也拷上
![clip_image008](http://tkfiles.storage.msn.com/y1pR6nssK-Ax5xgqDWF4ZUje3nnL91W8NBP5aoKh6MOdTWTshyVbIBb_3HFBxX_xycwiGGRCfR5X7U?PARTNER=WRITER)
...
由于要使用jta管理事务，dataSource就必须使用实现了javax.sql.XADataSource接口的连接池来连接，AtomikosTransactionsEssentials为我们提供了一个名为SimpleDataSourceBean的连接池（名字有点怪），废话不多说了，直接看修改的dataSource和sessionFactory，注意这里的数据库必须是mysql5.0以上，且驱动也要5.0以上否则不提供XADataSource支持。另外还有个叫Xapool的连接池也是支持xadatasource的，你可以试试看。[](http://tkfiles.storage.msn.com/y1pR6nssK-Ax5xenCWmdlDAGm31V4BEhcITZ2VCvKDKr_kvePfXZwzwbygiLgiphQVXbQRiraUCIrs?PARTNER=WRITER)
——————————————————————————————————————————
文件:applicationContext.xml
配置:
<bean id="dataSource1" class="com.atomikos.jdbc.DataSourceBean">
<property name="uniqueResourceName"><value>db1</value></property>
<property name="xaDataSourceClassName"><value>com.[MySQL](http://lib.csdn.net/base/14).jdbc.jdbc2.optional.MysqlXADataSource</value></property>
...
————————————————————————————————————————————
4、继续修改applicationContext.xml，现在是关键所在，开始添加jta事务管理了。注意看代码了，首先定一个atomikos提供的transactionManager[](http://tkfiles.storage.msn.com/y1pR6nssK-Ax5xD48lyVmngVRC7aax4C1MZAdxa0PJ8kS4CuuSD16GMmiDdIneG2t8Uvn-Eun6mlUE?PARTNER=WRITER)
——————————————————————————————————
文件:applicationContext.xml
配置:
...
————————————————————————————
再定一个atomikos对UserTransaction接口的实现[](http://tkfiles.storage.msn.com/y1pR6nssK-Ax5yaL2GRUaw91R7DvGojF0FhJPuelWWlJkc-h_Bc1GjWk30TFsIDmLN_3a4f5YZmN_g?PARTNER=WRITER)
——————————————————————————
文件:applicationContext.xml
配置:
...
看到这里是不是有点晕？[](http://tkfiles.storage.msn.com/y1pR6nssK-Ax5wntLTaJh0mjgyQ5O--9zV49Ioe9xkd0vGGBMCmNhRBQDqYWqHZqgRFbQhVAJ6gWuI?PARTNER=WRITER)，先死记硬背。
————————————————————————————
文件:applicationContext.xml
配置:
...
——————————————————————————————
user1DAOImpl和user2DAOImpl的sessionFactory属性稍作修改注入新的sessionFactory。user2DAOImpl类似，这里就不给出代码了。[](http://tkfiles.storage.msn.com/y1pR6nssK-Ax5wGjs0OA-QQDbgsAUdn8TuzcTfESsEEe9rG4j3O1wFGHeQinLZLe79a2i_0GVN99wo?PARTNER=WRITER)
——————————————————————————————
文件:applicationContext.xml
配置:
...
其他的都不必修改
接下来，run application。看下控制台，很好，正如我们期望的一样，第一次测试时，事务正常提交了。[](http://tkfiles.storage.msn.com/y1pR6nssK-Ax5wNx79SidhjxKW8qu_b8z6CPxzsFxpJEQ5tWIvJDcR0DmsKU5fB9ZC_aNKsYGf8VDY?PARTNER=WRITER)
...
看下数据库，果然，2个db中user表都被**“插入”**了[](http://tkfiles.storage.msn.com/y1pR6nssK-Ax5zBu7f23WB-H11pmkrkjbpXNr8u7HN_jQ0HEoGMGpjTcTvT-QVByy9XGy7z9_VCJnk?PARTNER=WRITER)一条记录。如下图所示：
...
6、按照管理，接下来就是应该测roll back了，好，给它制造困难，把serviceImpl中的“困难”加上。[](http://tkfiles.storage.msn.com/y1pR6nssK-Ax5xAi_V00t-tyvAWNqkItIwUDk4Oaiw79JC3tSk5GzhMPMCKEnOgWeokfLst8ZjL_XQ?PARTNER=WRITER) Run
 application
...
看一下控制台输出，果然抛出了异常[](http://tkfiles.storage.msn.com/y1pR6nssK-Ax5x1E3Qo0KEx0PhOsNZw6DZAMgwdVDTW8U__d_2zWikhvhbedmvzyJijviyu4ODYy0w?PARTNER=WRITER)
...
再观察下数据库中的记录。OK！roll back成功！[](http://tkfiles.storage.msn.com/y1pR6nssK-Ax5zzObMLbCY5Y_61MzcHVJX5XyL24DX-Vwr_L2-REs0Ap_l9APIk2aJR0KXUDwbA-lQ?PARTNER=WRITER)
...
7、至此分布式事务管理demo演示完毕，2个demo都是演示性质的，知侧重于配置和实现，至于其中的原理，由于我能力有限不能表述的很清楚，你可以查看spring的api文档进行探索。
