# mysql(for update)悲观锁总结与实践 - 零度的博客专栏 - CSDN博客
2016年09月29日 15:02:12[零度anngle](https://me.csdn.net/zmx729618)阅读数：11442标签：[mysql																[悲观锁](https://so.csdn.net/so/search/s.do?q=悲观锁&t=blog)](https://so.csdn.net/so/search/s.do?q=mysql&t=blog)
个人分类：[mysql](https://blog.csdn.net/zmx729618/article/category/6441555)
       悲观锁，正如其名，它指的是对数据被外界（包括本系统当前的其他事务，以及来自外部系统的事务处理）修改持保守态度，因此，在整个数据处理过程中，将数据处于锁定状态。悲观锁的实现，往往依靠数据库提供的锁机制（也只有数据库层提供的锁机制才能真正保证数据访问的排他性，否则，即使在本系统中实现了加锁机制，也无法保证外部系统不会修改数据）。
      使用场景举例：以[MySQL](http://lib.csdn.net/base/14) InnoDB为例， 商品goods表中有一个字段status，status为1代表商品未被下单，status为2代表商品已经被下单，那么我们对某个商品下单时必须确保该商品status为1。假设商品的id为1。
**1如果不采用锁，那么操作方法如下：**
//1.查询出商品信息
...
注：需要注意的是，在事务中，只有SELECT ... FOR UPDATE 或LOCK IN SHARE MODE 同一笔数据时会等待其它事务结束后才执行，一般SELECT ... 则不受此影响。拿上面的实例来说，当我执行select status from t_goods where id=1 for update;后。我在另外的事务中如果再次执行select status from t_goods where id=1 for update;则第二个事务会一直等待第一个事务的提交，此时第二个查询处于阻塞的状态，但是如果我是在第二个事务中执行select
...
Sql代码  ![收藏代码](http://chenzhou123520.iteye.com/images/icon_star.png)
...
Sql代码  ![收藏代码](http://chenzhou123520.iteye.com/images/icon_star.png)
- mysql> select * from t_goods where id=1 forupdate;  
- +----+--------+------+
- | id | status | name |  
- +----+--------+------+
- |  1 |      1 | 道具 |  
...
console1：查询结果为空
Sql代码  ![收藏代码](http://chenzhou123520.iteye.com/images/icon_star.png)
- mysql> select * from t_goods where id=3 forupdate;  
- Empty set
...
Sql代码  ![收藏代码](http://chenzhou123520.iteye.com/images/icon_star.png)
- mysql> select * from t_goods wherename='道具'forupdate;  
- +----+--------+------+
- | id | status | name |  
- +----+--------+------+
- |  1 |      1 | 道具 |  
...
Sql代码  ![收藏代码](http://chenzhou123520.iteye.com/images/icon_star.png)
- mysql> select * from t_goods wherename='装备'forupdate;  
...
- 
- mysql> select * from t_goods where id>0 forupdate;  
- +----+--------+------+
- | id | status | name |  
- +----+--------+------+
- |  1 |      1 | 道具 |  
...
console2：查询被阻塞，说明console1把表给锁住了
Sql代码  ![收藏代码](http://chenzhou123520.iteye.com/images/icon_star.png)
- mysql> select * from t_goods where id<>2 forupdate;  
console1：提交事务
Sql代码  ![收藏代码](http://chenzhou123520.iteye.com/images/icon_star.png)
- mysql> commit;  
- Query OK, 0 rows affected  
...
console1：
Sql代码  ![收藏代码](http://chenzhou123520.iteye.com/images/icon_star.png)
- mysql> select * from t_goods where status=1 forupdate;  
- +----+--------+------+
- | id | status | name |  
- +----+--------+------+
- |  1 |      1 | 道具 |  
...
Sql代码  ![收藏代码](http://chenzhou123520.iteye.com/images/icon_star.png)
- mysql> select * from t_goods where status=2 forupdate;  
- +----+--------+------+
- | id | status | name |  
- +----+--------+------+
- |  2 |      2 | 装备 |  
...
Sql代码  ![收藏代码](http://chenzhou123520.iteye.com/images/icon_star.png)
- mysql> select * from t_goods where status=3 forupdate;  
- Empty set
console2：查询status=3的数据，返回空数据
Sql代码  ![收藏代码](http://chenzhou123520.iteye.com/images/icon_star.png)
- mysql> select * from t_goods where status=3 forupdate;  
- Empty set
以上就是关于我对数据库悲观锁的理解和总结，有不对的地方欢迎拍砖
