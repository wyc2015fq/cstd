# mysql单列索引、多列索引的使用 - 零度的博客专栏 - CSDN博客
2016年09月29日 14:21:51[零度anngle](https://me.csdn.net/zmx729618)阅读数：13572
### 一、简介      
...
       5.精确匹配某一列和和范围匹配另外一列
       order by操作中出现的字段同样适用于按值查找的规则，where+order by中出现的字段需可以建立满足如上五种规则多列索引。使用多列所需需要按照最左索引列查找；不能跳过中间列；如果某一列是范围查询，那么其右边所有列无法使用索引。IN什么情况下是范围查询，什么情况下是多个等值查询？如果有order
 by排序时，多个等于条件查询就是范围查询，没有order by排序就没有限制。
...
      mysql>SELECT ｀uid｀ FROM people WHERE lname｀='Liu'  AND ｀fname｀='Zhiqun'
 AND ｀age｀=26 ； 因为我们不想扫描整表，故考虑用索引。
       1、单列索引：
