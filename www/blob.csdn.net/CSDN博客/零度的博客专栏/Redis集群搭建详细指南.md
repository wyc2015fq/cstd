# Redis集群搭建详细指南 - 零度的博客专栏 - CSDN博客
2018年10月23日 12:16:31[零度anngle](https://me.csdn.net/zmx729618)阅读数：189
原文出处：[https://www.cnblogs.com/mafly/p/redis_cluster.html](https://www.cnblogs.com/mafly/p/redis_cluster.html)
## Redis 集群简介
...
我现在就要在已经有安装了 Redis 的一个 CentOS 下开始进行集群搭建，如果你还不是很清楚 Linux 下如何安装 Redis ，可以去看这一篇文章[《了解一下 Redis 并在 CentOS 下进行安装配置》](http://blog.mayongfa.cn/258.html)。请注意，下面所有集群搭建环境都基于已安装好的 Redis 做的。
**1.创建文件夹**
我们计划集群中 Redis 节点的端口号为 `9001-9006` ，端口号即集群下各实例文件夹。数据存放在 `端口号/data` 文件夹中。
![mkdir_9001-9006](https://images2015.cnblogs.com/blog/539095/201706/539095-20170608233722965-555429215.png)
```
...
![mv9001](https://images2015.cnblogs.com/blog/539095/201706/539095-20170608234342168-1828683256.png)
...
![cluster_conf](https://images2015.cnblogs.com/blog/539095/201706/539095-20170608235351340-327384767.png)
**4.再复制出五个新 Redis 实例**
我们已经完成了一个节点了，其实接下来就是机械化的再完成另外五个节点，其实可以这么做：把 `9001 实例` 复制到另外五个文件夹中，唯一要修改的就是 `redis.conf` 中的所有和端口的相关的信息即可，其实就那么四个位置。开始操作，看图：
![cp9001-9006](https://images2015.cnblogs.com/blog/539095/201706/539095-20170609000331840-1109501384.png)
```
...
![%s-9001-9002g](https://images2015.cnblogs.com/blog/539095/201706/539095-20170609000850293-694480679.png)
```
vim redis.conf
:%s/9001/9002g
```
回车后，就会有替换几个地方成功的提示，不放心可以手工检查下：
![%s-success](https://images2015.cnblogs.com/blog/539095/201706/539095-20170609001136497-1098917089.png)
其实我们也就是替换了下面这四行：
```
port 9002
...
**1.启动 9001-9006 六个节点**
少废话，直接看图：
![redis-server_start](https://images2015.cnblogs.com/blog/539095/201706/539095-20170609001617887-1150016667.png)
```
...
set name mafly
```
![redis-server_start_test](https://images2015.cnblogs.com/blog/539095/201706/539095-20170609002010762-729079520.png)
连接成功了，但好像报错了阿？？？
...
gem install redis
```
![yum_ruby](https://images2015.cnblogs.com/blog/539095/201706/539095-20170609002556950-2102990948.png)
## 这才是真正的创建集群
先不废话，直接敲命令：
...
![cluster_create](https://images2015.cnblogs.com/blog/539095/201706/539095-20170609002923075-940692900.png)
...
![cluster_create_success](https://images2015.cnblogs.com/blog/539095/201706/539095-20170609003837872-1480989199.png)
上图则代表集群搭建成功啦！！！
**验证一下：**
...
cluster nodes
```
![cluster_info](https://images2015.cnblogs.com/blog/539095/201706/539095-20170609004309122-927584496.png)
...
![redirected_9002](https://images2015.cnblogs.com/blog/539095/201706/539095-20170609004602356-940998597.png)
