# 机器学习入门系列03，Error的来源：偏差和方差(bias和variance) - 零度的博客专栏 - CSDN博客
2017年03月29日 15:42:41[零度anngle](https://me.csdn.net/zmx729618)阅读数：2451
# 回顾
第二篇中神奇宝贝的例子：
![](http://wx3.sinaimg.cn/mw690/a9c4d5f6gy1fe0ofd7d2nj20vi0hsgnx.jpg)
        可以看出越复杂的model 再测试集上的性能并不是越好
这篇要讨论的就是 error 来自什么地方？error主要的来源有两个，bias（偏差） 和 variance（方差）
# 估测
![](http://wx2.sinaimg.cn/mw690/a9c4d5f6gy1fe0ofvddt0j20gi07uabz.jpg)
假设上图为神奇宝贝cp值的真正方程，当然这只有Niantic（制作《Pokemon Go》的游戏公司）知道。从训练集中可以找到真实方程
 的近似方程 。
## 估测bias 和 variance
### 估测变量  的平均值
- 假设的平均值为
...
靶心为真正的方程 
 ，深蓝色点为
 ，是实验求得的方程。求的期望值，即图中浅蓝色的点。
 和 之间的距离就是误差
 bias，而
 和 
 之间的距离就是误差 variance。4幅图的对比观察两个误差的影响。
bias就是射击时瞄准的误差，本来应该是瞄准靶心，但bias就造成瞄准准心的误差；而variance就是虽然瞄准在 ，但是射不准，总是射在
 的周围。
## 为什么会有很多的 ?
讨论系列02中的案例：这里假设是在平行宇宙中，抓了不同的神奇宝贝
![](http://wx2.sinaimg.cn/mw690/a9c4d5f6gy1fe0oi5o6ruj21320ggafm.jpg)
用同一个model，在不同的训练集中找到的 就是不一样的
...
结果可视化，一次平均的 没有5次的好，虽然5次的整体结果离散程度很高。
![](http://wx2.sinaimg.cn/mw690/a9c4d5f6gy1fe0ol56hmbj21340rw4bm.jpg)
一次model的bias比较大，而复杂的5次model，bias就比较小。
直观的解释：简单的model函数集的space比较小，所以可能space里面就没有包含靶心，肯定射不中。而复杂的model函数集的space比较大，可能就包含的靶心，只是没有办法找到确切的靶心在哪，但足够多的，就可能得到真正的
。
### bias v.s. variance
...
### bias大，Underfitting
此时应该重新设计model。因为之前的函数集里面可能根本没有包含。可以：
