# 关于分布式事务、两阶段提交、一阶段提交、Best Efforts 1PC模式和事务补偿机制的研究 - 零度的博客专栏 - CSDN博客
2017年09月06日 11:37:30[零度anngle](https://me.csdn.net/zmx729618)阅读数：320标签：[数据库																[分布式																[事务](https://so.csdn.net/so/search/s.do?q=事务&t=blog)](https://so.csdn.net/so/search/s.do?q=分布式&t=blog)](https://so.csdn.net/so/search/s.do?q=数据库&t=blog)
个人分类：[事务](https://blog.csdn.net/zmx729618/article/category/6213269)
**1.XA**
...
**2.JTA**
作为[Java](http://lib.csdn.net/base/java)平台上事务规范JTA（[Java](http://lib.csdn.net/base/java)Transaction
 API）也定义了对XA事务的支持，实际上，JTA是基于XA[架构](http://lib.csdn.net/base/architecture)上建模的，在JTA 中，事务管理器抽象为javax.transaction.TransactionManager接口，并通过底层事务服务（即JTS）实现。像很多其他的java规范一样，JTA仅仅定义了接口，具体的实现则是由供应商(如J2EE厂商)负责提供，目前JTA的实现主要由以下几种：
