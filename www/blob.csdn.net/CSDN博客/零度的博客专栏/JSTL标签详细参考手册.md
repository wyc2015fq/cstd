# JSTL标签详细参考手册 - 零度的博客专栏 - CSDN博客
2016年07月07日 10:15:40[零度anngle](https://me.csdn.net/zmx729618)阅读数：376
**前言 **
=========================================================================
...
3、在jsp文件的顶部加入以下内容：
Java代码 ![复制代码](http://elf8848.iteye.com/images/icon_copy.gif)![收藏代码](http://elf8848.iteye.com/images/icon_star.png)![](http://elf8848.iteye.com/images/spinner.gif)
...
** <c:forEach> 标签的语法 说明 ：**
语法1：迭代一集合对象之所有成员
Html代码 ![复制代码](http://elf8848.iteye.com/images/icon_copy.gif)![收藏代码](http://elf8848.iteye.com/images/icon_star.png)![](http://elf8848.iteye.com/images/spinner.gif)
...
属性                   类型                                  意义index                  number                    现在指到成员的索引count                  number                    总共指到成员的总和first                  boolean                   现在指到成员是否为第一个last                   boolean                   现在指到成员是否为最后一个 
** <c:forEach> 遍历 List列表：**
...
- 内容  
- </c:if>  
<c:if test="${not empty item.publish_time}">内容</c:if><c:if test="${item['domain']!=null}">内容</c:if><c:if test="${!empty permissionMap}"> 内容</c:if>
