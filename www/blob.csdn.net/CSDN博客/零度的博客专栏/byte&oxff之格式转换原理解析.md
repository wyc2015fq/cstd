# byte&oxff之格式转换原理解析 - 零度的博客专栏 - CSDN博客
2017年12月13日 14:05:29[零度anngle](https://me.csdn.net/zmx729618)阅读数：796
最近需要一个将字节数组转换为十六进制字符串输出的函数，于是开始编码如下：
```
//该代码存在问题
...
- 10
- 11
       运行测试，一旦遇到负数总出现未可预料的结果，如参数为{-125，125，115，89}时，运行结果为FFFFFF837D7359。 开始从网上查资料做修改，对byte数据进行与运算&oxff再进行转换，很抓狂到底为什么，再一顿搜索，原来问题出在java整数表示方法及位扩张问题上，在java中使用补码对整数进行表示。修改后代码如下：
```
//正确代码
public static String byteToHex(byte[] bt){
        StringBuffer sb = new StringBuffer();
...
- 11
- 12
###         对于此问题找到了比较全面的解释如下：转载自[http://my.oschina.net/joymufeng/blog/139952](http://my.oschina.net/joymufeng/blog/139952) 。
下面这行代码的输出是什么？
```
...
- 4
- 5
请尝试在运行上面的两个代码片段，如果你对输出结果感到很惊讶，请继续往下，正如你所看到的： 
第1个代码片段的运行结果是：-1 
...
**一、Java中如何编码负数？**
       Java采用”补码“(Two's Complement)编码负数，它是一种数值的编码方法，要分二步完成：第一步，对于负数的绝对值的每一个二进制位都取相反值，0变成1，1变成0。比如，-8的编码过程为：其绝对为+8，+8的二进制编码是00001000，取反后就是11110111。第二步，将上一步得到的值加1。11110111就变成11111000。所以，00001000的2的补码就是11111000。也就是说，-8在计算机（8位机）中就是用11111000表示。
**二、什么是符号扩展(Sign Extension)?**
       符号扩展（Sign Extension）用于在数值类型转换时扩展二进制位的长度，以保证转换后的数值和原数值的符号（正或负）和大小相同，一般用于较窄的类型（如byte）向较宽的类型（如int）转换。扩展二进制位长度指的是，在原数值的二进制位左边补齐若干个符号位（0表示正，1表示负）。举例来说，如果用6个bit表示十进制数10，二进制码为"00 1010"，如果将它进行符号扩展为16bits长度，结果是"0000 0000 0000 1010"，即在左边补上10个0（因为10是正数，符号为0），符号扩展前后数值的大小和符号都保持不变；如果用10bits表示十进制数-15，使用“2的补码”编码后，二进制码为"11
 1111 0001"，如果将它进行符号扩展为16bits，结果是"1111 1111 1111 0001",即在左边补上6个1（因为-15是负数，符号为1），符号扩展前后数值的大小和符号都保持不变。
**三、Java类型转换规则**
1、Java中int型字面量的书写方式有以下几种：
           1、 十进制方式，直接书写十进制数字
           2、八进制方式，格式以0打头，例如012表示十进制10
           3、十六进制方式，格式为0x打头，例如0xff表示十进制255
...
五、几个转型的例子
在进行类型转换时，一定要了解表达式的含义，不能光靠感觉。最好的方法是将你的意图明确表达出来。在将一个char型数值c转型为一个宽度更宽的类型时，并且不希望有符号扩展，可以如下编码：
`int i = c & 0xffff;`
```
```
```
