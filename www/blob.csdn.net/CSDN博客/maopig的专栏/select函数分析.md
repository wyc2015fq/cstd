# select函数分析 - maopig的专栏 - CSDN博客
2013年04月25日 17:33:33[maopig](https://me.csdn.net/maopig)阅读数：898
...
先说明两个结构体： 
第一，struct fd_set可以理解为一个集合，这个集合中存放的是文件描述符(file descriptor)，即文件句柄，这可以是我们所说的普通意义的文件，当然Unix下任何设备、管道、FIFO等都是文件形式，全部包括在内，所以毫无疑问一个socket就是一个文件，socket句柄就是一个文件描述符。fd_set集合可以通过一些宏由人为来操作，比如清空集合FD_ZERO(fd_set *)，将一个给定的文件描述符加入集合之中FD_SET(int ,fd_set *)，将一个给定的文件描述符从集合中删除FD_CLR(int
...
参考资料：[http://cuijinbird.blogchina.com/cuijinbird/1921117.html](http://cuijinbird.blogchina.com/cuijinbird/1921117.html)
Part 2:
