# iOS面试题及答案 - 努力，可能成功！放弃，注定失败！ - CSDN博客
置顶2018年11月07日 09:46:30[上天眷顾我](https://me.csdn.net/qq_30513483)阅读数：48标签：[p.p1 {margin: 0.0px 0.0p](https://so.csdn.net/so/search/s.do?q=p.p1 {margin: 0.0px 0.0p&t=blog)
...
```
3，＃import跟#include有什么区别，@ class呢，＃import <>跟#import“”有什么区别？
```
答：
...
```
5，目标Ç的类可以多重继承么？可以实现多个接口么？类别是什么？重写一个类的方式用继承好还是分类好？为什么？
```
...
```
7，@ property中有哪些属性关键字？/ @property后面可以有哪些修饰符？
```
属性可以拥有的特质分为四类:
1.原子性--- nonatomic 特质
...
```
8，属性关键字读写，只读，分配，保存，复制，非原子各个什么作用，在那种情况下用？
```
答：
...
```
11，用@property声明的NSString / NSArray / NSDictionary经常使用copy关键字，为什么？如果改用strong关键字，可能造成什么问题？
```
...
```
15，如何让自己的类用复制修饰符？如何重写带复制关键字的制定者？
```
...
```
16，写一个setter方法用于完成@property（nonatomic，retain）NSString * name，写一个setter方法用于完成@property（nonatomic，copy）NSString * name
```
答：
// retain
...
```
18，常见的目标Ç的数据类型有那些，和Ç的基本数据类型有什么区别如：NSInteger的的和INT
```
答：
...
```
21，目标C中创建线程的方法是什么？如果在主线程中执行代码，方法是什么？如果想延时执行代码，方法又是什么？
```
...
```
- 你实现过一个框架或者库以供别人使用么如果有，请谈一谈构建框架或者库时候的经验;如果没有，请设想和设计框架的公共API的，并指出大概需要如何做，需要注意一些什么方面，来使别人容易地使用你的框架。
```
