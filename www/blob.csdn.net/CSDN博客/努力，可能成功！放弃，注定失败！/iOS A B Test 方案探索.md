# iOS A/B Test 方案探索 - 努力，可能成功！放弃，注定失败！ - CSDN博客
置顶2017年01月09日 22:29:10[上天眷顾我](https://me.csdn.net/qq_30513483)阅读数：526
# 引子
...
A/B Test 就是为了同一个目标制定两个方案（比如两个website，app的页面），让一部分用户使用 A 方案，另一部分用户使用 B 方案，记录下用户的使用情况，看哪个方案更接近测试想要的结果，并确信该结论在推广到全部流量可信。
...
### 最基本的函数 A/B
![](http://upload-images.jianshu.io/upload_images/2573533-01b70d5389010f96.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
2017010534065A:BTest_functionA:B2_no.png
![](http://upload-images.jianshu.io/upload_images/2573533-5573d892cf7ce0b3.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
### 方法选择子 + 字典，缓存式 A/B
![](http://upload-images.jianshu.io/upload_images/2573533-983c5b173ad83d34.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
2017010548669A:BTest_Selector+dic1.png
![](http://upload-images.jianshu.io/upload_images/2573533-9468e77706946304.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
201701052186A:BTest_Selector+dic2.png
由于Objective-C 的Runtime 动态特性，我们可以把方法选择子缓存在一个字典中，在需要确定 A/B 方案的调用处判断一次，得到对应方案的方法缓存字典，在调用的时候，只需要去对应的缓存字典中调用就可以了，当然这里需要扩展NSObject类中的
```
-
...
### 设计模式之策略模式
![](http://upload-images.jianshu.io/upload_images/2573533-d57d42c9ca209a23.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
2017010594760OTA_ABTestClass.png
![](http://upload-images.jianshu.io/upload_images/2573533-2dc3d2ba6f460a39.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
201701058434ABstrategy_pattern1.png
![](http://upload-images.jianshu.io/upload_images/2573533-8f0f4061e69f4cd4.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
~libFlight_o ar xv /Users/f.li/Desktop/libFlight.a
  ...
  x - Flight.o
```
这样，在`libFlight_o`文件夹内，就有了
```
__.SYMDEF
 SORTED
```
和`Flight.o`这个两个文件。
同样，在`libHotel_o`文件夹内获得
```
__.SYMDEF
 SORTED
```
和`Hotel.o`
- 合并，重新打包。
把`__.SYMDEF SORTED`和`Flight.o`，还有`Hotel.o`移动到`libFlight_Hotel_o`文件夹内。把重新打包object
 file；
