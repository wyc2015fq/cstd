# Realm数据库 从入门到“放弃” - 努力，可能成功！放弃，注定失败！ - CSDN博客
置顶2016年11月21日 13:45:59[上天眷顾我](https://me.csdn.net/qq_30513483)阅读数：1379
![](http://upload-images.jianshu.io/upload_images/1194012-fdee34cd97308fac.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
#### 前言
...
[Swift/RLMSupport.swift](https://github.com/realm/realm-cocoa/blob/master/Realm/Swift/RLMSupport.swift) 文件一同编译进去。
...
- 下载[最新的Realm发行版本](https://static.realm.io/downloads/objc/realm-objc-2.0.2.zip)，并解压；
...
- 下载 [Realm 的最新版本](https://static.realm.io/downloads/objc/realm-objc-2.0.2.zip)并解压，将 Realm.framework 从 ios/static/文件夹拖曳到您 Xcode 项目中的文件导航器当中。确保 **Copy items if needed** 选中然后单击 **Finish**；
- 在 Xcode 文件导航器中选择您的项目，然后选择您的应用目标，进入到** Build Phases** 选项卡中。在 **Link Binary with Libraries** 中单击 + 号然后添加**libc++.dylib**；
#### 二. Realm 中的相关术语
...
使用内存数据库需要注意的是：
- 内存数据库会在临时文件夹中创建多个文件，用来协调处理诸如跨进程通知之类的事务。 实际上没有任何的数据会被写入到这些文件当中，除非操作系统由于内存过满， ~~需要清除磁盘上的多余空间。~~ 才会去把内存里面的数据存入到文件中。（感谢 @酷酷的哀殿 指出）
...
[被特殊类型标记的](https://realm.io/cn/docs/objc/latest/#optional-properties)NSNumber。CGFloat属性的支持被取消了，因为它不具备平台独立性。
...
文／一缕殇流化隐半边冰霜（简书作者）
原文链接：http://www.jianshu.com/p/50e0efb66bdf
