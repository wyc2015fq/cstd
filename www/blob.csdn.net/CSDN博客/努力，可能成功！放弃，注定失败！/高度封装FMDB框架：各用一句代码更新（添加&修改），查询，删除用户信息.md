# 高度封装FMDB框架：各用一句代码更新（添加&修改），查询，删除用户信息 - 努力，可能成功！放弃，注定失败！ - CSDN博客
置顶2016年10月10日 16:39:08[上天眷顾我](https://me.csdn.net/qq_30513483)阅读数：813标签：[高度封装FMDB框架																[iOS 资源大全																[iOS学习资料](https://so.csdn.net/so/search/s.do?q=iOS学习资料&t=blog)](https://so.csdn.net/so/search/s.do?q=iOS 资源大全&t=blog)](https://so.csdn.net/so/search/s.do?q=高度封装FMDB框架&t=blog)
...
## 2.1 需求
- 在插入板块中插入用户信息（用户id和用户名）。
- 在查询板块中，根据输入的用户id，可以显示对应的用户名。如果没有对应的用户id，则显示“没有对应id”。
- 在删除板块中，根据输入的用户id，可以删除对应的用户信息（包括用户id和用户名，也就是删除了表的一整行）。如果没有对应的用户id，则显示“没有对应id”。
- 点击导航栏右侧的按钮，进入“用户信息列表页”。在这一页显示当前表里的全部用户的信息（在cell的textLabel里显示用户名；在cell的detailTextLabel里显示用户id）。
## 2.2 效果图
![](http://upload-images.jianshu.io/upload_images/859001-404e4004af3b981a.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
左图：数据库操作页面（插入，查询，删除板块）| 右图：数据库全部用户信息
![](http://upload-images.jianshu.io/upload_images/859001-f32a934c4eb45af6.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
本地沙盒中sqlite表文件内容
### 2.3 代码讲解
**1. 最先导入这个封装类：**
...
- 操作表格前必须创建一次表格（一次就可以）。
- 只支持NSString类型的值。
- 除user_id字段以外只支持一个自定义字段。
- 不同的自定义字段必须对应不同的表格名，也就是说不同的自定义字段不能对应同一个表格名。
- 数据库操作API的返回值还不够完善，最好以枚举类型返回，有待改进。
- 没有使用FMDB的队列API。
...
文／Knight_SJ（简书作者）
原文链接：http://www.jianshu.com/p/7702f58be845
