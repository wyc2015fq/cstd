# self.delegate = self？ - 努力，可能成功！放弃，注定失败！ - CSDN博客
置顶2017年07月04日 23:29:41[上天眷顾我](https://me.csdn.net/qq_30513483)阅读数：1058标签：[iOS 资源大全																[iOS学习资料](https://so.csdn.net/so/search/s.do?q=iOS学习资料&t=blog)](https://so.csdn.net/so/search/s.do?q=iOS 资源大全&t=blog)
个人分类：[iOS开发](https://blog.csdn.net/qq_30513483/article/category/6072895)
## 本文来自简书原文地址:[http://www.jianshu.com/p/c92800c5a658](http://www.jianshu.com/p/c92800c5a658)
## 前言
...
## 莫名奇妙的现象
在项目中我们经常会用到 UITextField 类或者其子类，有时候为了图其方便会把 UITextField 的 delegate 设置为自己（
```
self.delegate
 = self
```
...
下面我们来看是什么原因导致的。
## 什么问题？
`self.delegate = self`（self 指 TXLimitedTextField 实例）调用栈无限递归？于是，我们针对
 TXLimitedTextField 类查找一下整个项目中有没有这种代码：
```
- (void)doSomething {
...
![](http://upload-images.jianshu.io/upload_images/1639372-1f148e036e7e8e96.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
about-delegate-conditional-break
进入断点调试后，发现一个有意思的事，如图所示：
![](http://upload-images.jianshu.io/upload_images/1639372-1bb5372cd769acbc.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
此时，细心的读者可能会产生一个疑惑，如果如上所述，那么上文提到新建的工程（TXLimitedTextField 类，如果写了 
```
self.delegate
 = self
```
）也应该会出现无限递归（死循环）才对啊？
![](http://upload-images.jianshu.io/upload_images/1639372-be5b2ec26160fbfe.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
about-delegate-self-infinite
然而事实上却没发生死循环。
笔者通过断点调试，发现 TXLimitedTextField 同样会调用 -keyboardInputChangedSelection:，断点截图同上，但不会出现死循环，最终导致程序崩溃的现象，笔者猜测分析，UITextField 类应该针对 
```
self.delegate
 = self
```
...
![](http://upload-images.jianshu.io/upload_images/1639372-76ea2a78b3a34f81.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
至于在 BlocksKit 中具体怎么处理该问题，在笔者的一个开源项目中进行了实践，在使用 InputKit 过程中，即使 
```
self.delegate
 = self
```
，也不会出现上述死循环的问题，本文不做详述，详见[InputKit](https://github.com/tingxins/InputKit).
## 闲谈
