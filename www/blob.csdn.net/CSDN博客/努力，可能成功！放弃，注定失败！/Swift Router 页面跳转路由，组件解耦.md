# Swift Router 页面跳转路由，组件解耦 - 努力，可能成功！放弃，注定失败！ - CSDN博客
置顶2017年08月07日 14:40:42[上天眷顾我](https://me.csdn.net/qq_30513483)阅读数：381标签：[Swift Router																[iOS 资源大全																[iOS学习资料](https://so.csdn.net/so/search/s.do?q=iOS学习资料&t=blog)](https://so.csdn.net/so/search/s.do?q=iOS 资源大全&t=blog)](https://so.csdn.net/so/search/s.do?q=Swift Router&t=blog)
个人分类：[iOS开发](https://blog.csdn.net/qq_30513483/article/category/6072895)
![](https://img-blog.csdn.net/20170807143924730)
[原文地址:http://www.jianshu.com/p/4ba2db91030b](http://www.jianshu.com/p/4ba2db91030b)
...
- [Router](https://github.com/anpufeng/Router/blob/master/Router/Router.swift) 比较Swifty,但是耦合比较严重，实现了很多暂时不需要的功能，每个VC需要将自己当成字符串告诉Router , 还要告诉当前的navigation , 使用了`NSClassFromString`根据字符创建的 AnyClass 。比较繁琐--
 但是还有很好的思想可以借鉴的
...
- 不能限制初始化方法
- 调用者应该非常简洁
那么不使用字符串很容易想到就是用枚举来替代，枚举中也可能映射VC呀，而且不用在`AppDelegate`中注册，不能限制初始化方法我们就用params字典去映射
[Router](https://github.com/anpufeng/Router/blob/master/Router/Router.swift) 里面的方式 感觉比较好 ， 自己肯定是知道自己怎么初始化的， 用协议的方式比用强迫字典初始化好很多
```
