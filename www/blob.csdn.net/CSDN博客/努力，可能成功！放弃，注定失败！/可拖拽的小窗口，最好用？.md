# 可拖拽的小窗口，最好用？ - 努力，可能成功！放弃，注定失败！ - CSDN博客
置顶2016年12月26日 10:13:46[上天眷顾我](https://me.csdn.net/qq_30513483)阅读数：771
# 一、前言
- 
...
![](http://upload-images.jianshu.io/upload_images/1085031-702608b4a959f9d9.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/600)
截图来自某牙直播
- 
**3、而且很久没分享了，像往常一样，封装轮子的过程都会详细介绍，分享更多的都是封装的思想。**
- 
**4、来看看效果图先吧：**
![](http://upload-images.jianshu.io/upload_images/1085031-da24d75b67f1e537.gif?imageMogr2/auto-orient/strip)
演示效果图，还可以吧
# 二、功能分析（针对所有控件）
- 
...
![](http://upload-images.jianshu.io/upload_images/1085031-64f6d296519d228a.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/800)
...
# 四、功能实现分析
- 
**1、分类或继承实现**。因为需要支持所有控件，我们都知道控件都是直接或者间接继承 `UIView` ，此时可以有两种办法实现，`继承` 和
`分类` ，此时我是用分类实现的，至于为什么呢？
...
- 
**3、Setter 方法里添加pan事件**。一开始想到的是通过 runtime 的 `Swizzle` 替换掉 系统的
`init` 方法，进而在自己的方法里面实现 拖拽手势添加，但后来考虑到，此时是给UIView 添加分类，而页面上的所有控件都是UIView 的 子类，此时 init 就会调用多次，会造成页面显示慢；于是直接在setter方法里面判断，当需要拖拽的时候，我才添加事件，不需要拖拽的时候移除手势，算是一个小优化。**此时还需要记录当前控件在父容器中的`index` ，这个等下就知道要用来干嘛。**
- 
...
- （3）、在拖拽结束（`UIGestureRecognizerStateEnd`）的时候，判断此时状态是否还重合，如果不重合，通过 
```
insertSubview
```
 将控件重置回原来的顺序（通过上面第三点记录的 `index`）
- 
...
```
![](http://upload-images.jianshu.io/upload_images/1085031-85821e9f5b229bb8.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
计算小坑
# 五、注意点：
- 1、如果你是使用约束来布局的话，此时可能出现当你拖拽出一定范围后，能正常停留在当前位置；但当你此时拖拽后的位置在约束好的位置附近，就会被吸附回去，如下图：（**此时图片是使用约束布局**）
![](http://upload-images.jianshu.io/upload_images/1085031-366a0e53b67e67a2.gif?imageMogr2/auto-orient/strip)
使用约束布局可能会出现这种情况
- 2、如果手势失败，会有断言提示。
# 六、总结
- 
...
![](http://upload-images.jianshu.io/upload_images/1085031-eb99cb25bcbaa15c.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
cocoapod
- 
**2、代码不难实现，具体代码就不拷贝上来了，代码中计算相对多一点，文字分析应该比较清楚了，如果还有什么其他问题，尽管找我。**
- 
**3、[Github地址](https://github.com/gitkong/FLDragView) ，欢迎大家关注我，喜欢给个star 和 like ，你的支持是我最大的动力，会随时更新原创文章喔！**
文／gitKong（简书作者）
原文链接：http://www.jianshu.com/p/42a6d524c37b
