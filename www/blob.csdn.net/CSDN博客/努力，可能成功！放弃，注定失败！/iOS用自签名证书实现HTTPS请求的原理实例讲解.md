# iOS用自签名证书实现HTTPS请求的原理实例讲解 - 努力，可能成功！放弃，注定失败！ - CSDN博客
置顶2016年12月21日 16:33:43[上天眷顾我](https://me.csdn.net/qq_30513483)阅读数：716标签：[iOS用自签名证书实现HTTPS请求																[iOS学习资料																[iOS常用方法																[iOS 资源大全](https://so.csdn.net/so/search/s.do?q=iOS 资源大全&t=blog)](https://so.csdn.net/so/search/s.do?q=iOS常用方法&t=blog)](https://so.csdn.net/so/search/s.do?q=iOS学习资料&t=blog)](https://so.csdn.net/so/search/s.do?q=iOS用自签名证书实现HTTPS请求&t=blog)
...
![](http://upload-images.jianshu.io/upload_images/938153-ad884d8caaf5a7fe.jpg?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
### HTTPS：
...
```
详细代码发布在了[github](https://github.com/orwater/HTTPSAdapterDemo)上，**使用时请注意在`ViewController`和`NetworkManager`中`#Warning`的提示**，将证书拖入项目并在`NetworkManager`中添加证书名称，在`ViewController`添加自己的URL。
文章中如有错误和疏漏，欢迎留言指教讨论。
#### 参考文章：
...
文／LannisZheng（简书作者）
原文链接：http://www.jianshu.com/p/0109f45395e3
