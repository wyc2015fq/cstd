# 奇异值分解(SVD)原理详解 - 苝花向暖丨楠枝向寒 - CSDN博客

2019年01月30日 16:46:26[苝花向暖丨楠枝向寒](https://me.csdn.net/weixin_40247263)阅读数：39


转自：[点击进入](https://blog.csdn.net/xiaocong1990/article/details/54909126/)
...
![image](https://img-blog.csdnimg.cn/2019013016382784.png)

这时候λ就被称为特征向量v对应的特征值，一个矩阵的一组特征向量是一组正交向量。特征值分解是将一个矩阵分解成下面的形式：

![image](https://img-blog.csdnimg.cn/20190130163837984.png)
...
![image](https://img-blog.csdnimg.cn/20190130163848472.png)

它其实对应的线性变换是下面的形式：

![image](https://img-blog.csdnimg.cn/20190130163900638.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MDI0NzI2Mw==,size_16,color_FFFFFF,t_70)    因为这个矩阵M乘以一个向量(x,y)的结果是：

![image](https://img-blog.csdnimg.cn/20190130163914905.png)

上面的矩阵是对称的，所以这个变换是一个对x，y轴的方向一个拉伸变换（每一个对角线上的元素将会对一个维度进行拉伸变换，当值>1时，是拉长，当值<1时时缩短），当矩阵不是对称的时候，假如说矩阵是下面的样子：
![image](https://img-blog.csdnimg.cn/20190130163927440.png)

它所描述的变换是下面的样子：
![image](https://img-blog.csdnimg.cn/20190130163947418.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MDI0NzI2Mw==,size_16,color_FFFFFF,t_70)
...
![image](https://img-blog.csdnimg.cn/2019013016403395.png)
...
![image](https://img-blog.csdnimg.cn/201901301640468.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MDI0NzI2Mw==,size_16,color_FFFFFF,t_70)

那么奇异值和特征值是怎么对应起来的呢？首先，我们将一个矩阵A的转置 * A，将会得到一个方阵，我们用这个方阵求特征值可以得到：![image](https://img-blog.csdnimg.cn/20190130164113855.png)

这里得到的v，就是我们上面的右奇异向量。此外我们还可以得到：

![image](https://img-blog.csdnimg.cn/20190130164124275.png)

这里的σ就是上面说的奇异值，u就是上面说的左奇异向量。奇异值σ跟特征值类似，在矩阵Σ中也是从大到小排列，而且σ的减少特别的快，在很多情况下，前10%甚至1%的奇异值的和就占了全部的奇异值之和的99%以上了。也就是说，我们也可以用前r大的奇异值来近似描述矩阵，这里定义一下部分奇异值分解：

![image](https://img-blog.csdnimg.cn/20190130164135384.png)

r是一个远小于m、n的数，这样矩阵的乘法看起来像是下面的样子：
![image](https://img-blog.csdnimg.cn/20190130164158636.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MDI0NzI2Mw==,size_16,color_FFFFFF,t_70)
...
anczos迭代就是一种解对称方阵部分特征值的方法（之前谈到了，解A’* A得到的对称方阵的特征值就是解A的右奇异向量），是将一个对称的方程化为一个三对角矩阵再进行求解。按网上的一些文献来看，Google应该是用这种方法去做的奇异值分解的。请见Wikipedia上面的一些引用的论文，如果理解了那些论文，也“几乎”可以做出一个SVD了。
...
![image](https://img-blog.csdnimg.cn/20190130164241680.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MDI0NzI2Mw==,size_16,color_FFFFFF,t_70)
...
![image](https://img-blog.csdnimg.cn/20190130164412101.png)

这样就从一个m行的矩阵压缩到一个r行的矩阵了，对SVD来说也是一样的，我们对SVD分解的式子两边乘以U的转置U’

![image](https://img-blog.csdnimg.cn/20190130164425555.png)
...
![image](https://img-blog.csdnimg.cn/20190130164445886.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MDI0NzI2Mw==,size_16,color_FFFFFF,t_70)      这就是一个矩阵，不过不太一样的是，这里的一行表示一个词在哪些title中出现了（一行就是之前说的一维feature），一列表示一个title中有哪些词，（这个矩阵其实是我们之前说的那种一行是一个sample的形式的一种转置，这个会使得我们的左右奇异向量的意义产生变化，但是不会影响我们计算的过程）。比如说T1这个title中就有guide、investing、market、stock四个词，各出现了一次，我们将这个矩阵进行SVD，得到下面的矩阵：
![image](https://img-blog.csdnimg.cn/20190130164459467.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MDI0NzI2Mw==,size_16,color_FFFFFF,t_70)      左奇异向量表示词的一些特性，右奇异向量表示文档的一些特性，中间的奇异值矩阵表示左奇异向量的一行与右奇异向量的一列的重要程序，数字越大越重要。
