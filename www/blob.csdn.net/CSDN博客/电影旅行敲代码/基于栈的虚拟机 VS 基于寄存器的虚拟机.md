# 基于栈的虚拟机 VS 基于寄存器的虚拟机 - 电影旅行敲代码 - CSDN博客
2015年12月17日 21:49:52[电影旅行敲代码](https://me.csdn.net/dashuniuniu)阅读数：5660标签：[编译器																[虚拟机](https://so.csdn.net/so/search/s.do?q=虚拟机&t=blog)](https://so.csdn.net/so/search/s.do?q=编译器&t=blog)
个人分类：[编译																[虚拟机](https://blog.csdn.net/dashuniuniu/article/category/6035487)](https://blog.csdn.net/dashuniuniu/article/category/2441797)
# 引子
...
![这里写图片描述](https://img-blog.csdn.net/20151217205351712)
### **基于栈的虚拟机**
...
首先从符号表上读取数据压入操作数栈，
![这里写图片描述](https://img-blog.csdn.net/20151217212448709)
然后从栈中弹出操作数执行加法运算，这步操作有物理机器执行，如下图所示：
![这里写图片描述](https://img-blog.csdn.net/20151217213928439)
...
![这里写图片描述](https://img-blog.csdn.net/20151218140426182)
从上图中我们可以看到，其实“寄存器”的概念只是当前栈帧中一块连续的内存区域。这些数据在运算的时候，直接送入物理CPU进行计算，无需再传送到operand stack上然后再进行运算。例如”ADD R3, R2, R1”的示意图就如下所示：
![这里写图片描述](https://img-blog.csdn.net/20151218141034563)
其实”ADD R3, R2, R1”还要经过译码的一个过程，当然当前这条指令的种类和操作数由虚拟机进行解释。后面我们会看到，在有些实现中，有一个很大的switch-case来进行指令的分派及真正的运算过程。
下图是Lua虚拟机的一些指令，该图片来自[这篇文章](https://docs.google.com/viewer?url=http://www.lua.org/doc/jucs05.pdf)，中译文[这里](https://docs.google.com/viewer?url=http://www.codingnow.com/2000/download/The%2520Implementation%2520of%2520Lua5.0.pdf)。
![这里写图片描述](https://img-blog.csdn.net/20151218101126658)
...
一种JVM基于SBD实现方式如下图所示：
![这里写图片描述](https://img-blog.csdn.net/20151219114623058)
