# Linux 下几款程序内存泄漏检查工具 - youbingchen的博客 - CSDN博客





2016年07月23日 10:15:06[youbingchen](https://me.csdn.net/youbingchen)阅读数：17194标签：[linux																[内存泄露](https://so.csdn.net/so/search/s.do?q=内存泄露&t=blog)](https://so.csdn.net/so/search/s.do?q=linux&t=blog)
...
![这里写图片描述](https://img-blog.csdn.net/20160723101300498)
...
![这里写图片描述](https://img-blog.csdn.net/20160723101334842)
...
![这里写图片描述](https://img-blog.csdn.net/20160723101414062)
...
# Valgrind 安装
- 1.解压安装包
`tar -jxvf valgrind-3.11.0.tar.bz2  -C /usr/local/src`- 2.进入目录安装

```bash
cd /usr/local/src/valgrind-3.11.0
```
- 3.运行./autogen.sh 设置环境（需要标准的autoconf工具）
`./autogen.sh `- 4.配置Valgrind，生成MakeFile文件
`./configure --prefix=/usr/local`- 5.编译和安装valgrind
`make  && make install`
# valgrind 使用
- 
第一步：准备好程序
...
![这里写图片描述](https://img-blog.csdn.net/20160723101435766)
![这里写图片描述](https://img-blog.csdn.net/20160723101444208)
