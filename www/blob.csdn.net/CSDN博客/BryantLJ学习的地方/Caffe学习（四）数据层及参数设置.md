# Caffe学习（四）数据层及参数设置 - BryantLJ学习的地方 - CSDN博客





2016年08月06日 08:40:24[遍地流金](https://me.csdn.net/u012177034)阅读数：37262
...
使用该方法来读入数据时，可以靠调用`MemoryDataLayer::Reset` (from C++) or`Net.set_input_arrays` (from Python)来指定一个具体的数据地址，如通常的存放所有输入数据的四维数组的首地址，这样就可以每次从改地址内读取batch_size大小的数据。
