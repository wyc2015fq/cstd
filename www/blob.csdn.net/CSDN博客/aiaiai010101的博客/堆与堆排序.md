# 堆与堆排序 - aiaiai010101的博客 - CSDN博客

2017年07月12日 19:49:36[aiaiai010101](https://me.csdn.net/aiaiai010101)阅读数：121
个人分类：[C++](https://blog.csdn.net/aiaiai010101/article/category/6791855)



**说堆的文章网上一大堆,不过说清楚的没几个,这篇凑乎,其实有几处内容也没有说的很清楚,我已经加了注释.**

**精通八大排序[算法](http://lib.csdn.net/base/31)系列：二、堆排序算法**
...
**注释：这里需要注意一点,这个MAX-HEAPIFY能够使得以i结点为根的树变为一个最大堆,**

**但是这里漏掉了非常关键的一部分：这个算法成立的前提是i的左子树和右子树都已经是最大堆了。**

**不然的话,比如下图,倘若结点10的值不是1,而是21,那么这个算法调整之后,右边以7为root的子树根本不受影响,完全无变化.**

**换句话说,应当把这个算法视为对某一个已经成型的最大堆的维护操作,而不要以为它能直接使得一个无序堆变成最大堆(当然了,也不是不可以,只是得循环使用了,由于它只能对已有的最大堆进行维护,那么想把一个无序堆变成最大堆,就得从最底层结点开始调整,因为底层调整成最大堆了,那么上一层结点才能在此基础上调用**MAX-HEAPIFY操作使得上一层结点也变成了最大堆,如此循环,直到到达顶层结点.**)**

**另外,递归的终止条件是当前结点已经成为叶结点(即没有左子结点)或者当前结点大于其任一子结点(即上述算法最后一个判断).**

**后边的堆排序其实也是建立在以上前提的基础上,想一想,为什么要把顶层结点(即最大值)与最后一个结点交换,当然这是出于节省空间的考虑,假设引入辅助空间,也只能在存储最大值之后,将最底层的结点选一个放入顶层结点,因为底层结点必然是叶结点,将其与堆割裂完全不影响整个堆依然是最大堆的事实.**
...
2  for i ← |_length[A]/2_| downto 1         **注释：之所以从length[A]/2开始调整,是因为大于此值的肯定都是叶结点.**
