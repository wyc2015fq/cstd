...
# Google Spanner原理- 全球级的分布式数据库 - forever1dreamsxx--NLP - CSDN博客


2012年09月19日 09:21:49[forever1dreamsxx](https://me.csdn.net/forever1dreamsxx)阅读数：1088


原文地址：[http://qing.weibo.com/2294942122/88ca09aa3300221n.html](http://qing.weibo.com/2294942122/88ca09aa3300221n.html)
标签:大数据云计算
...
![Google Spanner原理- 全球级的分布式数据库](http://ww2.sinaimg.cn/mw600/88ca09aajw1dx0vj232pkj.jpg)查看大图
...
**Spanner背景**要搞清楚Spanner原理，先得了解Spanner在Google的定位。
![Google Spanner原理- 全球级的分布式数据库](http://ww4.sinaimg.cn/mw600/88ca09aajw1dx0w7i97mqj.jpg)查看大图



从上图可以看到。Spanner位于F1和GFS之间，承上启下。所以先提一提F1和GFS。

**F1**和众多互联网公司一样，在早期Google大量使用了Mysql。Mysql是单机的，可以用Master-Slave来容错，分区来扩展。但是需要大量的手工运维工作，有很多的限制。因此Google开发了一个可容错可扩展的RDBMS——F1。和一般的分布式数据库不同，F1对应RDMS应有的功能，毫不妥协。起初F1是基于Mysql的，不过会逐渐迁移到Spanner。
F1有如下特点：
...
![Google Spanner原理- 全球级的分布式数据库](http://ww2.sinaimg.cn/mw600/88ca09aajw1dx0w8kj25ij.jpg)查看大图
...
![Google Spanner原理- 全球级的分布式数据库](http://ww1.sinaimg.cn/mw600/88ca09aajw1dx0way4308j.jpg)查看大图
...
![Google Spanner原理- 全球级的分布式数据库](http://ww4.sinaimg.cn/mw600/88ca09aajw1dx0wcbdvtjj.jpg)查看大图
...
![Google Spanner原理- 全球级的分布式数据库](http://ww3.sinaimg.cn/mw600/88ca09aajw1dx0wdup882j.jpg)查看大图
...
**TrueTime**
![Google Spanner原理- 全球级的分布式数据库](http://ww2.sinaimg.cn/mw600/88ca09aajw1dx0wey8mmmj.jpg)查看大图
...
![Google Spanner原理- 全球级的分布式数据库](http://ww4.sinaimg.cn/mw600/88ca09aajw1dx0wggmkzgj.jpg)查看大图
...
**只读事务**对于只读事务，Spanner首先要指定一个读事务时间戳。还需要了解在这个读操作中，需要访问的所有的读的Key。Spanner可以自动确定Key的范围。
