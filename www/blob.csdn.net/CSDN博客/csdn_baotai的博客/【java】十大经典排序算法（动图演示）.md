# 【java】十大经典排序算法（动图演示） - csdn_baotai的博客 - CSDN博客

2018年05月12日 19:17:17[csdn-[baotai]](https://me.csdn.net/csdn_baotai)阅读数：2199
...
1.3代码实现
|1234五678910111213|`function bubbleSort(arr) {``    ``var``len = arr.length;``    ``for``(``var``i = 0; i < len; i++) {``        ``for``(``var``j = 0; j < len - 1 - i; j++) {``            ``if``(arr[j] > arr[j+1]) {       ``// 相邻元素两两对比``                ``var``temp = arr[j+1];       ``// 元素交换``                ``arr[j+1] = arr[j];``                ``arr[j] = temp;``            ``}``        ``}``    ``}``    ``return``arr;``}`|
...
2.3代码实现
|1234五678910111213141516|`function selectionSort(arr) {``    ``var``len = arr.length;``    ``var``minIndex, temp;``    ``for``(``var``i = 0; i < len - 1; i++) {``        ``minIndex = i;``        ``for``(``var``j = i + 1; j < len; j++) {``            ``if``(arr[j] < arr[minIndex]) {    ``// 寻找最小的数``                ``minIndex = j;                ``// 将最小数的索引保存``            ``}``        ``}``        ``temp = arr[i];``        ``arr[i] = arr[minIndex];``        ``arr[minIndex] = temp;``    ``}``    ``return``arr;``} `|
...
3.2代码实现
|1234五67891011121314|`function insertionSort(arr) {``    ``var``len = arr.length;``    ``var``preIndex, current;``    ``for``(``var``i = 1; i < len; i++) {``        ``preIndex = i - 1;``        ``current = arr[i];``        ``while``(preIndex >= 0 && arr[preIndex] > current) {``            ``arr[preIndex + 1] = arr[preIndex];``            ``preIndex--;``        ``}``        ``arr[preIndex + 1] = current;``    ``}``    ``return``arr;``}`|
...
4.3代码实现
|1234五6789101112131415161718|`function shellSort(arr) {``    ``var``len = arr.length,``        ``temp,``        ``gap = 1;``    ``while``(gap < len / 3) {         ``// 动态定义间隔序列``        ``gap = gap * 3 + 1;``    ``}``    ``for``(gap; gap > 0; gap = Math.floor(gap / 3)) {``        ``for``(``var``i = gap; i < len; i++) {``            ``temp = arr[i];``            ``for``(``var``j = i-gap; j > 0 && arr[j]> temp; j-=gap) {``                ``arr[j + gap] = arr[j];``            ``}``            ``arr[j + gap] = temp;``        ``}``    ``}``    ``return``arr;``}`|

4.4算法分析

希尔排序的核心在于间隔序列的设定。既可以提前设定好间隔序列，也可以动态的定义间隔序列。动态定义间隔序列的算法是“算法（第4版）”的合着者Robert Sedgewick提出的。　
...
8.3代码实现
|1234五678910111213141516171819202122|`function countingSort(arr, maxValue) {``    ``var``bucket =``new``Array(maxValue + 1),``        ``sortedIndex = 0;``        ``arrLen = arr.length,``        ``bucketLen = maxValue + 1;``    ``for``(``var``i = 0; i < arrLen; i++) {``        ``if``(!bucket[arr[i]]) {``            ``bucket[arr[i]] = 0;``        ``}``        ``bucket[arr[i]]++;``    ``}``    ``for``(``var``j = 0; j < bucketLen; j++) {``        ``while``(bucket[j] > 0) {``            ``arr[sortedIndex++] = j;``            ``bucket[j]--;``        ``}``    ``}``    ``return``arr;``}`|
