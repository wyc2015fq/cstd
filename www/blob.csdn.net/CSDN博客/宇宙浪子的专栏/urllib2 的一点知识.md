# urllib2 的一点知识 - 宇宙浪子的专栏 - CSDN博客
2014年12月18日 10:58:12[_宇宙浪子_](https://me.csdn.net/bluehawksky)阅读数：362
刚好用到，这篇文章写得不错，转过来收藏。    转载自 道可道 | Python 标准库 urllib2 的使用细节
...
这里要注意的一个细节，使用 urllib2.install_opener() 会设置 urllib2 的全局 opener。这样后面的使用会很方便，但不能做更细粒度的控制，比如想在程序中使用两个不同的 Proxy 设置等。比较好的做法是不使用 install_opener 去更改全局的设置，而只是直接调用
