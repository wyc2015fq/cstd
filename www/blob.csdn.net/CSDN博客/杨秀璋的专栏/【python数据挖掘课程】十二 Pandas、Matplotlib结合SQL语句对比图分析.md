...
2017年03月20日 19:22:15[Eastmount](https://me.csdn.net/Eastmount)阅读数：4828
...
**        p1 = plt.subplot(221)        plt.bar(ind, num1, width, color='b', label='sum num')           plt.sca(p1)**
...
        运行效果如下所示，核心代码如下：
**data = np.array([num1, num2, num3, num4])        d = data.T #转置 12*4        df = DataFrame(d, index=hour1, columns=['All','2014', '2015', '2016'])        df.plot(kind='area', alpha=0.2) #设置颜色 透明度        plt.savefig('csdn.png',dpi=400)         plt.show()**
...
**data = np.random.rand(7,3)        print data        i = 0        while i<7:            data[i][0] = d1[i]            data[i][1] = d2[i]            data[i][2] = d3[i]            i = i + 1            matplotlib.style.use('ggplot')        #数据[7,3]数组 name为星期 columns对应年份        df=DataFrame(data, index=name, columns=['2008','2011','2016'])        df.plot(kind='bar')           plt.show()**
...
[利用Python进行数据分析——绘图和可视化(八) （强推）](http://www.bubuko.com/infodetail-134516.html)[用 Seaborn 画出好看的分布图（Python） [强推]](http://www.open-open.com/lib/view/open1434182977754.html)
