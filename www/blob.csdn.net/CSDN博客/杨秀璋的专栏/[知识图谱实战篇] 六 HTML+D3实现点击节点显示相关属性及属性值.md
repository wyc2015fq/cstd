...
2019年02月13日 18:25:57[Eastmount](https://me.csdn.net/Eastmount)阅读数：246
...
上述代码将获取六大实体的所有信息存储至 all_data.json 文件中，采用 data[tmp[‘name’]]=tmp 赋值变量data。接着将该json文件放入html目录中，供其调用。
...
此时，能够显示了选中节点的属性-属性值，显示如下图所示，但是其颜色和布局均未设置，后续将通过CSS进行设置。
...
同时，打开浏览器的Console也能查看对应节点的信息，通过 console.log(info[name]) 打印。
