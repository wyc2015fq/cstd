# mysql索引原理及慢查询优化 - fanyun的博客 - CSDN博客
2016年03月21日 13:57:09[老樊Lu码](https://me.csdn.net/fanyun_01)阅读数：843标签：[mysql](https://so.csdn.net/so/search/s.do?q=mysql&t=blog)
...
![](http://ww4.sinaimg.cn/mw690/7178f37egw1err37xzfakj20f808x75p.jpg)
考虑到磁盘IO是非常高昂的操作，计算机操作系统做了一些优化，当一次IO时，不光把当前磁盘地址的数据，而是把相邻的数据也都读取到内存缓冲区内，因为局部预读性原理告诉我们，当计算机访问一个地址的数据的时候，与其相邻的数据也会很快被访问到。每一次IO读取的数据我们称之为一页(page)。具体一页有多大数据跟操作系统有关，一般为4k或8k，也就是我们读取一页内的数据时候，实际上才发生了一次IO，这个理论对于索引的数据结构设计非常有帮助。
### 索引的数据结构
前面讲了生活中索引的例子，索引的基本原理，数据库的复杂性，又讲了操作系统的相关知识，目的就是让大家了解，任何一种数据结构都不是凭空产生的，一定会有它的背景和使用场景，我们现在总结一下，我们需要这种数据结构能够做些什么，其实很简单，那就是：每次查找数据时把磁盘IO次数控制在一个很小的数量级，最好是常数数量级。那么我们就想到如果一个高度可控的多路搜索树是否能满足需求呢？就这样，b+树应运而生。
### 详解b+树
![](http://ww2.sinaimg.cn/mw690/7178f37egw1err37xke42j20hc08caax.jpg)
...
```
|
 PRIMARY     | cl         | range | cm_log_cls_id,idx_last_upd_date | idx_last_upd_date     |
```
`8`
```
|
...
```
|
 DERIVED     | emp        | ALL   | NULL                            | NULL                  | NULL    | NULL              |
```
`13317`
```
|
...
```
|
 DERIVED     | emp_cert   | ref   | emp_certificate_empid           | emp_certificate_empid |
```
`4`
```
|
...
```
|
 PRIMARY      | emp        | eq_ref | PRIMARY                         | PRIMARY           |
```
`4`
```
|
 meituanorg.cl.ref_oid |
```
`1`
```
|
...
```
|
 UNION        | cl         | range  | cm_log_cls_id,idx_last_upd_date | idx_last_upd_date |
```
`8`
```
|
...
```
|
 UNION        | ec         | eq_ref | PRIMARY,emp_certificate_empid   | PRIMARY           |
```
`4`
```
|
...
```
|
 UNION        | emp        | eq_ref | PRIMARY                         | PRIMARY           |
```
`4`
```
|
 meituanorg.ec.emp_id  |
```
`1`
```
|
...
```
|
 SIMPLE      | sp    | ALL  | NULL          | NULL | NULL    | NULL |
```
`3613155`
```
|
...
```
|
 SIMPLE      | oei   | ref    | idx_category_left_right,idx_data_id | idx_category_left_right |
```
`5``|``const``|``8849`
```
|
...
```
|
 SIMPLE      | bu    | ref    | PRIMARY,idx_userid_status           | idx_userid_status       |
```
`4`
```
|
 meituancrm.oei.data_id   |
```
`76`
```
|
...
```
|
 SIMPLE      | cb    | ref    | idx_branch_id,idx_contact_branch_id | idx_branch_id           |
```
`4`
```
|
 meituancrm.bu.branch_id  |
```
`1`
```
|                                             
...
```
|
 SIMPLE      | c     | eq_ref | PRIMARY                             | PRIMARY                 |
```
`108`
```
|
 meituancrm.cb.contact_id |
```
`1`
```
|                                             
