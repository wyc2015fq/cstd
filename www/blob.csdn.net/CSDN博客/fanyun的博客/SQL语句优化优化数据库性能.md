# SQL语句优化优化数据库性能 - fanyun的博客 - CSDN博客
2016年03月21日 13:51:49[老樊Lu码](https://me.csdn.net/fanyun_01)阅读数：730标签：[mysql](https://so.csdn.net/so/search/s.do?q=mysql&t=blog)
...
trunc(sk_rq)=trunc(sysdate)，
 优化处理：sk_rq>=trunc(sysdate)
```
`and``sk_rq<trunc(sysdate+1)`
```
...
```
qc_bh
 || kh_bh=’5400250000’，优化处理：qc_bh=’5400’
```
`and``kh_bh=’250000’`
```
4. 更多方面SQL优化资料分享
