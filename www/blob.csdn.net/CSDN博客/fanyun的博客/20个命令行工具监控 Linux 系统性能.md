# 20个命令行工具监控 Linux 系统性能 - fanyun的博客 - CSDN博客
2016年03月21日 14:00:50[老樊Lu码](https://me.csdn.net/fanyun_01)阅读数：1023
所属专栏：[](https://blog.csdn.net/column/details/.html)[Linux系统开发维护实战](https://blog.csdn.net/column/details/linux-01.html)
**   译文出自：[Linux story-FOREST](http://www.linuxstory.org/command-line-tools-to-monitor-linux-performance/)**
...
## 1. top —Linux系统进程监控
top 命令是性能监控程序，它可以在很多 Linux/Unix 版本下使用，并且它也是 Linux系统管理员经常使用的监控系统性能的工具。Top 命令可以定期显示所有正在运行和实际运行并且更新到列表中，它显示出 CPU 的使用、内存的使用、交换内存、缓存大小、缓冲区大小、过程控制、用户和更多命令。它也会显示内存和 CPU 使用率过高的正在运行的进程。当我们对
...
```
r 
 b   swpd   free  inact active   si   so    bi    bo
```
`in`
```
cs us sy id wa st
...
```
listening
 on eth0, link-type EN10MB (Ethernet), capture size
```
`96`
```
bytes
...
IP tecmint.com.ssh >
```
`115.113``.``134.3``.``static``-mumbai.vsnl.net.``in``.``28472`
```
:
 P
```
`2532133365``:``2532133481``(``116`
```
)
 ack
```
