# 读书笔记 23种设计模式总结 - 去向前方的博客 - CSDN博客





2018年07月16日 10:33:21[Big_quant](https://me.csdn.net/lvsehaiyang1993)阅读数：145
...
- [1.单例模式（Singleton Pattern）](#1单例模式singleton-pattern)
- [2.工厂模式](#2工厂模式)
- [3.抽象工厂模式（Abstract Factory Pattern）](#3抽象工厂模式abstract-factory-pattern)
- [4.模板方法模式（Template Method Pattern）](#4模板方法模式template-method-pattern)
- [5.建造者模式（Builder Pattern）](#5建造者模式builder-pattern)
- [6.代理模式（Proxy Pattern）](#6代理模式proxy-pattern)
- [7.原型模式（Prototype Pattern）](#7原型模式prototype-pattern)
- [8.中介者模式](#8中介者模式)
- [9.命令模式](#9命令模式)
- [10.责任链模式](#10责任链模式)
- [11.装饰模式（Decorator Pattern）](#11装饰模式decorator-pattern)
- [12.策略模式（Strategy Pattern）](#12策略模式strategy-pattern)
- [13.适配器模式（Adapter Pattern）](#13适配器模式adapter-pattern)
- [14.迭代器模式（Iterator Pattern）](#14迭代器模式iterator-pattern)
- [16.观察者模式（Observer Pattern）](#16观察者模式observer-pattern)
- [17.门面模式（Facade Pattern）](#17门面模式facade-pattern)
- [19.访问者模式（Visitor Pattern）](#19访问者模式visitor-pattern)
- [21.解释器模式（Interpreter Pattern）（少用）](#21解释器模式interpreter-pattern少用)
- [22.享元模式（Flyweight Pattern）](#22享元模式flyweight-pattern)
- [23.桥梁模式（Bridge Pattern）](#23桥梁模式bridge-pattern)
...
3.覆盖或实现父类的方法时输入参数可以被放大 
        ...
        如果父类的输入参数类型大于子类的输入参数类型，会出现父类存在的地方，子类未必会存在，因为一旦把子类作为参数传入，调用者很可能进入子类的方法范畴。- 覆写或实现父类的方法时输出结果可以被缩小 
