# Python学习：数据结构 - 去向前方的博客 - CSDN博客





2018年07月21日 09:45:42[Big_quant](https://me.csdn.net/lvsehaiyang1993)阅读数：377
...
- [目录：](#目录)
- [5. 数据结构](#5-数据结构)- [5.1. 关于列表更多的内容](#51-关于列表更多的内容)
- [5.1.1. 把列表当作堆栈使用](#511-把列表当作堆栈使用)

- [5.1.2. 把列表当作队列使用](#512-把列表当作队列使用)- [5.1.3. 列表推导式](#513-列表推导式)
- [5.1.4. 嵌套的列表推导式](#514-嵌套的列表推导式)
- [5.2. del 语句](#52-del-语句)
...
也许大家会发现像 insert， remove 或者 sort 这些修改列表的方法没有打印返回值–它们返回 None。  [1](#fn:1)  在 python 中对所有可变的数据类型这是统一的设计原则
...
对一个字典执行 list(d.keys()) 将返回一个字典中所有关键字组成的无序列表（如果你想要排序，只需使用 sorted(d.keys()) [2](#fn:2).  使用 in 关键字（指Python语法）可以检查字典中是否存在某个关键字（指字典

>

> 
它支持支持成员测试以及迭代等操作，但是它的内容不是独立的原始字典 – 它只是一个 视图。 
...
- 别的语言可能会返回一个变化的对象，允许方法连续执行，像 d->insert(“a”)->remove(“b”)->sort();。 [↩](#fnref:1)
- 调用 d.keys() 将会返回一个 dictionary view 对象。 [↩](#fnref:2)
