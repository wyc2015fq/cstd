# Python学习笔记：错误和异常 - 去向前方的博客 - CSDN博客





2018年07月26日 12:18:55[Big_quant](https://me.csdn.net/lvsehaiyang1993)阅读数：551
...
开胃菜参考：[开胃菜](https://blog.csdn.net/lvsehaiyang1993/article/details/81054512)

使用Python解释器：[使用Python解释器](https://blog.csdn.net/lvsehaiyang1993/article/details/81060747)

本文对Python的简介：[Python简介](https://blog.csdn.net/lvsehaiyang1993/article/details/81113004)

Python流程介绍：[深入Python流程](https://blog.csdn.net/lvsehaiyang1993/article/details/81134064)

Python数据结构：[Python数据结构](https://blog.csdn.net/lvsehaiyang1993/article/details/81141514)

Python：模块：[Python模块](https://blog.csdn.net/lvsehaiyang1993/article/details/81152869)

Python：输入和输出[Python输入和输出](https://blog.csdn.net/lvsehaiyang1993/article/details/81209254)
# 目录：



- [目录：](#目录)
- [8. 错误和异常](#8-错误和异常)- [8.1. 语法错误](#81-语法错误)
- [8.2. 异常](#82-异常)
- [8.3. 异常处理](#83-异常处理)
- [8.4. 抛出异常](#84-抛出异常)
- [8.5. 用户自定义异常](#85-用户自定义异常)
- [8.6. 定义清理行为](#86-定义清理行为)
- [8.7. 预定义清理行为](#87-预定义清理行为)
...
语法分析器指出错误行，并且在检测到错误的位置前面显示一个小“箭头”。 错误是由箭头 前面 的标记引起的（或者至少是这么检测的）： 这个例子中，函数 [print()](https://docs.python.org/3/library/functions.html#print) 被发现存在错误，因为它前面少了一个冒号（ ‘:’ ）。 错误会输出文件名和行号，所以如果是从脚本输入的你就知道去哪里检查错误了。
...
一个 try 语句可能包含多个 [except](https://docs.python.org/3/reference/compound_stmts.html#try) 子句，分别指定处理不同的异常。至多只会有一个分支被执行。异常处理程序只会处理对应的 try 子句中发生的异常，在同一个 try 语句中，其他子句中发生的异常则不作处理。一个 except 子句可以在括号中列出多个异常的名字，例如:
...
在异常名（列表）之后，也可以为 except 子句指定一个变量。这个变量绑定于一个异常实例，它存储在 instance.args 的参数中。为了方便起见，异常实例定义了 ***_str_*() **，这样就可以直接访问过打印参数而不必引用 .args。这种做法不受鼓励。相反，更好的做法是给异常传递一个参数（如果要传递多个参数，可以传递一个元组），把它绑定到 message 属性。一旦异常发生，它会在抛出前绑定所有指定的属性。
...
要抛出的异常由 raise 的唯一参数标识。它必需是一个异常实例或异常类（继承自 Exception 的[类](http://www.pythondoc.com/pythontutorial3/classes.html#tut-classes)）。
...
在这个例子中，[Exception](https://docs.python.org/3/library/exceptions.html#Exception) 默认的 [*__init__()](https://docs.python.org/3/reference/datamodel.html#object.__init__) 被覆盖。新的方式简单的创建 value 属性。这就替换了原来创建 args 属性的方式。
...
如你所见， finally 子句在任何情况下都会执行。TypeError 在两个字符串相除的时候抛出，未被 except 子句捕获，因此在 [finally](https://docs.python.org/3/reference/compound_stmts.html#finally) 子句执行完毕后重新抛出。
