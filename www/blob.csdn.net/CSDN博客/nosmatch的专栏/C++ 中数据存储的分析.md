# C++ 中数据存储的分析 - nosmatch的专栏 - CSDN博客
2011年07月26日 23:42:57[nosmatch](https://me.csdn.net/HDUTigerkin)阅读数：3949
...
**堆**，就是那些由 new 分配的[内存](http://product.yesky.com/memory/)块，他们的释放编译器不去管，由我们的应用程序去控制，一般一个 new 就要对应一个 delete。如果程序员没有释放掉，那么在程序结束后，[操作系统](http://product.yesky.com/os/)会自动回收。堆可以动态地扩展和收缩。
...
　　函数内部定义的变量，在程序执行到它的定义处时，编译器为它在栈上分配空间，函数在栈上分配的空间在此函数执行结束时会释放掉，这样就产生了一个问题: 如果想将函数中此变量的值保存至下一次调用时，如何实现？最容易想到的方法是定义一个全局的变量，但定义为一个全局变量有许多缺点，最明显的缺点是破坏了此变量的访问范围（使得在此函数中定义的变量，不仅仅受此函数控制）。需要一个数据对象为整个类而非某个对象服务，同时又力求不破坏类的封装性，即要求此成员隐藏在类的内部，对外不可见。
**　　static 的内部机制**：
