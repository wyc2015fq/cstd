...
**[5](http://product.yesky.com/product/494/494148/). 使用PKILL快速杀掉进程**
　　笔者经常使用top、[kill](http://product.yesky.com/viruskillsoftware/kill/)或killall进程加上ps | aux查找进程号/进程名来杀掉进程。但是pkill命令则可去掉大量的工作，pkill [firefox](http://www.mydown.com/soft/network/browse/2/439502.shtml)就可以杀掉进程列表中所有与firefox符合的incheng。Pstree也是一个很酷的命令，它会根据所有关系以家庭树的方式显示所有进程。
**6.使用[Gnome](http://www.mydown.com/game/dmtp/36/423536.shtml)中心自定义Gnome**
...
**8.[快速清理](http://www.mydown.com/soft/utilitie/system/475/404475.shtml)磁盘垃圾**
...
**[9](http://product.yesky.com/product/149/149729/). 找出丢失的文件扩展名**
　　如果你收到一个没有扩展名的email附件，不知道是什么类型时([苹果](http://product.yesky.com/os/apple/)用户经常把文件扩展名忽略)，试一下file命令，直接加上文件名。同时也可以试一下strings命令，会显示出二进制文件中任何可显示的字符串，文件的类型通常就会在前面几行，所以用管道把这个命令和head连接起来是个好办法。
**10.不要忘了这些手册页面**
