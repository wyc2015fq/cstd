# 【java】生成二维码的util - Big Smile - CSDN博客
2019年01月20日 22:06:57[王啸tr1912](https://me.csdn.net/tr1912)阅读数：110
这两天接到了一个生成二维码的需求，和大家分享一下用工具生成二维码的快感：
1、首先我们需要在maven中引入依赖：
```
<dependency>
...
```
我们可以看到，这个util中的main方法中提供的示例包含了生成和读取两种方式的代码，我们如果只是生成的话，可以简化成这样：
```
public static String getQRcode(String text){
   ...
   try {
       generateQRCode(text, width, height, format, fileName);
   }catch (Exception e){
...
```
入参为二维码内容，返回值为生成二维码的文件路径，这样就很方便使用了，文件名字，用了字母+时间戳+1000内随机数构成，可以防止多线程访问的文件覆盖了。
分享一下我们二维码的生成之后的拿取和访问策略：
![](https://img-blog.csdnimg.cn/2019012022061782.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3RyMTkxMg==,size_16,color_FFFFFF,t_70)
