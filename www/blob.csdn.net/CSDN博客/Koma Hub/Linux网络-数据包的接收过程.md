# Linux网络 - 数据包的接收过程 - Koma Hub - CSDN博客
2019年03月26日 22:07:15[Koma_Wong](https://me.csdn.net/Rong_Toa)阅读数：16
个人分类：[Linux																[Socket](https://blog.csdn.net/Rong_Toa/article/category/7792858)](https://blog.csdn.net/Rong_Toa/article/category/7156254)
[Linux网络 - 数据包的接收过程](http://mp.weixin.qq.com/s?__biz=MzA3NTE0NTcxMQ==&mid=2662521041&idx=1&sn=68698f479bed1d1a41a946e24496c4ff&chksm=84337051b344f947efaffbf2c1cc792e0b006e1c5588e61649cce91dfd4f212c11aff3f7bd98&mpshare=1&scene=1&srcid=#rd)
> 
嵌入式Linux中文站 **嵌入式Linux中文站**
微信号 emblinux
功能介绍 嵌入式Linux中文站提供专业嵌入式Linux开发技术资讯
**Table of Contents**
[网卡到内存](#%E7%BD%91%E5%8D%A1%E5%88%B0%E5%86%85%E5%AD%98)
[内核的网络模块](#%E5%86%85%E6%A0%B8%E7%9A%84%E7%BD%91%E7%BB%9C%E6%A8%A1%E5%9D%97)
...
![](https://img-blog.csdnimg.cn/20190326213811476.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1JvbmdfVG9h,size_16,color_FFFFFF,t_70)
- 
...
![](https://img-blog.csdnimg.cn/20190326215112458.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1JvbmdfVG9h,size_16,color_FFFFFF,t_70)
- 
...
- 
**15：** 看是不是有AF_PACKET类型的socket（也就是我们常说的***原始套接字***），如果有的话，拷贝一份数据给它。**tcpdump抓包就是抓的这里的包**。
- 
...
![](https://img-blog.csdnimg.cn/20190326215928211.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1JvbmdfVG9h,size_16,color_FFFFFF,t_70)
- 
...
### UDP层
![](https://img-blog.csdnimg.cn/20190326220231277.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1JvbmdfVG9h,size_16,color_FFFFFF,t_70)
- 
...
- 
**sock_queue_rcv_skb：** 主要干了两件事，一是**检查这个socket的receive buffer是不是满了，如果满了的话，丢弃该数据包**，然后就是调用sk_filter看这个包是否是满足条件的包，**如果当前socket上设置了filter，且该包不满足条件的话，这个数据包也将被丢弃**（在Linux里面，每个socket上都可以像tcpdump里面一样定义filter，不满足条件的数据包将会被丢弃）
- 
...
## socket
应用层一般有两种方式接收数据，一种是**recvfrom**函数阻塞在那里等着数据来，这种情况下当socket收到通知后，recvfrom就会被唤醒，然后读取接收队列的数据；另一种是通过epoll或者select监听相应的socket，当收到通知后，再调用recvfrom函数去读取接收队列的数据。两种情况都能正常的接收到相应的数据包。
## 结束语
