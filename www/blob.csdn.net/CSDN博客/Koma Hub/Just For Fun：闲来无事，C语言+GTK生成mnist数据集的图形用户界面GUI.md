# Just For Fun：闲来无事，C语言+GTK生成mnist数据集的图形用户界面GUI - Koma Hub - CSDN博客
2018年03月18日 14:25:28[Koma_Wong](https://me.csdn.net/Rong_Toa)阅读数：199
...
还是老样子，先上[代码](https://github.com/RongToa/drawing-mnist-gtk-gui)：
```cpp
...
static void draw_brush (GtkWidget *widget, GdkInputSource source,
                        gdouble x, gdouble y, gdouble pressure)
{
    ...
    int ix =(int)( ((((int)x)%windowWidth)*1.0/(windowWidth*1.0)*width));
    int iy =(int)( ((((int)y)%windowHeight)*1.0/(windowHeight*1.0)*height));
    
    int i,j;
    for(i=ix-offset;i<ix+offset;i++){
        for(j=iy-offset;j<iy+offset;j++){
            if(i>=0&&i<width&&j>=0&&j<height){
                array[i*height + j] = 1.0;
            }
        }
    }
...
static gint motion_notify_event (GtkWidget *widget, GdkEventMotion *event)
{
    ...
    if (event->state & GDK_BUTTON1_MASK && pixmap != NULL)
    {
        if (gdk_device_get_history (event->device, event->window, 
                motion_time, event->time, &events, &n_events))
        {
...
{
    GtkWidget *dialog;
    static gint saveCount = 1;
    dialog = gtk_message_dialog_new (GTK_WINDOW (window),
				   GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
				   GTK_MESSAGE_INFO,
				   GTK_BUTTONS_OK,
				   "Panel save as file: \"binary\"\n" );
    ...
    fwrite(array,sizeof(float),width*height,fp);
    fclose(fp);
    saveCount++;
}
void quit (void) { gtk_main_quit (); }
int main (int argc, char *argv[])
{
    ...
    gtk_widget_set_size_request(window , windowWidth , windowHeight+50);
    ...
    gtk_widget_set_size_request (drawing_area, windowWidth, windowHeight);
    ...
    button = gtk_button_new_with_label ("OutputBinary");
    gtk_box_pack_start (GTK_BOX (vbox), button, FALSE, FALSE, 0);
    g_signal_connect (button, "clicked", G_CALLBACK (create_output_dialog), NULL);
...
![](https://img-blog.csdn.net/201803181413240)![](https://img-blog.csdn.net/20180318141331736)
然后用CWP的指令ximage显示这个生成的二进制文件“binary”：
`$ ximage < binary n1=64 perc=99`
![](https://img-blog.csdn.net/20180318141552698)
下面给出一些其他的例子：
![](https://img-blog.csdn.net/20180318141725340)![](https://img-blog.csdn.net/201803181417333)![](https://img-blog.csdn.net/20180318141741906)![](https://img-blog.csdn.net/20180318141750193)
![](https://img-blog.csdn.net/20180318141759981)![](https://img-blog.csdn.net/20180318141808645)
