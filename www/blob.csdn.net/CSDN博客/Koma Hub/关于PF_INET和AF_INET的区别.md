# 关于PF_INET和AF_INET的区别 - Koma Hub - CSDN博客
2019年01月26日 09:53:28[Koma_Wong](https://me.csdn.net/Rong_Toa)阅读数：65
个人分类：[Socket](https://blog.csdn.net/Rong_Toa/article/category/7792858)
...
理论上建立socket时是指定协议，应该用PF_xxxx，设置地址时应该用AF_xxxx。当然AF_INET和PF_INET的值是相同的，混用也 不会有太大的问题。也就是说你socket时候用PF_xxxx，设置的时候用AF_xxxx也是没关系的，这点随便找个TCPIP例子就可以验证出来了。如下，不论是AF_INET还是PF_INET都是可行的，只不过这样子的话，有点不符合规范。
```cpp
/* 服务器端开始建立socket描述符 */
