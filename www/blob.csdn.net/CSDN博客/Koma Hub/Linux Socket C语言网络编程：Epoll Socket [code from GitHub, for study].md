# Linux Socket C语言网络编程：Epoll Socket [code from GitHub, for study] - Koma Hub - CSDN博客
2018年07月14日 21:53:14[Koma_Wong](https://me.csdn.net/Rong_Toa)阅读数：135
...
#include<fcntl.h>
#ifndef CONNECT_SIZE
#defineCONNECT_SIZE256
#endif
#definePORT7777
#defineMAX_LINE2048
#defineLISTENQ20voidsetNonblocking(int sockfd)
{
	int opts;
...
intmain(int argc , char **argv)
{
	...
	/*声明epoll_event结构体变量，ev用于注册事件，数组用于回传要处理的事件*/struct epoll_event ev, events[20];
	/*(1) 得到监听描述符*/
	...
	/*注册事件*/epoll_ctl(epfd, EPOLL_CTL_ADD, listenfd, &ev);		
	...
	/*(4) 进入服务器接收请求死循环*/while(1)
	{
		...
		printf("nfds = %d\n" , nfds);
		/*处理发生的事件*/for(i=0 ; i<nfds ; ++i)
		{
			/*检测到用户链接*/if(events[i].data.fd == listenfd)
			{	
				/*接收客户端的请求*/
				...
				/*设置为非阻塞*/setNonblocking(connfd);
				ev.data.fd = connfd;
				ev.events = EPOLLIN | EPOLLET;
				epoll_ctl(epfd , EPOLL_CTL_ADD , connfd , &ev);
			}//if/*如果是已链接用户，并且收到数据，进行读入*/elseif(events[i].events & EPOLLIN){
			...
			}//elseelseif(events[i].events & EPOLLOUT)
			{
				if((sockfd = events[i].data.fd) < 0)
				continue;
				...
				ev.data.fd = sockfd;
				ev.events = EPOLLIN | EPOLLET;
				/*修改*/epoll_ctl(epfd , EPOLL_CTL_MOD , sockfd , &ev);
			}//else
		}//for
	}//whilefree(events);
	close(epfd);
...
## [](https://github.com/Rtoax/Knowledge/blob/master/Socket/epoll-socket.md#clientc)`client.c`
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
#include<errno.h>
...
## [](https://github.com/Rtoax/Knowledge/blob/master/Socket/epoll-socket.md#complied--run)complied & run
```
