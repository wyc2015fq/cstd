# /usr/include/dirent.h打开、读取、关闭目录 - Koma Hub - CSDN博客
2018年02月04日 19:56:10[Koma_Wong](https://me.csdn.net/Rong_Toa)阅读数：260
```cpp
struct dirent
{
...
{  
    ...
    if (!dir)  
    {  
        fprintf(stderr,"open directory error,%d\n",dir);  
        return 0;  
    }  
    while (filename = readdir(dir))  
    {  
        printf("%d, %d, %s\n",dir,filename->d_reclen,filename->d_name);  
    }  
    closedir(dir);  
    return 1;  
}
...
31285264, 24, .
31285264, 24, ..
31285264, 32, README.txt
31285264, 32, seismic
31285264, 24, test
31285264, 32, cExpert
31285264, 32, lib.so
31285264, 32, algorithms
31285264, 32, study-c++
31285264, 40, linuxTerminal
31285264, 32, include.h
```
而用linux的指令结果如下：
```
$ ls ../
algorithms  cExpert  include.h  lib.so  linuxTerminal  README.txt  seismic  study-c++  test
```
给出CentOS的/usr/include/dirent/h:
```cpp
