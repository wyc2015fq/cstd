# Stata：边际效应分析\交乘项的系数含义和图示 - stata连享会 - CSDN博客





2018年07月17日 09:43:47[arlionn](https://me.csdn.net/arlionn)阅读数：811
...
> 
作者：杨柳 | 连玉君  ( [知乎](https://zhuanlan.zhihu.com/arlion) | [简书](https://www.jianshu.com/u/69a30474ef33) | [码云](https://gitee.com/arlionn) )

[……Stata 现场培训报名中……](https://gitee.com/arlionn/stata_training/blob/master/README.md)- Stata连享会 [精品专题](https://gitee.com/arlionn/stata_training/blob/master/README.md)  || [精彩推文](https://github.com/arlionn/stata/blob/master/README.md)
...
[……Stata 现场培训报名中……](https://blog.csdn.net/arlionn/article/details/82746992)
...
Note: 有关如何使用因子变量的介绍请参见 [往期推文：Stata：因子变量的全攻略](https://www.jianshu.com/p/16b08797e591)。
...
#### 4.4.1  单个变量
- 
下面，仍以研究妇女工资的决定因素为例进行说明。在3.2.2节中，我们使用了 `stata` 的自带数据 `nlsw88.dta (1988年美国妇女小时工资)`，以 `wage (妇女的小时工资)` 作为被解释变量、以 `industry (行业类别)`、`union (是否工会成员)`、 `hours (每周工作小时数)`、`union (是否工会成员)` 与  `hours (每周工作小时数)` 的 `交乘项` 作为解释变量建立线性回归模型。`stata` 中的回归命令和结果见3.2.2节所述。
...
> 
**图10 当其他变量取均值，hours取不同值时，wage的预测边际值**
...
> 
**图11 当其他变量取均值，hours取值为4个数值时（均匀分布于hours变量的取值范围）wage的预测边际值**
...
> 
**图12 当其他变量取均值，hours取值为10至50之间时，wage的预测边际值**
...
> 
**图13 当其他变量取均值，hours取不同值时，工会与非工会成员的wage的预测边际值**
...
![](https://upload-images.jianshu.io/upload_images/7692714-8b1fb0b5068487af.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
![欢迎加入Stata连享会(公众号: StataChina)](http://upload-images.jianshu.io/upload_images/7692714-95f9bf00c48aa4f1.jpg?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
