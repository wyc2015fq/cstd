# neuraltalk2将eval.lua改成一个服务 不断的增量分析图像 - zj360202的专栏 - CSDN博客





2017年03月04日 16:34:28[zj360202](https://me.csdn.net/zj360202)阅读数：474
...
最后显示的时候，通过vis.json中的数据得到生成的对应图片的描述加上对应的图片显示在前端




接下来要把上面的对一个图片目录下面的数据进行批量标注的改成，监听一个目录 ，如果有文件就分析，如果没有就一直监听，这样就不用再多次加载模型了
...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    local sents = net_utils.decode_sequence(vocab, seq)
    for k=1,#sents do
      local entry = {image_id = img_index, caption = sents[k]}
      if opt.dump_path == 1 then
        entry.file_name = data.infos[k].file_path
      end
      table.insert(predictions, entry)
      if opt.dump_images == 1 then
        -- dump the raw image to vis/ folder   ------------cp  -->  mv      #predictions  -->  mg_index
        local cmd = 'mv "' .. path.join(opt.image_root, data.infos[k].file_path) .. '" vis/imgs/img' .. img_index .. '.jpg' -- bit gross
...
while true do
  ...
  if loader.N > 0 then
    ...
    local old_json = utils.read_json('vis/vis.json')
    for k,v in pairs(split_predictions) do
      table.insert(old_json,v)
    end
    if opt.dump_json == 1 then
      -- dump the json
      utils.write_json('vis/vis.json', old_json)
    end
  end
  utils.sleep(3)   --下面带上代码，在misc/utils文件中放着
end
```
...
这样，一个分析图像的服务就算是弄好了，只要有一个PYTHON或其它语言的一个输入流就可以了
