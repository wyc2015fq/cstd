...
2017年02月12日 13:25:10[悟乙己](https://me.csdn.net/sinat_26917383)阅读数：7289
...
但是笔者现在还有几个问题，- 这个ImageData只能显示一个batch的图像信息，不能在同一案例循环使用的是吧？
...
同时crop_size，new_height，new_width 一定要和论文中给出的一致（经验来源于博客：[Caffe C++API 提取任意一张图片的特征系列一—-ImageData](http://blog.csdn.net/sunshine_in_moon/article/details/50125899)）- 4、★crop_size  
...
caffe中提供了过采样的方法（oversample），详见/caffe/python/caffe/io.py，裁剪的是图片中央、4个角以及镜像共10张图片。（该部分见博客：[caffe中使用crop_size剪裁训练图片](http://blog.csdn.net/u010417185/article/details/52651761)）- 5、mirror:false，1表示开启镜像，0表示关闭，也可用ture和false来表示，是否要对输入图像采用随机水平镜像，**mirror可以产生镜像,弥补小数据集的不足**
