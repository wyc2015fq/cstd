...
2019年04月04日 23:17:25[IE06](https://me.csdn.net/kittyzc)阅读数：2739
...
$w,v$无限制
等价于
$\max_{w}\{wb+\max_{v:vB\le c-wA}vd\}$

等价于
$\max_{w}\{wb+\min_{x:Bx=d,x\ge 0}(c-wA)x\}$

内部的min问题是个线性规划问题，枚举可行域{$x : Bx=d,x\ge 0$}的极点（$I$）和极方向（$J$）便可以求解了。我们一边是逐步添加极点和极线约束，则原问题转为松弛主问题（RMP）：
...
求出来的$x$是原问题的一个极点或极线，将其添加到松弛主问题上继续求解。
## 2.3. benders分解求解步骤
...
- 将$w^*$代入对偶子问题（DSP）求解$x$，得到的最优值用来更新下界LB，并且给松弛主问题RMP添加约束条件$w(b-Ax^*)+cx^*\ge z$
