# 系统学习机器学习之神经网络（一）--MLP及局部模型,RBF综述 - 工作笔记 - CSDN博客





2015年12月22日 13:44:14[Eason.wxd](https://me.csdn.net/App_12062011)阅读数：15685
...
![](https://img-blog.csdn.net/20130601152525522)
...
而[多层感知器模型](http://blog.csdn.net/xiaowei_cqu/article/details/9004331)中，神经元传递函数是阶跃函数，输出端的只能对最后一个神经元系数求梯度，无法对其他权系数求梯度，所以无法利用梯度下降的方法学习其他的权值。Sigmoid函数

BP算法提出主要由于Sigmoid函数的出现，代替之前的阈值函数来构造神经元。

Sigmoid函数是单调递增的非线性函数，无限次可微。当且仅当权值较大时可以逼近阈值函数，当权值较小时逼线性函数。

![](https://img-blog.csdn.net/20130604162915953)


Sigmoid函数通常写作如下形式：

![](https://img-blog.csdn.net/20130604162545234)


取值范围是（-1,1），代替神经元阶跃函数可写作：

![](https://img-blog.csdn.net/20130604162618937)
...
![](https://img-blog.csdn.net/20131128125415937?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvYmVsbGFqbw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)
...
![](https://img-blog.csdn.net/20131128125853000?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvYmVsbGFqbw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)

设两个权重系数矩阵为：

![](https://img-blog.csdn.net/20131128125947093?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvYmVsbGFqbw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)

分析如下：

![](https://img-blog.csdn.net/20131128130115265?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvYmVsbGFqbw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)
...
![](https://img-blog.csdn.net/20131128130230000?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvYmVsbGFqbw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)

在这里，a0(3)可以认为为常数

![](https://img-blog.csdn.net/20131128130348906?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvYmVsbGFqbw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)
...
![](https://img-blog.csdn.net/20131128130448796?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvYmVsbGFqbw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)

 (2) 根据输入数据利用公式算出网络的输出 
![](https://img-blog.csdn.net/20131128130528343?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvYmVsbGFqbw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)

![](https://img-blog.csdn.net/20131128130607625?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvYmVsbGFqbw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)

![](https://img-blog.csdn.net/20131128130649734?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvYmVsbGFqbw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)
...
![](https://img-blog.csdn.net/20131128130737828?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvYmVsbGFqbw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)

即网络模型的解为： 
![](https://img-blog.csdn.net/20131128130807828?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvYmVsbGFqbw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)
...
完全内插法要求插值函数经过每个样本点，即![](http://pic002.cnblogs.com/images/2012/103496/2012071417362382.png)。样本点总共有P个。

RBF的方法是要选择P个基函数，每个基函数对应一个训练数据，各基函数形式为![](http://pic002.cnblogs.com/images/2012/103496/2012071417383089.png)，由于距离是径向同性的，因此称为径向基函数。||X-Xp||表示差向量的模，或者叫2范数。

基于为径向基函数的插值函数为：

![](http://pic002.cnblogs.com/images/2012/103496/2012071418435973.png)

![](http://pic002.cnblogs.com/images/2012/103496/2012082821495379.png)
...
![](http://pic002.cnblogs.com/images/2012/103496/2012071418470870.png)

写成向量的形式为![](http://pic002.cnblogs.com/images/2012/103496/2012071418490996.png)，显然Φ是个规模这P对称矩阵，且与X的维度无关，当Φ可逆时，有![](http://pic002.cnblogs.com/images/2012/103496/2012071418505872.png)。

对于一大类函数，当输入的X各不相同时，Φ就是可逆的。下面的几个函数就属于这“一大类”函数：

1）Gauss（高斯）函数

![](http://pic002.cnblogs.com/images/2012/103496/2012071418554280.png)

2）Reflected Sigmoidal（反常S型）函数

![](http://pic002.cnblogs.com/images/2012/103496/2012071418564117.png)

3）Inverse multiquadrics（拟多二次）函数

![](http://pic002.cnblogs.com/images/2012/103496/2012071418575836.png)
...
![](http://pic002.cnblogs.com/images/2012/103496/2012082821594020.png)
...
![](http://pic002.cnblogs.com/images/2012/103496/2012071419225414.png)
...
![](http://pic002.cnblogs.com/images/2012/103496/2012071419264055.png)

权向量![](http://pic002.cnblogs.com/images/2012/103496/2012071419282252.png)********************************(2)

G(X,Xp)称为Green函数，G称为Green矩阵。Green函数与算子D的形式有关，当D具有旋转不变性和平移不变性时，![](http://pic002.cnblogs.com/images/2012/103496/2012071419305611.png)。这类Green函数的一个重要例子是多元Gauss函数：

![](http://pic002.cnblogs.com/images/2012/103496/2012071419332343.png)。
...
![](http://pic002.cnblogs.com/images/2012/103496/2012071420463011.png)

dmax是所选数据中心之间的最大距离，M是数据中心的个数。扩展常数这么计算是为了避免径向基函数太尖或太平。
- 自组织选择法，比如对样本进行聚类、梯度训练法、资源分配网络等。各聚类中心确定以后，根据各中心之间的距离确定对应径向基函数的扩展常数。

![](http://pic002.cnblogs.com/images/2012/103496/2012071420583740.png)


λ是重叠系数。


接下来求权值W时就不能再用![](http://pic002.cnblogs.com/images/2012/103496/2012071421022791.png)了，因为对于广义RBF网络，其行数大于列数，此时可以求Φ伪逆。

![](http://pic002.cnblogs.com/images/2012/103496/2012071421054823.png)

 数据中心的监督学习算法

最一般的情况，RBF函数中心、扩展常数、输出权值都应该采用监督学习算法进行训练，经历一个误差修正学习的过程，与BP网络的学习原理一样。同样采用梯度下降法，定义目标函数为

![](http://pic002.cnblogs.com/images/2012/103496/2012071422405082.png)

ei为输入第i个样本时的误差信号。

![](http://pic002.cnblogs.com/images/2012/103496/2012071422445312.png)

上式的输出函数中忽略了阈值。

为使目标函数最小化，各参数的修正量应与其负梯度成正比，即

![](http://pic002.cnblogs.com/images/2012/103496/2012071422501571.png)

具体计算式为

![](http://pic002.cnblogs.com/images/2012/103496/2012071422551427.png)

上述目标函数是所有训练样本引起的误差总和，导出的参数修正公式是一种批处理式调整，即所有样本输入一轮后调整一次。目标函数也可以为瞬时值形式，即当前输入引起的误差

![](http://pic002.cnblogs.com/images/2012/103496/2012071423003161.png)

此时参数的修正值为：

![](http://pic002.cnblogs.com/images/2012/103496/2012071423010059.png)

下面我们就分别用本文最后提到的聚类的方法和数据中心的监督学习方法做一道练习题。

考虑Hermit多项式的逼近问题

![](http://pic002.cnblogs.com/images/2012/103496/2012071423151741.png)
...
|```cpp``````cpp#include<iostream>``````cpp#include<algorithm>``````cpp#include<limits>``````cpp#include<cassert>``````cpp#include<cmath>``````cpp#include<ctime>``````cpp#include<cstdlib>``````cpp#include<vector>``````cpp#include<iomanip>``````cpp#include"matrix.h"``````cppusing``````cppnamespace``````cppstd;``````cppconst``````cppint``````cppP=100;``````cpp//输入样本的数量``````cppvector<``````cppdouble``````cpp> X(P);``````cpp//输入样本``````cppMatrix<``````cppdouble``````cpp> Y(P,1);``````cpp//输入样本对应的期望输出``````cppconst``````cppint``````cppM=10;``````cpp//隐藏层节点数目``````cppvector<``````cppdouble``````cpp> center(M);``````cpp//M个Green函数的数据中心``````cppvector<``````cppdouble``````cpp> delta(M);``````cpp//M个Green函数的扩展常数``````cppMatrix<``````cppdouble``````cpp> Green(P,M);``````cpp//Green矩阵``````cppMatrix<``````cppdouble``````cpp> Weight(M,1);``````cpp//权值矩阵``````cpp/*Hermit多项式函数*/``````cppinline``````cppdouble``````cppHermit(``````cppdouble``````cppx){``````cpp``````cppreturn``````cpp1.1*(1-x+2*x*x)*``````cppexp``````cpp(-1*x*x/2);``````cpp}``````cpp/*产生指定区间上均匀分布的随机数*/``````cppinline``````cppdouble``````cppuniform(``````cppdouble``````cppfloor``````cpp,``````cppdouble``````cppceil``````cpp){``````cpp``````cppreturn``````cppfloor``````cpp+1.0*``````cpprand``````cpp()/RAND_MAX*(``````cppceil``````cpp-``````cppfloor``````cpp);``````cpp}``````cpp/*产生区间[floor,ceil]上服从正态分布N[mu,sigma]的随机数*/``````cppinline``````cppdouble``````cppRandomNorm(``````cppdouble``````cppmu,``````cppdouble``````cppsigma,``````cppdouble``````cppfloor``````cpp,``````cppdouble``````cppceil``````cpp){``````cpp``````cppdouble``````cppx,prob,y;``````cpp``````cppdo``````cpp{``````cpp``````cppx=uniform(``````cppfloor``````cpp,``````cppceil``````cpp);``````cpp``````cppprob=1/``````cppsqrt``````cpp(2*M_PI*sigma)*``````cppexp``````cpp(-1*(x-mu)*(x-mu)/(2*sigma*sigma));``````cpp``````cppy=1.0*``````cpprand``````cpp()/RAND_MAX;``````cpp``````cpp}``````cppwhile``````cpp(y>prob);``````cpp``````cppreturn``````cppx;``````cpp}``````cpp/*产生输入样本*/``````cppvoid``````cppgenerateSample(){``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<P;++i){``````cpp``````cppdouble``````cppin=uniform(-4,4);``````cpp``````cppX[i]=in;``````cpp``````cppY.put(i,0,Hermit(in)+RandomNorm(0,0.1,-0.3,0.3));``````cpp``````cpp}``````cpp}``````cpp/*寻找样本离哪个中心最近*/``````cppint``````cppnearest(``````cppconst``````cppvector<``````cppdouble``````cpp>& center,``````cppdouble``````cppsample){``````cpp``````cppint``````cpprect=-1;``````cpp``````cppdouble``````cppdist=numeric_limits<``````cppdouble``````cpp>::max();``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<center.size();++i){``````cpp``````cppif``````cpp(``````cppfabs``````cpp(sample-center[i])<dist){``````cpp``````cppdist=``````cppfabs``````cpp(sample-center[i]);``````cpp``````cpprect=i;``````cpp``````cpp}``````cpp``````cpp}``````cpp``````cppreturn``````cpprect;``````cpp}``````cpp/*计算簇的质心*/``````cppdouble``````cppcalCenter(``````cppconst``````cppvector<``````cppdouble``````cpp> &g){``````cpp``````cppint``````cpplen=g.size();``````cpp``````cppdouble``````cppsum=0.0;``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<len;++i)``````cpp``````cppsum+=g[i];``````cpp``````cppreturn``````cppsum/len;``````cpp}``````cpp/*KMeans聚类法产生数据中心*/``````cppvoid``````cppKMeans(){``````cpp``````cppassert``````cpp(P%M==0);``````cpp``````cppvector<vector<``````cppdouble``````cpp> > group(M);``````cpp//记录各个聚类中包含哪些样本``````cpp``````cppdouble``````cppgap=0.001;``````cpp//聚类中心的改变量小于为个值时，迭代终止``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<M;++i){``````cpp//从P个输入样本中随机选P个作为初始聚类中心``````cpp``````cppcenter[i]=X[10*i+3];``````cpp//输入是均匀分布的，所以我们均匀地选取``````cpp``````cpp}``````cpp``````cppwhile``````cpp(1){``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<M;++i)``````cpp``````cppgroup[i].clear();``````cpp//先清空聚类信息``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<P;++i){``````cpp//把所有输入样本归到对应的簇``````cpp``````cppint``````cppc=nearest(center,X[i]);``````cpp``````cppgroup[c].push_back(X[i]);``````cpp``````cpp}``````cpp``````cppvector<``````cppdouble``````cpp> new_center(M);``````cpp//存储新的簇心``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<M;++i){``````cpp``````cppvector<``````cppdouble``````cpp> g=group[i];``````cpp``````cppnew_center[i]=calCenter(g);``````cpp``````cpp}``````cpp``````cppbool``````cppflag=``````cppfalse``````cpp;``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<M;++i){``````cpp//检查前后两次质心的改变量是否都小于gap``````cpp``````cppif``````cpp(``````cppfabs``````cpp(new_center[i]-center[i])>gap){``````cpp``````cppflag=``````cpptrue``````cpp;``````cpp``````cppbreak``````cpp;``````cpp``````cpp}``````cpp``````cpp}``````cpp``````cppcenter=new_center;``````cpp``````cppif``````cpp(!flag)``````cpp``````cppbreak``````cpp;``````cpp``````cpp}``````cpp}``````cpp/*生成Green矩阵*/``````cppvoid``````cppcalGreen(){``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<P;++i){``````cpp``````cppfor``````cpp(``````cppint``````cppj=0;j<M;++j){``````cpp``````cppGreen.put(i,j,``````cppexp``````cpp(-1.0*(X[i]-center[j])*(X[i]-center[j])/(2*delta[j]*delta[j])));``````cpp``````cpp}``````cpp``````cpp}``````cpp}``````cpp/*求一个矩阵的伪逆*/``````cppMatrix<``````cppdouble``````cpp> getGereralizedInverse(``````cppconst``````cppMatrix<``````cppdouble``````cpp> &matrix){``````cpp``````cppreturn``````cpp(matrix.getTranspose()*matrix).getInverse()*(matrix.getTranspose());``````cpp}``````cpp/*利用已训练好的神经网络，由输入得到输出*/``````cppdouble``````cppgetOutput(``````cppdouble``````cppx){``````cpp``````cppdouble``````cppy=0.0;``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<M;++i)``````cpp``````cppy+=Weight.get(i,0)*``````cppexp``````cpp(-1.0*(x-center[i])*(x-center[i])/(2*delta[i]*delta[i]));``````cpp``````cppreturn``````cppy;``````cpp}``````cppint``````cppmain(``````cppint``````cppargc,``````cppchar``````cpp*argv[]){<br>``````cppsrand``````cpp(``````cpptime``````cpp(0));``````cpp``````cppgenerateSample();``````cpp//产生输入和对应的期望输出样本``````cpp``````cppKMeans();``````cpp//对输入进行聚类，产生聚类中心``````cpp``````cppsort(center.begin(),center.end());``````cpp//对聚类中心（一维数据）进行排序``````cpp``````cpp``````cpp//根据聚类中心间的距离，计算各扩展常数``````cpp``````cppdelta[0]=center[1]-center[0];``````cpp``````cppdelta[M-1]=center[M-1]-center[M-2];``````cpp``````cppfor``````cpp(``````cppint``````cppi=1;i<M-1;++i){``````cpp``````cppdouble``````cppd1=center[i]-center[i-1];``````cpp``````cppdouble``````cppd2=center[i+1]-center[i];``````cpp``````cppdelta[i]=d1<d2?d1:d2;``````cpp``````cpp}``````cpp``````cpp``````cppcalGreen();``````cpp//计算Green矩阵``````cpp``````cppWeight=getGereralizedInverse(Green)*Y;``````cpp//计算权值矩阵``````cpp``````cpp``````cpp//根据已训练好的神经网络作几组测试``````cpp``````cppfor``````cpp(``````cppint``````cppx=-4;x<5;++x){``````cpp``````cppcout<<x<<``````cpp"\t"``````cpp;``````cpp``````cppcout<<setprecision(8)<<setiosflags(ios::left)<<setw(15);``````cpp``````cppcout<<getOutput(x)<<Hermit(x)<<endl;``````cpp//先输出我们预测的值，再输出真实值``````cpp``````cpp}``````cpp``````cppreturn``````cpp0;``````cpp}```|
|----|




![](http://pic002.cnblogs.com/images/2012/103496/2012071515163418.png)

（2）用梯度下降法训练RBF网络，设η=0.001，M=10，初始权值为[-0.1,0.1]内的随机数，初始数据中心为[-4,4]内的随机数，初始扩展常数取[0.1,0.3]内的随机数，目标误差为0.9，最大训练次数为5000。



|```cpp#include<iostream>``````cpp#include<cassert>``````cpp#include<cmath>``````cpp#include<ctime>``````cpp#include<cstdlib>``````cpp#include<vector>``````cpp#include<iomanip>``````cppusing``````cppnamespace``````cppstd;``````cppconst``````cppint``````cppP=100;``````cpp//输入样本的数量``````cppvector<``````cppdouble``````cpp> X(P);``````cpp//输入样本``````cppvector<``````cppdouble``````cpp> Y(P);``````cpp//输入样本对应的期望输出``````cppconst``````cppint``````cppM=10;``````cpp//隐藏层节点数目``````cppvector<``````cppdouble``````cpp> center(M);``````cpp//M个Green函数的数据中心``````cppvector<``````cppdouble``````cpp> delta(M);``````cpp//M个Green函数的扩展常数``````cppdouble``````cppGreen[P][M];``````cpp//Green矩阵``````cppvector<``````cppdouble``````cpp> Weight(M);``````cpp//权值矩阵``````cppconst``````cppdouble``````cppeta=0.001;``````cpp//学习率``````cppconst``````cppdouble``````cppERR=0.9;``````cpp//目标误差``````cppconst``````cppint``````cppITERATION_CEIL=5000;``````cpp//最大训练次数``````cppvector<``````cppdouble``````cpp> error(P);``````cpp//单个样本引起的误差``````cpp/*Hermit多项式函数*/``````cppinline``````cppdouble``````cppHermit(``````cppdouble``````cppx){``````cpp``````cppreturn``````cpp1.1*(1-x+2*x*x)*``````cppexp``````cpp(-1*x*x/2);``````cpp}``````cpp/*产生指定区间上均匀分布的随机数*/``````cppinline``````cppdouble``````cppuniform(``````cppdouble``````cppfloor``````cpp,``````cppdouble``````cppceil``````cpp){``````cpp``````cppreturn``````cppfloor``````cpp+1.0*``````cpprand``````cpp()/RAND_MAX*(``````cppceil``````cpp-``````cppfloor``````cpp);``````cpp}``````cpp/*产生区间[floor,ceil]上服从正态分布N[mu,sigma]的随机数*/``````cppinline``````cppdouble``````cppRandomNorm(``````cppdouble``````cppmu,``````cppdouble``````cppsigma,``````cppdouble``````cppfloor``````cpp,``````cppdouble``````cppceil``````cpp){``````cpp``````cppdouble``````cppx,prob,y;``````cpp``````cppdo``````cpp{``````cpp``````cppx=uniform(``````cppfloor``````cpp,``````cppceil``````cpp);``````cpp``````cppprob=1/``````cppsqrt``````cpp(2*M_PI*sigma)*``````cppexp``````cpp(-1*(x-mu)*(x-mu)/(2*sigma*sigma));``````cpp``````cppy=1.0*``````cpprand``````cpp()/RAND_MAX;``````cpp``````cpp}``````cppwhile``````cpp(y>prob);``````cpp``````cppreturn``````cppx;``````cpp}``````cpp/*产生输入样本*/``````cppvoid``````cppgenerateSample(){``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<P;++i){``````cpp``````cppdouble``````cppin=uniform(-4,4);``````cpp``````cppX[i]=in;``````cpp``````cppY[i]=Hermit(in)+RandomNorm(0,0.1,-0.3,0.3);``````cpp``````cpp}``````cpp}``````cpp/*给向量赋予[floor,ceil]上的随机值*/``````cppvoid``````cppinitVector(vector<``````cppdouble``````cpp> &vec,``````cppdouble``````cppfloor``````cpp,``````cppdouble``````cppceil``````cpp){``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<vec.size();++i)``````cpp``````cppvec[i]=uniform(``````cppfloor``````cpp,``````cppceil``````cpp);``````cpp}``````cpp/*根据网络，由输入得到输出*/``````cppdouble``````cppgetOutput(``````cppdouble``````cppx){``````cpp``````cppdouble``````cppy=0.0;``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<M;++i)``````cpp``````cppy+=Weight[i]*``````cppexp``````cpp(-1.0*(x-center[i])*(x-center[i])/(2*delta[i]*delta[i]));``````cpp``````cppreturn``````cppy;``````cpp}``````cpp/*计算单个样本引起的误差*/``````cppdouble``````cppcalSingleError(``````cppint``````cppindex){``````cpp``````cppdouble``````cppoutput=getOutput(X[index]);``````cpp``````cppreturn``````cppY[index]-output;``````cpp}``````cpp/*计算所有训练样本引起的总误差*/``````cppdouble``````cppcalTotalError(){``````cpp``````cppdouble``````cpprect=0.0;``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<P;++i){``````cpp``````cpperror[i]=calSingleError(i);``````cpp``````cpprect+=error[i]*error[i];``````cpp``````cpp}``````cpp``````cppreturn``````cpprect/2;``````cpp}``````cpp/*更新网络参数*/``````cppvoid``````cppupdateParam(){``````cpp``````cppfor``````cpp(``````cppint``````cppj=0;j<M;++j){``````cpp``````cppdouble``````cppdelta_center=0.0,delta_delta=0.0,delta_weight=0.0;``````cpp``````cppdouble``````cppsum1=0.0,sum2=0.0,sum3=0.0;``````cpp``````cppfor``````cpp(``````cppint``````cppi=0;i<P;++i){``````cpp``````cppsum1+=error[i]*``````cppexp``````cpp(-1.0*(X[i]-center[j])*(X[i]-center[j])/(2*delta[j]*delta[j]))*(X[i]-center[j]);``````cpp``````cppsum2+=error[i]*``````cppexp``````cpp(-1.0*(X[i]-center[j])*(X[i]-center[j])/(2*delta[j]*delta[j]))*(X[i]-center[j])*(X[i]-center[j]);``````cpp``````cppsum3+=error[i]*``````cppexp``````cpp(-1.0*(X[i]-center[j])*(X[i]-center[j])/(2*delta[j]*delta[j]));``````cpp``````cpp}``````cpp``````cppdelta_center=eta*Weight[j]/(delta[j]*delta[j])*sum1;``````cpp``````cppdelta_delta=eta*Weight[j]/``````cpppow``````cpp(delta[j],3)*sum2;``````cpp``````cppdelta_weight=eta*sum3;``````cpp``````cppcenter[j]+=delta_center;``````cpp``````cppdelta[j]+=delta_delta;``````cpp``````cppWeight[j]+=delta_weight;``````cpp``````cpp}``````cpp}``````cppint``````cppmain(``````cppint``````cppargc,``````cppchar``````cpp*argv[]){``````cpp``````cppsrand``````cpp(``````cpptime``````cpp(0));``````cpp``````cpp/*初始化网络参数*/``````cpp``````cppinitVector(Weight,-0.1,0.1);``````cpp``````cppinitVector(center,-4.0,4.0);``````cpp``````cppinitVector(delta,0.1,0.3);``````cpp``````cpp/*产生输入样本*/``````cpp``````cppgenerateSample();``````cpp``````cpp/*开始迭代*/``````cpp``````cppint``````cppiteration=ITERATION_CEIL;``````cpp``````cppwhile``````cpp(iteration-->0){``````cpp``````cppif``````cpp(calTotalError()<ERR)``````cpp//误差已达到要求，可以退出迭代``````cpp``````cppbreak``````cpp;``````cpp``````cppupdateParam();``````cpp//更新网络参数``````cpp``````cpp}``````cpp``````cppcout<<``````cpp"迭代次数:"``````cpp<<ITERATION_CEIL-iteration-1<<endl;``````cpp``````cpp``````cpp//根据已训练好的神经网络作几组测试``````cpp``````cppfor``````cpp(``````cppint``````cppx=-4;x<5;++x){``````cpp``````cppcout<<x<<``````cpp"\t"``````cpp;``````cpp``````cppcout<<setprecision(8)<<setiosflags(ios::left)<<setw(15);``````cpp``````cppcout<<getOutput(x)<<Hermit(x)<<endl;``````cpp//先输出我们预测的值，再输出真实值``````cpp``````cpp}``````cpp``````cppreturn``````cpp0;``````cpp}```|
|----|




![](http://pic002.cnblogs.com/images/2012/103496/2012071517483766.png)
...
2、由于BP算 法的步骤是一定的，我们只要把其思想转化为程序就行了，即把数学表达式转换为程序。我们知道在计算机中每一种算法都需要一定的数据结构去支持。由于算法已 确定，那么我们只要分析和确定其数据结构即可。首先，我们考虑在如何计算机程序设计中表示权系数和阀值，在这里我们定义了3维数组W[Layer_Max][Node_Max][Node_Max+1]用来表示神经网络的全部权系数和阀值，我们约定W[i][j][k]存储网络的权系数，其中i表示为神经网络的第i层，j表示为第i层网络的第j个神经元，k表示为第i -1层的第j个神经网络。那么，W[i][j][k]表示为第i层的第j个神经元和第i
 －1层的第k个神经元的权系数。W[i][j][Layer[i-1]＋1]表示第i层第j个神经元的阀值。
