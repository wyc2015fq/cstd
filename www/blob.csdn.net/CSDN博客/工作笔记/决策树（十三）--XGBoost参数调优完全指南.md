# 决策树（十三）--XGBoost参数调优完全指南 - 工作笔记 - CSDN博客





2019年02月15日 01:02:16[Eason.wxd](https://me.csdn.net/App_12062011)阅读数：5431
...
转自：[https://blog.csdn.net/han_xiaoyang/article/details/52665396](https://blog.csdn.net/han_xiaoyang/article/details/52665396)
...
XGBoost(eXtreme Gradient Boosting)是Gradient Boosting算法的一个优化的版本。Gradient Boosting算法的很多细节建议在读本篇文章之前，好好理解清楚。帮助你对Boosting算法有一个宏观的理解，同时也会对GBM的参数调整有更好的体会。
...
[Words from the Auther of XGBoost](https://www.youtube.com/watch?v=X47SGnTMZIU)[Viedo]
## **5. XGBoost的参数**
...
在这里我会类比GBM来讲解，所以作为一种基础知识，强烈推荐先阅读[这篇文章](http://blog.csdn.net/han_xiaoyang/article/details/52663170)。
...
#### **4、max_leaf_nodes**
- 树上最大的节点或叶子的数量。
- 可以替代max_depth的作用。因为如果生成的是二叉树，一个深度为n的树最多生成$n2      n^2$个叶子。
...
我们从Data Hackathon 3.x AV版的hackathon中获得数据集，和[GBM 介绍文章](http://blog.csdn.net/han_xiaoyang/article/details/52663170)中是一样的。更多的细节可以参考[competition page](http://datahack.analyticsvidhya.com/contest/data-hackathon-3x)

数据集可以从[这里](http://www.analyticsvidhya.com/wp-content/uploads/2016/02/Dataset.rar)下载。我已经对这些数据进行了一些处理：
- `City`变量，因为类别太多，所以删掉了一些类别。
- `DOB`变量换算成年龄，并删除了一些数据。
- 增加了 `EMI_Loan_Submitted_Missing` 变量。如果`EMI_Loan_Submitted`变量的数据缺失，则这个参数的值为1。否则为0。删除了原先的`EMI_Loan_Submitted`变量。
- `EmployerName`变量，因为类别太多，所以删掉了一些类别。
- 因为`Existing_EMI`变量只有111个值缺失，所以缺失值补充为中位数0。
- 增加了 `Interest_Rate_Missing` 变量。如果`Interest_Rate`变量的数据缺失，则这个参数的值为1。否则为0。删除了原先的`Interest_Rate`变量。
- 删除了`Lead_Creation_Date`，从直觉上这个特征就对最终结果没什么帮助。
- `Loan_Amount_Applied, Loan_Tenure_Applied` 两个变量的缺项用中位数补足。
- 增加了 `Loan_Amount_Submitted_Missing` 变量。如果`Loan_Amount_Submitted`变量的数据缺失，则这个参数的值为1。否则为0。删除了原先的`Loan_Amount_Submitted`变量。
...
![](http://ml.xiniuedu.com/xgb_tuning/1.-inital.png)
...
![输出结果](http://ml.xiniuedu.com/xgb_tuning/2.png)
...
![输出结果](http://ml.xiniuedu.com/xgb_tuning/3.png)
...
![输出结果](http://ml.xiniuedu.com/xgb_tuning/4.png)
...
![gamma](http://ml.xiniuedu.com/xgb_tuning/5.png)
...
![最终参数](http://ml.xiniuedu.com/xgb_tuning/6.png)
...
![gsearch4](http://ml.xiniuedu.com/xgb_tuning/7.png)
...
![output](http://ml.xiniuedu.com/xgb_tuning/8.png)
...
![output](http://ml.xiniuedu.com/xgb_tuning/9.png)
...
![output7](http://ml.xiniuedu.com/xgb_tuning/10.png)
...
![out_put3](http://ml.xiniuedu.com/xgb_tuning/11.png)
...
![此处输入图片的描述](http://ml.xiniuedu.com/xgb_tuning/12.png)
![此处输入图片的描述](xgb4%20=%20XGBClassifier%28%20%20learning_rate%20=0.01,%20%20n_estimators=5000,%20%20max_depth=4,%20%20min_child_weight=6,%20%20gamma=0,%20%20subsample=0.8,%20%20colsample_bytree=0.8,%20%20reg_alpha=0.005,%20%20objective=%20%27binary:logistic%27,%20%20nthread=4,%20%20scale_pos_weight=1,%20%20seed=27%29%20modelfit%28xgb4,%20train,%20predictors%29)
...
你可以从 [这里](https://github.com/aarshayj/Analytics_Vidhya/tree/master/Articles/Parameter_Tuning_XGBoost_with_Example) 下载iPython notebook文件，里面包含了文章中提到的所有代码。如果你使用R语言，请阅读[这篇文章](http://www.analyticsvidhya.com/blog/2016/01/xgboost-algorithm-easy-steps/)。
