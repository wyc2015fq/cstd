# 目标检测（五）--R-CNN - 工作笔记 - CSDN博客





2017年03月28日 10:25:01[Eason.wxd](https://me.csdn.net/App_12062011)阅读数：6520
...
   本篇博文主要讲解2014年CVPR上的经典paper：《Rich feature hierarchies for Accurate Object Detection and Segmentation》，这篇文章的[算法](http://lib.csdn.net/base/datastructure)思想又被称之为：R-CNN（Regions with Convolutional Neural Network Features），是物体检测领域曾经获得state-of-art精度的经典文献。

   这篇paper的思想，改变了物体检测的总思路，现在好多文献关于[深度学习](http://lib.csdn.net/base/deeplearning)的物体检测的算法，基本上都是继承了这个思想，比如：《Spatial Pyramid Pooling in Deep Convolutional Networks for Visual Recognition》，所以学习经典算法，有助于我们以后搞物体检测的其它paper。
...
图片分类标注好的训练数据非常多，但是物体检测的标注数据却很少，如何用少量的标注数据，训练高质量的模型，这就是文献最大的特点，这篇paper采用了迁移学习的思想。文献就先用了ILSVRC2012这个训练[数据库](http://lib.csdn.net/base/mysql)（这是一个图片分类训练数据库），先进行网络的图片分类训练。这个数据库有大量的标注数据，共包含了1000种类别物体，因此预训练阶段cnn模型的输出是1000个神经元，或者我们也直接可以采用Alexnet训练好的模型参数。
...
这个就相当于一个竞赛，里面包含了20个物体类别：[http://host.robots.ox.ac.uk/pascal/VOC/voc2012/examples/index.html](http://host.robots.ox.ac.uk/pascal/VOC/voc2012/examples/index.html)  还有一个背景，总共就相当于21个类别，因此一会设计fine-tuning CNN的时候，我们softmax分类输出层为21个神经元。
...
因为图片扭曲后，估计会对后续CNN的训练精度有影响，于是作者也[测试](http://lib.csdn.net/base/softwaretest)了“各向同性缩放”方案。这个有两种办法
