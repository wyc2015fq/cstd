# 目标检测（六）--SPPNet - 工作笔记 - CSDN博客





2017年03月28日 10:51:19[Eason.wxd](https://me.csdn.net/App_12062011)阅读数：6669
...
multi-size训练，输入尺寸在[180,224]之间，假设最后一个卷积层的输出大小为*a*×*a*，若给定金字塔层有*n*×*n* 个bins，进行滑动窗池化，窗口尺寸为*w**i**n*=⌈*a*/*n*⌉，步长为*s**t**r*=⌊*a*/*n*⌋，使用一个网络完成一个完整epoch的训练，之后切换到另外一个网络。只是在训练的时候用到多尺寸，[测试](http://lib.csdn.net/base/softwaretest)时直接将SPPNet应用于任意尺寸的图像。
...
![这里写图片描述](https://img-blog.csdn.net/20150711121114131)

2).使用全图作为SPPNet的输入及224*224图像中心的crop对比，网络使用ZF-5,Overfeat-7,结果如下表
![这里写图片描述](https://img-blog.csdn.net/20150711122929490)

3).与其他方法在ImageNet分类的对比
![这里写图片描述](https://img-blog.csdn.net/20150711123037312)

SPPNet for Object Detection

R-CNN重复使用深层卷积网络在~2k个窗口上提取特征，特征提取非常耗时。SPPNet比较耗时的卷积计算对整幅图像只进行一次，之后使用spp将窗口特征图池化为一个固定长度的特征表示。

检测[算法](http://lib.csdn.net/base/datastructure)：使用ss生成~2k个候选框，缩放图像min(w,h)=s之后提取特征，每个候选框使用一个4层的空间金字塔池化特征，网络使用的是ZF-5的SPPNet形式。之后将12800d的特征输入全连接层，SVM的输入为全连接层的输出。SVM如下使用，正样本为groundtruth，负样本与正样本的IoU<0.3，训练20类样本只需1h。使用ZF-5与R-CNN对比实验结果如下：
![这里写图片描述](https://img-blog.csdn.net/20150712174410871)
