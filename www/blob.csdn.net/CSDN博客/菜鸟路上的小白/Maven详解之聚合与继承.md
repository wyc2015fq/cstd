# Maven详解之聚合与继承 - 菜鸟路上的小白 - CSDN博客





2016年11月22日 16:10:11[somnus_小凯](https://me.csdn.net/u012486840)阅读数：300
...
随着技术的飞速发展和各类用户对软件的要求越来越高，软件本身也变得越来越复杂，然后软件设计人员开始采用各种方式进行开发，于是就有了我们的分层[架构](http://lib.csdn.net/base/architecture)、分模块开发，来提高代码的清晰和重用。针对于这一特性，maven也给予了相应的配置。

情景分析一：

我们在开发过程中，创建了2个以上的模块，每个模块都是一个独立的maven project，在开始的时候我们可以独立的编译和[测试](http://lib.csdn.net/base/softwaretest)运行每个模块，但是随着项目的不断变大和复杂化，我们期望能够使用简单的操作来完成编译等工作，这时Maven给出了聚合的配置方式。
...
8.聚合模块减少的内容：聚合模块的内容仅仅是一个pom.xml文件，它不包含src/main/[Java](http://lib.csdn.net/base/javaee)、src/test/java等目录，因为它只是用来帮助其它模块构建的工具，本身并没有实质的内容。
...
情景分析二：我们在项目开发的过程中，可能多个模块独立开发，但是多个模块可能依赖相同的元素，比如说每个模块都需要Junit，使用[spring](http://lib.csdn.net/base/javaee)的时候，其核心jar也必须都被引入，在编译的时候，maven-compiler-plugin插件也要被引入
...
scm: 项目的[版本控制](http://lib.csdn.net/base/git)系统信息
...
**dependencyManagement**的特性：在**dependencyManagement中**配置的元素既不会给parent引入依赖，也不会给它的子模块引入依赖，仅仅是它的配置是可继承的
...
**[html]**[view
 plain](http://blog.csdn.net/wanghantong/article/details/36427411#)[copy](http://blog.csdn.net/wanghantong/article/details/36427411#)



- <spanstyle="white-space:pre"></span><properties>
...
- <spanstyle="white-space:pre"></span><dependencies>
- <dependency>
- <groupId>your groupId</groupId>
- <artifactId>your artifactId</artifactId>
- </dependency>
...
- <spanstyle="white-space:pre"></span><build>
- <pluginManagement>
- <plugins>
- <plugin>
- <groupId></groupId>
...
- <prename="code"class="html"><spanstyle="white-space:pre"></span></build>
