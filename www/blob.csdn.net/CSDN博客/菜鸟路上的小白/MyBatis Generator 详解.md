# MyBatis Generator 详解 - 菜鸟路上的小白 - CSDN博客





2017年06月05日 16:36:16[somnus_小凯](https://me.csdn.net/u012486840)阅读数：352
...
> 
[http://mbg.cndocs.tk/](http://mbg.cndocs.tk/)
...
## [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#-a-href-http-generator-sturgeon-mopaas-com-running-running-html-运行mybatis-generator-a-)[运行MyBatis Generator](http://mbg.cndocs.tk/running/running.html)


有4种运行MBG的方法，具体请看文档 [运行
 MyBatis Generator](http://mbg.cndocs.tk/running/running.html)
...
## [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#-a-href-http-generator-sturgeon-mopaas-com-configreference-xmlconfig-html-xml配置详解-a-)[XML配置详解](http://mbg.cndocs.tk/configreference/xmlconfig.html)
...
### [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#1-配置文件头)1. 配置文件头
...
### [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#3-code-lt-generatorconfiguration-gt-code-子元素)3. `<generatorConfiguration>`子元素
...
包含以下子元素（有严格的顺序）：
- 
[`<properties>`](http://mbg.cndocs.tk/configreference/properties.html) (0个或1个)

- 
[`<classPathEntry>`](http://mbg.cndocs.tk/configreference/classPathEntry.html) (0个或多个)

- 
[`<context>`](http://mbg.cndocs.tk/configreference/context.html) (1个或多个)


#### [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#3-1-a-href-http-generator-sturgeon-mopaas-com-configreference-properties-html-code-lt-properties-gt-code-a-元素)3.1 [`<properties>`](http://mbg.cndocs.tk/configreference/properties.html) 元素
...
#### [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#3-2-a-href-http-generator-sturgeon-mopaas-com-configreference-classpathentry-html-code-lt-classpathentry-gt-code-a-元素)3.2 [`<classPathEntry>`](http://mbg.cndocs.tk/configreference/classPathEntry.html) 元素
...
#### [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#3-3-a-href-http-generator-sturgeon-mopaas-com-configreference-context-html-code-lt-context-gt-code-a-元素)3.3 [`<context>`](http://mbg.cndocs.tk/configreference/context.html) 元素


在MBG的配置中，至少需要有一个`<context>`元素。

`<context>`元素用于指定生成一组对象的环境。例如指定要连接的[数据库](http://lib.csdn.net/base/mysql)，要生成对象的类型和要处理的数据库中的表。运行MBG的时候还可以指定要运行的`<context>`。
...
这个属性有以下可选值：
- `conditional`:*这是默认值*,这个模型和下面的`hierarchical`类似，除了如果那个单独的类将只包含一个字段，将不会生成一个单独的类。
...
- 
`introspectedColumnImpl`:该参数可以指定扩展`org.mybatis.generator.api.IntrospectedColumn`该类的实现类。该属性的作用可以查看[扩展MyBatis
 Generator](http://mbg.cndocs.tk/reference/extending.html)。
...
- [`<property>`](http://mbg.cndocs.tk/configreference/property.html) (0个或多个)
- [`<plugin>`](http://mbg.cndocs.tk/configreference/plugin.html) (0个或多个)
- [`<commentGenerator>`](http://mbg.cndocs.tk/configreference/commentGenerator.html) (0个或1个)
- [`<jdbcConnection>`](http://mbg.cndocs.tk/configreference/jdbcConnection.html) (1个)
- [`<javaTypeResolver>`](http://mbg.cndocs.tk/configreference/javaTypeResolver.html) (0个或1个)
- [`<javaModelGenerator>`](http://mbg.cndocs.tk/configreference/javaModelGenerator.html) (1个)
- [`<sqlMapGenerator>`](http://mbg.cndocs.tk/configreference/sqlMapGenerator.html) (0个或1个)
- [`<javaClientGenerator>`](http://mbg.cndocs.tk/configreference/javaClientGenerator.html) (0个或1个)
- [`<table>`](http://mbg.cndocs.tk/configreference/table.html) (1个或多个)

其中`<property>`属性比较特殊，后面讲解的时候都会和父元素一起进行讲解。在讲解`<property>`属性前，我们先看看**什么是分隔符？**。


这里通过一个例子说明。假设在[MySQL](http://lib.csdn.net/base/mysql)数据库中有一个表名为
```
user
 info
```
，你没有看错，中间是一个空格，这种情况下如果写出
```
select
 * from user info
```
...
属性`javaFileEncoding`设置要使用的[Java](http://lib.csdn.net/base/java)文件的编码，默认使用当前平台的编码，只有当生产的编码需要特殊指定时才需要使用，一般用不到。
...
##### [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#3-3-1-a-href-http-generator-sturgeon-mopaas-com-configreference-plugin-html-code-lt-plugin-gt-code-a-元素)3.3.1 [`<plugin>`](http://mbg.cndocs.tk/configreference/plugin.html) 元素
...
##### [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#3-3-2-a-href-http-generator-sturgeon-mopaas-com-configreference-commentgenerator-html-code-lt-commentgenerator-gt-code-a-元素)3.3.2 [`<commentGenerator>`](http://mbg.cndocs.tk/configreference/commentGenerator.html) 元素
...
一般情况下由于MBG生成的注释信息没有任何价值，而且有时间戳的情况下每次生成的注释都不一样，使用**[版本控制](http://lib.csdn.net/base/git)**的时候每次都会提交，因而一般情况下我们都会屏蔽注释信息，可以如下配置：
...
##### [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#3-3-3-a-href-http-generator-sturgeon-mopaas-com-configreference-jdbcconnection-html-code-lt-jdbcconnection-gt-code-a-元素)3.3.3 [`<jdbcConnection>`](http://mbg.cndocs.tk/configreference/jdbcConnection.html) 元素
...
##### [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#3-3-4-a-href-http-generator-sturgeon-mopaas-com-configreference-javatyperesolver-html-code-lt-javatyperesolver-gt-code-a-元素)3.3.4 [`<javaTypeResolver>`](http://mbg.cndocs.tk/configreference/javaTypeResolver.html) 元素
...
##### [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#3-3-5-a-href-http-generator-sturgeon-mopaas-com-configreference-javamodelgenerator-html-code-lt-javamodelgenerator-gt-code-a-元素)3.3.5 [`<javaModelGenerator>`](http://mbg.cndocs.tk/configreference/javaModelGenerator.html) 元素
...
- `targetProject`:指定目标项目路径，可以是绝对路径或相对路径（如
...
```
public
 void setUsername(String username) {this.username = username == null ? null : username.trim();}
```
的`setter`方法。默认值为`false`。
...
##### [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#3-3-6-a-href-http-generator-sturgeon-mopaas-com-configreference-sqlmapgenerator-html-code-lt-sqlmapgenerator-gt-code-a-元素)3.3.6 [`<sqlMapGenerator>`](http://mbg.cndocs.tk/configreference/sqlMapGenerator.html) 元素


该元素可选，最多配置一个。但是有如下两种必选的特殊情况：
- 如果`targetRuntime`目标是**iBATIS2**，该元素必须配置一个。

- 如果`targetRuntime`目标是**MyBatis3**，只有当`<javaClientGenerator>`需要XML时，该元素必须配置一个。
...
`rootInterface`用于指定一个所有生成的接口都继承的父接口。
 这个值可以通过`<table>`配置的`rootInterface`属性覆盖。
...
##### [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#3-3-8-a-href-http-generator-sturgeon-mopaas-com-configreference-table-html-code-lt-table-gt-code-a-元素)3.3.8 [`<table>`](http://mbg.cndocs.tk/configreference/table.html) 元素
...
- `escapeWildcards`:这个属性表示当查询列，是否对schema和表名中的SQL通配符
 ('_' and '%') 进行转义。 对于某些驱动当schema或表名中包含SQL通配符时（例如，一个表名是MY_TABLE，有一些驱动需要将下划线进行转义）是必须的。默认值是`false`。
...
- `selectAllOrderByClause`:该属性值会追加到`selectAll`方法后的SQL中，会直接跟
```
order
 by
```
...
- [`<generatedKey>`](http://mbg.cndocs.tk/configreference/generatedKey.html) (0个或1个)
- [`<columnRenamingRule>`](http://mbg.cndocs.tk/configreference/columnRenamingRule.html) (0个或1个)
- [`<columnOverride>`](http://mbg.cndocs.tk/configreference/columnOverride.html) (0个或多个)
- [`<ignoreColumn>`](http://mbg.cndocs.tk/configreference/ignoreColumn.html) (0个或多个)


下面对这4个元素进行详细讲解。

1. [`<generatedKey>`](http://mbg.cndocs.tk/configreference/generatedKey.html) 元素
...
这个元素还包含两个可选属性：
- `identity`:当设置为`true`时,该列会被标记为`identity`列，
 并且`<selectKey>`元素会被插入在`insert`后面。
 当设置为`false`时，`<selectKey>`会插入到`insert`之前（通常是序列）。**重要**:
 即使您`type`属性指定为`post`，您仍然需要为`identity`列将该参数设置为`true`。
 这将标志MBG从插入列表中删除该列。默认值是`false`。
- `type`:
```
type=post
 and identity=true
```
的时候生成的`<selectKey>`中的`order=AFTER`,当`type=pre`的时候，`identity只能为false`，生成的`<selectKey>`中的`order=BEFORE`。可以这么理解，自动增长的列只有插入到数据库后才能得到ID，所以是`AFTER`,使用序列时，只有先获取序列之后，才能插入数据库，所以是`BEFORE`。
...
2. [`<columnRenamingRule>`](http://mbg.cndocs.tk/configreference/columnRenamingRule.html) 元素
...
注意，在内部，MBG使用`java.util.regex.Matcher.replaceAll`方法实现这个功能。
...
3. [`<columnOverride>`](http://mbg.cndocs.tk/configreference/columnOverride.html) 元素
...
- `property`:要使用的Java属性的名称。如果没有指定，MBG会根据列名生成。
 例如，如果一个表的一列名为`STRT_DTE`，MBG会根据`<table>`的`useActualColumnNames`属性生成`STRT_DTE`或`strtDte`。
- `javaType`:该列属性值为完全限定的Java类型。如果需要，这可以覆盖由`JavaTypeResolver`计算出的类型。
 对某些数据库来说，这是必要的用来处理**“奇怪的”**数据库类型（例如MySql的
```
unsigned
 bigint
```
类型需要映射为`java.lang.Object`)。
- `jdbcType`:该列的JDBC类型(
```
INTEGER,
 DECIMAL, NUMERIC, VARCHAR
```
等等)。 如果需要，这可以覆盖由`JavaTypeResolver`计算出的类型。
 对某些数据库来说，这是必要的用来处理怪异的JDBC驱动 (例如DB2的`LONGVARCHAR`类型需要为iBATIS
 映射为`VARCHAR`)。
- `typeHandler`:用户定义的需要用来处理这列的类型处理器。它必须是一个继承iBATIS的`TypeHandler`类或`TypeHandlerCallback`接口（该接口很容易继承）的全限定的类名。如果没有指定或者是空白，iBATIS会用默认的类型处理器来处理类型。**重要**:MBG不会校验这个类型处理器是否存在或者可用。
...
4. [`<ignoreColumn>`](http://mbg.cndocs.tk/configreference/ignoreColumn.html) 元素
...
## [](http://git.oschina.net/free/Mybatis_Utils/blob/master/MybatisGeneator/MybatisGeneator.md#mybatis-generator最佳实践)MyBatis Generator最佳实践
...
当数据库字段使用`CHAR`时，建议在`<javaModelGenerator>`中设置
```
<property
...
<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE generatorConfiguration
        PUBLIC "-//mybatis.org//DTD MyBatis Generator Configuration 1.0//EN"
        "http://mybatis.org/dtd/mybatis-generator-config_1_0.dtd"><generatorConfiguration><contextid="MysqlContext"targetRuntime="MyBatis3"defaultModelType="flat"><propertyname="beginningDelimiter"value="`"/><propertyname="endingDelimiter"value="`"/><commentGenerator><propertyname="suppressDate"value="true"/></commentGenerator><jdbcConnectiondriverClass="com.mysql.jdbc.Driver"connectionURL="jdbc:mysql://localhost:3306/test"userId="root"password=""></jdbcConnection><javaModelGeneratortargetPackage="test.model"targetProject="src\main\java"><propertyname="trimStrings"value="true"/></javaModelGenerator><sqlMapGeneratortargetPackage="test.xml"targetProject="src\main\resources"/><javaClientGeneratortype="XMLMAPPER"targetPackage="test.dao"targetProject="src\main\java"/><tabletableName="%"><generatedKeycolumn="id"sqlStatement="Mysql"/></table></context></generatorConfiguration>
