...
# 小样本学习（Few-shot Learning）综述 - Paper weekly - CSDN博客


2019年04月01日 08:39:00[Paper_weekly](https://me.csdn.net/c9Yv2cf9I06K2A9E)阅读数：6
...
**面对这类问题，有一个专门的机器学习分支——Few-shot Learning 来进行研究和解决。**过去一年，我们对Few-shot Learning进行了系统的梳理和研究，将Few-shot Learning和 Capsule Network 融合，提出了 Induction Network，在文本分类上做到了新的 state-of-the-art。

创新总是基于对已有成果的梳理和思考，这篇综述算是一个小结，写出来和大家一起分享，一起讨论。

本文先介绍Few-shot Learning定义；由于最近几年Few-shot Learning在图像领域的进展领先于在自然语言处理领域，所以第二部分结合其在图像处理领域的研究进展，详细介绍Few-shot Learning的三类典型方法及每种方法的代表性模型；接下来介绍在自然语言处理领域的研究进展以及我们对 metric-based 的方法进行系统总结后提出的 few-shot learning framework。
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpcLSljZ7z5ZX5b5IAXpdu7yic3h1Vyd9u1vsiaBZtFKibYNVHs5nfIzyt1g/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpc7JAEk9wmPiaK9RvMoyDjLt8To6K076Sppk8SWChVGicFA00YGp8Sds5g/640?wx_fmt=png)
...
具体地，在 t 时刻，模型输入![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpcSZMAp1g9AQfyRd1KHKicgicmoA4WZBb0UoQ6SwxKrgE0nuX4WjWQYRQw/640?wx_fmt=png)，也就是在当前时刻预测输入样本的类别，并在下一时刻给出真实的 label，并且添加了 external memory 存储上一次的 x 输入，这使得下一次输入后进行反向传播时，可以让 y (label) 和 x 建立联系，使得之后的 x 能够通过外部记忆获取相关图像进行比对来实现更好的预测。

![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpcRG2dZWuibSvrB0NPeq1Cvc4RqhRbOibZEJQeffwicOg4LrmNIabBpJKMg/640?wx_fmt=png)
...
如果在Few-shot Learning的任务中去训练普通的基于 cross-entropy 的神经网络分类器，那么几乎肯定是会过拟合，因为神经网络分类器中有数以万计的参数需要优化。
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpcK0TudWHiadLjENK7XxLDIibXIbUsVqticQ4dVEnyicicsQlLX8q0FATzbCQ/640?wx_fmt=png)
**▲**图4：Siamese Network
...
具体地，它显式的定义一个基于支撑集![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpcOchrEd4hMibkYSM5ibG4xeHQWKQkcrUgujVIFgbIictfXziaVRggl0PV2A/640?wx_fmt=png)的分类器，对于一个新的数据![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpc7dxTMxHW6HiaBbvQsrvjM4C0Aj9RLy9ehDeJ1qq7MAxfnYlkHiaSplEA/640?wx_fmt=png)，其分类概率由![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpc7dxTMxHW6HiaBbvQsrvjM4C0Aj9RLy9ehDeJ1qq7MAxfnYlkHiaSplEA/640?wx_fmt=png)与支撑集 S 之间的距离度量得出：

![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpc5RCiac2UZiagf0WaW4DeEoVDAxrds1fuvaRYPLQdaRYdbjux15UF0w1w/640?wx_fmt=png)

其中 a 是基于距离度量的 attention score：

![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpcbgG3iaUdRmQiceAUaG1ThniczyePeB7X5EoqqwZicOvdKXnowmFaESduyg/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpc9icyYy9Pcy2icSR1qgV8fXANEXohgWHiayy3McyfZW6Olz9iafJy8zC9bQ/640?wx_fmt=png)
**▲**图5：Match Network
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpc7H4Fyib8Nibrl3GxND15g0k5TkQRm16OkpEQxIYdc58iabYzV9kk0pj8g/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpc5pRicbVyYuJKNqWMU2X0REEKU3PIhrXgcrGZvNXqWic4K6zseItictRLg/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpc57qjkXPc4Od10F2BT8NVvu9D412V2XlicLdvNaxKKlbevoT7iay4gtYQ/640?wx_fmt=png)
...
近年来，在自然语言处理领域也开始出现Few-shot Learning的数据集和模型，相比于图像，文本的语义中包含更多的变化和噪声，我们将在本节从数据集和模型两个方面介绍Few-shot Learning在自然语言处理领域的进展，以及我们团队基于对话工厂平台所做的探索。
...
3.**ODIC 数据集**来自阿里巴巴对话工厂平台的线上日志，用户会向平台提交多种不同的对话任务，和多种不同的意图，但是每种意图只有极少数的标注数据，这形成了一个典型的Few-shot Learning任务，该数据集包含 216 个意图，其中 159 个用于训练，57 个用于测试。
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpcshYfMOXv7OTiaAZwyZlUL39vo2zUicGrTv3NkLeg4sgb6mg7kBsao9eg/640?wx_fmt=png)
**▲**图9：基于混合注意力的原型网络

**Yu**[10]等人指出在图像领域的Few-shot Learning任务中，比如 Omniglot 和 miniImage 数据集，所有的数据都是从同一个大的数据集采样而来，也就是说所有的 meta-task 都是来自同一个领域，所以相关性是很强的。

所以之前的Few-shot Learning方法只需使用一个 meta model 即可解决剩余的 few-shot 任务。但是在现实场景当中，不同的 meta task 可能来自完全不同的领域，因此使用单独的度量方式不足以衡量所有的 meta task。

在这种场景下，Yu 提出使用多种度量方式融合来解跨领域的Few-shot Learning问题。在训练阶段，meta learner 通过任务聚类选择和结合多种度量方式来学习目标任务，不同领域的 meta task 首先通过聚类来划分，因此同一个簇内的 task 可以认为是相关的，然后在该簇中训练一个深度神经网络作为度量函数，这种机制保证了只有在同一个簇中的 task 才会共享度量函数。

在测试阶段，为每个 test task 使用所有度量函数的线性组合作为任务适应的度量方式。

![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpctASj9jlSRPiaZ0GWbkl6A4OQwLeC2UjWo5iamOzicmSAG0lgKCQGR1aUg/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpcqfNSjpcPAp5ZH2t0ibu57bY6KmibAKyFmcNTL0aib4fAubBTEKW7NYh1A/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpcxprHw9bV9icFr87y3Iib14rJDdWT2xrvjtz5RUZAOsYwQUx8Nyz9IhRQ/640?wx_fmt=png)
**▲**表1：Metric Based方法对比

![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpcAVWFTAPSq1GDItUvfrhrWZy9wYZJqEfRJHBZyFHsCpAj99e40k3z2w/640?wx_fmt=png)
**▲**表2：ODIC数据集实验结果

此外，我们在 ODIC 数据集上逐渐增加训练数据的类别数，如图 11，在测试集上得到的效果会逐渐提升，这满足了平台级的语言理解所需要的可泛化、可持续学习的需求。

![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/VBcD02jFhgnxtAYfe1ayyIrBDJNaeJpcp6BSvYkmVhuztGiboPS8ZaHXHemPrfLLgj2OLKHjwlGrmBFwVyic4N7A/640?wx_fmt=png)
**▲**图11：ODIC数据集变化趋势
...
针对 Metric Based 系列方法，我们提出了统一的 Encode-Induction-Relation 描述框架，介绍了我们团队在使用Few-shot Learning解决平台级自然语言理解所做的工作，即显式建模从样本表示到类表示的归纳能力。
