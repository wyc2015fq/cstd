...
2019年03月19日 14:49:15[Paper_weekly](https://me.csdn.net/c9Yv2cf9I06K2A9E)阅读数：4
...
隐变量 z （通常为服从高斯分布的随机噪声）通过 Generator 生成 Xfake, 判别器负责判别输入的 data 是生成的样本Xfake还是真实样本 Xreal。优化的目标函数如下：
...
可以证明，当 G，D 二者的 capacity 足够时，模型会收敛，二者将达到纳什均衡。此时，Pdata(x)=Pg(x)，判别器不论是对于Pdata(x)还是Pg(x)中采样的样本，其预测概率均为 1/2，即生成样本与真实样本达到了难以区分的地步。
...
Pg 表示生成数据分布，Pr 表示真实数据分布，E表示数学期望，x表示输入样本，x~Pg 表示 x 为生成样本的采样，x~Pr 表示 x 为真实样本的采样。y表示样本标签，M表示分类网络，通常选择 Inception network。
