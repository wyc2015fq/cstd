# pytorch之—工程规范 - wsp_1138886114的博客 - CSDN博客





2019年03月06日 17:27:58[SongpingWang](https://me.csdn.net/wsp_1138886114)阅读数：56
...
在学习深度学习框架时，掌握其基本知识和接口固然重要，但组织代码，使得代码具有良好的可读性和可扩展性同样重要，关于如何使得自己的程序更pythonic，更符合pytorch的设计理念。这些内容可能有些争议，因其受我个人喜好和coding风格影响较大，你可以将这部分当成是一种参考或提议。其实每个人都有自己认为合理的方式组织自己的程序。

在做深度学习实验或项目时，为了得到最优的模型结果，中间往往需要很多次的尝试和修改。我们希望尽可能的模块化代码，将代码分为逻辑块，配置文件块等，这样修改代码或者参数时不会有遗漏等情况。在从事大多数深度学习研究时，程序都需要实现以下几个功能：
- 模型定义
- 数据处理和加载
- 训练模型（Train&Validate）
- 训练过程的可视化
...
在本文我将应用这些内容，并结合实际的例子，来讲解如何用PyTorch完成Kaggle上的经典比赛：[Dogs vs. Cats](https://link.zhihu.com/?target=https%3A//www.kaggle.com/c/dogs-vs-cats-redux-kernels-edition)。本文所有示例程序均在github上开源 [https://github.com/chenyuntc/pytorch-best-practice](https://github.com/chenyuntc/pytorch-best-practice) 。

### 二、数据与工程文件

Dogs vs. Cats是一个传统的二分类问题，其训练集包含25000张图片，均放置在同一文件夹下，命名格式为..jpg, 如cat.10000.jpg、dog.100.jpg，测试集包含12500张图片，命名为.jpg，如1000.jpg。

参赛者需根据训练集的图片训练模型，并在测试集上进行预测，输出它是狗的概率。最后提交的csv文件如下，第一列是图片的，第二列是图片为狗的概率。
...
```python
├── checkpoints/            # 保存训练的模型，可重新载入模型，再次(恢复)训练
├── data/                   # 数据预处理、dataset实现
│   ├── __init__.py
│   ├── dataset.py
│   └── get_data.sh
├── models/                 # 模型定义，例如AlexNet和ResNet34等
│   ├── __init__.py
│   ├── AlexNet.py
│   ├── BasicModule.py
│   └── ResNet34.py
...
├── main.py                # 主文件，训练和测试程序入口，可通过不同的命令来指定不同的操作和参数
...
一个目录如果包含了__init__.py 文件，它就变成了一个包（package）。 `__init__.py` 可以为空，也可以定义包的属性和方法，但其必须存在，其它程序才能从这个目录中导入相应的模块或函数。
- 
例如在data/文件夹下**有**`__init__.py`(内容为空)，则在 [main.py](http://main.py) 中就可以如下使用函数方法
...
- 
而如果在data/`__init__.py`中**写入**`from .dataset import DogCat`则在main.py中就可以直接写：
...
- 
模型的定义与使用同理。`AlexNet.py`实现了AlexNet，`ResNet34`实现了ResNet34。在`models/__init__py`中代码如下：
...
# 这种写法意味着:我们可以通过字符串直接指定使用的模型，而不必使用判断语句
...
Kaggle提供的数据包括训练集和测试集，我们通过加一些判断来区分。对于训练集，我们希望做一些数据增强处理，如随机裁剪、随机翻转、加噪声等，而验证集和测试集则不需要。关于重写数据加载器[详情请查看](https://blog.csdn.net/wsp_1138886114/article/details/83620869)、[或者点击](https://blog.csdn.net/wsp_1138886114/article/details/84146704)。下面看dataset.py的代码：
...
class DogCat(data.Dataset):
   def __init__(self, root, transforms=None, train=True, test=False):
       ...
       np.random.seed(100)        # shuffle imgs
       ...
       if transforms is None:
           ...
           if self.test or not train:       # 测试集和验证集不用数据增强
...
其它自定义模型一般继承`BasicModule`，然后实现自己的模型。其中`AlexNet.py`实现了AlexNet，`ResNet34`实现了ResNet34。
...
class DefaultConfig(object):
    ...
    batch_size = 28  # 显存4G（batch_size=56约需8G显存） 
    use_gpu = True   # use GPU or not
    num_workers = 4  # 4线程加载数据
    print_freq = 20  # 每 N batch 输出信息

    debug_file = '/tmp/debug' # if os.path.exists(debug_file): enter ipdb
    result_file = 'result.csv'
      
    max_epoch = 10
    lr = 0.003          # initial learning rate 最好不要太大
...
这样我们在实际使用时，并不需要每次都修改`config.py`，只需要通过命令行传入所需参数，覆盖默认配置即可。例如：
...
在讲解主程序`main.py`之前，我们先来看看2017年3月谷歌开源的一个命令行工具`fire`[1](#fn1) ，通过`pip install fire`即可安装。下面来看看`fire`的基础用法，假设`example.py`文件内容如下：
...
可见，只要在程序中运行`fire.Fire()`，即可使用命令行参数`python file <function> [args,] {--kwargs,}`。fire还支持更多的高级功能，具体请参考官方指南[2](#fn2) 。
...
def val(model, dataloader): # 验证集上的准确率
    pass

def test(**kwargs):         # 测试（inference）
    pass
...
这里用到了PyTorchNet[3](#fn3)里面的一个工具: meter。meter提供了一些轻量级的工具，用于帮助用户快速统计训练过程中的一些指标。`AverageValueMeter`能够计算所有数的平均值和标准差，这里用来统计一个epoch中损失的平均值。`confusionmeter`用来统计分类问题中的分类情况，是一个比准确率更详细的统计指标。例如对于表格6-1，共有50张狗的图片，其中有35张被正确分类成了狗，还有15张被误判成猫；共有100张猫的图片，其中有91张被正确判为了猫，剩下9张被误判成狗。相比于准确率等统计信息，混淆矩阵更能体现分类的结果，尤其是在样本比例不均衡的情况下。
...
PyTorchNet从TorchNet[4](#fn4)迁移而来，提供了很多有用的工具，但其目前开发和文档都还不是很完善，本书不做过多的讲解。
...
鸣谢
[https://zhuanlan.zhihu.com/p/29024978](https://zhuanlan.zhihu.com/p/29024978)
- 
[https://github.com/google/python-fire](https://github.com/google/python-fire)[↩︎](#fnref1)

- 
[https://github.com/google/python-fire/blob/master/doc/guide.md](https://github.com/google/python-fire/blob/master/doc/guide.md)[↩︎](#fnref2)

- 
[https://github.com/pytorch/tnt](https://github.com/pytorch/tnt)[↩︎](#fnref3)

- 
[https://github.com/torchnet/torchnet](https://github.com/torchnet/torchnet)[↩︎](#fnref4)
