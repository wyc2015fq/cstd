# R-CNN—fast-R-CNN--faster-R-CNN 对比及相关原理解释 - wsp_1138886114的博客 - CSDN博客





2018年08月22日 11:01:32[SongpingWang](https://me.csdn.net/wsp_1138886114)阅读数：188
...
(5) 对于SVM分好类的Region Proposal做边框回归，用Bounding box回归值校正原来的建议窗口，生成预测窗口坐标.- 缺陷:
...
(5)     SVM和回归是事后操作，在**SVM和回归过程中CNN特征没有被学习更新**.
##### FAST-RCNN:
- FAST-CNN（特点）
(1) 输入测试图像； 
...
(6) 利用Softmax Loss(探测分类概率) 和Smooth L1 Loss(探测边框回归)对分类概率和边框回归(Bounding box regression)联合训练.- 相比R-CNN，主要两处不同:
(1)最后一层卷积层后加了一个ROI pooling layer； 

ROI pooling layer资料：[https://blog.csdn.net/auto1993/article/details/78514071](https://blog.csdn.net/auto1993/article/details/78514071)(2)损失函数使用了**多任务损失函数(multi-task loss)**，将边框回归直接加入到CNN网络中训练- 改进:
...
![这里写图片描述](https://img-blog.csdn.net/20180822104847449?watermark/2/text/aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dzcF8xMTM4ODg2MTE0/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70)
- Faster-RCNN（训练特点）
(1) 输入测试图像； 
...
(6) 利用**Softmax Loss(探测分类概率) 和Smooth L1 Loss(探测边框回归)**对**分类概率和边框回归(Bounding box regression)联合训练**.- 相比FAST-RCNN，主要两处不同:
(1)使用RPN(Region Proposal Network)代替原来的Selective Search方法产生建议窗口； 

(2)产生建议窗口的CNN和目标检测的CNN共享- 改进:
(1)     如何高效快速产生建议框？
...
2、softmax-loss
- softmax-loss 层和 softmax 层计算大致是相同的. softmax 是一个分类器，计算的是类别的概率（Likelihood），是Logistic Regression 的一种推广. **softmax 用于多分类.**
- **LogisticRegression 只能用于二分类**，
- 
...
总体思路:
- 
假设现在图像上有n个预分割的区域(Efficient Graph-Based ImageSegmentation),表示为R={R1, R2, …, Rn},
...
鸣谢： 
[https://blog.csdn.net/weixin_41923961/article/details/80113669](https://blog.csdn.net/weixin_41923961/article/details/80113669)
[https://www.cnblogs.com/dudumiaomiao/p/6560841.html](https://www.cnblogs.com/dudumiaomiao/p/6560841.html)
