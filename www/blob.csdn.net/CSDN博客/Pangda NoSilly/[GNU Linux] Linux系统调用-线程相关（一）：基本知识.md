# [GNU/Linux] Linux系统调用-线程相关（一）：基本知识 - Pangda NoSilly - CSDN博客

2017年07月25日 19:16:20[hepangda](https://me.csdn.net/hepangda)阅读数：629
...
### 函数原型及解释
`int pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*start_routine) (void *), void *arg);`- 返回值：若创建成功，则返回`0`，若创建失败，则返回错误号。
...
### 函数原型及解释
`void pthread_exit(void *retval);`- `retval`：该参数将作为线程的返回值，我们可以使用`pthread_join`来获得到这个指针。

# 等待线程终止并获取其终止状态

### 函数原型及解释
`int pthread_join(pthread_t thread, void **retval);`- 返回值：若函数执行成功，则返回`0`，发生错误则会返回`错误码`。
- `thread`：要获取终止状态的线程ID。
...
### 函数原型及解释
`int pthread_cancel(pthread_t thread);`- 返回值：若函数执行成功，则返回`0`，发生错误则会返回`错误码`。
- `thread`：要取消的线程的线程ID。
...
![进程与线程原语对比](https://img-blog.csdn.net/20170725173215299?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvaGVwYW5nZGE=/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)
