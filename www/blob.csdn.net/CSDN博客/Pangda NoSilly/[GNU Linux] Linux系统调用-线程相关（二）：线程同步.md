# [GNU/Linux] Linux系统调用-线程相关（二）：线程同步 - Pangda NoSilly - CSDN博客

2017年07月26日 11:33:32[hepangda](https://me.csdn.net/hepangda)阅读数：193
...
### 避免死锁：最长等待时间版本的加锁
`int pthread_mutex_timedlock(pthread_mutex_t *restrict mutex, const struct timespec *restrict abstime);`- 返回值：若执行成功则返回`0`，执行失败则返回代表错误类型的`错误码`。
- `mutex`：要加锁的互斥量。
