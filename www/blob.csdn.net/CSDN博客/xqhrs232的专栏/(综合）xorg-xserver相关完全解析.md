# (综合）xorg-xserver相关完全解析 - xqhrs232的专栏 - CSDN博客
2017年11月05日 15:33:48[xqhrs232](https://me.csdn.net/xqhrs232)阅读数：592
原文地址::[http://blog.csdn.net/qq160816/article/details/48624065](http://blog.csdn.net/qq160816/article/details/48624065)
相关文章
1、Linux下X Server工作方式----[http://blog.csdn.net/linuxheik/article/details/7593922](http://blog.csdn.net/linuxheik/article/details/7593922)
2、X窗口系统----[http://blog.csdn.net/linuxheik/article/details/8081259](http://blog.csdn.net/linuxheik/article/details/8081259)
3、【linux】X Server /
 X Client / Window manager 的关系----[http://blog.csdn.net/datamining2005/article/details/54694766](http://blog.csdn.net/datamining2005/article/details/54694766)
4、Linux X客户端、服务器、窗口管理器的关系----[http://blog.csdn.net/arnoldlu/article/details/17379481](http://blog.csdn.net/arnoldlu/article/details/17379481)
...
xinit 的用法为： xinit [[client] options ] [-- [server] [display] options] 。其中 client 用于指定一个基于 X 的应用程序， client 后面的 options 是传给这个应用程序的参数， server 是用于指定启动哪个 X 服务器，一般为 /usr/bin/X 或 /usr/bin/Xorg ， display 用于指定 display number ，一般
 为 0 ，表示第一个 display ， option 为传给 server 的参数。
...
如果不指定 server ， xinit 会查找 HOME( 环境变量 ) 目录下的 .xserverrc 文件，如果存在这个文件， xinit 直接调用 execvp 函数执行该文件。如果这个文件 不存在，那么 server 及其 display 为：  X :0 。如果系统目录中不存在 X 命令，那么我们需要在系统目录下建立一个名为 X 的链接，使其指向真正的 X server 命令（ Ubuntu 下为
 Xorg ）。
...
综上所述， startx 的默认启动过程为： startx 调用并将系统文件 /etc/X11/xinit/xinitrc 和 /etc/X11/xinit/xserverrc 作为参数传给 xinit ， xinit 就会先执行系统文件 /etc/X11/xinit/xserverrc 以启动 X Server ，然后执行 /etc/X11/xinit/xinitrc ，而 xinitrc 则会执行脚本 /etc/X11/Xsession
 ，而 Xsession 则会按顺序调用执行 /etc/X11/Xsession.d 目录下的文件，从而最终调用了 gnome-session 这个用于 启动 GNOME 桌面环境的程序
3.图形2d，3d加速简介
