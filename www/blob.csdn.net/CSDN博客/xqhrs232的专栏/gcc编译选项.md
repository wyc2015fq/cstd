# gcc编译选项 - xqhrs232的专栏 - CSDN博客
2016年10月20日 22:45:25[xqhrs232](https://me.csdn.net/xqhrs232)阅读数：264
个人分类：[Linux/Ubuntu/Fedora](https://blog.csdn.net/xqhrs232/article/category/906931)
原文地址::[http://blog.csdn.net/rheostat/article/details/19811407](http://blog.csdn.net/rheostat/article/details/19811407)
相关文章
1、[GCC 编译使用动态链接库和静态链接库--及先后顺序----及环境变量设置总结](http://blog.csdn.net/benpaobagzb/article/details/51364005)----[http://blog.csdn.net/benpaobagzb/article/details/51364005](http://blog.csdn.net/benpaobagzb/article/details/51364005)
## GCC编译器
### 常用选项
`-E`：只进行预处理，不编译
`-S`：只编译，不汇编
...
凡`-x`后面所列的所有文件都会被视为其指定的类型，要想改变类型可以再一次使用`-x`选项，或者使用
```
-x
 none
```
...
### 静态库的创建和使用
以创建静态库`static_lib.a`为例
- 编写C源文件`static_lib.c`，其中写入需要重复调用的函数，然后使用命令
```
gcc
 -c static_lib.c
```
...
### 动态库的创建和使用
以创建动态库`share_lib.so`为例
- 编写C源文件`share_lib.c`，写入需要重复调用的函数，然后使用命令
```
gcc
 -shared -fPIC -o share_lib.so share_lib.c
```
生成动态库文件`share_lib.so`
