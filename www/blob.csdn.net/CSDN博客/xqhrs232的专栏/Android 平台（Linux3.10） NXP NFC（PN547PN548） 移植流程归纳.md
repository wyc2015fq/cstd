# Android 平台（Linux3.10） NXP NFC（PN547PN548） 移植流程归纳 - xqhrs232的专栏 - CSDN博客
2018年01月14日 00:05:05[xqhrs232](https://me.csdn.net/xqhrs232)阅读数：391
原文地址::[http://blog.csdn.net/dearsq/article/details/52056409](http://blog.csdn.net/dearsq/article/details/52056409)
相关文章
1、NFC在Android平台的架构与移植----[http://blog.csdn.net/dearsq/article/details/50585287](http://blog.csdn.net/dearsq/article/details/50585287)
2、移植NFC驱动到android系统----[http://blog.csdn.net/whut_fn/article/details/50350225](http://blog.csdn.net/whut_fn/article/details/50350225)
3、NXP NFC移植及学习笔记(原创)----[http://www.mamicode.com/info-detail-1327715.html](http://www.mamicode.com/info-detail-1327715.html)
4、nfc pn544驱动----[http://download.csdn.net/download/mingchong2005/6018175](http://download.csdn.net/download/mingchong2005/6018175)
本文唯一地址：[http://blog.csdn.net/dearsq/article/details/52056409](http://blog.csdn.net/dearsq/article/details/52056409)
欢迎转载，转载请著名～谢谢！
...
- 1
### 2. 修改平台配置
#### IDH/device/PLATFORM_NAME/xxx.rc 为节点添加权限
...
- 12
修改完驱动部分后，进行编译，可以利用
`make -32 2>&1|tee makeKernel.log `- 1
...
请反馈给我们的工程师确认 Kernel 部分移植无误后再进行上层部分的移植，否则会增加不必要的工作量。谢谢。
### 3. 其他
Hisi 平台的 i2c 通信失败可能需要修改平台端 i2c 相关的代码。 
MTK 平台 的 dts 请参考释放的资料包中的 dts。在移植Middleware代码的时候有一些额外的配置（除了正常移植步骤外，需要先注释掉平台自己实现的一套 NFC API） ，所以请先完成 Kernel 部分的移植后再进行 Middleware 部分的移植。 
...
删除平台原有的 libnfc-nci
`rm external/libnfc-nci -rf`- 1
用我们所释放的对应代码替换之。
### 3. framework 部分
- 将释放代码中的 framework/base/core/java/com 下的 nxp 与 vzw 替换平台代码中的相应文件夹（若没有则添加）
- 将释放代码中的 framework/base/core/java/android/ 下的 nfc 文件夹替换平台代码中的相应文件夹
- 修改 frameworks/base/Android.mk ：用 BeyondCompare 等对比软件对比修改，主要是添加 nfc 相关的内容
...
### 8. 其他
MTK 平台的额外配置请根据 《MTK平台修改配置.txt》来进行修改。
