# 掌握 Linux 调试技术 - xqhrs232的专栏 - CSDN博客
2012年06月08日 14:23:06[xqhrs232](https://me.csdn.net/xqhrs232)阅读数：503标签：[linux																[allocation																[工具																[数据结构																[汇编																[makefile](https://so.csdn.net/so/search/s.do?q=makefile&t=blog)](https://so.csdn.net/so/search/s.do?q=汇编&t=blog)](https://so.csdn.net/so/search/s.do?q=数据结构&t=blog)](https://so.csdn.net/so/search/s.do?q=工具&t=blog)](https://so.csdn.net/so/search/s.do?q=allocation&t=blog)](https://so.csdn.net/so/search/s.do?q=linux&t=blog)
个人分类：[Linux/Ubuntu/Fedora](https://blog.csdn.net/xqhrs232/article/category/906931)
原文地址::[http://blog.csdn.net/cellbird/article/details/396798](http://blog.csdn.net/cellbird/article/details/396798)
在 Linux 上找出并解决程序错误的主要方法
[Steve Best](http://www-128.ibm.com/developerworks/cn/linux/sdk/l-debug/index.html#author1)
JFS 核心小组成员，IBM
2002 年 8 月 09 日
> 
...
本文讨论了四种调试 Linux 程序的情况。在第 1 种情况中，我们使用了两个有内存分配问题的样本程序，使用 MEMWATCH 和 Yet Another Malloc Debugger（YAMD）工具来调试它们。在第 2 种情况中，我们使用了 Linux 中的 strace 实用程序，它能够跟踪系统调用和信号，从而找出程序发生错误的地方。在第 3 种情况中，我们使用 Linux 内核的 Oops 功能来解决程序的段错误，并向您展示如何设置内核源代码级调试器（kernel source level debugger，kgdb），以使用
 GNU 调试器（GNU debugger，gdb）来解决相同的问题；kgdb 程序是使用串行连接的 Linux 内核远程 gdb。在第 4 种情况中，我们使用 Linux 上提供的魔术键控顺序（magic key sequence）来显示引发挂起问题的组件的信息。
常见调试方法
...
MEMWATCH
MEMWATCH 由 Johan Lindh 编写，是一个开放源代码 C 语言内存错误检测工具，您可以自己下载它（请参阅本文后面部分的 [参考资料](http://www-128.ibm.com/developerworks/cn/linux/sdk/l-debug/index.html#resources)）。只要在代码中添加一个头文件并在 gcc 语句中定义了 MEMWATCH 之后，您就可以跟踪程序中的内存泄漏和错误了。MEMWATCH
 支持 ANSI C，它提供结果日志纪录，能检测双重释放（double-free）、错误释放（erroneous free）、没有释放的内存（unfreed memory）、溢出和下溢等等。
**清单 1. 内存样本（test1.c）**
```
#include <stdlib.h>
#include <stdio.h>
...
YAMD
YAMD 软件包由 Nate Eldredge 编写，可以查找 C 和 C++ 中动态的、与内存分配有关的问题。在撰写本文时，YAMD 的最新版本为 0.32。请下载 yamd-0.32.tar.gz（请参阅 [参考资料](http://www-128.ibm.com/developerworks/cn/linux/sdk/l-debug/index.html#resources)）。执行 `make` 命令来构建程序；然后执行 
```
make
 install
```
...
execve("/sbin/mkfs.jfs", ["mkfs.jfs", "-f", "/dev/test1"], &
 ...
 write(2, "mkfs.jfs: can/'t determine device"..., ..._exit(1)
  = ?
```
清单 6 显示 `ioctl` 调用导致用来格式化分区的 `mkfs` 程序失败。 `ioctl BLKGETSIZE64` 失败。（ `BLKGET-SIZE64` 在调用 `ioctl` 的源代码中定义。) `BLKGETSIZE64 ioctl` 将被添加到 Linux 中所有的设备，而在这里，逻辑卷管理器还不支持它。因此，如果 `BLKGETSIZE64 ioctl` 调用失败，mkfs
...
kgdb
kgdb 程序（使用 gdb 的远程主机 Linux 内核调试器）提供了一种使用 gdb 调试 Linux 内核的机制。kgdb 程序是内核的扩展，它让您能够在远程主机上运行 gdb 时连接到运行用 kgdb 扩展的内核机器。您可以接着深入到内核中、设置断点、检查数据并进行其它操作（类似于您在应用程序上使用 gdb 的方式）。这个补丁的主要特点之一就是运行 gdb 的主机在引导过程中连接到目标机器（运行要被调试的内核）。这让您能够尽早开始调试。请注意，补丁为 Linux 内核添加了功能，所以 gdb 可以用来调试
 Linux 内核。
...
**格式化 Oops 消息**
![格式化 Oops 消息](http://www-128.ibm.com/developerworks/cn/linux/sdk/l-debug/fig1.jpg)
...
参考资料
- 您可以参阅本文在 developerWorks 全球站点上的 [英文原文](http://www.ibm.com/developerworks/library/l-debug/index.html). 
- 下载 [MEMWATCH](http://www.linkdata.se/sourcecode.html)。 
- 下载 [YAMD](http://www3.hmc.edu/~neldredge/yamd)。 
- 下载 [ElectricFence](ftp://ftp.perens.com/pub/ElectricFence)。 
- 请查看 [Dynamic Probes 调试功能程序](http://www-124.ibm.com/developerworks/oss/linux/projects/dprobes/)。 
- 请阅读文章“ [Linux software debugging with GDB](http://www.ibm.com/developerworks/library/l-gdb/)”。（*developerWorks*，2001 年 2 月） 
- 请访问 IBM [Linux Technology Center](http://www-128.ibm.com/developerworks/cn/linux/sdk/l-debug/oss.software.ibm.com/developerworks/opensource/linux&origin=l)。 
- 在 *developerWorks*Linux 专区可以找到 [更多的 Linux 文章](http://www.ibm.com/developerworks/linux/?article=lr)。 
关于作者
Steve Best 在位于德克萨斯州奥斯汀的 [IBM Linux Technology Center](http://www-128.ibm.com/developerworks/cn/linux/sdk/l-debug/oss.software.ibm.com/developerworks/opensource/linux&origin=l) 工作。目前，他在做 Linux 项目的日志纪录文件系统（Journaled
 File System，JFS）的工作。Steve 在操作系统方面有丰富的从业经验，他的着重的领域是文件系统、国际化和安全性。
