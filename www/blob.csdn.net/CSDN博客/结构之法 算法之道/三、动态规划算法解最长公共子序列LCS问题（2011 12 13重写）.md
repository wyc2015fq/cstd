# 三、动态规划算法解最长公共子序列LCS问题（2011.12.13重写） - 结构之法 算法之道 - CSDN博客





2010年12月31日 17:46:00[v_JULY_v](https://me.csdn.net/v_JULY_v)阅读数：92559标签：[算法																[zk																[c																[matrix																[面试																[string](https://so.csdn.net/so/search/s.do?q=string&t=blog)
个人分类：[08.MS 100' one Keys																[01.Algorithms（研究）](https://blog.csdn.net/v_JULY_v/article/category/769275)](https://blog.csdn.net/v_JULY_v/article/category/772664)
...
    下面，咱们运用此动态规划算法解此LCS问题。有一点必须声明的是，LCS问题即最长公共子序列问题，它**不**要求所求得的字符在所给的字符串中是连续的（例如：输入两个字符串BDCABA和ABCBDAB，字符串BCBA和BDAB都是是它们的最长公共子序列，则输出它们的长度4，并打印任意一个子序列）。
...
- 
**最大子序列**：最大子序列是要找出由数组成的一维数组中和最大的连续子序列。比如{5,-3,4,2}的最大子序列就是{5,-3,4,2}，它的和是8,达到最大；而{5,-6,4,2}的最大子序列是{4,2}，它的和是6。你已经看出来了，找最大子序列的方法很简单，只要前i项的和还没有小于0那么子序列就一直向后扩展，否则丢弃之前的子序列开始新的子序列，同时我们要记下各个子序列的和，最后找到和最大的子序列。更多请参看：程序员编程艺术[第七章、求连续子数组的最大和](http://blog.csdn.net/v_JULY_v/archive/2011/05/25/6444021.aspx)。
...
    程序运行结果如下所示：
![](http://hi.csdn.net/attachment/201112/13/0_1323746545loxO.gif)
...
        如果我们记字符串Xi和Yj的LCS的长度为c[i,j]，我们可以递归地求c[i,j]：![](http://hi.csdn.net/attachment/201108/17/0_1313577392SuoV.gif)
...
    OK，更多，请参考：程序员编程艺术[第十一章、最长公共子序列（LCS）问题](http://blog.csdn.net/v_july_v/article/details/6695482)。完。](https://so.csdn.net/so/search/s.do?q=面试&t=blog)](https://so.csdn.net/so/search/s.do?q=matrix&t=blog)](https://so.csdn.net/so/search/s.do?q=c&t=blog)](https://so.csdn.net/so/search/s.do?q=zk&t=blog)](https://so.csdn.net/so/search/s.do?q=算法&t=blog)
