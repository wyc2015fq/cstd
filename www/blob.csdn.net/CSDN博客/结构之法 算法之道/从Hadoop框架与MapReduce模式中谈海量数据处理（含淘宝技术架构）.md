# 从Hadoop框架与MapReduce模式中谈海量数据处理（含淘宝技术架构） - 结构之法 算法之道 - CSDN博客





2011年08月20日 13:08:53[v_JULY_v](https://me.csdn.net/v_JULY_v)阅读数：177738标签：[mapreduce																[hadoop																[框架																[存储																[hbase](https://so.csdn.net/so/search/s.do?q=hbase&t=blog)
个人分类：[27.Architecture design																[22.Big Data Processing](https://blog.csdn.net/v_JULY_v/article/category/1106578)](https://blog.csdn.net/v_JULY_v/article/category/854240)
...
![](http://hi.csdn.net/attachment/201108/20/0_131381852997kj.gif)
...
> 
    一个MapReduce作业（job）通常会把输入的数据集切分为若干独立的数据块，由 Map任务（task）以完全并行的方式处理它们。框架会对Map的输出**先进行排序**，然后把结果输入给Reduce任务。通常作业的输入和输出都会被存储在文件系统中。整个框架负责任务的调度和监控，以及重新执行已经失败的任务。如下图所示（Hadoop MapReduce处理流程图）：

![](http://hi.csdn.net/attachment/201108/20/0_1313816714BlYw.gif)
...
![](http://hi.csdn.net/attachment/201108/20/0_131381690985qz.gif)
...
**结语**：写文章是一种学习的过程。**尊重他人劳动成果，转载请注明出处。谢谢。July、2011/8/20。完。**](https://so.csdn.net/so/search/s.do?q=存储&t=blog)](https://so.csdn.net/so/search/s.do?q=框架&t=blog)](https://so.csdn.net/so/search/s.do?q=hadoop&t=blog)](https://so.csdn.net/so/search/s.do?q=mapreduce&t=blog)
