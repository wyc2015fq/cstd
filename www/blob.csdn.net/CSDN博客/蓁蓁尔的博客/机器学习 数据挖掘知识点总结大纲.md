# 机器学习 数据挖掘知识点总结大纲 - 蓁蓁尔的博客 - CSDN博客





2017年03月06日 16:09:40[蓁蓁尔](https://me.csdn.net/u013527419)阅读数：1187
...
Word2Vec(词向量学习模型)Dimensionality Reduction(降维)：
...
FA(Factor Analysis 因子分析法)Text Mining(文本挖掘)：
...
Skip-gram(Skip-gram Model)Association Mining(关联挖掘)：
...
GSpan(Graph-based Substructure Pattern Mining, 频繁子图挖掘)Sequential Patterns Analysis(序列模式分析)
...
PageRank(网页排名)Recommendation Engine(推荐引擎)：
...
ICF(Item-based Collaborative Filtering Recommendation, 基于项目的协同过滤推荐)Similarity Measure&Distance Measure(相似性与距离度量)：
...
KL(Kullback-Leibler Divergence, KL散度/Relative Entropy, 相对熵)Optimization(最优化)：
...
ACO(Ant Colony Optimization, 蚁群算法)Feature Selection(特征选择)：
...
Gini(基尼系数)Outlier Detection(异常点检测)：

Statistic-based(基于统计) 

Density-based(基于密度) 

Clustering-based(基于聚类)。Learning to Rank(基于学习的排序)：
...
转自：[](http://blog.csdn.net/heyongluoyao8/article/details/47840255)http://blog.csdn.net/heyongluoyao8/article/details/47840255
