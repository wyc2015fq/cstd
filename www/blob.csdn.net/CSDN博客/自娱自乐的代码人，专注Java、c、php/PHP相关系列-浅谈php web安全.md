# PHP相关系列 - 浅谈php web安全 - 自娱自乐的代码人，专注Java、c、php - CSDN博客





2012年10月10日 09:06:51[initphp](https://me.csdn.net/initphp)阅读数：1283标签：[php																[web																[mysql																[sql																[string](https://so.csdn.net/so/search/s.do?q=string&t=blog)
...
2012-09-27 13:18
621人阅读[评论](http://blog.csdn.net/heiyeshuwu/article/details/8024567#comments)(0)[收藏]()[举报](http://blog.csdn.net/heiyeshuwu/article/details/8024567#report)

作者：phpben

来源：[http://www.phpben.com/?post=79](http://www.phpben.com/?post=79)
...
这是因为mysql中其中的一个注悉是“#”，上面语句中#已经把后面的内容给注悉掉，所以密码可以不输入或任意输入。网上有些人介绍说用“/*”来注悉，笔者想提的是只有开始注悉没结束注悉“*/”时，mysql会报错，也不是说“/**/”不能注悉，而是这里很难添加上“*/”来结束注悉，还有“-- ”也是可以注悉mysql 但要注意“--”后至少有一个空格也就是“-- ”，当然防注入代码要把三种都考虑进来，**值得一提的是**很多防注入代码中没把“-- ”考虑进防注入范围。
...
![](http://www.phpben.com/uploadfile/13486579570.30098200.jpg)
...
逻辑关键可以**组合**很多注入代码；mysql注悉则把固有sql代码后面的字符全部给注悉掉从而让注入后的sql语句能正常运行；“\”也是能组合很多注入字符\x00,\x1a。
...
对比容易发现，post过来的数据通过addslashes过滤后的确很多注入已经不起作用，但是$id并没有intval，导致漏洞的存在，这是个**小细节**，不小心则导致漏洞。
...
还有上面代码过滤**下划线**的笔者注悉掉了，因为有些参数可以使用下划线，自己权衡怎么过滤;
...
![](http://www.phpben.com/uploadfile/13486580760.64092200.jpg)
...
- <dody>
- <formaction="phpben.com?user_name=<?php echo $user_name; ?>">
...
- <formaction="phpben.com?user_name=benwin"onSubmit="alert('这是xss攻击的例子');"class= "">
- <inputtype="submit"value="提交">
- </form>
...
- <inputtype="text"name="##"value="<?php echo $val; ?>"/>
...
- <ahref="benwin.php?id= <?php echo $id; ?>">a标签可以隐藏xss攻击</a>
...
![](http://www.phpben.com/uploadfile/13486581470.89759700.jpg)
...
如当你点击**QQ邮件**里面的长外链时候，回跳转到一个页面提示“有风险”之类，这样不仅可以减低跳出率，一些不懂的人看到这样的提示，若不是非必要而是处于好奇点击的连接一般不会继续点击访问；**还有**是QQ邮件正文里的图片在加载内容时是不加载图片的，要点击“显示图片”按钮才显示图片，这里一个原因之一就是避免攻击。
...
详细百度百科：[http://baike.baidu.com/view/662394.htm](http://baike.baidu.com/view/662394.htm)
...
- $_SESSION['ll_times'] = $ll_times;  
- }](https://so.csdn.net/so/search/s.do?q=sql&t=blog)](https://so.csdn.net/so/search/s.do?q=mysql&t=blog)](https://so.csdn.net/so/search/s.do?q=web&t=blog)](https://so.csdn.net/so/search/s.do?q=php&t=blog)
