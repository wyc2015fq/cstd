# 转载和积累系列 - TCP协议疑难杂症全景解析 - 自娱自乐的代码人，专注Java、c、php - CSDN博客





2014年05月05日 18:58:14[initphp](https://me.csdn.net/initphp)阅读数：936
...
***IF 数据的大小和窗口的大小都超过了MSS    Then 发送数据分段ELSE    IF 还有发出的TCP分段的确认没有到来        Then 积累数据到发送队列的末尾的TCP分段    ELSE        发送数据分段    EndIFEndIF***
...
***IF (没有超过拥塞窗口大小的数据分段未确认 || 数据分段中包含FIN ) &&    数据分段没有超越窗口边界    Then     IF 分段在中间(上述例子中的分段1和2) ||           分段是紧急模式            ||       通过上述的Nagle算法(改进后的Nagle算法)        Then 发送分段    EndIF EndIF***
