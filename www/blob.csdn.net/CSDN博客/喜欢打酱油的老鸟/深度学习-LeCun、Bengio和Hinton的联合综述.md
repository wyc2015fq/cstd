...
# 深度学习-LeCun、Bengio和Hinton的联合综述 - 喜欢打酱油的老鸟 - CSDN博客


2018年08月17日 14:01:17[喜欢打酱油的老鸟](https://me.csdn.net/weixin_42137700)阅读数：69标签：[深度学习																](https://so.csdn.net/so/search/s.do?q=深度学习&t=blog)个人分类：[人工智能																](https://blog.csdn.net/weixin_42137700/article/category/7820233)



# 深度学习-LeCun、Bengio和Hinton的联合综述
[https://blog.csdn.net/GarfieldEr007/article/details/51104575?locationNum=2](https://blog.csdn.net/GarfieldEr007/article/details/51104575?locationNum=2)
...
![](https://shiweipku.gitbooks.io/dnn_for_asr/content/556b858e8685e_middle.jpg)
`图1 多层神经网络和BP算法`多层神经网络（用连接点表示）可以对输入空间进行整合，使得数据（红色和蓝色线表示的样本）线性可分。注意输入空间中的规则网格（左侧）是如何被隐藏层转换的（转换后的在右侧）。这个例子中只用了两个输入节点，两个隐藏节点和一个输出节点，但是用于目标识别或自然语言处理的网络通常包含数十个或者数百个这样的节点。获得[C.Olah](http://colah.github.io/)的许可后重新构建的这个图。
...
具有两个隐层一个输出层的神经网络中计算前向传播的公式。每个都有一个模块构成，用于反向传播梯度。在每一层上，我们首先计算每个节点的总输入z，z是前一层输出的加权和。然后利用一个非线性函数f(.)来计算节点的输出。简单期间，我们忽略掉了阈值项。神经网络中常用的非线性函数包括了最近几年常用的校正线性单元（ReLU）![](http://chart.googleapis.com/chart?cht=tx&chl=f(z)=max(0,z))，和更多传统sigmoid函数，比如双曲线正切函数![](http://chart.googleapis.com/chart?cht=tx&chl=f(z)=%5Cfrac%7Bexp(z)%20-%20exp(-z)%7D%7Bexp(z)%20%2B%20exp(-z)%7D)和logistic函数![](http://chart.googleapis.com/chart?cht=tx&chl=f(z)%20=%5Cfrac%7B1%7D%7B1%2Bexp(-z)%7D)。
...
![](https://shiweipku.gitbooks.io/dnn_for_asr/content/pic2.jpg)
图2 卷积神经网络内部
...
![![](pic3.jpg)](https://shiweipku.gitbooks.io/dnn_for_asr/content/pic3.jpg)
图3 从图像到文字
