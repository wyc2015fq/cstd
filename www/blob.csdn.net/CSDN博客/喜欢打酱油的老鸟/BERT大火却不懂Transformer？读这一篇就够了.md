...
# BERT大火却不懂Transformer？读这一篇就够了 - 喜欢打酱油的老鸟 - CSDN博客


2019年01月18日 08:06:23[喜欢打酱油的老鸟](https://me.csdn.net/weixin_42137700)阅读数：24标签：[BERT																](https://so.csdn.net/so/search/s.do?q=BERT&t=blog)[NLP																](https://so.csdn.net/so/search/s.do?q=NLP&t=blog)[Transformer																](https://so.csdn.net/so/search/s.do?q=Transformer&t=blog)[
							](https://so.csdn.net/so/search/s.do?q=NLP&t=blog)[
																					](https://so.csdn.net/so/search/s.do?q=BERT&t=blog)个人分类：[人工智能																](https://blog.csdn.net/weixin_42137700/article/category/7820233)
[
																								](https://so.csdn.net/so/search/s.do?q=BERT&t=blog)


[https://www.toutiao.com/a6644034402303083011/](https://www.toutiao.com/a6644034402303083011/)
2019-01-08 16:06:46
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/b58f333be156445f8e0110a7d0045d5c)
**大数据文摘与百度NLP联合出品**
**编译：张驰、毅航、Conrad、龙心尘**
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/b7abaa85548041288312afe21ff89601)

那么拆开这个黑箱，我们可以看到它是由编码组件、解码组件和它们之间的连接组成。
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/da61dd53dd6d476ba83008cbf0cf82ee)

编码组件部分由一堆编码器（encoder）构成（论文中是将6个编码器叠在一起——数字6没有什么神奇之处，你也可以尝试其他数字）。解码组件部分也是由相同数量（与编码器对应）的解码器（decoder）组成的。
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/6965361537bf465eb142d74bba11f130)

所有的编码器在结构上都是相同的，但它们没有共享参数。每个解码器都可以分解成两个子层。
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/34ca8bf200264772b6ce88487eb83bc7)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p3.pstatp.com/large/pgc-image/63f8a9ebd5db4656b444fb59dd0b9de2)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p3.pstatp.com/large/pgc-image/732ddcd36e0e4bcaac0fe968cc9c4d50)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/aed94c82bd8c4395bf7fb7149263492f)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p9.pstatp.com/large/pgc-image/a738a090d65c429bbe91b7a49161fb46)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/ea0bca389faf47ef9354b91859271862)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p9.pstatp.com/large/pgc-image/b3c45dd8dab349cf96b5756527ddee90)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p3.pstatp.com/large/pgc-image/e9aed30b3e8c43aea5d915ebd8576bfa)

第三步和第四步是将分数除以8(8是论文中使用的键向量的维数64的平方根，这会让梯度更稳定。这里也可以使用其它值，8只是默认值)，然后通过softmax传递结果。softmax的作用是使所有单词的分数归一化，得到的分数都是正值且和为1。
![BERT大火却不懂Transformer？读这一篇就够了](http://p3.pstatp.com/large/pgc-image/20ea25a88cbb48f5aacafb8184566a13)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p9.pstatp.com/large/pgc-image/b5430cf135284007994d1d45e1c5537e)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p3.pstatp.com/large/pgc-image/7254992febf44995a52b2115a3d5b12b)

x矩阵中的每一行对应于输入句子中的一个单词。我们再次看到词嵌入向量 (512，或图中的4个格子)和q/k/v向量(64，或图中的3个格子)的大小差异。
最后，由于我们处理的是矩阵，我们可以将步骤2到步骤6合并为一个公式来计算自注意力层的输出。
![BERT大火却不懂Transformer？读这一篇就够了](http://p3.pstatp.com/large/pgc-image/0825178454f14243819429162bcfc05d)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p3.pstatp.com/large/pgc-image/ccbec68928af4f3b8ac882abe2736673)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/257af6d8463b4bfaaadc8cf804bfe3c1)

这给我们带来了一点挑战。前馈层不需要8个矩阵，它只需要一个矩阵(由每一个单词的表示向量组成)。所以我们需要一种方法把这八个矩阵压缩成一个矩阵。那该怎么做？其实可以直接把这些矩阵拼接在一起，然后用一个附加的权重矩阵WO与它们相乘。
![BERT大火却不懂Transformer？读这一篇就够了](http://p3.pstatp.com/large/pgc-image/30239b66872c4fa6ab0d6077ff285579)

这几乎就是多头自注意力的全部。这确实有好多矩阵，我们试着把它们集中在一个图片中，这样可以一眼看清。
![BERT大火却不懂Transformer？读这一篇就够了](http://p3.pstatp.com/large/pgc-image/5bb08e438902443899cd81d165e40d6a)

既然我们已经摸到了注意力机制的这么多“头”，那么让我们重温之前的例子，看看我们在例句中编码“it”一词时，不同的注意力“头”集中在哪里：
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/d9412c6dddb64fcd98899cfb149b3122)

当我们编码“it”一词时，一个注意力头集中在“animal”上，而另一个则集中在“tired”上，从某种意义上说，模型对“it”一词的表达在某种程度上是“animal”和“tired”的代表。
然而，如果我们把所有的attention都加到图示里，事情就更难解释了：
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/9a65e4f5e8ff45f18d90424e90188cb9)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/c55fc6daa2474c94af7dbea8b51e5463)

为了让模型理解单词的顺序，我们添加了位置编码向量，这些向量的值遵循特定的模式。
如果我们假设词嵌入的维数为4，则实际的位置编码如下：
![BERT大火却不懂Transformer？读这一篇就够了](http://p3.pstatp.com/large/pgc-image/bc3ec4dca17c413fa75b80f4d538d28e)

尺寸为4的迷你词嵌入位置编码实例
这个模式会是什么样子？
在下图中，每一行对应一个词向量的位置编码，所以第一行对应着输入序列的第一个词。每行包含512个值，每个值介于1和-1之间。我们已经对它们进行了颜色编码，所以图案是可见的。
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/ed568596bfa349828229299e4e0e8fec)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/43194c61f70541558275af3437553908)

如果我们去可视化这些向量以及这个和自注意力相关联的层-归一化操作，那么看起来就像下面这张图描述一样：
![BERT大火却不懂Transformer？读这一篇就够了](http://p3.pstatp.com/large/pgc-image/86ee8aec1b8346278e7c276c4457297f)

解码器的子层也是这样样的。如果我们想象一个2 层编码-解码结构的transformer，它看起来会像下面这张图一样：
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/b3adad7a6461402f922b890278d87246)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/2c48fcb40c80404ca51ca3b48a3bb6c8)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/ab27542383f443e5a724d9187e515514)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/1fe7b178d8894c6a8b4e54f52c585a41)

我们模型的输出词表在我们训练之前的预处理流程中就被设定好。
一旦我们定义了我们的输出词表，我们可以使用一个相同宽度的向量来表示我们词汇表中的每一个单词。这也被认为是一个one-hot 编码。所以，我们可以用下面这个向量来表示单词“am”：
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/7b085df60bee488fae51429de71c4a58)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/f7bfc461df674fc9baa85e1261d8d26c)
...
![BERT大火却不懂Transformer？读这一篇就够了](http://p3.pstatp.com/large/pgc-image/60c6b6a672064812bfeb81a1965318e4)

依据例子训练模型得到的目标概率分布。
在一个足够大的数据集上充分训练后，我们希望模型输出的概率分布看起来像这个样子：
![BERT大火却不懂Transformer？读这一篇就够了](http://p1.pstatp.com/large/pgc-image/46c5013413c742daace5fe16ad86202a)
