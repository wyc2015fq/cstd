# 使用 C++11 智能指针时要避开的 10 大错误 - yixianfeng41的专栏 - CSDN博客
2017年02月22日 23:27:27[第2梦](https://me.csdn.net/yixianfeng41)阅读数：826标签：[c++智能指针错误规](https://so.csdn.net/so/search/s.do?q=c++智能指针错误规&t=blog)
...
## 错误2：没有保证shared_ptr共享的资源/对象的线程安全性！
...
## 错误4：没有使用make_shared来初始化shared_ptr！
...
## 错误5：在创建一个对象（裸指针）时没有立即把它赋给shared_ptr。
...
## 错误7：当使用一个shared_ptr指向指针数组时没有使用自定义的删除方法！
看看下面这段代码：
```cpp
void StartJob()
{
...
## 错误9：没有删除通过unique_ptr.release()返回的裸指针！
...
## 错误10：在调用weak_ptr.lock()的时候没检查它的有效性！
