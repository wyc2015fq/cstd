# tensorflow学习笔记（四十二）：输入流水线 - Keith - CSDN博客





2017年05月16日 21:16:09[ke1th](https://me.csdn.net/u012436149)阅读数：6770
...
![](https://github.com/KeithYin/mynotes//master/tensorflow/imgs/IncremeterFifoQueue.gif)
## Coordinator(协调者)
...
- 创建一个`Queue`
- 创建一个`enqueue_op`
- 使用`QueueRunner`创建一个线程来执行`enqueue_op`,并把`QueueRunner`放入`collection`中
- 返回创建的`Queue`
...
- 创建一个`RandomShuffleQueue`用来保存样本
- 使用`QueueRunner`创建多个`enqueue`线程向`Queue`中放数据
- 创建一个`dequeue_many` OP
- 返回`dequeue_many` OP
