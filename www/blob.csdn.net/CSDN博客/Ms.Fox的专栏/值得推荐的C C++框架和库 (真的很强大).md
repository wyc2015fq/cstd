# 值得推荐的C/C++框架和库 (真的很强大) - Ms.Fox的专栏 - CSDN博客
2017年06月18日 23:06:05[lulu-lu](https://me.csdn.net/smbluesky)阅读数：5517
又要开始搞C++了~~
## 值得学习的C语言开源项目
## - 1. Webbench
Webbench是一个在[Linux](http://lib.csdn.net/base/linux)下使用的非常简单的网站压测工具。它使用fork()模拟多个客户端同时访问我们设定的URL，[测试](http://lib.csdn.net/base/softwaretest)网站在压力下工作的性能，最多可以模拟3万个并发连接去测试网站的负载能力。Webbench使用[C语言](http://lib.csdn.net/base/c)编写,
 代码实在太简洁，源码加起来不到600行。
下载链接：[http://home.tiscali.cz/~cz210552/webbench.html](http://home.tiscali.cz/~cz210552/webbench.html)
## - 2. Tinyhttpd
tinyhttpd是一个超轻量型Http Server，使用[c语言](http://lib.csdn.net/base/c)开发，全部代码只有502行(包括注释)，附带一个简单的Client，可以通过阅读这段代码理解一个
 Http Server 的本质。
...
## - 6. Memcached
Memcached 是一个高性能的分布式内存对象缓存系统，用于动态Web应用以减轻[数据库](http://lib.csdn.net/base/mysql)负载。它通过在内存中缓存数据和对象来减少读取数据库的次数，从而提供动态数据库驱动网站的速度。Memcached
 基于一个存储键/值对的 hashmap。Memcached-1.4.7的代码量还是可以接受的，只有10K行左右。
...
另一方面，最近的[操作系统](http://lib.csdn.net/base/operatingsystem)，例如[linux](http://lib.csdn.net/base/linux) 最新版的内核源代码据说超过了1000
...
- [libRocket](http://librocket.com/) ：[libRocket](http://librocket.com/) 是一个C++
 HTML/CSS 游戏接口中间件
