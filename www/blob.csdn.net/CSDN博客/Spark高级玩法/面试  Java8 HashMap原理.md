# 面试 | Java8 HashMap原理 - Spark高级玩法 - CSDN博客
2018年12月04日 00:00:00[Spark高级玩法](https://me.csdn.net/rlnLo2pNEfx9c)阅读数：115
概述
在官方文档中是这样描述HashMap的：
Hash table based implementation of the Map interface. This implementation provides all of the optional map operations, and permits null values and the null key. (The HashMap classisroughlyequivalenttoHashtable, exceptthatitisunsynchronizedandpermitsnulls.) Thisclassmakesnoguaranteesastotheorderofthemap; inparticular, itdoesnotguaranteethattheorderwillremainconstantovertime.
基于Map接口实现、允许null键/值、非同步、不保证有序(比如插入的顺序)、也不保证顺序不随时间变化。
数据结构
数组 + 链表 + 红黑树
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFWy0EEu2QFZiapfMVRgQgIP92OSuFvhPODpeMCgKKDHCd1oicHaqjC89soMvNPPgmOk1AQXKfKV12Fg/640?wx_fmt=png)
源码实现
1. 类的继承关系　
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFWy0EEu2QFZiapfMVRgQgIP9ibOJ8xpAHfsiaboGGmYuIKpMAYf8M87dRiaduFThibno1pQTqTapp9dTgQ/640?wx_fmt=png)
官方文档的注释如下：
...
如oldCap为16时，如图
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFWy0EEu2QFZiapfMVRgQgIP9Qc8UMYCmp0v6Qpgx7fPEpKjwOvPYMWQGiaqdJt23s3LXDccFIPmKia6g/640?wx_fmt=png)
...
elseif (e instanceof TreeNode)         //红黑树
...
// 将第一个元素赋值给e，用e来记录
                e = p;
// hash值不相等或key不相等
elseif (p instanceof TreeNode)  //红黑树
// 放入树中
...
//将指定映射的所有映射关系复制到此映射中
publicvoidputAll(Map<? extends K, ? extends V> m) {
    putMapEntries(m, true);
}
//将m的所有元素存入本HashMap实例中，evict为false时表示构造初始HashMap
final voidputMapEntries(Map<? extends K, ? extends V> m, boolean evict) {
int s = m.size();
if (s > 0) {
// table未初始化
...
//将链表转换为红黑树
final voidtreeifyBin(Node<K,V>[] tab, int hash) {
...
**推荐阅读：**
[三年0故障是如何做到的？](http://mp.weixin.qq.com/s?__biz=MzA3MDY0NTMxOQ==&mid=2247485999&idx=1&sn=1027524e59652efedacf4f33598c0f79&chksm=9f38e907a84f60117625a94a7e451cd09157dcb24424311c059e3b49ec81929a3986645334be&scene=21#wechat_redirect)
