# 译：Tensorflow实现的CNN文本分类 - Spark高级玩法 - CSDN博客
2018年05月12日 08:18:31[Spark高级玩法](https://me.csdn.net/rlnLo2pNEfx9c)阅读数：952
...
## **2. 模型**
原始文献的网络结构如下图： 
![640?wx_fmt=jpeg](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_jpg/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleX4KEG9795zMvqOtualMgaHqDP19u7VRR60GgdgZGOwpbJCQUicbsJQRg/640?wx_fmt=jpeg)
...
- 
我们不会对权重向量执行L2规范约束。 《A Sensitivity Analysis of (and Practitioners’ Guide to) Convolutional Neural Networks for Sentence Classification》这篇文章中发现约束对最终结果几乎没有影响。(关注公众号输入cnn获取)
- 
原始实验用两个输入数据通道 - 静态和非静态字矢量。 我们只使用一个通道。
将这些扩展代码添加到这里是比较简单的（几十行代码）。 看看帖子结尾的练习。
## **3. 代码实现**
为了允许各种超参数配置，我们将代码放入TextCNN类中，在init函数中生成模型图。
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXGDDshUzQ4QibOLGCibx9apchqsjpAaXz93OTxiaRYDTBxiccN3pYPwQl9Q/640?wx_fmt=png)
为了实例化类，我们传递以下参数：
- 
**sequence_length **- 句子的长度。注意：我们将所有句子填充到相同的长度（我们的数据集为59）。
- 
**num_classes **- 输出层中的类数，在我们的例子中为（消极，积极）。
- 
**vocab_size **- 我们的词汇量的大小。 这需要定义我们的嵌入层的大小，它将具有[vocabulary_size，embedding_size]的形状。
- 
**embedding_size **- 嵌入的维度。
- 
**filter_sizes **- 我们想要卷积过滤器覆盖的字数。 我们将为此处指定的每个大小设置num_filters。 例如，[3，4，5]意味着我们将有一个过滤器，分别滑过3，4和5个字，总共有3 * num_filters个过滤器。
- 
**num_filters **- 每个过滤器大小的过滤器数量（见上文）。
### **3.1 INPUT PLACEHOLDERS**
首先定义网络的输入数据
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleX9v0r4RgicZNqcr8S8PMpEyv3EQN9dlx1k5TL6n6bgJ9WjSod5sl3YKQ/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXPfK5iaPHvLvKWKWwNwNo8qyibbdNAbATc8hBQkUytIkMCKic1M1lo6laQ/640?wx_fmt=png)
我们在这里使用了几个功能：
- 
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleX6hFYwzevmSo83ngmpDqmyBhL97jzVrgjPZEFBX9TwsmDVScBCeEiawQ/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXh8Ap0sYTQyeBOqeMXaxCn1icEwELAe9ZMxkyK1JE8vwsWibAXTuQSa6Q/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleX34TD6s80ZERBkdZ3zTWXPwou7lR3dLGXZjTteNcDMlIS4lxxqKnpoQ/640?wx_fmt=png)
### **3.6 LOSS AND ACCURACY**
使用分数我们可以定义损失函数。 损失是对我们网络错误的衡量，我们的目标是将其最小化。分类问题的标准损失函数是交叉熵损失 cross-entropy loss。
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXN5VjkvLJIvkv0JJ0V2leib11EofaeSfYRzQQia3UcpeY9Dlg5cHOMX4g/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXVJaHoZaPFjsAsUhiaz4vsgtLA8ialoOYs6zZzyQ2AwibHTdnDTOlsj4TQ/640?wx_fmt=png)
TensorFlow可以看到其**结构图**如下：
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXgRvMOvJf2zLB5TOmvwmYuQDbtibicicOdCOqwDSTOCyrCgsMtZHLmTjxw/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXRkkyHj3d2hqhBHfFxv5vgRFqb96KLYzpfCtibN4yU6WDA6HiaLgeRKVQ/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXvPWt2l2byAOpHKcOicyPhDiagyTqdiaqcicATSicxVb149qFZafavMFvibwg/640?wx_fmt=png)
接下来，我们定义如何优化网络的损失函数。 TensorFlow有几个内置优化器。 我们正在使用Adam优化器。
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXq3D3K9vqrmDBI662DaWyMXKcdUdRauppB2A3hfUtUIom27gE2UbvDg/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXricS0WyrRo7dXK7MYiar6guGYXGfvFqUtte4t8ZvJdLIzsuCW3vvJbow/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXNKsIsmPR67xUQ0bwr7QeicfqmkS0Ev52sy8ELr3Zn7gPhsNoibW9sibTw/640?wx_fmt=png)
### **3.11 INITIALIZING THE VARIABLES**
在训练模型之前，我们还需要在图中初始化变量。
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXaeaP20hFrDdKCzPZEuq1WH4CGsMWho9hxPM3bByfZFsQXiafGQZC6KQ/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXSI8apO6xnkQv9EndhEL4ALlF8ztFicomvUu62Kz3Urzia5skLCawsCKA/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXIah7ibicxt1siaiaYyLJrT0889jxicbiaEVagL1I6Z9icyGKE242LSJmiaz2yQ/640?wx_fmt=png)
### **3.13 TRAINING LOOP**
最后，准备编写训练循环。 迭代数据的批次，调用每个批次的train_step函数，偶尔评估和检查我们的模型：
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXddhDtDBYws7ico9AiaEP3caOpicWgMkwqlFLuOEUKTziakQXSOtJXQwe3g/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/adI0ApTVBFUM7VjQtqKt8icdRaV4GkleXngPib6t2E6Eic15UHeFXV1OMSibA2cnwHib5PiaVn5hVlxa2qnzF8ib40BHA/640?wx_fmt=png)
**有几件事情脱颖而出：**
- 
...
**推荐阅读：**
[【深度学习】⑤--自然语言处理的相关应用](http://mp.weixin.qq.com/s?__biz=MzA3MDY0NTMxOQ==&mid=2247484458&idx=1&sn=670207cb6fa965bb39ec18726ef03262&chksm=9f38e702a84f6e147fa59f71afcd42b72d874d84c1dac264bf02ddbe894c06597a8376d71450&scene=21#wechat_redirect)
[基于机器学习对销量预测研究](http://mp.weixin.qq.com/s?__biz=MzA3MDY0NTMxOQ==&mid=2247484776&idx=1&sn=bbef18b79ca5fd6e5ed7c1b0becc2a08&chksm=9f38e640a84f6f568aec113c29d98cb50087426bd882879b92db825f036d0a1eceec67a5af96&scene=21#wechat_redirect)
[案例：Spark基于用户的协同过滤算法](http://mp.weixin.qq.com/s?__biz=MzA3MDY0NTMxOQ==&mid=2247484291&idx=1&sn=4599b4e31c2190e363aa379a92794ace&chksm=9f38e0aba84f69bd5b78b48e31b3f5b3792ec40e2d25fdbe6bc735f9c98ceb4584462b08e439&scene=21#wechat_redirect)
