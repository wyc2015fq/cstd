# 分布式开放消息系统(RocketMQ)的原理与实践 - Spark高级玩法 - CSDN博客
2018年04月27日 00:00:00[Spark高级玩法](https://me.csdn.net/rlnLo2pNEfx9c)阅读数：748
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqZIdYEEibIsptMib7uNic5Kvq0XWZwks5uUOdQtXgkBGiaZOtE96mMjLbSA/640?wx_fmt=png)
你可能会采用这种方式保证消息顺序
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqJTwVMpDenm2Pk82DytczNEJ1kK1dsibtVe1lvTialicXQibhqhuGCBBWFw/640?wx_fmt=png)
保证消息顺序，你改进后的方法
这样可以保证M1先于M2到达MQServer（生产者等待M1发送成功后再发送M2），根据先达到先被消费的原则，M1会先于M2被消费，这样就保证了消息的顺序。
这个模型也仅仅是理论上可以保证消息的顺序，在实际场景中可能会遇到下面的问题：
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqeMlmicKWuK9Crib2xm49s8iaEC1DnqOlWACBAV0iauVQYVPYAt0343yyKQ/640?wx_fmt=png)
网络延迟问题
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqRia7ic96JWGsGjZPkdsodUn0WZpP9XkveeaCEJpTicj1AOyib0bSTnvfEg/640?wx_fmt=png)
保证消息顺序的正确姿势
...
SendResult sendResult = producer.send(msg, new MessageQueueSelector() {    
    @Override    public MessageQueue select(List<MessageQueue> mqs, Message msg, Object arg) {        Integer id = (Integer) arg;        
        int index = id % mqs.size();
        return mqs.get(index);    }}, orderId);
```
```
...
private SendResult send()  {
    // 获取topic路由信息    TopicPublishInfo topicPublishInfo = this.tryToFindTopicPublishInfo(msg.getTopic());
    if (topicPublishInfo != null && topicPublishInfo.ok()) {        MessageQueue mq = null;        // 根据我们的算法，选择一个发送队列            // 这里的arg = orderId        mq = selector.select(topicPublishInfo.getMessageQueueList(), msg, arg);
        if (mq != null) {            
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqDooBCWSmyphHKNxBvQI2OdVmiaCr5yBrDTia0L0hLvVrfg4kWAyGP7uA/640?wx_fmt=png)
单机环境下转账事务示意图
当用户增长到一定程度，Bob和Smith的账户及余额信息已经不在同一台服务器上了，那么上面的流程就变成了这样：
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqXibEqzsM3WT0jmknKjdnBJrfzCUUe3xPhQfR0CV1MJtT11Z0wBJCEBg/640?wx_fmt=png)
集群环境下转账事务示意图
这时候你会发现，同样是一个转账的业务，在集群环境下，耗时居然成倍的增长，这显然是不能够接受的。那如何来规避这个问题？
> 
**大事务 = 小事务 + 异步**
将大事务拆分成多个小事务异步执行。这样基本上能够将跨机事务的执行效率优化到与单机一致。转账的事务就可以分解成如下两个小事务：
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqVfBiaa33iaNJOLwGfjiakHFGf1Uz860JF2qVqDmpNkcdkbVS3loic3g8Aw/640?wx_fmt=png)
小事务+异步消息
图中执行本地事务（Bob账户扣款）和发送异步消息应该保证同时成功或者同时失败，也就是扣款成功了，发送消息一定要成功，如果扣款失败了，就不能再发送消息。那问题是：我们是先扣款还是先发送消息呢？
首先看下先发送消息的情况，大致的示意图如下：
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqx7Ep4Tm16Guic42Mty1lib3XVG0Q2e01FZrGbn5aCbiaibJOrXeIR1FQZQ/640?wx_fmt=png)
事务消息：先发送消息
存在的问题是：如果消息发送成功，但是扣款失败，消费端就会消费此消息，进而向Smith账户加钱。
先发消息不行，那就先扣款吧，大致的示意图如下：
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqk1pFo6lnBtAT5G1KJHnTmIBguDrnNGewdugBcoGykfShRPSBA0ISOQ/640?wx_fmt=png)
事务消息-先扣款
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqiaqcz3kGTia07u0XVgEHTl0BBDDanudOIMZ2QRjpPI7JBia6Lgzp5Z92g/640?wx_fmt=png)
RocketMQ实现发送事务消息
...
// 构造MSG，省略构造参数Message msg = new Message(......);
// 发送消息
SendResult sendResult = producer.sendMessageInTransaction(msg, tranExecuter, null);producer.shutdown();
```
```
...
public TransactionSendResult sendMessageInTransaction(.....)  {    
    // 逻辑代码，非实际代码    // 1.发送消息    sendResult = this.send(msg);    
    // sendResult.getSendStatus() == SEND_OK    // 2.如果消息发送成功，处理与消息关联的本地事务单元    LocalTransactionState localTransactionState = tranExecuter.executeLocalTransactionBranch(msg, arg);    
    // 3.结束事务    this.endTransaction(sendResult, localTransactionState, localException);}
```
```
`endTransaction`方法会将请求发往`broker(mq server)`去更新事务消息的最终状态：
- 
根据`sendResult`找到`Prepared消息` ，`sendResult`包含事务消息的ID
- 
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqLBxAf6ZoMrRviaJvKo04hZNQQbhlzprBAC7oicvAVR3C1TSou5a0vdYw/640?wx_fmt=png)
消费事务消息
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqp8F4cfuDmcZUaicElWuicOia4ibcpLqXXpAVyhFo7gOvousjE1R7Eib73kQ/640?wx_fmt=png)
producer发送消息负载均衡
...
// 初始化Producer，整个应用生命周期内，只需要初始化1次producer.start();
// 构造MessageMessage msg = new Message("TopicTest1",// topic                        "TagA",// tag：给消息打标签,用于区分一类消息，可为null                        "OrderID188",// key：自定义Key，可以用于去重，可为null                        ("Hello MetaQ").getBytes());// body：消息内容
// 发送消息并返回结果SendResult sendResult = producer.send(msg);
// 清理资源，关闭网络连接，注销自己
producer.shutdown();
```
```
...
private SendResult sendDefaultImpl(Message msg,......) {
    // 检查Producer的状态是否是RUNNING    this.makeSureStateOK();
    // 检查msg是否合法：是否为null、topic,body是否为空、body是否超长    Validators.checkMessage(msg, this.defaultMQProducer);    
    // 获取topic路由信息    TopicPublishInfo topicPublishInfo = this.tryToFindTopicPublishInfo(msg.getTopic());    
    // 从路由信息中选择一个消息队列    MessageQueue mq = topicPublishInfo.selectOneMessageQueue(lastBrokerName);    
    // 将消息发送到该队列上去    sendResult = this.sendKernelImpl(msg, mq, communicationMode, sendCallback, timeout);}
```
```
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqT8vibsBHmmzvJFv00rA5QbfQDEvp8rX7pGt1HYahKX8QSLsDdkWia35g/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqO6ZHoqELxiaZXftVTWHloVWiaytaPcCzC7R6iafQ7ysnRNysta6Jbw2hQ/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqm2a20UnaDCj3OSg2Ub8ynQtaeNtHVZCxLyjbonYYjE6AFwefvq1jLw/640?wx_fmt=png)
Commit Log存储单元结构图
###### 3、消息存储实现
...
msg.setBodyCRC(UtilAll.crc32(msg.getBody()));StoreStatsService storeStatsService = this.defaultMessageStore.getStoreStatsService();synchronized (this) {
 ...
    // MapedFile：操作物理文件在内存中的映射以及将内存数据持久化到物理文件中    MapedFile mapedFile = this.mapedFileQueue.getLastMapedFile();        // 将Message追加到文件commitlog    result = mapedFile.appendMessage(msg, this.appendMessageCallback);    
    switch (result.getStatus()) {    
        case PUT_OK:break;    
        case END_OF_FILE:         
        // Create a new file, re-write the message        mapedFile = this.mapedFileQueue.getLastMapedFile();        result = mapedFile.appendMessage(msg, this.appendMessageCallback);
        break;     DispatchRequest dispatchRequest = new DispatchRequest(                topic,// 1                queueId,// 2                result.getWroteOffset(),// 3                result.getWroteBytes(),// 4                tagsCode,// 5                msg.getStoreTimestamp(),// 6                result.getLogicsOffset(),// 7                msg.getKeys(),// 8                /**                 * Transaction                 */                msg.getSysFlag(),// 9                msg.getPreparedTransactionOffset());// 10    // 1.分发消息位置到ConsumeQueue    // 2.分发到IndexService建立索引    this.defaultMessageStore.putDispatchRequest(dispatchRequest);}
```
```
###### 4、消息的索引文件
如果一个消息包含key值的话，会使用IndexFile存储消息索引，文件的内容结构如图：
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqCFPeIjHLE7lxPLyx2zxeRGIqrJjr0gzbuWInHsxxGjPtO66O06IGKw/640?wx_fmt=png)
消息索引
...
首先看下消费端的负载均衡：
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqtXpibxjBSBcFDy1ONpibONpZiarxQeJCLiapeSv3UdWXBtPCKkm0mJzIFg/640?wx_fmt=png)
消费端负载均衡
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/XCETLoXzTr8gbRtpeWjPWDzQ4ic96FiaEqLdr52bNqvLiccWICCptulp6Bw5M4ILVN18icdryMNqqcvliap9exbg5GQ/640?wx_fmt=png)
Push模式示意图
...
**推荐阅读：**
1，[开源消息中间件Kafka在华泰证券的探索与实践](http://mp.weixin.qq.com/s?__biz=MzA3MDY0NTMxOQ==&mid=2247484654&idx=1&sn=50331e2b175e72d01de213e227f68e62&chksm=9f38e7c6a84f6ed0e4adab7ccb50de5b8b31d54ec45ae97d4418e100ff6f429060cf796d1b6d&scene=21#wechat_redirect)
2，[kafka连接器两种部署模式详解](http://mp.weixin.qq.com/s?__biz=MzA3MDY0NTMxOQ==&mid=2247484229&idx=1&sn=d5e87da8a05c40b617f343044ba6d808&chksm=9f38e06da84f697b5181c1aedbeb9205e3af0a81dde6d19487cbde803e01d86fb9367b3a3b3b&scene=21#wechat_redirect)
3，[必读：Spark与kafka010整合](http://mp.weixin.qq.com/s?__biz=MzA3MDY0NTMxOQ==&mid=2247484604&idx=1&sn=c7fb09731502041562894ad9ef06a8ca&chksm=9f38e794a84f6e82dddf1b6c0b081f7f3e0512b85557e7165f87c443ca804f7af6e11846cffa&scene=21#wechat_redirect)
4，[大数据基础系列之kafka知识点和优点](http://mp.weixin.qq.com/s?__biz=MzA3MDY0NTMxOQ==&mid=2247483807&idx=1&sn=cda134ca527971b39512dd933891c151&chksm=9f38e2b7a84f6ba1d6692fdeec124803da07834b30957c14a3103cfab3fdfe421603dd7d93bc&scene=21#wechat_redirect)
