# MySQL的索引是什么？怎么优化？ - Spark高级玩法 - CSDN博客
2018年04月13日 00:00:00[Spark高级玩法](https://me.csdn.net/rlnLo2pNEfx9c)阅读数：257
...
![640](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIPJSoz9zA1Ry69aqdBmlsfdibXtmUgmsLBxq0BWACUMa5rZSiactmjRwtTDoQ3nmwfKlUSN7qzqjZTQ/640)
...
```
初体验，执行Explain的效果：
![640](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIPJSoz9zA1Ry69aqdBmlsfdRxZLv6fic1XCjticspLaRjXAHPsyYHENMGp7bpicq8Df5SJ1iagAmkFHfw/640)
...
```
![640](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIPJSoz9zA1Ry69aqdBmlsfdS0G2WzpPOEDt9icUMaS8qOUf8TSibwLtpibHpicusUcga5CPyCbdxytMHQ/640)
```
--id不同,值越大越先被执行
explain select * from  user_info  where id=(select user_id from order_info where  product_name ='p8');
```
![640](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIPJSoz9zA1Ry69aqdBmlsfdP32mxffITsWgABIpPYlHExUiaz5tEzdqSLRka5KpibmkNeCQWCRiaE8RQ/640)
### 2. select_type
...
![640](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIPJSoz9zA1Ry69aqdBmlsfd1trwjNPXXvmtrYbhRea2kxiaMJXu3z57SMF8aicA7VjQyvkcB0RJ49Qg/640)
id为1的<derived2>的表示id为2的u和o表衍生出来的。
### 4. type
type 字段比较重要，它提供了判断查询是否高效的重要依据依据。 通过 type 字段，我们判断此次查询是 全表扫描 还是 索引扫描等。
![640](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIPJSoz9zA1Ry69aqdBmlsfdq96vB9xaTMM0Bh2IaIRCRTxsqQFhW5YZvxh7IjshbOAodoAzLTIu0w/640)
type 常用的取值有:
- 
...
```
![640](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIPJSoz9zA1Ry69aqdBmlsfdNqxZq7MicD3Ds1mPMrNA0JIp2XjMDg5W9DTAia9cYnBFhFzF6pfFcvvg/640)
建立复合索引后再查询：
![640](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIPJSoz9zA1Ry69aqdBmlsfdq96EUW9ICVe4GHJXwyWOAszpdia8SA1Z95icaYmxuf3qrNAWAPtqrYgQ/640)
### 7. key_len
表示查询优化器使用了索引的字节数，这个字段可以评估组合索引是否完全被使用。
### 8. ref
这个表示显示索引的哪一列被使用了，如果可能的话,是一个常量。前文的type属性里也有ref，注意区别。
![640](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIPJSoz9zA1Ry69aqdBmlsfdj6wKS6mKfxJ2Soneqw5YnpicicWgJO7QiaVkmqdXXictRMYjhBPBQwrD0Q/640)
### 9. rows
...
### 10. extra
![640](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIPJSoz9zA1Ry69aqdBmlsfdGgzaOygOYvOXYcFvvicr9Txw0RveozOXicEjNpoWa2QNoWCtcBUdbibMQ/640)
...
## ![640](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIPJSoz9zA1Ry69aqdBmlsfdH5pjpibFI9LANQwibFH4cSiaWibMT51lmcyCo7NpPYvPLxYHa1ophmKefQ/640)
开始优化，在关联列上创建索引，明显看到type列的ALL变成ref，并且用到了索引，rows也从扫描9行变成了1行：
![640](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIPJSoz9zA1Ry69aqdBmlsfdv7ztogarygoMSh3aPBDG1AjJiblLiaoibqZbCJwWvY4oS7ibR8zKY6jDjA/640)
这里面一般有个规律是：左链接索引加在右表上面，右链接索引加在左表上面。
## 六、是否需要创建索引？   
索引虽然能非常高效的提高查询速度，同时却会降低更新表的速度。实际上索引也是一张表，该表保存了主键与索引字段，并指向实体表的记录，所以索引列也是要占用空间的。
![640](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/QCu849YTaIPJSoz9zA1Ry69aqdBmlsfdGgjqfyKLVcq7icD0fwuZib5GbgT1qSaa4pll9n95DKBW99xZDsTNmz0Q/640)
我是个普通的程序猿，水平有限，文章难免有错误，欢迎牺牲自己宝贵时间的读者就本文内容直抒己见，我的目的仅仅是希望文章对读者有所帮助。
- 
**原文：my.oschina.net/liughDevelop/blog/1788148**
