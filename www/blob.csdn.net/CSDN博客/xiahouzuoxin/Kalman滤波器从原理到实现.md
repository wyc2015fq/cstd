# Kalman滤波器从原理到实现 - xiahouzuoxin - CSDN博客





2014年09月26日 16:37:31[xiahouzuoxin](https://me.csdn.net/xiahouzuoxin)阅读数：125299
...
由牛顿第二定律，求得加速度：![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20a=F/m)

那么就有下面的位移和速度关系：

![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20s(t)=s(t-1)+v(t-1)%5CDelta%7BT%7D+%5Cfrac%7B1%7D%7B2%7D%5Cfrac%7BF%7D%7Bm%7D%7B(%5CDelta%7BT%7D)%7D%5E2)

![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20v(t)=v(t-1)+%5Cfrac%7BF%7D%7Bm%7D%5CDelta%7BT%7D)

如果将上面的表达式用矩阵写在一起，就变成下面这样：

![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Cbegin%7Bpmatrix%7D%20s(t)%20%5C%5C%20v(t)%20%5Cend%7Bpmatrix%7D=%7B%5Cbegin%7Bpmatrix%7D%201%20&%20%5CDelta%7BT%7D%20%5C%5C%200%20&%201%20%5Cend%7Bpmatrix%7D%7D%20%7B%5Cbegin%7Bpmatrix%7D%20s(t-1)%20%5C%5C%20v(t-1)%20%5Cend%7Bpmatrix%7D%7D%20+%20%7B%5Cbegin%7Bpmatrix%7D%20%5Cfrac%7B(%5CDelta%7BT%7D)%5E2%7D%7B2%7D%20%5C%5C%20%7B%5CDelta%7BT%7D%7D%20%5Cend%7Bpmatrix%7D%7D%20%7B%5Cfrac%7BF%7D%7Bm%7D%7D................................(1))

卡尔曼滤波器是建立在动态过程之上，由于物理量（位移，速度）的不可突变特性，这样就可以通过t-1时刻估计（预测）t时刻的状态，其状态空间模型为：

![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Cmathbf%7Bx(t)=Ax(t-1)+Bu(t)+w(t)%7D................................(2))

对比一下(1)(2)式，长得及其相似有木有：

![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20A=%7B%5Cbegin%7Bpmatrix%7D%201%20&%20%5CDelta%7BT%7D%20%5C%5C%200%20&%201%20%5Cend%7Bpmatrix%7D%7D,%20B=%7B%5Cbegin%7Bpmatrix%7D%20%5Cfrac%7B(%5CDelta%7BT%7D)%5E2%7D%7B2%7D%20%5C%5C%20%7B%5CDelta%7BT%7D%7D%20%5Cend%7Bpmatrix%7D%7D)

匀加速直线运动过程就是卡尔曼滤波中状态空间模型的一个典型应用。下面我们重点关注(2)式，鉴于研究的计算机信号都是离散的，将(2)是表示成离散形式为：
![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Cmathbf%7Bx(n)=Ax(n-1)+Bu(n)+w(n)%7D................................(3))
其中各个量之间的含义是：
...
![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Cmathbf%7Bz(n)=H(n)x(n)+v(n)%7D%20................................(4))
z(n)就是测量结果，H(n)是观测矢量，x(n)就是要求的物理量（位移、速度），v(n)~N(0,R)为测量噪声，同状态空间方程中的过程噪声一样，这也是一个后面要讨论的量。大部分情况下，如果物理量能直接通过传感器测量，![](http://www.forkosh.com/mathtex.cgi?%20%5Csmall%20%5Cmathbf%7BH(n)=I%7D)。

![img1](http://xiahouzuoxin.github.io/notes/images/Kalman%E6%BB%A4%E6%B3%A2%E5%99%A8%E4%BB%8E%E5%8E%9F%E7%90%86%E5%88%B0%E5%AE%9E%E7%8E%B0/img1.png)![img2](http://xiahouzuoxin.github.io/notes/images/Kalman%E6%BB%A4%E6%B3%A2%E5%99%A8%E4%BB%8E%E5%8E%9F%E7%90%86%E5%88%B0%E5%AE%9E%E7%8E%B0/img2.png)
...
![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Cmathbf%7B%5Chat%7Bx%7D(n%7Cn-1)=A%5Chat%7Bx%7D(n-1%7Cn-1)+Bu(n)%7D................................(5))

最小均方误差矩阵：

![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Cmathbf%7BP(n%7Cn-1)=AP(n-1%7Cn-1)A%5ET+Q%7D................................(6))
...
![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Cmathbf%7BK(n)=P(n%7Cn-1)H%5ET(n)%5BR(n)+H(n)P(n%7Cn-1)H%5ET(n)%5D%5E%7B-1%7D%7D.............(7))

修正值：

![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Cmathbf%7B%5Chat%7Bx%7D(n%7Cn)=A%5Chat%7Bx%7D(n%7Cn-1)+K(n)%5B%7Bz(n)-H(n)%5Chat%7Bx%7D(n%7Cn-1)%7D%5D%7D......................(8))**这里的A去掉，参见下面的图**

最小均方误差矩阵：

![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Cmathbf%7BP(n%7Cn)=%5BI-K(n)H(n)%5DP(n%7Cn-1)%7D................................(9))
...
![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Cmathbf%7BP(n%7Cn-1)=E%5C%7B%5Bx(n)-%5Chat%7Bx%7D(n%7Cn-1)%5D%5Bx(n)-%5Chat%7Bx%7D(n%7Cn-1)%5D%5ET%5C%7D%7D)
...
Kalman滤波算法的步骤是(5)(6)->(7)->(8)(9)。当然，建议找本教材复习下上面公式的推导过程，或参见Wiki上的介绍[http://en.wikipedia.org/wiki/Kalman_filter](http://en.wikipedia.org/wiki/Kalman_filter)。
...
还可以更简单一些：设小车做匀速（而非匀加速）直线运动，方便计算，假设速度绝对的恒定（不波动，所以相关的方差都为0），则u(t)==0恒成立。设预测（过程）位移噪声w(n)~N(0,2^2)，测量位移噪声v(n)~N(0,1^2)，n-1状态的位移![](http://www.forkosh.com/mathtex.cgi?%20%5CSmall%20%5Chat%7Bx%7D(n-1%7Cn-1)=50m%7D)，速度为v=10m/s，n时刻与n-1时刻的物理时差为ΔT=1s。同时，也用尺子测了一下，结果位移为z(n)=62m。

则A = [1 ΔT; 0 1]=[1 1; 0 1]，根据(5)，预测值为

![](http://www.forkosh.com/mathtex.cgi?%20%5CSmall%20%5Chat%7Bx%7D(n%7Cn-1)=1*%5Chat%7Bx%7D(n-1%7Cn-1)+v*%5CDelta%7BT%7D=%5B60m;%2010m/s%5D%7D)。
...
所以，最后的n时刻估计值既不是用n-1得到的估计值，也不是测量值，而是：![](http://www.forkosh.com/mathtex.cgi?%20%5CSmall%20%5Chat%7Bx%7D(n%7Cn)=%5B60;1%5D+%5B1/2;0%5D%7B62-%5B1%201%5D%5B60;1%5D%7D=%5B60.5m;%201m/s%5D)，因此，最终的Kalman滤波器的输出位移是60.5m。


从上面的递推关系知道，要估计n时刻就必须知道n-1时刻，那么n=0时刻该如何估计，因此，卡尔曼滤波要初始化的估计值x(-1|-1)和误差矩阵P(-1|-1)，设x(-1,-1)~N(Us, Cs)，则初始化：

![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Cmathbf%7Bx(-1%7C-1)=%5Cmu_s%7D)

![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Cmathbf%7BP(-1%7C-1)=C_s%7D)

综上，借用一张图说明一下Kalman滤波算法的流程：

![img3](http://xiahouzuoxin.github.io/notes/images/Kalman%E6%BB%A4%E6%B3%A2%E5%99%A8%E4%BB%8E%E5%8E%9F%E7%90%86%E5%88%B0%E5%AE%9E%E7%8E%B0/img3.png)
...
![img4](http://xiahouzuoxin.github.io/notes/images/Kalman%E6%BB%A4%E6%B3%A2%E5%99%A8%E4%BB%8E%E5%8E%9F%E7%90%86%E5%88%B0%E5%AE%9E%E7%8E%B0/img4.png)

![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20y_f=%5Cfrac%7B1%7D%7B%5Csqrt%7B2%5Cpi%7B%5Csigma%5E2_z%7D%7D%7D%5Cexp%5E%7B-%5Cfrac%7B(r-uz)%5E2%7D%7B2%5Csigma%5E2_z%7D%7D*%5Cfrac%7B1%7D%7B%5Csqrt%7B2%5Cpi%7B%5Csigma%5E2_x%7D%7D%7D%5Cexp%5E%7B-%5Cfrac%7B(r-ux)%5E2%7D%7B2%5Csigma%5E2_x%7D%7D=%5Cfrac%7B1%7D%7B%5Csqrt%7B2%5Cpi%7B%5Csigma%5E2%7D%7D%7D%5Cexp%5E%7B-%5Cfrac%7B(r-%5Cmu)%5E2%7D%7B2%5Csigma%5E2%7D%7D)

稍微计算一下，通过上式求出u和σ^2，
![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Cmu=%5Cfrac%7Bu_x%5Csigma%5E2_z+u_z%5Csigma%5E2_x%7D%7B%5Csigma%5E2_z+%5Csigma%5E2_x%7D=u_x+%5Cfrac%7B%5Csigma%5E2_x%7D%7B%5Csigma%5E2_z+%5Csigma%5E2_x%7D(u_z-u_x)........(10))![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Csigma%5E2=%5Cfrac%7B%5Csigma%5E2_x%5Csigma%5E2_z%7D%7B%5Csigma%5E2_x+%5Csigma%5E2_z%7D=%5Csigma%5E2_x(1-%5Cfrac%7B%5Csigma%5E2_x%7D%7B%5Csigma%5E2_x+%5Csigma%5E2_z%7D)........(11))
现在令
![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20K=%5Cfrac%7B%5Csigma%5E2_x%7D%7B%5Csigma%5E2_x+%5Csigma%5E2_z%7D........(12))
则(10)(11)变成：
![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Cmu==u_x+K(u_z-u_x)........(13))![](http://www.forkosh.com/mathtex.cgi?%20%5Clarge%20%5Csigma%5E2==%5Csigma%5E2_x(1-K)........(14))
...
![img5](http://xiahouzuoxin.github.io/notes/images/Kalman%E6%BB%A4%E6%B3%A2%E5%99%A8%E4%BB%8E%E5%8E%9F%E7%90%86%E5%88%B0%E5%AE%9E%E7%8E%B0/img5.png)
...
![img6](http://xiahouzuoxin.github.io/notes/images/Kalman%E6%BB%A4%E6%B3%A2%E5%99%A8%E4%BB%8E%E5%8E%9F%E7%90%86%E5%88%B0%E5%AE%9E%E7%8E%B0/img6.png)
...
- [http://robotsforroboticists.com/kalman-filtering/](http://robotsforroboticists.com/kalman-filtering/)公式彩色着色，含pyhton源码
- [http://alumni.media.mit.edu/~wad/mas864/psrc/kalman.c.txt](http://alumni.media.mit.edu/~wad/mas864/psrc/kalman.c.txt)包含Kalman滤波的C代码
- [http://www.cs.unc.edu/~welch/kalman/](http://www.cs.unc.edu/~welch/kalman/)比较全的Kalman链接
