# SpringCloudAPI网关服务Zuul - weixin_33985507的博客 - CSDN博客
2018年01月18日 08:37:16[weixin_33985507](https://me.csdn.net/weixin_33985507)阅读数：1
...
![](https://image-static.segmentfault.com/529/606/529606181-5a65986c05c55_articlex)
然后访问`http://localhost:8500/api-a/feign_consumer/find`和`http://localhost:8500/api-b-url/a/u/1`
![](https://image-static.segmentfault.com/342/766/3427661526-5a65986c1180d_articlex)
...
- filterType：过滤器类型，他决定过滤器在请求的哪个生命周期执行。在Zuul中有四种不同的生命周期过滤器：
```
- pre：可以在请求被路由之前调用；
- routing：在路由请求是调用；
...
- run： 过滤器的具体逻辑。上面的run方法中判断请求是否带有accessToken参数，如果没有则是非法请求，使用 `currentContext.setSendZuulResponse(false);`表示该请求不进行路由。然后设置响应码。
