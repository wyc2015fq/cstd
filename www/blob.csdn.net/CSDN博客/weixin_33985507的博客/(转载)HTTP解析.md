# (转载)HTTP解析 - weixin_33985507的博客 - CSDN博客
2011年01月23日 23:57:00[weixin_33985507](https://me.csdn.net/weixin_33985507)阅读数：6
## 1HTTP的特点 
  http（超文本传输协议）是一个基于请求与响应模式的、无状态的、应用层的协议，常基于TCP的连接方式，HTTP1.1版本中给出一种持续连接的机制，绝大多数的Web开发，都是构建在HTTP协议之上的Web应用。所有的[WWW](http://zh.wikipedia.org/zh-cn/WWW)文件都必须遵守这个标准。设计HTTP最初的目的是为了提供一种发布和接收[HTML](http://zh.wikipedia.org/zh-cn/HTML)页面的方法。
1是对TCP协议的封装。具有TCP协议的特点
2无状态。
3灵活：HTTP允许传输任意类型的数据对象。正在传输的类型由Content-Type加以标记。
4无连接的含义是限制每次连接只处理一个请求。服务器处理完客户的请求，并收到客户的应答后，即断开连接。采用这种方式可以节省传输时间。
## 2 HTTP协议的请求
...
CONNECT 保留将来使用HTTP/1.1协议中预留给能够将连接改为管道方式的代理服务器。     
...
[**HTTP协议相关技术补充**](http://blog.csdn.net/gueter/archive/2007/04/23/1575543.aspx)
    1、基础：   
...
2 利用HTTP协议的特性进行拒绝服务攻击的一些构思
3Http指纹识别技术
## 需要记下来的内容
1 请求的几个action   
2 返回的状态码    
3 HTTP协议的响应    
4 消息报头-各种报头指定了一些配置[http://en.wikipedia.org/wiki/HTTP#Request_message](http://en.wikipedia.org/wiki/HTTP#Request_message)
转载自 http://blog.csdn.net/gueter/archive/2007/03/08/1524447.aspx 
