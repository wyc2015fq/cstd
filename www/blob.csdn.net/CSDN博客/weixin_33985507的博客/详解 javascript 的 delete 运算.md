# 详解 javascript 的 delete 运算 - weixin_33985507的博客 - CSDN博客
2017年01月02日 05:24:25[weixin_33985507](https://me.csdn.net/weixin_33985507)阅读数：1
原文：[Understanding delete](https://link.juejin.im?target=http%3A%2F%2Fperfectionkills.com%2Funderstanding-delete%2F)
译文：[javascript 中的 delete](https://link.juejin.im?target=http%3A%2F%2Fjustjavac.com%2Fjavascript%2F2013%2F04%2F04%2Funderstanding-delete-in-javascript.html)
译者：[justjavac](https://link.juejin.im?target=http%3A%2F%2Fweibo.com%2Fjustjavac)
在这篇文章中作者从《[JavaScript面向对象编程指南](https://link.juejin.im?target=http%3A%2F%2Fwww.amazon.cn%2Fgp%2Fproduct%2FB00BLZMC4K%2Fref%3Das_li_ss_tl%3Fie%3DUTF8%26amp%3Bcamp%3D536%26amp%3Bcreative%3D3132%26amp%3BcreativeASIN%3DB00BLZMC4K%26amp%3BlinkCode%3Das2%26amp%3Btag%3Djsbook-23)》一书中关于 delete 的错误讲起，详细讲述了关于 delete 操作的实现，
局限以及在不同浏览器和插件（这里指
    firebug）中的表现。
下面翻译其中的主要部分。
...书中声称
...
我们还会讨论到 ECMAScript 版本 5 中的严格模式(strict
    mode)以及它如何改变 delete 运算符的行为。
> 
...
MDC([MDN](https://link.juejin.im?target=https%3A%2F%2Fdeveloper.mozilla.org%2Fen%2FJavaScript%2FReference%2FOperators%2FSpecial_Operators%2Fdelete_Operator)]) 的资源大概是这其中最详细的了，但不幸的是它遗漏了一些有趣的细节，这些细节中就包括了上述 firebug 的奇怪表现。
[MSDN 文档](https://link.juejin.im?target=http%3A%2F%2Fmsdn.microsoft.com%2Fen-us%2Flibrary%2F2b2z052x%2528VS.85%2529.aspx)几乎没什么用处。
## 一、理论 | Theory
那么，为什么我们能删除一个对象的属性：
```
var x = { a: 1 };
...
function x() {};
delete x; // false;
typeof x; // "function"复制代码
```
注意：delete 只有当一个**属性无法被删除**时才返回 false。
为了理解这一点，我们需要首先把握一些概念：
变量实例化（variable instantiation）和属性的内部属性（property attributes）
（译者按：关于 property 和 attributes 的区别见[参考文章](https://link.juejin.im?target=http%3A%2F%2Fwww.cnblogs.com%2Fwinner%2Farchive%2F2008%2F12%2F11%2F1353314.html)，根据下面涉及到的内容，拟译成内部属性）
...
- 
当一个源文件被看做是一个程序，它在全局作用域(scope)内执行，而这就被认为是一段全局代码 Global code。
在浏览器环境下，SCRIPT 元素的内容通常都被解析为一个程序，因而作为全局代码来执行。
- 
当然，任何在一段函数中直接执行的代码就被认为是一段函数代码 Function code，
在浏览器环境下，事件属性的内容（e.g. <a onclick="..."）通常都作为函数代码来解析和执行。
- 
最后，放入内建函数 eval 中的代码就作为 Eval code 来解析。
我们将很快看到为什么这一类型是特殊的。
...
GLOBAL_OBJECT.bar === bar; // true复制代码
```
...
typeof GLOBAL_OBJECT.baz; // "undefined"复制代码
```
...
[你可能记得](https://link.juejin.im?target=http%3A%2F%2Fperfectionkills.com%2Fonloadfunction-considered-harmful%2F%23how_does_it_work)，未声明的变量赋值会成为全局对象的属性，除非这一属性在作用域链内的其他地方被找到。
...
对于大部分来说，是的。
我写了一个[简单的测试单元](https://link.juejin.im?target=http%3A%2F%2Fkangax.github.com%2Fjstests%2Fdelete_compliance_test%2F)来检查全局代码、函数代码和Eval代码的遵守情况。
...
```
this.x = 1;
delete x; // TypeError: Object doesn't support this action复制代码
```
而下面则是另一个：
```
var x =1;
delete this.x; // TypeError: Cannot delete 'this.x'
// 译者按：在IE8下抛出此异常，在IE6,7下抛出的是和上面一样的异常复制代码
```
...
```
x = 1;
delete this.x; //TypeError: Cannot delete 'this.x'复制代码
```
...
在 [9 月我讨论了这个问题](https://link.juejin.im?target=https%3A%2F%2Fgroups.google.com%2Fgroup%2Fcomp.lang.javascript%2Fbrowse_thread%2Fthread%2F22e6b2d147f57ee5%2Fdda4dee3390fa71a)，其中 [Garrett Smith](https://link.juejin.im?target=http%3A%2F%2Fdhtmlkitchen.com%2F) 认为在 IE 中全局变量对象（Global variable object）实现为一个 JScript 对象，而全局对象则由宿主对象实现。
...
window.getBase() === getBase(); // false复制代码
```
## 五、误解 | Misconceptions
我们不能低估理解事物工作原理的重要性。
我看过网络上一些关于 delete 操作的误解。
例如，[Stackoverflow 上的一个答案](https://link.juejin.im?target=http%3A%2F%2Fstackoverflow.com%2Fquestions%2F1596782%2Fhow-to-unset-a-javascript-variable%2F1596889%231596889)（而且等级还很高），里面解释说“delete is supposed to be no-op when target isn’t an object
    property”。
...
typeof window.alert; // "function"复制代码
```
...
i_dont_exist_either = 1; // ReferenceError复制代码
```
...
你可以翻阅 [MDC/MDN 上的文章](https://link.juejin.im?target=https%3A%2F%2Fdeveloper.mozilla.org%2Fen%2FCore_JavaScript_1.5_Reference%2FOperators%2FSpecial_Operators%2Fdelete_Operator%23section_5)或阅读规范然后自己测试。
