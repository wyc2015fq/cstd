# JAVA NIO之浅谈内存映射文件原理与DirectMemory（转） - weixin_33985507的博客 - CSDN博客
2016年11月15日 08:50:00[weixin_33985507](https://me.csdn.net/weixin_33985507)阅读数：8
转自 [[Java][IO]JAVA NIO之浅谈内存映射文件原理与DirectMemory](https://link.jianshu.com?t=http://blog.csdn.net/szwangdf/article/details/10588489)
[Java](https://link.jianshu.com?t=http://lib.csdn.net/base/javaee)类库中的NIO包相对于IO 包来说有一个新功能是内存映射文件，日常编程中并不是经常用到，但是在处理大文件时是比较理想的提高效率的手段。本文我主要想结合[操作系统](https://link.jianshu.com?t=http://lib.csdn.net/base/operatingsystem)中（OS）相关方面的知识介绍一下原理。
...
/* 这里省略去 N 多代码 ------>>  */  
 ...
 /** 
  ...
  和 [Java ](http://lib.csdn.net/base/java)NIO 的内存映射无关的部分说了这么多篇幅，主要是为了做个铺垫，这样才能建立起一个知识体系，以便更好的理解内存映射文件的优点。
  内存映射文件和之前说的 标准IO操作最大的不同之处就在于它虽然最终也是要从磁盘读取数据，但是它并不需要将数据读取到OS内核缓冲区，而是直接将进程的用户私有地址空间中的一 部分区域与文件对象建立起映射关系，就好像直接从内存中读、写文件一样，速度当然快了。为了说清楚这个，我们以[Linux](http://lib.csdn.net/base/linux)操作系统为例子，看下图：
...
```
这里创建了一个只读模式的内存映射文件区域，接下来我就来[测试](http://lib.csdn.net/base/softwaretest)下与普通NIO中的通道操作相比性能上的优势，先看如下代码：
```
public class IOTest {
...
[Full GC 2481K->2435K(61312K), 0.0102255 secs]
   我们看到这里执行 GC的次数较少，但是触发了 两次 Full GC，原因在于直接内存不受 GC(新生代的Minor GC)影响，只有当执行老年代的 Full GC时候才会顺便回收直接内存！而直接内存是通过存储在JVM堆中的DirectByteBuffer对象来引用的，所以当众多的 DirectByteBuffer对象从新生代被送入老年代后才触发了 full gc。
