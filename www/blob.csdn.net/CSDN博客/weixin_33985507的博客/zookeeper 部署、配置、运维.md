# zookeeper 部署、配置、运维 - weixin_33985507的博客 - CSDN博客
2018年12月08日 15:54:00[weixin_33985507](https://me.csdn.net/weixin_33985507)阅读数：14
查看状态：echo stat|nc localhost 2181
查看配置文件：echo conf|nc localhost 2181
查看而客户端链接信息：echo cons|nc localhost 2181
查看watcher信息：echo wchs|nc localhost 2181
查看运行时信息：echo mntr|nc localhost 2181
本文以ZooKeeper3.4.3版本的官方指南为基础：[http://zookeeper.apache.org/doc/r3.4.3/zookeeperAdmin.html](http://zookeeper.apache.org/doc/r3.4.3/zookeeperAdmin.html)，补充一些作者运维实践中的要点，围绕ZK的部署和运维两个方面讲一些管理员需要知道的东西。本文并非一个ZK搭建的快速入门，关于这方面，可以查看《ZooKeeper快速搭建》。
1.部署
...
- 
安装JDK。相关链接：[http://java.sun.com/javase/downloads/index.jsp](http://java.sun.com/javase/downloads/index.jsp)
- 
...
- 
下载安装ZooKeeper，相关链接：[http://zookeeper.apache.org/releases.html](http://zookeeper.apache.org/releases.html)
- 
...
- 
至此，配置文件基本ok，可以尝试使用如下命令来启动zookeeper了：
...
注意，不同的ZK版本，依赖的log4j和slf4j版本也是不一样的，请看清楚自己的版本后，再执行上面这个命令。QuorumPeerMain类会启动ZooKeeper Server，同时，JMX MB也会被启动，方便管理员在JMX管理控制台上进行ZK的控制。这里有对ZK JMX的详细介绍：[http://zookeeper.apache.org/doc/r3.4.3/zookeeperJMX.html](http://zookeeper.apache.org/doc/r3.4.3/zookeeperJMX.html).  另外，完全可以有更简便的方式，直接使用%ZK_HOME%/bin 中的脚本启动即可。
./zkServer.sh start
- 连接ZK host来检验部署是否成功。
...
Node count: 4
1.3单机模式的配置
如果你想安装一个ZooKeeper来进行开发测试，通常可以使用单机模式来启动ZK。大体的步骤和上面说的是一样了，除了配置文件会更加简单一些。详细的配置方法可以查看这里：[http://zookeeper.apache.org/doc/r3.4.3/zookeeperStarted.html#sc_InstallingSingleMode](http://zookeeper.apache.org/doc/r3.4.3/zookeeperStarted.html#sc_InstallingSingleMode)
2.运 维
...
如果确认其它机器是正常工作的（这里要说明下，所谓正常工作还是指集群中有过半机器可用），那么可以开始删除本机的一些数据了，删除dataLogDir/version-2 两个目录下的所有文件。
...
0x1389e65c7f0c37c
0x1389e65c7f0c364
注意，wchc和wchp这两个命令执行的输出结果都是针对session的，对于运维人员来说可视化效果并不理想，可以尝试将cons命令执行输出的信息整合起来，就可以用客户端IP来代替会话ID了，具体可以看这个实现：[http://rdc.taobao.com/team/jm/archives/1450](http://rdc.taobao.com/team/jm/archives/1450)
...
作者：零度anngle
来源：CSDN
原文：[https://blog.csdn.net/zmx729618/article/details/55253075](https://blog.csdn.net/zmx729618/article/details/55253075)
