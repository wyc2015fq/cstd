# Mysql性能优化 - 三少GG - CSDN博客
2013年07月27日 21:36:42[三少GG](https://me.csdn.net/scut1135)阅读数：835
个人分类：[Web网站设计](https://blog.csdn.net/scut1135/article/category/762263)
MySQL性能优化的最佳20+条经验
[http://database.ctocio.com.cn/460/12182460.shtml](http://database.ctocio.com.cn/460/12182460.shtml)
...
![MySQL性能优化的最佳20+条经验](http://img.zdnet.com.cn/1/737/li4LchX0Qei0o.png)
　　当我们为 group_id 字段加上索引后：
![MySQL性能优化的最佳20+条经验](http://img.zdnet.com.cn/1/738/liDK2jjfrOT02.png)
...
![MySQL性能优化的最佳20+条经验](http://img.zdnet.com.cn/1/739/liux3QA7HVic.png)
...
![MySQL性能优化的最佳20+条经验](http://img.zdnet.com.cn/1/740/liKYKdnkMQK2E.png)
...
　　14 $stmt->fetch(); 
　　15 
　　16 printf("%s is from %s ", $username, $state); 
　　17 
　　18 $stmt->close(); 
　　19 }
**　13. 无缓冲的查询**
...
　　“mysql_unbuffered_query() sends the SQL query query to MySQL without automatically fetching and buffering the result rows as mysql_query() does. This saves a considerable amount of memory with SQL queries that produce large result sets, and you can start
 working on the result set immediately after the first row has been retrieved as you don’t have to wait until the complete SQL query has been performed.”
...
那么可以这样分库。
![](http://static.7ta.cn/7ta/UserFiles/alex/images/1_thumb.png)
问题：后来论坛数据猛增，访问压力开始变大。
解决：把存放论坛数据的数据库进行分库。
![](http://static.7ta.cn/7ta/UserFiles/alex/images/2_thumb.png)
问题：后来论坛帖子数量继续增长。访问吃力
解决：继续将存放帖子的数据库进行拆分。
2，水平拆分
把帖子数据拆分成12个数据库，把每3个数据存放到一台数据库服务器中。
![](http://static.7ta.cn/7ta/UserFiles/alex/images/4_thumb.png)
问题：单个数据库db1中单表数据量过大。
解决：db1中进行分表。
![](http://static.7ta.cn/7ta/UserFiles/alex/images/5_thumb.png)
概念：
垂直拆分：垂直才分也叫行拆分（Row Splitting），就是把组成一行的多个列分开放到不同的数据库（数据表）中，这些表具有不同的结构。垂直分表就是“业务拆分”。
水平拆分：水平拆分就是把一个表分成多个表，每个表的结构一样，但是储存的数据不一样。
二，读写分离
看看上面的例子中最后的结果。
![](http://static.7ta.cn/7ta/UserFiles/alex/images/6_thumb.png)
下面对存放帖子的数据库服务器Server2_1_1进行读写分离。
![](http://static.7ta.cn/7ta/UserFiles/alex/images/7_thumb.png)
...
![](http://static.7ta.cn/7ta/UserFiles/alex/images/8_thumb.png)
...
本文地址：http://www.qindamoni.com/1275
[大型项目MySQL性能优化实例](http://wenku.baidu.com/view/45214e84bceb19e8b8f6baff.html)
