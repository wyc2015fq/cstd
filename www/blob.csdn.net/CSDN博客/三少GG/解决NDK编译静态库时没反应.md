# 解决NDK编译静态库时没反应 - 三少GG - CSDN博客
2012年07月26日 14:24:05[三少GG](https://me.csdn.net/scut1135)阅读数：2502

### 居然是windows大小写不敏感的头文件，在linux下敏感造成include问题。。。还误以为是LOCAL_C_INCLUDES += $(LOCAL_PATH)/inc 的错误，吭爹啊！！！
其余都是些mfc变量的去除问题～～～
### 【转】解决NDK编译静态库时没反应
2011-05-03 17:24:39|  分类：[Android](http://icemaple810.blog.163.com/blog/#m=0&t=1&c=fks_084067081084085075081082086095085082083066080085087)|  标签：[ndk](http://icemaple810.blog.163.com/blog/#m=0&t=3&c=ndk)[编译](http://icemaple810.blog.163.com/blog/#m=0&t=3&c=%E7%BC%96%E8%AF%91)[静态](http://icemaple810.blog.163.com/blog/#m=0&t=3&c=%E9%9D%99%E6%80%81)[mk](http://icemaple810.blog.163.com/blog/#m=0&t=3&c=mk)|字号订阅
...
问题解决。
### [NDK-build 编译静态库 没有反应](http://blog.csdn.net/cazicaquw/article/details/7278265)
分类： [Android开发](http://blog.csdn.net/CAZICAQUW/article/category/810687)2012-02-21
 10:08202人阅读[评论](http://blog.csdn.net/cazicaquw/article/details/7278265#comments)(0)[收藏]()[举报](http://blog.csdn.net/cazicaquw/article/details/7278265#report)
在stackoverflow里找到了答案,在JNI目录下面，新建一个Application.mk 添加：APP_MODULES :=<module_name_1><module_name_2>...<module_name_n>
原文：
...
**[Android](http://www.linuxidc.com/topicnews.aspx?tid=11).mk内的变量定义**
3.1  LOCAL_PATH
一个[Android](http://www.linuxidc.com/topicnews.aspx?tid=11).mk file首先必须定义好LOCAL_PATH变量。它用于在开发树中查找源文件。例如:
- LOCAL_PATH:= $(call my-dir)  
宏函数’my-dir’, 由编译系统提供，用于返回当前路径（即包含[Android](http://www.linuxidc.com/topicnews.aspx?tid=11).mk file文件的目录）
...
apk文件名
4 include
include可[Android](http://www.linuxidc.com/topicnews.aspx?tid=11)多以这样的形式出现,如:include $( CLEAR_VARS),include $(BUILD_SHARED_LIBRARY).其实这个include可以理解成"执行"的意思，那么执行什么呢?当然是看后边的宏了.
...
``
**[Android.mk文件语法规范](http://www.cppblog.com/tdweng/articles/147176.html)**
...
LOCAL_MODULE := helloworld
**LOCAL_MODULE**变量必须定义，以标识你在Android.mk文件中描述的每个模块。名称必须是唯一的，而且不包含任何空格。注意编译系统会自动产生合适的前缀和后缀，换句话说，一个被命名为'foo'的共享库模块，将会生成'libfoo.so'文件。
重要注意事项
如果你把库命名为‘libhelloworld’，编译系统将不会添加任何的lib前缀，也会生成libhelloworld.so，这是为了支持来源于Android平台的源代码的Android.mk文件，如果你确实需要这么做的话。
LOCAL_SRC_FILES := helloworld.c
**LOCAL_SRC_FILES**变量必须包含将要编译打包进模块中的C或C++源代码文件。注意，你不用在这里列出头文件和包含文件，因为编译系统将会自动为你找出依赖型的文件；仅仅列出直接传递给编译器的源代码文件就好。【注意，默认的C++源码文件的扩展名是’.cpp’. 指定一个不同的扩展名也是可能的，只要定义LOCAL_DEFAULT_CPP_EXTENSION变量，不要忘记开始的小圆点（也就是定义为‘.cxx’,而不是‘cxx’）（当然这一步我们一般不会去改它）】
include $(BUILD_SHARED_LIBRARY)
**BUILD_SHARED_LIBRARY**是编译系统提供的变量，指向一个GNU Makefile脚本（应该就是在build/core目录下的shared_library.mk），负责收集自从上次调用'include $(CLEAR_VARS)'以来，定义在LOCAL_XXX变量中的所有信息，并且决定编译什么，如何正确地去做。并根据其规则生成静态库。同理对于静态库。
/****************************************************************************/
