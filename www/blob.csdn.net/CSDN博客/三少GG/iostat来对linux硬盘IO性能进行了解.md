# iostat来对linux硬盘IO性能进行了解 - 三少GG - CSDN博客
2014年09月23日 17:12:28[三少GG](https://me.csdn.net/scut1135)阅读数：733
个人分类：[重构C/C++](https://blog.csdn.net/scut1135/article/category/621651)

[iostat](http://www.wenzizone.cn/?tag=iostat)来对linux硬盘IO性能进行了解
...
sdb               0.00     4.00    0.00  350.00     0.00  2068.00     5.91     0.55    1.58   0.54  18.80rrqm/s:  	每秒进行 merge 的读操作数目。即 delta(rmerge)/s
wrqm/s:		每秒进行 merge 的写操作数目。即 delta(wmerge)/s
r/s:		每秒完成的读 [I/O](http://www.wenzizone.cn/?tag=io) 设备次数。即 delta(rio)/s
w/s:		每秒完成的写 [I/O](http://www.wenzizone.cn/?tag=io) 设备次数。即 delta(wio)/s
rsec/s:		每秒读扇区数。即 delta(rsect)/s
wsec/s:		每秒写扇区数。即 delta(wsect)/s
rkB/s:		每秒读K字节数。是 rsect/s 的一半，因为每扇区大小为512字节。(需要计算)
wkB/s:		每秒写K字节数。是 wsect/s 的一半。(需要计算)
avgrq-sz:	平均每次设备[I/O](http://www.wenzizone.cn/?tag=io)操作的数据大小 (扇区)。delta(rsect+wsect)/delta(rio+wio)
