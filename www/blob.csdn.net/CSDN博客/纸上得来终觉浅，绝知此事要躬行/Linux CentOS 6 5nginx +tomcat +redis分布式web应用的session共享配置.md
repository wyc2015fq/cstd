# Linux CentOS 6.5nginx +tomcat +redis分布式web应用的session共享配置 - 纸上得来终觉浅，绝知此事要躬行 - CSDN博客





2015年10月28日 18:10:03[boonya](https://me.csdn.net/boonya)阅读数：1637标签：[linux																[centos																[nginx																[redis																[session](https://so.csdn.net/so/search/s.do?q=session&t=blog)
个人分类：[Nginx																[Linux																[Memcache/Redis](https://blog.csdn.net/boonya/article/category/5904837)




原文地址：[http://www.cnblogs.com/lengfo/p/4260363.html](http://www.cnblogs.com/lengfo/p/4260363.html)
...
　　nginx 作为目前最流行的开源反向代理HTTP Server，用于实现资源缓存、web server负载均衡等功能，由于其轻量级、高性能、高可靠等特点在互联网项目中有着非常普遍的应用，相关概念网上有丰富的介绍。分布式web server集群部署后需要实现session共享，针对 tomcat 服务器的实现方案多种多样，比如 tomcat cluster session 广播、nginx IP hash策略、nginx sticky module等方案，本文主要介绍了使用
 redis 服务器进行 session 统一存储管理的共享方案。

　　相关应用结构参照下图：

![](http://images.cnitblog.com/blog/536814/201501/301356402377480.png)
...
　　3、找到源码中的 
build.gradle 文件，由于作者使用了第三方仓库（sonatype），需要注册帐号，太麻烦，注释后直接使用maven中央仓库，同时注释签名相关脚本并增加依赖包的输出脚本 copyJars（dist目录），修改后的build.gradle 文件如下：
...
![](http://images.cnitblog.com/blog/536814/201501/301049416916951.png)
...
　　3、访问 [http://10.129.221.70](http://10.129.221.70:8080/) （默认80端口）请求到 nginx 反向代理到指定Web服务器，由于默认使用轮询负载方式，
...
![](http://images.cnitblog.com/blog/536814/201501/301919135032547.png)



# 九、至此实现了基于nginx负载均衡下 tomcat 集群的 session 一致性。](https://blog.csdn.net/boonya/article/category/1740759)](https://blog.csdn.net/boonya/article/category/2126311)](https://so.csdn.net/so/search/s.do?q=redis&t=blog)](https://so.csdn.net/so/search/s.do?q=nginx&t=blog)](https://so.csdn.net/so/search/s.do?q=centos&t=blog)](https://so.csdn.net/so/search/s.do?q=linux&t=blog)
