# Mina Executor Filter执行线程模型过滤器 - 纸上得来终觉浅，绝知此事要躬行 - CSDN博客





2016年06月07日 13:15:01[boonya](https://me.csdn.net/boonya)阅读数：3158
...
原文地址：[Mina ExecutorFilter(Apache Mina user guide Chapter10 Executor Filter)](http://mina.apache.org/mina-project/userguide/ch10-executor-filter/ch10-executor-filter.html)


Mina1.X版本让用户在Acceptor级别定义线程模型。这是Acceptor配置的一部分。这导致了复杂性和Mina团队决定删除这个选项,代之以一个更通用的系统,基于一个过滤器:ExecutorFilter。

### ExecutorFilter类

这个类实现IoFilter接口。基本上,它包含一个Executor来将事件传递给线程池。如果一些CPU执行密集型任务，这将允许应用程序使用更高效的处理器。




这个过滤器可以在处理程序使用之前。假设大多数的处理将在应用程序中完成,或在某个CPU密集型过滤器(例如,一个CodecFilter)。更多...

下面附上ExecutorFilter源码：
