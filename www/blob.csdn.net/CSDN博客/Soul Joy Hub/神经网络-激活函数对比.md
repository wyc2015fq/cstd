# 神经网络-激活函数对比 - Soul Joy Hub - CSDN博客

2017年08月30日 11:02:01[卓寿杰_SoulJoy](https://me.csdn.net/u011239443)阅读数：450标签：[神经网络																[深度学习																[激活函数](https://so.csdn.net/so/search/s.do?q=激活函数&t=blog)](https://so.csdn.net/so/search/s.do?q=深度学习&t=blog)](https://so.csdn.net/so/search/s.do?q=神经网络&t=blog)
...
如需转载，请附上本文链接：[http://blog.csdn.net/cyh_24/article/details/50593400](http://blog.csdn.net/cyh_24/article/details/50593400)
...
产生的一个结果就是：如果数据进入神经元的时候是正的(e.g.  计算出的梯度也会始终都是正的。 
...
很显然，从图左可以看出，输入信号 是二维的情况下，使用ReLU之后的效果如下：
...
> 
核心思想就是，在训练过程中， 中 随机出来的，然后再测试过程中进行修正（有点像dropout的用法）。

数学表示如下：

![此处输入图片的描述](http://7pn4yt.com1.z0.glb.clouddn.com/blog-rrelu.png)

在测试阶段，把训练过程中所有的  中随机出来的。那么，在测试阶段，激活函数就是就是： 
...
所以，Maxout 具有 ReLU 的**优点**（如：计算简单，不会 saturation），同时又没有 ReLU 的一些缺点 （如：容易 go die）。不过呢，还是有一些**缺点**的嘛：就是把参数double了。
