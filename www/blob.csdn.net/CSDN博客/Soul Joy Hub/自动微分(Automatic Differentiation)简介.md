# 自动微分(Automatic Differentiation)简介 - Soul Joy Hub - CSDN博客

2018年01月16日 14:56:53[卓寿杰_SoulJoy](https://me.csdn.net/u011239443)阅读数：375
个人分类：[深度学习																[机器学习																[TensorFlow](https://blog.csdn.net/u011239443/article/category/6871932)](https://blog.csdn.net/u011239443/article/category/6268728)](https://blog.csdn.net/u011239443/article/category/6990149)
...
\\\frac{d }{dx}f(x)g(x) = \big(\frac{d }{dx}f(x)\big)g(x)+f(x)\big(\frac{d }{dx}g(x)\big)
...
上表中左半部分是从左往右每个图节点的求值结果，右半部分是每个节点对于，注意到每一步的求导都利用到上一步的求导结果，这样不至于重复计算，因此也不会产生像符号微分法的”expression swell”问题。 
...
这样一个有遍如上计算过程。然而实际算法模型中，比如神经网络，通常输入输出是极其不成比例的，也就是： 
...
上表左边和之前的forward mode一致，用于求解函数值，右边则是reverse mode的计算过程，注意必须从下网上看，也就是一开始先计算输出，这样的记号可以强调我们对当前计算结果进行缓存，以便用于后续计算，而不必重复计算。由链式法则我们可以计算输出对于每个节点的导数。 

比如对于节点: 

\frac{dy}{dv_3} = \frac{dy}{dv_5}\frac{dv_5}{dv_3}
 

用另一种记法变得到： 

\frac{dy}{dv_3} = \overline{v_5}\frac{dv_5}{dv_3}
...
\frac{dy}{dv_0} = \overline v_2 \frac{dv_2}{dv_0}+\overline v_3 \frac{d v_3}{d v_0}
...
那么要求的导数，那么根据链式法则可得： 
    ...
    \frac{\partial y}{\partial x_1} = \frac{\partial y}{\partial f}\frac{\partial f}{\partial x_1}  =  \frac{\partial y}{\partial x_1x_2}\frac{\partial x_1x_2}{\partial x_1}=output\_grad * x_2
 

则output_grad就是上面的类似。因此在程序中我们会返回如下：
...
此时我们要计算输出的导数，我们需要将其发射边关联节点的计算梯度搜集起来，比如上面的例子，我们需要搜集： 

node\_to\_output\_grads\_list = \{\\
    n1:\big [\frac{\partial y}{\partial n_3}\frac{\partial n_3}{\partial n_1},\frac{\partial y}{\partial n_4}\frac{\partial n_4}{\partial n_1}\big] \\\}
 

一旦搜集好对应输出边节点关于当前节点导数，那么当前节点的导数便可以由链式法则计算得出，也就是： 

    \frac{\partial y}{\partial n_1}=\frac{\partial y}{\partial n_3}\frac{\partial n_3}{\partial n_1}+\frac{\partial y}{\partial n_4}\frac{\partial n_4}{\partial n_1}
...
- [CSE599G1: Deep Learning System](http://dlsys.cs.washington.edu/schedule)
...
阅读全文
        ...
        版权声明：欢迎转载，原文地址[http://blog.csdn.net/aws3217150](http://blog.csdn.net/aws3217150)
