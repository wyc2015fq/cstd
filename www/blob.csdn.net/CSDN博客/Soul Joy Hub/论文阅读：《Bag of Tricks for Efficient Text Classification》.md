# 论文阅读：《Bag of Tricks for Efficient Text Classification》 - Soul Joy Hub - CSDN博客

2018年04月25日 11:22:29[卓寿杰_SoulJoy](https://me.csdn.net/u011239443)阅读数：703


[https://blog.csdn.net/u011239443/article/details/80076720](https://blog.csdn.net/u011239443/article/details/80076720)

论文地址：[https://arxiv.org/pdf/1607.01759v2.pdf](https://arxiv.org/pdf/1607.01759v2.pdf)
...
![](https://upload-images.jianshu.io/upload_images/1621805-a17482beaee6f03b.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
当目标数量很大时，计算线性分类器的计算量很大。 更准确地说，计算复杂度为$O（Kd）$，其中K是目标的数量，d是隐藏层的维数。 为了改善我们的运行时间，我们使用基于霍夫曼编码树的分层softmax。 在训练期间，计算复杂度降至$O（d log2（K））$。 在这棵树上，目标是树叶。

当搜索最可能的类别时，分层softmax在测试时间也是有利的。 每个节点都与从根节点到该节点的路径概率相关联。 如果节点与父节点n1，…，nl处于深度l + 1，则其概率为

![](https://upload-images.jianshu.io/upload_images/1621805-794bd4b9fddb0126.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

**这意味着节点的概率总是低于其父节点的概率。 通过深度遍历探索树并跟踪叶子之间的最大概率允许我们丢弃与较小概率相关的任何分支。** 在实践中，我们观察到在测试时$O（dlog2（K））$的复杂度降低。 这种方法进一步扩展到以$O（log（T））$为代价，使用二进制堆计算T-top目标。
...
![](https://upload-images.jianshu.io/upload_images/1621805-5bee4d6730d70c15.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

> 
表1：情绪数据集的测试准确度[％]。 所有数据集都使用相同的参数运行FastText。 它有10个隐藏的单位，我们评估它有没有bigrams。 对于VDCNN和char-CNN，我们显示没有数据增加的最佳报告数字。

![](https://upload-images.jianshu.io/upload_images/1621805-1bd0aa38890c0018.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
![](https://upload-images.jianshu.io/upload_images/1621805-67885b4cfb570c0c.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
**数据集和基线**为了测试我们方法的可伸缩性，对YFCC100M数据集进行了进一步评估，该数据集由几乎100M的带有字幕，标题和标签的图像组成。我们专注于根据标题和标题预测标签（我们不使用图像）。我们删除少于100次的字词和标签，并将数据分成训练，验证和测试集。该训练集包含91,188,648个样本。验证有930,497个样本和测试集543,424个样本。词汇大小为297,141，并且有312,116个标签。我们将发布一个脚本来重新创建这个数据集，以便我们的数据可以被复制。
...
![](https://upload-images.jianshu.io/upload_images/1621805-e7ba2165092da28f.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
表4显示了一些定性的例子。 
![](https://upload-images.jianshu.io/upload_images/1621805-72cf01300e31840b.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
