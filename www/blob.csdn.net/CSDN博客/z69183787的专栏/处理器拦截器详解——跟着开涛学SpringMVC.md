# 处理器拦截器详解——跟着开涛学SpringMVC - z69183787的专栏 - CSDN博客
2018年01月30日 11:39:06[OkidoGreen](https://me.csdn.net/z69183787)阅读数：133
个人分类：[SpringMvc-拦截器 & 静态资源处理](https://blog.csdn.net/z69183787/article/category/6694500)
## 5.1、处理器拦截器简介
...
**afterCompletion：**整个请求处理完毕回调方法，即在视图渲染完毕时回调，如性能监控中我们可以在此记录结束时间并输出消耗时间，还可以进行一些资源清理，类似于try-catch-finally中的finally，但仅调用处理器执行链中**preHandle返回true的拦截器的afterCompletion**。
` `
### 5.1.3、拦截器适配器
...
java代码：
Java代码  ![收藏代码](http://jinnianshilongnian.iteye.com/images/icon_star.png)
- publicabstractclass HandlerInterceptorAdapter implements HandlerInterceptor {  
- //省略代码 此处所以三个回调方法都是空实现，preHandle返回true。
- }  
### 5.1.4、运行流程图
![](http://sishuok.com/forum/upload/2012/9/4/1217fc1b3d71239c0267e1b9bfeec893__1.JPG)
图5-1 正常流程
![](http://sishuok.com/forum/upload/2012/9/4/37fa452b7328bc18e674ea8d12ddafd9__2.JPG)
图5-2 中断流程
...
- privatevoid triggerAfterCompletion(HandlerExecutionChain mappedHandler, int interceptorIndex,  
...
- //省略import
- publicclass HandlerInterceptor1 extends HandlerInterceptorAdapter {//此处一般继承HandlerInterceptorAdapter适配器即可
- @Override
...
在控制台输出 test.jsp
**（5、启动服务器测试**
输入网址：[http://localhost:9080/springmvc-chapter5/test](http://localhost:9080/springmvc-chapter5/test)
控制台输出：
java代码：
Java代码  ![收藏代码](http://jinnianshilongnian.iteye.com/images/icon_star.png)
...
Java代码  ![收藏代码](http://jinnianshilongnian.iteye.com/images/icon_star.png)
- package cn.javass.chapter5.web.interceptor;  
- publicclass StopWatchHandlerInterceptor extends HandlerInterceptorAdapter {  
