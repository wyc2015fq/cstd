# 解析 Nginx 负载均衡 - z69183787的专栏 - CSDN博客
2015年08月28日 11:20:47[OkidoGreen](https://me.csdn.net/z69183787)阅读数：628
...
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing1.jpg)
...
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing2.jpg)
从变量命名中，我们就可以大致猜出其作用。其中，current_weight和weight的区别主要是前者为权重排序的值，随着处理请求会动态的变化，后者是配置值，用于恢复初始状态。
接下来看下轮询的创建过程，代码如下图所示。
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing3.jpg)
...
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing4.jpg)
最后是实际的策略代码，逻辑很简单，代码实现也只有30行，直接上代码。
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing5.jpg)
2.2.           ip hash
ip hash是nginx内置的另一个负载均衡的策略，流程和轮询很类似，只是其中的算法和具体的策略有些变化，如下图所示：
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing6.jpg)
ip hash算法的核心实现如下图：
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing7.jpg)
...
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing8.jpg)
...
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing9.jpg)
3.3.           测试方案
首先介绍下关键的测试指标：
均衡性：是否能够将请求均匀的发送给后端
...
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing10.jpg)
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing11.jpg)
...
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing12.jpg)
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing13.jpg)
...
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing14.jpg)
图5为ip hash策略，ip hash是nginx内置策略，可以看做是前两种策略的特例：以来源ip为key。由于测试工具不便于模拟海量ip下的请求，因此这里截取线上实际的情况加以分析，如下图所示：
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing16.jpg)[](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing15.jpg)
图5 ip hash策略
...
![解析 Nginx 负载均衡](http://jbcdn2.b0.upaiyun.com/2012/07/Parse-nginx-load-balancing17.jpg)
