# webmagic的设计机制及原理-如何开发一个Java爬虫 - z69183787的专栏 - CSDN博客
2015年06月25日 11:57:12[OkidoGreen](https://me.csdn.net/z69183787)阅读数：3449
目录[-]
[webmagic的目标](http://my.oschina.net/flashsword/blog/145796#OSC_h2_1)[webmagic的模块划分](http://my.oschina.net/flashsword/blog/145796#OSC_h2_2)[Spider类-核心调度](http://my.oschina.net/flashsword/blog/145796#OSC_h3_3)[Downloader-页面下载](http://my.oschina.net/flashsword/blog/145796#OSC_h3_4)[PageProcessor-页面分析及链接抽取](http://my.oschina.net/flashsword/blog/145796#OSC_h3_5)[Jsoup](http://my.oschina.net/flashsword/blog/145796#OSC_h4_6)[HtmlParser](http://my.oschina.net/flashsword/blog/145796#OSC_h4_7)[Apache tika](http://my.oschina.net/flashsword/blog/145796#OSC_h4_8)[HtmlCleaner与XPath](http://my.oschina.net/flashsword/blog/145796#OSC_h4_9)[几个工具的对比](http://my.oschina.net/flashsword/blog/145796#OSC_h4_10)[webmagic的Selector](http://my.oschina.net/flashsword/blog/145796#OSC_h4_11)[Scheduler-URL管理](http://my.oschina.net/flashsword/blog/145796#OSC_h3_12)[Pipeline-离线处理和持久化](http://my.oschina.net/flashsword/blog/145796#OSC_h3_13)[结语](http://my.oschina.net/flashsword/blog/145796#OSC_h2_14)
![image](http://static.oschina.net/uploads/space/2013/1110/200709_oP1e_190591.jpg)
> 
此文章是webmagic 0.1.0版的设计手册，后续版本的入门及用户手册请看这里：[https://github.com/code4craft/webmagic/blob/master/user-manual.md](https://github.com/code4craft/webmagic/blob/master/user-manual.md)
...
不同的爬虫，对这几部分的要求是不一样的。
对于通用型的爬虫，例如搜索引擎蜘蛛，需要指对互联网大部分网页无差别进行抓取。这时候难点就在于页面下载和链接管理上–如果要高效的抓取更多页面，就必须进行更快的下载；同时随着链接数量的增多，需要考虑如果对大规模的链接进行去重和调度，就成了一个很大的问题。一般这些问题都会在大公司有专门的团队去解决，比如这里有一篇来自淘宝的[快速构建实时抓取集群](http://www.searchtb.com/2011/07/%E5%BF%AB%E9%80%9F%E6%9E%84%E5%BB%BA%E5%AE%9E%E6%97%B6%E6%8A%93%E5%8F%96%E9%9B%86%E7%BE%A4.html?spm=0.0.0.0.hHzGxv)。对Java来说，如果你要研究通用爬虫，那么可以看一下[heritrix](http://www.oschina.net/p/heritrix)或者[nutch](http://www.oschina.net/p/nutch)。
...
![image](http://my.oschina.net/action/blog/img_proxy?url=http%3A%2F%2Fcode4craft.github.io%2Fimages%2Fposts%2Fwebmagic.png)
### Spider类-核心调度
...
webmagic使用了HttpClient 4.2，并封装到了HttpClientDownloader。学习HttpClient的使用对于构建高性能爬虫是非常有帮助的，官方的[Tutorial](http://hc.apache.org/httpcomponents-client-ga/tutorial/html/)就是很好的学习资料。目前webmagic对HttpClient的使用仍在初步阶段，不过对于一般抓取任务，已经够用了。
...
里的内容，可以这样写：“
(.*?)
“。但是如果这个div内部还包含几个子div，这个时候使用正则表达式就会将子div的”
“作为终止符截取。为了解决这个问题，我们就需要进行HTML的分析。
...
#### HtmlParser
HtmlParser的功能比较完备，也挺灵活，但谈不上方便。这个项目很久没有维护了，最新版本是2.1。HtmlParser的核心元素是Node，对应一个HTML标签，支持getChildren()等树状遍历方式。HtmlParser另外一个核心元素是NodeFilter，通过实现NodeFilter接口，可以对页面元素进行筛选。这里有一篇HtmlParser的使用文章：[使用
 HttpClient 和 HtmlParser 实现简易爬虫](https://www.ibm.com/developerworks/cn/opensource/os-cn-crawler/)。
#### Apache tika
...
最后再次附上代码地址：[https://github.com/code4craft/webmagic](https://github.com/code4craft/webmagic)
