# java grpc简单例子 - z69183787的专栏 - CSDN博客
2018年09月09日 21:24:56[OkidoGreen](https://me.csdn.net/z69183787)阅读数：329
用eclipse新建一个maven项目，Id信息如下
```
<groupId>com.mingluck.test</groupId>
<artifactId>grpc</artifactId>
```
创建如下的目录结构（包括文件） 
![grpc](https://img-blog.csdn.net/20161115232315753)
看下面的test.proto的内容：
```
syntax = "proto3";
package grpc;
...
```
然后在cmd窗口中切换到项目pom.xml文件所在的目录，运行mvn compile命令，运行命令后目录结构会变成如下 
![grpc](https://img-blog.csdn.net/20161115234030388)
接下来可以写代码了，先实现服务端程序，HelloWorldServer.java
```
...
```
分别运行服务端和客户端，会看到rpc调用成功。 
为了防止客户端不断调用带来的开销(短连接)，下面的例子给出了连接池的方式，代码如下
```
package com.mingluck.grpc;
