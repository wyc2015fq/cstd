# React 组件之间如何交流 - z69183787的专栏 - CSDN博客
2016年08月18日 15:22:50[OkidoGreen](https://me.csdn.net/z69183787)阅读数：2100
### 前言
...
```
```
![](http://img2.tuicool.com/v6vMFf.gif)
我觉得原文作者用代码不是很直观，接下来我话一个流程走向简图来直观描述一下这个过程：
![](http://img0.tuicool.com/6zUJVra.png!web)
...
- 【this】：指向你的组件
- 【一个参数】：这个参数是一个 [React
 合成事件](http://wiki.jikexueyuan.com/project/react/event-system.html) ，SyntheticEvent。
...
```
```
这里有所有 React 支持的事件： [中文文档-事件系统](http://reactjs.cn/react/docs/events.html)
多个子组件使用同一个回调的情况
```java
```
// 父组件
...
```
```
![](http://img2.tuicool.com/AFFzEr2.gif)
...
下面讲介绍 3 种模式来处理事件，你能 [点击这里](https://github.com/millermedeiros/js-signals/wiki/Comparison-between-different-Observer-Pattern-implementations) 来比较一下它们。
简单总结一下：
...
React 团队使用的是： [js-signals](http://millermedeiros.github.io/js-signals/) 它基于
 Signals 模式，用起来相当不错。
### Events in React
...
```
```
![](http://img2.tuicool.com/3IVJvuQ.gif)
### ES6: yield and js-csp
ES6 中有一种传递信息的方式，使用生成函数(generators)和 yield 关键字。可以看一下 [https://github.com/ubolonton/js-csp.](https://github.com/ubolonton/js-csp.)
...
```
以上例子来自于屈屈的一篇博客： [ES6
 中的生成器函数介绍](http://imququ.com/post/generator-function-in-es6.html) 屈屈是一个大牛，大家可以经常关注他的博客。
