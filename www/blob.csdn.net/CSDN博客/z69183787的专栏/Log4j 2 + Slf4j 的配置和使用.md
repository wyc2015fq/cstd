# Log4j 2 + Slf4j 的配置和使用 - z69183787的专栏 - CSDN博客
2017年07月10日 14:03:55[OkidoGreen](https://me.csdn.net/z69183787)阅读数：436
...
![这里写图片描述](https://img-blog.csdn.net/20150325191622288)
2、代码中使用：
```java
...
级别：ERROR
3、配置（使用xml配置文件）：
官网配置指南：[http://logging.apache.org/log4j/2.x/manual/configuration.html](http://logging.apache.org/log4j/2.x/manual/configuration.html)
...
additivity指定是否同时输出log到父类的appender，缺省为true。（层级关系与[Java](http://lib.csdn.net/base/java)包类似，例如：com.foo是com.foo.Bar的父级；java是java.util的父级，是java.util.vector的祖先。root是最顶层。） 
...
定义log的格式（一种类似[C语言](http://lib.csdn.net/base/c)printf函数的打印格式）。
配置例子和注解如下：
```xml
...
- 10
- 11
Tips：根据官方[测试](http://lib.csdn.net/base/softwaretest)的数据，第一种用法比第二种快47倍！
