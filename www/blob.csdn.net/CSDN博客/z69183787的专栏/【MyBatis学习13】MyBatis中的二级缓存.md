# 【MyBatis学习13】MyBatis中的二级缓存 - z69183787的专栏 - CSDN博客
2017年07月28日 11:23:36[OkidoGreen](https://me.csdn.net/z69183787)阅读数：273
个人分类：[Mybatis-学习](https://blog.csdn.net/z69183787/article/category/7026142)
[http://blog.csdn.net/eson_15/article/details/51669608](http://blog.csdn.net/eson_15/article/details/51669608)
## 1. 二级缓存的原理
...
　　到这里，就明白了mybatis中二级缓存的执行原理了，这个跟[hibernate](http://lib.csdn.net/base/javaee)还是有点像的。也可以对照着我写的[hibernate的二级缓存](http://blog.csdn.net/eson_15/article/details/51405911)的博文看（不过建议把下面的ehcache部分看完再对照hibernate看）。
...
```
- 
　　这种情况是针对每次查询都需要最新的数据sql，要设置成useCache=false，禁用二级缓存，直接从[数据库](http://lib.csdn.net/base/mysql)中获取。 
...
### 3.2 整合方法
　　上文一开始提到过，mybatis提供了一个cache接口，如果要实现自己的缓存逻辑，实现cache接口开发即可。mybatis本身默认实现了一个，但是这个缓存的实现无法实现分布式缓存，所以我们要自己来实现。ehcache分布式缓存就可以，mybatis提供了一个针对cache接口的ehcache实现类，这个类在mybatis和ehcache的整合包中。所以首先我们需要导入整合包（[点我下载](http://download.csdn.net/detail/eson_15/9549252)）。 
