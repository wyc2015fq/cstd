# ZooKeeper 数据模型 - z69183787的专栏 - CSDN博客
2016年09月07日 17:53:05[OkidoGreen](https://me.csdn.net/z69183787)阅读数：305
个人分类：[分布式-Zookeeper](https://blog.csdn.net/z69183787/article/category/5568407)
[转载请注明作者和[原文链接](http://coolxing.iteye.com/blog/1871328),  如有谬误, 欢迎在评论中指正. ] 
ZooKeeper的数据结构, 与普通的文件系统极为类似. 见下图:
![](http://dl2.iteye.com/upload/attachment/0084/6518/7cc4fff6-d057-356b-876d-ad2bb7a48ddf.jpg)
图片引用自[developerworks](http://www.ibm.com/developerworks/cn/opensource/os-cn-zookeeper/)
图中的每个节点称为一个znode. 每个znode由3部分组成:
- stat. 此为状态信息, 描述该znode的版本, 权限等信息.
- data. 与该znode关联的数据.
- children. 该znode下的子节点.
### [](https://github.com/xing4git/blog/blob/master/zookeeper/ZooKeeper--%E6%95%B0%E6%8D%AE%E6%A8%A1%E5%9E%8B.md#zookeeper)ZooKeeper命令
在深入znode的各个部分之前, 首先需要熟悉一些常用的ZooKeeper命令.
连接server
Bash代码  ![收藏代码](http://coolxing.iteye.com/images/icon_star.png)
...
- world  
- cZxid = 0x10000042c
- ctime = Fri May 1717:57:33 CST 2013
- mZxid = 0x10000042c
- mtime = Fri May 1717:57:33 CST 2013
- pZxid = 0x10000042c
- cversion = 0
- dataVersion = 0
- aclVersion = 0
...
### [](https://github.com/xing4git/blog/blob/master/zookeeper/ZooKeeper--%E6%95%B0%E6%8D%AE%E6%A8%A1%E5%9E%8B.md#znode)znode节点的状态信息
...
ZooKeeper状态的每一次改变, 都对应着一个递增的`Transaction id`,
...
`persistent`. persistent节点不和特定的session绑定,
 不会随着创建该节点的session的结束而消失, 而是一直存在, 除非该节点被显式删除.
`ephemeral`. ephemeral节点是临时性的, 如果创建该节点的session结束了,
...
- Created /xing/ei  
`sequence`. 严格的说, sequence并非节点类型中的一种. sequence节点既可以是ephemeral的,
 也可以是persistent的. 创建sequence节点时, ZooKeeper server会在指定的节点名称后加上一个数字序列, 该数字序列是递增的. 因此可以多次创建相同的sequence节点, 而得到不同的节点. 使用-s参数指定创建sequence节点.
Bash代码  ![收藏代码](http://coolxing.iteye.com/images/icon_star.png)
...
- world  
- cZxid = 0x100000066
- ctime = Fri May 1722:30:01 CST 2013
- mZxid = 0x100000066
- mtime = Fri May 1722:30:01 CST 2013
- pZxid = 0x100000066
- cversion = 0
- dataVersion = 0
- aclVersion = 0
