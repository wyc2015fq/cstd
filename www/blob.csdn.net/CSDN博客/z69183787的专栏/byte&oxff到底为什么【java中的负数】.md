# byte&oxff到底为什么【java中的负数】 - z69183787的专栏 - CSDN博客
2017年12月03日 18:42:17[OkidoGreen](https://me.csdn.net/z69183787)阅读数：604
近日需要一个将字节数组转换为十六进制字符串输出的函数，于是开始编码如下：
```
//该代码存在问题
...
```
运行测试，一旦遇到负数总出现未可预料的结果，如参数为{-125,125，115,89}时运行结果为FFFFFF837D7359。 
开始从网上查资料做修改，对byte数据进行与运算&oxff再进行转换，很抓狂到底为什么，再一顿搜索，原来问题出在java整数表示方法及位扩张问题上，在java中使用补码对整数进行表示。修改后代码如下：
```
//正确代码
public static String byteToHex(byte[] bt){
        StringBuffer sb = new StringBuffer();
...
```
### 对于此问题找到了比较全面的解释如下：
转载自[http://my.oschina.net/joymufeng/blog/139952](http://my.oschina.net/joymufeng/blog/139952)
下面这行代码的输出是什么？
```
