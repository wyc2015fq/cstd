# MySQL 大表优化方案 - 纯洁的微笑 - CSDN博客
2018年08月08日 08:08:00[微笑很纯洁](https://me.csdn.net/ityouknow)阅读数：231
![640?wx_fmt=jpeg](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_jpg/PgqYrEEtEnrfiaooibIvDrFCqJ5kdGib5f8Y1WeT9AibarYTqAMCWysKvgyJmqwibdAibrb98PaXhFOublWzgFUAZnYA/640?wx_fmt=jpeg)
...
- 
对于连续数值，使用 `BETWEEN`不用 `IN`： `SELECT id FROM t WHERE num BETWEEN1AND5`
- 
...
- 
keybuffersize：索引块的缓存大小，增加会提升索引处理速度，对MyISAM表性能影响最大。对于内存4G左右，可设为256M或384M，通过查询 `show status like'key_read%'`，保证 `key_reads/key_read_requests`在0.1%以下最好
- 
innodbbufferpool_size：缓存数据块和索引块，对InnoDB表性能影响最大。通过查询 `show status like'Innodb_buffer_pool_read%'`，保证 `(Innodb_buffer_pool_read_requests–Innodb_buffer_pool_reads)/Innodb_buffer_pool_read_requests`越高越好
- 
...
- 
sortbuffersize：MySql执行排序使用的缓冲大小。如果想要增加 `ORDER BY`的速度，首先看是否可以让MySQL使用索引而不是额外的排序阶段。如果不能，可以尝试增加sortbuffersize变量的大小
- 
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/aVp1YC8UV0dgAgUGmP1DRKSVb3BmgIibBnLDxZ7lV7IO1OYbQibFttWZic3yViagTMdyscMKZOmBY1FibjOs1NYqUKQ/640?wx_fmt=png)
...
比如原始的用户表是：
![640?wx_fmt=jpeg](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_jpg/aVp1YC8UV0dgAgUGmP1DRKSVb3BmgIibBPW4FU198PlBasdDiaArgjcxYeSZm2deIiaNAnkMXiaSIKgjR6qyqxNdiag/640?wx_fmt=jpeg)
垂直拆分后是：
![640?wx_fmt=jpeg](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_jpg/aVp1YC8UV0dgAgUGmP1DRKSVb3BmgIibBuLOwXteLdwxXE4BNQibJtFmPDmtQ3hkEfQdhSZrkUcauTvmNZtHRCSA/640?wx_fmt=jpeg)
垂直拆分的优点是：
- 
...
![640?wx_fmt=jpeg](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_jpg/aVp1YC8UV0dgAgUGmP1DRKSVb3BmgIibBSQicFObXwdK9rFaR17MjPiacxl3ice1WZ8qR4DCHTD7icTVGibx530oa20A/640?wx_fmt=jpeg)
...
这是一个客户端架构的例子：
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/aVp1YC8UV0dgAgUGmP1DRKSVb3BmgIibBC7sudgbxK8N2RytrrNZF3fjKNHsiaZwMXZJHe1erdAGQqEprzWK5avQ/640?wx_fmt=png)
...
这是一个代理架构的例子：
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/aVp1YC8UV0dgAgUGmP1DRKSVb3BmgIibBL9DHWsbDu5ibjKQL8zQbI2ChdkzfZOic79AxEdEibSKUHnvyUeTnMt6cg/640?wx_fmt=png)
...
##### 各方案比较
![640?wx_fmt=jpeg](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_jpg/owvKmkcJ1XnLia5NaUxzMk52lhoAvtGCrxNrypUTqVnkfdPa53tdInJiadMf9LLgVjwYLgwmKHc5fmNx7qjF0TlQ/640?wx_fmt=jpeg)
