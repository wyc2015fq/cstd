# 正则表达式和 CPU 100%有什么故事？ - 纯洁的微笑 - CSDN博客
2018年06月29日 07:36:00[微笑很纯洁](https://me.csdn.net/ityouknow)阅读数：2471
前几天线上一个项目监控信息突然报告异常，上到机器上后查看相关资源的使用情况，发现 CPU 利用率将近 100%。通过 Java 自带的线程 Dump 工具，我们导出了出问题的堆栈信息。
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/PgqYrEEtEnrIw6icuuDGw8ck6Ts0qh5wNz5ibSfsofaXmpFWI7z5JibhSMpxHKEHyfY1wMzOneVO1EYPjW1UCNpDA/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/PgqYrEEtEnrIw6icuuDGw8ck6Ts0qh5wNzdzrY6KjqVLIj80dg8UpGjviabaBurEwNwIhBBTC5CEmlVic2EYJxYPA/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/PgqYrEEtEnrIw6icuuDGw8ck6Ts0qh5wNo0GuHDRuO5xg6icDu1vZmGSRfIg1RQoMl9byFI1ichiarq6ksNWXWkUjA/640?wx_fmt=png)
当你点击左下角的「regex debugger」时，它会告诉你一共经过多少步检查完毕，并且会将所有步骤都列出来，并标明发生回溯的位置。
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/PgqYrEEtEnrIw6icuuDGw8ck6Ts0qh5wNuLDBWTeqAERmvVVRW0tbQPtWibXcvwE0HcXtSXkqVBtzHOTWGgnSf9A/640?wx_fmt=png)
...
```
工具提示只用了 58 步就完成了检查。
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/PgqYrEEtEnrIw6icuuDGw8ck6Ts0qh5wNrFJ5U9SLh7VEFV6ibUX2ibGRYOTUibxEtbibzR4OuBB9k8BlA2Y24hECpQ/640?wx_fmt=png)
一个字符的差别，性能就差距了好几万倍。
## 树义有话说
一个小小的正则表达式竟然能够把 CPU 拖垮，也是很神奇了。这也给平时写程序的我们一个警醒，遇到正则表达式的时候要注意贪婪模式和回溯问题，否则我们每写的一个表达式都是一个雷。
通过查阅网上资料，我发现深圳阿里中心 LAZADA 的同学也在 17 年遇到了这个问题。他们同样也是在测试环境没有发现问题，但是一到线上的时候就发生了 CPU 100% 的问题，他们遇到的问题几乎跟我们的一模一样。
虽然把这篇文章写完了，但是关于 NFA 自动机的原理方面，特别是关于懒惰模式、独占模式的解释方面还是没有解释得足够深入。因为 NFA 自动机确实不是那么容易理解，所以在这方面还需要不断学习加强。欢迎有懂行的朋友来学习交流，互相促进。
> 
原文出处：[https://mp.weixin.qq.com/s/qVBzB8U_v-XmHAQUquFafw](https://mp.weixin.qq.com/s?__biz=MzA4MjIyNTY0MQ==&mid=2647738965&idx=1&sn=61706e46fc7cf175ebc17fe5472f9f95&scene=21#wechat_redirect)
END
