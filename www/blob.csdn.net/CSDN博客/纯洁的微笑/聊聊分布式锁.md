# 聊聊分布式锁 - 纯洁的微笑 - CSDN博客
2018年11月14日 09:09:00[微笑很纯洁](https://me.csdn.net/ityouknow)阅读数：197
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/WLIGprPy3z7asVtMGYjJn2w4yAOiclibO1YSbXo70dubOJW2KtxcvgQdSSfntWtXV8TiakG31EqSb1FduOYBJMoHg/640?wx_fmt=png)
前面我们所说的lock(),trylock(long timeout)，trylock()这几个方法可以用下面的伪代码实现。
## 3.1 lock()
lock一般是阻塞式的获取锁，意思就是不获取到锁誓不罢休，那么我们可以写一个死循环来执行其操作:
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/WLIGprPy3z7asVtMGYjJn2w4yAOiclibO1vxWHo52sGNicc8Z58YXd6oy9nuxYAXMicj4ddkt3G5toJ6TsPRAAL2ibQ/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/WLIGprPy3z7asVtMGYjJn2w4yAOiclibO1Qc2enNqlUPSN63jeG7utiaeA9hvbSCTY0F0r8icFicsO4iaZV3QdiamrLhg/640?wx_fmt=png)
需要注意的是这一段代码需要加事务，必须要保证这一系列操作的原子性。
## 3.2tryLock()和tryLock(long timeout)
tryLock()是非阻塞获取锁，如果获取不到那么就会马上返回，代码可以如下:
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/WLIGprPy3z7asVtMGYjJn2w4yAOiclibO1gNE60Arp9OWzEwOQzAMOic0GaMaFnC42rfibjSeWxpc6E7JU3UIT22gw/640?wx_fmt=png)
tryLock(long timeout)实现如下:
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/WLIGprPy3z7asVtMGYjJn2w4yAOiclibO1mQL2WY7vGPibru3pia72wRian4dRDibucehjYk3AsQsibAysMXZVUWchBHw/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/WLIGprPy3z7asVtMGYjJn2w4yAOiclibO1bVPeLEW84oWibB4MO3TL0HrNWWMjQS4qBwVnBHqBh7ZVCvnaYT6YS5g/640?wx_fmt=png)
## 3.4 锁超时
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/WLIGprPy3z7asVtMGYjJn2w4yAOiclibO1xVdNaDA7KHdrQMw761iapicphAXmSJXeVm5LiaXykU5raXLJsBeEmo4Jw/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/WLIGprPy3z7asVtMGYjJn2w4yAOiclibO1gXTQRcrZk0Ek1xmjDjLpsJib2qlgp8ia7z7W92XIDHyux3Mt15YntAGQ/640?wx_fmt=png)
我们利用acuire进行加锁，release进行解锁。
加锁的流程具体如下:
- 
首先进行可重入的判定:这里的可重入锁记录在ConcurrentMap
threadData这个Map里面，如果threadData.get(currentThread)是有值的那么就证明是可重入锁，然后记录就会加1。我们之前的Mysql其实也可以通过这种方法去优化，可以不需要count字段的值，将这个维护在本地可以提高性能。
- 
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/WLIGprPy3z7asVtMGYjJn2w4yAOiclibO1KKHt2rys0p9mqzWjFHE3oAj5MlwMFUDDQxia0fuQHibYYsA793PWvmzw/640?wx_fmt=png)
Redission不仅提供了Java自带的一些方法(lock,tryLock)，还提供了异步加锁，对于异步编程更加方便。
由于内部源码较多，就不贴源码了，这里用文字叙述来分析他是如何加锁的，这里分析一下tryLock方法:
- 
尝试加锁:首先会尝试进行加锁，由于需要兼容老版本的Redis，所以不能直接使用ex nx原子操作的API，那么就只能使用lua脚本，相关的lua脚本如下：
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/WLIGprPy3z7asVtMGYjJn2w4yAOiclibO11jDZkqdib5wQMibzZRW4MlEc584GUu6175hzHOniaVAM2r4P3VPtaWq3g/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/WLIGprPy3z7asVtMGYjJn2w4yAOiclibO1fibvbKicponuE6LQibDvzA7jQuBBNjFlRC3jqFE7DJia6Wwb7IhJibrl5Nw/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/WLIGprPy3z7asVtMGYjJn2w4yAOiclibO17aJEzqnDN1OEQJq5lwrZ0vMpsvXYNrkdHuoS2HqpphJ226C5hQJ9yA/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/WLIGprPy3z7asVtMGYjJn2w4yAOiclibO1Dene5eFnmNI5TNz0N4uWOQ9DhpuwKfPSbTCOrPUkUrexeMRGaf0KHg/640?wx_fmt=png)
...
[并行化-你的高并发大杀器](http://mp.weixin.qq.com/s?__biz=MzA5Mjg2MDQ5NQ==&mid=2452509219&idx=1&sn=57515682a867f36fd8526b5bed954348&chksm=87bc4b03b0cbc2150fac53b699c3715a7055a618880a4133a8211e94d2b0e1c712117836b135&scene=21#wechat_redirect)，[异步化-你的高并发大杀器](http://mp.weixin.qq.com/s?__biz=MzA5Mjg2MDQ5NQ==&mid=2452509086&idx=1&sn=cfec16ac2197abd557afd77a7f40b808&chksm=87bc48beb0cbc1a80716cf3c609f24f0c1434ebc4e36b998263ee416fe6e69ac0d5eb8d77735&scene=21#wechat_redirect)
### 7.Chubby的一些优化
