# 分布式架构的前世今生... - 纯洁的微笑 - CSDN博客
2018年07月10日 07:36:00[微笑很纯洁](https://me.csdn.net/ityouknow)阅读数：463
![640?wx_fmt=gif](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_gif/T0cUsHzkWHDUa6FMHWSGJuy59xsou0HE2sKIiakUgU1NlzAqfsQnjOKUKVck0Oiaziccb012tCYpB9lofxx7MibZBg/640?wx_fmt=gif)
### 一、前言
...
交易模块:创建交易及支付结算。
### 三、阶段一：单应用架构
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/T0cUsHzkWHDUa6FMHWSGJuy59xsou0HElCdYAm35SOjbSmLPNBSLSmgKJHSicYpIMNdo1AccrCK9a4slboykiaag/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/T0cUsHzkWHDUa6FMHWSGJuy59xsou0HEnDficickhOlktyj4Bq5wbGXw7wgYYjROaDfmMcra7R2UtHBWIkPCwQ8w/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/T0cUsHzkWHDUa6FMHWSGJuy59xsou0HENxVISyvncsuuf6LMZ82WRgcNLMAIqlsvs83GIAyVONavHlnhQ1ggpw/640?wx_fmt=png)
...
那么此时，系统架构又会变成如下方式：
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/T0cUsHzkWHDUa6FMHWSGJuy59xsou0HEbmTyfMS4uEoH14GUs7fcHj4vpEsrXDVSxichk25QHsoD9CGVeNPkibAw/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/T0cUsHzkWHDUa6FMHWSGJuy59xsou0HE3GpqH1ZGJME8cYHbu0yy8dNVDicHFJpNUaqcI7NbLCcR0LxuVv4QdQA/640?wx_fmt=png)
这个架构设计的变化会带来如下几个问题：
- 
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/T0cUsHzkWHDUa6FMHWSGJuy59xsou0HEic6Q307iaiaNVwzWuJscoq0HQ3Bkvenic55EGDibicqpc8WTbRMBMDibqpPsw/640?wx_fmt=png)
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/T0cUsHzkWHDUa6FMHWSGJuy59xsou0HE4udjOlLl28QNjqfOs02fDdB08obuRJC5SsVFtUOev8q18cqwlqXu5w/640?wx_fmt=png)
### 九、阶段七：数据库的水平/垂直拆分
我们的网站演进的变化过程，交易、商品、用户的数据都还在同一 个数据库中，尽管采取了增加缓存，读写分离的方式，但是随着数 据库的压力持续增加，数据库的瓶颈仍然是个最大的问题。因此我 们可以考虑对数据的垂直拆分和水平拆分。
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/T0cUsHzkWHDUa6FMHWSGJuy59xsou0HE6twpXy1nibBTiaiaZQ2xI7edkic0JK32eMRGib04N7puOSDvbyY0NSWg2Dg/640?wx_fmt=png)
垂直拆分:把数据库中不同业务数据拆分到不同的数据库。
水平拆分:把同一个表中的数据拆分到两个甚至更多的数据库中，水平拆分的原因是某些业务数据量已经达到了单个数据库的瓶颈，这时可以采取将表拆分到多个数据库中。
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/T0cUsHzkWHDUa6FMHWSGJuy59xsou0HEf9hSCMicgiadUdK0LHaEuicyHKCGXUCpTFoicjbEdcWHC6F08ohk7BQnoA/640?wx_fmt=png)
### 十、阶段八：应用的拆分
随着业务的发展，业务量越来越大，应用的压力越来越大。工程规模也越来越庞大。这个时候就可以考虑将应用拆分，按照领域模型将我们的用户、商品、交易拆分成多个子系统。
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/T0cUsHzkWHDUa6FMHWSGJuy59xsou0HEmj6j2vYBe3ErbXqEtomNeBtFX0ucQTo2ywHLZuBGKNXgxcfn91JreA/640?wx_fmt=png)
这样拆分以后，可能会有一些相同的代码，比如用户操作，在商品和交易都需要查询，所以会导致每个系统都会有用户查询访问相关操作。这些相同的操作一定是要抽象出来，否则就是一个坑。所以通过走服务化路线的方式来解决。
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/T0cUsHzkWHDUa6FMHWSGJuy59xsou0HEy2iaKgZ1lE7zHNfo1WyzHLaqaPLE9hvbU0TcaUDq6ltRNrsic8PmibQDw/640?wx_fmt=png)
