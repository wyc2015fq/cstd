# iOS crash分析 - game3108的专栏 - CSDN博客
2016年04月13日 23:29:35[game3108](https://me.csdn.net/game3108)阅读数：843
- - [本文转载于同事的分享文章 siyixie在这里记录一下](#本文转载于同事的分享文章-siyixie在这里记录一下)
- [iOS crash分析](#ios-crash分析)
- [收集crash](#收集crash)- [crash的源头](#crash的源头)
- [捕获crash](#捕获crash)
- [获取crash的具体信息](#获取crash的具体信息)
- [对比](#对比)
- [收集crash的步骤](#收集crash的步骤)
- [收集到的crash实例](#收集到的crash实例)
- [工具 atos](#工具-atos)- [关键load address slide](#关键load-address-slide)
- [获取load addressslide](#获取load-addressslide)
- [参考资料](#参考资料)
- [补充资料](#补充资料)- [memory layout 内存布局](#memory-layout-内存布局)
- [函数调用栈 callstack](#函数调用栈-callstack)
- [实际代码TPUncaughtExceptionHandlerm](#实际代码tpuncaughtexceptionhandlerm)
- [通过backtrace获取的crash信息](#通过backtrace获取的crash信息)
...
> 
[iOS Developer Library: Uncaught Exceptions](https://developer.apple.com/library/ios/documentation/Cocoa/Conceptual/Exceptions/Concepts/UncaughtExceptions.html)
...
### 捕获crash
- 
如果被转成了NSException,可以通过`NSSetUncaughtExecptionHandler()`捕获；
- 
如果以`unix signal`的形式发送给相应的线程，则可以通过C的函数注册对应的信号函数来处理 `signal(int sig, sig_t func)`
### 获取crash的具体信息
- 
...
### 收集crash的步骤
- 
注册 `UncaughtExecptionHandler`和`signal(int sig, sig_t func)`
- 
通过对应方法获取 函数调用栈的符号： `[NSException callStackSymbols]` 或者 `backtrace_symbols(void* const* array, int size)`
- 
保存成文件，并上传到服务器
- 
根据调用栈信息和符号表文件（.dSYM文件）进行符号解析。通常使用的命令是：`atos`。比如 `atos -o ./TouchPalDialer.app/TouchPalDialer -arch arm64 -s 0x2760 0x00000001004ed148`
### 收集到的crash实例
...
[苹果开发者文档(Technical Note TN2123)](https://developer.apple.com/library/mac/technotes/tn2004/tn2123.html)指出，需要区分两个加载地址（load address）：`实际加载地址``期望加载地址`
- 
**`实际加载地址 (actual load address)`**
是指程序运行时的地址。每次程序装载(load)都可能不同。
- 
**`期望加载地址 (intented load address)`**
是指编译之后指定的加载地址。
`偏移量 = 实际加载地址 -  期望加载地址`
`slide = acutal load address - intended load addrss`
### 获取`load address`,`slide`
- **加载地址(load address)**
如果能够获取有系统的CrashReporter产生的完成的崩溃日志文件(.crash文件)，则从其中的`Binary Images`部分获取当前程序的`**实际加载地址(acutal load address)**`
举例如下：
```java
Binary Images:
...
[http://stackoverflow.com/questions/6462214/how-to-read-objective-c-stack-traces](http://stackoverflow.com/questions/6462214/how-to-read-objective-c-stack-traces)
- 
...
[https://www.plausible.coop/blog/?p=176](https://www.plausible.coop/blog/?p=176)
- 
`intended load address` vs `actual load address`
[http://www.cocoabuilder.com/archive/xcode/312725-can-symbolicate-crash-reports.html](http://www.cocoabuilder.com/archive/xcode/312725-can-symbolicate-crash-reports.html)
- 
tools `man atos`
[https://developer.apple.com/library/mac/documentation/Darwin/Reference/ManPages/man1/atos.1.html](https://developer.apple.com/library/mac/documentation/Darwin/Reference/ManPages/man1/atos.1.html)
- 
`man otool`
[http://www.unix.com/man-page/OSX/1/otool/](http://www.unix.com/man-page/OSX/1/otool/)
- 
Tales From The Crash Mines: Issue #1, by Landon Fuller 
[https://www.mikeash.com/pyblog/tales-from-the-crash-mines-issue-1.html](https://www.mikeash.com/pyblog/tales-from-the-crash-mines-issue-1.html)
- 
你的iOS安装包真的“脱光”了么 
[http://zhuanlan.zhihu.com/geek-makes-life-better/19925959](http://zhuanlan.zhihu.com/geek-makes-life-better/19925959)
- 
使用Symbolicatecrash和xcrun atos分析crash log 
[http://blog.csdn.net/mkhgg/article/details/17247673](http://blog.csdn.net/mkhgg/article/details/17247673)
- 
stackoverflow 
How to read objective-c stack traces 
[http://stackoverflow.com/questions/6462214/how-to-read-objective-c-stack-traces](http://stackoverflow.com/questions/6462214/how-to-read-objective-c-stack-traces)
...
[http://stackoverflow.com/questions/13574933/ios-crash-reports-atos-not-working-as-expected](http://stackoverflow.com/questions/13574933/ios-crash-reports-atos-not-working-as-expected)
- 
`dwarfdump`: 关于XCode编译完App之后生成的dSYM文件 
[http://blog.csdn.net/yan8024/article/details/8186760](http://blog.csdn.net/yan8024/article/details/8186760)
- 
使用dwarfdump检查dSYM和app是否匹配 
[http://blog.csdn.net/yan8024/article/details/8186774](http://blog.csdn.net/yan8024/article/details/8186774)
- 
load address  
...
[http://www.cocoabuilder.com/archive/cocoa/276690-debugging-stack-traces.html](http://www.cocoabuilder.com/archive/cocoa/276690-debugging-stack-traces.html)
- 
