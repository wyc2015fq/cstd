# markdown math 数学公式语法 - 墨流觞的博客 - CSDN博客





2018年09月13日 18:44:54[墨氲](https://me.csdn.net/dss_dssssd)阅读数：1151
...
本篇文章为转载文章，我平时也是按照这篇文章写markdown 数学公式的，原网站连接[https://www.zybuluo.com/codeep/note/163962](https://www.zybuluo.com/codeep/note/163962)
...
2018-03-20 补档： 
  ...
  本文固定链接: [https://www.zybuluo.com/codeep/note/163962](https://www.zybuluo.com/codeep/note/163962)
> 
...
- [Cmd Markdown 公式指导手册](#cmd-markdown-公式指导手册)
- [一、公式使用参考](#一公式使用参考)- [1．如何插入公式](#1如何插入公式)
- [2．如何输入上下标](#2如何输入上下标)
- [3．如何输入括号和分隔符](#3如何输入括号和分隔符)
- [4．如何输入分数](#4如何输入分数)
- [5．如何输入开方](#5如何输入开方)
- [6．如何输入省略号](#6如何输入省略号)
- [7．如何输入矢量](#7如何输入矢量)
- [8．如何输入积分](#8如何输入积分)
- [9．如何输入极限运算](#9如何输入极限运算)
- [10．如何输入累加、累乘运算](#10如何输入累加累乘运算)
- [11．如何输入希腊字母](#11如何输入希腊字母)
- [12．如何输入其它特殊字符](#12如何输入其它特殊字符)- [(1)．关系运算符](#1关系运算符)
- [(2)．集合运算符](#2集合运算符)
- [(3)．对数运算符](#3对数运算符)
- [(4)．三角运算符](#4三角运算符)
- [(5)．微积分运算符](#5微积分运算符)
...
- [13．如何进行字体转换](#13如何进行字体转换)
- [14．大括号和行标的使用](#14大括号和行标的使用)
- [15．其它命令](#15其它命令)- [(1)．定义新的符号 \operatorname](#1定义新的符号-operatorname)
- [(2)．添加注释文字 \text](#2添加注释文字-text)
- [(3)．在字符间加入空格](#3在字符间加入空格)
- [(4)．更改文字颜色](#4更改文字颜色)
- [(5)．添加删除线](#5添加删除线)


- [二、矩阵使用参考](#二矩阵使用参考)- [1．如何输入无框矩阵](#1如何输入无框矩阵)
- [2．如何输入边框矩阵](#2如何输入边框矩阵)
- [3．如何输入带省略符号的矩阵](#3如何输入带省略符号的矩阵)
- [4．如何输入带分割符号的矩阵](#4如何输入带分割符号的矩阵)
- [5．如何输入行中矩阵](#5如何输入行中矩阵)

- [三、方程式序列使用参考](#三方程式序列使用参考)- [1．如何输入一个方程式序列](#1如何输入一个方程式序列)
- [2．在一个方程式序列的每一行中注明原因](#2在一个方程式序列的每一行中注明原因)

- [四、条件表达式使用参考](#四条件表达式使用参考)- [1．如何输入一个条件表达式](#1如何输入一个条件表达式)
- [2．如何输入一个左侧对齐的条件表达式](#2如何输入一个左侧对齐的条件表达式)
- [3．如何使条件表达式适配行高](#3如何使条件表达式适配行高)

- [五、数组与表格使用参考](#五数组与表格使用参考)- [1．如何输入一个数组或表格](#1如何输入一个数组或表格)
- [2．如何输入一个嵌套的数组或表格](#2如何输入一个嵌套的数组或表格)
- [3．如何输入一个方程组](#3如何输入一个方程组)

- [六、连分数使用参考](#六连分数使用参考)- [1．如何输入一个连分式](#1如何输入一个连分式)

- [七、交换图表使用参考](#七交换图表使用参考)- [1．如何输入一个交换图表](#1如何输入一个交换图表)

- [八、一些特殊的注意事项](#八一些特殊的注意事项)
...
- 例子：
`$$ f(x,y,z) = 3y^2z \left( 3+\frac{7x+5}{1+y^2} \right) $$`- 显示：

$ f(x,y,z) = 3y^2z \left( 3+\frac{7x+5}{1+y^2} \right) $

有时候要用 `\left.` 或 `\right.` 进行匹配而不显示本身。
- 例子：
`$$ \left. \frac{{\rm d}u}{{\rm d}x} \right| _{x=0} $$`- 显示：
...
如果分式很复杂，亦可使用 `分子 \over 分母` 命令，此时分数仅有一层。- 例子：
`$$\frac{a-1}{b-1} \quad and \quad {a+1\over b+1}$$`- 显示：
...
- 例子：
`$$\sqrt{2} \quad and \quad \sqrt[n]{3}$$`- 显示：
...
- 例子：
`$$f(x_1,x_2,\underbrace{\ldots}_{\rm ldots} ,x_n) = x_1^2 + x_2^2 + \underbrace{\cdots}_{\rm cdots} + x_n^2$$`- 显示：
...
`$$\overleftarrow{xy} \quad and \quad \overleftrightarrow{xy} \quad and \quad \overrightarrow{xy}$$`- 显示：
...
`$$\sum_{i=1}^n \frac{1}{i^2} \quad and \quad \prod_{i=1}^n \frac{1}{i^2} \quad and \quad \bigcup_{i=1}^{2} R$$`- 显示：
...
- 例子：
`$$ f(n)= \begin{cases} n/2, & \text {if $n$ is even} \\ 3n+1, & \text{if $n$ is odd} \end{cases} $$`- 显示： 
...
- 例子：
`$$ a \, b \mid a \; b \mid a \quad b \mid a \qquad b $$`- 显示：
...
其中，删除线效果有 `horizontalstrike`、`verticalstrike`、`updiagonalstrike` 和 `downdiagonalstrike`，可叠加使用。- 例子：
...
- 例子：
`这是一个行中矩阵的示例 $\bigl( \begin{smallmatrix} a & b \\ c & d \end{smallmatrix} \bigr)$ 。`- 显示：这是一个行中矩阵的示例 $\bigl( \begin{smallmatrix} a & b \\ c & d \end{smallmatrix} \bigr)$ 。
