# 一步一步理解GB、GBDT、xgboost - 别说话写代码的博客 - CSDN博客





2018年10月02日 15:50:07[别说话写代码](https://me.csdn.net/qq_21997625)阅读数：55标签：[机器学习GB																[GBDT																[xgboost](https://so.csdn.net/so/search/s.do?q=xgboost&t=blog)
...
转自：[https://www.cnblogs.com/wxquare/p/5541414.html](https://www.cnblogs.com/wxquare/p/5541414.html)
...
![1 \le m \le M](https://upload.wikimedia.org/math/e/4/7/e4746d8b9e912c1875cd9e5f612ab2ae.png)![F_{m+1}(x) = F_m(x) + h(x)](https://upload.wikimedia.org/math/7/5/7/7579bdfb140e56a86da109ec96b87421.png)

      GB算法的思想很简单，关键是怎么生成h(x)?

      如果目标函数是回归问题的均方误差，很容易想到最理想的h(x)应该是能够完全拟合![y - F_m(x)](https://upload.wikimedia.org/math/9/7/d/97dfe316cb29bf7289a75ade5be7de1c.png) ，这就是常说基于残差的学习。残差学习在回归问题中可以很好的使用，但是为了一般性（分类，排序问题），实际中往往是基于loss Function 在函数空间的的负梯度学习，对于回归问题![\frac{1}{2}(y - F(x))^2](https://upload.wikimedia.org/math/d/6/9/d69638d1c777cf19b3b3b04bc81d101e.png)残差和负梯度也是相同的。![L(y, f)](https://upload.wikimedia.org/math/6/1/3/613b882386356b2a890c03969c349759.png)中的f，不要理解为传统意义上的函数，而是一个函数向量![\! f(x_1), \ldots, f(x_n)](https://upload.wikimedia.org/math/a/9/6/a9631135ced6265452825a49f4c7badb.png)，向量中元素的个数与训练样本的个数相同，因此基于Loss Function函数空间的负梯度的学习也称为“伪残差”。

**GB算法的步骤：**

　　1.初始化模型为常数值：

![F_0(x) = \underset{\gamma}{\arg\min} \sum_{i=1}^n L(y_i, \gamma).](https://upload.wikimedia.org/math/7/f/7/7f7ee5504c0d54de6d2510bdae7f723a.png)
...
　　　　2.基于![\{(x_i, r_{im})\}_{i=1}^n](https://upload.wikimedia.org/math/2/b/b/2bbe4b0725baed85eae8dbbb20360ea6.png)生成基学习器![\! h_m(x)](https://upload.wikimedia.org/math/7/f/d/7fd32efb7a21cc484be23d1015ee074e.png)

　　　　3.计算最优的![\! \gamma_m](https://upload.wikimedia.org/math/8/7/2/8721621535a98cea1b0e38459f594057.png)

![\gamma_m = \underset{\gamma}{\operatorname{arg\,min}} \sum_{i=1}^n L\left(y_i, F_{m-1}(x_i) + \gamma h_m(x_i)\right).](https://upload.wikimedia.org/math/9/2/e/92e9576607b45540b71e23c14c737d0a.png)

      　　4.更新模型

![F_m(x) = F_{m-1}(x) + \gamma_m h_m(x).](https://upload.wikimedia.org/math/3/b/0/3b047653ac126f19f09112f47ddb9f9c.png)

##  2. Gradient boosting Decision Tree(GBDT)

　　GB算法中最典型的基学习器是决策树，尤其是CART，正如名字的含义，GBDT是GB和DT的结合。要注意的是这里的决策树是回归树，GBDT中的决策树是个弱模型，深度较小一般不会超过5，叶子节点的数量也不会超过10，对于生成的每棵决策树乘上比较小的缩减系数（学习率<0.1），有些GBDT的实现加入了随机抽样（subsample 0.5<=f <=0.8）提高模型的泛化能力。通过交叉验证的方法选择最优的参数。因此GBDT实际的核心问题变成怎么基于![\{(x_i, r_{im})\}_{i=1}^n](https://upload.wikimedia.org/math/2/b/b/2bbe4b0725baed85eae8dbbb20360ea6.png)使用CART回归树生成![\! h_m(x)](https://upload.wikimedia.org/math/7/f/d/7fd32efb7a21cc484be23d1015ee074e.png)？
...
　　1. [维基百科 Gradient boosting](https://en.wikipedia.org/wiki/Gradient_boosting)

　　2. [XGBoost: A Scalable Tree Boosting System](https://arxiv.org/abs/1603.02754)

　　3. [陈天奇的slides](http://homes.cs.washington.edu/~tqchen/pdf/BoostedTree.pdf)

　　4. [xgboost导读和实战.pdf](http://download.csdn.net/detail/happytofly/8781881)](https://so.csdn.net/so/search/s.do?q=GBDT&t=blog)](https://so.csdn.net/so/search/s.do?q=机器学习GB&t=blog)
