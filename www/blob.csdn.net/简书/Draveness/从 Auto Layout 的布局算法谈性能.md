# 从 Auto Layout 的布局算法谈性能 - 简书
# 从 Auto Layout 的布局算法谈性能
> 
这是使用 ASDK 性能调优系列的第二篇文章，前一篇文章中讲到了如何提升 iOS 应用的渲染性能，你可以点击 [这里](https://link.jianshu.com?t=https://github.com/Draveness/iOS-Source-Code-Analyze/blob/master/contents/AsyncDisplayKit/%E6%8F%90%E5%8D%87%20iOS%20%E7%95%8C%E9%9D%A2%E7%9A%84%E6%B8%B2%E6%9F%93%E6%80%A7%E8%83%BD.md) 了解这部分的内容。
...
![](https://upload-images.jianshu.io/upload_images/1975281-2038ec999f11829a.jpg)
box-layout
## 把 Auto Layout 批判一番
...
真正使 Auto Layout 大规模投入使用的应该还是 [Masonry](https://link.jianshu.com?t=https://github.com/SnapKit/Masonry)，它使用了链式的语法对 Auto Layout 进行了很好的封装，使得 Auto Layout 更加简单易用；时至今日，开发者也在日常使用中发现了 Masonry 的各种问题，于是出现了各种各样的布局框架，不过这都是后话了。
![](https://upload-images.jianshu.io/upload_images/1975281-6aab6ea2562f10ad.jpg)
masonry
## Auto Layout 的原理和 Cassowary
Auto Layout 的原理其实非常简单，在这里通过一个例子先简单的解释一下：
![](https://upload-images.jianshu.io/upload_images/1975281-a65ab7f1f389005d.png)
view-demonstrate
...
![](https://upload-images.jianshu.io/upload_images/1975281-a972ccb2f7469422.png)
layout-phase
### Cassowary 算法
在上世纪 90 年代，一个名叫 [Cassowary](https://link.jianshu.com?t=https://en.wikipedia.org/wiki/Cassowary_(software)) 的布局算法解决了用户界面的布局问题，它通过将布局问题抽象成线性等式和不等式约束来进行求解。
...
![](https://upload-images.jianshu.io/upload_images/1975281-fddec784d87b1eae.jpeg)
performance-loss
...
> 
你可以在 [这里](https://link.jianshu.com?t=https://github.com/Draveness/iOS-Source-Code-Analyze/tree/master/contents/AsyncDisplayKit/Layout) 找到这次对 Layout 性能测量使用的代码。
...
![](https://upload-images.jianshu.io/upload_images/1975281-a8200439b7bb3de2.jpeg)
...
![](https://upload-images.jianshu.io/upload_images/1975281-c39af3584b9042ea.jpeg)
...
![](https://upload-images.jianshu.io/upload_images/1975281-2073e953c36670b8.jpeg)
...
> 
在三年以前，有一篇关于 Auto Layout 性能分析的文章，可以点击这里了解这篇文章的内容 [Auto Layout Performance on iOS](https://link.jianshu.com?t=http://floriankugler.com/2013/04/22/auto-layout-performance-on-ios/)。
## ASDK 的布局引擎
Auto Layout 不止在复杂 UI 界面布局的表现不佳，它还会强制视图在主线程上布局；所以在 ASDK 中提供了另一种可以在后台线程中运行的布局引擎，它的结构大致是这样的：
![](https://upload-images.jianshu.io/upload_images/1975281-e3b0b352d74cb36a.png)
layout-hierarchy
...
```
ASDK 的文档中推荐在子类中覆写 `- layoutSpecThatFits:` 方法，返回一个用于布局的 `ASLayoutSpec` 对象，然后使用 `ASLayoutSpec` 中的 `- measureWithSizeRange:` 方法对它指定的视图进行布局，不过通过覆写 [ASDK 的布局引擎](#asdk-%E7%9A%84%E5%B8%83%E5%B1%80%E5%BC%95%E6%93%8E) 一节中的其它方法也都是可以的。
...
在 ASDK 1.0 时代，很多开发者都表示希望 ASDK 中加入 ComponentKit 的布局引擎；而现在，ASDK 布局引擎的大部分代码都是从 [ComponentKit](https://link.jianshu.com?t=http://componentkit.org) 中移植过来的（ComponentKit 是另一个 Facebook 团队开发的用于布局的框架）。
#### ASLayout
...
![](https://upload-images.jianshu.io/upload_images/1975281-f348ffa10a4fd82c.jpg)
stack
`ASStackLayoutSpec` 从 `Flexbox` 中获得了非常多的灵感，比如说 `justifyContent`、`alignItems` 等属性，它和苹果的 `UIStackView` 比较类似，不过底层并没有使用 Auto Layout 进行计算。如果没有接触过 `ASStackLayoutSpec` 的开发者，可以通过这个小游戏 [Foggy-ASDK-Layout](https://link.jianshu.com?t=http://nguyenhuy.github.io/froggy-asdk-layout/) 快速学习 `ASStackLayoutSpec` 的使用。
### 关于缓存以及异步并发
...
![](https://upload-images.jianshu.io/upload_images/1975281-edaf76d2e0e096f9.jpeg)
...
## References
- [Cassowary, Cocoa Auto Layout, and enaml constraints]([http://stacks.11craft.com/cassowary-cocoa-Auto](https://link.jianshu.com?t=http://stacks.11craft.com/cassowary-cocoa-Auto) Layout-and-enaml-constraints.html)
- [Solving constraint systems](https://link.jianshu.com?t=http://cassowary.readthedocs.io/en/latest/topics/theory.html)
- [Auto Layout Performance on iOS](https://link.jianshu.com?t=http://floriankugler.com/2013/04/22/auto-layout-performance-on-ios/)
- [The Cassowary Linear Arithmetic Constraint Solving Algorithm: Interface and Implementation](https://link.jianshu.com?t=https://constraints.cs.washington.edu/cassowary/cassowary-tr.pdf)
- [The Cassowary Linear Arithmetic Constraint Solving Algorithm](https://link.jianshu.com?t=http://constraints.cs.washington.edu/solvers/cassowary-tochi.pdf)
- [Solving Linear Arithmetic Constraints for User Interface Applications](https://link.jianshu.com?t=http://constraints.cs.washington.edu/solvers/uist97.pdf)
- [AsyncDisplayKit 介绍（二）布局系统](https://link.jianshu.com?t=https://medium.com/@jasonyuh/asyncdisplaykit%E4%BB%8B%E7%BB%8D-%E4%BA%8C-%E5%B8%83%E5%B1%80%E7%B3%BB%E7%BB%9F-1f1a674cf644#.8jskykm15)
> 
Github Repo：[iOS-Source-Code-Analyze](https://link.jianshu.com?t=https://github.com/draveness/iOS-Source-Code-Analyze)
Follow: [Draveness · GitHub](https://link.jianshu.com?t=https://github.com/Draveness)
Source: [http://draveness.me/layout-performance](https://link.jianshu.com?t=http://draveness.me/layout-performance)
