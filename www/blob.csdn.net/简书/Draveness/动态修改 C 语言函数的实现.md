...
# 动态修改 C 语言函数的实现
> 
关注仓库，及时获得更新：[iOS-Source-Code-Analyze](https://link.jianshu.com?t=https://github.com/draveness/iOS-Source-Code-Analyze)
...
C 语言往往会给我们留下**不可修改**的这一印象；在之前的几年时间里，笔者确实也是这么认为的，然而最近接触到的 [fishhook](https://link.jianshu.com?t=https://github.com/facebook/fishhook) 使我对 **C 语言的不可修改**有了更加深刻的理解。
> 
...
![](https://upload-images.jianshu.io/upload_images/1975281-4ffe232e9590086c.png)
fishhook-result
...
![](https://upload-images.jianshu.io/upload_images/1975281-88826fca01c070b1.png)
fishhook-symbo
...
![](https://upload-images.jianshu.io/upload_images/1975281-513d712edd354514.png)
fishbook-printf
...
![](https://upload-images.jianshu.io/upload_images/1975281-719b3d9a82c5574f.png)
fishhook-mach-o
...
整个过程可以用这么一张图来表示：
![](https://upload-images.jianshu.io/upload_images/1975281-6142a7c7fe233afb.png)
fishhook-before-afte
原理看起来还是很简单的，其中最复杂的部分就是从二进制文件中寻找某个符号的位置，在 fishhook 的 README 中，有这样一张图：
![](https://upload-images.jianshu.io/upload_images/1975281-8014d36bede7e5f2.png)
fishhook-imp
...
```
类似地，在 `symtab_command` 中获取符号表偏移量和字符串表偏移量，从 `dysymtab_command` 中获取间接符号表（indirect symbol table）偏移量，就能够获得*符号表*、*字符串表*以及*间接符号表*的引用了。
- 
...
![](https://upload-images.jianshu.io/upload_images/1975281-de8f1b2085db649e.png)
fishhook-hello
代码中只打印了 `hello`，说明 fishhook 对这种手写的函数是没有作用的，如果在下面这里打一个断点：
![](https://upload-images.jianshu.io/upload_images/1975281-64bb4b11a0c99653.png)
fishhook-hello-breakpoint
代码并不会进这里，因为 `hello` 这个函数是包含在当前镜像的，它只是从其它代码地址跳转到了当前函数实现，这与我们调用外部库时有很大的不同，当调用外部库时，我们需要 dyld 解决函数地址的问题，但是函数在当前镜像中却并不需要 [issue #25](https://link.jianshu.com?t=https://github.com/facebook/fishhook/issues/25)。
## 小结
fishhook 的实现非常的巧妙，但是它的使用也有一定的局限性，在接触到 fishhook 之前，从没有想到过可以通过一种方式修改 C 函数的实现。
## Reference
- [Mach-O 可执行文件](https://link.jianshu.com?t=https://objccn.io/issue-6-3/)
- [Static linking vs dynamic linking](https://link.jianshu.com?t=http://stackoverflow.com/questions/1993390/static-linking-vs-dynamic-linking)
## 其它
### main.m
```
#import <Foundation/Foundation.h>
...
关注仓库，及时获得更新：[iOS-Source-Code-Analyze](https://link.jianshu.com?t=https://github.com/draveness/iOS-Source-Code-Analyze)
Follow: [Draveness · Github](https://link.jianshu.com?t=https://github.com/Draveness)
原文链接:  [http://draveness.me/fishhook/](https://link.jianshu.com?t=http://draveness.me/fishhook/)
