...
在单变量回归中，一个平稳的时间序列 ![y_t](https://math.jianshu.com/math?formula=y_t) 经常被模型化为 AR 过程：

![y_t=\alpha_0+\alpha_1 y_{t-1}+\alpha_2 y_{t-2}+\dots +\alpha_k y_{t-k}+\epsilon_t](https://math.jianshu.com/math?formula=y_t%3D%5Calpha_0%2B%5Calpha_1%20y_%7Bt-1%7D%2B%5Calpha_2%20y_%7Bt-2%7D%2B%5Cdots%20%2B%5Calpha_k%20y_%7Bt-k%7D%2B%5Cepsilon_t)

当我们分析多个时间序列时，一个对 AR 模型自然的拓展就是 VAR 模型， 在这个模型中一组向量里的每个时间序列被模型化为决定于自己的滞后项以及这组向量里所有其他变量的滞后项。两阶 VAR 模型如下式：

![\begin{align} y_t = \alpha_0+\alpha_1 y_{t-1}+\alpha_2 x_{t-1}+\epsilon_{1t} \\ x_t = \beta_0+\beta_1 y_{t-1}+\beta_2 x_{t-1}+\epsilon_{2t} \end{align}](https://math.jianshu.com/math?formula=%5Cbegin%7Balign%7D%20y_t%20%3D%20%5Calpha_0%2B%5Calpha_1%20y_%7Bt-1%7D%2B%5Calpha_2%20x_%7Bt-1%7D%2B%5Cepsilon_%7B1t%7D%20%5C%5C%20x_t%20%3D%20%5Cbeta_0%2B%5Cbeta_1%20y_%7Bt-1%7D%2B%5Cbeta_2%20x_%7Bt-1%7D%2B%5Cepsilon_%7B2t%7D%20%5Cend%7Balign%7D)
...
![\begin{bmatrix} inflation_{ t }\\ unrate_{ t }\\ ffr_{ t } \end{bmatrix}=a_{ 0 }+A_{ 1 }\begin{bmatrix} inflation_{ t -1}\\ unrate_{ t -1}\\ ffr_{ t -1} \end{bmatrix}+\cdots +A_{ k }\begin{bmatrix} inflation_{ t -k}\\ unrate_{ t -k}\\ ffr_{ t -k} \end{bmatrix}+\begin{bmatrix} \epsilon_{ 1,t }\\ \epsilon_{ 2,t }\\ \epsilon_{ 3,t } \end{bmatrix}](https://math.jianshu.com/math?formula=%5Cbegin%7Bbmatrix%7D%20inflation_%7B%20t%20%7D%5C%5C%20unrate_%7B%20t%20%7D%5C%5C%20ffr_%7B%20t%20%7D%20%5Cend%7Bbmatrix%7D%3Da_%7B%200%20%7D%2BA_%7B%201%20%7D%5Cbegin%7Bbmatrix%7D%20inflation_%7B%20t%20-1%7D%5C%5C%20unrate_%7B%20t%20-1%7D%5C%5C%20ffr_%7B%20t%20-1%7D%20%5Cend%7Bbmatrix%7D%2B%5Ccdots%20%2BA_%7B%20k%20%7D%5Cbegin%7Bbmatrix%7D%20inflation_%7B%20t%20-k%7D%5C%5C%20unrate_%7B%20t%20-k%7D%5C%5C%20ffr_%7B%20t%20-k%7D%20%5Cend%7Bbmatrix%7D%2B%5Cbegin%7Bbmatrix%7D%20%5Cepsilon_%7B%201%2Ct%20%7D%5C%5C%20%5Cepsilon_%7B%202%2Ct%20%7D%5C%5C%20%5Cepsilon_%7B%203%2Ct%20%7D%20%5Cend%7Bbmatrix%7D)

![a_0](https://math.jianshu.com/math?formula=a_0)是由截距项组成的向量，![A_1](https://math.jianshu.com/math?formula=A_1) 到 ![A_K](https://math.jianshu.com/math?formula=A_K) 均为 ![3\times 3](https://math.jianshu.com/math?formula=3%5Ctimes%203) 的系数矩阵。包含这些变量的 VAR 模型或相近的模型变体经常出现在货币政策分析中。
...
如你所见，估计系数表格非常长。即使不考虑常数项，一个有 ![n](https://math.jianshu.com/math?formula=n) 个变量和 ![k](https://math.jianshu.com/math?formula=k) 阶滞后的 VAR 模型中也会有 ![kn^2](https://math.jianshu.com/math?formula=kn%5E2) 个系数。我们的 3 变量，6 阶滞后的 VAR 则有将近 60 个系数，但是我们却仅有 198 个观测。 选项 `dfk` 和 `small` 将对默认情形下报告的大样本统计量进行小样本调整。虽然结果会报告系数、标准误、![t](https://math.jianshu.com/math?formula=t) 统计量、![p](https://math.jianshu.com/math?formula=p)统计量等，但是并不能给我们直观的信息含量，因此很多论文甚至都不会报告这些系数表格，但是他们会报告一些更有信息量的统计量。接下来的两部分将会介绍两个 VAR 结果分析中常用的统计手段：格兰杰因果检验和脉冲响应函数。
### 3. 评价 VAR 结果：格兰杰因果检验

如果给定 ![y_t](https://math.jianshu.com/math?formula=y_t) 的滞后阶数，![x_t](https://math.jianshu.com/math?formula=x_t)的滞后滞后项在以 ![y_t](https://math.jianshu.com/math?formula=y_t) 为因变量的方程中联合统计显著，则称 ![x_t](https://math.jianshu.com/math?formula=x_t) 是 ![y_t](https://math.jianshu.com/math?formula=y_t) 的格兰杰原因 。例如，如果利率滞后项对失业率是联合显著的，则可以称利率是失业率的格兰杰原因。`vargranger` 命令可以进行格兰杰因果检验。
```
...
像之前一样，方程之间通过被解释变量进行区分。对于每个方程，`vargranger` 都可以先单独检验每个变量在 VAR 模型中的因果关系，然后检验所有变量整体的格兰杰因果关系。在对失业率方程进行的格兰杰因果检验中，如上表所示，"**ffr**  excluded"  检验的原假设是所有利率及其滞后项的系数在对失业率的预测中均为 0 ，备择假设是至少有一个不为 0 。![p](https://math.jianshu.com/math?formula=p) 值为 0.27 并未落在 5% 显著性水平的拒绝域内，因此我们不能拒绝利率不会影响失业率的原假设，也就是说我们的数据和模型不支持利率是失业率的格兰杰原因的假设。相反地，在利率方程中，通胀和失业率的滞后项都是统计显著的，因此可以说通胀和失业率是利率的格兰杰原因。
...
解决这个问题的一个方法是假设存在一个结构性冲击向量 ![u_t](https://math.jianshu.com/math?formula=u_t) ，定义其其分量相互独立，并且这个冲击向量与原来的冲击之间存在如下关系：

![\begin{align} \epsilon_t=A u_t \\ E(u_t u'_t)=I \end{align}](https://math.jianshu.com/math?formula=%5Cbegin%7Balign%7D%20%5Cepsilon_t%3DA%20u_t%20%5C%5C%20E(u_t%20u'_t)%3DI%20%5Cend%7Balign%7D)

如果我们定义误差项的协方差矩阵为 ![\Sigma](https://math.jianshu.com/math?formula=%5CSigma) ，则矩阵 A 与 ![\Sigma](https://math.jianshu.com/math?formula=%5CSigma) 之间的关系为：
![\Sigma = E(\epsilon_t \epsilon'_t)=E(A u_t u'_t A')=AE(u_t u'_t )A'=AA'](https://math.jianshu.com/math?formula=%5CSigma%20%3D%20E(%5Cepsilon_t%20%5Cepsilon'_t)%3DE(A%20u_t%20u'_t%20A')%3DAE(u_t%20u'_t%20)A'%3DAA')

因为我们有从样本估计出的协方差矩阵 ![\hat{\Sigma}](https://math.jianshu.com/math?formula=%5Chat%7B%5CSigma%7D) ，我们于是可以由以上等式构造出 ![\hat{A}](https://math.jianshu.com/math?formula=%5Chat%7BA%7D) :
![\hat{\Sigma} = \hat{A} \hat{A'} \qquad (1)](https://math.jianshu.com/math?formula=%5Chat%7B%5CSigma%7D%20%3D%20%5Chat%7BA%7D%20%5Chat%7BA'%7D%20%5Cqquad%20(1))

很多 ![A](https://math.jianshu.com/math?formula=A) 矩阵都满足方程 (1) 。一种可能的方法是假设  ![A](https://math.jianshu.com/math?formula=A)  是一个下三角矩阵，则 ![A](https://math.jianshu.com/math?formula=A) 即可以通过对 ![\Sigma](https://math.jianshu.com/math?formula=%5CSigma) 进行 Cholesky 分解得到。这种分解方法非常常见，因此使用 `var`  命令时可以直接指定使用 Cholesky 分解。
对矩阵 ![A](https://math.jianshu.com/math?formula=A) 做出下三角的假设在 VAR 模型的变量的顺序做出了要求，因为不同的顺序将会得到不同的 ![A](https://math.jianshu.com/math?formula=A) 。变量顺序的经济学含义是对任意一个方程，其因变量都会被顺序在其之前变量的冲击影响，而不会被顺序在其之后的变量影响。对于这篇推文，我会对我们所用变量的顺序做出如下的设定：通胀冲击可以同期影响所有三个变量；失业率冲击只会同期影响利率，但不会影响通胀；利率冲击顺序在最后，因此既不会同期影响失业率，也不会同期影响通胀。
有了 ![A](https://math.jianshu.com/math?formula=A) ，我们就可以制造方程间相互独立的冲击并且在 VAR 模型中观察这些冲击对变量施加的影响了。我们可以通过 `irf create` 命令建立脉冲响应方程，然后使用 `irf graph` 命令对脉冲响应结果画图。
...
![](https://upload-images.jianshu.io/upload_images/7692714-95a582eb514caacb.jpg)
...
这里使用的 VAR 和顺序都很清楚。所有的推断都基于 ![A](https://math.jianshu.com/math?formula=A) 矩阵，也就是 VAR 模型中变量的顺序。不同的顺序会产生不同的 ![A](https://math.jianshu.com/math?formula=A) 矩阵，从而得到不同的脉冲响应结果。此外，除了简单地指定变量顺序外，还有别的识别策略，我将在知乎的推文中讨论。
### 5. 结论
...
![](https://upload-images.jianshu.io/upload_images/7692714-8eb06f2ff32d897d.jpg)
