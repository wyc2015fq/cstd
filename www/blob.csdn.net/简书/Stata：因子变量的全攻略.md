...
![](https://upload-images.jianshu.io/upload_images/8108279-2bc66f2d02807f09.png)
...
![](https://upload-images.jianshu.io/upload_images/8108279-f07d884552d4489e.png)
...
![](https://upload-images.jianshu.io/upload_images/8108279-85d422d10eaa2f77.png)
...
![{{y}_{it}}={{\alpha }_{i}}+{{\lambda }_{t}}+{{x}_{it}}^{'}\beta +{{\varepsilon }_{it}} \quad (1)](https://math.jianshu.com/math?formula=%7B%7By%7D_%7Bit%7D%7D%3D%7B%7B%5Calpha%20%7D_%7Bi%7D%7D%2B%7B%7B%5Clambda%20%7D_%7Bt%7D%7D%2B%7B%7Bx%7D_%7Bit%7D%7D%5E%7B'%7D%5Cbeta%20%2B%7B%7B%5Cvarepsilon%20%7D_%7Bit%7D%7D%20%5Cquad%20(1))

上式等价于
![{{y}_{it}}=\sum\limits_{i=1}^{N}{{{D}_{i}}\cdot }\ {{\alpha }_{i}}+\sum\limits_{t=2}^{T}{{{W}_{t}}\cdot }\ {{\lambda }_{t}}+{{x}_{it}}^{'}\beta +{{\varepsilon }_{it}} \quad (2)](https://math.jianshu.com/math?formula=%7B%7By%7D_%7Bit%7D%7D%3D%5Csum%5Climits_%7Bi%3D1%7D%5E%7BN%7D%7B%7B%7BD%7D_%7Bi%7D%7D%5Ccdot%20%7D%5C%20%7B%7B%5Calpha%20%7D_%7Bi%7D%7D%2B%5Csum%5Climits_%7Bt%3D2%7D%5E%7BT%7D%7B%7B%7BW%7D_%7Bt%7D%7D%5Ccdot%20%7D%5C%20%7B%7B%5Clambda%20%7D_%7Bt%7D%7D%2B%7B%7Bx%7D_%7Bit%7D%7D%5E%7B'%7D%5Cbeta%20%2B%7B%7B%5Cvarepsilon%20%7D_%7Bit%7D%7D%20%5Cquad%20(2))

其中，![D_i](https://math.jianshu.com/math?formula=D_i) 是对应于每家公司的虚拟变量，对于公司 ![i](https://math.jianshu.com/math?formula=i)，![D_i=1](https://math.jianshu.com/math?formula=D_i%3D1)，否则 ![D_i=0](https://math.jianshu.com/math?formula=D_i%3D0)。对于一个有 ![N=1000](https://math.jianshu.com/math?formula=N%3D1000) 家公司的面板数据而言，共有 1000 个虚拟变量。![W_t](https://math.jianshu.com/math?formula=W_t)  是 ![T-1](https://math.jianshu.com/math?formula=T-1) 个年度虚拟变量[5]， 定义方式与 ![D_i](https://math.jianshu.com/math?formula=D_i) 相似。
若使用因子变量，则语法很简单[6]：
...
其中，`i.id` 表示 `N-1` 个公司虚拟变量（Stata 会默认将一家公司作为基准组），即(1)式中的 ![{{\alpha}_{i}}](https://math.jianshu.com/math?formula=%7B%7B%5Calpha%7D_%7Bi%7D%7D)。
...
在实证分析中，常常需要分析**政策实施后带来的效果**。这时，我们就需要采集**实验组**与**控制组**两组样本（**实验组**的样本代表政策实施前后的情况，**控制组**的样本代表不实施政策的情况），再把这两组样本合并为一份数据后进行回归分析。在建模时，我们需要设定一个是否为实验组或控制组的处理虚拟变量  *Treat*  放入模型，还需要设定一个反映政策实施时间前与实施后的时间虚拟变量 *Time*  放入模型，此外，还需要在模型中加入 *Treat*  变量与 *Time*  变量的交乘项，该交乘项的系数估计值 ![{{a}_{3}}](https://math.jianshu.com/math?formula=%7B%7Ba%7D_%7B3%7D%7D) 就是政策实施后带来的效果，即实验组在政策实施后与假设未实施政策的差异。回归模型如下式(3)所示：
![y={{a}_{0}}+{{a}_{1}} {Treat}+{{a}_{2}} {Time}+{{a}_{3}} {Treat}\times {Time}+{{x}^{'}}\beta +\varepsilon \quad (3)](https://math.jianshu.com/math?formula=y%3D%7B%7Ba%7D_%7B0%7D%7D%2B%7B%7Ba%7D_%7B1%7D%7D%20%7BTreat%7D%2B%7B%7Ba%7D_%7B2%7D%7D%20%7BTime%7D%2B%7B%7Ba%7D_%7B3%7D%7D%20%7BTreat%7D%5Ctimes%20%7BTime%7D%2B%7B%7Bx%7D%5E%7B'%7D%7D%5Cbeta%20%2B%5Cvarepsilon%20%5Cquad%20(3))
在 Stata 中的命令写法如下：
...
![{{y}_{it}}={{a}_{0}} + {{x}_{it}}^{'}\cdot {\gamma} + {{p}_{it}}^{'}\cdot {\eta} + {{x}_{it}^{'}}\cdot {{p}_{it}}\cdot {\beta}+ {{d}_{it}^{'}}\cdot \delta+ {{d}_{it}^{'}}\cdot {{p}_{it}}\cdot {{\alpha}_{t}}+{{\varepsilon}_{it}} \quad (4)](https://math.jianshu.com/math?formula=%7B%7By%7D_%7Bit%7D%7D%3D%7B%7Ba%7D_%7B0%7D%7D%20%2B%20%7B%7Bx%7D_%7Bit%7D%7D%5E%7B'%7D%5Ccdot%20%7B%5Cgamma%7D%20%2B%20%7B%7Bp%7D_%7Bit%7D%7D%5E%7B'%7D%5Ccdot%20%7B%5Ceta%7D%20%2B%20%7B%7Bx%7D_%7Bit%7D%5E%7B'%7D%7D%5Ccdot%20%7B%7Bp%7D_%7Bit%7D%7D%5Ccdot%20%7B%5Cbeta%7D%2B%20%7B%7Bd%7D_%7Bit%7D%5E%7B'%7D%7D%5Ccdot%20%5Cdelta%2B%20%7B%7Bd%7D_%7Bit%7D%5E%7B'%7D%7D%5Ccdot%20%7B%7Bp%7D_%7Bit%7D%7D%5Ccdot%20%7B%7B%5Calpha%7D_%7Bt%7D%7D%2B%7B%7B%5Cvarepsilon%7D_%7Bit%7D%7D%20%5Cquad%20(4))

其中，![i](https://math.jianshu.com/math?formula=i) 为残疾人个体；![t](https://math.jianshu.com/math?formula=t) 为年份；![{{y}_{it}}](https://math.jianshu.com/math?formula=%7B%7By%7D_%7Bit%7D%7D) 为工作周数或周平均工资；![{{x}_{it}^{'}}](https://math.jianshu.com/math?formula=%7B%7Bx%7D_%7Bit%7D%5E%7B'%7D%7D) 为一系列控制变量，![{{p}_{it}}](https://math.jianshu.com/math?formula=%7B%7Bp%7D_%7Bit%7D%7D) 为时间趋势项；![{{d}_{it}}](https://math.jianshu.com/math?formula=%7B%7Bd%7D_%7Bit%7D%7D)  为是否残疾，其系数为 ![\delta](https://math.jianshu.com/math?formula=%5Cdelta)； ![\alpha_t](https://math.jianshu.com/math?formula=%5Calpha_t) 为随年份变化的实施 ADA 与未实施 ADA 对残疾人工作保护效应的系数。当 ![t\ge1992](https://math.jianshu.com/math?formula=t%5Cge1992)  年时，![\alpha_t](https://math.jianshu.com/math?formula=%5Calpha_t) 量化了实施ADA之后对残疾人工作保护的效果（使用非残疾人作为控制组）；当 ![t<1992](https://math.jianshu.com/math?formula=t%3C1992)  年时，![\alpha_t](https://math.jianshu.com/math?formula=%5Calpha_t)  检验了实施 ADA 之前的各年份中残疾人和非残疾人的工作周数或周平均工资是否在统计上具有显著差异，相当于同时进行了DID模型的 **“共同趋势假设”** 的检验，若 ![\alpha_t](https://math.jianshu.com/math?formula=%5Calpha_t) 系数在1992年之前不显著，表明存在**“共同趋势”**。Stata 命令如下[7]：
```
...
![{ln{y}}={\alpha} + {\sum_{k=1}^{k}} {{\beta} _{k}} \cdot {ln{x}_{k}} + {\frac {1}{2}} {\sum_{k=1}^{k}} {\sum_{m=1}^{m}} {{\gamma} _{km}}\cdot {ln{x}_{k}} \cdot {ln{x}_{m}} \quad (5)](https://math.jianshu.com/math?formula=%7Bln%7By%7D%7D%3D%7B%5Calpha%7D%20%2B%20%7B%5Csum_%7Bk%3D1%7D%5E%7Bk%7D%7D%20%7B%7B%5Cbeta%7D%20_%7Bk%7D%7D%20%5Ccdot%20%7Bln%7Bx%7D_%7Bk%7D%7D%20%2B%20%7B%5Cfrac%20%7B1%7D%7B2%7D%7D%20%7B%5Csum_%7Bk%3D1%7D%5E%7Bk%7D%7D%20%7B%5Csum_%7Bm%3D1%7D%5E%7Bm%7D%7D%20%7B%7B%5Cgamma%7D%20_%7Bkm%7D%7D%5Ccdot%20%7Bln%7Bx%7D_%7Bk%7D%7D%20%5Ccdot%20%7Bln%7Bx%7D_%7Bm%7D%7D%20%5Cquad%20(5))
...
![{y} = {{\alpha} _{1}} + {x}{{\beta}_{1}} + {z}{{\beta} _{2}} + ({x}\cdot {z}){{\beta}_{3}} + {\varepsilon} \quad (6)](https://math.jianshu.com/math?formula=%7By%7D%20%3D%20%7B%7B%5Calpha%7D%20_%7B1%7D%7D%20%2B%20%7Bx%7D%7B%7B%5Cbeta%7D_%7B1%7D%7D%20%2B%20%7Bz%7D%7B%7B%5Cbeta%7D%20_%7B2%7D%7D%20%2B%20(%7Bx%7D%5Ccdot%20%7Bz%7D)%7B%7B%5Cbeta%7D_%7B3%7D%7D%20%2B%20%7B%5Cvarepsilon%7D%20%5Cquad%20(6))

![x](https://math.jianshu.com/math?formula=x)  对 ![y](https://math.jianshu.com/math?formula=y)  的边际影响为：
![\frac{\partial {y}}{\partial {x}} = {{\beta}_{1}} + {z} {{\beta} _{3}}](https://math.jianshu.com/math?formula=%5Cfrac%7B%5Cpartial%20%7By%7D%7D%7B%5Cpartial%20%7Bx%7D%7D%20%3D%20%7B%7B%5Cbeta%7D_%7B1%7D%7D%20%2B%20%7Bz%7D%20%7B%7B%5Cbeta%7D%20_%7B3%7D%7D)
...
![](https://upload-images.jianshu.io/upload_images/8108279-9857a40264ffa305.png)
...
![](https://upload-images.jianshu.io/upload_images/8108279-015fd7be593e903e.png)
...
![{wage}={\alpha} + {tenure}{{\beta} _{1}} + ({tenure}\cdot {tenure}){{\beta} _{2}} + {{x}_{3}}{{\beta} _{3}} +\cdots + {{x}_{8}}{{\beta} _{8}} + {\varepsilon} \quad (7)](https://math.jianshu.com/math?formula=%7Bwage%7D%3D%7B%5Calpha%7D%20%2B%20%7Btenure%7D%7B%7B%5Cbeta%7D%20_%7B1%7D%7D%20%2B%20(%7Btenure%7D%5Ccdot%20%7Btenure%7D)%7B%7B%5Cbeta%7D%20_%7B2%7D%7D%20%2B%20%7B%7Bx%7D_%7B3%7D%7D%7B%7B%5Cbeta%7D%20_%7B3%7D%7D%20%2B%5Ccdots%20%2B%20%7B%7Bx%7D_%7B8%7D%7D%7B%7B%5Cbeta%7D%20_%7B8%7D%7D%20%2B%20%7B%5Cvarepsilon%7D%20%5Cquad%20(7))

*tenure* 对 *wage* 的边际影响为：

![\frac{\partial {wage}}{\partial {tenure}} = {{\beta} _{1}} + 2\cdot {tenure}\cdot {{\beta} _{2}}](https://math.jianshu.com/math?formula=%5Cfrac%7B%5Cpartial%20%7Bwage%7D%7D%7B%5Cpartial%20%7Btenure%7D%7D%20%3D%20%7B%7B%5Cbeta%7D%20_%7B1%7D%7D%20%2B%202%5Ccdot%20%7Btenure%7D%5Ccdot%20%7B%7B%5Cbeta%7D%20_%7B2%7D%7D)
...
![](https://upload-images.jianshu.io/upload_images/8108279-28bd35c7fded9e59.png)
...
![](https://upload-images.jianshu.io/upload_images/8108279-baedbef6dacbf11a.png)
...
![](https://upload-images.jianshu.io/upload_images/8108279-03123cabcd9d53e0.png)
...
![](https://upload-images.jianshu.io/upload_images/7692714-c75dabfe9f11794e.jpg)
