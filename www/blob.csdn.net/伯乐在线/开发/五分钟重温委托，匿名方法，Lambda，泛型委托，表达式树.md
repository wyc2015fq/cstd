# 五分钟重温委托，匿名方法，Lambda，泛型委托，表达式树 - 文章 - 伯乐在线
原文出处： [Sam Xiao](http://www.cnblogs.com/xcj26/p/3536082.html)
...
public partial class WebForm3 : System.Web.UI.Page
{
    ...
    {
        //方法一：
        Funcint,int,int> cAdd1 = (int x, int y) => { return x + y; };
        int result1 = cAdd1(5, 6);
        //方法二：
        Funcint, int, int> cAdd2 = (x, y) => { return x + y; };
        int result2 = cAdd2(5, 6);
        //方法三：
        Funcint, int, int> cAdd3 = (x, y) => x + y;
        int result3 = cAdd2(5, 6);
    }
}
```
...
public partial class WebForm3 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        Expressionint, int, int>> exp = (x, y) => x + y;
        Funcint, int, int> fun = exp.Compile();
        int result = fun(2, 3);
    }
}
```
