# 变量在 PHP7 内部的实现（二） - 文章 - 伯乐在线
原文出处： [Nikita Popov](http://nikic.github.io/2015/06/19/Internal-value-representation-in-PHP-7-part-2.html)   译文出处：[Scholer（@Scholer-L）](http://0x1.im/blog/php/Internal-value-representation-in-PHP-7-part-2.html)
...
[第一部分](http://blog.jobbole.com/96686/)讲了 PHP5 和 PHP7 中关于变量最基础的实现和变化。这里再重复一下，主要的变化就是 zval 不再单独分配内存，不自己存储引用计数。整型浮点型等简单类型直接存储在 zval 中。复杂类型则通过指针指向一个独立的结构体。
