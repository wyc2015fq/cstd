# 10个值得深思的PHP面试问题 - 文章 - 伯乐在线
原文出处： [Toptal](https://www.toptal.com/php/interview-questions )   译文出处：[JackPu](http://www.jackpu.com/-10ge-zhi-de-shen-si-de-phpmian-shi-wen-ti/)
### Q1
第一个问题关于`弱类型`
...
但是`$x = (int)'0xFF';`是否也会变成255呢？显然不是，将一个字符串进行强制类型转换实际上用的是`convert_to_long`,它实际上是将字符串从左向右进行转换，遇到非数字字符则停止。因此`0xFF`到x就停止了。所以`$x=0`
