# C语言有哪些鲜为人知的特性？ - 文章 - 伯乐在线
本文由 [伯乐在线](http://blog.jobbole.com) - [艾凌风](http://www.jobbole.com/members/hanxiaomax) 翻译。未经许可，禁止转载！
英文出处：[Quora](http://www.quora.com/What-are-some-features-of-the-C-programming-language-that-are-not-well-known)。欢迎加入[翻译组](https://github.com/jobbole/translation-project)。
译注：本文摘编自 Quora 的一个热门问答贴。 请在linux系统下测试本文中出现的代码
### [Andrew Weimholt](http://www.quora.com/Andrew-Weimholt) 的回复:
`switch`语句中的`case` 关键词可以放在`if-else`或者是循环当中

C
```
; html-script: false ]switch (a)
{
    case 1:;
      // ...
      if (b==2)
    ...
    case 4:
```
### [Brian Bi](http://www.quora.com/Brian-Bi) 的回复：
#### 1. 声明紧随用途之后
理解声明有一条很简单的法则，不过不是什么“从左向右”这种没道理却到处宣传的法则。这一法则的观点是，一个声明是要告诉你，你所声明的对象要如何使用。例如：

C
```
; html-script: false ]int *p; /* *p是int类型的, 因此p是指向int类型的指针 */
...
C
```
; html-script: false ]int a[5] = {[1] = 2, [4] = 5};
int a[] = {[1] = 2, [4] = 5};
int a[5] = {0, 2, 0, 0, 5};
```
#### 3. 受限指针（C99）：
...
C
```
; html-script: false ]int f(const int* restrict x, int* y) {
    (*y)++;
    int z = *x;
    (*y)--;
    return z;
...
C
```
; html-script: false ]#define cbrt(X) _Generic((X), \
                        long double: cbrtl, \
                        default: cbrt, \
                        float: cbrtf \
                        )(X)
...
C
```
; html-script: false ]#include <stdio.h>
#include <math.h>
int main() {
    printf("%.0f\n",pow(2,747));
    return 0;
...
C
```
; html-script: false ]int Add(int x, int y)
{
    if (y == 0)
        return x;
    else
...
C
```
; html-script: false ]static void foo (void) { }
...
C
```
; html-script: false ]scanf("%s %s", temp, last_name); //典型答案，使用一个临时变量
```


C
```
; html-script: false ]scanf("%s", last_name);
scanf("%s", last_name);
...
C
```
; html-script: false ]scanf("%*s %s", last);
