# C 语言高效编程与代码优化 - 文章 - 伯乐在线
原文出处： [Koushik Ghosh](https://www.codeproject.com/Articles/6154/Writing-Efficient-C-and-C-Code-Optimization)   译文出处：[码农网 – gunner](http://www.codeceo.com/article/c-high-performance-coding.html)
...
## 声明
实际上，在我的项目中，我使用了很多[优化ARM编程的方法](http://www.arm.com/pdfs/DAI0034A_efficient_c.pdf%C2%A0)（该项目是基于ARM平台的），也使用了很多互联网上面的方法。但并不是所有文章提到的方法都能起到很好的作用。所以，我对有用的和高效的方法进行了总结收集。同时，我还修改了其中的一些方法，使他们适用于所有的编程环境，而不是局限于ARM环境。
## 哪里需要使用这些方法？
...
|慢而低效的代码|快而高效的代码| | | | |
|----|----|----|----|----|----|
|```c=getch();switch(c){    case 'A':    {        do something;        break;    }    case 'H':    {        do something;        break;    }    case 'Z':    {        do something;        break;    }}```|123456789101112131415161718|c=getch();switch(c){case'A':{dosomething;break;}case'H':{dosomething;break;}case'Z':{dosomething;break;}}|```c=getch();switch(c){    case 0:    {        do something;        break;    }    case 1:    {        do something;        break;    }    case 2:    {        do something;        break;    }}```|123456789101112131415161718|c=getch();switch(c){case0:{dosomething;break;}case1:{dosomething;break;}case2:{dosomething;break;}}|
|123456789101112131415161718|c=getch();switch(c){case'A':{dosomething;break;}case'H':{dosomething;break;}case'Z':{dosomething;break;}}| | | | |
|123456789101112131415161718|c=getch();switch(c){case0:{dosomething;break;}case1:{dosomething;break;}case2:{dosomething;break;}}| | | | |
## switch语句vs查找表
Switch的应用场景如下：
- 调用一到多个函数
- 设置变量值或者返回一个值
...
|```//Original Code :for(i=0; i<100; i++){    stuff();}for(i=0; i<100; i++){    morestuff();}```|123456789|//Original Code :for(i=0;i<100;i++){stuff();}for(i=0;i<100;i++){morestuff();}|```//It would be better to do:for(i=0; i<100; i++){    stuff();    morestuff();}```|123456|//It would be better to do:for(i=0;i<100;i++){stuff();morestuff();}|
|----|----|----|----|----|----|
|123456789|//Original Code :for(i=0;i<100;i++){stuff();}for(i=0;i<100;i++){morestuff();}| | | | |
|123456|//It would be better to do:for(i=0;i<100;i++){stuff();morestuff();}| | | | |
**函数循环**
...
|```for(i=0; i<3; i++){    something(i);}//is less efficient than```|12345|for(i=0;i<3;i++){something(i);}//is less efficient than|```something(0);something(1);something(2);```|123|something(0);something(1);something(2);|
|----|----|----|----|----|----|
|12345|for(i=0;i<3;i++){something(i);}//is less efficient than| | | | |
|123|something(0);something(1);something(2);| | | | |
...
- 避免调用其他函数：包括那些转而调用C库的函数（比如除法或者[浮点数](http://www.codeceo.com/article/float-number.html)操作函数）。
- 对于简短的函数使用__inline修饰（）。
**内联函数**
