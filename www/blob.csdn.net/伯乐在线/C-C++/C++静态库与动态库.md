# C++静态库与动态库 - 文章 - 伯乐在线
原文出处： [吴秦的博客](http://www.cnblogs.com/skynet/p/3372855.html)
...
![](http://ww3.sinaimg.cn/mw690/5e4d414cgw1es1armenn7j20fe07bt9v.jpg)
### **静态库**
...
![](http://ww3.sinaimg.cn/mw690/5e4d414cgw1es1armpxflj20dg08sdh1.jpg)
**Linux下创建与使用静态库**
Linux静态库命名规则
...
![](http://ww4.sinaimg.cn/mw690/5e4d414cgw1es1arn6stej20fe050wfc.jpg)
...
![](http://ww4.sinaimg.cn/mw690/5e4d414cgw1es1arnjhelj20fe04bq3f.jpg)
- -L：表示要连接的库所在目录
...
![](http://ww3.sinaimg.cn/mw690/5e4d414cgw1es1arnz6r6j20fe09zgmp.jpg)
Build项目即可生成静态库。
**使用静态库**
...
![](http://ww1.sinaimg.cn/mw690/5e4d414cgw1es1arog5xlj20fe0a5jry.jpg)
- 添加StaticMath.h 头文件目录，必须修改包含目录路径。打开工程“属性面板”→”配置属性”→“C/C++”→” 常规”，在“附加包含目录”属性值中，键入StaticMath.h 头文件所在目录的路径或浏览至该目录。
![](http://ww2.sinaimg.cn/mw690/5e4d414cgw1es1b3fkus6j20fe09zwfh.jpg)
编译运行OK。
![](http://ww4.sinaimg.cn/mw690/5e4d414cgw1es1aroqxaqj209l04ejrk.jpg)
如果引用的静态库不是在同一解决方案下的子工程，而是使用第三方提供的静态库lib和头文件，上面的方法设置不了。还有2中方法设置都可行。
方法二：
打开工程“属性面板”→”配置属性”→ “链接器”→ ”命令行”，输入静态库的完整路径即可。
![](http://ww3.sinaimg.cn/mw690/5e4d414cgw1es1arp5xzuj20fe09z3zl.jpg)
方法三：
- “属性面板”→”配置属性”→“链接器”→”常规”，附加依赖库目录中输入，静态库所在目录；
- “属性面板”→”配置属性”→“链接器”→”输入”，附加依赖库中输入静态库名StaticLibrary.lib。
![](http://ww3.sinaimg.cn/mw690/5e4d414cgw1es1arpi529j20fe09z753.jpg)
### **动态库**
通过上面的介绍发现静态库，容易使用和理解，也达到了代码复用的目的，那为什么还需要动态库呢？
**为什么还需要动态库？**
为什么需要动态库，其实也是静态库的特点导致。
- 空间浪费是静态库的一个问题。
![](http://ww1.sinaimg.cn/mw690/5e4d414cgw1es1arpv87yj20df0bhgnw.jpg)
- 另一个问题是静态库对程序的更新、部署和发布页会带来麻烦。如果静态库liba.lib更新了，所以使用它的应用程序都需要重新编译、发布给用户（对于玩家来说，可能是一个很小的改动，却导致整个程序重新下载，全量更新）。
动态库在程序编译时并不会被连接到目标代码中，而是在程序运行是才被载入。不同的应用程序如果调用相同的库，那么在内存里只需要有一份该共享库的实例，规避了空间浪费问题。动态库在程序运行是才被载入，也解决了静态库对程序的更新、部署和发布页会带来麻烦。用户只需要更新动态库即可，增量更新。
![](http://ww4.sinaimg.cn/mw690/5e4d414cgw1es1arq9ypgj20cr0b8766.jpg)
动态库特点总结：
...
```
-shared指定生成动态链接库。
![](http://ww1.sinaimg.cn/mw690/5e4d414cgw1es1arqn234j20fe04wjs9.jpg)
其实上面两个步骤可以合并为一个命令：
...
```
然后运行：./a.out，发现竟然报错了！！！
![](http://ww4.sinaimg.cn/mw690/5e4d414cgw1es1arr3lrjj20fe03xt9f.jpg)
可能大家会猜测，是因为动态库跟测试程序不是一个目录，那我们验证下是否如此：
![](http://ww4.sinaimg.cn/mw690/5e4d414cgw1es1arrem0mj20fe034dge.jpg)
...
![](http://ww1.sinaimg.cn/mw690/5e4d414cgw1es1ars28t3j20fd03st92.jpg)
### **Windows下创建与使用动态库**
**创建动态库（.dll）**
...
![](http://ww2.sinaimg.cn/mw690/5e4d414cgw1es1arsjmwnj20fe09zjsh.jpg)
Build项目即可生成动态库。
**使用动态库**
创建win32控制台测试程序：
...
![](http://ww1.sinaimg.cn/mw690/5e4d414cgw1es1arsp5tkj20fe0a3mxr.jpg)
- 添加DynamicMath.h 头文件目录，必须修改包含目录路径。打开工程“属性面板”→”配置属性”→“C/C++”→” 常规”，在“附加包含目录”属性值中，键入DynamicMath.h 头文件所在目录的路径或浏览至该目录。
![](http://ww4.sinaimg.cn/mw690/5e4d414cgw1es1art3nv3j20fe09ydgt.jpg)
编译运行OK。
![](http://ww1.sinaimg.cn/mw690/5e4d414cgw1es1bjp8rwlj208w03tgls.jpg)
方法二：
-  “属性面板”→”配置属性”→“链接器”→”常规”，附加依赖库目录中输入，动态库所在目录；
![](http://ww3.sinaimg.cn/mw690/5e4d414cgw1es1artiolfj20fe09zt9s.jpg)
- “属性面板”→”配置属性”→“链接器”→”输入”，附加依赖库中输入动态库编译出来的DynamicLibrary.lib。
![](http://ww3.sinaimg.cn/mw690/5e4d414cgw1es1artw15vj20fe09z754.jpg)
这里可能大家有个疑问，动态库怎么还有一个DynamicLibrary.lib文件？即无论是静态链接库还是动态链接库，最后都有lib文件，那么两者区别是什么呢？其实，两个是完全不一样的东西。
![](http://ww2.sinaimg.cn/mw690/5e4d414cgw1es1aru9xq5j20fe073t9p.jpg)
...
![](http://ww1.sinaimg.cn/mw690/5e4d414cgw1es1arull9wj20fe036jry.jpg)
### **总结**
二者的不同点在于代码被载入的时刻不同。
