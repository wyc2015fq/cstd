# 用 C 语言编写一个简单的垃圾回收器 - 文章 - 伯乐在线
本文由 [伯乐在线](http://blog.jobbole.com) - [forever_you](http://www.jobbole.com/members/youyongsong) 翻译。未经许可，禁止转载！
英文出处：[Matthew Plant](http://web.engr.illinois.edu/~maplant2/gc.html)。欢迎加入[翻译组](https://github.com/jobbole/translation-project)。
人们似乎认为编写垃圾回收机制是很难的，是一种只有少数智者和[Hans Boehm(et al)](http://hboehm.info/gc/index.html)才能理解的高深魔法。我认为编写垃圾回收最难的地方就是内存分配，这和阅读K&R所写的malloc样例难度是相当的。
...
C
```
; html-script: false ]    typedef struct header{
        unsigned int  size;
        struct block  *next;
    } header_t;
```
