# 为什么我希望用C而不是C++来实现ZeroMQ（第二篇） - 文章 - 伯乐在线
**译注**：这篇文章可能又会引起 C++ 程序员的诸多不适，就作者本文所描述的问题来看，某些“C++的问题”其实是可以有C++的解决方案的。请参阅[侵入式和非侵入式容器](http://boost-sandbox.sourceforge.net/doc/html/intrusive/intrusive_vs_nontrusive.html)。但是考虑到ZeroMQ是一个很底层的高性能网络库（ZeroMQ的目标是纳入Linux内核中，这也应该是改用C的一大原因，毕竟目前的ZeroMQ是用C++实现的），对错误处理、内存分配次数、并发效率等有着极高的要求，这些特定的限制往往不是所有的C++程序员所常见的应用场景。因此希望各位在阅读时能多从作者的角度来考虑这些问题，而不是一味地批判作者的C++编程实践能力。
在**[上一篇](http://blog.jobbole.com/19647/)**博文中，我已经讨论过了在需要进行严格错误处理的系统底层基础架构的开发中需要避免使用一些C++特性（异常、构造函数、析构函数）。我的结论是，当为C++加上了这样的使用限制后，用C来实现的话会使得代码更简短也更容易阅读。这么做的副作用是消除了对C++运行时库的依赖，而这不应该轻易地去掉，尤其是在嵌入式环境下。
![](http://jbcdn2.b0.upaiyun.com/2012/05/ZeroMQ.jpg)
...
![](http://jbcdn2.b0.upaiyun.com/2012/09/cpp1.jpg)
...
很明显，内存分配的次数会引起性能问题。分配内存所花费的时间可能是无关紧要的——在ZeroMQ中，这并不是关键路径（请参阅关于ZeroMQ中[关键路径](http://www.ituring.com.cn/article/4669)的分析）——但是，内存使用量以及内存碎片带来的问题就非常重要了。这直接影响到CPU缓存是如何填充的，以及由此带来的缓存miss率。回顾一下，到目前为止对物理内存的访问是现代计算机上最慢的操作，这样就知道这种性能影响会有多严重了。
当然，这还没完呢。
...
![](http://jbcdn2.b0.upaiyun.com/2012/09/cpp2.jpg)
...
英文原文：[martin_sustrik](http://www.250bpm.com/blog:8)      编译：[伯乐](http://www.jobbole.com/)在线— [陈舸](http://blog.jobbole.com/26314/)
