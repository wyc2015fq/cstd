# 看漫画学 SELinux 强制策略 - 文章 - 伯乐在线
原文出处： [Daniel J Walsh](https://opensource.com/business/13/11/selinux-policy-guide)   译文出处：[xiaow6](https://linux.cn/article-8326-1.html)
...
![Image showing a cartoon of a cat and dog.](http://jbcdn2.b0.upaiyun.com/2017/03/cb28ccec6da3b2e5f8465d9ee45dbf9e.png)
我们有一类希望能与之交互的客体，我们称之为食物。而我希望能够为食物增加类型：`cat_food` （猫的食物）和 `dog_food`（狗的食物）。
![Cartoon Cat eating Cat Food and Dog eating Dog Food](http://jbcdn2.b0.upaiyun.com/2017/03/e99f895e1c3fc798a93a8cf9c46a1dbc.png)
作为一个策略制定者，我可以说一只狗有权限去吃狗粮（`dog_chow`），而一只猫有权限去吃猫粮（`cat_chow`）。在 SELinux 中我可以将这条规则写入策略中。
![allow cat cat_chow:food eat; allow dog dog_chow:food eat](http://jbcdn2.b0.upaiyun.com/2017/03/f56f0d7d3c36593d3ad77d2f404a221d.png)
`allow cat cat_chow:food eat;`
`允许 猫 猫粮：食物 吃；`
`allow dog dog_chow:food eat;`
`允许 狗 狗粮：食物 吃；`
有了这些规则，内核会允许猫进程去吃打上猫粮标签 `cat_chow` 的食物，允许狗去吃打上狗粮标签 `dog_chow` 的食物。
![Cartoon Cat eating Cat Food and Dog eating Dog Food](http://jbcdn2.b0.upaiyun.com/2017/03/4caa4207c591f38cc90864b7bf7fa905.png)
此外，在 SELinux 系统中，由于禁止是默认规则，这意味着，如果狗进程想要去吃猫粮 `cat_chow`，内核会阻止它。
![](http://jbcdn2.b0.upaiyun.com/2017/03/cc794b7b193ebafed9f6f0dcb3cdfa44.png)
同理，猫也不允许去接触狗粮。
![Cartoon cat not allowed to eat dog fooda](http://jbcdn2.b0.upaiyun.com/2017/03/25908c80c27e6b87ec8990a6cbdee394.png)
**现实例子**
...
![SELinux rule](http://jbcdn2.b0.upaiyun.com/2017/03/4a8375b92fab7f293b77eb2c932764e1.png)
...
![Cartoon of two dogs fido and spot](http://jbcdn2.b0.upaiyun.com/2017/03/65fc49e2e55c9a91d554be2f3bd10ffa.png)
我们将狗粮标记为 `dog_chow:random1(Fido)` 和 `dog_chow:random2(Spot)`。
![SELinux rule](http://jbcdn2.b0.upaiyun.com/2017/03/7e3b2962e4a1bf45dcd5b03f43bc4ec7.png)
MCS 规则声明如果类型强制规则被遵守而且该 MCS 随机标签正确匹配，则访问是允许的，否则就会被拒绝。
Fido (`dog:random1`) 尝试去吃 `cat_chow:food` 被类型强制拒绝了。
![Cartoon of Kernel (Penquin) holding leash to prevent Fido from eating cat food.](http://jbcdn2.b0.upaiyun.com/2017/03/57bf7d0864be157070184eb58a0b8167.png)
Fido (`dog:random1`) 允许去吃 `dog_chow:random1`。
![Cartoon Fido happily eating his dog food](http://jbcdn2.b0.upaiyun.com/2017/03/e0ab7d3afe72882248ab577bae9cef06.png)
Fido (`dog:random1`) 去吃 spot(`dog_chow:random2`)的食物被拒绝。
![Cartoon of Kernel (Penquin) holding leash to prevent Fido from eating spots dog food.](http://jbcdn2.b0.upaiyun.com/2017/03/fa7be62fcacdc878dca0cce37024d162.png)
**现实例子**
...
[视频地址](https://dn-linuxcn.qbox.me/static/video/openshift_selinux.mp4)
### 多级别安全强制
...
![Cartoon of a Greyhound and a Chihuahua](http://jbcdn2.b0.upaiyun.com/2017/03/ae5dabb061d00feab0f66508307d8cd3.png)
...
![Cartoon of a Greyhound dog food and a Chihuahua dog food.](http://jbcdn2.b0.upaiyun.com/2017/03/3333320f312d440b2168932c2e81454c.png)
使用 MLS 策略，我们可以使 MLS 格雷伊猎犬标签支配吉娃娃标签。这意味着 `dog:Greyhound` 允许去吃 `dog_chow:Greyhound` 和 `dog_chow:Chihuahua`。
![SELinux rule](http://jbcdn2.b0.upaiyun.com/2017/03/1be8211541ded844b0f45f96e2df1c9f.png)
但是 `dog:Chihuahua` 不允许去吃 `dog_chow:Greyhound`。
![Cartoon of Kernel (Penquin) stopping the Chihahua from eating the greyhound food. Telling him it would be a big too beefy for him.](http://jbcdn2.b0.upaiyun.com/2017/03/ae4efe08402d894bd5c42f8d58d74076.png)
当然，由于类型强制， `dog:Greyhound` 和 `dog:Chihuahua` 仍然不允许去吃 `cat_chow:Siamese`，即使 MLS 类型 GreyHound 支配 Siamese。
![Cartoon of Kernel (Penquin) holding leash to prevent both dogs from eating cat food.](http://jbcdn2.b0.upaiyun.com/2017/03/18c51a51914cc46c5a37527f99ea62b6.png)
**现实例子**
