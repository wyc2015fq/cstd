# MySQL 主从复制搭建，基于日志（binlog） - 文章 - 伯乐在线
原文出处： [raye.wang](http://raye.wang/2017/04/14/mysqlzhu-cong-fu-zhi-da-jian-ji-yu-ri-zhi-binlog/)
## 什么是MySQL主从复制
...
master*log*file=’edu-mysql-bin.000001′ ##指定Slave从哪个日志文件开始读复制数据（Master上执行命令的结果的File字段）
master*log*pos=429 ## 从哪个POSITION号开始读（Master上执行命令的结果的Position字段）
master*connect*retry=30 ##当重新建立主从连接时，如果连接建立失败，间隔多久后重试。单位为秒，默认设置为60秒，同步延迟调优参数。
##### 四、查看主从同步状态
在Slave中执行命令
...
show slave status;
```
可看到Slave*IO*State为空， Slave*IO*Running和Slave*SQL*Running是No，表明Slave还没有开始复制过程。相反Slave*IO*Running和Slave*SQL*Running是Yes表明已经开始工作了
##### 五、开启主从同步
在Slave中执行命令
...
start slave;
```
查询查看主从同步状态，会发现Slave*IO*Running和Slave*SQL*Running是Yes了，表明开启成功
