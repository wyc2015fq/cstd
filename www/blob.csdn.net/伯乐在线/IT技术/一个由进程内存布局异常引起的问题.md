# 一个由进程内存布局异常引起的问题 - 文章 - 伯乐在线
原文出处： [twoon](http://www.cnblogs.com/catch/p/6370859.html)
前段时间业务反映某类服务器上更新了 bash 之后，ssh 连上去偶发登陆失败，客户端吐出错误信息如下所示：
![](http://jbcdn2.b0.upaiyun.com/2017/02/d84b7cc8abe8f00cb43e7935a7c6180e.png)
该版本 bash 为部门这边所定制，但实现上并没有改动原有逻辑，只是加入了些监控功能，那么这些错误从哪里来？
## 是 bash 的锅吗
...
![](http://jbcdn2.b0.upaiyun.com/2017/02/5c4d3bf1f56b5973e94dedfcc2cf2906.png)
所以现在的问题归结为：为什么目标进程的 brk 的区域突然那么小了，先检查一下 bash 的内存布局：
![](http://jbcdn2.b0.upaiyun.com/2017/02/536c5bfc9554e0175da75e9bb130d178.png)
...
![](http://jbcdn2.b0.upaiyun.com/2017/02/4bd5ea19321d97e0131cb70810a5e907.png)
看出来不同了没有，两个 bash 进程都是 64 位的，不同在于前者是 sshd 起的进程后者是我手动在终端上起起来的，手动 cat /proc/self/maps 看了下 64 位的 cat 的进程的内存布局也是正常的：
![](http://jbcdn2.b0.upaiyun.com/2017/02/67eed23e6c7d6785102261c3924dd8cd.png)
那 sshd 进程呢？
![](http://jbcdn2.b0.upaiyun.com/2017/02/4d9c524d4eb84ac49f8f9deff7536b83.png)
sshd 进程也不正常，而且意外发现 sshd 是 32 位的，于是写了个测试程序：
![](http://jbcdn2.b0.upaiyun.com/2017/02/e8556f0eb6844b8542466052adddf0e7.png)
...
现在我们来看看内核出了什么问题，目标系统版本如下，经过咨询系统组的人确认，该系统基于 centos 6.5: [http://vault.centos.org/6.5/centosplus/Source/SPackages/kernel-2.6.32-431.el6.centos.plus.src.rpm](http://vault.centos.org/6.5/centosplus/Source/SPackages/kernel-2.6.32-431.el6.centos.plus.src.rpm)
![](http://jbcdn2.b0.upaiyun.com/2017/02/5d6a63002e7ca2a459522b90f2698286.png)
首先看看 arch/x86/mm/mmap.c: arch_pick_mmap_layout() 这个函数，它的作用是根据进程和当前系统的设置初化 mmap 相关的入口：
![](http://jbcdn2.b0.upaiyun.com/2017/02/3c3cabcc5e08d4f251f6c5eda8fc97a9.png)
Exec-shield 是一类安全功能的开关，由红帽在很多年前主导搞的对 buffer overflow 攻击的一系列增强，具体可以参看这几个连接 [1](https://lwn.net/Articles/31032/)、[2](http://people.redhat.com/mingo/exec-shield/docs/WHP0006US_Execshield.pdf)，[3](https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/5/html/Deployment_Guide/s3-proc-sys-kernel.html)，[4](https://lists.gnu.org/archive/html/gcl-devel/2003-11/msg00167.html)，exec shield 在实现和使用上一直有问题，也破坏了有些旧程序的兼容性【6】，因此一直没进主干，只在 redhat 家族 6.x 及其派生系统上使用。
...
![](http://jbcdn2.b0.upaiyun.com/2017/02/d4d3581541c0b8744b9aa54c51278ea0.png)
...
![](http://jbcdn2.b0.upaiyun.com/2017/02/adf71a07d878e8e3bf8090a47396e08a.png)
上面这种针对 exec 内存的分配方式实际上很容易引起冲突，redhat 在这里也是打了不少补丁，参看[1](https://patchwork.ozlabs.org/patch/42996/)，[2](https://lwn.net/Articles/372580/)，[3](http://seclists.org/dailydave/2007/q2/107)。
## 问题并没有解决
...
4、加载动态连接器并跳过去执行。
![](http://jbcdn2.b0.upaiyun.com/2017/02/ea526e419fce616aca145288f9807c1b.png)
...
![](http://jbcdn2.b0.upaiyun.com/2017/02/5fb19d4a9cf3c45a6d6cfc81d2e8e010.png)
注意其中 1039 行，bprm->mm 表示新的内存空间(旧的还在，但马上就要释放并切换新的)，这里需要对新的内存空间进行设置，参看： fs/exec.c: exec_mmap()
![](http://jbcdn2.b0.upaiyun.com/2017/02/86b607982a191c17e877670612fe17e6.png)
...
## 参考
【0】[https://en.wikipedia.org/wiki/C_dynamic_memory_allocation](https://en.wikipedia.org/wiki/C_dynamic_memory_allocation)
【1】[https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/5/html/Tuning_and_Optimizing_Red_Hat_Enterprise_Linux_for_Oracle_9i_and_10g_Databases/sect-Oracle_9i_and_10g_Tuning_Guide-Growing_the_Oracle_SGA_to_2.7_GB_in_x86_Red_Hat_Enterprise_Linux_2.1_Without_VLM-Linux_Memory_Layout.html](https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/5/html/Tuning_and_Optimizing_Red_Hat_Enterprise_Linux_for_Oracle_9i_and_10g_Databases/sect-Oracle_9i_and_10g_Tuning_Guide-Growing_the_Oracle_SGA_to_2.7_GB_in_x86_Red_Hat_Enterprise_Linux_2.1_Without_VLM-Linux_Memory_Layout.html)
【2】understanding the linux kernel, page 819, flexible memory region layout: [https://books.google.com.hk/books?id=h0lltXyJ8aIC&pg=PT925&lpg=PT925&dq=linux+flexible+memory&source=bl&ots=gO7rIYb8HR&sig=pirB5pswdHFHSljy57EksxS3ABw&hl=en&sa=X&ved=0ahUKEwjpkfa-2_rRAhVGFJQKHcETDSUQ6AEITDAH#v=onepage&q=linux%20flexible%20memory&f=false](https://books.google.com.hk/books?id=h0lltXyJ8aIC&pg=PT925&lpg=PT925&dq=linux+flexible+memory&source=bl&ots=gO7rIYb8HR&sig=pirB5pswdHFHSljy57EksxS3ABw&hl=en&sa=X&ved=0ahUKEwjpkfa-2_rRAhVGFJQKHcETDSUQ6AEITDAH#v=onepage&q=linux%20flexible%20memory&f=false)
...
【4】[http://lxr.free-electrons.com/source/arch/x86/include/asm/processor.h#L770](http://lxr.free-electrons.com/source/arch/x86/include/asm/processor.h#L770)
【5】 [https://access.redhat.com/blogs/766093/posts/1975793](https://access.redhat.com/blogs/766093/posts/1975793)
【6】[https://lwn.net/Articles/31032/](https://lwn.net/Articles/31032/)
【7】[https://lwn.net/Articles/454949/](https://lwn.net/Articles/454949/)
【8】[http://lxr.free-electrons.com/source/fs/binfmt_elf.c#L549](http://lxr.free-electrons.com/source/fs/binfmt_elf.c#L549)
【9】[http://lxr.free-electrons.com/source/fs/binfmt_elf.c#L563](http://lxr.free-electrons.com/source/fs/binfmt_elf.c#L563)
【10】类似问题： [https://bugzilla.redhat.com/show_bug.cgi?id=870914](https://bugzilla.redhat.com/show_bug.cgi?id=870914)[https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=522849](https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=522849)
