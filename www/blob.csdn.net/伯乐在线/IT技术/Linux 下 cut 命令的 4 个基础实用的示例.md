# Linux 下 cut 命令的 4 个基础实用的示例 - 文章 - 伯乐在线
原文出处： [Sylvain Leroux](https://linuxhandbook.com/cut-command/)   译文出处：[Linux中国/Chang Liu](https://linux.cn/article-9895-1.html)
...
![Cut Linux 命令示例](http://jbcdn2.b0.upaiyun.com/2018/08/5902a39fabff80e97a3156ede1ace4b2.jpg)
### Linux 下 cut 命令的 4 个实用示例
假如你想，你可以观看下面的视频，视频中解释了本文中我列举的 cut 命令的使用例子。
- [https://www.youtube.com/PhE_cFLzVFw](https://www.youtube.com/PhE_cFLzVFw)
### 1、 作用在一系列字符上
...
```
`--zero-terminated (-z)`：
使用 [NUL 字符](https://en.wikipedia.org/wiki/Null_character) 来作为行终止符，而不是 [新行newline字符](https://en.wikipedia.org/wiki/Newline)。当你的数据包含 新行字符时， `-z` 选项就特别有用了，例如当处理文件名的时候（因为在文件名中新行字符是可以使用的，而 NUL 则不可以）。
