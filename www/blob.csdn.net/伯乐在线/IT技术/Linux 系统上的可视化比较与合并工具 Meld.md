# Linux 系统上的可视化比较与合并工具 Meld - 文章 - 伯乐在线
原文出处： [Ansh](https://www.howtoforge.com/tutorial/beginners-guide-to-visual-merge-tool-meld-on-linux/)   译文出处：[Linux中国/GitFuture](https://linux.cn/article-8402-1.html)
...
```
或者你也可以用系统自带的包管理软件下载这个工具。比如在 Ubuntu 上，你可以用 Ubuntu 软件中心（Ubuntu Software Center），或者用 [Ubuntu 软件](https://www.howtoforge.com/tutorial/ubuntu-16-04-lts-overview/)，它从 Ubuntu 16.04 版本开始取代了 Ubuntu 软件中心。
...
### 使用 Meld
装好了软件，就可以看到类似这样的画面：
![Meld started](http://jbcdn2.b0.upaiyun.com/2017/09/dbde9a54537cff686561f4ea367befd3.png)
有三个选项：比较文件（File comparison），比较目录（Directory comparison）以及版本控制视图（Version control view）。
点击“比较文件”选项，就可以选择需要比较的文件：
![Meld file comparison](http://jbcdn2.b0.upaiyun.com/2017/09/060bead6118e149e3a57c576d37d1376.png)
就像上面的截图那样明白，Meld 也可以进行三向比较，但是在这一系列文章的第一部分，我们只会讲更常用的双向比较。
接着，选择你想要比较的文件，点击“比较”（Compare）按钮。软件会在两边分别打开两个文件，高亮不同的行（以及不同的部分）。
![Compare files in Meld](http://jbcdn2.b0.upaiyun.com/2017/09/7eb08063baf7cb031a6a6f4b445926c3.png)
...
![Meld advanced file comparison](http://jbcdn2.b0.upaiyun.com/2017/09/3a1b7484b6c3766fda278db4b7d7f970.png)
在讨论这些变化前，这里提一下， Meld 的界面中有几个区域，可以给出文件之间的差异，让概况变得直观。这里特别需要注意窗口的左右两边垂直的栏。比如下面这个截图：
![Visual Comparison](http://jbcdn2.b0.upaiyun.com/2017/09/4ea9541f9c58840d7e096fa4bebd7813.png)
...
![File changes visualized in Meld](http://jbcdn2.b0.upaiyun.com/2017/09/d58833eab9d7ba00aabab813c6e0969b.png)
![Meld Example 2](http://jbcdn2.b0.upaiyun.com/2017/09/6f8a5ac900f6e386e0f4982445ee0da8.png)
![Meld Example 3](http://jbcdn2.b0.upaiyun.com/2017/09/92013c7ba1d58181b101d6d566b01afb.png)
接着，我们滑动文件，从一个改动跳到另一个。但是，当要比较的文件很大时，这会耗一点时间，当你想要滑动文件跳到一个改动的位置时，也会变得很困难。如果是这种情况的话，你可以用工具栏的橙色箭头，就在编辑区域的上方：
![Go to next change in Meld](http://jbcdn2.b0.upaiyun.com/2017/09/98ecd8d98e47b079342b86dd9b0867b3.png)
...
### 总结
我们刚刚了解到的不过是皮毛，因为 Meld 还能做很多事情。考虑到这是教程系列的第一部分，这也挺不错的。这仅仅是让你了解 Meld 的作用，你可以配置它，忽略一些特定类型的改动，让它移动，复制或者删除文件之间的个别差异，也可以从命令行启动它。在即将推出的系列教程中，我们将会讲述所有这些重要功能。
