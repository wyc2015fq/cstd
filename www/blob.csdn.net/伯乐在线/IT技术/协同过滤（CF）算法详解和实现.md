# 协同过滤（CF）算法详解和实现 - 文章 - 伯乐在线
原文出处： [Poll的笔记](http://www.cnblogs.com/maybe2030/p/4636341.html)
# 1 集体智慧和协同过滤
## 1.1 什么是集体智慧（社会计算）？
...
![01](http://jbcdn2.b0.upaiyun.com/2016/06/e5d1fb33b17dd6825f1ee24dbe0d85db.gif)
可以看出，当 n=2 时，欧几里德距离就是平面上两个点的距离。
当用欧几里德距离表示相似度，一般采用以下公式进行转换：距离越小，相似度越大
![02](http://jbcdn2.b0.upaiyun.com/2016/06/f13bdadb0bbe29966e9e4c4e01677bc0.gif)
- 皮尔逊相关系数（Pearson Correlation Coefficient）
皮尔逊相关系数一般用于计算两个定距变量间联系的紧密程度，它的取值在 [-1，+1] 之间。
![03](http://jbcdn2.b0.upaiyun.com/2016/06/f1389a8d6bed7c00cca3d1f305d13445.gif)
sx, sy是 x 和 y 的样品标准偏差。
- Cosine 相似度（Cosine Similarity）
Cosine 相似度被广泛应用于计算文档数据的相似度：
![04](http://jbcdn2.b0.upaiyun.com/2016/06/9e45bbb9e4031d69306811b6ab6041c9.gif)
- Tanimoto 系数（Tanimoto Coefficient）
Tanimoto 系数也称为 Jaccard 系数，是 Cosine 相似度的扩展，也多用于计算文档数据的相似度：
![05](http://jbcdn2.b0.upaiyun.com/2016/06/e2314c51f8b4f07fa5a71415dc4fbe01.gif)
相似邻居的计算
...
![06](http://jbcdn2.b0.upaiyun.com/2016/06/1c7be95b83f1702fbd8486c88df333a6.gif)
图 1.相似邻居计算示意图
## 2.3 计算推荐
...
![07](http://jbcdn2.b0.upaiyun.com/2016/06/1351c47dbcbd11dd658c9eed185bcce9.gif)
图 2 基于用户的 CF 的基本原理
基于物品的 CF（Item CF）
...
![08](http://jbcdn2.b0.upaiyun.com/2016/06/fbee0ba334100ae6cd148bca7f9466e8.gif)
图 3 基于物品的 CF 的基本原理
User CF vs. Item CF
...
测度包括：
Pearson correlation：
![09](http://jbcdn2.b0.upaiyun.com/2016/06/b860bea99e34934ec52c616ca5d4ae921.jpg)
其中¯rm 和¯rn 分别表示电影m 和n 获得的评分平均值，而Pmn 表示对电影m 和n 都提供了评分的用户集合，也即Pmn = Pm ∩ Pn 。
Cosine：
![10](http://jbcdn2.b0.upaiyun.com/2016/06/8d8e353b98d5191d5ceea1aa3eb05d439.jpg)
Adjusted Cosine：
![11](http://jbcdn2.b0.upaiyun.com/2016/06/7bfc85c0d74ff05806e0b5a0fa0c1df114.jpg)
其中¯rv 表示用户v 的评分平均值。
2. 选择邻居
为了预测用户u 对电影m 的评分值，我们首先从Pu 中选取与电影m有最高相似度的特定数量的电影，这些电影形成u−m 对的邻居（neighborhood），记为N(m; u) 。
3. 产生预测值
用户u 对电影m 的评分预测为上步获得的邻居N(m; u) 中评分的加权平均值：
![12](http://jbcdn2.b0.upaiyun.com/2016/06/c8b2f17833a4c73bb20f88876219ddcd14.jpg)
其中bu;n 为用户u 对电影n 的基准预测评分。这里的基准模型可以是任何可以产生预测评分的模型。
按照上述过程，在MATLAB仿真环境下得到的RMSE=1.0776，这里取得邻居个数为10。
下图为邻居个数的选取（0~20）对RMSE的影响曲线：
![13](http://jbcdn2.b0.upaiyun.com/2016/06/7e51746feafa7f2621f71943da8f603c12.jpg)
...
# 5 参考资料
- [Collective Intelligence in Action:](http://www.amazon.com/Collective-Intelligence-Action-Satnam-Alag/dp/1933988312) 详细介绍了如何利用集体智慧构建智能应用。
- [Toward the next generation of recommender systems: a survey of the state-of-the-art and possible extensions.](http://portal.acm.org/citation.cfm?id=1070751) Adomavicius, G. and Tuzhilin 在 2005 年发表的一片文章，详细总结了推荐引擎的发展和存在的问题
- [Collaborative_Filtering](http://en.wikipedia.org/wiki/Collaborative_Filtering)：Wikipedia 上对于协同过滤的介绍和相关论文。
- [Item-based collaborative filtering recommendation algorithms](http://portal.acm.org/citation.cfm?id=372071)：Amazon 最早提出 Item CF 的推荐策略的论文
- [Correlation and dependence](http://en.wikipedia.org/wiki/Correlation_and_dependence)：Wikipedia 上关联和相似度计算的介绍。
- [Tanimoto系数](http://en.wikipedia.org/wiki/Tanimoto_coefficient#Tanimoto_coefficient_.28extended_Jaccard_coefficient.29)：Wikipedia 上 Tanimoto 系数计算的介绍。
- [Cosine相似度](http://en.wikipedia.org/wiki/Cosine_similarity)：Wikipedia 上 Cosine 相似度计算的介绍。
- [推荐系统的覆盖率](http://blurkerlab.blogspot.com/2008/02/blog-post_3022.html)：关于推荐覆盖率的计算方法的介绍
- [Slope One](http://en.wikipedia.org/wiki/Slope_One)：Wikipedia 上 Slope One 推荐方法的介绍。
- [Slope One Predictors for Online Rating-Based Collaborative Filtering](http://www.daniel-lemire.com/fr/abstracts/SDM2005.html)：提出 Slope One 的论文，对 Slope One 预测方法进行全面深入的介绍。
- [xlvector – Recommender System](http://xlvector.net/blog/)：项亮的博客，关注与推荐系统各个侧面和各个维度的问题。
- [Apache Mahout简介](http://www.ibm.com/developerworks/cn/java/j-mahout/index.html)：Mahout 的创始者 Grant Ingersoll 介绍了机器学习的基本概念，并演示了如何使用 Mahout 来实现文档集群、提出建议和组织内容。
- [Apache Mahout](http://lucene.apache.org/mahout/)：Apache Mahout 项目的主页，搜索关于 Mahout 的所有内容。
- [Apache Mahout 推荐算法](https://cwiki.apache.org/confluence/display/MAHOUT/Recommender+Documentation)：给出 Mahout 中提供推荐计算的框架和安装指南
- [基于Apache Mahout构建社会化推荐引擎](http://www.ibm.com/developerworks/cn/java/j-lo-mahout/)：笔者 09 年发布的一篇关于基于 Mahout 实现推荐引擎的 developerWorks 文章，其中详细介绍了 Mahout 的安装步骤，并给出一个简单的电影推荐引擎的例子。
