# 深入学习 Redis（4）：哨兵 - 文章 - 伯乐在线
原文出处： [编程迷思](https://www.cnblogs.com/kismetv/p/9609938.html)
## 前言
在 [深入学习Redis（3）：主从复制](http://blog.jobbole.com/114194/) 中曾提到，Redis主从复制的作用有数据热备、负载均衡、故障恢复等；但主从复制存在的一个问题是故障恢复无法自动化。本文将要介绍的哨兵，它基于Redis主从复制，主要作用便是解决主节点故障恢复的自动化问题，进一步提高系统的高可用性。
文章主要内容如下：首先介绍哨兵的作用和架构；然后讲述哨兵系统的部署方法，以及通过客户端访问哨兵系统的方法；然后简要说明哨兵实现的基本原理；最后给出关于哨兵实践的一些建议。文章内容基于Redis 3.0版本。
## 系列文章
[深入学习Redis（1）：Redis内存模型](http://blog.jobbole.com/113789/)
[深入学习Redis（2）：持久化](http://blog.jobbole.com/114093/)
[深入学习Redis（3）：主从复制](http://blog.jobbole.com/114194/)
# 一、作用和架构
## 1.  作用
...
## 2.  架构
典型的哨兵架构图如下所示：
![](http://jbcdn2.b0.upaiyun.com/2018/09/8b0360c2b6eba06c316535320b2aecd2.png)
它由两部分组成，哨兵节点和数据节点：
...
![](http://jbcdn2.b0.upaiyun.com/2018/09/bf34c119fb79b61250d94a1d6aa7ce26.png)
## 2.  部署哨兵节点
哨兵节点本质上是特殊的Redis节点。
...
![](http://jbcdn2.b0.upaiyun.com/2018/09/a69a41455df2b730275ed61c3501a4ed.png)
此时如果查看哨兵节点的配置文件，会发现一些变化，以26379为例：
![](http://jbcdn2.b0.upaiyun.com/2018/09/466d088ddc678a5d4fcd7b4a91c6d9fc.png)
...
（1）首先，使用kill命令杀掉主节点：
![](http://jbcdn2.b0.upaiyun.com/2018/09/307cc9dbdc5df1e5dcd5ce77b2c1182e.png)
（2）如果此时立即在哨兵节点中使用info Sentinel命令查看，会发现主节点还没有切换过来，因为哨兵发现主节点故障并转移，需要一段时间。
![](http://jbcdn2.b0.upaiyun.com/2018/09/7fea5421d4c171ea55d9ff9a3a4c0112.png)
（3）一段时间以后，再次在哨兵节点中执行info Sentinel查看，发现主节点已经切换成6380节点。
![](http://jbcdn2.b0.upaiyun.com/2018/09/d4302e7e98fbe1fa66d391a1280f186a.png)
...
![](http://jbcdn2.b0.upaiyun.com/2018/09/7df7d6cccdf7e35e41e9a0c4643d4aea.png)
...
![](http://jbcdn2.b0.upaiyun.com/2018/09/befad1757e6a99ccc1752c71daa83ed8.png)
## 4.  总结
哨兵系统的搭建过程，有几点需要注意：
...
![](http://jbcdn2.b0.upaiyun.com/2018/09/d8c3c359c63cda07cd8fc01317866fd2.png)
...
![](http://jbcdn2.b0.upaiyun.com/2018/09/8a6ded5ebf9eab9d29ee1926b1a4a0a4.png)
# 五、配置与实践建议
## 1.  配置
下面介绍与哨兵相关的几个配置。
