# 用 Nginx + Lua(OpenResty) 开发高性能 Web 应用 - 文章 - 伯乐在线
原文出处： [张开涛](http://jinnianshilongnian.iteye.com/blog/2280928)
...
**1****、负载均衡**
**![](http://jbcdn2.b0.upaiyun.com/2016/08/9175825c4649e57eb5af0cf302d88a26.png)**
...
![](http://jbcdn2.b0.upaiyun.com/2016/08/0f4e3ac865406b816d4adc9bbea1b35d.png)
...
![](http://jbcdn2.b0.upaiyun.com/2016/08/7038f626b60cec8c0aeda6fea8ecc984.png)
如上所示，主要有三种应用模式：
...
2.2、第二张图，是读取本机文件系统，如静态资源合并：比如访问[http://item.jd.com/1856584.html](http://item.jd.com/1856584.html)，查看源码会发现【//misc.360buyimg.com/jdf/1.0.0/unit/??ui-base/1.0.0/ui-base.css,shortcut/2.0.0/shortcut.css,global-header/1.0.0/global-header.css,myjd/2.0.0/myjd.css,nav/2.0.0/nav.css,shoppingcart/2.0.0/shoppingcart.css,global-footer/1.0.0/global-footer.css,service/1.0.0/service.css”/>】这种请求，即多个请求合并为一个发给服务端，服务端进行了文件资源的合并；
![](http://jbcdn2.b0.upaiyun.com/2016/08/8c02019416d41486dc0ed27f3854a414.png)
...
还一些业务型应用场景如下图所示
![](http://jbcdn2.b0.upaiyun.com/2016/08/f278a4dad58623b0c30e36251194aca7.png)
...
比如商品页面的架构我们可以这样：
![](http://jbcdn2.b0.upaiyun.com/2016/08/b55b860664a361bc310d4b8f1566d453.png)
首先接收到商品变更消息，商品页面同步Worker会根据消息维度生成相关的页面推送到Nginx服务器；Nginx应用再通过SSI输出。目前京东商品详情页没有再采用这种架构，具体架构可以参考《[构建需求响应式亿级商品详情页](http://blog.jobbole.com/90359/)》。
...
**![](http://jbcdn2.b0.upaiyun.com/2016/08/738dea0892641b8ceab075d9abd948ca.png)**此处可以看到，不管是图二还是图三架构，都需要Worker去进行数据推送；假设本机数据丢了可怎么办？因此实际大部分应用不会是完全单机闭环的，而是会采用如下架构：
**![](http://jbcdn2.b0.upaiyun.com/2016/08/18eae831d065c774ffc21711b46faeee.png)**
![](http://jbcdn2.b0.upaiyun.com/2016/08/dbec2c779062472e75553a129fd9e281.png)
...
![](http://jbcdn2.b0.upaiyun.com/2016/08/65535dedfbdfa19325a3076fff148813.png)
即第一步先读取分布式存储（JIMDB是京东的一个分布式缓存/存储系统，类似于Redis）；如果不命中则回源到Tomcat集群（其会调用数据库、服务总线获取相关数据）来获取相关数据。可以参考《[构建需求响应式亿级商品详情页](http://blog.jobbole.com/90359/)》来获取更详细的架构实现。
JIMDB集群会进行多机房主从同步，各自机房读取自己机房的从JIMDB集群，如下图
![](http://jbcdn2.b0.upaiyun.com/2016/08/7078838bd912cce633abf9653e9b485c.png)
**4****、接入网关**
接入网关也可以叫做接入层，即接收到流量的入口，在入口我们可以进行如下事情：
![](http://jbcdn2.b0.upaiyun.com/2016/08/7eb5f6ad8bd03956c80f3236e9488505.png)
...
京东的交易大Nginx节点、无线部门正在开发的无线Nginx网关、和单品页统一服务都是接入网关的实践，而单品页统一服务架构可以参考《[京东商品详情页服务闭环实践](http://jinnianshilongnian.iteye.com/blog/2258111%20)》。
**5****、Web****应用**
此处所说的Web应用指的是页面模板渲染类型应用或者API服务类型应用；比如京东列表页/商品详情页就是一个模板渲染类型的应用，核心业务逻辑都是使用Lua写的，部署到Nginx容器。目前核心业务代码行数有5000多行，模板页面有2000多行，涉及到大量的计算逻辑，性能数据可以参考《[构建需求响应式亿级商品详情页](http://blog.jobbole.com/90359/)》。
![](http://jbcdn2.b0.upaiyun.com/2016/08/ad15d22f8a82f42e6664ec20bbce45c2.png)
...
### 参考源码：[Nginx+Lua(OpenResty) HelloWorld](/blog/2289728)
...
- [跟我学Nginx+Lua开发](/blog/2190344)
- [构建需求响应式亿级商品详情页](/blog/2235572)
- [京东商品详情页服务闭环实践](/blog/2258111%20)
