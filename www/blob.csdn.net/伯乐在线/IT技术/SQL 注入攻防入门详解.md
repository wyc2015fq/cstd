# SQL 注入攻防入门详解 - 文章 - 伯乐在线
原文出处： [滴答的雨](http://www.cnblogs.com/heyuquan/archive/2012/10/31/2748577.html)
...
## 1)   输入正常数据，效果如图：
![image](http://jbcdn2.b0.upaiyun.com/2016/08/fc72729becce980308a5d6210b475e7f.png)
合并的SQL为：
SELECT COUNT(*) FROM Login WHERE UserName=’admin’ AND Password=’123456′
## 2)   输入注入数据：
如图，即用户名为：用户名：admin’—，密码可随便输入
![image](http://jbcdn2.b0.upaiyun.com/2016/08/7a893e0c67be0e62c64a2ba5a8c40921.png)
合并的SQL为：
...
![image](http://jbcdn2.b0.upaiyun.com/2016/08/5243e5a165e91a5e847a83665ed15504.png)
## 4)   查询当前用户的数据库权限
...
看下效果图：
设置为一般性错误：
![image](http://jbcdn2.b0.upaiyun.com/2016/08/ae08be6d163a0b4f8964c657ff258c61.png)
设置为：
![image](http://jbcdn2.b0.upaiyun.com/2016/08/9cf528e71c2bfaff3840ce961b7bf211.png)
# 3.   把危险的和不必要的存储过程删除
...
|Xp_regaddmultistringXp_regdeletekeyXp_regdeletevalueXp_regenumvaluesXp_regreadXp_regremovemultistringXp_regwrite|可以访问注册表的存储过程|
|Sp_OACreateSp_OADestroySp_OAGetErrorInfoSp_OAGetPropertySp_OAMethodSp_OASetPropertySp_OAStop|如果你不需要请丢弃OLE自动存储过程|
# 4.   非参数化SQL与参数化SQL
## 1)   非参数化（动态拼接SQL）
...
## 3)   参数化查询示例
效果如图：
![image](http://jbcdn2.b0.upaiyun.com/2016/08/6e4bd4f96fbcbcff8a816d243a24d92e.png)
参数化关键代码：
...
```
注入截图如下：
![image](http://jbcdn2.b0.upaiyun.com/2016/08/d713b459af1e1d66d806cdc4d8494e73.png)
# 6.   专业的SQL注入工具及防毒软件
情景1
A：“丫的，又中毒了……”
B：“我看看，你这不是裸机在跑吗？”
...
截图如下：
![image](http://jbcdn2.b0.upaiyun.com/2016/08/8c80301cdd1100b59aff93fb758e15f1.png)
有两种将通配符转义为普通字符的方法：
...
[Sql Server参数化查询之**where in**和like实现详解](http://wenku.baidu.com/view/9f19df7701f69e3143329421.html) —–讲述6种参数化实现方案
...
原本打算10月中旬完成的这篇博文的，后面答应帮一个妹纸做一个小课程设计，所以这篇博文就中断了，直到昨天才完成博文。另外妹纸的课程设计我也提供下载（一个SQL参数化应用的小程序），采用的是三层架构，但时间比较赶架构还不够规范。 [下载……](http://files.cnblogs.com/heyuquan/SQL%E5%8F%82%E6%95%B0%E5%8C%96%E5%BA%94%E7%94%A8%E7%9A%84%E5%B0%8F%E7%A8%8B%E5%BA%8F.rar)
哈哈这边有个热心娃担心妹纸咯，我也想到这个问题，所以课程设计没有标题目，只提供下载……
![](http://jbcdn2.b0.upaiyun.com/2016/08/ac1f9817c9a3ffd0833cd153857bad88.jpg)
