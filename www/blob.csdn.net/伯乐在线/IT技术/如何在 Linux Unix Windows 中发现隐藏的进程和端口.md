# 如何在 Linux/Unix/Windows 中发现隐藏的进程和端口 - 文章 - 伯乐在线
原文出处： [Vivek Gite](https://www.cyberciti.biz/tips/linux-unix-windows-find-hidden-processes-tcp-udp-ports.html)   译文出处：[Linux中国/fan Li](https://linux.cn/article-9288-1.html)
...
- 进程相关的技术，包括将 `/proc` 目录与 [/bin/ps](https://www.cyberciti.biz/faq/show-all-running-processes-in-linux/) 命令的输出进行比较。
- 系统相关的技术，包括将 [/bin/ps](https://www.cyberciti.biz/faq/show-all-running-processes-in-linux/) 命令的输出结果同从系统调用方面得到的信息进行比较。
...
### 如何安装 unhide
首先建议你在只读介质上运行这个工具。如果使用的是 Ubuntu 或者 Debian 发行版，输入下述的 [apt-get](https://www.cyberciti.biz/tips/linux-debian-package-management-cheat-sheet.html)/[apt](//www.cyberciti.biz/faq/ubuntu-lts-debian-linux-apt-command-examples/) 命令以安装 Unhide：
...
输入以下的 [yum](https://www.cyberciti.biz/faq/rhel-centos-fedora-linux-yum-command-howto/) 命令（CentOS/RHEL [6.x](https://www.cyberciti.biz/faq/fedora-sl-centos-redhat6-enable-epel-repo/) 或 [7.x](https://www.cyberciti.biz/faq/installing-rhel-epel-repo-on-centos-redhat-7-x/) 上首先打开 EPEL 仓库）：
...
以下是来自 man 页面的介绍：
> 
`unhide-tcp` 取证工具通过对所有可用的 TCP/IP 端口进行暴力求解的方式，辨别所有正在监听，却没有列入 [/bin/netstat](https://www.cyberciti.biz/tips/linux-display-open-ports-owner.html) 或者 [/bin/ss](https://www.cyberciti.biz/tips/linux-investigate-sockets-network-connections.html) 命令输出的 TCP/IP 端口身份。
