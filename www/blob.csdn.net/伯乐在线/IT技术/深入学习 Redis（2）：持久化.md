# 深入学习 Redis（2）：持久化 - 文章 - 伯乐在线
原文出处： [编程迷思](https://www.cnblogs.com/kismetv/p/9137897.html)
## 前言
在上一篇文章中，介绍了[Redis的内存模型](http://blog.jobbole.com/113789/)，从这篇文章开始，将依次介绍Redis高可用相关的知识——持久化、复制(及读写分离)、哨兵、以及集群。
...
![](http://jbcdn2.b0.upaiyun.com/2018/06/662665d4fe41bbae40cff2548bc1c4ee.png)
而bgsave命令会创建一个子进程，由子进程来负责创建RDB文件，父进程(即Redis主进程)则继续处理请求。
![](http://jbcdn2.b0.upaiyun.com/2018/06/fbe173ede64bc88f834bc25d89f8f44f.png)
此时服务器执行日志如下：
![](http://jbcdn2.b0.upaiyun.com/2018/06/fbb24a124ecb5ddf9c02d425ddc488d2.png)
...
![](http://jbcdn2.b0.upaiyun.com/2018/06/940eea59eb4ff44da2ba1a95bf28ab2c.png)
...
![](http://jbcdn2.b0.upaiyun.com/2018/06/969e9ec6f50fedbdd85b68d2becb5b9a.png)
**其他自动触发机制**
除了save m n 以外，还有一些其他情况会触发bgsave：
- 在主从复制场景下，如果从节点执行全量复制操作，则主节点会执行bgsave命令，并将rdb文件发送给从节点
- 执行shutdown命令时，自动执行rdb持久化，如下图所示：
![](http://jbcdn2.b0.upaiyun.com/2018/06/026e36b9dd1e7d05a54e83405e4a8d19.png)
## 2. 执行流程
前面介绍了触发bgsave的条件，下面将说明bgsave命令的执行流程，如下图所示(图片来源：https://blog.csdn.net/a1007720052/article/details/79126253)：
![](http://jbcdn2.b0.upaiyun.com/2018/06/d7320b5cc683f2e4eafeff120f5242cc.png)
图片中的5个步骤所进行的操作如下：
...
![](http://jbcdn2.b0.upaiyun.com/2018/06/d9908e2d6d9a38ed4b11ae1f2e976cd1.png)
**RDB文件格式**
RDB文件格式如下图所示（图片来源：《Redis设计与实现》）：
![](http://jbcdn2.b0.upaiyun.com/2018/06/58d5a97304a02565e67339cabadade6c.png)
其中各个字段的含义说明如下：
...
![](http://jbcdn2.b0.upaiyun.com/2018/06/4d88ca2d61427189a211b9652d4d5dc6.png)
需要注意的是，RDB文件的压缩并不是针对整个文件进行的，而是对数据库中的字符串进行的，且只有在字符串达到一定长度(20字节)时才会进行。
## 4. 启动时加载
RDB文件的载入工作是在服务器启动时自动执行的，并没有专门的命令。但是由于AOF的优先级更高，因此当AOF开启时，Redis会优先载入AOF文件来恢复数据；只有当AOF关闭时，才会在Redis服务器启动时检测RDB文件，并自动载入。服务器载入RDB文件期间处于阻塞状态，直到载入完成为止。
Redis启动日志中可以看到自动载入的执行：
![](http://jbcdn2.b0.upaiyun.com/2018/06/1ae2921155458ca1803759968945bc9f.png)
...
![](http://jbcdn2.b0.upaiyun.com/2018/06/dbd265625fa30c7c5837136e70171248.png)
通过上述内容可以看出，由于重写后AOF执行的命令减少了，文件重写既可以减少文件占用的空间，也可以加快恢复速度。
**文件重写的触发**
文件重写的触发，分为手动触发和自动触发：
手动触发：直接调用bgrewriteaof命令，该命令的执行与bgsave有些类似：都是fork子进程进行具体的工作，且都只有在fork时阻塞。
![](http://jbcdn2.b0.upaiyun.com/2018/06/a3bfe8ea04147795290a39f4debe1602.png)
此时服务器执行日志如下：
![](http://jbcdn2.b0.upaiyun.com/2018/06/c81bed0689ff70ea950574eacb26684c.png)
...
![](http://jbcdn2.b0.upaiyun.com/2018/06/ab802fac49820819fd9f5896ebaaaeb5.png)
状态可以通过info persistence查看：
![](http://jbcdn2.b0.upaiyun.com/2018/06/e1385eb5e949e1cd2155ec601ad65fb7.png)
...
![](http://jbcdn2.b0.upaiyun.com/2018/06/402b56947d9affe03aa1e13bad670e45.png)
**文件重写的流程**
文件重写流程如下图所示(图片来源：http://www.cnblogs.com/yangmingxianshen/p/8373205.html)：
![](http://jbcdn2.b0.upaiyun.com/2018/06/1324311267755f0686b519bb6064f99f.png)
...
![](http://jbcdn2.b0.upaiyun.com/2018/06/d49381c9c2e7613607d5ea44f4e01e8a.png)
当AOF开启，但AOF文件不存在时，即使RDB文件存在也不会加载(更早的一些版本可能会加载，但3.0不会)，Redis启动日志如下：
![](http://jbcdn2.b0.upaiyun.com/2018/06/6bc3128e30e32da4892e6838adbcab3c.png)
**文件校验**
...
![](http://jbcdn2.b0.upaiyun.com/2018/06/1355e2b884fc0fd4dd40a6aba7dad6da.png)
**伪客户端**
...
（1）info Persistence
执行结果如下：
![](http://jbcdn2.b0.upaiyun.com/2018/06/f238e2d52950ad685901cade59bf8809.png)
其中比较重要的包括：
