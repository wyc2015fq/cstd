# MySQL 最佳实践：空间优化 - 文章 - 伯乐在线
原文出处： [阿里云RDS-数据库内核组](http://mysql.taobao.org/monthly/2016/06/08/)
在前三期介绍了RDS for MySQL参数优化，[锁问题](http://blog.jobbole.com/103813/)以及[延迟优化最佳实践](http://blog.jobbole.com/103810/)之后，本期将介绍存储空间相关的最佳实践。
...
![screenshot](http://jbcdn2.b0.upaiyun.com/2016/07/227f88898cc7518fb0b8b3fd2807b292.png)
...
- 临时文件空间
- 系统文件空间
![screenshot](http://jbcdn2.b0.upaiyun.com/2016/07/df408eccf47bfd03a1f4dd68390c1555.png)
接下来我们将一一介绍一下这些文件组成：
## 数据文件
...
![screenshot](http://jbcdn2.b0.upaiyun.com/2016/07/64717454cb39f11d676709218cf4522f.png)
### 常见的空间问题：
- 对表进行数据删除后空间不会进行释放
...
![screenshot](http://jbcdn2.b0.upaiyun.com/2016/07/a5314d4dec327ad4024836c71236e41f.png)
- 曾经看到这样的案例，应用频繁的对表进行更新，但是在该表上有较多的大字段，由于在row格式下，binlog会记录整行记录， 这样就导致了binlog增长非常迅猛，详细可以参考[Mysql大字段的频繁更新导致binlog暴增](http://hidba.org/?p=957)。所以在应用的设计初期，就要避免使用大字段：varchar(8000),text,blob,clob等。
...
![screenshot](http://jbcdn2.b0.upaiyun.com/2016/07/8af48c096beed5d12db116e510f4ccfd.png)
- 当临时空间上涨原因是SQL排序导致的时候，可以通过show processlist快速找出排序的SQL，然后kill 掉SQL；
- 同时对排序的sql添加合适的索引，避免排序，这是治根的办法，避免数据库中出现排序的SQL；
- 为了避免排序消耗的空间过大，可以设置临时空间的大小，具体可以参考[RDS参数优化loose_rds_max_tmp_disk_space](https://yq.aliyun.com/articles/55594)
## 系统文件：
...
![screenshot](http://jbcdn2.b0.upaiyun.com/2016/07/3c329b1a68e672e323a7af7d14ae46f9.png)
