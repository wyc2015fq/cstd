# 关于负载均衡的一切：总结与思考 - 文章 - 伯乐在线
原文出处： [xybaby](http://www.cnblogs.com/xybaby/p/7867735.html)
古人云，不患寡而患不均。
在计算机的世界，这就是大家耳熟能详的负载均衡（[load balancing](https://en.wikipedia.org/wiki/Load_balancing_(computing))），所谓负载均衡，就是说如果一组计算机节点（或者一组进程）提供**相同的（同质的）服务**，那么对服务的请求就应该均匀的分摊到这些节点上。负载均衡的前提一定是“provide a single Internet service from multiple servers”， 这些提供服务的节点被称之为server farm、server pool或者backend servers。
...
# 一分钟了解负载均衡的一切
本章节的标题和内容都来自[一分钟了解负载均衡的一切](http://zhuanlan.51cto.com/art/201609/517313.htm)这一篇文章。当然，原文的标题是夸张了点，不过文中列出了在一个大型web网站中各层是如何用到负载均衡的，一目了然。
![](http://jbcdn2.b0.upaiyun.com/2018/01/b4822f398c6b3d29e8f309640f2bb305.png)
...
数据层的负载均衡，在我之前的《[带着问题学习分布式系统之数据分片](http://www.cnblogs.com/xybaby/p/7076731.html)》中有详细介绍。
# 算法衡量
...
为了解决这个哈希算法的问题，又引入了[一致性哈希](https://en.wikipedia.org/wiki/Consistent_hashing)算法，简单来说，一个物理节点与多个虚拟节点映射，在hash的时候，使用虚拟节点数目而不是物理节点数目。当物理节点变化的时候，虚拟节点的数目无需变化，只涉及到虚拟节点的重新分配。而且，调整每个物理节点对应的虚拟节点数目，也就相当于每个物理节点有不同的权重
...
![](http://jbcdn2.b0.upaiyun.com/2018/01/67aae7bdbbab3340571198f7ffb00261.png)
...
##  其他
在[这篇文章](http://www.cnblogs.com/data2value/p/6107653.html)中提到，负载均衡是一种推模型，一定会选出一个服务节点，然后把请求推送过来。而换一种思路，使用消息队列，就变成了拉模型：空闲的服务节点主动去拉取请求进行处理，各个节点的负载自然也是均衡的。消息队列相比负载均衡好处在于，服务节点不会被大量请求冲垮，同时增加服务节点更加容易；缺点也很明显，请求不是事实处理的。
...
[一分钟了解负载均衡的一切](http://zhuanlan.51cto.com/art/201609/517313.htm)
[grpc load-balancing.md](https://github.com/grpc/grpc/blob/master/doc/load-balancing.md)
