# 基于 JavaScript 的操作系统你听说过吗？ - 文章 - 伯乐在线
原文出处： [Danny Morabito](http://www.toptal.com/nodejs/nodeos-the-javascript-based-operating-system)   译文出处：[Linux中国](https://linux.cn/article-6931-1.html)
...
![](http://jbcdn2.b0.upaiyun.com/2016/01/3d546d63f79536dafc79ab04f89c1302.jpg)
...
- 一台运行 Mac OSX 或 Linux 计算机，也许 Windows 也可以，但是我没试过。
- Docker。
...
```
![](http://jbcdn2.b0.upaiyun.com/2016/01/ab4af017fa2326948fbaa9fbf01e7b07.jpg)
...
- 一台运行 Linux 的计算机。你可以在 OS X 上编译它，但是跨平台编译需要花费很多时间，对于 Windows 也如此。
- Linux 编译构建相关的工具（make、g++、gcc、autoconf）。
- Qemu。
- 时间，真的需要很多。
如果万事俱备，你就可以从源代码开始编译了：
- 下载项目源代码：`bash git clone git@github.com:NodeOS/NodeOS.git`。
- 使用下列命令编译： `cd NodeOS` 并 `npm install`。
...
![](http://jbcdn2.b0.upaiyun.com/2016/01/e553a7a61189ad7248033664b94ebca5.jpg)
...
#!/usr/bin/env node
```
这个部分叫做[释伴](https://linux.cn/article-3664-1.html)（shebang）。NodeOS 实际上并不需要它，但是它用于告诉操作系统如何执行下面的代码。在这里的意思是，它告诉系统下面的每行代码都需要通过  `/usr/bin/env node`命令来解释执行。
