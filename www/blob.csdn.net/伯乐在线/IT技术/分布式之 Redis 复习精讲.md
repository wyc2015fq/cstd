# 分布式之 Redis 复习精讲 - 文章 - 伯乐在线
原文出处： [孤独烟](https://www.cnblogs.com/rjzheng/p/9096228.html)
## 引言
### 为什么写这篇文章?
博主的[《分布式之消息队列复习精讲》](http://blog.jobbole.com/114016/)得到了大家的好评，内心诚惶诚恐，想着再出一篇关于复习精讲的文章。但是还是要说明一下，复习精讲的文章偏面试准备，真正在开发过程中，还是脚踏实地，一步一个脚印，不要投机取巧。
...
![image](http://jbcdn2.b0.upaiyun.com/2018/05/57e5c741c49cd5a8529aa90b0df7e147.png)
...
![image](http://jbcdn2.b0.upaiyun.com/2018/05/fa481e96124c6f54ad0f464a515cce01.png)
### 2、使用redis有什么缺点
...
![image](http://jbcdn2.b0.upaiyun.com/2018/05/0a2db924f854f5508d64a0dfa798899a.png)
...
(五)sorted set
sorted set多了一个权重参数score,集合中的元素能够按score进行排列。可以做**排行榜应用，取TOP N操作**。另外，参照另一篇[《分布式之延时任务方案解析》](http://blog.jobbole.com/114009/)，该文指出了sorted set可以用来做**延时任务**。最后一个应用就是可以做**范围查找**。
### 5、redis的过期策略以及内存淘汰机制
...
**回答**:[《分布式之数据库和缓存双写一致性方案解析》](http://blog.jobbole.com/113992/)给出了详细的分析，在这里简单的说一说。首先，采取正确更新策略，先更新数据库，再删缓存。其次，因为可能存在删除缓存失败的问题，提供一个补偿措施即可，例如利用消息队列。
### 7、如何应对缓存穿透和缓存雪崩问题
