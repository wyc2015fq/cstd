# Linux Kernel代码艺术——数组初始化 - 文章 - 伯乐在线
原文出处： [hazir](http://www.cnblogs.com/hazir/p/array_initialization.html)
...
```
先对表中所有 __NR_syscall_max+1 项初始化为指向 sys_ni_syscall 的函数，该函数只返回 -ENOSYS，表示该系统调用未实现。接下来包含一个头文件`#include `，该文件是在编译时生成的，内容为：
