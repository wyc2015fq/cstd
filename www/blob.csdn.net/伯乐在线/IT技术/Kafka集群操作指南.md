# Kafka集群操作指南 - 文章 - 伯乐在线
原文出处： [lujinhong](http://www.lujinhong.com/kafka%E9%9B%86%E7%BE%A4%E6%93%8D%E4%BD%9C%E6%8C%87%E5%8D%97.html)
# #（一）单机版安装
...
Shell
```
Kafka also has a command line consumer that will dump out messages to standard output. Kafka也有一个命令行consumer可以读取消息并输出到标准输出：
...
```
cd ~/zookeeper
bin/zkCli.sh 在ZooKeeper执行如下命令创建chroot路径： create /kafka '' 这样，每次连接Kafka集群的时候（使用--zookeeper选项），也必须使用带chroot路径的连接字符串，后面会看到。
```
...
# #（十）增加副本数量Increasing replication factor
