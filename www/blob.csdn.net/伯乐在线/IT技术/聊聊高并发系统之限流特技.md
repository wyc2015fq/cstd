# 聊聊高并发系统之限流特技 - 文章 - 伯乐在线
原文出处： [张开涛](http://jinnianshilongnian.iteye.com/blog/2305117)
...
![](http://jbcdn2.b0.upaiyun.com/2016/08/72db8bed5bf244a10bca51d2bfb76692.png)
**漏桶算法**
...
![](http://jbcdn2.b0.upaiyun.com/2016/08/12026cf76d857d25217967bcd224e616.png)
令牌桶和漏桶对比：
...
for(int i = 1; i     System.out.println(limiter.acquire());
}
Thread.sleep(1000L);
for(int i = 1; i     System.out.println(limiter.acquire());
}
...
local limit = 5 –限流大小
local current =limit_counter:get(key)    if current ~= nil and current + 1> limit then –如果超出限流大小
lock:unlock()
return 0
end
if current == nil then
...
## **接入层限流**
接入层通常指请求流量的入口，该层的主要目的有：负载均衡、非法请求过滤、请求聚合、缓存、降级、限流、A/B测试、服务质量监控等等，可以参考笔者写的[《使用Nginx+Lua(OpenResty)开发高性能Web应用》](http://blog.jobbole.com/104689/)。
...
```
![](http://jbcdn2.b0.upaiyun.com/2016/08/9fdbe6263bc31c8b0bdcd019bb09e14c.png)
...
[09/Jun/2016:14:30:17 +0800][1465453817.301] 503
```
![](http://jbcdn2.b0.upaiyun.com/2016/08/706f78f74692cc229ab4397f26295e1a.png)
