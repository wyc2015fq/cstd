# 依赖注入那些事儿 - 文章 - 伯乐在线
原文出处： [张洋（@敲代码的张洋）](http://www.cnblogs.com/leoo2sk/archive/2009/06/17/di-and-ioc.html)
# 目录
目录
1 IGame游戏公司的故事
1.1 讨论会
...
![](http://jbcdn2.b0.upaiyun.com/2016/02/b9402400bba020f5ac016126bb10e445.gif)
**图1.1 需求描述ppt**
...
```
程序运行结果如下：
![](http://jbcdn2.b0.upaiyun.com/2016/02/767509b5affaf4030e14f512c0c0ca45.gif)
**图1.2 小李程序的运行结果**
## 1.3 架构师的建议
...
![](http://jbcdn2.b0.upaiyun.com/2016/02/c7e98ce0fecf9b0faf1b3b9de6b187fe.jpg)
**图1.3 小于的设计**
...
![](http://jbcdn2.b0.upaiyun.com/2016/02/c5a5aef07fd2b88221ef4350798b82b1.jpg)
**图3.1 Setter注入示意**
...
```
运行结果如下：
![](http://jbcdn2.b0.upaiyun.com/2016/02/8d5d2c5bbe25e603c48a858d34b1ccae.jpg)
**图3.2 Setter注入运行结果**
### 3.1.2 构造注入
...
![](http://jbcdn2.b0.upaiyun.com/2016/02/b99241b99c843e1110699248a6ccc45a.jpg)
**图3.3 构造注入示意**
...
![](http://jbcdn2.b0.upaiyun.com/2016/02/08388088f88199c124f0b6a49804e800.jpg)
**图3.4 依赖获取示意**
...
![](http://jbcdn2.b0.upaiyun.com/2016/02/eb7aedbd02c83f540e4f64b3f56bd861.jpg)
**图3.5 配置Mac风格后的运行结果**
现在，我们不动程序，仅仅将配置文件中的“Mac”改为Windows，运行后结果如下：
![](http://jbcdn2.b0.upaiyun.com/2016/02/cd5ff297a786d5d2cd86d50d4662a104.jpg)
...
下面是引入反射机制后，对上面例子的改进：
![](http://jbcdn2.b0.upaiyun.com/2016/02/075019c1de5aebdeae8e432ebd6a156d.jpg)
...
灵、牛头人、矮人等）很多属性和业务是想通的，所以很可能通过一个IRole或AbstractRole抽象类实现多态性，不过，Role在实例化后（一般在用户登录成功后），是不会变化的，很少有游戏允许同一个玩家在运行中变换Role类型，所以Role应该是一但实例化，就不会变化，但如果再实例化一个（如另一个玩家登录），则可能就变化了。最后，还有一种多态性是活性非常低的，如我们熟悉的数据访问层多态性，即使我们实现了SQL Server、Oracle和Access等多种数据库的访问层，并实现了依赖注入，但几乎遇不到程序运行着就改数据库或短期内数据库频繁变动的情况。
...
## 4.3 .NET平台上典型IoC Container推介
### 4.3.1 Spring.NET
![](http://jbcdn2.b0.upaiyun.com/2016/02/1865e2945d76c57cbbe3c139ef7993ac.png)
Spring.NET是Java平台上Spring对.NET平台的移植，使用方法和Spring很像，并且功能强大，是.NET平台上大中型开发IoC Container的首选之一。除了DI外，Spring.NET也包括AOP等诸多功能。
Spring.NET的官方网站是：[http://www.springframework.net/](http://www.springframework.net/)
### 4.3.2 Unity
![](http://jbcdn2.b0.upaiyun.com/2016/02/1b404867a0189c3310b1ccca041c18e3.gif)
对于小型项目和讲求敏捷的团队，Spring.NET可能有点太重量级，那么可以选择轻量级的Unity。Unity是微软patterns & practices团队推出的轻量级框架，非常好用，目前最新版本是1.2。
Unity的官方网站是：[http://unity.codeplex.com/](http://unity.codeplex.com/)
参考文献
[1]  Shivprasad koirala, Design pattern – Inversion of control and Dependency injection, [http://www.codeproject.com/KB/aspnet/IOCDI.aspx](http://www.codeproject.com/KB/aspnet/IOCDI.aspx)
[2]  Martin Fowler, Inversion of Control Containers and the Dependency Injection pattern, [http://www.martinfowler.com/articles/injection.html](http://www.martinfowler.com/articles/injection.html)
[3]  Paul, IoC Types, [http://docs.codehaus.org/display/PICO/IoC+Types](http://docs.codehaus.org/display/PICO/IoC+Types)
