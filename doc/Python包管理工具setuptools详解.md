...
使用find_packages()
 对于简单工程来说，手动增加packages参数很容易，刚刚我们用到了这个函数，它默认在和setup.py同一目录下搜索各个含有**init**.py的包。其实我们可以将包统一放在一个src目录中，另外，这个包内可能还有aaa.txt文件和data数据文件夹。
...
如果不加控制，则setuptools只会将**init**.py加入到egg中，想要将这些文件都添加，需要修改setup.py
...
链接：https://www.jianshu.com/p/ea9973091fdf
