...
抓包下载地址：<https://download.csdn.net/download/knowledgebao/10802218>

官网文档下载地址：<https://download.csdn.net/download/knowledgebao/10843927>
...
![img](https://img-blog.csdnimg.cn/2018112118162969.png)
...
关于C0,C1,C2/S0,S1,S2的内容，有一定的要求。但是标准中没有提别明确的规定，所以各个厂家的实现也不尽相同。一般都以Adobe的Server和flash的客户端为标准。

C0+C1:

```cpp
sRequest[0]			= 0x03;
memset(&sRequest[1],0x00,(DEFAULT_RTMP_HANDSHAKE_SIZE-1));
```
...
nChunk = 0;
sResponse[0] = 0x03;
sContent = m_pSession->GetContent();
GetComplexHandshake((BYTE*)&sResponse[1],(BYTE*)sContent);
UINT ZRTMPProtocol::GetComplexHandshake(BYTE* sSHandshake,BYTE* sCHandshake)
{
	UINT32	nSDHOffset = 0;
	UINT32	nCDHOffset = 0;
	UINT32	nSDigestOffset = 0;
	UINT32	nCDigestOffset = 0;
	BYTE	sTempHash[512] = { 0 };
	BYTE	pLastHash[512] = { 0 };
 
	if(sSHandshake != NULL && sCHandshake != NULL)
	{
		nSDHOffset				= GetDHOffset((BYTE*)sSHandshake,1);
		nCDHOffset				= GetDHOffset((BYTE*)sCHandshake,1);
		memcpy((sSHandshake+nSDHOffset),RTMP_PUBLIC_KEY,128);
 
		nSDigestOffset			= GetDigestOffset((BYTE*)sSHandshake,1);
		memcpy((sSHandshake+nSDigestOffset),RTMP_HMAC_KEY1,32);
		//S2
#if 0
		memcpy(sSHandshake + 1536, sCHandshake, 1536);//S2直接拷贝C1,flash播放不了。
#else
		nCDigestOffset			= GetDigestOffset((BYTE*)sCHandshake,1);
 
		ZMath::HMACsha256(
			(BYTE*)sCHandshake + nCDigestOffset, //pData
			(UINT)32, //dataLength
			(BYTE*)m_sRTMPGenuineFMSKey, //key
			(UINT)68, //keyLength
			sTempHash //pResult
			);
		...
		//generate the hash
		ZMath::HMACsha256(
			(BYTE*)sSHandshake + 1536, //pData
			(UINT)(1536 - 32), //dataLength
			(BYTE*)sTempHash, //key
			(UINT)32, //keyLength
			pLastHash //pResult
			);
		...
		memcpy(sSHandshake + 1536 * 2 - 32, pLastHash, 32);
#endif
	}
	return 0;
}
...
![img](https://upload-images.jianshu.io/upload_images/3691932-dd48d743bcbd7d8f.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1000/format/webp)
...
![img](https://upload-images.jianshu.io/upload_images/3691932-e61ff67ce8fe219e.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1000/format/webp)
...
![img](https://upload-images.jianshu.io/upload_images/3691932-e119ce26f96e58c8.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1000/format/webp)
...
![img](https://upload-images.jianshu.io/upload_images/3691932-c5596440e473b480.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1000/format/webp)
...
- **Set Chunk Size(Message Type ID=1):**设置chunk中Data字段所能承载的最大字节数，默认为128B，通信过程中可以通过发送该消息来设置chunk Size的大小（不得小于128B），而且通信双方会各自维护一个chunkSize，两端的chunkSize是独立的。比如当A想向B发送一个200B的Message，但默认的chunkSize是128B，因此就要将该消息拆分为Data分别为128B和72B的两个chunk发送，如果此时先发送一个设置chunkSize为256B的消息，再发送Data为200B的chunk，本地不再划分Message，B接受到Set Chunk Size的协议控制消息时会调整的接受的chunk的Data的大小，也不用再将两个chunk组成为一个Message。
  ...
  其中第一位必须为0，chunk Size占31个位，最大可代表2147483647＝0x7FFFFFFF＝2^31-1，但实际上所有大于16777215=0xFFFFFF的值都用不上，因为chunk size不能大于Message的长度，表示Message的长度字段是用3个字节表示的，最大只能为0xFFFFFF。
...
- Set Peer Bandwidth(Message Type ID=6):
  ...
  - Soft(Limit Type=1):接受端可以讲Window Ack Size设为消息中的值，也可以保存原来的值（前提是原来的Size小于该控制消息中的Window Ack Size）
...
**4.1.2.7** **seek** 命令客户端发送seek 命令查找一个多媒体文件或一个播放列表的偏移量 (以毫秒为单位)

![img](https://img-blog.csdnimg.cn/2018121119272390.png)
seek命令执行成功时服务器会发送一个状态消息 NetStream.Seek.Notify。失败的话，服务器端返回一个 _error 消息。

**4.1.2.8. pause 命令**

客户端发送pause 命令告知服务器端是暂停还是开始播放，客户端发送给服务器端的命令结构如下：

![img](https://img-blog.csdnimg.cn/20181211192944101.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2tub3dsZWRnZWJhbw==,size_16,color_FFFFFF,t_70)

当流暂停时，服务器器端发送一个状态消息NetStream.Pause.Notify。NetStream.Unpause.Notify 只有针对没有暂停的流进行发放。失败的话，服务器端返回一个_error 消息。

## 5. 基于RTMP协议的推流和拉流播放过程

先看一下推流过程

 

![img](https://upload-images.jianshu.io/upload_images/3691932-2ec6466fa601464f.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1000/format/webp)

再看一下拉流过程

 

![img](https://upload-images.jianshu.io/upload_images/3691932-b6b58ec31ab75f42.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1000/format/webp)

参考资料：

1，<https://blog.csdn.net/ilv_xj/article/details/79054013>

2，<https://www.jianshu.com/p/5ce11c20a9df>

有任何问题可以发送邮箱：knowledgebao@163.com