...
bool CSTLMeshFileLoader::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "stl");
}
//! creates/loads an animated mesh from the file.
//! \return Pointer to the created mesh. Returns 0 if loading failed.
//! If you no longer need the mesh, you should call IAnimatedMesh::drop().
//! See IReferenceCounted::drop() for more information.
IAnimatedMesh* CSTLMeshFileLoader::createMesh(IReadFile* file)
{
  ...
  while (file->getPos() < filesize) {
    ...
    if (attrib & 0x8000) {
      color = A1R5G5B5toA8R8G8B8(attrib);
    }
    if (normal == vector3df()) {
      normal = plane3df(vertex[2], vertex[1], vertex[0]).Normal;
    }
    mb->Vertices.push_back(S3DVertex(vertex[2], normal, color, vector2df()));
    mb->Vertices.push_back(S3DVertex(vertex[1], normal, color, vector2df()));
    mb->Vertices.push_back(S3DVertex(vertex[0], normal, color, vector2df()));
...
void CSTLMeshFileLoader::getNextVector(IReadFile* file, vector3df& vec, bool binary) const
{
  if (binary) {
    file->read(&vec.X, 4);
    file->read(&vec.Y, 4);
    ...
    fast_atof_move(tmp, vec.X);
    getNextToken(file, tmp);
    fast_atof_move(tmp, vec.Y);
    getNextToken(file, tmp);
    fast_atof_move(tmp, vec.Z);
  }
  vec.X = -vec.X;
}
//! Read next word
const char* CSTLMeshFileLoader::getNextToken(IReadFile* file, char* token) const
{
  goNextWord(file);
  u8 c;
  token = "";
