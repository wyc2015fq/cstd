...
CV_INLINE unsigned int charsToUInt(const char* str)
{
...
//! Constructor
CLWOMeshFileLoader::CLWOMeshFileLoader(ISceneManager* smgr,
    IFileSystem* fs)
...
bool CLWOMeshFileLoader::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "lwo");
}
//! creates/loads an animated mesh from the file.
IAnimatedMesh* CLWOMeshFileLoader::createMesh(IReadFile* file)
{
  File = file;
  if (Mesh) {
    Mesh->drop();
  ...
  for (i = 0; i < Materials_size; ++i) {
    u16 uvTag;
    for (int j = 0; j < 2; ++j) { // max 2 texture coords
      if (Materials[i]->Texture[j].UVname_size) {
        for (uvTag = 0; uvTag < UvName_size; ++uvTag) {
          if (Materials[i]->Texture[j].UVname == UvName[uvTag]) {
            Materials[i]->Texture[j].UVTag = uvTag;
            break;
          }
        }
        for (uvTag = 0; uvTag < DUvName_size; ++uvTag) {
  ...
  if (FormatVersion == 2) {
    u32* vertexCount;
    vertexCount.reallocate(Materials_size);
    for (i = 0; i < Materials_size; ++i) {
      vertexCount.push_back(0);
    }
    for (int polyIndex = 0; polyIndex < Indices_size; ++polyIndex) {
      vertexCount[MaterialMapping[polyIndex]] += Indices[polyIndex]_size;
    }
    for (i = 0; i < Materials_size; ++i) {
  ...
  for (int polyIndex = 0; polyIndex < Indices_size; ++polyIndex) {
    ...
    SMeshBuffer* mb = Materials[tag]->Meshbuffer;
    const u32* poly = Indices[polyIndex];
    const u32 polySize = poly_size;
    const u16 uvTag = Materials[tag]->Texture[0].UVTag;
    const u16 duvTag = Materials[tag]->Texture[0].DUVTag;
    S3DVertex vertex;
    vertex.Color = 0xffffffff;
    const u32 vertCount = mb->Vertices_size;
    int i;
    for (i = 0; i < polySize; ++i) {
      const u32 j = poly[i];
      vertex.Pos = Points[j];
      if (uvTag < UvIndex_size) {
        for (int uvsearch = 0; uvsearch < UvIndex[uvTag]_size; ++uvsearch) {
          if (j == UvIndex[uvTag][uvsearch]) {
            vertex.TCoords = TCoords[uvTag][uvsearch];
            break;
          }
        }
        if (duvTag < DUvName_size) {
          for (int polysearch = 0; polysearch < VmPolyPointsIndex[duvTag]_size; polysearch += 2) {
  ...
  for (i = 0; i < Materials_size; ++i) {
    ...
    os::Printer::log("LWO loader: Vertex count", char * (Materials[i]->Meshbuffer->Vertices_size));
#endif
    if (!Materials[i]->Meshbuffer->Vertices_size) {
      Materials[i]->Meshbuffer->drop();
      delete Materials[i];
      continue;
    }
    for (int j = 0; j < Materials[i]->Meshbuffer->Vertices_size; ++j) {
    ...
    // load textures
    SMaterial& irrMat = Materials[i]->Meshbuffer->Material;
    ...
    if (Materials[i]->Texture[6].Map != "") { // bump
      ...
      if (irrMat.getTexture(pos)) {
        ...
        //      irrMat.MaterialType=EMT_NORMAL_MAP_SOLID;
      }
    }
    ...
    if (Materials[i]->Texture[0].Projection != 5) {
      ...
      f32 resolutionS = reciprocal(Materials[i]->Texture[0].Size.Z);
      f32 resolutionT = reciprocal(Materials[i]->Texture[0].Size.Y);
      if (Materials[i]->Texture[0].Axis == 1) {
        resolutionS = reciprocal(Materials[i]->Texture[0].Size.X);
        resolutionT = reciprocal(Materials[i]->Texture[0].Size.Z);
      }
      else if (Materials[i]->Texture[0].Axis == 2) {
        resolutionS = reciprocal(Materials[i]->Texture[0].Size.X);
        resolutionT = reciprocal(Materials[i]->Texture[0].Size.Y);
      }
    ...
    // add bump maps
    if (Materials[i]->Meshbuffer->Material.MaterialType == EMT_NORMAL_MAP_SOLID) {
  ...
  while (File->getPos() < File->getSize()) {
    ...
    case charsToUIntD('S', 'R', 'F', 'S'): {
      ...
      while (size != 0) {
        ...
        mat->Name = "";
        mat->Meshbuffer = new SMeshBuffer();
    ...
    case charsToUIntD('C', 'L', 'I', 'P'): {
      ...
      while (size != 0) {
        ...
        File->read(&subsize, 2);
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(subsize);
#endif
        size -= 6;
        if (strncmp(type, "STI", 4)) {
...
#ifdef LWO_READER_DEBUG
        os::Printer::log("LWO loader: loaded clip", path);
#endif
...
void CLWOMeshFileLoader::readObj1(u32 size)
{
  ...
  while (size != 0) {
    ...
    File->seek(pos, false);
    int i;
    const u16 vertCount = mb->Vertices_size;
    for (i = 0; i < numVerts; ++i) {
      File->read(&vertIndex, 2);
#ifndef __BIG_ENDIAN__
      os::Byteswap::BYTESWAP(vertIndex);
#endif
...
void CLWOMeshFileLoader::readVertexMapping(u32 size)
{
...
#ifndef __BIG_ENDIAN__
  os::Byteswap::BYTESWAP(dimension);
#endif
  size -= 6;
  size -= readString(name);
#ifdef LWO_READER_DEBUG
  os::Printer::log("LWO loader: Vertex map", name);
#endif
  ...
  TCoords.push_back(vector2df > ());
  vector2df > & UvCoords = TCoords.getLast();
  UvCoords.reallocate(Points_size);
  UvIndex.push_back(u32 * ());
  u32* UvPointsArray = UvIndex.getLast();
  UvPointsArray.reallocate(Points_size);
  u32 index;
  vector2df tcoord;
  while (size) {
    size -= readVX(index);
...
#ifndef __BIG_ENDIAN__
    os::Byteswap::BYTESWAP(index);
    os::Byteswap::BYTESWAP(tcoord.X);
    os::Byteswap::BYTESWAP(tcoord.Y);
#endif
    UvCoords.push_back(tcoord);
    UvPointsArray.push_back(index);
  }
#ifdef LWO_READER_DEBUG
  os::Printer::log("LWO loader: UvCoords", char * (UvCoords_size));
#endif
}
void CLWOMeshFileLoader::readDiscVertexMapping(u32 size)
{
  ...
  size -= readString(name);
#ifdef LWO_READER_DEBUG
  os::Printer::log("LWO loader: Discontinuous vertex map", name);
#endif
  ...
  VmPolyPointsIndex.push_back(u32 * ());
  u32* VmPolyPoints = VmPolyPointsIndex.getLast();
  VmCoordsIndex.push_back(vector2df > ());
  vector2df > & VmCoords = VmCoordsIndex.getLast();
  u32 vmpolys;
  u32 vmpoints;
  vector2df vmcoords;
  while (size) {
    ...
    size -= 8;
#ifndef __BIG_ENDIAN__
    os::Byteswap::BYTESWAP(vmpoints);
    os::Byteswap::BYTESWAP(vmpolys);
    os::Byteswap::BYTESWAP(vmcoords.X);
    os::Byteswap::BYTESWAP(vmcoords.Y);
#endif
...
#ifdef LWO_READER_DEBUG
  os::Printer::log("LWO loader: VmCoords", char * (VmCoords_size));
#endif
}
void CLWOMeshFileLoader::readTagMapping(u32 size)
{
  ...
  File->read(&type, 4);
  size -= 4;
  if ((strncmp(type, "SURF", 4)) || (Indices_size == 0)) {
    File->seek(size, true);
    return;
  }
  while (size != 0) {
...
void CLWOMeshFileLoader::readObj2(u32 size)
{
  ...
  while (size != 0) {
    ...
    numVerts &= 0x03FF;
    size -= 2;
    Indices.push_back(u32 * ());
    u32 vertIndex;
    u32* polyArray = Indices.getLast();
  ...
  MaterialMapping.reallocate(Indices_size);
...
void CLWOMeshFileLoader::readMat(u32 size)
{
  ...
  size -= readString(name);
#ifdef LWO_READER_DEBUG
  os::Printer::log("LWO loader: material name", name);
#endif
  int i;
  for (i = 0; i < Materials_size; ++i) {
  ...
  SMaterial& irrMat = mat->Meshbuffer->Material;
  u8 currTexture = 0;
  while (size != 0) {
    char type[5];
    type[4] = 0;
...
#ifndef __BIG_ENDIAN__
          os::Byteswap::BYTESWAP(mat->Diffuse);
#endif
          size -= 4;
          subsize -= 4;
...
#ifndef __BIG_ENDIAN__
          os::Byteswap::BYTESWAP(mat->Luminance);
#endif
          size -= 4;
          subsize -= 4;
...
#ifndef __BIG_ENDIAN__
          os::Byteswap::BYTESWAP(mat->Specular);
#endif
          size -= 4;
          subsize -= 4;
...
#ifndef __BIG_ENDIAN__
          os::Byteswap::BYTESWAP(mat->Reflection);
#endif
          size -= 4;
          subsize -= 4;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Transparency);
#endif
        size -= 4;
        subsize -= 4;
...
#ifdef LWO_READER_DEBUG
      os::Printer::log("LWO loader: loading transparency", char * (mat->Transparency));
#endif
    }
    break;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Translucency);
#endif
        size -= 4;
        subsize -= 4;
        if (FormatVersion == 2) {
...
#ifndef __BIG_ENDIAN__
          os::Byteswap::BYTESWAP(irrMat.Shininess);
#endif
          size -= 4;
          subsize -= 4;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Sharpness);
#endif
        size -= 4;
        subsize -= 4;
        if (FormatVersion == 2) {
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->SmoothingAngle);
#endif
        size -= 4;
      }
      break;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->ReflSeamAngle);
#endif
        size -= 4;
        if (FormatVersion == 2) {
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->ReflBlur);
#endif
        size -= 4;
        if (FormatVersion == 2) {
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->RefrIndex);
#endif
        size -= 4;
        subsize -= 4;
        if (FormatVersion == 2) {
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->TranspMode);
#endif
        size -= 2;
      }
      break;
...
#ifdef LWO_READER_DEBUG
        os::Printer::log("LWO loader: loading image", mat->Texture[currTexture].Map);
#endif
      }
    }
    break;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->TranspBlur);
#endif
        size -= 4;
        if (FormatVersion == 2) {
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->HighlightColor);
#endif
        size -= 4;
        if (FormatVersion == 2) {
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->ColorFilter);
#endif
        size -= 4;
        if (FormatVersion == 2) {
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->AdditiveTransparency);
#endif
        size -= 4;
        if (FormatVersion == 2) {
...
#ifndef __BIG_ENDIAN__
          os::Byteswap::BYTESWAP(mat->GlowSize);
#endif
          size -= 4;
          if (FormatVersion == 2) {
...
#ifndef __BIG_ENDIAN__
          os::Byteswap::BYTESWAP(irrMat.Thickness);
#endif
          size -= 4;
          if (FormatVersion == 2) {
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->AlphaMode);
#endif
        size -= 2;
        File->read(&mat->AlphaValue, 4);
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->AlphaValue);
#endif
        size -= 4;
      }
      break;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Flags);
#endif
        if (mat->Flags & 1) {
          mat->Luminance = 1.0f;
        }
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->EdgeTransparency);
#endif
        size -= 4;
      }
      break;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Texture[currTexture].Flags);
#endif
        size -= 2;
      }
      break;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Texture[currTexture].WidthWrap);
#endif
        File->read(&mat->Texture[currTexture].HeightWrap, 2);
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Texture[currTexture].HeightWrap);
#endif
        size -= 4;
      }
      break;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Texture[currTexture].AntiAliasing);
#endif
        if (tmp16 & ~0x01) {
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Texture[currTexture].Opacity);
#endif
        size -= 4;
      }
      break;
...
#ifndef __BIG_ENDIAN__
      os::Byteswap::BYTESWAP(mat->Texture[currTexture].OpacType);
#endif
      File->read(&mat->Texture[currTexture].Opacity, 4);
#ifndef __BIG_ENDIAN__
      os::Byteswap::BYTESWAP(mat->Texture[currTexture].Opacity);
    ...
    case charsToUIntD('A', 'X', 'I', 'S'): {
      ...
      size -= 2;
#ifdef LWO_READER_DEBUG
      os::Printer::log("LWO loader: loading axis value", char * (tmp16));
#endif
    }
    break;
    ...
    case charsToUIntD('C', 'N', 'T', 'R'): {
      vector3df& center = mat->Texture[currTexture].Center;
      size -= readVec(center);
      if (FormatVersion == 2) {
        size -= readVX(mat->Envelope[22]);
      }
#ifdef LWO_READER_DEBUG
      os::Printer::log("LWO loader: loading texture center", (char * (center.X) + " " + char * (center.Y) + " " + char * (center.Z)));
#endif
    }
    break;
    case charsToUIntD('T', 'S', 'I', 'Z'):
    case charsToUIntD('S', 'I', 'Z', 'E'): {
      vector3df& tsize = mat->Texture[currTexture].Size;
      size -= readVec(tsize);
      if (FormatVersion == 2) {
        size -= readVX(mat->Envelope[22]);
      }
#ifdef LWO_READER_DEBUG
      os::Printer::log("LWO loader: loading texture size", (char * (tsize.X) + " " + char * (tsize.Y) + " " + char * (tsize.Z)));
#endif
    }
    break;
...
#ifdef LWO_READER_DEBUG
      os::Printer::log("LWO loader: loading texture rotation", (char * (rotation.X) + " " + char * (rotation.Y) + " " + char * (rotation.Z)));
#endif
    }
    break;
...
#ifdef LWO_READER_DEBUG
      os::Printer::log("LWO loader: texture reference object", tmpname);
#endif
    }
    break;
    ...
    case charsToUIntD('F', 'A', 'L', 'L'): {
      ...
      vector3df& falloff = mat->Texture[currTexture].Falloff;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Texture[currTexture].FParam[0]);
#endif
        size -= 4;
      }
      break;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Texture[currTexture].FParam[1]);
#endif
        size -= 4;
      }
      break;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Texture[currTexture].FParam[2]);
#endif
        size -= 4;
      }
      break;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Texture[currTexture].IParam[0]);
#endif
        size -= 2;
      }
      break;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Texture[currTexture].IParam[1]);
#endif
        size -= 2;
      }
      break;
...
#ifndef __BIG_ENDIAN__
        os::Byteswap::BYTESWAP(mat->Texture[currTexture].IParam[2]);
#endif
        size -= 2;
      }
      break;
...
#ifdef LWO_READER_DEBUG
      os::Printer::log("LWO loader: loading material vmap binding", mat->Texture[currTexture].UVname);
#endif
    }
    break;
...
#ifdef LWO_READER_DEBUG
        if (index != 5) {
          os::Printer::log("LWO loader: wrong channel projection type", char * (index));
        }
#endif
    ...
    default: {
#ifdef LWO_READER_DEBUG
      os::Printer::log("LWO loader: skipping ", char * ((char*)&uiType, 4));
#endif
      File->seek(subsize, true);
      size -= subsize;
    }
...
u32 CLWOMeshFileLoader::readColor(SColor& color)
{
  if (FormatVersion != 2) {
    u8 colorComponent;
    ...
    File->read(&col.b, 4);
#ifndef __BIG_ENDIAN__
    os::Byteswap::BYTESWAP(col.b);
#endif
    color = col.toSColor();
    return 12;
  }
}
u32 CLWOMeshFileLoader::readString(char* name, u32 size)
{
  c8 c;
  name = "";
  if (size) {
...
u32 CLWOMeshFileLoader::readVec(vector3df& vec)
{
  File->read(&vec.X, 4);
#ifndef __BIG_ENDIAN__
  os::Byteswap::BYTESWAP(vec.X);
#endif
  File->read(&vec.Y, 4);
#ifndef __BIG_ENDIAN__
  os::Byteswap::BYTESWAP(vec.Y);
#endif
  File->read(&vec.Z, 4);
#ifndef __BIG_ENDIAN__
  os::Byteswap::BYTESWAP(vec.Z);
#endif
  return 12;
}
...
ITexture* CLWOMeshFileLoader::loadTexture(const char* file)
{
  IVideoDriver* driver = SceneManager->getVideoDriver();
  if (FileSystem->existFile(file)) {
    return driver->getTexture(file);
  }
  char* strippedName = FileSystem->getFileBasename(file);
  if (FileSystem->existFile(strippedName)) {
    return driver->getTexture(strippedName);
  }
  char* newpath = FileSystem->getFileDir(File->getFileName());
  newpath.append("/");
  newpath.append(strippedName);
  if (FileSystem->existFile(newpath)) {
    return driver->getTexture(newpath);
  }
  os::Printer::log("Could not load texture", file, ELL_WARNING);
  return 0;
}
