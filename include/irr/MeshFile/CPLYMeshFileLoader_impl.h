...
// constructor
CPLYMeshFileLoader::CPLYMeshFileLoader(ISceneManager* smgr)
...
CPLYMeshFileLoader::~CPLYMeshFileLoader()
{
  ...
  int i;
  for (i = 0; i < ElementList_size; ++i) {
    delete ElementList[i];
  }
  ElementList.clear();
}
//! returns true if the file maybe is able to be loaded by this class
bool CPLYMeshFileLoader::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "ply");
}
//! creates/loads an animated mesh from the file.
IAnimatedMesh* CPLYMeshFileLoader::createMesh(IReadFile* file)
{
  if (!file) {
    return 0;
  }
  ...
  if (strcmp(getNextLine(), "ply")) {
    os::Printer::log("Not a valid PLY file", file->getFileName(), ELL_ERROR);
  }
  else {
    ...
    do {
      if (strcmp(word, "format") == 0) {
        ...
        if (!ElementList_size) {
          ...
          // get element
          SPLYElement* el = ElementList[ElementList_size - 1];
          // fill property struct
          SPLYProperty prop;
          prop.Type = getPropertyType(word);
          el->KnownSize += prop_size;
    ...
    if (continueReading) {
      ...
      CDynamicMeshBuffer* mb = new CDynamicMeshBuffer(EVT_STANDARD, vertCount > 65565 ? EIT_32BIT : EIT_16BIT);
...
bool CPLYMeshFileLoader::readVertex(const SPLYElement& Element, CDynamicMeshBuffer* mb)
{
  if (!IsBinaryFile) {
    getNextLine();
  }
  ...
  bool result = false;
  int i;
  for (i = 0; i < Element.Properties_size; ++i) {
...
bool CPLYMeshFileLoader::readFace(const SPLYElement& Element, CDynamicMeshBuffer* mb)
{
  if (!IsBinaryFile) {
    getNextLine();
  }
...
f32 CPLYMeshFileLoader::getFloat(E_PLY_PROPERTY_TYPE t)
{
  ...
  if (IsBinaryFile) {
    ...
    if (EndPointer - StartPointer > 0) {
      ...
      case EPLYPT_INT16:
        if (IsWrongEndian) {
          s16 x = *(s16*)(StartPointer);
          os::Byteswap::BYTESWAP(x);
          retVal = x;
        }
        else {
...
u32 CPLYMeshFileLoader::getInt(E_PLY_PROPERTY_TYPE t)
{
  ...
  if (IsBinaryFile) {
    ...
    if (EndPointer - StartPointer) {
      ...
      case EPLYPT_INT16:
        if (IsWrongEndian) {
          u16 x = *(reinterpret_cast<u16*>(StartPointer));
          os::Byteswap::BYTESWAP(x);
          retVal = x;
        }
        else {
      ...
      case EPLYPT_INT32:
        if (IsWrongEndian) {
          s32 x = *(reinterpret_cast<s32*>(StartPointer));
          os::Byteswap::BYTESWAP(x);
          retVal = (f32)x;
        }
        else {
      ...
      case EPLYPT_FLOAT32:
        if (IsWrongEndian) {
          f32 x = *(reinterpret_cast<f32*>(StartPointer));
          os::Byteswap::BYTESWAP(x);
          retVal = x;
        }
        else {
