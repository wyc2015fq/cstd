...
//! Constructor
CXMeshFileLoader::CXMeshFileLoader(ISceneManager* smgr, IFileSystem* fs)
...
bool CXMeshFileLoader::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "x");
}
//! creates/loads an animated mesh from the file.
//! \return Pointer to the created mesh. Returns 0 if loading failed.
//! If you no longer need the mesh, you should call IAnimatedMesh::drop().
//! See IReferenceCounted::drop() for more information.
IAnimatedMesh* CXMeshFileLoader::createMesh(IReadFile* f)
{
  if (!f) {
    return 0;
  }
  ...
  char* tmpString = "Time to load ";
  tmpString += BinaryFormat ? "binary" : "ascii";
  tmpString += " X file: ";
  tmpString += time;
  tmpString += "ms";
  os::Printer::log(tmpString);
#endif
  //Clear up
  MajorVersion = 0;
  MinorVersion = 0;
...
bool CXMeshFileLoader::load(IReadFile* file)
{
  ...
  for (int n = 0; n < Meshes_size; ++n) {
    SXMesh* mesh = Meshes[n];
    // default material if nothing loaded
    if (!mesh->Materials_size) {
      mesh->Materials.push_back(SMaterial());
    ...
    if (!mesh->FaceMaterialIndices_size) {
      mesh->FaceMaterialIndices.set_used(mesh->Indices_size / 3);
      for (i = 0; i < mesh->FaceMaterialIndices_size; ++i) {
        mesh->FaceMaterialIndices[i] = 0;
      }
    }
    if (!mesh->HasVertexColors) {
      for (int j = 0; j < mesh->FaceMaterialIndices_size; ++j) {
        for (int id = j * 3 + 0; id <= j * 3 + 2; ++id) {
...
#ifdef BETTER_MESHBUFFER_SPLITTING_FOR_X
    {
      ...
      u32* > verticesLinkIndex;
      verticesLinkIndex.reallocate(mesh->Vertices_size);
      u16** verticesLinkBuffer;
      verticesLinkBuffer.reallocate(mesh->Vertices_size);
      for (i = 0; i < mesh->Vertices_size; ++i) {
        verticesLinkIndex.push_back(u32 * ());
        verticesLinkBuffer.push_back(u16 * ());
      }
      for (i = 0; i < mesh->FaceMaterialIndices_size; ++i) {
        for (int id = i * 3 + 0; id <= i * 3 + 2; ++id) {
          u16* Array = verticesLinkBuffer[ mesh->Indices[id] ];
          bool found = false;
      ...
      for (i = 0; i < verticesLinkBuffer_size; ++i) {
        if (!verticesLinkBuffer[i]_size) {
          verticesLinkBuffer[i].push_back(0);
        }
      }
      for (i = 0; i < mesh->Vertices_size; ++i) {
        u16* Array = verticesLinkBuffer[i];
        verticesLinkIndex[i].reallocate(Array_size);
        for (int j = 0; j < Array_size; ++j) {
          SSkinMeshBuffer* buffer = mesh->Buffers[ Array[j] ];
          verticesLinkIndex[i].push_back(buffer->Vertices_Standard_size);
          buffer->Vertices_Standard.push_back(mesh->Vertices[i]);
        }
      }
      for (i = 0; i < mesh->FaceMaterialIndices_size; ++i) {
        SSkinMeshBuffer* buffer = mesh->Buffers[ mesh->FaceMaterialIndices[i] ];
        for (int id = i * 3 + 0; id <= i * 3 + 2; ++id) {
          u16* Array = verticesLinkBuffer[ mesh->Indices[id] ];
          for (int j = 0; j < Array_size ; ++j) {
            if (Array[j] == mesh->FaceMaterialIndices[i]) {
      ...
      for (int j = 0; j < mesh->WeightJoint_size; ++j) {
        ...
        if (id >= verticesLinkIndex_size) {
          os::Printer::log("X loader: Weight id out of range", ELL_WARNING);
          id = 0;
          weight.strength = 0.f;
        }
        if (verticesLinkBuffer[id]_size == 1) {
          weight.vertex_id = verticesLinkIndex[id][0];
          weight.buffer_id = verticesLinkBuffer[id][0];
        }
        else if (verticesLinkBuffer[id]_size != 0) {
          for (int k = 1; k < verticesLinkBuffer[id]_size; ++k) {
    ...
    {
      u32* verticesLinkIndex;
      s16* verticesLinkBuffer;
      verticesLinkBuffer.set_used(mesh->Vertices_size);
      // init with 0
      for (i = 0; i < mesh->Vertices_size; ++i) {
      ...
      for (i = 0; i < mesh->FaceMaterialIndices_size; ++i) {
        for (int id = i * 3 + 0; id <= i * 3 + 2; ++id) {
          if ((verticesLinkBuffer[mesh->Indices[id]] != -1) && (verticesLinkBuffer[mesh->Indices[id]] != (s16)mesh->FaceMaterialIndices[i])) {
            ...
            const u32 tmp = mesh->Vertices_size;
      ...
      if (mesh->FaceMaterialIndices_size != 0) {
        // store vertices in buffers and remember relation in verticesLinkIndex
        u32* vCountArray = new u32[mesh->Buffers_size];
        memset(vCountArray, 0, mesh->Buffers_size * sizeof(u32));
        ...
        if (mesh->TCoords2_size) {
          for (i = 0; i != mesh->Buffers_size; ++i) {
            mesh->Buffers[i]->Vertices_2TCoords.reallocate(vCountArray[i]);
            mesh->Buffers[i]->VertexType = EVT_2TCOORDS;
          }
        }
        else {
          for (i = 0; i != mesh->Buffers_size; ++i) {
        ...
        verticesLinkIndex.set_used(mesh->Vertices_size);
        ...
        for (i = 0; i < mesh->Vertices_size; ++i) {
          ...
          SSkinMeshBuffer* buffer = mesh->Buffers[ verticesLinkBuffer[i] ];
          if (mesh->TCoords2_size) {
            verticesLinkIndex[i] = buffer->Vertices_2TCoords_size;
            ...
            buffer->Vertices_2TCoords.getLast().TCoords2 = (i < mesh->TCoords2_size) ? mesh->TCoords2[i] : mesh->Vertices[i].TCoords;
          }
          else {
            verticesLinkIndex[i] = buffer->Vertices_Standard_size;
            buffer->Vertices_Standard.push_back(mesh->Vertices[i]);
          }
        }
...
bool CXMeshFileLoader::readFileIntoMemory(IReadFile* file)
{
  ...
  tmp[1] = Buffer[5];
  tmp[2] = 0x0;
  MajorVersion = strtoul10(tmp);
  tmp[0] = Buffer[6];
  tmp[1] = Buffer[7];
  MinorVersion = strtoul10(tmp);
  //! read format
...
{
  char* objectName = getNextToken();
  if (objectName_size == 0) {
    return false;
  }
...
bool CXMeshFileLoader::parseDataObjectTemplate()
{
  ...
  if (!readHeadOfDataObject(&name)) {
    os::Printer::log("Left delimiter in template data object missing.",
        name, ELL_WARNING);
    os::Printer::log("Line", char * (Line), ELL_WARNING);
    return false;
  }
  // read GUID
  getNextToken();
...
bool CXMeshFileLoader::parseDataObjectFrame(CSkinnedMesh::SJoint* Parent)
{
  ...
  if (name_size) {
    for (int n = 0; n < AnimatedMesh->getAllJoints().size(); ++n) {
...
bool CXMeshFileLoader::parseDataObjectTransformationMatrix(matrix4& mat)
{
#ifdef _XREADER_DEBUG
...
bool CXMeshFileLoader::parseDataObjectMesh(SXMesh& mesh)
{
  ...
  for (int k = 0; k < nFaces; ++k) {
    ...
    if (fcnt != 3) {
      ...
      mesh.Indices.set_used(mesh.Indices_size + ((triangles - 1) * 3));
      mesh.IndexCountPerFace[k] = (u16)(triangles * 3);
      for (int f = 0; f < fcnt; ++f) {
        polygonfaces[f] = readInt();
      }
      for (int jk = 0; jk < triangles; ++jk) {
  ...
  while (true) {
    ...
    if (objectName == "MeshNormals") {
      ...
      if ((uv2pos != -1) && (uv2type == 1)) {
        mesh.TCoords2.reallocate(mesh.Vertices_size);
      }
      for (j = 0; j < mesh.Vertices_size; ++j) {
        if ((normalpos != -1) && (normaltype == 2)) {
          mesh.Vertices[j].Normal.set(*((vector3df*)(dataptr + normalpos)));
        }
        if ((uvpos != -1) && (uvtype == 1)) {
          mesh.Vertices[j].TCoords.set(*((vector2df*)(dataptr + uvpos)));
        }
        if ((uv2pos != -1) && (uv2type == 1)) {
          mesh.TCoords2.push_back(*((vector2df*)(dataptr + uv2pos)));
        }
        dataptr += size;
      }
      delete [] data;
      ...
      if (dataformat & 0x102) { // 2nd uv set
        mesh.TCoords2.reallocate(mesh.Vertices_size);
        u8* dataptr = (u8*) data;
        const u32 size = ((dataformat >> 8) & 0xf) * sizeof(vector2df);
        for (int j = 0; j < mesh.Vertices_size; ++j) {
          mesh.TCoords2.push_back(*((vector2df*)(dataptr)));
          dataptr += size;
        }
      }
      delete [] data;
...
bool CXMeshFileLoader::parseDataObjectSkinWeights(SXMesh& mesh)
{
  ...
  // read vertex indices
  u32 i;
  const u32 jointStart = joint->Weights_size;
  joint->Weights.reallocate(jointStart + nWeights);
  mesh.WeightJoint.reallocate(mesh.WeightJoint_size + nWeights);
  mesh.WeightNum.reallocate(mesh.WeightNum_size + nWeights);
  for (i = 0; i < nWeights; ++i) {
    mesh.WeightJoint.push_back(n);
    mesh.WeightNum.push_back(joint->Weights_size);
  ...
  matrix4& MatrixOffset = joint->GlobalInversedMatrix;
...
bool CXMeshFileLoader::parseDataObjectMeshNormals(SXMesh& mesh)
{
  ...
  u32* normalIndices;
  normalIndices.set_used(mesh.Indices_size);
  // read face normal indices
  const u32 nFNormals = readInt();
  u32 normalidx = 0;
  u32* polygonfaces;
  for (int k = 0; k < nFNormals; ++k) {
    const u32 fcnt = readInt();
    u32 triangles = fcnt - 2;
...
bool CXMeshFileLoader::parseDataObjectMeshMaterialList(SXMesh& mesh)
{
  ...
  //if (nFaceIndices != mesh.IndexCountPerFace_size)
  ...
  u32 ind = 0;
  for (int tfi = 0; tfi < mesh.IndexCountPerFace_size; ++tfi) {
    if (tfi < nFaceIndices) {
      ind = readInt();
    }
  ...
  while (true) {
    ...
    if (objectName_size == 0) {
      ...
      int i;
      for (i = 0; i < TemplateMaterials_size; ++i)
...
bool CXMeshFileLoader::parseDataObjectMaterial(SMaterial& material)
{
#ifdef _XREADER_DEBUG
  ...
  while (true) {
    ...
    if (objectName_size == 0) {
    ...
    else if (objectName.equals_ignore_case("TextureFilename")) {
      ...
      os::Printer::log("Unknown data object in material in .x file", objectName, ELL_WARNING);
      if (!parseUnknownDataObject()) {
        return false;
      }
    }
...
bool CXMeshFileLoader::parseDataObjectAnimation()
{
  ...
  if (FrameName_size != 0) {
    ...
    joint->PositionKeys.reallocate(joint->PositionKeys_size + animationDump.PositionKeys_size);
    for (n = 0; n < animationDump.PositionKeys_size; ++n) {
      joint->PositionKeys.push_back(animationDump.PositionKeys[n]);
    }
    joint->ScaleKeys.reallocate(joint->ScaleKeys_size + animationDump.ScaleKeys_size);
    for (n = 0; n < animationDump.ScaleKeys_size; ++n) {
      joint->ScaleKeys.push_back(animationDump.ScaleKeys[n]);
    }
    joint->RotationKeys.reallocate(joint->RotationKeys_size + animationDump.RotationKeys_size);
    for (n = 0; n < animationDump.RotationKeys_size; ++n) {
...
bool CXMeshFileLoader::parseDataObjectAnimationKey(ISkinnedMesh::SJoint* joint)
{
  ...
  for (i = 0; i < numberOfKeys; ++i) {
    ...
    case 4: {
      ...
      keyR->rotation = quaternion(mat.getTransposed());
...
bool CXMeshFileLoader::parseDataObjectTextureFilename(char* texturename)
{
#ifdef _XREADER_DEBUG
...
//! if there is one
bool CXMeshFileLoader::readHeadOfDataObject(char** outname)
{
  char* nameOrBrace = getNextToken();
  if (nameOrBrace != "{") {
    if (outname) {
      (*outname) = nameOrBrace;
    }
...
char* CXMeshFileLoader::getNextToken()
{
  char* s;
...
bool CXMeshFileLoader::getNextTokenAsString(char* out)
{
  if (BinaryFormat) {
    out = getNextToken();
    return true;
...
bool CXMeshFileLoader::readVector2(vector2df& vec)
{
  vec.X = readFloat();
  vec.Y = readFloat();
  return true;
...
bool CXMeshFileLoader::readVector3(vector3df& vec)
{
  vec.X = readFloat();
  vec.Y = readFloat();
  vec.Z = readFloat();
...
bool CXMeshFileLoader::readRGB(SColor& color)
{
  SColorf tmpColor;
...
bool CXMeshFileLoader::readRGBA(SColor& color)
{
  SColorf tmpColor;
  tmpColor.r = readFloat();
  tmpColor.g = readFloat();
...
bool CXMeshFileLoader::readMatrix(matrix4& mat)
{
  int i;
  for (i = 0; i < 16; ++i) {
    mat[i] = readFloat();
