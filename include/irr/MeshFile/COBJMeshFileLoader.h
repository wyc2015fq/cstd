...
private:
  struct SObjMtl {
    SObjMtl() : Meshbuffer(0), Bumpiness(1.0f), Illumination(0),
      ...
      Meshbuffer->Material.AmbientColor = SColorf(0.2f, 0.2f, 0.2f, 1.0f).toSColor();
      Meshbuffer->Material.DiffuseColor = SColorf(0.8f, 0.8f, 0.8f, 1.0f).toSColor();
      Meshbuffer->Material.SpecularColor = SColorf(1.0f, 1.0f, 1.0f, 1.0f).toSColor();
    }
  ...
  const c8* readTextures(const c8* bufPtr, const c8* const bufEnd, SObjMtl* currMaterial, const char* relPath);
  ...
  char* copyLine(const c8* inBuf, const c8* const bufEnd);
  ...
  void readMTL(const c8* fileName, const char* relPath);
  //! Find and return the material with the given name
  SObjMtl* findMtl(const char* mtlName, const char* grpName);
  //! Read RGB color
  const c8* readColor(const c8* bufPtr, SColor& color, const c8* const pBufEnd);
  //! Read 3d vector of floats
  const c8* readVec3(const c8* bufPtr, vector3df& vec, const c8* const pBufEnd);
  //! Read 2d vector of floats
  const c8* readUV(const c8* bufPtr, vector2df& vec, const c8* const pBufEnd);
