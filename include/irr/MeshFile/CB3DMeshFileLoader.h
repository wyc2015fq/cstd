...
public:
  //! Constructor
  CB3DMeshFileLoader(ISceneManager* smgr);
...
private:
  ...
  bool readChunkTRIS(SSkinMeshBuffer* MeshBuffer, u32 MeshBufferID, s32 Vertices_Start);
  ...
  void readString(char* newstring);
  void readFloats(f32* vec, u32 count);
  SB3dChunk > B3dStack;
  SB3dMaterial > Materials;
  SB3dTexture > Textures;
  s32 > AnimatedVertices_VertexID;
  s32 > AnimatedVertices_BufferID;
  S3DVertex* BaseVertices;
