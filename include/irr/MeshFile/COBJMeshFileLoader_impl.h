...
bool COBJMeshFileLoader::isALoadableFileExtension(const char* filename) const
{
...
IAnimatedMesh* COBJMeshFileLoader::createMesh(IReadFile* file)
{
  ...
  vector3df > vertexBuffer;
  vector3df > normalsBuffer;
  vector2df > textureCoordBuffer;
  SObjMtl* currMtl = new SObjMtl();
  Materials.push_back(currMtl);
  u32 smoothingGroup = 0;
  const char* fullName = file->getFileName();
  const char* relPath = FileSystem->getFileDir(fullName) + "/";
  ...
  while (bufPtr != bufEnd) {
    ...
    case 'f': {             // face
      ...
      const char* wordBuffer = copyLine(bufPtr, bufEnd);
      const c8* linePtr = wordBuffer;
      const c8* const endPtr = linePtr + wordBuffer_size;
      int > faceCorners;
      ...
      while (0 != linePtr[0]) {
        ...
        retrieveVertexIndices(vertexWord, Idx, vertexWord + wlength + 1, vertexBuffer_size, textureCoordBuffer_size, normalsBuffer_size);
        ...
        if (n) {
          ...
          vertLocation = currMtl->Meshbuffer->Vertices_size - 1;
      ...
      int i;
      for (i = 1; i < faceCorners_size - 1; ++i) {
        // Add a triangle
  ...
  for (int m = 0; m < Materials_size; ++m) {
...
const c8* COBJMeshFileLoader::readTextures(const c8* bufPtr, const c8* const bufEnd, SObjMtl* currMaterial, const char* relPath)
{
  ...
  while (textureNameBuf[0] == '-') {
    ...
    else if (!strncmp(bufPtr, "-blendu", 7)) {
      bufPtr = goAndCopyNextWord(textureNameBuf, bufPtr, WORD_BUFFER_LENGTH, bufEnd);
    }
  ...
  char* texname(textureNameBuf);
  texname.replace('\\', '/');
  ITexture* texture = 0;
  bool newTexture = false;
  if (texname_size) {
    char* texnameWithUserPath(SceneManager->getParameters()->getAttributeAsString(OBJ_TEXTURE_PATH));
...
void COBJMeshFileLoader::readMTL(const c8* fileName, const char* relPath)
{
    ...
    case 'N':
      if (currMaterial) {
        ...
        case 's': { // Ns - shininess
          ...
          f32 shininessValue = fast_atof(nsStr);
    ...
    case 'T':
      if (currMaterial) {
        ...
        case 'f':   // Tf - Transmitivity
          ...
          f32 transparency = (fast_atof(redStr) + fast_atof(greenStr) + fast_atof(blueStr)) / 3;
...
//! Read RGB color
const c8* COBJMeshFileLoader::readColor(const c8* bufPtr, SColor& color, const c8* const bufEnd)
{
  const u32 COLOR_BUFFER_LENGTH = 16;
  c8 colStr[COLOR_BUFFER_LENGTH];
  color.setAlpha(255);
  bufPtr = goAndCopyNextWord(colStr, bufPtr, COLOR_BUFFER_LENGTH, bufEnd);
  color.setRed((s32)(fast_atof(colStr) * 255.0f));
  bufPtr = goAndCopyNextWord(colStr,   bufPtr, COLOR_BUFFER_LENGTH, bufEnd);
  color.setGreen((s32)(fast_atof(colStr) * 255.0f));
  bufPtr = goAndCopyNextWord(colStr,   bufPtr, COLOR_BUFFER_LENGTH, bufEnd);
  color.setBlue((s32)(fast_atof(colStr) * 255.0f));
  return bufPtr;
}
//! Read 3d vector of floats
const c8* COBJMeshFileLoader::readVec3(const c8* bufPtr, vector3df& vec, const c8* const bufEnd)
{
...
const c8* COBJMeshFileLoader::readUV(const c8* bufPtr, vector2df& vec, const c8* const bufEnd)
{
  const u32 WORD_BUFFER_LENGTH = 256;
  c8 wordBuffer[WORD_BUFFER_LENGTH];
  bufPtr = goAndCopyNextWord(wordBuffer, bufPtr, WORD_BUFFER_LENGTH, bufEnd);
...
COBJMeshFileLoader::SObjMtl* COBJMeshFileLoader::findMtl(const char* mtlName, const char* grpName)
{
...
u32 COBJMeshFileLoader::copyWord(c8* outBuf, const c8* const inBuf, u32 outBufLength, const c8* const bufEnd)
{
  ...
  u32 length = min_(i, outBufLength - 1);
...
char* COBJMeshFileLoader::copyLine(const c8* inBuf, const c8* bufEnd)
{
  if (!inBuf) {
    return char * ();
  }
  ...
  return char * (inBuf, (u32)(ptr - inBuf + ((ptr < bufEnd) ? 1 : 0)));
}
