...
CMY3DMeshFileLoader::CMY3DMeshFileLoader(ISceneManager* scmgr, IFileSystem* fs)
...
bool CMY3DMeshFileLoader::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "my3d");
}
IAnimatedMesh* CMY3DMeshFileLoader::createMesh(IReadFile* file)
{
  MaterialEntry.clear();
  MeshBufferEntry.clear();
  ChildNodes.clear();
  // working directory (from which we load the scene)
  char* filepath = FileSystem->getFileDir(file->getFileName());
  if (filepath == ".") {
    filepath = "";
  }
  else {
  ...
  for (s32 m = 0; m < sceneHeader.MaterialCount; ++m) {
    ...
    for (int t = 0; t < me.Header.TextureCount; ++t) {
      ...
      const char* LightingMapStr = "LightingMap";
      const s32 ls = LightingMapStr_size;
      const bool isSubString = (LightingMapStr == name.subString(max_(0, (pos - ls)), ls));
      if ((isSubString || (name[pos - 1] == 'm' &&
          name[pos - 2] == 'l' && name[pos - 3] == '_')) &&
          !gotLightMap) {
        const bool oldMipMapState = SceneManager->getVideoDriver()->getTextureCreationFlag(ETCF_CREATE_MIP_MAPS);
        SceneManager->getVideoDriver()->setTextureCreationFlag(ETCF_CREATE_MIP_MAPS, false);
        me.Texture2FileName = texturePath_size ? texturePath : filepath;
        ...
        me.MaterialType = EMT_LIGHTMAP_M2;
        gotLightMap = true;
        SceneManager->getVideoDriver()->setTextureCreationFlag(ETCF_CREATE_MIP_MAPS, oldMipMapState);
      }
        ...
        me.MaterialType = EMT_REFLECTION_2_LAYER;
      }
      else if (!gotMainMap && !gotLightMap) {
    ...
    if (!strncmp(me.Header.Name, "AlphaChannel-", 13)) {
      me.MaterialType = EMT_TRANSPARENT_ALPHA_CHANNEL;
    }
    else if (!strncmp(me.Header.Name, "SphereMap-", 10)) {
      me.MaterialType = EMT_SPHERE_MAP;
    }
  }
  // loading meshes
  ...
  for (s32 mesh_id = 0; mesh_id < sceneHeader.MeshCount; mesh_id++) {
    ...
    SMyVertex > Vertex;
    SMyFace > Face;
    SMyTVertex > TVertex1, TVertex2;
    SMyFace > TFace1, TFace2;
    s32 vertsNum = 0;
    s32 facesNum = 0;
    // vertices
    file->read(&id, sizeof(id));
    ...
    file->read(Vertex, sizeof(SMyVertex)*vertsNum);
    // faces
    file->read(&id, sizeof(id));
#ifdef __BIG_ENDIAN__
    id = os::Byteswap::BYTESWAP(id);
    ...
    file->read(Face, sizeof(SMyFace)*facesNum);
    ...
    for (s32 tex = 0; tex < (s32)meshHeader.TChannelCnt; tex++) {
      ...
      if (id != MY3D_TVERTS_ID) {
        char* msg = "Can not find MY3D_TVERTS_ID (";
        msg.append(char * (tex));
      ...
      if (tex == 0) {
        ...
        file->read(TVertex1, sizeof(SMyTVertex)*tVertsNum);
      }
      else if (tex == 1) {
        // 2nd texture channel
        TVertex2.set_used(tVertsNum);
        file->read(TVertex2, sizeof(SMyTVertex)*tVertsNum);
      }
      else {
      ...
      if (id != MY3D_TFACES_ID) {
        char* msg = "Can not find MY3D_TFACES_ID (";
        msg.append(char * (tex));
        msg.append("texture channel), loading failed!");
        os::Printer::log(msg, ELL_ERROR);
        return 0;
      ...
      if (tex == 0) {
        ...
        file->read(TFace1, sizeof(SMyFace)*tFacesNum);
      }
      else if (tex == 1) {
        // 2nd texture channel
        TFace2.set_used(tFacesNum);
        file->read(TFace2, sizeof(SMyFace)*tFacesNum);
      }
      else {
        // skip other texture channels
    ...
    if (!buffer ||
        (buffer->Vertices_size + vertsNum) > SceneManager->getVideoDriver()->getMaximalPrimitiveCount()) {
      // creating new mesh buffer for this material
      buffer = new SMeshBufferLightMap();
      buffer->Material.MaterialType = EMT_LIGHTMAP_M2; // EMT_LIGHTMAP_M4 also possible
      buffer->Material.Wireframe = false;
      buffer->Material.Lighting = false;
      if (matEnt) {
        buffer->Material.MaterialType = matEnt->MaterialType;
        if (buffer->Material.MaterialType == EMT_REFLECTION_2_LAYER) {
        ...
        buffer->Material.AmbientColor = SColor(255, 255, 255, 255);
        buffer->Material.DiffuseColor = SColor(255, 255, 255, 255);
        buffer->Material.EmissiveColor = SColor(0, 0, 0, 0);
        buffer->Material.SpecularColor = SColor(0, 0, 0, 0);
      }
      if (matEnt && matEnt->Header.Transparency != 0) {
        if (buffer->Material.MaterialType == EMT_REFLECTION_2_LAYER) {
          buffer->Material.MaterialType = EMT_TRANSPARENT_REFLECTION_2_LAYER;
          buffer->Material.Lighting  = true;
          buffer->Material.BackfaceCulling = true;
        }
        else {
          buffer->Material.MaterialType = EMT_TRANSPARENT_VERTEX_ALPHA;
          buffer->Material.Lighting = false;
          ...
          buffer->Material.MaterialType != EMT_SPHERE_MAP) {
    ...
    S3DVertex VertexA, VertexB, VertexC;
    ...
    if (matEnt &&
      ...
      SColor color(
          matEnt->Header.DiffuseColor.A, matEnt->Header.DiffuseColor.R,
          matEnt->Header.DiffuseColor.G, matEnt->Header.DiffuseColor.B);
      vert_color = color.getInterpolated(SColor(0, 0, 0, 0),
    ...
    if (buffer->Material.MaterialType == EMT_TRANSPARENT_ALPHA_CHANNEL) {
      buffer->Indices.reallocate(buffer->Indices_size + 6 * facesNum);
      buffer->Vertices.reallocate(buffer->Vertices_size + 6 * facesNum);
    }
    else {
      buffer->Indices.reallocate(buffer->Indices_size + 3 * facesNum);
      buffer->Vertices.reallocate(buffer->Vertices_size + 3 * facesNum);
    }
    for (int f = 0; f < facesNum; f++) {
      ...
      buffer->Indices.push_back(buffer->Vertices_size);
      ...
      //*****************************************************************
      if (buffer->Material.MaterialType == EMT_TRANSPARENT_ALPHA_CHANNEL) {
        VertexA.Normal = vector3df(-VertexA.Normal.X, -VertexA.Normal.Y, -VertexA.Normal.Z);
        VertexB.Normal = vector3df(-VertexB.Normal.X, -VertexB.Normal.Y, -VertexB.Normal.Z);
        VertexC.Normal = vector3df(-VertexC.Normal.X, -VertexC.Normal.Y, -VertexC.Normal.Z);
        buffer->Indices.push_back(buffer->Vertices_size);
        buffer->Vertices.push_back(VertexC);
        buffer->Indices.push_back(buffer->Vertices_size);
        buffer->Vertices.push_back(VertexB);
  ...
  for (int num = 0; num < MeshBufferEntry_size; ++num) {
...
ITexture* CMY3DMeshFileLoader::readEmbeddedLightmap(IReadFile* file, char* namebuf)
{
  ...
  else if (texDataHeader.PixelFormat == MY3D_PIXEL_FORMAT_16) {
    pixFormatStr = "16bit,";
  }
  else {
    char* msg = "Unknown format of image data (";
  ...
  if (texDataHeader.PixelFormat == MY3D_PIXEL_FORMAT_24) {
    ...
    light_img = SceneManager->getVideoDriver()->createImageFromData(
        ECF_R8G8B8,
        dimension2du(texDataHeader.Width, texDataHeader.Height),
        data, true);
  }
  else {
  ...
  const bool oldMipMapState = SceneManager->getVideoDriver()->getTextureCreationFlag(ETCF_CREATE_MIP_MAPS);
  SceneManager->getVideoDriver()->setTextureCreationFlag(ETCF_CREATE_MIP_MAPS, false);
  ITexture* lmtex = SceneManager->getVideoDriver()->addTexture(LightMapName, light_img);
  SceneManager->getVideoDriver()->setTextureCreationFlag(ETCF_CREATE_MIP_MAPS, oldMipMapState);
  light_img->drop();
  return lmtex;
}
...
{
  for (int m = 0; m < MaterialEntry_size; ++m)
...
{
  for (int m = 0; m < MeshBufferEntry_size; ++m) {
...
const ISceneNode* > & CMY3DMeshFileLoader::getChildNodes() const
{
  return ChildNodes;
}
// end scnene
