...
#endif
//! Constructor
CB3DMeshFileLoader::CB3DMeshFileLoader(ISceneManager* smgr)
...
bool CB3DMeshFileLoader::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "b3d");
}
//! creates/loads an animated mesh from the file.
//! \return Pointer to the created mesh. Returns 0 if loading failed.
//! If you no longer need the mesh, you should call IAnimatedMesh::drop().
//! See IReferenceCounted::drop() for more information.
IAnimatedMesh* CB3DMeshFileLoader::createMesh(IReadFile* f)
{
  if (!f) {
    return 0;
  }
...
bool CB3DMeshFileLoader::load()
{
  ...
  while ((B3dStack.getLast().startposition + B3dStack.getLast().length) > B3DFile->getPos()) {
    ...
    if (strncmp(B3dStack.getLast().name, "TEXS", 4) == 0) {
      ...
      B3dStack.erase(B3dStack_size - 1);
    }
  }
  B3dStack.clear();
  BaseVertices.clear();
...
bool CB3DMeshFileLoader::readChunkNODE(CSkinnedMesh::SJoint* inJoint)
{
  ...
  char* logStr;
  int i;
  for (i = 1; i < B3dStack_size; ++i) {
    logStr += "-";
  }
  logStr += "read ChunkNODE";
  os::Printer::log(logStr, joint->Name);
#endif
  f32 position[3], scale[3], rotation[4];
  readFloats(position, 3);
  readFloats(scale, 3);
  readFloats(rotation, 4);
  joint->Animatedposition = vector3df(position[0], position[1], position[2]) ;
  joint->Animatedscale = vector3df(scale[0], scale[1], scale[2]);
  joint->Animatedrotation = quaternion(rotation[1], rotation[2], rotation[3], rotation[0]);
  //Build LocalMatrix:
  ...
  while (B3dStack.getLast().startposition + B3dStack.getLast().length > B3DFile->getPos()) { // this chunk repeats
    ...
    if (strncmp(B3dStack.getLast().name, "NODE", 4) == 0) {
      ...
      VerticesStart = BaseVertices_size;
...
bool CB3DMeshFileLoader::readChunkMESH(CSkinnedMesh::SJoint* inJoint)
{
  ...
  while ((B3dStack.getLast().startposition + B3dStack.getLast().length) > B3DFile->getPos()) { //this chunk repeats
    ...
    if (strncmp(B3dStack.getLast().name, "VRTS", 4) == 0) {
      ...
      SSkinMeshBuffer* meshBuffer = AnimatedMesh->addMeshBuffer();
      if (brushID != -1) {
      ...
      if (!NormalsInFile) {
        s32 i;
        for (i = 0; i < (s32)meshBuffer->Indices_size; i += 3) {
          plane3df p(meshBuffer->getVertex(meshBuffer->Indices[i + 0])->Pos,
...
bool CB3DMeshFileLoader::readChunkVRTS(CSkinnedMesh::SJoint* inJoint)
{
  ...
  BaseVertices.reallocate(memoryNeeded + BaseVertices_size + 1);
  AnimatedVertices_VertexID.reallocate(memoryNeeded + AnimatedVertices_VertexID_size + 1);
  //--------------------------------------------//
  while ((B3dStack.getLast().startposition + B3dStack.getLast().length) > B3DFile->getPos()) { // this chunk repeats
    f32 position[3];
    ...
    // Create Vertex...
    S3DVertex Vertex(position[0], position[1], position[2],
        normal[0], normal[1], normal[2],
        SColorf(color[0], color[1], color[2], color[3]).toSColor(),
        tu, tv, tu2, tv2);
...
bool CB3DMeshFileLoader::readChunkTRIS(SSkinMeshBuffer* meshBuffer, u32 meshBufferID, s32 vertices_Start)
{
#ifdef _B3D_READER_DEBUG
  char* logStr;
  int i;
  ...
  meshBuffer->Indices.reallocate(memoryNeeded + meshBuffer->Indices_size + 1);
  while ((B3dStack.getLast().startposition + B3dStack.getLast().length) > B3DFile->getPos()) { // this chunk repeats
    s32 vertex_id[3];
    ...
    for (s32 i = 0; i < 3; ++i) {
      if ((u32)vertex_id[i] >= AnimatedVertices_VertexID_size) {
      ...
      if (AnimatedVertices_VertexID[ vertex_id[i] ] == -1) { //If this vertex is not in the meshbuffer
        ...
        if (BaseVertices[ vertex_id[i] ].TCoords2 != vector2df(0.f, 0.f)) {
        ...
        if (meshBuffer->VertexType == EVT_STANDARD) {
        ...
        if (B3dMaterial) {
          ...
          S3DVertex* Vertex = meshBuffer->getVertex(meshBuffer->getVertexCount() - 1);
          if (!HasVertexColors) {
  ...
  while ((B3dStack.getLast().startposition + B3dStack.getLast().length) > B3DFile->getPos()) { //this chunk repeats
    ...
    if (flags & 1) {
      ...
      if ((oldPosKey != 0) && (oldPos[0] == oldPos[1])) {
        const vector3df pos(data[0], data[1], data[2]);
        if (oldPos[1] == pos) {
    ...
    if (flags & 2) {
      ...
      if ((oldScaleKey != 0) && (oldScale[0] == oldScale[1])) {
        const vector3df scale(data[0], data[1], data[2]);
        if (oldScale[1] == scale) {
    ...
    if (flags & 4) {
      ...
      if ((oldRotKey != 0) && (oldRot[0] == oldRot[1])) {
        ...
        const quaternion rot(data[1], data[2], data[3], data[0]);
        if (oldRot[1] == rot) {
...
bool CB3DMeshFileLoader::readChunkANIM()
{
  ...
  os::Printer::log("FPS", char * ((double)animFPS), ELL_DEBUG);
#ifdef __BIG_ENDIAN__
...
#ifdef _B3D_READER_DEBUG
    os::Printer::log("read Texture", B3dTexture.TextureName);
#endif
...
#endif
#ifdef _B3D_READER_DEBUG
    os::Printer::log("Flags", char * (B3dTexture.Flags));
    os::Printer::log("Blend", char * (B3dTexture.Blend));
#endif
...
bool CB3DMeshFileLoader::readChunkBRUS()
{
  ...
  const u32 num_textures = min_(n_texs, MATERIAL_MAX_TEXTURES);
  ...
  while ((B3dStack.getLast().startposition + B3dStack.getLast().length) > B3DFile->getPos()) { //this chunk repeats
...
#endif
#ifdef _B3D_READER_DEBUG
    os::Printer::log("Blend", char * (B3dMaterial.blend));
    os::Printer::log("FX", char * (B3dMaterial.fx));
#endif
    u32 i;
    for (i = 0; i < num_textures; ++i) {
      ...
      if ((u32)texture_id < Textures_size) {
        B3dMaterial.Textures[i] = &Textures[texture_id];
#ifdef _B3D_READER_DEBUG
        os::Printer::log("Layer", char * ((int)i));
        os::Printer::log("using texture", Textures[texture_id].TextureName);
#endif
      }
      else {
    ...
    for (i = num_textures; i > 0; --i) {
      for (int j = i - 1; j < num_textures - 1; ++j) {
    ...
    if (B3dMaterial.Textures[1]) {
      if (B3dMaterial.alpha == 1.f) {
        if (B3dMaterial.Textures[1]->Blend == 5) { //(Multiply 2)
          B3dMaterial.Material.MaterialType = EMT_LIGHTMAP_M2;
        }
        else {
          B3dMaterial.Material.MaterialType = EMT_LIGHTMAP;
        }
        B3dMaterial.Material.Lighting = false;
      }
      else {
        B3dMaterial.Material.MaterialType = EMT_TRANSPARENT_VERTEX_ALPHA;
      ...
      if (B3dMaterial.Textures[0]->Flags & 0x2) { //(Alpha mapped)
        B3dMaterial.Material.MaterialType = EMT_TRANSPARENT_ALPHA_CHANNEL;
        B3dMaterial.Material.ZWriteEnable = false;
      }
      else if (B3dMaterial.Textures[0]->Flags & 0x4) { //(Masked)
        B3dMaterial.Material.MaterialType = EMT_TRANSPARENT_ALPHA_CHANNEL_REF;  // TODO: create color key texture
      }
      else if (B3dMaterial.Textures[0]->Flags & 0x40) {
        B3dMaterial.Material.MaterialType = EMT_SPHERE_MAP;
      }
      else if (B3dMaterial.Textures[0]->Flags & 0x80) {
        B3dMaterial.Material.MaterialType = EMT_SPHERE_MAP;  // TODO: Should be cube map
      }
      else if (B3dMaterial.alpha == 1.f) {
        B3dMaterial.Material.MaterialType = EMT_SOLID;
      }
      else {
        B3dMaterial.Material.MaterialType = EMT_TRANSPARENT_VERTEX_ALPHA;
        B3dMaterial.Material.ZWriteEnable = false;
    ...
    B3dMaterial.Material.DiffuseColor = SColorf(B3dMaterial.red, B3dMaterial.green, B3dMaterial.blue, B3dMaterial.alpha).toSColor();
    B3dMaterial.Material.ColorMaterial = ECM_NONE;
    //------ Material fx ------
    if (B3dMaterial.fx & 1) { //full-bright
      B3dMaterial.Material.AmbientColor = SColor(255, 255, 255, 255);
      B3dMaterial.Material.Lighting = false;
    }
    else {
    ...
    if (B3dMaterial.fx & 2) { //use vertex colors instead of brush color
      B3dMaterial.Material.ColorMaterial = ECM_DIFFUSE_AND_AMBIENT;
    }
...
{
  const bool previous32BitTextureFlag = SceneManager->getVideoDriver()->getTextureCreationFlag(ETCF_ALWAYS_32_BIT);
  SceneManager->getVideoDriver()->setTextureCreationFlag(ETCF_ALWAYS_32_BIT, true);
  // read texture from disk
  // note that mipmaps might be disabled by Flags & 0x8
  const bool doMipMaps = SceneManager->getVideoDriver()->getTextureCreationFlag(ETCF_CREATE_MIP_MAPS);
  int i;
  for (i = 0; i < MATERIAL_MAX_TEXTURES; ++i) {
    SB3dTexture* B3dTexture = material.Textures[i];
    if (B3dTexture && B3dTexture->TextureName_size && !material.Material.getTexture(i)) {
      if (!SceneManager->getParameters()->getAttributeAsBool(B3D_LOADER_IGNORE_MIPMAP_FLAG)) {
        SceneManager->getVideoDriver()->setTextureCreationFlag(ETCF_CREATE_MIP_MAPS, (B3dTexture->Flags & 0x8) ? true : false);
      }
      {
        ITexture* tex = 0;
        IFileSystem* fs = SceneManager->getFileSystem();
        char* texnameWithUserPath(SceneManager->getParameters()->getAttributeAsString(B3D_TEXTURE_PATH));
        if (texnameWithUserPath_size) {
          texnameWithUserPath += '/';
      ...
      if (material.Textures[i]->Flags & 0x10) { // Clamp U
        material.Material.TextureLayer[i].TextureWrapU = ETC_CLAMP;
      }
      if (material.Textures[i]->Flags & 0x20) { // Clamp V
        material.Material.TextureLayer[i].TextureWrapV = ETC_CLAMP;
      }
    }
  }
  SceneManager->getVideoDriver()->setTextureCreationFlag(ETCF_CREATE_MIP_MAPS, doMipMaps);
  SceneManager->getVideoDriver()->setTextureCreationFlag(ETCF_ALWAYS_32_BIT, previous32BitTextureFlag);
}
void CB3DMeshFileLoader::readString(char* newstring)
{
  newstring = "";
