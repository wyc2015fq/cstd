...
//! Constructor
CMS3DMeshFileLoader::CMS3DMeshFileLoader(IVideoDriver* driver)
...
bool CMS3DMeshFileLoader::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "ms3d");
}
//! creates/loads an animated mesh from the file.
//! \return Pointer to the created mesh. Returns 0 if loading failed.
//! If you no longer need the mesh, you should call IAnimatedMesh::drop().
//! See IReferenceCounted::drop() for more information.
IAnimatedMesh* CMS3DMeshFileLoader::createMesh(IReadFile* file)
{
  if (!file) {
    return 0;
  }
...
bool CMS3DMeshFileLoader::load(IReadFile* file)
{
  int tmp;
  if (!file) {
    return false;
  ...
  os::Printer::log("Loaded header version", char * (pHeader->Version));
#endif
...
#endif
#ifdef _IRR_DEBUG_MS3D_LOADER_
  os::Printer::log("Load vertices", char * (numVertices));
#endif
  pPtr += sizeof(u16);
...
#endif
#ifdef _IRR_DEBUG_MS3D_LOADER_
  os::Printer::log("Load Triangles", char * (numTriangles));
#endif
  pPtr += sizeof(u16);
  ...
  for (tmp = 0; tmp < numTriangles; ++tmp) {
#ifdef __BIG_ENDIAN__
...
#endif
#ifdef _IRR_DEBUG_MS3D_LOADER_
  os::Printer::log("Load Groups", char * (numGroups));
#endif
  pPtr += sizeof(u16);
  SGroup > groups;
...
#endif
#ifdef _IRR_DEBUG_MS3D_LOADER_
  os::Printer::log("Load Materials", char * (numMaterials));
#endif
  pPtr += sizeof(u16);
  if (numMaterials == 0) {
  ...
  for (i = 0; i < numMaterials; ++i) {
    ...
    SSkinMeshBuffer* tmpBuffer = AnimatedMesh->addMeshBuffer();
    tmpBuffer->Material.MaterialType = EMT_SOLID;
    tmpBuffer->Material.AmbientColor = SColorf(material->Ambient[0], material->Ambient[1], material->Ambient[2], material->Ambient[3]).toSColor();
    tmpBuffer->Material.DiffuseColor = SColorf(material->Diffuse[0], material->Diffuse[1], material->Diffuse[2], material->Diffuse[3]).toSColor();
    tmpBuffer->Material.EmissiveColor = SColorf(material->Emissive[0], material->Emissive[1], material->Emissive[2], material->Emissive[3]).toSColor();
    tmpBuffer->Material.SpecularColor = SColorf(material->Specular[0], material->Specular[1], material->Specular[2], material->Specular[3]).toSColor();
    tmpBuffer->Material.Shininess = material->Shininess;
    char* TexturePath(material->Texture);
    if (TexturePath.trim() != "") {
    ...
    char* AlphamapPath = (const c8*)material->Alphamap;
...
#endif
#ifdef _IRR_DEBUG_MS3D_LOADER_
  os::Printer::log("FPS", char * (framesPerSecond));
#endif
...
#endif
#ifdef _IRR_DEBUG_MS3D_LOADER_
  os::Printer::log("Joints", char * (jointCount));
#endif
  pPtr += sizeof(u16);
  if (pPtr > buffer + fileSize) {
    delete [] buffer;
  ...
  for (i = 0; i < jointCount; ++i) {
...
#ifdef _IRR_DEBUG_MS3D_LOADER_
    os::Printer::log("Joint", jnt->Name);
    os::Printer::log("Rotation keyframes", char * (pJoint->NumRotationKeyframes));
    os::Printer::log("Translation keyframes", char * (pJoint->NumTranslationKeyframes));
#endif
    jnt->LocalMatrix.makeIdentity();
    jnt->LocalMatrix.setRotationRadians(
        vector3df(pJoint->Rotation[0], pJoint->Rotation[1], pJoint->Rotation[2]));
    ...
    jnt->LocalMatrix.setTranslation(
        vector3df(pJoint->Translation[0], pJoint->Translation[1], -pJoint->Translation[2]));
    ...
    for (j = 0; j < numRotationKeyframes; ++j) {
      ...
      tmpMatrix.setRotationRadians(
          vector3df(kf->Parameter[0], kf->Parameter[1], kf->Parameter[2]));
      // convert right-handed to left-handed
      ...
      k->rotation  = quaternion(tmpMatrix.getTransposed());
    }
...
#endif
    pPtr += sizeof(s32);
    for (int j = 0; j < 4; ++j) { // four comment groups
#ifdef _IRR_DEBUG_MS3D_LOADER_
      os::Printer::log("Skipping comment group", char * (j + 1));
#endif
  ...
  for (int jointnum = 0; jointnum < AnimatedMesh->getAllJoints().size(); ++jointnum) {
    for (int j2 = 0; j2 < AnimatedMesh->getAllJoints().size(); ++j2) {
  ...
  for (i = 0; i < numTriangles; ++i) {
    ...
    for (s32 j = 2; j != -1; --j) {
      ...
      if (triangles[i].GroupIndex < groups_size &&
  ...
  for (i = 0; i < groups_size; ++i) {
    ...
    u16* indices = AnimatedMesh->getMeshBuffers()[grp.MaterialIdx]->Indices;
    for (int k = 0; k < grp.VertexIds_size; ++k)
      for (int l = 0; l < 3; ++l) {
...
char* CMS3DMeshFileLoader::stripPathFromString(const char* inString, bool returnPath) const
{
  ...
  if (returnPath) {
    ...
    return inString.subString(slashIndex + 1, inString_size - (slashIndex + 1));
  }
}
#endif
