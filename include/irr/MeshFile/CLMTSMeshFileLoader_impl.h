...
- Used the irrlicht Logger instead of cerr to output warnings and errors.
  ...
  from:
  CLMTSMeshFileLoader(IFileSystem* fs, IVideoDriver* driver)
  to:
...
//#include "os.h"
CLMTSMeshFileLoader::CLMTSMeshFileLoader(IFileSystem* fs,
    IVideoDriver* driver, IAttributes* parameters)
...
bool CLMTSMeshFileLoader::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "lmts");
}
IAnimatedMesh* CLMTSMeshFileLoader::createMesh(IReadFile* file)
{
  u32 i;
  u32 id;
  // HEADER
  ...
  if (Header.MagicID == 0x4C4D5354) {
    FlipEndianess = true;
    os::Byteswap::BYTESWAP(Header.MagicID);
    os::Byteswap::BYTESWAP(Header.Version);
    os::Byteswap::BYTESWAP(Header.HeaderSize);
    os::Byteswap::BYTESWAP(Header.TextureCount);
    os::Byteswap::BYTESWAP(Header.SubsetCount);
    os::Byteswap::BYTESWAP(Header.TriangleCount);
    os::Byteswap::BYTESWAP(Header.SubsetSize);
    os::Byteswap::BYTESWAP(Header.VertexSize);
  }
  ...
  for (i = 0; i < Header.SubsetCount; ++i) {
    ...
    if (FlipEndianess) {
      os::Byteswap::BYTESWAP(Subsets[i].Offset);
      os::Byteswap::BYTESWAP(Subsets[i].Count);
      os::Byteswap::BYTESWAP(Subsets[i].TextID1);
      os::Byteswap::BYTESWAP(Subsets[i].TextID2);
    }
    if (subsetUserSize > 0) {
  ...
  for (i = 0; i < (Header.TriangleCount * 3); ++i) {
    ...
    if (FlipEndianess) {
      os::Byteswap::BYTESWAP(Triangles[i].X);
      os::Byteswap::BYTESWAP(Triangles[i].Y);
      os::Byteswap::BYTESWAP(Triangles[i].Z);
      os::Byteswap::BYTESWAP(Triangles[i].U1);
      os::Byteswap::BYTESWAP(Triangles[i].U2);
      os::Byteswap::BYTESWAP(Triangles[i].V1);
      os::Byteswap::BYTESWAP(Triangles[i].V2);
    }
    if (triUserSize > 0) {
...
void CLMTSMeshFileLoader::constructMesh(SMesh* mesh)
{
  for (s32 i = 0; i < Header.SubsetCount; ++i) {
    ...
    for (int sc = 0; sc < Subsets[i].Count; sc++) {
      ...
      const vector3df normal = plane3df(
  ...
  for (int j = 0; j < mesh->MeshBuffers_size; ++j) {
...
void CLMTSMeshFileLoader::loadTextures(SMesh* mesh)
{
  ...
  const char* Path = Parameters->getAttributeAsString(LMTS_TEXTURE_PATH);
  char* s;
  for (int t = 0; t < Header.TextureCount; ++t) {
    ITexture* tmptex = 0;
    s = Path;
    ...
    if (FileSystem->existFile(s)) {
      ...
      os::Printer::log("LMTS WARNING: Texture does not exist", s, ELL_WARNING);
    }
    if (Textures[t].Flags & 0x01) {
      id2id.push_back(lig_size);
      lig.push_back(tmptex);
    }
    else {
      id2id.push_back(tex_size);
  ...
  int i;
  for (i = 0; i < Header.SubsetCount; ++i) {
    if (Subsets[i].TextID1 < Header.TextureCount && id2id[Subsets[i].TextID1] < tex_size) {
      mesh->getMeshBuffer(i)->getMaterial().setTexture(0, tex[id2id[Subsets[i].TextID1]]);
    }
    if (Subsets[i].TextID2 < Header.TextureCount && id2id[Subsets[i].TextID2] < lig_size) {
    ...
    if (!mesh->getMeshBuffer(i)->getMaterial().getTexture(1)) {
      mesh->getMeshBuffer(i)->getMaterial().MaterialType = EMT_SOLID;
    }
  }
}
