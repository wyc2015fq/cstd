...
/*!
*/
void CTRTextureGouraud2_scanline_bilinear(IBurningShader* s)
{
...
#endif
#ifdef IPOL_T0
  slopeT[0][0] = (line->t[0][1][0] - line->t[0][0][0]) * invDeltaX;
  slopeT[0][1] = (line->t[0][1][1] - line->t[0][0][1]) * invDeltaX;
#endif
#ifdef IPOL_T1
  slopeT[1][0] = (line->t[1][1][0] - line->t[1][0][0]) * invDeltaX;
  slopeT[1][1] = (line->t[1][1][1] - line->t[1][0][1]) * invDeltaX;
#endif
#ifdef SUBTEXEL
...
#ifdef IPOL_C0
        r1 = tofix(line->c[0][0][1], inversew);
        g1 = tofix(line->c[0][0][2], inversew);
        b1 = tofix(line->c[0][0][3], inversew);
#endif
#else
        tx0 = tofix(line->t[0][0][0], FIX_POINT_F32_MUL);
        ty0 = tofix(line->t[0][0][1], FIX_POINT_F32_MUL);
#ifdef IPOL_C0
        getTexel_plain2_rgb(&r1, &g1, &b1, line->c[0][0]);
#endif
#endif
#ifdef IPOL_C0
        getSample_texture_rgb(&r0, &g0, &b0, &s->IT[0], tx0, ty0);
        dst[i] = fix_to_color_rgb(imulFix(r0, r1), imulFix(g0, g1), imulFix(b0, b1));
#else
#ifdef BURNINGVIDEO_RENDERER_FAST
        const tFixPointu d = dithermask [ dIndex | (i) & 3 ];
        dst[i] = getTexel_plain(&s->IT[0], d + tx0, d + ty0);
#else
        getSample_texture_rgb(&r0, &g0, &b0, s->IT, tx0, ty0);
        dst[i] = fix_to_color_rgb(r0, g0, b0);
#endif
#endif
      }
#ifdef IPOL_Z
...
#endif
#ifdef IPOL_T0
    line->t[0][0][0] += slopeT[0][0];
    line->t[0][0][1] += slopeT[0][1];
#endif
#ifdef IPOL_T1
    line->t[1][0][0] += slopeT[1][0];
    line->t[1][0][1] += slopeT[1][1];
#endif
  }
}
void CTRTextureGouraud2_drawTriangle(IBurningShader* s, const s4DVertex* a, const s4DVertex* b, const s4DVertex* c)
{
  sScanLineData* line = s->line;
  sScanConvertData* scan = s->scan;
  f32 ca, ba, cb;
  ...
  scan->t[0][0][0] = a->Tex[0][0];
  scan->t[0][0][1] = a->Tex[0][1];
#endif
#ifdef IPOL_T1
  ...
  if ((f32) 0.0 != scan->invDeltaY[1]) {
    ...
    scan->t[0][1][0] = a->Tex[0][0];
    scan->t[0][1][1] = a->Tex[0][1];
#endif
#ifdef IPOL_T1
...
#endif
#ifdef IPOL_T0
      VSET2V(line->t[0][scan->left], scan->t[0][0]);
      VSET2V(line->t[0][scan->right], scan->t[0][1]);
#endif
#ifdef IPOL_T1
...
#endif
      // render a scanline
      CTRTextureGouraud2_scanline_bilinear(s);
