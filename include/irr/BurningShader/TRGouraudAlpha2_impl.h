...
/*!
*/
void CTRGouraudAlpha2_scanline_bilinear(IBurningShader* s)
{
  ...
  for (i = 0; i <= dx; ++i) {
    ...
    if (line->z[0] < z[i])
...
#endif
      {
        ...
        getSample_color_argb(&a0, &r0, &g0, &b0, tmp);
#else
...
void CTRGouraudAlpha2_drawTriangle(IBurningShader* s, const s4DVertex* a, const s4DVertex* b, const s4DVertex* c)
{
  sScanLineData* line = s->line;
  sScanConvertData* scan = s->scan;
  f32 ca, ba, cb;
...
#endif
      // render a scanline
      CTRGouraudAlpha2_scanline_bilinear(s);
