...
/*!
*/
void CTRTextureGouraudAlphaNoZ_scanline_bilinear(IBurningShader* s)
{
...
#ifdef INVERSE_W
        inversew = fix_inverse32(line->w[0]);
        getSample_texture_argb(&a0, &r0, &g0, &b0, &s->IT[0], tofix(line->t[0][0][0], inversew), tofix(line->t[0][0][1], inversew));
#else
        getSample_texture_argb(&a0, &r0, &g0, &b0, &s->IT[0], tofix(line->t[0][0][0], FIX_POINT_F32_MUL), tofix(line->t[0][0][1], FIX_POINT_F32_MUL));
#endif
...
void CTRTextureGouraudAlphaNoZ_drawTriangle(IBurningShader* s, const s4DVertex* a, const s4DVertex* b, const s4DVertex* c)
{
  sScanLineData* line = s->line;
  sScanConvertData* scan = s->scan;
  f32 ca, ba, cb;
...
#endif
      // render a scanline
      CTRTextureGouraudAlphaNoZ_scanline_bilinear(s);
...
/*!
*/
void CTRTextureGouraudAlphaNoZ_setParam(IBurningShader* s, u32 index, f32 value)
{
#ifdef BURNINGVIDEO_RENDERER_FAST
