...
/*!
*/
void CTRGouraud2_scanline_bilinear(IBurningShader* s)
{
  ...
  for (i = 0; i <= dx; ++i) {
    ...
    if (line->z[0] < z[i])
...
#endif
      {
        ...
        VOP1a_4(tmp, = , line->c[0][0], *, inversew);
        getSample_color_rgb(&r0, &g0, &b0, tmp);
#else
        getSample_color_rgb(&r0, &g0, &b0, line->c[0][0]);
#endif
...
void CTRGouraud2_drawTriangle(IBurningShader* s, const s4DVertex* a, const s4DVertex* b, const s4DVertex* c)
{
  sScanLineData* line = s->line;
  sScanConvertData* scan = s->scan;
  f32 ca, ba, cb;
...
#endif
      // render a scanline
      CTRGouraud2_scanline_bilinear(s);
