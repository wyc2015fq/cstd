...
/*!
*/
void CTRTextureGouraudAdd2_scanline_bilinear(IBurningShader* s)
{
  ...
  for (i = 0; i <= dx; ++i) {
    ...
    if (line->z[0] < z[i])
...
#endif
      {
        ...
        dst[i] = fix_to_color_rgb(clampfix_maxcolor(r1 + r0),
...
void CTRTextureGouraudAdd2_drawTriangle(IBurningShader* s, const s4DVertex* a, const s4DVertex* b, const s4DVertex* c)
{
  sScanLineData* line = s->line;
  f32 ca, ba, cb;
  f32 temp[4];
...
#endif
      // render a scanline
      CTRTextureGouraudAdd2_scanline_bilinear(s);
