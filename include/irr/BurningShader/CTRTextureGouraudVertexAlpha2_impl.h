...
/*!
*/
void CTRTextureVertexAlpha2_scanline_bilinear(IBurningShader* s)
{
  ...
  for (i = 0; i <= dx; ++i) {
    ...
    if (line->z[0] < z[i])
...
#endif
      {
        ...
        dst[i] = PixelAdd32(
            dst[i],
            getTexel_plain(&s->IT[0], tofix(line->t[0][0][0], inversew),
...
void CTRTextureVertexAlpha2_drawTriangle(IBurningShader* s, const s4DVertex* a, const s4DVertex* b, const s4DVertex* c)
{
  sScanLineData* line = s->line;
  sScanConvertData* scan = s->scan;
  f32 ca, ba, cb;
...
#endif
      // render a scanline
      CTRTextureVertexAlpha2_scanline_bilinear(s);
