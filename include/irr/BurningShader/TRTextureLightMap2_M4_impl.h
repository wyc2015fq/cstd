...
/*!
*/
REALINLINE void CTRTextureLightMap2_M4_scanline_bilinear2_mag(IBurningShader* s)
{
  ...
  const s32 xStart = ceil32(line->x[0]);
  const s32 xEnd = ceil32(line->x[1]) - 1;
  s32 dx;
  s32 i;
  f32 invDeltaX;
  f32 subPixel;
...
#endif
  a = (f32) i + subPixel;
  VOP2a_2(line->t[0][1], = , line->t[0][1], -, line->t[0][0], invDeltaX);
  VOP2a_2(line->t[1][1], = , line->t[1][1], -, line->t[1][0], invDeltaX);
  VOP1a_2(line->t[0][0], += , line->t[0][1], *, a);
  VOP1a_2(line->t[1][0], += , line->t[1][1], *, a);
  for (; i <= dx; i++) {
#ifdef IPOL_W
    if (line->w[0] >= z[i])
#else
...
#endif
      dst[i] = fix_to_color_rgb(clampfix_maxcolor(imulFix_tex4(r0, r1)), clampfix_maxcolor(imulFix_tex4(g0, g1)), clampfix_maxcolor(imulFix_tex4(b0, b1)));
    }
#ifdef IPOL_W
    line->w[0] += line->w[1];
#else
...
/*!
*/
REALINLINE void CTRTextureLightMap2_M4_scanline_bilinear2_min(IBurningShader* s)
{
  sScanLineData* line = s->line;
  sScanConvertData* scan = s->scan;
  tVideoSample* dst;
  ...
  invDeltaX = reciprocal_approxim(line->x[1] - line->x[0]);
  // search z-buffer for first not occulled pixel
  z = (fp24*)(s->DepthBuffer->Data + line->y * s->RenderTarget->Pitch) + xStart;
  // subTexel
  subPixel = ((f32) xStart) - line->x[0];
#ifdef IPOL_W
  b = (line->w[1] - line->w[0]) * invDeltaX;
  ...
  for (; i <= dx; i++) {
    ...
    if (line->z[0] < z[i]) {
      ...
      inversew = FIX_POINT_F32_MUL;
#endif
      getTexel_fix_rgb(&r0, &g0, &b0, &s->IT[0], tofix(line->t[0][0][0], inversew), tofix(line->t[0][0][1], inversew));
      getTexel_fix_rgb(&r1, &g1, &b1, &s->IT[1], tofix(line->t[1][0][0], inversew), tofix(line->t[1][0][1], inversew));
      dst[i] = fix_to_color_rgb(clampfix_maxcolor(imulFix_tex4(r0, r1)), clampfix_maxcolor(imulFix_tex4(g0, g1)), clampfix_maxcolor(imulFix_tex4(b0, b1)));
    }
#ifdef IPOL_W
    line->w[0] += line->w[1];
...
//#ifdef BURNINGVIDEO_RENDERER_FAST
#if 1
void CTRTextureLightMap2_M4_drawTriangle(IBurningShader* s, const s4DVertex* a, const s4DVertex* b, const s4DVertex* c)
{
  void CTRTextureLightMap2_M4_drawTriangle_Mag(IBurningShader * s, const s4DVertex * a, const s4DVertex * b, const s4DVertex * c);
  void CTRTextureLightMap2_M4_drawTriangle_Min(IBurningShader * s, const s4DVertex * a, const s4DVertex * b, const s4DVertex * c);
  if (s->IT[0].lodLevel <= 2) {
    CTRTextureLightMap2_M4_drawTriangle_Mag(s, a, b, c);
  }
  else {
    CTRTextureLightMap2_M4_drawTriangle_Min(s, a, b, c);
  }
}
void CTRTextureLightMap2_M4_drawTriangle_Min(IBurningShader* s, const s4DVertex* a, const s4DVertex* b, const s4DVertex* c)
{
...
#endif
      // render a scanline
      CTRTextureLightMap2_M4_scanline_bilinear2_min(s);
...
#endif
      // render a scanline
      CTRTextureLightMap2_M4_scanline_bilinear2_mag(s);
      scan->x[0] += scan->slopeX[0];
      scan->x[1] += scan->slopeX[1];
#ifdef IPOL_Z
      scan->z[0] += scan->slopeZ[0];
