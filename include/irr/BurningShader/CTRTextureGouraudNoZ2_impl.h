...
/*!
*/
void CTRTextureGouraudNoZ2_scanline_bilinear(IBurningShader* s)
{
...
#endif
        dst[i] = getTexel_plain(&s->IT[0], tx0, ty0);
        /*
              getSample_texture ( r0, g0, b0, &s->IT[0], tx0, ty0 );
...
void CTRTextureGouraudNoZ2_drawTriangle(IBurningShader* s, const s4DVertex* a, const s4DVertex* b, const s4DVertex* c)
{
  sScanLineData* line = s->line;
  sScanConvertData* scan = s->scan;
  f32 ca, ba, cb;
...
#endif
      // render a scanline
      CTRTextureGouraudNoZ2_scanline_bilinear(s);
