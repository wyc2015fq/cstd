...
/*!
*/
void CTRTextureGouraudAddNoZ2_scanline_bilinear(IBurningShader* s)
{
  ...
  for (i = 0; i <= dx; ++i) {
    ...
    if (line->z[0] < z[i])
...
#endif
      {
        ...
        dst[i] = fix_to_color_rgb(clampfix_maxcolor(r1 + (r0 >> 1)),
...
void CTRTextureGouraudAddNoZ2_drawTriangle(IBurningShader* s, const s4DVertex* a, const s4DVertex* b, const s4DVertex* c)
{
  sScanLineData* line = s->line;
  sScanConvertData* scan = s->scan;
  f32 ca, ba, cb;
...
#endif
      // render a scanline
      CTRTextureGouraudAddNoZ2_scanline_bilinear(s);
