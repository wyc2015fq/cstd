...
#include "BurningSoftwareDriver_helper.h"
#include "BurningShader/BurningShader.inl"
typedef struct CBurningVideoDriver
{
  IImage* BackBuffer;
  void* WindowId;
  IRECT* SceneSourceRect;
  ITexture* RenderTargetTexture;
  IImage* RenderTargetSurface;
  ...
  float16 Transformation[ETS_COUNT_BURNING];
  // Vertex Cache
...
static const SVSize vSize[] = { // Vertex Cache
  ...
  { VERTEX4D_FORMAT_TEXTURE_2 | VERTEX4D_FORMAT_COLOR_1 | VERTEX4D_FORMAT_BUMP_DOT3, sizeof(S3DVertex), 2 },
  { VERTEX4D_FORMAT_TEXTURE_2 | VERTEX4D_FORMAT_COLOR_1, sizeof(S3DVertex), 2 },  // reflection map
  { 0, sizeof(f32) * 3, 0 },  // vector3df*
};
...
//#include "S4DVertex.h"
//#include "CBlit.h"
#define MAT_TEXTURE(tex) ( (CSoftwareTexture2*) Material.org.getTexture ( tex ) )

//#include "SoftwareDriver2_nouse.h"

#if 0
//! constructor
void CBurningVideoDriver_init(IFileSystem* io, IImagePresenter* presenter)
    //CurrentOut(12 * 2, 128), Temp(12 * 2, 128)
{
  ...
  if (BackBuffer) {
    ...
    if (params.ZBufferBits) {
      DepthBuffer = createDepthBuffer(BackBuffer->getDimension());
    }
    // create stencil buffer
    if (params.Stencilbuffer) {
      StencilBuffer = createStencilBuffer(BackBuffer->getDimension());
    }
  }
...
//! sets transformation
void CBurningVideoDriver::setTransform(E_TRANSFORMATION_STATE state, const float16& mat)
{
  Transformation[state] = mat;
  setbit_cond(TransformationFlag[state], mat.isIdentity(), ETF_IDENTITY);
  switch (state) {
  case ETS_VIEW:
...
bool CBurningVideoDriver::setRenderTarget(ITexture* texture, bool clearBackBuffer,
...
//! sets a render target
void CBurningVideoDriver::setRenderTarget(CImage* image)
{
  if (RenderTargetSurface) {
    RenderTargetSurface->drop();
  }
  ...
  setViewPort(IRECT(0, 0, RenderTargetSize.Width, RenderTargetSize.Height));
  if (DepthBuffer) {
...
//! sets a viewport
void CBurningVideoDriver::setViewPort(const IRECT& area)
{
  ViewPort = area;
...
/*
  ...
  for ( u32 i = 0; i!= 6; ++i )
  {
    ...
    setbit_cond( flag, dotPlane <= 0.f, 1 << i );
  }
  // this is the base for ndc frustum <-w,w>,<-w,w>,<-w,w>
  setbit_cond( flag, ( v->Pos.z - v->Pos.w ) <= 0.f, 1 );
  setbit_cond( flag, (-v->Pos.z - v->Pos.w ) <= 0.f, 2 );
  setbit_cond( flag, ( v->Pos.x - v->Pos.w ) <= 0.f, 4 );
  setbit_cond( flag, (-v->Pos.x - v->Pos.w ) <= 0.f, 8 );
  setbit_cond( flag, ( v->Pos.y - v->Pos.w ) <= 0.f, 16 );
  setbit_cond( flag, (-v->Pos.y - v->Pos.w ) <= 0.f, 32 );
*/
...
REALINLINE u32 CBurningVideoDriver::clipToFrustumTest(const s4DVertex* v) const
{
  ...
  /*
    for ( u32 i = 0; i!= 6; ++i )
    {
      setbit_cond( flag, v->Pos.dotProduct ( NDCPlane[i] ) <= 0.f, 1 << i );
    }
  */
  return flag;
}
...
/*!
 ...
 Part II:
  ...
  replace w/w by 1/w
*/
CV_INLINE void CBurningVideoDriver::ndc_2_dc_and_project(s4DVertex* dest, s4DVertex* source, u32 vIn) const
{
  u32 g;
  for (g = 0; g != vIn; g += 2) {
...
CV_INLINE void CBurningVideoDriver::ndc_2_dc_and_project2(const s4DVertex** v, const u32 size) const
{
  u32 g;
  for (g = 0; g != size; g += 1) {
    ...
    const f32 iw = reciprocal(w);
    // to device coordinates
    const f32* p = Transformation [ ETS_CLIPSCALE ];
...
*/
CV_INLINE f32 CBurningVideoDriver::screenarea(const s4DVertex* v) const
{
...
/*!
*/
CV_INLINE f32 CBurningVideoDriver::texelarea(const s4DVertex* v, int tex) const
{
  f32 z;
...
*/
CV_INLINE f32 CBurningVideoDriver::screenarea2(const s4DVertex** v) const
{
...
/*!
*/
CV_INLINE f32 CBurningVideoDriver::texelarea2(const s4DVertex** v, s32 tex) const
{
  f32 z;
...
/*!
*/
CV_INLINE void CBurningVideoDriver::select_polygon_mipmap(s4DVertex* v, u32 vIn, u32 tex, const ISIZE& texSize) const
{
  f32 f[2];
  f[0] = (f32) texSize.Width - 0.25f;
  f[1] = (f32) texSize.Height - 0.25f;
#ifdef SOFTWARE_DRIVER_2_PERSPECTIVE_CORRECT
  for (int g = 0; g != vIn; g += 2) {
...
CV_INLINE void CBurningVideoDriver::select_polygon_mipmap2(s4DVertex** v, u32 tex, const ISIZE& texSize) const
{
  f32 f[2];
  f[0] = (f32) texSize.Width - 0.25f;
  f[1] = (f32) texSize.Height - 0.25f;
...
void CBurningVideoDriver::VertexCache_fill(const u32 sourceIndex, const u32 destIndex)
{
  ...
  memcpy32_small(&dest->Tex[0], &base->TCoords,
  ...
  if (0 == (LightSpace.Flags & VERTEXTRANSFORM)) {
    ...
    */
    ...
    for (t = 0; t != vSize[VertexCache.vType].TexSize; ++t) {
      const float16& M = Transformation [ ETS_TEXTURE_0 + t ];
      // texgen
      if (TransformationFlag [ ETS_TEXTURE_0 + t ] & (ETF_TEXGEN_CAMERA_NORMAL | ETF_TEXGEN_CAMERA_REFLECTION)) {
        ...
        n.normalize_xyz();
        const f32* view = Transformation[ETS_VIEW];
        ...
        memcpy32_small(&srcT, (&base->TCoords) + t,
            sizeof(f32) * 2);
      }
      ...
      case ETC_CLAMP_TO_BORDER:
        dest->Tex[t].x = clamp((f32)(M[0] * srcT.x + M[4] * srcT.y + M[8]), 0.f, 1.f);
        break;
      case ETC_MIRROR:
        dest->Tex[t].x = M[0] * srcT.x + M[4] * srcT.y + M[8];
        if (fract(dest->Tex[t].x) > 0.5f) {
      ...
      case ETC_CLAMP_TO_EDGE:
      case ETC_CLAMP_TO_BORDER:
        dest->Tex[t].y = clamp((f32)(M[1] * srcT.x + M[5] * srcT.y + M[9]), 0.f, 1.f);
        break;
      case ETC_MIRROR:
        dest->Tex[t].y = M[1] * srcT.x + M[5] * srcT.y + M[9];
        if (fract(dest->Tex[t].y) > 0.5f) {
  ...
  if (Lights_size && (vSize[VertexCache.vType].Format & VERTEX4D_FORMAT_BUMP_DOT3)) {
    const S3DVertex* tangent = ((S3DVertex*) source);
...
#endif
  if (LightSpace.Light_size && (vSize[VertexCache.vType].Format & VERTEX4D_FORMAT_BUMP_DOT3)) {
    const S3DVertex* tangent = ((S3DVertex*) source);
    sVec4 vp;
    ...
    int i;
    for (i = 0; i < 2 && i < LightSpace.Light_size; ++i) {
...
REALINLINE void CBurningVideoDriver::VertexCache_get(const s4DVertex** face)
{
  ...
  const u32 i0 = if_c_a_else_0(VertexCache.pType != EPT_TRIANGLE_FAN, VertexCache.indicesRun);
  switch (VertexCache.iType) {
  case 1: {
    ...
    E_INDEX_TYPE iType)
{
  ...
  if (Material.org.MaterialType == EMT_REFLECTION_2_LAYER) {
    VertexCache.vType = 3;
  }
  else {
    ...
    E_VERTEX_TYPE vType, E_PRIMITIVE_TYPE pType, E_INDEX_TYPE iType)
{
  ...
  if (pType == EPT_POINTS || pType == EPT_LINE_STRIP ||
      pType == EPT_LINE_LOOP || pType == EPT_LINES || pType == EPT_POLYGON ||
      pType == EPT_POINT_SPRITES) {
    return;
  }
  if (0 == CurrentShader) {
    return;
...
s32 CBurningVideoDriver::addDynamicLight(const SLight& dl)
{
  ...
  return LightSpace.Light_size - 1;
}
//! Turns a dynamic light on or off
void CBurningVideoDriver::turnLightOn(s32 lightIndex, bool turnOn)
{
  if (lightIndex > -1 && lightIndex < (s32)LightSpace.Light_size) {
...
void CBurningVideoDriver::setMaterial(const SMaterial& material)
{
  ...
  setbit_cond(LightSpace.Flags, Material.org.Shininess != 0.f, SPECULAR);
  setbit_cond(LightSpace.Flags, Material.org.FogEnable, FOG);
  setbit_cond(LightSpace.Flags, Material.org.NormalizeNormals, NORMALIZE);
#endif
  setCurrentShader();
}
/*!
...
void CBurningVideoDriver::getCameraPosWorldSpace()
{
  ...
  const f32* M = Transformation[ETS_VIEW_INVERSE];
...
void CBurningVideoDriver::getLightPosObjectSpace()
{
  ...
  int i;
  for (i = 0; i < 1 && i < LightSpace.Light_size; ++i) {
...
void CBurningVideoDriver::lightVertex(s4DVertex* dest, u32 vertexargb)
{
  ...
  for (i = 0; i != LightSpace.Light_size; ++i) {
    const SBurningShaderLight& light = LightSpace.Light[i];
    if (!light.LightIsOn) {
      continue;
    }
...
void CBurningVideoDriver::draw2DImage(const ITexture* texture, const position2di& destPos,
    ...
    bool useAlphaChannelOfTexture)
{
  if (texture) {
    ...
    position2di dest = destPos;
    recti clip = ViewPort;
    if (ViewPort.getSize().Width != ScreenSize.Width) {
      dest.X = ViewPort.UpperLeftCorner.X + round32(destPos.X * ViewPort.getWidth() / (f32)ScreenSize.Width);
      dest.Y = ViewPort.UpperLeftCorner.Y + round32(destPos.Y * ViewPort.getHeight() / (f32)ScreenSize.Height);
      if (clipRect) {
    ...
    if (useAlphaChannelOfTexture) {
      ((CSoftwareTexture2*)texture)->getImage()->copyToWithAlpha(RenderTargetSurface, destPos, sourceRect, color, clipRect);
    } else {
      ((CSoftwareTexture2*)texture)->getImage()->copyTo(RenderTargetSurface, destPos, sourceRect, clipRect);
    }
  }
}
//! Draws a part of the texture into the rectangle.
void CBurningVideoDriver::draw2DImage(const ITexture* texture, const IRECT& destRect,
...
//! Draws a 2d line.
void CBurningVideoDriver::draw2DLine(const position2di& start,
...
void CBurningVideoDriver::draw2DRectangle(SColor color, const IRECT& pos,
    const IRECT* clip)
{
  if (clip) {
    IRECT p(pos);
...
void CBurningVideoDriver::OnResize(const ISIZE& size)
{
...
const ISIZE& CBurningVideoDriver::getCurrentRenderTargetSize() const
{
  return RenderTargetSize;
}
//!Draws an 2d rectangle with a gradient.
void CBurningVideoDriver::draw2DRectangle(const IRECT& position,
    ...
    const IRECT* clip)
{
  ...
  const dimension2d<s32> renderTargetSize(ViewPort.getSize());
...
//! Draws a 3d line.
void CBurningVideoDriver::draw3DLine(const vector3df& start,
...
const char* CBurningVideoDriver::getName() const
{
#ifdef BURNINGVIDEO_RENDERER_BEAUTIFUL
  return "Burning's Video 0.47 beautiful";
#elif defined ( BURNINGVIDEO_RENDERER_ULTRA_FAST )
  return "Burning's Video 0.47 ultra fast";
#elif defined ( BURNINGVIDEO_RENDERER_FAST )
  return "Burning's Video 0.47 fast";
#else
  return "Burning's Video 0.47";
#endif
}
//! Returns the graphics card vendor name.
char* CBurningVideoDriver::getVendorInfo()
{
...
const float16& CBurningVideoDriver::getTransform(E_TRANSFORMATION_STATE state) const
{
  return Transformation[state];
}
//! Creates a render target texture.
ITexture* CBurningVideoDriver::addRenderTargetTexture(const ISIZE& size,
...
IImage* CBurningVideoDriver::createScreenShot(ECOLOR_FORMAT format, E_RENDER_TARGET target)
{
...
ITexture* CBurningVideoDriver::createDeviceDependentTexture(IImage* surface, const char* name, void* mipmapData)
{
...
void CBurningVideoDriver::drawStencilShadowVolume(const vector3df > & triangles, bool zfail, u32 debugDataVisible)
{
  const u32 count = triangles_size;
  IBurningShader* shader = BurningShader [ ETR_STENCIL_SHADOW ];
  CurrentShader = shader;
  shader->setRenderTarget(RenderTargetSurface, ViewPort);
  Material.org.MaterialType = EMT_SOLID;
  ...
  if (true) { // zpass does not work yet
    ...
    shader->setParam(1, 1);
    shader->setParam(2, 0);
    drawVertexPrimitiveList(triangles, count, 0, count / 3, (E_VERTEX_TYPE) 4, EPT_TRIANGLES, (E_INDEX_TYPE) 4);
...
void CBurningVideoDriver::drawStencilShadow(bool clearStencilBuffer, SColor leftUpEdge,
    SColor rightUpEdge, SColor leftDownEdge, SColor rightDownEdge)
{
  if (!StencilBuffer) {
    return;
  }
...
ISIZE CBurningVideoDriver::getMaxTextureSize() const
{
  return ISIZE(SOFTWARE_DRIVER_2_TEXTURE_MAXSIZE, SOFTWARE_DRIVER_2_TEXTURE_MAXSIZE);
}
