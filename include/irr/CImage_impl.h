...
void CImage::copyTo(IImage* target, const position2di& pos)
{
  Blit(BLITTER_TEXTURE, target, 0, &pos, this, 0, 0);
}
//! copies this surface partially into another at given position
void CImage::copyTo(IImage* target, const position2di& pos, const IRECT& sourceRect, const IRECT* clipRect)
{
  Blit(BLITTER_TEXTURE, target, clipRect, &pos, this, &sourceRect, 0);
}
//! copies this surface into another, using the alpha mask, a cliprect and a color to add with
void CImage::copyToWithAlpha(IImage* target, const position2di& pos, const IRECT& sourceRect, const SColor& color, const IRECT* clipRect)
{
...
void CImage::copyToScaling(IImage* target)
{
  ...
  const dimension2du& targetSize = target->getDimension();
  if (targetSize == Size) {
    copyTo(target);
    return;
  }
...
{
  const dimension2du destSize = target->getDimension();
...
CV_INLINE SColor CImage::getPixelBox(s32 x, s32 y, s32 fx, s32 fy, s32 bias) const
{
  SColor c;
  s32 a = 0, r = 0, g = 0, b = 0;
  for (s32 dx = 0; dx != fx; ++dx) {
    for (s32 dy = 0; dy != fy; ++dy) {
      c = getPixel(s32_min(x + dx, Size.Width - 1) ,
          s32_min(y + dy, Size.Height - 1)
                  );
      a += c.getAlpha();
      r += c.getRed();
      g += c.getGreen();
