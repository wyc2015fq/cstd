...
void CTRGouraudWire_drawIndexedTriangleList(ITriangleRenderer* s, S2DVertex* vertices, s32 vertexCount, const u16* indexList, s32 triangleCount)
{
      ...
      while (span < spanEnd) {
        ...
        if (leftx >= s->ViewPortRect.l &&
          ...
          if (leftZValue > *(zTarget + leftx)) {
            ...
            *(targetSurface + leftx) = RGB16(leftR >> 8, leftG >> 8, leftB >> 8);
          }
        }
        if (rightx >= s->ViewPortRect.l &&
          ...
          if (rightZValue > *(zTarget + rightx)) {
            ...
            *(targetSurface + rightx) = RGB16(rightR, rightG, rightB);
          }
        }
        leftxf += leftdeltaxf;
