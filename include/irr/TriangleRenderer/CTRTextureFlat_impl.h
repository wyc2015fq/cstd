...
void CTRTextureFlat_drawIndexedTriangleList(ITriangleRenderer* s, S2DVertex* vertices, s32 vertexCount, const u16* indexList, s32 triangleCount)
{
        ...
        if (rightx - leftx != 0) {
          ...
          while (hSpanBegin < hSpanEnd) {
            if (spanZValue > *spanZTarget) {
              ...
              *hSpanBegin = s->lockedTexture[((spanTy >> 8)&s->textureYMask) * s->lockedTextureWidth + ((spanTx >> 8)&s->textureXMask)];
            }
            spanTx += spanTxStep;
            spanTy += spanTyStep;
