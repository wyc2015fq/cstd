...
void CTRTextureGouraud_drawIndexedTriangleList(ITriangleRenderer* s, S2DVertex* vertices, s32 vertexCount, const u16* indexList, s32 triangleCount)
{
  ...
  s32 triangleHalf;
  //
  //s->lockedZBuffer = (TZBufferType*)ZBuffer->Data;
  //
        ...
        if (rightx + tDiffRight - leftx - tDiffLeft) {
          ...
          while (hSpanBegin < hSpanEnd) {
            ...
            if (spanZValue > *spanZTarget) {
              ...
              *hSpanBegin = RGB16(getR(color) * (spanR >> 8) >> 2, getG(color) * (spanG >> 8) >> 2,
                  getB(color) * (spanB >> 8) >> 2);
            }
            spanR += spanStepR;
            spanG += spanStepG;
            spanB += spanStepB;
