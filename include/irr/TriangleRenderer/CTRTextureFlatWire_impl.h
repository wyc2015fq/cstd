...
void CTRTextureFlatWire_drawIndexedTriangleList(ITriangleRenderer* s, S2DVertex* vertices, s32 vertexCount, const u16* indexList, s32 triangleCount)
{
      ...
      while (span < spanEnd) {
        ...
        if (leftx >= s->ViewPortRect.l &&
          ...
          if (leftZValue > *(zTarget + leftx)) {
            ...
            *(targetSurface + leftx) = s->lockedTexture[((leftTy >> 8)&s->textureYMask) * s->lockedTextureWidth + ((rightTx >> 8)&s->textureXMask)];
          }
        }
        if (rightx >= s->ViewPortRect.l &&
          ...
          if (rightZValue > *(zTarget + rightx)) {
            ...
            *(targetSurface + rightx) = s->lockedTexture[((rightTy >> 8)&s->textureYMask) * s->lockedTextureWidth + ((rightTx >> 8)&s->textureXMask)];
          }
        }
        leftxf += leftdeltaxf;
