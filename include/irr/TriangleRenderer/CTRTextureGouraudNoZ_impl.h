...
void CTRTextureGouraudNoZ_drawIndexedTriangleList(ITriangleRenderer* s, S2DVertex* vertices, s32 vertexCount, const u16* indexList, s32 triangleCount)
{
      ...
      while (span < spanEnd) {
        ...
        if (rightx + tDiffRight - leftx - tDiffLeft) {
          ...
          while (hSpanBegin < hSpanEnd) {
            ...
            *hSpanBegin = RGB16(getR(color) * (spanR >> 8) >> 2,
                getG(color) * (spanG >> 8) >> 2,
