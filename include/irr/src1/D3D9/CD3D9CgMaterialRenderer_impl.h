...
void CD3D9CgUniformSampler2D::update(const void* data, const SMaterial& material) const
{
  ...
  if (material.TextureLayer[LayerID].Texture) {
    IDirect3DBaseTexture9* Texture = reinterpret_cast<CD3D9Texture*>(material.TextureLayer[LayerID].Texture)->getDX9Texture();
...
CD3D9CgMaterialRenderer::CD3D9CgMaterialRenderer(CD3D9Driver* driver, s32& materialType,
    ...
    E_PRIMITIVE_TYPE inType, E_PRIMITIVE_TYPE outType, u32 vertices,
    ...
    E_PRIMITIVE_TYPE inType, E_PRIMITIVE_TYPE outType, u32 vertices)
{
  ...
  for (unsigned int i = 0; i < UniformInfo_size; ++i) {
