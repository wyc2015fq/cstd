...
CD3D9HLSLMaterialRenderer::CD3D9HLSLMaterialRenderer(IDirect3DDevice9* d3ddev,
    IVideoDriver* driver, s32& outMaterialTypeNr,
    ...
    const char* shaderTargetName)
{
  ...
  FILE* f = fopen(tmp, "wb");
  fwrite(vertexShaderProgram, cstr_len(vertexShaderProgram), 1, f);
  fflush(f);
  fclose(f);
    ...
    const char* shaderTargetName)
{
  ...
  if (Driver->queryFeature(EVDF_VERTEX_SHADER_2_0) || Driver->queryFeature(EVDF_VERTEX_SHADER_3_0))
  ...
  FILE* f = fopen(tmp, "wb");
  fwrite(pixelShaderProgram, cstr_len(pixelShaderProgram), 1, f);
  fflush(f);
  fclose(f);
  HRESULT h = stubD3DXCompileShaderFromFile(
      tmp,
    ...
    const f32* floats, int count)
{
  ...
  if (!hndl) {
    char* s = "HLSL Variable to set not found: '";
    s += name;
    s += "'. Available variables are:";
    os::Printer::log(s, ELL_WARNING);
    ...
    const s32* ints, int count)
{
  ...
  tbl->GetConstantDesc(hndl, &Description, &ucount);
  if (Description.RegisterSet != D3DXRS_SAMPLER) {
    HRESULT hr = tbl->SetIntArray(pID3DDevice, hndl, (const int*)ints, count);
    if (FAILED(hr)) {
...
void CD3D9HLSLMaterialRenderer::printHLSLVariables(LPD3DXCONSTANTTABLE table)
{
  ...
  if (!FAILED(hr)) {
    int i;
    for (i = 0; i < (int)tblDesc.Constants; ++i) {
      D3DXCONSTANT_DESC d;
      UINT n = 1;
