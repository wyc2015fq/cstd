    ...
    CurrentScale(0.0f)
{
  ...
  if (!driver->queryFeature(EVDF_PIXEL_SHADER_1_4) ||
      !driver->queryFeature(EVDF_VERTEX_SHADER_1_1)) {
  ...
  if (renderer) {
    ...
    CD3D9ParallaxMapRenderer* nmr = (CD3D9ParallaxMapRenderer*)renderer;
    ...
    CReadFile* file = new CReadFile("parallax.psh");
    s32 sz = file->getSize();
...
void CD3D9ParallaxMapRenderer::OnSetMaterial(const SMaterial& material,
...
void CD3D9ParallaxMapRenderer::OnSetConstants(IMaterialRendererServices* services, s32 userData)
{
  ...
  services->setVertexShaderConstant(driver->getTransform(ETS_WORLD).getTransposed(), 0, 4);
  // set eye position
  ...
  matrix4 minv = driver->getTransform(ETS_VIEW);
  minv.makeInverse();
  ...
  services->setVertexShaderConstant(worldViewProj.getTransposed(), 8, 4);
