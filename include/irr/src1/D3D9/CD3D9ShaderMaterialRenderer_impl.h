...
#endif
//! Public constructor
CD3D9ShaderMaterialRenderer::CD3D9ShaderMaterialRenderer(IDirect3DDevice9* d3ddev, IVideoDriver* driver,
...
void CD3D9ShaderMaterialRenderer::OnSetMaterial(const SMaterial& material, const SMaterial& lastMaterial,
...
bool CD3D9ShaderMaterialRenderer::createPixelShader(const c8* pxsh)
{
  ...
  stubD3DXAssembleShader(pxsh, (UINT)cstr_len(pxsh), 0, 0, 0, &code, &errors);
#else
...
bool CD3D9ShaderMaterialRenderer::createVertexShader(const char* vtxsh)
{
...
#ifdef _IRR_D3D_NO_SHADER_DEBUGGING
  // compile shader without debug info
  stubD3DXAssembleShader(vtxsh, (UINT)cstr_len(vtxsh), 0, 0, 0, &code, &errors);
#else
  // compile shader and emitt some debug informations to
  // make it possible to debug the shader in visual studio
  static int irr_dbg_file_nr = 0;
  ...
  FILE* f = fopen(tmp, "wb");
  fwrite(vtxsh, cstr_len(vtxsh), 1, f);
  fflush(f);
  fclose(f);
  stubD3DXAssembleShaderFromFile(tmp, 0, 0, D3DXSHADER_DEBUG, &code, &errors);
#endif
...
#pragma comment (lib, "d3dx9.lib")
...
#else
  {
    ...
    if (!pFn && !LoadFailed) {
      // try to load dll
      char* strDllName = "d3dx9_";
      strDllName += (int)D3DX_SDK_VERSION;
      strDllName += ".dll";
      HMODULE hMod = LoadLibrary(strDllName);
      if (hMod) {
