...
CV_INLINE void setTextureColorStage(IDirect3DDevice9* dev, DWORD i,
...
CV_INLINE void setTextureColorStage(IDirect3DDevice9* dev, DWORD i, DWORD arg1)
{
  dev->SetTextureStageState(i, D3DTSS_COLOROP, D3DTOP_SELECTARG1);
  dev->SetTextureStageState(i, D3DTSS_COLORARG1, arg1);
}
CV_INLINE void setTextureAlphaStage(IDirect3DDevice9* dev, DWORD i,
    DWORD arg1, DWORD op, DWORD arg2)
{
...
CV_INLINE void setTextureAlphaStage(IDirect3DDevice9* dev, DWORD i, DWORD arg1)
{
...
public:
  //! Constructor
  CD3D9MaterialRenderer(IDirect3DDevice9* d3ddev, IVideoDriver* driver)
  ...
  bool setVariable(bool vertexShader, const c8* name, const f32* floats, int count) {
    os::Printer::log("Invalid material to set variable in.");
    return false;
  }
  //! Bool interface for the above.
  bool setVariable(bool vertexShader, const c8* name, const bool* bools, int count) {
    os::Printer::log("Invalid material to set variable in.");
    return false;
  }
  //! Int interface for the above.
  bool setVariable(bool vertexShader, const c8* name, const s32* ints, int count) {
    os::Printer::log("Invalid material to set variable in.");
    return false;
  }
protected:
...
{
public:
  CD3D9MaterialRenderer_SOLID(IDirect3DDevice9* p, IVideoDriver* d)
...
{
public:
  CD3D9MaterialRenderer_ONETEXTURE_BLEND(IDirect3DDevice9* p, IVideoDriver* d)
    : CD3D9MaterialRenderer(p, d) {}
  void OnSetMaterial(const SMaterial& material, const SMaterial& lastMaterial,
      bool resetAllRenderstates, IMaterialRendererServices* services) {
    services->setBasicRenderStates(material, lastMaterial, resetAllRenderstates);
...
private:
  ...
  u32 getD3DModulate(E_MODULATE_FUNC func) const {
    ...
    switch (func) {
    case EMFN_MODULATE_1X:
      r = D3DTOP_MODULATE;
      break;
    case EMFN_MODULATE_2X:
      r = D3DTOP_MODULATE2X;
      break;
    case EMFN_MODULATE_4X:
      r = D3DTOP_MODULATE4X;
      break;
    }
    return r;
...
{
public:
  CD3D9MaterialRenderer_SOLID_2_LAYER(IDirect3DDevice9* p, IVideoDriver* d)
    : CD3D9MaterialRenderer(p, d) {}
  void OnSetMaterial(const SMaterial& material, const SMaterial& lastMaterial,
      bool resetAllRenderstates, IMaterialRendererServices* services) {
    services->setBasicRenderStates(material, lastMaterial, resetAllRenderstates);
...
{
public:
  CD3D9MaterialRenderer_TRANSPARENT_ADD_COLOR(IDirect3DDevice9* p, IVideoDriver* d)
    : CD3D9MaterialRenderer(p, d) {}
  void OnSetMaterial(const SMaterial& material, const SMaterial& lastMaterial,
      bool resetAllRenderstates, IMaterialRendererServices* services) {
    services->setBasicRenderStates(material, lastMaterial, resetAllRenderstates);
...
{
public:
  CD3D9MaterialRenderer_TRANSPARENT_VERTEX_ALPHA(IDirect3DDevice9* p, IVideoDriver* d)
    : CD3D9MaterialRenderer(p, d) {}
  void OnSetMaterial(const SMaterial& material, const SMaterial& lastMaterial,
      bool resetAllRenderstates, IMaterialRendererServices* services) {
    services->setBasicRenderStates(material, lastMaterial, resetAllRenderstates);
...
{
public:
  CD3D9MaterialRenderer_TRANSPARENT_ALPHA_CHANNEL(IDirect3DDevice9* p, IVideoDriver* d)
    : CD3D9MaterialRenderer(p, d) {}
  void OnSetMaterial(const SMaterial& material, const SMaterial& lastMaterial,
      bool resetAllRenderstates, IMaterialRendererServices* services) {
    services->setBasicRenderStates(material, lastMaterial, resetAllRenderstates);
    if (material.MaterialType != lastMaterial.MaterialType || resetAllRenderstates
      ...
      pID3DDevice->SetRenderState(D3DRS_SRCBLEND, D3DBLEND_SRCALPHA);
      pID3DDevice->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA);
      pID3DDevice->SetRenderState(D3DRS_ALPHAREF, floor32(material.MaterialTypeParam * 255.f));
...
{
public:
  CD3D9MaterialRenderer_TRANSPARENT_ALPHA_CHANNEL_REF(IDirect3DDevice9* p, IVideoDriver* d)
    : CD3D9MaterialRenderer(p, d) {}
  void OnSetMaterial(const SMaterial& material, const SMaterial& lastMaterial,
      bool resetAllRenderstates, IMaterialRendererServices* services) {
    services->setBasicRenderStates(material, lastMaterial, resetAllRenderstates);
...
{
public:
  CD3D9MaterialRenderer_LIGHTMAP(IDirect3DDevice9* p, IVideoDriver* d)
    : CD3D9MaterialRenderer(p, d) {}
  void OnSetMaterial(const SMaterial& material, const SMaterial& lastMaterial,
      bool resetAllRenderstates, IMaterialRendererServices* services) {
    services->setBasicRenderStates(material, lastMaterial, resetAllRenderstates);
...
{
public:
  CD3D9MaterialRenderer_DETAIL_MAP(IDirect3DDevice9* p, IVideoDriver* d)
    : CD3D9MaterialRenderer(p, d) {}
  void OnSetMaterial(const SMaterial& material, const SMaterial& lastMaterial,
      bool resetAllRenderstates, IMaterialRendererServices* services) {
    services->setBasicRenderStates(material, lastMaterial, resetAllRenderstates);
...
{
public:
  CD3D9MaterialRenderer_SPHERE_MAP(IDirect3DDevice9* p, IVideoDriver* d)
    : CD3D9MaterialRenderer(p, d) {}
  void OnSetMaterial(const SMaterial& material, const SMaterial& lastMaterial,
      bool resetAllRenderstates, IMaterialRendererServices* services) {
    services->setBasicRenderStates(material, lastMaterial, resetAllRenderstates);
...
{
public:
  CD3D9MaterialRenderer_REFLECTION_2_LAYER(IDirect3DDevice9* p, IVideoDriver* d)
    : CD3D9MaterialRenderer(p, d) {}
  void OnSetMaterial(const SMaterial& material, const SMaterial& lastMaterial,
      bool resetAllRenderstates, IMaterialRendererServices* services) {
    services->setBasicRenderStates(material, lastMaterial, resetAllRenderstates);
...
{
public:
  CD3D9MaterialRenderer_TRANSPARENT_REFLECTION_2_LAYER(IDirect3DDevice9* p, IVideoDriver* d)
    : CD3D9MaterialRenderer(p, d) {}
  void OnSetMaterial(const SMaterial& material, const SMaterial& lastMaterial,
      bool resetAllRenderstates, IMaterialRendererServices* services) {
    services->setBasicRenderStates(material, lastMaterial, resetAllRenderstates);
