...
CD3D9Driver::CD3D9Driver(const SIrrlichtCreationParameters& params, IFileSystem* io)
  : CNullDriver(io, params.WindowSize), CurrentRenderMode(ERM_NONE),
    ...
    LastVertexType((E_VERTEX_TYPE) - 1), VendorID(0),
    ...
    AlphaToCoverageSupport(false), Params(params)
{
  ...
  SphereMapMatrixD3D9._11 = 0.5f;
  SphereMapMatrixD3D9._12 = 0.0f;
  SphereMapMatrixD3D9._13 = 0.0f;
  SphereMapMatrixD3D9._14 = 0.0f;
  SphereMapMatrixD3D9._21 = 0.0f;
  SphereMapMatrixD3D9._22 = -0.5f;
  SphereMapMatrixD3D9._23 = 0.0f;
  SphereMapMatrixD3D9._24 = 0.0f;
  SphereMapMatrixD3D9._31 = 0.0f;
  SphereMapMatrixD3D9._32 = 0.0f;
  SphereMapMatrixD3D9._33 = 1.0f;
  SphereMapMatrixD3D9._34 = 0.0f;
  SphereMapMatrixD3D9._41 = 0.5f;
  SphereMapMatrixD3D9._42 = 0.5f;
  SphereMapMatrixD3D9._43 = 0.0f;
  SphereMapMatrixD3D9._44 = 1.0f;
  matrix4 mat;
...
bool CD3D9Driver::initDriver(HWND hwnd, bool pureSoftware)
{
  ...
  if (!FAILED(pID3D->GetAdapterIdentifier(Params.DisplayAdapter, 0, &dai))) {
    ...
    case 0x10DE :
      ...
      break;
    case 0x102B :
      VendorName = "Matrox Electronic Systems Ltd.";
      break;
    case 0x121A :
    ...
    case 0x8086 :
      ...
      break;
    default:
      VendorName = "Unknown VendorId: ";
      VendorName += (int)dai.VendorId;
      break;
    }
  }
  ...
  os::Printer::log("Currently available Video Memory (kB)", char * (pID3DDevice->GetAvailableTextureMem() / 1024));
  ...
  MaxTextureUnits = min_((u32)Caps.MaxSimultaneousTextures, MATERIAL_MAX_TEXTURES);
  ...
  DriverAttributes->setAttribute("MaxTextureSize", (s32)min_(Caps.MaxTextureHeight, Caps.MaxTextureWidth));
  DriverAttributes->setAttribute("MaxTextureLODBias", (s32)16);
  DriverAttributes->setAttribute("Version", (s32)901);
  ...
  DriverAttributes->setAttribute("AntiAlias", (s32)Params.AntiAlias);
...
bool CD3D9Driver::setActiveTexture(u32 stage, const ITexture* texture)
{
...
ITexture* CD3D9Driver::createDeviceDependentTexture(IImage* surface, const char* name, void* mipmapData)
{
...
bool CD3D9Driver::setRenderTarget(const IRenderTarget* targets,
    bool clearBackBuffer, bool clearZBuffer, SColor color)
{
  int i;
  if (targets_size == 0) {
  ...
  u32 maxMultipleRTTs = min_(MaxMRTs, targets_size);
  ...
  for (i = maxMultipleRTTs; i < NumSetMRTs; i++) {
    pID3DDevice->SetRenderTarget(i, NULL);
  }
...
//! sets a viewport
void CD3D9Driver::setViewPort(const IRECT& area)
{
  IRECT vp = area;
  IRECT rendert(0, 0, getCurrentRenderTargetSize().Width, getCurrentRenderTargetSize().Height);
...
const IRECT& CD3D9Driver::getViewPort() const
{
  return ViewPort;
}
...
CD3D9Driver::SHWBufferLink* CD3D9Driver::createHardwareBuffer(const IMeshBuffer* mb)
{
...
void CD3D9Driver::drawHardwareBuffer(SHWBufferLink* _HWBuffer)
{
  ...
  drawVertexPrimitiveList(vPtr, mb->getVertexCount(), iPtr, mb->getIndexCount() / 3, mb->getVertexType(), EPT_TRIANGLES, mb->getIndexType());
  if (HWBuffer->vertexBuffer) {
...
void CD3D9Driver::addOcclusionQuery(ISceneNode* node,
    const IMesh* mesh)
{
...
void CD3D9Driver::removeOcclusionQuery(ISceneNode* node)
{
  const s32 index = OcclusionQueries.linear_search(SOccQuery(node));
  if (index != -1) {
...
void CD3D9Driver::runOcclusionQuery(ISceneNode* node, bool visible)
{
  if (!node) {
    return;
  }
...
void CD3D9Driver::updateOcclusionQuery(ISceneNode* node, bool block)
{
  const s32 index = OcclusionQueries.linear_search(SOccQuery(node));
  if (index != -1) {
    // not yet started
...
u32 CD3D9Driver::getOcclusionQueryResult(ISceneNode* node) const
{
  const s32 index = OcclusionQueries.linear_search(SOccQuery(node));
  if (index != -1) {
...
void CD3D9Driver::draw2DImage(const ITexture* texture,
    const IRECT& destRect,
    const IRECT& sourceRect,
    const IRECT* clipRect,
    const SColor* const colors,
    bool useAlphaChannelOfTexture)
{
  ...
  setActiveTexture(0, const_cast<ITexture*>(texture));
...
void CD3D9Driver::draw2DImageBatch(const ITexture* texture,
    ...
    bool useAlphaChannelOfTexture)
{
  ...
  if (!setActiveTexture(0, const_cast<ITexture*>(texture))) {
    return;
  }
  setRenderStates2DMode(color.getAlpha() < 255, true, useAlphaChannelOfTexture);
  const u32 drawCount = min_<u32>(positions_size, sourceRects_size);
  S3DVertex* vtx(drawCount * 4);
  u16* indices(drawCount * 6);
  int i;
  for (i = 0; i < drawCount; i++) {
    position2di targetPos = positions[i];
    position2di sourcePos = sourceRects[i].UpperLeftCorner;
    // This needs to be signed as it may go negative.
    dimension2d<s32> sourceSize(sourceRects[i].getSize());
    if (clipRect) {
    ...
    const IRECT poss(targetPos, sourceSize);
  ...
  if (vtx_size) {
    setVertexShader(EVT_STANDARD);
    pID3DDevice->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, vtx_size, indices_size / 3, indices,
        D3DFMT_INDEX16, vtx, sizeof(S3DVertex));
  }
}
    ...
    bool useAlphaChannelOfTexture)
{
  ...
  position2di targetPos = pos;
  position2di sourcePos = sourceRect.UpperLeftCorner;
  // This needs to be signed as it may go negative.
  dimension2d<s32> sourceSize(sourceRect.getSize());
  if (clipRect) {
    if (targetPos.X < clipRect->UpperLeftCorner.X) {
      sourceSize.Width += targetPos.X - clipRect->UpperLeftCorner.X;
      if (sourceSize.Width <= 0) {
...
void CD3D9Driver::draw2DRectangle(const IRECT& position,
...
//! Draws a 2d line.
void CD3D9Driver::draw2DLine(const position2di& start,
...
bool CD3D9Driver::setRenderStates3DMode()
{
  ...
  if (ResetRenderStates || LastMaterial != Material) {
    ...
    if (CurrentRenderMode == ERM_3D &&
        ...
        LastMaterial.MaterialType >= 0 && LastMaterial.MaterialType < (s32)MaterialRenderers_size) {
      MaterialRenderers[LastMaterial.MaterialType].Renderer->OnUnsetMaterial();
    }
    // set new material.
    if (Material.MaterialType >= 0 && Material.MaterialType < (s32)MaterialRenderers_size)
    ...
    bool resetAllRenderstates)
{
  ...
  // specular highlights
  if (resetAllRenderstates || !equals(lastmaterial.Shininess, material.Shininess)) {
  ...
  for (int st = 0; st < MaxTextureUnits; ++st) {
    ...
    if (resetAllRenderstates ||
      ...
      if (material.TextureLayer[st].BilinearFilter || material.TextureLayer[st].TrilinearFilter || material.TextureLayer[st].AnisotropicFilter) {
        ...
        if (tftMag == D3DTEXF_ANISOTROPIC || tftMin == D3DTEXF_ANISOTROPIC) {
          pID3DDevice->SetSamplerState(st, D3DSAMP_MAXANISOTROPY, min_((DWORD)material.TextureLayer[st].AnisotropicFilter, Caps.MaxAnisotropy));
        }
...
void CD3D9Driver::setRenderStatesStencilShadowMode(bool zfail, u32 debugDataVisible)
{
  if ((CurrentRenderMode != ERM_SHADOW_VOLUME_ZFAIL &&
    ...
    if (CurrentRenderMode == ERM_3D &&
        static_cast<u32>(Material.MaterialType) < MaterialRenderers_size) {
...
void CD3D9Driver::setRenderStates2DMode(bool alpha, bool texture, bool alphaChannel)
{
  ...
  if (CurrentRenderMode != ERM_2D || Transformation3DChanged) {
    // unset last 3d material
    if (CurrentRenderMode == ERM_3D) {
      if (static_cast<u32>(LastMaterial.MaterialType) < MaterialRenderers_size) {
        MaterialRenderers[LastMaterial.MaterialType].Renderer->OnUnsetMaterial();
      }
    }
    ...
    //    if (!texture)
    //      m.setTranslation(float3(0.5f,0.5f,0));
    pID3DDevice->SetTransform(D3DTS_WORLD, (D3DMATRIX*)((void*)m));
    // adjust the view such that pixel center aligns with texels
    // Otherwise, subpixel artifacts will occur
    m.setTranslation(float3(-0.5f, -0.5f, 0));
    pID3DDevice->SetTransform(D3DTS_VIEW, (D3DMATRIX*)((void*)m));
    const dimension2du& renderTargetSize = getCurrentRenderTargetSize();
    m.buildProjectionMatrixOrthoLH(f32(renderTargetSize.Width), f32(-(s32)(renderTargetSize.Height)), -1.0, 1.0);
    m.setTranslation(float3(-1, 1, 0));
    pID3DDevice->SetTransform(D3DTS_PROJECTION, (D3DMATRIX*)((void*)m));
  ...
  pID3DDevice->SetTextureStageState(0, D3DTSS_COLORARG2, D3DTA_DIFFUSE);
  if (texture) {
    setTransform(ETS_TEXTURE_0, IdentityMatrix);
...
s32 CD3D9Driver::addDynamicLight(const SLight& dl)
{
  ...
  light.Range = min_(dl.Radius, MaxLightDistance);
...
const char* CD3D9Driver::getName() const
{
  return "Direct3D 9.0";
}
...
void CD3D9Driver::drawStencilShadowVolume(const float3 > & triangles, bool zfail, u32 debugDataVisible)
{
  if (!Params.Stencilbuffer) {
    return;
  }
  ...
  if (!zfail) {
    ...
    pID3DDevice->DrawPrimitiveUP(D3DPT_TRIANGLELIST, count / 3, triangles, sizeof(float3));
...
void CD3D9Driver::drawStencilShadow(bool clearStencilBuffer, SColor leftUpEdge,
...
//! Draws a 3d line.
void CD3D9Driver::draw3DLine(const float3 start,
...
bool CD3D9Driver::reset()
{
  ...
  for (i = 1; i < DepthBuffers_size; ++i) {
    for (int j = 0; j < Textures_size; ++j) {
  ...
  if (FAILED(hr)) {
    if (hr == D3DERR_DEVICELOST) {
      ...
      os::Printer::log("Resetting failed due to unknown reason.", char * ((int)hr), ELL_WARNING);
    }
    return false;
  }
  DeviceLost = false;
...
void CD3D9Driver::OnResize(const dimension2du& size)
{
  if (!pID3DDevice) {
    return;
  }
...
const matrix4& CD3D9Driver::getTransform(E_TRANSFORMATION_STATE state) const
{
...
ITexture* CD3D9Driver::addRenderTargetTexture(const dimension2du& size,
    const char* name,
...
IImage* CD3D9Driver::createScreenShot(ECOLOR_FORMAT format, E_RENDER_TARGET target)
{
  ...
  RECT clientRect;
  {
    ...
    clientRect.left = max_(clientRect.left, 0l);
    clientRect.top = max_(clientRect.top, 0l);
    clientRect.right = min_(clientRect.right, (long)displayMode.Width);
    clientRect.bottom = min_(clientRect.bottom, (long)displayMode.Height);
  }
  ...
  if (newImage) {
    ...
    if (D3DFMT_X8R8G8B8 == displayMode.Format && (0xFF000000 != (*dP & 0xFF000000))) {
      for (int y = 0; y < shotSize.Height; ++y) {
        for (int x = 0; x < shotSize.Width; ++x) {
      ...
      for (int y = 0; y < shotSize.Height; ++y) {
        convertColor(sP, ECF_A8R8G8B8, shotSize.Width, dP, format);
        sP += lockedRect.Pitch;
        dP += shotSize.Width;
      }
    }
...
const dimension2du& CD3D9Driver::getCurrentRenderTargetSize() const
{
...
bool CD3D9Driver::setClipPlane(u32 index, const plane3df& plane, bool enable)
{
...
void CD3D9Driver::checkDepthBuffer(ITexture* tex)
{
  ...
  const dimension2du optSize = tex->getSize().getOptimalSize(
      !queryFeature(EVDF_TEXTURE_NPOT),
      !queryFeature(EVDF_TEXTURE_NSQUARE), true);
  SDepthSurface* depth = 0;
  dimension2du destSize(0x7fffffff, 0x7fffffff);
  int i;
  for (i = 0; i < DepthBuffers_size; ++i) {
  ...
  if (!depth) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      DepthBuffers.erase(DepthBuffers_size - 1);
    }
  }
  else {
    depth->grab();
...
dimension2du CD3D9Driver::getMaxTextureSize() const
{
  return dimension2du(Caps.MaxTextureWidth, Caps.MaxTextureHeight);
}
#ifdef _IRR_COMPILE_WITH_CG_
...
IVideoDriver* createDirectX9Driver(const SIrrlichtCreationParameters& params, IFileSystem* io, HWND window)
{
