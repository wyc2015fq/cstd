...
{
  const SMaterial& m = material.org;
  u32 i;
  u32 enable;
  ...
  ShaderParam.SetRenderState(BD3DRS_ZENABLE, (material.org.ZBuffer == ECFN_NEVER) ? BD3DZB_FALSE : BD3DZB_USEW);
...
REALINLINE void CBurningShader_Raster_Reference::scanline2()
{
  ...
  pShader.xStart = ceil32(line.x[0]);
  pShader.xEnd = ceil32(line.x[1]) - 1;
  pShader.dx = pShader.xEnd - pShader.xStart;
  if (pShader.dx < 0) {
    return;
  }
  // slopes
  const f32 invDeltaX = reciprocal(line.x[1] - line.x[0]);
...
void CBurningShader_Raster_Reference::drawTriangle(const s4DVertex* a, const s4DVertex* b, const s4DVertex* c)
{
  ...
  scan.invDeltaY[0] = reciprocal(c->Pos.y - a->Pos.y);
  scan.invDeltaY[1] = reciprocal(b->Pos.y - a->Pos.y);
  scan.invDeltaY[2] = reciprocal(c->Pos.y - b->Pos.y);
