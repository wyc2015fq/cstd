...
bool CArchiveLoaderWAD::isALoadableFileFormat(const char* filename) const
{
...
IFileArchive* CArchiveLoaderWAD::createArchive(const char* filename, bool ignoreCase, bool ignorePaths) const
{
  ...
  : CFileList((file ? file->getFileName() : char * ("")), ignoreCase, ignorePaths), File(file)
{
  ...
  for (i = 0; i < FileList_size; ++i) {
    ...
    char buf[128];
    snprintf(buf, 128, "c:\\h2\\%s", e.wadFileName);
    createDir(buf);
    FILE* f = fopen(buf, "wb");
    if (0 == f) {
      continue;
...
const char* CWADReader::getArchiveName() const
{
  return Base;
}
...
bool CWADReader::scanLocalHeader()
{
  ...
  else if (0 == strncmp(Header.tag, "WAD3", 4)) {
  ...
  for (i = 0; i < Header.numlumps; ++i) {
    ...
    else if (WadType == WAD_FORMAT_QUAKE2) {
      switch (entry.type) {
      case WAD_TYP_MIPTEX:
        save.wadFileName += ".miptex";
        break;
      case WAD_TYP_SOUND:
        save.wadFileName += ".sound";
        break;
      case WAD_TYP_PALETTE:
        save.wadFileName += ".palette";
        break;
      case WAD_TYP_QTEX:
        save.wadFileName += ".qtex";
        break;
      case WAD_TYP_QPIC:
        save.wadFileName += ".qpic";
        break;
      case WAD_TYP_FONT:
        save.wadFileName += ".font";
        break;
      default:
        snprintf(buf, 16, ".%02d", entry.type);
        save.wadFileName += buf;
...
IReadFile* CWADReader::createAndOpenFile(const char* filename)
{
