...
extern "C" {
  void bz_internal_error1(int errorCode)
  {
    os::Printer::log("Error in bzip2 handling", char * (errorCode), ELL_ERROR);
  }
}
...
//#include "aesGladman/libaesGladman.inl"
#endif
...
bool CArchiveLoaderZIP::isALoadableFileFormat(const char* filename) const
{
  return hasFileExtension(filename, "zip", "pk3") ||
      hasFileExtension(filename, "gz", "tgz");
}
...
IFileArchive* CArchiveLoaderZIP::createArchive(const char* filename, bool ignoreCase, bool ignorePaths) const
{
...
CZipReader::CZipReader(IReadFile* file, bool ignoreCase, bool ignorePaths, bool isGZip)
  : CFileList((file ? file->getFileName() : char * ("")), ignoreCase, ignorePaths), File(file), IsGZip(isGZip)
{
#ifdef _DEBUG
  ...
  if (File->read(&header, sizeof(SGZIPMemberHeader)) == sizeof(SGZIPMemberHeader)) {
    ...
    if (header.flags & EGZF_FILE_NAME) {
      ...
      if (hasFileExtension(ZipFileName, "tgz")) {
        ZipFileName[ ZipFileName_size - 2] = 'a';
        ZipFileName[ ZipFileName_size - 1] = 'r';
      }
      else if (hasFileExtension(ZipFileName, "gz")) {
        ZipFileName[ ZipFileName_size - 3] = 0;
    ...
    entry.header.FilenameLength = ZipFileName_size;
...
bool CZipReader::scanZipHeader(bool ignoreGPBits)
{
  ...
  File->seek(entry.header.DataDescriptor.CompressedSize, true);
#ifdef _DEBUG
  //os::Debuginfo::print("added file from archive", ZipFileName);
#endif
  addItem(ZipFileName, entry.Offset, entry.header.DataDescriptor.UncompressedSize, ZipFileName.lastChar() == '/', FileInfo_size);
  FileInfo.push_back(entry);
  return true;
}
//! scans for a local header, returns false if there is no more local file header.
...
IReadFile* CZipReader::createAndOpenFile(const char* filename)
{
...
IReadFile* CZipReader::createAndOpenFile(u32 index)
{
  ...
  if ((e.header.GeneralBitFlag & ZIP_FILE_ENCRYPTED) && (e.header.CompressionMethod == 99)) {
    ...
    int rc = fcrypt_init(
        ...
        Password_size, // number of bytes in password
        salt, // the salt
    ...
    decrypted = createMemoryReadFile(decryptedBuf, decryptedSize, Files[index].FullName, true);
  ...
  case 8: {
    ...
    if (!pBuf) {
      snprintf(buf, 64, "Not enough memory for decompressing %s", Files[index].FullName);
    ...
    if (err != Z_OK) {
      snprintf(buf, 64, "Error decompressing %s", Files[index].FullName);
      os::Printer::log(buf, ELL_ERROR);
      delete [] pBuf;
      return 0;
    }
  ...
  case 99:
    ...
    return 0;
  default:
    snprintf(buf, 64, "file has unsupported compression method. %s", Files[index].FullName);
    os::Printer::log(buf, ELL_ERROR);
    return 0;
  };
}
