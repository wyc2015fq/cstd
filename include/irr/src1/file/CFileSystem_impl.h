...
{
  u32 i;
  for (i = 0; i < FileArchives_size; ++i) {
    FileArchives[i]->drop();
  }
  for (i = 0; i < ArchiveLoader_size; ++i) {
...
IReadFile* CFileSystem::createMemoryReadFile(void* memory, s32 len,
    const char* fileName, bool deleteMemoryWhenDropped)
{
  if (!memory) {
    return 0;
  }
...
IReadFile* CFileSystem::createLimitReadFile(const char* fileName,
...
IWriteFile* CFileSystem::createAndWriteFile(const char* filename, bool append)
{
...
bool CFileSystem::moveFileArchive(u32 sourceIndex, s32 relative)
{
  ...
  const s32 sourceEnd = ((s32) FileArchives_size) - 1;
  IFileArchive* t;
...
bool CFileSystem::addFileArchive(const char* filename, bool ignoreCase,
    ...
    IFileArchive** retArchive)
{
  ...
  if (archiveType == EFAT_UNKNOWN || archiveType == EFAT_FOLDER) {
    ...
    for (i = ArchiveLoader_size - 1; i >= 0 ; --i) {
...
// don't expose!
bool CFileSystem::changeArchivePassword(const char* filename,
    const char* password,
    IFileArchive** archive)
{
  for (s32 idx = 0; idx < (s32)FileArchives_size; ++idx) {
...
bool CFileSystem::addFileArchive(IReadFile* file, bool ignoreCase,
    bool ignorePaths, E_FILE_ARCHIVE_TYPE archiveType,
    const char* password, IFileArchive** retArchive)
{
...
//! removes an archive from the file system.
bool CFileSystem::removeFileArchive(const char* filename)
{
  const path absPath = getAbsolutePath(filename);
  int i;
  for (i = 0; i < FileArchives_size; ++i) {
...
#if defined(_IRR_WCHAR_FILESYSTEM)
    success = (_wchdir(newDirectory) == 0);
#else
...
char* CFileSystem::getAbsolutePath(const char* filename) const
{
#if defined(_IRR_WINDOWS_CE_PLATFORM_)
  return filename;
#elif defined(_IRR_WINDOWS_API_)
...
#if defined(_IRR_WCHAR_FILESYSTEM )
  p = _wfullpath(fpath, filename, _MAX_PATH);
  char* tmp(p);
  tmp.replace('\\', '/');
#else
  p = _fullpath(fpath, filename, _MAX_PATH);
  char* tmp(p);
  tmp.replace('\\', '/');
#endif
  return tmp;
...
//! is returned.
char* CFileSystem::getFileDir(const char* filename) const
{
  // find last forward or backslash
  s32 lastSlash = filename.findLast('/');
  const s32 lastBackSlash = filename.findLast('\\');
  lastSlash = lastSlash > lastBackSlash ? lastSlash : lastBackSlash;
  if ((u32)lastSlash < filename_size) {
...
char* CFileSystem::getFileBasename(const char* filename, bool keepExtension) const
{
  // find last forward or backslash
  s32 lastSlash = filename.findLast('/');
  const s32 lastBackSlash = filename.findLast('\\');
  ...
  if (!keepExtension) {
    ...
    if (end == -1 || end < lastSlash) {
      ...
      end = filename_size - end;
    }
  }
  if ((u32)lastSlash < filename_size) {
    return filename.subString(lastSlash + 1, filename_size - lastSlash - 1 - end);
  }
  else if (end != 0) {
    return filename.subString(0, filename_size - end);
  }
  else {
    return filename;
  }
...
char* CFileSystem::flattenFilename(char* directory, const char* root) const
{
  directory.replace('\\', '/');
...
path CFileSystem::getRelativeFilename(const char* filename, const char* directory) const
{
  ...
  if (prefix1_size > 1 && prefix1[1] == _IRR_TEXT(':')) {
    partition1 = locale_lower(prefix1[0]);
  }
  if (prefix2_size > 1 && prefix2[1] == _IRR_TEXT(':')) {
    partition2 = locale_lower(prefix2[0]);
  }
...
#endif
  for (; i < list1_size && i < list2_size
#if defined (_IRR_WINDOWS_API_)
      && (char * (*it1).make_lower() == char * (*it2).make_lower())
#else
      && (*it1 == *it2)
#endif
      ; ++i) {
...
IFileList* CFileSystem::createFileList()
{
  ...
  if (FileSystemType == FILESYSTEM_NATIVE) {
    ...
    //! create file list for the filesystem
    r = new CFileList(Path, false, false);
    ...
    for (i = 0; i < FileArchives_size; ++i) {
      ...
      for (int j = 0; j < merge->getFileCount(); ++j) {
...
IFileList* CFileSystem::createEmptyFileList(const char* path, bool ignoreCase, bool ignorePaths)
{
  return new CFileList(path, ignoreCase, ignorePaths);
}
//! determines if a file exists and would be able to be opened.
bool CFileSystem::existFile(const char* filename) const
{
  int i;
  for (i = 0; i < FileArchives_size; ++i)
...
#else
  HANDLE hFile = CreateFile(char * (filename), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
#endif
...
#elif defined(F_OK)
#if defined(_IRR_WCHAR_FILESYSTEM)
  return (_waccess(filename, F_OK) != -1);
#else
...
//! Creates a XML Reader from a file.
IXMLReaderUTF8* CFileSystem::createXMLReaderUTF8(const char* filename)
{
  IReadFile* file = createAndOpenFile(filename);
  if (!file) {
    return 0;
