...
public:
  ...
  See IReferenceCounted::drop() for more information.
  */
  IReadFile* createMemoryReadFile(void* memory, s32 len, const char* fileName, bool deleteMemoryWhenDropped = false) = 0;
  ...
  See IReferenceCounted::drop() for more information.
  */
  IReadFile* createLimitReadFile(const char* fileName, IReadFile* alreadyOpenedFile, long pos, long areaSize) = 0;
  ...
  See IReferenceCounted::drop() for more information.
  */
  IWriteFile* createMemoryWriteFile(void* memory, s32 len, const char* fileName, bool deleteMemoryWhenDropped = false) = 0;
  ...
  The returned pointer should be dropped when no longer needed.
  See IReferenceCounted::drop() for more information. */
  IWriteFile* createAndWriteFile(const char* filename, bool append = false) = 0;
  ...
  bool addFileArchive(const char* filename, bool ignoreCase = true,
  ...
  \return True if the archive was added successfully, false if not. */
  bool addFileArchive(IFileArchive* archive) = 0;
  //! Get the number of archives currently attached to the file system
  u32 getFileArchiveCount() = 0;
  ...
  bool removeFileArchive(u32 index) = 0;
  //! Removes an archive from the file system.
  /** This will close the archive and free any file handles, but will not
  close resources which have already been loaded and are now cached, for
  ...
  \return True on success, false on failure */
  bool removeFileArchive(const char* filename) = 0;
  //! Removes an archive from the file system.
  /** This will close the archive and free any file handles, but will not
  close resources which have already been loaded and are now cached, for
  example textures and meshes.
  ...
  \return True on success, false on failure */
  bool removeFileArchive(const IFileArchive* archive) = 0;
  ...
  bool moveFileArchive(u32 sourceIndex, s32 relative) = 0;
  //! Get the archive at a given index.
  IFileArchive* getFileArchive(u32 index) = 0;
  ...
  void addArchiveLoader(IArchiveLoader* loader) = 0;
  //! Gets the number of archive loaders currently added
  u32 getArchiveLoaderCount() = 0;
  //! Retrieve the given archive loader
  /** \param index The index of the loader to retrieve. This parameter is an 0-based
  array index.
  \return A pointer to the specified loader, 0 if the index is incorrect. */
  IArchiveLoader* getArchiveLoader(u32 index) = 0;
  ...
  without its complete path.
  \return True if the archive was added successfully, false if not. */
  _IRR_DEPRECATED_ bool addZipFileArchive(const c8* filename, bool ignoreCase = true, bool ignorePaths = true) {
  ...
  _IRR_DEPRECATED_ bool addFolderFileArchive(const c8* filename, bool ignoreCase = true, bool ignorePaths = true) {
  ...
  \return True if the archive was added successful, false if not. */
  _IRR_DEPRECATED_ bool addPakFileArchive(const c8* filename, bool ignoreCase = true, bool ignorePaths = true) {
  ...
  char* getAbsolutePath(const char* filename) = 0;
  //! Get the directory a file is located in.
  /** \param filename: The file to get the directory from.
  \return String containing the directory of the file. */
  char* getFileDir(const char* filename) = 0;
  //! Get the base part of a filename, i.e. the name without the directory part.
  /** If no directory is prefixed, the full name is returned.
  \param filename: The file to get the basename from
  \param keepExtension True if filename with extension is returned otherwise everything
  after the final '.' is removed as well. */
  char* getFileBasename(const char* filename, bool keepExtension = true) = 0;
  //! flatten a path and file name for example: "/you/me/../." becomes "/you"
  char*  flattenFilename(char*  directory, const char* root = "/") = 0;
  //! Get the relative filename, relative to the given directory
  char* getRelativeFilename(const char* filename, const char* directory) = 0;
  ...
  it has to be deleted using its IFileList::drop() method.
  See IReferenceCounted::drop() for more information. */
  IFileList* createEmptyFileList(const char* path, bool ignoreCase, bool ignorePaths) = 0;
  //! Set the active type of file system.
  EFileSystemType setFileListSystem(EFileSystemType listType) = 0;
  ...
  /** Use createXMLReaderUTF8() if you prefer char* instead of char*. See IIrrXMLReader for
  ...
  /** Use createXMLReader() if you prefer char* instead of char*. See IIrrXMLReader for
  more information on how to use the parser.
  \return 0, if file could not be opened, otherwise a pointer to the created
  IXMLReader is returned. After use, the reader
  ...
  See IReferenceCounted::drop() for more information. */
  IAttributes* createEmptyAttributes(IVideoDriver* driver = 0) = 0;
};
#endif
