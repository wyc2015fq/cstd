...
//! constructor
COCTLoader::COCTLoader(ISceneManager* smgr, IFileSystem* fs)
...
void COCTLoader::OCTLoadLights(IReadFile* file, ISceneNode* parent, f32 radius, f32 intensityScale, bool rewind)
{
  if (rewind) {
    file->seek(0);
  }
  ...
  for (i = 0; i < header.numLights; i++) {
    ...
    SceneManager->addLightSceneNode(parent, f3VECTOR(lights[i].pos[0], lights[i].pos[2], lights[i].pos[1]),
        SColorf(lights[i].color[0] * intensity, lights[i].color[1] * intensity, lights[i].color[2] * intensity, 1.0f),
        radius);
  }
}
...
IAnimatedMesh* COCTLoader::createMesh(IReadFile* file)
{
  if (!file) {
    return 0;
  }
  ...
  for (i = 0; i < header.numFaces; i++) {
    ...
    const float3 normal =
        plane3df(f3VECTOR(a[0], a[1], a[2]), f3VECTOR(b[0], c[1], c[2]), f3VECTOR(c[0], c[1], c[2])).Normal;
    const u32 textureID = min_(s32(faces[i].textureID), s32(header.numTextures - 1)) + 1;
    const u32 lightmapID = min_(s32(faces[i].lightmapID), s32(header.numLightmaps - 1)) + 1;
    SMeshBufferLightMap* meshBuffer = (SMeshBufferLightMap*)Mesh->getMeshBuffer(lightmapID * (header.numTextures + 1) + textureID);
    const u32 base = meshBuffer->Vertices_size;
    ...
    for (v = 0; v < faces[i].numVerts; ++v) {
      ...
      vert.Color = SColor(0, 255, 255, 255);
  ...
  tex.push_back(0);
  const char* relpath = FileSystem->getFileDir(file->getFileName()) + "/";
  for (i = 1; i < (header.numTextures + 1); i++) {
    char* path(textures[i - 1].fileName);
    path.replace('\\', '/');
  ...
  const dimension2du lmapsize(lightmapWidth, lightmapHeight);
  bool oldMipMapState = SceneManager->getVideoDriver()->getTextureCreationFlag(ETCF_CREATE_MIP_MAPS);
  SceneManager->getVideoDriver()->setTextureCreationFlag(ETCF_CREATE_MIP_MAPS, false);
  IImage* tmpImage = SceneManager->getVideoDriver()->createImage(ECF_R8G8B8, lmapsize);
  for (i = 1; i < (header.numLightmaps + 1); ++i) {
    char* lightmapname = file->getFileName();
    ...
    lig[i] = SceneManager->getVideoDriver()->addTexture(lightmapname, tmpImage);
  }
  tmpImage->drop();
  ...
  for (i = 0; i < header.numLightmaps + 1; i++) {
    for (int j = 0; j < header.numTextures + 1; j++) {
      ...
      if (meshBuffer->Material.getTexture(1) == 0) {
        ...
        meshBuffer->Material.MaterialType = EMT_SOLID;
...
bool COCTLoader::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "oct");
}
