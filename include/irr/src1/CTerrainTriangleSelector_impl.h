...
void CTerrainTriangleSelector::setTriangleData(ITerrainSceneNode* node, s32 LOD)
{
  ...
  const S3DVertex* vertices = static_cast<const S3DVertex*>(node->getRenderBuffer()->getVertices());
  // Clear current data
...
void CTerrainTriangleSelector::getTriangles(triangle3df* triangles,
    s32 arraySize, s32& outTriangleCount,
    const F3AABBOX& box, const matrix4* transform) const
{
  s32 count = TrianglePatches.TotalTriangles;
  if (count > arraySize) {
    count = arraySize;
...
void CTerrainTriangleSelector::getTriangles(triangle3df* triangles,
    s32 arraySize, s32& outTriangleCount, const F3LINE& line,
    const matrix4* transform) const
{
  const s32 count = min_((s32)TrianglePatches.TotalTriangles, arraySize);
  matrix4 mat;
  if (transform) {
    mat = (*transform);
  }
