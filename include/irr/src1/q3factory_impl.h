    ...
    bool showShaderName)
{
  ...
  for (u32 i = 0; i != additional_mesh->getMeshBufferCount(); ++i) {
    ...
    if (showShaderName && node) {
      snprintf((char*) buf, 64, "%hs:%d", node->getName(), node->getID());
      smgr->addBillboardTextSceneNode(
          font,
          (char*) buf,
          node,
          dimension2df(80.0f, 8.0f),
          f3VECTOR(0, 10, 0),
          sceneNodeID);
      sceneNodeID += 1;
    }
                   ...
                   )
{
  ...
  IEntity*& entity = masterMesh->getEntityList();
  ISceneManager* smgr = device->getSceneManager();
  char buf[128];
  ...
  */
  ...
  // walk list
  for (index = 0; (u32) index < entity_size; ++index) {
    ...
    for (u32 g = 0; g < 2; ++g) {
      ...
      for (u32 j = 0; j != mesh->Buffer_size; ++j) {
        ...
        if (0 == node) {
          snprintf(buf, 128, "q3ModelFactory shader %s failed", meshBuffer->Shader);
        ...
        if (itemElement->special & SPECIAL_SFX_ROTATE ||
          ...
          anim = smgr->createRotationAnimator(f3VECTOR(0.f,
              2.f, 0.f));
        ...
        if (itemElement->special & SPECIAL_SFX_BOUNCE) {
          ...
          //  p, p + float3 ( 0.f, 60.f, 0.f ), 1000, true, true );
          anim = smgr->createFlyCircleAnimator(
              p + f3VECTOR(0.f, 20.f, 0.f),
              20.f,
              0.005f,
              f3VECTOR(1.f, 0.f, 0.f),
              fract(nodeCount * 0.05f),
              1.f
              );
          node->addAnimator(anim);
          anim->drop();
    ...
    // show name
    if (showShaderName) {
      snprintf((char*) buf, sizeof(buf) / 2, "%hs", itemElement->key);
      smgr->addBillboardTextSceneNode(
          font,
          (char*) buf,
          parent,
                   ...
                   )
{
  ...
  parsepos = 0;
  float3 pos = getAsVector3df(group->get("origin"), parsepos);
  pos += translation;
  parsepos = 0;
  f32 angle = getAsFloat(group->get("angle"), parsepos);
  float3 target(0.f, 0.f, 1.f);
  target.rotateXZBy(angle - 90.f, f3VECTOR());
  if (camera) {
...
*/
float3 getGravity(const c8* surface)
{
  if (0 == strcmp(surface, "earth")) {
    return f3VECTOR(0.f, -90.f, 0.f);
  }
  if (0 == strcmp(surface, "moon")) {
    return f3VECTOR(0.f, -6.f / 100.f, 0.f);
  }
  if (0 == strcmp(surface, "water")) {
    return f3VECTOR(0.1f / 100.f, -2.f / 100.f, 0.f);
  }
  if (0 == strcmp(surface, "ice")) {
    return f3VECTOR(0.2f / 100.f, -9.f / 100.f, 0.3f / 100.f);
  }
  return f3VECTOR(0.f, 0.f, 0.f);
}
/*
...
void checkTimeFire(TimeFire* t, u32 listSize, u32 now)
{
  ...
  for (i = 0; i < listSize; ++i) {
    ...
    t[i].next = max_(now + t[i].delta, t[i].next + t[i].delta);
    t[i].flags |= FIRED;
  }
}
