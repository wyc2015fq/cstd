...
public:
  ...
  CV_INLINE CMatrix4<T>& operator=(const CMatrix4<T>& other);
  //! Sets all elements of this matrix to the value.
  CV_INLINE CMatrix4<T>& operator=(const T& scalar);
  ...
  /** Calculate b*a */
  CV_INLINE CMatrix4<T>& setbyproduct(const CMatrix4<T>& other_a, const CMatrix4<T>& other_b);
  //! Set this matrix to the product of two matrices
  ...
  CV_INLINE CMatrix4<T>& makeIdentity();
  //! Returns true if the matrix is the identity matrix
  CV_INLINE bool isIdentity() const;
  //! Returns true if the matrix is orthogonal
  CV_INLINE bool isOrthogonal() const;
  //! Returns true if the matrix is the identity matrix
  ...
  CV_INLINE CMatrix4<T>& setRotationRadians(const vector3d<T>& rotation);
  //! Make a rotation matrix from Euler angles. The 4th row and column are unmodified.
  ...
  CV_INLINE CMatrix4<T>& setInverseRotationRadians(const vector3d<T>& rotation);
  //! Make an inverted rotation matrix from Euler angles.
  /** The 4th row and column are unmodified. */
  CV_INLINE CMatrix4<T>& setInverseRotationDegrees(const vector3d<T>& rotation);
  //! Make a rotation matrix from angle and axis, assuming left handed rotation.
  /** The 4th row and column are unmodified. */
  CV_INLINE CMatrix4<T>& setRotationAxisRadians(const T& angle, const vector3d<T>& axis);
  //! Set Scale
  ...
  void rotateVect(float3 out, const float3 in) const;
  //! An alternate transform vector method, writing into an array of 3 floats
  void rotateVect(T* out, const float3 in) const;
  ...
  void transformVect(T* out, const float3 in) const;
  ...
  CMatrix4<T>& buildShadowMatrix(const float3 light, plane3df plane, f32 point = 1.0f);
  ...
  //! Gets transposed matrix
  CV_INLINE void getTransposed(CMatrix4<T>& dest) const;
  //! Builds a matrix that rotates from one vector to another
  /** \param from: vector to rotate from
  \param to: vector to rotate to
   */
  CMatrix4<T>& buildRotateFromTo(const float3 from, const float3 to);
  //! Builds a combined matrix which translates to a center before rotation and translates from origin afterwards
  /** \param center Position to rotate around
  \param translate Translation applied after the rotation
   */
  void setRotationCenter(const float3 center, const float3 translate);
   ...
   */
  void buildAxisAlignedBillboard(const float3 camPos,
      const float3 center,
...
// Default constructor
template <class T>
CV_INLINE CMatrix4<T>::CMatrix4(eConstructor constructor)
#if defined ( USE_MATRIX_TEST )
...
// Copy constructor
template <class T>
CV_INLINE CMatrix4<T>::CMatrix4(const CMatrix4<T>& other, eConstructor constructor)
#if defined ( USE_MATRIX_TEST )
  : definitelyIdentityMatrix(BIT_UNTESTED)
#endif
#if defined ( USE_MATRIX_TEST_DEBUG )
...
//! Add another matrix.
template <class T>
CV_INLINE CMatrix4<T> CMatrix4<T>::operator+(const CMatrix4<T>& other) const
{
...
//! Add another matrix.
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::operator+=(const CMatrix4<T>& other)
{
  M[0] += other[0];
  M[1] += other[1];
  M[2] += other[2];
...
//! Multiply by scalar.
template <class T>
CV_INLINE CMatrix4<T> CMatrix4<T>::operator*(const T& scalar) const
{
  CMatrix4<T> temp(EM4CONST_NOTHING);
  temp[0] = M[0] * scalar;
  temp[1] = M[1] * scalar;
...
//! Multiply by scalar.
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::operator*=(const T& scalar)
{
  M[0] *= scalar;
  M[1] *= scalar;
  M[2] *= scalar;
...
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setbyproduct_nocheck(const CMatrix4<T>& other_a, const CMatrix4<T>& other_b)
{
...
// goal is to reduce stack use and copy
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setbyproduct(const CMatrix4<T>& other_a, const CMatrix4<T>& other_b)
{
#if defined ( USE_MATRIX_TEST )
...
//! multiply by another matrix
template <class T>
CV_INLINE CMatrix4<T> CMatrix4<T>::operator*(const CMatrix4<T>& m2) const
{
#if defined ( USE_MATRIX_TEST )
...
template <class T>
CV_INLINE vector3d<T> CMatrix4<T>::getTranslation() const
{
  return vector3d<T>(M[12], M[13], M[14]);
}
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setTranslation(const vector3d<T>& translation)
{
  M[12] = translation.X;
  M[13] = translation.Y;
  M[14] = translation.Z;
...
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setInverseTranslation(const vector3d<T>& translation)
{
  M[12] = -translation.X;
  M[13] = -translation.Y;
  M[14] = -translation.Z;
...
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setScale(const vector3d<T>& scale)
{
  M[0] = scale.X;
  M[5] = scale.Y;
  M[10] = scale.Z;
...
*/
template <class T>
CV_INLINE vector3d<T> CMatrix4<T>::getScale() const
{
...
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setRotationDegrees(const vector3d<T>& rotation)
{
  return setRotationRadians(rotation * DEGTORAD);
}
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setInverseRotationDegrees(const vector3d<T>& rotation)
{
  return setInverseRotationRadians(rotation * DEGTORAD);
}
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setRotationRadians(const vector3d<T>& rotation)
{
...
template <class T>
CV_INLINE vector3d<T> CMatrix4<T>::getRotationDegrees() const
{
  ...
  const D3VECTOR invScale(reciprocal(scale.X), reciprocal(scale.Y), reciprocal(scale.Z));
...
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setInverseRotationRadians(const vector3d<T>& rotation)
{
...
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setRotationAxisRadians(const T& angle, const vector3d<T>& axis)
{
...
*/
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::makeIdentity()
{
  memset(M, 0, 16 * sizeof(T));
...
*/
template <class T>
CV_INLINE bool CMatrix4<T>::isIdentity() const
{
#if defined ( USE_MATRIX_TEST )
  ...
  if (!equals(M[12], (T)0) || !equals(M[13], (T)0) || !equals(M[14], (T)0) || !equals(M[15], (T)1)) {
    return false;
  }

  if (!equals(M[ 0], (T)1) || !equals(M[ 1], (T)0) || !equals(M[ 2], (T)0) || !equals(M[ 3], (T)0)) {
    return false;
  }

  if (!equals(M[ 4], (T)0) || !equals(M[ 5], (T)1) || !equals(M[ 6], (T)0) || !equals(M[ 7], (T)0)) {
    return false;
  }

  if (!equals(M[ 8], (T)0) || !equals(M[ 9], (T)0) || !equals(M[10], (T)1) || !equals(M[11], (T)0)) {
    return false;
  }
...
template <class T>
CV_INLINE bool CMatrix4<T>::isOrthogonal() const
{
...
*/
template <class T>
CV_INLINE bool CMatrix4<T>::isIdentity_integer_base() const
{
#if defined ( USE_MATRIX_TEST )
...
template <class T>
CV_INLINE void CMatrix4<T>::rotateVect(float3 vect) const
{
  float3 tmp = vect;
...
//! An alternate transform vector method, writing into a second vector
template <class T>
CV_INLINE void CMatrix4<T>::rotateVect(float3 out, const float3 in) const
{
...
//! An alternate transform vector method, writing into an array of 3 floats
template <class T>
CV_INLINE void CMatrix4<T>::rotateVect(T* out, const float3 in) const
{
...
template <class T>
CV_INLINE void CMatrix4<T>::inverseRotateVect(float3 vect) const
{
  float3 tmp = vect;
...
template <class T>
CV_INLINE void CMatrix4<T>::transformVect(float3 vect) const
{
  f32 vector[3];
...
template <class T>
CV_INLINE void CMatrix4<T>::transformVect(float3 out, const float3 in) const
{
...
template <class T>
CV_INLINE void CMatrix4<T>::transformVect(T* out, const float3 in) const
{
...
template <class T>
CV_INLINE void CMatrix4<T>::transformVec3(T* out, const T* in) const
{
...
//! Transforms a plane by this matrix
template <class T>
CV_INLINE void CMatrix4<T>::transformPlane(plane3d<f32>& plane) const
{
  float3 member;
...
//! Transforms a plane by this matrix
template <class T>
CV_INLINE void CMatrix4<T>::transformPlane(const plane3d<f32>& in, plane3d<f32>& out) const
{
  out = in;
  transformPlane(out);
}
//! Transforms a axis aligned bounding box
template <class T>
CV_INLINE void CMatrix4<T>::transformBox(F3AABBOX& box) const
{
#if defined ( USE_MATRIX_TEST )
...
template <class T>
CV_INLINE void CMatrix4<T>::transformBoxEx(F3AABBOX& box) const
{
#if defined ( USE_MATRIX_TEST )
...
//! Multiplies this matrix by a 1x4 matrix
template <class T>
CV_INLINE void CMatrix4<T>::multiplyWith1x4Matrix(T* matrix) const
{
  /*
  0  1  2  3
  4  5  6  7
...
template <class T>
CV_INLINE void CMatrix4<T>::inverseTranslateVect(float3 vect) const
{
  vect.X = vect.X - M[12];
  vect.Y = vect.Y - M[13];
  vect.Z = vect.Z - M[14];
...
template <class T>
CV_INLINE void CMatrix4<T>::translateVect(float3 vect) const
{
  vect.X = vect.X + M[12];
  vect.Y = vect.Y + M[13];
  vect.Z = vect.Z + M[14];
...
template <class T>
CV_INLINE bool CMatrix4<T>::getInverse(CMatrix4<T>& out) const
{
...
//! \param out: where result matrix is written to.
template <class T>
CV_INLINE bool CMatrix4<T>::getInversePrimitive(CMatrix4<T>& out) const
{
  out.M[0 ] = M[0];
  out.M[1 ] = M[4];
  out.M[2 ] = M[8];
...
*/
template <class T>
CV_INLINE bool CMatrix4<T>::makeInverse()
{
#if defined ( USE_MATRIX_TEST )
...
template <class T>
CV_INLINE bool CMatrix4<T>::operator==(const CMatrix4<T>& other) const
{
#if defined ( USE_MATRIX_TEST )
...
// Builds a right-handed perspective projection matrix based on a field of view
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixPerspectiveFovRH(
...
// Builds a left-handed perspective projection matrix based on a field of view
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixPerspectiveFovLH(
    f32 fieldOfViewRadians, f32 aspectRatio, f32 zNear, f32 zFar)
{
  const f64 h = reciprocal(tan(fieldOfViewRadians * 0.5));
  _IRR_DEBUG_BREAK_IF(aspectRatio == 0.f); //divide by zero
...
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixPerspectiveFovInfinityLH(
...
// Builds a left-handed orthogonal projection matrix.
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixOrthoLH(
...
// Builds a right-handed orthogonal projection matrix.
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixOrthoRH(
    f32 widthOfViewVolume, f32 heightOfViewVolume, f32 zNear, f32 zFar)
{
  _IRR_DEBUG_BREAK_IF(widthOfViewVolume == 0.f); //divide by zero
  _IRR_DEBUG_BREAK_IF(heightOfViewVolume == 0.f); //divide by zero
...
// Builds a right-handed perspective projection matrix.
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixPerspectiveRH(
    f32 widthOfViewVolume, f32 heightOfViewVolume, f32 zNear, f32 zFar)
{
  _IRR_DEBUG_BREAK_IF(widthOfViewVolume == 0.f); //divide by zero
  _IRR_DEBUG_BREAK_IF(heightOfViewVolume == 0.f); //divide by zero
...
// Builds a left-handed perspective projection matrix.
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixPerspectiveLH(
    f32 widthOfViewVolume, f32 heightOfViewVolume, f32 zNear, f32 zFar)
{
  _IRR_DEBUG_BREAK_IF(widthOfViewVolume == 0.f); //divide by zero
  _IRR_DEBUG_BREAK_IF(heightOfViewVolume == 0.f); //divide by zero
...
// Builds a matrix that flattens geometry into a plane.
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::buildShadowMatrix(const float3 light, plane3df plane, f32 point)
{
...
// Builds a left-handed look-at matrix.
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::buildCameraLookAtMatrixLH(
    const float3 position,
    const float3 target,
    const float3 upVector)
{
...
// Builds a right-handed look-at matrix.
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::buildCameraLookAtMatrixRH(
    const float3 position,
    const float3 target,
    const float3 upVector)
{
...
template <class T>
CV_INLINE CMatrix4<T> CMatrix4<T>::interpolate(const CMatrix4<T>& b, f32 time) const
{
...
template <class T>
CV_INLINE CMatrix4<T> CMatrix4<T>::getTransposed() const
{
...
// returns transposed matrix
template <class T>
CV_INLINE void CMatrix4<T>::getTransposed(CMatrix4<T>& o) const
{
  o[ 0] = M[ 0];
  o[ 1] = M[ 4];
  o[ 2] = M[ 8];
...
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::buildNDCToDCMatrix(const rect<s32>& viewport, f32 zScale)
{
 ...
 */
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::buildRotateFromTo(const float3 from, const float3 to)
{
  // unit vectors
  float3 f(from);
  float3 t(to);
 ...
 */
template <class T>
CV_INLINE void CMatrix4<T>::buildAxisAlignedBillboard(
    const float3 camPos,
    const float3 center,
    const float3 translation,
    const float3 axis,
    const float3 from)
{
  ...
  float3 up = axis;
  up.normalize();
  const float3 forward = (camPos - center).normalize();
  const float3 right = up.crossProduct(forward).normalize();
  // correct look vector
  const float3 look = right.crossProduct(up);
  // rotate from to
  // axis multiplication by sin
  const float3 vs = look.crossProduct(from);
  // cosinus angle
...
template <class T>
CV_INLINE void CMatrix4<T>::setRotationCenter(const float3 center, const float3 translation)
{
...
*/
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::buildTextureTransform(f32 rotateRad,
    const vector2df& rotatecenter,
    const vector2df& translate,
    const vector2df& scale)
{
...
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setTextureRotationCenter(f32 rotateRad)
{
  const f32 c = cosf(rotateRad);
  const f32 s = sinf(rotateRad);
  M[0] = (T)c;
...
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setTextureTranslate(f32 x, f32 y)
{
  M[8] = (T)x;
  M[9] = (T)y;
#if defined ( USE_MATRIX_TEST )
...
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setTextureTranslateTransposed(f32 x, f32 y)
{
  M[2] = (T)x;
  M[6] = (T)y;
#if defined ( USE_MATRIX_TEST )
...
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setTextureScale(f32 sx, f32 sy)
{
  M[0] = (T)sx;
  M[5] = (T)sy;
#if defined ( USE_MATRIX_TEST )
...
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setTextureScaleCenter(f32 sx, f32 sy)
{
  M[0] = (T)sx;
  M[5] = (T)sy;
  M[8] = (T)(0.5f - 0.5f * sx);
...
// sets all matrix data members at once
template <class T>
CV_INLINE CMatrix4<T>& CMatrix4<T>::setM(const T* data)
{
...
// sets if the matrix is definitely identity matrix
template <class T>
CV_INLINE void CMatrix4<T>::setDefinitelyIdentityMatrix(bool isDefinitelyIdentityMatrix)
{
#if defined ( USE_MATRIX_TEST )
...
// gets if the matrix is definitely identity matrix
template <class T>
CV_INLINE bool CMatrix4<T>::getDefinitelyIdentityMatrix() const
{
#if defined ( USE_MATRIX_TEST )
...
//! Compare two matrices using the equal method
template <class T>
CV_INLINE bool CMatrix4<T>::equals(const CMatrix4<T>& other, const T tolerance) const
{
#if defined ( USE_MATRIX_TEST )
...
// Multiply by scalar.
template <class T>
CV_INLINE CMatrix4<T> operator*(const T scalar, const CMatrix4<T>& mat)
{
  return mat * scalar;
}
