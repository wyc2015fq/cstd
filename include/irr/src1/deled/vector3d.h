...
public:
  ...
  bool operator<=(const vector3d<T>& other) const {
    return (X < other.X || equals(X, other.X, 0)) ||
  ...
  */
  ...
  const float3 toTarget(target->getAbsolutePosition() - seeker->getAbsolutePosition());
  const float3 requiredRotation = toTarget.getHorizontalAngle();
...
// Implementor note: CV_INLINE keyword needed due to template specialization for s32. Otherwise put specialization into a .cpp
template <>
CV_INLINE vector3d<s32> vector3d<s32>::operator /(s32 val) const
{
  return vector3d<s32>(X / val, Y / val, Z / val);
}
template <>
CV_INLINE vector3d<s32>& vector3d<s32>::operator /=(s32 val)
{
  X /= val;
  Y /= val;
  Z /= val;
...
template <>
CV_INLINE vector3d<s32> vector3d<s32>::getSphericalCoordinateAngles() const
{
  vector3d<s32> angle;
  const f64 length = X * X + Y * Y + Z * Z;
