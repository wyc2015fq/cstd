...
/** string<c8> and string<char> both accept Unicode AND ASCII/Latin-1,
so you can assign Unicode to string<c8> and ASCII/Latin-1 to string<char>
...
are simply expanded to the equivalent char, while Unicode/char
characters are truncated to 8-bit ASCII/Latin-1 characters, discarding all
other information in the char.
*/
enum eLocaleID {
  IRR_LOCALE_ANSI = 0,
  IRR_LOCALE_GERMAN = 1
...
static CV_INLINE void locale_set(eLocaleID id)
{
  locale_current = id;
}
//! Returns a character converted to lower case
static CV_INLINE u32 locale_lower(u32 x)
{
...
static CV_INLINE u32 locale_upper(u32 x)
{
  switch (locale_current) {
  case IRR_LOCALE_GERMAN:
  case IRR_LOCALE_ANSI:
...
public:
...
#endif
  int irr_dtoa(double number, char* tmpbuf, int len) {
    snprintf(tmpbuf, len, "%0.6f", number);
    return 0;
  }
  //! Constructs a string from an int
  int irr_itoa(int number, char* tmpbuf, int len) {
    ...
    strncpy(tmpbuf, &tmpbuf[idx], len);
    return 0;
  }
  //! Constructs a string from an unsigned int
  int irr_utoa(unsigned int number, char* tmpbuf, int len) {
    // temporary buffer for 16 numbers
    u32 idx = 15;
  ...
  int irr_ltoa(long number, char* tmpbuf, int len) {
    // store if negative and make positive
    bool negative = false;
  ...
  int irr_ultoa(unsigned long number, char* tmpbuf, int len) {
    // temporary buffer for 16 numbers
    u32 idx = 15;
  ...
  bool equals_ignore_case(const char* array, const char* other) {
    int i;
  ...
  s32 findLast(T c, s32 start = -1) const {
    start = clamp(start < 0 ? (s32)(used) - 2 : start, 0, (s32)(used) - 2);
  ...
  string<T>& replace(int toReplace, int replaceWith) {
    int i;
  ...
  string<T>& replace(const string<T>& toReplace, const string<T>& replaceWith) {
    ...
    const T* other = toReplace;
    const T* replace = replaceWith;
    const u32 other_size = toReplace_size;
    const u32 replace_size = replaceWith_size;
  ...
  string<T>& trim(const string<T>& whitespace = " \t\n\r") {
    ...
    const s32 begin = findFirstCharNotInList(whitespace, whitespace.used);

    if (begin == -1) {
      return (*this = "");
    }

    const s32 end = findLastCharNotInList(whitespace, whitespace.used);
