...
public:
  ...
  SColor operator+(const SColor& other) const {
    return SColor(min_(getAlpha() + other.getAlpha(), (u32)255u),
  ...
  SColor getInterpolated(const SColor& other, f32 d) const {
    ...
    return SColor((u32)round32(other.getAlpha() * inv + getAlpha() * d),
  ...
  SColor getInterpolated_quadratic(const SColor& c1, const SColor& c2, f32 d) const {
    ...
    const f32 mul2 = d * d;
    return SColor(
        clamp((s32)floor32(getAlpha() * mul0 + c1.getAlpha() * mul1 + c2.getAlpha() * mul2), (s32)0, (s32)255),
        clamp((s32)floor32(getRed()   * mul0 + c1.getRed()   * mul1 + c2.getRed()   * mul2), (s32)0, (s32)255),
        clamp((s32)floor32(getGreen() * mul0 + c1.getGreen() * mul1 + c2.getGreen() * mul2), (s32)0, (s32)255),
        clamp((s32)floor32(getBlue()  * mul0 + c1.getBlue()  * mul1 + c2.getBlue()  * mul2), (s32)0, (s32)255));
  }
...
public:
  ...
  SColor toSColor() const {
    return SColor((u32)round32(a * 255.0f), (u32)round32(r * 255.0f), (u32)round32(g * 255.0f), (u32)round32(b * 255.0f));
  }
  ...
  \param c2: second color to interpolate with
  \param d: value between 0.0f and 1.0f. */
  CV_INLINE SColorf getInterpolated_quadratic(const SColorf& c1, const SColorf& c2,
      f32 d) const {
    d = clamp(d, 0.f, 1.f);
    // this*(1-d)*(1-d) + 2 * c1 * (1-d) + c2 * d * d;
    const f32 inv = 1.f - d;
...
public:
  ...
  f32 Luminance;
private:
  CV_INLINE f32 toRGB1(f32 rm1, f32 rm2, f32 rh) const;
};
CV_INLINE void SColorHSL::fromRGB(const SColorf& color)
{
  const f32 maxVal = max_3(color.getRed(), color.getGreen(), color.getBlue());
  const f32 minVal = (f32)min_3(color.getRed(), color.getGreen(), color.getBlue());
  ...
  if (equals(maxVal, color.getRed())) {
    Hue = (color.getGreen() - color.getBlue()) / delta;
  }
  else if (equals(maxVal, color.getGreen())) {
...
CV_INLINE void SColorHSL::toRGB(SColorf& color) const
{
...
CV_INLINE f32 SColorHSL::toRGB1(f32 rm1, f32 rm2, f32 rh) const
{
  if (rh < 0) {
    rh += 1;
  }
