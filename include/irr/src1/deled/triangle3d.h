...
public:
  ...
  bool isPointInside(const vector3d<T>& p) const {
    D3VECTOR af64((f64)pointA.X, (f64)pointA.Y, (f64)pointA.Z);
    D3VECTOR bf64((f64)pointB.X, (f64)pointB.Y, (f64)pointB.Z);
    D3VECTOR cf64((f64)pointC.X, (f64)pointC.Y, (f64)pointC.Z);
    D3VECTOR pf64((f64)p.X, (f64)p.Y, (f64)p.Z);
  ...
  bool getIntersectionOfPlaneWithLine(const vector3d<T>& linePoint,
    ...
    const D3VECTOR linePointf64(linePoint.X, linePoint.Y, linePoint.Z);
    const D3VECTOR lineVectf64(lineVect.X, lineVect.Y, lineVect.Z);
    D3VECTOR outIntersectionf64;
    triangle3d<f64> trianglef64(D3VECTOR((f64)pointA.X, (f64)pointA.Y, (f64)pointA.Z)
        , D3VECTOR((f64)pointB.X, (f64)pointB.Y, (f64)pointB.Z)
        , D3VECTOR((f64)pointC.X, (f64)pointC.Y, (f64)pointC.Z));
    const D3VECTOR normalf64 = trianglef64.getNormal().normalize();
    f64 t2;
...
private:
  ...
  bool isOnSameSide(const D3VECTOR& p1, const D3VECTOR& p2,
      const D3VECTOR& a, const D3VECTOR& b) const {
    D3VECTOR bminusa = b - a;
    D3VECTOR cp1 = bminusa.crossProduct(p1 - a);
    D3VECTOR cp2 = bminusa.crossProduct(p2 - a);
    ...
    if (res < 0) {
      ...
      D3VECTOR cp1 = bminusa.normalize().crossProduct((p1 - a).normalize());

      if (iszero(cp1.X, (f64)ROUNDING_ERROR_f32)
          &&  iszero(cp1.Y, (f64)ROUNDING_ERROR_f32)
          &&  iszero(cp1.Z, (f64)ROUNDING_ERROR_f32)) {
        res = 0.f;
      }
    }
