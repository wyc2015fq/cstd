...
bool COpenGLShaderMaterialRenderer::checkError(const c8* type)
{
...
#endif
#else
  char* errString("Shaders not supported.");
#endif
  os::Printer::log(errString, ELL_ERROR);
  return true;
}
bool COpenGLShaderMaterialRenderer::createPixelShader(const c8* pxsh)
{
  ...
  for (i = 0; i < numShaders; ++i) {
    if (i == 0) {
      ...
      shdr += inshdr.subString(pos + 16, inshdr_size - pos - 16);
    }
