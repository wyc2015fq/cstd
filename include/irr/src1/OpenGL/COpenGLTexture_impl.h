...
COpenGLTexture::COpenGLTexture(IImage* origImage, const char* name, void* mipmapData, COpenGLDriver* driver)
  ...
  if (!image || IsRenderTarget) {
    ...
    if (!image) {
      if (mipmapLevel) {
        ...
        while (i != mipmapLevel);
        MipImage = image = Driver->createImage(ECF_A8R8G8B8, dimension2du(width, height));
      }
      else {
    ...
    if (mode != ETLM_WRITE_ONLY) {
      ...
      if (!mipmapLevel) {
...
#endif
        {
          ...
          int i;
          for (i = 0; i < image->getDimension().Height; i += 2) {
...
COpenGLFBOTexture::COpenGLFBOTexture(const dimension2du& size,
    const char* name, COpenGLDriver* driver,
