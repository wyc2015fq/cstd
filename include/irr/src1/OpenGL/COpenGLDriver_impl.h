...
COpenGLDriver::COpenGLDriver(const SIrrlichtCreationParameters& params,
    IFileSystem* io, CIrrDeviceWin32* device)
...
bool COpenGLDriver::initDriver(CIrrDeviceWin32* device)
{
  ...
  os::Printer::log("Pixel Format", char * (PixelFormat), ELL_DEBUG);
...
void COpenGLDriver::setTransform(E_TRANSFORMATION_STATE state, const matrix4& mat)
{
  ...
  case ETS_WORLD: {
    ...
    glLoadMatrixf((Matrices[ETS_VIEW]));
    ...
    glMultMatrixf(Matrices[ETS_WORLD]);
  }
  break;
  case ETS_PROJECTION: {
...
bool COpenGLDriver::updateVertexHardwareBuffer(SHWBufferLink_opengl* HWBuffer)
{
  ...
  if (!FeatureAvailable[IRR_ARB_vertex_array_bgra] && !FeatureAvailable[IRR_EXT_vertex_array_bgra]) {
    ...
    memcpy(buffer, vertices, vertexSize * vertexCount);
    vbuf = buffer;
...
u32 COpenGLDriver::getOcclusionQueryResult(ISceneNode* node) const
{
  const s32 index = OcclusionQueries.linear_search(SOccQuery(node));
  if (index != -1) {
...
static CV_INLINE u8* buffer_offset(const long offset)
{
  return ((u8*)0 + offset);
}
    ...
    E_VERTEX_TYPE vType, E_PRIMITIVE_TYPE pType, E_INDEX_TYPE iType)
{
  ...
  if (vertices) {
    if (FeatureAvailable[IRR_ARB_vertex_array_bgra] || FeatureAvailable[IRR_EXT_vertex_array_bgra]) {
      ...
      _IRR_DEBUG_BREAK_IF(ColorBuffer_size == 0);
  ...
  case EVT_2TCOORDS:
    if (vertices) {
      ...
      glColorPointer(colorSize, GL_UNSIGNED_BYTE, sizeof(S3DVertex), buffer_offset(24));
      glTexCoordPointer(2, GL_FLOAT, sizeof(S3DVertex), buffer_offset(28));
      glVertexPointer(3, GL_FLOAT, sizeof(S3DVertex), buffer_offset(0));
    }
    if (MultiTextureExtension) {
      extGlClientActiveTexture(GL_TEXTURE1_ARB);
      glEnableClientState(GL_TEXTURE_COORD_ARRAY);
      if (vertices) {
        glTexCoordPointer(2, GL_FLOAT, sizeof(S3DVertex), &(static_cast<const S3DVertex*>(vertices))[0].TCoords2);
      }
      else {
        glTexCoordPointer(2, GL_FLOAT, sizeof(S3DVertex), buffer_offset(36));
      }
    }
    break;
  case EVT_TANGENTS:
    ...
    if (MultiTextureExtension) {
      ...
      glEnableClientState(GL_TEXTURE_COORD_ARRAY);
      if (vertices) {
        glTexCoordPointer(3, GL_FLOAT, sizeof(S3DVertex), &(static_cast<const S3DVertex*>(vertices))[0].Tangent);
      }
      else {
        glTexCoordPointer(3, GL_FLOAT, sizeof(S3DVertex), buffer_offset(36));
      }
      extGlClientActiveTexture(GL_TEXTURE2_ARB);
      glEnableClientState(GL_TEXTURE_COORD_ARRAY);
      if (vertices) {
        glTexCoordPointer(3, GL_FLOAT, sizeof(S3DVertex), &(static_cast<const S3DVertex*>(vertices))[0].Binormal);
      }
      else {
        glTexCoordPointer(3, GL_FLOAT, sizeof(S3DVertex), buffer_offset(48));
      }
    }
    break;
  }
    ...
    E_PRIMITIVE_TYPE pType, E_INDEX_TYPE iType)
{
  ...
  case EPT_POINT_SPRITES: {
    ...
    if (pType == EPT_POINT_SPRITES && FeatureAvailable[IRR_ARB_point_sprite]) {
    ...
    //      else
    particleSize = clamp(particleSize, DimAliasedPoint[0], DimAliasedPoint[1]);
    ...
    E_VERTEX_TYPE vType, E_PRIMITIVE_TYPE pType, E_INDEX_TYPE iType)
{
  ...
  case EVT_2TCOORDS:
    if (vertices) {
      ...
      glColorPointer(colorSize, GL_UNSIGNED_BYTE, sizeof(S3DVertex), buffer_offset(24));
      glTexCoordPointer(2, GL_FLOAT, sizeof(S3DVertex), buffer_offset(28));
      glVertexPointer(2, GL_FLOAT, sizeof(S3DVertex), buffer_offset(0));
    }
    if (MultiTextureExtension) {
      extGlClientActiveTexture(GL_TEXTURE1_ARB);
      glEnableClientState(GL_TEXTURE_COORD_ARRAY);
...
void COpenGLDriver::draw2DImage(const ITexture* texture, const IRECT& destRect,
    ...
    bool useAlphaChannelOfTexture)
{
  ...
  for (i = 0; i < indices_size; ++i) {
    ...
    const IRECT poss(targetPos, sourceRects[currentIndex].getSize());
    glBegin(GL_QUADS);
    glTexCoord2f(tcoords.UpperLeftCorner.X, tcoords.UpperLeftCorner.Y);
    glVertex2f(GLfloat(poss.UpperLeftCorner.X), GLfloat(poss.UpperLeftCorner.Y));
    glTexCoord2f(tcoords.LowerRightCorner.X, tcoords.UpperLeftCorner.Y);
...
//! draw a 2d rectangle
void COpenGLDriver::draw2DRectangle(SColor color, const IRECT& position,
    const IRECT* clip)
{
  disableTextures();
...
void COpenGLDriver::draw2DRectangle(const IRECT& position,
...
//! Draws a 2d line.
void COpenGLDriver::draw2DLine(const position2di& start,
    const position2di& end, SColor color)
{
  if (start == end) {
...
bool COpenGLDriver::setActiveTexture(u32 stage, const ITexture* texture)
{
...
bool COpenGLDriver::disableTextures(u32 fromStage)
{
  bool result = true;
  int i;
  for (i = fromStage; i < MaxSupportedTextures; ++i) {
    result &= setActiveTexture(i, 0);
  }
  return result;
}
//! creates a matrix in supplied GLfloat array to pass to OpenGL
CV_INLINE void COpenGLDriver::getGLMatrix(GLfloat gl_matrix[16], const matrix4& m)
{
  memcpy(gl_matrix, m, 16 * sizeof(f32));
}
//! creates a opengltexturematrix from a D3D style texture matrix
CV_INLINE void COpenGLDriver::getGLTextureMatrix(GLfloat* o, const matrix4& m)
{
  o[0] = m[0];
  o[1] = m[1];
  o[2] = 0.f;
...
ITexture* COpenGLDriver::createDeviceDependentTexture(IImage* surface, const char* name, void* mipmapData)
{
...
bool COpenGLDriver::testGLError()
{
  ...
  case GL_STACK_OVERFLOW:
    os::Printer::log("GL_STACK_OVERFLOW", ELL_ERROR);
    break;
  case GL_STACK_UNDERFLOW:
    os::Printer::log("GL_STACK_UNDERFLOW", ELL_ERROR);
    break;
  case GL_OUT_OF_MEMORY:
    os::Printer::log("GL_OUT_OF_MEMORY", ELL_ERROR);
    break;
  case GL_TABLE_TOO_LARGE:
    os::Printer::log("GL_TABLE_TOO_LARGE", ELL_ERROR);
    break;
#if defined(GL_EXT_framebuffer_object)
  case GL_INVALID_FRAMEBUFFER_OPERATION_EXT:
    os::Printer::log("GL_INVALID_FRAMEBUFFER_OPERATION", ELL_ERROR);
    break;
#endif
  };
...
void COpenGLDriver::setRenderStates3DMode()
{
  if (CurrentRenderMode != ERM_3D) {
    ...
    glMatrixMode(GL_MODELVIEW);
    glLoadMatrixf((Matrices[ETS_VIEW] * Matrices[ETS_WORLD]));
    glMatrixMode(GL_PROJECTION);
    glLoadMatrixf(Matrices[ETS_PROJECTION]);
    ResetRenderStates = true;
  ...
  if (ResetRenderStates || LastMaterial != Material) {
    ...
    if (LastMaterial.MaterialType != Material.MaterialType &&
        static_cast<u32>(LastMaterial.MaterialType) < MaterialRenderers_size) {
      MaterialRenderers[LastMaterial.MaterialType].Renderer->OnUnsetMaterial();
    }
    // set new material.
    if (static_cast<u32>(Material.MaterialType) < MaterialRenderers_size)
    ...
    bool resetAllRenderStates)
{
  ...
  for (i = 0; i < MaxTextureUnits; ++i) {
    ...
    if (FeatureAvailable[IRR_EXT_texture_lod_bias]) {
      if (material.TextureLayer[i].LODBias) {
        const float tmp = clamp(material.TextureLayer[i].LODBias * 0.125f, -MaxTextureLODBias, MaxTextureLODBias);
    ...
    if (FeatureAvailable[IRR_EXT_texture_filter_anisotropic])
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT,
          material.TextureLayer[i].AnisotropicFilter > 1 ? min_(MaxAnisotropy, material.TextureLayer[i].AnisotropicFilter) : 1);
#endif
  }
  // fillmode
  ...
  if (resetAllRenderStates || lastmaterial.Thickness != material.Thickness) {
    if (AntiAlias) {
      ...
      glPointSize(clamp(static_cast<GLfloat>(material.Thickness), DimAliasedPoint[0], DimAliasedPoint[1]));
      glLineWidth(clamp(static_cast<GLfloat>(material.Thickness), DimSmoothedLine[0], DimSmoothedLine[1]));
    }
    else {
      glPointSize(clamp(static_cast<GLfloat>(material.Thickness), DimAliasedPoint[0], DimAliasedPoint[1]));
      glLineWidth(clamp(static_cast<GLfloat>(material.Thickness), DimAliasedLine[0], DimAliasedLine[1]));
    }
  }
  // Anti aliasing
...
s32 COpenGLDriver::addDynamicLight(const SLight& light)
{
  ...
  u32 newLightIndex = RequestedLights_size - 1;
...
{
  if (lightIndex < 0 || lightIndex >= (s32)RequestedLights_size) {
    return;
  }
  ...
  if (turnOn) {
    ...
    if (-1 != requestedLight.HardwareLightIndex) {
      ...
      for (int requested = 0; requested < RequestedLights_size; ++requested)
...
// method just a bit.
void COpenGLDriver::setViewPort(const IRECT& area)
{
  if (area == ViewPort) {
    return;
  }
...
void COpenGLDriver::drawStencilShadowVolume(const float3 > & triangles, bool zfail, u32 debugDataVisible)
{
  ...
  glEnableClientState(GL_VERTEX_ARRAY);
  glVertexPointer(3, GL_FLOAT, sizeof(float3), triangles);
  glStencilMask(~0);
...
void COpenGLDriver::drawStencilShadow(bool clearStencilBuffer, SColor leftUpEdge,
...
//! Draws a 3d line.
void COpenGLDriver::draw3DLine(const float3 start,
...
void COpenGLDriver::OnResize(const dimension2du& size)
{
...
ITexture* COpenGLDriver::addRenderTargetTexture(const dimension2du& size,
    const char* name,
    const ECOLOR_FORMAT format)
{
...
#endif
  {
    ...
    dimension2du destSize(min_(size.Width, ScreenSize.Width), min_(size.Height, ScreenSize.Height));
    ...
    bool clearZBuffer, SColor color)
{
  ...
  if (CurrentTarget == ERT_MULTI_RENDER_TEXTURES) {
    int i;
    for (i = 0; i < MRTargets_size; ++i) {
      if (MRTargets[i].TargetType == ERT_RENDER_TEXTURE) {
        for (++i; i < MRTargets_size; ++i)
          if (MRTargets[i].TargetType == ERT_RENDER_TEXTURE) {
...
bool COpenGLDriver::setRenderTarget(const IRenderTarget* targets,
    bool clearBackBuffer, bool clearZBuffer, SColor color)
{
  ...
  if ((MRTargets_size != 0) && (targets != MRTargets)) {
  ...
  MRTargets = targets;
  u32 maxMultipleRTTs = min_(static_cast<u32>(MaxMultipleRenderTargets), targets_size);
...
IImage* COpenGLDriver::createScreenShot(ECOLOR_FORMAT format, E_RENDER_TARGET target)
{
  ...
  if (FeatureAvailable[IRR_MESA_pack_invert]) {
    ...
    if (pixels) {
      ...
      int i;
      for (i = 0; i < ScreenSize.Height; i += 2) {
...
ITexture* COpenGLDriver::createDepthTexture(ITexture* texture, bool shared)
{
  ...
  if (shared) {
    int i;
    for (i = 0; i < DepthTextures_size; ++i) {
...
bool COpenGLDriver::setClipPlane(u32 index, const plane3df& plane, bool enable)
{
