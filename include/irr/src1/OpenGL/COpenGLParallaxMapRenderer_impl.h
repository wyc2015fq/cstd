...
const char OPENGL_PARALLAX_MAP_PSH[] =
    ...
    "END\n";
//! Constructor
COpenGLParallaxMapRenderer::COpenGLParallaxMapRenderer(COpenGLDriver* driver,
  ...
  : COpenGLShaderMaterialRenderer(driver, 0, baseMaterial), CompiledShaders(true)
{
  ...
  if (renderer) {
    ...
    COpenGLParallaxMapRenderer* nmr = reinterpret_cast<COpenGLParallaxMapRenderer*>(renderer);
...
void COpenGLParallaxMapRenderer::OnSetMaterial(const SMaterial& material,
...
void COpenGLParallaxMapRenderer::OnSetConstants(IMaterialRendererServices* services, s32 userData)
{
  ...
  services->setVertexShaderConstant(tWorld, 0, 4);
  ...
  services->setVertexShaderConstant(tr, 8, 4);
