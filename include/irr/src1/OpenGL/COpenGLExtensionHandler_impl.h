...
{
  int i;
  for (i = 0; i < IRR_OpenGL_Feature_Count; ++i) {
...
#define tmplog(x,y) os::Printer::log(x, char*(y))
...
void COpenGLExtensionHandler::initExtensions(bool stencilBuffer)
{
  ...
  Version = static_cast<u16>(floor32(ogl_ver) * 100 + round32(fract(ogl_ver) * 10.0f));
  if (Version >= 102) {
...
#define IRR_OGL_LOAD_EXTENSION(X) glXGetProcAddressARB(reinterpret_cast<const GLubyte*>(X))
...
#endif
    MaxSupportedTextures = max_(MaxSupportedTextures, static_cast<u8>(num));
  }
#endif
  ...
  if (FeatureAvailable[IRR_ARB_shading_language_100] || Version >= 200) {
    ...
    if (glGetError() == GL_INVALID_ENUM) {
      ...
      ShaderLanguageVersion = static_cast<u16>(floor32(sl_ver) * 100 + round32(fract(sl_ver) * 10.0f));
    }
  }
#endif
#ifdef _IRR_OPENGL_USE_EXTPOINTER_
...
#endif
    MaxTextureUnits = min_(MaxSupportedTextures, static_cast<u8>(MATERIAL_MAX_TEXTURES));
  ...
  if (FeatureAvailable[IRR_NVX_gpu_memory_info]) {
    ...
    os::Printer::log("Dedicated video memory (kB)", char * (val));
    glGetIntegerv(0x9048, &val);
    os::Printer::log("Total video memory (kB)", char * (val));
    glGetIntegerv(0x9049, &val);
    os::Printer::log("Available video memory (kB)", char * (val));
  }
#ifdef GL_ATI_meminfo
  if (FeatureAvailable[IRR_ATI_meminfo]) {
    GLint val[4];
    glGetIntegerv(GL_TEXTURE_FREE_MEMORY_ATI, val);
    os::Printer::log("Free texture memory (kB)", char * (val[0]));
    glGetIntegerv(GL_VBO_FREE_MEMORY_ATI, val);
    os::Printer::log("Free VBO memory (kB)", char * (val[0]));
    glGetIntegerv(GL_RENDERBUFFER_FREE_MEMORY_ATI, val);
    os::Printer::log("Free render buffer memory (kB)", char * (val[0]));
  }
#endif
#endif
}
