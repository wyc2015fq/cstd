...
struct STextureAtlas {
  ...
  void release();
  void addSource(const c8* name, IImage* image);
  void create(u32 pixelborder, E_TEXTURE_CLAMP texmode);
  void getScale(vector2df& scale);
  void getTranslation(const c8* name, vector2di& pos);
...
struct KeyFrameInterpolation {
  char* Name;   // Name of the current Animation/Bone
...
typedef  KeyFrameInterpolation > IAnimationList;
//! a List holding named Skins
typedef  char* > ISkinList;
...
public:
  ...
  bool loadModelFile(IReadFile* file, ISceneManager* smgr);
  //IAnimatedMesh
  u32 getFrameCount() const;
  IMesh* getMesh(s32 frame, s32 detailLevel, s32 startFrameLoop, s32 endFrameLoop);
  const F3AABBOX& getBoundingBox() const;
  E_ANIMATED_MESH_TYPE getMeshType() const;
  void renderModel(u32 param, IVideoDriver* driver, const matrix4& absoluteTransformation);
...
private:
  ...
  void initData();
  SHalflifeHeader* loadModel(IReadFile* file, const char* filename);
  bool postLoadModel(const char* filename);
...
#define MOUTH_CONTROLLER  4
  ...
  void ExtractBbox(s32 sequence, aabbox3df& box) const;
  void setUpBones();
...
public:
  //! Constructor
  CHalflifeMDLMeshFileLoader(ISceneManager* smgr);
