...
public:
  ...
  */
  ...
  void OnRenderPassPostRender(E_SCENE_NODE_RENDER_PASS renderPass) = 0;
  //! Called before the given scene node is rendered
  /** \param[in] node: the scene node that's about to be rendered */
  void OnNodePreRender(ISceneNode* node) = 0;
  ...
  void OnNodePostRender(ISceneNode* node) = 0;
};
#endif
