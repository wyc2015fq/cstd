...
CParticleSystemSceneNode::CParticleSystemSceneNode(bool createDefaultEmitter,
  ...
  : IParticleSystemSceneNode(parent, mgr, id, position, rotation, scale),
    Emitter(0), ParticleSize(dimension2df(5.0f, 5.0f)), LastEmitTime(0),
...
const list<IParticleAffector*>& CParticleSystemSceneNode::getAffectors() const
{
  return AffectorList;
}
...
SMaterial& CParticleSystemSceneNode::getMaterial(u32 i)
{
...
CParticleSystemSceneNode::createAnimatedMeshSceneNodeEmitter(
    ...
    const float3 direction, f32 normalDirectionModifier,
...
IParticleBoxEmitter* CParticleSystemSceneNode::createBoxEmitter(
    const aabbox3df& box, const float3 direction,
    u32 minParticlesPerSecond, u32 maxParticlesPerSecond,
    const SColor& minStartColor, const SColor& maxStartColor,
    u32 lifeTimeMin, u32 lifeTimeMax,
    s32 maxAngleDegrees, const dimension2df& minStartSize,
    const dimension2df& maxStartSize)
{
...
IParticleCylinderEmitter* CParticleSystemSceneNode::createCylinderEmitter(
    ...
    bool outlineOnly, const float3 direction,
    u32 minParticlesPerSecond, u32 maxParticlesPerSecond,
    const SColor& minStartColor, const SColor& maxStartColor,
    u32 lifeTimeMin, u32 lifeTimeMax, s32 maxAngleDegrees,
    const dimension2df& minStartSize,
...
IParticleSphereEmitter* CParticleSystemSceneNode::createSphereEmitter(
    const float3 center, f32 radius, const float3 direction,
    u32 minParticlesPerSecond, u32 maxParticlesPerSecond,
    const SColor& minStartColor, const SColor& maxStartColor,
    u32 lifeTimeMin, u32 lifeTimeMax,
    s32 maxAngleDegrees, const dimension2df& minStartSize,
...
IParticleFadeOutAffector* CParticleSystemSceneNode::createFadeOutParticleAffector(
    const SColor& targetColor, u32 timeNeededToFadeOut)
{
...
IParticleGravityAffector* CParticleSystemSceneNode::createGravityAffector(
    const float3 gravity, u32 timeForceLost)
{
...
IParticleRotationAffector* CParticleSystemSceneNode::createRotationAffector(
    const float3 speed, const float3 pivotPoint)
{
...
void CParticleSystemSceneNode::render()
{
  ...
  float3 view(camera->getTarget() - camera->getAbsolutePosition());
  view.normalize();
  view *= -1.0f;
#else
  const matrix4& m = camera->getViewFrustum()->getTransform(ETS_VIEW);
  const float3 view(-m[2], -m[6] , -m[10]);
#endif
  ...
  for (i = 0; i < Particles_size; ++i) {
...
#if 0
    float3 horizontal = camera->getUpVector().crossProduct(view);
    ...
    const float3 horizontal(m[0] * f, m[4] * f, m[8] * f);
    f = -0.5f * particle.size.Height;
    const float3 vertical(m[1] * f, m[5] * f, m[9] * f);
#endif
  ...
  driver->drawVertexPrimitiveList(Buffer->getVertices(), Particles_size * 4,
      Buffer->getIndices(), Particles_size * 2, EVT_STANDARD, EPT_TRIANGLES, Buffer->getIndexType());
  ...
  if (DebugDataVisible & EDS_BBOX) {
    ...
    driver->draw3DBox(Buffer->BoundingBox, SColor(0, 255, 255, 255));
  }
}
//! returns the axis aligned bounding box of this node
const F3AABBOX& CParticleSystemSceneNode::getBoundingBox() const
{
...
void CParticleSystemSceneNode::doParticleSystem(u32 time)
{
  ...
  for (; ait != AffectorList.end(); ++ait) {
    (*ait)->affect(now, Particles, Particles_size);
  }
  if (ParticlesAreGlobal) {
    Buffer->BoundingBox.reset(AbsoluteTransformation.getTranslation());
  }
  else {
    Buffer->BoundingBox.reset(float3(0, 0, 0));
  }
  // animate all particles
  f32 scale = (f32)timediff;
  int i;
  for (i = 0; i < Particles_size;) {
    ...
    if (now > Particles[i].endTime) {
      ...
      Particles[i] = Particles[Particles_size - 1];
      Particles.erase(Particles_size - 1);
    }
    else {
...
void CParticleSystemSceneNode::setParticleSize(const dimension2df& size)
{
  os::Printer::log("setParticleSize is deprecated, use setMinStartSize/setMaxStartSize in emitter.", ELL_WARNING);
...
{
  if (Particles_size * 4 > Buffer->getVertexCount() ||
      Particles_size * 6 > Buffer->getIndexCount()) {
    u32 oldSize = Buffer->getVertexCount();
    Buffer->Vertices.set_used(Particles_size * 4);
    u32 i;
    // fill remaining vertices
    for (i = oldSize; i < Buffer->Vertices_size; i += 4) {
    ...
    Buffer->Indices.set_used(Particles_size * 6);
    for (i = oldIdxSize; i < Buffer->Indices_size; i += 6) {
...
void CParticleSystemSceneNode::serializeAttributes(IAttributes* out, SAttributeReadWriteOptions* options) const
{
  ...
  for (list<IParticleAffector*>::ConstIterator it = AffectorList.begin();
...
void CParticleSystemSceneNode::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options)
{
  ...
  case EPET_BOX:
    ...
    break;
  case EPET_CYLINDER:
    Emitter = createCylinderEmitter(float3(0, 0, 0), 10.f, float3(0, 1, 0), 10.f); // (values here don't matter)
    break;
  case EPET_MESH:
    Emitter = createMeshEmitter(NULL);  // we can't set the mesh - the user will have to do this
    break;
  case EPET_RING:
    Emitter = createRingEmitter(float3(0, 0, 0), 10.f, 10.f); // (values here don't matter)
    break;
  case EPET_SPHERE:
    Emitter = createSphereEmitter(float3(0, 0, 0), 10.f); // (values here don't matter)
    break;
  default:
    break;
  }
  ...
  while (idx < cnt) {
    ...
    switch (atype) {
    case EPAT_ATTRACT:
      aff = createAttractionAffector(float3(0, 0, 0));
      break;
    case EPAT_FADE_OUT:
