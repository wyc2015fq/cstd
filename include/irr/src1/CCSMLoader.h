...
// - Replaced all dependencies to STL and stdio with  methods/constructs.
// - Moved everything into namespace scene
...
//  * Presents the loaded file as IAnimatedMesh for easy creation of IOctreeSceneNode
...
class CSMFile;
class ISceneManager;
class CCSMLoader : public IMeshLoader
{
public:
  CCSMLoader(ISceneManager* manager, IFileSystem* fs);
...
private:
  IMesh* createCSMMesh(IReadFile* file);
  IMesh* createIrrlichtMesh(const CSMFile* csmFile,
