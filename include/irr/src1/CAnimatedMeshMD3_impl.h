...
void CAnimatedMeshMD3::setBoundingBox(const aabbox3df& box)
{
...
IMesh* CAnimatedMeshMD3::getMesh(s32 frame, s32 detailLevel, s32 startFrameLoop, s32 endFrameLoop)
{
  ...
  startFrameLoop = s32_max(0, startFrameLoop >> IPolShift);
  ...
  if (LoopMode) {
    ...
    frame -= mask >> 1;
    // interpolation
    iPol = f32(frame & (mask - 1)) * reciprocal(f32(mask));
    // wrap anim
    frame >>= IPolShift;
    ...
    // clamp anim
    frame >>= IPolShift;
    frameA = s32_clamp(frame, startFrameLoop, endFrameLoop);
    frameB = s32_min(frameA + 1, endFrameLoop);
  }
  // build current vertex
  int i;
  for (i = 0; i != Mesh->Buffer_size; ++i) {
    ...
    IFileSystem* fs, IVideoDriver* driver)
{
  ...
  dest->Indices.set_used(source->Indices_size);
  u32 i;
  // fill in static face info
  for (i = 0; i < source->Indices_size; i += 3) {
    ...
    SMeshBufferLightMap* dest)
{
  ...
  for (s32 i = 0; i != source->MeshHeader.numVertices; ++i) {
    ...
    // normal
    const float3 nA(quake3::getMD3Normal(vA.normal[0], vA.normal[1]));
    const float3 nB(quake3::getMD3Normal(vB.normal[0], vB.normal[1]));
...
/*!
  loads a model
*/
bool CAnimatedMeshMD3::loadModelFile(u32 modelIndex, IReadFile* file,
    IFileSystem* fs, IVideoDriver* driver)
{
  if (!file) {
    return false;
  ...
  for (i = 0; i != (u32)Mesh->MD3Header.numMeshes; ++i) {
    ...
    file->read(buf->Tex, buf->MeshHeader.numVertices * sizeof(SMD3TexCoord));
    //! read vertices
    file->seek(offset + meshHeader.vertexStart);
    file->read(buf->Vertices, Mesh->MD3Header.numFrames * meshHeader.numVertices * sizeof(SMD3Vertex));
    //! read indices
    file->seek(offset + meshHeader.offset_triangles);
    file->read(buf->Indices, meshHeader.numTriangles * sizeof(SMD3Face));
    //! store meshBuffer
...
const F3AABBOX& CAnimatedMeshMD3::getBoundingBox() const
{
