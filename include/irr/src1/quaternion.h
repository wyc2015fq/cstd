...
public:
  ...
  bool operator!=(const quaternion& other) const;
  //! Assignment operator
  CV_INLINE quaternion& operator=(const quaternion& other);
#if !IRR_TEST_BROKEN_QUATERNION_USE
  //! Matrix assignment operator
  CV_INLINE quaternion& operator=(const matrix4& other);
#endif
  //! Add operator
  ...
  CV_INLINE f32 dotProduct(const quaternion& other) const;
  //! Sets new quaternion
  CV_INLINE quaternion& set(f32 x, f32 y, f32 z, f32 w);
  //! Sets new quaternion based on euler angles (radians)
  CV_INLINE quaternion& set(f32 x, f32 y, f32 z);
  //! Sets new quaternion based on euler angles (radians)
  CV_INLINE quaternion& set(const float3 vec);
  //! Sets new quaternion from other quaternion
  CV_INLINE quaternion& set(const quaternion& quat);
  //! returns if this quaternion equals the other one, taking floating point rounding errors into account
  CV_INLINE bool equals(const quaternion& other,
      const f32 tolerance = ROUNDING_ERROR_f32) const;
  //! Normalizes the quaternion
  CV_INLINE quaternion& normalize();
#if !IRR_TEST_BROKEN_QUATERNION_USE
  //! Creates a matrix from this quaternion
  matrix4 getMatrix() const;
#endif
  //! Creates a matrix from this quaternion
  void getMatrix(matrix4& dest, const float3 translation = f3VECTOR()) const;
  /*!
    Creates a matrix from this quaternion
    ...
    lookat *= m3;
  */
  void getMatrixCenter(matrix4& dest, const float3 center, const float3 translation) const;
  //! Creates a matrix from this quaternion
  CV_INLINE void getMatrix_transposed(matrix4& dest) const;
  ...
  */
  ...
  quaternion& fromAngleAxis(f32 angle, const float3 axis);
  //! Fills an angle (radians) around an axis (unit vector)
  void toAngleAxis(f32& angle, float3 axis) const;
  //! Output this quaternion to an euler angle (radians)
  void toEuler(float3 euler) const;
...
CV_INLINE quaternion::quaternion(f32 x, f32 y, f32 z)
{
  set(x, y, z);
}
// Constructor which converts euler angles to a quaternion
CV_INLINE quaternion::quaternion(const float3 vec)
{
  set(vec.X, vec.Y, vec.Z);
}
#if !IRR_TEST_BROKEN_QUATERNION_USE
// Constructor which converts a matrix to a quaternion
CV_INLINE quaternion::quaternion(const matrix4& mat)
{
  (*this) = mat;
}
#endif
// equal operator
CV_INLINE bool quaternion::operator==(const quaternion& other) const
{
  return ((X == other.X) &&
      (Y == other.Y) &&
      (Z == other.Z) &&
...
// assignment operator
CV_INLINE quaternion& quaternion::operator=(const quaternion& other)
{
  X = other.X;
  Y = other.Y;
  Z = other.Z;
...
#if !IRR_TEST_BROKEN_QUATERNION_USE
// matrix assignment operator
CV_INLINE quaternion& quaternion::operator=(const matrix4& m)
{
...
#endif
// multiplication operator
CV_INLINE quaternion quaternion::operator*(const quaternion& other) const
{
  quaternion tmp;
...
// multiplication operator
CV_INLINE quaternion quaternion::operator*(f32 s) const
{
  return quaternion(s * X, s * Y, s * Z, s * W);
}
// multiplication operator
CV_INLINE quaternion& quaternion::operator*=(f32 s)
{
  X *= s;
  Y *= s;
  Z *= s;
...
// add operator
CV_INLINE quaternion quaternion::operator+(const quaternion& b) const
{
  return quaternion(X + b.X, Y + b.Y, Z + b.Z, W + b.W);
}
#if !IRR_TEST_BROKEN_QUATERNION_USE
// Creates a matrix from this quaternion
CV_INLINE matrix4 quaternion::getMatrix() const
{
  matrix4 m;
  getMatrix(m);
  return m;
...
/*!
  Creates a matrix from this quaternion
*/
CV_INLINE void quaternion::getMatrix(matrix4& dest,
...
/*!
  ...
  lookat *= m2;
*/
CV_INLINE void quaternion::getMatrixCenter(matrix4& dest,
    const float3 center,
...
// Creates a matrix from this quaternion
CV_INLINE void quaternion::getMatrix_transposed(matrix4& dest) const
{
  dest[0] = 1.0f - 2.0f * Y * Y - 2.0f * Z * Z;
  dest[4] = 2.0f * X * Y + 2.0f * Z * W;
  dest[8] = 2.0f * X * Z - 2.0f * Y * W;
...
// Inverts this quaternion
CV_INLINE quaternion& quaternion::makeInverse()
{
  X = -X;
  Y = -Y;
  Z = -Z;
...
// sets new quaternion
CV_INLINE quaternion& quaternion::set(f32 x, f32 y, f32 z, f32 w)
{
  X = x;
  Y = y;
  Z = z;
...
CV_INLINE quaternion& quaternion::set(f32 x, f32 y, f32 z)
{
  f64 angle;
  angle = x * 0.5;
...
// sets new quaternion based on euler angles
CV_INLINE quaternion& quaternion::set(const float3 vec)
{
  return set(vec.X, vec.Y, vec.Z);
}
// sets new quaternion based on other quaternion
CV_INLINE quaternion& quaternion::set(const quaternion& quat)
{
  return (*this = quat);
}
//! returns if this quaternion equals the other one, taking floating point rounding errors into account
CV_INLINE bool quaternion::equals(const quaternion& other, const f32 tolerance) const
{
...
// normalizes the quaternion
CV_INLINE quaternion& quaternion::normalize()
{
...
CV_INLINE quaternion& quaternion::lerp(quaternion q1, quaternion q2, f32 time)
{
  const f32 scale = 1.0f - time;
  return (*this = (q1 * scale) + (q2 * time));
}
// set this quaternion to the result of the interpolation between two quaternions
CV_INLINE quaternion& quaternion::slerp(quaternion q1, quaternion q2, f32 time, f32 threshold)
{
...
// calculates the dot product
CV_INLINE f32 quaternion::dotProduct(const quaternion& q2) const
{
  return (X * q2.X) + (Y * q2.Y) + (Z * q2.Z) + (W * q2.W);
}
//! axis must be unit length, angle in radians
CV_INLINE quaternion& quaternion::fromAngleAxis(f32 angle, const float3 axis)
{
...
CV_INLINE void quaternion::toAngleAxis(f32& angle, float3 axis) const
{
...
CV_INLINE void quaternion::toEuler(float3 euler) const
{
  const f64 sqw = W * W;
  const f64 sqx = X * X;
  const f64 sqy = Y * Y;
...
CV_INLINE float3 quaternion::operator* (const float3 v) const
{
...
// set quaternion to identity
CV_INLINE quaternion& quaternion::makeIdentity()
{
  W = 1.f;
  X = 0.f;
  Y = 0.f;
...
CV_INLINE quaternion& quaternion::rotationFromTo(const float3 from, const float3 to)
{
  ...
  const float3 c = v0.crossProduct(v1) * invs;
