...
//#include "os.h"
//#include "irrMap.h"
static CV_INLINE float3 getAngleWeight(const float3 v1,
    const float3 v2,
    const float3 v3)
{
...
void recalculateNormalsT(IMeshBuffer* buffer, bool smooth, bool angleWeighted)
{
  ...
  if (!smooth) {
    ...
    for (i = 0; i < idxcnt; i += 3) {
      ...
      const float3 normal = plane3d<f32>(v1, v2, v3).Normal;
...
#define USE_IRR_VERSION
  ...
  tangent.set(0, 0, 0);
  binormal.set(0, 0, 0);
  float3 v1(vt2.X - vt1.X, tc2.X - tc1.X, tc2.Y - tc1.Y);
  float3 v2(vt3.X - vt1.X, tc3.X - tc1.X, tc3.Y - tc1.Y);
  float3 txb = v1.crossProduct(v2);
  if (!iszero(txb.X)) {
