...
#endif
//! Public constructor
CD3D8ShaderMaterialRenderer::CD3D8ShaderMaterialRenderer(IDirect3DDevice8* d3ddev, IVideoDriver* driver,
...
void CD3D8ShaderMaterialRenderer::OnSetMaterial(const SMaterial& material, const SMaterial& lastMaterial,
...
bool CD3D8ShaderMaterialRenderer::createPixelShader(const c8* pxsh)
{
  ...
  D3DXAssembleShader(pxsh, (UINT)cstr_len(pxsh), 0, 0, &code, &errors);
#else
...
#ifdef _IRR_D3D_NO_SHADER_DEBUGGING
  // compile shader without debug info
  D3DXAssembleShader(vtxsh, (UINT)cstr_len(vtxsh), 0, 0, &code, &errors);
#else
  // compile shader and emitt some debug informations to
  // make it possible to debug the shader in visual studio
  static int irr_dbg_file_nr = 0;
