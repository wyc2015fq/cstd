...
//! constructor
CD3D8Driver::CD3D8Driver(const SIrrlichtCreationParameters& params, IFileSystem* io)
    ...
    DriverWasReset(true), Params(params)
{
  ...
  SphereMapMatrixD3D8._11 = 0.5f;
  SphereMapMatrixD3D8._12 = 0.0f;
  SphereMapMatrixD3D8._13 = 0.0f;
  SphereMapMatrixD3D8._14 = 0.0f;
  SphereMapMatrixD3D8._21 = 0.0f;
  SphereMapMatrixD3D8._22 = -0.5f;
  SphereMapMatrixD3D8._23 = 0.0f;
  SphereMapMatrixD3D8._24 = 0.0f;
  SphereMapMatrixD3D8._31 = 0.0f;
  SphereMapMatrixD3D8._32 = 0.0f;
  SphereMapMatrixD3D8._33 = 1.0f;
  SphereMapMatrixD3D8._34 = 0.0f;
  SphereMapMatrixD3D8._41 = 0.5f;
  SphereMapMatrixD3D8._42 = 0.5f;
  SphereMapMatrixD3D8._43 = 0.0f;
  SphereMapMatrixD3D8._44 = 1.0f;
  UnitMatrixD3D8 = *(D3DMATRIX*)((void*)IdentityMatrix);
...
bool CD3D8Driver::initDriver(HWND hwnd, bool pureSoftware)
{
  ...
  if (Params.Stencilbuffer) {
    ...
    else if (FAILED(pID3D->CheckDepthStencilMatch(Params.DisplayAdapter, devtype,
...
bool CD3D8Driver::setActiveTexture(u32 stage, const ITexture* texture)
{
...
ITexture* CD3D8Driver::createDeviceDependentTexture(IImage* surface, const char* name, void* mipmapData)
{
...
bool CD3D8Driver::setRenderTarget(ITexture* texture,
...
ITexture* CD3D8Driver::addRenderTargetTexture(
    const dimension2du& size, const char* name,
...
//! sets a viewport
void CD3D8Driver::setViewPort(const IRECT& area)
{
  IRECT vp(area);
  IRECT rendert(0, 0, ScreenSize.Width, ScreenSize.Height);
...
const IRECT& CD3D8Driver::getViewPort() const
{
  return ViewPort;
}
...
void CD3D8Driver::draw2DImage(const ITexture* texture,
...
void CD3D8Driver::draw2DRectangle(const IRECT& position,
    SColor colorLeftUp, SColor colorRightUp, SColor colorLeftDown,
    SColor colorRightDown, const IRECT* clip)
{
  IRECT pos(position);
  if (clip) {
...
//! Draws a 2d line.
void CD3D8Driver::draw2DLine(const position2di& start,
    ...
    if (resetAllRenderstates ||
      ...
      if (material.TextureLayer[st].BilinearFilter || material.TextureLayer[st].TrilinearFilter || material.TextureLayer[st].AnisotropicFilter > 1) {
        ...
        if (tftMag == D3DTEXF_ANISOTROPIC || tftMin == D3DTEXF_ANISOTROPIC) {
          pID3DDevice->SetTextureStageState(st, D3DTSS_MAXANISOTROPY, min_((DWORD)material.TextureLayer[st].AnisotropicFilter, Caps.MaxAnisotropy));
        }
...
const char* CD3D8Driver::getName() const
{
  return "Direct3D 8.1";
}
...
void CD3D8Driver::drawStencilShadowVolume(const float3 > & triangles, bool zfail, u32 debugDataVisible)
{
...
void CD3D8Driver::drawStencilShadow(bool clearStencilBuffer, SColor leftUpEdge,
...
//! Draws a 3d line.
void CD3D8Driver::draw3DLine(const float3 start,
...
void CD3D8Driver::OnResize(const dimension2du& size)
{
  if (!pID3DDevice) {
    return;
  }
...
const matrix4& CD3D8Driver::getTransform(E_TRANSFORMATION_STATE state) const
{
...
IImage* CD3D8Driver::createScreenShot(ECOLOR_FORMAT format, E_RENDER_TARGET target)
{
#if defined( _IRR_XBOX_PLATFORM_)
  return 0;
#else
...
const dimension2du& CD3D8Driver::getCurrentRenderTargetSize() const
{
...
bool CD3D8Driver::setClipPlane(u32 index, const plane3df& plane, bool enable)
{
#if defined( _IRR_XBOX_PLATFORM_)
  return false;
#else
...
dimension2du CD3D8Driver::getMaxTextureSize() const
{
