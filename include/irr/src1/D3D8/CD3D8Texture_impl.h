...
CD3D8Texture::CD3D8Texture(CD3D8Driver* driver, const dimension2du& size, const char* name)
...
CD3D8Texture::CD3D8Texture(IImage* image, CD3D8Driver* driver,
    u32 flags, const char* name, void* mipmapData)
  : ITexture(name), Texture(0), RTTSurface(0), Driver(driver),
...
bool CD3D8Texture::createTexture(u32 flags, IImage* image)
{
  ImageSize = image->getDimension();
  dimension2du optSize = ImageSize.getOptimalSize(!Driver->queryFeature(EVDF_TEXTURE_NPOT), !Driver->queryFeature(EVDF_TEXTURE_NSQUARE), true, Driver->Caps.MaxTextureWidth);
...
bool CD3D8Texture::copyTexture(IImage* image)
{
  if (Texture && image) {
...
const dimension2du& CD3D8Texture::getOriginalSize() const
{
  return ImageSize;
}
//! Returns (=size) of the texture.
const dimension2du& CD3D8Texture::getSize() const
{
  return TextureSize;
}
