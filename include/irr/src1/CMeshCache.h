...
public:
  ...
  s32 getMeshIndex(const IMesh* const mesh) const;
  ...
  IAnimatedMesh* getMeshByIndex(u32 index);
  //! Returns a mesh based on its name.
  /** \param name Name of the mesh. Usually a filename.
  \return Pointer to the mesh or 0 if there is none with this number. */
  IAnimatedMesh* getMeshByName(const char* name);
  //! Get the name of a loaded mesh, based on its index.
  /** \param index: Index of the mesh, number between 0 and getMeshCount()-1.
  \return The name if mesh was found and has a name, else the path is empty. */
  const char* getMeshName(u32 index) const;
  //! Get the name of a loaded mesh, if there is any.
  /** \param mesh Pointer to mesh to query.
  \return The name if mesh was found and has a name, else the path is empty. */
  const char* getMeshName(const IMesh* const mesh) const;
  ...
  bool renameMesh(u32 index, const char* name);
  //! Renames a loaded mesh.
  /** Note that renaming meshes might change the ordering of the
  meshes, and so the index of the meshes as returned by
  getMeshIndex() or taken by some methods will change.
  ...
  \param name New name for the mesh.
  \return True if mesh was renamed. */
  bool renameMesh(const IMesh* const mesh, const char* name);
