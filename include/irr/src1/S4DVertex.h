...
struct sCompressedVec4 {
  ...
  void setColorf(const SColorf& color) {
    argb =  floor32(color.a * 255.f) << 24 |
        floor32(color.r * 255.f) << 16 |
        floor32(color.g * 255.f) << 8  |
        floor32(color.b * 255.f);
  }
  void setVec4(const sVec4& v);
  // f = a * t + b * ( 1 - t )
  void interpolate(const sCompressedVec4& a, const sCompressedVec4& b, const f32 t) {
    argb = PixelBlend32(b.argb, a.argb, floor32(t * 256.f));
  }
};
struct sVec4 {
  union {
  ...
  void normalize_xyz() {
    const f32 l = reciprocal_squareroot(x * x + y * y + z * z);
    x *= l;
    y *= l;
    z *= l;
  }
...
struct sVec3 {
  ...
  void setLength(f32 len) {
    const f32 l = len * reciprocal_squareroot(r * r + g * g + b * b);
    r *= l;
    g *= l;
    b *= l;
  }
};
CV_INLINE void sCompressedVec4::setVec4(const sVec4& v)
{
  argb =  floor32(v.x * 255.f) << 24 |
      floor32(v.y * 255.f) << 16 |
      floor32(v.z * 255.f) << 8  |
      floor32(v.w * 255.f);
}
...
/*
  load a color value
*/
CV_INLINE void getTexel_plain2(tFixPoint& r, tFixPoint& g, tFixPoint& b,
    const sVec4& v
                              )
{
  r = tofix(v.y);
...
/*
  load a color value
*/
CV_INLINE void getSample_color(tFixPoint& a, tFixPoint& r, tFixPoint& g, tFixPoint& b,
    const sVec4& v
                              )
{
  a = tofix(v.x);
...
/*
  load a color value
*/
CV_INLINE void getSample_color(tFixPoint& r, tFixPoint& g, tFixPoint& b, const sVec4& v)
{
  r = tofix(v.y, COLOR_MAX * FIX_POINT_F32_MUL);
  g = tofix(v.z, COLOR_MAX * FIX_POINT_F32_MUL);
  b = tofix(v.w, COLOR_MAX * FIX_POINT_F32_MUL);
...
/*
  load a color value
*/
CV_INLINE void getSample_color(tFixPoint& r, tFixPoint& g, tFixPoint& b,
