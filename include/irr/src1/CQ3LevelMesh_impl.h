...
bool CQ3LevelMesh::loadFile(IReadFile* file)
{
  ...
  os::Byteswap::BYTESWAP(header.strID);
  os::Byteswap::BYTESWAP(header.version);
#endif
  ...
  if (LoadParam.swapHeader) {
    for (i = 0; i < kMaxLumps; ++i) {
      os::Byteswap::BYTESWAP(Lumps[i].offset);
      os::Byteswap::BYTESWAP(Lumps[i].length);
    }
  }
  ReleaseEntity();
  // load everything
...
void CQ3LevelMesh::loadTextures(tBSPLump* l, IReadFile* file)
{
  ...
  if (LoadParam.swapHeader) {
    for (s32 i = 0; i < NumTextures; ++i) {
      os::Byteswap::BYTESWAP(Textures[i].flags);
      os::Byteswap::BYTESWAP(Textures[i].contents);
...
void CQ3LevelMesh::loadVerts(tBSPLump* l, IReadFile* file)
{
  ...
  if (LoadParam.swapHeader)
    for (s32 i = 0; i < NumVertices; i++) {
      os::Byteswap::BYTESWAP(Vertices[i].vPosition[0]);
      os::Byteswap::BYTESWAP(Vertices[i].vPosition[1]);
      os::Byteswap::BYTESWAP(Vertices[i].vPosition[2]);
      os::Byteswap::BYTESWAP(Vertices[i].vTextureCoord[0]);
      os::Byteswap::BYTESWAP(Vertices[i].vTextureCoord[1]);
      os::Byteswap::BYTESWAP(Vertices[i].vLightmapCoord[0]);
      os::Byteswap::BYTESWAP(Vertices[i].vLightmapCoord[1]);
      os::Byteswap::BYTESWAP(Vertices[i].vNormal[0]);
      os::Byteswap::BYTESWAP(Vertices[i].vNormal[1]);
      os::Byteswap::BYTESWAP(Vertices[i].vNormal[2]);
    }
}
/*!
*/
void CQ3LevelMesh::loadFaces(tBSPLump* l, IReadFile* file)
{
  ...
  if (LoadParam.swapHeader) {
    for (s32 i = 0; i < NumFaces; i++) {
      os::Byteswap::BYTESWAP(Faces[i].textureID);
      os::Byteswap::BYTESWAP(Faces[i].fogNum);
      os::Byteswap::BYTESWAP(Faces[i].type);
      os::Byteswap::BYTESWAP(Faces[i].vertexIndex);
      os::Byteswap::BYTESWAP(Faces[i].numOfVerts);
      os::Byteswap::BYTESWAP(Faces[i].meshVertIndex);
      os::Byteswap::BYTESWAP(Faces[i].numMeshVerts);
      os::Byteswap::BYTESWAP(Faces[i].lightmapID);
      os::Byteswap::BYTESWAP(Faces[i].lMapCorner[0]);
      os::Byteswap::BYTESWAP(Faces[i].lMapCorner[1]);
      os::Byteswap::BYTESWAP(Faces[i].lMapSize[0]);
      os::Byteswap::BYTESWAP(Faces[i].lMapSize[1]);
      os::Byteswap::BYTESWAP(Faces[i].lMapPos[0]);
      os::Byteswap::BYTESWAP(Faces[i].lMapPos[1]);
      os::Byteswap::BYTESWAP(Faces[i].lMapPos[2]);
      os::Byteswap::BYTESWAP(Faces[i].lMapBitsets[0][0]);
      os::Byteswap::BYTESWAP(Faces[i].lMapBitsets[0][1]);
      os::Byteswap::BYTESWAP(Faces[i].lMapBitsets[0][2]);
      os::Byteswap::BYTESWAP(Faces[i].lMapBitsets[1][0]);
      os::Byteswap::BYTESWAP(Faces[i].lMapBitsets[1][1]);
      os::Byteswap::BYTESWAP(Faces[i].lMapBitsets[1][2]);
      os::Byteswap::BYTESWAP(Faces[i].vNormal[0]);
      os::Byteswap::BYTESWAP(Faces[i].vNormal[1]);
      os::Byteswap::BYTESWAP(Faces[i].vNormal[2]);
      os::Byteswap::BYTESWAP(Faces[i].size[0]);
      os::Byteswap::BYTESWAP(Faces[i].size[1]);
    }
  }
}
/*!
...
void CQ3LevelMesh::loadEntities(tBSPLump* l, IReadFile* file)
{
  ...
  entity[l->length + 1 ] = 0;
  file->seek(l->offset);
  file->read(entity, l->length);
  parser_parse(entity, l->length, &CQ3LevelMesh::scriptcallback_entity);
}
/*!
  load fog brushes
*/
...
void CQ3LevelMesh::loadModels(tBSPLump* l, IReadFile* file)
{
  ...
  if (LoadParam.swapHeader) {
    for (s32 i = 0; i < NumModels; i++) {
      os::Byteswap::BYTESWAP(Models[i].min[0]);
      os::Byteswap::BYTESWAP(Models[i].min[1]);
      os::Byteswap::BYTESWAP(Models[i].min[2]);
      os::Byteswap::BYTESWAP(Models[i].max[0]);
      os::Byteswap::BYTESWAP(Models[i].max[1]);
      os::Byteswap::BYTESWAP(Models[i].max[2]);
      os::Byteswap::BYTESWAP(Models[i].faceIndex);
      os::Byteswap::BYTESWAP(Models[i].numOfFaces);
      os::Byteswap::BYTESWAP(Models[i].brushIndex);
      os::Byteswap::BYTESWAP(Models[i].numOfBrushes);
    }
  }
...
void CQ3LevelMesh::loadMeshVerts(tBSPLump* l, IReadFile* file)
{
  ...
  if (LoadParam.swapHeader) {
    ...
    for (i = 0; i < NumMeshVerts; i++) {
      os::Byteswap::BYTESWAP(MeshVerts[i]);
    }
  }
}
/*!
...
/*!
*/
CV_INLINE bool isQ3WhiteSpace(const u8 symbol)
{
  return symbol == ' ' || symbol == '\t' || symbol == '\r';
}
/*!
*/
CV_INLINE bool isQ3ValidName(const u8 symbol)
{
...
void CQ3LevelMesh::parser_parse(const void* data, const u32 size, CQ3LevelMesh::tParserCallback callback)
{
  ...
  do {
    ...
    case Q3_TOKEN_START_LIST: {
      ...
      groupList->VariableGroup.push_back(SVarGroup());
      last = active;
      active = groupList->VariableGroup_size - 1;
      entity.clear();
    }
    break;
...
*/
s32 CQ3LevelMesh::setShaderFogMaterial(SMaterial& material, const tBSPFace* face) const
{
  ...
  s32 shaderState = -1;
  if ((u32) face->fogNum < FogMap_size) {
...
/*
  this loader applies only textures for stage 1 & 2
*/
s32 CQ3LevelMesh::setShaderMaterial(SMaterial& material, const tBSPFace* face) const
{
  material.MaterialType = EMT_SOLID;
  material.Wireframe = false;
  material.Lighting = false;
  ...
  material.MaterialTypeParam = 0.f;
  s32 shaderState = -1;
  if (face->textureID >= 0 && face->textureID < (s32)Tex_size) {
    material.setTexture(0, Tex [ face->textureID ].Texture);
    shaderState = Tex [ face->textureID ].ShaderID;
  }
  if (face->lightmapID >= 0 && face->lightmapID < (s32)Lightmap_size) {
  ...
  //material.BackfaceCulling = false;
  if (shader->VarGroup->VariableGroup_size <= 4) {
...
*/
SMesh** CQ3LevelMesh::buildMesh(s32 num)
{
...
void CQ3LevelMesh::S3DVertex2TCoords_64::copy(S3DVertex& dest) const
{
#if defined (TJUNCTION_SOLVER_ROUND)
  dest.Pos.X = round_((f32) Pos.X);
  dest.Pos.Y = round_((f32) Pos.Y);
  dest.Pos.Z = round_((f32) Pos.Z);
...
void CQ3LevelMesh::copy(S3DVertex2TCoords_64* dest, const tBSPVertex* source, s32 vertexcolor) const
{
  ...
  if (vertexcolor) {
    //u32 a = s32_min( source->color[3] * LoadParam.defaultModulate, 255 );
    u32 a = source->color[3];
    u32 r = s32_min(source->color[0] * LoadParam.defaultModulate, 255);
    u32 g = s32_min(source->color[1] * LoadParam.defaultModulate, 255);
    u32 b = s32_min(source->color[2] * LoadParam.defaultModulate, 255);
...
CV_INLINE void CQ3LevelMesh::copy(S3DVertex* dest, const tBSPVertex* source, s32 vertexcolor) const
{
#if defined (TJUNCTION_SOLVER_ROUND)
  dest->Pos.X = round_(source->vPosition[0]);
  dest->Pos.Y = round_(source->vPosition[2]);
...
void CQ3LevelMesh::SBezier::tesselate(s32 level)
{
  ...
  const u32 idx = Patch->Vertices_size;
  ...
  Patch->Indices.reallocate(Patch->Indices_size + 6 * level * level);
  // connect
  for (j = 0; j < level; ++j) {
    for (k = 0; k < level; ++k) {
    ...
    s32 storevertexcolor)
{
  ...
  meshBuffer->Indices.reallocate(meshBuffer->Indices_size + 6 * (controlHeight - 1) * (controlWidth - 1));
    ...
    s32 storevertexcolor)
{
  ...
  S3DVertex2TCoords_64 > controlPoint;
  controlPoint.set_used(controlWidth * controlHeight);
  for (j = 0; j < controlPoint_size; ++j) {
  ...
  /*
    for ( j = 0; j!= bsize; ++j )
    {
      const float3 &v = Bezier.Patch->Vertices[j].Pos;
      for ( k = 0; k!= msize; ++k )
      {
        const float3 &m = meshBuffer->Vertices[k].Pos;
...
void CQ3LevelMesh::getConfiguration(IReadFile* file)
{
  ...
  file->seek(l.offset);
  file->read(entity, l.length);
  parser_parse(entity, l.length, &CQ3LevelMesh::scriptcallback_config);
  if (Entity_size) {
    Entity.getLast().name = file->getFileName();
  }
}
//! get's an interface to the entities
IEntity*& CQ3LevelMesh::getEntityList()
{
  //  Entity.sort();
  return Entity;
}
...
IMesh* CQ3LevelMesh::getBrushEntityMesh(quake3::IEntity& ent) const
{
  ...
  const char* modnum = group->get("model");
  if (!group->isDefined("model")) {
    return 0;
  }
  const char* temp = modnum + 1; // We skip the first character.
  num = strtol10(temp);
...
const IShader* CQ3LevelMesh::getShader(const c8* filename, bool fileNameIsValid)
{
  ...
  if (index >= 0) {
    if (LoadParam.verbose > 1) {
      ...
      os::Printer::log("quake3:getShader", message, ELL_INFORMATION);
    }
    return &Shader[index];
  }
  char* loadFile;
  ...
  if (LoadParam.verbose) {
    ...
    os::Printer::log("quake3:getShader Load shader", message, ELL_INFORMATION);
  }
  IReadFile* file = FileSystem->createAndOpenFile(loadFile);
  if (file) {
    getShader(file);
    file->drop();
  }
...
void CQ3LevelMesh::getShader(IReadFile* file)
{
  ...
  parser_parse(script, len, &CQ3LevelMesh::scriptcallback_shader);
}
...
void CQ3LevelMesh::InitShader()
{
  ...
  if (LoadParam.loadAllShaders) {
    ...
    char* save = FileSystem->getWorkingDirectory();
    char* newDir;
    newDir = "/";
    newDir += LoadParam.scriptDir;
    newDir += "/";
    FileSystem->changeWorkingDirectoryTo(newDir);
    char* s;
...
void CQ3LevelMesh::scriptcallback_config(SVarGroupList* grouplist, eToken token)
{
  ...
  if (token == Q3_TOKEN_END_LIST) {
    if (0 == grouplist->VariableGroup[0].Variable_size) {
      return;
    }
    element.name = grouplist->VariableGroup[0].Variable[0].name;
  }
  else {
    if (grouplist->VariableGroup_size != 2) {
      return;
    }
...
{
  if (token != Q3_TOKEN_END_LIST || grouplist->VariableGroup_size != 2) {
    return;
  }
  grouplist->grab();
  IEntity element;
...
{
  if (token != Q3_TOKEN_END_LIST || grouplist->VariableGroup[0].Variable_size == 0) {
    return;
  }
  IShader element;
  grouplist->grab();
...
void CQ3LevelMesh::calcBoundingBoxes()
{
  ...
  for (g = 0; g != E_Q3_MESH_SIZE; ++g) {
    for (int j = 0; j < Mesh[g]->MeshBuffers_size; ++j) {
  ...
  for (g = 1; g < NumModels; g++) {
    for (int j = 0; j < BrushEntities[g]->MeshBuffers_size; ++j) {
...
void CQ3LevelMesh::loadTextures()
{
  ...
  */
  dimension2du lmapsize(128, 128);
  IImage* lmapImg;
  for (t = 0; t < NumLightMaps ; ++t) {
    sprintf(lightmapname, "%s.lightmap.%d", LevelName, t);
...
const F3AABBOX& CQ3LevelMesh::getBoundingBox() const
{
  return Mesh[0]->getBoundingBox();
}
void CQ3LevelMesh::setBoundingBox(const aabbox3df& box)
{
