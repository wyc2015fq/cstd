...
bool CImageWriterPPM::isAWriteableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "ppm");
}
bool CImageWriterPPM::writeImage(IWriteFile* file, IImage* image, u32 param) const
{
  char cache[70];
  int size;
  const dimension2du& imageSize = image->getDimension();
  ...
  if (binary) {
    ...
    s32 n = 0;
    for (int h = 0; h < imageSize.Height; ++h) {
      for (int c = 0; c < imageSize.Width; ++c, ++n) {
        const SColor& pixel = image->getPixel(c, h);
