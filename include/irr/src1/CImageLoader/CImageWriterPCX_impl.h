...
bool CImageWriterPCX::isAWriteableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "pcx");
}
bool CImageWriterPCX::writeImage(IWriteFile* file, IImage* image, u32 param) const
{
  if (!file || !image) {
    return false;
  }
  ...
  for (i = 0; i < image->getDimension().Height; ++i) {
    cnt = 0;
    value = 0;
    for (int j = 0; j < 3; ++j) { // color planes
      for (int k = 0; k < image->getDimension().Width; ++k) {
