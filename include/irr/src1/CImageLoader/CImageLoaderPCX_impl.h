...
bool CImageLoaderPCX::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "pcx");
}
//! returns true if the file maybe is able to be loaded by this class
bool CImageLoaderPCX::isALoadableFileFormat(IReadFile* file) const
{
  u8 headerID;
...
IImage* CImageLoaderPCX::loadImage(IReadFile* file) const
{
  SPCXHeader header;
  s32* paletteData = 0;
  ...
  if (header.BitsPerPixel == 8) {
    ...
    case 1:
      ...
      break;
    case 3:
      image = new CImage(ECF_R8G8B8, dimension2du(width, height));
      if (image) {
