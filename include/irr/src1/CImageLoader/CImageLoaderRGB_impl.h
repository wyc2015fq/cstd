...
bool CImageLoaderRGB::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "rgb", "rgba", "sgi") ||
      hasFileExtension(filename, "int", "inta", "bw");
}
//! returns true if the file maybe is able to be loaded by this class
bool CImageLoaderRGB::isALoadableFileFormat(IReadFile* file) const
{
  rgbStruct rgb;
...
IImage* CImageLoaderRGB::loadImage(IReadFile* file) const
{
  IImage* image = 0;
  s32* paletteData = 0;
  ...
  if (checkFormat(file, rgb)) {
    ...
    if (rgb.Header.BPC != 1) {
      ...
      /*
            ...
            (image color â—?alpha) + (background color â—?(100% - alpha)).
            Using precomputed blending
            (image color) + (background color â—?(100% - alpha)).
      ...
      */
      ...
      case 1:
        ...
        image = new CImage(ECF_A1R5G5B5, dimension2du(rgb.Header.Xsize, rgb.Header.Ysize));
        if (image) {
      ...
      case 3:
        ...
        image = new CImage(ECF_R8G8B8, dimension2du(rgb.Header.Xsize, rgb.Header.Ysize));
        if (image) {
      ...
      case 4:
        ...
        image = new CImage(ECF_A8R8G8B8, dimension2du(rgb.Header.Xsize, rgb.Header.Ysize));
        if (image) {
...
bool CImageLoaderRGB::readHeader(IReadFile* file, rgbStruct& rgb) const
{
  ...
  os::Byteswap::BYTESWAP(rgb.Header.Magic);
  os::Byteswap::BYTESWAP(rgb.Header.Storage);
  os::Byteswap::BYTESWAP(rgb.Header.Dimension);
  os::Byteswap::BYTESWAP(rgb.Header.Xsize);
  os::Byteswap::BYTESWAP(rgb.Header.Ysize);
  os::Byteswap::BYTESWAP(rgb.Header.Zsize);
  os::Byteswap::BYTESWAP(rgb.Header.Pixmin);
  os::Byteswap::BYTESWAP(rgb.Header.Pixmax);
  os::Byteswap::BYTESWAP(rgb.Header.Colormap);
#endif
...
bool CImageLoaderRGB::checkFormat(IReadFile* file, rgbStruct& rgb) const
{
...
*/
bool CImageLoaderRGB::readOffsetTables(IReadFile* file, rgbStruct& rgb) const
{
  ...
  int i;
  for (i = 0; i < length; ++i) {
    os::Byteswap::BYTESWAP(rgb.StartTable[i]);
    os::Byteswap::BYTESWAP(rgb.LengthTable[i]);
  }
#endif
  return true;
}
...
*/
void CImageLoaderRGB::processFile(IReadFile* file, rgbStruct& rgb) const
{
  u16* tempShort;
  // calculate the size of the buffer needed: XSIZE * YSIZE * ZSIZE * BPC
...
*/
void CImageLoaderRGB::readRGBrow(u8* buf, int y, int z, IReadFile* file, rgbStruct& rgb) const
{
