...
bool CImageLoaderPPM::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "ppm", "pgm", "pbm");
}
//! returns true if the file maybe is able to be loaded by this class
bool CImageLoaderPPM::isALoadableFileFormat(IReadFile* file) const
{
  c8 id[2] = {0};
  file->read(&id, 2);
...
IImage* CImageLoaderPPM::loadImage(IReadFile* file) const
{
  IImage* image;
  if (file->getSize() < 12) {
    return 0;
  ...
  const u32 width = strtoul10(token);
  getNextToken(file, token);
  const u32 height = strtoul10(token);
  u8* data = 0;
  ...
  if (format == 1 || format == 4) {
    ...
    image = new CImage(ECF_A1R5G5B5, dimension2du(width, height));
    if (image) {
    ...
    getNextToken(file, token);
    const u32 maxDepth = strtoul10(token);
    ...
    if (format == 2 || format == 5) {
      if (binary) {
        ...
        data = new u8[size];
        file->read(data, size);
        image = new CImage(ECF_A8R8G8B8, dimension2du(width, height));
        if (image) {
        ...
        if (image) {
          ...
          for (i = 0; i < size; ++i) {
            getNextToken(file, token);
            const u8 num = (u8)strtoul10(token);
            *ptr++ = num;
            *ptr++ = num;
            *ptr++ = num;
            *ptr++ = 255;
...
void CImageLoaderPPM::getNextToken(IReadFile* file, char* token) const
{
  token = "";
  c8 c;
...
void CImageLoaderPPM::skipToNextToken(IReadFile* file) const
{
  c8 c;
  while (file->getPos() < file->getSize()) {
    file->read(&c, 1);
