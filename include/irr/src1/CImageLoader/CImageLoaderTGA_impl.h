...
bool CImageLoaderTGA::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "tga");
}
//! loads a compressed tga.
u8* CImageLoaderTGA::loadCompressedImage(IReadFile* file, const STGAHeader& header) const
{
  ...
  case 8: {
    if (header.ImageType == 3) { // grey image
      image = new CImage(ECF_R8G8B8,
          dimension2du(header.ImageWidth, header.ImageHeight));
      if (image)
