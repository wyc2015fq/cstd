...
bool CImageLoaderPSD::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "psd");
}
//! returns true if the file maybe is able to be loaded by this class
bool CImageLoaderPSD::isALoadableFileFormat(IReadFile* file) const
{
  if (!file) {
    return false;
  }
...
IImage* CImageLoaderPSD::loadImage(IReadFile* file) const
{
  u32* imageData = 0;
  PsdHeader header;
  ...
  os::Byteswap::BYTESWAP(header.channels);
  os::Byteswap::BYTESWAP(header.height);
  os::Byteswap::BYTESWAP(header.width);
  os::Byteswap::BYTESWAP(header.depth);
  os::Byteswap::BYTESWAP(header.mode);
#endif
...
#ifndef __BIG_ENDIAN__
  os::Byteswap::BYTESWAP(compressionType);
#endif
  ...
  if (res) {
    ...
    image = new CImage(ECF_A8R8G8B8,
        dimension2du(header.width, header.height), imageData);
  }
  if (!image) {
    delete [] imageData;
  }
...
bool CImageLoaderPSD::readRawImageData(IReadFile* file, const PsdHeader& header, u32* imageData) const
{
...
bool CImageLoaderPSD::readRLEImageData(IReadFile* file, const PsdHeader& header, u32* imageData) const
{
  ...
  */
  ...
  s32 size = 0;
  for (int y = 0; y < header.height * header.channels; ++y) {
...
#ifndef __BIG_ENDIAN__
    os::Byteswap::BYTESWAP(rleCount[y]);
#endif
    size += rleCount[y];
  }
  s8* buf = new s8[size];
  ...
  for (s32 channel = 0; channel < header.channels; channel++) {
    for (int y = 0; y < header.height; ++y, ++rcount) {
      bytesRead = 0;
