...
bool CImageLoaderLMP::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "lmp");
}
bool CImageLoaderLMP::isALoadableFileFormat(IReadFile* file) const
{
  return false;
}
/*!
...
*/
IImage* CImageLoaderLMP::loadImage(IReadFile* file) const
{
  SLMPHeader header;
  file->seek(0);
  ...
  IImage* image = new CImage(ECF_A8R8G8B8, dimension2du(header.width, header.height));
...
/*!
  Halflife
*/
bool CImageLoaderWAL2::isALoadableFileExtension(const char* filename) const
{
  // embedded in Wad(WAD3 format). originally it has no extension
  return hasFileExtension(filename, "wal2");
}
bool CImageLoaderWAL2::isALoadableFileFormat(IReadFile* file) const
{
  return false;
}
/*
...
*/
IImage* CImageLoaderWAL2::loadImage(IReadFile* file) const
{
  ...
  file->read(rawtex, rawtexsize);
  IImage* image = new CImage(format, dimension2du(header.width, header.height));
  switch (format) {
  case ECF_R8G8B8:
...
bool CImageLoaderWAL::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "wal");
}
bool CImageLoaderWAL::isALoadableFileFormat(IReadFile* file) const
{
  return false;
}
/*!
  quake2
*/
IImage* CImageLoaderWAL::loadImage(IReadFile* file) const
{
...
{
  return new CImageLoaderWAL();
}
IImageLoader* createImageLoaderHalfLife()
{
  return new CImageLoaderWAL2();
}
#endif
