  ...
  : SpriteBank(0), Driver(driver), Type(type)
{
  ...
  if ((Type == EGST_WINDOWS_CLASSIC) || (Type == EGST_WINDOWS_METALLIC)) {
    Colors[EGDC_3D_DARK_SHADOW]     = SColor(101, 50, 50, 50);
    Colors[EGDC_3D_SHADOW]          = SColor(101, 130, 130, 130);
    Colors[EGDC_3D_FACE]            = SColor(101, 210, 210, 210);
    Colors[EGDC_3D_HIGH_LIGHT]      = SColor(101, 255, 255, 255);
    Colors[EGDC_3D_LIGHT]           = SColor(101, 210, 210, 210);
    Colors[EGDC_ACTIVE_BORDER]      = SColor(101, 16, 14, 115);
    Colors[EGDC_ACTIVE_CAPTION]     = SColor(255, 255, 255, 255);
    Colors[EGDC_APP_WORKSPACE]      = SColor(101, 100, 100, 100);
    Colors[EGDC_BUTTON_TEXT]        = SColor(240, 10, 10, 10);
    Colors[EGDC_GRAY_TEXT]          = SColor(240, 130, 130, 130);
    Colors[EGDC_HIGH_LIGHT]         = SColor(101, 8, 36, 107);
    Colors[EGDC_HIGH_LIGHT_TEXT]    = SColor(240, 255, 255, 255);
    Colors[EGDC_INACTIVE_BORDER]    = SColor(101, 165, 165, 165);
    Colors[EGDC_INACTIVE_CAPTION]   = SColor(255, 30, 30, 30);
    Colors[EGDC_TOOLTIP]            = SColor(200, 0, 0, 0);
    Colors[EGDC_TOOLTIP_BACKGROUND] = SColor(200, 255, 255, 225);
    Colors[EGDC_SCROLLBAR]          = SColor(101, 230, 230, 230);
    Colors[EGDC_WINDOW]             = SColor(101, 255, 255, 255);
    Colors[EGDC_WINDOW_SYMBOL]      = SColor(200, 10, 10, 10);
    Colors[EGDC_ICON]               = SColor(200, 255, 255, 255);
    Colors[EGDC_ICON_HIGH_LIGHT]    = SColor(200, 8, 36, 107);
    Colors[EGDC_GRAY_WINDOW_SYMBOL] = SColor(240, 100, 100, 100);
    Colors[EGDC_EDITABLE]       = SColor(255, 255, 255, 255);
    Colors[EGDC_GRAY_EDITABLE]    = SColor(255, 120, 120, 120);
    Colors[EGDC_FOCUSED_EDITABLE] = SColor(255, 240, 240, 255);
  ...
  Texts[EGDT_MSG_BOX_OK] = "OK";
  Texts[EGDT_MSG_BOX_CANCEL] = "Cancel";
  Texts[EGDT_MSG_BOX_YES] = "Yes";
  Texts[EGDT_MSG_BOX_NO] = "No";
  Texts[EGDT_WINDOW_CLOSE] = "Close";
  Texts[EGDT_WINDOW_RESTORE] = "Restore";
  Texts[EGDT_WINDOW_MINIMIZE] = "Minimize";
  Texts[EGDT_WINDOW_MAXIMIZE] = "Maximize";
...
SColor CGUISkin::getColor(EGUI_DEFAULT_COLOR color) const
{
...
void CGUISkin::setColor(EGUI_DEFAULT_COLOR which, SColor newColor)
{
...
const char* CGUISkin::getDefaultText(EGUI_DEFAULT_TEXT text) const
{
...
//! "OK", "Cancel", "Yes", "No" and so on.
void CGUISkin::setDefaultText(EGUI_DEFAULT_TEXT which, const char* newText)
{
    ...
    const IRECT* clip)
{
  ...
  if (!UseGradient) {
    ...
    const SColor c1 = getColor(EGDC_3D_FACE);
    const SColor c2 = c1.getInterpolated(getColor(EGDC_3D_DARK_SHADOW), 0.4f);
...
\param rect: Defining area where to draw.
\param clip: Clip area. */
void CGUISkin::draw3DSunkenPane(IGUIElement* element, SColor bgcolor,
...
IRECT CGUISkin::draw3DWindowBackground(IGUIElement* element,
    ...
    IRECT* checkClientArea)
{
  ...
  if (!checkClientArea) {
    if (!UseGradient) {
      ...
      const SColor c1 = getColor(EGDC_WINDOW).getInterpolated(0xFFFFFFFF, 0.9f);
      const SColor c2 = getColor(EGDC_WINDOW).getInterpolated(0xFFFFFFFF, 0.8f);
      Driver->draw2DRectangle(rect, c1, c1, c2, c2, clip);
    }
    else {
      const SColor c2 = getColor(EGDC_3D_SHADOW);
      const SColor c1 = getColor(EGDC_3D_FACE);
  ...
  if (drawTitleBar) {
    if (checkClientArea) {
      ...
      //else
      if (Type == EGST_BURNING_SKIN) {
        const SColor c = titleBarColor.getInterpolated(SColor(titleBarColor.getAlpha(), 255, 255, 255), 0.8f);
        Driver->draw2DRectangle(rect, titleBarColor, titleBarColor, c, c, clip);
      }
      else {
        const SColor c = titleBarColor.getInterpolated(SColor(titleBarColor.getAlpha(), 0, 0, 0), 0.2f);
...
void CGUISkin::draw3DMenuPane(IGUIElement* element,
    const IRECT& r, const IRECT* clip)
{
  if (!Driver) {
    return;
  }
    ...
    const IRECT* clip)
{
  ...
  if (!UseGradient) {
    ...
    const SColor c1 = 0xF0000000 | getColor(EGDC_3D_FACE).color;
    const SColor c2 = 0xF0000000 | getColor(EGDC_3D_SHADOW).color;
    rect.LowerRightCorner.Y += 1;
...
void CGUISkin::draw3DTabButton(IGUIElement* element, bool active,
    const IRECT& frameRect, const IRECT* clip, EGUI_ALIGNMENT alignment)
{
  if (!Driver) {
    return;
  }
...
void CGUISkin::draw3DTabBody(IGUIElement* element, bool border, bool background,
    const IRECT& rect, const IRECT* clip, s32 tabHeight, EGUI_ALIGNMENT alignment)
{
  if (!Driver) {
    return;
  }
  ...
  if (background) {
    ...
    if (!UseGradient) {
      ...
      SColor c1 = getColor(EGDC_3D_FACE);
      SColor c2 = getColor(EGDC_3D_SHADOW);
...
void CGUISkin::draw2DRectangle(IGUIElement* element,
    const SColor& color, const IRECT& pos,
    const IRECT* clip)
{
...
void CGUISkin::serializeAttributes(IAttributes* out, SAttributeReadWriteOptions* options) const
{
  ...
  for (i = 0; i < EGDT_COUNT; ++i) {
    out->addString(GUISkinTextNames[i], Texts[i]);
  }
...
void CGUISkin::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options)
{
  ...
  for (i = 0; i < EGDT_COUNT; ++i) {
    Texts[i] = in->getAttributeAsString(GUISkinTextNames[i]);
  }
  for (i = 0; i < EGDI_COUNT; ++i) {
