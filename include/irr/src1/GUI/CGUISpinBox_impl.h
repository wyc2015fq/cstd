...
//#include <wchar.h>
//! constructor
CGUISpinBox::CGUISpinBox(const char* text, bool border, IGUIEnvironment* environment,
  ...
  : IGUISpinBox(environment, parent, id, rectangle),
    ...
    RangeMin(-FLT_MAX), RangeMax(FLT_MAX), FormatString("%f"),
    DecimalPlaces(-1)
{
#ifdef _DEBUG
  ...
  ButtonSpinDown = Environment->addButton(
      IRECT(rectangle.getWidth() - ButtonWidth, rectangle.getHeight() / 2 + 1,
  ...
  ButtonSpinUp = Environment->addButton(
      IRECT(rectangle.getWidth() - ButtonWidth, 0,
  ...
  const IRECT rectEdit(0, 0, rectangle.getWidth() - ButtonWidth - 1, rectangle.getHeight());
...
void CGUISpinBox::refreshSprites()
{
  ...
  if (sb) {
    ...
    ButtonSpinDown->setText("-");
    ButtonSpinUp->setText("+");
  }
}
...
{
  char str[100];
  snprintf(str, 99, FormatString, val);
  EditBox->setText(str);
  verifyValueRange();
}
f32 CGUISpinBox::getValue() const
{
  const char* val = EditBox->getText();
  if (!val) {
    return 0.f;
  }
  char* tmp(val);
  return fast_atof(tmp);
}
void CGUISpinBox::setRange(f32 min, f32 max)
{
  ...
  char str[100];
  snprintf(str, 99, FormatString, RangeMin);
  RangeMin = fast_atof(char * (str));
  snprintf(str, 99, FormatString, RangeMax);
  RangeMax = fast_atof(char * (str));
  verifyValueRange();
}
...
{
  f32 val = getValue();
  if (val + ROUNDING_ERROR_f32 < RangeMin) {
    val = RangeMin;
  }
  else if (val - ROUNDING_ERROR_f32 > RangeMax) {
    val = RangeMax;
  }
  else {
    return;
...
void CGUISpinBox::setText(const char* text)
{
...
const char* CGUISpinBox::getText() const
{
