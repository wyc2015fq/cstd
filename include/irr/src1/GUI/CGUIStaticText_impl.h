...
//! constructor
CGUIStaticText::CGUIStaticText(const char* text, bool border,
    IGUIEnvironment* environment, IGUIElement* parent,
    s32 id, const IRECT& rectangle,
    bool background)
  : IGUIStaticText(environment, parent, id, rectangle),
    ...
    OverrideColor(SColor(101, 255, 255, 255)), BGColor(SColor(101, 210, 210, 210)),
      ...
      if (!WordWrap) {
        if (VAlign == EGUIA_LOWERRIGHT) {
          frameRect.UpperLeftCorner.Y = frameRect.LowerRightCorner.Y -
              font->getDimension("A").Height - font->getKerningHeight();
        }
        ...
        IRECT r = frameRect;
        s32 height = font->getDimension("A").Height + font->getKerningHeight();
        s32 totalHeight = height * BrokenText_size;
...
void CGUIStaticText::setOverrideColor(SColor color)
{
  OverrideColor = color;
  OverrideColorEnabled = true;
}
//! Sets another color for the text.
void CGUIStaticText::setBackgroundColor(SColor color)
{
  BGColor = color;
...
SColor CGUIStaticText::getBackgroundColor() const
{
  return BGColor;
}
...
SColor CGUIStaticText::getOverrideColor() const
{
  ...
  if (!RightToLeft) {
    ...
    for (s32 i = 0; i < size; ++i) {
      ...
      if (isWhitespace || i == (size - 1)) {
        if (word_size) {
          ...
          const s32 whitelgth = font->getDimension(whitespace).Width;
          const s32 wordlgth = font->getDimension(word).Width;
          if (wordlgth > elWidth) {
            // This word is too long to fit in the available space, look for
            // the Unicode Soft HYphen (SHY / 00AD) character for a place to
            // break the word at
            int where = word.findFirst(char(0x00AD));
            if (where != -1) {
              char* first  = word.subString(0, where);
              char* second = word.subString(where, word_size - where);
    ...
    for (s32 i = size; i >= 0; --i) {
      ...
      if (c == ' ' || c == 0 || i == 0) {
        ...
        if (c != 0) {
          whitespace = char * (&c, 1) + whitespace;
        }
        // compute line break
        if (lineBreak) {
          line = whitespace + line;
...
void CGUIStaticText::setText(const char* text)
{
...
s32 CGUIStaticText::getTextHeight() const
{
  ...
  if (WordWrap) {
    ...
    return font->getDimension(Text).Height;
  }
}
s32 CGUIStaticText::getTextWidth() const
{
  ...
  if (WordWrap) {
    s32 widest = 0;
    for (int line = 0; line < BrokenText_size; ++line) {
...
void CGUIStaticText::serializeAttributes(IAttributes* out, SAttributeReadWriteOptions* options = 0) const
{
...
void CGUIStaticText::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options = 0)
{
