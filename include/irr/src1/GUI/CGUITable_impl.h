...
CGUITable::CGUITable(IGUIEnvironment* environment, IGUIElement* parent,
    s32 id, const IRECT& rectangle, bool clip,
...
#endif
  VerticalScrollBar = Environment->addScrollBar(false, IRECT(0, 0, 100, 100), this, -1);
  if (VerticalScrollBar) {
    VerticalScrollBar->grab();
    VerticalScrollBar->setNotClipped(false);
    VerticalScrollBar->setSubElement(true);
  }
  HorizontalScrollBar = Environment->addScrollBar(true, IRECT(0, 0, 100, 100), this, -1);
...
void CGUITable::addColumn(const char* caption, s32 columnIndex)
{
  Column tabHeader;
  ...
  if (columnIndex < 0 || columnIndex >= (s32)Columns_size) {
...
u32 CGUITable::addRow(u32 rowIndex)
{
  ...
  Rows[rowIndex].Items.reallocate(Columns_size);
  int i;
...
void CGUITable::removeRow(u32 rowIndex)
{
  ...
  Rows.erase(rowIndex);
  if (!(Selected < s32(Rows_size))) {
    Selected = Rows_size - 1;
  }
  recalculateHeights();
}
//! adds an list item, returns id of item
void CGUITable::setCellText(u32 rowIndex, u32 columnIndex, const char* text)
{
  if (rowIndex < Rows_size && columnIndex < Columns_size) {
...
void CGUITable::setCellText(u32 rowIndex, u32 columnIndex, const char* text, SColor color)
{
  if (rowIndex < Rows_size && columnIndex < Columns_size) {
    Rows[rowIndex].Items[columnIndex].Text = text;
    breakText(Rows[rowIndex].Items[columnIndex].Text, Rows[rowIndex].Items[columnIndex].BrokenText, Columns[columnIndex].Width);
...
void CGUITable::setCellColor(u32 rowIndex, u32 columnIndex, SColor color)
{
  if (rowIndex < Rows_size && columnIndex < Columns_size) {
    Rows[rowIndex].Items[columnIndex].Color = color;
    Rows[rowIndex].Items[columnIndex].IsOverrideColor = true;
...
const char* CGUITable::getCellText(u32 rowIndex, u32 columnIndex) const
{
  if (rowIndex < Rows_size && columnIndex < Columns_size) {
    return Rows[rowIndex].Items[columnIndex].Text;
  }
  return 0;
}
...
{
  Selected = -1;
  if (index >= 0 && index < (s32) Rows_size) {
    Selected = index;
  }
}
...
void CGUITable::recalculateHeights()
{
  ...
  if (Font != skin->getFont()) {
    ...
    ItemHeight = 0;
    if (Font) {
      ItemHeight = Font->getDimension("A").Height + (CellHeightPadding * 2);
      Font->grab();
    }
  }
  TotalItemHeight = ItemHeight * Rows_size;   //  header is not counted, because we only want items
  checkScrollbars();
}
...
void CGUITable::checkScrollbars()
{
  ...
  if (TotalItemWidth > clientClip.getWidth()) {
    ...
    HorizontalScrollBar->setMax(max_((int)0, (int)(TotalItemWidth - clientClip.getWidth())));
  }
  // needs vertical scroll be visible?
  if (TotalItemHeight > clientClip.getHeight()) {
    clientClip.LowerRightCorner.X -= scrollBarSize;
    VerticalScrollBar->setVisible(true);
    VerticalScrollBar->setMax(max_((int)0, (int)(TotalItemHeight - clientClip.getHeight())));
  ...
  if (VerticalScrollBar->isVisible()) {
    ...
    if (HorizontalScrollBar->isVisible()) {
      VerticalScrollBar->setRelativePosition(
          IRECT(RelativeRect.getWidth() - scrollBarSize, 1,
  ...
  if (HorizontalScrollBar->isVisible()) {
    ...
    if (VerticalScrollBar->isVisible()) {
      HorizontalScrollBar->setRelativePosition(IRECT(1, RelativeRect.getHeight() - scrollBarSize, RelativeRect.getWidth() - (1 + scrollBarSize), RelativeRect.getHeight() - 1));
    }
    else {
      HorizontalScrollBar->setRelativePosition(IRECT(1, RelativeRect.getHeight() - scrollBarSize, RelativeRect.getWidth() - 1, RelativeRect.getHeight() - 1));
    }
  }
}
...
bool CGUITable::OnEvent(const SEvent& event)
{
  if (isEnabled()) {
    ...
    case EET_MOUSE_INPUT_EVENT: {
      ...
      position2di p(event.MouseInput.X, event.MouseInput.Y);
...
{
  if (rowIndexA >= Rows_size) {
    return;
  }
  if (rowIndexB >= Rows_size) {
    return;
  }
...
bool CGUITable::dragColumnStart(s32 xpos, s32 ypos)
{
  ...
  for (s32 i = (s32)Columns_size - 1; i >= 0 ; --i) {
...
{
  if (!ResizableColumns || CurrentResizedColumn < 0 || CurrentResizedColumn >= s32(Columns_size)) {
    CurrentResizedColumn = -1;
    return false;
  }
...
void CGUITable::orderRows(s32 columnIndex, EGUI_ORDERING_MODE mode)
{
  ...
  if (mode == EGOM_ASCENDING) {
    for (s32 i = 0 ; i < s32(Rows_size) - 1 ; ++i) {
      for (s32 j = 0 ; j < s32(Rows_size) - i - 1 ; ++j) {
...
void CGUITable::draw()
{
  ...
  IRECT scrolledTableClient(tableRect);
  ...
  IRECT rowRect(scrolledTableClient);
  ...
  IRECT columnSeparator(clientClip);
  pos = scrolledTableClient.UpperLeftCorner.X;
  IRECT tableClip(tableRect);
  tableClip.clipAgainst(AbsoluteClippingRect);
  for (i = 0 ; i < Columns_size ; ++i) {
    const char* text = Columns[i].Name;
    u32 colWidth = Columns[i].Width;
    //dimension2d<s32 > dim = font->getDimension(text);
    IRECT columnrect(pos, tableRect.UpperLeftCorner.Y, pos + colWidth, headerBottom);
  ...
  IRECT columnrect(pos, tableRect.UpperLeftCorner.Y, tableRect.LowerRightCorner.X , headerBottom);
  skin->draw3DButtonPaneStandard(this, columnrect, &tableClip);
  IGUIElement::draw();
}
void CGUITable::breakText(const char* text, char* brokenText, u32 cellWidth)
{
  IGUISkin* skin = Environment->getSkin();
  if (!skin) {
    return;
  ...
  const u32 maxLengthDots = cellWidth - (CellWidthPadding * 2) - font->getDimension("...").Width;
  const u32 size = text_size;
  u32 pos = 0;
  u32 i;
  for (i = 0; i < size; ++i) {
    c[0] = text[i];
...
void CGUITable::serializeAttributes(IAttributes* out, SAttributeReadWriteOptions* options) const
{
  ...
  out->addInt("ColumnCount", Columns_size);
  int i;
  for (i = 0; i < Columns_size; ++i) {
    char* label;
    label = "Column";
    label += i;
    label += "name";
    out->addString(label, Columns[i].Name);
    label = "Column";
    label += i;
    label += "width";
    out->addInt(label, Columns[i].Width);
    label = "Column";
    label += i;
    label += "OrderingMode";
    out->addEnum(label, Columns[i].OrderingMode, GUIColumnOrderingNames);
  }
  out->addInt("RowCount", Rows_size);
  for (i = 0; i < Rows_size; ++i) {
    char* label;
    // Height currently not used and could be recalculated anyway
    //label = "Row"; label += i; label += "height";
    //out->addInt(label, Rows[i].Height );
    //label = "Row"; label += i; label += "ItemCount";
    //out->addInt(label, Rows[i].Items_size);
    int c;
    for (c = 0; c < Rows[i].Items_size; ++c) {
      label = "Row";
      label += i;
      label += "cell";
      label += c;
      label += "text";
      out->addString(label, Rows[i].Items[c].Text);
      // char* BrokenText;  // can be recalculated
      label = "Row";
      label += i;
      label += "cell";
      label += c;
      label += "color";
      out->addColor(label, Rows[i].Items[c].Color);
      label = "Row";
      label += i;
      label += "cell";
      label += c;
      label += "IsOverrideColor";
      out->addColor(label, Rows[i].Items[c].IsOverrideColor);
...
void CGUITable::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options)
{
  ...
  for (i = 0; i < columnCount; ++i) {
    ...
    label += i;
    label += "name";
    column.Name = char * (in->getAttributeAsString(label));
    label = "Column";
    label += i;
    label += "width";
    column.Width = in->getAttributeAsInt(label);
    label = "Column";
    label += i;
    label += "OrderingMode";
    column.OrderingMode = EGCO_NONE;
    s32 co = in->getAttributeAsEnumeration(label, GUIColumnOrderingNames);
    if (co > 0) {
  ...
  for (i = 0; i < rowCount; ++i) {
    ...
    // Height currently not used and could be recalculated anyway
    //label = "Row"; label += i; label += "height";
    //row.Height = in->getAttributeAsInt(label );
    Rows.push_back(row);
    //label = "Row"; label += i; label += "ItemCount";
    //u32 itemCount = in->getAttributeAsInt(label);
    int c;
    for (c = 0; c < columnCount; ++c) {
      ...
      label += c;
      label += "text";
      cell.Text = char * (in->getAttributeAsString(label));
      ...
      label += c;
      label += "color";
      cell.Color = in->getAttributeAsColor(label);
      label = "Row";
      label += i;
      label += "cell";
      label += c;
      label += "IsOverrideColor";
      cell.IsOverrideColor = in->getAttributeAsBool(label);
      cell.Data = NULL;
      Rows[Rows_size - 1].Items.push_back(cell);
    }
  }
