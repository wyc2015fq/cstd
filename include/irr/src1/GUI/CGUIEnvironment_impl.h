...
//#include "os.h"
const char* IRR_XML_FORMAT_GUI_ENV     = "irr_gui";
const char* IRR_XML_FORMAT_GUI_ELEMENT   = "element";
const char* IRR_XML_FORMAT_GUI_ELEMENT_ATTR_TYPE = "type";
const char* CGUIEnvironment::DefaultFontName = "#DefaultFont";
//! constructor
CGUIEnvironment::CGUIEnvironment(IFileSystem* fs, IVideoDriver* driver, IOSOperator* op)
  : IGUIElement(EGUIET_ROOT, 0, 0, 0, IRECT(position2di(0, 0), driver ? dimension2d<s32>(driver->getScreenSize()) : dimension2d<s32>(0, 0))),
...
CGUIEnvironment::~CGUIEnvironment()
{
  ...
  for (i = 0; i < GUIElementFactoryList_size; ++i) {
...
void CGUIEnvironment::clear()
{
  ...
  const list<IGUIElement*>& children = getRootGUIElement()->getChildren();
...
void CGUIEnvironment::OnPostRender(u32 time)
{
  ...
  if (ToolTip.Element == 0 &&
    ...
    dimension2du dim = getSkin()->getFont(EGDF_TOOLTIP)->getDimension(HoveredNoSubelement->getToolTipText());
...
//
void CGUIEnvironment::updateHoveredElement(position2di mousePos)
{
...
bool CGUIEnvironment::postEventFromUser(const SEvent& event)
{
  ...
  case EET_MOUSE_INPUT_EVENT:
    updateHoveredElement(position2di(event.MouseInput.X, event.MouseInput.Y));
...
IGUIElement* CGUIEnvironment::addGUIElement(const c8* elementName, IGUIElement* parent)
{
  ...
  for (s32 i = GUIElementFactoryList_size - 1; i >= 0 && !node; --i) {
...
bool CGUIEnvironment::saveGUI(const char* filename, IGUIElement* start)
{
...
//! \param filename: Name of the file.
bool CGUIEnvironment::loadGUI(const char* filename, IGUIElement* parent)
{
...
void CGUIEnvironment::readGUIElement(IXMLReader* reader, IGUIElement* node)
{
  ...
  if (!strcmp(IRR_XML_FORMAT_GUI_ENV, reader->getNodeName())) {
  ...
  else if (!strcmp(IRR_XML_FORMAT_GUI_ELEMENT, reader->getNodeName())) {
    // find node type and create it
    const char* attrName = reader->getAttributeValue(IRR_XML_FORMAT_GUI_ELEMENT_ATTR_TYPE);
    node = addGUIElement(attrName, node);
    if (!node) {
      os::Printer::log("Could not create GUI element of unknown type", attrName);
    }
  }
  // read attributes
  while (reader->read()) {
    ...
    case EXN_ELEMENT_END:
      if (!strcmp(IRR_XML_FORMAT_GUI_ELEMENT,  reader->getNodeName()) ||
          !strcmp(IRR_XML_FORMAT_GUI_ENV, reader->getNodeName())) {
        endreached = true;
      }
      break;
    case EXN_ELEMENT:
      if (!strcmp("attributes", reader->getNodeName())) {
        // read attributes
...
void CGUIEnvironment::writeGUIElement(IXMLWriter* writer, IGUIElement* node)
{
  ...
  // write children
  list<IGUIElement*>::ConstIterator it = node->getChildren().begin();
...
void CGUIEnvironment::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options)
{
  ...
  RelativeRect = AbsoluteRect =
      IRECT(position2di(0, 0),
          Driver ? dimension2di(Driver->getScreenSize()) : dimension2d<s32>(0, 0));
}
//! adds a button. The returned pointer must not be dropped.
IGUIButton* CGUIEnvironment::addButton(const IRECT& rectangle, IGUIElement* parent, s32 id, const char* text, const char* tooltiptext)
{
...
IGUIWindow* CGUIEnvironment::addWindow(const IRECT& rectangle, bool modal,
    const char* text, IGUIElement* parent, s32 id)
{
  ...
  if (modal) {
    ...
    // careful not to get function call, like OnEvent, in the window.
...
//! Adds a message box.
IGUIWindow* CGUIEnvironment::addMessageBox(const char* caption, const char* text,
    bool modal, s32 flag, IGUIElement* parent, s32 id, ITexture* image)
{
  if (!CurrentSkin) {
    return 0;
  }
  parent = parent ? parent : this;
  IRECT rect;
  dimension2du screenDim, msgBoxDim;
  ...
  if (modal) {
    // Careful, don't just set the modal as parent above. That will mess up the focus (and is hard to change because we have to be very
    // careful not to get function call, like OnEvent, in the CGUIMessageBox.
    CGUIModalScreen* modalScreen = new CGUIModalScreen(this, parent, -1);
    modalScreen->drop();
    modalScreen->addChild(win);
  }
...
IGUIScrollBar* CGUIEnvironment::addScrollBar(bool horizontal, const IRECT& rectangle, IGUIElement* parent, s32 id)
{
...
IGUITable* CGUIEnvironment::addTable(const IRECT& rectangle, IGUIElement* parent, s32 id, bool drawBackground)
{
...
IGUIImage* CGUIEnvironment::addImage(ITexture* image, position2di pos,
    bool useAlphaChannel, IGUIElement* parent, s32 id, const char* text)
{
  dimension2d<s32> sz(0, 0);
  if (image) {
    sz = dimension2d<s32>(image->getOriginalSize());
  }
...
IGUIImage* CGUIEnvironment::addImage(const IRECT& rectangle, IGUIElement* parent, s32 id, const char* text, bool useAlphaChannel)
{
  IGUIImage* img = new CGUIImage(this, parent ? parent : this,
      id, rectangle);
  if (text) {
...
IGUIMeshViewer* CGUIEnvironment::addMeshViewer(const IRECT& rectangle, IGUIElement* parent, s32 id, const char* text)
{
...
//! adds a checkbox
IGUICheckBox* CGUIEnvironment::addCheckBox(bool checked, const IRECT& rectangle, IGUIElement* parent, s32 id, const char* text)
{
...
//! adds a list box
IGUIListBox* CGUIEnvironment::addListBox(const IRECT& rectangle,
...
//! adds a tree view
IGUITreeView* CGUIEnvironment::addTreeView(const IRECT& rectangle,
...
IGUIFileOpenDialog* CGUIEnvironment::addFileOpenDialog(const char* title,
...
IGUIColorSelectDialog* CGUIEnvironment::addColorSelectDialog(const char* title,
    bool modal, IGUIElement* parent, s32 id)
{
  parent = parent ? parent : this;
...
IGUIStaticText* CGUIEnvironment::addStaticText(const char* text,
    const IRECT& rectangle,
...
IGUIEditBox* CGUIEnvironment::addEditBox(const char* text,
    const IRECT& rectangle, bool border,
    IGUIElement* parent, s32 id)
{
...
IGUISpinBox* CGUIEnvironment::addSpinBox(const char* text,
    const IRECT& rectangle,
...
IGUITabControl* CGUIEnvironment::addTabControl(const IRECT& rectangle,
...
IGUITab* CGUIEnvironment::addTab(const IRECT& rectangle,
    IGUIElement* parent, s32 id)
{
...
IGUIContextMenu* CGUIEnvironment::addContextMenu(const IRECT& rectangle,
    IGUIElement* parent, s32 id)
{
...
IGUIToolBar* CGUIEnvironment::addToolBar(IGUIElement* parent, s32 id)
{
  ...
  IGUIToolBar* b = new CGUIToolBar(this, parent, id, IRECT(0, 0, 10, 10));
  b->drop();
  return b;
}
//! Adds an element for fading in or out.
IGUIInOutFader* CGUIEnvironment::addInOutFader(const IRECT* rectangle, IGUIElement* parent, s32 id)
{
  IRECT rect;
  if (rectangle) {
    rect = *rectangle;
    ...
    rect = IRECT(position2di(0, 0), dimension2di(Driver->getScreenSize()));
  }
  if (!parent) {
    parent = this;
  }
...
IGUIComboBox* CGUIEnvironment::addComboBox(const IRECT& rectangle,
    IGUIElement* parent, s32 id)
{
  ...
  if (xml) {
    ...
    while (!found && xml->read()) {
      if (xml->getNodeType() == EXN_ELEMENT) {
        if (char * ("font") == xml->getNodeName()) {
          if (char * ("vector") == xml->getAttributeValue("type")) {
            t = EGFT_VECTOR;
            found = true;
          }
          else if (char * ("bitmap") == xml->getAttributeValue("type")) {
            t = EGFT_BITMAP;
            found = true;
          }
          else {
    ...
    if (t == EGFT_BITMAP) {
      ...
      char* workingDir = FileSystem->getWorkingDirectory();
...
IGUIFont* CGUIEnvironment::addFont(const char* name, IGUIFont* font)
{
  if (font) {
    SFont f;
...
IGUISpriteBank* CGUIEnvironment::getSpriteBank(const char* filename)
{
...
IGUISpriteBank* CGUIEnvironment::addEmptySpriteBank(const char* name)
{
