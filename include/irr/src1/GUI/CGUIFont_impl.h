...
//! constructor
CGUIFont::CGUIFont(IGUIEnvironment* env, const char* filename)
...
bool CGUIFont::load(IXMLReader* xml)
{
  ...
  while (xml->read()) {
    if (EXN_ELEMENT == xml->getNodeType()) {
      if (char * ("Texture") == xml->getNodeName()) {
        // add a texture
        char* fn = xml->getAttributeValue("filename");
        u32 i = (u32)xml->getAttributeValueAsInt("index");
        char* alpha = xml->getAttributeValue("hasAlpha");
        ...
        if (!SpriteBank->getTexture(i)) {
          ...
          if (alpha == char * ("false")) {
            Driver->makeColorKeyTexture(SpriteBank->getTexture(i), position2di(0, 0));
          }
        }
      }
      else if (char * ("c") == xml->getNodeName()) {
        // adding a character to this font
        SFontArea a;
        SGUISpriteFrame f;
        SGUISprite s;
        IRECT rectangle;
        a.underhang   = xml->getAttributeValueAsInt("u");
        a.overhang    = xml->getAttributeValueAsInt("o");
        a.spriteno    = SpriteBank->getSprites().size();
        s32 texno   = xml->getAttributeValueAsInt("i");
        // parse rectangle
        char* rectstr = xml->getAttributeValue("r");
        char ch    = xml->getAttributeValue("c")[0];
        const c8* c = rectstr;
        s32 val;
        val = 0;
        while (*c >= '0' && *c <= '9') {
        ...
        CharacterMap.insert(ch, Areas_size);
        // make frame
  ...
  // set bad character
  WrongCharacter = getAreaFromCharacter(' ');
  setMaxHeight();
  return true;
}
void CGUIFont::setMaxHeight()
{
  ...
  MaxHeight = 0;
  s32 t;
  IRECT > & p = SpriteBank->getPositions();
  int i;
  for (i = 0; i < p_size; ++i) {
    t = p[i].getHeight();
    if (t > MaxHeight) {
...
bool CGUIFont::loadTexture(IImage* image, const char* name)
{
  if (!image || !SpriteBank) {
    return false;
  }
...
void CGUIFont::readPositions(IImage* image, s32& lowerRightPositions)
{
  if (!SpriteBank) {
    return;
  }
  const dimension2du size = image->getDimension();
  SColor colorTopLeft = image->getPixel(0, 0);
  colorTopLeft.setAlpha(255);
  image->setPixel(0, 0, colorTopLeft);
  SColor colorLowerRight = image->getPixel(1, 0);
  SColor colorBackGround = image->getPixel(2, 0);
  SColor colorBackGroundTransparent = 0;
  image->setPixel(1, 0, colorBackGround);
  // start parsing
  position2di pos(0, 0);
  for (pos.Y = 0; pos.Y < (s32)size.Height; ++pos.Y) {
    for (pos.X = 0; pos.X < (s32)size.Width; ++pos.X) {
      const SColor c = image->getPixel(pos.X, pos.Y);
      if (c == colorTopLeft) {
        image->setPixel(pos.X, pos.Y, colorBackGroundTransparent);
        SpriteBank->getPositions().push_back(IRECT(pos, pos));
      }
        ...
        int ch = (int)(lowerRightPositions + 32);
  ...
  int idx = getAreaFromCharacter('F');
  int idx1 = getAreaFromCharacter('F');
}
...
//! set an Pixel Offset on Drawing ( scale position on width )
s32 CGUIFont::getKerningWidth(const char* thisLetter, const char* previousLetter) const
{
...
u32 CGUIFont::getSpriteNoFromChar(const char* c) const
{
  return Areas[getAreaFromCharacter(*c)].spriteno;
}
s32 CGUIFont::getAreaFromCharacter(const int c) const
{
  map<int, s32>::Node* n = CharacterMap.find(c);
  if (n) {
    return n->getValue();
  }
  else {
...
void CGUIFont::setInvisibleCharacters(const char* s)
{
  Invisible = s;
}
//! returns the dimension of text
dimension2du CGUIFont::getDimension(const char* text) const
{
  dimension2du dim(0, 0);
  dimension2du thisLine(0, MaxHeight);
  for (const char* p = text; *p; ++p) {
    bool lineBreak = false;
    if (*p == '\r') { // Mac or Windows breaks
      lineBreak = true;
      if (p[1] == '\n') { // Windows breaks
        ++p;
      }
...
void CGUIFont::draw(const char* text, const IRECT& position,
    SColor color,
    bool hcenter, bool vcenter, const IRECT* clip
                   )
{
  int idx = getAreaFromCharacter('F');
  ...
  dimension2d<s32> textDimension; // NOTE: don't make this u32 or the >> later on can fail when the dimension width is < position width
  position2di offset = position.UpperLeftCorner;
  ...
  if (clip) {
    IRECT clippedRect(offset, textDimension);
    clippedRect.clipAgainst(*clip);
    if (!clippedRect.isValid()) {
      return;
    }
  }
  u32* indices(text_size);
  position2di > offsets(text_size);
  int nc = text_size;
  const char* ws = text;
  int i;
  for (i = 0; i < nc; i++) {
    ...
    int idx = getAreaFromCharacter(c);
    SFontArea& area = Areas[idx];
...
s32 CGUIFont::getCharacterFromPos(const char* text, s32 pixel_x) const
{
  s32 x = 0;
  s32 idx = 0;
  while (text[idx]) {
