...
bool CGUIScrollBar::OnEvent(const SEvent& event)
{
  if (isEnabled()) {
    ...
    case EET_MOUSE_INPUT_EVENT: {
      const position2di p(event.MouseInput.X, event.MouseInput.Y);
...
void CGUIScrollBar::draw()
{
  ...
  SColor iconColor = skin->getColor(isEnabled() ? EGDC_WINDOW_SYMBOL : EGDC_GRAY_WINDOW_SYMBOL);
...
//!
s32 CGUIScrollBar::getPosFromMousePos(const position2di& pos) const
{
  f32 w, p;
  if (Horizontal) {
...
{
  CurrentIconColor = SColor(255, 255, 255, 255);
  IGUISkin* skin = Environment->getSkin();
  ...
  if (Horizontal) {
    ...
    if (!UpButton) {
      UpButton = new CGUIButton(Environment, this, -1, IRECT(0, 0, h, h), NoClip);
    ...
    UpButton->setRelativePosition(IRECT(0, 0, h, h));
    UpButton->setAlignment(EGUIA_UPPERLEFT, EGUIA_UPPERLEFT, EGUIA_UPPERLEFT, EGUIA_LOWERRIGHT);
    if (!DownButton) {
      DownButton = new CGUIButton(Environment, this, -1, IRECT(RelativeRect.getWidth() - h, 0, RelativeRect.getWidth(), h), NoClip);
    ...
    DownButton->setRelativePosition(IRECT(RelativeRect.getWidth() - h, 0, RelativeRect.getWidth(), h));
    DownButton->setAlignment(EGUIA_LOWERRIGHT, EGUIA_LOWERRIGHT, EGUIA_UPPERLEFT, EGUIA_LOWERRIGHT);
  }
  else {
    s32 w = RelativeRect.getWidth();
    if (!UpButton) {
      UpButton = new CGUIButton(Environment, this, -1, IRECT(0, 0, w, w), NoClip);
      UpButton->setSubElement(true);
      UpButton->setTabStop(false);
    }
    if (sprites) {
    ...
    UpButton->setRelativePosition(IRECT(0, 0, w, w));
    UpButton->setAlignment(EGUIA_UPPERLEFT, EGUIA_LOWERRIGHT, EGUIA_UPPERLEFT, EGUIA_UPPERLEFT);
    if (!DownButton) {
      DownButton = new CGUIButton(Environment, this, -1, IRECT(0, RelativeRect.getHeight() - w, w, RelativeRect.getHeight()), NoClip);
      DownButton->setSubElement(true);
      DownButton->setTabStop(false);
    }
    if (sprites) {
    ...
    DownButton->setRelativePosition(IRECT(0, RelativeRect.getHeight() - w, w, RelativeRect.getHeight()));
