...
void CGUITreeViewNode::setText(const char* text)
{
  Text = text;
}
void CGUITreeViewNode::setIcon(const char* icon)
{
  Icon = icon;
}
    ...
    ImageLeftOfIcon(true)
{
  ...
  if (scrollBarHorizontal) {
    ScrollBarH = new CGUIScrollBar(true, Environment, this, 1,
        IRECT(0, RelativeRect.getHeight() - s, RelativeRect.getWidth() - s, RelativeRect.getHeight()),
        !clip);
    ScrollBarH->drop();
...
void CGUITreeView::recalculateItemHeight()
{
  ...
  if (Font != skin->getFont()) {
    ...
    if (IconFont) {
      s32 height = IconFont->getDimension(" ").Height;
  ...
  if (ScrollBarV) {
    ScrollBarV->setMax(max_((int)0, (int)(TotalItemHeight - AbsoluteRect.getHeight())));
  }
  if (ScrollBarH) {
    ScrollBarH->setMax(max_((int)0, (int)(TotalItemWidth - AbsoluteRect.getWidth())));
  }
}
    ...
    if (frameRect.LowerRightCorner.Y >= AbsoluteRect.UpperLeftCorner.Y
      ...
      if (Font) {
        ...
        for (s32 n = 0; n < 2; ++n) {
          ...
          else if ((IconFont && reinterpret_cast<CGUITreeViewNode*>(node)->Icon_size)
              && ((ImageLeftOfIcon && n == 1)
                  || (!ImageLeftOfIcon && n == 0))) {
...
void CGUITreeView::setIconFont(IGUIFont* font)
{
  ...
  IconFont = font;
  if (IconFont) {
    height = IconFont->getDimension(" ").Height;
    if (height > ItemHeight) {
      ItemHeight = height;
    }
  }
