...
IGUIElement* CDefaultGUIElementFactory::addGUIElement(EGUI_ELEMENT_TYPE type, IGUIElement* parent)
{
  switch (type) {
  case EGUIET_BUTTON:
    return Environment->addButton(IRECT(0, 0, 100, 100), parent);
  case EGUIET_CHECK_BOX:
    return Environment->addCheckBox(false, IRECT(0, 0, 100, 100), parent);
  case EGUIET_COLOR_SELECT_DIALOG:
    return Environment->addColorSelectDialog(0, true, parent);
  case EGUIET_COMBO_BOX:
    return Environment->addComboBox(IRECT(0, 0, 100, 100), parent);
  case EGUIET_CONTEXT_MENU:
    return Environment->addContextMenu(IRECT(0, 0, 100, 100), parent);
  case EGUIET_MENU:
    return Environment->addMenu(parent);
  case EGUIET_EDIT_BOX:
    return Environment->addEditBox(0, IRECT(0, 0, 100, 100), true, parent);
  case EGUIET_FILE_OPEN_DIALOG:
    return Environment->addFileOpenDialog(0, true, parent);
  case EGUIET_IMAGE:
    return Environment->addImage(0, position2di(0, 0), true, parent);
  case EGUIET_IN_OUT_FADER:
    return Environment->addInOutFader(0, parent);
  case EGUIET_LIST_BOX:
    return Environment->addListBox(IRECT(0, 0, 100, 100), parent);
  case EGUIET_MESH_VIEWER:
    return Environment->addMeshViewer(IRECT(0, 0, 100, 100), parent);
  ...
  case EGUIET_SCROLL_BAR:
    return Environment->addScrollBar(false, IRECT(0, 0, 100, 100), parent);
  case EGUIET_STATIC_TEXT:
    return Environment->addStaticText(0, IRECT(0, 0, 100, 100), false, true, parent);
  case EGUIET_TAB:
    return Environment->addTab(IRECT(0, 0, 100, 100), parent);
  case EGUIET_TAB_CONTROL:
    return Environment->addTabControl(IRECT(0, 0, 100, 100), parent);
  case EGUIET_TABLE:
    return Environment->addTable(IRECT(0, 0, 100, 100), parent);
  case EGUIET_TOOL_BAR:
    return Environment->addToolBar(parent);
  case EGUIET_WINDOW:
    return Environment->addWindow(IRECT(0, 0, 100, 100), false, 0, parent);
  case EGUIET_SPIN_BOX:
    return Environment->addSpinBox("0.0", IRECT(0, 0, 100, 100), true, parent);
  case EGUIET_TREE_VIEW:
    return Environment->addTreeView(IRECT(0, 0, 100, 100), parent);
  default:
    return 0;
  }
}
...
{
  int i;
  for (i = 0; GUIElementTypeNames[i]; ++i)
