...
{
  IRECT clientRect; // client rect of parent
  if (Parent && Parent->hasType(EGUIET_WINDOW)) {
    clientRect = static_cast<IGUIWindow*>(Parent)->getClientRect();
  }
  else if (Parent) {
    clientRect = IRECT(0, 0, Parent->getAbsolutePosition().getWidth(),
  ...
  if (!font) {
    if (Parent && skin)
      RelativeRect = IRECT(clientRect.UpperLeftCorner.X, clientRect.UpperLeftCorner.Y,
  ...
  s32 height = font->getDimension("A").Height + 5;
  ...
  for (i = 0; i < (s32)Items_size; ++i)
    if (Items[i].SubMenu) {
      ...
      Items[i].SubMenu->setRelativePosition(
          IRECT(Items[i].PosY, height ,
              Items[i].PosY + w - 5, height + h));
    }
}
//! returns the item highlight-area
IRECT CGUIMenu::getHRect(const SItem& i, const IRECT& absolute) const
{
  IRECT r = absolute;
...
IRECT CGUIMenu::getRect(const SItem& i, const IRECT& absolute) const
{
