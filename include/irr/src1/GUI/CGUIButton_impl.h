...
CGUIButton::CGUIButton(IGUIEnvironment* environment, IGUIElement* parent,
    s32 id, IRECT rectangle, bool noclip)
...
void CGUIButton::setSprite(EGUI_BUTTON_STATE state, s32 index, SColor color, bool loop)
{
  if (SpriteBank) {
  ...
  case EET_MOUSE_INPUT_EVENT:
    if (event.MouseInput.Event == EMIE_LMOUSE_PRESSED_DOWN) {
      if (Environment->hasFocus(this) &&
          !AbsoluteClippingRect.isPointInside(position2di(event.MouseInput.X, event.MouseInput.Y))) {
...
void CGUIButton::draw()
{
  ...
  const position2di spritePos = AbsoluteRect.getCenter();
  if (!Pressed) {
    if (DrawBorder) {
      skin->draw3DButtonPaneStandard(this, AbsoluteRect, &AbsoluteClippingRect);
    }
    if (Image) {
      position2di pos = spritePos;
...
void CGUIButton::setImage(ITexture* image)
{
  ...
  Image = image;
  if (image) {
    ImageRect = IRECT(position2di(0, 0), image->getOriginalSize());
  }
  if (!PressedImage) {
...
void CGUIButton::setImage(ITexture* image, const IRECT& pos)
{
  setImage(image);
  ImageRect = pos;
}
...
void CGUIButton::setPressedImage(ITexture* image)
{
  ...
  PressedImage = image;
  if (image) {
    PressedImageRect = IRECT(position2di(0, 0), image->getOriginalSize());
  }
}
//! Sets the image which should be displayed on the button when it is in its pressed state.
void CGUIButton::setPressedImage(ITexture* image, const IRECT& pos)
{
  setPressedImage(image);
...
void CGUIButton::serializeAttributes(IAttributes* out, SAttributeReadWriteOptions* options = 0) const
{
...
void CGUIButton::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options = 0)
{
  ...
  IRECT rec = in->getAttributeAsRect("ImageRect");
  if (rec.isValid()) {
