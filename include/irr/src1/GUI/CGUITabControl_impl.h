...
void CGUITab::setBackgroundColor(SColor c)
{
  BackColor = c;
}
//! sets the color of the text
void CGUITab::setTextColor(SColor c)
{
  OverrideTextColorEnabled = true;
  TextColor = c;
}
SColor CGUITab::getTextColor() const
{
  return TextColor;
}
...
SColor CGUITab::getBackgroundColor() const
{
  return BackColor;
}
    ...
    UpButton(0), DownButton(0), TabMaxWidth(0), CurrentScrollTabIndex(0), TabExtraWidth(20)
{
  ...
  UpButton = Environment->addButton(IRECT(0, 0, 10, 10), this);
  if (UpButton) {
  ...
  DownButton = Environment->addButton(IRECT(0, 0, 10, 10), this);
  if (DownButton) {
...
//! Adds a tab
IGUITab* CGUITabControl::addTab(const char* caption, s32 id)
{
  CGUITab* tab = new CGUITab(Tabs_size, Environment, this, calcTabPos(), id);
  tab->setText(caption);
...
void CGUITabControl::addTab(CGUITab* tab)
{
  ...
  tab->grab();
  if (tab->getNumber() == -1) {
    tab->setNumber((s32)Tabs_size);
  }
  while (tab->getNumber() >= (s32)Tabs_size) {
    Tabs.push_back(0);
  }
  if (Tabs[tab->getNumber()]) {
    Tabs.push_back(Tabs[tab->getNumber()]);
    Tabs[Tabs_size - 1]->setNumber(Tabs_size);
  }
...
IGUITab* CGUITabControl::insertTab(s32 idx, const char* caption, s32 id)
{
  if (idx < 0 || idx > (s32)Tabs_size) { // idx == Tabs_size is indeed ok here as array can handle that
    return NULL;
  }
...
{
  if (idx < 0 || idx >= (s32)Tabs_size) {
    return;
  }
  Tabs[(u32)idx]->drop();
  Tabs.erase((u32)idx);
  int i;
  for (i = (u32)idx; i < Tabs_size; ++i) {
    Tabs[i]->setNumber(i);
  }
}
...
{
  if (CurrentScrollTabIndex < (s32)(Tabs_size) - 1) {
...
s32 CGUITabControl::calcTabWidth(s32 pos, IGUIFont* font, const char* text, bool withScrollControl) const
{
  if (!font) {
    return 0;
  }
  ...
  if (withScrollControl && ScrollControl && pos + len > UpButton->getAbsolutePosition().UpperLeftCorner.X - 2) {
    s32 tabMinWidth = font->getDimension("A").Width;
...
bool CGUITabControl::needScrollControl(s32 startIndex, bool withScrollControl)
{
  ...
  for (s32 i = startIndex; i < (s32)Tabs_size; ++i) {
    // get Text
    const char* text = 0;
    if (Tabs[i]) {
      text = Tabs[i]->getText();
...
IRECT CGUITabControl::calcTabPos()
{
  IRECT r;
...
void CGUITabControl::draw()
{
  ...
  CGUITab* activeTab = 0;
  int i;
  for (i = CurrentScrollTabIndex; i < Tabs_size; ++i) {
    // get Text
    const char* text = 0;
    if (Tabs[i]) {
      text = Tabs[i]->getText();
    ...
    if ((s32)i == ActiveTab) {
      ...
      // draw text
      IRECT textClipRect(frameRect);  // TODO: exact size depends on borders in draw3DTabButton which we don't get with current interface
...
void CGUITabControl::recalculateScrollButtonPlacement()
{
  ...
  UpButton->setRelativePosition(IRECT(ButtonX, ButtonY, ButtonX + ButtonSize, ButtonY + ButtonHeight));
  ButtonX += ButtonSize + 1;
  DownButton->setRelativePosition(IRECT(ButtonX, ButtonY, ButtonX + ButtonSize, ButtonY + ButtonHeight));
}
...
s32 CGUITabControl::getTabAt(s32 xpos, s32 ypos) const
{
  ...
  for (s32 i = CurrentScrollTabIndex; i < (s32)Tabs_size; ++i) {
    // get Text
    const char* text = 0;
    if (Tabs[i]) {
      text = Tabs[i]->getText();
