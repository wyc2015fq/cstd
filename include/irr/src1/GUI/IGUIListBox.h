...
public:
  //! constructor
  IGUIListBox(IGUIEnvironment* environment, IGUIElement* parent, s32 id, IRECT rectangle)
    : IGUIElement(EGUIET_LIST_BOX, environment, parent, id, rectangle) {}
  //! returns amount of list items
  u32 getItemCount() = 0;
  //! returns string of a list item. the may id be a value from 0 to itemCount-1
  const char* getListItem(u32 id) = 0;
  //! adds an list item, returns id of item
  u32 addItem(const char* text) = 0;
  //! adds an list item with an icon
  /** \param text Text of list entry
  \param icon Sprite index of the Icon within the current sprite bank. Set it to -1 if you want no icon
  \return The id of the new created item */
  u32 addItem(const char* text, s32 icon) = 0;
  //! Removes an item from the list
  void removeItem(u32 index) = 0;
  //! get the the id of the item at the given absolute coordinates
  /** \return The id of the listitem or -1 when no item is at those coordinates*/
  s32 getItemAt(s32 xpos, s32 ypos) = 0;
  //! Returns the icon of an item
  s32 getIcon(u32 index) = 0;
  ...
  void setSelected(const char* item) = 0;
  //! set whether the listbox should scroll to newly selected items
  void setAutoScrollEnabled(bool scroll) = 0;
  //! returns true if automatic scrolling is enabled, false if not.
  bool isAutoScrollEnabled() = 0;
  //! set all item colors at given index to color
  void setItemOverrideColor(u32 index, SColor color) = 0;
  //! set all item colors of specified type at given index to color
  void setItemOverrideColor(u32 index, EGUI_LISTBOX_COLOR colorType, SColor color) = 0;
  //! clear all item colors at index
  void clearItemOverrideColor(u32 index) = 0;
  //! clear item color at index for given colortype
  void clearItemOverrideColor(u32 index, EGUI_LISTBOX_COLOR colorType) = 0;
  //! has the item at index its color overwritten?
  bool hasItemOverrideColor(u32 index, EGUI_LISTBOX_COLOR colorType) = 0;
  //! return the overwrite color at given item index.
  SColor getItemOverrideColor(u32 index, EGUI_LISTBOX_COLOR colorType) = 0;
  //! return the default color which is used for the given colorType
  SColor getItemDefaultColor(EGUI_LISTBOX_COLOR colorType) = 0;
  //! set the item at the given index
  void setItem(u32 index, const char* text, s32 icon) = 0;
  //! Insert the item at the given index
  /** \return The index on success or -1 on failure. */
  s32 insertItem(u32 index, const char* text, s32 icon) = 0;
  //! Swap the items at the given indices
  void swapItems(u32 index1, u32 index2) = 0;
  //! set global itemHeight
  void setItemHeight(s32 height) = 0;
