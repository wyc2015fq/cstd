...
//! constructor
CGUIToolBar::CGUIToolBar(IGUIEnvironment* environment, IGUIElement* parent, s32 id, IRECT rectangle)
  : IGUIToolBar(environment, parent, id, rectangle), ButtonX(5)
{
  ...
  if (parent) {
    ...
    const list<IGUIElement*>& children = parent->getChildren();
    list<IGUIElement*>::ConstIterator it = children.begin();
    for (; it != children.end(); ++it) {
      IRECT r = (*it)->getAbsolutePosition();
  ...
  if (font)
  {
    s32 t = font->getDimension("A").Height + 5;
    if (t > height)
      height = t;
  }*/
...
void CGUIToolBar::draw()
{
  ...
  IRECT* clip = &AbsoluteClippingRect;
  // draw frame
...
IGUIButton* CGUIToolBar::addButton(s32 id, const char* text, const char* tooltiptext,
    ITexture* img, ITexture* pressed, bool isPushButton,
    bool useAlphaChannel)
{
  ButtonX += 3;
  IRECT rectangle(ButtonX, 2, ButtonX + 1, 3);
  if (img) {
  ...
  if (text) {
    ...
    if (font) {
      dimension2du dim = font->getDimension(text);
