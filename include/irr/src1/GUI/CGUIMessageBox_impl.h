...
//#include "ITexture.h"
//! constructor
CGUIMessageBox::CGUIMessageBox(IGUIEnvironment* environment, const char* caption,
    const char* text, s32 flags,
    IGUIElement* parent, s32 id, IRECT rectangle, ITexture* image)
...
void CGUIMessageBox::setButton(IGUIButton* button, bool isAvailable, const IRECT& btnRect, const char* text, IGUIElement* focusMe)
{
...
void CGUIMessageBox::refreshControls()
{
  ...
  if (!StaticText) {
    StaticText = Environment->addStaticText(MessageText, staticRect, false, false, this);
    ...
    StaticText->setText(MessageText);
  }
  ...
  if (IconTexture) {
    ...
    if (!Icon) {
      ...
      IRECT iconRect(iconPos.X, iconPos.Y, iconPos.X + IconTexture->getOriginalSize().Width, iconPos.Y + IconTexture->getOriginalSize().Height);
  ...
  if (Flags & EMBF_OK) {
    btnRect += position2di(buttonWidth + buttonDistance, 0);
  }
...
void CGUIMessageBox::serializeAttributes(IAttributes* out, SAttributeReadWriteOptions* options = 0) const
{
  ...
  out->addString("MessageText",   MessageText);
}
...
void CGUIMessageBox::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options = 0)
{
  ...
  MessageText = in->getAttributeAsString("MessageText");
