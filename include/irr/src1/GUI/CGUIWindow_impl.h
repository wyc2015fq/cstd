...
//! constructor
CGUIWindow::CGUIWindow(IGUIEnvironment* environment, IGUIElement* parent, s32 id, IRECT rectangle)
  : IGUIWindow(environment, parent, id, rectangle), Dragging(false), IsDraggable(true), DrawBackground(true), DrawTitlebar(true), IsActive(false)
{
  ...
  posx -= buttonw + 2;
  RestoreButton = Environment->addButton(IRECT(posx, 3, posx + buttonw, 3 + buttonw), this, -1,
      "", skin ? skin->getDefaultText(EGDT_WINDOW_RESTORE) : "Restore");
  ...
  posx -= buttonw + 2;
  MinButton = Environment->addButton(IRECT(posx, 3, posx + buttonw, 3 + buttonw), this, -1,
      "", skin ? skin->getDefaultText(EGDT_WINDOW_MINIMIZE) : "Minimize");
...
{
  if (! DrawBackground) {
    ClientRect = IRECT(0, 0, AbsoluteRect.getWidth(), AbsoluteRect.getHeight());
    return;
  }
  IGUISkin* skin = Environment->getSkin();
...
IRECT CGUIWindow::getClientRect() const
{
  return ClientRect;
}
//! Writes attributes of the element.
void CGUIWindow::serializeAttributes(IAttributes* out, SAttributeReadWriteOptions* options = 0) const
{
...
void CGUIWindow::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options = 0)
{
