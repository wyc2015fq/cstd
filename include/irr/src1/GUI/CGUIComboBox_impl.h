    ...
    Selected(-1), HAlign(EGUIA_UPPERLEFT), VAlign(EGUIA_CENTER), MaxSelectionRows(5), HasFocus(false)
{
  ...
  ListButton = Environment->addButton(r, this, -1, "");
  ...
  SelectedText = Environment->addStaticText("", r, false, false, this, -1, false);
...
const char* CGUIComboBox::getItem(u32 idx) const
{
  if (idx >= Items_size) {
    return 0;
  }
...
const char* CGUIComboBox::getText() const
{
  return getItem(Selected);
}
//! adds an item and returns the index of it
u32 CGUIComboBox::addItem(const char* text, u32 data)
{
...
void CGUIComboBox::setSelected(s32 idx)
{
  ...
  if (Selected == -1) {
    ...
    SelectedText->setText(Items[Selected].Name);
  }
}
    ...
    case EET_GUI_EVENT:
      ...
      case EGET_LISTBOX_CHANGED:
        if (event.GUIEvent.Caller == ListBox) {
          ...
          if (Selected < 0 || Selected >= (s32)Items_size) {
            setSelected(-1);
          }
          openCloseMenu();
          sendSelectionChangedEvent();
...
void CGUIComboBox::openCloseMenu()
{
  if (ListBox) {
    ...
    if (font) {
      h *= (font->getDimension("A").Height + 4);
    }
    // open list box
    IRECT r(0, AbsoluteRect.getHeight(),
    ...
    if (ListBox->getAbsolutePosition().LowerRightCorner.Y > Environment->getRootGUIElement()->getAbsolutePosition().getHeight()) {
      ListBox->setRelativePosition(IRECT(0, -ListBox->getAbsolutePosition().getHeight(), AbsoluteRect.getWidth(), 0));
    }
    for (s32 i = 0; i < (s32)Items_size; ++i) {
      ListBox->addItem(Items[i].Name);
    }
...
void CGUIComboBox::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options = 0)
{
  ...
  for (i = 0; i < c; ++i) {
    ...
    s += i;
    s += "Text";
    addItem(in->getAttributeAsString(s), 0);
  }
