...
//#include "os.h"
//! constructor
CGUIInOutFader::CGUIInOutFader(IGUIEnvironment* environment, IGUIElement* parent, s32 id, IRECT rectangle)
...
void CGUIInOutFader::draw()
{
  ...
  if (driver) {
    ...
    SColor newCol = FullColor.getInterpolated(TransColor, d);
    driver->draw2DRectangle(newCol, AbsoluteRect, &AbsoluteClippingRect);
  }
  IGUIElement::draw();
}
//! Gets the color to fade out to or to fade in from.
SColor CGUIInOutFader::getColor() const
{
  return Color[1];
}
//! Sets the color to fade out to or to fade in from.
void CGUIInOutFader::setColor(SColor color)
{
  SColor s = color;
  SColor d = color;
  s.setAlpha(255);
...
void CGUIInOutFader::setColor(SColor source, SColor dest)
{
  Color[0] = source;
  Color[1] = dest;
  if (Action == EFA_FADE_OUT) {
