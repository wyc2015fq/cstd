...
CGUIListBox::CGUIListBox(IGUIEnvironment* environment, IGUIElement* parent,
    s32 id, IRECT rectangle, bool clip,
    ...
    MoveOverSelect(moveOverSelect), AutoScroll(true), HighlightWhenNotFocused(true)
{
  ...
  ScrollBar = new CGUIScrollBar(false, Environment, this, -1,
      IRECT(RelativeRect.getWidth() - s, 0, RelativeRect.getWidth(), RelativeRect.getHeight()),
      !clip);
...
const char* CGUIListBox::getListItem(u32 id) const
{
  if (id >= Items_size) {
    return 0;
  }
...
u32 CGUIListBox::addItem(const char* text)
{
...
void CGUIListBox::recalculateItemHeight()
{
  ...
  if (Font != skin->getFont()) {
    ...
    if (Font) {
      if (0 == ItemHeightOverride) {
        ItemHeight = Font->getDimension("A").Height + 4;
      }
      Font->grab();
    }
  }
  TotalItemHeight = ItemHeight * Items_size;
  ScrollBar->setMax(max_((int)0, (int)(TotalItemHeight - AbsoluteRect.getHeight())));
...
//! sets the selected item. Set this to -1 if no item should be selected
void CGUIListBox::setSelected(const char* item)
{
  s32 index = -1;
  if (item) {
    for (index = 0; index < (s32) Items_size; ++index) {
  ...
  if (isEnabled()) {
    ...
    case EET_KEY_INPUT_EVENT:
      if (event.KeyInput.PressedDown &&
        ...
        if (Selected >= (s32)Items_size) {
          Selected = Items_size - 1;  // will set Selected to -1 for empty listboxes which is correct
        }
        recalculateScrollPos();
        // post the news
        ...
        if (now - LastKeyTime < 500) {
          ...
          if (!(KeyBuffer_size == 1 && KeyBuffer[0] == event.KeyInput.Char)) {
            KeyBuffer += " ";
            KeyBuffer[KeyBuffer_size - 1] = event.KeyInput.Char;
          }
        }
        else {
          KeyBuffer = " ";
        ...
        if (Selected > -1 && KeyBuffer_size > 1) {
          if (Items[Selected].text_size >= KeyBuffer_size &&
              KeyBuffer.equals_ignore_case(Items[Selected].text.subString(0, KeyBuffer_size))) {
            return true;
          }
        }
        s32 current;
        for (current = start + 1; current < (s32)Items_size; ++current) {
          if (Items[current].text_size >= KeyBuffer_size) {
            if (KeyBuffer.equals_ignore_case(Items[current].text.subString(0, KeyBuffer_size))) {
...
void CGUIListBox::draw()
{
  ...
  IRECT* clipRect = 0;
  // draw background
  IRECT frameRect(AbsoluteRect);
  // draw items
  IRECT clientClip(AbsoluteRect);
  ...
  for (s32 i = 0; i < (s32)Items_size; ++i) {
    if (frameRect.LowerRightCorner.Y >= AbsoluteRect.UpperLeftCorner.Y &&
      ...
      IRECT textRect = frameRect;
      textRect.UpperLeftCorner.X += 3;
      if (Font) {
        if (IconBank && (Items[i].icon > -1)) {
          position2di iconPos = textRect.UpperLeftCorner;
...
u32 CGUIListBox::addItem(const char* text, s32 icon)
{
  ListItem i;
  i.text = text;
  i.icon = icon;
...
bool CGUIListBox::getSerializationLabels(EGUI_LISTBOX_COLOR colorType, char* useColorLabel, char* colorLabel) const
{
  switch (colorType) {
  case EGUI_LBC_TEXT:
...
void CGUIListBox::serializeAttributes(IAttributes* out, SAttributeReadWriteOptions* options = 0) const
{
  ...
  for (i = 0; i < Items_size; ++i) {
    char* label("text");
    label += i;
    out->addString(label, Items[i].text);
    for (s32 c = 0; c < (s32)EGUI_LBC_COUNT; ++c) {
      char* useColorLabel, colorLabel;
      ...
      if (Items[i].OverrideColors[c].Use) {
        out->addBool(label, true);
        label = colorLabel;
        label += i;
        out->addColor(label, Items[i].OverrideColors[c].Color);
      }
      else {
        out->addBool(label, false);
      }
    }
  }
}
...
void CGUIListBox::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options = 0)
{
  ...
  for (s32 i = 0; i < count; ++i) {
    ...
    ListItem item;
    label += i;
    item.text = in->getAttributeAsString(label);
    addItem(item.text, item.icon);
    for (int c = 0; c < EGUI_LBC_COUNT; ++c) {
      char* useColorLabel, colorLabel;
      if (!getSerializationLabels((EGUI_LISTBOX_COLOR)c, useColorLabel, colorLabel)) {
        return;
      }
      label = useColorLabel;
      label += i;
      Items[i].OverrideColors[c].Use = in->getAttributeAsBool(label);
      if (Items[i].OverrideColors[c].Use) {
        label = colorLabel;
        label += i;
        Items[i].OverrideColors[c].Color = in->getAttributeAsColor(label);
      }
    }
  }
}
void CGUIListBox::recalculateItemWidth(s32 icon)
{
  if (IconBank && icon > -1 &&
      ...
      IconBank->getSprites()[(u32)icon].Frames_size) {
...
void CGUIListBox::setItem(u32 index, const char* text, s32 icon)
{
  if (index >= Items_size) {
    return;
  }
...
s32 CGUIListBox::insertItem(u32 index, const char* text, s32 icon)
{
  ListItem i;
  i.text = text;
  i.icon = icon;
...
{
  if (index1 >= Items_size || index2 >= Items_size) {
    return;
  }
...
void CGUIListBox::setItemOverrideColor(u32 index, SColor color)
{
  for (int c = 0; c < EGUI_LBC_COUNT; ++c) {
...
void CGUIListBox::setItemOverrideColor(u32 index, EGUI_LISTBOX_COLOR colorType, SColor color)
{
  if (index >= Items_size || colorType < 0 || colorType >= EGUI_LBC_COUNT) {
    return;
  }
...
{
  for (int c = 0; c < (u32)EGUI_LBC_COUNT; ++c) {
...
SColor CGUIListBox::getItemOverrideColor(u32 index, EGUI_LISTBOX_COLOR colorType) const
{
  if ((u32)index >= Items_size || colorType < 0 || colorType >= EGUI_LBC_COUNT) {
    return SColor();
  }
  return Items[index].OverrideColors[colorType].Color;
}
SColor CGUIListBox::getItemDefaultColor(EGUI_LISTBOX_COLOR colorType) const
{
  IGUISkin* skin = Environment->getSkin();
  if (!skin) {
    return SColor();
