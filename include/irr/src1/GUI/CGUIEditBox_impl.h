...
*/
//! constructor
CGUIEditBox::CGUIEditBox(const char* text, bool border,
  ...
  : IGUIEditBox(environment, parent, id, rectangle), MouseMarking(false),
    ...
    OverrideColor(SColor(101, 255, 255, 255)), OverrideFont(0), LastBreakFont(0),
    Operator(0), BlinkStartTime(0), CursorPos(0), HScrollPos(0), VScrollPos(0), Max(0),
    WordWrap(false), MultiLine(false), AutoScroll(true), PasswordBox(false),
    PasswordChar('*'), HAlign(EGUIA_UPPERLEFT), VAlign(EGUIA_CENTER),
...
void CGUIEditBox::setOverrideColor(SColor color)
{
  OverrideColor = color;
  OverrideColorEnabled = true;
}
SColor CGUIEditBox::getOverrideColor() const
{
...
{
  IRECT oldAbsoluteRect(AbsoluteRect);
...
void CGUIEditBox::setPasswordBox(bool passwordBox, char passwordChar)
{
  ...
  if (event.KeyInput.Control) {
    ...
    case KEY_KEY_X:
      ...
      if (!PasswordBox && Operator && MarkBegin != MarkEnd) {
        ...
        Operator->copyToClipboard(sc);
        if (isEnabled()) {
          // delete
          char* s;
          s = Text.subString(0, realmbgn);
          s.append(Text.subString(realmend, Text_size - realmend));
          Text = s;
          CursorPos = realmbgn;
          newMarkBegin = 0;
          newMarkEnd = 0;
    ...
    case KEY_KEY_V:
      ...
      if (Operator) {
        ...
        if (p) {
          // TODO: we should have such a function in string
          char* widep(p);
          if (MarkBegin == MarkEnd) {
            // insert text
            char* s = Text.subString(0, CursorPos);
            s.append(widep);
            s.append(Text.subString(CursorPos, Text_size - CursorPos));
            if (!Max || s_size <= Max) { // thx to Fish FH for fix
              Text = s;
              s = widep;
              CursorPos += s_size;
            }
            ...
            // replace text
            char* s = Text.subString(0, realmbgn);
            s.append(widep);
            s.append(Text.subString(realmend, Text_size - realmend));
            if (!Max || s_size <= Max) { // thx to Fish FH for fix
              Text = s;
              s = widep;
              CursorPos = realmbgn + s_size;
            }
          }
        }
        newMarkBegin = 0;
  ...
  // default keyboard handling
    ...
    case KEY_END: {
      ...
      if (WordWrap || MultiLine) {
        ...
        p = BrokenTextPositions[p] + (s32)BrokenText[p]_size;
        if (p > 0 && (Text[p - 1] == '\r' || Text[p - 1] == '\n')) {
          p -= 1;
        }
      }
      if (event.KeyInput.Shift) {
    ...
    case KEY_RIGHT:
      ...
      break;
    case KEY_UP:
      if (MultiLine || (WordWrap && BrokenText_size > 1)) {
        s32 lineNo = getLineFromPos(CursorPos);
        s32 mb = (MarkBegin == MarkEnd) ? CursorPos : (MarkBegin > MarkEnd ? MarkBegin : MarkEnd);
        if (lineNo > 0) {
          s32 cp = CursorPos - BrokenTextPositions[lineNo];
          if ((s32)BrokenText[lineNo - 1]_size < cp) {
            CursorPos = BrokenTextPositions[lineNo - 1] + max_((u32)1, BrokenText[lineNo - 1]_size) - 1;
          }
          else {
    ...
    case KEY_DOWN:
      if (MultiLine || (WordWrap && BrokenText_size > 1)) {
        s32 lineNo = getLineFromPos(CursorPos);
        s32 mb = (MarkBegin == MarkEnd) ? CursorPos : (MarkBegin < MarkEnd ? MarkBegin : MarkEnd);
        if (lineNo < (s32)BrokenText_size - 1) {
          s32 cp = CursorPos - BrokenTextPositions[lineNo];
          if ((s32)BrokenText[lineNo + 1]_size < cp) {
            CursorPos = BrokenTextPositions[lineNo + 1] + max_((u32)1, BrokenText[lineNo + 1]_size) - 1;
          }
    ...
    case KEY_DELETE:
      ...
      if (Text_size != 0) {
        ...
        if (MarkBegin != MarkEnd) {
          ...
          s.append(Text.subString(CursorPos + 1, Text_size - CursorPos - 1));
          Text = s;
        }
        if (CursorPos > (s32)Text_size) {
          CursorPos = (s32)Text_size;
        }
        BlinkStartTime = os::Timer::getTime();
        newMarkBegin = 0;
        newMarkEnd = 0;
...
void CGUIEditBox::draw()
{
  ...
  if (font) {
    ...
    const s32 lineCount = ml ? BrokenText_size : 1;
    // Save the override color information.
    // Then, alter it if the edit box is disabled.
    const bool prevOver = OverrideColorEnabled;
    const SColor prevColor = OverrideColor;
    if (Text_size) {
      ...
      for (s32 i = 0; i < lineCount; ++i) {
        ...
        // get current line
        if (PasswordBox) {
          if (BrokenText_size != 1) {
            BrokenText.clear();
            BrokenText.push_back(char * ());
          }
          if (BrokenText[0]_size != Text_size) {
            BrokenText[0] = Text;
        ...
        if (focus && MarkBegin != MarkEnd && i >= hlineStart && i < hlineStart + hlineCount) {
          ...
          if (i == hlineStart) {
            ...
            mbegin = font->getDimension(s).Width;
            // deal with kerning
          ...
          if (i == hlineStart + hlineCount - 1) {
            ...
            mend = font->getDimension(s2).Width;
            lineEndPos = (s32)s2_size;
          }
          else {
    ...
    if (IsEnabled) {
      ...
      charcursorpos = font->getDimension(s).Width +
          font->getKerningWidth("_", CursorPos - startPos > 0 ? & ((*txtLine)[CursorPos - startPos - 1]) : 0);
...
void CGUIEditBox::setText(const char* text)
{
  Text = text;
  if (u32(CursorPos) > Text_size) {
    CursorPos = Text_size;
  }
  HScrollPos = 0;
  breakText();
...
{
  IGUIFont* font = getActiveFont();
  const u32 lineCount = (WordWrap || MultiLine) ? BrokenText_size : 1;
  char** txtLine = 0;
  s32 startPos = 0;
  x += 3;
  int i;
...
void CGUIEditBox::breakText()
{
  ...
  for (s32 i = 0; i < size; ++i) {
    ...
    if (c == ' ' || c == 0 || i == (size - 1)) {
      ...
      s32 whitelgth = font->getDimension(whitespace).Width;
      s32 worldlgth = font->getDimension(word).Width;
      if (WordWrap && length + worldlgth + whitelgth > elWidth && line_size > 0) {
        // break to next line
        length = worldlgth;
        BrokenText.push_back(line);
        BrokenTextPositions.push_back(lastLineStart);
        lastLineStart = i - (s32)word_size;
        line = word;
      }
      else {
        // add word to line
...
void CGUIEditBox::setTextRect(s32 line)
{
  ...
  const u32 lineCount = (WordWrap || MultiLine) ? BrokenText_size : 1;
  if (WordWrap || MultiLine) {
    d = font->getDimension(BrokenText[line]);
  }
  else {
    d = font->getDimension(Text);
...
s32 CGUIEditBox::getLineFromPos(s32 pos)
{
  ...
  s32 i = 0;
  while (i < (s32)BrokenTextPositions_size) {
    if (BrokenTextPositions[i] > pos) {
      return i - 1;
    }
    ++i;
  }
  return (s32)BrokenTextPositions_size - 1;
}
void CGUIEditBox::inputChar(char c)
{
  if (!isEnabled()) {
    return;
  }
...
void CGUIEditBox::calculateScrollPos()
{
  ...
  // NOTE: Calculations different to vertical scrolling because setTextRect interprets VAlign relative to line but HAlign not relative to row
  {
    ...
    // get cursor area
    u32 cursorWidth = font->getDimension("_").Width;
    char** txtLine = hasBrokenText ? &BrokenText[cursLine] : &Text;
    s32 cPos = hasBrokenText ? CursorPos - BrokenTextPositions[cursLine] : CursorPos; // column
    s32 cStart = font->getDimension(txtLine->subString(0, cPos)).Width;   // pixels from text-start
  ...
  if (hasBrokenText) {
    u32 lineHeight = font->getDimension("A").Height + font->getKerningHeight();
    ...
    if (lineHeight >= (u32)FrameRect.getHeight()) {
      ...
      if (CurrentTextRect.UpperLeftCorner.Y > FrameRect.UpperLeftCorner.Y && VAlign != EGUIA_LOWERRIGHT) {
        ...
        u32 lastLine = BrokenTextPositions.empty() ? 0 : BrokenTextPositions_size - 1;
...
void CGUIEditBox::serializeAttributes(IAttributes* out, SAttributeReadWriteOptions* options = 0) const
{
  ...
  char* ch = " ";
  ch[0] = PasswordChar;
  out->addString("PasswordChar", ch);
  out->addEnum("HTextAlign", HAlign, GUIAlignmentNames);
  out->addEnum("VTextAlign", VAlign, GUIAlignmentNames);
  IGUIEditBox::serializeAttributes(out, options);
}
//! Reads attributes of the element
void CGUIEditBox::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options = 0)
{
  ...
  char* ch = in->getAttributeAsString("PasswordChar");
  if (!ch_size) {
