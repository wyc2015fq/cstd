...
class IGUIElement : public IAttributeExchangingObject, public IEventReceiver
{
public:
  //! Constructor
  ...
  IRECT getRelativePosition() const {
    return RelativeRect;
  }
  //! Sets the relative rectangle of this element.
  /** \param r The absolute position to set */
  void setRelativePosition(const IRECT& r) {
    if (Parent) {
      const IRECT& r2 = Parent->getAbsolutePosition();
      dimension2df d((f32)(r2.getSize().Width), (f32)(r2.getSize().Height));
  ...
  void setRelativePosition(const position2di& position) {
    const dimension2di mySize = RelativeRect.getSize();
    const IRECT rectangle(position.X, position.Y,
  ...
  IRECT getAbsolutePosition() const {
    return AbsoluteRect;
  }
  //! Returns the visible area of the element.
  IRECT getAbsoluteClippingRect() const {
  ...
  void setAlignment(EGUI_ALIGNMENT left, EGUI_ALIGNMENT right, EGUI_ALIGNMENT top, EGUI_ALIGNMENT bottom) {
    ...
    AlignBottom = bottom;
    if (Parent) {
      IRECT r(Parent->getAbsolutePosition());
      dimension2df d((f32)r.getSize().Width, (f32)r.getSize().Height);
      if (AlignLeft   == EGUIA_SCALE) {
  ...
  */
  IGUIElement* getElementFromPoint(const position2di& point) {
    IGUIElement* target = 0;
    // we have to search from back to front, because later children
    // might be drawn over the top of earlier ones.
    list<IGUIElement*>::Iterator it = Children.getLast();
    if (isVisible()) {
  ...
  //! Moves this element.
  void move(position2di absoluteMovement) {
  ...
  */
  ...
  const list<IGUIElement*>& getChildren() const {
    return Children;
  }
  ...
  IGUIElement* getElementFromId(s32 id, bool searchchildren = false) const {
    IGUIElement* e = 0;
  ...
  */
  ...
  void serializeAttributes(IAttributes* out, SAttributeReadWriteOptions* options = 0) const {
    ...
    out->addPosition2d("MinSize", position2di(MinSize.Width, MinSize.Height));
    out->addPosition2d("MaxSize", position2di(MaxSize.Width, MaxSize.Height));
  ...
  void deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options = 0) {
    ...
    setText(in->getAttributeAsString("Caption"));
    ...
    position2di p = in->getAttributeAsPosition2d("MaxSize");
...
protected:
  // not because needed in constructor
  ...
  void recalculateAbsolutePosition(bool recursive) {
    IRECT parentAbsolute(0, 0, 0, 0);
    IRECT parentAbsoluteClip;
    f32 fw = 0.f, fh = 0.f;
    if (Parent) {
