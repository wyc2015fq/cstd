...
IVideoDriver* createOpenGLDriver(const SIrrlichtCreationParameters& params, IFileSystem* io, this);
#endif
}
struct SEnvMapper {
  HWND hWnd;
  CIrrDeviceWinCE* irrDev;
};
...
CIrrDeviceWinCE* getDeviceFromHWnd(HWND hWnd)
{
  list<SEnvMapper>::Iterator it = EnvMap.begin();
  for (; it != EnvMap.end(); ++it)
    if ((*it).hWnd == hWnd) {
    ...
    ExternalWindow(false)
{
  ...
  if (!CreationParams.WindowId && (CreationParams.DriverType != EDT_NULL)) {
    const char* ClassName = "CIrrDeviceWinCE";
    // Register Class
    WNDCLASS wc;
    ...
    wc.hIcon = (HICON)LoadImage(hInstance, "irrlicht.ico", IMAGE_ICON, 0, 0, 0);
    RegisterClass(&wc);
    ...
    const s32 windowLeft = s32_max(0, (GetSystemMetrics(SM_CXSCREEN) - realWidth) >> 1);
    const s32 windowTop = s32_max(0, (GetSystemMetrics(SM_CYSCREEN) - realHeight) >> 1);
    // create window
...
void CIrrDeviceWinCE::createDriver()
{
  ...
  case EDT_OPENGL:
    ...
    VideoDriver = createOpenGLDriver(CreationParams, FileSystem);
    if (!VideoDriver) {
...
void CIrrDeviceWinCE::resizeIfNecessary()
{
  ...
  if (r.right < 2 || r.bottom < 2) {
    ...
    os::Printer::log(tmp);
    getVideoDriver()->OnResize(dimension2du(r.right, r.bottom));
    getWin32CursorControl()->OnResize(getVideoDriver()->getScreenSize());
  }
  Resized = false;
}
//! sets the caption of the window
void CIrrDeviceWinCE::setWindowCaption(const char* text)
{
...
bool CIrrDeviceWinCE::present(IImage* image, void* windowId, IRECT* src)
{
  HWND hwnd = HWnd;
  if (windowId) {
    hwnd = (HWND)windowId;
...
IVideoModeList* CIrrDeviceWinCE::getVideoModeList()
{
...
void CIrrDeviceWinCE::getWindowsVersion(char* out)
{
  out = "WinCE";
}
