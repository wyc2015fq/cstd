...
IVideoDriver* CIrrDeviceStub::getVideoDriver()
{
  return VideoDriver;
}
//! return file system
IFileSystem* CIrrDeviceStub::getFileSystem()
{
  return FileSystem;
}
//! returns the gui environment
IGUIEnvironment* CIrrDeviceStub::getGUIEnvironment()
{
  return GUIEnvironment;
}
//! returns the scene manager
ISceneManager* CIrrDeviceStub::getSceneManager()
{
  return SceneManager;
}
...
ICursorControl* CIrrDeviceStub::getCursorControl()
{
  return CursorControl;
}
//! \return Returns a pointer to a list with all video modes supported
//! by the gfx adapter.
IVideoModeList* CIrrDeviceStub::getVideoModeList()
{
...
u32 CIrrDeviceStub::checkSuccessiveClicks(s32 mouseX, s32 mouseY, EMOUSE_INPUT_EVENT inputEvent)
{
  ...
  if ((clickTime - MouseMultiClicks.LastClickTime) < MouseMultiClicks.DoubleClickTime
      && abs_(MouseMultiClicks.LastClick.X - mouseX) <= MAX_MOUSEMOVE
      && abs_(MouseMultiClicks.LastClick.Y - mouseY) <= MAX_MOUSEMOVE
...
struct SDefaultRandomizer : public IRandomizer {
  void reset(s32 value = 0x0f0f0f0f) {
...
void CIrrDeviceStub::setInputReceivingSceneManager(ISceneManager* sceneManager)
{
  if (sceneManager) {
    sceneManager->grab();
  }
...
ECOLOR_FORMAT CIrrDeviceStub::getColorFormat() const
{
  return ECF_R5G6B5;
}
//! No-op in this implementation
bool CIrrDeviceStub::activateJoysticks(SJoystickInfo > & joystickInfo)
{
  return false;
}
/*!
