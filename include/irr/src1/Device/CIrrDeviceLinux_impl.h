        ...
        ExternalWindow(false), AutorepeatSupport(0)
{
  ...
  os::Printer::log(linuxversion, ELL_INFORMATION);
  // create keymap
  createKeyMap();
  // create window
  if (CreationParams.DriverType != EDT_NULL) {
  ...
  CursorControl = new CCursorControl(this, CreationParams.DriverType == EDT_NULL);
  // create driver
  createDriver();
  if (!VideoDriver) {
    return;
...
#if defined(_IRR_COMPILE_WITH_JOYSTICK_EVENTS_)
  for (int joystick = 0; joystick < ActiveJoysticks_size; ++joystick) {
...
bool CIrrDeviceLinux::switchToFullscreen(bool reset)
{
  ...
  if (XF86VidModeQueryExtension(display, &eventbase, &errorbase)) {
    ...
    if (bestMode != -1) {
      ...
      os::Printer::log("hdisplay: ", char * (modes[bestMode]->hdisplay), ELL_INFORMATION);
      os::Printer::log("vdisplay: ", char * (modes[bestMode]->vdisplay), ELL_INFORMATION);
    ...
    if (XRRQueryExtension(display, &eventbase, &errorbase)) {
      ...
      if (bestMode != -1) {
        ...
        os::Printer::log("width: ", char * (modes[bestMode].width), ELL_INFORMATION);
        os::Printer::log("height: ", char * (modes[bestMode].height), ELL_INFORMATION);
...
bool CIrrDeviceLinux::createWindow()
{
  ...
  if (CreationParams.DriverType == EDT_OPENGL) {
  ...
  if (!visual) {
    ...
    os::Printer::log("Visual chosen: ", char * (static_cast<u32>(visual->visualid)), ELL_DEBUG);
  }
#endif
  // create color map
  Colormap colormap;
  ...
  Context = 0;
  if (isAvailableGLX && CreationParams.DriverType == EDT_OPENGL) {
    if (UseGLXWindow) {
  ...
  if (CreationParams.DriverType == EDT_SOFTWARE || CreationParams.DriverType == EDT_BURNINGSVIDEO) {
...
bool CIrrDeviceLinux::run()
{
  ...
  if ((CreationParams.DriverType != EDT_NULL) && display) {
    SEvent irrevent;
    ...
    while (XPending(display) > 0 && !Close) {
      ...
      case KeyPress: {
        ...
        irrevent.KeyInput.Char = ((char*)(buf))[0];
        ...
        if (irrevent.KeyInput.Key == 0) {
          ...
          if (idx == -1) {
            os::Printer::log("Could not find EKEY_CODE, using orig. X11 keycode instead", char * (event.xkey.keycode), ELL_INFORMATION);
          }
          else {
            os::Printer::log("EKEY_CODE is 0, using orig. X11 keycode instead", char * (event.xkey.keycode), ELL_INFORMATION);
          }
...
void CIrrDeviceLinux::setWindowCaption(const char* text)
{
#ifdef _IRR_COMPILE_WITH_X11_
  if (CreationParams.DriverType == EDT_NULL) {
    return;
  ...
  XTextProperty txt;
  if (Success == XwcTextListToTextProperty(display, const_cast<char**>(&text),
...
bool CIrrDeviceLinux::present(IImage* image, void* windowId, IRECT* srcRect)
{
#ifdef _IRR_COMPILE_WITH_X11_
  ...
  const u32 minWidth = min_(image->getDimension().Width, destwidth);
  const u32 destPitch = SoftwareImage->bytes_per_line;
  ECOLOR_FORMAT destColor;
  switch (SoftwareImage->bits_per_pixel) {
  case 16:
    if (SoftwareImage->depth == 16) {
      destColor = ECF_R5G6B5;
    }
    else {
      destColor = ECF_A1R5G5B5;
    }
    break;
  case 24:
    destColor = ECF_R8G8B8;
    break;
  case 32:
    destColor = ECF_A8R8G8B8;
    break;
  default:
  ...
  const u32 srcheight = min_(image->getDimension().Height, destheight);
  const u32 srcPitch = image->getPitch();
  for (int y = 0; y != srcheight; ++y) {
    CColorConverter::convert_viaFormat(srcdata, image->getColorFormat(), minWidth, destData, destColor);
    srcdata += srcPitch;
    destData += destPitch;
  }
  image->unlock();
...
ECOLOR_FORMAT CIrrDeviceLinux::getColorFormat() const
{
#ifdef _IRR_COMPILE_WITH_X11_
...
{
#ifdef _IRR_COMPILE_WITH_X11_
  if (CreationParams.DriverType == EDT_NULL || CreationParams.Fullscreen) {
    return;
  }
...
IVideoModeList* CIrrDeviceLinux::getVideoModeList()
{
#ifdef _IRR_COMPILE_WITH_X11_
  if (!VideoModeList->getVideoModeCount()) {
    ...
    if (display) {
      ...
      if (XF86VidModeQueryExtension(display, &eventbase, &errorbase)) {
        ...
        // find fitting mode
        VideoModeList->setDesktop(defaultDepth, dimension2du(
            modes[0]->hdisplay, modes[0]->vdisplay));
        int i;
        for (i = 0; i < modeCount; ++i) {
          VideoModeList->addMode(dimension2du(
...
bool CIrrDeviceLinux::activateJoysticks(SJoystickInfo > & joystickInfo)
{
#if defined (_IRR_COMPILE_WITH_JOYSTICK_EVENTS_)
  joystickInfo.clear();
  u32 joystick;
  for (joystick = 0; joystick < 32; ++joystick) {
    ...
    info.persistentData.JoystickEvent.Joystick = ActiveJoysticks_size;
  ...
  for (joystick = 0; joystick < joystickInfo_size; ++joystick) {
    char logString[256];
...
{
#if defined (_IRR_COMPILE_WITH_JOYSTICK_EVENTS_)
  if (0 == ActiveJoysticks_size) {
    return;
  }
  for (int j = 0; j < ActiveJoysticks_size; ++j) {
...
Bool PredicateIsEventType(Display* display, XEvent* event, XPointer arg)
{
  if (event && event->type == *(int*)arg) {
    //    os::Printer::log("remove event:", char*((int)arg), ELL_INFORMATION);
    return True;
  }
  return False;
}
...
#ifdef _IRR_COMPILE_WITH_X11_
Cursor CIrrDeviceLinux::TextureToMonochromeCursor(ITexture* tex, const IRECT& sourceRect, const position2di& hotspot)
{
...
#ifdef _IRR_LINUX_XCURSOR_
Cursor CIrrDeviceLinux::TextureToARGBCursor(ITexture* tex, const IRECT& sourceRect, const position2di& hotspot)
{
...
Cursor CIrrDeviceLinux::TextureToCursor(ITexture* tex, const IRECT& sourceRect, const position2di& hotspot)
{
#ifdef _IRR_LINUX_XCURSOR_
...
CIrrDeviceLinux::CCursorControl::CCursorControl(CIrrDeviceLinux* dev, bool null)
  : Device(dev)
#ifdef _IRR_COMPILE_WITH_X11_
  , PlatformBehavior(ECPB_NONE), lastQuery(0)
#endif
...
void CIrrDeviceLinux::CCursorControl::clearCursors()
{
  ...
  int i;
  for (i = 0; i < Cursors_size; ++i) {
    for (int f = 0; f < Cursors[i].Frames_size; ++f) {
...
{
  if ((u32)ActiveIcon < Cursors_size && !Cursors[ActiveIcon].Frames.empty() && Cursors[ActiveIcon].FrameTime) {
...
void CIrrDeviceLinux::CCursorControl::setActiveIcon(ECURSOR_ICON iconId)
{
#ifdef _IRR_COMPILE_WITH_X11_
  if (iconId >= (s32)Cursors_size) {
    return;
  }
  if (Cursors[iconId].Frames_size) {
...
ECURSOR_ICON CIrrDeviceLinux::CCursorControl::addIcon(const SCursorSprite& icon)
{
#ifdef _IRR_COMPILE_WITH_X11_
  if (icon.SpriteId >= 0) {
    CursorX11 cX11;
...
void CIrrDeviceLinux::CCursorControl::changeIcon(ECURSOR_ICON iconId, const SCursorSprite& icon)
{
#ifdef _IRR_COMPILE_WITH_X11_
  if (iconId >= (s32)Cursors_size) {
    return;
...
dimension2di CIrrDeviceLinux::CCursorControl::getSupportedIconSize() const
{
