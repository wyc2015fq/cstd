...
CIrrDeviceConsole* DeviceToClose;
...
BOOL WINAPI ConsoleHandler(DWORD CEvent)
{
  switch (CEvent) {
  case CTRL_C_EVENT:
    os::Printer::log("Closing console device", "CTRL+C");
    break;
  case CTRL_BREAK_EVENT:
    os::Printer::log("Closing console device", "CTRL+Break");
    break;
  case CTRL_CLOSE_EVENT:
    os::Printer::log("Closing console device", "User closed console");
    break;
  case CTRL_LOGOFF_EVENT:
    os::Printer::log("Closing console device", "User is logging off");
    break;
  case CTRL_SHUTDOWN_EVENT:
    os::Printer::log("Closing console device", "Computer shutting down");
    break;
  }
...
void sighandler(int sig)
{
  ...
  code += sig;
  code += " received";
  os::Printer::log("Closing console device", code);
  DeviceToClose->closeDevice();
}
#endif
  ...
  : CIrrDeviceStub(params), IsWindowFocused(true), ConsoleFont(0), OutFile(stdout)
{
  ...
  for (int y = 0; y < CreationParams.WindowSize.Height; ++y) {
    char* str;
    str.reserve(CreationParams.WindowSize.Width);
    for (int x = 0; x < CreationParams.WindowSize.Width; ++x) {
      str += " ";
    }
  ...
  if (VideoDriver) {
    ...
    if (GUIEnvironment) {
      ConsoleFont = new CGUIConsoleFont(this);
      IGUISkin* skin = GUIEnvironment->getSkin();
      if (skin) {
        int i;
        for (i = 0; i < EGDF_COUNT; ++i) {
          skin->setFont(ConsoleFont, EGUI_DEFAULT_FONT(i));
        }
      }
    }
#endif
  ...
  while (count) {
    ...
    case MOUSE_EVENT: {
      ...
      if (in.Event.MouseEvent.dwEventFlags & MOUSE_MOVED) {
        CursorControl->setPosition(position2di(e.MouseInput.X, e.MouseInput.Y));
    ...
    case WINDOW_BUFFER_SIZE_EVENT:
      VideoDriver->OnResize(
          dimension2du(in.Event.WindowBufferSizeEvent.dwSize.X,
...
void CIrrDeviceConsole::setWindowCaption(const char* text)
{
#ifdef _IRR_WINDOWS_NT_CONSOLE_
...
bool CIrrDeviceConsole::present(IImage* surface, void* windowId, IRECT* src)
{
  if (surface) {
    for (int y = 0; y < surface->getDimension().Height; ++y) {
      for (int x = 0; x < surface->getDimension().Width; ++x) {
        // get average pixel
  ...
  for (i = 0; i < Text_size; ++i) {
    s32 y = Text[i].Pos.Y;
    if (y < (s32)OutputBuffer_size && y > 0)
      for (int c = 0; c < Text[i].Text_size && c + Text[i].Pos.X < OutputBuffer[y]_size; ++c)
        //if (Text[i].Text[c] != ' ')
      {
...
#endif
  // draw output
  for (int y = 0; y < OutputBuffer_size; ++y) {
    setTextCursorPos(0, y);
    fprintf(OutFile, "%s", OutputBuffer[y]);
  }
  return surface != 0;
}
...
void CIrrDeviceConsole::addPostPresentText(s16 X, s16 Y, const char* text)
{
  SPostPresentText p;
  p.Text = text;
  p.Pos.X = X;
