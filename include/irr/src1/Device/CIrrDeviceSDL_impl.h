...
IVideoDriver* createDirectX8Driver(const SIrrlichtCreationParameters& params, IFileSystem* io, HWND window);
#endif
#ifdef _IRR_COMPILE_WITH_DIRECT3D_9_
IVideoDriver* createDirectX9Driver(const SIrrlichtCreationParameters& params, IFileSystem* io, HWND window);
#endif
#ifdef _IRR_COMPILE_WITH_OPENGL_
IVideoDriver* createOpenGLDriver(const SIrrlichtCreationParameters& params, IFileSystem* io, CIrrDeviceSDL* device);
#endif
//! constructor
    ...
    Resizable(false), WindowHasFocus(false), WindowMinimized(false)
{
  ...
  os::Printer::log(sdlversion, ELL_INFORMATION);
  // create keymap
  createKeyMap();
...
{
#if defined(_IRR_COMPILE_WITH_JOYSTICK_EVENTS_)
  const u32 numJoysticks = Joysticks_size;
  int i;
...
bool CIrrDeviceSDL::run()
{
  ...
  for (i = 0; i < Joysticks_size; ++i) {
    ...
    if (joystick) {
      int j;
      // query all buttons
      const int numButtons = min_(SDL_JoystickNumButtons(joystick), 32);
      ...
      const int numAxes = min_(SDL_JoystickNumAxes(joystick), (int)SEvent::SJoystickEvent::NUMBER_OF_AXES);
...
bool CIrrDeviceSDL::activateJoysticks(SJoystickInfo > & joystickInfo)
{
#if defined(_IRR_COMPILE_WITH_JOYSTICK_EVENTS_)
  joystickInfo.clear();
  // we can name up to 256 different joysticks
  const int numJoysticks = min_(SDL_NumJoysticks(), 256);
  ...
  for (joystick = 0; joystick < (int)joystickInfo_size; ++joystick) {
    char logString[256];
...
void CIrrDeviceSDL::setWindowCaption(const char* text)
{
  char* textc = text;
  SDL_WM_SetCaption(textc, textc);
}
//! presents a surface in the client area
bool CIrrDeviceSDL::present(IImage* surface, void* windowId, IRECT* srcClip)
{
  ...
  if ((surface->getColorFormat() == ECF_R8G8B8) ||
      (surface->getColorFormat() == ECF_A8R8G8B8)) {
  ...
  else if (surface->getColorFormat() == ECF_R5G6B5) {
  ...
  else if (surface->getColorFormat() == ECF_A1R5G5B5) {
    sdlSurface->format->Rloss = 3;
...
IVideoModeList* CIrrDeviceSDL::getVideoModeList()
{
  if (!VideoModeList->getVideoModeCount()) {
    ...
    if (modes != 0) {
      if (modes == (SDL_Rect**) - 1) {
        ...
        int i;
        for (i = 0; modes[i]; ++i) {
          VideoModeList->addMode(dimension2du(modes[i]->w, modes[i]->h), vi->vfmt->BitsPerPixel);
        }
      }
    }
  }
...
ECOLOR_FORMAT CIrrDeviceSDL::getColorFormat() const
{
  if (Screen) {
