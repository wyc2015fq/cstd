...
struct SJoystickWin32Control {
  ...
  void directInputAddJoystick(LPCDIDEVICEINSTANCE lpddi) {
    ...
    activeJoystick.Index = ActiveJoysticks_size;
    ...
    for (i = 0; i < (activeJoystick.devcaps.dwAxes) - caxis; i++) {
      if (i + caxis < 8) {
  ...
  void pollJoysticks() {
    ...
    u32 joystick;
    DIJOYSTATE2 info;
    for (joystick = 0; joystick < ActiveJoysticks_size; ++joystick) {
    ...
    ExternalWindow(false), Win32CursorControl(0), JoyControl(0)
{
  ...
  os::Printer::log(winversion, ELL_INFORMATION);
...
void CIrrDeviceWin32::setWindowCaption(const char* text)
{
...
bool CIrrDeviceWin32::present(IImage* image, void* windowId, IRECT* src)
{
  HWND hwnd = HWnd;
  if (windowId) {
...
//! by the gfx adapter.
IVideoModeList* CIrrDeviceWin32::getVideoModeList()
{
  if (!VideoModeList->getVideoModeCount()) {
    ...
    while (EnumDisplaySettings(NULL, i, &mode)) {
      VideoModeList->addMode(dimension2du(mode.dmPelsWidth, mode.dmPelsHeight),
          mode.dmBitsPerPel);
      ++i;
    }
    if (EnumDisplaySettings(NULL, ENUM_CURRENT_SETTINGS, &mode)) {
      VideoModeList->setDesktop(mode.dmBitsPerPel, dimension2du(mode.dmPelsWidth, mode.dmPelsHeight));
    }
  }
...
#endif
void CIrrDeviceWin32::getWindowsVersion(char* out)
{
  ...
  case VER_PLATFORM_WIN32_NT:
    ...
    if (osvi.dwMajorVersion <= 4) {
      sprintf(tmp, "version %ld.%ld %s (Build %ld)",
          osvi.dwMajorVersion,
          osvi.dwMinorVersion,
          char * (osvi.szCSDVersion),
          osvi.dwBuildNumber & 0xFFFF);
    }
    else {
      sprintf(tmp, "%s (Build %ld)", char * (osvi.szCSDVersion),
          osvi.dwBuildNumber & 0xFFFF);
    }
    out.append(tmp);
    break;
...
bool CIrrDeviceWin32::activateJoysticks(SJoystickInfo > & joystickInfo)
{
  if (JoyControl) {
...
HCURSOR CIrrDeviceWin32::TextureToCursor(HWND hwnd, ITexture* tex, const IRECT& sourceRect, const position2di& hotspot)
{
  //
...
CIrrDeviceWin32::CCursorControl::CCursorControl(CIrrDeviceWin32* device, const dimension2du& wsize, HWND hwnd, bool fullscreen)
...
dimension2di CIrrDeviceWin32::CCursorControl::getSupportedIconSize() const
{
  dimension2di result;
