...
CIrrDeviceFB::CIrrDeviceFB(const SIrrlichtCreationParameters& params)
  : CIrrDeviceStub(params), Framebuffer(-1), EventDevice(-1), SoftwareImage(0),
    Pitch(0), FBColorFormat(ECF_A8R8G8B8), Close(false)
{
#ifdef _DEBUG
  ...
  CursorControl = new CCursorControl(this, params.DriverType == EDT_NULL);
  // create driver
  createDriver();
  if (!VideoDriver) {
    return;
...
bool CIrrDeviceFB::createWindow(const dimension2du& windowSize, u32 bits)
{
  char buf[256];
  ...
  if (CreationParams.DriverType != EDT_NULL) {
    ...
    if (fbscreeninfo.bits_per_pixel == 16) {
      if (fbscreeninfo.transp.length == 0) {
        FBColorFormat = ECF_R5G6B5;
      }
      else {
        FBColorFormat = ECF_A1R5G5B5;
      }
    }
    else {
      if (fbscreeninfo.transp.length == 0) {
        FBColorFormat = ECF_R8G8B8;
      }
      else {
        FBColorFormat = ECF_A8R8G8B8;
      }
    }
...
bool CIrrDeviceFB::present(IImage* image, void* windowId, IRECT* src)
{
  ...
  // clip images
  srcheight = min_(srcheight, CreationParams.WindowSize.Height);
  srcwidth = min_(srcwidth, CreationParams.WindowSize.Width);
  u8* srcdata = (u8*)image->lock();
  for (int y = 0; y < srcheight; ++y) {
    CColorConverter::convert_viaFormat(srcdata, image->getColorFormat(), srcwidth, destData, FBColorFormat);
...
void CIrrDeviceFB::setWindowCaption(const char* text)
{
}
