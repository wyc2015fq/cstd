...
void AngleQuaternion(const float3 angles, vec4_hl quaternion)
{
  f32   angle;
...
CV_INLINE void VectorTransform(const vec3_hl in1, const f32 in2[3][4], float3 out)
{
...
void getBoneVector(float3 out, u32 index)
{
  out.X = BoneTransform[index][0][3];
  out.Z = BoneTransform[index][1][3];
  out.Y = BoneTransform[index][2][3];
}
void getBoneBox(aabbox3df& box, u32 index, f32 size = 0.5f)
{
...
void getTransformedBoneVector(float3 out, u32 index, const vec3_hl in)
{
...
CHalflifeMDLMeshFileLoader::CHalflifeMDLMeshFileLoader(
    ISceneManager* smgr) : SceneManager(smgr)
{
#ifdef _DEBUG
...
bool CHalflifeMDLMeshFileLoader::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "mdl");
}
//! creates/loads an animated mesh from the file.
//! \return Pointer to the created mesh. Returns 0 if loading failed.
//! If you no longer need the mesh, you should call IAnimatedMesh::drop().
//! See IReferenceCounted::drop() for more information.
IAnimatedMesh* CHalflifeMDLMeshFileLoader::createMesh(IReadFile* file)
{
...
*/
bool CAnimatedMeshHalfLife::loadModelFile(IReadFile* file,
    ISceneManager* smgr)
{
  if (!file) {
    return false;
...
static float3 TransformedVerts[MAXSTUDIOVERTS];  // transformed vertices
//static float3 TransformedNormals[MAXSTUDIOVERTS];  // light surface normals
/*!
*/
void CAnimatedMeshHalfLife::initModel()
{
  ...
  for (i = 0; i < Header->numseq; i++) {
    ...
    ipol.Frames = max_(1, seq[i].numframes - 1);
  ...
  */
  ...
  for (i = 0; i < Header->numbodyparts; ++i) {
    ...
    part.defaultModel = max_(0, (s32) body[i].base - 1);
    SHalflifeModel* model = (SHalflifeModel*)((u8*) Header + body[i].modelindex);
    for (int g = 0; g < body[i].nummodels; ++g) {
      SubModel sub;
  ...
  vector2df tex_scale;
  vector2di tex_trans(0, 0);
#ifdef HL_TEXTURE_ATLAS
  TextureAtlas.getScale(tex_scale);
#endif
  for (int bodypart = 0 ; bodypart < Header->numbodyparts ; ++bodypart) {
    const SHalflifeBody* body = (SHalflifeBody*)((u8*) Header + Header->bodypartindex) + bodypart;
    for (int modelnr = 0; modelnr < body->nummodels; ++modelnr) {
      ...
      for (i = 0; i < model->nummesh; ++i) {
        ...
        u16* index = buffer->Indices;
        S3DVertex* v = buffer->Vertices;
        ...
        // material
        SMaterial& m = buffer->getMaterial();
...
#ifdef HL_TEXTURE_ATLAS
        char* store = TextureBaseName + "atlas";
#else
        char* fname;
        char* ext;
        splitFilename(currentex->name, 0, &fname, &ext);
        char* store = TextureBaseName + fname;
#endif
      ...
      for (i = 0; i < model->nummesh; i++) {
        ...
        while ((c = *(tricmd++))) {
          ...
          for (g = 0; g < c; ++g, v += 1, tricmd += 4) {
            // fill vertex
            const float3 av = TransformedVerts[tricmd[0]];
            v->Pos = av;
            /*
                      const float3 an = TransformedNormals[tricmd[1]];
                      v->Normal = an;
                      //v->Normal.normalize();
            */
          }
...
/*!
  render Bones
*/
void CAnimatedMeshHalfLife::renderModel(u32 param, IVideoDriver* driver, const matrix4& absoluteTransformation)
{
  SHalflifeBone* bone = (SHalflifeBone*)((u8*) Header + Header->boneindex);
  SColor blue(0xFF000080);
  SColor red(0xFF800000);
  SColor yellow(0xFF808000);
  SColor cyan(0xFF008080);
  aabbox3df box;
  u32 i;
...
IMesh* CAnimatedMeshHalfLife::getMesh(s32 frameInt, s32 detailLevel, s32 startFrameLoop, s32 endFrameLoop)
{
  ...
  u32 frameA = floor32(frame);
  //  f32 blend = fract ( frame );
  SHalflifeSequence* seq = (SHalflifeSequence*)((u8*) Header + Header->seqindex);
  // find SequenceIndex from summed list
  u32 frameCount = 0;
  int i;
  for (i = 0; i < Header->numseq; ++i) {
    u32 val = max_(1, seq[i].numframes - 1);
...
/*!
*/
void STextureAtlas::addSource(const c8* name, IImage* image)
{
...
/*!
*/
void STextureAtlas::getScale(vector2df& scale)
{
  for (s32 i = static_cast<s32>(atlas_size) - 1; i >= 0; --i) {
...
/*!
*/
void STextureAtlas::getTranslation(const c8* name, vector2di& pos)
{
  int i;
  for (i = 0; i < atlas_size; ++i) {
...
void STextureAtlas::create(u32 border, E_TEXTURE_CLAMP texmode)
{
  ...
  const s32 frame = s32_max(0, (border - 1) / 2);
  ...
  wsum = s32_max(wsum, w);
  // build image
  dimension2du dim = dimension2du(wsum, hsum).getOptimalSize();
  ...
  for (i = 0; i < atlas_size; i++) {
    ...
    for (b = 0; b < frame; ++b) {
      for (a = 0 - b; a <= (s32) atlas[i].width + b; ++a) {
        col[0] = atlas[i].image->getPixel(s32_clamp(a, 0, atlas[i].width - 1), 0);
        col[1] = atlas[i].image->getPixel(s32_clamp(a, 0, atlas[i].width - 1), atlas[i].height - 1);
      ...
      for (a = -1 - b; a <= (s32) atlas[i].height + b; ++a) {
        col[0] = atlas[i].image->getPixel(0, s32_clamp(a, 0, atlas[i].height - 1));
        col[1] = atlas[i].image->getPixel(atlas[i].width - 1, s32_clamp(a, 0, atlas[i].height - 1));
...
/*!
*/
SHalflifeHeader* CAnimatedMeshHalfLife::loadModel(IReadFile* file, const char* filename)
{
  ...
  if (header->textureindex && idst) {
    ...
    char* fname;
    char* ext;
    splitFilename(file->getFileName(), &path, &fname, &ext);
    ...
    for (i = 0; i < header->numtextures; ++i) {
      ...
      IImage* image = SceneManager->getVideoDriver()->createImage(ECF_R8G8B8, dimension2du(tex[i].width, tex[i].height));
...
#else
      splitFilename(tex[i].name, 0, &fname, &ext);
...
/*!
*/
bool CAnimatedMeshHalfLife::postLoadModel(const char* filename)
{
  char* path;
  char* texname;
  char* submodel;
  splitFilename(filename , &path, &texname, 0);
  // preload textures
...
/*!
*/
void CAnimatedMeshHalfLife::ExtractBbox(s32 sequence, aabbox3df& box) const
{
...
void CAnimatedMeshHalfLife::calcBoneAdj()
{
  ...
  for (int j = 0; j < Header->numbonecontrollers; j++) {
    ...
    if (bonecontroller[j].type & STUDIO_RLOOP) {
      ...
      value = clamp(BoneController[i] / range, 0.f, 1.f);
    ...
    SHalflifeSequence* seq, SHalflifeAnimOffset* anim, f32 f)
{
  ...
  SHalflifeBone* bone = (SHalflifeBone*)((u8*)Header + Header->boneindex);
  int i;
  for (i = 0; i < Header->numbones; i++, bone++, anim++) {
    float3 angle1, angle2;
...
const F3AABBOX& CAnimatedMeshHalfLife::getBoundingBox() const
{
...
IMeshBuffer* CAnimatedMeshHalfLife::getMeshBuffer(const SMaterial& material) const
{
  return MeshIPol->getMeshBuffer(material);
}
void CAnimatedMeshHalfLife::setMaterialFlag(E_MATERIAL_FLAG flag, bool newvalue)
{
  MeshIPol->setMaterialFlag(flag, newvalue);
}
//! set user axis aligned bounding box
void CAnimatedMeshHalfLife::setBoundingBox(const aabbox3df& box)
{
