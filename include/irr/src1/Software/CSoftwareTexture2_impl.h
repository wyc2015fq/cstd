...
CSoftwareTexture2::CSoftwareTexture2(IImage* image, const char* name,
  ...
  : ITexture(name), MipMapLOD(0), Flags(flags), OriginalFormat(ECF_UNKNOWN)
{
#ifdef _DEBUG
  ...
  if (image) {
    ...
    setbit_cond(Flags,
        image->getColorFormat() == ECF_A8R8G8B8 ||
        image->getColorFormat() == ECF_A1R5G5B5,
        HAS_ALPHA);
    dimension2du optSize(
...
void CSoftwareTexture2::regenerateMipMapLevels(void* mipmapData)
{
  ...
  dimension2du origSize = OrigSize;
  for (i = 1; i < SOFTWARE_DRIVER_2_MIPMAPPING_MAX; ++i) {
    newSize = MipMap[i - 1]->getDimension();
    newSize.Width = s32_max(1, newSize.Width >> SOFTWARE_DRIVER_2_MIPMAPPING_SCALE);
    newSize.Height = s32_max(1, newSize.Height >> SOFTWARE_DRIVER_2_MIPMAPPING_SCALE);
    origSize.Width = s32_max(1, origSize.Width >> 1);
    origSize.Height = s32_max(1, origSize.Height >> 1);
    if (mipmapData) {
