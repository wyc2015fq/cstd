...
CCameraSceneNode::CCameraSceneNode(ISceneNode* parent, ISceneManager* mgr, s32 id,
    const float3 position, const float3 lookat)
...
void CCameraSceneNode::setTarget(const float3 pos)
{
  Target = pos;
  if (TargetAndRotationAreBound) {
    const float3 toTarget = Target - getAbsolutePosition();
...
void CCameraSceneNode::setRotation(const float3 rotation)
{
  if (TargetAndRotationAreBound) {
...
const float3 CCameraSceneNode::getTarget() const
{
  return Target;
}
//! sets the up vector of the camera
//! \param pos: New upvector of the camera.
void CCameraSceneNode::setUpVector(const float3 pos)
{
  UpVector = pos;
}
//! Gets the up vector of the camera.
//! \return Returns the up vector of the camera.
const float3 CCameraSceneNode::getUpVector() const
{
  return UpVector;
}
...
const F3AABBOX& CCameraSceneNode::getBoundingBox() const
{
