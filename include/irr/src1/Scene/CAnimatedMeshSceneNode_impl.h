...
void CAnimatedMeshSceneNode::setCurrentFrame(f32 frame)
{
  ...
  CurrentFrameNr = clamp(frame, (f32)StartFrame, (f32)EndFrame);
...
IMesh* CAnimatedMeshSceneNode::getMeshForCurrentFrame()
{
  if (Mesh->getMeshType() != EAMT_SKINNED) {
    ...
    s32 frameBlend = (s32)(fract(getFrameNr()) * 1000.f);
    ...
    if (JointMode == EJUOR_READ) { //read from mesh
      ...
      //---slow---
      for (int n = 0; n < JointChildSceneNodes_size; ++n)
...
void CAnimatedMeshSceneNode::render()
{
  ...
  if (DebugDataVisible && PassCount == 1) {
    ...
    if (DebugDataVisible & EDS_BBOX_BUFFERS) {
      for (int g = 0; g < m->getMeshBufferCount(); ++g) {
        const IMeshBuffer* mb = m->getMeshBuffer(g);
        if (Mesh->getMeshType() == EAMT_SKINNED) {
          driver->setTransform(ETS_WORLD, AbsoluteTransformation * ((SSkinMeshBuffer*)mb)->Transformation);
        }
        driver->draw3DBox(mb->getBoundingBox(), SColor(255, 190, 128, 128));
      }
    }
    // show skeleton
    if (DebugDataVisible & EDS_SKELETON) {
      ...
      if (Mesh->getMeshType() == EAMT_MD3) {
        ...
        if (taglist) {
          for (int ts = 0; ts != taglist->size(); ++ts) {
            (*taglist)[ts].setto(matr);
            driver->setTransform(ETS_WORLD, matr);
            for (int a = 0; a != arrowMesh->getMeshBufferCount(); ++a) {
...
bool CAnimatedMeshSceneNode::setFrameLoop(s32 begin, s32 end)
{
  ...
  if (end < begin) {
    StartFrame = s32_clamp(end, 0, maxFrameCount);
    EndFrame = s32_clamp(begin, StartFrame, maxFrameCount);
  }
  else {
    StartFrame = s32_clamp(begin, 0, maxFrameCount);
    EndFrame = s32_clamp(end, StartFrame, maxFrameCount);
  }
...
const F3AABBOX& CAnimatedMeshSceneNode::getBoundingBox() const
{
  return Box;
}
...
SMaterial& CAnimatedMeshSceneNode::getMaterial(u32 i)
{
  if (i >= Materials_size) {
...
IBoneSceneNode* CAnimatedMeshSceneNode::getJointNode(const c8* jointName)
{
  ...
  if ((s32)JointChildSceneNodes_size <= number) {
...
IBoneSceneNode* CAnimatedMeshSceneNode::getJointNode(u32 jointID)
{
  ...
  checkJoints();
  if (JointChildSceneNodes_size <= jointID) {
...
const SMD3QuaternionTag* CAnimatedMeshSceneNode::getMD3TagTransformation(const char* tagname)
{
...
{
  const u32 ttime = (u32)floor32(time * 1000.0f);
...
void CAnimatedMeshSceneNode::animateJoints(bool CalculateAbsolutePositions)
{
  ...
  if (Mesh && Mesh->getMeshType() == EAMT_SKINNED) {
    ...
    if (Transiting != 0.f) {
      ...
      if (PretransitingSave_size < JointChildSceneNodes_size) {
        for (int n = PretransitingSave_size; n < JointChildSceneNodes_size; ++n) {
...
void CAnimatedMeshSceneNode::beginTransition()
{
  ...
  if (TransitionTime != 0) {
    ...
    Transiting = reciprocal((f32)TransitionTime);
  }
  TransitingBlend = 0.f;
}
/*!
...
ISceneNode* CAnimatedMeshSceneNode::clone(ISceneNode* newParent, ISceneManager* newManager)
{
  ...
  if (newParent) {
    newNode->setParent(newParent);  // not in constructor because overload for updateAbsolutePosition won't be called
    newNode->drop();
  }
