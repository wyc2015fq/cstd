...
void COctreeSceneNode::render()
{
  ...
  const aabbox3d<float>& box = frust.getBoundingBox();
  switch (VertexType) {
  case EVT_STANDARD: {
    if (BoxBased) {
  ...
  case EVT_2TCOORDS: {
    ...
    const Octree<S3DVertex>::SIndexData* d = LightMapOctree->getIndexData();
    int i;
    for (i = 0; i < Materials_size; ++i) {
      if (0 == d[i].CurrentSize) {
        continue;
      ...
      if (transparent == isTransparentPass) {
        ...
        if (UseVBOs) {
          if (UseVisibilityAndVBOs) {
            u16* oldPointer = LightMapMeshes[i].Indices;
            const u32 oldSize = LightMapMeshes[i].Indices_size;
          ...
          driver->drawIndexedTriangleList(
              ...
              LightMapMeshes[i].Vertices_size,
              d[i].Indices, d[i].CurrentSize / 3);
      }
    }
    // for debug purposes only
  ...
  case EVT_TANGENTS: {
    ...
    const Octree<S3DVertex>::SIndexData* d =  TangentsOctree->getIndexData();
    int i;
    for (i = 0; i < Materials_size; ++i) {
      if (0 == d[i].CurrentSize) {
        continue;
      ...
      if (transparent == isTransparentPass) {
        driver->setMaterial(Materials[i]);
        driver->drawIndexedTriangleList(
            &TangentsMeshes[i].Vertices[0], TangentsMeshes[i].Vertices_size,
            d[i].Indices, d[i].CurrentSize / 3);
      }
    }
    // for debug purposes only
...
const F3AABBOX& COctreeSceneNode::getBoundingBox() const
{
  return Box;
}
//! creates the tree
...
bool COctreeSceneNode::createTree(IMesh* mesh)
{
  ...
  if (mesh->getMeshBufferCount()) {
    ...
    Materials.reallocate(Materials_size + meshReserve);
    switch (VertexType) {
    case EVT_STANDARD: {
      StdMeshes.reallocate(StdMeshes_size + meshReserve);
      for (i = 0; i < mesh->getMeshBufferCount(); ++i) {
        IMeshBuffer* b = mesh->getMeshBuffer(i);
        if (b->getVertexCount() && b->getIndexCount()) {
          Materials.push_back(b->getMaterial());
          StdMeshes.push_back(Octree<S3DVertex>::SMeshChunk());
          Octree<S3DVertex>::SMeshChunk& nchunk = StdMeshes.getLast();
          nchunk.MaterialId = Materials_size - 1;
          u32 v;
    ...
    break;
    case EVT_2TCOORDS: {
      LightMapMeshes.reallocate(LightMapMeshes_size + meshReserve);
      for (i = 0; i < mesh->getMeshBufferCount(); ++i) {
        IMeshBuffer* b = mesh->getMeshBuffer(i);
        if (b->getVertexCount() && b->getIndexCount()) {
          Materials.push_back(b->getMaterial());
          LightMapMeshes.push_back(Octree<S3DVertex>::SMeshChunk());
          Octree<S3DVertex>::SMeshChunk& nchunk = LightMapMeshes.getLast();
          nchunk.MaterialId = Materials_size - 1;
          if (UseVisibilityAndVBOs) {
      ...
      LightMapOctree = new Octree<S3DVertex>(LightMapMeshes, MinimalPolysPerNode);
      nodeCount = LightMapOctree->getNodeCount();
    }
    break;
    case EVT_TANGENTS: {
      TangentsMeshes.reallocate(TangentsMeshes_size + meshReserve);
      int i;
      for (i = 0; i < mesh->getMeshBufferCount(); ++i) {
        IMeshBuffer* b = mesh->getMeshBuffer(i);
        if (b->getVertexCount() && b->getIndexCount()) {
          Materials.push_back(b->getMaterial());
          TangentsMeshes.push_back(Octree<S3DVertex>::SMeshChunk());
          Octree<S3DVertex>::SMeshChunk& nchunk = TangentsMeshes.getLast();
          nchunk.MaterialId = Materials_size - 1;
          u32 v;
          nchunk.Vertices.reallocate(b->getVertexCount());
          switch (b->getVertexType()) {
          case EVT_STANDARD:
            for (v = 0; v < b->getVertexCount(); ++v) {
              ...
              nchunk.Vertices.push_back(S3DVertex(tmpV.Pos, tmpV.Color, tmpV.TCoords));
            }
            break;
          case EVT_2TCOORDS:
            for (v = 0; v < b->getVertexCount(); ++v) {
      ...
      TangentsOctree = new Octree<S3DVertex>(TangentsMeshes, MinimalPolysPerNode);
...
SMaterial& COctreeSceneNode::getMaterial(u32 i)
{
  if (i >= Materials_size) {
