...
public:
  ...
  ISceneNode* getMS3DJointNode(const c8* jointName);
  //! Deprecated command, please use getJointNode.
  ISceneNode* getXJointNode(const c8* jointName);
  ...
  const SMD3QuaternionTag* getMD3TagTransformation(const char* tagname);
...
private:
  ...
  IBoneSceneNode** JointChildSceneNodes;
  matrix4* PretransitingSave;
  // Quake3 Model
  struct SMD3Special {
    char* Tagname;
