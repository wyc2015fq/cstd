...
const F3AABBOX& CBoneSceneNode::getBoundingBox() const
{
  return Box;
}
/*
//! Returns the relative transformation of the scene node.
matrix4 CBoneSceneNode::getRelativeTransformation() const
{
  return matrix4(); // RelativeTransformation;
}
*/
...
void CBoneSceneNode::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options)
{
  ...
  const char* boneName = in->getAttributeAsString("BoneName");
  setName(boneName);
