...
CV_INLINE s32 CSceneNodeAnimatorFollowSpline::clamp(s32 idx, s32 size)
{
...
void CSceneNodeAnimatorFollowSpline::animateNode(ISceneNode* node, u32 timeMs)
{
  ...
  const s32 unwrappedIdx = floor32(dt);
  ...
  const f32 u =  pong ? 1.f - fract(dt) : fract(dt);
  const s32 idx = pong ? (pSize - 2) - (unwrappedIdx % (pSize - 1))
      : (PingPong ? unwrappedIdx % (pSize - 1)
          : unwrappedIdx % pSize);
  //const f32 u = 0.001f * fmodf( dt, 1000.0f );
  const float3 p0 = Points[ clamp(idx - 1, pSize) ];
  const float3 p1 = Points[ clamp(idx + 0, pSize) ];   // starting point
  const float3 p2 = Points[ clamp(idx + 1, pSize) ];   // end point
  const float3 p3 = Points[ clamp(idx + 2, pSize) ];
  // hermite polynomials
  ...
  // tangents
  const float3 t1 = (p2 - p0) * Tightness;
  const float3 t2 = (p3 - p1) * Tightness;
  // interpolated point
...
void CSceneNodeAnimatorFollowSpline::serializeAttributes(IAttributes* out, SAttributeReadWriteOptions* options) const
{
  ...
  for (i = 0; i < count; ++i) {
    char* tname = "Point";
    tname += (int)(i + 1);
    out->addVector3d(tname, i < Points_size ? Points[i] : float3(0, 0, 0));
  }
}
...
void CSceneNodeAnimatorFollowSpline::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options)
{
  ...
  for (i = 1; true; ++i) {
    char* pname = "Point";
    pname += i;
    if (in->existsAttribute(pname)) {
      Points.push_back(in->getAttributeAsVector3d(pname));
    }
    else {
      break;
    }
  ...
  if (options && (options->Flags & EARWF_FOR_EDITOR) &&
      Points_size > 2 && Points.getLast() == float3(0, 0, 0)) {
    Points.erase(Points_size - 1);
    if (Points_size > 2 && Points.getLast() == float3(0, 0, 0)) {
      Points.erase(Points_size - 1);
    }
  }
}
