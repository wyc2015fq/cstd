...
ISceneNodeAnimator* CDefaultSceneNodeAnimatorFactory::createSceneNodeAnimator(ESCENE_NODE_ANIMATOR_TYPE type, ISceneNode* target)
{
  ...
  case ESNAT_FLY_CIRCLE:
    anim = Manager->createFlyCircleAnimator(float3(0, 0, 0), 10);
    break;
  case ESNAT_FLY_STRAIGHT:
    anim = Manager->createFlyStraightAnimator(float3(0, 0, 0), float3(100, 100, 100), 10000, true);
    break;
  case ESNAT_FOLLOW_SPLINE: {
    float3 > points;
    points.push_back(float3(0, 0, 0));
    points.push_back(float3(10, 5, 10));
    anim = Manager->createFollowSplineAnimator(0, points);
  }
  break;
  case ESNAT_ROTATION:
    anim = Manager->createRotationAnimator(float3(0.3f, 0, 0));
    break;
  case ESNAT_TEXTURE: {
    ITexture* > textures;
...
{
  int i;
  for (i = 0; SceneNodeAnimatorTypeNames[i]; ++i)
