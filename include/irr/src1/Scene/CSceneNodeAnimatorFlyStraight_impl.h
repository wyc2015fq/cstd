...
//! constructor
CSceneNodeAnimatorFlyStraight::CSceneNodeAnimatorFlyStraight(const float3 startPoint,
    const float3 endPoint, u32 timeForWay,
...
void CSceneNodeAnimatorFlyStraight::animateNode(ISceneNode* node, u32 timeMs)
{
  ...
  if (!Loop && !PingPong && t >= TimeForWay) {
    ...
    float3 rel = Vector * phase * TimeFactor;
