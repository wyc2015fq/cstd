...
CTextSceneNode::CTextSceneNode(ISceneNode* parent, ISceneManager* mgr, s32 id,
    ...
    const float3 position, const char* text,
    SColor color)
...
void CTextSceneNode::render()
{
  ...
  position2di pos = Coll->getScreenCoordinatesFrom3DPosition(getAbsolutePosition(),
      SceneManager->getActiveCamera());
  IRECT r(pos, dimension2d<s32>(1, 1));
  Font->draw(Text, r, Color, true, true);
}
//! returns the axis aligned bounding box of this node
const F3AABBOX& CTextSceneNode::getBoundingBox() const
{
  return Box;
}
//! sets the text string
void CTextSceneNode::setText(const char* text)
{
  Text = text;
}
//! sets the color of the text
void CTextSceneNode::setTextColor(SColor color)
{
  Color = color;
}
    ...
    Font(0), ColorTop(colorTop), ColorBottom(shade_bottom), Mesh(0)
{
  ...
  if (font) {
    ...
    if (font->getType() == EGFT_BITMAP) {
      ...
      Mesh = new SMesh();
      int i;
      for (i = 0; i < Font->getSpriteBank()->getTextureCount(); ++i) {
...
//! sets the text string
void CBillboardTextSceneNode::setText(const char* text)
{
  if (!Mesh) {
    return;
  }
  ...
  const  IRECT > & sourceRects = Font->getSpriteBank()->getPositions();
  const  SGUISprite > & sprites = Font->getSpriteBank()->getSprites();
  f32 dim[2];
  f32 tex[4];
  u32 i;
  for (i = 0; i != Text_size; ++i) {
    ...
    dim[0] = reciprocal((f32) Font->getSpriteBank()->getTexture(texno)->getSize().Width);
    dim[1] = reciprocal((f32) Font->getSpriteBank()->getTexture(texno)->getSize().Height);
    const IRECT& s = sourceRects[rectno];
    // add space for letter to buffer
    SMeshBuffer* buf = (SMeshBuffer*)Mesh->getMeshBuffer(texno);
    u32 firstInd = buf->Indices_size;
    u32 firstVert = buf->Vertices_size;
...
//! returns the axis aligned bounding box of this node
const F3AABBOX& CBillboardTextSceneNode::getBoundingBox() const
{
  return BBox;
}
...
SMaterial& CBillboardTextSceneNode::getMaterial(u32 i)
{
...
//! sets the color of the text
void CBillboardTextSceneNode::setTextColor(SColor color)
{
  Color = color;
}
//! Set the color of all vertices of the billboard
//! \param overallColor: the color to set
void CBillboardTextSceneNode::setColor(const SColor& overallColor)
{
  if (!Mesh) {
    return;
  }
...
void CBillboardTextSceneNode::setColor(const SColor& topColor, const SColor& bottomColor)
{
  if (!Mesh) {
    return;
  }
...
void CBillboardTextSceneNode::getColor(SColor& topColor, SColor& bottomColor) const
{
  topColor = ColorTop;
