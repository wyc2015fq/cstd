...
void CSceneNodeAnimatorCameraFPS::animateNode(ISceneNode* node, u32 timeMs)
{
  ...
  // Update rotation
  float3 target = (camera->getTarget() - camera->getAbsolutePosition());
  float3 relativeRotation = target.getHorizontalAngle();
  if (CursorControl) {
    ...
    vector2d<u32> mousepos(u32(CursorControl->getPosition().X), u32(CursorControl->getPosition().Y));
    rect<u32> screenRect(0, 0, driver->getScreenSize().Width, driver->getScreenSize().Height);
  ...
  float3 movedir = target;
  matrix4 mat;
  mat.setRotationDegrees(float3(relativeRotation.X, relativeRotation.Y, 0));
  mat.transformVect(target);
  if (NoVerticalMovement) {
    mat.setRotationDegrees(float3(0, relativeRotation.Y, 0));
...
void CSceneNodeAnimatorCameraFPS::setKeyMap(const SKeyMap > & keymap)
{
  KeyMap = keymap;
}
const SKeyMap > & CSceneNodeAnimatorCameraFPS::getKeyMap() const
{
  return KeyMap;
}
