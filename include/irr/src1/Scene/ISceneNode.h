...
typedef list<ISceneNode*> ISceneNodeList;
//! Typedef for list of scene node animators
typedef list<ISceneNodeAnimator*> ISceneNodeAnimatorList;
...
class ISceneNode : public IAttributeExchangingObject
{
  ...
  */
  ...
  const F3AABBOX getTransformedBoundingBox() const {
    F3AABBOX box = getBoundingBox();
  ...
  matrix4 getRelativeTransformation() const {
    ...
    if (RelativeScale != float3(1.f, 1.f, 1.f)) {
      matrix4 smat;
  ...
  */
  ...
  const list<ISceneNodeAnimator*>& getAnimators() const {
    return Animators;
  }
  ...
  /** A bitwise OR of the types from @ref E_DEBUG_SCENE_TYPE.
  ...
  @ref E_DEBUG_SCENE_TYPE that are currently visible. */
  ...
  const list<ISceneNode*>& getChildren() const {
    return Children;
  }
