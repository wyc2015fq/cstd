...
public:
  ...
  ISceneNodeAnimator* createSceneNodeAnimator(ESCENE_NODE_ANIMATOR_TYPE type, ISceneNode* target) = 0;
  //! creates a scene node animator based on its type name
  /** \param typeName: Type of the scene node animator to add.
  \param target: Target scene node of the new animator.
  \return Returns pointer to the new scene node animator or null if not successful. You need to
  drop this pointer after calling this, see IReferenceCounted::drop() for details. */
  ISceneNodeAnimator* createSceneNodeAnimator(const c8* typeName, ISceneNode* target) = 0;
  //! returns amount of scene node animator types this factory is able to create
  u32 getCreatableSceneNodeAnimatorTypeCount() = 0;
  ...
  ESCENE_NODE_ANIMATOR_TYPE getCreateableSceneNodeAnimatorType(u32 idx) = 0;
  //! returns type name of a createable scene node animator type
  /** \param idx: Index of scene node animator type in this factory. Must be a value between 0 and
  getCreatableSceneNodeAnimatorTypeCount() */
  const c8* getCreateableSceneNodeAnimatorTypeName(u32 idx) = 0;
  //! returns type name of a createable scene node animator type
  ...
  const c8* getCreateableSceneNodeAnimatorTypeName(ESCENE_NODE_ANIMATOR_TYPE type) = 0;
};
#endif
