...
CSkyDomeSceneNode::CSkyDomeSceneNode(ITexture* sky, u32 horiRes, u32 vertRes,
...
void CSkyDomeSceneNode::generateMesh()
{
  ...
  const f32 azimuth_step = (PI * 2.f) / HorizontalResolution;
  ...
  const f32 elevation_step = SpherePercentage * HALF_PI / (f32)VerticalResolution;
...
const F3AABBOX& CSkyDomeSceneNode::getBoundingBox() const
{
...
SMaterial& CSkyDomeSceneNode::getMaterial(u32 i)
{
