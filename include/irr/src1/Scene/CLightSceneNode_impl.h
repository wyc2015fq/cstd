...
CLightSceneNode::CLightSceneNode(ISceneNode* parent, ISceneManager* mgr, s32 id,
    const float3 position, SColorf color, f32 radius)
  : ILightSceneNode(parent, mgr, id, position), DriverLightIndex(-1), LightIsOn(true)
{
  ...
  LightData.SpecularColor = color.getInterpolated(SColor(255, 255, 255, 255), 0.7f);
  setRadius(radius);
}
//! pre render event
...
void CLightSceneNode::render()
{
  ...
  if (DebugDataVisible & EDS_BBOX) {
    ...
    case ELT_DIRECTIONAL:
      driver->draw3DLine(float3(0.f, 0.f, 0.f),
...
const F3AABBOX& CLightSceneNode::getBoundingBox() const
{
  return BBox;
}
...
void CLightSceneNode::doLightRecalc()
{
  if ((LightData.Type == ELT_SPOT) || (LightData.Type == ELT_DIRECTIONAL)) {
    LightData.Direction = float3(.0f, .0f, 1.0f);
