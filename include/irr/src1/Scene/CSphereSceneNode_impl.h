...
const F3AABBOX& CSphereSceneNode::getBoundingBox() const
{
...
SMaterial& CSphereSceneNode::getMaterial(u32 i)
{
  if (i > 0 || !Mesh) {
...
void CSphereSceneNode::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options)
{
  ...
  Radius = max_(Radius, 0.0001f);
  if (!equals(Radius, oldRadius) || PolyCountX != oldPolyCountX || PolyCountY != oldPolyCountY) {
    if (Mesh) {
      Mesh->drop();
    }
    Mesh = SceneManager->getGeometryCreator()->createSphereMesh(Radius, PolyCountX, PolyCountY);
