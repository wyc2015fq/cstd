...
CTerrainSceneNode::CTerrainSceneNode(ISceneNode* parent, ISceneManager* mgr,
    IFileSystem* fs, s32 id, s32 maxLOD, E_TERRAIN_PATCH_SIZE patchSize,
...
bool CTerrainSceneNode::loadHeightMap(IReadFile* file, SColor vertexColor,
    s32 smoothFactor)
{
  if (!file) {
    return false;
  ...
  for (s32 x = 0; x < TerrainData.Size; ++x) {
    ...
    for (s32 z = 0; z < TerrainData.Size; ++z) {
      S3DVertex& vertex = static_cast<S3DVertex*>(mb->getVertexBuffer())[index++];
...
bool CTerrainSceneNode::loadHeightMapRAW(IReadFile* file,
    ...
    s32 width, SColor vertexColor, s32 smoothFactor)
{
  if (!file) {
    return false;
  }
  ...
  if (!width) {
    TerrainData.Size = floor32(sqrtf((f32)(filesize / bytesPerPixel)));
  }
  else {
...
SMaterial& CTerrainSceneNode::getMaterial(u32 i)
{
...
void CTerrainSceneNode::setScale(const float3 scale)
{
...
void CTerrainSceneNode::setRotation(const float3 rotation)
{
...
void CTerrainSceneNode::setRotationPivot(const float3 pivot)
{
...
void CTerrainSceneNode::setPosition(const float3 newpos)
{
...
void CTerrainSceneNode::preRenderCalculationsIfNeeded()
{
  ...
  const float3 cameraPosition = camera->getAbsolutePosition();
  const float3 cameraRotation = F3LINE(cameraPosition, camera->getTarget()).getVector().getHorizontalAngle();
  float3 cameraUp = camera->getUpVector();
  cameraUp.normalize();
...
void CTerrainSceneNode::render()
{
  ...
  if (DebugDataVisible) {
    ...
    if (DebugDataVisible & EDS_BBOX) {
      driver->draw3DBox(TerrainData.BoundingBox, SColor(255, 255, 255, 255));
    }
    const s32 count = TerrainData.PatchCount * TerrainData.PatchCount;
    s32 visible = 0;
    if (DebugDataVisible & EDS_BBOX_BUFFERS) {
      for (s32 j = 0; j < count; ++j) {
        driver->draw3DBox(TerrainData.Patches[j].BoundingBox, SColor(255, 255, 0, 0));
...
const F3AABBOX& CTerrainSceneNode::getBoundingBox() const
{
  return TerrainData.BoundingBox;
}
//! Return the bounding box of a patch
const F3AABBOX& CTerrainSceneNode::getBoundingBox(s32 patchX, s32 patchZ) const
{
...
void CTerrainSceneNode::getMeshBufferForLOD(IDynamicMeshBuffer& mb, s32 LOD) const
{
  ...
  LOD = clamp(LOD, 0, TerrainData.MaxLOD - 1);
...
s32 CTerrainSceneNode::getIndicesForPatch(u32* indices, s32 patchX, s32 patchZ, s32 LOD)
{
...
s32 CTerrainSceneNode::getCurrentLODOfPatches(s32* LODs) const
{
  s32 numLODs;
  LODs.clear();
  const s32 count = TerrainData.PatchCount * TerrainData.PatchCount;
  ...
  for (s32 x = 0; x < TerrainData.Size; ++x) {
    ...
    for (s32 z = 0; z < TerrainData.Size; ++z) {
      ...
      if (RenderBuffer->getVertexType() == EVT_2TCOORDS) {
        if (resolution2 == 0) {
          ((S3DVertex&)RenderBuffer->getVertexBuffer()[index]).TCoords2 = RenderBuffer->getVertexBuffer()[index].TCoords;
        }
        else {
          ((S3DVertex&)RenderBuffer->getVertexBuffer()[index]).TCoords2.X = 1.f - x2val;
          ((S3DVertex&)RenderBuffer->getVertexBuffer()[index]).TCoords2.Y = z2val;
        }
      }
      ++index;
      zval += resBySize;
    ...
    const s32 PatchIndex, u32 vX, u32 vZ) const
{
  ...
  else if (vZ == (u32)TerrainData.CalcPatchSize) { // bottom border
  ...
  else if (vX == (u32)TerrainData.CalcPatchSize) { // right border
...
void CTerrainSceneNode::setCurrentLODOfPatches(const s32* lodarray)
{
  const s32 count = TerrainData.PatchCount * TerrainData.PatchCount;
  for (s32 i = 0; i < count; ++i) {
...
f32 CTerrainSceneNode::getHeight(f32 x, f32 z) const
{
  ...
  if (X >= 0 && X < TerrainData.Size - 1 &&
    ...
    const S3DVertex* Vertices = (const S3DVertex*)Mesh->getMeshBuffer(0)->getVertices();
    const float3 a = Vertices[X * TerrainData.Size + Z].Pos;
    const float3 b = Vertices[(X + 1) * TerrainData.Size + Z].Pos;
    const float3 c = Vertices[X * TerrainData.Size + (Z + 1)].Pos;
    const float3 d = Vertices[(X + 1) * TerrainData.Size + (Z + 1)].Pos;
    ...
    SAttributeReadWriteOptions* options) const
{
  ...
  out->addString("Heightmap", HeightmapFile);
...
{
  char* newHeightmap = in->getAttributeAsString("Heightmap");
  ...
  if (newHeightmap_size != 0 && newHeightmap != HeightmapFile) {
    IReadFile* file = FileSystem->createAndOpenFile(newHeightmap);
    if (file) {
      loadHeightMap(file, SColor(255, 255, 255, 255), smoothFactor);
      file->drop();
    }
    else {
      os::Printer::log("could not open heightmap", newHeightmap);
    }
  }
  // set possible new scale
  if (equals(tcoordScale1, 0.f)) {
    tcoordScale1 = 1.0f;
  }
  if (equals(tcoordScale2, 0.f)) {
    tcoordScale2 = 1.0f;
  }
  if (!equals(tcoordScale1, TCoordScale1) ||
      !equals(tcoordScale2, TCoordScale2)) {
...
ISceneNode* CTerrainSceneNode::clone(ISceneNode* newParent, ISceneManager* newManager)
{
  ...
  // (temporary solution)
  // load file
  IReadFile* file = FileSystem->createAndOpenFile(HeightmapFile);
  if (file) {
    nb->loadHeightMap(file, SColor(255, 255, 255, 255), 0);
    file->drop();
  }
  // scale textures
