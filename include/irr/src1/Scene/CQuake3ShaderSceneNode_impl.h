...
{
#ifdef _DEBUG
  char* dName = "CQuake3ShaderSceneNode ";
  dName += Shader->name;
  setDebugName(dName);
#endif
  // name the Scene Node
...
void CQuake3ShaderSceneNode::cloneBuffer(SMeshBuffer* dest, const SMeshBufferLightMap* buffer, bool translateCenter)
{
  ...
  const u32 vsize = buffer->Vertices_size;
  dest->Vertices.set_used(vsize);
  int i;
  for (i = 0; i != vsize; ++i) {
    const S3DVertex& src = buffer->Vertices[i];
...
void CQuake3ShaderSceneNode::loadTextures(IFileSystem* fileSystem)
{
  ...
  Q3Texture.clear();
  for (i = 0; i != Shader->VarGroup->VariableGroup_size; ++i) {
  ...
  for (i = 0; i < Shader->VarGroup->VariableGroup_size; ++i) {
    ...
    const char* mapname = group->get("map");
    if (0 == mapname_size) {
      continue;
    }
    ...
    if (mapname == "$lightmap") {
      ...
      pos = 0;
      quake3::getTextures(Q3Texture [i].Texture, mapname, pos, fileSystem, driver);
    }
  }
  // get anim map
  for (i = 0; i < Shader->VarGroup->VariableGroup_size; ++i) {
    if (Q3Texture [i].Texture_size) {
      continue;
    }
    group = Shader->getGroup(i);
    const char* animmap = group->get("animmap");
    if (0 == animmap_size) {
      continue;
    }
    // first parameter is frequency
    pos = 0;
    Q3Texture [i].TextureFrequency =  max_(0.0001f, quake3::getAsFloat(animmap, pos));
    quake3::getTextures(Q3Texture [i].Texture, animmap, pos, fileSystem, driver);
  }
  // get clamp map
  for (i = 0; i < Shader->VarGroup->VariableGroup_size; ++i) {
    if (Q3Texture [i].Texture_size) {
    ...
    group = Shader->getGroup(i);
    const char* clampmap = group->get("clampmap");
    if (0 == clampmap_size) {
      continue;
    }
    Q3Texture [i].TextureAddressMode = ETC_CLAMP_TO_EDGE;
    pos = 0;
    quake3::getTextures(Q3Texture [i].Texture, clampmap, pos, fileSystem, driver);
  }
  if (mipmap & 2) {
...
E_SCENE_NODE_RENDER_PASS CQuake3ShaderSceneNode::getRenderStage() const
{
  ...
  */
  if (group->isDefined("sort", "opaque")) {
  ...
  else if (group->isDefined("sort", "additive")) {
    ...
    for (int stage = 2; stage < Shader->VarGroup->VariableGroup_size; ++stage) {
      if (0 == Q3Texture [ stage ].Texture_size) {
        continue;
      }
      group = Shader->getGroup(stage);
      quake3::SBlendFunc blendfunc(EMFN_MODULATE_1X);
...
void CQuake3ShaderSceneNode::render()
{
  ...
  // generic stage
  group = Shader->getGroup(1);
  material.BackfaceCulling = quake3::getCullingFunction(group->get("cull"));
  ...
  matrix4 textureMatrix(matrix4::EM4CONST_NOTHING);
  for (int stage = 1; stage < Shader->VarGroup->VariableGroup_size; ++stage) {
    ...
    ITexture* tex = q.Texture_size ? q.Texture [ q.TextureIndex ] : 0;
    if (0 == tex) {
      continue;
    }
    // current stage
  ...
  if (DebugDataVisible & EDS_NORMALS) {
    ...
    //f32 colCycle = 270.f / (f32) s32_max ( mb->getVertexCount() - 1, 1 );
    int i;
    for (i = 0; i != mb->getVertexCount(); ++i) {
      // Align to v->normal
      m2.buildRotateFromTo(float3(0.f, 1.f, 0), v->Normal);
      ...
      */
      switch (i) {
      case 0:
        deb_m.EmissiveColor.set(0xFFFFFFFF);
        break;
      case 1:
        deb_m.EmissiveColor.set(0xFFFF0000);
        break;
      case 2:
        deb_m.EmissiveColor.set(0xFF00FF00);
        break;
      case 3:
        deb_m.EmissiveColor.set(0xFF0000FF);
        break;
      default:
        deb_m.EmissiveColor = v->Color;
        break;
      }
      driver->setMaterial(deb_m);
      for (int a = 0; a != mesh->getMeshBufferCount(); ++a) {
  ...
  if (DebugDataVisible & EDS_BBOX) {
    ...
    driver->setMaterial(deb_m);
    driver->setTransform(ETS_WORLD, AbsoluteTransformation);
    driver->draw3DBox(getBoundingBox(), SColor(255, 255, 0, 0));
  }
}
...
*/
void CQuake3ShaderSceneNode::deformvertexes_wave(f32 dt, quake3::SModifierFunction& function)
{
  function.wave = reciprocal(function.wave);
  const f32 phase = function.phase;
  const u32 vsize = Original->Vertices_size;
  int i;
  for (i = 0; i != vsize; ++i) {
    const S3DVertex& src = Original->Vertices[i];
...
*/
void CQuake3ShaderSceneNode::deformvertexes_move(f32 dt, quake3::SModifierFunction& function)
{
  function.wave = reciprocal(function.wave);
  const f32 f = function.evaluate(dt);
  const u32 vsize = Original->Vertices_size;
...
*/
void CQuake3ShaderSceneNode::deformvertexes_normal(f32 dt, quake3::SModifierFunction& function)
{
  function.func = quake3::SINUS;
  const u32 vsize = Original->Vertices_size;
  int i;
  for (i = 0; i != vsize; ++i) {
    const S3DVertex& src = Original->Vertices[i];
...
*/
void CQuake3ShaderSceneNode::deformvertexes_bulge(f32 dt, quake3::SModifierFunction& function)
{
  function.func = quake3::SINUS;
...
/*!
  ...
  be square.
*/
void CQuake3ShaderSceneNode::deformvertexes_autosprite(f32 dt, quake3::SModifierFunction& function)
{
  u32 vsize = Original->Vertices_size;
  u32 g;
  u32 i;
  const float3 camPos = SceneManager->getActiveCamera()->getPosition();
  S3DVertex* dv = MeshBuffer->Vertices;
  const S3DVertex* vin = Original->Vertices;
  matrix4 lookat(matrix4::EM4CONST_NOTHING);
  quaternion q;
  for (i = 0; i < vsize; i += 4) {
    // quad-plane
    float3 center = 0.25f * (vin[i + 0].Pos + vin[i + 1].Pos + vin[i + 2].Pos + vin[i + 3].Pos);
    float3 forward = camPos - center;
...
/*!
*/
void CQuake3ShaderSceneNode::deformvertexes_autosprite2(f32 dt, quake3::SModifierFunction& function)
{
  u32 vsize = Original->Vertices_size;
  u32 g;
  u32 i;
  const float3 camPos = SceneManager->getActiveCamera()->getAbsolutePosition();
  S3DVertex* dv = MeshBuffer->Vertices;
  const S3DVertex* vin = Original->Vertices;
  matrix4 lookat(matrix4::EM4CONST_NOTHING);
  sortaxis > axis;
...
void CQuake3ShaderSceneNode::vertextransform_rgbgen(f32 dt, quake3::SModifierFunction& function)
{
  ...
  case quake3::WAVE: {
    ...
    s32 value = clamp(floor32(f), 0, 255);
  ...
  break;
  case quake3::CONSTANT: {
    //rgbgen const ( x y z )
    SColorf cf(function.x, function.y, function.z);
    SColor col = cf.toSColor();
    for (i = 0; i != vsize; ++i) {
...
*/
void CQuake3ShaderSceneNode::vertextransform_alphagen(f32 dt, quake3::SModifierFunction& function)
{
  u32 i;
  const u32 vsize = Original->Vertices_size;
  ...
  case quake3::LIGHTINGSPECULAR: {
    ...
    const f32* m = view;
    for (i = 0; i != vsize; ++i) {
      const float3 n = Original->Vertices[i].Normal;
...
*/
void CQuake3ShaderSceneNode::transformtex(const matrix4& m, const u32 addressMode)
{
  u32 i;
  const u32 vsize = MeshBuffer->Vertices_size;
  f32 tx1;
  f32 ty1;
  if (addressMode) {
    for (i = 0; i != vsize; ++i) {
...
*/
void CQuake3ShaderSceneNode::animate(u32 stage, matrix4& texture)
{
  const quake3::SVarGroup* group = Shader->getGroup(stage);
  // select current texture
  SQ3Texture& q3Tex = Q3Texture [ stage ];
  if (q3Tex.TextureFrequency != 0.f) {
    s32 v = floor32(TimeAbs * q3Tex.TextureFrequency);
    q3Tex.TextureIndex = v % q3Tex.Texture_size;
  }
  matrix4 m2;
  quake3::SModifierFunction function;
  f32 f[16];
  // walk group for all modifiers
  for (int g = 0; g != group->Variable_size; ++g) {
    ...
    u32 pos = 0;
    function.masterfunc0 = (quake3::eQ3ModifierFunction) quake3::isEqual(v.name, pos, modifierList, 6);
    if (quake3::UNKNOWN == function.masterfunc0) {
      continue;
    }
    ...
    pos = 0;
    function.masterfunc1 = (quake3::eQ3ModifierFunction) quake3::isEqual(v.content, pos, funclist, 22);
    if (function.masterfunc1 != quake3::UNKNOWN) {
      function.masterfunc1 = (quake3::eQ3ModifierFunction)((u32) function.masterfunc1 + quake3::FUNCTION2 + 1);
    }
    switch (function.masterfunc1) {
    case quake3::SCROLL:
      // tcMod scroll <sSpeed> <tSpeed>
      f[0] = quake3::getAsFloat(v.content, pos) * TimeAbs;
      f[1] = quake3::getAsFloat(v.content, pos) * TimeAbs;
      m2.setTextureTranslate(f[0], f[1]);
      break;
    case quake3::SCALE:
      // tcmod scale <sScale> <tScale>
      f[0] = quake3::getAsFloat(v.content, pos);
      f[1] = quake3::getAsFloat(v.content, pos);
      m2.setTextureScale(f[0], f[1]);
      break;
    case quake3::ROTATE:
      // tcmod rotate <degress per second>
      m2.setTextureRotationCenter(quake3::getAsFloat(v.content, pos) *
          DEGTORAD *
          TimeAbs
                                 );
      break;
    case quake3::TRANSFORM:
      // tcMod <transform> <m00> <m01> <m10> <m11> <t0> <t1>
      memset(f, 0, sizeof(f));
      f[10] = f[15] = 1.f;
      f[0] = quake3::getAsFloat(v.content, pos);
      f[1] = quake3::getAsFloat(v.content, pos);
      f[4] = quake3::getAsFloat(v.content, pos);
      f[5] = quake3::getAsFloat(v.content, pos);
      f[8] = quake3::getAsFloat(v.content, pos);
      f[9] = quake3::getAsFloat(v.content, pos);
      m2.setM(f);
      break;
    case quake3::STRETCH: // stretch
    case quake3::TURBULENCE: // turb
    case quake3::WAVE: // wave
    case quake3::IDENTITY: // identity
    case quake3::IDENTITYLIGHTING:
    case quake3::VERTEX: // vertex
    case quake3::MOVE:
    case quake3::CONSTANT: {
      // turb == sin, default == sin
      function.func = quake3::SINUS;
      if (function.masterfunc0 == quake3::DEFORMVERTEXES) {
        switch (function.masterfunc1) {
        case quake3::WAVE:
          // deformvertexes wave
        ...
        case quake3::MOVE:
          //deformvertexes move
          function.x = quake3::getAsFloat(v.content, pos);
          function.z = quake3::getAsFloat(v.content, pos);
          function.y = quake3::getAsFloat(v.content, pos);
          break;
        default:
          break;
        }
      ...
      case quake3::MOVE:
        ...
        case quake3::RGBGEN:
          ...
          if (function.rgbgen == quake3::CONSTANT) {
            quake3::isEqual(v.content, pos, groupToken, 2);
            function.x = quake3::getAsFloat(v.content, pos);
            function.y = quake3::getAsFloat(v.content, pos);
            function.z = quake3::getAsFloat(v.content, pos);
          }
          ...
          break;
        case quake3::ALPHAGEN:
          function.alphagen = function.masterfunc1;
          if (function.alphagen == quake3::CONSTANT) {
            function.x = quake3::getAsFloat(v.content, pos);
          }
    ...
    case quake3::ENVIRONMENT:
      ...
      break;
    case quake3::DOLLAR_LIGHTMAP:
      // map == lightmap, tcgen == lightmap
      function.tcgen = quake3::LIGHTMAP;
      break;
    case quake3::BULGE:
    ...
    case quake3::NORMAL:
      ...
      function.amp = quake3::getAsFloat(v.content, pos);
      function.frequency = quake3::getAsFloat(v.content, pos);
      deformvertexes_normal(TimeAbs, function);
      break;
    case quake3::AUTOSPRITE:
      // deformvertexes autosprite
      deformvertexes_autosprite(TimeAbs, function);
      break;
    case quake3::AUTOSPRITE2:
      // deformvertexes autosprite2
...
const F3AABBOX& CQuake3ShaderSceneNode::getBoundingBox() const
{
  return MeshBuffer->getBoundingBox();
}
u32 CQuake3ShaderSceneNode::getMaterialCount() const
{
  return Q3Texture_size;
}
SMaterial& CQuake3ShaderSceneNode::getMaterial(u32 i)
{
