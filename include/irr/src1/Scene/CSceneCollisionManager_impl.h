...
ISceneNode* CSceneCollisionManager::getSceneNodeFromScreenCoordinatesBB(
    const position2di& pos, s32 idBitMask, bool noDebugObjects, ISceneNode* root)
{
  const F3LINE ln = getRayFromScreenCoordinates(pos, 0);
  if (ln.start == ln.end) {
    return 0;
  }
...
ISceneNode* CSceneCollisionManager::getSceneNodeFromRayBB(
    const F3LINE& ray,
    s32 idBitMask, bool noDebugObjects, ISceneNode* root)
{
  ISceneNode* best = 0;
  f32 dist = FLT_MAX;
  F3LINE truncatableRay(ray);
  ...
  for (; it != children.end(); ++it) {
    ...
    if (current->isVisible()) {
      if ((noDebugObjects ? !current->isDebugObject() : true) &&
        ...
        const aabbox3df& objectBox = current->getBoundingBox();
        ...
        if (objectBox.isPointInside(objectRay.start)) {
          ...
          const float3 dir = (objectRay.end - objectRay.start).normalize();
          const float3 minDist = (objectBox.MinEdge - objectRay.start) / dir;
          const float3 maxDist = (objectBox.MaxEdge - objectRay.start) / dir;
          const float3 realMin(min_(minDist.X, maxDist.X), min_(minDist.Y, maxDist.Y), min_(minDist.Z, maxDist.Z));
          const float3 realMax(max_(minDist.X, maxDist.X), max_(minDist.Y, maxDist.Y), max_(minDist.Z, maxDist.Z));
          const f32 minmax = min_3(realMax.X, realMax.Y, realMax.Z);
          // nearest distance to intersection
          const f32 maxmin = max_3(realMin.X, realMin.Y, realMin.Z);
          ...
          for (s32 face = 0; face < 6; ++face) {
            ...
            if (facePlane.getIntersectionWithLine(ray.start, rayVector, intersection)) {
              ...
              if (toIntersectionSq < outbestdistance) {
                ...
                f32 distToBorder = max_3(min_(abs_(objectBox.MinEdge.X - intersection.X), abs_(objectBox.MaxEdge.X - intersection.X)),
    ...
    triangle3df& outBestTriangle)
{
  ...
  for (; it != children.end(); ++it) {
    ...
    if (selector && current->isVisible() &&
      ...
      const aabbox3df& box = current->getBoundingBox();
      float3 candidateCollisionPoint;
    ...
    ICameraSceneNode* camera, s32 idBitMask, bool noDebugObjects)
{
  ...
  const float3 start = camera->getAbsolutePosition();
  float3 end = camera->getTarget();
  end = start + ((end - start).normalize() * camera->getFarValue());
  return getSceneNodeFromRayBB(F3LINE(start, end), idBitMask, noDebugObjects);
}
//! Finds the collision point of a line and lots of triangles, if there is one.
bool CSceneCollisionManager::getCollisionPoint(const F3LINE& ray,
    ITriangleSelector* selector, float3 outIntersection,
    triangle3df& outTriangle,
    ISceneNode* outNode)
{
  if (!selector) {
  ...
  s32 cnt = 0;
  selector->getTriangles(Triangles, totalcnt, cnt, ray);
  const float3 linevect = ray.getVector().normalize();
  float3 intersection;
  f32 nearest = FLT_MAX;
  bool found = false;
  const f32 raylength = ray.getLengthSQ();
  const f32 minX = min_(ray.start.X, ray.end.X);
  const f32 maxX = max_(ray.start.X, ray.end.X);
  const f32 minY = min_(ray.start.Y, ray.end.Y);
  const f32 maxY = max_(ray.start.Y, ray.end.Y);
  const f32 minZ = min_(ray.start.Z, ray.end.Z);
  const f32 maxZ = max_(ray.start.Z, ray.end.Z);
  for (s32 i = 0; i < cnt; ++i) {
...
float3 CSceneCollisionManager::getCollisionResultPosition(
    ITriangleSelector* selector,
    const float3 position, const float3 radius,
...
{
  const plane3d<f32> trianglePlane = triangle.getPlane();
  ...
  if (!foundCollision) {
    float3 velocity = colData->velocity;
    float3 base = colData->basePoint;
    ...
    // check against edges:
    // p1 --- p2
    float3 edge = triangle.pointB - triangle.pointA;
    float3 baseToVertex = triangle.pointA - base;
...
//! Collides a moving ellipsoid with a 3d world with gravity and returns
//! the resulting new position of the ellipsoid.
float3 CSceneCollisionManager::collideEllipsoidWithWorld(
    ITriangleSelector* selector, const float3 position,
    ...
    ISceneNode* outNode)
{
  ...
  float3 eSpacePosition = colData.R3Position / colData.eRadius;
  float3 eSpaceVelocity = colData.R3Velocity / colData.eRadius;
  // iterate until we have our final position
  float3 finalPos = collideWithWorld(
      0, colData, eSpacePosition, eSpaceVelocity);
  outFalling = false;
  // add gravity
  if (gravity != float3(0, 0, 0)) {
...
float3 CSceneCollisionManager::collideWithWorld(s32 recursionDepth,
    SCollisionData& colData, float3 pos, float3 vel)
{
  ...
  F3AABBOX box(colData.R3Position);
  ...
  scaleMatrix.setScale(
      float3(1.0f / colData.eRadius.X,
          1.0f / colData.eRadius.Y,
          1.0f / colData.eRadius.Z));
  s32 triangleCnt = 0;
  colData.selector->getTriangles(Triangles, totalTriangleCnt, triangleCnt, box, &scaleMatrix);
  ...
  const float3 destinationPoint = pos + vel;
  float3 newBasePoint = pos;
  ...
  const float3 slidePlaneOrigin = colData.intersectionPoint;
  const float3 slidePlaneNormal = (newBasePoint - colData.intersectionPoint).normalize();
  plane3d<f32> slidingPlane(slidePlaneOrigin, slidePlaneNormal);
  float3 newDestinationPoint =
      destinationPoint -
      (slidePlaneNormal * slidingPlane.getDistanceTo(destinationPoint));
  // generate slide vector
  const float3 newVelocityVector = newDestinationPoint -
...
F3LINE CSceneCollisionManager::getRayFromScreenCoordinates(
    const position2di& pos, ICameraSceneNode* camera)
{
  F3LINE ln(0, 0, 0, 0, 0, 0);
  if (!SceneManager) {
    return ln;
  }
  if (!camera) {
  ...
  float3 farLeftUp = f->getFarLeftUp();
  float3 lefttoright = f->getFarRightUp() - farLeftUp;
  float3 uptodown = f->getFarLeftDown() - farLeftUp;
  const IRECT& viewPort = Driver->getViewPort();
  dimension2du screenSize(viewPort.getWidth(), viewPort.getHeight());
...
position2di CSceneCollisionManager::getScreenCoordinatesFrom3DPosition(
    const float3 pos3d, ICameraSceneNode* camera, bool useViewPort)
{
...
CV_INLINE bool CSceneCollisionManager::getLowestRoot(f32 a, f32 b, f32 c, f32 maxR, f32* root)
{
