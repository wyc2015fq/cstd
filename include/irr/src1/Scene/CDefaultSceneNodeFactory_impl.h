...
ISceneNode* CDefaultSceneNodeFactory::addSceneNode(ESCENE_NODE_TYPE type, ISceneNode* parent)
{
  ...
  case ESNT_TEXT:
    return Manager->addTextSceneNode(0, "example");
  ...
  case ESNT_MESH:
    return Manager->addMeshSceneNode(0, parent, -1, float3(),
        float3(), float3(1, 1, 1), true);
  case ESNT_LIGHT:
  ...
  case ESNT_ANIMATED_MESH:
    return Manager->addAnimatedMeshSceneNode(0, parent, -1, float3(),
        float3(), float3(1, 1, 1), true);
...
{
  if (idx < SupportedSceneNodeTypes_size) {
    return SupportedSceneNodeTypes[idx].TypeName;
  }
  else {
    return 0;
  }
...
{
  int i;
  for (i = 0; i < SupportedSceneNodeTypes_size; ++i)
    if (SupportedSceneNodeTypes[i].Type == type) {
      return SupportedSceneNodeTypes[i].TypeName;
    }
  return 0;
}
