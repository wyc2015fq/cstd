...
public:
  ...
  const F3AABBOX& getBoundingBox(s32 patchX, s32 patchZ) = 0;
  ...
  IMeshBuffer* getRenderBuffer() = 0;
  //! Gets the meshbuffer data based on a specified level of detail.
  /** \param mb A reference to an IDynamicMeshBuffer object
  \param LOD The level of detail you want the indices from. */
  void getMeshBufferForLOD(IDynamicMeshBuffer& mb, s32 LOD = 0) = 0;
  ...
  /** \param LODs A reference to a s32> to hold the
  values
  \return Number of elements in the array */
  s32 getCurrentLODOfPatches(s32* LODs) = 0;
  //! Manually sets the LOD of a patch
  /** \param patchX Patch x coordinate.
  \param patchZ Patch z coordinate.
  \param LOD The level of detail to set the patch to. */
  void setLODOfPatch(s32 patchX, s32 patchZ, s32 LOD = 0) = 0;
  //! Get center of terrain.
  const float3 getTerrainCenter() = 0;
  //! Get height of a point of the terrain.
  f32 getHeight(f32 x, f32 y) = 0;
  ...
  void setDynamicSelectorUpdate(bool bVal) = 0;
  ...
  size. */
  bool overrideLODDistance(s32 LOD, f64 newDistance) = 0;
  ...
  void scaleTexture(f32 scale = 1.0f, f32 scale2 = 0.0f) = 0;
