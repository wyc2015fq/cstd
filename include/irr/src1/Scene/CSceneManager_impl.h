    ...
    IRR_XML_FORMAT_SCENE("irr_scene"), IRR_XML_FORMAT_NODE("node"), IRR_XML_FORMAT_NODE_ATTR_TYPE("type")
{
  ...
  Parameters.setAttribute(DEBUG_NORMAL_COLOR, SColor(255, 34, 221, 221));
  if (Driver) {
    Driver->grab();
  }
  if (FileSystem) {
...
CSceneManager::~CSceneManager()
{
  ...
  u32 i;
  for (i = 0; i < MeshLoaderList_size; ++i) {
    MeshLoaderList[i]->drop();
  }
  for (i = 0; i < SceneLoaderList_size; ++i) {
  ...
  for (i = 0; i < SceneNodeFactoryList_size; ++i) {
    SceneNodeFactoryList[i]->drop();
  }
  for (i = 0; i < SceneNodeAnimatorFactoryList_size; ++i) {
...
IAnimatedMesh* CSceneManager::getMesh(const char* filename)
{
...
ITextSceneNode* CSceneManager::addTextSceneNode(IGUIFont* font,
    const char* text, SColor color, ISceneNode* parent,
    const float3 position, s32 id)
{
  if (!font) {
    return 0;
  }
...
IBillboardTextSceneNode* CSceneManager::addBillboardTextSceneNode(IGUIFont* font,
    const char* text, ISceneNode* parent,
...
IVolumeLightSceneNode* CSceneManager::addVolumeLightSceneNode(
    ...
    const SColor foot, const SColor tail,
...
IMeshSceneNode* CSceneManager::addSphereSceneNode(f32 radius, s32 polyCount,
    ISceneNode* parent, s32 id, const float3 position,
    const float3 rotation, const float3 scale)
{
  if (!parent) {
    parent = this;
...
IMeshSceneNode* CSceneManager::addMeshSceneNode(IMesh* mesh, ISceneNode* parent, s32 id,
    ...
    const float3 scale, bool alsoAddIfMeshPointerZero)
{
...
ICameraSceneNode* CSceneManager::addCameraSceneNode(ISceneNode* parent,
    const float3 position, const float3 lookat, s32 id,
    bool makeActive)
{
  if (!parent) {
    parent = this;
...
ICameraSceneNode* CSceneManager::addCameraSceneNodeMaya(ISceneNode* parent,
    ...
    bool makeActive)
{
  ICameraSceneNode* node = addCameraSceneNode(parent, float3(),
      float3(0, 0, 100), id, makeActive);
  if (node) {
...
ILightSceneNode* CSceneManager::addLightSceneNode(ISceneNode* parent,
    const float3 position, SColorf color, f32 range, s32 id)
{
  if (!parent) {
    parent = this;
  }
...
IBillboardSceneNode* CSceneManager::addBillboardSceneNode(ISceneNode* parent,
    const dimension2df& size, const float3 position, s32 id,
    SColor colorTop, SColor colorBottom
                                                         )
{
  if (!parent) {
...
ISceneNode* CSceneManager::addSkyBoxSceneNode(ITexture* top, ITexture* bottom,
    ...
    ITexture* back, ISceneNode* parent, s32 id)
{
  if (!parent) {
    parent = this;
  }
...
IAnimatedMesh* CSceneManager::addTerrainMesh(const char* name,
...
IAnimatedMesh* CSceneManager::addArrowMesh(const char* name,
...
IAnimatedMesh* CSceneManager::addSphereMesh(const char* name,
...
IAnimatedMesh* CSceneManager::addVolumeLightMesh(const char* name,
    const u32 SubdivideU, const u32 SubdivideV,
    const SColor FootColor, const SColor TailColor)
{
  if (MeshCache->isMeshLoaded(name)) {
    return MeshCache->getMeshByName(name);
  }
...
const F3AABBOX& CSceneManager::getBoundingBox() const
{
...
bool CSceneManager::isCulled(const ISceneNode* node) const
{
  ...
  if (!result && (node->getAutomaticCulling() & EAC_BOX)) {
    F3AABBOX tbox = node->getBoundingBox();
...
void CSceneManager::drawAll()
{
  ...
  for (i = ETS_COUNT - 1; i >= ETS_TEXTURE_0; --i) {
    Driver->setTransform((E_TRANSFORMATION_STATE)i, IdentityMatrix);
  }
  ...
  */
  ...
  //render lights scenes
  {
    ...
    if (LightManager) {
      ...
      float3 camWorldPos(0, 0, 0);
      if (ActiveCamera) {
        camWorldPos = ActiveCamera->getAbsolutePosition();
      }
      DistanceNodeEntry > SortedLights;
      SortedLights.set_used(LightList_size);
      for (light = (s32)LightList_size - 1; light >= 0; --light) {
    ...
    u32 maxLights = LightList_size;
    if (!LightManager) {
      maxLights = min_(Driver->getMaximalDynamicLightAmount(), maxLights);
    }
  ...
  // render default objects
  {
    ...
    if (LightManager) {
      LightManager->OnRenderPassPreRender(CurrentRendertime);
      for (i = 0; i < SolidNodeList_size; ++i) {
...
#ifdef _IRR_SCENEMANAGER_DEBUG
    Parameters.setAttribute("drawn_solid", (s32) SolidNodeList_size);
#endif
  ...
  // render shadows
  {
    ...
    if (LightManager) {
      LightManager->OnRenderPassPreRender(CurrentRendertime);
      for (i = 0; i < ShadowNodeList_size; ++i) {
  ...
  // render transparent objects.
  {
    ...
    if (LightManager) {
      LightManager->OnRenderPassPreRender(CurrentRendertime);
      for (i = 0; i < TransparentNodeList_size; ++i) {
...
#ifdef _IRR_SCENEMANAGER_DEBUG
    Parameters.setAttribute("drawn_transparent", (s32) TransparentNodeList_size);
#endif
  ...
  // render transparent effect objects.
  {
    ...
    if (LightManager) {
      LightManager->OnRenderPassPreRender(CurrentRendertime);
      for (i = 0; i < TransparentEffectNodeList_size; ++i) {
...
#ifdef _IRR_SCENEMANAGER_DEBUG
    Parameters.setAttribute("drawn_transparent_effect", (s32) TransparentEffectNodeList_size);
#endif
...
void CSceneManager::setShadowColor(SColor color)
{
  ShadowColor = color;
}
//! Returns the current color of shadows.
SColor CSceneManager::getShadowColor() const
{
  return ShadowColor;
}
//! creates a rotation animator, which rotates the attached scene node around itself.
ISceneNodeAnimator* CSceneManager::createRotationAnimator(const float3 rotationPerSecond)
{
...
ISceneNodeAnimator* CSceneManager::createFlyCircleAnimator(
    ...
    f32 radiusEllipsoid)
{
  const f32 orbitDurationMs = (DEGTORAD * 360.f) / speed;
...
ISceneNodeAnimator* CSceneManager::createFlyStraightAnimator(const float3 startPoint,
    const float3 endPoint, u32 timeForWay, bool loop, bool pingpong)
{
...
ISceneNodeAnimator* CSceneManager::createTextureAnimator(const ITexture* > & textures,
...
ISceneNodeAnimatorCollisionResponse* CSceneManager::createCollisionResponseAnimator(
    ITriangleSelector* world, ISceneNode* sceneNode, const float3 ellipsoidRadius,
    const float3 gravityPerSecond,
    const float3 ellipsoidTranslation, f32 slidingValue)
{
...
void CSceneManager::getSceneNodesFromType(ESCENE_NODE_TYPE type, ISceneNode* > & outNodes, ISceneNode* start)
{
  if (start == 0) {
    start = getRootSceneNode();
  }
...
bool CSceneManager::saveScene(const char* filename, ISceneUserDataSerializer* userDataSerializer, ISceneNode* node)
{
  bool ret = false;
...
//! Saves the current scene into a file.
bool CSceneManager::saveScene(IXMLWriter* writer, const char* currentPath, ISceneUserDataSerializer* userDataSerializer, ISceneNode* node)
{
  if (!writer) {
    return false;
  }
  ...
  writeSceneNode(writer, node, userDataSerializer, currentPath, true);
  return true;
}
//! Loads a scene.
bool CSceneManager::loadScene(const char* filename, ISceneUserDataSerializer* userDataSerializer, ISceneNode* rootNode)
{
  IReadFile* file = FileSystem->createAndOpenFile(filename);
  if (!file) {
...
bool CSceneManager::loadScene(IReadFile* file, ISceneUserDataSerializer* userDataSerializer, ISceneNode* rootNode)
{
  ...
  s32 i = SceneLoaderList_size - 1;
  for (; i >= 0 && !ret; --i)
    if (SceneLoaderList[i]->isALoadableFileFormat(file)) {
      ret = SceneLoaderList[i]->loadScene(file, userDataSerializer, rootNode);
    }
  if (!ret) {
    os::Printer::log("Could not load scene file, perhaps the format is unsupported: ", file->getFileName(), ELL_ERROR);
  }
  return ret;
}
//! writes a scene node
    ...
    const fschar_t* currentPath, bool init)
{
  ...
  if (init) {
    ...
    writer->writeElement(name, false, IRR_XML_FORMAT_NODE_ATTR_TYPE,
        char * (getSceneNodeTypeName(node->getType())));
  }
  ...
  if (node->getMaterialCount() && Driver) {
    const char* materialElement = "materials";
  ...
  if (!node->getAnimators().empty()) {
    const char* animatorElement = "animators";
  ...
  if (userDataSerializer) {
    ...
    if (userData) {
      const char* userDataElement = "userData";
      writer->writeLineBreak();
...
{
  const char* name = 0;
  for (s32 i = (s32)SceneNodeFactoryList_size - 1; !name && i >= 0; --i) {
...
{
  ISceneNode* node = 0;
  for (s32 i = (s32)SceneNodeFactoryList_size - 1; i >= 0 && !node; --i) {
...
ISceneNodeAnimator* CSceneManager::createSceneNodeAnimator(const char* typeName, ISceneNode* target)
{
  ...
  for (s32 i = (s32)SceneNodeAnimatorFactoryList_size - 1; i >= 0 && !animator; --i) {
...
{
  const char* name = 0;
  for (s32 i = SceneNodeAnimatorFactoryList_size - 1; !name && i >= 0; --i) {
...
void CSceneManager::setAmbientLight(const SColorf& ambientColor)
{
  AmbientLight = ambientColor;
}
//! Returns ambient color of the scene
const SColorf& CSceneManager::getAmbientLight() const
{
  return AmbientLight;
}
