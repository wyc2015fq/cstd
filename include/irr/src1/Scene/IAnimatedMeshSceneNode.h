...
public:
  ...
  void OnAnimationEnd(IAnimatedMeshSceneNode* node) = 0;
};
...
public:
  ...
  void setCurrentFrame(f32 frame) = 0;
  //! Sets the frame numbers between the animation is looped.
  /** The default is 0 - MaximalFrameCount of the mesh.
  \param begin: Start frame number of the loop.
  \param end: End frame number of the loop.
  \return True if successful, false if not. */
  bool setFrameLoop(s32 begin, s32 end) = 0;
  //! Sets the speed with which the animation is played.
  /** \param framesPerSecond: Frames per second played. */
  void setAnimationSpeed(f32 framesPerSecond) = 0;
  //! Gets the speed with which the animation is played.
  /** \return Frames per second played. */
  f32 getAnimationSpeed() = 0;
  ...
  IBoneSceneNode* getJointNode(const c8* jointName) = 0;
  //! same as getJointNode(const c8* jointName), but based on id
  IBoneSceneNode* getJointNode(u32 jointID) = 0;
  //! Gets joint count.
  ...
  bool setMD2Animation(EMD2_ANIMATION_TYPE anim) = 0;
  ...
  bool setMD2Animation(const c8* animationName) = 0;
  ...
  void setLoopMode(bool playAnimationLooped) = 0;
  ...
  void setAnimationEndCallback(IAnimationEndCallBack* callback = 0) = 0;
  ...
  const SMD3QuaternionTag* getMD3TagTransformation(const char* tagname) = 0;
  //! Set how the joints should be updated on render
  void setJointMode(E_JOINT_UPDATE_ON_RENDER mode) = 0;
  //! Sets the transition time in seconds
  /** Note: This needs to enable joints, and setJointmode set to
  EJUOR_CONTROL. You must call animateJoints(), or the mesh will
  not animate. */
  void setTransitionTime(f32 Time) = 0;
  //! animates the joints in the mesh based on the current frame.
  /** Also takes in to account transitions. */
  void animateJoints(bool CalculateAbsolutePositions = true) = 0;
  //! render mesh ignoring its transformation.
  /** Culling is unaffected. */
  void setRenderFromIdentity(bool On) = 0;
  //! Creates a clone of this scene node and its children.
  /** \param newParent An optional new parent.
  \param newManager An optional new scene manager.
  \return The newly created clone of this node. */
  ISceneNode* clone(ISceneNode* newParent = 0, ISceneManager* newManager = 0) = 0;
};
#endif
