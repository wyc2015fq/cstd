...
public:
  ...
  Irrlicht 1.9 */
  _IRR_DEPRECATED_ IAnimatedMesh* getMeshByFilename(const char* filename) {
  ...
  Irrlicht 1.9 */
  _IRR_DEPRECATED_ const char* getMeshFilename(u32 index) const {
  ...
  /** \deprecated Use getMeshName() instead. This method may be removed by
  Irrlicht 1.9 */
  _IRR_DEPRECATED_ const char* getMeshFilename(const IMesh* const mesh) const {
    return getMeshName(mesh).getInternalName();
  }
  //! Renames a loaded mesh.
  /** \deprecated Use renameMesh() instead. This method may be removed by
  Irrlicht 1.9 */
  _IRR_DEPRECATED_ bool setMeshFilename(u32 index, const char* filename) {
    return renameMesh(index, filename);
  }
  //! Renames a loaded mesh.
  /** \deprecated Use renameMesh() instead. This method may be removed by
  Irrlicht 1.9 */
  _IRR_DEPRECATED_ bool setMeshFilename(const IMesh* const mesh, const char* filename) {
    return renameMesh(mesh, filename);
  }
  //! Returns a mesh based on its name.
  /** \param name Name of the mesh. Usually a filename.
  \return Pointer to the mesh or 0 if there is none with this number. */
  IAnimatedMesh* getMeshByName(const char* name) = 0;
  //! Get the name of a loaded mesh, based on its index.
  /** \param index: Index of the mesh, number between 0 and getMeshCount()-1.
  \return The name if mesh was found and has a name, else the path is empty. */
  const char* getMeshName(u32 index) = 0;
  //! Get the name of the loaded mesh if there is any.
  /** \param mesh Pointer to mesh to query.
  \return The name if mesh was found and has a name, else the path is empty. */
  const char* getMeshName(const IMesh* const mesh) = 0;
  //! Renames a loaded mesh.
  ...
  bool renameMesh(u32 index, const char* name) = 0;
  ...
  \param name New name for the mesh.
  \return True if mesh was renamed. */
  bool renameMesh(const IMesh* const mesh, const char* name) = 0;
  //! Check if a mesh was already loaded.
  /** \param name Name of the mesh. Usually a filename.
  \return True if the mesh has been loaded, else false. */
  bool isMeshLoaded(const char* name) = 0;
