...
//#include "os.h"
IMesh* CGeometryCreator::createCubeMesh(const float3 size) const
{
  ...
  for (i = 0; i < 12; ++i) {
    buffer->Vertices[i].Pos -= f3VECTOR(0.5f, 0.5f, 0.5f);
...
IMesh* CGeometryCreator::createHillPlaneMesh(
    ...
    const dimension2du& tc, SMaterial* material,
    f32 hillHeight, const dimension2df& ch,
    const dimension2df& textureRepeatCount) const
{
  dimension2du tileCount = tc;
  dimension2df countHills = ch;
  ...
  // center
  const position2df center((tileSize.Width * tileCount.Width) * 0.5f, (tileSize.Height * tileCount.Height) * 0.5f);
  // texture coord step
  const dimension2df tx(
  ...
  for (x = 0; x < tileCount.Width - 1; ++x) {
    for (int y = 0; y < tileCount.Height - 1; ++y) {
  ...
  // recalculate normals
  int i;
  for (i = 0; i < buffer->Indices_size; i += 3) {
    const float3 normal = plane3d<f32>(
...
IMesh* CGeometryCreator::createTerrainMesh(IImage* texture,
    ...
    const dimension2du& maxVtxBlockSize,
    bool debugBorders) const
{
  ...
  const dimension2du hMapSize = heightmap->getDimension();
  const dimension2du tMapSize = texture->getDimension();
  const position2df thRel(static_cast<f32>(tMapSize.Width) / hMapSize.Width, static_cast<f32>(tMapSize.Height) / hMapSize.Height);
  maxHeight /= 255.0f; // height step per color value
  position2di processed(0, 0);
  while (processed.Y < hMapSize.Height) {
    while (processed.X < hMapSize.Width) {
      dimension2du blockSize = maxVtxBlockSize;
      ...
      u32 y;
      vector2df pos(0.f, processed.Y * stretchSize.Height);
      const vector2df bs(1.f / blockSize.Width, 1.f / blockSize.Height);
      vector2df tc(0.f, 0.5f * bs.Y);
      ...
      for (y = 0; y < blockSize.Height - 1; ++y) {
        for (int x = 0; x < blockSize.Width - 1; ++x) {
          const s32 c = c1 + x;
      ...
      if (buffer->Vertices_size) {
        c8 textureName[64];
        // create texture for this block
        IImage* img = driver->createImage(texture->getColorFormat(), dimension2du(floor32(blockSize.Width * thRel.X), floor32(blockSize.Height * thRel.Y)));
        texture->copyTo(img, position2di(0, 0), recti(
            position2di(floor32(processed.X * thRel.X), floor32(processed.Y * thRel.Y)),
            dimension2du(floor32(blockSize.Width * thRel.X), floor32(blockSize.Height * thRel.Y))), 0);
    ...
    const SColor vtxColor1) const
{
  ...
  int i;
  for (i = 0; i < mesh2->getMeshBufferCount(); ++i) {
...
IMesh* CGeometryCreator::createSphereMesh(f32 radius, u32 polyCountX, u32 polyCountY) const
{
  ...
  const SColor clr(255, 255, 255, 255);
  u32 level = 0;
  ...
  for (int y = 0; y < polyCountY; ++y) {
    ...
    for (int xz = 0; xz < polyCountX; ++xz) {
      ...
      const float3 pos(static_cast<f32>(radius * cos(axz) * sinay),
      ...
      if (y == 0) {
        if (normal.Y != -1.0f && normal.Y != 1.0f) {
          tu = static_cast<f32>(acos(clamp(normal.X / sinay, -1.0, 1.0)) * 0.5 * RECIPROCAL_PI64);
        }
        if (normal.Z < 0.0f) {
          tu = 1 - tu;
        }
      ...
      buffer->Vertices[i] = S3DVertex(pos.X, pos.Y, pos.Z,
          ...
          clr, tu,
          static_cast<f32>(ay * RECIPROCAL_PI64));
      ++i;
      axz += AngleX;
    }
...
IMesh* CGeometryCreator::createCylinderMesh(f32 radius, f32 length,
    u32 tesselation, const SColor& color,
    bool closeTop, f32 oblique) const
{
  SMeshBuffer* buffer = new SMeshBuffer();
  const f32 recTesselation = reciprocal((f32)tesselation);
  const f32 recTesselationHalf = recTesselation * 0.5f;
  const f32 angleStep = (PI * 2.f) * recTesselation;
  ...
  v.TCoords.Y = 1.f;
  buffer->Vertices.push_back(v);
  u32 index = buffer->Vertices_size - 1;
  for (i = 0; i != nonWrappedSize; i += 2) {
...
{
  SMeshBuffer* buffer = new SMeshBuffer();
  const f32 angleStep = (PI * 2.f) / tesselation;
  const f32 angleStepHalf = angleStep * 0.5f;
  S3DVertex v;
  u32 i;
  v.Color = colorTop;
  ...
  const u32 nonWrappedSize = buffer->Vertices_size - 1;
  // close top
  v.Pos.X = oblique;
  v.Pos.Y = length;
  v.Pos.Z = 0.f;
...
IMesh* CGeometryCreator::createVolumeLightMesh(
    ...
    const f32 lpDistance, const float3 lightDim) const
{
  int i;
  SMeshBuffer* Buffer = new SMeshBuffer();
  Buffer->setHardwareMappingHint(EHM_STATIC);
  const float3 lightPoint(0, -(lpDistance * lightDim.Y), 0);
  ...
  // Slices in X/U space
  for (i = 0; i <= subdivideU; ++i) {
    // These are the two endpoints for a slice at the foot
    float3 end1(bx, 0.0f, -az);
    float3 end2(bx, 0.0f, az);
  ...
  // Slices in Z/V space
  for (i = 0; i <= subdivideV; ++i) {
    // These are the two endpoints for a slice at the foot
    float3 end1(-ax, 0.0f, bz);
    float3 end2(ax, 0.0f, bz);
    end1 -= lightPoint;   // get a vector from point to lightsource
    end1.normalize();   // normalize vector
    end1 *= lightDim.Y; // multiply it out by shootlength
