...
#if defined(_IRR_COMPILE_WITH_X11_DEVICE_)
// constructor  linux
COSOperator::COSOperator(const char* osVersion, CIrrDeviceLinux* device)
  : OperatingSystem(osVersion), IrrDeviceLinux(device)
{
}
#endif
// constructor
COSOperator::COSOperator(const char* osVersion) : OperatingSystem(osVersion)
{
#ifdef _DEBUG
...
const char* COSOperator::getOperatingSystemVersion() const
{
...
void COSOperator::copyToClipboard(const c8* text) const
{
  ...
  HGLOBAL clipbuffer;
  char* buffer;
  clipbuffer = GlobalAlloc(GMEM_DDESHARE, cstr_len(text) + 1);
