...
/** Constructor*/
CDMFLoader::CDMFLoader(ISceneManager* smgr, IFileSystem* filesys)
...
void CDMFLoader::findFile(bool use_mat_dirs, const char* path, const char* matPath, char* filename)
{
...
IAnimatedMesh* CDMFLoader::createMesh(IReadFile* file)
{
  if (!file) {
    return 0;
  }
  IVideoDriver* driver = SceneMgr->getVideoDriver();
  //Load stringlist
  StringList dmfRawFile;
  LoadFromFile(file, dmfRawFile);
  if (dmfRawFile_size == 0) {
    return 0;
  }
  ...
  if (GetDMFHeader(dmfRawFile, header)) {
    ...
    os::Printer::log("Loading materials", char * ((int)header.numMaterials));
#endif
    ...
    for (i = 0; i < header.numMaterials; i++) {
      ...
      buffer->Material.MaterialType = EMT_LIGHTMAP_LIGHTING;
    ...
    for (i = 0; i < header.numFaces; i++) {
#ifdef _IRR_DMF_DEBUG_
      os::Printer::log("Polygon with #vertices", char * ((int)faces[i].numVerts));
#endif
      if (faces[i].numVerts < 3) {
        continue;
      }
      const float3 normal =
          triangle3df(verts[faces[i].firstVert].pos,
      ...
      SSkinMeshBuffer* meshBuffer = (SSkinMeshBuffer*)mesh->getMeshBuffer(
          faces[i].materialID);
      const bool use2TCoords = meshBuffer->Vertices_2TCoords_size ||
          materiali[faces[i].materialID].lightmapName_size;
      if (use2TCoords && meshBuffer->Vertices_Standard_size) {
        meshBuffer->convertTo2TCoords();
      }
      const u32 base = meshBuffer->Vertices_2TCoords_size ? meshBuffer->Vertices_2TCoords_size : meshBuffer->Vertices_Standard_size;
      // Add this face's verts
      if (use2TCoords) {
        // make sure we have the proper type set
        meshBuffer->VertexType = EVT_2TCOORDS;
        for (int v = 0; v < faces[i].numVerts; v++) {
          const dmfVert& vv = verts[faces[i].firstVert + v];
          S3DVertex vert(vv.pos,
              normal, SColor(255, 255, 255, 255), vv.tc, vv.lc);
        ...
        for (int v = 0; v < faces[i].numVerts; v++) {
          const dmfVert& vv = verts[faces[i].firstVert + v];
          S3DVertex vert(vv.pos,
              normal, SColor(255, 255, 255, 255), vv.tc);
          if (materiali[faces[i].materialID].textureBlend == 4 &&
              SceneMgr->getParameters()->getAttributeAsBool(DMF_FLIP_ALPHA_TEXTURES)) {
            vert.TCoords.set(vv.tc.X, -vv.tc.Y);
          }
      ...
      for (int v = 0; v < faces[i].numVerts - 2; v++) {
        if (v & 1) { // odd
          c = h - 1;
        }
        else { // even
  ...
  }
  {
    ...
    for (i = 0; i < mesh->getMeshBufferCount(); i++) {
      ...
      //Primary texture is just a color
        ...
        SColor color(axtoi(materiali[i].textureName));
        ...
        IImage* immagine = driver->createImage(ECF_A8R8G8B8,
            dimension2du(8, 8));
        immagine->fill(color);
      ...
      if (materiali[i].lightmapFlag == 0) {
        ...
        mat.AmbientColor = header.dmfAmbient.getInterpolated(SColor(255, 0, 0, 0), mult / 100.f);
      }
      if (materiali[i].textureBlend == 4) {
        mat.MaterialType = EMT_TRANSPARENT_ALPHA_CHANNEL;
      ...
      if (lig && header.dmfVersion < 1.1) {
        const dimension2du ligsize = lig->getSize();
        void* pp = lig->lock();
        if (pp) {
          ECOLOR_FORMAT format = lig->getColorFormat();
          if (format == ECF_A1R5G5B5) {
            s16* p = (s16*)pp;
            s16 tmp = 0;
            for (int x = 0; x < ligsize.Width; x++) {
              for (int y = 0; y < ligsize.Height / 2; y++) {
...
bool CDMFLoader::isALoadableFileExtension(const char* filename) const
{
  return hasFileExtension(filename, "dmf");
}
