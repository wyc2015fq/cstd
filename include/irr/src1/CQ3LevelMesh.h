...
public:
  ...
  IMesh* getMesh(s32 frameInMs, s32 detailLevel = 255,
  ...
  void getShader(IReadFile* file);
  //! loads the shader definition
  const quake3::IShader* getShader(const c8* filename, bool fileNameIsValid = true);
  //! returns a already loaded Shader
  const quake3::IShader* getShader(u32 index) const;
  //! loads a configuration file
  void getConfiguration(IReadFile* file);
  //! get's an interface to the entities
  quake3::IEntity*& getEntityList();
  //! returns the requested brush entity
  IMesh* getBrushEntityMesh(s32 num) const;
  //! returns the requested brush entity
  IMesh* getBrushEntityMesh(quake3::IEntity& ent) const;
  //Link to held meshes? ...
...
private:
  ...
  struct S3DVertex2TCoords_64 {
    ...
    D3VECTOR Normal;
    SColorf Color;
    vector2d<f64> TCoords;
    vector2d<f64> TCoords2;
    void copy(S3DVertex& dest) const;
    S3DVertex2TCoords_64() {}
    S3DVertex2TCoords_64(const D3VECTOR& pos, const D3VECTOR& normal, const SColorf& color,
        const vector2d<f64>& tcoords, const vector2d<f64>& tcoords2)
  ...
  CV_INLINE void copy(S3DVertex* dest, const tBSPVertex* source,
  ...
  quake3::IShader > Shader;
  quake3::IShader > Entity;   //quake3::IEntity* Entity;
  quake3::strv_t ShaderFile;
  void InitShader();
  void ReleaseShader();
  void ReleaseEntity();
  s32 setShaderMaterial(SMaterial& material, const tBSPFace* face) const;
  s32 setShaderFogMaterial(SMaterial& material, const tBSPFace* face) const;
  struct SToBuffer {
    s32 takeVertexColor;
    u32 index;
  };
