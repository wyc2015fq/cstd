...
int IRRCALLCONV main(int argc, char* argv[])
{
  ...
  */
  ...
  const dimension2du videoDim(800, 600);
  ...
  */
  IQ3LevelMesh* const mesh =
      (IQ3LevelMesh*) smgr->getMesh(mapname);
  /*
  ...
  */
  if (mesh) {
    ...
    IGUIFont* font = device->getGUIEnvironment()->getFont("../../media/fontlucida.png");
    u32 count = 0;
#endif
  ...
  */
  if (mesh) {
    quake3::IEntity*& entityList = mesh->getEntityList();
  ...
  */
  ...
  const position2di pos(videoDim.Width - 128, videoDim.Height - 64);
  switch (driverType) {
  ...
  */
  ...
  while (device->run())
    if (device->isWindowActive()) {
      driver->beginScene(true, true, _ARGB(255, 20, 20, 40));
      smgr->drawAll();
      gui->drawAll();
      driver->endScene();
      ...
      //if (lastFPS != fps)
      {
        IAttributes* const attr = smgr->getParameters();
        char* str = "Q3 [";
        str += driver->getName();
        str += "] FPS:";
        str += fps;
