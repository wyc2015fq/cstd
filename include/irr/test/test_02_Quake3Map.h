...
can be found in the namespace 'irr'. To get rid of the  in front of the
name of every class, we tell the compiler that we use that namespace from now
on, and we will not have to write that ''. There are 5 other sub
...
int main()
{
  ...
  */
  ...
  we do not always have to call IrrlichtDevice::getVideoDriver() and
  IrrlichtDevice::getSceneManager().
  */
  ...
  So we add the .pk3 file to our IFileSystem. After it was added,
  ...
  */
  ...
  ISceneManager::getMesh(). We get a pointer returned to an
  IAnimatedMesh. As you might know, Quake 3 maps are not
  ...
  ISceneManager::addOctreeSceneNode().
  The Octree optimizes the scene a little bit, trying to draw only geometry
  which is currently visible. An alternative to the Octree would be a
  IMeshSceneNode, which would always draw the complete
  geometry of the mesh, without optimization. Try it: Use
  ISceneManager::addMeshSceneNode() instead of
  addOctreeSceneNode() and compare the primitives drawn by the video
  driver. (There is a IVideoDriver::getPrimitiveCountDrawn()
  method in the IVideoDriver class). Note that this
  ...
  */
  ...
  ISceneNode level using the methods
  ISceneNode::setPosition() (in this case),
  ISceneNode::setRotation(), and
  ISceneNode::setScale().
  */
  if (node) {
    node->setPosition(f3VECTOR(-1300, -144, -1249));
  }
  /*
  ...
  ISceneManager::addCameraSceneNodeMaya(). But for this
  ...
  */
  ...
  IrrlichtDevice::ICursorControl.
  */
  device->getCursorControl()->setVisible(false);
  /*
  We have done everything, so lets draw it. We also write the current
  frames per second and the primitives drawn into the caption of the
  window. The test for IrrlichtDevice::isWindowActive() is optional,
  but prevents the engine to grab the mouse cursor after task switching
  when other programs are active. The call to
  IrrlichtDevice::yield() will avoid the busy loop to eat up all CPU
  cycles when the window is not active.
  */
  int lastFPS = -1;
  while (device->run()) {
    if (device->isWindowActive()) {
      driver->beginScene(true, true, _ARGB(255, 200, 200, 200));
      smgr->drawAll();
      driver->endScene();
      int fps = driver->getFPS();
      if (lastFPS != fps) {
        char* str = "Irrlicht Engine - Quake 3 Map example [";
        str += driver->getName();
        str += "] FPS:";
        str += fps;
