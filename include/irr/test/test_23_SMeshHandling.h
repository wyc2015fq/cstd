...
public:
  ...
  vector3df getnormal(u16 x, u16 y, f32 s) const {
    ...
    return f3VECTOR(s * 2 * (zl - zr), 4, s * 2 * (zd - zu)).normalize();
  }
};
...
int main(int argc, char* argv[])
{
  ...
  IrrlichtDevice* device = createDevice(driverType,
      dimension2du(800, 600), 32, false, false, false,
      &receiver);
  if (device == 0) {
    return 1;
  }
  ...
  */
  ...
  meshnode->setMaterialFlag(EMF_BACK_FACE_CULLING, false);
  // light is just for nice effects
  ILightSceneNode* node = smgr->addLightSceneNode(0, f3VECTOR(0, 100, 0),
      SColorf(1.0f, 0.6f, 0.7f, 1.0f), 500.0f);
  if (node) {
    node->getLightData().Attenuation.set(0.f, 1.f / 500.f, 0.f);
    ISceneNodeAnimator* anim = smgr->createFlyCircleAnimator(f3VECTOR(0, 150, 0), 250.0f);
    if (anim) {
  ...
  if (camera) {
    camera->setPosition(f3VECTOR(-20.f, 150.f, -20.f));
    camera->setTarget(f3VECTOR(200.f, -80.f, 150.f));
  ...
  */
  while (device->run()) {
    ...
    if (receiver.IsKeyDown(KEY_KEY_W)) {
      meshnode->setMaterialFlag(EMF_WIREFRAME, !meshnode->getMaterial(0).Wireframe);
    }
    else if (receiver.IsKeyDown(KEY_KEY_1)) {
      hm.generate(eggbox);
      mesh.init(hm, 50.f, grey, driver);
    }
    ...
    driver->beginScene(true, true, _ARGB(0xff000000));
    smgr->drawAll();
    driver->endScene();
  }
  device->drop();
