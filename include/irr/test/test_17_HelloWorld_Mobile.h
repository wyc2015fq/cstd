...
public:
  CSampleSceneNode(ISceneNode* parent, ISceneManager* mgr, s32 id)
    ...
    Vertices[0] = S3DVertex(0, 0, 10, 1, 1, 0, _ARGB(255, 0, 255, 255), 0, 1);
    Vertices[1] = S3DVertex(10, 0, -10, 1, 0, 0, _ARGB(255, 255, 0, 255), 1, 1);
    Vertices[2] = S3DVertex(0, 20, 0, 0, 1, 1, _ARGB(255, 255, 255, 0), 1, 0);
    Vertices[3] = S3DVertex(-10, 0, -10, 0, 0, 1, _ARGB(255, 0, 255, 0), 0, 0);
...
IrrlichtDevice* startup()
{
  ...
  device = createDevice(driverType, dimension2du(240, 320), 16, true);
#else
  // on PC. use window mode
  device = createDevice(driverType, dimension2du(240, 320), 16, false);
#endif
  if (0 == device) {
    return 0;
  }
...
#endif
  IGUIStaticText* text = guienv->addStaticText("FPS: 25",
      IRECT(140, 15, 200, 30), false, false, 0, 100);
  guienv->addButton(rect<int>(200, 10, 238, 30), 0, 2, "Quit");
  // add irrlicht logo
...
int run(IrrlichtDevice* device)
{
  while (device->run())
    if (device->isWindowActive()) {
      device->getVideoDriver()->beginScene(true, true, _ARGB(0, 100, 100, 100));
...
{
public:
  EventReceiver_terrain(IrrlichtDevice* device, ISceneNode* terrain, ISceneNode* skybox, ISceneNode* skydome) :
  ...
  bool OnEvent(const SEvent& event) {
    ...
    if (event.EventType == EET_KEY_INPUT_EVENT && !event.KeyInput.PressedDown) {
      ...
      case KEY_KEY_W: // switch wire frame mode
        Terrain->setMaterialFlag(EMF_WIREFRAME,
            !Terrain->getMaterial(0).Wireframe);
        Terrain->setMaterialFlag(EMF_POINTCLOUD, false);
        return true;
      case KEY_KEY_P: // switch wire frame mode
        Terrain->setMaterialFlag(EMF_POINTCLOUD,
            !Terrain->getMaterial(0).PointCloud);
        Terrain->setMaterialFlag(EMF_WIREFRAME, false);
        return true;
      case KEY_KEY_D: // toggle detail map
        Terrain->setMaterialType(
            Terrain->getMaterial(0).MaterialType == EMT_SOLID ?
            EMT_DETAIL_MAP : EMT_SOLID);
        return true;
      case KEY_KEY_S: // toggle skies
        showBox = !showBox;
...
int example_terrain()
{
  ...
  */
  ...
  env->addStaticText(
      ...
      IRECT(5, 250, 235, 320), true, true, 0, -1, true);
  // add camera
  ICameraSceneNode* camera =
      smgr->addCameraSceneNodeFPS(0, 100.0f, 1.2f);
  camera->setPosition(f3VECTOR(2700 * 2, 255 * 2, 2600 * 2));
  camera->setTarget(f3VECTOR(2397 * 2, 343 * 2, 2700 * 2));
  ...
  */
  ...
  ITerrainSceneNode* terrain = smgr->addTerrainSceneNode(
      ...
      0,          // parent node
      -1,         // node id
      f3VECTOR(0.f, 0.f, 0.f),   // position
      f3VECTOR(0.f, 0.f, 0.f),   // rotation
      f3VECTOR(40.f, 4.4f, 40.f),  // scale
      _ARGB(255, 255, 255, 255),     // vertexColor
      5,          // maxLOD
      ETPS_17,       // patchSize
      4         // smoothFactor
      );
  if (terrain) {
    ...
    */
    ...
    ISceneNodeAnimator* anim = smgr->createCollisionResponseAnimator(
        selector, camera, f3VECTOR(60, 100, 60),
        f3VECTOR(0, 0, 0),
        f3VECTOR(0, 50, 0));
    selector->drop();
    ...
    */
    ...
    S3DVertex* data = (S3DVertex*)buffer->getVertexBuffer().getData();
...
int example_helloworld()
{
  ...
  */
  smgr->addCameraSceneNode(0, f3VECTOR(0, 30, -40), f3VECTOR(0, 5, 0));
  EventReceiver_basic receiver(device);
  device->setEventReceiver(&receiver);
  return run(device);
}
