...
int main()
{
  ...
  */
  ...
  if (node) {
    node->setPosition(f3VECTOR(0, 0, 30));
  ...
  */
  ...
  if (n) {
    ...
    ISceneNodeAnimator* anim =
        smgr->createFlyCircleAnimator(f3VECTOR(0, 0, 30), 20.0f);
    if (anim) {
      n->addAnimator(anim);
      anim->drop();
    }
  ...
  */
  ...
  if (anms) {
    ISceneNodeAnimator* anim =
        smgr->createFlyStraightAnimator(f3VECTOR(100, 0, 60),
            f3VECTOR(-100, 0, 60), 3500, true);
    if (anim) {
    ...
    "anms->setMD2Animation(EMAT_RUN)" for the 'run'
    ...
    */
    ...
    anms->setScale(f3VECTOR(2.f, 2.f, 2.f));
    anms->setRotation(f3VECTOR(0, -90, 0));
  ...
  */
  ...
  IGUIStaticText* diagnostics = device->getGUIEnvironment()->addStaticText(
      "", IRECT(10, 10, 400, 20));
  diagnostics->setOverrideColor(_ARGB(255, 255, 255, 0));
  /*
  ...
  */
  ...
  while (device->run()) {
    ...
    if (lastFPS != fps) {
      char* tmp("Movement Example - Irrlicht Engine [");
      tmp += driver->getName();
      tmp += "] fps: ";
      tmp += fps;
