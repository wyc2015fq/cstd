...
public:
  ...
  void OnPreRender(ISceneNode** lightList) {
  ...
  void OnNodePreRender(ISceneNode* node) {
    ...
    if (LIGHTS_NEAREST_NODE == Mode) {
      ...
      LightDistanceElement* sortingArray = 0;
      int sortingArray_size = SceneLightList->size();
      MYREALLOC(sortingArray, sortingArray_size);
      u32 i;
      for (i = 0; i < sortingArray_size; ++i) {
        ISceneNode* lightNode = (*SceneLightList)[i];
        const f64 distance = lightNode->getAbsolutePosition().getDistanceFromSQ(nodePosition);
        sortingArray[i] = (LightDistanceElement(lightNode, distance));
      }
      sortingArray.sort();
      ...
      int i;
      for (i = 0; i < SceneLightList->size(); ++i) {
        if ((*SceneLightList)[i]->getType() != ESNT_LIGHT) {
          continue;
        }
...
private:
  ...
  void turnOnZoneLights(ISceneNode* node) {
    list<ISceneNode*> const& children = node->getChildren();
    for (list<ISceneNode*>::ConstIterator child = children.begin();
  ...
  public:
    ...
    LightDistanceElement(ISceneNode* n, f64 d)
      : node(n), distance(d) { }
    ISceneNode* node;
    f64 distance;
...
int main(int argumentCount, char* argumentValues[])
{
  ...
  if (skin) {
    skin->setColor(EGDC_BUTTON_TEXT, _ARGB(255, 255, 255, 255));
    IGUIFont* font = guienv->getFont("../../media/fontlucida.png");
    if (font) {
      skin->setFont(font);
    }
  }
  guienv->addStaticText("1 - No light management", IRECT(10, 10, 200, 30));
  guienv->addStaticText("2 - Closest 3 lights", IRECT(10, 30, 200, 50));
  guienv->addStaticText("3 - Lights in zone", IRECT(10, 50, 200, 70));
  /*
  ...
  */
  for (f32 zoneX = -100.f; zoneX <= 100.f; zoneX += 50.f)
    for (f32 zoneY = -60.f; zoneY <= 60.f; zoneY += 60.f) {
      ...
      zoneRoot->setPosition(f3VECTOR(zoneX, zoneY, 0));
      // Each zone contains a rotating cube
      IMeshSceneNode* node = smgr->addCubeSceneNode(15, zoneRoot);
      ISceneNodeAnimator* rotation = smgr->createRotationAnimator(f3VECTOR(0.25f, 0.5f, 0.75f));
      ...
      billboard->setPosition(f3VECTOR(0, -14, 30));
      ...
      smgr->addLightSceneNode(billboard, f3VECTOR(0, 0, 0), SColorf(1, 0, 0), lightRadius);
      billboard = smgr->addBillboardSceneNode(node);
      billboard->setPosition(f3VECTOR(-21, -14, -21));
      billboard->setMaterialType(EMT_TRANSPARENT_ADD_COLOR);
      billboard->setMaterialTexture(0, driver->getTexture("../../media/particle.bmp"));
      billboard->setMaterialFlag(EMF_LIGHTING, false);
      smgr->addLightSceneNode(billboard, f3VECTOR(0, 0, 0), SColorf(0, 1, 0), lightRadius);
      billboard = smgr->addBillboardSceneNode(node);
      billboard->setPosition(f3VECTOR(21, -14, -21));
      billboard->setMaterialType(EMT_TRANSPARENT_ADD_COLOR);
      billboard->setMaterialTexture(0, driver->getTexture("../../media/particle.bmp"));
      billboard->setMaterialFlag(EMF_LIGHTING, false);
      smgr->addLightSceneNode(billboard, f3VECTOR(0, 0, 0), SColorf(0, 0, 1), lightRadius);
      ...
      node->setPosition(f3VECTOR(0, 21, 0));
    }
  smgr->addCameraSceneNode(0, f3VECTOR(0, 0, -130), f3VECTOR(0, 0, 0));
  ...
  int lastFps = -1;
  while (device->run()) {
    driver->beginScene(true, true, _ARGB(255, 100, 101, 140));
    smgr->drawAll();
    guienv->drawAll();
    driver->endScene();
