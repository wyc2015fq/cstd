...
int main()
{
  ...
  */
  mesh = smgr->addHillPlaneMesh("myHill",
      dimension2df(20, 20),
      dimension2du(40, 40), 0, 0,
      dimension2df(0, 0),
      dimension2df(10, 10));
  node = smgr->addWaterSurfaceSceneNode(mesh->getMesh(0), 3.0f, 300.0f, 30.0f);
  node->setPosition(f3VECTOR(0, 7, 0));
  ...
  */
  // create light
  node = smgr->addLightSceneNode(0, f3VECTOR(0, 0, 0),
      SColorf(1.0f, 0.6f, 0.7f, 1.0f), 800.0f);
  ISceneNodeAnimator* anim = 0;
  anim = smgr->createFlyCircleAnimator(f3VECTOR(0, 150, 0), 250.0f);
  ...
  */
  ...
  IParticleEmitter* em = ps->createBoxEmitter(
      F3AABBOX(-7, 0, -7, 7, 1, 7), // emitter size
      f3VECTOR(0.0f, 0.06f, 0.0f), // initial direction
      80, 100,                            // emit rate
      _ARGB(0, 255, 255, 255),    // darkest color
      _ARGB(0, 255, 255, 255),    // brightest color
  ...
  paf->drop();
  ps->setPosition(f3VECTOR(-70, 60, 40));
  ps->setScale(f3VECTOR(2, 2, 2));
  ...
  */
  IVolumeLightSceneNode* n = smgr->addVolumeLightSceneNode(0, -1,
      ...
      _ARGB(0, 255, 255, 255), // foot color
      _ARGB(0, 0, 0, 0));      // tail color
  if (n) {
    n->setScale(f3VECTOR(56.0f, 56.0f, 56.0f));
    n->setPosition(f3VECTOR(-120, 50, 40));
    // load textures for animation
    ITexture** textures[10] = {0};
    int ntex = 0;
    for (s32 g = 7; g > 0; --g) {
      char* tmp;
      tmp = "../../media/portal";
    ...
    ISceneNodeAnimator* glow = smgr->createTextureAnimator(textures, ntex, 150);
    // add the animator
    n->addAnimator(glow);
  ...
  */
  ...
  anode->setPosition(f3VECTOR(-50, 20, -60));
  anode->setAnimationSpeed(15);
  // add shadow
  anode->addShadowVolumeSceneNode();
  smgr->setShadowColor(_ARGB(150, 0, 0, 0));
  // make the model a little bit bigger and normalize its normals
  // because of the scaling, for correct lighting
  anode->setScale(f3VECTOR(2, 2, 2));
  anode->setMaterialFlag(EMF_NORMALIZE_NORMALS, true);
  /*
  Finally we simply have to draw everything, that's all.
  */
  ICameraSceneNode* camera = smgr->addCameraSceneNodeFPS();
  camera->setPosition(f3VECTOR(-50, 50, -150));
  ...
  while (device->run())
    if (device->isWindowActive()) {
      ...
      if (lastFPS != fps) {
        char* str = "Irrlicht Engine - SpecialFX example [";
        str += driver->getName();
        str += "] FPS:";
        str += fps;
