...
*/
struct GameData {
  GameData(const char* startupDir) :
    retVal(0), StartupDir(startupDir), createExDevice(0), Device(0) {
    setDefault();
  }
  void setDefault();
  s32 save(const char* filename);
  ...
  s32 sound;
  char StartupDir[256];
  stringc CurrentMapName;
  strv_t CurrentArchiveList[1];
...
*/
s32 GameData::load(const char* filename)
{
  if (!Device) {
    return 0;
  }
...
*/
s32 GameData::save(const char* filename)
{
  return 0;
  if (!Device) {
    return 0;
  ...
  file->write(buf, (s32) cstr_len(buf));
  for (i = 0; i != fs->getFileArchiveCount(); ++i) {
    snprintf(buf, 128, "archive %s\n", stringc(fs->getFileArchive(i)->getFileList()->getPath()));
    file->write(buf, (s32) cstr_len(buf));
  }
  file->drop();
  return 1;
...
public:
  ...
  void LoadMap(const stringc& mapName, s32 collision);
  void CreatePlayers();
...
void CQuake3EventHandler::CreateGUI()
{
  ...
  env->getSkin()->setColor(EGDC_BUTTON_TEXT, _ARGB(240, 0xAA, 0xAA, 0xAA));
  env->getSkin()->setColor(EGDC_3D_HIGH_LIGHT, _ARGB(240, 0x22, 0x22, 0x22));
  env->getSkin()->setColor(EGDC_3D_FACE, _ARGB(240, 0x44, 0x44, 0x44));
  env->getSkin()->setColor(EGDC_EDITABLE, _ARGB(240, 0x44, 0x44, 0x44));
  env->getSkin()->setColor(EGDC_FOCUSED_EDITABLE, _ARGB(240, 0x54, 0x54, 0x54));
  env->getSkin()->setColor(EGDC_WINDOW, _ARGB(240, 0x66, 0x66, 0x66));
  // minimal gui size 800x600
  dimension2du dim(800, 600);
  dimension2du vdim(Game->Device->getVideoDriver()->getScreenSize());
  if (vdim.Height >= dim.Height && vdim.Width >= dim.Width) {
    //dim = vdim;
  }
  else {
  }
  gui.Window = env->addWindow(IRECT(0, 0, dim.Width, dim.Height), false, "Quake3 Explorer");
  ...
  gui.StatusLine = env->addStaticText(0, IRECT(5, dim.Height - 30, dim.Width - 5, dim.Height - 10),
      false, false, gui.Window, -1, true
                                     );
  env->addStaticText("VideoDriver:", IRECT(dim.Width - 400, 24, dim.Width - 310, 40), false, false, gui.Window, -1, false);
  gui.VideoDriver = env->addComboBox(IRECT(dim.Width - 300, 24, dim.Width - 10, 40), gui.Window);
  gui.VideoDriver->addItem("Direct3D 9.0c", EDT_DIRECT3D9);
  gui.VideoDriver->addItem("Direct3D 8.1", EDT_DIRECT3D8);
  gui.VideoDriver->addItem("OpenGL 1.5", EDT_OPENGL);
  gui.VideoDriver->addItem("Software Renderer", EDT_SOFTWARE);
  gui.VideoDriver->addItem("Burning's Video (TM) Thomas Alten", EDT_BURNINGSVIDEO);
  ...
  env->addStaticText("VideoMode:", IRECT(dim.Width - 400, 44, dim.Width - 310, 60), false, false, gui.Window, -1, false);
  gui.VideoMode = env->addComboBox(IRECT(dim.Width - 300, 44, dim.Width - 10, 60), gui.Window);
  ...
  if (modeList) {
    ...
    for (i = 0; i != modeList->getVideoModeCount(); ++i) {
      ...
      const c8* a = "";
      if (equals(aspect, 1.3333333333f)) {
        a = "4:3";
      }
      else if (equals(aspect, 1.6666666f)) {
        a = "15:9 widescreen";
      }
      else if (equals(aspect, 1.7777777f)) {
        a = "16:9 widescreen";
      }
      else if (equals(aspect, 1.6f)) {
        a = "16:10 widescreen";
      }
      else if (equals(aspect, 2.133333f)) {
        a = "20:9 widescreen";
      }
      snprintf(buf, sizeof(buf), "%d x %d, %s", w, h, a);
      gui.VideoMode->addItem(stringc(buf), val);
    }
  }
  ...
  gui.FullScreen = env->addCheckBox(Game->deviceParam.Fullscreen, IRECT(dim.Width - 400, 64, dim.Width - 300, 80), gui.Window, -1, "Fullscreen");
  gui.FullScreen->setToolTipText("Set Fullscreen or Window Mode");
  gui.Bit32 = env->addCheckBox(Game->deviceParam.Bits == 32, IRECT(dim.Width - 300, 64, dim.Width - 240, 80), gui.Window, -1, "32Bit");
  gui.Bit32->setToolTipText("Use 16 or 32 Bit");
  env->addStaticText("MultiSample:", IRECT(dim.Width - 235, 64, dim.Width - 150, 80), false, false, gui.Window, -1, false);
  gui.MultiSample = env->addScrollBar(true, IRECT(dim.Width - 150, 64, dim.Width - 70, 80), gui.Window, -1);
  ...
  gui.SetVideoMode = env->addButton(IRECT(dim.Width - 60, 64, dim.Width - 10, 80), gui.Window, -1, "set");
  gui.SetVideoMode->setToolTipText("Set Video Mode with current values");
  env->addStaticText("Gamma:", IRECT(dim.Width - 400, 104, dim.Width - 310, 120), false, false, gui.Window, -1, false);
  gui.Gamma = env->addScrollBar(true, IRECT(dim.Width - 300, 104, dim.Width - 10, 120), gui.Window, -1);
  gui.Gamma->setMin(50);
  gui.Gamma->setMax(350);
  gui.Gamma->setSmallStep(1);
  gui.Gamma->setLargeStep(10);
  gui.Gamma->setPos(floor32(Game->GammaValue * 100.f));
  ...
  env->addStaticText("Tesselation:", IRECT(dim.Width - 400, 124, dim.Width - 310, 140), false, false, gui.Window, -1, false);
  gui.Tesselation = env->addScrollBar(true, IRECT(dim.Width - 300, 124, dim.Width - 10, 140), gui.Window, -1);
  ...
  gui.Collision = env->addCheckBox(true, IRECT(dim.Width - 400, 150, dim.Width - 300, 166), gui.Window, -1, "Collision");
  gui.Collision->setToolTipText("Set collision on or off ( flythrough ). \nPress F7 on your Keyboard");
  gui.Visible_Map = env->addCheckBox(true, IRECT(dim.Width - 300, 150, dim.Width - 240, 166), gui.Window, -1, "Map");
  gui.Visible_Map->setToolTipText("Show or not show the static part the Level. \nPress F3 on your Keyboard");
  gui.Visible_Shader = env->addCheckBox(true, IRECT(dim.Width - 240, 150, dim.Width - 170, 166), gui.Window, -1, "Shader");
  gui.Visible_Shader->setToolTipText("Show or not show the Shader Nodes. \nPress F4 on your Keyboard");
  gui.Visible_Fog = env->addCheckBox(true, IRECT(dim.Width - 170, 150, dim.Width - 110, 166), gui.Window, -1, "Fog");
  gui.Visible_Fog->setToolTipText("Show or not show the Fog Nodes. \nPress F5 on your Keyboard");
  gui.Visible_Unresolved = env->addCheckBox(true, IRECT(dim.Width - 110, 150, dim.Width - 10, 166), gui.Window, -1, "Unresolved");
  gui.Visible_Unresolved->setToolTipText("Show the or not show the Nodes the Engine can't handle. \nPress F6 on your Keyboard");
  gui.Visible_Skydome = env->addCheckBox(true, IRECT(dim.Width - 110, 180, dim.Width - 10, 196), gui.Window, -1, "Skydome");
  gui.Visible_Skydome->setToolTipText("Show the or not show the Skydome.");
  //Respawn = env->addButton ( IRECT( dim.Width - 260, 90, dim.Width - 10, 106 ), 0,-1, "Respawn" );
  env->addStaticText("Archives:", IRECT(5, dim.Height - 530, dim.Width - 600, dim.Height - 514), false, false, gui.Window, -1, false);
  gui.ArchiveAdd = env->addButton(IRECT(dim.Width - 725, dim.Height - 530, dim.Width - 665, dim.Height - 514), gui.Window, -1, "add");
  gui.ArchiveAdd->setToolTipText("Add an archive, usually packed zip-archives (*.pk3) to the Filesystem");
  gui.ArchiveRemove = env->addButton(IRECT(dim.Width - 660, dim.Height - 530, dim.Width - 600, dim.Height - 514), gui.Window, -1, "del");
  gui.ArchiveRemove->setToolTipText("Remove the selected archive from the FileSystem.");
  gui.ArchiveUp = env->addButton(IRECT(dim.Width - 575, dim.Height - 530, dim.Width - 515, dim.Height - 514), gui.Window, -1, "up");
  gui.ArchiveUp->setToolTipText("Arrange Archive Look-up Hirachy. Move the selected Archive up");
  gui.ArchiveDown = env->addButton(IRECT(dim.Width - 510, dim.Height - 530, dim.Width - 440, dim.Height - 514), gui.Window, -1, "down");
  gui.ArchiveDown->setToolTipText("Arrange Archive Look-up Hirachy. Move the selected Archive down");
  gui.ArchiveList = env->addTable(IRECT(5, dim.Height - 510, dim.Width - 450, dim.Height - 410), gui.Window);
  ...
  env->addStaticText("Maps:", IRECT(5, dim.Height - 400, dim.Width - 450, dim.Height - 380), false, false, gui.Window, -1, false);
  gui.MapList = env->addListBox(IRECT(5, dim.Height - 380, dim.Width - 450, dim.Height - 40), gui.Window, -1, true);
  ...
  env->addStaticText("Scenegraph:", IRECT(dim.Width - 400, dim.Height - 400, dim.Width - 5, dim.Height - 380), false, false, gui.Window, -1, false);
  gui.SceneTree = env->addTreeView(IRECT(dim.Width - 400, dim.Height - 380, dim.Width - 5, dim.Height - 40),
  ...
  gui.Logo = env->addImage(driver->getTexture("irrlichtlogo3.png"), position2di(5, 16), true, 0);
...
*/
void CQuake3EventHandler::AddArchive(const char* archiveName)
{
  ...
  if (gui.MapList) {
    ...
    for (i = 0; i < fileList->getFileCount(); ++i) {
      ...
      if (s.find(".bsp") >= 0) {
        ...
        if (image) {
          IImage* filter = driver->createImage(ECF_R8G8B8, dim);
...
/* Load new map
*/
void CQuake3EventHandler::LoadMap(const stringc& mapName, s32 collision)
{
  if (0 == mapName_size) {
    return;
  }
  ...
  IReadFile* file = fs->createMemoryReadFile(&Game->loadParam,
      sizeof(Game->loadParam), "levelparameter.cfg", false);
  // load cfg file
  smgr->getMesh(file);
  file->drop();
  // load the actual map
  ...
  */
  ...
  //s32 minimalNodes = b0 ? s32_max ( 2048, b0->getVertexCount() / 32 ) : 2048;
...
void CQuake3EventHandler::AddSky(u32 dome, const c8* texture)
{
  ...
  //SkyNode->getMaterial(0).ZBuffer = EMDF_DEPTH_LESS_EQUAL;
...
bool CQuake3EventHandler::OnEvent(const SEvent& eve)
{
  ...
  if (Game->guiActive && eve.EventType == EET_GUI_EVENT) {
    if (eve.GUIEvent.Caller == gui.MapList && eve.GUIEvent.EventType == EGET_LISTBOX_SELECTED_AGAIN) {
      s32 selected = gui.MapList->getSelected();
      if (selected >= 0) {
        stringc loadMap = gui.MapList->getListItem(selected);
    ...
    else if (eve.GUIEvent.Caller == gui.ArchiveRemove && eve.GUIEvent.EventType == EGET_BUTTON_CLICKED) {
      Game->Device->getFileSystem()->removeFileArchive(gui.ArchiveList->getSelected());
      Game->CurrentMapName = "";
      AddArchive("");
    }
    else if (eve.GUIEvent.Caller == gui.ArchiveAdd && eve.GUIEvent.EventType == EGET_BUTTON_CLICKED) {
    ...
    else if (eve.GUIEvent.Caller == gui.ArchiveFileOpen && eve.GUIEvent.EventType == EGET_FILE_SELECTED) {
      AddArchive(gui.ArchiveFileOpen->getFileName());
      gui.ArchiveFileOpen = 0;
    }
    else if (eve.GUIEvent.Caller == gui.ArchiveFileOpen && eve.GUIEvent.EventType == EGET_DIRECTORY_SELECTED) {
      AddArchive(gui.ArchiveFileOpen->getDirectoryName());
    }
    else if (eve.GUIEvent.Caller == gui.ArchiveFileOpen && eve.GUIEvent.EventType == EGET_FILE_CHOOSE_DIALOG_CANCELLED) {
      gui.ArchiveFileOpen = 0;
    }
      ...
      if (Game->Device->getFileSystem()->moveFileArchive(gui.ArchiveList->getSelected(), rel)) {
        s32 newIndex = s32_clamp(gui.ArchiveList->getSelected() + rel, 0, gui.ArchiveList->getRowCount() - 1);
        AddArchive("");
        gui.ArchiveList->setSelected(newIndex);
        Game->CurrentMapName = "";
      }
    }
    else if (eve.GUIEvent.Caller == gui.VideoDriver && eve.GUIEvent.EventType == EGET_COMBO_BOX_CHANGED) {
      Game->deviceParam.DriverType = (E_DRIVER_TYPE) gui.VideoDriver->getItemData(gui.VideoDriver->getSelected());
    }
    else if (eve.GUIEvent.Caller == gui.VideoMode && eve.GUIEvent.EventType == EGET_COMBO_BOX_CHANGED) {
    ...
    else if (eve.GUIEvent.Caller == gui.FullScreen && eve.GUIEvent.EventType == EGET_CHECKBOX_CHANGED) {
      Game->deviceParam.Fullscreen = gui.FullScreen->isChecked();
    }
    else if (eve.GUIEvent.Caller == gui.Bit32 && eve.GUIEvent.EventType == EGET_CHECKBOX_CHANGED) {
      Game->deviceParam.Bits = gui.Bit32->isChecked() ? 32 : 16;
    }
    else if (eve.GUIEvent.Caller == gui.MultiSample && eve.GUIEvent.EventType == EGET_SCROLL_BAR_CHANGED) {
      Game->deviceParam.AntiAlias = gui.MultiSample->getPos();
    }
    else if (eve.GUIEvent.Caller == gui.Tesselation && eve.GUIEvent.EventType == EGET_SCROLL_BAR_CHANGED) {
      Game->loadParam.patchTesselation = gui.Tesselation->getPos();
    }
    else if (eve.GUIEvent.Caller == gui.Gamma && eve.GUIEvent.EventType == EGET_SCROLL_BAR_CHANGED) {
      Game->GammaValue = gui.Gamma->getPos() * 0.01f;
      Game->Device->setGammaRamp(Game->GammaValue, Game->GammaValue, Game->GammaValue, 0.f, 0.f);
    }
    else if (eve.GUIEvent.Caller == gui.SetVideoMode && eve.GUIEvent.EventType == EGET_BUTTON_CLICKED) {
      Game->retVal = 2;
      Game->Device->closeDevice();
    }
    else if (eve.GUIEvent.Caller == gui.Window && eve.GUIEvent.EventType == EGET_ELEMENT_CLOSED) {
      Game->Device->closeDevice();
    }
    else if (eve.GUIEvent.Caller == gui.Collision && eve.GUIEvent.EventType == EGET_CHECKBOX_CHANGED) {
      // set fly through active
      Game->flyTroughState ^= 1;
      Player[0].cam()->setAnimateTarget(Game->flyTroughState == 0);
      printf("collision %d\n", Game->flyTroughState == 0);
    }
    else if (eve.GUIEvent.Caller == gui.Visible_Map && eve.GUIEvent.EventType == EGET_CHECKBOX_CHANGED) {
    ...
    else if (eve.GUIEvent.Caller == gui.Visible_Shader && eve.GUIEvent.EventType == EGET_CHECKBOX_CHANGED) {
    ...
    else if (eve.GUIEvent.Caller == gui.Visible_Skydome && eve.GUIEvent.EventType == EGET_CHECKBOX_CHANGED) {
      if (SkyNode) {
    ...
    else if (eve.GUIEvent.Caller == gui.Respawn && eve.GUIEvent.EventType == EGET_BUTTON_CLICKED) {
      Player[0].respawn();
    }
    return false;
  }
  ...
  if (eve.EventType == EET_KEY_INPUT_EVENT && eve.KeyInput.PressedDown == false) {
    ...
    if (eve.KeyInput.Key == KEY_ESCAPE) {
      SetGUIActive(3);
    }
  ...
  if (eve.EventType == EET_KEY_INPUT_EVENT && eve.KeyInput.Key == KEY_KEY_C) {
    ...
    if (anim && 0 == Game->flyTroughState) {
      if (false == eve.KeyInput.PressedDown) {
        // stand up
        anim->setEllipsoidRadius(f3VECTOR(30, 45, 30));
        anim->setEllipsoidTranslation(f3VECTOR(0, 40, 0));
      }
      else {
        // on your knees
        anim->setEllipsoidRadius(f3VECTOR(30, 20, 30));
        anim->setEllipsoidTranslation(f3VECTOR(0, 20, 0));
      }
      return true;
    }
  }
...
void CQuake3EventHandler::useItem(Q3Player* player)
{
  ...
  // create fire ball
  ISceneNode* node = 0;
  node = smgr->addBillboardSceneNode(BulletParent, dimension2df(10, 10), start);
  ...
  for (i = 0; i < Impacts_size; ++i) {
    ...
    for (g = 0; g != 2; ++g) {
      ...
      IParticleEmitter* em = pas->createBoxEmitter(
          F3AABBOX(-4.f, 0.f, -4.f, 20.f, smoke[g].minparticleSize, 20.f),
          direction, smoke[g].minParticle, smoke[g].maxParticle,
          _ARGB(0, 0, 0, 0), _ARGB(0, 128, 128, 128),
          250, 4000, 60);
      ...
      pas->setMaterialType(EMT_TRANSPARENT_ADD_COLOR);
...
void CQuake3EventHandler::Render()
{
  ...
  if (anaglyph) {
    ...
    driver->beginScene(true, true, _ARGB(0, 0, 0, 0));
    ...
    vector3df pos;
    matrix4   move;
    move.setTranslation(f3VECTOR(-1.5f, 0.0f, 0.0f));
...
void CQuake3EventHandler::Animate()
{
  ...
  if (player->Anim[0].flags & FIRED) {
    ...
    snprintf(msg, 128,
        "Q3 %s [%ls], FPS:%03d Tri:%.03fm Cull %d/%d nodes (%d,%d,%d)",
        Game->CurrentMapName,
        driver->getName(),
        driver->getFPS(),
    ...
    snprintf(msg, 128,
        "%03d fps, F1 GUI on/off, F2 respawn, F3-F6 toggle Nodes, F7 Collision on/off"
        ", F8 Gravity on/off, Right Mouse Toggle GUI",
