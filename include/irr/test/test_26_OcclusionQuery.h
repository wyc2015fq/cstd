...
/*
We create an IrrlichtDevice and the scene nodes. One occluder, one
...
int main()
{
  ...
  */
  ISceneNode* plane = smgr->addMeshSceneNode(smgr->addHillPlaneMesh(
      ...
      f3VECTOR(0, 0, 20), f3VECTOR(270, 0, 0));
  if (plane) {
  ...
  */
  ...
  while (device->run()) {
    ...
    */
    ...
    if (lastFPS != fps) {
      char* tmp("OcclusionQuery Example [");
      tmp += driver->getName();
      tmp += "] fps: ";
      tmp += fps;
