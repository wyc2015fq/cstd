...
{
public:
  MyEventReceiver(ISceneNode* terrain, ISceneNode* skybox, ISceneNode* skydome) :
  ...
  bool OnEvent(const SEvent& event) {
    ...
    if (event.EventType == EET_KEY_INPUT_EVENT && !event.KeyInput.PressedDown) {
      ...
      case KEY_KEY_S: // toggle skies
        ...
        Skydome->setVisible(!showBox);
        return true;
      case KEY_KEY_X: // toggle debug information
        showDebug = !showDebug;
        Terrain->setDebugDataVisible(showDebug ? EDS_BBOX_ALL : EDS_OFF);
        return true;
      default:
        break;
      }
...
int main()
{
  ...
  // you can add more parameters if desired, check SIrrlichtCreationParameters
  ...
  */
  ...
  env->addStaticText(
      ...
      IRECT(10, 421, 250, 475), true, true, 0, -1, true);
  // add camera
  ...
  */
  ...
  while (device->run())
    if (device->isWindowActive()) {
      ...
      if (lastFPS != fps) {
        char* str = "Terrain Renderer - Irrlicht Engine [";
        str += driver->getName();
        str += "] FPS:";
        str += fps;
