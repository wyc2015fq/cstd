...
int main()
{
  ...
  if (model_node) {
    ...
    model_node->setMD2Animation(EMAT_RUN);
  ...
  if (map) {
    ...
    //Set position
    map_node->setPosition(f3VECTOR(-850, -220, -850));
  }
  /*
  ...
  */
  ...
  //Front
  camera[0] = smgr->addCameraSceneNode(0, f3VECTOR(50, 0, 0), f3VECTOR(0, 0, 0));
  //Top
  camera[1] = smgr->addCameraSceneNode(0, f3VECTOR(0, 50, 0), f3VECTOR(0, 0, 0));
  //Left
  camera[2] = smgr->addCameraSceneNode(0, f3VECTOR(0, 0, 50), f3VECTOR(0, 0, 0));
  //User-controlled
  camera[3] = smgr->addCameraSceneNodeFPS();
  // don't start at sydney's position
  if (camera[3]) {
    camera[3]->setPosition(f3VECTOR(-50, 0, -50));
  }
  /*
  ...
  */
  while (device->run()) {
    ...
    driver->setViewPort(IRECT(0, 0, ResX, ResY));
    driver->beginScene(true, true, _ARGB(255, 100, 100, 100));
    //If SplitScreen is used
    if (SplitScreen) {
      //Activate camera1
      smgr->setActiveCamera(camera[0]);
      //Set viewpoint to the first quarter (left top)
      driver->setViewPort(IRECT(0, 0, ResX / 2, ResY / 2));
      //Draw scene
      smgr->drawAll();
      //Activate camera2
      smgr->setActiveCamera(camera[1]);
      //Set viewpoint to the second quarter (right top)
      driver->setViewPort(IRECT(ResX / 2, 0, ResX, ResY / 2));
      //Draw scene
      smgr->drawAll();
      //Activate camera3
      smgr->setActiveCamera(camera[2]);
      //Set viewpoint to the third quarter (left bottom)
      driver->setViewPort(IRECT(0, ResY / 2, ResX / 2, ResY));
      //Draw scene
      smgr->drawAll();
      //Set viewport the last quarter (right bottom)
      driver->setViewPort(IRECT(ResX / 2, ResY / 2, ResX, ResY));
    }
    //Activate camera4
    ...
    */
    ...
    if (driver->getFPS() != lastFPS) {
      ...
      char* tmp = "Irrlicht SplitScreen-Example (FPS: ";
      tmp += lastFPS;
      tmp += ")";
