...
int main()
{
  ...
  IrrlichtDevice* device =
      createDevice(driverType, dimension2du(640, 480), 16, false);
  if (device == 0) {
  ...
  */
  ...
  if (q3node) {
    q3node->setPosition(f3VECTOR(-1350, -130, -1400));
  ...
  */
  ...
  camera->setPosition(f3VECTOR(50, 50, -60));
  camera->setTarget(f3VECTOR(-70, 30, -60));
  if (selector) {
    ISceneNodeAnimator* anim = smgr->createCollisionResponseAnimator(
        selector, camera, f3VECTOR(30, 50, 30),
        f3VECTOR(0, -10, 0), f3VECTOR(0, 30, 0));
  ...
  bill->setSize(dimension2df(20.0f, 20.0f));
  ...
  node->setPosition(f3VECTOR(-90, -15, -140)); // Put its feet on the floor.
  node->setScale(f3VECTOR(1.6f)); // Make it appear realistically scaled
  ...
  node = smgr->addAnimatedMeshSceneNode(smgr->getMesh("../../media/ninja.b3d"),
      0, IDFlag_IsPickable | IDFlag_IsHighlightable);
  node->setScale(f3VECTOR(10));
  node->setPosition(f3VECTOR(-75, -66, -80));
  node->setRotation(f3VECTOR(0, 90, 0));
  ...
  node = smgr->addAnimatedMeshSceneNode(smgr->getMesh("../../media/dwarf.x"),
      0, IDFlag_IsPickable | IDFlag_IsHighlightable);
  node->setPosition(f3VECTOR(-70, -66, -30)); // Put its feet on the floor.
  node->setRotation(f3VECTOR(0, -90, 0)); // And turn it towards the camera.
  node->setAnimationSpeed(20.f);
  node->getMaterial(0).Lighting = true;
  selector = smgr->createTriangleSelector(node);
  node->setTriangleSelector(selector);
  ...
  node = smgr->addAnimatedMeshSceneNode(smgr->getMesh("../../media/yodan.mdl"),
      0, IDFlag_IsPickable | IDFlag_IsHighlightable);
  node->setPosition(f3VECTOR(-90, -25, 20));
  node->setScale(f3VECTOR(0.8f));
  node->getMaterial(0).Lighting = true;
  node->setAnimationSpeed(20.f);
  // Just do the same as we did above.
  selector = smgr->createTriangleSelector(node);
  ...
  ILightSceneNode* light = smgr->addLightSceneNode(0, f3VECTOR(-60, 100, 400),
  ...
  while (device->run())
    if (device->isWindowActive()) {
      ...
      if (selectedSceneNode) {
        ...
        driver->draw3DTriangle(hitTriangle, _ARGB(0, 255, 0, 0));
      ...
      if (lastFPS != fps) {
        char* str = "Collision detection example - Irrlicht Engine [";
        str += driver->getName();
        str += "] FPS:";
        str += fps;
