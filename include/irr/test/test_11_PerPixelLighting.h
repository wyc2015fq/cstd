...
{
public:
  MyEventReceiver(ISceneNode* room, ISceneNode* earth,
    ...
    IGUIWindow* window = env->addWindow(
        IRECT(460, 375, 630, 470), false, "Use 'E' + 'R' to change");
    ListBox = env->addListBox(
        IRECT(2, 22, 165, 88), window);
    ...
    ProblemText = env->addStaticText(
        ...
        IRECT(150, 20, 470, 80));
    ProblemText->setOverrideColor(_ARGB(100, 255, 255, 255));
    // set start material (prefer parallax mapping if available)
    IMaterialRenderer* renderer =
        Driver->getMaterialRenderer(EMT_PARALLAX_MAP_SOLID);
...
private:
  ...
  void setMaterial() {
    ...
    case 0:
      ...
      break;
    case 1:
      type = EMT_NORMAL_MAP_SOLID;
      break;
    case 2:
      type = EMT_PARALLAX_MAP_SOLID;
      break;
    }
    ...
    switch (ListBox->getSelected()) {
    case 0:
      type = EMT_TRANSPARENT_VERTEX_ALPHA;
      break;
    case 1:
      type = EMT_NORMAL_MAP_TRANSPARENT_VERTEX_ALPHA;
      break;
    case 2:
      type = EMT_PARALLAX_MAP_TRANSPARENT_VERTEX_ALPHA;
      break;
    }
    ...
    the case.
    */
    IMaterialRenderer* renderer = Driver->getMaterialRenderer(type);
...
int main()
{
  ...
  */
  ...
  camera->setPosition(f3VECTOR(-200, 200, -200));
  ...
  */
  driver->setFog(_ARGB(0, 138, 125, 81), EFT_FOG_LINEAR, 250, 1000, .003f, true, false);
  /*
  ...
  */
  IAnimatedMesh* roomMesh = smgr->getMesh("../../media/room.3ds");
  ISceneNode* room = 0;
  ISceneNode* earth = 0;
  if (roomMesh) {
  ...
  */
  ...
  if (earthMesh) {
    ...
    matrix4 m;
    m.setScale(f3VECTOR(50, 50, 50));
    manipulator->transform(tangentSphereMesh, m);
    earth = smgr->addMeshSceneNode(tangentSphereMesh);
    earth->setPosition(f3VECTOR(-70, 130, 45));
    // load heightmap, create normal map from it and set it
    ITexture* earthNormalMap = driver->getTexture("../../media/earthbump.jpg");
    if (earthNormalMap) {
    ...
    ISceneNodeAnimator* anim =
        smgr->createRotationAnimator(f3VECTOR(0, 0.1f, 0));
  ...
  */
  ...
  ILightSceneNode* light1 =
      smgr->addLightSceneNode(0, f3VECTOR(0, 0, 0),
          SColorf(0.5f, 1.0f, 0.5f, 0.0f), 800.0f);
  light1->setDebugDataVisible(EDS_BBOX);
  // add fly circle animator to light 1
  ISceneNodeAnimator* anim =
      smgr->createFlyCircleAnimator(f3VECTOR(50, 300, 0), 190.0f, -0.003f);
  light1->addAnimator(anim);
  anim->drop();
  // attach billboard to the light
  IBillboardSceneNode* bill =
      smgr->addBillboardSceneNode(light1, dimension2df(60, 60));
  ...
  */
  ...
  ISceneNode* light2 =
      smgr->addLightSceneNode(0, f3VECTOR(0, 0, 0),
          SColorf(1.0f, 0.2f, 0.2f, 0.0f), 800.0f);
  // add fly circle animator to light 2
  anim = smgr->createFlyCircleAnimator(f3VECTOR(0, 150, 0), 200.0f,
      0.001f, f3VECTOR(0.2f, 0.9f, 0.f));
  ...
  IParticleEmitter* em = ps->createBoxEmitter(
      F3AABBOX(-3, 0, -3, 3, 1, 3),
      f3VECTOR(0.0f, 0.03f, 0.0f),
      80, 100,
      _ARGB(10, 255, 255, 255), _ARGB(10, 255, 255, 255),
      400, 1100);
  em->setMinStartSize(dimension2df(30.0f, 40.0f));
  em->setMaxStartSize(dimension2df(30.0f, 40.0f));
  ps->setEmitter(em);
  em->drop();
  ...
  */
  ...
  while (device->run())
    if (device->isWindowActive()) {
      ...
      if (lastFPS != fps) {
        char* str = "Per pixel lighting example - Irrlicht Engine [";
        str += driver->getName();
        str += "] FPS:";
        str += fps;
