...
{
public:
  void OnSetConstants(IMaterialRendererServices* services,
      s32 userData) {
    ...
    if (UseHighLevelShaders) {
      services->setVertexShaderConstant("mInvWorld", invWorld, 16);
    }
    else {
      services->setVertexShaderConstant(invWorld, 0, 4);
    }
    // set clip matrix
    ...
    if (UseHighLevelShaders) {
      services->setVertexShaderConstant("mWorldViewProj", worldViewProj, 16);
    }
    else {
      services->setVertexShaderConstant(worldViewProj, 4, 4);
    }
    // set camera position
    vector3df pos = device->getSceneManager()->
    ...
    if (UseHighLevelShaders) {
      services->setVertexShaderConstant("mTransWorld", world, 16);
      ...
      services->setVertexShaderConstant(world, 10, 4);
    }
  }
};
/*
...
int main()
{
  ...
  */
  char* vsFileName; // filename for the vertex shader
  char* psFileName; // filename for the pixel shader
  switch (driverType) {
  case EDT_DIRECT3D8:
  ...
  */
  ...
  is just a simple 32 bit value, like EMT_SOLID. So we only need
  ...
  */
  ...
  if (gpu) {
    ...
    if (UseHighLevelShaders) {
      ...
      // enum value EGSL_CG
      const E_GPU_SHADING_LANGUAGE shadingLanguage =
          UseCgShaders ? EGSL_CG : EGSL_DEFAULT;
      ...
      newMaterialType1 = gpu->addShaderMaterialFromFiles(vsFileName,
          psFileName, mc, EMT_SOLID);
      newMaterialType2 = gpu->addShaderMaterialFromFiles(vsFileName,
          psFileName, mc, EMT_TRANSPARENT_ADD_COLOR);
    }
    mc->drop();
  }
  /*
  ...
  */
  ...
  node->setPosition(f3VECTOR(0, -10, 50));
  node->setMaterialTexture(0, driver->getTexture("../../media/wall.bmp"));
  node->setMaterialFlag(EMF_LIGHTING, false);
  ...
  smgr->addTextSceneNode(gui->getBuiltInFont(),
      ...
      _ARGB(255, 255, 255, 255), node);
  anim = smgr->createRotationAnimator(f3VECTOR(0, 0.3f, 0));
  node->addAnimator(anim);
  anim->drop();
  /*
  ...
  */
  ...
  node = smgr->addCubeSceneNode(50);
  node->setPosition(f3VECTOR(0, 50, 25));
  node->setMaterialTexture(0, driver->getTexture("../../media/wall.bmp"));
  node->setMaterialFlag(EMF_LIGHTING, false);
  ...
  */
  ...
  cam->setPosition(f3VECTOR(-100, 50, 100));
  cam->setTarget(f3VECTOR(0, 0, 0));
  ...
  */
  ...
  while (device->run())
    if (device->isWindowActive()) {
      driver->beginScene(true, true, _ARGB(255, 0, 0, 0));
      smgr->drawAll();
      driver->endScene();
      int fps = driver->getFPS();
      if (lastFPS != fps) {
        char* str = "Irrlicht Engine - Vertex and pixel shader example [";
        str += driver->getName();
        str += "] FPS:";
        str += fps;
