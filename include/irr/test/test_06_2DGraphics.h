...
int main()
{
  ...
  */
  ...
  IRECT imp1(349, 15, 385, 78);
  IRECT imp2(387, 15, 423, 78);
  /*
  ...
  */
  while (device->run() && driver) {
    if (device->isWindowActive()) {
      ...
      driver->beginScene(true, true, _ARGB(255, 120, 102, 136));
      /*
      ...
      */
      ...
      driver->draw2DImage(images, position2di(50, 50),
          IRECT(0, 0, 342, 224), 0,
          _ARGB(255, 255, 255, 255), true);
      // draw flying imp
      driver->draw2DImage(images, position2di(164, 125),
          (time / 500 % 2) ? imp1 : imp2, 0,
          _ARGB(255, 255, 255, 255), true);
      // draw second flying imp with colorcylce
      driver->draw2DImage(images, position2di(270, 105),
          (time / 500 % 2) ? imp1 : imp2, 0,
          _ARGB(255, (time) % 255, 255, 255), true);
      /*
      ...
      */
      ...
      driver->draw2DImage(images, IRECT(10, 10, 108, 48),
          IRECT(354, 87, 442, 118));
      driver->enableMaterial2D(false);
      /*
      Finally draw a half-transparent rect under the mouse cursor.
      */
      position2di m = device->getCursorControl()->getPosition();
      driver->draw2DRectangle(_ARGB(100, 255, 255, 255),
          IRECT(m.X - 20, m.Y - 20, m.X + 20, m.Y + 20));
      driver->endScene();
    }
  }
  device->drop();
