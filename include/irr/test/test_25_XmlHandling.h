...
public:
  ...
  SettingManager(const stringc& settings_file): SettingsFile(settings_file), NullDevice(0) {
    // Irrlicht null device, we want to load settings before we actually created our device, therefore, nulldevice
    NullDevice = createDevice(EDT_NULL);
    ...
    DriverOptions.insert("Software", EDT_SOFTWARE);
    DriverOptions.insert("OpenG", EDT_OPENGL);
    DriverOptions.insert("Direct3D9", EDT_DIRECT3D9);
    //some resolution options
    ResolutionOptions.insert("640x480", dimension2du(640, 480));
    ResolutionOptions.insert("800x600", dimension2du(800, 600));
    ResolutionOptions.insert("1024x768", dimension2du(1024, 768));
    //our preferred defaults
    SettingMap.insert("driver", "Direct3D9");
    SettingMap.insert("resolution", "640x480");
    SettingMap.insert("fullscreen", "0"); //0 is false
  }
  ...
  */
  bool load() {
    ...
    IXMLReader* xml = NullDevice->getFileSystem()->createXMLReader(SettingsFile);  //create xml reader
    if (!xml) {
      return false;
    }
    const stringc settingTag("setting"); //we'll be looking for this tag in the xml
    stringc currentSection; //keep track of our current section
    const stringc videoTag("video"); //constant for videotag
    ...
    while (xml->read()) {
      ...
      case EXN_ELEMENT: {
        ...
        //we are in the video section and we find a setting to parse
          ...
          //read in the key
          stringc key = xml->getAttributeValueSafe("name");
          ...
          if (!key.empty()) {
            ...
            SettingMap[key] = xml->getAttributeValueSafe("value");
          }
        }
        //..
  ...
  bool save() {
    ...
    //create xml writer
    IXMLWriter* xwriter = NullDevice->getFileSystem()->createXMLWriter(SettingsFile);
    if (!xwriter) {
      return false;
    }
    ...
    map<stringc, stringc>::Iterator i = SettingMap.getIterator();
    for (; !i.atEnd(); i++) {
      ...
      xwriter->writeElement("setting", true, "name", i->getKey(), "value", i->getValue());
      xwriter->writeLineBreak();
    }
    xwriter->writeLineBreak();
    //close video section
    xwriter->writeClosingTag("video");
    xwriter->writeLineBreak();
    //..
    // You can add writing sound settings, savegame information etc
    //..
    //close mygame section
    xwriter->writeClosingTag("mygame");
    //delete xml writer
    xwriter->drop();
    return true;
  }
  // Set setting in our manager
  void setSetting(const stringc& name, const stringc& value) {
    SettingMap[name] = value;
  }
  // set setting overload to quickly assign integers to our setting map
  void setSetting(const stringc& name, s32 value) {
    SettingMap[name] = stringc(value);
  }
  // Get setting as string
  stringc getSetting(const stringc& key) const {
    ...
    map<stringc, stringc>::Node* n = SettingMap.find(key);
    if (n) {
      return n->getValue();
    }
    else {
  ...
  //
  bool getSettingAsBoolean(const stringc& key) const {
    stringc s = getSetting(key);
    if (s.empty()) {
      return false;
    }
    return s.equals_ignore_case("1");
  }
  //
  s32 getSettingAsInteger(const stringc& key) const {
    //we implicitly cast to string instead of stringc because strtol10 does not accept wide strings
...
public:
  map<stringc, s32> DriverOptions; //available options for driver config
  map<stringc, dimension2du> ResolutionOptions; //available options for resolution config
private:
  ...
  map<stringc, stringc> SettingMap; //current config
  stringc SettingsFile; // location of the xml, usually the
      ...
      case EGET_BUTTON_CLICKED: {
        ...
        if (event.GUIEvent.Caller == App.ButtonSave) {
          ...
          if (App.ListboxDriver->getSelected() != -1) {
            App.Settings->setSetting("driver",  App.ListboxDriver->getListItem(App.ListboxDriver->getSelected()));
          }
          //if there is a selection write it
          if (App.ListboxResolution->getSelected() != -1) {
            App.Settings->setSetting("resolution", App.ListboxResolution->getListItem(App.ListboxResolution->getSelected()));
          }
          App.Settings->setSetting("fullscreen",  App.CheckboxFullscreen->isChecked());
          if (App.Settings->save()) {
            App.Gui->addMessageBox("settings save", "settings saved, please restart for settings to change effect", "", true);
          }
        }
...
void createSettingsDialog(SAppContext& app)
{
  ...
  for (s32 i = 0; i < EGDC_COUNT ; ++i) {
    SColor col = app.Gui->getSkin()->getColor((EGUI_DEFAULT_COLOR)i);
    col.setAlpha(255);
    app.Gui->getSkin()->setColor((EGUI_DEFAULT_COLOR)i, col);
  }
  //create video settings windows
  IGUIWindow* windowSettings = app.Gui->addWindow(IRECT(10, 10, 400, 400), true, "Videosettings");
  app.Gui->addStaticText("Select your desired video settings", rect< s32 >(10, 20, 200, 40), false, true, windowSettings);
  // add listbox for driver choice
  app.Gui->addStaticText("Driver", rect< s32 >(10, 50, 200, 60), false, true, windowSettings);
  app.ListboxDriver = app.Gui->addListBox(IRECT(10, 60, 220, 120), windowSettings, 1, true);
  //add all available options to the driver choice listbox
  map<stringc, s32>::Iterator i = app.Settings->DriverOptions.getIterator();
  for (; !i.atEnd(); i++) {
    app.ListboxDriver->addItem(i->getKey());
  }
  //set currently selected driver
  app.ListboxDriver->setSelected(app.Settings->getSetting("driver"));
  // add listbox for resolution choice
  app.Gui->addStaticText("Resolution", rect< s32 >(10, 130, 200, 140), false, true, windowSettings);
  app.ListboxResolution = app.Gui->addListBox(IRECT(10, 140, 220, 200), windowSettings, 1, true);
  //add all available options to the resolution listbox
  map<stringc, dimension2du>::Iterator ri = app.Settings->ResolutionOptions.getIterator();
  for (; !ri.atEnd(); ri++) {
    app.ListboxResolution->addItem(ri->getKey());
  }
  //set currently selected resolution
  app.ListboxResolution->setSelected(app.Settings->getSetting("resolution"));
  ...
  app.CheckboxFullscreen = app.Gui->addCheckBox(
      ...
      IRECT(10, 220, 220, 240), windowSettings, -1,
      "Fullscreen");
  //last but not least add save button
  app.ButtonSave = app.Gui->addButton(
      IRECT(80, 250, 150, 270), windowSettings, 2,
      "Save video settings");
  //exit/cancel button
  app.ButtonExit = app.Gui->addButton(
      IRECT(160, 250, 240, 270), windowSettings, 2,
      "Cancel and exit");
}
/*
...
int main()
{
  ...
  if (!app.Settings->load()) {
    ...
    map<stringc, s32>::Node* driver = app.Settings->DriverOptions.find(app.Settings->getSetting("driver"));
    if (driver) {
      if (IrrlichtDevice::isDriverSupported(static_cast<E_DRIVER_TYPE>(driver->getValue()))) {
    ...
    map<stringc, dimension2du>::Node* res = app.Settings->ResolutionOptions.find(app.Settings->getSetting("resolution"));
    if (res) {
  ...
  app.Device->setWindowCaption("Xmlhandling - Irrlicht engine tutorial");
  ...
  while (!app.ShouldQuit && app.Device->run()) {
    if (app.Device->isWindowActive()) {
      app.Driver->beginScene(true, true, _ARGB(0, 200, 200, 200));
      app.Gui->drawAll();
      app.Driver->endScene();
    }
    app.Device->sleep(10);
