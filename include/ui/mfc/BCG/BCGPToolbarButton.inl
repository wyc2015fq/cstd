								...
								BOOL bDrawBorder, BOOL bGrayDisabledButtons)
{
	...
	if (m_bTextBelow && bHorz)
	{
		...
		dy = sizeImage.cy + 2 * nTextMargin_tb;
	}
	else
	{
	...
	if ((m_bTextBelow && bHorz) || IsDrawText ())
	{
		...
		if (m_bTextBelow && bHorz)
		{
			...
			ptTextOffset.y += sizeImage.cy + nTextMargin_tb;
		...
		else
		{
			if (IsDrawImage ())
			{
				...
				rectText.left = x + ptTextOffset.x + nTextMargin_tb;
			}
			else
			{
				rectText.left = x + nTextMargin_tb + 1;
			}
	...
	else
	{
		...
		if (!m_strText.IsEmpty ())
		{
			if (m_bTextBelow && bHorz)
			{
				...
				m_sizeText.cx += 2 * nTextMargin_tb;

				size.cx = max (size.cx, m_sizeText.cx) + 4 * nTextMargin_tb;
