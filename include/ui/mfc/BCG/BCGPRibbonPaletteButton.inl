...
CSize CBCGPRibbonPaletteIcon::GetRegularSize (CDC* /*pDC*/)
{
	...
	{
		sizeIcon.cx += 2 * nImageMargin_rpbtn;
		sizeIcon.cy += 2 * nImageMargin_rpbtn;
	}
...
CSize CBCGPRibbonPaletteButton::GetRegularSize (CDC* pDC)
{
	...
	{
		size.cx = (sizeImage.cx + 2 * nImageMargin_rpbtn) * m_nPanelColumns;
		size.cy = sizeImage.cy + 3 * nImageMargin_rpbtn + 2 * nBorderMarginY;
	}

	//---------------------------------------
	// Add space for menu and scroll buttons:
	//---------------------------------------
	size.cx += GetDropDownImageWidth () + 3 * nImageMargin_rpbtn;
...
void CBCGPRibbonPaletteButton::RebuildIconLocations ()
{
	...
	for (int i = 0; i < m_arIcons.GetSize (); i++)
	{
		...
		else
		{
			CRect rectIcon (
				CPoint (
					rectImages.left + sizeIcon.cx * nColumn + nImageMargin_rpbtn / 2,
