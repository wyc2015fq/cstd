...
static const UINT IdAutoCommand_btn = 1;
...
void CBCGPButton::DoDrawItem (CDC* pDCPaint, CRect rectClient, UINT itemState)
{
	...
	if (((m_bVisualManagerStyle && !m_bDontSkin) || bDefaultCheckRadio) && !m_bTransparent)
	{
		if (bDefaultCheckRadio)
		{
			...
			if (m_bIsLeftText)
			{
				rectCheck.left = rectCheck.right - nCheckHeight;
			}
			else
			{
				rectCheck.right = rectCheck.left + nCheckHeight;
			}

			rectCheck.top = rectCheck.CenterPoint ().y - nCheckHeight / 2;
			rectCheck.bottom = rectCheck.top + nCheckHeight;
...
void CBCGPButton::OnDraw (CDC* pDC, const CRect& rect, UINT uiState)
{
	...
	{
#ifndef _BCGSUITE_
		if (!m_bOnGlass && CBCGPVisualManager::GetInstance ()->IsEmbossDisabledBtnText ())
#else
		if (!m_bOnGlass)
#endif
		{
...
void CBCGPButton::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	else
	{
		...
		if (m_nAutoRepeatTimeDelay > 0)
		{
			SetTimer (IdAutoCommand_btn, m_nAutoRepeatTimeDelay, NULL);
		}
	}
