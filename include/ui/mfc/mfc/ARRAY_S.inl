...
static void ConstructStrings(CString* pNewData, int nCount)
{
  ASSERT(nCount >= 0);
  while (nCount--) {
...
static void DestructStrings(CString* pOldData, int nCount)
{
  ASSERT(nCount >= 0);
  while (nCount--) {
...
{
  ASSERT_VALID(this);
  DestructStrings(m_pData, m_nSize);
  delete[](BYTE*)m_pData;
}
void CStringArray::SetSize(int nNewSize, int nGrowBy)
{
  ...
  if (nNewSize == 0) {
    ...
    ConstructStrings(m_pData, nNewSize);
    ...
    if (nNewSize > m_nSize) {
      ...
      ConstructStrings(&m_pData[m_nSize], nNewSize - m_nSize);
    }
    else if (m_nSize > nNewSize) { // destroy the old elements
      DestructStrings(&m_pData[nNewSize], m_nSize - nNewSize);
    }
    m_nSize = nNewSize;
  }
  else {
    ...
    ConstructStrings(&pNewData[m_nSize], nNewSize - m_nSize);
...
void CStringArray::InsertEmpty(int nIndex, int nCount)
{
  ...
  if (nIndex >= m_nSize) {
    ...
    ConstructStrings(&m_pData[nIndex], nCount);
  }
...
void CStringArray::RemoveAt(int nIndex, int nCount)
{
  ...
  DestructStrings(&m_pData[nIndex], nCount);
  if (nMoveCount)
