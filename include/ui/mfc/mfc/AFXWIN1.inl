...
{
  ASSERT(pDC != NULL);
  return Attach(::PathToRegion(pDC->m_hDC));
}
...
{
  ASSERT(m_hObject != NULL);
  ::SetRectRgn((HRGN)m_hObject, lpRect->left, lpRect->top,
...
{
  ASSERT(m_hDC != NULL);
  return SelectGdiObject(m_hDC, pObject->GetSafeHandle());
}
...
{
  ASSERT(m_hDC != NULL);  // call virtual
...
{
  ASSERT(m_hDC != NULL);
  return ::DrawState(m_hDC, (HBRUSH)pBrush->GetSafeHandle(),
...
{
  ASSERT(m_hDC != NULL);
  return ::TabbedTextOut(m_hDC, x, y, str, str.GetLength(),
      nTabPositions, lpnTabStopPositions, nTabOrigin);
}
...
{
  ASSERT(m_hAttribDC != NULL);
  return ::GetTabbedTextExtent(m_hAttribDC,
...
{
  ASSERT(m_hDC != NULL);
  return ::GetTabbedTextExtent(m_hDC,
      str, str.GetLength(), nTabPositions, lpnTabStopPositions);
}
...
{
  ASSERT(m_hAttribDC != NULL);
  int nResult = ::GetTextFace(m_hAttribDC,
      256, rString.GetBuffer(256));
...
{
  ASSERT(m_hDC != NULL);
  return ::MaskBlt(m_hDC, x, y, nWidth, nHeight, pSrcDC->GetSafeHdc(),
...
{
  ASSERT(m_hDC != NULL);
  return ::PlgBlt(m_hDC, lpPoint, pSrcDC->GetSafeHdc(), xSrc, ySrc, nWidth,
...
{
  ASSERT(m_hAttribDC != NULL);
  return CPalette::FromHandle((HPALETTE)::GetCurrentObject(m_hAttribDC, OBJ_PAL));
}
...
{
  ASSERT(m_hDC != NULL);
  return ::ExtEscape(m_hDC, nEscape, nInputSize, lpszInputData,
