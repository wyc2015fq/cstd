...
#endif
static const int DateTimeCtrl_iSpinWidth = 15;
static const int iDropButtonWidth = 17;
static const int DateTimeCtrl_iSpinID = 1;
static const COLORREF clrTransparent = RGB(255, 0, 255);
static const int DateTimeCtrl_iFirstAllowedYear = 100;  // COleDateTime limitation
static const int DateTimeCtrl_iLastAllowedYear = 9998;  // COleDateTime limitation
...
CBCGPDateTimeCtrl::CBCGPDateTimeCtrl()
{
  ...
  m_iYearPos = 0;
  m_monthFormat = 0;
  m_MinDate = COleDateTime(DateTimeCtrl_iFirstAllowedYear, 1, 1, 0, 0, 0);
  m_MaxDate = COleDateTime(DateTimeCtrl_iLastAllowedYear, 12, 31, 23, 59, 59);
  m_weekStart = 1;
...
int CBCGPDateTimeCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
  ...
  if (!m_wndSpin.Create(WS_CHILD | WS_VISIBLE | UDS_ALIGNRIGHT | UDS_AUTOBUDDY,
      rectSpin, this, DateTimeCtrl_iSpinID)) {
...
void CBCGPDateTimeCtrl::AdjustControl(CRect rectClient)
{
  ...
  int nSpinWidthCurr = DateTimeCtrl_iSpinWidth;
  ...
  if (m_spinButton) {
    if (m_wndSpin.GetSafeHwnd() == NULL) {
      m_wndSpin.Create(WS_CHILD | WS_VISIBLE | UDS_ALIGNRIGHT | UDS_AUTOBUDDY,
          CRect(0, 0, 0, 0), this, DateTimeCtrl_iSpinID);
      m_wndSpin.m_bIsDateTimeControl = TRUE;
    }
  ...
  //-----------------------------
  COleDateTime minAllowedDate(DateTimeCtrl_iFirstAllowedYear, 1, 1, 0, 0, 0);
  COleDateTime maxAllowedDate(DateTimeCtrl_iLastAllowedYear, 12, 31, 23, 59, 59);
...
void CBCGPDateTimeCtrl::ScrollCurrPart(int iDir)
{
  ...
  case YEAR:
    iYear += iDir;
    iYear = min(DateTimeCtrl_iLastAllowedYear, max(DateTimeCtrl_iFirstAllowedYear, iYear));
    if (iDay > GetDaysInMonth(iMonth, iYear)) {
      iDay = GetDaysInMonth(iMonth, iYear);
    }
    break;
...
{
  if (wParam == DateTimeCtrl_iSpinID) {
