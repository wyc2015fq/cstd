...
public:
  ...
  virtual int  ResizeBySlider(int /*nOffset*/) {
    return 0; // can'te be resized by slider
  }
  ...
  int   GetBarCount() const {
    return (int) m_lstControlBars.GetCount();
  }
  ...
  bool  IsHorizontal() const {
    return (m_dwRowAlignment & CBRS_ALIGN_TOP || m_dwRowAlignment & CBRS_ALIGN_BOTTOM);
  }

  DWORD GetRowAlignment() const {
    return m_dwRowAlignment;
  }
  ...
  CBCGPDockBar* GetDockBar() const {
    return m_pParentDockBar;
  }

  const CObList& GetControlBarList() const {
    return m_lstControlBars;
  }
