...
//#define new DEBUG_NEW
#endif
static const int DurationCtrl_iSpinWidth = 15;
static const int DurationCtrl_iSpinID = 1;
...
void CBCGPDurationCtrl::AdjustControl(CRect rectClient, BOOL bRedraw/* = TRUE)*/)
{
  ...
  int nSpinWidthCurr = DurationCtrl_iSpinWidth;
  ...
  if (m_spinButton) {
    if (m_wndSpin.GetSafeHwnd() == NULL) {
      ...
      m_wndSpin.Create(WS_CHILD | WS_VISIBLE | UDS_ALIGNRIGHT | UDS_AUTOBUDDY,
          rectSpin, this, DurationCtrl_iSpinID);
...
{
  if (wParam == DurationCtrl_iSpinID) {
