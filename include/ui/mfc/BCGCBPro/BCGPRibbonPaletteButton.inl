...
void CBCGPRibbonPaletteIcon::SetCheckRect()
{
  ...
  if (2 * sizeCheckBox.cx + nImageMargin1 > m_rect.Width() || 2 * sizeCheckBox.cy + nImageMargin1 > m_rect.Height()) {
    return;
  }
  m_rectCheckBox = m_rect;
  m_rectCheckBox.DeflateRect(nImageMargin1 + 1, nImageMargin1 + 1);
...
CSize CBCGPRibbonPaletteIcon::GetRegularSize(CDC* pDC)
{
  ...
  if (!m_pOwner->m_bSmallIcons) {
    sizeItem.cx += 2 * nImageMargin1;
    sizeItem.cy += 2 * nImageMargin1;
  }
...
CSize CBCGPRibbonPaletteButton::GetRegularSize(CDC* pDC)
{
  ...
  if (m_bSmallIcons) {
    ...
    size.cx = (sizeImage.cx + 2 * nImageMargin1) * m_nPanelColumns;
    size.cy = sizeImage.cy + 3 * nImageMargin1 + 2 * nBorderMarginY;
  }
  //---------------------------------------
  // Add space for menu and scroll buttons:
  //---------------------------------------
  size.cx += GetDropDownImageWidth() + 3 * nImageMargin1;
  return size;
}
//***************************************************************************
...
void CBCGPRibbonPaletteButton::OnAfterChangeRect(CDC* pDC)
{
  ...
  CRect rectImages = m_rect;
  int nMargin = m_bSmallIcons ? 0 : nImageMargin1;
    ...
    COLORREF clrText)
{
  ...
  if (!m_bSmallIcons) {
    rectIcon.DeflateRect(nImageMargin1, nImageMargin1);
  }
...
void CBCGPRibbonPaletteButton::RebuildIconLocations()
{
  ...
  for (int i = 0; i < m_arIcons.GetSize(); i++) {
    ...
    if (nRow - m_nScrollOffset >= m_nImagesInColumn ||
      ...
      CRect rectIcon(
          CPoint(
              rectImages.left + sizeIcon.cx * nColumn + nImageMargin1 / 2,
...
CBCGPPopupMenu* CBCGPRibbonPaletteMenuButton::CreatePopupMenu()
{
  ...
  pMenu->SetMenuMode();
  int nMargin = m_paletteButton.m_bSmallIcons ? 0 : nImageMargin1;
...
void CBCGPRibbonComboGalleryCtrl::OnDraw(CDC* pDC, const CRect& rect, UINT uiState)
{
  ...
  CRect rectItem = rect;
  rectItem.right -= sizeArrow.cx + 2 * nImageMargin1;
