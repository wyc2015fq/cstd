...
public:
  ...
  const CString& GetToolTipText() const {
  ...
  UINT GetViewResID() const {
    return m_nViewResID;
  }
  const CString& GetViewTitle() const {
    return m_strViewTitle.IsEmpty() ? m_strName : m_strViewTitle;
  }
...
public:
  ...
  virtual LPCTSTR GetAccValue() const {
    return NULL;
  }
  virtual LPCTSTR GetAccDescription() const {
    return NULL;
  }
  virtual long GetAccRole() const {
    return ROLE_SYSTEM_PUSHBUTTON;
  }
  virtual LPCTSTR GetAccDefaultAction() const {
    return _T("Open");
  }
...
public:
  ...
  virtual BOOL IsWide() const {
    return m_Type == BCGP_TILE_WIDE || m_Type == BCGP_TILE_DOUBLE_SIZE;
  }
  virtual BOOL IsHigh() const {
    return m_Type == BCGP_TILE_DOUBLE_SIZE;
  }

  BCGP_WINUI_TILE_TYPE GetType() const {
    return m_Type;
  }

  int GetBadgeNumber() const {
    return m_nBadgeNumber;
  }
  void SetBadgeNumber(int nBadgeNumber);

  BCGP_WINUI_BADGE_GLYPH GetBadgeGlyph() const {
    return m_BadgeGlyph;
  }
  void SetBadgeGlyph(BCGP_WINUI_BADGE_GLYPH glyph);

  int GetCustomBadgeIndex() const {
    return m_nCustomBadgeIndex;
  }
  ...
  const CBCGPImage& GetImage() const {
    return m_Image;
  }
  BOOL IsStretchImage() const {
    return m_bStretchImage;
  }
  ...
  const CBCGPBrush& GetBackgroundBrush() const {
    return m_brushBackground;
  }
  void SetBackgroundBrush(const CBCGPBrush& brush) {
    m_brushBackground = brush;
    m_brushBackgroundDark.Empty();
  }

  const CBCGPColor& GetBorderColor() const {
  ...
  void SetBorderWidth(double dblWidth) {
...
public:
  ...
  // Attributes
public:
  const CBCGPRect& GetTilesArea() const {
    return m_rectTiles;
  }

  BOOL IsRoundedShapes() const {
    return m_bRoundedShapes;
  }
  void SetRoundedShapes(BOOL bSet = TRUE);

  BOOL IsHorizontalLayout() const {
    return m_bIsHorizontalLayout;
  }
  ...
  double GetHorzMargin() const {
    return m_nHorzMargin;
  }
  double GetVertMargin() const {
    return m_nVertMargin;
  }

  const CBCGPSize& GetSquareSize() const {
    return m_sizeSquare;
  }
  ...
  const CBCGPColor& GetCaptionForegroundColor() const {
    return m_brCaptionForeground.GetColor();
  }
  void SetCaptionForegroundColor(const CBCGPColor& colorCaptionForeground, BOOL bRedraw = TRUE);

  const CBCGPColor& GetTextColor() const {
    return m_colorTileText;
  }

  const CBCGPBrush& GetTileBrush() const {
    return m_brTileFill;
  }
  const CBCGPBrush& GetTileBrushDark() const {
    return m_brTileFillDark;
  }
  ...
  const CBCGPTextFormat& GetCaptionTextFormat() const {
    return m_textFormatCaption;
  }
  const CBCGPTextFormat& GetGroupCaptionTextFormat() const {
    return m_textFormatGroupCaption;
  }
  const CBCGPTextFormat& GetCaptionButtonTextFormat() const {
    return m_textFormatCaptionButton;
  }
  const CBCGPTextFormat& GetHeaderTextFormat() const {
    return m_textFormatHeader;
  }
  const CBCGPTextFormat& GetNameTextFormat() const {
    return m_textFormatName;
  }
  ...
  const CBCGPImage& GetCustomBadgeGlyphs() const {
    return m_CustomBadgeGlyphs;
  }

  void SetScrollBarColorTheme(CBCGPVisualScrollBarColorTheme& theme);
  const CBCGPVisualScrollBarColorTheme& GetScrollBarColorTheme() const {
    return m_ScrollBar.GetColorTheme();
  }

  CWnd* GetCurrentView() const {
    return m_pWndCurrView;
  }
  CBCGPWinUIBaseObject* GetViewParent() const {
    return m_pLastClicked;
  }

  // Tiles customization
  void EnableTilesDragAndDrop(BOOL bEnable = TRUE) {
    m_bTilesDragAndDrop = bEnable;
  }
  BOOL IsTilesDragAndDropEnabled() const {
    return m_bTilesDragAndDrop;
  }

  BOOL IsDraggingTile() const {
    return m_ptDragTile != CBCGPPoint(-1., -1.) && m_pPressed != NULL;
  }
...
public:
  ...
  BOOL IsNavigationBackButtonEnabled() const {
    return m_bHasNavigationBackButton;
  }

  const CList<CBCGPWinUITile*, CBCGPWinUITile*>& GetTilesList() const {
    return m_lstTiles;
  }
  ...
  CBCGPWinUITile* GetCurSel() const {
    return DYNAMIC_DOWNCAST(CBCGPWinUITile, m_pSelected);
  }
...
protected:
  ...
  BOOL IsObjectSelected(CBCGPWinUIBaseObject* pObject) const {
    return pObject == m_pSelected;
  }
...
public:
  ...
  const CBCGPBrush& GetFillBrush() const {
    return m_brFillGroup;
  }
  const CBCGPBrush& GetOutlineBrush() const {
    return m_brOutlineGroup;
  }
...
protected:
  ...
  virtual BOOL IsRightAligned() const {
    return TRUE;
  }
