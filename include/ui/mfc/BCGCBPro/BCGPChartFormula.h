...
public:
  ...
  virtual BOOL IsNonDiscreteCurve() const {
    return FALSE;
  }

  void SetParentSeries(CBCGPChartSeries* pSeries);
  CBCGPChartSeries* GetParentSeries() const {
  ...
  void SetLParam(LPARAM lp = NULL) {
    m_lParam = lp;
  }

  virtual int GetInputSeriesCount() const {
    return (int)m_arInputSeries.GetSize();
  }
...
public:
  ...
  virtual BCGPChartCategory GetOutputSeriesCategory() const {
    return BCGPChartLine;
  }
  virtual BCGPChartType GetOutputSeriesType() const {
    return BCGP_CT_SIMPLE;
  }
...
public:
  ...
  MovingAverageType GetMAType() const {
    return m_maType;
  }
...
public:
  ...
  int GetKPeriod() const {
    return m_nKPeriod;
  }
  int GetSlowing() const {
    return m_nSlowing;
  }
...
public:
  ...
  virtual BCGPChartCategory GetOutputSeriesCategory() const {
    return BCGPChartArea;
  }
  virtual BCGPChartType GetOutputSeriesType() const {
    return BCGP_CT_RANGE;
  }
...
public:
  ...
  int GetFastEMAPeriod() const {
    return m_nFastEMAPeriod;
  }
  int GetSlowEMAPeriod() const {
    return m_nSlowEMAPeriod;
  }
...
public:
  ...
  TrendLineType GetTrendType() const {
    return m_type;
  }

  void SetCoefficients(const CArray<double, double>& arCoef);
  void AddCoefficient(double dblCoef);
  void RemoveAllCoefficients() {
    m_arCoef.RemoveAll();
  }
  const CArray<double, double>& GetCoefficients() const {
    return m_arCoef;
  }
  ...
  int GetPolynomialTrendOrder() const {
    return m_nPolynomialTrendOrder;
  }

  const CBCGPPointsArray& GetScreenPoints() const {
