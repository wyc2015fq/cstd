...
static const int iDefaultComboHeight1 = 150;
static const int iDefaultSize = 150;
static const int iHorzMargin1 = 1;
...
void CBCGPToolbarComboBoxButton::Initialize()
{
  ...
  m_nDropDownHeight = iDefaultComboHeight1;
  m_bIsHotEdit = FALSE;
  m_uiMenuResID = 0;
  m_bIsRibbon = FALSE;
...
void CBCGPToolbarComboBoxButton::AdjustRect()
{
  ...
  m_pWndCombo->SetWindowPos(NULL,
      m_rect.left + iHorzMargin1, m_rect.top,
      m_rect.Width() - 2 * iHorzMargin1, m_nDropDownHeight,
  ...
  if (m_bFlat) {
    ...
    m_rect.left = m_rectCombo.left - iHorzMargin1;
    m_rect.right = m_rectCombo.right + iHorzMargin1;
    if (!m_bTextBelow || m_strText.IsEmpty()) {
    ...
    if (m_pWndEdit != NULL) {
      ...
      m_pWndEdit->SetWindowPos(NULL,
          m_rect.left + iHorzMargin1 + iBorderOffset, m_rect.top + iBorderOffset,
          m_rect.Width() - 2 * iHorzMargin1 - m_rectButton.Width() - iBorderOffset - 3,
