...
public:
  ...
  virtual BOOL OnMouseDblClick(int /*nButton*/, const CBCGPPoint&) {
    return FALSE;
  }
...
protected:
  ...
  virtual void OnSetIAccessible(IAccessible* pIAccessible) {
    m_pStdObject = pIAccessible;
  }
#endif
};
...
public:
  ...
  void SetOffset(double dblOffset) {
    m_dblOffset = dblOffset;
  }

  const CBCGPRect& GetPrevButton() const {
    return m_rectPrev;
  }
  const CBCGPRect& GetNextButton() const {
    return m_rectNext;
  }
  const CBCGPRect& GetThumb() const {
    return m_rectThumb;
  }

  double GetScrollSize() const {
    return m_bIsHorizontal ? m_rectScroll.Width() : m_rectScroll.Height();
  }

  void SetColorTheme(CBCGPVisualScrollBarColorTheme& theme) {
    m_ColorTheme = theme;
  }
  const CBCGPVisualScrollBarColorTheme& GetColorTheme() const {
    return m_ColorTheme;
  }
...
public:
  ...
  BOOL IsHitTestSelected() const {
    return m_bHitTestSelected;
  }
