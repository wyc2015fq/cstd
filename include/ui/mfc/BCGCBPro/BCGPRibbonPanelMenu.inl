...
#endif
static const int uiPopupTimerEvent1 = 1;
static const int uiRemovePopupTimerEvent1 = 2;
static const UINT IdAutoCommand2 = 3;
static const int nScrollBarID2 = 1;
...
void CBCGPRibbonPanelMenuBar::OnLButtonDown(UINT nFlags, CPoint point)
{
  ...
  if (m_pPressed != NULL) {
    ...
    if (m_pPressed->IsAutoRepeatMode(nDelay)) {
      SetTimer(IdAutoCommand2, nDelay, NULL);
  ...
  if (bHotWasChanged) {
    ...
    if (pDroppedDown != NULL) {
      ...
      if (CBCGPToolBar::IsCustomizeMode() ||
        ...
        KillTimer(uiRemovePopupTimerEvent1);
        m_pDelayedCloseButton = NULL;
        pDroppedDown->ClosePopupMenu();
        if (pParentMenu != NULL) {
        ...
        SetTimer(uiRemovePopupTimerEvent1,
    ...
    if (pHotButton != NULL && pHotButton->HasMenu()) {
      ...
      if ((m_pDelayedButton = pHotButton) != NULL) {
        if (m_pDelayedButton == m_pDelayedCloseButton) {
          ...
          SetTimer(uiPopupTimerEvent1, m_uiPopupTimerDelay, NULL);
        }
      }
    }
  ...
  if (m_pPanel != NULL) {
    ...
    if (m_pPanel->m_pPaletteButton != NULL || IsBackStageView()) {
      m_wndScrollBarVert.Create(WS_CHILD | WS_VISIBLE | SBS_VERT,
          CRect(0, 0, 0, 0), this, nScrollBarID2);
...
void CBCGPRibbonPanelMenuBar::OnTimer(UINT_PTR nIDEvent)
{
  ...
  if (nIDEvent == uiPopupTimerEvent1) {
    KillTimer(uiPopupTimerEvent1);
    //---------------------------------
  ...
  else if (nIDEvent == uiRemovePopupTimerEvent1) {
    KillTimer(uiRemovePopupTimerEvent1);
    if (m_pDelayedCloseButton != NULL) {
  ...
  else if (nIDEvent == IdAutoCommand2) {
