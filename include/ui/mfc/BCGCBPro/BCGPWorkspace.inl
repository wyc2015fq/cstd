...
static const CString strWindowPlacementRegSection1      = _T("WindowPlacement");
    ...
    CRect& rectNormalPosition, int& nFlags, int& nShowCmd)
{
  CBCGPRegistrySP regSP;
  CBCGPRegistry& reg = regSP.Create(FALSE, TRUE);
  if (!reg.Open(GetRegSectionPath(strWindowPlacementRegSection1))) {
    return FALSE;
  }
    ...
    const CRect& rectNormalPosition, int nFlags, int nShowCmd)
{
  CBCGPRegistrySP regSP;
  CBCGPRegistry& reg = regSP.Create(FALSE, FALSE);
  if (!reg.CreateKey(GetRegSectionPath(strWindowPlacementRegSection1))) {
    return FALSE;
  }
