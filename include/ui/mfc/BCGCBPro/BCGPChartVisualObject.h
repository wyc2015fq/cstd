...
struct BCGCBPRODLLEXPORT BCGPChartMouseConfig {
  ...
  BCGPChartMouseConfig::ZoomScrollOptions GetZoomScrollOptions() const {
    return m_zoomScrollOption;
  }
...
public:
  ...
  virtual CBCGPEngine3D* GetEngine3D() const {
    return m_pEngine3D;
  }

  void SetSortNeeded(BOOL bSet) {
    m_bSortNeeded = bSet;
  }
  BOOL IsSortNeeded() const {
    return m_bSortNeeded;
  }

  void SetCalculateNormals(BOOL bSet);
  BOOL IsCalculateNormals() const {
    return m_bCalculateNormals;
  }

  void SetWallsIgnoreRotation(BOOL bSet);
  BOOL IsWallIgnoreRotation() const {
    return m_bWallsIgnoreRotation;
  }
  ...
  double GetXRotation() const {
    return m_dblXRotation - 90.;
  }
  double GetYRotation() const {
    return m_dblYRotation;
  }
  double GetZRotation() const {
    return m_dblZRotation;
  }
  ...
  void SetScrollOffset(const CBCGPPoint& ptOffset) {
    m_ptScrollOffset = ptOffset;
  }
  CBCGPPoint GetScrollOffset() const {
    return m_ptScrollOffset;
  }

  double GetBaseDepthPercent() const {
    return m_dblBaseDepthPercent;
  }
  double GetHeightScalePercent() const {
    return m_dblHeightScalePercent;
  }
  double GetDepthScalePercent() const;
  double GetFrontDistancePercent() const {
    return m_dblFrontDistancePercent;
  }
  double GetPerspectivePercent() const {
    return m_dblPerspective;
  }
  double GetZoomFactor()  const {
    return m_dblZoomFactor;
  }

  void SetProportionalScale(BOOL bSet = TRUE) {
    m_bProportionalScale = bSet;
  }
  ...
  ExplicitGrouping GetExplicitGrouping() const {
    return m_explicitGrouping;
  }
  void SetExplicitGrouping(ExplicitGrouping grouping, BOOL bRedraw = TRUE);

  const CBCGPRect& GetDiagramRect() const {
    return m_rectDiagram;
  }

  BOOL IsXHorizontal() const {
    return m_bIsXHorizontal;
  }
  void SetXHorizontal(BOOL bSet);

  void SetDrawWallOptions(CBCGPChartDiagram3D::DrawWallOptions options, BOOL bRedraw = FALSE);
  DrawWallOptions GetDrawWallOptions() const {
    return (DrawWallOptions) m_drawWallOptions;
  }
  void StripWallsAndAxes(BOOL bStrip = TRUE, BOOL bRedraw = FALSE);
  void SetThickWallsAndFloor(BOOL bSet = TRUE, BOOL bRedraw = FALSE);
  BOOL IsThickWallsAndFloor() const {
    return m_bThickWallsAndFloor;
  }
  ...
  int  GetLeftVisibleWallPos() const {
    return m_nLeftVisibleWallPos;
  }
  int  GetRightVisibleWallPos() const {
    return m_nRightVisibleWallPos;
  }
  double GetWallThicknessNormal() const {
    return m_dblWallThicknessNormal;
  }

  void SetShowBackfaces(BOOL bSet)  {
    m_bShowBackfaces = bSet;
  }
  BOOL IsShowBackfaces() const {
...
public:
  ...
  virtual CBCGPChartDiagram3D* GetDiagram3D() const {
    return m_pDiagram3D;
  }
  ...
  void SetPlotAreaPadding(const CBCGPRect& rectPadding) {
    m_rectPlotAreaPadding = rectPadding;
  }
  CBCGPRect GetPlotAreaPadding(BOOL bScaled = FALSE) const;

  void SetTitleAreaPadding(const CBCGPSize& szPadding) {
    m_szTitleAreaPadding = szPadding;
  }
  CBCGPSize GetTitleAreaPadding(BOOL bScaled = FALSE) const;

  void SetLegendAreaPadding(const CBCGPSize& szPadding) {
    m_szLegendAreaPadding = szPadding;
  }
  CBCGPSize GetLegendAreaPadding(BOOL bScaled = FALSE) const;

  void SetLegendElementSpacing(const CBCGPSize& szSpacing) {
    m_szLegendElementSpacing = szSpacing;
  }
  CBCGPSize GetLegendElementSpacing(BOOL bScaled = FALSE) const;

  void SetHitTestDataPointPrecision(const CBCGPSize& szPrecision) {
    m_szHitTestDataPointPrecision = szPrecision;
  }
  CBCGPSize GetHitTestDataPointPrecision(BOOL bScaled = FALSE) const;

  const CBCGPRect& GetDiagramArea() const {
    return m_rectDiagramArea;
  }
  const CBCGPRect& GetLegendArea() const {
    return m_rectLegendArea;
  }
  const CBCGPRect& GetDataTableArea() const {
    return m_rectDataTableArea;
  }
  double GetDataTableHeaderColumnWidth() const {
    return m_dblDataTableHeaderColumnWidth;
  }
  double GetDataTableRowHeight() const {
    return m_dblDataTableRowHeight;
  }

  void SetSmartLabelsParams(const BCGPChartSmartLabelParams& params, BOOL bRedraw = TRUE);
  const BCGPChartSmartLabelParams& GetSmartLabelsParams() const {
    return m_smartLabelsParams;
  }

public:
  BCGPChartFormatArea& GetChartAreaFormat() {
    return m_chartAreaFormat;
  }
  BCGPChartFormatArea& GetPlotAreaFormat() {
    return m_plotAreaFormat;
  }
  BCGPChartFormatLabel& GetTitleLabelFormat() {
    return m_titleAreaFormat;
  }
  BCGPChartFormatArea& GetLegendFormat() {
    return m_legendAreaFormat;
  }
  BCGPChartFormatDataTable& GetDataTableFormat() {
    return m_dataTableAreaFormat;
  }
  BCGPChartMouseConfig& GetMouseConfig() {
    return m_mouseConfig;
  };

  const CString& GetChartTitle() const {
  ...
  // Smart labels
  BOOL IsSmartLabelsEnabled() const {
    return m_bSmartLabelsEnabled;
  }
...
public:
  ...
  void EnableResizeAxes(BOOL bEnable = TRUE) {
    m_bEnableResizeAxes = bEnable;
  }
  BOOL IsResizeAxesEnabled() const {
    return m_bEnableResizeAxes;
  }
  ...
  void SetShowSurfaceMapInLegend(BOOL bSet = TRUE) {
    m_bShowSurfaceMapInLegend = bSet;
  }
  BOOL IsShowSurfaceMapInLegend() const {
    return m_bShowSurfaceMapInLegend && (GetChartCategory() == BCGPChartSurface3D);
  }
  ...
  const BCGPChartDataLabelOptions& GetDataLabelOptions() const {
    return m_dataLabelOptions;
  }

  // Selection and Zoom
  BCGPChartFormatSelection& GetSelectionFormat() {
  ...
  CBCGPChartAxis* GetThumbSizeAxis() const {
    return m_pThumbSizeAxis;
  }

  BOOL IsPanMode() const {
    return m_bPanMode;
  }
  BOOL IsThumbTrackMode() const {
    return m_bThumbTrackMode;
  }
  BOOL IsThumbSizeMode() const {
    return m_bThumbSizeMode;
  }
  ...
  void EnableMagnifier(BOOL bEnable = TRUE) {
    m_bEnableMagnifier = bEnable;
  }
  void EnableScroll(BOOL bEnable = TRUE) {
    m_bEnableScroll = bEnable;
    ResetGestureOptions();
  }
  void EnablePan(BOOL bEnable = TRUE) {
    m_bEnablePan = bEnable;
    ResetGestureOptions();
  }
  BOOL IsMagnifierEnabled() const {
    return m_bEnableMagnifier;
  }
  ...
  BOOL IsClipDiagramToAxes() const {
    return m_bClipDiagramToAxes;
  }
  void EnableClipDiagramToAxes(BOOL bEnable = TRUE) {
    m_bClipDiagramToAxes = bEnable;
  }
  ...
  BCGPChartMouseConfig::ZoomScrollOptions GetZoomScrollConfig() const {
    return m_mouseConfig.GetZoomScrollOptions();
  }
  ...
  const CObList& GetRelatedLegends() const {
    return m_lstRelatedLegends;
  }
