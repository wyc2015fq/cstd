...
public:
  ...
  BOOL IsZoomEnabled() const {
    return (Get(BCGP_GID_ZOOM) & BCGP_GC_ZOOM) == BCGP_GC_ZOOM;
  }
  BOOL IsRotateEnabled() const {
    return (Get(BCGP_GID_ROTATE) & BCGP_GC_ROTATE) == BCGP_GC_ROTATE;
  }
  BOOL IsTwoFingerTapEnabled() const {
    return (Get(BCGP_GID_TWOFINGERTAP) & BCGP_GC_TWOFINGERTAP) == BCGP_GC_TWOFINGERTAP;
  }
  BOOL IsPressAndTapEnabled() const {
    return (Get(BCGP_GID_PRESSANDTAP) & BCGP_GC_PRESSANDTAP) == BCGP_GC_PRESSANDTAP;
  }
  BOOL IsPanAllEnabled() const {
    return (Get(BCGP_GID_PAN) & BCGP_GC_PAN) == BCGP_GC_PAN;
  }
  BOOL IsPanVerticalEnabled() const {
    return (Get(BCGP_GID_PAN) & BCGP_GC_PAN_WITH_SINGLE_FINGER_VERTICALLY) == BCGP_GC_PAN_WITH_SINGLE_FINGER_VERTICALLY;
  }
  BOOL IsPanHorizontalEnabled() const {
    return (Get(BCGP_GID_PAN) & BCGP_GC_PAN_WITH_SINGLE_FINGER_HORIZONTALLY) == BCGP_GC_PAN_WITH_SINGLE_FINGER_HORIZONTALLY;
  }
  BOOL IsPanWithGutterEnabled() const {
    return (Get(BCGP_GID_PAN) & BCGP_GC_PAN_WITH_GUTTER) == BCGP_GC_PAN_WITH_GUTTER;
  }
  BOOL IsPanWithInertiaEnabled() const {
    return (Get(BCGP_GID_PAN) & BCGP_GC_PAN_WITH_INERTIA) == BCGP_GC_PAN_WITH_INERTIA;
  }
