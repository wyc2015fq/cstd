...
struct BCGCBPRODLLEXPORT AFX_NOVTABLE CBCGPChartSurfaceTriangle {
  ...
  _inline BOOL IsLevelEmpty() const {
    return m_arLevels.GetSize() == 0;
  }
...
public:
  ...
  const CList<CBCGPChartSide3D*, CBCGPChartSide3D*>& GetVisibleSides() const {
    return m_lstVisibleSides;
  }
...
protected:
  ...
  virtual BOOL ForceAddBackfaces() const {
    return FALSE;
  }
