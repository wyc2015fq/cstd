...
static const CString strDummyAmpSeq2 = _T("\001\001");
    ...
    BOOL bDrawBorder, BOOL bGrayDisabledButtons)
{
  ...
  strWithoutAmp.Replace(_T("&&"), strDummyAmpSeq2);
  strWithoutAmp.Remove(_T('&'));
  strWithoutAmp.Replace(strDummyAmpSeq2, _T("&"));
    ...
    if (bHorz) {
      ...
      if (!globalData.m_bUnderlineKeyboardShortcuts && !CBCGPToolBar::IsCustomizeMode()) {
        if (strText.Find(_T("&&")) >= 0) {
          strText.Replace(_T("&&"), strDummyAmpSeq2);
          strText.Remove(_T('&'));
          strText.Replace(strDummyAmpSeq2, _T("&&"));
        }
        else {
