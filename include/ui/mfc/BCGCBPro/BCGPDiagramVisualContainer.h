...
public:
  ...
  virtual CBCGPDiagramVisualObject* CreateCustomObject(const CString& /*strName*/) const {
    return NULL;
  }

  struct HitTestAnchorOptions {
    HitTestAnchorOptions() {
  ...
  const CBCGPBrush& GetAnchorFillBrush() const {
    return m_brAnchorFill;
  }
  const CBCGPBrush& GetAnchorOutlineBrush() const {
    return m_brAnchorOutline;
  }
