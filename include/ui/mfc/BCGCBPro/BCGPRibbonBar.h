...
public:
  ...
  BOOL AreAllTabsAreInvisible() const {
    return m_bAllTabsAreInvisible;
  }
  ...
  const CSize GetControlsSpacing() const {
    return m_sizePadding;
  }

  virtual BOOL GetSeparatorCustomLabel(CString& /*strSeparatorLabel*/) const {
    return FALSE;
  }
...
public:
  ...
  virtual BOOL IsShowGroupBorder(CBCGPRibbonButtonsGroup* /*pGroup*/) const  {
    return FALSE;
  }
  ...
  virtual BOOL OnFilterSearchResult(const CBCGPBaseRibbonElement* /*pHit*/) {
    return TRUE;
  }
  ...
  virtual CWnd* GetTopLevelOwner() const {
    return CWnd::GetSafeOwner();
  }
...
public:
  ...
  CBCGPToolBarImages& GetScenicImage() {
    return m_ImageScenic;
  }
  CString GetScenicText()        {
    return m_strTextScenic;
  }
