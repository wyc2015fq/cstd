...
struct BCGCBPRODLLEXPORT CBCGPChartAxisScalePart {
  ...
  double GetSize() const {
    return fabs(m_dblScreenEnd - m_dblScreenStart);
  }
  double GetRange() const {
    return GetMaximumDisplayed() - GetMinimumDisplayed();
  }
...
public:
  ...
  int GetDisplayedLinesCount() const {
    return m_nDisplayedLines;
  }

  void SetMajorTickMarkLen(int nLen);
  void SetMinorTickMarkLen(int nLen) {
    m_nMinorTickMarkLen = nLen;
  }
  ...
  double GetLogScaleBase() const {
    return m_dblLogScaleBase;
  }

  double GetDisplayUnits() const {
    return m_dblDisplayUnits;
  }
  void SetDisplayUnits(double dblUnits = 0.) {
    m_dblDisplayUnits = dblUnits;
  }
  ...
  virtual CBCGPChartData::ComponentIndex GetComponentIndex() const {
  ...
  BOOL IsMajorGridLinesVisible() const {
    return m_bShowMajorGridLines;
  };
  BOOL IsMinorGridLinesVisible() const {
    return m_bShowMinorGridLines;
  }
  ...
  int GetMinorUnitCount() const {
    return m_nMinorUnitCount;
  }

  BOOL IsMajorUnitIntervalInterlacingEnabled() const {
    return m_bFillMajorUnitInterval;
  }
  void SetFirstInterlacedIntervalIndex(int nIndex = 1) {
    m_nFirstIntervalToFill = (nIndex < 0) ? 0 : nIndex;
  }
  int GetFirstInterlacedIntervalIndex() const {
    return m_nFirstIntervalToFill;
  }
  ...
  int GetInterlaceStep() const {
    return m_nUnitFillStep;
  }
  ...
  virtual BOOL IsZoomEnabled() const {
    return m_bEnableZoom && !m_bLogScale;
  }
  void EnableScroll(BOOL bEnable = TRUE) {
    m_bEnableScroll = bEnable;
  }
  virtual BOOL IsScrollEnabled() const {
    return m_bEnableScroll && !m_bLogScale;
  }
  virtual BOOL CanBeScrolled() const;
  void SetMaxZoomInFactor(double dblValue = 1000);
  double GetMaxZoomInFactor() const {
    return m_dblMaxZoomInFactor;
  }
  void EnableNegativeValuesForFixedInterval(BOOL bEnable = TRUE) {
    m_bEnableNegativeValuesForFixedInterval = bEnable;
  }
  ...
  void EnableIndependentZoom(BOOL bEnable = TRUE) {
    m_bIndependentZoom = bEnable;
  }
  virtual BOOL IsIndependentZoomEnabled() const {
    return m_bIndependentZoom;
  }

  void SetScrollRange(double dblMinValue = -DBL_MAX, double dblMaxValue = DBL_MAX, BOOL bForce = TRUE);
  double GetMinScrollValue() const {
    return m_dblMinScrollValue;
  }
  double GetMaxScrollValue() const {
    return m_dblMaxScrollValue;
  }

  BOOL SetThumbRect(const CBCGPPoint& ptHit, BOOL bLeftGrip, double dblOffset);
  void EnableThumbGrip(BOOL bEnable = TRUE);
  BOOL IsThumbGripEnabled() const {
    return m_bIsThumbGripEnabled;
  }

  void ShowScrollBar(BOOL bShow = TRUE, BOOL bRedraw = TRUE);
  virtual BOOL CanShowScrollBar() const;
  CBCGPRect GetScrollBarRect() const {
    return m_rectScrollBar;
  }

  void SetAlwaysShowScrollBar(BOOL bShow = TRUE, BOOL bRedraw = TRUE);
  virtual BOOL IsAlwaysShowScrollBar() const {
    return m_bAlwaysShowScrollBar;
  }
  ...
  virtual BOOL IsFixedDisplayRange() const {
    return m_bFixedRange;
  }
  virtual BOOL IsFixedMinimumDisplayValue() const {
    return m_bFixedMinimum;
  }
  virtual BOOL IsFixedMaximumDisplayValue() const {
    return m_bFixedMaximum;
  }

  virtual BOOL CanDisplayDataBetweenTickMarks() const {
    return TRUE;
  }
  ...
  BOOL IsFixedIntervalWidth() const {
    return m_bIsFixedIntervalWidth;
  }
  int  GetValuesPerInterval() const {
    return m_nValuesPerInterval;
  }
  int  GetFixedIntervalWidth() const {
    return m_nFixedIntervalWidth;
  }
  double GetFixedIntervalWidthScaled() const;
  void EnableStretchFixedIntervals(BOOL bEnable = TRUE) {
    m_bStretchFixedIntervals = bEnable;
  }
  BOOL IsStretchFixedIntervalsEnabled() const {
    return m_bStretchFixedIntervals;
  }

  void SetRightOffsetInPixels(int nValuesCount = 0);
  int  GetRightOffsetInPixels() const {
    return m_nRightOffset;
  }
  int  GetRightOffsetAsNumberOfValues() const;

  void SetFixedUnitCount(int nCount, BOOL bSet);
  BOOL IsFixedUnitCount() const {
    return m_bIsFxedUnitCount;
  }
  int  GetFixedUnitCount() const {
    return m_nFixedUnitCount;
  }

  virtual void SetFixedMajorUnit(double dblMajorUnit, BOOL bSet = TRUE);
  BOOL IsFixedMajorUnit() const {
    return m_bFixedMajorUnit;
  }

  void SetMinAllowedMajorUnit(double dblMinAllowedMajorUnit);
  double GetMinAllowedMajorUnit() const {
    return m_dblMinAllowedMajorUnit;
  }

  double GetMajorUnit() const {
    return m_dblMajorUnit;
  }
  double GetMinorUnit() const {
    return m_dblMinorUnit;
  }

  BOOL IsAutoMajorUnitEnabled() const {
    return m_bIsAutoMajorUnit;
  }
  void EnableAutoMajorUnit(BOOL bEnable = TRUE) {
    m_bIsAutoMajorUnit = bEnable;
  }

  virtual int GetMaxTotalLines() const {
    return m_nMaxTotalLines;
  }
  int GetMajorUnitSpacing() const {
    return m_nMajorUnitSpacing;
  }
  double GetMajorUnitSpacingScaled() const;
  void SetMajorUnitSpacing(int nSpacing = 0) {
    m_nMajorUnitSpacing = abs(nSpacing);
  }

  CBCGPChartAxis* GetCustomPerpendicularAxis() const {
    return m_pPerpendicularAxis;
  }

  void UsePerpendicularAxisForZoning(BOOL bUse) {
    m_bUsePerpAxisForZoning = bUse;
  }
  BOOL IsUsePerpendicularAxisForZoning() const {
    return m_bUsePerpAxisForZoning;
  }

  virtual void SetIndexedSeries(CBCGPChartSeries* pSeries);
  CBCGPChartSeries* GetIndexedSeries() const {
    return m_pIndexedSeries;
  }
  virtual BOOL IsIndexedSeries() const {
    return m_pIndexedSeries != NULL && m_pIndexedSeries->IsIndexMode();
  }

  virtual BOOL IsComponentXSet() const {
    return IsAxisComponentSet();
  }
  ...
  double GetBottomOffset() const {
    return m_dblBottomOffset;
  }
  double GetTopOffset() const {
    return m_dblTopOffset;
  }
  int GetAxisGap() const {
    return m_nAxisGap;
  }
  double GetAxisGapScaled() const;

  CBCGPChartAxis* GetSplitTop() const {
    return m_pSplitTop;
  }
  CBCGPChartAxis* GetSplitBottom() const  {
    return m_pSplitBottom;
  }
  ...
  void UseApproximation(BOOL bUse) {
    m_bUseApproximation = bUse;
  }

  BOOL IsLabelInterlacingEnabled() const {
    return m_bEnableInterlaceLabels;
  }
  void EnableLabelInterlacing(BOOL bEnable = TRUE);

  BOOL IsPreventLabelOverlapIn3D() const {
    return TRUE;
  }
  void SetPreventLabelOverlapIn3D(BOOL bSet = TRUE) {
    m_bIsPreventLabelOverlapIn3D = bSet;
  }
  ...
  CBCGPChartScaleBreakOptions GetScaleBreaksOptions() const {
    return m_scaleBreakOptions;
  }
...
protected:
  ...
  virtual BOOL IsVariableLabelAngle() const {
    return FALSE;
  }
  ...
  virtual BOOL IsAxisComponentSet() const {
    return TRUE;
  }
};
...
public:
  ...
  virtual CBCGPChartData::ComponentIndex GetComponentIndex() const {
    return CBCGPChartData::CI_X;
  }
  virtual void CalcMajorMinorUnits(CBCGPGraphicsManager* pGM, UINT nUnitsToDisplay = -1);
  virtual void GetDisplayedLabel(double dblValue, CString& strLabel);

  virtual BOOL CanDisplayDataBetweenTickMarks() const {
    return !m_bIsAxisComponentSet && !m_bOnlyFormula;
  }
  virtual BOOL IsComponentXSet() const {
    return IsAxisComponentSet();
  }
  ...
  virtual BOOL IsLogScale() const {
    return m_bLogScale && IsComponentXSet() && !m_bFormatAsDate;
  }
...
public:
  ...
  CBCGPChartTernaryAxis::LabelMode GetLabelMode() const {
    return m_labelMode;
  }
