...
public:
  ...
  virtual BCGP_3D_RENDERING_TYPE GetRenderingType() const {
    return CBCGPEngine3D::BCGP_RT_SOFTWARE;
  }
  virtual BOOL IsSoftwareRendering() const {
    return TRUE;
  }
  ...
  virtual BOOL IsRenderToWindow() const {
    return FALSE;
  }

  CBCGPGraphicsManager* GetDefaultManager() const {
    return m_pDefaultGM;
  }
  ...
  const CBCGPRect& GetSceneRect() const {
    return m_rectScene;
  }
