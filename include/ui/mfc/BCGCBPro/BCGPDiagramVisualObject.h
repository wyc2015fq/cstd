...
public:
  ...
  CBCGPDiagramItemID(const CBCGPDiagramItemID& src) {
    m_nId = src.m_nId;
    m_bConnector = src.m_bConnector;
  }

  BOOL IsNull() const {
    return m_nId == Undefined;
  }
...
public:
  ...
  const CBCGPDiagramItemID& GetItemID() const {
    return m_ItemId;
  }
  ...
  const CBCGPStrokeStyle& GetStrokeStyle() const {
  ...
  virtual BOOL FromTag(const CString& /*strCustomProps*/) {
    return TRUE;
  }
...
public:
  ...
  BOOL IsTrackingAnchorPoints() const {
    return m_nTrackPointIndex != -1;
  }
