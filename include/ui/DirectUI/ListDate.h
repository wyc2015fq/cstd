...
class CListDate : public CDateTimePickerCtrlT<CWindowWnd> {
  typedef CDateTimePickerCtrlT<CWindowWnd> base;
public:
  DECLARE_WND_CLASS( _T( "ListDate" ) )
  CListDate() {
    m_nItem = NULL_ITEM;
...
public:
  BOOL Create( HWND hWndParent, int nItem, int nSubItem, CRect& rcRect, UINT nFlags, SYSTEMTIME& stItemDate ) {
    ...
    if ( base::Create( hWndParent, CRect( rcRect.left + 3, rcRect.top + 2, rcRect.right - 3, rcRect.bottom - 2 ), NULL, dwStyle ) == NULL )
      return FALSE;
    // remove border
    ...
    if ( nFlags & ITEM_FLAGS_DATE_ONLY )
      ...
      SetFormat( CStdString( szDateFormat ) + _T( " " ) + CString( szTimeFormat ) );
