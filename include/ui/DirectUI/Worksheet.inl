...
#include "GetBaseName.h"
#include "Text.h"
#include "Workbook.h"
#include "BorderPropertyPage.h"
#include "FontPropertyPage.h"
#include "AlignmentPropertyPage.h"
#include "PatternPropertyPage.h"
...
#include "EditImp.h"
  ...
  BOOL Worksheet::SetName1(LPCTSTR name)
  {
...
#define AtlTrace printf
  void Worksheet::Scroll_ResetHScrollBar(LPCRECT lprcClient)
  {
    ...
    const BOOL bWindowVis=IsVisible();
#ifdef _DEBUG
...
//#endif
  ...
  */
  ...
  int Worksheet::put_TopVisScrollRow(int nTopVisScrollRow,const CellAxisInfo* pcai/*,bool bScroll*/)
  {
    ...
    if( nTopVisScrollRow!=oTopVisScrollRow)
    {
      int nScrollHeight=0;
...
//#endif
  ...
  BOOL Worksheet::delete_cols(int beg,int end,bool bFireMsg)
  {
    ...
    BOOL const isWindow=1;//IsWindow();
  ...
  void Worksheet::Range_SetPattern(const CCellRange& cr,StyleDesc _style,UINT nPatternMask)
  {
    if(HEADER_ROW==cr.TopRow() && HEADER_COL==cr.LeftCol()){
      ...
      typedef vector<CMergeCell> CMergeCellVector;
      const CMergeCellVector& vMerge=GetMergeCells();
      for(CMergeCellVector::const_iterator it2=vMerge.begin();it2!=vMerge.end();++it2){
        if(const StyleID_t sid=it2->GetCellStore().GetStyleID()){
          style=Style_GetStyleByID(sid);
          Style_CopyPatternProperties(style,_style,nPatternMask);
          Style_SetCellStyle(it2->TopLeft(),style);
        }
      }
      ...
      map<CCell,CellStore> maps;cellStores_.EnumCells(maps);
      for(map<CCell,CellStore>::const_iterator it1=maps.begin();it1!=maps.end();++it1){
        if(const StyleID_t sid=it1->second.GetStyleID() && cr.ColInside(it1->first.col)){
          style=Style_GetStyleByID(sid);
          Style_CopyPatternProperties(style,_style,nPatternMask);
          Style_SetCellStyle(it1->first,style);
        }
      }
      const vector<CMergeCell>& vMerge=GetMergeCells();
      for(vector<CMergeCell>::const_iterator it2=vMerge.begin();it2!=vMerge.end();++it2){
        if(const StyleID_t sid=it2->GetCellStore().GetStyleID() && cr.Inside(*it2)){
          style=Style_GetStyleByID(sid);
          Style_CopyPatternProperties(style,_style,nPatternMask);
          Style_SetCellStyle(it2->TopLeft(),style);
        }
  ...
  CMergeCell* Worksheet::Merge(const int minR,const int minC,const int maxR,const int maxC)
  {
    ...
    *style.GetBorders()=bdrs;
    ...
    //if(m_hWnd)
    {
      ...
      UpdateWindow();
    }//else selectionImp_.OnMergeCells(pMerge->TopRow(),pMerge->LeftCol(),pMerge->BottomRow(),pMerge->RightCol());
    return pMerge;
  }
  ...
  CCellRange Worksheet::Unmerge(int minR,int minC,int maxR,int maxC)
  {
    ...
    if(cr.Inside(GetActiveCell()))
      SetActiveCell(cr.TopLeft(), Int2Type<false>());
    RecalcVisibleMergeCells();
    //RECT rc=get_RangeRect(&cr);
    //InvalidateRect(&rc);
    //if(m_hWnd)
