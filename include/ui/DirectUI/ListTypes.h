...
#if 0
template <class T >
class CListArray : public CSimpleArray< T > {
public:
  void Reverse() {
...
#else
template <class T >
class CListArray : public std::vector< T > {
...
public:
  ...
  void Reverse() {
    std::reverse( begin(), begin() + GetSize() );
  }
  void Sort() {
    std::sort( begin(), begin() + GetSize() );
  }
  template < class Pred >
  inline
  void Sort( Pred Compare ) {
    sort( begin(), begin() + GetSize(), Compare );
  }
  BOOL IsEmpty() {
    return ( GetSize() == 0 );
  }
  BOOL InsertAt( int nIndex, T& t ) {
    base::insert( begin() + nIndex, t );
    return TRUE;
  }
  BOOL RemoveAll() {
    base::clear();
