...
namespace mycell{
	...
	class Workbook;
	class Worksheet : public CWindow
		,public MergeCellMgr
	...
	public:
		...
		*/
		...
		{
			return hScrollBar_.GetScrollInfo(lpScrollInfo);
		}
		BOOL Scroll_GetVScrollInfo(LPSCROLLINFO lpScrollInfo) const
		{
			return vScrollBar_.GetScrollInfo(lpScrollInfo);
		}
		BOOL Scroll_SetHScrollInfo(LPSCROLLINFO lpScrollInfo,BOOL bRedraw=1)
		{
			return hScrollBar_.SetScrollInfo(lpScrollInfo,bRedraw);
		}
		BOOL Scroll_SetVScrollInfo(LPSCROLLINFO lpScrollInfo,BOOL bRedraw=1)
		{
			return vScrollBar_.SetScrollInfo(lpScrollInfo,bRedraw);
		}
	...
	private:
		DWORD bits_;//位组合
		//EMouseMode mouseMode_;
		//ESelectionMode selectionMode_;
		RowHeader rh_;
		ColHeader ch_;
		
		Workbook* pWorkbook_;
		CScrollBar hScrollBar_,vScrollBar_;

		static CCell tooltipCell_;
		static pair<TOOLINFO,CToolTipCtrl> tooltip_;

		//Selections selections_;
		//ActiveCell activeCell_;
		///StyleID_t nTableStyleID_;
		CellStore tableDefCellStore_;

		VisibleMergeCellMgr visMergeCells_;
		//VisibleMergeCellMgr visFreezeAlthogonalMergeCells_;
		//VisibleMergeCellMgr visFreezeRowsMergeCells_;
		//VisibleMergeCellMgr visFreezeColsMergeCells_;

		//StyleDesc defaultStyle_;
		StyleStore& styleStore_;
		CellStoreMgr cellStores_;
		//DWORD dwTickCount_;
		//PenFactory penFct_;
		ActiveSelectionBounds asbounds_;
		//map<CellType,CComPtr<ICellEditor> > mapEditor_;
