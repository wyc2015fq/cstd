...
CvSize scaled_image_size;
int original_image_step;
int scaled_image_step;
...
// Private API ////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
GtkType cvImageWidget_get_type(int);
static GtkWidgetClass* parent_class = NULL;
// flag to help size initial window
#define CV_WINDOW_NO_IMAGE 2
#undef CV_ARE_SIZES_EQ
#define CV_ARE_SIZES_EQ(mat1, mat2) \
  ((mat1)->width == (mat2)->width && (mat1)->height == (mat2)->height)
int cvImageWidgetSetImage(CvImageWidget* widget, const uchar* arr, CvSize mat_size, int step, int cn)
{
  int origin = 0;
  //printf("cvImageWidgetSetImage\n");
  widget->original_image_step = step;
#define cvReleaseMat(x) free(*(x)), *(x)=0
  if (widget->original_image && !CV_ARE_SIZES_EQ(&mat_size, &widget->original_image_size)) {
    cvReleaseMat(&widget->original_image);
  }
#define cvCreateMat(a, b, c) (uchar*)malloc((a)*(b)*3)
  if (!widget->original_image) {
    widget->original_image = cvCreateMat(mat_size.height, mat_size.width, CV_8UC3);
    gtk_widget_queue_resize(GTK_WIDGET(widget));
  }
  //cvConvertImage( mat, widget->original_image, (origin != 0 ? CV_CVTIMG_FLIP : 0) + CV_CVTIMG_SWAP_RB );
  IM2IM(mat_size.height, mat_size.width, arr, step, cn, widget->original_image, widget->original_image_step, 3);
  widget->original_image_size.height = mat_size.height;
  widget->original_image_size.width = mat_size.width;
  widget->original_image_step = mat_size.width * 3;
#define BGRRGB(src, dst) BGRx2BGR(src, dst, 2)
  IMTRANS(mat_size.height, mat_size.width, widget->original_image, widget->original_image_step, 3, widget->original_image, widget->original_image_step, 3, BGRRGB);
  if (widget->scaled_image) {
    //cvResize( widget->original_image, widget->scaled_image, CV_INTER_AREA );
    IMRESIZE(widget->original_image_size.height, widget->original_image_size.width,
        widget->original_image, widget->original_image_step, 3,
        widget->scaled_image_size.height, widget->scaled_image_size.width,
        widget->scaled_image, widget->scaled_image_step, 3, INTER_BILINEARITY
            );
  }
    ...
    GtkRequisition* requisition)
{
  ...
  if (image_widget->original_image &&
    ...
    //printf("original ");
    requisition->width = image_widget->original_image_size.width;
    requisition->height = image_widget->original_image_size.height;
  }
  // default case
  else if (image_widget->scaled_image) {
    //printf("scaled ");
    requisition->width = image_widget->scaled_image_size.width;
    requisition->height = image_widget->scaled_image_size.height;
  }
...
static int cvImageWidget_set_size(GtkWidget* widget, int max_width, int max_height)
{
  CvImageWidget* image_widget = CV_IMAGE_WIDGET(widget);
  ...
  CvSize scaled_image_size = cvImageWidget_calc_size(image_widget->original_image_size.width,
      image_widget->original_image_size.height, max_width, max_height);
  if (image_widget->scaled_image &&
      (image_widget->scaled_image_size.width != scaled_image_size.width ||
          image_widget->scaled_image_size.height != scaled_image_size.height)) {
    ...
    GtkAllocation* allocation)
{
  ...
  if ((image_widget->flags & CV_WINDOW_AUTOSIZE) == 0 && image_widget->original_image) {
    ...
    if (image_widget->flags & CV_WINDOW_NO_IMAGE) {
      cvImageWidget_set_size(widget, image_widget->original_image_size.width,
          image_widget->original_image_size.height);
    }
    else {
    ...
    IMRESIZE(image_widget->original_image_size.height, image_widget->original_image_size.width,
        image_widget->original_image, image_widget->original_image_step, 3,
        image_widget->scaled_image_size.height, image_widget->scaled_image_size.width,
        image_widget->scaled_image, image_widget->scaled_image_step, 3, INTER_BILINEARITY
            );
  }
  if (GTK_WIDGET_REALIZED(widget)) {
    ...
    if (image_widget->original_image &&
        ((image_widget->flags & CV_WINDOW_AUTOSIZE) ||
            (image_widget->flags & CV_WINDOW_NO_IMAGE))) {
      widget->allocation.width = image_widget->original_image_size.width;
      widget->allocation.height = image_widget->original_image_size.height;
      gdk_window_move_resize(widget->window, allocation->x, allocation->y,
          image_widget->original_image_size.width, image_widget->original_image_size.height);
      if (image_widget->flags & CV_WINDOW_NO_IMAGE) {
        image_widget->flags &= ~CV_WINDOW_NO_IMAGE;
        gtk_widget_queue_resize(GTK_WIDGET(widget));
      }
    ...
    GdkEventExpose* event)
{
  ...
  if (image_widget->scaled_image) {
    ...
    int x0 = (widget->allocation.width - image_widget->scaled_image_size.width) / 2;
    int y0 = (widget->allocation.height - image_widget->scaled_image_size.height) / 2;
    gdk_draw_rgb_image(widget->window, widget->style->fg_gc[ GTK_STATE_NORMAL ],
        x0, y0, MIN(image_widget->scaled_image_size.width, widget->allocation.width),
        MIN(image_widget->scaled_image_size.height, widget->allocation.height),
        GDK_RGB_DITHER_MAX, image_widget->scaled_image, image_widget->scaled_image_step);
  }
  else if (image_widget->original_image) {
    gdk_draw_rgb_image(widget->window, widget->style->fg_gc[ GTK_STATE_NORMAL ],
        0, 0,
        MIN(image_widget->original_image_size.width, widget->allocation.width),
        MIN(image_widget->original_image_size.height, widget->allocation.height),
        GDK_RGB_DITHER_MAX, image_widget->original_image, image_widget->original_image_step);
  }
  return TRUE;
}
static int
...
static int cvImageWidget_class_init(CvImageWidgetClass* klass)
{
...
static int icvOnTrackbar(GtkWidget* widget, gpointer user_data);
...
int cvChangeMode_GTK(const char* name, double prop_value) //Yannick Verdie
{
...
static int icvDeleteWindow(CvWindow* window)
{
  CvTrackbar* trackbar;
  if (window->prev) {
...
CV_IMPL int cvDestroyWindow(const char* name)
{
...
CV_IMPL int cvDestroyAllWindows(int)
{
  CV_LOCK_MUTEX();
  while (hg_windows) {
    CvWindow* window = hg_windows;
...
CV_IMPL int cvShowImage(const char* name, int height, int width, const unsigned char* arr, int step, int cn)
{
  ...
  if (window && arr) {
    ...
    cvImageWidgetSetImage(image_widget, arr, cvSize(width, height), step, cn);
  }
  CV_UNLOCK_MUTEX();
  __END__;
}
CV_IMPL int cvResizeWindow(const char* name, int width, int height)
{
...
CV_IMPL int cvMoveWindow(const char* name, int x, int y)
{
...
icvCreateTrackbar(const char* trackbar_name, const char* window_name,
    ...
    CvTrackbarCallback2 on_notify2, int* userdata)
{
  int result = 0;
...
CV_IMPL int cvSetMouseCallback(const char* window_name, CvMouseCallback on_mouse, int* param)
{
...
CV_IMPL int cvSetTrackbarPos(const char* trackbar_name, const char* window_name, int pos)
{
...
CV_IMPL int* cvGetWindowHandle(const char* window_name)
{
  int* widget = 0;
  ...
  window = icvFindWindowByName(window_name);
  if (window) {
    widget = (int*) window->widget;
  }
  __END__;
  return widget;
}
CV_IMPL const char* cvGetWindowName(int* window_handle)
{
...
static gboolean icvOnMouse(GtkWidget* widget, GdkEvent* event, gpointer user_data)
{
  ...
  if (cv_event >= 0) {
    ...
    if ((image_widget->flags & CV_WINDOW_AUTOSIZE) == 0 &&
      ...
      int x0 = (widget->allocation.width - image_widget->scaled_image_size.width) / 2;
      int y0 = (widget->allocation.height - image_widget->scaled_image_size.height) / 2;
      pt.x = cvRound(((pt32f.x - x0) * image_widget->original_image_size.width) /
          image_widget->scaled_image_size.width);
      pt.y = cvRound(((pt32f.y - y0) * image_widget->original_image_size.height) /
          image_widget->scaled_image_size.height);
    }
    else {
      pt = cvPointFrom32f(pt32f);
    }
    if ((unsigned) pt.x < (unsigned)(image_widget->original_image_size.width) &&
        (unsigned) pt.y < (unsigned)(image_widget->original_image_size.height)) {
