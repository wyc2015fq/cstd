...
void CPage5::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_SHOW_COMMANDS, m_bShowCommands);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CPage5, CBCGPPropertyPage)
	...
	ON_BN_CLICKED(IDC_SHOW_COMMANDS, OnShowCommands)
	//}}AFX_MSG_MAP
...
void CPage5::OnPropListCustomColors() 
{
	...
	if (m_bPropListCustomColors)
	{
		...
		m_wndPropList.SetCommandTextColor (RGB (255, 0, 0));
	}
	else
	{
		...
		m_wndPropList.SetCommandTextColor ((COLORREF)-1);
	}
...
void CPage5::OnShowCommands() 
{
	UpdateData ();

	if (!m_bShowCommands)
	{
		m_wndPropList.ClearCommands ();
		return;
	}
	...
	lstCommands.AddTail (_T("Command 3"));

	m_wndPropList.SetCommands (lstCommands);
}

LRESULT CPage5::OnCommandClicked(WPARAM, LPARAM lp)
{
	CString str;
	str.Format (_T("Command %d clicked..."), (int) lp + 1);
