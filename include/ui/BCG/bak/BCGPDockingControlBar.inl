...
{
	ASSERT_VALID (this);
	CRect rect (0, 0, sizeDefault.w, sizeDefault.h);
									   ...
									   CCreateContext* pContext)
{
	...
	{
		m_sizeDialog.h += GetCaptionHeight();
		m_rectRestored.r = m_rectRestored.l + m_sizeDialog.w;
		m_rectRestored.b = m_rectRestored.t + m_sizeDialog.h;
	}
...
void CBCGPDockingControlBar::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp) 
{
    ...
    CPoint ptOrgBtnRight = CPoint (rcClient.r - sizeButton.w - g_nCaptionHorzMargin,
		rcClient.t - cyGripper - m_nBorderSize + (cyGripper - sizeButton.h) / 2);
    CPoint ptOrgBtnLeft = CPoint (rcClient.l + g_nCaptionHorzMargin, ptOrgBtnRight.y);
	...
	if (cyGripper > 0)
	{
		...
		for (i = 0; i < m_arrButtons.GetSize(); i ++)
		{
			...
			if (pbtn->m_bLeftAlign)
			{
				...
				if (!bHide)
				{
					ptOrgBtnLeft.Offset (sizeButton.w + 2, 0);
				}
			}
			else
			{
				...
				if (!bHide)
				{
					ptOrgBtnRight.Offset (- sizeButton.w - 2, 0);
				}
			}
		}
		...
		for (i = 0; i < m_arrButtons.GetSize(); i ++)
		{
			...
			if (pbtn->m_bLeftAlign)
			{
				pbtn->m_bHidden = CRect (lpncsp->rgrc[0]).l + pbtn->GetRect().l >= ptOrgBtnRight.x;
			}
		}
	}
	else
	...
	rcClient.r = max(rcClient.r, rcClient.l);
	rcClient.b = max(rcClient.b, rcClient.t);
...
void CBCGPDockingControlBar::OnNcPaint() 
{
    ...
    if (cyGripper > 0)
	{
		...
		rectCaption.OffsetRect (-rectCaption.l, -rectCaption.t);
		rectCaption.DeflateRect (0, 1);

		rectCaption.l = rcClient.l;
		rectCaption.t --;
		rectCaption.b = rectCaption.t + cyGripper - 2;
	...
	if (m_bMemDC)
	{
		...
		//-------------------------------------- 
		dcPaint.BitBlt (rcBar.l, rcBar.t, rcBar.Width(), rcBar.Height(),
					   &dcMem, rcBar.l, rcBar.t, SRCCOPY);
...
int CBCGPDockingControlBar::HitTest (CPoint point, BOOL bDetectCaption)
{
	...
	if (IsDocked())
	{
		...
		int nBorderHeight = 1;
		// caption
		rect.SetRect (rectWnd.l + nBorderWidth, rectWnd.t + nBorderHeight, 
							 rectWnd.r - nBorderWidth, 
							 rectWnd.t + nBorderHeight + GetCaptionHeight());
...
//****
void CBCGPDockingControlBar::OnPaint(CDC* pDC) 
{
	ASSERT_VALID (this);
...
void CBCGPDockingControlBar::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if ((GetDockMode() & BCGP_DT_IMMEDIATE) != 0)
	{
		...
		if (m_bPrepareToFloat)
		{
			...
			if ((abs (ptDragOffset.x) > m_sizeDragSencitivity.w || 
				 abs (ptDragOffset.y) > m_sizeDragSencitivity.h) && m_bReadyToFloat)
			{
				if (IsTabbed())
				{
		...
		if (nHot == nHit)
		{
			...
			case HTMINBUTTON:
				if (CBCGPDockManager::IsDockBarMenu())
				{
					...
					CPoint ptMenu (rectButton.l, rectButton.b + 1);
...
BOOL CBCGPDockingControlBar::CheckStopSlideCondition (BOOL bDirection)
{
	...
	case CBRS_ALIGN_RIGHT:
		if (m_ahSlideMode == BCGP_AHSM_MOVE)
		{
			if (bIsRTL)
			{
				bStop = bDirection ? rectWnd.l >= rectAutoHideDockBar.r :
					 rectWnd.r <= rectAutoHideDockBar.r;

			}
			else
			{
				bStop = bDirection ? rectWnd.r <= rectAutoHideDockBar.l : 
									 rectWnd.l >= rectAutoHideDockBar.l;
			}
		}
		else
		{
	...
	case CBRS_ALIGN_TOP:	
		if (m_ahSlideMode == BCGP_AHSM_MOVE)
		{
			bStop = bDirection ? rectWnd.t >= rectAutoHideDockBar.b :
								 rectWnd.b <= rectAutoHideDockBar.b;
		}
		else
		{
	...
	case CBRS_ALIGN_BOTTOM:	
		if (m_ahSlideMode == BCGP_AHSM_MOVE)
		{
			bStop = bDirection ? rectWnd.b <= rectAutoHideDockBar.t :
								 rectWnd.t >= rectAutoHideDockBar.t;
		}
		else
		{
...
void CBCGPDockingControlBar::OnSlide (BOOL bSlideDirection)
{
	...
	SetWindowPos (NULL, rect.l, rect.t,
					rect.Width(), rect.Height(),
					SWP_NOZORDER | SWP_NOACTIVATE);

	::SetWindowPos (m_hDefaultSlider, NULL, rectSlider.l, rectSlider.t,
...
void CBCGPDockingControlBar::CalcRectForSliding (CRect& rect, CRect& rectSlider, BOOL bSlideDirection)
{
	...
	case CBRS_ALIGN_LEFT:
		if (bSlideDirection)   
		{
			rect.r += m_nSlideDelta;
			if (rect.Width() > m_rectRestored.Width())
			{
				rect.r = rect.l + m_rectRestored.Width();
			}
		}
		else
		{
		...
		}
		{
			...
			rectSlider.l = rect.r;
			rectSlider.r = rectSlider.l + nSliderWidth;
		}
		break;
	...
	case CBRS_ALIGN_RIGHT:
		if (bSlideDirection)   
		{
			rect.l -= m_nSlideDelta;
			if (rect.Width() > m_rectRestored.Width())
			{
				rect.l = rect.r - m_rectRestored.Width();
			}
		}
		else
		{
		...
		}
		{
			int nSliderWidth = rectSlider.Width();
			rectSlider.r = rect.l;
			rectSlider.l = rectSlider.r - nSliderWidth;
		}
		break;
	...
	case CBRS_ALIGN_TOP:
		if (bSlideDirection)   
		{
			rect.b += m_nSlideDelta;
			if (rect.Height() > m_rectRestored.Height())
			{
				rect.b = rect.t + m_rectRestored.Height();
			}
		}
		else
		{
		...
		}
		{
			...
			rectSlider.t = rect.b;
			rectSlider.b = rectSlider.t + nSliderHeight;
		}
		break;
	...
	case CBRS_ALIGN_BOTTOM:
		if (bSlideDirection)   
		{
			rect.t -= m_nSlideDelta;
			if (rect.Height() > m_rectRestored.Height())
			{
				rect.t = rect.b - m_rectRestored.Height();
			}
		}
		else
		{
		...
		}
		{
			int nSliderHeight = rectSlider.Height();
			rectSlider.b = rect.t;
			rectSlider.t = rectSlider.b - nSliderHeight;
		}
		break;
...
CPoint CBCGPDockingControlBar::CalcCorrectOffset (CRect rect, BOOL bIsRTL)
{
	...
	case CBRS_ALIGN_LEFT:
		if (bIsRTL)
		{
			if (rect.r < rectAutoHideDockBar.l)
			{
				return CPoint (rectAutoHideDockBar.l - rect.r, 0);
			}
		}
		else
		{
			if (rect.l > rectAutoHideDockBar.r)
			{
				return CPoint (rectAutoHideDockBar.r - rect.l, 0);
			}
		}
		break;
	case CBRS_ALIGN_RIGHT:
		...
		break;
	case CBRS_ALIGN_TOP:
		if (rect.t > rectAutoHideDockBar.b)
		{
			return CPoint (0, rectAutoHideDockBar.b - rect.t);
		}
		break;
	case CBRS_ALIGN_BOTTOM:
		if (rect.b < rectAutoHideDockBar.t)
		{
			return CPoint (0, rectAutoHideDockBar.t - rect.b);
		}
		break;
	}
	return CPoint (0, 0);
																...
																CBCGPDockingControlBar** ppTabbedControlBar)
{
	...
	{
		pTabbedBarAttachTo->SetWindowPos (NULL, rectWndTab.l, rectWndTab.t, 
								  ...
								  BCGP_DOCK_METHOD dockMethod)
{
	...
	if (dockMethod == BCGP_DM_DBL_CLICK || dockMethod == BCGP_DM_SHOW)
	{
		...
		if (pRecentDefaultSlider != NULL)
		{
			...
			/*
			SetWindowPos (NULL, 
				m_recentDockInfo.m_recentSliderInfo.m_rectDockedRect.l, 
				m_recentDockInfo.m_recentSliderInfo.m_rectDockedRect.t, 
												...
												BOOL bOuterEdge)
{
	...
	if (rectBar.Width() < minSize.w)
	{
		rectBar.r = rectBar.l + minSize.w;
		bSetMinSize = TRUE;
	}
	if (rectBar.Height() < minSize.h)
	{
		rectBar.b = rectBar.t + minSize.h;
		bSetMinSize = TRUE;
	}
...
void CBCGPDockingControlBar::DrawCaption (CDC* pDC, CRect rectCaption)
{
	...
	rectCaption.r = rcbtnRight.l;
	rectCaption.l = rcbtnLeft.r;
	rectCaption.t++;
...
void CBCGPDockingControlBar::AdjustBarWindowToContainer (CBCGPSlider* pSlider)
{
	...
	if (!rectContainer.IsRectEmpty())
	{
		...
		if (pFrame != NULL)
		{
			...
			{
				rectWnd.OffsetRect (rectContainer.l - rectWnd.l, 
									rectContainer.t - rectWnd.t); 
			if (rectWnd.Width() > rectContainer.Width())
			{
				rectWnd.r = rectWnd.l + rectContainer.Width();
			}
			if (rectWnd.Height() > rectContainer.Height())
			{
				rectWnd.b = rectWnd.t + rectContainer.Height();
			}

			SetWindowPos (NULL, rectWnd.l, rectWnd.t, 
...
BCGP_CS_STATUS CBCGPDockingControlBar::GetDockStatus (CPoint pt, int nSencitivity) 
{
	...
	else
	{
		...
		rectBar.t += GetCaptionHeight();
		rectBar.t += rectTabAreaTop.Height(); 
		rectBar.b -= rectTabAreaBottom.Height(); 
