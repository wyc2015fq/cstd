...
void CBCGPBarContainer::GetWindowRect (CRect& rect, BOOL bIgnoreVisibility) const
{
	...
	if (m_pBarLeftTop != NULL && (m_pBarLeftTop->IsBarVisible() || bIgnoreVisibility || bAutoHideMode))
	{
		...
		if (rectLeft.IsRectEmpty())
		{
			...
			if (rectLeft.Width() == 0)
			{
				rectLeft.InflateRect (0, 0, sz.w, 0);
			}
			
			if (rectLeft.Height() == 0)
			{
				rectLeft.InflateRect (0, 0, 0, sz.h);
			}
	...
	if (m_pBarRightBottom != NULL && (m_pBarRightBottom->IsBarVisible() || bIgnoreVisibility || bAutoHideMode))
	{
		...
		if (rectRight.IsRectEmpty())
		{
			...
			{
				rectRight.InflateRect (0, 0, sz.w, 0);
			}
			
			if (rectRight.Height() == 0)
			{
				rectRight.InflateRect (0, 0, 0, sz.h);
			}
		}
	}
...
void CBCGPBarContainer::GetMinSize (CSize& size) const
{
	...
	if (m_pSlider != NULL && (m_pSlider->IsBarVisible() || bAutoHideMode))
	{
		if (IsSliderHorz())
		{
			size.w = max (minSizeLeft.w, minSizeRight.w);
			size.w = max (sizeLeftContainer.w, size.w);
			size.w = max (sizeRightContainer.w, size.w);
			size.h = minSizeLeft.h + minSizeRight.h + sizeLeftContainer.h + 
						sizeRightContainer.h + m_pSlider->GetWidth();
		}
		else
		{
			size.h = max (minSizeLeft.h, minSizeRight.h);
			size.h = max (sizeLeftContainer.h, size.h);
			size.h = max (sizeRightContainer.h, size.h);
			size.w = minSizeLeft.w + minSizeRight.w + sizeLeftContainer.w + 
						sizeRightContainer.w + m_pSlider->GetWidth();
		}
	}
	else
	{
...
void CBCGPBarContainer::GetMinSizeLeft (CSize& size) const
{
	...
	size.w = max (minSizeLeft.w, sizeLeftContainer.w);
	size.h = max (minSizeLeft.h, sizeLeftContainer.h);
...
void CBCGPBarContainer::GetMinSizeRight(CSize& size) const
{
	...
	size.w = max (minSizeRight.w, sizeRightContainer.w);
	size.h = max (minSizeRight.h, sizeRightContainer.h);
}
//-----------------------------------------------------------------------------------//
CBCGPDockingControlBar* CBCGPBarContainer::AddControlBar (CBCGPDockingControlBar* pBar)
{
	...
	{
		rectNew.l = rectContainer.l;
		rectNew.t = rectContainer.t;
	}
	else if (!rectParentContainer.IsRectEmpty())
	{
		rectNew.l = rectParentContainer.l;
		rectNew.t = rectParentContainer.t;
	}

	CSize sizeMin;
	pBar->GetMinSize (sizeMin);

	if (nNewWidth < sizeMin.w)
	{
		nNewWidth = sizeMin.w;
	}

	if (nNewHeight < sizeMin.h)
	{
		nNewHeight = sizeMin.h;
	}
	...
	if (!IsContainerEmpty() && m_pSlider != NULL)
	{
		...
		{			
			...
			else
			{
				...
				rectNew.t = rectContainer.b - nNewHeight;
			}
		}
		else
		{
			...
			else
			{
				...
				rectNew.l = rectContainer.r - nNewWidth;
			}
		}
	}

	rectNew.b = rectNew.t + nNewHeight;
	rectNew.r = rectNew.l + nNewWidth;
	...
	if (bIsRecentLeftBar)
	{
		...
		if (m_pSlider != NULL)
		{
			if (IsSliderHorz())
			{
				rectSlider.t = rectNew.b;
				rectSlider.b = rectSlider.t + m_pSlider->GetWidth();
				rectSecondBar.t = rectSlider.b;
			}
			else
			{
				rectSlider.l     = rectNew.r;
				rectSlider.r    = rectSlider.l + m_pSlider->GetWidth();
				rectSecondBar.l  = rectSlider.r;
			}
		}
	...
	else
	{
		...
		if (m_pSlider != NULL)
		{
			if (IsSliderHorz())
			{
				rectSlider.b = rectNew.t;
				rectSlider.t = rectSlider.b - m_pSlider->GetWidth();
				rectSecondBar.b = rectSlider.t;
			}
			else
			{
				rectSlider.r = rectNew.l;
				rectSlider.l = rectSlider.r - m_pSlider->GetWidth();
				rectSecondBar.r = rectSlider.l;
			}
		}
		...
		if (pNextContainer != NULL)
		{
			...
			if (pParentSlider != NULL)
			{
				...
				if (bIsLeftContainer)
				{
					...
					{
						nOffset += nNewHeight;
						rectSlider.t = rectContainer.b;
						rectSlider.b = rectSlider.t + pParentSlider->GetWidth();
					}
					else 
					{
						nOffset += nNewWidth;
						rectSlider.l = rectContainer.r;
						rectSlider.r = rectSlider.l + pParentSlider->GetWidth();
					}
				}
				else
				{
					if (pParentSlider->IsHorizontal())
					{
						...
						rectSlider.b = rectContainer.t;
						rectSlider.t = rectSlider.b - pParentSlider->GetWidth();
					}
					else 
					{
						nOffset = -(nNewWidth + pParentSlider->GetWidth());;
						rectSlider.r = rectContainer.l;
						rectSlider.l = rectSlider.r - pParentSlider->GetWidth();
					}
				}
											   ...
											   HDWP& hdwp)
{
	...
	{
		rectPart.b += nOffset;
		if (rectPart.Height() < sizeMin.h)
		{
			rectPart.b = rectPart.t + sizeMin.h;
		}
	}
	else if (bLeftPart && !IsSliderHorz())
	{
		rectPart.r += nOffset;
		if (rectPart.Width() < sizeMin.w)
		{
			rectPart.r = rectPart.l + sizeMin.w;
		}
	}
	else if (!bLeftPart && IsSliderHorz())
	{
		...
		if (rectPart.Height() < sizeMin.h)
		{
			rectPart.t = rectPart.b - sizeMin.h;
		}
	}
	else
	{
		...
		if (rectPart.Width() < sizeMin.w)
		{
			rectPart.l = rectPart.r - sizeMin.w;
		}
	}
...
void CBCGPBarContainer::ResizeContainer (CRect rect, HDWP& hdwp, BOOL bRedraw)
{
	...
	if (!IsLeftPartEmpty (bCheckVisibility) && IsRightPartEmpty (bCheckVisibility))
	{
		if (m_pBarLeftTop != NULL)
		{
			if (rect.Width() < sizeMinLeft.w && CBCGPControlBar::m_bHandleMinSize)
			{
				rect.r = rect.l + sizeMinLeft.w;
			}
			if (rect.Height() < sizeMinLeft.h && CBCGPControlBar::m_bHandleMinSize)
			{
				rect.b = rect.t + sizeMinLeft.h;
			}
	...
	else if (IsLeftPartEmpty (bCheckVisibility) && !IsRightPartEmpty (bCheckVisibility))
	{
		if (m_pBarRightBottom != NULL)
		{
			if (rect.Width() < sizeMinRight.w && CBCGPControlBar::m_bHandleMinSize)
			{
				rect.r = rect.l + sizeMinRight.w;
			}
			if (rect.Height() < sizeMinRight.h && CBCGPControlBar::m_bHandleMinSize)
			{
				rect.b = rect.t + sizeMinRight.h;
			} 
	...
	else if (!IsLeftPartEmpty (bCheckVisibility) && !IsRightPartEmpty (bCheckVisibility))
	{
		...
		if (IsSliderHorz())
		{
			...
			if (m_bMaintainPercentage)
			{
				if (nDelta != 0)
				{
					rectFinalLeft.b = rectFinalLeft.t + rectLeft.Height() + 
			...
			rectFinalSlider.t = rectFinalLeft.b;
			rectFinalSlider.b = rectFinalSlider.t + m_pSlider->GetWidth();
			rectFinalRight.t = rectFinalSlider.b;

            if (CBCGPControlBar::m_bHandleMinSize)
			{
				int deltaLeft = sizeMinLeft.h - rectFinalLeft.Height();
				int deltaRight = sizeMinRight.h - rectFinalRight.Height();
				...
				{
					rectFinalLeft.b += deltaLeft;
					rectFinalRight.t = rectFinalLeft.b + nSliderWidth;
					if (rectFinalRight.Height() < sizeMinRight.h)
					{
						rectFinalRight.b = rectFinalRight.t + sizeMinRight.h;	
					}
				}
				else if (deltaLeft <= 0 && deltaRight > 0)
				{
					rectFinalLeft.b -= deltaRight;
					if (rectFinalLeft.Height() < sizeMinLeft.h)
					{
						rectFinalLeft.b = rectFinalLeft.t + sizeMinLeft.h;
					}
				...
				rectFinalSlider.t = rectFinalLeft.b;
				rectFinalSlider.b = rectFinalSlider.t + nSliderWidth;	

				dLeftPercent = ((double) rectFinalLeft.Height()) / 
										rectContainer.Height() * 100;

				if (rectFinalLeft.Width() < sizeMinLeft.w)
				{
					rectFinalLeft.r = rectFinalLeft.l + sizeMinLeft.w;
					rectFinalRight.r = rectFinalRight.l + sizeMinLeft.w;
				}
			}			
		...
		else
		{
			...
			if (m_bMaintainPercentage)
			{
				if (nDelta != 0)
				{
					rectFinalLeft.r = rectFinalLeft.l + rectLeft.Width() + 
									(int) (((double) nDelta * dLeftPercent) / 100.);
				}
				else
				{
			...
			rectFinalSlider.l = rectFinalLeft.r;
			rectFinalSlider.r = rectFinalSlider.l + m_pSlider->GetWidth();
			rectFinalRight.l = rectFinalSlider.r;
			
			if (CBCGPControlBar::m_bHandleMinSize)
			{
				int deltaLeft = sizeMinLeft.w - rectFinalLeft.Width();
				int deltaRight = sizeMinRight.w - rectFinalRight.Width();
				int nSliderWidth = m_pSlider->GetWidth();
				...
				{
					rectFinalLeft.r += deltaLeft;
					rectFinalRight.l = rectFinalLeft.r + nSliderWidth;
					if (rectFinalRight.Width() < sizeMinRight.w)
					{
						rectFinalRight.r = rectFinalRight.l + sizeMinRight.w;	
					}
				}
				else if (deltaLeft <= 0 && deltaRight > 0)
				{
					rectFinalLeft.r -= deltaRight;
					if (rectFinalLeft.Width() < sizeMinLeft.w)
					{
						rectFinalLeft.r = rectFinalLeft.l + sizeMinLeft.w;
					}
				...
				rectFinalSlider.l = rectFinalLeft.r;
				rectFinalSlider.r = rectFinalSlider.l + nSliderWidth;	
				...
				{
					rectFinalLeft.b = rectFinalLeft.t + sizeMinLeft.h;
					rectFinalRight.b = rectFinalRight.t + sizeMinLeft.h;
				}
			}				
										...
										BOOL bMoveSlider, HDWP& hdwp)
{
	...
	bStretchHorz ? sizeStretch.w = nOffset : sizeStretch.h = nOffset;

	int nAvailSpace = bStretchHorz ? CalcAvailableSpace (sizeStretch, bLeftBar).w : 
									CalcAvailableSpace (sizeStretch, bLeftBar).h;
	// set the sign here
		...
		!m_pSlider->IsHorizontal() && !bStretchHorz))
	{
		...
		if (bMoveSlider && m_pSlider != NULL)
		{
			...
			{
				bLeftBar ? rectSlider.r += nActualSize : 
						   rectSlider.l += nActualSize;
			}
			else
			{
				bLeftBar ? rectSlider.b += nActualSize : 
							rectSlider.t += nActualSize;
			}
			if (m_pSlider->IsBarVisible())
			{
...
int CBCGPBarContainer::OnMoveInternalSlider (int nOffset, HDWP& hdwp)
{
	...
	if (!rectLeft.IsRectEmpty())
	{
		...
		{
			rectLeft.b += nOffset;
			if (rectLeft.Height() < sizeMinLeft.h)
			{
				rectLeft.b = rectLeft.t + sizeMinLeft.h;
			}
		}
		else
		{
			rectLeft.r += nOffset;
			if (rectLeft.Width() < sizeMinLeft.w)
			{
				rectLeft.r = rectLeft.l + sizeMinLeft.w;
			}
		}
	}
	...
	if (!rectRight.IsRectEmpty())
	{
		...
		{
			rectRight.t += nOffset;
			if (rectRight.Height() < sizeMinRight.h)
			{
				rectRight.t = rectRight.b - sizeMinRight.h;
			}
		}
		else
		{
			rectRight.l += nOffset;
			if (rectRight.Width() < sizeMinRight.w)
			{
				rectRight.l = rectRight.r - sizeMinRight.w;
			}
		}
	}
								  ...
								  BOOL bLeftBar, HDWP& hdwp)
{
	...
	if (pBar != NULL)
	{
		...
		if (bHorz)
		{
			bLeftBar ? rectBar.b += nOffset : rectBar.t -= nOffset;  
		}	
		else
		{
			bLeftBar ? rectBar.r += nOffset : rectBar.l += nOffset;
		}
...
void CBCGPBarContainer::MoveWnd (CWnd* pWnd, int nOffset, BOOL bHorz)
{
	...
	if (pWnd != NULL)
	{
		...
		int nActualOffset = bHorz ? rectWnd.l - rectParent.l : 
									rectWnd.t - rectParent.t;
											...
											BOOL bLeftBar)
{
	...
	if (pBar != NULL)
	{
		...
		if (IsSliderHorz())
		{
			bLeftBar ? rectBar.b += nRequiredOffset :  
						rectBar.t += nRequiredOffset;  
			nAvailableSpace = pBar->CalcAvailableSize (rectBar).h;
		}	
		else
		{
			bLeftBar ? rectBar.r += nRequiredOffset : 
						rectBar.l += nRequiredOffset;
			nAvailableSpace = pBar->CalcAvailableSize (rectBar).w;
		}
	}
	else if (pContainer != NULL) 
	{
		...
		nAvailableSpace = IsSliderHorz() ? 
			pContainer->CalcAvailableSpace (CSize (0, nRequiredOffset), bLeftBar).h : 
			pContainer->CalcAvailableSpace (CSize (nRequiredOffset, 0), bLeftBar).w;
	}
...
CSize CBCGPBarContainer::CalcAvailableSpace (CSize sizeStretch, BOOL bLeftBar)
{
	...
	if (bLeftBar)
	{
		rectWndNew.r += sizeStretch.w;
		rectWndNew.b += sizeStretch.h;
	}
	else
	{
		rectWndNew.l += sizeStretch.w;
		rectWndNew.t += sizeStretch.h;
	}

	CSize sizeMin;
	GetMinSize (sizeMin);

	CSize sizeAvailable (sizeStretch.w, sizeStretch.h);

	if (rectWndNew.Width() < sizeMin.w)
	{
		sizeAvailable.w = rectWndOrg.Width() - sizeMin.w;
		// if already less or eq. to minimum
		if (sizeAvailable.w < 0)
		{
			sizeAvailable.w = 0;
		}
		...
		if (sizeStretch.w < 0)
		{
			sizeAvailable.w = -sizeAvailable.w;
		}
	}

	if (rectWndNew.Height() < sizeMin.h)
	{
		sizeAvailable.h = rectWndNew.Height() - sizeMin.h;
		if (sizeAvailable.h < 0)
		{
			sizeAvailable.h = 0;
		}
		...
		if (sizeStretch.h < 0)
		{
			sizeAvailable.h = -sizeAvailable.h;
		}
	}
...
void CBCGPBarContainer::OnShowControlBar (CBCGPDockingControlBar* pBar, BOOL bShow)
{
	...
	{
		m_pBarLeftTop->SetWindowPos (NULL, rectContainer.l, rectContainer.t,
			rectContainer.Width(), rectContainer.Height(), 
			SWP_NOZORDER | SWP_NOACTIVATE);
	}
	else if (m_pBarRightBottom != NULL && m_pBarRightBottom != pBar)
	{
		m_pBarRightBottom->SetWindowPos (NULL, rectContainer.l, rectContainer.t,
			rectContainer.Width(), rectContainer.Height(), 
			SWP_NOZORDER | SWP_NOACTIVATE);
	}
	else if (m_pLeftContainer != NULL)
