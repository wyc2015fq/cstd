			...
			BOOL /*bDrawBorder*/, BOOL /*bGrayDisabledButtons*/)
{
	...
	if (m_bDefaultDraw)
	{
		...
		if ((int) m_uiCustomizeCmdId > 0)
		{
			...
			{
				rectMenu.r = rectMenu.l + sizeImage.w + 2 * nMargin;
			}
		...
		if (!m_lstInvisibleButtons.IsEmpty())
		{
			...
			{
				rectMore.l = rectMore.r - sizeImage.w - 2 * nMargin;
			}
...
//****
ISIZE CCustomizeButton::OnCalculateSize (CDC* /*pDC*/, const CSize& sizeDefault, BOOL bHorz)
{
	if (m_bIsEmpty)
	{
		return CSize (0, 0);
	...
	if (bHorz)
	{
		return CSize (	CBCGPMenuImages::Size().w * xLargeIcons + 2 * nMargin, 
						sizeDefault.h);
	}
	else
	{
		return CSize (	sizeDefault.w, 
						CBCGPMenuImages::Size().h * xLargeIcons + 2 * nMargin);
	}
}
//****
