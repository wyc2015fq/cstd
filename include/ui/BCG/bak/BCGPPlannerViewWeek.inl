...
void CBCGPPlannerViewWeek::OnDrawClient (CDC* pDC, const CRect& rect)
{
	...
	pDC->MoveTo (m_ViewRects[0].r, rect.t);
	pDC->LineTo (m_ViewRects[0].r, rect.b);

	for (nDay = 1; nDay < 7; nDay++)
	{
		if (m_ViewRects[nDay].t != m_rectApps.t)
		{
			pDC->MoveTo (m_ViewRects[nDay].l, m_ViewRects[nDay].t - 1);
			pDC->LineTo (m_ViewRects[nDay].r, m_ViewRects[nDay].t - 1);
		}
	}
