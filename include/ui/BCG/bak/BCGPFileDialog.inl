	...
	case WM_SIZE:
		{
			...
			rectTabs.t += pFD->m_iLogoAreaHeight;
...
void CBCGPFileDialog::OnInitDone()
{
	...
	while (pWndChild != NULL)
	{
		...
		pWndChild->SetWindowPos (NULL, 
			rectCtl.l + iBorderWidth, 
			rectCtl.t + iBorderHeight + m_iLogoAreaHeight,
	...
	m_ImagesRecent.Create (	globalData.m_sizeSmallIcon.w,
							globalData.m_sizeSmallIcon.h,
							ILC_COLOR, 0, 10);
		...
		for (int i = 0; i < pMRUFiles->GetSize(); i ++)
		{
			...
			if (!strFile.IsEmpty())
			{
				...
				if (himlSmall != NULL)
				{
					...
					CBitmap* hIcon = pImages->ExtractIcon (sfi.iIcon);
...
void CBCGPFileDialog::CollectControls()
{
	...
	while (pWndChild != NULL)
	{
		...
		if (strClass.CompareNoCase (_T("SHELLDLL_DefView")) == 0)
		{
			rectList.l = rectCtl.l;
			rectList.r = rectCtl.r;
			rectList.b = rectCtl.b - 10;
...
//****
void CBCGPFileDialog::SetDlgIcon (CBitmap* hIconBig, CBitmap* hIconSmall/* = NULL*/)
{
	m_hIconBig = hIconBig;
