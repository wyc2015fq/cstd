...
void CBCGPDockManager::AlignAutoHideBar (CBCGPSlider* pDefaultSlider, BOOL bIsVisible)
{
	...
	if (bIsVisible)
	{
		...
		if (bHorz)
		{
			dwAlignment & CBRS_ALIGN_TOP ? rectSlider.b = rectSlider.t + sizeRequered.h : 
										   rectSlider.t = rectSlider.b - sizeRequered.h;
		}
		else
		{
			dwAlignment & CBRS_ALIGN_LEFT ? rectSlider.r = rectSlider.l + sizeRequered.w : 
											rectSlider.l = rectSlider.r - sizeRequered.w;
		}
	...
	else
	{
		...
		if (bHorz)
		{
			rectSlider.l = rectControlBar.l = m_rectOuterEdgeBounds.l;
			rectSlider.r = rectControlBar.r = m_rectOuterEdgeBounds.r;
		}
		else
		{
			rectSlider.t = rectControlBar.t = m_rectOuterEdgeBounds.t;
			rectSlider.b = rectControlBar.b = m_rectOuterEdgeBounds.b;			
		}
		...
		case CBRS_ALIGN_LEFT:
			if (bIsRTL)
			{
				if (rectSlider.r != m_rectOuterEdgeBounds.r)
				{
					ptOffset.x = m_rectOuterEdgeBounds.r - rectSlider.r;
				}
			}
			else
			{
				if (rectSlider.l != m_rectOuterEdgeBounds.l)
				{
					ptOffset.x = m_rectOuterEdgeBounds.l - rectSlider.l;
				}
			}
			break;
		case CBRS_ALIGN_RIGHT:
			...
			break;
		case CBRS_ALIGN_TOP:
			if (rectSlider.t != m_rectOuterEdgeBounds.t)
			{
				ptOffset.y = m_rectOuterEdgeBounds.t - rectSlider.t;
			}
			break;
		case CBRS_ALIGN_BOTTOM:
			if (rectSlider.b != m_rectOuterEdgeBounds.b)
			{
				ptOffset.y = m_rectOuterEdgeBounds.b - rectSlider.b;
			}
			break;
		}
		...
		pDefaultSlider->SetWindowPos (NULL, rectSlider.l, rectSlider.t, 
											rectSlider.Width(), rectSlider.Height(), 
											SWP_NOZORDER | SWP_NOACTIVATE);
		pControlBar->SetWindowPos (NULL, rectControlBar.l, rectControlBar.t, 
		...
		if (IsPointNearDockBar (ptMouse, dwAlignment, bOuterEdge))
		{
			*ppTargetBar = NULL;
			...
			case CBRS_ALIGN_LEFT:
				if (bIsRTL)
				{
					rectResult.l = rectResult.r - rectWnd.Width();
				}
				else
				{
					rectResult.r = rectResult.l + rectWnd.Width();
				}
				break;
			case CBRS_ALIGN_RIGHT:
				if (bIsRTL)
				...
				break;
			case CBRS_ALIGN_TOP:
				rectResult.b = rectResult.t + rectWnd.Height();
				break;
			case CBRS_ALIGN_BOTTOM:
				rectResult.t = rectResult.b - rectWnd.Height();
				break;
			}
...
BOOL CBCGPDockManager::AdjustRectToClientArea (CRect& rectResult, DWORD dwAlignment)
{
	...
	if (dwAlignment & CBRS_ORIENT_HORZ && rectResult.Height() >= nAllowedHeight)
	{
		...
		{
			rectResult.b = rectResult.t + nAllowedHeight;
			bAdjusted = TRUE;
		}
		else if (dwAlignment & CBRS_ALIGN_BOTTOM)
		{
			rectResult.t = rectResult.b - nAllowedHeight;
			bAdjusted = TRUE;
		}
	}
				...
				rectResult.Width() >= nAllowedWidth)
	{
		...
		if (dwAlignment & CBRS_ALIGN_LEFT)
		{
			if (bIsRTL)
			{
				rectResult.l = rectResult.r - nAllowedWidth;
			}
			else
			{
				rectResult.r = rectResult.l + nAllowedWidth;
			}
			bAdjusted = TRUE;
		}
...
BOOL CBCGPDockManager::OnMoveMiniFrame	(CWnd* pFrame)
{
	...
	if (pBCGMiniFrame != NULL)
	{
		...
		pBCGMiniFrame->SetWindowPos (NULL, rect.l, rect.t, 0, 0, SWP_NOZORDER | SWP_NOSIZE | SWP_NOACTIVATE);
	}
...
void CBCGPDockManager::AdjustDockingLayout (HDWP hdwp)
{
	...
	for (pos = m_lstControlBars.GetHeadPosition(); pos != NULL;)
	{
		...
		if (bHorizontal)
		{
			dwAlignment & CBRS_ALIGN_TOP ? rectControlBar.b = rectControlBar.t + sizeRequered.h : 
										   rectControlBar.t = rectControlBar.b - sizeRequered.h;
		}
		else
		{
			dwAlignment & CBRS_ALIGN_LEFT ? rectControlBar.r = rectControlBar.l + sizeRequered.w : 
											rectControlBar.l = rectControlBar.r - sizeRequered.w;
		}
		...
		else
		{
			...
			hdwp = pNextControlBar->SetWindowPos (NULL, rectControlBar.l, 
						rectControlBar.t, rectControlBar.Width(), rectControlBar.Height(), 
						SWP_NOZORDER | SWP_NOACTIVATE, hdwp);
		}

		if (dwAlignment & CBRS_ALIGN_TOP)
		{
			rectCurrBounds.t += rectControlBarScreen.Height();
		}
		else if (dwAlignment & CBRS_ALIGN_BOTTOM)
		{
			rectCurrBounds.b -= rectControlBarScreen.Height();
		}
		else if (dwAlignment & CBRS_ALIGN_LEFT)
		{
			rectCurrBounds.l += rectControlBarScreen.Width();
		}
		else 
		{
			rectCurrBounds.r -= rectControlBarScreen.Width();
		}
...
void CBCGPDockManager::CalcBarOffset (CBCGPAutoHideDockBar* pBar)
{
	...
	for (POSITION pos = m_lstControlBars.GetHeadPosition(); pos != NULL;)
	{
		...
		if (pBarNext->IsKindOf (RUNTIME_CLASS (CBCGPAutoHideDockBar)))
		{
			...
			{
				if (rectBarOrg.t == rectBarNext.b)
				{
					pNextSlidingDockBar->SetOffsetLeft (rectBarOrg.Width());
				}

				if (rectBarOrg.r == rectBarNext.l)
				{
										...
										BOOL bHorizontal, BOOL bResizable)
{
	...
	case CBRS_ALIGN_LEFT:
		...
		rectResult.b = rectResult.t + rectToAlignBy.Height();
		rectResult.r = rectResult.l + nCurrWidth;
		break;

	case CBRS_ALIGN_TOP:
		rectResult.TopLeft() = rectToAlignBy.TopLeft();
		rectResult.r = rectResult.l + rectToAlignBy.Width();
		rectResult.b = rectResult.t + nCurrHeight;
		break;

	case CBRS_ALIGN_RIGHT:
		rectResult.BottomRight() = rectToAlignBy.BottomRight();
		rectResult.t = rectResult.b - rectToAlignBy.Height();
		rectResult.l = rectResult.r - nCurrWidth;
		break;
	case CBRS_ALIGN_BOTTOM:
		rectResult.BottomRight() = rectToAlignBy.BottomRight();
		rectResult.l = rectResult.r - rectToAlignBy.Width();
		rectResult.t = rectResult.b - nCurrHeight;	
		break;
	}
...
void CBCGPDockManager::SetDockState()
{
	...
	for (pos = m_lstControlBars.GetHeadPosition(); pos != NULL;)
	{
		...
		else if (pBarNext->IsKindOf (RUNTIME_CLASS (CBCGPControlBar))) 
		{
			...
			else if (pBarNext->IsRestoredFromRegistry())
			{
				...
				pBarNext->SetWindowPos (NULL, rect.l, rect.t, rect.Width(), rect.Height(),
	...
	for (pos = m_lstLoadedBars.GetHeadPosition (); pos != NULL;)
	{
		...
		if (pNextBar->IsKindOf (RUNTIME_CLASS (CBCGPDockingControlBar)))
		{
			...
			pDockingBar->SetWindowPos (NULL, rect.l, rect.t, rect.Width(), rect.Height(),
										SWP_NOZORDER | SWP_FRAMECHANGED | SWP_NOACTIVATE);
	...
	for (pos = m_lstLoadedAutoHideBarIDs.GetHeadPosition(); pos != NULL;)
	{
		...
		if (pBar != NULL)
		{
			...
			pBar->SetWindowPos (NULL, info.m_rectBar.l, info.m_rectBar.t, 
										  ...
										  CBCGPDockingControlBar* pNewBar)
{
	...
	if (pOriginalBar->IsAutoHideMode())
	{
		...
		pNewBar->SetWindowPos (NULL, rectOrgWnd.l, rectOrgWnd.t, 
...
CBCGPSmartDockingParams::CBCGPSmartDockingParams()
{
	...
	m_clrTransparent = _RGB (255, 0, 255);
