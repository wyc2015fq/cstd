...
const int nTextMargin_sb = 4;	// Gap between image and text
...
//****
void CBCGPStatusBar::GetItemRect(int nIndex, IRECT* lpRect) const
{
	ASSERT_VALID(this);
...
//****
void CBCGPStatusBar::SetPaneIcon (int nIndex, CBitmap* hIcon, BOOL bUpdate, BOOL bAlphaBlend/* = FALSE*/)
{
	ASSERT_VALID(this);
...
//****
void CBCGPStatusBar::SetPaneIcon (int nIndex, CBitmap* hBmp,
								 COLORREF clrTransparent, BOOL bUpdate, BOOL bAlphaBlend/* = FALSE*/)
{
	...
	//---------------------------------------------------------
	CBitmap* hbmpCopy = (CBitmap*) ::CopyImage (hBmp, IMAGE_BITMAP, 0, 0, 0);
							...
							UINT nFrameRate, BOOL bUpdate)
{
	...
	{
		CBitmap* hIcon = ::ImageList_GetIcon (hImageList, i, ILD_TRANSPARENT);
		::ImageList_AddIcon (pSBP->hImage, hIcon);
		::DestroyIcon (hIcon);
	}
...
void CBCGPStatusBar::InvalidatePaneContent (int nIndex)
{
	...
	if (!(pSBP->nStyle & SBPS_NOBORDERS))
		...
		rect.t -= CY_BORDER;  // base line adjustment
...
void CBCGPStatusBar::OnDrawPane (CDC* pDC, CBCGStatusBarPaneInfo* pPane)
{
	...
	if (pPane->clrBackground != (COLORREF)-1)
	{
		...
		pDC->PatBlt (rectPane.l, rectPane.t, rectPane.Width(), rectPane.Height(), PATCOPY);
	...
	if (pPane->hImage != NULL && pPane->cxIcon > 0)
	{
		...
		rectIcon.r = rectIcon.l + pPane->cxIcon;

		int x = max (0, (rectIcon.Width() - pPane->cxIcon) / 2);
		int y = max (0, (rectIcon.Height() - pPane->cyIcon) / 2);

		::ImageList_DrawEx (pPane->hImage, pPane->nCurrFrame, pDC,
				rectIcon.l + x, rectIcon.t + y,
				pPane->cxIcon, pPane->cyIcon, CLR_NONE, 0, ILD_NORMAL);
	}

	CRect rectText = rectPane;
	rectText.l += pPane->cxIcon;

	if (pPane->cxIcon > 0)
	{
		rectText.l += nTextMargin_sb;
	}
...
void CBCGPStatusBar::RecalcLayout()
{
	...
	for (i = 0, pSBP = (CBCGStatusBarPaneInfo*)m_pData; i < m_nCount; i++, pSBP++)
	{
		...
		else
		{
			...
			rect.r = rect.l + cxPane + CX_BORDER * 4;
			rect.r = min(rect.r, xMax);

			pSBP->rect = rect;

			rect.l = rect.r + m_cxDefaultGap;
		}
	}
	...
	if (m_cxSizeBox != 0)
	{
		...
		m_rectSizeBox = rect;
		m_rectSizeBox.l = rect.r;
		m_rectSizeBox.r = m_rectSizeBox.l + cxMax;
	}
	else
	{
		m_rectSizeBox.SetRectEmpty();
