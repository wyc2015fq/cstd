...
static const int iBorderSize_tp = 1;
static const int iNavToolbarId = 1;
static const int iAnimTimerId_tp =		ID_CHECK_AUTO_HIDE_CONDITION + 1;
static const int iScrollTimerId_tp =	ID_CHECK_AUTO_HIDE_CONDITION + 2;

static inline BOOL IsSystemCommand1(UINT uiCmd)
{
...
void CBCGPTasksPaneToolBar::AdjustLocations()
{
	...
	if (pMenuBtn != NULL)
	{
		...
		int nMin = rectMenuBtn.l + rectMenuBtn.Height() * 3;
		int nMax = rectClient.r - 1;
		if (pCloseBtn != NULL && bShowCloseButton)
		{
			nMax = rectClient.r - 1 - rectMenuBtn.Height();
		}
		...
		if (pCloseBtn != NULL && bShowCloseButton)
		{
			...
			rectCloseBtn.l = rectMenuBtn.r;
			rectCloseBtn.r = rectMenuBtn.r + rectCloseBtn.Height();

			if (rectCloseBtn.r < rectClient.r - 1)
			{
				rectCloseBtn.OffsetRect (rectClient.r - 1 - rectCloseBtn.r, 0);
			}
...
CBCGPTasksPane::CBCGPTasksPane(): CBCGPDockingControlBar(), m_nMaxHistory (10)
{
	...
	m_iScrollBtnHeight = CBCGPMenuImages::Size().h + 2 * iBorderSize_tp;
	m_iScrollMode = 0;
...
int CBCGPTasksPane::ReposTasks (BOOL bCalcHeightOnly/* = FALSE*/)
{
	...
	if ((m_rectTasks.t < 0) || (m_rectTasks.b <= m_rectTasks.t) ||
		(m_rectTasks.l < 0) || (m_rectTasks.r <= m_rectTasks.l))
	{
	...
	m_nRowHeight = max (tm.tmHeight, m_sizeIcon.h);
	m_nAnimGroupExtraHeight = 0;

	int y = rectTasks.t - m_nVertScrollOffset * m_nRowHeight;
	...
	for (pos = m_lstTaskGroups.GetHeadPosition(); pos != NULL;)
	{
		...
		if (!pGroup->m_strName.IsEmpty())
		{
			...
			if (IsToolBox())
			{
				...
				nCaptionHeight = max( sizeText.h + nVOffset, nHeight );
		}
		else
		{
			nCaptionHeight = 0;
		...
		if (pGroup->m_hIcon != NULL && 
			(pGroup->m_sizeIcon.w < rectTasks.Width() - nCaptionHeight))
		{
			if (nCaptionHeight < pGroup->m_sizeIcon.h)
			{
				y += pGroup->m_sizeIcon.h - nCaptionHeight;
			}
		}

		if (!bCalcHeightOnly)
		{
				pGroup->m_rect = CRect (rectTasks.l, y, 
					rectTasks.r, y + nCaptionHeight);
		}
		...
		if (m_bCanCollapse && pGroup->m_bIsCollapsed && !pGroup->m_strName.IsEmpty() &&
			!(m_bAnimationEnabled && pGroup == m_pAnimatedGroup && m_sizeAnim.h > 0 && !bCalcHeightOnly))
		{
			if (!bCalcHeightOnly)
			{
				// ---------------------
				...
				// ---------------------
				pGroup->m_rectGroup = CRect (rectTasks.l, yGroup - 1, 
					rectTasks.r, yGroup - 1);
				...
				for (POSITION posTask = pGroup->m_lstTasks.GetHeadPosition(); posTask != NULL;)
				{
					...
					else if (IsToolBox())
					{
						...
						rectChildWnd.b = y + (pTask->m_bVisible ? pTask->m_nWindowHeight : 0);
						rectChildWnd.t = max (m_rectTasks.t + 1, y);
						rectChildWnd.b = min (m_rectTasks.b, rectChildWnd.b);
						...
						{
							rectChildWnd.DeflateRect (sizeGroupBorders.w, 0);
						}
		...
		else	// not collapsed
		{
			...
			for (POSITION posTask = pGroup->m_lstTasks.GetHeadPosition(); posTask != NULL;)
			{
				...
				if (pTask->m_hwndTask == NULL)
				{
						if (pTask->m_bVisible)
						{
							...
							if ((pTask->m_uiCommandID == 0) ? m_bWrapLabels : m_bWrapTasks)
							{
								...
								cy = max (cy, m_sizeIcon.h);
								rectTask.b = rectTask.t + cy;
							...
							else
							{
								...
								dc.SelectObject (pFontOld);
								int cy = max (sizeText.h, m_sizeIcon.h);
								...
								{
									pTask->m_rect = CRect (
										rectTasks.l + nTaskHOffset, 
										y, 
										rectTasks.l + sizeText.w + m_sizeIcon.w + nTaskHOffset + nIconHOffset, 
										y + cy);
									pTask->m_rect.r = max(pTask->m_rect.l, rectTasks.r - nTaskHOffset);
								}
				...
				if (!bCalcHeightOnly)
				{
					...
					{
						if (y > yGroup + m_sizeAnim.h)
						{
							int nSave = y;
							y = yGroup + max (0, m_sizeAnim.h);
							pTask->m_rect.b = max (pTask->m_rect.t, min (pTask->m_rect.b, y - 1));
			...
			if (!bCalcHeightOnly)
			{
				pGroup->m_rectGroup = CRect (rectTasks.l, yGroup, rectTasks.r, y);
			}
		}
	...
	if (!bCalcHeightOnly)
	{
		...
		if (pLastGroup != NULL)
		{
				...
				m_nVertScrollTotal == 0)
			{
				...
				for (POSITION posTask = pLastGroup->m_lstTasks.GetTailPosition(); posTask != NULL;)
				{
					...
					if (pTask->m_bVisible)
					{
						nOffset = rectTasks.b - pLastGroup->m_rectGroup.b;
						break;
					}
				}
		...
		for (pos = m_lstTaskGroups.GetHeadPosition(); pos != NULL;)
		{
			...
			if (pGroup->m_pPage == pActivePage)
			{
				...
				BOOL bAnimating = m_bAnimationEnabled && pGroup == m_pAnimatedGroup && m_sizeAnim.h > 0;
				for (POSITION posTask = pGroup->m_lstTasks.GetHeadPosition(); posTask != NULL;)
				{
					CBCGPTask* pTask = (CBCGPTask*) pGroup->m_lstTasks.GetNext (posTask);
					ASSERT_VALID (pTask);
...
void CBCGPTasksPane::OnDrawTasks (CDC* pDC, CRect /*rectWorkArea*/)
{
	...
	for (POSITION pos = m_lstTaskGroups.GetHeadPosition(); pos != NULL; )
	{
		...
		if (!pGroup->m_bIsCollapsed || pGroup->m_strName.IsEmpty() ||
			(m_bAnimationEnabled && pGroup == m_pAnimatedGroup && m_sizeAnim.h > 0))
		{
			...
			{
				CSize sizeGroupBorders = GetTasksGroupBorders();
				if (sizeGroupBorders.w > 0 || sizeGroupBorders.h > 0)
				{
	...
	if (m_bUseScrollButtons)
	{
		...
		{	
			CBCGPVisualManager::GetInstance()->OnDrawScrollButtons(pDC, m_rectScrollUp,
				iBorderSize_tp, CBCGPMenuImages::IdArowUp, m_iScrollMode < 0);
		}

		if (IsScrollDnAvailable())
		{
			CBCGPVisualManager::GetInstance()->OnDrawScrollButtons(pDC, m_rectScrollDn,
				iBorderSize_tp, CBCGPMenuImages::IdArowDown, m_iScrollMode > 0);
		}
	}
}
//****
...
void CBCGPTasksPane::SetIconsList (HIMAGELIST hIcons)
{
	...
	else
	{
		...
		::ImageList_GetIconSize (hIcons, (int*) &m_sizeIcon.w, (int*) &m_sizeIcon.h);
	}
...
int CBCGPTasksPane::AddGroup (int nPageIdx, LPCTSTR lpszGroupName, BOOL bBottomLocation/* = FALSE*/, 
							 BOOL bSpecial/* = FALSE*/, CBitmap* hIcon/* = NULL*/)
{
	ASSERT (nPageIdx >= 0);
	ASSERT (nPageIdx <= m_lstTasksPanes.GetCount()-1);
...
void CBCGPTasksPane::CreateFonts()
{
	...
	if (m_fontBoldUnderline.IsValid())
	{
		m_fontBoldUnderline.DeleteObject();
	}
	if (m_fontUnderline.IsValid())
	{
...
void CBCGPTasksPane::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if (m_bUseScrollButtons)
	{
		...
		if (m_iScrollMode != 0)
		{
			SetTimer (iScrollTimerId_tp, m_iScrollTimerDuration, NULL);
			return;
		}
	}
...
void CBCGPTasksPane::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if (bIsGroupCaptionClick)
	{
		...
		if (m_bAnimationEnabled)
		{
			...
			SetTimer (iAnimTimerId_tp, m_iAnimTimerDuration, NULL);
...
void CBCGPTasksPane::SetScrollSizes()
{
	...
	else
	{
		...
		if (m_bUseScrollButtons)
		{
			nPageHeight -= m_iScrollBtnHeight + iBorderSize_tp;
		}
...
void CBCGPTasksPane::AdjustScroll()
{
	...
	if ((m_bUseNavigationToolbar || ForceShowNavToolbar()) && bMultiPage)
	{
		int nToolbarHeight = m_wndToolBar.CalcFixedLayout (FALSE, TRUE).h;

		m_rectToolbar = rectClient;
		m_rectToolbar.b = m_rectToolbar.t + nToolbarHeight;

		rectClient.t += m_rectToolbar.Height();

		m_wndToolBar.SetWindowPos (NULL, m_rectToolbar.l, m_rectToolbar.t, 
	...
	if (!m_bUseScrollButtons && m_nVertScrollTotal > 0)
	{
		...
		m_rectTasks.r -= cxScroll;

		m_wndScrollVert.SetWindowPos (NULL, rectClient.r - cxScroll, rectClient.t,
	...
	if (m_bUseScrollButtons)
	{
		if (IsScrollUpAvailable())
		{
			...
			m_rectScrollUp.t += iBorderSize_tp;
			m_rectScrollUp.b = m_rectScrollUp.t + m_iScrollBtnHeight;

			rectClient.t += m_iScrollBtnHeight + iBorderSize_tp;
		}
		...
		if (IsScrollDnAvailable())
		{
			...
			rectClient.b -= m_iScrollBtnHeight + iBorderSize_tp;
		}
...
//****
void CBCGPTasksPane::OnPaint(CDC* pDC) 
{
...
void CBCGPTasksPane::OnTimer(UINT_PTR nIDEvent) 
{
	...
	case iAnimTimerId_tp:
		if (m_pAnimatedGroup != NULL && m_nRowHeight != 0)
		{
			...
			int nAnimatedGroupHeight = m_sizeAnim.h + m_nAnimGroupExtraHeight;
			...
			{
				m_sizeAnim.h -= nSteps * m_nRowHeight;
			}
			else // expanding
			{
				m_sizeAnim.h += nSteps * m_nRowHeight;
			...
			rectUpdate.t = m_pAnimatedGroup->m_rect.t - 1;
			int nSaveTop = rectUpdate.t;
			...
			rectUpdate.t = min (m_pAnimatedGroup->m_rect.t - 1, nSaveTop);
			InvalidateRect (rectUpdate);

			RedrawWindow (NULL, NULL, RDW_ERASE);

			// stop rule:
			if (m_pAnimatedGroup->m_bIsCollapsed && m_sizeAnim.h < 0 ||
				!m_pAnimatedGroup->m_bIsCollapsed && m_sizeAnim.h > m_pAnimatedGroup->m_rectGroup.Height())
			{
				m_pAnimatedGroup = NULL;
				m_sizeAnim = CSize (0, 0);
			}
		...
		{
			KillTimer (iAnimTimerId_tp);
			m_pAnimatedGroup = NULL;
		}
		break;

	case iScrollTimerId_tp:
		{
			CPoint point;
			::GetCursorPos (&point);
...
{
	SendMessage(WM_SETMESSAGESTRING,
...
BOOL CBCGPTasksPane::CreateNavigationToolbar()
{
	...
	if (m_uiToolbarBmpRes == 0)
	{
		...
		if (dblImageScale != 1.)
		{
			sizeNavButton = CSize ((int)(.5 + dblImageScale * sizeNavButton.w), (int)(.5 + dblImageScale * sizeNavButton.h));
		}
