...
static const int iDefaultSize_cbdt = 100;
static const int iHorzMargin_cbdt = 3;
static const int iVertMargin_cbdt = 3;
			...
			CBCGPToolbarButton (uiId, iImage)
{
	...
	m_iWidth = (iWidth == 0) ? iDefaultSize_cbdt : iWidth;
...
//****
ISIZE CBCGPToolbarDateTimeCtrl::OnCalculateSize (CDC* pDC, const CSize& sizeDefault, BOOL bHorz)
{
	if(!IsVisible())
	{
		return CSize(0,0);
...
void CBCGPToolbarDateTimeCtrl::OnMove()
{
	...
	m_pWndDateTime->SetWindowPos (NULL, 
		m_rect.l + iHorzMargin_cbdt, 
		m_rect.t + (m_rect.Height() - m_sizeText.h - rectDateTime.Height()) / 2,
		m_rect.Width() - 2 * iHorzMargin_cbdt, 
		globalData.GetTextHeight() + 2 * iVertMargin_cbdt,
...
void CBCGPToolbarDateTimeCtrl::OnSize (int iSize)
{
	...
	{
		m_pWndDateTime->SetWindowPos (NULL, 
			m_rect.l + iHorzMargin_cbdt, m_rect.t,
			m_rect.Width() - 2 * iHorzMargin_cbdt, 
			globalData.GetTextHeight() + 2 * iVertMargin_cbdt,
			SWP_NOZORDER | SWP_NOACTIVATE);
...
void CBCGPToolbarDateTimeCtrl::OnChangeParentWnd (CWnd* pWndParent)
{
	...
	rect.b = rect.t + globalData.GetTextHeight() + 2 * iVertMargin_cbdt;
...
void CBCGPToolbarDateTimeCtrl::AdjustRect()
{
	...
	m_rect.InflateRect (iHorzMargin_cbdt, iVertMargin_cbdt);
}
//****
						...
						BOOL bDrawBorder, BOOL bGrayDisabledButtons)
{
	...
	else if ((m_bTextBelow && bHorz) && !m_strText.IsEmpty())
	{
		...
		CRect rectText;
		rectText.l = (rect.l + rect.r - m_sizeText.w) / 2;
		rectText.r = (rect.l + rect.r + m_sizeText.w) / 2;
		rectText.t = rect.b + rect.t;
		rectText.b = rectText.t + m_sizeText.h;
	...
	CDC* pDC, const CRect& rect, BOOL bSelected)
{
	...
	rectDateTime.l = rectDateTime.r - iDateTimeWidth;
	...
	rectBtn.l = rectBtn.r - rectBtn.Height();
