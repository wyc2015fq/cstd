...
void CBCGPSDPlaceMarkerWnd::ShowAt (CRect rect)
{
    if (m_bTabbed || m_rectLast != rect)
    {
        ...
        SetWindowPos (&CWnd::wndTop, rect.l, rect.t, rect.Width(), rect.Height(),
...
void CBCGPSDPlaceMarkerWnd::ShowTabbedAt (CRect rect, CRect rectTab)
{
    if (!m_bTabbed || m_rectLast != rect || m_rectTab != rectTab)
    {
		...
		{
			rgnTab.CreateRectRgn (rectTab.l, 0, rectTab.Width(), rectTab.Height());
		}
		else
		{
		...
		if (CBCGPTabbedControlBar::m_bTabsAlwaysTop)
		{
			SetWindowPos (&CWnd::wndTop, rect.l, rectTab.t, 
				rect.Width(), rect.Height() + m_rectTab.Height(),
				SWP_NOACTIVATE | SWP_SHOWWINDOW | SWP_NOREDRAW);
		}
		else
		{
			SetWindowPos (&CWnd::wndTop, rect.l, rect.t, 
				rect.Width(), rect.Height() + m_rectTab.Height(),
				SWP_NOACTIVATE | SWP_SHOWWINDOW | SWP_NOREDRAW);
		}
		m_bShown = TRUE;
...
void CBCGPSDPlaceMarkerWnd::OnPaint(CDC* pDC) 
{
	...
	COLORREF colorFill = m_bUseThemeColorInShading ?
		globalData.clrActiveCaption : _RGB (47, 103, 190);
    ...
    {
		CBrush brFill (CBCGPDrawManager::PixelAlpha (_RGB (
			255 - GetRV (colorFill), 
			255 - GetGV (colorFill), 
			255 - GetBV (colorFill)), 
			50));
