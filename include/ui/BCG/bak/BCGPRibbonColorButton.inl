...
class CBCGPRibbonColorMenuButton : public CBCGPRibbonButton
{
	...
	virtual void OnDraw (CDC* pDC)
	{
		...
		const int cxImageBar = CBCGPToolBar::GetMenuImageSize().w + 
		...
		rectText.l += cxImageBar + TEXT_MARGIN;
		rectText.DeflateRect (m_szMargin.w, m_szMargin.w);
		...
		if (m_nType == nMenuButtonOther)
		{
			...
			rectImage.r = rectImage.l + cxImageBar;
			...
			if (globalData.m_hiconColors == NULL)
			{
				...
				globalData.m_hiconColors = (CBitmap*) ::LoadImage (
			...
			::DrawIconEx (pDC, 
				rectImage.l + (rectImage.Width() - nIconSize) / 2, 
				rectImage.t + (rectImage.Height() - nIconSize) / 2,
		...
		else if (m_nType == nMenuButtonAuto)
		{
			...
			rectColorBox.r = rectColorBox.l + cxImageBar;
			rectColorBox.DeflateRect (2, 2);

			int nBoxSize = min (rectColorBox.Width(), rectColorBox.Height());

			rectColorBox = CRect (CPoint (
				rectColorBox.l + (rectColorBox.Width() - nBoxSize) / 2,
				rectColorBox.t + (rectColorBox.Height() - nBoxSize) / 2),
...
void CBCGPRibbonColorButton::DrawImage (CDC* pDC, RibbonImageType type, CRect rectImage)
{
	...
	rectColor.t = rectColor.b - nColorHeight + 1;
	...
	{
		CBrush br (PALETTERGB(	GetRV (color),
								GetGV (color), 
								GetBV (color)));
...
void CBCGPRibbonColorButton::OnShowPopupMenu()
{
	...
	{
		m_imagesPalette.SetImageSize (CSize (m_sizeBox.w, m_sizeBox.h - 3));
	}
	else
	{
...
void CBCGPRibbonColorButton::SetColorBoxSize (CSize sizeBox)
{
	...
	if (globalData.GetRibbonImageScale() != 1.)
	{
		sizeBox.w = (int) (.5 + globalData.GetRibbonImageScale() * sizeBox.w);
		sizeBox.h = (int) (.5 + globalData.GetRibbonImageScale() * sizeBox.h);
	}
	...
	if (color != (COLORREF)-1)
	{
		...
		if (!CBCGPColorBar::m_ColorNames.Lookup (color, str))
		{
			str.Format (_T("Hex={%02X,%02X,%02X}"), 
				GetRV (color), GetGV (color), GetBV (color));
		}
