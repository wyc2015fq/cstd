...
HWND _bcgpWindowFromPoint(HWND hWnd, IPOINT pt);
...
BOOL CBCGPReBar::_AddBar(CWnd* pBar, REBARBANDINFO* pRBBI)
{
	...
	pRBBI->cxMinChild = size.w;
	pRBBI->cyMinChild = size.h;
...
CSize CBCGPReBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
	...
	if (!rectBound.IsRectEmpty())
	{
		...
		rectBound.r -= rect.Width();
		rectBound.b -= rect.Height();
	}
	bStretch = 1;
...
void CBCGPReBar::OnPaint(CDC* pDC)
{
	Default();
}
