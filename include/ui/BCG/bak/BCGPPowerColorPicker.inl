...
class CBCGPCellObj : public CObject
{
				...
				const long lDelta)
	{
		...
		//-----------------------------------------------
		long lRed = AlignColor (GetRV (color), lDelta);
		long lGreen = AlignColor (GetGV (color), lDelta);
		long lBlue = AlignColor (GetBV (color), lDelta);

		m_CellColor = _RGB (lRed, lGreen, lBlue);
	...
	void GetPoints (int x, int y, int nCellWidth, IPOINT* pptArray)
	{
	...
	void DrawSelected (CDC* pDC)
	{
		...
		IPOINT ptArrayTwo[CELL_EDGES];
		GetPoints(m_x, m_y - 1, m_nCellWidth + 2, ptArrayTwo);
		
		rgnTwo.CreatePolygonRgn((IPOINT*)&ptArrayTwo, CELL_EDGES, ALTERNATE);
		pDC->FrameRgn(&rgnTwo, pBrWhite, 2, 2);
		
		IPOINT ptArrayThree[CELL_EDGES];
		GetPoints(m_x, m_y, m_nCellWidth + 2, ptArrayThree);
		
		rgnThree.CreatePolygonRgn((IPOINT*)&ptArrayThree, CELL_EDGES, ALTERNATE);
		pDC->FrameRgn(&rgnThree, pBrBlack, 1, 1);
		
		IPOINT ptArrayOne[CELL_EDGES];
		GetPoints(m_x, m_y, m_nCellWidth - 1, ptArrayOne);
		
		rgnOne.CreatePolygonRgn((IPOINT*)&ptArrayOne, CELL_EDGES, ALTERNATE);
		pDC->FrameRgn(&rgnOne, pBrBlack, 1, 1);
	}

	IPOINT		m_CellPoints [CELL_EDGES];
...
void CBCGPColorPickerCtrl::CreateHexagon()
{
	...
	int x = (rectClient.l + rectClient.r) / 2;
	int y = (rectClient.t + rectClient.b) / 2;
...
void CBCGPColorPickerCtrl::DrawPicker (CDC* pDC)
{
	...
	if (m_bmpPicker.GetSafeHandle() == NULL)
	{
		...
		if (dcMem.CreateCompatibleDC (pDC) &&
			m_bmpPicker.CreateCompatibleBitmap (pDC, szColorPicker.w, szColorPicker.h))
		{
			CBitmap* pOldBmp = dcMem.SelectObject (&m_bmpPicker);

			int nStep = (globalData.m_nBitsPerPixel > 8) ? 1 : 4;

			for (int i= 0;i<szColorPicker.h;i += nStep)
			{
				for(int j=0;j<szColorPicker.w;j += nStep)
				{
					CPoint pt (j, szColorPicker.h - i - nStep);
					COLORREF color = CBCGPDrawManager::HLStoRGB_ONE((double)j/(double)szColorPicker.w, DEFAULT_LUMINANCE, (double)i/(double)szColorPicker.h);
...
void CBCGPColorPickerCtrl :: DrawLuminanceBar(CDC* pDC)
{
	...
	for (int y = rectClient.t; y <= rectClient.b; y ++)
	{
...
void CBCGPColorPickerCtrl::DrawCursor (CDC* pDC, const CRect& rect)
{
	...
	if (m_COLORTYPE == PICKER)
	{
		...
		pDC->FillSolidRect((rect.l + nHalfSize) - 1, rect.t, 3, 5, colorFocus);		// Top
		pDC->FillSolidRect((rect.l + nHalfSize) - 1, rect.b - 5, 3, 5, colorFocus);	// Bottom
		pDC->FillSolidRect(rect.l, (rect.t + nHalfSize) - 1, 5, 3, colorFocus);		// Left
		pDC->FillSolidRect(rect.r - 5, (rect.t + nHalfSize) - 1, 5, 3, colorFocus);	// Right
	}
...
void CBCGPColorPickerCtrl::OnMouseMove (UINT nFlags, CPoint point) 
{
	...
	point.x = min (max (rectClient.l, point.x), rectClient.r);
	point.y = min (max (rectClient.t, point.y), rectClient.b);
...
void CBCGPColorPickerCtrl::DrawItem (LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (m_bMemDC)
	{
		...
		//-------------------------------------- 
		pDCDraw->BitBlt (rectClip.l, rectClip.t, rectClip.Width(), rectClip.Height(),
...
double CBCGPColorPickerCtrl::LumFromPoint (int nY)
{
	...
	nY = min (max (rectClient.t, nY), rectClient.b);
	return ((double) rectClient.b - nY) / rectClient.Height();
}
...
int CBCGPColorPickerCtrl::PointFromLum (double dblLum)
{
	...
	rectClient.DeflateRect (0, DEFAULT_OFFSET_OF_LUMINANCE_BAR);
	return rectClient.t + (int) ((1. - dblLum) * rectClient.Height());
}
...
CPoint CBCGPColorPickerCtrl::GetCursorPos()
{
	...
	{
	case LUMINANCE:
		point =  CPoint (rectClient.l + m_nLumBarWidth + 6, 
...
CRect CBCGPColorPickerCtrl::GetCursorRect()
{
	...
	case PICKERH:
		GetClientRect (rect);
		rect.l = GetCursorPos().x - PICKER_CURSOR_SIZEH / 2;
		rect.r = rect.l + PICKER_CURSOR_SIZEH;
		return rect;
