...
public:
	...
	void GetSubItems (CList<CBCGPGridRow*>& lst, BOOL bRecursive = FALSE);
...
protected:
	...
	void AddTerminalItem (CList<CBCGPGridRow*>& lstItems);
	void AddGroupedItem (CList<CBCGPGridRow*>& lstItems);
	int InsertAutoGroupBefore (
				CList<CBCGPGridRow*>& lstItems, 
				int& posInsertBefore, int posFirstItemInGroup, int nParentLevel,
...
protected:
	CArray<CBCGPGridItem*>	m_arrRowItems;	// Items in this row
	...
	CList<CBCGPGridRow*>	m_lstSubItems;	// Sub-Items list
...
protected:
	// Attributes:
	CList<CBCGPGridColumnsItem*>	m_lstColumns;
	CMap<int, int> m_mapSortColumn;	// sorted columns
	CList<int> m_lstGroupingColumns;		// grouping columns
	CArray<int> m_arrColumnOrder;			// groups order
...
typedef CArray<CBCGPGridRow*> CachePageArray;
...
protected:
	CList<CBCGPGridCachePageInfo>	m_lstCache;
};
...
protected:
	...
	CList<CBCGPGridRow*>		m_lstItems;			// List of top-level items
	CList<CBCGPGridRow*>		m_lstTerminalItems;	// List of terminal items
	CList<CBCGPGridRow*>		m_lstGroupedItems;	// List of grouped items
	CList<CBCGPGridRow*>		m_lstAutoGroups;
	CArray <CBCGPGridPage>	m_arrPages;			// Pages, used when printing

	CMap<int, CRuntimeClass*>			m_mapItemsRTC;			// Default item class by column
	CRuntimeClass*	m_pDefaultItemRTC;		// Default item class

	CList<CBCGPGridRange*>	m_lstSel;		// Current selection
	...
	BOOL		m_bHeaderSelecting;		// Is selecting by header?
	...
	static CBitmap* m_hcurDeleteColumn;
	static CBitmap* m_hcurNoDropColumn;
	...
	// Merge cells
	//------------
	CList <CBCGPGridItem*>	m_lstMergedItemsToUpdate; // used in UpdateMergedItems()
...
private:
	CBCGPGridItemID		m_idCur;		// Cached index for currently processing item
	CList<CBCGPGridRow*>		m_lstOldAutoGroups;	// used in ReposItems()
...
public:
	...
	void GetSelectedItems (CList <CBCGPGridItem*> &lstSelected);
...
// Overrides
	...
	public:
	virtual BOOL Create(DWORD dwStyle, const IRECT& rect, CWnd* pParentWnd, UINT nID);
...
public:
	...
	virtual int HitTestGroupByBox (CPoint point, IRECT* lprectItem = NULL);
	virtual int GetGroupByBoxDropIndex (CPoint point, LPPOINT lpptDrop = NULL);

	virtual CBCGPGridRow* HitTestRowHeader (CPoint point, IRECT* lprectItem = NULL);
...
protected:
	...
	int GetMergedItemsInRange (const CBCGPGridRange& range, CList <CBCGPGridRange> &lstRanges);
	void DoGetMergedItemsInRange (CBCGPGridRow* pRow, const CBCGPGridRange& range, CList <CBCGPGridRange> &lstRanges);
