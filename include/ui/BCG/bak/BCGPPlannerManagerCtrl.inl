...
template <class TYPE>
void CopyList1(const TYPE& lstSrc, TYPE& lstDst)
{
	lstDst.RemoveAll();
	...
	, m_bUseMultiResourceDefault (FALSE)
{
...

	{
		...
		if (bmp.LoadBitmap (IDB_BCGBARRES_PLANNER_APP_UPDOWN))
		{
			...
			m_szUpDown.w = bm.bmWidth / 2;
			m_szUpDown.h = bm.bmHeight;

			m_ilUpDown.Create (m_szUpDown.w, m_szUpDown.h, ILC_COLOR | ILC_MASK, 2, 0);
			m_ilUpDown.Add (&bmp, _RGB(255, 0, 255));
		}
	}
...
void CBCGPPlannerManagerCtrl::SetImages (LPCTSTR szResID, int cxImage, COLORREF clrTransparent)
{
	...
	CBitmap* hBmp = (CBitmap*)::LoadImage (AfxFindResourceHandle (RT_BITMAP, szResID), 
	...
	if (bmp.IsValid())
	{
		...
		s_ImageSize.w = cxImage;
		s_ImageSize.h = bm.bmHeight;

		int nCount = bm.bmWidth / cxImage;

		s_ImageList.Create (s_ImageSize.w, s_ImageSize.h, nFlags, nCount, 0);
...
BOOL CBCGPPlannerManagerCtrl::Create(DWORD dwStyle, const IRECT& rect, CWnd* pParentWnd, UINT nID) 
{
	return CWnd::Create (	
...
void CBCGPPlannerManagerCtrl::OnDraw (CDC* pDC)
{
	...
	rectClient.r = pView->m_rectApps.r;
...
void CBCGPPlannerManagerCtrl::OnPaint(CDC* pDC) 
{
	...
	if (bMemDC)
	{
		dc.BitBlt (rectClient.l, rectClient.t, rectClient.Width(), rectClient.Height(),
			&memDC, 0, 0, SRCCOPY);
	}
...
HFONT CBCGPPlannerManagerCtrl::SetCurrFont (CDC* pDC)
{
	...
	return (HFONT) ::SelectObject (pDC, pFont->GetSafeHandle() );
}
//****
...
CBitmap* CBCGPPlannerManagerCtrl::GetUpDownIcon(int nType)
{
	ASSERT (0 <= nType && nType <= 1);

	CBitmap* hIcon = m_ilUpDown.ExtractIcon (nType);
	return hIcon;
}
...
BOOL CBCGPPlannerManagerCtrl::CanStartDragDrop() const
{
		 ...
		 GetSelectedAppointmentsCount() > 0)
	{
		...
		rt.OffsetRect (CPoint (-czScroll.w / 2, -czScroll.h / 2));
