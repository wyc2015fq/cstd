...
void CBCGPCalendar::OnPaint(CDC* pDC) 
{
	...
	if (GetRV (m_Colors.clrCaptionText) > 192 &&
		GetGV (m_Colors.clrCaptionText) > 192 &&
		GetBV (m_Colors.clrCaptionText) > 192)
	{
									  ...
									  int& xStart, int& nDaysInPrevMonth) const
{
	...
	for (int i = 0; i < 42; i++, x++)
	{
		...
		{
			rects [i] = CRect (
				CPoint (rect.l + x * m_sizeBox.w,
						rect.t + y * m_sizeBox.h),
				m_sizeBox);
		}
	}
}
...
void CBCGPCalendar::OnDrawMonth (CDC* pDC, CRect rect, int nMonthIndex)
{
	...
	rectCaption.b = rectCaption.t + m_sizeBox.h + 2;
	...
	else
	{
		...
		rectCaption.l = rectClient.l + m_nHorzMargin;
		rectCaption.r = rectClient.r - m_nHorzMargin;
		rectCaption.t += m_nVertMargin;
	}
	...
	if (m_bGradientFillCaption)
	{
		...
		if (GetRV (m_Colors.clrCaption) > 192 &&
			GetGV (m_Colors.clrCaption) > 192 &&
			GetBV (m_Colors.clrCaption) > 192)
		{
	...
	rect.DeflateRect (m_nDaysHorzMarginLeft, 0, m_nDaysHorzMarginRight, 0);
	rect.t = rectCaption.b;

	//----------------
	// Draw week days:
	//----------------
	CRect rectWeeks = rect;
	rectWeeks.b = rectWeeks.t + m_sizeBox.h + 1;

	for (i = 0; i < DAYS_IN_WEEK; i++)
	{
		CRect rectWeekDay = rectWeeks; 
		rectWeekDay.l = rectWeeks.l + m_sizeBox.w * i - 1;
		rectWeekDay.r = rectWeekDay.l + m_sizeBox.w;
	...
	int yLine = rectWeeks.b - 1;
	const int xLineLeft = rect.l;
	const int xLineRight = rect.r + 2;

	pDC->MoveTo (xLineLeft, yLine);
	pDC->LineTo (xLineRight, yLine);

	rect.t = rectWeeks.b + 2;
	...
	{
		rectWeekNums = rect;
		rectWeekNums.r = rect.l;
		rectWeekNums.l = rectWeekNums.r - m_sizeBox.w;
	}
	...
	for (i = 0; i < 42; i++)
	{
		...
		if (rectDay.b > yLineBottom)
		{
			nWeeks++;
		}

		yLineBottom = rectDay.b;
	}
	...
	if (m_bWeekNumbers)
	{
		...
		for (i = 0; i < nWeeks; i++)
		{
			...
			rectNum.t += m_sizeBox.h * i;
			rectNum.b = rectNum.t + m_sizeBox.h;
			...
			if (!m_bSingleMonth)
			{
				pDC->LineTo (xLineLeft - 1, rectNum.b);
			}
		}
...
BOOL CBCGPCalendar::Create(DWORD dwStyle, const IRECT& rect, CWnd* pParentWnd, UINT nID) 
{
...
void CBCGPCalendar::RecalcLayout (BOOL bRedraw)
{
	...
	if (m_bTodayButton)
	{
		...
		int xToday = rectClient.CenterPoint().x - sizeToday.w / 2;
		int yToday = rectClient.b - sizeToday.h - 4;

		m_btnToday.SetWindowPos (NULL, xToday, yToday, sizeToday.w, sizeToday.h,
			SWP_NOZORDER | SWP_NOACTIVATE);

		rectClient.b = yToday - 4;
	...
	for (i = 0; i < 10; i ++)
	{
		...
		iMaxDigitWidth = max (iMaxDigitWidth, sizeDigit.w);
		iMaxDigitHeight = max (iMaxDigitHeight, sizeDigit.h);
	}

	dc.SelectObject(globalData.fontSmall);

	CString strWeekNum = _T("99");
	m_nDaysHorzMarginLeft = dc.GetTextExtent (strWeekNum).w + iMaxDigitWidth;
	...
	if (m_bSingleMonth)
	{
		m_sizeBox.w = min (3 * m_sizeBox.w / 2,
			(rectClient.Width() - 2 * m_nHorzMargin - m_nDaysHorzMarginLeft - m_nDaysHorzMarginRight) / DAYS_IN_WEEK);

		m_sizeBox.h = min (3 * m_sizeBox.h / 2,
			(rectClient.Height() - 2 * m_nVertMargin) / 8);
	}

	m_sizeCalendar = CSize (
		DAYS_IN_WEEK * m_sizeBox.w + m_nDaysHorzMarginLeft + m_nDaysHorzMarginRight,
		8 * m_sizeBox.h + 5);

	m_nCalendarsInRow = m_bSingleMonth ? 1 : max (1, rectClient.Width() / m_sizeCalendar.w);

	int nCalendarsInColumn = m_bSingleMonth ? 1 : max (1, rectClient.Height() / m_sizeCalendar.h);
	...
	if (m_bTodayButton)
	{
		if (m_sizeCalendar.h > rectClient.Height())
		{
			m_btnToday.ShowWindow (SW_HIDE);
			m_btnToday.EnableWindow (FALSE);
		}
	...
	m_rectBtnPrev.r = m_rectBtnPrev.l + m_sizeBox.w;
	m_rectBtnPrev.b = m_rectBtnPrev.t + m_sizeBox.h + 2;

	GetMonthRect (m_nCalendarsInRow - 1, m_rectBtnNext);

	m_rectBtnNext.l = m_rectBtnNext.r - m_sizeBox.w;
	m_rectBtnNext.b = m_rectBtnNext.t + m_sizeBox.h + 2;
...
BOOL CBCGPCalendar::GetMonthRect (int nMonthIndex, CRect& rect) const
{
	...
	const int xMargin = max (0, 
		(rectClient.Width() - m_sizeCalendar.w * m_nCalendarsInRow) / 2);

	const int x = nMonthIndex % m_nCalendarsInRow;
	const int y = nMonthIndex / m_nCalendarsInRow;

	rect = CRect (
		CPoint (rectClient.l + x * m_sizeCalendar.w + xMargin,
				rectClient.t + y * m_sizeCalendar.h),
		m_sizeCalendar);
...
COleDateTime CBCGPCalendar::HitTest (CPoint point, int& nDir, IRECT* lpRect)
{
	nDir = 0;
...
COleDateTime CBCGPCalendar::HitTestWeekNum (CPoint point, BOOL bStart)
{
	for (int nMonthIndex = 0; nMonthIndex < m_nMonths; nMonthIndex++)
	{
		...
		rect.t += m_sizeBox.h * 2 + 4;
		rect.b = rect.t + m_sizeBox.h;

		if (bStart)
		{
			rect.r = rect.l + m_nDaysHorzMarginLeft;
		}
...
int CBCGPCalendar::HitTestMonthName (CPoint point, CRect& rectMonthName)
{
	...
	for (int nMonth = 0; nMonth < m_nMonths; nMonth++)
	{
		...
		rectCaption.DeflateRect (m_sizeBox.w + 2, 0);
...
int CBCGPCalendar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if (GetStyle() & WS_POPUP)
	{
		...
		if (m_pParentBtn != NULL)
		{
			...
			rectWindow = CRect (
				CPoint (rectParent.r - size.w, rectParent.b),
				size);
		}
		...
		if (rectWindow.l < rectScreen.l)
		{
			rectWindow.OffsetRect (rectScreen.l - rectWindow.l, 0);
		}
		else if (rectWindow.r > rectScreen.r)
		{
			rectWindow.OffsetRect (rectScreen.r - rectWindow.r, 0);
		}

		if (rectWindow.t < rectScreen.t)
		{
			rectWindow.OffsetRect (0, rectScreen.t - rectWindow.t);
		}
		else if (rectWindow.b > rectScreen.b)
		{
			rectWindow.OffsetRect (0, rectScreen.b - rectWindow.b);
		}

		SetWindowPos (&wndTop, rectWindow.l, rectWindow.t, 
			size.w, size.h, SWP_SHOWWINDOW);
		SetFocus();
	}
...
BOOL CBCGPCalendarBar::OnClickDate (COleDateTime date)
{
	...
	else
	{
		...
		SendMessage(WM_COMMAND, m_nCommandID);
	}
...
int CBCGPMonthPickerWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...

	{
		...
		for (int i = 0; i < 12; i ++)
		{
			...
			cx = max (cx, sizeText.w);
			cy = max (cy, sizeText.h);
		}
	}
	...
	CSize sizeWindow (cx + sizeYear.w + 20, m_nRowHeight * nVisbleMonths + 2);
	...
	SetWindowPos (NULL, 
		rectWindow.l - sizeWindow.w / 2,
		rectWindow.t - sizeWindow.h / 2,
...
//****
void CBCGPMonthPickerWnd::OnPaint(CDC* pDC) 
{
	...
	rectMonth.b = rectMonth.t + m_nRowHeight;
...
void CBCGPMonthPickerWnd::OnTimer(UINT_PTR nIDEvent) 
{
	...
	if (point.y < rectClient.t)
	{
		PrevMonth();
		RedrawWindow();
		SetTimer (m_nIDTimerEvent, max (50, m_nElapseInterval * m_nRowHeight / (rectClient.t - point.y)), NULL); 
	}
	else if (point.y > rectClient.b)
	{
		NextMonth();
		RedrawWindow();
		SetTimer (m_nIDTimerEvent, max (50, m_nElapseInterval * m_nRowHeight / (point.y - rectClient.b)), NULL);
	}
	else
	{
		SetTimer (m_nIDTimerEvent, m_nElapseInterval, NULL);
