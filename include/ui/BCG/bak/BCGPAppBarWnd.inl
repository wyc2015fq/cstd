...
void CBCGPAppBarWnd::DoQueryPos (CRect& rect)
{
	...
	if (m_bAppRegistered)
	{
		...
		{
			iWidth = rect.r - rect.l;
			rect.t = 0;
			rect.b = rectScreen.Height();
		}
		else
		{
			iHeight = rect.b - rect.t;
			rect.l = 0;
			rect.r = rectScreen.Width();
		}
		...
		{
		case ABE_LEFT:
			rect.r = rect.l + iWidth;
			break;

		case ABE_RIGHT:
			rect.l = rect.r - iWidth;
			break;

		case ABE_TOP:
			rect.b = rect.t + iHeight;
			break;

		case ABE_BOTTOM:
			rect.t = rect.b - iHeight;
			break;
		}
	}
}
...
BOOL CBCGPAppBarWnd::DoSetSide (UINT uSide, BOOL bMove, BOOL bNoSizeMove)
{
	...
	{
	case ABE_LEFT:
		rect.r = rect.l + m_abs.m_auDimsDock [0];
		break;
	case ABE_TOP:
		rect.b = rect.t + m_abs.m_auDimsDock [1];
		break;
	case ABE_RIGHT:
		rect.l = rect.r - m_abs.m_auDimsDock [2];
		break;
	case ABE_BOTTOM:
		rect.t = rect.b - m_abs.m_auDimsDock [3];
		break;
	default:
		ASSERT (FALSE);
		return FALSE;
...
void CBCGPAppBarWnd::OnSizeMove()
{
	...
	if (!m_bAppAutoHiding || !m_bHidden)
	{
		...
		case ABE_TOP:
			...
			break;
		case ABE_BOTTOM:
			rect.t = rect.b - size.h;
			break;
		case ABE_LEFT:
			rect.r = rect.l + size.w;
			break;
		case ABE_RIGHT:
			rect.l = rect.r - size.w;
			break;
		}
...
BOOL CBCGPAppBarWnd::Float()
{
	...
	SetWindowPos (NULL, m_abs.m_rcFloat.l, m_abs.m_rcFloat.t,
...
void CBCGPAppBarWnd::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if (m_bMoving)
	{
		...
		if (uSide != ABE_FLOAT)
		{
			...
			{
				m_rectDrag.t = 0;
				m_rectDrag.b = nCYScreen;
				if (uHorz == ABE_LEFT)
				{
					m_rectDrag.l = 0;
					m_rectDrag.r = m_rectDrag.l + m_abs.m_auDimsDock [0];
					uSide = ABE_LEFT;
				}
				else
				{
					m_rectDrag.r = nCXScreen;
					m_rectDrag.l = m_rectDrag.r - m_abs.m_auDimsDock [2];
					uSide = ABE_RIGHT;
				}
			}
			else if ((m_dwFlags & ABF_ALLOWTOPBOTTOM) != 0)
			{
				...
				{
					m_rectDrag.t = 0;
					m_rectDrag.b = m_rectDrag.t + m_abs.m_auDimsDock [1];
					uSide = ABE_TOP;
				}
				else
				{
					m_rectDrag.b = nCYScreen;
					m_rectDrag.t = m_rectDrag.b - m_abs.m_auDimsDock [3];
					uSide = ABE_BOTTOM;
				}
			}
			else
...
void CBCGPAppBarWnd::OnCancelMode() 
{
	...
	if (m_bMoving)
	{
		...
		{
			m_abs.m_uSide = m_uSidePrev;
			SetWindowPos (NULL, m_rectPrev.l, m_rectPrev.t,
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::SlideWindow(IRECT* prc)
{
	...
	if (m_bAppRegistered)
	{
		...
		if (!m_bDisableAnimation && bFullDragEnabled)
		{
			...
			int dx = (rectNew.r - rectOld.r) + (rectNew.l - rectOld.l);
			int dy = (rectNew.b - rectOld.b) + (rectNew.t - rectOld.t);

			BOOL bEnlarging = 
					(rectNew.b - rectNew.t) > (rectOld.b - rectOld.t) ||
					(rectNew.r - rectNew.l) > (rectOld.r - rectOld.l);

			if (bEnlarging)
			{
				rectOld = rectNew;
				rectOld.OffsetRect (-dx, -dy);
				SetWindowPos(NULL, rectOld.l, rectOld.t,
			...
			{
				int x = rectOld.l + dx * (t - t0) / dt;
				int y = rectOld.t + dy * (t - t0) / dt;
		...
		SetWindowPos(NULL, rectNew.l, rectNew.t,
					 rectNew.r - rectNew.l, rectNew.b - rectNew.t,
					 SWP_NOZORDER | SWP_NOACTIVATE | SWP_DRAWFRAME | SWP_FRAMECHANGED);
	}
}
//---------------------------------------------------------------------------------------------
...
void CBCGPAppBarWnd::GetCaptionRect (CRect& rectCaption) const
{
	...
	rectCaption.SetRect (	rectWnd.l + rectBorderSize.l,
