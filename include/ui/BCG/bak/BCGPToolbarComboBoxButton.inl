...
static const int iDefaultComboHeight_tbcbb = 150;
static const int iDefaultSize = 150;
static const int iHorzMargin_tbcbb = 1;
...
void CBCGPToolbarComboBoxButton::Initialize()
{
	...
	m_nDropDownHeight = iDefaultComboHeight_tbcbb;
	m_bIsHotEdit = FALSE;
	m_uiMenuResID = 0;
	m_bIsRibbon = FALSE;
	m_bIsCtrl = FALSE;
...
//****
ISIZE CBCGPToolbarComboBoxButton::OnCalculateSize (CDC* pDC, const CSize& sizeDefault, BOOL bHorz)
{
	m_bHorz = bHorz;
	...
	if (bHorz)
	{
		...
		return CSize (m_iWidth, cy + m_sizeText.h);
...
void CBCGPToolbarComboBoxButton::OnChangeParentWnd (CWnd* pWndParent)
{
	...
	rect.b = rect.t + m_nDropDownHeight;
...
void CBCGPToolbarComboBoxButton::AdjustRect()
{
	...
	m_pWndCombo->SetWindowPos (NULL,
		m_rect.l + iHorzMargin_tbcbb, m_rect.t,
		m_rect.Width() - 2 * iHorzMargin_tbcbb, m_nDropDownHeight,
	...
	if (m_bFlat)
	{
		...
		m_rectButton.l = m_rectButton.r - CBCGPMenuImages::Size().w * 2;

		m_rectButton.DeflateRect (2, 2);

		m_rect.l = m_rectCombo.l - iHorzMargin_tbcbb;
		m_rect.r = m_rectCombo.r + iHorzMargin_tbcbb;

		if (!m_bTextBelow || m_strText.IsEmpty())
		{
			m_rect.t = m_rectCombo.t;
			m_rect.b = m_rectCombo.b;
		}
		...
		if (m_pWndEdit != NULL)
		{
			...
			m_pWndEdit->SetWindowPos (NULL,
				m_rect.l + iHorzMargin_tbcbb + iBorderOffset, m_rect.t + iBorderOffset,
				m_rect.Width() - 2 * iHorzMargin_tbcbb - m_rectButton.Width() - iBorderOffset - 3,
						...
						BOOL bDrawBorder, BOOL bGrayDisabledButtons)
{
	...
	if ((m_bTextBelow && bHorz) && !m_strText.IsEmpty())
	{
		...
		CRect rectText = rect;
		rectText.t = (m_rectCombo.b + rect.b - m_sizeText.h) / 2;
	...
	CDC* pDC, const CRect& rect, BOOL bSelected)
{
	...
	rectCombo.l = rectCombo.r - nComboWidth;
	...
	rectBtn.l = rectBtn.r - rectBtn.Height() + 2;
...
//****
void CBCGPComboEdit::OnPaint(CDC* pDC) 
{
	CString str;
	GetWindowText (str);
