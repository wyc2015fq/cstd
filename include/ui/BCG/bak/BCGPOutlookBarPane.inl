...
CBCGPOutlookBarPane::CBCGPOutlookBarPane()
{
	...
	m_clrTransparentColor = _RGB (255, 0, 255);
...
//
{
	...
	CBitmap* hBmp = (CBitmap*) ::LoadImage (	
								...
								int iInsertAt)
{
	...
	if (uiImage != 0)
	{
		...
		iImageIndex = AddBitmapImage ((CBitmap*) bmp.GetSafeHandle());
	}

	return InternalAddButton (iImageIndex, lpszLabel, iIdCommand, iInsertAt);
}
//****
BOOL CBCGPOutlookBarPane::AddButton (CBitmap* hBmp, LPCTSTR lpszLabel, UINT iIdCommand, int iInsertAt)
{
	ASSERT (hBmp != NULL);
...
//****
BOOL CBCGPOutlookBarPane::AddButton (CBitmap* hIcon, LPCTSTR lpszLabel, UINT iIdCommand, int iInsertAt,
									 BOOL bAlphaBlend)
{
	ASSERT (hIcon != NULL);
	...
	else
	{
		...
		CBitmap bmp;
		bmp.CreateCompatibleBitmap (&dc, size.w, size.h);

		CBitmap* pOldBmp = dcMem.SelectObject (&bmp);

		if (m_clrTransparentColor != (COLORREF)-1)
		{
			dcMem.FillSolidRect (0, 0, size.w, size.h, m_clrTransparentColor);
		}

		::DrawIconEx (dcMem.GetSafeHdc(), 0, 0, hIcon, size.w, size.h,
			0, NULL, DI_NORMAL);
...
//****
int CBCGPOutlookBarPane::AddBitmapImage (CBitmap* hBitmap)
{
	ASSERT (hBitmap != NULL);
...
void CBCGPOutlookBarPane::AdjustLocations()
{
	...
	CSize sizeDefault = CSize (rectClient.Width() - 2, m_csImage.h);
	...
	int iOffset = rectClient.t - m_iScrollOffset + m_nExtraSpace;

	if (m_iFirstVisibleButton > 0 && 
		sizeBtn.w <= rectClient.Width() - SCROLL_BUTTON_OFFSET && 
		sizeBtn.h <= rectClient.Height() - SCROLL_BUTTON_OFFSET)
	{
		int nAdjButton = SCROLL_BUTTON_OFFSET;

		m_btnUp.SetWindowPos (NULL, 
				rectClient.r - sizeBtn.w - nAdjButton,
				rectClient.t + SCROLL_BUTTON_OFFSET,
				-1, -1,
	...
	for (POSITION pos = m_Buttons.GetHeadPosition(); pos != NULL;)
	{
		...
		sizeButton.w = min (nWidth, sizeButton.w);

		rectButton = CRect (
			CPoint (rectClient.l + (nWidth - sizeButton.w) / 2, iOffset), 
			sizeButton);
		iOffset = rectButton.b + m_nExtraSpace;

		pButton->SetRect (rectButton);
	}

	m_bScrollDown = (iOffset > rectClient.b);
		...
		sizeBtn.h <= rectClient.Height() - SCROLL_BUTTON_OFFSET)
	{
		...
		m_btnDown.SetWindowPos (&wndTop, rectClient.r - sizeBtn.w - nAdjButton,
					rectClient.b - sizeBtn.h - SCROLL_BUTTON_OFFSET,
					-1, -1,
