...
CMap<UINT,UINT,CBitmap*,CBitmap*>	CBCGPTabWnd::m_mapDocIcons;
...
BOOL CBCGPTabWnd::Create (Style style, const IRECT& rect, CWnd* pParentWnd, 
...
//****
void CBCGPTabWnd::OnPaint(CDC* pDC) 
{
	...
	{
	case RESIZE_VERT:
		rectClient.r -= m_rectResize.Width();
		break;
	...
	{
		rectTabs.t = m_rectTabsArea.t;
	}
	else
	{
		rectTabs.b = m_rectTabsArea.b;
	}
	...
	if (nTabBorderSize == 0)
	{
		if (m_location == LOCATION_BOTTOM)
		{
			rectFrame.b = m_rectTabsArea.t + 1;
		}
		else
		{
			rectFrame.t = m_rectTabsArea.b - 1;
		}
	...
	else
	{
		int yLine = m_location == LOCATION_BOTTOM ? 
			m_rectTabsArea.t : m_rectTabsArea.b;

		if (!m_bFlat)
		{
			if (m_location == LOCATION_BOTTOM)
			{
				rectFrame.b = m_rectTabsArea.t;
			}
			else
			{
				rectFrame.t = m_rectTabsArea.b;
			}
		}
		...
		if (m_bFlatFrame)
		{
			...
			if (m_bFlat)
			{
				if (m_location == LOCATION_BOTTOM)
				{
					rectBorder.b = m_rectTabsArea.t + 1;
				}
				else
				{
					rectBorder.t = m_rectTabsArea.b - 1;
				}
			}
			...
			{
				pDC->PatBlt (rectFrame.l, rectFrame.t, nTabBorderSize, rectFrame.Height(), PATCOPY);
				pDC->PatBlt (rectFrame.l, rectFrame.t, rectFrame.Width(), nTabBorderSize, PATCOPY);
				pDC->PatBlt (rectFrame.r - nTabBorderSize - 1, rectFrame.t, nTabBorderSize + 1, rectFrame.Height(), PATCOPY);
				pDC->PatBlt (rectFrame.l, rectFrame.b - nTabBorderSize, rectFrame.Width(), nTabBorderSize, PATCOPY);

				if (m_location == LOCATION_BOTTOM)
				{
					pDC->PatBlt (rectFrame.l, m_rectWndArea.b, rectFrame.Width(), 
						rectFrame.b - m_rectWndArea.b, PATCOPY);
				}
				else
				{
					pDC->PatBlt (rectFrame.l, rectFrame.t, rectFrame.Width(), 
						m_rectWndArea.t - rectFrame.t, PATCOPY);
				}
			}
			...
			if (m_bFlat)
			{
				...
				pDC->MoveTo (rectFrame.l + nTabBorderSize, yLine);
				pDC->LineTo (rectFrame.r - nTabBorderSize, yLine);
			}
			...
			else
			{
				...
				if (!m_bIsOneNoteStyle)
				{
					...
					if (m_location == LOCATION_BOTTOM)
					{
						...
						pDC->MoveTo (rectBorder.l, rectBorder.b - 1);
						pDC->LineTo (xRight, rectBorder.b - 1);
					}
					else
					{
						pDC->SelectObject (&penHiLight);

						pDC->MoveTo (rectBorder.l, rectBorder.t);
						pDC->LineTo (xRight, rectBorder.t);
					}
				}
			}
		}
		else
		{
			if (m_bDrawFrame)
			{
					...
					rectFrame.Width() > 0 && rectFrame.Height() > 0)
				{
					pDC->PatBlt (rectFrame.l, rectFrame.t, nTabBorderSize, rectFrame.Height(), PATCOPY);
					pDC->PatBlt (rectFrame.l, rectFrame.t, rectFrame.Width(), nTabBorderSize, PATCOPY);
					pDC->PatBlt (rectFrame.r - nTabBorderSize, rectFrame.t, nTabBorderSize, rectFrame.Height(), PATCOPY);
					pDC->PatBlt (rectFrame.l, rectFrame.b - nTabBorderSize, rectFrame.Width(), nTabBorderSize, PATCOPY);
	...
	if (bTopEdge)
	{
		...
		pDC->MoveTo (rectClient.l, m_rectTabsArea.b);
		pDC->LineTo (rectClient.l, rectClient.t);
		pDC->LineTo (rectClient.r - 1, rectClient.t);
		pDC->LineTo (rectClient.r - 1, m_rectTabsArea.b);
	}
	...
	if (m_rectTabsArea.Width() > 5 && m_rectTabsArea.Height() > 5)
	{
		...
		if (m_iActiveTab >= 0)
		{
			...
			if (m_bFlat)
			{
				...
				//---------------------------------
				const int xLeft = max (	m_rectTabsArea.l + 1,
										pTabActive->m_rect.l + 1);

				if (pTabActive->m_rect.r > m_rectTabsArea.l + 1)
				{
					...
					if (m_location == LOCATION_BOTTOM)
					{
						pDC->MoveTo (xLeft, pTabActive->m_rect.t);
						pDC->LineTo (pTabActive->m_rect.r, pTabActive->m_rect.t);
					}
					else
					{
						pDC->MoveTo (xLeft, pTabActive->m_rect.b);
						pDC->LineTo (pTabActive->m_rect.r, pTabActive->m_rect.b);
					}
	...
	if (m_bFlat && m_nTabsHorzOffset > 0)
	{
		...
		const int xDivider = m_rectTabsArea.l - 1;

		if (m_location == LOCATION_BOTTOM)
		{
			pDC->MoveTo (xDivider, m_rectTabsArea.t + 1);
			pDC->LineTo (xDivider, m_rectTabsArea.b - 2);
		}
		else
		{
			pDC->MoveTo (xDivider, m_rectTabsArea.b);
			pDC->LineTo (xDivider, m_rectTabsArea.t + 2);
		}
	}
...
void CBCGPTabWnd::AdjustTabs()
{
	...
	//----------------------------------------------
	int x = m_rectTabsArea.l - m_nTabsHorzOffset;
	int i = 0;
	...
	for (i = 0; i < m_iTabsNum; i++)
	{
		...
		if (pTab->m_bVisible)
		{
			pTab->m_nFullWidth = sizeImage.w + TAB_IMAGE_MARGIN +
				(pTab->m_bIconOnly ? 
					0 : dc.GetTextExtent (pTab->m_strText).w) + 2 * TAB_TEXT_MARGIN;
		...
		pTab->m_rect = CRect (CPoint (x, m_rectTabsArea.t),
		...
		if (m_bTabDocumentsMenu && pTab->m_rect.r > m_rectTabsArea.r)
		{
			BOOL bHideTab = TRUE;

			if (i == m_iActiveTab && i == 0)
			{
				int nWidth = m_rectTabsArea.r - pTab->m_rect.l;

				if (nWidth >= nExtraWidth + 2 * TAB_TEXT_MARGIN)
				{
					pTab->m_rect.r = m_rectTabsArea.r;
					bHideTab = FALSE;
				}
			}
		...
		if (m_pToolTip->GetSafeHwnd() != NULL)
		{
			...
			if (pTab->m_rect.l < m_rectTabsArea.l ||
				pTab->m_rect.r > m_rectTabsArea.r)
			{
				bShowTooltip = TRUE;
			}
		...
		if (m_bFlat)
		{
			...
			//--------------------------------------------
			pTab->m_rect.r += m_nTabsHeight / 2;
		}
	}

	if (m_bScroll || x < m_rectTabsArea.r)
	{
	...
	else
	{
		...
		{
			nTabWidth = max (
				m_sizeImage.w + m_rectTabsArea.Height() / 2,
		...
		if (nCutTabsNum > 0)
		{
			...
			for (i = 0; i < m_iTabsNum; i ++)
			{
				...
				if (nTabWidth < sizeImage.w + TAB_IMAGE_MARGIN)
				{
					// Too narrow!
				...
				else
				{
					...
					{
						nCurrTabWidth = sizeImage.w + 2 * CBCGPBaseTabWnd::TAB_TEXT_MARGIN;
					}
				}
	...
	if (m_bActiveTabCloseButton && m_iActiveTab >= 0)
	{
		...
		m_rectCloseButton.l = m_rectCloseButton.r - m_rectCloseButton.Height();
		...
		if (m_pToolTipClose->GetSafeHwnd() != NULL)
		{
			...
			if (m_pToolTip->GetSafeHwnd() != NULL && pActiveTab != NULL)
			{
				...
				{
					rectActiveTabTT.r = m_rectCloseButton.l - 1;
...
void CBCGPTabWnd::Draw3DTab (CDC* pDC, CBCGPTabInfo* pTab, BOOL bActive)
{
	...
	if ((m_bIsOneNoteStyle || m_bIsVS2005Style || m_bLeftRightRounded)
		&& pTab->m_rect.l < m_rectTabsArea.l)
	{
		return;
	}
...
void CBCGPTabWnd::RecalcLayout()
{
	...
	case RESIZE_VERT:
		...
		rectClient.r -= RESIZEBAR_SIZE;
		m_rectResize.l = rectClient.r + 1;
		break;

	case RESIZE_HORIZ:
		m_rectResize = rectClient;
		rectClient.b -= RESIZEBAR_SIZE;
		m_rectResize.t = rectClient.b + 1;
		break;
	...
	if (m_bScroll)
	{
		nScrollBtnWidth = CBCGPMenuImages::Size().w + 4 +
			CBCGPVisualManager::GetInstance()->GetButtonExtraBorder().w;
	...
	if (m_bFlat)
	{
		if (m_location == LOCATION_BOTTOM)
		{
			if (nTabBorderSize > 1)
			{
				m_rectTabsArea.b -= nTabBorderSize - 1;
			}

			m_rectTabsArea.t = m_rectTabsArea.b - nTabsHeight;
		}
		else
		{
			if (nTabBorderSize > 1)
			{
				m_rectTabsArea.t += nTabBorderSize - 1;
			}

			m_rectTabsArea.b = m_rectTabsArea.t + nTabsHeight;
		}

		m_rectTabsArea.l += nButtonsWidth + 1;
		m_rectTabsArea.r--;

		if (m_rectTabsArea.r < m_rectTabsArea.l)
		{
			if (nTabBorderSize > 0)
			{
				m_rectTabsArea.l = rectClient.l + nTabBorderSize + 1;
				m_rectTabsArea.r = rectClient.r - nTabBorderSize - 1;
			}
			else
			{
				m_rectTabsArea.l = rectClient.l;
				m_rectTabsArea.r = rectClient.r;
			}
		}
		...
		ReposButtons (	CPoint (rectClient.l + nTabBorderSize + 1, y), 
	...
	else
	{
		...
		if (m_bScroll)
		{
			m_rectTabsArea.r -= nButtonsWidth;
			...
			// Reposition scroll butons:
			ReposButtons (	
				CPoint (m_rectTabsArea.r + 1, m_rectTabsArea.CenterPoint().y - nScrollBtnWidth / 2),
				CSize (nScrollBtnWidth, nScrollBtnWidth), bHideTabs, nButtonMargin);
		}
	}

	m_rectWndArea = rectClient;
	m_nScrollBarRight = m_rectTabsArea.r - ::GetSystemMetrics (SM_CXVSCROLL);
	...
	if (m_bFlat)
	{
		if (m_location == LOCATION_BOTTOM)
		{
			m_rectWndArea.b = m_rectTabsArea.t;
		...
		{
			m_rectWndArea.t = m_rectTabsArea.b + nTabBorderSize;
	...
	else
	{
		if (m_location == LOCATION_BOTTOM)
		{
			m_rectWndArea.b = m_rectTabsArea.t - nTabBorderSize;
		}
		else
		{
			m_rectWndArea.t = m_rectTabsArea.b + nTabBorderSize;
		...
		for (int i = 0; i < m_iTabsNum; i ++)
		{
			...
			if (pTab->m_bVisible && pTab->m_pWnd->GetSafeHwnd() != NULL)
			{
				pTab->m_pWnd->SetWindowPos (NULL,
					m_rectWndArea.l, m_rectWndArea.t,
					m_rectWndArea.Width(), m_rectWndArea.Height(),
	...
	if (m_location != LOCATION_BOTTOM)
	{
		rcUpdateArea.b = m_rectWndArea.b;
	}
	else
	{
		rcUpdateArea.t = m_rectWndArea.t;
	}
...
void CBCGPTabWnd::AdjustWndScroll()
{
	...
	if (!bHideTabs)
	{
		...
		{
			rectScroll.t++;
			rectScroll.r = m_nScrollBarRight;
			rectScroll.l = rectScroll.r - m_nHorzScrollWidth;
			rectScroll.b -= 2;

			m_rectTabSplitter = rectScroll;
			m_rectTabSplitter.t ++;
			m_rectTabSplitter.r = rectScroll.l;
			m_rectTabSplitter.l = m_rectTabSplitter.r - SPLITTER_WIDTH;

			m_rectTabsArea.r = m_rectTabSplitter.l;
...
void CBCGPTabWnd::StopResize (BOOL bCancel)
{
	if (m_bResize)
	{
		...
		if (!bCancel)
		{
			...
			{
				rectWnd.r = m_rectResizeDrag.r;
			}
			else if (m_ResizeMode == RESIZE_HORIZ)
			{
				rectWnd.b = m_rectResizeDrag.b;
			}
...
void CBCGPTabWnd::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (m_bResize)
	{
		...
		if (m_ResizeMode == RESIZE_VERT) 
		{
			...
			size.w = size.h = nWidth;

			rectNew.l = point.x - nWidth / 2;
			rectNew.r = rectNew.l + nWidth;

			if (rectNew.l < m_rectResizeBounds.l)
			{
				rectNew.l = m_rectResizeBounds.l;
				rectNew.r = rectNew.l + nWidth;
			}
			else if (rectNew.r > m_rectResizeBounds.r)
			{
				rectNew.r = m_rectResizeBounds.r;
				rectNew.l = rectNew.r - nWidth;
			}
		}
		else if (m_ResizeMode == RESIZE_HORIZ)
		{
			int nHeight = rectNew.Height();
			size.w = size.h = nHeight;

			rectNew.t = point.y - nHeight / 2;
			rectNew.b = rectNew.t + nHeight;

			if (rectNew.t < m_rectResizeBounds.t)
			{
				rectNew.t = m_rectResizeBounds.t;
				rectNew.b = rectNew.t + nHeight;
			}
			else if (rectNew.b > m_rectResizeBounds.b)
			{
				rectNew.b = m_rectResizeBounds.b;
				rectNew.t = rectNew.b - nHeight;
			}
		}
	...
	if (m_bTrackSplitter)
	{
		int nSplitterLeftPrev = m_rectTabSplitter.l;

		m_nHorzScrollWidth = min (
			m_nScrollBarRight - m_rectTabsArea.l - SPLITTER_WIDTH, 
			m_nScrollBarRight - point.x);

		m_nHorzScrollWidth = max (MIN_SROLL_WIDTH, m_nHorzScrollWidth);
		AdjustWndScroll();

		if (m_rectTabSplitter.l > nSplitterLeftPrev)
		{
			CRect rect = m_rectTabSplitter;
			rect.l = nSplitterLeftPrev - 20;
			rect.r = m_rectTabSplitter.l;
...
void CBCGPTabWnd::SetTabsHeight()
{
	...
	{
		const int nImageHeight = m_sizeImage.h <= 0 ? 0 : m_sizeImage.h + 7;
...
BOOL CBCGPTabWnd::EnsureVisible (int iTab)
{
	...
	if (m_bTabDocumentsMenu)
	{
		if (rectTab.l >= m_rectTabsArea.r || rectTab.IsRectEmpty())
		{
	...
	if (m_bIsOneNoteStyle || m_bIsVS2005Style || m_bLeftRightRounded)
	{
		if (rectTab.l < m_rectTabsArea.l || 
			rectTab.r > m_rectTabsArea.r)
		{
	...
	else
	{
		if (rectTab.l < m_rectTabsArea.l)
		{
			m_nTabsHorzOffset -= (m_rectTabsArea.l - rectTab.l);
			bAdjustTabs = TRUE;
		}
		else if (rectTab.r > m_rectTabsArea.r &&
			rectTab.Width() <= m_rectTabsArea.Width())
		{
			m_nTabsHorzOffset += (rectTab.r - m_rectTabsArea.r);
			bAdjustTabs = TRUE;
		}
	}
...
BOOL CBCGPTabWnd::OnCommand(WPARAM wParam, LPARAM lParam) 
{
	...
	else if ((HWND)lParam == m_btnScrollRight.GetSafeHwnd())
	{
		if (m_bTabDocumentsMenu)
		{
			...
			CPoint ptMenu (rectButton.l, rectButton.b + 2);
...
void CBCGPTabWnd::ReposButtons (CPoint pt, CSize sizeButton, BOOL bHide, int nButtonMargin)
{
	...
	for (POSITION pos = m_lstButtons.GetHeadPosition(); pos != NULL;)
	{
		...
		else
		{
			...
			{
				x = m_rectTabsArea.l - sizeButton.w - 1;
			}

			::SetWindowPos (hWndButton, NULL,
				x, pt.y,
				sizeButton.w, sizeButton.h,
				SWP_NOACTIVATE | SWP_NOZORDER);

			if ((!m_bIsOneNoteStyle && !m_bIsVS2005Style && !m_bLeftRightRounded) || !bIsFirst)
			{
				pt.x += sizeButton.w + nButtonMargin;
			}
		}
	...
	if (m_bIsOneNoteStyle || m_bIsVS2005Style || m_bLeftRightRounded)
	{
		...
		else
		{
			...
			m_btnScrollRight.EnableWindow (m_bTabDocumentsMenu ||
				(pLastTab->m_rect.r > m_rectTabsArea.r &&
...
int CBCGPTabWnd::GetTabFromPoint (CPoint& pt) const
{
	...
	if (m_iActiveTab >= 0)
	{
		...
		{
			if (m_iActiveTab > 0 && pt.x < rectTab.l + rectTab.Height())
			{
				const int x = pt.x - rectTab.l;
				const int y = pt.y - rectTab.t;
...
void CBCGPTabWnd::OnShowTabDocumentsMenu (CPoint point)
{
	...
	for (int i = 0; i < m_iTabsNum; i ++)
	{
		...
		{
			CBitmap* hIcon = pTab->m_pWnd->GetIcon (FALSE);
			if (hIcon == NULL)
			{
				hIcon = (CBitmap*)(LONG_PTR) GetClassLongPtr (pTab->m_pWnd->GetSafeHwnd(), GCLP_HICONSM);
			}
...
//****
CBitmap* CBCGPTabWnd::GetDocumentIcon (UINT nCmdID)
{
	CBitmap* hIcon = NULL;
...
void CBCGPTabWnd::DrawResizeDragRect (CRect& rectNew, CRect& rectOld)
{
	...
	if (m_ResizeMode == RESIZE_VERT)
	{
		size.w = size.h = m_rectResizeDrag.Width() / 2 + 1;
	}
	else
	{
		size.w = size.h = m_rectResizeDrag.Height() / 2 + 1;
	}
...
int CBCGPTabWnd::GetButtonState (CBCGTabButton* pButton)
{
	...
	return m_bIsOneNoteStyle || m_bIsVS2005Style || m_bLeftRightRounded ? CBCGPMenuImages::ImageBlack : CBCGPMenuImages::ImageDkGray;
}
