...
protected:
	CBCGPToolBarColorButton(COLORREF color = _RGB (0, 0, 0), 
								...
								BOOL bDrawBorder, BOOL /*bGrayDisabledButtons*/)
{
	...
	rectFrame.DeflateRect (afxData.wBorder2, afxData.hBorder2);
	...
	if (m_bIsOther || m_bIsAutomatic || m_bIsLabel)
	{
		...
		if (m_bIsAutomatic && m_Color != (COLORREF)-1)
		{
			...
			rectColor.DeflateRect (afxData.wBorder2 + 1, afxData.hBorder2 + 1);
			rectColor.r = rectColor.l + rectColor.Height();

			if (!bDisabled)
			{
				CBrush br (PALETTERGB(GetRV (m_Color),
									GetGV(m_Color), 
									GetBV(m_Color)));
			...
			rectText.l = rectColor.r + afxData.wBorder2;
		}
		...
		else
		{
				...
				pColorButton->m_bIsLabel)
			{
				...
				if (pColorButton->m_bIsOther && bIsOtherColor)
				{
					rectButton = CRect (
						CPoint (x, y),
						CSize (rectClient.Width() - m_BoxSize.w, 
								m_nRowHeight - m_nVertMargin / 2));
					x = rectButton.r;
					y += (rectButton.Height() - m_BoxSize.h) / 2;
				}
				else
				{
					rectButton = CRect (
...
CSize CBCGPColorBar::CalcSize (BOOL bVertDock)
{
	...
	return CSize(
		sizeGrid.w * m_BoxSize.w + 2 * m_nVertMargin, 
		sizeGrid.h * m_BoxSize.h + GetExtraHeight (sizeGrid.w) + 2 * m_nHorzMargin);
}
//****
...
//
{
		...
		(m_bShowDocColorsWhenDocked || IsFloating()))
	{
		...
		nExtraHeight += m_nRowHeight + nDocColorRows * m_BoxSize.h + 2 * SEPARATOR_SIZE + m_nVertMargin;
	}
...
void CBCGPColorBar::ContextToSize (BOOL bSquareButtons, BOOL bCenterButtons)
{
	...
	// First, adjust height:
	int nCurrHeight = CalcSize (TRUE).h;
	int yDelta = nCurrHeight < rectClient.Height() ? 1 : -1;

	while ((nCurrHeight = CalcSize (TRUE).h) != rectClient.Height())
	{
		if (yDelta < 0)
		{
	...
	// Now, adjust width:
	int nCurrWidth = CalcSize (TRUE).w;
	int xDelta = nCurrWidth < rectClient.Width() ? 1 : -1;

	while ((nCurrWidth = CalcSize (TRUE).w) != rectClient.Width())
	{
		if (xDelta < 0)
		{
	...
	if (bSquareButtons)
	{
		m_BoxSize.w = m_BoxSize.h = min (m_BoxSize.w, m_BoxSize.h);
		m_nRowHeight = m_BoxSize.h * 3 / 2;
	}
	...
	if (bCenterButtons)
	{
		...
		m_nHorzOffset = (rectClient.Width() - size.w) / 2;
		m_nVertOffset = (rectClient.Height() - size.h) / 2;
	}
	else
	{
		m_nHorzOffset = m_nVertOffset = 0;
...
BOOL CBCGPColorBar::OnSendCommand (const CBCGPToolbarButton* pButton)
{
	...
	else
	{
		...
		SendMessage(WM_COMMAND, m_nCommandID);    // send command
...
int CBCGPColorBar::InitColors (CPalette* pPalette, CArray<COLORREF, COLORREF>& arColors)
{
	...
	for (int i = 0; i < nColorsCount; i++)
	{
		...
		arColors [i] = _RGB (palEntry.peRed, palEntry.peGreen, palEntry.peBlue);
	}
								...
								CPalette& palette)
{
	...
	{
		pLogPalette->palPalEntry[i].peRed   = GetRV (arColors[i]);
		pLogPalette->palPalEntry[i].peGreen = GetGV (arColors[i]);
		pLogPalette->palPalEntry[i].peBlue  = GetBV (arColors[i]);
