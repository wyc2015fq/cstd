...
void CBCGPRibbonUndoButton::OnShowPopupMenu()
{
	...
	if (m_sizeMaxText == CSize (0, 0))
	{
		...
		for (int i = 0; i < m_arLabels.GetSize(); i++)
		{
			...
			m_sizeMaxText.w = max (m_sizeMaxText.w, szText.w);
			m_sizeMaxText.h = max (m_sizeMaxText.h, szText.h);
		}

		m_sizeMaxText.w = max (m_sizeMaxText.w, dc.GetTextExtent (m_strCancel).w);
		m_sizeMaxText.w = max (m_sizeMaxText.w, dc.GetTextExtent (m_strUndoOne).w);
		m_sizeMaxText.w = max (m_sizeMaxText.w, dc.GetTextExtent (m_strUndoFmt).w);

		m_sizeMaxText.w += 2 * nTextMarginHorz;
