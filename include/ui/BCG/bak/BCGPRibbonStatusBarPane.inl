...
CBCGPRibbonStatusBarPane::CBCGPRibbonStatusBarPane (UINT nCmdID, 
	LPCTSTR lpszText, BOOL bIsStatic, CBitmap* hIcon, LPCTSTR lpszAlmostLargeText) :
...
//****
CBCGPRibbonStatusBarPane::CBCGPRibbonStatusBarPane (UINT nCmdID, 
	LPCTSTR lpszText, CBitmap* hBmpAnimationList, int cxAnimation,
	COLORREF clrTransp, CBitmap* hIcon, BOOL bIsStatic) :
	CBCGPRibbonButton (nCmdID, lpszText, hIcon)
{
	CommonInit();
...
void CBCGPRibbonStatusBarPane::OnCalcTextSize (CDC* pDC)
{
	...
	{
		const int nTextWidth = pDC->GetTextExtent (m_strAlmostLargeText).w;

		m_bIsTextTruncated = nTextWidth < m_sizeTextRight.w;
...
//****
void CBCGPRibbonStatusBarPane::SetAnimationList (CBitmap* hBmpAnimationList, int cxAnimation,
												 COLORREF clrTransp)
{
	ASSERT_VALID (this);
...
CSize CBCGPRibbonStatusBarPane::GetIntermediateSize (CDC* pDC)
{
	...
	if (m_AnimImages.GetCount() > 0)
	{
		...
		return CSize (
			imageSize.w + 2 * m_szMargin.w,
			imageSize.h + 2 * m_szMargin.h);
	}

	CSize size = CBCGPRibbonButton::GetIntermediateSize (pDC);
	size.w -= GetTextOffset() + 1;
	...
	else
	{
		if (globalData.GetRibbonImageScale() != 1.)
		{
			...
			CSize sizeDest = CSize (
					(int) (.5 + globalData.GetRibbonImageScale() * sizeImage.w), 
					(int) (.5 + globalData.GetRibbonImageScale() * sizeImage.h));
			...
			ptImage.Offset (
				max (0, (m_rect.Width() - sizeDest.w) / 2), 
				max (0, (m_rect.Height() - sizeDest.h) / 2));
