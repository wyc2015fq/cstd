...
CSize CBCGPMenuBar::CalcLayout(DWORD dwMode, int nLength)
{
	...
	if (!bVert)
	{
		...
		//-------------------------------------
		size = CalcSize (FALSE);
		size.w = rect.Width() - rectClient.Width() / 2;
		size.h -= rectClient.Height();
	}
	else
	{
		//-------------------------------------
		...
		size.h = rect.Height() - rectClient.Height() / 2;
		size.w -= rectClient.Width();
	}
	...
	if (bMax)
	{
		...
		else
		{
            ...
            if (m_hSysMenu != NULL)
            {
				...
				if (m_hSysIcon == NULL)
				{
					m_hSysIcon = (CBitmap*)(LONG_PTR) GetClassLongPtr (*pWnd, GCLP_HICONSM);
				}
...
void CBCGPMenuBar::AdjustLocations()
{
	...
	if (m_bMaximizeMode)
	{
		int iButtonWidth = m_szSystemButton.w;
		int iButtonHeight = m_szSystemButton.h;
		...
		if (!bHorz)
		{
			rect.b += rectClient.Height();
		}

		rect.l = rect.r - iButtonWidth;
		rect.t = rect.b - iButtonHeight;
	...
	if (pHelpCombobox != NULL)
	{
		...
		{
			rectCombo.r = xRight;
			rectCombo.l = rectCombo.r - m_nHelpComboWidth;
		}
		else
		{
...
BOOL CBCGPMenuBar::LoadState (LPCTSTR lpszProfileName, int nIndex, UINT /*uiID*/)
{
	...
	if (m_bAutoDocMenus && pDocManager != NULL)
	{
		...
		for (POSITION pos = pDocManager->GetFirstDocTemplatePosition(); pos != NULL;)
		{
			...
			else if (GetOwner()->GetSafeHwnd() != NULL) 
			{
        		...
        		SendMessage(BCGM_RESETMENU, uiMenuResId);
    			g_menuHash.SaveMenuBar (pTemplate->m_hMenuShared, this);
    ...
    else if (GetOwner()->GetSafeHwnd() != NULL)
    {
		...
		if (uiResID == 0)
		{
			...
			UINT uiResID = (UINT) SendMessage(WM_HELPHITTEST);
			if (uiResID != 0)
			{
				uiResID -= HID_BASE_RESOURCE;
			}
		}
        SendMessage(BCGM_RESETMENU, uiResID);
    	g_menuHash.SaveMenuBar (m_hDefaultMenu, this);
	...
	if (m_bMaximizeMode && iIndex >= 0)
	{
		...
		if (iIndex > GetCount() - m_nSystemButtonsNum)
		{
			...
			if (m_nSystemButtonsNum > 0)
			{
				...
				CPoint ptDrag (rectBtn.r, rectBtn.t + rectBtn.Height() / 2);
