...
public:
	...
	CBCGPRibbonStatusBarPane (
		...
		CBitmap*		hIcon = NULL,				// Pane icon
		LPCTSTR		lpszAlmostLargeText = NULL);// The almost large text in pane

	CBCGPRibbonStatusBarPane (
		UINT		nCmdID,						// Pane command id
		LPCTSTR		lpszText,					// Pane label
		CBitmap*		hBmpAnimationList,			// Pane animation list
...
// Operations
public:
	void SetAnimationList (	CBitmap* hBmpAnimationList, int cxAnimation = 16, 
...
protected:
	virtual int GetTextOffset() const	{	return m_szMargin.w / 2;	}
