...
CRect CBCGPRibbonTab::GetKeyTipRect (CDC* pDC, BOOL /*bIsMenu*/)
{
	...
	rectKeyTip.l = rectTab.CenterPoint().x - sizeKeyTip.w / 2;
	rectKeyTip.r = rectKeyTip.l + sizeKeyTip.w;

	rectKeyTip.t = rectTabText.b - 2;
...
int CBCGPRibbonCategory::GetMaxHeight (CDC* pDC)
{
	...
	return nMaxHeight + pDC->GetTextExtent (m_strName).h + nPanelMarginTop + nPanelMarginBottom;
}
//****
void CBCGPRibbonCategory::RecalcLayout (CDC* pDC)
{
	...
	else if (m_nLastCategoryWidth != m_rect.Width() || m_nLastCategoryOffsetY != m_rect.t)
	{
		m_nLastCategoryWidth = m_rect.Width();
		m_nLastCategoryOffsetY = m_rect.t;
...
void CBCGPRibbonCategory::UpdateScrollButtons()
{
	...
	if (m_nScrollOffset > 0)
	{
		...
		rectScrollLeft.r = rectScrollLeft.l + cxScrollWidth;
	}

	if (m_rect.Width() + m_nScrollOffset < m_nMinWidth)
	{
		rectScrollRight = m_rect;
		rectScrollRight.l = rectScrollRight.r - cxScrollWidth;
	}
...
void CBCGPRibbonCategory::ReposPanels (CDC* pDC)
{
	...
	int x = rectClient.l - m_nScrollOffset;
	...
	for (int i = 0; i < m_arPanels.GetSize(); i++)
	{
		...
		CRect rectPanel = CRect (x, rectClient.t, 
								x + nCurrPanelWidth, rectClient.b);

		pPanel->Repos (pDC, rectPanel);

		x = pPanel->m_rect.r + nPanelMarginRight;

		if (rectPanel.r <= rectClient.l + 2 * nPanelMarginLeft ||
			rectPanel.l >= rectClient.r - 2 * nPanelMarginRight)
		{
...
void CBCGPRibbonCategory::ReposPanelsVert (CDC* pDC)
{
	...
	for (int i = 0; i < m_arPanels.GetSize(); i++)
	{
		...
		pPanel->m_rect.r = x + rectClient.Width();
		...
		if (!rectLaunch.IsRectEmpty())
		{
			...
			rectLaunch.r = pPanel->m_rect.r - 1;
			rectLaunch.l = rectLaunch.r - cx;
		...
		{
			pPanel->m_rectCaption.r = pPanel->m_rect.r;
		}

		y = pPanel->m_rect.b + nPanelMarginBottom * 2;

		if (rectPanel.b <= rectClient.t + 2 * nPanelMarginTop ||
			rectPanel.t >= rectClient.b - 2 * nPanelMarginBottom)
		{
			rectPanel.SetRectEmpty();
			pPanel->Repos (pDC, rectPanel);
		}
		...
		BOOL nImageIndex, BOOL bCenter)
{
	...
	{
		ptImage.Offset (
			max (0, (rect.Width() - sizeImage.w) / 2), 
			max (0, (rect.Height() - sizeImage.h) / 2));
	}
...
void CBCGPRibbonCategory::NormalizeFloatingRect (CBCGPRibbonBar* pRibbonBar, CRect& rectCategory)
{
	...
	rectCategory.r = min (rectCategory.r, rectScreen.r);
	rectCategory.l = max (rectCategory.l, rectScreen.l);
}
//****
void CBCGPRibbonCategory::ShowFloating (CRect rectFloating)
{
	...
	pMenu->Create (m_pParenrRibbonBar, 
		bIsRTL ? rectFloating.r : rectFloating.l, 
		rectFloating.t, (HMENU) NULL);
}
//****
...
void CBCGPRibbonCategory::EnsureVisible (CBCGPRibbonButton* pButton)
{
	...
	if (rectButton.IsRectEmpty())
	{
		...
		if (nPanelIndex < nFirstVisiblePanel)
		{
			while (OnScrollHorz (TRUE))
			{
				if (!pParentPanel->GetRect().IsRectEmpty() &&
					pParentPanel->GetRect().l >= rectClient.l)
				{
					break;
				}
			}
		...
		else if (nPanelIndex > nLastVisiblePanel)
		{
			...
			{
				if (!pParentPanel->GetRect().IsRectEmpty() &&
					pParentPanel->GetRect().r <= rectClient.r)
				{
					break;
				}
			}
	...
	if (rectButton.l < m_rect.l - nPanelMarginRight)
	{
		OnScrollHorz (TRUE, rectClient.l - rectButton.l);
	}
	else if (rectButton.r > m_rect.r + nPanelMarginRight)
	{
		OnScrollHorz (FALSE, rectButton.r - rectClient.r);
	}
}
//****
