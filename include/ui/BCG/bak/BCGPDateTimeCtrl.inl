...
void CBCGPDateTimeCtrl::DrawItem(LPDRAWITEMSTRUCT lpDIS) 
{
	...
	for (int i = iStart; i < m_iPartsNumber; i ++)
	{
		...
		if (i < m_iPartsNumber - 1)
		{
			...
			rect.l = m_arPartRects [i].r;
			rect.r = m_arPartRects [i + 1].l;
...
void CBCGPDateTimeCtrl::OnShowCalendarPopup() 
{
	...
	CRect rectCalendar (CPoint (rectWindow.l, rectWindow.b),
		CSize (0, 0));
...
void CBCGPDateTimeCtrl::AdjustControl (CRect rectClient)
{
	...
	if (m_bCheckBoxIsAvailable)
	{
		m_rectCheck = CRect (rectClient.l + 1, rectClient.t + 1, 
			rectClient.l + m_iControlHeight - 1, rectClient.b - 1);
		m_rectText.l = m_rectCheck.r + 2;
	}
	
	m_rectText.t = rectClient.t;
	m_rectText.b = rectClient.t + m_iControlHeight;

	// Calculate parts:
	CalcPartRects (&dc);

	// Adjust control size:
	m_rectText.r = m_arPartRects [m_iPartsNumber - 1].r + 2;
	m_iControlWidth = m_rectText.r;
	...
	if (m_dropCalendar && m_showDate)
	{
		...
		int iDropStart = rectClient.l + m_iControlWidth - iDropButtonWidth - 1;
		if (m_spinButton)
		{
		...
		m_rectDropButton = CRect (	iDropStart,
									...
									rectClient.t + m_iControlHeight - 2);
	}
	...
	if (m_spinButton)
	{
		...
		m_wndSpin.SetWindowPos (NULL, 
				rectClient.l + m_iControlWidth - iSpinWidth - 2, rectClient.t,
...
//****
int CBCGPDateTimeCtrl::GetPartFromPoint (IPOINT point)
{
...
void CBCGPDateTimeCtrl::CalcPartRects (CDC* pDC)
{
	...
	for (i = iStart; i < m_iPartsNumber; i ++)
	{
		...
		m_arPartRects [i].l = x;
		m_arPartRects [i].r = x + pDC->GetTextExtent (strPart).w;
		...
		x = m_arPartRects [i].r + pDC->GetTextExtent (strSeparator).w;
	}
...
void CBCGPDateTimeCtrl::BuidWidestDate (CDC* pDC)
{
	...
	else
	{
		...
		for (int iMonth = 1; iMonth <= 12; iMonth ++)
		{
			...
			int iMonthWidth = pDC->GetTextExtent (strMonth).w;
