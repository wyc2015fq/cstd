...
COLORREF CBCGPBaseIntelliSenseLB::m_clrSelectedItemBkColor = _GetSysColor(CLR_HIGHLIGHT);
COLORREF CBCGPBaseIntelliSenseLB::m_clrSelectedItemTextColor = _GetSysColor(CLR_HIGHLIGHTTEXT);
COLORREF CBCGPBaseIntelliSenseLB::m_clrWindow = _GetSysColor(CLR_WINDOW);
COLORREF CBCGPBaseIntelliSenseLB::m_clrWindowText = _GetSysColor(CLR_WINDOWTEXT);
...
CBCGPBaseIntelliSenseLB::CBCGPBaseIntelliSenseLB() : m_pImageList (NULL), m_nLBHeight (0), 
											 m_pFont (NULL)
{
	m_sizeMaxItem.w = m_sizeMaxItem.h = 0;
	...
	if (lpDrawItemStruct->itemData != NULL)
	{
		...
		if (m_pImageList != NULL)
		{
			m_pImageList->Draw (pDC, pData->m_nImageListIndex, rectItem.TopLeft(), 
								ILD_NORMAL);
			rectItem.l += GetImageSize (pData->m_nImageListIndex).w + 
		...
		CFont* pOldFont = pDC->SelectObject (m_pFont);
		rectItem.l += m_nFocusRectToTextSpacing;
...
void CBCGPBaseIntelliSenseLB::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
	...
	lpMeasureItemStruct->itemHeight = max (sizeText.h, sizeImage.h) + 2;
	lpMeasureItemStruct->itemWidth  = sizeText.w + sizeImage.w + 
									  m_nImageToFocusRectSpacing + 
									  m_nFocusRectToTextSpacing + 
									  m_nRightSpacing + 1;

	m_sizeMaxItem.w = max ((UINT) m_sizeMaxItem.w, lpMeasureItemStruct->itemWidth);
	m_sizeMaxItem.h = max ((UINT) m_sizeMaxItem.h, lpMeasureItemStruct->itemHeight);
...
void CBCGPBaseIntelliSenseLB::UpdateColors()
{
	...
	CBCGPBaseIntelliSenseLB::m_clrSelectedItemBkColor = _GetSysColor(CLR_HIGHLIGHT);
	CBCGPBaseIntelliSenseLB::m_clrSelectedItemTextColor = _GetSysColor(CLR_HIGHLIGHTTEXT);
	CBCGPBaseIntelliSenseLB::m_clrWindow = _GetSysColor(CLR_WINDOW);
	CBCGPBaseIntelliSenseLB::m_clrWindowText = _GetSysColor(CLR_WINDOWTEXT);
#else
