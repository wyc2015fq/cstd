...
void CBCGPToolTipCtrl::OnPaint(CDC* pDC) 
{
	if (m_Params.m_bBallonTooltip)
	{
		CToolTipCtrl::OnPaint(CDC* pDC);
		return;
	}
	...
	COLORREF clrLine = m_Params.m_clrBorder == (COLORREF)-1 ?
		_GetSysColor(CLR_INFOTEXT) : m_Params.m_clrBorder;

	COLORREF clrText = m_Params.m_clrText == (COLORREF)-1 ?
		_GetSysColor(CLR_INFOTEXT) : m_Params.m_clrText;
	...
	if (m_sizeImage != CSize (0, 0) && m_Params.m_bDrawIcon)
	{
		...
		rectImage.r = rectImage.l + m_sizeImage.w;
		rectImage.b = rectImage.t + m_sizeImage.h;

		OnDrawIcon (pDC, rectImage);

		rectText.l += m_sizeImage.w + m_ptMargin.x;
	}
	...
	// Draw label:
	//------------
	int nTextHeight = OnDrawLabel (pDC, rectText, FALSE).h;
	...
	if (!m_strDesrciption.IsEmpty() && m_Params.m_bDrawDescription)
	{
		...
		rectDescr.t += nTextHeight + 3 * m_ptMargin.y / 2;

		if (m_Params.m_bDrawSeparator)
		{
			OnDrawSeparator (pDC, rectDescr.l,
				rectDescr.r, rectDescr.t - m_ptMargin.y / 2);
		}
...
void CBCGPToolTipCtrl::OnShow(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
	*pResult = 0;
	...
	{
		cy = max (cy, m_sizeImage.h);
	}
	else
	{
		sizeDescr = OnDrawDescription (&dc, rectText, TRUE);

		cy += sizeDescr.h + 2 * m_ptMargin.y;
		cx = max (cx, sizeDescr.w);

		cy = max (cy, m_sizeImage.h);
	}

	if (m_sizeImage.w > 0 && m_Params.m_bDrawIcon)
	{
									 ...
									 COLORREF clrLine)
{
	...
	pDC->MoveTo (rect.l + nOffset, rect.t);
	pDC->LineTo (rect.r - nOffset - 1, rect.t);

	pDC->LineTo (rect.r - 1, rect.t + nOffset);
	pDC->LineTo (rect.r - 1, rect.b - 1 - nOffset);

	pDC->LineTo (rect.r - nOffset - 1, rect.b - 1);
	pDC->LineTo (rect.l + nOffset, rect.b - 1);

	pDC->LineTo (rect.l, rect.b - 1 - nOffset);
	pDC->LineTo (rect.l, rect.t + nOffset);

	pDC->LineTo (rect.l + nOffset, rect.t);
}
//****
...
CSize CBCGPToolTipCtrl::OnDrawLabel (CDC* pDC, CRect rect, BOOL bCalcOnly)
{
	...
	else
	{
		...
		else
		{
			...
			sizeText.h = pDC->DrawText (strText, rect, nFormat);
			sizeText.w = rect.Width();
		}
	}
...
CSize CBCGPToolTipCtrl::OnDrawDescription (CDC* pDC, CRect rect, BOOL bCalcOnly)
{
	...
	{
		rect.r = rect.l + nFixedWidth;

		if (m_sizeImage.w > 0 && m_Params.m_bDrawIcon)
		{
			rect.r -= m_sizeImage.w + m_ptMargin.x;
		}
	}
	else
	{
		rect.r = rect.l + m_Params.m_nMaxDescrWidth;
	}
...
int CBCGPToolTipCtrl::GetFixedWidth()
{
	...
	if (m_sizeImage.w <= (int) (globalData.GetRibbonImageScale() * 32))
	{
