...
// If rcBounds.t and rcBounds.b are equal, rcBounds.t is used as Y-coord
// for all specified controls.
// Use TA_... constants for dwAdjustment
static void AdjustControls (CWnd** pControlsArray, UINT nControlsCount, const IRECT& rcBounds, DWORD dwAdjustment)
{
	...
	{
	case TA_LEFT:
		xOffset = rcBounds.l - nLeft;
		break;
	case TA_CENTER:
		xOffset = ((rcBounds.r + rcBounds.l) - (nRight + nLeft)) / 2;
		break;
	case TA_RIGHT:
		xOffset = rcBounds.r - nRight;
		break;
	}
	...
	for (UINT i = 0; i < nControlsCount; ++i)
	{
		...
		int y = (rcBounds.t == rcBounds.b) ? rcBounds.t : rect.t;
		pControlsArray[i]->MoveWindow (rect.l + xOffset, y, rect.r - rect.l, rect.b - rect.t, TRUE);
	}
}
...
void CBCGPMessageBox::Initialize()
{
	...
	{
		m_rectIcon.r = ::GetSystemMetrics(SM_CXICON);
		m_rectIcon.b = ::GetSystemMetrics(SM_CYICON);
	}
...
void CBCGPMessageBox::OnPaint(CDC* pDC)
{
	CPaintDC dc(this); // device context for painting
	dc.DrawIcon (m_rectIcon.l, m_rectIcon.t, m_hMessageIcon);
	DrawText (&dc);
	...
	if (m_Params.bShowCheckBox && m_Params.bShowSeparator)
	{
		...
		rectClient.t  = m_cySeparatorLine - 1;
		rectClient.b = m_cySeparatorLine + 1;
...
void CBCGPMessageBox::AddButton (UINT id, HINSTANCE hInst, LPCTSTR lpszCaption /*= NULL*/)
{
	...
	int nButtonWidth = m_szButton.w;
	CRect rectButton (m_ptNextButtonPos.x, 0, m_ptNextButtonPos.x + nButtonWidth, m_szButton.h);
	...
	if (pBtn->GetSafeHwnd() != NULL)
	{
		...
		if (szButtonText.w > m_szButton.w - cxButtonMargins * 2)
		{
			// Resize button to show whole text
			szButtonText.w = min (szButtonText.w, 512); // Preserve from too long button text
			nButtonWidth = szButtonText.w + cxButtonMargins * 2;
			pBtn->SetWindowPos (NULL, 0, 0, nButtonWidth, m_szButton.h, SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOCOPYBITS);
		}
...
void CBCGPMessageBox::PreInitDialog()
{
...

	{
		...
		m_cxIconSpacing = rectTemp.l;
		m_cyVerticalSpacing = rectTemp.t;
		m_cxButtonSpacing = rectTemp.r;
		m_cyCheckBoxHeight = rectTemp.b;
	...
	m_rectText.SetRect (0, 0, nMaxMessageBoxWidth, rectScreen.Height() - m_szButton.h - m_cxButtonSpacing);
	if (!m_rectIcon.IsRectEmpty())
	{
		m_rectText.r -= (m_rectIcon.Width() + m_cxIconSpacing);
	}


	CDC* hdcScreen = ::GetDC (NULL); // Screen DC
	...
	if (m_Params.bShowCheckBox)
	{
		...
		szCheckBox.w += 2 * m_cyCheckBoxHeight;
		if (m_hMessageIcon != NULL)
		{
			szCheckBox.w += m_rectIcon.Width();
		}
		nMinClientWidth = max (nMinClientWidth, szCheckBox.w);
	}
	...
	if (m_hMessageIcon != NULL)
	{
		...
		if (m_rectIcon.Height() > m_rectText.Height())
		{
			...
			szTextOffset.h = (m_rectIcon.Height() - m_rectText.Height()) / 2;
		}

		m_rectText.OffsetRect (szTextOffset);

		szTotal.w += m_rectIcon.Width() + m_cxIconSpacing;
		szTotal.h = max (szTotal.h, m_rectIcon.Height());
	}

	if (szTotal.w < nMinClientWidth)
	{
		szTotal.w = nMinClientWidth;
	}
	szTotal.h += m_cyVerticalSpacing;

	if (m_bRightAlignment)
	{
		m_rectText.OffsetRect (szTotal.w - m_rectText.r, 0);
	}

	// Adjusting buttons

	int yButtons = szTotal.h + m_rectClientMargins.t;
	...
	if (m_Params.bShowCheckBox)
	{
		int xCheckBox = m_rectClientMargins.l;
		if (m_hMessageIcon != NULL)
		{
			xCheckBox += m_rectIcon.Width() + m_cxIconSpacing;
		}

		CRect rectCheckBox (xCheckBox, yButtons, m_rectClientMargins.l + szTotal.w, yButtons + m_cyCheckBoxHeight);
		int yOffset = rectCheckBox.Height() + m_cyVerticalSpacing;

		if (m_Params.bShowSeparator)
		{
			m_cySeparatorLine = rectCheckBox.t;
	...
	m_rectIcon.OffsetRect (m_rectClientMargins.l, m_rectClientMargins.t);
	m_rectText.OffsetRect (m_rectClientMargins.l, m_rectClientMargins.t);

	CRect rectButtons (m_rectClientMargins.l, yButtons, m_rectClientMargins.r + szTotal.w, yButtons);
	AdjustControls (&m_arrButtons[0], (UINT)m_arrButtons.GetSize(), rectButtons, TA_CENTER);

	szTotal.h += m_szButton.h; // place for buttons

	CRect rectClient (0, 0, szTotal.w, szTotal.h);
