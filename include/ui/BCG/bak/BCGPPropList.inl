...
CBCGPProp* CBCGPProp::HitTest (CPoint point, ClickArea* pnArea)
{
	...
	if (m_Rect.PtInRect (point))
	{
		if (pnArea != NULL)
		{
			if (IsGroup() && point.x < m_Rect.l + m_Rect.Height())
			{
				*pnArea = ClickExpandBox;
			}
			else if (HasValueField() && 
				point.x > m_pWndList->m_rectList.l + m_pWndList->m_nLeftColumnWidth)
			{
				*pnArea = ClickValue;
			}
			else
...
void CBCGPProp::Repos (int& y)
{
	...
	if (bShowProperty)
	{
		...
		m_Rect = CRect (
			m_pWndList->m_rectList.l + dx,
			y, 
			m_pWndList->m_rectList.r, 
			y + m_pWndList->m_nRowHeight);

		if (!m_rectButton.IsRectEmpty())
		{
			m_rectButton.t = m_Rect.t + 1;
			m_rectButton.b = m_Rect.b;
		}

		y += m_pWndList->m_nRowHeight;

		CRect rectName = m_Rect;
		rectName.r = m_pWndList->m_rectList.l + m_pWndList->m_nLeftColumnWidth;
...
void CBCGPProp::OnDrawName (CDC* pDC, CRect rect)
{
	...
	if (IsSelected() && (!m_pWndList->m_bVSDotNetLook || !IsGroup() ||  m_bIsValueList))
	{
		...
		if (!m_pWndList->m_bFocused)
		{
			clrTextOld = pDC->SetTextColor (
				m_pWndList->DrawControlBarColors() ? globalData.clrBarText : globalData.clrBtnText);
	...
	m_bNameIsTrancated = pDC->GetTextExtent (m_strName).w > rect.Width();
	...
	if (IsSelected() && m_pWndList->m_bVSDotNetLook && IsGroup() && !m_bIsValueList)
	{
		...
		rectFocus.r = 
			min (rect.r, rectFocus.l + pDC->GetTextExtent (m_strName).w);
...
void CBCGPProp::OnDrawValue (CDC* pDC, CRect rect)
{
	...
	m_bValueIsTrancated = pDC->GetTextExtent (strVal).w > rect.Width();
...
void CBCGPProp::OnDrawButton (CDC* pDC, CRect rect)
{
	...
	COLORREF clrTextOld = pDC->SetTextColor (m_pWndList->DrawControlBarColors() ?
...
void CBCGPProp::OnDrawExpandBox (CDC* pDC, CRect rect)
{
	...
	if (m_bIsValueList)
	{
		if (m_pWndList->m_brBackground.IsValid())
		{
...
void CBCGPProp::OnDrawDescription (CDC* pDC, CRect rect)
{
	...
	HFONT hOldFont = (HFONT) ::SelectObject (pDC, m_pWndList->m_fontBold.GetSafeHandle());
...
void CBCGPProp::AdjustInPlaceEditRect (CRect& rectEdit, CRect& rectSpin)
{
	...
	rectEdit.l = m_pWndList->m_rectList.l + m_pWndList->m_nLeftColumnWidth 
		+ TEXT_MARGIN - nMargin + 1;

	if (HasButton())
	{
		AdjustButtonRect();
		rectEdit.r = m_rectButton.l;
	}
...
void CBCGPProp::SetModifiedFlag()
{
	...
	if (m_pWndList->m_bMarkModifiedProperties)
	{
		...
		if (m_pWndInPlace->GetSafeHwnd() != NULL)
		{
			...
			{
				AdjustButtonRect();
				rectInPlace.r = m_rectButton.l;
			}

			m_pWndInPlace->SetWindowPos (NULL, 
				rectInPlace.l + nXOffset, rectInPlace.t, 
...
void CBCGPColorProp::OnClickButton (CPoint /*point*/)
{
	...
	CPoint pt (
		m_pWndList->m_rectList.l + m_pWndList->m_nLeftColumnWidth + 1, 
		m_rectButton.b + 1);
...
void CBCGPDateTimeProp::OnDrawValue (CDC* pDC, CRect rect)
{
	...
	rect.l = m_pWndList->GetListRect().l + 
...
void CBCGPDateTimeProp::OnPosSizeChanged (CRect /*rectOld*/)
{
	...
	else
	{
		...
		m_wndDateTime.SetWindowPos (NULL, rectEdit.l, rectEdit.t,
...
void CBCGPDateTimeProp::AdjustInPlaceEditRect (CRect& rectEdit, CRect& rectSpin)
{
	...
	rectEdit.l = m_pWndList->GetListRect().l + 
...
void CBCGPPropList::AdjustLayout()
{
	...
	if (m_bHeaderCtrl)
	{
		...
		GetHeaderCtrl().SetWindowPos (NULL, rectClient.l, rectClient.t, 
			rectClient.Width(), m_nHeaderHeight,
			SWP_NOZORDER | SWP_NOACTIVATE);

		HDITEM hdItem;
		hdItem.mask = HDI_WIDTH ;
		hdItem.wy = m_nLeftColumnWidth + 2;

		GetHeaderCtrl().SetItem (0, &hdItem);

		hdItem.wy = rectClient.Width() + 10;
	...
	m_rectList.t += m_nHeaderHeight;

	if (HasCommands() && m_nCommandsHeight != -1 && rectClient.Height() > 0)
	{
		...
		m_rectList.b -= m_nCommandsHeight;
	}
	...
	if (HasCommands() && m_nCommandsHeight != -1 && rectClient.Height() > 0)
	{
		...
		rectCommands.t = m_rectList.b; 
		rectCommands.b = rectCommands.t + m_nCommandsHeight;
		rectCommands.DeflateRect (TEXT_MARGIN, TEXT_MARGIN);

		int i = 0;
		int x = rectCommands.l;
		int y = rectCommands.t + m_nRowHeight / 4;
		...
		for (POSITION pos = m_lstCommands.GetHeadPosition(); pos != NULL; i++)
		{
			...
			if (x > rectCommands.l && x + sizeLabel.w > rectCommands.r)
			{
				x = rectCommands.l;
				y += m_nRowHeight;
			}

			int x2 = min (rectCommands.r, x + sizeLabel.w + 1);
			int y2 = min (rectCommands.b, y + sizeLabel.h + 1);

			if (y < rectCommands.b)
			{
				rectCommand = CRect (x, y, x2, y2);
				x += sizeLabel.w + TEXT_MARGIN;
			}
	...
	if (m_nVertScrollTotal > 0)
	{
		...
		m_wndScrollVert.SetWindowPos (NULL, m_rectList.r, m_rectList.t,
...
void CBCGPPropList::ReposProperties()
{
	...
	int y = m_rectList.t - m_nRowHeight * m_nVertScrollOffset - 1;
...
//****
void CBCGPPropList::OnPaint(CDC* pDC) 
{
	...
	int yBottom = m_rectList.b;
	
	if (HasCommands())
	{
		CRect rectCommands = rectClient;
		rectCommands.t = yBottom;
		rectCommands.b = rectCommands.t + m_nCommandsHeight;
		yBottom = rectCommands.b;
...
void CBCGPPropList::OnDrawList (CDC* pDC)
{
	...
	int nXCenter = m_rectList.l + m_nLeftColumnWidth;

	pDC->MoveTo (nXCenter, m_rectList.t - 1);
	pDC->LineTo (nXCenter, m_rectList.b);
...
void CBCGPPropList::OnDrawCommands (CDC* pDC, CRect rect)
{
	...
	{
		pDC->FillRect (rect, DrawControlBarColors() ? &globalData.brBarFace : &globalData.brBtnFace);
	}
...
BOOL CBCGPPropList::OnDrawProperty (CDC* pDC, CBCGPProp* pProp) const
{
	...
	{
		if (pProp->m_Rect.t >= m_rectList.b)
		{
			return FALSE;
		}

		if (pProp->m_Rect.b >= m_rectList.t)
		{
			const int nXCenter = m_rectList.l + m_nLeftColumnWidth;
			COLORREF clrTextOld = (COLORREF)-1;
			...
			if (m_bVSDotNetLook)
			{
				...
				if (!pProp->IsGroup())
				{
					rectLeft.r = min (nXCenter, rectLeft.l);
				}

				if (pProp->m_bIsValueList)
				{
					rectLeft.r = rectLeft.l + rectLeft.Height();
				}

				rectLeft.l = m_rectList.l;
				rectLeft.b = min (rectLeft.b, m_rectList.b);

				if (rectLeft.l < rectLeft.r)
				{
			...
			pDC->MoveTo (m_rectList.l, pProp->m_Rect.b);
			pDC->LineTo (m_rectList.r, pProp->m_Rect.b);
...
void CBCGPPropList::OnPropertyChanged (CBCGPProp* pProp) const
{
	...
	SendMessage(BCGM_PROPERTY_CHANGED, GetDlgCtrlID(),
		LPARAM (pProp));
}
//****
...
void CBCGPPropList::OnHeaderItemChanged(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (pHeader->iItem == 0)
	{
		...
		m_nLeftColumnWidth = min (max (m_nRowHeight, hdItem.wy - 2), rectClient.Width() - ::GetSystemMetrics (SM_CXHSCROLL) - 5);
...
void CBCGPPropList::TrackDescr (int nOffset)
{
	...
	else
	{
		...
		nOffset = max (nOffset, rectClient.t + m_nRowHeight + m_nHeaderHeight);
		nOffset = min (nOffset, rectClient.b - m_nRowHeight);

		m_rectTrackDescr = rectClient;
		m_rectTrackDescr.t = nOffset - 1;
		m_rectTrackDescr.b = m_rectTrackDescr.t + 2;
...
void CBCGPPropList::TrackCommands (int nOffset)
{
	...
	else
	{
		...
		m_rectTrackCommands.t = nOffset - 1;
		m_rectTrackCommands.b = m_rectTrackCommands.t + 2;
...
void CBCGPPropList::TrackToolTip (CPoint point)
{
	...
	if (abs (point.x - (m_rectList.l + m_nLeftColumnWidth)) <= STRETCH_DELTA)
	{
		m_IPToolTip.Deactivate();
		return;
	}
	...
	if (point.x < m_rectList.l + m_nLeftColumnWidth)
	{
		if (pProp->IsGroup())
		{
			rectToolTip.l += m_nRowHeight;
	...
	else
	{
		...
		rectToolTip.l = m_rectList.l + m_nLeftColumnWidth + 1;
		bIsValueTT = TRUE;
	}
...
void CBCGPPropList::SetCurSel (CBCGPProp* pProp, BOOL bRedraw)
{
	...
	if (pOldSelectedItem != NULL)
	{
		...
		if (bRedraw)
		{
			...
			if (!pOldSelectedItem->IsGroup() || !m_bGroupNameFullWidth)
			{
				...
				{
					rectOld.r = rectOld.l + m_nLeftColumnWidth;
				}
			}

			rectOld.r = max (rectButton.r, rectOld.r);
	...
	if (pProp != NULL)
	{
		...
		if (bRedraw)
		{
			...
			if (!pProp->IsGroup() || !m_bGroupNameFullWidth)
			{
				...
				{
					rect.r = rect.l + m_nLeftColumnWidth;
				}
			}

			rect.r = max (pProp->m_rectButton.r, rect.r);
			InvalidateRect (rect);
	...
	if (bRedraw)
	{
		if (m_bDescriptionArea || HasCommands())
		{
			CRect rectClient;
			GetClientRect (rectClient);
...
void CBCGPPropList::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (HasCommands())
	{
		...
		if (abs (point.y - (m_rectList.b + TEXT_MARGIN)) <= STRETCH_DELTA)
		{
			SetCapture();
		...
		if (point.y > m_rectList.b && !m_bDescriptionArea)
		{
			return;
		}
	}
	...
	if (m_bDescriptionArea)
	{
		if (abs (point.y - (m_rectList.b + nCommandsHeight + TEXT_MARGIN)) <= STRETCH_DELTA)
		{
			SetCapture();
			TrackDescr (point.y);
...
//****
BOOL CBCGPPropList::Create(DWORD dwStyle, const IRECT& rect, CWnd* pParentWnd, UINT nID)
{
...
void CBCGPPropList::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	...
	if (point.y <= m_rectList.b &&
		abs (point.x - (m_rectList.l + m_nLeftColumnWidth)) <= STRETCH_DELTA)
	{
		//--------------------------------------------------------
	...
	if (m_pSel->IsGroup() &&
		...
		point.x < m_rectList.l + m_nLeftColumnWidth))
	{
		m_pSel->Expand (!m_pSel->IsExpanded());
	}
...
BOOL CBCGPPropList::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	if (nHitTest == HTCLIENT)
	{
		...
		if (HasCommands() &&
			abs (point.y - (m_rectList.b + TEXT_MARGIN)) <= STRETCH_DELTA)
		{
			::SetCursor (globalData.m_hcurStretchVert);
			return TRUE;
		}

		int nCommandsHeight = HasCommands() ? m_nCommandsHeight : 0;

		if (m_bDescriptionArea &&
			abs (point.y - (m_rectList.b + nCommandsHeight + TEXT_MARGIN)) <= STRETCH_DELTA)
		{
			::SetCursor (globalData.m_hcurStretchVert);
			return TRUE;
		}
	...
	case VK_UP:
		{
			...
			// Select prev. item:
			CPoint point (m_pSel->m_Rect.r - 1, m_pSel->m_Rect.t - 2);
	...
	case VK_DOWN:
		{
			...
			// Select next item:
			CPoint point (m_pSel->m_Rect.r - 1, m_pSel->m_Rect.b + 2);
	...
	case VK_NEXT:
		{
			...
			if (m_nVertScrollPage != 0)
			{
				...
				CPoint point (m_pSel->m_Rect.r - 1, 
					m_pSel->m_Rect.t + m_nVertScrollPage * m_nRowHeight);
...
void CBCGPPropList::EnsureVisible (CBCGPProp* pProp, BOOL bExpandParents/* = FALSE*/)
{
	...
	if (rect.t >= m_rectList.t - 1 && rect.b <= m_rectList.b)
	{
		return;
	}
	...
	if (rect.t < m_rectList.t - 1 && rect.b >= m_rectList.t - 1)
	{
		OnVScroll (SB_LINEUP, 0, NULL);
	}
	else if (rect.b > m_rectList.b && rect.t <= m_rectList.b)
	{
		OnVScroll (SB_LINEDOWN, 0, NULL);
	}
	else
	{
		OnVScroll (SB_THUMBPOSITION, rect.t / m_nRowHeight - m_nVertScrollOffset, NULL);
	}
...
void CBCGPPropList::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	for (int i = 0; i < m_arCommandRects.GetSize(); i++)
	{
		if (m_arCommandRects [i].PtInRect (point))
		{
			SendMessage(BCGM_PROPERTY_COMMAND_CLICKED, GetDlgCtrlID(), LPARAM (i));
			return;
		}
	}
}
...
void CBCGPPropList::OnNcPaint() 
{
	...
	if (m_nCommandsHeight == -1 && !m_lstCommands.IsEmpty())
	{
