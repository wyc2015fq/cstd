...
public:
	...
	virtual BOOL Create(
			...
			DWORD		dwStyle,
			UINT		nID,
			COLOR*	pPalette = NULL,	// Palette color, NULL - standard 20 colors
...
protected:
	CBCGPColorBar(	const CArray<COLORREF>& colors,
					COLORREF color, 
					LPCTSTR lpszAutoColor,
					LPCTSTR lpszOtherColor,
					LPCTSTR lpszDocColors,
					CList<COLORREF>& lstDocColors,
					int nColumns,
					int	nRowsDockHorz,
					int	nColDockVert,
...
protected:
	...
	static CMap<COLORREF,CString>	m_ColorNames;
...
protected:
	...
	static int InitColors (COLOR* pPalette, 
						CArray<COLORREF>& arColors);
	static BOOL CreatePalette (const CArray<COLORREF>& arColors,
								COLOR& palette);

	CSize GetColorGridSize (BOOL bVertDock) const;
	int GetExtraHeight (int nNumColumns) const;
	COLOR* SelectPalette (CDC* pDC);
