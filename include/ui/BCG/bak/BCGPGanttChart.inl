...
BCGP_GANTT_CHART_COLORS CBCGPGanttChart::GetActualColors() const
{
	...
	{
		colors.clrGroupBarFill = CBCGPDrawManager::MixColors (colors.clrBarFill, _RGB(48, 48, 48), 0.5f);
	}
	if (colors.clrSelectionBorder == CLR_DEFAULT)
	{
		colors.clrSelectionBorder = CBCGPDrawManager::MixColors (colors.clrSelection, _RGB(0, 0, 0), 0.5f);
	}
...
void CBCGPGanttChart::DoPaint (CDC& dc)
{
	...
	for (i = m_nFirstVisibleRow; m_nFirstVisibleRow >= 0 && i <= m_nLastVisibleRow && i < nItemRects; ++i)
	{
		...
		if (nRowTop <= rcChart.b && nRowBottom >= rcChart.t)
		{
	...
	{
		CRect rectRow (m_rectChart.l, m_arrItemRects[i].nRowTopY, m_rectChart.r, m_arrItemRects[i].nRowBottomY);
	...
	if (m_bShowEmptyRows)
	{
		...
		{
			rectRow.t = m_arrItemRects[m_nLastVisibleRow].nRowTopY;
			rectRow.b = m_arrItemRects[m_nLastVisibleRow].nRowBottomY;
		}
		while (rectRow.b < rcChart.b)
		{
			rectRow.t = rectRow.b;
			rectRow.b = rectRow.t + GetDefaultItemHeight();
	...
	if (bDraggingItems)
	{
		for (i = nFirstVisibleRow; (i <= nLastVisibleRow) && (i >= 0) && (i < nItemRects); ++i)
		{
			...
			if (pItem->IsSelected())
			{
				...
				{
					rcBar.r += m_szDragOffset.w;
				}
				CRect rcClip;
				rcClip.IntersectRect(&rcBar, &rcChart);
				if (!rcClip.IsRectEmpty() && rcBar.r > rcBar.l)
				{
...
void CBCGPGanttChart::SetupItemDrawContext(CBCGPGanttDrawContext& ctx) const
{
	...
	ctx.m_clrBorder = CBCGPDrawManager::MixColors (ctx.m_clrFill, _RGB(48, 48, 48), 0.75);
}
...
BOOL CBCGPGanttChart::HeaderCellFromPoint (CPoint pt, BCGP_GANTT_CHART_HEADER_CELL_INFO& cellInfo) const
{
	...
	while (IsTimeVisible (t))
	{
		...
		if (pt.x >= xLeft && pt.x < xRight)
		{
			...
			cellInfo.rectCell.l = xLeft;
			cellInfo.rectCell.r = xRight;
			cellInfo.rectColumn = cellInfo.rectCell;
			cellInfo.rectColumn.t = rcChart.t;
			cellInfo.rectColumn.b = rcChart.b;
...
void CBCGPGanttChart::DrawHeader (CDC& dc, const BCGP_GANTT_CHART_HEADER& header, const CRect& rcPaint) const
{
	...
	while (IsTimeVisible (t))
	{
		...
		ci.rectCell = rcPaint;
		ci.rectCell.l = TimeToClient (tmStart);
		ci.rectCell.r = TimeToClient (tmEnd);
		ci.rectColumn = ci.rectCell;
		ci.rectColumn.t = rcChart.t;
		ci.rectColumn.b = rcChart.b;
...
void CBCGPGanttChart::DrawItemBackground (CDC& dc, const CRect& rcItem, const CBCGPGanttItem* pItem) const
{
	...
	COleDateTime t = ClientToTime (rcClip.l);
	COleDateTime tEnd = ClientToTime (rcClip.r);
	...
	for (;;)
	{
		...
		{
			CRect rcFill = rcClip;
			rcFill.l = TimeToClient (tmRangeStart);
			rcFill.r = TimeToClient (tmStart); // start of current range
	...
	if (m_pUpdateInfo->bItemsLayoutChanged)
	{
		...
		while (pos != NULL)
		{
			...
			if (h > 0)
			{
				...
				r.rectBar.l = TimeToClient (pCurrentItem->GetStartTime());
				r.rectBar.r = TimeToClient (pCurrentItem->GetFinishTime());
				r.rectBar.t = y + (h - nBarHeight) / 2;
				r.rectBar.b = r.rectBar.t + nBarHeight;

				if (pCurrentItem->IsMileStone())
				{
					r.rectBar.l -= (nBarHeight / 2) + 1;
					r.rectBar.r = r.rectBar.l + nBarHeight + 1;
					r.rectBar.b += 1;
				}
...
void CBCGPGanttChart::PerformAdjustLayout()
{
	...
	m_bInAdjustLayout = TRUE;
	{
		...
		if (bHorzBarVisible)
		{
			rcClient.b -= ::GetSystemMetrics (SM_CXHSCROLL);
		}
		if (bVertBarVisible)
		{
			rcClient.r -= ::GetSystemMetrics (SM_CXVSCROLL);
		}
		...
		if (m_hdrLargeHeader.bVisible)
		{
			...
			{
				m_rectLargeHeader.t = m_rectChart.t;
				m_rectLargeHeader.b = m_rectLargeHeader.t + m_hdrLargeHeader.nHeight;
				m_rectChart.t = m_rectLargeHeader.b;
			}
			else
			{
				m_rectLargeHeader.b = m_rectChart.b;
				m_rectLargeHeader.t = m_rectLargeHeader.b - m_hdrLargeHeader.nHeight;
				m_rectChart.b = m_rectLargeHeader.t;
			}
		...
		if (m_hdrSmallHeader.bVisible)
		{
			...
			{
				m_rectSmallHeader.t = m_rectChart.t;
				m_rectSmallHeader.b = m_rectSmallHeader.t + m_hdrSmallHeader.nHeight;
				m_rectChart.t = m_rectSmallHeader.b;
			}
			else
			{
				m_rectSmallHeader.b = m_rectChart.b;
				m_rectSmallHeader.t = m_rectSmallHeader.b - m_hdrSmallHeader.nHeight;
				m_rectChart.b = m_rectSmallHeader.t;
			}
			if (m_pToolTip != NULL && m_bShowTooltips)
			{
		...
		for (int i = 0; i < n; ++i)
		{
			...
			if (r.nRowTopY <= m_rectChart.b && r.nRowBottomY >= m_rectChart.t)
			{
			...
			else if (r.nRowTopY > m_rectChart.b)
			{
...
{
	return GetVisibleChartArea().t - m_nVerticalOffset;
}
...
int CBCGPGanttChart::TimeToClient(COleDateTime time) const
{
	...
	return GetVisibleChartArea().l + (int)(0.5 + (double)t / (double)m_dtPixelTimeDelta);
}

COleDateTime CBCGPGanttChart::ClientToTime(int x) const
{
	int chartX = x - GetVisibleChartArea().l;
...
void CBCGPGanttChart::GetVisibleTimeRange(COleDateTime* ptmStart, COleDateTime* ptmEnd) const
{
	if (ptmStart != NULL)
	{
		*ptmEnd = ClientToTime (GetVisibleChartArea().l);
	}
	if (ptmEnd != NULL)
	{
		*ptmEnd = ClientToTime (GetVisibleChartArea().r);
	}
}
...
void CBCGPGanttChart::CalculateConnectorLine (CBCGPGanttConnection& link, CRect rcSourceBar, CRect rcDestBar)
{
	...
	bool bSrcAboveDest = (rcSourceBar.b < rcDestBar.t);

	// Calculate bars connection points
	CPoint ptSource;
	ptSource.y = rcSourceBar.CenterPoint().y;
	ptSource.x = (bFromSrcStart) ? rcSourceBar.l : rcSourceBar.r - 1;

	CPoint ptDest;
	ptDest.y = (bSrcAboveDest) ? rcDestBar.t - 1 : rcDestBar.b;

	if (link.m_LinkType == BCGPGANTTLINK_START_TO_START || link.m_LinkType == BCGPGANTTLINK_FINISH_TO_START)
	{
		ptDest.x = rcDestBar.l + 5;
	}
	else
	{
		ptDest.x = rcDestBar.r - 6;
	}

	if (ptDest.x < rcDestBar.l + 2 || ptDest.x > rcDestBar.r - 3 || link.m_pDestItem->IsMileStone())
	{
...
BOOL CBCGPGanttChart::Create (DWORD dwStyle, const IRECT& rect, CWnd* pParentWnd, UINT nID)
{
...
void CBCGPGanttChart::UpdateScrollBars (BOOL bValuesOnly)
{
	...
	if (!bValuesOnly)
	{
		...
		rcScrollH.t = rcScrollH.b - cxScrollHeight;
		if (bShowVert)
		{
			rcScrollH.r -= cxScrollWidth;
		}

		CRect rcScrollV = rcClient;
		rcScrollV.l = rcScrollV.r - cxScrollWidth;
		if (bShowHorz)
		{
			rcScrollV.b -= cxScrollHeight;
		}
		...
		if (bHorzBarExists)
		{
			m_wndScrollHorz.SetWindowPos (NULL, rcScrollH.l, rcScrollH.t, rcScrollH.Width(), rcScrollH.Height(), SWP_NOZORDER | SWP_NOACTIVATE);
			m_wndScrollHorz.RedrawWindow();
		}

		if (bVertBarExists)
		{
			m_wndScrollVert.SetWindowPos (NULL, rcScrollV.l, rcScrollV.t, rcScrollV.Width(), rcScrollV.Height(), SWP_NOZORDER | SWP_NOACTIVATE);
...
void CBCGPGanttChart::OnPaint(CDC* pDC)
{
	if (m_pUpdateInfo->NeedsUpdate())
	{
		PerformAdjustLayout();
	...
	if (bHorzBarVisible && bVertBarVisible)
	{
		...
		rcUnused.t = rcUnused.b - nScrollHeight;
		rcUnused.l = rcUnused.r - nScrollWidth;
		dc.FillSolidRect (rcUnused, globalData.clrBtnFace);
	}
	if (bHorzBarVisible)
	{
		rcClient.b -= nScrollHeight;
	}
	if (bVertBarVisible)
	{
		rcClient.r -= nScrollWidth;
	}
...
BOOL CBCGPGanttChart::IsPointInItemResizeArea (CPoint point, CBCGPGanttItem** ppItem) const
{
	for (int i = (int)m_arrItemRects.GetSize() - 1; i >= 0; --i)
	{
		...
		if (rect.PtInRect (point))
		{
			...
			return (rect.Width() >= 12 && point.x >= rect.r - 4);
		}
	}
...
void CBCGPGanttChart::OnMouseMove (UINT nFlags, CPoint point)
{
	...
	case ecsMoveItems:
		if (bMouseButtonsPressed)
		{
			...
			m_szDragOffset.w = point.x - m_ptDragStart.x;
...
void CBCGPGanttChart::OnLButtonUp (UINT nFlags, CPoint point)
{
	...
	case ecsSelectingItem:
		...
		break;
	case ecsMoveItems:
		if (m_szDragOffset.w != 0)
		{
			COleDateTimeSpan dtDelta = (m_dtPixelTimeDelta * m_szDragOffset.w);
