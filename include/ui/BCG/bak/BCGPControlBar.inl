...
#define strControlBarProfile  _T ("BCGControlBars")
...
BOOL CBCGPControlBar::Create(LPCTSTR lpszClassName, DWORD dwStyle, const IRECT& rect, 
...
BOOL CBCGPControlBar::CreateEx(DWORD dwStyleEx, LPCTSTR lpszClassName, DWORD dwStyle, 
							   const IRECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwBCGStyle,
							   CCreateContext* pContext)
{
	ASSERT_VALID (this);
...
{
	m_cxLeftBorder = lpRect->l;
	m_cxRightBorder = lpRect->r;
	m_cyTopBorder = lpRect->t;
	m_cyBottomBorder = lpRect->b;
}
//****
									   ...
									   bool bShow)
{
	...
	if (dockMethod == BCGP_DM_MOUSE)
	{
		...
		if (ptScreen.x > rectFinalMiniFrame.l + rectFinalMiniFrame.Width() ||
			ptScreen.x < rectFinalMiniFrame.l)
		{
			ptScreen.x = rectFinalMiniFrame.l + rectFinalMiniFrame.Width() / 2;
		}

		if (ptScreen.y > rectFinalMiniFrame.t + rectFinalMiniFrame.Height() || 
			ptScreen.y < rectFinalMiniFrame.t)
		{
			ptScreen.y = rectFinalMiniFrame.t + pParentMiniFrame->GetCaptionHeight() / 2;
		}
		...
		pParentMiniFrame->SetWindowPos (NULL, rectFinalMiniFrame.l, 
											  rectFinalMiniFrame.t,
...
BOOL CBCGPControlBar::MoveByAlignment (DWORD dwAlignment, int nOffset)
{
	...
	return (BOOL) (SetWindowPos (&wndTop, rect.l, rect.t, rect.Width(), rect.Height(), 
...
CSize CBCGPControlBar::MoveControlBar (CRect rectNew, BOOL bForceMove, HDWP& /*hdwp*/)
{
	...
	if (!bForceMove && abs (sizeNew.w) < sizeMin.w)
	{
		if (rectCurrent.l == rectNew.l ||
			rectCurrent.l != rectNew.l && 
			rectCurrent.r != rectNew.r)
		{
			rectNew.r = rectCurrent.l + sizeMin.w;
		}
		else if (rectCurrent.r == rectNew.r)
		{
			rectNew.l = rectCurrent.r - sizeMin.w;			
		}
		sizeActual.w = rectCurrent.Width() - rectNew.Width();  
	}

	if (!bForceMove && abs (sizeNew.h) < sizeMin.h)
	{
		if (rectCurrent.t == rectNew.t || 
			rectCurrent.t != rectNew.t && 
			rectCurrent.b != rectNew.b)
		{
			rectNew.b = rectCurrent.t + sizeMin.h;
		} 
		else if (rectCurrent.b == rectNew.b)
		{
			rectNew.t = rectCurrent.b - sizeMin.h;
		}

		sizeActual.h = rectCurrent.Height() - rectNew.Height();  
	}
...
int CBCGPControlBar::GetAvailableStretchSize() const
{
	...
	return IsHorizontal() ? rect.Width() - sizeMin.w: 
							 rect.Height() - sizeMin.h;
}
//****
CSize CBCGPControlBar::CalcAvailableSize (CRect rectRequired)
{
	...
	if (rectRequired.Width() < sizeMin.w)
	{
		rectRequired.r = rectRequired.l + sizeMin.w;
	}

	if (rectRequired.Height() < sizeMin.h)
	{
		rectRequired.b = rectRequired.t + sizeMin.h;
	}
	...
	sizeAvailable.w = rectRequired.Width() - rectCurrent.Width();
	sizeAvailable.h = rectRequired.Height() - rectCurrent.Height();
...
bool CBCGPControlBar::IsLeftOf (CRect rect, bool bWindowRect) const
{
	...
	{
		return (rect.l < rectBar.l);
	}
	else
	{
		return (rect.t < rectBar.t);
	}
}
//****
	...
	*ppTargetBar = DYNAMIC_DOWNCAST (CBCGPDockBar, pBaseBar);
	...
	if (m_pParentDockBar != NULL)
	{
		...
		if (m_pParentDockBar->IsHorizontal())
		{
			...
			if (rectVirtual.l < rectDockBarWnd.l && 
				rectDockBarWnd.l - rectVirtual.l > nOffset * 2 ||
				rectVirtual.r > rectDockBarWnd.r && 
				rectVirtual.r - rectDockBarWnd.r > nOffset * 2)
			{
				bTreatMouse = true;	
			}
		}
		...
		{
			if (rectVirtual.t < rectDockBarWnd.t && 
				rectDockBarWnd.t - rectVirtual.t > nOffset * 2 ||
				rectVirtual.b > rectDockBarWnd.b && 
				rectVirtual.b - rectDockBarWnd.b > nOffset * 2)
			{
				bTreatMouse = true;	
			}
	...
	else
	{
		...
		bool bMouseLeft = ptMousePos.x < rectDockBarWnd.l;
		bool bMouseRight = ptMousePos.x > rectDockBarWnd.r;
		bool bMouseTop  = ptMousePos.y < rectDockBarWnd.t;
		bool bMouseBottom = ptMousePos.y > rectDockBarWnd.b;

		double	dPixelsOnDock = nOffset;
		int		nMouseOffset  = 0;
		if (bMouseLeft)
		{
			dPixelsOnDock = ((rectBarWnd.r - ptMousePos.x) * 100. / 
								rectBarWnd.Width()) / 100. * nOffset;
			nMouseOffset = rectDockBarWnd.l - ptMousePos.x;
			
		}
		else if (bMouseRight)
		{
			dPixelsOnDock = ((ptMousePos.x - rectBarWnd.l) * 100. / 
								rectBarWnd.Width()) / 100. * nOffset;
			nMouseOffset = ptMousePos.x - rectDockBarWnd.r;
		}
		else if (bMouseTop)
		{
			dPixelsOnDock = ((rectBarWnd.b - ptMousePos.y) * 100. / 
								rectBarWnd.Height()) / 100. * nOffset;
			nMouseOffset = rectDockBarWnd.t - ptMousePos.y;
		}
		else if (bMouseBottom)
		{
			dPixelsOnDock = ((ptMousePos.y - rectBarWnd.t) * 100. / 
								rectBarWnd.Height()) / 100. * nOffset;
			nMouseOffset = ptMousePos.y - rectDockBarWnd.b;
		}
...
void CBCGPControlBar::UpdateVirtualRect()
{
	...
	m_rectVirtual.r = m_rectVirtual.l + size.w;
	m_rectVirtual.b = m_rectVirtual.t + size.h;
...
void CBCGPControlBar::UpdateVirtualRect (CSize sizeNew)
{
	...
	m_rectVirtual.r = m_rectVirtual.l + sizeNew.w;
	m_rectVirtual.b = m_rectVirtual.t + sizeNew.h;
...
void CBCGPControlBar::CalcInsideRect(CRect& rect, BOOL bHorz) const
{
	...
	if (bHorz)
	{
		rect.l += m_cxLeftBorder;
		rect.t += m_cyTopBorder;
		rect.r -= m_cxRightBorder;
		rect.b -= m_cyBottomBorder;

		if ((dwStyle & (CBRS_GRIPPER|CBRS_FLOATING)) == CBRS_GRIPPER)
		{
			if (GetExStyle() & WS_EX_LAYOUTRTL)
			{
				rect.r -= CX_BORDER_GRIPPER+CX_GRIPPER+CX_BORDER_GRIPPER;
			}
			else
			{
				rect.l += CX_BORDER_GRIPPER+CX_GRIPPER+CX_BORDER_GRIPPER;
			}
		}
	}
	else
	{
		rect.l += m_cyTopBorder;
		rect.t += m_cxLeftBorder;
		rect.r -= m_cyBottomBorder;
		rect.b -= m_cxRightBorder;

		if ((dwStyle & (CBRS_GRIPPER|CBRS_FLOATING)) == CBRS_GRIPPER)
		{
			rect.t += CY_BORDER_GRIPPER+CY_GRIPPER+CY_BORDER_GRIPPER;
		}
	}
}
//-------------------------------------------------------------------------------------//
...
void CBCGPControlBar::OnRTLChanged (BOOL bIsRTL)
{
	...
	{
		SetWindowPos (NULL, m_rectVirtual.l, m_rectVirtual.t, 
