...
BOOL CBCGPRibbonFloaty::Show (int x, int y)
{
	...
	if (!Create (m_wndRibbonBar.m_pRibbonBar, 
		x, y - size.h - ::GetSystemMetrics (SM_CYCURSOR) / 2, (HMENU) NULL))
	{
		return FALSE;
	}
...
BOOL CBCGPRibbonFloaty::ShowWithContextMenu (int x, int y, UINT uiMenuResID, CWnd* pWndOwner)
{
	...
	if (!Create (m_wndRibbonBar.m_pRibbonBar, x, y - size.h - yOffset, (HMENU) NULL))
	{
		return FALSE;
	}
	...
	if (pPopup != NULL)
	{
		...
		if (rectMenu.t < y)
		{
			SetWindowPos (NULL, rectMenu.l, rectMenu.t - size.h - yOffset, -1, -1,
		...
		if (rectMenu.t < rectFloaty.b)
		{
			SetWindowPos (NULL, rectMenu.l, rectMenu.b + yOffset, -1, -1,
				SWP_NOZORDER | SWP_NOSIZE | SWP_NOACTIVATE);
		}
	}
