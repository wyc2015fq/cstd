...
void CBCGPEditCtrl::PrepareDrawText(CDC* pDC)
{
	...
	m_nTabLogicalSize = pDC->GetTextExtent (strTabs).w;

	CString strSlashes (_T ('/'), m_nTabSize);
	int m_nSlashesLogicalSize = pDC->GetTextExtent (strSlashes).w;
...
//****
void CBCGPEditCtrl::OnPaint(CDC* pDC) 
{
	...
	// Draw side bar:
	//---------------
	if (m_nLeftMarginWidth > 0 && rectClip.l < m_nLeftMarginWidth)
	{
		CRect rectLeft = rectClient;
		rectLeft.r = rectLeft.l + m_nLeftMarginWidth;
		rectLeft.t -= nVertOffset;
	...
	if (m_bEnableLineNumbersMargin)
	{
		if (m_nLineNumbersMarginAutoWidth > 0 && rectClip.l < nMargin + m_nLineNumbersMarginAutoWidth)
		{
			CRect rectLeft = rectClient;
			rectLeft.l = rectLeft.l + nMargin;
			rectLeft.r = rectLeft.l + m_nLineNumbersMarginAutoWidth;
			rectLeft.t -= nVertOffset;
	...
	if (m_bEnableOutlineMargin)
	{
		if (m_nOutlineMarginWidth > 0 && rectClip.l < nMargin + m_nOutlineMarginWidth)
		{
			CRect rectLeft = rectClient;
			rectLeft.l = rectLeft.l + nMargin;
			rectLeft.r = rectLeft.l + m_nOutlineMarginWidth;
			rectLeft.t -= nVertOffset;
...
CFont* CBCGPEditCtrl::SelectFont (CDC* pDC, BOOL bUnderline)
{
	...
	{
		if (bUnderline && m_fontUnderline.IsValid())
		{
...
BOOL CBCGPEditCtrl::OnSetOvrMode()
{
	...
	::CreateCaret (GetSafeHwnd(), NULL, sizeCaret.w, sizeCaret.h);
	ShowCaret();
	return TRUE;
}
//****
...
void CBCGPEditCtrl::RecalcLayout()
{
	...
	m_rectText.l += m_nLeftMarginWidth;
	if (m_bEnableLineNumbersMargin)
	{
		m_rectText.l += m_nLineNumbersMarginAutoWidth;
	}
	if (m_bEnableOutlineMargin)
	{
		m_rectText.l += m_nOutlineMarginWidth;
	}
...
int CBCGPEditCtrl::HitTest (CPoint& pt, BOOL bNormalize, BOOL bIgnoreTextBounds)
{
	...
	int x = -1;
	int y = m_rectText.t; // - m_nScrollOffsetVert * m_nLineHeight;
	int nOffset = 0;
	...
	for (int i = nStrartFrom; i <= nRow; i++)
	{
		...
		if (i == nRow)
		{
			...
			int nStringExtent = GetStringExtent (&dc, strRow, strRow.GetLength()).w;
			int nXRight = m_rectText.l + 1 + nStringExtent;
			...
			else
			{
				...
				int nExtent = 0; //GetStringExtent (&dc, (LPCTSTR) strRow, nStartOffsetToCheck).w;
				...
				for (iIdx = nStartOffsetToCheck; iIdx < strRow.GetLength(); iIdx++)
				{
					...
					if (pHiddenText != NULL)
					{
						OnCalcOutlineSymbol (&dc, CPoint (m_rectText.l + 1 + nExtent, y), pHiddenText);
					...
					else if (m_bEnableSymSupport)
					{
						...
						if (LookUpSymbol (strRow, iIdx, strRow.GetLength(), chNext, symDef, nSymExtraLen))
						{
							...
							nCharWidth = imgInfo.rcImage.r - imgInfo.rcImage.l;
						}
					}
					if (nCharWidth == 0 && !m_mapCharWidth.Lookup (chNext, nCharWidth))
					{
						nCharWidth = GetStringExtent (&dc, CString (chNext), 1).w;
					...
					if (cx >= pt.x + nScrollOffsetPix + nAddition)
					{
						...
						nOffsetInRowPix = cx - (m_rectText.l + 1);
						break;
					}
...
BOOL CBCGPEditCtrl::SetCaret (int nOffset, BOOL bScrollToCaret, BOOL bRedrawOnScroll)
{
	...
	if (bScrollToCaret)
	{
		...
		int nDeltaY = 0;
		if (m_ptCaret.y < m_rectText.t + m_nLineHeight)
		{
			nDeltaY = ((m_ptCaret.y - m_rectText.t) / m_nLineHeight);
		}
		else if (m_ptCaret.y >= m_rectText.b - m_nLineHeight * 2)
		{
			nDeltaY = ((m_ptCaret.y - m_rectText.b + m_nLineHeight * 2) / m_nLineHeight);
		}
...
void CBCGPEditCtrl::OnChangeFont (CDC* pDC)
{
	...
	if (m_pSymbolsImgList != NULL)
	{
		...
		{
			int nImgHeight = imgInfo.rcImage.b - imgInfo.rcImage.t;
...
BOOL CBCGPEditCtrl::OffsetToPoint (int nOffset, CPoint& pt, LPPOINT ptRowColumn, LPINT pnScrollOffset)
{
	...
	for (i = iRowOffset; i < nOffset; )
	{
		...
		if (pHiddenText != NULL)
		{
			...
			nStringExtent += GetStringExtent (&dc, (LPCTSTR) m_strBuffer + i, pHiddenText->m_nStart - i).w;
		...
		{
			nStringExtent += GetStringExtent (&dc, (LPCTSTR) m_strBuffer + i, nOffset - i).w;
			break;
		}
	}

	pt.x = m_rectText.l + 1 + nStringExtent - nScrolledExtent - 1;

	if (pnScrollOffset != NULL)
	{
		*pnScrollOffset = m_nScrollOffsetHorz;
	
		if (pt.x < m_rectText.l + 1 + m_nMaxCharWidth)
		{
			// scroll left:
			int nDeltaX = ((pt.x - (m_rectText.l + 1)) / m_nMaxCharWidth) - m_nScrollLeftOffset;
			*pnScrollOffset = m_nScrollOffsetHorz + nDeltaX;
		}
		else if (pt.x >= m_rectText.r - m_nMaxCharWidth)
		{
			// scroll right:
			...
			int nDeltaX = ((pt.x - m_rectText.r) / m_nMaxCharWidth) + nScrollRightOffset;
			*pnScrollOffset = m_nScrollOffsetHorz + nDeltaX;
		}
	}

	pt.x = max (pt.x, m_rectText.l - m_nScrollOffsetHorz * m_nMaxCharWidth + 1);
	pt.y = m_rectText.t + (nRow - m_nScrollOffsetVert) * m_nLineHeight;
...
BOOL CBCGPEditCtrl::Up (BOOL bSetCaretToMaxColumn, BOOL bRedrawOnScroll)
{
	...
	if (m_ptCaret.y - m_nLineHeight < m_rectText.t )
	{
...
BOOL CBCGPEditCtrl::Down (BOOL bSetCaretToMaxColumn, BOOL bRedrawOnScroll)
{
	...
	if (m_ptCaret.y + m_nLineHeight >= m_rectText.b - m_nLineHeight && m_nCurRow < m_szTotalScroll.h)
	{
...
void CBCGPEditCtrl::RedrawText (int nStartRow, int nEndRow/* = -1*/)
{
	...
	if (nEndRow != -1)
	{
		...
		rectArea.t = (nStartRowVirt - m_nScrollOffsetVert) * m_nLineHeight;

		switch (nEndRow)
		{
		case 0:	// one line
			rectArea.b = rectArea.t + m_nLineHeight;
			break;
		...
		default:
			...
			rectArea.b = min (rectArea.b,
				(rectArea.t + m_nLineHeight) * (nEndRow - nStartRow + 1));
		}
	}
...
void CBCGPEditCtrl::OnFillBackground (CDC* pDC)
{
	...
	if (m_clrBack == -1)
	{
		::FillRect (pDC, &rectClient, GetSysColorBrush (COLOR_WINDOW));
	}
	else
	{
		CBrush br (m_clrBack);
...
void CBCGPEditCtrl::OnDrawSideBar (CDC* pDC, CRect rect)
{
	...
	CPoint ptTopLeft (m_rectText.l, m_rectText.t);
	CPoint ptBottomRight (m_rectText.r - 1, m_rectText.b - 1);
		...
		else if (pMarker->m_unPriority > unPriority)
		{
			...
			for (POSITION posInner = posFirst; posInner != NULL && nCnt < nCount; nCnt++)
			{
					...
					pMarkerNext->m_nLine <= nRowColumnBottom + 1)
				{
					...
					rectMarker.t = (GetVirtualRow (pMarkerNext->m_nLine) - m_nScrollOffsetVert) * m_nLineHeight;
					rectMarker.b = rectMarker.t + m_nLineHeight;
...
void CBCGPEditCtrl::OnDrawLineNumbersBar (CDC* pDC, CRect rect)
{
	...
	bmpPattern.CreateBitmap (8, 8, 1, 1, 
		(rect.t % 2) ? wHatchBits1 : wHatchBits2);

	CBrush br;
	br.CreatePatternBrush (&bmpPattern);

	CRect rectLine = rect;
	rectLine.l = rectLine.r - 1;
	...
	// Determine start and end offsets of the visible part of buffer
	CPoint ptTopLeft (m_rectText.l + 1, m_rectText.t);
	CPoint ptBottomRight (m_rectText.r - 1, m_rectText.b - 1);
	...
	{
		CRect rect (rect);
		rect.t = (nVirtualRow - m_nScrollOffsetVert) * m_nLineHeight;
		rect.b = rect.t + m_nLineHeight;
...
void CBCGPEditCtrl::OnDrawOutlineBar (CDC* pDC, CRect rect)
{
	...
	for (int nRow = nStartRow; nRow <= nEndRow && nOffset < m_strBuffer.GetLength(); nRow++)
	{
		...
		rectButton.t = (nRow - m_nScrollOffsetVert) * m_nLineHeight;
		rectButton.b = rectButton.t + m_nLineHeight;
...
void CBCGPEditCtrl::RedrawRestOfLine (int nOffset)
{
	if (nOffset >= 0)
	{
		...
		{
			CRect rect (m_rectText.l, pt.y, m_rectText.r, pt.y + m_nLineHeight);
...
void CBCGPEditCtrl::RedrawRestOfText (int nOffset)
{
	if (nOffset >= 0)
	{
		...
		else
		{
			...
			rect.l = 0;
			rect.t = pt.y - m_nScrollOffsetVert * m_nLineHeight;
...
void CBCGPEditCtrl::OnDrawText (CDC* pDC)
{
	...
	else
	{
		...
		rectText.l = m_rectText.l + 1;
		rectText.r = m_rectText.r;
	}
	...
	rectRow.b = rectRow.t + m_nLineHeight;

//////////////////////////////////////////////////////////////////////////

	// determine start and end offsets of the visible part of buffer

	CPoint ptTopLeft (rectText.l, rectText.t);
	CPoint ptBottomRight (rectText.r - 1, rectText.b - 1);
	...
	rectRow.l = rectText.l - m_nScrollOffsetHorz * m_nMaxCharWidth + 1;

	rectRow.t = bIsPrint ? rectText.t : 0;
	rectRow.b = rectRow.t + m_nLineHeight;
	int nTabOrigin = rectRow.l;
	...
	for (i = nStartOffset; i <= nEndOffset; i++)
	{
		...
		{
			nCharWidth = GetStringExtent (pDC, CString (chNext), 1).w;
		...
		if (chNext == g_chEOL && pHiddenText == NULL)
		{
			...
			if (iIdx != 0)
			{
				if (m_bKeepTabs)
				{
					rectRow.l += DrawString (pDC, lpszOutBuffer, rectRow, nTabOrigin, clrDefaultBack);
				}
				else
				{
					DrawString (pDC, lpszOutBuffer, rectRow, nTabOrigin, clrDefaultBack);
					rectRow.l += pDC->GetTextExtent (lpszOutBuffer).w;
				}
							   ...
							   COLORREF /*clrBack*/)
{
	...
	if (m_nLineVertSpacing > 0)
	{
		rect.t += m_nLineVertSpacing / 2;
	}

	if (m_bKeepTabs)  
	{
		int nOldMode = pDC->SetBkMode (TRANSPARENT);
		int nRes = pDC->TabbedTextOut (rect.l, rect.t, str, 1, &m_nTabLogicalSize, nOrigin).w;
		pDC->SetBkMode (nOldMode);
	...
	if (m_bEnableSymSupport)
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			if (LookUpSymbol (lpszString, i, nCount, chNext, symDef, nSymExtraLen))
			{
				IMAGEINFO imgInfo;
				m_pSymbolsImgList->GetImageInfo (symDef.m_nIndex, &imgInfo);
				nTotalSymLen += imgInfo.rcImage.r - imgInfo.rcImage.l;
				i += nSymExtraLen;
			}
			else
			{
...
int CBCGPEditCtrl::GetHorzRowExtent (CDC* pDC, int nOffset)
{
	if (m_nScrollOffsetHorz > 0)
	{
		...
		if (nNextRowEnd != -1 && nOffset < m_strBuffer.GetLength())
		{
			...
			return GetStringExtent (pDC, strNextRowStart, nCount).w;
		}
	}
	return 0;
}
...
void CBCGPEditCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (m_iStartSel < 0 || !IsPointInSelectionRect (point))
	{
		...
		CPoint ptSave = point;
		if (point.x >= 0 && point.x < m_rectText.l)
		{
			point.x = m_rectText.l + 1;
		}
...
void CBCGPEditCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if ((nFlags & MK_LBUTTON) && !m_bIntelliSenseMode && GetCapture() == this)
	{
		...
		if (point.x < m_rectText.l)
		{
			point.x = m_rectText.l + 1;
		}
		if (point.x > m_rectText.r)
		{
			point.x = m_rectText.r;
		}
...
BOOL CBCGPEditCtrl::GetSelectionRect (CRect& rect1, CRect& rect2, CRect& rect3) 
{
	...
	else
	{
		...
		rect1 = CRect (ptStart.x, ptStart.y, m_rectText.r, ptStart.y + m_nLineHeight);
		rect2 = CRect (m_rectText.l, ptStart.y + m_nLineHeight,m_rectText.r, ptEnd.y);
		rect3 = CRect (m_rectText.l, ptEnd.y, ptEnd.x, ptEnd.y + m_nLineHeight);
	}
...
void CBCGPEditCtrl::UpdateScrollBars()
{
	...
	siVert.nMin = 0;
	siVert.nMax = m_szTotalScroll.h;
	siVert.nPage = m_rectText.Height() / m_nLineHeight;
	siVert.nMax = m_szTotalScroll.h + (m_bScrollVertEmptyPage ? siVert.nPage : 0);
...
DROPEFFECT CBCGPEditCtrl::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point)
{
	...
	if (m_rectText.b - point.y < m_nLineHeight)
	{
		point.y += m_nLineHeight;
	}
	else if (point.y - m_rectText.t < m_nLineHeight)
	{
		point.y -= m_nLineHeight;
	}
	else if (m_rectText.r - point.x < m_nMaxCharWidth) 
	{
		point.x += m_nMaxCharWidth;
		bScrollRight = TRUE;
	}
	else if (point.x - (m_rectText.l + 1) < m_nMaxCharWidth)
	{
		point.x -= m_nMaxCharWidth;
	}
...
//****
BOOL CBCGPEditCtrl::Create(DWORD dwStyle, const IRECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext /* = NULL */) 
{
...
void CBCGPEditCtrl::OnDrawMarker (CDC* pDC, CRect rectMarker, const CBCGPEditMarker* pMarker)
{
	...
	if (pMarker->m_dwMarkerType & g_dwBCGPEdit_BookMark)
	{
		...
		if (bGradientMarkers)
		{
			...
			pDC->ExcludeClipRect (CRect (rectMarker.l, rectMarker.t, rectMarker.l + 1, rectMarker.t + 1));
			pDC->ExcludeClipRect (CRect (rectMarker.r - 1, rectMarker.t, rectMarker.r, rectMarker.t + 1));
			pDC->ExcludeClipRect (CRect (rectMarker.l, rectMarker.b - 1, rectMarker.l + 1, rectMarker.b));
			pDC->ExcludeClipRect (CRect (rectMarker.r - 1, rectMarker.b - 1, rectMarker.r, rectMarker.b));

			dm.FillGradient2 (rectMarker, _RGB (133, 168, 236), _RGB (53, 85, 144), 45);

			pDC->SelectClipRgn (NULL);

			rectMarker.DeflateRect (1, 1);
			dm.FillGradient2 (rectMarker, _RGB (255, 255, 255), _RGB (157, 187, 243), 45);
		}
		else
		{
			CBrush br (_RGB (0, 255, 255));
...
void CBCGPEditCtrl::RedrawMarkerArea (int nRow, BOOL bFullLine)
{
	...
	rectArea.r = bFullLine ? m_rectText.r : rectArea.l + m_nLeftMarginWidth;
	
	if (nRow != -1)
	{
		int nRowVirt = GetVirtualRow (nRow);
		rectArea.t = (nRowVirt - m_nScrollOffsetVert) * m_nLineHeight;
		rectArea.b = rectArea.t + m_nLineHeight;
	}
...
{
	m_clrBackSelActive = _GetSysColor(CLR_HIGHLIGHT);
	m_clrTextSelActive = _GetSysColor(CLR_HIGHLIGHTTEXT);
	...
	m_clrBackOutline = _RGB (255, 255, 255);
	m_clrLineOutline = _RGB (128, 128, 128);
	m_clrBackLineNumber = GetDefaultBackColor();
	m_clrTextLineNumber = _RGB (0, 130, 132);
	m_clrBackSidebar = _GetSysColor(CLR_3DLIGHT);
...
{
	return (m_clrText == -1) ? _GetSysColor(CLR_WINDOWTEXT) : m_clrText;
}
//****
COLORREF CBCGPEditCtrl::GetDefaultBackColor() const
{
	return (m_clrBack == -1) ? _GetSysColor(CLR_WINDOW) : m_clrBack;
}
//****
...
void CBCGPEditCtrl::RedrawOutlineArea (int nRow)
{
	...
	rectArea.l = rectArea.l + m_nLeftMarginWidth;
	rectArea.r = m_rectText.r;
										  ...
										  DWORD dwMarkerType, UINT unPriority)
{
	...
	if (clrBk == (DWORD) -1)
	{
		clrBk = _GetSysColor(CLR_HIGHLIGHTTEXT);
	}
										 ...
										 BOOL bInsideOpenBlockAtStart, BOOL bInsideOpenBlockAtEnd, BOOL bEndOfBlock)
{
	...
	if (pRowOutlineBlock != NULL)
	{
		...
		{
			pDC->MoveTo (ptCenter.x, rectButton.t);
			pDC->LineTo (ptCenter.x, rect.t + 1);
		}
		if (bInsideOpenBlockAtEnd)
		{
			pDC->MoveTo (ptCenter.x, rectButton.b);
			pDC->LineTo (ptCenter.x, rect.b - 1);
		}
	}
	else
	{
...
void CBCGPEditCtrl::OnNcPaint() 
{
		...
		GetScrollBarCtrl (SB_VERT) != NULL)
	{
		...
		rectBox.l = rectBox.r - ::GetSystemMetrics (SM_CXVSCROLL) - 1;
		rectBox.t = rectBox.b - ::GetSystemMetrics (SM_CYHSCROLL) - 1;

		COLORREF clrBkOld = dc.GetBkColor();
		dc.FillSolidRect (rectBox, _GetSysColor(CLR_BTNFACE));
...
#else
	pDC->Draw3dRect (rect, _GetSysColor(CLR_3DDKSHADOW), _GetSysColor(CLR_BTNHIGHLIGHT));
...
BOOL CBCGPEditCtrl::OnUpdateLineNumbersMarginWidth (CDC* pDC)
{
    ...
    m_nLineNumbersMarginAutoWidth = max (m_nLineNumbersMarginWidth, size.w + 3);
...
void CBCGPEditCtrl::ExportToRTF (CString& strRTF)
{
	...
	for (int i = 0; i < arrClrTbl.GetSize(); i++)
	{
		BYTE bR = GetRV (arrClrTbl [i]);
		BYTE bG = GetGV (arrClrTbl [i]);
		BYTE bB = GetBV (arrClrTbl [i]);
