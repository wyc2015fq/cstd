							...
							UINT nID)
{
	...
	if (rcBorders.l < 1)
	{
		rcBorders.l = 1;	// Otherwise, I have a problem with a "double" grippers
	}

	if (rcBorders.t < 1)
	{
		rcBorders.t = 1;	// Otherwise, I have a problem with a "double" grippers
	}
...
//****
void CBCGPToolBar::SetSizes (ISIZE sizeButton, ISIZE sizeImage)
{
	ASSERT(sizeButton.w > 0 && sizeButton.h > 0);
	...
	CSize sizeImageLarge (	(int) (.5 + m_dblLargeImageRatio * m_sizeImage.w), 
							(int) (.5 + m_dblLargeImageRatio * m_sizeImage.h));
	...
	if (m_bLargeIcons)
	{
		m_sizeCurButton.w = (int) (.5 + m_dblLargeImageRatio * m_sizeCurButton.w);
		m_sizeCurButton.h = (int) (.5 + m_dblLargeImageRatio * m_sizeCurButton.h);

		m_sizeCurImage.w = (int) (.5 + m_dblLargeImageRatio * m_sizeCurImage.w);
		m_sizeCurImage.h = (int) (.5 + m_dblLargeImageRatio * m_sizeCurImage.h);
	}
...
//****
void CBCGPToolBar::SetLockedSizes (ISIZE sizeButton, ISIZE sizeImage, BOOL bDontScale)
{
	ASSERT(sizeButton.w > 0 && sizeButton.h > 0);
	...
	CSize sizeImageLarge (
		(int) (.5 + m_dblLargeImageRatio * m_sizeImageLocked.w), 
		(int) (.5 + m_dblLargeImageRatio * m_sizeImageLocked.h));
	...
	if (m_bLargeIcons)
	{
		m_sizeCurButtonLocked.w = (int) (.5 + m_dblLargeImageRatio * m_sizeCurButtonLocked.w);
		m_sizeCurButtonLocked.h = (int) (.5 + m_dblLargeImageRatio * m_sizeCurButtonLocked.h);

		m_sizeCurImageLocked.w = (int) (.5 + m_dblLargeImageRatio * m_sizeCurImageLocked.w);
		m_sizeCurImageLocked.h = (int) (.5 + m_dblLargeImageRatio * m_sizeCurImageLocked.h);
	}
}
//****
...
BOOL CBCGPToolBar::LoadBitmapEx (CBCGPToolBarParams& params, BOOL bLocked)
{
	...
	if (params.m_uiColdResID != 0)
	{
		...
		ASSERT (m_Images.GetImageSize().h == m_ColdImages.GetImageSize().h);
	}
	else if (m_bAutoGrayInactiveImages)
	{
	...
	if (params.m_uiMenuResID != 0)
	{
		...
		ASSERT (m_MenuImages.GetImageSize().h == m_sizeMenuImage.h);
	}
								...
								BOOL bLocked)
{
	...
	if (!bDontScaleImages && globalData.GetRibbonImageScale() != 1.)
	{
		...
		sizeButton = CSize ((int)(.5 + sizeButton.w * dblImageScale), (int)(.5 + sizeButton.h * dblImageScale));
	}
...
//****
void CBCGPToolBar::GetItemRect(int nIndex, IRECT* lpRect) const
{
	ASSERT_VALID(this);

	ASSERT(nIndex >= 0 && nIndex < m_Buttons.GetCount());
	ASSERT(AfxIsValidAddress(lpRect, sizeof(IRECT)));
...
//****
void CBCGPToolBar::GetInvalidateItemRect(int nIndex, IRECT* lpRect) const
{
	ASSERT_VALID(this);
...
void CBCGPToolBar::DoPaint(CDC* pDCPaint)
{
	...
	if (bHorz)
	{
		rect.b = rect.t + GetRowHeight();
	}
	else
	{
		rect.r = rect.l + GetColumnWidth();
	}
	...
	if (dblImageScale != 1.)
	{
		...
		sizeImageDest = CSize ((int)(.5 + sizeImageDest.w * dblImageScale), (int)(.5 + sizeImageDest.h * dblImageScale));
	}
...
void CBCGPToolBar::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	else
	{
		...
		if (m_pDragButton->CanBeStretched() &&
			abs (point.x - rect.r) <= STRETCH_DELTA &&
			!m_bAltCustomizeMode)
		{
...
void CBCGPToolBar::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (IsCustomizeMode() && !m_bLocked)
	{
		if (m_bStretchButton)
		{
			...
			if (point.x - m_pDragButton->Rect().l >= BUTTON_MIN_WIDTH)
			{
				CClientDC dc (this);

				CRect rectTrackOld = m_rectTrack;
				m_rectTrack.r = point.x;
...
void CBCGPToolBar::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (IsCustomizeMode() && !m_bLocked)
	{
		if (m_bStretchButton)
		{
			...
			if (rect.Width() >= BUTTON_MIN_WIDTH &&
				abs (m_pDragButton->Rect().r - point.x) > STRETCH_DELTA)
			{
		...
		nIDCmd != 0 && nIDCmd != (UINT) -1)
	{
		...
		{
			SendMessage(WM_COMMAND, nIDCmd);    // send command
		}
	}
	else
	{
...
int CBCGPToolBar::FindDropIndex (const CPoint p, CRect& rectDrag) const
{
		...
		if (bHorz)
		{
			...
			for (pos = m_Buttons.GetHeadPosition(); pos != NULL; iButton ++)
			{
				...
				if (!pButton->IsHidden() && pButton->IsVisible())
				{
					...
					if (iButton > 0 && rect.t > rectPrev.b)
					{
						iOffset	= rect.t - rectPrev.b;
						break;
					}
				...
				if (!pButton->IsHidden() && pButton->IsVisible())
				{
					...
					if (iRow == iCursorRow)
					{
						if (point.x < rect.l)
						{
							iDragButton = iButton;
							rectDrag = rect;
							rectDrag.r = rectDrag.l;
							break;
						}
						else if (point.x <= rect.r)
						{
							rectDrag = rect;
							if (point.x - rect.l > rect.r - point.x)
							{
								iDragButton = iButton + 1;
								rectDrag.l = rectDrag.r;
							}
		...
		else
		{
			...
			for (POSITION pos = m_Buttons.GetHeadPosition(); pos != NULL; iButton ++)
			{
				...
				if (point.y < rect.t)
				{
					iDragButton = iButton;
					rectDrag = rect;
					rectDrag.b = rectDrag.t;
					break;
				}
				else if (point.y <= rect.b)
				{
					rectDrag = rect;
					if (point.y - rect.t > rect.b - point.y)
					{
						iDragButton = iButton + 1;
						rectDrag.t = rectDrag.b;
					}
					else
					{
						iDragButton = iButton;
	...
	if (iDragButton >= 0)
	{
		...
		{
			rectDrag.l = 
				max (rectClient.l, rectDrag.l - iCursorSize / 2);

			rectDrag.r = rectDrag.l + iCursorSize;
			if (rectDrag.r > rectClient.r)
			{
				rectDrag.r = rectClient.r;
				rectDrag.l = rectDrag.r - iCursorSize;
			}
		}
		else
		{
			rectDrag.t = 
				max (rectClient.t, rectDrag.t - iCursorSize / 2);

			rectDrag.b = rectDrag.t + iCursorSize;
			if (rectDrag.b > rectClient.b)
			{
				rectDrag.b = rectClient.b;
				rectDrag.t = rectDrag.b - iCursorSize;
			}
		}
	}
...
void CBCGPToolBar::DrawDragMarker (CDC* pDC)
{
	...
	for (int i = 0; i < 2; i ++)
	{
		if (bHorz)
		{
			pDC->MoveTo (m_rectDrag.l + m_rectDrag.Width() / 2 + i - 1, m_rectDrag.t);
			pDC->LineTo (m_rectDrag.l + m_rectDrag.Width() / 2 + i - 1, m_rectDrag.b);

			pDC->MoveTo (m_rectDrag.l + i, m_rectDrag.t + i);
			pDC->LineTo (m_rectDrag.r - i, m_rectDrag.t + i);

			pDC->MoveTo (m_rectDrag.l + i, m_rectDrag.b - i - 1);
			pDC->LineTo (m_rectDrag.r - i, m_rectDrag.b - i - 1);
		}
		else
		{
			pDC->MoveTo (m_rectDrag.l, m_rectDrag.t + m_rectDrag.Height() / 2 + i - 1);
			pDC->LineTo (m_rectDrag.r, m_rectDrag.t + m_rectDrag.Height() / 2 + i - 1);

			pDC->MoveTo (m_rectDrag.l + i, m_rectDrag.t + i);
			pDC->LineTo (m_rectDrag.l + i, m_rectDrag.b - i);

			pDC->MoveTo (m_rectDrag.r - i - 1, m_rectDrag.t + i);
			pDC->LineTo (m_rectDrag.r - i - 1, m_rectDrag.b - i);
		}
	}
...
BOOL CBCGPToolBar::LoadState (LPCTSTR lpszProfileName, int nIndex, UINT uiID)
{
	...
	if (m_pParentDockBar != NULL && m_pDockBarRow != NULL)
	{
		...
		m_pParentDockBar->ResizeRow (m_pDockBarRow, 
			 IsHorizontal() ? sizeCurr.h : sizeCurr.w);
	}
...
{
	BOOL bExpanded = (m_sizeLast.w < lpwndpos->cx ||
						 m_sizeLast.h < lpwndpos->cy);
	...
	if (!bStretch && IsCustomizeMode() && m_iSelected != -1 && !m_bLocked)
	{
		...
		if (HitTest (ptCursor) == m_iSelected)
		{
			...
			if (pButton->CanBeStretched() &&
				abs (ptCursor.x - pButton->Rect().r) <= STRETCH_DELTA)
			{
				bStretch = TRUE;
			}
		}
...
CSize CBCGPToolBar::StretchControlBar (int nLength, BOOL bVert)
{
	...
	{
		m_nMRUWidth = IsHorizontal() ? sizeResult.w : sizeResult.h;
	}
...
CSize CBCGPToolBar::CalcLayout(DWORD dwMode, int nLength)
{
...

	{
		CRect rect; rect.SetRectEmpty();
		CalcInsideRect(rect, (dwMode & LM_HORZ));
		sizeResult.h -= rect.Height();
		sizeResult.w -= rect.Width();

		CSize size = CBCGPBaseToolBar::CalcFixedLayout((dwMode & LM_STRETCH), (dwMode & LM_HORZ));
		sizeResult.w = max(sizeResult.w, size.w);
		sizeResult.h = max(sizeResult.h, size.h);
...
CSize CBCGPToolBar::CalcSize (BOOL bVertDock)
{
	...
	for (POSITION pos = m_Buttons.GetHeadPosition(); pos != NULL;)
	{
		...
		CSize sizeDefault (sizeGrid.w, 
				m_bDrawTextLabels ? GetButtonSize().h : sizeGrid.h);
		CSize sizeButton = pButton->OnCalculateSize (&dc, sizeDefault, !bVertDock);

		if (m_bDrawTextLabels)
		{
			sizeButton.h = m_nMaxBtnHeight;
		}
		...
		if (!bVertDock)
		{
			...
			sizeResult.w = max (cur.x + sizeButton.w, sizeResult.w);
			sizeResult.h = max (cur.y + sizeButton.h, sizeResult.h);
							   ...
							   int nColumnWidth/* = -1*/, int nRowHeight/* = -1*/)
{
	...
	for (POSITION pos = m_Buttons.GetHeadPosition(); pos != NULL;)
	{
		...
		if (x + sizeButton.w > nWidth &&
            y + sizeButton.h < nHeight &&
			!(pButton->m_nStyle & TBBS_SEPARATOR))
		{
			if (pPrevButton != NULL)
			{
...
void CBCGPToolBar::SizeToolBar (int nLength, BOOL bVert)
{
	...
	if (!bVert)
	{
		...
		size = CalcSize (bVert);
		WrapToolBar (size.w, nDefaultHeight, &dc, nColumnWidth, nRowHeight);
	}
	else
	{
		int iWidth = 32767;
		...
		if (nLength > size.h)
		{
			...
			do
			{
				iWidth += GetButtonSize().w;
				WrapToolBar (iWidth, nDefaultHeight, &dc, nColumnWidth, nRowHeight);
				size = CalcSize (FALSE);
			}
			while (nLength < size.h);
		}
...
void CBCGPToolBar::AdjustLocations()
{
	...
	if (bHorz)
	{
		iStartOffset = rectClient.l + 1;
	}
	else
	{
		iStartOffset = rectClient.t + 1;
	}
	...
	if (m_pCustomizeBtn != NULL && !IsFloating() && !IsCustomizeMode())
	{
		...
		sizeCustButton = m_pCustomizeBtn->OnCalculateSize (&dc,
			CSize (	bHorz ? sizeGrid.w : rectClient.Width(), 
			bHorz ? rectClient.Height() : sizeGrid.h), bHorz);
	...
	for (POSITION pos = m_Buttons.GetHeadPosition(); pos != NULL;)
	{
		...
		if (bHorz)
		{
			rectButton.l = iOffset;
			rectButton.r = rectButton.l + sizeButton.w;
			rectButton.t = y;
			rectButton.b = rectButton.t + sizeButton.h;
			
			iOffset += sizeButton.w;
			nRowActualWidth += sizeButton.w;
		}
		else
		{
			rectButton.l = rectClient.l;
			rectButton.r = rectClient.l + sizeButton.w;
			rectButton.t = iOffset;
			rectButton.b = iOffset + sizeButton.h;

			iOffset += sizeButton.h;
		}
			...
			!IsFloating() && !IsCustomizeMode())
		{
			...
			if ((bHorz && rectButton.r > xRight - fakeSizeCustButton.w) ||
				(!bHorz && rectButton.b > rectClient.b - fakeSizeCustButton.h))
			{
				bVisible = FALSE;
				iOffset = iOffsetPrev;
	...
	if (m_pCustomizeBtn != NULL)
	{
		...
		else
		{
			if (bHorz)
			{
				rectButton.r = xRight - 1;
				rectButton.l = rectButton.r - sizeCustButton.w + 1;
			}
			else
			{
				rectButton.b --;
				rectButton.t = rectButton.b - sizeCustButton.h;
			}
...
BOOL CBCGPToolBar::RestoreOriginalstate()
{
	...
	else if (m_pParentDockBar != NULL)
	{
		...
		{
			SetWindowPos (NULL, 0, 0, sizeCurr.w, sizeCurr.h, SWP_NOMOVE  | SWP_NOACTIVATE | SWP_NOZORDER);
			UpdateVirtualRect();
		}
...
void CBCGPToolBar::ShowCommandMessageString (UINT uiCmdId)
{
	...
	SendMessage(WM_SETMESSAGESTRING, (WPARAM) uiTrackId);
}
//****
...
void CBCGPToolBar::AdjustLayout()
{
	...
	if (pBar != NULL)
	{
		...
		else
		{
			...
			sizeMin.h -= rect.Height();
			sizeMin.w -= rect.Width();

			sizeMin.w = max(sizeMin.w, size.w);
			sizeMin.h = max(sizeMin.h, size.h);

			bandInfo.wMinChild = m_sizeButton.w;
			bandInfo.hMinChild = sizeMin.h;

			bandInfo.wIdeal = sizeMin.w;
...
//****
void CBCGPToolBar::SetMenuSizes (ISIZE sizeButton, ISIZE sizeImage)
{
	ASSERT(sizeButton.w > 0 && sizeButton.h > 0);
	...
	//-----------------------------------------------------------------
	ASSERT(sizeButton.w >= sizeImage.w + 6);
	ASSERT(sizeButton.h >= sizeImage.h + 6);
...
{
	CSize size = (m_sizeMenuImage.w == -1) ? m_sizeImage : m_sizeMenuImage;

	if (globalData.GetRibbonImageScale() != 1.)
	{
		size = CSize (
			(int)(.5 + size.w * globalData.GetRibbonImageScale()),
			(int)(.5 + size.h * globalData.GetRibbonImageScale()));
	}
...
void CBCGPToolBar::SetLargeIcons (BOOL bLargeIcons/* = TRUE*/)
{
	...
	if (m_bLargeIcons)
	{
		m_sizeCurButton.w = (int) (.5 + m_dblLargeImageRatio * m_sizeButton.w);
		m_sizeCurButton.h = (int) (.5 + m_dblLargeImageRatio * m_sizeButton.h);

		m_sizeCurImage.w = (int) (.5 + m_dblLargeImageRatio * m_sizeImage.w);
		m_sizeCurImage.h = (int) (.5 + m_dblLargeImageRatio * m_sizeImage.h);
	}
	else
	{
	...
	for (POSITION posTlb = gAllToolbars.GetHeadPosition(); posTlb != NULL;)
	{
		...
		if (CWnd::FromHandlePermanent (pToolBar->m_hWnd) != NULL)
		{
			...
			if (pToolBar->m_bLocked)
			{
				...
				if (m_bLargeIcons)
				{
					if (pToolBar->m_sizeCurButtonLocked.w != (int) (.5 + m_dblLargeImageRatio * pToolBar->m_sizeButtonLocked.w) ||
						pToolBar->m_sizeCurButtonLocked.h != (int) (.5 + m_dblLargeImageRatio * pToolBar->m_sizeButtonLocked.h) ||
						pToolBar->m_sizeCurImageLocked.w != (int) (.5 + m_dblLargeImageRatio * pToolBar->m_sizeImageLocked.w) ||
						pToolBar->m_sizeCurImageLocked.h != (int) (.5 + m_dblLargeImageRatio * pToolBar->m_sizeImageLocked.h))
					{
						pToolBar->m_sizeCurButtonLocked.w = (int) (.5 + m_dblLargeImageRatio * pToolBar->m_sizeButtonLocked.w);
						pToolBar->m_sizeCurButtonLocked.h = (int) (.5 + m_dblLargeImageRatio * pToolBar->m_sizeButtonLocked.h);
...
int CBCGPToolBar::CalcMaxButtonHeight()
{
	...
	for (POSITION pos = m_Buttons.GetHeadPosition(); pos != NULL;)
	{
		...
		if (pButton->m_bTextBelow)
		{
			...
			nMaxBtnHeight = max (nMaxBtnHeight, sizeButton.h);
		}
	}

	m_bDrawTextLabels = (nMaxBtnHeight > GetButtonSize().h);
	dc.SelectObject (pOldFont);
...
{
	GetItemRect ((int) wParam, (IRECT*)lParam );
	return TRUE;
}
//****
...
void CBCGPToolBar::AdjustSize()
{
	...
	if (pParent != NULL && pParent->GetSafeHwnd() != NULL)
	{
		...
		if (sizeCurr.w == 32767 || sizeCurr.h == 32767)
		{
			CRect rectParent;
			...
			if (sizeCurr.w == 32767)
			{
				sizeCurr.w = rectParent.Width();

				if (m_nMaxLen != 0)
				{
					sizeCurr.w = min (sizeCurr.w, m_nMaxLen);
				}
			}
			else
			{
				sizeCurr.h = rectParent.Height();

				if (m_nMaxLen != 0)
				{
					sizeCurr.h = min (sizeCurr.h, m_nMaxLen);
				}
			}
		}
		...
		else
		{
			if (IsCustomizeMode())
			{
				if (rect.Height() != sizeCurr.h && sizeCurr.h != 32767 && 
					IsHorizontal() ||
					rect.Width() != sizeCurr.w && sizeCurr.w != 32767 && 
					!IsHorizontal())
				{
					SetWindowPos (NULL, 0, 0, sizeCurr.w, sizeCurr.h, SWP_NOMOVE  | SWP_NOACTIVATE | SWP_NOZORDER);
					UpdateVirtualRect();
			...
			else
			{
					...
					!IsHorizontal())
				{
					...
					int nNewWidth = max (sizeMin.w, sizeCurr.w);
					int nNewHeight = max (sizeMin.h, sizeCurr.h);
		...
		if (!IsFloating() && IsVisible() && m_pParentDockBar != NULL && m_pDockBarRow != NULL)
		{
			...
			if (rectWnd.Height() != sizeCurr.h && bIsHorz)
			{
				rectWnd.b = rectWnd.t + sizeCurr.h;
				bResize = TRUE;
			}
			else if (rectWnd.Width() != sizeCurr.w && !bIsHorz)
			{
				rectWnd.r = rectWnd.l + sizeCurr.w;
				bResize = TRUE;
			}
										...
										BOOL bHorz)
{
	...
	if (pButton->m_bWrap && bHorz)
	{
		rectSeparator.l = rectClient.l;
		rectSeparator.r = rectClient.r;

		rectSeparator.t = pButton->Rect().b;
		rectSeparator.b = rectSeparator.t + LINE_OFFSET;
	}	
}
//****
...
void CBCGPToolBar::SetTwoRowsWithBrother()
{
	...
	if(pRowThis == pRowBrother)
	{
		...
		if(m_bElderBrother)
		{
			...
			CBCGPDockBarRow* pNewRow = pDockBar->AddRow (pos, szBarSize.h);
