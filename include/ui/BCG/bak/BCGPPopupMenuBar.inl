...
static const int uiRemovePopupTimerEvent_pmb = 2;
...
void CBCGPPopupMenuBar::AdjustLocations()
{
	...
	int y = rectClient.t + iVertMargin - m_iOffset * GetRowHeight();
	...
	sizeMenuButton.h = max (sizeMenuButton.h, 
	...
	for (POSITION pos = m_Buttons.GetHeadPosition(); pos != NULL;)
	{
		...
		{
			rectButton.l = x + m_xSeparatorOffsetLeft;
			rectButton.r = right + rectClient.l - m_xSeparatorOffsetRight;
			rectButton.b = rectButton.t + iSeparatorHeight;
		}
		else
		{
			...
			rectButton.l = x;
			rectButton.r = right + rectClient.l;
...
CSize CBCGPPopupMenuBar::CalcSize (BOOL /*bVertDock*/)
{
	...
	else
	{
		...
		for (POSITION pos = m_Buttons.GetHeadPosition(); pos != NULL;)
		{
				...
				!CBCGPToolBar::IsCustomizeMode())
			{
				...
				{
					size.h = max (column.h, size.h);
					size.w += column.w + iHorzMargin;
					m_arColumns.Add (size.w);
				}
				column.w = column.h = 0;
			}
			///////////////////////////////

			int iHeight = sizeButton.h;
			...
			else
			{
				...
				{
					sizeButton.w += iMinTabSpace;
				}

				pButton->m_bWholeText = 
					(m_iMaxWidth <= 0 || 
					sizeButton.w <= m_iMaxWidth - 2 * iHorzMargin);

				column.w = max (sizeButton.w, column.w);
			}
	...
	size.h += 2 * iVertMargin;
	size.w += 2 * iHorzMargin;

	if (m_iMaxWidth > 0 && size.w > m_iMaxWidth)
	{
		size.w = m_iMaxWidth;
	}

	if (m_iMinWidth > 0 && size.w < m_iMinWidth)
	{
		size.w = m_iMinWidth;
	}
...
void CBCGPPopupMenuBar::OnChangeHot (int iHot)
{
	...
	if (pMenuButton != pCurrPopupMenu)
	{
		...
		if (pCurrPopupMenu != NULL)
		{
			...
			{
				KillTimer (uiRemovePopupTimerEvent_pmb);
				m_pDelayedClosePopupMenuButton = NULL;
			...
			else
			{
				...
				SetTimer (uiRemovePopupTimerEvent_pmb, 
...
BOOL CBCGPPopupMenuBar::OnKey(UINT nChar)
{
	...
	if (pNewSelButton != pOldSelButton)
	{
		...
		if (rectNew.t < rectClient.t || rectNew.b > rectClient.b)
		{
...
void CBCGPPopupMenuBar::OnTimer(UINT_PTR nIDEvent) 
{
	...
	else if (nIDEvent == uiRemovePopupTimerEvent_pmb)
	{
		KillTimer (uiRemovePopupTimerEvent_pmb);
...
void CBCGPPopupMenuBar::OnFillBackground (CDC* pDC)
{
	...
	for (POSITION pos = m_Buttons.GetHeadPosition(); pos != NULL;)
	{
		...
		if (CBCGPToolBar::IsCommandRarelyUsed (pButton->m_nID))
		{
			...
			{
				rectRarelyUsed.b = pButton->Rect().b;
				bDraw = TRUE;
			}
		}
		else
		{
			if (!bFirstRarelyUsedButton)
			{
				rectRarelyUsed.b = pButton->Rect().t;
				bDraw = TRUE;
			}
	...
	if (nOldStyle != nStyle)
	{
		...
		if (!(nOldStyle & nStyle & TBBS_PRESSED))
		{
			...
			if (pMenuButton != NULL && bWasChecked != bChecked)
			{
				...
				rectImage.InflateRect (afxData.wBorder2 * 2, afxData.hBorder2 * 2);
...
int CBCGPPopupMenuBar::GetGutterWidth()
{
	...
	int cx = 0;
	int cxImage = CBCGPToolBar::GetMenuImageSize().w;
