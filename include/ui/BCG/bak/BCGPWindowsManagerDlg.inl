...
void CBCGPWindowsManagerDlg::FillWindowList(void)
{
	...
	while (hwndT != NULL)
	{
		...
		int cxCurr = dcList.GetTextExtent (szWndTitle).w; 
...
void CBCGPWindowsManagerDlg::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDIS) 
{
	...
	{
		CBCGPDialog::OnDrawItem(nIDCtl, lpDIS);
		return;
	}
	...
	// Draw icon:
	//-----------
	CBitmap* hIcon = (CBitmap*)(LONG_PTR) GetClassLongPtr ((HWND) lpDIS->itemData, GCLP_HICONSM);
	if (hIcon != NULL)
	{
		CRect rectIcon = rect;
		...
		::DrawIconEx (pDC, rectIcon.l, rectIcon.t, 
