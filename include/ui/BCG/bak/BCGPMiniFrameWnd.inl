...
//--------------------------------------------------------------------------------------//
BOOL CBCGPMiniFrameWnd::Create(LPCTSTR lpszWindowName, DWORD dwStyle, const IRECT& rect, CWnd* pParentWnd, CCreateContext* pContext) 
{
...
BOOL CBCGPMiniFrameWnd::CreateEx(DWORD dwStyleEx, LPCTSTR lpszWindowName, 
								 DWORD dwStyle, const IRECT& rect, 
...
CBCGPDockingControlBar* CBCGPMiniFrameWnd::DockControlBar (BOOL& bWasDocked)
{
	...
	else if ((docktype & BCGP_DT_STANDARD) != 0 || (docktype & BCGP_DT_SMART) != 0)
	{
		...
		if (!rectFinal.IsRectEmpty() && 
			...
			!bWasDocked)
		{
			SetWindowPos (NULL, rectFinal.l, rectFinal.t, 
				...
				if (!bCtrlHeld)
				{
					...
					if (!bOnCaption)
					{
						...
						if (pBar != NULL)
						{
							...
							if (rcWnd.PtInRect (ptScreen) && (ptScreen.y - rcWnd.t) < pBar->GetCaptionHeight())
							{
...
//--------------------------------------------------------------------------------------//
void CBCGPMiniFrameWnd::OnMoving(UINT fwSide, IRECT* pRect) 
{
...
void CBCGPMiniFrameWnd::OnCheckRollState()
{
	...
	{
		rectWnd.b = rectWnd.t + m_nHeightBeforeRollUp;
		SetWindowPos (NULL, rectWnd.l,  rectWnd.t, rectWnd.Width(), rectWnd.Height(),
...
void CBCGPMiniFrameWnd::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp) 
{
	...
	lpncsp->rgrc[0].t += m_nCaptionHeight + rectBorderSize.t;
	lpncsp->rgrc[0].b -= rectBorderSize.b;
	lpncsp->rgrc[0].l += rectBorderSize.l;
	lpncsp->rgrc[0].r -= rectBorderSize.r;
...
void CBCGPMiniFrameWnd::OnNcPaint() 
{
	...
	for (POSITION pos = m_lstCaptionButtons.GetHeadPosition(); pos != NULL;)
	{
		...
		if (!pBtn->m_bHidden)
		{
			if (pBtn->m_bLeftAlign)
			{
				if (xBtnsRight == -1)
				{
					xBtnsRight = pBtn->GetRect().r + 2;
				}
				else
				{
					xBtnsRight = max (xBtnsRight, pBtn->GetRect().r + 2);
				}
			}
			else
			{
				if (xBtnsLeft == -1)
				{
					xBtnsLeft = pBtn->GetRect().l;
				}
				else
				{
					xBtnsLeft = min (xBtnsLeft, pBtn->GetRect().l);
				}
			}
		}
	}
	...
	if (xBtnsLeft != -1)
	{
		rectText.r = xBtnsLeft;
	}
	if (xBtnsRight != -1)
	{
		rectText.l = xBtnsRight;
	}
	...
	if (m_bMemDC)
	{
		...
		if (nClipType != NULLREGION)
		{
			...
			dc.BitBlt (rectClip.l, rectClip.t, rectClip.Width(), rectClip.Height(),
						   &dcMem, rectClip.l, rectClip.t, SRCCOPY);
		}
...
void CBCGPMiniFrameWnd::OnDrawBorder (CDC* pDC)
{
	...
	rectWnd.OffsetRect (rectBorderSize.l, m_nCaptionHeight + rectBorderSize.t);
...
BCGNcHitTestType CBCGPMiniFrameWnd::HitTest (CPoint point, BOOL bDetectCaption)
{
	...
	CRect rectCaption (rectWnd.l + rectBorderSize.l, rectWnd.t + rectBorderSize.t, 
						 rectWnd.r - rectBorderSize.r, 
						 rectWnd.t + rectBorderSize.t + m_nCaptionHeight);
	...
	if (bEnableCornerArrows)
	{
		...
		rectBorder.SetRect (rectWnd.l, rectWnd.t, rectWnd.l + nCursorWidth, rectWnd.t + nCursorHeight);
		...
		// top border
		rectBorder.SetRect (rectWnd.l + nCursorWidth, rectWnd.t, 
					  rectWnd.r - nCursorWidth, rectWnd.t + rectBorderSize.t);
		if (rectBorder.PtInRect (point))
		{
			return HTTOP;
		}
		...
		// top right border
		rectBorder.SetRect (rectWnd.r - nCursorWidth, rectWnd.t, 
					  rectWnd.r, rectWnd.t + nCursorHeight);
		if (rectBorder.PtInRect (point))
		{
			return HTTOPRIGHT;
		}
		...
		// right border
		rectBorder.SetRect (rectWnd.r - rectBorderSize.r, rectWnd.t + nCursorHeight, 
					  rectWnd.r, rectWnd.b - nCursorHeight);
		if (rectBorder.PtInRect (point))
		{
			return HTRIGHT;
		}
		...
		// bottom right
		rectBorder.SetRect (rectWnd.r - nCursorWidth, rectWnd.b - nCursorHeight, 
					  rectWnd.r, rectWnd.b);
		if (rectBorder.PtInRect (point))
		{
		...
		// bottom
		rectBorder.SetRect (rectWnd.l + nCursorWidth, rectWnd.b - rectBorderSize.b, 
					  rectWnd.r - nCursorWidth, rectWnd.b);
		if (rectBorder.PtInRect (point))
		{
			return HTBOTTOM;
		}
		...
		// bottom left
		rectBorder.SetRect (rectWnd.l, rectWnd.b - nCursorHeight, 
					  rectWnd.l + nCursorWidth, rectWnd.b);
		if (rectBorder.PtInRect (point))
		{
			return HTBOTTOMLEFT;
		}
		...
		// left
		rectBorder.SetRect (rectWnd.l, rectWnd.t + nCursorHeight, 
					  rectWnd.l + rectBorderSize.l, rectWnd.b - nCursorHeight);
	...
	else
	{
		...
		// top border
		rectBorder.SetRect (rectWnd.l, rectWnd.t, rectWnd.r, 
					  rectWnd.t + rectBorderSize.t);
		if (rectBorder.PtInRect (point))
		{
		...
		// left
		rectBorder.SetRect (rectWnd.l, rectWnd.t, 
					  rectWnd.l + rectBorderSize.l, rectWnd.b);
		...
		// bottom
		rectBorder.SetRect (rectWnd.l, rectWnd.b - rectBorderSize.b, 
					  rectWnd.r, rectWnd.b);
		if (rectBorder.PtInRect (point))
		{
		...
		// right border
		rectBorder.SetRect (rectWnd.r - rectBorderSize.r, rectWnd.t, 
					  rectWnd.r, rectWnd.b);
		if (rectBorder.PtInRect (point))
		{
...
//--------------------------------------------------------------------------------------//
void CBCGPMiniFrameWnd::OnSizing(UINT fwSide, IRECT* pRect) 
{
	...
	int nNewHeight = rect.Height() - 
		rectBorderSize.t - rectBorderSize.b - m_nCaptionHeight;

	int nNewWidth  = rect.Width() - rectBorderSize.l - rectBorderSize.r;
	...
	int nXBarDelta = sizeBar.w - sizeBarOld.w;
	int nYBarDelta = sizeBar.h - sizeBarOld.h;
	...
	if (nXBarDelta != 0)
	{
		...
		{
			pRect->r = pRect->l + rectOldWnd.Width() + nXBarDelta;
		}
		else
		{
			pRect->l = pRect->r - rectOldWnd.Width() - nXBarDelta;
		}
	}
	else
	{
		fwSide == WMSZ_RIGHT ? 
			pRect->r = rectOldWnd.r : pRect->l = rectOldWnd.l;
	}
	...
	if (nYBarDelta != 0)
	{
		...
		{
			pRect->b = pRect->t + rectOldWnd.Height() + nYBarDelta;
		}
		else
		{
			pRect->t = pRect->b - rectOldWnd.Height() - nYBarDelta;
		}
	}
	else
	{
		fwSide == WMSZ_BOTTOM ? 
			pRect->b = rectOldWnd.b : pRect->t = rectOldWnd.t;
	}
	...
	{
		SetWindowPos (NULL, pRect->l, pRect->t, 
							pRect->r - pRect->l, pRect->b - pRect->t,
							SWP_NOZORDER | SWP_NOACTIVATE);
	}
}
//--------------------------------------------------------------------------------------//
void CBCGPMiniFrameWnd::OnSize(UINT nType, int cx, int cy) 
{
	...
	SetWindowPos (NULL, rect.l, rect.t, 0, 0, SWP_NOZORDER | SWP_NOSIZE | SWP_NOACTIVATE);
...
void CBCGPMiniFrameWnd::SizeToContent()
{
	...
	if (pBar != NULL)
	{
		...
		CRect rectClient;
		GetClientRect (rectClient);
		int nXDelta = rectClient.Width() - sizeBar.w;
		int nYDelta = rectClient.Height() - sizeBar.h;
...
void CBCGPMiniFrameWnd::OnBarRecalcLayout()
{
	...
	if (pBar != NULL)
	{
		if (CBCGPControlBar::m_bHandleMinSize)
		{
			...
			if (rect.Width() < size.w)
			{
				rect.r = rect.l + size.w;
				bSizeChanged = TRUE;
			}
			if (rect.Height() < size.h)
			{
		...
		pBar->SetWindowPos (NULL, rect.l, rect.t, rect.Width(), rect.Height(), 
...
CBCGPCaptionButton* CBCGPMiniFrameWnd::FindButton (CPoint point) const
{
	...
	{
		point.x = rectWnd.r - point.x + rectWnd.l;
	}
...
void CBCGPMiniFrameWnd::ArrangeCaptionButtons()
{
	...
	CPoint ptOrgRight (rectCaption.r - g_nCaptionButtonMargin, 
					rectCaption.t + (rectCaption.Height() - btnSize.h) / 2);
	CPoint ptOrgLeft (rectCaption.l + g_nCaptionButtonMargin, 
					rectCaption.t + (rectCaption.Height() - btnSize.h) / 2);
	...
	for (POSITION pos = m_lstCaptionButtons.GetTailPosition(); pos != NULL; i++)
	{
		...
		if (!pBtn->m_bHidden)
		{
			...
			{
				pBtn->Move (ptOrgLeft);
				ptOrgLeft.x += btnSize.w;
				ptOrgLeft.x += g_nCaptionButtonSpacing;
			}
			else
			{
				ptOrgRight.x -= btnSize.w;

				ptOrgRight.x = max (ptOrgRight.x, rectCaption.l);
...
void CBCGPMiniFrameWnd::GetCaptionRect (CRect& rectCaption) const
{
	...
	rectCaption = CRect (rectWnd.l + rectBorderSize.l, rectWnd.t + rectBorderSize.t, 
						 rectWnd.r - rectBorderSize.r, 
						 rectWnd.t + rectBorderSize.t + m_nCaptionHeight);
...
void CBCGPMiniFrameWnd::CalcMinSize (CSize& sizeBase, MINMAXINFO FAR* lpMMI)
{
	...
	lpMMI->ptMinTrackSize.x = max (sizeBase.w, GetCaptionButtonTotalWidth()) + 
									rectBorderSize.l + rectBorderSize.r;

	lpMMI->ptMinTrackSize.x = max (lpMMI->ptMinTrackSize.x, m_sizeMinSize.w);

	lpMMI->ptMinTrackSize.y = sizeBase.h + m_nCaptionHeight + 
								rectBorderSize.t + rectBorderSize.b;

	lpMMI->ptMinTrackSize.y = max (lpMMI->ptMinTrackSize.y, m_sizeMinSize.h);
}
//****
...
void CBCGPMiniFrameWnd::Serialize (CArchive& ar)
{
	...
	else
	{
		...
		if (m_bRolledUp)
		{
			rect.b = rect.t + m_nHeightBeforeRollUp;
		}
...
BOOL CBCGPMiniFrameWnd::StartTearOff (CBCGPPopupMenu* pMenu)
{
	...
	int x = (rectFrame.l + rectFrame.r) / 2;
	int xOffset = x - rectFrame.l;

	int y = rectFrame.t + 5;
	int yOffset = y - rectFrame.t;
...
void CBCGPMiniFrameWnd::RecalcCaptionHeight()
{
	m_nCaptionHeight = GetSystemMetrics (SM_CYSMCAPTION) + 
		CBCGPVisualManager::GetInstance()->GetCaptionButtonExtraBorder().h;
	m_sizeMinSize.w = m_sizeMinSize.h = m_nCaptionHeight + 15;
}
//****
...
void CBCGPMiniFrameWnd::BringAllMiniFramesToWorkArea (int nVisibleWidth)
{
	for (POSITION pos = m_lstFrames.GetHeadPosition(); pos != NULL;)
	{
		...
		if (pWnd->GetSafeHwnd() != NULL)
		{
			...
			pWnd->SetWindowPos (NULL, rectWindow.l, rectWindow.t, 0, 0, 
