...
void CBCGPReportRow::OnDrawPreview (CDC* pDC, CRect rect)
{
	...
	rectDescr.t++;
	rectDescr.l = rect.l + m_pWndList->GetExtraHierarchyOffset() + dx;
...
void CBCGPReportRow::OnDrawExpandBox (CDC* pDC, CRect rect)
{
	...
	if (m_pWndList->m_bIsPrinting)
	{
		...
		if (IsGroup())
		{
			rectFill.DeflateRect (0, 0, 0, szOne.h);
		}
...
void CBCGPReportRow::OnPrintName (CDC* pDC, CRect rect)
{
	...
	if (rectClipText.IntersectRect (&rectClipText, &rectClip))
	{
		...
		int nVCenterOffset = (rectText.Height() - pDC->GetTextExtent (m_strName).h + nDescent) / 2;

		pDC->SetTextAlign (TA_LEFT | TA_TOP);
		pDC->ExtTextOut (rectText.l, rectText.t + nVCenterOffset, ETO_CLIPPED, &rectClipText, m_strName, NULL);
...
int CBCGPReportRow::CalcPreview (const CRect& rect)
{
		...
		pReportWnd->IsRowExtraHeightAllowed() && pReportWnd->m_bPreviewRow)
	{
		...
		rectDescr.t = rect.b;
		rectDescr.b = rectDescr.b + nMaxHeight;
		...
		if (nHeight > 0)
		{
			int nBottomMargin = (m_pWndList->m_bIsPrinting ? 2 * m_pWndList->m_PrintParams.m_pageInfo.m_szOne.h : 2);
...
//****
void CBCGPReportCtrl::OnPaint(CDC* pDC) 
{
#ifndef _BCGPGRID_STANDALONE
