...
public:
	...
	virtual BOOL SetTabHicon (int iTab, CBitmap* hIcon);
	...
	void SetAutoColors (const CArray<COLORREF>& arColors);
	const CArray<COLORREF>& GetAutoColors() const
	{
	...
	BOOL SetTabsOrder (const CArray<int>& arOrder);
	...
	void ResetImageList();
	BOOL IsIconAdded (CBitmap* hIcon, int& iIcon) 
	{
		return (m_mapAddedIcons.Lookup (hIcon, iIcon));
	}

	void AddIcon (CBitmap* hIcon, int iIcon) {m_mapAddedIcons.SetAt (hIcon, iIcon);}
...
protected:
	...
	CMap<CBitmap*,CBitmap*,int,int> m_mapAddedIcons;	// Icons already loaded into the image list
