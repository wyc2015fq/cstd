...
{
	m_AeroMargins.wLeftWidth = 0;
	m_AeroMargins.wRightWidth = 0;
	m_AeroMargins.hTopHeight = 0;
	m_AeroMargins.hBottomHeight = 0;
...
//****
BOOL CBCGPDlgImpl::ProcessMouseClick (IPOINT pt)
{
...
//****
BOOL CBCGPDlgImpl::ProcessMouseMove (IPOINT pt)
{
	if (!CBCGPToolBar::IsCustomizeMode() &&
		CBCGPPopupMenu::GetActiveMenu() != NULL)
	{
...
BOOL CBCGPDlgImpl::EnableAero (BCGPMARGINS& margins)
{
	...
	while (pWndChild != NULL)
	{
		...
		if (bIsAeroEnabled && HasAeroMargins())
		{
			...
			if (m_AeroMargins.wLeftWidth != 0)
			{
				CRect rectAero = rectClient;
				rectAero.r = rectAero.l + m_AeroMargins.wLeftWidth;
			...
			if (!bGlass && m_AeroMargins.wRightWidth != 0)
			{
				CRect rectAero = rectClient;
				rectAero.l = rectAero.r - m_AeroMargins.wRightWidth;
			...
			if (!bGlass && m_AeroMargins.hTopHeight != 0)
			{
				CRect rectAero = rectClient;
				rectAero.b = rectAero.t + m_AeroMargins.hTopHeight;
			...
			if (!bGlass && m_AeroMargins.hBottomHeight != 0)
			{
				CRect rectAero = rectClient;
				rectAero.t = rectAero.b - m_AeroMargins.hBottomHeight;
...
{
	return 	m_AeroMargins.wLeftWidth != 0 ||
			m_AeroMargins.wRightWidth != 0 ||
			m_AeroMargins.hTopHeight != 0 ||
			m_AeroMargins.hBottomHeight != 0;
}
...
void CBCGPDlgImpl::ClearAeroAreas (CDC* pDC)
{
	...
	if (m_AeroMargins.wLeftWidth != 0)
	{
		...
		pDC->FillSolidRect (rectAero, _RGB (0, 0, 0));
	}

	if (m_AeroMargins.wRightWidth != 0)
	{
		CRect rectAero = rectClient;
		rectAero.l = rectAero.r - m_AeroMargins.wRightWidth;
	...
	if (m_AeroMargins.hBottomHeight != 0)
	{
		CRect rectAero = rectClient;
		rectAero.t = rectAero.b - m_AeroMargins.hBottomHeight;
...
BOOL CBCGPDlgImpl::OnNcCalcSize(BOOL /*bCalcValidRects*/, NCCALCSIZE_PARAMS FAR* lpncsp)
{
	...
	if (IsOwnerDrawCaption())
	{
		lpncsp->rgrc[0].t += GetCaptionHeight();
	}
...
CRect CBCGPDlgImpl::GetCaptionRect()
{
	...
	CRect rectCaption (	rectWnd.l + szSystemBorder.w, 
						...
						rectWnd.t + szSystemBorder.h + GetCaptionHeight());
	...
	if (rectCaption.PtInRect (point))
	{
		if ((m_Dlg.GetExStyle() & WS_EX_TOOLWINDOW) == 0)
		{
			...
			rectSysMenu.r = rectSysMenu.l + ::GetSystemMetrics (SM_CXSMICON) + 2 +
				(m_Dlg.IsZoomed() ? szSystemBorder.w : 0);
...
void CBCGPDlgImpl::OnChangeVisualManager()
{
	...
	else
	{
		...
		if (bZoomed && bChangeBorder)
		{
			...
			{
				params.rgrc[0].t += GetCaptionHeight();
			}

			m_Dlg.SetWindowPos (NULL, params.rgrc[0].l, params.rgrc[0].t, 
