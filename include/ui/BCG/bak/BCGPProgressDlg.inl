...
void CBCGPProgressDlg::OffsetWnd (CWnd* pWnd, int cy)
{
	...
	pWnd->SetWindowPos (NULL, rectWindow.l, rectWindow.t + cy, -1, -1,
...
BOOL CBCGPProgressDlg::OnInitDialog() 
{
		...
		m_Params.m_nAnimationHeight > 0)
	{
		...
		rectAnim.t += cyOffset;
		rectAnim.b = rectAnim.t + m_Params.m_nAnimationHeight;
	...
	if (m_Params.m_strMessage2.IsEmpty())
	{
		...
		int nOffset = rectLine.t - rectMessage2.t;
...
BOOL CBCGPProgressDlg::OnEraseBkgnd(CDC* pDC) 
{
	...
	else
	{
		...
		if (m_yLine >= 0)
		{
			...
			rectBottom.t = m_yLine;

			pDC->FillRect (rectBottom, &globalData.brBtnFace);

			CBCGPDrawManager dm (*pDC);
			dm.DrawLine (rectBottom.l, m_yLine, rectBottom.r, m_yLine, globalData.clrBtnShadow);
		}
	}
...
void CBCGPProgressDlg::RedrawHeader()
{
	...
	rectHeader.b = rectHeader.t + m_Params.m_nHeaderHeight;

	RedrawWindow (rectHeader, NULL, RDW_UPDATENOW | RDW_INVALIDATE);
}
//****
void CBCGPProgressDlg::OnPaint(CDC* pDC) 
{
