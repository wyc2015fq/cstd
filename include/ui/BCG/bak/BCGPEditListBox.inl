...
//****
void CBCGPEditListBase::OnPaint(CDC* pDC) 
{
...
void CBCGPEditListBase::AdjustLayout()
{
	...
	m_rectCaption.b = m_rectCaption.t +
		max (tm.tmHeight * 4 / 3, m_sizeButton.h);

	int x = rectClient.r - 1 - m_sizeButton.w;
	for (POSITION pos = m_lstButtons.GetTailPosition(); pos != NULL;)
	{
		CBCGPButton* pButton = m_lstButtons.GetPrev (pos);
		ASSERT (pButton != NULL);

		pButton->MoveWindow (x, rectClient.t + 1, m_sizeButton.w, 
			m_rectCaption.Height() - 2);
		x -= m_sizeButton.w;
	}

	pWndList->MoveWindow (	rectClient.l,
							rectClient.t + m_rectCaption.Height(),
							rectClient.Width(),
