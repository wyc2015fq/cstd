...
void CBCGPRibbonSeparator::OnDraw (CDC* pDC)
{
	...
	if (m_bIsHoriz)
	{
		rect.t = rect.CenterPoint().y;
		rect.b = rect.t + 1;
	}
	else
	{
		rect.l = rect.CenterPoint().x;
		rect.r = rect.l + 1;
	...
	if (pParentBar != NULL)
	{
		...
		if (m_bIsDefaultMenuLook && m_pParentMenu != NULL)
		{
			...
			rect.l += 2 * CBCGPVisualManager::GetInstance()->GetMenuImageMargin() + 2;
...
CSize CBCGPBaseRibbonElement::GetKeyTipSize (CDC* pDC)
{
	...
	sizeText.w = max (sizeText.w, sizeMin.w);
	sizeText.h = max (sizeText.h, sizeMin.h);

	return CSize (sizeText.w + 10, sizeText.h + 2);
}
//****
...
BOOL CBCGPBaseRibbonElement::IsScrolledOut() const
{
	...
	return (m_rect.b > rectMenu.b || m_rect.t < rectMenu.t);
}
//****
								 ...
								 COLORREF clrText)
{
	...
	if (CBCGPToolBarImages::m_bIsDrawOnGlass)
	{
		...
		CBCGPVisualManager::GetInstance()->DrawTextOnGlass (pDC, strText, rectText, uiDTFlags, 0, 
			bIsZoomed ? _RGB (255, 255, 255) : clrText);

		return pDC->GetTextExtent (strText).h;
	}
