...
void CBCGPDragFrameImpl::MoveDragFrame (BOOL bForceMove)
{
	...
	if (abs (ptOffset.x) < szSencitivity.w && 
		abs (ptOffset.y) < szSencitivity.h && 
	...
	if (m_rectDrag.IsRectEmpty())
	{
		...
		else if (m_pDraggedWnd->IsKindOf (RUNTIME_CLASS (CBCGPControlBar)))
		{
			...
			{
				m_rectDrag.r = 
					m_rectDrag.l + pBar->m_recentDockInfo.m_rectRecentFloatingRect.Width();
				m_rectDrag.b = 
					m_rectDrag.t + pBar->m_recentDockInfo.m_rectRecentFloatingRect.Height();
			}

			if (!m_rectDrag.PtInRect (m_ptHot))
			{
				int nOffset = m_rectDrag.l - m_ptHot.x;
...
void CBCGPDragFrameImpl::DrawFrameTab (CBCGPDockingControlBar* pTargetBar, BOOL bErase)
{
	...
	{
		rectWnd.t += globalData.GetTextHeight();  
		rectSmallTab.b = rectSmallTab.t + globalData.GetTextHeight(); 
		rectSmallTab.l += 10;
		rectSmallTab.r = rectSmallTab.l + 40;
	}
	else
	{
		rectWnd.b -= globalData.GetTextHeight();  
		rectSmallTab.t = rectSmallTab.b - globalData.GetTextHeight(); 
		rectSmallTab.l += 10;
		rectSmallTab.r = rectSmallTab.l + 40;
	}

	if (rectSmallTab.r >= rectWnd.r)
	{
		rectSmallTab.r = rectWnd.r - nThikness - 4;
	}
	...
	if (CBCGPTabbedControlBar::m_bTabsAlwaysTop)
	{
		rectLine.SetRect (rectSmallTab.l + nThikness, 
							rectSmallTab.b - nThikness,  
							rectSmallTab.r - nThikness, 
							rectSmallTab.b + nThikness);
	}
	else
	{
		rectLine.SetRect (rectSmallTab.l + nThikness, 
							rectSmallTab.t  - nThikness,  
							rectSmallTab.r - nThikness, 
							rectSmallTab.t + nThikness);
	}
