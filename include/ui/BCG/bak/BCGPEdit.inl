...
void CBCGPEdit::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp) 
{
	...
	{
		lpncsp->rgrc [0].r -= m_nBrowseButtonWidth;
	}
}
//****
void CBCGPEdit::OnNcPaint() 
{
	...
	m_rectBtn.l = m_rectBtn.r -  m_nBrowseButtonWidth;

	CRect rectClient;
	GetClientRect (rectClient);
	ClientToScreen (&rectClient);

	m_rectBtn.OffsetRect (rectClient.r + m_nBrowseButtonWidth - rectWindow.r, 0);
	m_rectBtn.t += rectClient.t - rectWindow.t;
	m_rectBtn.b -= rectWindow.b - rectClient.b;

	CRect rect = m_rectBtn;
	rect.OffsetRect (-rectWindow.l, -rectWindow.t);
...
void CBCGPEdit::OnDrawBrowseButton (CDC* pDC, CRect rect, BOOL bIsButtonPressed, BOOL bHighlight)
{
	...
	if (m_ImageBrowse.IsValid())
	{
		...
		CPoint ptImage;
		ptImage.x = rect.CenterPoint().x - m_sizeImage.w / 2;
		ptImage.y = rect.CenterPoint().y - m_sizeImage.h / 2;
...
void CBCGPEdit::OnChangeLayout()
{
	...
	m_nBrowseButtonWidth = max (20, m_sizeImage.w + 8);
	...
	case BrowseMode_Calculator:
		{
			...
			if (!m_pCalcPopup->Create (this, rectWindow.l, rectWindow.b, NULL, TRUE))
			{
				ASSERT (FALSE);
				m_pCalcPopup = NULL;
			}
...
//****
void CBCGPEdit::SetBrowseButtonImage (CBitmap* hIcon, BOOL bAutoDestroy)
{
	if (m_ImageBrowse.IsValid())
	{
		m_ImageBrowse.DeleteImageList();
...
//****
void CBCGPEdit::SetBrowseButtonImage (CBitmap* hBitmap, BOOL bAutoDestroy)
{
	if (m_ImageBrowse.IsValid())
	{
		m_ImageBrowse.DeleteImageList();
	...
	CBitmap* hbmpCopy = (CBitmap*) ::CopyImage (hBitmap, IMAGE_BITMAP, 0, 0, 0);
	m_ImageBrowse.Add (CBitmap::FromHandle (hbmpCopy), _RGB (192, 192, 192));
...
void CBCGPEdit::SetBrowseButtonImage (UINT uiBmpResId)
{
	...
	SetBrowseButtonImage ((CBitmap*) ::CopyImage (images.GetImageWell(), IMAGE_BITMAP, 0, 0, 0), TRUE /* bAutoDestroy */);
	m_bDefaultImage = FALSE;
}
//****
...
void CBCGPEdit::SetIntenalImage()
{
	...
	m_ImageBrowse.Add (CBitmap::FromHandle (hbmp), _RGB (255, 0, 255));
...
//****
void CBCGPEdit::OnPaint(CDC* pDC) 
{
	if (!m_bOnGlass)
	{
		Default();
