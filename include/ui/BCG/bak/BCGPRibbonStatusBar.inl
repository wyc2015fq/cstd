...
public:
	...
	virtual CSize GetIntermediateSize (CDC* pDC)
	{
		...
		size.w += size.h * 2;	// Reserve space for checkbox
		...
		if (!strValue.IsEmpty())
		{
			size.w += pDC->GetTextExtent (strValue).w + 4 * m_szMargin.w;
		}
...
BOOL CBCGPRibbonStatusBar::GetExtendedArea (CRect& rect) const
{
	...
	for (int i = 0; i < m_arExElements.GetSize(); i++)
	{
		...
		{
			rect = rectClient;
			rect.l = rectElem.l - xExtAreaMargin;
	...
	{
		rect = rectClient;
		rect.l = rect.r - m_rectSizeBox.Width() - xExtAreaMargin;
...
void CBCGPRibbonStatusBar::RecalcLayout()
{
	...
	if (m_cxSizeBox != 0)
	{
		...
		if (m_bBottomFrame)
		{
			...
			m_rectResizeBottom.t = m_rectResizeBottom.b - GetSystemMetrics (SM_CYSIZEFRAME);
		}
	}
	else
	{
	...
	for (i = (int) m_arExElements.GetSize() - 1; i >= 0; i--)
	{
		...
		if (xMax - sizeElem.w < rect.l || !pElem->m_bIsVisible)
		{
		...
		else
		{
			...
			{
				pElem->SetRect (CRect (xMax - sizeElem.w, rect.t, xMax, rect.b));
			}
			else
			{
				int yOffset = max (0, (rect.Height() - sizeElem.h) / 2);

				pElem->SetRect (CRect (CPoint (xMax - sizeElem.w, rect.t + yOffset), sizeElem));
			}

			xMax = pElem->GetRect().l;
		}
	...
	else
	{
		...
		for (i = 0; i < (int) m_arElements.GetSize(); i++)
		{
			...
			if (x + sizeElem.w > xMax || !pElem->m_bIsVisible)
			{
				pElem->SetRect (CRect (0, 0, 0, 0));
			}
			else
			{
				if (pElem->CanBeStretched())
				{
					pElem->SetRect (CRect (x, rect.t, x + sizeElem.w, rect.b));
				}
				else
				{
					sizeElem.h	= min (sizeElem.h, rect.Height());
					int yOffset = max (0, (rect.Height() - sizeElem.h) / 2);

					pElem->SetRect (CRect (CPoint (x, rect.t + yOffset), sizeElem));
				}
...
//****
void CBCGPRibbonStatusBar::OnPaint(CDC* pDC) 
{
