...
//****
ISIZE CBCGPCustomizeMenuButton::OnCalculateSize (CDC* pDC, const CSize& sizeDefault, BOOL bHorz)
{
	if (bSeparator)
	{
		return CSize(0,  4);
	...
	int nTotalHeight = sizeStandard.h + 2;
									   ...
									   BOOL bDrawBorder, BOOL bGrayDisabledButtons)
{
	...
	if (bSeparator)
	{
		...
		rcSeparator.l = 2*CBCGPToolBar::GetMenuImageSize().w + 
	...
	if (bHighlight && m_bIsEnabled)
	{
		...
		if (!CBCGPVisualManager::GetInstance()->IsHighlightWholeMenuItem() &&
			!bIsResetItem)
		{
			rcHighlight.l += 2 * CBCGPToolBar::GetMenuImageSize().w + 
	...
	rectCheck.l += CBCGPVisualManager::GetInstance()->GetMenuImageMargin() + 1;
	rectCheck.r = rectCheck.l + sizeMenuImage.w + 
