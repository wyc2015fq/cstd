...
CSize CBCGPRibbonEdit::GetIntermediateSize (CDC* pDC)
{
	...
	if (m_pParent != NULL)
	{
		...
		int nSmallButtonHeight = m_pParent->GetImageSize (FALSE).h + 6;

		m_szMargin.h = max (2, (nSmallButtonHeight - nTextHeight) / 2);
	}

	int cy = nTextHeight + 2 * m_szMargin.h;
	...
	if (!m_bQuickAccessMode && !m_bFloatyMode)
	{
		...
		{
			m_nLabelImageWidth += sizeImageSmall.w + 2 * m_szMargin.w;
			cy = max (cy, sizeImageSmall.h);
		}

		if (m_sizeTextRight.w > 0)
		{
			m_nLabelImageWidth += m_sizeTextRight.w + 2 * m_szMargin.w;
			cy = max (cy, m_sizeTextRight.h);
		}
...
CSize CBCGPRibbonEdit::GetCompactSize (CDC* pDC)
{
	...
	int nLabelWidth = m_sizeTextRight.w;
	m_sizeTextRight.w = 0;

	CSize size = GetIntermediateSize (pDC);

	m_sizeTextRight.w = nLabelWidth;
	return size;
}
//****
...
void CBCGPRibbonEdit::OnDraw (CDC* pDC)
{
	...
	m_rectCommand.l = m_rect.l = m_rect.r - cx;
...
void CBCGPRibbonEdit::OnDrawLabelAndImage (CDC* pDC)
{
	...
	if (sizeImageSmall != CSize (0, 0))
	{
		...
		rectImage.l += m_szMargin.w;
		rectImage.r = rectImage.l + sizeImageSmall.w;
		rectImage.OffsetRect (0, max (0, (rectImage.Height() - sizeImageSmall.h) / 2));
	...
	if (m_sizeTextRight.w > 0 && !m_bCompactMode)
	{
									  ...
									  BOOL /*bHighlighted*/)
{
	...
	if (m_bHasDropDownList)
	{
		...
		rectDropButton.l = m_rectCommand.r;
	...
	else
	{
		...
		rectCaret.r = rectCaret.l + 7;

		CPen* pOldPen = (CPen*) pDC->SelectStockObject (BLACK_PEN);

		pDC->MoveTo (rectCaret.l, rectCaret.t);
		pDC->LineTo (rectCaret.r, rectCaret.t);

		pDC->MoveTo (rectCaret.CenterPoint().x, rectCaret.t);
		pDC->LineTo (rectCaret.CenterPoint().x, rectCaret.b);

		pDC->MoveTo (rectCaret.l, rectCaret.b);
		pDC->LineTo (rectCaret.r, rectCaret.b);
...
void CBCGPRibbonEdit::ReposEditCtrl()
{
	...
	if (rectCommandOld != m_rectCommand || !m_pWndEdit->IsWindowVisible())
	{
		...
		rectEdit.l = rectEdit.r - cx;

		if (m_bHasSpinButtons)
		{
			rectEdit.DeflateRect (m_szMargin.w, m_szMargin.h, 2, m_szMargin.h);
		}
		else
		{
			rectEdit.DeflateRect (m_szMargin.w, m_szMargin.h);
		}
...
//****
void CBCGPRibbonEditCtrl::OnPaint(CDC* pDC) 
{
	CRect rect;
	GetClientRect (rect);
