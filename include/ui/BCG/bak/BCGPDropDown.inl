...
//****
void CBCGPDropDownFrame::OnPaint(CDC* pDC) 
{
...
void CBCGPDropDownFrame::RecalcLayout (BOOL /*bNotify*/) 
{
	...
	size.w += iBorderSize * 3;
	size.h += iBorderSize * 3;
	...
	if (m_x + size.w > rectScreen.r)
	{
			...
			(pParentBar->IsHorizontal()) == 0)
		{
			...
			m_x = rectParent.l - size.w;
		}
		else
		{
			m_x = rectScreen.Width() - size.w - 1;
		}
	}
	
	if (m_y + size.h > rectScreen.b)
	{
		m_y -= size.h;
	...
	SetWindowPos (NULL, m_x, m_y, size.w, size.h,
		SWP_NOZORDER | SWP_NOACTIVATE);
}
//****
										...
										BOOL bHorz, BOOL bCustomizeMode, BOOL bHighlight, BOOL bDrawBorder, BOOL bGrayDisabledButtons)
{
	...
	rectParent.r -= nActualArrowSize / 2 + 1;
	...
	if (m_pToolBar != NULL)
	{
		...
		if (globalData.GetRibbonImageScale() != 1.)
		{
			...
			sizeDest = CSize ((int)(.5 + sizeDest.w * dblImageScale), (int)(.5 + sizeDest.h * dblImageScale));
		}

		if (!m_bLocalUserButton)
		{
			m_pToolBar->m_ImagesLocked.SetTransparentColor (globalData.clrBtnFace);
			m_pToolBar->m_ImagesLocked.PrepareDrawImage (ds, sizeDest);
		}
		else
		{
	...
	CPoint triang [] = 
	{
		CPoint( rect.r - nActualArrowSize + offset - 1, rect.b - nHalfArrowSize + offset + 1),
		CPoint (rect.r - nHalfArrowSize + offset + 1, rect.b - nHalfArrowSize + offset + 1),
		CPoint (rect.r - nHalfArrowSize + offset + 1, rect.b - nActualArrowSize + offset - 1)
	};
...
BOOL CBCGPDropDownToolbarButton::DropDownToolbar (CWnd* pWnd)
{
	...
	{
		point = CPoint (m_rect.l - 1, m_rect.b);
		pWnd->ClientToScreen (&point);
	}
...
//****
ISIZE CBCGPDropDownToolbarButton::OnCalculateSize (CDC* pDC, const CSize& sizeDefault, BOOL bHorz)
{
