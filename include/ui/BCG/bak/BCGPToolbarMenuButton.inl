			...
			BOOL bDrawBorder, BOOL bGrayDisabledButtons)
{
	...
	if (m_bDrawDownArrow)
	{
		if (bHorz)
		{
			rectParent.r -= sizeImage.w + nSeparatorSize - 2 + sizeExtra.w;
			m_rectArrow.l = rectParent.r + 1;
		...
		{
			rectParent.b -= sizeImage.h + nSeparatorSize - 1;
			m_rectArrow.t = rectParent.b;
		}
	}
	...
	if (m_bDrawDownArrow)
	{
			...
			m_nID != 0 && m_nID != (UINT) -1 && !m_bMenuOnly)
		{
			...
			if (bHorz)
			{
				rectSeparator.r = rectSeparator.l + nSeparatorSize;
			}
			else
			{
				rectSeparator.b = rectSeparator.t + nSeparatorSize;
			}
...
//****
ISIZE CBCGPToolbarMenuButton::OnCalculateSize (CDC* pDC, const CSize& sizeDefault, BOOL bHorz)
{
	m_bHorz = bHorz;
	...
	if (m_bDrawDownArrow || m_bMenuMode)
	{
		...
		{
			nArrowSize = (bHorz) ? 
				CBCGPMenuImages::Size().w : CBCGPMenuImages::Size().h;
	...
	if (pParentMenu != NULL)
	{
		size.h = pParentMenu->GetRowHeight();
	...
	if (m_bMenuMode)
	{
		size.w += sizeDefault.w + 2 * TEXT_MARGIN;
	}
					...
					BOOL bContentOnly)
{
	...
	CBitmap* hDocIcon = CBCGPTabWnd::GetDocumentIcon (m_nID);
	...
	CRect rectImage;
	rectImage = rect;
	rectImage.l += CBCGPVisualManager::GetInstance()->GetMenuImageMargin();
	rectImage.r = rectImage.l + sizeMenuImage.w + 
	...
	if (CBCGPVisualManager::GetInstance()->IsHighlightWholeMenuItem())
	{
		...
		rectFrameBtn.l += rectOffset.l;
		rectFrameBtn.t += rectOffset.t;
		rectFrameBtn.b -= rectOffset.b;
		rectFrameBtn.r = rectImage.r;
	}
	else
	{
	...
	if (!bDisableImage && (IsDrawImage() && pImages != NULL) || hDocIcon != NULL)
	{
		...
		CPoint ptImageOffset (
			(rectImage.Width() - sizeMenuImage.w) / 2, 
			(rectImage.Height() - sizeMenuImage.h) / 2);
		...
		if ((m_nStyle & TBBS_PRESSED) || 
			...
			bCustomizeMode)
		{
			CRect rectIcon (CPoint (rectImage.l + ptImageOffset.x, 
							rectImage.t + ptImageOffset.y),
							sizeMenuImage);
			...
			else
			{
				...
				{
					pt.x += max (0, (rectImage.Width() - pImages->GetImageSize().w) / 2);
					pt.y += max (0, (rectImage.Width() - pImages->GetImageSize().h) / 2);
				}
		...
		if (!bImageIsReady)
		{
			...
			else
			{
				...
				pImages->Draw (pDC, 
					rectImage.l + ptImageOffset.x, rectImage.t + ptImageOffset.y, 
	...
	//-------------------------------
	CRect rectText = rect;
	rectText.l = rectFrameBtn.r + CBCGPVisualManager::GetInstance()->GetMenuImageMargin() + 2;
	...
	rectText.l += TEXT_MARGIN;

	if (!m_bWholeText)
	{
		CString strEllipses (_T("..."));
		while (strText.GetLength() > 0 &&
			pDC->GetTextExtent (strText + strEllipses).w > rectText.Width())
		{
	...
	if (!strAccel.IsEmpty())
	{
		...
		rectAccel.r -= TEXT_MARGIN + sizeImage.w;
	...
	if (m_nID == (UINT) -1 || m_bDrawDownArrow || m_bMenuOnly)
	{
		...
		if (HasButton())
		{
			...
			m_rectButton.l = m_rectButton.r - pDC->GetTextExtent (strTriangle).w;
...
BOOL CBCGPToolbarMenuButton::OpenPopupMenu (CWnd* pWnd)
{
	...
	if (pParentMenu != NULL)
	{
		...
		{
			point.x = rectParent.l - nMenuGap;
			dropDir = CBCGPPopupMenu::DROP_DIRECTION_LEFT;
		}
		else
		{
			point.x = rectParent.r + nMenuGap;
		...
		(pParentMenuBar->IsHorizontal()) == 0)
	{
		...
		//------------------------------------------------
		point = CPoint (m_rect.r, m_rect.t);
		pWnd->ClientToScreen (&point);
	...
	else
	{
		...
		{
			point = CPoint (m_rect.r - 1, m_rect.t);
		}
		else
		{
			if (m_pPopupMenu->IsRightAlign())
			{
				point = CPoint (m_rect.r - 1, m_rect.b - 1);
			}
			else
			{
				point = CPoint (m_rect.l, m_rect.b - 1);
			}
		}
...
{
	xOffsetLeft = CBCGPToolBar::GetMenuImageSize().w / 2 + TEXT_MARGIN;
	xOffsetRight = CBCGPMenuImages::Size().w;
}
//****
...
void CBCGPToolbarMenuButton::GetImageRect (CRect& rectImage)
{
	...
	rectImage.r = rectImage.l + 
					CBCGPToolBar::GetMenuImageSize().w + CBCGPVisualManager::GetInstance()->GetMenuImageMargin();
}
//****
...
//****
void CBCGPToolbarMenuButton::DrawDocumentIcon (CDC* pDC, const CRect& rectImage, CBitmap* hIcon)
{
	ASSERT_VALID (pDC);

	int cx = globalData.m_sizeSmallIcon.w;
	int cy = globalData.m_sizeSmallIcon.h;
	...
	::DrawIconEx (pDC,
		rectImage.l + x, rectImage.t + y, hIcon,
