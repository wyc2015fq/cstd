...
void CBCGPToolBoxPage::ReposButtons()
{
	...
	const int nButtonWidth = m_Mode == ToolBoxPageMode_Images ? 
		m_sizeImage.w + 3 * TOOLBOX_IMAGE_MARGIN :
		rectClient.Width();

	const int nButtonHeight = m_Mode == ToolBoxPageMode_Images ? 
		m_sizeImage.h + 3 * TOOLBOX_IMAGE_MARGIN :
		max (globalData.GetTextHeight(), m_sizeImage.h) + TOOLBOX_IMAGE_MARGIN;

	int x = rectClient.l;
	int y = rectClient.t + TOOLBOX_IMAGE_MARGIN - 
	...
	for (int i = 0; i < m_arButtons.GetSize(); i++)
	{
		...
		if ((m_nScrollValue > 0 ? rectButton.b : rectButton.t) >= rectClient.t
			&& rectButton.b <= rectClient.b)
		{
		...
		if (x + nButtonWidth > rectClient.r)
		{
			x = rectClient.l;
			y += nButtonHeight;
...
int CBCGPToolBoxPage::GetMaxHeight() const
{
	...
	{
		nButtonHeight = max (globalData.GetTextHeight(), m_sizeImage.h) + TOOLBOX_IMAGE_MARGIN;
	}
	else
	{
		nButtonHeight = m_sizeImage.h + 3 * TOOLBOX_IMAGE_MARGIN;
		const int nButtonWidth = m_sizeImage.w + 3 * TOOLBOX_IMAGE_MARGIN;
								 ...
								 CRuntimeClass* pButtonClass)
{
	...
	CBitmap* hBitmap = m_Images.GetImageWell();

	BITMAP bmp;
	::GetObject (hBitmap, sizeof (BITMAP), (LPVOID) &bmp);

	m_sizeImage.w = nImageWidth;
	...
	{
		double dblImageScale = globalData.GetRibbonImageScale();
		m_sizeImage = CSize ((int)(.5 + m_sizeImage.w * dblImageScale), (int)(.5 + m_sizeImage.h * dblImageScale));
	}
...
//****
int CBCGPToolBoxPage::AddButton (LPCTSTR lpszText, CBitmap* hIcon)
{
	ASSERT_VALID (this);
...
void CBCGPToolBoxButton::OnDraw (CDC* pDC)
{
	...
	if (m_pImages != NULL && m_iImageIndex >= 0)
	{
		...
		{
			double dblImageScale = globalData.GetRibbonImageScale();
			sizeImage = CSize ((int)(.5 + sizeImage.w * dblImageScale), (int)(.5 + sizeImage.h * dblImageScale));
		}
		...
		{
			rectImage.r = rectImage.l + sizeImage.w + 10;
		}
