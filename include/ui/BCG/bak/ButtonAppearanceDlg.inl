...
void CButtonAppearanceDlg::OnAddImage() 
{
	...
	try
	{
		...
		if (!bitmap.CreateCompatibleBitmap (&dc, sizeImage.w, sizeImage.h))
		{
		...
		CRect rect (0, 0, sizeImage.w, sizeImage.h);
		memDC.FillRect (CRect (0, 0, sizeImage.w, sizeImage.h),
		...
		int iImageIndex = m_pImages->AddImage ((CBitmap*) bitmap);
		if (iImageIndex < 0)
		{
			AfxMessageBox (IDP_BCGBARRES_CANNT_CREATE_IMAGE);
			return;
...
void CButtonAppearanceDlg::OnEditImage() 
{
	...
	try
	{
		...
		const COLORREF clrGrayStd = _RGB (192, 192, 192);

		CBitmap* pOldBitmap = memDC.SelectObject (&bitmap);
		COLORREF clrTransparent = m_pImages->SetTransparentColor (clrGrayStd);

		memDC.FillSolidRect (CRect (0, 0, sizeImage.w, sizeImage.h), clrGrayStd);
		...
		m_pImages->UpdateImage (m_iSelImage, (CBitmap*) bitmap);
...
BOOL CButtonAppearanceDlg::OnInitDialog() 
{
	...
	if (pImages != NULL)
	{
		...
		sizePreview.w = min (sizePreview.w, sizeImage.w);
		sizePreview.h = min (sizePreview.h, sizeImage.h);
	}

	m_rectDefaultImage.r = m_rectDefaultImage.l + sizePreview.w;
	m_rectDefaultImage.b = m_rectDefaultImage.t + sizePreview.h;
...
//****
void CButtonAppearanceDlg::OnPaint(CDC* pDC) 
{
	...
	pImages->Draw (&dc, m_rectDefaultImage.l, m_rectDefaultImage.t, iImage);
