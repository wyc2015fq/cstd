...
int CBCGPMDIChildWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if ((GetStyle() & WS_SYSMENU) == 0)
	{
		...
		if (m_pMDIFrame != NULL && !m_pMDIFrame->IsMDITabbedGroup())
		{
			...
			SetWindowPos (NULL, rect.l, rect.t, rect.Width(), rect.Height(), 
...
//****
CBitmap* CBCGPMDIChildWnd::GetFrameIcon() const
{
	ASSERT_VALID (this);
	...
	if (hIcon == NULL)
	{
		hIcon = (CBitmap*)(LONG_PTR) GetClassLongPtr (GetSafeHwnd(), GCLP_HICONSM);
	}
...
void CBCGPMDIChildWnd::AdjustClientArea()
{
	...
	if (pChildWnd != NULL)
	{
		...
		if (!CBCGPDockManager::m_bFullScreenMode)
		{
			...
			rectClientAreaBounds.l += m_rectBorder.l;
			rectClientAreaBounds.t  += m_rectBorder.t;
			rectClientAreaBounds.r -= m_rectBorder.r;
			rectClientAreaBounds.b -= m_rectBorder.b;
			
			
			
			pChildWnd->SetWindowPos (&wndBottom, rectClientAreaBounds.l, 
											rectClientAreaBounds.t, 
	...
	if (!m_bIsMinimized)
	{
			...
			m_dockManager.IsOLEContainerMode())
		{
			if (pView != NULL && pView->IsKindOf (RUNTIME_CLASS (CBCGPPrintPreviewView)))
			{
				...
				pView->SetWindowPos (NULL, rectClient.l, rectClient.t, 
...
//****
void CBCGPMDIChildWnd::OnSizing(UINT fwSide, IRECT* pRect) 
{
...
void CBCGPMDIChildWnd::OnStyleChanged(int nStyleType, LPSTYLESTRUCT lpStyleStruct)
{
	...
	else if (!bIsInMDITabbedGroup)
	{
		...
		{
			SetWindowPos (NULL, m_rectOriginal.l, m_rectOriginal.t, 
