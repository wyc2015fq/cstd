...
void CBCGPShadowWnd::Repos() 
{
	...
	if (!IsWindowVisible())
	{
		...
		SetWindowPos (&CWnd::wndTop, rectWindow.l, rectWindow.t, rectWindow.Width(), rectWindow.Height(),
			SWP_NOACTIVATE);
	...
	{
		SetWindowPos (NULL, rectWindow.l, rectWindow.t, rectWindow.Width(), rectWindow.Height(),
...
void CBCGPPopupMenu::Initialize()
{
	...
	m_iScrollBtnHeight = CBCGPMenuImages::Size().h + 2 * 
...
BOOL CBCGPPopupMenu::Create (CWnd* pWndParent, int x, int y, HMENU hMenu, BOOL bLocked, BOOL bOwnMessage)
{
	...
	if (x == -1 && y == -1)	// Undefined position
	{
		if (pWndParent != NULL)
		{
			...
			m_ptLocation = CPoint (rectParent.l + 5, rectParent.t + 5);
		}
		else
		{
			m_ptLocation = CPoint (0, 0);
	...
	if (m_bRightAlign)
	{
		m_ptLocation.x -= m_FinalSize.w - 1;
		m_ptLocationInitial = m_ptLocation;
	...
	if (bIsAnimate)
	{
		...
		{
		case UNFOLD:
			m_AnimSize.w = pMenuBar->GetColumnWidth();

		case SLIDE:
			m_AnimSize.h = pMenuBar->GetRowHeight();
			break;
		}
...
void CBCGPPopupMenu::OnSize(UINT nType, int cx, int cy) 
{
	...
	if (m_iLogoWidth > 0)
	{
		...
		{
		case MENU_LOGO_LEFT:
			rectLogo.r = rectLogo.l + nBorderSize + m_iLogoWidth;
			break;

		case MENU_LOGO_RIGHT:
			rectLogo.l = rectLogo.r - nBorderSize - m_iLogoWidth;
			break;

		case MENU_LOGO_TOP:
			rectLogo.b = rectLogo.t + nBorderSize + m_iLogoWidth;
			break;

		case MENU_LOGO_BOTTOM:
			rectLogo.t = rectLogo.b - nBorderSize - m_iLogoWidth;
			break;
		}
...
//****
void CBCGPPopupMenu::OnPaint(CDC* pDC) 
{
...
void CBCGPPopupMenu::RecalcLayout (BOOL /*bNotify*/) 
{
	...
	if (m_bResizeTracking)
	{
		if (bRTL)
		{
			m_sizeCurrent.w = min (
				m_sizeCurrent.w, 
				m_ptLocation.x - rectScreen.l - 2 * nBorderSize);
		}
		else
		{
			m_sizeCurrent.w = min (
				m_sizeCurrent.w, 
				rectScreen.r - m_ptLocation.x - 2 * nBorderSize);
		}

		m_sizeCurrent.h = min (
			m_sizeCurrent.h, 
			rectScreen.b - m_ptLocation.y - 2 * nBorderSize - m_rectResize.Height());
	}

	//---------------------------------------
	// Normalize location in the screen area:
	//---------------------------------------
	m_ptLocation.x = max (rectScreen.l, min (m_ptLocation.x, rectScreen.r));

	if (m_pParentBtn == NULL)
	{
		m_ptLocation.y = max (rectScreen.t, min (m_ptLocation.y, rectScreen.b));
	}
	...
	if (!m_bResizeTracking && !m_bWasResized)
	{
		size.w += nBorderSize * 2;
		size.h += nBorderSize * 2;
	...
	if (m_pMenuCustomizationPage != NULL)
	{
		size.h += ::GetSystemMetrics (SM_CYSMCAPTION);
		size.h += 2 * ::GetSystemMetrics (SM_CYBORDER) + 5;
	}
	else if (bIsTearOff)
	{
		m_rectTearOffCaption = CRect (CPoint (nBorderSize, nBorderSize), 
			CSize (size.w - 2 * nBorderSize, iTearOffBarHeight));
		size.h += iTearOffBarHeight;
	...
	if (m_nMaxHeight != -1 && size.h > m_nMaxHeight)
	{
		if (!m_bResizeTracking && !m_bWasResized)
		{
			int nMaxHeight = m_nMaxHeight - nBorderSize * 2;

			size.h = nMaxHeight - (nMaxHeight % pMenuBar->GetRowHeight()) + nBorderSize * 2 + 2;
	...
	if (m_bIsResizable)
	{
		const int nResizeBarHeight = 
			m_sizeMinResize.w > 0 ? iResizeBarBarHeightRight : iResizeBarBarHeight;

		if (m_bIsResizeBarOnTop)
		{
			m_rectResize = CRect (CPoint (nBorderSize, nBorderSize),
				CSize (size.w - 2 * nBorderSize, nResizeBarHeight));
		}
		else
		{
			m_rectResize = CRect (CPoint (nBorderSize, size.h - nBorderSize),
				CSize (size.w - 2 * nBorderSize, nResizeBarHeight));
		}

		size.h += nResizeBarHeight;
	}
	...
	if (pParentMenuBar != NULL && pParentMenuBar->IsFloating())
	{
		...
		case DROP_DIRECTION_TOP:
			m_ptLocation.y = m_ptLocationInitial.y - size.h - m_pParentBtn->Rect().Height() + 1;
		...
		case DROP_DIRECTION_LEFT:
			...
			if (bRTL)
			{
				m_ptLocation.x = m_ptLocationInitial.x + size.w;

				if (m_ptLocation.x > rectScreen.r)
				{
					m_ptLocation.x = rectScreen.r;
					m_DropDirection = DROP_DIRECTION_NONE;
				}
			}
			else
			{
				...
				if (m_ptLocation.x < rectScreen.l)
				{
					m_ptLocation.x = rectScreen.l;
					m_DropDirection = DROP_DIRECTION_NONE;
				}
			}
			break;
						...
						if (pParentWnd->GetSafeHwnd() != NULL)
						{
							...
							if (m_DropDirection == DROP_DIRECTION_LEFT)
							{
								m_ptLocation.x = 
									bRTL ?
										rectQCParent.l :
										rectQCParent.l - size.w;
							}
							else
							{
								m_ptLocation.x = 
									bRTL ?
										rectQCParent.r + size.w :
										rectQCParent.r;
							}
						}
					}
	...
	//---------------------------------------------
	if ((bRTL && (m_ptLocation.x - size.w < rectScreen.l)) || 
		(!bIsRightAlign && (m_ptLocation.x + size.w > rectScreen.r)))
	{
		//-----------------------------------------------------
		...
		if (pParentMenu != NULL)
		{
			...
			m_ptLocation.x = bRTL ? rectParent.r + size.w : rectParent.l - size.w;
			...
			if (m_pParentRibbonElement != NULL)
			{
				...
				if (!m_pParentRibbonElement->IsMenuMode())
				{
					...
					m_ptLocation.x = bRTL ? rectParent.l + size.w : rectParent.r - size.w;
				}
			}
			...
			(pParentMenuBar->IsHorizontal()) == 0)
		{
			...
			m_ptLocation.x = bRTL ? rectParentBtn.r + size.w : rectParentBtn.l - size.w;
			m_DropDirection = DROP_DIRECTION_LEFT;
		}
		else
		{
			m_ptLocation.x = bRTL ? rectScreen.l + size.w + 1 : m_bRightAlign ? rectScreen.l + 1 : rectScreen.r - size.w - 1;
			m_DropDirection = DROP_DIRECTION_NONE;
		}

		if (!bRTL && (m_ptLocation.x < rectScreen.l))
		{
			m_ptLocation.x = rectScreen.l;
			m_DropDirection = DROP_DIRECTION_NONE;
		}

		if (bRTL && (m_ptLocation.x > rectScreen.r))
		{
			m_ptLocation.x = rectScreen.r;
			m_DropDirection = DROP_DIRECTION_NONE;
		}
	...
	if (m_ptLocation.y + size.h > rectScreen.b)
	{
		...
		{
			CPoint ptRight (rectParentBtn.r, 0);
            pWndParent->ClientToScreen (&ptRight);

            CPoint ptTop (0, rectParentBtn.t - size.h);
			pWndParent->ClientToScreen (&ptTop);

            if (ptTop.y < 0)
            {
				int yParentButtonTop = ptTop.y + size.h;

				//-----------------------------------------------------
				// Where more space: on top or on bottom of the button?
				//-----------------------------------------------------
				if (rectScreen.b - yParentButtonTop < 
					yParentButtonTop - rectScreen.t)
				{
					m_ptLocation.y = rectScreen.t;
					m_DropDirection = DROP_DIRECTION_NONE;
					size.h += ptTop.y;
				}
				else
				{
					size.h = rectScreen.b - m_ptLocation.y;
					m_bIsAnimDown = TRUE;
					...
					if (m_bIsResizable)
					{
						...
						//-----------------------------------
						m_bIsResizeBarOnTop = FALSE;
						m_rectResize = CRect (CPoint (nBorderSize, size.h - nBorderSize - nResizeBarHeight),
							CSize (size.w - 2 * nBorderSize, nResizeBarHeight));
					}
				}
		...
		else
		{
			...
			{
				m_ptLocation.y = rectQCParent.b - size.h - 1;
			}
			else
			{
				m_ptLocation.y -= size.h;
		...
		if (m_ptLocation.y < rectScreen.t)
		{
			m_ptLocation.y = rectScreen.t;
			m_DropDirection = DROP_DIRECTION_NONE;
		}
	...
	if (m_ptLocation.y < rectScreen.t)
	{
		...
		{
			CPoint ptRight (m_pParentBtn->Rect().r, 0);
            m_pParentBtn->GetParentWnd()->ClientToScreen (&ptRight);

            CPoint ptBottom (0, m_pParentBtn->Rect().b);
		...
		if (m_ptLocation.y + size.h > rectScreen.b)
		{
			...
			if (size.h > rectScreen.Height())
			{
				size.h = rectScreen.Height();
				m_bHasBeenResized = TRUE;
				m_bScrollable = TRUE;
			}
	...
	{
		size.w += ::GetSystemMetrics (SM_CXVSCROLL);

		if (!m_rectResize.IsRectEmpty())
		{
			m_rectResize.r += ::GetSystemMetrics (SM_CXVSCROLL);
		}
	}
		...
		CBCGPToolBar::IsCustomizeMode())
	{
		if (!CBCGPToolBar::IsCustomizeMode())
		{
			size.w += m_iShadowSize;
			size.h += m_iShadowSize;
		}
		...
		{
			SetWindowPos (NULL, m_ptLocation.x - (bRTL ? size.w : 0), m_ptLocation.y, size.w, size.h,
		...
		m_pParentBtn->GetParentWnd() != NULL)
	{
		...
		CRect rectShadowRight (CPoint (bRTL ? rectMenu.l - 1 - m_iShadowSize : rectMenu.r + 1, rectMenu.t), 
		...
		CRect rectShadowBottom (CPoint (rectMenu.l, rectMenu.b + 1),
...
void CBCGPPopupMenu::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
		...
		m_bScrollable && pMenuBar->m_iHighlighted >= 0)
	{
		...
		else
		{
			...
			if (pItem->Rect().t < rectBar.t)
			{
				//---------------------
				// Scroll up is needed!
				//---------------------
				iOffsetDelta = (pItem->Rect().t - rectBar.t) / 
					pMenuBar->GetRowHeight() - 1;
			}
			else if (pItem->Rect().b > rectBar.b)
			{
				//-----------------------
				// Scroll down is needed!
				//-----------------------
				iOffsetDelta = (pItem->Rect().b - rectBar.b) / 
					pMenuBar->GetRowHeight() + 1;
			}

			if (iOffsetDelta != 0)
			{
				int iTotalRows = m_FinalSize.h / pMenuBar->GetRowHeight() - 2;
...
void CBCGPPopupMenu::OnTimer(UINT_PTR nIDEvent) 
{
	...
	case iAnimTimerId:
		if (!m_bAnimationIsDone)
		{
			...
			{
			case UNFOLD:
				m_AnimSize.w += nSteps * pMenuBar->GetColumnWidth();
				// no break intentionally

			case SLIDE:
				m_AnimSize.h += nSteps * pMenuBar->GetRowHeight();
				break;
			...
			if ((GetAnimationType() != FADE && m_AnimSize.h - m_iShadowSize >= m_FinalSize.h) ||
				(GetAnimationType() == UNFOLD && m_AnimSize.w - m_iShadowSize >= m_FinalSize.w) ||
				(GetAnimationType() == FADE && m_iFadePercent > 100))
			{

				m_AnimSize.w = m_FinalSize.w + m_iShadowSize;
				m_AnimSize.h = m_FinalSize.h + m_iShadowSize;
...
BOOL CBCGPPopupMenu::IsScrollDnAvailable()
{
	...
	return rectLastItem.b > m_nMenuBarHeight + pMenuBar->GetRowHeight();
}
//****
...
void CBCGPPopupMenu::ShowAllCommands()
{
	...
	if (pParentMenuButton != NULL)
	{
		...
		if (m_bmpShadowRight.IsValid())
		{
			m_bmpShadowRight.DeleteObject();
		}
		
		if (m_bmpShadowBottom.IsValid())
		{
...
BOOL CBCGPPopupMenu::InitMenuBar()
{
	...
	//----------------------------------------
	CRecentFileList* pMRUFiles = 	(AfxGetApp())->m_pRecentFileList;
...
void CBCGPPopupMenu::UpdateBottomWindows (BOOL bCheckOnly)
{
	...
	CRect rectMenu (m_ptLocation, 
		CSize (m_FinalSize.w + m_iShadowSize, m_FinalSize.h + m_iShadowSize));

	if (GetExStyle() & WS_EX_LAYOUTRTL)
	{
		rectMenu.OffsetRect (-(m_FinalSize.w + m_iShadowSize), 0);
	}
	...
	if (rectInter != rectMain)
	{
		...
		if (!bCheckOnly)
		{
			SetWindowPos (NULL, -1, -1, m_FinalSize.w, m_FinalSize.h,
						SWP_NOMOVE | SWP_NOZORDER |
						SWP_NOACTIVATE);
		}
	}
...
void CBCGPPopupMenu::DoPaint (CDC* pPaintDC)
{
	...
	if (bDrawShadows)
	{
		...
		if (bRTL)
		{
			rectClient.l += m_iShadowSize;
		}
		else
		{
			rectClient.r -= m_iShadowSize;
		}

		rectClient.b -= m_iShadowSize;
		...
		if (globalData.bIsWindowsVista)
		{
			...
			pPaintDC->BitBlt (0, 0,
					...
					&dcDesktop, rectMenu.l, rectMenu.t, SRCCOPY);
	...
	if (m_bScrollable)
	{
		...
		if (IsScrollDnAvailable())
		{
			if (GetMenuBar() != NULL)
			{
				...
				rectFill.b = m_rectScrollDn.t;
				rectFill.t = rectFill.b - nBorderSize - 1;
	...
	if (!m_rectResize.IsRectEmpty())
	{
		...
		if (m_bIsResizeBarOnTop)
		{
			resizeType = m_sizeMinResize.w > 0 ? MENU_RESIZE_TOP_RIGHT : MENU_RESIZE_TOP;
		}
		else
		{
			resizeType = m_sizeMinResize.w > 0 ? MENU_RESIZE_BOTTOM_RIGHT : MENU_RESIZE_BOTTOM;
		}
...
void CBCGPPopupMenu::DrawFade (CDC* pPaintDC)
{
	...
	int cx = m_FinalSize.w + m_iShadowSize;
	int cy = m_FinalSize.h + m_iShadowSize;
...
BOOL CBCGPPopupMenu::AdjustScroll (BOOL bForceMenuBarResize)
{
	...
	{
	case MENU_LOGO_LEFT:
		rectClient.l += m_iLogoWidth;
		break;

	case MENU_LOGO_RIGHT:
		rectClient.r -= m_iLogoWidth;
		break;

	case MENU_LOGO_TOP:
		rectClient.t += m_iLogoWidth;
		break;

	case MENU_LOGO_BOTTOM:
		rectClient.b -= m_iLogoWidth;
		break;
	}

	rectClient.t += m_rectTearOffCaption.Height();

	if (m_bIsResizeBarOnTop)
	{
		rectClient.t += m_rectResize.Height();
	}
	else
	{
		rectClient.b -= m_rectResize.Height();
	}
	...
	if (m_bScrollable)
	{
		if (m_bShowScrollBar)
		{
			rectClient.r -= ::GetSystemMetrics (SM_CXVSCROLL);
		}
		else
		{
			if (IsScrollUpAvailable())
			{
				...
				m_rectScrollUp.t += nBorderSize;
				m_rectScrollUp.b = m_rectScrollUp.t + m_iScrollBtnHeight;

				rectClient.t += m_iScrollBtnHeight + nBorderSize;
			}

			if (IsScrollDnAvailable())
			{
				m_rectScrollDn = rectClient;
				m_rectScrollDn.t = m_rectScrollDn.b - m_iScrollBtnHeight;

				rectClient.b -= m_iScrollBtnHeight + nBorderSize;
			}
		}
	}
	...
	{
		pMenuBar->SetWindowPos (NULL, rectClient.l, rectClient.t,
					rectClient.Width(), 
					rectClient.Height(),
					uiSWPFlags);
...
CBCGPPopupMenu::MENUAREA_TYPE CBCGPPopupMenu::CheckArea (const CPoint& ptScreen) const
{
	...
	{
	case MENU_LOGO_LEFT:
		rectLogo.r = rectLogo.l + m_iLogoWidth;
		break;

	case MENU_LOGO_RIGHT:
		rectLogo.l = rectLogo.r - m_iLogoWidth;
		break;

	case MENU_LOGO_TOP:
		rectLogo.b = rectLogo.t + m_iLogoWidth;
		break;

	case MENU_LOGO_BOTTOM:
		rectLogo.t = rectLogo.b - m_iLogoWidth;
		break;
	}
	...
	if (ptScreen.x > rectWindow.r - m_iShadowSize)
	{
		return SHADOW_RIGHT;
	}

	if (ptScreen.y > rectWindow.b - m_iShadowSize)
	{
...
BOOL CBCGPPopupMenu::TearOff (CPoint point)
{
	...
	if (pBar != NULL)	// Already exist, just show it
	{
		...
		rectBar.l = ptScreen.x;
		rectBar.r = rectBar.l + cx;

		int cy = rectBar.Height();
		rectBar.t = ptScreen.y;
		rectBar.b = rectBar.t + cy;
...
//****
void CBCGPPopupMenu::UpdateShadow (IRECT* lprectScreen)
{
	ASSERT_VALID (this);
	...
	if (lprectScreen != NULL)
	{
		...
		if (bIsRTL)
		{
			rectShadowRight.r = rectShadowRight.l + m_iShadowSize + 1;
		}
		else
		{
			rectShadowRight.l = rectShadowRight.r - m_iShadowSize - 1;
		}
		...
		rectShadowBottom.t = rectShadowBottom.b - m_iShadowSize - 1;
	...
	if (bIsRTL)
	{
		...
		SetWindowPos (NULL, rectWindow.l + iShadowSize, rectWindow.t,
	...
	if (bIsRTL)
	{
		SetWindowPos (NULL, rectWindow.l, rectWindow.t,
...
//****
void CBCGPPopupMenu::UpdateAllShadows (IRECT* lprectScreen)
{
...
void CBCGPPopupMenu::SetScrollBar()
{
	...
	rectScrollBar.l = rectScrollBar.r - cxScroll;
	rectClient.r -= cxScroll;
	...
	else
	{
		m_wndScrollBarVert.SetWindowPos (NULL, 
			rectScrollBar.l, rectScrollBar.t,
...
void CBCGPPopupMenu::EnableResize (CSize sizeMinResize)
{
	...
	{
		sizeMinResize.h += 
			sizeMinResize.w > 0 ? iResizeBarBarHeightRight : iResizeBarBarHeight;
	}
		...
		case WM_MOUSEMOVE:
			{
				...
				const int dx = (bRTL ? rectWindow.r - msg.pt.x : msg.pt.x - rectWindow.l) + iShadowSize + 2 * nBorderSize;

				CSize sizeNew = CSize (
					m_sizeMinResize.w > 0 ?
						max (m_sizeMinResize.w, dx) :
						rectWindow.Width() - iShadowSize,
					max (m_sizeMinResize.h, 
						m_bIsResizeBarOnTop ? rectWindow.b - msg.pt.y: msg.pt.y - rectWindow.t));

				if (sizeNew != m_sizeCurrent)
				{
					m_sizeCurrent = sizeNew;

					if (m_bIsResizeBarOnTop && sizeNew.h > m_sizeMinResize.h)
					{
