...
//****
void CBCGPMainClientAreaWnd::CalcWindowRect(IRECT* lpClientRect, UINT nAdjustType) 
{
	...
	if (m_wndTab.GetSafeHwnd() != NULL)
	{
		...
		if (m_bTabIsVisible)
		{
			...
			m_wndTab.SetWindowPos (NULL, 
				lpClientRect->l, lpClientRect->t,
				lpClientRect->r - lpClientRect->l,
				lpClientRect->b - lpClientRect->t,
			...
			lpClientRect->t += (rectTabWnd.t - rectTabClient.t);
			lpClientRect->b += (rectTabWnd.b - rectTabClient.b);
			lpClientRect->l += (rectTabWnd.l - rectTabClient.l);
			lpClientRect->r += (rectTabWnd.r - rectTabClient.r);
		...
		int nHeightDelta = lpClientRect->b - lpClientRect->t - 
							rectOld.Height();
		...
		if (!m_bTabIsVisible)
		{
			...
			while (hwndT != NULL)
			{
				...
				if (dwStyle & WS_MINIMIZE)
				{
					...
					if (rectWnd.t < rectClient.t)
					{
						rectWnd.t = rectClient.t;
					}

					::SetWindowPos (hwndT, NULL, 
									rectWnd.l, rectWnd.t, 0, 0, 
	...
	for (int i = 0; i < m_wndTab.GetTabsNum(); i++)
	{
		...
		CRect rect (0, 0, lpClientRect->r - lpClientRect->l, 
							lpClientRect->b - lpClientRect->t);
		...
		rect.l -= rectClient.l - rectScreen.l;
		rect.t -= rectClient.t - rectScreen.t;
		rect.r += rectScreen.r - rectClient.r;
		rect.b += rectScreen.b - rectClient.b;
		...
		{
			pWnd->SetWindowPos (&wndTop, rect.l, rect.t, 
...
//****
void CBCGPMainClientAreaWnd::CalcWindowRectForMDITabbedGroups(IRECT* lpClientRect, UINT /*nAdjustType*/) 
{
...
void CBCGPMainClientAreaWnd::AdjustMDIChildren (CBCGPTabWnd* pTabWnd)
{
	...
	for (int i = 0; i < pTabWnd->GetTabsNum(); i++)
	{
		...
		if (pWnd != NULL)
		{
			pWnd->SetWindowPos (&wndTop, rectTabWnd.l, rectTabWnd.t, 
...
CBCGPTabWnd* CBCGPMainClientAreaWnd::CreateTabGroup (CBCGPTabWnd* pWndTab)
{
	...
	{
		m_TabIcons.Create (
			globalData.m_sizeSmallIcon.w, globalData.m_sizeSmallIcon.h, 
	...
	while (pWndChild != NULL)
	{
		...
		if (m_mdiTabParams.m_bTabIcons)
		{
			...
			else
			{
				...
				{
					hIcon = (CBitmap*)(LONG_PTR) GetClassLongPtr (*pWndChild, GCLP_HICONSM);
				}
			}
...
LRESULT CBCGPMainClientAreaWnd::OnGetDragBounds (WPARAM wp, LPARAM lp)
{
	...
	if (m_groupAlignment == GROUP_VERT_ALIGN)
	{
		rectTabWndToResize.l += m_nResizeMargin;
		rectTabWndToResize.r -= m_nResizeMargin;
	}
	else
	{
		rectTabWndToResize.t += m_nResizeMargin;
		rectTabWndToResize.b -= m_nResizeMargin;
	}
...
LRESULT CBCGPMainClientAreaWnd::OnDragComplete (WPARAM wp, LPARAM lp)
{
	...
	pTabWndToResize->SetWindowPos (NULL, -1, -1, 
								   lpRectResized->r - lpRectResized->l,
								   lpRectResized->b - lpRectResized->t, 
	...
	m_groupAlignment == GROUP_VERT_ALIGN ?  rectNextWnd.l = lpRectResized->r : 
											rectNextWnd.t = lpRectResized->b;

	pNextTabWnd->SetWindowPos (NULL, rectNextWnd.l, rectNextWnd.t, 
...
LRESULT CBCGPMainClientAreaWnd::OnTabGroupMouseMove (WPARAM /*wp*/, LPARAM lp)
{
	...
	{
		bCalcVertRect = rectScreenHoveredWnd.r - pointScreen.x < 
						rectScreenHoveredWnd.b - pointScreen.y;
	}
	else
	{
	...
	{
		rectMargin.l = rectScreenHoveredWnd.r - m_nNewGroupMargin;
		bCalcVertRect = TRUE;
	}
	else if (m_groupAlignment == GROUP_HORZ_ALIGN || !bCalcVertRect)
	{
		rectMargin.t  = rectScreenHoveredWnd.b - m_nNewGroupMargin; 
		bCalcVertRect = FALSE;
	}

	CRect rectNew = rectScreenHoveredWnd;

	bCalcVertRect ? rectNew.l = rectScreenHoveredWnd.r - rectScreenHoveredWnd.Width() / 2:
					rectNew.t = rectScreenHoveredWnd.b - rectScreenHoveredWnd.Height() / 2; 
...
LRESULT CBCGPMainClientAreaWnd::OnMoveTabComplete(WPARAM wp, LPARAM lp)
{
	...
	if (!bMenuResult)
	{
		...
		else if (pTabWnd->GetTabsNum() > 1)
		{
			...
			{
				rectHoveredWnd.t = rectHoveredWnd.b - rectHoveredWnd.Height() / 2;
			}
			else
			{
				rectHoveredWnd.l = rectHoveredWnd.r - rectHoveredWnd.Width() / 2;
				bVertGroup = TRUE;
			}
												...
												BOOL bVertical)
{
	...
	if (bVertical)		
	{
		rectWndAfter.r -= rectGroup.Width();
		rectGroup.t = rectWndAfter.t;
		rectGroup.b = rectWndAfter.b;
	}
	else
	{
		rectWndAfter.b -= rectGroup.Height();
		rectGroup.l = rectWndAfter.l;
		rectGroup.r = rectWndAfter.r;
	}

	pTabWndAfter->SetWindowPos (NULL, -1, -1, rectWndAfter.Width(), rectWndAfter.Height(),
									SWP_NOZORDER | SWP_NOMOVE | SWP_NOACTIVATE);
	pNewTabWnd->SetWindowPos (NULL, rectGroup.l, rectGroup.t, 
...
void CBCGPMainClientAreaWnd::RemoveTabGroup (CBCGPTabWnd* pTabWnd, BOOL /*bRecalcLayout*/)
{
	...
	if (pSiblingTabWndToResize != NULL)
	{
		...
		pSiblingTabWndToResize->SetWindowPos (NULL, rectSiblingWnd.l, rectSiblingWnd.t, 
...
void CBCGPMainClientAreaWnd::SerializeTabGroup (CArchive& ar, CBCGPTabWnd* pTabWnd, BOOL bSetRelation)
{
	...
	else
	{
		...
		pTabWnd->SetWindowPos (NULL, rectWindow.l, rectWindow.r, 
