...
BOOL CBCGPImageEditDlg::OnInitDialog() 
{
	...
	//------------------------	
	{
		...
		const int nButtonWidth = m_wndPaletteBar.GetButtonSize().w;
		m_wndPaletteBar.WrapToolBar (nButtonWidth * 3);
		
		const CSize szLayout = m_wndPaletteBar.CalcSize (FALSE);
		rectPaletteBar.b = rectPaletteBar.t + szLayout.h + iBorderHeight * 2;
		...
		if (globalData.GetRibbonImageScale())
		{
			...
			m_wndPaletteBarLocation.SetWindowPos (NULL, -1, -1,
				rectPalette.Width(), szLayout.h + 4 * iBorderHeight,
	...
	// Create color picker:
	{
		...
		m_wndColorBar.SetColor (_RGB (0, 0, 0));
	}
	...
	m_rectPreviewImage.l = (m_rectPreviewImage.l + m_rectPreviewImage.r - m_sizeImage.w) / 2;
	m_rectPreviewImage.r = m_rectPreviewImage.l + m_sizeImage.w;

	m_rectPreviewImage.t = (m_rectPreviewImage.t + m_rectPreviewImage.b - m_sizeImage.h) / 2;
	m_rectPreviewImage.b = m_rectPreviewImage.t + m_sizeImage.h;
...
//****
void CBCGPImageEditDlg::OnPaint(CDC* pDC) 
{
	...
	dc.BitBlt (m_rectPreviewImage.l, m_rectPreviewImage.t,
				m_sizeImage.w, m_sizeImage.h, &dcMem,
				0, 0, SRCCOPY);
...
void CBCGPImageEditDlg::OnBcgToolClear() 
{
	...
	CRect rect (0, 0, m_sizeImage.w, m_sizeImage.h);
...
void CBCGPImageEditDlg::OnBcgToolCopy() 
{
	...
	try
	{
		...
		memDCDest.BitBlt (0, 0, m_sizeImage.w, m_sizeImage.h,
...
void CBCGPImageEditDlg::OnBcgToolPaste() 
{
	...
	memDCDst.FillRect (CRect (0, 0, m_sizeImage.w, m_sizeImage.h), 
						&globalData.brBtnFace);

	int x = max (0, (m_sizeImage.w - bmp.bmWidth) / 2);
	int y = max (0, (m_sizeImage.h - bmp.bmHeight) / 2);

	CBCGPToolBarImages::TransparentBlt (memDCDst.GetSafeHdc(),
		x, y, m_sizeImage.w, m_sizeImage.h,
		&memDCSrc, 0, 0, _RGB (192, 192, 192));
