						   ...
						   DWORD dwTabbedStyle, DWORD dwBCGStyle)
{
	...
	{
		SetWindowPos (NULL, -1, -1, m_sizeDialog.w, m_sizeDialog.h, SWP_NOZORDER | SWP_NOMOVE | SWP_NOACTIVATE);
	}
...
void CBCGPDialogBar::OnWindowPosChanging(WINDOWPOS FAR* lpwndpos) 
{
	...
	if (!CanBeResized())
	{
		...
		if (IsHorizontal() && lpwndpos->cy < sizeMin.h)
		{
			lpwndpos->cy = sizeMin.h;
			lpwndpos->hwndInsertAfter = HWND_BOTTOM;
		}
		else if (!IsHorizontal() && lpwndpos->cx < sizeMin.w)
		{
			lpwndpos->cx = sizeMin.w;
			lpwndpos->hwndInsertAfter = HWND_BOTTOM;
		}
	}
}
