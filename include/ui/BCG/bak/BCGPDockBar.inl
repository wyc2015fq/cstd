...
BOOL CBCGPDockBar::Create(DWORD dwStyle, const IRECT& rect, CWnd* pParentWnd, DWORD dwBCGStyle, CCreateContext* pContext) 
{
	ASSERT_VALID (this);
	return CBCGPDockBar::CreateEx (0, dwStyle, rect, pParentWnd, dwBCGStyle, pContext);
}
//----------------------------------------------------------------------------------//
BOOL CBCGPDockBar::CreateEx(DWORD dwStyleEx, DWORD dwStyle, const IRECT& rect, CWnd* pParentWnd, DWORD dwBCGStyle, CCreateContext* pContext) 
{
	ASSERT_VALID (this);
	...
	case CBRS_ALIGN_RIGHT:
		rectDockBar.l = rectParent.r;
		m_nDockBarID = AFX_IDW_DOCKBAR_RIGHT;
		break;

	case CBRS_ALIGN_TOP:
		rectDockBar.b = rectParent.t;
		m_nDockBarID = AFX_IDW_DOCKBAR_TOP;
		break;

	case CBRS_ALIGN_BOTTOM:
		rectDockBar.t  = rectParent.b;
									...
									CPoint ptOffset)
{
	...
	if (IsHorizontal())
	{
		nBaseLineOffset = min (rectRow.b - rectVirtual.b, rectRow.t - rectVirtual.t);
		nOffsetLimit = rectVirtual.Height() * 2 / 3; // / 2;
	}
	else
	{
		nBaseLineOffset = min (rectRow.r - rectVirtual.r, rectRow.l - rectVirtual.l);
...
//----------------------------------------------------------------------------------//
void CBCGPDockBar::CalcWindowRect(IRECT* lpClientRect, UINT nAdjustType) 
{
	ASSERT_VALID (this);
								    ...
								    LPCRECT lpRect)
{
	...
	if (!m_lstControlBars.Find (pControlBar))
	{
			...
			else
			{
				...
				if (CBCGPDockManager::m_bRestoringDockState)
				{
					if (pControlBar->m_recentDockInfo.m_nRecentRowIndex > nRowCount - 1)
					{
						...
						{
							AddRow (NULL, bVertDock ? szBarSize.w : szBarSize.h);
						}
					}
		...
		if (pRowToDock == NULL)
		{
			...
			pRowToDock = AddRow (posNewBar, bVertDock ? szBarSize.w : szBarSize.h);
		}
		...
		{
			ResizeRow (pRowToDock, bVertDock ? sizeBarNew.w : sizeBarNew.h);
		}
										 ...
										 CBCGPControlBar* pTargetBar)
{
	...
	if (IsHorizontal())
	{
		rectFinal.SetRect (rectTargetBar.l - szBarSize.w - 10, 
							rectTargetBar.t, rectTargetBar.l - 10, rectTargetBar.b);
	}
	else
	{
		rectFinal.SetRect (rectTargetBar.l,  
							rectTargetBar.t - szBarSize.h - 10, 
							rectTargetBar.r, rectTargetBar.t - 10);
	}
...
void CBCGPDockBar::RepositionBars (CRect& rectNewClientArea)
{
	...
	if (sizeNew != sizeOld)
	{
		int nHorzOffset = sizeNew.w - sizeOld.w;
		int nVertOffset = sizeNew.h - sizeOld.h;
...
int CBCGPDockBar::ResizeRow  (CBCGPDockBarRow* pRow, int nNewSize, BOOL bAdjustLayout)
{
	...
	if (nOffset < 0 && !pRow->IsEmpty())
	{
		...
		if (IsHorizontal() && nNewSize - size.h < 0 ||
			!IsHorizontal() && nNewSize - size.w < 0)
		{
			return 0;
		}
	}
...
CBCGPDockBarRow* CBCGPDockBar::RowFromPoint (CPoint pt, bool& bOuterRow) const
{
	...
	if (IsHorizontal() && pt.y < rectWnd.t ||
		!IsHorizontal() && pt.x < rectWnd.l)
	{
		bOuterRow = true;
	}
...
CSize CBCGPDockBar::CalcFixedLayout (BOOL bStretch, BOOL bHorz)
{
	...
	for (POSITION pos = m_lstDockBarRows.GetHeadPosition(); pos != NULL;)
	{
		...
		int nHeightRequired =  bHorzBar ? sizeRowRequired.h : sizeRowRequired.w;
...
void CBCGPDockBar::ResizeDockBar (int nNewWidth, int nNewHeight) // not called from anywhere !!!
{
	...
	case CBRS_ALIGN_LEFT:
		if (nNewHeight != -1)
		{
			rectDockBar.b = rectDockBar.t + nNewHeight;
		}
		break;
	...
	case CBRS_ALIGN_TOP:
		if (nNewWidth != -1)
		{
			rectDockBar.r = rectDockBar.l + nNewWidth;
		}
		break;
...
//----------------------------------------------------------------------------------//
void CBCGPDockBar::OnPaint(CDC* pDC) 
{
...
BOOL CBCGPDockBar::IsRectWithinDockBar (CRect rect, CPoint& ptDelta)
{
	...
	if (IsHorizontal())
	{
		if (rect.l < rectWnd.l)
		{
			ptDelta.x = rectWnd.l - rect.l;
			return FALSE;
		}
		if (rect.r >  rectWnd.r)
		{
			ptDelta.x = rectWnd.r - rect.r;
			return FALSE;
		}
	}
	else
	{
		if (rect.t < rectWnd.t)
		{
			ptDelta.y = rectWnd.t - rect.t;
			return FALSE;
		}
		if (rect.b > rectWnd.b)
		{
			ptDelta.y = rect.b - rectWnd.b;
			return FALSE;
		}
	}
...
{
	int nDeltaLeft = point.x - rect.l;
	int nDeltaTop = point.y - rect.t;
	int nDeltaRight = rect.r - point.x;
	int nDeltaBottom = rect.b - point.y;
								   ...
								   const CRect& rectWnd, UINT nFlags)
{
	...
	return (BOOL)(SetWindowPos (pWndInsertAfter, 
						 rectWnd.l, rectWnd.t, rectWnd.Width(), rectWnd.Height(),
