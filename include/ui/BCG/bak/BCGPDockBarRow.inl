									  ...
									  LPCRECT lpRect, BOOL bAddLast)
{
	...
	if (lpRect != NULL && !IsRectEmpty (lpRect))
	{
		...
		if (IsHorizontal())
		{
			ptOffset.x = rectOrg.l - rectRow.l;
			ptOffset.y = rectDockBar.t + m_nRowOffset;
		}
		else
		{
			ptOffset.x = rectDockBar.l + m_nRowOffset;
			ptOffset.y = rectOrg.t - rectRow.t;
		}
	}
	else
	{
		...
		if (IsHorizontal())
		{
			ptOffset.x = rectRow.l + nAdditionalBarOffset;
		...
		else
		{
			...
			ptOffset.y = rectRow.t + nAdditionalBarOffset;
	...
	pControlBar->SetWindowPos (NULL, ptOffset.x, ptOffset.y, szBarSize.w, szBarSize.h, 
...
void CBCGPDockBarRow::AddControlBarFromRow (CBCGPControlBar* pControlBar, BCGP_DOCK_METHOD dockMethod)
{
	...
	if (dockMethod == BCGP_DM_MOUSE)
	{
		...
		int nNCOffset = rectClient.l - rectWnd.l;
	...
	else
	{
		...
		nOffsetOnRow = IsHorizontal()  ? rectWnd.l : rectWnd.t;
	}
	...
	pControlBar->SetWindowPos (NULL, rectPos.l, rectPos.t, rectPos.r, rectPos.b, 
...
CSize CBCGPDockBarRow::CalcFixedLayout (BOOL bStretch, BOOL bHorz)
{
	...
	for (POSITION pos = m_lstControlBars.GetHeadPosition(); pos != NULL;)
	{
		...
		if (bHorzBar)
		{
			sizeRequired.w += sizeBar.w;
			sizeRequired.h = max (sizeRequired.h, sizeBar.h);
		}
		else
		{
			sizeRequired.w = max (sizeRequired.w, sizeBar.w);
			sizeRequired.h += sizeBar.h;
		}
	}

	if (bHorzBar && sizeRequired.h > 0)
	{
		 sizeRequired.h += m_nExtraSpace;
	}	
	
	if (!bHorzBar && sizeRequired.w > 0)
	{
		sizeRequired.w += m_nExtraSpace;
	}
...
void CBCGPDockBarRow::ShiftControlBars (CBCGPControlBar* pControlBar, int nOffset, BOOL bForward)
{
	...
	while (pos != NULL)
	{
		...
		if (pNextBar != pControlBar && !rectBar.IsRectEmpty())
		{
			if (IsHorizontal())
			{
				nMoveOffset -= bForward ? rectNextBar.l - rectBar.r 
										: rectNextBar.r - rectBar.l;
			}
			else
			{
				nMoveOffset -= bForward ? rectNextBar.t - rectBar.b 
										: rectNextBar.b - rectBar.t;
			}
		}
		...
		pNextBar->SetWindowPos (NULL, rectNextBar.l, rectNextBar.t, 
									  ...
									  BOOL bSwapControlBars, HDWP& hdwp)
{
	...
	if (CheckControlBars (rectBarWnd, pControlBar, bForward, ptMove, bSwapControlBars, hdwp))
	{
		...
		pControlBar->SetWindowPos (NULL, rectBarWnd.l, rectBarWnd.t, 
...
void CBCGPDockBarRow::MoveControlBar (CBCGPControlBar* pControlBar, CRect rectTarget, HDWP& /*hdwp*/)
{
	...
	if (IsHorizontal())
	{
		rectBarWnd.l = rectTarget.l;
		rectBarWnd.r = rectTarget.r;
	}
	else
	{
		rectBarWnd.t = rectTarget.t;
		rectBarWnd.b = rectTarget.b;
	}
	m_pParentDockBar->ScreenToClient (&rectBarWnd);
...
void CBCGPDockBarRow::MoveControlBar (CBCGPControlBar* pControlBar, int nAbsolutOffset, HDWP& /*hdwp*/)
{
	...
	if (IsHorizontal())
	{
		...
		rectBarWnd.l = rectRow.l + nAbsolutOffset;
		rectBarWnd.r = rectBarWnd.l + nBarLength;
	}
	else
	{
		nBarLength = rectBarWnd.Height();
		rectBarWnd.t = rectRow.t + nAbsolutOffset;
		rectBarWnd.b = rectBarWnd.t + nBarLength;
	}
										...
										BOOL bSwapControlBars, HDWP& hdwp)
{
	...
	if (rectIntersectVirt.IntersectRect (rectNextControlBarVirt, rectCurrentBar) && bSwapControlBars)
	{
			...
			!bForward && !pCurrentBar->IsLeftOf (rectNextControlBarVirt))
		{
			...
			if (IsHorizontal())
			{
				bForward ? rectNew.r = rectCurrentBar.l   
						 : rectNew.l = rectCurrentBar.r ;
				bForward ? rectNew.l = rectNew.r - rectNextControlBar.Width()
						 : rectNew.r = rectNew.l + rectNextControlBar.Width();
			}
			else
			{
				bForward ? rectNew.b = rectCurrentBar.t
						 : rectNew.t = rectCurrentBar.b;
				bForward ? rectNew.t = rectNew.b - rectNextControlBar.Height()
						 : rectNew.b = rectNew.t + rectNextControlBar.Height();
			}
					 ...
					 !bForward && pNextBar->IsLeftOf (rectNextControlBarVirt))
			{
				if (IsHorizontal())
				{
					nOffsetToMove = min (abs (ptOffset.x), 
										abs (rectNextControlBarVirt.l - rectNextControlBar.l));
				}
				else
				{
					nOffsetToMove = min (abs (ptOffset.y), 
										abs (rectNextControlBarVirt.t - rectNextControlBar.t));
				}
			}
...
void CBCGPDockBarRow::ResolveIntersection (CBCGPControlBar* pBar, bool bForward, HDWP& hdwp)
{
	...
	for (POSITION pos = posStart; pos != NULL;)
	{
		...
		if (bForward && 
			(IsHorizontal() && rectMovedBar.l > rectBarWnd.r ||
			 !IsHorizontal() && rectMovedBar.t > rectBarWnd.b) ||
			!bForward && 
			(IsHorizontal() && rectMovedBar.r < rectBarWnd.l ||
			 !IsHorizontal() && rectMovedBar.b < rectBarWnd.t))
		{
			pos = posSave;
			continue;
		}
		...
		if (bForward) 
		{
			nMoveOffset = IsHorizontal() ? rectBarWnd.r - rectMovedBar.l  
										  : rectBarWnd.b - rectMovedBar.t;
		}
		else
		{
			nMoveOffset = IsHorizontal() ? rectMovedBar.r - rectBarWnd.l
										  : rectMovedBar.b - rectBarWnd.t;
		}
...
BOOL CBCGPDockBarRow::IsEnoughSpaceToMove (CBCGPControlBar* pControlBar, bool bForward, int& nAllowedOffset)
{
	...
	if (IsHorizontal())
	{
		nAllowedOffset = bForward ? rectRow.r - rectControlBar.r : 
									rectRow.l - rectControlBar.l;
	}
	else
	{
		nAllowedOffset = bForward ? rectRow.b - rectControlBar.b : 
									rectRow.t - rectControlBar.t;
	}
...
void CBCGPDockBarRow::ArrangeControlBars (int nMargin, int nSpacing)
{
	...
	for (POSITION pos = m_lstControlBars.GetHeadPosition(); pos != NULL;)
	{
		...
		if (bFistBar)
		{
			IsHorizontal() ? ptOffset.x = rectRow.l + nMargin :
							  ptOffset.y = rectRow.t + nMargin;
		}
		...
		if (IsHorizontal()) 
		{
			...
			rectBar.l = ptOffset.x;
			rectBar.r = rectBar.l + nWidth;
			nLen = nWidth;
		}
		else
		{
			int nHeight = rectBar.Height();
			rectBar.t = ptOffset.y;
			rectBar.b = rectBar.t + nHeight;
			nLen = nHeight;
		}

		ScreenToClient (rectBar);

		pBar->SetWindowPos (NULL, rectBar.l, rectBar.t, 
...
void CBCGPDockBarRow::ArrangeBars (CBCGPControlBar* pInitialBar)
{
	...
	if (m_lstControlBars.GetCount() == 1)
	{
		...
		if (nAvailLength < 0)
		{
			...
			pInitialBar->SetWindowPos (NULL, rectBar.l, rectBar.t, 
			rectBar.Width(), rectBar.Height(), SWP_NOZORDER | SWP_NOACTIVATE);
			return;
		}
	}
...
int CBCGPDockBarRow::GetOutOfBoundsOffset (CBCGPControlBar* pBar, BOOL bLeftTopBound)
{
	...
	if (IsHorizontal())
	{
		nBoundOffset = bLeftTopBound ?  rectRow.l - rectBar.l : 
										rectBar.r - rectRow.r;
	}
	else
	{
		nBoundOffset = bLeftTopBound ? rectRow.t - rectBar.t : 
										rectBar.b - rectRow.b;
	}
...
void CBCGPDockBarRow::RepositionBars (CRect& rectNewParentBarArea, UINT nSide, BOOL bExpand, int nOffset)
{
	...
	if (IsExclusiveRow())
	{
		...
		if (IsHorizontal())
		{
			pBar->SetWindowPos (NULL, rectRowWnd.l, rectRowWnd.t, 
	...
	if (IsHorizontal())
	{
		rectRowWnd.l = rectNewParentWnd.l;
		rectRowWnd.r = rectNewParentWnd.r;
	}
	else
	{
		rectRowWnd.t = rectNewParentWnd.t;
		rectRowWnd.b = rectNewParentWnd.b;
	}
	...
	if (nSide != (UINT)-1 && bExpand && GetAvailableLength (TRUE) + nStretchSize > 0)
	{
		...
		for (POSITION pos = m_lstControlBars.GetHeadPosition(); pos != NULL;)
		{
			...
			if (rectNextControlBar != rectNextControlBarVirt)
			{
				...
				bool bMoveBackward = (IsHorizontal() && rectNextControlBar.l > rectNextControlBarVirt.l) ||
								  (!IsHorizontal() && rectNextControlBar.t > rectNextControlBarVirt.t);		
...
void CBCGPDockBarRow::AdjustBarToRowArea (CBCGPControlBar* pBar, const CRect& rectRow, HDWP& hdwp)
{
	...
	if (IsHorizontal()) 
	{
		if (rectBarWnd.l < rectRow.l)
		{
			ptOffset = CPoint (rectRow.l - rectBarWnd.l, 0);
			MoveControlBar (pBar, ptOffset, FALSE, hdwp);
		}

		if (rectBarWnd.r > rectRow.r)
		{
			ptOffset = CPoint (rectRow.r - rectBarWnd.r, 0);
			MoveControlBar (pBar, ptOffset, FALSE, hdwp);
		}
	}
	else
	{
		if (rectBarWnd.t < rectRow.t)
		{
			ptOffset = CPoint (0, rectRow.t - rectBarWnd.t);
			MoveControlBar (pBar, ptOffset, FALSE, hdwp);
		}

		if (rectBarWnd.b > rectRow.b)
		{
			ptOffset = CPoint (0, rectRow.b - rectBarWnd.b);
			MoveControlBar (pBar, ptOffset, FALSE, hdwp);
		}
	}
}
...
int CBCGPDockBarRow::CalcLastBarOffset()
{
	...
	return IsHorizontal() ? rect.r : rect.b;
}
//****
...
void CBCGPDockBarRow::CommitTrans()
{
	for (POSITION pos = m_lstControlBars.GetHeadPosition(); pos != NULL;)
	{
		...
		pNextBar->SetWindowPos (NULL, rect.l, rect.t, rect.Width(), 
...
int CBCGPDockBarRow::StretchBarRect (CBCGPControlBar* pBar, int nStretchSize)
{
	...
	if (IsHorizontal())
	{
		rect.r += nActualStretchSize; 
	}
	else
	{
		rect.b += nActualStretchSize; 
	}	
