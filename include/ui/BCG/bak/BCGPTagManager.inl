...
static LPCTSTR s_Point     = _T("IPOINT");
static LPCTSTR s_Size      = _T("ISIZE");
static LPCTSTR s_Rect      = _T("IRECT");
...
BOOL CBCGPTagManager::ParseColor (const CString& strItem, COLORREF& value)
{
	...
	if (sa.GetSize() > 0)
	{
		...
		if (count == 3)
		{
			value = AddaptColor ((COLORREF)_RGB(_ttol(sa[0]), _ttol(sa[1]), _ttol(sa[2])),
...
BOOL CBCGPTagManager::ParseColorHEX (const CString& strItem, COLORREF& value)
{
	...
	if (bRes)
	{
		value = AddaptColor (_RGB (clr[2], clr[1], clr[0]), m_clrBase, m_clrTarget);
	}
