...
void CBCGPTabbedToolbar::RecalcLayout()
{
	if (m_pTabWnd != NULL)
	{
		...
		rect.t = rect.l = 0;
		rect.b = size.h + m_pTabWnd->GetTabsHeight() + 
						m_pTabWnd->GetTabBorderSize() * 2 + 2;
		rect.r = size.w + m_pTabWnd->GetTabBorderSize() * 2;

		CRect rectTabWndOld;
		m_pTabWnd->GetWindowRect (rectTabWndOld);

		m_pTabWnd->SetWindowPos (NULL, rect.l, rect.t, rect.Width(), rect.Height(), SWP_NOZORDER  | SWP_NOACTIVATE);
