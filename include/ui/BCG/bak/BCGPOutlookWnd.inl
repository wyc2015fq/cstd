			...
			BOOL /*bDrawBorder*/, BOOL /*bGrayDisabledButtons*/)
{
	...
	int x = rect.l + max (0, (rect.Width() - sizeImage.w) / 2);
	int y = rect.t + max (0, (rect.Height() - 2 * sizeImage.h) / 2);

	BOOL bIsWhiteText = FALSE;

	if (!bHighlight && !IsDroppedDown())
	{
		if (GetRV (globalData.clrBarText) > 128 &&
			GetGV (globalData.clrBarText) > 128 &&
			GetBV (globalData.clrBarText) > 128)
		{
			bIsWhiteText = TRUE;
		}
	}

	CBCGPMenuImages::IMAGE_STATE imageState = bIsWhiteText ?
		CBCGPMenuImages::ImageWhite : CBCGPMenuImages::ImageBlack;

	CBCGPMenuImages::Draw (pDC, CBCGPMenuImages::IdMoreButtons, CPoint (x, y), imageState);

	y += sizeImage.h;

	CBCGPMenuImages::Draw (pDC, CBCGPMenuImages::IdArowDown, CPoint (x, y), imageState);
}
...
void CBCGPOutlookBarToolBar::AdjustLocations()
{
	...
	if (m_pCustomizeBtn != NULL)
	{
		...
		sizeCustomizeButton.w = 
			max (sizeCustomizeButton.w, CBCGPMenuImages::Size().w + 10);
	}
	...
	int x = rectToolbar.r -  sizeCustomizeButton.w + 2;

	int nCountToHide = nCount - (rectToolbar.Width() - sizeCustomizeButton.w + 2) / 
		(sizeButton.w - 2);
...
void CBCGPOutlookWnd::RecalcLayout()
{
	...
	if (bIsMode2003)
	{
		...
		nToolBarHeight = sizeImage.h + nVertMargin;
	}
	...
	if (bIsMode2003)
	{
		...
		m_rectCaption.b = m_rectCaption.t + globalData.GetTextHeight() + 2 * CBCGPBaseTabWnd::TAB_TEXT_MARGIN;
		m_rectCaption.t += nTopEdgeHeight - 1;

		m_rectSplitter = rectClient;
		m_rectSplitter.b -= nToolBarHeight + m_nTabsHeight * nVisiblePageButtons;
		m_rectSplitter.t = m_rectSplitter.b - nSplitterHeight;

		m_rectWndArea.t = m_rectCaption.b;
		m_rectWndArea.b = m_rectSplitter.t;
	}
	else
	{
		m_rectCaption.SetRectEmpty();
	...
	int y = bIsMode2003 ? m_rectSplitter.b : rectClient.t;
	...
	if (nVisibleTabsNum > 1 || !IsHideSingleTab())
	{
		for (int i = 0; i < m_iTabsNum; i ++)
		{
			...
			pTab->m_rect.t = y;
			pTab->m_rect.r++;
			pTab->m_rect.b = y + m_nTabsHeight;

			if (pTab->m_rect.t >= rectClient.b - nToolBarHeight && 
				bIsMode2003)
			{
				...
				(i == m_iActiveTab || i == m_iActiveTab + 1))
			{
				...
				pTab->m_rect.r -= m_nTabsHeight;
				rectScroll.l = pTab->m_rect.r;

				if (i == m_iActiveTab)
				{
					m_btnUp.SetWindowPos (NULL, rectScroll.l, rectScroll.t,
						rectScroll.Width(), rectScroll.Height(),
						SWP_NOACTIVATE | SWP_NOZORDER);
				}
				else
				{
					m_btnDown.SetWindowPos (NULL, rectScroll.l, rectScroll.t,
						rectScroll.Width(), rectScroll.Height(),
						SWP_NOACTIVATE | SWP_NOZORDER);
				}
			}
			...
			{
				m_rectWndArea.t = y + m_nTabsHeight;
				...
				y = rectClient.b - m_nTabsHeight * nVisibleAfter + 1;
				m_rectWndArea.b = y - 1;
			}
			else
			{
	...
	{
		m_rectWndArea.b -= m_nTabsHeight;

		m_btnDown.SetWindowPos (NULL, rectClient.r - m_nTabsHeight + 1,
			rectClient.b - m_nTabsHeight + 1,
	...
	if (bIsMode2003)
	{
		...
		m_wndToolBar.SetWindowPos (NULL, 
			rectClient.l, rectClient.b - nToolBarHeight,
...
BOOL CBCGPOutlookWnd::SetActiveTab (int iTab)
{
	...
	if (bAnimate)
	{
		...
		{
			bMoveDown ? rectOldWnd.t += dy : rectOldWnd.b += dy;
			
			pWndOld->SetWindowPos (NULL,
					rectOldWnd.l, rectOldWnd.t, 
...
//****
void CBCGPOutlookWnd::OnPaint(CDC* pDC) 
{
	...
	if (m_nBorderSize > 0)
	{
		...
		dc.PatBlt (rectClient.l, rectClient.t, m_nBorderSize, rectClient.Height(), PATCOPY);
		dc.PatBlt (rectClient.l, rectClient.t, rectClient.Width(), m_nBorderSize, PATCOPY);
		dc.PatBlt (rectClient.r - m_nBorderSize - 1, rectClient.t, m_nBorderSize + 1, rectClient.Height(), PATCOPY);
		dc.PatBlt (rectClient.l, rectClient.b - m_nBorderSize, rectClient.Width(), m_nBorderSize, PATCOPY);
	...
	dc.MoveTo (m_rectWndArea.l - 1, m_rectWndArea.b);
	dc.LineTo (m_rectWndArea.r + 1, m_rectWndArea.b);
	...
	if (!m_rectCaption.IsRectEmpty())
	{
		...
		rectTop.t -= nTopEdgeHeight + 1;
		rectTop.b = rectTop.t + nTopEdgeHeight + 1;
	...
	if (m_bScrollButtons && !IsMode2003())
	{
		if (m_iActiveTab == m_iTabsNum - 1)
		{
			...
			rectFill.t = rectFill.b - m_nTabsHeight;
...
void CBCGPOutlookWnd::DrawTabButton (CDC& dc, int iButtonIdx, BOOL bDrawPressedButton)
{
	...
	CBitmap* hIcon = GetTabHicon (iButtonIdx);
	...
	if (sizeImage.w + CBCGPBaseTabWnd::TAB_IMAGE_MARGIN <= rectBtn.Width())
	{
		CRect rectImage = rectBtn;

		rectImage.t += (rectBtn.Height() - sizeImage.h) / 2;
...
#define GRIPPER_MARGIN	4
	...
	{
		rcText.r -= CX_GRIPPER + GRIPPER_MARGIN * 2;
	}
...
void CBCGPOutlookWnd::GetTabArea (CRect& rectTabAreaTop, CRect& rectTabAreaBottom) const
{
	...
	for (int i = 0; i < m_iTabsNum; i ++)
	{
		...
		if (rectTabAreaTop.b == pTab->m_rect.t)
		{
			rectTabAreaTop.b += pTab->m_rect.Height();
		}
		...
		{
			rectTabAreaBottom.b += pTab->m_rect.Height();
		}
	}
...
void CBCGPOutlookWnd::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	int nDelta = (m_rectSplitter.t - point.y) / m_nTabsHeight;
	if (nDelta == 0)
	{
		return;
	}
