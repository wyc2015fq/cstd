...
//****
CBitmap* CBCGPBaseTabWnd::GetTabHicon (int iTab) const
{
	if (iTab < 0 || iTab >= m_iTabsNum)
	{
		return NULL;
...
//****
BOOL CBCGPBaseTabWnd::SetTabHicon (int iTab, CBitmap* hIcon)
{
	if (iTab < 0 || iTab >= m_iTabsNum)
	{
		return FALSE;
	...
	m_sizeImage.w = max (m_sizeImage.w, globalData.m_sizeSmallIcon.w);
	m_sizeImage.h = max (m_sizeImage.h, globalData.m_sizeSmallIcon.h);
...
BOOL CBCGPBaseTabWnd::DetachTab (BCGP_DOCK_METHOD dockMethod, int nTabNum, BOOL bHide)
{
	...
	if (dockMethod == BCGP_DM_MOUSE)
	{
		...
		if (abs (ptHotDelta.x) < szSencitivity.w && abs (ptHotDelta.y) < szSencitivity.h)
		{
			return FALSE;
		}
	}
	...
	if (pDockingBar != NULL && pDockingBar->CanBeAttached())
	{
		...
		if (GetTabsNum() == 1 && m_bHideSingleTab)
		{
			...
			pDockingBar->SetWindowPos (NULL, rectWnd.l, rectWnd.t, 
...
void CBCGPBaseTabWnd::EnterDragMode()
{
	...
	m_nOffsetFromTabLeft = m_ptHot.x - rectTab.l;
}
//****
		...
		if (IsPtInTabArea (point) && nNumTabs > 1 && m_bEnableTabSwap)
		{
			...
			if (nTabNum != m_iActiveTab && nTabNum != -1)
			{
				...
				if (nCurrTabNum != nTabNum)
				{
					...
					ptCursorNewPos.x = rectTab.l + m_nOffsetFromTabLeft;
...
CSize CBCGPBaseTabWnd::GetMaxWindowSize() const
{
	...
	for (int i = 0; i < nTabCount; i++)
	{
		...
		if (pTabInfo->m_pWnd != NULL)
		{
			...
			sizeMax.w = max (sizeCurr.w, sizeMax.w);
			sizeMax.h = max (sizeCurr.h, sizeMax.h);
		}
	}
...
void CBCGPBaseTabWnd::InitAutoColors()
{
	...
	{
		m_arAutoColors.Add (_RGB (197, 212, 242));
		m_arAutoColors.Add (_RGB (255, 220, 120));
		m_arAutoColors.Add (_RGB (190, 206, 161));
		m_arAutoColors.Add (_RGB (240, 160, 161));
		m_arAutoColors.Add (_RGB (188, 168, 225));
		m_arAutoColors.Add (_RGB (156, 193, 182));
		m_arAutoColors.Add (_RGB (247, 184, 134));
		m_arAutoColors.Add (_RGB (217, 173, 194));
		m_arAutoColors.Add (_RGB (165, 194, 215));
		m_arAutoColors.Add (_RGB (179, 166, 190));
		m_arAutoColors.Add (_RGB (234, 214, 163));
		m_arAutoColors.Add (_RGB (246, 250, 125));
		m_arAutoColors.Add (_RGB (181, 233, 157));
		m_arAutoColors.Add (_RGB (95, 195, 207));
		m_arAutoColors.Add (_RGB (193, 131, 131));
		m_arAutoColors.Add (_RGB (202, 202, 213));
	}
	else
	{
		m_arAutoColors.Add (_RGB (0, 255, 0));
		m_arAutoColors.Add (_RGB (0, 255, 255));
		m_arAutoColors.Add (_RGB (255, 0, 255));
		m_arAutoColors.Add (_RGB (192, 192, 192));
		m_arAutoColors.Add (_RGB (255, 255, 0));
	}
}
//****
