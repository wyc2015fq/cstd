...
public:
	...
	//--------------------------------
	static void SetSizes (ISIZE sizeButton, ISIZE sizeImage);
	static void SetMenuSizes (ISIZE sizeButton, ISIZE sizeImage);
	static CSize GetMenuImageSize();
	static CSize GetMenuButtonSize();

	void SetLockedSizes (ISIZE sizeButton, ISIZE sizeImage, BOOL bDontScale = FALSE);
	...
	static void SetNonPermittedCommands (CList<UINT>& lstCommands);
	static BOOL IsCommandPermitted (UINT uiCmd)
	{
		return m_lstUnpermittedCommands.Find (uiCmd) == NULL;
	}

	static void SetBasicCommands (CList<UINT>& lstCommands);
	...
	static const CList<UINT>& GetBasicCommands()
	{
	...
	virtual int GetRowHeight() const
	{
		...
		return max (globalData.GetTextHeight (m_dwStyle & CBRS_ORIENT_HORZ),
			(m_bMenuMode ?
			(m_sizeMenuButton.h > 0 ?
				m_sizeMenuButton.h : m_sizeButton.h) :
			GetButtonSize().h));
	}
	...
	{
		return m_bMenuMode ?
			m_sizeMenuButton.w > 0 ?
				m_sizeMenuButton.w : m_sizeButton.w :
			GetButtonSize().w;
	}
...
public: 
	...
	virtual void GetItemRect(int nIndex, IRECT* lpRect) const;
	virtual void GetInvalidateItemRect(int nIndex, IRECT* lpRect) const;
...
protected:
	...
	static CMap<UINT, int>	m_DefaultImages;
	...
	CMap<UINT, CBCGPToolbarButton*>	m_AcellKeys;	// Keyborad acceleration keys

	static BOOL m_bShowTooltips;
	static BOOL m_bShowShortcutKeys;
	static BOOL m_bLargeIcons;

	static CList<UINT>	m_lstUnpermittedCommands;
	static CList<UINT>	m_lstBasicCommands;
