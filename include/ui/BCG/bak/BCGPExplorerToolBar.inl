...
ISIZE CBCGPExplorerNavigationButton::OnCalculateSize (CDC* /*pDC*/, const CSize& /*sizeDefault*/, BOOL /*bHorz*/)
{
	ASSERT_VALID (this);
...
ISIZE CBCGPExplorerNavigationHistoryButton::OnCalculateSize (CDC* /*pDC*/, const CSize& /*sizeDefault*/, BOOL /*bHorz*/)
{
	ASSERT_VALID (this);
	...
	return CSize (sizeFrame.w - 2 * sizeImage.w - nHorzMargin - 1, sizeImage.h + 2);
}
...
BOOL CBCGPExplorerNavigationHistoryButton::OnClick (CWnd* pWnd, BOOL /*bDelay*/)
{
	...
	CPoint point = CPoint (m_rect.l, m_rect.b - 1);
...
void CBCGPExplorerToolBar::AdjustLocations()
{
	...
	if (m_nStretchID == 0)
	{
		...
		for (POSITION pos = m_Buttons.GetHeadPosition(); pos != NULL;)
		{
			...
			if (pButton->IsVisible())
			{
				...
				{
					rectButton.t = rectClient.t + (rectClient.Height() - nButtonHeight) / 2;
					rectButton.b = rectButton.t + nButtonHeight;
	...
	else
	{
		...
		for (pos = m_Buttons.GetHeadPosition(); pos != NULL; nIndex++)
		{
			...
			if (pButton->IsVisible())
			{
				...
				cxTotalWidth += pButton->OnCalculateSize (&dc, sizeGrid, TRUE).w;
			}
		}
		...
		for (pos = m_Buttons.GetHeadPosition(); pos != NULL; nIndex++)
		{
			...
			if (pButton->IsVisible())
			{
				...
				{
					sizeButton.w += cxFreeSpace;
					rectRedraw.l = x;
				}

				rectButton.l = x;
				rectButton.r = rectButton.l + sizeButton.w;
				rectButton.t = rectClient.t + max (0, (rectClient.Height() - sizeButton.h) / 2);
	...
	if (m_nBackID != 0 && m_nForwardID != 0)
	{
		...
		m_rectNavButtons.r = rectForward.r + nHorzMargin;
...
void CBCGPExplorerToolBar::OnDrawNavButtonsFrame (CDC* pDC)
{
	...
	const int cyFrame = GetNavFrameSize().h;

	CRect rectFrame = m_rectNavButtons;
	rectFrame.t = rectFrame.CenterPoint().y - cyFrame / 2;
	rectFrame.b = rectFrame.t + cyFrame;

	int nIndex = 0;

	if (!m_rectHistoryButton.IsRectEmpty())
	{
		rectFrame.r = m_rectHistoryButton.r;
		nIndex = 1;
...
void CBCGPExplorerToolBar::AdjustLayout()
{
	...
	if (globalData.DwmIsCompositionEnabled() && m_bIsAeroEnabled && !m_bIsHidden)
	{
		...
		margins.hTopHeight = rectWindow.Height();
...
CSize CBCGPExplorerToolBar::CalcLayout (DWORD dwMode, int nLength)
{
	...
	if (m_nBackID != 0 && m_nForwardID != 0)
	{
		size.h = max (size.h, GetNavFrameSize().h + 2);
	}
