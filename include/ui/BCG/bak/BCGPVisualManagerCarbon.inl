...
/*
void CBCGPVisualManagerCarbon::SetResourceHandle (HINSTANCE hinstRes)
{
	...
	if (m_hinstRes != hinstRes)
	{
		...
		if (CBCGPVisualManager::GetInstance()->IsKindOf (
			RUNTIME_CLASS (CBCGPVisualManagerCarbon)))
		{
...
void CBCGPVisualManagerCarbon::UpdateLinked()
{
	...
	{
		CBitmap* bmpSrc = m_arLinkImages[i].bmpSrc->GetImageWell();
		CBitmap* bmpDst = m_arLinkImages[i].bmpDst->GetImageWell();
...
void CBCGPVisualManagerCarbon::OnUpdateSystemColors()
{
	...
	if (tm.ExcludeTag (_T("MAINWND"), strItem))
	{
		...
		if (tmItem.ExcludeTag (_T("CAPTION"), strCaption))
		{
			...
			tmCaption.ReadControlRenderer (_T("BORDER"), m_ctrlBaseMainBorderCaption, MakeResourceID(_T("IDB_MAINBORDER_CAPTION")));
				...
				for (int i = 0; i < 2; i++)
				{
					...
					if (tmButtons.ExcludeTag (szTags[i + 2], str))
					{
						...
						if (tmBtn.ReadSize (_T("IconSize"), sizeIcon))
						{
							...
							m_SysBtnClose[i].LoadStr (MakeResourceID(_T("IDB_SYS_BTN_CLOSE") + suffix));
						}

						CBCGPTagManager::ParseControlRenderer (tmBtn.GetBuffer(), 
							m_ctrlBaseSysBtnBackC[i], MakeResourceID(_T("IDB_SYS_BTN_BACK_C") + suffix));
						m_ctrlBaseSysBtnBackC[i].CopyTo (m_SysBtnBackC[i]);
						CBCGPTagManager::ParseControlRenderer (tmBtn.GetBuffer(), 
							m_SysBtnBackCH[i], MakeResourceID(_T("IDB_SYS_BTN_BACK_C_H") + suffix));
					}
				}
			}
		}
		...
		// border
		tmItem.ReadControlRenderer (_T("BORDER"), m_ctrlBaseMainBorder, MakeResourceID(_T("IDB_MAINBORDER")));
		m_ctrlBaseMainBorder.CopyTo (m_ctrlMainBorder);
		tmItem.ReadControlRenderer (_T("BORDER_L"), m_ctrlBaseMainBorderL, MakeResourceID(_T("IDB_MAINBORDER_L")));
		m_ctrlBaseMainBorderL.CopyTo (m_ctrlMainBorderL);
		tmItem.ReadControlRenderer (_T("BORDER_R"), m_ctrlBaseMainBorderR, MakeResourceID(_T("IDB_MAINBORDER_R")));
		m_ctrlBaseMainBorderR.CopyTo (m_ctrlMainBorderR);
		tmItem.ReadControlRenderer (_T("BORDER_TB"), m_ctrlBaseMainBorderTB, MakeResourceID(_T("IDB_MAINBORDER_TB")));
		m_ctrlBaseMainBorderTB.CopyTo (m_ctrlMainBorderTB);
		tmItem.ReadControlRenderer (_T("BORDER_MDICHILD"), m_ctrlBaseMDIChildBorder, MakeResourceID(_T("IDB_MDICHILDBORDER")));
	...
	if (tm.ExcludeTag (_T("MINIWND"), strItem))
	{
		...
		if (tmItem.ExcludeTag (_T("CAPTION"), strCaption))
		{
			...
			tmCaption.ReadControlRenderer (_T("BORDER"), m_ctrlBaseMiniBorderCaption, MakeResourceID(_T("IDB_MINIBORDER_CAPTION")));
			m_ctrlBaseMiniBorderCaption.CopyTo (m_ctrlMiniBorderCaption);
			tmCaption.ReadControlRenderer (_T("BUTTONS"), m_ctrlMiniSysBtn, MakeResourceID(_T("IDB_MINI_SYS_BTN")));
		}

		// border
		tmItem.ReadControlRenderer (_T("BORDER"), m_ctrlBaseMiniBorder, MakeResourceID(_T("IDB_MINIBORDER")));
		m_ctrlBaseMiniBorder.CopyTo (m_ctrlMiniBorder);
		tmItem.ReadControlRenderer (_T("BORDER_TB"), m_ctrlBaseMiniBorderTB, MakeResourceID(_T("IDB_MINIBORDER_TB")));
	...
	if (tm.ExcludeTag (_T("MENU"), strItem))
	{
		...
		tmItem.ReadControlRenderer (_T("ItemBack"), m_ctrlBaseMenuItemBack, MakeResourceID(_T("IDB_MENU_ITEM_BACK")));
		m_ctrlBaseMenuItemBack.CopyTo (m_ctrlMenuItemBack);
		tmItem.ReadToolBarImages (_T("ItemCheck"), m_BaseMenuItemMarkerC, MakeResourceID(_T("IDB_MENU_ITEM_MARKER_C")));
		m_BaseMenuItemMarkerC.CopyTo (m_MenuItemMarkerC);
		tmItem.ReadToolBarImages (_T("ItemRadio"), m_BaseMenuItemMarkerR, MakeResourceID(_T("IDB_MENU_ITEM_MARKER_R")));
	...
	if (tm.ExcludeTag (_T("BARS"), strItem))
	{
		...
		if (tmItem.ExcludeTag (_T("TOOLBAR"), strBar))
		{
			...
			tmBar.ReadToolBarImages (_T("GRIPPER"), m_BaseToolBarGripper, MakeResourceID(_T("IDB_GRIPPER")));
			m_BaseToolBarGripper.CopyTo (m_ToolBarGripper);
			tmBar.ReadToolBarImages (_T("TEAR"), m_BaseToolBarTear, MakeResourceID(_T("IDB_TEAR")));
			m_BaseToolBarTear.CopyTo (m_ToolBarTear);
			tmBar.ReadControlRenderer (_T("BUTTON"), m_ctrlBaseToolBarBtn, MakeResourceID(_T("IDB_TOOLBAR_BTN")));
			...
			if (!tmBar.ReadColor (_T("SeparatorDark"), m_clrBaseSeparatorDark))
			{
				m_clrBaseSeparatorDark = 
					CBCGPDrawManager::PixelAlpha (m_clrBaseToolBarBottomLine, _RGB (255, 255, 255), 95);
			}
			if (!tmBar.ReadColor (_T("SeparatorLight"), m_clrBaseSeparatorLight))
			{
				m_clrBaseSeparatorLight	= _RGB (255, 255, 255);
			}
		}
		...
		if (tmItem.ExcludeTag (_T("MENUBAR"), strBar))
		{
			...
			tmBar.ReadControlRenderer (_T("BUTTON"), m_ctrlBaseMenuBarBtn, MakeResourceID(_T("IDB_MENUBAR_BTN")));
		...
		{
			CBCGPTagManager tmBar (strBar);
			tmBar.ReadControlRenderer (_T("BORDER"), m_ctrlBasePopupBorder, MakeResourceID(_T("IDB_POPUPMENU_BORDER")));
		...
		if (tmItem.ExcludeTag (_T("STATUSBAR"), strBar))
		{
			...
			tmBar.ReadControlRenderer (_T("BACK"), m_ctrlBaseStatusBarBack, MakeResourceID(_T("IDB_STATUSBAR_BACK")));
			m_ctrlBaseStatusBarBack.CopyTo (m_ctrlStatusBarBack);
			tmBar.ReadControlRenderer (_T("BACK_EXT"), m_ctrlBaseStatusBarBack_Ext, MakeResourceID(_T("IDB_STATUSBAR_BACK_EXT")));
			m_ctrlBaseStatusBarBack_Ext.CopyTo (m_ctrlStatusBarBack_Ext);

			tmBar.ReadToolBarImages (_T("PANEBORDER"), m_BaseStatusBarPaneBorder, MakeResourceID(_T("IDB_STATUSBAR_PANEBORDER")));
			m_BaseStatusBarPaneBorder.CopyTo (m_StatusBarPaneBorder);
			tmBar.ReadToolBarImages (_T("SIZEBOX"), m_BaseStatusBarSizeBox, MakeResourceID(_T("IDB_STATUSBAR_SIZEBOX")));
	...
	if (tm.ExcludeTag (_T("RIBBON"), strItem))
	{
		...
		if (tmItem.ExcludeTag (_T("PANEL"), str))
		{
			...
			if (tmPanel.ExcludeTag (_T("BUTTONS"), strButtons))
			{
				...
				tmButtons.ReadControlRenderer (_T("BUTTON_PUSH"), m_ctrlBaseRibbonBtnPush, MakeResourceID(_T("IDB_RIBBON_BTN_PUSH")));
	...
	m_clrInactiveTabText = m_clrTabTextInactive;
										...
										BOOL bActive, BOOL /*bMDI = FALSE*/)
{
	...
	if (pImage != NULL)
	{
		pImage->DrawEx (pDC, rect, 0, horz, vert, CRect (0, 0, 0, 0), alphaSrc);
	}
}
											   ...
											   const CObList& lstSysButtons)
{
	...
	rectCaptionB.t = rectCaptionB.b - m_ctrlMainBorderTB.GetParams().m_rectImage.Height();
	m_ctrlMainBorderTB.Draw (&memDC, rectCaptionB, indexBorder);
...
BOOL CBCGPVisualManagerCarbon::OnNcPaint (CWnd* pWnd, const CObList& lstSysButtons, CRect rectRedraw)
{
	...
	if (dc.GetSafeHdc() != NULL)
	{
		...
		CRect rectStatus (0, 0, 0, szSysBorder.h);
		if (bIsStatusBar)
		{
		...
		if (!bIsStatusBar)
		{
				...
				(bDialog && !m_ctrlDialogBorder.IsValid()))
			{
				...
				m_ctrlMDIChildBorder.DrawFrame (&dc, rectBottom, indexBorder);
			}
			else if (bDialog)
			{
				m_ctrlDialogBorder.DrawFrame (&dc, rectBottom, indexBorder);
			}
			else
			{
				m_ctrlMainBorder.DrawFrame (&dc, rectBottom, indexBorder);
			}
		}
		...
		if (bBottomLine)
		{
			...
			rectBottom.b = rectBottom.t + m_ctrlMainBorderTB.GetParams().m_rectImage.Height();
...
BOOL CBCGPVisualManagerCarbon::OnUpdateNcButtons (CWnd* pWnd, const CObList& lstSysButtons, CRect rectCaption)
{
	...
	for (POSITION pos = lstSysButtons.GetHeadPosition(); pos != NULL;)
	{
		...
		x -= sizeButton.w;
		pButton->SetRect (CRect (CPoint (x, rectCaption.t), sizeButton));
	}
						...
						BOOL bNCArea/* = FALSE*/)
{
    ...
    else if (pBar->IsKindOf (RUNTIME_CLASS (CBCGPStatusBar)))
    {
		...
		rect.b = rect.t + m_ctrlMainBorderTB.GetParams().m_rectImage.Height();
								...
								CBCGPMiniFrameWnd* pFrameWnd, BOOL bActive)
{
	...
	rectCaptionB.t = rectCaptionB.b - m_ctrlMiniBorderTB.GetParams().m_rectImage.Height();
										...
										CRect rectBorder, CRect rectBorderSize)
{
	...
	rectBorder.b = rectBorder.t + m_ctrlMiniBorderTB.GetParams().m_rectImage.Height();
...
COLORREF CBCGPVisualManagerCarbon::GetTabTextColor (const CBCGPBaseTabWnd* pTabWnd, int iTab, BOOL bIsActive)
{
	...
	if (!bIsActive && (pTabWnd->IsOneNoteStyle() || pTabWnd->IsVS2005Style()) && pTabWnd->GetTabBkColor (iTab) == (COLORREF)-1)
	{
		return m_clrTabTextInactive;
	}
												 ...
												 CBCGPVisualManager::BCGBUTTON_STATE state)
{
	...
	if (globalData.m_nBitsPerPixel > 8 && !globalData.IsHighContastMode())
	{
		...
		CBCGPDrawManager dm (*pDC);
		dm.DrawGradientRing (rect, m_clrHighlightDnGradientLight, m_clrHighlightGradientLight,
			(COLORREF)-1,
			45, nRadius);
	}
