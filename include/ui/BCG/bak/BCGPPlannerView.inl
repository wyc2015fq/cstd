...
HFONT CBCGPPlannerView::SetCurrFont (CDC* pDC, BOOL bBold/* = FALSE*/)
{
	...
	return (HFONT) ::SelectObject (pDC, hFont);
}
...
void CBCGPPlannerView::AdjustCaptionFormat (CDC* pDC)
{
	...
	for (int n = 0; n < sa.GetSize(); n++)
	{
		...
		for (int nDay = 0; nDay < nDays; nDay++)
		{
			...
			if (m_ViewRects [nDay].Width() < sz.w + 6)
			{
				bFlag = TRUE;
				break;
			}
...
void CBCGPPlannerView::AdjustLayout (BOOL bRedraw)
{
	...
	m_nRowHeight    = max
						(
							...
							(
								...
								m_pPlanner->GetClockIconSize().h + 2
							), 
							CBCGPPlannerManagerCtrl::GetImageSize().h + 2
						);
	...
	{
		m_rectApps.r -= cxScroll;

		pScrollBar->ShowScrollBar (TRUE);
		pScrollBar->SetWindowPos (NULL, m_rectApps.r, 
			rectClient.t,	cxScroll, rectClient.Height(), SWP_NOZORDER | SWP_NOACTIVATE);
	}
	else
	{
					...
					if (bAdd)
					{
						...
						else
						{
							if (!rect.IsRectNull())
							{
								...
								if (rectDraw.b <= rect.t)
								{
									res |= 0x01;
								}
								else if (rect.b <= (rectDraw.t + 1))
								{
									res |= 0x02;
								}
							}
...
void CBCGPPlannerView::OnDrawUpDownIcons (CDC* pDC)
{
	...
	for (int i = 0; i < 2; i++)
	{
		CBitmap* hIcon = GetPlanner()->GetUpDownIcon (i);
		if (hIcon == NULL)
		{
			continue;
		}
...
void CBCGPPlannerView::DrawAppointment (CDC* pDC, CBCGPAppointment* pApp, CBCGPAppointmentDrawStructEx* pDS)
{
	...
	if (bIsRoundedCorners)
	{
		...
		if (bAlternative)
		{
			if (!bAllBoders)
			{
				...
				{
					CRgn rgnTmp;
					rgnTmp.CreateRectRgn (rect.l - 1, rect.t, rect.l + 3, rect.b);
				...
				{
					CRgn rgnTmp;
					rgnTmp.CreateRectRgn (rect.r - 4, rect.t, rect.r + 1, rect.b);
					rgn.CombineRgn (&rgn, &rgnTmp, RGN_OR);
				}
			}
		}
	...
	if (!pApp->IsAllDay())
	{
		...
		else if (!strStart.IsEmpty())
		{
			...
			if ((!bAlternative && rect.Width() >= (szDT.w + szSpace.w) * 2) ||
				( bAlternative && rect.Width() >= (szDT.w + szSpace.w) * 3))
			{
				if (bAlternative)
				{
					if (pDS->GetBorder() & CBCGPAppointmentDrawStruct::BCGP_APPOINTMENT_DS_BORDER_START)
	...
	int nTextWidth = pDC->GetTextExtent (pApp->m_strDescription).w;

	if (dwIcons.GetSize() > 0)
	{
		long nIconsWidth = rtIcons.l - rect.l;
		
		CPoint pt (rect.l + 1, rect.t + (rect.Height() - szImage.h) / 2);
...
HFONT CBCGPPlannerView::GetFont (BOOL bBold/* = FALSE*/)
{
	...
	if (bBold)
	{
		return (HFONT) (m_FontBold.IsValid() 
							? m_FontBold.GetSafeHandle() 
							: globalData.fontBold);
	}

	return (HFONT) (m_Font.IsValid() 
...
{
	return (HFONT) (m_FontVert.IsValid() 
...
void CBCGPPlannerView::ScreenToClient (IRECT* rect) const
{
	ASSERT_VALID(m_pPlanner);
...
void CBCGPPlannerView::ClientToScreen (IRECT* rect) const
{
	ASSERT_VALID(m_pPlanner);
