...
void CButtonsList::DrawItem (LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (m_pImages != NULL)
	{
		...
		for (POSITION pos = m_Buttons.GetHeadPosition(); pos != NULL;)
		{
			...
			if (rect.t >= rectClient.b)
			{
				break;
			}

			if (rect.b > rectClient.t)
			{
...
void CButtonsList::SetImages (CBCGPToolBarImages* pImages)
{
	...
	m_sizeButton.w = m_pImages->GetImageSize().w + 6;
	m_sizeButton.h = m_pImages->GetImageSize().h + 7;
...
//****
CBCGPToolbarButton* CButtonsList::HitTest (IPOINT point) const
{
	CRect rectClient;
	GetClientRect (&rectClient);
...
void CButtonsList::SelectButton (CBCGPToolbarButton* pButton)
{
	...
	{
		if (rectSelected.t >= rectClient.b || 
			rectSelected.b <= rectClient.t)
		{
			int iNewOffset = 
				max (0,
					min (rectSelected.b - m_iScrollOffset - rectClient.Height(), 
						m_iScrollTotal));
...
void CButtonsList::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* /*pScrollBar*/)
{
	...
	case SB_LINEUP:
		iScrollOffset -= m_sizeButton.h + iYOffset;
		break;
	...
	case SB_PAGEUP:
		iScrollOffset -= m_iScrollPage * (m_sizeButton.h + iYOffset);
		break;
	...
	case SB_THUMBPOSITION:
		iScrollOffset = ((m_sizeButton.h + iYOffset) / 2 + nPos) / 
			(m_sizeButton.h + iYOffset) * (m_sizeButton.h + iYOffset);
		break;
	...
	if (iScrollOffset != m_iScrollOffset)
	{
		...
		rectClient.r -= ::GetSystemMetrics (SM_CXVSCROLL) + 2;
		rectClient.InflateRect (-1, -1);
...
void CButtonsList::RebuildLocations()
{
	...
	rectButtons.r -= ::GetSystemMetrics (SM_CXVSCROLL) + 1;
	rectButtons.InflateRect (-iXOffset, -iYOffset);

	int x = rectButtons.l;
	int y = rectButtons.t - m_iScrollOffset;
	...
	for (POSITION pos = m_Buttons.GetHeadPosition(); pos != NULL;)
	{
		...
		if (x + sizeButton.w > rectButtons.r)
		{
			if (x == rectButtons.l)
			{
				sizeButton.w = rectButtons.r - rectButtons.l;
			}
			else
			{
				x = rectButtons.l;
				y += sizeButton.h + iYOffset;
			}
		}

		pButton->SetRect (CRect (CPoint (x, y), 
			CSize (sizeButton.w, m_sizeButton.h)));

		x += sizeButton.w + iXOffset;
	}

	CBCGPToolbarButton* pLastButton = (CBCGPToolbarButton*) m_Buttons.GetTail();
	ASSERT (pLastButton != NULL);

	int iVisibleRows = rectButtons.Height() / (m_sizeButton.h + iYOffset);
	int iTotalRows = pLastButton->Rect().b / (m_sizeButton.h + iYOffset);
	...
	if (iNonVisibleRows > 0)	// Not enouth space.
	{
		if (m_wndScrollBar.GetSafeHwnd() == NULL)
		{
			...
			rectSB.l = rectSB.r - ::GetSystemMetrics (SM_CXVSCROLL) - 1;

			m_wndScrollBar.Create (WS_CHILD | WS_VISIBLE | SBS_VERT,
									rectSB, this, 1);
		}
		
		m_iScrollTotal = iNonVisibleRows * (m_sizeButton.h + iYOffset);
