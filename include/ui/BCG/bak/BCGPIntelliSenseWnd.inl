								   ...
								   CRuntimeClass* pLBDataRTC)
{
	...
	int nLBWidth = m_pLstBoxData->m_sizeMaxItem.w;
	...
	int nLBOffset = sizeImage.w + GetSystemMetrics (SM_CXBORDER) + 
	...
	if (rectLB.b > rectScreen.b)
	{
		rectLB.b = rectLB.t - m_pParentEditCtrl->GetLineHeight();
		rectLB.t = rectLB.b - nRectLBHeight;
	}

	if (rectLB.r > rectScreen.r)
	{
		rectLB.r = pt.x;
		rectLB.l = rectLB.r - nRectLBWidth;
	}

	SetWindowPos (NULL, rectLB.l, rectLB.t, nRectLBWidth, nRectLBHeight,
					SWP_NOZORDER  | SWP_NOACTIVATE); 
