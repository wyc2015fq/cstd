...
void CBCGPRibbonContextCaption::OnDraw (CDC* pDC)
{
	...
	if (GetParentRibbonBar()->IsTransparentCaption())
	{
		...
		CBCGPVisualManager::GetInstance()->DrawTextOnGlass (
			...
			bIsZoomed ? 0 : 10,
			bIsZoomed ? _RGB (255, 255, 255) : (COLORREF)-1);
	}
	else
	{
...
//****
void CBCGPRibbonMainButton::SetImage (CBitmap* hBmp)
{
	ASSERT_VALID (this);
...
//****
void CBCGPRibbonMainButton::SetScenicImage (CBitmap* hBmp)
{
	ASSERT_VALID (this);
...
void CBCGPRibbonMainButton::DrawImage (CDC* pDC, RibbonImageType /*type*/, CRect rectImage)
{
	...
	ImageAlignHorz horz = CBCGPToolBarImages::ImageAlignHorzLeft;
	ImageAlignVert vert = ImageAlignVertTop;
	...
	if (m_pRibbonBar->IsScenicLook())
	{
		...
		CRect rectArrow(CPoint (rectImage.r - sizeArrow.w / 2 + xMargin,
			rectImage.t + (sizeImage.h - sizeArrow.h) / 2), 
			sizeArrow);
...
BOOL CBCGPRibbonMainButton::ShowMainMenu()
{
	...
	pMenu->Create (m_pRibbonBar, 
		bIsRTL ? rectBtn.r : rectBtn.l, y, (HMENU) NULL);
...
CSize CBCGPRibbonBar::CalcFixedLayout(BOOL, BOOL /*bHorz*/)
{
	...
	if (scale > 1.)
	{
		sizeMainButton.w = (int)(.5 + scale * sizeMainButton.w);
		sizeMainButton.h = (int)(.5 + scale * sizeMainButton.h);
	}
	...
	else
	{
		...
		{
			m_nCaptionHeight = max (m_nCaptionHeight, sizeAQToolbar.h + 2 * yMargin);
		}

		const int nQuickAceesToolbarHeight = IsQuickAccessToolbarOnTop() ? 0 : sizeAQToolbar.h;
	...
	for (int i = 0; i < m_arCategories.GetSize(); i++)
	{
		...
		if (pCategory == pActiveCategory)
		{
			if (m_dwHideFlags == BCGPRIBBONBAR_HIDE_ELEMENTS && !bForceRestore)
			{
				...
				rectCategory.t = pCategory->GetTabRect().b;
				rectCategory.b = rectCategory.t + m_nCategoryHeight;
...
//****
void CBCGPRibbonBar::OnPaint(CDC* pDC) 
{
	...
	if (!m_rectCaption.IsRectEmpty())
	{
		...
		{
			pDC->FillSolidRect (rectFill, _RGB (0, 0, 0));
	...
	rectTabs.t = m_rectCaption.IsRectEmpty() ? rectClient.t : m_rectCaption.b;
	rectTabs.b = rectTabs.t + m_nTabsHeight;
...
void CBCGPRibbonBar::OnLButtonDown(UINT nFlags, CPoint point) 
{
		...
		IsScenicLook())
	{
		...
		rectIcon.r = rectIcon.l + rectIcon.Height();

		if (rectIcon.PtInRect (point))
		{
			CPoint ptMenu (m_rectCaption.l, m_rectCaption.b);
...
void CBCGPRibbonBar::RecalcLayout()
{
	...
	const int nCaptionTextWidth = dc.GetTextExtent (strCaption).w;
	...
	{
		m_rectCaption = rect;
		m_rectCaption.b = m_rectCaption.t + m_nCaptionHeight;

		int x = m_rectCaption.r;
		...
		if (globalData.DwmIsCompositionEnabled())
		{
			...
			{
				rect.t += cyFrameBorder / 2 + yMargin;
		...
		else
		{
			...
			const int yOffsetCaptionButton = max (0, 
				(m_rectCaption.Height() - sizeCaptionButton.h) / 2);
			...
			for (i = RIBBON_CAPTION_BUTTONS - 1; i >= 0; i--)
			{
				...
				CRect rectCaptionButton (
					CPoint (x - sizeCaptionButton.w, m_rectCaption.t + yOffsetCaptionButton),
					sizeCaptionButton);

				m_CaptionButtons [i].SetRect (rectCaptionButton);

				x -= sizeCaptionButton.w;
		...
		if (globalData.DwmIsCompositionEnabled())
		{
			m_rectCaptionText.t += GetSystemMetrics (SM_CYSIZEFRAME) / 2;
		}

		m_rectCaptionText.r = x - xMargin;
		m_rectCaptionText.OffsetRect (0, nCaptionOffsetY);

		xSysButtonsLeft = m_rectCaptionText.r;
	}
	...
	if (m_pMainButton != NULL)
	{
		...
		else
		{
			...
			m_pMainButton->SetRect (CRect (
				CPoint (rect.l, rect.t + yOffset), sizeMainButton));

			if (!IsScenicLook())
			{
				m_rectCaptionText.l = m_pMainButton->GetRect().r + xMargin;
			}
			else
			{
				CRect rectMainBtn = rect;
				rectMainBtn.t = m_rectCaption.IsRectEmpty() ? rect.t : m_rectCaption.b;
				rectMainBtn.b = rectMainBtn.t + m_nTabsHeight;
				rectMainBtn.r = rectMainBtn.l + sizeMainButton.w;

				m_pMainButton->SetRect (rectMainBtn);

				if (IsQuickAccessToolbarOnTop())
				{
					m_rectCaptionText.l = m_rectCaption.l + 
	...
	else
	{
		...
		if (IsQuickAccessToolbarOnTop())
		{
			...
			const int yOffset = max (0, 
				(m_rectCaptionText.Height() - sizeAQToolbar.h) / 2);

			m_QAToolbar.m_rect.t += yOffset;
			m_QAToolbar.m_rect.b = m_QAToolbar.m_rect.t + sizeAQToolbar.h;

			if (globalData.DwmIsCompositionEnabled())
			{
				m_QAToolbar.m_rect.t += yMargin;
			}

			m_QAToolbar.m_rect.r = 
				min (m_QAToolbar.m_rect.l + sizeAQToolbar.w, 
					m_rectCaptionText.r - 50);

			m_QAToolbar.OnAfterChangeRect (&dc);
			
			int nQAActualWidth = m_QAToolbar.GetActualWidth();
			int nQARight = m_QAToolbar.m_rect.l + nQAActualWidth + xMargin;

			if (nQARight < m_QAToolbar.m_rect.r)
			{
				m_QAToolbar.m_rect.r = nQARight;
			}

			m_rectCaptionText.l = m_QAToolbar.m_rect.r;
			if (!IsScenicLook())
			{
				m_rectCaptionText.l += CBCGPVisualManager::GetInstance()->GetRibbonQATRightMargin();
			}
			else
			{
				m_rectCaptionText.l += 3 * xMargin;
			}
		}
		else
		{
			m_QAToolbar.m_rect = rect;
			m_QAToolbar.m_rect.t = m_QAToolbar.m_rect.b - sizeAQToolbar.h;
			nQAToolbarHeight = sizeAQToolbar.h;

			rectCategory.b = m_QAToolbar.m_rect.t;
		}
	}
	...
	if (!bHideAll)
	{
		const int yTabTop = m_rectCaption.IsRectEmpty() ? rect.t : m_rectCaption.b;
		const int yTabBottom = rect.b - nCategoryHeight - nQAToolbarHeight;
		...
		const int yOffset = max (0, (yTabBottom - yTabTop - sizeTabElemens.h) / 2);
		const int nTabElementsHeight = min (m_nTabsHeight, sizeTabElemens.h);

		m_TabElements.m_rect = CRect (
			CPoint (rect.r - sizeTabElemens.w, yTabTop + yOffset), 
			CSize (sizeTabElemens.w, nTabElementsHeight));
		...
		if (nTabs > 0)
		{
			const int nTabLeftOffset = sizeMainButton.w + 1;
			const int cxTabsArea = rect.Width() - nTabLeftOffset - sizeTabElemens.w - xMargin;
			const int nMaxTabWidth = cxTabsArea / nTabs;

			int x = rect.l + nTabLeftOffset;
					...
					if (uiContextID != 0 && m_bReplaceFrameCaption)
					{
						...
						if (bIsSingle)
						{
							...
							if (pCaption != NULL)
							{
								...
								const int nMinCaptionWidth = dc.GetTextExtent (pCaption->GetText()).w + 2 * xTabMargin;
			...
			for (i = 0; i < m_arCategories.GetSize(); i++)
			{
				...
				if (uiContextID != 0 && m_bReplaceFrameCaption)
				{
					...
					if (pCaption != NULL)
					{
						...
						rectCaption.l = rectOld.l == 0 ? x : rectOld.l;
						rectCaption.r = min (xSysButtonsLeft, x + nCaptionWidth);

						if (bIsFirstContextTab)
						{
							if (IsQuickAccessToolbarOnTop() &&
								rectCaption.l - xTabMargin < m_QAToolbar.m_rect.r)
							{
								m_QAToolbar.m_rect.r = rectCaption.l - xTabMargin;

								if (m_QAToolbar.m_rect.r <= m_QAToolbar.m_rect.l)
								{
									m_QAToolbar.m_rect.SetRectEmpty();
								}
								...
								m_rectCaptionText.l = rectCaption.r + xTabMargin;
								bCaptionOnRight = TRUE;
							}
							else
							{
								const int yCaptionRight = min (m_rectCaptionText.r, x);
								const int nCaptionWidthLeft = yCaptionRight - m_rectCaptionText.l;
								const int nCaptionWidthRight = m_rectCaption.r - rectCaption.r - xTabMargin;
								...
								{
									m_rectCaptionText.r = yCaptionRight;
								}
							}
	...
	if (m_pActiveCategory != NULL)
	{
		...
		if (nCategoryHeight > 0)
		{
			...
			{
				rectCategoryRedraw.l = rectLastPaneOld.r;
			}
		}
	}
		...
		m_bReplaceFrameCaption)
	{
		...
		BCGPMARGINS margins;
		margins.wLeftWidth = 0;
		margins.wRightWidth = 0;
		margins.hTopHeight = m_rectCaption.b;
		margins.hBottomHeight = 0;
	...
	else
	{
		...
		CRect rectTabs = rect;
		rectTabs.t = m_rectCaption.IsRectEmpty() ? rect.t : m_rectCaption.b;
		rectTabs.b = rectTabs.t + m_nTabsHeight + 2 * yTabMargin;
												 ...
												 BOOL bCheckPanelCaption)
{
	...
	if (m_pMainButton != NULL)
	{
		...
		if (!IsScenicLook())
		{
			rectMainButton.l = rectMainButton.t = 0;
		}
									...
									const CRect& rectImage)
{
	...
	if (sizeElemImage == CSize (0, 0) ||
		sizeElemImage.w > rectImage.Width() ||
		sizeElemImage.h > rectImage.Height())
	{
		globalData.EnableRibbonImageScale (bIsRibbonImageScale);
		return FALSE;
	}

	int dx = (rectImage.Width() - sizeElemImage.w) / 2;
	int dy = (rectImage.Height() - sizeElemImage.h) / 2;
...
void CBCGPRibbonBar::OnControlBarContextMenu (CWnd* pParentFrame, CPoint point)
{
	...
	if (point == CPoint (-1, -1))
	{
		...
		if (pFocused != NULL)
		{
			...
			OnShowRibbonContextMenu (this, rectFocus.l, rectFocus.t, pFocused);
...
//****
void CBCGPRibbonBar::OnSizing(UINT fwSide, IRECT* pRect) 
{
	if (CBCGPPopupMenu::GetActiveMenu() != NULL)
	{
		CBCGPPopupMenu::GetActiveMenu()->SendMessage (WM_CLOSE);
...
LRESULT CBCGPRibbonBar::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	...
	if (message == WM_NCHITTEST)
	{
		...
		if (!GetParent()->IsZoomed())
		{
			...
			rectResizeTop.r = m_rectSysButtons.l - 1;
			rectResizeTop.b = rectResizeTop.t + GetSystemMetrics (SM_CYSIZEFRAME) / 2;
...
void CBCGPRibbonBar::UpdateToolTipsRect()
{
	if (m_pToolTip->GetSafeHwnd() != NULL)
	{
		...
		if (m_bIsTransparentCaption)
		{
			rectToolTipClient.t = m_rectCaption.b + 1;
			rectToolTipCaption = m_rectCaption;
			rectToolTipCaption.r = m_rectSysButtons.l - 1;
		}
		...
		case VK_RIGHT:
			{
				...
				for (int x = xStart; pFocusedNew == NULL; x += xStep)
				{
					...
					if (x >= rectCurr.l && x <= rectCurr.r)
					{
						break;
					}

					CRect rectArea (x, rectCurr.t, x + nStep, rectCurr.b);
		...
		case VK_DOWN:
			{
				...
				for (int i = 0; pFocusedNew == NULL; i++)
				{
					...
					if (x1 < rectElems.l && x2 > rectElems.r)
					{
						break;
					}
