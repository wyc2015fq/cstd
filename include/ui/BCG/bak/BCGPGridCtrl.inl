...
// Overrides
	...
	public:
	virtual BOOL Create(CBCGPGridCtrl* pOwnerGrid, const IRECT& rect, CWnd* pParentWnd);
	//}}AFX_VIRTUAL
...
CBCGPGridItem* CBCGPGridItem::HitTest (CPoint point, CBCGPGridRow::ClickArea* pnArea)
{
	...
	if (rect.PtInRect (point))
	{
		...
		if (pGrid != NULL && pGrid->m_nHorzScrollOffset > 0 && pGrid->m_Columns.IsFreezeColumnsEnabled())
		{
			...
			if (point.x < pGrid->m_rectList.l + pGrid->m_Columns.GetFreezeOffset() &&
...
void CBCGPGridItem::OnDrawValue (CDC* pDC, CRect rect)
{
	...
	m_bValueIsTrancated = pDC->GetTextExtent (GetLabel()).w > rect.Width();
}
//****
...
COLORREF CBCGPGridItem::OnFillBackground (CDC* pDC, CRect rect)
{
	...
	else if (bActiveItem && bNoHighlightActiveItem)
	{
		...
		if (pWndList->m_brBackground.IsValid())
		{
...
void CBCGPGridItem::OnDrawBorders (CDC* pDC, CRect rect)
{
	...
	if (borders.t != GRID_BORDERSTYLE_EMPTY)
	{
	}

	if (borders.l != GRID_BORDERSTYLE_EMPTY)
	{
	}

	if (borders.b != GRID_BORDERSTYLE_EMPTY)
	{
		pDC->MoveTo (rect.l, rect.b);
		pDC->LineTo (rect.r + 1, rect.b);
	}

	if (borders.r != GRID_BORDERSTYLE_EMPTY)
	{
...
void CBCGPGridItem::OnPrintValue (CDC* pDC, CRect rect)
{
	...
	if (rectClipText.IntersectRect (&rectClipText, &rectClip))
	{
		...
		int nVCenterOffset = (rectText.Height() - pDC->GetTextExtent (strVal).h + nDescent) / 2;

		pDC->SetTextAlign (TA_LEFT | TA_TOP);
		pDC->ExtTextOut (rectText.l, rectText.t + nVCenterOffset, ETO_CLIPPED, &rectClipText, strVal, NULL);
	}
...
BOOL CBCGPGridItem::OnEdit (LPPOINT)
{
	...
	if (m_pWndInPlace != NULL)
	{
		...
		if (m_dwFlags & BCGP_GRID_ITEM_HAS_LIST)
		{
			...
			m_pWndCombo->SetWindowPos (NULL, rectCombo.l,
				rectCombo.t - dy + 1, -1, -1, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
...
void CBCGPGridItem::AdjustButtonRect()
{
	...
	{
		m_rectButton = m_Rect;
		m_rectButton.l = m_rectButton.r - m_rectButton.Height() + 3;
		m_rectButton.t ++;
	}
	else
	{
...
void CBCGPGridItem::AdjustInPlaceEditRect (CRect& rectEdit, CRect& rectSpin)
{
	...
	if (GetMergedCells() != NULL)
	{
		...
		if (rectEdit.t < pWndList->m_rectList.t)
		{
			rectEdit.t = min (rectEdit.b, pWndList->m_rectList.t);
		}
		if (rectEdit.b > pWndList->m_rectList.b)
		{
			rectEdit.b = max (rectEdit.t, pWndList->m_rectList.b);
		}
	}
	else
	{
	...
	if (rectEdit.l < pWndList->m_rectList.l)
	{
		rectEdit.l = pWndList->m_rectList.l;
	}
	if (rectEdit.r > pWndList->m_rectList.r)
	{
		rectEdit.r = max (rectEdit.l, pWndList->m_rectList.r);
	}
	...
	rectEdit.l += TEXT_MARGIN - nMargin;

	if (HasButton())
	{
		AdjustButtonRect();
		rectEdit.r = m_rectButton.l - 1;
	}
	else
	{
		rectEdit.r--;
	...
	{
		rectSpin = m_Rect;
		rectSpin.r = rectEdit.r;
		rectSpin.l = rectSpin.r - rectSpin.Height();
		rectSpin.t ++;
		rectEdit.r = rectSpin.l;
...
CComboBox* CBCGPGridItem::CreateCombo (CWnd* pWndParent, CRect rect)
{
	...
	rect.r = max (rect.l + m_sizeCombo.w, rect.r);
	rect.b = rect.t + m_sizeCombo.h;
...
{
	borders.t = GRID_BORDERSTYLE_EMPTY;
	borders.l = GRID_BORDERSTYLE_EMPTY;
	borders.b = GRID_BORDERSTYLE_DEFAULT;
	borders.r = GRID_BORDERSTYLE_DEFAULT;
}
//****
...
void CBCGPGridColorItem::OnDrawValue (CDC* pDC, CRect rect)
{
	...
	rectColor.r = rectColor.l + rectColor.Height();
...
void CBCGPGridColorItem::OnPrintValue (CDC* pDC, CRect rect)
{
	...
	rectColor.r = rectColor.l + rectColor.Height();
	rectColor.DeflateRect (szOne.w, szOne.h);
	rectColor.t += szOne.h;
	rectColor.l += szOne.w;
	...
	if (rectColor.IntersectRect (&rectColor, &rectClip))
	{
		...
		pDC->LineTo (rectColor.r, rectColor.t);
		pDC->LineTo (rectColor.BottomRight());
		pDC->LineTo (rectColor.l, rectColor.b);
...
void CBCGPGridColorItem::AdjustInPlaceEditRect (CRect& rectEdit, CRect& rectSpin)
{
	...
	if (rectEdit.r > pWndList->GetListRect().r)
	{
		rectEdit.r = max (rectEdit.l, pWndList->GetListRect().r);
	}
	...
	rectEdit.l = rectEdit.l + m_Rect.Height() + TEXT_MARGIN - nMargin + 1;
...
CString CBCGPGridColorItem::FormatItem()
{
	...
	str.Format (_T("%02x%02x%02x"),
		GetRV (m_Color), GetGV (m_Color), GetBV (m_Color));
...
void CBCGPGridCheckItem::OnDrawValue (CDC* pDC, CRect rect)
{
	...
	rectCheck.l = rectCheck.CenterPoint().x - nWidth / 2;
	rectCheck.r = rectCheck.l + nWidth;
...
void CBCGPGridURLItem::OnDrawValue (CDC* pDC, CRect rect)
{
	...
	m_bValueIsTrancated = pDC->GetTextExtent (str).w > rect.Width();
									 ...
									 CBCGPGridRow::ClickArea* pnArea)
{
	...
	if (m_Rect.PtInRect (point))
	{
		...
		rectExpand.r = min (rectExpand.l + m_pWndList->m_nRowHeight, rectExpand.r);
...
void CBCGPGridRow::Repos (int& y)
{
	...
	if (bShowItem)
	{
		...
		int nRowLeft = rectList.l;
		int nRowLeftScrolled = rectList.l - nHorzScrollOffset;
		...
		while (nPos != m_pWndList->m_Columns.End())
		{
			...
			int nTreeOffset = bIsTreeColumn ? m_pWndList->GetExtraHierarchyOffset() * szOne.w + dx : 0; 
			if (bIsTreeColumn)
			{
				nWidth += m_pWndList->GetExtraHierarchyOffset() * szOne.w +
					m_pWndList->GetHierarchyOffset() * szOne.w;
			}
			...
			if (bShowItem && bShowColumn)
			{
				...
				{
					pItem->m_rectButton.t = pItem->m_Rect.t + 1;
					pItem->m_rectButton.b = pItem->m_Rect.b;
				}
			}
			else
			{
...
void CBCGPGridRow::Shift (int dx, int dy)
{
	...
	{
		const int nFreezeOffset = m_pWndList->m_rectList.l + m_pWndList->m_Columns.GetFreezeOffset();
		CRect rect = m_Rect;
		rect.r = nFreezeOffset;
...
void CBCGPGridRow::OnDrawName (CDC* pDC, CRect rect)
{
	...
	COLORREF clrHighlight = globalData.clrHotLinkNormalText;
		...
		if (m_pWndList->m_bHighlightGroups && IsSelected())
		{
			...
			rectFocus.t = rectFocus.CenterPoint().y - nTextHeight / 2;
			rectFocus.b = rectFocus.t + nTextHeight;
			rectFocus.r = rect.r;
			rectFocus.r = 
				min (rect.r, rectFocus.l + pDC->GetTextExtent (strName).w);
		...
		m_bNameIsTrancated = pDC->GetTextExtent (strName).w > rect.Width();
	}
	else
	{
	...
	for (int i = 0; i < m_arrRowItems.GetSize(); i++)
	{
		...
		if (!rectValue.IsRectEmpty())
		{
				...
				!m_pWndList->m_Columns.IsColumnFrozen (i))
			{
				...
				rectClipItem.l = max (nFreezeOffset, rectClipItem.l);
				if (rectClipItem.l >= rectClipItem.r)
				{
					continue;
				}
			}
...
void CBCGPGridRow::OnPrintName (CDC* pDC, CRect rect)
{
	...
	if (!HasValueField())
	{
		...
		if (rectClipText.IntersectRect (&rectClipText, &rectClip))
		{
			...
			int nDescent = tm.tmDescent;
			int nVCenterOffset = (rectText.Height() - pDC->GetTextExtent (strName).h + nDescent) / 2;

			pDC->SetTextAlign (TA_LEFT | TA_TOP);
			pDC->ExtTextOut (rectText.l, rectText.t + nVCenterOffset, ETO_CLIPPED, &rectClipText, strName, NULL);
...
void CBCGPGridRow::OnDrawExpandBox (CDC* pDC, CRect rect)
{
	...
	if (m_pWndList->m_bIsPrinting)
	{
		...
		ptCenter.Offset (-(ptCenter.x % szOne.w), -(ptCenter.y % szOne.h));

		int nMinSize = (szOne.w != 0 && szOne.h != 0) ?
			min (rect.Width() / szOne.w, rect.Height() / szOne.h) : 0;
		int nBoxSize = min (9, nMinSize); // in logical units

		rect = CRect (ptCenter, szOne);
		rect.InflateRect ((nBoxSize / 2) * szOne.w, (nBoxSize / 2) * szOne.h);
		...
		rect.DeflateRect (2 * szOne.w, 2 * szOne.h);

		pDC->MoveTo (rect.l, ptCenter.y + szOne.h / 2);
		pDC->LineTo (rect.r, ptCenter.y + szOne.h / 2);

		if (!bIsOpened)
		{
			pDC->MoveTo (ptCenter.x + szOne.w / 2, rect.t);
			pDC->LineTo (ptCenter.x + szOne.w / 2, rect.b);
		}
...
CRect CBCGPGridRow::GetNameTooltipRect()
{
	...
	if (!m_Rect.IsRectEmpty())
	{
			...
			!m_lstSubItems.IsEmpty())
		{
			...
			rectName.l += m_pWndList->GetRowHeight() + dx;
		}

		if (rectName.r > rectName.l)
		{
			return rectName;
		}
	}
...
HBRUSH CBCGPGridRow::OnCtlColor(CDC* pDC, UINT nCtlColor)
{
	...
	if (IsGroup() && m_pWndList->m_brGroupBackground.IsValid())
	{
	...
	else if (m_pWndList->m_brBackground.IsValid())
	{
									   ...
									   BOOL bIsGroupBox)
{
	...
	else if (nSortVal != 0 && !bNoSortArrow)
	{
		...
		{
			rectArrow.l = rectArrow.r - rectArrow.Height();
			rect.r = rectArrow.l - 1;
...
void CBCGPGridColumnsInfo::GetColumnRect (int nColumn, CRect& rect) const
{
	...
	while (nPos != End())
	{
		...
		rectColumn.l = x;
		rectColumn.r = rectColumn.l + GetColumnWidth (iColumn);
	...
	rectColumn.l = x - m_pWndList->m_nHorzScrollOffset;
	rectColumn.r = m_pWndList->m_rectHeader.r + 2;
	rect = rectColumn;
}
//****
										 ...
										 CBCGPGridColumnsInfo::ClickArea* pnArea) const
{
	...
	int nXLeft = m_pWndList->m_rectHeader.l;
	int nXLeftScrolled = m_pWndList->m_rectHeader.l -m_pWndList->m_nHorzScrollOffset;
	BOOL bFirst = TRUE;
	int nCount = 0;	// count visible columns
...
CBitmap* CBCGPGridCtrl::m_hcurDeleteColumn = NULL;
CBitmap* CBCGPGridCtrl::m_hcurNoDropColumn = NULL;
...
void CBCGPGridCtrl::AdjustLayout()
{
	...
	CSize szScroll (0, 0);
	CPoint ptScroll (rectClient.l, rectClient.t);
	if (m_nVertScrollTotal > 0)
	{
		m_rectList.r -= cxScroll;
		m_rectHeader.r -= cxScroll;
		szScroll.h = rectClient.Height();
	...
	{
		m_rectRowHeader.b -= cyScroll;
		m_rectList.b -= cyScroll;
		szScroll.w = max (0, m_rectRowHeader.Width() + m_rectList.Width());
		szScroll.h = max (0, szScroll.h - cyScroll);
	}

	m_wndScrollVert.EnableScrollBar (szScroll.h > 0 ? ESB_ENABLE_BOTH : ESB_DISABLE_BOTH);

	if (szScroll.h > 0)
	{
		m_wndScrollVert.SetWindowPos (NULL, m_rectList.r, ptScroll.y,
			cxScroll, szScroll.h, SWP_NOZORDER | SWP_NOACTIVATE);
	}
	else
	{
	...
	if (szScroll.w > 0)
	{
		m_wndScrollHorz.SetWindowPos (NULL, ptScroll.x, rectClient.b - cyScroll,
			szScroll.w, cyScroll, SWP_NOZORDER | SWP_NOACTIVATE);
	}
	else
	{
...
void CBCGPGridCtrl::ReposItems()
{
		...
		for (POSITION pos = m_CachedItems.m_lstCache.GetHeadPosition(); pos != NULL; )
		{
			...
			for (int i = 0; i < cpi.nSize; i++)
			{
				...
				if (pItem != NULL)
				{
					...
					int y = m_rectList.t - 1 + nIndex * m_nRowHeight - m_nVertScrollOffset;
					m_idCur.m_nRow = nIndex;
					pItem->Repos (y);

					if (m_nFirstVisibleItem == -1 &&
						pItem->GetRect().b >= m_rectList.t)
					{
	...
	int y = m_rectList.t - m_nVertScrollOffset - 1;
	int nIndex = 0;
...
HFONT CBCGPGridCtrl::SetCurrFont (CDC* pDC)
{
	...
	if (pDC->IsPrinting())
	{
		...
		{
			return (HFONT) ::SelectObject (pDC, m_hPrinterFont);
		}
	}
	
	return (HFONT) ::SelectObject (pDC, 
		m_hFont != NULL ? m_hFont : GetDefaultFont());
}
//****
void CBCGPGridCtrl::OnPaint(CDC* pDC) 
{
	...
	m_rectClip.t = max (m_rectClip.t, m_rectList.t);

	CRect rect;
	GetClientRect (rect);
	rect.r = m_rectList.r;
	rect.b = m_rectList.b + m_nGridFooterHeight;

	CRect rectGripper;
	GetClientRect (rectGripper);
	rectGripper.l = m_rectList.r;
	rectGripper.t = m_rectList.b;
	...
	rectFill.t += m_rectHeader.Height() + m_nGroupByBoxHeight;
	rectFill.l += m_rectRowHeader.Width();
	...
	rectGroupByBox.b = min (rectGroupByBox.t + m_nGroupByBoxHeight, rectClient.b);
	OnDrawGroupByBox (pDC, rectGroupByBox);

	OnDrawSelectAllArea (pDC);
	OnDrawHeader (pDC);

	::SelectObject (pDC, hfontOld);
									  ...
									  BOOL bDrawRightBorder)
{
	...
	const int nRightBorder = rectFill.l + GetHierarchyOffset() + GetExtraHierarchyOffset();
		...
		m_ColorData.m_LeftOffsetColors.m_nGradientAngle == 0)
	{
		...
		rectGradient.r = nRightBorder;
		m_ColorData.m_LeftOffsetColors.Draw (pDC, rectGradient, TRUE);

		if (rectGradient.r < rectFill.r)
		{
			CRect rectOneColor = rectFill;
			rectOneColor.l = rectGradient.r;
		...
		m_ColorData.m_LeftOffsetColors.m_clrBorder != (COLORREF)-1)
	{
		...
		pDC->MoveTo (rectFill.r - 1, rectFill.t);
		pDC->LineTo (rectFill.r - 1, rectFill.b);
...
void CBCGPGridCtrl::OnDrawList (CDC* pDC)
{
	...
	int nXLeft = m_rectList.l - m_nHorzScrollOffset;
	int nCount = 0;	// count visible columns
	if (!IsKindOf (RUNTIME_CLASS (CBCGPReportCtrl)))
	{
		...
		while (nPos != m_Columns.End())
		{
			...
			if (!m_bGridItemBorders)
			{
				...
				pDC->MoveTo (nXLeft - 1, m_rectList.t - 1);
				pDC->LineTo (nXLeft - 1, m_rectList.b);
			}
		}
	}
...
BOOL CBCGPGridCtrl::OnDrawItem (CDC* pDC, CBCGPGridRow* pItem)
{
	...
	if (!pItem->m_Rect.IsRectEmpty())
	{
		if (pItem->m_Rect.t >= m_rectClip.b)
		{
			return FALSE;
		}

		if (pItem->m_Rect.b >= m_rectClip.t)
		{
			...
			{
				rectLeft.r = rectLeft.l + GetExtraHierarchyOffset() + dx;
			}
			rectLeft.b++;

			CRgn rgnClipLeft;
			CRect rectLeftClip = rectLeft;
			rectLeftClip.l = max (rectLeftClip.l, m_rectList.l);
			rectLeftClip.b = min (rectLeftClip.b + 1, m_rectList.b);
			...
			if (m_Columns.GetColumnCount (TRUE) > 0)
			{
				...
				if (nCol0Idx != -1)
				{
					...
					if (nCol0Width > 0)
					{
						rectLeftClip.r = min (rectLeftClip.r, pItem->m_Rect.l + nCol0Width + (bNoScrollCol0? m_nHorzScrollOffset : 0));
					}
				}
			}

			if (rectLeftClip.l < rectLeftClip.r)
			{
			...
			if (!pItem->HasValueField() || bIsAutoGroup)
			{
				...
				if (!m_bHighlightGroups)
				{
					...
					{
						if (m_brGroupBackground.IsValid() ||
							m_brBackground.IsValid())
						{
							CBrush& br = (m_brGroupBackground.IsValid()) ? 
				...
				!pItem->m_lstSubItems.IsEmpty())
			{
				...
				rectName.l += m_nRowHeight + dx;
				rectExpand.r = rectName.l;
				rectExpand.DeflateRect (dx, 0, 0, 0);

				CRgn rgnClipExpand;
				CRect rectExpandClip = rectExpand;
				rectExpandClip.b = min (rectExpandClip.b, m_rectList.b);
				...
				!pItem->IsGroup() && GetExtraHierarchyOffset() > 0)
			{
				...
				rectRowMarker.l = max (
					rectRowMarker.r - GetExtraHierarchyOffset(), 
					rectRowMarker.l);

				CRgn rgnClipMarker;
				CRect rectMarkerClip = rectRowMarker;
				rectMarkerClip.l = min (rectMarkerClip.l, m_rectList.l);
				rectMarkerClip.b = min (rectMarkerClip.b, m_rectList.b);
			...
			if (rectName.r > rectName.l)
			{
				...
				rectNameClip.b = min (rectNameClip.b, m_rectList.b);
				...
				if (pItem->IsGroup())
				{
					hOldFont = (HFONT) ::SelectObject (pDC, m_fontBold.GetSafeHandle());
				}
			...
			if (pItem->HasValueField() && !bIsAutoGroup)
			{
				...
				rectValClip.b = min (rectValClip.b, m_rectList.b);
				rectValClip.b++;
				...
				if (!m_bGridItemBorders)
				{
					...
					pDC->MoveTo (m_rectList.l + GetExtraHierarchyOffset() + dx - m_nHorzScrollOffset, pItem->m_Rect.b);
					pDC->LineTo (m_rectList.r, pItem->m_Rect.b);
				}
...
void CBCGPGridCtrl::OnDrawGroupByBox (CDC* pDC, CRect rect)
{
	...
	if (m_Columns.GetGroupColumnCount() > 0)
	{
		...
		for (int i = 0; i < m_Columns.GetGroupColumnCount(); i++)
		{
			...
			if (nCol != -1)
			{
				...
				int nItemWidth = pDC->GetTextExtent (GetColumnName (nCol)).w + 
					CALCULATED_COLUMNWIDTH + CALCULATED_TEXT_MARGIN;

				rectItem.b = min (rectItem.t + nItemHeight, rect.b);
				rectItem.r = min (rectItem.l + nItemWidth, rect.r);
	...
	else if (!m_bIsPrinting)
	{
		...
		int nTextWidth = pDC->GetTextExtent (strTitle).w;
		rect.r = min (rect.r, rect.l + nTextWidth + 2 * CALCULATED_TEXT_MARGIN);
...
void CBCGPGridCtrl::OnDrawHeader (CDC* pDC)
{
	...
	const int nFreezeOffset = m_rectList.l + m_Columns.GetFreezeOffset();
	...
	for (int i = 0; i <= m_Columns.GetColumnCount(); i++)
	{
		...
		if (m_nHorzScrollOffset > 0)
		{
			...
			if (m_rectHeader.l > rectColumnClip.l)
			{
				rectColumnClip.l = min (rectColumnClip.r, m_rectHeader.l);
			}

			if (m_Columns.IsFreezeColumnsEnabled() && !m_Columns.IsColumnFrozen (i))
			{
				if (nFreezeOffset > rectColumnClip.l)
				{
					rectColumnClip.l = min (rectColumnClip.r, nFreezeOffset);
				}
			}
...
void CBCGPGridCtrl::OnDrawRowHeaderItem (CDC* pDC, CBCGPGridRow* pItem)
{
	...
	// ----------------------------------
	{
		...
		rectItem.t = pItem->m_Rect.t;
		rectItem.b = pItem->m_Rect.b;
...
CRect CBCGPGridCtrl::OnGetSelectionRect()
{
	...
	if (pSelRow != NULL)
	{
		...
		if (IsFilterEnabled() && m_idActive.IsColumn())
		{
			...
			rectSel.t = m_rectList.t;
			rectSel.b = m_rectList.b;
		}

		if (pSelItem == NULL)
		{
			rectSel.r = max (rectSel.r - 1, rectSel.l);
			rectSel.l = min (rectSel.r, rectSel.l + nLeftOffset);

			if (bFreeze)
			{
				rectSel.l = min (rectSel.r, m_rectList.l + nLeftOffset);
			}
		}
	...
	if (pLastSelRow != NULL)
	{
		...
		if (IsFilterEnabled() && m_idLastSel.IsColumn())
		{
			...
			rectLastSel.t = m_rectList.t;
			rectLastSel.b = m_rectList.b;
		}

		if (pLastSelItem == NULL)
		{
			rectLastSel.l = min (rectLastSel.r, rectLastSel.l + nLeftOffset);
			rectLastSel.r = rectSel.r;

			if (bFreeze)
			{
				rectLastSel.l = min (rectLastSel.r, m_rectList.l + nLeftOffset);
			}
		}
	...
	if (bFreeze && pSelItem != NULL && pLastSelItem != NULL)
	{
		...
		{
			m_rectClipSel.l = min (nFreezeOffset, m_rectClipSel.r);
		}
		else if (bA && bB)
		{
		}
		else
		{
			...
			if (bA && !bB)
			{
				...
				if (rectTrackSel.l < rectSel.l)
				{
					rectTrackSel.l = rectSel.l;
				}
			}
			else if (!bA && bB)
			{
				// scrolled column must not change left border of selection
				if (rectTrackSel.l < rectLastSel.l)
				{
					rectTrackSel.l = rectLastSel.l;
				}
			}
			...
			if (rectTrackSel.r < nFreezeOffset)
			{
				rectTrackSel.r = nFreezeOffset;
			}
	...
	{
		m_rectClipSel.l = min (m_rectList.l, m_rectClipSel.r);
	}
...
void CBCGPGridCtrl::OnItemChanged (CBCGPGridItem* pItem, int nRow, int nColumn)
{
	...
	SendMessage(BCGM_GRID_ITEM_CHANGED, GetDlgCtrlID(),
		LPARAM (&ii));
}
//****
...
void CBCGPGridCtrl::TrackHeader (int nOffset)
{
	...
	else
	{
		...
		m_rectTrackHeader.l += (nOffset - m_rectList.l);
		m_rectTrackHeader.r = m_rectTrackHeader.l + 1;

		// Calculate column left offset:
		int nLeftOffset = m_rectHeader.l;
		int nCount = 0;	// count visible columns
		...
		rectTrackHeaderLeft.l += (nNewOffset - m_rectList.l);
		rectTrackHeaderLeft.r = rectTrackHeaderLeft.l + 1;
...
void CBCGPGridCtrl::OnDrawHeaderMenuButton (CDC* pDC, CRect rect, int nColumn, BOOL bSortArrow)
{
	...
	{			
		...
		CPoint pt = rect.TopLeft();
		pt.x ++;
		pt.y += max (0, (rect.Height() - sizeImage.h) / 2);

		if (rect.Width() > sizeImage.w)
		{
			pt.x += (rect.Width() - sizeImage.w) / 2;
...
CRect CBCGPGridCtrl::GetHeaderMenuButtonRect (CRect rectItem, int nColumn) const
{
	...
	{
		rectBtn = rectItem;
		rectBtn.l = max (rectBtn.r - rectBtn.Height(), rectBtn.l);
	}
...
int CBCGPGridCtrl::GetGroupColumnRect (int nPos, CRect& rect, CDC* pDC)
{
	...
	for (int i = 0; i < m_Columns.GetGroupColumnCount(); i++)
	{
		...
		if (nCol != -1)
		{
			...
			int nItemWidth = pDC->GetTextExtent (strColumn).w + 
				BCGPGRID_GROUPBYBOX_COLUMNWIDTH + TEXT_MARGIN;

			rectItem.b = min (rectItem.t + nItemHeight, rectGroupByBox.b);
			rectItem.r = min (rectItem.l + nItemWidth, rectGroupByBox.r);
...
void CBCGPGridCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (nColumnHit != -1)
	{
		...
		TrackHeader (rectHeaderColumn.r);
		m_bTracking = TRUE;
		return;
	}
...
void CBCGPGridCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (OnTrackHeader() && point.y >= m_rectList.t)
	{
		CPoint ptHeader = point;
		ptHeader.y = m_rectHeader.t;
...
//****
BOOL CBCGPGridCtrl::Create(DWORD dwStyle, const IRECT& rect, CWnd* pParentWnd, UINT nID)
{
	return CWnd::Create (
...
{	
	...
	for (; pos != NULL; i++)
	{
		...
		if (!pItem->m_Rect.IsRectEmpty())
		{
			if (pItem->m_Rect.b >= m_rectList.b)
			{
				nLast = i - 1;
				return nCount;
			}

			if (pItem->m_Rect.b >= m_rectList.t)
			{
				nCount ++;
...
void CBCGPGridCtrl::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* /*pScrollBar*/)
{
	...
	{
		rectClip.l = min (m_rectList.l + m_Columns.GetFreezeOffset(), m_rectList.r);
	}
		...
		else if (!m_bWholeRowSel)
		{
			...
			if (pItemCurSel != NULL)
			{
				...
				else
				{
					...
					if (pItemCurSel->GetMergedCells() != NULL)
					{
						rect.l = pItemCurSel->GetMergedRect().l;
					}
					CPoint point (rect.l - 2, rect.b - 1);
	...
	case VK_UP:
		{
			...
			if (pRowCurSel != NULL)
			{
				...
				{
					rect.t = pItemCurSel->GetMergedRect().t;
				}
				CPoint point (rect.r - 1, rect.t - 2);
		...
		else if (!m_bWholeRowSel)
		{
			...
			if (pItemCurSel != NULL)
			{
				...
				else
				{
					...
					if (pItemCurSel->GetMergedCells() != NULL)
					{
						rect.r = pItemCurSel->GetMergedRect().r;
					}
					CPoint point (rect.r + 2, rect.b - 1);
	...
	case VK_DOWN:
		{
			...
			if (pRowCurSel != NULL)
			{
				...
				if (pItemCurSel != NULL && pItemCurSel->GetMergedCells() != NULL)
				{
					rect.b = pItemCurSel->GetMergedRect().b;
				}
				CPoint point (rect.r - 1, rect.b + 2);
	...
	case VK_NEXT:
		{
			...
			if (m_nVertScrollPage != 0)
			{
				...
				if (pRowCurSel != NULL)
				{
					...
					ASSERT_VALID (pRowCurSel);
					CRect rect = pRowCurSel->GetRect();
					CPoint point (rect.r - 1,
								  max (rect.t + m_nVertScrollPage - m_nRowHeight,
									   rect.b + 1));
	...
	case VK_PRIOR:
		{
			...
			if (m_nVertScrollPage != 0)
			{
				...
				if (pRowCurSel != NULL)
				{
					...
					CRect rect = pRowCurSel->GetRect();
					CPoint point (rect.r - 1,
								  min (rect.b - m_nVertScrollPage + m_nRowHeight,
									   rect.t - 1));
...
void CBCGPGridCtrl::EnsureVisible (CBCGPGridRow* pRowItem, BOOL bExpandParents/* = FALSE*/)
{
	...
	if (rectRow.t < m_rectList.t - 1 || rectRow.b > m_rectList.b)
	{
		...
		int nNewVertOffset = (rectRow.b > m_rectList.b) ?
			// scroll down
			m_nVertScrollOffset + (rectRow.b - m_rectList.b) :
			// scroll up
			m_nVertScrollOffset + (rectRow.t - m_rectList.t);
	...
	if (rectItem.l < nLeftLimit - 1 || rectItem.r > m_rectList.r)
	{
		int nNewHorzOffset = (rectItem.r > m_rectList.r) ?
			// scroll rigth
			m_nHorzScrollOffset + (rectItem.r - m_rectList.r) :
			// scroll left
			m_nHorzScrollOffset + (rectItem.l - nLeftLimit);
...
void CBCGPGridCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	else if (m_bTracking)
	{
		...
		int nNewOffset = m_Columns.IsColumnFrozen (m_nTrackColumn) ? nLeftOffset : nLeftOffset - m_nHorzScrollOffset;
		int nNewWidth = (point.x - m_rectList.l) - nNewOffset;
		if (nCount == 0)
		{
			nNewWidth -= GetHierarchyOffset() + GetExtraHierarchyOffset();
		}

		BOOL bUpdate = (rectTrackHeader.l == point.x);
		if (bUpdate)
		{
			nNewWidth = max (m_nRowHeight, nNewWidth);
			SetHeaderItemWidth (m_nTrackColumn, nNewWidth);
										...
										BOOL bSelect, BOOL bRedraw)
{
	...
	if (m_bRowMarker)
	{
		if (IsRowMarkerOnRowHeader())
		{
			// update row marker:
			CRect rect = pRow->m_Rect;
			rect.l = m_rectRowHeader.l;
			rect.r = m_rectRowHeader.r;
			InvalidateRect (rect);
		}
		else if (!pRow->IsGroup() && GetExtraHierarchyOffset() > 0)
		{
			...
			if (m_nHorzScrollOffset > 0 && m_Columns.IsFreezeColumnsEnabled())
			{
				rect.r = m_rectList.l + GetExtraHierarchyOffset() + dx;
			}
			else
			{
				rect.r = rect.l + GetExtraHierarchyOffset() + dx;
			}
			InvalidateRect (rect);
		}
	}
...
//****
int CBCGPGridCtrl::HitTestGroupByBox (CPoint point, IRECT* lprectItem/* = NULL*/)
{
	ASSERT_VALID (this);
...
int CBCGPGridCtrl::GetGroupByBoxDropIndex (CPoint point, LPPOINT lpptDrop/* = NULL*/)
{
	...
	ptDrop.x = rectGroupByBox.l;
	ptDrop.y = (rectGroupByBox.t + rectGroupByBox.b - BCGPGRID_GROUPBYBOX_VMARGIN) / 2;
	...
	for (i = 0; i < m_Columns.GetGroupColumnCount(); i++)
	{
		...
		if (bFirst)
		{
			ptDrop.x = rectColumn.l;
			ptDrop.y = (rectColumn.t + rectColumn.b) / 2;
...
//****
CBCGPGridRow* CBCGPGridCtrl::HitTestRowHeader (CPoint point, IRECT* lprectItem)
{
	ASSERT_VALID (this);
		...
		if (pHitRow != NULL)
		{
			if (lprectItem != NULL)
			{
				*lprectItem = pHitRow->GetRect();
				lprectItem->l = m_rectRowHeader.l;
				lprectItem->r = m_rectRowHeader.r;
			}
...
int CBCGPGridCtrl::OnCalcPrintPages (CDC* pDC, CPrintInfo* pInfo)
{
	...
	rectDraw.t += m_PrintParams.m_nPageHeaderHeight;
	rectDraw.b -= m_PrintParams.m_nPageFooterHeight;
...
void CBCGPGridCtrl::SetPrintColors()
{
	...
	m_clrPrintBorder = _RGB (0, 0, 0);			// black
	m_clrPrintHeader = _RGB (128, 128, 128);		// grey
	m_clrPrintHeaderBG = _RGB (255, 255, 255);	// white
	m_clrPrintGroup = m_clrPrintHeader;
	m_clrPrintGroupBG = m_clrPrintHeaderBG;
	m_clrPrintLeftOffset = _RGB (192, 192, 192);	// light grey
	m_clrPrintBG = _RGB (255, 255, 255);			// white
	m_clrPrintLine = _RGB (192, 192, 192);			// light grey
	m_clrPrintText = _RGB (0, 0, 0);				// black
}
//****
void CBCGPGridCtrl::OnPrint(CDC* pDC, CPrintInfo* pInfo)
{
	...
	rectGroupByBox.b = rectGroupByBox.t + m_PrintParams.m_nGroupByBoxHeight;
	rectGroupByBox.b = min (rectGroupByBox.b, pInfo->m_rectDraw.b);
	OnDrawGroupByBox (pDC, rectGroupByBox);
...
void CBCGPGridCtrl::OnPrintPageHeader(CDC* pDC, CPrintInfo* pInfo)
{
	...
	if (OnGetPageHeaderRect (pDC, pInfo, rectHeader))
	{
		...
		if (pInfo->m_rectDraw.t < rectHeader.b)
		{
			pInfo->m_rectDraw.t = min (rectHeader.b, pInfo->m_rectDraw.b);
		}
	}
}
//****
void CBCGPGridCtrl::OnPrintPageFooter(CDC* pDC, CPrintInfo* pInfo)
{
	...
	if (OnGetPageFooterRect (pDC, pInfo, rectFooter))
	{
		...
		if (pInfo->m_rectDraw.b > rectFooter.t)
		{
			pInfo->m_rectDraw.b = max (rectFooter.t, pInfo->m_rectDraw.t);
		}
	}
}
//****
...
CRect CBCGPGridCtrl::OnGetGroupByBoxRect (CDC* pDC, const CRect& rectDraw)
{
	...
	if (m_bGroupByBox)
	{
		...
		rect = rectDraw;
		rect.b = rect.t + nGroupByBoxHeight;
	}
	else
	{
		rect.SetRectEmpty();
...
CRect CBCGPGridCtrl::OnGetHeaderRect (CDC* pDC, const CRect& rectDraw)
{
	...
	if (m_bHeader)
	{
		...
		rect = rectDraw;
		rect.t += (m_bIsPrinting ? m_PrintParams.m_nGroupByBoxHeight : m_nGroupByBoxHeight);
		rect.b = rect.t + nHeaderHeight;

		if (m_bRowHeader)
		{
			rect.l += (m_bIsPrinting ? m_PrintParams.m_nRowHeaderWidth : m_nRowHeaderWidth);
		}
	}
	else
	{
...
CRect CBCGPGridCtrl::OnGetRowHeaderRect (CDC*, const CRect& rectDraw)
{
 	...
 	if (m_bRowHeader)
	{
		rect = rectDraw;
		rect.t += (m_bIsPrinting ? m_PrintParams.m_nGroupByBoxHeight : m_nGroupByBoxHeight);
		rect.r = rect.l + (m_bIsPrinting ? m_PrintParams.m_nRowHeaderWidth : m_nRowHeaderWidth);

		if (!m_rectHeader.IsRectEmpty())
		{
			rect.t = m_rectHeader.b;
		}
	}
	else
	{
...
CRect CBCGPGridCtrl::OnGetSelectAllAreaRect (CDC*, const CRect&)
{
	...
	if (m_bIsPrinting)
	{
		rect.SetRect (m_PrintParams.m_rectRowHeader.l, m_PrintParams.m_rectHeader.t, m_PrintParams.m_rectRowHeader.r, m_PrintParams.m_rectHeader.b);
	}
	else
	{
		rect.SetRect (m_rectRowHeader.l, m_rectHeader.t, m_rectRowHeader.r, m_rectHeader.b);
	}
...
CRect CBCGPGridCtrl::OnGetGridRect (CDC*, const CRect& rectDraw)
{
	...
	if (m_bIsPrinting)
	{
		rectList.t += m_PrintParams.m_nGroupByBoxHeight;
		rectList.t += m_PrintParams.m_rectHeader.Height();
		rectList.l += m_PrintParams.m_rectRowHeader.Width();
	}
	else
	{
		rectList.t += m_nGroupByBoxHeight;
		rectList.t += m_rectHeader.Height();
		rectList.t += m_nGridHeaderHeight;
		if (rectList.t + m_nRowHeight < rectList.b)
		{
			rectList.b = 
				max (rectList.t + m_nRowHeight, rectList.b - m_nGridFooterHeight);
		}

		rectList.l += m_rectRowHeader.Width();
	}
...
CRect CBCGPGridCtrl::GetGridHeaderRect() const
{
	...
	rectHeader.t += m_nGroupByBoxHeight + m_rectHeader.Height();
	rectHeader.b = min (rectHeader.t + m_nGridHeaderHeight, rectHeader.b);
	return rectHeader;
}
//****
CRect CBCGPGridCtrl::GetGridFooterRect() const
{
	...
	if (m_nHorzScrollTotal > 0)
	{
		...
		rectFooter.b -= cyScroll;
	}

	rectFooter.t = m_rectList.b;
	rectFooter.b = max (rectFooter.t + m_nGridFooterHeight, rectFooter.b);
...
void CBCGPGridCtrl::PrintLayout (CDC* pDC, CPrintInfo* pInfo)
{
	...
	m_PrintParams.m_pageInfo.m_szOne.w = ::MulDiv (1, nXMul, nXDiv);;
	m_PrintParams.m_pageInfo.m_szOne.h = ::MulDiv (1, nYMul, nYDiv);;
	...
	int y = m_PrintParams.m_rectList.t - 1;
...
int CBCGPGridCtrl::CalcPages (CDC* pDC, CPrintInfo* /*pInfo*/)
{
	...
	rectPage.t = m_PrintParams.m_rectList.t - 1;
	...
	while (pos != NULL && nFirstInPage + nTotal <= m_PrintParams.m_pageInfo.m_nLastItem)
	{
		...
		if (!pItem->m_Rect.IsRectEmpty())
		{
			//if (pItem->m_Rect.t >= rectPage.b)
			if (pItem->m_Rect.b >= rectPage.b)
			{
				if (nTotal == 0)
				{
					ASSERT (FALSE);
			...
			else if (pItem->m_Rect.b >= rectPage.t)
			{
...
int CBCGPGridCtrl::OnAddGridPageRowForPrint (CBCGPGridPage& page, CRect rectPage)
{
	...
	int nHorzTotal = m_Columns.GetTotalWidth() * m_PrintParams.m_pageInfo.m_szOne.w;
...
void CBCGPGridCtrl::OnPrintHeader(CDC* pDC, CPrintInfo* /*pInfo*/)
{
	...
	int nXLeft = m_PrintParams.m_rectList.l - nHorzScrollOffset;
	int nPos = m_Columns.Begin();
	while (nPos != m_Columns.End())
	{
		int iColumn = m_Columns.Next (nPos);
		...
		if (bIsTreeColumn)
		{
			nWidth += GetHierarchyOffset() * szOne.w +
				GetExtraHierarchyOffset() * szOne.w;
		}
		...
		if (nWidth > 0) // if column visible
		{
			CRect rectItem = rectHeader;
			rectItem.l = nXLeft;
			rectItem.r = rectItem.l + nWidth;

			CRect rectClipItem = rectItem;
			if (rectItem.l < rectHeader.l)
			{
				rectClipItem.l = max (rectHeader.l, rectClipItem.l);
			}
			if (rectItem.r > rectHeader.r)
			{
				rectClipItem.r = min (rectHeader.r, rectClipItem.r);
			}
			...
			if (rectClipItem.Width() > 0)
			{
				...
				pDC->ExtTextOut (rectText.l, rectText.t, ETO_CLIPPED, &rectClipItem, strColumn, NULL);
				...
				if (nColState != 0)
				{
					...
					rectArrow.l = max (rectArrow.r - rectArrow.Height(), rectArrow.l);
				...
				// Draw border:
				//-------------
				if (rectClipItem.l <= rectItem.l && rectClipItem.r >= rectItem.l)
				{
					pDC->MoveTo (rectItem.TopLeft());
					pDC->LineTo (rectItem.l, rectItem.b);
				}
			}
		}
	...
	rectItems.l = max (m_PrintParams.m_rectList.l - nHorzScrollOffset, rectHeader.l);
	rectItems.r = min (nXLeft, rectHeader.r);

	if (rectHeader.l <= nXLeft && rectHeader.r >= nXLeft)
	{
		pDC->MoveTo (rectItems.r, rectItems.t);
		pDC->LineTo (rectItems.BottomRight());
	}

	pDC->MoveTo (rectItems.TopLeft());
	pDC->LineTo (rectItems.r, rectItems.t);
	pDC->MoveTo (rectItems.l, rectItems.b);
	pDC->LineTo (rectItems.BottomRight());
...
void CBCGPGridCtrl::OnPrintRowHeaderItem(CDC* pDC, CPrintInfo* pInfo, CBCGPGridRow* pItem)
{
...

	{
		...
		if (rectClipItem.IntersectRect (&rectClipItem, &rectHeader))
		{
			...
			pDC->LineTo (rectClipItem.l, rectClipItem.b);
			pDC->MoveTo (rectClipItem.TopLeft());
			pDC->LineTo (rectClipItem.r, rectClipItem.t);
			pDC->MoveTo (rectClipItem.l, rectClipItem.b);
			pDC->LineTo (rectClipItem.BottomRight());
			pDC->MoveTo (rectClipItem.r, rectClipItem.t);
			pDC->LineTo (rectClipItem.BottomRight());
...
void CBCGPGridCtrl::OnPrintSelectAllArea(CDC* pDC, CPrintInfo*)
{
	...
	pDC->LineTo (rectArea.l, rectArea.b);
	pDC->MoveTo (rectArea.TopLeft());
	pDC->LineTo (rectArea.r, rectArea.t);
	pDC->MoveTo (rectArea.l, rectArea.b);
	pDC->LineTo (rectArea.BottomRight());
	pDC->MoveTo (rectArea.r, rectArea.t);
	pDC->LineTo (rectArea.BottomRight());
...
void CBCGPGridCtrl::OnPrintLineNumber (CDC* pDC, CPrintInfo*, CBCGPGridRow* pRow, CRect rect)
{
	...
	const int CALCULATED_TEXT_MARGIN = TEXT_MARGIN * m_PrintParams.m_pageInfo.m_szOne.w;
	...
	if (rectClipLabel.IntersectRect (&rectClipLabel, &rectClip))
	{
		...
		int nDescent = tm.tmDescent;
		int nVCenterOffset = (rectLabel.Height() - pDC->GetTextExtent (strLabel).h + nDescent) / 2;

		pDC->SetTextAlign (TA_LEFT | TA_TOP);
		pDC->ExtTextOut (rectLabel.l, rectLabel.t + nVCenterOffset, ETO_CLIPPED, &rectClipLabel, strLabel, NULL);
	}
}
//****
void CBCGPGridCtrl::OnPrintList(CDC* pDC, CPrintInfo* pInfo)
{
	...
	while (nPos != m_Columns.End())
	{
		...
		if (bDrawVLines && 
			nXLeft >= m_PrintParams.m_rectList.l && nXLeft <= m_PrintParams.m_rectList.r)
		{
			pDC->MoveTo (nXLeft - 1, m_PrintParams.m_rectList.t - 1);
			pDC->LineTo (nXLeft - 1, nItemsBottom);
		}
	}

	CRect rectClipItems = rectItems;
	rectClipItems.l = m_PrintParams.m_rectList.l - nHorzScrollOffset;
	rectClipItems.r = nXLeft;
	...
	if (m_bVirtualMode)
	{
		for (int i = 0; i < pPageInfo->m_nTotalInPage; i++)
		{
			...
			if (pItem != NULL)
			{
				...
				int y = m_PrintParams.m_rectList.t - 1 + i * m_PrintParams.m_nRowHeight;
...
int CBCGPGridCtrl::OnPrintItem (CDC* pDC, CPrintInfo* pInfo, CBCGPGridRow* pItem) const
{
	...
	if (!pItem->m_Rect.IsRectEmpty())
	{
		if (pItem->m_Rect.t >= m_PrintParams.m_rectList.b)
		{
			return FALSE;
		}

		if (pItem->m_Rect.b >= m_PrintParams.m_rectList.t)
		{
			CBCGPGridPageInfo* pPageInfo = (CBCGPGridPageInfo*) pInfo->m_lpUserData;
			ASSERT (pPageInfo != NULL);
			...
			// --------------------------
			CRect rectLeft = pItem->m_Rect;
			rectLeft.t += pPageInfo->m_szOne.h;
			rectLeft.r = rectLeft.l + GetExtraHierarchyOffset() * pPageInfo->m_szOne.w + dx;
			rectLeft.b += pPageInfo->m_szOne.h;

			CRect rectClipLeft = rectLeft;

			int nCol0Right = pItem->m_Rect.r;
			if (m_Columns.GetColumnCount (TRUE) > 0)
			{
				int nCol0Idx = m_Columns.OrderToIndex (0);
				if (nCol0Idx != -1)
				{
					...
					if (nCol0Width > 0)
					{
						nCol0Right = pItem->m_Rect.l + nCol0Width * pPageInfo->m_szOne.w;
						rectClipLeft.r = min (rectClipLeft.r, nCol0Right);
					}
				}
			}
			...
			{
				CRect rectFill = rectName;
				rectFill.t += pPageInfo->m_szOne.h;
				rectFill.DeflateRect (dx, 0, 0, 0);
				...
				if (rectClipFill.Width() > 0)
				{
					...
					if (pItem->IsGroup())
					{
						...
						rectUnderline.t = rectUnderline.b;
						rectUnderline.InflateRect (0, pPageInfo->m_szOne.h);
			...
			{
				CRect rectExpand = rectName;
				rectName.l += m_PrintParams.m_nRowHeight + dx;
				rectExpand.r = rectName.l;
				rectExpand.DeflateRect (dx, 0, 0, 0);
			...
			if (rectName.r > rectName.l)
			{
				...
				{
					hOldFont = (HFONT) ::SelectObject (pDC, m_hPrinterBoldFont);
				}
			...
			if (pItem->HasValueField())
			{
				...
				rectItems.l += GetExtraHierarchyOffset() * pPageInfo->m_szOne.w  + dx;
				rectItems.l = min (rectItems.l, nCol0Right);
				...
				if (rectClipItems.Width() > 0 &&
					rectItems.b > rectClipItems.t && rectItems.b <= rectClipItems.b)
				{
					pDC->MoveTo (rectClipItems.l, rectItems.b);
					pDC->LineTo (rectClipItems.r, rectItems.b);
				}
			}
...
CBCGPGridRow* CBCGPGridCtrl::CreateVirtualRow (int nRowIndex)
{
	...
	if (m_bIsPrinting)
	{
		int y = m_PrintParams.m_rectList.t - 1 + nRowIndex * m_PrintParams.m_nRowHeight -
			m_PrintParams.m_nVertScrollOffset;
	...
	{
		int y = m_rectList.t - 1 + nRowIndex * m_nRowHeight - m_nVertScrollOffset;
...
CRect CBCGPGridCtrl::GetVirtualRowRect (int nRow) const
{
    ...
    int nRowTop = m_rectList.t - m_nVertScrollOffset - 1 + nRow * m_nRowHeight;
	int nRowLeft = m_rectList.l - nHorzScrollOffset;
											   ...
											   CBCGPGridRow::ClickArea* pnArea) const
{
	...
	// Calc row index:
	//----------------
	int nYTop = (pt.y - m_rectList.t + m_nVertScrollOffset + 1);
...
void CBCGPGridCtrl::MarkMergedItemChanged (const CRect& rectNew, CBCGPGridItem* pItem)
{
	...
	if (pItem->m_Rect.t >= m_rectList.b ||
		pItem->m_Rect.b < m_rectList.t)
	{
...
{	// TODO to speed up add parameter (const CBCGPGridItem* pItemVisible)
	...
	if (pRange->m_nTop < idVisible.m_nRow)
	{
		...
		rectResult.t -= nRowsBefore * rectVisible.Height();
	}
	if (pRange->m_nBottom > idVisible.m_nRow)
	{
		int nRowsAfter = pRange->m_nBottom - idVisible.m_nRow;
		rectResult.b += nRowsAfter * rectVisible.Height();
	}
...
BOOL CBCGPHeaderItemDragWnd::Create (CBCGPGridCtrl* pGrid, int nItem)
{
	...
	if (m_pWndGrid->m_bDragGroupItem || m_pWndGrid->m_bDragFromChooser)
	{
		...
		else
		{
			...
			rect.r = rect.l + max (sizeText.w + 20, nWidth);
		}
		rect.b = rect.t + sizeText.h + 10;
...
//****
void CBCGPHeaderItemDragWnd::OnPaint(CDC* pDC) 
{
...
BOOL CBCGPHeaderItemDropWnd::OnEraseBkgnd(CDC* pDC) 
{
	...
	pDC->FillSolidRect (rectClient, _RGB (255, 0, 0));
	return TRUE;
}
//****
...
BOOL CBCGPGridColumnChooser::Create(CBCGPGridCtrl* pOwnerGrid,
									const IRECT& rect, CWnd* pParentWnd)
{
...
int CBCGPGridColumnChooser::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	else if (ptPos.x + size.w > rectScreen.r)
	{
		ptPos.x = rectScreen.r - size.w;
	}
	...
	else if (ptPos.y + size.h > rectScreen.b)
	{
		ptPos.y = rectScreen.b - size.h;
	}
...
int CBCGPGridColumnListBox::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	int nMaxColumnWidth = dc.GetTextExtent (strParentCaption).w + 20;
...
//****
void CBCGPGridColumnChooser::OnPaint(CDC* pDC) 
{
	CPaintDC dc(this); // device context for painting
	...
	rect.t = max (rect.t, rect.CenterPoint().y - globalData.GetTextHeight() / 2);
