											...
											int nOffset, HDWP& hdwp)
{
	...
	if (pSlider == m_pDefaultSlider)
	{
		...
		case CBRS_ALIGN_LEFT:
			if (bIsRTL)
			{
				rectContainer.l += nOffset;
			}
			else
			{
				rectContainer.r += nOffset;
			}
			if (rectContainer.Width() < sizeMinContainer.w)
			{
				rectContainer.r = rectContainer.l + sizeMinContainer.w;
			}
			break;
		case CBRS_ALIGN_RIGHT:
			...
			if (rectContainer.Width() < sizeMinContainer.w)
			{
				rectContainer.l = rectContainer.r - sizeMinContainer.w;
			}
			break;
		case CBRS_ALIGN_TOP:
			rectContainer.b += nOffset;
			if (rectContainer.Height() < sizeMinContainer.h)
			{
				rectContainer.b = rectContainer.t + sizeMinContainer.h;
			}
			break;
		case CBRS_ALIGN_BOTTOM:
			rectContainer.t += nOffset;
			if (rectContainer.Height() < sizeMinContainer.h)
			{
				rectContainer.t = rectContainer.b - sizeMinContainer.h;
			}
			break;
		}
												...
												int& nMaxOffset, int& nStep)
{
	...
	if (pSlider->IsDefault())
	{
		...
		{
			nMinOffset = rectContainer.l - rectSlider.l + 1;
			nMaxOffset = rectMainClientArea.r - rectSlider.r - 1;
		}
		else if (dwAlignment & CBRS_ALIGN_TOP)
		{
			nMinOffset = rectContainer.t - rectSlider.t + 1;
			nMaxOffset = rectMainClientArea.b - rectSlider.b - 1;
		}
		else if (dwAlignment & CBRS_ALIGN_RIGHT)
		{
			nMinOffset = rectMainClientArea.l - rectSlider.l + 1;
			nMaxOffset = rectContainer.r - rectSlider.r - 1;
		}
		else if (dwAlignment & CBRS_ALIGN_BOTTOM)
		{
			nMinOffset = rectMainClientArea.t - rectSlider.t + 1;
			nMaxOffset = rectContainer.b - rectSlider.b - 1;
		}
	...
	else
	{
		...
		{
			nMinOffset = rectContainer.t - rectSlider.t + sizeMinLeft.h + 1;
			nMaxOffset = rectContainer.b - rectSlider.b - sizeMinRight.h - 1;
		}
		else
		{
			nMinOffset = rectContainer.l - rectSlider.l + sizeMinLeft.w;
			nMaxOffset = rectContainer.r - rectSlider.r - sizeMinRight.w - 1;
		}			
...
{
	if (rectInserted.Width() < sizeMinInserted.w)
	{
		rectInserted.r = rectInserted.l + sizeMinInserted.w;
	}

	if (rectInserted.Height() < sizeMinInserted.h)
	{
		rectInserted.b = rectInserted.t + sizeMinInserted.h;
	}
	...
	if (dwAlignment & CBRS_ORIENT_HORZ)
	{
		...
		rectSlider.l = rectInserted.l = rectOriginal.l;
		rectSlider.r = rectInserted.r = rectOriginal.r;
	...
	else
	{
		...
		rectSlider.t = rectInserted.t = rectOriginal.t;
		rectSlider.b = rectInserted.b = rectOriginal.b;
	...
	{
	case CBRS_ALIGN_TOP:
		rectInserted.t = rectOriginal.t;
		rectInserted.b = rectInserted.t + nNewSize;
		rectOriginal.t = rectInserted.b + CBCGPSlider::GetDefaultWidth();
		rectSlider.t = rectInserted.b; 
		rectSlider.b = rectOriginal.t; 
		break;
	case CBRS_ALIGN_BOTTOM:
		rectInserted.t = rectOriginal.b - nNewSize;
		rectInserted.b = rectOriginal.b;
		rectOriginal.b = rectInserted.t - CBCGPSlider::GetDefaultWidth();
		rectSlider.t = rectOriginal.b;
		rectSlider.b = rectInserted.t;
		break;
	case CBRS_ALIGN_LEFT:
		rectInserted.l = rectOriginal.l;
		rectInserted.r = rectInserted.l + nNewSize;
		rectOriginal.l = rectInserted.r + CBCGPSlider::GetDefaultWidth();
		rectSlider.l = rectInserted.r;
		rectSlider.r = rectOriginal.l;
		break;
	case CBRS_ALIGN_RIGHT:
		rectInserted.r = rectOriginal.r;
		rectInserted.l = rectInserted.r - nNewSize;
		rectOriginal.r = rectInserted.l - CBCGPSlider::GetDefaultWidth();
		rectSlider.l = rectOriginal.r;
		rectSlider.r = rectInserted.l;
		break;
	}
}
//-----------------------------------------------------------------------------------//
													  ...
													  DWORD dwAlignment)
{
	...
	if (rectBarToInsert.Width() < sizeMinToInsert.w)
	{
		rectBarToInsert.r = rectBarToInsert.l + sizeMinToInsert.w;
	}

	if (rectBarToInsert.Height() < sizeMinToInsert.h)
	{
		rectBarToInsert.b = rectBarToInsert.t + sizeMinToInsert.h;
	}
	...
	{
		// align the rectangle of the bar to insert by the width of the sell
		rectSlider.l = rectBarToInsert.l = rectBarOriginal.l;
		rectSlider.r = rectBarToInsert.r = rectBarOriginal.r;
		
		if (rectBarToInsert.Height() > rectBarOriginal.Height() / 2) //- sizeMinOriginal.h * 2- CBCGPSlider::GetDefaultWidth())
		{
			nNewSize = rectBarOriginal.Height() / 2; //  - sizeMinOriginal.h * 4 - CBCGPSlider::GetDefaultWidth();
		}
		else 
		{
	...
	{
		// align the rectangle of the bar to insert by the height of the sell
		rectSlider.t = rectBarToInsert.t = rectBarOriginal.t;
		rectSlider.b = rectBarToInsert.b = rectBarOriginal.b;

		if (rectBarToInsert.Width() > rectBarOriginal.Width() / 2) //- sizeMinOriginal.w * 2 - CBCGPSlider::GetDefaultWidth())
		{
			nNewSize = rectBarOriginal.Width() / 2; //- sizeMinOriginal.w * 4;
		}
		else 
		{
	...
	{
	case CBRS_ALIGN_TOP:
		rectBarToInsert.t = rectBarOriginal.t;
		rectBarToInsert.b = rectBarToInsert.t + nNewSize;
		rectBarOriginal.t = rectBarToInsert.b + CBCGPSlider::GetDefaultWidth();
		rectSlider.t = rectBarToInsert.b; 
		rectSlider.b = rectBarOriginal.t; 
		pLeftBar = pBarToInsert;
		pRightBar = pBarOriginal;
		break;
	case CBRS_ALIGN_BOTTOM:
		rectBarToInsert.t = rectBarOriginal.b - nNewSize;
		rectBarToInsert.b = rectBarOriginal.b;
		rectBarOriginal.b = rectBarToInsert.t - CBCGPSlider::GetDefaultWidth();
		rectSlider.t = rectBarOriginal.b;
		rectSlider.b = rectBarToInsert.t;
		dwSliderStyle = CBCGPSlider::SS_HORZ;
	...
	case CBRS_ALIGN_LEFT:
		if (bIsRTL)
		{
			rectBarToInsert.r = rectBarOriginal.r;
			rectBarToInsert.l = rectBarToInsert.r - nNewSize;
			rectBarOriginal.r = rectBarToInsert.l - CBCGPSlider::GetDefaultWidth();
			rectSlider.l = rectBarOriginal.r;
			rectSlider.r = rectBarToInsert.l;
			pLeftBar = pBarOriginal;
			pRightBar = pBarToInsert;
			bRightNode = TRUE;
		}
		...
		{
			rectBarToInsert.l = rectBarOriginal.l;
			rectBarToInsert.r = rectBarToInsert.l + nNewSize;
			rectBarOriginal.l = rectBarToInsert.r + CBCGPSlider::GetDefaultWidth();
			rectSlider.l = rectBarToInsert.r; 
			rectSlider.r = rectBarOriginal.l; 
			pLeftBar = pBarToInsert;
			pRightBar = pBarOriginal;
		}	
	...
	pBarOriginal->SetWindowPos (NULL, rectBarOriginal.l, rectBarOriginal.t, 
									  rectBarOriginal.Width(), rectBarOriginal.Height(), 
									  SWP_NOZORDER | SWP_NOACTIVATE);

	pBarToInsert->SetWindowPos (NULL, rectBarToInsert.l, rectBarToInsert.t, 
																	   ...
																	   BOOL& bCaption)
{
	...
	for (POSITION pos = m_lstControlBars.GetHeadPosition(); pos != NULL;)
	{
		...
		rectWnd.t += nCaptionHeight;
		rectWnd.b -= rectTabAreaBottom.Height();
									...
									CBCGPDockingControlBar** ppTargetControlBar)
{
	...
	*ppTargetControlBar = NULL;
	...
	point.Offset (rectBorderSize.l, 
				  rectBorderSize.t + pMiniFrameWnd->GetCaptionHeight());
