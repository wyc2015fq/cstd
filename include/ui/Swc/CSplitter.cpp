...
void  CSplitter::RecalLayout()
{
  ...
  if (m_rcSpplitter.left < m_rectCli.left) {
    m_rcSpplitter.left = m_rectCli.left + 1;
  }

  if (m_rcSpplitter.right > m_rectCli.right) {
    m_rcSpplitter.right = m_rectCli.right - 1;
  }

  if (m_rcSpplitter.bottom > m_rectCli.bottom) {
    m_rcSpplitter.bottom = m_rectCli.bottom - 1;
  }

  if (m_rcSpplitter.top < m_rectCli.top) {
    m_rcSpplitter.top = m_rectCli.top + 1;
  }
