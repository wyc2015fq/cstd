...
public:
  ...
  LPTSTR  RegisterDefaultClass(LPCTSTR lpClassName = NULL, DWORD dwStyle = 0, LPCTSTR lpszIcon = 0, LPCTSTR lpszCursor = 0) {
    ...
    wcex.hIcon      = LoadIcon(wcex.hInstance, (char*)IDI_APPLICATION) ;
    wcex.hCursor    = LoadCursor(wcex.hInstance, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_BTNFACE + 1);
    wcex.lpszMenuName = (LPCSTR)NULL;
    wcex.lpszClassName  = szTemp;
    wcex.hIconSm    = LoadIcon(NULL, (char*)IDI_APPLICATION);
  ...
  __forceinline HWND SetCapture() {
    ASSERT(::IsWindow(m_hWnd))  return ::SetCapture(m_hWnd);
  }
  __forceinline HWND GetFocus() {
    return ::GetFocus();
  }
  __forceinline HWND SetFocus() {
    ASSERT(::IsWindow(m_hWnd))  return ::SetFocus(m_hWnd);
  }

  __forceinline HWND GetDesktopWindow() {
  ...
  __forceinline void GetDlgItem(int nID, HWND* pm_hWnd) {
    ASSERT(::IsWindow(m_hWnd))(pm_hWnd != NULL);
    *pm_hWnd = ::GetDlgItem(m_hWnd, nID);
  }
