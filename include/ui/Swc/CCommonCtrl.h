...
public:
  ...
  virtual BOOL OnChar(LPNMCHAR pChar, BOOL& bNotify) {
    return FALSE;
  }
  virtual BOOL OnClick(LPNMHDR pNMHDR, BOOL& bNotify) {
    return FALSE;
  }
  virtual BOOL OnCustomDraw(LPNMCUSTOMDRAW pCustomDraw, BOOL& bNotify)  {
    return CDRF_DODEFAULT;
  }
  virtual BOOL OnDblclk(LPNMHDR pNMHDR, BOOL& bNotify) {
    return FALSE;
  }
  virtual BOOL OnHover(LPNMHDR pNMHDR, BOOL& bNotify) {
    return FALSE;
  }
  virtual BOOL OnKeydown(LPNMKEY pKey, BOOL& bNotify) {
    return FALSE;
  }
  virtual BOOL OnKillFocus(LPNMHDR pNMHDR, BOOL& bNotify) {
    return FALSE;
  }
  virtual BOOL OnNCHitTest(LPNMMOUSE pMouse, BOOL& bNotify) {
    return FALSE;
  }
  virtual BOOL OnOutOfMemory(LPNMHDR pNMHDR, BOOL& bNotify) {
    return FALSE;
  }
  virtual BOOL OnRClick(LPNMHDR pNMHDR, BOOL& bNotify) {
    return FALSE;
  }
  virtual BOOL OnRDblclk(LPNMHDR pNMHDR, BOOL& bNotify) {
    return FALSE;
  }
  virtual BOOL OnReleasedCapture(LPNMHDR pNMHDR, BOOL& bNotify) {
    return FALSE;
  }
  virtual BOOL OnReturn(LPNMHDR pNMHDR, BOOL& bNotify) {
    return FALSE;
  }
  virtual BOOL OnSetCursor(LPNMMOUSE pMouse, BOOL& bNotify) {
    return FALSE;
  }
  virtual BOOL OnSetFocus(LPNMHDR pNMHDR, BOOL& bNotify)  {
    return FALSE;
  }
...
public:
  CImageCtrl(HIMAGELIST hImageList = NULL) {
...
public:
  ...
  BOOL Attach(HIMAGELIST hNewImageList) {
    if (m_hImageList != NULL || hNewImageList == NULL) {
      return FALSE;
    }
  ...
  BOOL Draw(int iImage, HDC hdcDst, int x, int y, UINT fStyle) {
    if (m_hImageList == NULL || hdcDst == NULL) {
      return FALSE;
    }
  ...
  BOOL Copy(int iDst, HIMAGELIST himlSrc, int iSrc, UINT uFlags) {
    if (m_hImageList == NULL || himlSrc == NULL) {
      return NULL;
    }
...
public:
  ...
  int InsertColumn(int nCol, LPCTSTR lpszText, int Flag, int nWidth, int nSubItem) {
    ...
    if (nWidth != -1) {
      lvNewColumn.mask |= LVCF_WIDTH;
    }

    if (nSubItem != -1) {
      lvNewColumn.mask |= LVCF_SUBITEM;
    }
...
public:
  ...
  BOOL IsPressed() {
    return (fsState & TBSTATE_PRESSED) != 0;
  }
  BOOL IsEnabled() {
  ...
  BOOL IsStyleDropDown() {
    return (fsStyle & TBSTYLE_DROPDOWN) != 0;
  }
  BOOL IsImage() {
    return (BOOL)(iImage >= 0) != 0;
  }
