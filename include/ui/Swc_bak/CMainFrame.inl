...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
  ...
  if (!cmedBottom.Create(&splitterMain, IRECT(0, 0, 0, 0) , WS_CHILD, 1000)) {
    return -1;
  }

  if (!cmedLeft.Create(&splitterLeft, IRECT(0, 0, 0, 0) , WS_CHILD, 1001)) {
    return -1;
  }

  if (!cmedRight.Create(&splitterCenter, IRECT(0, 0, 0, 0) , WS_CHILD, 1002)) {
    return -1;
  }
...
BOOL CMainFrame::Create(CWin* pParentWnd)
{
  ...
  return CWin::Create(lpclassname, NULL, WS_VISIBLE | WS_CHILD | WS_CLIPSIBLINGS | WS_CLIPCHILDREN, IRECT(0, 0, 0, 0), pParentWnd->GetSafeHwnd(), 0xffff);
}
...
{
  if (!m_Tabbed.Create(WS_VISIBLE | WS_CHILD, IRECT(0, 0, 0, 0), &splitterCenter, 0x333)) {
    return ;
  }
