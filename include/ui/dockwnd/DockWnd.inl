...
static void redrawDockingState(DOCKINFO* dwp)
{
  ...
  if (dwp->uDockedState < 0) {
    setFloatingWinPos(hwnd, dwp, SWP_SHOWWINDOW);
  }
...
static char checkDockingPos(DOCKINFO* dwp)
{
  ...
  if (DragRecPlacement.left < 0) {
    DragRecPlacement.left = 0;
  }

  DragRecPlacement.top = MousePos.y - (dwp->nFrameHeight >> 1);

  if (DragRecPlacement.top < 0) {
    DragRecPlacement.top = 0;
  }
...

  {
    ...
    if (DragRecPlacement.left >= prc1.left && DragRecPlacement.right <= prc1.right &&
      ...
      if (DragRecPlacement.bottom > prc2.bottom && (dwp->dwStyle & DWS_ALLOW_DOCKBOTTOM)) {
        uDockSide = DWS_DOCKED_BOTTOM;
      }

      // Check intersection at top
      if (DragRecPlacement.top < prc2.top && (dwp->dwStyle & DWS_ALLOW_DOCKTOP)) {
        uDockSide = DWS_DOCKED_TOP;
      }

      // Check intersection at left
      if (DragRecPlacement.left < prc2.left && (dwp->dwStyle & DWS_ALLOW_DOCKLEFT)) {
        uDockSide = DWS_DOCKED_LEFT;
      }

      // Check intersection at right
      if (DragRecPlacement.right > prc2.right && (dwp->dwStyle & DWS_ALLOW_DOCKRIGHT)) {
        uDockSide = DWS_DOCKED_RIGHT;
      }
    }
      ...
      if (wParam == HTBORDER) {
        ...
        if (!(dwp->dwStyle & DWS_NORESIZE)) {
          ...
          case DWS_DOCKED_LEFT: {
            ...
            if (rc.right >= lParam && rc.right - 3 <= lParam) {
              wParam = HTRIGHT;
            }
          ...
          case DWS_DOCKED_RIGHT: {
            ...
            if (rc.left <= lParam && rc.left + 3 >= lParam) {
              wParam = HTLEFT;
            }
          ...
          case DWS_DOCKED_TOP: {
            ...
            if (rc.bottom >= lParam && rc.bottom - 3 <= lParam) {
              wParam = HTBOTTOM;
            }
          ...
          case DWS_DOCKED_BOTTOM: {
            ...
            if (rc.top <= lParam && rc.top + 3 >= lParam) {
              wParam = HTTOP;
            }
    ...
    if (dwp) {
      ...
      if (!(dwp->dwStyle & DWS_DONTSAVEPOS)) {
        ...
        if (dwp->uDockedState > 0) {
          ...
          if (dwp->dwStyle & DWS_DRAWGRIPPERFLOATING) {
            border.left = 3;
          }
      ...
      if (msg != WM_CANCELMODE && MouseMoved) {
        ...
        if (dwp->uDockedState > 0) {
          ...
          // side of the container window)
            ...
            // the frame moved
            if (dwp->focusWindow) {
              InvalidateRect(dwp->focusWindow, 0, 1);
            }
...
ret0:
  ...
  case WM_SETTINGCHANGE: {
    if (dwp->uDockedState < 0 && !(dwp->dwStyle & DWS_HIDDEN)) {
...
def:
...

  {
    ...
    if ((result = (*dwp->DockMsg)(dwp, msg, wParam, lParam)) == -1) {
      goto def;
    }
...
DOCKINFO* WINAPI DockingAlloc(char initialDocking)
{
  ...
  if ((dwp = (DOCKINFO*)GlobalAlloc(GMEM_FIXED, sizeof(DOCKINFO)))) {
    ...
    if (initialDocking == (char)DWS_FLOATING) {
      initialDocking |= DWS_DOCKED_BOTTOM;
    }
...
LRESULT WINAPI DockingActivate(DOCKPARAMS* params)
{
  ...
  if (params->lParam == -1) {
    ...
    if (params->container == (HWND)params->lParam) {
      goto mine;
    }
    ...
    if (params->container) {
      EnumChildWindows(params->container, activateProc, (LPARAM)params);
    }
...
int init(HANDLE hinstDLL, DWORD fdwReason)
#endif
{
  switch (fdwReason) {
    // ==============================================================
