...
int test_xlib9()
{
  ...
  while ((event = xcb_wait_for_event(connection))) {
    ...
    case XCB_EXPOSE: {
      drawText(connection, screen, window, 10, 20, "fixed", "Press ESC key to exit.");
      drawText(connection, screen, window, 10, 40, "Dejavu Sans Mono", "Press ESC key to exit.");
      drawText(connection, screen, window, 10, 60, "文泉驿正黑", "按ESC键退出。");
      break;
    }
