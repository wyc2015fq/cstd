...
IDSTR(WM_KEYFIRST, "用于WinCE系统,本消息在使用GetMessage和PeekMessage函数时,用于过滤键盘消息")
IDSTR(WM_KEYDOWN, "当一个非系统按键被按下时(<ALT>键没有被按下),会发送本消息给拥有键盘焦点的窗口")
IDSTR(WM_KEYUP, "当一个非系统按键?皇头诺鹗?<ALT>键没有被按下),会发送本消息给拥有键盘焦点的窗口")
...
IDSTR(WM_RESERVED_0370, "MFC内部保留的未公开?FC自身也未使用的消息")
...
IDSTR(BM_SETCHECK, "设置单选按钮或复选框的?刺?)
    ...
    IDSTR(CB_INITSTORAGE, "只适用于Win95, 当将要向组合框中列表框中加入大量表项时, 本消息将预先分配一块内存, 以免大?刻砑颖硐疃啻畏峙淠诖? 从而加快程序运行速度")
    ...
    IDSTR(LB_SETITEMHEIGHT, "用于列表框中指定列表项显示高度")
    ...
    IDSTR(TTM_ENUMTOOLSA, "获取当前是哪个工具提示控件在显示文本")
    ...
    IDSTR(EM_DISPLAYBAND, "将Rich Edit控件的输出发送至设备, 通过反复使用EM_FORMATRANGE和EM_DISPLAYBAND消息, 打印该控件内容的应用程序可实现条带化操作(将输出分割?闲〔糠钟糜诖蛴?")
    ...
    IDSTR(EM_FORMATRANGE, "对于特定设备而言, 要格式化Rich Edit控件中部分内容, 可使用本消息, 在输出设备文本格式化完成后, 可使用EM_DISPLAYBAND消息将输 ? 龇⑺椭辽璞 ? 如 : 打印)")
...
IDSTR(EM_SETOLECALLBACK, "Rich Edit控件支持由OLE所定义的客户端的支持,客户端使用EM_SETOLECALLBACK消息注册一个IRichEditOleCallback接口,控件将使用它获取所需接口和存储")
...
IDSTR(LVM_GETCOUNTPERPAGE, "在Listview控件中获取当前可视项目数量,宏:ListView_GetCountPerPage")
IDSTR(LVM_GETORIGIN, "在Listview控件中获取当前的原点,宏:ListView_GetOrigin")
IDSTR(LVM_UPDATE, "用于刷新Listview控件,若该控件具有LVS_AUTOARRANGE风格,则?嵋鹋帕胁僮?宏:ListView_Update")
...
IDSTR(LVN_GETDISPINFOA, "由虚列表控件发送,请求父窗提供显示项目或对某个项目排序所需的信息")
...
IDSTR(STN_DBLCLK, "通知父窗(通过WM_COMMAND获知),用户点击了静态控件,须具SS_NOTIFY风格")
IDSTR(STN_DISABLE, "通知父窗(通过WM_COMMAND获知),静态控件被允许使用,须具SS_NOTIFY风格")
IDSTR(STN_ENABLE, "通知父窗(通过WM_COMMAND获知),静态控件被禁用,须具SS_NOTIFY风格")
//Toolbar Control工具栏控件消息
IDSTR(TB_ADDBITMAP, "将一个或多个按钮图像添加到工具栏的按钮图像列表中,相关结构:TBADDBITMAP")
IDSTR(TB_ADDBUTTONS, "向一个工具栏中添加一个或多个按钮,相关结构:TBBUTTON")
IDSTR(TB_ADDSTRING, "添加一个新字符串到工具栏的内部字符串列表中")
IDSTR(TB_AUTOSIZE, "调整工具栏的尺寸。当设置按钮或位图尺寸、添加字符串时,发送本消息")
IDSTR(TB_BUTTONCOUNT, "获取工具栏当前按钮的个数")
IDSTR(TB_BUTTONSTRUCTSIZE, "指定工具栏TBBUTTON结构的大小")
IDSTR(TB_CHANGEBITMAP, "改变工具栏的按钮图像(图像列表中的索引)")
IDSTR(TB_CHECKBUTTON, "核选或清除工具栏的指定按钮。当一个按钮被核选时,它看起来就像被按下一样")
IDSTR(TB_COMMANDTOINDEX, "获取与指定命令标识符相关联的工具栏按钮的索引(从零开始)")
IDSTR(TB_CUSTOMIZE, "显示\"自定义工具栏\"对话框,该对话框允许通过增删按钮来定制工具条")
IDSTR(TB_DELETEBUTTON, "删除工具栏中的一个按钮")
IDSTR(TB_ENABLEBUTTON, "使工具栏指定按钮有效或无效,注:当按钮有效时就可被按下或核选")
IDSTR(TB_GETANCHORHIGHLIGHT, "获取工具栏的固定的加亮设置")
IDSTR(TB_GETBITMAP, "获取与工具栏按钮相关联的位图的索引")
IDSTR(TB_GETBITMAPFLAGS, "从工具栏获取位图标志(DWORD值)")
IDSTR(TB_GETBUTTON, "获取工具栏指定按钮的信息")
IDSTR(TB_GETBUTTONINFO, "获取工具栏中按钮的扩展信息,相关结构:TBBUTTONINFO")
IDSTR(TB_GETBUTTONSIZE, "获取工具栏按钮的尺寸")
IDSTR(TB_GETBUTTONTEXT, "获取工具栏按钮的文本")
IDSTR(TB_GETCOLORSCHEME, "获取工具栏的色彩方案信息")
IDSTR(TB_GETDISABLEDIMAGELIST, "获取工具栏中被设置为无效的图像列表")
IDSTR(TB_GETEXTENDEDSTYLE, "获取工具栏的扩展风格")
IDSTR(TB_GETHOTIMAGELIST, "获取工具栏的热点按钮的图像列表,当鼠标悬浮于热点按钮上时,按钮会加亮显示")
IDSTR(TB_GETHOTITEM, "获取工具栏的热点项的索引(从零开始)")
IDSTR(TB_GETIMAGELIST, "获取工具栏中用于显示按钮常规状态图像的列表")
IDSTR(TB_GETINSERTMARK, "获取工具栏的插入标记,相关结构:TBINSERTMARK")
IDSTR(TB_GETINSERTMARKCOLOR, "获取工具栏中包含当前插入标记的颜色(COLORREF值)")
IDSTR(TB_GETITEMRECT, "获取工具栏中的一个按钮的边界矩形,不接收状态被设为TBSTATE_HIDDEN的按钮的边界矩形")
IDSTR(TB_GETMAXSIZE, "获取工具栏中所有可见按钮与分隔条的总尺寸")
IDSTR(TB_GETOBJECT, "获取工具栏的IDropTarget接口,IDropTarget用于实现以拖放方式获取资源等")
IDSTR(TB_GETPADDING, "获取工具栏的填充距(像素)(padding:补白、填充距)")
IDSTR(TB_GETRECT, "获取工具栏指定按钮的边界矩形信息")
IDSTR(TB_GETROWS, "获取工具栏中当前显示的按钮行数,注:除非工具栏是用TBSTYLE_WRAPABLE风格创建的,否则行数总为1")
IDSTR(TB_GETSTATE, "获取工具栏定按钮的状态信息,比如:是否是有效、被按下或被核选")
IDSTR(TB_GETSTYLE, "获取工具栏当前风格")
IDSTR(TB_GETTEXTROWS, "获取用于显示在工具栏按钮上的文本行的最大数字")
IDSTR(TB_GETTOOLTIPS, "获取与工具栏相关联的工具提示控件(若有)的句柄,通常工具栏会自己创建并维护其工具提示控件,一般不必使用本消息")
IDSTR(TB_GETUNICODEFORMAT, "获取工具栏的UNICODE格式标志")
IDSTR(TB_HIDEBUTTON, "隐藏或显示工具栏指定按钮")
IDSTR(TB_HITTEST, "点击测试,获取工具栏指定点的位置的整数值,若该值为零或正值,则表示该点所在按钮的索引(非分隔条),为负则该点不在某按钮上")
IDSTR(TB_INDETERMINATE, "设置或清除工具栏指定按钮的不确定状态,不确定的按钮被显示为灰色")
IDSTR(TB_INSERTBUTTON, "在工具栏中插入一个按钮")
IDSTR(TB_INSERTMARKHITTEST, "为工具栏上的某点(该点坐标是相对于工具栏客户区的)获取插入标记的信息,相关结构:TBINSERTMARK")
IDSTR(TB_ISBUTTONCHECKED, "确定工具栏指定按钮是否被核选")
IDSTR(TB_ISBUTTONENABLED, "确定工具栏指定按钮是否有效")
IDSTR(TB_ISBUTTONHIDDEN, "确定工具栏指定按钮是否被隐藏")
IDSTR(TB_ISBUTTONHIGHLIGHTED, "检查工具栏指定按钮是否处于加亮状态")
IDSTR(TB_ISBUTTONINDETERMINATE, "确定工具栏指定按钮是否处于不确定状态,不确定按钮被显示为灰色")
IDSTR(TB_ISBUTTONPRESSED, "确定工具栏指定按钮是否被按下")
IDSTR(TB_LOADIMAGES, "加载图像到工具栏图像列表中")
IDSTR(TB_MAPACCELERATOR, "为一个工具栏按钮映射快捷键字符,该?址氚磁ノ谋局屑酉禄叩淖址且谎?)
IDSTR(TB_MARKBUTTON, "为工具栏指定按钮设置加亮状态")
IDSTR(TB_MOVEBUTTON, "将一个工具栏按钮从一个索引移动到另一个索引")
IDSTR(TB_PRESSBUTTON, "按下或释放工具栏的指定按钮")
IDSTR(TB_REPLACEBITMAP, "以新位图替换工具栏中现有的位图,相关结构:TBREPLACEBITMAP")
IDSTR(TB_SAVERESTORE, "保存或恢复工具栏的状态,相关结构:TBSAVEPARAMS")
IDSTR(TB_SETANCHORHIGHLIGHT, "对工具栏进行加亮设置")
IDSTR(TB_SETBITMAPSIZE, "设置添加到工具栏的位图尺寸")
IDSTR(TB_SETBUTTONINFO, "设置工具栏指定按钮的信息")
IDSTR(TB_SETBUTTONSIZE, "设置工具栏的按钮尺寸(尺寸必须与所载入的位图尺寸一样)")
IDSTR(TB_SETBUTTONWIDTH, "设置工具栏按钮宽度的最大值和最小值")
IDSTR(TB_SETCMDID, "设置工具栏指定按钮的命令标识符(按钮被按下时,标识符将被发送给属主窗??")
IDSTR(TB_SETCOLORSCHEME, "设置工具栏的色彩方案信息")
IDSTR(TB_SETDISABLEDIMAGELIST, "设置工具栏中用来显示无效按钮的图像列表")
IDSTR(TB_SETDRAWTEXTFLAGS, "设置工具栏的文本绘制标志")
IDSTR(TB_SETEXTENDEDSTYLE, "设置工具栏的扩展风格")
IDSTR(TB_SETHOTIMAGELIST, "设置工具栏中用于显示热点按钮的图像列表")
IDSTR(TB_SETHOTITEM, "设置工具栏热点项索引(若无TBSTYLE_FLAT风格,则本消息将会被忽略)")
IDSTR(TB_SETIMAGELIST, "设置工具栏的默认图像列表")
IDSTR(TB_SETINDENT, "设置工具栏中首个按钮的缩进")
IDSTR(TB_SETINSERTMARK, "设置工具栏的当前插入标记")
IDSTR(TB_SETINSERTMARKCOLOR, "设置工具栏插入标记的颜色")
IDSTR(TB_SETMAXTEXTROWS, "设置显示在工具栏按钮中的文本的最大行数")
IDSTR(TB_SETPADDING, "设置工具栏的填充距(像素)(padding:补白、填充距)")
IDSTR(TB_SETPARENT, "为工具栏指定一个父窗(用于发送通知消息)")
IDSTR(TB_SETROWS, "设置工具栏按钮的行数")
IDSTR(TB_SETSTATE, "设置工具条栏指定按钮的状态")
IDSTR(TB_SETSTYLE, "设置工具栏的风格")
IDSTR(TB_SETTOOLTIPS, "将一个工具提示控件与工具栏进行关联")
IDSTR(TB_SETUNICODEFORMAT, "设置工具栏的UNICODE格式标志")
//Toolbar Control工具栏通知消息
IDSTR(TBN_BEGINADJUST, "通知工具栏的父窗:用户已开始自定义一个工具栏,相关结构:NMHDR")
IDSTR(TBN_BEGINDRAG, "通知工具栏的父窗:用户已开始拖动工具栏的某个按钮,相关结构:NMTOOLBAR")
IDSTR(TBN_CUSTHELP, "通知工具栏的父窗:用户已经选择了自定义工具栏对话框上的帮助按钮,相关结构:NMHDR")
IDSTR(TBN_ENDADJUST, "通知工具栏的父窗:用户已停止自定义一个工具栏,相关结构:NMHDR")
IDSTR(TBN_ENDDRAG, "通知工具栏的父窗:用户已停止拖动工具栏的某个按钮,相关结构:NMTOOLBAR")
IDSTR(TBN_GETBUTTONINFO, "获取工具栏的自定义信息(通知消息),相关结构:NMTOOLBAR")
IDSTR(TBN_QUERYDELETE, "通知工具栏的父窗:用户自定义工具栏时一个按钮是否可被删除")
IDSTR(TBN_QUERYINSERT, "通知工具栏的父窗:用户自定义工具栏时一个按钮是否可被插入到指定按钮的左侧")
IDSTR(TBN_RESET, "通知工具栏的父窗:用户在自定义工具栏对话框中已经对工具栏进行重置了")
IDSTR(TBN_TOOLBARCHANGE, "通知工具栏的父窗:用户已经定制完一个工具栏")
//IP address control(IP地址控件)消息
IDSTR(IPM_CLEARADDRESS, "清空IP地址控件中的内容")
IDSTR(IPM_GETADDRESS, "从IP地址控件的4个域中获取地址值,相关宏:FIRST_IPADDRESS/SECOND_IPADDRESS/THIRD_IPADDRESS/FOURTH_IPADDRESS")
IDSTR(IPM_ISBLANK, "确定IP地址控件的全部域是否为空")
IDSTR(IPM_SETADDRESS, "为IP地址控件的4个域设置地址,宏:MAKEIPADDRESS")
IDSTR(IPM_SETFOCUS, "为IP地址控件指定域设置键盘焦点,同时该域中的文本会被选中")
IDSTR(IPM_SETRANGE, "为IP地址控件指定域设置有效范围,宏:MAKEIPRANGE")
//IP address control(IP地址控件)通知消息
IDSTR(IPN_FIELDCHANGED, "用户改变了IP地址控件某域或从一个域移动其它域时发送本通知消息,相关结构NMIPADDRESS")
//progress bar进度条消息
IDSTR(PBM_DELTAPOS, "以指定的增量来增加进度条控件的当前位置")
IDSTR(PBM_GETPOS, "检取进度条的当前位置")
IDSTR(PBM_GETRANGE, "获取当前进度条控件的上下限范围")
IDSTR(PBM_SETBARCOLOR, "设置进度条控件上滑块的颜色(COLORREF值)")
IDSTR(PBM_SETBKCOLOR, "设置进度条控件的背景色")
IDSTR(PBM_SETPOS, "以指定位置来设置进度条的当前位置,并重绘进度条来反映新位置")
IDSTR(PBM_SETRANGE, "设置进度条控件范围的上下限,并重绘进度条来反映新范围")
IDSTR(PBM_SETRANGE32, "为进度条设置32位的上下限范围")
IDSTR(PBM_SETSTEP, "为进度条控件指定步长(缺省为10)")
IDSTR(PBM_STEPIT, "用步进值来增加一个进度条控件的当前位置")
//Trackbar Control(跟踪器),也叫:Slider Control,滑块控件消息
IDSTR(TBM_CLEARSEL, "清除滑动块控件当前位置")
IDSTR(TBM_CLEARTICS, "删除滑动块控件当?翱潭认?)
IDSTR(TBM_GETBUDDY, "获取滑动块控件指定位置的伙伴窗口的句柄")
IDSTR(TBM_GETCHANNELRECT, "获取滑动块控件的通道的边界矩形的大小和位置,滑动块在通道上移动并选择一个范围后,它用高亮色显示")
IDSTR(TBM_GETLINESIZE, "获取滑动块控件的行的大小,缺省的行大小是1")
IDSTR(TBM_GETNUMTICS, "获取滑动块控件中的刻度线")
IDSTR(TBM_GETPAGESIZE, "获取滑动块控件一页的大小,页的大小表明在响应TB_PAGEUP和TB_PAGEDOWN通知时滑动块会移动多少")
IDSTR(TBM_GETPOS, "获取滑动块控件中滑动块的当前位置")
IDSTR(TBM_GETPTICS, "获取滑动块控件中的一个刻度线的当前物理位置(用客户坐标表示)")
IDSTR(TBM_GETRANGEMAX, "获取滑动块控件中滑动块的最大位置")
IDSTR(TBM_GETRANGEMIN, "获取滑动块控件中滑动块的最小位置")
IDSTR(TBM_GETSELEND, "获取滑动块控件中当前选择的结束位置")
IDSTR(TBM_GETSELSTART, "获取滑动块控件中当前选择的开始位置")
IDSTR(TBM_GETTHUMBLENGTH, "获取滑动块控件中滑动块(拇指)的长度")
IDSTR(TBM_GETTHUMBRECT, "获取滑动块控件中滑动块(拇指)的边界矩形的大小和位置")
IDSTR(TBM_GETTIC, "获取滑动块控件中刻度线的位置")
IDSTR(TBM_GETTICPOS, "获取滑动块控件中一个刻度线的当前物理位置(用客户坐标表示)")
IDSTR(TBM_GETTOOLTIPS, "获取滑动块控件的工具提示控件的句柄(若有),若TBS_TOOLTIPS风格,则返回NULL")
IDSTR(TBM_GETUNICODEFORMAT, "获取滑动块控件的UNICODE格式标志")
IDSTR(TBM_SETBUDDY, "设置滑动块控件指定位置的伙伴窗口")
IDSTR(TBM_SETLINESIZE, "设置滑动块控件行的大小,行的大小表示在响应TB_LINEUP和TV_LINEDOWN通知时,滑动块移动多少")
IDSTR(TBM_SETPAGESIZE, "设置滑动块控件页的大小,页的大小表示在响应TB_PAGEUP和TB_PAGEDOWN通知时,滑动块移动多少")
IDSTR(TBM_SETPOS, "设置滑动块控件中滑动块的当前位置")
IDSTR(TBM_SETRANGE, "设置滑动块控件滑动块的范围(位置的最小值和最大值)")
IDSTR(TBM_SETRANGEMAX, "设置滑动块控件中滑动块的最大位置")
IDSTR(TBM_SETRANGEMIN, "设置滑动块控件中滑动块的最小位置")
IDSTR(TBM_SETSEL, "设置滑动块控件当前选择的开始和结束位置")
IDSTR(TBM_SETSELEND, "设置滑动块控件中当前选择的结束位置")
IDSTR(TBM_SETSELSTART, "设置滑动块控件中当前选择的开始位置")
IDSTR(TBM_SETTHUMBLENGTH, "设置滑动块控件中滑动块(拇指)的长度")
IDSTR(TBM_SETTIC, "设置滑动块控件中一个刻度线的位置")
IDSTR(TBM_SETTICFREQ, "设置显示在滑动块控件中的刻度线的的频率,例如:若频率设为2,则每两个增量显示一个刻度线,缺省频率是1")
IDSTR(TBM_SETTIPSIDE, "设置滑动块控件中用于显示工具提示控件的位置")
IDSTR(TBM_SETTOOLTIPS, "为滑动块控件设置工具提示控件(用句柄进行设置)")
IDSTR(TBM_SETUNICODEFORMAT, "设置滑动块控件的UNICODE格式标志")
//Tab Control,标签控件消息
IDSTR(TCM_ADJUSTRECT, "根据给定的窗口矩形计算标签控件的显示区域,或根据一个给定的显示区域计算相应的窗口矩形")
IDSTR(TCM_DELETEALLITEMS, "删除标签控件中所有选项卡")
IDSTR(TCM_DELETEITEM, "从标签控件中删除指定选项卡")
IDSTR(TCM_DESELECTALL, "重新设置标签控件中的项,清除任何被按下的选项卡")
IDSTR(TCM_GETCURFOCUS, "获取标签控件中拥有当前焦点的选项卡索引")
IDSTR(TCM_GETCURSEL, "获取标签控件中当前选择的选项卡")
IDSTR(TCM_GETEXTENDEDSTYLE, "获取标签控件的扩展风格")
IDSTR(TCM_GETIMAGELIST, "获取与标签控件相关联的图像列表")
IDSTR(TCM_GETITEM, "获取标签控件中某选项卡的信息")
IDSTR(TCM_GETITEMCOUNT, "获取标签控件中的项数")
IDSTR(TCM_GETITEMRECT, "获取标签控件中指定选项卡的边界矩形")
IDSTR(TCM_GETROWCOUNT, "获取标签控件中当前行数,注:TCS_MULTILINT风格的标签控件可有多行选项卡")
IDSTR(TCM_GETTOOLTIPS, "获取与标签控件相关联的工具提示控件的句柄")
IDSTR(TCM_GETUNICODEFORMAT, "获取标签控件的UNICODE格式标志")
IDSTR(TCM_HIGHLIGHTITEM, "使标签控件的某选项卡处于高亮状态")
IDSTR(TCM_HITTEST, "确定是哪个选项卡(若有)位于指定的屏幕位置")
IDSTR(TCM_INSERTITEM, "插入一个新选项卡到标签控件")
IDSTR(TCM_REMOVEIMAGE, "从标签控件的图像列表中删除指定图像")
IDSTR(TCM_SETCURFOCUS, "设置焦点到标签控件中指定的选项卡")
IDSTR(TCM_SETCURSEL, "设置标签控件中当前选择的选项卡")
IDSTR(TCM_SETEXTENDEDSTYLE, "设置标签控件的扩展风格")
IDSTR(TCM_SETIMAGELIST, "为标签控件指定一个已创建的图像列表")
IDSTR(TCM_SETITEM, "设置标签控件中选项卡的某些或所有属性")
IDSTR(TCM_SETITEMEXTRA, "标签控件为空时,改变标签控件中各选项卡的额外字节数(默认4字节),注:额外字节可把程序数据与各选项卡关联起来")
IDSTR(TCM_SETITEMSIZE, "设置标签控件中选项卡的宽度和高度")
IDSTR(TCM_SETMINTABWIDTH, "设置标签控件中选项卡的最小宽度")
IDSTR(TCM_SETPADDING, "设置标签控件中的每个选项卡的图标和标签周围的空间大小(填充距)")
IDSTR(TCM_SETTOOLTIPS, "设置与标签控件相关联的工具提示控件(用句柄进行设置)")
IDSTR(TCM_SETUNICODEFORMAT, "设置标签控件的UNICODE格式标志")
...
IDSTR(TCN_SELCHANGE, "通知标签控件的父窗:选项卡的当前选择已被改变")
IDSTR(TCN_SELCHANGING, "通知标签控件的父窗:选项卡的当前选择将要改变")
...
IDSTR(EM_GETRECT, "获取编辑控件的带格式的文本边界矩形(与其窗口大小无关)")
IDSTR(EM_SETRECT, "设置编辑控件的带格式的文本边界矩形(与其窗口大小无关)")
IDSTR(EM_SETRECTNP, "设置多行编辑控件的带格式的文本边界矩形,而不必重绘")
IDSTR(EM_SCROLL, "使多行编辑控件滚动一行(SB_LINEDOWN/SB_LINEUP)或一页(SB_PAGEDOWN/SB_PAGEUP)")
IDSTR(EM_LINESCROLL, "以行为单位使多行编辑控件左右或上下滚动")
IDSTR(EM_SCROLLCARET, "滚动编辑控件中的caret插入光标,使之可视")
IDSTR(EM_GETMODIFY, "测试编辑控件的内容是否被改变(它有个内部标记来表明其内容是否被改变)")
IDSTR(EM_SETMODIFY, "设置或清除编辑控件的改变标志;改变标记表明文本是否被改变(用户改变文本时,会自动设置此标志)")
IDSTR(EM_GETLINECOUNT, "获取多行编辑控件中的总行数")
IDSTR(EM_LINEINDEX, "获得多行编辑控件中某行的字符索引")
IDSTR(EM_SETHANDLE, "设置可被多行编辑控件使用的局部内存的句柄,编辑控件可用此缓冲区存储当前显示的文本,而不必自己分配")
IDSTR(EM_GETHANDLE, "获取多行编辑控件中当前分配的内存句柄,此句柄是个局部内存句柄")
IDSTR(EM_GETTHUMB, "取得多行文本编辑控件的滚动框的当前位置(象素)")
IDSTR(EM_LINELENGTH, "获得编辑控件中的行的长度")
IDSTR(EM_REPLACESEL, "用指定文本覆盖编辑控件中当前被选中的文本")
IDSTR(EM_GETLINE, "在编辑控件中获得一行文本")
IDSTR(EM_LIMITTEXT, "用户在编辑控件中输入文本时的文本长度限制")
IDSTR(EM_CANUNDO, "确定对编辑控件的操作能否撤销")
IDSTR(EM_UNDO, "撤销上一次对编辑控件的操作")
IDSTR(EM_FMTLINES, "设置多行编辑控件中是否包含软回车符")
IDSTR(EM_LINEFROMCHAR, "获取包含指定字符索引的行的行号(字符索引指从开始到指定字符的字符数)")
IDSTR(EM_SETTABSTOPS, "在多行编辑控件中设置跳格键的跳幅(控件中文本的任何制表键间会产生一段空白)")
IDSTR(EM_SETPASSWORDCHAR, "设置或清除编辑控件中密码的替换显示字符")
IDSTR(EM_EMPTYUNDOBUFFER, "清除控件的撤消缓冲区,使其不能撤消上一次编辑操作")
IDSTR(EM_GETFIRSTVISIBLELINE, "确定编辑控件中可视的最顶端行的行号")
IDSTR(EM_SETREADONLY, "设置编辑控件的只读状态")
IDSTR(EM_SETWORDBREAKPROC, "设置编辑控件的新的断字处理回调函数")
IDSTR(EM_GETWORDBREAKPROC, "获取编辑控件的新的断字处理回调函数")
IDSTR(EM_GETPASSWORDCHAR, "获取编辑控件中密码的替换显示字符")
IDSTR(EM_SETMARGINS, "设置编辑控件的左右边空")
IDSTR(EM_GETMARGINS, "获取编辑控件的左右边空")
IDSTR(EM_SETLIMITTEXT, "设置编辑控件中文本的输入长度限制")
IDSTR(EM_GETLIMITTEXT, "获取编辑控件中文本的输入长度限制")
IDSTR(EM_POSFROMCHAR, "获得指定字符索引的左上角的坐标")
IDSTR(EM_CHARFROMPOS, "获得编辑控件中最靠近指定位置的字符的行和字符索引")
IDSTR(EM_SETIMESTATUS, "修改编辑控件的IME输入法的属性")
IDSTR(EM_GETIMESTATUS, "获取编辑控件的IME输入法的属性")
//Edit Control,编辑控件(编辑框)通知消息
IDSTR(EN_SETFOCUS, "通知编辑控件的父窗(通过WM_COMMAND获知):编辑框获得输入焦点")
IDSTR(EN_KILLFOCUS, "通知编辑控件的父窗(通过WM_COMMAND获知):编辑框失去输入焦点")
...
IDSTR(EN_MAXTEXT, "通知父窗,编辑控件当前输入文本已超过指定字符数(并作截尾处理)")
IDSTR(EN_HSCROLL, "用户单击了编辑控件的水平滚动条,父窗在屏幕更新之前被通知")
IDSTR(EN_VSCROLL, "用户单击了编辑控件的垂直滚动条,父窗在屏幕更新之前被通知")
//Hot Key Control,热键控件消息
IDSTR(HKM_GETHOTKEY, "从热键控件中获取一个虚拟键码和修正符标志")
IDSTR(HKM_SETHOTKEY, "为热键控件设置热键组合")
IDSTR(HKM_SETRULES, "为热键控件定义不可用组合和缺省修正符组合")
//Month Calendar Control,月历控件消息
IDSTR(MCM_GETCOLOR, "获取月历控件中各部分的颜色设置")
IDSTR(MCM_GETCURSEL, "获取月历控件中当前选定日期指定的系统时间")
IDSTR(MCM_GETFIRSTDAYOFWEEK, "获取月历控件最左边显示的星期值")
IDSTR(MCM_GETMAXSELCOUNT, "获取月历控件中能被选择的日期最大值")
IDSTR(MCM_GETMAXTODAYWIDTH, "获取月历控件中\"今天\"这个字符串的最大宽度")
IDSTR(MCM_GETMINREQRECT, "获取月历控件显示完整月份所需的最小值")
IDSTR(MCM_GETMONTHDELTA, "获取月历控件的滚动速率")
IDSTR(MCM_GETMONTHRANGE, "获取代表月历控件显示的日期上限和下限的有关信息")
IDSTR(MCM_GETRANGE, "获取月历控件中所设置的最大和最小日期值")
IDSTR(MCM_GETSELRANGE, "获取代表由用户选定当前日期上限和下限的有关信息")
IDSTR(MCM_GETTODAY, "获取月历控件中今天的日期")
IDSTR(MCM_GETUNICODEFORMAT, "获取月历控件的UNICODE格式标志")
IDSTR(MCM_HITTEST, "确定月历控件是否位于指定位置")
IDSTR(MCM_SETCOLOR, "改变月历控件中各部分的颜色设置")
IDSTR(MCM_SETCURSEL, "设定月历控件当前选定的日期")
IDSTR(MCM_SETDAYSTATE, "在月历控件中设置要显示的日期")
IDSTR(MCM_SETFIRSTDAYOFWEEK, "在月历控件的最左边设置要显示的星期值")
IDSTR(MCM_SETMAXSELCOUNT, "将月历控件中能被选择的日期值设置为最大")
IDSTR(MCM_SETMONTHDELTA, "为月历控件设置滚动速率")
IDSTR(MCM_SETRANGE, "设置月历控件中所许可的最大和最小日期值")
IDSTR(MCM_SETSELRANGE, "将被选定的月历控件范围设置为给定的日期范围")
IDSTR(MCM_SETTODAY, "设置月历控件中今天的日期")
...
IDSTR(MCN_SELCHANGE, "当前选择项已被改变,相关结构:NMSELCHANGE")
IDSTR(MCN_SELECT, "在月历控件中选择一个日期,相关结构:NMSELCHANGE")
IDSTR(NM_RELEASEDCAPTURE, "释放月历控件中鼠标的捕获消息,相关结构:NMHDR")
//date and time picker control,日期时间选择控件(DTP控件)消息
IDSTR(DTM_GETMCCOLOR, "获取DTP控件中下拉月历指定部分的颜色,宏:DateTime_GetMonthCalColor")
IDSTR(DTM_GETMCFONT, "获取DTP控件中下拉月历的当前所用字体,宏:DateTime_GetMonthCalFont")
IDSTR(DTM_GETMONTHCAL, "获取DTP控件中下拉月历的句柄,宏:DateTime_GetMonthCal")
IDSTR(DTM_GETRANGE, "获取DTP控件中最小和最大允许的系统时间,宏:DateTime_GetRange")
...
IDSTR(DTM_SETMCCOLOR, "设置DTP控件中下拉月历指定部分的颜色,宏:DateTime_SetMonthCalColor")
IDSTR(DTM_SETMCFONT, "设置DTP控件中下拉月历的当前所用字体,宏:DateTime_SetMonthCalFont")
...
IDSTR(DTN_CLOSEUP, "DTP控件中的月历控件将要关闭,相关结构:NMHDR")
IDSTR(DTN_DATETIMECHANGE, "DTP控件中的内容已发生改变,相关结构:NMDATETIMECHANGE")
IDSTR(DTN_DROPDOWN, "DTP控件中的月历控件将要显示,相关结构:NMHDR")
IDSTR(DTN_FORMAT, "DTP控件中的文本需要显示在回调域中,相关结构:NMDATETIMEFORMAT")
IDSTR(DTN_FORMATQUERY, "DTP控件中的文本显示所需最大尺寸,相关结构:NMDATETIMEFORMATQUERY")
IDSTR(DTN_USERSTRING, "用户已完成DTP控件的编辑,相关结构:NMDATETIMESTRING")
...
//Status Bar,状态栏消息
IDSTR(SB_GETBORDERS, "获取状态栏的边界值")
IDSTR(SB_GETICON, "获取状态栏的窗格图标")
IDSTR(SB_GETPARTS, "获取状态栏的窗格数量,也可用来?袢≈付ù案竦挠冶咦?)
IDSTR(SB_GETRECT, "获取状态栏某窗格的边界矩形")
IDSTR(SB_GETTEXT, "从状态栏的指定窗格获取文本")
IDSTR(SB_GETTEXTLENGTH, "从状态栏的指定窗格获取文本长度(用字符数表示)")
IDSTR(SB_GETTIPTEXT, "获取状态栏某窗格的工具提示文本")
IDSTR(SB_GETUNICODEFORMAT, "获取状态栏的UNICODE格式标志")
IDSTR(SB_ISSIMPLE, "状态栏是否处于简单文本模式")
IDSTR(SB_SETBKCOLOR, "设置状态栏的背景色")
IDSTR(SB_SETICON, "设置状态栏的窗格图标")
IDSTR(SB_SETMINHEIGHT, "设置状态栏的绘图区的最小高度")
IDSTR(SB_SETPARTS, "设置状态栏的窗格数量")
IDSTR(SB_SETTEXT, "设置状态栏的窗格文本")
IDSTR(SB_SETTIPTEXT, "设置状态栏某窗格的工具提示文本")
IDSTR(SB_SETUNICODEFORMAT, "设置状态栏的UNICODE格式标志")
IDSTR(SB_SIMPLE, "把状态栏设为简单文本模式")
//Status Bar,状态栏通知消息
IDSTR(SBN_SIMPLEMODECHANGE, "由于SB_SIMPLE消息导致状态栏简单模式发生改变时发送的通知消息")
//ReBar control,ReBar控件(伸缩条)消息
IDSTR(RB_BEGINDRAG, "ReBar控件开始拖放")
IDSTR(RB_DELETEBAND, "删除ReBar控件中某个指定索引的带")
IDSTR(RB_DRAGMOVE, "更新被拖动的ReBar控件的位置")
IDSTR(RB_ENDDRAG, "ReBar控件停止拖放")
IDSTR(RB_GETBANDBORDERS, "获取ReBar控件指定带的区域")
IDSTR(RB_GETBANDCOUNT, "获取ReBar控件中当前带数")
IDSTR(RB_GETBANDINFO, "获取ReBar控件中指定带的信息")
IDSTR(RB_GETBARHEIGHT, "获取ReBar控件高度")
IDSTR(RB_GETBARINFO, "获取有关ReBar控件的信息获及其使用的图像列表")
IDSTR(RB_GETBKCOLOR, "获取ReBar控件的背景色")
IDSTR(RB_GETDROPTARGET, "获取ReBar控件的IDropTarget接口指针(不再使用时,要Release释放之)")
IDSTR(RB_GETCOLORSCHEME, "获取ReBar控件的颜色方案")
IDSTR(RB_GETPALETTE, "获取ReBar控件的当前调色板")
IDSTR(RB_GETRECT, "获取ReBar控件矩形区域")
IDSTR(RB_GETROWCOUNT, "获取ReBar控件的带所占行数")
IDSTR(RB_GETROWHEIGHT, "获取ReBar控件指定行的高度")
IDSTR(RB_GETTEXTCOLOR, "获取ReBar控件的文本前景色")
IDSTR(RB_GETTOOLTIPS, "获取与ReBar控件相关的工具提示控件的句柄")
IDSTR(RB_GETUNICODEFORMAT, "获取ReBar控件的UNICODE格式标志")
IDSTR(RB_HITTEST, "若ReBar带在屏幕上的指定点存在,则确定ReBar带的哪部分位于该点上")
IDSTR(RB_IDTOINDEX, "将一个带的标识符ID转换成ReBar控件中的带索引")
IDSTR(RB_INSERTBAND, "ReBar控件中插入一个新带")
IDSTR(RB_MAXIMIZEBAND, "将ReBar控件中的一个带调整到它的理想或最大尺寸")
IDSTR(RB_MINIMIZEBAND, "将ReBar控件中的一个带调整到它的理想或最小尺寸")
IDSTR(RB_MOVEBAND, "将ReBar控件的一个带从一个索引移动到另一个索引")
IDSTR(RB_SETBANDINFO, "设置ReBar控件中的已存在带的特征")
IDSTR(RB_SETBARINFO, "设置ReBar控件的信息")
IDSTR(RB_SETBKCOLOR, "设置ReBar控件的背景色")
IDSTR(RB_SETCOLORSCHEME, "设置ReBar控件的颜色方案")
IDSTR(RB_SETPALETTE, "为ReBar控件设置新的调色板")
IDSTR(RB_SETPARENT, "设置ReBar控件的属主窗口")
IDSTR(RB_SETTEXTCOLOR, "设置ReBar控件的文本前景色")
IDSTR(RB_SETTOOLTIPS, "使一个工具提示控件与ReBar控件相关联")
IDSTR(RB_SETUNICODEFORMAT, "设置ReBar控件的UNICODE格式标志")
IDSTR(RB_SHOWBAND, "显示或隐藏ReBar控件中的指定带")
IDSTR(RB_SIZETORECT, "使ReBar控件的尺寸与一个指定矩形最优匹配")
//rebar control.rebar控件通知消息
IDSTR(RBN_AUTOSIZE, "当Rebar控件自己自动调整大小时由(用RBS_AUTOSIZE风格创建的)Rebar控件发送")
IDSTR(RBN_BEGINDRAG, "当用户开始拖动带区时由Rebar控件发送")
IDSTR(RBN_CHILDSIZE, "当调整带区的子窗口大小时由Rebar控件发送")
IDSTR(RBN_DELETEDBAND, "在带区已被删除后由Rebar控件发送")
IDSTR(RBN_DELETINGBAND, "当带区即将被删除时由Rebar控件发送")
IDSTR(RBN_ENDDRAG, "当用户停止拖动带区时由Rebar控件发送")
...
IDSTR(PSM_ADDPAGE, "添加新页面到属性表,宏:PropSheet_AddPage")
IDSTR(PSM_APPLY, "应用属性表的新属性,宏:PropSheet_Apply")
IDSTR(PSM_CANCELTOCLOSE, "使属性表改变确定按钮上的文本为关闭(标志着应用的改变不可被取消),宏:PropSheet_CancelToClose")
IDSTR(PSM_CHANGED, "使属性表激活应用按钮(标志着用户已经编辑了一个属性),宏:PropSheet_Changed")
IDSTR(PSM_GETCURRENTPAGEHWND, "获取属性表当前页的窗口句柄,宏:PropSheet_GetCurrentPageHwnd")
IDSTR(PSM_GETTABCONTROL, "获取属性表中标签控件的句柄,宏:PropSheet_GetTabControl")
IDSTR(PSM_ISDIALOGMESSAGE, "发送一个消息至属性表对话框,并指明该对话框是否已处理了该消息,宏:PropSheet_IsDialogMessage")
IDSTR(PSM_PRESSBUTTON, "在属性表中模拟选择某指定按钮,宏:PropSheet_PressButton")
IDSTR(PSM_QUERYSIBLINGS, "查询属性表的兄弟页,相互传递参数,宏:PropSheet_QuerySiblings")
IDSTR(PSM_REBOOTSYSTEM, "指明需重启系统以使改变生效(若某页改变了系统配置),宏:PropSheet_RebootSystem")
IDSTR(PSM_REMOVEPAGE, "从属性表中移走一页,并销毁与此页相关的窗口,宏:PropSheet_RemovePage")
IDSTR(PSM_RESTARTWINDOWS, "指明属性表需重启以使改变生效,宏:PropSheet_RestartWindows")
IDSTR(PSM_SETCURSEL, "选择(激活)属性表中某页,宏:PropSheet_SetCurSel")
IDSTR(PSM_SETCURSELID, "根据页ID选择(激活)属性表中某页,宏:PropSheet_SetCurSelByID")
IDSTR(PSM_SETFINISHTEXT, "设置属性表中Finish按钮的文本,宏:PropSheet_SetFinishText")
IDSTR(PSM_SETTITLE, "指定属性表的标题,宏:PropSheet_SetTitle")
IDSTR(PSM_SETWIZBUTTONS, "使向导属性表中的Back,Next或Finish按钮有效或无效,宏:PropSheet_SetWizButtons")
IDSTR(PSM_UNCHANGED, "禁止应用按钮,重新初始化属性表,宏:PropSheet_UnChanged")
//Property Sheet,属性页通知消息
IDSTR(PSN_APPLY, "属性页的应用按钮被按下,相关结构:PSHNOTIFY")
IDSTR(PSN_GETOBJECT, "通知允许属性页进行OLE拖放对象,相关结构:NMOBJECTNOTIFY")
IDSTR(PSN_HELP, "用户点击了属性页的帮助按扭,相关结构:PSHNOTIFY")
IDSTR(PSN_KILLACTIVE, "属性页失去焦点(其它属性页获得了焦点或用户点击了确定按钮),相关结构:PSHNOTIFY")
IDSTR(PSN_QUERYCANCEL, "用户点击了属性页的取消按扭,相关结构:PSHNOTIFY")
IDSTR(PSN_RESET, "用户点击了取消按扭,属性页将被销毁,相关结构:PSHNOTIFY")
IDSTR(PSN_SETACTIVE, "某属性页被激活获得焦点,相关结构:PSHNOTIFY")
IDSTR(PSN_WIZBACK, "用户点击了向导属性表的前一页按钮,相关结构:PSHNOTIFY")
IDSTR(PSN_WIZFINISH, "用户点击了向导属性表的结束按钮,相关结构:PSHNOTIFY")
IDSTR(PSN_WIZNEXT, "用户点击了向导属性表的下一页按钮,相关结构:PSHNOTIFY")
...
//AppBar,应用桌面工具栏消息
IDSTR(ABM_ACTIVATE, "通知系统AppBar已被激活,相关结构:APPBARDATA")
IDSTR(ABM_GETAUTOHIDEBAR, "获取在屏幕某边自动隐藏的AppBar的句柄")
IDSTR(ABM_GETSTATE, "获取自动隐藏且置顶的任务栏的状态")
IDSTR(ABM_GETTASKBARPOS, "获取任务栏的边界矩形")
IDSTR(ABM_NEW, "注册一个新的AppBar,并指定一个让系统发送通知消息的消息ID")
IDSTR(ABM_QUERYPOS, "为AppBar请求大小与屏幕坐标")
IDSTR(ABM_REMOVE, "反注册一个AppBar(使之从系统内部列表中去除)")
IDSTR(ABM_SETAUTOHIDEBAR, "注册(或反注册)一个在屏幕某边自动隐藏的AppBar")
IDSTR(ABM_SETPOS, "为一个AppBar设置大小与屏幕坐标")
IDSTR(ABM_WINDOWPOSCHANGED, "通知系统一个AppBar的屏幕坐标已被改变")
//appbar,应用桌面工具栏通知消息
IDSTR(ABN_FULLSCREENAPP, "通知一个AppBar:某全屏应用程序正在打开?蚬乇?)
IDSTR(ABN_POSCHANGED, "通知一个AppBar:一个可能会被到AppBar的大小与位置的事件已经发生")
IDSTR(ABN_STATECHANGE, "通知一个AppBar:任务栏自动隐藏或置顶状态已被改变")
IDSTR(ABN_WINDOWARRANGE, "通知一个AppBar:用户已从任务栏的上下文菜单中选择了层叠或平铺")
//Control Panel,控制面板消息
IDSTR(CPL_DBLCLK, "用户双击控制面板中某组件图标时向该组件所在CPL库发送本消息,CPlApplet相关")
IDSTR(CPL_EXIT, "控制面板关闭时会对各组件发送CPL_STOP消息,接着对各CPL库发送本消息,此时CPL库释放在CPL_INIT消息中分配的内存和资源")
IDSTR(CPL_GETCOUNT, "获取控制面板程序数量")
IDSTR(CPL_INIT, "发送本消息以指示CPL库作初始化工作")
IDSTR(CPL_INQUIRE, "Windows3.x适用,获取各组件的图标、名称和提示信息,CPL库可在处理这条消息时依次初始化各组件的对话框")
IDSTR(CPL_NEWINQUIRE, "获取各组件的图标、名称和提示信息,CPL库可在处理这条消息时依次初始化各组件的对话框")
IDSTR(CPL_SELECT, "基于WIN32的CPL库已废除本消息")
IDSTR(CPL_STOP, "控制面板关闭时会对各组件发送CPL_STOP消息,接着对各CPL库发送本消息,此时CPL库释放在CPL_INIT消息中分配的内存和资源")
IDSTR(WM_CPL_LAUNCH, "请求启动某控制面板程序")
IDSTR(WM_CPL_LAUNCHED, "某控制面板程序已启动")
//File Manager,文件管理器消息
IDSTR(FM_GETDRIVEINFO, "从文件管理器窗口获取驱动器信息(总空间大小或空闲空间大小等)")
IDSTR(FM_GETFILESEL, "获取文件管理器(目录窗口或搜索结果窗口)中已选择文件的信息")
IDSTR(FM_GETFILESELLFN, "获取文件管理器(目录窗口或搜索结果窗口)中已选择文件的信息(包含长文件名)")
IDSTR(FM_GETFOCUS, "获取拥有焦点的文件管理器窗口的类型(如:返回FMFOCUS_SEARCH表示搜索结果窗口拥有当前焦点)")
IDSTR(FM_GETSELCOUNT, "获取文件管理器(目录窗口或搜索结果窗口)中已选择的文件数量")
IDSTR(FM_GETSELCOUNTLFN, "获取文件管理器(目录窗口或搜索结果窗口)中已选择的文件数量(包含长文件名)")
IDSTR(FM_REFRESH_WINDOWS, "刷新文件管理器激活窗(或所有窗口)时发送本消息给扩展DLL")
IDSTR(FM_RELOAD_EXTENSIONS, "使文件管理器重载所有(在Winfile.ini的AddOns节列出的)扩展DLL")
IDSTR(FMEVENT_HELPMENUITEM, "用户在某菜单或工具栏按钮上按下<F1>时向文件管理器扩展DLL发送本消息")
IDSTR(FMEVENT_HELPSTRING, "文件管理器需要为某菜单或工具栏按钮设置帮助字符串时向扩展DLL发送本消息")
IDSTR(FMEVENT_INITMENU, "用户从文件管理器的菜单选择了扩展菜单时向扩展DLL发送本消息")
IDSTR(FMEVENT_LOAD, "文件管理器加载扩展DLL时发送,如:扩展功能菜单等")
IDSTR(FMEVENT_SELCHANGE, "用户在文件管理器的目录窗口或搜索结果窗口中选择了文件名时向扩展DLL发送本消息")
IDSTR(FMEVENT_TOOLBARLOAD, "文件管理器在加载其工具栏时向扩展DLL发送本消息")
IDSTR(FMEVENT_UNLOAD, "文件管理器卸载扩展DLL时发送")
IDSTR(FMEVENT_USER_REFRESH, "用户在文件管理器的\"视图\"中选择了刷新菜单项时发送本消息给扩展DLL")
//系统托盘消息
IDSTR(NIM_ADD, "添加图标到系统托盘,相关结构:NOTIFYICONDATA")
IDSTR(NIM_DELETE, "从系统托盘删除图标,相关结构:NOTIFYICONDATA")
IDSTR(NIM_MODIFY, "修改系统托盘中的图标、提示或通知消息的ID,相关结构:NOTIFYICONDATA")
//Common Control,控件的公共通知消息
IDSTR(NM_CHAR, "字符键被处理后由控件发送,相关结构:NMCHAR")
IDSTR(NM_CLICK, "通知父窗:用户在控件上点击了鼠标左键,相关结构:NMHDR")
IDSTR(NM_DBLCLK, "通知父窗:用户在控件上双击了鼠标左键,相关结构:NMHDR")
IDSTR(NM_HOVER, "用户的鼠标掠过控件,相关结构:NMHDR")
IDSTR(NM_KEYDOWN, "控件获得键盘焦点并按下某键,相关结构:NMKEY")
IDSTR(NM_KILLFOCUS, "通知父窗:用户失去输入焦点,相关结构:NMHDR")
IDSTR(NM_NCHITTEST, "控件收到WM_NCHITTEST消息,相关结构:NMMOUSE")
IDSTR(NM_OUTOFMEMORY, "通知父窗:内存不足,操作不能完成,相关结构:NMHDR")
IDSTR(NM_RCLICK, "通知父窗:用户在控件上点击了鼠标右键,相关结构:NMHDR")
IDSTR(NM_RDBLCLK, "通知父窗:用户在控件上双击了鼠标右键,相关结构:NMHDR")
IDSTR(NM_RELEASEDCAPTURE, "通知父窗:控件要释放鼠标捕捉,相关结构:NMHDR")
IDSTR(NM_RETURN, "通知父窗:控件获得键盘焦点并按下回车键,相关结构:NMHDR")
IDSTR(NM_SETCURSOR, "通知父窗:控件响应WM_SETCURSOR要设置光标,相关结构:NMMOUSE")
IDSTR(NM_SETFOCUS, "通知父窗:控件收到输入焦点,相关结构:NMHDR")
IDSTR(NM_TOOLTIPSCREATED, "通知父窗:控件已创建一个工具提示控件,相关结构:NMTOOLTIPSCREATED")
//Up-Down Control(Spin Control),微调按钮消息
IDSTR(UDM_GETACCEL, "获取旋转按钮的加速信息,相关结构:UDACCEL")
IDSTR(UDM_GETBASE, "获取旋转按钮的当前基数")
IDSTR(UDM_GETBUDDY, "获取旋转按钮的当前伙伴窗口的句柄")
IDSTR(UDM_GETPOS, "获取旋转按钮当前位置(根据伙?榇翱诘谋晏饣竦梅祷刂?")
IDSTR(UDM_GETRANGE, "获取旋转按钮的上下限范围")
IDSTR(UDM_GETRANGE32, "获取旋转按钮的上下限范围(32位DWORD值)")
IDSTR(UDM_GETUNICODEFORMAT, "获取旋转按钮的UNICODE格式标志")
IDSTR(UDM_SETACCEL, "设置旋转按钮的加速信息,相关结构:UDACCEL")
IDSTR(UDM_SETBASE, "设置旋转按钮的基数")
IDSTR(UDM_SETBUDDY, "为旋转按钮设置伙伴窗口")
IDSTR(UDM_SETPOS, "设置旋转按钮的当前位置")
IDSTR(UDM_SETRANGE, "设置旋转按钮的上下限范围")
IDSTR(UDM_SETRANGE32, "设置旋转按钮的上下限范围(32位DWORD值)")
IDSTR(UDM_SETUNICODEFORMAT, "设置旋转按钮的UNICODE格式标志" ,
    //Up-Down Control(Spin Control),微调按钮(旋转按钮)通知消息
    IDSTR(UDN_DELTAPOS, "微调按钮控件的位置将要改变(向其父窗口发送),相关结构:NMUPDOWN" ,
        //Video Capture,视频采集(捕捉)消息
        IDSTR(WM_CAP_ABORT, "终止视频数据采集操作,宏:capCaptureAbort")
        IDSTR(WM_CAP_DLG_VIDEOCOMPRESSION, "显示视频采集压缩器选择对话框,宏:apDlgVideoCompression")
        IDSTR(WM_CAP_DLG_VIDEODISPLAY, "显示视频输出调节对话框(包含控制显示的图像的色调、亮度控件),宏:capDlgVideoDisplay")
        IDSTR(WM_CAP_DLG_VIDEOFORMAT, "显示视频格式选择对话框(选择图像尺寸、位深度和硬压缩选项）,宏:capDlgVideoFormat")
        IDSTR(WM_CAP_DLG_VIDEOSOURCE, "显示一个对话框,在该对话框中可选择视频资源,包括视频输入资源,宏:capDlgVideoSource")
        IDSTR(WM_CAP_DRIVER_CONNECT, "将视频捕获窗口与其驱动相连接,宏:capDriverConnect")
        IDSTR(WM_CAP_DRIVER_DISCONNECT, "将视频捕捉窗口与驱动断开,宏:capDriverDisconnect")
        IDSTR(WM_CAP_DRIVER_GET_CAPS, "获取视频捕获驱动及其硬件性能参数(信息保存在CAPDRIVERCAPS结构中),宏:capDriverGetCaps")
        IDSTR(WM_CAP_DRIVER_GET_NAME, "获取连接到视频捕获窗口的驱动名称,宏:capDriverGetName")
        IDSTR(WM_CAP_DRIVER_GET_VERSION, "获取连接到视频捕获窗口的驱动版本,宏:capDriverGetVersion")
        IDSTR(WM_CAP_EDIT_COPY, "视频采集时,从帧缓存区拷贝一幅图像到剪贴板,宏:capEditCopy")
        IDSTR(WM_CAP_FILE_ALLOCATE, "为视频捕获文件预分配磁盘空间(可减少数据保存时的处理时间),宏:capFileAlloc")
        IDSTR(WM_CAP_FILE_GET_CAPTURE_FILE, "获取当前视频采集文件名,宏:capFileGetCaptureFile")
        IDSTR(WM_CAP_FILE_SAVEAS, "另存视频捕获数据到一个新文件,宏:capFileSaveAs")
        IDSTR(WM_CAP_FILE_SAVEDIB, "将视频捕获数据缓存区的一幅图像拷贝至一张DIB位图中,宏:capFileSaveDIB")
        IDSTR(WM_CAP_FILE_SET_CAPTURE_FILE, "指定视频捕获文件名(本消息并不实际创建文件),宏:capFileSetCaptureFile")
        IDSTR(WM_CAP_FILE_SET_INFOCHUNK, "设置或清除AVI文件中插入的信息块(如文本或自定义数据),宏:capFileSetInfoChunk")
        IDSTR(WM_CAP_GET_AUDIOFORMAT, "获得当前音频数据格式或音频数据结构的大小(默认格式是mono,8 bit,11 kHz PCM),宏:capGetAudioFormat/capGetAudioFormatSize ")
        IDSTR(WM_CAP_GET_MCI_DEVICE, "获得当前使用的视频采集设备MCI驱动名称, 宏:capGetMCIDeviceName")
        IDSTR(WM_CAP_GET_SEQUENCE_SETUP, "获取当前视频捕获的帧频率(每秒捕获几帧),宏:capCaptureGetSetup")
        IDSTR(WM_CAP_GET_STATUS, "获得当前视频捕获窗口的状态,宏:capGetStatus")
        IDSTR(WM_CAP_GET_USER_DATA, "获取关联到一个视频捕获窗口的数据(长整型值),宏:capGetUserData ")
        IDSTR(WM_CAP_GET_VIDEOFORMAT, "获取包含视频格式的结构体或视频格式大小(其实就是BITMAPINFO),宏:capGetVideoForma/capGetVideoFormatSize")
        IDSTR(WM_CAP_GRAB_FRAME, "从视频采集驱动获取并显示一个单独的帧(静态图像),宏:capGrabFrame")
        IDSTR(WM_CAP_GRAB_FRAME_NOSTOP, "视频捕捉,填充未压缩的单帧图像到帧缓存区并显示之(与WM_CAP_GRAB_FRAME不同:覆盖或预览状态不会改变),宏:capGrabFrameNoStop")
        IDSTR(WM_CAP_PAL_AUTOCREATE, "请求视频信号取样帧及自动创建与调色板,宏:capPaletteAuto")
        IDSTR(WM_CAP_PAL_MANUALCREATE, "请求视频信号手工取样帧及创建自定义的调色板(替换默认的调色板),宏:capPaletteAuto")
        IDSTR(WM_CAP_PAL_OPEN, "从调色板文件中加载一个调色板,宏:capPaletteOpen")
        IDSTR(WM_CAP_PAL_PASTE, "从剪切板中拷贝加载一个调色板,宏:capPalettePaste")
        IDSTR(WM_CAP_PAL_SAVE, "保存调色板至调色板文件,宏:capPaletteSave")
        IDSTR(WM_CAP_SEQUENCE, "开始视频与音频的捕获(到文件),宏:capCaptureSequence")
        IDSTR(WM_CAP_SEQUENCE_NOFILE, "开始视频与音频的捕获(不写入文件),宏:capCaptureSequenceNoFile")
        IDSTR(WM_CAP_SET_AUDIOFORMAT, "设置音频数据捕获格式,宏:capSetAudioFormat")
        IDSTR(WM_CAP_SET_CALLBACK_CAPCONTROL, "指定回调函数用于控制捕获的开始和结束,宏:capSetCallbackOnCapControl")
        IDSTR(WM_CAP_SET_CALLBACK_ERROR, "在应用程序中指定回调函数(出错时就调用它),宏:capSetCallbackOnError")
        IDSTR(WM_CAP_SET_CALLBACK_FRAME, "在应用程序中指定回调函数(当预览图像帧被捕获了的时候就调用它),宏:capSetCallbackOnFrame")
        IDSTR(WM_CAP_SET_CALLBACK_STATUS, "在应用程序中指定回调函数(当状态改变时就调用它),宏:capSetCallbackOnStatus")
        IDSTR(WM_CAP_SET_CALLBACK_VIDEOSTREAM, "在应用程序中指定回调函数(在流捕获期间,当一个新的视频缓存区可用时就调用它),宏:capSetCallbackOnVideoStream ")
        IDSTR(WM_CAP_SET_CALLBACK_WAVESTREAM, "在应用程序中指定回调函数(在流捕获期间,当一个新的音频缓存区可用时就调用它),宏:capSetCallbackOnWaveStream")
        IDSTR(WM_CAP_SET_CALLBACK_YIELD, "流捕获期间可使用Yield回调函数(Yield回调函数由消息循环组成),每次捕获视频帧时至少调用一次Yield,具体视帧速率决定,宏:capSetCallbackOnYield")
        IDSTR(WM_CAP_SET_MCI_DEVICE, "指定要选定的MCI设备名称(如:一张影碟或一盘录像带来充当视频源),宏:capSetMCIDeviceName")
        IDSTR(WM_CAP_SET_OVERLAY, "打开覆盖模式(将自动关闭预览模式),覆盖模式:不占用CPU资源,直接在显示器上显示视频(由捕获设备硬件来完成),宏:capOverlay")
        IDSTR(WM_CAP_SET_PREVIEW, "打开或关闭预览模式,宏:capPreview")
        IDSTR(WM_CAP_SET_PREVIEWRATE, "设置预览模式下图像的帧速度,宏:capPreviewRate")
        IDSTR(WM_CAP_SET_SCALE, "打开或关闭预览视频的缩放比例,宏:capPreviewScale")
        IDSTR(WM_CAP_SET_SCROLL, "设置视频帧的滚动条的位置(预览模式或叠加模式),宏:capSetScrollPos")
        IDSTR(WM_CAP_SET_SEQUENCE_SETUP, "设置流捕捉时的配置参数,刷新CAPTUREPARMS结构,宏:capCaptureSetSetup")
        IDSTR(WM_CAP_SET_USER_DATA, "关联数据(长整型值)到一个视频捕获窗口,宏:capSetUserData")
        IDSTR(WM_CAP_SET_VIDEOFORMAT, "对视频格式进行修改设置,宏:capSetVideoFormat")
        IDSTR(WM_CAP_SINGLE_FRAME, "指定捕获视频流中的个别帧,宏:capCaptureSingleFrame")
        IDSTR(WM_CAP_SINGLE_FRAME_CLOSE, "关闭单帧捕获文件,宏:capCaptureSingleFrameClose")
        IDSTR(WM_CAP_SINGLE_FRAME_OPEN, "打开单帧捕获文件,宏:capCaptureSingleFrameOpen")
        IDSTR(WM_CAP_STOP, "停止视频捕获操作,宏:capCaptureStop")
        //Common Dialog Box,通用对话框消息
        //Font dialog box,字体通用对话框
        IDSTR(WM_CHOOSEFONT_GETLOGFONT, "获取字体通用对话框中用户当前字体选择信息,相关结构:LOGFONT")
        IDSTR(WM_CHOOSEFONT_SETFLAGS, "设置字体通用对话框的显示选项,相关结构:CHOOSEFONT")
        IDSTR(WM_CHOOSEFONT_SETLOGFONT, "设置字体通用对话框中用户当前字体信息,相关结构:LOGFONT")
        //Page Setup dialog,页面设置通用对话框
        IDSTR(WM_PSD_ENVSTAMPRECT, "通知钩子函数:页面设置对话框要在信封样本页上绘制邮票区")
        IDSTR(WM_PSD_FULLPAGERECT, "通知PagePaintHook钩子函数:页面设置对话框上样本页的整个页面区的坐标")
        IDSTR(WM_PSD_GREEKTEXTRECT, "通知钩子函数:页面设置对话框要在样本页上绘制页面内容")
        IDSTR(WM_PSD_MARGINRECT, "通知钩子函数:页面设置对话框将要绘制样本页样本页的的当前边距")
        IDSTR(WM_PSD_MINMARGINRECT, "通知PagePaintHook钩子函数:页面设置对话框上样本页的当前最小边距")
        IDSTR(WM_PSD_PAGESETUPDLG, "页面设置对话框将要绘制样本页,钩子函数利用此消息准备供给样本页里的内容")
        IDSTR(WM_PSD_YAFULLPAGERECT, "页面设置对话框将要绘制信封样本页的返回地址部分,此消息仅发向信封和其它纸张大小")
        //Open or Save As dialog box,打开保存为通用对话框
        IDSTR(CDM_GETFILEPATH, "获得打开保存为通用对话框中被选择文件的路径和文件名")
        IDSTR(CDM_GETFOLDERIDLIST, "从打开保存为通用对话框获得当前文件夹的PIDL(外壳对象标志符列表),注:外壳编程中,要使用PIDL路径代替普通路径")
        IDSTR(CDM_GETFOLDERPATH, "获得打开保存为通用对话框中当前打开文件夹的路径")
        IDSTR(CDM_GETSPEC, "获得打开保存为通用对话框中被选择文件的文件名(不含路径)")
        IDSTR(CDM_HIDECONTROL, "隐藏打开保存为通用对话框(OFN_EXPLORER)中的指定控件")
        IDSTR(CDM_SETCONTROLTEXT, "为保存为通用对话框(OFN_EXPLORER)中的指定控件设置文本")
        IDSTR(CDM_SETDEFEXT, "为保存为通用对?翱?OFN_EXPLORER)中设置缺省文件扩展名")
        //Open or Save As dialog box can send to a hook procedure,打开另存为通用对话框的HOOK相关消息
        IDSTR(CDN_FILEOK, "打开另存为通用对话框向OFNHookProc钩子函数发送确定按钮被按下的消息")
        IDSTR(CDN_FOLDERCHANGE, "打开另存为通用对话框向OFNHookProc钩子函数发送一个新文件夹被打开的消息")
        IDSTR(CDN_HELP, "打开另存为通用对话框中的帮助按钮被按下时发送本消息到OFNHookProc钩子函数")
        IDSTR(CDN_INITDONE, "系统已完成在打开另存为通用对话框中排列控件(为子对话框的控件腾出空间)时发送本消息到OFNHookProc钩子函数")
        IDSTR(CDN_SELCHANGE, "用户在打开另存为对话框中的文件列表里选择了一个新文件或文件夹时发送本消息到OFNHookProc钩子函数")
        IDSTR(CDN_SHAREVIOLATION, "当用户选择的文件名发生网络共享冲突时打开另存为对话框发送本消息(共享违例)到OFNHookProc钩子函数")
        IDSTR(CDN_TYPECHANGE, "打开另存为对话框中从文件类型列表中选择一个新文件类型时发送本消息到OFNHookProc钩子函数")
        //Dialog box default push button,对话框缺省按钮消息
        IDSTR(DM_GETDEFID, "获取对话框中缺省按钮的ID号")
        IDSTR(DM_REPOSITION, "使一个(顶层窗口且非chind窗口)对话框回到桌面(屏幕)中,使整个对话框可视")
        IDSTR(DM_SETDEFID, "改变对话框中缺省按钮的ID号")
        //TAPI:电话应用程序接口消息
        //Remote Access Service Messages,RAS(远程访问服务)消息,主要用于远程拨号(ADSL宽带拨号等)和VPN(虚拟专用网络)
        IDSTR(WM_RASDIALEVENT, "拨号事件通知消息,RAS连接期间状态事件发生改变时,系统发送本消息到相关窗口")
        //输入法编辑器消息
        IDSTR(IMC_CLOSESTATUSWINDOW, "发送给IME窗口,隐藏状态窗口")
        IDSTR(IMC_GETCANDIDATEPOS, "发送给IME窗口,获取候选窗口的位置")
        IDSTR(IMC_GETCOMPOSITIONFONT, "发送给IME窗口,获取用来显示按键组合窗口中的文本的逻辑字体")
        IDSTR(IMC_GETCOMPOSITIONWINDOW, "发送给IME窗口,获取按键组合窗口的位置")
        IDSTR(IMR_COMPOSITIONWINDOW, "通知:选定的IME需要应用程序提供有关按键组合窗口的信息")
        IDSTR(IMR_DOCUMENTFEED, "通知:选定的IME需要从应用程序那里取得已转换的字符串")
        IDSTR(IMR_QUERYCHARPOSITION, "通知:选定的IME需要应用程序提供有关组合字符串中某个字符的位置信息")
        IDSTR(IMR_COMPOSITIONFONT, "通知:选定的IME需要应用程序提供有关用在按键组合窗口中的字体信息")
        IDSTR(IMR_CONFIRMRECONVERTSTRING, "通知:选定的IME需要应用程序提供有关组合字符串中某个字符的位置信息")
        IDSTR(IMR_RECONVERTSTRING, "通知:选定的IME需要应用程序提供一个用于自动更正的字符串")
        IDSTR(IMR_CANDIDATEWINDOW, "通知:选定的IME需要应用程序提供有关候选窗口的信息")
        //IDSTR(IMC_GETOPENSTATUS),"")
        //IDSTR(IMC_GETSENTENCEMODE),"")
        IDSTR(IMC_GETSTATUSWINDOWPOS, "发送给IME窗口,获取状态窗口的位置")
        IDSTR(IMC_OPENSTATUSWINDOW, "发送给IME窗口,显示状态窗口")
        IDSTR(IMC_SETCANDIDATEPOS, "发送给IME窗口,设置候选窗口的位置")
        IDSTR(IMC_SETCOMPOSITIONFONT, "发送给IME窗口,设置用来显示按键组合窗口中的文本的逻辑字体")
        IDSTR(IMC_SETCOMPOSITIONWINDOW, "发送给IME窗口,设置按键组合窗口的样式")
        //IDSTR(IMC_SETCONVERSIONMODE),"")
        //IDSTR(IMC_SETOPENSTATUS),"")
        //IDSTR(IMC_SETSENTENCEMODE),"")
        IDSTR(IMC_SETSTATUSWINDOWPOS, "发送给IME窗口,设置状态窗口的位置")
        IDSTR(IMN_CHANGECANDIDATE, "IME通知应用程序:候选窗口中的内容将改变")
        IDSTR(IMN_CLOSECANDIDATE, "IME通知应用程序:候选窗口将关闭")
        IDSTR(IMN_CLOSESTATUSWINDOW, "IME通知应用程序:状态窗口将关闭")
        IDSTR(IMN_GUIDELINE, "IME通知应用程序:将显示一条出错或其他信息")
        IDSTR(IMN_OPENCANDIDATE, "IME通知应用程序:将打开候选窗口")
        IDSTR(IMN_OPENSTATUSWINDOW, "IME通知应用程序:将创建状态窗口")
        IDSTR(IMN_SETCANDIDATEPOS, "IME通知应用程序:已结束候选处理同时将移动候选窗口")
        IDSTR(IMN_SETCOMPOSITIONFONT, "IME通知应用程序:输入内容的字体已被更改")
        IDSTR(IMN_SETCOMPOSITIONWINDOW, "IME通知应用程序:按键组合窗口的样式或位置已被更改")
        IDSTR(IMN_SETCONVERSIONMODE, "IME通知应用程序:输入内容的转换模式已被更改")
        IDSTR(IMN_SETOPENSTATUS, "IME通知应用程序:输入内容的状态已被更改")
        IDSTR(IMN_SETSENTENCEMODE, "IME通知应用程序:输入内容的语句模式已被更改")
        IDSTR(IMN_SETSTATUSWINDOWPOS, "IME通知应用程序:输入内容中的状态窗口的位置已被更改")
        //IDSTR(IMR_CANDIDATEFORM),"")
        //IDSTR(IMR_COMPOSITIONFORM),"")
        //Pager Control,Pager控件消息,分页控件消息
        IDSTR(PGM_FORWARDMOUSE, "启用或禁用Pager分页控件的鼠标消息转发,若启用则Pager控件转发WM_MOUSEMOVE消息给包含的窗口,宏:Pager_ForwardMouse")
        IDSTR(PGM_GETBKCOLOR, "获取Pager分页控件的当前背景色,宏:Pager_GetBkColor")
        IDSTR(PGM_GETBORDER, "获取Pager分页控件的当前边界尺寸,宏:Pager_GetBorder")
        IDSTR(PGM_GETBUTTONSIZE, "获取Pager分页控件的当前按钮大小,宏:Pager_GetButtonSize")
        IDSTR(PGM_GETBUTTONSTATE, "获取Pager分页控件中指定按钮的状态,宏:Pager_GetButtonState")
        IDSTR(PGM_GETDROPTARGET, "获取Pager分页控件的IDropTarget接口指针,宏:Pager_GetDropTarget")
        IDSTR(PGM_GETPOS, "获取Pager分页控件的滚动条的当前位置,宏:Pager_GetPos")
        IDSTR(PGM_RECALCSIZE, "强制重新计算Pager分页控件包含的窗口的大小,宏:Pager_RecalcSize")
        IDSTR(PGM_SETBKCOLOR, "设置Pager分页控件的当前背景色,宏:Pager_SetBkColor")
        IDSTR(PGM_SETBORDER, "设置Pager分页控件的当前边界尺寸,宏:Pager_SetBorder")
        IDSTR(PGM_SETBUTTONSIZE, "设置Pager分页控件的当前按钮大小,宏:Pager_SetButtonSize")
        IDSTR(PGM_SETCHILD, "设置Pager分页控件中包含的窗口,宏:Pager_SetChild")
        IDSTR(PGM_SETPOS, "获取Pager分页控件的滚动条的当前位置,宏:Pager_SetPos")
        //Pager Control,Pager控件通知消息
        IDSTR(PGN_CALCSIZE, "Pager控件发送的通知消息,用以获得所包含窗口的滚动尺寸,相关结构:NMPGCALCSIZE")
        IDSTR(PGN_SCROLL, "在所包含窗口被滚动之前,Pager控件发送的通知消息,相关结构:NMPGSCROLL")
        //MIDI消息
        IDSTR(MIM_CLOSE, "当MIDI输入设备被关闭时,把该消息发送给MIDI输入?氐骱?)
            IDSTR(MIM_DATA, "当MIDI输入设备接收一条MIDI消息后, 把该消息发送给MIDI输入回调函数")
            IDSTR(MIM_ERROR, "当收到一条无效的MIDI消息时, 把该消息发送给MIDI输入回调函数")
            IDSTR(MIM_LONGDATA, "当输入缓冲区中填写了MIDI系统专用数据且将把该缓冲区交还给应用程序时, 发送该消息给MIDI回调函数")
            IDSTR(MIM_LONGERROR, "当收到一条无效的MIDI系统专用消息时, 把该消息发送给一个MIDI输入回调函数")
            IDSTR(MIM_MOREDATA, "当MIDI输入设备收到一条MIDI消息, 但应用程序处理MIM_DATA消息不够快跟不上输入设备时, 发送该消息给MIDI输入回调函数")
            IDSTR(MIM_OPEN, "当MIDI输入设备被打开时, 把该消息发送给MIDI输入回调函数")
            IDSTR(MM_MIM_CLOSE, "当关闭某MIDI输入设备时, 发送该消息给一个窗口。一旦发送了该消息, 设备句柄将不再有效")
            IDSTR(MM_MIM_DATA, "当MIDI输入设备收到一条完整的MIDI消息时, 发送该消息给一个窗口")
            IDSTR(MM_MIM_ERROR, "当收到一条无效的MIDI消息时, 把该消息发给一个窗口")
            IDSTR(MM_MIM_LONGDATA, "当输入缓冲区中填写了MIDI系统专用数据且将把该缓冲区交还给应用程序时, 发送该消息给一个窗口")
            IDSTR(MM_MIM_LONGERROR, " 当收到一条无效的(或不完整的)MIDI系统专用消息时, 发送该消息给一个窗口")
            IDSTR(MM_MIM_MOREDATA, "当MIDI输入设备收到一条MIDI消息, 但应用程序处理MIM_DATA消息不够快跟不上输入设备时, 发送该消息给一个回调窗口")
            IDSTR(MM_MIM_OPEN, "当MIDI输入设备被打开时, 发送该消息给一个窗口")
            IDSTR(MM_MOM_CLOSE, "当MIDI输出设备被关闭时, 发送该消息给一个窗口; 一旦发送了这一消息, 设备句柄将不再有效")
            IDSTR(MM_MOM_DONE, "当指定的系统专用缓冲区中的内容被播放完, 并且将把该缓冲区交还给应用程序时, 发送该消息给一个MIDI回调函数")
            IDSTR(MM_MOM_OPEN, "当MIDI输出设备被打开时, 发送该消息给一个窗口")
            IDSTR(MM_MOM_POSITIONCB, "当在MIDI输出流中一个MEVT_F_CALLBACK(标志)事件到达时, 发送该消息给一个窗口")
            IDSTR(MOM_CLOSE, "当MIDI输出设备被关闭时, 把该消息发送给MIDI输出回调函数")
            IDSTR(MOM_DONE, "当指定的专用系统或流缓冲区已经播放, 并且正在返回到应用程序时, 发送该信息到MIDI输出回调函数")
            IDSTR(MOM_OPEN, "当MIDI输出设备被打开时, 把该消息发送给MIDI输出回调函数")
            IDSTR(MOM_POSITIONCB, "当在MIDI输出流中一个MEVT_F_CALLBACK(标志)事件到达时, 发送该消息给一个窗口")
            //驱动程序消息
            IDSTR(DRV_CLOSE, "驱动程序因CloseDriver调?枚乇帐? 系统向它发出本消息, 驱动程序常使用DRV_CLOSE通知来释放特定实例的数据结构")
            IDSTR(DRV_CONFIGURE, "驱动程序完成初始化安装及每当用户要重新配置驱动程序时, 驱动安装程序会发送该消息")
            IDSTR(DRV_DISABLE, "驱动程序被释放或由Windows进入DOS时, 向驱动程序发DRV_DISABLE消息")
            IDSTR(DRV_ENABLE, "当驱动程序首次加载后, 系统向它发DRV_ENABLE消息, 利用该消息设置硬件并挂接中断")
            IDSTR(DRV_EXITSESSION, "系统退出前, 向所有打开的驱动程序发出该消息")
            IDSTR(DRV_FREE, "驱动程序被丢弃前, 系统发送给它的最后一个消息就是DRV_FREE")
            IDSTR(DRV_INSTALL, "驱动程序安装过程中, 它将收到该消息")
            IDSTR(DRV_LOAD, "驱动程序首次加载后, 系统发送给它的就是DRV_LOAD消息, 此时可初始化实例数据")
            IDSTR(DRV_OPEN, "驱动程序被OpenDriver调用打开时, 系统向它发该消息")
            IDSTR(DRV_POWER, "在符合APM标准的系统中, 当系统将进入或退出挂起模式时, 驱动程序将收到DRV_PORWER消息")
            IDSTR(DRV_QUERYCONFIGURE, "驱动程序的安装程序利用该消息来确定驱动程序是否能由用户配置。若该消息返回TURE, 安装程序会使能\"configure\"或\"Setup\"按钮")
        IDSTR(DRV_REMOVE, "驱动程序将从安装的驱动程序列表中删除时,将会收到该消息")
        IDSTR(DRV_USER, "用户自定义的驱动程序消息的消息值范围从DRV_RESERVED开始到DRV_USER")
        IDSTR(DRV_EXITAPPLICATION, "应用程序退出时,系统向驱动程序发出该消息")
        IDSTR(DRV_RESERVED, "用户自定义的驱动程序消息的消息值范围从DRV_RESERVED开始到DRV_USER")
        //Power Management,电源管理消息,PBT_:Power BaTtery
        IDSTR(PBT_APMBATTERYLOW, "该消息发送到应用程序,通知BIOS的电池电量不足,窗口通过WM_POWERBROADCAST收到此消息")
        IDSTR(PBT_APMOEMEVENT, "高级电源管理的基本输入/输出系统 (BIOS)发出高级电源管理的OEM事件信号时(APM:高级电源管理),发送本消息,窗口通过WM_POWERBROADCAST收到此消息,注:这不是个统一事件,各OEM厂商会根据自己的判断尝试捕捉各自事件")
        IDSTR(PBT_APMPOWERSTATUSCHANGE, "计算机电源状态改变时,发送本消息,如:检测到电池电源切换A/C或低于一个阈值后的剩余电池电量的下降,窗口通过WM_POWERBROADCAST收到此消息")
        IDSTR(PBT_APMQUERYSUSPEND, "发送该消息以请求允许挂起计算机,授予权限的应用程序应在返回前挂起,窗口通过WM_POWERBROADCAST收到此消息")
        IDSTR(PBT_APMQUERYSUSPENDFAILED, "待机请求被拒绝时,发送该消息,窗口通过WM_POWERBROADCAST收到此消息")
        IDSTR(PBT_APMRESUMEAUTOMATIC, "计算机自动唤醒以处理某事件时,发送该消息,窗口通过WM_POWERBROADCAST收到此消息")
        IDSTR(PBT_APMRESUMECRITICAL, "由于BIOS电池问题使计算机被挂起后恢复操作时,发送该消息,驱动程序通过WM_POWERBROADCAST收到此消息")
        IDSTR(PBT_APMRESUMESUSPEND, "系统待机后恢复运行时,发送该消息,窗口通过WM_POWERBROADCAST收到此消")
        IDSTR(PBT_APMSUSPEND, "系统进入待机后,会立即发送该消息,窗口通过WM_POWERBROADCAST收到此消息")
        //setup functions,驱动安装函数消息
        IDSTR(SPFILENOTIFY_COPYERROR, "驱动安装过程中,文件拷贝发生错误时,该通知发送给回调例程")
        IDSTR(SPFILENOTIFY_DELETEERROR, "驱动安装过程中,文件删除操作发生错误时,该通知发送给回调例程")
        IDSTR(SPFILENOTIFY_ENDCOPY, "驱动安装过程中,队列完成复制操作时,该通知传递给回调函数(即使用户取消或有错误发生,该通知也被发送)")
        IDSTR(SPFILENOTIFY_ENDDELETE, "驱动安装过程中,当队列完成删除操作时,该通知被返回给回调例程(即使用户取消或有错误发生,该通知也被发送)")
        ...
        // IDSTR(LINE_ADDRESSSTATE),"由应用程序当前已打开的线路的地址状态发生改变时,发送本消息。应用程序可调用lineGetAddressStatus函数以确定该地址的当前状态")
        // IDSTR(LINE_AGENTSPECIFIC),"由应用程序当前已打开的线路的ACD代理的身份发生改变时,发?捅鞠ⅰＳτ贸绦蚩傻饔胠ineGetAgentStatus去确定代理的当前身份")
        // IDSTR(LINE_AGENTSTATUS),"由应用程序当前已打开的线路的ACD代理的身份发生改变时,发送本消息。应用程序可调用lineGetAgentStatus去确定代理的当前身份")
        // IDSTR(LINE_APPNEWCALL),"有呼叫进来(有新来电)")
        // IDSTR(LINE_CALLDEVSPECIFIC),"")
        // IDSTR(LINE_CALLDEVSPECIFICFEATURE),"")
        // IDSTR(LINE_CALLINFO),"呼叫信息,可获取来电去电的很多信息")
        // IDSTR(LINE_CALLSTATE),"呼叫状态(正在拨出/断线/来电/拨通等)")
        // IDSTR(LINE_CLOSE),"")
        // IDSTR(LINE_CREATE),"")
        ...
        // IDSTR(LINECALLSTATE_DISCONNECTED),"自己挂断或无响应")
        // IDSTR(LINECALLINFOSTATE_OTHER),"对方挂断")
        // IDSTR(LINECALLSTATE_DIALING),"正在拨出")
        //
        // IDSTR(PHONE_BUTTON),"")
        // IDSTR(PHONE_CLOSE),"")
        // IDSTR(PHONE_CREATE),"")
