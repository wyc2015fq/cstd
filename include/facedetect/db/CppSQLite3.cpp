...
int CppSQLite3Binary::setEncoded(const unsigned char* pBuf)
{
    clear();
    ...
    if (!mpBuf)
    {
        return (CPPSQLITE_ERROR);// "Cannot allocate memory", DONT_DELETE_MSG;
    }
...
const unsigned char* CppSQLite3Binary::getBinary()
{
    if (mbEncoded)
    {
        ...
        if (mnBinaryLen == -1)
        {
            return NULL;//(CPPSQLITE_ERROR, "Cannot decode binary", DONT_DELETE_MSG);
        }
...
unsigned char* CppSQLite3Binary::allocBuffer(int nLen)
{
    ...
    if (!mpBuf)
    {
        CppSQLite3Exception(CPPSQLITE_ERROR, "Cannot allocate memory", DONT_DELETE_MSG);
        return NULL;
    }
...
const char* CppSQLite3Query::fieldValue(int nField) const
{
    ...
    {
        CppSQLite3Exception(CPPSQLITE_ERROR,
...
int64 CppSQLite3Query::getInt64Field(int nField, int64 nNullValue/*=0*/) const
{
    if (fieldDataType(nField) == SQLITE_NULL)
    {
        return nNullValue;
...
int64 CppSQLite3Query::getInt64Field(const char* szField, int64 nNullValue/*=0*/) const
{
    int nField = fieldIndex(szField);
...
int CppSQLite3Query::fieldIndex(const char* szField) const
{
    ...
    return CppSQLite3Exception(CPPSQLITE_ERROR,
...
int CppSQLite3Query::finalize()
{
    if (mpVM && mbOwnVM)
    {
        ...
        {
            const char* szError = sqlite3_errmsg(mpDB);
            // return CppSQLite3Exception(nRet, (char*)szError, DONT_DELETE_MSG);
            return nRet;
        }
    }
}
...
CppSQLite3Query CppSQLite3Statement::execQuery()
{
    ...
    else
    {
        nRet = sqlite3_reset(mpVM);
        const char* szError = sqlite3_errmsg(mpDB);
        CppSQLite3Exception(nRet, (char*)szError, DONT_DELETE_MSG);
...
int CppSQLite3DB::execDML(const char* szSQL)
{
    ...
    {
        return CppSQLite3Exception(nRet, szError);
    }
}
...
CppSQLite3Table CppSQLite3DB::getTable(const char* szSQL)
{
    ...
    {
        CppSQLite3Exception(nRet, szError);
