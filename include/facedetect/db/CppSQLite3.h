...
#else
static int CppSQLite3Exception(int code, const char* mpszErrMess, bool bDeleteMsg = false) {
  printf("error: %s\n", mpszErrMess);
  if (bDeleteMsg && mpszErrMess)
  {
    sqlite3_free((void*)mpszErrMess);
    mpszErrMess = 0;
  }
  return code;
}
...
public:
    ...
    int setEncoded(const unsigned char* pBuf);
...
public:
    ...
    int64 getInt64Field(int nField, int64 nNullValue=0) const;
    int64 getInt64Field(const char* szField, int64 nNullValue=0) const;
