...
struct _jsfeat {
    ...
    // box blur option
	static const unsigned short BOX_BLUR_NOSCALE = 0x01;
    // svd options
	static const unsigned short SVD_U_T = 0x01;
	static const unsigned short SVD_V_T = 0x02;
    ...
    struct data_t {
		...
		data_t(int size_in_bytes) {
			this->next = NULL;
			this->resize(size_in_bytes);
		}
        void resize(uint32 new_size, void* buffer = NULL) {
            // we need align size to multiple of 8
			// clear references
			if (new_size > this->size) {
				this->size = ((new_size + 7) | 0) & -8;
	...
	*/
	...
	struct _cache {
		...
		typedef data_t _pool_node_t;

		data_t* _pool_head, *_pool_tail;
		int _pool_size = 0;

		void allocate(int capacity, int data_size) {
			_pool_head = _pool_tail = new _pool_node_t(data_size);
			for (int i = 0; i < capacity; ++i) {
				_pool_node_t* node = new _pool_node_t(data_size);
				_pool_tail = _pool_tail->next = node;
				_pool_size++;
			}
		}

		data_t& get_buffer(uint32 size_in_bytes) {
...
struct _math_t {
    int32 qsort_stack[48*2];

    void get_gaussian_kernel(int size, double sigma, data_t kernel, int data_type) {
		int i = 0;
		double x = 0.0, t = 0.0, sigma_x = 0.0, scale_2x = 0.0, sum = 0.0;
        data_t& kern_node = jsfeat.cache.get_buffer(size<<2);
        float* _kernel = kern_node.f32;//new Float32Array(size);
        ...
        if(data_type & jsfeat.U8_t) {
            ...
            sum = 256.0/sum;
            for (i = 0; i < size; ++i) {
                kernel.i32[i] = (_kernel[i] * sum + 0.5);
            }
        } else {
            // classic kernel
            sum = 1.0/sum;
            for (i = 0; i < size; ++i) {
                kernel.f32[i] = _kernel[i] * sum;
            }
        }
	...
	template <typename T>
    void perspective_4point_transform(T model,
        ...
        while( sp >= 0 ) {
            ...
            for(;;) {
                ...
                //insert_sort:
                    ...
                    n = Math_min( (left1 - left0), (left - left1) );
                    m = (left-n);
                    for( i = 0; i < n; ++i,++m ) {
                        t = array[left0+i];
                    ...
                    n = Math_min( (right0 - right1), (right1 - right) );
                    m = (right0-n+1);
                    for( i = 0; i < n; ++i,++m ) {
                        t = array[left+i];
	...
	template <typename T>
    T& median(T* array, int low, int high) {
        T w;
    ...
    var matmath = (function() {
        ...
        return {
            identity: function(M, value) {
                ...
                var src=M.data;
                var rows=M.rows, cols=M.cols, cols_1=(cols+1);
                var len = rows * cols;
                var k = len;
...
(function(global) {
    ...
    var linalg = (function() {
        ...
        var JacobiSVDImpl = function(At, astep, _W, Vt, vstep, m, n, n1) {
            ...
            var i=0,j=0,k=0,iter=0,max_iter=Math_max(m, 30);
...
(function(global) {
    ...
    var ransac_params_t = (function () {
        ...
        ransac_params_t.prototype.update_iters = function(_eps, max_iters) {
	        ...
	        return (denom >= 0 || -num >= max_iters*(-denom) ? max_iters : Math.round(num/denom));
        };
    	...
    	var get_subset = function(kernel, from, to, need_cnt, max_cnt, from_sub, to_sub) {
		    ...
		    for(; ssiter < max_try; ++ssiter)  {
		        ...
		        for (; i < need_cnt && ssiter < max_try;) {
		            ...
		            while (!ok) {
		                ok = true;
		                idx_i = indices[i] = Math.floor(Math.random() * max_cnt);
		                for (j = 0; j < i; ++j) {
		                    if (idx_i == indices[j])
		                    { ok = false; break; }
		                }
    	...
    	return {
    		...
    		ransac: function(params, kernel, from, to, count, model, mask, max_iters) {
			    ...
			    for (; iter < niters; ++iter) {
			        ...
			        if( numinliers > Math_max(inliers_max, model_points-1) ) {
			            M.copy_to(model);
...
struct _imgproc {
     void _resample_u8(const matrix_t& src, matrix_t& dst, int nw, int nh) {
		...
		double fsx1=0.0,fsx2=0.0, alpha = 0, beta = 0, beta1 = 0;
        ...
        int32* buf = buf_node.i32;
		int32* sum = sum_node.i32;
		int32* xofs = xofs_node.i32;

        for (; dx < nw; dx++) {
            fsx1 = dx * scale_x, fsx2 = fsx1 + scale_x;
            sx1 = (fsx1 + 1.0 - 1e-6), sx2 = fsx2;
            sx1 = Math_min(sx1, w - 1);
            sx2 = Math_min(sx2, w - 1);
        ...
        for (sy = 0; sy < h; sy++) {
            ...
            if ((dy + 1) * scale_y <= sy + 1 || sy == h - 1) {
                beta = (Math_max(sy + 1 - (dy + 1) * scale_y, 0.0) * 256);
                beta1 = 256 - beta;
                b = nw * dy;
                if (beta <= 0) {
                    for (dx = 0; dx < nw * ch; dx++) {
                        dst_d[b+dx] = Math_min(Math_max((sum[dx] + buf[dx] * 256) / inv_scale_256, 0.), 255);
                        sum[dx] = buf[dx] = 0;
                    }
                } else {
                    for (dx = 0; dx < nw * ch; dx++) {
                        dst_d[b+dx] = Math_min(Math_max((sum[dx] + buf[dx] * beta1) / inv_scale_256, 0.), 255);
                        sum[dx] = buf[dx] * beta;
                        buf[dx] = 0;
                    }
                }
    ...
    void _resample(const matrix_t& src, matrix_t& dst, int nw, int nh) {
        ...
        for (sy = 0; sy < h; sy++) {
            ...
            if ((dy + 1) * scale_y <= sy + 1 || sy == h - 1) {
                beta = Math_max(sy + 1 - (dy + 1) * scale_y, 0.0);
                beta1 = 1.0 - beta;
                b = nw * dy;
    ...
    void _convol_u8(int* buf, const uint8* src_d, uint8* dst_d, int w, int h, const int* filter, int kernel_size, int half_kernel) {
        int i=0,j=0,k=0,sp=0,dp=0,sum=0,sum1=0,sum2=0,sum3=0,f0=filter[0],fk=0;
        ...
        for (; i < h; ++i) { 
            ...
            for (j = 0; j <= w-4; j+=4) {
                ...
                dst_d[dp+j] = Math_min(sum >> 8, 255);
                dst_d[dp+j+1] = Math_min(sum1 >> 8, 255);
                dst_d[dp+j+2] = Math_min(sum2 >> 8, 255);
                dst_d[dp+j+3] = Math_min(sum3 >> 8, 255);
            }
            for (; j < w; ++j) {
                sum = buf[j] * f0;
                for (k = 1; k < kernel_size; ++k) {
        ...
        for (i = 0; i < w; ++i) {
            ...
            for (j = 0; j <= h-4; j+=4, dp+=w4) { 
                ...
                dst_d[dp] = Math_min(sum >> 8, 255);
                dst_d[dp+w] = Math_min(sum1 >> 8, 255);
                dst_d[dp+w2] = Math_min(sum2 >> 8, 255);
                dst_d[dp+w3] = Math_min(sum3 >> 8, 255);
            }
            for (; j < h; ++j, dp+=w) {
                sum = buf[j] * f0;
                for (k = 1; k < kernel_size; ++k) {
    ...
    void _convol(float* buf, const float* src_d, float* dst_d, int w, int h, const float* filter, int kernel_size, int half_kernel) {
		int i = 0, j = 0, k = 0, sp = 0, dp = 0;
        ...
        // for raw arrays
       void grayscale(const uchar* src, int w, int h, matrix_t& dst, color_t code) {
            // this is default image data representation in browser
            if (code == COLOR_UNDEFINED) { code = jsfeat.COLOR_RGBA2GRAY; }
            int x=0, y=0, i=0, j=0, ir=0,jr=0;
            ...
            int cn2 = cn<<1, cn3 = (cn*3);

            dst.resize(w, h, 1);
            uchar* dst_u8 = dst.data;
        ...
        void box_blur_gray(const matrix_t& src, matrix_t&dst, int radius, int options = 0) {
            if (options <= 0) { options = 0; }
            int w=src.cols, h=src.rows, h2=h<<1, w2=w<<1;
            int i=0,x=0,y=0,end=0;
            int windowSize = ((radius << 1) + 1);
            int radiusPlusOne = (radius + 1), radiusPlus2 = (radiusPlusOne+1);
            double scale = options&jsfeat.BOX_BLUR_NOSCALE ? 1 : (1.0 / (windowSize*windowSize));

            data_t& tmp_buff = jsfeat.cache.get_buffer((w*h)<<2);

            int sum=0, dstIndex=0, srcIndex = 0, nextPixelIndex=0, previousPixelIndex=0;
            int32* data_i32 = tmp_buff.i32; // to prevent overflow
        ...
        void gaussian_blur(const matrix_t& src, matrix_t& dst, int kernel_size, double sigma = 0) {
            if (sigma <= 0) { sigma = 0.0; }
            if (kernel_size == 0) { kernel_size = 0; }
            kernel_size = kernel_size == 0 ? (Math_max(1, (4.0 * sigma + 1.0 - 1e-8)) * 2 + 1) : kernel_size;
			...
			//int* buf;
			//float* filter;
			int buf_sz = (kernel_size + Math_max(h, w)) | 0;

            data_t& buf_node = jsfeat.cache.get_buffer(buf_sz<<2);
			data_t& filt_node = jsfeat.cache.get_buffer(kernel_size<<2);
			data_t& buf = buf_node;
			data_t& filter = filt_node;
#if 0
            if(is_u8) {
                buf = buf_node.i32;
            ...
            if(is_u8) {
                _convol_u8(buf.i32, src.u8, dst.u8, w, h, filter.i32, kernel_size, half_kernel);
            } else {
                _convol(buf.f32, src.f32, dst.f32, w, h, filter.f32, kernel_size, half_kernel);
            }
...
#if 0
        void hough_transform( matrix_t& lines, const matrix_t& img, double rho_res, double theta_res, double threshold ) {
            ...
            double max_theta = Math.PI;

            int numangle = Math.round((max_theta - min_theta) / theta_res);
			int numrho = Math.round(((width + height) * 2 + 1) / rho_res);
            ...
            linesMax = Math_min(numangle*numrho, _sort_buf.length);
            scale = 1.0 / (numrho+2);
            lines = new Array();
        ...
        canny: function(src, dst, low_thresh, high_thresh) {
            ...
            var row0=1,row1=(w+2+1),row2=(2*(w+2)+1),map_w=(w+2),map_i=(map_w+1),stack_i=0;
        ...
        warp_perspective: function(src, dst, transform, fill_value) {
            ...
            var src_width=src.cols, src_height=src.rows, dst_width=dst.cols, dst_height=dst.rows|0;
            var src_d=src.data, dst_d=dst.data;
            ...
            for(var dptr = 0; y < dst_height; ++y) {
                ...
                for(x = 0; x < dst_width; ++x, ++dptr, xs0+=m00, ys0+=m10, ws+=m20) {
                    ...
                    if(xs > 0 && ys > 0 && ixs < (src_width - 1) && iys < (src_height - 1)) {
                        a = Math_max(xs - ixs, 0.0);
                        b = Math_max(ys - iys, 0.0);
    ...
    var fast_corners = (function() {
        ...
        _cmp_score_16 = function(src, off, pixel, d, threshold) {
            ...
            for( k = 0; k < 16; k += 2 ) {
                ...
                a = Math_min(a, d[k+7]);
                a = Math_min(a, d[k+8]);
                a0 = Math_max(a0, Math_min(a, d[k]));
                a0 = Math_max(a0, Math_min(a, d[k+9]));
            }

            b0 = -a0;
            for( k = 0; k < 16; k += 2 ) {
                b = Math_max(d[k+1], d[k+2]);
                b = Math_max(b, d[k+3]);
                b = Math_max(b, d[k+4]);
                b = Math_max(b, d[k+5]);
                ...
                b = Math_max(b, d[k+7]);
                b = Math_max(b, d[k+8]);
                b0 = Math_min(b0, Math_max(b, d[k]));
                b0 = Math_min(b0, Math_max(b, d[k+9]));
            }
        ...
        return {
            set_threshold: function(threshold) {
                _threshold = Math_min(Math_max(threshold, 0), 255);
            ...
            detect: function(src, corners, border) {
                ...
                var pixel = pixel_off;
                var sd = score_diff;
                var sy = Math_max(3, border);
                var ey = Math_min((h-2), (h-border));
                var sx = Math_max(3, border);
                var ex = Math_min((w - 3), (w - border));
...
(function(global) {
    ...
    var yape06 = (function() {
        ...
        var compute_laplacian = function(src, dst, w, h, Dxx, Dyy, sx,sy, ex,ey) {
            var y=0,x=0,yrow=(sy*w+sx),row=yrow;
        ...
        var hessian_min_eigen_value = function(src, off, tr, Dxx, Dyy, Dxy, Dyx) {
            ...
            return Math_min(Math.abs(tr - sqrt_delta), Math.abs(-(tr + sqrt_delta)));
        }
        ...
        return {
            ...
            detect: function(src, points, border) {
                ...
                var sx = Math_max(5, border)|0;
                var sy = Math_max(3, border)|0;
                var ex = Math_min(w-5, w-border)|0;
                var ey = Math_min(h-3, h-border)|0;
    ...
    var yape = (function() {
        ...
        return {
            ...
            init: function(width, height, radius, pyramid_levels) {
                ...
                var i;
                radius = Math_min(radius, 7);
                radius = Math_max(radius, 3);
            ...
            detect: function(src, points, border) {
                ...
                var sx = Math_max(R+1, border)|0;
                var sy = Math_max(R+1, border)|0;
                var ex = Math_min(w-R-2, w-border)|0;
                var ey = Math_min(h-R-2, h-border)|0;
    ...
    var haar = (function() {
        ...
        return {
            ...
            detect_single_scale: function(int_sum, int_sqsum, int_tilted, int_canny_sum, width, height, scale, classifier) {
                var win_w = (classifier.size[0] * scale),
                    win_h = (classifier.size[1] * scale),
                    step_x = (0.5 * scale + 1.5),
                    step_y = step_x;
                var i,j,k,x,y,ex=(width-win_w),ey=(height-win_h)|0;
                var w1=(width+1),edge_dens,mean,variance,std;
            ...
            group_rectangles: function(rects, min_neighbors) {
                ...
                for(i = 0; i < n; ++i) {
                    ...
                    comps[idx].confidence = Math_max(comps[idx].confidence, r1.confidence);
                }
                ...
                for(i = 0; i < n; ++i) {
                    ...
                    for(j = 0; j < n; ++j) {
                        ...
                        if(i != j &&
                           ...
                           (r2.neighbors > Math_max(3, r1.neighbors) || r1.neighbors < 3)) {
                            flag = false;
                            break;
                        }
                    }
    ...
    var bbf = (function() {
        ...
        return {
            ...
            build_pyramid: function(src, min_width, min_height, interval) {
                ...
                this->scale_to = (Math.log(Math_min(sw / min_width, sh / min_height)) / Math.log(this->scale))|0;
...
_jsfeat _jsfeat::jsfeat;
_jsfeat& jsfeat = _jsfeat::jsfeat;
