...
static uint32 rng_mt19937_gen_int32(rng_t* r) {
  mt19937ar_t* mt = r->uniform.mt;
  return mt19937ar_int32(mt);
}
...
static uint32 rng_lcg64_gen_int32(rng_t* r) {
  return lcg64_int32(&r->uniform.seed64);
}
...
static double rng_normal(rng_t* r, double _Mean, double _Sigma) {
  ...
  bool _Keep = false;
  RNG_STATIC(r);
  if (_Keep && r->normal.polar._Valid) {	// return stored value
    _Res = r->normal.polar._X2;
    r->normal.polar._Valid = false;
    ...
    if (_Keep) {	// save second value for next call
      r->normal.polar._X2 = _Fx * _V2;
      r->normal.polar._Valid = true;
    }
    _Res = _Fx * _V1;
  }
...
{
  int i, sz = _arr_rows * _arr_cols, iters = cRound(iterFactor * sz);
