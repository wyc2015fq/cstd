...
void DFA::FindMatchingPatternInFile()
{
  ...
  // 打开一个只读文件和一个写文件
  char filePath[128];
  cout << endl << "请先创建好一个文本文件,将从中查找与给定正规式匹配的子串,然后输出查找结果!" << endl;
  cout << endl << "请输入要查找的文件路径和文件名: ";
  cin >> filePath;
  fstream infile;
  ...
  cout << endl << "查找结果如下:(结果显示后,请输入任意字符来结束该程序!)" << endl;
      ...
      while (curr < len) {
        ...
        while ((state != -1) && (curr < len)) {
          ...
          if (AcceptStates[state] == 1) {
            ...
            for (int wang = 0; wang < curr + 1; wang++) {
              cout << buf[wang];
            }
  ...
  while (curr < len) {
    ...
    while ((state != -1) && (curr < len)) {
      ...
      if (AcceptStates[state] == 1) {
        ...
        //outfile.write(buf, curr+1);
        for (int danse = 0; danse < curr + 1; danse++) {
          cout << buf[danse];
        }
