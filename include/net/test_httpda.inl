...
#define S_ISVTX 01000 //文件的sticky 位
#define S_IRUSR (S_IREAD) //00400 文件所有者具可读取权限
#define S_IWUSR (S_IWRITE)//00200 文件所有者具可写入权限
#define S_IXUSR (S_IEXEC) //00100 文件所有者具可执行权限
...
/* ******************************************************************** */
void accept_request(int* pclient)
{
  int client = *pclient;
  char buf[1024];
  int numchars;
  ...
  if (stat(path, &st) == -1) {
    ...
    /* 如果是个目?迹蚰鲜褂酶媚柯枷?index.html 文件 */
...
int test_httpd()
{
  ...
  while (1) {
    ...
    if (thread_create(1, newthread, accept_request, &client_sock, sizeof(client_sock)) != 0) {
