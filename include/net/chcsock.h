...
#define CPM_CLOSE(_SOCKET) CPM_CloseSocket(_SOCKET)
  ...
  */
  int CPM_Send(CPM_SOCKET s, const void FAR* buf, int len, int flags, int ms);
  int CPM_SafeSend(CPM_SOCKET s, const void FAR* pbData, int dwDataLen, int flags, int ms);
  int CPM_SafeSendFile(CPM_SOCKET s, void* pf, int flags, int ms);
...
#define CPM_SEND2(_SOCKET, _VAL) CPM_SafeSend(_SOCKET, (const char *)&(_VAL), sizeof(_VAL), DO_PRINT_FLAGS, NULL)
  ...
  */
  int CPM_Recv(CPM_SOCKET s, void* buf, int len, int flags, int ms);
  int CPM_SafeRecv(CPM_SOCKET s, char FAR* pbData, int dwDataLen, int flags, int ms);
  int CPM_SafeRecvFile(CPM_SOCKET s, void* pf, int flags, int ms);
