...
int strfind(const char* s1, const char* s2, int nPos)
{
  char* s = strstr(s1 + nPos, s2);
  return s ? s - s1 + nPos : -1;
}
int CHttpSocket::GetField(const char* szSession, char* szValue, int nMaxLength)
{
  ...
  nPos = -1;
  nPos = strfind(strRespons, szSession, 0);
  if (nPos != -1) {
    nPos += strlen(szSession);
    nPos += 2;
    nCr = strfind(strRespons, "\r\n", nPos);
    strncpy(szValue, strRespons + nPos, nCr - nPos);
    return (nCr - nPos);
  }
  else {
    return -1;
