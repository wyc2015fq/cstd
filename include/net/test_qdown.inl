...
URLInfo parse_url(const char* url)
{
  ...
  if (hptr != NULL) {
    strcpy(url_info.host, inet_ntop(hptr->h_addrtype, *(hptr->h_addr_list), IP, sizeof(IP)));
  }
  //printf("%s\n",url_info.host);
  return url_info;
}
Connection open_url(const char* url)
{
  ...
  if (bind(conn.sock, (struct sockaddr*) & local_addr, sizeof(local_addr)) < 0) {
    printf("bind error\n");
  }
  if (conn.sock) {
    if (connect(conn.sock, (struct sockaddr*)&remote_addr, sizeof(remote_addr)) != -1) {
      conn.avaliable = 1;
    }
  }
  return conn;
...
begin_down:
    ...
    if (body != NULL) {
      ...
      fflush(f);
      ct += i;
      thread_mutex_lock(&g_mut);
      g_downloaded += i;
      thread_mutex_unlock(&g_mut);
      while (ct < targ->limit) {
        i = recv(conn.sock, buf, sizeof(buf), 0);
        if (i == 0) {
          fclose(f);
...
void* monitor(void* args)
{
  ...
  strcat(cfgName, ".cfg");
  while (1) {
    p = (float)(g_downloaded / (g_total + 0.0));
...
void download(const char* url, int thread_amount, const char* file_name)
{
  ...
  thread_t tids[ MAX_THREAD ], monitor_id;
  Resource res;
...
new_task:
    ...
    for (i = 0; i < thread_amount; i++) {
      ...
      tids[ i ] = thread_create(download_part, & targs[ i ]);
    }
  }
  else {
    f = fopen(cfgName, "r");
  ...
  monitor_id = thread_create(monitor, res.file_name);
  ...
  thread_join(monitor_id);
}
int test_qdown1(int ac, char* av[])
{
  int thread_amount = 5;
...
int test_qdown()
{
  const char* url = "";
  const char* fn = "";
  url = "http://code.it168.com/v/vcduoxianchengxiazaigongjukaiyuandaima/HttpDownLoad.cpp";
  fn = "HttpDownLoad.cpp";
  download(url, 5, fn);
  return 0;
}
