...
static inline INT get_pathtypes_size(const Path* path)
{
...
static inline Status init_region(Region* region, const RegionType type)
{
...
/* recursive deletion of Region nodes */
void delete_element(region_element* element)
{
  ...
  case RegionDataRect:
    break;
  case RegionDataPath:
    //PathDelete(element->elementdata.pathdata.path);
    break;
  case RegionDataEmptyRect:
  case RegionDataInfiniteRect:
    break;
...
static inline Status clone_element(const region_element* element, region_element* element2)
{
    Status stat;

    /* root node is allocated with Region */
    ...
    switch (element->type)
    {
        ...
        default:
          if (element->elementdata.combine.left) {
            MYREALLOC(element2->elementdata.combine.left, 1);
            stat = clone_element(element->elementdata.combine.left, element2->elementdata.combine.left);
          } else {
            delete_element(element2->elementdata.combine.left);
            element2->elementdata.combine.left = NULL;
          }
          if (stat == Ok)
          {
            if (element->elementdata.combine.right) {
              MYREALLOC(element2->elementdata.combine.right, 1);
              stat = clone_element(element->elementdata.combine.right, element2->elementdata.combine.right);
            } else {
              delete_element(element2->elementdata.combine.right);
 ...
 */
static inline void fuse_region(Region* region, region_element* left,
 ...
 */
Status  GdipCloneRegion(const Region *region, Region *clone)
{
    TRACE("%p %p\n", region, clone);
    ...
    return clone_element(&region->node, &clone->node);
}
...
//   
Status  GdipCreateRegionPath(const Path *path, Region *region)
{
    ...
    if (count)
    {
        ...
        stat = PathGetPointsI(path, pointsi, count);
        if (stat != Ok)
        {
          FREE(pointsi);
          FREE(pointsf);
        ...
        stat = PathGetPoints(path, pointsf, count);
        if (stat != Ok)
        {
          FREE(pointsi);
          FREE(pointsf);
 ...
 */
Status  GdipCombineRegionPath(Region *region, const Path *path, CombineMode mode)
{
  Region path_region[1] = {0};
    ...
    if(mode == CombineModeReplace) {
        delete_element(&region->node);
        GdipCloneRegion(path_region, region);
        return Ok;
    }
 ...
 */
Status  GdipCombineRegionRect(Region *region,
        GDIPCONST RectF *rect, CombineMode mode)
{
    Region *rect_region;
    region_element *left, *right = NULL;
    Status stat;
    ...
    if(mode == CombineModeReplace){
        delete_element(&region->node);
        memcpy(region, rect_region, sizeof(Region));
 ...
 */
Status  GdipCombineRegionRectI(Region *region,
        GDIPCONST Rect *rect, CombineMode mode)
{
    RectF rectf;
 ...
 */
Status  GdipCombineRegionRegion(Region *region1,
        Region *region2, CombineMode mode)
{
    region_element *left, *right = NULL;
    Status stat;
    Region *reg2copy;
    ...
    if(mode == CombineModeReplace){
        ...
        memcpy(region1, reg2copy, sizeof(Region));
        GdipFree(reg2copy);
        return Ok;
    }
...
Status  GdipCreateRegion(Region **region)
{
    ...
    *region = GdipAlloc(sizeof(Region));
    if(!*region)
        return OutOfMemory;
 ...
 */
Status  GdipCreateRegionRect(GDIPCONST RectF *rect,
        Region **region)
{
    Status stat;
 ...
 */
Status  GdipCreateRegionRectI(GDIPCONST Rect *rect,
        Region **region)
{
    RectF rectf;
...
Status  GdipCreateRegionRgnData(GDIPCONST BYTE *data, INT size, Region **region)
{
    FIXME("(%p, %d, %p): stub\n", data, size, region);
...
Status  GdipCreateRegionHrgn(HRGN hrgn, Region **region)
{
    ...
    for(i = 0; i < buf->rdh.nCount; i++){
        if((stat = PathAddRectangle(path, (REAL)rect->left, (REAL)rect->top,
        ...
        INT* filled)
{
    ...
    switch (element->type)
    {
        ...
        {
            INT i;
            const Path* path = element->elementdata.pathdata.path;
 ...
 *  first.
 */
Status  GdipGetRegionData(Region *region, BYTE *buffer, UINT size,
        UINT *needed)
{
    INT filled = 0;
 ...
 */
Status  GdipGetRegionDataSize(Region *region, UINT *needed)
{
    TRACE("%p, %p\n", region, needed);
...
static Status get_path_hrgn(Path *path, Graphics *graphics, HRGN *hrgn)
{
    HDC new_hdc=NULL;
...
static Status get_region_hrgn(struct region_element *element, Graphics *graphics, HRGN *hrgn)
{
    switch (element->type)
    {
        case RegionDataInfiniteRect:
        ...
        case RegionDataRect:
        {
            Path* path;
            Status stat;
            RectF* rc = &element->elementdata.rect;

            stat = PathCreate(FillModeAlternate, &path);
            if (stat != Ok)
                return stat;
            stat = PathAddRectangle(path, rc->X, rc->Y, rc->Width, rc->Height);
...
Status  GdipIsEmptyRegion(Region *region, Graphics *graphics, BOOL *res)
{
    Status status;
    RectF rect;
 ...
 */
Status  GdipIsEqualRegion(Region *region, Region *region2, Graphics *graphics,
                                      BOOL *res)
{
    HRGN hrgn1, hrgn2;
    Status stat;
 ...
 */
Status  GdipIsInfiniteRegion(Region *region, Graphics *graphics, BOOL *res)
{
 ...
 */
Status  GdipIsVisibleRegionRect(Region* region, REAL x, REAL y, REAL w, REAL h, Graphics *graphics, BOOL *res)
{
    HRGN hrgn;
    Status stat;
    RECT rect;
 ...
 */
Status  GdipIsVisibleRegionRectI(Region* region, INT x, INT y, INT w, INT h, Graphics *graphics, BOOL *res)
{
    TRACE("(%p, %d, %d, %d, %d, %p, %p)\n", region, x, y, w, h, graphics, res);
    if(!region || !res)
        return InvalidParameter;
 ...
 */
Status  GdipIsVisibleRegionPoint(Region* region, REAL x, REAL y, Graphics *graphics, BOOL *res)
{
    HRGN hrgn;
    Status stat;
 ...
 */
Status  GdipIsVisibleRegionPointI(Region* region, INT x, INT y, Graphics *graphics, BOOL *res)
{
    TRACE("(%p, %d, %d, %p, %p)\n", region, x, y, graphics, res);
...
/* Transforms Region elements with given matrix */
static Status transform_region_element(region_element* element, Matrix *matrix)
{
    Status stat;
...
/* Translates Region elements with specified offsets */
...
static Status get_region_scans_data(Region *region, Matrix *matrix, LPRGNDATA *data)
{
    Region *region_copy;
    Status stat;
    HRGN hrgn;
...
Status  GdipGetRegionScansI(Region *region, Rect *scans, INT *count, Matrix *matrix)
{
    Status stat;
    INT i;
    LPRGNDATA data;
...
Status  GdipGetRegionScans(Region *region, RectF *scans, INT *count, Matrix *matrix)
{
    Status stat;
    INT i;
    LPRGNDATA data;
