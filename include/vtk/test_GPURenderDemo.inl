...
#define COLLECTION_DEF(X, Y)  typedef struct { X* first, *last;} Y
#define InitTraversal(a, b) b = a
//#define InitTraversalLast(a, b) for (b = a->first; b->next; b = b->next)
//#define GetNextProp(a, b)   b = b->next
//#define GetLastNode(a)    a->last
#define LISTDEF(T)   T* next, *prev
#define foreach_next(a, b)  for (b = a->first; (b = (b)->next); )
#define foreach_prev(a, b)  for (b = a->last; (b = (b)->prev); )
//#define listsize(a, b, sz)  for (sz=0, b = a->first; (b = (b)->next); ++sz)


template <typename Y> inline
int listsize_impl(Y* b) {
  int n = 0;
  for (; b = b->next;++n);
  return n;
}
...
int listsize(T* a) {
  return listsize_impl(a->first);
}

template <typename T, typename Y> inline
void AddItem(T* a, Y* b) {
  if (NULL==a->first) {
    a->first = b;
  }
  a->last = b;
...
template <typename T> inline
void RemoveAllItems(T* a) {
  while(a->first)
    RemoveItem(a, a->first);
}
template <typename T, typename Y> inline
bool IsItemPresent(T* a, Y* c) {
  Y* b;
  foreach_next(a, b) {
    if (c==b) {
      return true;
...
COLLECTION_DEF(vtkLight, vtkLightCollection);
COLLECTION_DEF(vtkActor, vtkActorCollection);
COLLECTION_DEF(vtkVolume, vtkVolumeCollection);
COLLECTION_DEF(vtkCuller, vtkCullerCollection);
COLLECTION_DEF(vtkRenderer, vtkRendererCollection);
COLLECTION_DEF(vtkActor2D, vtkActor2DCollection);
COLLECTION_DEF(vtkProp, vtkPropCollection);
COLLECTION_DEF(vtkAssemblyNode, vtkAssemblyPath);
COLLECTION_DEF(vtkAssemblyPath, vtkAssemblyPaths);
...
#include "vtkMath.h"
#include "vtkWindow.h"
#include "vtkWindow.cxx"
#include "vtkRenderWindowInteractor.cxx"
#include "vtkRenderWindow.h"
#include "vtkRenderWindow.cxx"
#include "vtkRenderer.cxx"
#include "vtkCamera.cxx"
#include "vtkMath.cxx"
#include "vtkMatrix4x4.cxx"
#include "vtkViewport.cxx"
//#include "vtkTransform.cxx"
//#include "vtkAbstractTransform.cxx"
#if 0
#include "vtkOpenGLRenderWindow.cxx"
#include "vtkWin32OpenGLRenderWindow.h"
#include "vtkOpenGLGPUVolumeRayCastMapper.inl"
#include "vtkglProjectedTetrahedraFS.cxx"
#include "vtkglProjectedTetrahedraVS.cxx"
#include "vtkOpenGLProjectedTetrahedraMapper.cxx"
#include "vtkOpenGLRayCastImageDisplayHelper.cxx"
#include "vtkRenderingVolumeOpenGL2ObjectFactory.cxx"
#include "vtkSmartVolumeMapper.cxx"
#endif

int test_GPURenderDemo() {
  //vtkOpenGLRenderWindow aa;
  return 0;
}
...
int test_GPURenderDemo(int argc, char *argv[])
{
  ...
  dirname = "D:/CTdata/CENOVIXupcompress";
