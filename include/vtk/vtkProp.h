...
public:
  ...
  virtual void GetActors2D(vtkActor2DCollection *) {}
  virtual void GetVolumes(vtkVolumeCollection *) {}
   ...
   * cause a Modified(this) when we set this value since it is not
   ...
   */
  void AddConsumer(vtkProp *c);
  void RemoveConsumer(vtkProp *c);
  vtkProp *GetConsumer(int i);
  int IsConsumer(vtkProp *c);
  //@}
