...
class VTKCOMMONTRANSFORMS_EXPORT vtkPerspectiveTransform : public vtkHomogeneousTransform
{
 public:
   ...
   */
  void Identity() { this->Concatenation->Identity(); Modified(this); };
  ...
  void Inverse() VTK_OVERRIDE
    { this->Concatenation->Inverse(); Modified(this); };
  ...
  void PreMultiply() {
    ...
    this->Concatenation->SetPreMultiplyFlag(1); Modified(this); };
  ...
  void PostMultiply()  {
    if (!this->Concatenation->GetPreMultiplyFlag()) { return; }
    this->Concatenation->SetPreMultiplyFlag(0); Modified(this); };
  ...
  void Push() { if (this->Stack == NULL) {
                    this->Stack = new vtkTransformConcatenationStack(); }
