...
UINT fnStageHash(PSPOT pStar, PSPOT pSort, UINT dwCount)
{
  ...
  if ((g_Hash.Flags & HSF_FLAG_INIT) == 0) {
    // 自动初始化CRC余式表
    CRCMakeFast(g_Hash.Table, HSF_SEED_HASH); // 0xEDB88320
    g_Hash.Flags = HSF_TYPE_CRC32 | HSF_FLAG_INIT;
    g_Hash.Seed = HSF_FLAG_INIT;
  }
  g_Hash.Value = 0xFFFFFFFF; // last value
  return CRCSSH(&g_Hash, pSort, sizeof(SPOT) * dwCount);
}
