...
void CGraph::SetColor(int dataGroup, COLOR groupColor)
{
	//CGraphDataColor *dataColor = new CGraphDataColor(dataGroup, groupColor);
	colorList[colorList_len++] = groupColor;
}

COLOR CGraph::GetColor(int dataGroup)
{
	if((colorList_len <= dataGroup) || (colorList_len == 0))
		return BLACK;
	
	return colorList[dataGroup];
}

void CGraph::DrawGraph(gc_t* pDC)
{
	char* tickLabel;
	IRECT graphRect = rcClient;

	//reset graph to be clear background
	COLOR backColor;
	backColor = _RGB(255,255,255);  //replace with desired background color
  gcSolidBrush(pDC, backColor);
	gcFillRectangle(pDC, graphRect.l, graphRect.t, RCW(&graphRect), RCH(&graphRect));
	
	maxHeight = RCH(&graphRect) - 20;  //for frame window and status bar
	maxWidth = RCW(&graphRect) - 5;  //for frame window

	//draw graph title
  font_t titleFont[1] = {0};
	int tFontSize = 24;
	while((strlen(graphTitle) * (tFontSize / 2)) > maxWidth)
	{
		tFontSize -= 2;
	}
	gcFont(titleFont, tFontSize, 0, 0, 0, 700, FALSE, FALSE, 0,
		ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
		DEFAULT_QUALITY, DEFAULT_PITCH | FF_ROMAN, "Arial");

	int charWidth = titleFont->w;
	//next line is centered....trust me !!
	gcTextOut(pDC, (RCW(&graphRect) / 2) - ((strlen(graphTitle) / 2) * charWidth), 10, graphTitle, NULL);
	...
	else
	{
		...
		font_t* pOldFont = (font_t*) pDC->SelectObject(&sideFont);
...
void CGraph::DrawAxis(gc_t* pDC)
{
	TEXTMETRIC	tm;
	pDC->SetTextColor(BLACK);
	font_t sideFont, axisFont, dummyFont;
	...
	dummyFont.CreateFont(12, 0, 0, 0, 700, FALSE, FALSE, 0,
		ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
		DEFAULT_QUALITY, DEFAULT_PITCH | FF_ROMAN,"Arial");
	pOldFont = (font_t*)pDC->SelectObject(&dummyFont);
	switch(graphType)
	{
		case BAR_GRAPH :
		case LINE_GRAPH :
...
void CGraph::DrawSeries(gc_t* pDC)
{
	switch(graphType)
	{
		case BAR_GRAPH :
...
int CGraph::DrawLegend(gc_t* pDC)
{
	...
	font_t legendTitleFont;
	legendTitleFont.CreateFont(16, 0, 0, 0, 700, FALSE, FALSE, 0,
		ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
		DEFAULT_QUALITY, DEFAULT_PITCH | FF_ROMAN,"Arial");
	font_t* pOldFont = (font_t*) pDC->SelectObject(&legendTitleFont);
	pDC->GetTextMetrics(&tm);
	int charWidth = tm.tmAveCharWidth;
	...
	for(int i = 0; i < seriesSize; i++)
	{
		...
		legendFont.CreateFont(legendFontSize, 0, 0, 0, 700, FALSE, FALSE, 0,
			ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
			DEFAULT_QUALITY, DEFAULT_PITCH | FF_ROMAN,"Arial");
		pOldFont = (font_t*) pDC->SelectObject(&legendFont);
...
int CGraph::PrintGraph(gc_t* pDC, CPrintInfo* pInfo)
{
	CString		str;
	font_t		graphTitleFont, textFont;
	TEXTMETRIC	tm;
	CString tickLabel;
	...
	font_t* pOldFont = (font_t*) pDC->SelectObject(&graphTitleFont);
	pDC->GetTextMetrics(&tm);
...
int CGraph::PrintLegend(gc_t *pDC)
{
	font_t		legendLabelFont, legendTitleFont;
	TEXTMETRIC	tm;
	...
	for(int i = 0; i < seriesSize; i++)
	{
		pOldFont = (font_t*) pDC->SelectObject(&legendLabelFont);
		pDC->GetTextMetrics(&tm);
...
void CGraph::PrintAxis(gc_t *pDC)
{
	TEXTMETRIC tm;
	pDC->SetTextColor(BLACK);
	font_t sideFont, axisFont;
...
//		case RADAR_GRAPH :
		...
		case XY_LINE_GRAPH :
				...
				sideFont.CreateFont(tFontSize, 0, 2700, 0, 700, FALSE, FALSE, 0,
					...
					DEFAULT_QUALITY, DEFAULT_PITCH | FF_ROMAN,"Arial");
				font_t* pOldFont;
				pOldFont = (font_t*) pDC->SelectObject(&sideFont);
				pDC->GetTextMetrics(&tm);
				charWidth = tm.tmAveCharWidth;
				...
				if(graphAlignment)
				{
					...
					for(int y = 1; y <= numTicks; y++)  
					{
						...
						yFont.CreateFont(yTickFontSize * 20, 0, 0, 0, 700, FALSE, FALSE, 0,
							ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
							DEFAULT_QUALITY, DEFAULT_PITCH | FF_ROMAN,"Arial");
						pOldFont = (font_t*) pDC->SelectObject(&yFont);
						...
						for(int x = 1; x <= graphSeries->GetCount(); x++)
						{
							...
							else
							{
								...
								sideFont2.CreateFont(xTickFontSize * 20, 0, ((360 - xAxisAlign) * 10), 0, 700, FALSE, FALSE, 0,
									ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
									DEFAULT_QUALITY, DEFAULT_PITCH | FF_ROMAN,"Arial");
								pOldFont = (font_t*) pDC->SelectObject(&sideFont2);
								pDC->GetTextMetrics(&tm);
								charWidth = tm.tmAveCharWidth;
								if(xAxisAlign < 180)
					...
					else
					{
						...
						for(int x = 1; x <= numTicks; x++)  
						{
							...
							xFont.CreateFont(xTickFontSize * 20, 0, 0, 0, 700, FALSE, FALSE, 0,
								ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
								DEFAULT_QUALITY, DEFAULT_PITCH | FF_ROMAN,"Arial");
							pOldFont = (font_t*) pDC->SelectObject(&xFont);
							pDC->GetTextMetrics(&tm);
							charWidth = tm.tmAveCharWidth;
...
void CGraph::PrintSeries(gc_t *pDC)
{
	switch(graphType)
	{
		case BAR_GRAPH :
...
void CGraph::DrawBarSeries(gc_t* pDC)
{
	int barWidth;
...
void CGraph::DrawLineSeries(gc_t* pDC)
{
	int barWidth;
	int dataPlotSize;   //used to plot rects of data
	int tickXLocation, tickYLocation;
...
void CGraph::DrawPieSeries(gc_t* pDC)
{
...
void CGraph::DrawScatterSeries(gc_t* pDC)
{
...
void CGraph::DrawBoxWhiskerSeries(gc_t* pDC)
{
	int upperQuartile;
	int lowerQuartile;
	int median;
...
void CGraph::DrawStackedBarSeries(gc_t* pDC)
{
	int barWidth;
	int barL, barT, barR, barB;
	int tickXLocation, tickYLocation;
...
void CGraph::DrawXYLineSeries(gc_t* pDC)
{
...
//void CGraph::DrawRadarSeries(gc_t* pDC)
//{
//}

void CGraph::Draw3DBarSeries(gc_t* pDC)
{
	int barWidth;
	int dataPlotSize;   //used to plot rects of data
	int barL, barT, barR, barB;
...
void CGraph::Draw3DLineSeries(gc_t* pDC)
{
	int barHeight;
	POSITION pos;
	CGraphSeries* tmpSeries;
		...
		for(int s = 0; s < seriesSize; s++)
		{
			...
			for(int x = 1; x <= graphSeries->GetCount(); x++)
			{
				...
				if(tmpSeries->GetData(s) > -1)
				{
					...
					COLOR bottomColor;
					bottomColor = _RGB(bottomRed, bottomGreen, bottomBlue);
...
void CGraph::Draw3DPieSeries(gc_t* pDC)
{
	double dataSum = 0.00;  //for storing cumulative sum
	double labelData = 0.00;
	int lastXLocation, lastYLocation;
	...
	for(int x = 1; x <= graphSeries->GetCount(); x++)
	{
		...
		IRECT pieRect (pieLeft, 
				...
				pieRight, 
				centerYPie + radius);
		IRECT shadowRect (pieLeft + depth,
		...
		for(s = 0; s < seriesSize; s++)
		{
			if(tmpSeries->GetData(s) > 0)
			{
				...
				COLOR shadowColor(_RGB(red, green, blue));
...
void CGraph::Draw3DStackedBarSeries(gc_t* pDC)
{
	int barWidth;
	int barL, barT, barR, barB;
	int tickXLocation, tickYLocation;
...
void CGraph::PrintBarSeries(gc_t* pDC)
{
	int barWidth;
	int dataPlotSize;   //used to plot rects of data
	int barL, barT, barR, barB;
...
void CGraph::PrintLineSeries(gc_t* pDC)
{
	int barWidth;
	int dataPlotSize;   //used to plot rects of data
	int tickXLocation, tickYLocation;
...
void CGraph::PrintPieSeries(gc_t* pDC)
{
	int seriesSpace;
	POSITION pos;
	CGraphSeries* tmpSeries;
	...
	for(int x = 1; x <= graphSeries->GetCount(); x++)
	{
		...
		pieFont.CreateFont(xTickFontSize * 20, 0, 0, 0, 700, FALSE, FALSE, 0,
			ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
			DEFAULT_QUALITY, DEFAULT_PITCH | FF_ROMAN,"Arial");
		font_t* pOldFont = (font_t*) pDC->SelectObject(&pieFont);
...
void CGraph::PrintScatterSeries(gc_t* pDC)
{
	//rightXTick and topYTick contain the outer bounds of the axis ticks.
	//So, if maxTick is 100 on both axis lines, then 100, 100 would be top
	//right.  We will use the bounds to see spacing from apex points and then
...
void CGraph::PrintBoxWhiskerSeries(gc_t* pDC)
{
	int upperQuartile;
	int lowerQuartile;
	int median;
...
void CGraph::PrintStackedBarSeries(gc_t* pDC)
{
	int barWidth;
	int barL, barT, barR, barB;
	int tickXLocation, tickYLocation;
...
void CGraph::PrintXYLineSeries(gc_t* pDC)
{
...
//void CGraph::PrintRadarSeries(gc_t* pDC)
//{
//}

void CGraph::Print3DBarSeries(gc_t* pDC)
{
	int barWidth;
	int dataPlotSize;   //used to plot rects of data
	int barL, barT, barR, barB;
...
void CGraph::Print3DLineSeries(gc_t* pDC)
{
	int barWidth;
	int dataPlotSize;   //used to plot rects of data
	int tickXLocation, tickYLocation;
...
void CGraph::Print3DPieSeries(gc_t* pDC)
{
	double dataSum = 0.00;  //for storing cumulative sum
	double labelData = 0.00;
	int lastXLocation, lastYLocation;
...
void CGraph::Print3DStackedBarSeries(gc_t* pDC)
{
	int barWidth;
	int barL, barT, barR, barB;
	int tickXLocation, tickYLocation;
