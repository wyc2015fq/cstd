Model_VertexArray版本
2010-01-05 19:22 233人阅读 评论(0) 收藏 举报
floatarraysdeleteclass360
Sphere
Cylinder
Torus
代码：
...
public:  
    ...
    //-----------------------------------------------------------------------------  
    // Name: SetupCylinder()  
    // Desc: Create a cylinder centered at 0, 0, 0 with circle radius, height  
    //       circle's precision  
    //       and height segment.(>=1)  
    //-----------------------------------------------------------------------------  
    void SetupCylinder(float radius, int cirPrecision, float height, int heighSegment)  
    {  
        const float TWOPI  = 6.28318530717958f;  
        ...
        {  
            float heigh_i = (float)(i)/(float)(heighSegment);  
            //0.0 <= delta <= 1.0  
            py = heigh_i*height;   
            ey = 0.0f;  
            ...
            {  
                ...
                pVertex[ i*(cirPrecision+1)+j ] =   
                    CVertex(px, py, pz, (float)(j)/cirPrecision, heigh_i, ex, ey, ez);  
            }  
        }  
