...
{
    agg::slider_ctrl<_rgba8> m_thickness;
    agg::slider_ctrl<_rgba8> m_gamma;
    agg::slider_ctrl<_rgba8> m_contrast;
    double m_rx;
    double m_ry;
...
public:
    ...
    virtual void on_draw()
    {
        ...
        renb.copy_bar(0,0,int(width())/2, int(height()),                _rgba(dark,dark,dark));
        renb.copy_bar(int(width())/2+1,0, int(width()), int(height()),  _rgba(light,light,light));
        renb.copy_bar(0,int(height())/2+1, int(width()), int(height()), _rgba(1.0,dark,dark));
        ...
        ras.reset();
        ras.add_path(gpoly);
        agg::render_scanlines_aa_solid(ras, sl, renb, _rgba8(80,127,80));
        ...
        ras.reset();
        ras.add_path(poly);
        agg::render_scanlines_aa_solid(ras, sl, renb, _rgba8(255,0,0));

        ell.init(width() / 2, height() / 2, m_rx-5.0, m_ry-5.0, 150);
        ras.reset();
        ras.add_path(poly);
        agg::render_scanlines_aa_solid(ras, sl, renb, _rgba8(0,255,0));

        ell.init(width() / 2, height() / 2, m_rx-10.0, m_ry-10.0, 150);
        ras.reset();
        ras.add_path(poly);
        agg::render_scanlines_aa_solid(ras, sl, renb, _rgba8(0,0,255));

        ell.init(width() / 2, height() / 2, m_rx-15.0, m_ry-15.0, 150);
        ras.reset();
        ras.add_path(poly);
        agg::render_scanlines_aa_solid(ras, sl, renb, _rgba8(0,0,0));

        ell.init(width() / 2, height() / 2, m_rx-20.0, m_ry-20.0, 150);
        ras.reset();
        ras.add_path(poly);
        agg::render_scanlines_aa_solid(ras, sl, renb, _rgba8(255,255,255));
