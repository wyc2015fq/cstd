...
{
    agg::gamma_ctrl<_rgba8>  m_profile;
    agg::spline_ctrl<_rgba8> m_spline_r;
    agg::spline_ctrl<_rgba8> m_spline_g;
    agg::spline_ctrl<_rgba8> m_spline_b;
    agg::spline_ctrl<_rgba8> m_spline_a;
    agg::rbox_ctrl<_rgba8>   m_rbox;
...
public:
        ...
        m_mouse_move(false)
    {
        ...
        m_spline_r.background_color(_rgba(1.0, 0.8, 0.8));
        m_spline_g.background_color(_rgba(0.8, 1.0, 0.8));
        m_spline_b.background_color(_rgba(0.8, 0.8, 1.0));
        m_spline_a.background_color(_rgba(1.0, 1.0, 1.0));
    ...
    virtual void on_draw()
    {
        ...
        for(i = 0; i < 256; i++)
        {
            color_profile[i] = color_type(_rgba(m_spline_r.spline()[i], 
                                                    m_spline_g.spline()[i],
                                                    m_spline_b.spline()[i],
                                                    m_spline_a.spline()[i]));
        }
...
//        gr_circle.m_gradient.init(150, 80, 80);
    ...
    virtual void on_key(int x, int y, unsigned key, unsigned flags) 
    {
        if(key == agg::key_f1)
        {
            ...
            for(i = 0; i < 256; i++)
            {
                color_type c = _rgba(m_spline_r.spline()[i], 
                                         m_spline_g.spline()[i],
                                         m_spline_b.spline()[i],
                                         m_spline_a.spline()[i]);
