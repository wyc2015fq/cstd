...
{
    agg::rbox_ctrl<_rgba8>    m_method;
    agg::slider_ctrl<_rgba8>  m_radius;
    agg::polygon_ctrl<_rgba8> m_shadow_ctrl;
    agg::cbox_ctrl<_rgba8>    m_channel_r;
    agg::cbox_ctrl<_rgba8>    m_channel_g;
    agg::cbox_ctrl<_rgba8>    m_channel_b;
    ...
    agg::stack_blur    <_rgba8, agg::stack_blur_calc_rgb<> >     m_stack_blur;
    agg::recursive_blur<_rgba8, agg::recursive_blur_calc_rgb<> > m_recursive_blur;
...
public:
        ...
        m_shape(m_path)
    {
        ...
        m_shadow_ctrl.line_color(_rgba(0, 0.3, 0.5, 0.3));
    }
    ...
    virtual void on_draw()
    {
        ...
        agg::render_scanlines_aa_solid(m_ras, m_sl, renb, _rgba(0.2,0.3,0));
        ...
        if(m_method.cur_item() != 2)
        {
            ...
            if(pixf2.attach(pixf, int(bbox.x1), int(bbox.y1), int(bbox.x2), int(bbox.y2)))
            {
                ...
                if(m_method.cur_item() == 0)
                {
                    ...
                    //------------------
                    agg::stack_blur_rgb24(pixf2, agg::uround(m_radius.value()), agg::uround(m_radius.value()));
                }
                else
                {
        ...
        agg::render_scanlines_aa_solid(m_ras, m_sl, renb, _rgba(0.6,0.9,0.7, 0.8));
        ...
        m_ras.add_path(st);
        agg::render_scanlines_aa_solid(m_ras, m_sl, renb, _rgba(0,0,0));
