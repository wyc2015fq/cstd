...
{
    agg::slider_ctrl<_rgba> m_polygon_angle;
    agg::slider_ctrl<_rgba> m_polygon_scale;

    agg::slider_ctrl<_rgba> m_pattern_angle;
    agg::slider_ctrl<_rgba> m_pattern_size;

    agg::slider_ctrl<_rgba> m_pattern_alpha;

    agg::cbox_ctrl<_rgba> m_rotate_polygon;
    agg::cbox_ctrl<_rgba> m_rotate_pattern;
    agg::cbox_ctrl<_rgba> m_tie_pattern;
...
public:
    ...
    void generate_pattern()
    {
        ...
        rb.clear(_rgba_pre(0.4, 0.0, 0.1, m_pattern_alpha.value())); // Pattern background color

        m_ras.add_path(smooth);
        rs.color(_rgba8(110,130,50));
        agg::render_scanlines(m_ras, m_sl, rs);

        m_ras.add_path(stroke);
        rs.color(_rgba8(0,50,80));
        agg::render_scanlines(m_ras, m_sl, rs);
    }
