...
{
public:
    style_handler(const _rgba8* styles, unsigned count) : 
    ...
    const _rgba8& color(unsigned style) const 
    {
        if (style < m_count)
    ...
    void generate_span(_rgba8* span, int x, int y, unsigned len, unsigned style)
    {
    }

private:
    _rgba8          m_transparent;
    const _rgba8*   m_styles;
    unsigned            m_count;
};
...
class the_application : public agg::platform_support
{
    ...
    agg::slider_ctrl<_rgba8> m_alpha1;
    agg::slider_ctrl<_rgba8> m_alpha2;
    agg::slider_ctrl<_rgba8> m_alpha3;
    agg::slider_ctrl<_rgba8> m_alpha4;
    agg::cbox_ctrl<_rgba8>   m_invert_order;
...
public:
    ...
    virtual void on_draw()
    {
        ...
        agg::pod_vector<_rgba8> gr(pixf_pre.width());
        unsigned i;
        for(i = 0; i < pixf.width(); i++)
        {
            gr.add(_rgba8(255, 255, 0).gradient(_rgba8(0, 255, 255), 
                                                    double(i) / pixf.width()));
        }
        ...
        agg::scanline_u8 sl;
        agg::span_allocator<_rgba8> alloc;
        ...
        styles[3] = _rgba8(lut.dir(255),
                               lut.dir(0),
                               lut.dir(108),
                               200).premultiply();

        styles[2] = _rgba8(lut.dir(51),
                               lut.dir(0),
                               lut.dir(151),
                               180).premultiply();

        styles[1] = _rgba8(lut.dir(143),
                               lut.dir(90),
                               lut.dir(6),
                               200).premultiply();

        styles[0] = _rgba8(lut.dir(0),
                               lut.dir(0),
                               lut.dir(255),
                               220).premultiply();
