...
{
    agg::rbox_ctrl<_rgba8> m_polygons;
    agg::rbox_ctrl<_rgba8> m_operation;
    double m_x;
    double m_y;
    ...
    unsigned render_gpc(Scanline& sl, Ras& ras)
    {
        ...
        switch(m_polygons.cur_item())
        {
            case 0:
            {
                ...
                ras.reset();
                ras.add_path(ps1);
                ren.color(_rgba(0, 0, 0, 0.1));
                agg::render_scanlines(ras, sl, ren);

                ras.reset();
                ras.add_path(ps2);
                ren.color(_rgba(0, 0.6, 0, 0.1));
                agg::render_scanlines(ras, sl, ren);
            ...
            case 2:
            {
                ...
                ren.color(_rgba(0.0, 0.5, 0.5, 0.1));
                agg::render_scanlines(ras, sl, ren);
...
// Sometimes takes long to produce the result
    ...
    virtual void on_mouse_button_down(int x, int y, unsigned flags)
    {
        ...
        for(i = 0; i < 1000; i++)
        {
            ...
            ras.add_path(gpc);
            ren_solid.color(_rgba(0.5, 0.0, 0, 0.5));
