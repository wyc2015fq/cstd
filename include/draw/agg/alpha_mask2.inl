...
unsigned parse_lion(agg::path_storage& ps, _rgba8* colors, unsigned* path_idx);
void parse_lion()
{
...
{
    agg::slider_ctrl<_rgba> m_num_cb;
...
public:
    ...
    virtual void on_draw()
    {
        ...
        for(i = 0; i < 50; i++)
        {
            m.line_color(_rgba8(rand() & 0x7F, 
                                    rand() & 0x7F, 
                                    rand() & 0x7F, 
                                    (rand() & 0x7F) + 0x7F)); 
            m.fill_color(_rgba8(rand() & 0x7F, 
                                    rand() & 0x7F, 
                                    rand() & 0x7F, 
                                    (rand() & 0x7F) + 0x7F));
        ...
        grad_func grf;
        grad_color grc(_rgba8(0,0,0), _rgba8(0,0,0));
        agg::ellipse ell;
        ...
        for(i = 0; i < 50; i++)
        {
            ...
            grm.invert();
            grc.colors(_rgba8(255, 255, 255, 0),
                       _rgba8(rand() & 0x7F, 
                                  rand() & 0x7F, 
                                  rand() & 0x7F, 
                                  255));
