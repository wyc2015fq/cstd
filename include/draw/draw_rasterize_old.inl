...
#else
#define PUT_PIX_SOLID(dst, _alpha, _x, _y) *(COLOR*)(dst) = *(COLOR*)(pclr);
int draw_rasterize(img_t* im, const IRECT* pclip, const brush_t* br, scancell* s, int flip_y,
    int opt, const DPOINT* pt0, const int* lens, int n, const PrimRectUV* prcuv, int nrcuv, const bitmap_t* tex1, int alphamode, int nvtx, ImVertex* vtx)
{
  const uchar* pclr = 0;
  const uchar* pmask = (const uchar*)&br->alpha_mask;
  uchar _a = 0;
#define PIXBLEND3(dst, _a, _x, _y)    (PIXBLEND(dst[0], pclr[0], _a*pmask[0]>>8), PIXBLEND(dst[1], pclr[1], _a*pmask[1]>>8), PIXBLEND(dst[2], pclr[2], _a*pmask[2]>>8))
#define PUTCLR(dst, _alpha, _x, _y) {_a = (_alpha * pclr[3])>>8; PIXBLEND(dst[0], pclr[0], _a*pmask[0]>>8); PIXBLEND(dst[1], pclr[1], _a*pmask[1]>>8); PIXBLEND(dst[2], pclr[2], _a*pmask[2]>>8); }
  switch (br->type) {
  case br_solid: {
    pclr = (const uchar*)(br->clrs);
#define GETCLR_INIT(y)
#define GETCLR(x, y) {}
#include "draw2d_poly_case_cn.inl"
  }
  break;
