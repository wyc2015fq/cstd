...
static Status PathSetSize_(Path* path, int n, const char* file, int line) {
  ...
  if (n>0) {
    if (n > path->Size) {
      path->Points = (FPOINT*)prealloc_(path->Points, sizeof(FPOINT)*n, file, line);
      if (NULL==path->Points) {
        return OutOfMemory;
      }
      path->Types = (BYTE*)prealloc_(path->Types, sizeof(BYTE)*n, file, line);
      if (NULL==path->Types) {
        return OutOfMemory;
      }
      path->Size = n;
...
static Status PointI2F(FPOINT** pptsF, const IPOINT* points, int count) {
  int i;
  FPOINT *ptsf = (FPOINT*)prealloc(*pptsF, sizeof(FPOINT) * count);
  if(!ptsf)
    return OutOfMemory;
                           ...
                           int count, float tension)
{
  ...
  pt = (FPOINT*)pmalloc((len_pt + count + 1) * sizeof(FPOINT));
  pts = pt + len_pt;
  if(!pt) {
    pfree(pt);
    return OutOfMemory;
                     ...
                     float tension)
{
  ...
  pt = (FPOINT*)pmalloc(sizeof(FPOINT) * len_pt);
  if(!pt)
    return OutOfMemory;
                      ...
                      int count, float tension)
{
  ...
  ptf = (FPOINT*)pmalloc(sizeof(FPOINT)*count);
  if(!ptf)
    return OutOfMemory;
...
static Status PathAddLine2I(Path *path, const IPOINT *points, int count)
{
  ...
  pointsF = (FPOINT*)pmalloc(sizeof(FPOINT) * count);
...
static Status PathAddPolygonX(Path *path, const void* pt, const int* lens, int n, GCTYPE type, int step) {
...
{
  count = -abs(count);
  return PathAddPolygonX(path, points, &count, 1, GC_FLOAT, sizeof(*points));
}
...
static Status PathAddPolygonI(Path *path, const IPOINT *points, int count, bool closed)
{
  ...
  return PathAddPolygonX(path, points, &count, 1, GC_INT32, sizeof(*points));
}
...
static Status PathAddRectanglesI(Path *path, const IRECT *rects, int count)
{
  ...
  rectsF = (FRECT*)pmalloc(sizeof(FRECT)*count);
                          ...
                          const StringFormat *format, gdip_format_string_callback callback, void *user_data)
{
  ...
  stringdup = (wchar*)pmalloc((length + 1) * sizeof(WCHAR));
  ...
  if (hotkeyprefix_count)
    hotkeyprefix_offsets = (int*)pmalloc(sizeof(INT) * hotkeyprefix_count);
...
static Status PathAddString(Path* path, const WCHAR* string, int length, const ttffont_t* f, float wrap_width, float scale, float angle, float x, float y, const FRECT* layoutRect, const StringFormat* format)
{
  ...
  arg->path = path;
  arg->font = f;
  length = length<0 ? wstr_len(string) : length;
...
static Status PathAddStringA(Path* path, const char* string, int length, const ttffont_t* f, float wrap_width, float scale, float angle, float x, float y, const FRECT* layoutRect, const StringFormat* format)
{
  wchar* wstring = NULL;
  int wlength=0;
  length = length<0 ? cstr_len(string) : length;
  wstring = (wchar*)pmalloc((length+1)*2);
  wlength = sys_mb2wc(CE_GB2312, string, length, wstring, length);
