...
int16u value16(const int8u* p, bool m_big_endian)
{
  int16u v;
  if(m_big_endian)
  {
...
//-------------------------------------------------------------------------
int gsv_text(Path* path,double m_start_x,double m_y,double m_width,double m_height,
             double m_space,double m_line_space,char* m_text,const void* m_font,bool m_flip)
{
  int8* m_glyphs;
  int8u* m_indices;
  ...
  int8* m_bglyph;
  int8* m_eglyph;
  double m_w=0, m_h=0, m_x=0, base_height;
  char* m_cur_chr;
  bool m_big_endian = false;
  int t = 1;
  if(*(char*)&t == 0) m_big_endian = true;

  m_font = m_font ? m_font : gsv_default_font;
  //-------------------------------------------------------------------------
  {
    if(m_font == 0) return 0;
    m_indices = (int8u*)m_font;
    base_height = value16(m_indices + 4, m_big_endian);
    m_indices += value16(m_indices, m_big_endian);
  ...
  //-------------------------------------------------------------------------
  {
    ...
    for(; *m_cur_chr; ) {
      ...
      idx <<= 1;
      m_bglyph = m_glyphs + value16(m_indices + idx, m_big_endian);
      m_eglyph = m_glyphs + value16(m_indices + idx + 2, m_big_endian);
      
      PathMoveTo(path, m_x, m_y);
      while(m_bglyph < m_eglyph) {
        dx = (int)(*m_bglyph++);
        yf = (yc = *m_bglyph++) & 0x80;
        yc <<= 1; 
        yc >>= 1;
        ...
        m_x += (double)(dx) * m_w;
        m_y += (double)(dy) * m_h;
        PathAddPoint(path, m_x, m_y, yf ? PathPointTypeStart : PathPointTypeLine);
      }
      m_x += m_space;
    }
  }
