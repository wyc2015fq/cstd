...
int my_malloc(void* pp, int size) {
	char** pc = (char**)pp;
	*pc = (char*)malloc(size);
	return *pc != NULL;
}

int my_realloc(void* pp, int size) {
	char** pc = (char**)pp;
	*pc = (char*)realloc(*pc, size);
	return *pc != NULL;
}
...
#define JDA_VECTOR_NEW(v, size_) \
  do { \
    (v).size = 0; \
    (v).capacity = 2 * size_; \
    my_malloc(&(v).data, (v).capacity * sizeof((v).data[0])); \
  } while (0)
...
#define JDA_VECTOR_INSERT(v, value) \
  do { \
    ...
    if ((v).size > (v).capacity) { \
      ...
      my_realloc(&(v).data, capacity_new * sizeof(value)); \
...
#define JDA_VECTOR_INSERT_MORE(v, values, size_) \
  do { \
    ...
    if (size_new > (v).capacity) { \
      ...
      my_realloc(&(v).data, capacity_new * sizeof((values)[0])); \
      (v).capacity = capacity_new; \
    } \
