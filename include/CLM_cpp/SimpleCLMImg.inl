...
void write_out_landmarks(const string& outfeatures, const CLM& clm_model)
{
  create_directory_from_file(outfeatures);
  std::ofstream featuresFile;
  featuresFile.open(outfeatures);
...
void create_display_image(const Mat& orig, Mat& display_image, CLM& clm_model)
{
  ...
  // now scale it
  resize(display_image.clone(), display_image, Size(), scaling, scaling);
...
int SimpleCLMImg(int argc, char** argv)
{
  ...
  get_image_input_output_params(files, depth_files, output_landmark_locations, output_pose_locations, output_images, bounding_boxes, arguments);
  ...
  for (size_t i = 0; i < files.size(); i++) {
    ...
    if (bounding_boxes.empty()) {
      ...
      if (clm_parameters.curr_face_detector == CLMParameters::HOG_SVM_DETECTOR) {
        vector<double> confidences;
        DetectFacesHOG(face_detections, grayscale_image, face_detector_hog, confidences);
      }
      else
#endif
#ifdef _USE_CVFACE
      ...
      if (face_detections.size() > 0) {
        char bbox[256] = { 0 };
        strcpy(bbox, file.c_str());
        int len1 = strlen(bbox);
        char* cc = bbox + len1 - 3;
        printf("%s\n", cc);
        *cc = 0;
        strcpy(cc, "txt");
        printf("%s\n", bbox);
        FILE* pf = fopen(bbox, "wb");

        if (pf) {
          Rect_<double>* r = &face_detections[0];
      ...
      for (size_t face = 0; face < face_detections.size(); ++face) {
        ...
        bool success = DetectLandmarksInImage(grayscale_image, depth_image, face_detections[face], clm_model, clm_parameters);
        ...
        if (!output_landmark_locations.empty()) {
          ...
          string slash("/");
          std::string preferredSlash = slash;

          string out_feat_path(output_landmark_locations.at(i));
          string dir = parent_path(out_feat_path);
          string fname = replace_extension(filename(out_feat_path), "");
          string ext = extension(out_feat_path);
          string outfeatures = dir + preferredSlash + fname + name + ext;
        ...
        if (!output_pose_locations.empty()) {
          ...
          string out_pose_path(output_pose_locations.at(i));
          string dir = parent_path(out_pose_path);
          string fname = filename(out_pose_path);
          string ext = extension(out_pose_path);
          string outfeatures = dir + preferredSlash + fname + name + ext;
        ...
        if (clm_parameters.track_gaze) {
          ...
          DrawBox(read_image, pose_estimate_to_draw, Scalar(255.0, 0, 0), 3, fx, fy, cx, cy);
        ...
        if (!output_images.empty() && success) {
          ...
          if (!outimage.empty()) {
            ...
            string out_feat_path(outimage);
            string dir = parent_path(out_feat_path);
            string fname = filename(out_feat_path);
            string ext = extension(out_feat_path);
            outimage = dir + preferredSlash + fname + name + ext;
      ...
      DetectLandmarksInImage(grayscale_image, bounding_boxes[i], clm_model, clm_parameters);
