...
void output_HOG_frame(std::ofstream* hog_file, bool good_frame, const Mat_<double>& hog_descriptor, int num_rows, int num_cols)
{
  ...
  const double* descriptor_it = hog_descriptor.ptr();
...
void visualise_tracking(Mat& captured_image, const CLM& clm_model, const CLMParameters& clm_parameters, Point3f gazeDirection0, Point3f gazeDirection1, int frame_count, double fx, double fy, double cx, double cy)
{
...
for (string class_name : au_names_class) {
    ...
    while (!captured_image.empty()) {
      ...
      if (video_input || images_as_video) {
        detection_success = DetectLandmarksInVideo(grayscale_image, clm_model, clm_parameters);
      }
      else {
        detection_success = DetectLandmarksInImage(grayscale_image, clm_model, clm_parameters);
      }
      ...
      if (!output_similarity_align.empty() || hog_output_file.is_open() || !output_au_files.empty()) {
        ...
        if (!clm_parameters.quiet_mode) {
          imshow("sim_warp", sim_warped_img);
        }
