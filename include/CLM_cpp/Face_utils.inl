...
void AlignFace(Mat& aligned_face, const Mat& frame, const CLM& clm_model, bool rigid, double sim_scale, int out_width, int out_height)
{
  ...
  Matx22d scale_rot_matrix = AlignShapesWithScale(source_landmarks, destination_landmarks);
  Matx23d warp_matrix;
  ...
  warpAffine(frame, aligned_face, warp_matrix, Size(out_width, out_height), INTER_LINEAR);
}

// Aligning a face to a common reference frame
void AlignFaceMask(Mat& aligned_face, const Mat& frame, const CLM& clm_model, const Mat_<int>& triangulation, bool rigid, double sim_scale, int out_width, int out_height)
{
  // Will warp to scaled mean shape
  Mat_<double> similarity_normalised_shape = clm_model.pdm.mean_shape * sim_scale;
  ...
  split(aligned_face, aligned_face_channels);

  for (size_t i = 0; i < aligned_face_channels.size(); ++i) {
    aligned_face_channels[i] = aligned_face_channels[i].mul(paw.pixel_mask);
  }

  merge(aligned_face_channels, aligned_face);

}


#ifdef _USE_DLIB
void Visualise_FHOG(const Mat_<double>& descriptor, int num_rows, int num_cols, Mat& visualisation)
{

  // First convert to dlib format
  dlib::array2d<dlib::matrix<float, 31, 1> > hog(num_rows, num_cols);

  const double* descriptor_it = descriptor.ptr();
...
void Extract_FHOG_descriptor(Mat_<double>& descriptor, const Mat& image, int& num_rows, int& num_cols, int cell_size)
{
  ...
  MatIterator_<double> descriptor_it = descriptor.begin();
...
void ExtractSummaryStatistics(const Mat_<double>& descriptors, Mat_<double>& sum_stats, bool use_mean, bool use_stdev, bool use_max_min)
{
  ...
  for (int i = 0; i < descriptors.cols; ++i) {
    Scalar mean, stdev;
    meanStdDev(descriptors.col(i), mean, stdev);
    ...
    if (use_max_min) {
      double min, max;
      minMaxIdx(descriptors.col(i), &min, &max);
...
void AddDescriptor(Mat_<double>& descriptors, Mat_<double> new_descriptor, int curr_frame, int num_frames_to_keep)
{
