...
public:
  ...
  vector<vector<vector<vector<Mat > > > > cnn_convolutional_layers;
  // Bit ugly with so much nesting, but oh well
  vector<vector<vector<vector<pair<int, Mat > > > > > cnn_convolutional_layers_dft;
  vector<vector<vector<float > > > cnn_convolutional_layers_bias;
  vector< vector<int> > cnn_subsampling_layers;
  vector< vector<Mat > > cnn_fully_connected_layers;
  ...
  vector<Mat > mean_images;
  vector<Mat > standard_deviations;
  ...
  DetectionValidator(const DetectionValidator& other): orientations(other.orientations), bs(other.bs), paws(other.paws),
    ...
    for (i = 0; i < other.ws_nn.size(); ++i) {
      this->ws_nn[i].resize(other.ws_nn[i].size());

      for (k = 0; k < other.ws_nn[i].size(); ++k) {
        // Make sure the matrix is copied.
    ...
    for (v = 0; v < other.cnn_fully_connected_layers.size(); ++v) {
      this->cnn_fully_connected_layers[v].resize(other.cnn_fully_connected_layers[v].size());

      for (l = 0; l < other.cnn_fully_connected_layers[v].size(); ++l) {
        // Make sure the matrix is copied.
    ...
    for (i = 0; i < other.mean_images.size(); ++i) {
      // Make sure the matrix is copied.
    ...
    for (i = 0; i < other.standard_deviations.size(); ++i) {
      // Make sure the matrix is copied.
  ...
  double Check(const Vec3d& orientation, const Mat& intensity_img, Mat& detected_landmarks);

  // Reading in the model
  void Read(const char* location);

  // Getting the closest view center based on orientation
  int GetViewId(const Vec3d& orientation) const;
...
private:
  ...
  double CheckSVR(const Mat& warped_img, int view_id);

  // Feed-forward Neural Network
  double CheckNN(const Mat& warped_img, int view_id);

  // Convolutional Neural Network
  double CheckCNN(const Mat& warped_img, int view_id);

  // A normalisation helper
  void NormaliseWarpedToVector(const Mat& warped_img, Mat& feature_vec, int view_id);
