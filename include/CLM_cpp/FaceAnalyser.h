...
public:
  ...
  void AddNextFrame(const Mat& frame, const CLM& clm, double timestamp_seconds, bool online = false, bool visualise = true);

  // If the features are extracted manually (shouldn't really be used)
  void PredictAUs(const Mat_<double>& hog_features, const Mat_<double>& geom_features, const CLM& clm_model, bool online);
...
private:
  ...
  void UpdateRunningMedian(Mat_<unsigned int>& histogram, int& hist_sum, Mat_<double>& median, const Mat_<double>& descriptor, bool update, int num_bins, double min_val, double max_val);
  void ExtractMedian(Mat_<unsigned int>& histogram, int hist_count, Mat_<double>& median, int num_bins, double min_val, double max_val);
  ...
  vector<Mat_<unsigned int>> au_prediction_correction_histogram;
  ...
  Mat_<double> AU_prediction_track;
  Mat_<double> geom_desc_track;
