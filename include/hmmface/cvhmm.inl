...
//    Notes:
//F*/
static CvStatus icvCreateObsInfo(CvImgObsInfo* obs, CvSize num_obs, int obs_size)
{
  int total = num_obs.h * num_obs.w;
  obs->obs_x = num_obs.w;
  obs->obs_y = num_obs.h;
...
static CvStatus icvReleaseObsInfo(CvImgObsInfo* obs_info)
{
  cvFree(&(obs_info->obs));
  cvFree(&(obs_info->mix));
...
//
//F*/
static CvEHMM* icvCreate2DHMM(const int* state_number, const int* num_mix, int obs_size)
{
  int i;
  int real_states = 0;
  ...
  /* organize memory */
  for (i = 0; i < real_states; i++) {
    all_states[i].mu      = pointers;
    pointers += num_mix[i] * obs_size;
    all_states[i].inv_var = pointers;
    pointers += num_mix[i] * obs_size;
    all_states[i].log_var_val = pointers;
    pointers += num_mix[i];
    all_states[i].weight      = pointers;
    pointers += num_mix[i];
  }
...
static CvStatus icvRelease2DHMM(CvEHMM** phmm)
{
  CvEHMM* hmm = phmm[0];
  int i;
...
static CvStatus icvUniformImgSegm(CvImgObsInfo* obs_info, CvEHMM* hmm)
{
#if 1
  ...
  /* check bad arguments */
  if (hmm->num_states > obs_info->obs_y) {
...
//    Notes:
//F*/
static CvStatus icvInitMixSegm(CvImgObsInfo* obs_info_array, int num_img, CvEHMM* hmm)
{
  int  k, i, j;
  ...
  //CvTermCriteria criteria = cvTermCriteria(CV_TERMCRIT_EPS | CV_TERMCRIT_ITER, 1000,    /* iter */ 0.01f);  /* eps  */
  int total = 0;
  CvEHMMState* first_state = hmm->u.ehmm->u.state;
  for (i = 0 ; i < hmm->num_states; i++) {
  ...
  for (i = 0; i < total; i++) {
    if (first_state[i].num_mix == 1) {
      ...
      /* clusterize vectors  */
      cvKMeans_32f(first_state[i].num_mix, samples[i], num_samples[i], obs_info_array[0].obs_size, a_class[i]);
      //cvKMeans(first_state[i].num_mix, samples[i], num_samples[i], obs_info_array[0].obs_size, criteria, a_class[i]);
    }
  }
...
static float icvComputeGaussMixture(CvVect32f vect, float* mu,
...
//    Notes:
//F*/
static CvStatus icvEstimateObsProb(CvImgObsInfo* obs_info, CvEHMM* hmm)
{
  int i, j;
  int total_states = 0;
...
#define MAX_BUF_SIZE  1200
    ...
    for (i = 0; i < hmm->num_states; i++) {
      ...
      for (j = 0; j < obs_info->obs_y; j++) {
        ...
        /* 2. calculate exponent of log_mix_prob (i.e. probability for each mixture) */
        icvExp_32f64f(log_mix_prob, mix_prob, max_mix * obs_x * n_states); //cvbFastExp
        ...
        icvLog_64f32f(mix_prob, ehmm->obsProb[j], obs_x * n_states);
      }
    }
    if (log_mix_prob != local_log_mix_prob) {
      cvFree(&log_mix_prob);
    }
    return res;
#undef MAX_BUF_SIZE
...
//    Notes:
//F*/
CvStatus icvEstimateTransProb(CvImgObsInfo* obs_info_array, int num_img, CvEHMM* hmm)
{
  int  i, j, k;
  CvEHMMState* first_state = hmm->u.ehmm->u.state;
  ...
  for (i = 0; i < hmm->num_states; i++) {
    ...
    for (j = 0; j < hmm->num_states; j++) {
      hmm->transP[i * hmm->num_states + j] = hmm->transP[i * hmm->num_states + j] ?
  ...
  for (k = 0; k < hmm->num_states; k++) {
    ...
    for (i = 0; i < ehmm->num_states; i++) {
      ...
      inv_total = total ? 1.f / total :  0;
      for (j = 0; j < ehmm->num_states; j++) {
        ehmm->transP[i * ehmm->num_states + j] = (ehmm->transP[i * ehmm->num_states + j]) ?
...
//    Notes:
//F*/
static CvStatus icvMixSegmL2(CvImgObsInfo* obs_info_array, int num_img, CvEHMM* hmm)
{
  int     k, i, j, m;
...
#endif
static CvStatus icvViterbiSegmentation(int num_states, int num_obs, CvMatr32f transP,
...
//    Notes:
//F*/
static float icvEViterbi(CvImgObsInfo* obs_info, CvEHMM* hmm)
{
  int    i, j, counter;
...
CvStatus icvEstimateHMMStateParams(CvImgObsInfo* obs_info_array, int num_img, CvEHMM* hmm)
{
  ...
  for (i = 0; i < total; i++) {
    memset((void*)first_state[i].mu, 0, first_state[i].num_mix * vect_len * sizeof(float));
    memset((void*)first_state[i].inv_var, 0, first_state[i].num_mix * vect_len * sizeof(float));
  }
  /* compute sums */
  for (i = 0; i < num_img; i++) {
    CvImgObsInfo* info = obs_info_array + i;
  ...
  for (i = 0; i < total; i++) {
    ...
    for (m = 0; m < state->num_mix; m++) {
      ...
      icvInvSqrt_32f(invar, invar, vect_len);
      // icvPow_
    }
  }
  /***************Weights***********************/
