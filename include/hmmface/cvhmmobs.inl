...
//                          obsSize.w * obsSize.h floats.
//                          The following conditions should be satisfied:
//                          0 < objSize.w <= dctSize.w,
//                          0 < objSize.h <= dctSize.h.
...
//
//             Nx = floor((roi.w - dctSize.w + delta.w)/delta.w);
//             Ny = floor((roi.h - dctSize.h + delta.h)/delta.h);
//
//             So, total number of observation vectors is Nx*Ny, and total size of
//             array obs must be >= Nx*Ny*obsSize.w*obsSize.h*sizeof(float).
...
//               (x*delta.w,y*delta.h) in the source image
...
//                   for( x = 0; x < Nx; x++ )
//                   {
//                       D = DCT(I[x*delta.w : x*delta.w + dctSize.w,
//                                  y*delta.h : y*delta.h + dctSize.h]);
//                       O[x,y] = D[0:obsSize.w, 0:obsSize.h];
//                   }
//               }
//F*/
...
static CvStatus icvImgToObs_DCT_8u32f_C1R(const uchar* img, int imgStep, CvSize roi, float* obs, CvSize dctSize, CvSize obsSize, CvSize delta)
{
  ...
  int y;
  int Nx, Ny;
  int n1 = dctSize.h, m1 = n1 / 2;
  int n2 = dctSize.w, m2 = n2 / 2;
  if (!img || !obs) {
  ...
  if (obsSize.w <= 0 || dctSize.w < obsSize.w ||
      obsSize.h <= 0 || dctSize.h < obsSize.h) {
    return CV_BADRANGE_ERR;
  }
  if (dctSize.w > MAX_DCT_SIZE || dctSize.h > MAX_DCT_SIZE) {
    return CV_BADRANGE_ERR;
  }
  Nx = (roi.w - dctSize.w + delta.w) / delta.w;
  Ny = (roi.h - dctSize.h + delta.h) / delta.h;
  if (Nx <= 0 || Ny <= 0) {
    return CV_BADRANGE_ERR;
  }
  buffer = (work_t*)cvAlloc(roi.w * obsSize.h * sizeof(buffer[0]));
  if (!buffer) {
    return CV_OUTOFMEM_ERR;
  }
  icvCalcDCTMatrix(tab_x, dctSize.w);
  icvCalcDCTMatrix(tab_y, dctSize.h);
  buf_limit = buffer + obsSize.h * roi.w;
  for (y = 0; y < Ny; y++, img += delta.h * imgStep) {
    int x, i, j, k;
    work_t k0 = 0;
    /* do transfroms for each column. Calc only first obsSize.h DCT coefficients */
    for (x = 0; x < roi.w; x++) {
      float is = 0;
      work_t* buf = buffer + x;
      work_t* tab = tab_y + 2;
    ...
    for (x = 0; x + dctSize.w <= roi.w; x += delta.w) {
      for (i = 0; i < obsSize.h; i++) {
        work_t* buf = buffer + x + roi.w * i;
        work_t* tab = tab_x + 2;
        float* obs_limit = obs + obsSize.w;
        s = 0;
        if (n2 & 1) {
          s = buf[m2];
          k0 = (work_t)(s * tab[-1]);
...
#if 0
CV_IMPL void cvImgToObs_DCT(const void* arr, float* obs, CvSize dctSize, CvSize obsSize, CvSize delta)
{
