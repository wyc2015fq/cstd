...
BOOL CUDDlg::OnInitDialog() 
{
	...
	m_listTree.InsertColumn( 0, _T("select"), LVCFMT_CENTER, (width / 3) / 2 );
	m_listTree.InsertColumn( 1, _T("ID"), LVCFMT_CENTER, (width / 3) / 2 );
	if ( m_flag == OPER_TIMEGROUP )
	{		
		m_listTree.InsertColumn( 2, _T("Time group"), LVCFMT_CENTER, width - (width / 3)  );
		LoadTimegroup();
	}
	else if ( m_flag == OPER_RIGHT )
	{
		m_listTree.InsertColumn( 2, _T("Author"), LVCFMT_CENTER, width - (width / 3)  );
		LoadRight();
	}
	else
	{
		m_listTree.InsertColumn( 2, _T("User Group"), LVCFMT_CENTER, width - (width / 3)  );
		LoadUserGroup();
	}
...
void CUDDlg::UpLoadTimeGroup()
{
	...
	if ( index > 0 )
	{
		...
		DEBUG_INFO1( _T("%s Time Group upload is complete"), m_IP );
	}
	delete [] pGroups;
}
...
void CUDDlg::UpLoadRight()
{
	...
	if ( index > 0 )
	{
 		DEV_RIGHT_ARRAY tmp = { index , pRight};
 		CPM_ULRight(m_IP, tmp);
		DEBUG_INFO1( _T("%s Author upload is complete"), m_IP );
	}
	delete [] pRight;
}
...
void CUDDlg::UpUserGroup()
{
	...
	if ( index > 0 )
	{
		DEV_USERGROUP_ARRAY tmp = { index, pRight };
...
void CUDDlg::TimeGroupInfo( const char* dev_addr, const DEV_TIMEGROUP_ARRAY& time_group )
{
	...
	for (int i = 0; i < time_group.m_nCount; ++i)
	{
		...
		DEBUG_INFO1( _T("Time group %d download is complete"), pTmp->m_TGID );
		pTmp++;
	}
...
// 			mode |= pTmp->m_bSeqOpenDoor << 7;
		...
		DEBUG_INFO1( _T("Author %d download is complete"), pTmp->m_RightID );
		pTmp++;
	}
}
...
void CUDDlg::DownDelTimeGroup()
{
	...
	if ( index > 0 )
	{
		DEV_TIMEGROUP_ARRAY tmp = { index, pGroups };
 		CPM_DELTimeGroup(m_IP, tmp, DEV_ANY_TIME);
		DEBUG_INFO1( _T("%s Deleting completed time group"), m_IP );
	}
	delete [] pGroups;
}
...
void CUDDlg::DownDelRight()
{
	...
	if ( index > 0 )
	{
		...
		DEBUG_INFO1( _T("%s Permissions to delete completed"), m_IP );
	}
	delete [] pRight;
}
...
void CUDDlg::DelUserGroup()
{
	...
	if ( index > 0 )
	{
		...
		DEBUG_INFO1( _T("%s Permissions to delete complete"), m_IP );
	}
	delete [] pRight;
}
