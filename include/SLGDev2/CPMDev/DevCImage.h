...
{
public:
  DevCImage(const CSock* s, NCMD* pCMD, BOOL bServer, void* pvClientData);
  DevCImage(const CSock* s, const DEV_DL_RECORD_RANGE& cRange, int nFlag, int nSeq, char* pbBuf, unsigned int& nLen);  //обть
private:
  void ProcessNCMD(const CSock* s, NCMD* pCMD, BOOL Server, void* pvClientData);
};
