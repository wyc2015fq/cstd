...
public:
  ...
  virtual ~SuperCmd() {}
public:
  virtual void Recv(const CSock* s, NCMD* pNCMD, BOOL bServer, void* pvClientData) = 0;
protected:
  BOOL m_bMax;
};
template <class T>
...
public:
  ...
  virtual ~DevCmd() {}
public:
  virtual void Recv(const CSock* s, NCMD* pNCMD, BOOL  bServer, void* pvClientData) {
    T dev_data(s, pNCMD, bServer, pvClientData);
  }
};
template <class T>
class AutoRegister
