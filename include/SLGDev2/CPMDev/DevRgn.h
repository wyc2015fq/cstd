...
{
public:
  DevRgn(const CSock* s, NCMD* pCMD, BOOL bServer, void* pvClientData);
  DevRgn(const CSock* s, const char* cszGroupIP, int nSeq, char* pbBuf, unsigned int& nLen);//设置
  DevRgn(const CSock* s, int nSeq, char* pbBuf, unsigned int& nLen);//获取
private:
  void ProcessNCMD(const CSock* s, NCMD* pCMD, BOOL bServer, void* pvClientData);
  void SetRgn(const CSock* s, NCMD* pCMD, BOOL bServer, void* pvClientData);
  void GetRgn(const CSock* s, NCMD* pCMD, BOOL bServer, void* pvClientData);
};
