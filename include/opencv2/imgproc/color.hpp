...
#ifndef _OPENCV2_IMGPROC_COLOR_HPP_
#define _OPENCV2_IMGPROC_COLOR_HPP_
...
enum SizePolicy
{
    SizePolicy_TO_YUV, SizePolicy_FROM_YUV, SizePolicy_NONE
};

template< typename VScn, typename VDcn, typename VDepth, SizePolicy sizePolicy = SizePolicy_NONE >
struct CvtHelper
{
    CvtHelper(InputArray _src, OutputArray _dst, int dcn)
    {
        ...
        switch (sizePolicy)
        {
        case SizePolicy_TO_YUV:
            CV_Assert( sz.width % 2 == 0 && sz.height % 2 == 0);
            dstSz = Size(sz.width, sz.height / 2 * 3);
            break;
        case SizePolicy_FROM_YUV:
            CV_Assert( sz.width % 2 == 0 && sz.height % 3 == 0);
            dstSz = Size(sz.width, sz.height * 2 / 3);
            break;
        case SizePolicy_NONE:
        default:
            dstSz = sz;
            break;
        }
...
#endif // _OPENCV2_IMGPROC_COLOR_HPP_
