...
#define LIST_DEL0(item)   if (item->prev) { item->prev->next = item->next; } if (item->next) { item->next->prev = item->prev; }
...
CC_INLINE void list_add_after(struct list* elem, struct list* to_add)
{
...
CC_INLINE void list_add_before(struct list* elem, struct list* to_add)
{
  to_add->next = elem;
...
CC_INLINE void list_add_head(struct list* list, struct list* elem)
{
  list_add_after(list, elem);
}

/* add element at the tail of the list */
CC_INLINE void list_add_tail(struct list* list, struct list* elem)
{
  list_add_before(list, elem);
}

/* remove an element from its list */
CC_INLINE void list_remove(struct list* elem)
{
...
CC_INLINE struct list* list_next(const struct list* list, const struct list* elem) {
...
CC_INLINE struct list* list_prev(const struct list* list, const struct list* elem) {
...
CC_INLINE struct list* list_head(const struct list* list) {
  return list_next(list, list);
}

/* get the last element */
CC_INLINE struct list* list_tail(const struct list* list) {
  return list_prev(list, list);
}

/* check if a list is empty */
CC_INLINE int list_empty(const struct list* list)
{
  return list->next == list;
}

/* initialize a list */
CC_INLINE void list_init(struct list* list)
{
  list->next = list->prev = list;
}

/* count the elements of a list */
CC_INLINE unsigned int list_count(const struct list* list)
{
  unsigned count = 0;
...
CC_INLINE void list_move_tail(struct list* dst, struct list* src)
{
  if (list_empty(src)) {
    return;
  }
...
CC_INLINE void list_move_head(struct list* dst, struct list* src)
{
  if (list_empty(src)) {
    return;
  }
