...
#define DO_BLEND  a = (a*ca) >> 8
#define  ICC_BLEND1(tptr)            \
        {                                   \
            _cb = tptr[0];                  \
            _cb += ((cb - _cb)*a + 127)>> 8;\
            tptr[0] = (uchar)_cb;           \
        }
#define  ICC_BLEND3(tptr)            \
        {                                   \
            _cb = tptr[0];                  \
            _cb += ((cb - _cb)*a + 127)>> 8;\
            _cg = tptr[1];                  \
...
ISIZE iSIZE(img_t* im) {
  ...
  static void
    PolyLine(img_t* img, const IPOINT* v, int npts, bool closed,
      const void* color, int thickness = 1, int line_type = LINE_8, int shift = 0);

  static void
    FillConvexPoly(img_t* img, const IPOINT* v, int npts,
      const void* color, int line_type = LINE_8, int shift = 0);
...
#define CC_INSTRUMENT_REGION()
  ...
  bool clipLine(ISIZE img_size, IPOINT& pt1, IPOINT& pt2)
  {
    ...
    int64_type &x1 = pt1.x, &y1 = pt1.y, &x2 = pt2.x, &y2 = pt2.y;
  ...
  bool clipLine1(ISize img_size, IPOINT& pt1, IPOINT& pt2)
  {
    ...
    bool inside = clipLine(lSize2(img_size.width, img_size.height), p1, p2);
    pt1.x = (int)p1.x;
    pt1.y = (int)p1.y;
    pt2.x = (int)p2.x;
    pt2.y = (int)p2.y;
...
//#define ASSERT assert
  ...
  */
  ...
  //LSize lSize2(ISize sz) { return lSize2(sz.w, sz.h); }

  static void LineAA(img_t* img, IPOINT pt1, IPOINT pt2, const void* color)
  {
    int64_t dx, dy;
    int ecount, scount = 0;
    int slope;
    ...
    {
      Line(img, iPoint((int)(pt1.x >> XY_SHIFT), (int)(pt1.y >> XY_SHIFT)), iPoint((int)(pt2.x >> XY_SHIFT), (int)(pt2.y >> XY_SHIFT)), color);
      return;
    }
...
#undef ICC_PUT_POINT
  ...
  static void Line2(img_t* img, IPOINT pt1, IPOINT pt2, const void* color)
  {
    int64_t dx, dy;
    int ecount;
    int64_t ax, ay;
    ...
    ISIZE sizeScaled = lSize2(((int64_type)size.width) << XY_SHIFT, ((int64_type)size.height) << XY_SHIFT);
...
#define CC_OUT
  ...
  */
  ...
  static void
    EllipseEx(img_t* img, IPOINT center, ISIZE axes,
      int angle, int arc_start, int arc_end,
      const void* color, int thickness = 1, int line_type = LINE_8)
  {
    axes.width = ABS(axes.width), axes.height = ABS(axes.height);
    int delta = (int)((MAX(axes.width, axes.height) + (XY_ONE >> 1)) >> XY_SHIFT);
    delta = delta < 3 ? 90 : delta < 10 ? 30 : delta < 15 ? 18 : 5;

    std::vector<Point2d> _v;
    ellipse2Poly(dPoint((double)center.x, (double)center.y), dSize((double)axes.width, (double)axes.height), angle, arc_start, arc_end, delta, _v);

    std::vector<IPOINT> v;
    IPOINT prevPt = lPoint2(-1, -1);
    v.resize(0);
    for (unsigned int i = 0; i < _v.size(); ++i)
    {
      IPOINT pt;
      pt.x = (int64_type)ROUND(_v[i].x / XY_ONE) << XY_SHIFT;
      pt.y = (int64_type)ROUND(_v[i].y / XY_ONE) << XY_SHIFT;
      pt.x += ROUND(_v[i].x - pt.x);
      pt.y += ROUND(_v[i].y - pt.y);
      if (pt != prevPt) {
        v.push_back(pt);
        prevPt = pt;
      }
    ...
    else
    {
      ...
      CollectPolyEdges(img, &v[0], (int)v.size(), edges, color, line_type, XY_SHIFT, iPOINT(0,0));
  ...
  *                                Polygons filling                                        *
  ...
  static inline void ICC_HLINE_X(uchar* ptr, int xl, int xr, const uchar* color, int pix_size)
  {
    int pix_num = xr - xl + 1;
    uchar* hline_min_ptr = (uchar*)(ptr)+(xl)*(pix_size);
    BLEND_LINE(hline_min_ptr, color, pix_size, pix_num);
  }
  //end ICC_HLINE_X()

  static inline void ICC_HLINE(uchar* ptr, int xl, int xr, const void* color, int pix_size)
  {
    ICC_HLINE_X(ptr, xl, xr, reinterpret_cast<const uchar*>(color), pix_size);
  }
  //end ICC_HLINE()

  /* filling convex polygon. v - array of vertices, ntps - number of points */
  static void
    FillConvexPoly(img_t* img, const IPOINT* v, int npts, const void* color, int line_type, int shift)
  {
    struct
    {
      int idx, di;
        ...
        for (i = 0; i < 2; i++)
        {
          if (y >= edge[i].ye)
          {
            ...
            for (; edges-- > 0; )
            {
              ...
              if (ty > y)
              {
                int64_type xs = v[idx0].x;
                int64_type xe = v[idx].x;
                if (shift != XY_SHIFT)
                {
                  xs <<= XY_SHIFT - shift;
                  xe <<= XY_SHIFT - shift;
      ...
      const void* color, int line_type, int shift, IPOINT offset)
  {
    ...
    for (i = 0; i < count; i++, pt0 = pt1)
    {
      ...
      if (line_type < CC_AA)
      {
        ...
        Line(img, iPOINT(t0.x, t0.y), iPOINT(t1.x, t1.y), color, line_type);
      }
      else
      {
    ...
    FillEdgeCollection(img_t* img, std::vector<PolyEdge>& edges, const void* color)
  {
    ...
    for (i = 0; i < total; i++)
    {
      ...
      int64_type x1 = e1.x + (e1.y1 - e1.y0) * e1.dx;
    ...
    if (y_max < 0 || y_min >= size.height || x_max < 0 || x_min >= ((int64_type)size.width << XY_SHIFT))
      return;
...
//#define ICC_PUT_POINT( ptr, x )     memcpy( ptr + (x)*pix_size, color, pix_size );
#define ICC_PUT_POINT( ptr, x )     blend( ptr + (x)*pix_size, color, a, pix_size );
...
#undef  ICC_PUT_POINT
  ...
  static void ThickLine(img_t* img, IPOINT p0, IPOINT p1, const void* color, int thickness = 1, int line_type = LINE_8, int flags = 0, int shift = 0)
  {
    ...
    else
    {
      ...
      for (i = 0; i < 2; i++)
      {
        if (flags & (i + 1))
        {
          ...
          {
            EllipseEx(img, p0, lSize2(thickness, thickness),
  ...
  static void
    PolyLine(img_t* img, const IPOINT* v, int count, bool is_closed,
  ...
  *                              External functions                                        *
  ...
  void drawMarker(img_t* img, IPOINT position, color_t color, int markerType, int markerSize, int thickness = 1, int line_type = LINE_8)
  {
    ...
    case MARKER_CROSS:
      line(img, iPoint(position.x - (markerSize / 2), position.y), iPoint(position.x + (markerSize / 2), position.y), color, thickness, line_type);
      line(img, iPoint(position.x, position.y - (markerSize / 2)), iPoint(position.x, position.y + (markerSize / 2)), color, thickness, line_type);
      break;

      // The tilted cross marker case
    case MARKER_TILTED_CROSS:
      line(img, iPoint(position.x - (markerSize / 2), position.y - (markerSize / 2)), iPoint(position.x + (markerSize / 2), position.y + (markerSize / 2)), color, thickness, line_type);
      line(img, iPoint(position.x + (markerSize / 2), position.y - (markerSize / 2)), iPoint(position.x - (markerSize / 2), position.y + (markerSize / 2)), color, thickness, line_type);
      break;
    ...
    case MARKER_DIAMOND:
      line(img, iPoint(position.x, position.y - (markerSize / 2)), iPoint(position.x + (markerSize / 2), position.y), color, thickness, line_type);
      line(img, iPoint(position.x + (markerSize / 2), position.y), iPoint(position.x, position.y + (markerSize / 2)), color, thickness, line_type);
      line(img, iPoint(position.x, position.y + (markerSize / 2)), iPoint(position.x - (markerSize / 2), position.y), color, thickness, line_type);
      line(img, iPoint(position.x - (markerSize / 2), position.y), iPoint(position.x, position.y - (markerSize / 2)), color, thickness, line_type);
      break;
    ...
    case MARKER_TRIANGLE_UP:
      line(img, iPoint(position.x - (markerSize / 2), position.y + (markerSize / 2)), iPoint(position.x + (markerSize / 2), position.y + (markerSize / 2)), color, thickness, line_type);
      line(img, iPoint(position.x + (markerSize / 2), position.y + (markerSize / 2)), iPoint(position.x, position.y - (markerSize / 2)), color, thickness, line_type);
      line(img, iPoint(position.x, position.y - (markerSize / 2)), iPoint(position.x - (markerSize / 2), position.y + (markerSize / 2)), color, thickness, line_type);
      break;
    ...
    int thickness = 1, int line_type = LINE_8, int shift = 0, double tipLength = 0)
  {
    ...
    p.x = ROUND(pt2.x + tipSize * ::cos(angle - CC_PI / 4));
    p.y = ROUND(pt2.y + tipSize * ::sin(angle - CC_PI / 4));
    line(img, p, pt2, color, thickness, line_type, shift);
  }
    ...
    color_t color, int thickness = 1, int line_type = LINE_8, int shift = 0)
  {
    ...
    {
      IPOINT _center = lPoint2(center.x, center.y);
      int64_type _radius(radius);
      _center.x <<= XY_SHIFT - shift;
      _center.y <<= XY_SHIFT - shift;
      _radius <<= XY_SHIFT - shift;
      EllipseEx(img, _center, lSize2(_radius, _radius),
    ...
    color_t color, int thickness = 1, int line_type = LINE_8, int shift = 0)
  {
    ...
    IPOINT _center = lPoint2(center.x, center.y);
    ISIZE _axes = lSize2(axes.w, axes.h);
    _center.x <<= XY_SHIFT - shift;
    _center.y <<= XY_SHIFT - shift;
  ...
  void ellipse(img_t* img, const CRotatedRect& box, color_t color, int thickness = 1, int lineType = LINE_8)
  {
    ...
    int _angle = ROUND(box.angle);
    IPOINT center = lPoint2(ROUND(box.center.x), ROUND(box.center.y));
    center.x = (center.x << XY_SHIFT) + ROUND((box.center.x - center.x)*XY_ONE);
    center.y = (center.y << XY_SHIFT) + ROUND((box.center.y - center.y)*XY_ONE);
    ISIZE axes = lSize2(ROUND(box.size.width),
      ROUND(box.size.height));
    axes.width = (axes.width << (XY_SHIFT - 1)) + ROUND((box.size.width - axes.width)*(XY_ONE >> 1));
    axes.height = (axes.height << (XY_SHIFT - 1)) + ROUND((box.size.height - axes.height)*(XY_ONE >> 1));
    ...
    color_t color, int line_type = LINE_8, int shift = 0, IPOINT offset = iPOINT(0, 0))
  {
    ...
    for (i = 0; i < ncontours; i++)
    {
      std::vector<IPOINT> _pts(pts[i], pts[i] + npts[i]);
...

  {
    ...
    int64_type view_x = (int64_type)org.x << XY_SHIFT;
    int64_type view_y = ((int64_type)org.y << XY_SHIFT) + base_line*vscale;
    ...
    for (int i = 0; i < (int)text.size(); i++)
    {
      ...
      for (ptr += 2;; )
      {
        ...
        else
        {
          ...
          p.y = (uchar)ptr[1] - 'R';
          ptr += 2;
          pts.push_back(IPOINT(p.x*hscale + view_x, p.y*vscale + view_y));
        }
      }
      view_x += dx;
    }
  ...
  ISize getTextSize(const String& text, int fontFace, double fontScale, int thickness, int* _base_line)
  {
    ...
    size.height = ROUND((cap_line + base_line)*fontScale + (thickness + 1) / 2);
    ...
    size.width = ROUND(view_x + thickness);
    if (_base_line)
      *_base_line = ROUND(base_line*fontScale + thickness*0.5);
    return size;
  }
...
void cv::fillConvexPoly(img_t* img, InputArray _points,
  color_t color, int lineType, int shift = 0)
{
  CC_INSTRUMENT_REGION()
...
void cv::polylines(img_t* img, InputArrayOfArrays pts,
  bool isClosed, color_t color,
  int thickness = 1, int lineType, int shift = 0)
{
  CC_INSTRUMENT_REGION()
  ...
  int line_type, CvPoint _offset)
{
  ...
  while ((contour = (CvSeq*)cvNextTreeNode(&iterator)) != 0)
  {
    ...
    int elem_type = CC_MAT_TYPE(contour->flags);
...
cvColorToScalar(double packed_color, int type)
{
  ...
  if (CC_MAT_DEPTH(type) == CC_8U)
  {
    int icolor = ROUND(packed_color);
    if (CC_MAT_CN(type) > 1)
    {
  ...
  else if (CC_MAT_DEPTH(type) == CC_8S)
  {
    int icolor = ROUND(packed_color);
    if (CC_MAT_CN(type) > 1)
    {
...
cvCircle(CvArr* _img, CvPoint center, int radius,
  CvScalar color, int thickness = 1, int line_type = LINE_8, int shift = 0)
{
  cv::Mat img = cv::cvarrToMat(_img);
