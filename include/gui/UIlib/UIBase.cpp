...
void CWindowWnd::SetIcon(UIRes nRes)
{
#if 0
  ...
  HICON hIcon = CreateIconFromResourceEx((PBYTE)nRes.buf, nRes.len, TRUE, 0x00030000, ::GetSystemMetrics(SM_CXICON), ::GetSystemMetrics(SM_CYICON), LR_DEFAULTCOLOR);
  hIcon = CreateFromBuff((PBYTE)nRes.buf, nRes.len);
  ASSERT(hIcon);
  ::SendMessage(m_hWnd, WM_SETICON, (WPARAM)TRUE, (LPARAM)hIcon);
  hIcon = CreateIconFromResourceEx((PBYTE)nRes.buf, nRes.len, TRUE, 0x00030000, ::GetSystemMetrics(SM_CXSMICON), ::GetSystemMetrics(SM_CXSMICON), LR_DEFAULTCOLOR);
  hIcon = CreateFromBuff((PBYTE)nRes.buf, nRes.len);
  ASSERT(hIcon);
  ::SendMessage(m_hWnd, WM_SETICON, (WPARAM)FALSE, (LPARAM)hIcon);
}
