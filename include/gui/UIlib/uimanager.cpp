...
//#define IDB_ICONS50           "res/icons50.bmp"
//#define IDB_ICONS16           "res/icons16.bmp"
...
//从缓冲区中创建一个位图句柄
HBITMAP LoadBitmapFromBuffer(UIRes nRes)
{
	LPBYTE pBuffer = nRes.buf;
	HBITMAP hBmp = NULL;
	HDC hdc = NULL;
...
HIMAGELIST ImageList_LoadImageFromFile(UIRes nres, int size) {
	HINSTANCE m_hInstance = CPaintManagerUI::GetResourceInstance();
	return ImageList_LoadImage(m_hInstance, (LPSTR)nres.buf, size, 0, RGB(255, 0, 255), IMAGE_BITMAP, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
}

HIMAGELIST ImageList_LoadImageFromBuffer(UIRes nres, int size) {
	HBITMAP hbm = LoadBitmapFromBuffer(nres);
	HIMAGELIST himg = ImageList_Create(size, size, ILC_COLOR32 | ILC_MASK, 9, 4);  //这里的参数请根据图片自行调整
	ImageList_AddMasked(himg, hbm, 0x00FF00FF); //指定紫红为透明色
	DeleteBitmap(hbm);
	return himg;
}
...
HICON CPaintManagerUI::GetThemeIcon(int iIndex, int cxySize) const
{
  if ( m_himgIcons16 == NULL ) {
    m_himgIcons16 = ImageList_LoadImageFromBuffer(MKRES(IDB_ICONS16), 16);
    m_himgIcons24 = ImageList_LoadImageFromBuffer(MKRES(IDB_ICONS16), 16);
    m_himgIcons32 = ImageList_LoadImageFromBuffer(MKRES(IDB_ICONS16), 16);
    m_himgIcons50 = ImageList_LoadImageFromBuffer(MKRES(IDB_ICONS50), 50);
  }
