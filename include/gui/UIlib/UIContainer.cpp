...
bool CCanvasUI::SetWatermark(LPCTSTR pstrBitmap, int iOrientation)
{
  if ( m_hBitmap != NULL ) { ::DeleteObject(m_hBitmap); }
  char buf[256];
  snprintf(buf, 256, "res/%s.bmp", pstrBitmap);
  m_hBitmap = (HBITMAP) ::LoadImage(m_pManager->GetResourceInstance(), buf, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
