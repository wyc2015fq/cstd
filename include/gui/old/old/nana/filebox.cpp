...
namespace nana
{
		...
		: public form
	{
		...
		private:
			...
			static std::string _m_trans(std::size_t bytes)  /// \todo : use filesystem_ext::pretty_file_size
			{
	...
	public:
			...
			: form(owner, API::make_center(owner, 630, 440)), io_read_(io_read)
		{
			...
			ls_file_.set_sort_compare(0, [](const std::string& a, any* fs_a, const std::string& b, any* fs_b, bool reverse) -> bool
				{
			...
			ls_file_.set_sort_compare(2, [](const std::string& a, any* anyptr_a, const std::string& b, any* anyptr_b, bool reverse) -> bool
				{
			...
			ls_file_.set_sort_compare(3, [this](const std::string&, any* anyptr_a, const std::string&, any* anyptr_b, bool reverse) -> bool
				{
				...
				if(file.size())
				{
					...
					if(io_read_)
					{
						if(false == good)
						{
							...
							mb<<L"The file \""<<charset(tar, unicode::utf8)<<L"\"\n is not existing. Please check and retry.";
							mb();
						}
						else
