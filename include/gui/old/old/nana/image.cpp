...
namespace paint
{
	...
	HICON image_accessor::icon(const paint::image& img)
	{
	...
	namespace detail
	{
		//class image_ico
			...
			size image_ico::size() const
			{
				return size_;
			}

			void image_ico::paste(const rectangle& src_r, graph_reference graph, const point& p_dst) const
			{
			...
			void image_ico::stretch(const rectangle&, graph_reference graph, const rectangle& r) const
			{
				if(ptr_ && (graph.empty() == false))
				{
#if defined(NANA_WINDOWS)
