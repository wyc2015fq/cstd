...
namespace nana
{
	namespace paint
	{
		namespace helper
		{
				...
				unsigned operator()(const int top, const wchar_t * buf, std::size_t bufsize)
				{
					...
					case align::left:
						for(auto & ent : reordered)
						{
							...
							size ts = detail::text_extent_size(dw, ent.begin, len);
			...
			struct draw_string_omitted
			{
				...
				unsigned operator()(const int top, const wchar_t * buf, std::size_t bufsize)
				{
					...
					for(auto & i : reordered)
					{
						...
						size ts = detail::text_extent_size(dw, i.begin, len);
						if(ts.height > pixels)	pixels = ts.height;
						...
						else
						{
							...
							paint::graphics dum_graph({ r.width, r.height });
			...
			struct draw_string_auto_changing_lines
			{
				...
				unsigned operator()(const int top, const wchar_t * buf, std::size_t bufsize)
				{
					...
					for(auto & i : reordered)
					{
						size ts = detail::text_extent_size(dw, i.begin, i.end - i.begin);
						if(ts.height > pixels) pixels = ts.height;
		...
		size text_renderer::extent_size(int x, int y, const wchar_t* str, std::size_t len, unsigned restricted_pixels) const
		{
			size extents;
			if(graph_)
			{
