...
namespace nana{	namespace widgets
{
	namespace skeletons
	{
		...
		public:
			...
			{
				std::function<void(graph_reference, const rectangle& text_area, const ::color&)> background;
			};
			...
			void set_highlight(const ::std::string& name, const ::color&, const ::color&);
			...
			void border_renderer(std::function<void(graph_reference, const ::color& bgcolor)>);
		...
		private:
			...
			void _m_scrollbar();
			::size _m_text_area() const;
			void _m_get_scrollbar_size();
			void _m_reset();
			::upoint _m_put(::std::wstring);
			...
			void _m_draw_parse_string(const keyword_parser&, bool rtl, ::point pos, const ::color& fgcolor, const wchar_t*, std::size_t len) const;
			//_m_draw_string
			//@brief: Draw a line of string
			void _m_draw_string(int top, const ::color&, const upoint& str_pos, const ::std::wstring&, bool if_mask) const;
			...
			bool _m_get_sort_select_points(upoint&, upoint&) const;
		...
		private:
			...
			struct attributes
			{
				...
				paint::graphics counterpart; //this is used to keep the background that painted by external part.

				std::unique_ptr<scroll<true>>		vscroll;
				std::unique_ptr<scroll<false>>	hscroll;
			}attributes_;
			...
			struct text_area_type
			{
				...
				unsigned	vscroll;
				unsigned	hscroll;
				std::function<void(paint::graphics&, const ::color&)> border_renderer;
			}text_area_;
			...
			struct coordinate
			{
				point		offset;	//x stands for pixels, y for lines
