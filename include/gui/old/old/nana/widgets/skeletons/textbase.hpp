	...
	public:
		...
		bool load(const char* file_utf8)
		{
			...
			while(ifs.good())
			{
				...
				text_cont_.emplace_back(static_cast<string_type&&>(charset{ str_mbs }));
		...
		bool load(const char* file_utf8, unicode encoding)
		{
			...
			if(ifs.good())
			{
				...
				text_cont_.emplace_back(static_cast<string_type&&>(charset{ str, encoding }));
		...
		void store(std::string fs, bool is_unicode, ::unicode encoding) const
		{
			...
			if(ofs && text_cont_.size())
			{
				...
				if (is_unicode)
				{
					...
					if (text_cont_.size() > 1)
					{
						...
						{
							std::string(charset(*i).to_bytes(encoding)).swap(mbs);
							mbs += "\r\n";
