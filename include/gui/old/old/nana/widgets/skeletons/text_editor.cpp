...
namespace nana{	namespace widgets
{
	namespace skeletons
	{
		...
		public:
			...
			virtual std::vector<::upoint> render(const ::color& fgcolor) = 0;
			virtual	point		caret_to_screen(upoint) = 0;
			virtual upoint	screen_to_caret(point scrpos) = 0;
		...
		public:
			...
			void update_line(std::size_t textline, std::size_t) override
			{
				...
				editor_._m_draw_string(top, API::fgcolor(editor_.window_), upoint(0, editor_.points_.caret.y), editor_.textbase_.getline(textline), true);
			}

			std::vector<upoint> render(const ::color& fgcolor) override
			{
				std::vector<upoint> line_index;

				::upoint str_pos(0, static_cast<unsigned>(editor_.points_.offset.y));
			...
			point	caret_to_screen(upoint pos) override
			{
			...
			upoint screen_to_caret(point scrpos) override
			{
				upoint res{ 0, static_cast<unsigned>(_m_textline_from_screen(scrpos.y)) };
		...
		public:
			...
			void update_line(std::size_t textline, std::size_t secondary_before) override
			{
				if (take_lines(textline) == secondary_before)
				{
					...
					for (std::size_t pos = 0; pos < secondary_before; ++pos, top+=pixels)
					{
						...
						editor_._m_draw_string(top, fgcolor, upoint(static_cast<unsigned>(sct.begin - text_ptr), editor_.points_.caret.y), std::wstring(sct.begin, sct.end), true);
					}
				}
				else
			...
			upoint screen_to_caret(point scrpos) override
			{
				...
				upoint res(static_cast<unsigned>(real_str.begin - mtr.line_sections.front().begin), static_cast<unsigned>(row.first));
				scrpos.x -= editor_.text_area_.area.x;
				if (scrpos.x < 0)
					scrpos.x = 0;
		...
		private:
			...
			bool _m_pos_from_secondary(std::size_t textline, const upoint& secondary, unsigned & pos)
			{
				...
				scheme_(schm), keywords_(new keywords)
		{
			...
			text_area_.border_renderer = [this](graph_reference graph, const ::color& bgcolor)
			{
				if (!API::widget_borderless(this->window_))
				{
					::facade<element::border> facade;
					facade.draw(graph, bgcolor, API::fgcolor(this->window_), ::rectangle{ API::window_size(this->window_) }, API::element_state(this->window_));
				}
			};
		}
		...
		void text_editor::set_highlight(const std::string& name, const ::color& fgcolor, const ::color& bgcolor)
		{
		...
		void text_editor::border_renderer(std::function<void(paint::graphics&, const ::color&)> f)
		{
		...
		bool text_editor::mouse_enter(bool entering)
		{
			if ((false == entering) && (false == text_area_.captured))
				API::window_cursor(window_, cursor::arrow);
		...
		void text_editor::move_caret(const upoint& crtpos)
		{
			...
			if (hit_text_area(pos) && (line_bottom > text_area_.area.y))
			{
				...
				if (line_bottom > _m_end_pos(false))
					caret_->dimension(size(1, line_pixels - (line_bottom - _m_end_pos(false))));
	...
	//public:
		...
		void text_editor::_m_scrollbar()
		{
			...
			if (text_area_.vscroll)
			{
				...
				if (!wdptr)
				{
					attributes_.vscroll.reset(new scroll<true>);
					wdptr = attributes_.vscroll.get();
					wdptr->create(window_, rectangle(x, text_area_.area.y, text_area_.vscroll, tx_area.height));
			...
			if(text_area_.hscroll)
			{
				...
				if(nullptr == wdptr)
				{
					attributes_.hscroll.reset(new scroll<false>);
					wdptr = attributes_.hscroll.get();
					wdptr->create(window_, rectangle(text_area_.area.x, y, tx_area.width, text_area_.hscroll));
				...
				size text_size = _m_text_extent_size(textbase_.getline(maxline.first).c_str(), maxline.second);
		...
		size text_editor::_m_text_area() const
		{
			return size((text_area_.area.width > text_area_.vscroll ? text_area_.area.width - text_area_.vscroll : 0),
		...
		upoint text_editor::_m_put(std::wstring text)
		{
		...
		void text_editor::_m_draw_parse_string(const keyword_parser& parser, bool rtl, ::point pos, const ::color& fgcolor, const wchar_t* str, std::size_t len) const
		{
		...
		public:
			...
			void write_selection(const point& text_pos, unsigned text_px, const wchar_t* text, std::size_t len)
			{
				...
				graph_.rectangle(::rectangle{ text_pos, { text_px, line_px_ } }, true, selection_bgcolor_);
		...
		void text_editor::_m_draw_string(int top, const ::color& clr, const upoint& text_coord, const std::wstring& text, bool if_mask) const
		{
		...
		bool text_editor::_m_get_sort_select_points(upoint& a, upoint& b) const
		{
			if (select_.a == select_.b)
				return false;
