...
namespace nana
{
	...
	namespace drawerbase
	{
		namespace combox
		{
			...
			public:
				...
				item_proxy&	select();
				bool		selected() const;
				item_proxy& icon(const paint::image&);
				paint::image icon() const;
	...
	public:
		...
		{
			typedef typename detail::type_escape<Key>::type key_t;
			std::shared_ptr<detail::key_interface> p(new key<key_t, std::less<key_t> >(kv), [](detail::key_interface*p)
			{
				delete p;
			});
		...
		{
			typedef typename detail::type_escape<Key>::type key_t;
			std::unique_ptr<detail::key_interface> p(new key<key_t, std::less<key_t> >(kv));
			_m_erase(p.get());
		}
		...
		{
			typedef typename detail::type_escape<Key>::type key_t;
			std::unique_ptr<detail::key_interface> p(new key<key_t, std::less<key_t> >(std::move(kv)));
			_m_erase(p.get());
		}
		...
		void image(std::size_t, const paint::image&);
		paint::image image(std::size_t) const;
		void image_pixels(unsigned);  ///<Sets the width of image area. Default is 16 pixels.
	private:
		item_proxy _m_at_key(std::shared_ptr<detail::key_interface>&&);
		void _m_erase(detail::key_interface*);
	...
	private:
		...
		any * _m_anyobj(std::size_t pos, bool alloc_if_empty) const override;
	};
}
