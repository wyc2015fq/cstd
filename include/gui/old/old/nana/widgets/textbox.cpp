...
namespace drawerbase {
	...
	{
		//class event_agent
			event_agent::event_agent(::textbox& wdg, const std::vector<upoint>& text_pos)
	...
	//class textbox
		...
		void textbox::store(std::string file)
		{
			...
			auto editor = get_drawer_trigger().editor();
			if (editor)
				editor->textbase().store(std::move(file), false, unicode::utf8);	//3rd parameter is just for syntax, it will be ignored
		}

		void textbox::store(std::string file, unicode encoding)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		void textbox::set_highlight(const std::string& name, const ::color& fgcolor, const ::color& bgcolor)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		void textbox::_m_typeface(const paint::font& font)
		{
