...
namespace nana
{
	...
	namespace drawerbase
	{
		namespace menubar
		{
			...
			//class trigger
				...
				menu* trigger::push_back(const std::string& text)
				{
					wchar_t shkey;
				...
				menu* trigger::at(std::size_t pos) const
				{
				...
				void trigger::refresh(graph_reference graph)
				{
					...
					for (auto i : items_->cont())
					{
						...
						size text_s = graph.text_extent_size(text);
						...
						if (hotkey)
						{
							...
							size hotkey_size = graph.text_extent_size(text.c_str() + hotkey_pos, 1);

							unsigned ascent, descent, inleading;
							graph.text_metrics(ascent, descent, inleading);
							int x = item_pos.x + 8 + off_w;
							int y = item_pos.y + text_top_off + ascent + 1;
							graph.line({ x, y }, { x + static_cast<int>(hotkey_size.width) - 1, y }, ::colors::black);
						}
				...
				std::size_t trigger::_m_item_by_pos(const ::point& pos)
				{
				...
				bool trigger::_m_track_mouse(const ::point& pos)
				{
	...
	//class menubar
		...
		void menubar::create(window wd)
		{
			...
			evt_resized_ = API::events(wd).resized.connect([this](const ::arg_resized& arg)
			{
				auto sz = this->size();
				sz.width = arg.width;
				this->size(sz);
