...
namespace nana
{
		...
		namespace label
		{
			...
			public:
				...
				void render(graph_reference graph, const ::color& fgcolor, align th, align_v tv)
				{
					...
					paint::font ft = graph.typeface();	//used for restoring the font
			...
			private:
				...
				void _m_inser_if_traceable(int x, int y, const size& sz, widgets::skeletons::fblock* fbp)
				{
				...
				void _m_set_default(const ::paint::font& ft, const ::color& fgcolor)
				{
				...
				const ::color& _m_fgcolor(widgets::skeletons::fblock* fp)
				{
				...
				std::size_t _m_font_size(widgets::skeletons::fblock* fp)
				{
				...
				bool _m_bold(widgets::skeletons::fblock* fp)
				{
					while(fp->bold_empty)
					{
						fp = fp->parent;
				...
				const std::string& _m_fontname(widgets::skeletons::fblock* fp)
				{
					while(fp->font.empty())
					{
						fp = fp->parent;
			...
			private:
				...
				::widgets::skeletons::fblock * fblock_ = nullptr;
	...
	//class label
		...
		size label::measure(unsigned limited) const
		{
			...
			paint::graphics * graph_ptr = impl->graph;
			paint::graphics substitute;
			if(graph_ptr->empty())
			{
		...
		::size label::measure(paint::graphics& graph, const ::std::string& str, unsigned allowed_width_in_pixel, bool format_enabled, align h_align, align_v v_align)
		{
			throw_not_utf8(str);
