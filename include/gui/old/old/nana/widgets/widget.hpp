...
namespace nana
{
	...
	public:
		...
		{
			_m_caption(internationalization().get(msgid, std::forward<Args>(args)...));
		}

		void i18n(i18n_eval);

		void cursor(cursor);
		cursor cursor() const;		///< Retrieves the shape of cursor
	...
	protected:
		std::unique_ptr<::detail::widget_notifier_interface> _m_wdg_notifier();
	private:
	...
	protected:
		...
		virtual cursor _m_cursor() const;
		virtual void _m_cursor(cursor);
		...
		virtual void _m_size(const size&);
		virtual void _m_move(int x, int y);
		virtual void _m_move(const rectangle&);

		virtual void _m_typeface(const paint::font& font);
		virtual paint::font _m_typeface() const;

		virtual void _m_fgcolor(const color&);
		virtual color _m_fgcolor() const;
		virtual void _m_bgcolor(const color&);
		virtual color _m_bgcolor() const;
	};
	...
	template<typename Category, typename DrawerTrigger, typename Events = ::general_events, typename Scheme = ::widget_geometrics>
	...
	private:
		...
		std::unique_ptr<scheme_type> scheme_;
	        ...
	        /// Base class of all the classes defined as a non-graphics-buffer widget window. The second template parameter DrawerTrigger is always ignored.\see panel
	...
	private:
		...
		std::unique_ptr<scheme_type> scheme_;
	        ...
	        /// Base class of all the classes defined as a root window. \see form
	template<typename DrawerTrigger, typename Events, typename Scheme>
	...
	public:
		...
		void icon(const paint::image& ico)
		{
		...
		void outline_size(const ::size& sz)
		{
			API::window_outline_size(handle(), sz);
		}

		::size outline_size() const
		{
	...
	private:
		...
		std::unique_ptr<scheme_type>	scheme_;
	           ...
	           /// Base class of all the classes defined as a frame window. \see frame
	...
	class widget_object<category::frame_tag, Drawer, Events, Scheme>: public widget{};
	           ...
	           /// Especialization. Base class of all the classes defined as a frame window. \see frame
