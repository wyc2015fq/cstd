...
namespace nana{	namespace drawerbase
{
	namespace button
	{
		...
		void trigger::_m_draw_title(graph_reference graph, bool enabled)
		{
			...
			if(ts.width)
			{
				...
				if(enabled)
				{
					...
					graph.palette(true, attr_.focus_color && attr_.focused ? ::color(colors::blue) : attr_.fgcolor);
					...
					if(shortkey)
					{
						...
						wchar_t keystr[2] = {utf::char_at(mbstr.c_str() + shortkey_pos, 0, 0), 0};
		...
		void draw_corner_point(::paint::graphics& graph, const rectangle& r)
		{
		...
		void trigger::icon(const paint::image& img)
		{
			if(img.empty())	return;
		...
		//@brief: Defaine a button widget and it provides the interfaces to be operational
			...
			button& button::icon(const paint::image& img)
			{
