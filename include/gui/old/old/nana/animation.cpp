...
namespace nana
{
	...
	{
		std::size_t length;
		std::function<bool(std::size_t, paint::graphics&, size&)> frbuilder;

		framebuilder(std::function<bool(std::size_t, paint::graphics&, size&)> f, std::size_t l)
	...
	struct frame
	{
		...
		frame(std::function<bool(std::size_t, paint::graphics&, size&)> frbuilder, std::size_t length)
	...
	//class frameset
		...
		struct frameset::impl
		{
			...
			void render_this(std::map<window, output_t>& outs, paint::graphics& framegraph, size& framegraph_dimension) const
			{
				...
				case frame::kind::oneshot:
					_m_render(outs, [&frmobj](paint::graphics& tar, const point& pos)
					{
				...
				case frame::kind::framebuilder:
					...
					if(good_frame_by_frmbuilder)
					{
						...
						_m_render(outs, [&r, &framegraph](paint::graphics& tar, const point& pos) mutable
						{
							r.x = pos.x;
							r.y = pos.y;
			...
			void render_this(paint::graphics& graph, const point& pos, paint::graphics& framegraph, size& framegraph_dimension, bool rebuild_frame) const
			{
				if(this_frame == frames.end())
					return;
	...
	//class animation
		...
		struct animation::impl
		{
			...
			void render_this_specifically(paint::graphics& graph, const point& pos)
			{
