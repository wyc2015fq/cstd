...
namespace detail
{
	...
	public:
		...
		static void paste_children_to_graphics(core_window_t*, paint::graphics& graph);
		...
		static bool read_visual_rectangle(core_window_t*, rectangle& visual);

		//read_overlaps
		//	reads the overlaps that are overlapped a rectangular block
		static bool read_overlaps(core_window_t*, const rectangle& vis_rect, std::vector<wd_rectangle>& blocks);
	...
	private:
		...
		static void _m_paste_children(core_window_t*, bool is_child_refreshed, bool have_refreshed, const rectangle& parent_rect, paint::graphics& graph, const point& graph_rpos);
		...
		static void _m_notify_glasses(core_window_t* const sigwd, const rectangle& r_visual);
	private:
		struct data_section
		{
