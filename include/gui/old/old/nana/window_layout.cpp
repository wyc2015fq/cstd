		...
		//class window_layout
			...
			bool window_layout::maproot(core_window_t* wd, bool have_refreshed, bool is_child_refreshed)
			{
				...
				if (read_visual_rectangle(wd, vr))
				{
					...
					_m_paste_children(wd, is_child_refreshed, have_refreshed, vr, graph, point());
					...
					if (wd->parent)
					{
						...
						if (read_overlaps(wd, vr, blocks))
						{
							...
							for (auto & el : blocks)
							{
								...
								_m_paste_children(el.window, is_child_refreshed, false, el.r, graph, point{});
							}
						}
					}
			...
			void window_layout::paste_children_to_graphics(core_window_t* wd, paint::graphics& graph)
			{
			...
			bool window_layout::read_overlaps(core_window_t* wd, const rectangle& vis_rect, std::vector<wd_rectangle>& blocks)
			{
				wd_rectangle block;
				while (wd->parent)
				{
			...
			void window_layout::make_bground(core_window_t* const wd)
			{
				...
				for (auto child : wd->parent->children)
				{
					...
					{
						if (category::flags::lite_widget != child->other.category)
							glass_buffer.bitblt(rectangle{ ovlp.x - wd->pos_owner.x, ovlp.y - wd->pos_owner.y, ovlp.width, ovlp.height }, child->drawer.graphics, point(ovlp.x - child->pos_owner.x, ovlp.y - child->pos_owner.y));
			...
			void window_layout::_m_paste_children(core_window_t* wd, bool is_child_refreshed, bool have_refreshed, const rectangle& parent_rect, paint::graphics& graph, const point& graph_rpos)
			{
				rectangle rect;
			...
			void window_layout::_m_notify_glasses(core_window_t* const sigwd, const rectangle& /*r_visual*/)
			{
