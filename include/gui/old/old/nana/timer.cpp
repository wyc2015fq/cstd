	...
	public:
		...
		timer_core* create(unsigned ms, Factory && factory)
		{
			...
			try
			{
				...
				auto p = factory();
				auto tmid = p;
				::detail::platform_spec::instance().set_timer(reinterpret_cast<std::size_t>(tmid), ms, &timer_driver::_m_timer_proc);
#endif
...
#else
				::detail::platform_spec::instance().kill_timer(reinterpret_cast<std::size_t>(tid));
#endif
...
#else
			::detail::platform_spec::instance().set_timer(reinterpret_cast<std::size_t>(timer_), ms, &timer_driver::_m_timer_proc);
#endif
		}
