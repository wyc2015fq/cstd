	...
	namespace detail
	{
		...
		public:
			...
			void insert(handle_type handle, unsigned tid)
			{
				...
				is_queue<std::is_same<cond_type, null_type>::value, std::vector<handle_type> >::insert(handle, queue_);
			...
			void remove(const handle_type handle)
			{
				...
				if(holder_.cend() != i)
				{
					is_queue<std::is_same<cond_type, null_type>::value, std::vector<handle_type> >::erase(handle, queue_);
