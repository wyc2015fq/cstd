...
#else
					owns_caret = detail::platform_spec::instance().caret_update(iwd->root, *iwd->root_graph, false);
#endif
				}
...
#else
					detail::platform_spec::instance().caret_update(iwd->root, *iwd->root_graph, true);
#endif
				}
			}
		}
		...
		void drawer::_m_effect_bground(bool before)
		{
			...
			if (effect.bground)
			{
				if (before)
				{
					...
					data_impl_->window_handle->other.glass_buffer.blend(::rectangle{ data_impl_->window_handle->other.glass_buffer.size() }, graphics, point(), effect.bground_fade_rate);
			}
		}
