...
#pragma pack()
	...
	void assign_arg(arg_mouse& arg, basic_window* wd, unsigned msg, const parameter_decoder& pmdec)
	{
		arg.window_handle = (window)(wd);
	...
	void assign_arg(arg_wheel& arg, basic_window* wd, const parameter_decoder& pmdec)
	{
		...
		::ScreenToClient((HWND)(wd->root), &point);

		arg.upwards = (pmdec.mouse.button.wheel_delta >= 0);
		arg.distance = (unsigned)(arg.upwards ? pmdec.mouse.button.wheel_delta : -pmdec.mouse.button.wheel_delta);

		arg.pos.x = (int)(point.x) - wd->pos_root.x;
		arg.pos.y = (int)(point.y) - wd->pos_root.y;
		arg.left_button = pmdec.mouse.button.left;
		arg.mid_button = pmdec.mouse.button.middle;
		arg.right_button = pmdec.mouse.button.right;
		arg.ctrl = pmdec.mouse.button.ctrl;
	...
	{
		unsigned width = (unsigned)(r->right - r->left) - wd->extra_width;
		unsigned height = (unsigned)(r->bottom - r->top) - wd->extra_height;
		...
		if(req_width != width)
		{
			...
			case WMSZ_TOPLEFT:
				r->left = r->right - (int)(req_width) - wd->extra_width;
				break;
			case WMSZ_RIGHT:
			...
			case WMSZ_TOP:
			case WMSZ_BOTTOM:
				r->right = r->left + (int)(req_width) + wd->extra_width;
				break;
			}
		}
		...
		if(req_height != height)
		{
			...
			case WMSZ_TOPLEFT:
			case WMSZ_TOPRIGHT:
				r->top = r->bottom - (int)(req_height) - wd->extra_height;
				break;
			case WMSZ_BOTTOM:
			case WMSZ_BOTTOMLEFT:
			case WMSZ_BOTTOMRIGHT:
			case WMSZ_LEFT:
			case WMSZ_RIGHT:
				r->bottom = r->top + (int)(req_height) + wd->extra_height;
				break;
			}
		}
	}
	...
	LRESULT CALLBACK Bedrock_WIN32_WindowProc(HWND root_window, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		auto native_window = (native_window_type)(root_window);
		...
		if(root_runtime)
		{
			...
			case WM_IME_STARTCOMPOSITION:
				if(msgwnd->other.attribute.root->ime_enabled)
				{
					...
					LOGFONTW logfont;
					::GetObjectW((HFONT)(native_font), sizeof logfont, &logfont);
			...
			case WM_GETMINMAXINFO:
				{
					...
					{
						mmi->ptMinTrackSize.x = (LONG)(msgwnd->min_track_size.width + msgwnd->extra_width);
						mmi->ptMinTrackSize.y = (LONG)(msgwnd->min_track_size.height + msgwnd->extra_height);
						take_over = true;
					}
					...
					if(false == msgwnd->flags.fullscreen)
					{
						...
						{
							mmi->ptMaxTrackSize.x = (LONG)(msgwnd->max_track_size.width + msgwnd->extra_width);
							mmi->ptMaxTrackSize.y = (LONG)(msgwnd->max_track_size.height + msgwnd->extra_height);
			...
			case WM_WINDOWPOSCHANGED:
				if (((WINDOWPOS*)(lParam)->flags & SWP_SHOWWINDOW) && (!msgwnd->visible))
					brock.event_expose(msgwnd, true);
				else if (((WINDOWPOS*)(lParam)->flags & SWP_HIDEWINDOW) && msgwnd->visible)
			...
			case WM_KILLFOCUS:
				...
				brock.event_focus_changed(msgwnd, (native_window_type)(wParam), false);
				def_window_proc = true;
				break;
			...
			case WM_RBUTTONUP:
				...
				if(msgwnd->flags.enabled)
				{
					...
					if (msgwnd->dimension.is_hit(arg.pos))
					{
						...
						if ((::mouse::left_button == arg.button) && (pressed_wd == msgwnd))
						{
							click_arg.window_handle = (window)(msgwnd);
					...
					if(wd_manager.available(msgwnd) && msgwnd->flags.enabled)
					{
						...
						if (click_arg.window_handle)
							retain->click.emit(click_arg, (window)(msgwnd));

						if (wd_manager.available(msgwnd))
						{
							arg.evt_code = event_code::mouse_up;
							retain->mouse_up.emit(arg, (window)(msgwnd));
						}
					}
			...
			case WM_MOUSEHWHEEL:
				{
					...
					if (pointer_wd == root_window)
					{
						...
						auto scrolled_wd = wd_manager.find_window((native_window_type)(pointer_wd), scr_pos.x, scr_pos.y);
						...
						while (evt_wd)
						{
							if (evt_wd->annex.events_ptr->mouse_wheel.length() != 0)
							{
								...
								point mspos{ scr_pos.x, scr_pos.y };
			...
			case WM_DROPFILES:
				{
					...
					if(msgwnd)
					{
						...
						if(msgwnd)
						{
							...
							dropfiles.window_handle = (window)(msgwnd);

							msgwnd->annex.events_ptr->mouse_dropfiles.emit(dropfiles, (window)(msgwnd));
							wd_manager.do_lazy_refresh(msgwnd, false);
						}
					}
			...
			case WM_SIZING:
				{
					...
					unsigned width = (unsigned)(r->right - r->left) - msgwnd->extra_width;
					unsigned height = (unsigned)(r->bottom - r->top) - msgwnd->extra_height;
					...
					if(msgwnd->max_track_size.width || msgwnd->min_track_size.width)
					{
						if(wParam == WMSZ_LEFT || wParam == WMSZ_BOTTOMLEFT || wParam == WMSZ_TOPLEFT)
						{
							if(msgwnd->max_track_size.width && (width > msgwnd->max_track_size.width))
								r->left = r->right - (int)(msgwnd->max_track_size.width) - msgwnd->extra_width;
							if(msgwnd->min_track_size.width && (width < msgwnd->min_track_size.width))
								r->left = r->right - (int)(msgwnd->min_track_size.width) - msgwnd->extra_width;
						}
						else if(wParam == WMSZ_RIGHT || wParam == WMSZ_BOTTOMRIGHT || wParam == WMSZ_TOPRIGHT)
						{
							if(msgwnd->max_track_size.width && (width > msgwnd->max_track_size.width))
								r->right = r->left + (int)(msgwnd->max_track_size.width) + msgwnd->extra_width;
							if(msgwnd->min_track_size.width && (width < msgwnd->min_track_size.width))
								r->right = r->left + (int)(msgwnd->min_track_size.width) + msgwnd->extra_width;
						}
					}
					...
					if(msgwnd->max_track_size.height || msgwnd->min_track_size.height)
					{
						if(wParam == WMSZ_TOP || wParam == WMSZ_TOPLEFT || wParam == WMSZ_TOPRIGHT)
						{
							if(msgwnd->max_track_size.height && (height > msgwnd->max_track_size.height))
								r->top = r->bottom - (int)(msgwnd->max_track_size.height) - msgwnd->extra_height;
							if(msgwnd->min_track_size.height && (height < msgwnd->min_track_size.height))
								r->top = r->bottom - (int)(msgwnd->min_track_size.height) - msgwnd->extra_height;
						}
						else if(wParam == WMSZ_BOTTOM || wParam == WMSZ_BOTTOMLEFT || wParam == WMSZ_BOTTOMRIGHT)
						{
							if(msgwnd->max_track_size.height && (height > msgwnd->max_track_size.height))
								r->bottom = r->top + (int)(msgwnd->max_track_size.height) + msgwnd->extra_height;
							if(msgwnd->min_track_size.height && (height < msgwnd->min_track_size.height))
								r->bottom = r->top + (int)(msgwnd->min_track_size.height) + msgwnd->extra_height;
						}
					}

					size size_before(	(unsigned)(r->right - r->left - msgwnd->extra_width),
											(unsigned)(r->bottom - r->top - msgwnd->extra_height));

					arg_resizing arg;
					arg.window_handle = (window)(msgwnd);
					...
					{
						adjust_sizing(msgwnd, r, (int)(wParam), arg.width, arg.height);
						return TRUE;
					}
				}
				break;
			...
			case WM_PAINT:
				{
					...
					else
					{
						...
						if (!update_area.empty())
							msgwnd->drawer.map((window)(msgwnd), true, &update_area);
					}
			...
			case WM_SYSCHAR:
				...
				msgwnd = wd_manager.find_shortkey(native_window, (unsigned long)(wParam));
				if(msgwnd)
				{
					arg_keyboard arg;
					arg.evt_code = event_code::shortkey;
					arg.key = (wchar_t)(wParam < 0x61 ? wParam + 0x61 - 0x41 : wParam);
					...
					if(msgwnd)
					{
						...
						else
						{
							...
							if (msgwnd->root_widget->other.attribute.root->menubar == msgwnd)
							{
								...
								int cmd = (menu_wd && (keyboard::escape == (wchar_t)(wParam)) ? 1 : 0);
			...
			case WM_CHAR:
				...
				if (msgwnd && msgwnd->flags.enabled)
				{
					...
					if (VK_TAB != wParam || !root_runtime->condition.ignore_tab)
					{
						...
						msgwnd->annex.events_ptr->key_char.emit(arg, (window)(msgwnd));
			...
			case WM_DESTROY:
				...
				detail::platform_spec::instance().release_window_icon(msgwnd->root);
				break;
			case WM_NCDESTROY:
	...
	{
		auto graph_context = (HDC)(wd->root_graph->handle()->context);

		for (auto child : wd->children)
		{
			if (!child->visible) continue;

			if (::category::flags::widget == child->other.category)
			{
				::BitBlt((HDC)(drawable), child->pos_root.x, child->pos_root.y, (int)(child->dimension.width), (int)(child->dimension.height),
					graph_context, child->pos_root.x, child->pos_root.y, SRCCOPY);
			}
			else if (::category::flags::lite_widget == child->other.category)
...
#ifdef _WIN64
			::SetClassLongPtr((HWND)(wd->root), GCLP_HCURSOR,
				(LONG_PTR)(thrd->cursor.handle));
#else
			::SetClassLong((HWND)(wd->root), GCL_HCURSOR,
				(unsigned long)((size_t)(thrd->cursor.handle)));
#endif
		}
