...
// CWnds for setting z-order with SetWidgetPos's pWndInsertAfter parameter
const AFX_DATADEF CWnd CWnd::wndTop(HWID_TOP);
const AFX_DATADEF CWnd CWnd::wndBottom(HWID_BOTTOM);
const AFX_DATADEF CWnd CWnd::wndTopMost(HWID_TOPMOST);
const AFX_DATADEF CWnd CWnd::wndNoTopMost(HWID_NOTOPMOST);
...
AFX_STATIC BOOL AFXAPI _AfxModifyStyle(HWID hWnd, int nStyleOffset,
    DWORD dwRemove, DWORD dwAdd, UINT nFlags)
{
  ASSERT(hWnd != NULL);
  DWORD dwStyle = ::GetWidgetLong(hWnd, nStyleOffset);
  DWORD dwNewStyle = (dwStyle & ~dwRemove) | dwAdd;
  if (dwStyle == dwNewStyle) {
    return FALSE;
  }
  ::SetWidgetLong(hWnd, nStyleOffset, dwNewStyle);
  if (nFlags != 0) {
    ::SetWidgetPos(hWnd, NULL, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE | nFlags);
  }
  return TRUE;
}
BOOL PASCAL
CWnd::ModifyStyle(HWID hWnd, DWORD dwRemove, DWORD dwAdd, UINT nFlags)
{
  return _AfxModifyStyle(hWnd, GWL_STYLE, dwRemove, dwAdd, nFlags);
}
BOOL PASCAL
CWnd::ModifyStyleEx(HWID hWnd, DWORD dwRemove, DWORD dwAdd, UINT nFlags)
{
...
LRESULT AFXAPI AfxCallWndProc(CWnd* pWnd, HWID hWnd, UINT nMsg,
...
CHandleMap* PASCAL afxMapHWID(BOOL bCreate)
{
  AFX_MODULE_THREAD_STATE* pState = AfxGetModuleThreadState();
  if (pState->m_pmapHWID == NULL && bCreate) {
    BOOL bEnable = AfxEnableMemoryTracking(FALSE);
#ifndef _AFX_PORTABLE
    _PNH pnhOldHandler = AfxSetNewHandler(&AfxCriticalNewHandler);
#endif
    pState->m_pmapHWID = new CHandleMap(RUNTIME_CLASS(CTempWnd),
  ...
  return pState->m_pmapHWID;
}
CWnd* PASCAL CWnd::FromHandle(HWID hWnd)
{
  CHandleMap* pMap = afxMapHWID(TRUE); //create map if not exist
  ASSERT(pMap != NULL);
...
CWnd* PASCAL CWnd::FromHandlePermanent(HWID hWnd)
{
  CHandleMap* pMap = afxMapHWID();
  CWnd* pWnd = NULL;
  if (pMap != NULL) {
...
HWID CWnd::Detach()
{
  HWID hWnd = m_hWnd;
  if (hWnd != NULL) {
    CHandleMap* pMap = afxMapHWID(); // don't create if not exist
    if (pMap != NULL) {
...
LRESULT CALLBACK
AfxWndProc(HWID hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
{
...
BOOL CWnd::CreateEx(DWORD dwExStyle, LPCTSTR lpszClassName,
    ...
    HWID hWndParent, HMENU nIDorHMenu, LPVOID lpParam)
{
  ...
  HWID hWnd = ::CreateWindowEx(cs.dwExStyle, cs.lpszClass,
...
void CWnd::OnNcDestroy()
{
  ...
  LONG pfnWndProc = GetWidgetLong(m_hWnd, GWL_WNDPROC);
  Default();
  if (GetWidgetLong(m_hWnd, GWL_WNDPROC) == pfnWndProc) {
    WNDPROC pfnSuper = *GetSuperWndProcAddr();
    if (pfnSuper != NULL) {
      SetWidgetLong(m_hWnd, GWL_WNDPROC, (LONG)pfnSuper);
    }
  }
  Detach();
  ASSERT(m_hWnd == NULL);
...
void CWnd::AssertValid() const
{
  ...
  ASSERT(HWID_TOP == NULL); // same as desktop
  if (m_hWnd == HWID_BOTTOM) {
    ASSERT(this == &CWnd::wndBottom);
  }
  else if (m_hWnd == HWID_TOPMOST) {
    ASSERT(this == &CWnd::wndTopMost);
  }
  else if (m_hWnd == HWID_NOTOPMOST) {
    ...
    //
    // In general, CWnd objects should be passed by HWID from
    // one thread to another. The receiving thread can wrap
    // the HWID with a CWnd object by using CWnd::FromHandle.
    //
...
void CWnd::Dump(CDumpContext& dc) const
{
  CObject::Dump(dc);
  dc << "\nm_hWnd = " << (UINT)m_hWnd;
  if (m_hWnd == NULL || m_hWnd == HWID_BOTTOM ||
      m_hWnd == HWID_TOPMOST || m_hWnd == HWID_NOTOPMOST) {
  ...
  dc << "\nstyle = " << (void*)::GetWidgetLong(m_hWnd, GWL_STYLE);
  if (::GetWidgetLong(m_hWnd, GWL_STYLE) & WS_CHILD) {
...
int CWnd::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
  ...
  HWID hWndChild = _AfxChildWindowFromPoint(m_hWnd, point);
  if (hWndChild != NULL) {
...
int CWnd::GetDlgItemText(int nID, CString& rString) const
{
  ...
  if (m_pCtrlCont == NULL) {
#endif
    HWID hWnd = ::GetDlgItem(m_hWnd, nID);
    if (hWnd != NULL) {
    ...
    CWnd* pWnd, LPCRECT lpRect)
{
  ASSERT(m_hMenu != NULL);
  _AFX_THREAD_STATE* pThreadState = AfxGetThreadState();
  HWID hWndOld = pThreadState->m_hTrackingWindow;
...
BOOL CWnd::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
  ...
  if (message == WM_ACTIVATE) {
    _AfxHandleActivate(this, wParam, CWnd::FromHandle((HWID)lParam));
  }
  ...
  case AfxSig_bWww: // really AfxSig_bWiw
      lResult = (this->*mmf.pfn_bWww)(CWnd::FromHandle((HWID)wParam),
          (short)LOWORD(lParam), HIWORD(lParam));
    break;
  case AfxSig_bWCDS:
      lResult = (this->*mmf.pfn_bWCDS)(CWnd::FromHandle((HWID)wParam),
  ...
  case AfxSig_iwWw:
      lResult = (this->*mmf.pfn_iwWw)(LOWORD(wParam),
          CWnd::FromHandle((HWID)lParam), HIWORD(wParam));
    break;
  case AfxSig_iww:
      lResult = (this->*mmf.pfn_iww)(LOWORD(wParam), HIWORD(wParam));
    break;
  case AfxSig_iWww: // really AfxSig_iWiw
      lResult = (this->*mmf.pfn_iWww)(CWnd::FromHandle((HWID)wParam),
          (short)LOWORD(lParam), HIWORD(lParam));
    break;
  case AfxSig_is:
  ...
  case AfxSig_vwl:
    ...
    break;
  case AfxSig_vbWW:
      (this->*mmf.pfn_vbWW)(m_hWnd == (HWID)lParam,
          CWnd::FromHandle((HWID)lParam),
          CWnd::FromHandle((HWID)wParam));
    break;
  case AfxSig_vD:
  ...
  case AfxSig_vMwb:
    ...
    break;
  case AfxSig_vW:
      (this->*mmf.pfn_vW)(CWnd::FromHandle((HWID)wParam));
    break;
  case AfxSig_vW2:
      (this->*mmf.pfn_vW)(CWnd::FromHandle((HWID)lParam));
    break;
  case AfxSig_vWww:
      (this->*mmf.pfn_vWww)(CWnd::FromHandle((HWID)wParam), LOWORD(lParam),
          HIWORD(lParam));
    break;
  case AfxSig_vWp: {
      CPoint point((DWORD)lParam);
      (this->*mmf.pfn_vWp)(CWnd::FromHandle((HWID)wParam), point);
    }
    break;
  case AfxSig_vWh:
      (this->*mmf.pfn_vWh)(CWnd::FromHandle((HWID)wParam),
          (HANDLE)lParam);
    break;
  case AfxSig_vwW:
      (this->*mmf.pfn_vwW)(wParam, CWnd::FromHandle((HWID)lParam));
    break;
  case AfxSig_vwWb:
      (this->*mmf.pfn_vwWb)((UINT)(LOWORD(wParam)),
          CWnd::FromHandle((HWID)lParam), (BOOL)HIWORD(wParam));
    break;
  case AfxSig_vwwW:
    case AfxSig_vwwx: {
...
HWID AFXAPI AfxGetParentOwner(HWID hWnd)
{
  ...
  return (::GetWidgetLong(hWnd, GWL_STYLE) & WS_CHILD) ?
...
CWnd* CWnd::GetParentOwner() const
{
  ...
  HWID hWndParent = m_hWnd;
  HWID hWndT;
  while ((::GetWidgetLong(hWndParent, GWL_STYLE) & WS_CHILD) &&
...
CWnd* PASCAL CWnd::GetSafeOwner(CWnd* pParent, HWID* pWndTop)
{
  HWID hWnd = GetSafeOwner_(pParent->GetSafeHwnd(), pWndTop);
...
CWnd* PASCAL CWnd::GetDescendantWindow(HWID hWnd, int nID, BOOL bOnlyPerm)
{
  ...
  for (hWndChild = ::GetTopWindow(hWnd); hWndChild != NULL;
      hWndChild = ::GetNextWindow(hWndChild, GW_HWIDNEXT)) {
    pWndChild = GetDescendantWindow(hWndChild, nID, bOnlyPerm);
    if (pWndChild != NULL) {
      return pWndChild;
    }
...
void PASCAL CWnd::SendMessageToDescendants(HWID hWnd, UINT message,
    WPARAM wParam, LPARAM lParam, BOOL bDeep, BOOL bOnlyPerm)
{
  // walk through HWIDs to avoid creating temporary CWnd objects
  // unless we need to call this function recursively
  for (HWID hWndChild = ::GetTopWindow(hWnd); hWndChild != NULL;
      hWndChild = ::GetNextWindow(hWndChild, GW_HWIDNEXT)) {
    ...
    LPCRECT lpRect, LPCRECT lpClipRect)
{
  ...
  if (IsWindowVisible() || lpRect != NULL || lpClipRect != NULL) {
    ...
    // directly instead.
    HWID hWndChild = ::GetWindow(m_hWnd, GW_CHILD);
    if (hWndChild != NULL) {
      for (; hWndChild != NULL;
          hWndChild = ::GetNextWindow(hWndChild, GW_HWIDNEXT)) {
        CRect rect;
    ...
    UINT nFlags, LPRECT lpRectParam, LPCRECT lpRectClient, BOOL bStretch)
{
  ...
  for (HWID hWndChild = ::GetTopWindow(m_hWnd); hWndChild != NULL;
      hWndChild = ::GetNextWindow(hWndChild, GW_HWIDNEXT)) {
...
void AFXAPI AfxRepositionWindow(AFX_SIZEPARENTPARAMS* lpLayout,
    HWID hWnd, LPCRECT lpRect)
{
  ASSERT(hWnd != NULL);
  ASSERT(lpRect != NULL);
  HWID hWndParent = ::GetParent(hWnd);
  ...
  // try to use DeferWindowPos for speed, otherwise use SetWidgetPos
  if (lpLayout != NULL) {
    ...
    ::SetWidgetPos(hWnd, NULL, lpRect->left, lpRect->top,
...
BOOL PASCAL CWnd::WalkPreTranslateTree(HWID hWndStop, MSG* pMsg)
{
  ASSERT(hWndStop == NULL || ::IsWindow(hWndStop));
  ASSERT(pMsg != NULL);
  // walk from the target window up to the hWndStop window checking
  // if any window wants to translate this message
  for (HWID hWnd = pMsg->hwnd; hWnd != NULL; hWnd = ::GetParent(hWnd)) {
    CWnd* pWnd = CWnd::FromHandlePermanent(hWnd);
    if (pWnd != NULL) {
...
BOOL PASCAL CWnd::ReflectLastMsg(HWID hWndChild, LRESULT* pResult)
{
...
BOOL PASCAL CWnd::GrayCtlColor(HDC hDC, HWID hWnd, UINT nCtlColor,
...
BOOL CWnd::UpdateData(BOOL bSaveAndValidate)
{
  ...
  _AFX_THREAD_STATE* pThreadState = AfxGetThreadState();
  HWID hWndOldLockout = pThreadState->m_hLockoutNotifyWindow;
...
void CWnd::CenterWindow(CWnd* pAlternateOwner)
{
  ...
  HWID hWndCenter = pAlternateOwner->GetSafeHwnd();
  if (pAlternateOwner == NULL) {
    ...
    if (hWndCenter != NULL) {
      ...
      HWID hWndTemp =
          (HWID)::SendMessage(hWndCenter, WM_QUERYCENTERWND, 0, 0);
      if (hWndTemp != NULL) {
        hWndCenter = hWndTemp;
      }
    }
  ...
  if (!(dwStyle & WS_CHILD)) {
    ...
    if (hWndCenter != NULL) {
      DWORD dwStyle = ::GetWidgetLong(hWndCenter, GWL_STYLE);
    ...
    if (hWndCenter == NULL) {
      HWID hwDefault = AfxGetMainWnd()->GetSafeHwnd();
      GetMonitorInfo(
  ...
  SetWidgetPos(NULL, xLeft, yTop, -1, -1,
...
int CWnd::RunModalLoop(DWORD dwFlags)
{
  ...
  HWID hWndParent = ::GetParent(m_hWnd);
...
BOOL CWnd::SubclassWindow(HWID hWnd)
{
  if (!Attach(hWnd)) {
    return FALSE;
  }
  ...
  WNDPROC oldWndProc = (WNDPROC)::SetWidgetLong(hWnd, GWL_WNDPROC,
  ...
  if (*lplpfn == NULL) {
    *lplpfn = oldWndProc; // the first control of that type created
    ...
    ::SetWidgetLong(hWnd, GWL_WNDPROC, (DWORD)oldWndProc);
  }
#endif
  return TRUE;
}
BOOL CWnd::SubclassDlgItem(UINT nID, CWnd* pParent)
{
  ...
  HWID hWndControl = ::GetDlgItem(pParent->m_hWnd, nID);
  if (hWndControl != NULL) {
...
HWID CWnd::UnsubclassWindow()
{
  ASSERT(::IsWindow(m_hWnd));
  // set WNDPROC back to original value
  WNDPROC* lplpfn = GetSuperWndProcAddr();
  SetWidgetLong(m_hWnd, GWL_WNDPROC, (LONG)*lplpfn);
  *lplpfn = NULL;
  // and Detach the HWID from the CWnd object
  return Detach();
}
////////////////////////////////////////////////////////////////////////////
