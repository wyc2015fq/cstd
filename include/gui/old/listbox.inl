...
/* Listbox structure */
typedef struct {
  HWID self; /* Our own window handle */
  HWID owner; /* Owner window to send notifications to */
...
#define SEND_NOTIFICATION(descr,code) \
  (SendMessage( (descr)->owner, WM_COMMAND, \
      MAKEWPARAM( GetWidgetLongPtrW((descr->self),GWLP_ID), (code)), (LPARAM)(descr->self) ))
...
*/
#if 0
static const CHAR listboxW[] = {'L', 'i', 's', 't', 'B', 'o', 'x', 0};
 ...
 */
static const CHAR combolboxW[] = {'C', 'o', 'm', 'b', 'o', 'L', 'B', 'o', 'x', 0};
...
static INT LISTBOX_GetCurrentPageSize(const LB_DESCR* descr)
{
  ...
  for (i = descr->top_item, height = 0; i < descr->nb_items; i++) {
    if ((height += descr->items[i].height) > descr->height) {
      break;
    }
  }
  if (i == descr->top_item) {
...
static INT LISTBOX_GetMaxTopIndex(const LB_DESCR* descr)
{
  ...
  if (descr->style & LBS_OWNERDRAWVARIABLE) {
    ...
    for (max = descr->nb_items - 1; max >= 0; max--)
      if ((page -= descr->items[max].height) < 0) {
        break;
      }
    if (max < descr->nb_items - 1) {
      max++;
    }
  }
  else if (descr->style & LBS_MULTICOLUMN) {
    if ((page = descr->width / descr->column_width) < 1) {
      page = 1;
    }
...
static LRESULT LISTBOX_SetTopItem(LB_DESCR* descr, INT index, BOOL scroll)
{
  ...
  if (descr->style & LBS_MULTICOLUMN) {
    index -= index % descr->page_size;
  }
  if (descr->top_item == index) {
    return LB_OKAY;
  }
  if (scroll) {
    INT diff;
    ...
    ScrollWidgetEx(descr->self, 0, diff, NULL, NULL, 0, NULL,
...
static void LISTBOX_UpdateSize(LB_DESCR* descr)
{
  ...
  if (!(descr->style & LBS_NOINTEGRALHEIGHT) && !(descr->style & LBS_OWNERDRAWVARIABLE)) {
    ...
    if ((descr->height > descr->item_height) && remaining) {
      ...
      SetWidgetPos(descr->self, 0, 0, 0, rect.right - rect.left,
  ...
  if (descr->style & LBS_OWNERDRAWVARIABLE) {
    ...
    if (y >= 0) {
      while (index < descr->nb_items) {
        if ((pos += descr->items[index].height) > y) {
          break;
        }
        index++;
      }
    ...
    if (y >= descr->item_height * descr->page_size) {
      return -1;
    }
    if (y >= 0) {
    ...
    if (x >= 0) {
      index += (x / descr->column_width) * descr->page_size;
    }
    else {
      index -= (((x + 1) / descr->column_width) - 1) * descr->page_size;
    }
  }
  else {
    index += (y / descr->item_height);
    ...
    INT index, UINT action, BOOL ignoreFocus)
{
  ...
  if (IS_OWNERDRAW(descr)) {
    ...
    GetWidgetRect(descr->self, &r);
    hrgn = set_control_clipping(hdc, &r);
    dis.CtlType = ODT_LISTBOX;
    dis.CtlID = GetWidgetLongPtrW(descr->self, GWLP_ID);
    ...
    if (!item)
      ...
      ExtTextOutA(hdc, rect->left + 1, rect->top, ETO_OPAQUE | ETO_CLIPPED, rect, item->str,
...
static void LISTBOX_RepaintItem(LB_DESCR* descr, INT index, UINT action)
{
  ...
  if (descr->font) {
    oldFont = (HFONT)SelectObject(hdc, descr->font);
  }
...
static BOOL LISTBOX_SetTabStops(LB_DESCR* descr, INT count, LPINT tabs)
{
  ...
  if (!(descr->tabs = (INT*)HeapAlloc(GetProcessHeap(), 0, descr->nb_tabs * sizeof(INT)))) {
    return FALSE;
  }
 ...
 * LISTBOX_GetText
 */
static LRESULT LISTBOX_GetText(LB_DESCR* descr, INT index, LPSTR buffer, BOOL unicode)
{
  DWORD len;
  if ((index < 0) || (index >= descr->nb_items)) {
    SetLastError(ERROR_INVALID_INDEX);
  ...
  if (HAS_STRINGS(descr)) {
    if (!buffer) {
      len = strlen(descr->items[index].str);
      return len;
    }
...
static inline INT LISTBOX_lstrcmpiA(LCID lcid, LPCSTR str1, LPCSTR str2)
{
  INT ret = CompareStringA(lcid, NORM_IGNORECASE, str1, -1, str2, -1);
 ...
 */
static INT LISTBOX_FindStringPos(LB_DESCR* descr, LPCSTR str, BOOL exact)
{
  INT index, min, max, res = -1;
  if (!(descr->style & LBS_SORT)) {
  ...
  while (min != max) {
    index = (min + max) / 2;
    if (HAS_STRINGS(descr)) {
      res = LISTBOX_lstrcmpiA(descr->locale, str, descr->items[index].str);
    }
    else {
      COMPAREITEM_T cis;
      UINT id = (UINT)GetWidgetLongPtrW(descr->self, GWLP_ID);
 ...
 */
static INT LISTBOX_FindFileStrPos(LB_DESCR* descr, LPCSTR str)
{
  INT min, max, res = -1;
  if (!HAS_STRINGS(descr)) {
  ...
  while (min != max) {
    INT index = (min + max) / 2;
    LPCSTR p = descr->items[index].str;
    if (*p == '[') { /* drive or directory */
      if (*str != '[') {
        ...
        if (str[1] == '-') {
          ...
          res = LISTBOX_lstrcmpiA(descr->locale, str, p);
        }
      }
    }
    else { /* filename */
 ...
 */
static INT LISTBOX_FindString(LB_DESCR* descr, INT start, LPCSTR str, BOOL exact)
{
  INT i;
  LB_ITEMDATA* item;
  if (start >= descr->nb_items) {
    start = -1;
  }
  ...
  if (HAS_STRINGS(descr)) {
    ...
    if (exact) {
      for (i = start + 1; i < descr->nb_items; i++, item++)
        if (!LISTBOX_lstrcmpiA(descr->locale, str, item->str)) {
          return i;
        }
...
#define strncmpiA strnicmp
      /* Special case for drives and directories: ignore prefix */
#define CHECK_DRIVE(item) \
  if ((item)->str[0] == '[') \
  { \
    if (!strncmpiA( str, (item)->str+1, len )) return i; \
    if (((item)->str[1] == '-') && !strncmpiA(str, (item)->str+2, len)) \
      return i; \
  }
      INT len = strlen(str);
      for (i = start + 1; i < descr->nb_items; i++, item++) {
        if (!strncmpiA(str, item->str, len)) {
          return i;
        }
        CHECK_DRIVE(item);
      }
...
static void LISTBOX_SetHorizontalPos(LB_DESCR* descr, INT pos)
{
  ...
  if (abs(diff) < descr->width) {
    ...
    ScrollWidgetEx(descr->self, diff, 0, NULL, NULL, 0, NULL,
        SW_INVALIDATE | SW_ERASE | SW_SCROLLCHILDREN);
  }
  else {
    InvalidateRect(descr->self, NULL, TRUE);
...
static void LISTBOX_MakeItemVisible(LB_DESCR* descr, INT index, BOOL fully)
{
  ...
  if (index <= descr->top_item) {
    ...
    if (index < descr->top_item + (descr->page_size * cols)) {
      return;
    }
    ...
    for (top = index; top > descr->top_item; top--)
      if ((height += descr->items[top - 1].height) > descr->height) {
        break;
      }
  }
  else {
    if (index < descr->top_item + descr->page_size) {
      return;
    }
    if (!fully && (index == descr->top_item + descr->page_size) &&
        (descr->height > (descr->page_size * descr->item_height))) {
      return;
    }
    ...
    INT last, BOOL on)
{
  ...
  if (last == -1 || last >= descr->nb_items) {
    last = descr->nb_items - 1;
  }
  if (first < 0) {
    first = 0;
  }
    ...
    BOOL on, BOOL send_notify)
{
  ...
  if (descr->style & LBS_MULTIPLESEL) {
    ...
    if (oldsel != -1) {
      descr->items[oldsel].selected = FALSE;
    }
    if (index != -1) {
      descr->items[index].selected = TRUE;
    }
    if (oldsel != -1) {
      LISTBOX_RepaintItem(descr, oldsel, ODA_SELECT);
    }
    descr->selected_item = index;
    if (index != -1) {
      LISTBOX_RepaintItem(descr, index, ODA_SELECT);
    }
    if (send_notify && descr->nb_items) SEND_NOTIFICATION(descr,
          (index != -1) ? LBN_SELCHANGE : LBN_SELCANCEL);
    else if (descr->lphc) { /* set selection change flag for parent combo */
...
static void LISTBOX_MoveCaret(LB_DESCR* descr, INT index, BOOL fully_visible)
{
  ...
  you want to mimic Widgets behavior:
 ...
 */
static LRESULT LISTBOX_InsertString(LB_DESCR* descr, INT index, LPCSTR str)
{
  LPSTR new_str = NULL;
  ULONG_PTR data = 0;
  LRESULT ret;
  if (HAS_STRINGS(descr)) {
    static const CHAR empty_stringW[] = { 0 };
    if (!str) {
      str = empty_stringW;
    }
    if (!(new_str = (CHAR*)HeapAlloc(GetProcessHeap(), 0, (strlen(str) + 1) * sizeof(CHAR)))) {
      SEND_NOTIFICATION(descr, LBN_ERRSPACE);
      return LB_ERRSPACE;
    }
    strcpy(new_str, str);
...
static void LISTBOX_DeleteItem(LB_DESCR* descr, INT index)
{
  ...
  LPSTR item_str = descr->items[index].str;
  if (!descr->nb_items) {
...
static LRESULT LISTBOX_RemoveItem(LB_DESCR* descr, INT index)
{
  ...
  if (index < descr->nb_items)
    RtlMoveMemory(item, item + 1,
        (descr->nb_items - index) * sizeof(LB_ITEMDATA));
  if (descr->anchor_item == descr->nb_items) {
    descr->anchor_item--;
  }
...
{
  INT i;
  for (i = descr->nb_items - 1; i >= 0; i--) {
    LISTBOX_DeleteItem(descr, i);
  }
    ...
    if ((handle = FindFirstFileA(filespec, &entry)) == INVALID_HANDLE_VALUE) {
      ...
      do {
        ...
        if (entry.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
          static const CHAR bracketW[] = { ']', 0 };
          static const CHAR dotW[] = { '.', 0 };
          if (!(attrib & DDL_DIRECTORY) ||
              !strcmp(entry.cFileName, dotW)) {
            continue;
          }
          buffer[0] = '[';
          if (!long_names && entry.cAlternateFileName[0]) {
            strcpy(buffer + 1, entry.cAlternateFileName);
          }
          else {
            strcpy(buffer + 1, entry.cFileName);
          }
          strcat(buffer, bracketW);
        }
...
static LRESULT LISTBOX_HandleLButtonDown(LB_DESCR* descr, DWORD keys, INT x, INT y)
{
  ...
  if (!descr->in_focus) {
    if (!descr->lphc) {
      WidgetSetFocus(descr->self);
    }
    else {
      WidgetSetFocus((descr->lphc->hWndEdit) ? descr->lphc->hWndEdit : descr->lphc->self);
    }
  }
  if (index == -1) {
    return 0;
  ...
  if (!descr->lphc) {
    if (GetWidgetLongW(descr->self, GWL_EXSTYLE) & WS_EX_DRAGDETECT) {
      POINT pt;
      pt.x = x;
      pt.y = y;
...
static LRESULT LISTBOX_HandleLButtonDownCombo(LB_DESCR* descr, UINT msg, DWORD keys, INT x, INT y)
{
  ...
  mousePos.x = x;
  mousePos.y = y;
  GetWidgetRect(descr->self, &clientRect);
  if (PtInRect(&clientRect, mousePos)) {
    ...
    HWID hWndOldCapture;
    /* Check the Non-Widget Area */
    screenMousePos = mousePos;
    hWndOldCapture = WidgetGetCapture();
    ReleaseCapture();
    GetWidgetRect(descr->self, &screenRect);
    ...
    if (!PtInRect(&screenRect, screenMousePos)) {
      ...
      INT nHitTestType = 0;
      LONG style = GetWidgetLongW(descr->self, GWL_STYLE);
      ...
      /* Widgets sends this message when a scrollbar is clicked
      */
      if (nHitTestType != 0) {
      ...
      */
      if (hWndOldCapture != 0) {
        WidgetSetCapture(hWndOldCapture);
      }
    }
  }
  return 0;
...
static LRESULT LISTBOX_HandleLButtonUp(LB_DESCR* descr)
{
  ...
  if (descr->captured) {
    ...
    if (WidgetGetCapture() == descr->self) {
      ReleaseCapture();
    }
    ...
    INT x, INT y)
{
  ...
  if (descr->style & LBS_MULTICOLUMN) {
    ...
    if (y < 0) {
      dir = LB_TIMER_UP; /* above */
    }
    else if (y >= descr->height) {
      dir = LB_TIMER_DOWN; /* below */
    }
  }
  index = LISTBOX_GetItemFromPoint(descr, x, y);
  if (index == -1) {
    index = descr->focus_item;
  }
  if (!LISTBOX_HandleTimer(descr, index, dir)) {
    dir = LB_TIMER_NONE;
  }
...
static LRESULT LISTBOX_HandleKeyDown(LB_DESCR* descr, DWORD key)
{
  ...
  if (caret == -1) switch (key) {
    ...
    case VK_PRIOR:
      if (descr->style & LBS_MULTICOLUMN) {
        ...
        caret = descr->focus_item - LISTBOX_GetCurrentPageSize(descr) + 1;
      }
      if (caret < 0) {
        caret = 0;
      }
  ...
  if (caret >= 0) {
    ...
    if (descr->style & LBS_NOTIFY) {
      if (descr->lphc && IsVisible(descr->self)) {
 ...
 * LISTBOX_HandleChar
 */
static LRESULT LISTBOX_HandleChar(LB_DESCR* descr, CHAR charW)
{
  INT caret = -1;
  CHAR str[2];
  str[0] = charW;
 ...
 * LISTBOX_Create
 */
static BOOL LISTBOX_Create(HWID hwnd, LPHEADCOMBO lphc)
{
  LB_DESCR* descr;
  MEASUREITEMSTRUCT mis;
  RECT rect;
  if (!(descr = (LB_DESCR*)HeapAlloc(GetProcessHeap(), 0, sizeof(LB_DESCR)))) {
    return FALSE;
  }
  GetWidgetRect(hwnd, &rect);
  descr->self = hwnd;
  descr->owner = GetParent(descr->self);
  descr->style = GetWidgetLongW(descr->self, GWL_STYLE);
  descr->width = rect.right - rect.left;
  descr->height = rect.bottom - rect.top;
  descr->items = NULL;
  descr->nb_items = 0;
  ...
  SetWidgetLongPtrW(descr->self, 0, (LONG_PTR)descr);
  ...
  */
  ...
  if (descr->style & LBS_OWNERDRAWVARIABLE) {
    descr->style |= LBS_NOINTEGRALHEIGHT;
  }
...
static BOOL LISTBOX_Destroy(LB_DESCR* descr)
{
  ...
  SetWidgetLongPtrW(descr->self, 0, 0);
 ...
 */
LRESULT ListBoxWndProc_common(HWID hwnd, UINT msg, WPARAM wParam, LPARAM lParam, BOOL unicode)
{
  LB_DESCR* descr = (LB_DESCR*)GetWidgetLongPtrW(hwnd, 0);
  LPHEADCOMBO lphc = 0;
  LRESULT ret;
  if (!descr) {
    if (!IsWidget(hwnd)) {
    ...
    if (msg == WM_CREATE) {
      ...
      if (lpcs->style & LBS_COMBOBOX) {
        lphc = (LPHEADCOMBO)lpcs->lpCreateParams;
      }
      if (!LISTBOX_Create(hwnd, lphc)) {
        return -1;
      }
      TRACE("creating hwnd %p descr %p\n", hwnd, (void*)GetWidgetLongPtrW(hwnd, 0));
      return 0;
    }
    /* Ignore all other messages before we get a WM_CREATE */
    return unicode ? DefWidgetProcW(hwnd, msg, wParam, lParam) :
        DefWidgetProcA(hwnd, msg, wParam, lParam);
  }
  if (descr->style & LBS_COMBOBOX) {
    lphc = descr->lphc;
  }
  TRACE("[%p]: msg %s wp %08lx lp %08lx\n",
      descr->self, SPY_GetMsgName(msg), wParam, lParam);
  switch (msg) {
  ...
  case LB_GETCOUNT:
    return descr->nb_items;
  case LB_GETTEXT:
    return LISTBOX_GetText(descr, wParam, (LPSTR)lParam, unicode);
  case LB_GETTEXTLEN:
  ...
  case LB_SELECTSTRING: {
    INT index;
    LPSTR textW;
    //if(HAS_STRINGS(descr)) TRACE("LB_SELECTSTRING: %s\n", unicode ? debugstr_w((LPSTR)lParam) : debugstr_a((LPSTR)lParam));
    textW = (LPSTR)lParam;
  ...
  case WM_MOUSEWHEEL:
    if (wParam & (MK_SHIFT | MK_CONTROL)) {
      return DefWidgetProcW(descr->self, msg, wParam, lParam);
    }
  ...
  case WM_MOUSEMOVE:
    if (lphc && ((lphc->dwStyle & CBS_DROPDOWNLIST) != CBS_SIMPLE)) {
      ...
      */
      if (GetWidgetRect(descr->self, &clientRect) && PtInRect(&clientRect, mousePos)) {
        descr->captured = TRUE;
      }
      LISTBOX_HandleMouseMove(descr, mousePos.x, mousePos.y);
      descr->captured = captured;
    }
    else if (WidgetGetCapture() == descr->self) {
  ...
  case WM_KEYDOWN:
    if (lphc && (lphc->dwStyle & CBS_DROPDOWNLIST) != CBS_SIMPLE) {
      /* for some reason Widgets makes it possible to
...
DWORD WINAPI GetListBoxInfo(HWID hwnd)
{
  TRACE("%p\n", hwnd);
