...
typedef void (*pfThemedPaint)(HWID hwid, HDC hdc, ButtonState drawState, UINT dtFlags);
...
static inline char* get_button_text(HWID hwid)
{
  INT len = 512;
  char* text = NULL;
  MYREALLOC(text, len + 1);
  if (text) {
    GetWidgetText(hwid, text, len + 1);
  }
  return text;
}
static void PB_draw(HWID hwid, HDC hDC, ButtonState drawState, UINT dtFlags)
{
  static const int states[] = { PBS_NORMAL, PBS_DISABLED, PBS_HOT, PBS_PRESSED, PBS_DEFAULTED };
  RECT bgRect, textRect = {0};
  HFONT font = (HFONT)SendWidgetMessage(hwid, WM_GETFONT, 0, 0);
  HFONT hPrevFont = font ? (HFONT)SelectObject(hDC, font) : NULL;
  int state = states[ drawState ];
  char* text = get_button_text(hwid);
  GetWidgetRect(hwid, &bgRect);
  FillSolidRect(hDC, &bgRect, GetSysColor(COLOR_BTNFACE));
  //Draw3dRect();
  //FillRect
  if (text) {
    //DrawText(theme, hDC, BP_PUSHBUTTON, state, text, lstrlenW(text), dtFlags, 0, &textRect);
    free(text);
  }
  if (hPrevFont) {
...
static void CB_draw(HWID hwid, HDC hDC, ButtonState drawState, UINT dtFlags)
{
  ...
  HFONT font = (HFONT)SendWidgetMessage(hwid, WM_GETFONT, 0, 0);
  HFONT hPrevFont = font ? (HFONT)SelectObject(hDC, font) : NULL;
  LRESULT checkState = SendWidgetMessage(hwid, BM_GETCHECK, 0, 0);
  DWORD dwStyle = GetWidgetLong(hwid, GWL_STYLE);
  ...
  char* text = get_button_text(hwid);
  GetWidgetRect(hwid, &bgRect);
  //GetBackgroundContentRect(hDC, part, state, &bgRect, &textRect);
  ...
  // DrawThemeParentBackground(hwid, hDC, NULL);
...
static void GB_draw(HWID hwid, HDC hDC, ButtonState drawState, UINT dtFlags)
{
  static const int states[] = { GBS_NORMAL, GBS_DISABLED, GBS_NORMAL, GBS_NORMAL, GBS_NORMAL };
  RECT bgRect, textRect, contentRect = {0};
  HFONT font = (HFONT)SendWidgetMessage(hwid, WM_GETFONT, 0, 0);
  HFONT hPrevFont = font ? (HFONT)SelectObject(hDC, font) : NULL;
  int state = states[ drawState ];
  char* text = get_button_text(hwid);
  ...
  if (text) {
    SIZE textExtent;
    GetTextExtentPoint32A(hDC, text, strlen(text), &textExtent);
...
static BOOL BUTTON_Paint(HWID hwid, HDC hParamDC)
{
  DWORD dwStyle = GetWidgetLong(hwid, GWL_STYLE);
  DWORD dwStyleEx = GetWidgetLong(hwid, GWL_EXSTYLE);
  UINT dtFlags = get_drawtext_flags(dwStyle, dwStyleEx);
  int state = (int)SendWidgetMessage(hwid, BM_GETSTATE, 0, 0);
  ButtonState drawState;
  pfThemedPaint paint = btnThemedPaintFunc[ dwStyle & BUTTON_TYPE ];
  if (!paint) {
    return FALSE;
  }
  if (IsWidgetEnabled(hwid)) {
    if (state & BST_PUSHED) {
  ...
  paint(hwid, hParamDC, drawState, dtFlags);
  return TRUE;
}
/**********************************************************************
 * The button control subclass window proc.
 */
LRESULT CALLBACK Button_Proc(HWID hwid, UINT msg, WPARAM wParam, LPARAM lParam)
{
  //const char* themeClass = WC_BUTTONA;
  //LRESULT result;
  switch (msg) {
  case WM_CREATE:
    return CallOriginalClass(hwid, msg, wParam, lParam);
  case WM_DESTROY:
    return 0;//CallOriginalClass(hwid, msg, wParam, lParam);
  case WM_THEMECHANGED:
    break;
  case WM_SYSCOLORCHANGE:
    return CallOriginalClass(hwid, msg, wParam, lParam);
    /* Do nothing. When themed, a WM_THEMECHANGED will be received, too, which will do the repaint. */
    break;
  case WM_PAINT:
    if (BUTTON_Paint(hwid, (HDC)wParam)) {
      return 0;
    }
    else {
      return CallOriginalClass(hwid, msg, wParam, lParam);
  ...
  case WM_ENABLE:
    RedrawWidget(hwid, NULL, NULL, RDW_FRAME | RDW_INVALIDATE | RDW_UPDATENOW);
    return CallOriginalClass(hwid, msg, wParam, lParam);
  case WM_MOUSEMOVE: {
    WIDGETTRACKMOUSEEVENT mouse_event = {0};
    mouse_event.cbSize = sizeof(WIDGETTRACKMOUSEEVENT);
    mouse_event.dwFlags = TME_QUERY;
    if (!WidgetTrackMouseEvent(&mouse_event) || !(mouse_event.dwFlags & (TME_HOVER | TME_LEAVE))) {
      mouse_event.dwFlags = TME_HOVER | TME_LEAVE;
      mouse_event.hwndTrack = hwid;
      mouse_event.dwHoverTime = 1;
      WidgetTrackMouseEvent(&mouse_event);
    }
    break;
  }
  case WM_MOUSEHOVER: {
    int state = (int)SendWidgetMessage(hwid, BM_GETSTATE, 0, 0);
    SetWidgetLong(hwid, 0, state | BST_HOT);
    WidgetInvalidateRect(hwid, NULL, FALSE);
    break;
  }
  case WM_MOUSELEAVE: {
    int state = (int)SendWidgetMessage(hwid, BM_GETSTATE, 0, 0);
