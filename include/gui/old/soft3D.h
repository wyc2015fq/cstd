...
typedef struct device_t {
  float16 transform;          // 坐标变换器
  int w;                  // 窗口宽度
  int h;                 // 窗口高度
  uchar* framebuffer;         // 像素缓存：framebuffer[y] 代表第 y行
  int* zbuffer;               // 深度缓存：zbuffer[y] 为第 y行指针
  uchar* texture;             // 纹理：同样是每行索引
  int frame_pitch;
  int texture_pitch;
  int tex_w;              // 纹理宽度
  int tex_h;              // 纹理高度
  int max_u;                  // 纹理最大宽度：tex_w - 1
  int max_v;                  // 纹理最大高度：tex_h - 1
