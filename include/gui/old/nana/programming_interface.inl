...
namespace API
{
	namespace detail
	{
		::widget_geometrics* make_scheme(::detail::scheme_factory_interface&&);
	}
	...
	namespace dev
	{
		...
		{
			return std::unique_ptr<Scheme>{static_cast<Scheme*>(API::detail::make_scheme(::detail::scheme_factory<Scheme>()))};
		}
		...
		::detail::native_string_type window_caption(window) throw();
		void window_caption(window, ::detail::native_string_type);
	...
	namespace detail
	{
		...
		bool emit_event(event_code, window, const ::event_arg&);
		...
		private:
			virtual bool _m_enum_fn(::widget*) = 0;
		};
		...
		public:
			...
			{}
		private:
			bool _m_enum_fn(::widget* wd) override
			{
				return _m_enum_call<Widget>(wd);
			}

			template<typename T, typename std::enable_if<std::is_same<::widget, T>::value>::type* = nullptr>
			bool _m_enum_call(::widget* wd)
			{
				enum_fn_(*wd);
				return true;
			}
	...
	template<typename Widget=::widget, typename EnumFunction>
	void enum_widgets(window wd, bool recursive, EnumFunction && fn)
	{
		static_assert(std::is_convertible<Widget, ::widget>::value, "enum_widgets<Widget>: The specified Widget is not a widget type.");
    ...
    /// native_window_type and HWND through reinterpret_cast operator. Under X System, a conversion can
    /// be employed between native_window_type and Window through reinterpret_cast operator.
	...
	template<typename Widget=::widget>
	typename ::dev::widget_traits<Widget>::event_type & events(window wd)
	{
		using event_type = typename ::dev::widget_traits<Widget>::event_type;
		...
		if (std::is_same<::general_events, event_type>::value)
	...
	template<typename EventArg, typename std::enable_if<std::is_base_of< ::event_arg, EventArg>::value>::type* = nullptr>
	...
	template<typename Widget = ::widget>
	typename ::dev::widget_traits<Widget>::scheme_type & scheme(window wd)
	{
		using scheme_type = typename ::dev::widget_traits<Widget>::scheme_type;
		...
		if (std::is_same<::widget_geometrics, scheme_type>::value)
	...
	bool window_graphics(window, paint::graphics&);
	bool root_graphics(window, paint::graphics&);
	bool get_visual_rectangle(window, rectangle&);

	void typeface(window, const paint::font&);
	...
	window find_window(const point& mspos);
	...
	mouse_action mouse_action(window);
	element_state element_state(window);
