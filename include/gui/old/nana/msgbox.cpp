...
namespace nana
{
		...
		private:
			void _m_icon(msgbox::icon_t ico)
			{
				...
				const unsigned pic_question[] = {
					...
					if(rawpx)
					{
						paint::pixel_buffer pxbuf(32, 32);
	...
	{
		std::string str = cs.to_bytes(unicode::utf8);
		sstream_ << str;
		return *this;
	}
	...
	{
	public:
		inputbox_window(window owner, paint::image (&imgs)[4], ::rectangle (&valid_areas)[4], const ::std::string & desc, const ::std::string& title, std::size_t contents, unsigned fixed_pixels, const std::vector<unsigned>& each_height)
	...
	window inputbox::integer::create(window parent, unsigned label_px)
	{
		...
		paint::graphics graph{ ::size{ 10, 10 } };
		...
		impl->dock.events().resized.connect_unignorable([impl, label_px, value_px](const ::arg_resized&)
		{
	...
	window inputbox::real::create(window parent, unsigned label_px)
	{
		...
		impl->dock.events().resized.connect_unignorable([impl, label_px, value_px](const ::arg_resized&)
		{
			impl->label.size(::size{ label_px, 24 });
			impl->spinbox.size(::size{ value_px, 24 });
		});
	...
	void inputbox::image(::paint::image img, bool is_left, const rectangle& valid_area)
	{
	...
	void inputbox::image_v(::paint::image img, bool is_top, const rectangle& valid_area)
	{
		auto pos = (is_top ? 0 : 1);
		images_[pos] = std::move(img);
		valid_areas_[pos] = valid_area;
