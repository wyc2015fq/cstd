...
namespace nana
{
	...
	namespace element
	{
		...
		{
			bool draw(graph_reference graph, const color& bgcolor, const color& fgcolor, const rectangle& r, element_state es, const data& crook_data) override
			{
				if(crook_data.radio)
				{
				...
				{
					::color highlighted(0x5e, 0xb6, 0xf7);
					...
					case element_state::disabled:
						bld_bgcolor = bld_fgcolor = color(0xb2, 0xb7, 0xbc);
						break;
					default:
		...
		class menu_crook
			: public crook_interface
		{
			bool draw(graph_reference graph, const ::color&, const ::color& fgcolor, const rectangle& r, element_state, const data& crook_data) override
			{
		...
		{
			bool draw(graph_reference graph, const ::color&, const ::color&, const ::rectangle& r, element_state, direction dir) override
			{
			...
			: public arrow_interface
		{
			...
			static void _m_line(paint::graphics & graph, int x, int y, bool horizontal)
			{
				graph.set_pixel(x, y);
				if (horizontal)
				{
		...
		{
			bool draw(graph_reference graph, const ::color& arg_bgcolor, const ::color&, const rectangle& r, element_state estate) override
			{
		...
		class x_icon
			: public element_interface
		{
			bool draw(graph_reference graph, const ::color&, const ::color& fgcolor, const rectangle& r, element_state estate) override
			{
				auto clr = fgcolor;
	...
	namespace element
	{
			...
			: public draw_method
		{
			...
			draw_image(const paint::image& img)
				: image(img)
			{}
			...
			void paste(const rectangle& from_r, graph_reference dst, const point& dst_pos) override
			{
				image.paste(from_r, dst, dst_pos);
			}

			void stretch(const rectangle& from_r, graph_reference dst, const rectangle& to_r) override
			{
			...
			: public draw_method
		{
			...
			draw_graph(const paint::graphics& g)
				: graph(g)
			{}
		...
		bground& bground::image(const paint::image& img, bool vertical, const rectangle& valid_area)
		{
			delete method_;
			...
			if (valid_area.width && valid_area.height)
				...
				valid_area_ = rectangle(img.size());
			return *this;
		}

		bground& bground::image(const paint::graphics& graph, bool vertical, const rectangle& valid_area)
		{
			delete method_;
			...
			if (valid_area.width && valid_area.height)
				...
				valid_area_ = rectangle(graph.size());
			return *this;
		}
		...
		bool bground::draw(graph_reference dst, const ::color&, const ::color&, const rectangle& to_r, element_state state)
		{
			...
			if (left_)
			{
				...
				if (bottom_)
				{
					...
					src_r.height = bottom_;
					method_->paste(src_r, dst, point(to_r.x, to_r.y + static_cast<int>(to_r.height - bottom_)));
				}
			}
			...
			if (right_)
			{
				...
				{
					src_r.height = top_;
					method_->paste(src_r, dst, point(to_x, to_r.y));
				}
				if (bottom_)
				{
					src_r.y += (static_cast<int>(from_r.height) - static_cast<int>(bottom_));
					src_r.height = bottom_;
					method_->paste(src_r, dst, point(to_x, to_r.y + int(to_r.height - bottom_)));
				}
			}
