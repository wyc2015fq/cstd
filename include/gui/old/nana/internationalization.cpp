...
namespace nana
{
	namespace internationalization_parts
	{
		...
		void load(const std::string& file, bool utf8)
		{
			...
			while (true)
			{
				...
				if (!utf8)
					msgid = charset(std::move(msgid)).to_bytes(unicode::utf8);
				...
				if (utf8)
					str = charset(std::move(tknizer.get_str()), unicode::utf8);
				else
					str = charset(std::move(tknizer.get_str()));
	...
	bool internationalization::_m_get(std::string& msgid, std::string& msgstr) const
	{
		...
		msgstr = charset(std::move(msgid), unicode::utf8);
		return false;
	}
	...
	{
		args_.emplace_back(new arg_string(charset(str)));
	}
	...
	void i18n_eval::_m_add_args(std::string&& str)
	{
		args_.emplace_back(new arg_string(charset(std::move(str))));
	}
