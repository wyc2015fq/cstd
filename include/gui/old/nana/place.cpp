...
namespace nana
{
	...
	namespace place_parts
	{
		...
		private:
			...
			{
				throw std::runtime_error("place: " + err + " at " + std::to_string(static_cast<unsigned>(sp_ - divstr_)));
			}
			...
			if (splitter_.empty())
			{
				...
				auto grab_fn = [this](const arg_mouse& arg)
				{
					...
					if (event_code::mouse_down == arg.evt_code)
					{
						...
						if (cursor::size_we != splitter_cursor_)
						{
					...
					else if (event_code::mouse_move == arg.evt_code)
					{
						...
						const bool vert = (::cursor::size_we != splitter_cursor_);
	...
	private:
		cursor	splitter_cursor_{cursor::arrow};
	...
	private:
		...
		void notify_move() override
		{
			...
			if (!indicator_.docker)
			{
				...
				indicator_.docker->z_order(nullptr, ::z_order_action::topmost);
		...
		: public division
	{
		...
		public:
				...
				: panel<true>(wd, true), dir_(dir), dock_dv_(dock_dv), pane_dv_(pane_dv)
			{
				...
				this->cursor(_m_is_vert(dir_) ? ::cursor::size_ns : ::cursor::size_we);
	...
	private:
		static bool _m_is_vert(::direction dir)
		{
	...
	place& place::dock(const std::string& name, std::string factory_name, std::function<std::unique_ptr<widget>(window)> factory)
	{
		...
		if (!factory_name.empty())
		{
			if (impl_->dock_factoris.find(factory_name) != impl_->dock_factoris.end())
				throw std::invalid_argument("place - the specified factory name(" + factory_name + ") already exists");
