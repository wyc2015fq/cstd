	...
	namespace drawerbase
	{
		namespace tabbar
		{
			...
			public:
				...
				any& at_no_bound_check(std::size_t pos)
				{
					return iterator_at(pos)->value;
				}

				const any& at(std::size_t pos) const
				{
					if(pos < list_.size())
				...
				const any& at_no_bound_check(std::size_t pos) const
				{
					return iterator_at(pos)->value;
				}
				...
				void attach(window wd, paint::graphics& graph)
				{
					basis_.wd = wd;
					basis_.graph = &graph;
				}
				...
				void insert(std::size_t pos, native_string_type&& text, any&& value)
				{
					if (pos >= list_.size())
						pos = list_.size();
				...
				bool tab_color(std::size_t pos, bool is_bgcolor, const ::color& clr)
				{
					if (pos >= list_.size())
						throw std::out_of_range("tabbar: invalid position");
				...
				void tab_image(std::size_t pos, const paint::image& img)
				{
					if (pos >= list_.size())
						throw std::out_of_range("tabbar: invalid position");
					...
					for(auto i = list_.cbegin(); i != list_.cend(); ++i, ++index)
					{
						...
						if(m.r.x + static_cast<int>(basis_.item_pixels) > 0)
						{
							...
							if(item.text.size())
							{
								...
								paint::text_renderer tr(*basis_.graph);
			...
			//class trigger
				...
				any& trigger::at(std::size_t i) const
				{
					return layouter_->at(i);
				}

				any& trigger::at_no_bound_check(std::size_t i) const
				{
				...
				void trigger::insert(std::size_t pos, native_string_type&& text, any&& value)
				{
				...
				void trigger::tab_color(std::size_t i, bool is_bgcolor, const ::color& clr)
				{
					if(layouter_->tab_color(i, is_bgcolor, clr))
						API::refresh_window(layouter_->widget_handle());
				}

				void trigger::tab_image(std::size_t i, const paint::image& img)
				{
...
namespace nana
{
		...
		void tabbar_lite::push_back(std::string text, ::any any)
		{
			auto & items = get_drawer_trigger().get_model()->items();
			internal_scope_guard lock;
		...
		void tabbar_lite::push_front(std::string text, ::any any)
		{
			auto & items = get_drawer_trigger().get_model()->items();
			internal_scope_guard lock;
