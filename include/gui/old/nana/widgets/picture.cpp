...
namespace nana
{
				...
				if (graph && (bground_mode::basic != API::effects_bground_mode(*impl_->wdg_ptr)))
				{
					if (w < graph->size().width || h < graph->size().height /*  .width   ???  */ || impl_->backimg.image.alpha())
					{
						...
						if (bground.gradual_from.invisible() || bground.gradual_to.invisible())
							...
							graph->gradual_rectangle(::rectangle{ graph->size() }, bground.gradual_from, bground.gradual_to, !bground.horizontal);
					}
				}
			}
			//end class drawer
	...
	//class picture
		...
		void picture::load(::paint::image img, const ::rectangle& valid_area)
		{
