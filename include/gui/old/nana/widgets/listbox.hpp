		...
		namespace listbox
		{
			...
			struct cell
			{
				struct format
				{
					...
					format() = default;
					format(const ::color& bgcolor, const ::color& fgcolor);
				};
				...
				cell(::std::string, const ::color& bgcolor, const ::color& fgcolor);
			...
			private:
				void clear() override
				{
					throw std::runtime_error("listbox disallow to remove items because of immutable model");
				}
			...
			public:
				...
				STLContainer& container()
				{
					...
					if (!model_)
						throw std::runtime_error("listbox empty model_guard");
...
4. A sort compare is used for sorting the items. It is a strict weak ordering comparer that must meet the requirement:
	...
	A simple example.
		bool sort_compare( const std::string& s1, any*, 
						   const std::string& s2, any*, bool reverse)
		{
	...
	sorted by comparing these customer's object.
		bool sort_compare( const std::string&, any* o1, 
						   const std::string&, any* o2, bool reverse)
		{
...
with API::dev::make_scheme<Scheme>() which call API::detail::make_scheme(::detail::scheme_factory<Scheme>())
which call restrict::bedrock.make_scheme(static_cast<::detail::scheme_factory_base&&>(factory));
...
the detail::basic_window member pointer scheme
...
*/
	...
	//Associative category access
		...
		*/
		...
		{
			using key_type = typename ::detail::type_escape<const typename std::decay<Key>::type>::type;

			auto p = std::make_shared<key<key_type, std::less<key_type>>>(std::forward<Key>(key));
		...
		*/
		...
		void assoc_erase(Key&& key)
		{
			...
			::key<key_type, std::less<key_type>> wrap(key);
			_m_erase_key(&wrap);
		}
		...
		*/
		...
		void set_sort_compare(	size_type col,
								std::function<bool(const std::string&, any*, const std::string&, any*, bool reverse)> strick_ordering);
	...
	private:
		...
		drawerbase::listbox::category_t* _m_assoc(std::shared_ptr<detail::key_interface>, bool create_if_not_exists);
		void _m_erase_key(detail::key_interface*);
	};
}//end namespace nana
