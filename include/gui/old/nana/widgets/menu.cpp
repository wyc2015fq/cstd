			...
			private:
				...
				void item(graph_reference graph, const rectangle& r, const attr& at)
				{
					if(at.item_state == state::active)
					{
						...
						point points[4] = {
							point(r.x, r.y),
							point(r.x + r.width - 1, r.y),
							point(r.x, r.y + r.height - 1),
				...
				void item_image(graph_reference graph, const point& pos, unsigned image_px, const paint::image& img)
				{
				...
				void item_text(graph_reference graph, const point& pos, const std::string& text, unsigned text_pixels, const attr& at)
				{
					graph.palette(true, at.enabled ? colors::black : colors::gray_border);
					paint::text_renderer tr(graph);
				...
				void refresh(graph_reference graph)
				{
					...
					for (auto & m : menu_->items)
					{
						...
						if (m.image.empty() == false)
							renderer->item_image(graph, point(item_r.x + 5, item_r.y + static_cast<int>(item_h_px - image_px) / 2 - 1), image_px, m.image);
