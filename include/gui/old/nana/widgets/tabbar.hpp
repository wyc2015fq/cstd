...
namespace nana
{
	...
	namespace drawerbase
	{
		namespace tabbar
		{
			...
			public:
				...
				any& at(std::size_t) const;
				any& at_no_bound_check(std::size_t) const;
				...
				void insert(std::size_t, native_string_type&&, any&&);
				std::size_t length() const;
				bool close_fly(bool);
				window attach(std::size_t, window, bool drop_other);
				void erase(std::size_t);
				void tab_color(std::size_t, bool is_bgcolor, const ::color&);
				void tab_image(size_t, const paint::image&);
	...
	public:
		...
		{
			return this->append( static_cast<std::wstring&&>(charset(std::move(text), unicode::utf8)), attach_wd, std::move(value));
		}
		...
		tabbar& append(std::wstring text, window attach_wd, value_type value = {})
		{
			...
			this->get_drawer_trigger().insert(::npos, to_nstring(std::move(text)), std::move(value));
			if (attach_wd)
		...
		{
			this->get_drawer_trigger().insert(::npos, to_nstring(std::move(text)), value_type());
			API::update_window(*this);
		}
		...
		void tab_bgcolor(std::size_t pos, const ::color& clr)
		{
			this->get_drawer_trigger().tab_color(pos, true, clr);
		}

		void tab_fgcolor(std::size_t pos, const ::color& clr)
		{
...
{	
	...
	public:
		tabbar_lite() = default;
		tabbar_lite(window, bool visible = true, const::rectangle& = {});
	...
	public: //modifiers
		...
		void push_back(std::string text, ::any par = {});
		void push_front(std::string text, ::any par = {});
