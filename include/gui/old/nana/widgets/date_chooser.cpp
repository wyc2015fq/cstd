	...
	namespace drawerbase
	{
		namespace date_chooser
		{
			...
			void perf_transform_helper(window window_handle, transform_action tfid, trigger::graph_reference graph, trigger::graph_reference dirtybuf, trigger::graph_reference newbuf, const point& refpos)
			{
				...
				{
					paint::graphics dzbuf(newbuf.size());
					paint::graphics nzbuf(newbuf.size());
					...
					for (int i = 1; i < count; ++i)
					{
						...
						nzbuf.blend(::rectangle{ nzbuf.size() }, dzbuf, point(), fade * (count - i));
			...
			public:
				...
				void render(graph_reference graph)
				{
					...
					if (graph.height() > 2 + topbar_height)
					{
						...
						paint::graphics gbuf({ width, graph.height() - 2 - topbar_height });
				...
				void perf_transform(graph_reference graph, window window_handle, transform_action transf)
				{
					...
					paint::graphics dirtybuf({ r.width, r.height });
					dirtybuf.bitblt(r, graph, refpos);

					render(graph);

					paint::graphics gbuf({ r.width, r.height });
