...
namespace nana
{
	...
	namespace drawerbase
	{
		namespace combox
		{
			...
			{
			public:
				std::shared_ptr<detail::key_interface> key;

				paint::image	item_image;
				std::string		item_text;
				mutable std::shared_ptr<any>	any_ptr;

				item(std::shared_ptr<detail::key_interface> && kv)
					: key(std::move(kv))
				{
				}
			...
			private:
				...
				const paint::image & image() const override
				{
					return item_image;
				}
			...
			public:
				...
				any * anyobj(std::size_t pos, bool allocate_if_empty) const
				{
					if(pos >= items_.size())
						return nullptr;
					...
					if (allocate_if_empty && (nullptr == any_ptr))
						any_ptr = std::make_shared<any>();
					return any_ptr.get();
				}

				void text_area(const size& s)
				{
					rectangle r(2, 2, s.width > 19 ? s.width - 19 : 0, s.height > 4 ? s.height - 4 : 0);
					if(image_enabled_)
					{
				...
				void editable(bool enb)
				{
					if(editor_)
					{
						...
						if (!enb)
						{
							editor_->ext_renderer().background = [this](graph_reference graph, const ::rectangle&, const ::color&)
							{
				...
				void option(std::size_t index, bool ignore_condition)
				{
					...
					if(ignore_condition || (old_index != index) || (items_[index]->item_text != widget_->caption()))
					{
						...
						editor_->text(::charset(items_[index]->item_text, ::unicode::utf8), false);
				...
				std::size_t at_key(std::shared_ptr<detail::key_interface>&& p)
				{
					std::size_t pos = 0;
					for (auto & m : items_)
					{
				...
				void image(std::size_t pos, const paint::image& img)
				{
					if (pos < items_.size())
					{
			...
			//class item_proxy
				...
				item_proxy& item_proxy::icon(const paint::image& img)
				{
				...
				paint::image item_proxy::icon() const
				{
	...
	//class combox
		...
		void combox::image(std::size_t i, const paint::image& img)
		{
			internal_scope_guard lock;
			if(empty()) return;
		...
		paint::image combox::image(std::size_t pos) const
		{
			internal_scope_guard lock;
		...
		any * combox::_m_anyobj(std::size_t pos, bool alloc_if_empty) const
		{
			internal_scope_guard lock;
			return _m_impl().anyobj(pos, alloc_if_empty);
		}

		auto combox::_m_at_key(std::shared_ptr<detail::key_interface>&& p) -> item_proxy
		{
			internal_scope_guard lock;
			auto & impl = _m_impl();
		...
		void combox::_m_erase(detail::key_interface* p)
		{
			internal_scope_guard lock;
			_m_impl().erase(p);
		}
