	...
	//class window_manager
		...
		//class revertible_mutex
			...
			void window_manager::revertible_mutex::lock()
			{
				...
				if(0 == thr_.tid)
					thr_.tid = system::this_thread_id();
				++thr_.refcnt;
			}
			...
			{
				if(thr_.tid == system::this_thread_id())
					if(0 == --thr_.refcnt)
						thr_.tid = 0;
			...
			{
				if(thr_.refcnt && (thr_.tid == system::this_thread_id()))
				{
			...
			void window_manager::revertible_mutex::forward()
			{
				...
				if(stack_.size())
				{
					...
					if(thr.tid == system::this_thread_id())
					{
						stack_.pop_back();
		...
		void window_manager::default_icon(const paint::image& _small, const paint::image& big)
		{
		...
		{	
			...
			if(category::flags::lite_widget != wd->other.category)
			{
				...
				if(category::flags::root == wd->other.category)
				{
					...
					if(false == passive)
						native_interface::window_size(wd->root, sz + size(wd->extra_width, wd->extra_height));
				}
#ifndef WIDGET_FRAME_DEPRECATED
		...
		bool window_manager::get_graphics(core_window_t* wd, paint::graphics& result)
		{
			//Thread-Safe Required!
			std::lock_guard<decltype(mutex_)> lock(mutex_);
			if (!impl_->wd_register.available(wd))
		...
		void window_manager::_m_disengage(core_window_t* wd, core_window_t* for_new)
		{
			...
			if (established)
			{
				...
				std::function<void(core_window_t*, const point&)> set_pos_root;
