...
namespace API
{
	using basic_window = ::detail::basic_window;
	using interface_type = ::detail::native_interface;

	namespace detail
	{
		::widget_geometrics* make_scheme(::detail::scheme_factory_interface&& factory)
		{
			return restrict::bedrock.scheme().create(static_cast<::detail::scheme_factory_interface&&>(factory));
		}

		bool emit_event(event_code evt_code, window wd, const ::event_arg& arg)
		{
			return restrict::bedrock.emit(evt_code, reinterpret_cast<::detail::basic_window*>(wd), arg, true, restrict::bedrock.get_thread_context());
		}
	...
	namespace dev
	{
		...
		::detail::native_string_type window_caption(window wd) throw()
		{
			auto const iwd = reinterpret_cast<basic_window*>(wd);
			internal_scope_guard isg;
		...
		void window_caption(window wd, ::detail::native_string_type title)
		{
			auto const iwd = reinterpret_cast<basic_window*>(wd);
			internal_scope_guard lock;
			if (restrict::wd_manager().available(iwd))
	...
	{
		auto iwd = reinterpret_cast<::detail::basic_window*>(wd);
		internal_scope_guard lock;
	...
	void create_caret(window wd, const size& caret_size)
	{
		...
		if (restrict::wd_manager().available(iwd) && !(iwd->annex.caret_ptr))
			iwd->annex.caret_ptr = new ::detail::caret(iwd, caret_size);
	}
	...
	bool window_graphics(window wd, paint::graphics& graph)
	{
		return restrict::wd_manager().get_graphics(reinterpret_cast<basic_window*>(wd), graph);
	}

	bool root_graphics(window wd, paint::graphics& graph)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if(restrict::wd_manager().available(iwd))
	...
	void typeface(window wd, const paint::font& font)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if(restrict::wd_manager().available(iwd))
