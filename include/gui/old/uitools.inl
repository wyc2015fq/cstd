 ...
 */
#define DrawEdge DrawEdge_
#define DrawFrameControl DrawFrameControl_
#define SetRect SetRect_
#define SetRectEmpty SetRectEmpty_
#define CopyRect CopyRect_
#define IsRectEmpty IsRectEmpty_
#define PtInRect PtInRect_
#define OffsetRect OffsetRect_
#define InflateRect InflateRect_
#define IntersectRect IntersectRect_
#define UnionRect UnionRect_
#define EqualRect EqualRect_
#define SubtractRect SubtractRect_
#define FillRect FillRect_
#define InvertRect InvertRect_
#define FrameRect FrameRect_
#define DrawFocusRect DrawFocusRect_
#define DrawStateA DrawStateA_
#define DrawStateW DrawStateW_
#define ERR printf
#include <stdarg.h>
...
#define RGB_ENTRY(name,val) {NULL, (val)}
...
HPEN SYSCOLOR_GetPen(INT index)
{
  ...
  if (!system_colors[index].pen) {
    system_colors[index].pen = CreatePen(PS_SOLID, 1, GetSysColor(index));
  }
    ...
    UINT uType, UINT uFlags)
{
  ...
  if (InnerI != -1) {
    InnerPen = SYSCOLOR_GetPen(InnerI);
  }
  if (OuterI != -1) {
    OuterPen = SYSCOLOR_GetPen(OuterI);
  }
...
INT WINAPI FillRect(HDC hdc, const RECT* rect, HBRUSH hbrush)
{
  ...
  if (hbrush <= (HBRUSH)(COLOR_MAX + 1)) {
    hbrush = GetSysColorBrush(HandleToULong(hbrush) - 1);
  }
  prev_brush = (HBRUSH)SelectObject(hdc, hbrush);
    ...
    UINT uType, UINT uFlags)
{
  ...
  if (LTInnerI != -1) {
    LTInnerPen = SYSCOLOR_GetPen(LTInnerI);
  }
  if (LTOuterI != -1) {
    LTOuterPen = SYSCOLOR_GetPen(LTOuterI);
  }
  if (RBInnerI != -1) {
    RBInnerPen = SYSCOLOR_GetPen(RBInnerI);
  }
  if (RBOuterI != -1) {
    RBOuterPen = SYSCOLOR_GetPen(RBOuterI);
  }
  MoveToEx(hdc, 0, 0, &SavePoint);
  /* Draw the outer edge */
...
char* wine_dbgstr_rect(const RECT* rc)
{
  static char buf[64];
  _snprintf(buf, 64, "l=%d,t=%d,r=%d,b=%d", rc->left, rc->top, rc->right, rc->bottom);
  return buf;
}
/**********************************************************************
 * DrawEdge (USER32.@)
...
HBRUSH SYSCOLOR_55AABrush = CreatePatternBrush(h55AABitmap);
static void UITOOLS_DrawCheckedRect(HDC dc, LPRECT rect)
{
  if (GetSysColor(COLOR_BTNHIGHLIGHT) == RGB(255, 255, 255)) {
    ...
    hbsave = (HBRUSH)SelectObject(dc, SYSCOLOR_55AABrush);
...
INT WINAPI FrameRect(HDC hdc, const RECT* rect, HBRUSH hbrush)
{
  ...
  if (!(prevBrush = (HBRUSH)SelectObject(hdc, hbrush))) {
    return 0;
  }
 ...
 */
BOOL WINAPI DrawAnimatedRects_(HWND hwnd, INT idAni, const RECT* lprcFrom, const RECT* lprcTo)
{
