// Platform source code edit control
/** @file PlatWin.cxx
 ** Implementation of platform facilities on Windows.
 **/
// Copyright 1998-2003 by Neil Hodgson <neilh@Platform.org>
...
// Want to use MIN and std::max so don't want Windows.h version of min and max
#if !defined(NOMINMAX)
#define NOMINMAX
#endif
#undef _WIN32_WINNT
...
#include "scintilla/DBCS.hpp"
#include "scintilla/FontQuality.h"
...
UINT CodePageFromCharSet(DWORD characterSet, UINT documentCodePage)
{
  ...
  case SC_CHARSET_DEFAULT:
    return documentCodePage ? documentCodePage : 1252;
...
RECT RectFromRectF(RectF prc) noexcept {
  RECT rc = { static_cast<LONG>(prc.left), static_cast<LONG>(prc.top),
      static_cast<LONG>(prc.right), static_cast<LONG>(prc.bottom) };
  return rc;
}

RectF RectFFromRect(RECT prc) noexcept {
  return RectF( static_cast<XYPOSITION>(prc.left), static_cast<XYPOSITION>(prc.top),
      static_cast<XYPOSITION>(prc.right), static_cast<XYPOSITION>(prc.bottom) );
}
...
bool LoadD2D()
{
  ...
  if (!triedLoadingD2D) {
    ...
    if (hDLLD2D) {
      ...
      if (fnD2DCF) {
        // A single threaded factory as Platform always draw on the GUI thread
...
void SetLogFont(LOGFONTW & lf, const char* faceName, int characterSet, float size, int weight, bool italic, int extraFontFlag)
{
  ...
  UniConversion::UTF16FromUTF8(faceName, lf.lfFaceName, LF_FACESIZE);
}
  ...
  next(0), usage(0), size(1.0), hash(0)
{
  ...
  if (technology == SCWIN_TECH_GDI) {
    ...
    UniConversion::UTF16FromUTF8(fp.faceName, wszFace, faceSize);
...
bool FontCached::SameAs(const FontParameters & fp)
{
  if (
    ...
    UniConversion::UTF16FromUTF8(fp.faceName, wszFace, LF_FACESIZE);
...
struct TextWide : public VarBuffer<wchar_t, stackBufferLength> {
  int tlen;	// Using int instead of size_t as most Win32 APIs take int.
  TextWide(std::string text, bool unicodeMode, int codePage = 0) :
    VarBuffer<wchar_t, stackBufferLength>(text.length()) {
    if (unicodeMode) {
      tlen = static_cast<int>(UniConversion::UTF16FromUTF8(text, buffer, text.length()));
    } else {
...
DWORD dwordMultiplied(Color colour, unsigned int alpha)
{
...
{
  return Color(::GetSysColor(COLOR_3DHIGHLIGHT));
}
...
struct SurfaceGDI : public Surface {
  bool unicodeMode;
  HDC hdc;
  bool hdcOwned;
  HPEN pen;
...
public:
  ...
  SurfaceGDI::~SurfaceGDI() override {
    Clear();
  }
  ...
  void SurfaceGDI::PenColour(Color fore) {
    if (pen) {
      ::SelectObject(hdc, penOld);
      ::DeleteObject(pen);
      pen = 0;
    ...
    pen = ::CreatePen(0, 1, fore.GetBGR());
    penOld = SelectPen(hdc, pen);
  }

  void SurfaceGDI::BrushColor(Color back) {
    if (brush) {
      ::SelectObject(hdc, brushOld);
      ::DeleteObject(brush);
      brush = 0;
    ...
    const Color colourNearest = Color(::GetNearestColor(hdc, back.GetBGR()));
    brush = ::CreateSolidBrush(colourNearest.GetBGR());
  ...
  void SurfaceGDI::Polygon(PointF* pts, size_t npts, Color fore, Color back) {
    PenColour(fore);
    BrushColor(back);
  ...
  void SurfaceGDI::RectangleDraw(RectF rc, Color fore, Color back) {
    PenColour(fore);
    BrushColor(back);
    const RECT rcw = RectFromRectF(rc);
    ::Rectangle(hdc, rcw.left, rcw.top, rcw.right, rcw.bottom);
  }

  void SurfaceGDI::FillRectangle(RectF rc, Color back) {
    // Using ExtTextOut rather than a FillRect ensures that no dithering occurs.
    // There is no need to allocate a brush either.
    const RECT rcw = RectFromRectF(rc);
    ::SetBkColor(hdc, back.GetBGR());
    ::ExtTextOut(hdc, rcw.left, rcw.top, ETO_OPAQUE, &rcw, TEXT(""), 0, nullptr);
  }

  void SurfaceGDI::FillRectangle(RectF rc, Surface & surfacePattern) {
    HBRUSH br;
  ...
  void SurfaceGDI::RoundedRectangle(RectF rc, Color fore, Color back) {
    PenColour(fore);
    BrushColor(back);
    const RECT rcw = RectFromRectF(rc);
...
#pragma comment(lib, "msimg32.lib")
  void SurfaceGDI::AlphaRectangle(RectF rc, int cornerSize, Color fill, int alphaFill,
                                  Color outline, int alphaOutline, int /* flags*/) {
    const RECT rcw = RectFromRectF(rc);
    if (rc.Width() > 0) {
  ...
  void SurfaceGDI::GradientRectangle(RectF rc, const std::vector<ColourStop> & stops, GradientOptions) {
    // Would be better to average start and end.
    const Color colourAverage = stops[0].colour.MixedWith(stops[1].colour);
    AlphaRectangle(rc, 0, colourAverage, colourAverage.GetAlpha(),
                   colourAverage, colourAverage.GetAlpha(), 0);
  }

  void SurfaceGDI::DrawRGBAImage(RectF rc, int width, int height, const unsigned char* pixelsImage) {
    if (rc.Width() > 0) {
      HDC hMemDC = ::CreateCompatibleDC(hdc);
      if (rc.Width() > width) {
  ...
  void SurfaceGDI::Ellipse(RectF rc, Color fore, Color back) {
    PenColour(fore);
    BrushColor(back);
    const RECT rcw = RectFromRectF(rc);
  ...
  void SurfaceGDI::Copy(RectF rc, PointF from, Surface & surfaceSource) {
    ::BitBlt(hdc,
  ...
  IScreenLineLayout* SurfaceGDI::Layout(const IScreenLine*) {
    return NULL;
  }

  typedef VarBuffer<int, stackBufferLength> TextPositionsI;

  void SurfaceGDI::DrawTextCommon(RectF rc, Font & font_, XYPOSITION ybase, std::string text, UINT fuOptions) {
    SetFont(font_);
    const RECT rcw = RectFromRectF(rc);
  ...
  void SurfaceGDI::DrawTextNoClip(RectF rc, Font & font_, XYPOSITION ybase, std::string text,
                                  Color fore, Color back) {
    ::SetTextColor(hdc, fore.GetBGR());
    ::SetBkColor(hdc, back.GetBGR());
    DrawTextCommon(rc, font_, ybase, text, ETO_OPAQUE);
  }

  void SurfaceGDI::DrawTextClipped(RectF rc, Font & font_, XYPOSITION ybase, std::string text,
                                   Color fore, Color back) {
    ::SetTextColor(hdc, fore.GetBGR());
    ::SetBkColor(hdc, back.GetBGR());
  ...
  void SurfaceGDI::DrawTextTransparent(RectF rc, Font & font_, XYPOSITION ybase, std::string text,
                                       Color fore) {
  ...
  XYPOSITION SurfaceGDI::WidthText(Font & font_, std::string text) {
    SetFont(font_);
    SIZE sz = { 0, 0 };
    if (!unicodeMode) {
      ::GetTextExtentPoint32A(hdc, text.data(), MIN(static_cast<int>(text.length()), maxLenText), &sz);
    } else {
      const TextWide tbuf(text, unicodeMode, codePage);
  ...
  void SurfaceGDI::MeasureWidths(Font & font_, std::string text, XYPOSITION* positions) {
    ...
    if (unicodeMode) {
      ...
      for (int ui = 0; ui < fit; ui++) {
        ...
        const unsigned int byteCount = UniConversion::UTF8BytesOfLead[uch];
  ...
  void SurfaceGDI::SetClip(RectF rc) {
...
struct SurfaceD2D : public Surface {
  bool unicodeMode;
  int x, y;
  ...
  void Polygon(PointF* pts, size_t npts, Color fore, Color back) override;
  void RectangleDraw(RectF rc, Color fore, Color back) override;
  void FillRectangle(RectF rc, Color back) override;
  void FillRectangle(RectF rc, Surface & surfacePattern) override;
  void RoundedRectangle(RectF rc, Color fore, Color back) override;
  void AlphaRectangle(RectF rc, int cornerSize, Color fill, int alphaFill,
                      Color outline, int alphaOutline, int flags) override;
  void GradientRectangle(RectF rc, const std::vector<ColourStop> & stops, GradientOptions options) override;
  void DrawRGBAImage(RectF rc, int width, int height, const unsigned char* pixelsImage) override;
  void Ellipse(RectF rc, Color fore, Color back) override;
  void Copy(RectF rc, PointF from, Surface & surfaceSource) override;

  IScreenLineLayout* Layout(const IScreenLine* screenLine) override;

  void DrawTextCommon(RectF rc, Font & font_, XYPOSITION ybase, std::string text, UINT fuOptions);
  void DrawTextNoClip(RectF rc, Font & font_, XYPOSITION ybase, std::string text, Color fore, Color back) override;
  void DrawTextClipped(RectF rc, Font & font_, XYPOSITION ybase, std::string text, Color fore, Color back) override;
  void DrawTextTransparent(RectF rc, Font & font_, XYPOSITION ybase, std::string text, Color fore) override;
  void MeasureWidths(Font & font_, std::string text, XYPOSITION* positions) override;
  XYPOSITION WidthText(Font & font_, std::string text) override;
  ...
  void SetClip(RectF rc) override;
  ...
  void SurfaceD2D::Release() override {
    Clear();
  }
  ...
  bool SurfaceD2D::Initialised() override {
  ...
  void SurfaceD2D::Init(WindowID /* wid */) override {
    Release();
    SetScale();
  }

  void SurfaceD2D::Init(SurfaceID sid, WindowID) override {
    Release();
    SetScale();
    pRenderTarget = static_cast<ID2D1RenderTarget*>(sid);
  }

  void SurfaceD2D::InitPixMap(int width, int height, Surface* surface_, WindowID) override {
    Release();
    SetScale();
  ...
  void SurfaceD2D::PenColour(Color fore) override {
    D2DPenColour(fore);
  }

  void SurfaceD2D::D2DPenColour(Color fore, int alpha = 255) {
    if (pRenderTarget) {
      D2D_COLOR_F col;
      col.r = (fore.GetBGR() & 0xff) / 255.0f;
      col.g = ((fore.GetBGR() & 0xff00) >> 8) / 255.0f;
      col.b = (fore.GetBGR() >> 16) / 255.0f;
      col.a = alpha / 255.0f;
      if (pBrush) {
        pBrush->SetColor(col);
      } else {
  ...
  void SurfaceD2D::SetFont(Font & font_) {
    ...
    if (pfm->characterSet) {
      codePageText = Platform::CodePageFromCharSet(pfm->characterSet, codePage);
    }
    if (pRenderTarget) {
  ...
  int SurfaceD2D::LogPixelsY() override {
    return logPixelsY;
  }

  int SurfaceD2D::DeviceHeightFont(int points) override {
    return ::MulDiv(points, LogPixelsY(), 72);
  }

  void SurfaceD2D::MoveTo(int x_, int y_) override {
    x = x_;
    y = y_;
  }
};
...
void SurfaceD2D::Polygon(PointF* pts, size_t npts, Color fore, Color back)
{
  if (pRenderTarget) {
...
void SurfaceD2D::RectangleDraw(RectF rc, Color fore, Color back)
{
  if (pRenderTarget) {
...
void SurfaceD2D::FillRectangle(RectF rc, Color back)
{
  if (pRenderTarget) {
    D2DPenColour(back);
...
void SurfaceD2D::FillRectangle(RectF rc, Surface & surfacePattern)
{
...
void SurfaceD2D::RoundedRectangle(RectF rc, Color fore, Color back)
{
  if (pRenderTarget) {
...
void SurfaceD2D::AlphaRectangle(RectF rc, int cornerSize, Color fill, int alphaFill,
                                Color outline, int alphaOutline, int /* flags*/)
{
  if (pRenderTarget) {
    if (cornerSize == 0) {
...
namespace
{
  ...
  D2D_COLOR_F ColorFromColourAlpha(Color colour) noexcept {
    D2D_COLOR_F col;
...
void SurfaceD2D::GradientRectangle(RectF rc, const std::vector<ColourStop> & stops, GradientOptions options)
{
  if (pRenderTarget) {
...
void SurfaceD2D::DrawRGBAImage(RectF rc, int width, int height, const unsigned char* pixelsImage)
{
  if (pRenderTarget) {
    if (rc.Width() > width) {
      rc.left += floor((rc.Width() - width) / 2);
...
void SurfaceD2D::Ellipse(RectF rc, Color fore, Color back)
{
  if (pRenderTarget) {
...
void SurfaceD2D::Copy(RectF rc, PointF from, Surface & surfaceSource)
{
...
struct BlobInline : public IDWriteInlineObject {
  XYPOSITION width;
...
public:
BlobInline(XYPOSITION width_ = 0.0f) noexcept :
  width(width_) {
  }
...
struct ScreenLineLayout : public IScreenLineLayout {
  ...
  static std::wstring ReplaceRepresentation(std::string text);
  static size_t GetPositionInLayout(std::string text, size_t position);
public:
...
void ScreenLineLayout::FillTextLayoutFormats(const IScreenLine* screenLine, IDWriteTextLayout* textLayout, std::vector<BlobInline> & blobs)
{
  ...
  std::string text = screenLine->Text();
  ...
  for (size_t bytePosition = 0; bytePosition < screenLine->Length();) {
    ...
    const unsigned int byteCount = UniConversion::UTF8BytesOfLead[uch];
    const UINT32 codeUnits = static_cast<UINT32>(UniConversion::UTF16LengthFromUTF8ByteCount(byteCount));
...
std::wstring ScreenLineLayout::ReplaceRepresentation(std::string text)
{
...
size_t ScreenLineLayout::GetPositionInLayout(std::string text, size_t position)
{
  const std::string textUptoPosition = text.substr(0, position);
  return UniConversion::UTF16Length(textUptoPosition);
}
...
size_t ScreenLineLayout::PositionFromX(XYPOSITION xDistance, bool charPosition)
{
  ...
  return UniConversion::UTF8PositionFromUTF16Position(text, pos);
}
...
IScreenLineLayout* SurfaceD2D::Layout(const IScreenLine* screenLine)
{
  return (ScreenLineLayout*)(screenLine);
}

void SurfaceD2D::DrawTextCommon(RectF rc, Font & font_, XYPOSITION ybase, std::string text, UINT fuOptions)
{
  SetFont(font_);
  // Use Unicode calls
...
void SurfaceD2D::DrawTextNoClip(RectF rc, Font & font_, XYPOSITION ybase, std::string text,
                                Color fore, Color back)
{
  if (pRenderTarget) {
...
void SurfaceD2D::DrawTextClipped(RectF rc, Font & font_, XYPOSITION ybase, std::string text,
                                 Color fore, Color back)
{
  if (pRenderTarget) {
    FillRectangle(rc, back);
...
void SurfaceD2D::DrawTextTransparent(RectF rc, Font & font_, XYPOSITION ybase, std::string text,
                                     Color fore)
{
  // Avoid drawing spaces in transparent mode
  for (size_t i = 0; i < text.length(); i++) {
...
XYPOSITION SurfaceD2D::WidthText(Font & font_, std::string text)
{
  FLOAT width = 1.0;
  SetFont(font_);
  const TextWide tbuf(text, unicodeMode, codePageText);
...
void SurfaceD2D::MeasureWidths(Font & font_, std::string text, XYPOSITION* positions)
{
  SetFont(font_);
  if (!pIDWriteFactory || !pTextFormat) {
  ...
  if (unicodeMode) {
    ...
    for (size_t i = 0; i < text.length() && ui < tbuf.tlen;) {
      positions[i] = poses.buffer[ui];
      if (DBCS::IsLeadByte(codePageText, text[i])) {
...
void SurfaceD2D::SetClip(RectF rc)
{
  if (pRenderTarget) {
    D2D1_RECT_F rcClip = { rc.left, rc.top, rc.right, rc.bottom };
    pRenderTarget->PushAxisAlignedClip(rcClip, D2D1_ANTIALIAS_MODE_ALIASED);
...
static LRESULT WINAPI guiWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (uMsg) {
  case WM_LBUTTONDOWN:
    host->mouseSignel.emit(MouseEvent(Event::Type::MouseButtonPress, Cursor::position(), Mouse::LeftButton, 0, Keyboard::NoModifier));
    break;
  case WM_LBUTTONUP:
    host->mouseSignel.emit(MouseEvent(Event::Type::MouseButtonRelease, Cursor::position(), Mouse::LeftButton, 0, Keyboard::NoModifier));
    break;
  case WM_LBUTTONDBLCLK:
    host->mouseSignel.emit(MouseEvent(Event::Type::MouseButtonDblClick, Cursor::position(), Mouse::LeftButton, 0, Keyboard::NoModifier));
    break;
  case WM_RBUTTONDOWN:
    host->mouseSignel.emit(MouseEvent(Event::Type::MouseButtonPress, Cursor::position(), Mouse::RightButton, 0, Keyboard::NoModifier));
    break;
  case WM_PAINT: {
    PAINTSTRUCT paint;
    SurfaceGDI surf;
    HDC hDC = BeginPaint(hwnd, &paint);
    RectF rcPaint = RectFFromRect(paint.rcPaint);
    surf.Init(hDC, hwnd);
    host->paintSignel.emit(&surf, rcPaint);
    EndPaint(hwnd, &paint);
                 }
  break;
  default: {
...
bool Window::Create()
{
  ...
  wid = ::CreateWindowExA(style_ex, IMGUIWIN32_CLASSNAME, "Window", style, 0, 0, 300, 300, NULL, 0, ::GetModuleHandle(0), 0);
  SetWindowLongPtrA((HWND)wid, GWLP_USERDATA, (LONG_PTR)this);
  return true;
}

int Window::SetCaption(const char* title)
{
  return ::SetWindowText((HWND)wid, title);
}
std::string Window::GetCaption() const
{
  char title[256];
  ::GetWindowText((HWND)wid, title, 256);
  return title;
}

int Window::Move(int x, int y, int width, int height)
{
  //A Window may have a border, this should be adjusted the client area fit for the specified size.
  ::RECT client, wd_area;
  ::GetClientRect((HWND)wid, &client);	//The right and bottom of client by GetClientRect indicate the width and height of the area
  ::GetWindowRect((HWND)wid, &wd_area);
  //if (x>0)
  {
    wd_area.left = x;
    wd_area.top = y;
  ...
  int ret = ::MoveWindow((HWND)wid, wd_area.left, wd_area.top, wd_area.right + delta_w, wd_area.bottom + delta_h, true);
  ::GetClientRect((HWND)wid, &client);
  ::GetWindowRect((HWND)wid, &wd_area);
...
RectF Window::GetPosition() const
{
  RECT rc;
  ::GetWindowRect(HwndFromWindowID(wid), &rc);
  return RectF::FromInts(rc.left, rc.top, rc.right, rc.bottom);
}

void Window::SetPosition(RectF rc)
{
...
void Window::SetPositionRelative(RectF rc, const Window* relativeTo)
{
  ...
  if (style & WS_POPUP) {
    ...
    const RECT rcMonitor = RectFromRectF(rc);
...
RectF Window::GetClientPosition() const
{
  RECT rc = { 0, 0, 0, 0 };
  if (wid) {
...
void Window::InvalidateRectangle(RectF rc)
{
  const RECT rcw = RectFromRectF(rc);
...
/* Returns rectangle of monitor pt is on, both rect and pt are in Window's
   coordinates */
RectF Window::GetMonitorRect(Point pt)
{
  const RectF rcPosition = GetPosition();
...
#if 0
struct ListBoxX : public ListBox {
  int lineHeight;
  FontID fontCopy;
  int technology;
  RGBAImageSet images;
  ...
  RectF rcPreSize;
  PointF dragOffset;
  PointF location;	// Caret location at which the list is opened
  int wheelDelta; // mouse wheel residue

  HWND GetHWND() const;
  void AppendListItem(const char* text, const char* numword);
  static void AdjustWindowRect(RectF* rc);
...
public:
  ...
  void Create(Window & parent_, int ctrlID_, PointF location_, int lineHeight_, bool unicodeMode_, int technology_) override;
  void SetAverageCharWidth(int width) override;
  void SetVisibleRows(int rows) override;
  int GetVisibleRows() const override;
  RectF GetDesiredRect() override;
...
void ListBoxX::Create(Window & parent_, int ctrlID_, PointF location_, int lineHeight_, bool unicodeMode_, int technology_)
{
  parent = &parent_;
  ctrlID = ctrlID_;
  location = location_;
  ...
  location = PointF::FromInts(locationw.x, locationw.y);
}
...
RectF ListBoxX::GetDesiredRect()
{
  RectF rcDesired = GetPosition();
  int rows = Length();
...
{
  // This method is no longer called in Platform
  ...
  if ((pDrawItem->itemAction == ODA_SELECT) || (pDrawItem->itemAction == ODA_DRAWENTIRE)) {
    ...
    if (pimage) {
      ...
      if (technology == SCWIN_TECH_GDI) {
        ...
        const RectF rcImage = RectF::FromInts(left, pDrawItem->rcItem.top,
...
void ListBoxX::AdjustWindowRect(RectF* rc)
{
  RECT rcw = RectFromRectF(*rc);
  ::AdjustWindowRectEx(&rcw, WS_THICKFRAME, false, WS_EX_WINDOWEDGE);
  *rc = RectF::FromInts(rcw.left, rcw.top, rcw.right, rcw.bottom);
}
...
{
  RectF rc = RectF::FromInts(0, 0, MinClientWidth(), ItemHeight());
  AdjustWindowRect(&rc);
...
{
  RectF rc = GetPosition();
  POINT ptw;
  ::GetCursorPos(&ptw);
  PointF pt = PointF::FromInts(ptw.x, ptw.y);
  pt.x += dragOffset.x;
  pt.y += dragOffset.y;
  switch (resizeHit) {
  case HTLEFT:
