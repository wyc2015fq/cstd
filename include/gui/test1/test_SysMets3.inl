...
sysmetrics [] = {
  ...
  SM_CXBORDER, TEXT("SM_CXBORDER"), TEXT("Widget border width"),
  SM_CYBORDER, TEXT("SM_CYBORDER"), TEXT("Widget border height"),
  ...
  SM_CXSIZEFRAME, TEXT("SM_CXSIZEFRAME"), TEXT("Widget sizing frame width"),
  SM_CYSIZEFRAME, TEXT("SM_CYSIZEFRAME"), TEXT("Widget sizing frame height"),
...
LRESULT CALLBACK test_SysMets3_WndProc(HWID, UINT, WPARAM, LPARAM);
int test_SysMets3()
{
  ...
  if (!RegisterWidget(szAppName, &wndclass)) {
    MessageBox(NULL, TEXT("Program requires Widgets NT!"),
        szAppName, MB_ICONERROR);
    return 0;
  }
  hWid = CreateWidget(0, szAppName, TEXT("Get System Metrics No. 3"),
...
LRESULT CALLBACK test_SysMets3_WndProc(HWID hWid, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_SIZE:
    ...
    SetScrollInfo(hWid, SB_VERT, &si, TRUE);
    ...
    SetScrollInfo(hWid, SB_HORZ, &si, TRUE);
    return 0;
  case WM_VSCROLL:
  ...
  //printf("si.nTrackPos = %d\n", si.nTrackPos);
      break;
    default:
      break;
    }
    // Set the position and then retrieve it. Due to adjustments
    // by Widgets it may not be the same as the value set.
    si.fMask = SIF_POS;
    SetScrollInfo(hWid, SB_VERT, &si, TRUE);
    GetScrollInfo(hWid, SB_VERT, &si);
    // If the position has changed, scroll the window and update it
    if (si.nPos != iVertPos) {
      ScrollWidget(hWid, 0, cyChar * (iVertPos - si.nPos), NULL, NULL);
      UpdateWidget(hWid);
    }
    return 0;
  case WM_HSCROLL:
    ...
    if (si.nPos != iHorzPos) {
      ScrollWidget(hWid, cxChar * (iHorzPos - si.nPos), 0, NULL, NULL);
      UpdateWidget(hWid);
    }
    return 0;
  case WM_KEYDOWN:
    switch (wParam) {
    case VK_HOME:
      SendMessage(hWid, WM_VSCROLL, SB_TOP, 0) ;
      break ;
    case VK_END:
      SendMessage(hWid, WM_VSCROLL, SB_BOTTOM, 0) ;
      break ;
    case VK_PRIOR:
      SendMessage(hWid, WM_VSCROLL, SB_PAGEUP, 0) ;
      break ;
    case VK_NEXT:
      SendMessage(hWid, WM_VSCROLL, SB_PAGEDOWN, 0) ;
      break ;
    case VK_UP:
      SendMessage(hWid, WM_VSCROLL, SB_LINEUP, 0) ;
      break ;
    case VK_DOWN:
      SendMessage(hWid, WM_VSCROLL, SB_LINEDOWN, 0) ;
      break ;
    case VK_LEFT:
      SendMessage(hWid, WM_HSCROLL, SB_PAGEUP, 0) ;
      break ;
    case VK_RIGHT:
      SendMessage(hWid, WM_HSCROLL, SB_PAGEDOWN, 0) ;
      break ;
    }
    return 0 ;
  case WM_MOUSEWHEEL:
    ...
    while (iAccumDelta >= iDeltaPerLine) {
      SendMessage(hWid, wParam & MK_CONTROL ? WM_HSCROLL : WM_VSCROLL, SB_LINEUP, 0) ;
      iAccumDelta -= iDeltaPerLine ;
    }
    while (iAccumDelta <= -iDeltaPerLine) {
      SendMessage(hWid, wParam & MK_CONTROL ? WM_HSCROLL : WM_VSCROLL, SB_LINEDOWN, 0) ;
      iAccumDelta += iDeltaPerLine ;
    }
    return 0 ;
  case WM_PAINT:
    if (1) {
      RECT rect = {0};
      //RECT rect2 = {0};
      hdc = BeginPaint(hWid, &ps);
      GetWidgetRect(hWid, &rect);
      set_control_clipping(hdc, &rect);
      // Get vertical scroll bar position
      //GetClientRect(hWid, &rect);
      //GetWidgetRect(hWid, &rect2);
  ...
  return DefWidgetProc(hWid, message, wParam, lParam);
}
