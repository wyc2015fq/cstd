...
int res_pack_getimg(const char* res, int reslen, img_t* im)
{
  ...
  int ret = imload_mem(bf[2].data, bf[2].len, 4, im);
  return ret;
}
...
int loadTexture(const char* picpath, texture_t* tex)
{
  img_t im[1] = {0};
  int ret = 0;
  GETGUIIO2();
...
char* res_find(strpair_t* res, int nres, const char* name)
{
  int i, len = 0;
  for (i = 0; i < nres; ++i) {
    if (0 == strcmp(name, res[i].name)) {
      return res[i].value;
    }
  }
  return NULL;
}
int res_loadTexture(strpair_t* res, int nres, const char* name, texture_t* tex)
{
  char* buf = NULL;
  int ret = 0;
  img_t im[1] = {0};
  char* base64str = res_find(res, nres, name);
