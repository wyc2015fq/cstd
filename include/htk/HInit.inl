...
static int trace31 = 0;               /* Trace level */
static ConfParam *cParm[MAXGLOBS];   /* configuration parameters */
static int nParm31 = 0;               /* total num params */
...
void SetConfParms(void)
{
   ...
   nParm31 = GetConfig("HINIT", TRUE, cParm, MAXGLOBS);
   if (nParm31>0) {
      if (GetConfInt(cParm,nParm31,"TRACE",&i)) trace31 = i;
   }
}
...
void UniformSegment(void)
{
   ...
   cov.var = NULL;      /* dummy */
   UCollectData(seqMat);
   if (trace31&T_UNI) ShowSeqMat(seqMat);

   /* Cluster Each State/Stream and Init HMM Parms */
   for (n=2; n<nStates; n++) {
      if (trace31&T_UNI) printf(" state %d ",n);
      for (s=1; s<=nStreams; s++){
         size = hset.swidth[s];
         floor = vFloor[s];
         ...
         if (hset.hsKind == TIEDHS){
            ...
            M = ste->nMix; 
         if (trace31&T_UNI) printf(" stream %d\n",s);
         seq = seqMat[n][s];
         switch (hset.hsKind){
         case PLAINHS:
         case SHAREDHS:
            ...
            if (trace31&T_UNI) ShowClusterSet(cset);
            for (m=1; m<=M; m++){
...
LogFloat ViterbiAlign(int segNum,int segLen, IntVec states, IntVec *mixes)
{
   ...
   if (trace31 & T_VIT) ShowP(1,lastP);  
   ...
   for (segIdx=2; segIdx<=segLen; segIdx++) {
      ...
      if (trace31 & T_VIT) ShowP(segIdx,lastP);   
   }
...
void EstimateModel(void)
{
   ...
   if (trace31&T_TOP) printf("Starting Estimation Process\n");
   if (newModel){
      UniformSegment();
   }
   totalP=LZERO;
   for (iter=1; !converged && iter<=maxIter; iter++){
      ...
      for (newP=0.0,i=1;i<=numSegs;i++) {
         ...
         if (trace31&T_ALN) ShowAlignment(i,segLen,states,mixes);
