...
void InitLUtil(void)
{
   ...
   nParm35 = GetConfig("LUTIL", TRUE, cParm, MAXGLOBS);
   if (nParm35>0){
      if (GetConfInt(cParm,nParm35,"TRACE",&i)) trace35 = i;
   }
...
NameId GetNameId(HashTab *htab, char *name, Boolean insert)
{
   ...
   if ((trace35&T_HASH) && htab->numAccesses%100 == 0)
