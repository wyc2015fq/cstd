...
static Boolean natReadOrder17 = FALSE;     /* Preserve natural mach read order*/
static Boolean natWriteOrder17 = FALSE;    /* Preserve natural mach write order*/
...
static ConfParam *cParm17[MAXGLOBS];      /* config parameters */
static int nParm17 = 0;

/* ScriptWord1: return next word from script */
static char * ScriptWord1(void)
{
   int ch,qch,i;
   ...
   if (ch=='\'' || ch=='"'){
      ...
      if (ch==EOF)
         HError(5051,"ScriptWord1: Closing quote missing in script file");
      wasQuoted = TRUE;
   } else {
      do {
         scriptBuf[i++] = ch; 
...
ReturnStatus SetScriptFile(char *fn)
{
   ...
   while (ScriptWord1() != NULL) ++scriptcount;
   rewind(script);
   scriptBufLoaded = FALSE;
   return(SUCCESS);
}
...
static Boolean FilterSet(IOFilter filter, char *s)
{
   ...
   if ((env = getenv(filtermap[filter])) != NULL){
      strcpy(s,env);
      return TRUE;
   } else if (GetConfStr(cParm17,nParm17,filtermap[filter],s))
      return TRUE;
   else
      return FALSE;
}
...
Boolean RawReadInt(Source *src, int *i, int n, Boolean bin, Boolean swap)
{
   ...
   if (bin){
      ...
      if (swap)
         for(p=i,j=0;j<n;p++,j++)
            SwapInt32((int32*)p);  /* Read in SUNSO unless natReadOrder17=T */
...
{
   return(RawReadShort(src,s,n,binary,(vaxOrder && !natReadOrder17)));
}

/* EXPORT->ReadInt: read n ints from src in ascii or binary */
Boolean ReadInt(Source *src, int *i, int n, Boolean binary)
{
   return(RawReadInt(src,i,n,binary,(vaxOrder && !natReadOrder17)));
}

/* EXPORT->ReadFloat: read n floats from src in ascii or binary */
Boolean ReadFloat(Source *src, float *x, int n, Boolean binary)
{
   return(RawReadFloat(src,x,n,binary,(vaxOrder && !natReadOrder17)));
}
...
void WriteShort (FILE *f, short *s, int n, Boolean binary)
{
   ...
   if (binary){
      if (vaxOrder && !natWriteOrder17){
         for(p=s,j=0;j<n;p++,j++)
            SwapShort(p);  /* Write in SUNSO unless natWriteOrder17=T */
      }
...
void WriteInt(FILE *f, int *i, int n, Boolean binary)
{
   ...
   if (binary){
      if (vaxOrder && !natWriteOrder17){
         for(p=i,j=0;j<n;p++,j++)
            SwapInt32((int32*)p);  /* Write in SUNSO unless natWriteOrder17=T */
      }
...
ReturnStatus InitShell(int argc, char *argv[], char *ver, char *sccs)
{
   ...
   nParm17 = GetConfig("HSHELL", TRUE, cParm17, MAXGLOBS);
   if (nParm17>0) {
      if (GetConfInt(cParm17,nParm17,"TRACE",&i)) trace17 = i;
      if (GetConfBool(cParm17,nParm17,"NONUMESCAPES",&b)) noNumEscapes = b;
      if (GetConfBool(cParm17,nParm17,"ABORTONERR",&b)) abortOnError = b;
      if (GetConfBool(cParm17,nParm17,"NATURALREADORDER",&b)) natReadOrder17 = b;
      if (GetConfBool(cParm17,nParm17,"NATURALWRITEORDER",&b)) natWriteOrder17 = b;
      if (GetConfBool(cParm17,nParm17,"EXTENDFILENAMES",&b)) extendedFileNames = b;
      if (GetConfInt(cParm17,nParm17,"MAXTRYOPEN",&i)) {
         maxTry = i;
...
void PrintStdOpts(char *opt)
{
   ...
   printf(" -T N    Set trace17 flags to N                 0\n");
