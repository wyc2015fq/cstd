...
static float minFrwdP1 = 10.0;            /* mix prune threshold */
static int trace5     =  1;            /* Trace level */

static Boolean PhoneMEE5 = TRUE;      /*IMPORTANT*/           /* If true and the MPE routines are called, do MPE, else MWE (word level) */
...
static Boolean ExactCorrectness=FALSE; /*IMPORTANT*/         /* Do 'exact' version of MPE/MWE, not using approximation.  This is slightly better for
                                                                e.g. Wall Street Journal and BN, you may have to tune InsCorrectness5 (e.g, -0.9), but its
...
static float InsCorrectness5 = -1;                            /* Correctness of an inserted phone.  Can be tuned, it affects recognition insertion rate.
                                                                E.g. InsCorrectness5 = -0.85 will increase insertions upon testing, relative to default = -1. */
...
static Boolean Quinphone5 = FALSE;            /* Set this TRUE if this is a quinphone model set.  The code for quinphones depends on the naming conventions 
...
static float latProbScale5 = 1.0; /*IMPORTANT*/  /* Scales the lattice-arc and lm probabilities.  Normally set to e.g. 1/12 or 1/15, the inverse
...
static float phnInsPen5 = 0.0;                 /* Insertion penalty for each phone, not subject to lm scaling.  Normally zero, but setting it to e.g.
...
static ConfParam *cParm5[MAXGLOBS];  /* config parameters */
static int nParm5 = 0;
...
#define IsSilence(name)  ( Quinphone5 ? (!strncmp(name,"sil",3)||!strncmp(name,"sp",2)) : (!strcmp(name,"sil")||!strcmp(name,"sp")))   
...
#define SET_totalProbScale hfwdbkwd_totalProbScale=langProbScale*latProbScale5*probScale
#define translm(lmlike) ((lmlike*hfwdbkwd_totalProbScale)+phnInsPen5)
/* #define DEBUG_MPE */
...
int GetNoContextPhone(LabId phone, int *nStates_quinphone/*actually,number of HMMs per phone*/, int *state_quinphone, HArc *a, int *frame_end){ 
   ...
   if(PhoneMEEUseContext){
      if(Quinphone5) HError(1, "Quinphone5 not compatible with context phones...");
      return (int) phone;
   }
   strcpy(buf,lab);
#ifdef SUPPORT_QUINPHONE
...
#ifdef SUPPORT_QUINPHONE
      if(PhoneMEEUseContext&&Quinphone5) HError(1, "Context and quinphone not compatible.");
#endif
	 ...
	 for(a=fbInfo->aInfo->start;a;a=a->foll){  /* first do all arcs for which calcArc==NULL 
            ...
            if(a->calcArc) a->mpe->correctness = 0; /* probably redundant. */
...
#ifdef SUPPORT_QUINPHONE
               if(Quinphone5 && quinphone_nstates>1 && quinphone_state > 2){ ZeroCorrectness = TRUE; }
#endif
			   ...
			   /* ref length div by overlap length */
                           tmpCorrect = proportion*(1-InsCorrectness5); /*default:InsCorrectness5=-1*/
                           tmpCorrect+=InsCorrectness5;
                          
                           if(tmpCorrect > currCorrect)  currCorrect = tmpCorrect; 
                        }
                       
                        else{
                           tmpCorrect = proportion * -InsCorrectness5;
                           tmpCorrect+= InsCorrectness5;
                           if(tmpCorrect > currCorrect)  currCorrect = tmpCorrect; 
                        }
                     }
                  ...
                  for(i=i_start;i<=i_end;i++){ /*look for all the ref. phones in this time interval.*/
                     for(cal = correctArc[i];cal;cal=cal->t){
                        ...
                        if(otherWord == (int)word)
                           /*Work out how much overlap we have with the correct phone*/
                           /* ref length div by overlap length */
                           currCorrect = MAX(currCorrect, InsCorrectness5 + proportion*(-InsCorrectness5+1));
                        else
                           currCorrect = MAX(currCorrect, InsCorrectness5 * ( 1 - proportion));
...
float GetFinalError(float curr_total, float curr_corr, Boolean hyp_is_sil){  /* re the "CALCASERROR" option, other version of approx MPE. */
  ...
  return ins*-InsCorrectness5 + del + sub;
}
...
/* ZeroAlpha1: zero alpha's of all models */
static void ZeroAlpha1(int sq, int eq)
{
   int Nq,j,q;
   DVector aq;
...
static void StepAlpha1(int t)
{
   ...
   /* not needed. */
   /*  if(t>2)
       ZeroAlpha1(fbInfo->aInfo->qLo[t-2],fbInfo->aInfo->qHi[t-2]);   / * Because the alphat vectors are swapped over each time,
   ...
   for (q = fbInfo->aInfo->qLo[t]; q <= fbInfo->aInfo->qHi[t]; q++) { /*This is just to avoid iterating over all q's.*/
      ...
      if(t >= ac->t_start && t<=ac->t_end){ /*If it's in the beam...*/
         ...
         if((outprob = ac->otprob[t]) == NULL)
            HError(2322,"StepAlpha1: Outprob NULL at time %d model %d in StepAlpha1",t,q);
         ...
         if(t==ac->t_end){ /*Work out the exit prob, just for checking purposes......  */
            ...
            if ( fabs(x - ac->locc) > 0.001 )
               HError(1, "StepAlpha1: problem with occs.. (fabs(x-locc)=%d (>0.001))",x-ac->locc); /*x is like an occupancy for that segment.*/
         }
      }
   }
}
...
/* ShStrP1: Stream Outp calculation exploiting sharing */
static float * ShStrP1(Vector v, int t, StreamElem *ste, AdaptXForm *xform, MemHeap *amem)
{
   WtAcc *wa;
   MixtureElem *me;
   MixPDF *mp;
...
/* Setotprob1: allocate and calculate otprob matrix at time t */
static void Setotprob1(int t)
{
   int q,j,Nq,s;
   float ***outprob;
   StreamElem *ste;
   ...
   if (fbInfo->hsKind == TIEDHS)
      PrecomputeTMix(fbInfo->hset,&(fbInfo->al_ot),minFrwdP1,0);
      ...
      if(t>=fbInfo->aInfo->ac[q].t_start && t<=fbInfo->aInfo->ac[q].t_end) { /* HMM is active at this time... */
         ...
         for (j=2;j<Nq;j++){
		  ...
		  break;
               ...
               case SHAREDHS:
		  if (fbInfo->S==1)
		     outprob[j][0] = ShStrP1(fbInfo->al_ot.fv[s],t+StartTime,ste,fbInfo->inXForm,fbInfo->aInfo->mem);
		  else
		     outprob[j][s] = ShStrP1(fbInfo->al_ot.fv[s],t+StartTime,ste,fbInfo->inXForm,fbInfo->aInfo->mem);
		  break;
...
/* UpTranParms1: update the transition counters of given hmm */

static void UpTranParms1(int t, int q){ 
   TrAcc *ta,*tammi=NULL;   
   Acoustic *ac = fbInfo->aInfo->ac+q;
   HLink hmm = ac->hmm;
...
/* UpMixParms1: update mu/va accs of given hmm  */
static double UpMixParms1(int q, HLink hmm, int t, DVector aqt, 
			 DVector aqt1, DVector gqt)
{
   ...
   for (j=2;j<N;j++) {
      ...
      if(aqt[j]+gqt[j]-outprob[j][0][0] < -minFrwdP1) continue; /*go to the next state.  This added for the tiedhs case,
                                                              to speed things up.*/
			...
			pMix->indx = -1;
               ...
               if (-x<minFrwdP1) {
                  ...
                  if(isnan(Lr) || Lr > 1.001)
                     HError(1, "FwdBkwdLat: UpMixParms1: invalid Lr.");
                  if (Lr>1.001)
...
/* CheckData2: check data file consistent with HMM definition */
static void CheckData2(char *fn, BufferInfo *info) 
{
   if (info->tgtVecSize!=fbInfo->hset->vecSize)
      HError(2350,"CheckData2: Vector size in %s[%d] is incompatible with hset [%d]",
             fn,info->tgtVecSize,fbInfo->hset->vecSize);
   if (!fbInfo->twoDataFiles){
      if (info->tgtPK != fbInfo->hset->pkind)
         HError(2350,"CheckData2: Parameterisation in %s is incompatible with hset ",
                fn);
   }
}
...
/* StepForward1: Step from 1 to T calc'ing Alpha columns and updating parms */
static void StepForward1()
{
   int q,t,negs;
   ...
   for (t=1;t<=fbInfo->T;t++) {
      ...
      if (fbInfo->hsKind == TIEDHS)  PrecomputeTMix(fbInfo->hset,&fbInfo->al_ot,minFrwdP1,0);

      StepAlpha1(t); /* Calculate this time's Alpha column. */
	...
	if (fbInfo->uFlags&(UPMEANS|UPVARS|UPMIXES|UPXFORM|UPMIXES))
	  if((occ=UpMixParms1(q,hmm,t,aqt,aqt1,bqt)) > LSMALL){
...
void FBLatFirstPass(FBLatInfo *_fbInfo, FileFormat dff, char * datafn, char *datafn2, Lattice *MPECorrLat){
   ...
   /*[trace5:] PrintArcInfo(stdout, &fbInfo->aInfo);*/
   ...
   if (fbInfo->twoDataFiles){
      ...
      CheckData2(datafn2,&fbInfo->up_info);
      /*      SyncBuffers(pbuf,pbuf2); */
      T2 = ObsInBuffer(fbInfo->up_pbuf);
   }else
      CheckData2(datafn,&fbInfo->al_info);
  ...
  
   {
      ...
      for(a=fbInfo->aInfo->end;a;a=a->prec){ /*Calculate betaPlus .*/
         ...
         betaPlus += ac->aclike * latProbScale5;
      ...
      for(a=fbInfo->aInfo->start;a;a=a->foll){
         ...
         alpha += ac->aclike * latProbScale5;
         a->alpha = alpha;
         occ = a->alpha + a->betaPlus - ac->aclike*latProbScale5- fbInfo->pr; 
      ...
      if(MPE){  
	   ...
	   if(CalcAsError)  SetCorrectnessAsError(fbInfo, MPECorrLat); /* first set 'correctness' field for each arc, a value between -1 and 1. */
            ...
            { 
               ...
               for(a=fbInfo->aInfo->start;a;a=a->foll){
                  Acoustic *ac = a->ac;
                  float occ = a->alpha + a->betaPlus - ac->aclike*latProbScale5 - fbInfo->pr;   /* Occupancy of this cluster of identical arcs which is due to a. */
...
void InitFBLat(void)
{
   ...
   for(iter=0;iter<=1;iter++){
      if(iter) nParm5 = GetConfig("HFBLAT", TRUE, cParm5, MAXGLOBS);
      else nParm5 = GetConfig("HFWDBKWDBLAT", TRUE, cParm5, MAXGLOBS); /* Backward compatibility */

      if (nParm5>0){
         if (GetConfInt(cParm5,nParm5,"TRACE",&i)) trace5 = i;
         if (GetConfFlt(cParm5,nParm5,"MINFORPROB",&f))  minFrwdP1 = f;
         if (GetConfFlt(cParm5,nParm5,"PROBSCALE",&f))  probScale = f;
         if (GetConfFlt(cParm5,nParm5,"LANGPROBSCALE",&f))  langProbScale = f;
         if (GetConfFlt(cParm5,nParm5,"LATPROBSCALE",&f))  latProbScale5 = f; /* this config also used in HExactMPE.c */
         if (GetConfFlt(cParm5,nParm5,"PHNINSPEN",&f))  phnInsPen5 = f; /* this config also used in HExactMPE.c */
         
         if (GetConfBool(cParm5,nParm5,"NOSILENCE",&b)) NoSilence = b;
#ifdef SUPPORT_QUINPHONE
         if (GetConfBool(cParm5,nParm5,"QUINPHONE",&b)) Quinphone5 = b;/* this config also used in HExactMPE.c */
#endif
#ifdef  SUPPORT_EXACT_CORRECTNESS
         if (GetConfBool(cParm5,nParm5,"EXACTCORRECTNESS",&b)) ExactCorrectness = b;
#endif
         if (GetConfBool(cParm5,nParm5,"PHONEMEE",&b)) PhoneMEE5=b; /* Back-compat. this config also used in HExactMPE.c */
         if (GetConfBool(cParm5,nParm5,"CALCASERROR",&b)) CalcAsError = b; 
         if (GetConfBool(cParm5,nParm5,"MWE",&b)) if(b) PhoneMEE5=FALSE; /* this config also used in HExactMPE.c */
         if (GetConfBool(cParm5,nParm5,"MEECONTEXT",&b)) PhoneMEEUseContext=b; /*will prob. want to set
                                                                               this false. */ /*back-compat*/
         if (GetConfBool(cParm5,nParm5,"USECONTEXT",&b)) PhoneMEEUseContext=b; /*will prob. want to set
                                                                               this false. */
         if (GetConfFlt(cParm5,nParm5,"INSCORRECTNESS",&f)) InsCorrectness5=-fabs(f); /* to make sure negative.*/
