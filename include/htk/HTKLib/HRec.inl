...
struct psetinfo
{
   ...
   Boolean mixShared;
   int nsp;
   PreComp1 *sPre;           /* Array[1..nsp] State PreComps */
   int nmp;
   PreComp1 *mPre;           /* Array[1..nmp] Shared mixture PreComps */
   int ntr;
...
static ConfParam *cParm16[MAXGLOBS];      /* config parameters */
static int nParm16 = 0;
...
void InitRec(void)
{
   ...
   nParm16 = GetConfig("HREC", TRUE, cParm16, MAXGLOBS);
   if (nParm16>0){
      if (GetConfInt(cParm16,nParm16,"TRACE",&i)) trace16 = i;
      if (GetConfBool(cParm16,nParm16,"FORCEOUT",&b)) forceOutput = b;
   }
}
...
#ifdef SANITY
   if (pre==NULL)
      HError(8520,"cPOutP: State has no PreComp1 attached");
#endif
...
PSetInfo *InitPSetInfo(HMMSet *hset)
{
   ...
   psi->sPre=(PreComp1*) New(&psi->heap, sizeof(PreComp1)*psi->nsp);
   psi->sPre--;
   for(i=1,pre=psi->sPre+1;i<=psi->nsp;i++,pre++) pre->id=-1;
   if (hset->numSharedMix>0) {
      psi->mixShared=TRUE;
      psi->nmp=hset->numSharedMix;
      psi->mPre=(PreComp1*) New(&psi->heap, sizeof(PreComp1)*psi->nmp);
      psi->mPre--;
