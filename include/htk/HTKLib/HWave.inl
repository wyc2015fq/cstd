...
static int trace22 = 0;

#define T_OPEN    0002     /* Report Wave Open Operations */

/* --------------------- Global Variables ------------------- */

static Boolean natReadOrder22 = FALSE;    /* Preserve natural read byte order*/
static Boolean natWriteOrder22 = FALSE;   /* Preserve natural write byte order*/
...
static ConfParam *cParm22[MAXGLOBS];       /* config parameters */
static int numParm22 = 0;
...
void InitWave(void)
{
   ...
   numParm22 = GetConfig("HWAVE", TRUE, cParm22, MAXGLOBS);
   if (numParm22>0){
      if (GetConfInt(cParm22,numParm22,"TRACE",&i)) trace22 = i;
      if (GetConfBool(cParm22,numParm22,"NATURALREADORDER",&b)) natReadOrder22 = b;
      if (GetConfBool(cParm22,numParm22,"NATURALWRITEORDER",&b)) natWriteOrder22 = b;
   }
}
...
static Wave CreateWave(MemHeap *x, FileFormat fmt)
{
   ...
   if (w->fmt == UNDEFF){
      if (GetConfStr(cParm22,numParm22,"SOURCEFORMAT",buf))
...
static Boolean MustSwap(SrcOrder so)
{
   ...
   if (GetConfStr(cParm22,numParm22,"BYTEORDER",bos)) { /* force required order */
...
static ReturnStatus LoadData2(FILE *f, Wave w, long fBytes)
{
   long bufSize;
   long nRead;

   if ( fBytes==INT_MAX || fBytes<0 ){
      HRError(6221,"LoadData2: File size appears to be infinite");
      return(FAIL);
   }
   ...
   if ((nRead=fread(w->data, 1, fBytes, f)) != fBytes  && !w->isPipe) {
      HRError(6253,"LoadData2: Cannot read data into memory");
      return(FAIL);
   }
...
static long GetALIENHeaderInfo(FILE *f, Wave w, InputAction *ia)
{
   ...
   if (!GetConfInt(cParm22,numParm22,"HEADERSIZE",&hdSize)){
...
static void DecodeIMuLaw(Wave w)
{
   ...
   mode = imSum;
   if (GetConfStr(cParm22,numParm22,"STEREOMODE",smode)){
                   ...
                   HTime frPeriod, HTime *sampPeriod)
{
   ...
   if (*sampPeriod > 0.0)  
      ...
      if (GetConfFlt(cParm22,numParm22,"SOURCERATE",&t))
         w->sampPeriod = t;
   }
   *sampPeriod = w->sampPeriod;
   ...
   if(LoadData2(f, w, fBytes)<SUCCESS){
      FClose(f,w->isPipe);  
      HRError(6213,"OpenWaveInput: LoadData2 failed");
      return(NULL);
   }
...
Wave OpenWaveOutput(MemHeap *x, HTime *sampPeriod, long bufSize)
{
   ...
   if (*sampPeriod>0.0)
      ...
      if (GetConfFlt(cParm22,numParm22,"TARGETRATE",&t))
...
ReturnStatus CloseWaveOutput(Wave w, FileFormat fmt, char *fname)
{
   ...
   if (fmt == UNDEFF){
      if (GetConfStr(cParm22,numParm22,"TARGETFORMAT",buf))
         fmt = Str2Format(buf);
      else
         fmt = HTK;
   }
