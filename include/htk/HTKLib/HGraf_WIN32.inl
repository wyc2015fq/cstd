...
static ConfParam *cParm6[MAXGLOBS];      /* config parameters */
static int nParm6 = 0;
static int trace6 = 0;                   /* Just for consistency */
...
void InitGraf(void)
{
   ...
   nParm6 = GetConfig("HGRAF", TRUE, cParm6, MAXGLOBS);
   if (nParm6>0){
      if (GetConfInt(cParm6,nParm6,"TRACE",&i)) trace6 = i;
   }
}
...
void HDrawImage(unsigned char *p, int x, int y, int width, int height)
{
   ...
   char *data = (char*)New(&gcheap,sizeof(BITMAPINFOHEADER) + 
...
void HDumpGraf(char *fname)
{
   ...
   GetDIBits(memDC,theBitmap,0,0,NULL,(BITMAPINFO*)&BitmapHeader,BI_RGB);
