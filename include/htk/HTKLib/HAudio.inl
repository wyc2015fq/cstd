...
static ConfParam *cParm23[MAXGLOBS];       /* config parameters */
static int numParm23 = 0;
...
static void StartAudi(AudioIn a)
{
...
#ifdef MMAPI_AUDIO
   {
      ...
      if ((a->mmError=waveInOpen(NULL, WAVE_MAPPER, (WAVEFORMATEX*)a->waveFmt,
                                 0,0,WAVE_FORMAT_QUERY))!=MMSYSERR_NOERROR)
         HError(6006,"StartAudi: Requested data format is not supported [ERR=%d]",a->mmError);
      if ((a->mmError=waveInOpen(&a->waveIn, WAVE_MAPPER,(WAVEFORMATEX*)a->waveFmt,
                                 (DWORD)callBackIn,a->magic,CALLBACK_FUNCTION))!=MMSYSERR_NOERROR)
...
static void InitAudo(AudioOut a, HTime *sampPeriod)
{
#ifdef MMAPI_AUDIO
   {
      ...
      if ((a->mmError=waveOutOpen(&a->waveOut, WAVE_MAPPER, 
                                  (WAVEFORMATEX*)a->waveFmt, (DWORD)callBackOut, a->magic,
...
void InitAudio(void)
{
   ...
   numParm23 = GetConfig("HAUDIO", TRUE, cParm23, MAXGLOBS);
   if (numParm23>0){
      if (GetConfInt(cParm23,numParm23,"TRACE",&i)) trace23 = i;
      if (GetConfBool(cParm23,numParm23,"LINEOUT",&b)) lineOut = b;
      if (GetConfBool(cParm23,numParm23,"PHONESOUT",&b)) phonesOut = b;
      if (GetConfBool(cParm23,numParm23,"SPEAKEROUT",&b)) speakerOut = b;
      if (GetConfBool(cParm23,numParm23,"LINEIN",&b)) lineIn = b;
      if (GetConfBool(cParm23,numParm23,"MICIN",&b)) micIn = b;
      if (GetConfInt(cParm23,numParm23,"VOLUMETYPE",&i)) volType = (VolType) i;
   }
#ifdef MMAPI_AUDIO
#define getpid()  1
...
/* ChangeState1: change state of audio input and trace23 if enabled */
static void ChangeState1(AudioIn a, AudioInStatus newState)
{
...
AudioIn OpenAudioInput(MemHeap *x, HTime *sampPeriod, HTime winDur, HTime frPeriod)
{
   ...
   if (*sampPeriod <= 0.0  && GetConfFlt(cParm23,numParm23,"SOURCERATE",&t))
      *sampPeriod = t;
...
static void StartAudioSignal(void)
{
   ...
   StartAudi(sigAudio);
   ChangeState1(sigAudio,AI_SAMPLING);
}
