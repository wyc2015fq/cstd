...
//#define LN10a 2.30258509299404568 /* Defined to save recalculating it */

static Boolean rawMITFormat = FALSE;    /* Don't use HTK quoting and escapes */


static ConfParam *cParm9[MAXGLOBS];      /* config parameters */
static int nParm9 = 0;
...
void InitLM(void)
{
   ...
   nParm9 = GetConfig("HLM", TRUE, cParm9, MAXGLOBS);
   if (nParm9>0){
      if (GetConfInt(cParm9,nParm9,"TRACE",&i)) trace9 = i;
      if (GetConfBool(cParm9,nParm9,"RAWMITFORMAT",&b)) rawMITFormat = b;
   }
}
...
/* GetInLine32: read a complete line from source */
static char *GetInLine32(char *buf)
{
   int  i, c;
...
/* SyncStr32: read input until str found */
static void SyncStr32(char *buf,char *str)
{
   while (strcmp(buf, str)!=0) {
      if (GetInLine32(buf)==NULL)
         HError(8150,"SyncStr32: EOF searching for %s", str);
   }
}
...
LogFloat LMTrans (LModel *lm, LMState src, LabId wdid, LMState *dest)
{
   ...
   ne = (NEntry*) GetNEntry (nglm, hist, FALSE);
   for ( ; !ne && (l > 0); --l) {
      hist[l] = 0;              /* back off */
      ne = (NEntry*) GetNEntry (nglm, hist, FALSE);
   }
