...
static int nParm30 = 0;

static Boolean outCMapRaw30 = FALSE;       /* Output file in raw mode */
static Boolean inCMapRaw30 = FALSE;        /* Read input files in raw mode */
...
void InitCMap(void)
{
   ...
   nParm30 = GetConfig("LCMAP", TRUE, cParm, MAXGLOBS);
   if (nParm30>0){
      if (GetConfInt(cParm,nParm30,"TRACE",&i)) trace30 = i;
      if (GetConfBool(cParm,nParm30,"INCMAPRAW",&b)) inCMapRaw30 = b; 
      if (GetConfBool(cParm,nParm30,"OUTCMAPRAW",&b)) outCMapRaw30 = b; 
      if (GetConfInt(cParm,nParm30,"UNKNOWNID",&i)) unkndx = i;
      if (GetConfStr(cParm,nParm30,"UNKNOWNNAME",buf)) strcpy(unkStr,buf);
   }
...
static void ReadMapHeader(Source *src, ClassMap *cm, int *entries)
{
   ...
   *entries = n;
   ...
   if (kind == NO_HDR) {
      ...
      cm->hdrless = TRUE;
      cm->htkEsc = !inCMapRaw30; cm->lang = NULL;
   } else { 
...
void CreateClassMap(char *fn, ClassMap *c, WordMap *w)
{
   ...
   if (fn != NULL) {    /* read header */
	 ...
	 HError(15010,"CreateClassMap: Cannot open file %s", fn);
      ...
      if (trace30&T_CML) printf("Creating empty class map\n");
   }
   if (fn != NULL) {
      LoadMapData(&src,c,n);
      if (trace30&T_CML) printf("%d classes loaded\n",c->entries);
   }
}
