...
static int trace14=0;
static ConfParam *cParm14[MAXGLOBS];      /* config parameters */
static int nParm14 = 0;
...
void InitNet(void)
{
   ...
   nParm14 = GetConfig("HNET", TRUE, cParm14, MAXGLOBS);
   if (nParm14>0){
      if (GetConfBool(cParm14,nParm14,"FORCECXTEXP",&b)) forceCxtExp = b;
      if (GetConfBool(cParm14,nParm14,"FORCELEFTBI",&b)) forceLeftBiphones = b;
      if (GetConfBool(cParm14,nParm14,"FORCERIGHTBI",&b)) forceRightBiphones = b;
      if (GetConfBool(cParm14,nParm14,"ALLOWCXTEXP",&b)) allowCxtExp = b;
      if (GetConfBool(cParm14,nParm14,"ALLOWXWRDEXP",&b)) allowXWrdExp = b;
      if (GetConfBool(cParm14,nParm14,"CFWORDBOUNDARY",&b)) cfWordBoundary = b;
      if (GetConfBool(cParm14,nParm14,"FACTORLM",&b)) factorLM = b;
      if (GetConfStr(cParm14,nParm14,"ADDSILPHONES",frcSilBuf)) frcSil=frcSilBuf;
      if (GetConfStr(cParm14,nParm14,"STARTSUBLAT",subLatStartBuf)) 
         subLatStart=subLatStartBuf;
      if (GetConfStr(cParm14,nParm14,"ENDSUBLAT",subLatEndBuf)) 
         subLatEnd=subLatEndBuf;
      if (GetConfBool(cParm14,nParm14,"REMDUPPRON",&b)) remDupPron = b;
      if (GetConfBool(cParm14,nParm14,"MARKSUBLAT",&b)) sublatmarkers = b;
      if (GetConfInt(cParm14,nParm14,"TRACE",&i)) trace14 = i;
   }
}
...
static void PrintNode1(NetNode *node,HMMSet *hset)
{
...
static void PrintChain1(Network *wnet,HMMSet *hset)
{
   NetNode *thisNode;
   
   thisNode = wnet->chain;
   while (thisNode != NULL) {
      PrintNode1(thisNode,hset);
   ...
   for (i=0,nNull=0,t=0; i < lat->nn; i++) {
      ...
      for (j=1,thisPron=thisWord->pron,npii=0; thisPron!=NULL;
         ...
         if (thisPron->nphones==0 || nAdd==0 || n==0) {
            ...
            if (thisPron->nphones==0) {
               if (thisWord!=net->nullWord && (trace14&T_CXT)) 
