...
static int trace19 = 0;

/* --------------------------- Initialisation ---------------------- */

static ConfParam *cParm19[MAXGLOBS];      /* config parameters */
static int nParm19 = 0;
static int maxIter1 = 10;               /* max num cluster iterations */
static int minClustSize = 3;           /* min num vectors in cluster */
static Boolean ldBinary1 = TRUE;        /* load/dump in binary */
...
void InitTrain(void)
{
   ...
   nParm19 = GetConfig("HTRAIN", TRUE, cParm19, MAXGLOBS);
   if (nParm19>0){
      if (GetConfInt(cParm19,nParm19,"TRACE",&i)) trace19 = i;
      if (GetConfInt(cParm19,nParm19,"MAXCLUSTITER",&i)) maxIter1 = i;
      if (GetConfInt(cParm19,nParm19,"MINCLUSTSIZE",&i)) minClustSize = i;
      if (GetConfBool(cParm19,nParm19,"BINARYACCFORMAT",&b)) ldBinary1 = b;
      if (GetConfBool(cParm19,nParm19,"STREAMPROJECTION",&b)) strmProj = b;
   }
}
                        ...
                        CovKind dck, CovKind cck, Covariance dcov)
{
   ...
   for (c=2; c<=ccs->numClust; c++) {
      ...
      for (iter=0,converged=FALSE; !converged; iter++){
         ...
         converged = (iter>=maxIter1) || ((oldCost-newCost) / oldCost < 0.001);
         FindCentres(1,curNumCl); oldCost = newCost;
      }
      if (trace19 & T_DCM) DumpClusterMap();
   }
...
/* CreateTrAcc1: create an accumulator for transition counts */
static TrAcc *CreateTrAcc1(MemHeap *x, int numStates, int nPara)
{
   TrAcc *ta;
   int count;
...
/* CreateWtAcc1: create an accumulator for mixture weights */
static WtAcc *CreateWtAcc1(MemHeap *x, int nMix, int nPara)
{
   WtAcc *wa;
   int count;
   ...
   do {
      ...
      while (GoNextState(&hss,TRUE)) {
         while (GoNextStream(&hss,TRUE)) {
            ste = hss.ste;
            ste->hook = CreateWtAcc1(x,hss.M, nPara);
      ...
      if (!IsSeenV(hmm->transP)) {
         SetHook(hmm->transP,CreateTrAcc1(x,hmm->numStates,nPara));
         TouchV(hmm->transP);       
      }
   } while (GoNextHMM(&hss));
   EndHMMScan(&hss);
...
void AttachPreComps(HMMSet *hset, MemHeap *x)
{
   ...
   do {
      ...
      while (GoNextState(&hss,TRUE)) {
         while (GoNextStream(&hss,TRUE)) {
            ste = hss.ste;
            ste->hook = CreateWtAcc1(x,hss.M, 1);
            if (hss.isCont)                     /* PLAINHS or SHAREDHS */
               while (GoNextMix(&hss,TRUE)) {
                  if (DoPreComps(hset->hsKind))
                     hss.mp->hook = CreatePreComp(x);
...
{
   WriteVector(f,wa->c,ldBinary1);
   WriteFloat(f,&(wa->occ),1,ldBinary1);
   if (!ldBinary1) fprintf(f,"\n");
}

/* DumpMuAcc: dump mean acc to file f */
static void DumpMuAcc(FILE *f, MuAcc *ma)
{
   WriteVector(f,ma->mu,ldBinary1);
   WriteFloat(f,&(ma->occ),1,ldBinary1);
   if (!ldBinary1) fprintf(f,"\n");
}
...
static void DumpVaAcc(FILE *f, VaAcc *va, CovKind ck)
{
   ...
   case DIAGC:
   case INVDIAGC:
      WriteVector(f,va->cov.var,ldBinary1);
      break;
   case FULLC:
   case LLTC:
      WriteTriMat(f,va->cov.inv,ldBinary1);
      break;
   default:
      HError(7170,"DumpVaAcc: bad cov kind");
   }
   WriteFloat(f,&(va->occ),1,ldBinary1);
   if (!ldBinary1) fprintf(f,"\n");
}
...
{
   WriteMatrix(f,ta->tran,ldBinary1);
   WriteVector(f,ta->occ,ldBinary1);
   if (!ldBinary1) fprintf(f,"\n");
}
...
FILE * DumpAccsParallel(HMMSet *hset, char *fname, int n, UPDSet uFlags, int index)
{
   ...
   do {
      ...
      WriteInt(f,(int *)&hmm->hook,1,ldBinary1); 
      while (GoNextState(&hss,TRUE)) {
         while (GoNextStream(&hss,TRUE)) {
...
static void LoadWtAcc(Source *src, WtAcc *wa, int numMixtures)
{
   ...
   ReadVector(src,cTemp,ldBinary1);
   for (m=1;m<=numMixtures;m++){
     if(!finite(cTemp[m]))
       HError(7191, "Infinite WtAcc!");
     wa->c[m] += cTemp[m];
   }
   ReadFloat(src,&f,1,ldBinary1);
   wa->occ += f;
...
static void LoadMuAcc(Source *src, MuAcc *ma, int vSize)
{
   ...
   ReadVector(src,vTemp,ldBinary1);
   for (k=1;k<=vSize;k++){
     if(!finite(vTemp[k]))
...
static void LoadVaAcc(Source *src, VaAcc *va, int vSize, CovKind ck)
{
   ...
   case LLTC:
      ...
      ReadTriMat(src,mTemp,ldBinary1);
      for (k=1;k<=vSize;k++)
         for (kk=1; kk<=k; kk++) {
...
static void LoadTrAcc(Source *src, TrAcc *ta, int numStates)
{
   ...
   ReadMatrix(src,tTemp,ldBinary1);
   for (i=1;i<=numStates;i++)
      for (j=1;j<=numStates;j++)
         ta->tran[i][j] += tTemp[i][j];
   ReadVector(src,nTemp,ldBinary1);
   for (i=1;i<=numStates;i++)
      ta->occ[i] += nTemp[i];
