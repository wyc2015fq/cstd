...
#define T_TRE   00010    /* Trace use of regression class1 tree  */
...
/* General variables */
static ConfParam *cParm0[MAXGLOBS];      /* config parameters */
static int nParm0 = 0;
static int trace0 = 0;                   /* trace0 info */
...
void InitAdapt (XFInfo *xfinfo) 
{
   ...
   nParm0 = GetConfig("HADAPT", TRUE, cParm0, MAXGLOBS);
   ...
   if (nParm0>0){
      ...
      if (GetConfInt(cParm0,nParm0,"TRACE",&i)) trace0 = i;
      if (GetConfFlt(cParm0,nParm0,"MINOCCTHRESH",&d)) minOccThresh = (float) d;
      if (GetConfBool(cParm0,nParm0,"STOREMINFO",&b)) storeMInfo = b;
      if (GetConfBool(cParm0,nParm0,"KEEPXFORMDISTINCT",&b)) keepXFormDistinct = b;
      if (GetConfBool(cParm0,nParm0,"SAVESPKRMODELS",&b)) saveSpkrModels = b;
      /* Adaptation transformation set-up */
      if (GetConfBool(cParm0,nParm0,"USEBIAS",&b)) useBias = b;
      if (GetConfFlt(cParm0,nParm0,"SPLITTHRESH",&d)) xformSplitThresh = (float) d;
      if (GetConfStr (cParm0,nParm0,"TRANSKIND",buf)) xKind = Str2XFormKind(buf);
      if (GetConfStr (cParm0,nParm0,"BLOCKSIZE",buf)) 
	 xformBlockSize = ParseConfIntVec(&infoStack,buf);
      if (GetConfStr (cParm0,nParm0,"BASECLASS",buf)) 
         xformBaseClass = CopyString(&infoStack,buf);
      if (GetConfStr (cParm0,nParm0,"REGTREE",buf)) 
         xformRegTree = CopyString(&infoStack,buf);
      if (GetConfStr (cParm0,nParm0,"ADAPTKIND",buf)) 
         xformAdaptKind = Str2AdaptKind(buf);
      if (GetConfInt(cParm0,nParm0,"MAXXFORMITER",&i)) maxXFormIter = i;
      if (GetConfBool(cParm0,nParm0,"MLLRDIAGCOV",&b)) mllrDiagCov = b;      
      if (GetConfBool(cParm0,nParm0,"SWAPXFORMS",&b)) swapXForms = b;      
      if (GetConfBool(cParm0,nParm0,"MLLRCOV2CMLLR",&b)) mllrCov2CMLLR = b; 

      if (GetConfFlt(cParm0,nParm0,"SEMITIEDFLOOR",&d)) semiTiedFloorScale = (float) d;
      if (GetConfInt(cParm0,nParm0,"MAXSEMITIEDITER",&i)) maxSemiTiedIter = i;
      if (GetConfStr (cParm0,nParm0,"SEMITIEDMACRO",buf)) 
         semiTiedMacro = CopyString(&infoStack,buf);
      if (GetConfBool(cParm0,nParm0,"SEMITIED2INPUTXFORM",&b)) semiTied2InputXForm = b; 
      if (GetConfBool(cParm0,nParm0,"SEMITIEDVFLOOR",&b)) semiTiedVFloor = b; 
      if (GetConfInt(cParm0,nParm0,"NUMNUISANCEDIM",&i)) numNuisanceDim = i;
      if (GetConfBool(cParm0,nParm0,"STATICSEMITIED",&b)) staticSemiTied = b; 
      if (GetConfBool(cParm0,nParm0,"INITNUISANCEFR",&b)) initNuisanceFR = b; 
      if (GetConfBool(cParm0,nParm0,"INITNUISANCEFRIDENT",&b)) initNuisanceFRIdent = b; 
      if (GetConfBool(cParm0,nParm0,"SAVESEMITIEDBINARY",&b)) saveSemiTiedBinary = b; 

      /* Backward compatibility with old configuration options */
      /* MLLRMEAN specification */
      if (GetConfFlt(cParm0,nParm0,"MLLRMEANSPLITTHRESH",&d)) mllrMeanSplitThresh = (float) d;
      if (GetConfStr (cParm0,nParm0,"MLLRMEANBLOCKSIZE",buf)) 
         mllrMeanBlockSize = ParseConfIntVec(&infoStack,buf);
      if (GetConfStr (cParm0,nParm0,"MLLRMEANBASECLASS",buf)) 
         mllrMeanBaseClass = CopyString(&infoStack,buf);
      if (GetConfStr (cParm0,nParm0,"MLLRMEANREGTREE",buf)) 
         mllrMeanRegTree = CopyString(&infoStack,buf);
      if (GetConfStr (cParm0,nParm0,"MLLRMEANADAPTKIND",buf)) 
         mllrMeanAdaptKind = Str2AdaptKind(buf);
      /* MLLRCOV specification */      
      if (GetConfFlt(cParm0,nParm0,"MLLRCOVSPLITTHRESH",&d)) mllrCovSplitThresh = (float) d;
      if (GetConfStr (cParm0,nParm0,"MLLRCOVBLOCKSIZE",buf)) 
         mllrCovBlockSize = ParseConfIntVec(&infoStack,buf);
      if (GetConfStr (cParm0,nParm0,"MLLRCOVBASECLASS",buf)) 
         mllrCovBaseClass = CopyString(&infoStack,buf);
      if (GetConfStr (cParm0,nParm0,"MLLRCOVREGTREE",buf)) 
         mllrCovRegTree = CopyString(&infoStack,buf);
      if (GetConfStr (cParm0,nParm0,"MLLRCOVADAPTKIND",buf)) 
         mllrCovAdaptKind = Str2AdaptKind(buf);
      /* CMLLR specification */
      if (GetConfFlt(cParm0,nParm0,"CMLLRSPLITTHRESH",&d)) cmllrSplitThresh = (float) d;
      if (GetConfStr (cParm0,nParm0,"CMLLRBLOCKSIZE",buf))
         cmllrBlockSize = ParseConfIntVec(&infoStack,buf);
      if (GetConfStr (cParm0,nParm0,"CMLLRBASECLASS",buf))
         cmllrBaseClass = CopyString(&infoStack,buf);
      if (GetConfStr (cParm0,nParm0,"CMLLRREGTREE",buf))
         cmllrRegTree = CopyString(&infoStack,buf);
      if (GetConfStr (cParm0,nParm0,"CMLLRADAPTKIND",buf))
...
static RegTree* GetRegTree(HMMSet *hset, AdaptXForm *xform)
{
   ...
   if (basename == NULL) {
      /* must have specified a regression class1 tree with TREE adapt kind */
...
static int GetVecSizeClass(BaseClass *bclass, int class1)
{
   ILink i;
   MixtureElem *me;

   /* currently does not check consistency of vector sizes */
   i=bclass->ilist[class1];
   me = (MixtureElem *)i->item;
   return VectorSize(me->mpdf->mean);
}

static IntVec GetBlockSize(AdaptXForm *xform, int class1)
{
   ...
   if (strmProj) {
      ...
      blockSize = CreateIntVec(xform->mem,1);
      blockSize[1] = GetVecSizeClass(xform->bclass,class1);
   }
   return blockSize;  
}
...
static void SetMInfo(HMMSet *hset, AdaptXForm *xform)
{
   ...
   EndHMMScan(&hss);
   if (trace0&T_ACC) printf("Attached %d MInfo structures\n",nMInfo);
   hset->attMInfo = TRUE;
}
...
static void UpdateMInfo(HMMSet *hset, AdaptXForm *xform)
{
   ...
   EndHMMScan(&hss);
   if (trace0&T_ACC) printf("Attached %d additional MInfo structures\n",nMInfo);
}
...
static void AttachXFormInfo(HMMSet *hset)
{
   ...
   EndHMMScan(&hss); 
   if (trace0&T_ACC) printf("Attached %d XFormInfo structures\n",nXFormInfo);
...
static void SetAInfo(HMMSet *hset, AdaptXForm *xform, Boolean parent)
{
   ...
   if (trace0&T_ACC) printf("Attached %d AInfo structures\n",nAInfo);
}
...
static void CreateBaseTriMat(MemHeap *x, MixPDF *mp, AdaptXForm *xform, int class1)
{
  TriMat *tm;
  int vsize = VectorSize(mp->mean);
  IntVec blockSize = GetBlockSize(xform,class1);
  RegAcc *regAcc, *ra;
  MixPDF *me;
  BaseClass *bclass;
  ILink i;
  ...
  bclass = xform->bclass;
  for (i=bclass->ilist[class1]; i!=NULL; i=i->next) { 
    if (xform->info->accBTriMat) {
...
{  
  ...
  if (trace0&T_ACC) printf("Attached %d RegAcc structures\n",nRegAcc);
			 ...
			 RegTree *rtree, IntVec classes)
{
   ...
   genXForm = FALSE;
   if (trace0&T_TRE) printf("Node %d (%f)\n",node->nodeIndex,node->nodeOcc);
...
static Boolean ParseTree(RegTree *rtree, AdaptXForm *xform)
{
   ...
   if (xform->xformSet->xkind == SEMIT) 
      HError(999,"No support for semi-tied transforms with regression class1 trees");
...
static void InitSemiTiedFR(AdaptXForm *xform, LinXForm *xf, IntVec classes, TriMat totCov)
{
   ...
   if (trace0 & T_FRS) ShowDVector("Initial",fisherRatio,size);
   for (i=1;i<=size;i++) {
      max = 0; vmax = -1;
      for (j=1;j<=size;j++) {
...
void GenXForm(RegNode *node, AdaptXForm *xform, IntVec classes)
{
   ...
   while (classes[class1] == 0) class1++;
   if (strmProj)
      accs = CreateAccStruct(&gstack,xform,xform->hset->vecSize,GetBlockSize(xform,class1));
   else 
      accs = CreateAccStruct(&gstack,xform,node->vsize,GetBlockSize(xform,class1));
...
static AdaptXForm *CreateBaseAdaptXForm(HMMSet *hset, char *xformName)
{
   ...
   if ((hset->hmmSetId != NULL) && (!MaskMatch(xform->bclass->mmfIdMask,buf,hset->hmmSetId)))
      HError(999,"HMM set ID %s is not compatible with base class1 mask %s",
...
Vector ApplyCompFXForm(MixPDF *mp, Vector svec, AdaptXForm *xform, LogFloat *det, int t)
{
  ...
  if (xform == NULL) {
    ...
    *det = 0; 
	...
	FreeVector(&gstack,vec); 
	if (trace0&T_OBC) printf("Updated ObsCache %p at time %d\n",oc,t);
      } 
      vec = oc->obs;
      *det = oc->det;
      return vec;
...
void ApplyHMMSetXForm(HMMSet *hset, AdaptXForm *xform)
{
  ...
  EndHMMScan(&hss);
  if (trace0&T_ADT) printf("Adapted %d components\n",nAdpt);
}
