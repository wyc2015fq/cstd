...
static ConfParam *cParm1[MAXGLOBS];  /* config parameters */
static int nParm1 = 0;
...
void InitArc(void)
{
   ...
   nParm1 = GetConfig("HARC", TRUE, cParm1, MAXGLOBS);
   if (nParm1>0){
      if (GetConfInt(cParm1,nParm1,"TRACE",&i)) trace1 = i;                          /*   Set TRACE=1 for minimal tracing, 3 for fuller tracing.  */
      if (GetConfFlt(cParm1,nParm1,"LMSCALE",&f)){ LMSCALE = f; IsLMScale = TRUE; } /*   Overrides lattice-specified one.  */
      if (GetConfFlt(cParm1,nParm1,"FRAMEDUR",&f)){ FRAMEDUR = f; }                 /*   Important.  Frame duration in seconds.  If != 0.01, specify it. */
      if (GetConfFlt(cParm1,nParm1,"WDPEN",&f)){ WDPEN = f; IsWdPen = TRUE; }       /*   Overrides lattice-specified one.  */
   }
}
