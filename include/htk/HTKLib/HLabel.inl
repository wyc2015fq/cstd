...
static ConfParam *cParm7[MAXGLOBS];        /* config parameters */
...
/* Hash35: return a hash value for given label name */
static unsigned Hash35(char *name)
{
   unsigned hashval;
...
void InitLabel(void)
{
   ...
   numParm = GetConfig("HLABEL", TRUE, cParm7, MAXGLOBS);
   if (numParm>0){
      if (GetConfInt(cParm7,numParm,"TRACE",&i)) trace7 = i;
      if (GetConfBool(cParm7,numParm,"STRIPTRIPHONES",&b)) 
         stripTriPhones = b;
      if (GetConfBool(cParm7,numParm,"V1COMPAT",&b))  compatMode = b;
      if (GetConfStr(cParm7,numParm,"LABELSQUOTE",str))
         labelQuote=str[0];
      if (GetConfInt(cParm7,numParm,"TRANSALT",&i)) transAlt = i;
      if (GetConfInt(cParm7,numParm,"TRANSLEV",&i)) transLev = i;
      if (GetConfFlt(cParm7,numParm,"HTKLABELTIMESCALE",&d)) htkLabelTimeScale = d;
   }
}
...
LabId GetLabId(char *name, Boolean insert)
{
   ...
   if ((trace7&T_HASH) && numAccesses%100 == 0) 
      PrintNameTabStats();
   h = Hash35(name); p = hashtab[h];
...
static void GetTrSym(Source *src, Boolean htk)
{
   ...
   default:
      if (htk) {
         UnGetCh(curch,src);
         if (!ReadString(src,trStr1)) {
            trSym=TREOF;
            break;
         }
         curch=GetCh(src);
         if (trSOL && strcmp(LEVELSEP,trStr1)==0) {
         ...
         do {
            if (nxtch>=255) break;
            trStr1[nxtch++]=curch; curch=GetCh(src);
         }
         while (!isspace(curch) && curch != ',' && curch != EOF);
         trStr1[nxtch]='\0';
         src->wasQuoted=FALSE;
      }
      if (!src->wasQuoted && IsNumeric(trStr1)){
         sscanf(trStr1,"%lf",&trNum);
         trSym = TRNUM;
         break;
      }
      if (htk && compatMode && 
          (strcmp(LEVELSEP,trStr1)==0 || strcmp(".",trStr1)==0)) {
         src->wasNewline=FALSE;
         SkipWhiteSpace(src);
         if (src->wasNewline) {
            curch = CRETURN;
            trSym=(strcmp(LEVELSEP,trStr1)==0?TRLEV:TREOF);
            break;
         }
         curch = GetCh(src);
      }
      if (stripTriPhones) TriStrip(trStr1);
      trSym = TRSTR;
      break;
   }
}
...
static void LoadSCRIBELabels(MemHeap *x, Transcription *t, Source *src)
{
   ...
   if (!GetConfFlt(cParm7,numParm,"SOURCERATE",&sp))  
   ...
   do { /* load this and all subsequent ltype labels */
      ...
      GetTrSym(src,FALSE);
      while (trSym == TRSTR){
         strcat(buf,"_"); strcat(buf,trStr1);
         GetTrSym(src,FALSE);
      }
...
Transcription *LOpen(MemHeap *x, char * fname, FileFormat fmt)
{
   ...
   if (fmt == UNDEFF){
      if (GetConfStr(cParm7,numParm,"SOURCELABEL",buf))
         fmt = Str2Format(buf);
      else
         fmt = HTK;
   }
...
ReturnStatus LSave(char *fname, Transcription *t, FileFormat fmt)
{
   ...
   if (fmt == UNDEFF){
      if (GetConfStr(cParm7,numParm,"TARGETLABEL",buf))
         fmt = Str2Format(buf);
      else
         fmt = HTK;
   }
