...
static int trace01 = 0;
#define T_TOP   0001       /* top level tracing */
#define T_SQU   0002       /* trace01 squashing */
#define T_SRT   0004       /* trace01 NG Buffer sorting */
#define T_ITR   0010       /* print NG input set tree */
#define T_MOP   0020       /* print max parallel input streams */
#define T_IST   0040       /* trace01 parallel input streaming */
#define T_FOF   0100       /* print info on FoF i/o */

/* --------------------- Global Variables ------------------- */

static ConfParam *cParm[MAXGLOBS];      /* config parameters */
static int nParm01 = 0;
...
void InitGBase(void)
{
   ...
   nParm01 = GetConfig("LGBASE", TRUE, cParm, MAXGLOBS);
   if (nParm01>0){
      if (GetConfInt(cParm,nParm01,"TRACE",&i)) trace01 = i;
      if (GetConfBool(cParm,nParm01,"NATURALREADORDER",&b)) natReadOrder = b;
      if (GetConfBool(cParm,nParm01,"NATURALWRITEORDER",&b)) natWriteOrder = b;
      if (GetConfBool(cParm,nParm01,"CHECKORDER",&b)) checkOrder = b;
   }
   /* Set byte order */
   sqOffset =  sizeof(UInt) - SQUASH;
   if (trace01&T_SQU)  printf("Squash offset is %d\n",sqOffset);
}
...
static void SortGFList(NGInputSet *inset)
{
   ...
   if (trace01&T_SRT) ShowInputState("Full sort",inset);
}
...
static void ReSortGFList(NGInputSet *inset)
{
   ...
   inset->gfsort[i] = this;
   if (trace01&T_SRT) ShowInputState("Re-sorted",inset);
}
...
static void GetInsetGram(NGInputSet *inset, NGram ng, float *wt)
{
   ...
   if (ngs->nItems == 0) { /* close this source and open all its successors */
      CloseNGramFile(ngs);
      if (trace01&T_IST) printf(" closing file %s\n", inset->gf[cur]->fn);
