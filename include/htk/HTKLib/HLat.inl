...
static int trace8=0;
static ConfParam *cParm8[MAXGLOBS];      /* config parameters */
static int nParm8 = 0;
...
void InitLat(void)
{
   ...
   nParm8 = GetConfig("HLAT", TRUE, cParm8, MAXGLOBS);
   if (nParm8>0){
      if (GetConfInt(cParm8,nParm8,"TRACE",&i)) trace8 = i;
      if (GetConfBool(cParm8,nParm8,"BEAMPRUNEARCS",&b)) 
         beamPruneArcs = b;
      if (GetConfBool(cParm8,nParm8,"COMPRESSMERGE",&b)) 
         compressMerge = b;
      if (GetConfStr(cParm8,nParm8,"LLFEXT",buf))
         llfExt = CopyString(&gstack,buf);
      if (GetConfInt(cParm8,nParm8,"MAXLLFS",&i)) maxLLFs = i;
   }
