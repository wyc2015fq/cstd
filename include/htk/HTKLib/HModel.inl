...
#define T_TOK  00004       /* Scanner Token1 Processing */
...
static ConfParam *cParm13[MAXGLOBS];      /* config parameters */
static int nParm13 = 0;
static Boolean checking   = TRUE;       /* check HMM defs */
static Boolean saveBinary1 = FALSE;      /* save HMM defs in binary */
...
void InitModel(void)
{
   ...
   InitSymNames();
   nParm13 = GetConfig("HMODEL", TRUE, cParm13, MAXGLOBS);
   if (nParm13>0){
      if (GetConfInt(cParm13,nParm13,"TRACE",&i)) trace13 = i;
      if (GetConfBool(cParm13,nParm13,"CHKHMMDEFS",&b)) checking = b;
      if (GetConfBool(cParm13,nParm13,"SAVEBINARY",&b)) saveBinary1 = b;
      if (GetConfBool(cParm13,nParm13,"KEEPDISTINCT",&b)) keepDistinct = b;
      if (GetConfBool(cParm13,nParm13,"SAVEGLOBOPTS",&b)) saveGlobOpts = b;
      if (GetConfBool(cParm13,nParm13,"SAVEREGTREE",&b)) saveRegTree = b;
      if (GetConfBool(cParm13,nParm13,"SAVEBASECLASS",&b)) saveBaseClass = b;
      if (GetConfBool(cParm13,nParm13,"SAVEINPUTXFORM",&b)) saveInputXForm = b;
      if (GetConfBool(cParm13,nParm13,"ALLOWOTHERHMMS",&b)) allowOthers = b;
      if (GetConfBool(cParm13,nParm13,"DISCRETELZERO",&b)) discreteLZero = b;
      if (GetConfStr (cParm13,nParm13,"ORPHANMACFILE",buf))
         strcpy(orphanMacFile,buf);
      if (GetConfStr (cParm13,nParm13,"HMMSETKIND",buf)) {
      ...
      if (GetConfBool(cParm13,nParm13,"REORDERCOMPS",&b)) reorderComps = b;
      if (GetConfInt(cParm13,nParm13,"PDE1BLOCKEND",&i)) pde1BlockEnd = i;
      if (GetConfInt(cParm13,nParm13,"PDE2BLOCKEND",&i)) pde2BlockEnd = i;
      if (GetConfFlt(cParm13,nParm13,"PDETHRESHOLD1",&d)) pdeTh1 = d;
      if (GetConfFlt(cParm13,nParm13,"PDETHRESHOLD2",&d)) pdeTh2 = d;
   }
}
...
typedef enum {   /* Only a character big !! */
   ...
   HMMSETID=119,
   PARMKIND=120, 
   MACRO, EOFSYM1, NULLSYM   /* Special Syms - not literals */
} Symbol;
...
typedef struct {     /* Token1 returned by the scanner */
...
ReturnStatus InitScanner(char *fname, Source *src, Token1 *tok, HMMSet *hset)
{
...
static ReturnStatus GetToken(Source *src, Token1 *tok)
{
   ...
   if (c != '<' && c != ':' && c != '~'  && c != '.' && c != '#') {
      if (c == EOF) {
         if (trace13&T_TOK) printf("HModel:   tok=<EOF>\n");
         tok->sym=EOFSYM1; return(SUCCESS);
      }
   ...
   if (c == '~'){                    /* If macro sym return immediately */
      ...
      if (trace13&T_TOK) printf("HModel:   MACRO ~%c\n",c);
      return(SUCCESS);
   }
   ...
   if (c=='#') {           /* if V1 mmf header convert to ~h */
      ...
      if (trace13&T_TOK) printf("HModel:   MACRO ~h (#!MMF!#)\n");
      return(SUCCESS);
   }
   if (c=='.'){            /* if . and not EOF convert to ~h */
      ...
      UnGetCh(c,src);
      tok->sym = MACRO; tok->macroType = 'h';
      if (trace13&T_TOK) printf("HModel:   MACRO ~h (.)\n");
      return(SUCCESS);     
   }  
   if (c=='<') {                 /* Read verbose symbol string into buf */
      ...
      for (sym=0; sym<NUMSYM; sym++) /* Look symbol up in symMap */
         if (strcmp(symMap[sym].name,buf) == 0) {
            ...
            if (trace13&T_TOK) printf("HModel:   tok=<%s>\n",buf);
   ...
   if ((tok->pkind = Str2ParmKind(buf)) != ANON){
      tok->sym = PARMKIND;
      if (trace13&T_TOK) printf("HModel:   tok=SK[%s]\n",buf);
      return(SUCCESS);
   }
...
static ReturnStatus GetOption(HMMSet *hset, Source *src, Token1 *tok, int *nState)
{
   DurKind dk;
   char buf[MAXSTRLEN];
   ...
   InputXForm* GetInputXForm(HMMSet *hset, Source *src, Token1 *tok);
...
ReturnStatus GetTiedWeights(Source *src, Token1 *tok, int M, Vector tpdf)
{
   float weight=0.0;
   short repCount=0;      /* repeat counter for weights */
   int c,m;
   
   if (trace13&T_PAR) printf("HModel: GetTiedWeights: M=%d\n",M);
   for (m=1; m<=M; m++) {
...
ReturnStatus GetDiscreteWeights(Source *src, Token1 *tok, int M, ShortVec dpdf)
{
   short weight=0;
   short repCount=0;      /* repeat counter for weights */
   int c,m;
   
   if (trace13&T_PAR) printf("HModel: GetDiscreteWeights: M=%d\n",M);
   for (m=1; m<=M; m++) {
      if (repCount>0)         /* get mixture weight */
         --repCount;
      else {
...
ReturnStatus GetTiedMixtures(HMMSet *hset, Source *src, Token1 *tok, 
                             int M, int s, Vector tpdf)
{
   ...
   if (trace13&T_PAR) printf("HModel: GetTiedMixtures\n");
...
static ReturnStatus GetOptions(HMMSet *hset, Source *src, Token1 *tok, int *nState)
{
   int p=0;
   
   *nState=0; 

   if (trace13&T_PAR) printf("HModel: GetOptions\n");
   if(GetToken(src,tok)<SUCCESS){
...
static void CompressItemList(MemHeap *x, ILink ilist, ILink *bilist)
{
  ...
  if ((ndel>0) && (trace13&T_XFD))
    printf(" CompressItemList: kept %d components, deleted %d components\n",ncomp,ndel);
}

static BaseClass* GetBaseClass1(HMMSet *hset,Source *src, Token1 *tok)
{
  BaseClass *bclass;
  char buf[MAXSTRLEN];
  ...
  if (tok->sym == MMFIDMASK) { 
    ...
    if (tok->sym == PARAMETERS) {
      if (!ReadString(src,buf)){
	HMError(src,"<PARAMETERS> symbol expected in GetBaseClass1");
	return(NULL);
      }
    } 
	...
	return(NULL);
      ...
      HMError(src,"<NUMCLASSES> symbol expected in GetBaseClass1");
      return(NULL);
    }
    if(GetToken(src,tok)<SUCCESS){
      HMError(src,"GetToken failed");
    ...
    for (i=1;i<=nbases;i++) {
      if (tok->sym != CLASS) {
	HMError(src,"<CLASS> symbol expected in GetBaseClass1");
	return(NULL);
      }
	...
	HError(999,"Error reading classes in BaseClass");
      ...
      PItemList(&ilist,&type,hset,src,(trace13&T_PAR));
    ...
    HMError(src,"<MMFIDMASK> symbol expected in GetBaseClass1");
    return(NULL);
  }
  bclass->nUse++;
  return bclass;
...
/* GetRegTree1: parse src and return the regression tree structure */
static RegTree *GetRegTree1(HMMSet *hset, Source *src, Token1 *tok)
{
   RegTree *rtree = NULL;
   RegNode *rnode, *root;
   ...
   if (trace13&T_PAR) printf("HModel: GetRegTree1\n");
   if (tok->sym == BASECLASS){
     ...
     if (tok->sym==MACRO && tok->macroType=='b') {
       if (!ReadString(src,buf))
	 HError(7013,"GetRegTree1: cannot read base class macro name");
	...
	return(NULL);
       ...
       rtree->bclass = GetBaseClass1(hset,src,tok);
	 ...
	 break;
       default:
	 HRError(7085,"GetRegTree1:Unexpected token symbol");
	 return(NULL);
       }
       if(GetToken(src,tok)<SUCCESS){
	 HMError(src,"GetToken failed");
...
static SVector GetMean(HMMSet *hset, Source *src, Token1 *tok)
{
   SVector m = NULL;
   short size;
   
   if (trace13&T_PAR) printf("HModel: GetMean\n");
   if (tok->sym==MEAN) {      
...
static SVector GetVariance(HMMSet *hset, Source *src, Token1 *tok)
{
   SVector v = NULL;
   short size; 
   
   if (trace13&T_PAR) printf("HModel: GetVariance\n");
   if (tok->sym==VARIANCE) {
      if (!ReadShort(src,&size,1,tok->binForm)){
...
static STriMat GetCovar(HMMSet *hset, Source *src, Token1 *tok)
{
   STriMat m = NULL;
   short swidth;
   
   if (trace13&T_PAR) printf("HModel: GetCovar\n");
...
static SMatrix GetTransform(HMMSet *hset, Source *src, Token1 *tok)
{
   SMatrix m = NULL;
   MemHeap *hmem;
   short xformRows,xformCols;
   
   if (trace13&T_PAR) printf("HModel: GetTransform\n");
   if (tok->sym==XFORM) {
...
static SVector GetDuration(HMMSet *hset, Source *src, Token1 *tok)
{
   SVector v = NULL;
   short size;
   
   if (trace13&T_PAR) printf("HModel: GetDuration\n");
   if (tok->sym==DURATION) {
      if (!ReadShort(src,&size,1,tok->binForm)){
...
static SVector GetSWeights(HMMSet *hset, Source *src, Token1 *tok)
{
   SVector v = NULL;
   short size;
   
   if (trace13&T_PAR) printf("HModel: GetSWeights\n");
   if (tok->sym==SWEIGHTS) {
      if (!ReadShort(src,&size,1,tok->binForm)){
...
static MixPDF *GetMixPDF(HMMSet *hset, Source *src, Token1 *tok)
{
   MixPDF *mp;
  
   if (trace13&T_PAR) printf("HModel: GetMixPDF\n");
...
static ReturnStatus GetMixture(HMMSet *hset,Source *src,Token1 *tok,int M,MixtureElem *spdf)
{
   float w = 1.0;
   short m = 1;

   if (trace13&T_PAR) printf("HModel: GetMixture\n");
   if (tok->sym == MIXTURE) {
...
static ReturnStatus GetStream(HMMSet *hset, Source *src, Token1 *tok,
...
static StateInfo *GetStateInfo(HMMSet *hset, Source *src, Token1 *tok)
{
   StateInfo *si;
   int i,S;
   short nMix[SMAX];

   if (trace13&T_PAR) printf("HModel: GetStateInfo\n");
   S = hset->swidth[0];
...
static SMatrix GetTransMat(HMMSet *hset, Source *src, Token1 *tok)
{
   SMatrix m;
   int i,j;
   short size;
   ...
   if (trace13&T_PAR) printf("HModel: GetTransMat\n");
   if (tok->sym == TRANSP) {
      if (!ReadShort(src,&size,1,tok->binForm)){
...
static ReturnStatus GetHMMDef(HMMSet *hset, Source *src, Token1 *tok,
                              HLink hmm, int nState)
{
   ...
   if (trace13&T_PAR) printf("HModel: GetHMMDef\n");
   if (tok->sym != BEGINHMM){
...
static SVector GetBias(HMMSet *hset, Source *src, Token1 *tok)
{
   SVector m = NULL;
   MemHeap *hmem;
   short size;
   
   if (trace13&T_PAR) printf("HModel: GetBias\n");
   if (tok->sym==BIAS) {      
      if (hset==NULL) hmem = &xformStack;
      else hmem = hset->hmem;
      if (!ReadShort(src,&size,1,tok->binForm)){
...
static LinXForm* GetLinXForm(HMMSet *hset, Source *src, Token1 *tok)
{
  LinXForm *xf;
  MemHeap *hmem;
  int i,b;
  ...
  if (trace13&T_PAR) printf("HModel: GetLinXForm\n");
  if (tok->sym == VECSIZE) {
    if (hset==NULL) hmem = &xformStack;
    else hmem = hset->hmem;
...
static XFormSet* GetXFormSet(HMMSet *hset, Source *src, Token1 *tok)
{
  char buf[MAXSTRLEN];
  int i,b;
  XFormSet *xformSet;

  if (trace13&T_PAR) printf("HModel: GetXFormSet\n");
...
InputXForm* GetInputXForm(HMMSet *hset, Source *src, Token1 *tok)
{
  ...
  if (trace13&T_PAR) printf("HModel: GetXForm\n");
  if (tok->sym == MMFIDMASK) { 
    if (hset==NULL) hmem = &xformStack;
    else hmem = hset->hmem;
...
static AdaptXForm* GetAdaptXForm(HMMSet *hset, Source *src, Token1 *tok)
{
  char buf[MAXSTRLEN];
  AdaptXForm *xform;
  LabId id;
  ...
  if (trace13&T_PAR) printf("HModel: GetAdaptXForm\n");
  ...
  if (tok->sym == ADAPTKIND) {
	...
	return(NULL);
      ...
      xform->bclass = GetBaseClass1(hset,src,tok);
...
/* Hash1: return a hash value for given name */
static unsigned Hash1(char *name)
{
   unsigned hashval;
...
MLink FindMacroStruct(HMMSet *hset, char type, Ptr structure)
{
   ...
   if (hset->pmap == NULL){   /* 1st use so create pmap hash table */
      ...
      if (trace13&T_PMP) printf("HModel: creating pointer map hash table\n");
      ...
      if (trace13&T_PMP) printf("HModel: %d pointers hashed\n",n);
   }
   ...
   while (tok.sym != EOFSYM1){
      ...
      if (type == 'o') {    /* load an options macro */
	   ...
	   if ((type != 'j') && (CheckOptions(hset)<SUCCESS)) {
	      ...
	      break;
            case 'r': 
	      structure = GetRegTree1(hset,&src,&tok);break;
	    case 'b': 
	      structure = GetBaseClass1(hset,&src,&tok); break;
...
static ReturnStatus GetXFormMacros(HMMSet *hset, Source *src, Token1 *tok, int fidx)
{
  char type, buf[MAXSTRLEN];
  LabId id;
  Ptr structure;
			      ...
			      Source *src, Token1 *tok)
{
  ...
  if ((fname==NULL) || ((f=FOpen(fname,NoFilter,&isPipe)) == NULL)) {
    ...
    if (trace13&T_TOP) printf("Loading macro file %s\n",buf);
    return buf;
  } else {
    FClose(f,isPipe);
    if (trace13&T_TOP) printf("Loading macro file %s\n",fname);
...
BaseClass *LoadBaseClass(HMMSet *hset, char* macroname, char *fname)
{
  ...
  if ((id == NULL) || ((m = FindMacroName(hset,'b',id))==NULL))  { 
    ...
    if ((id==NULL)||((m = FindMacroName(hset,'b',id))==NULL)) { /* not stored in macro format */
      ...
      structure = bclass = GetBaseClass1(hset,&src,&tok);
...
RegTree *LoadRegTree(HMMSet *hset, char* macroname, char *fname)
{
  ...
  if ((id == NULL) || ((m = FindMacroName(hset,'r',id))==NULL)) { 
    ...
    if ((id==NULL) || ((m = FindMacroName(hset,'r',id))==NULL)) { /* not stored in macro format */
      id = GetLabId(macroname,TRUE);
      structure = regTree = GetRegTree1(hset,&src,&tok);
      ...
      for (h=0; h<MACHASHSIZE; h++)
         for (m=hset->mtab[h]; m!=NULL; m=m->next)
            if (m->fidx==0){
               ...
               if (trace13&T_ORP && m->type != 'l' && m->type != '*')
