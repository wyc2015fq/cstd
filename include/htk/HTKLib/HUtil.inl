...
static ConfParam *cParm20[MAXGLOBS];      /* config parameters */
static int nParm20 = 0;
...
void InitUtil(void)
{
   ...
   nParm20 = GetConfig("HUTIL", TRUE, cParm20, MAXGLOBS);
   if (nParm20>0){
      if (GetConfInt(cParm20,nParm20,"TRACE",&i)) trace20 = i;
   }
...
static Source *source1;         /* Current source1 for item list */
static int ch1;                 /* Current character from source1 */
...
static char *GetAlpha1(char *s)
{
   ...
   wasQuoted=FALSE;
   SkipSpaces();
   if (ch1 == DBL_QUOTE || ch1 == SING_QUOTE){
      wasQuoted = TRUE; q = ch1;
      ReadCh();
   }
   for (i=0; i<MAXSTRLEN ; i++){
      if (wasQuoted){
         ...
         if (isspace(ch1) || strchr(term,ch1)){
            s[i] = '\0';
            return s;
         }
      }
      if (ch1==ESCAPE_CHAR) {
         ...
         if (ch1<'0' || ch1>'7') {
            n = ch1 - '0'; ReadCh();
            if (ch1<'0' || ch1>'7') EdError("Octal digit expected");
            n = n*8 + ch1 - '0'; ReadCh();
            if (ch1<'0' || ch1>'7') EdError("Octal digit expected");
            ch1 += n*8 - '0';
         }
...
/* GetInt1: read integer coerced given range */
static int GetInt1(int lo, int hi)
{
   char buf[20];
   int i = 0, num;
...
static void PIntRange(IntSet s)
{
   ...
   i1 = GetInt1(1,s.nMembers);
   SkipSpaces();
   if (ch1 == '-') {
      ReadCh();
      i2 = GetInt1(1,s.nMembers);
   }
   else
      i2=i1;
   for (i=i1; i<=i2; i++)
