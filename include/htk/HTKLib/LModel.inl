...
static Boolean natReadOrder32 = FALSE;    /* Preserve natural read byte order */
static Boolean natWriteOrder32 = FALSE;   /* Preserve natural write byte order */
...
void InitLModel(void)
{
   ...
   nParm32 = GetConfig("LMODEL", TRUE, cParm, MAXGLOBS);
   if (nParm32>0){
      if (GetConfInt(cParm,nParm32,"TRACE",&i)) trace32 = i;
#ifdef HTK_TRANSCRIBER
      if (trace32&T_PROB) trace32=trace32^T_PROB;
#endif
      if (GetConfBool(cParm,nParm32,"RAWMITFORMAT",&b)) htkEsc = !b;
      if (GetConfBool(cParm,nParm32,"USEINTID",&b)) defIntID = b;
      if (GetConfBool(cParm,nParm32,"NATURALREADORDER",&b)) natReadOrder32 = b;
      if (GetConfBool(cParm,nParm32,"NATURALWRITEORDER",&b)) natWriteOrder32 = b;
   }
#ifdef ULTRA_LM
...
static void LoadUltraNGrams(Source *src, BackOffLM *lm)
{
   ...
   Boolean mustSwap = (vaxOrder && !natReadOrder32);
		       ...
		       FLEntry **feTab, int *fetCount)
{
   ...
   LMProbType ptype;
   FLEntry *fe,*tgtFE;
   Boolean mustSwap = (vaxOrder && !natWriteOrder32);
			 ...
			 LMProbType tgtPType, MemHeap *heap)
{
   ...
   if (isUltra) {                        /* ultra file format */
	 ...
	 HError(15450,"LoadLangModel: Unable to find KEY (%s)",lnBuf);
      ultraKey[KEY_LENGTH-1] = (vaxOrder && natReadOrder32) ? 1 : 0;
...
/* WriteNGram32: recursive write routine */
static int WriteNGram32(FILE *f, BackOffLM *lm, FLEntry **feStack,
		      int g, int nSize, Boolean intId)
{
   ...
   if (g < nSize) {
	 ...
	 feStack[g] = fe;
	 nItem += WriteNGram32(f,lm,feStack,g+1,nSize,intId);
      }
   } else {
      gi = lm->gInfo+nSize;
      if (gi->fmt==LMF_BINARY || gi->fmt==LMF_TEXT)
	 ...
	 if ((ndx < 1) || (ndx > lm->vocSize))
	    HError(15490,"WriteNGram32: Component %d of %d-gram, FE index (%d)",
		   j,nSize,ndx);
	 ...
	 if ((se->ndx < 1) || (se->ndx > lm->vocSize)) {
	    HError(15490,"WriteNGram32: Invalid SE index (%d)",se->ndx);
	 }
	 switch (ptype) {
	    case LMP_FLOAT :
...
static int SaveNGram(FILE *f, int G, BackOffLM *lm)
{
   ...
   fprintf(f, "\n\\%d-grams:\n", G);
   feStack[0] = &(lm->root);
   total = WriteNGram32(f,lm,feStack,1,G,useIntID);
...
void SaveLangModel(char *lmFn, BackOffLM *lm)
{
   ...
   if (isUltra) {
#ifdef ULTRA_LM
      ultraKey[KEY_LENGTH-1] = (vaxOrder && natWriteOrder32) ? 1 : 0;
      fprintf(f,"KEY: ");
   ...
   */
   ...
   if (nSize==1) {  /* lookup unigram separately */
	 ...
	 if (trace32&T_PROB)
	    printf("replaced, ");
	 acs->nmiss++;
	 if ((trace32&T_TOP) &&  (fe != NULL) && (fe->nse == 0)) {
