...
void InitWMap(void)
{
   ...
   nParm36 = GetConfig("LWMAP", TRUE, cParm, MAXGLOBS);
   if (nParm36>0){
      if (GetConfInt(cParm,nParm36,"TRACE",&i)) trace36 = i;
      if (GetConfBool(cParm,nParm36,"INWMAPRAW",&b)) inWMapRaw = b; 
      if (GetConfBool(cParm,nParm36,"OUTWMAPRAW",&b)) outWMapRaw = b; 
   }
}
...
/* ReadMapHeader36: read header and store info in wm */
static void ReadMapHeader36(Source *src, WordMap *wm)
{
   LMFileHdr hdr;
   LMHdrKind kind;
   int n;
   ...
   if (n<0 || n > 9999000)
      HError(15151,"ReadMapHeader36: Unlikely num map entries[%d] in %s",n,src->name); 
   wm->used = n;
   ...
   if (kind == NO_HDR) {
      ...
      if (wm->name == NULL)
         HError(15152,"ReadMapHeader36: No NAME header in %s",src->name);      
      if (!GetLMHdrInt("SEQNO",&n,hdr))
         HError(15153,"ReadMapHeader36: No SEQNO header in %s",src->name);
      wm->seqno = n;
      wm->htkEsc = TRUE;
      if ((s=GetLMHdrStr("ESCMODE",hdr,TRUE)) != NULL) {
         if (strcmp(s,"HTK")==0) wm->htkEsc = TRUE; else
         if (strcmp(s,"RAW")==0) wm->htkEsc = FALSE; else
            HError(15155,"ReadMapHeader36: Unknown escmode %s in %s",s,src->name);
      }
      ...
      if ((s=GetLMHdrStr("FIELDS",hdr,TRUE)) != NULL) {
         ...
         if (strcmp(s,"ID,WFC")==0 || strcmp(s,"WFC,ID")==0) 
            ...
            HError(15150,"ReadMapHeader36: Unknown fields tag %s in %s",s,src->name);
      }
   }
}
...
/* LoadMapData36: load contents of src into wordmap */
static void LoadMapData36(Source *src, WordMap *wm)
{
   char buf[256],*fn;
   MapEntry *me;
   int ibuf[2],i,n,ndx;
   ...
   for (me=wm->me,i=0; i<wm->used; i++,me++){
      if (!GetSrcString(src,buf,wm->htkEsc))
         HError(15113,"LoadMapData36: Cannot read word %d from %s",i+1,fn);
      id = GetLabId(buf,TRUE);
      if (wm->isMap){
         if (id->aux != NULL)
            HError(15155,"LoadMapData36: Word %s is duplicated in %s",buf,fn); 
         n = (wm->hasCnts) ? 2 : 1; ibuf[1] = 0;
         if (!ReadInt(src,ibuf,n,FALSE))
            HError(15113,"LoadMapData36: Cannot read index/count of %s in %s",buf,fn);
...
static void CreateListorMap(char *fn, WordMap *wm, int freeSlots, Boolean isMap)
{
   ...
   if (fn != NULL) {    /* read header */
      ...
      wm->isMap = isMap;
      if (trace36&T_WMP) printf("Creating empty map/list\n");
   }
   /* Create Data Storage */
   ...
   if (fn != NULL) {
      LoadMapData36(&src,wm);
      if (trace36&T_WMP) printf("%d words loaded\n",wm->used);
   }
   wm->wlt = wm->clt = NULL;
...
/* WriteMapHeader36: write HLM header to f */
static void WriteMapHeader36(FILE *f, WordMap *w)
{
   fprintf(f,"Name  = %s\n",w->name);
   fprintf(f,"SeqNo = %d\n",w->seqno);
   fprintf(f,"Entries = %d\n",w->used);
...
static void WriteWordMap(FILE *f, WordMap *w, Boolean noHeader, Boolean debug)
{
   ...
   if (!noHeader) WriteMapHeader36(f,w);
   for (i=0; i<w->used; i++){
      id = w->id[i];
