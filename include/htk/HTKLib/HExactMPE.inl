...
static ConfParam *cParm3[MAXGLOBS];  /* config parameters */
static int nParm3 = 0;
...
void InitExactMPE(void)
{
   ...
   nParm3 = GetConfig("HEXACTMPE", TRUE, cParm3, MAXGLOBS);
   if (nParm3>0){
      if (GetConfInt(cParm3,nParm3,"PHONEBEAM",&i)) PHONE_BEAM = i;
      if (GetConfFlt(cParm3,nParm3,"EXACTCORRPRUNE",&f)) EXACTCORR_PRUNE = f; /*default: log(0.0002) = -8.5 */
   }
   nParm3 = GetConfig("HFBLAT", TRUE, cParm3, MAXGLOBS); 
   if (nParm3>0){
      if (GetConfFlt(cParm3,nParm3,"LATPROBSCALE",&f))  latProbScale = f;/* repeat of config also used in HFBLat.c */
      if (GetConfFlt(cParm3,nParm3,"PHNINSPEN",&f))  phnInsPen = f;     /* repeat of config also used in HFBLat.c */
#ifdef SUPPORT_QUINPHONE
      if (GetConfBool(cParm3,nParm3,"QUINPHONE",&b)) Quinphone = b;  /* repeat of config also used in HFBLat.c */
#endif
      if (GetConfBool(cParm3,nParm3,"PHONEMEE",&b)) PhoneMEE=b;     /* repeat of config also used in HFBLat.c */
      if (GetConfBool(cParm3,nParm3,"MWE",&b)) if(b) PhoneMEE=FALSE; /* this config also used in HFBLat.c */
      if (GetConfFlt(cParm3,nParm3,"INSCORRECTNESS",&f)) InsCorrectness=-fabs(f); /* to make sure negative.*/
      if (GetConfFlt(cParm3,nParm3,"SUBCORRECTNESS",&f)) SubCorrectness=-fabs(f); /* to make sure negative.*/
   }
