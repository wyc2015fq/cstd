...
static int trace33 = 0;

/* ---------------- Configuration Parameters --------------------- */

static ConfParam *cParm[MAXGLOBS];
static int nParm33 = 0;            /* total num params */
...
void InitPCalc(void)
{
   ...
   nParm33 = GetConfig("LPCALC", TRUE, cParm, MAXGLOBS);
   if (nParm33>0){
#ifndef HTK_TRANSCRIBER
      if (GetConfInt(cParm,nParm33, "TRACE",&i))    trace33 = i;
#endif
      if (GetConfStr(cParm,nParm33, "STARTWORD",s)) strcpy(sstStr,s);
   }
...
void InitBuildInfo(BuildInfo *bi)
{
   ...
   bi->wmap = NULL;
   bi->inSet = NULL;
   if (GetConfInt(cParm,nParm33, "UNIFLOOR",&i)) bi->uniFloor = i;
   if (GetConfInt(cParm,nParm33, "KRANGE",&i))   bi->kRange = i;
   if (GetConfStr(cParm,nParm33, "DCTYPE",s)) {
     if (!strcmp(s,"TG")) 
       bi->dctype = DC_KATZ;
   ...
   for (i=2; i<=LM_NSIZE; i++) {
      ...
      if (GetConfInt(cParm,nParm33,s,&j)) bi->cutOff[i] = j;
   }
}
...
static void CalcTGCoefs(MemHeap *heap, BackOffInfo *boi, int nSize, FoFTab *ftab) 
{
   ...
   for (r=1; r<=K; r++) {
      if (trace33&T_TOP) printf("coef[%d]=%e",r,tgi->coef[r]);
      if (tgi->coef[r]>1.0) {
	 tgi->coef[r] = DEF_TG_COEF; 
	 if (trace33&T_TOP) printf(", clamped to %.4f",tgi->coef[r]);
      }
      if (trace33&T_TOP) printf("\n");
   }
   tgi->kRange = K;
}
