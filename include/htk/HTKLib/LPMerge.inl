...
static int trace34 = 0;
#define T_TOP  0001     /* Top Level tracing */

/* ---------------- Configuration Parameters --------------------- */

static ConfParam *cParm[MAXGLOBS];
static int nParm34 = 0;            /* total num params */
...
void InitPMerge(void)
{
   ...
   nParm34 = GetConfig("LPMERGE", TRUE, cParm, MAXGLOBS);
   if (nParm34>0){
      if (GetConfInt(cParm,nParm34, "TRACE",&i))    trace34 = i;
   }
}
...
/* InitTargetModel34: initialise target model structure */
static BackOffLM *InitTargetModel34(MemHeap *heap, int nSize, WordMap *wList)
{
   int i,N;
   FLEntry *fe;
   SMEntry *se;
		       ...
		       int nSize, WordMap *wList)
{
   ...
   mi->nLModel = nLModel;
   mi->wList = wList;
   mi->lm = tgtLM = InitTargetModel34(heap,nSize,wList);
