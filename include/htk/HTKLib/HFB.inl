...
#define T_OPT   0002    /* Option trace4 */
...
static ConfParam *cParm4[MAXGLOBS];      /* config parameters */
static int nParm4 = 0;
...
void InitFB(void)
{
   ...
   for (m = 0; m < 2; ++m) {
      nParm4 = GetConfig(m==0 ? "HFWDBKWD" : "HFB", TRUE, cParm4, MAXGLOBS);
      if (nParm4>0){
         if (GetConfInt(cParm4,nParm4,"TRACE",&i)) trace4 = i;
         if (GetConfInt(cParm4,nParm4,"HSKIPSTART",&i)) skipstartInit = i;
         if (GetConfInt(cParm4,nParm4,"HSKIPEND",&i)) skipendInit = i;
         if (GetConfFlt(cParm4,nParm4,"PRUNEINIT", &d)) pruneSetting.pruneInit = d;
         if (GetConfFlt(cParm4,nParm4,"PRUNEINC", &d)) pruneSetting.pruneInc = d;
         if (GetConfFlt(cParm4,nParm4,"PRUNELIM", &d)) pruneSetting.pruneLim = d;
         if (GetConfFlt(cParm4,nParm4,"MINFORPROB", &d)) pruneSetting.minFrwdP = d;
         if (GetConfBool(cParm4,nParm4,"ALIGNCOMPLEVEL",&b)) alCompLevel = b;
         if (GetConfBool(cParm4,nParm4,"PDE",&b)) pde = b;
      }
   }
}
                      ...
                      int Q, int skipstart, int skipend)
{
   ...
   ZeroAlpha(ab,eq+1,Q);
   if (trace4&T_PRU && p->pruneThresh < NOPRUNE)
                      ...
                      int Q, int T, LogDouble pr, int skipstart, int skipend)
{
   ...
   if (trace4&T_PRU && NonSkipRegion(skipstart,skipend,t)){
...
/* NewOtprobVec1: create prob vector size [0..M] */
static float * NewOtprobVec1(MemHeap *x, int M)
{
   float *v;
   int MM,m;
		       ...
		       AdaptXForm *xform, MemHeap *abmem)
{
   ...
   if (wa->time==t)           /* seen this state before */
      ...
      M = ste->nMix;
      outprobjs = NewOtprobVec1(abmem,M);
      me = ste->spdf.cpdf+1;
                      ...
                      Observation ot, int t, int S, int qHi, int qLo)
{
   ...
   if (trace4&T_OUT && NonSkipRegion(skipstart,skipend,t)) 
      ...
      if (otprob[t][q] == NULL)
         {
            ...
            for (j=2;j<Nq;j++){
               ...
               for (s=1;s<=S;s++,ste++){
                  ...
                  case DISCRETEHS:
                     if (S==1) {
                        outprobj[0] = NewOtprobVec1(&ab->abMem,1);
                        outprobj[0][0] = SOutP(hset,s,&ot,ste);
                     } else {
                        outprobj[s] = NewOtprobVec1(&ab->abMem,1);
...
/* CheckData1: check data file consistent with HMM definition */
static void CheckData1(HMMSet *hset, char *fn, BufferInfo *info, 
                      Boolean twoDataFiles) 
{
   if (info->tgtVecSize!=hset->vecSize)
      HError(7350,"CheckData1: Vector size in %s[%d] is incompatible with hset [%d]",
             fn,info->tgtVecSize,hset->vecSize);
   if (!twoDataFiles){
      if (info->tgtPK != hset->pkind)
         HError(7350,"CheckData1: Parameterisation in %s is incompatible with hset",
                fn);
   }
}
                        ...
                        LogDouble pr)
{
   ...
   if (trace4&T_TRA && NonSkipRegion(fbInfo->skipstart,fbInfo->skipend,t)) {
                       ...
                       Boolean twoDataFiles, LogDouble pr)
{
   ...
   if (trace4&T_MIX && fbInfo->uFlags&UPMIXES && 
   ...
   for (j=2;j<N;j++) {
      ...
      for (s=1;s<=S;s++,ste++){
		   ...
		   otvs = ApplyCompFXForm(mp,ot.fv[s],fbInfo->paXForm,&det,t);
                  ...
                  */ 
                  ...
                  if (fbInfo->uFlags&UPMIXES) {
                     wa->c[m] +=Lr;
                     if (trace4&T_MIX && NonSkipRegion(fbInfo->skipstart,fbInfo->skipend,t))
         ...
         wa->occ += steSumLr;
         if (trace4&T_MIX && mmix && fbInfo->uFlags&UPMIXES && 
             NonSkipRegion(fbInfo->skipstart,fbInfo->skipend,t))
            printf("[%7.2f]\n",wa->occ);
      }
   }
...
static void StepForward(FBInfo *fbInfo, UttInfo *utt)
{
   ...
   for (t=1;t<=utt->T;t++) {
      ...
      if (trace4&T_ALF && NonSkipRegion(fbInfo->skipstart,fbInfo->skipend,t)) 
      ...
      if (trace4&T_OCC && NonSkipRegion(fbInfo->skipstart,fbInfo->skipend,t)) 
         TraceOcc(ab,utt,t);
   }
}

/* load the labels into the UttInfo structure from file */
void LoadLabs1(UttInfo *utt, FileFormat lff, char * datafn, 
...
void LoadData1(HMMSet *hset, UttInfo *utt, FileFormat dff, 
              char * datafn, char * datafn2)
{
   ...
   if (utt->twoDataFiles){
      ...
      CheckData1(hset,datafn2,&info2,utt->twoDataFiles);
      T2 = ObsInBuffer(utt->pbuf2);
   }else
      CheckData1(hset,datafn,&info,utt->twoDataFiles);
