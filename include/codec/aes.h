...
public:
  ...
  InvCipher(unsigned char* input, unsigned char* output); // decipher 16-bit input
private:
  ...
  SetNbNkNr(int keySize);
  AddRoundKey(int round); //ÂÖÃÜÔ¿¼Ó
  SubBytes(); //SºÐ×Ö½Ú´ú»»
  InvSubBytes(); //ÄæSºÐ×Ö½Ú´ú»»
  ShiftRows(); //ÐÐÒÆÎ»
  InvShiftRows();
  MixColumns(); //ÁÐ»ìÏý
  InvMixColumns();
