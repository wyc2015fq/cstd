...
// 该文档可从http://www.ietf.org/rfc/rfc1321.txt 获得。MD5的用途主要有：
...
static int bytes_tostring(const unsigned char* b, int blen, char* str, int slen) {
  int i, j=0;
  const char* S_HEX = "0123456789abcdef";
  ASSERT(slen>2*blen);
  for(i = 0; i < blen; i++, j+=2) {
    str[j+0] = S_HEX[(b[i] >> 4) & 0x0f];
    str[j+1] = S_HEX[b[i] & 0x0f];
	}
  str[j] = 0;
  return j;
}
static int md5_string(const void* data, int datalen, char* str, int slen) {
  md5_state_t md5[1] = {0};
  uchar md5sum[20] = {0};
  md5_init(md5);
  md5_append(md5, (const unsigned char *)data, datalen);
  md5_finish(md5, md5sum);
  return bytes_tostring(md5sum, 16, str, slen);
}
#include <stdio.h>
#include <string.h>
static int test_md5()
