                                   ...
                                   char **significance_map)
{
  ...
  if (set->type == QCCSPECK_SET_S)
  ...
  else
    {
      for (row = 0; row < set->num_rows; row++)
        for (col = 0; col < set->num_cols; col++)
          if (((row >= set->origin_row) || (col >= set->origin_col)) &&
              (significance_map[row][col] ==
               QCCSPECK_SIGNIFICANT))
            {
              set->significance = QCCSPECK_SIGNIFICANT;
              return(0);
  ...
  if (buffer->type == QCCBITBUFFER_OUTPUT)
    {
      if (model->current_context != QCCSPECK_CONTEXT_NOCODE)
        {
          if (QccSPECKSetSignificance(current_set,
                                      significance_map))
            {
  ...
  while (current_subset_node != NULL)
    {
      ...
      if (prior_significance == QCCSPECK_INSIGNIFICANT)
        ...
        if (prior_significance == QCCSPECK_EMPTYSET)
          context_offset = 1;
        else
          context_offset = 2;
      ...
      switch (subset_cnt)
        ...
        case 1:
          model->current_context =
            QCCSPECK_CONTEXT_SIGNIFICANCE_S2 + context_offset;
          break;
        case 2:
          model->current_context =
            QCCSPECK_CONTEXT_SIGNIFICANCE_S1 + context_offset;
          break;
        case 3:
          if (!significance_cnt)
            ...
            model->current_context =
              QCCSPECK_CONTEXT_SIGNIFICANCE_S0 + context_offset;
          break;
        }
        
      return_value =
        QccSPECKProcessS(current_subset_node,
  ...
  if (current_set->significance != QCCSPECK_INSIGNIFICANT)
    {
      ...
      if (QccSPECKSetIsPixel(current_set))
        {
          current_set->significance = QCCSPECK_NEWLY_SIGNIFICANT;
          return_value =
            QccSPECKInputOutputSign(&sign_array
        ...
        {
          return_value =
            QccSPECKCodeS(current_set_node,
                          current_list_node,
                          LSP,
                          garbage,
                          coefficients,
      ...
      if (!QccSPECKSetTransparent(&set_S, mask))
        {
          ...
          switch (subband)
            ...
            case 2:
              model->current_context =
                QCCSPECK_CONTEXT_SIGNIFICANCE_S2 +
                (prior_significance != QCCSPECK_INSIGNIFICANT);
              break;
            case 1:
              model->current_context =
                QCCSPECK_CONTEXT_SIGNIFICANCE_S1 +
                (prior_significance != QCCSPECK_INSIGNIFICANT);
              break;
            }
                   ...
                   QccBitBuffer *output_buffer)
{
  ...
  if ((model = QccENTArithmeticEncodeStart(QccSPECKNumSymbols,
