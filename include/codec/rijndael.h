...
public:
  ...
  bool MakeKey(const char* strkey, char const* chain, int keylength = DEFAULT_BLOCK_SIZE, int blockSize = DEFAULT_BLOCK_SIZE);
private:
#define throw_exception(x) return false;
  //Auxiliary Function
  bool Xor(char* buff, char const* chain) {
    if (false == m_bKeyInit) {
      throw_exception(sm_szErrorMsg1);
    }
