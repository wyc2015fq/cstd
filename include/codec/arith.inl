...
int encode(const void* src, int slen)
{
  ...
  start_encoding();
  for (; i < slen; ++i) { /* Loop through characters. */
    int ch;
    int symbol;
    ch = usrc[i]; /* Read the next character. */
...
int decode(void* dst, int maxlen)
{
  int i = 0;
  ...
  for (;;) {
    ...
    udst[i++] = (unsigned char)index_to_char[symbol]; /* Translate to a character.*/
  }
  return i;
}
