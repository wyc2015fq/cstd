...
COverLappedSocket::~COverLappedSocket()
{
  if (m_RecvIOOper.mOperBuf) {
    VirtualFree(m_RecvIOOper.mOperBuf, DATA_BUF, MEM_DECOMMIT | MEM_RELEASE);
  }
}
...
BOOL COverLappedSocket::recv(SOCK_DATA_NODE** pNode)
{
  *pNode = NULL;
  ...
  if (m_RecvIOOper.nOperBytes  <= sizeof(PACKGE_HEADR)) {
    return false;
  }
  ...
  if (m_RecvIOOper.nOperBytes < header.len + sizeof(PACKGE_HEADR)) {
    return false;
  }
