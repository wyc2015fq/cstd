...
CNet_sock::CNet_sock(CNetManager& mgr) :
  m_Mgr(mgr)
{
  m_map.clear();
}
...
void CNet_sock::SerchMember(SOCKET sock, const SOCK_ADDRESS& GroupIPAddr)
{
...
void CNet_sock::StopSerch(SOCKET sock)
{
  Erase(sock);
}

void CNet_sock::Listen(SOCKET sock, BOOL bStartUp)
{
  bStartUp ? Insert(sock, eLISTEN) : Erase(sock);
}

void CNet_sock::JoinGroup(SOCKET sock, BOOL bJoin)
{
  bJoin ? Insert(sock, eRESPSERCH) : Erase(sock);
}

BOOL CNet_sock::Insert(SOCKET sock, SOCK_CLASS sc)
{
  AoutLock lock(&m_syn);
...
BOOL CNet_sock::Erase(SOCKET sock)
{
  AoutLock lock(&m_syn);
  SOCK_MAP::iterator it = m_map.find(sock);
...
void CNet_sock::HandleOper(SOCKET sock)
{
  SOCK_CLASS sc = eNONE;
  m_syn.Lock();
...
void CNet_sock::ListenOper(SOCKET sock)
{
...
void CNet_sock::SerchOper(SOCKET sock)
{
  SOCK_ADDRESS PeerAddr = {0};
  int8 flag = 0;
