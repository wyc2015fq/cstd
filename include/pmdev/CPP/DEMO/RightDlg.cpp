...
BOOL CRightDlg::OnInitDialog() 
{
	...
	m_listTime.InsertColumn( 0, _T("选择"),   LVCFMT_CENTER, (width / 8)  );
	m_listTime.InsertColumn( 1, _T("时间组"), LVCFMT_CENTER, width - (width / 8)  );
...
void CRightDlg::LoadRight()
{
	...
	m_hRootItem = m_treeRight.InsertItem(_T("权限组"),0,1);
...
void CRightDlg::OnButtonNew() 
{
	...
	sql.Format( _T("insert into[right] ([Name]) values( \'%s\')"), _T("权限") );
	m_ado.ExecuteNoSelectSQL( sql );
	m_hCurItem = m_treeRight.InsertItem( _T("权限"), 0, 1, m_hRootItem );
