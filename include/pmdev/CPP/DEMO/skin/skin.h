...
enum {
  SW_CSkinButton = 0,           // 按钮
  SW_CSkinStatic,           // 说明条
  SW_CSkinGroupBox,           // 说明条
  SW_CSkinEdit,           // 编辑框
  SW_CEditWnd,           // 编辑框
  SW_CXPEdit,           // 编辑框
  SW_CSkinWin,           // 对话框窗口
  SW_CSkinOutlookBar,           // 树
  SW_CSkinListVeiw,
  SW_CSkinHeaderCtrl2,
  SW_CSkinVerticleScrollbar2,           // 弹出窗口
  SW_CSkinHorizontalScrollbar2,           // 弹出窗口
  SW_CCreditsCtrl,           // 弹出窗口
  SW_CPPToolTip,           // 弹出窗口
  SW_CScrollWnd,           // 滚动条
  SW_CSkinScrollBar,           // 滚动条
  SW_CSkinScrollWnd,           // 滚动条
  SW_SCROLLWND,           // 滚动条
  SW_CButtonST,           // 滚动条
  SW_CSuperGridCtrl,           // 滚动条
  //SW_WINDOW, // 弹出窗口
};
// 按钮控件风格
enum {
  SW_PUSHBUTTON = 0,         // 0 按钮
  SW_CHECKBUTTON,           // 1 复选框
  SW_RADIOBUTTON,           // 2 单选框
  SW_GROUPBOX,              // 3 组窗口
};
...
typedef struct TrackMouse {
  ...
  BOOL m_bSelectOver; //鼠标左键按下位于按钮之上盘旋为true,反之为flase
  POINT m_pt;
}
TrackMouse;
...
typedef struct CSkinWin {
  CSUBCLASSWNDDEF;
  HBITMAP m_bmpCaption;       //位图 标题条
  HBITMAP m_bmpLeftBorder;    //位图 左边框
  HBITMAP m_bmpRightBorder;   //位图 右边框
  HBITMAP m_bmpBottomBorder;  //位图 下边框
  HBITMAP m_bmpMaxButton;     //位图 最大化标记按纽
  HBITMAP m_bmpMinButton;     //位图 最小化标记按纽
  HBITMAP m_bmpCloseButton;   //位图 关闭标记按纽
  HBITMAP m_bmpRestoreButton; //位图 恢复窗口标记按纽
  HBITMAP m_bmpHelpButton;    //
  int m_CaptionHeight;        //标题条高度
  int m_BorderLeftWidth;      //左边框宽度
  int m_BorderRightWidth;     //右边框宽度
  int m_BorderBottomHeight;   //下边框高度
  int m_nGray;                //最大最小是否禁止
  ...
  BOOL m_bTracking;
  BOOL m_bSizable ; //允许改变尺寸
  BOOL m_bMinable ; //最小
  BOOL m_bMaxable ; //最大
  BOOL m_bHelp;     //帮助
  BOOL m_bSysmenu ; //系统关闭
  BOOL m_bClose;
  BOOL m_bCaption ; //标题栏
  BOOL m_bFrame;    //有标题篮的FRAME
  BOOL m_bActive;
  BOOL m_InitialShow; //第一次显示窗口
  HICON m_hIcon;
  HRGN m_hRgn;
...
typedef struct CSkinOutlookBar {
  CSUBCLASSWNDDEF;
  HBITMAP m_bmpSkin;          //皮肤
  RECT m_rcSkin;              //皮肤中要用到的区域
  int m_nSelectedItem;        //被用户选中的节点ID
  int m_nSelectedFolderItem;  //被用户选中的节点ID所在的目录节点号
  int m_nItemNumber;          //节点个数
  int m_nItemHeight;          //皮肤图片中每个节点按钮的高度(像素)
  unsigned char m_bLevel[ OB_MAXITEMNUM ]; //节点是不是目录，1表示目录节点, 0表示子节点
  unsigned char m_bShowIndex[ OB_MAXITEMNUM ];
  BOOL m_bOnlyOneExpend;      //是否只展开一个目录 1是 0不是
  int m_nShowIndexLen;
  int m_nHitOn;
  COLORREF m_crText;
  COLORREF m_crDisableText;
...
typedef struct CSkinStatic {
  CSUBCLASSWNDDEF;
  HBITMAP m_bmpStaticBackGround;    //背景皮肤
  COLORREF m_crText;
  COLORREF m_crDisableText;
  COLORREF m_crWindow;
...
typedef struct CSkinEdit {
  CSUBCLASSWNDDEF;
  HBITMAP m_bmpEditBackGround;    //背景皮肤
  COLORREF m_crText;
  COLORREF m_crDisableText;
  COLORREF m_crBorder;
...
typedef struct CSkinGroupBox {
  CSUBCLASSWNDDEF;
  COLORREF m_crLine; // 线框颜色
  COLORREF m_crLineShade; // 线框阴影颜色
  BOOL m_bDrawLineShade; // 画线框阴影
}
CSkinGroupBox;
///////////////////////////////////////////////////////////////////////////////
// CPPToolTip 提示控件
...
typedef struct CButtonST {
  ...
  BOOL m_bIsFlat;        // Is a flat button?  平坦风格
