...
void CGroupTimeDlg::LoadTimeGroup()
{
	...
	m_hRootItem = m_treeTG.InsertItem(_T("所有时间组"),0,1);
...
void CGroupTimeDlg::LoadWeekDay()
{
	...
	HTREEITEM	hItem;
	hItem = m_treeWeek.InsertItem(_T("星期日"),0,1);
	m_hWeekFirst = hItem;
	m_treeWeek.SetItemData(hItem,(DWORD)64);
	hItem = m_treeWeek.InsertItem(_T("星期一"),0,1);
	m_treeWeek.SetItemData(hItem,(DWORD)1);
	hItem = m_treeWeek.InsertItem(_T("星期二"),0,1);
	m_treeWeek.SetItemData(hItem,(DWORD)2);
	hItem = m_treeWeek.InsertItem(_T("星期三"),0,1);
	m_treeWeek.SetItemData(hItem,(DWORD)4);
	hItem = m_treeWeek.InsertItem(_T("星期四"),0,1);
	m_treeWeek.SetItemData(hItem,(DWORD)8);
	hItem = m_treeWeek.InsertItem(_T("星期五"),0,1);
	m_treeWeek.SetItemData(hItem,(DWORD)16);
	hItem = m_treeWeek.InsertItem(_T("星期六"),0,1);
...
void CGroupTimeDlg::OnButtonNew() 
{
	...
	sql.Format( _T("insert into[timegroup] ([checkflag],[name]) values( %d, \'%s\')"), 0x08, _T("时间组") );
	m_ado.ExecuteNoSelectSQL(sql);
	m_hCurItem = m_treeTG.InsertItem(_T("时间组"),0,1,m_hRootItem);
