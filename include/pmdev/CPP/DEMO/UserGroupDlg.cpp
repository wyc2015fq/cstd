...
BOOL UserGroupDlg::OnInitDialog() 
{
	...
	m_listUser.InsertColumn(1, _T("名称"), LVCFMT_CENTER, width/2);
	...
	m_listSelUser.InsertColumn(0, _T("验证"), LVCFMT_CENTER, width/3);
	m_listSelUser.InsertColumn(1, _T("ID"), LVCFMT_CENTER, width/3);
	m_listSelUser.InsertColumn(2, _T("名称"), LVCFMT_CENTER, width/3);
...
void UserGroupDlg::LoadGroup()
{
	...
	m_hRootItem = m_treeUG.InsertItem(_T("用户组"),0,1);
...
void UserGroupDlg::OnButtonNew() 
{
	...
	sql.Format( _T("insert into[ugroup] ([Name]) values( \'%s\')"), _T("用户组") );
	m_ado.ExecuteNoSelectSQL( sql );
	m_hCurItem = m_treeUG.InsertItem( _T("用户组"), 0, 1, m_hRootItem );
