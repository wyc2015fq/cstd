...
BOOL DevSetDlg::OnInitDialog() 
{
	...
	m_cmbValide.AddString( _T("1:1人脸加载") );
	m_cmbValide.AddString( _T("混合人脸加载") );
	m_cmbValide.AddString( _T("1:N人脸加载") );
...
void DevSetDlg::OnButtonUp() 
{
	...
	if ( ((CButton*)GetDlgItem(IDC_CHECK_OPEN_A))->GetCheck() )
	{
		...
		{
			AfxMessageBox( _T("1:1模式下必须选择刷卡或者是工号验证方式"));
			return;
		}
	...
	else
	{
		...
		{
			AfxMessageBox( _T("无人脸验证模式下必须选择刷卡验证方式"));
			return;
		}
		mode |= 0x00000002;
		mode |= ((CButton*)GetDlgItem(IDC_CHECK_OPEN_C))->GetCheck() ? 0x00000001 : 0x00000000;
	...
	dev_att.m_nWGType = ((CButton*)GetDlgItem(IDC_CHECK_WG_B))->GetCheck();	
...
//	CHCRecordsetPtr tmp = m_ado.ExecuteNoSelectSQL(sql);
	...
	if ( CPM_ULDevWorkAttInfo( m_IP, dev_att) )
	{
		AfxMessageBox( _T("数据上传成功") );
	}
	else
	{
		AfxMessageBox( _T("数据上传失败") );
	}
}
...
void DevSetDlg::OnButtonDown() 
{
	...
	if ( !(CPM_DLDevWorkAttInfo(m_IP)) )
	{
		AfxMessageBox( _T("数据发送请求失败") );
	}
}
