...
CVideo::CVideo() : m_Net(NULL)
{
  InitialTable();
}

BOOL CVideo::StartVideo(const char* cszIP, void* hwnd, frame_callback_t frame_rgb)
{
  AoutLock lock(&m_syn);
  ...
  if (it == m_map.end()) {
    ...
    pro->user         = hwnd;
    pro->frame_rgb         = frame_rgb;
    m_map.insert(std::pair<unsigned long, VideoPro*>(l, pro));
    //DEBUG_INFO3( "StartVideo user = %08x , W = %d , H = %d" , pro->user , pro->m_Rect.right-pro->m_Rect.left , pro->m_Rect.bottom-pro->m_Rect.top );
  }
  else {
    it->second->user = hwnd;
    it->second->frame_rgb         = frame_rgb;
  }
...
void CVideo::ShowVideo(const char* cszIP, const char* cpbImage, int w, int h, int step, int nImageType)
{
  ...
  if (pro->frame_rgb) {
    pro->frame_rgb(cszIP, pro->m_pRGBTRANS,
        pro->m_pBitmapInfo->bmiHeader.biWidth,
        pro->m_pBitmapInfo->bmiHeader.biHeight,
        pro->m_pBitmapInfo->bmiHeader.biWidth * 3,
        0, pro->user);
  }
