{
  ...
  case DEV_USER_EX_OPER:
    if (DEV_OPER_DOWNLOAD == eFlag && OPER_BATCH_DL_COMPLETE == eResult) {
      ...
      {
        dev_insert_users(s->user, pUserBatch->m_pUserArray, pUserBatch->m_nCount, 1);
  ...
  case DEV_USER_RANGE_OPER:
    if (DEV_OPER_DOWNLOAD == eFlag) {
      ...
      if (nCount > 0) {
        ...
        arrID = MALLOC(DEV_CID, nCount);

        for (i = 0; i < nCount; ++i) {
          arrID[i] = dev_dl_user_range->m_pArray[i].m_UserID;
        }
  ...
  case DEV_RECORD_EX_OPER: {
    if (DEV_OPER_DOWNLOAD == eFlag) {
      ...
      {
        dev_insert_records(s->record, pBatchRecord->m_pRecordArray, pBatchRecord->m_nCount, 1);
        listbox_set_item_count(s->lb_rec, s->record->m_nCount);
      }
    }
  }
  break;
