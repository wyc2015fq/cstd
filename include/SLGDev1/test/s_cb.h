...
int ssl_print_curves(BIO* out, SSL* s, int noshared)
{
    ...
    curves = (int*)app_malloc(ncurves * sizeof(int), "curves to print");
                             ...
                             unsigned int* cookie_len)
{
    ...
    length += sizeof(port);
    buffer = (unsigned char*)app_malloc(length, "cookie generate buffer");
                                   ...
                                   void* other, void* ex)
{
    ...
    case SSL_SECOP_OTHER_CIPHER:
        BIO_puts(sdb->out, SSL_CIPHER_get_name((const struct ssl_cipher_st*)other));
        break;
#ifndef OPENSSL_NO_EC
    ...
    case SSL_SECOP_OTHER_CERT:
    {
        if (cert_md)
        {
            int sig_nid = X509_get_signature_nid((const struct x509_st*)other);
            BIO_puts(sdb->out, OBJ_nid2sn(sig_nid));
        }

        else
        {
            EVP_PKEY* pkey = X509_get0_pubkey((const struct x509_st*)other);
            const char* algname = "";
    ...
    {
        const unsigned char* salg = (const unsigned char*)other;
