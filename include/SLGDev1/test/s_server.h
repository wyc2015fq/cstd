...
{
    tlsextnextprotoctx* next_proto = (tlsextnextprotoctx*)arg;
...
#endif
    buf = (char*)app_malloc(bufsize, "server buffer");
    ...
    for (;;)
    {
        ...
        if (!read_from_sslcon)
        {
            ...
            */
            tv.tv_sec = 1;
            tv.tv_usec = 0;
            i = select(width, (struct fd_set*)&readfds, NULL, NULL, &tv);
...
static int www_body(int s, int stype, unsigned char* context)
{
    ...
    width = s + 1;
    buf = (char*)app_malloc(bufsize, "server www buffer");
    ...
    for (;;)
    {
                ...
                ((www == 2) && (strncmp("GET /stats ", buf, 11) == 0)))
        {
            ...
            if (www == 1 && strncmp("GET /reneg", buf, 10) == 0)
            {
                ...
                FD_ZERO(&readfds);
                openssl_fdset(s, &readfds);
                i = select(width, (struct fd_set*)&readfds, NULL, NULL, NULL);
...
static int rev_body(int s, int stype, unsigned char* context)
{
    ...
    SSL* con;
    BIO* io, *ssl_bio, *sbio;
    buf = (char*)app_malloc(bufsize, "server rev buffer");
    io = BIO_new(BIO_f_buffer());
    ssl_bio = BIO_new(BIO_f_ssl());
...
{
    simple_ssl_session* sess = (simple_ssl_session*)app_malloc(sizeof(*sess), "get session");
    unsigned char* p;
