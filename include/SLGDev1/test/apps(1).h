...
#endif
struct bio_st* bio_err = 0;
char* opt_getprog()
{
    return "";
}
int opt_format(char const*, unsigned long, int*)
{
    return 0;
}
char* opt_arg()
...
int password_callback(char* buf, int bufsiz, int verify, PW_CB_DATA* cb_tmp)
{
    ...
    if (ui)
    {
        ...
        if (ok >= 0 && verify)
        {
            buff = (char*)app_malloc(bufsiz, "password buffer");
...
CA_DB* load_index(const char* dbfile, DB_ATTR* db_attr)
{
    ...
    retdb = (struct ca_db_st*)app_malloc(sizeof(*retdb), "new DB");
    retdb->db = tmpdb;
    tmpdb = NULL;
...
unsigned char* next_protos_parse(size_t* outlen, const char* in)
{
    ...
    out = (unsigned char*)app_malloc(strlen(in) + 1, "NPN buffer");
...
static const char* get_dp_url(DIST_POINT* dp)
{
    ...
    for (i = 0; i < sk_GENERAL_NAME_num(gens); i++)
    {
        ...
        uri = (struct asn1_string_st*)GENERAL_NAME_get0_value(gen, &gtype);
...
static STACK_OF(X509_CRL)* crls_http_cb(X509_STORE_CTX* ctx, X509_NAME* nm)
{
    ...
    crldp = (struct stack_st_DIST_POINT*)X509_get_ext_d2i(x, NID_crl_distribution_points, NULL, NULL);
    ...
    crldp = (struct stack_st_DIST_POINT*)X509_get_ext_d2i(x, NID_freshest_crl, NULL, NULL);
    crl = load_crl_crldp(crldp);
    sk_DIST_POINT_pop_free(crldp, DIST_POINT_free);
...
static int WIN32_rename(const char* from, const char* to)
{
    ...
    else     /* UNICODE path */
    {
        ...
        tfrom = (char*)malloc(sizeof(*tfrom) * (flen + tlen));
 ...
 */
BIO* bio_open_owner(const char* filename, int format, int private1)
{
    FILE* fp = NULL;
    BIO* b = NULL;
    int fd = -1, bflags, mode, textmode;

    if (!private1 || filename == NULL || strcmp(filename, "-") == 0)
    {
