...
public:
    ...
    {
        return m_Verify && m_Socket && SLG_S_SendData(m_Socket , buf , lpLen);
    }

    BOOL NetDrSend(const char* buf, unsigned int* lpLen)
    {
        return m_Socket && SLG_S_SendData(m_Socket , buf , lpLen);
    }
...
void RecvData(SOCKET hSock , const char* buf, unsigned int len, void* param)
{
    ...
    if (!pNetObj->m_Verify && (pCmd->m_MCmd != CMD_M_AUTH || !(pCmd->m_CFlag & AUTH_VERIFY)))
    {
    ...
    if (pNetObj->m_Idle)
    {
        ...
        case CMD_M_USER:
            if (CMD_S_REALTIME_LOAD != pCmd->m_SCmd)
            {
    ...
    if (pCmd->m_MCmd != CMD_M_HEART && pCmd->m_MCmd != CMD_M_APPUPDATE && pCmd->m_MCmd != CMD_M_REBOOT)
    {
...
CNetS::CNetS() : m_Socket(0) , m_Verify(false) , m_NetInitOK(false) , m_Idle(false)
{
...
BOOL CNetS::NetInit(int af)
{
    ...
    if (SLG_S_Init(af, 1024 * 1024 , SERVER_SELECTED))
    {
        //SLGDB::SetCallBack( NetCallBack , this );
        SLG_S_SetRecvCallBack(RecvData , this);
        SLG_S_SetCNCallBack(ConnetcStatus , this);
        memset(&m_MultAdress , 0 , sizeof(m_MultAdress));
        //net_addr(MULTYGROUP_IP, MULTYGROUP_PORT, &m_MultAdress);
        net_addr(ANYIP(af), MULTYGROUP_PORT, &m_MultAdress);
        SLG_S_AddInMultyGroup(true , &m_MultAdress, this);
        m_NetInitOK = SLG_S_StartListen(true);
        return m_NetInitOK;
    }
...
BOOL CNetS::NetUinit(void)
{
    m_NetInitOK = false;
    NetDisconnect();
    SLG_S_StartListen(false);
    SLG_S_AddInMultyGroup(false , &m_MultAdress, this);
    SLG_S_SetRecvCallBack(0 , this);
    SLG_S_SetCNCallBack(0 , this);
    //SLGDB::SetCallBack( 0 , 0 );
    SLG_S_UnInit();
    m_Socket = 0;
    return true;
}
...
BOOL CNetS::NetListen(BOOL flag)
{
    ...
    BOOL ret = SLG_S_StartListen(flag);
    TEST_INFO;
    return ret;
}
...
BOOL CNetS::NetChangeMultAdress(const addr_in* addr)
{
    ...
    if (SLG_S_AddInMultyGroup(false , &m_MultAdress, this))
    {
        if (SLG_S_AddInMultyGroup(true , addr, this))
        {
            m_MultAdress = *addr;
...
{
    if (0 != m_Socket && NULL != addr)
    {
        return SLG_S_Address(m_Socket , addr);
    }
