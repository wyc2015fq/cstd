...
struct FormatBGR24
{
    ...
    static VOID Copy(LPBYTE lpSurface, const ImageSpace& imageSpace, DWORD dwWidth, DWORD dwHeight, LONG lPitch)
    {
        ...
        for (LONG i = 0; i < dwHeight; ++i)
        {
            ZeroMemory(lpSurface, lPitch);

            for (LONG j = 0; j < dwWidth; ++j)
            {
                memcpy(lpSurface + 4 * j , lpSrcY + 3 * j , 3);
            }
...
struct FormatRGB32
{
    ...
    static VOID Copy(LPBYTE lpSurface, const ImageSpace& imageSpace, DWORD dwWidth, DWORD dwHeight, LONG lPitch)
    {
        memcpy(lpSurface , imageSpace.pBuffer[0] , dwWidth * dwHeight * 4);
    }
};
struct FormatBGR32
{
...
public:
    ...
    HRESULT Draw(const ImageSpace& imageSpace)
    {
...
Exit:
    ...
    HRESULT Create(HWND hWnd, DDSURFACEDESC2& ddOffScreenSurfaceDesc , CopyCallback pCopyFun)
    {
        Release();
        ...
        hr = m_pDirectDraw->SetCooperativeLevel(hWnd, DDSCL_NORMAL | DDSCL_NOWINDOWCHANGES);
