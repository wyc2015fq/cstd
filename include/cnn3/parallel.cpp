...
//
  ...
  P2PSync<Dtype>::P2PSync(SHARED_PTR(Solver<Dtype) > root_solver,
                          P2PSync<Dtype>* parent, CJSON* param)
      ...
      solver_()
  {
    ...
    if (parent == NULL) {
      ...
      // set_root_solver(false);
      solver_.reset(new WorkerSolver<Dtype>(param, root_solver.get()));
      //set_root_solver(true);
    }
  ...
  {
    SetDevice(solver_->param().device_id());
    CHECK(root_solver());
    //set_root_solver(false);
...
//  CHECK(false);
  ...
  void P2PSync<Dtype>::on_gradients_ready()
  {
    ...
    if (parent_) {
      ...
      caffe_gpu_scal(size_, Dtype(1.0 / solver_count()), diff_);
    }
#endif
  }
