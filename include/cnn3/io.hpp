...
const int kProtoReadBytesLimit = INT_MAX;  // Max size of 2 GB minus 1 byte.
  ...
  inline bool ReadProtoFromTextFile(const string & filename, cJSON* proto)
  {
    return ReadProtoFromTextFile(filename.c_str(), proto);
  }

  inline void ReadProtoFromTextFileOrDie(const char* filename, cJSON* proto)
  {
    CHECK(ReadProtoFromTextFile(filename, proto));
  }

  inline void ReadProtoFromTextFileOrDie(const string & filename, cJSON* proto)
  {
    ReadProtoFromTextFileOrDie(filename.c_str(), proto);
  }

  void WriteProtoToTextFile(CJSON* proto, const char* filename);
  inline void WriteProtoToTextFile(CJSON* proto, const string & filename)
  {
    WriteProtoToTextFile(proto, filename.c_str());
  }

  bool ReadProtoFromBinaryFile(const char* filename, cJSON* proto);

  inline bool ReadProtoFromBinaryFile(const string & filename, cJSON* proto)
  {
    return ReadProtoFromBinaryFile(filename.c_str(), proto);
  }

  inline void ReadProtoFromBinaryFileOrDie(const char* filename, cJSON* proto)
  {
  ...
  void WriteProtoToBinaryFile(CJSON* proto, const char* filename);
  inline void WriteProtoToBinaryFile(
    CJSON* proto, const string & filename)
  {
...
#undef TYPEFLAG_DEF
  ...
  bool Blob_NCHW(BlobData* blob, bool encoded, const T* data, int dim_w, int dim_h = 1, int dim_c = 1, int dim_n = 1) {
    ...
    blob->add_dim(dim_c);
    blob->add_dim(dim_n);
    blob->set_dim(data, size*sizeof(T));
    blob->type = ((TypeFlag)TypeFlag_TF<T>::flag);
    blob->encoded = (encoded);
    return true;
  }
