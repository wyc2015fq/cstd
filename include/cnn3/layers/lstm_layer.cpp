...
namespace caffe
{
  ...
  template <typename Dtype>
  void LSTMLayer<Dtype>::FillUnrolledNet(CJSON* net_param) const
  {
    const int num_output = this->layer_param_.recurrent_param().num_output();
    ...
    for (int t = 1; t <= this->T_; ++t) {
      ...
      {
        CJSON* input_sum_layer = net_param->add_layer();
      ...
      //         h_t := o_t .* \tanh[c_t]
      {
        CJSON* lstm_unit_param = net_param->add_layer();
    ...
    {
      CJSON* c_T_copy_param = net_param->add_layer();
