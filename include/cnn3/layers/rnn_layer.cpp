...
namespace caffe
{
  ...
  template <typename Dtype>
  void RNNLayer<Dtype>::FillUnrolledNet(CJSON* net_param) const
  {
    const int num_output = this->layer_param_.recurrent_param().num_output();
    ...
    //     W_xh_x = W_xh * x + b_h
    {
      CJSON* x_transform_param = net_param->add_layer();
    ...
    if (this->static_input_) {
      ...
      CJSON* x_static_transform_param = net_param->add_layer();
      ...
      CJSON* reshape_param = net_param->add_layer();
    ...
    CJSON* x_slice_param = net_param->add_layer();
    ...
    for (int t = 1; t <= this->T_; ++t) {
      ...
      //                       0   otherwise
      {
        CJSON* cont_h_param = net_param->add_layer();
      ...
      {
        CJSON* w_param = net_param->add_layer();
      ...
      {
        CJSON* h_input_sum_param = net_param->add_layer();
      ...
      {
        CJSON* h_neuron_param = net_param->add_layer();
      ...
      //          = \tanh( W_ho_h_t )
      {
        CJSON* o_neuron_param = net_param->add_layer();
