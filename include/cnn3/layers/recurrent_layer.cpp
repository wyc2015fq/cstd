...
#include "filler.hpp"
#include "layer.hpp"
#include "layers/recurrent_layer.hpp"
...
namespace caffe
{
                                         ...
                                         const vector<Blob<Dtype>*> & top)
  {
    ...
    if (layer_name.size()) {
      for (int i = 0; i < net_param.layer_size(); ++i) {
        CJSON* layer = net_param.mutable_layer(i);
    ...
    for (int i = 0; i < output_names.size(); ++i) {
      CJSON* layer = net_param.add_layer();
                                      ...
                                      const vector<Blob<Dtype>*> & top)
  {
    ...
    if (bottom.size() == 1) {
      cont_shape.push_back(T_);
      cont_shape.push_back(N_);
    } else {
      cont_shape = bottom[1]->shape();
    }
