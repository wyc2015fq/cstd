...
#include "test/test_gradient_check_util.hpp"
...
namespace caffe
{
  ...
  protected:
    ...
    void ReferenceLRNForward(const Blob<Dtype> & blob_bottom,
                             CJSON* layer_param, Blob<Dtype>* blob_top);
  ...
  void LRNLayerTest<TypeParam>::ReferenceLRNForward(
    const Blob<Dtype> & blob_bottom, CJSON* layer_param,
    Blob<Dtype>* blob_top)
  {
    typedef typename TypeParam::Dtype Dtype;
  ...
  void CuDNNLRNLayerTest<TypeParam>::ReferenceLRNForward(
    const Blob<TypeParam> & blob_bottom, CJSON* layer_param,
