...
Phase get_phase_from_flags(Phase default_value)
{
  if (FLAGS_phase == "") {
...
// test nets.
#if 0
void CopyLayers(Solver<float>* solver, const std::string & model_list)
{
...
int train()
{
  ...
  CJSON* param = cJSON_OpenFile(FLAGS_solver.c_str());
  CJSON* solver_param = param->GetObjectItem("solver");
#if 0
  ...
  if (FLAGS_gpu.size() == 0 && (0==strcmp(solver_param->GetObjectString("solver_mode", "GPU"), "GPU") )) {
    FLAGS_gpu = solver_param->GetObjectString("device_id", "0");
  }
#ifdef USE_CUDNN
...
#if 0
  Solver<float> solver(CreateSolver(solver_param));
  ...
  if (gpus.size() > 1) {
    P2PSync<float> sync(solver, NULL, solver->param());
    sync.Run(gpus);
  } else {
...
int test()
{
  ...
  Net<float> caffe_net(FLAGS_model, TEST, FLAGS_level, &stages);
...
int time()
{
  ...
  Phase phase = get_phase_from_flags(TRAIN);
  vector<string> stages = get_stages_from_flags();
  // Set device id and mode
  vector<int> gpus;
  get_gpus(&gpus);
  ...
  caffe_net.Backward();
  const vector<Layer<float> > & layers = caffe_net.layers();
