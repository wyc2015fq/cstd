...
namespace caffe
{
  bool NetNeedsV0ToV1Upgrade(CJSON* net_param)
  {
  ...
  bool NetNeedsV1ToV2Upgrade(CJSON* net_param)
  {
    return net_param.layers_size() > 0;
  }

  void UpgradeV0PaddingLayers(CJSON* param,
                              CJSON* param_upgraded_pad)
  {
    ...
    for (int i = 0; i < param.layers_size(); ++i) {
      const V1CJSON* layer_connection = param.layers(i);
      const V0CJSON* layer_param = layer_connection.layer();
      ...
      for (int j = 0; j < layer_connection.bottom_size(); ++j) {
        ...
        const V1CJSON* source_layer = param.layers(top_idx);
  ...
  bool NetNeedsDataUpgrade(CJSON* net_param)
  {
    for (int i = 0; i < net_param.layers_size(); ++i) {
...
#define CONVERT_LAYER_TRANSFORM_PARAM(TYPE, Name, param_name) \
  ...
  void UpgradeNetDataTransformation(CJSON* net_param)
  {
    for (int i = 0; i < net_param->layers_size(); ++i) {
  ...
  bool NetNeedsInputUpgrade(CJSON* net_param)
  {
    return net_param.input_size() > 0;
  }

  bool UpgradeV1LayerParameter(const V1CJSON* v1_layer_param,
                               CJSON* layer_param)
  {
    layer_param->Clear();
  ...
  bool UpgradeV1Net(CJSON* v1_net_param, CJSON* net_param)
  {
  ...
  void UpgradeNetInput(CJSON* net_param)
  {
    ...
    if (has_shape || has_dim) {
      CJSON* layer_param = net_param->add_layer();
  ...
  bool UpgradeV0LayerParameter(const V1CJSON* v0_layer_connection,
    V1CJSON* layer_param)
  {
    bool is_fully_compatible = true;
    layer_param->Clear();
    ...
    if (v0_layer_connection.has_layer()) {
      const V0CJSON* v0_layer_param = v0_layer_connection.layer();
...
// Return true iff the solver contains any old solver_type specified as enums
  bool SolverNeedsTypeUpgrade(CJSON* solver_param)
  {
  ...
  bool UpgradeV0Net(CJSON* v0_net_param_padding_layers,
    CJSON* net_param)
  {
...
// Read parameters from a file into a SolverParameter proto message.
  ...
  bool UpgradeNetAsNeeded(const string & param_file, CJSON* param)
  {
    bool success = true;
  ...
  bool NetNeedsUpgrade(CJSON* net_param)
  {
