...
#include <stdio.h>
#include <math.h>
#include "DCT2D_8x8.inl"
#include "DCT2D_16x16.inl"
#ifdef _OPENMP
#include <omp.h>
#endif
#undef ABS
...
void Image2Patches(vector<float>&, vector< vector< vector< vector< float > > > >&, int, int, int, int, int);
void Patches2Image(vector<float>&, vector< vector< vector< vector< float > > > >&, int, int, int, int, int);
    ...
    int height_p)
{
  ...
  for (j = 0; j < height - height_p + 1; j ++) {
    for (i = 0; i < width - width_p + 1; i ++) {
      int counter_pixel = 0;
