          ...
          if (';' != p[ j ]) { //skip over comment
            while (j < i && p[ j ] != '=') {
              ...
              if ('=' == p[ j ]) {
                ...
                if (key_len == (j1 - newline_start) && strncmp(key, p + newline_start, key_len) == 0) {
                  //find the key ok
                  *key_s = newline_start;
                  *key_e = j - 1;
                  *value_s = j + 1;
...
*/
int ini_get_string(const char* section, const char* key, const char* default_value, char* value, int size, const char* file)
{
...
*/
int ini_get_int(const char* section, const char* key, int default_value, const char* file)
{
  char value[ 32 ] = {0};
  if (!ini_get_string(section, key, NULL, value, sizeof(value), file)) {
 ...
 * @return 1 : success\n 0 : failure
 */
int ini_set_string(const char* section, const char* key, const char* value, const char* file)
{
  char buf[ MAX_FILE_SIZE ] = {0};
  ...
  if (-1 == sec_s) {
    if (0 == file_size) {
      snprintf(w_buf, MAX_FILE_SIZE, "[%s]\n%s=%s\n", section, key, value);
    }
    else {
