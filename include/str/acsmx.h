...
#define ACSM_FAIL_STATE   -1     // 宏定义

typedef struct _acsm_pattern {  //  _acsm_pattern  为结构体
  struct  _acsm_pattern* next;    //  指针类型为结构体
  unsigned char*         patrn;
  unsigned char*         casepatrn;    //  指针的类型   unsigned  char
  int      n;
  int      nocase;
  void*    id;   //  指向无类型对象的指针
  int    nmatch;
