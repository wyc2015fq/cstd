 ...
 */
#include "esignal.h"
#include "esig_asc.inl"
#include "esig_edr.inl"
#include "esig_nat.inl"
/*
...
static int GetLine(char* buf, int len, stream_t* file);
static int GetLong(long* val, int len, stream_t* file);
...
OpenIn(char* filename, /* name of input file */
    ...
    long* hdr_size, /* header size (output) */
    long* rec_size, /* record size (output) */
    stream_t** file) /* input file (output) */
{
  stream_t* input;
  if (filename == NULL) {
    return NULL;
