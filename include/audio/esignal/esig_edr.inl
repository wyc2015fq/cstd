...
static int ReadEdrArray(Array* array, stream_t* file, int longlong);
static int EdrReadArray(Array* data, long length, stream_t* file, int longlong)
{
  long n;
  for (n = 0;
...
*/
static FieldSpec* ReadEdrFieldSpec(stream_t* file, int longlong);
int ReadEdrFieldList(FieldList* listp, /* output variable */
    stream_t* file, /* input file */
    int longlong)
{
  FieldList list;
  long num_fields;
 ...
 */
static int ReadEdrData(FieldSpec* field, stream_t* file, int longlong);
int ReadEdrRecord(FieldSpec** fields, stream_t* file, int longlong)
{
  long i;
  long nopt;
  Uchar flags;
...
*/
static int EdrRead(void* data, int type, long length, stream_t* file, int longlong);
long ReadEdrSamples(void* data, long nrec, FieldSpec** fields, stream_t* file, int longlong)
{
  int type;
  long length;
  if (data == NULL) {
...
static int WriteEdrArray(Array* array, stream_t* file, int longlong);
static int EdrWriteArray(Array* data, long length, stream_t* file, int longlong)
{
  long n;
  for (n = 0;
...
static int WriteEdrFieldSpec(FieldSpec* field, stream_t* file, int longlong);
/*
 * Write field list to file in EDR1 or EDR2 format.
 * Return TRUE on success, FALSE on failure.
 */
int WriteEdrFieldList(FieldList list, /* field list */
 ...
 * Return TRUE on success, FALSE on failure.
*/
static int WriteEdrData(FieldSpec* field, stream_t* file, int longlong);
int WriteEdrRecord(FieldSpec** fields, stream_t* file, int longlong)
{
  long i;
  long nopt;
  Uchar flags;
...
*/
static int EdrWrite(void* data, int type, long length, stream_t* file, int longlong);
long WriteEdrSamples(void* data, long nrec, FieldSpec** fields, stream_t* file, int longlong)
{
  int type;
  long length;
  if (data == NULL) {
...
static int ReadEdrData(FieldSpec* field, stream_t* file, int longlong)
{
  ...
  return (EdrRead(field->data, field->type, length, file, longlong) == length);
}
static int ReadEdrString(char** string, stream_t* file, int longlong);
/*
...
static int ReadEdrString(char** string, stream_t* file, int longlong)
{
  ...
  for (i = 0; i < length && (ch = stream_get8(file)) != EOF; i++) {
    str[i] = ch;
  }
  if (i < length) {
    free(str);
...
static int WriteEdrString(char* string, stream_t* file, int longlong);
/*
