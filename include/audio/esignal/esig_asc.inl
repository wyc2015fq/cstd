 ...
 */
static int SkipComment1(stream_t* file)
{
  int ch;
  do {
    ch = stream_get8(file);
 ...
 */
static int SkipSpace(int ch, stream_t* file)
{
  for (;;) {
    if (isspace(ch)) {
      ch = stream_get8(file);
 ...
 */
static int GetNonSpace(stream_t* file)
{
  int ch;
  ch = stream_get8(file);
 ...
 * returned to the input stream by means of "stream_unget8" and is the first
 ...
 * return -1
 */
static int ReadAsciiName(int depth, char** names, stream_t* file)
{
 ...
 */
static int ReadAsciiType(short* type, stream_t* file)
{
 ...
 */
static int ReadSpace(stream_t* file)
{
  int ch;
  int okay;
  ch = stream_get8(file);
 ...
 * FALSE for error.
 */
static int ReadAsciiDims(short* rank, long** dim, stream_t* file)
{
  long d;
  long* dd;
  int r;
 ...
 * failure.
 */
static int ReadAsciiString(char** string, stream_t* file)
{
  int ch;
  char* str = NULL;
  long alloc_len = 0;
  ...
  for (ch = stream_get8(file); isprint(ch) && ch != '"'; ch = stream_get8(file)) {
    if (ch == '\\') {
      ch = stream_get8(file);
      switch (ch) {
      case '\\': /* fall through */
 ...
 */
static int ReadOptSpace(stream_t* file)
{
  int ch;
  ch = GetNonSpace(file);
  if (ch == EOF) {
 ...
 * is given by x.
 */
static int ReadAsciiDouble(double* x, stream_t* file)
{
 ...
 */
static int ReadAsciiAxisNames(char** *axis_names, int rank, stream_t* file)
{
  char** ax;
  int ch;
  char* str;
 ...
 */
static int ReadAsciiMisc(int rank, char** units, double* scale, double* offset, char** *axis_names, stream_t* file)
{
  /*
...
#define MAX_ATTR 11
  ...
  do {
    ...
    if (strcmp(attr, "units") == 0) {
      if (!got_units++ && ReadAsciiString(&un, file)) {
        if (units != NULL) {
          *units = un;
        }
      }
 ...
 */
static int
ReadAsciiOccurrence(short* occurrence, stream_t* file)
{
  int ch;
  int occ;
  ch = GetNonSpace(file);
...
static int AsciiRead(void* datap, int type, long length, stream_t* file);
/*
 ...
 * Return TRUE for success, FALSE for failure.
 */
static int ReadAsciiArray(Array* array, stream_t* file)
{
  short type;
  short rank;
  long* dim;
  ...
  if (length == 0) {
    ...
    if (data == NULL || AsciiRead(data, type, length, file) != length) {
      if (dim != NULL) {
        free(dim);
      }
 ...
 */
static int ReadAsciiData(FieldSpec* field, stream_t* file)
{
  size_t size;
  long length;
 ...
 */
static int ReadAsciiNewline(stream_t* file)
{
  int ch;
  ch = stream_get8(file);
  for (;;) {
 ...
 * Return TRUE for success, FALSE for failure.
 */
static int ReadAsciiFieldStruct(FieldSpec** field, char* name, stream_t* file)
{
  short type;
  short rank;
  long* dim;
 ...
 */
static int ReadAsciiEscape(unsigned long* val, stream_t* file)
{
  int ch;
  ch = stream_get8(file);
  switch (ch) {
...
static int AsciiRead(void* datap, int type, long length, stream_t* file)
{
  ...
  case ARRAY: {
    ...
    do {
      ...
      if (!ReadOptSpace(file) || !ReadAsciiArray(&data[j], file)) {
        return j;
      }
      ch = GetNonSpace(file);
      if (ch != ')') {
 ...
 */
static int ReadAsciiFieldSpec(FieldSpec** field, int depth, char** names, stream_t* file)
{
  FieldSpec* spec;
  FieldSpec* subfield;
  int name_len;
 ...
 * FALSE on failure.
 */
int ReadAsciiFieldList(FieldList* listp, stream_t* file)
{
 ...
 * Return TRUE on success, FALSE on failure.
 */
int ReadAsciiRecord(FieldSpec** fields,
    stream_t* file)
{
  int ch;
  long i;
 ...
 */
long ReadAsciiSamples(void* data,
    long nrec,
    FieldSpec** fields,
    stream_t* file)
{
 ...
 */
static int WriteAsciiString(char* data, stream_t* file)
{
...
static int AsciiWrite(void* datap, int type, long length, stream_t* file, Annot* annotate);
/*
 *
 */
 ...
 *
 */
static int AsciiWriteSub(void* data, int type, long length, stream_t* file, int rank, long* dim, Annot* annotate)
{
  int i;
  long dim0, j;
  long step, stride;
  ...
  if (rank == 0) {
    ...
    if (type == ARRAY) {
      ...
      step = (annotate->width - annotate->indent - 3 * rank - 2) / ApproxWidth(type);
      if (step < 1) {
        step = 1;
      }
    }
 ...
 * Return TRUE on success, FALSE on failure.
 */
static int WriteAsciiData(FieldSpec* field, stream_t* file, Annot* annotate)
{
 ...
 * with depth > 1.
 */
static int WriteAsciiFieldSpec(FieldSpec* field, int depth, stream_t* file, Annot* annotate)
{
  static char* names[MAX_FLD_DEPTH]; /* field name components */
  int i; /* loop index */
 ...
 * Return TRUE on success, FALSE on failure.
 */
int WriteAsciiRecord(FieldSpec** fields, /* linear array of fields */
    stream_t* file, /* output file */
 ...
 */
static int
WriteAsciiArray(Array* array, stream_t* file, Annot* annotate)
{
  short type;
  short rank;
  long* dim;
