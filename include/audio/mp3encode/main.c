...
{
    fprintf(stderr,"********欢迎来临武汉大学电子信息学院*********\n");
}

static void print_usage()
{
    fprintf(stderr,"USAGE   :  [options] <infile>   <outfile>\n");
...
static void set_defaults()
{
    ...
    if(config.byte_order==order_unknown) 
...
{
    time_t end_time;
	config.infile="child.wav";
	config.outfile="child.mp3";
	//config.infile="child.wav";     //要转换的音频

    time(&config.start_time);
    print_header();
	//printf("**请输入要装换wav文件的路径及完整名字:");
	//scanf("%s",config.infile);
	//printf("\n**请输入装换后mp3完整名字:");
	//scanf("%s",config.outfile);
	//printf("正在转换的音频是:%s\n",config.infile);
    set_defaults();
