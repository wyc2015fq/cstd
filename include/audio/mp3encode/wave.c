...
static bool checkString(FILE *file,char *string)
{
    ...
    if(fread(temp,1,length,file)!=length) 
...
bool wave_open()
{
    ...
    if((config.wave.file = fopen(config.infile,"rb")) == NULL)        //打开文件
...
/* PCM specific */
    if(config.wave.channels>2)           //声道检测
