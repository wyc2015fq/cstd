...
/********************************************************************/
/**************************** Layer II *************/
void II_decode_bitalloc(Bit_stream_struc* bs, unsigned int bit_alloc[2][SBLIMIT], frame_params* fr_ps)
{
  int i, j;
...
/**************************** Layer I *************/
void I_decode_bitalloc(Bit_stream_struc* bs, unsigned int bit_alloc[2][SBLIMIT], frame_params* fr_ps)
{
  int i, j;
  int stereo = fr_ps->stereo;
  int sblimit = fr_ps->sblimit;
...
/****************************************************************/
/************************** Layer I stuff ************************/
void I_decode_scale(Bit_stream_struc* bs, unsigned int bit_alloc[2][SBLIMIT], unsigned int scale_index[2][3][SBLIMIT], frame_params* fr_ps)
{
  int i, j;
  int stereo = fr_ps->stereo;
  int sblimit = fr_ps->sblimit;
  ...
  for (i = 0; i < sblimit; i++) {
    for (j = 0; j < stereo; j++) {
      if (bit_alloc[j][i]) {
        ...
        case 2 :
          scale_index[j][0][i] = scale_index[j][1][i] = scale_index[j][2][i] = getbits(bs, 6);
          break;
        default :
          break;
        }
...
/**************************************************************/
/******************************* Layer I stuff ******************/
void I_buffer_sample(Bit_stream_struc* bs, unsigned int FAR sample[2][3][SBLIMIT], unsigned int bit_alloc[2][SBLIMIT], frame_params* fr_ps)
{
  int i, j, k;
  int stereo = fr_ps->stereo;
  int sblimit = fr_ps->sblimit;
...
/*************************** Layer II stuff ************************/
void II_buffer_sample(Bit_stream_struc* bs, unsigned int FAR sample[2][3][SBLIMIT], unsigned int bit_alloc[2][SBLIMIT], frame_params* fr_ps)
{
  int i, j, k, m;
  int stereo = fr_ps->stereo;
  int sblimit = fr_ps->sblimit;
...
/*
/**************************************************************/
static const double c[17] = { 1.33333333333, 1.60000000000, 1.14285714286,
...
static const double d[17] = { 0.500000000, 0.500000000, 0.250000000, 0.500000000,
...
/**************************** Layer I stuff ******************************/
void I_denormalize_sample(double FAR fraction[2][3][SBLIMIT], unsigned int scale_index[2][3][SBLIMIT], frame_params* fr_ps)
{
  int i, j, k;
  int stereo = fr_ps->stereo;
  int sblimit = fr_ps->sblimit;
...
{
  static const double DEWINDOW[HAN_SIZE] = {
#include "DEWINDOW.txt"
  };
  memcpy(window, DEWINDOW, sizeof(double)*HAN_SIZE);
}
void read_syn_window1(double FAR window[HAN_SIZE])
{
  int i, j[4];
  FILE* fp;
  double f[4];
...
void III_get_side_info(Bit_stream_struc* bs, III_side_info_t* si, frame_params* fr_ps)
{
  int ch, gr, i;
  int stereo = fr_ps->stereo;
  si->main_data_begin = getbits(bs, 9);/*read 9 bit from the bit stream */
  if (stereo == 1) {
...
void III_put_side_info(Bit_stream_struc* bs, III_side_info_t* si, frame_params* fr_ps)
{
  int ch, gr, i;
  int stereo = fr_ps->stereo;
