/********************************************************************
  ...
  本文件编写人：
          YZ      yzfree##yeah.net
          tearshark zlan##corp.netease.com
          menghui   mssoft@sina.com
  ...
  修正历史：
      ...
      2003-8    修正calloc函数对内存内容初始化错误的BUG（想飞的小猪）
      2002-7    改进提示信息输出格式，使之与编译器结合更紧密(2.2)
      2002-7    改进防护宏信息存储机制
  ...
  case DLL_PROCESS_DETACH:
    ...
    if (ima_get_count()) {
      ...
      while (p = ima_search(0)) {
        ...
        im_output_debug_info("%s(%d) : [OUT] 此行代码分配的内存没有被程序释放！\n", _short_filename(p->filename), p->linenum);
...
{
  char*  p = (char*)im_malloc_act(num * size, fn, ln);
...
ISEEMEM_API void CALLAGREEMENT im_free_act(void* paddr, char* fn, int ln)
{
  ...
  /* 写保护状态检测 */
  if (aln->attrib != 0) {
    im_output_debug_info("%s(%d) : [INF] 此行代码分配的内存在没有解除写保护之前就被释放了，不知程序设计上是否正确？\n", _short_filename(aln->filename), aln->linenum);
  }
  ...
  case  1:      /* 前溢出 */
    im_output_debug_info("%s(%d) : [FRE] [前溢出] 此行代码分配的内存发生了前写溢出！\n", _short_filename(aln->filename), aln->linenum);
    break;

  case  2:      /* 后溢出 */
    im_output_debug_info("%s(%d) : [FRE] [后溢出] 此行代码分配的内存发生了后写溢出！\n", _short_filename(aln->filename), aln->linenum);
    break;

  case  3:      /* 前、后均溢出 */
    im_output_debug_info("%s(%d) : [FRE] [前、后溢出] 此行代码分配的内存发生了前、后写溢出！\n", _short_filename(aln->filename), aln->linenum);
    break;
  ...
  if ((paddr >= im_start_addr) && (paddr < (im_start_addr + IM_MAX_REVSIZE))) {
    ...
    while (pa) {
      ...
      if ((paddr >= (pa->guard_addr + im_page_size / 2)) && (paddr < (unsigned char*)(pa->fg_addr))) {
        im_output_debug_info("%s(%d) : [前越界] 这可能是由于%s中%d～%d行之间的某些受控代码，在向文件%s中第%d行源码分配的内存中%s数据时，发生了前越界现象，请注意调试器给出的异常定位点。\n",
      ...
      /* 如果是在下一个防护页的前半部，则可能是后溢出 */
        ...
        im_output_debug_info("%s(%d) : [后越界] 这可能是由于%s中%d～%d行之间的某些受控代码，在向文件%s中第%d行源码分配的内存中%s数据时，发生了后越界现象，请注意调试器给出的异常定位点。\n",
            _short_filename(fn), fln,
            _short_filename(fn), fln, bln,
            _short_filename(pa->filename), pa->linenum,
            (per->ExceptionRecord->ExceptionInformation[0] == 0) ? "读取" : "写入");
      ...
      /* 受控代码试图写受保护的内存块 */
        ...
        if (pa->attrib != 0) {
          im_output_debug_info("%s(%d) : [写保护] 文件%s中%d～%d行之间的某些受控代码，试图向受保护的内存中（%s第%d行源码分配的内存）写入数据，请检查受控代码，同时请参考调试器给出的异常定位点。\n",
              _short_filename(fn), fln,
              _short_filename(fn), fln, bln,
              _short_filename(pa->filename), pa->linenum,
              (per->ExceptionRecord->ExceptionInformation[0] == 0) ? "读取" : "写入");
          ...
          im_output_debug_info("%s(%d) : [未知原因的异常] 文件%s中%d～%d行之间的某些受控代码在操作%s第%d行源码分配的内存时出现了异常情况，防护系统暂时无法判断引发该问题的原因，建议仔细查看受控代码！\n",
              _short_filename(fn), fln,
              _short_filename(fn), fln, bln,
              _short_filename(pa->filename), pa->linenum);
        }
    ...
    if (!pa) {
      /* 输出信息 */
      im_output_debug_info("%s(%d) : [读写已释放的内存] 该问题可能是由%s中%d～%d行之间的某些受控代码，试图对已释放的内存进行操作（读或写）引起的。建议检查受控代码中使用了哪些已被释放的内存。\n",
          _short_filename(fn), fln,
          _short_filename(fn), fln, bln);
    }
  }
    ...
    /* 输出信息 */
    im_output_debug_info("%s(%d) : [读写系统和程序内存区] 发生异常的地址不在防护系统的内存中，请注意查看%s中%d～%d行之间的受控代码，同时也请参考调试器给出的异常定位点。\n",
        _short_filename(fn), fln,
        _short_filename(fn), fln, bln);
  }
...
ISEEMEM_API void CALLAGREEMENT im_check_all(long bln, int mark)
{
  ...
  while (pa) {
    ...
    case  1:      /* 前溢出 */
      if (mark == 0) {
        im_output_debug_info("%s(%d) : [WAT] [前溢出] 在进入此行代码处的防护语句块前，侦测到%s中第%d行代码分配的内存发生了前写溢出！请核查。\n", _short_filename(fn), fln, _short_filename(pa->filename), pa->linenum);
      }
      else {
        im_output_debug_info("%s(%d) : [WAT】 [前溢出] 此行代码分配的内存发生了前写溢出！引发溢出的代码在%s第%d～%d行之间，请仔细核查。\n", _short_filename(pa->filename), pa->linenum, _short_filename(fn), fln, bln);
      }
    ...
    case  2:      /* 后溢出 */
      if (mark == 0) {
        im_output_debug_info("%s(%d) : [WAT] [后溢出] 在进入此行代码处的防护语句块前，侦测到%s中第%d行代码分配的内存发生了后写溢出！请核查。\n", _short_filename(fn), fln, _short_filename(pa->filename), pa->linenum);
      }
      else {
        im_output_debug_info("%s(%d) : [WAT] [后溢出] 此行代码分配的内存发生了后写溢出！引发溢出的代码在%s第%d～%d行之间，请仔细核查。\n", _short_filename(pa->filename), pa->linenum, _short_filename(fn), fln, bln);
      }
    ...
    case  3:      /* 前、后均溢出 */
      if (mark == 0) {
        im_output_debug_info("%s(%d) : [WAT] [前、后溢出] 在进入此行代码处的防护语句块前，侦测到%s中第%d行代码分配的内存发生了前、后写溢出！请核查。\n", _short_filename(fn), fln, _short_filename(pa->filename), pa->linenum);
      }
      else {
        im_output_debug_info("%s(%d) : [WAT] [前、后溢出] 此行代码分配的内存发生了前、后写溢出！引发溢出的代码在%s第%d～%d行之间，请仔细核查。\n", _short_filename(pa->filename), pa->linenum, _short_filename(fn), fln, bln);
      }
    ...
    default:
      if (mark == 0) {
        im_output_debug_info("%s(%d) : [WAT] [操作系统异常] 在进入此行代码处的防护语句块前，察觉操作系统似乎处于不稳定状态，建议退出程序！\n", _short_filename(fn), fln);
      }
      else {
        im_output_debug_info("%s(%d) : [WAT] [操作系统异常] 操作系统处于不稳定状态，建议退出程序！（检测点位于%s第%d～%d行）\n", _short_filename(fn), fln, _short_filename(fn), fln, bln);
      }
