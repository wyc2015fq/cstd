...
bool CxImagePCX::Decode(CxFile* hFile)
{
  ...
  cx_try {
    ...
    if (pcxHeader.Encoding != 1) {
      cx_throw("PCX file has unknown encoding scheme");
    }
    ...
    lpHead1 = pcximage = (BYTE*)pmalloc(nbytes);
    ...
    for (DWORD idx = 0; idx < head.biClrUsed; idx++) {
      SetPaletteColor((BYTE)idx, ColorMap[idx][0], ColorMap[idx][1], ColorMap[idx][2]);
    }

    lpHead2 = pcxpixels = (BYTE*)pmalloc(Width + pcxHeader.BytesPerLine * 8);
...
bool CxImagePCX::Encode(CxFile* hFile)
{
  ...
  cx_try {
    ...
    if (head.biClrUsed == 0) {
      ...
      BYTE* plane = (BYTE*)pmalloc(pcxHeader.BytesPerLine);
      BYTE* raw = (BYTE*)pmalloc(head.biWidth);

      for (y = head.biHeight - 1; y >= 0 ; y--) {

        for (x = 0; x < head.biWidth; x++) {
          raw[x] = (BYTE)GetPixelIndex(x, y);
        }
