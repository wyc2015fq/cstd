...
/* 读一个矩形 */
static int CALLAGREEMENT _read_pictrect(ISFILE* pfile, LPPCT_RECT prc)
{
  PCT_RECT rc;
...
/* 跨过PCT图象首部的无用信息区 */
static enum EXERESULT CALLAGREEMENT _read_header(ISFILE* pfile, int* pversion)
{
  PCT_MAGIC   magic;
  ...
  __try {
    __try {
      ...
      while (mark == 0) {
        ...
        case PCTDEF_DIRBITSRGN:
          ...
          if ((code != PCTDEF_DIRBITSRT) && (code != PCTDEF_DIRBITSRGN)) {
            ...
            bytes_per_line = (unsigned short)stmp;
          }
          else {
          ...
          if ((code != PCTDEF_DIRBITSRT) && (code != PCTDEF_DIRBITSRGN)) {
            ...
            pRgb = (RGBQUAD*)pinfo_str->palette;
      ...
      if (pinfo_str->data_state == 0) {
        ...
        pinfo_str->width  = (unsigned long)(frame.right - frame.left);
        pinfo_str->height = (unsigned long)(frame.bottom - frame.top);
        pinfo_str->order  = 0;
      ...
      /* 申请解压缓冲区 */
      if ((pbuf = (unsigned char*)isirw_malloc(linesize * 2 + 4)) == 0) {
        b_status = ER_MEMORYERR;
        __leave;
      }

      if ((pdec = (unsigned char*)isirw_malloc(linesize + 4)) == 0) {
        b_status = ER_MEMORYERR;
        __leave;
      }
      ...
      for (i = 0; i < pinfo_str->height; i++) {
        ...
        if (bytes_per_line < 8) {
          ...
          /* 合成象素到数据包 */
          {
            ...
            unsigned char* p = (unsigned char*)pinfo_str->pp_line_addr[i];
