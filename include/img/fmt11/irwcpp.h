/********************************************************************
  ...
  或发信到：
      ...
      isee##vip.163.com
  ----------------------------------------------------------------
  本文件用途：  ISee图像浏览器—图象读写引擎C++语言版接口定义文件
...
typedef struct _tagImageInfoStr {
  ...
  unsigned long order;                /* 放置方式（正向或倒向）*/
...
#ifdef  _AFXDLL
#define CLASSEXTTYPE  __declspec(dllexport)
#else
...
public:
  ...
  unsigned long IP_GetPixel(int x, int y);
  unsigned long IP_SetPixel(int x, int y, unsigned long newpix);
  int       CloseIPMode();
...
protected:
  ...
  int       _AllocSpecPack(LPSPEC_DATAPACK* ppPack, LPBITMAPINFO pbmi, int cnvmark, int sn, char* pbits, int bitmark);
  int       _SaveSpecPack(ISEEIO_CONFER_TYPE type, char* name, unsigned long par1, unsigned long par2, CSAVETYPE cnvmark, LPSAVESTR par);
  LPSUBIMGBLOCK _CalSubImageAddr(LPINFOSTR pinfo, int num);
...
#define CNVTYPE_CHECK(cnv)  ((void*)0)
  ...
  // 内部数据
private:
  unsigned long   m_uMagic;           // 类标签-CSEEIMAGE_MAGIC
