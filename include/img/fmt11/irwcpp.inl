/********************************************************************
  ...
  或发信到：
      ...
      isee##vip.163.com
  ----------------------------------------------------------------
  本文件用途：  ISee图像浏览器—图象读写引擎C++语言版接口实现文件
...
int CSeeImage::Load(CSITYPER type, ...)
{
  ...
  case  CR_MEMORY: {                // 本地内存类型
    ...
    // 向引擎发送读取命令
    if (isirw_s_read_img_c(ISEEIO_CONFER_LOC_MEMORY, mname, (unsigned long)addr, len, (int)type, (IRWE_CNVMARK)cnvmark, NULL, &m_pPack) == ISIRW_LASTERR_SUCCESS) {
      assert(m_pPack != NULL);
  ...
  case  CR_SEEIMAGE: {              // 另一个CSeeImage对象
    ...
    bmi.bmi.biBitCount = (unsigned short)iis.bitcount;
    bmi.bmi.biPlanes   = 1;
    ...
    memcpy((void*)bmi.pal, (const void*)iis.palette, sizeof(RGBQUAD)*iis.pal_count);

    // 申请并填写专业包
    eer = (enum EXERESULT)_AllocSpecPack(&m_pPack, (LPBITMAPINFO)&bmi, cnvmark, (int)type, iis.pbits, 1);
  ...
  case  CR_NIHILITY: {              // 虚无（即创建）
    ...
    switch (bitcount) {
    case  1:
      *(unsigned long*)&(bmi.pal[0]) = 0;
      *(unsigned long*)&(bmi.pal[1]) = 0xffffff;
      break;

    case  4:
      for (i = 0; i < 16; i++) {
        *(unsigned long*)&(bmi.pal[i]) = ((i * 17) << 16) | ((i * 17) << 8) | (i * 17);
      }
    ...
    case  8:
      for (i = 0; i < 256; i++) {
        *(unsigned long*)&(bmi.pal[i]) = (i << 16) | (i << 8) | (i);
      }
    ...
    // 申请并填写专业包
    eer = (enum EXERESULT)_AllocSpecPack(&m_pPack, (LPBITMAPINFO)&bmi, cnvmark, (int)type, NULL, 1);
  ...
  case  CR_CLIP: {                // 剪贴板
    ...
    if (::OpenClipboard(NULL)) {
      ...
      pbmi  = (LPBITMAPINFOHEADER)lpDIB;
      pbits = (char*)(pbmi + 1);

      // 申请并填写专业包
      eer = (enum EXERESULT)_AllocSpecPack(&m_pPack, (LPBITMAPINFO)pbmi, cnvmark, (int)type, pbits, 0);
  ...
  case  CR_WINDOW: {                // 窗口
    ...
    if ((m_pPack = isirw_alloc_SPEC_DATAPACK(ISEEIO_CONFER_LOC_FILE, CSINIHFILENAME, 0, 0, (IRWE_CNVMARK)cnvmark)) != NULL) {
      ...
      unsigned long linesize = DIBSCANLINE_DWORDALIGN(width * bitcount);
      unsigned long palcnt;
      ...
      // 申请象素缓冲区
      if ((pinfo->p_bit_data = (unsigned char*)isirw_malloc((int)linesize * height)) == NULL) {
        isirw_free_SPEC_DATAPACK(m_pPack);
        m_pPack = psOldPack;
        return ER_MEMORYERR;
      }
      ...
      bih.bmi.biSize      = sizeof(BITMAPINFOHEADER);
      ...
      pinfo->width    = (unsigned long)width;
      pinfo->height   = (unsigned long)height;
      pinfo->bitcount = (unsigned long)bitcount;
  ...
  case  CR_RESOURCE_ID: {             // 资源（以ID方式给出）
    ...
    pmi = (LPBITMAPINFO)pdib;

    if ((pmi->bmiHeader.biSize != sizeof(BITMAPINFOHEADER)) ||
    ...
    if (pmi->bmiHeader.biCompression == BI_RGB) {
      // 申请并填写专业包
      eer = (enum EXERESULT)_AllocSpecPack(&m_pPack, (LPBITMAPINFO)pdib, cnvmark, (int)type, (char*)(pdib + sizeof(BITMAPINFOHEADER)), 0);
    }
    else {
      int   imgsize = 0;
      int   bmpoff  = 0;
      ...
      if (pmi->bmiHeader.biBitCount <= 8) {
        if (pmi->bmiHeader.biClrUsed == 0) {
          ...
          case  1:
            imgsize += 2 * sizeof(RGBQUAD);
            break;

          case  2:
            imgsize += 4 * sizeof(RGBQUAD);
            break;

          case  4:
            imgsize += 16 * sizeof(RGBQUAD);
            break;

          case  8:
            imgsize += (unsigned int)256 * sizeof(RGBQUAD);
            break;
          ...
          imgsize += pmi->bmiHeader.biClrUsed * sizeof(RGBQUAD);
        }
      }
      else {
        if (pmi->bmiHeader.biCompression == BI_BITFIELDS) {
          imgsize += 3 * sizeof(unsigned long);
        }
      }
      ...
      // 制作内存BMP文件
      pfi->bfType = ((unsigned short)('M' << 8) | 'B');
      pfi->bfSize = imgsize;
      ...
      // 向引擎发送读取命令
      if (isirw_s_read_img_c(ISEEIO_CONFER_LOC_MEMORY, "a.bmp", (unsigned long)pbmp, imgsize, (int)type, (IRWE_CNVMARK)cnvmark, NULL, &m_pPack) == ISIRW_LASTERR_SUCCESS) {
        assert(m_pPack != NULL);
  ...
  case  CR_HBITMAP: {               // HBITMAP句柄
    ...
    // 设置DIB信息块内容
    binfo.bmi.biSize   = sizeof(BITMAPINFOHEADER);
    ...
    if ((m_pPack = isirw_alloc_SPEC_DATAPACK(ISEEIO_CONFER_LOC_FILE, CSINIHFILENAME, 0, 0, (IRWE_CNVMARK)cnvmark)) != NULL) {
      ...
      if (!::GetDIBits(hDC, hBmp, 0, height, (LPVOID)pinfo->p_bit_data, (LPBITMAPINFO)&binfo, DIB_RGB_COLORS)) {
        ::ReleaseDC(hWnd, hDC);
        isirw_free_SPEC_DATAPACK(m_pPack);
        m_pPack = psOldPack;
        return ER_BADIMAGE;
      ...
      case  1:
        ...
        for (i = 0; i < (int)palcnt; i++) {
          pinfo->palette[i] = *(unsigned long*) & (binfo.pal[i]);
        }
  ...
  case  CR_HDIB: {                // HDIB句柄
    ...
    pbmi  = (LPBITMAPINFO)(char*)hDib;
    pbits = (char*)(((char*)pbmi) + sizeof(BITMAPINFOHEADER));
  ...
  if (pimginfo == NULL) {
    ...
    if (flag == CCURIMGNUM) {           // 当前图象的序号（0基）
      ...
      if (m_pPack->lab1 == 0) {
        ...
        // 获取当前图象结构的地址
        LPSUBIMGBLOCK psub = _CalSubImageAddr(pinfo, m_pPack->lab1);
    ...
    memset((void*)(((char*)pimginfo) + sizeof(unsigned long)), 0, sizeof(IMAGEINFOSTR) - sizeof(unsigned long));
    ...
    if (m_pPack->lab1 == 0) {
      ...
      if (flag & CPALETTE) {          // 获取调色板数据
        memcpy((void*)pimginfo->palette, (const void*)pinfo->palette, sizeof(unsigned long)*pinfo->pal_count);
      }
      ...
      if (flag & CPALETTE) {          // 获取调色板数据
        memcpy((void*)pimginfo->palette, (const void*)psub->palette, sizeof(unsigned long)*psub->pal_count);
      }
...
int CSeeImage::Save(CSITYPEW type, ...)
{
  ...
  case  CW_MEMORY: {                // 本地内存类型
    ...
    // 保存图象
    result = _SaveSpecPack((ISEEIO_CONFER_TYPE)type, pname, (unsigned long)0, (unsigned long)0, bitcount, &savestr);
  ...
  case  CW_CLIP: {                // 剪贴板
    ...
    case  4:
    case  8:
      colssize = (1 << pinfo->bitcount) * sizeof(RGBQUAD);
      bpp      = 1;
      break;
    ...
    LPBITMAPINFO  pbmi = (LPBITMAPINFO)pda;
    ...
    if (pinfo->pal_count > 0) {
      memcpy((void*)pbmi->bmiColors, (const void*)pinfo->palette, sizeof(RGBQUAD)*pinfo->pal_count);
    }
  ...
  case  CW_HBITMAP: {               // HBITMAP句柄
    ...
    memcpy((void*)&binfo.bmi, (const void*)&bih, sizeof(BITMAPINFOHEADER));

    // 拷贝调色板数据
    if (pinfo->pal_count > 0) {
      memcpy((void*)binfo.pal, (const void*)pinfo->palette, sizeof(RGBQUAD)*pinfo->pal_count);
    }
  ...
  case  CW_HDIB: {                // HDIB句柄
    ...
    LPBITMAPINFO  pbmi = (LPBITMAPINFO)pimg;
  ...
  case  CD_NON: {                 // 正常绘制
    ...
    if (m_hDrawDib && (pinfo->order == 1)) {
      DrawDibDraw(m_hDrawDib, hdc,
          ...
          (LPBITMAPINFOHEADER)&m_cDispInfo,
      ...
      StretchDIBits(hdc,
          ...
          pinfo->p_bit_data, (LPBITMAPINFO)&m_cDispInfo, DIB_RGB_COLORS, SRCCOPY);
    }
  }
  break;
...
// 获取指定位置的象素值
unsigned long CSeeImage::IP_GetPixel(int x, int y)
{
  // 引擎必须已经开启，并且必须已处于IP模式
  ...
  return ((unsigned long*)(m_pPack->cnv_img->pp_line_addr[y]))[x];
}


// 设置指定位置的象素值
unsigned long CSeeImage::IP_SetPixel(int x, int y, unsigned long newpix)
{
  // 引擎必须已经开启，并且必须已处于IP模式
  ...
  unsigned long* pl = (unsigned long*)(m_pPack->cnv_img->pp_line_addr[y]);
  unsigned long old = pl[x];
...
char* CSeeImage::_LoadDIBFromPE(char* fname, int id)
{
  ...
  try {
    ...
    if (ptResource != 0) {
      ...
      if (!enumid) {              // 正常ID
        for (i = 0; i < nResource; i++) {
          if (stResDirEntry1[i].Id == (unsigned short)id) {
            break;
          }
        }
      }
  ...
  if (hSrc) {
    ...
    if (hResData) {
      ...
      if (lpResData) {
        ...
        if (*((DWORD*)lpResData) == sizeof(BITMAPINFOHEADER)) {
          pbuf = (char*)lpResData;
        }
      }
    }
...
//
int CSeeImage::_AllocSpecPack(LPSPEC_DATAPACK* ppPack, LPBITMAPINFO pbmi, int cnvmark, int sn, char* pbits, int bitmark)
{
  ...
  if ((pPack = isirw_alloc_SPEC_DATAPACK(ISEEIO_CONFER_LOC_FILE, CSINIHFILENAME, 0, 0, (IRWE_CNVMARK)cnvmark)) != NULL) {
    ...
    case  1:
      ...
      if (bitmark == 0) {
        pbits += palcnt * sizeof(RGBQUAD);
      }

      for (i = 0; i < (int)palcnt; i++) {
        pinfo->palette[i] = *(unsigned long*) & (pbmi->bmiColors[i]);
      }
...
//
int CSeeImage::_SaveSpecPack(ISEEIO_CONFER_TYPE type, char* name, unsigned long par1, unsigned long par2, CSAVETYPE cnvmark, LPSAVESTR par)
{
  assert(name && par);
...
//  子图象结构地址，失败返回NULL
//
LPSUBIMGBLOCK CSeeImage::_CalSubImageAddr(LPINFOSTR pinfo, int num)
{
  // 图象序号不能为0，0是主图象
  assert(pinfo && num);

  LPSUBIMGBLOCK psub = pinfo->psubimg;
...
void CSeeImage::_UpdateDispInfo()
{
  ...
  // 填写新图象的尺寸、显示等信息
  m_cDispInfo.bmi.biSize          = sizeof(BITMAPINFOHEADER);
  ...
  memcpy((void*)m_cDispInfo.pal, (const void*)pinfo->palette, sizeof(RGBQUAD)*pinfo->pal_count);
}
