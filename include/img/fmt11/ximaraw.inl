...
bool CxImageRAW::Decode(CxFile* hFile)
{
  ...
  cx_try {
    ...
    if (dcr.meta_length) {
      dcr.meta_data = (char*) pmalloc(dcr.meta_length);
    ...
    if (dcr.opt.user_qual >= 0) {
      dcr.quality = dcr.opt.user_qual;
    }

    if (dcr.opt.user_black >= 0) {
      dcr.black = dcr.opt.user_black;
    }
    ...
    if (dcr.is_foveon && !dcr.opt.document_mode) {
      dcr_foveon_interpolate(&dcr);
    }

#endif

    if (!dcr.is_foveon && dcr.opt.document_mode < 2) {
      dcr_scale_colors(&dcr);
    }
    ...
    if (!dcr.is_foveon && dcr.colors == 3) {
      dcr_median_filter(&dcr);
    }

    if (!dcr.is_foveon && dcr.opt.highlight == 2) {
      dcr_blend_highlights(&dcr);
    }

    if (!dcr.is_foveon && dcr.opt.highlight > 2) {
      dcr_recover_highlights(&dcr);
    }
