#ifndef _BLUR_INL_
#define _BLUR_INL_
int16u const g_stack_blur8_mul[255] = {
...
int8u const g_stack_blur8_shr[255] = {
...
public:
  ...
  //--------------------------------------------------------------------
  template<class Img> void blur_x(img_t* img, unsigned radius) {
    if (radius < 1) {
      return;
    }
  ...
  //--------------------------------------------------------------------
  template<class Img> void blur_y(img_t* img, unsigned radius) {
    pixfmt_transposer<Img> img2(img);
    blur_x(img2, radius);
  }
  //--------------------------------------------------------------------
  template<class Img> void blur(img_t* img, unsigned radius) {
    blur_x(img, radius);
    pixfmt_transposer<Img> img2(img);
    blur_x(img2, radius);
  }
...
#endif
int stack_blur_cn1(const img_t* src, img_t* dst, unsigned rx, unsigned ry)
{
  enum {pix_step = 1};
...
#define pix_ptr(im, x, y) (im->tt.data + im->s*(y) + (x)*im->c)
      src_pix_ptr = pix_ptr(src, 0, y);
      pix = *src_pix_ptr;
      ...
      src_pix_ptr = pix_ptr(src, xp, y);
      dst_pix_ptr = pix_ptr(dst, 0, y);

      for (x = 0; x < w; x++) {
        *dst_pix_ptr = (sum * mul_sum) >> shr_sum;
        dst_pix_ptr += pix_step;
        sum -= sum_out;
  ...
  if (ry > 0) {
    ...
    for (x = 0; x < w; x++) {
      ...
      src_pix_ptr = pix_ptr(src, x, yp);
      dst_pix_ptr = pix_ptr(dst, x, 0);
...
//========================================================stack_blur_rgb24
int stack_blur_cn3(img_t* img, unsigned rx, unsigned ry)
{
  ...
  if (ry > 0) {
    ...
    for (x = 0; x < w; x++) {
      ...
      src_pix_ptr = pix_ptr(img, x, yp);
      dst_pix_ptr = pix_ptr(img, x, 0);
...
int stack_blur_cn4(img_t* img, unsigned rx, unsigned ry)
{
  typedef Color32 color_type;
  enum {R = 0, G = 1, B = 2, A = 3};
  unsigned x, y, xp, yp, i;
...
public:
  ...
  //--------------------------------------------------------------------
  template<class Img> void blur_x(img_t* img, double radius) {
    if (radius < 0.62) {
      return;
    }
  ...
  //--------------------------------------------------------------------
  template<class Img> void blur_y(img_t* img, double radius) {
    pixfmt_transposer<Img> img2(img);
    blur_x(img2, radius);
  }
  //--------------------------------------------------------------------
  template<class Img> void blur(img_t* img, double radius) {
    blur_x(img, radius);
    pixfmt_transposer<Img> img2(img);
    blur_x(img2, radius);
  }
...
double Sigmoid(double x, double alpha, double beta, double gamma)
{
  if (x <= alpha) {
    return 0;
  }
...
void SigmoidBlur(img_t* pDst, const img_t* pSrc, int boxw)
{
  int width = pSrc->w;
  int height = pSrc->h;
  ...
  // access variables
  int sline = pSrc->s / 4;
  UINT32* src = (UINT32*)pSrc->tt.data;
  // lock the dst bits
  int dline = pDst->s / 4;
  UINT32* dst = (UINT32*)pDst->tt.data;
  double a, r, g, b, weight, weightsum;
  UINT32 ia, ir, ig, ib, c;
  double* weights = MALLOC(double, 2 * boxw + 1);
  int i, row, col;
  weights[boxw] = 1.0f;

  for (i = 0; i < boxw; i++) {
    double y = Sigmoid((double)i, 0.0f, (double)((boxw + 1) / 2), (double)(boxw + 1));
    weights[i] = y;
    weights[boxw * 2 - i] = y;
  }
  ...
  for (row = 0; row < height; row++) {
    for (col = 0; col < width; col++) {
      ...
      dst[col + row * dline] = ((ia << 24) | (ir << 16) | (ig << 8) | (ib << 0));
    }
  }
...
int FastBlur_cn4(const img_t* pSrc, img_t* pDst, int boxw)
{
  ...
  // lock the dst bits
  UINT32* dst = (UINT32*)pDst->tt.data;
  double a, r, g, b, weightsum;
  UINT32 ia, ir, ig, ib, c, alpha;
  int row, col;
  imsetsamesize(pDst, pSrc);
  // these bitmpas need to be the same size
