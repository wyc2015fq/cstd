...
Xi= 128*(Xi1>=Xi)+64*(Xi2>=Xi)+32*(Xi3>=Xi)+16*(Xi4>=Xi)+8*(Xi5>=Xi)+4*(Xi6>=Xi)+2*(Xi7>=Xi)+1*(Xi8>=Xi);
X = Xi(2:sy+1,2:sx+1);
%X=Xi(1:sy,1:sx);
hst = X;
    ...
    for i = 1:sx
        for j = 1:sy
            result(i,j) =mapping3(X(i,j)+1);
        end
    end
hst= result;

%hst=sum(hist(X,0:255)');
%hst=(imhist(uint8(X),8))';
