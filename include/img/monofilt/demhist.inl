...
void update_brightcont( int arg )
{
    ...
    for( i = 0; i < hist_size; i++ )
        cvRectangle( hist_image, iPOINT(i*bin_w, hist_image->height),
                     iPOINT((i+1)*bin_w, hist_image->height - cvRound(cvGetReal1D(hist->bins,i))),
                     cvScalarAll(0), -1, 8, 0 );
...
int test_demhist()
{
    ...
    hist = cvCreateHist(1, &hist_size, CC_HIST_ARRAY, ranges, 1);
    lut_mat = cvCreateMatHeader( 1, 256, CC_8UC1 );
