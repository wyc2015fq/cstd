...
#define CODE_FIRST  258   /* first pfree code entry */
...
typedef struct {
  ...
  unsigned short  lzw_free_ent; /* next pfree entry in hash table */
  ...
  code_t* dec_free_entp;    /* next pfree entry */
...
static int LZWEncode_impl(LZWCodecState* sp, const uchar* bp, int32 cc, uchar* op)
{
  ...
  while (cc > 0) {
     ...
     * are at least 4 bytes pfree--room for 2 codes.
     */
    if (op > limit) {
...
static int LZWEncode(const uchar* bp, int32 cc, uchar* op)
{
  ...
  int n = 0;
  assert(sp != NULL);
  sp->enc_hashtab = (hash_t*) pmalloc(HSIZE * sizeof(hash_t));
...
static int LZWDecode(uchar* op0, int32 occ0, const uchar* bp, int sp_rawcc)
{
  ...
  if (sp->dec_codetab == NULL) {
    sp->dec_codetab = (code_t*)pmalloc(CSIZE * sizeof(code_t));
