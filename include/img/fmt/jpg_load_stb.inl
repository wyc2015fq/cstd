...
static int stbi__jpeg_decode_block(stbi__jpeg* j, short data[64], stbi__huffman* hdc, stbi__huffman* hac, int16* fac, int b, uint8* dequant)
{
  int diff, dc, k;
  int t;
...
static int load_jpeg_image(stbi__jpeg* z, img_t* im, int req_comp)
{
  int n, decode_n;
  ...
  // resample and color-convert
  {
    ...
    imsetsize(im, z->s->img_y, z->s->img_x, n, 1);
    output = im->data;
...
{
  return (int) stream_read((stream_t*) user, data, size);
}

static void stbi__stdio_skip(void* user, int n)
{
  stream_seek((stream_t*) user, n, SEEK_CUR);
}

static int stbi__stdio_eof(void* user)
{
  return stream_eof((stream_t*) user);
}
...
static int jpg_load(stream_t* f, int req_comp, img_t* im)
{
  stbi__context s = {0};
  stbi__jpeg j = {0};
  int comp;
...
static int jpg_load_file_close(FILE* pf, int cn_req, img_t* im)
{
  int ret = 0;
