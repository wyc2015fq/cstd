...
void CTrueColorToolBar::SetColumns (UINT nColumns)
{
	int		nCount = this->GetToolBarCtrl ().GetButtonCount () ;
	int		nSep = 0 ; // number of Separator
	for (int i=0 ; i < nCount ; i++)
	{
		UINT	nStyle = this->GetButtonStyle (i) ;
		if (i == nCount - 1) // the last one
			nStyle &= ~TBBS_WRAPPED ;
		else
			...
			{
				BOOL	bWrap = (((i + 1 + nSep) % nColumns) == 0) ;
				if (bWrap)
				{
					if (!(this->GetButtonStyle (i+1) & TBBS_SEPARATOR))
						nStyle |= TBBS_WRAPPED ;
				}
				else
					nStyle &= ~TBBS_WRAPPED ;
			}
		this->SetButtonStyle (i, nStyle) ;
	}
	this->Invalidate () ;
