...
static const float icvAtanTab[ 8 ] = {
  0.f + _CC_ATAN_CF2, 90.f - _CC_ATAN_CF2,
...
CC_IMPL float cvFastArctan(float y, float x)
{
...
int icvFastArctan_32f(const float* y, const float* x, float* angle, int len)
{
  int i = 0;
...
IPCVAPI_IMPL(int, icvInvSqrt_32f, (const float* src, float* dst, int len), (src, dst, len))
{
  int i = 0;
...
IPCVAPI_IMPL(int, icvSqrt_32f, (const float* src, float* dst, int len), (src, dst, len))
{
  int i = 0;
...
IPCVAPI_IMPL(int, icvSqrt_64f, (const double* src, double* dst, int len), (src, dst, len))
{
  int i = 0;
...
IPCVAPI_IMPL(int, icvInvSqrt_64f, (const double* src, double* dst, int len), (src, dst, len))
{
  int i = 0;
    ...
    int angle_in_degrees)
{
  ...
  if (mag) {
    CC_CALL(mag = cvGetMat(mag, &magstub, &coi3));
  ...
  if (angle) {
    CC_CALL(angle = cvGetMat(angle, &anglestub, &coi4));
  ...
  size.width *= CC_MAT_CN(xmat->type);

  if (CC_IS_MAT_CONT(xmat->type & ymat->type & cont_flag)) {
  ...
  if (depth == CC_64F && angle) {
    x_buffer = (float*) cvStackAlloc(block_size * sizeof(float));
    y_buffer = (float*) cvStackAlloc(block_size * sizeof(float));
  }
  else if (depth == CC_32F && mag) {
    mag_buffer = (float*) cvStackAlloc(block_size * sizeof(float));
  }

  if (depth == CC_32F) {
    for (y = 0; y < size.height; y++) {
      float* x_data = (float*)(xmat->data.ptr + xmat->s * y);
      float* y_data = (float*)(ymat->data.ptr + ymat->s * y);
      float* mag_data = mag ? (float*)(mag->data.ptr + mag->s * y) : 0;
      float* angle_data = angle ? (float*)(angle->data.ptr + angle->s * y) : 0;
    ...
    for (y = 0; y < size.height; y++) {
      double* x_data = (double*)(xmat->data.ptr + xmat->s * y);
      double* y_data = (double*)(ymat->data.ptr + ymat->s * y);
      double* mag_data = mag ? (double*)(mag->data.ptr + mag->s * y) : 0;
      double* angle_data = angle ? (double*)(angle->data.ptr + angle->s * y) : 0;
    ...
    int len, int angle_in_degrees)
{
  ...
  const double k2 = (2 * CC_PI) / N;

  const double sin_a0 = -0.166630293345647 * k2 * k2 * k2;
  const double sin_a2 = k2;

  const double cos_a0 = -0.499818138450326 * k2 * k2;
    ...
    CvArr* xarr, CvArr* yarr, int angle_in_degrees)
{
  ...
  if (xmat) {
    if (!CC_IS_MAT(xmat)) {
      CC_CALL(xmat = cvGetMat(xmat, &xstub, &coi1));
    }
  ...
  if (ymat) {
    if (!CC_IS_MAT(ymat)) {
      CC_CALL(ymat = cvGetMat(ymat, &ystub, &coi2));
    }
  ...
  size.width *= CC_MAT_CN(angle->type);

  if (CC_IS_MAT_CONT(cont_flag)) {
    size.width *= size.height;
    size.height = 1;
  }
  ...
  if (depth == CC_32F) {
    for (y = 0; y < size.height; y++) {
      float* x_data = (float*)(xmat ? xmat->data.ptr + xmat->s * y : 0);
      float* y_data = (float*)(ymat ? ymat->data.ptr + ymat->s * y : 0);
      float* mag_data = (float*)(mag ? mag->data.ptr + mag->s * y : 0);
      float* angle_data = (float*)(angle->data.ptr + angle->s * y);
    ...
    for (y = 0; y < size.height; y++) {
      double* x_data = (double*)(xmat ? xmat->data.ptr + xmat->s * y : 0);
      double* y_data = (double*)(ymat ? ymat->data.ptr + ymat->s * y : 0);
      double* mag_data = (double*)(mag ? mag->data.ptr + mag->s * y : 0);
      double* angle_data = (double*)(angle->data.ptr + angle->s * y);
...
IPCVAPI_IMPL(int, icvExp_32f, (const float* x, float* y, int n), (x, y, n))
{
  static const double
...
IPCVAPI_IMPL(int, icvExp_64f, (const double* x, double* y, int n), (x, y, n))
{
  static const double
...
#if 0
CC_IMPL void cvExp(const CvArr* srcarr, CvArr* dstarr)
{
  CC_FUNCNAME("cvExp");
  ...
  if (!CC_IS_MAT(src)) {
    CC_CALL(src = cvGetMat(src, &srcstub, &coi1));
  }

  if (!CC_IS_MAT(dst)) {
    CC_CALL(dst = cvGetMat(dst, &dststub, &coi2));
  }
  ...
  src_depth = CC_MAT_DEPTH(src->type);
  dst_depth = CC_MAT_DEPTH(dst->type);

  if (!CC_ARE_CNS_EQ(src, dst) || src_depth < CC_32F || dst_depth < src_depth) {
    CC_ERROR_FROM_CODE(CC_StsUnmatchedFormats);
  }
  ...
  size.width *= CC_MAT_CN(src->type);

  if (CC_IS_MAT_CONT(src->type & dst->type)) {
    size.width *= size.height;
    size.height = 1;
  }
  ...
  for (y = 0; y < size.height; y++) {
    unsigned char* src_data = src->data.ptr + src->s * y;
    unsigned char* dst_data = dst->data.ptr + dst->s * y;
...
IPCVAPI_IMPL(int, icvLog_32f, (const float* x, float* y, int n), (x, y, n))
{
...
IPCVAPI_IMPL(int, icvLog_64f, (const double* x, double* y, int n), (x, y, n))
{
  static const double shift[] = {
    0, -1. / 512
  };
...
#if 0
CC_IMPL void cvLog(const CvArr* srcarr, CvArr* dstarr)
{
  CC_FUNCNAME("cvLog");
  ...
  if (!CC_ARE_CNS_EQ(src, dst) || dst_depth < CC_32F || src_depth < dst_depth) {
    CC_ERROR_FROM_CODE(CC_StsUnmatchedFormats);
  }
...
ICC_DEF_IPOW_OP(8u, unsigned char, int, CC_CAST_8U)
...
typedef int (* CvIPowFunc)(const void* src, void* dst, int len, int power);
typedef int (* CvSqrtFunc)(const void* src, void* dst, int len);

CC_IMPL void cvPow(const CvArr* srcarr, CvArr* dstarr, double power)
{
  ...
  if (is_ipower) {
    ...
    CvSqrtFunc sqrt_func = power < 0 ?
        (depth == CC_32F ? (CvSqrtFunc) icvInvSqrt_32f : (CvSqrtFunc) icvInvSqrt_64f) :
          (depth == CC_32F ? (CvSqrtFunc) icvSqrt_32f : (CvSqrtFunc) icvSqrt_64f);
    ...
    block_size = MIN(size.width, ICC_MATH_BLOCK_SIZE);
    temp_buffer = cvStackAlloc(block_size * CC_ELEM_SIZE(depth));
...
IPCVAPI_IMPL(int, icvCheckArray_32f_C1R,
...
IPCVAPI_IMPL(int, icvCheckArray_64f_C1R,
...
CC_IMPL int cvCheckArr(const CvArr* arr, int flags,
    double minVal, double maxVal)
{
  ...
  if (arr) {
    ...
    type = CC_MAT_TYPE(mat->type);
    size = cvGetMatSize(mat);

    size.width *= CC_MAT_CN(type);

    if (CC_IS_MAT_CONT(mat->type)) {
      size.width *= size.height;
      size.height = 1;
    }

    if (CC_MAT_DEPTH(type) == CC_32F) {
      status = icvCheckArray_32f_C1R(mat->data.fl, mat->s, size,
          flags, minVal, maxVal);
    }
    else if (CC_MAT_DEPTH(type) == CC_64F) {
      status = icvCheckArray_64f_C1R(mat->data.db, mat->s, size,
          flags, minVal, maxVal);
    }
    else {
      CC_ERROR(CC_StsUnsupportedFormat, "");
