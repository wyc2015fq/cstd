...
#define CC_EXPORTS
#define cvScalarAll(a) cvScalar( (a), (a), (a), (a) )

typedef unsigned char uchar;

typedef IPOINT CvPoint;
typedef XSCALAR CvScalar;
...
class CC_EXPORTS CvBaseImageFilter {
public:
  CvBaseImageFilter();
  /* calls init() */
  CvBaseImageFilter( int _max_width, int _src_type, int _dst_type,
                     ...
                     int _border_mode = IMG_BORDER_REPLICATE,
  ...
  /* initializes the class for processing an image of maximal width _max_width,
     ...
     to get interpolate pixel values outside the image _border_mode=IMG_BORDER_*** is used,
     _border_value specify the pixel value in case of IMG_BORDER_CONSTANT border mode.
  ...
  */ 
  // virtual int process( const CvMat* _src, CvMat* _dst, CvRect _src_roi=cvRect(0,0,-1,-1),
...
protected:
  ...
  //virtual int fill_cyclic_buffer( const uchar* src, int src_step, int y, int y1, int y2 );
...
class CC_EXPORTS CvSepFilter : public CvBaseImageFilter {
public:
  CvSepFilter();
  ...
  virtual void clear();
  const CvMat* get_x_kernel() const {
    return kx;
  }
  const CvMat* get_y_kernel() const {
    return ky;
  }
  int get_x_kernel_flags() const {
    return kx_flags;
  }
  int get_y_kernel_flags() const {
    return ky_flags;
  }
...
class CC_EXPORTS CvLinearFilter : public CvBaseImageFilter {
public:
  CvLinearFilter();
  ...
  virtual void clear();
  const CvMat* get_kernel() const {
    return kernel;
  }
  uchar* get_kernel_sparse_buf() {
    return k_sparse;
  }
  int get_kernel_sparse_count() const {
...
class CC_EXPORTS CvBoxFilter : public CvBaseImageFilter {
public:
  CvBoxFilter();
...
class CC_EXPORTS CvLaplaceFilter : public CvSepFilter {
public:
  CvLaplaceFilter();
...
class CC_EXPORTS CvMorphology : public CvBaseImageFilter {
public:
  CvMorphology();
  CvMorphology( int _operation, int _max_width, int _src_dst_type,
                int _element_shape, uchar* _element,
  ...
  virtual void clear();
  //const uchar* get_element() const { return element; }
  int get_element_shape() const {
    return el_shape;
  }
  int get_operation() const {
    return operation;
  }
  uchar* get_element_sparse_buf() {
    return el_sparse;
  }
  int get_element_sparse_count() const {
  ...
  static void init_binary_element( uchar* _element, int _element_shape,
                                   CvPoint _anchor = cvPoint( -1, -1 ) );
protected:
