	...
	void getTransfer(img_t* im, int palette_size) {
      enum {max_palette_size = 10};
			uchar input_palette[max_palette_size*4];
      ASSERT(palette_size<max_palette_size);
      gridacc_kmeans(im->h, im->w, im->tt.data, im->s, im->c, 2, palette_size, input_palette);
#if 0
			int* IndexSortOrder = MALLOC(int, palette_size);
      ...
      for(var i=0;	i < palette_size; i++ ) {
        for(var j=i+1;	j < palette_size; j++ ) {
