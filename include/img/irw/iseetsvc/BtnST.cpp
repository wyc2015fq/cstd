...
//	ISee组注释：
//	---------------------------------------------------------------------------
//	本文件并非ISee组成员编写（原作者为Davide Calabro），所以版权并不是GPL，
//	你可以根据作者给出的版权信息将其用于其它软件。ISee组对下面的代码进行了适
//	量的改写，所以如果你想使用这个按钮类，建议你到作者的网站下载原始版本。因
//	改写而可能引发的BUG与原作者无关。
//
//	感谢Davide Calabro！
//	---------------------------------------------------------------------------
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, LPCRECT pRect)
{
	...
	else
	{
		if (m_bIsFocused)
		{
			...
			CBrush	cbobk(crColor);
			pDC->FillRect((LPCRECT)rc, &cbobk);

			rc.DeflateRect(2, 2, 0, 0);
			crColor = m_crColors[BTNST_COLOR_BK_FOCUS];
			CBrush	cbfbk(crColor);
			pDC->FillRect((LPCRECT)rc, &cbfbk);
