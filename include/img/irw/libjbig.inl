 ...
 * capability and your pmalloc, prealloc and pfree are not reentrant,
...
static void* checked_realloc(void* ptr, size_t size)
{
  ...
  */
  ...
  /* <<<<<<<<<<<<<<<<<<< modify end */
#if 0
  fprintf(stderr, "%p = prealloc(%p, %ld)\n", p, ptr, (long) size);
#endif
  return p;
}
...
ARITH_INL void arith_encode(struct jbg_arenc_state* s, int cx, int pix)
{
  ...
  extern uchar jbg_nmps[], jbg_nlps[];
...
static struct jbg_buf* jbg_buf_init(struct jbg_buf** free_list) {
  ...
  /* Test whether a block from the pfree list is available */
  if (*free_list) {
    new_block = *free_list;
    ...
    void* file)
{
  ...
  s->lhp[0] = p;
  s->lhp[1] = (uchar**)checked_malloc(planes * sizeof(uchar*));
  bufsize = ((jbg_ceil_half(x, 1) + 7) / 8) * jbg_ceil_half(y, 1);
  for (i = 0; i < planes; i++) {
    s->highres[i] = 0;
    s->lhp[1][i] = (uchar*)checked_malloc(sizeof(uchar) * bufsize);
  }
  s->free_list = NULL;
 ...
 * is returned.
 */
int jbg_enc_lrlmax(struct jbg_enc_state* s, ulong x,
    ulong y)
{
  for (s->d = 0; s->d < 6; s->d++)
...
{
  uchar* hp, *lp1, *lp2, *p0, *p1, *q1, *q2;
  ulong hl, ll, hx, hy, lx, ly, hbpl, lbpl;
  ulong line_h0 = 0, line_h1 = 0;
  ulong line_h2, line_h3, line_l1, line_l2, line_l3;
  struct jbg_arenc_state* se;
  ulong i, j, y;
  unsigned t;
  int ltp, ltp_old, cx;
  ulong c_all, c[MX_MAX + 1], cmin, cmax, clmin, clmax;
      ...
      for (j = 0; j < hx; lp1++, lp2++) {
        ...
        do {
          assert(hp - (s->lhp[s->highres[plane]][plane] +
              (stripe * hl + i) * hbpl) == (ptrdiff_t) j >> 3);
...
static void resolution_reduction(struct jbg_enc_state* s, int plane,
    int higher_layer)
{
  ulong hx, hy, lx, ly, hbpl, lbpl;
  uchar* hp1, *hp2, *hp3, *lp;
  ulong line_h1, line_h2, line_h3, line_l2;
  ulong i, j;
  int pix, k, l;
  /* number of pixels in highres image */
...
static void output_sde(struct jbg_enc_state* s,
    ulong stripe, int layer, int plane)
{
...
void jbg_enc_out(struct jbg_enc_state* s)
{
  ...
  */
  /* NEW */
  bih[4] = (uchar)(xd >> 24);
  bih[5] = (uchar)((xd >> 16) & 0xff);
  bih[6] = (uchar)((xd >> 8) & 0xff);
  bih[7] = (uchar)(xd & 0xff);
  bih[8] = (uchar)(yd >> 24);
  bih[9] = (uchar)((yd >> 16) & 0xff);
  bih[10] = (uchar)((yd >> 8) & 0xff);
  bih[11] = (uchar)(yd & 0xff);
  bih[12] = (uchar)(s->l0 >> 24);
  bih[13] = (uchar)((s->l0 >> 16) & 0xff);
  bih[14] = (uchar)((s->l0 >> 8) & 0xff);
  bih[15] = (uchar)(s->l0 & 0xff);
  /* <<<<<<<<<<<<<<<<<<< modify end */
  bih[16] = s->mx;
  bih[17] = s->my;
  bih[18] = s->order;
 ...
 */
static size_t decode_pscd(struct jbg_dec_state* s, uchar* data,
    size_t len)
{
  ulong stripe;
  unsigned int layer, plane;
  ulong hl, ll, y, hx, hy, lx, ly, hbpl, lbpl;
  uchar* hp, *lp1, *lp2, *p1, *q1;
  register ulong line_h1, line_h2, line_h3;
  register ulong line_l1, line_l2, line_l3;
    ...
    do {
      ...
      *cnt = 0;
      ...
      if (s->bie_len < 20) {
        ...
        if (s->dl == 0) {
          s->s = (struct jbg_ardec_state**)checked_malloc(s->planes * sizeof(struct jbg_ardec_state*));
          s->tx = (int**)checked_malloc(s->planes * sizeof(int*));
          ...
          s->lhp[0] = (uchar**)checked_malloc(s->planes * sizeof(uchar*));
          s->lhp[1] = (uchar**)checked_malloc(s->planes * sizeof(uchar*));
          for (i = 0; i < s->planes; i++) {
            s->s[i] = (struct jbg_ardec_state*)checked_malloc((s->d - s->dl + 1) *
            ...
            s->lhp[s->d & 1][i] = (uchar*)checked_malloc(sizeof(uchar) * hsize);
            s->lhp[(s->d - 1) & 1][i] = (uchar*)checked_malloc(sizeof(uchar) * lsize);
          }
        }
        else {
          for (i = 0; i < s->planes; i++) {
            s->s[i] = (struct jbg_ardec_state*)checked_realloc(s->s[i], (s->d - s->dl + 1) * sizeof(struct jbg_ardec_state));
            s->tx[i] = (int*)checked_realloc(s->tx[i], (s->d - s->dl + 1) * sizeof(int));
            s->ty[i] = (int*)checked_realloc(s->ty[i], (s->d - s->dl + 1) * sizeof(int));
            s->reset[i] = (int*)checked_realloc(s->reset[i],
                (s->d - s->dl + 1) * sizeof(int));
            s->lntp[i] = (int*)checked_realloc(s->lntp[i],
                (s->d - s->dl + 1) * sizeof(int));
            s->lhp[s->d & 1][i] = (uchar*)checked_realloc(s->lhp[s->d & 1][i],
                sizeof(uchar) * hsize);
            s->lhp[(s->d - 1) & 1][i] = (uchar*)checked_realloc(s->lhp[(s->d - 1) & 1][i],
                sizeof(uchar) * lsize);
          }
        }
        for (i = 0; i < s->planes; i++)
      ...
      if (s->bie_len < 20 + 1728 &&
        ...
        if (!s->dppriv || s->dppriv == jbg_dptable) {
          s->dppriv = (char*)checked_malloc(sizeof(char) * 1728);
        }
