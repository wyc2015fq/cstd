...
#define isio_length(stream) stream_filesize(stream)
#define isio_tell(stream) stream_tell(stream)
#define isio_eof(stream) stream_eof(stream)
#define isio_read(buffer, len, count, stream) (stream_read(stream, buffer, (len)*(count))/(count))
#define isio_write(buffer, len, count, stream) (stream_write(stream, buffer, (len)*(count))/(count))
#define isio_seek(stream, offset, origin) stream_seek(stream, offset, origin)

/////////////////////////////////////////////////////////////////////////
static const uchar flagtable[256] = {
...
static const uchar lowflagtable[256] = {
...
// 最低位1的位置、1的位数、最高位连续1的位数
// range : [0, 32]
CC_INLINE int flagCount(unsigned n)
{
  return flagtable[n & 0xff] + flagtable[(n >> 8) & 0xff] + flagtable[(n >> 16) & 0xff] + flagtable[(n >> 24) & 0xff];
}
// range : [0, 32]
int highContinuousFlagCount(unsigned n)
{
  while ((n & (n + (n & ~(n - 1)))) != 0) {
    n &= n - 1;
  }
...
// range : [0, 32], index started with 1
int lowFlagIndex(unsigned n)
{
  int offset = 0;
  if (0 == n) {
    return 0;
  }
  while (((n >> offset) & 255) == 0) {
    offset += 8;
  }
  return lowflagtable[(n >> offset) & 255] + offset;
}
//最高位1的位置
static const uchar highflagtable[256] = {
...
int highFlagIndex(unsigned n)
{
  ...
  return highflagtable[n >> offset] + offset;
}

/////////////////////////////////////////////////////////////////////////

/* 分配一个子图像节点，并初始化节点内数据 */
SUBIMGBLOCK* _alloc_SUBIMGBLOCK(INFOSTR* pinfo_str)
{
  SUBIMGBLOCK* p_subimg = 0;
  pinfo_str->imgnumbers++;
  /* 申请子图像信息节点(SUBIMGBLOCK) */
  if ((p_subimg = (SUBIMGBLOCK*)prealloc(pinfo_str->psubimg, pinfo_str->imgnumbers * sizeof(SUBIMGBLOCK))) == 0) {
    return 0; /* 内存不足 */
  }
  /* 初始化为 0 */
...
void INFOSTR_free(INFOSTR* pinfo_str) {
  /* 释放子图像 */
  if (pinfo_str->psubimg != 0) {
    int i;
    for (; i<pinfo_str->imgnumbers; ++i) {
      SUBIMGBLOCK* psubimg = pinfo_str->psubimg + i;
      _free_SUBIMGBLOCK(psubimg);
    }
    pfree(pinfo_str->psubimg);
    pinfo_str->psubimg = 0;
  ...
  if (pinfo_str->play_order) {
    isirw_free(pinfo_str->play_order);
    pinfo_str->play_order = (ulong*)0;
  }
...
/* 获取各颜色分量的值 */
EXERESULT _get_mask(int bitcount,ulong* pb_mask,ulong* pg_mask,ulong* pr_mask,ulong* pa_mask)
{
  ...
  return b_status; /* 0-成功， -1－非法的图像格式 */
}
...
/* 在颜色表中查找指定颜色的RGB值 */
static int _search_color(char* name, ulong* pcol)
{
  int i, j;
  int succ = 0, value = 0;
  char* p = name;
...
static int _color_to_rgb(char* name, int len, ulong* rgb)
{
  char buf[256];
  assert(name);
  if (name[0] == '#') { /* 直接RGB给定方式 */
...
int SUBIMGBLOCK_Set(SUBIMGBLOCK* d,int left, int top,int width, int height,int bitcount,
int number,int order,int dowith,int userinput,int time,
ulong r_mask,ulong g_mask,ulong b_mask,ulong a_mask,long  colorkey,
const uchar*const * pp_line_addr, const uchar* p_bit_data, int stride, int pal_count, const COLOR* palette) {
  int i;
  d->stride = _calcu_scanline_size(width, bitcount);
  d->left = left;
  d->top = top;
  d->width = width;
  d->height = height;
  ...
  d->a_mask = a_mask;
  d->colorkey = colorkey;
  MEMCPY(d->palette, palette, MAX_PALETTE_COUNT);
  
  MEMCLONE(d->pp_line_addr, pp_line_addr, height);
  d->p_bit_data = (BYTE*)prealloc(d->p_bit_data, d->stride * height + 4);
  for (i = 0; i < (int)(height); i++) {
    memcpy(d->p_bit_data + i*d->stride, p_bit_data + i*stride, d->stride);
    d->pp_line_addr[i] = (d->p_bit_data + ((d->height - i - 1) * stride));
  }
  return 0;
}
int SUBIMGBLOCK_Clone(SUBIMGBLOCK* d, const SUBIMGBLOCK* s) {
  SUBIMGBLOCK_Set(d, s->left, s->top,s->width, s->height,s->bitcount,
    s->number,s->order,s->dowith,s->userinput,s->time,
    s->r_mask,s->g_mask,s->b_mask,s->a_mask,s->colorkey,
    s->pp_line_addr,s->p_bit_data, s->stride, s->pal_count,s->palette);
  return 0;
}
int INFOSTR_Set(INFOSTR* d,
ulong width,
ulong height,
ulong bitcount,
ulong order,
...
ulong b_mask,
ulong a_mask,
const int* para_value,
IMGTYPE  imgtype,
IMGFORMAT  imgformat,
IMGCOMPRESS compression,
const uchar*const * pp_line_addr,
const uchar* p_bit_data,
int stride,
unsigned int  pal_count,
const COLOR* palette,
ulong play_number,
const ulong* play_order,
ulong time,
long colorkey,
ulong imgnumbers,
ushort  data_state) {
  ...
  d->a_mask = a_mask;
  d->imgtype = imgtype;
  d->imgformat = imgformat;
  d->compression = compression;
  d->pal_count = pal_count;
  d->time = time;
  d->colorkey = colorkey;
  d->imgnumbers = imgnumbers;
  MEMCPY(d->para_value, para_value, ISD_ITEM_NUM);
  MEMCPY(d->palette, palette, MAX_PALETTE_COUNT);

  MEMCLONE(d->pp_line_addr, pp_line_addr, height);
  MEMCLONE(d->play_order, play_order, play_number);
  d->p_bit_data = (BYTE*)prealloc(d->p_bit_data, d->stride * height + 4);
  for (i = 0; i < (int)(height); i++) {
    memcpy(d->p_bit_data + i*d->stride, p_bit_data + i*stride, d->stride);
    d->pp_line_addr[i] = (d->p_bit_data + ((d->height - i - 1) * stride));
...
int INFOSTR_Clone(INFOSTR* d, const INFOSTR* s) {
  int i, imgnumbers = s->imgnumbers;
  INFOSTR_Set(d, s-> width,
s->height,
s->bitcount,
s->order,
s->r_mask,
...
s->imgnumbers,
s->data_state);
  if (s->imgnumbers>imgnumbers) {
    d->psubimg = (SUBIMGBLOCK*)prealloc(d->psubimg, s->imgnumbers * sizeof(SUBIMGBLOCK));
    MEMSET(d->psubimg+imgnumbers, 0, (s->imgnumbers-imgnumbers)*sizeof(SUBIMGBLOCK));
    for (i=imgnumbers; i<s->imgnumbers; ++i) {
      SUBIMGBLOCK_Clone(d->psubimg + i, s->psubimg + i);
    }
  } else {
    for (i=s->imgnumbers; i<imgnumbers; ++i) {
      _free_SUBIMGBLOCK(d->psubimg + i);
    }
    d->imgnumbers = s->imgnumbers;
  }
  return 0;
}
