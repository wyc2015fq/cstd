/********************************************************************
 ...
 ----------------------------------------------------------------
 修正历史：
 2003-6 修正对1、2、4位TIFF图象读取时存在的BUG。(ehom support@ehom.net)
...
int tiff_rgba_image_begin(ISE_TIFFRGBAImage* img, TIFF* tif, int stop, int type, void* pinfo)
{
  ...
  do {
    ...
    img->ycbcr_mark = 0;
    if (type == 0) {
      img->ise.pinfo_str = (INFOSTR*)pinfo;
    }
    else {
      img->ise.psub_str = (SUBIMGBLOCK*)pinfo;
    }
    ...
    case PHOTOMETRIC_PALETTE:
      ...
      if (img->info_type == 0) { /* 主图象结构 */
        ...
        for (i = 0; i < n_color; i++) {
          ((wRGBQUAD*)(&img->ise.pinfo_str->palette[i]))->rgbRed = (uchar)((((ushort)img->redcmap[i]) >> 8) & 0xff);
          ((wRGBQUAD*)(&img->ise.pinfo_str->palette[i]))->rgbGreen = (uchar)((((ushort)img->greencmap[i]) >> 8) & 0xff);
          ((wRGBQUAD*)(&img->ise.pinfo_str->palette[i]))->rgbBlue = (uchar)((((ushort)img->bluecmap[i]) >> 8) & 0xff);
          ((wRGBQUAD*)(&img->ise.pinfo_str->palette[i]))->rgbReserved = (uchar)0;
        }
      }
      else { /* 子图象结构 */
        img->ise.psub_str->pal_count = n_color;
        for (i = 0; i < n_color; i++) {
          ((wRGBQUAD*)(&img->ise.psub_str->palette[i]))->rgbRed = (uchar)((((ushort)img->redcmap[i]) >> 8) & 0xff);
          ((wRGBQUAD*)(&img->ise.psub_str->palette[i]))->rgbGreen = (uchar)((((ushort)img->greencmap[i]) >> 8) & 0xff);
          ((wRGBQUAD*)(&img->ise.psub_str->palette[i]))->rgbBlue = (uchar)((((ushort)img->bluecmap[i]) >> 8) & 0xff);
          ((wRGBQUAD*)(&img->ise.psub_str->palette[i]))->rgbReserved = (uchar)0;
        }
      }
      break;
    case PHOTOMETRIC_MINISWHITE:
...
static void _cal_scanline_pdest(ISE_TIFFRGBAImage* img, int x, int y, int* scanline, char** pdest)
{
  ...
  if (img->info_type == 0) {
    ...
    *pdest = ((uchar*)img->ise.pinfo_str->p_bit_data) + y * (*scanline) + (x * img->ise.pinfo_str->bitcount) / 8;
  }
  else {
    *scanline = DIBSCANLINE_WIDTHBYTES(img->ise.psub_str->bitcount * img->ise.psub_str->width);
    *pdest = ((uchar*)img->ise.psub_str->p_bit_data) + y * (*scanline) + (x * img->ise.psub_str->bitcount) / 8;
  }
}
/*****************************************************************************/
/* 以下为解码函数 */
...
static ise_tileContigRoutine init_ycbcr_conversion(ISE_TIFFRGBAImage* img)
{
  ...
  switch ((hs << 4) | vs) {
  case 0x44:
    return ((ise_tileContigRoutine)putcontig8bitYCbCr44tile);
  case 0x42:
    return ((ise_tileContigRoutine)putcontig8bitYCbCr42tile);
  case 0x41:
    return ((ise_tileContigRoutine)putcontig8bitYCbCr41tile);
  case 0x22:
    return ((ise_tileContigRoutine)putcontig8bitYCbCr22tile);
  case 0x21:
    return ((ise_tileContigRoutine)putcontig8bitYCbCr21tile);
  case 0x11:
    return ((ise_tileContigRoutine)putcontig8bitYCbCr11tile);
  }
