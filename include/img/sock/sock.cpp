...
bool CPM_InitSock( NET_MODEL NetModel, unsigned int uMemPoolSize, unsigned int uMemPoolUnit)
{
	...
	if ( !INITSOCK )    return false;
	
	assert( NULL == gNetManager );
	gNetManager = new CNetManager();
	if( gNetManager )
	{
		if( uMemPoolSize <= 0 ) uMemPoolSize = MEMRESERVER_SIZE;
		if( uMemPoolUnit <= 0 ) uMemPoolUnit = MEMRESERVER_UNIT;
...
{
	if ( gNetManager ) gNetManager->RegListenCB(lpfnListen, pvContext);
}

void CPM_SetDataRecvCallBack( DataRecvCallBack lpfnRecv, void* pvContext )
{
	if ( gNetManager ) gNetManager->RegRecvCB(lpfnRecv, pvContext);
}

void CPM_SetDataRecvFromCallBack( DataRecvFromCallBack lpfnRecv, void* pvContext )
{
	if ( gNetManager ) gNetManager->RegRecvFromCB(lpfnRecv, pvContext);
}

bool CPM_StartWork( bool bStartUp )
{
	if ( !gNetManager ) return false;
...
{
	if ( gNetManager ) gNetManager->ReleaseSocket( (SOCKET)psock );
}
...
{
	if ( gNetManager ) gNetManager->DropIPGroup( (SOCKET)hSOCK, GroupIPAddr );
}

void CPM_Listen( HANDLE hSOCK )
{
	if ( gNetManager ) gNetManager->Listen( (SOCKET)hSOCK );
}

void CPM_SetSendBufSize( HANDLE hSOCK, int nSize )
{
	if ( gNetManager ) gNetManager->SetSendBuf( (SOCKET)hSOCK, nSize );
}

void CPM_SetRecvBufSize( HANDLE hSOCK, int nSize )
{
	if ( gNetManager ) gNetManager->SetRecvBuf( (SOCKET)hSOCK, nSize );
}
...
{
	if ( gNetManager ) gNetManager->Recv( (SOCKET)psock );
}
