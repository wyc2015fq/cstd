...
{
	return ioctlsocket(sock, cmd , (uint32*)&enable) == SOCKET_ERROR ? false : true;
}
...
bool sock_name( SOCKET sock, char* ip, uint16& port )
{
	sockaddr_in addr = {0};
    SOCK_LENTH nLen = sizeof( sockaddr_in );
	if ( SOCKET_ERROR == getsockname( sock, (sockaddr*)&addr, &nLen) ) return false;
	if (ip) strcpy( ip, inet_ntoa( addr.sin_addr ) );
	port = ntohs( addr.sin_port );
	return true;
}
...
{
	return ioctlsocket(sock, FIONREAD, (uint32*)&amount) == SOCKET_ERROR ? false : true;
}
...
void sock_getacceptexsockaddrs( WSABUF* lpBuffer, int8* ip, uint16& port )
{
	...
	if ( RemoteSockaddr )
	{
		if (ip) strcpy( ip, inet_ntoa( RemoteSockaddr->sin_addr ) );
