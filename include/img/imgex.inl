...
#define _GetRValue(c)	((uchar)(((c) & 0x00FF0000) >> 16))		// Red color component
#define _GetGValue(c)	((uchar)(((c) & 0x0000FF00) >> 8))		// Green color component
#define _GetBValue(c)	((uchar)((c) & 0x000000FF))				// Blue color component
#else
#define _GetRValue(c)  GetRV
#define _GetGValue(c)  GetGV
#define _GetBValue(c)  GetBV
#endif
...
typedef int fixed;												// Our new fixed point type
#define itofx(x) ((x) << 8)										// Integer to fixed point
#define ftofx(x) (int)((x) * 256)								// Float to fixed point
#define dtofx(x) (int)((x) * 256)								// Double to fixed point
...
float _ARG(double xa, double ya)
{
	double _arg = 0.0;
...
int img_ConvertTo32Bpp(const img_t* im, int biBitCount, const COLOR* m_lpPalette, img_t* im2)
{
    int i, j;
    imsetsize(im2, im->h, im->w, 4, im->f);
		for(i=0; i<im->h; i++)
    {
      const uchar* A = im->tt.data + i*im->s;
      const uint16* A2 = (const uint16*)A;
      const uint32* A4 = (const uint32*)A;
      uint32* B = (uint32*)(im2->tt.data + i*im2->s);
      // Update bitmap
      switch (biBitCount)
      {
      case 8:
      ...
      case 16:
        for(j=0; j<im->w; j++) {
          uchar red = (A2[j] & 0x7C00) >> 10;
          uchar green = (A2[j] & 0x03E0) >> 5;
          uchar blue = A2[j] & 0x001F;
          B[j] = _RGB(red, green, blue);
        }
        break;
        
      case 24:
        for(j=0; j<im->w; j++) {
          B[j] = _RGB(A[j+2], A[j+1], A[j]);
        }
        break;
...
int img_ConvertTo24Bpp(const img_t* im, int biBitCount, const COLOR* m_lpPalette, img_t* im2)
{
  // Check for valid bitmap
  int i, j;
  imsetsize(im2, im->h, im->w, 3, im->f);
	...
	for(i=0; i<im->h; i++)
  {
    const uchar* A = im->tt.data + i*im->s;
    const uint16* A2 = (const uint16*)A;
    uchar* B = (uchar*)(im2->tt.data + i*im2->s);
    // Update bitmap
    switch (biBitCount)
    {
    case 8:
      for(j=0; j<im->w; j++, B+=3) {
        const uchar* m_lpPalette_u8 = (const uchar*)(m_lpPalette+A[j]);
        B[0] = m_lpPalette_u8[0];
        B[1] = m_lpPalette_u8[1];
        B[2] = m_lpPalette_u8[2];
      }
      break;
...
#if 0
int img_ScaleNearestNeighbour(img_t* im, int horizontalPercent, int verticalPercent)
{
	// Check for valid bitmap
		// Calculate scaling params
		int _horizontalPercent = max(1, horizontalPercent);
		int _verticalPercent = max(1, verticalPercent);
		float dx = 100.0f / (float)_horizontalPercent;
		float dy = 100.0f / (float)_verticalPercent;
		int _width = (int)((1.0f/dx) * (float)im->w + 0.5f);
		int _height = (int)((1.0f/dy) * (float)im->h + 0.5f);
		...
		uchar* lpData = (uchar*)pmalloc(dwSize*sizeof(uchar));
...
int img_ScaleBilinear(int horizontalPercent, int verticalPercent)
{
	// Check for valid bitmap
	
	{
		...
		for(i=0; i<_height; i++)
		{
			...
			for(j=0; j<_width; j++)
			{
				...
				uchar red = (uchar)fxtoi(Mulfx(f_w1, f_r1) + Mulfx(f_w2, f_r2) + Mulfx(f_w3, f_r3) + Mulfx(f_w4, f_r4));
				uchar green = (uchar)fxtoi(Mulfx(f_w1, f_g1) + Mulfx(f_w2, f_g2) + Mulfx(f_w3, f_g3) + Mulfx(f_w4, f_g4));
				uchar blue = (uchar)fxtoi(Mulfx(f_w1, f_b1) + Mulfx(f_w2, f_b2) + Mulfx(f_w3, f_b3) + Mulfx(f_w4, f_b4));
...
int img_ScaleBicubic(int horizontalPercent, int verticalPercent)
{
	// Check for valid bitmap
	
	{
		...
		for(i=0; i<_height; i++)
		{
			...
			for(j=0; j<_width; j++)
			{
				...
				uchar red = (uchar)max(0, min(255, fxtoi(Mulfx(f_red,f_gama))));
				uchar green = (uchar)max(0, min(255, fxtoi(Mulfx(f_green,f_gama))));
				uchar blue = (uchar)max(0, min(255, fxtoi(Mulfx(f_blue,f_gama))));
				lpDstData[dwDstTotalOffset>>2] = _RGB(red, green, blue);
...
int img_Scale(int horizontalPercent, int verticalPercent)
{
...
int img_ScaleNearestNeighbour2(int width, int height)
{
	// Check for valid bitmap
	
	{
		...
		float dx = (float)im->w / (float)_width;
		float dy = (float)im->h / (float)_height;
		int _pitch = m_iBpp * _width;
		while ((_pitch & 3) != 0)
			_pitch++;
		fixed f_dx = ftofx(dx);
...
int img_ScaleBilinear2(int width, int height)
{
	// Check for valid bitmap
...
int img_ScaleBicubic2(int width, int height)
{
	// Check for valid bitmap
...
int img_Scale2(int width, int height)
{
	// Check for resampling mode
	switch (m_ResampleMode)
	{
...
int img_RotateNearestNeighbour(int degrees, _PIXEL bgColor)
{
	// Check for valid bitmap
	
	{
		...
		int _width = (int)(abs((float)im->w*cos(_angle)) + abs((float)im->h*sin(_angle)) + 0.5f);
		int _height = (int)(abs((float)im->w*sin(_angle)) + abs((float)im->h*cos(_angle)) + 0.5f);
		int _pitch = m_iBpp * _width;
		while ((_pitch & 3) != 0)
			_pitch++;
		fixed f_0_5 = ftofx(0.5f);
		fixed f_H = itofx(im->h/2);
		fixed f_W = itofx(im->w/2);
...
int img_RotateBilinear(int degrees, _PIXEL bgColor)
{
	// Check for valid bitmap
		...
		for(i=0; i<_height; i++)
		{
			...
			for(j=0; j<_width; j++)
			{
				...
				if ((m > 0) && (m < im->h-1) && (n > 0) && (n < im->w-1))
				{
					...
					uchar red = (uchar)max(0, min(255, fxtoi(f_red)));
					uchar green = (uchar)max(0, min(255, fxtoi(f_green)));
					uchar blue = (uchar)max(0, min(255, fxtoi(f_blue)));
					lpDstData[dwDstTotalOffset>>2] = _RGB(red, green, blue);
				}
				else
					lpDstData[dwDstTotalOffset>>2] = bgColor;
...
int img_RotateBicubic(int degrees, _PIXEL bgColor)
{
	// Check for valid bitmap
...
int img_Rotate(int degrees, _PIXEL bgColor)
{
	// Check for resampling mode
	switch (m_ResampleMode)
	{
...
int img_Crop(int x, int y, int width, int height)
{
	// Check for valid bitmap
	
	{
		// Calculate cropping params
		int _y1 = max(0, min(im->h, y));
		int _x1 = max(0, min(im->w, x));
		int _y2 = max(0, min(im->h, height+_y1));
		int _x2 = max(0, min(im->w, width+_x1));
		int _height = _y2 - _y1;
		int _width = _x2 - _x1;
		...
		DWORD dwSrcVerticalOffset = (im->h-_y2) * m_iPitch;
		DWORD dwSrcTotalOffset;
		LPDWORD lpSrcData = (LPDWORD)m_lpData;
		for(i=0; i<_height; i++)
		{
...
int img_Shear(int degreesX, int degreesY, _PIXEL bgColor)
{
	// Check for valid bitmap
	
	{
		// Calculate shearing params
		int _degreesY = max(-89, min(89, degreesY));
		int _degreesX = max(-89, min(89, degreesX));
...
int img_ShearVerticalNearestNeighbour(int degrees, _PIXEL bgColor)
{
	// Check for valid bitmap
	
	{
		...
		int _offset = (int)((float)im->w * _alpha + 0.5f);
		int _height = im->h + abs(_offset);
		int _width = im->w;
		fixed f_alpha = ftofx(_alpha);
		fixed f_offset = ftofx(_offset);

		// Create temporary bitmap
		int _pitch = _width * m_iBpp;
		while ((_pitch & 3) != 0)
			_pitch++;
		DWORD dwSize = _pitch * _height;
		uchar* lpData = (uchar*)pmalloc(dwSize*sizeof(uchar));
...
int img_ShearVerticalBilinear(int degrees, _PIXEL bgColor)
{
	// Check for valid bitmap
...
int img_ShearVerticalBicubic(int degrees, _PIXEL bgColor)
{
	// Check for valid bitmap
...
int img_ShearVertical(int degrees, _PIXEL bgColor)
{
	// Check for resampling mode
	switch (m_ResampleMode)
	{
...
int img_ShearHorizontalNearestNeighbour(int degrees, _PIXEL bgColor)
{
	// Check for valid bitmap
	
	{
		// Calculate shearing params
		float _alpha = tan((float)degrees/180.0f*_PI);
		int _offset = (int)((float)im->h * _alpha + 0.5f);
		int _height = im->h;
		int _width = im->w + abs(_offset);
		fixed f_alpha = ftofx(_alpha);
		fixed f_offset = ftofx(_offset);
...
int img_ShearHorizontalBilinear(int degrees, _PIXEL bgColor)
{
	// Check for valid bitmap
...
int img_ShearHorizontalBicubic(int degrees, _PIXEL bgColor)
{
	// Check for valid bitmap
...
int img_ShearHorizontal(int degrees, _PIXEL bgColor)
{
	// Check for resampling mode
	switch (m_ResampleMode)
	{
...
int img_FlipHorizontal()
{
	...
	
	{
		...
		DWORD dwSrcHorizontalStartOffset = (im->w-1) * m_iBpp;
		DWORD dwSrcHorizontalOffset;
...
int img_FlipVertical()
{
	...
	
	{
		...
		LPDWORD lpDstData = (LPDWORD)m_lpData;
		DWORD dwSrcTotalOffset = (im->h-1) * m_iPitch;
		LPDWORD lpSrcData = (LPDWORD)m_lpData;
		LPDWORD lpTmpData = (LPDWORD)pmalloc(m_iPitch*sizeof(uchar));
		for(i=0; i<_height; i++)
		{
			// Update bitmap
			memcpy(lpTmpData, lpDstData+(dwDstTotalOffset>>2), m_iPitch*sizeof(uchar));
			memcpy(lpDstData+(dwDstTotalOffset>>2), lpSrcData+(dwSrcTotalOffset>>2), m_iPitch*sizeof(uchar));
			memcpy(lpSrcData+(dwSrcTotalOffset>>2), lpTmpData, m_iPitch*sizeof(uchar));
...
int img_MirrorLeft()
{
	...
	
	{
		...
		// Mirror bitmap left
		DWORD dwDstHorizontalStartOffset = (im->w-1) * m_iBpp;
		DWORD dwDstHorizontalOffset;
		DWORD dwDstVerticalOffset = 0;
		DWORD dwDstTotalOffset;
		LPDWORD lpDstData = (LPDWORD)m_lpData;
...
int img_MirrorBottom()
{
	...
	
	{
		...
		DWORD dwDstTotalOffset = (im->h-1) * m_iPitch;
		LPDWORD lpDstData = (LPDWORD)m_lpData;
...
int img_Clear(_PIXEL clearColor)
{
	// Check for valid bitmap
...
int img_Sepia(int depth)
{
	...
	
	{
		...
		fixed f_w2 = ftofx(0.587f);
		fixed f_w3 = ftofx(0.114f);
		int _depth = max(0, min(100, depth));
...
/*		// Sepia bitmap
		...
		for(i=0; i<im->h; i++)
		{
			...
			for(j=0; j<im->w; j++)
			{
				...
				fixed f_blue = itofx(_GetBValue(lpDstData[dwTotalOffset>>2]));
				fixed f_value = Mulfx(f_w1,f_red) + Mulfx(f_w2,f_green) + Mulfx(f_w3,f_blue);
				uchar red = (uchar)fxtoi(f_value+f_depth2);
				if (red < ((_depth<<1)-1))
					red = 255;
				uchar green = (uchar)fxtoi(f_value+f_depth);
				if (green < (_depth-1))
					green = 255;
				uchar blue = (uchar)fxtoi(f_value);
...
int img_Pixelize(int size)
{
	// Check for valid bitmap
	
	{
		// Calculate pixelize params
		int _size = max(1, min((im->w>>4), size));
		...
		LPDWORD lpSrcData = (LPDWORD)m_lpData;
		LPDWORD lpDstData = (LPDWORD)lpData;
		for(i=0; i<im->h; i+=_size)
		{
			dwHorizontalOffset = 0;
			for(j=0; j<im->w; j+=_size)
			{
				// Update total offset
				dwTotalOffset = dwVerticalOffset + dwHorizontalOffset;
...
int img_Brightness(int brightness)
{
	// Check for valid bitmap
	
	{
		...
		for(i=0; i<im->h; i++)
		{
			...
			for(j=0; j<im->w; j++)
			{
				...
				// Update bitmap
				uchar red = _GetRValue(lpDstData[dwTotalOffset>>2]);
				uchar green = _GetGValue(lpDstData[dwTotalOffset>>2]);
				uchar blue = _GetBValue(lpDstData[dwTotalOffset>>2]);
				red = (uchar)max(0, min(red+_brightness, 255));
				green = (uchar)max(0, min(green+_brightness, 255));
				blue = (uchar)max(0, min(blue+_brightness, 255));
				lpDstData[dwTotalOffset>>2] = _RGB(red, green, blue);
	...
	
	{
		...
		for(i=0; i<im->h; i++)
		{
			...
			for(j=0; j<im->w; j++)
			{
				...
				uchar red = (uchar)max(0, min(fxtoi(f_red), 255));
				uchar green = (uchar)max(0, min(fxtoi(f_green), 255));
				uchar blue = (uchar)max(0, min(fxtoi(f_blue), 255));
				lpDstData[dwTotalOffset>>2] = _RGB(red, green, blue);
	...
	
	{
		...
		for(i=0; i<im->h; i++)
		{
			...
			for(j=0; j<im->w; j++)
			{
				...
				uchar red = (uchar)max(0, min(fxtoi(Divfx(f_red,f_9)), 255));
				uchar green = (uchar)max(0, min(fxtoi(Divfx(f_green,f_9)), 255));
				uchar blue = (uchar)max(0, min(fxtoi(Divfx(f_blue,f_9)), 255));
				lpDstData[dwTotalOffset>>2] = _RGB(red, green, blue);
	...
	
	{
		...
		for(i=0; i<im->h; i++)
		{
			...
			for(j=0; j<im->w; j++)
			{
				...
				uchar red = (uchar)max(0, min(fxtoi(Divfx(f_red,f_16)), 255));
				uchar green = (uchar)max(0, min(fxtoi(Divfx(f_green,f_16)), 255));
				uchar blue = (uchar)max(0, min(fxtoi(Divfx(f_blue,f_16)), 255));
				lpDstData[dwTotalOffset>>2] = _RGB(red, green, blue);
	...
	
	{
		...
		for(i=0; i<im->h; i++)
		{
			...
			for(j=0; j<im->w; j++)
			{
				...
				uchar red = (uchar)max(0, min(fxtoi(Mulfx(f_red,f_1_2)), 255));
				uchar green = (uchar)max(0, min(fxtoi(Mulfx(f_green,f_1_2)), 255));
				uchar blue = (uchar)max(0, min(fxtoi(Mulfx(f_blue,f_1_2)), 255));
				lpDstData[dwTotalOffset>>2] = _RGB(red, green, blue);
...
int img_Spread(int distanceX, int distanceY)
{
	// Check for valid bitmap
	
	{
		// Calculate spread params
		int _distanceY = max(1, min(im->h/2-1, distanceY));
		int _distanceX = max(1, min(im->w/2-1, distanceX));
		...
		for(i=0; i<im->h; i++)
		{
			...
			for(j=0; j<im->w; j++)
			{
				...
				// Update bitmap
				DWORD dwSrcOffset = dwTotalOffset;
				int m = i + (rand()%_distanceY) - _distanceY/2;
				if (m < 0)
					m = 0;
				else if (m >= im->h-1)
					m = im->h - 1;
				int n = j + (rand()%_distanceX) - _distanceX/2;
				if (n < 0)
					n = 0;
				else if (n >= im->w-1)
					n = im->w - 1;
...
int img_Offset(int offsetX, int offsetY)
{
	// Check for valid bitmap
	
	{
		// Calculate offset params
		int _offsetY = max(-im->h/2+1, min(im->h/2-1, offsetY));
		int _offsetX = max(-im->w/2+1, min(im->w/2-1, offsetX));
		...
		for(i=0; i<im->h; i++)
		{
			...
			for(j=0; j<im->w; j++)
			{
				...
				// Update bitmap
				int m = (i + _offsetY) % im->h;
				int n = (j + _offsetX) % im->w;
...
int img_BlackAndWhite(int offset)
{
	// Check for valid bitmap
	
	{
		...
		fixed f_w2 = ftofx(0.587f);
		fixed f_w3 = ftofx(0.114f);
		int _offset = max(0, min(255, offset));
...
int img_GlowingEdges(int blur, int threshold, int scale)
{
	// Check for valid bitmap
	
	{
		...
		int _scale = max(0, min(100, scale));
...
int img_EqualizeHistogram(int levels)
{
	// Check for valid bitmap
	
	{
		...
		int _levels = max(0, min(255, levels));

		// Build histogram
		int lpRedHistogram[256], lpGreenHistogram[256], lpBlueHistogram[256];
		memset(lpRedHistogram, 0, 256*sizeof(int));
		memset(lpGreenHistogram, 0, 256*sizeof(int));
		memset(lpBlueHistogram, 0, 256*sizeof(int));
		DWORD dwHorizontalOffset;
		DWORD dwVerticalOffset = 0;
		DWORD dwTotalOffset;
		LPDWORD lpSrcData = (LPDWORD)m_lpData;
		for (i=0; i<im->h; i++)
		{
			...
			for (j=0; j<im->w; j++)
			{
				...
				uchar red = _GetRValue(lpSrcData[dwTotalOffset>>2]);
				uchar green = _GetGValue(lpSrcData[dwTotalOffset>>2]);
				uchar blue = _GetBValue(lpSrcData[dwTotalOffset>>2]);
				lpRedHistogram[red]++;
		...
		int lpRedCumulativeFrequency[256], lpGreenCumulativeFrequency[256], lpBlueCumulativeFrequency[256];
		memset(lpRedCumulativeFrequency, 0, 256*sizeof(int));
		memset(lpGreenCumulativeFrequency, 0, 256*sizeof(int));
		memset(lpBlueCumulativeFrequency, 0, 256*sizeof(int));
		...
		float _histoCoef = (float)_levels / (float)(im->w*im->h);
		...
		for (i=0; i<im->h; i++)
		{
			...
			for (j=0; j<im->w; j++)
			{
				...
				// Update bitmap
				uchar hue = _GetRValue(lpDstData[dwTotalOffset>>2]);
				uchar saturation = _GetGValue(lpDstData[dwTotalOffset>>2]);
				uchar value = _GetBValue(lpDstData[dwTotalOffset>>2]);
				value = (uchar)((float)lpBlueCumulativeFrequency[value] * _histoCoef);
...
int img_Median()
{
	...
	
	{
		...
		LPDWORD lpSrcData = (LPDWORD)m_lpData;
		LPDWORD lpDstData = (LPDWORD)lpData;
		uchar lpRed[9], lpGreen[9], lpBlue[9];
		for(i=0; i<im->h; i++)
		{
			dwHorizontalOffset = 0;
			for(j=0; j<im->w; j++)
...
int img_Posterize(int levels)
{
	// Check for valid bitmap
	
	{
		// Calculate posterize params
		int _levels = max(2, min(16, levels));
		int _offset = 256 / _levels;
		uchar lpRedPalette[16], lpGreenPalette[16], lpBluePalette[16];
		for(k=0; k<_levels; k++)
		{
			lpRedPalette[k] = (uchar)(k * _offset);
			lpGreenPalette[k] = (uchar)(k * _offset);
			lpBluePalette[k] = (uchar)(k * _offset);
		}
		...
		for(i=0; i<im->h; i++)
		{
			...
			for(j=0; j<im->w; j++)
			{
				...
				uchar green = _GetGValue(lpDstData[dwTotalOffset>>2]);
				uchar blue = _GetBValue(lpDstData[dwTotalOffset>>2]);
				red = (uchar)(red / _offset);
				green = (uchar)(green / _offset);
				blue = (uchar)(blue /_offset);
...
int img_Solarize(int threshold)
{
	// Check for valid bitmap
	
	{
		// Calculate solarization params
		int _threshold = max(0, min(255, threshold));
		...
		for(i=0; i<im->h; i++)
		{
			...
			for(j=0; j<im->w; j++)
			{
				...
				red = (uchar)max(0, min(255, fxtoi(f_red)));
				green = (uchar)max(0, min(255, fxtoi(f_green)));
				blue = (uchar)max(0, min(255, fxtoi(f_blue)));
				lpDstData[dwTotalOffset>>2] = _RGB(red, green, blue);
...
int img_Draw(HDC hDC)
{
	// Check for valid bitmap
	
	{
		// Draw bitmap
		::SetDIBitsToDevice(hDC, 0, 0, im->w, im->h, 0, 0, 0, im->h, m_lpData, (LPBITMAPINFO)&m_bih, DIB_RGB_COLORS);
	}
}

int img_Draw(HDC hDC, int dstX, int dstY)
{
	// Check for valid bitmap
	
	{
		// Draw bitmap
		::SetDIBitsToDevice(hDC, dstX, dstY, im->w, im->h, 0, 0, 0, im->h, m_lpData, (LPBITMAPINFO)&m_bih, DIB_RGB_COLORS);
	}
}

int img_Draw(int dstX, int dstY, int width, int height, CBitmapEx& bitmapEx, int srcX, int srcY)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
		// Calculate destination params
		int _dstStartX = max(0, min(im->w-1, dstX));
		if (srcX < 0)
			_dstStartX = max(0, min(im->w-1, dstX-srcX));
		int _dstStartY = max(0, min(im->h-1, dstY));
		if (srcY < 0)
			_dstStartY = max(0, min(im->h-1, dstY-srcY));
		int _dstEndX = max(0, min(dstX+width, im->w-1));
		int _dstEndY = max(0, min(dstY+height, im->h-1));

		// Calculate source params
		int _srcStartX = max(0, min(bitmapEx.GetWidth()-1, srcX));
		if (dstX < 0)
			_srcStartX = max(0, min(bitmapEx.GetWidth()-1, srcX-dstX));
		int _srcStartY = max(0, min(bitmapEx.GetHeight()-1, srcY));
		if (dstY < 0)
			_srcStartY = max(0, min(bitmapEx.GetHeight()-1, srcY-dstY));
		int _srcEndX = max(0, min(srcX+width, bitmapEx.GetWidth()-1));
		int _srcEndY = max(0, min(srcY+height, bitmapEx.GetHeight()-1));

		// Check drawing region
		int _pitch = bitmapEx.GetPitch();
		int _bpp = bitmapEx.GetBpp() >> 3;
		int _width = min(_dstEndX-_dstStartX, _srcEndX-_srcStartX);
		int _height = min(_dstEndY-_dstStartY, _srcEndY-_srcStartY);
		if ((_width == 0) || (_height == 0))
			return;
...
/*		// Draw bitmap
		...
		DWORD dwDstHorizontalOffset;
		DWORD dwDstVerticalOffset = (im->h-_dstStartY-1) * m_iPitch;
		DWORD dwDstTotalOffset;
		LPDWORD lpDstData = (LPDWORD)m_lpData;
...
int img_Draw(int dstX, int dstY, int width, int height, CBitmapEx& bitmapEx, int srcX, int srcY, int alpha)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
		...
		int _pitch = bitmapEx.GetPitch();
		int _bpp = bitmapEx.GetBpp() >> 3;
		int _alpha = max(0, min(alpha, 100));
...
/*		// Draw bitmap
		...
		for(i=0; i<=_height; i++)
		{
			...
			for(j=0; j<=_width; j++)
			{
				...
				uchar red = (uchar)fxtoi(Mulfx(f_sred, f_alpha) + Mulfx(f_dred, f_1-f_alpha));
				uchar green = (uchar)fxtoi(Mulfx(f_sgreen, f_alpha) + Mulfx(f_dgreen, f_1-f_alpha));
				uchar blue = (uchar)fxtoi(Mulfx(f_sblue, f_alpha) + Mulfx(f_dblue, f_1-f_alpha));
				lpDstData[dwDstTotalOffset>>2] = _RGB(red, green, blue);
...
int img_Draw(_QUAD dstQuad, CBitmapEx& bitmapEx)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
		...
		_dstQuad.p1.x = max(0, min(im->w-1, _dstQuad.p1.x));
		_dstQuad.p1.y = max(0, min(im->h-1, _dstQuad.p1.y));
		_dstQuad.p2.x = max(0, min(im->w-1, _dstQuad.p2.x));
		_dstQuad.p2.y = max(0, min(im->h-1, _dstQuad.p2.y));
		_dstQuad.p3.x = max(0, min(im->w-1, _dstQuad.p3.x));
		_dstQuad.p3.y = max(0, min(im->h-1, _dstQuad.p3.y));
		_dstQuad.p4.x = max(0, min(im->w-1, _dstQuad.p4.x));
		_dstQuad.p4.y = max(0, min(im->h-1, _dstQuad.p4.y));

		// Calculate destination params
		int _dstStartX = max(0, min(min(_dstQuad.p1.x, _dstQuad.p2.x), min(_dstQuad.p3.x, _dstQuad.p4.x)));
		int _dstStartY = max(0, min(min(_dstQuad.p1.y, _dstQuad.p2.y), min(_dstQuad.p3.y, _dstQuad.p4.y)));
		int _dstEndX = min(im->w-1, max(max(_dstQuad.p1.x, _dstQuad.p2.x), max(_dstQuad.p3.x, _dstQuad.p4.x)));
		int _dstEndY = min(im->h-1, max(max(_dstQuad.p1.y, _dstQuad.p2.y), max(_dstQuad.p3.y, _dstQuad.p4.y)));
		int _dstWidth = _dstEndX - _dstStartX + 1;
		int _dstHeight = _dstEndY - _dstStartY + 1;
		float _dstInvHeight = 1.0f / (float)_dstHeight;
		float _dstInvWidth = 1.0f / (float)_dstWidth;

		// Check drawing region
		int _height = bitmapEx.GetHeight();
		int _width = bitmapEx.GetWidth();
		int _pitch = bitmapEx.GetPitch();
		int _bpp = bitmapEx.GetBpp() >> 3;
		...
		switch (m_ResampleMode)
		{
			case RM_NEARESTNEIGHBOUR:
				{
					for(i=_dstStartY; i<=_dstEndY; i++)
					{
						for(j=_dstStartX; j<=_dstEndX; j++)
						{
							// Update bitmap
							...
							if (det >= 0.0f)
							{
								...
								{
									int m = (int)(delta*_height+0.5f);
									int n = (int)(mu*_width+0.5f);
...
int img_Draw(_QUAD dstQuad, CBitmapEx& bitmapEx, int alpha)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
						...
						for(j=_dstStartX; j<=_dstEndX; j++)
						{
							...
							if (det >= 0.0f)
							{
								...
								if ((mu >= 0.0f) && (mu < 1.0f) && (delta >= 0.0f) && (delta < 1.0f))
								{
									...
									fixed f_dgreen = itofx(_GetGValue(pixel2));
									fixed f_dblue = itofx(_GetBValue(pixel2));
									red = (uchar)fxtoi(Mulfx(f_sred, f_alpha) + Mulfx(f_dred, f_1-f_alpha));
									green = (uchar)fxtoi(Mulfx(f_sgreen, f_alpha) + Mulfx(f_dgreen, f_1-f_alpha));
									blue = (uchar)fxtoi(Mulfx(f_sblue, f_alpha) + Mulfx(f_dblue, f_1-f_alpha));
									SetPixel(j, i, _RGB(red, green, blue));
								}
							}
						}
...
int img_Draw(_QUAD dstQuad, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
		...
		int _srcStartX = max(0, min(bitmapEx.GetWidth()-1, srcX));
		int _srcStartY = max(0, min(bitmapEx.GetHeight()-1, srcY));
		int _srcEndX = max(0, min(bitmapEx.GetWidth()-1, _srcStartX+srcWidth));
		int _srcEndY = max(0, min(bitmapEx.GetHeight()-1, _srcStartY+srcHeight));
		int _srcWidth = _srcEndX - _srcStartX + 1;
		int _srcHeight = _srcEndY - _srcStartY + 1;
					...
					for(i=_dstStartY; i<=_dstEndY; i++)
					{
						for(j=_dstStartX; j<=_dstEndX; j++)
						{
							...
							if (det >= 0.0f)
							{
								...
								if ((mu >= 0.0f) && (mu < 1.0f) && (delta >= 0.0f) && (delta < 1.0f))
								{
									int m = (int)(delta*_srcHeight+0.5f) + _srcStartY;
									int n = (int)(mu*_srcWidth+0.5f) + _srcStartX;
									SetPixel(j, i, bitmapEx.GetPixel(n, m));
								}
							}
						}
					...
					for(i=_dstStartY; i<=_dstEndY; i++)
					{
						for(j=_dstStartX; j<=_dstEndX; j++)
						{
							...
							if (det >= 0.0f)
							{
								...
								if ((mu >= 0.0f) && (mu < 1.0f) && (delta >= 0.0f) && (delta < 1.0f))
								{
									int m = (int)(delta*_srcHeight+_srcStartY);
									int n = (int)(mu*_srcWidth+_srcStartX);
...
int img_Draw(_QUAD dstQuad, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, int alpha)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawNearestNeighbour(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
		...
		if (srcY < 0)
			_dstStartY = max(0, min(im->h-1, dstY-srcY));
		int _dstEndX = max(0, min(dstX+dstWidth, im->w-1));
		int _dstEndY = max(0, min(dstY+dstHeight, im->h-1));
		int _dstWidth = _dstEndX - _dstStartX + 1;
		int _dstHeight = _dstEndY - _dstStartY + 1;
		...
		if (dstY < 0)
			_srcStartY = max(0, min(bitmapEx.GetHeight()-1, srcY-dstY));
		int _srcEndX = max(0, min(srcX+srcWidth, bitmapEx.GetWidth()-1));
		int _srcEndY = max(0, min(srcY+srcHeight, bitmapEx.GetHeight()-1));
		int _srcWidth = _srcEndX - _srcStartX + 1;
		int _srcHeight = _srcEndY - _srcStartY + 1;
		...
		int _pitch = bitmapEx.GetPitch();
		int _bpp = bitmapEx.GetBpp() >> 3;
		int _horizontalPercent = (int)(((float)_dstWidth / (float)_srcWidth) * 100.0f);
		int _verticalPercent = (int)(((float)_dstHeight / (float)_srcHeight) * 100.0f);
...
/*		// Draw bitmap
		...
		DWORD dwSrcTotalOffset;
		LPDWORD lpSrcData = (LPDWORD)bitmapEx.GetData();
		for(i=0; i<_dstHeight; i++)
		{
			dwDstHorizontalOffset = dwDstHorizontalStartOffset;
			for(j=0; j<_dstWidth; j++)
			{
				// Update destination total offset
				dwDstTotalOffset = dwDstVerticalOffset + dwDstHorizontalOffset;
...
int img_DrawBilinear(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawBicubic(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_Draw(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight)
{
	// Check for resampling mode
	switch (m_ResampleMode)
	{
...
int img_DrawNearestNeighbour(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, int alpha)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawBilinear(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, int alpha)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawBicubic(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, int alpha)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_Draw(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, int alpha)
{
	// Check for resampling mode
	switch (m_ResampleMode)
	{
...
int img_DrawTransparent(int dstX, int dstY, int width, int height, CBitmapEx& bitmapEx, int srcX, int srcY, _PIXEL transparentColor)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawTransparent(_QUAD dstQuad, CBitmapEx& bitmapEx, _PIXEL transparentColor)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawTransparent(int dstX, int dstY, int width, int height, CBitmapEx& bitmapEx, int srcX, int srcY, int alpha, _PIXEL transparentColor)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawTransparent(_QUAD dstQuad, CBitmapEx& bitmapEx, int alpha, _PIXEL transparentColor)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawTransparentNearestNeighbour(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, _PIXEL transparentColor)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawTransparentBilinear(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, _PIXEL transparentColor)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawTransparentBicubic(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, _PIXEL transparentColor)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawTransparent(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, _PIXEL transparentColor)
{
	// Check for resampling mode
	switch (m_ResampleMode)
	{
...
int img_DrawTransparent(_QUAD dstQuad, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, _PIXEL transparentColor)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawTransparentNearestNeighbour(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, int alpha, _PIXEL transparentColor)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawTransparentBilinear(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, int alpha, _PIXEL transparentColor)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawTransparentBicubic(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, int alpha, _PIXEL transparentColor)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawTransparent(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, int alpha, _PIXEL transparentColor)
{
	// Check for resampling mode
	switch (m_ResampleMode)
	{
...
int img_DrawTransparent(_QUAD dstQuad, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, int alpha, _PIXEL transparentColor)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawBlended(int dstX, int dstY, int width, int height, CBitmapEx& bitmapEx, int srcX, int srcY, int startAlpha, int endAlpha, DWORD mode)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
		...
		int _pitch = bitmapEx.GetPitch();
		int _bpp = bitmapEx.GetBpp() >> 3;
		int _startAlpha = max(0, min(startAlpha, 100));
		float _startAlphaPercent = (float)_startAlpha / 100.0f;
		int _endAlpha = max(0, min(endAlpha, 100));
...
int img_DrawBlendedNearestNeighbour(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, int startAlpha, int endAlpha, DWORD mode)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawBlendedBilinear(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, int startAlpha, int endAlpha, DWORD mode)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawBlendedBicubic(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, int startAlpha, int endAlpha, DWORD mode)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawBlended(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, int startAlpha, int endAlpha, DWORD mode)
{
	// Check for resampling mode
	switch (m_ResampleMode)
	{
...
int img_DrawMasked(int dstX, int dstY, int width, int height, CBitmapEx& bitmapEx, int srcX, int srcY, _PIXEL transparentColor)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawAlpha(int dstX, int dstY, int width, int height, CBitmapEx& bitmapEx, int srcX, int srcY, int alpha, _PIXEL alphaColor)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
		...
		for(i=0; i<=_height; i++)
		{
			...
			for(j=0; j<=_width; j++)
			{
				...
				fixed f_dgreen = itofx(_GetGValue(pixel2));
				fixed f_dblue = itofx(_GetBValue(pixel2));
				uchar red = (uchar)fxtoi(Mulfx(f_dred, f_alphaDst) + Mulfx(f_sred, f_1-f_alphaDst));
				uchar green = (uchar)fxtoi(Mulfx(f_dgreen, f_alphaDst) + Mulfx(f_sgreen, f_1-f_alphaDst));
				uchar blue = (uchar)fxtoi(Mulfx(f_dblue, f_alphaDst) + Mulfx(f_sblue, f_1-f_alphaDst));
				lpDstData[dwDstTotalOffset>>2] = _RGB(red, green, blue);
...
int img_DrawCombined(int dstX, int dstY, int width, int height, CBitmapEx& bitmapEx, int srcX, int srcY, DWORD mode)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawCombinedNearestNeighbour(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, DWORD mode)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawCombinedBilinear(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, DWORD mode)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawCombinedBicubic(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, DWORD mode)
{
	// Check for valid bitmap
	if (bitmapEx.IsValid())
	{
...
int img_DrawCombined(int dstX, int dstY, int dstWidth, int dstHeight, CBitmapEx& bitmapEx, int srcX, int srcY, int srcWidth, int srcHeight, DWORD mode)
{
	// Check for resampling mode
	switch (m_ResampleMode)
	{
...
int img_DrawTextA(int dstX, int dstY, LPSTR lpszText, _PIXEL textColor, int textAlpha, LPTSTR lpszFontName, int fontSize, BOOL bBold, BOOL bItalic)
{
	// Check for valid bitmap
	if ((lpszText != NULL))
	{
		...
		HDC hDC = ::GetDC(NULL);
		int iFontHeight = -MulDiv(fontSize, ::GetDeviceCaps(hDC, LOGPIXELSY), 72);
		...
		for(k=0; k<iLen; k++)
		{
			...
			else
			{
				...
				int iSize = ::GetGlyphOutlineA(hDC, lpszText[k], GGO_GRAY8_BITMAP, &gm, 0, NULL, &m2);

				// Check for valid letter
				if (lpszText[k] != ' ')
				{
					// Allocate font letter buffer
					uchar* lpFontBitmap = (uchar*)pmalloc(iSize*sizeof(uchar));

					// Get font letter data
					::GetGlyphOutlineA(hDC, lpszText[k], GGO_GRAY8_BITMAP, &gm, iSize, lpFontBitmap, &m2);

					// Calculate letter params
					int iVerticalSkip = sz.cy - gm.gmBlackBoxY + (gm.gmBlackBoxY-gm.gmptGlyphOrigin.y);
					int iHorizontalSkip = gm.gmptGlyphOrigin.x;
					int pitch = gm.gmBlackBoxX;
					while ((pitch & 3) != 0)
						pitch++;

					// Draw single letter
					int iVerticalOffset = 0;
					int cy = (int)gm.gmBlackBoxY;
					int cx = (int)gm.gmBlackBoxX;
					for(i=0; i<cy; i++)
					{
						int iHorizontalOffset = 0;
						for(j=0; j<cx; j++)
						{
							int index = iVerticalOffset + iHorizontalOffset;
							...
							uchar red = (uchar)fxtoi(Mulfx(f_alpha,f_sred) + Mulfx(f_1malpha,f_dred));
							uchar green = (uchar)fxtoi(Mulfx(f_alpha,f_sgreen) + Mulfx(f_1malpha,f_dgreen));
							uchar blue = (uchar)fxtoi(Mulfx(f_alpha,f_sblue) + Mulfx(f_1malpha,f_dblue));
...
int img_DrawTextW(int dstX, int dstY, LPWSTR lpszText, _PIXEL textColor, int textAlpha, LPTSTR lpszFontName, int fontSize, BOOL bBold, BOOL bItalic)
{
	// Check for valid bitmap
	if ((lpszText != NULL))
	{
		// Draw text on the bitmap
		int iLen = (int)wcslen(lpszText);
		HDC hDC = ::GetDC(NULL);
		int iFontHeight = -MulDiv(fontSize, ::GetDeviceCaps(hDC, LOGPIXELSY), 72);
		int iWeight = FW_NORMAL;
		if (bBold)
		...
		for(k=0; k<iLen; k++)
		{
			...
			{
				// Get font letter info
				int iSize = ::GetGlyphOutlineW(hDC, lpszText[k], GGO_GRAY8_BITMAP, &gm, 0, NULL, &m2);
...
int img_SetPixel(int x, int y, _PIXEL pixel)
{
	// Check for valid bitmap
	
	{
		// Set pixel color
		int _x = max(0, min(im->w-1, x));
		int _y = max(0, min(im->h-1, y));
		DWORD dwTotalOffset = (im->h-1-_y)*m_iPitch + _x*m_iBpp;
		LPDWORD lpData = (LPDWORD)m_lpData;
		lpData[dwTotalOffset>>2] = pixel;
	}
}

_PIXEL img_GetPixel(int x, int y)
{
...
_PIXEL img_RGB2HSV(_PIXEL rgbPixel)
{
	_PIXEL hsvPixel = 0;

	// Convert RGB value to HSV value
	uchar red = _GetRValue(rgbPixel);
	uchar green = _GetGValue(rgbPixel);
	uchar blue = _GetBValue(rgbPixel);
	uchar temp = (red + green + blue) / 3;
	...
	if (_hue >= 360.0f)
		_hue = _hue - 360.0f;
	uchar hue = (uchar)((_hue / 360.0f) * 255.0f);
	uchar saturation = (uchar)((_saturation / 100.0f) * 255.0f);
	uchar value = (uchar)((_value / 100.0f) * 255.0f);
	hsvPixel = _RGB(hue, saturation, value);

	return hsvPixel;
}

_PIXEL img_HSV2RGB(_PIXEL hsvPixel)
{
	_PIXEL rgbPixel = 0;

	// Convert HSV value to RGB value
	uchar hue = _GetRValue(hsvPixel);
	uchar saturation = _GetGValue(hsvPixel);
	uchar value = _GetBValue(hsvPixel);
	...
	uchar red = (uchar)_red;
	uchar green = (uchar)_green;
	uchar blue = (uchar)_blue;
	rgbPixel = _RGB(red, green, blue);

	return rgbPixel;
}

int img_ConvertToHSV()
{
	// Check for valid bitmap
...
int img_ReplaceColor(int x, int y, _PIXEL newColor, int alpha, int error, BOOL bImage)
{
	// Check for valid bitmap
	if ((IsValid()) && (m_ColorMode == CM_RGB))
	{
		...
		uchar reffRed = _GetRValue(reffPixel);
		uchar reffGreen = _GetGValue(reffPixel);
		uchar reffBlue = _GetBValue(reffPixel);
		...
		if (bImage)
		{
			...
			for(i=0; i<im->h; i++)
			{
				...
				for(j=0; j<im->w; j++)
				{
					...
					uchar currRed = _GetRValue(currPixel);
					uchar currGreen = _GetGValue(currPixel);
					uchar currBlue = _GetBValue(currPixel);
					...
					if (_diff < _error)
					{
						...
						uchar red = (uchar)fxtoi(Mulfx(f_sred, f_1-f_alpha) + Mulfx(f_dred, f_alpha));
						uchar green = (uchar)fxtoi(Mulfx(f_sgreen, f_1-f_alpha) + Mulfx(f_dgreen, f_alpha));
						uchar blue = (uchar)fxtoi(Mulfx(f_sblue, f_1-f_alpha) + Mulfx(f_dblue, f_alpha));
						lpDstData[dwTotalOffset>>2] = _RGB(red, green, blue);
					}
		...
		else
		{
			...
			int _x = max(0, min(im->w-1, x));
			int _y = max(0, min(im->h-1, im->h-1-y));

			// Create mask
			_LPPOINT pPixels = (_LPPOINT)pmalloc(im->h*im->w*sizeof(_POINT));
			int count = 0;
			pPixels[count].x = _x;
			pPixels[count].y = _y;
			count++;
			LPBOOL pMask = (LPBOOL)pmalloc(im->h*im->w*sizeof(BOOL));
			memset(pMask, FALSE, im->h*im->w*sizeof(BOOL));
			pMask[pPixels[count-1].y*im->h+pPixels[count-1].x] = TRUE;
			...
			do
			{
				...
				float _valDiff = (float)((currBlue-reffBlue)*(currBlue-reffBlue));
				float _diff = _hueDiff + _satDiff + _valDiff;
				if ((_diff < _error) && (pMask[pPixels[count].y*im->h+pPixels[count].x]))
				{
					fixed f_sred = itofx(_GetRValue(lpDstData[dwTotalOffset>>2]));
					fixed f_sgreen = itofx(_GetGValue(lpDstData[dwTotalOffset>>2]));
					fixed f_sblue = itofx(_GetBValue(lpDstData[dwTotalOffset>>2]));
				...
				if (_x > 0)
				{
					...
					float _valDiff = (float)((currBlue-reffBlue)*(currBlue-reffBlue));
					float _diff = _hueDiff + _satDiff + _valDiff;
					if ((_diff < _error) && (!pMask[_y*im->h+_x-1]))
					{
						pPixels[count].x = _x - 1;
						pPixels[count].y = _y;
						count++;
						pMask[_y*im->h+_x-1] = TRUE;
					}
				}
				if (_y > 0)
				{
					...
					float _valDiff = (float)((currBlue-reffBlue)*(currBlue-reffBlue));
					float _diff = _hueDiff + _satDiff + _valDiff;
					if ((_diff < _error) && (!pMask[(_y-1)*im->h+_x]))
					{
						pPixels[count].x = _x;
						pPixels[count].y = _y - 1;
						count++;
						pMask[(_y-1)*im->h+_x] = TRUE;
					}
				}
				if (_x < im->w-1)
				{
...
int img_CreateFireEffect()
{
	...
	
	{
		...
		if (m_lpFire != NULL)
			pfree(m_lpFire);
		m_lpFire = (uchar*)pmalloc(im->h*im->w*sizeof(uchar));
		memset(m_lpFire, 0, im->h*im->w*sizeof(uchar));
...
int img_UpdateFireEffect(BOOL bLarge, int iteration, int height)
{
	// Check for valid bitmap
	if ((m_lpFire != NULL))
	{
		int i, j;
		int _height = max(4, min(128, height));
		int _heightSize = 4*_height + 1;
		fixed f_w1 = ftofx(0.299f);
		fixed f_w2 = ftofx(0.587f);
		fixed f_w3 = ftofx(0.114f);
			...
			for (i=0; i<im->h; i++)
			{
				for (j=0; j<im->w; j++)
				{
					if (bLarge)
					{
						...
						int _x4 = j % im->w;
						int _y4 = (i + 3) % im->h;
						DWORD dwOffset1 = (_y1 * im->w) + _x1;
						DWORD dwOffset2 = (_y2 * im->w) + _x2;
						DWORD dwOffset3 = (_y3 * im->w) + _x3;
						DWORD dwOffset4 = (_y4 * im->w) + _x4;
						m_lpFire[dwOffset] = (uchar)(((m_lpFire[dwOffset1] + m_lpFire[dwOffset2] + m_lpFire[dwOffset3] + m_lpFire[dwOffset4]) * _height) / _heightSize);
						dwOffset++;
					}
					else
					{
		...
		dwOffset = 0;
		DWORD dwHorizontalOffset;
		DWORD dwVerticalOffset = (im->h-1) * m_iPitch;
		DWORD dwTotalOffset;
		LPDWORD lpDstData = (LPDWORD)m_lpData;
...
int img_CreateWaterEffect()
{
	...
	
	{
		...
		if (m_lpWaterHeightField1 != NULL)
			pfree(m_lpWaterHeightField1);
		m_lpWaterHeightField1 = (int*)pmalloc(im->h*im->w*sizeof(int));
		if (m_lpWaterHeightField2 != NULL)
			pfree(m_lpWaterHeightField2);
		m_lpWaterHeightField2 = (int*)pmalloc(im->h*im->w*sizeof(int));
		memset(m_lpWaterHeightField1, 0, im->h*im->w*sizeof(int));
		memset(m_lpWaterHeightField2, 0, im->h*im->w*sizeof(int));
...
int img_UpdateWaterEffect(int iteration)
{
	// Check for valid bitmap
	if ((m_lpWaterHeightField1 != NULL) && (m_lpWaterHeightField2 != NULL))
	{
		for(k=0; k<iteration; k++)
		{
			// Calculate water params
			int *lpOldWaterMap, *lpNewWaterMap;
			...
			for(i=1; i<im->h-1; i++)
			{
				...
				for(j=1; j<im->w-1; j++)
				{
					...
					// Update buffers
					DWORD dwOffset1 = dwTotalOffset + im->w;
					DWORD dwOffset2 = dwTotalOffset - im->w;
					DWORD dwOffset3 = dwTotalOffset + 1;
					DWORD dwOffset4 = dwTotalOffset - 1;
					DWORD dwOffset5 = dwTotalOffset - im->w - 1;
					DWORD dwOffset6 = dwTotalOffset - im->w + 1;
					DWORD dwOffset7 = dwTotalOffset + im->w - 1;
					DWORD dwOffset8 = dwTotalOffset + im->w + 1;
					int newHeight = ((lpOldWaterMap[dwOffset1] + 
		...
		// Update water
		DWORD dwSrcHorizontalOffset;
		DWORD dwSrcVerticalOffset = im->w;
		DWORD dwSrcTotalOffset;
		int* lpWaterData = &m_lpWaterHeightField1[0];
		DWORD dwDstHorizontalOffset;
		DWORD dwDstVerticalOffset = (im->h-1) * m_iPitch;
		DWORD dwDstTotalOffset;
		LPDWORD lpDstData = (LPDWORD)m_lpData;
		LPDWORD lpSrcData = (LPDWORD)m_pWaterBitmap->GetData();
		int iIndex;
		int iTotalSize = im->w * im->h;
		DWORD dwOffset;
		for(i=1; i<im->h-1; i++)
		{
			...
			for(j=1; j<im->w-1; j++)
			{
				...
				// Update bitmap
				int dy = lpWaterData[dwSrcTotalOffset] - lpWaterData[dwSrcTotalOffset+im->w];
				int dx = lpWaterData[dwSrcTotalOffset] - lpWaterData[dwSrcTotalOffset+1];
				iIndex = dwSrcTotalOffset + (dy>>m_iLightModifier)*im->w + (dx>>m_iLightModifier);
				if ((iIndex >= 0) && (iIndex < iTotalSize))
				{
					...
					{
						uchar red = _GetRValue(lpSrcData[dwOffset>>2]);
						uchar green = _GetGValue(lpSrcData[dwOffset>>2]);
						uchar blue = _GetBValue(lpSrcData[dwOffset>>2]);
						red = (uchar)max(0, min(255, red-dx));
						green = (uchar)max(0, min(255, green-dx));
						blue = (uchar)max(0, min(255, blue-dx));
						lpDstData[dwDstTotalOffset>>2] = _RGB(red, green, blue);
					}
				}
...
int img_MakeWaterBlob(int x, int y, int size, int height)
{
	// Check for valid bitmap
	if ((m_lpWaterHeightField1 != NULL) && (m_lpWaterHeightField2 != NULL))
	{
		...
		// Make water blob
		int _x = max(1, min(im->w-2, x));
		int _y = max(1, min(im->h-2, y));
		int qsize = size * size;
		int left = -size;
		if((_x-size) < 1)
			left -= (_x - size - 1);
		int right = size;
		if((_x+size) > im->w-1)
			right -= (_x + size - im->w + 1);
		int top = -size;
		if((_y-size) < 1)
			top -= (_y - size - 1);
		int bottom = size;
		if((_y+size) > im->h-1)
			bottom -= (_y + size - im->h + 1);
		for(int cy=top; cy<bottom; cy++)
		{
			...
			for(int cx=left; cx<right; cx++)
			{
				if(cx*cx + cyq < qsize)
				{
					DWORD dwOffset = (cy+_y)*im->w + (cx+_x);
...
int img_CreateSmokeEffect()
{
	...
	
	{
		...
		if (m_lpSmokeField != NULL)
			pfree(m_lpSmokeField);
		m_lpSmokeField = (float*)pmalloc(_NOISE_HEIGHT*_NOISE_WIDTH*_NOISE_DEPTH*sizeof(float));
		...
		srand(GetTickCount());
		DWORD dwOffset = 0;
		for(i=0; i<_NOISE_HEIGHT; i++)
		{
			for(j=0; j<_NOISE_WIDTH; j++)
			{
				for(k=0; k<_NOISE_DEPTH; k++)
				{
...
int img_UpdateSmokeEffect(int offsetX, int offsetY, int offsetZ)
{
	// Check for valid bitmap
	
	
		// Update smoke buffer
		int k = GetTickCount() / offsetZ;
		int shiftY = GetTickCount() / offsetY;
		int shiftX = GetTickCount() / offsetX;
		DWORD dwHorizontalOffset;
		DWORD dwVerticalOffset = 0;
		DWORD dwTotalOffset;
		...
		for(i=0; i<im->h; i++)
		{
			...
			for(j=0; j<im->w; j++)
			{
				...
				float value = 0.0f;
				for(_size=size; _size>=1; _size=_size>>1)
				{
					float _i = (float)(i+shiftY) / (float)_size;
					float _j = (float)(j+shiftX) / (float)_size;
					float _k = (float)k / (float)_size;
					float fractY = _i - (int)_i;
					float fractX = _j - (int)_j;
					float fractZ = _k - (int)_k;
					int y1 = (int(_i) + _NOISE_HEIGHT) % _NOISE_HEIGHT;
					int x1 = (int(_j) + _NOISE_WIDTH) % _NOISE_WIDTH;
					int z1 = (int(_k) + _NOISE_DEPTH) % _NOISE_DEPTH;
					int y2 = (y1 + _NOISE_HEIGHT - 1) % _NOISE_HEIGHT;
					int x2 = (x1 + _NOISE_WIDTH - 1) % _NOISE_WIDTH;
					int z2 = (z1 + _NOISE_DEPTH - 1) % _NOISE_DEPTH;
...
_SIZE img_MeasureTextA(LPSTR lpszText, LPTSTR lpszFontName, int fontSize, BOOL bBold, BOOL bItalic)
{
	_SIZE szText = {0, 0};
...
_SIZE img_MeasureTextW(LPWSTR lpszText, LPTSTR lpszFontName, int fontSize, BOOL bBold, BOOL bItalic)
{
	_SIZE szText = {0, 0};
...
int img_GetRedChannel(uchar* lpBuffer)
{
	// Check for valid bitmap
	if ((lpBuffer != NULL))
	{
...
int img_GetGreenChannel(uchar* lpBuffer)
{
	// Check for valid bitmap
	if ((lpBuffer != NULL))
	{
...
int img_GetBlueChannel(uchar* lpBuffer)
{
	// Check for valid bitmap
	if ((lpBuffer != NULL))
	{
...
int img_GetRedChannelHistogram(int lpBuffer[256], BOOL bPercent)
{
	// Check for valid bitmap
	if ((lpBuffer != NULL))
	{
		// Clear red channel histogram
		memset(lpBuffer, 0, 256*sizeof(int));
		...
		if (bPercent)
		{
			int imageArea = im->w * im->h;
			for(k=0; k<256; k++)
				lpBuffer[k] = (int)((float)lpBuffer[k] / (float)imageArea * 10000.0f);
		}
	}
}

int img_GetGreenChannelHistogram(int lpBuffer[256], BOOL bPercent)
{
	// Check for valid bitmap
	if ((lpBuffer != NULL))
	{
...
int img_GetBlueChannelHistogram(int lpBuffer[256], BOOL bPercent)
{
	// Check for valid bitmap
	if ((lpBuffer != NULL))
	{
