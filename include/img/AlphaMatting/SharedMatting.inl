...
void SharedMatting::loadImage(const char * filename)
{
    ...
    imsetsize(matte, height, width, 1, 1);
}
...
{
  img_t trimap[1] = {0};
    if (!imread(filename, 0, 1, trimap)) {
        cout << "Loading Trimap Failed!" << endl;
        exit(-1);
    }
    uchar * d   = (uchar *)trimap->tt.data;
