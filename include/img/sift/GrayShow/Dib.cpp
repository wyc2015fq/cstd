...
CDib::CDib(const CDib& dib)
{
	...
	m_hDrawDib=NULL;
	m_pDib=NULL;
	bmpFileHeader = dib.bmpFileHeader;
	DWORD size = bmpFileHeader.bfSize-sizeof(BITMAPFILEHEADER);
	m_pDib = new BYTE[size];
	memcpy(m_pDib, dib.m_pDib, size);
	m_pDibBits = GetBits();
}

void CDib::Resize(LONG width, LONG height)
{
	...
	long fullWidth = ( width * GetBiBitCount() + 31)/32 * 4;
	DWORD size = bmpFileHeader.bfSize-sizeof(BITMAPFILEHEADER);
	DWORD bmpHeaderSize = size - GetWidthStep()*GetHeight();
	BYTE *cp = new BYTE[bmpHeaderSize];
	memcpy(cp, m_pDib, bmpHeaderSize);
	
	DWORD nsize = bmpHeaderSize + fullWidth*height;
	bmpFileHeader.bfSize = nsize + sizeof(BITMAPFILEHEADER);
	delete m_pDib;
	m_pDib = new BYTE[nsize];
	memcpy(m_pDib, cp, bmpHeaderSize);
...
LONG CDib::GetWidthStep()
{
	BITMAPINFOHEADER *lpDib=(BITMAPINFOHEADER *)m_pDib;
	return ( lpDib->biWidth * lpDib->biBitCount + 31)/32 * 4;
}
...
CSize CDib::GetDibSaveDim()
{
	BITMAPINFOHEADER *lpDib=(BITMAPINFOHEADER *)m_pDib;
	CSize sizeSaveDim;
	sizeSaveDim.cx	= ( lpDib->biWidth * lpDib->biBitCount + 31)/32 * 4;
	sizeSaveDim.cy	= lpDib->biHeight; 
	return sizeSaveDim;
}