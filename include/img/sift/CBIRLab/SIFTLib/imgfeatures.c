...
{
	CvScalar color = CC_RGB( 255, 255, 255 );
	int i;
...
static void draw_oxfd_feature( IplImage* img, struct feature* feat, CvScalar color )
{
	...
	cvInitMatHeader( &M, 2, 2, CC_64FC1, m, CC_AUTOSTEP );
	cvInitMatHeader( &V, 2, 2, CC_64FC1, v, CC_AUTOSTEP );
	cvInitMatHeader( &E, 2, 1, CC_64FC1, e, CC_AUTOSTEP );
...
static void draw_lowe_feature( IplImage* img, struct feature* feat, CvScalar color )
{
	...
	h1_x = cvRound( blen *  cos( ori + CC_PI / 18.0 ) ) + start_x;
	h1_y = cvRound( blen * -sin( ori + CC_PI / 18.0 ) ) + start_y;
	h2_x = cvRound( blen *  cos( ori - CC_PI / 18.0 ) ) + start_x;
	h2_y = cvRound( blen * -sin( ori - CC_PI / 18.0 ) ) + start_y;
