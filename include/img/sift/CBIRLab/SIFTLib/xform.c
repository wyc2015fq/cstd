...
CvMat* dlt_homog( CvPoint2D64f* pts, CvPoint2D64f* mpts, int n )
{
	...
	D = cvCreateMat( 9, 9, CC_64FC1 );
	VT = cvCreateMat( 9, 9, CC_64FC1 );
	cvSVD( A, D, NULL, VT, CC_SVD_MODIFY_A + CC_SVD_V_T );
	v9 = cvMat( 1, 9, CC_64FC1, NULL );
	cvGetRow( VT, &v9, 8 );
	h = cvMat( 1, 9, CC_64FC1, _h );
	cvCopy( &v9, &h, NULL );
	h = cvMat( 3, 3, CC_64FC1, _h );
...
CvMat* lsq_homog( CvPoint2D64f* pts, CvPoint2D64f* mpts, int n )
{
	...
	A = cvCreateMat( 2*n, 8, CC_64FC1 );
	B = cvCreateMat( 2*n, 1, CC_64FC1 );
	X = cvMat( 8, 1, CC_64FC1, x );
	H = cvCreateMat(3, 3, CC_64FC1);
	cvZero( A );
	for( i = 0; i < n; i++ )
	{
...
CvPoint2D64f persp_xform_pt( CvPoint2D64f pt, CvMat* T )
{
	...
	cvInitMatHeader( &XY, 3, 1, CC_64FC1, xy, CC_AUTOSTEP );
	cvInitMatHeader( &UV, 3, 1, CC_64FC1, uv, CC_AUTOSTEP );
	cvMatMul( T, &XY, &UV );
								 ...
								 struct feature*** matched )
{
	...
	for( i = 0; i < n; i++ )
		...
		{
			rdata = pmalloc( sizeof( struct ransac_data ) );
