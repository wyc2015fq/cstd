				   ...
				   int img_dbl, int descr_width, int descr_hist_bins )
{
	...
	*feat = cvCvtSeqToArray( features, *feat, CC_WHOLE_SEQ );
	for( i = 0; i < n; i++ )
	{
		pfree( (*feat)[i].feature_data );
...
static IplImage* create_init_img( IplImage* img, int img_dbl, double sigma )
{
	...
	if( img_dbl )
	{
		...
		dbl = cvCreateImage( cvSize( img->width*2, img->height*2 ),
			IMG_DEPTH_32F, 1 );
		cvResize( gray, dbl, CC_INTER_CUBIC );
		cvSmooth( dbl, dbl, CC_GAUSSIAN, 0, 0, sig_diff, sig_diff );
		cvReleaseImage( &gray );
		return dbl;
	}
	else
	{
		...
		cvSmooth( gray, gray, CC_GAUSSIAN, 0, 0, sig_diff, sig_diff );
		return gray;
	}
}
...
static IplImage* convert_to_gray32( IplImage* img )
{
	...
	gray32 = cvCreateImage( cvGetSize(img), IMG_DEPTH_32F, 1 );
	if( img->nChannels == 1 )
		gray8 = cvClone( img );
	else
	{
		gray8 = cvCreateImage( cvGetSize(img), IMG_DEPTH_8U, 1 );
		cvCvtColor( img, gray8, CC_BGR2GRAY );
	}
...
static IplImage* downsample( IplImage* img )
{
	...
	cvResize( img, smaller, CC_INTER_NN );
						 ...
						 double* xi, double* xr, double* xc )
{
	...
	H_inv = cvCreateMat( 3, 3, CC_64FC1 );
	cvInvert( H, H_inv, CC_SVD );
	cvInitMatHeader( &X, 3, 1, CC_64FC1, x, CC_AUTOSTEP );
							...
							int c, double xi, double xr, double xc )
{
	...
	cvInitMatHeader( &X, 3, 1, CC_64FC1, x, CC_AUTOSTEP );
	cvInitMatHeader( &T, 1, 1, CC_64FC1, t, CC_AUTOSTEP );
	dD = deriv_3D( dog_pyr, octv, intvl, r, c );
	cvGEMM( dD, &X, 1, NULL, 0, &T,  CC_GEMM_A_T );
	cvReleaseMat( &dD );
...
static struct feature* new_feature( void )
{
	...
	feat = pmalloc( sizeof( struct feature ) );
	memset( feat, 0, sizeof( struct feature ) );
	ddata = pmalloc( sizeof( struct detection_data ) );
...
static void calc_feature_scales( CvSeq* features, double sigma, int intvls )
{
	...
	for( i = 0; i < n; i++ )
	{
		feat = CC_GET_SEQ_ELEM( struct feature, features, i );
		ddata = feat_detection_data( feat );
...
{
	double* hist;
	double mag, ori, w, exp_denom, PI2 = CC_PI * 2.0;
	int bin, i, j;
							 ...
							 double scl, int d, int n )
{
	...
	double cos_t, sin_t, hist_width, exp_denom, r_rot, c_rot, grad_mag,
		grad_ori, w, rbin, cbin, obin, bins_per_rad, PI2 = 2.0 * CC_PI;
	int radius, i, j;
