...
//  float scale = 0.25;		//缩放倍数为倍
    CvSize dst_cvsize;			//目标图像尺寸
 
    /* the first command line parameter must be image file name */
	src = cvLoadImage(filename);
    dst_cvsize.width = dst_width;		//目标图像的宽为源图象宽的scale倍
    dst_cvsize.height =dst_height ;	//目标图像的高为源图象高的scale倍
 
    m_srcImage = cvCreateImage( dst_cvsize, src->depth, src->nChannels);	//构造目标图象
    cvResize(src, m_srcImage, CC_INTER_LINEAR);
...
/*	printf("Generating scale space...\n");*/
	...
	IplImage* imgGray = cvCreateImage(cvGetSize(m_srcImage), IMG_DEPTH_32F , 1);
	...
	{
		cvCvtColor(m_srcImage, imgTemp, CC_BGR2GRAY);
	}
	else
	{
	...
	cvSmooth(imgGray, imgGray, CC_GAUSSIAN, 0, 0, SIGMA_ANTIALIAS);

	// Create an image double the dimensions, resize imgGray and store it in m_gList[0][0]
	m_gList[0][0] = cvCreateImage(cvSize(imgGray->width*2, imgGray->height*2), IMG_DEPTH_32F , 1);
	cvPyrUp(imgGray, m_gList[0][0]);

	// Preblur this base image
	cvSmooth(m_gList[0][0], m_gList[0][0], CC_GAUSSIAN, 0, 0, SIGMA_PREBLUR);
	...
	for(i=0;i<m_numOctaves;i++)
	{
		...
		for(j=1;j<m_numIntervals+3;j++)
		{
			...
			cvSmooth(m_gList[i][j-1], m_gList[i][j], CC_GAUSSIAN, 0, 0, sigma_f);
...
//	printf("Assigning orientations...\n");
	...
	for(i=0;i<m_numOctaves;i++)
	{
		...
		for(j=1;j<m_numIntervals+1;j++)
		{
			...
			cvSmooth(magnitude[i][j-1], imgWeight, CC_GAUSSIAN, 0, 0, 1.5*abs_sigma);
...
// 									printf("Bad Orientation: %f\n", sampleOrient);
						...
						for(k=0;k<NUM_BINS;k++)
						{
							...
							if(hist_orient[k]> 0.8*max_peak)
							{
								...
								CvMat *X = cvCreateMat(3, 3, CC_32FC1);
								CvMat *matInv = cvCreateMat(3, 3, CC_32FC1);
...
//	printf("Extract keypoint descriptors...\n");
	...
	for(unsigned int ikp = 0;ikp<m_numKeypoints;ikp++)
	{
		...
		CvMat *weight = cvCreateMat(FEATURE_WINDOW_SIZE, FEATURE_WINDOW_SIZE, CC_32FC1);
...
CvMat* SIFT::BuildInterpolatedGaussianTable(unsigned int size, double sigma)
{
	...
	double sog=0;
	CvMat* ret = cvCreateMat(size, size, CC_32FC1);
...
// 
// 		cvLine(img, cvPoint(kp.xi, kp.yi), cvPoint(kp.xi, kp.yi), CC_RGB(255,255,255), 3);
// 		cvLine(img, cvPoint(kp.xi, kp.yi), cvPoint(kp.xi+10*cos(kp.orien[0]), kp.yi+10*sin((double)kp.orien[0])), CC_RGB(255,255,255), 1);
// 	}
// 
