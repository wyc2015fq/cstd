...
CGrayShow::~CGrayShow()
{
	...
	{
		delete []m_tOriPixelArray[l];
		delete []m_tResPixelArray[l];
		delete []ImageArray[l] ;
	}
...
void CGrayShow::LoadImage(char *FilePathName)
{
	int i,j;
	IplImage *img;
	//检测是否为空，不为空则要释放内存
	...
	img=cvLoadImage(FilePathName,-1);
// 	if (img==NULL)
// 	{
// 		cout<<FilePathName<<"  "<<"图片格式出错"<<endl;
// 		return ;
// 	}
	...
	BYTE	*colorTable;
	colorTable = (BYTE *)img->imageData;
	int byteBitCount  = img->depth;
