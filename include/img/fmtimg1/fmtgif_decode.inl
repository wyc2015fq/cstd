...
BOOL DecodeExtension(fmtgif* fmt, ihand_t m_strm)
{
  BOOL bContinue;
  unsigned char count;
  unsigned char fc;
...
int gif_get_byte1(fmtgif* fmt, ihand_t m_strm)
{
...
short init_exp(fmtgif* fmt, short size)
{
...
*/
short get_next_code(fmtgif* fmt, ihand_t m_strm)
{
  short i, x;
  uint32 ret;
...
typedef struct CImageIterator {
  int Ity, Itx;
  int h, w, s;
  BYTE* IterImage;
} CImageIterator;
int out_line(fmtgif* fmt, CImageIterator* iter, unsigned char* pixels, int linelen)
{
  long x;
  ...
  if (fmt->biBitCount < 8) {
    for (x = 0; x < iter->w; x++) {
      BYTE pos;
      BYTE* iDst = pixels + (x * fmt->biBitCount >> 3);
...
short decoder(fmtgif* fmt, ihand_t file, CImageIterator* iter, short linewidth, int* pbad_code_count)
{
  ...
  * an fmt->ending code.
  */
  while ((c = get_next_code(fmt, file)) != fmt->ending) {
    ...
    if (c == fmt->clear) {
      ...
      */
      while ((c = get_next_code(fmt, file)) == fmt->clear);
      ...
      *bufptr++ = (BYTE)c;
      ...
      if (--bufcnt == 0) {
        if (iter) {
          if ((ret = (short)out_line(fmt, iter, buf, linewidth)) < 0) {
            pfree(buf);
            return(ret);
          }
        }
      ...
      *sp++ = (BYTE)code;
      ...
      if (fmt->slot < fmt->top_slot) {
        fmt->suffix[fmt->slot] = (BYTE)(fc = (BYTE)code);
  ...
  if (bufcnt != linewidth && iter) {
    ret = (short)out_line(fmt, iter, buf, (linewidth - bufcnt));
  }
...
long seek_next_image(const fmtgif* fmt, long position)
{
  char ch1, ch2;
...
int get_num_frames(fmtgif* fmt, ihand_t m_strm, struct_TabCol* TabColSrc, struct_dscgif* dscgif)
{
  struct_image image;
  ...
  for (bContinue = TRUE; bContinue;) {
    ...
    if (bPreviousWasNull || ch == 0) {
      ...
      case ',': { // image
        ...
        decoder(fmt, m_strm, 0, image.w, &badcode);

        if (badcode) {
          seek_next_image(fmt, pos_start);
        }
        else {
...
void GifMix(CxImage* imgsrc2, struct_image* imgdesc)
{
  ...
  long xmin = imgdesc.l;
  long xmax = min(GetWidth(), (uint32)(imgdesc.l + imgdesc.w));
...
BOOL fmtgif_read_header(ihand_t m_strm, img_t* im, fmtgif* fmt)
{
  if (m_strm == NULL) {
    return FALSE;
  }
  ...
  im->f = get_num_frames(fmt, m_strm, &fmt->TabCol, &fmt->dscgif);

  im->h = fmt->dscgif.scrheight;
  im->w = fmt->dscgif.scrwidth;
  im->c = 4;
  im->s = im->w * im->c;

  //im->tt.data=(unsigned char*)pmalloc(im->h*im->s*im->f);
  //memset(im->tt.data, 0, im->h*im->s*im->f);
  if ((fmt->nFrame < 0) || (fmt->nFrame >= im->f)) {
    return FALSE;
  }
...
BOOL fmtgif_read_data(ihand_t m_strm, img_t* im, fmtgif* fmt)
{
  struct_image image;
  ...
  for (bContinue = TRUE; bContinue;) {
    ...
    if (bPreviousWasNull || ch == 0) {
      int badcode;
      CImageIterator iter[1] = {0};
      int bpp; //<DP> select the correct bit per pixel value

      switch (ch) {
      case '!': { // extension
        bContinue = DecodeExtension(fmt, m_strm);
        break;
      }
      ...
      case ',': { // image
        ...
        if (iImage == 0) {
          ...
          backimage.Clear((BYTE)fmt->gifgce.transpcolindex);
        ...
        pos_start = Tell();
        //if (fmt->interlaced) log << "Interlaced" << endl;
        decoder(fmt, m_strm, iter, image.w, &badcode);
  ...
  if (bTrueColor >= 2 && imaRGB) {
    if (fmt->gifgce.flags & 0x1) {
      imaRGB->SetTransColor(GetPaletteColor((BYTE)fmt->nBkgndIndex));
