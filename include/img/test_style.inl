...
int VistaButton1(HDDC hDC, RECT rect)
{
  // define some colors
  COLORREF ButtonOuterBorder = _RGB(0x75, 0x7D, 0x95);
  COLORREF ButtonBorder = _RGB(0x03, 0x07, 0x0D);
  COLORREF ButtonInnerBorder = _RGB(0xBF, 0xC4, 0xCE);
  COLORREF GlowColor = _RGB(0x30, 0x73, 0xCE);
  COLORREF CheckedGlow = _RGB(0x57, 0xC6, 0xEF);
  COLORREF GlossyEffectNorth = _RGB(0x68, 0x7C, 0xAC);
  COLORREF GlossyEffectSouth = _RGB(0xAA, 0xB5, 0xD0);
  COLORREF CheckedButtonFill = _RGB(0x18, 0x38, 0x9E);
  // some temporary stacks
  Style _s[10] = {0};
  Style* s = _s;
  // the main rect
  RECT vrect = (rect), nrect = (rect);
  RECT glr = (rect);
  nrect.bottom = (rect.top + rect.bottom) / 2;
  // define the glow rect
  InflateRect(&glr, 90, 10);
  OffsetRect(&glr, 0, RCH(&vrect) / 2);
  // set the radius
  SetShape(s, vrect, ROUNDRECT, 2);
  // set the OuterBorder size and color
  s->OuterBorder.width = 1;
  s->OuterBorder.ulclr = ButtonOuterBorder;
  s->OuterBorder.brclr = ButtonOuterBorder;
  s->LayerType = TRIGRAD;
  s->gradMode = VERT;
  ARRINIT4(s->clr, _SetAlpha(GlossyEffectSouth, 25), GlossyEffectNorth, Black, Black);
  ARRINIT4(s->rParams, 0.0f, 0.5f, 0.5f, 1.0f);
  //PaintStack(hDC, s);
  // Create Face2 Layers
  s[1] = *s;
  ++s;
  s->MiddleBorder.width = 1;
  s->MiddleBorder.ulclr = ButtonBorder;
  s->MiddleBorder.brclr = ButtonBorder;
  s->InnerBorder.width = 1;
  s->InnerBorder.ulclr = ButtonInnerBorder;
  s->InnerBorder.brclr = ButtonInnerBorder;
  PaintStyle(hDC, s);
#if 0
  //--------------------- Hover Layer ----------------------//
...
/////////////////////////////////////////
VistaButton2::VistaButton2(RECT rect, Clr color)
{
  // some temporary stacks
  Stack VB2Base, VB2Glow;
  // the main rect
  RECT vrect(rect), nrect;
  // define the glow rect
  RECT glr(rect);
...
///////// AQUA BUTTON STYLE /////////////
/////////////////////////////////////////
AquaButton::AquaButton(RECT rect, Clr face, Clr hot, Clr outerstroke, CString str, int size)
{
  // some temporary stacks
  ...
  Clr bordercolor(_RGB(143, 190, 232));
  Base.SetRect(r);
...
///////// RAISED FRAME STACK ////////////
/////////////////////////////////////////
RaisedFrame::RaisedFrame(RECT rect, Clr base, Clr border, int bwidth)
{
  RFrame.SetRect(rect);
...
///////// RAISED BORDER STACK ///////////
/////////////////////////////////////////
RaisedBorder::RaisedBorder(RECT rect, Clr border, Clr highlight, int bwidth)
{
...
///////// RAISED DIALOG FRAME ///////////
/////////////////////////////////////////
RaisedDlgFrame::RaisedDlgFrame(RECT drect, RECT crect, Clr border, Clr highlight, int bwidth)
{
  // set the main stack
