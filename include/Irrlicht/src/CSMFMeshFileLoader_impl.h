...
IAnimatedMesh* CSMFMeshFileLoader::createMesh(io::IReadFile* file)
{
  ...
  s32 limbCount;
  s32 i;
  io::BinaryFile::read(file, &version, 2);
  io::BinaryFile::read(file, &flags, 1);
  io::BinaryFile::read(file, &limbCount, 4);
  // load mesh data
...
void CSMFMeshFileLoader::loadLimb(io::IReadFile* file, SMesh* mesh, const core::matrix4& parentTransformation)
{
  ...
  io::BinaryFile::read(file, &vertexCount, 2);
  ...
  for (i = 0; i < triangleCount; ++i) {
    u16 index;
    io::BinaryFile::read(file, &index, 2);
...
#endif
void BinaryFile::read(io::IReadFile* file, void* out, int len, bool bigEndian)
{
  file->read(out, len);
