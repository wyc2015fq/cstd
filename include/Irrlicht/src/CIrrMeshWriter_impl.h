...
bool CIrrMeshWriter::writeMesh(io::IWriteFile* file, scene::IMesh* mesh, s32 flags)
{
  ...
  core::stringw infoComment = " This file contains a static mesh in the Irrlicht Engine format with ";
...
void CIrrMeshWriter::writeBoundingBox(const core::aabbox3df& box)
{
  Writer->writeElement("boundingBox", true,
      "minEdge", getVectorAsStringLine(box.MinEdge).c_str(),
      "maxEdge", getVectorAsStringLine(box.MaxEdge).c_str());
}
...
void CIrrMeshWriter::writeMeshBuffer(const scene::IMeshBuffer* buffer)
{
  ...
  Writer->writeElement("vertices", false,
      "type", vertexTypeStr.c_str(),
      "vertexCount", core::stringw(buffer->getVertexCount()).c_str());
  Writer->writeLineBreak();
  ...
  Writer->writeElement("indices", false,
      "indexCount", core::stringw(buffer->getIndexCount()).c_str());
  Writer->writeLineBreak();
