...
#if defined(_IRR_COMPILE_WITH_JOYSTICK_EVENTS_)
  for (int joystick = 0; joystick < ActiveJoysticks.size(); ++joystick) {
...
bool CIrrDeviceLinux::present(video::IImage* image, void* windowId, core::rect<s32>* srcRect)
{
  ...
  case 16:
    ...
    break;
  case 24:
    destColor = video::ECF_R8G8B8;
    break;
  case 32:
    destColor = video::ECF_A8R8G8B8;
    break;
  default:
...
void CIrrDeviceLinux::pollJoysticks()
{
  ...
  for (int j = 0; j < ActiveJoysticks.size(); ++j) {
...
void CIrrDeviceLinux::CCursorControl::clearCursors()
{
  ...
  for (i = 0; i < Cursors.size(); ++i) {
    for (int f = 0; f < Cursors[i].Frames.size(); ++f) {
...
gui::ECURSOR_ICON CIrrDeviceLinux::CCursorControl::addIcon(const gui::SCursorSprite& icon)
{
  ...
  if (icon.SpriteId >= 0) {
    ...
    int i;
    for (i = 0; i < icon.SpriteBank->getSprites()[icon.SpriteId].Frames.size(); ++i) {
...
void CIrrDeviceLinux::CCursorControl::changeIcon(gui::ECURSOR_ICON iconId, const gui::SCursorSprite& icon)
{
  ...
  int i;
  for (i = 0; i < Cursors[iconId].Frames.size(); ++i) {
