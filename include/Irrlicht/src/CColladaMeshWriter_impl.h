...
void CColladaMeshWriter::writeNodeLights(irr::scene::ISceneNode* node)
{
  ...
  if (node->getType() == ESNT_LIGHT) {
    ...
    case video::ELT_POINT:
      ...
      writeNode("constant_attenuation ", core::stringw(lightData.Attenuation.X).c_str());
      writeNode("linear_attenuation  ", core::stringw(lightData.Attenuation.Y).c_str());
      writeNode("quadratic_attenuation", core::stringw(lightData.Attenuation.Z).c_str());
    ...
    case video::ELT_SPOT:
      ...
      writeNode("linear_attenuation  ", core::stringw(lightData.Attenuation.Y).c_str());
      writeNode("quadratic_attenuation", core::stringw(lightData.Attenuation.Z).c_str());
      writeNode("falloff_angle", core::stringw(lightData.OuterCone * core::RADTODEG).c_str());
      writeNode("falloff_exponent", core::stringw(lightData.Falloff).c_str());
...
void CColladaMeshWriter::writeNodeCameras(irr::scene::ISceneNode* node)
{
  ...
  if (isCamera(node)) {
    ...
    if (cameraNode->isOrthogonal()) {
      ...
      Writer->writeLineBreak();
      //      writeNode("xmag", core::stringw("1.0").c_str()); // TODO: do we need xmag, ymag?
      //      writeNode("ymag", core::stringw("1.0").c_str());
      writeNode("aspect_ratio", core::stringw(cameraNode->getAspectRatio()).c_str());
      writeNode("znear", core::stringw(cameraNode->getNearValue()).c_str());
      writeNode("zfar", core::stringw(cameraNode->getFarValue()).c_str());
      ...
      Writer->writeLineBreak();
      writeNode("yfov", core::stringw(cameraNode->getFOV()*core::RADTODEG).c_str());
      writeNode("aspect_ratio", core::stringw(cameraNode->getAspectRatio()).c_str());
      writeNode("znear", core::stringw(cameraNode->getNearValue()).c_str());
      writeNode("zfar", core::stringw(cameraNode->getFarValue()).c_str());
...
void CColladaMeshWriter::writeAllMeshGeometries()
{
  ...
  for (; !it.atEnd(); it++) {
    ...
    if (getGeometryWriting() == ECGI_PER_MESH_AND_MATERIAL && colladaMesh.GeometryMeshMaterials.size() > 1) {
      int i;
      for (i = 0; i < colladaMesh.GeometryMeshMaterials.size(); ++i) {
...
irr::core::stringw CColladaMeshWriter::toString(const irr::video::ECOLOR_FORMAT format) const
{
  ...
  case video::ECF_A1R5G5B5:
    return irr::core::stringw("A1R5G5B5");
  case video::ECF_R5G6B5:
    return irr::core::stringw("R5G6B5");
  case video::ECF_R8G8B8:
    return irr::core::stringw("R8G8B8");
  case video::ECF_A8R8G8B8:
    return irr::core::stringw("A8R8G8B8");
  default:
...
irr::core::stringw CColladaMeshWriter::nameForMesh(const scene::IMesh* mesh, int instance) const
{
  ...
  return irr::core::stringw("missing_name_generator");
}
...
irr::core::stringw CColladaMeshWriter::nameForMaterial(const video::SMaterial& material, int materialId, const scene::IMesh* mesh, const scene::ISceneNode* node)
{
  ...
  if (nameGenerator) {
    ...
    matName = irr::core::stringw("missing_name_generator");
  }
  if (getExportSMaterialsOnlyOnce()) {
...
{
  int i;
  for (i = 0; i < MaterialNameCache.size(); ++i) {
...
{
  if (trilinear) {
    return core::stringw("LINEAR_MIPMAP_LINEAR");
  }
  else if (bilinear) {
    return core::stringw("LINEAR_MIPMAP_NEAREST");
  }
  return core::stringw("NONE");
}
...
void CColladaMeshWriter::writeMeshGeometry(const irr::core::stringw& meshname, scene::IMesh* mesh)
{
  ...
  Writer->writeLineBreak();
  core::stringw vertexCountStr((int)totalVertexCount * 3);
  ...
  Writer->writeElement("technique_common", false);
  Writer->writeLineBreak();
  vertexCountStr = core::stringw((int)totalVertexCount);
  ...
  Writer->writeLineBreak();
  vertexCountStr = core::stringw((int)totalVertexCount * 2);
  ...
  Writer->writeLineBreak();
  vertexCountStr = core::stringw((int)totalVertexCount * 3);
  ...
  if (totalTCoords2Count) {
    ...
    Writer->writeLineBreak();
    vertexCountStr = core::stringw((int)totalTCoords2Count * 2);
    ...
    Writer->writeElement("technique_common", false);
    Writer->writeLineBreak();
    vertexCountStr = core::stringw((int)totalTCoords2Count);
  ...
  for (i = 0; i < mesh->getMeshBufferCount(); ++i) {
    ...
    core::stringw strPolyCount((int)polyCount);
    ...
    Writer->writeLineBreak();
    Writer->writeElement("input", true, "semantic", "NORMA", "source", toRef(meshNormalId).c_str(), "offset", "2");
    Writer->writeLineBreak();
...
void CColladaMeshWriter::writeColorFx(const video::SMaterial& material, const char* colorname, E_COLLADA_COLOR_SAMPLER cs, const char* attr1Name, const char* attr1Value)
{
