...
IAnimatedMesh* CDMFLoader::createMesh(io::IReadFile* file)
{
  ...
  if (GetDMFHeader(dmfRawFile, header)) {
    ...
    os::Printer::log("Loading materials", core::stringc((int)header.numMaterials).c_str());
#endif
...
#ifdef _IRR_DMF_DEBUG_
      os::Printer::log("Polygon with #vertices", core::stringc((int)faces[i].numVerts).c_str());
#endif
