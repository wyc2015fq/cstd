...
void CGUITable::checkScrollbars()
{
  ...
  if (TotalItemWidth > clientClip.getWidth()) {
    ...
    HorizontalScrollBar->setMax(core::max_((int)0, (int)(TotalItemWidth - clientClip.getWidth())));
  }
  // needs vertical scroll be visible?
  if (TotalItemHeight > clientClip.getHeight()) {
    clientClip.LowerRightCorner.X -= scrollBarSize;
    VerticalScrollBar->setVisible(true);
    VerticalScrollBar->setMax(core::max_((int)0, (int)(TotalItemHeight - clientClip.getHeight())));
...
void CGUITable::draw()
{
  ...
  for (i = 0 ; i < Columns.size() ; ++i) {
    const char* text = Columns[i].Name.c_str();
    u32 colWidth = Columns[i].Width;
