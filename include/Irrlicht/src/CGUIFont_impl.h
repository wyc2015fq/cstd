...
bool CGUIFont::load(io::IXMLReader* xml)
{
  ...
  while (xml->read()) {
    if (io::EXN_ELEMENT == xml->getNodeType()) {
      if (core::stringw("Texture") == xml->getNodeName()) {
        // add a texture
        core::stringc fn = xml->getAttributeValue("filename");
        u32 i = (u32)xml->getAttributeValueAsInt("index");
        core::stringw alpha = xml->getAttributeValue("hasAlpha");
      ...
      else if (core::stringw("c") == xml->getNodeName()) {
        ...
        // parse rectangle
        core::stringc rectstr = xml->getAttributeValue("r");
...
{
  core::map<int, s32>::Node* n = CharacterMap.find(c);
  if (n) {
    return n->getValue();
  }
  else {
...
core::dimension2d<u32> CGUIFont::getDimension(const char* text) const
{
