...
struct SSkinMeshBuffer : public IMeshBuffer {
  ...
  virtual void recalculateBoundingBox() {
    ...
    case video::EVT_STANDARD: {
      if (Vertices_Standard.empty()) {
        ...
        for (i = 1; i < Vertices_Standard.size(); ++i) {
    ...
    case video::EVT_2TCOORDS: {
      if (Vertices_2TCoords.empty()) {
        ...
        for (i = 1; i < Vertices_2TCoords.size(); ++i) {
    ...
    case video::EVT_TANGENTS: {
      if (Vertices_Tangents.empty()) {
        ...
        for (i = 1; i < Vertices_Tangents.size(); ++i) {
  ...
  virtual void convertTo2TCoords() {
    if (VertexType == video::EVT_STANDARD) {
      for (int n = 0; n < Vertices_Standard.size(); ++n) {
  ...
  virtual void convertToTangents() {
    if (VertexType == video::EVT_STANDARD) {
      ...
      for (int n = 0; n < Vertices_2TCoords.size(); ++n) {
        video::S3DVertexTangents Vertex;
