...
bool CXMeshFileLoader::load(io::IReadFile* file)
{
  ...
  for (int n = 0; n < Meshes.size(); ++n) {
    ...
    if (!mesh->HasVertexColors) {
      for (int j = 0; j < mesh->FaceMaterialIndices.size(); ++j) {
...
#ifdef BETTER_MESHBUFFER_SPLITTING_FOR_X
    {
      ...
      for (int j = 0; j < mesh->WeightJoint.size(); ++j) {
        ...
        if (verticesLinkBuffer[id].size() == 1) {
          ...
          for (int k = 1; k < verticesLinkBuffer[id].size(); ++k) {
...
bool CXMeshFileLoader::parseDataObjectMeshMaterialList(SXMesh& mesh)
{
  ...
  u32 ind = 0;
  for (int tfi = 0; tfi < mesh.IndexCountPerFace.size(); ++tfi) {
    if (tfi < nFaceIndices) {
      ind = readInt();
    }
  ...
  while (true) {
    ...
    if (objectName.size() == 0) {
      ...
      int i;
      for (i = 0; i < TemplateMaterials.size(); ++i)
