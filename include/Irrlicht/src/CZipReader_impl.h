...
IReadFile* CZipReader::createAndOpenFile(u32 index)
{
  ...
  case 8: {
    ...
    if (!pBuf) {
      snprintf(buf, 64, "Not enough memory for decompressing %s", Files[index].FullName.c_str());
    ...
    if (err != Z_OK) {
      snprintf(buf, 64, "Error decompressing %s", Files[index].FullName.c_str());
      os::Printer::log(buf, ELL_ERROR);
      delete [] pBuf;
      return 0;
    }
  ...
  case 99:
    ...
    return 0;
  default:
    snprintf(buf, 64, "file has unsupported compression method. %s", Files[index].FullName.c_str());
    os::Printer::log(buf, ELL_ERROR);
    return 0;
  };
}
