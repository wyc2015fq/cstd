...
CSkinnedMesh::~CSkinnedMesh()
{
  ...
  for (int j = 0; j < LocalBuffers.size(); ++j) {
    if (LocalBuffers[j]) {
      LocalBuffers[j]->drop();
    }
  }
...
void CSkinnedMesh::skinMesh()
{
  ...
  if (!HardwareSkinning) {
    ...
    //rigid animation
    for (i = 0; i < AllJoints.size(); ++i) {
      for (int j = 0; j < AllJoints[i]->AttachedMeshes.size(); ++j) {
    ...
    for (i = 0; i < Vertices_Moved.size(); ++i)
      for (int j = 0; j < Vertices_Moved[i].size(); ++j) {
...
void CSkinnedMesh::finalize()
{
  ...
  if (AllJoints.size() || RootJoints.size()) {
    ...
    if (!RootJoints.size()) {
      for (int CheckingIdx = 0; CheckingIdx < AllJoints.size(); ++CheckingIdx) {
        bool foundParent = false;
        for (i = 0; i < AllJoints.size(); ++i) {
          for (int n = 0; n < AllJoints[i]->Children.size(); ++n) {
  ...
  if (HasAnimation) {
    ...
    for (i = 0; i < AllJoints.size(); ++i) {
      ...
      if (PositionKeys.size() > 2) {
        for (int j = 0; j < PositionKeys.size() - 2; ++j) {
      ...
      if (PositionKeys.size() > 1) {
        for (int j = 0; j < PositionKeys.size() - 1; ++j) {
      ...
      if (ScaleKeys.size() > 2) {
        for (int j = 0; j < ScaleKeys.size() - 2; ++j) {
      ...
      if (ScaleKeys.size() > 1) {
        for (int j = 0; j < ScaleKeys.size() - 1; ++j) {
      ...
      if (RotationKeys.size() > 2) {
        for (int j = 0; j < RotationKeys.size() - 2; ++j) {
      ...
      if (RotationKeys.size() > 1) {
        for (int j = 0; j < RotationKeys.size() - 1; ++j) {
  ...
  if (LocalBuffers.empty()) {
    ...
    for (int j = 1; j < LocalBuffers.size(); ++j) {
    ...
    IAnimatedMeshSceneNode* node, ISceneManager* smgr)
{
  ...
  for (i = 0; i < jointChildSceneNodes.size(); ++i) {
    ...
    s32 parentID = -1;
    for (int j = 0; (parentID == -1) && (j < AllJoints.size()); ++j) {
      if (i != j) {
        const SJoint* const parentTest = AllJoints[j];
        for (int n = 0; n < parentTest->Children.size(); ++n) {
...
void CSkinnedMesh::convertMeshToTangents()
{
  ...
  for (int b = 0; b < LocalBuffers.size(); ++b) {
    if (LocalBuffers[b]) {
