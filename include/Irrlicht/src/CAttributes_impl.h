...
void CAttributes::readAttributeFromXML(io::IXMLReader* reader)
{
  ...
  if (element == "enum") {
    ...
    s32 n = 0;
    const core::stringw tmpName("value");
    for (; n < count; ++n) {
  ...
  for (; i < (s32)Attributes.size(); ++i) {
    if (Attributes[i]->getType() == EAT_STRINGWARRAY) {
      ...
      for (; n < arrayinput.size(); ++n) {
        arraynames.push_back((tmpName + core::stringw((int)n)).c_str());
      ...
      writer->writeElement(
          ...
          "name", core::stringw(Attributes[i]->Name.c_str()).c_str(),
