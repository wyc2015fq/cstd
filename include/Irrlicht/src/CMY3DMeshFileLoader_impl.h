...
IAnimatedMesh* CMY3DMeshFileLoader::createMesh(io::IReadFile* file)
{
  ...
  for (s32 mesh_id = 0; mesh_id < sceneHeader.MeshCount; mesh_id++) {
    ...
    if (!buffer ||
      ...
      if (matEnt) {
        ...
        else if (buffer->Material.MaterialType == video::EMT_SPHERE_MAP) {
          buffer->Material.Lighting  = true;
        }
  ...
  for (int num = 0; num < MeshBufferEntry.size(); ++num) {
...
{
  for (int m = 0; m < MaterialEntry.size(); ++m)
...
{
  for (int m = 0; m < MeshBufferEntry.size(); ++m) {
