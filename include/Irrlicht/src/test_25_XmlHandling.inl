...
public:
  ...
  */
  bool load() {
    ...
    const stringw settingTag("setting"); //we'll be looking for this tag in the xml
    stringw currentSection; //keep track of our current section
    const stringw videoTag("video"); //constant for videotag
    ...
    while (xml->read()) {
      ...
      case irr::io::EXN_ELEMENT: {
        ...
        //we are in the video section and we find a setting to parse
          ...
          //read in the key
          stringw key = xml->getAttributeValueSafe("name");
  ...
  bool save() {
    ...
    for (; !i.atEnd(); i++) {
      ...
      xwriter->writeElement("setting", true, "name", i->getKey().c_str(), "value", i->getValue().c_str());
...
void createSettingsDialog(SAppContext& app)
{
  ...
  gui::IGUIWindow* windowSettings = app.Gui->addWindow(rect<s32>(10, 10, 400, 400), true, "Videosettings");
