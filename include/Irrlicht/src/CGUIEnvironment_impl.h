...
IGUIButton* CGUIEnvironment::addButton(const core::rect<s32>& rectangle, IGUIElement* parent, s32 id, const char* text, const char* tooltiptext)
{
...
IGUIImage* CGUIEnvironment::addImage(const core::rect<s32>& rectangle, IGUIElement* parent, s32 id, const char* text, bool useAlphaChannel)
{
  IGUIImage* img = new CGUIImage(this, parent ? parent : this,
      id, rectangle);
  if (text) {
...
IGUIMeshViewer* CGUIEnvironment::addMeshViewer(const core::rect<s32>& rectangle, IGUIElement* parent, s32 id, const char* text)
{
...
//! adds a checkbox
IGUICheckBox* CGUIEnvironment::addCheckBox(bool checked, const core::rect<s32>& rectangle, IGUIElement* parent, s32 id, const char* text)
{
  ...
  if (xml) {
    ...
    while (!found && xml->read()) {
      if (xml->getNodeType() == io::EXN_ELEMENT) {
        if (core::stringw("font") == xml->getNodeName()) {
          if (core::stringw("vector") == xml->getAttributeValue("type")) {
            t = EGFT_VECTOR;
            found = true;
          }
          else if (core::stringw("bitmap") == xml->getAttributeValue("type")) {
            t = EGFT_BITMAP;
            found = true;
          }
          else {
