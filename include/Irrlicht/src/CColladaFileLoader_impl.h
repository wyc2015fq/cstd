...
void CColladaFileLoader::readEffect(io::IXMLReaderUTF8* reader, SColladaEffect* effect)
{
            ...
            if (reader->getNodeType() == io::EXN_ELEMENT) {
              ...
              if (emissionNode == node || ambientNode == node ||
                ...
                while (reader->read()) {
                  ...
                  else if (reader->getNodeType() == io::EXN_ELEMENT &&
  ...
  int i;
  for (i = 0; i < video::MATERIAL_MAX_TEXTURES; ++i) {
    ...
    if (reader->getNodeType() == io::EXN_ELEMENT) {
      if (instanceMaterialName == reader->getNodeName()) {
        ...
        if (MaterialsToBind.find(meshbufferReference)) {
          ...
          os::Printer::log("#meshbuffers", core::stringc((int)toBind.size()).c_str(), ELL_DEBUG);
#endif
          SMesh tmpmesh;
          int i;
          for (i = 0; i < toBind.size(); ++i) {
    ...
    const core::stringc& geometryId)
{
  ...
  if (!reader->isEmptyElement())
    while (reader->read()) {
      ...
      if (reader->getNodeType() == io::EXN_ELEMENT) {
        ...
        else if (parsePolygonOK && polygons.size()) {
          core::stringc data = reader->getNodeData();
          data.trim();
          const c8* p = &data[0];
  ...
  if (textureCoordSetCount < 2) {
    ...
    for (i = 0; i < polygons.size(); ++i) {
      ...
      for (int v = 0; v < polygons[i].Indices.size(); v += maxOffset) {
        video::S3DVertex vtx;
        vtx.Color.set(255, 255, 255, 255);
        // for all input semantics
        for (int k = 0; k < localInputs.size(); ++k) {
      ...
      if (polygonsSectionName == polygonType &&
        ...
        if (FlipAxis) {
          for (int ind = indices.size() - 3; ind > 0 ; --ind) {
          ...
          for (int ind = 0; ind + 2 < indices.size(); ++ind) {
            mbuffer->Indices.push_back(indices[0]);
            mbuffer->Indices.push_back(indices[ind + 1]);
            mbuffer->Indices.push_back(indices[ind + 2]);
          }
...
#ifdef COLLADA_READER_DEBUG
  os::Printer::log("COLLADA added meshbuffer", core::stringc((int)buffer->getVertexCount()) + " vertices, " + core::stringc((int)buffer->getIndexCount()) + " indices.", ELL_DEBUG);
#endif
  buffer->drop();
}
...
{
  int i;
  for (i = 0; i < ColladaParameters.size(); ++i)
