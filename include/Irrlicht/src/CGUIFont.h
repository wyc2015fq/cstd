...
public:
  ...
  virtual core::dimension2d<u32> getDimension(const char* text) const;
  ...
  //! set an Pixel Offset on Drawing ( scale position on width )
  virtual s32 getKerningWidth(const char* thisLetter = 0, const char* previousLetter = 0) const;
  virtual s32 getKerningHeight() const;
  //! gets the sprite bank
  virtual IGUISpriteBank* getSpriteBank() const;
  //! returns the sprite number from a given character
  virtual u32 getSpriteNoFromChar(const char* c) const;
  virtual void setInvisibleCharacters(const char* s);
private:
  struct SFontArea {
