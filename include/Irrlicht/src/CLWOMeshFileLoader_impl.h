...
IAnimatedMesh* CLWOMeshFileLoader::createMesh(io::IReadFile* file)
{
  ...
  if (FormatVersion == 2) {
    ...
    for (int polyIndex = 0; polyIndex < Indices.size(); ++polyIndex) {
  ...
  for (int polyIndex = 0; polyIndex < Indices.size(); ++polyIndex) {
    ...
    for (i = 0; i < polySize; ++i) {
      ...
      if (uvTag < UvIndex.size()) {
        for (int uvsearch = 0; uvsearch < UvIndex[uvTag].size(); ++uvsearch) {
        ...
        if (duvTag < DUvName.size()) {
          for (int polysearch = 0; polysearch < VmPolyPointsIndex[duvTag].size(); polysearch += 2) {
  ...
  for (i = 0; i < Materials.size(); ++i) {
    ...
    for (int j = 0; j < Materials[i]->Meshbuffer->Vertices.size(); ++j) {
