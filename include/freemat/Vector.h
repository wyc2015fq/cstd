...
class VectorData : public QSharedData
{
  ...
  void allocate(size_t len) {
    try {
      ...
      throw_Exception(QString("Cannot allocate enough memory to store an array of size %1").arg(m_len));
    }
  }
...
public:
  ...
  inline VectorData(size_t dim) {
    try {
      ...
      throw_Exception(QString("Cannot allocate enough memory to store an array of size %1").arg(dim));
    }
  }
  ...
  inline void resize(size_t new_len) {
    if (new_len <= m_capacity) {
      ...
      try {
        ...
        throw_Exception(QString("Cannot allocate enough memory to resize array to size %1").arg(new_len));
      }
    }
  }
...
public:
  ...
  inline Vector(size_t dim) {
    d = new VectorData<T>(dim);
  }
