...
void ClearLibs(interpreter_t* eval)
{
  ...
  //of dynamic functions in current context_t in case of using thread.
  for (i = 0; i < DynamicFunctions.size(); i++) {
    ...
    interpreter_t* eval)
{
  ...
  if (arg.size() < 2) {
    throw_Exception("Must supply at least the library file and symbol name");
  }
...
char* parseArgumentName(const char*& cp)
{
  ...
  if (!isalpha(*cp))
    throw_Exception(QString("malformed import function prototype") +
...
char* parseBoundsCheck(const char*& cp)
{
  ...
  if (bracketDepth > 0)
    throw_Exception(QString("malformed bounds check - error starting at ") +
        cp);
    ...
    interpreter_t* eval)
{
  ...
  if (arg.size() < 5)
    throw_Exception(QString("import requires 5 arguments:") +
  ...
  if (libfullpath.isNull()) {
    throw_Exception("unable to find file " + libfile);
  }
  ...
  while (*cp != 0) {
    ...
    if (tn == NULL)
      throw_Exception(QString("illegal syntax in function") +
    ...
    if (*cp != 0) {
      if (*cp != ',') {
        throw_Exception("expecting a ',' between arugments");
      }
...
ArrayVector BlaslibFunction(int nargout, const ArrayVector& arg)
{
  ...
  if (arg.size() < 1) {
    ...
    return ArrayVector(mxArray(QString(list.c_str())));
  }
  else {
    if (!arg[0].isString()) {
      throw_Exception("argument to blaslib function must be a string");
    }

    QString txt = arg[0].asString();

    if (!wrapper.LoadLibByName(txt.toStdString())) {
      throw_Exception("Library by this name does not exist.");
    }
  }
...
ArrayVector BlaslibFunction(int nargout, const ArrayVector& arg)
{
  throw_Exception("BLAS library is loaded statically.");
  return ArrayVector();
}
#endif
