if (nargout <= 2) {
  if (p != 0) {
    T1* l_data = (T1*)l->data, *u_data = (T1*)u->data, *a_data = (T1*)A->data;
    ...
    if (nrows > ncols) {
      ...
      for (i = 0; i < lcols; i++) {
        l_data[fullpivot[i] + i * lrows].r = 1.0;
        l_data[fullpivot[i] + i * lrows].i = 0.0;
      }
  ...
  if (p != 0) {
    ...
    if (nrows > ncols) {
      ...
      // diagonal of L with 1s
      for (i = 0; i < lcols; i++) {
        l_data[i + i * lrows].r = 1.0;
        l_data[i + i * lrows].i = 0.0;
      }
