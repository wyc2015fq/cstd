...
public:
  ...
  inline index_t size() const {
    return m_dim[m_pinDim];
  }
  inline void set(const T& val) {
    m_ptr->set(m_ndx, val);
  }
  inline const T get() const {
    return m_ptr->get(m_ndx);
  }
  inline NTuple pos() const {
    return m_pos;
  }
  ~BaseIterator() {}
};
...
public:
  ...
  inline index_t size() const {
    return qMin(m_source.size(), m_dest.size());
  }
  inline BOOL isValid() const {
    return (m_dest.isValid() && m_source.isValid());
  }
  inline void next() {
    m_dest.next();
    m_source.next();
...
public:
  ...
  inline T get(index_t) const {
    return m_value;
  }
  inline T getNoBoundsCheck(index_t) const {
    return m_value;
  }
};
