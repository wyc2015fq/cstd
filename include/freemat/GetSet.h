...
{
  index_t offset = getSliceIndex(arg->dims, index);

  for (index_t i = 1; i <= arg->dims[0]; i++) {
...
void Set(T& arg, const IndexArray& index, const T& data)
{
  ...
  if (IsColonOp(index)) {
    ...
    if (myDims.count() != data.length()) {
      throw_Exception("assignment A(:) = B requires A and B to be the same size");
    }
  ...
  if (max_ndx > arg.length()) {
    arg.resize(max_ndx);
  }

  if (index.length() != data.length()) {
    throw_Exception("Assignment A(I) = B requires I and B to be the same size");
  }
...
void Set(T& arg, const IndexArrayVector& index, const T& data)
{
  ...
  if (!arg.isEmpty() && isSliceIndexCase(arg->dims, index)) {
  ...
  if (arg.isEmpty()) {
    ...
    if (data.isVector()) {
      ...
      for (i = 0; i < index.size(); i++) {
        if (IsColonOp(index[i])) {
          ndx.push_back(ExpandColons(index[i], data->dims[colonDim++]));
        }
        else {
          ndx.push_back(index[i]);
        }
    ...
    for (i = 0; i < index.size(); i++) {
      ndx.push_back(ExpandColons(index[i], arg->dims[i]));
      secdims[i] = ndx[i].length();
      maxsze[i] = MaxValue(ndx[i]);
    }
  }
  ...
  if (secdims.count() != data.length()) {
    throw_Exception("mismatch in size for assignment A(I1,I2,...) = B");
  }

  if (!(maxsze <= arg->dims)) {
    arg.resize(max(arg->dims, maxsze));
  }
...
const T Get(const T& arg, const IndexArray& index)
{
  ...
  if (arg.isEmpty()) {
    throw_Exception("Cannot index into empty variable,");
  }
...
const T Get(const T& arg, const IndexArrayVector& index)
{
  ...
  if (isSliceIndexCase(arg->dims, index)) {
