...
void EmitMultiExpressionVariable(const TREE& t)
{
  ...
  for (index = 1; index < t.numchildren() - 1; index++) {
...
void EmitRHS(const TREE& t)
{
  ...
  if (IsVariableDefined(t.first().text())) {
    ...
    for (index = 1; index < t.numchildren(); index++) {
      EmitSingleDeref(t.child(index));
    }
  }
  else {
    ...
    for (i = 0; i < t.second().numchildren(); i++) {
      Emit(" << ");
...
void EmitExpression(const TREE& t)
{
  ...
  case TOK_INTEGER:
    Emit("mxArray::int32Constructor(");
    Emit(QString::fromStdString(t.text()));
    Emit(")");
    return;

  case TOK_FLOAT:
    Emit("mxArray::floatConstructor(");
    Emit(QString::fromStdString(t.text()));
    Emit(")");
    return;

  case TOK_DOUBLE:
    Emit("mxArray::doubleConstructor(");
    Emit(QString::fromStdString(t.text()));
    Emit(")");
    return;
  ...
  case ':':
    if (t.numchildren() == 0) {
      Emit(QString("mxArray::stringConstructor(\":\")"));
    }
    else if (t.first().is(':')) {
      Emit("DoubleColon(");
...
void EmitArgumentAliases(MFunctionDef* m_def)
{
  ...
  for (i = 0; i < m_def->arguments.size(); i++) {
...
void EmitForStatement(const TREE& t)
{
  ...
  Emit(QString("mxArray %1 = ").arg(indexvar));
  EmitExpression(t.first().second());
  Emit(";\n");
  EmitIndent();
  Emit(QString("for (%1=1;%1<=%2.getLength();%1++) {\n").arg(loopvar).arg(indexvar));
  indent_level++;
  EmitIndent();
  Emit(QString("mxArray %1(%2.getVectorSubset(mxArray::int32Constructor(%3)));\n").arg(myloopvar).arg(indexvar).arg(loopvar));
...
void ProtoCompile(interpreter_t* m_eval, MFunctionDef* m_def, string filename)
{
  ...
  if (!fp->open(QIODevice::WriteOnly)) {
    throw_Exception("Unable to open file " + filename + " for writing.");
  }
...
ArrayVector fccFunction(int nargout, const ArrayVector& args, interpreter_t* m_eval)
{
  for (i = 0; i < args.size(); i++) {
    ...
    if (!m_eval->lookupFunction(name, val)) {
      throw_Exception("Unable to resolve " + name + " to a function");
    }

    if (!(val->type() == FM_M_FUNCTION)) {
      throw_Exception("Function " + name + " is not an M file (and cannot be compiled");
    }
...
static void VariableReferencesList(const TREE& t, vstr_t* idents)
{
  if (t.is(TOK_VARIABLE)) {
    BOOL exists = FALSE;
...
void VMStream::Run(interpreter_t* m_eval)
{
  ...
  for (i = 0; i < syms.size(); i++) {
    ...
    if (psym && psym->isArgument()) {
      ...
      if (!ptr.valid()) {
        throw_Exception("Undefined variable reference: " + syms[i]);
      }
      ...
      if (psym->isScalar() && ptr->isScalar()) {
        ...
        case FM_DCOMPLEX:
        case FM_STRING:
          throw_Exception("Cannot JIT code because of type of :" + syms[i]);

        case FM_LOGICAL:
          if (psym->m_type != t_boolean) {
            throw_Exception("Expected " + syms[i] + " to be a different type than it is");
          }
...
void JITBlock(VMStream& o, TREE t, interpreter_t* m_eval);
RegisterReference JITExpression(VMStream& o, TREE t, interpreter_t* m_eval);
...
RegisterReference JITPromote(VMStream& o, RegisterReference a, opcodeClass c)
{
  ...
  throw_Exception("Illegal type promotion call.");
}

RegisterReference JITBooleanOrOperator(VMStream& o, TREE t, interpreter_t* m_eval)
{
...
RegisterReference JITBooleanNotOperator(VMStream& o, TREE t, interpreter_t* m_eval)
{
  RegisterReference result(o.GetReg(), t_boolean);
  RegisterReference a(JITExpression(o, t.first(), m_eval));
  RegisterReference atest(JITPromote(o, a, t_boolean));
...
RegisterReference JITBooleanAndOperator(VMStream& o, TREE t, interpreter_t* m_eval)
{
  RegisterReference result(o.GetReg(), t_boolean);
  RegisterReference a(JITExpression(o, t.first(), m_eval));
  RegisterReference atest(JITPromote(o, a, t_boolean));
...
RegisterReference JITComparisonOperator(VMStream& o, TREE t, opcodeType op, interpreter_t* m_eval)
{
  RegisterReference a(JITExpression(o, t.first(), m_eval));
  RegisterReference b(JITExpression(o, t.second(), m_eval));
...
RegisterReference JITUnaryOperator(VMStream& o, TREE t, opcodeType op, interpreter_t* m_eval)
{
  RegisterReference a(JITExpression(o, t.first(), m_eval));
...
RegisterReference JITBinaryOperator(VMStream& o, TREE t, opcodeType op, interpreter_t* m_eval)
{
  RegisterReference a(JITExpression(o, t.first(), m_eval));
  RegisterReference b(JITExpression(o, t.second(), m_eval));
  opcodeClass outputClass;
...
SymbolInformation* JITAddArgument(VMStream& o, string name, interpreter_t* m_eval, BOOL scalarVariable)
{
  ArrayReference ptr(m_eval->getContext()->lookupVariable(name));

  if (!ptr.valid()) {
    throw_Exception("Undefined variable reference:" + name);
  }

  if (!ptr->is2D()) {
    throw_Exception("Cannot JIT multi-dimensional array:" + name);
  }

  if (ptr->isString() || ptr->isReferenceType()) {
    throw_Exception("Cannot JIT strings or reference types:" + name);
  }

  if (ptr->isComplex()) {
    throw_Exception("Cannot JIT complex arrays:" + name);
  }

  if (!ptr->isScalar() && scalarVariable) {
    throw_Exception("JIT requires " + name + " be a scalar");
  }
  ...
  case FM_DCOMPLEX:
  case FM_STRING:
    throw_Exception("Cannot JIT code because of type of :" + name);
...
RegisterReference JITRHS(VMStream& o, TREE t, interpreter_t* m_eval)
{
  ...
  if (t.numchildren() == 1) {
    if (!v->isScalar()) {
      throw_Exception("Non-scalar reference returned in scalar context!");
    }
  ...
  if (t.numchildren() > 2) {
    throw_Exception("multiple levels of dereference not handled yet...");
  }

  if (v->isScalar()) {
    throw_Exception("array indexing of scalar values...");
  }

  TREE s(t.second());

  if (!s.is(TOK_PARENS)) {
    throw_Exception("non parenthetical dereferences not handled yet...");
  }

  if (s.numchildren() == 0) {
    throw_Exception("Expecting at least 1 array reference for dereference...");
  }

  if (s.numchildren() > 2) {
    throw_Exception("Expecting at most 2 array references for dereference...");
  }
  ...
  throw_Exception("dereference not handled yet...");
}
...
RegisterReference JITExpression(VMStream& o, TREE t, interpreter_t* m_eval)
{
  ...
  case TOK_MATDEF:
  case TOK_CELLDEF:
    throw_Exception("JIT compiler does not support complex, string, END, matrix or cell defs");
  ...
  case '^':
    throw_Exception("^ is not currently handled by the JIT compiler");
    break;
...
void JITIfStatement(VMStream& o, TREE t, interpreter_t* m_eval)
{
  ...
  for (i = 0; i < endInstructions.size(); i++) {
...
void JITAssignment(VMStream& o, TREE t, interpreter_t* m_eval)
{
  TREE s(t.first());
  ...
  if (v->m_type != RHS.type()) {
    throw_Exception("polymorphic assignment to scalar detected.");
  }

  if (s.numchildren() == 1) {
    if (!v->isScalar()) {
      throw_Exception("scalar assignment to array variable.");
    }
...
void JITForLoop(VMStream& o, TREE t, interpreter_t* m_eval);

void JITStatementType(VMStream& o, TREE t, interpreter_t* m_eval)
{
  switch (t.token()) {
  case '=':
  ...
  case TOK_MULTI:
    throw_Exception("multi function calls do not JIT compile");

  case TOK_SPECIAL:
    throw_Exception("special function calls do not JIT compile");
  ...
  case TOK_WHILE:
    throw_Exception("nested while loops do not JIT compile");
  ...
  case TOK_BREAK:
    throw_Exception("break is not currently handled by the JIT compiler");
    break;

  case TOK_CONTINUE:
    throw_Exception("continue is not currently handled by the JIT compiler");
    break;

  case TOK_DBSTEP:
    throw_Exception("dbstep is not currently handled by the JIT compiler");
    break;

  case TOK_DBTRACE:
    throw_Exception("dbtrace is not currently handled by the JIT compiler");
    break;

  case TOK_RETURN:
    throw_Exception("return is not currently handled by the JIT compiler");
    break;

  case TOK_SWITCH:
    throw_Exception("switch is not currently handled by the JIT compiler");
    break;

  case TOK_TRY:
    throw_Exception("try is not currently handled by the JIT compiler");
    break;

  case TOK_QUIT:
    throw_Exception("quit is not currently handled by the JIT compiler");
    break;

  case TOK_RETALL:
    throw_Exception("retall is not currently handled by the JIT compiler");
    break;

  case TOK_KEYBOARD:
    throw_Exception("keyboard is not currently handled by the JIT compiler");
    break;

  case TOK_GLOBAL:
    throw_Exception("global is not currently handled by the JIT compiler");
    break;

  case TOK_PERSISTENT:
    throw_Exception("persistent is not currently handled by the JIT compiler");
    break;
...
void JITStatement(VMStream& o, TREE t, interpreter_t* m_eval)
{
...
void JITForLoop(VMStream& o, TREE t, interpreter_t* m_eval)
{
  if (!(t.first().is('=') && t.first().second().is(':') &&
    ...
    throw_Exception("For loop cannot be compiled - need integer bounds");
  }
...
VMStream CompileForBlock(const TREE t, interpreter_t* m_eval)
{
  VMStream o;
  JITForLoop(o, t, m_eval);
  return o;
...
void LoadCompileFunction(context_t* context)
{
