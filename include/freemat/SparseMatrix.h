...
public:
  ...
  const SparseData<T>& constData() const {
    return m_data;
  }
  SparseData<T>& data() {
    return m_data;
  ...
  SparseMatrix(QVector<index_t> row, QVector<index_t> col, QVector<T> val) {
    if (!((row.size() == col.size()) &&
        (row.size() == val.size()))) {
      throw_Exception("Invalid sizes for IJV style constructor");
    }
  ...
  inline BOOL isColumnVector() const {
    return m_dims.isColumnVector();
  }
  inline BOOL isRowVector() const {
    return m_dims.isRowVector();
  }
    ...
    for (typename SparseData<T>::iterator i = m_data.begin(); i != m_data.end(); ++i) {
      ...
      for (typename SparseSlice<T>::iterator j = i.value().begin(); j != i.value().end(); ++j) {
        if (!delete_set.contains(uint64(j.key()))) {
          while (dp < j.key()) {
            if (delete_set.contains(uint64(dp))) {
              ++deleted_count;
            }
  ...
  void del(const IndexArrayVector& index) {
    ...
    if (IsColonOp(index[0])) {
      ...
      throw_Exception("Unsupported deletion for sparse matrices.");
    }
  }
...
template <typename T>
void PrintMe(const SparseMatrix<T>& a, interpreter_t* io);

template <typename T>
void PrintMe(const SparseMatrix<T>& ar, const SparseMatrix<T>& ai, interpreter_t* io);
...
public:
  ...
  T value() const {
    if (!isValid()) {
      throw_Exception("Error -- attempt to retrieve value of invalid iterator");
    }
...
{
  if (A.columns() != B.rows()) {
    throw_Exception("Non conforming arrays for matrix multiply");
  }
...
template <typename T>
SparseMatrix<T> MatrixMultiply(const SparseMatrix<T>& A, const mxArray* B)
{
  if (A.columns() != B.rows()) {
    throw_Exception("Non conforming arrays for matrix multiply");
  }
...
template <typename T>
SparseMatrix<T> MatrixMultiply(const mxArray* A, const SparseMatrix<T>& B)
{
  if (A.columns() != B.rows()) {
    throw_Exception("Non conforming arrays for matrix multiply");
  }
...
{
  SparseMatrix<T> retval(arg->dims);
...
BOOL IsSymmetric(const SparseMatrix<T>& arg, const SparseMatrix<T>& img)
{
  ...
  while (Aiter.isValid()) {
    if (arg.get(NTuple(Aiter.col(), Aiter.row())) != Aiter.realValue()) {
      return FALSE;
    }

    if (img.get(NTuple(Aiter.col(), Aiter.row())) != -Aiter.imagValue()) {
      return FALSE;
    }
...
SparseMatrix<T> GetDiagonal(const SparseMatrix<T>& arg, int diagonal)
{
  ...
  if (diagonal < 0) {
    ...
    if (outLen == 0) {
      return SparseMatrix<T>(NTuple(0, 0));
    }
