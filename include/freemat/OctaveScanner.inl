...
{
  if (fm_oct_reserved_initialized) {
    return;
  }
...
{
  if (current() == '.') {
    if (tryFetchBinary(".*", TOK_DOTTIMES)) {
      return;
    }

    if (tryFetchBinary("./", TOK_DOTRDIV)) {
      return;
    }

    if (tryFetchBinary(".\\", TOK_DOTLDIV)) {
      return;
    }

    if (tryFetchBinary(".^", TOK_DOTPOWER)) {
      return;
    }

    if (tryFetchBinary(".'", TOK_DOTTRANSPOSE)) {
      return;
    }
  }

  if (tryFetchBinary("++", TOK_INCR)) {
    return;
  }

  if (tryFetchBinary("--", TOK_DECR)) {
    return;
  }

  if (tryFetchBinary("+=", TOK_PLUS_EQ)) {
    return;
  }

  if (tryFetchBinary("-=", TOK_MINUS_EQ)) {
    return;
  }

  if (tryFetchBinary("<=", TOK_LEQ)) {
    return;
  }

  if (tryFetchBinary(">=", TOK_GEQ)) {
    return;
  }

  if (tryFetchBinary("==", TOK_EQU)) {
    return;
  }

  if (tryFetchBinary("!=", TOK_NEQ)) {
    return;
  }

  if (tryFetchBinary("&&", TOK_SAND)) {
    return;
  }

  if (tryFetchBinary("||", TOK_SOR)) {
    return;
  }
...
void OctaveScanner::fetchOctString()
{
  ...
  if (ahead(len + 1) == '\n') {
    throw_Exception("unterminated string" + context());
  }
