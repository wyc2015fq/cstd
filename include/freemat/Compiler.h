...
protected:
  ...
  inline void DecOp(VMInstruction* op) {
    switch (op->opclass) {
    case t_boolean:
      throw_Exception("Decrement not supported for logical variables");
      break;
  ...
  inline void NegOp(VMInstruction* op) {
    switch (op->opclass) {
    case t_boolean:
      throw_Exception("Neg not supported for boolean arguments");
      break;

    case unsigned_integer:
      throw_Exception("Neg not supported for unsigned arguments");
      break;
  ...
  inline void NotOp(VMInstruction* op) {
    ...
    throw_Exception("NOT not supported for non-boolean arguments");
  }
...
public:
  VMStream() : reglist(0) {}
  inline void push_back(VMInstruction* t) {
    p_data.push_back(t);
  }
  inline unsigned size() {
    return p_data.size();
  }
  VMInstruction* operator[](int i) {
    return p_data[i];
  }
  SymbolInformation* find_symbol(string name) {
    return p_symbols.findSymbol(name);
  }
  void add_symbol(const string& key, const SymbolInformation& val) {
    p_symbols.insertSymbol(key, val);
  }
  StringVector get_symbol_names() {
    return p_symbols.getCompletions("");
  }
  void Run(interpreter_t* m_eval);
  inline unsigned GetReg() {
    return reglist++;
  }
};
