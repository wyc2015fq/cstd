...
template <typename T>
static mxArray realMatrixMultiply(const BasicArray<T> &A, const BasicArray<T> &B) {
...
template <typename T>
static mxArray complexMatrixMultiply(const BasicArray<T> &A, const BasicArray<T> &B) {
  BasicArray<T> C(NTuple(A.rows(),B.cols()));
  complexMatrixMatrixMultiply<T>(int(A.rows()/2),int(B.cols()),int(A.cols()),C.data(),A.constData(),B.constData());
  return mxArray(SplitReal(C),SplitImag(C));
}
