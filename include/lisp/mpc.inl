...
int mpc_parse_input(mpc_input_t* i, mpc_parser_t* init, mpc_result_t* final)
{
  ...
  while (!mpc_stack_empty(stk)) {
    ...
    case MPC_TYPE_UNDEFINED:
      MPC_FAILURE(mpc_err_fail(i->filename, i->state, "Parser Undefined!"));
    case MPC_TYPE_PASS:
      MPC_SUCCESS(NULL);
    case MPC_TYPE_FAIL:
      MPC_FAILURE(mpc_err_fail(i->filename, i->state, p->data.fail.m));
    case MPC_TYPE_LIFT:
    ...
    case MPC_TYPE_LIFT_VAL:
      ...
      /* Basic Parsers */
    case MPC_TYPE_SOI:
      MPC_PRIMATIVE(NULL, mpc_input_soi(i));
    case MPC_TYPE_EOI:
      MPC_PRIMATIVE(NULL, mpc_input_eoi(i));
    case MPC_TYPE_ANY:
      MPC_PRIMATIVE(s, mpc_input_any(i, &s));
    case MPC_TYPE_SINGLE:
      MPC_PRIMATIVE(s, mpc_input_char(i, p->data.single.x, &s));
    case MPC_TYPE_RANGE:
      MPC_PRIMATIVE(s, mpc_input_range(i, p->data.range.x, p->data.range.y, &s));
    case MPC_TYPE_ONEOF:
      MPC_PRIMATIVE(s, mpc_input_oneof(i, p->data.string.x, &s));
    case MPC_TYPE_NONEOF:
      MPC_PRIMATIVE(s, mpc_input_noneof(i, p->data.string.x, &s));
    case MPC_TYPE_SATISFY:
      MPC_PRIMATIVE(s, mpc_input_satisfy(i, p->data.satisfy.f, &s));
    case MPC_TYPE_STRING:
      MPC_PRIMATIVE(s, mpc_input_string(i, p->data.string.x, &s));
      /* Application Parsers */
...
{
  return mpc_expect(mpc_oneof("012345689"), "digit");
}
...
static void mpc_print_unretained(mpc_parser_t* p, int force)
{
  ...
  if (p->type == MPC_TYPE_SOI) {
    printf("<^>");
  }
  if (p->type == MPC_TYPE_EOI) {
    printf("<$>");
  }
...
static mpc_val_t* mpca_stmt_list_apply_to(mpc_val_t* x, void* s)
{
  ...
  while (*stmts) {
    ...
    if (st->flags & MPC_LANG_PREDICTIVE) {
