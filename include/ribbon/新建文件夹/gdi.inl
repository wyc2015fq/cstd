...
int dPOINT_AddLine(DPOINT* pt, int i, int n, IPOINT pt1, IPOINT pt2)
{
...
static int dPOINT_AddArc(DPOINT* pt, int i, int n, DRECT rc, REAL startAngle, REAL sweepAngle)
{
  return dPOINT_AddArc(pt, i, n, rc.l, rc.t, RCW(&rc), RCH(&rc), startAngle, sweepAngle);
}
static int dPOINT_AddArc(DPOINT* pt, int i, int n, IRECT rc, REAL startAngle, REAL sweepAngle)
{
  return dPOINT_AddArc(pt, i, n, rc.l, rc.t, RCW(&rc), RCH(&rc), startAngle, sweepAngle);
}
...
int dPOINT_AddCurve(DPOINT* pt, int i, int n, IPOINT a, IPOINT b, IPOINT c)
{
...
static int RoundRectangle(DPOINT* pt, int i, int n, IRECT r, int radius, int corners)
{
  ...
  if (ne > 0) {
    i = dPOINT_AddArc(pt, i, n, dRECT(r.r - ne, r.t, r.r, r.t + ne), -90, 90);
  }
  i = dPOINT_AddLine(pt, i, n, r.r, r.t + ne, r.r, r.b - se);
  if (se > 0) {
    i = dPOINT_AddArc(pt, i, n, dRECT(r.r - se, r.b - se, r.r, r.b), 0, 90);
  }
  i = dPOINT_AddLine(pt, i, n, r.r - se, r.b, r.l + sw, r.b);
  if (sw > 0) {
    i = dPOINT_AddArc(pt, i, n, dRECT(r.l, r.b - sw, r.l + sw, r.b), 90, 90);
  }
  i = dPOINT_AddLine(pt, i, n, r.l, r.b - sw, r.l, r.t + nw);
  if (nw > 0) {
    i = dPOINT_AddArc(pt, i, n, dRECT(r.l, r.t, r.l + nw, r.t + nw), 180, 90);
  }
...
/// <param name="r"></param>
/// <param name="radius"></param>
static int RoundRectangle(DPOINT* pt, int i, int n, IRECT r, int radius)
{
