...
namespace detail
{
	...
	//class conf
		...
		string conf::value(const char* key)
		{
			...
			while(ifs_.good())
			{
				...
				if((kpos != str.npos) && ((kpos == 0) || (str.substr(0, kpos) == string(kpos, ' '))))
				{
					size_t aspos = str.find("=", kpos + len);
					if(aspos != str.npos)
					{
						if((aspos == kpos + len) || (str.substr(kpos + len, aspos) == string(aspos - kpos - len, ' ')))
						{
							string res = str.substr(aspos + 1);
	...
	//class charset_conv
		...
		string charset_conv::charset(const char* str) const
		{
			...
			string rstr(strbuf, outbuf);
			delete [] strbuf;
			return rstr;
		}

		string charset_conv::charset(const char* buf, size_t len) const
		{
			if(reinterpret_cast<iconv_t>(-1) == handle_)
				return string();
	...
	class timer_runner
	{
		typedef void (*timer_proc_t)(size_t id);
	...
	public:
		...
		void set(size_t id, size_t interval, timer_proc_t proc)
		{
	...
	private:
		...
		map<unsigned, timer_group> threadmap_;
		map<size_t, timer_tag> holder_;
	};
		...
		:display_(0), colormap_(0), def_X11_error_handler_(0), grab_(0)
	{
		...
		if(langstr)
		{
			...
			if(dotpos != langstr_dup.npos)
			{
				...
				transform(beg, langstr_dup.end(), beg, toupper);
			}
		}
		else
	...
	void platform_spec::make_owner(native_window_type owner, native_window_type wd)
	{
		...
		if(context.owned == 0)
			context.owned = new vector<native_window_type>;
	...
	native_window_type platform_spec::get_owner(native_window_type wd) const
	{
		...
		return (i != wincontext_.end() ? i->second.owner : NULL);
	}
	...
	void platform_spec::remove(native_window_type wd)
	{
		...
		if(i->second.owner)
		{
			...
			if(u != wincontext_.end())
			{
				...
				if(vec)
				{
					auto j = find(vec->begin(), vec->end(), i->first);
					if(j != vec->end())
						vec->erase(j);
				}
			}
	...
	void platform_spec::caret_open(native_window_type wd, const size& caret_sz)
	{
		...
		if(NULL == addr)
		{
			...
			{
				XIMStyles* imstyle;
				::XGetIMValues(addr->input_method, XNQueryInputStyle, &imstyle, NULL, NULL);
				if(imstyle)
				{
					if(imstyle->count_styles)
					{
						addr->input_font = 0;
						XVaNestedList preedit_attr = ::XVaCreateNestedList(0, XNSpotLocation, &(addr->input_spot), NULL);
						XVaNestedList status_attr = ::XVaCreateNestedList(0, XNAreaNeeded, &(addr->input_status_area), NULL);
						...
						if(has_status)
						{
							addr->input_context = ::XCreateIC(addr->input_method, XNInputStyle, (XIMPreeditPosition | XIMStatusArea),
														...
														XNClientWindow, reinterpret_cast<Window>(wd), NULL);
						}
						else
						...
						if((addr->input_context == 0) && has_preedit)
						{
							addr->input_context = ::XCreateIC(addr->input_method, XNInputStyle, (XIMPreeditPosition | XIMStatusNothing),
															XNPreeditAttributes, preedit_attr, XNClientWindow, reinterpret_cast<Window>(wd), NULL);
						}

						if(addr->input_context)
						{
							XVaNestedList attr = ::XVaCreateNestedList(0, XNAreaNeeded, &(addr->input_status_area), XNClientWindow, reinterpret_cast<Window>(wd), NULL);
							::XGetICValues(addr->input_context, XNStatusAttributes, attr, NULL);
							::XFree(attr);
						}
						else
							addr->input_context = ::XCreateIC(addr->input_method, 
											...
											XNFocusWindow, reinterpret_cast<Window>(wd), NULL);
						...
						if(addr->input_context)
						{
							...
							::XSetICValues(addr->input_context, XNResetState, XIMPreserveState, NULL);
						}
						::XFree(preedit_attr);
						::XFree(status_attr);
					}
		...
		if(is_start_routine)
		{
			...
			caret_holder_.thr.reset(new thread(fn));
		}
	}
	...
	void platform_spec::caret_close(native_window_type wd)
	{
		...
		if(is_end_routine && (caret_holder_.thr != NULL) && (caret_holder_.thr->joinable()))
		{
	...
	bool platform_spec::caret_update(native_window_type wd, graphics& /*root_graph*/, bool after_mapping)
	{
		platform_scope_guard psg;
		auto i = caret_holder_.carets.find(wd);
		if(i != caret_holder_.carets.end())
	...
	void platform_spec::set_timer(size_t id, size_t interval, void (*timer_proc)(size_t))
	{
		lock_guard<decltype(timer_.mutex)> lock(timer_.mutex);
		if(0 == timer_.runner)
	...
	void platform_spec::kill_timer(size_t id)
	{
	...
	void* platform_spec::request_selection(native_window_type requestor, Atom type, size_t& size)
	{
		if(requestor)
		{
			...
			if(owner)
			{
				...
				unique_lock<decltype(selim->cond_mutex)> lock(selim->cond_mutex);
	...
	void platform_spec::write_selection(native_window_type owner, Atom type, const void * buf, size_t bufsize)
	{
		...
		{
			string * utf8str = selection_.content.utf8_string;
			if(utf8str)
				utf8str->clear();
			else
				utf8str = new string;
	...
	//Icon Storage
	const graphics& platform_spec::keep_window_icon(native_window_type wd, const image& img)
	{
		graphics & graph = iconbase_[wd];
		graph.make(img.size());
		img.paste(graph, {});
		return graph;
	}
			...
			if(evt.xselection.property)
			{
				...
				if(evt.xselection.property == self.atombase_.clipboard)
				{
					...
					if(self.selection_.items.size())
					{
						...
						lock_guard<decltype(im->cond_mutex)> lock(im->cond_mutex);
						im->cond.notify_one();
				...
				else if(evt.xselection.property == self.atombase_.xdnd_selection)
				{
					...
					if(bytes_left > 0 && type == self.xdnd_.good_type)
					{
															...
															&dummy_bytes_left, &data))
						{
							...
							stringstream ss(reinterpret_cast<char*>(data));
							while(true)
							{
								string file;
								getline(ss, file);
		...
		else if(SelectionRequest == evt.type)
		{
			...
			else if(XA_STRING == evt.xselectionrequest.target || self.atombase_.utf8_string == evt.xselectionrequest.target)
			{
				...
				::XChangeProperty(self.display_, evt.xselectionrequest.requestor, evt.xselectionrequest.property, evt.xselectionrequest.target, 8, 0,
									reinterpret_cast<unsigned char*>(str.size() ? const_cast<string::value_type*>(str) : 0), static_cast<int>(str.size()));
			}
			else
