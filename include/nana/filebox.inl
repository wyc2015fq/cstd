...
namespace nana
{
		...
		: public form
	{
		struct item_fs
		{
			...
			{
				wstringstream tm;
				tm<<(item.modified_time.tm_year + 1900)<<'-';    /// \todo : use filesystem_ext:: pretty_file_date
		...
		private:
			static wstringstream& _m_add(wstringstream& ss, unsigned v)
			{
				if(v < 10)
					ss<<L'0';
				ss<<v;
			...
			static string _m_trans(size_t bytes)  /// \todo : use filesystem_ext::pretty_file_size
			{
	...
	public:
		...
		filebox_implement(window owner, bool io_read, const char* title)
			: form(owner, API::make_center(owner, 630, 440)), io_read_(io_read)
		{
			...
			path_.events().selected.connect_unignorable([this](const arg_categorize<int>&)
			{
				...
				if(path.size() == 0) path = "/";    /// \todo : use filesystem_ext::def_rootstr?
			...
			ls_file_.set_sort_compare(0, [](const char* a, any* fs_a, const char* b, any* fs_b, bool reverse) -> bool
				{
			...
			ls_file_.set_sort_compare(2, [](const char* a, any* anyptr_a, const char* b, any* anyptr_b, bool reverse) -> bool
				{
			...
			ls_file_.set_sort_compare(3, [](const char*, any* anyptr_a, const char*, any* anyptr_b, bool reverse) -> bool
				{
		...
		void def_extension(const char* ext)
		{
			def_ext_ = ext;
		}
	...
	private:
		...
		void _m_init_tree()
		{
			...
			vector<pair<string, treebox::item_proxy>> paths;
		...
		void _m_load_cat_path(string path)
		{
			...
			catch(fs::filesystem_error&)
			{
				...
				dw.draw([](graphics& graph){
					string text = "Permission denied to access the directory";
		...
		void _m_list_fs()
		{
			...
			auto ext_types = cb_types_.anyobj<vector<string> >(cb_types_.option());
		...
		void _m_finish(kind::t type, const char* tar)
		{
		...
		bool _m_append_def_extension(char* tar) const
		{
			...
			auto exts = cb_types_.anyobj<vector<string> >(cb_types_.option());
			...
			if(selection_.target.empty())
			{
				...
				if(file.size())
				{
					...
					if(file[0] == '.')
					{
						...
						mb.icon(msgbox::icon_warning);
						mb<<file<<endl<<i18n("NANA_FILEBOX_ERROR_INVALID_FILENAME");
						mb();
						return;
					}
	...
	string filebox_implement::saved_init_path;
	string filebox_implement::saved_selected_path;
	...
	//class filebox
		...
		string filebox::title(string s)
		{
			impl_->title.swap(s);
			return s;
		}

		filebox& filebox::init_path(const char* ipstr)
		{
			if(ipstr.empty())
			{
				impl_->path.clear();
		...
		filebox& filebox::init_file(const char* ifstr)
		{
			impl_->file = ifstr;
			return *this;
		}

		filebox& filebox::add_filter(const char* description, const char* filetype)
		{
		...
		string filebox::path() const
		{
			return impl_->path;
		}
		
		string filebox::file() const
		{
			return impl_->file;
		}
		...
		bool filebox::show() const
		{
			...
			wstring filter_holder;
			wstring default_extension;
			if(impl_->filters.size())
			{
				for(auto & f : impl_->filters)
				{
					filter_holder += to_wstring(f.des);
					filter_holder += static_cast<wstring::value_type>('\0');
					wstring fs = to_wstring(f.type);
					size_t pos = 0;
					while(true)
					{
						pos = fs.find(L" ", pos);
			...
			ofn.lpstrTitle = (wtitle.empty() ? NULL : wtitle);
			ofn.nFilterIndex = 0;
			ofn.lpstrFileTitle = NULL;
			ofn.nMaxFileTitle = 0;
			ofn.lpstrInitialDir = (wpath.empty() ? NULL : wpath);
			...
			wfile.resize(wcslen(wfile.data()));
