...
namespace nana
{
	...
	//class internal_scope_guard
		...
		{
			bedrock::instance().wd_manager().internal_lock().lock();
		}

		internal_scope_guard::~internal_scope_guard()
		{
			bedrock::instance().wd_manager().internal_lock().unlock();
		}
	...
	//class internal_revert_guard
		...
		{
			bedrock::instance().wd_manager().internal_lock().revert();
		}

		internal_revert_guard::~internal_revert_guard()
		{
			bedrock::instance().wd_manager().internal_lock().forward();
		}
	...
	namespace detail
	{
		...
		{
			vector<core_window_t*> v;
			wd_manager().all_handles(v);

			vector<native_window_type> roots;
			native_window_type root = NULL;
			for (auto wd : v)
			{
				if (((0 == thread_id) || (wd->thread_id == thread_id)) && (wd->root != root))
				{
					root = wd->root;
					if (roots.end() == find(roots.begin(), roots.end(), root))
		...
		void bedrock::event_expose(core_window_t * wd, bool exposed)
		{
			...
			if (emit(event_code_expose, wd, arg, false, get_thread_context()))
			{
		...
		void bedrock::event_move(core_window_t* wd, int x, int y)
		{
			if (wd)
			{
				...
				arg.x = x;
				arg.y = y;
				emit(event_code_move, wd, arg, true, get_thread_context());
			}
		}
		...
		bool bedrock::event_msleave(core_window_t* hovered)
		{
			if (wd_manager().available(hovered) && hovered->flags.enabled)
			{
				...
				arg_mouse arg;
				arg.evt_code = event_code_mouse_leave;
				...
				emit(event_code_mouse_leave, hovered, arg, true, get_thread_context());
				return true;
			}
			return false;
		}
		...
		void bedrock::event_focus_changed(core_window_t* root_wd, native_window_type receiver, bool getting)
		{
			...
			if (getting)
			{
				if (root_wd->flags.enabled && root_wd->flags.take_active)
				{
					...
					if (!emit(event_code_focus, focused, arg, true, get_thread_context()))
		...
		void bedrock::set_menubar_taken(core_window_t* wd)
		{
			...
			//assigning of a NULL taken window is to restore the focus of pre taken
		...
		{
			if ((pi_data_->menu.owner == NULL) ||
				(owner && (pi_data_->menu.owner == owner))
				)
			{
				return (is_keyboard_condition ? (pi_data_->menu.has_keyboard ? pi_data_->menu.window : NULL) : pi_data_->menu.window);
			}
		...
		void bedrock::erase_menu(bool try_destroy)
		{
			if (pi_data_->menu.window)
			{
				...
				pi_data_->menu.window = pi_data_->menu.owner = NULL;
		...
		void bedrock::_m_emit_core(event_code evt_code, core_window_t* wd, bool draw_only, const event_arg& event_arg, const bool bForce__EmitInternal)
		{
			...
			case event_code_mouse_up:
			{
				...
				void(drawer::*drawer_event_fn)(const arg_mouse&, const bool);
				basic_event<arg_mouse>* evt_addr;
				...
				case event_code_mouse_up:
					...
					break;
				default:
					throw runtime_error("Invalid mouse event code");
				}
			...
			case event_code_key_char:
			case event_code_key_release:
			case event_code_shortkey:
			{
				auto arg = dynamic_cast<const arg_keyboard*>(&event_arg);
				if (NULL == arg)
					return;

				void(drawer::*drawer_event_fn)(const arg_keyboard&, const bool);
				basic_event<arg_keyboard>* evt_addr;
				...
				case event_code_shortkey:
					...
					break;
				default:
					throw runtime_error("Invalid keyboard event code");
				}
				{
					//enable refreshing flag, this is a RAII class for exception-safe
					flag_guard fguard(this, wd);
			...
			case event_code_destroy:
				...
				break;
			default:
				throw runtime_error("Invalid event code");
			}
		}
