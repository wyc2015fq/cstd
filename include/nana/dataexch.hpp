...
namespace system{
	...
	public:
		...
		void set(const string & text_utf8, native_window_type owner = NULL);
		void set(const wchar* text, native_window_type owner = NULL);

		bool set(const graphics& g, native_window_type owner = NULL);

		void get(char* text_utf8);
		void get(wchar* text);

		wstring wget();
	private:
		bool _m_set(format, const void* buf, size_t size, native_window_type);
