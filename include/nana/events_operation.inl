	...
	namespace detail
	{
		//class events_operation
			...
			void events_operation::erase(event_handle evt)
			{
				...
				if (i != handles_.end())
				{
					reinterpret_cast<docker_interface*>(evt)->get_event()->remove(evt);
				}
			}
		...
		//class docker_base
			...
			event_interface * docker_base::get_event() const
			{
				return event_ptr;
			}
		...
		//class event_base
			...
			size_t event_base::length() const
			{
				internal_scope_guard lock;
				return (NULL == dockers_ ? 0 : dockers_->size());
			}
			...
			void event_base::remove(event_handle evt)
			{
				...
				if (dockers_)
				{
					...
					{
						if (reinterpret_cast<docker_interface*>(evt) == *i)
						{
							...
							else
							{
								...
								dockers_->erase(i);
								delete reinterpret_cast<docker_interface*>(evt);
							}
							break;
						}
					}
			...
			event_handle event_base::_m_emplace(docker_interface* docker_ptr, bool in_front)
			{
				internal_scope_guard lock;
				if (NULL == dockers_)
					dockers_ = new vector<docker_interface*>;
