...
namespace nana
{
	...
	namespace restrict
	{
		namespace
		{
			...
			inline drawer& get_drawer(window wd)
			{
    ...
    //class drawing
  		...
  		{
			if (!API::is_window(wd))
				throw invalid_argument("drawing: invalid window parameter");

			if (reinterpret_cast<core_window_t*>(wd)->is_draw_through())
				throw invalid_argument("drawing: the window is draw_through enabled");
		}
		...
		{
			return API::empty_window(handle_) ||  reinterpret_cast<core_window_t*>(handle_)->root_graph->empty();
		}
		...
		void drawing::draw(const draw_fn_t& f)
		{
			...
			get_drawer(handle_).draw(draw_fn_t(f), false);		
		}

		void drawing::draw(draw_fn_t&& f)
		{
			if(API::empty_window(handle_))	return;
			get_drawer(handle_).draw(move(f), false);
		}

		drawing::diehard_t drawing::draw_diehard(const draw_fn_t& f)
		{
			if(API::empty_window(handle_)) return NULL;
			return reinterpret_cast<diehard_t>(get_drawer(handle_).draw(draw_fn_t(f), true));
		}

		drawing::diehard_t drawing::draw_diehard(draw_fn_t&& f)
		{
			if(API::empty_window(handle_))	return NULL;
			return reinterpret_cast<diehard_t>(get_drawer(handle_).draw(move(f), true));
		}
		...
		void drawing::erase(diehard_t d)
		{
			...
			if(!API::empty_window(handle_))
				get_drawer(handle_).erase(d);
		}

		void drawing::clear()
		{
			if(API::empty_window(handle_))	return;
			get_drawer(handle_).clear();
		}
	//end class drawing
}//end namespace nana
