...
namespace nana
{
	namespace detail
	{
      ...
      virtual ~scheme_factory_interface() {};
		...
		{
		private:
			factory_identifier* get_id() const
			{
				return &fid_;
			}

			widget_geometrics* create()
			{
				return (new Scheme);
			}

			widget_geometrics* create(widget_geometrics& other)
			{
		...
		{
			struct implement;
      color_schemes(const color_schemes&) {}
      color_schemes& operator=(const color_schemes&) {}
		public:
			typedef widget_geometrics scheme;
