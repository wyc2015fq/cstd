...
namespace nana
{
	namespace filesystem_ext
	{
		...
		string pretty_file_size(const fs::path& path)  
		{
			try {
		...
		string pretty_file_date(const fs::path& path) // todo: move to .cpp
		{
			try {
#if NANA_USING_BOOST_FILESYSTEM
				// The return type of boost::last_write_time isn't
				// the same as in nana and std implementations of this function
				auto ftime = chrono::system_clock::from_time_t(fs::last_write_time(path));
#else
				...
				stringstream tm;
				tm << put_time(localtime(&cftime), "%Y-%m-%d, %H:%M:%S");
				return tm.str();
			}
			catch (...) {
				return{};
		...
		bool modified_file_time(const fs::path& p, struct tm& t)
		{
			...
			if (::GetFileAttributesEx(p, GetFileExInfoStandard, &attr))
			{
				...
				if (::FileTimeToLocalFileTime(&attr.ftLastWriteTime, &local_file_time))
				{
					...
					t.tm_yday = date::day_in_year(st.wYear, st.wMonth, st.wDay);
...
namespace nana {	namespace experimental {	namespace filesystem
	{
		...
		//class filesystem_error
			filesystem_error::filesystem_error(const char* msg, error_code err)
				: system_error(err, msg)
			{}

			filesystem_error::filesystem_error(const char* msg, const path& path1, error_code err)
				:	system_error(err, msg),
					path1_(path1)
			{}

			filesystem_error::filesystem_error(const char* msg, const path& path1, const path& path2, error_code err)
				:	system_error(err, msg),
					path1_(path1),
					path2_(path2)
			{}
	...
	//class path
		...
		{
#if defined(NANA_WINDOWS)
			return (::GetFileAttributes(pathstr_) == INVALID_FILE_ATTRIBUTES);
#elif defined(NANA_LINUX) || defined(NANA_MACOS)
			struct stat sta;
			return (::stat(pathstr_, &sta) == -1);
#endif
		}
		...
		{
#if defined(NANA_WINDOWS)
			unsigned long attr = ::GetFileAttributes(pathstr_);
		...
		string path::generic_u8string() const // uppss ...   
		{
			auto str = pathstr_;
			replace(str.begin(), str.end(), '\\', '/');  // uppss ...  revise this !!!!!
			return to_utf8(str);
		}
		...
		void path::_m_assign(const char* source_utf8)
		{
			pathstr_ = to_nstring(source_utf8);
		}

		void path::_m_assign(const wchar* source)
		{
		...
		//class directory_iterator
			struct inner_handle_deleter
			{
				...
				void directory_iterator::_m_prepare(const path& file_path)
				{
					...
					if (false == end_)
					{
						find_ptr_ = shared_ptr<find_handle>(new find_handle(handle), inner_handle_deleter());
						handle_ = handle;
					}
				}
		...
		namespace detail
		{
			...
			{
#if defined(NANA_WINDOWS)
				return (FALSE != ::RemoveDirectoryW(p)) || not_found_error(static_cast<int>(::GetLastError()));
#elif defined(NANA_POSIX)
			...
			{
				vector<directory_iterator::value_type> files;
				basic_string<CharT> path = dir;
				path += '\\';
		...
		file_status status(const path& p)
		{
			...
			if (err != error_code())
				throw filesystem_error("status", p, err);
		...
		file_status status(const path& p, error_code& ec)
		{
			ec = error_code();
#if defined(NANA_WINDOWS)
			auto attr = ::GetFileAttributesW(p);
			if (INVALID_FILE_ATTRIBUTES == attr)
			{
		...
		uintmax_t file_size(const path& p)
		{
			...
			if (get_file_size_ex)
			{
				HANDLE handle = ::CreateFile(p, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
...
#	endif
		...
		{
			struct tm t;
			filesystem_ext::modified_file_time(p, t);   
			chrono::system_clock::time_point dateTime =chrono::system_clock::from_time_t( mktime(&t) );
			return 	dateTime;
		}
			...
			{
#if defined(NANA_WINDOWS)
				return (FALSE != ::CreateDirectoryW(p, 0));
#elif defined(NANA_POSIX)
				return (0 == ::mkdir(p, static_cast<int>(perms::all)));
#endif
			}
