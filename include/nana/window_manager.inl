...
namespace nana
{
	...
	namespace detail
	{
		...
		vector<unsigned long> shortkey_container::keys(window wd) const
		{
			if (wd)
			{
				for (auto & m : impl_->base)
		...
		root_misc::root_misc(root_misc&& other):
			window(other.window),
			root_graph(move(other.root_graph)),
			shortkeys(move(other.shortkeys)),
			condition(move(other.condition))
		{
		}
			...
			root_graph({ width, height })
		{
			...
			condition.pressed = NULL;
			condition.pressed_by_space = NULL;
			condition.hovered = NULL;
		}
		...
		{
			//Cached
			native_window_type	recent_access{ NULL };
			root_misc *			misc_ptr{ NULL };

			map<native_window_type, root_misc> table;
		};
		...
		root_misc* root_register::insert(native_window_type wd, root_misc&& misc)
		{
			...
			auto ret = impl_->table.emplace(wd, move(misc));
...
namespace detail
{
	...
	public:
		using iterator = typename vector<key_value_rep>::iterator;
		...
		vector<key_value_rep>& table()
		{
			return table_;
		}
	private:
		vector<key_value_rep> table_;
	};
	...
	//class window_manager			
			...
			struct window_manager::wdm_private_impl
			{
				...
				image default_icon_big;
				image default_icon_small;

				lite_map<core_window_t*, vector<function<void()>>> safe_place;
			};
		//end struct wdm_private_impl
			...
			struct thread_refcount
			{
				...
				vector<unsigned> callstack_refs;
			...
			struct window_manager::revertible_mutex::implementation
			{
				...
				vector<thread_refcount> records;
			};
			...
			void window_manager::revertible_mutex::lock()
			{
				...
				if (0 == impl_->thread_id)
					impl_->thread_id = system::this_thread_id();
			...
			{
				if (impl_->thread_id == system::this_thread_id())
					if (0 == --(impl_->refs))
						impl_->thread_id = 0;
			...
			void window_manager::revertible_mutex::revert()
			{
				if (impl_->thread_id == system::this_thread_id())
				{
					...
					for (size_t i = 0; i < current_refs; ++i)
						impl_->mutex.unlock();
				}
				else
					throw runtime_error("The revert is not allowed");
			}
			...
			void window_manager::revertible_mutex::forward()
			{
				...
				{
					auto const this_tid = system::this_thread_id();
					...
					throw runtime_error("The forward is not matched. Please report this issue");
				}
			...
			{
				template<typename T>
				bool erase(vector<T>& container, T value)
				{
		...
		size_t window_manager::number_of_core_window() const
		{
		...
		void window_manager::all_handles(vector<core_window_t*> &v) const
		{
			//Thread-Safe Required!
			lock_guard<mutex_type> lock(mutex_);
		...
		{
			native_window_type native = NULL;
			if (owner)
			{
				//Thread-Safe Required!
				lock_guard<mutex_type> lock(mutex_);
				...
				if (impl_->wd_register.available(owner))
				{
					if (owner->flags.destroying)
						throw runtime_error("the specified owner is destoryed");
			...
			{
				auto wd = new core_window_t(owner, widget_notifier_interface::get_notifier(wdg), (category::root_tag**)NULL);
				if (nested)
				{
					wd->owner = NULL;
					wd->parent = owner;
		...
		window_manager::core_window_t* window_manager::create_frame(core_window_t* parent, const rectangle& r, widget* wdg)
		{
			...
			if (impl_->wd_register.available(parent) == false)	return NULL;

			core_window_t * wd = new core_window_t(parent, widget_notifier_interface::get_notifier(wdg), r, (category::frame_tag**)NULL);
		...
		window_manager::core_window_t* window_manager::create_widget(core_window_t* parent, const rectangle& r, bool is_lite, widget* wdg)
		{
			...
			if (impl_->wd_register.available(parent) == false)
				throw invalid_argument("invalid parent/owner handle");

			if (parent->flags.destroying)
				throw logic_error("the specified parent is destory");

			auto wdg_notifier = widget_notifier_interface::get_notifier(wdg);

			core_window_t * wd;
			if (is_lite)
				wd = new core_window_t(parent, move(wdg_notifier), r, (category::lite_widget_tag**)NULL);
			else
				wd = new core_window_t(parent, move(wdg_notifier), r, (category::widget_tag**)NULL);
		...
		void window_manager::close(core_window_t *wd)
		{
			...
			if(category::flags::root == wd->other.category)
			{
				...
				arg.cancel = false;
				brock.emit(event_code_unload, wd, arg, true, brock.get_thread_context());
				if (false == arg.cancel)
				{
					...
					if(wd->flags.modal || (wd->owner == NULL) || wd->owner->flags.take_active)
		...
		void window_manager::destroy(core_window_t* wd)
		{
			...
			while (parent && (parent->other.category == category::flags::lite_widget))
			{
		...
		void window_manager::icon(core_window_t* wd, const image& small_icon, const image& big_icon)
		{
...
#else
				auto nv = (category::flags::root == wd->other.category ? wd->root : NULL);
#endif
		...
		window_manager::core_window_t* window_manager::find_window(native_window_type root, const point& pos)
		{
			...
			if((false == attr_.capture.ignore_children) || (NULL == attr_.capture.window) || (attr_.capture.window->root != root))
			{
				//Thread-Safe Required!
				lock_guard<mutex_type> lock(mutex_);
			...
			if (impl_->wd_register.available(wd))
			{
				if (category::flags::root != wd->other.category)
				{
					...
					if (x != wd->pos_owner.x || y != wd->pos_owner.y)
					{
						...
						brock.emit(event_code_move, wd, arg, true, brock.get_thread_context());
						return true;
					}
				}
				else if (!passive)
		...
		bool window_manager::move(core_window_t* wd, const rectangle& r)
		{
			...
			if(category::flags::root != wd->other.category)
			{
				...
				if(size_changed)
					size(wd, size{r.width, r.height}, true, false);
			}
			else
			{
				rectangle root_r = r;
				...
				if(size_changed)
				{
					...
					brock.emit(event_code_resized, wd, arg, true, brock.get_thread_context());
				}
				else
		...
		bool window_manager::size(core_window_t* wd, size sz, bool passive, bool ask_update)
		{	
			//Thread-Safe Required!
			lock_guard<mutex_type> lock(mutex_);
			if (!impl_->wd_register.available(wd))
			...
			if (sz != wd->dimension)
			{
				arg_resizing arg;
				arg.window_handle = reinterpret_cast<window>(wd);
				arg.border = window_border_none;
				arg.width = sz.width;
				arg.height = sz.height;
				brock.emit(event_code_resizing, wd, arg, false, brock.get_thread_context());
				sz.width = arg.width;
			...
			{
				//If allocation fails, here throws bad_alloc.
				graph.make(sz);
			...
			if(category::flags::lite_widget != wd->other.category)
			{
				...
				if(category::flags::root == wd->other.category)
				{
					...
					if(false == passive)
						if (!native_interface::window_size(wd->root, sz + size(wd->extra_width, wd->extra_height)))
						{
							wd->dimension = pre_sz;
							wd->drawer.graphics.swap(graph);
							wd->root_graph->swap(root_graph);
			...
			arg.width = sz.width;
			arg.height = sz.height;
			brock.emit(event_code_resized, wd, arg, ask_update, brock.get_thread_context());
			return true;
		}

		window_manager::core_window_t* window_manager::root(native_window_type wd) const
		{
			static pair<native_window_type, core_window_t*> cache;
		...
		bool window_manager::get_graphics(core_window_t* wd, graphics& result)
		{
			//Thread-Safe Required!
			lock_guard<mutex_type> lock(mutex_);
			if (!impl_->wd_register.available(wd))
		...
		bool window_manager::get_visual_rectangle(core_window_t* wd, rectangle& r)
		{
			//Thread-Safe Required!
			lock_guard<mutex_type> lock(mutex_);
			return (impl_->wd_register.available(wd) ?
		...
		vector<window_manager::core_window_t*> window_manager::get_children(core_window_t* wd) const
		{
			lock_guard<mutex_type> lock(mutex_);
			if (impl_->wd_register.available(wd))
				return wd->children;
		...
		{	
			...
			if (impl_->wd_register.available(newpa) && (NULL == wd->owner) && (wd->parent != newpa) && (!wd->flags.modal))
			{
		...
		window_manager::core_window_t* window_manager::set_focus(core_window_t* wd, bool root_has_been_focused, arg_focus::reason reason)
		{
			...
			if(wd != prev_focus)
			{
				...
				if (impl_->wd_register.available(prev_focus))
				{
					...
					brock.emit(event_code_focus, prev_focus, arg, true, brock.get_thread_context());
				}
			...
			brock.emit(event_code_focus, wd, arg, true, brock.get_thread_context());
		...
		void window_manager::capture_window(core_window_t* wd, bool captured, bool ignore_children)
		{
			...
			else if(wd == attr_.capture.window)
			{
				...
				if(attr_cap.size())
				{
					pair<core_window_t*, bool> last = attr_cap.back();
					attr_cap.pop_back();
				...
				if(wd && (NULL == attr_.capture.window))
		...
		void window_manager::enable_tabstop(core_window_t* wd)
		{
			//Thread-Safe Required!
			lock_guard<mutex_type> lock(mutex_);
			if (impl_->wd_register.available(wd) && (tab_type::none == wd->flags.tab))
			{
				wd->root_widget->other.attribute.root->tabstop.push_back(wd);
				wd->flags.tab |= tab_type::tabstop;
			}
		}
		...
		window_manager::core_window_t* get_tabstop(window_manager::core_window_t* wd, bool forward)
		{
			...
			if (forward)
			{
				if (tab_type::none == wd->flags.tab)
					return (tabs.front());
				else if (tab_type::tabstop & wd->flags.tab)
				{
					auto i = find(tabs.begin(), end, wd);
					if (i != end)
					{
						++i;
		...
		bool window_manager::calc_window_point(core_window_t* wd, point& pos)
		{
			//Thread-Safe Required!
			lock_guard<mutex_type> lock(mutex_);
			if (impl_->wd_register.available(wd))
		...
		auto window_manager::shortkeys(core_window_t* wd, bool with_children) -> vector<pair<core_window_t*, unsigned long>>
		{
			vector<pair<core_window_t*, unsigned long>> result;
				...
				if (root_rt)
				{
					...
					if (with_children)
					{
						for (auto child : wd->children)
						{
							...
							copy(child_keys.begin(), child_keys.end(), back_inserter(result));
						}
					}
				}
			}
		...
		void window_manager::set_safe_place(core_window_t* wd, function<void()>&& fn)
		{
			if (fn)
			{
				lock_guard<mutex_type> lock(mutex_);
				...
				impl_->safe_place[wd].emplace_back(move(fn));
			}
		}
		...
		void window_manager::_m_disengage(core_window_t* wd, core_window_t* for_new)
		{
			...
			decltype(for_new->root_widget->other.attribute.root) pa_root_attr = NULL;
			...
			vector<pair<core_window_t*,unsigned long>> sk_holder;
			...
			if ((!established) || (pa_root_attr != root_attr))
			{
				...
				if (wd->other.category == category::flags::root)
				{
					...
					wd->other.attribute.root->focus = NULL;
				}
				else
				{
				...
				if (wd->flags.tab & tab_type::tabstop)
				{
			...
			if (!established)
			{
				using effect_renderer = edge_nimbus_renderer<basic_window>;
			...
			if (established)
			{
				...
				function<void(core_window_t*, const point&)> set_pos_root;
				set_pos_root = [&set_pos_root](core_window_t* wd, const point& delta_pos)
				{
					for (auto child : wd->children)
					{
		...
		void window_manager::_m_destroy(core_window_t* wd)
		{
			...
			arg_destroy arg;
			arg.window_handle = reinterpret_cast<window>(wd);
			brock.emit(event_code_destroy, wd, arg, true, brock.get_thread_context());
		...
		{
			if(wd == NULL || false == wd->visible)	return false;
