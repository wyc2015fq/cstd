...
#include <nana/gui/detail/window_layout.hpp>
		...
		public:
			...
			void render(core_window_t * wd, bool forced, const rectangle* update_area = NULL)
			{
				bool copy_separately = true;
				vector<pair<rectangle, core_window_t*>>	rd_set;
				...
				if (wd->root_widget->other.attribute.root->effects_edge_nimbus.size())
				{
					...
					for(auto & action : nimbus)
					{
						if(_m_edge_nimbus(focused, action.window) && window_layer::read_visual_rectangle(action.window, r))
						{
							...
							{
								if (update_area)
									overlap(*update_area, rectangle(r), r);
				...
				if (copy_separately)
				{
					...
					{
						if (update_area)
							overlap(*update_area, rectangle(vr), vr);
		...
		private:
			...
			void _m_render_edge_nimbus(core_window_t* wd, const rectangle & visual)
			{
				...
				if (overlap(r, rectangle{ wd->root_graph->size() }, good_r))
				{
						...
						(good_r.right() > visual.right()) || (good_r.bottom() > visual.bottom()))
					{
						...
						pixel_buffer pixbuf(graph->handle(), r);
						...
						vector<typename window_layer::wd_rectangle> overlaps;
