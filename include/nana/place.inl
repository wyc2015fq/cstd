...
namespace nana
{
	namespace place_parts
	{
		...
		void check_field_name(const char* name)
		{
			if (*name && (*name != '_' && !(('a' <= *name && *name <= 'z') || ('A' <= *name && *name <= 'Z'))))
				throw invalid_argument("nana.place: bad field name");
		}
	...
	namespace place_parts
	{
		...
		public:
			...
			vector<number_t>& array() noexcept
			{
				return array_;
			}
			...
			{
				return move(reparray_);
			}

			vector<number_t>& parameters() noexcept
			{
				return parameters_;
			}
			...
			token read()
			{
				...
				case '.': case '-':
					...
					if (readbytes)
					{
						...
						_m_throw_error("invalid character '" + string(1, *sp_) + "'");
					break;
				default:
					if ('0' <= *sp_ && *sp_ <= '9')
					{
				...
				string err = "an invalid character '";
				err += *sp_;
				err += "'";
				_m_throw_error(err);
		...
		private:
			void _m_throw_error(const char* err)
			{
				throw runtime_error("place: " + err + " at " + to_string(static_cast<unsigned>(sp_ - divstr_)));
			}
			...
			size_t _m_number(const char* sp, bool negative) noexcept
			{
		...
		private:
			...
			string idstr_;
			number_t number_;
			vector<number_t> array_;
			repeated_array		reparray_;
			vector<number_t> parameters_;
		};	//end class tokenizer
	}

	static bool is_vert_dir(direction dir)
	{
	...
	static size_t find_idstr(const char* text, const char* idstr, size_t off = 0)
	{
		const auto len = strlen(idstr);
	...
	static pair<size_t, size_t> get_field_bound(const char* div, size_t start_pos)
	{
		int depth = 0;
		if ('<' == div[start_pos])
		{
	...
	static pair<size_t, size_t> get_field_bound(const char* div, const char* idstr, int depth)
	{
	...
	struct place::implement
	{
		...
		event_handle event_size_handle{NULL};

		string div_text;
		unique_ptr<division> root_division;
		map<string, field_gather*> fields;
		map<string, field_dock*> docks;
		map<string, field_dock*> dock_factoris;

		function<void(window, graphics&, mouse_action)> split_renderer;
		set<div_splitter*> splitters;

		//A temporary pointer used to refer to a specified div object which
		//will be deleted in modification process.
		unique_ptr<division> tmp_replaced;
		...
		static division * search_div_name(division* start, const char*) noexcept;
		unique_ptr<division> scan_div(place_parts::tokenizer&);
	...
	public:
		...
		static event_handle erase_element(vector<element_t>& elements, window handle) noexcept
		{
	...
	private:
		void _m_insert_widget(window wd, bool to_fasten)
		{
			if (API::empty_window(wd))
				throw invalid_argument("Place: An invalid window handle.");

			if (API::get_parent_window(wd) != place_ptr_->window_handle())
				throw invalid_argument("Place: the window is not a child of place bind window");
	...
	{
	public:
		div_dockpane * attached{ NULL };					//attached div object
		unique_ptr<place_parts::dockarea> dockarea;	//the dockable widget
		map<string, function<unique_ptr<widget>(window)>> factories;	//factories for dockpane
	...
	public:
		...
		division(kind k, char*& n) noexcept
			: kind_of_division(k),
			name(move(n))
		{}
		...
		pair<double, double> calc_weight_floor()
		{
			pair<double, double> floor;
				...
				if ((fit_policy::none != this->fit) && this->field)
				{
					...
					for (auto & elm : this->field->elements)
					{
						...
						if (extent)
						{
							...
							max_value = (max)(max_value, (vert_fields ? extent->second.width : extent->second.height));
					...
					{
						double percent = 0;
						for (size_t i = 0; i < fit_count - 1; ++i)
						{
	...
	public:
		double _m_extend_with_margin(size_t edge, double & extended)
		{
	...
	public:
		...
		string name;
		vector<unique_ptr<division>> children;
		...
		field_gather * field{ NULL };
		division * div_next{ NULL };
		division * div_owner{ NULL };

		struct run_data
		{
			pair<double, double> weight_floor;
			map<window, size> fit_extents;
		}run_;
	};//end class division
	...
	{
	public:
		div_arrange(bool vert, char*& name, place_parts::repeated_array&& arr) noexcept
			: division((vert ? kind::vertical_arrange : kind::arrange), move(name)),
			arrange_(move(arr))
		{}
		...
		void collocate(window wd)
		{
			...
			vector<division*> delay_collocates;
			double precise_px = 0;
			for (auto& child_ptr : children)					/// First collocate child div's !!!
			{
				...
				child_area.w_ref() = static_cast<unsigned>((max)(endpos - child_area.x(), 0));
	...
	private:
		static pair<unsigned, size_t> _m_calc_fa(const place_parts::number_t& number, unsigned area_px, double& precise_px)
		{
			pair<unsigned, size_t> result;
			switch (number.kind_of())
			{
			case number_t::kind::integer:
		...
		pair<unsigned, size_t> _m_fixed_and_adjustable(kind match_kind, unsigned area_px) const noexcept
		{
			pair<unsigned, size_t> result;
		...
		static double _m_find_lowest_revised_division(const vector<revised_division>& revises, double level_px) noexcept
		{
		...
		static size_t _m_remove_revised(vector<revised_division>& revises, double value, size_t& full_count) noexcept
		{
			full_count = 0;
			size_t reached_mins = 0;
		...
		double _m_revise_adjustable(pair<unsigned, size_t>& fa, unsigned area_px)
		{
			...
			size_t min_count = 0;
			double sum_min_px = 0;
			vector<revised_division> revises;
			...
			for (auto& child : children)
			{
				...
				double min_px = numeric_limits<double>::lowest(), max_px = numeric_limits<double>::lowest();
				...
				if (min_px >= 0 && max_px >= 0 && min_px > max_px)
				{
					if(weight_floor > 0)
						...
						min_px = numeric_limits<double>::lowest();
					else if (child->max_px.kind_of() == number_t::kind::percent)
						max_px = numeric_limits<double>::lowest();
				}
	...
	{
	public:
		div_grid(char*& name, place_parts::repeated_array&& arrange, vector<rectangle>&& collapses) noexcept
			: division(kind::grid, move(name)),
			arrange_(move(arrange)),
			collapses_(move(collapses))
		{
		...
		void revise_collapses() noexcept
		{
			...
			for (size_t i = 0; i < collapses_.size() - 1; ++i)
			{
			...
			if ((gap_size * dimension.first < area.width) && (gap_size * dimension.second < area.height))
			{
				if (dimension.first <= 1 && dimension.second <= 1)
				{
					...
					case 2: case 3: case 4:
						edge = 2;	break;
					default:
						edge = static_cast<size_t>(sqrt(n_of_wd));
					...
					size_t arr_pos = 0;
					for (size_t u = 0; (u < edge && i != end); ++u)
					{
						double x = area.x;
						for (size_t v = 0; (v < edge && i != end); ++v, ++i)
						{
							unsigned value = 0;
				...
				else
				{
					...
					unique_ptr<char[]> table_ptr{ new char[dimension.first * dimension.second] };
					...
					for (size_t u = 0; (u < dimension.second && i != end); ++u)
					{
						...
						for (size_t v = 0; (v < dimension.first && i != end); ++v)
						{
							...
							pair<unsigned, unsigned> room{ 1, 1 };
	...
	private:
		void _m_find_collapse(int x, int y, pair<unsigned, unsigned>& collapse) const noexcept
		{
			for (auto & col : collapses_)
			{
				if (col.x == x && col.y == y)
	...
	public:
		...
		void set_renderer(const function<void(window, graphics&, mouse_action)> & fn, bool update)
		{
		...
		{
			splitter_cursor_ = (horizontal ? cursor_size_we : cursor_size_ns);
		}
	private:
		void collocate(window wd)
		{
			...
			if (splitter_.empty())
			{
				...
				auto grab_fn = [this](const arg_mouse& arg)
				{
					...
					if (event_code_mouse_down == arg.evt_code)
					{
						...
						if (cursor_size_we != splitter_cursor_)
						{
							left_pos_ = area_left.y;
							right_pos_ = area_right.bottom();
							px_ptr = &rectangle::height;
						}
						else
						{
					...
					else if (event_code_mouse_move == arg.evt_code)
					{
						if(!grabbed_)
							return;
							
						auto const vert = (cursor_size_we != splitter_cursor_);
						...
						auto left_px = clamp(static_cast<int>(left_pixels_) + delta, 0, total_pixels);
						...
						auto right_px = clamp(static_cast<int>(right_pixels_) - delta, 0, total_pixels);
			...
			{
				const bool vert = (cursor_size_we != splitter_cursor_);
				...
				const auto pos = clamp(static_cast<int>(area_px - right_px - splitter_px), limited_range.x(), limited_range.right());
	...
	private:
		static int _m_search_name(const division* div, char* name) noexcept
		{
			if (div->name.size())
			{
				name = div->name;
		...
		static void _m_remove_attr(char* div, const char* attr)
		{
			...
			if (div.npos != endpos && div[endpos] == '=')
			{
				...
				if (div.npos == endpos)
					throw runtime_error("please report an issue if throws");
			}
			else
				endpos = attr_pos + len;
		...
		void _m_update_div(char* div)
		{
			...
			auto bound = get_field_bound(div, name, depth);
			...
			string::size_type tag_pos{ left ? div.find('<', bound.second + 1) : div.rfind('>', bound.first - 1) };
			if (div.npos == tag_pos)
				throw runtime_error("place report an issue if it throws");
			...
			string weight = "weight=" + to_string(percent * 100) + "% ";
		...
		{
			const bool vert = (cursor_size_ns == splitter_cursor_);
			...
			if (!leaf_right->min_px.empty())
				endpos = (min)(right_base - static_cast<int>(leaf_right->min_px.get_value(area.w())), endpos);

			if (!leaf_right->max_px.empty())
				pos = (max)(right_base - static_cast<int>(leaf_right->max_px.get_value(area.w())), pos);

			area.x_ref() = pos;
			area.w_ref() = unsigned(endpos - pos + splitter_px);

			dragger_.target(splitter_, area.result(), (vert ? arrange::vertical : arrange::horizontal));

			return area;
		}
	private:
		implement* const impl_;
		cursor	splitter_cursor_{cursor_arrow};
	...
	{
	public:
		div_dockpane(string && name, implement* impl, direction pane_dir) noexcept
			:	division(kind::dockpane, move(name)),
				impl_ptr_{impl}
		{
			dir = pane_dir;
			this->display = false;
	...
	private:
		...
		void notify_move()
		{
			...
			if (!indicator_.docker)
			{
				...
				indicator_.docker->z_order(NULL, z_order_action_topmost);
			...
			if (_m_hit_test(true)) //hit test on docker
			{
				if (!indicator_.dock_area)
				{
					...
					drawing dw(indicator_.dock_area->handle());
					dw.draw([this](graphics& graph)
					{
						...
						color clr = colors::deep_sky_blue;
						r.y = 0;
						r.height = border_px;
		...
		void request_close()
		{
			...
			{
				unique_ptr<typename remove_pointer<decltype(ptr)>::type> del(ptr);
			});
	...
	public:
		field_dock * dockable_field{ NULL };

		unique_ptr<widget>	splitter;
	private:
		implement * impl_ptr_;
		...
		{
			graphics graph;
			unique_ptr<panel<true>> dock_area;
			unique_ptr<form> docker;
		}indicator_;
	};
		...
		: public division
	{
		...
		{
		public:
			splitter(window wd, direction dir, division* dock_dv, division* pane_dv)
				: panel<true>(wd, true), dir_(dir), dock_dv_(dock_dv), pane_dv_(pane_dv)
			{
				this->bgcolor(colors::alice_blue);
				this->cursor(is_vert_dir(dir_) ? cursor_size_ns : cursor_size_we);

				auto grab_fn = [this, wd](const arg_mouse& arg)
				{
					auto const is_vert = is_vert_dir(dir_);

					if (event_code_mouse_down == arg.evt_code)	//press mouse button
					{
					...
					else if (event_code_mouse_move == arg.evt_code)	//hover
					{
		...
		private:
			...
			point	range_;
			point	base_pos_;	//x = mouse pos, y = splitter pos
			unsigned		base_px_;	//weight of div_dockpane when mouse button is been pressing;

		};
	public:
		div_dock(string && name, implement* impl) noexcept
			: division(kind::dock, move(name)), impl_(impl)
		{}
		...
		void collocate(window wd)
		{
			...
			size room(area.width - splitter_px * (horz_count - 1), area.height - splitter_px * (vert_count - 1));
	...
	place::implement::division * place::implement::search_div_name(division* start, const char* name) noexcept
	{
		if (start)
		{
			if (start->name == name)
	...
	static int get_parameter(place_parts::tokenizer& tknizer, size_t pos)
	{
		...
		throw invalid_argument("nana.place: the type of the " + string{pos_strs[pos]} +"th parameter for collapse should be integer.");
	}

	auto place::implement::scan_div(place_parts::tokenizer& tknizer) -> unique_ptr<division>
	{
		using token = place_parts::tokenizer::token ;

		unique_ptr<division> div;
		...
		place_parts::margin margin;
		vector<number_t> array;
		vector<rectangle> collapses;
		vector<unique_ptr<division>> children;
		direction div_dir = direction::west;
		...
		for (token tk = tknizer.read(); (tk != token::eof && tk != token::div_end); tk = tknizer.read())
		{
			...
			case token::dock:
				if (token::eof != div_type && token::dock != div_type)
					throw invalid_argument("nana.place: conflict of div type at " + to_string(tknizer.pos()));
			...
			case token::splitter:
				...
				if (!children.empty() && (division::kind::splitter != children.back()->kind_of_division))
				{
					...
					children.emplace_back(unique_ptr<division>{ splitter });
				}
				break;
			case token::div_start:
			{
				...
				children.emplace_back(move(div));
			}
				break;
			case token::vert:
				div_type = tk;
			...
			case token::collapse:
				{
					if (tknizer.parameters().size() != 4)
						throw invalid_argument("nana.place: collapse requires 4 parameters.");
			...
			case token::identifier:
				...
				break;
			case token::left:
				div_dir = direction::west; break;
			case token::right:
				div_dir = direction::east; break;
			case token::top:
				div_dir = direction::north; break;
			case token::bottom:
				div_dir = direction::south; break;
		...
		field_gather * attached_field = NULL;
		if (name.size())
		{
			...
			if (fields.end() != i)
			{
				...
				if (attached_field->attached)
					throw runtime_error("place, the name '" + name + "' is redefined.");
			}
		}
		...
		case token::vert:                           // "vertical" div
			for (auto& ch : children)
				if (ch->weigth_type == unmatch)
					throw invalid_argument("nana.place: unmatch vertical-heigth/horizontal-width betwen division '"
						                         +name+"' and children division '" + ch->name);

			div.reset(new div_arrange(token::vert == div_type, move(name), move(arrange)));
			break;
		case token::grid:
		{
			unique_ptr<div_grid> p(new div_grid(move(name), move(arrange), move(collapses)));
			...
			break;
		case token::dock:
			div.reset(new div_dock(move(name), this));
			break;
		default:
			throw invalid_argument("nana.place: invalid division type.");
		}
		...
		if (children.size())
		{
			...
			if (token::dock == div_type)
			{
				...
				vector<unique_ptr<division>> adjusted_children;
				for (auto & child : children)
				{
					auto dockpn = new div_dockpane(move(child->name), this, child->dir);
					dockpn->div_owner = child->div_owner;
					dockpn->weight = child->weight;
					adjusted_children.emplace_back(unique_ptr<division>{ dockpn });
				}
		...
		div->margin = move(margin);
		div->dir = div_dir;

		div->display = !undisplayed;
		div->visible = !(undisplayed || invisible);
		div->fit = fit;
		div->fit_parameters = move(fit_parameters);
	...
	void place::implement::check_unique(const division* div) const
	{
		...
		//reduce the size of the generated code, becuase set<string>
		//will create a new template class.
		map<string, field_gather*> unique;
		field_gather tmp(NULL);

		function<void(const implement::division* div)> check_fn;
		check_fn = [&check_fn, &unique, &tmp](const implement::division* div)
		{
			if (!div->name.empty())
			{
				...
				if (ptr)
					throw invalid_argument("place, the name '" + div->name + "' is redefined.");
				ptr = &tmp;
			}
	...
	void place::implement::connect(division* start)
	{
		...
		map<string, field_dock*> docks_to_be_closed;
		//disconnect
		for (auto & dk : docks)
		{
		...
		function<void(division* div)> connect_fn;
			...
			if (div->name.size())
			{
				if (division::kind::dock == div->kind_of_division || division::kind::dockpane == div->kind_of_division)
				{
					...
					if (i != docks.end())
					{
						...
						if (fd_dock->attached)
						{
							fd_dock->attached->dockable_field = NULL;
		...
		for (auto& e : docks_to_be_closed)
		{
			...
			e.second->attached->dockable_field = NULL;
			e.second->attached = NULL;
		}
	}
	...
	void place::implement::disconnect() noexcept
	{
		for (auto & fd : fields)
		{
			...
			{
				fd.second->attached->field = NULL;
				fd.second->attached = NULL;
			}
		}
	}
	...
	void place::bind(window wd)
	{
		if (impl_->window_handle)
			throw runtime_error("place.bind: it has already bound to a window.");
	...
	void place::splitter_renderer(function<void(window, graphics&, mouse_action)> fn)
	{
	...
	const char* place::div() const noexcept
	{
	...
	void update_div(char* div, const char* field, const char* attr, update_operation operation);

	void place::modify(const char* name, const char* div_text)
	{
		if (NULL == div_text)
			throw invalid_argument("nana.place: invalid div-text");

		if (NULL == name) name = "";

		//check the name, it throws invalid_argument
		...
		unique_ptr<implement::division>* replaced = NULL;
		...
		if ((!p->attached) && impl_->root_division)
		{
			...
			if (div)
			{
				if (div->field && (div->field != p))
					throw runtime_error("nana.place: unexpected error, the division attachs a unexpected field.");
	...
	void update_div(char* div, const char* field, const char* attr, update_operation operation)
	{
		...
		if (fieldstr.npos == pos)
		{
			...
			if (operation == update_operation::insert)
				div.insert(fieldname_pos + strlen(field), " " + string(attr));
			else if (operation == update_operation::replace)
			{
				div.erase(bound.first + 1, fieldstr.length());
				div.insert(bound.first + 1, string(attr) + " " + string(field));
			}
		}
		else
		{
			...
			{
				div.erase(bound.first + pos + 1, strlen(attr));
	...
	void place::erase(window handle)
	{
		...
		for (auto & fld : impl_->fields)
		{
			...
			{
				API::umake_event(evt);
				recollocate |= (NULL != fld.second->attached);
			}
	...
	place& place::dock(const char* name, string factory_name, function<unique_ptr<widget>(window)> factory)
	{
		//check the name, it throws invalid_argument
		//if name violate the naming convention.
		...
		if (!factory_name.empty())
		{
			if (impl_->dock_factoris.find(factory_name) != impl_->dock_factoris.end())
				throw invalid_argument("place - the specified factory name(" + factory_name + ") already exists");

			impl_->dock_factoris[factory_name] = dock_ptr;
			dock_ptr->factories[factory_name] = move(factory);
		}
		...
		if (factory_name.empty() && dock_ptr->attached)
		{
			...
			{
				dock_ptr->dockarea.reset(new place_parts::dockarea);
	...
	widget* place::dock_create(const char* factory)
	{
		auto i = impl_->dock_factoris.find(factory);
		if (i == impl_->dock_factoris.end())
			throw invalid_argument("invalid factory name(" + factory + ") of dockpane");
