...
{
	inline_indicator * indicator_{ NULL };
	index_type  pos_ ;
	textbox     txt_ ;
	button      btn_ ;
...
private:
	...
	void activate(inline_indicator& ind, index_type pos)
	{
		indicator_ = &ind;
		pos_ = pos;
	}
	...
	void resize(const size& dimension)
	{
		auto sz = dimension;
		sz.width -= 50;
		txt_.size(sz);
...
int main()
{
	...
	//Then append items
	lsbox.at(0).append({ string("Hello0"), string("World0") });
	lsbox.at(0).append({ string("Hello1"), string("World1") });
	lsbox.at(0).append({ string("Hello2"), string("World2") });
	lsbox.at(0).append({ string("Hello3"), string("World3") });

	//Create a new category
	lsbox.append("Category 1");

	//Append items for category 1
	lsbox.at(1).append({ string("Hello4"), string("World4") });
	lsbox.at(1).append({ string("Hello5"), string("World5") });
	lsbox.at(1).append({ string("Hello6"), string("World6") });
	lsbox.at(1).append({ string("Hello7"), string("World7") });
