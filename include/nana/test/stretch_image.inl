...
ostream& operator<< (ostream& os, const size& z)
{
...
public:
    tsform()
    {
		if (!img_.open(("../Examples/bground.6states.bmp")))
			throw runtime_error("Imposible to open the image");    /// \todo Open the image file.
		...
		dw.draw([this](graphics& graph)
		{
			...
			cout << "Pict: " << r << ", Windows: " << rw << "\n";
		});

  //      // get the graphycs !?
		//if (!API::window_graphics(*this, gr_))
		//	throw runtime_error("Imposible to get the graph");
  //      
...
private:
    ...
    {
        static bool interop;
        image_process::selector sl;
        sl.stretch(interop ? "bilinear interoplation" : "proximal interoplation");
        interop = !interop;
		cout << (interop ? "Click: bilinear interoplation\n" 
