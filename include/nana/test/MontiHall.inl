...
{ 
  ...
  public: 
    monty_hall(); 
  private: 
    void _m_pick_door  (const arg_click& ei); 
...
monty_hall::monty_hall() 
      : form(  API::make_center(400, 150) , appear::decorate<appear::taskbar>()   ) 
 { 
  this->caption( ("The Monty Hall Problem")); 
  string text =  "Hi, I am the host, you are on a Game Show:\n"
  ...
  label_.create(*this, rectangle(size(400, 100))); 
  label_.caption(text); 
  string door_name[3] = 
                            { ("Door No.&1"),  ("Door No.&2"),  ("Door No.&3")}; 
  for(int i = 0; i < 3; ++i) 
  { 
    door_[i].create(*this, rectangle(50 + 110 * i, 110, 100, 24)); 
    door_[i].caption(door_name[i]); 
    door_[i].events().click([this](const arg_click& ei){ _m_pick_door(ei); });
  } 
}

 void monty_hall::_m_pick_door(const arg_click& ei)
{ 
     int index = 0; 
     for(; index < 3; ++index) 
     { 
...
{ 
   ...
   { 
      case state_begin: 
                     door_has_car_ = (system::timestamp() / 1000) % 3; 
                     _m_remove_door(door); 
                     state_ = state_picked; 
                break; 
      case state_picked: 
...
{ 
     ...
     { 
         if(i != exclude) 
             doors.push_back(i);        } 
     unsigned ts = (system::timestamp() / 1000) % 2; 
     if(door_has_car_ == doors[ts]) 
         ts = (ts ? 0: 1); 
     door_[doors[ts]].enabled(false); 
     doors.erase(doors.begin() + ts); 
     string text =  "I know what's behind all the doors and" 
     ...
     string::size_type pos = text.find( ("Door No.X")); 
