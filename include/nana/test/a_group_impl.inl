...
namespace dm
{
    ...
    public:
	    ...
	    {
		    widgets_.emplace_back(new T(*this, forward<Args>(args)...)); // warning C4800: 'const wchar_t *': forcing value to bool 'true' or 'false'  
    ...
    public:
          ...
          titel (*this, titel_)
        {
            ...
            drawing dw(*this);
		    dw.draw([gap,sz,bg,obg](graphics& graph)
		    {
			    graph.rectangle(true, obg);
                graph.round_rectangle(rectangle(       point ( gap-1,   sz.height/2), 
                                                 size  (graph.width()-2*(gap-1),   graph.height()-sz.height/2-(gap-1))),
