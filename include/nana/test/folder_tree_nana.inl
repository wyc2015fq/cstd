...
int main()
{
	...
	fm.caption("Nana C++ Library - Treebox-filesystem example.");
	treebox tree{ fm, { 10, 10, 380, 480 } };
    
	auto node = tree.insert(fs_ext::def_root, fs_ext::def_rootname);

	dir_it sub_root{ fs_ext::def_rootstr};
	auto p = sub_root->path();
	string dir_name=p.filename().generic_u8string();
	...
	tree.events().expanded([&tree](const arg_treebox& arg)
	{
		...
		{
			string dir_name=dir.path().filename().generic_u8string();
			...
			if (d != dir_it{})
			{
				string sdir_name = d->path().filename().generic_u8string();
