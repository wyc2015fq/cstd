...
int main()
{
	...
	//	the third parameter and after the thired, are passed to the constructor of button. button(PaneHandle, forward<Args>(third_args)...);
	plc.dock<button>("pane1", "f1", string("Button1"));
	plc.dock<button>("pane2", "f2", string("Button2"));
	plc.dock<button>("pane3", "f3", string("Button3"));
	plc.dock<button>("pane4", "f4", string("Button4"));
