...
namespace nana
{
	typedef edge_nimbus_renderer<bedrock::core_window_t> edge_nimbus_renderer_t;
	...
	//class drawer_trigger
		...
		void drawer_trigger::filter_event(const vector<event_code> evt_codes, const bool bDisabled)
		{
		...
		{
			for (int i = 0; i < static_cast<int>(event_code_end); i++)
				filter_event(static_cast<event_code>(i), false);
		}
	...
	bool event_filter_status::operator[](const event_code evt_code) const
	{
		return static_cast<bool>((evt_disabled_ >> static_cast<int>(evt_code)) & 1);
	}
	...
	namespace detail
	{
		...
		enum{
			event_size = static_cast<int>(event_code_end)
		};

		struct drawer::data_implement
		{
			bool			refreshing{ false };
			basic_window*	window_handle{ NULL };
			drawer_trigger*	realizer{ NULL };
			method_state	mth_state[event_size];
			vector<dynamic_drawing::object*>	draws;
		};
		...
		{
			_m_emit(event_code_click, arg, &drawer_trigger::click, bForce__EmitInternal);
		}

		void drawer::dbl_click(const arg_mouse& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_dbl_click, arg, &drawer_trigger::dbl_click, bForce__EmitInternal);
		}

		void drawer::mouse_enter(const arg_mouse& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_mouse_enter, arg, &drawer_trigger::mouse_enter, bForce__EmitInternal);
		}

		void drawer::mouse_move(const arg_mouse& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_mouse_move, arg, &drawer_trigger::mouse_move, bForce__EmitInternal);
		}

		void drawer::mouse_leave(const arg_mouse& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_mouse_leave, arg, &drawer_trigger::mouse_leave, bForce__EmitInternal);
		}

		void drawer::mouse_down(const arg_mouse& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_mouse_down, arg, &drawer_trigger::mouse_down, bForce__EmitInternal);
		}

		void drawer::mouse_up(const arg_mouse& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_mouse_up, arg, &drawer_trigger::mouse_up, bForce__EmitInternal);
		}

		void drawer::mouse_wheel(const arg_wheel& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_mouse_wheel, arg, &drawer_trigger::mouse_wheel, bForce__EmitInternal);
		}

		void drawer::mouse_dropfiles(const arg_dropfiles& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_mouse_drop, arg, &drawer_trigger::mouse_dropfiles, bForce__EmitInternal);
		}

		void drawer::resizing(const arg_resizing& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_resizing, arg, &drawer_trigger::resizing, bForce__EmitInternal);
		}

		void drawer::resized(const arg_resized& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_resized, arg, &drawer_trigger::resized, bForce__EmitInternal);
		}

		void drawer::move(const arg_move& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_move, arg, &drawer_trigger::move, bForce__EmitInternal);
		}

		void drawer::focus(const arg_focus& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_focus, arg, &drawer_trigger::focus, bForce__EmitInternal);
		}

		void drawer::key_press(const arg_keyboard& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_key_press, arg, &drawer_trigger::key_press, bForce__EmitInternal);
		}

		void drawer::key_char(const arg_keyboard& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_key_char, arg, &drawer_trigger::key_char, bForce__EmitInternal);
		}

		void drawer::key_release(const arg_keyboard& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_key_release, arg, &drawer_trigger::key_release, bForce__EmitInternal);
		}

		void drawer::shortkey(const arg_keyboard& arg, const bool bForce__EmitInternal)
		{
			_m_emit(event_code_shortkey, arg, &drawer_trigger::shortkey, bForce__EmitInternal);
		}
...
#else
					owns_caret = platform_spec::instance().caret_update(iwd->root, *iwd->root_graph, false);
#endif
				}
...
#else
					platform_spec::instance().caret_update(iwd->root, *iwd->root_graph, true);
#endif
				}
			}
		}
		...
		{
			for (auto i = begin(data_impl_->mth_state), end = end(data_impl_->mth_state); i != end; ++i)
		...
		void drawer::clear()
		{
			vector<dynamic_drawing::object*> then;
			for (auto p : data_impl_->draws)
			{
				if(p->diehard())
					then.emplace_back(p);
		...
		void* drawer::draw(function<void(graphics&)> && f, bool diehard)
		{
			if(f)
			{
				auto p = new dynamic_drawing::user_draw_function(move(f), diehard);
				data_impl_->draws.emplace_back(p);
				return (diehard ? p : NULL);
			}
			return NULL;
		}
		...
		void drawer::_m_effect_bground_subsequent()
		{
			...
			if (effect.bground)
			{
				...
				{
					graphics.blend(rectangle{ data_impl_->window_handle->other.glass_buffer.size() }, data_impl_->window_handle->other.glass_buffer, {}, 1 - effect.bground_fade_rate);
				}
			}
