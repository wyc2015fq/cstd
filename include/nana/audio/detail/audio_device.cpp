...
namespace nana{namespace audio
{
	namespace detail
	{
		...
		//class audio_device
			...
			bool audio_device::open(size_t channels, size_t rate, size_t bits_per_sample)
			{
#if defined(NANA_WINDOWS)
				close();
			...
			{
#if defined(NANA_WINDOWS)
				lock_guard<decltype(queue_lock_)> lock(queue_lock_);
...
#elif defined(NANA_LINUX)
				size_t frames = m->bufsize / bytes_per_frame_;
				size_t buffered = 0; //in bytes
				while(frames > 0)
				{
			...
			void __stdcall audio_device::_m_dev_callback(HWAVEOUT handle, UINT msg, audio_device * self, DWORD_PTR, DWORD_PTR)
			{
				if(WOM_DONE == msg)
				{
					...
					{
						lock_guard<decltype(queue_lock_)> lock(self->queue_lock_);
