...
{       /// Create an Open or Save dialog box to let user select the name of a file.
	...
	public:
		 ...
		 */
		filebox& init_path(const char* initial_directory);

		filebox& init_file(const char*); ///< Init file, if it contains a path, the init path is replaced by the path of init file.
		...
		filebox& add_filter(const char* description,  ///< for example. "Text File"
                            const char* filetype      ///< filter pattern(for example, "*.TXT")
                            );
