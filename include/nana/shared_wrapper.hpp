...
namespace system
{
	...
	class shared_wrapper
	{
		...
		struct function_ptr
		{
			typedef typename conditional<is_function<Function>::value,
										Function*,
										typename conditional<is_function<Function>::value && is_pointer<Function>::value, Function, int>::type
			>::type type;
		};
	...
	public:
		...
		typename function_ptr<Function>::type symbols(const char* symbol)
		{
			...
			//if(traits::is_function_pointer<fptr_type>::value == false)
			if (::is_function<typename remove_pointer<fptr_type>::type>::value == false)
				throw invalid_argument("shared_wrapper::symbols, template<_Function> is not a function type or a function pointer type");

			if(symbol == 0)
				throw invalid_argument("shared_wrapper::symbols, symbol is null string");

			if(impl_.handle == 0)
				throw logic_error("shared_wrapper::symbols, empty handle");

			if(impl_.symbol != symbol)
			{
				void *result = shared_helper::symbols(impl_.handle, symbol);
				if(result == 0)
					throw logic_error("shared_wrapper::symbols, empty proc address");
