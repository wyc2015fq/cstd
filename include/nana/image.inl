...
namespace paint
{
	...
	HICON image_accessor::icon(const image& img)
	{
		auto ico_res = dynamic_cast<image_ico_resource*>(img.image_ptr_.get());
		if (ico_res)
			return reinterpret_cast<HICON>(ico_res->native_handle());

		auto ico = dynamic_cast<image_ico*>(img.image_ptr_.get());
		if (ico)
	...
	//class image
		...
		shared_ptr<image::image_impl_interface> create_image(const fs::path & p)
		{
			shared_ptr<image::image_impl_interface> ptr;

			auto ext = p.extension().native();
			if (ext.empty())
				return ptr;

			transform(ext.begin(), ext.end(), ext.begin(), [](int ch)
			{
				if ('A' <= ch && ch <= 'Z')
					ch -= ('A' - 'a');
				return ch;
...
#else
				ifstream ifs(to_osmbstr(to_utf8(p.native())), ios::binary);
#endif
				if (ifs)
				{
					...
					if (*reinterpret_cast<const short*>("BM") == meta)
						ptr = make_shared<image_bmp>();
					else if (*reinterpret_cast<const short*>("MZ") == meta)
						ptr = make_shared<image_ico_resource>();
				}
			}
		...
		bool image::open(const char* file)
		{
			fs::path path(file);
		...
		bool image::open(const wchar* file)
		{
			fs::path path(file);
			image_ptr_ = create_image(path);
			return (image_ptr_ ? image_ptr_->open(path) : false);
		...
		bool image::open(const void* data, size_t bytes)
		{
			close();
		...
		{
			return ((NULL == image_ptr_) || image_ptr_->empty());
		}

		image::operator unspecified_bool_t() const
		{
			return (image_ptr_ ? &image::empty : NULL);
		}
		...
		{
			return (image_ptr_ ? image_ptr_->size() : size());
		}
		...
		void image::stretch(const rectangle& r_src, graphics& dst, const rectangle & r_dst) const
		{
			if (image_ptr_)
			{
				if (dst.empty())
