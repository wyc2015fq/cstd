...
namespace detail
{
	...
	public:
		...
		typedef list<msg_packet_tag> msg_queue_type;
		...
		void insert(Window wd)
		{
			...
			{
				lock_guard<decltype(table_.mutex)> lock(table_.mutex);

				//No thread is running, so msg dispatcher should start the msg driver.
				start_driver = (0 == table_.thr_table.size());
				thread_binder * thr;

				map<unsigned, thread_binder*>::iterator i = table_.thr_table.find(tid);
				if(i == table_.thr_table.end())
				{
					thr = new thread_binder;
					thr->tid = tid;
					table_.thr_table.insert(make_pair(tid, thr));
				}
				else
					thr = i->second;
			...
			if(start_driver && proc_.event_proc && proc_.timer_proc)
			{
				...
				is_work_ = true;
				thrd_ = unique_ptr<thread>(new thread([this](){ this->_m_msg_driver(); }));
			}
		}
	...
	private:
		...
		bool _m_wait_for_queue(unsigned tid)
		{
			...
			unique_lock<decltype(thr->mutex)> lock(thr->mutex);
			return (thr->cond.wait_for(lock, chrono::milliseconds(10)) != cv_status::timeout);
		}
		
	private:
		Display * display_;
		volatile bool is_work_{ false };
		unique_ptr<thread> thrd_;

		struct table_tag
		{
			recursive_mutex mutex;
			map<unsigned, thread_binder*> thr_table;
			map<Window, thread_binder*> wnd_table;
		}table_;
