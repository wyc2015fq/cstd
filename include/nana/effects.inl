...
namespace nana
{
	namespace effects
	{
		...
		namespace impl
		{
			...
			public:
				...
				void take_effect(window wd, graph_reference graph) const
				{
					if(fade_rate_ < 0.001)
						return;
					graph.blend(rectangle{ graph.size() }, API::bgcolor(wd), fade_rate_);
				}
			private:
			...
			public:
				...
				{
					graph.blur(rectangle{ graph.size() }, radius_);
				}
			private:
		...
		//class bground_transparent
			bground_transparent::bground_transparent(size_t percent)
				: percent_(percent)
			{}
		...
		//class bground_blur
			bground_blur::bground_blur(size_t radius)
				: radius_(radius)
			{}
