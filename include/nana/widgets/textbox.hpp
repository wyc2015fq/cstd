...
namespace nana
{
	...
	{
		textbox& widget;
		const vector<upoint>& text_position;	///< position of characters that the first character of line which are displayed

		arg_textbox(textbox&, const vector<upoint>&);
	};
	...
	namespace drawerbase
	{
		namespace textbox
		{
			...
			{
			public:
				event_agent(textbox&, const vector<upoint>&);
			private:
			...
			private:
				...
				void text_exposed(const vector<upoint>&);
			private:
				textbox & widget_;
				const vector<upoint>& text_position_;
			};
	...
	class textbox
		:public widget_object<category::widget_tag, drawerbase::textbox::drawer, drawerbase::textbox::textbox_events, widgets::skeletons::text_editor_scheme>
	{
	public:
		...
		using text_positions = vector<upoint>;
		...
		void store(string file, unicode encoding);
		...
		textbox& indention(bool, function<string()> generator = {});
		 ...
		 */
		textbox& reset(const char* text = string(), bool end_caret = true);      ///< discard the old text and set a new text
		...
		bool getline(size_t pos, char*) const;

		/// Read the text from a specified line with a set offset. It returns true for success.
		bool getline(size_t line_index,size_t offset,char* text) const;
		...
		textbox& append(const char* text, bool at_caret);
        textbox& append(const wchar* text, bool at_caret);
		...
		bool selected() const;
		bool get_selected_points(upoint &a, upoint &b) const;
		 ...
		 */
		pair<upoint, upoint> selection() const;
		...
		void set_highlight(const char* name, const color& fgcolor, const color& bgcolor);
		void erase_highlight(const char* name);
		void set_keywords(const char* name, bool case_sensitive, bool whole_word_match, initializer_list<wstring> kw_list);
		void set_keywords(const char* name, bool case_sensitive, bool whole_word_match, initializer_list<string> kw_list_utf8);
		void erase_keyword(const char* kw);

		/// Sets the text alignment
		textbox& text_align(align alignment);
		 ...
		 */
		size_t display_line_count() const noexcept;

		/// Returns the number of text lines.
		size_t text_line_count() const noexcept;
	protected:
		//Overrides widget's virtual functions
		native_string_type _m_caption() const;
		void _m_caption(native_string_type&&);
		void _m_typeface(const font&);
	};
}//end namespace nana
