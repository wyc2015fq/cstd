...
namespace nana
{
	...
	namespace drawerbase
	{
		namespace spinbox
		{
			...
			public:
				...
				virtual bool value(const char* new_value, bool& diff) = 0;

				virtual bool check_value(const char*) const = 0;
			...
			public:
				...
				bool value(const char* value_str, bool & diff)
				{
					stringstream ss;
					ss << value_str;
				...
				bool check_value(const char* str) const
				{
					if (str.empty())
						return true;
			...
			{
			public:
				range_text(vector<string>&& texts):
					texts_(move(texts))
				{
				}

				const vector<string>& range() const
				{
					return texts_;
				}
				...
				bool value(const char* value_str, bool & diff)
				{
					auto i = find(texts_.cbegin(), texts_.cend(), value_str);
					if (i != texts_.cend())
					{
						diff = (*i == value_str);
			...
			class implementation
			{
				...
				public:
					...
					void first_change() {}	//empty, because spinbox does not have this event.

					void text_changed()
					{
						auto wdg = static_cast<spinbox*>(API::get_widget(impl_->editor_->window_handle()));
			...
			public:
				...
				void attach(widget& wdg, graphics& graph)
				{
					auto wd = wdg.handle();
					graph_ = &graph;
					auto scheme = static_cast<widgets::skeletons::text_editor_scheme*>(API::dev::get_scheme(wd));
					editor_ = new widgets::skeletons::text_editor(wd, graph, scheme);
				...
				bool value(const char* value_str, bool reset_editor)
				{
					bool diff;
				...
				void set_range(unique_ptr<range_interface> ptr)
				{
					range_.swap(ptr);
				...
				void modifier(char*& prefix, char*& suffix)
				{
					modifier_.prefix = move(prefix);
					modifier_.suffix = move(suffix);
				...
				bool mouse_button(const arg_mouse& arg, bool pressed)
				{
					if (!pressed)
					{
				...
				bool mouse_move(bool left_button, const point& pos)
				{
			...
			private:
				...
				buttons _m_where(const point& pos) const
				{
					auto spins_r = _m_spins_area();
			...
			private:
				...
				unique_ptr<range_interface> range_;
				timer timer_;
	...
	spinbox::spinbox(window wd, const rectangle& r, bool visible)
	{
	...
	void spinbox::range(int begin, int last, int step)
	{
		...
		get_drawer_trigger().impl()->set_range(unique_ptr<range_interface>(new range_numeric<int>(begin, last, step)));
	...
	{
		using namespace drawerbase::spinbox;
		get_drawer_trigger().impl()->set_range(unique_ptr<range_interface>(new range_numeric<double>(begin, last, step)));
		API::refresh_window(handle());
	}

	void spinbox::range(vector<string> values)
	{
		using namespace drawerbase::spinbox;
		get_drawer_trigger().impl()->set_range(unique_ptr<range_interface>(new range_text(move(values))));
		API::refresh_window(handle());
	}

	vector<string> spinbox::range_string() const
	{
		auto range = dynamic_cast<const drawerbase::spinbox::range_text*>(get_drawer_trigger().impl()->range());
		if (NULL == range)
			throw runtime_error("the type of spinbox range is not string");

		return range->range();
	}

	pair<int, int> spinbox::range_int() const
	{
		auto range = dynamic_cast<const drawerbase::spinbox::range_numeric<int>*>(get_drawer_trigger().impl()->range());
		if (NULL == range)
			throw runtime_error("the type of spinbox range is not integer");

		return range->range();
	}

	pair<double, double> spinbox::range_double() const
	{
		auto range = dynamic_cast<const drawerbase::spinbox::range_numeric<double>*>(get_drawer_trigger().impl()->range());
		if (NULL == range)
			throw runtime_error("the type of spinbox range is not double");

		return range->range();
	}

	::string spinbox::value() const
	{
		internal_scope_guard lock;
		if (handle())
	...
	void spinbox::value(const char* s)
	{
		internal_scope_guard lock;
		if (handle())
		{
	...
	void spinbox::modifier(string prefix, string suffix)
	{
		get_drawer_trigger().impl()->modifier(move(prefix), move(suffix));
	}

	void spinbox::modifier(const wstring & prefix, const wchar* suffix)
	{
		modifier(to_utf8(prefix), to_utf8(suffix));
	}

	auto spinbox::_m_caption() const -> native_string_type
	{
		internal_scope_guard lock;
		auto editor = get_drawer_trigger().impl()->editor();
		if (editor)
