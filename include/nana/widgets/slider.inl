...
namespace nana
{
	...
	namespace drawerbase
	{
		namespace slider
		{
			...
			{
			private:
				void background(window, graph_reference graph, bool transparent, const scheme& schm)
				{
					if (!transparent)
						graph.rectangle(true, schm.background);
				}

				void bar(window, graph_reference graph, const data_bar& data, const scheme& schm)
				{
					auto area = data.area;
				...
				void adorn(window, graph_reference graph, const data_adorn& data, const scheme& schm)
				{
					rectangle area{
				...
				void vernier(window, graph_reference graph, const data_vernier& data, const scheme& schm)
				{
					if (data.vert)
				...
				void slider(window, graph_reference graph, mouse_action mouse_act, const data_slider& data, const scheme& schm)
				{
					rectangle area{ graph.size() };
			...
			private:
				void _m_draw_vernier_horz(graph_reference graph, const data_vernier& data, const scheme& schm)
				{
					...
					graphics graph_vern{ label_size };
					...
					{
						for (size::value_type l = 0; l < arrow_size.height; ++l)
						{
							auto ptr = arrow_pxbuf + l * arrow_size.width;

							for (size::value_type x = 0; x < arrow_size.width / 2; ++x)
								swap(ptr[x], ptr[(arrow_size.width - 1) - x]);
						}
					}

					pixel_buffer pxbuf{ arrow_size.width, arrow_size.height };
				...
				void _m_draw_vernier_vert(graph_reference graph, const data_vernier& data, const scheme& schm)
				{
					...
					graphics graph_horz{ size(label_size).shift() };
					...
					pixel_buffer{ graph_horz.handle(), 0, graph_horz.height() }.rotate(90, colors::white).paste(graph_vern.handle(), {});
					...
					if (static_cast<int>(label_size.height + arrow_weight) > (data.end_position - data.position))
					{
						...
						for (size::value_type l = 0; l < arrow_size.height / 2; ++l)
						{
							...
							unsigned tmp[9];
							memcpy(tmp, swap_x, line_bytes);
							memcpy(swap_x, swap_y, line_bytes);
							memcpy(swap_y, tmp, line_bytes);
						}
					}
					else
					{
			...
			public:
				...
				void attached(slider& wdg, graph_reference)
				{
				...
				void vernier(function<string(unsigned maximum, unsigned cursor_value)> vernier_string)
				{
				...
				parts seek_where(point pos) const
				{
					rectangle r = _m_bar_area();
				...
				void set_slider_refpos(point pos)
				{
					if (attr_.slider.vert)
						swap(pos.x, pos.y);

					slider_state_.mouse_state = mouse_action::pressed;
				...
				bool release_slider()
				{
					if(mouse_action::pressed == slider_state_.mouse_state)
					{
						API::release_capture(other_.wd);

						if (other_.wd != API::find_window(API::cursor_position()))
						{
							slider_state_.mouse_state = mouse_action::normal;
							attr_.is_draw_adorn = false;
						}
						else
							slider_state_.mouse_state = mouse_action::hovered;
				...
				{
					return (mouse_action::pressed == slider_state_.mouse_state);
				}

				bool move_slider(const point& pos)
				{
				...
				bool move_adorn(const point& pos)
				{
				...
				bool reset_adorn()
				{
					...
					if((mouse_action::pressed == slider_state_.mouse_state) && (API::capture_window() == this->other_.wd))
						return false;

					auto state_changed = ((slider_state_.mouse_state != mouse_action::normal)
										|| (attr_.adorn_pos != attr_.slider.pos));
			...
			private:
				...
				{
					other_.widget->events().value_changed.emit(arg_slider{ *other_.widget }, other_.widget->handle());
				}

				rectangle _m_bar_area() const
				{
			...
			private:
				...
				struct prototype_tag
				{
					...
					function<string(unsigned maximum, unsigned vernier_value)> vernier;
				}proto_;

				struct slider_state_tag
				{
					int		snap_pos;
					point refpos; //a point for slider when the mouse was clicking on slider.
					mouse_action mouse_state{ mouse_action::normal };
				}slider_state_;
			};
			...
			//class trigger
				...
				{
					model_ptr_->attached(static_cast< slider&>(widget), graph);
				}
	...
	//class slider
		...
		void slider::vernier(function<string(unsigned maximum, unsigned cursor_value)> vernier_string)
		{
