	...
	namespace drawerbase
	{
		namespace listbox
		{
			...
			public:
				...
				virtual string text() const noexcept = 0;

				/// Sets the caption of column
				/**
				 * @param text_utf8 A UTF-8 string for the caption.
				 */
				virtual void text(string text_utf8) = 0;

				/// Sets alignment of column text
				/**
				 * @param align Alignment
				 */
				virtual void text_align(align align) noexcept = 0;
			...
			class const_virtual_pointer
			{
				...
				const_virtual_pointer(const const_virtual_pointer&) {}
				const_virtual_pointer& operator=(const const_virtual_pointer&) {}

				const_virtual_pointer(const_virtual_pointer&) {}
				const_virtual_pointer& operator=(const_virtual_pointer&) {}
			public:
				...
				template<typename Type>
				const typename remove_const<Type>::type *get() const noexcept
				{
					using value_type = typename remove_const<Type>::type;
					auto target = dynamic_cast<real_pointer<value_type>*>(intern_);
					return (target ? target->ptr : NULL);
				}
			private:
				intern * intern_;
			};
			...
			struct cell
			{
				struct format
				{
					...
					format(const color& bgcolor, const color& fgcolor) noexcept;
				};

				using format_ptr = ::unique_ptr<format>;
				...
				cell(const cell&);
				cell(cell&&) noexcept;
				cell(::string) noexcept;
				cell(::string, const format&);
			...
			public:
				...
				virtual void emplace(size_t pos) = 0;
				virtual void emplace_back() = 0;

				virtual void assign(size_t pos, const vector<cell>& cells) = 0;
				virtual vector<cell> to_cells(size_t pos) const = 0;
			...
			{
				using value_translator = function<Value(const vector<cell>& cells)>;
				using cell_translator = function<vector<cell>(const Value&)>;
			...
			private:
				...
				void assign(size_t pos, const vector<cell>& cells)
				{
					container_.at(pos) = translator_.to_value(cells);
				}

				vector<cell> to_cells(size_t pos) const
				{
					return translator_.to_cell(container_.at(pos));
				}

				bool push_back(const const_virtual_pointer& dptr)
				{
			...
			private:
				void clear()
				{
					throw runtime_error("listbox disallow to remove items because of immutable model");
				}
			...
			class model_guard
			{
				model_guard(const model_guard&) {}
				model_guard& operator=(const model_guard&) {}
			public:
				...
				{
					using stlcontainer = typename decay<STLContainer>::type;

					if (!model_)
						throw runtime_error("listbox empty model_guard");

					using type = basic_container<stlcontainer>;
					auto p = dynamic_cast<type*>(model_->container());
					if (NULL == p)
						throw invalid_argument("invalid listbox model container type");

					if (NULL == p->pointer())
						throw runtime_error("the modal is immutable, please declare model_guard with const");
			...
			public:
				...
				container_interface* container() noexcept
				{
					return &container_;
				}

				const container_interface* container() const noexcept
				{
					return &container_;
				}
			private:
			...
			public:
				...
				shared_model_container(STLContainer& container, value_translator vtrans, cell_translator ctrans)
					: container_ptr_(new shared_container<STLContainer>(container, move(vtrans), move(ctrans)))
				{
				}

				shared_model_container(const STLContainer& container, cell_translator ctrans)
					: container_ptr_(new shared_immutable_container<STLContainer>(container, move(ctrans)))
				{
				}
			...
			private:
				Mutex mutex_;
				unique_ptr<container_interface> container_ptr_;
			};
			...
			struct index_pair
			{
				constexpr static const size_type npos = npos;
			...
			using index_pairs = ::vector<index_pair>;

			enum inline_widget_status{
				checked,
				checking,
				selected,
				selecting
			...
			using inline_notifier_interface = inline_widget_notifier_interface<index_pair, inline_widget_status, ::string>;
			...
			public:
				...
				oresolver& operator<<(const char* text_utf8);
				oresolver& operator<<(const wchar*);
				oresolver& operator<<(wchar*&);
				oresolver& operator<<(cell);
				oresolver& operator<<(nullptr_t);

				vector<cell> && move_cells() noexcept;
			...
			{
			public:
				iresolver(vector<cell>) noexcept;
				...
				iresolver& operator>>(char* text_utf8);
				iresolver& operator>>(wchar*);
				iresolver& operator>>(cell&);
				iresolver& operator>>(nullptr_t) noexcept;
			private:
				vector<cell> cells_;
				size_t pos_{0};
			};
			...
			public:
				...
				*/
				...
				item_proxy & bgcolor(const color&);
				color bgcolor() const;

				item_proxy& fgcolor(const color&);
				color fgcolor() const;
				...
				item_proxy&		text(size_type abs_col, string);
				item_proxy&		text(size_type abs_col, const wchar*);
				string	text(size_type abs_col) const;
				...
				T & value() const
				{
					auto * pany = _m_value();
					if(NULL == pany)
						throw runtime_error("listbox::item_proxy.value<T>() is empty");

					T * p = any_cast<T>(_m_value());
					if(NULL == p)
						throw runtime_error("listbox::item_proxy.value<T>() invalid type of value");
					return *p;
				}
				template<typename T>
				T & value() 
			...
			private:
				vector<cell> _m_cells() const;
				any		* _m_value(bool alloc_if_empty);
				const any	* _m_value() const;
			private:
				essence * ess_;
				category_t*	cat_{NULL};

				index_pair	pos_; //Position of an item, it never represents a category when item proxy is available.
			};

			class cat_proxy
				: public iterator < input_iterator_tag, cat_proxy >
			{
			public:
				...
				{
					_m_reset_model(new standalone_model_container<typename decay<STLContainer>::type, Mutex>(forward<STLContainer>(container), move(vtrans), move(ctrans)));
				}

				template<typename Mutex, typename STLContainer, typename ValueTranslator, typename CellTranslator>
				void shared_model(STLContainer& container, ValueTranslator vtrans, CellTranslator ctrans)
				{
					_m_reset_model(new shared_model_container<typename decay<STLContainer>::type, Mutex>(container, move(vtrans), move(ctrans)));
				}
				...
				{
					_m_reset_model(new shared_model_container<typename decay<STLContainer>::type, Mutex>(container, move(ctrans)));
				}

				model_guard model();

				/// Appends one item at the end of this category with the specifies text in the column fields
				void append(initializer_list<string> texts_utf8);
				void append(initializer_list<wstring> texts);

				size_type columns() const;

				cat_proxy& text(string);
				cat_proxy& text(wstring);
				string text() const;
				...
				void push_back(string text_utf8);
			...
			private:
				void _m_append(vector<cell> && cells);
			...
			struct export_options
			{
				...
				using columns_indexs = vector<size_type>;
	...
	namespace drawerbase
	{
		namespace listbox
		{
				...
				: public widget_geometrics
			{
				...
				parameters::mouse_wheel mouse_wheel{}; ///< The number of lines/characters to scroll when vertical/horizontal mouse wheel is moved.
			};
		}
	}//end namespace drawerbase
...
3. listbox creates the category 0 by default. 
...
4. A sort compare is used for sorting the items. It is a strict weak ordering comparer that must meet the requirement:
	...
	A simple example.
		bool sort_compare( const char* s1, any*, 
						   const char* s2, any*, bool reverse)
		{
	...
	sorted by comparing these customer's object.
		bool sort_compare( const char*, any* o1, 
						   const char*, any* o2, bool reverse)
		{
...
with API::dev::make_scheme<Scheme>() which call API::make_scheme(scheme_factory<Scheme>())
which call bedrock.make_scheme(static_cast<scheme_factory_base&&>(factory));
which call pi_data_->scheme.create(move(factory));
which call factory.create(scheme_template(move(factory)));
...
the basic_window member pointer scheme
...
*/
	...
	//Associative category access
		...
		*/
		...
		{
			using key_type = typename type_escape<const typename decay<Key>::type>::type;

			auto p = make_shared<key<key_type, less<key_type>>>(forward<Key>(key));
		...
		*/
		...
		cat_proxy assoc_at(Key&& key)
		{
			...
			if (NULL == categ)
				throw out_of_range("listbox: invalid key.");
		...
		*/
		...
		void assoc_erase(Key&& key)
		{
			...
			key<key_type, less<key_type>> wrap(key);
			_m_erase_key(&wrap);
		}
		...
		void scroll(bool to_bottom, size_type cat_pos = npos);
		...
		size_type append_header(string text_utf8, unsigned width = 120);
		size_type append_header(wstring text, unsigned width = 120);

		cat_proxy append(string category);		///< Appends a new category to the end
		cat_proxy append(wstring category);		///< Appends a new category to the end
		void append(initializer_list<string> categories); ///< Appends categories to the end
		void append(initializer_list<wstring> categories); ///< Appends categories to the end
		 ...
		 * @except out_of_range if !(pos < columns())
		 */
		...
		*/
		...
		cat_proxy insert(cat_proxy, ::string);
		cat_proxy insert(cat_proxy, ::wstring);
		 ...
		 */
		void insert_item(const index_pair& abs_pos, ::string text);
		 ...
		 */
		void insert_item(const index_pair& abs_pos, const ::wchar* text);
		...
		void set_sort_compare(	size_type col,
								function<bool(const char*, any*, const char*, any*, bool reverse)> strick_ordering);
		 ...
		 * @return the reference of *this.
		 */
		listbox& category_icon(function<void(graphics& graph, const rectangle& rt_icon, bool expanded)> icon_renderer);
		 ...
		 * @return the reference of *this.
		 */
		listbox& category_icon(const image& img_expanded, const image&& img_collapsed);
	private:
		drawerbase::listbox::essence & _m_ess() const;
		any* _m_anyobj(size_type cat, size_type index, bool allocate_if_empty) const;
		drawerbase::listbox::category_t* _m_assoc(shared_ptr<key_interface>, bool create_if_not_exists);
		void _m_erase_key(key_interface*) noexcept;
	};
}//end namespace nana
