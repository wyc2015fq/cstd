		...
		namespace skeletons
		{
			struct content_view::implementation
			{
					...
					window_handle(handle)
				{
					...
					{
						auto curs = API::cursor_position();
						API::calc_window_point(window_handle, curs);
				...
				bool drive(const point& cursor_pos)
				{
					...
					auto speed_horz = 0;
					if (skew.x)
						speed_horz = skew.x / (max)(1, static_cast<int>(horz.step())) + (skew.x < 0 ? -1 : 1);

					auto speed_vert = 0;
					if (skew.y)
						speed_vert = skew.y / (max)(1, static_cast<int>(vert.step())) + (skew.y < 0 ? -1 : 1);

					speed_horz = (min)(5, (max)(speed_horz, -5));
					speed_vert = (min)(5, (max)(speed_vert, -5));
			...
			void content_view::disp_area(const rectangle& da, const point& skew_horz, const point& skew_vert, const size& extra_px, bool try_update)
			{
				if (impl_->disp_area != da)
				{
					...
					if (static_cast<int>(impl_->content_size.width) - impl_->origin.x < static_cast<int>(imd_area.width))
						impl_->origin.x = (max)(0, static_cast<int>(impl_->content_size.width) - static_cast<int>(imd_area.width));

					if (static_cast<int>(impl_->content_size.height) - impl_->origin.y < static_cast<int>(imd_area.height))
						impl_->origin.y = (max)(0, static_cast<int>(impl_->content_size.height) - static_cast<int>(imd_area.height));
			...
			void content_view::set_wheel_speed(function<unsigned()> fn)
			{
				impl_->provider.wheel_speed = move(fn);
			}
		}
	}
}
