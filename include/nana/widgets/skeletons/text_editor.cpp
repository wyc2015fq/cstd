...
namespace nana{	namespace widgets
{
	namespace skeletons
	{
		...
		public:
			...
			using container = deque<unique_ptr<undoable_command_interface<command>>>;
			...
			void push(unique_ptr<undoable_command_interface<command>> && ptr)
			{
				if (!ptr || !enabled_)
					return;
				...
				commands_.emplace_back(move(ptr));
				++pos_;
			}

			size_t count(bool is_undo) const
			{
		...
		private:
			container commands_;
			bool		enabled_{ true };
			size_t max_steps_{ 30 };
			size_t pos_{ 0 };
		};

		template<typename T>
		using undo_command_ptr = unique_ptr <undoable_command_interface<T>> ;
		...
		protected:
			EnumCommand get() const
			{
				return cmd_;
			}

			virtual bool merge(const undoable_command_interface<EnumCommand>&)
			{
				return false;
			}
		protected:
		...
		public:
			...
			void set_removed(wstring str)
			{
				selected_text_ = move(str);
			}

			void execute(bool redo)
			{
				...
				if (redo)
				{
					...
					else
					{
						if (is_enter)
						{
							editor_.points_.caret = upoint(0, pos_.y + 1);
		...
		{
		public:
			undo_input_text(text_editor & editor, const wchar* text)
			...
			void execute(bool redo)
			{
				...
				else
				{
					...
					if (!selected_text_.empty())
					{
						editor_.points_.caret = (min)(sel_a_, sel_b_);
						editor_._m_put(selected_text_);
						editor_.points_.caret = sel_b_;
		...
		public:
			...
			void set_destination(const upoint& dest_a, const upoint& dest_b)
			{
				dest_a_ = dest_a;
				dest_b_ = dest_b;
			}
		private:
			upoint dest_a_, dest_b_;
		};
		...
		struct keyword_desc
		{
			...
			keyword_desc(const wchar* txt, const char* schm, bool cs, bool wwm)
		...
		public:
			...
			shared_ptr<colored_area_type> find(size_t pos) const
			{
		...
		public:
			...
			shared_ptr<colored_area_type> get(size_t line_pos)
			{
#ifdef _MSC_VER
				...
				return *colored_areas_.emplace(i,
						make_shared<colored_area_type>(colored_area_type{line_pos, 1, color{}, color{}})
					);
			}
			...
			shared_ptr<colored_area_type> at(size_t index)
			{
				return colored_areas_.at(index);
			}
		private:
			window window_handle_;
			vector<shared_ptr<colored_area_type>> colored_areas_;
		};
		...
		struct text_editor::implementation
		{
			...
			vector<upoint> text_position;	//positions of text since last rendering.
			...
			struct inner_capacities
			{
				...
				function<bool(char_type)> pred_acceptive;
			}capacities;

			struct inner_counterpart
			{
				bool enabled{ false };
				graphics buffer;	//A offscreen buffer which keeps the background that painted by external part.
			}counterpart;
			...
			{
				map<string, shared_ptr<keyword_scheme>> schemes;
				deque<keyword_desc> base;
			}keywords;

			unique_ptr<content_view> cview;
		};
		...
		{
		public:
			using row_coordinate = pair<size_t, size_t>; ///< A coordinate type for line position. first: the absolute line position of text. second: the secondary line position of a part of line.
			 ...
			 */
			virtual vector<text_section> line(size_t pos) const = 0;
			...
			virtual void merge_lines(size_t first, size_t second) = 0;
			//Calculates how many lines the specified line of text takes with a specified pixels of width.
			virtual void add_lines(size_t pos, size_t lines) = 0;
			virtual void prepare() = 0;
			virtual void pre_calc_line(size_t line, unsigned pixels) = 0;
			virtual void pre_calc_lines(unsigned pixels) = 0;
			virtual size_t take_lines() const = 0;
			/// Returns the number of lines that the line of text specified by pos takes.
			virtual size_t take_lines(size_t pos) const = 0;
		};
		...
		public:
			...
			vector<text_section> line(size_t pos) const
			{
				//Every line of normal behavior only has one text_section
				vector<text_section> sections;
				sections.emplace_back(this->sections_[pos]);
				return sections;
			}

			row_coordinate text_position_from_screen(int top) const
			{
				const size_t textlines = editor_.textbase().lines();
				const auto line_px = static_cast<int>(editor_.line_height());
				if ((0 == textlines) || (0 == line_px))
					return{};

				if (top < editor_.text_area_.area.y)
					top = (max)(editor_._m_text_topline() - 1, 0);
				else
					top = (top - editor_.text_area_.area.y + editor_.impl_->cview->origin().y) / line_px;

				return{ (textlines <= static_cast<size_t>(top) ? textlines - 1 : static_cast<size_t>(top)),
					0 };
			}
			...
			void merge_lines(size_t first, size_t second)
			{
				if (first > second)
					swap(first, second);
				...
				for (auto & sct : const_sections)
				{
					...
					if (sct.begin < text || (text + text.size() < sct.begin))
			...
			void add_lines(size_t pos, size_t line_size)
			{
			...
			size_t take_lines() const
			{
				return editor_.textbase().lines();
			}

			size_t take_lines(size_t) const
			{
				return 1;
			}
		private:
			text_editor& editor_;
			vector<text_section> sections_;
			...
			: public text_editor::editor_behavior_interface
		{
			struct line_metrics
			{
				size_t		take_lines;	//The number of lines that text of this line takes.
				vector<text_section>	line_sections;
			};
		public:
			...
			row_coordinate text_position_from_screen(int top) const
			{
				...
				auto text_row = (max)(0, (top - editor_.text_area_.area.y + editor_.impl_->cview->origin().y) / line_px);

				coord = _m_textline(static_cast<size_t>(text_row));
			...
			void add_lines(size_t pos, size_t lines)
			{
				if (pos < linemtr_.size())
				{
					...
					for (auto & mtr : const_linemtr)
					{
						if (line < pos || (pos + lines) <= line)
						{
							...
							if (p < linestr || (linestr + linestr.size() < p))
			...
			void pre_calc_line(size_t line, unsigned pixels)
			{
				...
				if (lnstr.empty())
				{
					...
					mtr.line_sections.emplace_back(lnstr, lnstr, unsigned{});
					mtr.take_lines = 1;
					return;
				}
				...
				unsigned text_px = 0;
				const wchar_t * secondary_begin = NULL;
				for (auto & ts : sections)
				{
					...
					if (text_px >= pixels)
					{
						...
						{
							size_t len = ts.end - ts.begin;
							unique_ptr<unsigned[]> pxbuf(new unsigned[len]);
			...
			size_t take_lines(size_t pos) const
			{
		...
		private:
			...
			void _m_text_section(const wchar* str, vector<text_section>& tsec)
			{
				if (str.empty())
				{
					tsec.emplace_back(str, str, unsigned{});
					return;
				}
				const auto end = str + str.size();
			...
			row_coordinate _m_textline(size_t scrline) const
			{
				row_coordinate coord;
				for (auto & mtr : linemtr_)
				{
		...
		private:
			text_editor& editor_;
			vector<line_metrics> linemtr_;
		}; //end class behavior_linewrapped

		class text_editor::keyword_parser
		{
		public:
			void parse(const wchar_t* c_str, size_t len, implementation::inner_keywords& keywords) //need string_view
			{
				if ( keywords.base.empty() || (0 == len) || (*c_str == 0) )
					return;

				wstring text{ c_str, len };

				using index = wstring::size_type;
				...
				for (auto & ds : keywords.base)
				{
                    ...
                    for (index rest{text.size()}; rest >= ds.text.size() ; ++pos, rest = text.size() - pos)
                    {
					    ...
					    {
						    if (cistr.empty())
							    cistr.append(text, text.size());

						    pos = cistr.find(ds.text, pos);
						    if (pos == cistr.npos)
							    break;
					    }
		...
		private:
			static bool _m_whole_word(const wchar* text, wstring::size_type pos, size_t len)
			{
				if (pos)
				{
					auto chr = text[pos - 1];
					if ((iswalpha(chr) && !iswspace(chr)) || chr == '_')
						return false;
				}
		...
		void text_editor::set_highlight(const char* name, const color& fgcolor, const color& bgcolor)
		{
			...
			auto sp = make_shared<keyword_scheme>();
			sp->fgcolor = fgcolor;
			sp->bgcolor = bgcolor;
			impl_->keywords.schemes[name].swap(sp);
		}

		void text_editor::erase_highlight(const char* name)
		{
			impl_->keywords.schemes.erase(name);
		}

		void text_editor::set_keyword(const ::wchar* kw, const char* name, bool case_sensitive, bool whole_word_matched)
		{
		...
		void text_editor::erase_keyword(const ::wchar* kw)
		{
		...
		void text_editor::set_accept(function<bool(char_type)> pred)
		{
			impl_->capacities.pred_acceptive = move(pred);
		}
		...
		void text_editor::indent(bool enb, function<string()> generator)
		{
			impl_->indent.enabled = enb;
			impl_->indent.generator = move(generator);
		}
		...
		void text_editor::text_align(align alignment)
		{
		...
		bool text_editor::text_area(const rectangle& r)
		{
		...
		bool text_editor::tip_string(::char*& str)
		{
			if(attributes_.tip_string == str)
				return false;

			attributes_.tip_string = move(str);
			return true;
		}
		...
		void text_editor::undo_max_steps(size_t maxs)
		{
			impl_->undo.max_steps(maxs);
		}

		size_t text_editor::undo_max_steps() const
		{
		...
		bool text_editor::mouse_enter(bool entering)
		{
			if ((false == entering) && (false == text_area_.captured))
				API::window_cursor(window_, cursor_arrow);
		...
		void text_editor::mouse_pressed(const arg_mouse& arg)
		{
			...
			if (event_code_mouse_down == arg.evt_code)
			{
			...
			else if (event_code_mouse_up == arg.evt_code)
			{
				select_.ignore_press = false;
		...
		bool text_editor::select_word(const arg_mouse& arg)
		{
			...
			while (select_.b.x < line.size() && !iswspace(line[select_.b.x]))
				++select_.b.x;

			// Expand the selection backward to the word's start.
			while (select_.a.x > 0 && !iswspace(line[select_.a.x - 1]))
				--select_.a.x;
		...
		bool text_editor::getline(size_t pos, wchar* text) const
		{
		...
		void text_editor::text(wstring str, bool end_caret)
		{
			impl_->undo.clear();
			...
			if (!end_caret)
			{
				auto undo_ptr = unique_ptr<undo_input_text>{ new undo_input_text(*this, str) };
				undo_ptr->set_caret_pos();

				_m_put(move(str));

				impl_->undo.push(move(undo_ptr));
		...
		wstring text_editor::text() const
		{
			wstring str;
			size_t lines = impl_->textbase.lines();
			if(lines > 0)
			{
		...
		bool text_editor::move_caret(upoint crtpos, bool stay_in_view)
		{
			...
			if (crtpos != points_.caret)
			{
				...
				{
					crtpos.x = (min)(static_cast<decltype(crtpos.x)>(impl_->textbase.getline(crtpos.y).size()), crtpos.x);
				}
				else
				{
			...
			if (text_area.is_hit(coord) && (line_bottom > text_area.y))
			{
				...
				if (line_bottom > text_area.bottom())
					caret->dimension(size(1, line_pixels - (line_bottom - text_area.bottom())));
		...
		bool text_editor::get_selected_points(upoint &a, upoint &b) const
		{
		...
		bool text_editor::hit_select_area(upoint pos, bool ignore_when_select_all) const
		{
			upoint a, b;
		...
		const vector<upoint>& text_editor::text_position() const
		{
		...
		size_t text_editor::line_count(bool text_lines) const
		{
			if (text_lines)
	...
	//public:
		void text_editor::put(wstring text)
		{
			if (text.empty())
				return;

			auto undo_ptr = unique_ptr<undo_input_text>{ new undo_input_text(*this, text) };
			...
			undo_ptr->set_caret_pos();
			points_.caret = _m_put(move(text));
		...
		void text_editor::put(wchar_t ch)
		{
			...
			auto undo_ptr = unique_ptr<undo_input_text>{new undo_input_text(*this, ch_str)};
			...
			textbase().insert(points_.caret, move(ch_str));
		...
		void text_editor::copy() const
		{
			...
			if (!text.empty())
				system::dataexch().set(text, API::root(this->window_));
		}
		...
		void text_editor::enter(bool record_undo)
		{
			...
			auto undo_ptr = unique_ptr<undo_input_text>(new undo_input_text(*this, wstring(1, '\n')));
			...
			{
				if (textbase.lines() == 0)
					textbase.insertln(0, wstring{});
				textbase.insertln(points_.caret.y, wstring{});
			}
		...
		{
			auto undo_ptr = unique_ptr<undo_backspace>(new undo_backspace(*this));
			...
			if(select_.a == select_.b)
			{
				...
				else if (points_.caret.y)
				{
					...
					undo_ptr->set_caret_pos();
					undo_ptr->set_removed(wstring(1, '\n'));
				}
				else
					undo_ptr.reset();
			}
		...
		void text_editor::set_undo_queue_length(size_t len)
		{
		...
		void text_editor::_m_handle_move_key(const arg_keyboard& arg)
		{
			...
			case keyboard::os_pageup:
				if(origin.y > 0)
				{
					...
					origin.y -= (min)(origin.y, static_cast<int>(impl_->cview->view_area().height));
					coord = off + origin;
				}
				break;
			case keyboard::os_pagedown:
				...
				{
					auto off = coord - origin;
					origin.y = (min)(origin.y + static_cast<int>(impl_->cview->view_area().height), static_cast<int>(impl_->cview->content_size().height - impl_->cview->view_area().height));
					coord = off + origin;
				}
				break;
			}
		...
		void text_editor::_m_draw_border()
		{
			if (!API::widget_borderless(this->window_))
			{
				...
				else
				{
					...
					facade.draw(graph_, _m_bgcolor(), API::fgcolor(this->window_), rectangle{ API::window_size(this->window_) }, API::element_state(this->window_));
				}
		...
		color text_editor::_m_draw_colored_area(graphics& graph, const pair<size_t, size_t>& row, bool whole_line)
		{
		...
		vector<upoint> text_editor::_m_render_text(const color& text_color)
		{
			vector<upoint> line_indexes;
			...
			{
				upoint str_pos(0, static_cast<unsigned>(row.first));
				str_pos.x = static_cast<unsigned>(sections[row.second].begin - textbase().getline(row.first));

				int top = _m_text_top_base() - (impl_->cview->origin().y % line_height());
				const unsigned pixels = line_height();

				const size_t scrlines = screen_lines() + 1;
				for (size_t pos = 0; pos < scrlines; ++pos, top += pixels)
				{
		...
		void text_editor::_m_pre_calc_lines(size_t line_off, size_t lines)
		{
		...
		point	text_editor::_m_caret_to_coordinate(upoint pos, bool to_screen_coordinate) const
		{
			auto const behavior = impl_->capacities.behavior;
			auto const sections = behavior->line(pos.y);

			size_t lines = 0;	//lines before the caret line;
			for (size_t i = 0; i < pos.y; ++i)
			{
				lines += behavior->take_lines(i);
			}

			const text_section * sct_ptr = NULL;
			point scrpos;
			if (0 != pos.x)
			{
				wstring str;
				...
				{
					size_t chsize = sct.end - sct.begin;
					str.clear();
					if (mask_char_)
					...
					if ((sct_pos + 1 < sections.size()) ? (pos.x < chsize) : (pos.x <= chsize))
					{
						sct_ptr = &sct;
						if (pos.x == chsize)
							scrpos.x = _m_text_extent_size(str, sct.end - sct.begin).width;
						else
		...
		upoint text_editor::_m_coordinate_to_caret(point scrpos, bool from_screen_coordinate) const
		{
			...
			upoint res(static_cast<unsigned>(real_str.begin - sections.front().begin), static_cast<unsigned>(row.first));

			scrpos.x = (max)(0, (scrpos.x - _m_text_x(sections[row.second])));
			...
			for (size_t i = 0; i < row.second; ++i)
				res.x += static_cast<int>(sections[i].end - sections[i].begin);

			return res;
		}

		bool text_editor::_m_pos_from_secondary(size_t textline, const upoint& secondary, unsigned & pos)
		{
			...
			auto chptr = sct.begin + (min)(secondary.x, static_cast<unsigned>(sct.end - sct.begin));
			pos = static_cast<unsigned>(chptr - textbase().getline(textline));
			return true;
		}


		bool text_editor::_m_pos_secondary(const upoint& charpos, upoint& secondary_pos) const
		{
		...
		void text_editor::_m_update_line(size_t pos, size_t secondary_count_before)
		{
			...
			if (behavior->take_lines(pos) == secondary_count_before)
			{
				...
				auto text_ptr = textbase().getline(pos);

				auto sections = behavior->line(pos);
				for (auto & sct : sections)
				{
					_m_draw_string(top, fgcolor, upoint(static_cast<unsigned>(sct.begin - text_ptr), points_.caret.y), sct, true);
					top += pixels;
				}
		...
		color text_editor::_m_bgcolor() const
		{
		...
		void text_editor::_m_reset_content_size(bool calc_lines)
		{
			...
			else
			{
				...
				csize.width = _m_text_extent_size(textbase().getline(maxline.first), maxline.second).width + caret_size().width;
			}
		...
		upoint text_editor::_m_put(wstring text)
		{
			auto & textbase = this->textbase();
			...
			if (_m_resolve_text(text, lines) && attributes_.multi_lines)
			{
				...
				if (str_orig.size() == crtpos.x)
					textbase.insert(crtpos, move(substr));
				else
			...
			else
			{
				...
				textbase.insert(points_.caret, move(text));
		...
		upoint text_editor::_m_erase_select()
		{
			upoint a, b;
			if (get_selected_points(a, b))
			{
				...
				if(a.y != b.y)
				{
					textbase.erase(a.y, a.x, wstring::npos);
		...
		wstring text_editor::_m_make_select_string() const
		{
			wstring text;
		...
		size_t eat_endl(const wchar_t* str, size_t pos)
		{
			auto ch = str[pos];
			if (0 == ch)
				return pos;
			...
			if (memcmp(str + pos, endlstr, sizeof(wchar_t) * 2) == 0)
				return pos + 2;

			return pos + 1;
		}

		bool text_editor::_m_resolve_text(const wchar* text, vector<pair<size_t, size_t>> & lines)
		{
			auto const text_str = text;
			size_t begin = 0;
			while (true)
			{
		...
		size text_editor::_m_text_extent_size(const char_type* str, size_type n) const
		{
			if(mask_char_)
			{
				wstring maskstr;
		...
		bool text_editor::_m_adjust_view()
		{
			...
			if (!attributes_.line_wrapped)
			{
				...
				if (coord.x < origin.x)
				{
					extra.x -= (min)(extra_count_horz, points_.caret.x);
				...
				{
					extra.x = (min)(static_cast<unsigned>(textbase().getline(points_.caret.y).size()), points_.caret.x + extra_count_horz);
			...
			if (coord.y < origin.y)
			{
				...
				moved_origin.y = (max)(0, coord.y - extra_px) - origin.y;
			}
			else if (coord.y + line_px >= origin.y + static_cast<int>(view_area.height))
			{
				...
				auto new_origin = (min)(coord.y + line_px + extra_px, bottom) - static_cast<int>(view_area.height);
		...
		bool text_editor::_m_move_select(bool record_undo)
		{
			...
			upoint caret = points_.caret;
			const auto text = _m_make_select_string();
			if (!text.empty())
			{
				auto undo_ptr = unique_ptr<undo_move_text>(new undo_move_text(*this));
				undo_ptr->set_selected_text();
		...
		int text_editor::_m_text_x(const text_section& sct) const
		{
			...
			if (align::left != this->attributes_.alignment)
			{
				auto blank_px = behavior->max_pixels() - sct.pixels;

				if (align::center == this->attributes_.alignment)
		...
		void text_editor::_m_draw_parse_string(const keyword_parser& parser, bool rtl, point pos, const color& fgcolor, const wchar_t* str, size_t len) const
		{
			...
			{
				const wchar_t* ent_begin = NULL;

				int ent_off = 0;
				if (str <= ent.begin && ent.begin < str_end)
				{
					ent_begin = ent.begin;
					ent_off = static_cast<int>(accumulate(glyphs, glyphs + (ent.begin - str), unsigned{}));
				}
				...
				if (ent_begin)
				{
					...
					auto ent_pixels = accumulate(glyphs + (ent_begin - str), glyphs + (ent_end - str), unsigned{});
		...
		{
		public:
			helper_pencil(graphics& graph, const text_editor& editor, keyword_parser& parser):
				graph_( graph ),
				editor_( editor ),
				parser_( parser ),
			...
			void write_selection(const point& text_pos, unsigned text_px, const wchar_t* text, size_t len, bool has_focused)
			{
				graph_.palette(true, selection_color(true, has_focused));

				graph_.rectangle(rectangle{ text_pos, { text_px, line_px_ } }, true,
					selection_color(false, has_focused));

				graph_.string(text_pos, text, len);
			}

			void rtl_string(point strpos, const wchar_t* str, size_t len, size_t str_px, unsigned glyph_front, unsigned glyph_selected, bool has_focused)
			{
				editor_._m_draw_parse_string(parser_, true, strpos, selection_color(true, has_focused), str, len);

				//Draw selected part
				graphics graph({ glyph_selected, line_px_ });
				...
				graph_.bitblt(rectangle(strpos.x + sel_xpos, strpos.y, glyph_selected, line_px_), graph);
			};
		private:
			graphics& graph_;
		...
		void text_editor::_m_draw_string(int top, const color& clr, const upoint& text_coord, const text_section& sct, bool if_mask) const
		{
			...
			const wchar_t *sbegin = NULL, *send = NULL;
			...
			if (get_selected_points(a, b))
			{
				...
				{
					auto sbegin_pos = (max)(a.x, text_coord.x);
					auto send_pos = (min)(text_coord.x + text_len, b.x);
				...
				else if (b.y == text_coord.y)
				{
					...
					{
						sbegin = text_ptr;
						send = text_ptr + (min)(b.x - text_coord.x, text_len);
					}
				}
			}
				...
				for (auto & ent : reordered)
				{
					...
					if ((text_draw_pos.x + static_cast<int>(ent_px) > text_area_.area.x) && (text_draw_pos.x < text_right))
					{
						...
						else
						{
							...
							auto ent_sbegin = (max)(sbegin, ent.begin);
							auto ent_send = (min)(send, ent.end);
							...
							auto head_px = accumulate(pxbuf.get(), pxbuf.get() + select_pos, unsigned{});
							auto select_px = accumulate(pxbuf.get() + select_pos, pxbuf.get() + select_pos + select_len, unsigned{});
			...
			if (extra_space)
			{
				...
				if (b.x != pos || text_coord.y != b.y)
				{
					...
					graph_.rectangle(rectangle{ text_draw_pos, { whitespace_w, line_h_pixels } }, true);
				}
			}
		}

		bool text_editor::_m_update_caret_line(size_t secondary_before)
		{
		...
		{
			auto len = static_cast<size_t>(ent.end - ent.begin);
			...
			if (graph_.glyph_pixels(ent.begin, len, pxbuf.get()))
			{
				...
				if (is_right_text(ent))
				{
					auto total_px = accumulate(pxbuf.get(), px_end, unsigned{});
		...
		unsigned text_editor::_m_pixels_by_char(const wchar* lnstr, size_t pos) const
		{
			if (pos > lnstr.size())
				return 0;

			auto const reordered = unicode_reorder(lnstr, lnstr.size());

			auto target = lnstr + pos;
			...
			for (auto & ent : reordered)
			{
				...
				if (ent.begin <= target && target <= ent.end)
				{
					if (is_right_text(ent))
					{
						...
						unique_ptr<unsigned[]> pxbuf(new unsigned[len]);
						graph_.glyph_pixels(ent.begin, len, pxbuf.get());
						return accumulate(pxbuf.get() + (target - ent.begin), pxbuf.get() + len, text_w);
					}
					//LTR
