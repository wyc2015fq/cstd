...
namespace nana{ namespace widgets{	namespace skeletons
{
	...
	{
	public:
		tokenizer(const wchar* s, bool format_enabled)
			:	iptr_(s.data()),
		...
		const ::wchar* idstr() const
		{
			return idstr_;
		}

		const pair<wstring, wstring>& binary() const
		{
			return binary_;
		}

		pair<unsigned, unsigned> binary_number() const
		{
			return{ stoul(binary_.first), stoul(binary_.second) };
		}

		int number() const
		{
			return stoi(idstr_, NULL, 0);
		}
	private:
	...
	struct fblock
	{
		...
		aligns::t	text_align;
		color	bgcolor;	//If the color is not specified, it will be ignored, and the system will search for its parent.
		color	fgcolor;	//ditto
	...
	private:
		...
		virtual const wchar* text() const
		{
			return str_;
		}
	...
	{
	public:
		data_image(const wchar* imgpath, const size & sz, size_t limited)
	...
	private:
		...
		virtual void nontext_render(graph_reference graph, int x, int y)
		{
			if(size_ != image_.size())
				image_.stretch(rectangle{ image_.size() }, graph, rectangle(x, y, size_.width, size_.height));
			else
	...
	public:
		typedef list<deque<value> >::iterator iterator;
		typedef deque<value> linecontainer;
		...
		void close()
		{
			...
			{
				for(deque<value>::iterator u = values.begin(); u != values.end(); ++u)
					delete u->data_ptr;
			}
		...
		void parse(const wchar* s, bool format_enabled)
		{
			close();
	...
	private:
		void _m_parse_format(tokenizer & tknizer, stack<fblock*> & fbstack)
		{
			...
			while(true)
			{
				...
				case token::target:
					if(token::equal != tknizer.read())
						throw runtime_error("error: a '=' is required behind 'target'");

					if(token::string != tknizer.read())
						throw runtime_error("error: the value of 'target' should be a string");

					fp->target = tknizer.idstr();
					break;
				case token::url:
					if(token::equal != tknizer.read())
						throw runtime_error("error: a '=' is required behind 'url'");

					if(token::string != tknizer.read())
						throw runtime_error("error: the value of 'url' should be a string");
		...
		void _m_data_factory(token tk, const wchar* idstr, fblock* fp, deque<value>& line)
		{
			value v;
			v.fblock_ptr = fp;
		...
		void _m_data_image(fblock* fp, deque<value>& line)
		{
			value v;
			v.fblock_ptr = fp;
	...
	private:
		vector<fblock*> fblocks_;
		list<deque<value> > lines_;
