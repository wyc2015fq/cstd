...
namespace nana
{
	namespace drawerbase
	{
		namespace progress
		{
			...
			public:
				...
				{
					widget_ = static_cast<progress*>(&wdg);
				}

				progress* widget_ptr() const
				{
					return widget_;
				}
				...
				unsigned inc()
				{
					auto val = value(NULL) + 1;
					return value(&val);
				}

				unsigned value(const unsigned* value_ptr)
				{
					//Sets new value if value_ptr is not a NULL
					if (value_ptr)
					{
						if (unknown_)
							value_ += (*value_ptr ? 5 : 0);
						else
							value_ = (min)(max_, *value_ptr);
				...
				{
					//Sets new maximum if value_ptr is not a NULL
					if (value_ptr)
					{
			...
			{	
				...
				if (false == progress_->unknown(NULL))
				{
				...
				else
				{
					...
					auto const value = progress_->value(NULL);

					auto left = (max)(0, static_cast<int>(value - block)) + static_cast<int>(border_px);
					auto right = static_cast<int>((min)(value, width + border_px -1));
	...
	//class progress
		...
		{
			return get_drawer_trigger().progress()->value(NULL);
		}
		...
		{
			return get_drawer_trigger().progress()->maximum(NULL);
		}
		...
		{
			return get_drawer_trigger().progress()->unknown(NULL);
		}
	//end class progress
}//end namespace nana
