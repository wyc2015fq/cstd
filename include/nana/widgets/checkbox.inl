...
namespace nana{ namespace drawerbase
{
	namespace checkbox
	{
		...
		//class drawer
			...
			void drawer::refresh(graph_reference graph)
			{
				...
				if (graph.width() > 16 + interval)
				{
					...
					if (!wdg->enabled())
					{
						...
						tr.render({ 17 + interval, 2 }, title, title.length(), pixels);
						graph.palette(true, static_cast<color_rgb>(0x808080));
					}
					else
						graph.palette(true, wdg->fgcolor());

					tr.render({ 16 + interval, 1 }, title, title.length(), pixels);
				}
			...
			void drawer::mouse_up(graph_reference graph, const arg_mouse&)
			{
				...
				{
					impl_->crook.reverse();
					arg_checkbox arg{ static_cast<checkbox*>(impl_->widget_ptr) };
					API::events<checkbox>(impl_->widget_ptr->handle()).checked.emit(arg, impl_->widget_ptr->handle());
				}
				refresh(graph);
				API::dev::lazy_refresh();
			}
	...
	//class checkbox
		...
		checkbox::checkbox(window wd, const char* text, bool visible)
			: checkbox(wd, string(text), visible)
		{
		}
	...
	//class radio_group
		...
		void radio_group::add(checkbox& uiobj)
		{
			...
			el.eh_keyboard = uiobj.events().key_press.connect_unignorable([this](const arg_keyboard& arg)
			{
				...
				auto i = find_if(ui_container_.begin(), ui_container_.end(), [window_handle](const element_tag& e){
		...
		size_t radio_group::checked() const
		{
			...
			{
				if (i->uiobj->checked())
					return static_cast<size_t>(i - ui_container_.cbegin());
			}

			return ui_container_.size();
		}

		size_t radio_group::size() const
		{
			return ui_container_.size();
		}
