...
namespace nana
{
		...
		//class trigger
			...
			//class form_base
				form_base::form_base(window owner, bool nested, const rectangle& r, const appearance& app)
					: widget_object<category::root_tag, drawerbase::form::trigger, events_root_extension>(owner, nested, r, app)
				{}

				place & form_base::get_place()
				{
					if (this->empty())
						throw runtime_error("form::get_plac: the form has destroyed.");
	...
	//class form
		...
		form::form(const form& fm, const size& sz, const appearance& apr)
		...
		form::form(const rectangle& r, const appearance& apr)
			: form_base(NULL, false, r, apr)
		{}

		form::form(window owner, const size& sz, const appearance& apr)
