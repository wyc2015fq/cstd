...
namespace nana
{
	...
	namespace drawerbase
	{
		...
		namespace treebox
		{
			...
			bool no_sensitive_compare(const char* text, const char *pattern, size_t len)
			{
				if(len <= text.length())
				{
					auto s = text;
			...
			const node_type* find_track_child_node(const node_type* node, const node_type * end, const char* pattern, size_t len, bool &finish)
			{
			...
			private:
				void _m_draw()
				{
					...
					{
						graphics item_graph({ item_r_.width, item_r_.height });
			...
			private:
				...
				virtual bool comp_attribute(component_t comp, comp_attribute_t& comp_attr) const
				{
			...
			private:
				...
				pat::cloneable<compset_placer_interface> placer_;
				widget	*widget_;
			...
			public:
				...
				{
					API::take_active(handle(), false, NULL);
				}
			...
			public:
				...
				rectangle text_pos() const;
			private:
				trigger::implement * impl_;
				point item_pos_;
				const point pos_;		//Mouse pointer position
				component	what_;
				node_type * node_;
				node_attribute node_attr_;
				rectangle node_r_;
				rectangle node_text_r_;
			};
			...
			struct trigger::basic_implement
			{
				...
				struct shape_tag
				{
					...
					mutable map<string, node_image_tag> image_table;
			...
			public:
				basic_implement()
				{
					...
					node_state.tooltip = NULL;
					node_state.comp_pointed = component::end;
					node_state.pointed = NULL;
					node_state.selected = NULL;
					node_state.pressed_node = NULL;
				...
				{
					ndattr.has_children = (NULL != node->child);
				...
				bool draw(bool reset_scroll, bool ignore_update = false, bool ignore_auto_draw = false)
				{
					if(data.graph && (false == data.stop_drawing))
					{
						...
						if (attr.auto_draw || ignore_auto_draw)
						{
							...
							//Draw tree
							attr.tree_cont.for_each(shape.first, Renderer(this, point(static_cast<int>(attr.tree_cont.indent_size(shape.first) * shape.indent_pixels) - shape.offset_x, 1)));
				...
				const trigger::node_type* find_track_node(wchar_t key)
				{
					...
					unsigned long now = system::timestamp();
					...
					if(begin)
					{
						...
						while(node)
						{
							if(no_sensitive_compare(node->value.second.text, pattern, pattern.length())) return node;

							bool finish;
							const node_type *child = find_track_child_node(node, end, pattern, pattern.length(), finish);
							if(child)
								return child;
							...
							if(!node->next)
							{
								node = (node->owner ? node->owner->next : NULL);
								if(NULL == node)
								{
									node = attr.tree_cont.get_root()->child;
									end = begin;
				...
				bool make_adjust(node_type * node, int reason)
				{
						...
						else
						{
							...
							if (visual_size > max_allow)
							{
								if (first_pos + max_allow > visual_size)
									shape.first = tree.advance_if(NULL, visual_size - max_allow, pred_allow_child{});
							}
							else
								shape.first = NULL;
						}
					...
					case 4:
						if(shape.first != node)
						{
							...
							{
								shape.first = tree.advance_if(NULL, node_pos - max_allow + 1, pred_allow_child{});
								return true;
							}
						}
						break;
				...
				bool set_checked(node_type * node, checkstate cs)
				{
					if (node && node->value.second.checked != cs)
					{
						...
						if (node->owner)
						{
							...
							data.widget_ptr->events().checked.emit(arg_treebox{ *data.widget_ptr, iprx, (checkstate::unchecked != cs) }, data.widget_ptr->handle());
							data.stop_drawing = false;
						}
						return true;
					}
				...
				bool set_selected(node_type * node)
				{
					if(node_state.selected != node)
					{
						...
						if (node_state.selected)
						{
							...
							data.widget_ptr->events().selected.emit(arg_treebox{ *data.widget_ptr, iprx, false }, data.widget_ptr->handle());
						}
						...
						{
							item_proxy iprx(data.trigger_ptr, node_state.selected);
							data.widget_ptr->events().selected.emit(arg_treebox{ *data.widget_ptr, iprx, true }, data.widget_ptr->handle());
						}
						data.stop_drawing = false;
						return true;
					}
				...
				bool set_expanded(node_type* node, bool value)
				{
					if(node && node->value.second.expanded != value)
					{
						...
						if(node->child)
						{
							data.stop_drawing = true;
							item_proxy iprx(data.trigger_ptr, node);
							data.widget_ptr->events().expanded.emit(arg_treebox{ *data.widget_ptr, iprx, value }, data.widget_ptr->handle());
							data.stop_drawing = false;
						}
						return true;
					}
				...
				void show_scroll()
				{
					if(NULL == data.graph) return;

					size_t max_allow = max_allowed();
					size_t visual_items = visual_item_size();
					...
					else
					{
						if(scroll.empty())
						{
							scroll.create(*data.widget_ptr, rectangle(data.graph->width() - 16, 0, 16, data.graph->height()));

							scroll.events().value_changed.connect_unignorable([this](const arg_scroll&)
							{
								adjust.scroll_timestamp = system::timestamp();
								adjust.timer.start();

								shape.first = attr.tree_cont.advance_if(NULL, shape.scroll.value(), pred_allow_child{});
				...
				size_t visual_item_size() const
				{
					return attr.tree_cont.child_size_if(string(), pred_allow_child{});
				}
				...
				image* image(const node_type* node)
				{
					const char* idstr = node->value.second.img_idstr;
					if(idstr.size())
					{
					...
					if (node && (nl.what() != component::end))
					{
						if ((nl.what() != node_state.comp_pointed) || (node != node_state.pointed))
						{
							...
							if (node_state.pointed)
							{
								...
								data.widget_ptr->events().hovered.emit(arg_treebox{ *data.widget_ptr, iprx, false }, data.widget_ptr->handle());
							...
							item_proxy iprx(data.trigger_ptr, node_state.pointed);
							data.widget_ptr->events().hovered.emit(arg_treebox{ *data.widget_ptr, iprx, true }, data.widget_ptr->handle());
			...
			//class item_proxy
				...
				{
					//Make it an end itertor if one of them is a NULL
					if(NULL == trg || NULL == node)
					{
						trigger_ = NULL;
						node_ = NULL;
					}
				...
				item_proxy item_proxy::append(const char* key, string name)
				{
					if(NULL == trigger_ || NULL == node_)
						return item_proxy();
					return item_proxy(trigger_, trigger_->insert(node_, key, move(name)));
				}
				...
				{
					auto * impl = trigger_->impl();
					if(impl->set_selected(s ? node_ : NULL))
						impl->draw(true);

					return *this;
				}

				const char* item_proxy::icon() const
				{
					return node_->value.second.img_idstr;
				}

				item_proxy& item_proxy::icon(const char* id)
				{
					node_->value.second.img_idstr = id;
					return *this;
				}

				item_proxy& item_proxy::key(const char* kstr)
				{
					trigger_->rename(node_, kstr.data(), NULL);
					return *this;
				}

				const char* item_proxy::key() const
				{
					return node_->value.first;
				}

				const char* item_proxy::text() const
				{
					return node_->value.second.text;
				}

				item_proxy& item_proxy::text(const char* id)
				{
					trigger_->rename(node_, NULL, id.data());
					return *this;
				}

				size_t item_proxy::size() const
				{
					size_t n = 0;
				...
				item_proxy item_proxy::visit_recursively(function<bool(item_proxy)> action)
				{
					if (!action(*this))
						return *this;
			...
			private:
				...
				virtual void enable(component_t comp, bool enabled)
				{
					switch(comp)
					{
				...
				virtual bool enabled(component_t comp) const
				{
					switch(comp)
					{
					case component_t::crook:
				...
				virtual bool locate(component_t comp, const item_attribute_t& attr, rectangle * r) const
				{
					switch(comp)
					{
				...
				: public renderer_interface
			{
				...
				void begin_paint(widget& wdg)
				{
					window_handle_ = wdg.handle();
				}

				void bground(graph_reference graph, const compset_interface * compset) const
				{
					comp_attribute_t attr;

					if(compset->comp_attribute(component::bground, attr))
					{
						const color color_table[][2] = { { { 0xE8, 0xF5, 0xFD }, { 0xD8, 0xF0, 0xFA } }, //highlighted
						{ { 0xC4, 0xE8, 0xFA }, { 0xB6, 0xE6, 0xFB } }, //Selected and highlighted
						{ { 0xD5, 0xEF, 0xFC }, {0x99, 0xDE, 0xFD } }  //Selected but not highlighted
														};

						const color *clrptr = NULL;
						...
						if (clrptr)
						{
							...
							{
								graphics item_graph{ attr.area.dimension() };
				...
				void expander(graph_reference graph, const compset_interface * compset) const
				{
					comp_attribute_t attr;
				...
				void crook(graph_reference graph, const compset_interface * compset) const
				{
					comp_attribute_t attr;
				...
				virtual void icon(graph_reference graph, const compset_interface * compset) const
				{
					comp_attribute_t attr;
					if(compset->comp_attribute(component::icon, attr))
					{
						...
						if(! img->empty())
						{
							...
							{
								size fit_size;
								fit_zoom(size, attr.area.dimension(), fit_size);
				...
				virtual void text(graph_reference graph, const compset_interface * compset) const
				{
					comp_attribute_t attr;
			...
			//class trigger::item_locator
				...
				rectangle trigger::item_locator::text_pos() const
				{
			...
			public:
				...
				item_renderer(implement * impl, const point& pos)
					:	impl_(impl),
						pos_(pos)
				{
				}
			...
			private:
				...
				virtual bool comp_attribute(component_t comp, comp_attribute_t& attr) const
				{
					attr.area = node_r_;
		...
		namespace treebox
		{
			//class trigger
				//struct treebox_node_type
					...
					trigger::treebox_node_type::treebox_node_type(string text)
						:text(move(text)), expanded(false), checked(checkstate::unchecked)
					{}
					...
					:	impl_(new implement)
				{
					...
					impl_->data.renderer = pat::cloneable<renderer_interface>(internal_renderer());
					impl_->data.comp_placer = pat::cloneable<compset_placer_interface>(internal_placer());

					impl_->adjust.timer.elapse([this]
					{
						auto & adjust = impl_->adjust;
						if (adjust.scroll_timestamp && (system::timestamp() - adjust.scroll_timestamp >= 500))
						{
							...
							{
								point pos = impl_->node_state.tooltip->pos();
				...
				void trigger::renderer(pat::cloneable<renderer_interface>&& r)
				{
					impl_->data.renderer = move(r);
				}

				const pat::cloneable<renderer_interface>& trigger::renderer() const
				{
					return impl_->data.renderer;
				}

				void trigger::placer(pat::cloneable<compset_placer_interface>&& r)
				{
					impl_->data.comp_placer = move(r);
				}

				const pat::cloneable<compset_placer_interface>& trigger::placer() const
				{
					return impl_->data.comp_placer;
				}

				trigger::node_type* trigger::insert(node_type* node, const char* key, char*& title)
				{
					node_type * p = impl_->attr.tree_cont.node(node, key);
					if(p)
						p->value.second.text.swap(title);
					else
						p = impl_->attr.tree_cont.insert(node, key, treebox_node_type(move(title)));
				...
				trigger::node_type* trigger::insert(const char* path, char*& title)
				{
					auto x = impl_->attr.tree_cont.insert(path, treebox_node_type(move(title)));
					if (x)
						impl_->draw(true);
					return x;
				}
				...
				node_image_tag& trigger::icon(const char* id) const
				{
				...
				void trigger::icon_erase(const char* id)
				{
				...
				void trigger::node_icon(node_type* node, const char* id)
				{
				...
				void trigger::attached(widget_reference widget, graph_reference graph)
				{
					...
					impl_->data.widget_ptr = static_cast< treebox*>(&widget);
					widget.caption("nana treebox");
				}

				void trigger::detached()
				{
					impl_->data.graph = NULL;
				}
				...
				void trigger::mouse_up(graph_reference, const arg_mouse& arg)
				{
					...
					impl_->node_state.pressed_node = NULL;
				...
				void trigger::mouse_leave(graph_reference, const arg_mouse&)
				{
					if (impl_->node_state.pointed && (!impl_->node_state.tooltip))
					{
						...
						impl_->data.widget_ptr->events().hovered.emit(arg_treebox{ *impl_->data.widget_ptr, iprx, false }, impl_->data.widget_ptr->handle());
						impl_->node_state.pointed = NULL;
						impl_->draw(false);
						API::dev::lazy_refresh();
					}
				}
	...
	//class treebox
		...
		const pat::cloneable<treebox::renderer_interface> & treebox::renderer() const
		{
			return get_drawer_trigger().impl()->data.renderer;
		}

		const pat::cloneable<treebox::compset_placer_interface> & treebox::placer() const
		{
		...
		treebox::node_image_type& treebox::icon(const char* id) const
		{
			return get_drawer_trigger().icon(id);
		}

		void treebox::icon_erase(const char* id)
		{
			get_drawer_trigger().icon_erase(id);
		}

		auto treebox::find(const char* keypath) -> item_proxy
		{
			auto * trg = &get_drawer_trigger();
			return item_proxy(trg, trg->impl()->attr.tree_cont.find(keypath));
		}

		treebox::item_proxy treebox::insert(const char* path_key, string title)
		{
			return item_proxy(&get_drawer_trigger(), get_drawer_trigger().insert(path_key, move(title)));
		}

		treebox::item_proxy treebox::insert(item_proxy i, const char* key, string title)
		{
			return item_proxy(&get_drawer_trigger(), get_drawer_trigger().insert(i._m_node(), key, move(title)));
		}
		...
		void treebox::erase(const char* keypath)
		{
			auto i = find(keypath);
			if (!i.empty())
				this->erase(i);
		...
		string treebox::make_key_path(item_proxy i, const char* splitter) const
		{
