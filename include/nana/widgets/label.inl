...
public:
  struct pixel_tag
  {
    ...
    size_t pixels;
    size_t baseline;	//The baseline for drawing text.
    vector<iterator> values;	//line values
  };
  ...
  public:
    ...
    void render(graph_reference graph, const color& fgcolor, align th, align_v tv)
    {
      traceable_.clear();
      ...
      deque<vector<pixel_tag> > pixel_lines;
      
      size_t extent_v_pixels = 0;	//the pixels, in height, that text will be painted.
      ...
      for (auto & line : dstream_)
      {
        ...
        pixel_lines.emplace_back(move(rs.pixels));
    ...
    bool find(int x, int y, wchar* target, wchar* url) const noexcept
    {
    ...
    size measure(graph_reference graph, unsigned limited, align th, align_v tv)
    {
      size retsize;
...
private:
  ...
  void _m_insert_if_traceable(int x, int y, const size& sz, widgets::skeletons::fblock* fbp)
  {
  ...
  void _m_set_default(const font& ft, const color& fgcolor)
  {
  ...
  const color& _m_fgcolor(widgets::skeletons::fblock* fp) noexcept
  {
  ...
  double _m_font_size(widgets::skeletons::fblock* fp) noexcept
  {
  ...
  bool _m_bold(widgets::skeletons::fblock* fp) noexcept
  {
    while(fp->bold_empty)
    {
      fp = fp->parent;
  ...
  const char* _m_fontname(widgets::skeletons::fblock* fp) noexcept
  {
    while(fp->font.empty())
    {
      fp = fp->parent;
  ...
  void _m_change_font(graph_reference graph, widgets::skeletons::fblock* fp)
  {
    if(fp != fblock_)
    {
  ...
  unsigned _m_line_pixels(dstream::linecontainer& line, unsigned def_line_pixels, render_status & rs)
  {
    ...
    vector<iterator> line_values;
    
    for(auto i = line.begin(); i != line.end(); ++i)
    {
      data * data_ptr = i->data_ptr;
      size sz = data_ptr->size();
      total_w += sz.width;
...
void _m_draw_block(graph_reference graph, const wchar* s, dstream::linecontainer::iterator block_start, render_status& rs)
{
  ...
  {
    size_t pos = bidi.begin - s.data();
    size_t len = bidi.end - bidi.begin;
...
static pair<size_t, size_t> _m_locate(dstream::linecontainer::iterator& i, size_t pos)
{
  size_t n = i->data_ptr->text().length();
  while(pos >= n)
  {
    pos -= n;
...
private:
  ...
  widgets::skeletons::fblock * fblock_ = NULL;
  ::deque<traceable> traceable_;
...
struct trigger::implement
{
  ...
  widget * wd{NULL};
  graphics * graph{NULL};
  unique_ptr<measurer> msr_ptr{ NULL };
  ...
  wstring target;	//It indicates which target is tracing.
  wstring url;
  
  window for_associated_wd{ NULL };
  
  void add_listener(function<void(command, const char*)>&& fn)
  {
    listener_.emplace_back(move(fn));
  }
  
  void call_listener(command cmd, const wchar* tar)
  {
  ...
  private:
    vector<function<void(command, const char*)>> listener_;
};
...
void trigger::mouse_move(graph_reference, const arg_mouse& arg)
{
  ...
  if(impl_->renderer.find(arg.pos.x, arg.pos.y, target, url))
  {
    ...
    if (cur_state)
      impl_->wd->cursor(1 == cur_state ? cursor_arrow : cursor_hand);
  }
  else
  {
    bool restore = false;
...
label::label(window wd, const char* text, bool visible)
{
  throw_not_utf8(text);
  create(wd, rectangle(), visible);
  bgcolor(API::bgcolor(wd));
...
label& label::format(bool f)
{
  ...
  {
    window wd = *this;
    impl->renderer.parse(to_wstring(API::dev::window_caption(wd)));
    API::refresh_window(wd);
  }
  return *this;
}

label& label::add_format_listener(function<void(command, const char*)> f)
{
  get_drawer_trigger().impl()->add_listener(move(f));
  return *this;
}

label& label::click_for(window associated_window)
{
  get_drawer_trigger().impl()->for_associated_wd = associated_window;
  return *this;
}

size label::measure(unsigned limited) const
{
  if(empty())
    return size();
...
size label::measure(graphics& graph, const char* str, unsigned allowed_width_in_pixel, bool format_enabled, align h_align, align_v v_align)
{
  throw_not_utf8(str);
...
void label::_m_caption(native_string_type&& str)
{
  ...
  API::dev::window_caption(wd, move(str));
  API::refresh_window(wd);
}
//end class label
