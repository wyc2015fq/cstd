...
namespace nana
{
	namespace drawerbase
	{
		namespace menubar
		{
			...
			public:
				...
				using scheme = drawerbase::menubar::scheme;

				item_renderer(window, graph_reference);
				virtual void background(const point&, const size&, state);
	...
	class menubar
		:	public widget_object<category::widget_tag, drawerbase::menubar::trigger, general_events, drawerbase::menubar::scheme>
	{
	public:
		...
		menu& push_back(const char*);	///< Appends a new (empty) menu.
		menu& at(size_t index) const;		    ///< Gets the menu specified by index.
		size_t length() const;		        ///< Number of menus.
		...
		bool hovered() const;
	private:
		event_handle evt_resized_{NULL};
	};//end class menubar
}//end namespace nana
