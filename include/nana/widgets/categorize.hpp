...
namespace nana
{
	...
	namespace drawerbase
	{
		namespace categorize
		{
			...
			{
			public:
				event_agent(categorize<T>& wdg)
					: widget_(wdg)
				{}

				void selected(any & var)
				{
			...
			public:
				...
				virtual ~renderer() = 0;
				virtual void background(graph_reference, window wd, const rectangle&, const ui_element&) = 0;
				virtual void root_arrow(graph_reference, const rectangle&, mouse_action) = 0;
				virtual void item(graph_reference, const rectangle&, size_t index, const char* name, unsigned textheight, bool has_child, mouse_action) = 0;
			...
			public:
				...
				void insert(const char*, any);
				bool childset(const char*, any);
				bool childset_erase(const char*);
				bool clear();

				//splitstr
				//@brief: Sets the splitstr. If the parameter will be ingored if it is an empty string.
				void splitstr(const char*);
				...
				template<typename T>
				void create_event_agent(categorize<T>& wdg)
				{
			...
			private:
				unique_ptr<event_agent_interface> event_agent_;
				scheme * scheme_{NULL};
			};
	...
	public:
		...
		categorize(window wd, bool visible)
			: categorize(wd, rectangle(), visible)
		{
		}
		...
		categorize(window wd, const wchar* text, bool visible = true)
			: categorize(wd, rectangle(), visible)
		{
			this->caption(text);
		}
		...
		categorize& insert(const char* name, const value_type& value)
		{
		...
		categorize& childset(const char* name, const value_type& value)
		{
			throw_not_utf8(name);
		...
		categorize& childset_erase(const char* name)
		{
		...
		categorize& splitstr(const char* sstr)
		{
			this->get_drawer_trigger().splitstr(sstr);
			return *this;
		}

		string splitstr() const
		{
			return this->get_drawer_trigger().splitstr();
		}

		/// Retrieves a reference of the current category's value type object. If current category is empty, it throws a exception of runtime_error.
	...
	private:
		...
		void _m_caption(native_string_type&& str)
		{
