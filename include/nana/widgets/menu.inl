...
namespace nana
{
	namespace drawerbase
	{
		namespace menu
		{
			...
			struct menu_type
			{
				...
				using item_container = vector<unique_ptr<item_type>>;
				using iterator = item_container::iterator;

				menu* owner;
				vector<menu_type*>	links;
			...
			//struct menu_item_type
				...
				menu_item_type::menu_item_type()
				{
					...
					linked.menu_ptr = NULL;
				}

				menu_item_type::menu_item_type(string text, const event_fn_t& fn)
					: text(move(text)), event_handler(fn)
				{
					flags.enabled = true;
			...
			private:
				...
				void item(graph_reference graph, const rectangle& r, const attr& at)
				{
					if (!at.enabled)
						return;
					...
					if(at.item_state == state::active)
					{
						...
						draw(graph).corner(r, 1);

						if(at.enabled)
							graph.gradual_rectangle(rectangle(r).pare_off(1), static_cast<color_rgb>(0xE8F0F4), static_cast<color_rgb>(0xDBECF4), true);
					}
					...
					if(at.checked && (checks::none != at.check_style))
					{
						...
						graph.rectangle(rectangle(r).pare_off(1), true, clr);
				...
				void item_image(graph_reference graph, const point& pos, unsigned image_px, const image& img)
				{
					if (img.size().width > image_px || img.size().height > image_px)
					{
						img.stretch(rectangle{ img.size() }, graph, rectangle{ pos, size(image_px, image_px) });
						return;
					}
				...
				void item_text(graph_reference graph, const point& pos, const char* text, unsigned text_pixels, const attr& at)
				{
					graph.palette(true, at.enabled ? colors::black : colors::gray_border);
					text_renderer tr(graph);

					auto wstr = to_wstring(text);
					tr.render(pos, wstr, wstr.length(), text_pixels, true);
				}

				void sub_arrow(graph_reference graph, const point& pos, unsigned pixels, const attr&)
				{
			...
			public:
				...
				void check_style(size_t index, checks s)
				{
					if(good_checks(s))
					{
				...
				void checked(size_t pos, bool check)
				{
				...
				bool set_linkage(size_t pos, menu_type &linked, bool own_creation)
				{
			...
			menu_item_type::item_proxy::item_proxy(size_t pos, menu* m):
				pos_{pos},
				menu_{m}
			{}
			...
			menu_item_type::item_proxy& menu_item_type::item_proxy::text(string title)
			{
				menu_->text(pos_, title);
				return *this;
			}

			string menu_item_type::item_proxy::text() const
			{
				return menu_->text(pos_);
			}

			size_t menu_item_type::item_proxy::index() const
			{
				return pos_;
			}
			...
			public:
				...
				void set_run(menu_builder& mbuilder, menu_type& menu, function<void()> && menu_tree_destroyer)
				{
					mbuilder_ = &mbuilder;
					menu_ = &menu;
					fn_close_tree_ = move(menu_tree_destroyer);
				}
				...
				void refresh(graph_reference graph)
				{
					...
					rectangle item_r(2, 2, graph_->width() - 4, item_h_px);
					...
					for (auto & m : menu_->items)
					{
						...
						wchar_t hotkey;
						string::size_type hotkey_pos;
						auto text = API::transform_shortkey_text(item_ptr->text, hotkey, &hotkey_pos);

						if (item_ptr->image.empty() == false)
							renderer->item_image(graph, point(item_r.x + 5, item_r.y + static_cast<int>(item_h_px - image_px) / 2 - 1), image_px, item_ptr->image);

						renderer->item_text(graph, point(item_r.x + 40, item_r.y + text_top_off), text, strpixels, attr);
						...
						if (item_ptr->linked.menu_ptr)
							renderer->sub_arrow(graph, point(graph_->width() - 20, item_r.y), item_h_px, attr);
				...
				bool track_mouse(const point& pos)
				{
					if (!state_.nullify_mouse)
					{
						auto & items = menu_->items;
						size_t index = _m_get_index_by_pos(pos.x, pos.y);
						if (index != state_.active)
						{
							...
							state_.active_timestamp = system::timestamp();
							return true;
						}
					}
				...
				menu_type* get_sub(point& pos, unsigned long& tmstamp) const
				{
			...
			private:
				...
				size_t _m_get_index_by_pos(int x, int y) const
				{
				...
				size _m_client_size() const
				{
					...
					if (menu_->items.size())
					{
						for (auto & m : menu_->items)
						{
							...
							{
								size item_size = graph_->text_extent_size(m->text);
			...
			private:
				widget		*widget_{NULL};
				graphics	*graph_{NULL};

				menu_builder* mbuilder_{ NULL };
				menu_type* menu_{ NULL };

				function<void()> fn_close_tree_;
				...
				{
					point	monitor_pos;	//It is used for determinating the monitor.
					upoint border;
				}detail_;
			...
			public:
				...
				menu_window(window wd, bool is_wd_parent_menu, const point& pos, menu_builder& mbuilder, menu_type& menu_data)
					...
					:	base_type(wd, false, rectangle(pos, size(2, 2)), appear::bald<appear::floating>()),
						...
						event_focus_{ NULL }
				{
					...
					submenu_.child = submenu_.parent = NULL;
					submenu_.object = NULL;
				...
				void popup(bool owner_menubar)
				{
					if (!want_focus_)
					{
						...
						API::take_active(this->handle(), false, NULL);
					}

					if(submenu_.parent == NULL)
					{
					...
					auto fn = [this](const arg_mouse& arg)
					{
						...
						else if ((event_code_mouse_up == arg.evt_code) && (mouse::left_button == arg.button))
							pick();
					};
			...
			private:
				...
				void _m_destroy()
				{
					...
					{
						this->submenu_.parent->submenu_.child = NULL;
						this->submenu_.parent->submenu_.object = NULL;
					}
				...
				bool _m_manipulate_sub(unsigned long delay_ms, bool forced)
				{
					...
					if (menu_ptr && (system::timestamp() - tmstamp < delay_ms))
						return false;
					...
					if (menu_ptr)
					{
						...
						if ((submenu_.object == NULL) && menu_ptr && (forced || root->state_.auto_popup_submenu))
						{
							...
							API::set_window_z_order(handle(), mwnd.handle(), z_order_action_none);
	...
	//class menu
		...
		auto menu::append(string text_utf8, const menu::event_fn_t& handler) -> item_proxy
		{
			unique_ptr<item_type> item{ new item_type{ move(text_utf8), handler } };
			impl_->mbuilder.data().items.emplace_back(move(item));
		...
		auto menu::insert(size_t pos, string text_utf8, const event_fn_t& handler) -> item_proxy
		{
			auto & items = impl_->mbuilder.data().items;
			if (pos > items.size())
				throw out_of_range("menu: a new item inserted to an invalid position");
		...
		menu *menu::create_sub_menu(size_t index)
		{
			unique_ptr<menu> guard{new menu};
		...
		void menu::check_style(size_t index, checks style)
		{
			impl_->mbuilder.check_style(index, style);
		}

		void menu::checked(size_t index, bool check)
		{
			impl_->mbuilder.checked(index, check);
		}

		bool menu::checked(size_t index) const
		{
			return impl_->mbuilder.data().items.at(index)->flags.checked;
		}

		void menu::answerer(size_t index, const event_fn_t& fn)
		{
			impl_->mbuilder.data().items.at(index)->event_handler = fn;
		}

		void menu::destroy_answer(function<void()> fn)
		{
			impl_->destroy_answer = move(fn);
		}

		void menu::gaps(const point& pos)
		{
		...
		{
			return (impl_->window_ptr ? impl_->window_ptr->handle() : NULL);
		}
	...
	popuper menu_popuper(menu& mobj, mouse ms)
	{
		return popuper(mobj, ms);
	}

	popuper menu_popuper(menu& mobj, window owner, const point& pos, mouse ms)
	{
		return popuper(mobj, owner, pos, ms);
	}
	...
	//class popuper
		popuper::popuper(menu& mobj, mouse ms)
			: mobj_(mobj), owner_(NULL), take_mouse_pos_(true), mouse_(ms)
		{}
