	...
	namespace button
	{
		...
		public:
			...
			optional<size> measure(graph_reference graph, unsigned limit_pixels, bool /*limit_width*/) const
			{
				...
				wchar_t shortkey;
				return graph.text_extent_size(API::transform_shortkey_text(trigger_->wdg_->caption(), shortkey, NULL));
			}
		...
		void trigger::refresh(graph_reference graph)
		{
			...
			if (false == cite_.draw(graph, attr_.bgcolor, attr_.fgcolor, rectangle{ graph.size() }, e_state))
			{
		...
		{
			wchar_t shortkey;
			string::size_type shortkey_pos;
			string mbstr = API::transform_shortkey_text(wdg_->caption(), shortkey, &shortkey_pos);
			wstring str = to_wstring(mbstr);

			size ts = graph.text_extent_size(str);
			size gsize = graph.size();
			...
			size_t txtlen = str.size();
			const auto txtptr = str;
			if(ts.width)
			{
				...
				if(enabled)
				{
					...
					auto text_color = (attr_.focus_color && attr_.focused ? color(colors::blue) : attr_.fgcolor);
		...
		void trigger::emit_click()
		{
			...
			arg.mouse_args = NULL;
			API::emit_event(event_code_click, arg.window_handle, arg);
		}

		void trigger::icon(const image& img)
		{
			if(img.empty())	return;

			if(NULL == attr_.icon)
				attr_.icon = new image;
			*attr_.icon = img;
		}
		//end class trigger
		...
		//@brief: Defaine a button widget and it provides the interfaces to be operational
			...
			button& button::icon(const image& img)
			{
			...
			button& button::set_bground(const char* name)
			{
				internal_scope_guard lock;
			...
			void button::_m_caption(native_string_type&& text)
			{
				...
				wchar_t shortkey;
				API::transform_shortkey_text(to_utf8(text), shortkey, NULL);
				if (shortkey)
					API::register_shortkey(handle(), shortkey);

				base_type::_m_caption(move(text));
			}
		//end class button
}//end namespace nana
