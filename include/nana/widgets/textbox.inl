...
namespace nana
{
	arg_textbox::arg_textbox(textbox& wdg, const vector<upoint>& text_pos)
...
namespace drawerbase {
	...
	{
		//class event_agent
			event_agent::event_agent(textbox& wdg, const vector<upoint>& text_pos)
				:widget_(wdg), text_position_(text_pos)
			{}

			void event_agent::first_change()
			{
				widget_.events().first_change.emit(arg_textbox{ widget_, text_position_ }, widget_);
			}

			void event_agent::text_changed()
			{
				widget_.events().text_changed.emit(arg_textbox{ widget_, text_position_ }, widget_);
			}

			void event_agent::text_exposed(const vector<upoint>& text_pos)
			{
				arg_textbox arg(widget_, text_pos);
	...
	//class draweer
		...
		void drawer::attached(widget_reference wdg, graph_reference graph)
		{
			...
			editor_ = new text_editor(wd, graph, dynamic_cast<widgets::skeletons::text_editor_scheme*>(scheme));

			evt_agent_.reset(new event_agent(static_cast<textbox&>(wdg), editor_->text_position()));
	...
	//class textbox
		...
		void textbox::store(string file)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
				editor->textbase().store(move(file), false, unicode::utf8);	//3rd parameter is just for syntax, it will be ignored
		}

		void textbox::store(string file, unicode encoding)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
				editor->textbase().store(move(file), true, encoding);
		}
		...
		textbox& textbox::indention(bool enb, function<string()> generator)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		textbox& textbox::reset(const char* str, bool end_caret)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		bool textbox::getline(size_t line_index, char* text) const
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		bool textbox::getline(size_t line_index,size_t start_point,char* text) const
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if(editor)
		...
		textbox& textbox::append(const char* text, bool at_caret)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if(editor)
        ...
        textbox& textbox::append(const wchar* text, bool at_caret)
        {
            internal_scope_guard lock;
            auto editor = get_drawer_trigger().editor();
            if(editor)
		...
		void textbox::set_accept(function<bool(wchar_t)> fn)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if(editor)
				editor->set_accept(move(fn));
		}

		textbox& textbox::tip_string(string str)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if(editor && editor->tip_string(move(str)))
				API::refresh_window(handle());
			return *this;
		}
		...
		bool textbox::get_selected_points(upoint &a, upoint &b) const
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
		...
		pair<upoint, upoint> textbox::selection() const
		{
			pair<upoint, upoint> points;
		...
		void textbox::set_highlight(const char* name, const color& fgcolor, const color& bgcolor)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		void textbox::erase_highlight(const char* name)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		void textbox::set_keywords(const char* name, bool case_sensitive, bool whole_word_match, initializer_list<wstring> kw_list)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		void textbox::set_keywords(const char* name, bool case_sensitive, bool whole_word_match, initializer_list<string> kw_list_utf8)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
			{
				for (auto & kw : kw_list_utf8)
					editor->set_keyword(charset(kw, unicode::utf8), name, case_sensitive, whole_word_match);
				API::refresh_window(handle());
			}
		}

		void textbox::erase_keyword(const char* kw)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		textbox& textbox::text_align(align alignment)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		vector<upoint> textbox::text_position() const
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		void textbox::set_undo_queue_length(size_t len)
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		size_t textbox::display_line_count() const noexcept
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		size_t textbox::text_line_count() const noexcept
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		auto textbox::_m_caption() const -> native_string_type
		{
			internal_scope_guard lock;
			auto editor = get_drawer_trigger().editor();
			if (editor)
		...
		void textbox::_m_typeface(const font& font)
		{
