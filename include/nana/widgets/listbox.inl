...
namespace nana
{
	static void check_range(size_t pos, size_t size)
	{
		if (!(pos < size))
			throw out_of_range("listbox: invalid element position");
	}
	...
	namespace drawerbase
	{
		namespace listbox
		{
			...
			{
				model_lock_guard(const model_lock_guard&) {}
				model_lock_guard& operator=(const model_lock_guard&) {}
			public:
			...
			//struct cell
				cell::format::format(const color& bgcolor, const color& fgcolor) noexcept
					: bgcolor{ bgcolor }, fgcolor{ fgcolor }
				{}

				cell::cell(const cell& rhs)
					:	text(rhs.text),
						custom_format{ rhs.custom_format ? new format(*rhs.custom_format) : NULL }
				{}

				//A workaround that VS2013 does not support to define an explicit default move constructor
				cell::cell(cell&& other) noexcept
					:	text(move(other.text)),
						custom_format{ move(other.custom_format) }
				{
				}

				cell::cell(string text) noexcept
					: text(move(text))
				{}

				cell::cell(string text, const format& fmt)
					:	text(move(text)),
						custom_format(make_unique<format>( fmt ))
				{}
				...
				cell& cell::operator=(const cell& rhs)
				{
					...
					{
						text = rhs.text;
						custom_format.reset(rhs.custom_format ? new format{*rhs.custom_format} : NULL);
					}
					return *this;
				}
			...
			public:
					...
					: public column_interface
				{
					...
					unsigned width_px;
					pair<unsigned, unsigned> range_width_px;
					...
					function<bool(const char*, any*, const char*, any*, bool reverse)> weak_ordering;
					...
					column(column&& other):
						caption(move(other.caption)),
						width_px(other.width_px),
						range_width_px(other.range_width_px),
						visible_state(other.visible_state),
						index(other.index),
						alignment(other.alignment),
						weak_ordering(move(other.weak_ordering)),
						ess_(other.ess_)
					{
					}
				...
				public:
					...
					// Sets the width ands the ranged width
					void width(unsigned pixels) noexcept
					{
						width_px = pixels;
					...
					void width(unsigned minimum, unsigned maximum)
					{
						...
						if ((minimum >= maximum) && (minimum != 0))
							throw invalid_argument("listbox.column.width() minimum must be less than maximum");

						range_width_px.first = minimum;
						range_width_px.second = maximum;

						unsigned px = clamp(static_cast<int>(width_px), static_cast<int>(minimum), static_cast<int>(maximum));
						if (width_px != px)
						{
							width_px = px;
							_m_refresh();
					...
					size_type position(bool disp_order) const noexcept;	//The definition is provided after essence
					...
					void text(string text_utf8)
					{
						caption = to_nstring(move(text_utf8));
						_m_refresh();
					}

					void text_align(align align) noexcept
					{
						if (alignment != align)
						{
							alignment = align;
					...
					void fit_content(unsigned maximize = 100000) noexcept;
					...
					void visible(bool is_visible) noexcept
					{
			...
			public:
				...
				string to_string(const export_options& exp_opt) const
				{
					string head_str;
					bool first{true};
				...
				{
					cont_.emplace_back(ess, move(text), pixels, static_cast<size_type>(cont_.size()));
				...
				size_type cast(size_type pos, bool disp_order) const
				{
					...
					throw invalid_argument("listbox: invalid header index");
				}
				...
				pair<int, unsigned> range(size_type pos) const
				{
			...
			struct item_data
			{
				using container = vector<cell>;

				unique_ptr<container> cells;
				color bgcolor;
				color fgcolor;
				image img;
				size img_show_size;
				...
				item_data(const item_data& r)
					:	cells(r.cells ? make_unique<container>(*r.cells) : NULL),
						bgcolor(r.bgcolor),
						fgcolor(r.fgcolor),
						img(r.img),
						flags(r.flags),
						anyobj(r.anyobj ? new any(*r.anyobj) : NULL)
				{}

				item_data(container&& cont)
					: cells(make_unique<container>(move(cont)))
				{
					flags.selected = flags.checked = false;
				}

				item_data(char*& s)
					: cells(make_unique<container>())
				{
					flags.selected = flags.checked = false;
					cells->emplace_back(move(s));
				}
				...
				item_data& operator=(const item_data& r)
				{
					...
					{
						if (r.cells)
							cells = make_unique<container>(*r.cells);

						flags = r.flags;
						anyobj.reset(r.anyobj ? new any(*r.anyobj) : NULL);
						bgcolor = r.bgcolor;
						fgcolor = r.fgcolor;
						img = r.img;
					}
				...
				string to_string(const export_options& exp_opt, const vector<cell>* model_cells) const
				{
					string item_str;
			...
			struct category_t
			{
				using container = deque<item_data>;

				native_string_type text;
				vector<size_t> sorted;
				container items;

				unique_ptr<model_interface> model_ptr;

				bool expand{ true };
				bool display_number{ true };

				//A cat may have a key object to identify the category
				shared_ptr<key_interface> key_ptr;

				deque<pat::cloneable<pat::abstract_factory<inline_notifier_interface>>> factories;
				deque<unique_ptr<inline_indicator>> indicators;
				...
				vector<cell> cells(size_type pos) const
				{
					if (model_ptr)
			...
			struct inline_pane
			{
				...
				unique_ptr<inline_notifier_interface> inline_ptr;
			...
			{
			public:
				using container = list<category_t>;
				using item_type = item_data;

				function<function<bool(const char*, any*,
								const char*, any*, bool reverse)>(size_t) > fetch_ordering_comparer;

				struct sort_attributes
				{
					size_t	column;		///< The position of the column to be sorted
					bool		resort;
					bool		reverse;	
				};
                ...
                void sort()
				{
					...
					if(weak_ordering_comp)
					{
						...
						{
							const bool use_model = (cat.model_ptr != NULL);

							stable_sort(cat.sorted.begin(), cat.sorted.end(), [&cat, &weak_ordering_comp, use_model, this](size_t x, size_t y){
					...
					{	//No user-defined comparer is provided, and default comparer is applying.
						for (auto & cat : categories_)
						{
							...
							stable_sort(cat.sorted.begin(), cat.sorted.end(), [this, &cat, use_model](size_t x, size_t y){
								//The predicate must be a strict weak ordering.
								//!comp(x, y) != comp(x, y)
								if (use_model)
								{
				 ...
				 * @param reverse A pointer to a boolean which indicates whether to reverse sort. If this parameter is NULL, the sort is negated to the current reverse state.
				 * @return true if the column is sorted, false otherwise.
				 */
				bool sort_column(size_t pos, const bool * reverse)
				{
					if (npos == pos)
					{
						sort_attrs_.column = npos;
				...
				{
					categories_.emplace_back(move(text));
				...
				category_t* create_cat(shared_ptr<key_interface>& ptr)
				{
				...
				category_t* create_cat(size_t pos, native_string_type&& text)
				{
					return &(*categories_.emplace(this->get(pos), move(text)));
				}

				/// Insert  before item in absolute "pos" a new item with "text" in column 0, and place it in last display position of this cat
				void insert(const index_pair& pos, char*& text, const size_t columns)
				{
					...
					catobj.items.emplace(catobj.items.begin() + (pos.item < item_count ? pos.item : item_count), move(text));
				}
				...
				index_pair index_cast(const index_pair& from, bool from_display_order) const
				{
					...
					throw out_of_range("listbox: invalid item position");
				}
				...
				static void throw_if_immutable_model(model_interface* model)
				{
					if (model && model->container()->immutable())
					{
						...
						throw runtime_error("listbox disallow to insert/remove items because of immutable model");
					}
				}
				...
				void assign_model(const index_pair& pos, const vector<cell>& cells)
				{
					if (pos.cat < categories_.size())
					{
						auto i = categories_.cbegin();
				...
				{
					return (get(pos.cat)->model_ptr != NULL);
				}
				...
				vector<cell> at_model_abs(const index_pair& pos) const
				{
				...
				vector<cell> at_model(const index_pair& pos) const
				{
					auto model_ptr = get(pos.cat)->model_ptr.get();
					if (!model_ptr)
						return{};
				...
				index_pair advance(const index_pair& pos, int n) const
				{
					...
					{
						//Forward
						size_t index = (npos == pos.item ? 0 : pos.item + 1);
				...
				void text(category_t* cat, size_type pos, size_type abs_col, char*& str, size_type columns)
				{
					if ((abs_col < columns) && (pos < cat->items.size()))
					{
						vector<cell> model_cells;
				...
				vector<pair<index_pair, bool>> select_display_range_if(const index_pair& fr_abs, index_pair to_dpl, bool deselect_others, Pred pred)
				{
					...
					vector<pair<index_pair, bool>> pairs;
			...
			private:
				...
				vector<inline_pane*> active_panes_;
			};//end class es_lister
			...
			struct essence
			{
				...
				listbox* listbox_ptr{NULL};
				listbox::scheme_type* scheme_ptr{NULL};
				graphics *graph{NULL};
				bool auto_draw{true};
				...
				pair<parts, size_t> pointer_where;	//The 'first' stands for which object, such as header and lister, 'second' stands for item
				...
				unique_ptr<widgets::skeletons::content_view> content_view;

				function<void(graphics&, const rectangle&, bool)> ctg_icon_renderer;	///< Renderer for the category icon
				...
				map<pat::abstract_factory_base*, deque<unique_ptr<inline_pane>>> inline_table, inline_buffered_table;

				essence()
				{
					pointer_where.first = parts::unknown;

					lister.fetch_ordering_comparer = [this](size_t pos) -> function<bool(const char*, any*, const char*, any*, bool reverse)>
					{
				...
				{
					auto px = (max)(scheme_ptr->item_height_ex + text_height, unsigned(1));
				...
				struct pred_mouse_selection
				{
					...
					bool operator()(const pair<index_pair, bool>& m) const noexcept
					{
						return (pos_ == m.first);
					}
				};

				pair<int, int> columns_range() const
				{
					rectangle r;
					if (!rect_header(r))
						return{};
				...
				void update_mouse_selection(const point& screen_pos)
				{
					...
					if (logic_pos.y > mouse_selection.begin_position.y)
					{
						...
						logic_pos.y = (max)(logic_pos.y, coordinate_cast(imd_area.position(), true).y);
					}
					...
					if ((max)(mouse_selection.end_position.x, mouse_selection.begin_position.x) >= content_x &&
						(min)(mouse_selection.end_position.x, mouse_selection.begin_position.x) < content_x + static_cast<int>(header.pixels()))
					{
						auto const begin_off = (max)((min)(mouse_selection.begin_position.y, mouse_selection.end_position.y), 0) / item_height();

						auto begin = lister.advance(lister.first(), begin_off);
						if (!begin.empty())
						{
							vector<pair<index_pair, bool>> selections;
							...
							if ((mouse_selection.end_position.y < 0) || (lister.distance(lister.first(), begin) == begin_off))
							{
								...
								auto last_off = (max)(mouse_selection.begin_position.y, mouse_selection.end_position.y) / item_height();
								...
								selections = lister.select_display_range_if(begin, last, false, [this](const index_pair& abs_pos) {
									...
									{
										if (mouse_selection.already_selected.cend() != find(mouse_selection.already_selected.cbegin(), mouse_selection.already_selected.cend(), abs_pos))
										{
								...
								for (auto & pair : selections)
								{
									...
									if (mouse_selection.selections.cend() ==
										find(mouse_selection.selections.cbegin(), mouse_selection.selections.cend(), pair.first))
									{
...
#endif
								{
									if (selections.cend() == find_if(selections.cbegin(), selections.cend(), pred_mouse_selection{ *i }))
									{
				...
				size calc_content_size(bool try_update = true)
				{
					size ctt_size(
				...
				rectangle checkarea(int x, int y) const noexcept /// move to scheme ?? 16 ?
				{
					return rectangle(x + 4, y + (static_cast<int>(item_height()) - 16) / 2, 16, 16);
				}
				...
				pair<parts, size_t> where(const point& pos) const noexcept
				{
					pair<parts, size_t> new_where{ parts::unknown, npos };
				...
				void widget_to_header(point& pos) noexcept
				{
					--pos.y;
				...
				vector<size_type> header_seq(unsigned lister_w) const
				{
					vector<size_type> seqs;
				...
				{
					unique_ptr<inline_pane> pane_ptr;
					...
					inline_table[factory].emplace_back(move(pane_ptr));
					return ptr;
				}
			};
			...
			{
				cells_.emplace_back(string(n ? "true" : "false"));
				return *this;
			}
			oresolver& oresolver::operator<<(short n)
			{
				cells_.emplace_back(to_string(n));
				return *this;
			}
			...
			{
				cells_.emplace_back(to_string(f));
				return *this;
			}
			...
			{
				cells_.emplace_back(string(text));
				return *this;
			}
			...
			oresolver& oresolver::operator<<(const wchar* text)
			{
				cells_.emplace_back(to_utf8(text));
				return *this;
			}

			oresolver& oresolver::operator<<(wchar*& text)
			{
				cells_.emplace_back(to_utf8(text));
				return *this;
			}
			...
			{
				cells_.emplace_back(move(cl));
				return *this;
			}

			oresolver& oresolver::operator<<(nullptr_t)
			{
				cells_.emplace_back();
			...
			vector<cell>&& oresolver::move_cells() noexcept
			{
				return move(cells_);
			}

			listbox& oresolver::listbox() noexcept
			{
			...
			{
				if (pos_ < cells_.size())
					n = static_cast<unsigned short>(stoul(cells_[pos_++].text));
				return *this;
			}
			...
			{
				if (pos_ < cells_.size())
					n = stoll(cells_[pos_++].text);
				return *this;
			}
			iresolver& iresolver::operator>>(unsigned long long& n)
			{
				if (pos_ < cells_.size())
					n = stoull(cells_[pos_++].text);
				return *this;
			}
			...
			{
				if (pos_ < cells_.size())
					f = stold(cells_[pos_++].text);
				return *this;
			}

			iresolver& iresolver::operator>>(char* text)
			{
				if (pos_ < cells_.size())
			...
			iresolver& iresolver::operator>>(wchar* text)
			{
				if (pos_ < cells_.size())
			...
			iresolver::iresolver(vector<cell> cl) noexcept
				: cells_(move(cl))
			{}
			...
			iresolver& iresolver::operator>>(nullptr_t) noexcept
			{
				++pos_;
				return *this;
			}
			...
			class inline_indicator
				: public inline_widget_indicator<index_pair, string>
			{
			public:
				using parts = essence::parts;

				inline_indicator(essence* ess, size_t column_pos)
				...
				void attach(index_type pos, inline_pane* pane)
				{
					...
					panes_.emplace_back(make_pair(pos, pane));
				}
			...
			public:
				...
				void modify(index_type pos, const value_type& value) const
				{
			...
			private:
				essence * const ess_;
				const size_t column_pos_;
				vector<pair<index_type, inline_pane*>> panes_;
			};
			...
			void es_lister::scroll(const index_pair& abs_pos, bool to_bottom)
			{
				...
				if ((abs_pos.item != npos) && (abs_pos.item >= cat.items.size()))
					throw invalid_argument("listbox: invalid pos to scroll");
			...
			void es_lister::erase(const index_pair& pos)
			{
				...
				if (pos.item < cat.items.size())
				{
					...
					cat.sorted.erase(find(cat.sorted.begin(), cat.sorted.end(), cat.items.size()));
			...
			string es_lister::to_string(const export_options& exp_opt) const
			{
				string list_str;
				bool first{true};
				for(auto & cat: cat_container())
				{
					...
					auto const pcell = (cat.model_ptr ? &model_cells : NULL);
			...
			public:
				...
				bool detect_splitter(const rectangle& r, int x) noexcept
				{
				...
				bool grab_move(const point& pos)
				{
					...
					{   // resize column, not move it
						...
						{
							//Column ranged width
							new_w = clamp(static_cast<int>(new_w), static_cast<int>(col.range_width_px.first), static_cast<int>(col.range_width_px.second));
						}
						else
						{
							//Default scheme
							new_w = (max)(new_w, essence_->scheme_ptr->suspension_width + essence_->scheme_ptr->min_column_width);
						}
			...
			private:
				size_type _m_target_strip(int x, const rectangle& rect, size_type grab, bool& place_front)
				{
					...
					x = clamp(x, x_offset, x_offset + static_cast<int>(rect.width));
				...
				void _m_draw_header_item(graph_reference graph, unsigned margin, const rectangle& column_r, const color& fgcolor, const es_header::column& column, item_state state)
				{
					color bgcolor;
					...
					if(API::dev::copy_transparent_background(essence_->listbox_ptr->handle(), column_r, graph, column_r.position()))
					{
						graphics grad_graph{column_r.dimension()};
					...
					aligner text_aligner{ graph, column.alignment, column.alignment };
					...
					if (column.index == sort.column)
					{
						...
						arrow.direction(sort.reverse ? direction::south : direction::north);
				...
				void _m_make_float(const rectangle& rect, const point& pos)
				{
					...
					graphics fl_graph({ col.width_px + margin, essence_->scheme_ptr->header_height });
			...
			public:
				...
				void draw(const rectangle& rect)
				{
					...
					//clear active panes
					essence_->lister.append_active_panes(NULL);
					...
					if (!first_disp.empty())
					{
						...
						if (idx.cat == 0 || !idx.is_category())
						{
							...
							size_t size = i_categ->items.size();
							for (size_t offs = first_disp.item; offs < size; ++offs, ++idx.item)
							{
					...
					{
						point box_position{
							min(essence_->mouse_selection.begin_position.x, essence_->mouse_selection.end_position.x),
							min(essence_->mouse_selection.begin_position.y, essence_->mouse_selection.end_position.y)
						};
						size box_size{
								static_cast<size::value_type>(abs(essence_->mouse_selection.begin_position.x - essence_->mouse_selection.end_position.x)),
								static_cast<size::value_type>(abs(essence_->mouse_selection.begin_position.y - essence_->mouse_selection.end_position.y))
						};

						graphics box_graph{ box_size };
			...
			private:
				void _m_draw_categ(const category_t& categ, int x, int y, int txtoff, unsigned width, color bgcolor, item_state state)
				{
					...
					else
					{
						...
						arrow.direction(categ.expand ? direction::south : direction::east);
					...
					if (categ.display_number)
					{
						...
						native_string_type str = to_nstring('(' + to_string(categ.items.size()) + ')');
					...
					if (35 + text_px < width)
					{
						point pos{ x + 30 + static_cast<int>(text_px), y + static_cast<int>(item_height) / 2 };
				...
				void _m_draw_item(const category_t& cat,
					              ...
					              const vector<size_type>& seqs,    ///< columns to print
					              color bgcolor, 
					              color fgcolor, 
					              item_state state
					)
				{
					...
					const unsigned show_w = (min)(content_r.width, width - essence_->content_view->origin().x);
						...
						if (col.width_px > essence_->scheme_ptr->text_margin)
						{
							...
							if (0 == display_order)
							{
								...
								if (essence_->if_image)
								{
									...
									if (item.img)
									{
										rectangle imgt(item.img_show_size);
										img_r = imgt;
							...
							if ( content_pos + essence_->scheme_ptr->text_margin < col.width_px) // we have room
							{
								...
								if (inline_wdg)
								{
									...
									bool visible_state = true;
									if (overlap(content_r, { wdg_x, coord.y, wdg_w, essence_->item_height() }, pane_r))
									{
										point pane_pos;
									...
									size sz{ wdg_w, essence_->item_height() };
							...
							if (cells.size() > column_pos)        // process only if the cell is visible
							{
								...
								if (draw_column)
								{
									aligner text_aligner{*graph, col.alignment};
				...
				inline_pane * _m_get_inline_pane(const category_t& cat, size_t column_pos) const
				{
				...
				inline_pane* _m_find_inline_pane(const index_pair& pos, size_t column_pos) const
				{
				...
				void _m_draw_item_border(int item_top) const
				{
					...
					draw(*essence_->graph).corner(r, 1);
			...
			//class trigger: public drawer_trigger
				...
				void trigger::mouse_move(graph_reference graph, const arg_mouse& arg)
				{
					...
					if(essence_->pointer_where.first == parts::header)
					{
						...
						if(essence_->rect_header(r))
						{
							...
							{
								set_splitter = true;
								essence_->lister.wd_ptr()->cursor(cursor_size_we);
							}
						}
					}
					...
					{
						if((drawer_header_->splitter() != npos) || (essence_->lister.wd_ptr()->cursor() == cursor_size_we))
						{
							essence_->lister.wd_ptr()->cursor(cursor_arrow);
				...
				void trigger::mouse_down(graph_reference graph, const arg_mouse& arg)
				{
					...
					else if(ptr_where.first == parts::list || ptr_where.first == parts::checker)
					{
						...
						{
							auto * item_ptr = (item_pos.is_category() ? NULL : &lister.at(item_pos));
								...
								if (!lister.single_status(true))
								{
									...
									else
									{
										if (mouse::right_button == arg.button)
										{
											...
											if (selected.cend() == find(selected.cbegin(), selected.cend(), item_pos))
				...
				void trigger::mouse_up(graph_reference graph, const arg_mouse& arg)
				{
					...
					if (essence_->header.sortable() && essence_->pointer_where.first == parts::header && prev_state == item_state::pressed)
					{
						if(essence_->pointer_where.second < essence_->header.cont().size())
							need_refresh = essence_->lister.sort_column(essence_->pointer_where.second, NULL);
					}
				...
				void trigger::dbl_click(graph_reference graph, const arg_mouse&)
				{
					...
					{
						if (cursor_size_we == essence_->lister.wd_ptr()->cursor())
						{
					...
					case keyboard::copy:
						{
							...
							system::dataexch().set(essence_->to_string(exp_opt), API::root(essence_->listbox_ptr->handle()));
							return;
						}
			...
			//class item_proxy
				...
				size_t item_proxy::columns() const noexcept
				{
				...
				{
					ess_->lister.text(cat_, pos_.item, col, move(cl), columns());
					ess_->update();
					return *this;
				}

				item_proxy& item_proxy::text(size_type col, string str)
				{
					ess_->lister.text(cat_, pos_.item, col, move(str), columns());
					ess_->update();
					return *this;
				}

				item_proxy& item_proxy::text(size_type col, const wchar* str)
				{
				...
				string item_proxy::text(size_type col) const
				{
					return cat_->cells(pos_.item).at(col).text;
				}

				void item_proxy::icon(const image& img)
				{
					if (img)
					{
				...
				{
					return this->operator==(wstring(s));
				}
				...
				bool item_proxy::operator==(const wchar* s) const
				{
				...
				auto item_proxy::_m_cells() const -> vector<cell>
				{
			...
			//the member cat_ is used for fast accessing to the category
				...
				{
					if (!cat_->model_ptr)
						throw runtime_error("listbox has not a model for the category");

					return{ cat_->model_ptr.get() };
				}

				void cat_proxy::append(initializer_list<string> arg)
				{
				...
				void cat_proxy::append(initializer_list<wstring> arg)
				{
					const auto items = columns();
					push_back({});
					item_proxy ip{ ess_, index_pair(pos_, size() - 1) };
				...
				cat_proxy& cat_proxy::text(string s)
				{
				...
				cat_proxy& cat_proxy::text(wstring s)
				{
					auto text = to_nstring(s);
					internal_scope_guard lock;
					if (text != cat_->text)
				...
				string cat_proxy::text() const
				{
					internal_scope_guard lock;
					return to_utf8(cat_->text);
				}

				void cat_proxy::push_back(string s)
				{
					internal_scope_guard lock;
					...
					if (cat_->model_ptr)
					{
						...
						cat_->items.emplace_back(move(s));
				...
				{
					if (cat_->items.empty())
						throw runtime_error("listbox.back() no element in the container.");
				...
				void cat_proxy::inline_factory(size_type column, pat::cloneable<pat::abstract_factory<inline_notifier_interface>> factory)
				{
					...
					cat_->factories[column] = move(factory);
					cat_->indicators[column].reset(new inline_indicator(ess_, column));
				}

				void cat_proxy::_m_append(vector<cell> && cells)
				{
					//check invalid cells
					for (auto & cl : cells)
					{
					...
					{
						cells.resize(columns());
						cat_->items.emplace_back(move(cells));
					}
				...
				void cat_proxy::_m_try_append_model(const const_virtual_pointer& dptr)
				{
					...
					if (!cat_->model_ptr)
						throw runtime_error("listbox hasn't a model");

					ess_->lister.throw_if_immutable_model(cat_->model_ptr.get());

					if (!cat_->model_ptr->container()->push_back(dptr))
						throw invalid_argument("listbox, the type of operand object is mismatched with model container value_type");
	...
	//class listbox
		...
		void listbox::scroll(bool to_bottom, size_type cat_pos)
		{
			...
			{
				if (cat_pos >= cats)
					throw invalid_argument("listbox: invalid category");
			}
			else
				cat_pos = cats - 1;
		...
		listbox::size_type listbox::append_header(string s, unsigned width)
		{
			internal_scope_guard lock;
			auto & ess = _m_ess();
			auto pos = ess.header.create(&ess, to_nstring(move(s)), width);
			ess.update();
            return pos;
		}

		listbox::size_type listbox::append_header(wstring s, unsigned width)
		{
			internal_scope_guard lock;
			auto & ess = _m_ess();
			auto pos = ess.header.create(&ess, to_nstring(move(s)), width);
		...
		listbox::cat_proxy listbox::append(string s)
		{
			internal_scope_guard lock;
			auto & ess = _m_ess();
			auto new_cat_ptr = ess.lister.create_cat(to_nstring(move(s)));
			ess.update();

			return cat_proxy{ &ess, new_cat_ptr };
		}

		listbox::cat_proxy listbox::append(wstring s)
		{
			internal_scope_guard lock;
			auto & ess = _m_ess();
			auto new_cat_ptr = ess.lister.create_cat(to_nstring(move(s)));
		...
		void listbox::append(initializer_list<string> categories)
		{
			internal_scope_guard lock;
			auto & ess = _m_ess();
		...
		void listbox::append(initializer_list<wstring> categories)
		{
			internal_scope_guard lock;
			auto & ess = _m_ess();
		...
		rectangle listbox::content_area() const
		{
			...
			carea.width -= (min)(carea.width, ess.header.margin());
			return carea;
		}

		auto listbox::insert(cat_proxy cat, string str) -> cat_proxy
		{
			internal_scope_guard lock;
			auto & ess = _m_ess();
			auto new_cat_ptr = ess.lister.create_cat(cat.position(), to_nstring(move(str)));
			return cat_proxy{ &ess, new_cat_ptr };
		}

		auto listbox::insert(cat_proxy cat, wstring str) -> cat_proxy
		{
			internal_scope_guard lock;
			auto & ess = _m_ess();
			auto new_cat_ptr = ess.lister.create_cat(cat.position(), to_nstring(move(str)));
		...
		void listbox::insert_item(const index_pair& pos, string text)
		{
			internal_scope_guard lock;
			auto & ess = _m_ess();
			ess.lister.insert(pos, move(text), this->column_size());
		...
		void listbox::insert_item(const index_pair& pos, const wchar* text)
		{
		...
		{
			sort(indexes.begin(), indexes.end(), [](const index_pair& pos1, const index_pair& pos2)
			{
				return (pos1 > pos2);
			});
			...
			size_t this_cat = this->size_categ();
			for (auto & pos : indexes)
			{
				if (this_cat != pos.cat)
				{
		...
		void listbox::set_sort_compare(size_type col, function<bool(const char*, any*, const char*, any*, bool reverse)> strick_ordering)
		{
			_m_ess().header.at(col).weak_ordering = move(strick_ordering);
		}
		...
		listbox& listbox::category_icon(function<void(graphics& graph, const rectangle& rt_icon, bool expanded)> icon_renderer)
		{
		...
		listbox& listbox::category_icon(const image& img_expanded, const image&& img_collapsed)
		{
			_m_ess().ctg_icon_renderer = [img_expanded, img_collapsed](graphics& graph, const rectangle& rt_icon, bool expanded)
			{
				if (expanded)
				{
		...
		drawerbase::listbox::category_t* listbox::_m_assoc(shared_ptr<key_interface> ptr, bool create_if_not_exists)
		{
			auto & ess = _m_ess();
		...
		void listbox::_m_erase_key(key_interface* p) noexcept
		{
