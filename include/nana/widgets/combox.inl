...
namespace nana
{
	...
	namespace drawerbase
	{
		namespace combox
		{
			...
			{
			public:
				event_agent(combox& wdg)
					: widget_(wdg)
				{}

				void first_change(){}	//empty, because combox does not have this event.

				void text_changed()
				{
					widget_.events().text_changed.emit(arg_combox{ widget_ }, widget_);
				}
			private:
				combox & widget_;
			};
			...
			{
			public:
				shared_ptr<key_interface> key;

				image	item_image;
				string		item_text;
				mutable shared_ptr<any>	any_ptr;

				item(shared_ptr<key_interface> && kv)
					: key(move(kv))
				{
				}
			...
			class drawer_impl
			{
				...
				public:
					...
					optional<size> measure(graph_reference graph, unsigned limit_pixels, bool /*limit_width*/) const
					{
						...
						size content_size;
						for (size_t i = 0; i < drw_->the_number_of_options(); ++i)
						{
							auto & m = drw_->at(i);
							auto sz = graph.text_extent_size(m.item_text);

							content_size.width = (max)(content_size.width, sz.width);
							content_size.height = (max)(content_size.height, sz.height);
						}
			...
			public:
				...
				{
					widget_ = static_cast< combox*>(&wd);

					auto scheme = dynamic_cast< widgets::skeletons::text_editor_scheme*>(API::dev::get_scheme(wd));
					...
					evt_agent_.reset(new event_agent{ static_cast<combox&>(wd) });
				...
				{
					items_.emplace_back(make_shared<item>(move(text)));
					API::refresh_window(widget_->handle());
				}

				any * anyobj(size_t pos, bool allocate_if_empty) const
				{
					if(pos >= items_.size())
						return NULL;

					auto & any_ptr = items_[pos]->any_ptr;
					if (allocate_if_empty && (NULL == any_ptr))
						any_ptr = make_shared<any>();
					return any_ptr.get();
				}
				...
				void editable(bool enb)
				{
					if(editor_)
					{
						...
						if (!enb)
						{
							editor_->customized_renderers().background = [this](graph_reference graph, const rectangle&, const color&)
							{
								...
								{
									pare_off_px = 2;
									swap(clr_from, clr_to);
								}

								graph.gradual_rectangle(rectangle(graph.size()).pare_off(pare_off_px), clr_from, clr_to, true);
								if (API::is_transparent_background(this->widget_ptr()->handle()))
								{
									graphics trns_graph{ graph.size() };
							...
							editor_->customized_renderers().background = NULL;
				...
				{
					return (state_.lister != NULL);
				}

				void open_lister_if_push_button_positioned()
				{
					if((NULL == state_.lister) && !items_.empty() && (!editor_->attr().editable || (parts::push_button == state_.pointer_where)))
					{
						module_.items.clear();
						copy(items_.cbegin(), items_.cend(), back_inserter(module_.items));
						state_.lister = &form_loader<float_listbox, false>()(widget_->handle(), rectangle(0, widget_->size().height, widget_->size().width, 10), true);
						...
						{
							state_.lister = NULL;	//The lister closes by itself.
				...
				void option(size_t index, bool ignore_condition)
				{
					if(items_.size() <= index)
						return;

					size_t old_index = module_.index;
					module_.index = index;
					...
					if(ignore_condition || (old_index != index) || (items_[index]->item_text != widget_->caption()))
					{
						...
						widget_->events().selected.emit(arg_combox(*widget_), widget_->handle());
					}
				}

				size_t at_key(shared_ptr<key_interface>&& p)
				{
					size_t pos = 0;
					for (auto & m : items_)
					{
						if (m->key && pred_equal(m->key.get(), p.get()))
							return pos;
						++pos;
					}

					pos = items_.size();
					items_.emplace_back(make_shared<item>(move(p)));
				...
				void erase(key_interface * kv)
				{
					size_t pos = 0;
					for (auto & m : items_)
					{
						if (m->key && pred_equal(m->key.get(), kv))
						{
							erase(pos);
							return;
						}
				...
				const item& at(size_t pos) const
				{
					return *items_.at(pos);
				}
				...
				void erase(size_t pos)
				{
					...
					else if ((npos != module_.index) && (pos < module_.index))
						--module_.index;
				...
				void image(size_t pos, const image& img)
				{
					if (pos < items_.size())
					{
			...
			private:
				void _m_text_area(const size& s)
				{
					auto extension = measurer_->extension();

					rectangle r(2, 2, s.width > extension.width ? s.width - extension.width : 0, s.height > extension.height ? s.height - extension.height : 0);
					if (image_enabled_)
					{
				...
				void _m_draw_push_button(bool enabled)
				{
					...
					button.draw(*graph_, color{ 3, 65, 140 }, colors::white, r, estate);
				...
				void _m_draw_image()
				{
					...
					point pos((image_pixels_ - imgsz.width) / 2 + 2, (vpix - imgsz.height) / 2 + 2);
					img.stretch(rectangle{ img.size() }, *graph_, rectangle(pos, imgsz));
				}
			private:
				vector<shared_ptr<item>> items_;
				float_listbox::module_type module_;
				combox * widget_{ NULL };
				graphics * graph_{ NULL };
				drawerbase::float_listbox::item_renderer* item_renderer_{ NULL };

				bool image_enabled_{ false };
				unsigned image_pixels_{ 16 };
				widgets::skeletons::text_editor * editor_{ NULL };
				unique_ptr<event_agent> evt_agent_;

				unique_ptr<content_measurer> measurer_;
				struct state_type
				{
					bool	focused;
					...
					float_listbox * lister;
					size_t	item_index_before_selection;
				}state_;
			...
			//class item_proxy
				item_proxy::item_proxy(drawer_impl* impl, size_t pos)
					:	impl_(impl),
						pos_(pos)
				{}

				item_proxy& item_proxy::text(const char* s)
				{
					throw_not_utf8(s);
				...
				item_proxy& item_proxy::icon(const image& img)
				{
				...
				any * item_proxy::_m_anyobj(bool alloc_if_empty) const
				{
	...
	//class combox
		...
		combox::combox(window wd, string text, bool visible)
		{
			throw_not_utf8(text);
			create(wd, rectangle(), visible);
			caption(move(text));
		...
		void combox::set_accept(function<bool(wchar_t)> pred)
		{
			internal_scope_guard lock;
			auto editor = _m_impl().editor();
			if(editor)
				editor->set_accept(move(pred));
		}

		combox& combox::push_back(string text)
		{
			internal_scope_guard lock;
			_m_impl().insert(move(text));
			return *this;
		}

		size_t combox::the_number_of_options() const
		{
			internal_scope_guard lock;
			return _m_impl().the_number_of_options();
		}

		size_t combox::option() const
		{
			internal_scope_guard lock;
			return _m_impl().option();
		}

		void combox::option(size_t pos)
		{
			internal_scope_guard lock;
		...
		::string combox::text(size_t pos) const
		{
			internal_scope_guard lock;
			return _m_impl().at(pos).item_text;
		}

		void combox::erase(size_t pos)
		{
			internal_scope_guard lock;
			_m_impl().erase(pos);
		}
		...
		void combox::image(size_t i, const image& img)
		{
			internal_scope_guard lock;
			if(empty()) return;
		...
		image combox::image(size_t pos) const
		{
			internal_scope_guard lock;
		...
		auto combox::_m_caption() const -> native_string_type
		{
			internal_scope_guard lock;
			auto editor = _m_impl().editor();
			if (editor)
		...
		any * combox::_m_anyobj(size_t pos, bool alloc_if_empty) const
		{
			internal_scope_guard lock;
			return _m_impl().anyobj(pos, alloc_if_empty);
		}

		auto combox::_m_at_key(shared_ptr<key_interface>&& p) -> item_proxy
		{
			internal_scope_guard lock;
			auto & impl = _m_impl();
			return item_proxy(&impl, impl.at_key(move(p)));
		}

		void combox::_m_erase(key_interface* p)
		{
			internal_scope_guard lock;
			_m_impl().erase(p);
		}
