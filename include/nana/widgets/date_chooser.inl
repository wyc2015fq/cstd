...
namespace nana
{
	namespace drawerbase
	{
		namespace date_chooser
		{
			...
			void perf_transform_helper(window window_handle, transform_action tfid, trigger::graph_reference graph, trigger::graph_reference dirtybuf, trigger::graph_reference newbuf, const point& refpos)
			{
				...
				{
					rectangle dr(0, refpos.y, 0, dirtybuf.height());
					rectangle nr(refpos.x, refpos.y, 0, newbuf.height());
					for (int i = 1; i < count; ++i)
					{
						...
						nr.width = off_x;
						graph.bitblt(nr, newbuf, point(static_cast<int>(dr.width), 0));
				...
				{
					double delta = dirtybuf.width() / double(count);
					rectangle dr(refpos.x, refpos.y, 0, dirtybuf.height());
					rectangle nr(0, refpos.y, 0, newbuf.height());
					...
					for (int i = 1; i < count; ++i)
					{
						...
						graph.bitblt(dr, dirtybuf, point(off_x, 0));
				...
				{
					graphics dzbuf(newbuf.size());
					graphics nzbuf(newbuf.size());
					...
					for (int i = 1; i < count; ++i)
					{
						...
						dzbuf.blend(rectangle{ nzbuf.size() }, nzbuf, {}, 1 - fade * (count - i));
				...
				else if (tfid == transform_action::to_enter)
				{
					...
					for (int i = 1; i < count; ++i)
					{
						...
						dzbuf.blend(rectangle{ nzbuf.size() }, nzbuf, {}, 1.0 - fade * (count - i));
				...
				graph.bitblt(rectangle(refpos, newbuf.size()), newbuf);
			}
			...
			public:
					...
					case keyboard::enter:
						...
						else
						{
							if (!trace_.empty_logic_pos())
							{
								...
								if (0 < value && value <= static_cast<int>(date::month_days(view_month_.year, view_month_.month)))
				...
				void weekname(size_t pos, char*& name)
				{
					if (pos < 7)
						weekstr_[pos] = move(name);
				}

				where pos_where(const size& area, const point& pos)
				{
				...
				int get_trace_logic_pos() const
				{
					...
					if (page_mode::date == page)
					{
						...
						int dw = date::day_of_week(view_month_.year, view_month_.month, 1);
						n -= (dw ? dw + 7 : 14);
					}
					return n;
				}
				...
				{
					return date::month_days(view_month_.year, view_month_.month);
				}
				...
				void choose(window window_handle, int day)
				{
					...
					arg_datechooser evt_arg{ static_cast<date_chooser*>(API::get_widget(window_handle)) };
					API::events<date_chooser>(window_handle).date_changed.emit(evt_arg, window_handle);
				}
				...
				void render(graph_reference graph)
				{
					...
					{
						const point refpos(1, static_cast<int>(topbar_height)+1);

						_m_calc_basis(graph, refpos);

						graphics gbuf({ width, graph.height() - 2 - topbar_height });
				...
				{
					point refpos(1, static_cast<int>(topbar_height)+1);
					rectangle r(0, 0, graph.width() - 2, graph.height() - 2 - topbar_height);

					graphics dirtybuf({ r.width, r.height });
					dirtybuf.bitblt(r, graph, refpos);

					render(graph);

					graphics gbuf({ r.width, r.height });
			...
			private:
				...
				void _m_calc_basis(graph_reference graph, const point& refpos)
				{
				...
				{
					color arrow_bgcolor;
					rectangle arrow_r{ static_cast<int>(border_size), (topbar_height - 16) / 2 + 1, 16, 16 };
					...
					if (graph.width() > 32 + border_size * 2)
					{
						...
						if (page_mode::date == page)
						{
							str = internationalization()(monthstr[this->view_month_.month - 1]);
							str += "  ";
						}
						str += to_string(view_month_.year);

						size txt_s = graph.text_extent_size(str);
				...
				void _m_draw_pos(graph_reference graph, const upoint& logic_pos, const char* text_utf8, bool primary, bool sel)
				{
					rectangle r(static_cast<int>(logic_pos.x * dwbasis_.row_s), static_cast<int>(logic_pos.y * dwbasis_.line_s),
				...
				{
					int x = date::day_of_week(view_month_.year, view_month_.month, 1);
					...
					if (before)
					{
						...
						auto days = date::month_days(year, month);
						...
						{
							this->_m_draw_pos(graph, logic_pos, to_string(beg + logic_pos.x), false, same && (static_cast<unsigned>(date_.day) == beg + logic_pos.x));
						}
					}
					else
					{
						...
						for (upoint logic_pos{begin_logic_pos}; logic_pos.y < 7; ++logic_pos.y)
						{
							...
							{
								_m_draw_pos(graph, logic_pos, to_string(day), false, same && (date_.day == day));
								++day;
							}
							logic_pos.x = 0;
						}
				...
				void _m_draw_days(graph_reference graph)
				{
					...
					int days = static_cast<int>(date::month_days(view_month_.year, view_month_.month));

					bool same = (date_.year == view_month_.year && date_.month == view_month_.month);

					logic_pos.x = date::day_of_week(view_month_.year, view_month_.month, 1);
					...
					while (day <= days)
					{
						for (; logic_pos.x < 7; ++logic_pos.x)
						{
							_m_draw_pos(graph, logic_pos, to_string(day), true, (same && date_.day == day));
							if (++day > days) break;
						}
						if (day > days) break;
						logic_pos.y++;
			...
			private:
				...
				struct color_rep
				{
					color highlighted{ static_cast<color_rgb>(0x4d56c8) };
					color selected{ static_cast<color_rgb>(0x2f3699) };
					color normal{ colors::black };
					color bgcolor{ static_cast<color_rgb>(0x88c4ff) };
				}colors_;
	...
	//class date_chooser
		...
		date date_chooser::read() const
		{
			return get_drawer_trigger().get_model()->read();
		}

		void date_chooser::weekstr(unsigned index, ::string str)
		{
			get_drawer_trigger().get_model()->weekname(index, move(str));
