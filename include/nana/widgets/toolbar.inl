...
namespace nana
{
	arg_toolbar::arg_toolbar(toolbar& tbar, size_t btn)
	...
	namespace drawerbase
	{
		namespace toolbar
		{
			struct item_type
			{
				...
				item_type(const char* text, const image& img, kind type)
			...
			{
			public:
				using container_type = vector<item_type*>;
				...
				void insert(size_type pos, string text, const image& img, item_type::kind type)
				{
					item_type* m = new item_type(move(text), img, type);
				...
				void push_back(const char* text, const image& img)
				{
				...
				{
					insert(cont_.size(), text, image(), item_type::kind::button);
				}
				...
				{
					if(pos < cont_.size())
						cont_.insert(cont_.begin() + pos, static_cast<item_type*>(NULL)); //both works in C++0x and C++2003
					else
						cont_.push_back(NULL);
				}
			...
			public:
				...
				item_renderer(graphics& graph, bool textout, unsigned scale, const color& bgcolor)
				...
				void operator()(int x, int y, unsigned width, unsigned height, item_type& item, state_t state)
				{
					...
					{
						rectangle background_r(x, y, width, height);
					...
					if(!item.image.empty())
					{
						...
						item.image.paste(rectangle{ imgsize }, graph, pos);
						if(item.enable == false)
						{
							graphics gh(imgsize);
							gh.bitblt(rectangle{ imgsize }, graph, pos);
							gh.rgb_to_wb();
							gh.paste(graph, pos.x, pos.y);
						}
						else if (state == state_t::normal)
						{
							graph.blend(rectangle(pos, imgsize), color(0xc0, 0xdd, 0xfc).blend(bgcolor, 0.5), 0.25);
						}
			...
			{
				event_handle event_size{ NULL };
				graphics* graph_ptr{ NULL };
			...
			//class drawer
				...
				void drawer::attached(widget_reference widget, graph_reference graph)
				{
					...
					widget_ = static_cast< toolbar*>(&widget);
					...
					impl_->event_size = API::events(widget.parent()).resized.connect_unignorable([this](const arg_resized& arg)
					{
						...
						API::window_size(wd, size(arg.width, widget_->size().height));
				...
				void drawer::detached()
				{
					...
					impl_->event_size = NULL;
					impl_->graph_ptr = NULL;
				}
				...
				void drawer::mouse_move(graph_reference graph, const arg_mouse& arg)
				{
					...
					if(impl_->which != which)
					{
						...
						{
							arg_toolbar arg{ *widget_, impl_->which };
						...
						if (which == npos || container.at(which)->enable)
						{
							...
							{
								arg_toolbar arg{ *widget_, which };
								widget_->events().enter.emit(arg, widget_->handle());
							}
						}

						if(which != npos)
							impl_->tooltip.show(widget_->handle(), point(arg.pos.x, arg.pos.y + 20), (*(container.begin() + which))->text, 0);
						else
							impl_->tooltip.close();
					}
				}
	...
	//class toolbar
		...
		void toolbar::append(const char* text, const image& img)
		{
			get_drawer_trigger().items().push_back(text, img);
			API::refresh_window(handle());
		}

		void toolbar::append(const char* text)
		{
