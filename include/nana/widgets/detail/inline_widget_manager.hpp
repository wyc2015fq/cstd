...
namespace nana
{
	namespace detail
	{
		...
		class inline_widget_manager
		{
			...
			{
				panel<false> docker;
				unique_ptr<inline_widget> widget_ptr;
			};
		public:
			...
			void set_factory(unique_ptr<factory> fac)
			{
				factory_.swap(fac);
			}
		...
		private:
			...
			unique_ptr<factory> factory_;
			vector<unique_ptr<widget_holder>> widgets_;
			vector<unique_ptr<widget_holder>> swap_widgets_;
		};
	}
}
