...
namespace detail
{
		...
		struct tree_node
		{
			...
			tree_node(tree_node* owner)
				:owner(owner), next(NULL), child(NULL)
			{}
		...
		public:
			...
			{
				return (verify(node) && (node->owner != &root_) ? node->owner : NULL);
			}

			node_type * node(node_type* node, const char* key)
			{
				if(node)
				{
			...
			node_type* insert(node_type* node, const char* key, const element_type& elem)
			{
				if(NULL == node)
			...
			node_type* insert(const char* key, const element_type& elem)
			{
			...
			node_type* find(const char* path) const
			{
				return _m_locate(path);
			}

			node_type* ref(const char* path)
			{
			...
			unsigned child_size_if(const char* key, PredAllowChild pac) const
			{
			...
			unsigned distance_if(const node_type * node, PredAllowChild pac) const
			{
				...
				while(iterator && iterator != node)
				{
					...
					while((NULL == iterator) && stack.size())
					{
			...
			template<typename PredAllowChild>
			node_type* advance_if(node_type* node, size_t off, PredAllowChild pac)
			{
				if(NULL == node)	node = root_.child;
		...
		private:
			...
			struct each_make_node
			{
				...
				bool operator()(const char* key_node)
				{
					node_type *child = node->child;
					node_type *tail = NULL;
					while(child)
			...
			struct find_key_node
			{
				...
				bool operator()(const char* key_node)
				{
					return ((node = _m_find(node->child, key_node)) != NULL);
				}

				node_type *node;
			};
		private:
			static node_type* _m_find(node_type* node, const char* key_node)
			{
				while(node)
				{
			...
			void _m_for_each(const char* key, Function function) const
			{
				if(key.size())
				{
			...
			node_type* _m_locate(const char* key)
			{
				if(key.size())
				{
			...
			node_type* _m_locate(const char* key) const
			{
				if(key.size())
				{
					find_key_node fkn(*this);
