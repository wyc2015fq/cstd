		...
		namespace float_listbox
		{
				...
				void render(widget_reference, graph_reference graph, const rectangle& r, const item_interface* item, state_t state)
				{
					...
					if(image_enabled_)
					{
						...
						if(item->image())
						{
							...
							item->image().stretch(rectangle{ item->image().size() }, graph, rectangle(to_pos, imgsz));
						}
						x += (image_pixels_ + 2);
					}
			...
			public:
				...
				void move_items(bool upwards, bool recycle)
				{
					...
					{
						size_t init_index = state_.index;
				...
				void attach(widget& wd, graphics& graph)
				{
					widget_ = &wd;
				...
				void draw()
				{
					if(module_)
					{
						...
						{
							//Draw items
							size_t items = (pages ? module_->max_items : module_->items.size());							
							items += state_.offset_y;

							const unsigned item_pixels = state_.renderer->item_pixels(*graph_);
							rectangle item_r(2, 2, graph_->width() - outter_w, item_pixels);

							state_.renderer->image(_m_image_enabled(), image_pixels_);
							for(size_t i = state_.offset_y; i < items; ++i)
							{
					...
					graph_->rectangle(rectangle(graph_->size()).pare_off(1), false, colors::white);
				}
			private:
			...
			private:
				...
				{
					size_t offset_y{0};
					size_t index{npos};			//The index of the selected item.
				...
				scroll<true> scrollbar_;

				const module_def* module_{NULL};
			};
