...
namespace nana
{
	namespace drawerbase
	{
		namespace picture
		{
			...
			{
				widget* wdg_ptr{NULL};
				unique_ptr<content_measurer> measurer;
				...
				struct back_image_tag
				{
					...
					align	align_horz{ align::left };
					align_v align_vert{ align_v::top };
					unique_ptr<element::bground> bground;	//If it is not a null ptr, the widget is stretchable mode
					bool			stretchable{ false };		//If it is true, the widget is stretchable mode without changing aspect ratio.
				}backimg;

				void draw_background(graphics& graph, const size& dimension)
				{
					if (!API::dev::copy_transparent_background(*wdg_ptr, graph))
					{
						...
						if (dimension.width < graph_size.width || dimension.height < graph_size.height || backimg.image.alpha())
						{
							if (gradual_bground.gradual_from.invisible() || gradual_bground.gradual_to.invisible())
								...
								graph.gradual_rectangle(rectangle{graph_size }, gradual_bground.gradual_from, gradual_bground.gradual_to, !gradual_bground.horizontal);
						}
					}
				}
			};
			...
			void drawer::refresh(graph_reference graph)
			{
				...
				if (!backimg.bground)
				{
					...
					if (backimg.stretchable)
					{
						...
						backimg.image.stretch(valid_area, graph, rectangle{ pos, fit_size });
					}
					else
					{
						switch (backimg.align_horz)
	...
	//class picture
		...
		void picture::load(image img, const rectangle& valid_area)
		{
			internal_scope_guard lock;
			auto& backimg = get_drawer_trigger().impl_->backimg;
			backimg.image = move(img);
		...
		void picture::align(align horz, align_v vert)
		{
			internal_scope_guard lock;
		...
		void picture::set_gradual_background(const color& from, const color& to, bool horizontal)
		{
