		...
		namespace tabbar
		{
			...
			struct item_t
			{
				...
				item_t(native_string_type&& text, any && value)
					: text(move(text)), value(move(value))
				{}
			};
			...
			{
			private:
				virtual void background(graph_reference graph, const rectangle&, const color& bgcolor)
				{
					if(bgcolor_ != bgcolor)
					{
						bgcolor_ = bgcolor;
				...
				virtual void add(graph_reference graph, const rectangle& r, state_t sta)
				{
				...
				virtual void close(graph_reference graph, const rectangle& r, state_t sta)
				{
				...
				virtual void close_fly(graph_reference graph, const rectangle& r, bool active, state_t sta)
				{
					...
					if (sta == item_renderer::highlight)
					{
						color bgcolor{ static_cast<color_rgb>(0xCCD2DD) };
						color rect_clr{ static_cast<color_rgb>(0x9da3ab) };
				...
				virtual void back(graph_reference graph, const rectangle& r, state_t sta)
				{
					_m_draw_arrow(graph, r, sta, direction::west);
				}

				virtual void next(graph_reference graph, const rectangle& r, state_t sta)
				{
					_m_draw_arrow(graph, r, sta, direction::east);
				}

				virtual void list(graph_reference graph, const rectangle& r, state_t sta)
				{
					_m_draw_arrow(graph, r, sta, direction::south);
				}
			private:
				void _m_draw_arrow(graph_reference graph, const rectangle& r, state_t sta, direction dir)
				{
			...
			public:
				...
				rectangle area(button_t btn, unsigned height) const
				{
					rectangle r(-1, 0, 0, 0);
			...
			{
			public:
				typedef list<item_t>::iterator iterator;
				typedef list<item_t>::const_iterator const_iterator;

				any& at(size_t i)
				{
					if(i < list_.size())
						return at_no_bound_check(i);
					throw out_of_range("invalid position of tabbar");
				}
				...
				const_iterator iterator_at(size_t pos) const
				{
					auto i = list_.cbegin();
					advance(i, pos);
					return i;
				...
				any& at_no_bound_check(size_t pos)
				{
					return iterator_at(pos)->value;
				}

				const any& at(size_t pos) const
				{
					if(pos < list_.size())
				...
				const any& at_no_bound_check(size_t pos) const
				{
					return iterator_at(pos)->value;
				}
				...
				void attach(window wd, graphics& graph)
				{
					basis_.wd = wd;
					basis_.graph = &graph;
				}
				...
				void insert(size_t pos, native_string_type&& text, any&& value)
				{
					if (pos >= list_.size())
						pos = list_.size();

					list_.emplace(iterator_at(pos), move(text), move(value));
					this->activate(pos);
					render();
				}
				...
				bool erase(size_t pos)
				{
					if(pos < list_.size())
					{
						...
						if ((NULL == evt_agent_) || evt_agent_->removed(pos, close_attach))
						{
							if (close_attach)
				...
				window attach(size_t pos, window wd, bool drop_other)
				{
					if (pos >= list_.size())
						throw out_of_range("tabbar: invalid position");
				...
				bool tab_color(size_t pos, bool is_bgcolor, const color& clr)
				{
					if (pos >= list_.size())
						throw out_of_range("tabbar: invalid position");
				...
				bool text(size_t pos, const native_string_type& str)
				{
					if (pos >= list_.size())
						throw out_of_range("tabbar: invalid position");
				...
				native_string_type text(size_t pos) const
				{
					if (pos >= list_.size())
						throw out_of_range("tabbar: invalid position");
					...
					if(trace_.what == trace_.toolbox)
					{
						if(toolbox_.renderable(trace_.u.button))
						{
							...
							case toolbox::ButtonAdd:
								if(event_code_mouse_up == arg.evt_code)
			...
			private: //Fundation
				...
				{
					menulister_.clear();
					auto fn = [this](menu::item_proxy& ipx)
					{
				...
				rectangle _m_close_fly_area(int x)
				{
					return rectangle(x + basis_.item_pixels - 18, (basis_.graph->height() - 14) / 2, 14, 14);
				}
				...
				size_t _m_where_itembar(int x, int y, int end)
				{
					...
					if(i < list_.size())
					{
						...
						{
							rectangle r = _m_close_fly_area(left);
				...
				rectangle _m_toolbox_area(toolbox::button_t btn) const
				{
					rectangle r(0, 0, toolbox_.item_pixels(), basis_.graph->height());
				...
				void _m_adjust()
				{
					if((NULL == basis_.graph) || (0 == list_.size())) return;
				...
				void _m_render()
				{
					if(!basis_.renderer || (NULL == basis_.graph))
						return;
					...
					m.r = rectangle{ basis_.graph->size() };
					...
					for(auto i = list_.cbegin(); i != list_.cend(); ++i, ++index)
					{
						...
						if(m.r.x + static_cast<int>(basis_.item_pixels) > 0)
						{
							...
							if(false == item.img.empty())
								item.img.stretch(rectangle{ item.img.size() }, *basis_.graph, rectangle(m.r.x + 4, (m.r.height - 16) / 2, 16, 16));

							if(item.text.size())
							{
								size ts = basis_.graph->text_extent_size(item.text);
								basis_.graph->palette(true, m.fgcolor.invisible() ? fgcolor : m.fgcolor);
								text_renderer tr(*basis_.graph);

								wstring wtext = to_wstring(item.text);
								tr.render({ m.r.x + 24, m.r.y + static_cast<int>(m.r.height - ts.height) / 2 },
											wtext, wtext.length(), basis_.item_pixels - 24 - 18, true);
							}
						}
				...
				void _m_render_toolbox(const color& bgcolor)
				{
					...
					for(int i = toolbox::ButtonAdd; i < toolbox::ButtonSize; ++i)
					{
						...
						rectangle r = _m_toolbox_area(btn);
			...
			private:
				list<item_t>		list_;
				event_agent_interface*	evt_agent_ = NULL;
				toolbox toolbox_;
				menu menulister_;
			...
			//class trigger
				...
				void trigger::activate(size_t pos)
				{
					if(layouter_->activate(pos))
						API::refresh_window(layouter_->widget_handle());
				}

				size_t trigger::activated() const
				{
					return layouter_->active();
				}

				any& trigger::at(size_t i) const
				{
					return layouter_->at(i);
				}

				any& trigger::at_no_bound_check(size_t i) const
				{
				...
				void trigger::insert(size_t pos, native_string_type&& text, any&& value)
				{
					layouter_->insert(pos, move(text), move(value));
				}
				...
				window trigger::attach(size_t pos, window wd, bool drop_other)
				{
					return layouter_->attach(pos, wd, drop_other);
				}

				void trigger::erase(size_t pos)
				{
					layouter_->erase(pos);
				}

				void trigger::tab_color(size_t i, bool is_bgcolor, const color& clr)
				{
					if(layouter_->tab_color(i, is_bgcolor, clr))
						API::refresh_window(layouter_->widget_handle());
				}

				void trigger::tab_image(size_t i, const image& img)
				{
					layouter_->tab_image(i, img);
					API::refresh_window(layouter_->widget_handle());
				}

				void trigger::text(size_t i, const native_string_type& str)
				{
					if(layouter_->text(i, str))
						API::refresh_window(layouter_->widget_handle());
				}

				native_string_type trigger::text(size_t i) const
				{
...
namespace nana
{
		namespace drawerbase
		{
			namespace tabbar_lite
			{
				struct item
				{
					::string text;
					any	value;
					::pair<int, int> pos_ends;
					window attached_window{ NULL };

					item(string t, any v)
						: text(move(t)), value(move(v))
					{}
				};

				void calc_px(vector<unsigned>& values, unsigned limit)
				{
					unsigned	base = 0;
				...
				public:
					...
					void set_widget(tabbar_lite& wdg)
					{
						widget_ = &wdg;
					}

					tabbar_lite* widget_ptr() const
					{
						return widget_;
					}
				...
				private:
					tabbar_lite * widget_{ NULL };
					forward_list<item> items_;
					indexes indexes_;
				};
				...
				private:
					void _m_calc_metrics(graph_reference graph, forward_list<item>& items)
					{
						vector<unsigned> pxs;
				...
				//class driver
					...
					{
						model_->set_widget(dynamic_cast<tabbar_lite&>(wdg));
					}
		...
		tabbar_lite::tabbar_lite(window parent_wd, bool visible, const rectangle& r)
		{
			this->create(parent_wd, r, visible);
		}

		//capacity
		size_t tabbar_lite::length() const
		{
			auto& items = get_drawer_trigger().get_model()->items();
			internal_scope_guard lock;
			return static_cast<size_t>(distance(items.cbegin(), items.cend()));
		}

		//modifiers
		void tabbar_lite::attach(size_t pos_set, window wd)
		{
			...
			throw out_of_range("invalid position of tabbar_lite");
		}

		window tabbar_lite::attach(size_t pos_set) const
		{
			auto model = get_drawer_trigger().get_model();
			internal_scope_guard lock;
		...
		void tabbar_lite::push_back(string text, any any)
		{
			auto & items = get_drawer_trigger().get_model()->items();
			internal_scope_guard lock;
			...
			while (true)
			{
				...
				if (i == items.cend())
				{
					items.emplace_after(next, move(text), move(any));
					break;
				}
		...
		void tabbar_lite::push_front(string text, any any)
		{
			auto & items = get_drawer_trigger().get_model()->items();
			internal_scope_guard lock;

			items.emplace_front(move(text), move(any));
			API::refresh_window(handle());
		}

		size_t tabbar_lite::selected() const
		{
			auto model = get_drawer_trigger().get_model();
			internal_scope_guard lock;
		...
		void tabbar_lite::erase(size_t pos, bool close_attached)
		{
			auto model = get_drawer_trigger().get_model();
			internal_scope_guard lock;
			...
			auto attached_wd = next(i)->attached_window;
