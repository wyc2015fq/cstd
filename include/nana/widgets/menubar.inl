...
namespace nana
{
	...
	namespace drawerbase
	{
		namespace menubar
		{
			struct item_type
			{
				item_type(char*& text, wchar_t shortkey, size_t shortkey_pos):
					text(move(text)),
					shortkey(shortkey),
					shortkey_pos(shortkey_pos)
				{}

				string		text;	///< Transformed text, the shortkey charactor has been proccessed.
				wchar_t			shortkey;
				size_t		shortkey_pos;
				menu	menu_obj;
				point	pos;
			...
			struct trigger::essence
			{
				...
				menu& push_back(const char* text)
				{
					wchar_t shortkey;
					size_t shortkey_pos;
					...
					if (shortkey && shortkey < 0x61)
						shortkey += (0x61 - 0x41);
					items.emplace_back(new item_type{ move(transformed_text), shortkey, shortkey_pos });
				...
				size_t find(wchar_t shortkey) const
				{
					if (shortkey)
					{
			...
			//class item_renderer
				...
				void item_renderer::background(const point& pos, const size& size, state item_state)
				{
			...
			//class trigger
				...
				void trigger::shortkey(graph_reference graph, const arg_keyboard& arg)
				{
					...
					auto index = ess_->find(arg.key);
					if(index != npos && (index != ess_->state.active || NULL == ess_->state.menu))
					{
						ess_->close_menu();
						ess_->state.nullify_mouse = true;
						ess_->state.active = index;
				...
				void trigger::_m_move(graph_reference graph, bool to_left)
				{
					...
					const size_t last_pos = ess_->items.size() - 1;
	...
	//class menubar
		...
		{
			widget_object<category::widget_tag, drawerbase::menubar::trigger, general_events, drawerbase::menubar::scheme>
				::create(wd, rectangle(size(API::window_size(wd).width, 28)));

			API::dev::set_menubar(handle(), true);
			evt_resized_ = API::events(wd).resized.connect_unignorable([this](const arg_resized& arg)
			{
				auto sz = this->size();
				sz.width = arg.width;
				this->size(sz);
		...
		menu& menubar::push_back(const char* text)
		{
			return get_drawer_trigger().ess().push_back(text);
		}

		menu& menubar::at(size_t pos) const
		{
