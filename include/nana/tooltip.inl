...
namespace nana
{
		...
		namespace tooltip
		{
			...
			private:
				void refresh(graph_reference graph)
				{
					...
					graph.rectangle(rectangle(graph.size()).pare_off(1), true, {0xf0, 0xf0, 0xf0});
				}
			};

			point pos_by_screen(point pos, const size& sz, bool overlap_allowed)
			{
			...
			public:
				tip_form()
					:	base_type(NULL, false, rectangle(), appear::bald<appear::floating>()),
						duration_(0)
				{
			...
			private:
				...
				bool tooltip_empty() const
				{
					return this->empty();
				}

				void tooltip_text(const char* text)
				{
					label_.caption(text);
					auto text_s = label_.measure(screen().from_window(label_).workarea().width * 2 / 3);
					this->size(size{ text_s.width + 10, text_s.height + 10 });
					...
					if (duration_)
					{
						...
						timer_.elapse(bind(&tip_form::_m_tick_duration, this));
					}
					else
					{
						timer_.interval(500);
						timer_.elapse(bind(&tip_form::_m_tick, this));
					}
					timer_.start();
				}

				virtual size tooltip_size() const
				{
					return this->size();
				}

				virtual void tooltip_move(const point& scr_pos, bool ignore_pos)
				{
				...
				virtual void duration(size_t d)
				{
					duration_ = d;
					timer_.reset();
				}
			...
			class controller
			{
				typedef pair<window, string> pair_t;

				typedef function<void(tooltip_interface*)> deleter_type;

				class tip_form_factory
					: public tooltip::factory_if_type
				{
					tooltip_interface * create()
					{
						return new tip_form;
					}

					void destroy(tooltip_interface* p)
					{
						delete p;
					}
				};
			...
			public:
				static shared_ptr<tooltip::factory_if_type>& factory()
				{
					static shared_ptr<tooltip::factory_if_type> fp;
					if (NULL == fp)
						fp = make_shared<tip_form_factory>();
				...
				void set(window wd, const char* str)
				{
					if (str.empty())
						_m_untip(wd);
					else
				...
				void show(const char* text, const point* pos, size_t duration)
				{
					if (NULL == window_ || window_->tooltip_empty())
					{
						auto fp = factory();

						window_ = unique_ptr<tooltip_interface, deleter_type>(fp->create(), [fp](tooltip_interface* ti)
						{
							fp->destroy(ti);
						});
					}
			...
			private:
				pair_t& _m_get(window wd)
				{
					...
					{
						if (event_code_mouse_enter == arg.evt_code)
						{
							auto & pr = _m_get(arg.window_handle);
							if (pr.second.size())
								this->show(pr.second, NULL, 0);
						}
						else
							this->close();
					};
					...
					cont_.emplace_back(wd, string());
					return cont_.back();
				}
			private:
				unique_ptr<tooltip_interface, deleter_type> window_;
				vector<pair_t> cont_;
			};
		}//namespace tooltip
	}//namespace drawerbase
	...
	//class tooltip
		...
		void tooltip::set(window wd, const char* text)
		{
		...
		void tooltip::show(window wd, point pos, const char* text, size_t duration)
		{
			internal_scope_guard lock;
