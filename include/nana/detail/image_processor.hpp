...
namespace detail
{
	namespace algorithms
	{
		...
		inline pixel_color_t * pixel_at(pixel_color_t * p, size_t bytes)
		{
			return reinterpret_cast<pixel_color_t*>(reinterpret_cast<char*>(p) + bytes);
		}

		inline const pixel_color_t * pixel_at(const pixel_color_t * p, size_t bytes)
		{
		...
		{
			void process(const pixel_buffer& s_pixbuf, const rectangle& r_src, pixel_buffer & pixbuf, const rectangle& r_dst) const
			{
			...
			: public image_process::stretch_interface
		{
			...
			void process(const pixel_buffer & s_pixbuf, const rectangle& r_src, pixel_buffer & pixbuf, const rectangle& r_dst) const
			{
				...
				const pixel_argb_t * s_raw_pixel_buffer = s_pixbuf.raw_ptr(0);
				...
				for(size_t row = 0; row < r_dst.height; ++row)
				{
					...
					const pixel_argb_t * s_line = pixel_at(s_raw_pixel_buffer,  sy * s_bytes_per_line);
					const pixel_argb_t * next_s_line = pixel_at(s_line, (sy < bottom ? s_bytes_per_line : 0));
		...
		{
			//process
			virtual void process(const pixel_buffer& s_pixbuf, const rectangle& s_r, pixel_buffer& d_pixbuf, const point& d_pos) const
			{
				...
				if(d_rgb && s_rgb)
				{
					...
					size_t d_step_bytes = d_pixbuf.bytes_per_line() - (s_r.width - rest) * sizeof(pixel_argb_t);
					size_t s_step_bytes = s_pixbuf.bytes_per_line() - (s_r.width - rest) * sizeof(pixel_argb_t);
		...
		{
			//process
			virtual void process(const pixel_buffer& s_pixbuf, const rectangle& s_r, pixel_buffer& d_pixbuf, const point& d_pos, double fade_rate) const
			{
				...
				if(d_rgb && s_rgb)
				{
					auto ptr = alloc_fade_table(fade_rate);//new unsigned char[0x100 * 2];
		...
		{
			virtual void process(pixel_buffer & pixbuf, const point& pos_beg, const point& pos_end, const color& clr, double fade_rate) const
			{
				auto rgb_color = clr.px_color().value;
				const size_t bytes_pl = pixbuf.bytes_per_line();
				
				unsigned char * fade_table = NULL;
				unique_ptr<unsigned char[]> autoptr;
				pixel_argb_t rgb_imd = {};
				if(fade_rate != 0.0)
				{
		...
		{
			void process(pixel_buffer& pixbuf, const rectangle& area, size_t u_radius) const
			{
				...
				unique_ptr<int[]> all_table(new int[(wh << 1) + wh + (large_edge << 1) + div_256]);
				...
				for(int y = 0; y < h; ++y)
				{
					...
					else
					{
						for(int i = - radius; i <= radius; ++i)
						{
							auto px = linepix[min(wm, (i > 0 ? i : 0))];
							sum_r += px.element.red;
							sum_g += px.element.green;
							sum_b += px.element.blue;
						}
