...
namespace nana{	namespace paint
{
	namespace detail
	{
		...
		public:
			...
			bool open(const void* file_data, size_t bytes)
			{
				...
				if (sizeof(bitmap_core_header) == header_bytes)
				{
					...
					throw invalid_argument("BMP with OS/2 BITMAPCOREHEADER is not supported now.");
				}

				auto header = reinterpret_cast<const bitmap_info_header *>(bmp_file + 1);

				const size_t bmp_height = abs(header->biHeight);
			...
			{
				ifstream ifs(filename.string(), ios::binary);

				auto const bytes = static_cast<unsigned>(file_size(filename));
				if (ifs && (bytes > static_cast<int>(sizeof(bitmap_file_header))))
				{
					unique_ptr<char[]> buffer{ new char[bytes] };

					ifs.read(buffer.get(), bytes);
					if (bytes == static_cast<size_t>(ifs.gcount()))
		...
		private:
			...
			{
				auto const image_height = abs(header->biHeight);
				const size_t total_pixels = header->biWidth * static_cast<size_t>(image_height);
