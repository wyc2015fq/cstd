...
{
	bool _m_read_ico(const void* data, size_t /*size*/)
	{
		auto width = 0;
		auto height = 0;
...
public:
	...
	bool open(const path& ico_file)
	{
		ifstream file(ico_file.string(), ios::binary);
	...
	void* native_handle()
	{
		...
		native_handle_ = ::LoadImage(NULL, path_, IMAGE_ICON, 0, 0, LR_LOADFROMFILE);
		return native_handle_;
#else
		return NULL;
#endif
