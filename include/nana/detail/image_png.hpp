...
namespace nana
{
	namespace paint{	namespace detail{
		...
		public:
			bool open(const path& png_file)
			{
				auto fp = ::fopen(to_osmbstr(to_utf8(png_file.native())), "rb");
			...
			{
			public:
				png_reader(const void* data, size_t bytes) noexcept
				...
				static void PNGCAPI read(png_structp png_ptr, png_bytep buf, png_size_t bytes)
				{
					...
					if (read_bytes)
						memcpy(buf, self->data_ptr_, read_bytes);
