...
namespace detail
{
	...
	size drawable_size(drawable_type dw)
	{
		if(0 == dw) return size();
#if defined(NANA_WINDOWS)
		BITMAP bmp;
	...
	unique_ptr<unsigned char[]> alloc_fade_table(double fade_rate)
	{
		unique_ptr<unsigned char[]> ptr(new unsigned char[0x100 * 2]);
	...
	void blend(drawable_type dw, const rectangle& area, pixel_color_t color, double fade_rate)
	{
		...
		rectangle r;
		if (false == overlap(rectangle{ drawable_size(dw) }, area, r))
			return;
	...
	size raw_text_extent_size(drawable_type dw, const wchar_t* text, size_t len)
	{
		if(NULL == dw || NULL == text || 0 == len) return size();
#if defined(NANA_WINDOWS)
		::SIZE size;
		...
		::XftTextExtentsUtf8(platform_spec::instance().open_display(), fs,
								reinterpret_cast<XftChar8*>(const_cast<char*>(utf8str)), utf8str.size(), &ext);
		return size(ext.xOff, fs->ascent + fs->descent);
	#else
		XRectangle ink;
		XRectangle logic;
		::XmbTextExtents(reinterpret_cast<XFontSet>(dw->font->native_handle()), text, len, &ink, &logic);
		return size(logic.width, logic.height);
	#endif
#endif
		return size();
	}

	size text_extent_size(drawable_type dw, const wchar_t * text, size_t len)
	{
		if (NULL == dw || NULL == text || 0 == len)
			return{};

		size extents = raw_text_extent_size(dw, text, len);
	...
	void draw_string(drawable_type dw, const point& pos, const wchar_t * str, size_t len)
	{
#if defined(NANA_WINDOWS)
