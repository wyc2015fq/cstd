...
//( [&]( const arg_treebox &tbox_arg_info ) { if (tbox_arg_info.operated) RefreshList(tbox_arg_info.item); });
		}
	private:
		void _m_expand(window, item_proxy node, bool exp)
		{
			if(!exp) return; //If this is contracted.

			string path = treebox_.make_key_path(node,  ("/")) +  ("/");
			//Trim the head of whitespace, more portable, because the root
			//under Linux is a whitespace
			string::size_type path_start_pos = path.find_first_not_of( (" "));
			if(path_start_pos != string::npos)
	...
	public:
		tab_page_datechooser(window wd)
			: panel<false>(wd)
		{
			date_.create(*this, rectangle(10, 10, 260, 200));
			textbox_.create(*this, rectangle(280, 10, 170, 23));
			textbox_.tip_string( ("Input a date:"));

            date_.events().dbl_click([this]()
            {
                auto dt=date_.read().read();
                textbox_.reset(charset(to_string(dt.year) + "-" + to_string(dt.month)+ "-" + to_string(dt.day)));
            });
		}
	private:
		date_chooser date_;
	...
	public:
			...
			:	panel<false>(wd)
		{
			...
			for(int i = 0; i < 6; ++i)
			{
				auto p = make_shared<checkbox>(*this);
				box_.push_back(p);
				...
				p->events().click([this]()
					{
						size_t index = this->group_.checked();
						string str = this->box_[index]->caption();
	...
	private:
		place place_;
		radio_group group_;
		vector<shared_ptr<checkbox>> box_;
		label label_;
	...
	private:
		void _m_init_buttons()
		{
			...
			ptr->icon( image("../Examples/image.ico"));
#else
			ptr->icon(image("../Examples/image.bmp"));
#endif
		...
		void _m_init_comboxs()
		{
			for(int i = 0; i < 2; ++i)
			{
				auto p = make_shared<combox>(*this);
			...
			ptr->events().selected( [this, mb](const arg_combox& acmb) mutable
			{
		...
		{
			const string tipstr[] = { ("Unknwon in progress"),  ("Known in progress")};
			for(int i = 0; i < 2; ++i)
			{
				auto p = make_shared<progress>(*this);
			...
			timer_.elapse([this](const arg_elapse& a)
			{
		...
		void _m_init_tabbar()
		{
			...
			tabpages_.push_back(make_shared<tab_page_listbox>(*this));
			tabbar_.push_back( ("treebox"));
			tabpages_.push_back(make_shared<tab_page_treebox>(*this));
			tabbar_.push_back( ("date_chooser"));
			tabpages_.push_back(make_shared<tab_page_datechooser>(*this));
			tabbar_.push_back( ("radio_group"));
			tabpages_.push_back(make_shared<tab_page_radiogroup>(*this));
	...
	private:
		...
		timer timer_;
		tooltip tooltip_;
		vector<shared_ptr<button>> buttons_;
		vector<shared_ptr<combox>> comboxs_;
		vector<shared_ptr<label>>	labels_;
		vector<shared_ptr<progress>> progresses_;
		tabbar<string> tabbar_;
		vector<shared_ptr<panel<false>>> tabpages_;
