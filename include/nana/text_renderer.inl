...
namespace nana
{
	namespace paint
	{
		...
		{
			template<typename F>
			void for_each_line(const wchar_t * str, size_t len, int top, F & f)
			{
			...
			struct draw_string
			{
				...
				unsigned operator()(const int top, const wchar_t * buf, size_t bufsize)
				{
					...
					case align::left:
						for(auto & ent : reordered)
						{
							...
							size ts = text_extent_size(dw, ent.begin, len);
							if(ts.height > pixels)	pixels = ts.height;
						
							if(pos.x + static_cast<int>(ts.width) > 0)
								draw_string(dw, pos, ent.begin, len);
					...
					case align::center:
						{
							unsigned lenpx = 0;
							unique_ptr<unsigned[]> entity_pxs(new unsigned[reordered.size()]);

							auto ent_px = entity_pxs.get();

							for(auto & ent : reordered)
							{
								auto ts = text_extent_size(dw, ent.begin, ent.end - ent.begin);
								if(ts.height > pixels) pixels = ts.height;
								lenpx += ts.width;
								*ent_px++ = ts.width;
							}
							...
							for(auto & ent : reordered)
							{
								if (pos.x + static_cast<int>(*ent_px) > 0)
									draw_string(dw, pos, ent.begin, ent.end - ent.begin);
					...
					case align::right:
						{
							...
							for(auto i = reordered.crbegin(); i != reordered.crend(); ++i)
							{
								...
								{
									pos.x -= static_cast<int>(ts.width);
									draw_string(dw, pos, i->begin, len);
								}
			...
			struct draw_string_omitted
			{
				...
				unsigned operator()(const int top, const wchar_t * buf, size_t bufsize)
				{
					...
					for(auto & i : reordered)
					{
						...
						size ts = text_extent_size(dw, i.begin, len);
						if(ts.height > pixels)	pixels = ts.height;
						...
						else
						{
							...
							graphics dum_graph({ r.width, r.height });
							...
							if(omitted_pixels)
								draw_string(dw, point{ endpos, top }, L"...", 3);
							break;
						}
					}
					return pixels;	
			...
			struct draw_string_auto_changing_lines
			{
				...
				unsigned operator()(const int top, const wchar_t * buf, size_t bufsize)
				{
					...
					vector<size> ts_keeper;

					auto const reordered = unicode_reorder(buf, bufsize);

					for(auto & i : reordered)
					{
						size ts = text_extent_size(dw, i.begin, i.end - i.begin);
						if(ts.height > pixels) pixels = ts.height;
					...
					if(x + static_cast<int>(str_w) > endpos)
					{
						...
						for(auto & i : reordered)
						{
							...
							if(beyond_edge)
							{
								const size_t len = i.end - i.begin;
								if(len > 1)
								{
									unique_ptr<unsigned[]> pixel_buf(new unsigned[len]);
									...
									do
									{
										...
										if(idx_splitted == len)
										{
											draw_string(dw, pos, i.begin + idx_head, idx_splitted - idx_head);
										...
										else
										{
											...
											if(u != head)
											{
												draw_string(dw, pos, head, u - head);
												idx_head += u - head;
												pos.x = x;
												pos.y += static_cast<int>(line_pixels);
												line_pixels = i_ts_keeper->height;
							...
							{
								draw_string(dw, pos, i.begin, i.end - i.begin);
								pos.x += static_cast<int>(i_ts_keeper->width);
							}
				...
				static bool splittable(const wchar_t * str, size_t index)
				{
			...
			struct extent_auto_changing_lines
			{
				...
				unsigned operator()(int top, const wchar_t * buf, size_t bufsize)
				{
					unsigned pixels = 0;
					...
					if(x + static_cast<int>(str_w) > endpos)
					{
						...
						for(auto & i : reordered)
						{
							...
							if(beyond_edge)
							{
								...
								if(len > 1)
								{
									unique_ptr<unsigned[]> scope_res(new unsigned[len]);
		...
		size text_renderer::extent_size(int x, int y, const wchar_t* str, size_t len, unsigned restricted_pixels) const
		{
			size extents;
			if(graph_)
			{
		...
		void text_renderer::render(const point& pos, const wchar_t * str, size_t len)
		{
			if (graph_)
			{
		...
		void text_renderer::render(const point& pos, const wchar_t* str, size_t len, unsigned restricted_pixels, bool omitted)
		{
			if (graph_)
			{
		...
		void text_renderer::render(const point& pos, const wchar_t * str, size_t len, unsigned restricted_pixels)
		{
			if (graph_)
			{
		...
		void aligner::draw(const char* text, point pos, unsigned width)
		{
			draw(to_wstring(text), pos, width);
		}

		void aligner::draw(const wchar* text, point pos, unsigned width)
		{
			...
			if (text_px <= width)
			{
				...
				graph_.bidi_string(pos, text, text.size());
				return;
			}

			const auto ellipsis = graph_.text_extent_size("...", 3).width;

			unique_ptr<unsigned[]> pixels(new unsigned[text.size()]);
			graph_.glyph_pixels(text, text.size(), pixels.get());
			...
			if (align::right == text_align_ex_)
			{
				...
				graph_.bidi_string(pos, text + substr_len, text.size() - substr_len);
				pos.x -= ellipsis;
			}
			else
			{
				...
				graph_.bidi_string(pos, text, substr_len);
