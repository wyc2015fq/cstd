...
namespace demo
{
	...
	public:
			...
			: panel<false>(wd)
		{
			...
			// find first directory --> use find ?
			for (const auto& dir : fs::directory_iterator{ fs_ext::def_rootstr })
			{
				if (!fs::is_directory(dir)) continue;
				string fname = dir.path().filename().generic_u8string();
	...
	{
		date_chooser  date_    { *this, rectangle(10, 10, 260, 200) };
		textbox	      textbox_ { *this, rectangle(280, 10, 170, 23) };
	 public:
	...
	public:
			...
			:	panel<false>(wd)
		{
            ...
            place_["real_gr"]<<rgr_ ;
			const string str[6] = { "Airbus", "AHTOHOB",
					                     "Boeing", "Bombardier",
                                         "Cessna", "EMBRAER"     };
			map<string, vector<string>> airplanes
			{
				{ "Airbus"    ,{ "320" , "330" } },
			...
			for(const auto& airplane : airplanes)
			{
				rgr_.add_option(airplane.first).events().checked([this](const arg_checkbox& msg) {
							string str = msg.widget->caption()   ;
							this->label_.caption( string("You have selected ") + str);
							this->categorize_.caption(string("Manufacturer\\") + str);
				      });
	...
	private:
		void _m_init_buttons()
		{
			...
			b_i.icon( image("../Examples/image.bmp"));
		...
		void _m_init_comboxs()
		{
			...
			cb_e.editable(true);
			cb_e.events().selected( [this, mb](const arg_combox& acmb) mutable
			{
			...
			cb_u.events().selected ( [this, mb](const arg_combox& acmb) mutable
			{
