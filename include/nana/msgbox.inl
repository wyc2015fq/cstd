...
namespace nana
{
		...
		{
		public:
			msgbox_window(window wd, const char* title, msgbox::button_t btn, msgbox::icon_t ico)
					...
					owner_(wd), pick_(msgbox::pick_yes)
			{
				...
				this->size(size{width_pixel, height_pixel});
				_m_icon(ico);
			}
			...
			void prompt(const char* text)
			{
				if(text.size())
				{
					...
					text_.create(*this, rectangle(12 + ico_pixels, 25, 1, 1));

					text_.bgcolor(colors::white);
					text_.caption(text);

					size ts = text_.measure(text_pixels);
					...
					rectangle r = API::make_center(owner_, sz.width, sz.height + ts.height);
					this->move(r);
				}
	...
	//class msgbox
	msgbox::msgbox()
		: wd_(NULL), button_(ok), icon_(icon_none)
	{}
	...
	msgbox::msgbox(const char* title)
		: wd_(NULL), title_(title), button_(ok), icon_(icon_none)
	{
		review_utf8(title_);
	}

	msgbox::msgbox(window wd, const char* title, button_t b)
	...
	msgbox & msgbox::operator<<(const wchar* str)
	{
	...
	msgbox & msgbox::operator<<(const char* u8str)
	{
		return operator<<(string{ u8str });
	}

	msgbox & msgbox::operator<<(const charset& cs)
	{
		string str = cs.to_bytes(unicode::utf8);
		sstream_ << str;
		return *this;
	}

	msgbox & msgbox::operator<<(ostream& (*manipulator)(ostream&))
	{
	...
	msgbox::pick_t msgbox::show() const
	{
		...
		auto bt = ::MessageBoxW(reinterpret_cast<HWND>(API::root(wd_)), to_wstring(sstream_.str()), to_wstring(title_), type);
	...
	{
	public:
		inputbox_window(window owner, image (&imgs)[4], rectangle (&valid_areas)[4], const ::string & desc, const char* title, size_t contents, unsigned fixed_pixels, const vector<unsigned>& each_height)
		...
		void set_input(const vector<window>& inputs, function<bool(window)> verifier)
		{
	...
	inputbox::boolean::boolean(::string label, bool initial_value)
		: impl_(new implement)
	{
		impl_->value = initial_value;
		impl_->label_text = move(label);
	...
	const char* inputbox::boolean::label() const
	{
	...
	window inputbox::boolean::create(window owner, unsigned label_px)
	{
		...
		graphics graph{ size{ 10, 10 } };
		auto value_px = graph.text_extent_size(impl->label_text).width + 20;

		impl->checkbox.create(impl->dock, rectangle{ (max)(static_cast<int>(label_px) - 18, 0), 0, value_px, 0 });
		impl->checkbox.check(impl->value);
		impl->checkbox.caption(impl->label_text);

		impl->dock.events().resized.connect_unignorable([impl, value_px](const arg_resized&)
		{
	...
	inputbox::integer::integer(::string label, int init_value, int begin, int last, int step)
		: impl_(new implement)
	{
		auto impl = impl_.get();
	...
	//Implementation of abstract_content
	const char* inputbox::integer::label() const
	{
	...
	window inputbox::integer::create(window parent, unsigned label_px)
	{
		...
		impl->label.text_align(align::right, align_v::center);
		...
		impl->spinbox.value(to_string(impl->value));

		impl->dock.events().resized.connect_unignorable([impl, label_px, value_px](const arg_resized& arg)
		{
	...
	unsigned inputbox::integer::fixed_pixels() const
	{
		...
		int longest = (abs(static_cast<int>(impl_->begin < 0 ? impl_->begin * 10 : impl_->begin)) < abs(static_cast<int>(impl_->last < 0 ? impl_->last * 10 : impl_->last)) ? impl_->last : impl_->begin);
		graphics graph{ size{ 10, 10 } };
		return graph.text_extent_size(to_wstring(longest)).width + 34;
	}
	//end class integer
	...
	inputbox::real::real(::string label, double init_value, double begin, double last, double step)
		: impl_(new implement)
	{
		auto impl = impl_.get();
		impl->value = init_value;
	...
	//Implementation of abstract_content
	const char* inputbox::real::label() const
	{
		return impl_->label_text;
	}
	...
	{
		//get the longest value
		auto longest = (abs(static_cast<int>(impl_->begin < 0 ? impl_->begin * 10 : impl_->begin)) < abs(static_cast<int>(impl_->last < 0 ? impl_->last * 10 : impl_->last)) ? impl_->last : impl_->begin);
		graphics graph{ size{ 10, 10 } };
		return graph.text_extent_size(to_wstring(longest)).width + 34;
	}
	//end class real
	...
	inputbox::text::text(::string label, ::string init_text)
		: impl_(new implement)
	{
	...
	inputbox::text::text(::string label, vector<::string> options)
		: impl_(new implement)
	{
	...
	void inputbox::text::tip_string(wstring tip)
	{
		impl_->tip = to_utf8(tip);
	}

	void inputbox::text::tip_string(string tip_utf8)
	{
	...
	::string inputbox::text::value() const
	{
	...
	//Implementation of abstract_content
	const char* inputbox::text::label() const
	{
		return impl_->label_text;
	}
	...
	inputbox::date::date(::string label)
		: impl_(new implement)
	{
		impl_->label_text.swap(label);
	}
	...
	::string inputbox::date::value() const
	{
		return to_string(impl_->month) + '-' + to_string(impl_->day) + ", " + to_string(impl_->year);
	}
	...
	//Implementation of abstract_content
	const char* inputbox::date::label() const
	{
		return impl_->label_text;
	}
	...
	window inputbox::date::create(window parent, unsigned label_px)
	{
		...
		for (auto i = begin(monthstr), end = end(monthstr); i != end; ++i)
			impl->wdg_month.push_back(i18n(*i));

		left += 104;
		impl->wdg_day.create(impl->dock, rectangle{ left, 0, 38, 0 });
		impl->wdg_day.range(1, date::month_days(today.year, today.month), 1);
		...
		impl->wdg_day.value(to_string(today.day));
		impl->wdg_year.value(to_string(today.year));

		impl->dock.events().resized.connect_unignorable([impl, label_px](const arg_resized& arg)
		{
			impl->label.size({ label_px, arg.height });
		...
		auto make_days = [impl]
		{
			...
			int days = date::month_days(year, static_cast<unsigned>(month));
			...
			impl->wdg_day.value(to_string(day));
		};
	...
	struct inputbox::path::implement
	{
		...
		implement(const filebox& fb, char*& labelstr)
			: fbox(fb), label_text(move(labelstr))
		{
			throw_not_utf8(label_text);
		}
	};

	inputbox::path::path(::string label, const filebox& fb)
		: impl_(new implement(fb, move(label)))
	{
	}
	...
	::string inputbox::path::value() const
	{
	...
	//Implementation of abstract_content
	const char* inputbox::path::label() const
	{
		return impl_->label_text;
	}
	...
	inputbox::inputbox(window owner, ::string desc, ::string title)
		:	owner_{ owner },
	...
	void inputbox::image(image img, bool is_left, const rectangle& valid_area)
	{
		auto pos = (is_left ? 2 : 3);
		images_[pos] = move(img);
		valid_areas_[pos] = valid_area;
	}

	void inputbox::image_v(image img, bool is_top, const rectangle& valid_area)
	{
		auto pos = (is_top ? 0 : 1);
		images_[pos] = move(img);
		valid_areas_[pos] = valid_area;
	...
	void inputbox::verify(function<bool(window)> verifier)
	{
		verifier_ = move(verifier);
	}
	...
	void inputbox::_m_fetch_args(vector<abstract_content*>&)
	{}

	bool inputbox::_m_open(vector<abstract_content*>& contents, bool modal)
	{
		vector<unsigned> each_pixels;
