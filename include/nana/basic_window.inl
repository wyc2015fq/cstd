		...
		//struct basic_window
			//struct basic_window::other_tag
				basic_window::other_tag::other_tag(category::flags categ)
					: category(categ), active_window(NULL), upd_state(update_state::none)
				{
			...
			basic_window::basic_window(basic_window* owner, unique_ptr<widget_notifier_interface>&& wdg_notifier, category::root_tag**)
				: widget_notifier(move(wdg_notifier)), other(category::flags::root)
			{
				drawer.bind(this);
				_m_init_pos_and_size(NULL, rectangle());
			...
			void basic_window::bind_native_window(native_window_type wd, unsigned width, unsigned height, unsigned extra_width, unsigned extra_height, graphics& graphics)
			{
			...
			void basic_window::_m_initialize(basic_window* agrparent)
			{
				...
				{
					if(agrparent && (system::this_thread_id() != agrparent->thread_id))
						agrparent = NULL;
				...
				thread_id = system::this_thread_id();
				if(agrparent && (thread_id != agrparent->thread_id))
					thread_id = agrparent->thread_id;
			}

			bool basic_window::set_events(const shared_ptr<general_events>& p)
			{
				annex.events_ptr = p;
				return true;
			}
