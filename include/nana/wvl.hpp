...
namespace nana
{
	namespace detail
	{
		...
		private:
			static void insert_form(widget*);
		};
        ...
        public:
            ...
            {
                auto p = new Form(forward<Args>(args)...);

                if (p->empty())
                    throw logic_error("form_loader failed to create the form");

                form_loader_private::insert_form(p);
                if (IsVisible)
                    p->show();
    ...
    using form_loader = form_loader<Form, IsVisible>;
	...
	void exec(
		      ...
		      function<void()> = {} ///< emit events to mimics user actions and may asert results
	         );
