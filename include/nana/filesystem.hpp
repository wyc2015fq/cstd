...
// add boost::filesystem into filesystem
namespace std {
	namespace experimental {
		namespace filesystem {
			...
			enum file_type {
				none = boost::ft_status_unknown,
				not_found = boost::ft_file_not_found,
				regular = boost::ft_regular_file,
				directory = boost::ft_directory_file,
				symlink = boost::ft_symlink_file,
				block = boost::ft_block_file,
				character = boost::ft_character_file,
				fifo = boost::ft_fifo_file,
				socket = boost::ft_socket_file,
				unknown = boost::ft_type_unknown,
			};
		} // filesystem
	} // experimental
} // std
...
namespace nana  { namespace experimental { namespace filesystem
{
	...
	enum file_type
	{
		ft_none = 0,   ///< has not been determined or an error occurred while trying to determine
		ft_not_found = -1, ///< Pseudo-type: file was not found. Is not considered an error
		ft_regular = 1,
		ft_directory = 2  ,
		ft_symlink =3, ///< Symbolic link file
		ft_block =4,  ///< Block special file
		ft_character= 5 ,  ///< Character special file
		ft_fifo = 6 ,  ///< FIFO or pipe file
		ft_socket =7,
		ft_unknown= 8  ///< The file does exist, but is of an operating system dependent type not covered by any of the other
	};

	enum perms
	{
		pe_none = 0,		///< There are no permissions set for the file.
		pe_all = 0x1FF,		///< owner_all | group_all | others_all
		pe_mask = 0xFFF,		///< all | set_uid | set_gid | sticky_bit.
		pe_unknown = 0xFFFF	///<  not known, such as when a file_status object is created without specifying the permissions
	};
    //enum copy_options;
	...
	typedef clock_t file_time_type; ///< trivial-clock> ;

	class file_status
	{
		file_type m_ft;// = ft_none;
		perms     m_prms;// = pt_unknown;

	public:
		explicit file_status(file_type ft = ft_none, perms prms = pe_unknown);
	...
	private:
		void _m_assign(const char* source_utf8);
		void _m_assign(const wchar* source);
	private:
  ...
  enum error_code {aaaa,};

	class filesystem_error
	{
	public:
		filesystem_error(const char* msg, error_code);

		filesystem_error(const char* msg, const path& path1, error_code err);
		filesystem_error(const char* msg, const path& path1, const path& path2, error_code err);
	...
	file_status status(const path& p, error_code&);

	uint64 file_size(const path& p);
	//uintmax_t file_size(const path& p, error_code& ec) noexcept;

	inline bool is_directory(file_status s) noexcept
	{ return s.type() == ft_directory ;}
	...
	basic_string<CharType> parent_path(const basic_string<CharType>& path)
	{
		...
		return index ? path.substr(0, index + 1) : basic_string<CharType>();
	}
#ifndef CXX_NO_INLINE_NAMESPACE
} //end namespace v1
#endif
