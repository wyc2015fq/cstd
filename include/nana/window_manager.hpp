...
namespace detail
{
	...
	class window_manager
	{
		...
		{
			revertible_mutex(const revertible_mutex&) {}
			revertible_mutex& operator=(const revertible_mutex&) {}
			revertible_mutex(revertible_mutex&) {}
			revertible_mutex& operator=(revertible_mutex&) {}
		public:
			revertible_mutex();
			~revertible_mutex();
	...
	public:
		...
		void all_handles(vector<core_window_t*>&) const;
		...
		void icon(core_window_t*, const image& small_icon, const image& big_icon);
		...
		bool size(core_window_t*, size, bool passive, bool ask_update);

		core_window_t* root(native_window_type) const;

		//Copy the root buffer that wnd specified into DeviceContext
		void map(core_window_t*, bool forced, const rectangle* update_area = NULL);

		bool update(core_window_t*, bool redraw, bool force, const rectangle* update_area = NULL);
		void refresh_tree(core_window_t*);

		void do_lazy_refresh(core_window_t*, bool force_copy_to_screen, bool refresh_tree = false);

		bool get_graphics(core_window_t*, graphics&);
		bool get_visual_rectangle(core_window_t*, rectangle&);

		vector<core_window_t*> get_children(core_window_t*) const;
		...
		bool calc_window_point(core_window_t*, point&);
		...
		vector<pair<core_window_t*, unsigned long>> shortkeys(core_window_t*, bool with_children);

		core_window_t* find_shortkey(native_window_type, unsigned long key);

		void set_safe_place(core_window_t* wd, function<void()>&& fn);
	...
	private:
		...
		struct attribute
		{
			struct captured
			{
				...
				vector<pair<core_window_t*, bool> > history;
			}capture;
		}attr_;
