	...
	bool shared_wrapper::open(const char* filename)
		{
			...
			if(filename)
			{
				string file;
				string ofn = filename;
				string::size_type length = ofn.length();

				if(length > 13)
				{
					transform(filename + length - 13, filename + length , back_inserter(file), tolower);
					if(file == ".nana_shared")
					{
#if defined(NANA_LINUX) || defined(NANA_MACOS)
				...
				impl_.handle = shared_helper::open(filename);
			}
	...
	void shared_wrapper::close()
		{
			if(impl_.handle)
			{
				shared_helper::close(impl_.handle);
				impl_.symbol = "";
