...
namespace nana
{
	namespace place_parts
	{
		...
		{
		public:
			void set_renderer(const function<void(window, graphics&, mouse_action)>& rd)
			{
				renderer_ = rd;
			}
		private:
			...
			void mouse_enter(graph_reference graph, const arg_mouse&)
			{
				refresh(graph);
				API::dev::lazy_refresh();
			}

			void mouse_move(graph_reference graph, const arg_mouse&)
			{
				refresh(graph);
				API::dev::lazy_refresh();
			}
		...
		private:
			window window_handle_{NULL};
			function<void(window, graphics&, mouse_action)> renderer_;
		};
		...
		{
		public:
			void on_close(function<void()>&& fn)
			{
				close_fn_ = move(fn);
			}
		private:
			virtual void attached(widget_reference wdg, graph_reference graph)
			{
				window_handle_ = wdg;
				text_rd_.reset(new text_renderer(graph));
			}
			...
			void refresh(graph_reference& graph)
			{
				...
				if (x_pointed_)
				{
					...
					if(x_state_ == mouse_action::pressed)
		...
		private:
			rectangle _m_button_area() const
			{
		...
		public:
			...
			unique_ptr<text_renderer> text_rd_;
			bool x_pointed_{ false };
			mouse_action x_state_{ mouse_action::normal };
			facade<element::x_icon>	x_icon_;

			function<void()>	close_fn_;
		};
			...
			: public widget_object <category::lite_widget_tag, drawer_trigger>
		{
			...
			using factory = function<unique_ptr<widget>(window)>;

			struct panel
			{
				unique_ptr<widget> widget_ptr;
			};
		public:
				...
				auto grab_fn = [this](const arg_mouse& arg)
				{
					...
					else if (event_code_mouse_move == arg.evt_code)
					{
						if (arg.left_button && moves_.started)
						{
							...
							if (!floating())
							{
								if (abs(move_pos.x) > 4 || abs(move_pos.y) > 4)
									float_away(move_pos);
							}
							else
							{
					...
					{
						if ((mouse::left_button == arg.button) && moves_.started)
						{
			...
			void float_away(const point& move_pos)
			{
				if (container_)
					return;
		...
		private:
			widget* _m_add_pane(factory & fn)
			{
				...
				if (wdg)
				{
					if (tabbar_)
					{
						tabbar_->push_back(charset(wdg->caption()));
		...
		private:
			window host_window_{NULL};
			place_parts::dock_notifier_interface* notifier_{ NULL };
			unique_ptr<form>	container_;
			dockarea_caption	caption_;
			deque<panel>	panels_;
			unique_ptr<tabbar_lite> tabbar_;

			struct moves
			{
				bool started{ false };
				point start_pos;
				point start_container_pos;
			}moves_;
		};//class dockarea
		...
		{
		public:
			enum kind{ none, integer, real, percent };
		...
		public:
			...
			void set_array(const vector<number_t>& v)
			{
				all_edges_ = false;
				margins_ = v;
			}

			number_t get_edge(size_t edge) const
			{
			...
			rectangle area(const rectangle& field_area) const
			{
				if (margins_.empty())
					return field_area;
