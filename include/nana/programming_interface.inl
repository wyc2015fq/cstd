...
namespace nana
{
	...
	namespace restrict
	{
		namespace
		{
			...
			inline window_manager& wd_manager()
			{
...
namespace API
{
	...
	using interface_type = native_interface;

	namespace detail
	{
		widget_geometrics* make_scheme(scheme_factory_interface&& factory)
		{
			return bedrock.scheme().create(static_cast<scheme_factory_interface&&>(factory));
		}

		bool emit_event(event_code evt_code, window wd, const event_arg& arg)
		{
			return bedrock.emit(evt_code, reinterpret_cast<basic_window*>(wd), arg, true, bedrock.get_thread_context(), false);
		}

		bool emit_internal_event(event_code evt_code, window wd, const event_arg& arg)
		{
			return bedrock.emit(evt_code, reinterpret_cast<basic_window*>(wd), arg, true, bedrock.get_thread_context(), true);
		}
		...
		void enum_widgets_function_base::enum_widgets(window wd, bool recursive)
		{
			...
			auto children = wd_manager().get_children(reinterpret_cast<basic_window*>(wd));
		...
		{
			if (!wd_manager().available(reinterpret_cast<basic_window*>(wd)))
				return NULL;
	...
	effects::edge_nimbus effects_edge_nimbus(window wd)
	{
		auto const iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard isg;
		return (wd_manager().available(iwd) ? iwd->effect.edge_nimbus : effects::edge_nimbus::none);
	}
	...
	void effects_bground(initializer_list<window> wdgs, const effects::bground_factory_interface& factory, double fade_rate)
	{
		for (auto wd : wdgs)
	...
	bground_mode effects_bground_mode(window wd)
	{
		auto const iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard isg;
		if(wd_manager().available(iwd) && iwd->effect.bground)
	...
	void effects_bground_remove(window wd)
	{
		...
		if(wd_manager().available(iwd))
		{
			if(wd_manager().enable_effects_bground(iwd, false))
				API::refresh_window(wd);
		}
	}
	...
	namespace dev
	{
		...
		void affinity_execute(window window_handle, const function<void()>& fn)
		{
			interface_type::affinity_execute(root(window_handle), fn);
		}

		bool set_events(window wd, const shared_ptr<general_events>& gep)
		{
		...
		widget_geometrics* get_scheme(window wd)
		{
			auto iwd = reinterpret_cast<basic_window*>(wd);
			internal_scope_guard lock;
			return (wd_manager().available(iwd) ? iwd->annex.scheme : NULL);
		}

		void set_measurer(window wd, dev::widget_content_measurer_interface* measurer)
		{
			auto iwd = reinterpret_cast<basic_window*>(wd);
			internal_scope_guard lock;
			if (wd_manager().available(iwd))
		...
		native_string_type window_caption(window wd)
		{
			auto const iwd = reinterpret_cast<basic_window*>(wd);
			internal_scope_guard isg;
		...
		void window_caption(window wd, native_string_type title)
		{
			auto const iwd = reinterpret_cast<basic_window*>(wd);
			internal_scope_guard lock;
			if (wd_manager().available(iwd))
			{
				...
				wd_manager().update(iwd, true, false);
			}
		}

		window create_window(window owner, bool nested, const rectangle& r, const appearance& ap, widget* wdg)
		{
			return reinterpret_cast<window>(wd_manager().create_root(reinterpret_cast<basic_window*>(owner), nested, r, ap, wdg));
		}

		window create_widget(window parent, const rectangle& r, widget* wdg)
		{
			return reinterpret_cast<window>(wd_manager().create_widget(reinterpret_cast<basic_window*>(parent), r, false, wdg));
		}

		window create_lite_widget(window parent, const rectangle& r, widget* wdg)
		{
			return reinterpret_cast<window>(wd_manager().create_widget(reinterpret_cast<basic_window*>(parent), r, true, wdg));
		}

#ifndef WIDGET_FRAME_DEPRECATED
		window create_frame(window parent, const rectangle& r, widget* wdg)
		{
			return reinterpret_cast<window>(wd_manager().create_frame(reinterpret_cast<basic_window*>(parent), r, wdg));
		}
#endif

		graphics* window_graphics(window wd)
		{
			internal_scope_guard isg;
			if(wd_manager().available(reinterpret_cast<basic_window*>(wd)))
		...
		{
			bedrock.delay_restore(enable ? 0 : 1);
		}
		...
		void register_menu_window(window wd, bool has_keyboard)
		{
			internal_scope_guard lock;
			if (wd_manager().available(reinterpret_cast<basic_window*>(wd)))
				bedrock.set_menu(reinterpret_cast<basic_window*>(wd)->root, has_keyboard);
		}
		...
		bool copy_transparent_background(window wd, graphics& graph)
		{
		...
		bool copy_transparent_background(window wd, const rectangle& src_r, graphics& graph, const point& dst_pt)
		{
			auto iwd = reinterpret_cast<basic_window*>(wd);
			internal_scope_guard lock;
		...
		void lazy_refresh()
		{
			bedrock.thread_context_lazy_refresh();
		}

		void draw_shortkey_underline(graphics& graph, const char* text, wchar_t shortkey, size_t shortkey_position, const point& text_pos, const color& line_color)
		{
			if (shortkey)
			{
				auto off_x = (shortkey_position ? graph.text_extent_size(text, shortkey_position).width : 0);
	...
	{
		internal_scope_guard lock;
		bedrock.close_thread_window(system::this_thread_id());
	}
	...
	string transform_shortkey_text(string text, wchar_t &shortkey, string::size_type *skpos)
	{
		shortkey = 0;
		...
		while(true)
		{
			...
			if(pos != wstring::npos)
			{
				...
				{
					shortkey = utf::char_at(text + pos, 0, NULL);
	...
	{
		return wd_manager().register_shortkey(reinterpret_cast<basic_window*>(wd), key);
	}

	void unregister_shortkey(window wd)
	{
		wd_manager().unregister_shortkey(reinterpret_cast<basic_window*>(wd), false);
	}
	...
	rectangle make_center(window wd, unsigned width, unsigned height)
	{
		rectangle r = make_center(width, height);
	...
	void window_icon_default(const image& small_icon, const image& big_icon)
	{
		wd_manager().icon(NULL, small_icon, big_icon);
	}

	void window_icon(window wd, const image& small_icon, const image& big_icon)
	{
		if(NULL != wd)
			wd_manager().icon(reinterpret_cast<basic_window*>(wd), small_icon, big_icon);
	}

	bool empty_window(window wd)
	{
		return (wd_manager().available(reinterpret_cast<basic_window*>(wd)) == false);
	}

	bool is_window(window wd)
	{
		return wd_manager().available(reinterpret_cast<basic_window*>(wd));
	}
	...
	{
		return reinterpret_cast<window>(wd_manager().root(wd));
	}
	...
	{
		return wd_manager().insert_frame(reinterpret_cast<basic_window*>(frame), native_window);
	}
	...
	native_window_type frame_container(window frame)
	{
		...
		internal_scope_guard lock;
		if (wd_manager().available(frm) && (frm->other.category == category::flags::frame))
	...
	{
		wd_manager().close(reinterpret_cast<basic_window*>(wd));
	}

	void show_window(window wd, bool show)
	{
		wd_manager().show(reinterpret_cast<basic_window*>(wd), show);
	}
	...
	void zoom_window(window wd, bool ask_for_max)
	{
		...
		internal_scope_guard lock;
		if(wd_manager().available(core_wd))
		{
	...
	window get_owner_window(window wd)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if(wd_manager().available(iwd) && (iwd->other.category == category::flags::root))
		{
			auto owner = interface_type::get_owner_window(iwd->root);
			if(owner)
				return reinterpret_cast<window>(wd_manager().root(owner));
		}
	...
	{
		return wd_manager().set_parent(reinterpret_cast<basic_window*>(wd), reinterpret_cast<basic_window*>(new_parent));
	}

	void umake_event(event_handle eh)
	{
		bedrock.evt_operation().erase(eh);
	}

	point window_position(window wd)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if(wd_manager().available(iwd))
	...
	void move_window(window wd, const point& pos)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if(wd_manager().move(iwd, pos.x, pos.y, false))
		{
			basic_window* update_wd = NULL;
			...
			if (anc != update_wd)
				wd_manager().update(anc, false, false);
		}
	}
	...
	void move_window(window wd, const rectangle& r)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if(wd_manager().move(iwd, r))
		{
			if (category::flags::root != iwd->other.category)
				iwd = iwd->seek_non_lite_widget_ancestor();

			wd_manager().update(iwd, false, false);
		}
	}
	...
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		native_window_type native_after = NULL;
		internal_scope_guard lock;
		if (wd_manager().available(iwd) && (category::flags::root == iwd->other.category))
		{
			if(wd_after)
			{
				auto iwd_after = reinterpret_cast<basic_window*>(wd_after);
				if (wd_manager().available(iwd_after) && (iwd_after->other.category == category::flags::root))
				{
					native_after = iwd_after->root;
					action_if_no_wd_after = z_order_action_none;
				}
				else
					return false;
			}
	...
	void draw_through(window wd, function<void()> draw_fn)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if (!wd_manager().available(iwd))
			throw invalid_argument("draw_through: invalid window parameter");

		if (category::flags::root != iwd->other.category)
			throw invalid_argument("draw_through: the window is not a root widget");
	...
	void map_through_widgets(window wd, native_drawable_type drawable)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if (wd_manager().available(iwd) && iwd->is_draw_through() )
			bedrock.map_through_widgets(iwd, drawable);
	}

	size window_size(window wd)
	{
		rectangle r;
	...
	void window_size(window wd, const size& sz)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if(wd_manager().size(iwd, sz, false, false))
		{
			if (category::flags::root != iwd->other.category)
				iwd = iwd->seek_non_lite_widget_ancestor();
	...
	size window_outline_size(window wd)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if (!wd_manager().available(iwd))
	...
	bool track_window_size(window wd, const size& sz, bool true_for_max)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if(wd_manager().available(iwd) == false)
			return false;

		size & ts = (true_for_max ? iwd->max_track_size : iwd->min_track_size);
		if(!sz.empty())
		{
			if(true_for_max)
			{
	...
	void window_enabled(window wd, bool enabled)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if(wd_manager().available(iwd) && (iwd->flags.enabled != enabled))
		{
			iwd->flags.enabled = enabled;
			wd_manager().update(iwd, true, true);
			if(category::flags::root == iwd->other.category)
	...
	bool window_enabled(window wd)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		return (wd_manager().available(iwd) ? iwd->flags.enabled : false);
	}
	...
	{
		wd_manager().update(reinterpret_cast<basic_window*>(wd), true, false);
	}

	void refresh_window_tree(window wd)
	{
		wd_manager().refresh_tree(reinterpret_cast<basic_window*>(wd));
	}
	...
	{
		wd_manager().update(reinterpret_cast<basic_window*>(wd), false, true);
	}


	void window_caption(window wd, const char* title_utf8)
	{
	...
	void window_caption(window wd, const wchar* title)
	{
		auto const iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if (wd_manager().available(iwd))
	...
	string window_caption(window wd)
	{
		auto const iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if (wd_manager().available(iwd))
	...
	{
		internal_scope_guard lock;
		return reinterpret_cast<window>(bedrock.focus());
	}

	void focus_window(window wd)
	{
		wd_manager().set_focus(reinterpret_cast<basic_window*>(wd), false, arg_focus::reason::general);
		wd_manager().update(reinterpret_cast<basic_window*>(wd), false, false);
	}

	window capture_window()
	{
		return reinterpret_cast<window>(wd_manager().capture_window());
	}

	void set_capture(window wd, bool ignore_children)
	{
		wd_manager().capture_window(reinterpret_cast<basic_window*>(wd), true, ignore_children);
	}

	void release_capture(window wd)
	{
		//The 3rd parameter is useless when the 2nd parameter is false.
		wd_manager().capture_window(reinterpret_cast<basic_window*>(wd), false, false);
	}
	...
	public:
		...
		void disable_throw() noexcept
		{
			throw_ = false;
		}

		void effective_range(const rectangle& range)
		{
			internal_scope_guard lock;
			auto caret = _m_caret();
			if (caret)
		...
		void dimension(const size& size)
		{
			internal_scope_guard lock;
			auto caret = _m_caret();
			if (caret)
	...
	private:
		...
		{
			if (wd_manager().available(window_) && window_->annex.caret_ptr)
				return window_->annex.caret_ptr;

			if (throw_)
				throw runtime_error("nana.api: access invalid caret");
	...
	void create_caret(window wd, const size& caret_size)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if (wd_manager().available(iwd) && !(iwd->annex.caret_ptr))
			iwd->annex.caret_ptr = new caret(iwd, caret_size);
	}
	...
	void destroy_caret(window wd)
	{
		...
		if(wd_manager().available(iwd))
		{
			...
			iwd->annex.caret_ptr = NULL;
			delete p;
		}
	}

	unique_ptr<caret_interface> open_caret(window window_handle, bool disable_throw)
	{		
		auto p = new caret_proxy{ reinterpret_cast<basic_window*>(window_handle) };
		if (disable_throw)
			p->disable_throw();

		return unique_ptr<caret_interface>{ p };
	}

	void tabstop(window wd)
	{
		wd_manager().enable_tabstop(reinterpret_cast<basic_window*>(wd));
	}
	...
	{
		basic_window* ts_wd = wd_manager().tabstop(reinterpret_cast<basic_window*>(wd), next);
		wd_manager().set_focus(ts_wd, false, arg_focus::reason::general);
		wd_manager().update(ts_wd, false, false);
	...
	void take_active(window wd, bool active, window take_if_active_false)
	{
		...
		if (wd_manager().available(iwd))
		{
			if (active || (take_if_false && (wd_manager().available(take_if_false) == false)))
				take_if_false = 0;
	...
	bool window_graphics(window wd, graphics& graph)
	{
		return wd_manager().get_graphics(reinterpret_cast<basic_window*>(wd), graph);
	}

	bool root_graphics(window wd, graphics& graph)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if(wd_manager().available(iwd))
	...
	bool get_visual_rectangle(window wd, rectangle& r)
	{
		return wd_manager().get_visual_rectangle(reinterpret_cast<basic_window*>(wd), r);
	}

	void typeface(window wd, const font& font)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if(wd_manager().available(iwd))
	...
	bool calc_screen_point(window wd, point& pos)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if(wd_manager().available(iwd))
	...
	bool calc_window_point(window wd, point& pos)
	{
		return wd_manager().calc_window_point(reinterpret_cast<basic_window*>(wd), pos);
	}

	window find_window(const point& pos)
	{
		...
		if(wd)
		{
			...
			return reinterpret_cast<window>(
						wd_manager().find_window(wd, clipos));
		}
		return NULL;
	}
	...
	mouse_action mouse_action(window wd)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if(wd_manager().available(iwd))
	...
	element_state element_state(window wd)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		if(wd_manager().available(iwd))
		{
			...
			case mouse_action::normal_captured:
				return (is_focused ? element_state::focus_normal : element_state::normal);
			case mouse_action::hovered:
				return (is_focused ? element_state::focus_hovered : element_state::hovered);
			case mouse_action::pressed:
				return element_state::pressed;
			default:
				if(false == iwd->flags.enabled)
					return element_state::disabled;
			}
		}
	...
	bool ignore_mouse_focus(window wd)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;
		return (wd_manager().available(iwd) ? iwd->flags.ignore_mouse_focus : false);
	}

	void at_safe_place(window wd, function<void()> fn)
	{
		wd_manager().set_safe_place(reinterpret_cast<basic_window*>(wd), move(fn));
	}

	optional<pair<size, size>> content_extent(window wd, unsigned limited_px, bool limit_width)
	{
		auto iwd = reinterpret_cast<basic_window*>(wd);
		internal_scope_guard lock;

		if (wd_manager().available(iwd) && iwd->annex.content_measurer)
		{
			graphics* graph = &iwd->drawer.graphics;
			graphics temp_graph;
			if (graph->empty())
			{
				temp_graph.make({ 1, 1 });
			...
			if (extent)
				return make_pair(extent.value(), extent.value() + iwd->annex.content_measurer->extension());
		}
