...
namespace nana
{
	namespace detail
	{
		//class window_layout
			...
			bool window_layout::maproot(core_window_t* wd, bool have_refreshed, bool req_refresh_children)
			{
				...
				if (read_visual_rectangle(wd, vr))
				{
					...
					if (category::flags::lite_widget != wd->other.category)
						graph.bitblt(vr, wd->drawer.graphics, point(vr.x - wd->pos_root.x, vr.y - wd->pos_root.y));

					_m_paste_children(wd, have_refreshed, req_refresh_children, vr, graph, point());

					if (wd->parent)
					{
						vector<wd_rectangle>	blocks;
						if (read_overlaps(wd, vr, blocks))
						{
							...
							for (auto & el : blocks)
							{
								...
								_m_paste_children(el.window, false, req_refresh_children, el.r, graph, point{});
							}
						}
					}
			...
			void window_layout::paste_children_to_graphics(core_window_t* wd, graphics& graph)
			{
			...
			bool window_layout::read_visual_rectangle(core_window_t* wd, rectangle& visual)
			{
			...
			bool window_layout::read_overlaps(core_window_t* wd, const rectangle& vis_rect, vector<wd_rectangle>& blocks)
			{
				auto const is_wd_root = (category::flags::root == wd->other.category);
				wd_rectangle block;
				while (wd->parent)
				{
					auto i = find(wd->parent->children.cbegin(), wd->parent->children.cend(), wd);
					if (i != wd->parent->children.cend())
					{
						for (++i; i != wd->parent->children.cend(); ++i)
						{
							...
							if (is_wd_root ? 
								...
								:
								((category::flags::root != cover->other.category) && (NULL == cover->effect.bground)))
							{
			...
			bool window_layout::enable_effects_bground(core_window_t * wd, bool enabled)
			{
				...
				if (false == enabled)
				{
					...
					wd->effect.bground = NULL;
					wd->effect.bground_fade_rate = 0;
				}

				//Find the window whether it is registered for the bground effects
				auto i = find(data_sect.effects_bground_windows.begin(), data_sect.effects_bground_windows.end(), wd);
			...
			void window_layout::make_bground(core_window_t* const wd)
			{
				...
				if (category::flags::lite_widget == wd->parent->other.category)
				{
					...
					glass_buffer.bitblt(rectangle{ wd->dimension }, beg->drawer.graphics, wd->pos_root - beg->pos_root);
					
					rectangle r(wd->pos_owner, wd->dimension);
						...
						for (auto child : pre->children)
						{
							...
							if (child->visible && overlap(r, rectangle(child->pos_owner, child->dimension), ovlp))
							{
								if (category::flags::lite_widget != child->other.category)
									glass_buffer.bitblt(rectangle(ovlp.x - pre->pos_owner.x, ovlp.y - pre->pos_owner.y, ovlp.width, ovlp.height), child->drawer.graphics, point(ovlp.x - child->pos_owner.x, ovlp.y - child->pos_owner.y));
					...
					glass_buffer.bitblt(rectangle{ wd->dimension }, wd->parent->drawer.graphics, wd->pos_owner);
				...
				for (auto child : wd->parent->children)
				{
					...
					{
						if (category::flags::lite_widget != child->other.category)
							glass_buffer.bitblt(rectangle{ ovlp.x - wd->pos_owner.x, ovlp.y - wd->pos_owner.y, ovlp.width, ovlp.height }, child->drawer.graphics, {ovlp.position() - child->pos_owner});
			...
			void window_layout::_m_paste_children(core_window_t* wd, bool have_refreshed, bool req_refresh_children, const rectangle& parent_rect, graphics& graph, const point& graph_rpos)
			{
				rectangle rect;
				for (auto child : wd->children)
				{
					...
					if (NULL == child->effect.bground)
					{
						if (overlap(rectangle{ child->pos_root, child->dimension }, parent_rect, rect))
						{
							if (category::flags::lite_widget != child->other.category)
							{
								...
								graph.bitblt(rectangle(rect.x - graph_rpos.x, rect.y - graph_rpos.y, rect.width, rect.height),
									child->drawer.graphics, point(rect.x - child->pos_root.x, rect.y - child->pos_root.y));
							}
			...
			void window_layout::_m_paint_glass_window(core_window_t* wd, bool is_redraw, bool is_child_refreshed, bool called_by_notify, bool notify_other)
			{
				...
				if (read_visual_rectangle(wd, vr))
				{
					...
					//Map root
					root_graph.bitblt(vr, wd->drawer.graphics, point(vr.x - wd->pos_root.x, vr.y - wd->pos_root.y));
					_m_paste_children(wd, is_child_refreshed, (is_redraw || called_by_notify), vr, root_graph, point());
					...
					if (wd->parent)
					{
						...
						for (auto & n : blocks)
						{
							root_graph.bitblt(n.r, (n.window->drawer.graphics), point(n.r.x - n.window->pos_root.x, n.r.y - n.window->pos_root.y));
						}
					}
			...
			{
				rectangle r_of_sigwd(sigwd->pos_root, sigwd->dimension);
				for (auto wd : data_sect.effects_bground_windows)
				{
					if (wd == sigwd || !wd->displayed() ||
						(false == overlapped(rectangle{ wd->pos_root, wd->dimension }, r_of_sigwd)))
						continue;
