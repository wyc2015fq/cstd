...
namespace path
{
	...
	struct creator<tasks::user_temporary>
	{
		static wstring fetch()
		{
			...
			return wstring(temp).substr(0,  wcslen(temp) - 1);
		}
	...
	struct creator<tasks::wer_user_archive>
	{
		static wstring fetch()
		{
			return reg_string(HKEY_LOCAL_MACHINE, 
...
class junk_sweeper
{
	...
	size_t bytes_{};
	size_t size_ {};
	vector<deque<pair<wstring, bool> > > files_;

	using self_type = junk_sweeper ;

 public:
	 using file_iterator = directory_iterator; // ::file_iterator file_iterator;
	...
	void clear(messenger& msnger)
	{
		...
		for (auto &temp_dir : files_ )
			for(auto &file : temp_dir)
			{
				...
				if (file.second)
				{
					// rmdir(file.first.c_(L), true);
					wcout << (L"Deleting directory: ") << file.first << (L"\n");
				}
				else
				{
					// rmfile(file.first.c_(L));
					wcout << (L"Deleting file: ") << file.first << (L"\n");
				}
 ...
 private:
	 ...
	 public:
		...
		void operator()(const file_info_type& v)
		{
			...
			if(v.directory)
			{
				...
				//for_each<file_iterator, helper&>(file_iterator(hlp.path), file_iterator(), hlp);
				bytes += hlp.bytes;
				size += hlp.size;
			...
			files.push_back(make_pair(file, v.directory));
			msnger_.active();
	...
	struct implment
	{
		static void job(junk_sweeper& js, messenger& msnger)
		{
			...
			for_each<file_iterator, helper&>(file_iterator(path), file_iterator(), hlp);
...
class frm_main: public form
{
	...
	button   scan_btn_{ *this, (L"Scan junk files") };	 
	progress	pgbar_	{ *this };							 
	label		lbl_	{ *this, (L"Nana C++ Library") };  
	picture	pic_	{ *this };							 
...
public:
	typedef appear appear;
	
	frm_main()
		: form(API::make_center(400, 350), appear::decorate<appear::minimize, appear::taskbar>())
	{
...
private:
	class pgbar_analysis_msnger: public messenger
	{
		...
		label    &lbl_;
	public:
		pgbar_analysis_msnger(progress& pgbar, label& lbl)
	...
	class pgbar_delete_msnger: public messenger
	{
		...
		label&	lbl_;
	public:
		pgbar_delete_msnger(progress& pgbar, label& lbl)
			:pgbar_(pgbar), lbl_(lbl)
		{}

		void active()
		{
			//threads::thread::check_break(0);
			pgbar_.inc();
		}
...
private:
	void _m_button()
	{
		...
		else
		{
            ...
            thread ([this](){_m_search();}).detach();
		}
	}

	static string cap(uintmax_t bytes)
	{
		...
		string::size_type dotpos = cap.find('.');
		if(dotpos != cap.npos)
		{
			string::size_type spare = cap.size() - dotpos - 1;
			if(spare > 2)
			{
				spare = 2;
				cap.erase(dotpos + 2);
			...
			for(string::size_type i = 0; i < spare; ++i)
			{
				if(cap[cap.size() - 1] == '0')
	...
	void _m_search()
	{
		...
		lbl_.caption(wstring(charset(ss.(L))));
	}
