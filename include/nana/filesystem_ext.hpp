...
public:
	...
	directory_only_iterator(Arg&& arg, Args&&... args) : directory_iterator(arg, forward<Args>(args)...)
	{
		find_first();
	}
...
class regular_file_only_iterator : public directory_iterator
{
	using directory_iterator = directory_iterator;
...
public:
	...
	template <typename Arg, typename... Args>
	regular_file_only_iterator(Arg&& arg, Args&&... args) : directory_iterator(forward<Arg>(arg), forward<Args>(args)...)
	{
		find_first();
	}
...
string pretty_file_size(const path& path);

string pretty_file_date(const path& path);
