...
namespace detail
{
	...
	struct native_interface
	{
		...
		static void affinity_execute(native_window_type, const function<void()>&);
		...
		static bool window_icon(native_window_type, const image& big_icon, const image& small_icon);
		...
		static point	window_position(native_window_type);
		...
		//For Caret
		static void caret_create(native_window_type, const size&);
		static void caret_destroy(native_window_type);
		static void caret_pos(native_window_type, const point&);
		...
		static bool calc_screen_point(native_window_type, point&);
		static bool calc_window_point(native_window_type, point&);

		static native_window_type find_window(int x, int y);
		static size check_track_size(size sz, unsigned extra_width, unsigned extra_height, bool true_for_max);
	};
