...
namespace detail
{
	...
	struct element_store::data
	{
		...
		element::element_interface * fast_ptr{ NULL };
	};
	...
	element::element_interface * const * element_store::bground(const char* name)
	{
		element_interface * const * addr = &(impl_->table[name].fast_ptr);
		return addr;
	}

	void element_store::bground(const char* name, const pat::cloneable<element_interface>& rhs)
	{
	...
	void element_store::bground(const char* name, pat::cloneable<element_interface>&& rv)
	{
		auto & store = impl_->table[name];

		store.entity = move(rv);
		store.fast_ptr = store.entity.get();
	}
