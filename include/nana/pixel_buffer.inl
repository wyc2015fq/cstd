...
{
	rectangle valid_rectangle(const size& s, const rectangle& r)
	{
		rectangle good_r;
		overlap(rectangle{ s }, r, good_r);
		return good_r;
	}
		...
		: private noncopyable
	{
		...
		pixel_buffer_storage& operator=(const pixel_buffer_storage&) {}
		...
		bool _m_alloc()
		{
			...
			unique_ptr<pixel_color_t[]> pxbuf{ new pixel_color_t[pixel_size.width * pixel_size.height] };
#if defined(NANA_X11)
	...
	public:
		...
		const rectangle valid_r;
		const size pixel_size;
		pixel_color_t * raw_pixel_buffer{ NULL };
		...
		{
			image_process::stretch_interface * stretch_receptacle{NULL};
			image_process::stretch_interface * const * stretch;
			image_process::alpha_blend_interface * alpha_blend_receptacle{NULL};
			image_process::alpha_blend_interface * const * alpha_blend;
			image_process::blend_interface * blend_receptacle{NULL};
			image_process::blend_interface * const * blend;
			image_process::line_interface * line_receptacle{NULL};
			image_process::line_interface * const * line;
			image_process::blur_interface * blur_receptacle{NULL};
			image_process::blur_interface * const * blur;

			image_processor_tag()
			{
				auto & provider = image_process_provider::instance();
		...
		pixel_buffer_storage(drawable_type drawable, const rectangle& want_r)
			:	drawable(drawable),
				valid_r(valid_rectangle(drawable_size(drawable), want_r)),
		...
		void assign(const unsigned char* rawbits, size_t width, size_t height, size_t bits_per_pixel, size_t bytes_per_line, bool is_negative)
		{
			if (!raw_pixel_buffer)
				return;
			...
			if(32 == bits_per_pixel)
			{
				...
				{
					size_t line_bytes = (pixel_size.width < width ? pixel_size.width : width) * sizeof(pixel_color_t);
		...
		void put(Drawable dw, GC gc, int src_x, int src_y, int x, int y, unsigned width, unsigned height)
		{
			...
			else if(16 == depth)
			{
				...
				unique_ptr<unsigned short[]> table_5_6bit_holder(new unsigned short[512]);
	...
	pixel_buffer::pixel_buffer(drawable_type drawable, const rectangle& want_rectangle)
	{
	...
	{
		open(drawable, rectangle(0, static_cast<int>(top), 0, static_cast<unsigned>(lines)));
	}
	...
	void pixel_buffer::attach(drawable_type drawable, const rectangle& want_r)
	{
		storage_.reset();
		if(drawable)
		{
			rectangle r;
			if (overlap(rectangle{ drawable_size(drawable) }, want_r, r))
				storage_ = make_shared<pixel_buffer_storage>(drawable, r);
		}
	}

	bool pixel_buffer::open(drawable_type drawable)
	{
		size sz = drawable_size(drawable);
		if(sz.empty()) return false;

#if defined(NANA_WINDOWS)
		auto * sp = storage_.get();
		if((NULL == sp) || (sp->pixel_size != sz) || sp->drawable/*attached*/)
		{
			storage_ = make_shared<pixel_buffer_storage>(sz.width, sz.height);
			sp = storage_.get();
		}
		...
		size_t read_lines = ::GetDIBits(drawable->context, drawable->pixmap, 0, static_cast<UINT>(sz.height), sp->raw_pixel_buffer, &bmpinfo, DIB_RGB_COLORS);

		return (sz.height == read_lines);
#elif defined(NANA_X11)
		try
		{
			storage_ = make_shared<pixel_buffer_storage>(drawable, rectangle{sz});
			storage_->detach();
			return true;
		}
		catch(...)
	...
	bool pixel_buffer::open(drawable_type drawable, const rectangle & want_rectangle)
	{
		auto sz = drawable_size(drawable);
		...
		rectangle r;
		if (false == overlap(rectangle{ sz }, want_r, r))
			return false;
#if defined(NANA_WINDOWS)
		BITMAPINFO bmpinfo;
		...
		storage_ = make_shared<pixel_buffer_storage>(want_r.width, want_r.height);
		size_t read_lines = ::GetDIBits(context, pixmap, 0, static_cast<UINT>(want_r.height), storage_->raw_pixel_buffer, &bmpinfo, DIB_RGB_COLORS);
		...
		{
			//The format of Xorg 16bits depth is 565
			unique_ptr<unsigned[]> table_holder{new unsigned[96]};
	...
	bool pixel_buffer::open(size_t width, size_t height)
	{
		if(width && height)
		{
			storage_ = make_shared<pixel_buffer_storage>(width, height);
			return true;
		}
		return false;
	}
	...
	{
		return (NULL == storage_);
	}

	pixel_buffer::operator unspecified_bool_t() const
	{
		return (storage_ ? &pixel_buffer::empty : NULL);
	}
	...
	size_t pixel_buffer::bytes() const
	{
		auto sp = storage_.get();
		if(sp)
			return sizeof(pixel_color_t) * (static_cast<size_t>(sp->pixel_size.width) * static_cast<size_t>(sp->pixel_size.height));
		return 0;
	}

	size_t pixel_buffer::bytes_per_line() const
	{
		return (storage_ ? storage_->bytes_per_line : 0);
	}

	size pixel_buffer::size() const
	{
		return (storage_ ? storage_->pixel_size : size());
	}
	...
	pixel_color_t * pixel_buffer::raw_ptr(size_t row) const
	{
		auto sp = storage_.get();
	...
	pixel_color_t * pixel_buffer::operator[](size_t row) const
	{
		auto sp = storage_.get();
		return reinterpret_cast<pixel_color_t*>(reinterpret_cast<char*>(sp->raw_pixel_buffer) + sp->bytes_per_line * row);
	}

	void pixel_buffer::fill_row(size_t row, const unsigned char* buffer, size_t bytes, unsigned bits_per_pixel)
	{
		auto sp = storage_.get();
		if (!sp)
			return;
		...
		if (32 == bits_per_pixel)
		{
			memcpy(row_ptr, buffer, px_count * 4);
		}
		else if (24 == bits_per_pixel)
		{
	...
	{
		if(storage_)
			paste(rectangle(storage_->pixel_size), drawable, p_dst);
	}

	void pixel_buffer::paste(const rectangle& src_r, drawable_type drawable, const point& p_dst) const
	{
		auto sp = storage_.get();
		if(drawable && sp)
		{
			if(sp->alpha_channel)
			{
				rectangle s_good_r, d_good_r;
				if(overlap(src_r, sp->pixel_size, rectangle(p_dst.x, p_dst.y, src_r.width, src_r.height), drawable_size(drawable), s_good_r, d_good_r))
				{
					pixel_buffer d_pixbuf;
					d_pixbuf.attach(drawable, d_good_r);
					(*(sp->img_pro.alpha_blend))->process(*this, s_good_r, d_pixbuf, point(d_good_r.x, d_good_r.y));
				}
				return;
			}
#if defined(NANA_WINDOWS)
	...
	void pixel_buffer::line(const char* name)
	{
		if (storage_ && name.size())
		{
			auto & img_pro = storage_->img_pro;
			img_pro.line_receptacle = image_process_provider::instance().ref_line(name);
			if(img_pro.line_receptacle == *image_process_provider::instance().line())
				img_pro.line = image_process_provider::instance().line();
			else
				img_pro.line = &img_pro.line_receptacle;
		}
	}

	void pixel_buffer::line(const point &pos_beg, const point &pos_end, const color& clr, double fade_rate)
	{
		auto sp = storage_.get();
		if(NULL == sp) return;
		...
		point good_pos_beg, good_pos_end;
		if(intersection(rectangle(sp->pixel_size), pos_beg, pos_end, good_pos_beg, good_pos_end))
			(*(sp->img_pro.line))->process(*this, good_pos_beg, good_pos_end, clr, fade_rate);
	}

	void pixel_buffer::rectangle(const rectangle &r, const color& clr, double fade_rate, bool solid)
	{
		auto sp = storage_.get();
		...
		if(fade)
		{
			autoptr = alloc_fade_table(1 - fade_rate);
			fade_table = autoptr.get();
			rgb_imd.value = rgb_color;
			rgb_imd = fade_color_intermedia(rgb_imd, fade_table);
		}
		...
		if (solid)
		{
			...
			if (fade)
			{
				for (int top = ybeg; top < yend; ++top)
				{
					...
					{
						*i = fade_color_by_intermedia(*i, rgb_imd, fade_table);
					}
		...
		if((ybeg == r.y) && (r.y + static_cast<int>(r.height) == yend))
		{
			...
			if(fade)
			{
				...
				{
					*i = fade_color_by_intermedia(*i, rgb_imd, fade_table);
					*i_other = fade_color_by_intermedia(*i_other, rgb_imd, fade_table);
				}
			}
			else
			{
	...
	void pixel_buffer::gradual_rectangle(const rectangle& draw_rct, const color& from, const color& to, double /*fade_rate*/, bool vertical)
	{
		auto sp = storage_.get();
		if (NULL == sp) return;

		rectangle rct;
		if (false == overlap(rectangle(sp->pixel_size), draw_rct, rct))
			return;
	...
	//stretch
	void pixel_buffer::stretch(const char* name)
	{
		if (storage_ && name.size())
		{
			auto& img_pro = storage_->img_pro;
			auto op_default = image_process_provider::instance().stretch();
			img_pro.stretch_receptacle = image_process_provider::instance().ref_stretch(name);
	...
	void pixel_buffer::stretch(const rectangle& src_r, drawable_type drawable, const rectangle& r) const
	{
		auto sp = storage_.get();
		if(NULL == sp) return;

		rectangle good_src_r, good_dst_r;
		if(overlap(src_r, sp->pixel_size, r, drawable_size(drawable), good_src_r, good_dst_r))
		{
			pixel_buffer dst;
			dst.attach(drawable, good_dst_r);
			(*(sp->img_pro.stretch))->process(*this, good_src_r, dst, rectangle(0, 0, good_dst_r.width, good_dst_r.height));
		}
	}

	//blend
	void pixel_buffer::blend(const char* name)
	{
		if (storage_ && name.size())
		{
			auto& img_pro = storage_->img_pro;
			auto op_default = image_process_provider::instance().blend();
			img_pro.blend_receptacle = image_process_provider::instance().ref_blend(name);
	...
	void pixel_buffer::blend(const rectangle& s_r, drawable_type dw_dst, const point& d_pos, double fade_rate) const
	{
		auto sp = storage_.get();
		if(NULL == sp) return;

		rectangle s_good_r, d_good_r;
		if(overlap(s_r, sp->pixel_size, rectangle(d_pos.x, d_pos.y, s_r.width, s_r.height), drawable_size(dw_dst), s_good_r, d_good_r))
		{
			pixel_buffer d_pixbuf;
			d_pixbuf.attach(dw_dst, d_good_r);
			(*(sp->img_pro.blend))->process(*this, s_good_r, d_pixbuf, point(), fade_rate);
		}
	}

	void pixel_buffer::blur(const rectangle& r, size_t radius)
	{
		auto sp = storage_.get();
		if(NULL == sp || radius < 1)	return;

		rectangle good_r;
		if (overlap(r, rectangle{ this->size() }, good_r))
	...
	pixel_buffer pixel_buffer::rotate(double angle, const color& extend_color)
	{
		...
		size size_rotated{ sp->pixel_size };
		...
		else
		{
			...
			size_rotated.width = static_cast<unsigned>(abs(point_rotate.from(pw).x) * 2) + 1;
			size_rotated.height = static_cast<unsigned>(abs(point_rotate.from(ph).y) * 2) + 1;
		}
		...
		for (int y = 0; y < static_cast<int>(size_rotated.height); ++y)
		{
			...
			for (point point_dest{ static_cast<int>(dest.x), static_cast<int>(dest.y) }; point_dest.x < right_point; ++point_dest.x)
			{
				...
				{
					*buf = this->raw_ptr(static_cast<size_t>(point_source.y))[static_cast<int>(point_source.x)];
				}
				else
