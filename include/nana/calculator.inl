...
// workaround insufficiency in VS2013.
	...
	const string plus_minus(to_utf8(L"\u00b1")  ;   // 0xB1    u8"\261"
#else
	const string plus_minus( u8"\u00b1" );
#endif
...
void numkey_pressed(stateinfo& state, const arg_click& arg)
{
	...
	string rstr = state.result.caption();
	if(rstr == "0")	rstr.clear();

	string d = API::window_caption(arg.window_handle);
	if(d == ".")
	{
		if(rstr.find('.') == rstr.npos)
			state.result.caption(rstr.size() ? rstr + d : string("0."));
	}
	else
...
void opkey_pressed(stateinfo& state, const arg_click& arg)
{
	...
	else if("%" == d)
	{
		...
		if(s.size())
		{
			...
			state.result.caption(to_string(d));
			state.opstate = stateinfo::state::init;
		}
		return;			
	}
	...
	string oprandstr = state.result.caption();
	if(0 == oprandstr.size()) oprandstr = '0';

	string pre_operation = state.operation;
	string proc;
	if("=" != d)
	{
		state.operation = d;
		if(state.opstate != stateinfo::state::assigned)
			state.oprand = stod(oprandstr);
		else
			pre_operation = "=";
	...
	string outstr = to_string(state.outcome);
...
int main()
{
	...
	forward_list<button> op_keys;
	map<char,button*> bts;

	char keys[] = "Cm%/789X456-123+0.="; // \261
	font keyfont("", 10, true);
		...
		1, 1, [&bts, &result ]()
	{
		...
		cout << "The result of 2 + 2 is: " << result.caption() << "\n";
		int r=stoi(result.caption());
