...
namespace nana
{
	...
	namespace element
	{
		...
		{
			bool draw(graph_reference graph, const color& bgcolor, const color& fgcolor, const rectangle& r, element_state es, const data& crook_data)
			{
				if(crook_data.radio)
				{
				...
				{
					color highlighted(static_cast<color_rgb>(0x5eb6f7));
		...
		class menu_crook
			: public crook_interface
		{
			bool draw(graph_reference graph, const color&, const color& fgcolor, const rectangle& r, element_state, const data& crook_data)
			{
		...
		{
			bool draw(graph_reference graph, const color& bgcolor, const color&, const rectangle& r, element_state estate, unsigned)
			{
				...
				graph.rectangle(rectangle(r).pare_off(1), false, bgcolor);
				return true;
			}
		};
		...
		{
			bool draw(graph_reference graph, const color&, const color&, const rectangle& r, element_state, direction dir)
			{
				point pos{ r.x + 3, r.y + 3 };
				switch (dir)
				{
				case direction::east:
					pos.x += 3;
			...
			: public arrow_interface
		{
			...
			static void _m_line(graphics & graph, int x, int y, bool horizontal)
			{
				graph.set_pixel(x, y);
				if (horizontal)
				{
		...
		{
			bool draw(graph_reference graph, const color& arg_bgcolor, const color&, const rectangle& r, element_state estate)
			{
				...
				point left_top{ r.x + 1, r.y + 1 }, right_top{r.right() - 2, r.y + 1};
				point left_mid{ r.x + 1, r.y + 1 + static_cast<int>(part_px) }, right_mid{ right_top.x, left_mid.y };
				point left_bottom{ r.x + 1, r.bottom() - 2 }, right_bottom{ r.right() - 2, r.bottom() - 2 };
				...
				rectangle part_r{ r.x + 2, r.y + 2, r.width - 4, part_px };
		...
		class x_icon
			: public element_interface
		{
			bool draw(graph_reference graph, const color&, const color& fgcolor, const rectangle& r, element_state estate)
			{
				auto clr = fgcolor;
		...
		: noncopyable, nonmovable
	{
		...
		using factory_interface = pat::cloneable<element::factory_abstract>;
	public:
		~element_object()
		{
			if(factory_)
		...
		void push(const factory_interface& rhs)
		{
			...
			if(NULL == factory_ || NULL == element_ptr_)
			{
				if(element_ptr_)
					factory_->destroy(element_ptr_);
	...
	private:
		...
		element_type * element_ptr_{NULL};
		vector<pair<element_type*, factory_interface>> spare_;
	};
		...
		: noncopyable, nonmovable
	{
		...
		struct item
		{
			...
			map<string, shared_ptr<element_object<ElementInterface>>> table;
		};

		element_manager()
		{
			crook_.employee = NULL;
			border_.employee = NULL;
		}
	...
	public:
		...
		void crook(const char* name, const pat::cloneable<element::provider::factory_interface<element::crook_interface>>& factory)
		{
			_m_add(name, crook_, factory);
		}

		element::crook_interface * const * crook(const char* name) const
		{
			return _m_get(name, crook_).cite();
		}

		void cross(const char* name, const pat::cloneable<element::provider::factory_interface<element::element_interface>>& factory)
		{
			_m_add(name, cross_, factory);
		}

		element::element_interface* const * cross(const char* name) const
		{
			return _m_get(name, cross_).cite();
		}

		void border(const char* name, const pat::cloneable<element::provider::factory_interface<element::border_interface>>& factory)
		{
			_m_add(name, border_, factory);
		}

		element::border_interface * const * border(const char* name) const
		{
			return _m_get(name, border_).cite();
		}

		void arrow(const char* name, const pat::cloneable<element::provider::factory_interface<element::arrow_interface>>& factory)
		{
			_m_add((name.empty() ? "arrowhead" : name), arrow_, factory);
		}

		element::arrow_interface * const * arrow(const char* name) const
		{
			return _m_get((name.empty() ? "arrowhead" : name), arrow_).cite();
		}

		void button(const char* name, const pat::cloneable<element::provider::factory_interface<element::element_interface>>& factory)
		{
			_m_add((name.empty() ? "annex" : name), button_, factory);
		}

		element::element_interface * const * button(const char* name) const
		{
			return _m_get((name.empty() ? "annex" : name), button_).cite();
		}

		void x_icon(const char* name, const pat::cloneable<element::provider::factory_interface<element::element_interface>>& factory)
		{
			_m_add(name, x_icon_, factory);
		}

		element::element_interface * const * x_icon(const char* name) const
		{
	...
	private:
		...
		template<typename ElementInterface>
		void _m_add(const char* name, item<ElementInterface>& m, const pat::cloneable<element::provider::factory_interface<ElementInterface>>& factory)
		{
			...
			if(NULL == eop)
				eop = make_shared<element_object_t>();
		...
		template<typename ElementInterface>
		const element_object<ElementInterface>& _m_get(const char* name, const item<ElementInterface>& m) const
		{
			lock_guard lock(mutex_);
	...
	{
		//class provider
		void provider::add_crook(const char* name, const pat::cloneable<factory_interface<crook_interface>>& factory)
		{
			element_manager::instance().crook(name, factory);
		}

		crook_interface* const * provider::cite_crook(const char* name)
		{
			return element_manager::instance().crook(name);
		}

		void provider::add_cross(const char* name, const pat::cloneable<factory_interface<element_interface>>& factory)
		{
			element_manager::instance().cross(name, factory);
		}

		element_interface* const* provider::cite_cross(const char* name)
		{
			return element_manager::instance().cross(name);
		}

		void provider::add_border(const char* name, const pat::cloneable<factory_interface<border_interface>>& factory)
		{
			element_manager::instance().border(name, factory);
		}

		border_interface* const * provider::cite_border(const char* name)
		{
			return element_manager::instance().border(name);
		}

		void provider::add_arrow(const char* name, const pat::cloneable<factory_interface<arrow_interface>>& factory)
		{
			element_manager::instance().arrow(name, factory);
		}

		arrow_interface* const * provider::cite_arrow(const char* name)
		{
			return element_manager::instance().arrow(name);
		}

		void provider::add_button(const char* name, const pat::cloneable<factory_interface<element_interface>>& factory)
		{
			element_manager::instance().button(name, factory);
		}

		element_interface* const* provider::cite_button(const char* name)
		{
			return element_manager::instance().button(name);
		}

		void provider::add_x_icon(const char* name, const pat::cloneable<factory_interface<element_interface>>& factory)
		{
			element_manager::instance().x_icon(name, factory);
		}

		element_interface* const* provider::cite_x_icon(const char* name)
		{
	...
	//template<> class facade<element::crook>
		...
		bool facade<element::crook>::draw(graph_reference graph, const color& bgcol, const color& fgcol, const rectangle& r, element_state es)
		{
	...
	//class facade<element::cross>
		...
		bool facade<element::cross>::draw(graph_reference graph, const color&, const color& fgcolor, const rectangle& r, element_state)
		{
	...
	//class facade<element::border>
		...
		bool facade<element::border>::draw(graph_reference graph, const color& bgcolor, const color& fgcolor, const rectangle& r, element_state es)
		{
	...
	//class facade<element::arrow>
		...
		void facade<element::arrow>::direction(direction dir)
		{
			dir_ = dir;
		}

		//Implement element_interface
		bool facade<element::arrow>::draw(graph_reference graph, const color& bgcolor, const color& fgcolor, const rectangle& r, element_state estate)
		{
			graph.palette(false, fgcolor);
	...
	//class facade<element::button>::
		...
		//Implement element_interface
		bool facade<element::button>::draw(graph_reference graph, const color& bgcolor, const color& fgcolor, const rectangle& r, element_state estate)
		{
	...
	//class facade<element::x_icon>
		...
		//Implement element_interface
		bool facade<element::x_icon>::draw(graph_reference graph, const color& bgcolor, const color& fgcolor, const rectangle& r, element_state estate)
		{
			return (*cite_)->draw(graph, bgcolor, fgcolor, r, estate);
		}
	...
	namespace element
	{
		...
		{
			brock::instance().get_element_store().bground(name, move(obj));
		}
		...
		bool cite_bground::draw(graph_reference dst, const color& bgcolor, const color& fgcolor, const rectangle& r, element_state state)
		{
		...
		struct bground::draw_method
		{
			...
			virtual void paste(const rectangle& from_r, graph_reference, const point& dst_pos) = 0;
			virtual void stretch(const rectangle& from_r, graph_reference dst, const rectangle & to_r) = 0;
		};
			...
			: public draw_method
		{
			...
			void paste(const rectangle& from_r, graph_reference dst, const point& dst_pos)
			{
				image.paste(from_r, dst, dst_pos);
			}

			void stretch(const rectangle& from_r, graph_reference dst, const rectangle& to_r)
			{
		...
		{
			draw_method * method{ NULL };

			bool		vert{ false };
			rectangle	valid_area;
			vector<element_state> states;
			map<element_state, element_state> join;
		...
		bground& bground::image(const image& img, bool vertical, const rectangle& valid_area)
		{
			delete impl_->method;
			...
			if (valid_area.width && valid_area.height)
				...
				impl_->valid_area = rectangle(img.size());
			return *this;
		}

		bground& bground::image(const graphics& graph, bool vertical, const rectangle& valid_area)
		{
			delete impl_->method;
			...
			if (valid_area.width && valid_area.height)
				...
				impl_->valid_area = rectangle(graph.size());
			return *this;
		}

		//Set the state sequence of the background picture.
		void bground::states(const vector<element_state> & s)
		{
			impl_->states = s;
		}

		void bground::states(vector<element_state> && s)
		{
			impl_->states = move(s);
		}
		...
		bool bground::draw(graph_reference dst, const color&, const color&, const rectangle& to_r, element_state state)
		{
			...
			rectangle from_r = impl_->valid_area;
			if (impl_->vert)
			{
			...
			if (left)
			{
				...
				if (bottom)
				{
					...
					src_r.height = bottom;
					method->paste(src_r, dst, point(to_r.x, to_r.y + static_cast<int>(to_r.height - bottom)));
				}
			}
			...
			if (right)
			{
				...
				{
					src_r.height = top;
					method->paste(src_r, dst, point(to_x, to_r.y));
				}
				if (bottom)
				{
					src_r.y += (static_cast<int>(from_r.height) - static_cast<int>(bottom));
					src_r.height = bottom;
					method->paste(src_r, dst, point(to_x, to_r.y + int(to_r.height - bottom)));
				}
			}
