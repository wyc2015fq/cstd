...
namespace nana
{
	...
	//class unicode_bidi
		vector<unicode_bidi::entity> unicode_bidi::reorder(const char_type* str, size_t len)
		{
			levels_.clear();
			const char_type * const end = str + len;

			vector<remember> stack;
			...
			const char_type * begin_character = NULL;
		...
		vector<unicode_bidi::entity>::iterator unicode_bidi::_m_search_first_character()
		{
			return levels_.begin();
		}

	
		auto unicode_bidi::_m_eor(vector<entity>::iterator i) ->bidi_char
		{
		...
		void unicode_bidi::_m_reordering_resolved_levels(vector<entity> & reordered)
		{
			reordered = levels_;
			...
			for(unsigned level = highest_level; level >= 1; --level)
			{
				for(auto i = levels_.begin(); i != levels_.end(); ++i)
				{
					if(i->level >= level)
					{
						...
						for(; p < plast; ++p, --plast)
							swap(reordered[p], reordered[plast]);
	...
	vector<unicode_bidi::entity> unicode_reorder(const wchar_t* text, size_t length)
	{
