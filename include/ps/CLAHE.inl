...
#else
typedef unsigned short byte;     /* for 12 bit-per-pixel images (default) */
  ...
  for (i = 0; i < uiNrGreylevels; i++) {
    if (pulHistogram[i] > ulClipLimit) {
      ...
      if (pulHistogram[i] > ulUpper) {        /* high bin count */
        //ulNrExcess -= pulHistogram[i] - ulUpper; 应该修正为：
...
void MakeHistogram(byte* pImage, unsigned int uiXRes, int uiSizeX, int uiSizeY,
    unsigned long* pulHistogram, int uiNrGreylevels, byte* pLookupTable)
{
  byte* pImagePointer;
  int i;
...
void MapHistogram(unsigned long* pulHistogram, byte Min, byte Max,
...
void Interpolate(byte* pImage, int uiXRes, unsigned long* pulMapLU,
    unsigned long* pulMapRU, unsigned long* pulMapLB,  unsigned long* pulMapRB,
    unsigned int uiXSize, unsigned int uiYSize, byte* pLUT)
{
  ...
  if (uiNum & (uiNum - 1))   /* If uiNum is not a power of two, use division */
    for (uiYCoef = 0, uiYInvCoef = uiYSize; uiYCoef < uiYSize;
      ...
      for (uiXCoef = 0, uiXInvCoef = uiXSize; uiXCoef < uiXSize;
        ...
        *pImage++ = (byte)((uiYInvCoef * (uiXInvCoef * pulMapLU[GreyValue]
...
int CLAHE(unsigned int uiYRes, unsigned int uiXRes, byte* pImage,
    byte Min, byte Max, unsigned int uiNrX, unsigned int uiNrY,
