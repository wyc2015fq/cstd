...
int ConvertToGrayBitmap(int Width, int Height, const byte* Src, int SrcStride, byte* Dest, int DestStride)
{
  const byte* SrcP;
  byte* DestP;
  int X, Y;
  ...
  for (Y = 0; Y < Height; Y++) {
    SrcP = Src + Y * SrcStride;         // 必须在某个地方开启unsafe功能，其实C#中的unsafe很safe，搞的好吓人。
    DestP = Dest + Y * DestStride;
...
int GetHistGram(int Width, int Height, const byte* Src, int SrcStride, int* HistGram)
{
  byte* SrcP;
  int X, Y;
...
int GetHuangFuzzyThreshold1(int* HistGram, int HistGram_Length)
{
  int X, Y;
  int First, Last;
  int Threshold = -1;
  ...
  if (First == Last) {
    return First;  // 图像中只有一个颜色
  }

  if (First + 1 == Last) {
    return First;  // 图像中只有二个颜色
  }
  ...
  // 建立公式（4）及（6）所用的查找表
  Smu_Length = Last + 1 - First;
  Smu = MALLOC(double, Smu_Length);
  ...
  for (Y = First; Y <= Last; Y++) {
    int mu = round((double)W[Y] / S[Y]);             // 公式17
    Entropy = 0;
    ...
    mu = round((double)(W[Last] - W[Y]) / (S[Last] - S[Y]));  // 公式18
...
int Binaryzation(int Width, int Height, const byte* Src, int SrcStride, byte* Dest, int DestStride)
{
  const byte* SrcP;
  byte* DestP;
  int HistGram[256];
  ...
  GetHistGram(Width, Height, Src, SrcStride, HistGram);
  Threshold = GetHuangFuzzyThreshold1(HistGram, 256);

  for (Y = 0; Y < Height; Y++) {
    SrcP = Src + Y * SrcStride;         // 必须在某个地方开启unsafe功能，其实C#中的unsafe很safe，搞的好吓人。
    DestP = Dest + Y * DestStride;

    for (X = 0; X < Width; X++, SrcP++, DestP++) {
      *DestP = *SrcP > Threshold ? 255 : 0;  // 写成255和0，C#编译器不认。
    }
  }
...
#define BlockCopy(a, b, c, d, e)   memcpy(c, a, e);
  ...
  while (IsDimodal(HistGramCC) == false) {                                      // 判断是否已经是双峰的图像了
    ...
    BlockCopy(HistGramCC, 0, HistGramC, 0, 256 * sizeof(double));
    Iter++;
...
static int GetIntermodesThreshold(int* HistGram, int* HistGramS)
{
  ...
  while (IsDimodal(HistGramCC) == false) {                                                // 判断是否已经是双峰的图像了
    ...
    HistGramCC[255] = (HistGramC[254] + HistGramC[255] + HistGramC[255]) / 3;           // 最后一点
    BlockCopy(HistGramCC, 0, HistGramC, 0, 256 * sizeof(double));         // 备份数据，为下一次迭代做准备
    Iter++;
...
/// </summary>
/// <param name="HistGram">灰度图像的直方图</param>
/// <param name="Tile">背景在图像中所占的面积百分比 = 50</param>
/// <returns></returns>
...
static int Get1DMaxEntropyThreshold(int* HistGram)
{
  ...
  for (Y = MinValue + 1; Y < MaxValue; Y++) {
    ...
    for (X = MinValue; X <= Y; X++) {
      EntropyBack += (-HistGramD[X] / SumIntegral * log(HistGramD[X] / SumIntegral));
    }

    EntropyFore = 0;

    for (X = Y + 1; X <= MaxValue; X++) {
      EntropyFore += (-HistGramD[X] / (1 - SumIntegral) * log(HistGramD[X] / (1 - SumIntegral)));
    }
...
//   W. Tsai, "Moment-preserving thresholding: a MALLOC(approach," Computer
...
static int GetShanbhagThreshold(int* HistGram, int HistGram_Length)
{
  ...
  double* norm_histo = MALLOC(double, HistGram_Length); /* normalized histogram */
  double* P1 = MALLOC(double, HistGram_Length); /* cumulative normalized histogram */
  double* P2 = MALLOC(double, HistGram_Length);
...
static int GetYenThreshold(int* HistGram, int HistGram_Length)
{
  ...
  double* norm_histo = MALLOC(double, HistGram_Length); /* normalized histogram */
  double* P1 = MALLOC(double, HistGram_Length); /* cumulative normalized histogram */
  double* P1_sq = MALLOC(double, HistGram_Length);
  double* P2_sq = MALLOC(double, HistGram_Length);
...
{
#define casedef(x, y)
  casedef("灰度平均值", GetMeanThreshold(HistGram));
  casedef("黄式模糊阈值", GetHuangFuzzyThreshold(HistGram));
  casedef("谷底最小值", GetMinimumThreshold(HistGram,  HistGramS));
  casedef("双峰平均值", GetIntermodesThreshold(HistGram,  HistGramS));
  casedef("百分比阈值", GetPTileThreshold(HistGram));
  casedef("迭代阈值法", GetIterativeBestThreshold(HistGram));
  casedef("大津法", GetOSTUThreshold(HistGram));
  casedef("一维最大熵", Get1DMaxEntropyThreshold(HistGram));
  casedef("动能保持", GetMomentPreservingThreshold(HistGram));
  casedef("Kittler最小错误", GetKittlerMinError(HistGram));
  casedef("ISODATA法", GetIsoDataThreshold(HistGram));
  casedef("Shanbhag法", GetShanbhagThreshold(HistGram));
  casedef("Yen法", GetYenThreshold(HistGram));
  return 0;
}

int DrawHistGram(int Width, int Height, byte* Src, int SrcStride, int* Histgram, int Thr)
{
  int X, Y, Max = 0;
  byte* P;
