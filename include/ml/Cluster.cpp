...
/***********************************************************
*函数名称    CalFeature(Pattern *m_pattern)
*参数        Pattern *m_pattern
*返回值      void
...
void GetFeature(int height, int wide, BYTE* m_pData,
  ...
  for (i = 0; i < patternnum; i++) {//=patternnum
    CalFeature(height, wide, m_pData, &m_pattern[i]);//调用函数计算第i号模板的值
  }
...
/***********************************************************
*函数名称    Distance(const double* pattern1, const double*  pattern2, const int distype)
*参数        const double* pattern1   样品1
*            const double* pattern2   样品2
...
/***********************************************************
*函数名称    GetDistance( Pattern pattern1, Pattern  pattern2, const int distype)
*参数        Pattern pattern1   样品1
*            Pattern pattern2   样品2
*            const int distype           距离模式  1：欧氏距离；2：夹角余弦距离；
*            3：特征是二值时的夹角余弦距离；4：具有二值特征的Tanimoto测度
*返回值      double
*函数功能    计算样品1和样品2间的距离，距离模式由参数distype给定
...
/***************************************************************
*函数名称    GetDistance(Pattern pattern, Center center, const int distype)
*参数        Pattern pattern  样品
*            Center center    中心
*            const int distype           距离模式  1：欧氏距离；2：夹角余弦距离；
*                             3：特征是二值时的夹角余弦距离；
*                             4：具有二值特征的Tanimoto测度
...
/*************************************************************
*函数名称    GetDistance(Center mCenter1, Center mCenter2, int distype)
*参数        Center mCenter1   中心1
*            Center mCenter2   中心2
*            const int distype           距离模式   1：欧氏距离；2：夹角余弦距离；
*                            3：特征是二值时的夹角余弦距离；
*                            4：具有二值特征的Tanimoto测度
...
/************************************************************
*函数名称    GetFuzzyDistance(Pattern pattern1, Pattern pattenr2, int distype)
*参数        Pattern pattern1  样品1
*            Pattern pattern2  样品2
  ...
  while (change && counter < times) {
    ...
    for (i = 0; i < patternnum; i++) {//对所有样品重新归类
      ...
      if (m_pattern[i].category == m_center[index].index) {
        ...
        CalCenter(m_pattern, patternnum, &m_center[index]);//计算新属类中心
        change = TRUE;
      }
    }
  }//end of while
