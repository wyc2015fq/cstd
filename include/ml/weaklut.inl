...
double LUT_value(double x, const LUTWEAK* lut, int i)
{
...
void get_min_map(int n, const double* f, const double* w, int sp,
    double* poswi, double* negwi, int bin, LUTWEAK* wk)
{
  int i;
...
// vi - 分类器在样本上的得分
// wi - 样本权重
int LUT_update_weight_impl(int SampNum, const double* f, double* vi, double* wi, int PosNum, double ha, LUTWEAK* wk)
{
  //self& ww = *this;
...
{
  LUTWEAK* wk = (LUTWEAK*)hh->ca->weak + hh->ca->weaklen;
  LUT_update_weight_impl(hh->samp_num, hh->fi, hh->vi, hh->wi, hh->pos_num, hh->ha, wk);
  wk->id = pwi->id;
  return 0;
}

int LUT_get_error(BOOST* hh, WORKINFO* pwi, get_feat_f getfeatfun)
{
  ...
  for (pro = 0, i = pwi->beg; i < pwi->end; ++i) {
    ...
    tmpSamp = getfeatfun(hh, i);
    get_min_map(SampNum, tmpSamp, wi, PosNum, poswi, negwi, LUT_BIN, NULL);
    Error = 0.f;
