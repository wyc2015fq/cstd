...
typedef struct tagVQ_CENTERINFO {
  double* Data;
  double* SumData;
  int nDimension;
  int Num;
...
int LBGCluster( VQ_VECTOR *X, int N, VQ_CENTER *Y, int M ) {
  VQ_CENTERINFO * Center;
  int L = 1000, m = 1, nCenter, i, j, k; //L,迭代的次数
  int nDimension = X[ 0 ].nDimension;
  double D0, D, *Distance;
  if ( N < M )
    return -1;
  Center = ( VQ_CENTERINFO* ) malloc( M * sizeof( VQ_CENTERINFO ) );
  if ( Center == NULL )
    return -1;
...
int LBGClusterCor( VQ_VECTOR *X, int N, VQ_CENTER *Y, int M ) {
  ...
  int nDimension = X[ 0 ].nDimension;
  double SumCor0, SumCor, *Correlation;
  if ( N < M )
    return -1;
  Center = ( VQ_CENTERINFO* ) malloc( M * sizeof( VQ_CENTERINFO ) );
  if ( Center == NULL )
    return -1;
  Correlation = ( double* ) malloc( N * sizeof( double ) );
  if ( Correlation == NULL )
    return -1;
...
int MaxCluster( VQ_VECTOR *X, int N, VQ_CENTER *Y, int M ) {
  VQ_CENTERINFO * Center;
  double L = 0, Dist, *Distance, *SumData;
  int m = 1, nDimension, i, j, k, l;
  if ( N < M )
    return -1;
  Center = ( VQ_CENTERINFO* ) malloc( M * sizeof( VQ_CENTERINFO ) );
  if ( Center == NULL )
  ...
  nDimension = X[ 0 ].nDimension;
  for ( i = 0; i < M; i++ ) {
    for ( k = 0; k < nDimension; k++ )
      Center[ i ].SumData[ k ] = 0;
    Center[ i ].Num = 0;
  }
  SumData = ( double* ) malloc( sizeof( double ) * nDimension );
  for ( k = 0; k < nDimension; k++ )
    SumData[ k ] = 0;
  for ( i = 0; i < N; i++ )
    for ( k = 0; k < nDimension; k++ )
