...
void gluLookAt(GLdouble eyeX, GLdouble eyeY, GLdouble eyeZ, GLdouble centerX, GLdouble centerY, GLdouble centerZ, GLdouble upX, GLdouble upY, GLdouble upZ)
{
  GLdouble directMat[16] = {0};
  V3 fvDirect, fvUpD, fvC, fvUp;
  directMat[15] = 1;
  V3_Set(fvDirect, centerX - eyeX, centerY - eyeY, centerZ - eyeZ);
  gl_V3_Norm(&fvDirect);
  V3_Set(fvUpD, upX, upY, upZ);
  gl_V3_Norm(&fvUpD);
  fvC = gl_V3_Cross(fvDirect, fvUpD);
  gl_V3_Norm(&fvC);
  fvUp = gl_V3_Cross(fvC, fvDirect);
  gl_V3_Norm(&fvUp);
  fvDirect.v[0] = -fvDirect.v[0];
  fvDirect.v[1] = -fvDirect.v[1];
  fvDirect.v[2] = -fvDirect.v[2];
  directMat[0] = fvC.v[0];
  directMat[4] = fvC.v[1];
  directMat[8] = fvC.v[2];
  directMat[1] = fvUp.v[0];
  directMat[5] = fvUp.v[1];
  directMat[9] = fvUp.v[2];
  directMat[2] = fvDirect.v[0];
  directMat[6] = fvDirect.v[1];
  directMat[10] = fvDirect.v[2];
...
void gluDisk(GLUquadric* qobj, GLdouble innerRadius, GLdouble outerRadius,
...
void gluNurbsSurface(GLUnurbs* nobj, GLint sknot_count, float* sknot,
