...
#define MYPTR(x)   (x)
    ...
    if (FAILED(IDirectInput8_CreateDevice(_glfw.win32.dinput8.api, MYREF(di->guidInstance), &device, NULL)))
    {
    ...
    data.device = device;
    data.objects = (_GLFWjoyobjectWin32*)calloc(dc.dwAxes + dc.dwButtons + dc.dwPOVs, sizeof(_GLFWjoyobjectWin32));
    ...
    js->axes = (float*)calloc(js->axisCount, sizeof(float));
    js->buttonCount += data.buttonCount + data.povCount * 4;
    js->buttons = (unsigned char*)calloc(js->buttonCount, 1);
