...
#ifndef _A20_CAP_INL_
#define _A20_CAP_INL_
//#Rockie Cheng
...
int cap_thd(void)
{
  ...
  pic_width = fmt.fmt.pix.width;
  pic_height = fmt.fmt.pix.height;
  ...
  while(1)    // while(1)
  {
    ...
    for(;;) {
      ...
      if(FD_ISSET(fd,&fds)) {
        ...
        memcpy(g_cap_buf, buffers[vbuf.index].start, buffers[vbuf.index].length);
...
int a20cap_getframe(img_t* im) {
    img_t im1[1] = {0};
    static int i=0;
    ++i;
    IMINIT(im1, pic_height, pic_width, g_cap_buf, pic_width*2, 2, 1);
    imcolorcvt(im1, im, T_YUYV, T_RGB);
    //printf("%d %d %d\n", i, im->w, im->h);
    return 0;
}
...
int a20cap_start()
{
    ...
    g_cap_buf = (char*)malloc(640*480*4);
    typedef void*(*fun_t)(void*);
    ret = pthread_create(&id1, NULL, (fun_t)cap_thd,NULL);
    if(ret)
    {
