...
void rgb2gray(_II s, _IO d, int h, int w, int ss, int ds)
{
	for (int i = 0; i < h; ++i)
	{
		for (int j = 0; j < w; ++j)
...
template<typename _II, typename _IO> inline 
void rgb2grayx(const _II *s, _IO *d, int h, int w, int ss, int ds)
{
	for (int i = 0; i < h; ++i)
	{
		for (int j = 0; j < w; ++j)
		{
			const _IO TMP_B = _IO(0.114 * *(s + i * ss + j * 3 + 0));
			const _IO TMP_G = _IO(0.587 * *(s + i * ss + j * 3 + 1));
			const _IO TMP_R = _IO(0.299 * *(s + i * ss + j * 3 + 2));
...
template<typename _T0, typename _T1> inline
...
void init(_T *s, _T v, int len)
{
	...
	for ( ; p0 < s + len; ++p0) *(p0) = v;
}

// 从大到小排序
template<typename _II> inline 
...
static inline void RegionRectify(unsigned char *pl, int col, int row, int step, unsigned int min_pts) {
	...
	for (c = 0; c < kind + 1; ++c) *(pn + c) = 0;
	for (c = 0; c < col * row; ++c) ++pn[*(pt + c)];

	for (c = 1; c < kind + 1; ++c) {
		if (pn[c] < min_pts) pn[c] = 0;
	}

	for (c = 0; c < col; ++c) {
		for (r = 0; r < row; ++r) {
			t = *(pt + c * row + r);
			if (t != 0 && pn[t] == 0) *(pl + c * step + r) = 0;
		}
	}
...
template<class _II, class _IO, class _T> inline
	void Erode(_II s, _IO d, _T h, _T w, _T ds, _T dd)
	{
		for (int i = 0; i < h; ++i)
		{
			for (int j = 0; j < w; ++j)
...
template<class _II, class _IO, class _T> inline
void Expand(_II s, _IO d, _T h, _T w, _T ds, _T dd)
{
	for (int i = 0; i < h; ++i)
	{
		for (int j = 0; j < w; ++j)
...
{
	// 判断是否需要做光照突变修正
	if (Sum(label, label + row * col, 0.0) > (0.25 * row * col)) 
	{
		double xa, xb, t[3];
		int i, j, k, p, q;
		...
		for (i = 0; i < row; ++i) 
		{
			for (j = 0; j < col; ++j) 
			{
				...
				*(itx + (i + 1) * (col + 2) + (j + 1)) = xa; *(ibx + (i + 1) * (col + 2) + (j + 1)) = xb;
			}
		}
...
void RectifyShadow(const unsigned char *pt, int step, const double *b, const double *it, const double *ib, unsigned char *label, int row, int col) {
	...
	for (i = 0; i < counter; ++i) 
	{
		...
		for (j = 0; j < 9; ++j) 
		{
			*(B + j * 9 + 0) = *(ibx + (p + j) * (col + 8) + (q + 0)); *(T + j * 9 + 0) = *(itx + (p + j) * (col + 8) + (q + 0));
			*(B + j * 9 + 1) = *(ibx + (p + j) * (col + 8) + (q + 1)); *(T + j * 9 + 1) = *(itx + (p + j) * (col + 8) + (q + 1));
			*(B + j * 9 + 2) = *(ibx + (p + j) * (col + 8) + (q + 2)); *(T + j * 9 + 2) = *(itx + (p + j) * (col + 8) + (q + 2));
			*(B + j * 9 + 3) = *(ibx + (p + j) * (col + 8) + (q + 3)); *(T + j * 9 + 3) = *(itx + (p + j) * (col + 8) + (q + 3));
			*(B + j * 9 + 4) = *(ibx + (p + j) * (col + 8) + (q + 4)); *(T + j * 9 + 4) = *(itx + (p + j) * (col + 8) + (q + 4));
			*(B + j * 9 + 5) = *(ibx + (p + j) * (col + 8) + (q + 5)); *(T + j * 9 + 5) = *(itx + (p + j) * (col + 8) + (q + 5));
			*(B + j * 9 + 6) = *(ibx + (p + j) * (col + 8) + (q + 6)); *(T + j * 9 + 6) = *(itx + (p + j) * (col + 8) + (q + 6));
			*(B + j * 9 + 7) = *(ibx + (p + j) * (col + 8) + (q + 7)); *(T + j * 9 + 7) = *(itx + (p + j) * (col + 8) + (q + 7));
			*(B + j * 9 + 8) = *(ibx + (p + j) * (col + 8) + (q + 8)); *(T + j * 9 + 8) = *(itx + (p + j) * (col + 8) + (q + 8));
		}
			...
			if (*(label + i * c + j) > 0)
			{
					 ...
					 *(src + size0 + 2) < *(b + size1 + 2))
				{
					...
					if (eb > et && ncc >= th_NCC)
					{
						...
						const double avg = Sum(TB, TB + 9, 0.0) / 9.0;
...
// 类函数定义.
void GMM::UpdateModel(const unsigned char *pt, int step, int num)
{
	...
	if (num > 1) 
	{
		for (i = 0; i < row; ++i) 
		{
			for (j = 0; j < col; ++j) 
			{
				...
				for (x = 0; x < k; ++x) 
				{
					*(ts + x * 3 + 0) = *(pt + i * step + j * 3 + 0) - *(m + i * col * kind * 3 + j * kind * 3 + x * 3 + 0);
					*(ts + x * 3 + 1) = *(pt + i * step + j * 3 + 1) - *(m + i * col * kind * 3 + j * kind * 3 + x * 3 + 1);
					*(ts + x * 3 + 2) = *(pt + i * step + j * 3 + 2) - *(m + i * col * kind * 3 + j * kind * 3 + x * 3 + 2);
				...
				if (k == 1) 
				{
					t0 = 2.5 * *(v + i * col * kind + j * kind + 0);

					if (*(t + 0) < t0 && *(t + 1) < t0 && *(t + 2) < t0) 
					{
						*(w + i * col * kind + j * kind + 0) += (afa - afa * *(w + i * col * kind + j * kind + 0));

						theta = afa / *(w + i * col * kind + j * kind + 0);

						*(m + i * col * kind * 3 + j * kind * 3 + 0) += (theta * *(ts + 0));
						*(m + i * col * kind * 3 + j * kind * 3 + 1) += (theta * *(ts + 1));
						*(m + i * col * kind * 3 + j * kind * 3 + 2) += (theta * *(ts + 2));

						t0 = *(v + i * col * kind + j * kind + 0);
						*(v + i * col * kind + j * kind + 0) = sqrt(fabs( (1 - theta) * (t0 * t0) + theta * InnerProduct(t, t+3, t, 0.0) ));
					}

					else 
					{
						*(w + i * col * kind + j * kind + 0) = (1 - afa) * *(w + i * col * kind + j * kind + 0);
						*(w + i * col * kind + j * kind + 1) = vir_w;
						*(v + i * col * kind + j * kind + 1) = vir_v;
						*(m + i * col * kind * 3 + j * kind * 3 + 3) = *(pt + i * step + j * 3 + 0);
						*(m + i * col * kind * 3 + j * kind * 3 + 4) = *(pt + i * step + j * 3 + 1);
						*(m + i * col * kind * 3 + j * kind * 3 + 5) = *(pt + i * step + j * 3 + 2);
				...
				else 
				{
					...
					for (x = 0; x < k; ++x) 
					{
						t0 = *(w + i * col * kind + j * kind + x) / *(v + i * col * kind + j * kind + x);
						*(w1 + x) = t0;
						*(w2 + x) = t0;
					}
					...
					{
						z = *(index + y);
						t0 = 2.5 * *(v + i * col * kind + j * kind + z);

						if (x == 0 && *(t + z * 3 + 0) < t0 && *(t + z * 3 + 1) < t0 && *(t + z * 3 + 2) < t0) 
						{
							x = 1;
							*(w + i * col * kind + j * kind + z) = (1 - afa) * *(w + i * col * kind + j * kind + z) + afa;
							theta = afa / *(w + i * col * kind + j * kind + z);

							*(m + i * col * kind * 3 + j * kind * 3 + z * 3 + 0) += theta * *(ts + z * 3 + 0);
							*(m + i * col * kind * 3 + j * kind * 3 + z * 3 + 1) += theta * *(ts + z * 3 + 1);
							*(m + i * col * kind * 3 + j * kind * 3 + z * 3 + 2) += theta * *(ts + z * 3 + 2);

							t0 = *(v + i * col * kind + j * kind + z);
							*(v + i * col * kind + j * kind + z) = sqrt(fabs( (1 - theta) * (t0 * t0) + theta * InnerProduct(t + z * 3, t + (z + 1) * 3, t + z * 3, 0.0) ));
						}

						else 
						{
							*(w + i * col * kind + j * kind + z) = (1 - afa) * *(w + i * col * kind + j * kind + z);
						}
					}
					...
					if (x == 0)
					{
						if (k == 3) 
						{
							...
							*(w + i * col * kind + j * kind + z) = vir_w;
							*(v + i * col * kind + j * kind + z) = vir_v;
							*(m + i * col * kind * 3 + j * kind * 3 + z * 3 + 0) = *(pt + i * step + j * 3 + 0);
							*(m + i * col * kind * 3 + j * kind * 3 + z * 3 + 1) = *(pt + i * step + j * 3 + 1);
							*(m + i * col * kind * 3 + j * kind * 3 + z * 3 + 2) = *(pt + i * step + j * 3 + 2);
						}

						else if (k == 2) 
						{
							*(w + i * col * kind + j * kind + 2) = vir_w;
							*(v + i * col * kind + j * kind + 2) = vir_v;
							*(m + i * col * kind * 3 + j * kind * 3 + 6) = *(pt + i * step + j * 3 + 0);
							*(m + i * col * kind * 3 + j * kind * 3 + 7) = *(pt + i * step + j * 3 + 1);
							*(m + i * col * kind * 3 + j * kind * 3 + 8) = *(pt + i * step + j * 3 + 2);
							*(s + i * col + j) = 3;
						}
					}
				}
				...
				// 权重归一
				t0 = Sum(w + i * col * kind + j * kind, w + i * col * kind + (j + 1) * kind, 0.0);
				*(w + i * col * kind + j * kind + 0) /= t0;
				*(w + i * col * kind + j * kind + 1) /= t0;
				*(w + i * col * kind + j * kind + 2) /= t0;
			}
		}
	}
	...
	else 
	{
		...
		for (i = 0; i < row; ++i) 
		{
			for (j = 0; j < col; ++j) 
			{
				*(w + i * col * kind + j * kind) = 1.0;
				*(v + i * col * kind + j * kind) = vir_v;

				*(m + i * col * kind * 3 + j * kind * 3 + 0) = *(pt + i * step + j * 3 + 0);
				*(m + i * col * kind * 3 + j * kind * 3 + 1) = *(pt + i * step + j * 3 + 1);
				*(m + i * col * kind * 3 + j * kind * 3 + 2) = *(pt + i * step + j * 3 + 2);
			...
			for (j = 0; j < col; ++j) 
			{
				*(label + i * col + j) = 0;
				...
				if (pmask == 0 || *(pmask + i * col + j) != 0) 
				{
					...
					{
						{
							*(ts + x * 3 + 0) = rgb0 - *(m + i * col * kind * 3 + j * kind * 3 + x * 3 + 0);
							*(ts + x * 3 + 1) = rgb1 - *(m + i * col * kind * 3 + j * kind * 3 + x * 3 + 1);
							*(ts + x * 3 + 2) = rgb2 - *(m + i * col * kind * 3 + j * kind * 3 + x * 3 + 2);
						}
						{
							*(t + x * 3 + 0) = fabs(*(ts + x * 3 + 0));
							*(t + x * 3 + 1) = fabs(*(ts + x * 3 + 1));
						...
						t0 = 2.5 * *(v + i * col * kind + j * kind + x);

						if (r != 1 && *(t + x * 3 + 0) < t0 && *(t + x * 3 + 1) < t0 && *(t + x * 3 + 2) < t0) r = 1;
					}
					...
					// get index end
					sum = *(w + i * col * kind + j * kind + *(index + 0)); 
					z = 1;

					while (sum < thrd && z < k) sum += *(w + i * col * kind + j * kind + *(index + z++));
					
					for (x = 0, y = 0; x < z; ++x) 
					{
						t0 = 2.5 * *(v + i * col * kind + j * kind + *(index + x));

						if (*(t + *(index + x) * 3 + 0) > t0 || *(t + *(index + x) * 3 + 1) > t0 || *(t + *(index + x) * 3 + 2) > t0) ++y;
					}

					if (y == z) *(label + i * col + j) = 1;
					...
					if (r == 1) 
					{
						for (z = 0, x = 0; x < k; ++x) 
						{
							t0 = *(v + i * col * kind + j * kind + *(index + x)) * 2.5;
							
							if (z == 0 && *(t + *(index + x) * 3 + 0) < t0 && *(t + *(index + x) * 3 + 1) < t0 && *(t + *(index + x) * 3 + 2) < t0) 
							{
								z = 1;

								*(w + i * col * kind + j * kind + *(index + x)) = (1 - afa) * *(w + i * col * kind + j * kind + *(index + x)) + afa;
								
								theta = afa / *(w + i * col * kind + j * kind + *(index + x));
								
								*(m + i * col * kind * 3 + j * kind * 3 + *(index + x) * 3 + 0) += (theta * *(ts + *(index + x) * 3 + 0));
								*(m + i * col * kind * 3 + j * kind * 3 + *(index + x) * 3 + 1) += (theta * *(ts + *(index + x) * 3 + 1));
								*(m + i * col * kind * 3 + j * kind * 3 + *(index + x) * 3 + 2) += (theta * *(ts + *(index + x) * 3 + 2));
								
								t0 = *(v + i * col * kind + j * kind + *(index + x));
								*(v + i * col * kind + j * kind + *(index + x)) = sqrt(fabs( (1 - theta) * (t0 * t0) + theta * InnerProduct(t + *(index + x) * 3, t + (1 + *(index + x)) * 3, t + *(index + x) * 3, 0.0) ));
							}

							else 
							{ 
								*(w + i * col * kind + j * kind + *(index + x)) = (1 - afa) * *(w + i * col * kind + j * kind + *(index + x)); 
							}
						}
					}
					...
					else 
					{
						if (k == 3) 
						{
							...
							*(w + i * col * kind + j * kind + z) = vir_w;
							*(v + i * col * kind + j * kind + z) = vir_v;
							*(m + i * col * kind * 3 + j * kind * 3 + z * 3 + 0) = rgb0;
							*(m + i * col * kind * 3 + j * kind * 3 + z * 3 + 1) = rgb1;
							*(m + i * col * kind * 3 + j * kind * 3 + z * 3 + 2) = rgb2;
						}
						...
						else 
						{
							if (k == 2) 
							{
								*(w + i * col * kind + j * kind + 2) = vir_w;
								*(v + i * col * kind + j * kind + 2) = vir_v;
								*(m + i  *col * kind * 3 + j * kind * 3 + 6) = rgb0;
								*(m + i  *col * kind * 3 + j * kind * 3 + 7) = rgb1;
								*(m + i  *col * kind * 3 + j * kind * 3 + 8) = rgb2;
								*(s + i * col + j) = 3;
							}
							...
							else 
							{
								*(w + i * col * kind + j * kind + 1) = vir_w;
								*(v + i * col * kind + j * kind + 1) = vir_v;
								*(m + i * col * kind * 3 + j * kind * 3 + 3) = rgb0;
								*(m + i * col * kind * 3 + j * kind * 3 + 4) = rgb1;
								*(m + i * col * kind * 3 + j * kind * 3 + 5) = rgb2;
								*(s + i * col + j) = 2;
							}
						}
					}
...
void GMM::RectifyShape()
{
	...
	delete []tl; tl = NULL;
	delete []pt; pt = NULL;
	delete []label0; label0 = NULL;
}

int GMM::RectLabel() 
{
	return region(label, blob, row, col, col);
}
