	...
	for (int i = 0; i < m_CurObjNum; ++i)
	{
		for (int j = 0; j < num; ++j)
		{
			if (1 == *(norm + i * num + j))
			{
				...
				// 出现在合并或分裂情况中的轨迹.
				else if (COMBINE == pt->s || SPLIT == pt->s)
				{
					pt->s = NORMAL;
					pt->score = 1;
				}
				...
				// 方向信息(总在质心信息更新之前).
				pt->direction = MoveDirection(pt->rect.cx, pt->rect.cy, *(center + j * 2 + 1), *(center + j * 2 + 0));

				// 质心信息.
				pt->rect.cy = *(center + j * 2 + 0);
				pt->rect.cx = *(center + j * 2 + 1);
	...
	for (int i = 0; i < m_CurObjNum; ++i)
	{
		if (1 < *(split + i * (num + 1) + 0))
		{
			...
			else 
			{
				...
				if ((SPLIT == pt->s && SPLT_CONFIRM > ++(pt->score) && pt->attri != VEHICLE) || NORMAL == pt->s || COMBINE == pt->s)
				{
					// 正常和合并状态处理.
					if (SPLIT != pt->s)
					{
					...
					// 方向信息.
					pt->direction = MoveDirection(pt->rect.cx, pt->rect.cy, *(tmpBlock.GetCentroid() + 1), *(tmpBlock.GetCentroid() + 0));

					// 质心信息.
					pt->rect.cy = *(tmpBlock.GetCentroid() + 0);
					pt->rect.cx = *(tmpBlock.GetCentroid() + 1);
				              ...
				              int num)
{
	...
	for (int i = 0; i < num; ++i)
	{
		...
		if (1 < *(combine + i * (m_CurObjNum + 1) + 0))
		{
			...
			for (j = 0; j < *(combine + i * (m_CurObjNum + 1) + 0); ++j)
			{
				...
				{
					// 处于场景边缘的轨迹状态置为删除.
					if (TraceOnBorder(pt, EDGE_CONFIRM) > 0 || (pt->s == EMERGE && pt->score < COMB_NEW_THD))
					{
						pt->s = DELETE;
					}
					...
					else 
					{
						...
						// 方向信息.
						pt->direction = MoveDirection(pt->rect.cx, pt->rect.cy, pt->rect.cx + offsetX, pt->rect.cy + offsetY);
	...
	delete []tmpBin; tmpBin = NULL;
	delete []tmpRGB; tmpRGB = NULL;
	delete []rgb; rgb = NULL;
}
...
void TriDimTracker::MultiTrack(const uchar *src, const unsigned int *block, int num, const Calibration *pCbr, const TriDimModel *pTDM)
{
	...
	// 内存释放.
	delete []relation; relation = NULL;
	delete []map; map = NULL;
	delete []pHeight; pHeight = NULL;
	delete []compact; compact = NULL;
	delete []pFeetWP; pFeetWP = NULL;
	delete []pFeetIP; pFeetIP = NULL;
