...
namespace AForge.Genetic
{
    ...
    public class GPTreeChromosome : ChromosomeBase
    {
        ...
        /// </remarks>
        ///
        static int MaxInitialLevel
        {
            get { return maxInitialLevel; }
            set { maxInitialLevel = MAX( 1, MIN( 25, value ) ); }
        }
        ...
        {
            get { return maxLevel; }
            set { maxLevel = MAX( 1, MIN( 50, value ) ); }
        }
        ...
        public override void Generate( )
        {
            ...
            if ( root.Gene.ArgumentsCount != 0 )
            {
                ...
                for (i = 0; i < root.Gene.ArgumentsCount; i++ )
                {
                    // create new child
        ...
        protected void Generate( GPTreeNode node, int level )
        {
            ...
            if ( node.Gene.ArgumentsCount != 0 )
            {
                ...
                for (i = 0; i < node.Gene.ArgumentsCount; i++ )
                {
                    // create new child
                    GPTreeNode child = new GPTreeNode( );
                    ...
                    else
                    {
                        ...
                        if ( node.Children.Count != node.Gene.ArgumentsCount )
                        {
                            ...
                            else
                            {
                                ...
                                for (i = node.Children.Count; i < node.Gene.ArgumentsCount; i++ )
                                {
                                    // create new child
                                    GPTreeNode child = new GPTreeNode( );
