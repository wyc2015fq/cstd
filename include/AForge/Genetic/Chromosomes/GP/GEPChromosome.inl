...
namespace AForge.Genetic
{
    ...
    public class GEPChromosome : ChromosomeBase
    {
        ...
        protected void TransposeIS( )
        {
            ...
            int transposonLength = rand.Next( MIN( maxTargetLength, maxSourceLength ) ) + 1;
            // genes copy
            IGPGene[] genesCopy = new IGPGene[transposonLength];

            // copy genes from source point
            for (i = sourcePoint, j = 0; j < transposonLength; i++, j++ )
            {
                genesCopy[j] = genes[i].Clone( );
            }

            // copy genes to target point
            for (i = targetPoint, j = 0; j < transposonLength; i++, j++ )
            {
                genes[i] = genesCopy[j];
            }
        }
        ...
        protected void TransposeRoot( )
        {
            ...
            // shift the head
            for (i = headLength - 1; i >= transposonLength; i-- )
            {
                genes[i] = genes[i - transposonLength];
            }

            // put new root
            for (i = 0; i < transposonLength; i++ )
            {
                genes[i] = genesCopy[i];
            }
        }
