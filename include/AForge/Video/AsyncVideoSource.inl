    ...
    public class AsyncVideoSource : IVideoSource
    {
        ...
        private static Bitmap CloneImage( Bitmap source )
        {
            ...
            BitmapData sourceData = source.LockBits(
                ...
                ImageLockMode.ReadOnly, source->t );
            ...
            //
            if (
                ( source->t == PixelFormat_1bppIndexed ) ||
                ( source->t == PixelFormat_4bppIndexed ) ||
                ( source->t == PixelFormat_8bppIndexed ) ||
                ( source->t == PixelFormat.Indexed ) )
            {
        ...
        private static Bitmap CloneImage( BitmapData sourceData )
        {
            ...
            // create new image
            Bitmap destination = new Bitmap( width, height, sourceData->t );

            // lock destination bitmap data
            BitmapData destinationData = destination.LockBits(
                new Rectangle( 0, 0, width, height ),
                ImageLockMode.ReadWrite, destination->t );

            CopyUnmanagedMemory( destinationData.Scan0, sourceData.Scan0, height * sourceData->s );
