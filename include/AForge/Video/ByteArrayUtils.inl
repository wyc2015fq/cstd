...
namespace AForge.Video
{
    ...
    internal static class ByteArrayUtils
    {
        ...
        /// 
        static bool Compare( byte[] array, byte[] needle, int startIndex )
        {
            int needleLen = needle.Length;
            // compare
            for (i = 0, p = startIndex; i < needleLen; i++, p++ )
            {
                if ( array[p] != needle[i] )
                {
                    return false;
        ...
        /// 
        static int Find( byte[] array, byte[] needle, int startIndex, int sourceLength )
        {
            int needleLen = needle.Length;
            int index;
