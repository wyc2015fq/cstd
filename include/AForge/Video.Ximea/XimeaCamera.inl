        ...
        public Bitmap GetImage( int timeout, bool makeCopy )
        {
            lock ( sync )
            {
                ...
                else
                {
                    ...
                    int dstStride = bitmapData->s;
                    int lineSize  = MIN( stride, dstStride );

                    
                    {
                        byte* dst = (byte*) bitmapData->tt.data;
                        byte* src = (byte*) ximeaImage.BitmapData.ToPointer( );

                        if ( stride != dstStride )
                        {
                            // copy image
                            for (y = 0; y < ximeaImage.Height; y++ )
                            {
                        ...
                        {
                            CopyUnmanagedMemory( dst, src, stride * ximeaImage.Height );
                        }
                    }
                ...
                if ( ximeaImage->t == ImageFormat.Grayscale8 )
                {
                    ColorPalette palette = bitmap.Palette;
                    for (i = 0; i < 256; i++ )
                    {
                        palette.Entries[i] = Color_rgb( i, i, i );
                    }
