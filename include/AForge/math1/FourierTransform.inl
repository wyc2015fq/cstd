...
namespace AForge.Math
{
	...
	static class FourierTransform
	{
		...
		/// 
        static void DFT( Complex[] data, Direction direction )
		{
			int			n = data.Length;
			double		arg, cos, sin;
			...
			for (i = 0; i < n; i++ )
			{
				...
				arg = - (int) direction * 2.0 * CV_PI * (double) i / (double) n;
		...
		/// <param name="direction">Transformation direction.</param>
		/// 
        static void DFT2( Complex[,] data, Direction direction )
		{
			...
			for (i = 0; i < n; i++ )
			{
				for (j = 0; j < m; j++ )
				{
					...
					arg = - (int) direction * 2.0 * CV_PI * (double) j / (double) m;
        ...
        /// 
        static void FFT( Complex[] data, Direction direction )
		{
			int		n = data.Length;
			int		m = Tools.Log2( n );
        ...
        /// <exception cref="ArgumentException">Incorrect data length.</exception>
        /// 
        static void FFT2( Complex[,] data, Direction direction )
		{
        ...
        private static Complex[] GetComplexRotation( int numberOfBits, Direction direction )
		{
			...
			if ( complexRotation[numberOfBits - 1, directionIndex] == null )
			{
				...
				double		uR = 1.0;
				double		uI = 0.0;
				double		angle = CV_PI / n * (int) direction;
				double		wR = cos( angle );
				double		wI = sin( angle );
				double		t;
