...
namespace AForge.Math.Geometry
{
    ...
    /// <code>
    /// private List&lt;IPOINT&gt; idealCicle = new List&lt;IPOINT&gt;( );
    /// private List&lt;IPOINT&gt; distorredCircle = new List&lt;IPOINT&gt;( );
    ...
    /// for (i = 0; i &lt; 360; i += 10 )
    /// {
    ///     float angle = (float) ( (float) i / 180 * CV_PI );
    /// 
    ///     // add point to ideal circle
    ///     idealCicle.Add( new IPOINT(
    ...
    /// 
    ///     distorredCircle.Add( new IPOINT(
    ///         (int) ( distorredRadius * cos( angle ) ),
    ///         (int) ( distorredRadius * sin( angle ) ) ) );
    /// }
    /// 
    /// // check shape
    ...
    public class SimpleShapeChecker
    {
        ...
        public float MinAcceptableDistortion
        {
            ...
            set { minAcceptableDistortion = MAX( 0, value ); }
        }
        ...
        public float RelativeDistortionLimit
        {
            ...
            set { relativeDistortionLimit = MAX( 0, MIN( 1, value ) ); }
        }
        ...
        public float AngleError
        {
            ...
            set { angleError = MAX( 0, MIN( 20, value ) ); }
        }
        ...
        public float LengthError
        {
            ...
            set { lengthError = MAX( 0, MIN( 1, value ) ); }
        }
        ...
        /// 
        public ShapeType CheckShapeType( List<IPOINT> edgePoints )
        {
        ...
        /// 
        public bool IsCircle( List<IPOINT> edgePoints )
        {
            Point center;
            float radius;
        ...
        /// shape is less than 8.</note></para></remarks>
        /// 
        public bool IsCircle( List<IPOINT> edgePoints, out Point center, out float radius )
        {
            ...
            for (i = 0, n = edgePoints.Count; i < n; i++ )
            {
                meanDistance += (float) fabs( center.DistanceTo( edgePoints[i] ) - radius );
            }
            meanDistance /= edgePoints.Count;

            float maxDitance = MAX( minAcceptableDistortion,
        ...
        /// 
        public bool IsQuadrilateral( List<IPOINT> edgePoints )
        {
            List<IPOINT> corners;
        ...
        /// quadrilateral shape or <see langword="false"/> otherwise.</returns>
        /// 
        public bool IsQuadrilateral( List<IPOINT> edgePoints, out List<IPOINT> corners )
        {
        ...
        /// 
        public bool IsTriangle( List<IPOINT> edgePoints )
        {
            List<IPOINT> corners;
        ...
        /// triangle shape or <see langword="false"/> otherwise.</returns>
        /// 
        public bool IsTriangle( List<IPOINT> edgePoints, out List<IPOINT> corners )
        {
            corners = GetShapeCorners( edgePoints );
        ...
        /// </note></para></remarks>
        /// 
        public bool IsConvexPolygon( List<IPOINT> edgePoints, out List<IPOINT> corners )
        {
            corners = GetShapeCorners( edgePoints );
            return CheckIfPointsFitShape( edgePoints, corners );
        }
        ...
        /// </remarks>
        /// 
        public PolygonSubType CheckPolygonSubType( List<IPOINT> corners )
        {
            ...
            if ( corners.Count == 3 )
            {
                ...
                if ( ( fabs( angle1 - 60 ) <= angleError ) &&
                     ( fabs( angle2 - 60 ) <= angleError ) &&
                     ( fabs( angle3 - 60 ) <= angleError ) )
                {
                ...
                else
                {
                    ...
                    if ( ( fabs( angle1 - angle2 ) <= angleError ) ||
                         ( fabs( angle2 - angle3 ) <= angleError ) ||
                         ( fabs( angle3 - angle1 ) <= angleError ) )
                    {
                        subType = PolygonSubType.IsoscelesTriangle;
                    }

                    // check for rectangled triangle
                    if ( ( fabs( angle1 - 90 ) <= angleError ) ||
                         ( fabs( angle2 - 90 ) <= angleError ) ||
                         ( fabs( angle3 - 90 ) <= angleError ) )
                    {
            ...
            else if ( corners.Count == 4 )
            {
                ...
                if ( angleBetween1stPair <= angleError )
                {
                    ...
                    if ( angleBetween2ndPair <= angleError )
                    {
                        ...
                        if ( fabs( GeometryTools.GetAngleBetweenVectors( corners[1], corners[0], corners[2] ) - 90 ) <= angleError )
                        {
                        ...
                        if ( fabs( side1Length - side2Length ) <= maxLengthDiff )
                        {
        ...
        /// 
        public bool CheckIfPointsFitShape( List<IPOINT> edgePoints, List<IPOINT> corners )
        {
            ...
            {
                IPOINT currentPoint = corners[i];
                IPOINT nextPoint = ( i + 1 == cornersCount ) ? corners[0] : corners[i + 1];
            ...
            for (i = 0, n = edgePoints.Count; i < n; i++ )
            {
                ...
                for (j = 0; j < cornersCount; j++ )
                {
                    ...
                    if ( !isVert[j] )
                    {
                        distance = (float) fabs( ( k[j] * edgePoints[i].X + b[j] - edgePoints[i].Y ) / div[j] );
                    }
                    else
                    {
                        distance = fabs( edgePoints[i].X - corners[j].X );
                    }
        ...
        private List<IPOINT> GetShapeCorners( List<IPOINT> edgePoints )
        {
