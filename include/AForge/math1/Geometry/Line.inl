...
namespace AForge.Math.Geometry
{
    ...
    public sealed class Line
    {
        ...
        /// 
        static Line FromSlopeIntercept( float slope, float intercept )
        {
        ...
        /// 
        static Line FromRTheta( float radius, float theta )
        {
        ...
        /// <returns>Returns a <see cref="Line"/> representing the specified line.</returns>
        /// 
        static Line FromPointTheta( Point point, float theta )
        {
        ...
        private Line( float radius, float theta, bool unused )
        {
            if ( radius < 0 )
            {
                ArgumentOutOfRangeException( "radius", radius, "Must be non-negative" );
            }

            theta *= (float) ( CV_PI / 180 );
        ...
        public float GetAngleBetweenLines( Line secondLine )
        {
            ...
            else
            {
                ...
                if ( isVertical1 )
                {
                    angle = (float) ( CV_PI / 2 - Math.Atan( k2 ) * Math.Sign( k2 ) );
                }
                else
                {
                    angle = (float) ( CV_PI / 2 - Math.Atan( k ) * Math.Sign( k ) );
                }
            }

            // convert radians to degrees
            angle *= (float) ( 180.0 / CV_PI );
        ...
        public float DistanceToPoint( Point point )
        {
            ...
            if ( !IsVertical )
            {
                ...
                distance = fabs( ( k * point.X + b - point.Y ) / div );
            }
            else
            {
                distance = fabs( b - point.X );
            }
