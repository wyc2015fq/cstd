...
namespace AForge.Math.Geometry
{
    ...
    /// List&lt;IPOINT&gt; hull = hullFinder.FindHull( points );
    /// </code>
    /// </remarks>
    /// 
    public class GrahamConvexHull : IConvexHullAlgorithm
    {
        ...
        /// 
        public List<IPOINT> FindHull( List<IPOINT> points )
        {
            // do nothing if there 3 points or less
            if ( points.Count <= 3 )
            {
                return new List<IPOINT>( points );
            }
            ...
            for (i = 1, n = pointsToProcess.Count; i < n; i++ )
            {
            ...
            for (i = 0, n = pointsToProcess.Count; i < n; i++ )
            {
            ...
            // convert points back
            List<IPOINT> convexHull = new List<IPOINT>( );
        ...
        private class PointToProcess : IComparable
        {
            ...
            public PointToProcess( IPOINT point )
            {
                X = point.X;
                Y = point.Y;
