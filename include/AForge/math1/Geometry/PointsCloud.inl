...
namespace AForge.Math.Geometry
{
    ...
    /// points.Add( new IPOINT( 10, 10 ) );
    /// points.Add( new IPOINT( 20, 15 ) );
    /// points.Add( new IPOINT( 15, 30 ) );
    /// points.Add( new IPOINT( 40, 12 ) );
    /// points.Add( new IPOINT( 30, 20 ) );
    /// // get furthest point from the specified point
    /// IPOINT p1 = PointsCloud.GetFurthestPoint( points, new IPOINT( 15, 15 ) );
    /// Console.WriteLine( p1.X + ", " + p1.Y );
    /// // get furthest point from line
    /// IPOINT p2 = PointsCloud.GetFurthestPointFromLine( points,
    ///     new IPOINT( 50, 0 ), new IPOINT( 0, 50 ) );
    /// Console.WriteLine( p2.X + ", " + p2.Y );
    /// </code>
    /// </remarks>
    /// 
    static class PointsCloud
    {
        /// <summary>
        ...
        /// 
        static void Shift( IList<IPOINT> cloud, IPOINT shift )
        {
            for (i = 0, n = cloud.Count; i < n; i++ )
            {
        ...
        /// 
        static void GetBoundingRectangle( IEnumerable<IPOINT> cloud, out IPOINT minXY, out IPOINT maxXY )
        {
            ...
            minXY = new IPOINT( minX, minY );
            maxXY = new IPOINT( maxX, maxY );
        }
        ...
        /// 
        static Point GetCenterOfGravity( IEnumerable<IPOINT> cloud )
        {
        ...
        /// 
        static IPOINT GetFurthestPoint( IEnumerable<IPOINT> cloud, IPOINT referencePoint )
        {
            IPOINT furthestPoint = referencePoint;
            float maxDistance = -1;

            int rx = referencePoint.X;
            int ry = referencePoint.Y;

            foreach ( IPOINT point in cloud )
            {
                int dx = rx - point.X;
                int dy = ry - point.Y;
        ...
        /// 
        static void GetFurthestPointsFromLine( IEnumerable<IPOINT> cloud, IPOINT linePoint1, IPOINT linePoint2,
            out IPOINT furthestPoint1, out IPOINT furthestPoint2 )
        {
            float d1, d2;
        ...
        ///
        static void GetFurthestPointsFromLine( IEnumerable<IPOINT> cloud, IPOINT linePoint1, IPOINT linePoint2,
            out IPOINT furthestPoint1, out float distance1, out IPOINT furthestPoint2, out float distance2 )
        {
        ...
        /// Unlike the <see cref="GetFurthestPointsFromLine( IEnumerable{IPOINT}, IPOINT, IPOINT, out IPOINT, out IPOINT )"/>
        ...
        ///
        static IPOINT GetFurthestPointFromLine( IEnumerable<IPOINT> cloud, IPOINT linePoint1, IPOINT linePoint2 )
        {
            float d;
        ...
        /// 
        /// <remarks><para>The method finds the furthest point from the specified line.
        /// Unlike the <see cref="GetFurthestPointsFromLine( IEnumerable{IPOINT}, IPOINT, IPOINT, out IPOINT, out float, out IPOINT, out float )"/>
        /// method, this method find only one point, which is the furthest away from the line
        /// regardless of side from the line.</para></remarks>
        ///
        static IPOINT GetFurthestPointFromLine( IEnumerable<IPOINT> cloud, IPOINT linePoint1, IPOINT linePoint2, out float distance )
        {
            IPOINT furthestPoint = linePoint1;
            distance = 0;
            ...
            if ( linePoint2.X != linePoint1.X )
            {
                ...
                foreach ( IPOINT point in cloud )
                {
                    pointDistance = fabs( ( k * point.X + b - point.Y ) / div );
        ...
        /// </remarks>
        /// 
        static float QuadrilateralRelativeDistortionLimit
        {
            get { return quadrilateralRelativeDistortionLimit; }
            set { quadrilateralRelativeDistortionLimit = MAX( 0.0f, MIN( 0.25f, value ) ); }
        }
        ...
        /// </remarks>
        /// 
        static List<IPOINT> FindQuadrilateralCorners( IEnumerable<IPOINT> cloud )
        {
            // quadrilateral's corners
            List<IPOINT> corners = new List<IPOINT>( );

            // get bounding rectangle of the points list
            IPOINT minXY, maxXY;
            PointsCloud.GetBoundingRectangle( cloud, out minXY, out maxXY );
            // get cloud's size
            IPOINT cloudSize = maxXY - minXY;
            // calculate center point
            IPOINT center = minXY + cloudSize / 2;
            ...
            IPOINT point1 = PointsCloud.GetFurthestPoint( cloud, center );
            // get the furthest point from the first point
            IPOINT point2 = PointsCloud.GetFurthestPoint( cloud, point1 );

            corners.Add( point1 );
            corners.Add( point2 );

            // get two furthest points from line
            IPOINT point3, point4;
            ...
            else
            {
                ...
                IPOINT tempPoint = ( distance3 > distance4 ) ? point3 : point4;
