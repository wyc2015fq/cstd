...
namespace AForge.Math.Geometry
{
    ...
    public class CoplanarPosit
    {
        ...
        private void POS( Point[] imagePoints, Vector3 eps, out Matrix3x3 rotation1, out Matrix3x3 rotation2, out Vector3 translation1, out Vector3 translation2 )
        {
            ...
            if ( j2i2dif == 0 )
            {
                theta = (float) ( ( -CV_PI / 2 ) * System.Math.Sign( ij ) );
                r = (float) sqrt( fabs( 2 * ij ) );
            }
            else
            {
                r = (float) sqrt( sqrt( j2i2dif * j2i2dif + 4 * ij * ij ) );
            ...
            float lambda = (float) ( r * cos( theta ) );
            float mu =     (float) ( r * sin( theta ) );
