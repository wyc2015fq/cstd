...
namespace Accord.Math.Decompositions
{
    ...
    ///     obeys <c>A * v = ¦Ë * B * v</c> where <c>A</c> and <c>B</c> are matrices. If <c>v</c>
    ///     obeys this equation, with some <c>¦Ë</c>, then we call <c>v</c> the generalized eigenvector
    ///     of <c>A</c> and <c>B</c>, and <c>¦Ë</c> is called the generalized eigenvalue of <c>A</c>
    ///     and <c>B</c> which corresponds to the generalized eigenvector <c>v</c>. The possible
    ///     values of <c>¦Ë</c>, must obey the identity <c>det(A - ¦Ë*B) = 0</c>.</para>
    ///   <para>
    ...
    public sealed class GeneralizedEigenvalueDecomposition
    {
        ...
        private static int qzhes(int n, double[,] a, double[,] b, bool matz, double[,] z)
        {
            ...
            for (k = 0; k < n - 2; ++k)
            {
                ...
                for (lb = 0; lb < nk1; ++lb)
                {
                    ...
                    u1 = a[l, k] / s;
                    u2 = a[l1, k] / s;
                    r = Special.Sign(sqrt(u1 * u1 + u2 * u2), u1);
                    v1 = -(u1 + r) / r;
                    v2 = -u2 / r;
                    u2 = v2 / v1;
        ...
        L100:
            ...
            if (fabs(b11) > epsb) goto L120;

            b[l, l] = 0.0;
            s = (fabs(a[l, l]) + fabs(a[l1, l]));
            u1 = a[l, l] / s;
            u2 = a[l1, l] / s;
            r = Special.Sign(sqrt(u1 * u1 + u2 * u2), u1);
            v1 = -(u1 + r) / r;
        ...
        L120:
            ...
            for (ll = ld; ll + 1 <= enm2; ++ll)
            {
                ...
                if (fabs(a[l, lm1]) <= (fabs(t / a[l1, l])) * epsa)
                    goto L100;
            }
        ...
        L160:
            ...
            L200:
                ...
                u2 = a2 / s;
                u3 = a3 / s;
                r = Special.Sign(sqrt(u1 * u1 + u2 * u2 + u3 * u3), u1);
                v1 = -(u1 + r) / r;
                v2 = -u2 / r;
                v3 = -u3 / r;
                u2 = v2 / v1;
        ...
        private static int qzval(int n, double[,] a, double[,] b, double[] alfr, double[] alfi, double[] beta, bool matz, double[,] z)
        {
            ...
            // 2-by-2 block
            L420:
                if (fabs(b[na, na]) <= epsb) goto L455;
                if (fabs(b[en, en]) > epsb) goto L430;
                a1 = a[en, en];
                a2 = a[en, na];
                bn = 0.0;
                goto L435;
