...
const float SingleEpsilon = 1.1920929E-07f;
...
#define DBL_NaN    1.1125369292536007e-308
...
/// </summary>
static double Rgamma(double a, double z)
{
...
static double Digamma(double x)
{
    ...
    if (x <= 0.0)
    {
        ...
        if (p == q) {
            OverflowException("Function computation resulted in arithmetic overflow.");
        }
...
/// </summary>
static double BesselJ(double x)
{
    double ax, y, z, xx;
...
/// </summary>
static double BesselJ2(int n, double x)
{
    int j, m;
...
static double BesselY0(double x)
{
    if (x < 8.0)
    {
        ...
        return (ans1 / ans2) + 0.636619772 * BesselJ0(x) * log(x);
    }
    else
    {
        double z = 8.0 / x;
...
/// </summary>
static double BesselY(double x)
{
    if (x < 8.0)
    {
        double y = x * x;
        ...
        return (ans1 / ans2) + 0.636619772 * (BesselJ(x) * log(x) - 1.0 / x);
    }
    else
    {
        double z = 8.0 / x;
...
/// </summary>
static double BesselY2(int n, double x)
{
    int j;
...
static int ftop=0;
static double fcache[33];
static double lnfcache[101];
...
static double Factorial(int n)
{
    ...
    if (n < 0)
    {
        ...
        ArgumentException("Argument can not be negative");
    }
    if (n > 32)
    {
...
static double Lfactorial(int n)
{
    ...
    {
        // Factorial is not defined for negative numbers.
        ArgumentException("Argument cannot be negative.");
    }
    if (n <= 1)
    {
...
{
    return floor(0.5 + exp(Lfactorial(n) - Lfactorial(k) - Lfactorial(n - k)));
}
...
static double Ibeta(double aa, double bb, double xx)
{
    ...
    if (aa <= 0.0) {
        ArgumentOutOfRangeException("aa domain error");
    }
    if (bb <= 0.0) {
        ArgumentOutOfRangeException("bb domain error");
    }
    ...
    if ((xx <= 0.0) || (xx >= 1.0))
    {
        ...
        ArgumentOutOfRangeException("xx domain error");
    }
    ...
    y = a * log(x);
    t = b * log(xc);
    if ((a + b) < GammaMax && fabs(y) < LogMax && fabs(t) < LogMax)
    {
        t = pow(xc, b);
        t *= pow(x, a);
        t /= a;
