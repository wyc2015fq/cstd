    ...
    public class Histogram : System.Windows.Forms.Control
    {
        ...
        protected override void OnPaint( PaintEventArgs pe )
        {
            ...
            width = ( ( values == null ) || ( vertical == true ) ) ?
                ...
                MIN( values.Length, ClientRectangle.Width - 2 );

            height = ( ( values == null ) || ( vertical == false ) ) ?
                ClientRectangle.Height - 2 :
                MIN( values.Length, ClientRectangle.Height - 2 );
            ...
            if ( values != null )
            {
                int start = MIN( this.start, this.stop );
                int stop  = MAX( this.start, this.stop );

                if ( tracking )
                {
                    // fill region of selection
                    Brush brush = new SolidBrush( Color_rgb( 92, 92, 92 ) );

                    if ( vertical )
                    {
                        g.FillRectangle( brush, x, y + start, width, fabs( start - stop ) + 1 );
                    }
                    else
                    {
                        g.FillRectangle( brush, x + start, y, fabs( start - stop ) + 1, height );
                    }
                    brush.Dispose( );
                }
                ...
                if ( max != 0 )
                {
                    ...
                    // draw histogram
                    for (i = 0, len = ( vertical ) ? height : width; i < len; i++ )
                    {
                        if ( logarithmic )
                        {
                            value = ( values[i] == 0 ) ? 0 : (int) ( log10( values[i] ) * factor );
                        }
                        else
                        {
        ...
        private void Histogram_MouseMove( object sender, System.Windows.Forms.MouseEventArgs e )
        {
            if ( ( allowSelection ) && ( values != null ) )
            {
                ...
                else
                {
                    ...
                    stop = MIN( stop, ( ( vertical ) ? height : width ) - 1 );
                    stop = MAX( stop, 0 );
                    ...
                    if ( SelectionChanged != null )
                        SelectionChanged( this, new HistogramEventArgs( MIN( start, stop ), MAX( start, stop ) ) );
                }
            }
        }
