...
namespace AForge.Controls
{
    ...
    public partial class ManipulatorControl : Control
    {
        ...
        private SolidBrush disabledBrash              = new SolidBrush( Color_rgb( 240, 240, 240 ) );
        ...
        public PointF ManipulatorPosition
        {
            ...
            set
            {
                manipulatatorX = MAX( -1.0f, MIN( 1.0f, value.X ) );
                manipulatatorY = MAX( -1.0f, MIN( 1.0f, value.Y ) );

                if ( !isSquareLook )
                {
                    // get distance from center
                    double cR = sqrt( manipulatatorX * manipulatatorX + manipulatatorY * manipulatatorY );
        ...
        private void ManipulatorControl_Paint( object sender, PaintEventArgs e )
        {
            ...
            areaSize = MIN( ClientRectangle.Width, ClientRectangle.Height ) - areaMargin * 2;
        ...
        private void ManipulatorControl_MouseDown( object sender, MouseEventArgs e )
        {
            if ( e.Button == MouseButtons.Left )
            {
                ...
                else
                {
                    ...
                    if ( sqrt( cX * cX + cY * cY ) <= areaRadius )
                    {
                        tracking = true;
                    }
                }
        ...
        private void ManipulatorControl_MouseMove( object sender, MouseEventArgs e )
        {
            if ( tracking )
            {
                ...
                if ( isSquareLook )
                {
                    cX = MIN( areaRadius, MAX( -areaRadius, cX ) );
                    cY = MIN( areaRadius, MAX( -areaRadius, cY ) );
                }
                else
                {
                    // get distance from center
                    int cR = (int) sqrt( cX * cX + cY * cY );
