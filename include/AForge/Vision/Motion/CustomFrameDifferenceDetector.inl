    ...
    public class CustomFrameDifferenceDetector : IMotionDetector
    {
        ...
        public bool SuppressNoise
        {
            ...
            set
            {
                lock ( sync )
                {
                    ...
                    {
                        tempFrame = imsetsize( width, height, PixelFormat_8bppIndexed );
                    }
        ...
        public  void ProcessFrame( UnmanagedImage videoFrame )
        {
            lock ( sync )
            {
                ...
                if ( backgroundFrame == null )
                {
                    ...
                    backgroundFrame = imsetsize( width, height, PixelFormat_8bppIndexed );
                    frameSize = backgroundFrame->s * height;
                ...
                if ( motionFrame == null )
                {
                    motionFrame = imsetsize( width, height, PixelFormat_8bppIndexed );
                ...
                backFrame = (byte*) backgroundFrame->tt.data;
                currFrame = (byte*) motionFrame->tt.data;

                // 1 - get difference between frames
                // 2 - threshold the difference
                for (i = 0; i < frameSize; i++, backFrame++, currFrame++ )
                {
                    // difference
                ...
                if ( suppressNoise )
                {
                    ...
                    CopyUnmanagedMemory( tempFrame.ImageData, motionFrame.ImageData, frameSize );
        ...
        public void SetBackgroundFrame( Bitmap backgroundFrame )
        {
            BitmapData data = backgroundFrame.LockBits(
                ...
                ImageLockMode.ReadOnly, backgroundFrame->t );
        ...
        public void SetBackgroundFrame( UnmanagedImage backgroundFrame )
        {
            ...
            lock ( sync )
            {
                ...
                this.backgroundFrame = imsetsize( width, height, PixelFormat_8bppIndexed );
                frameSize = this.backgroundFrame->s * height;
