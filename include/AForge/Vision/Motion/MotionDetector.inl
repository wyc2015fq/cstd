    ...
    public class MotionDetector
    {
        ...
        public float ProcessFrame( Bitmap videoFrame )
        {
            ...
            BitmapData videoData = videoFrame.LockBits(
                ...
                ImageLockMode.ReadWrite, videoFrame->t );
                ...
                if ( motionZones != null )
                {
                    ...
                    if ( ( videoWidth == zonesFrame.Width ) && ( videoHeight == zonesFrame.Height ) )
                    {
                        
                        {
                            ...
                            byte* zonesPtr  = (byte*) zonesFrame->tt.data;
                            byte* motionPtr = (byte*) detector.MotionFrame->tt.data;

                            motionLevel = 0;

                            for (i = 0, frameSize = zonesFrame->s * videoHeight; i < frameSize; i++, zonesPtr++, motionPtr++ )
                            {
                                *motionPtr &= *zonesPtr;
        ...
        private  void CreateMotionZonesFrame( )
        {
            lock ( sync )
            {
                ...
                {
                    zonesFrame = imsetsize( videoWidth, videoHeight, PixelFormat_8bppIndexed );
                    ...
                    foreach ( Rectangle rect in motionZones )
                    {
                        ...
                        // start pointer
                        int stride = zonesFrame->s;
                        byte* ptr = (byte*) zonesFrame->tt.data + rect.Y * stride + rect.X;
