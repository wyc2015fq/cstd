    ...
    public class ImageStatisticsYCbCr
    {
        ...
        public ImageStatisticsYCbCr( Bitmap image )
        {
            ...
            BitmapData imageData = image.LockBits(
                ...
                ImageLockMode.ReadOnly, PixelFormat_24bppRgb );
        ...
        {
            CheckSourceFormat( image->t );
            CheckMaskProperties( mask->t, new Size( mask.Width, mask.Height ), new Size( image.Width, image.Height ) );
            ...
            try
            {
                ...
                {
                    ProcessImage( new UnmanagedImage( imageData ), (byte*) maskData->tt.data, maskData->s );
                }
            }
            finally
            {
        ...
        {
            CheckSourceFormat( image->t );
            CheckMaskProperties( PixelFormat_8bppIndexed,
        ...
        public ImageStatisticsYCbCr( UnmanagedImage image, UnmanagedImage mask )
        {
            ...
            {
                ProcessImage( image, (byte*) mask->tt.data, mask->s );
            }
        }
        ...
        private  void ProcessImage( UnmanagedImage image, byte* mask, int maskLineSize )
        {
            ...
            else
            {
                ...
                {
                    // for each pixel
                    for (x = 0; x < width; x++, p += pixelSize, mask++ )
                    {
                        if ( *mask == 0 )
                            continue;
        ...
        {
            if ( maskFormat != PixelFormat_8bppIndexed )
            {
