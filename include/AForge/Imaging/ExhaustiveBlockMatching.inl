...
namespace AForge.Imaging
{
    ...
    /// List&lt;IPOINT&gt; points = scd.ProcessImage( sourceImage );
    /// 
    ...
    ///     ImageLockMode.ReadWrite, sourceImage->t );
    /// 
    ...
    public class ExhaustiveBlockMatching : IBlockMatching
    {
        ...
        public float SimilarityThreshold
        {
            ...
            set { similarityThreshold = MIN( 1, MAX( 0, value ) ); }
        }
        ...
        /// 
        public List<BlockMatch> ProcessImage( Bitmap sourceImage, List<IPOINT> coordinates, Bitmap searchImage )
        {
            // lock source image
            BitmapData sourceImageData = sourceImage.LockBits(
                new Rectangle( 0, 0, sourceImage.Width, sourceImage.Height ),
                ImageLockMode.ReadOnly, sourceImage->t );

            BitmapData searchImageData = searchImage.LockBits(
                new Rectangle( 0, 0, searchImage.Width, searchImage.Height ),
                ImageLockMode.ReadOnly, searchImage->t );
        ...
        /// 
        public List<BlockMatch> ProcessImage( BitmapData sourceImageData, List<IPOINT> coordinates, BitmapData searchImageData )
        {
        ...
        /// <exception cref="ArgumentException">Source and search images must have same pixel format.</exception>
        /// 
        public List<BlockMatch> ProcessImage( UnmanagedImage sourceImage, List<IPOINT> coordinates, UnmanagedImage searchImage )
        {
            ...
            if ( ( sourceImage->t != PixelFormat_8bppIndexed ) && ( sourceImage->t != PixelFormat_24bppRgb ) )
                UnsupportedImageFormatException( "Source images can be graysclae (8 bpp indexed) or color (24 bpp) image only" );

            // source images must have the same pixel format.
            if ( sourceImage->t != searchImage->t )
            ...
            int stride = sourceImage->s;
            int pixelSize = ( sourceImage->t == PixelFormat_8bppIndexed ) ? 1 : 3;
            ...
            {
                byte* ptrSource = (byte*) sourceImage->tt.data;
                byte* ptrSearch = (byte*) searchImage->tt.data;

                // for each point fed
                for (iPoint = 0; iPoint < pointsCount; iPoint++ )
                {
                    ...
                    for (searchWindowRow = 0; searchWindowRow < searchWindowSize; searchWindowRow++ )
                    {
                        ...
                        for (searchWindowCol = 0; searchWindowCol < searchWindowSize; searchWindowCol++ )
                        {
                            ...
                            {
                                for (blockCol = 0; blockCol < blockLineSize; blockCol++, ptrSourceBlock++, ptrSearchBlock++ )
                                {
                    ...
                    if ( blockSimilarity >= threshold )
                    {
                        matchingsList.Add( new BlockMatch(
                            new IPOINT( refPointX, refPointY ), new IPOINT( bestMatchX, bestMatchY ),
