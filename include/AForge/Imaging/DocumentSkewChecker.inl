...
namespace AForge.Imaging
{
    ...
    public class DocumentSkewChecker
    {
        ...
        public int StepsPerDegree
        {
            ...
            set
            {
                stepsPerDegree = MAX( 1, MIN( 10, value ) );
        ...
        public double MaxSkewToDetect
        {
            ...
            set
            {
                maxSkewToDetect = MAX( 0, MIN( 45, value ) );
                needToInitialize = true;
            }
        }
        ...
        public int LocalPeakRadius
        {
            ...
            set { localPeakRadius = MAX( 1, MIN( 10, value ) ); }
        }
        ...
        public double GetSkewAngle( UnmanagedImage image, Rectangle rect )
        {
            ...
            int offset = image->s - rect.Width;

            // calculate Hough map's width
            int halfHoughWidth = (int) sqrt( halfWidth * halfWidth + halfHeight * halfHeight );
            ...
            
            {
                byte* src = (byte*) image->tt.data +
                    rect.Top * image->s + rect.Left;
                byte* srcBelow = src + image->s;

                // for each row
                for (y = startY; y < stopY; y++ )
                {
                    // for each pixel
                    for (x = startX; x < stopX; x++, src++, srcBelow++ )
                    {
            ...
            for (theta = 0; theta < maxTheta; theta++ )
            {
                ...
                for (radius = 0; radius < maxRadius; radius++ )
                {
                    ...
                    for (tt = theta - localPeakRadius, ttMax = theta + localPeakRadius; tt < ttMax; tt++ )
                    {
                        ...
                        for (tr = radius - localPeakRadius, trMax = radius + localPeakRadius; tr < trMax; tr++ )
                        {
                            // skip out of map values
                            if ( tr < 0 )
                                continue;
        ...
        private void InitHoughMap( )
        {
            if ( needToInitialize )
            {
                ...
                thetaStep = ( 2 * maxSkewToDetect * CV_PI / 180 ) / houghHeight;
                ...
                for (i = 0; i < houghHeight; i++ )
                {
                    sinMap[i] = Math.Sin( ( minTheta * CV_PI / 180 ) + ( i * thetaStep ) );
                    cosMap[i] = Math.Cos( ( minTheta * CV_PI / 180 ) + ( i * thetaStep ) );
                }
            }
        }
    }
