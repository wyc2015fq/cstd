...
namespace AForge.Imaging.Filters
{
    ...
    public class FlatFieldCorrection : BaseInPlaceFilter
    {
        ...
        protected override  void ProcessFilter( UnmanagedImage image )
        {
            ...
            int offset = image->s - ( ( image->t == PixelFormat_8bppIndexed ) ? width : width * 3 );
            ...
            else
            {
                if ( backgroundImage != null )
                {
                    ...
                    if ( ( width != backgroundImage.Width ) || ( height != backgroundImage.Height ) || ( image->t != backgroundImage->t ) )
                    {
                    ...
                    bgLockedData = backgroundImage.LockBits(
                        ...
                        ImageLockMode.ReadOnly, backgroundImage->t );
            ...
            if ( image->t == PixelFormat_8bppIndexed )
            {
                ...
                for (y = 0; y < height; y++ )
                {
                    for (x = 0; x < width; x++, src++, bg++ )
                    {
                        if ( *bg != 0 )
                        {
                            *src = (byte) MIN( mean * *src / *bg, 255 );
                        }
                    }
                    src += offset;
                    bg  += offset;
            ...
            else
            {
                ...
                for (y = 0; y < height; y++ )
                {
                    for (x = 0; x < width; x++, src += 3, bg += 3 )
                    {
                        // red
                        if ( bg[RGB_R] != 0 )
                        {
                            src[RGB_R] = (byte) MIN( meanR * src[RGB_R] / bg[RGB_R], 255 );
                        }
                        // green
                        if ( bg[RGB_G] != 0 )
                        {
                            src[RGB_G] = (byte) MIN( meanG * src[RGB_G] / bg[RGB_G], 255 );
                        }
                        // blue
                        if ( bg[RGB_B] != 0 )
                        {
                            src[RGB_B] = (byte) MIN( meanB * src[RGB_B] / bg[RGB_B], 255 );
                        }
                    }
                    src += offset;
                    bg  += offset;
