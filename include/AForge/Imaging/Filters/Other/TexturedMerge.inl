...
namespace AForge.Imaging.Filters
{
    ...
    public class TexturedMerge : BaseInPlaceFilter2
    {
        ...
        protected override  void ProcessFilter( UnmanagedImage image, UnmanagedImage overlay )
        {
            ...
            int srcOffset = image->s - widthToProcess * pixelSize;
            int ovrOffset = overlay->s - widthToProcess * pixelSize;
            ...
            for (y = 0; y < heightToProcess; y++ )
            {
                ...
                for (x = 0; x < widthToProcess; x++ )
                {
                    ...
                    for (i = 0; i < pixelSize; i++, ptr++, ovr++ )
                    {
                        *ptr = (byte) MIN( 255.0f, *ptr * t1 + *ovr * t2 );
                    }
                }
                ptr += srcOffset;
                ovr += ovrOffset;
