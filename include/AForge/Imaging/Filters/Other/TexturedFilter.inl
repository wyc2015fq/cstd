...
namespace AForge.Imaging.Filters
{
    ...
    public class TexturedFilter : BaseFilter
    {
        ...
        public double FilterLevel
        {
            ...
            set { filterLevel = MAX( 0.0, MIN( 1.0, value ) ); }
        }
        ...
        public double PreserveLevel
        {
            ...
            set { preserveLevel = MAX( 0.0, MIN( 1.0, value ) ); }
        }
        ...
        public IFilter Filter1
        {
            ...
            set
            {
                if ( value is IFilterInformation )
                {
                    ...
                    if ( !info.FormatTranslations.ContainsKey( PixelFormat_24bppRgb ) )
                        UnsupportedImageFormatException( "The specified filter does not support 24 bpp color images." );
                    if (
                        ( info.FormatTranslations[PixelFormat_24bppRgb] != PixelFormat_24bppRgb ) &&
                        ( info.FormatTranslations[PixelFormat_24bppRgb] != PixelFormat_8bppIndexed ) )
                        UnsupportedImageFormatException( "The specified filter does not produce image of supported format." );
                }
                else
                {
        ...
        protected override  void ProcessFilter( UnmanagedImage sourceData, UnmanagedImage destinationData )
        {
            ...
            if ( filteredImage1->t == PixelFormat_8bppIndexed )
            {
            ...
            if ( filter2 != null )
            {
                ...
                if ( filteredImage2->t == PixelFormat_8bppIndexed )
                {
                    GrayscaleToRGB coloringFilter = new GrayscaleToRGB( );
            ...
            
            {
                ...
                byte* src1 = (byte*) filteredImage1->tt.data;
                byte* src2 = (byte*) filteredImage2->tt.data;

                int dstOffset  = destinationData->s - 3 * width;
                int src1Offset = filteredImage1->s  - 3 * width;
                int src2Offset = filteredImage2->s  - 3 * width;
                ...
                else
                {
                    ...
                    for (y = 0; y < height; y++ )
                    {
                        ...
                        for (x = 0; x < width; x++ )
                        {
                            ...
                            for (i = 0; i < 3; i++, src1++, src2++, dst++ )
                            {
                                *dst = (byte) MIN( 255.0f, t1 * *src1 + t2 * *src2 );
                            }
                        }
                        src1 += src1Offset;
                        src2 += src2Offset;
