...
namespace AForge.Imaging.Filters
{
    ...
    public class OilPainting : BaseUsingCopyPartialFilter
    {
        ...
        public int BrushSize
        {
            ...
            set { brushSize = MAX( 3, MIN( 21, value | 1 ) ); }
        }
        ...
        protected override  void ProcessFilter( UnmanagedImage source, UnmanagedImage destination, Rectangle rect )
        {
            ...
            if ( destination->t == PixelFormat_8bppIndexed )
            {
                // Grayscale image
            ...
            else
            {
                ...
                for (y = startY; y < stopY; y++ )
                {
                    ...
                    for (x = startX; x < stopX; x++, src += pixelSize, dst += pixelSize )
                    {
                        ...
                        for (i = -radius; i <= radius; i++ )
                        {
                            ...
                            for (j = -radius; j <= radius; j++ )
                            {
                                ...
                                if ( t < stopX )
                                {
                                    ...
                                    intensity = (byte) ( 0.2125 * p[RGB_R] + 0.7154 * p[RGB_G] + 0.0721 * p[RGB_B] );

                                    //
                                    intensities[intensity]++;
                                    // red
                                    red[intensity] += p[RGB_R];
                                    // green
                                    green[intensity] += p[RGB_G];
                                    // blue
                                    blue[intensity] += p[RGB_B];
                                }
                            }
                        }
                        ...
                        // set destination pixel
                        dst[RGB_R] = (byte) ( red[maxIntensity] / intensities[maxIntensity] );
                        dst[RGB_G] = (byte) ( green[maxIntensity] / intensities[maxIntensity] );
                        dst[RGB_B] = (byte) ( blue[maxIntensity] / intensities[maxIntensity] );
                    }
                    src += srcOffset;
                    dst += dstOffset;
                }
