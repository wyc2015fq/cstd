...
namespace AForge.Imaging.Filters
{
    ...
    /// 
    /// IPOINT[,] warpMap = new IPOINT[height, width];
    ///
    /// int size = 8;
    ...
    public class ImageWarp : BaseFilter
    {
        ...
        /// 
        public ImageWarp( IPOINT[,] warpMap )
        {
        ...
        /// 
        protected override  void ProcessFilter( UnmanagedImage source, UnmanagedImage destination )
        {
            int pixelSize = Image_GetPixelFormatSize( source->t ) / 8;

            // image width and height
            int width  = source.Width;
            int height = source.Height;

            int widthToProcess  = MIN( width,  warpMap.GetLength( 1 ) );
            int heightToProcess = MIN( height, warpMap.GetLength( 0 ) );
            ...
            for (y = 0; y < heightToProcess; y++ )
            {
                ...
                {
                    CopyUnmanagedMemory( dst, src + y * srcStride + widthToProcess * pixelSize, ( width - widthToProcess ) * pixelSize );
                }
            ...
            for (y = heightToProcess; y < height; y++, dst += dstStride )
            {
                CopyUnmanagedMemory( dst, src + y * srcStride, width * pixelSize );
            }
        }
    }
}
