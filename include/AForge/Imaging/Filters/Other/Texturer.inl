...
namespace AForge.Imaging.Filters
{
    ...
    public class Texturer : BaseInPlacePartialFilter
    {
        ...
        protected override  void ProcessFilter( UnmanagedImage image, Rectangle rect )
        {
            ...
            {
                widthToProcess  = MIN( width, texture.GetLength( 1 ) );
                heightToProcess = MIN( height, texture.GetLength( 0 ) );
            }

            int offset = image->s - widthToProcess * pixelSize;

            // do the job
            byte* ptr = (byte*) image->tt.data;

            // allign pointer to the first pixel to process
            ptr += ( rect.Top * image->s + rect.Left * pixelSize );
            ...
            for (y = 0; y < heightToProcess; y++ )
            {
                for (x = 0; x < widthToProcess; x++ )
                {
                    ...
                    // process each pixel
                    for (i = 0; i < pixelSize; i++, ptr++ )
                    {
                        *ptr = (byte) MIN( 255.0f, ( preserveLevel * ( *ptr ) ) + ( filterLevel * ( *ptr ) ) * t );
                    }
                }
                ptr += offset;
            }
