...
namespace AForge.Imaging.Filters
{
    ...
    public class MaskedFilter : BaseInPlacePartialFilter
    {
        ...
        public Bitmap MaskImage
        {
            ...
            set
            {
                if ( ( maskImage != null ) && ( maskImage->t != PixelFormat_8bppIndexed ) )
                {
        ...
        public UnmanagedImage UnmanagedMaskImage
        {
            ...
            set
            {
                if ( ( unmanagedMaskImage != null ) && ( unmanagedMaskImage->t != PixelFormat_8bppIndexed ) )
                {
                    throw new ArgumentException( "The mask image must be 8 bpp grayscale image." );
                }
        ...
        protected override  void ProcessFilter( UnmanagedImage image, Rectangle rect )
        {
            ...
            else if ( unmanagedMaskImage != null )
            {
                ...
                ProcessImage( image, rect, (byte*) unmanagedMaskImage->tt.data,
                              unmanagedMaskImage->s );
            }
            else if ( maskImage != null )
            {
                ...
                BitmapData maskData = maskImage.LockBits( new Rectangle( 0, 0, image.Width, image.Height ),
                    ImageLockMode.ReadOnly, PixelFormat_8bppIndexed );

                try
                {
                    ProcessImage( image, rect, (byte*) maskData->tt.data,
                                  maskData->s );
                }
                finally
                {
        ...
        private  void ProcessImage( UnmanagedImage image, Rectangle rect, byte* mask, int maskLineSize )
        {
            ...
            int filteredStride = filteredImage->s;
            ...
            if ( ( pixelSize <= 4 ) && ( pixelSize != 2 ) )
            {
                ...
                switch ( pixelSize )
                {
                    case 1:
                        ...
                        {
                            for (x = startX; x < stopX; x++, imagePtr++, filteredPtr++, mask++ )
                            {
                                if ( *mask != 0 )
                                {
                    ...
                    case 3:
                        ...
                        for (y = startY; y < stopY; y++ )
                        {
                            for (x = startX; x < stopX; x++, imagePtr += 3, filteredPtr += 3, mask++ )
                            {
                                if ( *mask != 0 )
                                {
                                    imagePtr[RGB_R] = filteredPtr[RGB_R];
                                    imagePtr[RGB_G] = filteredPtr[RGB_G];
                                    imagePtr[RGB_B] = filteredPtr[RGB_B];
                                }
                            }
                            imagePtr += srcOffset;
                            filteredPtr += filteredOffset;
                    ...
                    case 4:
                        ...
                        for (y = startY; y < stopY; y++ )
                        {
                            for (x = startX; x < stopX; x++, imagePtr += 4, filteredPtr += 4, mask++ )
                            {
                                if ( *mask != 0 )
                                {
                                    imagePtr[RGB_R] = filteredPtr[RGB_R];
                                    imagePtr[RGB_G] = filteredPtr[RGB_G];
                                    imagePtr[RGB_B] = filteredPtr[RGB_B];
                                    imagePtr[RGB_A] = filteredPtr[RGB_A];
                                }
                            }
                            imagePtr += srcOffset;
                            filteredPtr += filteredOffset;
            ...
            {
                // 16 bits per channel
                byte* imagePtrBase = (byte*) image->tt.data +
                                     srcStride * startY + pixelSize * startX;
                byte* filteredPtrBase = (byte*) filteredImage->tt.data +
                                        filteredStride * startY + pixelSize * startX;
