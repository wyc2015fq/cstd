...
namespace AForge.Imaging.Filters
{
    ...
    public class StereoAnaglyph : BaseInPlaceFilter2
    {
        ...
        protected override  void ProcessFilter( UnmanagedImage image, UnmanagedImage overlay )
        {
            ...
            int offset    = image->s - width * 3;
            int ovrOffset = overlay->s - width * 3;
            ...
            switch ( anaglyphAlgorithm )
            {
                case Algorithm.TrueAnaglyph:
                    ...
                    {
                        // for each pixel
                        for (x = 0; x < width; x++, ptr += 3, ovr += 3 )
                        {
                            ptr[RGB_R] = (byte) ( ptr[RGB_R] * 0.299 + ptr[RGB_G] * 0.587 + ptr[RGB_B] * 0.114 );
                            ptr[RGB_G] = 0;
                            ptr[RGB_B] = (byte) ( ovr[RGB_R] * 0.299 + ovr[RGB_G] * 0.587 + ovr[RGB_B] * 0.114 );
                        }
                        ptr += offset;
                        ovr += ovrOffset;
                    }
                ...
                case Algorithm.GrayAnaglyph:
                    ...
                    for (y = 0; y < height; y++ )
                    {
                        ...
                        {
                            ptr[RGB_R] = (byte) ( ptr[RGB_R] * 0.299 + ptr[RGB_G] * 0.587 + ptr[RGB_B] * 0.114 );
                            ptr[RGB_G] = (byte) ( ovr[RGB_R] * 0.299 + ovr[RGB_G] * 0.587 + ovr[RGB_B] * 0.114 );
                            ptr[RGB_B] = ptr[RGB_G];
                        }
                        ptr += offset;
                        ovr += ovrOffset;
                ...
                case Algorithm.OptimizedAnaglyph:
                    ...
                    for (y = 0; y < height; y++ )
                    {
                        ...
                        for (x = 0; x < width; x++, ptr += 3, ovr += 3 )
                        {
                            ptr[RGB_R] = (byte) ( ptr[RGB_G] * 0.7 + ptr[RGB_B] * 0.3 );
                            ptr[RGB_G] = ovr[RGB_G];
                            ptr[RGB_B] = ovr[RGB_B];
                        }
                        ptr += offset;
