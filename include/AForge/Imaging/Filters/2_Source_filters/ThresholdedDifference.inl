...
namespace AForge.Imaging.Filters
{
    ...
    public class ThresholdedDifference : BaseFilter2
    {
        ...
        public ThresholdedDifference( )
        {
            ...
            formatTranslations[PixelFormat_24bppRgb]    = PixelFormat_8bppIndexed;
            formatTranslations[PixelFormat_32bppRgb]    = PixelFormat_8bppIndexed;
            formatTranslations[PixelFormat_32bppArgb]   = PixelFormat_8bppIndexed;
            formatTranslations[PixelFormat_32bppPArgb]  = PixelFormat_8bppIndexed;
        }
        ...
        /// 
        protected override  void ProcessFilter( UnmanagedImage sourceData, UnmanagedImage overlay, UnmanagedImage destinationData )
        {
            whitePixelsCount = 0;
            ...
            int pixelSize = Bitmap.GetPixelFormatSize( sourceData->t ) / 8;
            ...
            if ( pixelSize == 1 )
            {
                ...
                int ovrOffset = overlay->s - width;
                int dstOffset = destinationData->s - width;

                // for each line
                for (y = 0; y < height; y++ )
                {
                    // for each pixel
                    for (x = 0; x < width; x++, src++, ovr++, dst++ )
                    {
                        int diff = *src - *ovr;
            ...
            {
                // color image
                int srcOffset = sourceData->s - pixelSize * width;
                int ovrOffset = overlay->s - pixelSize * width;
                int dstOffset = destinationData->s - width;
                ...
                {
                    // for each pixel
                    for (x = 0; x < width; x++, src += pixelSize, ovr += pixelSize, dst++ )
                    {
                        int diffR = src[RGB_R] - ovr[RGB_R];
                        int diffG = src[RGB_G] - ovr[RGB_G];
                        int diffB = src[RGB_B] - ovr[RGB_B];
