...
namespace AForge.Imaging.Filters
{
    ...
    /// corners.Add( new IPOINT(  99,  99 ) );
    /// corners.Add( new IPOINT( 156,  79 ) );
    /// corners.Add( new IPOINT( 184, 126 ) );
    /// corners.Add( new IPOINT( 122, 150 ) );
    /// // create filter
    ...
    public class BackwardQuadrilateralTransformation : BaseInPlaceFilter
    {
        ...
        private List<IPOINT> destinationQuadrilateral = null;
        ...
        /// </para></remarks>
        ///
        public List<IPOINT> DestinationQuadrilateral
        {
        ...
        /// 
        public BackwardQuadrilateralTransformation( Bitmap sourceImage, List<IPOINT> destinationQuadrilateral )
            : this( )
        {
            this.sourceImage = sourceImage;
        ...
        /// <param name="destinationQuadrilateral">Quadrilateral in destination image to transform into.</param>
        /// 
        public BackwardQuadrilateralTransformation( UnmanagedImage sourceUnmanagedImage, List<IPOINT> destinationQuadrilateral )
            : this( )
        {
            this.sourceUnmanagedImage = sourceUnmanagedImage;
            this.destinationQuadrilateral = destinationQuadrilateral;
        ...
        protected override  void ProcessFilter( UnmanagedImage image )
        {
            ...
            {
                // source and destination images must have same pixel format
                if ( image->t != sourceUnmanagedImage->t )
                    throw new InvalidImagePropertiesException( "Source and destination images must have same pixel format." );
        ...
        private  void ProcessFilter( UnmanagedImage dstImage, UnmanagedImage srcImage )
        {
            ...
            int pixelSize = Image_GetPixelFormatSize( srcImage->t ) / 8;
            int srcStride = srcImage->s;
            int dstStride = dstImage->s;
            ...
            List<IPOINT> srcRect = new List<IPOINT>( );
            srcRect.Add( new IPOINT( 0, 0 ) );
            srcRect.Add( new IPOINT( srcWidth - 1, 0 ) );
            srcRect.Add( new IPOINT( srcWidth - 1, srcHeight - 1 ) );
            srcRect.Add( new IPOINT( 0, srcHeight - 1 ) );

            double[,] matrix = QuadTransformationCalcs.MapQuadToQuad( destinationQuadrilateral, srcRect );

            // do the job
            byte* ptr = (byte*) dstImage->tt.data;
            byte* baseSrc = (byte*) srcImage->tt.data;
                ...
                for (y = startY; y < stopY; y++ )
                {
                    ...
                    for (x = startX; x < stopX; x++ )
                    {
                        ...
                        if ( ( srcX >= 0 ) && ( srcY >= 0 ) && ( srcX < srcWidth ) && ( srcY < srcHeight ) )
                        {
                            ...
                            for (i = 0; i < pixelSize; i++, ptr++, p1++, p2++, p3++, p4++ )
                            {
                                *ptr = (byte) (
                                    dy2 * ( dx2 * ( *p1 ) + dx1 * ( *p2 ) ) +
                                    dy1 * ( dx2 * ( *p3 ) + dx1 * ( *p4 ) ) );
