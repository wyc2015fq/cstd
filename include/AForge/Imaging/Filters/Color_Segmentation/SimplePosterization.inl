...
namespace AForge.Imaging.Filters
{
    ...
    public class SimplePosterization : BaseInPlacePartialFilter
    {
        ...
        protected override  void ProcessFilter( UnmanagedImage image, Rectangle rect )
        {
            ...
            for (i = 0; i < 256; i++ )
            {
                map[i] = (byte) MIN( 255, ( i / posterizationInterval ) * posterizationInterval + posterizationOffset );
            }
            ...
            else
            {
                ...
                for (y = startY; y < stopY; y++ )
                {
                    ...
                    {
                        ptr[RGB_R] = map[ptr[RGB_R]];
                        ptr[RGB_G] = map[ptr[RGB_G]];
                        ptr[RGB_B] = map[ptr[RGB_B]];
                    }
                    ptr += offset;
                }
            }
