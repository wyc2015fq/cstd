...
namespace AForge.Imaging.Filters
{
    ...
    public class ErrorDiffusionToAdjacentNeighbors : ErrorDiffusionDithering
    {
        ...
        /// 
        protected override  void Diffuse( int error, byte* ptr )
        {
            ...
            for (jI = 1, jC = 0, k = coefficientsRow.Length; jC < k; jI++, jC++ )
            {
                if ( x + jI >= stopX )
                    break;
            ...
            for (i = 1, n = coefficients.Length; i < n; i++ )
            {
                ...
                // process the row
                for (jC = 0, k = coefficientsRow.Length, jI = -( k >> 1 ); jC < k; jI++, jC++ )
                {
                    if ( x + jI >= stopX )
                        break;
                    if ( x + jI < startX )
