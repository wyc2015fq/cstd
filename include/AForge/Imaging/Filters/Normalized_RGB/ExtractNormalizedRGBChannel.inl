...
namespace AForge.Imaging.Filters
{
    ...
    public class ExtractNormalizedRGBChannel : BaseFilter
    {
        ...
        protected override  void ProcessFilter( UnmanagedImage sourceData, UnmanagedImage destinationData )
        {
            ...
            if ( pixelSize <= 4 )
            {
                int srcOffset = sourceData->s - width * pixelSize;
                ...
                for (y = 0; y < height; y++ )
                {
                    for (x = 0; x < width; x++, src += pixelSize, dst++ )
                    {
                        sum = ( src[RGB_R] + src[RGB_G] + src[RGB_B] );
            ...
            else
            {
                ...
                byte* srcBase   = (byte*) sourceData->tt.data;
                byte* dstBase   = (byte*) destinationData->tt.data;
