...
namespace AForge.Imaging.Filters
{
    ...
    public abstract class BaseInPlaceFilter : IFilter, IInPlaceFilter, IFilterInformation
    {
        ...
        public Bitmap Apply( BitmapData imageData )
        {
            ...
            PixelFormat dstPixelFormat = imageData->t;
            ...
            Bitmap dstImage = ( dstPixelFormat == PixelFormat_8bppIndexed ) ?
            ...
            // copy image
            CopyUnmanagedMemory( dstData.Scan0, imageData.Scan0, imageData->s * height );
        ...
        {
            // check pixel format of the source image
            CheckSourceFormat( image->t );

            // create new destination image
            UnmanagedImage dstImage = imsetsize( image.Width, image.Height, image->t );
        ...
        {
            // check pixel format of the source image
            CheckSourceFormat( sourceImage->t );

            // ensure destination image has correct format
            if ( destinationImage->t != sourceImage->t )
            {
            ...
            int dstStride = destinationImage->s;
            int srcStride = sourceImage->s;
            int lineSize  = MIN( srcStride, dstStride );

            
            {
                byte* dst = (byte*) destinationImage->tt.data;
                byte* src = (byte*) sourceImage->tt.data;

                // copy image
                for (y = 0, height = sourceImage.Height; y < height; y++ )
                {
