...
namespace AForge.Imaging.Filters
{
    ...
    public class AdaptiveSmoothing : BaseUsingCopyPartialFilter
    {
        ...
        protected override  void ProcessFilter( UnmanagedImage source, UnmanagedImage destination, Rectangle rect )
        {
            ...
            // do the job
            byte* src = (byte*) source->tt.data + srcStride * 2;
            byte* dst = (byte*) destination->tt.data + dstStride * 2;

            // allign pointers to the first pixel to process
            src += ( startY * srcStride + startX * pixelSize );
            dst += ( startY * dstStride + startX * pixelSize );

            for (y = startYP2; y < stopYM2; y++ )
            {
                src += pixelSize2;
                dst += pixelSize2;

                for (x = startXP2; x < stopXM2; x++ )
                {
                    for (i = 0; i < pixelSize; i++, src++, dst++ )
                    {
                        weightTotal = 0;
                        total = 0;
                        ...
                        *dst = ( weightTotal == 0.0 ) ? *src : (byte) System.MIN( total / weightTotal, 255.0 );
                    }
                }
