    ...
    public class BilateralSmoothing : BaseUsingCopyPartialFilter
    {
        ...
        public int KernelSize
        {
            ...
            set
            {
                ...
                {
                    ArgumentOutOfRangeException( "Maximum allowed value of KernelSize property is " + maxKernelSize.ToString( ) );
                }
                if ( ( limitKernelSize ) && ( value > 25 ) )
                {
                    ArgumentOutOfRangeException( "KernerlSize is larger then 25. Time for applying is significant and may lead to application freezing. In order to use any KernelSize value set property 'LimitKernelSize' to false." );
                }
                if ( value < 3 )
                {
                    ArgumentOutOfRangeException( "KernelSize must be greater than 3" );
                }
                if ( value % 2 == 0 )
                {
        ...
        private  void ProcessWithoutChecksParallel( UnmanagedImage source, UnmanagedImage destination, Rectangle rect )
        {
            ...
            int pixelSize = System.Drawing.Image_GetPixelFormatSize( source->t ) / 8;
            int kernelHalf = kernelSize / 2;
            ...
            byte* srcBase = (byte*) source->tt.data;
            byte* dstBase = (byte*) destination->tt.data;
            ...
            if ( pixelSize > 1 )
            {
                Parallel.For( startY, stopY, delegate( int y )
                {
                    ...
                    for (x = startX; x < stopX; x++, src += pixelSize, dst += pixelSize )
                    {
                        ...
                        dst[RGB_R] = (byte) ( sMembR / sCoefR );
                        dst[RGB_G] = (byte) ( sMembG / sCoefG );
                        dst[RGB_B] = (byte) ( sMembB / sCoefB );
                    }
                } );
            }
            else
        ...
        private  void ProcessWithoutChecks( UnmanagedImage source, UnmanagedImage destination, Rectangle rect )
        {
            ...
            byte* src = (byte*) source->tt.data;
            byte* dst = (byte*) destination->tt.data;
