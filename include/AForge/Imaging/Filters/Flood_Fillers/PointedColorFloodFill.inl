...
namespace AForge.Imaging.Filters
{
    ...
    /// filter.Tolerance = Color_rgb( 150, 92, 92 );
    /// filter.FillColor = Color_rgb( 255, 255, 255 );
    /// filter.StartingPoint = new IPOINT( 150, 100 );
    /// // apply the filter
    ...
    public  class PointedColorFloodFill : BaseInPlacePartialFilter
    {
        ...
        private IPOINT startingPoint = new IPOINT( 0, 0 );
        // filling tolerance
        private Color tolerance = Color_rgb( 0, 0, 0 );
        ...
        {
            get { return Color_rgb( fillR, fillG, fillB ); }
            set
            {
                fillR = value.R;
                fillG = value.G;
        ...
        protected override  void ProcessFilter( UnmanagedImage image, Rectangle rect )
        {
            ...
            if ( image->t == PixelFormat_8bppIndexed )
            {
                ...
                minG = (byte) ( MAX(   0, startColor - tolerance.G ) );
                maxG = (byte) ( MIN( 255, startColor + tolerance.G ) );
            ...
            else
            {
                ...
                minR = (byte) ( MAX(   0, startColor[RGB_R] - tolerance.R ) );
                maxR = (byte) ( MIN( 255, startColor[RGB_R] + tolerance.R ) );
                minG = (byte) ( MAX(   0, startColor[RGB_G] - tolerance.G ) );
                maxG = (byte) ( MIN( 255, startColor[RGB_G] + tolerance.G ) );
                minB = (byte) ( MAX(   0, startColor[RGB_B] - tolerance.B ) );
                maxB = (byte) ( MIN( 255, startColor[RGB_B] + tolerance.B ) );
        ...
        private  void LinearFloodFill4Gray( IPOINT startingPoint )
        {
            Queue<IPOINT> points = new Queue<IPOINT>( );
            points.Enqueue( startingPoint );

            while ( points.Count > 0 )
            {
                IPOINT point = points.Dequeue( );
                ...
                for (i = leftLineEdge; i <= rightLineEdge; i++, ptr++ )
                {
                    // go up
                    if ( ( y > startY ) && ( !checkedPixels[y - 1, i] ) && ( CheckGrayPixel( *( ptr - stride ) ) ) )
                    {
                        ...
                        {
                            points.Enqueue( new IPOINT( i, upperY ) );
                    ...
                    if ( ( y < stopY ) && ( !checkedPixels[y + 1, i] ) && ( CheckGrayPixel( *( ptr + stride ) ) ) )
                    {
                        ...
                        {
                            points.Enqueue( new IPOINT( i, lowerY ) );
        ...
        private  void LinearFloodFill4RGB( IPOINT startPoint )
        {
            Queue<IPOINT> points = new Queue<IPOINT>( );
            points.Enqueue( startingPoint );
            ...
            while ( points.Count > 0 )
            {
                ...
                for (i = leftLineEdge; i <= rightLineEdge; i++, ptr += 3 )
                {
                    // go up
        ...
        {
            return  ( pixel[RGB_R] >= minR ) && ( pixel[RGB_R] <= maxR ) &&
                    ( pixel[RGB_G] >= minG ) && ( pixel[RGB_G] <= maxG ) &&
                    ( pixel[RGB_B] >= minB ) && ( pixel[RGB_B] <= maxB );
        }
