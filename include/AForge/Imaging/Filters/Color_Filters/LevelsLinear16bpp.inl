...
namespace AForge.Imaging.Filters
{
    ...
    public class LevelsLinear16bpp : BaseInPlacePartialFilter
    {
        ...
        {
            int pixelSize = Image_GetPixelFormatSize( image->t ) / 16;
            ...
            if ( image->t == PixelFormat_16bppGrayScale )
            {
                ...
                {
                    ushort* ptr = (ushort*) ( basePtr + y * image->s ) + startX;
            ...
            else
            {
                ...
                for (y = startY; y < stopY; y++ )
                {
                    ushort* ptr = (ushort*) ( basePtr + y * image->s ) + startX * pixelSize;

                    for (x = startX; x < stopX; x++, ptr += pixelSize )
                    {
                        // red
                        ptr[RGB_R] = mapRed[ptr[RGB_R]];
                        // green
                        ptr[RGB_G] = mapGreen[ptr[RGB_G]];
                        // blue
                        ptr[RGB_B] = mapBlue[ptr[RGB_B]];
                    }
                }
            }
        }
