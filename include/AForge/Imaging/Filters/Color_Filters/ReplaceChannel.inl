...
namespace AForge.Imaging.Filters
{
    ...
    public class ReplaceChannel : BaseInPlacePartialFilter
    {
        ...
        public Bitmap ChannelImage
        {
            ...
            set
            {
                if ( value != null )
                {
                    ...
                    if ( ( value->t != PixelFormat_8bppIndexed ) &&
                         ( value->t != PixelFormat_16bppGrayScale ) )
        ...
        private ReplaceChannel( )
        {
            ...
            formatTranslations[PixelFormat_48bppRgb]  = PixelFormat_16bppGrayScale;
            formatTranslations[PixelFormat_64bppArgb] = PixelFormat_16bppGrayScale;
        }
        ...
        protected override  void ProcessFilter( UnmanagedImage image, Rectangle rect )
        {
            ...
            int chStride = 0;
            PixelFormat chFormat = PixelFormat_16bppGrayScale;
            ...
            if ( channelImage != null )
            {
                ...
                chData = channelImage.LockBits(
                    ...
                    ImageLockMode.ReadOnly, channelImage->t );

                ch = (byte*) chData->tt.data;
                chStride = chData->s;
                chFormat = chData->t;
            }
            else
            {
                ...
                chStride = unmanagedChannelImage->s;
                chFormat = unmanagedChannelImage->t;
            }

            if ( pixelSize <= 4 )
            {
                // check channel image's format
                if ( chFormat != PixelFormat_8bppIndexed )
                    throw new InvalidImagePropertiesException( "Channel image's format does not correspond to format of the source image." );

                int offsetCh = chData->s - rect.Width;

                // do the job
                byte* dst = (byte*) image->tt.data;

                // allign pointers to the first pixel to process
                dst += ( startY * image->s + startX * pixelSize );
                ...
                {
                    // for each pixel
                    for (x = startX; x < stopX; x++, dst += pixelSize, ch++ )
                    {
                        dst[channel] = *ch;
                    }
                    dst += offset;
            ...
            {
                // check channel image's format
                if ( chFormat != PixelFormat_16bppGrayScale )
                    throw new InvalidImagePropertiesException( "Channel image's format does not correspond to format of the source image." );

                int stride = image->s;

                // do the job
                byte* baseDst = (byte*) image->tt.data;
                ...
                for (y = startY; y < stopY; y++ )
                {
                    ...
                    // for each pixel
                    for (x = startX; x < stopX; x++, dst += pixelSize, chPtr++ )
                    {
                        dst[channel] = *chPtr;
                    }
                }
