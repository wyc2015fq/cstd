        ...
        protected override  void ProcessFilter( UnmanagedImage sourceData, UnmanagedImage destinationData )
        {
            ...
            if ( !performDemosaicing )
            {
                ...
                for (y = 0; y < height; y++ )
                {
                    ...
                    {
                        dst[RGB_R] = dst[RGB_G] = dst[RGB_B] = 0;
            ...
            else
            {
                ...
                for (y = 0; y < height; y++ )
                {
                    ...
                    for (x = 0; x < width; x++, src++, dst += 3 )
                    {
                        ...
                        dst[RGB_R] = (byte) ( rgbValues[RGB_R] / rgbCounters[RGB_R] );
                        dst[RGB_G] = (byte) ( rgbValues[RGB_G] / rgbCounters[RGB_G] );
                        dst[RGB_B] = (byte) ( rgbValues[RGB_B] / rgbCounters[RGB_B] );
                    }
                    src += srcOffset;
                    dst += dstOffset;
                }
