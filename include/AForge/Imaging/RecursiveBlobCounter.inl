...
namespace AForge.Imaging
{
    ...
    public class RecursiveBlobCounter : BlobCounterBase
    {
        ...
        {
            get { return Color_rgb( backgroundThresholdR, backgroundThresholdG, backgroundThresholdB ); }
            set
            {
        ...
        protected override void BuildObjectsMap( UnmanagedImage image )
        {
            ...
            for (x = 0, mx = imageWidth + 2; x < mx; x++ )
            {
                tempLabels[x] = -1;
                tempLabels[x + ( imageHeight + 1 ) * ( imageWidth + 2 )] = -1;
            }
            for (y = 0, my = imageHeight + 2; y < my; y++ )
            {
            ...
            
            {
                ...
                {
                    pixelSize = Bitmap.GetPixelFormatSize( image->t ) / 8;
                    ...
                    {
                        // for each pixel
                        for (x = 0; x < imageWidth; x++, src += pixelSize, p++ )
                        {
                            // check for non-labeled pixel
                            if ( (
                                    ( src[RGB_R] > backgroundThresholdR ) ||
                                    ( src[RGB_G] > backgroundThresholdG ) ||
                                    ( src[RGB_B] > backgroundThresholdB )
                                  ) && 
                                ( tempLabels[p] == 0 ) )
                            {
                                objectsCount++;
        ...
        private  void LabelColorPixel( byte* pixel, int labelPointer )
        {
            if ( ( tempLabels[labelPointer] == 0 ) && (
                ( pixel[RGB_R] > backgroundThresholdR ) ||
                ( pixel[RGB_G] > backgroundThresholdG ) ||
                ( pixel[RGB_B] > backgroundThresholdB ) ) )
            {
                tempLabels[labelPointer] = objectsCount;
