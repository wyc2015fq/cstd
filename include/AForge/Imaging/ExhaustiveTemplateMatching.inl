    ...
    public class ExhaustiveTemplateMatching : ITemplateMatching
    {
        ...
        public TemplateMatch[] ProcessImage( Bitmap image, Bitmap template, Rectangle searchZone )
        {
            ...
            if (
                ...
                ( image->t != template->t ) )
            {
                UnsupportedImageFormatException( "Unsupported pixel format of the source or template image." );
            }
            ...
            BitmapData templateData = template.LockBits(
                ...
                ImageLockMode.ReadOnly, template->t );
        ...
        public TemplateMatch[] ProcessImage( UnmanagedImage image, UnmanagedImage template, Rectangle searchZone )
        {
            ...
            {
                byte* baseSrc = (byte*) image->tt.data;
                byte* baseTpl = (byte*) template->tt.data;

                int sourceOffset = image->s - templateWidth * pixelSize;
                int templateOffset = template->s - templateWidth * pixelSize;
            ...
            // for each row
            for (y = 2, maxY = mapHeight + 2; y < maxY; y++ )
            {
                // for each pixel
                for (x = 2, maxX = mapWidth + 2; x < maxX; x++ )
                {
                    int currentValue = map[y, x];

                    // for each windows' row
                    for (i = -2; ( currentValue != 0 ) && ( i <= 2 ); i++ )
                    {
