...
namespace AForge.Imaging
{
    ...
    ///         new IPOINT( (int) x0 + w2, h2 - (int) y0 ),
    ///         new IPOINT( (int) x1 + w2, h2 - (int) y1 ),
    ///         Color.Red );
    /// }
    /// </code>
    /// 
    ...
    public class HoughLineTransformation
    {
        ...
        public Bitmap ToBitmap( )
        {
            ...
            int offset = imageData->s - width;
            float scale = 255.0f / maxMapIntensity;

            // do the job
            
            {
                byte * dst = (byte*) imageData->tt.data;

                for (y = 0; y < height; y++ )
                {
                    for (x = 0; x < width; x++, dst++ )
                    {
                        *dst = (byte) System.MIN( 255, (int) ( scale * houghMap[y, x] ) );
                    }
                    dst += offset;
                }
            }
