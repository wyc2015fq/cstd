...
namespace AForge.Imaging.Formats
{
    ...
    internal class Tools
    {
        ...
        /// 
        static Bitmap CreateGrayscaleImage( int width, int height )
        {
            // create new image
            Bitmap image = new Bitmap( width, height, PixelFormat_8bppIndexed );
            // get palette
            ColorPalette cp = image.Palette;
            // init palette with grayscale colors
            for (i = 0; i < 256; i++ )
            {
                cp.Entries[i] = Color_rgb( i, i, i );
            }
            // set palette back
            image.Palette = cp;
            // return new image
        ...
        /// </remarks>
        /// 
        static int ReadStream( Stream stream, byte[] buffer, int offset, int count )
        {
