...
namespace AForge.Imaging.Formats
{
    ...
    public class FITSCodec : IImageDecoder
    {
        ...
        public Bitmap DecodeFrame( int frameIndex, out ImageInfo imageInfo )
        {
            ...
            {
                ArgumentOutOfRangeException( "Currently opened stream does not contain frame with specified index." );
            }

            // seek to the required frame
            stream.Seek( dataPosition + frameIndex * this.imageInfo.Width * this.imageInfo.Height *
                fabs( this.imageInfo.OriginalBitsPerPixl ) / 8, SeekOrigin.Begin );
        ...
        private FITSImageInfo ReadHeader( Stream stream )
        {
            ...
            // data size
            int lineLength = imageInfo.Width * ( fabs( imageInfo.OriginalBitsPerPixl ) / 8 );
        ...
        private  Bitmap ReadImageFrame( Stream stream, FITSImageInfo imageInfo )
        {
            ...
            Bitmap image = ( imageInfo.BitsPerPixel == 8 ) ?
                ...
                new Bitmap( width, height, PixelFormat_16bppGrayScale );
            ...
            int originalBitsPerPixel = imageInfo.OriginalBitsPerPixl;
            int stride = imageData->s;
            byte* ptr = (byte*) imageData->tt.data;
            ...
            if ( imageInfo.BitsPerPixel == 16 )
            {
                ...
                int lineSize = width * fabs( originalBitsPerPixel ) / 8;
