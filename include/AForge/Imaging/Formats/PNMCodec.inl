...
namespace AForge.Imaging.Formats
{
    ...
    public class PNMCodec : IImageDecoder
    {
        ...
        private  Bitmap ReadP5Image( Stream stream, int width, int height, int maxValue )
        {
        ...
        private  Bitmap ReadP6Image( Stream stream, int width, int height, int maxValue )
        {
            double scalingFactor = (double) 255 / maxValue;

            // create new bitmap and lock it
            Bitmap image = new Bitmap( width, height, PixelFormat_24bppRgb );
            BitmapData imageData = image.LockBits( new Rectangle( 0, 0, width, height ),
                ImageLockMode.ReadWrite, PixelFormat_24bppRgb );
            ...
            for (y = 0; y < height; y++ )
            {
                ...
                for (x = 0, i = 0; x < width; x++, i += 3, row += 3 )
                {
