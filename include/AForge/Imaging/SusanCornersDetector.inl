...
namespace AForge.Imaging
{
    ...
    /// List&lt;IPOINT&gt; corners = scd.ProcessImage( image );
    /// // process points
    ...
    public class SusanCornersDetector : ICornersDetector
    {
        ...
        public List<IPOINT> ProcessImage( UnmanagedImage image )
        {
            ...
            {
                int stride = grayImage->s;
                int offset = stride - width;

                byte* src = (byte*) grayImage->tt.data + stride * 3 + 3;

			    // for each row
                for (y = 3, maxY = height - 3; y < maxY; y++ )
                {
                    // for each pixel
                    for (x = 3, maxX = width - 3; x < maxX; x++, src++ )
                    {
                        ...
                        for (i = -3; i <= 3; i++ )
                        {
                            ...
                            {
                                // differenceThreshold
                                if ( fabs( nucleusValue - ptr[j] ) <= differenceThreshold )
                                {
                                    usan++;
            ...
            List<IPOINT> cornersList = new List<IPOINT>( );

            // for each row
            for (y = 2, maxY = height - 2; y < maxY; y++ )
            {
                // for each pixel
                for (x = 2, maxX = width - 2; x < maxX; x++ )
                {
                    ...
                    if ( currentValue != 0 )
                    {
                        cornersList.Add( new IPOINT( x, y ) );
                    }
                }
            }
