    ...
    public class MoravecCornersDetector : ICornersDetector
    {
        ...
        public List<IPOINT> ProcessImage( UnmanagedImage image )
        {
            ...
            
            {
                ...
                // for each row
                for (y = windowRadius, maxY = height - windowRadius; y < maxY; y++ )
                {
                    // for each pixel
                    for (x = windowRadius, maxX = width - windowRadius; x < maxX; x++ )
                    {
                        ...
                        for (k = 0; k < 8; k++ )
                        {
                            ...
                            for (i = 0; i < windowSize; i++ )
                            {
                                ...
                                for (j = 0, maxJ = windowSize * pixelSize; j < maxJ; j++, ptr1++, ptr2++ )
                                {
                                    int dif = *ptr1 - *ptr2;
                                    sum += dif * dif;
                                }
            ...
            for (y = windowRadius, maxY = height - windowRadius; y < maxY; y++ )
            {
                ...
                for (x = windowRadius, maxX = width - windowRadius; x < maxX; x++ )
                {
                    ...
                    // for each windows' rows
                    for (i = -windowRadius; ( currentValue != 0 ) && ( i <= windowRadius ); i++ )
                    {
                        // for each windows' pixels
