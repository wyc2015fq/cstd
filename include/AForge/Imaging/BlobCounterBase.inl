    ...
    public abstract class BlobCounterBase
    {
        ...
        public void ProcessImage( UnmanagedImage image )
        {
            ...
            if ( filterBlobs )
            {
                ...
                for (i = 0, n = objectLabels.Length; i < n; i++ )
                {
        ...
        public Blob[] GetObjects( UnmanagedImage image, bool extractInOriginalSize )
        {
            ...
            if (
                ...
                ( image->t != PixelFormat_32bppRgb ) &&
                ( image->t != PixelFormat_32bppArgb ) &&
                ( image->t != PixelFormat_32bppRgb ) &&
                ( image->t != PixelFormat_32bppPArgb )
                )
            ...
            for (k = 0; k < objectsCount; k++ )
            {
                ...
                // create new image
                UnmanagedImage dstImage = imsetsize( blobImageWidth, blobImageHeight, image->t );

                // copy image
                
                {
                    byte* src = (byte*) image->tt.data + ymin * srcStride + xmin * pixelSize;
                    byte* dst = (byte*) dstImage->tt.data;
                    int p = ymin * width + xmin;

                    if ( extractInOriginalSize )
                    {
                        // allign destination pointer also
                        dst += ymin * dstImage->s + xmin * pixelSize;
                    }

                    int srcOffset = srcStride - objectWidth * pixelSize;
                    int dstOffset = dstImage->s - objectWidth * pixelSize;
                    ...
                    {
                        // copy each pixel
                        for (x = xmin; x <= xmax; x++, p++, dst += pixelSize, src += pixelSize )
                        {
        ...
        public void ExtractBlobsImage( UnmanagedImage image, Blob blob, bool extractInOriginalSize )
        {
            ...
            // create new image
            blob.Image = imsetsize( blobImageWidth, blobImageHeight, image->t );
            ...
            {
                byte* src = (byte*) image->tt.data + ymin * srcStride + xmin * pixelSize;
                byte* dst = (byte*) blob.Image->tt.data;
                int p = ymin * width + xmin;

                if ( extractInOriginalSize )
                {
                    // allign destination pointer also
                    dst += ymin * blob.Image->s + xmin * pixelSize;
                }

                int srcOffset = srcStride - objectWidth * pixelSize;
                int dstOffset = blob.Image->s - objectWidth * pixelSize;
                int labelsOffset = width - objectWidth;
        ...
        /// can not be extracted.</exception>
        /// 
        public void GetBlobsLeftAndRightEdges( Blob blob, out List<IPOINT> leftEdge, out List<IPOINT> rightEdge )
        {
            // check if objects map was collected
            if ( objectLabels == null )
                throw new ApplicationException( "Image should be processed before to collect objects map." );

            leftEdge  = new List<IPOINT>( );
            rightEdge = new List<IPOINT>( );
            ...
            for (y = ymin; y <= ymax; y++ )
            {
                ...
                p = y * imageWidth + xmax;
                for (x = xmax; x >= xmin; x--, p-- )
                {
                    if ( objectLabels[p] == label )
                    {
                        rightEdge.Add( new IPOINT( x, y ) );
                        break;
                    }
                }
            }
        ...
        /// can not be extracted.</exception>
        /// 
        public void GetBlobsTopAndBottomEdges( Blob blob, out List<IPOINT> topEdge, out List<IPOINT> bottomEdge )
        {
            // check if objects map was collected
            if ( objectLabels == null )
                throw new ApplicationException( "Image should be processed before to collect objects map." );

            topEdge    = new List<IPOINT>( );
            bottomEdge = new List<IPOINT>( );
            ...
            for (x = xmin; x <= xmax; x++ )
            {
                ...
                for (y = ymin; y <= ymax; y++, p += imageWidth )
                {
                    if ( objectLabels[p] == label )
                    {
                        topEdge.Add( new IPOINT( x, y ) );
                        break;
                    }
                }
                ...
                p = ymax * imageWidth + x;
                for (y = ymax; y >= ymin; y--, p -= imageWidth )
                {
                    if ( objectLabels[p] == label )
                    {
                        bottomEdge.Add( new IPOINT( x, y ) );
                        break;
                    }
                }
            }
        ...
        /// can not be extracted.</exception>
        /// 
        public List<IPOINT> GetBlobsEdgePoints( Blob blob )
        {
            // check if objects map was collected
            if ( objectLabels == null )
                throw new ApplicationException( "Image should be processed before to collect objects map." );

            List<IPOINT> edgePoints = new List<IPOINT>( );
            ...
            for (x = xmin; x <= xmax; x++ )
            {
                // scan from top to bottom
                int p = ymin * imageWidth + x;
                for (y = ymin, y0 = 0; y <= ymax; y++, y0++, p += imageWidth )
                {
                    if ( objectLabels[p] == label )
                    {
                        // avoid adding the point we already have
                ...
                // scan from bottom to top
                p = ymax * imageWidth + x;
                for (y = ymax, y0 = ymax - ymin; y >= ymin; y--, y0--, p -= imageWidth )
                {
                    if ( objectLabels[p] == label )
                    {
                        // avoid adding the point we already have
        ...
        private  void CollectObjectsInfo( UnmanagedImage image )
        {
            ...
            {
                int offset = image->s - imageWidth;
                byte g; // pixel's grey value

                // walk through labels array
                for (y = 0; y < imageHeight; y++ )
                {
                    for (x = 0; x < imageWidth; x++, i++, src++ )
                    {
                        // get current label
            ...
            else
            {
                // color images
                int pixelSize = Bitmap.GetPixelFormatSize( image->t ) / 8;
                int offset = image->s - imageWidth * pixelSize;
                byte r, g, b; // RGB value

                // walk through labels array
                for (y = 0; y < imageHeight; y++ )
                {
                    for (x = 0; x < imageWidth; x++, i++, src += pixelSize )
                    {
                        // get current label
                        label = objectLabels[i];
            ...
            for (j = 1; j <= objectsCount; j++ )
            {
                ...
                blob.ColorMean = Color_rgb( (byte) ( meanR[j] / blobArea ), (byte) ( meanG[j] / blobArea ), (byte) ( meanB[j] / blobArea ) );
                blob.ColorStdDev = Color_rgb(
                    (byte) ( sqrt( stdDevR[j] / blobArea - blob.ColorMean.R * blob.ColorMean.R ) ),
                    (byte) ( sqrt( stdDevG[j] / blobArea - blob.ColorMean.G * blob.ColorMean.G ) ),
                    (byte) ( sqrt( stdDevB[j] / blobArea - blob.ColorMean.B * blob.ColorMean.B ) ) );
