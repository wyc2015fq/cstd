...
namespace AForge.Imaging.ColorReduction
{
    ...
    public class ColorErrorDiffusionToAdjacentNeighbors : ErrorDiffusionColorDithering
    {
        ...
        /// 
        protected override  void Diffuse( int rError, int gError, int bError, byte* ptr )
        {
            ...
            for (jI = 1, jP = pixelSize, jC = 0, k = coefficientsRow.Length; jC < k; jI++, jC++, jP += pixelSize )
            {
                if ( x + jI >= width )
                    break;
            ...
            for (i = 1, n = coefficients.Length; i < n; i++ )
            {
                if ( y + i >= height )
                    break;
                ...
                // process the row
                for (jC = 0, k = coefficientsRow.Length, jI = -( k >> 1 ), jP = -( k >> 1 ) * pixelSize; jC < k; jI++, jC++, jP += pixelSize )
                {
                    if ( x + jI >= width )
                        break;
                    if ( x + jI < 0 )
        ...
        private void CalculateCoefficientsSum( )
        {
            ...
            for (i = 0, n = coefficients.Length; i < n; i++ )
            {
                int[] coefficientsRow = coefficients[i];

                for (j = 0, k = coefficientsRow.Length; j < k; j++ )
                {
