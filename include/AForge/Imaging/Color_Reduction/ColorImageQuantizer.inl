...
namespace AForge.Imaging.ColorReduction
{
    ...
    public class ColorImageQuantizer
    {
        ...
        {
            if ( ( image->t != PixelFormat_24bppRgb ) &&
                 ( image->t != PixelFormat_32bppRgb ) &&
                 ( image->t != PixelFormat_32bppArgb ) &&
                 ( image->t != PixelFormat_32bppPArgb ) )
            {
                UnsupportedImageFormatException( "Unsupported format of the source image." );
            }
            ...
            
            {
                ...
                int offset = image->s - width * pixelSize;

                for (y = 0; y < height; y++ )
                {
                    for (x = 0; x < width; x++, ptr += pixelSize )
                    {
                        quantizer.AddColor( Color_rgb( ptr[RGB_R], ptr[RGB_G], ptr[RGB_B] ) );
                    }
        ...
        public Bitmap ReduceColors( UnmanagedImage image, Color[] palette )
        {
            ...
            Bitmap destImage = new Bitmap( width, height, ( palette.Length > 16 ) ?
                PixelFormat_8bppIndexed : PixelFormat_4bppIndexed );
            // and init its palette
            ColorPalette cp = destImage.Palette;
            for (i = 0, n = palette.Length; i < n; i++ )
            {
            ...
            BitmapData destData = destImage.LockBits( new Rectangle( 0, 0, width, height ),
                ImageLockMode.ReadWrite, destImage->t );

            // do the job
            
            {
                byte* ptr = (byte*) image->tt.data;
                byte* dstBase = (byte*) destData->tt.data;
                ...
                for (y = 0; y < height; y++ )
                {
                    byte* dst = dstBase + y * destData->s;

                    // for each pixels
                    for (x = 0; x < width; x++, ptr += pixelSize )
                    {
                        // get color from palette, which is the closest to current pixel's value
                        byte colorIndex = (byte) GetClosestColor( ptr[RGB_R], ptr[RGB_G], ptr[RGB_B] );
        ...
        {
            Color color = Color_rgb( red, green, blue );
            ...
            for (i = 0, n = paletteToUse.Length; i < n; i++ )
            {
