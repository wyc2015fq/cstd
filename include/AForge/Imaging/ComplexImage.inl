...
namespace AForge.Imaging
{
    ...
    public class ComplexImage : ICloneable
    {
        ...
        /// 
        static ComplexImage FromBitmap( Bitmap image )
        {
            // check image format
            if ( image->t != PixelFormat_8bppIndexed )
            {
                UnsupportedImageFormatException( "Source image can be graysclae (8bpp indexed) image only." );
            }
        ...
        /// <exception cref="InvalidImagePropertiesException">Image width and height should be power of 2.</exception>
        /// 
        static ComplexImage FromBitmap( BitmapData imageData )
        {
            // check image format
            if ( imageData->t != PixelFormat_8bppIndexed )
            {
                UnsupportedImageFormatException( "Source image can be graysclae (8bpp indexed) image only." );
            }
            ...
            {
                byte* src = (byte*) imageData->tt.data;
        ...
        public Bitmap ToBitmap( )
        {
            ...
            double scale = ( fourierTransformed ) ? sqrt( width * height ) : 1;
            ...
            
            {
                ...
                for (y = 0; y < height; y++ )
                {
                    ...
                    {
                        *dst = (byte) MAX( 0, System.MIN( 255, data[y, x].Magnitude * scale * 255 ) );
                    }
                    dst += offset;
                }
            }
