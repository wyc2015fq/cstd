    ...
    public class VideoCaptureDevice : IVideoSource
    {
            ...
            get
            {
                if ( crossbarVideoInputs == null )
                {
                    ...
                    if ( crossbarVideoInputs == null )
                    {
                        ...
                        {
                            for (i = 0; ( i < 500 ) && ( crossbarVideoInputs == null ); i++ )
                            {
                                Thread.Sleep( 10 );
                            }
                        }
            ...
            get
            {
                if ( videoCapabilities == null )
                {
                    ...
                    if ( videoCapabilities == null )
                    {
                        ...
                        {
                            for (i = 0; ( i < 500 ) && ( videoCapabilities == null ); i++ )
                            {
                                Thread.Sleep( 10 );
                            }
                        }
            ...
            get
            {
                if ( snapshotCapabilities == null )
                {
                    ...
                    if ( snapshotCapabilities == null )
                    {
                        ...
                        {
                            for (i = 0; ( i < 500 ) && ( snapshotCapabilities == null ); i++ )
                            {
                                Thread.Sleep( 10 );
                            }
                        }
        ...
        public void DisplayCrossbarPropertyPage( IntPtr parentWindow )
        {
            lock ( sync )
            {
                ...
                for (i = 0; ( i < 500 ) && ( !isCrossbarAvailable.HasValue ) && ( IsRunning ); i++ )
                {
                    Thread.Sleep( 10 );
                }
        ...
        public bool CheckIfCrossbarAvailable( )
        {
            lock ( sync )
            {
                if ( !isCrossbarAvailable.HasValue )
                {
                    ...
                    {
                        for (i = 0; ( i < 500 ) && ( !isCrossbarAvailable.HasValue ); i++ )
                        {
                            Thread.Sleep( 10 );
                        }
                    }
        ...
        {
            framesReceived++;
            bytesReceived += image.Width * image.Height * ( Bitmap.GetPixelFormatSize( image->t ) >> 3 );
        ...
        private class Grabber : ISampleGrabberCB
        {
            ...
            public int BufferCB( double sampleTime, IntPtr buffer, int bufferLen )
            {
                ...
                {
                    // create new image
                    System.Drawing.Bitmap image = new Bitmap( width, height, PixelFormat_24bppRgb );

                    // lock bitmap data
                    BitmapData imageData = image.LockBits(
                        new Rectangle( 0, 0, width, height ),
                        ImageLockMode.ReadWrite,
                        PixelFormat_24bppRgb );

                    // copy image data
                    int srcStride = imageData->s;
                    int dstStride = imageData->s;

                    
                    {
                        byte* dst = (byte*) imageData->tt.data + dstStride * ( height - 1 );
