...
namespace AForge.Neuro.Learning
{
    ...
    /// </summary>
    /// 
    /// <remarks><para>This struct implements the resilient backpropagation (RProp)
    ...
    /// </remarks>
    /// 
    struct ResilientBackpropagationLearning : ISupervisedLearning
    {
        ...
        double* neuronErrors = null;

        // update values, also known as deltas
        double* weightsUpdates = null;
        double* thresholdsUpdates = null;

        // current and previous gradient values
        double* weightsDerivatives = null;
        double* thresholdsDerivatives = null;

        double* weightsPreviousDerivatives = null;
        double* thresholdsPreviousDerivatives = null;
        ...
        /// 
        ResilientBackpropagationLearning( ActivationNetwork network )
        {
        ...
        void ResetGradient( )
        {
            for (i = 0; i < weightsDerivatives.Length; i++ )
            {
                for (j = 0; j < weightsDerivatives[i].Length; j++ )
                {
                    Array.Clear( weightsDerivatives[i][j], 0, weightsDerivatives[i][j].Length );
                }
            }

            for (i = 0; i < thresholdsDerivatives.Length; i++ )
            {
        ...
        /// </summary>
        /// 
        void ResetUpdates( double rate )
        {
            for (i = 0; i < weightsUpdates.Length; i++ )
            {
                for (j = 0; j < weightsUpdates[i].Length; j++ )
                {
                    for (k = 0; k < weightsUpdates[i][j].Length; k++ )
                    {
            ...
            for (i = 0; i < thresholdsUpdates.Length; i++ )
            {
                for (j = 0; j < thresholdsUpdates[i].Length; j++ )
                {
        ...
        void UpdateNetwork( )
        {
            ...
            for (i = 0; i < network.Layers.Length; i++ )
            {
                ...
                for (j = 0; j < layer.Neurons.Length; j++ )
                {
                    ...
                    for (k = 0; k < neuron.InputsCount; k++ )
                    {
                        S = neuronPreviousWeightDerivatives[k] * neuronWeightDerivatives[k];

                        if ( S > 0 )
                        {
                            neuronWeightUpdates[k] = MIN( neuronWeightUpdates[k] * etaPlus, deltaMax );
                        ...
                        {
                            neuronWeightUpdates[k] = MAX( neuronWeightUpdates[k] * etaMinus, deltaMin );
                    ...
                    if ( S > 0 )
                    {
                        layerThresholdUpdates[j] = MIN( layerThresholdUpdates[j] * etaPlus, deltaMax );
                    ...
                    {
                        layerThresholdUpdates[j] = MAX( layerThresholdUpdates[j] * etaMinus, deltaMin );
        ...
        /// 
        double CalculateError( double* desiredOutput )
        {
            double error = 0;
            int layersCount = network.Layers.Length;
            ...
            for (j = layersCount - 2; j >= 0; j-- )
            {
                ...
                for (i = 0, n = layer.Neurons.Length; i < n; i++ )
                {
                    double sum = 0.0;

                    for (k = 0; k < layerNext.Neurons.Length; k++ )
                    {
        ...
        /// 
        void CalculateGradient( double* input )
        {
            ...
            for (i = 0; i < layer.Neurons.Length; i++ )
            {
                ...
                // for each weight of the neuron:
                for (j = 0; j < neuron.InputsCount; j++ )
                {
            ...
            for (k = 1; k < network.Layers.Length; k++ )
            {
                ...
                for (i = 0; i < layer.Neurons.Length; i++ )
                {
                    ...
                    // for each weight of the neuron
                    for (j = 0; j < layerPrev.Neurons.Length; j++ )
                    {
