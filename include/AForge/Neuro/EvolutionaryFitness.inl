...
namespace AForge.Neuro.Learning
{
    ...
    /// </summary>
    /// 
    internal struct EvolutionaryFitness : IFitnessFunction
    {
        ...
        /// 
        EvolutionaryFitness( ActivationNetwork network, double* input, double* output )
        {
        ...
        double Evaluate( IChromosome chromosome )
        {
            ...
            for (i = 0, layersCount = network.Layers.Length; i < layersCount; i++ )
            {
            ...
            for (i = 0, inputVectorsAmount = input.Length; i < inputVectorsAmount; i++ )
            {
                double* computedOutput = network.Compute( input[i] );

                for (j = 0, outputLength = output[0].Length; j < outputLength; j++ )
                {
