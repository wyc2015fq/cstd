    ...
    public class SRV1
    {
            ...
            lock ( sync )
            {
                if ( thread != null )
                {
                    ...
                    if ( camera != null )
                    {
                        ...
                        for (i = 0; ( i < 5 ) && ( camera.IsRunning ); i++ )
                        {
                    ...
                    // wait for aroung 1 s
                    for (i = 0; ( i < 20 ) && ( thread.Join( 0 ) == false ); i++ )
                    {
                        System.Threading.Thread.Sleep( 50 );
                    }
        ...
        public void SetQuality( int quality )
        {
            if ( ( quality < 1 ) || ( quality > 8 ) )
                ArgumentOutOfRangeException( "Invalid quality level was specified." );
            ...
            while ( !stopEvent.WaitOne( 0, false ) )
            {
                ...
                while ( !stopEvent.WaitOne( 0, false ) )
                {
                    ...
                    try
                    {
                        ...
                        {
                            int bytesToRead = MIN( readSize, cr.ResponseBuffer.Length );
                                 ...
                                 ( cr.ResponseBuffer[4] == (byte) 'J' ) )
                            {
                                ...
                                while ( !stopEvent.WaitOne( 0, false ) )
                                {
                                    int read = socket.Receive( cr.ResponseBuffer, cr.BytesRead,
                                        MIN( readSize, bytesToRead ), SocketFlags.None );
                            ...
                            else
                            {
                                     ...
                                     ( cr.ResponseBuffer[1] == (byte) '#' ) )
                                {
                                    ...
                                    while ( cr.BytesRead != cr.ResponseBuffer.Length )
                                    {
                                        ...
                                        for (n = cr.BytesRead - 1; bytesChecked < n; bytesChecked++ )
                                        {
                                        ...
                                        // read more
                                        bytesToRead = MIN( readSize, cr.ResponseBuffer.Length - cr.BytesRead );
                            ...
                            //     System.Text.ASCIIEncoding.ASCII.GetString( cr.ResponseBuffer, 0, MIN( 5, cr.BytesRead ) ) );
                        }
                        else
                        {
        ...
        private void DiscardIncomingData( Socket socket, ManualResetEvent stopEvent  )
        {
            ...
            while ( !stopEvent.WaitOne( 0, false ) )
            {
                ...
                if ( socket.Available == 0 )
                {
                    ...
                    //     System.Text.ASCIIEncoding.ASCII.GetString( buffer, 0, MIN( 100, read ) ) );
