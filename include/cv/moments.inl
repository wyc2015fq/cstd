...
static void
icvCompleteMomentState(CMoments* moments)
{
    double cx = 0, cy = 0;
...
static void
icvContourMoments(const img_t* contour, CMoments* moments)
{
    int is_float = CC_MAT_TYPE(contour) == CC_32F;// && CC_MAT_CN(contour)==2;
    int contour_cn = CC_MAT_CN(contour);
    int contour_total = contour->h*contour->w*contour_cn/2;

    if (!CC_IS_MAT_CONT(contour)) {
        CC_Error(CC_StsBadArg, "Only continuous arrays are supported here");
    }
    ...
    if(contour_total)
    {
        ...
        int i;
        CPoint* iptr = (CPoint*)contour->tt.data;
        CPoint2D32f* fptr = (CPoint2D32f*)contour->tt.data;
...
static void
icvAccumulateMoments(double *tiles, CSize size, CSize tile_size, CMoments * moments)
{
    int x, y;
...
#define ICC_DEF_CALC_MOMENTS_IN_TILE(__op__, name, flavor, srctype, temptype, momtype) \
static CStatus C_STDCALL icv##name##_##flavor##_CnCR                                   \
(const srctype* img, int step, CSize size, int cn, int coi, double *moments)          \
{                                                                                        \
...
ICC_DEF_CALC_MOMENTS_IN_TILE(CC_NOP, MomentsInTile, 8u, uchar, int, int)
ICC_DEF_CALC_MOMENTS_IN_TILE(CC_NOP, MomentsInTile, 16u, ushort, int, int64)
ICC_DEF_CALC_MOMENTS_IN_TILE(CC_NOP, MomentsInTile, 16s, short, int, int64)
ICC_DEF_CALC_MOMENTS_IN_TILE(CC_NOP, MomentsInTile, 32f, float, double, double)
ICC_DEF_CALC_MOMENTS_IN_TILE(CC_NOP, MomentsInTile, 64f, double, double, double)

ICC_DEF_CALC_MOMENTS_IN_TILE(CC_NONZERO, MomentsInTileBin, 8u, uchar, int, int)
ICC_DEF_CALC_MOMENTS_IN_TILE(CC_NONZERO, MomentsInTileBin, 16s, ushort, int, int)
ICC_DEF_CALC_MOMENTS_IN_TILE(CC_NONZERO_FLT, MomentsInTileBin, 32f, int, int, int)
ICC_DEF_CALC_MOMENTS_IN_TILE(CC_NONZERO_FLT, MomentsInTileBin, 64f, int64, double, double)
...
CC_DEF_INIT_FUNC_TAB_2D(MomentsInTile, CnCR)
CC_DEF_INIT_FUNC_TAB_2D(MomentsInTileBin, CnCR)

////////////////////////////////// IPP moment functions //////////////////////////////////

CC_IMPL void
cvMoments(const img_t* mat, CMoments* moments, int binary CC_DEFAULT(0))
{
    ...
    if(CC_IS_MAT_CONT(mat) && 2==cn && (type == CC_32S || type == CC_32F))
    {
        icvContourMoments(mat, moments);
        EXIT;
    }

    if(cn > 1 && coi == 0)
        CC_ERROR(CC_StsBadArg, "Invalid image type");
    ...
    for(y = 0, k = 0; y < size.height; y += tile_size.height)
    {
        ...
        for(x = 0; x < size.width; x += tile_size.width, k++)
        {
            ...
            IPPI_CALL(func(mat->tt.data + y*mat->step + x*pix_size,
...
//    Returns:
//      CC_NO_ERR if success or error code
//    Notes:
//F*/
CC_IMPL void
cvGetHuMoments(CMoments * mState, CHuMoments * HuState)
{
    CC_FUNCNAME("cvGetHuMoments");
...
//F*/
CC_IMPL double
cvGetSpatialMoment(CMoments * moments, int x_order, int y_order)
{
    int order = x_order + y_order;
    double moment = -DBL_MAX;

    CC_FUNCNAME("cvGetSpatialMoment");
...
//F*/
CC_IMPL double
cvGetCentralMoment(CMoments * moments, int x_order, int y_order)
{
    int order = x_order + y_order;
    double mu = 0;

    CC_FUNCNAME("cvGetCentralMoment");
...
//F*/
CC_IMPL double
cvGetNormalizedCentralMoment(CMoments * moments, int x_order, int y_order)
{
    int order = x_order + y_order;
    double mu = 0;
    double m00s, m00;

    CC_FUNCNAME("cvGetCentralNormalizedMoment");
