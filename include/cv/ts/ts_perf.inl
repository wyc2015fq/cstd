...
static void randu(img_t& m)
{
    const int bigValue = 0x00000FFF;
    if (m CC_MAT_DEPTH() < CV_32F)
    {
        int minmax[] = {0, 256};
        img_t mr = img_t(m->rows, (int)(m->cols * m CC_ELEM_SIZE()), CV_8U, m->tt.data, m->step[0]);
        randu(mr, img_t(1, 1, CV_32S, minmax), img_t(1, 1, CV_32S, minmax + 1));
    }
    else if (m CC_MAT_DEPTH() == CV_32F)
    {
        //float minmax[] = {-FLT_MAX, FLT_MAX};
        float minmax[] = {-bigValue, bigValue};
        img_t mr = m cvReshape(,1);
        randu(mr, img_t(1, 1, CV_32F, minmax), img_t(1, 1, CV_32F, minmax + 1));
    }
    else
    {
        ...
        img_t mr = m cvReshape(,1);
        randu(mr, img_t(1, 1, CV_64F, minmax), img_t(1, 1, CV_64F, minmax + 1));
    }
}
...
class PerfEarlyExitException: public Exception {};
...
Regression& Regression::add(TestBase* test, const std::string& name, const img_t* array, double eps, ERROR_TYPE err)
{
    if(test) test->setVerified();
    return instance()(name, array, eps, err);
}

Regression& Regression::addMoments(TestBase* test, const std::string& name, const Moments& array, double eps, ERROR_TYPE err)
{
    int len = (int)sizeof(Moments) / sizeof(double);
    img_t m(1, len, CV_64F, (void*)&array);

    return Regression::add(test, name, m, eps, err);
}

Regression& Regression::addKeypoints(TestBase* test, const std::string& name, const std::vector<KeyPoint>& array, double eps, ERROR_TYPE err)
{
    int len = (int)array.size();
    img_t pt      (len, 1, CV_32FC2, len ? (void*)&array[0].pt : 0,       sizeof(KeyPoint));
    img_t size    (len, 1, CV_32FC1, len ? (void*)&array[0].size : 0,     sizeof(KeyPoint));
    img_t angle   (len, 1, CV_32FC1, len ? (void*)&array[0].angle : 0,    sizeof(KeyPoint));
    img_t response(len, 1, CV_32FC1, len ? (void*)&array[0].response : 0, sizeof(KeyPoint));
    img_t octave  (len, 1, CV_32SC1, len ? (void*)&array[0].octave : 0,   sizeof(KeyPoint));
    img_t class_id(len, 1, CV_32SC1, len ? (void*)&array[0].class_id : 0, sizeof(KeyPoint));
...
Regression& Regression::addMatches(TestBase* test, const std::string& name, const std::vector<DMatch>& array, double eps, ERROR_TYPE err)
{
    int len = (int)array.size();
    img_t queryIdx(len, 1, CV_32SC1, len ? (void*)&array[0].queryIdx : 0, sizeof(DMatch));
    img_t trainIdx(len, 1, CV_32SC1, len ? (void*)&array[0].trainIdx : 0, sizeof(DMatch));
    img_t imgIdx  (len, 1, CV_32SC1, len ? (void*)&array[0].imgIdx : 0,   sizeof(DMatch));
    img_t distance(len, 1, CV_32FC1, len ? (void*)&array[0].distance : 0, sizeof(DMatch));
...
void Regression::init(const std::string& testSuitName, const std::string& ext)
{
    ...
    try
    {
        if (storageIn.open(storageInPath, FileStorage::READ))
        {
...
Regression::Regression() : regRNG(cGetTickCount())//this rng should be really random
{
}
...
FileStorage& Regression::write()
{
    if (!storageOut.isOpened() && !storageOutPath.empty())
    {
        ...
        else if (mode == FileStorage::WRITE && !rootIn.empty())
        {
...
std::string Regression::getCurrentTestNodeName()
{
    ...
    const char* type_param = test_info->tid_param();
    if (type_param != 0)
        (nodename += "--") += type_param;
...
bool Regression::isVector(const img_t* a)
{
    return a.kind() == _InputArray::STD_VECTOR_MAT || a.kind() == _InputArray::STD_VECTOR_VECTOR ||
           a.kind() == _InputArray::STD_VECTOR_UMAT;
}

double Regression::getElem(img_t& m, int y, int x, int cn)
{
    switch (m CC_MAT_DEPTH())
    {
...
void Regression::write(img_t m)
{
    ...
    write() << "last" << "{" << "x" << m.size.p[1] - 1 << "y" << m.size.p[0] - 1
        << "val" << getElem(m, m.size.p[0] - 1, m.size.p[1] - 1, m CC_MAT_CN() - 1) << "}";

    int x, y, cn;
    x = regRNG.uniform(0, m.size.p[1]);
    y = regRNG.uniform(0, m.size.p[0]);
    cn = regRNG.uniform(0, m CC_MAT_CN());
    write() << "rng1" << "{" << "x" << x << "y" << y;
    if(cn > 0) write() << "cn" << cn;
...
void Regression::verify(FileNode node, img_t actual, double eps, std::string argname, ERROR_TYPE err)
{
    ...
    if (err == ERROR_RELATIVE)
        eps *= MAX(std::abs(expect_min), std::abs(expect_max));

    double actual_min, actual_max;
    minMaxIdx(actual, &actual_min, &actual_max);
    ...
    FileNode last = node["last"];
    double actual_last = getElem(actual, actual.size.p[0] - 1, actual.size.p[1] - 1, actual CC_MAT_CN() - 1);
...
void Regression::write(const img_t* array)
{
    write() << "kind" << array.kind();
    write() << "type" << array CC_MAT_TYPE();
    if (isVector(array))
    {
        int total = (int)array CC_MAT_TOTAL();
        int idx = regRNG.uniform(0, total);
        write() << "len" << total;
        write() << "idx" << idx;

        img_t m = array.getMat(idx);

        if (m CC_MAT_TOTAL() * m CC_MAT_CN() < 26) //5x5 or smaller
            write() << "val" << m;
        else
            write(m);
    }
    ...
    {
        if (array CC_MAT_TOTAL() * array CC_MAT_CN() < 26) //5x5 or smaller
            write() << "val" << array;
        else
            write(array);
    }
...
static int countViolations(const img_t& expected, const img_t& actual, const img_t& diff, double eps, double* max_violation = 0, double* max_allowed = 0)
{
    img_t diff64f;
    diff cvReshape(,1).convertTo(diff64f, CV_64F);

    img_t expected_abs = abs(expected cvReshape(,1));
    img_t actual_abs = abs(actual cvReshape(,1));
    img_t maximum, mask;
    max(expected_abs, actual_abs, maximum);
    multiply(maximum, Vec<double, 1>(eps), maximum, CV_64F);
    compare(diff64f, maximum, mask, CMP_GT);

    int v = countNonZero(mask);

    if (v > 0 && max_violation != 0 && max_allowed != 0)
    {
        int loc[10] = {0};
        minMaxIdx(maximum, 0, max_allowed, 0, loc, mask);
...
void Regression::verify(FileNode node, const img_t* array, double eps, ERROR_TYPE err)
{
    ...
    ASSERT_EQ(expected_type, array CC_MAT_TYPE()) << "  Argument \"" << node.name() << "\" has unexpected type";

    FileNode valnode = node["val"];
    if (isVector(array))
    {
        int expected_length = (int)node["len"];
        ASSERT_EQ(expected_length, (int)array CC_MAT_TOTAL()) << "  Vector \"" << node.name() << "\" has unexpected length";
        int idx = node["idx"];

        img_t actual = array.getMat(idx);

        if (valnode.isNone())
        {
            ASSERT_LE((size_t)26, actual CC_MAT_TOTAL() * (size_t)actual CC_MAT_CN())
                    << "  \"" << node.name() << "[" <<  idx << "]\" has unexpected number of elements";
            verify(node, actual, eps, format("%s[%d]", node.name().c_str(), idx), err);
        }
        else
        {
            img_t expected;
            ...
            else
            {
                ...
                img_t diff;
                absdiff(expected, actual, diff);

                if (err == ERROR_ABSOLUTE)
                {
                    if (!checkRange(diff, true, 0, 0, eps))
                    {
                        if(expected CC_MAT_TOTAL() * expected CC_MAT_CN() < 12)
                            std::cout << " Expected: " << std::endl << expected << std::endl << " Actual:" << std::endl << actual << std::endl;

                        double max;
                        minMaxIdx(diff cvReshape(,1), 0, &max);
    ...
    else
    {
        if (valnode.isNone())
        {
            ASSERT_LE((size_t)26, array CC_MAT_TOTAL() * (size_t)array CC_MAT_CN())
                    << "  Argument \"" << node.name() << "\" has unexpected number of elements";
            verify(node, array, eps, "Argument \"" + node.name() + "\"", err);
        }
        else
        {
            img_t expected;
...
Regression& Regression::operator() (const std::string& name, const img_t* array, double eps, ERROR_TYPE err)
{
    // exit if current test is already failed
    if(::testing::UnitTest::GetInstance()->current_test_info()->result()->Failed()) return *this;

    if(!array.empty() && array CC_MAT_DEPTH() == CV_USRTYPE1)
    {
...
#ifdef ENABLE_INSTRUMENTATION
static void printShift(instr::InstrNode *pNode, instr::InstrNode* pRoot)
{
    ...
    std::vector<instr::InstrNode*> cache;
    instr::InstrNode *pTmpNode = pNode;
...
static double calcLocalWeight(instr::InstrNode *pNode)
{
...
static double calcGlobalWeight(instr::InstrNode *pNode)
{
    instr::InstrNode* globNode = pNode;
...
static void printNodeRec(instr::InstrNode *pNode, instr::InstrNode *pRoot)
{
    printf("%s", (pNode->m_payload.m_funName.substr(0, 40) + ((pNode->m_payload.m_funName.size()>40)?"...":"")).c_str());

    // Write instrumentation flags
    if(pNode->m_payload.m_instrType != instr::TYPE_GENERAL || pNode->m_payload.m_implType != instr::IMPL_PLAIN)
    {
        printf("<");
        if(pNode->m_payload.m_instrType == instr::TYPE_WRAPPER)
            printf("W");
        else if(pNode->m_payload.m_instrType == instr::TYPE_FUN)
            printf("F");
        else if(pNode->m_payload.m_instrType == instr::TYPE_MARKER)
            printf("MARK");

        if(pNode->m_payload.m_instrType != instr::TYPE_GENERAL && pNode->m_payload.m_implType != instr::IMPL_PLAIN)
            printf("_");

        if(pNode->m_payload.m_implType == instr::IMPL_IPP)
            printf("IPP");
        else if(pNode->m_payload.m_implType == instr::IMPL_OPENCL)
            printf("OCL");
...

    {
        ...
        for(size_t i = 1; i < pNode->m_childs.size(); i++)
        {
            ...
            for(size_t j = i+1; j < pNode->m_childs.size(); j++)
            {
                ...
                {
                    swap(pNode->m_childs[i], pNode->m_childs[j]);
                    i++;
                }
            }
        }
...
static String nodeToString(instr::InstrNode *pNode)
{
    String string;
...
static uint64 getNodeTimeRec(instr::InstrNode *pNode, instr::TYPE type, instr::IMPL impl)
{
    uint64 ticks = 0;
...
static uint64 getImplTime(instr::IMPL impl)
{
    uint64 ticks = 0;
    instr::InstrNode *pRoot = instr::getTrace();

    ticks = getNodeTimeRec(pRoot, instr::TYPE_FUN, impl);
...
::String InstumentData::treeToString()
{
    String string = nodeToString(instr::getTrace());
    return string;
}
...
{
    printf("[ TRACE    ]\n");
    printNodeRec(instr::getTrace(), instr::getTrace());
#ifdef HAVE_IPP
    printf("\nIPP weight: %.1f%%", ((double)getImplTime(instr::IMPL_IPP)*100/(double)getTotalTime()));
#endif
#ifdef HAVE_OPENCL
    printf("\nOPENCL weight: %.1f%%", ((double)getImplTime(instr::IMPL_OPENCL)*100/(double)getTotalTime()));
#endif
    printf("\n[/TRACE    ]\n");
    fflush(stdout);
}
                 ...
                 int argc, const char* const argv[])
{
    ...
    param_max_outliers  = MIN(100., MAX(0., args.get<double>("perf_max_outliers")));
    param_min_samples   = MAX(1u, args.get<unsigned int>("perf_min_samples"));
    param_max_deviation = MAX(0., args.get<double>("perf_max_deviation"));
    param_seed          = args.get<unsigned int>("perf_seed");
    param_time_limit    = MAX(0., args.get<double>("perf_time_limit"));
    ...
    if(param_instrument > 0)
    {
        if(param_instrument == 2)
            instr::setFlags(instr::getFlags()|instr::FLAGS_EXPAND_SAME_NAMES);
        instr::setUseInstrumentation(true);
    }
    else
        instr::setUseInstrumentation(false);
#endif
    ...
    param_cuda_device      = MAX(0, MIN(cuda::getCudaEnabledDeviceCount(), args.get<int>("perf_cuda_device")));

    if (param_impl == "cuda")
    {
        cuda::DeviceInfo info(param_cuda_device);
        ...
        cuda::setDevice(param_cuda_device);
    ...
    timeLimitDefault = param_time_limit == 0.0 ? 1 : (int64)(param_time_limit * getTickFrequency());
...
int64 TestBase::_calibrate()
{
    class _helper : public ::perf::TestBase
    {
        ...
        virtual void PerfTestBody()
        {
            ...
            SetUp();
            img_t a(2048, 2048, CV_32S, Scalar(1));
            img_t b(2048, 2048, CV_32S, Scalar(2));
            declare.time(30);
            double s = 0;
    ...
    {
        CV_Assert(compensation < 0.01 * getTickFrequency());
...
void TestBase::declareArray(SizeVector& sizes, InputOutputArray a, WarmUpType wtype)
{
    if (!a.empty())
    {
        sizes.push_back(std::pair<int, Size>(getSizeInBytes(a), getSize(a)));
        warmup(a, wtype);
    }
    else if (a.kind() != _InputArray::NONE)
        ADD_FAILURE() << "  Uninitialized input/output parameters are not allowed for performance tests";
}

void TestBase::warmup(InputOutputArray a, WarmUpType wtype)
{
    if (a.empty())
        return;
    else if (a.isUMat())
    {
        if (wtype == WARMUP_RNG || wtype == WARMUP_WRITE)
        {
            ...
            if (depth == CV_8U)
                ...
                CV_Error(CC_StsUnsupportedFormat, "Unsupported format");
        }
        return;
    }
    else if (a.kind() != _InputArray::STD_VECTOR_MAT && a.kind() != _InputArray::STD_VECTOR_VECTOR)
        warmup_impl(a, wtype);
    else
    {
        size_t total = a CC_MAT_TOTAL();
        for (size_t i = 0; i < total; ++i)
            warmup_impl(a.getMat((int)i), wtype);
    }
}

int TestBase::getSizeInBytes(const img_t* a)
{
    if (a.empty()) return 0;
    int total = (int)a CC_MAT_TOTAL();
    if (a.kind() != _InputArray::STD_VECTOR_MAT && a.kind() != _InputArray::STD_VECTOR_VECTOR)
        return total * CV_ELEM_SIZE(a CC_MAT_TYPE());
...
Size TestBase::getSize(const img_t* a)
{
    if (a.kind() != _InputArray::STD_VECTOR_MAT && a.kind() != _InputArray::STD_VECTOR_VECTOR)
        return a.size();
    return Size();
...
bool TestBase::next()
{
    ...
    theRNG().state = param_seed; //this rng should generate same numbers for each run
    ++currentIter;
    ...
    do {
        ...
        else
        {
            ...
            if (totalTime - lastActivityPrintTime >= getTickFrequency() * 10)
            {
                std::cout << '.' << std::endl;
        ...
        if (!perf_validation_results.empty() && !name.empty())
        {
            ...
            if (!isSame)
            {
                if (perfValidationStage < 2)
                {
                    ...
                    {
                        unsigned int new_minIters = MAX(minIters * 5, currentIter * 3);
...
#endif
                        has_next = true;
                        minIters = MIN(minIters * 5, nIters);
    ...
    if (log_power_checkpoints)
    {
        ...
        if (currentIter == 1) RecordProperty("test_start", format("%llu",t1).c_str());
        if (!has_next) RecordProperty("test_complete", format("%llu",t1).c_str());
    }
#endif
...
void TestBase::warmup_impl(img_t m, WarmUpType wtype)
{
    switch(wtype)
    {
    case WARMUP_READ:
        ...
        return;
    case WARMUP_WRITE:
        m cvReshape(,1).setTo(Scalar::all(0));
        return;
    case WARMUP_RNG:
        randu(m);
        return;
...
bool TestBase::startTimer()
{
    ...
    if(currentIter == 0)
    {
        instr::setFlags(instr::getFlags()|instr::FLAGS_MAPPING); // enable mapping for the first run
        instr::resetTrace();
    }
#endif
    lastTime = cGetTickCount();
    return true; // dummy true for conditional loop
}

void TestBase::stopTimer()
{
    int64 time = cGetTickCount();
    if (lastTime == 0)
...
#ifdef ENABLE_INSTRUMENTATION
    instr::setFlags(instr::getFlags()&~instr::FLAGS_MAPPING); // disable mapping to decrease overhead for +1 run
#endif
}
...
performance_metrics& TestBase::calcMetrics()
{
    ...
    metrics.frequency = getTickFrequency();
...
void TestBase::validateMetrics()
{
    ...
    if (getCurrentPerformanceStrategy() == PERF_STRATEGY_BASE)
    {
        ...
        EXPECT_LE(m.outliers, MAX((unsigned int)cCeil(m.samples * param_max_outliers / 100.), 1u))
...
void TestBase::reportMetrics(bool toJUnitXML)
{
    ...
    else if (toJUnitXML)
    {
        ...
        RecordProperty("frequency", format("%.0f", m.frequency).c_str());
        RecordProperty("min", format("%.0f", m.min).c_str());
        RecordProperty("median", format("%.0f", m.median).c_str());
        RecordProperty("gmean", format("%.0f", m.gmean).c_str());
        RecordProperty("gstddev", format("%.6f", m.gstddev).c_str());
        RecordProperty("mean", format("%.0f", m.mean).c_str());
        RecordProperty("stddev", format("%.0f", m.stddev).c_str());
#ifdef ENABLE_INSTRUMENTATION
        if(instr::useInstrumentation())
        {
            String tree = InstumentData::treeToString();
            RecordProperty("functions_hierarchy", tree.c_str());
            RecordProperty("total_ipp_weight",    format("%.1f", ((double)getImplTime(instr::IMPL_IPP)*100/(double)getTotalTime())));
            RecordProperty("total_opencl_weight", format("%.1f", ((double)getImplTime(instr::IMPL_OPENCL)*100/(double)getTotalTime())));
            instr::resetTrace();
        }
#endif
#ifdef CV_COLLECT_IMPL_DATA
...
void TestBase::TearDown()
{
    ...
    if(param_collect_impl)
    {
        ...
        if(implConf.ipp_mt)
        {
            if(implConf.icv) {printf("ICV_MT "); std::vector<String> fun = implConf.GetCallsForImpl(CV_IMPL_IPP|CV_IMPL_MT); printf("("); for(int i=0; i<fun.size();i++ ){printf("%s ", fun[i].c_str());} printf(") "); }
            if(implConf.ipp) {printf("IPP_MT "); std::vector<String> fun = implConf.GetCallsForImpl(CV_IMPL_IPP|CV_IMPL_MT); printf("("); for(int i=0; i<fun.size();i++ ){printf("%s ", fun[i].c_str());} printf(") "); }
        }
        else
        {
            if(implConf.icv) {printf("ICV "); std::vector<String> fun = implConf.GetCallsForImpl(CV_IMPL_IPP); printf("("); for(int i=0; i<fun.size();i++ ){printf("%s ", fun[i].c_str());} printf(") "); }
            if(implConf.ipp) {printf("IPP "); std::vector<String> fun = implConf.GetCallsForImpl(CV_IMPL_IPP); printf("("); for(int i=0; i<fun.size();i++ ){printf("%s ", fun[i].c_str());} printf(") "); }
        }
        if(implConf.ocl) {printf("OCL "); std::vector<String> fun = implConf.GetCallsForImpl(CV_IMPL_OCL); printf("("); for(int i=0; i<fun.size();i++ ){printf("%s ", fun[i].c_str());} printf(") "); }
...
TestBase::_declareHelper& TestBase::_declareHelper::iterations(unsigned int n)
{
    ...
    test->nIters = MIN(n, TestBase::iterationsLimitDefault);
...
TestBase::_declareHelper& TestBase::_declareHelper::time(double timeLimitSecs)
{
    test->times.clear();
    test->currentIter = (unsigned int)-1;
    test->timeLimit = (int64)(timeLimitSecs * getTickFrequency());
    test->metrics.clear();
    return *this;
}
...
TestBase::_declareHelper& TestBase::_declareHelper::in(InputOutputArray a1, WarmUpType wtype)
{
...
TestBase::_declareHelper& TestBase::_declareHelper::in(InputOutputArray a1, InputOutputArray a2, WarmUpType wtype)
{
    if (!test->times.empty()) return *this;
    TestBase::declareArray(test->inputData, a1, wtype);
...
TestBase::_declareHelper& TestBase::_declareHelper::in(InputOutputArray a1, InputOutputArray a2, InputOutputArray a3, WarmUpType wtype)
{
    if (!test->times.empty()) return *this;
    TestBase::declareArray(test->inputData, a1, wtype);
    TestBase::declareArray(test->inputData, a2, wtype);
...
TestBase::_declareHelper& TestBase::_declareHelper::in(InputOutputArray a1, InputOutputArray a2, InputOutputArray a3, InputOutputArray a4, WarmUpType wtype)
{
    if (!test->times.empty()) return *this;
    TestBase::declareArray(test->inputData, a1, wtype);
    TestBase::declareArray(test->inputData, a2, wtype);
...
TestBase::_declareHelper& TestBase::_declareHelper::out(InputOutputArray a1, WarmUpType wtype)
{
    if (!test->times.empty()) return *this;
...
TestBase::_declareHelper& TestBase::_declareHelper::out(InputOutputArray a1, InputOutputArray a2, WarmUpType wtype)
{
    if (!test->times.empty()) return *this;
    TestBase::declareArray(test->outputData, a1, wtype);
...
TestBase::_declareHelper& TestBase::_declareHelper::out(InputOutputArray a1, InputOutputArray a2, InputOutputArray a3, WarmUpType wtype)
{
    if (!test->times.empty()) return *this;
    TestBase::declareArray(test->outputData, a1, wtype);
    TestBase::declareArray(test->outputData, a2, wtype);
...
TestBase::_declareHelper& TestBase::_declareHelper::out(InputOutputArray a1, InputOutputArray a2, InputOutputArray a3, InputOutputArray a4, WarmUpType wtype)
{
    if (!test->times.empty()) return *this;
    TestBase::declareArray(test->outputData, a1, wtype);
    TestBase::declareArray(test->outputData, a2, wtype);
...
{
    std::vector<KeyPoint>& pts_;
    comparators::KeypointGreater cmp;

    KeypointComparator(std::vector<KeyPoint>& pts) : pts_(pts), cmp() {}
...
void perf::sort(std::vector<KeyPoint>& pts, InputOutputArray descriptors)
{
    img_t desc = descriptors;

    CV_Assert(pts.size() == (size_t)desc->rows);
    FREE(); int* = MALLOC(int, ) idxs(desc->rows);
    ...
    std::vector<KeyPoint> spts(pts.size());
    img_t sdesc(desc.size(), desc CC_MAT_TYPE());

    for(int j = 0; j < desc->rows; ++j)
    {
        spts[j] = pts[idxs[j]];
        img_t row = sdesc.row(j);
