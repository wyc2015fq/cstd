...
class SkipTestException: public Exception
{
public:
    int dummy; // workaround for MacOSX Xcode 7.3 bug (don't make class "empty")
    SkipTestException() : dummy(0) {}
    SkipTestException(const String& message) : dummy(0) { this->msg = message; }
};
...
CV_EXPORTS void randUni( RNG& rng, CvMat& a, const Scalar& param1, const Scalar& param2 );
...
CV_EXPORTS CvMat randomMat(RNG& rng, Size size, int type, double minVal, double maxVal, bool useRoi);
CV_EXPORTS CvMat randomMat(RNG& rng, const vector<int>& size, int type, double minVal, double maxVal, bool useRoi);
CV_EXPORTS void add(const CvMat& a, double alpha, const CvMat& b, double beta,
                      Scalar gamma, CvMat& c, int ctype, bool calcAbs=false);
CV_EXPORTS void multiply(const CvMat& a, const CvMat& b, CvMat& c, double alpha=1);
CV_EXPORTS void divide(const CvMat& a, const CvMat& b, CvMat& c, double alpha=1);

CV_EXPORTS void convert(const CvMat& src, CvMat* dst, int dtype, double alpha=1, double beta=0);
CV_EXPORTS void copy(const CvMat& src, CvMat& dst, const CvMat& mask=CvMat(), bool invertMask=false);
CV_EXPORTS void set(CvMat& dst, const Scalar& gamma, const CvMat& mask=CvMat());

// working with multi-channel arrays
CV_EXPORTS void extract( const CvMat& a, CvMat& plane, int coi );
CV_EXPORTS void insert( const CvMat& plane, CvMat& a, int coi );
...
CV_EXPORTS int check( const CvMat& data, double min_val, double max_val, vector<int>* idx );

// modifies values that are close to zero
CV_EXPORTS void  patchZeros( CvMat& mat, double level );

CV_EXPORTS void transpose(const CvMat& src, CvMat& dst);
CV_EXPORTS void erode(const CvMat& src, CvMat& dst, const CvMat& _kernel, Point anchor=Point(-1,-1),
                      int borderType=0, const Scalar& borderValue=Scalar());
CV_EXPORTS void dilate(const CvMat& src, CvMat& dst, const CvMat& _kernel, Point anchor=Point(-1,-1),
                       int borderType=0, const Scalar& borderValue=Scalar());
CV_EXPORTS void filter2D(const CvMat& src, CvMat& dst, int ddepth, const CvMat& kernel,
                         Point anchor, double delta, int borderType,
                         const Scalar& borderValue=Scalar());
CV_EXPORTS void copyMakeBorder(const CvMat& src, CvMat& dst, int top, int bottom, int left, int right,
                               int borderType, const Scalar& borderValue=Scalar());
CV_EXPORTS CvMat calcSobelKernel2D( int dx, int dy, int apertureSize, int origin=0 );
CV_EXPORTS CvMat calcLaplaceKernel2D( int aperture_size );

CV_EXPORTS void initUndistortMap( const CvMat& a, const CvMat& k, Size sz, CvMat& mapx, CvMat& mapy );

CV_EXPORTS void minMaxLoc(const CvMat& src, double* minval, double* maxval,
                          vector<int>* minloc, vector<int>* maxloc, const CvMat& mask=CvMat());
CV_EXPORTS double norm(const CvMat* src, int normType, const CvMat* mask=noArray());
CV_EXPORTS double norm(const CvMat* src1, const CvMat* src2, int normType, const CvMat* mask=noArray());
CV_EXPORTS Scalar mean(const CvMat& src, const CvMat& mask=CvMat());
CV_EXPORTS double PSNR(const CvMat* src1, const CvMat* src2);

CV_EXPORTS bool cmpUlps(const CvMat& data, const CvMat& refdata, int expMaxDiff, double* realMaxDiff, vector<int>* idx);

// compares two arrays. max_diff is the maximum actual difference,
// success_err_level is maximum allowed difference, idx is the index of the first
// element for which difference is >success_err_level
// (or index of element with the maximum difference)
CV_EXPORTS int cmpEps( const CvMat& data, const CvMat& refdata, double* max_diff,
...
CV_EXPORTS int cmpEps2( TS* ts, const CvMat& data, const CvMat& refdata, double success_err_level,
                        bool element_wise_relative_error, const char* desc );

CV_EXPORTS int cmpEps2_64f( TS* ts, const double* val, const double* refval, int len,
                        double eps, const char* param_name );

CV_EXPORTS void logicOp(const CvMat& src1, const CvMat& src2, CvMat& dst, char c);
CV_EXPORTS void logicOp(const CvMat& src, const Scalar& s, CvMat& dst, char c);
CV_EXPORTS void min(const CvMat& src1, const CvMat& src2, CvMat& dst);
CV_EXPORTS void min(const CvMat& src, double s, CvMat& dst);
CV_EXPORTS void max(const CvMat& src1, const CvMat& src2, CvMat& dst);
CV_EXPORTS void max(const CvMat& src, double s, CvMat& dst);

CV_EXPORTS void compare(const CvMat& src1, const CvMat& src2, CvMat& dst, int cmpop);
CV_EXPORTS void compare(const CvMat& src, double s, CvMat& dst, int cmpop);
CV_EXPORTS void gemm(const CvMat& src1, const CvMat& src2, double alpha,
                     const CvMat& src3, double beta, CvMat& dst, int flags);
CV_EXPORTS void transform( const CvMat& src, CvMat& dst, const CvMat& transmat, const CvMat& shift );
CV_EXPORTS double crossCorr(const CvMat& src1, const CvMat& src2);
CV_EXPORTS void threshold( const CvMat& src, CvMat& dst, double thresh, double maxval, int thresh_type );
CV_EXPORTS void minMaxIdx( const CvMat* _img, double* minVal, double* maxVal,
                    Point* minLoc, Point* maxLoc, const CvMat* _mask );

struct CV_EXPORTS MatInfo
{
    MatInfo(const CvMat& _m) : m(&_m) {}
    const CvMat* m;
};
...
struct CV_EXPORTS DefaultRngAuto
{
    ...
    DefaultRngAuto() : old_state(theRNG().state) { theRNG().state = cvtest::param_seed; }
...
CV_EXPORTS void fillGradient(CvMat& img, int delta = 5);
CV_EXPORTS void smoothBorder(CvMat& img, const Scalar& color, int delta = 3);
