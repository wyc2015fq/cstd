...
\*****************************************************************************************/
const Size szQVGA = Size(320, 240);
const Size szVGA = Size(640, 480);
const Size szSVGA = Size(800, 600);
const Size szXGA = Size(1024, 768);
const Size szSXGA = Size(1280, 1024);
const Size szWQHD = Size(2560, 1440);

const Size sznHD = Size(640, 360);
const Size szqHD = Size(960, 540);
const Size sz240p = szQVGA;
const Size sz720p = Size(1280, 720);
const Size sz1080p = Size(1920, 1080);
const Size sz1440p = szWQHD;
const Size sz2160p = Size(3840, 2160);//UHDTV1 4K
const Size sz4320p = Size(7680, 4320);//UHDTV2 8K

const Size sz3MP = Size(2048, 1536);
const Size sz5MP = Size(2592, 1944);
const Size sz2K = Size(2048, 2048);

const Size szODD = Size(127, 61);

const Size szSmall24 = Size(24, 24);
const Size szSmall32 = Size(32, 32);
const Size szSmall64 = Size(64, 64);
const Size szSmall128 = Size(128, 128);
...
/*****************************************************************************************\
*                MatType - printable wrapper over integer 'type' of CvMat                   *
\*****************************************************************************************/
class MatType
{
public:
...
#define CV_ENUM(class_name, ...)                                                        \
    namespace {                                                                         \
    using namespace cv;using namespace cuda; using namespace ocl;               \
    struct class_name {                                                                 \
...
{
public:
    static Regression& add(TestBase* test, const std::string& name, const CvMat* array, double eps = DBL_EPSILON, ERROR_TYPE err = ERROR_ABSOLUTE);
    static Regression& addMoments(TestBase* test, const std::string& name, const Moments & array, double eps = DBL_EPSILON, ERROR_TYPE err = ERROR_ABSOLUTE);
    static Regression& addKeypoints(TestBase* test, const std::string& name, const std::vector<KeyPoint>& array, double eps = DBL_EPSILON, ERROR_TYPE err = ERROR_ABSOLUTE);
    static Regression& addMatches(TestBase* test, const std::string& name, const std::vector<DMatch>& array, double eps = DBL_EPSILON, ERROR_TYPE err = ERROR_ABSOLUTE);
    static void Init(const std::string& testSuitName, const std::string& ext = ".xml");

    Regression& operator() (const std::string& name, const CvMat* array, double eps = DBL_EPSILON, ERROR_TYPE err = ERROR_ABSOLUTE);
...
private:
    ...
    RNG regRNG;//own random numbers generator to make collection and verification work identical
    std::string storageInPath;
    std::string storageOutPath;
    FileStorage storageIn;
    FileStorage storageOut;
    FileNode rootIn;
    ...
    static bool isVector(const CvMat* a);
    static double getElem(CvMat& m, int x, int y, int cn = 0);

    void init(const std::string& testSuitName, const std::string& ext);
    void write(const CvMat* array);
    void write(CvMat m);
    void verify(FileNode node, const CvMat* array, double eps, ERROR_TYPE err);
    void verify(FileNode node, CvMat actual, double eps, std::string argname, ERROR_TYPE err);
};
...
typedef struct ImplData
{
    ...
    void GetImpl()
    {
        flagsToVars(getImpl(implCode, funName));
    }

    std::vector<String> GetCallsForImpl(int impl)
    {
...
public:
    ...
    class PerfSkipTestException: public Exception
    {
    public:
...
protected:
    ...
    static void warmup(InputOutputArray a, WarmUpType wtype = WARMUP_READ);
...
private:
    typedef std::vector<std::pair<int, Size> > SizeVector;
    ...
    static void warmup_impl(CvMat m, WarmUpType wtype);
    static int getSizeInBytes(const CvMat* a);
    static Size getSize(const CvMat* a);
    static void declareArray(SizeVector& sizes, InputOutputArray a, WarmUpType wtype);

    class CV_EXPORTS _declareHelper
    {
    public:
        _declareHelper& in(InputOutputArray a1, WarmUpType wtype = WARMUP_READ);
        _declareHelper& in(InputOutputArray a1, InputOutputArray a2, WarmUpType wtype = WARMUP_READ);
        _declareHelper& in(InputOutputArray a1, InputOutputArray a2, InputOutputArray a3, WarmUpType wtype = WARMUP_READ);
        _declareHelper& in(InputOutputArray a1, InputOutputArray a2, InputOutputArray a3, InputOutputArray a4, WarmUpType wtype = WARMUP_READ);

        _declareHelper& out(InputOutputArray a1, WarmUpType wtype = WARMUP_WRITE);
        _declareHelper& out(InputOutputArray a1, InputOutputArray a2, WarmUpType wtype = WARMUP_WRITE);
        _declareHelper& out(InputOutputArray a1, InputOutputArray a2, InputOutputArray a3, WarmUpType wtype = WARMUP_WRITE);
        _declareHelper& out(InputOutputArray a1, InputOutputArray a2, InputOutputArray a3, InputOutputArray a4, WarmUpType wtype = WARMUP_WRITE);
...
typedef std::tr1::tuple<Size, MatType> Size_MatType_t;
...
// macro.  Example:
//
//   typedef ::perf::TestBaseWithParam<Size> FooTest;
//
//   PERF_TEST_P(FooTest, DoTestingRight, ::testing::Values(::perf::szVGA, ::perf::sz720p) {
//     CvMat b(GetParam(), CV_8U, Scalar(10));
//     CvMat a(GetParam(), CV_8U, Scalar(20));
//     CvMat c(GetParam(), CV_8U, Scalar(0));
//
...
struct CV_EXPORTS RectLess_ :
        public std::binary_function<Rect_<T>, Rect_<T>, bool>
{
  bool operator()(const Rect_<T>& r1, const Rect_<T>& r2) const
  {
    return r1.x < r2.x ||
            (r1.x == r2.x && r1.y < r2.y) ||
...
struct CV_EXPORTS KeypointGreater :
        public std::binary_function<KeyPoint, KeyPoint, bool>
{
...
void CV_EXPORTS sort(std::vector<KeyPoint>& pts, InputOutputArray descriptors);
} //namespace perf
