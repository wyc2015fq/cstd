...
Size randomSize(RNG& rng, double maxSizeLog)
{
    ...
    if( (unsigned)rng % 2 != 0 )
        T_SWAP(_Tp, width_log, height_log);
    Size sz;
    sz.width = cRound(exp(width_log));
    sz.height = cRound(exp(height_log));
    return sz;
}
...
img_t randomMat(RNG& rng, Size size, int type, double minVal, double maxVal, bool useRoi)
{
    Size size0 = size;
    if( useRoi )
    {
        size0.width += MAX(rng.uniform(0, 10) - 5, 0);
        size0.height += MAX(rng.uniform(0, 10) - 5, 0);
    }
...
img_t randomMat(RNG& rng, const vector<int>& size, int type, double minVal, double maxVal, bool useRoi)
{
    int i, dims = (int)size.size();
    vector<int> size0(dims);
    vector<CRange> r(dims);
    bool eqsize = true;
    for( i = 0; i < dims; i++ )
    {
        size0[i] = size[i];
        ...
        if( useRoi )
        {
            size0[i] += MAX(rng.uniform(0, 5) - 2, 0);
            r[i] = CRange((size0[i] - size[i])/2, (size0[i] - size[i])/2 + size[i]);
        }
        eqsize = eqsize && size[i] == size0[i];
    }

    img_t m(dims, &size0[0], type);
...
void add(const img_t& _a, double alpha, const img_t& _b, double beta,
        Scalar gamma, img_t& c, int ctype, bool calcAbs)
{
    img_t a = _a, b = _b;
    if( a.empty() || alpha == 0 )
    {
    ...
    if( ctype < 0 )
        ctype = a CC_MAT_DEPTH();
    ctype = CV_MAKETYPE(CV_MAT_DEPTH(ctype), a CC_MAT_CN());
    c cvSetMat(a.dims, &a.size[0], ctype);
    const img_t *arrays[] = {&a, &b, &c, 0};
    img_t planes[3], buf[3];

    NAryMatIterator it(arrays, planes);
    size_t i, nplanes = it.nplanes;
    int cn=a CC_MAT_CN();
    int total = (int)planes[0] CC_MAT_TOTAL(), maxsize = MIN(12*12*MAX(12/cn, 1), total);

    CV_Assert(planes[0]->rows == 1);
    buf[0] cvSetMat(1, maxsize, CV_64FC(cn));
    if(!b.empty())
        buf[1] cvSetMat(1, maxsize, CV_64FC(cn));
    buf[2] cvSetMat(1, maxsize, CV_64FC(cn));
    scalarToRawData(gamma, buf[2]->tt.data, CV_64FC(cn), (int)(maxsize*cn));

    for( i = 0; i < nplanes; i++, ++it)
    {
        for( int j = 0; j < total; j += maxsize )
        {
            int j2 = MIN(j + maxsize, total);
            img_t apart0 = planes[0].colRange(j, j2);
            img_t cpart0 = planes[2].colRange(j, j2);
            img_t apart = buf[0].colRange(0, j2 - j);

            apart0.convertTo(apart, apart CC_MAT_TYPE(), alpha);
            size_t k, n = (j2 - j)*cn;
            double* aptr = apart img_ptr(double, );
            const double* gptr = buf[2] img_ptr(double, );
            ...
            {
                img_t bpart0 = planes[1].colRange((int)j, (int)j2);
                img_t bpart = buf[1].colRange(0, (int)(j2 - j));
                bpart0.convertTo(bpart, bpart CC_MAT_TYPE(), beta);
                const double* bptr = bpart img_ptr(double, );
            ...
            if( calcAbs )
                for( k = 0; k < n; k++ )
                    aptr[k] = fabs(aptr[k]);
            apart.convertTo(cpart0, cpart0 CC_MAT_TYPE(), 1, 0);
        }
    }
}
...
void convert(const img_t& src, img_t* _dst, int dtype, double alpha, double beta)
{
    if (dtype < 0) dtype = _dst CC_MAT_DEPTH();

    dtype = CV_MAKETYPE(CV_MAT_DEPTH(dtype), src CC_MAT_CN());
    _dst cvSetMat(src.dims, &src.size[0], dtype);
    img_t *dst = _dst;
    if( alpha == 0 )
    {
    ...
    if( dtype == src CC_MAT_TYPE() && alpha == 1 && beta == 0 )
    {
        copy( src, dst );
        return;
    }
    ...
    NAryMatIterator it(arrays, planes);
    size_t total = planes[0] CC_MAT_TOTAL()*planes[0] CC_MAT_CN();
    size_t i, nplanes = it.nplanes;
...
void copy(const img_t& src, img_t& dst, const img_t& mask, bool invertMask)
{
    ...
    if(mask.empty())
    {
        ...
        NAryMatIterator it(arrays, planes);
        size_t i, nplanes = it.nplanes;
        size_t planeSize = planes[0] CC_MAT_TOTAL()*src CC_ELEM_SIZE();

        for( i = 0; i < nplanes; i++, ++it )
            memcpy(planes[1]->tt.data, planes[0]->tt.data, planeSize);

        return;
    }

    CV_Assert( src.size == mask.size && mask CC_MAT_TYPE() == CV_8U );

    const img_t *arrays[]={&src, &dst, &mask, 0};
    img_t planes[3];

    NAryMatIterator it(arrays, planes);
    size_t j, k, elemSize = src CC_ELEM_SIZE(), total = planes[0] CC_MAT_TOTAL();
    size_t i, nplanes = it.nplanes;
    ...
    for( i = 0; i < nplanes; i++, ++it)
    {
        const uchar* sptr = planes[0]->tt.data;
        uchar* dptr = planes[1]->tt.data;
        const uchar* mptr = planes[2]->tt.data;
...
void set(img_t& dst, const Scalar& gamma, const img_t& mask)
{
    double buf[12];
    scalarToRawData(gamma, &buf, dst CC_MAT_TYPE(), dst CC_MAT_CN());
    const uchar* gptr = (const uchar*)&buf[0];

    if(mask.empty())
    {
        const img_t* arrays[] = {&dst, 0};
        img_t plane;
        NAryMatIterator it(arrays, &plane);
        size_t i, nplanes = it.nplanes;
        size_t j, k, elemSize = dst CC_ELEM_SIZE(), planeSize = plane CC_MAT_TOTAL()*elemSize;
        ...
        for( i = 0; i < nplanes; i++, ++it )
        {
            ...
            else if( i == 0 )
            {
                ...
                memcpy(dptr, dst->tt.data, planeSize);
        }
        return;
    }

    CV_Assert( dst.size == mask.size && mask CC_MAT_TYPE() == CV_8U );

    const img_t *arrays[]={&dst, &mask, 0};
    img_t planes[2];

    NAryMatIterator it(arrays, planes);
    size_t j, k, elemSize = dst CC_ELEM_SIZE(), total = planes[0] CC_MAT_TOTAL();
    size_t i, nplanes = it.nplanes;

    for( i = 0; i < nplanes; i++, ++it)
    {
        uchar* dptr = planes[0]->tt.data;
        const uchar* mptr = planes[1]->tt.data;
...
void insert(const img_t& src, img_t& dst, int coi)
{
    CV_Assert( dst.size == src.size && src CC_MAT_DEPTH() == dst CC_MAT_DEPTH() &&
              0 <= coi && coi < dst CC_MAT_CN() );

    const img_t* arrays[] = {&src, &dst, 0};
    img_t planes[2];
    NAryMatIterator it(arrays, planes);
    size_t i, nplanes = it.nplanes;
    size_t j, k, size0 = src CC_ELEM_SIZE(), size1 = dst CC_ELEM_SIZE(), total = planes[0] CC_MAT_TOTAL();

    for( i = 0; i < nplanes; i++, ++it )
    {
        const uchar* sptr = planes[0]->tt.data;
        uchar* dptr = planes[1]->tt.data + coi*size0;
...
void extract(const img_t& src, img_t& dst, int coi)
{
    dst cvSetMat( src.dims, &src.size[0], src CC_MAT_DEPTH() );
    CV_Assert( 0 <= coi && coi < src CC_MAT_CN() );
    ...
    for( i = 0; i < nplanes; i++, ++it )
    {
        const uchar* sptr = planes[0]->tt.data + coi*size1;
        uchar* dptr = planes[1]->tt.data;
...
void transpose(const img_t& src, img_t& dst)
{
    CV_Assert(src.dims == 2);
    dst cvSetMat(src->cols, src->rows, src CC_MAT_TYPE());
    int i, j, k, esz = (int)src CC_ELEM_SIZE();
...
randUniInt_(RNG& rng, _Tp* data, size_t total, int cn, const Scalar& scale, const Scalar& delta)
{
    for( size_t i = 0; i < total; i += cn )
        for( int k = 0; k < cn; k++ )
        {
            int val = cFloor( randInt(rng)*scale[k] + delta[k] );
...
void randUni( RNG& rng, img_t& a, const Scalar& param0, const Scalar& param1 )
{
    Scalar scale = param0;
    Scalar delta = param1;
    double C = a CC_MAT_DEPTH() < CV_32F ? 1./(65536.*65536.) : 1.;
    ...
    const img_t *arrays[]={&a, 0};
    img_t plane;

    NAryMatIterator it(arrays, &plane);
    size_t i, nplanes = it.nplanes;
    int depth = a CC_MAT_DEPTH(), cn = a CC_MAT_CN();
    size_t total = plane CC_MAT_TOTAL()*cn;
    ...
    for( i = 0; i < nplanes; i++, ++it )
    {
        ...
        {
        case CV_8U:
            randUniInt_(rng, plane img_ptr(uchar, ), total, cn, scale, delta);
            break;
        case CV_8S:
        ...
        case CV_16S:
            ...
            break;
        case CV_32S:
            randUniInt_(rng, plane img_ptr(int, ), total, cn, scale, delta);
            break;
        case CV_32F:
            randUniFlt_(rng, plane img_ptr(float, ), total, cn, scale, delta);
            break;
        case CV_64F:
            randUniFlt_(rng, plane img_ptr(double, ), total, cn, scale, delta);
            break;
        default:
            CV_Assert(0);
        }
...
template<typename _Tp> static void
erode_(const img_t& src, img_t& dst, const vector<int>& ofsvec)
{
    int width = dst->cols*src CC_MAT_CN(), n = (int)ofsvec.size();
...
template<typename _Tp> static void
dilate_(const img_t& src, img_t& dst, const vector<int>& ofsvec)
{
    int width = dst->cols*src CC_MAT_CN(), n = (int)ofsvec.size();
    const int* ofs = &ofsvec[0];
...
void erode(const img_t& _src, img_t& dst, const img_t& _kernel, Point anchor,
           int borderType, const Scalar& _borderValue)
{
    //if( _src CC_MAT_TYPE() == CV_16UC3 && _cvGetSize(src) == Size(1, 2) )
    //    putchar('*');
    img_t kernel = _kernel, src;
    Scalar borderValue = _borderValue;
    if( kernel.empty() )
        kernel = img_t::ones(3, 3, CV_8U);
    else
    {
        CV_Assert( kernel CC_MAT_TYPE() == CV_8U );
    }
    if( anchor == Point(-1,-1) )
        anchor = Point(kernel->cols/2, kernel->rows/2);
    if( borderType == CC_BORDER_CONSTANT )
        borderValue = getMaxVal(src CC_MAT_DEPTH());
    copyMakeBorder(_src, src, anchor.y, kernel->rows - anchor.y - 1,
                   anchor.x, kernel->cols - anchor.x - 1,
                   borderType, borderValue);
    dst cvSetMat( _cvGetSize(src), src CC_MAT_TYPE() );

    vector<int> ofs;
    int step = (int)(src->step/src CC_ELEM_SIZE1()), cn = src CC_MAT_CN();
...
void dilate(const img_t& _src, img_t& dst, const img_t& _kernel, Point anchor,
            int borderType, const Scalar& _borderValue)
{
    img_t kernel = _kernel, src;
    Scalar borderValue = _borderValue;
    ...
    if( anchor == Point(-1,-1) )
        anchor = Point(kernel->cols/2, kernel->rows/2);
    if( borderType == CC_BORDER_CONSTANT )
        borderValue = getMinVal(src CC_MAT_DEPTH());
    copyMakeBorder(_src, src, anchor.y, kernel->rows - anchor.y - 1,
                   anchor.x, kernel->cols - anchor.x - 1,
                   borderType, borderValue);
    dst cvSetMat( _cvGetSize(src), src CC_MAT_TYPE() );
...
template<typename _Tp> static void
filter2D_(const img_t& src, img_t& dst, const vector<int>& ofsvec, const vector<double>& coeffvec)
{
    const int* ofs = &ofsvec[0];
    const double* coeff = &coeffvec[0];
    int width = dst->cols*dst CC_MAT_CN(), ncoeffs = (int)ofsvec.size();
...
void filter2D(const img_t& _src, img_t& dst, int ddepth, const img_t& kernel,
              Point anchor, double delta, int borderType, const Scalar& _borderValue)
{
    img_t src, _dst;
    Scalar borderValue = _borderValue;
    CV_Assert( kernel CC_MAT_TYPE() == CV_32F || kernel CC_MAT_TYPE() == CV_64F );
    if( anchor == Point(-1,-1) )
        anchor = Point(kernel->cols/2, kernel->rows/2);
    if( borderType == CC_BORDER_CONSTANT )
        borderValue = getMinVal(src CC_MAT_DEPTH());
    copyMakeBorder(_src, src, anchor.y, kernel->rows - anchor.y - 1,
                   anchor.x, kernel->cols - anchor.x - 1,
                   borderType, borderValue);
    _dst cvSetMat( _cvGetSize(src), CV_MAKETYPE(CV_64F, src CC_MAT_CN()) );

    vector<int> ofs;
    vector<double> coeff(kernel->rows*kernel->cols);
    img_t cmat(kernel->rows, kernel->cols, CV_64F, &coeff[0]);
    convert(kernel, cmat, cmat CC_MAT_TYPE());
...
static int borderInterpolate( int p, int len, int borderType )
{
    ...
    else if( borderType == CC_BORDER_WRAP )
    {
        ...
        CV_Error( CC_StsBadArg, "Unknown/unsupported border type" );
    return p;
}


void copyMakeBorder(const img_t& src, img_t& dst, int top, int bottom, int left, int right,
                    int borderType, const Scalar& borderValue)
{
    dst cvSetMat(src->rows + top + bottom, src->cols + left + right, src CC_MAT_TYPE());
    int i, j, k, esz = (int)src CC_ELEM_SIZE();
    ...
    if( borderType == CC_BORDER_CONSTANT )
    {
        ...
        uchar* val = &valvec[0];
        scalarToRawData(borderValue, val, src CC_MAT_TYPE(), (src->cols + left + right)*src CC_MAT_CN());
...
static void setpos( const img_t& mtx, vector<int>& pos, size_t idx )
{
    pos.resize(mtx.dims);
    if( idx > 0 )
    {
        ...
        {
            int sz = mtx.size[i]*(i == mtx.dims-1 ? mtx CC_MAT_CN() : 1);
            pos[i] = (int)(idx % sz);
            idx /= sz;
        }
    }
...
void minMaxLoc(const img_t& src, double* _minval, double* _maxval,
               vector<int>* _minloc, vector<int>* _maxloc,
               const img_t& mask)
{
    CV_Assert( src CC_MAT_CN() == 1 );
    const img_t *arrays[]={&src, &mask, 0};
    img_t planes[2];

    NAryMatIterator it(arrays, planes);
    size_t startidx = 1, total = planes[0] CC_MAT_TOTAL();
    size_t i, nplanes = it.nplanes;
...
norm_(const _Tp* src, size_t total, int cn, int normType, double startval, const uchar* mask)
{
    ...
    if( normType == NORM_INF )
    {
        if( !mask )
            for( i = 0; i < total; i++ )
                result = MAX(result, (double)std::abs(0+src[i]));// trick with 0 used to quiet gcc warning
        else
            for( int c = 0; c < cn; c++ )
            {
                for( i = 0; i < total; i++ )
                    if( mask[i] )
                        result = MAX(result, (double)std::abs(0+src[i*cn + c]));
            }
    }
...
norm_(const _Tp* src1, const _Tp* src2, size_t total, int cn, int normType, double startval, const uchar* mask)
{
    ...
    if( normType == NORM_INF )
    {
        if( !mask )
            for( i = 0; i < total; i++ )
                result = MAX(result, (double)std::abs(src1[i] - src2[i]));
        else
            for( int c = 0; c < cn; c++ )
            {
                for( i = 0; i < total; i++ )
                    if( mask[i] )
                        result = MAX(result, (double)std::abs(src1[i*cn + c] - src2[i*cn + c]));
            }
    }
    else if( normType == NORM_L1 )
    {
...
double norm(const img_t* _src, int normType, const img_t* _mask)
{
    ...
    if( normType == NORM_HAMMING || normType == NORM_HAMMING2 )
    {
        if( !mask.empty() )
        {
            ...
            return cvtest::norm(temp, normType, img_t());
        }

        CV_Assert( src CC_MAT_DEPTH() == CV_8U );
        ...
        for( i = 0; i < nplanes; i++, ++it )
            result += normHamming(planes[0]->tt.data, total, cellSize);
        return result;
    }
    int normType0 = normType;
    normType = normType == NORM_L2SQR ? NORM_L2 : normType;

    CV_Assert( mask.empty() || (src.size == mask.size && mask CC_MAT_TYPE() == CV_8U) );
    ...
    size_t total = planes[0] CC_MAT_TOTAL();
    size_t i, nplanes = it.nplanes;
    int depth = src CC_MAT_DEPTH(), cn = planes[0] CC_MAT_CN();
    double result = 0;
    ...
    for( i = 0; i < nplanes; i++, ++it )
    {
        ...
        case CV_64F:
            ...
            break;
        default:
            CV_Error(CC_StsUnsupportedFormat, "");
        };
    }
...
double norm(const img_t* _src1, const img_t* _src2, int normType, const img_t* _mask)
{
    ...
    if( normType == NORM_HAMMING || normType == NORM_HAMMING2 )
    {
        ...
        CV_Assert( temp CC_MAT_DEPTH() == CV_8U );

        const img_t *arrays[]={&temp, 0};
        img_t planes[1];
    ...
    CV_Assert( src1 CC_MAT_TYPE() == src2 CC_MAT_TYPE() && src1.size == src2.size );
    CV_Assert( mask.empty() || (src1.size == mask.size && mask CC_MAT_TYPE() == CV_8U) );
    CV_Assert( normType == NORM_INF || normType == NORM_L1 || normType == NORM_L2 );
    ...
    size_t total = planes[0] CC_MAT_TOTAL();
    size_t i, nplanes = it.nplanes;
    int depth = src1 CC_MAT_DEPTH(), cn = planes[0] CC_MAT_CN();
    double result = 0;

    for( i = 0; i < nplanes; i++, ++it )
    {
        const uchar* sptr1 = planes[0]->tt.data;
        const uchar* sptr2 = planes[1]->tt.data;
        const uchar* mptr = planes[2]->tt.data;
...
{
    CV_Assert( _src1 CC_MAT_DEPTH() == CV_8U );
    double diff = sqrt(cvtest::norm(_src1, _src2, NORM_L2SQR)/(_src1 CC_MAT_TOTAL()*_src1 CC_MAT_CN()));
...
double crossCorr(const img_t& src1, const img_t& src2)
{
    CV_Assert( src1.size == src2.size && src1 CC_MAT_TYPE() == src2 CC_MAT_TYPE() );
...
void logicOp( const img_t& src1, const img_t& src2, img_t& dst, char op )
{
    CV_Assert( op == '&' || op == '|' || op == '^' );
    CV_Assert( src1 CC_MAT_TYPE() == src2 CC_MAT_TYPE() && src1.size == src2.size );
    dst cvSetMat( src1.dims, &src1.size[0], src1 CC_MAT_TYPE() );
    const img_t *arrays[]={&src1, &src2, &dst, 0};
    img_t planes[3];

    NAryMatIterator it(arrays, planes);
    size_t total = planes[0] CC_MAT_TOTAL()*planes[0] CC_ELEM_SIZE();
    size_t i, nplanes = it.nplanes;
...
void logicOp(const img_t& src, const Scalar& s, img_t& dst, char op)
{
    ...
    size_t i, nplanes = it.nplanes;
    double buf[12];
    scalarToRawData(s, buf, src CC_MAT_TYPE(), (int)(96/planes[0] CC_ELEM_SIZE1()));
...
compare_(const _Tp* src1, const _Tp* src2, uchar* dst, size_t total, int cmpop)
{
    ...
    case CMP_GT:
        ...
        break;
    default:
        CV_Error(CC_StsBadArg, "Unknown comparison operation");
    }
}
...
void compare(const img_t& src1, const img_t& src2, img_t& dst, int cmpop)
{
    CV_Assert( src1 CC_MAT_TYPE() == src2 CC_MAT_TYPE() && src1 CC_MAT_CN() == 1 && src1.size == src2.size );
    dst cvSetMat( src1.dims, &src1.size[0], CV_8U );
    const img_t *arrays[]={&src1, &src2, &dst, 0};
    img_t planes[3];
...
void compare(const img_t& src, double value, img_t& dst, int cmpop)
{
    CV_Assert( src CC_MAT_CN() == 1 );
    dst cvSetMat( src.dims, &src.size[0], CV_8U );
    const img_t *arrays[]={&src, &dst, 0};
    img_t planes[2];
...
bool cmpUlps(const img_t& src1, const img_t& src2, int imaxDiff, double* _realmaxdiff, vector<int>* loc)
{
    CV_Assert( src1 CC_MAT_TYPE() == src2 CC_MAT_TYPE() && src1.size == src2.size );
    const img_t *arrays[]={&src1, &src2, 0};
    img_t planes[2];
...
int check( const img_t& a, double fmin, double fmax, vector<int>* _idx )
{
    const img_t *arrays[]={&a, 0};
    img_t plane;
    NAryMatIterator it(arrays, &plane);
    size_t total = plane CC_MAT_TOTAL()*plane CC_MAT_CN();
    size_t i, nplanes = it.nplanes;
    ...
    for( i = 0; i < nplanes; i++, ++it, startidx += total )
    {
        const uchar* aptr = plane->tt.data;
...
int cmpEps( const img_t& arr, const img_t& refarr, double* _realmaxdiff,
            double success_err_level, vector<int>* _idx,
            bool element_wise_relative_error )
{
    CV_Assert( arr CC_MAT_TYPE() == refarr CC_MAT_TYPE() && arr.size == refarr.size );

    int ilevel = refarr CC_MAT_DEPTH() <= CV_32S ? cFloor(success_err_level) : 0;
    int result = CMP_EPS_OK;

    const img_t *arrays[]={&arr, &refarr, 0};
    img_t planes[2];
    NAryMatIterator it(arrays, planes);
    size_t total = planes[0] CC_MAT_TOTAL()*planes[0] CC_MAT_CN(), j = total;
    size_t i, nplanes = it.nplanes;
    int depth = arr CC_MAT_DEPTH();
    size_t startidx = 1, idx = 0;
    ...
    if( refarr CC_MAT_DEPTH() >= CV_32F && !element_wise_relative_error )
    {
...
int cmpEps2( TS* ts, const img_t& a, const img_t& b, double success_err_level,
...
{
    img_t _val(1, len, CV_64F, (void*)val);
    img_t _refval(1, len, CV_64F, (void*)refval);
...
void gemm( const img_t& _a, const img_t& _b, double alpha,
           const img_t& _c, double beta, img_t& d, int flags )
{
    img_t a = _a, b = _b, c = _c;
    ...
    if( !c.empty() && c.data == d.data && (flags & CC_GEMM_3_T) )
        c = c.clone();
    ...
    CV_Assert( a CC_MAT_TYPE() == b CC_MAT_TYPE() && a.dims == 2 && b.dims == 2 && cn <= 2 );

    if( flags & CC_GEMM_1_T )
    {
        T_SWAP(_Tp,  a_rows, a_cols );
        T_SWAP(_Tp,  a_step, a_delta );
    }

    if( flags & CC_GEMM_2_T )
    {
        T_SWAP(_Tp,  b_rows, b_cols );
        T_SWAP(_Tp,  b_step, b_delta );
    }
    ...
    if( !c.empty() )
    {
        ...
        {
            T_SWAP(_Tp,  c_rows, c_cols );
            T_SWAP(_Tp,  c_step, c_delta );
        }

        CV_Assert( c.dims == 2 && c CC_MAT_TYPE() == a CC_MAT_TYPE() && c_rows == a_rows && c_cols == b_cols );
    }

    d cvSetMat(a_rows, b_cols, a CC_MAT_TYPE());

    if( a CC_MAT_DEPTH() == CV_32F )
        GEMM_(a img_ptr(float, ), a_step, a_delta, b img_ptr(float, ), b_step, b_delta,
              !c.empty() ? c img_ptr(float, ) : 0, c_step, c_delta, d img_ptr(float, ),
              (int)d->step1(), a_rows, b_cols, a_cols, cn, alpha, beta );
    else
        GEMM_(a img_ptr(double, ), a_step, a_delta, b img_ptr(double, ), b_step, b_delta,
              !c.empty() ? c img_ptr(double, ) : 0, c_step, c_delta, d img_ptr(double, ),
              (int)d->step1(), a_rows, b_cols, a_cols, cn, alpha, beta );
}
...
void transform( const img_t& src, img_t& dst, const img_t& transmat, const img_t& _shift )
{
    double mat[20];

    int scn = src CC_MAT_CN();
    int dcn = dst CC_MAT_CN();
    int depth = src CC_MAT_DEPTH();
    int mattype = transmat CC_MAT_DEPTH();
    img_t shift = _shift cvReshape(,1, 0);
    bool haveShift = !shift.empty();

    CV_Assert( scn <= 4 && dcn <= 4 &&
              (mattype == CV_32F || mattype == CV_64F) &&
              (!haveShift || (shift CC_MAT_TYPE() == mattype && (shift->rows == 1 || shift->cols == 1))) );
...
static void minmax(const img_t& src1, const img_t& src2, img_t& dst, char op)
{
    dst cvSetMat(src1.dims, src1.size, src1 CC_MAT_TYPE());
    CV_Assert( src1 CC_MAT_TYPE() == src2 CC_MAT_TYPE() && src1.size == src2.size );
    const img_t *arrays[]={&src1, &src2, &dst, 0};
    img_t planes[3];
    ...
    NAryMatIterator it(arrays, planes);
    size_t total = planes[0] CC_MAT_TOTAL()*planes[0] CC_MAT_CN();
    size_t i, nplanes = it.nplanes, depth = src1 CC_MAT_DEPTH();
...
static void minmax(const img_t& src1, double val, img_t& dst, char op)
{
    dst cvSetMat(src1.dims, src1.size, src1 CC_MAT_TYPE());
...
void min(const img_t& src1, double val, img_t& dst)
{
    minmax( src1, val, dst, 'm' );
}

void max(const img_t& src1, double val, img_t& dst)
{
    minmax( src1, val, dst, 'M' );
}
...
static void muldiv(const img_t& src1, const img_t& src2, img_t& dst, double scale, char op)
{
    dst cvSetMat(src2.dims, src2.size, src2 CC_MAT_TYPE());
    CV_Assert( src1.empty() || (src1 CC_MAT_TYPE() == src2 CC_MAT_TYPE() && src1.size == src2.size) );
    const img_t *arrays[]={&src1, &src2, &dst, 0};
    img_t planes[3];

    NAryMatIterator it(arrays, planes);
    size_t total = planes[1] CC_MAT_TOTAL()*planes[1] CC_MAT_CN();
    size_t i, nplanes = it.nplanes, depth = src2 CC_MAT_DEPTH();
...
void multiply(const img_t& src1, const img_t& src2, img_t& dst, double scale)
{
    muldiv( src1, src2, dst, scale, '*' );
}

void divide(const img_t& src1, const img_t& src2, img_t& dst, double scale)
{
    muldiv( src1, src2, dst, scale, '/' );
}
...
Scalar mean(const img_t& src, const img_t& mask)
{
    CV_Assert(mask.empty() || (mask CC_MAT_TYPE() == CV_8U && mask.size == src.size));
    Scalar sum;
    int nz = 0;
    ...
    return sum * (1./MAX(nz, 1));
}


void  patchZeros( img_t& mat, double level )
{
    int j, ncols = mat->cols * mat CC_MAT_CN();
    int depth = mat CC_MAT_DEPTH();
...
img_t calcSobelKernel2D( int dx, int dy, int _aperture_size, int origin )
{
...
img_t calcLaplaceKernel2D( int aperture_size )
{
    int ksize = aperture_size == 1 ? 3 : aperture_size;
    img_t kernel(ksize, ksize, CV_32F);
...
void initUndistortMap( const img_t& _a0, const img_t& _k0, Size sz, img_t& _mapx, img_t& _mapy )
{
    _mapx cvSetMat(sz, CV_32F);
    _mapy cvSetMat(sz, CV_32F);

    double a[9], k[5]={0,0,0,0,0};
    img_t _a(3, 3, CV_64F, a);
    img_t _k(_k0->rows,_k0->cols, CV_MAKETYPE(CV_64F,_k0 CC_MAT_CN()),k);
...
std::ostream& operator << (std::ostream& out, const MatInfo& m)
{
    ...
    else
    {
        ...
        out << depthstr[m.m->depth()] << "C" << m.m->CC_MAT_CN() << " " << m.m->dims << "-dim (";
...
static img_t getSubArray(const img_t& m, int border, vector<int>& ofs0, vector<int>& ofs)
{
    ...
    for( i = 0; i < d; i++ )
    {
        r[i].start = MAX(0, ofs0[i] - border);
        r[i].end = MIN(ofs0[i] + 1 + border, m.size[i]);
        ofs[i] = MIN(ofs0[i], border);
    }
    return m(&r[0]);
}
...
struct MatPart
{
    MatPart(const img_t& _m, const vector<int>* _loc)
    : m(&_m), loc(_loc) {}
    const img_t* m;
...
static std::ostream& operator << (std::ostream& out, const MatPart& m)
{
    ...
    {
        int i, depth = m.m->depth(), cn = m.m->CC_MAT_CN(), width = m.m->cols*cn;
...
MatComparator::operator()(const char* expr1, const char* expr2,
                          const img_t& m1, const img_t& m2)
{
    if( m1 CC_MAT_TYPE() != m2 CC_MAT_TYPE() || m1.size != m2.size )
    ...
    img_t m[] = {m1 cvReshape(,1,0), m2 cvReshape(,1,0)};
    int dims = m[0].dims;
    vector<int> loc;
    ...
    {
        loc = loc0;
        m1part = img_t(1, 1, m[0] CC_MAT_DEPTH(), m[0].ptr(&loc[0]));
        m2part = img_t(1, 1, m[1] CC_MAT_DEPTH(), m[1].ptr(&loc[0]));
    }
    else
    {
    ...
    << maxdiff << ") between "
    << MatInfo(m1) << " '" << expr1 << "' and '" << expr2 << "' at " << img_t(loc0) << ".\n\n"
...
void threshold( const img_t& _src, img_t& _dst,
                            double thresh, double maxval, int thresh_type )
{
    int i, j;
    int depth = _src CC_MAT_DEPTH(), cn = _src CC_MAT_CN();
...
void minMaxIdx( const img_t* _img, double* minVal, double* maxVal,
                    Point* minLoc, Point* maxLoc, const img_t* _mask )
{
    img_t img = _img;
    img_t mask = _mask;
    CV_Assert(img.dims <= 2);

    _minMaxIdx((const float*)img.data, mask.data, minVal, maxVal, (size_t*)minLoc, (size_t*)maxLoc, (int)img CC_MAT_TOTAL(),1);
    if( minLoc )
