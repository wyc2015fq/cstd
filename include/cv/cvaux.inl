...
CVAPI(CvSeq*) cvSegmentImage(const img_t* src, img_t* dst,
...
typedef int (C_CDECL * CvCallback)(int index, void* buffer, void* user_data);
typedef union
{
    CvCallback callback;
    void* data;
...
CVAPI(void)  cvCalcEigenObjects(int nObjects, void* input, void* output,
                                 ...
                                 CTermCriteria* calcLimit, img_t* avg,
                                 float* eigVals);

/* Calculates dot product (obj - avg) * eigObj (i.e. projects image to eigen vector) */
CVAPI(double)  cvCalcDecompCoeff(img_t* obj, img_t* eigObj, img_t* avg);

/* Projects image to eigen space (finds all decomposion coefficients */
CVAPI(void)  cvEigenDecomposite(img_t* obj, int nEigObjs, void* eigInput,
                                 int ioFlags, void* userData, img_t* avg,
                                 float* coeffs);
...
CVAPI(void)  cvEigenProjection(void* eigInput, int nEigObjs, int ioFlags,
                                void* userData, float* coeffs, img_t* avg,
                                img_t* proj);
...
CVAPI(CvImgObsInfo*)  cvCreateObsInfo(CSize numObs, int obsSize);
...
/* The function takes an image on input and and returns the sequnce of observations
   ...
   coefficient matrix */
CVAPI(void)  cvImgToObs_DCT(const img_t* arr, float* obs, CSize dctSize,
                             CSize obsSize, CSize delta);
...
CVAPI(void)  cvCreateHandMask(CvSeq* hand_points,
                                   img_t *img_mask, CRect *roi);

/* Finds hand region in range image data */
CVAPI(void)  cvFindHandRegion (CPoint3D32f* points, int count,
                                CvSeq* indexs,
                                float* line, CSize2D32f size, int flag,
                                CPoint3D32f* center,
                                CvMemStorage* storage, CvSeq **numbers);

/* Finds hand region in range image data (advanced version) */
CVAPI(void)  cvFindHandRegionA(CPoint3D32f* points, int count,
                                CvSeq* indexs,
                                float* line, CSize2D32f size, int jc,
                                CPoint3D32f* center,
                                CvMemStorage* storage, CvSeq **numbers);
...
CVAPI(void)  icvDrawMosaic(CvSubdiv2D* subdiv, img_t* src, img_t* dst);
...
CC_INLINE double icvSqDist2D32f(CPoint2D32f pt1, CPoint2D32f pt2)
{
...
{
    CC_GRAPH_WEIGHTED_VERTEX_FIELDS()
}
CvGraphWeightedVtx;

typedef struct CvGraphWeightedEdge
{
    CC_GRAPH_WEIGHTED_EDGE_FIELDS()
}
CvGraphWeightedEdge;
...
/*CVAPI(void) cvStartFindCliques(CvGraph* graph, CvCliqueFinder* finder, int reverse, 
                                   int weighted CC_DEFAULT(0),  int weighted_edges CC_DEFAULT(0));
CVAPI(int) cvFindNextMaximalClique(CvCliqueFinder* finder, int* clock_rest CC_DEFAULT(0)); 
...
//            CC_NOT_WEIGHTED - weight of a clique is simply its size
//            CC_WEIGHTED_VTX - weight of a clique is the sum of weights of its vertices
//            CC_WEIGHTED_EDGE - the same but edges
//            CC_WEIGHTED_ALL - the same but both edges and vertices
//      weight_vtx - optional vector of floats, with size = graph->total.
//            If weight_type is either CC_WEIGHTED_VTX or CC_WEIGHTED_ALL
...
//            If weight_type is either CC_WEIGHTED_EDGE or CC_WEIGHTED_ALL
...
/*CVAPI(float) cvSubgraphWeight(CvGraph *graph, CvSeq *subgraph,
                                  CvGraphWeightType weight_type CC_DEFAULT(CC_NOT_WEIGHTED),
                                  CvVect32f weight_vtx CC_DEFAULT(0),
                                  CvMatr32f weight_edge CC_DEFAULT(0));*/
...
//            function finds them here, otherwise function expects
//            edges of graph to be inherited from CvGraphWeightedEdge.
//            Note that in case of CC_WEIGHTED_EDGE or CC_WEIGHTED_ALL
...
//    Notes:
//      in cases of CC_WEIGHTED_EDGE and CC_WEIGHTED_ALL weights should be nonnegative.
//      start_clique has a priority over subgraph_of_ban.
//F*/
/*CVAPI(CvSeq*) cvFindCliqueEx(CvGraph *graph, CvMemStorage *storage,
                                 int is_complementary CC_DEFAULT(0),
                                 CvGraphWeightType weight_type CC_DEFAULT(CC_NOT_WEIGHTED),
                                 CvVect32f weight_vtx CC_DEFAULT(0),
                                 CvMatr32f weight_edge CC_DEFAULT(0),
                                 CvSeq *start_clique CC_DEFAULT(0),
                                 CvSeq *subgraph_of_ban CC_DEFAULT(0),
                                 float *clique_weight_ptr CC_DEFAULT(0),
                                 int num_generations CC_DEFAULT(3),
                                 int quality CC_DEFAULT(2));*/
...
cvFindStereoCorrespondence(
                   const  img_t* leftImage, const  img_t* rightImage,
                   int     mode,
                   img_t*  dispImage,
                   int     maxDisparity,                                
                   double  param1 CC_DEFAULT(CC_UNDEF_SC_PARAM), 
                   double  param2 CC_DEFAULT(CC_UNDEF_SC_PARAM), 
                   double  param3 CC_DEFAULT(CC_UNDEF_SC_PARAM), 
                   double  param4 CC_DEFAULT(CC_UNDEF_SC_PARAM), 
                   double  param5 CC_DEFAULT(CC_UNDEF_SC_PARAM));
...
typedef struct CvStereoCamera
{
    ...
    /* New part for stereo */
    CPoint3D32f epipole[2];
    CPoint2D32f quad[2][4]; /* coordinates of destination quadrangle after
                                epipolar geometry rectification */
    double coeffs[2][3][3];/* coefficients for transformation */
    CPoint2D32f border[2][4];
    CSize warpSize;
...
#define CC_CAMERA_TO_WARP 1
#define CC_WARP_TO_CAMERA 2

CVAPI(int) icvConvertWarpCoordinates(double coeffs[3][3],
                                CPoint2D32f* cameraPoint,
                                CPoint2D32f* warpPoint,
                                int direction);
...
CVAPI(int) icvGetCrossPieceVector(CPoint2D32f p1_start,CPoint2D32f p1_end,CPoint2D32f v2_start,CPoint2D32f v2_end,CPoint2D32f *cross);
CVAPI(int) icvGetCrossLineDirect(CPoint2D32f p1,CPoint2D32f p2,float a,float b,float c,CPoint2D32f* cross);
CVAPI(float) icvDefinePointPosition(CPoint2D32f point1,CPoint2D32f point2,CPoint2D32f point);
CVAPI(int) icvStereoCalibration(int numImages,
                            int* nums,
                            CSize imageSize,
                            CPoint2D32f* imagePoints1,
                            CPoint2D32f* imagePoints2,
                            CPoint3D32f* objectPoints,
...
CVAPI(void) cvComputePerspectiveMap(const double coeffs[3][3], img_t* rectMapX, img_t* rectMapY);
...
CVAPI(int) icvGetAngleLine(CvPoint2D64d startPoint, CSize imageSize,CvPoint2D64d *point1,CvPoint2D64d *point2);

CVAPI(void) icvGetCoefForPiece(CvPoint2D64d p_start,CvPoint2D64d p_end,
                        double *a,double *b,double *c,
                        int* result);

/*CVAPI(void) icvGetCommonArea(CSize imageSize,
...
CVAPI(void) icvGetCrossRectDirect(CSize imageSize,
                            double a,double b,double c,
...
CVAPI(void) icvGetQuadsTransform(CSize        imageSize,
                        CvMatr64d     camMatr1,
                        CvMatr64d     rotMatr1,
                        CvVect64d     transVect1,
                        CvMatr64d     camMatr2,
...
CVAPI(img_t*) icvCreateIsometricImage(img_t* src, img_t* dst,
                              int desired_type, int desired_num_channels);

CVAPI(void) cvDeInterlace(const img_t* frame, img_t* fieldEven, img_t* fieldOdd);
...
CVAPI(CvGLCM*) cvCreateGLCM(const img_t* srcImage,
                                int stepMagnitude,
                                const int* stepDirections CC_DEFAULT(0),
                                int numStepDirections CC_DEFAULT(0),
                                int optimizationType CC_DEFAULT(CC_GLCM_OPTIMIZATION_NONE));

CVAPI(void) cvReleaseGLCM(CvGLCM** GLCM, int flag CC_DEFAULT(CC_GLCM_ALL));

CVAPI(void) cvCreateGLCMDescriptors(CvGLCM* destGLCM,
                                        int descriptorOptimizationType
                                        CC_DEFAULT(CC_GLCMDESC_OPTIMIZATION_ALLOWDOUBLENEST));
...
CVAPI(img_t*) cvCreateGLCMImage(CvGLCM* GLCM, int step);
...
CVAPI(CvFaceTracker*) cvInitFaceTracker(CvFaceTracker* pFaceTracking, const img_t* imgGray,
                                                CRect* pRects, int nRects);
CVAPI(int) cvTrackFace(CvFaceTracker* pFaceTracker, img_t* imgGray,
                              CRect* pRects, int nRects,
                              CPoint* ptRotate, double* dbAngleRotate);
...
CvSeq * cvFindFace(img_t * Image,CvMemStorage* storage);
CvSeq * cvPostBoostingFindFace(img_t * Image,CvMemStorage* storage);
...
{
    int id;
    CPoint2D32f p; // pgruebele: So we do not loose precision, this needs to be float
} Cv3dTracker2dTrackedObject;

CC_INLINE Cv3dTracker2dTrackedObject cv3dTracker2dTrackedObject(int id, CPoint2D32f p)
{
...
{
    int id;
    CPoint3D32f p;             // location of the tracked object
} Cv3dTrackerTrackedObject;

CC_INLINE Cv3dTrackerTrackedObject cv3dTrackerTrackedObject(int id, CPoint3D32f p)
{
...
typedef struct
{
    ...
    CPoint2D32f principal_point; /* copied from intrinsics so this structure */
                                  /* has all the info we need */
} Cv3dTrackerCameraInfo;

typedef struct
{
    CPoint2D32f principal_point;
...
CVAPI(CvBool) cv3dTrackerCalibrateCameras(int num_cameras,
                     ...
                     CSize etalon_size,
                     float square_size,
                     img_t *samples[],                                   /* size is num_cameras */
...
{
    CC_VORONOIEDGE2D_FIELDS()
} CvVoronoiEdge2D;

#define CC_VORONOINODE2D_FIELDS()       \
    CC_SET_ELEM_FIELDS(CvVoronoiNode2D) \
    CPoint2D32f pt;                    \
    float radius;

typedef struct CvVoronoiNode2D
{
    CC_VORONOINODE2D_FIELDS()
} CvVoronoiNode2D;

#define CC_VORONOIDIAGRAM2D_FIELDS() \
    CC_GRAPH_FIELDS()                \
    CvSet *sites;

typedef struct CvVoronoiDiagram2D
{
    CC_VORONOIDIAGRAM2D_FIELDS()
} CvVoronoiDiagram2D;
...
CVAPI(int)  cvVoronoiDiagramFromContour(CvSeq* ContourSeq,
                                           ...
                                           CvLeeParameters contour_type CC_DEFAULT(CC_LEE_INT),
                                           int contour_orientation CC_DEFAULT(-1),
                                           int attempt_number CC_DEFAULT(10));

/* Computes Voronoi Diagram for domains in given image */
CVAPI(int)  cvVoronoiDiagramFromImage(img_t* pImage,
                                         CvSeq** ContourSeq,
                                         CvVoronoiDiagram2D** VoronoiDiagram,
                                         CvMemStorage* VoronoiStorage,
                                         CvLeeParameters regularization_method CC_DEFAULT(CC_LEE_NON),
                                         float approx_precision CC_DEFAULT(CC_LEE_AUTO));
...
CVAPI(void) cvInitPerspectiveTransform(CSize size, const CPoint2D32f vertex[4], double matrix[3][3],
                                              img_t* rectMap);

/*CVAPI(void) cvInitStereoRectification(CvStereoCamera* params,
                                             img_t* rectMap1, img_t* rectMap2,
                                             int do_undistortion);*/
...
CVAPI(void)  cvMakeScanlines(const CvMatrix3* matrix, CSize  img_size,
...
CVAPI(void)  cvDeleteMoire(img_t*  img);
...
typedef void (C_CDECL * CvReleaseBGStatModel)(struct CvBGStatModel** bg_model);
typedef int (C_CDECL * CvUpdateBGStatModel)(img_t* curr_frame, struct CvBGStatModel* bg_model);

#define CC_BG_STAT_MODEL_FIELDS()                                                   \
    int             type; /*type of BG model*/                                      \
    CvReleaseBGStatModel release;                                                   \
    CvUpdateBGStatModel update;                                                     \
    img_t*       background;   /*8UC3 reference background image*/               \
    img_t*       foreground;   /*8UC1 foreground image*/                         \
    img_t**      layers;       /*8UC3 reference background image, can be null */ \
...
CC_INLINE void cvReleaseBGStatModel(CvBGStatModel** bg_model)
{
...
CC_INLINE int cvUpdateBGStatModel(img_t* current_frame, CvBGStatModel*  bg_model)
{
...
CVAPI(int)  cvChangeDetection(img_t*  prev_frame,
                               img_t*  curr_frame,
                               img_t*  change_mask);
...
/* Creates FGD model */
CVAPI(CvBGStatModel*) cvCreateFGDStatModel(img_t* first_frame,
                    CvFGDStatModelParams* parameters CC_DEFAULT(NULL));
...
#define CC_BGFG_MOG_BACKGROUND_THRESHOLD     0.7     /* threshold sum of weights for background test */
#define CC_BGFG_MOG_STD_THRESHOLD            2.5     /* lambda=2.5 is 99% */
#define CC_BGFG_MOG_WINDOW_SIZE              200     /* Learning rate; alpha = 1/CC_GBG_WINDOW_SIZE */
#define CC_BGFG_MOG_NGAUSSIANS               5       /* = K = number of Gaussians in mixture */
#define CC_BGFG_MOG_WEIGHT_INIT              0.05
#define CC_BGFG_MOG_SIGMA_INIT               30
#define CC_BGFG_MOG_MINAREA                  15.f
...
typedef struct CvGaussBGValues
{
    int         match_sum;
    double      weight;
    double      variance[CC_BGFG_MOG_NCOLORS];
    double      mean[CC_BGFG_MOG_NCOLORS];
}
CvGaussBGValues;
...
CVAPI(CvBGStatModel*) cvCreateGaussianBGModel(img_t* first_frame,
                CvGaussBGStatModelParams* parameters CC_DEFAULT(NULL));
...
{
    CC_CALIB_ETALON_USER = -1,
    CC_CALIB_ETALON_CHESSBOARD = 0,
    CC_CALIB_ETALON_CHECKERBOARD = CC_CALIB_ETALON_CHESSBOARD
}
CvCalibEtalonType;

class CC_EXPORTS CvCalibFilter
{
public:
    ...
    /* Sets etalon type - one for all cameras.
       ...
       E.g., if etalon type is CC_ETALON_TYPE_CHESSBOARD then:
    ...
    virtual bool FindEtalon(img_t** imgs);
    ...
    virtual bool Push(const CPoint2D32f** points = 0);
    ...
    virtual bool Undistort(img_t** src, img_t** dst);
    ...
    virtual bool GetLatestPoints(int idx, CPoint2D32f** pts,
                                                  int* count, bool* found);

    /* Draw the latest detected/partially detected etalon */
    virtual void DrawPoints(img_t** dst);

    /* Draw the latest detected/partially detected etalon */
    virtual void DrawPoints(img_t** dst);

    virtual bool Rectify(img_t** srcarr, img_t** dstarr);
    virtual bool Rectify(img_t** srcarr, img_t** dstarr);
...
protected:
    ...
    CPoint2D32f* points[MAX_CAMERAS];
    img_t*  undistMap[MAX_CAMERAS][2];
    img_t*  undistImg;
    int     latestCounts[MAX_CAMERAS];
    CPoint2D32f* latestPoints[MAX_CAMERAS];
    img_t*  rectMap[MAX_CAMERAS][2];
