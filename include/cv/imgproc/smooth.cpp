...
static bool ocl_boxFilter3x3_8UC1( const img_t* _src, img_t* _dst, int ddepth,
                                   Size ksize, Point anchor, int borderType, bool normalize )
{
    ...
    _dst->create(Size, CC_MAKETYPE(ddepth, cn));
...
static bool ocl_boxFilter( const img_t* _src, img_t* _dst, int ddepth,
                           Size ksize, Point anchor, int borderType, bool normalize, bool sqr = false )
{
    const ocl::Device & dev = ocl::Device::getDefault();
    int type = _src->type(), sdepth = CC_MAT_DEPTH(type), cn = CC_MAT_CN(type), esz = CC_ELEM_SIZE(type);
    ...
    int wdepth = std::max(CC_32F, std::max(ddepth, sdepth)),
         ...
         (ksize.width == 5 && ksize.height == 5 && cn == 1)))
    {
        ...
        if (!kernel.create("filterSmall", ocl::imgproc::filterSmall_oclsrc, build_options))
            return false;
    }
    else
    {
        ...
        for ( ; ; )
        {
            ...
            kernel.create("boxFilter", ocl::imgproc::boxFilter_oclsrc, opts);
            if (kernel.empty())
                return false;
                    ...
                    Point anchor, bool normalize, int borderType )
{
    ...
    static bool openvx_boxfilter(const img_t* _src, img_t* _dst, int ddepth,
                                 Size ksize, Point anchor,
                                 bool normalize, int borderType)
    {
        if (ddepth < 0)
            ddepth = CC_8UC1;
        if (_src->type() != CC_8UC1 || ddepth != CC_8U || !normalize ||
...
static bool ipp_boxfilter(Mat &src, Mat &dst, Size ksize, Point anchor, bool normalize, int borderType)
{
    ...
    if(ipp::getIppTopFeatures() == ippCPUID_SSE42 && (((src.depth() == CC_16S || src.depth() == CC_16U) && (src.channels() == 3 || src.channels() == 4)) || (src.depth() == CC_8U && src.channels() == 3 && (ksize.width > 5 || ksize.height > 5))))
        return false;

    // Other optimizations has some degradations too
    if((((src.depth() == CC_16S || src.depth() == CC_16U) && (src.channels() == 4)) || (src.depth() == CC_8U && src.channels() == 1 && (ksize.width > 5 || ksize.height > 5))))
        return false;
#endif
    ...
    try
    {
        ...
        CC_INSTRUMENT_FUN_IPP(::ipp::iwiFilterBox, iwSrc, iwDst, iwKSize, ::ipp::IwDefault(), ippBorder);
    }
    ...
    return true;
#else
    CC_UNUSED(src); CC_UNUSED(dst); CC_UNUSED(ksize); CC_UNUSED(anchor); CC_UNUSED(normalize); CC_UNUSED(borderType);
    return false;
#endif
}
...
void boxFilter( const img_t* _src, img_t* _dst, int ddepth,
                Size ksize, Point anchor,
                bool normalize, int borderType )
{
    ...
    Mat src = _src->getMat();
    int stype = src.type(), sdepth = CC_MAT_DEPTH(stype), cn = CC_MAT_CN(stype);
    if( ddepth < 0 )
        ddepth = sdepth;
    _dst->create( src.size(), CC_MAKETYPE(ddepth, cn) );
    Mat dst = _dst->getMat();
    ...
    CC_IPP_RUN_FAST(ipp_boxfilter(src, dst, ksize, anchor, normalize, borderType));
...
void sqrBoxFilter( const img_t* _src, img_t* _dst, int ddepth,
                       Size ksize, Point anchor,
                       bool normalize, int borderType )
{
    CC_INSTRUMENT_REGION()

    int srcType = _src->type(), sdepth = CC_MAT_DEPTH(srcType), cn = CC_MAT_CN(srcType);
    Size Size = _src->size();
                                   ...
                                   double sigma1, double sigma2 )
{
    ...
    if( ksize.width <= 0 && sigma1 > 0 )
        ksize.width = cvRound(sigma1*(depth == CC_8U ? 3 : 4)*2 + 1)|1;
    if( ksize.height <= 0 && sigma2 > 0 )
        ksize.height = cvRound(sigma2*(depth == CC_8U ? 3 : 4)*2 + 1)|1;
    ...
    getGaussianKernel( ksize.width, sigma1, std::max(depth, CC_32F), kx );
    if( ksize.height == ksize.width && std::abs(sigma1 - sigma2) < DBL_EPSILON )
        ky = kx;
    else
        getGaussianKernel( ksize.height, sigma2, std::max(depth, CC_32F), ky );
}
...
static bool ocl_GaussianBlur_8UC1(const img_t* _src, img_t* _dst, Size ksize, int ddepth,
                                  const img_t* _kernelX, const img_t* _kernelY, int borderType)
{
    ...
    if (ksize.width == 3)
        kernel.create("gaussianBlur3x3_8UC1_cols16_rows2", ocl::imgproc::gaussianBlur3x3_oclsrc, build_opts);
    else if (ksize.width == 5)
        kernel.create("gaussianBlur5x5_8UC1_cols4", ocl::imgproc::gaussianBlur5x5_oclsrc, build_opts);
...
static bool openvx_gaussianBlur(const img_t* _src, img_t* _dst, Size ksize,
...
public:
    ...
    virtual void operator() (const Range& range) const
    {
        ...
        try
        {
            ...
            CC_INSTRUMENT_FUN_IPP(::ipp::iwiFilterGaussian, m_src, m_dst, m_kernelSize, m_sigma, ::ipp::IwDefault(), m_border, tile);
        }
...
static bool ipp_GaussianBlur(const img_t* _src, img_t* _dst, Size ksize,
                   double sigma1, double sigma2, int borderType )
{
#ifdef HAVE_IPP_IW
    CC_INSTRUMENT_REGION_IPP()
    ...
    try
    {
        ...
        if(IPP_GAUSSIANBLUR_PARALLEL && threads > 1) {
            ...
            CC_INSTRUMENT_FUN_IPP(::ipp::iwiFilterGaussian, iwSrc, iwDst, ksize.width, sigma1, ::ipp::IwDefault(), ippBorder);
        }
    }
...
void GaussianBlur( const img_t* _src, img_t* _dst, ISize ksize,
                   double sigma1, double sigma2,
                   int borderType )
{
    CC_INSTRUMENT_REGION()
    ...
    int sdepth = CC_TYPECN_DEPTH(type), cn = CC_TYPECN_CN(type);

    if(sdepth == CC_8U && ((borderType & BORDER_ISOLATED) || !_src->getMat().isSubmatrix()))
    {
    ...
    CC_OCL_RUN(useOpenCL, ocl_GaussianBlur_8UC1(_src, _dst, ksize, CC_MAT_DEPTH(type), kx, ky, borderType));

    CC_OCL_RUN(_dst->isUMat() && _src->dims() <= 2 && (size_t)_src->rows() > kx.total() && (size_t)_src->cols() > kx.total(),
    ...
    CC_IPP_RUN_FAST(ipp_GaussianBlur(src, dst, ksize, sigma1, sigma2, borderType));
...
static bool ocl_medianFilter(const img_t* _src, img_t* _dst, int m)
{
    size_t localsize[2] = { 16, 16 };
    size_t globalsize[2];
    int type = _src->type(), depth = CC_MAT_DEPTH(type), cn = CC_MAT_CN(type);
    ...
    String kname = format( useOptimized ? "medianFilter%d_u" : "medianFilter%d", m) ;
    String kdefs = useOptimized ?
    ...
    static bool openvx_medianFilter(const img_t* _src, img_t* _dst, int ksize)
    {
        if (_src->type() != CC_8UC1 || _dst->type() != CC_8U
        ...
        try
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    Mat mask(ksize, ksize, CC_8UC1, Scalar(255));
...
static bool ipp_medianFilter(Mat &src0, Mat &dst, int ksize)
{
...

    {
        ...
        case ipp8u:
            if(channels == 1)
                return CC_INSTRUMENT_FUN_IPP(ippiFilterMedianBorder_8u_C1R, src.ptr<Ipp8u>(), (int)src.step, dst.ptr<Ipp8u>(), (int)dst.step, dstRoiSize, maskSize, ippBorderRepl, 0, buffer) >= 0;
            else if(channels == 3)
                return CC_INSTRUMENT_FUN_IPP(ippiFilterMedianBorder_8u_C3R, src.ptr<Ipp8u>(), (int)src.step, dst.ptr<Ipp8u>(), (int)dst.step, dstRoiSize, maskSize, ippBorderRepl, 0, buffer) >= 0;
            else if(channels == 4)
                return CC_INSTRUMENT_FUN_IPP(ippiFilterMedianBorder_8u_C4R, src.ptr<Ipp8u>(), (int)src.step, dst.ptr<Ipp8u>(), (int)dst.step, dstRoiSize, maskSize, ippBorderRepl, 0, buffer) >= 0;
            else
                return false;
        case ipp16u:
            if(channels == 1)
                return CC_INSTRUMENT_FUN_IPP(ippiFilterMedianBorder_16u_C1R, src.ptr<Ipp16u>(), (int)src.step, dst.ptr<Ipp16u>(), (int)dst.step, dstRoiSize, maskSize, ippBorderRepl, 0, buffer) >= 0;
            else if(channels == 3)
                return CC_INSTRUMENT_FUN_IPP(ippiFilterMedianBorder_16u_C3R, src.ptr<Ipp16u>(), (int)src.step, dst.ptr<Ipp16u>(), (int)dst.step, dstRoiSize, maskSize, ippBorderRepl, 0, buffer) >= 0;
            else if(channels == 4)
                return CC_INSTRUMENT_FUN_IPP(ippiFilterMedianBorder_16u_C4R, src.ptr<Ipp16u>(), (int)src.step, dst.ptr<Ipp16u>(), (int)dst.step, dstRoiSize, maskSize, ippBorderRepl, 0, buffer) >= 0;
            else
                return false;
        case ipp16s:
            if(channels == 1)
                return CC_INSTRUMENT_FUN_IPP(ippiFilterMedianBorder_16s_C1R, src.ptr<Ipp16s>(), (int)src.step, dst.ptr<Ipp16s>(), (int)dst.step, dstRoiSize, maskSize, ippBorderRepl, 0, buffer) >= 0;
            else if(channels == 3)
                return CC_INSTRUMENT_FUN_IPP(ippiFilterMedianBorder_16s_C3R, src.ptr<Ipp16s>(), (int)src.step, dst.ptr<Ipp16s>(), (int)dst.step, dstRoiSize, maskSize, ippBorderRepl, 0, buffer) >= 0;
            else if(channels == 4)
                return CC_INSTRUMENT_FUN_IPP(ippiFilterMedianBorder_16s_C4R, src.ptr<Ipp16s>(), (int)src.step, dst.ptr<Ipp16s>(), (int)dst.step, dstRoiSize, maskSize, ippBorderRepl, 0, buffer) >= 0;
            else
                return false;
        case ipp32f:
            if(channels == 1)
                return CC_INSTRUMENT_FUN_IPP(ippiFilterMedianBorder_32f_C1R, src.ptr<Ipp32f>(), (int)src.step, dst.ptr<Ipp32f>(), (int)dst.step, dstRoiSize, maskSize, ippBorderRepl, 0, buffer) >= 0;
            else
                return false;
        default:
            return false;
...
void medianBlur( const img_t* _src0, img_t* _dst, int ksize )
{
    CC_INSTRUMENT_REGION()
    ...
    CC_IPP_RUN_FAST(ipp_medianFilter(src0, dst, ksize));
...
#if !(CC_SIMD128)
            && ( src0.depth() > CC_8U || src0.channels() == 2 || src0.channels() > 4 )
#endif
        );
    ...
    {
        copyMakeBorder( src0, src, 0, 0, ksize/2, ksize/2, BORDER_REPLICATE|BORDER_ISOLATED);

        int cn = src0.channels();
        CC_Assert( src.depth() == CC_8U && (cn == 1 || cn == 3 || cn == 4) );

        double img_size_mp = (double)(src0.total())/(1 << 20);
        if( ksize <= 3 + (img_size_mp < 1 ? 12 : img_size_mp < 4 ? 6 : 2)*
            (CC_SIMD128 && hasSIMD128() ? 1 : 3))
...
static bool ocl_bilateralFilter_8u(const img_t* _src, img_t* _dst, int d,
                                   ...
                                   int borderType)
{
    ...
    Mat mspace_weight(1, d * d, CC_32FC1, space_weight);
    Mat mspace_ofs(1, d * d, CC_32SC1, space_ofs);
    ...
    int borderType )
{
    ...
    CC_Assert( (src.type() == CC_8UC1 || src.type() == CC_8UC3) && src.data != dst.data );
                     ...
                     int borderType )
{
    ...
    CC_Assert( (src.type() == CC_32FC1 || src.type() == CC_32FC3) && src.data != dst.data );
...
public:
    ...
    virtual void operator() (const Range& range) const
    {
        ...
        try
        {
            ...
            CC_INSTRUMENT_FUN_IPP(::ipp::iwiFilterBilateral, src, dst, radius, valSquareSigma, posSquareSigma, ::ipp::IwDefault(), borderType, tile);
        }
        catch(::ipp::IwException)
        {
            *pOk = false;
...
static bool ipp_bilateralFilter(Mat &src, Mat &dst, int d, double sigmaColor, double sigmaSpace, int borderType)
{
    ...
    try
    {
        ...
        if(IPP_BILATERAL_PARALLEL && threads > 1) {
            ...
            CC_INSTRUMENT_FUN_IPP(::ipp::iwiFilterBilateral, iwSrc, iwDst, radius, valSquareSigma, posSquareSigma, ::ipp::IwDefault(), ippBorder);
        }
    }
    catch (::ipp::IwException)
    {
    ...
    return true;
#else
    CC_UNUSED(src); CC_UNUSED(dst); CC_UNUSED(d); CC_UNUSED(sigmaColor); CC_UNUSED(sigmaSpace); CC_UNUSED(borderType);
    return false;
#endif
}
#endif
...
void bilateralFilter( const img_t* _src, img_t* _dst, int d,
                      ...
                      int borderType )
{
    ...
    CC_IPP_RUN_FAST(ipp_bilateralFilter(src, dst, d, sigmaColor, sigmaSpace, borderType));
...
{
    Mat src = cvarrToMat(srcarr), dst0 = cvarrToMat(dstarr), dst = dst0;

    CC_Assert( dst.size() == src.size() &&
        (smooth_type == CC_BLUR_NO_SCALE || dst.type() == src.type()) );

    if( param2 <= 0 )
        param2 = param1;

    if( smooth_type == CC_BLUR || smooth_type == CC_BLUR_NO_SCALE )
        boxFilter( src, dst, dst.depth(), Size(param1, param2), Point(-1,-1),
            smooth_type == CC_BLUR, BORDER_REPLICATE );
    else if( smooth_type == CC_GAUSSIAN )
        GaussianBlur( src, dst, Size(param1, param2), param3, param4, BORDER_REPLICATE );
    else if( smooth_type == CC_MEDIAN )
        medianBlur( src, dst, param1 );
    else
        bilateralFilter( src, dst, param1, param3, param4, BORDER_REPLICATE );
