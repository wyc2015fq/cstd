...
BaseFilter::BaseFilter() { ksize = CSize(-1,-1); anchor = Point(-1,-1); }
...
FilterEngine::FilterEngine()
{
    ...
    rowBorderType = columnBorderType = CC_BORDER_REPLICATE;
...
void FilterEngine::init( const Ptr<BaseFilter>& _filter2D,
                         ...
                         const Scalar& _borderValue )
{
    _srcType = CC_MAT_TYPE(_srcType);
    _bufType = CC_MAT_TYPE(_bufType);
    _dstType = CC_MAT_TYPE(_dstType);
    ...
    CC_Assert( columnBorderType != CC_BORDER_WRAP );

    if( isSeparable() )
    {
        CC_Assert( rowFilter && columnFilter );
        ksize = CSize(rowFilter->ksize, columnFilter->ksize);
        anchor = Point(rowFilter->anchor, columnFilter->anchor);
    }
    else
    {
        CC_Assert( bufType == srcType );
        ksize = filter2D->ksize;
        anchor = filter2D->anchor;
    }

    CC_Assert( 0 <= anchor.x && anchor.x < ksize.width &&
               0 <= anchor.y && anchor.y < ksize.height );

    borderElemSize = srcElemSize/(CC_MAT_DEPTH(srcType) >= CC_32S ? sizeof(int) : 1);
    ...
    if( rowBorderType == CC_BORDER_CONSTANT || columnBorderType == CC_BORDER_CONSTANT )
    {
        constBorderValue.resize(srcElemSize*borderLength);
        int srcType1 = CC_MAKETYPE(CC_MAT_DEPTH(srcType), MIN(CC_MAT_CN(srcType), 4));
        scalarToRawData(_borderValue, &constBorderValue[0], srcType1,
                        borderLength*CC_MAT_CN(srcType));
    }

    wholeSize = CSize(-1,-1);
}

#define VEC_ALIGN CC_MALLOC_ALIGN

int FilterEngine::start(const CSize &_wholeSize, const CSize &sz, const Point &ofs)
{
    int i, j;

    wholeSize = _wholeSize;
    roi = Rect(ofs, sz);
    CC_Assert( roi.x >= 0 && roi.y >= 0 && roi.width >= 0 && roi.height >= 0 &&
    ...
    int _maxBufRows = MAX(ksize.height + 3, MAX(anchor.y, ksize.height-anchor.y-1)*2+1);
    ...
    if( maxWidth < roi.width || _maxBufRows != (int)rows.size() )
    {
        ...
        if( columnBorderType == CC_BORDER_CONSTANT )
        {
            constBorderRow.resize(getElemSize(bufType)*(maxWidth + ksize.width - 1 + VEC_ALIGN));
            uchar *dst = cAlignPtr(&constBorderRow[0], VEC_ALIGN), *tdst;
        ...
        int maxBufStep = bufElemSize*(int)cAlignSize(maxWidth +
    ...
    bufStep = bufElemSize*(int)cAlignSize(roi.width + (!isSeparable() ? ksize.width - 1 : 0),16);
    ...
    if( dx1 > 0 || dx2 > 0 )
    {
        if( rowBorderType == CC_BORDER_CONSTANT )
        {
            int nr = isSeparable() ? 1 : (int)rows.size();
            for( i = 0; i < nr; i++ )
            {
                uchar* dst = isSeparable() ? &srcRow[0] : cAlignPtr(&ringBuf[0],VEC_ALIGN) + bufStep*i;
...
int FilterEngine::start(const img_t& src, const CSize &wsz, const Point &ofs)
{
    start( wsz, cvGetSize(src), ofs);
    return startY - ofs.y;
}
                           ...
                           uchar* dst, int dststep )
{
    ...
    bool makeBorder = (_dx1 > 0 || _dx2 > 0) && rowBorderType != CC_BORDER_CONSTANT;
    int dy = 0, i = 0;

    src -= xofs1*esz;
    count = MIN(count, remainingInputRows());

    CC_Assert( src && dst && count > 0 );
    ...
    for(;; dst += dststep*i, dy += i)
    {
        ...
        for( ; dcount-- > 0; src += srcstep )
        {
            ...
            uchar* brow = cAlignPtr(&ringBuf[0], VEC_ALIGN) + bi*bufStep;
            ...
            if( isSep )
                (*rowFilter)(row, brow, width, CC_MAT_CN(srcType));
        }
        ...
        for( i = 0; i < max_i; i++ )
        {
            ...
            if( srcY < 0 ) // can happen only with constant border type
                brows[i] = cAlignPtr(&constBorderRow[0], VEC_ALIGN);
            else
            {
                CC_Assert( srcY >= startY );
                if( srcY >= startY + rowCount )
                    break;
                int bi = (srcY - startY0) % bufRows;
                brows[i] = cAlignPtr(&ringBuf[0], VEC_ALIGN) + bi*bufStep;
            }
        }
        if( i < kheight )
            break;
    ...
    dstY += dy;
    CC_Assert( dstY <= roi.height );
    return dy;
}

void FilterEngine::apply(const img_t& src, img_t& dst, const CSize & wsz, const Point & ofs)
{
    CC_INSTRUMENT_REGION()

    CC_Assert( src->tid == srcType && dst->tid == dstType );

    int y = start(src, wsz, ofs);
    proceed(src->tt.data + y*src->step,
            (int)src->step,
            endY - startY,
            dst->tt.data,
            (int)dst->step );
...
int getKernelType(const img_t* filter_kernel, Point anchor)
{
    img_t _kernel = filter_kernel;
    CC_Assert( _kernel CC_MAT_CN() == 1 );
    int i, sz = _kernel->rows*_kernel->cols;

    img_t kernel;
    _kernel.convertTo(kernel, CC_64F);

    const double* coeffs = kernel img_ptr(double, );
    double sum = 0;
    int type = CC_KERNEL_SMOOTH + CC_KERNEL_INTEGER;
    if( (_kernel->rows == 1 || _kernel->cols == 1) &&
        anchor.x*2 + 1 == _kernel->cols &&
        anchor.y*2 + 1 == _kernel->rows )
        type |= (CC_KERNEL_SYMMETRICAL + CC_KERNEL_ASYMMETRICAL);

    for( i = 0; i < sz; i++ )
    {
        double a = coeffs[i], b = coeffs[sz - i - 1];
        if( a != b )
            type &= ~CC_KERNEL_SYMMETRICAL;
        if( a != -b )
            type &= ~CC_KERNEL_ASYMMETRICAL;
        if( a < 0 )
            type &= ~CC_KERNEL_SMOOTH;
        if( a != saturate_cast<int>(a) )
            type &= ~CC_KERNEL_INTEGER;
        sum += a;
    }
...
{
    ColumnNoVec() {}
    ColumnNoVec(const img_t&, int, int, double) {}
...
{
    SymmRowSmallNoVec() {}
    SymmRowSmallNoVec(const img_t&, int) {}
    int operator()(const uchar*, uchar*, int, int) const { return 0; }
};
...
struct SymmColumnSmallNoVec
{
    ...
    SymmColumnSmallNoVec(const img_t&, int, int, double) {}
    int operator()(const uchar**, uchar*, int) const { return 0; }
};
...
{
    FilterNoVec() {}
    FilterNoVec(const img_t&, int, double) {}
    int operator()(const uchar**, uchar*, int) const { return 0; }
};
...
struct RowVec_8u32s
{
    ...
    RowVec_8u32s( const img_t& _kernel )
    {
        kernel = _kernel;
        smallValues = true;
    ...
    int operator()(const uchar* _src, uchar* _dst, int width, int cn) const
    {
        ...
        int* dst = (int*)_dst;
        const int* _kx = kernel img_ptr(int, );
        width *= cn;
...
struct SymmRowSmallVec_8u32s
{
    ...
    SymmRowSmallVec_8u32s( const img_t& _kernel, int _symmetryType )
    {
        kernel = _kernel;
    ...
    int operator()(const uchar* src, uchar* _dst, int width, int cn) const
    {
        ...
        int* dst = (int*)_dst;
        bool symmetrical = (symmetryType & CC_KERNEL_SYMMETRICAL) != 0;
        const int* kx = kernel img_ptr(int, ) + _ksize/2;
        if( !smallValues )
            return 0;
...
struct SymmColumnVec_32s8u
{
    ...
    SymmColumnVec_32s8u(const img_t& _kernel, int _symmetryType, int _bits, double _delta)
    {
        symmetryType = _symmetryType;
        _kernel.convertTo(kernel, CC_32F, 1./(1 << _bits), 0);
        delta = (float)(_delta/(1 << _bits));
        CC_Assert( (symmetryType & (CC_KERNEL_SYMMETRICAL | CC_KERNEL_ASYMMETRICAL)) != 0 );
    }
    ...
    int operator()(const uchar** _src, uchar* dst, int width) const
    {
        ...
        const float* ky = kernel img_ptr(float, ) + ksize2;
        int i = 0, k;
        bool symmetrical = (symmetryType & CC_KERNEL_SYMMETRICAL) != 0;
...
struct SymmColumnSmallVec_32s16s
{
    ...
    SymmColumnSmallVec_32s16s(const img_t& _kernel, int _symmetryType, int _bits, double _delta)
    {
        symmetryType = _symmetryType;
        _kernel.convertTo(kernel, CC_32F, 1./(1 << _bits), 0);
        delta = (float)(_delta/(1 << _bits));
...
{
    RowVec_16s32f() {}
    RowVec_16s32f( const img_t& _kernel )
    {
        kernel = _kernel;
        sse2_supported = checkHardwareSupport(CC_CPU_SSE2);
    }
    ...
    int operator()(const uchar* _src, uchar* _dst, int width, int cn) const
    {
        ...
        const float* _kx = kernel img_ptr(float, );
        width *= cn;
...
struct SymmColumnVec_32f16s
{
    ...
    SymmColumnVec_32f16s(const img_t& _kernel, int _symmetryType, int, double _delta)
    {
        symmetryType = _symmetryType;
        kernel = _kernel;
        delta = (float)_delta;
...
struct RowVec_32f
{
    RowVec_32f()
    {
        haveSSE = checkHardwareSupport(CC_CPU_SSE);
    }

    RowVec_32f( const img_t& _kernel )
    {
        kernel = _kernel;
        haveSSE = checkHardwareSupport(CC_CPU_SSE);
...
private:
    ...
    int ippiOperator(const uchar* _src, uchar* _dst, int width, int cn) const
    {
        ...
        CAutoBuffer<uchar> buf(bufsz + 64);
        ...
        if( (cn == 1 && CC_INSTRUMENT_FUN_IPP(ippiFilterRowBorderPipeline_32f_C1R,(src, step, &dst, roisz, _kx, _ksize, 0,
                                                            ippBorderRepl, borderValue[0], bufptr)) < 0) ||
            (cn == 3 && CC_INSTRUMENT_FUN_IPP(ippiFilterRowBorderPipeline_32f_C3R,(src, step, &dst, roisz, _kx, _ksize, 0,
...
{
    SymmRowSmallVec_32f() {}
    SymmRowSmallVec_32f( const img_t& _kernel, int _symmetryType )
    {
        kernel = _kernel;
        symmetryType = _symmetryType;
    }
    ...
    int operator()(const uchar* _src, uchar* _dst, int width, int cn) const
    {
        ...
        bool symmetrical = (symmetryType & CC_KERNEL_SYMMETRICAL) != 0;
        const float* kx = kernel img_ptr(float, ) + _ksize/2;
        width *= cn;
...
struct SymmColumnVec_32f
{
    ...
    SymmColumnVec_32f(const img_t& _kernel, int _symmetryType, int, double _delta)
    {
        symmetryType = _symmetryType;
        kernel = _kernel;
        delta = (float)_delta;
...
struct SymmColumnSmallVec_32f
{
    ...
    SymmColumnSmallVec_32f(const img_t& _kernel, int _symmetryType, int, double _delta)
    {
        symmetryType = _symmetryType;
        kernel = _kernel;
        delta = (float)_delta;
...
{
    FilterVec_8u() {}
    FilterVec_8u(const img_t& _kernel, int _bits, double _delta)
    {
        img_t kernel;
        _kernel.convertTo(kernel, CC_32F, 1./(1 << _bits), 0);
        delta = (float)(_delta/(1 << _bits));
...
{
    FilterVec_8u16s() {}
    FilterVec_8u16s(const img_t& _kernel, int _bits, double _delta)
    {
        img_t kernel;
        _kernel.convertTo(kernel, CC_32F, 1./(1 << _bits), 0);
        delta = (float)(_delta/(1 << _bits));
...
{
    FilterVec_32f() {}
    FilterVec_32f(const img_t& _kernel, int, double _delta)
    {
        delta = (float)_delta;
        std::vector<Point> coords;
...
struct SymmRowSmallVec_8u32s
{
    ...
    int operator()(const uchar* src, uchar* _dst, int width, int cn) const
    {
         if( !checkHardwareSupport(CC_CPU_NEON) )
             return 0;
...
struct SymmColumnVec_32f16s
{
    ...
    SymmColumnVec_32f16s(const img_t& _kernel, int _symmetryType, int, double _delta)
    {
        ...
        delta = (float)_delta;
        CC_Assert( (symmetryType & (CC_KERNEL_SYMMETRICAL | CC_KERNEL_ASYMMETRICAL)) != 0 );
         neon_supported = checkHardwareSupport(CC_CPU_NEON);
    }
...
{
    RowFilter( const img_t& _kernel, int _anchor, const VecOp& _vecOp=VecOp() )
    {
        if( _kernel CC_IS_CONT_MAT() )
            kernel = _kernel;
        else
            _kernel.copyTo(kernel);
        anchor = _anchor;
        ksize = kernel->rows + kernel->cols - 1;
        CC_Assert( kernel->tid == DataType<DT>::type &&
...
{
    SymmRowSmallFilter( const img_t& _kernel, int _anchor, int _symmetryType,
                        const VecOp& _vecOp = VecOp())
        : RowFilter<ST, DT, VecOp>( _kernel, _anchor, _vecOp )
    {
        symmetryType = _symmetryType;
        CC_Assert( (symmetryType & (CC_KERNEL_SYMMETRICAL | CC_KERNEL_ASYMMETRICAL)) != 0 && this->ksize <= 5 );
    }
    ...
    void operator()(const uchar* src, uchar* dst, int width, int cn)
    {
        ...
        bool symmetrical = (this->symmetryType & CC_KERNEL_SYMMETRICAL) != 0;
        DT* D = (DT*)dst;
...
template<class CastOp, class VecOp> struct ColumnFilter : public BaseColumnFilter
{
    ...
    ColumnFilter( const img_t& _kernel, int _anchor,
        ...
        const VecOp& _vecOp=VecOp() )
    {
        ...
        castOp0 = _castOp;
        vecOp = _vecOp;
        CC_Assert( kernel->tid == DataType<ST>::type &&
                   (kernel->rows == 1 || kernel->cols == 1));
    }
...
template<class CastOp, class VecOp> struct SymmColumnFilter : public ColumnFilter<CastOp, VecOp>
{
    ...
    SymmColumnFilter( const img_t& _kernel, int _anchor,
...
Ptr<BaseRowFilter> getLinearRowFilter( int srcType, int bufType,
                                                   const img_t* _kernel, int anchor,
                                                   int symmetryType )
{
    img_t kernel = _kernel;
    int sdepth = CC_MAT_DEPTH(srcType), ddepth = CC_MAT_DEPTH(bufType);
    int cn = CC_MAT_CN(srcType);
    CC_Assert( cn == CC_MAT_CN(bufType) &&
        ddepth >= MAX(sdepth, CC_32S) &&
        kernel->tid == ddepth );
    int ksize = kernel->rows + kernel->cols - 1;

    if( (symmetryType & (CC_KERNEL_SYMMETRICAL|CC_KERNEL_ASYMMETRICAL)) != 0 && ksize <= 5 )
    {
        ...
        if( sdepth == CC_32F && ddepth == CC_32F )
                                             ...
                                             int bits )
{
    ...
    else
    {
        ...
        if( ksize == 3 )
        {
            ...
            if( ddepth == CC_16S && sdepth == CC_32S && bits == 0 )
        ...
        if( ddepth == CC_16U && sdepth == CC_64F )
            return makePtr<SymmColumnFilter<Cast<double, ushort>, ColumnNoVec> >
                (kernel, anchor, delta, symmetryType);
        if( ddepth == CC_16S && sdepth == CC_32S )
...
Ptr<FilterEngine> createSeparableLinearFilter(
    ...
    const img_t* __rowKernel, const img_t* __columnKernel,
    Point _anchor, double _delta,
    int _rowBorderType, int _columnBorderType,
    const Scalar& _borderValue )
{
    img_t _rowKernel = __rowKernel, _columnKernel = __columnKernel;
    _srcType = CC_MAT_TYPE(_srcType);
    _dstType = CC_MAT_TYPE(_dstType);
    int sdepth = CC_MAT_DEPTH(_srcType), ddepth = CC_MAT_DEPTH(_dstType);
    int cn = CC_MAT_CN(_srcType);
    CC_Assert( cn == CC_MAT_CN(_dstType) );
    ...
    img_t rowKernel, columnKernel;

    int bdepth = MAX(CC_32F,MAX(sdepth, ddepth));
    int bits = 0;

    if( sdepth == CC_8U &&
        ((rtype == CC_KERNEL_SMOOTH+CC_KERNEL_SYMMETRICAL &&
          ctype == CC_KERNEL_SMOOTH+CC_KERNEL_SYMMETRICAL &&
          ddepth == CC_8U) ||
         ((rtype & (CC_KERNEL_SYMMETRICAL+CC_KERNEL_ASYMMETRICAL)) &&
          (ctype & (CC_KERNEL_SYMMETRICAL+CC_KERNEL_ASYMMETRICAL)) &&
          (rtype & ctype & CC_KERNEL_INTEGER) &&
          ddepth == CC_16S)) )
    {
        bdepth = CC_32S;
        bits = ddepth == CC_8U ? 8 : 0;
        _rowKernel.convertTo( rowKernel, CC_32S, 1 << bits );
        _columnKernel.convertTo( columnKernel, CC_32S, 1 << bits );
        bits *= 2;
        _delta *= (1 << bits);
    }
    else
    {
        if( _rowKernel->tid != bdepth )
            _rowKernel.convertTo( rowKernel, bdepth );
        else
            rowKernel = _rowKernel;
        if( _columnKernel->tid != bdepth )
            _columnKernel.convertTo( columnKernel, bdepth );
        else
            columnKernel = _columnKernel;
    }

    int _bufType = CC_MAKETYPE(bdepth, cn);
...
void preprocess2DKernel( const img_t& kernel, std::vector<Point>& coords, std::vector<uchar>& coeffs )
{
    int i, j, k, nz = countNonZero(kernel), ktype = kernel->tid;
    if(nz == 0)
        nz = 1;
    CC_Assert( ktype == CC_8U || ktype == CC_32S || ktype == CC_32F || ktype == CC_64F );
    coords.resize(nz);
...
template<typename ST, class CastOp, class VecOp> struct Filter2D : public BaseFilter
{
    ...
    Filter2D( const img_t& _kernel, Point _anchor,
        double _delta, const CastOp& _castOp=CastOp(),
        const VecOp& _vecOp=VecOp() )
    {
        anchor = _anchor;
        ...
        castOp0 = _castOp;
        vecOp = _vecOp;
        CC_Assert( _kernel->tid == DataType<KT>::type );
        preprocess2DKernel( _kernel, coords, coeffs );
...
// c f i c f i 0 0
template <typename T>
static int _prepareKernelFilter2D(std::vector<T> & data, const img_t & kernel)
{
    img_t _kernel; kernel.convertTo(_kernel, DataDepth<T>::value);
...
static bool ocl_filter2D( const img_t* _src, img_t* dst, int ddepth,
                   const img_t* _kernel, Point anchor,
                   double delta, int borderType )
{
    int type = _src->tid, sdepth = CC_MAT_DEPTH(type), cn = CC_MAT_CN(type);
    ddepth = ddepth < 0 ? sdepth : ddepth;
    int dtype = CC_MAKE_TYPE(ddepth, cn), wdepth = MAX(MAX(sdepth, ddepth), CC_32F),
            wtype = CC_MAKE_TYPE(wdepth, cn);
    if (cn > 4)
        return false;
    ...
    if (wdepth == CC_64F && !doubleSupport)
        return false;

    const char * const borderMap[] = { "CC_BORDER_CONSTANT", "CC_BORDER_REPLICATE", "CC_BORDER_REFLECT",
                                       "CC_BORDER_WRAP", "CC_BORDER_REFLECT_101" };

    img_t kernelMat = _kernel;
    CSize sz = _cvGetSize(src), wholeSize;
    ...
    UMat kernalDataUMat;
    if (device.isIntel() && (device->tid & ocl::Device::TYPE_GPU) &&
        ((ksize.width < 5 && ksize.height < 5) ||
        (ksize.width == 5 && ksize.height == 5 && cn == 1)))
    {
        kernelMat = kernelMat cvReshape(,0, 1);
        String kerStr = ocl::kernelToStr(kernelMat, CC_32F);
        ...
        if (!k cvSetMat("filter2DSmall", ocl::imgproc::filter2DSmall_oclsrc, build_options))
            return false;
    }
    else
    {
        ...
        String kerStr = ocl::kernelToStr(kernelMatDataFloat, CC_32F);
        ...
        for ( ; ; )
        {
            ...
            if (!k cvSetMat("filter2D", ocl::imgproc::filter2D_oclsrc, opts))
                return false;
    ...
    _dst cvSetMat(sz, dtype);
    UMat dst = _dst.getUMat();

    int srcOffsetX = (int)((src.offset % src->step) / src CC_ELEM_SIZE());
...
static bool ocl_sepRowFilter2D(const UMat & src, UMat & buf, const img_t & kernelX, int anchor,
                               int borderType, int ddepth, bool fast8uc1, bool int_arithm)
{
    int type = src->tid, cn = CC_MAT_CN(type), sdepth = CC_MAT_DEPTH(type);
    bool doubleSupport = ocl::Device::getDefault().doubleFPConfig() > 0;
    CSize bufSize = buf.size();
    int buf_type = buf->tid, bdepth = CC_MAT_DEPTH(buf_type);
    ...
    const char * const borderMap[] = { "CC_BORDER_CONSTANT", "CC_BORDER_REPLICATE", "CC_BORDER_REFLECT", "CC_BORDER_WRAP", "CC_BORDER_REFLECT_101" },
        * const btype = borderMap[borderType & ~CC_BORDER_ISOLATED];
    ...
    char cvt[40];
    String build_options = format("-D RADIUSX=%d -D LSIZE0=%d -D LSIZE1=%d -D CN=%d -D %s -D %s -D %s"
    ...
    CSize srcWholeSize; Point srcOffset;
    ...
    ocl::Kernel k(kernelName.c_str(), ocl::imgproc::filterSepRow_oclsrc,
                  build_options);
    if (k.empty())
        return false;

    if (fast8uc1)
        k.args(ocl::KernelArg::PtrReadOnly(src), (int)(src->step / src CC_ELEM_SIZE()), srcOffset.x,
               srcOffset.y, src->cols, src->rows, srcWholeSize.width, srcWholeSize.height,
               ocl::KernelArg::PtrWriteOnly(buf), (int)(buf->step / buf CC_ELEM_SIZE()),
...
static bool ocl_sepColFilter2D(const UMat & buf, UMat & dst, const img_t & kernelY, double delta, int anchor, bool int_arithm)
{
    bool doubleSupport = ocl::Device::getDefault().doubleFPConfig() > 0;
    if (dst CC_MAT_DEPTH() == CC_64F && !doubleSupport)
        return false;
    ...
    int dtype = dst->tid, cn = CC_MAT_CN(dtype), ddepth = CC_MAT_DEPTH(dtype);
    CSize sz = dst.size();
    int buf_type = buf->tid, bdepth = CC_MAT_DEPTH(buf_type);
    ...
    char cvt[40];
    String build_options = format("-D RADIUSY=%d -D LSIZE0=%d -D LSIZE1=%d -D CN=%d"
    ...
    ocl::Kernel k("col_filter", ocl::imgproc::filterSepCol_oclsrc,
                  build_options);
    if (k.empty())
        return false;
...
static bool ocl_sepFilter2D_SinglePass(const img_t* _src, img_t* dst,
                                       img_t row_kernel, img_t col_kernel,
                                       double delta, int borderType, int ddepth, int bdepth, bool int_arithm)
{
    ...
    int stype = _src->tid, sdepth = CC_MAT_DEPTH(stype), cn = CC_MAT_CN(stype),
            esz = CC_ELEM_SIZE(stype), wdepth = MAX(MAX(sdepth, ddepth), bdepth),
            dtype = CC_MAKE_TYPE(ddepth, cn);
    size_t src_step = _src->step(), src_offset = _src.offset();
    bool doubleSupport = ocl::Device::getDefault().doubleFPConfig() > 0;

    if ((src_offset % src_step) % esz != 0 || (!doubleSupport && (sdepth == CC_64F || ddepth == CC_64F)) ||
            !(borderType == CC_BORDER_CONSTANT || borderType == CC_BORDER_REPLICATE ||
              borderType == CC_BORDER_REFLECT || borderType == CC_BORDER_WRAP ||
              borderType == CC_BORDER_REFLECT_101))
        return false;
    ...
    String opts = format("-D BLK_X=%d -D BLK_Y=%d -D RADIUSX=%d -D RADIUSY=%d%s%s"
                             ...
                             ocl::typeToStr(CC_MAKE_TYPE(wdepth, cn)), ocl::typeToStr(dtype),
...
static bool ocl_sepFilter2D( const img_t* _src, img_t* dst, int ddepth,
                      const img_t* _kernelX, const img_t* _kernelY, Point anchor,
                      double delta, int borderType )
{
    const ocl::Device & d = ocl::Device::getDefault();
    CSize imgSize = _cvGetSize(src);
        ...
        ctype == CC_KERNEL_SMOOTH+CC_KERNEL_SYMMETRICAL)
    {
        if (ocl::Device::getDefault().isIntel())
        {
            for (int i=0; i<kernelX->cols; i++)
                kernelX.at<float>(0, i) = (float) cRound(kernelX.at<float>(0, i) * (1 << shift_bits));
            if (kernelX.data != kernelY.data)
                for (int i=0; i<kernelX->cols; i++)
                    kernelY.at<float>(0, i) = (float) cRound(kernelY.at<float>(0, i) * (1 << shift_bits));
        } else
        {
            bdepth = CC_32S;
    ...
    CC_OCL_RUN_(kernelY->cols <= 21 && kernelX->cols <= 21 &&
                ...
                ocl_sepFilter2D_SinglePass(_src, _dst, kernelX, kernelY, delta,
                                           borderType & ~CC_BORDER_ISOLATED, ddepth, bdepth, int_arithm), true)

    UMat src = _src.getUMat();
    CSize srcWholeSize; Point srcOffset;
    src.locateROI(srcWholeSize, srcOffset);

    bool fast8uc1 = type == CC_8UC1 && srcOffset.x % 4 == 0 &&
            src->cols % 4 == 0 && src->step % 4 == 0;

    CSize srcSize = cvGetSize(src);
    CSize bufSize(srcSize.width, srcSize.height + kernelY->cols - 1);
    UMat buf(bufSize, CC_MAKETYPE(bdepth, cn));
    if (!ocl_sepRowFilter2D(src, buf, kernelX, anchor.x, borderType, ddepth, fast8uc1, int_arithm))
        return false;

    _dst cvSetMat(srcSize, CC_MAKETYPE(ddepth, cn));
    UMat dst = _dst.getUMat();
...
Ptr<BaseFilter> getLinearFilter(int srcType, int dstType,
                                const img_t* filter_kernel, Point anchor,
                                double delta, int bits)
{
    img_t _kernel = filter_kernel;
    int sdepth = CC_MAT_DEPTH(srcType), ddepth = CC_MAT_DEPTH(dstType);
    int cn = CC_MAT_CN(srcType), kdepth = _kernel CC_MAT_DEPTH();
    CC_Assert( cn == CC_MAT_CN(dstType) && ddepth >= sdepth );

    anchor = normalizeAnchor(anchor, _kernel.size());

    /*if( sdepth == CC_8U && ddepth == CC_8U && kdepth == CC_32S )
    ...
    if( sdepth == CC_8U && ddepth == CC_16S && kdepth == CC_32S )
    ...
    kdepth = sdepth == CC_64F || ddepth == CC_64F ? CC_64F : CC_32F;
    img_t kernel;
    if( _kernel->tid == kdepth )
        kernel = _kernel;
    else
        _kernel.convertTo(kernel, kdepth, _kernel->tid == CC_32S ? 1./(1 << bits) : 1.);
    ...
    if( sdepth == CC_16U && ddepth == CC_16U )
    ...
    if( sdepth == CC_16S && ddepth == CC_16S )
...
Ptr<FilterEngine> createLinearFilter( int _srcType, int _dstType,
                                              const img_t* filter_kernel,
                                              Point _anchor, double _delta,
                                              int _rowBorderType, int _columnBorderType,
                                              const Scalar& _borderValue )
{
    ...
    /*int sdepth = CC_MAT_DEPTH(_srcType), ddepth = CC_MAT_DEPTH(_dstType);
    int ktype = _kernel CC_MAT_DEPTH() == CC_32S ? CC_KERNEL_INTEGER : getKernelType(_kernel, _anchor);
    if( sdepth == CC_8U && (ddepth == CC_8U || ddepth == CC_16S) &&
        _kernel->rows*_kernel->cols <= (1 << 10) )
    {
        bits = (ktype & CC_KERNEL_INTEGER) ? 0 : 11;
        _kernel.convertTo(kernel, CC_32S, 1 << bits);
    }*/
...
struct ReplacementFilter : public hal_Filter2D
{
    cvhalFilter2D* ctx;
    bool isInitialized;
    ...
    void apply(uchar* src_data, size_t src_step, uchar* dst_data, size_t dst_step, int width, int height, int full_width, int full_height, int offset_x, int offset_y)
    {
        if (isInitialized)
        {
            ...
            if (res != CC_HAL_ERROR_OK)
                CC_Error(CC_StsNotImplemented, "HAL Filter returned an error");
        }
    }
    ~ReplacementFilter()
    {
        if (isInitialized)
        {
            ...
            if (res != CC_HAL_ERROR_OK)
                CC_Error(CC_StsNotImplemented, "HAL Filter Free returned an error");
        }
    }
};

#ifdef HAVE_IPP
typedef IppStatus(C_STDCALL* IppiFilterBorder)(
...
template <>
struct IppFilterTrait<CC_16S>
{
    enum { kernel_type_id = CC_16SC1 };
    typedef Ipp16s kernel_type;
    typedef IppStatus(C_STDCALL* copy_fun_type)(const kernel_type* pSrc, int srcStep, kernel_type* pDst, int dstStep, IppiSize roiSize);
...
template <>
struct IppFilterTrait<CC_32F>
{
    enum { kernel_type_id = CC_32FC1 };
...
template <int kdepth>
struct IppFilter : public hal_Filter2D
{
              ...
              int borderType, double delta, int anchor_x, int anchor_y, bool isSubmatrix, bool isInplace)
    {
        ...
        bool runIpp = true
                      && (borderTypeNI == CC_BORDER_CONSTANT || borderTypeNI == CC_BORDER_REPLICATE)
                      && (sdepth == ddepth)
                      && (getIppFunc(stype))
                      && ((int)ippBorderType > 0)
        ...
        if (status >= 0) {
            ...
            img_t kerFlip(CSize(kernelSize.width, kernelSize.height), trait::kernel_type_id, kernelBuffer, (int)good_kernel_step);
            img_t kernel(CSize(kernel_width, kernel_height), trait::kernel_type_id, kernel_data, kernel_step);
    ...
    void apply(uchar* src_data, size_t src_step, uchar* dst_data, size_t dst_step, int width, int height, int, int, int, int)
    {
        ...
        if (dst_data == src_data)
            CC_Error(CC_StsBadArg, "Inplace IPP Filter2D is not supported");
        IppiFilterBorder ippiFilterBorder = getIppFunc(src_type);
        IppiSize dstRoiSize = { width, height };
        kernel_type borderValue[4] = { 0, 0, 0, 0 };
        IppStatus status = CC_INSTRUMENT_FUN_IPP(ippiFilterBorder, src_data, (int)src_step, dst_data, (int)dst_step, dstRoiSize, ippBorderType, borderValue, spec, buffer);
        if (status >= 0) {
            CC_IMPL_ADD(CC_IMPL_IPP);
        }
    }
...
struct DftFilter : public hal_Filter2D
{
    int src_type;
    int dst_type;
    double delta;
    ...
    static bool isAppropriate(int stype, int dtype, int kernel_width, int kernel_height)
    {
        ...
        int sdepth = CC_MAT_DEPTH(stype);
        int ddepth = CC_MAT_DEPTH(dtype);
        int dft_filter_size = ((sdepth == CC_8U && (ddepth == CC_8U || ddepth == CC_16S)) || (sdepth == CC_32F && ddepth == CC_32F)) && checkHardwareSupport(CC_CPU_SSE3) ? 130 : 50;
#else
        CC_UNUSED(stype);
        CC_UNUSED(dtype);
              ...
              int borderType_, double delta_, int anchor_x, int anchor_y, bool, bool)
    {
        ...
        kernel = img_t(CSize(kernel_width, kernel_height), kernel_type, kernel_data, kernel_step);
        src_type = stype;
        dst_type = dtype;
        delta = delta_;
    ...
    void apply(uchar* src_data, size_t src_step, uchar* dst_data, size_t dst_step, int width, int height, int, int, int, int)
    {
        ...
        img_t temp;
        int src_channels = CC_MAT_CN(src_type);
        int dst_channels = CC_MAT_CN(dst_type);
        int ddepth = CC_MAT_DEPTH(dst_type);
        ...
        if (src_channels != 1 && delta != 0) {
            ...
            // as floating-point math.  So wee need an intermediate img_t
            // with a float datatype.  If the dest is already floats,
            // we just use that.
            int corrDepth = ddepth;
            if ((ddepth == CC_32F || ddepth == CC_64F) && src_data != dst_data) {
                temp = img_t(CSize(width, height), dst_type, dst_data, dst_step);
            } else {
                corrDepth = ddepth == CC_64F ? CC_64F : CC_32F;
                temp cvSetMat(CSize(width, height), CC_MAKETYPE(corrDepth, dst_channels));
            }
            crossCorr(src, kernel, temp, cvGetSize(src),
                      CC_MAKETYPE(corrDepth, src_channels),
                      anchor, 0, borderType);
            add(temp, delta, temp);
            if (temp.data != dst_data) {
                temp.convertTo(dst, dst->tid);
            }
        } else {
            if (src_data != dst_data)
                temp = img_t(CSize(width, height), dst_type, dst_data, dst_step);
            else
                temp cvSetMat(CSize(width, height), dst_type);
            crossCorr(src, kernel, temp, cvGetSize(src),
                      CC_MAKETYPE(ddepth, src_channels),
...
struct OcvFilter : public hal_Filter2D
{
    Ptr<FilterEngine> f;
    int src_type;
    int dst_type;
    ...
    {
        isIsolated = (borderType & CC_BORDER_ISOLATED) != 0;
        src_type = stype;
        dst_type = dtype;
        int borderTypeValue = borderType & ~CC_BORDER_ISOLATED;
        img_t kernel = img_t(CSize(kernel_width, kernel_height), kernel_type, kernel_data, kernel_step);
    ...
    void apply(uchar* src_data, size_t src_step, uchar* dst_data, size_t dst_step, int width, int height, int full_width, int full_height, int offset_x, int offset_y)
    {
        img_t src(CSize(width, height), src_type, src_data, src_step);
        img_t dst(CSize(width, height), dst_type, dst_data, dst_step);
        f->apply(src, dst, CSize(full_width, full_height), Point(offset_x, offset_y));
    }
};


struct ReplacementSepFilter : public hal_SepFilter2D
{
    cvhalFilter2D *ctx;
    bool isInitialized;
             ...
             int offset_x, int offset_y)
    {
        if (isInitialized)
        {
            ...
            if (res != CC_HAL_ERROR_OK)
                CC_Error(CC_StsNotImplemented, "Failed to run HAL sepFilter implementation");
        }
    }
...
struct OcvSepFilter : public hal_SepFilter2D
{
    Ptr<FilterEngine> f;
    int src_type;
    int dst_type;
              ...
              int anchor_x, int anchor_y, double delta, int borderType)
    {
        src_type = stype;
        dst_type = dtype;
        img_t kernelX(CSize(kernelx_len, 1), ktype, kernelx_data);
        img_t kernelY(CSize(kernely_len, 1), ktype, kernely_data);

        f = createSeparableLinearFilter( stype, dtype, kernelX, kernelY,
                                         Point(anchor_x, anchor_y),
                                         delta, borderType & ~CC_BORDER_ISOLATED );
        return true;
    }
    void apply(uchar* src_data, size_t src_step, uchar* dst_data, size_t dst_step,
             int width, int height, int full_width, int full_height,
...
Ptr<hal_Filter2D> Filter2D::create(uchar* kernel_data, size_t kernel_step, int kernel_type,
                                   ...
                                   int borderType, double delta, int anchor_x, int anchor_y, bool isSubmatrix, bool isInplace)
{
    ...
    CC_IPP_CHECK()
    {
        if (kernel_type == CC_32FC1) {
            IppFilter<CC_32F>* impl = new IppFilter<CC_32F>();
            if (impl->init(kernel_data, kernel_step, kernel_type, kernel_width, kernel_height,
                           max_width, max_height, stype, dtype,
                           borderType, delta, anchor_x, anchor_y, isSubmatrix, isInplace))
            {
        ...
        if (kernel_type == CC_16SC1) {
            IppFilter<CC_16S>* impl = new IppFilter<CC_16S>();
            if (impl->init(kernel_data, kernel_step, kernel_type, kernel_width, kernel_height,
                           max_width, max_height, stype, dtype,
                           borderType, delta, anchor_x, anchor_y, isSubmatrix, isInplace))
            {
...
{
    {
        ...
        if (impl->init(stype, dtype, ktype,
                       ...
                       anchor_x, anchor_y, delta, borderType))
        {
            return Ptr<hal_SepFilter2D>(impl);
        }
        delete impl;
    }
    {
...
void filter2D( const img_t* _src, img_t* dst, int ddepth,
                   const img_t* _kernel, Point anchor0,
                   double delta, int borderType )
{
    CC_INSTRUMENT_REGION()
    ...
    img_t *src = _src, kernel = _kernel;

    if( ddepth < 0 )
        ddepth = src CC_MAT_DEPTH();

    _dst cvSetMat( cvGetSize(src), CC_MAKETYPE(ddepth, src CC_MAT_CN()) );
    img_t *dst = _dst;
    Point anchor = normalizeAnchor(anchor0, kernel.size());

    Point ofs;
    CSize wsz(src->cols, src->rows);
    if( (borderType & CC_BORDER_ISOLATED) == 0 )
        src.locateROI( wsz, ofs );

    Ptr<hal_Filter2D> c = hal_Filter2D::create(kernel.data, kernel->step, kernel->tid, kernel->cols, kernel->rows,
                                                 dst->cols, dst->rows, src->tid, dst->tid,
...
void sepFilter2D( const img_t* _src, img_t* dst, int ddepth,
                      const img_t* _kernelX, const img_t* _kernelY, Point anchor,
                      double delta, int borderType )
{
    CC_INSTRUMENT_REGION()

    CC_OCL_RUN(_dst.isUMat() && _src.dims() <= 2 && (size_t)_src->rows() > _kernelY CC_MAT_TOTAL() && (size_t)_src->cols() > _kernelX CC_MAT_TOTAL(),
               ocl_sepFilter2D(_src, _dst, ddepth, _kernelX, _kernelY, anchor, delta, borderType))

    img_t *src = _src, kernelX = _kernelX, kernelY = _kernelY;
    ...
    CC_Assert( kernelX->tid == kernelY->tid &&
               (kernelX->cols == 1 || kernelX->rows == 1) &&
               (kernelY->cols == 1 || kernelY->rows == 1) );

    img_t contKernelX = kernelX CC_IS_CONT_MAT() ? kernelX : kernelX.clone();
    img_t contKernelY = kernelY CC_IS_CONT_MAT() ? kernelY : kernelY.clone();
    Ptr<hal_SepFilter2D> c = hal_SepFilter2D::create(src->tid, dst->tid, kernelX->tid,
                                                       contKernelX.data, kernelX->cols + kernelX->rows - 1,
                                                       contKernelY.data, kernelY->cols + kernelY->rows - 1,
                                                       anchor.x, anchor.y, delta, borderType & ~CC_BORDER_ISOLATED);
    c->apply(src.data, src->step, dst.data, dst->step, dst->cols, dst->rows, wsz.width, wsz.height, ofs.x, ofs.y);
}


CC_IMPL void
cvFilter2D( const img_t* src, img_t* dst, const img_t* _kernel, CPoint anchor )
{
    img_t src = cvarrToMat(srcarr), dst = cvarrToMat(dstarr);
    img_t kernel = cvarrToMat(_kernel);

    CC_Assert( cvGetSize(src) == dst.size() && src CC_MAT_CN() == dst CC_MAT_CN() );

    filter2D( src, dst, dst CC_MAT_DEPTH(), kernel, anchor, 0, CC_BORDER_REPLICATE );
}
