...
BaseFilter::BaseFilter() { ksize = iSize(-1,-1); anchor = iPoint(-1,-1); }
BaseFilter::~BaseFilter() {}
void BaseFilter::reset() {}

FilterEngine::FilterEngine()
    : srcType(-1), dstType(-1), bufType(-1), maxWidth(0), dx1(0), dx2(0),
...
FilterEngine::FilterEngine( BaseFilter* _filter2D,
...
void FilterEngine::init( BaseFilter* _filter2D,
                         BaseRowFilter* _rowFilter,
                         BaseColumnFilter* _columnFilter,
                         int _srcType, int _dstType, int _bufType,
                         int _rowBorderType, int _columnBorderType,
                         const Scalar& _borderValue )
{
    ...
    CC_Assert( columnBorderType != BORDER_WRAP );

    if( isSeparable() )
    {
        CC_Assert( rowFilter && columnFilter );
        ksize = iSize(rowFilter->ksize, columnFilter->ksize);
        anchor = iPoint(rowFilter->anchor, columnFilter->anchor);
    }
    else
    {
...
int FilterEngine::start(const ISize &_wholeSize, const ISize &sz, const IPoint &ofs)
{
    int i, j;
    ...
    int _maxBufRows = MAX(ksize.height + 3,
                               MAX(anchor.y,
...
int FilterEngine::start(const img_t*& src, const ISize &wsz, const IPoint &ofs)
{
...
void FilterEngine::apply(const img_t*& src, img_t*& dst, const ISize & wsz, const IPoint & ofs)
{
    CC_INSTRUMENT_REGION()

    CC_Assert( src->type() == srcType && dst->type() == dstType );
...
int getKernelType(const img_t* filter_kernel, IPoint anchor)
{
    img_t* _kernel = filter_kernel.getMat();
    CC_Assert( _kernel.channels() == 1 );
...
struct SymmColumnVec_32s8u
{
    ...
    SymmColumnVec_32s8u(const img_t*& _kernel, int _symmetryType, int _bits, double _delta)
    {
        ...
        CC_Assert( (symmetryType & (KERNEL_SYMMETRICAL | KERNEL_ASYMMETRICAL)) != 0 );
    }
...
struct RowVec_32f
{
    RowVec_32f()
    {
        ...
        haveAVX2 = CC_CPU_HAS_SUPPORT_AVX2;
...
template<typename ST, typename DT, class VecOp> struct RowFilter : public BaseRowFilter
{
    RowFilter( const img_t*& _kernel, int _anchor, const VecOp& _vecOp=VecOp() )
    {
        ...
        CC_Assert( kernel.type() == DataType<DT>::type &&
    ...
    public RowFilter<ST, DT, VecOp>
{
    ...
    {
        symmetryType = _symmetryType;
        CC_Assert( (symmetryType & (KERNEL_SYMMETRICAL | KERNEL_ASYMMETRICAL)) != 0 && this->ksize <= 5 );
    }
...
template<class CastOp, class VecOp> struct ColumnFilter : public BaseColumnFilter
{
        ...
        const VecOp& _vecOp=VecOp() )
    {
        ...
        castOp0 = _castOp;
        vecOp = _vecOp;
        CC_Assert( kernel.type() == DataType<ST>::type &&
                   (kernel->rows == 1 || kernel->cols == 1));
    }
...
FilterEngine* createSeparableLinearFilter(
    ...
    const img_t* _rowKernel, const img_t* _columnKernel,
    IPoint _anchor, double _delta,
    int _rowBorderType, int _columnBorderType,
    const Scalar& _borderValue )
{
  //img_t* _rowKernel = __rowKernel->getMat();
  //img_t* _columnKernel = __columnKernel->getMat();
    _srcType = CC_MAT_TYPE(_srcType);
    _dstType = CC_MAT_TYPE(_dstType);
    TypeId sdepth = CC_MAT_DEPTH(_srcType), ddepth = CC_MAT_DEPTH(_dstType);
    ...
    int rtype = getKernelType(_rowKernel,
        _rowKernel->rows == 1 ? iPoint(_anchor.x, 0) : iPoint(0, _anchor.x));
    int ctype = getKernelType(_columnKernel,
        _columnKernel->rows == 1 ? iPoint(_anchor.y, 0) : iPoint(0, _anchor.y));
    img_t tmp[2];
    img_t *rowKernel = tmp+0;
    img_t *columnKernel = tmp+1;

    TypeId bdepth = MAX(CC_32F,MAX(sdepth, ddepth));
    int bits = 0;
    ...
    else
    {
        if( _rowKernel->type() != bdepth )
            ...
            rowKernel = (img_t*)_rowKernel;
        if( _columnKernel->type() != bdepth )
            _columnKernel->convertTo( columnKernel, bdepth );
        else
            columnKernel = (img_t*)_columnKernel;
    }

    int _bufType = CC_MAKETYPE(bdepth, cn);
    BaseRowFilter* _rowFilter = getLinearRowFilter(
        _srcType, _bufType, rowKernel, _anchor.x, rtype);
    BaseColumnFilter* _columnFilter = getLinearColumnFilter(
        _bufType, _dstType, columnKernel, _anchor.y, ctype, _delta, bits );

    return ( new FilterEngine(NULL , _rowFilter, _columnFilter,
...
template<typename ST, class CastOp, class VecOp> struct Filter2D : public BaseFilter
{
    ...
    Filter2D( const img_t*& _kernel, IPoint _anchor,
        double _delta, const CastOp& _castOp=CastOp(),
        const VecOp& _vecOp=VecOp() )
    {
        anchor = _anchor;
        ...
        castOp0 = _castOp;
        vecOp = _vecOp;
        CC_Assert( _kernel.type() == DataType<KT>::type );
        preprocess2DKernel( _kernel, coords, coeffs );
...
static bool ocl_filter2D( const img_t* _src, img_t* _dst, int ddepth,
                   const img_t* _kernel, IPoint anchor,
                   double delta, int borderType )
{
    int type = _src->type(), sdepth = CC_MAT_DEPTH(type), cn = CC_MAT_CN(type);
    ...
    Uimg_t* kernalDataUimg_t*;
    if (device.isIntel() && (device.type() & ocl::Device::TYPE_GPU) &&
        ((ksize.width < 5 && ksize.height < 5) ||
        (ksize.width == 5 && ksize.height == 5 && cn == 1)))
    {
        kernelimg_t* = kernelimg_t*.reshape(0, 1);
        String kerStr = ocl::kernelToStr(kernelimg_t*, CC_32F);
        ...
        if (!k.create("filter2DSmall", ocl::imgproc::filter2DSmall_oclsrc, build_options))
            return false;
    }
    else
    {
        localsize = localsize_general;
        std::vector<float> kernelimg_t*DataFloat;
        int kernel_size_y2_aligned = _prepareKernelFilter2D<float>(kernelimg_t*DataFloat, kernelimg_t*);
        String kerStr = ocl::kernelToStr(kernelimg_t*DataFloat, CC_32F);
        ...
        for ( ; ; )
        {
            ...
            if (!k.create("filter2D", ocl::imgproc::filter2D_oclsrc, opts))
                return false;
...
static bool ocl_sepRowFilter2D(const Uimg_t* & src, Uimg_t* & buf, const img_t* & kernelX, int anchor,
                               int borderType, int ddepth, bool fast8uc1, bool int_arithm)
{
    int type = src->type(), cn = CC_MAT_CN(type), sdepth = CC_MAT_DEPTH(type);
    bool doubleSupport = ocl::Device::getDefault().doubleFPConfig() > 0;
    ISize bufSize = buf.size();
    int buf_type = buf.type(), bdepth = CC_MAT_DEPTH(buf_type);
    ...
    ISize srcWholeSize; IPoint srcOffset;
...
static bool ocl_sepColFilter2D(const Uimg_t* & buf, Uimg_t* & dst, const img_t* & kernelY, double delta, int anchor, bool int_arithm)
{
    bool doubleSupport = ocl::Device::getDefault().doubleFPConfig() > 0;
    if (dst->depth() == CC_64F && !doubleSupport)
        return false;
    ...
    int dtype = dst->type(), cn = CC_MAT_CN(dtype), ddepth = CC_MAT_DEPTH(dtype);
    ISize sz = dst->size();
    int buf_type = buf.type(), bdepth = CC_MAT_DEPTH(buf_type);
...
static bool ocl_sepFilter2D_SinglePass(const img_t* _src, img_t* _dst,
                                       img_t* row_kernel, img_t* col_kernel,
                                       double delta, int borderType, int ddepth, int bdepth, bool int_arithm)
{
    ISize size = _src->size(), wholeSize;
    IPoint origin;
    int stype = _src->type(), sdepth = CC_MAT_DEPTH(stype), cn = CC_MAT_CN(stype),
...
bool ocl_sepFilter2D( const img_t* _src, img_t* _dst, int ddepth,
                      const img_t* _kernelX, const img_t* _kernelY, IPoint anchor,
                      double delta, int borderType )
{
    const ocl::Device & d = ocl::Device::getDefault();
    ISize imgSize = _src->size();

    int type = _src->type(), sdepth = CC_MAT_DEPTH(type), cn = CC_MAT_CN(type);
    if (cn > 4)
        return false;

    img_t* kernelX = _kernelX->getMat().reshape(1, 1);
    if (kernelX->cols % 2 != 1)
        return false;
    img_t* kernelY = _kernelY->getMat().reshape(1, 1);
    if (kernelY->cols % 2 != 1)
        return false;
    ...
    int rtype = getKernelType(kernelX,
        kernelX->rows == 1 ? iPoint(anchor.x, 0) : iPoint(0, anchor.x));
    int ctype = getKernelType(kernelY,
        kernelY->rows == 1 ? iPoint(anchor.y, 0) : iPoint(0, anchor.y));
    ...
    CC_OCL_RUN_(kernelY->cols <= 21 && kernelX->cols <= 21 &&
                ...
                anchor == iPoint(kernelX->cols >> 1, kernelY->cols >> 1) &&
    ...
    ISize srcSize = src->size();
    ISize bufSize(srcSize.width, srcSize.height + kernelY->cols - 1);
    Uimg_t* buf(bufSize, CC_MAKETYPE(bdepth, cn));
    if (!ocl_sepRowFilter2D(src, buf, kernelX, anchor.x, borderType, ddepth, fast8uc1, int_arithm))
        return false;

    _dst->create(srcSize, CC_MAKETYPE(ddepth, cn));
    Uimg_t* dst = _dst->getUimg_t*();
...
Ptr<BaseFilter> getLinearFilter(int srcType, int dstType,
                                const img_t* filter_kernel, IPoint anchor,
                                double delta, int bits)
{
    img_t* _kernel = filter_kernel.getMat();
    int sdepth = CC_MAT_DEPTH(srcType), ddepth = CC_MAT_DEPTH(dstType);
    int cn = CC_MAT_CN(srcType), kdepth = _kernel.depth();
    ...
    if( _kernel.type() == kdepth )
        ...
        _kernel.convertTo(kernel, kdepth, _kernel.type() == CC_32S ? 1./(1 << bits) : 1.);
                                              ...
                                              const Scalar& _borderValue )
{
    ...
    int ktype = _kernel.depth() == CC_32S ? KERNEL_INTEGER : getKernelType(_kernel, _anchor);
                                ...
                                double delta, int borderType, bool isSubmatrix)
{
    ...
    bool success = (res == CC_HAL_ERROR_OK);
              ...
              bool isSubmatrix)
{
    ...
    IppDataType     type        = ippiGetDataType(CC_MAT_DEPTH(stype));
    int             channels    = CC_MAT_CN(stype);

    CC_UNUSED(isSubmatrix);
    ...
    if(kernel_type == CC_32FC1 && (type == ipp16s || type == ipp16u) && (kernel_width > 7 || kernel_height > 7))
        return false;
    ...
    if(kernel_type != CC_16SC1 && kernel_type != CC_32FC1)
        return false;
    ...
    try
    {
        ...
        ::ipp::IwiImage         iwKernel(ippiSize(kernel_width, kernel_height), ippiGetDataType(CC_MAT_DEPTH(kernel_type)), CC_MAT_CN(kernel_type), 0, (void*)kernel_data, kernel_step);
        ...
        CC_INSTRUMENT_FUN_IPP(::ipp::iwiFilter, iwSrc, iwDst, iwKernel, ::ipp::IwiFilterParams(1, 0, ippAlgHintNone, ippRndFinancial), iwBorderType);
    }
    ...
    return true;
#else
    CC_UNUSED(stype); CC_UNUSED(dtype); CC_UNUSED(kernel_type); CC_UNUSED(src_data); CC_UNUSED(src_step);
    CC_UNUSED(dst_data); CC_UNUSED(dst_step); CC_UNUSED(width); CC_UNUSED(height); CC_UNUSED(full_width);
    CC_UNUSED(full_height); CC_UNUSED(offset_x); CC_UNUSED(offset_y); CC_UNUSED(kernel_data); CC_UNUSED(kernel_step);
    CC_UNUSED(kernel_width); CC_UNUSED(kernel_height); CC_UNUSED(anchor_x); CC_UNUSED(anchor_y); CC_UNUSED(delta);
    CC_UNUSED(borderType); CC_UNUSED(isSubmatrix);
    return false;
#endif
}
#endif
                        ...
                        double delta, int borderType)
{
    ...
    IPoint anchor = iPoint(anchor_x, anchor_y);
    img_t* kernel = img_t*(ISize(kernel_width, kernel_height), kernel_type, kernel_data, kernel_step);

    img_t* src(ISize(width, height), stype, src_data, src_step);
    img_t* dst(ISize(width, height), dtype, dst_data, dst_step);
    img_t* temp;
    int src_channels = CC_MAT_CN(stype);
    int dst_channels = CC_MAT_CN(dtype);
    int ddepth = CC_MAT_DEPTH(dtype);
    ...
    if (src_channels != 1 && delta != 0) {
        ...
        if ((ddepth == CC_32F || ddepth == CC_64F) && src_data != dst_data) {
            temp = img_t*(ISize(width, height), dtype, dst_data, dst_step);
        } else {
            corrDepth = ddepth == CC_64F ? CC_64F : CC_32F;
            temp.create(ISize(width, height), CC_MAKETYPE(corrDepth, dst_channels));
        }
        ...
        if (src_data != dst_data)
            ...
            temp.create(ISize(width, height), dtype);
        crossCorr(src, kernel, temp, src->size(),
                        ...
                        double delta, int borderType)
{
    ...
    img_t* kernel = img_t*(ISize(kernel_width, kernel_height), kernel_type, kernel_data, kernel_step);
    Ptr<FilterEngine> f = createLinearFilter(stype, dtype, kernel, iPoint(anchor_x, anchor_y), delta,
                                             borderTypeValue);
    img_t* src(ISize(width, height), stype, src_data, src_step);
    img_t* dst(ISize(width, height), dtype, dst_data, dst_step);
    f->apply(src, dst, iSize(full_width, full_height), iPoint(offset_x, offset_y));
}
...
{
    img_t tmp[4];
    img_t* kernelX = imcreate(tmp + 0, iSIZE(kernelx_len, 1), ktype, kernelx_data);
    img_t* kernelY = imcreate(tmp + 1, iSize(kernely_len, 1), ktype, kernely_data);
    FilterEngine* f = createSeparableLinearFilter(stype, dtype, kernelX, kernelY,
                                                      iPoint(anchor_x, anchor_y),
                                                      delta, borderType & ~BORDER_ISOLATED);

    img_t* src = imcreate(tmp + 2, iSize(width, height), stype, src_data, src_step);
    img_t* dst = imcreate(tmp + 3, iSize(width, height), dtype, dst_data, dst_step);
    f->apply(src, dst, iSize(full_width, full_height), iPoint(offset_x, offset_y));
};
...
CC_DEPRECATED Ptr<hal::Filter2D> Filter2D::create(uchar * , size_t , int ,
                                 int , int ,
                                 int , int ,
                                 int , int ,
                                 int , double ,
...
CC_DEPRECATED Ptr<hal::SepFilter2D> SepFilter2D::create(int , int , int ,
                                    uchar * , int ,
                                    uchar * , int ,
                                    int , int ,
              ...
              bool isSubmatrix)
{
    ...
    CC_IPP_RUN_FAST(ippFilter2D(stype, dtype, kernel_type,
                              src_data, src_step,
                              dst_data, dst_step,
                              width, height,
                              full_width, full_height,
...
void filter2D( const img_t* _src, img_t* _dst, int ddepth,
                   const img_t* _kernel, IPoint anchor0,
                   double delta, int borderType )
{
    CC_INSTRUMENT_REGION()

    CC_OCL_RUN(_dst->isUimg_t*() && _src->dims() <= 2,
               ocl_filter2D(_src, _dst, ddepth, _kernel, anchor0, delta, borderType))

    img_t* src = _src->getMat(), kernel = _kernel.getMat();

    if( ddepth < 0 )
        ddepth = src->depth();

    _dst->create( src->size(), CC_MAKETYPE(ddepth, src->channels()) );
    img_t* dst = _dst->getMat();
    IPoint anchor = normalizeAnchor(anchor0, kernel.size());

    IPoint ofs;
    ISize wsz(src->cols, src->rows);
...
void sepFilter2D( const img_t* src, img_t* _dst, int ddepth,
                      const img_t* kernelX, const img_t* kernelY, IPoint anchor,
                      double delta, int borderType )
{
    CC_INSTRUMENT_REGION()

    CC_OCL_RUN(_dst->isUimg_t*() && _src->dims() <= 2 && (size_t)_src->rows() > _kernelY->total() && (size_t)_src->cols() > _kernelX->total(),
    ...
    IPoint ofs;
    ISize wsz = iSIZE(src->cols, src->rows);
    if( (borderType & BORDER_ISOLATED) == 0 )
        src->locateROI( wsz, ofs );

    CC_Assert( kernelX->type() == kernelY->type() &&
               (kernelX->cols == 1 || kernelX->rows == 1) &&
               (kernelY->cols == 1 || kernelY->rows == 1) );
    img_t tmp[2];
    const img_t* contKernelX = kernelX->isContinuous() ? kernelX : kernelX->clone(tmp + 0);
    const img_t* contKernelY = kernelY->isContinuous() ? kernelY : kernelY->clone(tmp + 1);

    sepFilter2D(src->type(), dst->type(), kernelX->type(),
                     src->data, src->step, dst->data, dst->step,
                     dst->cols, dst->rows, wsz.width, wsz.height, ofs.x, ofs.y,
...
CC_IMPL void
cvFilter2D( const CvArr* srcarr, CvArr* dstarr, const Cvimg_t** _kernel, CvPoint anchor )
{
    img_t* src = cvarrToimg_t*(srcarr), dst = cvarrToimg_t*(dstarr);
    img_t* kernel = cvarrToimg_t*(_kernel);

    CC_Assert( src->size() == dst->size() && src->channels() == dst->channels() );

    filter2D( src, dst, dst->depth(), kernel, anchor, 0, BORDER_REPLICATE );
}
