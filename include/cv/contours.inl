...
static const CPoint icvCodeDeltas2[8] =
...
cvStartReadChainPoints(CChain * chain, CvChainPtReader * reader)
{
    ...
    for(i = 0; i < 8; i++)
    {
        reader->deltas[i][0] = (char) icvCodeDeltas2[i].x;
        reader->deltas[i][1] = (char) icvCodeDeltas2[i].y;
    }
...
cvReadChainPoint(CvChainPtReader * reader)
{
    ...
    if(ptr)
    {
        ...
        assert((code & ~7) == 0);
        reader->pt.x = pt.x + icvCodeDeltas2[code].x;
        reader->pt.y = pt.y + icvCodeDeltas2[code].y;
    }
                     ...
                     int  method, CPoint offset)
{
    ...
    scanner = (CvContourScanner)cAlloc(sizeof(*scanner));
    if(!scanner)
    ...
    else
    {
        ...
        for(;;)
        {
            ...
            else
            {
                ...
                pt.x += icvCodeDeltas2[s].x;
                pt.y += icvCodeDeltas2[s].y;
    ...
    for(; y < height; y++, img += step)
    {
        for(; x < width; x++)
        {
            ...
            if(p != prev)
            {
                ...
                {
                    result = icvApproximateChainTC89((CChain *) seq,
                ...
                int  method, CPoint offset)
{
    ...
    if(method == CC_LINK_RUNS)
    {
        ...
        CC_CALL(count = icvFindContoursInInterval(img, storage,
                                    firstContour, cntHeaderSize));
    }
    else
    {
        CC_CALL(scanner = cvStartFindContours(img, storage,
