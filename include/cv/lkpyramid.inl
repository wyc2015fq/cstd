...
static void
intersect(CPoint2D32f pt, CSize win_size, CSize img_size,
           CPoint* min_pt, CPoint* max_pt)
{
    CPoint ipt;
                           ...
                           double **scale, uchar ** buffer)
{
    ...
    *size = 0;
    ...
    if((flags & CC_LKFLOW_PYR_A_READY) != 0 && !pyrA ||
        (flags & CC_LKFLOW_PYR_B_READY) != 0 && !pyrB)
...
#define ALIGN 8
    ...
    *buffer = (uchar *)cAlloc(bufferBytes);
    if(!buffer[0])
    ...
    *size = (CSize *)(*scale + level1);
    ...
    if(level > 0)
    {
        ...
        for(i = 1; i <= level; i++)
        {
            ...
            if(!(flags & CC_LKFLOW_PYR_A_READY))
            {
                prev_level = cvMat(size[0][i-1].height, size[0][i-1].width, CC_8UC1);
                next_level = cvMat(size[0][i].height, size[0][i].width, CC_8UC1);
            ...
            if(!(flags & CC_LKFLOW_PYR_B_READY))
            {
                prev_level = cvMat(size[0][i-1].height, size[0][i-1].width, CC_8UC1);
                next_level = cvMat(size[0][i].height, size[0][i].width, CC_8UC1);
...
icvCalcIxIy_32f(const float* src, int src_step, float* dstX, float* dstY, int dst_step,
                 CSize src_size, const float* smooth_k, float* buffer0)
{
...
icvCalcOpticalFlowPyrLK_8uC1R(const uchar* imgA, const uchar* imgB,
                               int imgStep, CSize imgSize,
                               uchar* pyrA, uchar* pyrB,
                               const CPoint2D32f* featuresA,
                               CPoint2D32f* featuresB,
                               int count, CSize winSize,
                               int level, char* status,
                               float *error, CTermCriteria criteria, int flags)
{
#define MAX_SIZE  100
#define MAX_LEVEL 10
#define MAX_ITERS 100
    ...
    int threadCount = cGetNumThreads();
    float* _patchI[CC_MAX_THREADS];
    float* _patchJ[CC_MAX_THREADS];
    float* _Ix[CC_MAX_THREADS];
    float* _Iy[CC_MAX_THREADS];

    int i, l;

    CSize patchSize = cSize(winSize.width * 2 + 1, winSize.height * 2 + 1);
    ...
    if(!status)
    {
        _status = (char*)cAlloc(count*sizeof(_status[0]));
        if(!_status)
        {
            result = CC_OUTOFMEM_ERR;
            goto func_exit;
        }
        status = _status;
    }
                                               ...
                                               winSize.width*2+1, cvAlgHintAccurate) >= 0)
    {
        ...
        if(!error)
        {
            _error = (float*)cAlloc(count*sizeof(_error[0]));
            if(!_error)
            {
                result = CC_OUTOFMEM_ERR;
                goto func_exit;
    ...
    if(!(flags & CC_LKFLOW_INITIAL_GUESSES))
    ...
    for(l = level; l >= 0; l--)
    {
        ...
        {
            CPoint2D32f v;
            CPoint minI, maxI, minJ, maxJ;
            CSize isz, jsz;
            int pt_status;
            CPoint2D32f u;
            CPoint prev_minJ = { -1, -1 }, prev_maxJ = { -1, -1 };
            double Gxx = 0, Gxy = 0, Gyy = 0, D = 0;
            float prev_mx = 0, prev_my = 0;
            int j, x, y;
            int threadIdx = cGetThreadNum();
            ...
            for(j = 0; j < criteria.max_iter; j++)
            {
                ...
                jsz = cSize(maxJ.x - minJ.x, maxJ.y - minJ.y);
...
static  CStatus  icvCalcAffineFlowPyrLK_8uC1R(uchar * imgA, uchar * imgB,
                                                int imgStep, CSize imgSize,
                                                uchar * pyrA, uchar * pyrB,
                                                CPoint2D32f * featuresA,
                                                CPoint2D32f * featuresB,
                                                float *matrices, int count,
                                                CSize winSize, int level,
...
#define MAX_ITERS 100
    ...
    CSize srcPatchSize = cSize(patchSize.width + 2, patchSize.height + 2);
    ...
    for(i = 0; i < count; i++)
    {
        ...
        for(l = level; l >= 0; l--)
        {
            ...
            img_t mat;
            cvInitMatHeader(&mat, 6, 6, CC_64FC1, G);

            if(cvInvert(&mat, &mat, CC_SVD) < 1e-3)
            {
...
static int icvMinimalPyramidSize(CSize img_size)
{
...
cvCalcOpticalFlowPyrLK(const void* arrA, const void* arrB,
                        ...
                        const CPoint2D32f * featuresA,
                        CPoint2D32f * featuresB,
                        int count, CSize winSize, int level,
                        char *status, float *error,
                        CTermCriteria criteria, int flags)
{
    CC_FUNCNAME("cvCalcOpticalFlowPyrLK");

    __BEGIN__;

    img_t stubA, *imgA = arrA;
    img_t stubB, *imgB = arrB;
    img_t pstubA, *pyrA = pyrarrA;
    img_t pstubB, *pyrB = pyrarrB;
    CSize img_size;

    CC_CALL(imgA = cvGetMat(imgA, &stubA));
    CC_CALL(imgB = cvGetMat(imgB, &stubB));

    if(CC_MAT_TYPE(imgA->tid) != CC_8UC1)
        CC_ERROR(CC_StsUnsupportedFormat, "");

    if(!CC_ARE_TYPES_EQ(imgA, imgB))
        CC_ERROR(CC_StsUnmatchedFormats, "");

    if(!CC_ARE_SIZES_EQ(imgA, imgB))
        CC_ERROR(CC_StsUnmatchedSizes, "");

    if(imgA->step != imgB->step)
        CC_ERROR(CC_StsUnmatchedSizes, "imgA and imgB must have equal steps");

    img_size = cvGetMatSize(imgA);

    if(pyrA)
    {
        CC_CALL(pyrA = cvGetMat(pyrA, &pstubA));

        if(pyrA->step*pyrA->height < icvMinimalPyramidSize(img_size))
            CC_ERROR(CC_StsBadArg, "pyramid A has insufficient size");
    }
    else
    {
        pyrA = &pstubA;
    ...
    if(pyrB)
    {
        CC_CALL(pyrB = cvGetMat(pyrB, &pstubB));

        if(pyrB->step*pyrB->height < icvMinimalPyramidSize(img_size))
            CC_ERROR(CC_StsBadArg, "pyramid B has insufficient size");
    }
    else
    {
        pyrB = &pstubB;
    ...
    IPPI_CALL(icvCalcOpticalFlowPyrLK_8uC1R(imgA->tt.data, imgB->tt.data, imgA->step,
                                              img_size, pyrA->tt.data, pyrB->tt.data,
                                              featuresA, featuresB,
...
cvCalcAffineFlowPyrLK(const void* arrA, const void* arrB,
                       ...
                       CTermCriteria criteria, int flags)
{
    CC_FUNCNAME("cvCalcAffineFlowPyrLK");
    ...
    IPPI_CALL(icvCalcAffineFlowPyrLK_8uC1R(imgA->tt.data, imgB->tt.data, imgA->step,
                                             img_size, pyrA->tt.data, pyrB->tt.data,
