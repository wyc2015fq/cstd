...
#define _CC_ACOS_TABLE_SIZE  513

static const float icv_acos_table[_CC_ACOS_TABLE_SIZE] = {
        ...
        for(i = 0; i < count; i++)
        {
            ...
            if((dx | dy) != 0)
            {
                ...
                if(pass == 2)
                {
                    ...
                    angle_coeff = buffer[i] * (_CC_ACOS_TABLE_SIZE / 2);
                }
                ...
                for(j = 0; j < count; j++)
                {
                    ...
                    if(i != j)        /* process edge pair */
                    {
                        ...
                        if(pass == 2)
                        {
                            ...
                            dp = cRound(dp * angle_coeff * buffer[j]) +
                                (_CC_ACOS_TABLE_SIZE / 2);
                            dp = MAX(dp, 0);
                            dp = MIN(dp, _CC_ACOS_TABLE_SIZE - 1);
                            hist_row = pghi + dist_dim *
                                cRound(icv_acos_table[dp] * angle_scale);

                            d1 = cRound(d1 * dist_coeff);
                            d2 = cRound(d2 * dist_coeff);
                        }
...
cvCalcPGH(const CvSeq * contour, CvHistogram * hist)
{
    ...
    if(!CC_IS_HIST(hist))
        CC_ERROR(CC_StsBadArg, "The histogram header is invalid ");

    if(CC_IS_SPARSE_HIST(hist))
        CC_ERROR(CC_StsUnsupportedFormat, "Sparse histogram are not supported");

    dims = cvGetDims(hist->bins, size); 

    if(dims != 2)
        CC_ERROR(CC_StsBadSize, "The histogram must be two-dimensional");

    if(!CC_IS_SEQ_POLYGON(contour) || CC_SEQ_ELTYPE(contour) != CC_32SC2)
        CC_ERROR(CC_StsUnsupportedFormat, "The contour is not valid or the point type is not supported");

    IPPI_CALL(icvCalcPGH(contour, ((CvMatND*)(hist->bins))->tt.fl, size[0], size[1]));
