...
static CStatus icvInitEMD(const float *signature1, int size1,
...
static CStatus icvFindBasicVariables(float **cost, char **is_x,
...
static CStatus icvNewSolution(CvEMDState * state);
...
/* The main function */
CC_IMPL float
cvCalcEMD2(const img_t* signature_arr1,
            ...
            void *user_param)
{
    ...
    CC_CALL(signature1 = cvGetMat(signature1, &sign_stub1));
    CC_CALL(signature2 = cvGetMat(signature2, &sign_stub2));
    ...
    if(CC_MAT_TYPE(signature1->tid) != CC_32F)
        CC_ERROR(CC_StsUnsupportedFormat, "The signatures must be 32fC1");

    if(flow)
    {
        CC_CALL(flow = cvGetMat(flow, &flow_stub));
        ...
        if(CC_MAT_TYPE(flow->tid) != CC_32F)
    ...
    IPPI_CALL(result = icvInitEMD(signature1->tt.fl, size1,
                                    signature2->tt.fl, size2,
                                    dims, dist_func, user_param,
                                    cost->tt.fl, cost->step,
    ...
    for(xp = state._x; xp < state.end_x; xp++)
    {
        ...
        if(xp != state.enter_x && ci >= 0 && cj >= 0)
        {
            ...
            if(flow)
                ((float*)(flow->tt.data + flow->step*ci))[cj] = val;
        }
    }
