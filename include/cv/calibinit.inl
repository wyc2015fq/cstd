...
//static img_t* debug_img = 0;

static int icvGenerateQuads(CvCBQuad **quads, CvCBCorner **corners,
                             CvMemStorage *storage, img_t *image, int flags);
                             ...
                             int flags)
{
    ...
    CC_FUNCNAME("cvFindChessBoardCornerGuesses2");
    ...
    if(CC_MAT_TYPE(img) != CC_8U || CC_MAT_CN(img->tid) == 2)
        CC_ERROR(CC_StsUnsupportedFormat, "Only 8-bit grayscale or color images are supported");

    if(pattern_size.width <= 2 || pattern_size.height <= 2)
        CC_ERROR(CC_StsOutOfRange, "pattern should have at least 2x2 size");

    if(!out_corners)
        CC_ERROR(CC_StsNullPtr, "Null pointer to corners");

    CC_CALL(storage = cvCreateMemStorage(0));
    CC_CALL(thresh_img = cvCreateMat(img->rows, img->cols, CC_8UC1));

    if(CC_MAT_CN(img->tid) != 1 || (flags & CC_CALIB_CB_NORMALIZE_IMAGE))
    {
        // equalize the input image histogram -
        // that should make the contrast between "black" and "white" areas big enough
        CC_CALL(norm_img = cvCreateMat(img->rows, img->cols, CC_8UC1));

        if(CC_MAT_CN(img->tid) != 1)
        {
            CC_CALL(cvCvtColor(img, norm_img, CC_BGR2GRAY));
            img = norm_img;
        }
    ...
    for(dilations = min_dilations; dilations <= max_dilations; dilations++)
    {
        ...
        if(flags & CC_CALIB_CB_ADAPTIVE_THRESH)
        {
            int block_size = cRound(MIN(img->cols,img->rows)*0.2)|1;
            cvDilate(img, thresh_img, 0, dilations);

            // convert to binary
            cvAdaptiveThreshold(img, thresh_img, 255,
                CC_ADAPTIVE_THRESH_MEAN_C, CC_THRESH_BINARY, block_size, 0);
        ...
        else
        {
            ...
            int thresh_level = cRound(mean - 10);
            thresh_level = MAX(thresh_level, 10);

            cvThreshold(img, thresh_img, thresh_level, 255, CC_THRESH_BINARY);
        ...
        cvRectangle(thresh_img, cPoint(0,0), cPoint(thresh_img->cols-1,
                     thresh_img->rows-1), CC_RGB(255,255,255), 3, 8);

        CC_CALL(quad_count = icvGenerateQuads(&quads, &corners, storage, thresh_img, flags));
        if(quad_count <= 0)
            continue;

        // Find quad's neighbors
        CC_CALL(icvFindQuadNeighbors(quads, quad_count));

        CC_CALL(quad_group = (CvCBQuad**)cAlloc(sizeof(quad_group[0]) * quad_count));
        CC_CALL(corner_group = (CvCBCorner**)cAlloc(sizeof(corner_group[0]) * quad_count*4));

        for(group_idx = 0; ; group_idx++)
        {
            int count;
            CC_CALL(count = icvFindConnectedQuads(quads, quad_count, quad_group, group_idx, storage));
            ...
            CC_CALL(count = icvCleanFoundConnectedQuads(count, quad_group, pattern_size));
            CC_CALL(count = icvCheckQuadGroup(quad_group, count, corner_group, pattern_size));
...
icvCleanFoundConnectedQuads(int quad_count, CvCBQuad **quad_group, CSize pattern_size)
{
    ...
    CPoint2D32f *centers = 0;

    CC_FUNCNAME("icvCleanFoundConnectedQuads");

    __BEGIN__;

    CPoint2D32f center = {0,0};
    int i, j, k;
    ...
    CC_CALL(centers = (CPoint2D32f *)cAlloc(sizeof(centers[0])*quad_count));
    CC_CALL(temp_storage = cvCreateMemStorage(0));
                   ...
                   CvCBCorner **out_corners, CSize pattern_size)
{
    ...
    CC_CALL(corners = (CvCBCorner**)cAlloc(quad_count*4*sizeof(corners[0])));
                  ...
                  CvMemStorage *storage, img_t *image, int flags)
{
    ...
    min_size = cRound(image->cols * image->rows * .03 * 0.01 * 0.92);
    ...
    CC_CALL(root = cvCreateSeq(0, sizeof(CvSeq), sizeof(CvSeq*), temp_storage));

    // initialize contour retrieving routine
    CC_CALL(scanner = cvStartFindContours(image, temp_storage, sizeof(CvContourEx),
    ...
    CC_CALL(*out_quads = (CvCBQuad*)cAlloc(root->total * sizeof((*out_quads)[0])));
    CC_CALL(*out_corners = (CvCBCorner*)cAlloc(root->total * 4 * sizeof((*out_corners)[0])));
...
{
    CC_FUNCNAME("cvDrawChessboardCorners");
    ...
    CC_CALL(image = cvGetMat(_image, &stub));
