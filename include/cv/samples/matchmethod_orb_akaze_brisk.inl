...
int main(int argc, char *argv[])
{
   ...
   // This algorithm would be used to match descriptors see http://docs.opencv.org/trunk/db/d39/classcv_1_1DescriptorMatcher.html#ab5dc5036569ecc8d47565007fa518257
    ...
    img_t img1 = imread(fileName[0], IMREAD_GRAYSCALE);
    img_t img2 = imread(fileName[1], IMREAD_GRAYSCALE);
    ...
    for (itDesc = typeDesc.begin(); itDesc != typeDesc.end(); ++itDesc)
    {
        ...
        try
        {
            ...
            b->detect(img1, keyImg1, img_t());
            // and compute their descriptors with method  compute
            b->compute(img1, keyImg1, descImg1);
            // or detect and compute descriptors in one step
            b->detectAndCompute(img2, img_t(),keyImg2, descImg2,false);
            // Match method loop
            for (itMatcher = typeAlgoMatch.begin(); itMatcher != typeAlgoMatch.end(); ++itMatcher){
                descriptorMatcher = DescriptorMatcher::create(*itMatcher);
                if ((*itMatcher == "BruteForce-Hamming" || *itMatcher == "BruteForce-Hamming(2)") && (b->descriptorType() == CC_32F || b->defaultNorm() <= NORM_L2SQR))
                {
                    cout << "**************************************************************************\n";
                ...
                try
                {
                    descriptorMatcher->match(descImg1, descImg2, matches, img_t());
                    // Keep best matches only to have a nice drawing.
                    // We sort distance between descriptor matches
                    img_t index;
                    int nbMatch=int(matches.size());
                    img_t tab(nbMatch, 1, CC_32F);
                    ...
                    for (it = bestMatches.begin(); it != bestMatches.end(); ++it)
                    {
                        ...
                        CPoint2d p=keyImg1[it->queryIdx].pt-keyImg2[it->trainIdx].pt;
