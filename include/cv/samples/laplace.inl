...
{
    VideoCapture cap;
    CommandLineParser parser(argc, argv, "{help h | | }{ c | 0 | }{ p | | }");
    if ( parser.has("help") )
    {
        help();
        return 0;
    ...
    for(;;)
    {
        ...
        if(smoothType == GAUSSIAN)
            GaussianBlur(frame, smoothed, CSize(ksize, ksize), sigma, sigma);
        else if(smoothType == BLUR)
            blur(frame, smoothed, CSize(ksize, ksize));
        else
            medianBlur(frame, smoothed, ksize);

        Laplacian(smoothed, laplace, CC_16S, 5);
