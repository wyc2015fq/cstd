...
Point3f get_nearest_3D_point(std::vector<Point3f> &points_list, Point3f origin);
...
{
  _A_matrix = CvMat::zeros(3, 3, CC_64FC1);   // intrinsic camera parameters
  _A_matrix.at<double>(0, 0) = params[0];       //      [ fx   0  cx ]
  _A_matrix.at<double>(1, 1) = params[1];       //      [  0  fy  cy ]
  _A_matrix.at<double>(0, 2) = params[2];       //      [  0   0   1 ]
  _A_matrix.at<double>(1, 2) = params[3];
  _A_matrix.at<double>(2, 2) = 1;
  _R_matrix = CvMat::zeros(3, 3, CC_64FC1);   // rotation matrix
  _t_matrix = CvMat::zeros(3, 1, CC_64FC1);   // translation matrix
  _P_matrix = CvMat::zeros(3, 4, CC_64FC1);   // rotation-translation matrix
...
void PnPProblem::set_P_matrix( const CvMat &R_matrix, const CvMat &t_matrix)
{
...
bool PnPProblem::estimatePose( const std::vector<Point3f> &list_points3d,
                               const std::vector<Point2f> &list_points2d,
                               int flags)
{
  CvMat distCoeffs = CvMat::zeros(4, 1, CC_64FC1);
  CvMat rvec = CvMat::zeros(3, 1, CC_64FC1);
  CvMat tvec = CvMat::zeros(3, 1, CC_64FC1);

  bool useExtrinsicGuess = false;

  // Pose estimation
  bool correspondence = solvePnP( list_points3d, list_points2d, _A_matrix, distCoeffs, rvec, tvec,
...
void PnPProblem::estimatePoseRANSAC( const std::vector<Point3f> &list_points3d, // list with model 3D coordinates
                                     const std::vector<Point2f> &list_points2d,     // list with scene 2D coordinates
                                     int flags, CvMat &inliers, int iterationsCount,  // PnP method; inliers container
                                     float reprojectionError, double confidence )    // Ransac parameters
{
  CvMat distCoeffs = CvMat::zeros(4, 1, CC_64FC1);  // vector of distortion coefficients
  CvMat rvec = CvMat::zeros(3, 1, CC_64FC1);          // output rotation vector
  CvMat tvec = CvMat::zeros(3, 1, CC_64FC1);    // output translation vector
...
std::vector<Point2f> PnPProblem::verify_points(Mesh *mesh)
{
  std::vector<Point2f> verified_points_2d;
  for( int i = 0; i < mesh->getNumVertices(); i++)
  {
    Point3f point3d = mesh->getVertex(i);
    Point2f point2d = this->backproject3DPoint(point3d);
...
Point2f PnPProblem::backproject3DPoint(const Point3f &point3d)
{
  ...
  CvMat point3d_vec = CvMat(4, 1, CC_64FC1);
  ...
  CvMat point2d_vec = CvMat(4, 1, CC_64FC1);
...
bool PnPProblem::backproject2DPoint(const Mesh *mesh, const Point2f &point2d, Point3f &point3d)
{
  ...
  CvMat point2d_vec = CvMat::ones(3, 1, CC_64F); // 3x1
  point2d_vec.at<double>(0) = u * lambda;
  point2d_vec.at<double>(1) = v * lambda;
  point2d_vec.at<double>(2) = lambda;

  // Point in camera coordinates
  CvMat X_c = _A_matrix.inv() * point2d_vec ; // 3x1

  // Point in world coordinates
  CvMat X_w = _R_matrix.inv() * ( X_c - _t_matrix ); // 3x1

  // Center of projection
  CvMat C_op = CvMat(_R_matrix.inv()).mul(-1) * _t_matrix; // 3x1

  // Ray direction vector
  CvMat ray = X_w - C_op; // 3x1
  ...
  std::vector<Point3f> intersections_list;

  // Loop for all the triangles and check the intersection
  for (unsigned int i = 0; i < triangles_list.size(); i++)
  {
    Point3f V0 = mesh->getVertex(triangles_list[i][0]);
    Point3f V1 = mesh->getVertex(triangles_list[i][1]);
    Point3f V2 = mesh->getVertex(triangles_list[i][2]);
...
bool PnPProblem::intersect_MollerTrumbore(Ray &Ray, Triangle &Triangle, double *out)
{
  ...
  Point3f V1 = Triangle.getV0();  // Triangle vertices
  Point3f V2 = Triangle.getV1();
  Point3f V3 = Triangle.getV2();
