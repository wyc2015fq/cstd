...
{
    cout << endl << progName
        << " shows how to use CvMat and IplImages together (converting back and forth)." << endl
...
int main( int argc, char** argv )
{
    ...
    CvMat I = cvarrToMat(IplI); // Convert to the new style container. Only header created. Image not copied.
#else
    CvMat I = imread(imagename);        // the newer cvLoadImage alternative, MATLAB-style function
...
//! [start]
    ...
    vector<CvMat> planes;    // Use the STL's vector structure to store multiple CvMat objects
    ...
    //! [noisy]
    CvMat noisyI(I.size(), CC_8U);           // Create a matrix of the specified size and type
    ...
    GaussianBlur(noisyI, noisyI, CvSize(3, 3), 0.5, 0.5);
    ...
    // CvMat::convertTo() replaces cvConvertScale.
    // One must explicitly specify the output matrix type (we keep it intact - planes[1]->tid)
    ...
    // CvMat::mul replaces cvMul(). Again, no temporary arrays are created in case of simple expressions.
    ...
    //      All the memory will be automatically released by the Vector<>, CvMat and Ptr<> destructor.
    return 0;
}
