...
void drawBoundingBox(img_t image, vector<CPoint2f> bb);
void drawStatistics(img_t image, const Stats& stats);
void printStatistics(string name, Stats stats);
vector<CPoint2f> Points(vector<KeyPoint> keypoints);
Rect2d selectROI(const String &video_name, const img_t &frame);
    ...
    struct Data
    {
        ...
        static void mouseHandler(int event, int x, int y, int flags, void *param)
        {
            ...
            case EVENT_LBUTTONDOWN:
                data->box = cRect( x, y, 0, 0 );
                data->center = CPoint2f((float)x,(float)y);
                break;
