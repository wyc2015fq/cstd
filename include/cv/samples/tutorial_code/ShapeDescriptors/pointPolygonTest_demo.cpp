...
int main( void )
{
  /// Create an image
  const int r = 100;
  CvMat src = CvMat::zeros( CvSize( 4*r, 4*r ), CC_8UC1 );
  ...
  CvMat src_copy = src.clone();

  findContours( src_copy, contours, hierarchy, RETR_TREE, CHAIN_APPROX_SIMPLE);

  /// Calculate the distances to the contour
  CvMat raw_dist( src.size(), CC_32FC1 );
  ...
  minMaxLoc( raw_dist, &minVal, &maxVal, 0, 0, CvMat() );
  minVal = abs(minVal); maxVal = abs(maxVal);

  /// Depicting the  distances graphically
  CvMat drawing = CvMat::zeros( src.size(), CC_8UC3 );
