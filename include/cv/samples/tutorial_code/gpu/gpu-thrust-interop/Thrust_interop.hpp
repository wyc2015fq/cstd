...
template<typename T> struct step_functor : public thrust::unary_function<int, int>
{
    ...
    __host__ step_functor(cuda::GpuMat& mat)
    {
        CC_Assert(mat.depth() == DataType<T>::depth);
        columns = mat.cols;
...
*/
template<typename T>
thrust::permutation_iterator<thrust::device_ptr<T>, thrust::transform_iterator<step_functor<T>, thrust::counting_iterator<int>>>  GpuMatBeginItr(cuda::GpuMat mat, int channel = 0)
{
    if (channel == -1)
    {
        mat = mat.reshape(1);
    ...
    CC_Assert(mat.depth() == DataType<T>::depth);
    CC_Assert(channel < mat.channels());
...
*/
template<typename T>
thrust::permutation_iterator<thrust::device_ptr<T>, thrust::transform_iterator<step_functor<T>, thrust::counting_iterator<int>>>  GpuMatEndItr(cuda::GpuMat mat, int channel = 0)
{
    if (channel == -1)
    {
        mat = mat.reshape(1);
