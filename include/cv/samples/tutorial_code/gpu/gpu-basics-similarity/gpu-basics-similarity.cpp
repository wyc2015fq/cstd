...
double getPSNR(const CvMat& I1, const CvMat& I2);      // CPU versions
Scalar getMSSIM( const CvMat& I1, const CvMat& I2);

double getPSNR_CUDA(const CvMat& I1, const CvMat& I2);  // Basic CUDA versions
Scalar getMSSIM_CUDA( const CvMat& I1, const CvMat& I2);
...
//! [psnr]
double getPSNR_CUDA_optimized(const CvMat& I1, const CvMat& I2, BufferPSNR& b);
...
//! [ssim]
Scalar getMSSIM_CUDA_optimized( const CvMat& i1, const CvMat& i2, BufferMSSIM& b);
...
{
    help();
    CvMat I1 = imread(argv[1]);           // Read the two images
    CvMat I2 = imread(argv[2]);
...
//! [getpsnr]
double getPSNR(const CvMat& I1, const CvMat& I2)
{
    CvMat s1;
    absdiff(I1, I2, s1);       // |I1 - I2|
    s1.convertTo(s1, CC_32F);  // cannot make a square on 8 bits
...
double getPSNR_CUDA_optimized(const CvMat& I1, const CvMat& I2, BufferPSNR& b)
{
    ...
    b.gI1.convertTo(b.t1, CC_32F);
    b.gI2.convertTo(b.t2, CC_32F);
...
//! [getpsnrcuda]
double getPSNR_CUDA(const CvMat& I1, const CvMat& I2)
{
    cuda::GpuMat gI1, gI2, gs, t1,t2;
    ...
    gI1.convertTo(t1, CC_32F);
    gI2.convertTo(t2, CC_32F);
...
Scalar getMSSIM( const CvMat& i1, const CvMat& i2)
{
    ...
    CvMat I2_2   = I2.mul(I2);        // I2^2
    CvMat I1_2   = I1.mul(I1);        // I1^2
    CvMat I1_I2  = I1.mul(I2);        // I1 * I2

    /*************************** END INITS **********************************/

    CvMat mu1, mu2;   // PRELIMINARY COMPUTING
    GaussianBlur(I1, mu1, CvSize(11, 11), 1.5);
    GaussianBlur(I2, mu2, CvSize(11, 11), 1.5);

    CvMat mu1_2   =   mu1.mul(mu1);
    CvMat mu2_2   =   mu2.mul(mu2);
    CvMat mu1_mu2 =   mu1.mul(mu2);

    CvMat sigma1_2, sigma2_2, sigma12;

    GaussianBlur(I1_2, sigma1_2, CvSize(11, 11), 1.5);
    sigma1_2 -= mu1_2;

    GaussianBlur(I2_2, sigma2_2, CvSize(11, 11), 1.5);
    sigma2_2 -= mu2_2;

    GaussianBlur(I1_I2, sigma12, CvSize(11, 11), 1.5);
    sigma12 -= mu1_mu2;
...
Scalar getMSSIM_CUDA( const CvMat& i1, const CvMat& i2)
{
    ...
    gI1.convertTo(tmp1, CC_MAKE_TYPE(CC_32F, gI1.channels()));
    gI2.convertTo(tmp2, CC_MAKE_TYPE(CC_32F, gI2.channels()));
    ...
    Ptr<cuda::Filter> gauss = cuda::createGaussianFilter(vI2[0]->tid, -1, CvSize(11, 11), 1.5);
...
Scalar getMSSIM_CUDA_optimized( const CvMat& i1, const CvMat& i2, BufferMSSIM& b)
{
    ...
    b.gI1.convertTo(b.t1, CC_32F, stream);
    b.gI2.convertTo(b.t2, CC_32F, stream);

    cuda::split(b.t1, b.vI1, stream);
    cuda::split(b.t2, b.vI2, stream);
    Scalar mssim;

    Ptr<cuda::Filter> gauss = cuda::createGaussianFilter(b.vI1[0]->tid, -1, CvSize(11, 11), 1.5);
    ...
    for( int i = 0; i < b.gI1.channels(); ++i )
    {
        ...
        //here too it would be an extra data transfer due to call of operator*(Scalar, CvMat)
