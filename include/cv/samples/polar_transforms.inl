...
int main( int argc, char** argv )
{
    ...
    img_t log_polar_img, lin_polar_img, recovered_log_polar, recovered_lin_polar_img;
    ...
    for(;;)
    {
        ...
        CPoint2f center( (float)frame->cols / 2, (float)frame->rows / 2 );
        double radius = (double)frame->cols / 4;
        double M = (double)frame->cols / log(radius);

        logPolar(frame,log_polar_img, center, M, CC_INTER_LINEAR + WARP_FILL_OUTLIERS);
        linearPolar(frame,lin_polar_img, center, radius, CC_INTER_LINEAR + WARP_FILL_OUTLIERS);

        logPolar(log_polar_img, recovered_log_polar, center, M, CC_WARP_INVERSE_MAP + CC_INTER_LINEAR);
        linearPolar(lin_polar_img, recovered_lin_polar_img, center, radius, CC_WARP_INVERSE_MAP + CC_INTER_LINEAR + WARP_FILL_OUTLIERS);
