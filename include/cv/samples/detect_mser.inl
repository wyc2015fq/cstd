...
static void onMouse(int event, int x, int y, int flags, void*)
{
    ...
    float pi = static_cast<float>(CC_PI);
    if (thetaObs>pi)
    {
...
#ifdef HAVE_OPENGL
static void DrawOpenGLMSER(img_t img, img_t result)
{
    img_t imgGray;
    if (img->tid != CC_8UC1)
...
static img_t MakeSyntheticImage()
{
    img_t img(800, 800, CC_8UC1);
    map<int, char> val;
    int fond = 0;
    img = Scalar(fond);
    val[fond] = 1;
    ...
    for (int i = 0; i<13; i++)
        {
        rectangle(img, Rect(p0, CSize(width[i], width[i])), Scalar(color[i]), 1);
    ...
    calcHist(&img, 1, 0, img_t(), hist, 1, &histSize, histRange, true, false);
    cout << "****************Maximal region************************\n";
...
int main(int argc, char *argv[])
{
    ...
    img_t result(img->rows, img->cols, CC_8UC3);
