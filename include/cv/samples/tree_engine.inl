...
static void train_and_print_errs(Ptr<StatModel> model, const Ptr<TrainData>& data)
{
    ...
    {
        printf( "train error: %f\n", model->calcError(data, false, NULL) );
        printf( "test error: %f\n\n", model->calcError(data, true, NULL) );
    }
}

int main(int argc, char** argv)
{
    CommandLineParser parser(argc, argv, "{ help h | | }{r | 0 | }{ts | | }{@input | | }");
    if (parser.has("help"))
    {
        help();
        return 0;
    ...
    dtree->setPriors(img_t());
    train_and_print_errs(dtree, data);

    if( (int)data->getClassLabels() CC_MAT_TOTAL() <= 2 ) // regression or 2-class classification problem
    {
        printf("======BOOST=====\n");
    ...
    img_t ref_labels = data->getClassLabels();
    img_t test_data = data->getTestSampleIdx();
    img_t predict_labels;
    rtrees->predict(data->getSamples(), predict_labels);

    img_t variable_importance = rtrees->getVarImportance();
