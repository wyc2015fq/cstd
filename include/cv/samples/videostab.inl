...
#if defined(HAVE_OPENCC_CUDAIMGPROC) && defined(HAVE_OPENCC_CUDAOPTFLOW)
        if (gpu)
        {
...
#else
        CC_Assert(gpu == false && "CUDA modules are not available");
#endif
...
int main(int argc, const char **argv)
{
    try
    {
        ...
        if (arg("gpu") == "yes")
        {
            ...
            img_t hostTmp = img_t::zeros(1, 1, CC_32F);
        ...
        if (isTwoPass)
        {
            ...
            if (arg("lin-prog-stab") == "yes")
            {
                ...
                stab->setFrameSize(CSize(source->width(), source->height()));
        ...
        if (arg("border-mode") == "reflect")
            stabilizer->setBorderMode(CC_BORDER_REFLECT);
        else if (arg("border-mode") == "replicate")
            stabilizer->setBorderMode(CC_BORDER_REPLICATE);
        else if (arg("border-mode") == "const")
            stabilizer->setBorderMode(CC_BORDER_CONSTANT);
        else
