...
static void onTrackbar( int, void* )
{
    ...
    img_t edge = gray >= edgeThresh, dist, labels, dist8u;
    ...
    if( voronoiType < 0 )
    {
        ...
        img_t dist32s, dist8u1, dist8u2;

        dist.convertTo(dist32s, CC_32S, 1, 0.5);
        dist32s &= Scalar::all(255);

        dist32s.convertTo(dist8u1, CC_8U, 1, 0);
        dist32s *= -1;

        dist32s += Scalar::all(255);
        dist32s.convertTo(dist8u2, CC_8U);

        img_t planes[] = {dist8u1, dist8u2, dist8u2};
        merge(planes, 3, dist8u);
    }
    else
    {
        dist8u cvSetMat(labels.size(), CC_8UC3);
        for( int i = 0; i < labels->rows; i++ )
        {
            ...
            for( int j = 0; j < labels->cols; j++ )
            {
                ...
                int b = cRound(colors[idx][0]*scale);
                int g = cRound(colors[idx][1]*scale);
                int r = cRound(colors[idx][2]*scale);
                d[j*3] = (uchar)b;
                d[j*3+1] = (uchar)g;
                d[j*3+2] = (uchar)r;
            }
