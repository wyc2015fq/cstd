...
static void help();
static void detectFaces(img_t&, vector<Rect_<int> >&, string);
static void detectEyes(img_t&, vector<Rect_<int> >&, string);
static void detectNose(img_t&, vector<Rect_<int> >&, string);
static void detectMouth(img_t&, vector<Rect_<int> >&, string);
static void detectFacialFeaures(img_t&, const vector<Rect_<int> >, string, string, string);
...
static void detectFaces(img_t& img, vector<Rect_<int> >& faces, string cascade_path)
{
    CascadeClassifier face_cascade;
    face_cascade.load(cascade_path);

    face_cascade.detectMultiScale(img, faces, 1.15, 3, 0|CASCADE_SCALE_IMAGE, CSize(30, 30));
    return;
}

static void detectFacialFeaures(img_t& img, const vector<Rect_<int> > faces, string eye_cascade,
        string nose_cascade, string mouth_cascade)
{
    for(unsigned int i = 0; i < faces.size(); ++i)
    {
        ...
        img_t ROI = img(Rect(face.x, face.y, face.width, face.height));
...
static void detectEyes(img_t& img, vector<Rect_<int> >& eyes, string cascade_path)
{
    CascadeClassifier eyes_cascade;
    eyes_cascade.load(cascade_path);

    eyes_cascade.detectMultiScale(img, eyes, 1.20, 5, 0|CASCADE_SCALE_IMAGE, CSize(30, 30));
    return;
}

static void detectNose(img_t& img, vector<Rect_<int> >& nose, string cascade_path)
{
    CascadeClassifier nose_cascade;
    nose_cascade.load(cascade_path);

    nose_cascade.detectMultiScale(img, nose, 1.20, 5, 0|CASCADE_SCALE_IMAGE, CSize(30, 30));
    return;
}

static void detectMouth(img_t& img, vector<Rect_<int> >& mouth, string cascade_path)
{
    CascadeClassifier mouth_cascade;
    mouth_cascade.load(cascade_path);

    mouth_cascade.detectMultiScale(img, mouth, 1.20, 5, 0|CASCADE_SCALE_IMAGE, CSize(30, 30));
    return;
}
