...
static int readWarp(string iFilename, img_t& warp, int motionType);
static int saveWarp(string fileName, const img_t& warp, int motionType);
static void draw_warped_roi(img_t& image, const int width, const int height, img_t& W);
...
static void draw_warped_roi(img_t& image, const int width, const int height, img_t& W)
{
    CPoint2f top_left, top_right, bottom_left, bottom_right;

    img_t H = img_t (3, 1, CC_32F);
    img_t U = img_t (3, 1, CC_32F);

    img_t warp_mat = img_t::eye (3, 3, CC_32F);
...
int main (const int argc, const char * argv[])
{
    ...
    img_t inputImage = imread(imgFile,0);
    if (inputImage.empty())
    {
    ...
    if (tempImgFile!="") {
        ...
        resize(inputImage, target_image, CSize(216, 216));
        img_t warpGround;
        RNG rng;
        double angle;
        switch (mode_temp) {
        case MOTION_TRANSLATION:
            ...
            warpAffine(target_image, template_image, warpGround,
                CSize(200,200), CC_INTER_LINEAR + CC_WARP_INVERSE_MAP);
            break;
        case MOTION_EUCLIDEAN:
            angle = CC_PI/30 + CC_PI*rng.uniform((double)-2.f, (double)2.f)/180;
    ...
    if (warpType == "homography")
        warp_matrix = img_t::eye(3, 3, CC_32F);
    else
        warp_matrix = img_t::eye(2, 3, CC_32F);
    ...
    // start timing
    const double tic_init = (double) cGetTickCount ();
    ...
    // end timing
    const double toc_final  = (double) cGetTickCount ();
    ...
    img_t warped_image = img_t(template_image->rows, template_image->cols, CC_32FC1);
    if (warp_mode != MOTION_HOMOGRAPHY)
    ...
    if (verbose)
    {
        ...
        img_t identity_matrix = img_t::eye(3,3,CC_32F);
