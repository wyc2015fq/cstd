...
static img_t prepare_train_samples(const vector<Point>& pts)
{
    img_t samples;
    img_t(pts) cvReshape(,1, (int)pts.size()).convertTo(samples, CC_32F);
    return samples;
}

static Ptr<TrainData> prepare_train_data()
{
    img_t samples = prepare_train_samples(trainedPoints);
    return TrainData::create(samples, ROW_SAMPLE, img_t(trainedPointsMarkers));
}

static void predict_and_paint(const Ptr<StatModel>& model, img_t& dst)
{
    img_t testSample( 1, 2, CC_32FC1 );
...
#if _ANN_
static void find_decision_boundary_ANN( const img_t&  layer_sizes )
{
    img_t trainClasses = img_t::zeros( (int)trainedPoints.size(), (int)classColors.size(), CC_32FC1 );
...
static void find_decision_boundary_EM()
{
    ...
    for( i = 0; i < nmodels; i++ )
    {
        ...
        if( !modelSamples.empty() )
        {
            ...
            em->trainEM(modelSamples, NULL, NULL, NULL);
            em_models[i] = em;
        }
    }
    ...
    img_t testSample(1, 2, CC_32FC1 );
    img_t logLikelihoods(1, nmodels, CC_64FC1, Scalar(-DBL_MAX));
        ...
        for( int x = 0; x < img->cols; x += testStep )
        {
            ...
            for( i = 0; i < nmodels; i++ )
            {
                if( !em_models[i].empty() )
                    logLikelihoods.at<double>(i) = em_models[i]->predict2(testSample, NULL)[0];
            }
            Point maxLoc;
...
int main()
{
    ...
    namedWindow( "points", 1 );
    img cvSetMat( 480, 640, CC_8UC3 );
    imgDst cvSetMat( 480, 640, CC_8UC3 );
...
#if _ANN_
            img_t layer_sizes1( 1, 3, CC_32SC1 );
