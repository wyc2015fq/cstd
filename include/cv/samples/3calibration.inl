...
static bool run3Calibration(vector<vector<CPoint2f> > imagePoints1,
                            vector<vector<CPoint2f> > imagePoints2,
                            vector<vector<CPoint2f> > imagePoints3,
                            CSize imageSize, CSize boardSize,
                            float squareSize, float aspectRatio,
                            int flags,
                            img_t& cameraMatrix1, img_t& distCoeffs1,
                            img_t& cameraMatrix2, img_t& distCoeffs2,
                            img_t& cameraMatrix3, img_t& distCoeffs3,
                            img_t& R12, img_t& T12, img_t& R13, img_t& T13)
{
    int c, i;

    // step 1: calibrate each camera individually
    vector<vector<CPoint3f> > objpt(1);
    vector<vector<CPoint2f> > imgpt;
    calcChessboardCorners(boardSize, squareSize, objpt[0]);
    vector<img_t> rvecs, tvecs;

    for( c = 1; c <= 3; c++ )
    {
        const vector<vector<CPoint2f> >& imgpt0 = c == 1 ? imagePoints1 : c == 2 ? imagePoints2 : imagePoints3;
        imgpt.clear();
        int N = 0;
        ...
        img_t distCoeffs = img_t::zeros(5, 1, CC_64F);
    ...
    vector<vector<CPoint2f> > imgpt_right;

    // step 2: calibrate (1,2) and (3,2) pairs
    for( c = 2; c <= 3; c++ )
    {
        const vector<vector<CPoint2f> >& imgpt0 = c == 2 ? imagePoints2 : imagePoints3;

        imgpt.clear();
        imgpt_right.clear();
        int N = 0;

        for( i = 0; i < (int)MIN(imagePoints1.size(), imgpt0.size()); i++ )
        ...
        objpt.resize(imgpt.size(),objpt[0]);
        img_t cameraMatrix = c == 2 ? cameraMatrix2 : cameraMatrix3;
        img_t distCoeffs = c == 2 ? distCoeffs2 : distCoeffs3;
        img_t R, T, E, F;
...
int main( int argc, char** argv )
{
    ...
    vector<vector<CPoint2f> > imgpt[3];
    ...
    img_t view, viewGray;
    img_t cameraMatrix[3], distCoeffs[3], R[3], P[3], R12, T12;
    for( k = 0; k < 3; k++ )
    {
    ...
    img_t R13=Mat_<double>::eye(3,3), T13=Mat_<double>::zeros(3,1);
    ...
    for( k = 0; k < 3; k++ )
        initUndistortRectifyMap(cameraMatrix[k], distCoeffs[k], R[k], P[k], imageSize, CC_16SC2, map1[k], map2[k]);

    img_t canvas(imageSize.height, imageSize.width*3, CC_8UC3), small_canvas;
    destroyWindow("view");
    canvas = Scalar::all(0);
    ...
    for( i = 0; i < (int)(imageList.size()/3); i++ )
    {
        ...
        for( k = 0; k < 3; k++ )
        {
            ...
            img_t rview = canvas.colRange(k2*imageSize.width, (k2+1)*imageSize.width);
            remap(view, rview, map1[k1], map2[k1], CC_INTER_LINEAR);
        }
        printf("%s %s %s\n", imageList[i*3].c_str(), imageList[i*3+1].c_str(), imageList[i*3+2].c_str());
        resize( canvas, small_canvas, CSize(1500, 1500/3) );
