...
static bool readCameraMatrix(const string& filename,
                             img_t& cameraMatrix, img_t& distCoeffs,
                             CSize& calibratedImageSize )
{
    ...
    if( distCoeffs->tid != CC_64F )
        distCoeffs = Mat_<double>(distCoeffs);
    if( cameraMatrix->tid != CC_64F )
        cameraMatrix = Mat_<double>(cameraMatrix);

    return true;
}

static void calcChessboardCorners(CSize boardSize, float squareSize, vector<CPoint3f>& corners)
{
    corners.resize(0);

    for( int i = 0; i < boardSize.height; i++ )
        for( int j = 0; j < boardSize.width; j++ )
            corners.push_back(CPoint3f(float(j*squareSize),
                                      float(i*squareSize), 0));
}


static CPoint3f image2plane(CPoint2f imgpt, const img_t& R, const img_t& tvec,
                           const img_t& cameraMatrix, double Z)
{
    img_t R1 = R.clone();
    ...
    return CPoint3f((float)(v(0,0)*iw), (float)(v(1,0)*iw), (float)Z);
}


static Rect extract3DBox(const img_t& frame, img_t& shownFrame, img_t& selectedObjFrame,
                         const img_t& cameraMatrix, const img_t& rvec, const img_t& tvec,
                         const vector<CPoint3f>& box, int nobjpt, bool runExtraSegmentation)
{
    selectedObjFrame = img_t::zeros(frame.size(), frame->tid);
    if( nobjpt == 0 )
        return Rect();
    vector<CPoint3f> objpt;
    vector<CPoint2f> imgpt;
    ...
    if( nobjpt > 3 )
        for( int i = 0; i < 4; i++ )
            objpt.push_back(CPoint3f(objpt[i].x, objpt[i].y, box[3].z));

    projectPoints(img_t(objpt), rvec, tvec, cameraMatrix, img_t(), imgpt);
    ...
    if( nobjpt <= 2 )
        return Rect();
    vector<Point> hull;
    convexHull(Mat_<Point>(img_t(imgpt)), hull);
    img_t selectedObjMask = img_t::zeros(frame.size(), CC_8U);
    fillConvexPoly(selectedObjMask, &hull[0], (int)hull.size(), Scalar::all(255), 8, 0);
    Rect roi = boundingRect(img_t(hull)) & Rect(Point(), frame.size());

    if( runExtraSegmentation )
    {
        selectedObjMask = Scalar::all(CC_GC_BGD);
        fillConvexPoly(selectedObjMask, &hull[0], (int)hull.size(), Scalar::all(CC_GC_PR_FGD), 8, 0);
        img_t bgdModel, fgdModel;
        grabCut(frame, selectedObjMask, roi, bgdModel, fgdModel,
                3, CC_GC_INIT_WITH_RECT + CC_GC_INIT_WITH_MASK);
...
static int select3DBox(const string& windowname, const string& selWinName, const img_t& frame,
                       const img_t& cameraMatrix, const img_t& rvec, const img_t& tvec,
                       vector<CPoint3f>& box)
{
    ...
    vector<CPoint3f> tempobj(8);
    vector<CPoint2f> imgpt(4), tempimg(8);
    vector<Point> temphull;
    int nobjpt = 0;
    img_t R, selectedObjMask, selectedObjFrame, shownFrame;
    Rodrigues(rvec, R);
    box.resize(4);
             ...
             dragging) && nobjpt < 4 )
        {
            ...
            else
            {
                ...
                if( npt == 2 )
                {
                    ...
                    float len = 1.f/sqrt(dx*dx+dy*dy);
                    tempobj[0] = CPoint3f(dy*len + box[nearestIdx].x,
                                         -dx*len + box[nearestIdx].y, 0.f);
                }
                else
                    tempobj[0] = CPoint3f(box[nearestIdx].x, box[nearestIdx].y, 1.f);

                projectPoints(img_t(tempobj), rvec, tvec, cameraMatrix, img_t(), tempimg);

                CPoint2f a = imgpt[nearestIdx], b = tempimg[0], d1 = b - a, d2 = m - a;
...
static bool readModelViews( const string& filename, vector<CPoint3f>& box,
...
static bool writeModelViews(const string& filename, const vector<CPoint3f>& box,
                            ...
                            const vector<Vec6f>& poseList)
{
    ...
    CC_Assert( nviews == roiList.size() && nviews == poseList.size() );
...
int main(int argc, char** argv)
{
    ...
    CommandLineParser parser(argc, argv, "{help h||}{w||}{h||}{s|1|}{i||}{o||}{@input|0|}");
    if (parser.has("help"))
    {
        puts(helphelp);
        puts(help);
    ...
    img_t frame, shownFrame, selectedObjFrame, mapxy;
    ...
    vector<CPoint3f> box, boardPoints;
    ...
    for(int i = 0;;i++)
    {
        ...
        if( frame.empty() )
        {
            ...
            img_t dummy;
            initUndistortRectifyMap(cameraMatrix, distCoeffs, img_t(),
                                    cameraMatrix, frame0.size(),
                                    CC_32FC2, mapxy, dummy );
            distCoeffs = img_t::zeros(5, 1, CC_64F);
        }
        remap(frame0, frame, mapxy, img_t(), CC_INTER_LINEAR);
        vector<CPoint2f> foundBoardCorners;
        boardFound = findChessboardCorners(frame, boardSize, foundBoardCorners);

        img_t rvec, tvec;
        if( boardFound )
            solvePnP(img_t(boardPoints), img_t(foundBoardCorners), cameraMatrix,
                     distCoeffs, rvec, tvec, false);

        frame.copyTo(shownFrame);
        drawChessboardCorners(shownFrame, boardSize, img_t(foundBoardCorners), boardFound);
        selectedObjFrame = img_t::zeros(frame.size(), frame->tid);
        ...
        if( boardFound && grabNext )
        {
            ...
            if( !box.empty() )
            {
                ...
                if( r.area() )
                {
                    ...
                    float p[6];
                    img_t RV(3, 1, CC_32F, p), TV(3, 1, CC_32F, p+3);
                    rvec.convertTo(RV, RV->tid);
                    tvec.convertTo(TV, TV->tid);
