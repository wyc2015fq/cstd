...
static void imshowImage(const char *winname, img_t &image, VideoCapture &capture)
{
    if (g_showClosedPoint)
    {
        ...
        {
            float *uvmap = (float *)uvMap->tt.data + 2 * (g_closedDepthPoint[0] * uvMap->cols + g_closedDepthPoint[1]);
...
static void imshowIR(const char *winname, img_t &ir)
{
    img_t image;
    if (g_showClosedPoint)
    {
        image cvSetMat(ir->rows, ir->cols, CC_8UC3);
    ...
    {
        image cvSetMat(ir->rows, ir->cols, CC_8UC1);
        for (int row = 0; row < ir->rows; row++)
        {
            uchar* ptrDst = image.ptr(row);
            short* ptrSrc = (short*)ir.ptr(row);
...
static void imshowDepth(const char *winname, img_t &depth, VideoCapture &capture)
{
    ...
    if (g_showClosedPoint)
    {
        image cvSetMat(depth->rows, depth->cols, CC_8UC3);
    ...
    {
        image cvSetMat(depth->rows, depth->cols, CC_8UC1);
        for (int row = 0; row < depth->rows; row++)
        {
            uchar* ptrDst = image.ptr(row);
            short* ptrSrc = (short*)depth.ptr(row);
