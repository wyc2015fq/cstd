...
read_num_class_data( const string& filename, int var_count,
                     img_t* _data, img_t* _responses )
{
    const int M = 1024;
    char buf[M+2];

    img_t el_ptr(1, var_count, CC_32F);
    int i;
    ...
    fclose(f);
    img_t(responses).copyTo(*_responses);
...
prepare_train_data(const img_t& data, const img_t& responses, int ntrain_samples)
{
    img_t sample_idx = img_t::zeros( 1, data->rows, CC_8U );
    img_t train_samples = sample_idx.colRange(0, ntrain_samples);
    train_samples.setTo(Scalar::all(1));

    int nvars = data->cols;
    img_t var_type( nvars + 1, 1, CC_8U );
    var_type.setTo(Scalar::all(VAR_ORDERED));
    var_type.at<uchar>(nvars) = VAR_CATEGORICAL;

    return TrainData::create(data, ROW_SAMPLE, responses,
                             NULL, sample_idx, NULL, var_type);
}
...
static void test_and_save_classifier(const Ptr<StatModel>& model,
                                     const img_t& data, const img_t& responses,
...
//                   int maxCategories, const img_t& priors,
...
//                   TermCriteria termCrit );
    ...
    img_t var_importance = model->getVarImportance();
    if( !var_importance.empty() )
    {
        double rt_imp_sum = sum( var_importance )[0];
        printf("var#\timportance (in %%):\n");
        int i, n = (int)var_importance CC_MAT_TOTAL();
        for( i = 0; i < n; i++ )
                        ...
                        const string& filename_to_load )
{
    ...
    else
    {
        ...
        img_t new_data( ntrain_samples*class_count, var_count + 1, CC_32F );
        img_t new_responses( ntrain_samples*class_count, 1, CC_32S );
        ...
        img_t var_type( 1, var_count + 2, CC_8U );
        var_type.setTo(Scalar::all(VAR_ORDERED));
        ...
        model->setMaxDepth(5);
        model->setUseSurrogates(false);
        model->setPriors(img_t(priors));
        model->train(tdata);
        cout << endl;
    }

    img_t temp_sample( 1, var_count + 1, CC_32F );
    float* tptr = temp_sample img_ptr(float, );
    ...
    for( i = 0; i < nsamples_all; i++ )
    {
        ...
        for( j = 0; j < class_count; j++ )
        {
            ...
            float s = model->predict( temp_sample, NULL, StatModel::RAW_OUTPUT );
            if( max_sum < s )
            {
                max_sum = s;
                best_class = j + 'A';
                      ...
                      const string& filename_to_load )
{
    ...
    else
    {
        ...
        img_t train_data = data cvGetRows(,0, ntrain_samples);
        img_t train_responses = img_t::zeros( ntrain_samples, class_count, CC_32F );
        ...
        img_t layer_sizes( 1, nlayers, CC_32S, layer_sz );
...
int main( int argc, char *argv[] )
{
    ...
    CommandLineParser parser(argc, argv, "{data|../data/letter-recognition.data|}{save||}{load||}{boost||}"
