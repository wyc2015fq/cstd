...
static void updateBrightnessContrast( int /*arg*/, void* )
{
    ...
    img_t dst, hist;
    image.convertTo(dst, CC_8U, a, b);
    imshow("image", dst);

    calcHist(&dst, 1, 0, img_t(), hist, 1, &histSize, 0);
    img_t histImage = img_t::ones(200, 320, CC_8U)*255;

    normalize(hist, hist, 0, histImage->rows, NORM_MINMAX, CC_32F);

    histImage = Scalar::all(255);
    int binW = cRound((double)histImage->cols/histSize);

    for( int i = 0; i < histSize; i++ )
        rectangle( histImage, Point(i*binW, histImage->rows),
                   Point((i+1)*binW, histImage->rows - cRound(hist.at<float>(i))),
                   Scalar::all(0), -1, 8, 0 );
