...
inline bool isFlowCorrect(CPoint2f u)
{
...
static Vec3b computeColor(float fx, float fy)
{
    ...
    const float a = atan2(-fy, -fx) / (float)CC_PI;
...
static void drawOpticalFlow(const Mat_<CPoint2f>& flow, img_t& dst, float maxmotion = -1)
{
    dst cvSetMat(flow.size(), CC_8UC3);
...
static void writeOpticalFlowToFile(const Mat_<CPoint2f>& flow, const string& fileName)
{
...
int main(int argc, const char* argv[])
{
    ...
    img_t frame0 = imread(frame0_name, IMREAD_GRAYSCALE);
    img_t frame1 = imread(frame1_name, IMREAD_GRAYSCALE);
    ...
    const double start = (double)cGetTickCount();
    tvl1->calc(frame0, frame1, flow);
    const double timeSec = (cGetTickCount() - start) / getTickFrequency();
