...
int element_shape = CC_MORPH_RECT;
...
static void OpenClose(int, void*)
{
    ...
    int an = n > 0 ? n : -n;
    img_t element = getStructuringElement(element_shape, CSize(an*2+1, an*2+1), Point(an, an) );
    if( n < 0 )
        morphologyEx(src, dst, CC_MORPH_OPEN, element);
    else
        morphologyEx(src, dst, CC_MORPH_CLOSE, element);
...
int main( int argc, char** argv )
{
    ...
    for(;;)
    {
        ...
        if( c == 27 )
            break;
        if( c == 'e' )
            element_shape = CC_MORPH_ELLIPSE;
        else if( c == 'r' )
            element_shape = CC_MORPH_RECT;
        else if( c == 'c' )
            element_shape = CC_MORPH_CROSS;
        else if( c == ' ' )
