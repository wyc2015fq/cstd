...
int main(int argc, char *argv[])
{
    ...
    img_t img(600, 800, CC_8UC1);
    CommandLineParser parser(argc, argv, "{@input |../data/detect_blob.png| }{h help | | }");
    if (parser.has("h"))
    {
        help();
        return 0;
    ...
    for (itDesc = typeDesc.begin(); itDesc != typeDesc.end(); ++itDesc)
    {
        ...
        try
        {
            ...
            img_t desc, result(img->rows, img->cols, CC_8UC3);
            if (b.dynamicCast<SimpleBlobDetector>() != NULL)
            {
                Ptr<SimpleBlobDetector> sbd = b.dynamicCast<SimpleBlobDetector>();
                sbd->detect(img, keyImg, img_t());
