...
void mouseHandler(int event, int x, int y, int, void*)
{
    ...
    if (event == EVENT_RBUTTONUP)
    {
        ...
        final = img_t::zeros(img0.size(),CC_8UC3);
        res1 = img_t::zeros(img0.size(),CC_8UC1);
...
{
    CommandLineParser parser(argc, argv, "{help h | | show help message}{@input | | input image}");
    if (parser.has("help"))
    {
        parser.printMessage();
        return 0;
    ...
    img_t src = imread(input_image);

    minx = INT_MAX; miny = INT_MAX; maxx = INT_MIN; maxy = INT_MIN;

    img0 = src;

    channel = img0 CC_MAT_CN();
