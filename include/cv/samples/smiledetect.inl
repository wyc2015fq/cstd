                    ...
                    double scale, bool tryflip)
{
    ...
    for ( size_t i = 0; i < faces.size(); i++ )
    {
        ...
        const int half_height=cRound((float)r.height/2);
        r.y=r.y + half_height;
        ...
        int rect_height = cRound((float)img->rows * intensityZeroOne);
        Scalar col = Scalar((float)255 * intensityZeroOne, 0, 0);
        rectangle(img, cPoint(0, img->rows), cPoint(img->cols/10, img->rows - rect_height), col, -1);
    }
