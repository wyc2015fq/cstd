...
static void help()
{
        cout << "\nThis program demonstrates usage of depth sensors (Kinect, XtionPRO,...).\n"
            ...
            "   CAP_OPENNI_DEPTH_MAP            - depth values in mm (CC_16UC1)\n"
            "   CAP_OPENNI_POINT_CLOUD_MAP      - XYZ in meters (CC_32FC3)\n"
            "   CAP_OPENNI_DISPARITY_MAP        - disparity in pixels (CC_8UC1)\n"
            "   CAP_OPENNI_DISPARITY_MAP_32F    - disparity in pixels (CC_32FC1)\n"
            "   CAP_OPENNI_VALID_DEPTH_MASK     - mask of valid pixels (not ocluded, not shaded etc.) (CC_8UC1)\n"
            "2.) Data given from RGB image generator\n"
            "   CAP_OPENNI_BGR_IMAGE            - color image (CC_8UC3)\n"
            "   CAP_OPENNI_GRAY_IMAGE           - gray image (CC_8UC1)\n"
            "2.) Data given from IR image generator\n"
            "   CAP_OPENNI_IR_IMAGE             - gray image (CC_16UC1)\n"
         << endl;
}

static void colorizeDisparity( const img_t& gray, img_t& rgb, double maxDisp=-1.f, float S=1.f, float V=1.f )
{
    CC_Assert( !gray.empty() );
    CC_Assert( gray->tid == CC_8UC1 );
    ...
    rgb cvSetMat( gray.size(), CC_8UC3 );
    rgb = Scalar::all(0);
    if( maxDisp < 1 )
        return;
...
{
    filename.clear();
    CommandLineParser parser(argc, argv, "{h help||}{cd|1|}{fmd|0|}{mode|-1|}{m|010100|}{r||}");
    if (parser.has("h"))
    {
        help();
    ...
    if( !isVideoReading && imageMode >= 0 )
    {
        ...
        switch ( imageMode )
        {
            ...
            case 4:
                ...
                break;
            default:
                CC_Error( CC_StsBadArg, "Unsupported image mode property.\n");
        }
        if (!modeRes)
            cout << "\nThis image mode is not supported by the device, the default value (CC_CAP_OPENNI_SXGA_15HZ) will be used.\n" << endl;
    }
    ...
    for(;;)
    {
        ...
        else
        {
            if( retrievedImageFlags[0] && capture.retrieve( depthMap, CAP_OPENNI_DEPTH_MAP ) )
            {
                ...
                img_t show; depthMap.convertTo( show, CC_8UC1, scaleFactor );
            ...
            {
                img_t ir8;
                irImage.convertTo(ir8, CC_8U, 256.0 / 3500, 0.0);
                imshow("IR image", ir8);
            }
        }
