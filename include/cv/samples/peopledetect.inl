...
static void detectAndDraw(const HOGDescriptor &hog, img_t &img)
{
    ...
    hog.detectMultiScale(img, found, 0, CSize(8,8), CSize(32,32), 1.05, 2);
    ...
    for (size_t i = 0; i < found_filtered.size(); i++)
    {
        ...
        r.x += cRound(r.width*0.1);
        r.width = cRound(r.width*0.8);
        r.y += cRound(r.height*0.07);
        r.height = cRound(r.height*0.8);
