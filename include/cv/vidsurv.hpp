...
{
    cvWriteStructBegin(fs,name,CC_NODE_SEQ|CC_NODE_FLOW);
...
void inline cvReadStructByName(CvFileStorage* fs, CvFileNode* node, char* name, void* addr, char* desc)
{
    ...
    {
        if(CC_NODE_IS_SEQ(pSeqNode->tag))
        {
...
class CC_EXPORTS CvFGDetector: public CvVSModule
{
public:
...
CC_EXPORTS CvFGDetector* cvCreateFGDetectorBase(int type, void *param);
...
#define CC_BLOB_MINW 5
#define CC_BLOB_MINH 5
#define CC_BLOB_ID(pB) (((CvBlob*)(pB))->ID)
#define CC_BLOB_CENTER(pB) cvPoint2D32f(((CvBlob*)(pB))->x,((CvBlob*)(pB))->y)
#define CC_BLOB_X(pB) (((CvBlob*)(pB))->x)
#define CC_BLOB_Y(pB) (((CvBlob*)(pB))->y)
#define CC_BLOB_WX(pB) (((CvBlob*)(pB))->w)
#define CC_BLOB_WY(pB) (((CvBlob*)(pB))->h)
#define CC_BLOB_RX(pB) (0.5f*CC_BLOB_WX(pB))
#define CC_BLOB_RY(pB) (0.5f*CC_BLOB_WY(pB))
#define CC_BLOB_RECT(pB) cvRect(cvRound(((CvBlob*)(pB))->x-CC_BLOB_RX(pB)),cvRound(((CvBlob*)(pB))->y-CC_BLOB_RY(pB)),cvRound(CC_BLOB_WX(pB)),cvRound(CC_BLOB_WY(pB)))
/* END BLOB STRUCTURE*/


/* simple BLOBLIST */
class CC_EXPORTS CvBlobSeq
{
public:
    ...
    virtual CvBlob* GetBlobByID(int BlobID)
    {
        ...
        for(i=0;i<m_pSeq->total;++i)
            if(BlobID == CC_BLOB_ID(GetBlob(i)))
                return GetBlob(i);
        return NULL;
    };
...
class CC_EXPORTS CvBlobTrackSeq
{
public:
...
class CC_EXPORTS CvBlobDetector: public CvVSModule
{
public:
...
CC_EXPORTS CvBlobDetector* cvCreateBlobDetectorSimple();
CC_EXPORTS CvBlobDetector* cvCreateBlobDetectorCC();


struct CC_EXPORTS CvDetectedBlob : public CvBlob
{
    float response;
};

CC_INLINE CvDetectedBlob cvDetectedBlob(float x, float y, float w, float h, int ID = 0, float response = 0.0F)
{
    CvDetectedBlob b;
...
class CC_EXPORTS CvObjectDetector
{
public:
...
CC_INLINE CvRect cvRectIntersection(const CvRect r1, const CvRect r2)
{
...
{
    { CvDrawShape::ELLIPSE, CC_RGB(255,0,0) },
    { CvDrawShape::ELLIPSE, CC_RGB(0,255,0) },
    { CvDrawShape::ELLIPSE, CC_RGB(0,0,255) },
    { CvDrawShape::ELLIPSE, CC_RGB(255,255,0) },
    { CvDrawShape::ELLIPSE, CC_RGB(0,255,255) },
    { CvDrawShape::ELLIPSE, CC_RGB(255,0,255) }
};*/

class CC_EXPORTS CvImageDrawer
{
public:
...
class CC_EXPORTS CvBlobTrackGen: public CvVSModule
{
public:
...
/* declaration of constructors of implemented modules */
CC_EXPORTS CvBlobTrackGen* cvCreateModuleBlobTrackGen1();
CC_EXPORTS CvBlobTrackGen* cvCreateModuleBlobTrackGenYML();



/* BLOB TRACKER INTERFACE */
class CC_EXPORTS CvBlobTracker: public CvVSModule
{
public:
    ...
    virtual int     GetBlobIndexByID(int BlobID)
    {
        ...
        {
            CvBlob* pB=GetBlob(i-1);
            if(CC_BLOB_ID(pB) == BlobID) return i-1;
        }
        return -1;
    };
    /* return pinter to blob by its unique ID */
...
class CC_EXPORTS CvBlobTrackerOne:public CvVSModule
{
public:
...
CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerList(CvBlobTrackerOne* (*create)());
/*BLOB TRACKER ONE INTERFACE */
...
CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerMS1(CvBlobTrackerParamMS* param);
CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerMS2(CvBlobTrackerParamMS* param);
CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerMS1ByList();
...
CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerLHR(CvBlobTrackerParamLH* /*param*/ = NULL);
/* scale optimization */
CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerLHRS(CvBlobTrackerParamLH* /*param*/ = NULL);

/* simple blob tracker based on connected component tracking */
CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerCC();
/* connected component tracking and MSPF resolver for collision */
CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerCCMSPF();
/* blob tracker that integrate MS and CC */
CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerMSFG();
CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerMSFGS();
/* MS without CC */
CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerMS();
/* particle filtering using bahata... coefficient */
CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerMSPF();

/* =========== tracker integrators trackers =============*/
/* integrator based on Partical Filtering method */
//CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerIPF();
/* rule based integrator */
//CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerIRB();
/* integrator based on data fusion used particle filtering */
//CC_EXPORTS CvBlobTracker* cvCreateBlobTrackerIPFDF();




/* Trajectory postprocessing module */
class CC_EXPORTS CvBlobTrackPostProc: public CvVSModule
{
public:
    virtual void    AddBlob(CvBlob* pBlob) = 0;
...
/* Trajectory generation module */
class CC_EXPORTS CvBlobTrackPostProcOne: public CvVSModule
{
public:
...
CC_EXPORTS CvBlobTrackPostProc* cvCreateBlobTrackPostProcList(CvBlobTrackPostProcOne* (*create)());


/* declaration of constructors of implemented modules */
CC_EXPORTS CvBlobTrackPostProc* cvCreateModuleBlobTrackPostProcKalman();
CC_EXPORTS CvBlobTrackPostProc* cvCreateModuleBlobTrackPostProcTimeAverRect();
CC_EXPORTS CvBlobTrackPostProc* cvCreateModuleBlobTrackPostProcTimeAverExp();
...
CC_EXPORTS CvBlobTrackPredictor* cvCreateModuleBlobTrackPredictKalman();



/* Trajectory analyser module */
class CC_EXPORTS CvBlobTrackAnalysis: public CvVSModule
{
public:
    virtual void    AddBlob(CvBlob* pBlob) = 0;
...
class CC_EXPORTS CvBlobTrackFVGen : public CvVSModule
{
public:
    virtual void    AddBlob(CvBlob* pBlob) = 0;
    virtual void    Process(IplImage* pImg, IplImage* pFG) = 0;
...
/* Trajectory Analyser module */
class CC_EXPORTS CvBlobTrackAnalysisOne
{
public:
...
/* create blob traking post processing module based on simle module */
CC_EXPORTS CvBlobTrackAnalysis* cvCreateBlobTrackAnalysisList(CvBlobTrackAnalysisOne* (*create)());

/* declaration of constructors of implemented modules */
/* based on histogramm analysis of 2D FV (x,y)*/
CC_EXPORTS CvBlobTrackAnalysis* cvCreateModuleBlobTrackAnalysisHistP();
/* based on histogramm analysis of 4D FV (x,y,vx,vy)*/
CC_EXPORTS CvBlobTrackAnalysis* cvCreateModuleBlobTrackAnalysisHistPV();
/* based on histogramm analysis of 5D FV (x,y,vx,vy,state)*/
CC_EXPORTS CvBlobTrackAnalysis* cvCreateModuleBlobTrackAnalysisHistPVS();
/* based on histogramm analysis of 4D FV (startpos,stoppos)*/
CC_EXPORTS CvBlobTrackAnalysis* cvCreateModuleBlobTrackAnalysisHistSS();

/* based on SVM classifier analysis of 2D FV (x,y)*/
//CC_EXPORTS CvBlobTrackAnalysis* cvCreateModuleBlobTrackAnalysisSVMP();
/* based on SVM classifier analysis of 4D FV (x,y,vx,vy)*/
//CC_EXPORTS CvBlobTrackAnalysis* cvCreateModuleBlobTrackAnalysisSVMPV();
/* based on SVM classifier analysis of 5D FV (x,y,vx,vy,state)*/
//CC_EXPORTS CvBlobTrackAnalysis* cvCreateModuleBlobTrackAnalysisSVMPVS();
/* based on SVM classifier analysis of 4D FV (startpos,stoppos)*/
//CC_EXPORTS CvBlobTrackAnalysis* cvCreateModuleBlobTrackAnalysisSVMSS();

/* track analysis based on distance between tracks */
CC_EXPORTS CvBlobTrackAnalysis* cvCreateModuleBlobTrackAnalysisTrackDist();

/* analizer based on reation Road and height map*/
//CC_EXPORTS CvBlobTrackAnalysis* cvCreateModuleBlobTrackAnalysis3DRoadMap();

/* analizer that make OR desicion using set of analizers */
CC_EXPORTS CvBlobTrackAnalysis* cvCreateModuleBlobTrackAnalysisIOR();

/* estimator of human height */
class CC_EXPORTS CvBlobTrackAnalysisHeight: public CvBlobTrackAnalysis
{
public:
    virtual double  GetHeight(CvBlob* pB) = 0;
}; 
//CC_EXPORTS CvBlobTrackAnalysisHeight* cvCreateModuleBlobTrackAnalysisHeightScale();



/* AUTO BLOB TRACKER INTERFACE - pipeline of 3 modules */
class CC_EXPORTS CvBlobTrackerAuto: public CvVSModule
{
public:
...
CC_EXPORTS CvBlobTrackerAuto* cvCreateBlobTrackerAuto1(CvBlobTrackerAutoParam1* param = NULL);
...
/*CC_EXPORTS int cvCompareTracks(CvBlobTrackSeq *groundTruth,
...
CC_EXPORTS void cvCreateTracks_One(CvBlobTrackSeq *TS);
CC_EXPORTS void cvCreateTracks_Same(CvBlobTrackSeq *TS1, CvBlobTrackSeq *TS2);
CC_EXPORTS void cvCreateTracks_AreaErr(CvBlobTrackSeq *TS1, CvBlobTrackSeq *TS2, int addW, int addH);


/* HIST API */
class CC_EXPORTS CvProb
{
public:
    virtual ~CvProb() {};
...
/* some Prob */
CC_EXPORTS CvProb* cvCreateProbS(int dim, CvSize size, int sample_num);
CC_EXPORTS CvProb* cvCreateProbMG(int dim, CvSize size, int sample_num);
CC_EXPORTS CvProb* cvCreateProbMG2(int dim, CvSize size, int sample_num);
CC_EXPORTS CvProb* cvCreateProbHist(int dim, CvSize size);

#define CC_BT_HIST_TYPE_S     0
#define CC_BT_HIST_TYPE_MG    1
#define CC_BT_HIST_TYPE_MG2   2
#define CC_BT_HIST_TYPE_H     3
inline CvProb* cvCreateProb(int type, int dim, CvSize size = cvSize(1,1), void* /*param*/ = NULL)
{
    if(type == CC_BT_HIST_TYPE_S) return cvCreateProbS(dim, size, -1);
    if(type == CC_BT_HIST_TYPE_MG) return cvCreateProbMG(dim, size, -1);
    if(type == CC_BT_HIST_TYPE_MG2) return cvCreateProbMG2(dim, size, -1);
    if(type == CC_BT_HIST_TYPE_H) return cvCreateProbHist(dim, size);
    return NULL;
}
...
#define CC_NOISE_NONE               0
#define CC_NOISE_GAUSSIAN           1
#define CC_NOISE_UNIFORM            2
#define CC_NOISE_SPECKLE            3
#define CC_NOISE_SALT_AND_PEPPER    4
/* Add some noise to image */
/* pImg - (input) image without noise */
/* pImg - (output) image with noise */
/* noise_type - type of added noise */
/*  CC_NOISE_GAUSSIAN - pImg += n , n - is gaussian noise with Ampl standart deviation */
/*  CC_NOISE_UNIFORM - pImg += n , n - is uniform noise with Ampl standart deviation */
/*  CC_NOISE_SPECKLE - pImg += n*pImg , n - is gaussian noise with Ampl standart deviation */
/*  CC_NOISE_SALT_AND_PAPPER - pImg = pImg with blacked and whited pixels, 
            Ampl is density of brocken pixels (0-there are not broken pixels, 1 - all pixels are broken)*/
/* Ampl - "amplitude" of noise */                                
CC_EXPORTS void cvAddNoise(IplImage* pImg, int noise_type, double Ampl, CvRandState* rnd_state = NULL);
...
CC_EXPORTS CvTestSeq* cvCreateTestSeq(char* pConfigfile, char** videos, int numvideo, float Scale = 1, int noise_type = CC_NOISE_NONE, double noise_ampl = 0);
CC_EXPORTS void cvReleaseTestSeq(CvTestSeq** ppTestSeq);

/* generete next frame from test video seq and return pointer to it */
CC_EXPORTS IplImage* cvTestSeqQueryFrame(CvTestSeq* pTestSeq);
/* return pointer to current foreground mask */
CC_EXPORTS IplImage* cvTestSeqGetFGMask(CvTestSeq* pTestSeq);
/* return pointer to current image */
CC_EXPORTS IplImage* cvTestSeqGetImage(CvTestSeq* pTestSeq);
/* return frame size of result test video */
CC_EXPORTS CvSize cvTestSeqGetImageSize(CvTestSeq* pTestSeq);
/* return number of frames result test video */
CC_EXPORTS int cvTestSeqFrameNum(CvTestSeq* pTestSeq);
...
CC_EXPORTS int cvTestSeqGetObjectNum(CvTestSeq* pTestSeq);
...
CC_EXPORTS int cvTestSeqGetObjectPos(CvTestSeq* pTestSeq, int ObjIndex, CvPoint2D32f* pPos);
CC_EXPORTS int cvTestSeqGetObjectSize(CvTestSeq* pTestSeq, int ObjIndex, CvPoint2D32f* pSize);

/* add noise to finile image */
CC_EXPORTS void cvTestSeqAddNoise(CvTestSeq* pTestSeq, int noise_type = CC_NOISE_NONE, double noise_ampl = 0);
/* add Intensity variation */
CC_EXPORTS void cvTestSeqAddIntensityVariation(CvTestSeq* pTestSeq, float DI_per_frame, float MinI, float MaxI);
CC_EXPORTS void cvTestSeqSetFrame(CvTestSeq* pTestSeq, int n);
