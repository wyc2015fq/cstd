...
cvArcLength(const void *array, CSlice slice, int is_closed)
{
    ...
    if(contour->total > 1)
    {
        ...
        for(i = 0; i < count; i++)
        {
            ...
            buffer->tt.fl[j] = dx * dx + dy * dy;
            if(++j == N || i == count - 1)
            {
                buffer->cols = j;
                cvPow(&buffer, &buffer, 0.5);
                for(; j > 0; j--)
                    perimeter += buffer->tt.fl[j-1];
            }
        }
    }
...
static CStatus
icvFindCircle(CPoint2D32f pt0, CPoint2D32f pt1,
               CPoint2D32f pt2, CPoint2D32f * center, float *radius)
{
...
CC_INLINE double icvIsPtInCircle(CPoint2D32f pt, CPoint2D32f center, float radius)
{
...
static int
icvFindEnslosingCicle4pts_32f(CPoint2D32f * pts, CPoint2D32f * _center, float *_radius)
{
    ...
    float max_dist = 0;
    CPoint2D32f center;
    CPoint2D32f min_center;
    float radius, min_radius = FLT_MAX;
    CPoint2D32f res_pts[4];
    ...
    center = cPoint2D32f((pts[idxs[0]].x + pts[idxs[1]].x)*0.5f,
...
cvMinEnclosingCircle(const void* array, CPoint2D32f * _center, float *_radius)
{
    ...
    CC_FUNCNAME("cvMinEnclosingCircle");
    ...
    if(!_center || !_radius)
        CC_ERROR(CC_StsNullPtr, "Null center or radius pointers");

    if(CC_IS_SEQ(array))
    {
        sequence = (CvSeq*)array;
        if(!CC_IS_SEQ_POINT_SET(sequence))
            CC_ERROR(CC_StsBadArg, "The passed sequence is not a valid contour");
    }
    else
    {
        CC_CALL(sequence = cvPointSeqFromMat(
    ...
    CC_CALL(cvStartReadSeq(sequence, &reader, 0));

    count = sequence->total;
    is_float = CC_SEQ_ELTYPE(sequence) == CC_32FC2;

    if(!is_float)
    {
        CPoint *pt_left, *pt_right, *pt_top, *pt_bottom;
        CPoint pt;
        pt_left = pt_right = pt_top = pt_bottom = (CPoint *)(reader.ptr);
        CC_READ_SEQ_ELEM(pt, reader);

        for(i = 1; i < count; i++)
        {
            CPoint* pt_ptr = (CPoint*)reader.ptr;
            CC_READ_SEQ_ELEM(pt, reader);
    ...
    {
        CPoint2D32f *pt_left, *pt_right, *pt_top, *pt_bottom;
        CPoint2D32f pt;
        pt_left = pt_right = pt_top = pt_bottom = (CPoint2D32f *) (reader.ptr);
        CC_READ_SEQ_ELEM(pt, reader);

        for(i = 1; i < count; i++)
        {
            CPoint2D32f* pt_ptr = (CPoint2D32f*)reader.ptr;
            CC_READ_SEQ_ELEM(pt, reader);
    ...
    for(k = 0; k < max_iters; k++)
    {
        ...
        for(i = 0; i < count; i++)
        {
            if(!is_float)
            {
                ptfl.x = (float)((CPoint*)reader.ptr)->x;
                ptfl.y = (float)((CPoint*)reader.ptr)->y;
            }
            else
            {
                ptfl = *(CPoint2D32f*)reader.ptr;
            }
            CC_NEXT_SEQ_ELEM(sequence->elem_size, reader);
...
icvContourArea(const CvSeq* contour, double *area)
{
    if(contour->total)
    {
        ...
        if(!is_float)
        {
            xi_1 = ((CPoint*)(reader.ptr))->x;
            yi_1 = ((CPoint*)(reader.ptr))->y;
        }
        else
        {
            xi_1 = ((CPoint2D32f*)(reader.ptr))->x;
            yi_1 = ((CPoint2D32f*)(reader.ptr))->y;
        }
        CC_NEXT_SEQ_ELEM(contour->elem_size, reader);
        ...
        while(lpt-- > 0)
        {
            ...
            if(!is_float)
            {
                xi = ((CPoint*)(reader.ptr))->x;
                yi = ((CPoint*)(reader.ptr))->y;
            }
            else
            {
                xi = ((CPoint2D32f*)(reader.ptr))->x;
                yi = ((CPoint2D32f*)(reader.ptr))->y;
            }
            CC_NEXT_SEQ_ELEM(contour->elem_size, reader);
...
static CStatus icvContourSecArea(CvSeq * contour, CSlice slice, double *area)
{
...
CC_IMPL double
cvContourArea(const void *array, CSlice slice)
{
    double area = 0;

    CC_FUNCNAME("cvContourArea");
    ...
    {
        CC_CALL(contour = cvPointSeqFromMat(
            CC_SEQ_KIND_CURVE, array, &contour_header, &block));
    }
...
*/
static void
icvFitEllipse_F(CvSeq* points, CBox2D* box)
{
    img_t* D = 0;
    
    CC_FUNCNAME("icvFitEllipse_F");
    ...
    int n = points->total;
    CvSeqReader reader;
    int is_float = CC_SEQ_ELTYPE(points) == CC_32FC2;

    img_t _S = cvMat(6,6,CC_64F,S), _C = cvMat(6,6,CC_64F,C), _T = cvMat(6,6,CC_64F,T);
    img_t _EIGVECS = cvMat(6,6,CC_64F,eigenvectors), _EIGVALS = cvMat(6,1,CC_64F,eigenvalues);
    ...
    for(i = 0; i < n; i++)
    {
        if(!is_float)
        {
            offx += ((CPoint*)reader.ptr)->x;
            offy += ((CPoint*)reader.ptr)->y;
        }
        else
        {
            offx += ((CPoint2D32f*)reader.ptr)->x;
            offy += ((CPoint2D32f*)reader.ptr)->y;
        }
    ...
    {
        double x, y;
        double* Dptr = D->tt.db + i*6;
        
        if(!is_float)
        {
            x = ((CPoint*)reader.ptr)->x - offx;
            y = ((CPoint*)reader.ptr)->y - offy;
        }
        else
        {
            x = ((CPoint2D32f*)reader.ptr)->x - offx;
            y = ((CPoint2D32f*)reader.ptr)->y - offy;
        }
        CC_NEXT_SEQ_ELEM(points->elem_size, reader);
    ...
    // calc angle
    box->angle = (float)(180 - atan2(eigenvectors[2], eigenvectors[3])*180/CC_PI);
...
cvFitEllipse2(const img_t* array)
{
    ...
    CC_CALL(Ad = (double*)cAlloc(n*5*sizeof(Ad[0])));
    CC_CALL(bd = (double*)cAlloc(n*sizeof(bd[0])));
...
cvBoundingRect(img_t* array, int update)
{
    ...
    if(CC_IS_SEQ(array))
    {
        ...
        {
            /*if(update == 1)
                CC_ERROR(CC_StsBadArg, "The header is too small to fit the rectangle, "
    ...
    {
        CC_CALL(mat = cvGetMat(array, &stub));
        if(CC_MAT_TYPE(mat) == CC_32SC1 ||
          CC_MAT_TYPE(mat) == CC_32FC1 ||
          CC_MAT_TYPE(mat) == CC_32SC2 ||
          CC_MAT_TYPE(mat) == CC_32FC2
)
        {
            CC_CALL(ptseq = cvPointSeqFromMat(
                CC_SEQ_KIND_GENERIC, mat, &contour_header, &block));
            mat = 0;
        }
        else if(CC_MAT_TYPE(mat) != CC_8UC1 &&
                CC_MAT_TYPE(mat) != CC_8SC1)
            CC_ERROR(CC_StsUnsupportedFormat,
    ...
    if(mat)
    {
        ...
        {
            uchar* _ptr = mat->tt.data + i*mat->step;
    ...
    {   
        ...
        else
        {
            ...
            /* because right and bottom sides of
               ...
               cFloor is used here instead of cCeil */
