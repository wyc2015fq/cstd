             ...
             CvMemStorage* storage, int start_tab_size)
{
    ...
    CC_CALL(map = (CvGenericHash*)cvCreateSet(flags, header_size, elem_size, storage));

    map->tab_size = start_tab_size;
    start_tab_size *= sizeof(map->table[0]);
    CC_CALL(map->table = (void**)cvMemStorageAlloc(storage, start_tab_size));
               ...
               const char* err_msg, const char* source_file, int source_line)
{
    ...
    cvError(CC_StsParseError, func_name, buf, source_file, source_line);
}
...
{
    CC_FUNCNAME("icvFSCreateCollection");

    __BEGIN__;
    
    if(CC_NODE_IS_MAP(tag))
    {
        if(collection->tag != CC_NODE_NONE)
        {
            assert(fs->is_xml != 0);
            CC_PARSE_ERROR("Sequence element should not have name (use <_></_>)");
        }

        CC_CALL(collection->tt.map = cvCreateMap(0, sizeof(CvFileNodeHash),
                            sizeof(CvFileMapNode), fs->memstorage, 16));
    }
    else
    {
        CvSeq* seq;
        CC_CALL(seq = cvCreateSeq(0, sizeof(CvSeq), sizeof(CvFileNode), fs->memstorage));
        
        // if <collection> contains some scalar element, add it to the newly created collection
        if(CC_NODE_TYPE(collection->tag) != CC_NODE_NONE)
            cvSeqPush(seq, collection);

        collection->tt.seq = seq;
    }

    collection->tag = tag;
    cvSetSeqBlockSize(collection->tt.seq, 8);
...
icvFSReleaseCollection(CvSeq* seq)
{
    if(seq)
    {
        ...
        for(i = 0; i < total; i++)
        {
            ...
            if((!is_map || CC_IS_SET_ELEM(node)) && CC_NODE_IS_COLLECTION(node->tag))
            {
                if(CC_NODE_IS_USER(node->tag) && node->info && node->tt.obj.decoded)
                    cvRelease((void**)&node->tt.obj.decoded);
                if(!CC_NODE_SEQ_IS_SIMPLE(node->tt.seq))
                    icvFSReleaseCollection(node->tt.seq);
            }
...
icvFSDoResize(CvFileStorage* fs, char* ptr, int len)
{
    ...
    CC_CALL(new_ptr = (char*)cAlloc(new_size + 256));
...
{
    CC_FUNCNAME("cvReleaseFileStorage");

    __BEGIN__;

    if(!p_fs)
        CC_ERROR(CC_StsNullPtr, "NULL double pointer to file storage");
...
cvGetHashedKey(CvFileStorage* fs, const char* str, int len, int create_missing)
{
    ...
    if(!node && create_missing)
    {
        ...
        CC_CALL(node->str = cvMemStorageAllocString(map->storage, str, len));
               ...
               int create_missing)
{
    ...
    for(k = 0; k < attempts; k++)
    {
        ...
        if(!CC_NODE_IS_MAP(map_node->tag))
        {
            if((!CC_NODE_IS_SEQ(map_node->tag) || map_node->tt.seq->total != 0) &&
                CC_NODE_TYPE(map_node->tag) != CC_NODE_NONE)
                CC_ERROR(CC_StsError, "The node is neither a map nor an empty collection");
            EXIT;
        }
...
{
    CvFileNode* value = 0;
    CC_FUNCNAME("cvGetFileNodeByName");
    ...
    if(!str)
        CC_ERROR(CC_StsNullPtr, "Null element name");
                      ...
                      CvFileNode* seq_node, int index)
{
    ...
    CC_FUNCNAME("cvGetFileNodeFromSeq");
    ...
    if(!seq_node)
        seq = fs->roots;
    else if(!CC_NODE_IS_SEQ(seq_node->tag))
    {
        if(CC_NODE_IS_MAP(seq_node->tag))
            CC_ERROR(CC_StsError, "The node is map. Use cvGetFileNodeFromMap().");
        if(CC_NODE_TYPE(seq_node->tag) == CC_NODE_NONE)
            CC_ERROR(CC_StsError, "The node is an empty object (None).");
        if(index != 0 && index != -1)
    ...
    if(!seq)
        CC_ERROR(CC_StsNullPtr, "The file storage is empty");
...
{
    CC_FUNCNAME("icvProcessSpecialDouble");
    ...
    if(c != '.')
        CC_PARSE_ERROR("Bad format of floating-point constant");
                ...
                CvFileNode* map_node, CvFileNode** value_placeholder)
{
    ...
    CC_CALL(str_hash_node = cvGetHashedKey(fs, ptr, (int)(endptr - ptr), 1));
    CC_CALL(*value_placeholder = cvGetFileNode(fs, map_node, str_hash_node, 1));
    ptr = saveptr;
                  ...
                  int parent_flags, int min_indent)
{
    ...
    int is_parent_flow = CC_NODE_IS_FLOW(parent_flags);
    int value_type = CC_NODE_NONE;
    int len;
    ...
    if(c == '!') // handle explicit type specification
    {
        ...
        *endptr = '\0';
        ...
        {
            CC_CALL(node->info = cvFindType(ptr));
            if(!node->info)
                node->tag &= ~CC_NODE_USER;
        }

        *endptr = d;
        CC_CALL(ptr = icvYMLSkipSpaces(fs, endptr, min_indent, INT_MAX));
...
force_real:
            ...
            /*if(endptr == ptr || isalpha(*endptr))
                CC_CALL(icvProcessSpecialDouble(fs, endptr, &fval, &endptr));*/
...
force_int:
    ...
    else if(c == '\'' || c == '\"') // an explicit string
    {
        ...
        CC_CALL(node->tt.str = cvMemStorageAllocString(fs->memstorage, buf, len));
    }
    else if(c == '[' || c == '{') // collection as a flow
    {
        ...
        CC_CALL(icvFSCreateCollection(fs, CC_NODE_TYPE(struct_flags) +
        ...
        for(++ptr ;;)
        {
            ...
            CC_CALL(ptr = icvYMLSkipSpaces(fs, ptr, new_min_indent, INT_MAX));
            if(*ptr == '}' || *ptr == ']')
            {
                if(*ptr != d)
            ...
            if(node->tt.seq->total != 0)
            {
                if(*ptr != ',')
                    CC_PARSE_ERROR("Missing , between the elements");
                CC_CALL(ptr = icvYMLSkipSpaces(fs, ptr + 1, new_min_indent, INT_MAX));
            }

            if(CC_NODE_IS_MAP(struct_flags))
            {
                CC_CALL(ptr = icvYMLParseKey(fs, ptr, node, &elem));
                CC_CALL(ptr = icvYMLSkipSpaces(fs, ptr, new_min_indent, INT_MAX));
            }
            else
            {
                if(*ptr == ']')
                    break;
                elem = (CvFileNode*)cvSeqPush(node->tt.seq, 0);
            }
            CC_CALL(ptr = icvYMLParseValue(fs, ptr, elem, struct_flags, new_min_indent));
            if(CC_NODE_IS_MAP(struct_flags))
                elem->tag |= CC_NODE_NAMED;
            is_simple &= !CC_NODE_IS_COLLECTION(elem->tag);
        }
        node->tt.seq->flags |= is_simple ? CC_NODE_SEQ_SIMPLE : 0;
    }
    else
    {
...
force_string:
            ...
            if(is_parent_flow || c != ':')
            {
                ...
                while(str_end > ptr && c == ' ');
                str_end++;
                CC_CALL(node->tt.str = cvMemStorageAllocString(fs->memstorage, ptr, (int)(str_end - ptr)));
                ptr = endptr;
                EXIT;
            }
        ...
        CC_CALL(icvFSCreateCollection(fs, struct_flags +
                    (node->info ? CC_NODE_USER : 0), node));
        ...
        for(;;)
        {
            ...
            else
            {
                ...
                CC_CALL(elem = (CvFileNode*)cvSeqPush(node->tt.seq, 0));
            }

            CC_CALL(ptr = icvYMLSkipSpaces(fs, ptr, indent + 1, INT_MAX));
            CC_CALL(ptr = icvYMLParseValue(fs, ptr, elem, struct_flags, indent + 1));
            if(CC_NODE_IS_MAP(struct_flags))
                elem->tag |= CC_NODE_NAMED;
            is_simple &= !CC_NODE_IS_COLLECTION(elem->tag);

            CC_CALL(ptr = icvYMLSkipSpaces(fs, ptr, 0, INT_MAX));
...
icvYMLParse(CvFileStorage* fs)
{
    ...
    for(;;)
    {
        ...
        if(memcmp(ptr, "...", 3) != 0)
        {
            ...
            CC_CALL(ptr = icvYMLParseValue(fs, ptr, root_node, CC_NODE_NONE, 0));
    ...
    if(key)
    {
        ...
        for(i = 0; i < keylen; i++)
        {
            ...
            if(!isalnum(c) && c != '-' && c != '_' && c != ' ')
                CC_ERROR(CC_StsBadArg, "Invalid character occurs in the key");
        }
...
{
    CC_FUNCNAME("icvYMLStartWriteStruct");
    ...
    CC_CALL(icvYMLWrite(fs, key, data, cvFuncName));
...
{
    CC_FUNCNAME("icvYMLEndWriteStruct");
...
{
    //CC_FUNCNAME("icvYMLStartNextStream");
...
{
    CC_FUNCNAME("icvYMLWriteInt");

    __BEGIN__;
    
    char buf[128];
    CC_CALL(icvYMLWrite(fs, key, icv_itoa(value, buf, 10), cvFuncName));
...
{
    CC_FUNCNAME("icvYMLWriteReal");

    __BEGIN__;
    
    char buf[128];
    CC_CALL(icvYMLWrite(fs, key, icvDoubleToString(buf, value), cvFuncName));
...
{
    CC_FUNCNAME("icvYMLWriteComment");
...
icvXMLParseValue(CvFileStorage* fs, char* ptr, CvFileNode* node,
                  int value_type CC_DEFAULT(CC_NODE_NONE))
{
    CC_FUNCNAME("icvXMLParseValue");
    
    __BEGIN__;

    CvFileNode *elem = node;
    int have_space = 1, is_simple = 1;
    int is_user_type = CC_NODE_IS_USER(value_type);
    memset(node, 0, sizeof(*node));

    value_type = CC_NODE_TYPE(value_type);
    ...
    for(;;)
    {
        ...
        {
            CC_CALL(ptr = icvXMLSkipSpaces(fs, ptr, 0));
            have_space = 1;
            c = *ptr;
        }
        ...
        if(c =='<')
        {
            ...
            CC_CALL(ptr = icvXMLParseTag(fs, ptr, &key, &list, &tag_type));
            ...
            if(type_name)
            {
                ...
                {
                    CC_CALL(info = cvFindType(type_name));
                    if(info)
            ...
            if(!CC_NODE_IS_COLLECTION(node->tag))
            {
                CC_CALL(icvFSCreateCollection(fs, is_noname ? CC_NODE_SEQ : CC_NODE_MAP, node));
            }
            else if(is_noname ^ CC_NODE_IS_SEQ(node->tag))
                CC_PARSE_ERROR(is_noname ? "Map element should have a name" :
                              "Sequence element should not have name (use <_></_>)"); 

            if(is_noname)
                elem = (CvFileNode*)cvSeqPush(node->tt.seq, 0);
            else
                CC_CALL(elem = cvGetFileNode(fs, node, key, 1));
                
            CC_CALL(ptr = icvXMLParseValue(fs, ptr, elem, elem_type));
            if(!is_noname)
                elem->tag |= CC_NODE_NAMED;
            is_simple &= !CC_NODE_IS_COLLECTION(elem->tag);
            elem->info = info;
            CC_CALL(ptr = icvXMLParseTag(fs, ptr, &key2, &list, &tag_type));
            if(tag_type != CC_XML_CLOSING_TAG || key2 != key)
                CC_PARSE_ERROR("Mismatched closing tag");
            have_space = 1;
        }
        else
        {
            ...
            elem = node;
            if(node->tag != CC_NODE_NONE)
            {
                if(!CC_NODE_IS_COLLECTION(node->tag))
                    CC_CALL(icvFSCreateCollection(fs, CC_NODE_SEQ, node));
                ...
                (isdigit(d) || d == '.') || c == '.' && isalnum(d))) // a number
            {
                ...
                if(*endptr == '.' || *endptr == 'e')
                {
                    fval = icv_strtod(fs, ptr, &endptr);
                    /*if(endptr == ptr || isalpha(*endptr))
                        CC_CALL(icvProcessSpecialDouble(fs, ptr, &fval, &endptr));*/
            ...
            else
            {
                ...
                CC_CALL(elem->tt.str = cvMemStorageAllocString(fs->memstorage, buf, i));
            }

            if(!CC_NODE_IS_COLLECTION(value_type) && value_type != CC_NODE_NONE)
                break;
            have_space = 0;
        }
    }

    if((CC_NODE_TYPE(node->tag) == CC_NODE_NONE ||
        CC_NODE_TYPE(node->tag) != value_type &&
        !CC_NODE_IS_COLLECTION(node->tag)) &&
        CC_NODE_IS_COLLECTION(value_type))
    {
        CC_CALL(icvFSCreateCollection(fs, CC_NODE_IS_MAP(value_type) ?
                                        CC_NODE_MAP : CC_NODE_SEQ, node));
    }

    if(value_type != CC_NODE_NONE &&
        value_type != CC_NODE_TYPE(node->tag))
        CC_PARSE_ERROR("The actual type is different from the specified type");

    if(CC_NODE_IS_COLLECTION(node->tag) && is_simple)
            node->tt.seq->flags |= CC_NODE_SEQ_SIMPLE;

    node->tag |= is_user_type ? CC_NODE_USER : 0;
                ...
                CvAttrList** _list, int* _tag_type)
{
    ...
    for(;;)
    {
        ...
        CC_CALL(attrname = cvGetHashedKey(fs, ptr, (int)(endptr - ptr), 1));
        ptr = endptr;
        ...
        else
        {
            ...
            if(!last || count >= max_count)
            {
                ...
                CC_CALL(chunk = (CvAttrList*)cvMemStorageAlloc(fs->memstorage, attr_buf_size));
        ...
        if(last)
        {
            ...
            if(*ptr != '=')
            {
                CC_CALL(ptr = icvXMLSkipSpaces(fs, ptr, CC_XML_INSIDE_TAG));
                if(*ptr != '=')
...
icvXMLParse(CvFileStorage* fs)
{
    ...
    /*{
        ...
        if(version && strncmp(version, "1.", 2) != 0)
            CC_ERROR(CC_StsParseError, "Unsupported version of XML");
    }*/
    {
        const char* encoding = cvAttrValue(list, "encoding");
        if(encoding && strcmp(encoding, "ASCII") != 0)
            CC_PARSE_ERROR("Unsupported encoding");
    }
    ...
    while(*ptr != '\0')
    {
        ...
        if(*ptr != '\0')
        {
            ...
            if(tag_type != CC_XML_OPENING_TAG ||
                ...
                CC_PARSE_ERROR("<opencv_storage> tag is missing");

            root_node = (CvFileNode*)cvSeqPush(fs->roots, 0);
            CC_CALL(ptr = icvXMLParseValue(fs, ptr, root_node, CC_NODE_NONE));
            CC_CALL(ptr = icvXMLParseTag(fs, ptr, &key2, &list, &tag_type));
...
icvXMLWriteTag(CvFileStorage* fs, const char* key, int tag_type, CvAttrList list)
{
    ...
    if(tag_type == CC_XML_OPENING_TAG || tag_type == CC_XML_EMPTY_TAG)
    {
        if(CC_NODE_IS_COLLECTION(struct_flags))
        {
            if(CC_NODE_IS_MAP(struct_flags) ^ (key != 0))
    ...
    *ptr++ = '<';
    ...
    for(i = 0; i < len; i++)
    {
        char c = key[i];
        if(!isalnum(c) && c != '_' && c != '-')
            CC_ERROR(CC_StsBadArg, "Invalid character in the key");
        ptr[i] = c;
    }
    ptr += len;
...
icvXMLStartWriteStruct(CvFileStorage* fs, const char* key, int struct_flags,
                        const char* type_name CC_DEFAULT(0))
{
    CC_FUNCNAME("icvXMLStartWriteStruct");
    ...
    CC_CALL(icvXMLWriteTag(fs, key, CC_XML_OPENING_TAG, cvAttrList(attr,0)));

    parent.struct_flags = fs->struct_flags & ~CC_NODE_EMPTY;
    ...
    if(key)
    {
        CC_CALL(fs->struct_tag = cvMemStorageAllocString(fs->strstorage, (char*)key, -1));
    }
    else
    {
        fs->struct_tag.ptr = 0;
...
icvXMLEndWriteStruct(CvFileStorage* fs)
{
    ...
    CC_CALL(icvXMLWriteTag(fs, fs->struct_tag.ptr, CC_XML_CLOSING_TAG, cvAttrList(0,0)));
...
{
    //CC_FUNCNAME("icvXMLStartNextStream");
...
{
    CC_FUNCNAME("icvXMLWriteScalar");

    __BEGIN__;

    if(CC_NODE_IS_MAP(fs->struct_flags) ||
        (!CC_NODE_IS_COLLECTION(fs->struct_flags) && key))
    {
        icvXMLWriteTag(fs, key, CC_XML_OPENING_TAG, cvAttrList(0,0));
        char* ptr = icvFSResizeWriteBuffer(fs, fs->buffer, len);
        memcpy(ptr, data, len);
        fs->buffer = ptr + len;
        icvXMLWriteTag(fs, key, CC_XML_CLOSING_TAG, cvAttrList(0,0));
    }
    else
    {
        char* ptr = fs->buffer;
        ...
        if(key)
            CC_ERROR(CC_StsBadArg, "elements with keys can not be written to sequence");

        fs->struct_flags = CC_NODE_SEQ;

        if((new_offset > fs->wrap_margin && new_offset - fs->struct_indent > 10) ||
            (ptr > fs->buffer_start && ptr[-1] == '>' && !CC_NODE_IS_EMPTY(fs->struct_flags)))
        {
            ptr = icvXMLFlush(fs);
        }
...
cvOpenFileStorage(const char* filename, CvMemStorage* dststorage, int flags)
{
    ...
    if(!filename)
        CC_ERROR(CC_StsNullPtr, "NULL filename");

    CC_CALL(fs = (CvFileStorage*)cAlloc(sizeof(*fs)));
    memset(fs, 0, sizeof(*fs));

    CC_CALL(fs->memstorage = cvCreateMemStorage(default_block_size));
    fs->dststorage = dststorage ? dststorage : fs->memstorage;

    CC_CALL(fs->filename = (char*)cvMemStorageAlloc(fs->memstorage, strlen(filename)+1));
    strcpy(fs->filename, filename);

    fs->flags = CC_FILE_STORAGE;
    ...
    if(fs->write_mode)
    {
        ...
        int buf_size = CC_FS_MAX_LEN*(fs->is_xml ? 6 : 4) + 1024;
        ...
        fs->is_first = 1;
        fs->struct_indent = 0;
        fs->struct_flags = CC_NODE_EMPTY;
        CC_CALL(fs->buffer_start = fs->buffer = (char*)cAlloc(buf_size + 1024));
        fs->buffer_end = fs->buffer_start + buf_size;
        if(fs->is_xml)
        {
            int file_size = (int)ftell(fs->file);
            CC_CALL(fs->strstorage = cvCreateChildMemStorage(fs->memstorage));
            ...
            else
            {
                ...
                CC_CALL(xml_buf = (char*)cAlloc(xml_buf_size+2));
                ...
                if(last_occurence < 0)
                    CC_ERROR(CC_StsError, "Could not find </opencv_storage> in the end of file.\n");
                fclose(fs->file);
    ...
    else
    {
        ...
        buf_size = MAX(buf_size, CC_FS_MAX_LEN*2 + 1024);
        
        CC_CALL(fs->str_hash = cvCreateMap(0, sizeof(CvStringHash),
                        sizeof(CvStringHashNode), fs->memstorage, 256));
        
        CC_CALL(fs->roots = cvCreateSeq(0, sizeof(CvSeq),
                        sizeof(CvFileNode), fs->memstorage));

        CC_CALL(fs->buffer = fs->buffer_start = (char*)cAlloc(buf_size + 256));
        fs->buffer_end = fs->buffer_start + buf_size;
        fs->buffer[0] = '\n';
        fs->buffer[1] = '\0';
...
CC_IMPL void
cvWriteStructBegin(CvFileStorage* fs, const char* key, int struct_flags,
                    const char* type_name, CvAttrList /*attributes*/)
{
    CC_FUNCNAME("cvWriteStructBegin");

    __BEGIN__;

    CC_CHECK_OUTPUT_FILE_STORAGE(fs);
    CC_CALL(fs->start_write_struct(fs, key, struct_flags, type_name));
...
CC_IMPL void
cvWriteStructEnd(CvFileStorage* fs)
{
    CC_FUNCNAME("cvWriteStructEnd");

    __BEGIN__;

    CC_CHECK_OUTPUT_FILE_STORAGE(fs);
    CC_CALL(fs->end_write_struct(fs));
...
{
    CC_FUNCNAME("cvWriteInt");

    __BEGIN__;

    CC_CHECK_OUTPUT_FILE_STORAGE(fs);
    CC_CALL(fs->write_int(fs, key, value));
...
{
    CC_FUNCNAME("cvWriteReal");

    __BEGIN__;

    CC_CHECK_OUTPUT_FILE_STORAGE(fs);
    CC_CALL(fs->write_real(fs, key, value));
...
{
    CC_FUNCNAME("cvWriteString");

    __BEGIN__;

    CC_CHECK_OUTPUT_FILE_STORAGE(fs);
    CC_CALL(fs->write_string(fs, key, value, quote));
...
{
    CC_FUNCNAME("cvWriteComment");

    __BEGIN__;

    CC_CHECK_OUTPUT_FILE_STORAGE(fs);
    CC_CALL(fs->write_comment(fs, comment, eol_comment));
...
{
    CC_FUNCNAME("cvStartNextStream");

    __BEGIN__;

    CC_CHECK_OUTPUT_FILE_STORAGE(fs);
    CC_CALL(fs->start_next_stream(fs));
...
{
    sprintf(dt, "%d%c", CC_MAT_CN(elem_type), icvTypeSymbol[(elem_type)]);
...
icvCalcElemSize(const char* dt, int initial_size)
{
    ...
    CC_CALL(fmt_pair_count = icvDecodeFormat(dt, fmt_pairs, CC_FS_MAX_FMT_PAIRS));
    fmt_pair_count *= 2;
    for(i = 0, size = initial_size; i < fmt_pair_count; i += 2)
    {
        comp_size = CC_ELEM_SIZE(fmt_pairs[i+1]);
        size = cvAlign(size, comp_size);
        size += comp_size * fmt_pairs[i];
    }
    if(initial_size == 0)
    {
        comp_size = CC_ELEM_SIZE(fmt_pairs[1]);
        size = cvAlign(size, comp_size);
    }
...
icvDecodeSimpleFormat(const char* dt)
{
    ...
    CC_FUNCNAME("icvDecodeSimpleFormat");
    ...
    elem_type = CC_MAKETYPE(fmt_pairs[1], fmt_pairs[0]);
        ...
        for(k = 0; k < fmt_pair_count; k++)
        {
            ...
            for(i = 0; i < count; i++)
            {
                ...
                if(fs->is_xml)
                {
                    ...
                    CC_CALL(icvXMLWriteScalar(fs, 0, ptr, buf_len));
                }
                else
                    CC_CALL(icvYMLWrite(fs, 0, ptr, cvFuncName));
            }
...
{
    CC_FUNCNAME("cvStartReadRawData");
    ...
    {
        CC_CALL(cvStartReadSeq(src->tt.seq, reader, 0));
    }
                    ...
                    int len, void* _data, const char* dt)
{
    ...
    CC_FUNCNAME("cvReadRawDataSlice");
        ...
        for(k = 0; k < fmt_pair_count; k++)
        {
            ...
            for(i = 0; i < count; i++)
            {
                ...
                if(CC_NODE_IS_INT(node->tag))
                {
                    ...
                    {
                    case CC_8U:
                        *(uchar*)data = CC_CAST_8U(ival);
                        data++;
                        break;
                    case CC_8S:
                        *(char*)data = CC_CAST_8S(ival);
                        break;
                    case CC_16U:
                        *(ushort*)data = CC_CAST_16U(ival);
                        data += sizeof(ushort);
                        break;
                    case CC_16S:
                        *(short*)data = CC_CAST_16S(ival);
                        data += sizeof(short);
                        break;
                    case CC_32S:
                        *(int*)data = ival;
                ...
                CC_NEXT_SEQ_ELEM(sizeof(CvFileNode), *reader);
                if(!--len)
                    goto end_loop;
            }
               ...
               void* data, const char* dt)
{
    ...
    CC_CALL(cvStartReadRawData(fs, src, &reader));
    cvReadRawDataSlice(fs, &reader, CC_NODE_IS_SEQ(src->tag) ?
                        src->tt.seq->total : 1, data, dt);
...
{
    int i, total = node->tt.seq->total;
    int elem_size = node->tt.seq->elem_size;
    int is_map = CC_NODE_IS_MAP(node->tag);
    CvSeqReader reader;

    cvStartReadSeq(node->tt.seq, &reader, 0);
...
icvWriteFileNode(CvFileStorage* fs, const char* name, const CvFileNode* node)
{
    ...
    {
    case CC_NODE_INT:
        fs->write_int(fs, name, node->tt.i);
        break;
    case CC_NODE_REAL:
        fs->write_real(fs, name, node->tt.f);
        break;
    case CC_NODE_STR:
        fs->write_string(fs, name, node->tt.str.ptr, 0);
        break;
    case CC_NODE_SEQ:
    case CC_NODE_MAP:
        fs->start_write_struct(fs, name, CC_NODE_TYPE(node->tag) +
                (CC_NODE_SEQ_IS_SIMPLE(node->tt.seq) ? CC_NODE_FLOW : 0),
                node->info ? node->info->tid_name : 0);
        icvWriteCollection(fs, node);
        fs->end_write_struct(fs);
        break;
    case CC_NODE_NONE:
        fs->start_write_struct(fs, name, CC_NODE_SEQ, 0);
        fs->end_write_struct(fs);
        break;
    default:
                 ...
                 const CvFileNode* node, int embed)
{
    ...
    {
        CC_CALL(icvWriteCollection(fs, node));
    }
    else
    {
        CC_CALL(icvWriteFileNode(fs, new_node_name, node));
    }
    /*
    int i, stream_count;
             ...
             const void* struct_ptr, CvAttrList /*attr*/)
{
    ...
    assert(CC_IS_MAT(mat));

    CC_CALL(cvWriteStructBegin(fs, name, CC_NODE_MAP, CC_TYPE_NAME_MAT));
    cvWriteInt(fs, "rows", mat->rows);
    cvWriteInt(fs, "cols", mat->cols);
    cvWriteString(fs, "dt", icvEncodeFormat(CC_MAT_TYPE(mat), dt), 0);
    ...
    for(y = 0; y < size.height; y++)
        cvWriteRawData(fs, mat->tt.data + y*mat->step, size.width, dt);
    cvWriteStructEnd(fs);
    cvWriteStructEnd(fs);
...
{
    return CC_NODE_IS_COLLECTION(node->tag) ? node->tt.seq->total :
...
icvReadMat(CvFileStorage* fs, CvFileNode* node)
{
    ...
    CC_CALL(rows = cvReadIntByName(fs, node, "rows", 0));
               ...
               const void* struct_ptr, CvAttrList /*attr*/)
{
    ...
    assert(CC_IS_MATND(mat));

    CC_CALL(cvWriteStructBegin(fs, name, CC_NODE_MAP, CC_TYPE_NAME_MATND));
    ...
    CC_CALL(cvInitNArrayIterator(1, &mat, 0, &stub, &iterator));
...
icvReadMatND(CvFileStorage* fs, CvFileNode* node)
{
    ...
    CC_CALL(sizes_node = cvGetFileNodeByName(fs, node, "sizes"));
    dt = cvReadStringByName(fs, node, "dt", 0);

    if(!sizes_node || !dt)
        CC_ERROR(CC_StsError, "Some of essential matrix attributes are absent");

    dims = CC_NODE_IS_SEQ(sizes_node->tag) ? sizes_node->tt.seq->total :
    ...
    CC_CALL(cvReadRawData(fs, sizes_node, sizes, "i"));
    CC_CALL(elem_type = icvDecodeSimpleFormat(dt));
    ...
    CC_CALL(mat = cvCreateMatND(dims, sizes, elem_type));
    CC_CALL(cvReadRawData(fs, data, mat->tt.data, dt));
                   ...
                   const void* struct_ptr, CvAttrList /*attr*/)
{
    ...
    CC_CALL(memstorage = cvCreateMemStorage());

    CC_CALL(cvWriteStructBegin(fs, name, CC_NODE_MAP, CC_TYPE_NAME_SPARSE_MAT));
...
icvReadSparseMat(CvFileStorage* fs, CvFileNode* node)
{
    ...
    CC_CALL(mat = cvCreateSparseMat(dims, sizes, elem_type));

    cn = CC_MAT_CN(elem_type);
    idx = (int*)alloca(dims*sizeof(idx[0]));
    elements = data->tt.seq;
    ...
    for(i = 0; i < elements->total;)
    {
        ...
        else
        {
            ...
            for(; k < dims; k++)
            {
                ...
                if(!CC_NODE_IS_INT(elem->tag) || elem->tt.i < 0)
                    CC_ERROR(CC_StsParseError, "Sparse matrix data is corrupted");
                idx[k] = elem->tt.i;
            }
        }
        CC_NEXT_SEQ_ELEM(elements->elem_size, reader);
        i++;
        CC_CALL(val = cvPtrND(mat, idx, 0, 1, 0));
        CC_CALL(cvReadRawDataSlice(fs, &reader, cn, val, dt));
        i += cn;
    }
               ...
               const void* struct_ptr, CvAttrList /*attr*/)
{
    ...
    CC_CALL(cvWriteStructBegin(fs, name, CC_NODE_MAP, CC_TYPE_NAME_IMAGE));
    ...
    sprintf(dt_buf, "%d%c", image->nChannels, icvTypeSymbol[type]);
...
icvReadImage(CvFileStorage* fs, CvFileNode* node)
{
    ...
    CC_CALL(width = cvReadIntByName(fs, node, "width", 0));
    ...
    CC_CALL(image = cvCreateImage(cSize(width,height), depth, CC_MAT_CN(elem_type)));
    ...
    for(y = 0; y < height; y++)
    {
        CC_CALL(cvReadRawDataSlice(fs, &reader, width,
...
{
    CC_FUNCNAME("icvWriteHeaderData");
    ...
    {
        int dt_header_size;
        CC_CALL(dt_header_size = icvCalcElemSize(header_dt, initial_header_size));
              ...
              int initial_elem_size, char* dt_buf)
{
    ...
    {
        int dt_elem_size;
        CC_CALL(dt_elem_size = icvCalcElemSize(dt, initial_elem_size));
    ...
    {
        int align = CC_MAT_TYPE(seq->flags) == CC_64F ? sizeof(double) : sizeof(size_t);
        int full_elem_size = cvAlign(CC_ELEM_SIZE(seq->flags) + initial_elem_size, align);
        if(seq->elem_size != full_elem_size)
            CC_ERROR(CC_StsUnmatchedSizes,
            "CSize of sequence element (elem_size) is inconsistent with seq->flags");
             ...
             CvAttrList attr, int level)
{
    ...
    CC_CALL(cvWriteStructBegin(fs, name, CC_NODE_MAP, CC_TYPE_NAME_SEQ));
    ...
    CC_CALL(dt = icvGetFormat(seq, "dt", &attr, 0, dt_buf));
    cvWriteString(fs, "dt", dt, 0);

    CC_CALL(icvWriteHeaderData(fs, seq, &attr, sizeof(CvSeq)));
    cvWriteStructBegin(fs, "data", CC_NODE_SEQ + CC_NODE_FLOW);
                 ...
                 const void* struct_ptr, CvAttrList attr)
{
    ...
    if(!is_recursive)
    {
        CC_CALL(icvWriteSeq(fs, name, seq, attr, -1));
    }
    else
    {
        CvTreeNodeIterator tree_iterator;

        CC_CALL(cvWriteStructBegin(fs, name, CC_NODE_MAP, CC_TYPE_NAME_SEQ_TREE));
        CC_CALL(cvWriteStructBegin(fs, "sequences", CC_NODE_SEQ));
        ...
        for(;;)
        {
            if(!tree_iterator.node)
                break;
            CC_CALL(icvWriteSeq(fs, 0, tree_iterator.node, attr, tree_iterator.level));
...
icvReadSeq(CvFileStorage* fs, CvFileNode* node)
{
    ...
    CC_CALL(flags_str = cvReadStringByName(fs, node, "flags", 0));
    ...
    if(endptr == flags_str || (flags & CC_MAGIC_MASK) != CC_SEQ_MAGIC_VAL)
    ...
    if(header_dt)
    {
        CC_CALL(header_size = icvCalcElemSize(header_dt, header_size));
    }
    else if(rect_node)
    ...
    CC_CALL(elem_size = icvCalcElemSize(dt, 0));
    CC_CALL(seq = cvCreateSeq(flags, header_size, elem_size, fs->dststorage));

    if(header_node)
    {
        CC_CALL(cvReadRawData(fs, header_node, (char*)seq + sizeof(CvSeq), header_dt));
    }
    else if(rect_node)
    {
        CvPoint2DSeq* point_seq = (CvPoint2DSeq*)seq;
...
icvReadSeqTree(CvFileStorage* fs, CvFileNode* node)
{
    ...
    sequences = sequences_node->tt.seq;
    ...
    for(i = 0; i < total; i++)
    {
        ...
        CvSeq* seq;
        int level;
        CC_CALL(seq = (CvSeq*)cvRead(fs, elem));
        CC_CALL(level = cvReadIntByName(fs, elem, "level", -1));
        if(level < 0)
               ...
               const void* struct_ptr, CvAttrList attr)
{
    ...
    CC_CALL(flag_buf = (int*)cAlloc(vtx_count*sizeof(flag_buf[0])));
...
icvReadGraph(CvFileStorage* fs, CvFileNode* node)
{
    ...
    flags = (int)strtol(flags_str, &endptr, 16);
    if(endptr == flags_str ||
        (flags & (CC_SEQ_KIND_MASK|CC_MAGIC_MASK)) != (CC_GRAPH|CC_SET_MAGIC_VAL))
        CC_ERROR(CC_StsError, "Invalid graph signature");
    ...
    if(vtx_dt > 0)
    {
        CC_CALL(src_vtx_size = icvCalcElemSize(vtx_dt, 0));
        CC_CALL(vtx_size = icvCalcElemSize(vtx_dt, vtx_size));
        CC_CALL(fmt_pair_count = icvDecodeFormat(edge_dt,
...

    {
        ...
        CC_CALL(edge_size = icvCalcElemSize(dst_edge_dt, sizeof(CvGraphEdge)));
        CC_CALL(src_edge_size = icvCalcElemSize(edge_dt, 0));
    }
    
    CC_CALL(graph = cvCreateGraph(flags, header_size, vtx_size, edge_size, fs->dststorage));

    if(header_node)
        CC_CALL(cvReadRawData(fs, header_node, (char*)graph + sizeof(CvGraph), header_dt));

    read_buf_size = MAX(src_vtx_size*3, 1 << 16);
    read_buf_size = MAX(src_edge_size*3, read_buf_size);
    CC_CALL(read_buf = (char*)cAlloc(read_buf_size));
    CC_CALL(vtx_buf = (CvGraphVtx**)cAlloc(vtx_count * sizeof(vtx_buf[0])));
    ...
    for(k = 0; k < 2; k++)
    {
        ...
        for(i = 0; i < elem_count; i++)
        {
            ...
            else
            {
                ...
                CC_CALL(result = cvGraphAddEdgeByPtr(graph,
...
cvRegisterType(const CvTypeInfo* _info)
{
    ...
    CC_CALL(info = (CvTypeInfo*)cAlloc(sizeof(*info) + len + 1));
...
void* cvClone(const void* struct_ptr)
{
    ...
    CC_CALL(struct_copy = info->clone(struct_ptr));
...
cvRead(CvFileStorage* fs, CvFileNode* node, CvAttrList* list)
{
    ...
    CC_CALL(obj = node->info->read(fs, node));
         ...
         const void* ptr, CvAttrList attributes)
{
    ...
    CC_CALL(info->write(fs, name, ptr, attributes));
        ...
        const char* _name, const char* comment, CvAttrList attributes)
{
    ...
    char name_buf[CC_FS_MAX_LEN + 256];
    ...
    if(comment)
        CC_CALL(cvWriteComment(fs, comment, 0));
    CC_CALL(cvWrite(fs, name, struct_ptr, attributes));
        ...
        const char* name, const char** _real_name)
{
    ...
    CvFileNode* node = 0;
    CC_CALL(fs = cvOpenFileStorage(filename, memstorage, CC_STORAGE_READ));
    ...
    if(name)
    {
        CC_CALL(node = cvGetFileNodeByName(fs, 0, name));
    }
    else
    {
        int i, k;
