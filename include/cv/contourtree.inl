...
#define CC_MATCH_CHECK(status, cvFun)                                    \
  {                                                                        \
    status = cvFun;                                                        \
    if(status != CC_OK)                                                  \
     goto M_END;                                                           \
...
static CStatus
icvCalcTriAttr(const CvSeq * contour, CPoint t2, CPoint t1, int n1,
                CPoint t3, int n3, double *s, double *s_c,
...
{
    CPoint *pt_p;              /*  pointer to previos points   */
    CPoint *pt_n;              /*  pointer to next points      */
    CPoint *pt1, *pt2;         /*  pointer to current points   */

    CPoint t, tp1, tp2, tp3, tn1, tn2, tn3;
...
/*   Convert Sequence to array    */
    ...
    pt_p = (CPoint *) cAlloc(lpt * sizeof(CPoint));
    pt_n = (CPoint *) cAlloc(lpt * sizeof(CPoint));

    num_p = (int *) cAlloc(lpt * sizeof(int));
    num_n = (int *) cAlloc(lpt * sizeof(int));

    hearder_size = sizeof(CvContourTree);
    seq_flags = CC_SEQ_POLYGON_TREE;
    cvStartWriteSeq(seq_flags, hearder_size, sizeof(_CvTrianAttr), storage, &writer);

    ptr_p = (_CvTrianAttr **) cAlloc(lpt * sizeof(_CvTrianAttr *));
    ptr_n = (_CvTrianAttr **) cAlloc(lpt * sizeof(_CvTrianAttr *));
...
/*  binary tree constraction    */
    while(i > 4)
    {
        ...
        for(j = 0; j < i; j++)
        {
            ...
            CC_MATCH_CHECK(status, icvCalcTriAttr(contour, tn2, tn1, nmn1, tn3, nmn3,
...
/*   form child vertexs for the root     */
    {
        ...
        tree_two.pt = tn2;
        tree_two.sign = (char) (CC_SIGN(sn2));
        ...
        if(s_c > sn2_c)
        {
            ...
            CC_WRITE_SEQ_ELEM(tree_two, writer);
            cur_adr = (_CvTrianAttr *) (writer.ptr - writer.seq->elem_size);
    ...
    {
        tree_one.pt = tp1;
        tree_one.sign = (char) (CC_SIGN(sp1));
        ...
        tree_two.pt = tn1;
        tree_two.sign = (char) (CC_SIGN(sn1));
...
/*    CC_WRITE_SEQ_ELEM (tree_one, start_writer);   */
    i_tree++;
...
/*   calculate interceptive area   */
        *s_c = cvContourArea(contour, cSlice(n1, n3+1));
    }
    else
    {
        *h = 0;
                          ...
                          CTermCriteria  criteria)
{
    ...
    CC_FUNCNAME("cvContourFromContourTree");
    ...
    if(!CC_IS_SEQ_POLYGON_TREE(tree))
    ...
    i_buf = 0;
    cur_level = 0;
    log_iter = (char) (criteria.type == CC_TERMCRIT_ITER ||
                       (criteria.type == CC_TERMCRIT_ITER + CC_TERMCRIT_EPS));
    log_eps = (char) (criteria.type == CC_TERMCRIT_EPS ||
                      (criteria.type == CC_TERMCRIT_ITER + CC_TERMCRIT_EPS));
    ...
    seq_flags = CC_SEQ_POLYGON;
    cvStartWriteSeq(seq_flags, out_hearder_size, sizeof(CPoint), storage, &writer);

    ptr_buf = (_CvTrianAttr **) cAlloc(lpt * sizeof(_CvTrianAttr *));
    if(ptr_buf == NULL)
        CC_ERROR_FROM_STATUS(CC_OUTOFMEM_ERR);
    if(log_iter)
    {
        level_buf = (int *) cAlloc(lpt * (sizeof(int)));
