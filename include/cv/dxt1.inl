...
CC_IMPL void
cvDFT(const img_t* src, img_t* dst, int flags, int nonzero_rows CC_DEFAULT(0))
{
    ...
    if (dst!=src) {
      cvSetTo(dst, src->h, src->w, src->tid, CC_MAT_CN(src));
    }
    ...
    src0 = (img_t*)src;
    elem_size = CC_ELEM_SIZE(src->tid);
...
CC_IMPL void
cvMulSpectrums(const img_t* srcA, const img_t* srcB,
                img_t* dst, int flags)
{
...
CC_IMPL void
cvDCT(const img_t* src, img_t* dst, int flags)
{
