...
typedef CStatus (C_STDCALL * CvColorCvtFunc0)(const void* src, int srcstep, void* dst, int dststep, CSize size);
typedef CStatus (C_STDCALL * CvColorCvtFunc1)(const void* src, int srcstep, void* dst, int dststep, CSize size, int param0);
typedef CStatus (C_STDCALL * CvColorCvtFunc2)(const void* src, int srcstep, void* dst, int dststep, CSize size, int param0, int param1);
typedef CStatus (C_STDCALL * CvColorCvtFunc3)(const void* src, int srcstep, void* dst, int dststep, CSize size, int param0, int param1, int param2);

/****************************************************************************************\
*                 Various 3/4-channel to 3/4-channel RGB transformations                 *
\****************************************************************************************/

#define CC_IMPL_BGRX2BGR(flavor, arrtype)                             \
static CStatus C_STDCALL                                              \
icvBGRx2BGR_##flavor##_CnC3R(const arrtype* src, int srcstep,          \
                              arrtype* dst, int dststep,                \
                              CSize size, int src_cn, int blue_idx)   \
{                                                                       \
    int i;                                                              \
                                                                        \
...
#define CC_IMPL_BGR2BGRX(flavor, arrtype)                             \
static CStatus C_STDCALL                                              \
icvBGR2BGRx_##flavor##_C3C4R(const arrtype* src, int srcstep,          \
                              arrtype* dst, int dststep,                \
                              CSize size, int blue_idx)               \
{                                                                       \
    int i;                                                              \
                                                                        \
    srcstep /= sizeof(src[0]);                                          \
...
#define CC_IMPL_BGRA2RGBA(flavor, arrtype)                            \
static CStatus C_STDCALL                                              \
...
CC_IMPL_BGRX2BGR(8u, uchar)
CC_IMPL_BGRX2BGR(16u, ushort)
CC_IMPL_BGRX2BGR(32f, int)
CC_IMPL_BGR2BGRX(8u, uchar)
CC_IMPL_BGR2BGRX(16u, ushort)
CC_IMPL_BGR2BGRX(32f, int)
CC_IMPL_BGRA2RGBA(8u, uchar)
CC_IMPL_BGRA2RGBA(16u, ushort)
CC_IMPL_BGRA2RGBA(32f, int)
...
#define CC_IMPL_BGRx2ABC_IPP(flavor, arrtype)                         \
static CStatus C_STDCALL                                              \
icvBGRx2ABC_IPP_##flavor##_CnC3R(const arrtype* src, int srcstep,      \
    arrtype* dst, int dststep, CSize size, int src_cn,                 \
        ...
        {                                                               \
            ...
            {                                                           \
                ...
                buffer[k+2] = b;                                        \
                                                                        ...
                                                                        \
            status = ipp_func(buffer, CC_STUB_STEP,                    \
                               dst1, CC_STUB_STEP, cSize(di,1));      \
            if(status < 0)                                            \
                return status;                                          \
        }                                                               \
    }                                                                   \
...
icvBGRx2ABC_IPP_8u_CnC3R(const uchar* src, int srcstep,
    uchar* dst, int dststep, CSize size, int src_cn,
    int blue_idx, CvColorCvtFunc0 ipp_func)
{
    int block_size = MIN(1 << 14, size.width);
    uchar* buffer;
...
//CC_IMPL_BGRx2ABC_IPP(8u, uchar)
CC_IMPL_BGRx2ABC_IPP(16u, ushort)
CC_IMPL_BGRx2ABC_IPP(32f, float)

#define CC_IMPL_ABC2BGRx_IPP(flavor, arrtype)                         \
static CStatus C_STDCALL                                              \
icvABC2BGRx_IPP_##flavor##_C3CnR(const arrtype* src, int srcstep,      \
    arrtype* dst, int dststep, CSize size, int dst_cn,                 \
    int blue_idx, CvColorCvtFunc0 ipp_func)                            \
{                                                                       \
    ...
    {                                                                   \
        ...
        {                                                               \
            ...
            di = MIN(block_size, size.width - i);                       \
                                                                        \
            status = ipp_func(src1, CC_STUB_STEP,                      \
                               buffer, CC_STUB_STEP, cSize(di,1));    \
            if(status < 0)                                            \
                return status;                                          \
                                                                        \
...
CC_IMPL_ABC2BGRx_IPP(8u, uchar)
CC_IMPL_ABC2BGRx_IPP(16u, ushort)
CC_IMPL_ABC2BGRx_IPP(32f, float)
...
#define CC_IMPL_GRAY2BGRX(flavor, arrtype)                    \
static CStatus C_STDCALL                                      \
...
CC_IMPL_GRAY2BGRX(8u, uchar)
CC_IMPL_GRAY2BGRX(16u, ushort)
CC_IMPL_GRAY2BGRX(32f, float)


static CStatus C_STDCALL
icvBGR5x52Gray_8u_C2C1R(const uchar* src, int srcstep,
                         uchar* dst, int dststep,
                         CSize size, int green_bits)
{
    int i;
    assert(green_bits == 5 || green_bits == 6);
        ...
        if(green_bits == 6)
            for(i = 0; i < size.width; i++)
            {
                ...
                t = ((t << 3) & 0xf8)*cscGb + ((t >> 3) & 0xfc)*cscGg +
                    ((t >> 8) & 0xf8)*cscGr;
                dst[i] = (uchar)CC_DESCALE(t,csc_shift);
            }
        else
            for(i = 0; i < size.width; i++)
            {
...
icvBGRx2Gray_16u_CnC1R(const ushort* src, int srcstep,
                        ushort* dst, int dststep, CSize size,
                        int src_cn, int blue_idx)
{
    int i;
    ...
    for(; size.height--; src += srcstep, dst += dststep)
        for(i = 0; i < size.width; i++, src += src_cn)
            dst[i] = (ushort)CC_DESCALE((unsigned)(src[0]*cb +
...
icvBGRx2Gray_32f_CnC1R(const float* src, int srcstep,
                        float* dst, int dststep, CSize size,
                        int src_cn, int blue_idx)
{
    int i;
...
#define CC_IMPL_BGRx2YCrCb(flavor, arrtype, worktype, scale_macro, cast_macro,     \
                            YUV_YB, YUV_YG, YUV_YR, YUV_CR, YUV_CB, YUV_Cx_BIAS)   \
static CStatus C_STDCALL                                                  \
icvBGRx2YCrCb_##flavor##_CnC3R(const arrtype* src, int srcstep,            \
    arrtype* dst, int dststep, CSize size, int src_cn, int blue_idx)      \
{                                                                           \
    int i;                                                                  \
    srcstep /= sizeof(src[0]);                                              \
    dststep /= sizeof(src[0]);                                              \
...
CC_IMPL_BGRx2YCrCb(8u, uchar, int, yuv_descale, CC_CAST_8U,
                    yuvYb, yuvYg, yuvYr, yuvCr, yuvCb, 128)

CC_IMPL_BGRx2YCrCb(16u, ushort, int, yuv_descale, CC_CAST_16U,
                    yuvYb, yuvYg, yuvYr, yuvCr, yuvCb, 32768)

CC_IMPL_BGRx2YCrCb(32f, float, float, CC_NOP, CC_NOP,
                    yuvYb_32f, yuvYg_32f, yuvYr_32f, yuvCr_32f, yuvCb_32f, 0.5f)


#define CC_IMPL_YCrCb2BGRx(flavor, arrtype, worktype, prescale_macro,      \
...
CC_IMPL_YCrCb2BGRx(8u, uchar, int, yuv_prescale, yuv_descale, CC_CAST_8U,
                    yuvBCb, yuvGCr, yuvGCb, yuvRCr, 128)

CC_IMPL_YCrCb2BGRx(16u, ushort, int, yuv_prescale, yuv_descale, CC_CAST_16U,
                    yuvBCb, yuvGCr, yuvGCb, yuvRCr, 32768)

CC_IMPL_YCrCb2BGRx(32f, float, float, CC_NOP, CC_NOP, CC_NOP,
...
#define CC_IMPL_BGRx2XYZ(flavor, arrtype, worktype,                        \
...
{                                                                           \
    ...
    if(blue_idx)                                                          \
    {                                                                       \
        CC_SWAP(matrix[0], matrix[2], t);                                 \
        CC_SWAP(matrix[3], matrix[5], t);                                 \
        CC_SWAP(matrix[6], matrix[8], t);                                 \
    }                                                                       \
                                                                            \
    for(; size.height--; src += srcstep, dst += dststep)                  \
    {                                                                       \
...
CC_IMPL_BGRx2XYZ(8u, uchar, int, xyz_descale, CC_CAST_8U, _32s)
CC_IMPL_BGRx2XYZ(16u, ushort, int, xyz_descale, CC_CAST_16U, _32s)
CC_IMPL_BGRx2XYZ(32f, float, float, CC_NOP, CC_NOP, _32f)


#define CC_IMPL_XYZ2BGRx(flavor, arrtype, worktype, scale_macro,           \
                          cast_macro, suffix)                              \
static CStatus C_STDCALL                                                  \
...
{                                                                           \
    ...
    if(blue_idx)                                                          \
    {                                                                       \
        CC_SWAP(matrix[0], matrix[6], t);                                 \
        CC_SWAP(matrix[1], matrix[7], t);                                 \
        CC_SWAP(matrix[2], matrix[8], t);                                 \
    }                                                                       \
                                                                            \
    for(; size.height--; src += srcstep, dst += dststep)                  \
    {                                                                       \
...
CC_IMPL_XYZ2BGRx(8u, uchar, int, xyz_descale, CC_CAST_8U, _32s)
CC_IMPL_XYZ2BGRx(16u, ushort, int, xyz_descale, CC_CAST_16U, _32s)
CC_IMPL_XYZ2BGRx(32f, float, float, CC_NOP, CC_NOP, _32f)
...
icvABC2BGRx_8u_C3CnR(const uchar* src, int srcstep, uchar* dst, int dststep,
                      CSize size, int dst_cn, int blue_idx, CvColorCvtFunc2 cvtfunc_32f,
                     const float* pre_coeffs, int postscale)
{
    ...
    for(; size.height--; src += srcstep, dst += dststep)
    {
        for(i = 0; i < size.width; i += block_size)
        {
            ...
            status = cvtfunc_32f(buffer, 0, buffer, 0, cSize(di,1), 3, blue_idx);
            if(status < 0)
                return status;
            ...
            if(postscale)
            {
                for(k = 0; k < di*3; k += 3, dst += dst_cn)
                {
                    int b = cRound(buffer[k]*255.);
                    int g = cRound(buffer[k+1]*255.);
                    int r = cRound(buffer[k+2]*255.);
            ...
            else
            {
                for(k = 0; k < di*3; k += 3, dst += dst_cn)
                {
                    int b = cRound(buffer[k]);
                    int g = cRound(buffer[k+1]);
                    int r = cRound(buffer[k+2]);
...
icvBGRx2ABC_8u_CnC3R(const uchar* src, int srcstep, uchar* dst, int dststep,
                      CSize size, int src_cn, int blue_idx, CvColorCvtFunc2 cvtfunc_32f,
                      int prescale, const float* post_coeffs)
{
    ...
    for(; size.height--; src += srcstep, dst += dststep)
    {
        for(i = 0; i < size.width; i += block_size)
        {
            ...
            for(k = 0; k < di*3; k += 3)
            {
                int a = cRound(buffer[k]*post_coeffs[0] + post_coeffs[1]);
                int b = cRound(buffer[k+1]*post_coeffs[2] + post_coeffs[3]);
                int c = cRound(buffer[k+2]*post_coeffs[4] + post_coeffs[5]);
                dst1[k] = CC_CAST_8U(a);
...
icvHSV2BGRx_32f_C3CnR(const float* src, int srcstep, float* dst,
                       int dststep, CSize size, int dst_cn, int blue_idx)
{
    int i;
    srcstep /= sizeof(src[0]);
    dststep /= sizeof(dst[0]);
...
icvHSV2BGRx_8u_C3CnR(const uchar* src, int srcstep, uchar* dst, int dststep,
                      CSize size, int dst_cn, int blue_idx)
{
                       ...
                       CSize size, int src_cn, int blue_idx)
{
    ...
    for(; size.height--; src += srcstep, dst += dststep)
    {
        for(i = 0; i < size.width; i += 3, src += src_cn)
        {
            ...
            {
                if(y > labT_32f)
                    L = labLScale_32f * cCbrt(y) - labLShift_32f;
                else
                    L = labLScale2_32f * y;
...
icvBayer2BGR_8u_C1C3R(const uchar* bayer0, int bayer_step,
                       ...
                       CSize size, int code)
{
    int blue = code == CC_BayerBG2BGR || code == CC_BayerGB2BGR ? -1 : 1;
    int start_with_green = code == CC_BayerGB2BGR || code == CC_BayerGR2BGR;
...
CC_IMPL img_t*
cvCvtColor(const img_t* src, img_t* dst, ColorCvtCode code)
{
    CC_FUNCNAME("cvCvtColor");
    ...
    if(type != CC_8U && type != CC_16U && type != CC_32F)
    ...
    if(CC_IS_MAT_CONT(src) && CC_IS_MAT_CONT(dst) &&
        code != CC_BayerBG2BGR && code != CC_BayerGB2BGR &&
        code != CC_BayerRG2BGR && code != CC_BayerGR2BGR) 
    {
    ...
    case CC_RGB2BGRA:
        ...
        func1 = type == CC_8U ? (CvColorCvtFunc1)icvBGR2BGRx_8u_C3C4R :
                type == CC_16U ? (CvColorCvtFunc1)icvBGR2BGRx_16u_C3C4R :
                type == CC_32F ? (CvColorCvtFunc1)icvBGR2BGRx_32f_C3C4R : 0;
        param[0] = code == CC_BGR2BGRA ? 0 : 2; // blue_idx
        break;
    ...
    case CC_RGB2BGR:
        ...
        func2 = type == CC_8U ? (CvColorCvtFunc2)icvBGRx2BGR_8u_CnC3R :
                type == CC_16U ? (CvColorCvtFunc2)icvBGRx2BGR_16u_CnC3R :
                type == CC_32F ? (CvColorCvtFunc2)icvBGRx2BGR_32f_CnC3R : 0;
        param[0] = src_cn;
        param[1] = code == CC_BGRA2BGR ? 0 : 2; // blue_idx
        break;
    ...
    case CC_BGRA2RGBA:
        ...
        func0 = type == CC_8U ? (CvColorCvtFunc0)icvBGRA2RGBA_8u_C4R :
                type == CC_16U ? (CvColorCvtFunc0)icvBGRA2RGBA_16u_C4R :
                type == CC_32F ? (CvColorCvtFunc0)icvBGRA2RGBA_32f_C4R : (CvColorCvtFunc0)0;
        break;
    ...
    case CC_RGBA2BGR555:
        ...
        param[0] = src_cn;
        param[1] = code == CC_BGR2BGR565 || code == CC_BGR2BGR555 ||
                   code == CC_BGRA2BGR565 || code == CC_BGRA2BGR555 ? 0 : 2; // blue_idx
        param[2] = code == CC_BGR2BGR565 || code == CC_RGB2BGR565 ||
                   code == CC_BGRA2BGR565 || code == CC_RGBA2BGR565 ? 6 : 5; // green_bits
        break;
    ...
    case CC_BGR5552RGBA:
        ...
        param[0] = dst_cn;
        param[1] = code == CC_BGR5652BGR || code == CC_BGR5552BGR ||
                   code == CC_BGR5652BGRA || code == CC_BGR5552BGRA ? 0 : 2; // blue_idx
        param[2] = code == CC_BGR5652BGR || code == CC_BGR5652RGB ||
                   code == CC_BGR5652BGRA || code == CC_BGR5652RGBA ? 6 : 5; // green_bits
        break;
    ...
    case CC_RGBA2GRAY:
        ...
        func2 = type == CC_8U ? (CvColorCvtFunc2)icvBGRx2Gray_8u_CnC1R :
                type == CC_16U ? (CvColorCvtFunc2)icvBGRx2Gray_16u_CnC1R :
                type == CC_32F ? (CvColorCvtFunc2)icvBGRx2Gray_32f_CnC1R : 0;
        
        param[0] = src_cn;
        param[1] = code == CC_BGR2GRAY || code == CC_BGRA2GRAY ? 0 : 2;
        break;
    ...
    case CC_BGR5552GRAY:
        ...
        param[0] = code == CC_BGR5652GRAY ? 6 : 5; // green_bits
        break;
    ...
    case CC_GRAY2BGRA:
        ...
        func1 = type == CC_8U ? (CvColorCvtFunc1)icvGray2BGRx_8u_C1CnR :
                type == CC_16U ? (CvColorCvtFunc1)icvGray2BGRx_16u_C1CnR :
                type == CC_32F ? (CvColorCvtFunc1)icvGray2BGRx_32f_C1CnR : 0;
    ...
    case CC_GRAY2BGR555:
        ...
        param[0] = code == CC_GRAY2BGR565 ? 6 : 5; // green_bits
        break;
    ...
    case CC_RGB2HLS:
        ...
        if(type == CC_8U)
            func2 = code == CC_BGR2YCrCb || code == CC_RGB2YCrCb ? (CvColorCvtFunc2)icvBGRx2YCrCb_8u_CnC3R :
                    code == CC_BGR2XYZ || code == CC_RGB2XYZ ? (CvColorCvtFunc2)icvBGRx2XYZ_8u_CnC3R :
                    code == CC_BGR2HSV || code == CC_RGB2HSV ? (CvColorCvtFunc2)icvBGRx2HSV_8u_CnC3R :
                    code == CC_BGR2Lab || code == CC_RGB2Lab ? (CvColorCvtFunc2)icvBGRx2Lab_8u_CnC3R :
                    code == CC_BGR2Luv || code == CC_RGB2Luv ? (CvColorCvtFunc2)icvBGRx2Luv_8u_CnC3R :
                    code == CC_BGR2HLS || code == CC_RGB2HLS ? (CvColorCvtFunc2)icvBGRx2HLS_8u_CnC3R : 0;
        else if(type == CC_16U)
            func2 = code == CC_BGR2YCrCb || code == CC_RGB2YCrCb ? (CvColorCvtFunc2)icvBGRx2YCrCb_16u_CnC3R :
                    code == CC_BGR2XYZ || code == CC_RGB2XYZ ? (CvColorCvtFunc2)icvBGRx2XYZ_16u_CnC3R : 0;
        else if(type == CC_32F)
            func2 = code == CC_BGR2YCrCb || code == CC_RGB2YCrCb ? (CvColorCvtFunc2)icvBGRx2YCrCb_32f_CnC3R :
                    code == CC_BGR2XYZ || code == CC_RGB2XYZ ? (CvColorCvtFunc2)icvBGRx2XYZ_32f_CnC3R :
                    code == CC_BGR2HSV || code == CC_RGB2HSV ? (CvColorCvtFunc2)icvBGRx2HSV_32f_CnC3R :
                    code == CC_BGR2Lab || code == CC_RGB2Lab ? (CvColorCvtFunc2)icvBGRx2Lab_32f_CnC3R :
                    code == CC_BGR2Luv || code == CC_RGB2Luv ? (CvColorCvtFunc2)icvBGRx2Luv_32f_CnC3R :
                    code == CC_BGR2HLS || code == CC_RGB2HLS ? (CvColorCvtFunc2)icvBGRx2HLS_32f_CnC3R : 0;
        
        param[0] = src_cn;
        param[1] = code == CC_BGR2XYZ || code == CC_BGR2YCrCb || code == CC_BGR2HSV ||
                   code == CC_BGR2Lab || code == CC_BGR2Luv || code == CC_BGR2HLS ? 0 : 2;
        break;
    ...
    case CC_HLS2RGB:
        ...
        if(type == CC_8U)
            func2 = code == CC_YCrCb2BGR || code == CC_YCrCb2RGB ? (CvColorCvtFunc2)icvYCrCb2BGRx_8u_C3CnR :
                    code == CC_XYZ2BGR || code == CC_XYZ2RGB ? (CvColorCvtFunc2)icvXYZ2BGRx_8u_C3CnR :
                    code == CC_HSV2BGR || code == CC_HSV2RGB ? (CvColorCvtFunc2)icvHSV2BGRx_8u_C3CnR :
                    code == CC_HLS2BGR || code == CC_HLS2RGB ? (CvColorCvtFunc2)icvHLS2BGRx_8u_C3CnR :
                    code == CC_Lab2BGR || code == CC_Lab2RGB ? (CvColorCvtFunc2)icvLab2BGRx_8u_C3CnR :
                    code == CC_Luv2BGR || code == CC_Luv2RGB ? (CvColorCvtFunc2)icvLuv2BGRx_8u_C3CnR : 0;
        else if(type == CC_16U)
            func2 = code == CC_YCrCb2BGR || code == CC_YCrCb2RGB ? (CvColorCvtFunc2)icvYCrCb2BGRx_16u_C3CnR :
                    code == CC_XYZ2BGR || code == CC_XYZ2RGB ? (CvColorCvtFunc2)icvXYZ2BGRx_16u_C3CnR : 0;
        else if(type == CC_32F)
            func2 = code == CC_YCrCb2BGR || code == CC_YCrCb2RGB ? (CvColorCvtFunc2)icvYCrCb2BGRx_32f_C3CnR :
                    code == CC_XYZ2BGR || code == CC_XYZ2RGB ? (CvColorCvtFunc2)icvXYZ2BGRx_32f_C3CnR :
                    code == CC_HSV2BGR || code == CC_HSV2RGB ? (CvColorCvtFunc2)icvHSV2BGRx_32f_C3CnR :
                    code == CC_HLS2BGR || code == CC_HLS2RGB ? (CvColorCvtFunc2)icvHLS2BGRx_32f_C3CnR :
                    code == CC_Lab2BGR || code == CC_Lab2RGB ? (CvColorCvtFunc2)icvLab2BGRx_32f_C3CnR :
                    code == CC_Luv2BGR || code == CC_Luv2RGB ? (CvColorCvtFunc2)icvLuv2BGRx_32f_C3CnR : 0;
        
        param[0] = dst_cn;
        param[1] = code == CC_XYZ2BGR || code == CC_YCrCb2BGR || code == CC_HSV2BGR ||
                   code == CC_Lab2BGR || code == CC_Luv2BGR || code == CC_HLS2BGR ? 0 : 2;
        break;
    ...
    if(func0)
    {
        IPPI_CALL(func0(src->tt.data, src_step, dst->tt.data, dst_step, size));
    }
    else if(func1)
    {
        IPPI_CALL(func1(src->tt.data, src_step,
            dst->tt.data, dst_step, size, param[0]));
    }
    else if(func2)
    {
        IPPI_CALL(func2(src->tt.data, src_step,
            dst->tt.data, dst_step, size, param[0], param[1]));
    }
    else if(func3)
    {
        IPPI_CALL(func3(src->tt.data, src_step,
            dst->tt.data, dst_step, size, param[0], param[1], param[2]));
    }
    else
        CC_ERROR(CC_StsUnsupportedFormat, "The image format is not supported");
