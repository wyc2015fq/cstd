...
static CC_IMPLEMENT_QSORT_EX(icvSortDblPtr2, double*, CC_CMP_NUM_PTR, int)

#define CC_CMP_PAIRS(a,b) ((a).val < (b).val)
static CC_IMPLEMENT_QSORT_EX(icvSortPairs, CvPair32s32f, CC_CMP_PAIRS, int)
    ...
    bool _shared, bool _add_labels, bool _update_data)
{
    ...
    if(_sample_idx)
    {
        CC_CALL(sample_idx = cvPreprocessIndexArray(_sample_idx, sample_all));
    ...
    CC_CALL(var_type0 = cvPreprocessVarType(_var_type, var_idx, var_all, &r_type));
    ...
    {
        var_type->tt.i[vi] = var_type0->tt.data[vi] == CC_VAR_CATEGORICAL ?
    ...
    CC_CALL(buf = cvCreateMat(buf_count, buf_size, CC_32SC1));
    CC_CALL(cat_count = cvCreateMat(1, cat_var_count+1, CC_32SC1));
    CC_CALL(cat_ofs = cvCreateMat(1, cat_count->cols+1, CC_32SC1));
    CC_CALL(cat_map = cvCreateMat(1, cat_count->cols*10 + 128, CC_32SC1));
    ...
    for(vi = 0; vi <= var_count; vi++)
    {
             ...
             // it will be stored similarly to categorical vars (i.e. no pairs)
        {
            ...
            cat_count->tt.i[cat_var_count] = 0;
            cat_ofs->tt.i[cat_var_count] = total_c_count;
...
CvDTreeNode* CvDTreeTrainData::subsample_data(const img_t* _subsample_idx)
{
    ...
    else
    {
        ...
        CC_CALL(subsample_co = cvCreateMat(1, total*2, CC_32SC1));
        cvZero(subsample_co);
...
CvPair32s32f* CvDTreeTrainData::get_ord_var_data(CvDTreeNode* n, int vi)
{
    ...
    return (CvPair32s32f*)(buf->tt.i + n->buf_idx*buf->cols +
...
int* CvDTreeTrainData::get_cat_var_data(CvDTreeNode* n, int vi)
{
    ...
    return buf->tt.i + n->buf_idx*buf->cols + n->offset +
    ...
    const img_t* _missing, bool preprocessed_input) const
{
    ...
    sample = _sample->tt.fl;
    step = CC_IS_MAT_CONT(_sample->tid) ? 1 : _sample->step/sizeof(sample[0]);
    ...
    vidx = data->var_idx && !preprocessed_input ? data->var_idx->tt.i : 0;
    cmap = data->cat_map ? data->cat_map->tt.i : 0;
    cofs = data->cat_ofs ? data->cat_ofs->tt.i : 0;
