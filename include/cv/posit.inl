...
static  CStatus  icvCreatePOSITObject(CPoint3D32f *points,
                                        int numPoints,
                                        CvPOSITObject **ppObject)
{
    ...
    /* memory allocation */
    pObject = (CvPOSITObject *) cAlloc(sizeof(CvPOSITObject) +
...
static  CStatus  icvPOSIT(CvPOSITObject *pObject, CPoint2D32f *imagePoints,
                            float focalLength, CTermCriteria criteria,
                            CvMatr32f rotation, CvVect32f translation)
{
    ...
    if(!translation)
        return CC_NULLPTR_ERR;
    if((criteria.type == 0) || (criteria.type > (CC_TERMCRIT_ITER | CC_TERMCRIT_EPS)))
        return CC_BADFLAG_ERR;
    if((criteria.type & CC_TERMCRIT_EPS) && criteria.epsilon < 0)
        return CC_BADFACTOR_ERR;
    if((criteria.type & CC_TERMCRIT_ITER) && criteria.max_iter <= 0)
        return CC_BADFACTOR_ERR;
    ...
    while(!converged)
    {
        ...
        count++;
        converged = ((criteria.type & CC_TERMCRIT_EPS) && (diff < criteria.epsilon));
        converged |= ((criteria.type & CC_TERMCRIT_ITER) && (count == criteria.max_iter));
    }
    invScale = 1 / scale;
...
static  CStatus  icvReleasePOSITObject(CvPOSITObject ** ppObject)
{
    cFree(ppObject);
    return CC_NO_ERR;
}
...
CC_IMPL CvPOSITObject *
cvCreatePOSITObject(CPoint3D32f * points, int numPoints)
{
    CvPOSITObject *pObject = 0;

    CC_FUNCNAME("cvCreatePOSITObject");
...
CC_IMPL void
cvPOSIT(CvPOSITObject * pObject, CPoint2D32f * imagePoints,
         double focalLength, CTermCriteria criteria,
         CvMatr32f rotation, CvVect32f translation)
{
    CC_FUNCNAME("cvPOSIT");
...
{
    CC_FUNCNAME("cvReleasePOSITObject");
