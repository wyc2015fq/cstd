...
class CC_EXPORTS CvImage
{
public:
    CvImage() : image(0), refcount(0) {}
    CvImage(CvSize size, TypeId type, int channels)
    {
    ...
    void create(CvSize size, TypeId type, int channels)
    {
    ...
    TypeId type() const { return image ? image->depth : 0; }
...
public:
    ...
    CvMatrix(int rows, int cols, int type, CvMat* hdr,
              void* data=0, int step=CC_AUTOSTEP)
    ...
    CvMatrix(int rows, int cols, int type, void* data, int step=CC_AUTOSTEP)
    ...
    int type() const { return matrix ? CC_MAT_TYPE(matrix->tid) : 0; }
    TypeId type() const { return matrix ? CC_MAT_TYPE(matrix->tid) : 0; }
    int channels() const { return matrix ? CC_MAT_CN(matrix->tid) : 0; }
    int pix_size() const { return matrix ? CC_ELEM_SIZE(matrix->tid) : 0; }

    uchar* data() { return matrix ? matrix->tt.data : 0; }
    const uchar* data() const { return matrix ? matrix->tt.data : 0; }
    int step() const { return matrix ? matrix->step : 0; }

    void set_data(void* data, int step=CC_AUTOSTEP)
    { cvSetData(matrix, data, step); }

    uchar* row(int i) { return !matrix ? 0 : matrix->tt.data + i*matrix->step; }
    const uchar* row(int i) const
    { return !matrix ? 0 : matrix->tt.data + i*matrix->step; }
...
typedef IplImage* (C_CDECL * CvLoadImageFunc)(const char* filename, int colorness);
typedef CvMat* (C_CDECL * CvLoadImageMFunc)(const char* filename, int colorness);
typedef int (C_CDECL * CvSaveImageFunc)(const char* filename, const CvArr* image);
typedef void (C_CDECL * CvShowImageFunc)(const char* windowname, const CvArr* image);
...
#define CC_SET_IMAGE_IO_FUNCTIONS() \
