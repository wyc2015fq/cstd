...
protected:
    static const CvSize m_imageSize;
    ...
    std::vector<Vec3d> m_fovCenter;
    /** normal of the field of view, near and far plane */
    std::vector<Vec3d> m_fovNormal;
    /** points on a plane calibration target */
    std::vector<Point3d> m_pointTarget;
    /** rotations for the calibration target */
    std::vector<Vec3d> m_pointTargetRvec;
    /** translations for the calibration target */
    std::vector<Vec3d> m_pointTargetTvec;
    /** camera matrix */
    Matx33d m_cameraMatrix;
    /** distortion coefficients */
    Vec<double, NUM_DIST_COEFF_TILT> m_distortionCoeff;
    ...
    */
    void randomDistortionCoeff(
        Vec<double, NUM_DIST_COEFF_TILT>& coeff,
        const Vec<double, NUM_DIST_COEFF_TILT>& max)
    {
    ...
    void numericalDerivative(
        CvMat& jac,
        double eps,
        const std::vector<Point3d>& obj,
        const Vec3d& rvec,
        const Vec3d& tvec,
        const Matx33d& camera,
        const Vec<double, NUM_DIST_COEFF_TILT>& distor);
    ...
    void addNoiseRemoveInvalidPoints(
        ...
        std::vector<Point2f>& noisyImagePoints,
        double halfWidthNoise);
};

/** Number of Pixel of the sensor */
const CvSize cameraCalibrationTiltTest::m_imageSize(1600, 1200);
/** CvSize of a pixel in mm */
...
void cameraCalibrationTiltTest::SetUp()
{
    ...
    // view
    std::vector<Vec3d> fovBottomTop(6);
    std::vector<Vec3d>::iterator itFov = fovBottomTop.begin();
    for (size_t iBottomTop = 0; iBottomTop < 2; ++iBottomTop)
    {
        // mapping sensor to field of view
        *itFov = Vec3d(0,yImage[iBottomTop],zImage[iBottomTop]);
    ...
    for (size_t i = 0; i < 3; ++i)
    {
        ...
        m_fovNormal[i] = normalize(m_fovNormal[i]);
        m_fovNormal[i] = Vec3d(
            m_fovNormal[i](0),
            -m_fovNormal[i](2),
            m_fovNormal[i](1));
        ...
        Vec3d rvec = Vec3d(0,0,1).cross(m_fovNormal[i]);
        rvec = normalize(rvec);
    ...
    angle.push_back(maxAngle);
    Matx33d baseMatrix;
    Rodrigues(m_pointTargetRvec.front(), baseMatrix);
    ...
    m_cameraMatrix = Matx33d(
        f,0,cx,
        0,f,cy,
        0,0,1);
    // distortion coefficients
    m_distortionCoeff = Vec<double, NUM_DIST_COEFF_TILT>::all(0);
    // tauX
    ...
    const Vec<double, NUM_DIST_COEFF_TILT>& distor)
{
    ...
    Vec3d t(tvec);
    Matx33d cm(camera);
    Vec<double, NUM_DIST_COEFF_TILT> dc(distor);
    ...
    for (int col = 0; col < 10+NUM_DIST_COEFF_TILT; ++col)
    {
        ...
        *(param[col]) = save + eps;
        projectPoints(obj, r, t, cm, dc, pix0);
        *(param[col]) = save - eps;
        projectPoints(obj, r, t, cm, dc, pix1);
        *(param[col]) = save;

        std::vector<Point2d>::const_iterator it0 = pix0.begin();
        std::vector<Point2d>::const_iterator it1 = pix1.begin();
        int row = 0;
        for (;it0 != pix0.end(); ++it0, ++it1)
        {
            Point2d d = invEps*(*it0 - *it1);
    ...
    std::vector<Point3d>& objectPoints)
{
    ...
    std::vector<Point2d>::iterator itImg = imagePoints.begin();
    std::vector<Point3d>::iterator itObj = objectPoints.begin();
...
void cameraCalibrationTiltTest::addNoiseRemoveInvalidPoints(
    ...
    double halfWidthNoise)
{
    std::vector<Point2f>::iterator itImg = imagePoints.begin();
    std::vector<Point3f>::iterator itObj = objectPoints.begin();
    noisyImagePoints.clear();
    noisyImagePoints.reserve(imagePoints.size());
    while (itImg != imagePoints.end())
    {
        Point2f pix = *itImg + Point2f(
...
{
    std::vector<Point2d> imagePoints;
    std::vector<Point3d> objectPoints = m_pointTarget;
    Vec3d rvec = m_pointTargetRvec.front();
    Vec3d tvec = m_pointTargetTvec.front();

    Vec<double, NUM_DIST_COEFF_TILT> coeffNoiseHalfWidth(
        .1, .1, // k1 k2
        .01, .01, // p1 p2
    ...
    for (size_t numTest = 0; numTest < 10; ++numTest)
    {
        ...
        Vec<double, NUM_DIST_COEFF_TILT> distortionCoeff = m_distortionCoeff;
        ...
        CvMat jacobian(2*numPoints, numParams, CC_64FC1);
        ...
        CvMat numericJacobian(2*numPoints, numParams, CC_64FC1);
        double eps = 1e-7;
        numericalDerivative(
            numericJacobian,
            eps,
        ...
        CvMat check = abs(jacobian(Range::all(), Range(0,6)) - numericJacobian(Range::all(), Range(0,6)))/
            (1 + abs(jacobian(Range::all(), Range(0,6))));
        double minVal, maxVal;
        minMaxIdx(check, &minVal, &maxVal);
        EXPECT_LE(maxVal, .01);
        // absolute difference for distortion and camera matrix
        EXPECT_MAT_NEAR(jacobian(Range::all(), Range(6,numParams)), numericJacobian(Range::all(), Range(6,numParams)), 1e-5);
    }
}
    ...
    for (size_t numTest = 0; numTest < 10; ++numTest)
    {
        ...
        for (double x = 0; x <= m_imageSize.width-1; x += step)
            for (double y = 0; y <= m_imageSize.height-1; y += step)
                distorted.push_back(Point2d(x,y));
        std::vector<Point2d> normalizedUndistorted;

        // undistort
        undistortPoints(distorted,
        ...
        std::vector<Point3d> objectPoints;
        for (std::vector<Point2d>::const_iterator itPnt = normalizedUndistorted.begin();
            itPnt != normalizedUndistorted.end(); ++itPnt)
            objectPoints.push_back(Point3d(itPnt->x, itPnt->y, 1));

        // project
        std::vector<Point2d> imagePoints(objectPoints.size());
...
void showVec(const std::string& name, const INPUT& in, const CvMat& est)
{
...
TEST_F(cameraCalibrationTiltTest, calibrateCamera)
{
    ...
    std::vector<Point3f> pointTarget;
    pointTarget.reserve(m_pointTarget.size());
    for (std::vector<Point3d>::const_iterator it = m_pointTarget.begin(); it != m_pointTarget.end(); ++it)
        pointTarget.push_back(Point3f(
        (float)(it->x),
        (float)(it->y),
        (float)(it->z)));
    ...
    for (size_t numTest = 0; numTest < 5; ++numTest)
    {
        ...
        std::vector<std::vector<Point3f> > viewsObjectPoints;
        std::vector<std::vector<Point2f> > viewsImagePoints;
        std::vector<std::vector<Point2f> > viewsNoisyImagePoints;

        // simulate calibration data with projectPoints
        std::vector<Vec3d>::const_iterator itRvec = m_pointTargetRvec.begin();
        std::vector<Vec3d>::const_iterator itTvec = m_pointTargetTvec.begin();
        // loop over different views
        for (;itRvec != m_pointTargetRvec.end(); ++ itRvec, ++itTvec)
        {
            std::vector<Point3f> objectPoints(pointTarget);
            std::vector<Point2f> imagePoints;
            std::vector<Point2f> noisyImagePoints;
        ...
        // Output
        std::vector<CvMat> outRvecs, outTvecs;
        CvMat outCameraMatrix(3, 3, CC_64F, Scalar::all(1)), outDistCoeff;
        ...
        int flag =
            ...
            CALIB_TILTED_MODEL;
        // estimate
        double backProjErr = calibrateCamera(
            viewsObjectPoints,
        ...
        if (pixelNoiseHalfWidth > 0)
        {
            ...
            for (size_t i = 0; i < viewsNoisyImagePoints.size(); ++i)
            {
                ...
                std::vector<Point2f> backProjection;
                projectPoints(
                    viewsObjectPoints[i],
                    outRvecs[i],
                    outTvecs[i],
