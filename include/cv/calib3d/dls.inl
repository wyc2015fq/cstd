...
dls::dls(const img_t& opoints, const img_t& ipoints)
{

    N =  MAX(opoints cvCheckVector(3, CC_32F), opoints cvCheckVector(3, CC_64F));
    p = img_t(3, N, CC_64F);
    z = img_t(3, N, CC_64F);
    mn = img_t::zeros(3, 1, CC_64F);
    ...
    if (opoints CC_MAT_DEPTH() == ipoints CC_MAT_DEPTH())
    {
        if (opoints CC_MAT_DEPTH() == CC_32F)
            init_points<CPoint3f, CPoint2f>(opoints, ipoints);
        else
            init_points<CPoint3d, CPoint2d>(opoints, ipoints);
    }
    else if (opoints CC_MAT_DEPTH() == CC_32F)
        init_points<CPoint3f, CPoint2d>(opoints, ipoints);
    else
        init_points<CPoint3d, CPoint2f>(opoints, ipoints);
}
...
bool dls::compute_pose(img_t& R, img_t& t)
{

    std::vector<img_t> R_;
    R_.push_back(rotx(CC_PI/2));
    R_.push_back(roty(CC_PI/2));
    R_.push_back(rotz(CC_PI/2));

    // version that calls dls 3 times, to avoid Cayley singularity
    for (int i = 0; i < 3; ++i)
    {
        // Make a random rotation
        img_t pp = R_[i] * ( p - repeat(mn, 1, p->cols) );
...
void dls::run_kernel(const img_t& pp)
{
    img_t Mtilde(27, 27, CC_64F);
    img_t D = img_t::zeros(9, 9, CC_64F);
    build_coeff_matrix(pp, Mtilde, D);

    img_t eigenval_r, eigenval_i, eigenvec_r, eigenvec_i;
    ...
    img_t sols = img_t::zeros(3, 27, CC_64F);
    std::vector<double> cost;
    int count = 0;
    for (int k = 0; k < 27; ++k)
    {
        //  V(:,k) = V(:,k)/V(1,k);
        img_t V_kA = eigenvec_r.col(k); // 27x1
        img_t V_kB = img_t(1, 1, z CC_MAT_DEPTH(), V_kA.at<double>(0)); // 1x1
        img_t V_k; solve(V_kB.t(), V_kA.t(), V_k); // A/B = B'\A'
        img_t( V_k.t()).copyTo( eigenvec_r.col(k) );
...
#endif
        {
            ...
            img_t H = Hessian(stmp);

            img_t eigenvalues, eigenvectors;
            ...
            if(positive_eigenvalues(&eigenvalues))
            {
                ...
                // sols(:,i) = stmp;
                img_t stmp_mat(3, 1, CC_64F, &stmp);

                stmp_mat.copyTo( sols.col(count) );

                img_t Cbar = cayley2rotbar(stmp_mat);
                img_t Cbarvec = Cbar cvReshape(,1,1).t();

                // cost(i) = CbarVec' * D * CbarVec;
                img_t cost_mat =  Cbarvec.t() * D * Cbarvec;
    ...
    std::vector<img_t> C_est, t_est;
    for (int j = 0; j < sols->cols; ++j)
    {
        // recover the optimal orientation
        // C_est(:,:,j) = 1/(1 + sols(:,j)' * sols(:,j)) * cayley2rotbar(sols(:,j));

        img_t sols_j = sols.col(j);
        double sols_mult = 1./(1.+img_t( sols_j.t() * sols_j ).at<double>(0));
        img_t C_est_j = cayley2rotbar(sols_j).mul(sols_mult);
        C_est.push_back( C_est_j );

        img_t A2 = img_t::zeros(3, 3, CC_64F);
        img_t b2 = img_t::zeros(3, 1, CC_64F);
        for (int i = 0; i < N; ++i)
        {
            img_t eye = img_t::eye(3, 3, CC_64F);
            img_t z_mul = z.col(i)*z.col(i).t();
    ...
    {
        img_t cam_points = C_est[k] * pp + repeat(t_est[k], 1, pp->cols);
        img_t cam_points_k = cam_points.row(2);

        if(is_empty(&cam_points_k))
        {
            img_t C_valid = C_est[k], t_valid = t_est[k];
...
void dls::build_coeff_matrix(const img_t& pp, img_t& Mtilde, img_t& D)
{
    img_t eye = img_t::eye(3, 3, CC_64F);
    ...
    img_t H = img_t::zeros(3, 3, CC_64F);
    img_t A = img_t::zeros(3, 9, CC_64F);
    img_t pp_i(3, 1, CC_64F);

    img_t z_i(3, 1, CC_64F);
    for (int i = 0; i < N; ++i)
    {
        z.col(i).copyTo(z_i);
    ...
    // A\B
    solve(H, A, A, CC_DECOMP_NORMAL);
    H.release();

    img_t ppi_A(3, 1, CC_64F);
    for (int i = 0; i < N; ++i)
    {
        z.col(i).copyTo(z_i);
    ...
    img_t M2 = cayley_LS_M(f1coeff, f2coeff, f3coeff, u);

    img_t M2_1 = M2(CRange(0,27), CRange(0,27));
    img_t M2_2 = M2(CRange(0,27), CRange(27,120));
    img_t M2_3 = M2(CRange(27,120), CRange(27,120));
    img_t M2_4 = M2(CRange(27,120), CRange(0,27));
    M2.release();

    // A/B = B'\A'
    img_t M2_5; solve(M2_3.t(), M2_2.t(), M2_5);
...
void dls::compute_eigenvec(const img_t& Mtilde, img_t& eigenval_real, img_t& eigenval_imag,
                                                  img_t& eigenvec_real, img_t& eigenvec_imag)
{
#ifdef HAVE_EIGEN
    Eigen::MatrixXd Mtilde_eig, zeros_eig;
    cv2eigen(Mtilde, Mtilde_eig);
    cv2eigen(img_t::zeros(27, 27, CC_64F), zeros_eig);
    ...
    eigenval_imag = eigenvec_imag = img_t();
#endif

}

void dls::fill_coeff(const img_t * D_mat)
{
...
img_t dls::LeftMultVec(const img_t& v)
{
    img_t mat_ = img_t::zeros(3, 9, CC_64F);
...
img_t dls::cayley_LS_M(const std::vector<double>& a, const std::vector<double>& b, const std::vector<double>& c, const std::vector<double>& u)
{
    // TODO: input matrix pointer
    // TODO: shift coefficients one position to left

    img_t M = img_t::zeros(120, 120, CC_64F);
...
img_t dls::Hessian(const double s[])
{
    ...
    // fill Hessian matrix
    img_t H(3, 3, CC_64F);
    H.at<double>(0,0) = img_t(img_t(f1coeff) cvGetRows(,1,21).t()*img_t(20, 1, CC_64F, &Hs1)).at<double>(0,0);
    H.at<double>(0,1) = img_t(img_t(f1coeff) cvGetRows(,1,21).t()*img_t(20, 1, CC_64F, &Hs2)).at<double>(0,0);
    H.at<double>(0,2) = img_t(img_t(f1coeff) cvGetRows(,1,21).t()*img_t(20, 1, CC_64F, &Hs3)).at<double>(0,0);

    H.at<double>(1,0) = img_t(img_t(f2coeff) cvGetRows(,1,21).t()*img_t(20, 1, CC_64F, &Hs1)).at<double>(0,0);
    H.at<double>(1,1) = img_t(img_t(f2coeff) cvGetRows(,1,21).t()*img_t(20, 1, CC_64F, &Hs2)).at<double>(0,0);
    H.at<double>(1,2) = img_t(img_t(f2coeff) cvGetRows(,1,21).t()*img_t(20, 1, CC_64F, &Hs3)).at<double>(0,0);

    H.at<double>(2,0) = img_t(img_t(f3coeff) cvGetRows(,1,21).t()*img_t(20, 1, CC_64F, &Hs1)).at<double>(0,0);
    H.at<double>(2,1) = img_t(img_t(f3coeff) cvGetRows(,1,21).t()*img_t(20, 1, CC_64F, &Hs2)).at<double>(0,0);
    H.at<double>(2,2) = img_t(img_t(f3coeff) cvGetRows(,1,21).t()*img_t(20, 1, CC_64F, &Hs3)).at<double>(0,0);

    return H;
}

img_t dls::cayley2rotbar(const img_t& s)
{
    double s_mul1 = img_t(s.t()*s).at<double>(0,0);
    img_t s_mul2 = s*s.t();
    img_t eye = img_t::eye(3, 3, CC_64F);

    return img_t( eye.mul(1.-s_mul1) + skewsymm(&s).mul(2.) + s_mul2.mul(2.) ).t();
}

img_t dls::skewsymm(const img_t * X1)
{
...
img_t dls::mean(const img_t& M)
{
    img_t m = img_t::zeros(3, 1, CC_64F);
    for (int i = 0; i < M->cols; ++i) m += M.col(i);
    return m.mul(1./(double)M->cols);
}

bool dls::is_empty(const img_t * M)
{
...
bool dls::positive_eigenvalues(const img_t * eigenvalues)
{
