...
{
public:
    dls(const img_t& opoints, const img_t& ipoints);
    ~dls();

    bool compute_pose(img_t& R, img_t& t);
...
private:
    ...
    // main algorithm
    img_t LeftMultVec(const img_t& v);
    void run_kernel(const img_t& pp);
    void build_coeff_matrix(const img_t& pp, img_t& Mtilde, img_t& D);
    void compute_eigenvec(const img_t& Mtilde, img_t& eigenval_real, img_t& eigenval_imag,
                                                 img_t& eigenvec_real, img_t& eigenvec_imag);
    void fill_coeff(const img_t * D);

    // useful functions
    img_t cayley_LS_M(const std::vector<double>& a, const std::vector<double>& b,
                        const std::vector<double>& c, const std::vector<double>& u);
    img_t Hessian(const double s[]);
    img_t cayley2rotbar(const img_t& s);
    img_t skewsymm(const img_t * X1);

    // extra functions
    img_t rotx(const double t);
    img_t roty(const double t);
    img_t rotz(const double t);
    img_t mean(const img_t& M);
    bool is_empty(const img_t * v);
    bool positive_eigenvalues(const img_t * eigenvalues);

    img_t p, z, mn;        // object-image points
    int N;                // number of input points
    std::vector<double> f1coeff, f2coeff, f3coeff, cost_; // coefficient for coefficients matrix
    std::vector<img_t> C_est_, t_est_;    // optimal candidates
    img_t C_est__, t_est__;                // optimal found solution
