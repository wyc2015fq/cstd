...
/* For C support, replace with extern "C" { */
#undef HSIZE
/* Constants */
...
struct RHO_HEST_REFC : RHO_HEST{
    ...
    struct nr_t{
        ...
        unsigned  size;            /* Non-Randomness: CSize */
...
static int  rhoEnsureCapacity(RHO_HEST* p, unsigned N, double beta){
...
static void rhoSeed(RHO_HEST* p, uint64_t seed){
    p->fastSeed(seed);
}
...
static unsigned
rhoHest(RHO_HEST* p,       /* Homography estimation context. */
...
inline int    RHO_HEST_REFC::initialize(void){
    ...
    int areAllAllocsSuccessful = !imempty(&mem.perObj);
...
#define MK_OFFSET(v)                                     \
    size_t v ## _of = total;                             \
    total = cAlignSize(v ## _of  +  v ## _sz, MEM_ALIGN)
...
#undef MK_OFFSET
    ...
    /* Allocate dynamic memory managed by img_t */
    cvSetMat(&mem.perObj, 1, (int)(total + MEM_ALIGN), CC_8U);

    /* Extract aligned pointer */
    unsigned char* ptr = cAlignPtr(unsigned char, mem.perObj.data, MEM_ALIGN);
...
#undef MK_OFFSET
    ...
    /* Allocate dynamic memory managed by img_t */
    cvSetMat(&mem.perRun, 1, (int)(total + MEM_ALIGN), CC_8U);

    /* Extract aligned pointer */
    unsigned char* ptr = cAlignPtr(unsigned char, mem.perRun.data, MEM_ALIGN);
