...
#define ICC_MATH_BLOCK_SIZE  256

#define _CC_SQRT_MAGIC     0xbe6f0000

#define _CC_SQRT_MAGIC_DBL CC_BIG_UINT(0xbfcd460000000000)

#define _CC_ATAN_CF0  (-15.8131890796f)
#define _CC_ATAN_CF1  (61.0941945596f)
#define _CC_ATAN_CF2  0.f /*(-0.140500406322f)*/

static const float icvAtanTab[8] = { 0.f + _CC_ATAN_CF2, 90.f - _CC_ATAN_CF2,
    180.f - _CC_ATAN_CF2, 90.f + _CC_ATAN_CF2,
    360.f - _CC_ATAN_CF2, 270.f + _CC_ATAN_CF2,
    180.f + _CC_ATAN_CF2, 270.f - _CC_ATAN_CF2
};

static const uint icvAtanSign[8] =
    { 0, 0x80000000, 0x80000000, 0, 0x80000000, 0, 0, 0x80000000 };

CC_IMPL float
cFastArctan(float y, float x)
{
    suf32_t _x, _y;
    int ix, iy, ygx, idx;
    double z;
    ...
    /* ix = ix != 0 ? ix : 1.f */
    _x.i = ((ix ^ CC_1F) & ((ix == 0) - 1)) ^ CC_1F;
    
    z = _y.f / _x.f;
    return (float)((_CC_ATAN_CF0*fabs(z) + _CC_ATAN_CF1)*z + icvAtanTab[idx]);
}


IPCVAPI_IMPL(CStatus, icvFastArctan_32f,
    (const float *__y, const float *__x, float *angle, int len), (__y, __x, angle, len))
{
    ...
    for(; i <= len - 4; i += 4)
    {
...

        {
            ...
            z0 = (float)((_CC_ATAN_CF0*fabs(z0) + _CC_ATAN_CF1)*z0 + icvAtanTab[idx[0]]);
            z1 = (float)((_CC_ATAN_CF0*fabs(z1) + _CC_ATAN_CF1)*z1 + icvAtanTab[idx[1]]);
            z2 = (float)((_CC_ATAN_CF0*fabs(z2) + _CC_ATAN_CF1)*z2 + icvAtanTab[idx[2]]);
            z3 = (float)((_CC_ATAN_CF0*fabs(z3) + _CC_ATAN_CF1)*z3 + icvAtanTab[idx[3]]);
    ...
    /* process the rest */
    for(; i < len; i++)
        angle[i] = cFastArctan(__y[i], __x[i]);
...
\* ************************************************************************** */
CC_IMPL  float  cCbrt(float value)
{
    float fr;
    suf32_t v, m;
    int ix, s;
...
IPCVAPI_IMPL(CStatus, icvInvSqrt_32f, (const float *src, float *dst, int len), (src, dst, len))
{
    int i = 0;
...
IPCVAPI_IMPL(CStatus, icvSqrt_32f, (const float *src, float *dst, int len), (src, dst, len))
{
    int i = 0;
...
IPCVAPI_IMPL(CStatus, icvSqrt_64f, (const double *src, double *dst, int len), (src, dst, len))
{
    int i = 0;
...
IPCVAPI_IMPL(CStatus, icvInvSqrt_64f, (const double *src, double *dst, int len), (src, dst, len))
{
    int i = 0;
...
#define ICC_DEF_SQR_MAGNITUDE_FUNC(flavor, arrtype, magtype)\
...
ICC_DEF_SQR_MAGNITUDE_FUNC(32f, float, float)
ICC_DEF_SQR_MAGNITUDE_FUNC(64f, double, double)
...
CC_IMPL void
cvCartToPolar(const img_t* xmat, const img_t* ymat,
               img_t* mag, img_t* angle CC_DEFAULT(NULL),
               int angle_in_degrees CC_DEFAULT(0))
{
    CC_FUNCNAME("cvCartToPolar");
    ...
    if(!CC_ARE_TYPES_EQ(xmat, ymat))
        CC_ERROR_FROM_CODE(CC_StsUnmatchedFormats);

    if(!CC_ARE_SIZES_EQ(xmat, ymat))
        CC_ERROR_FROM_CODE(CC_StsUnmatchedSizes);

    type = CC_MAT_DEPTH(xmat);
    if(type < CC_32F)
        CC_ERROR(CC_StsUnsupportedFormat, "");

    if(mag)
    {
        if(!CC_ARE_TYPES_EQ(mag, xmat))
            CC_ERROR_FROM_CODE(CC_StsUnmatchedFormats);
        
        if(!CC_ARE_SIZES_EQ(mag, xmat))
            CC_ERROR_FROM_CODE(CC_StsUnmatchedSizes);
        cont_flag = CC_IS_MAT_CONT(mag);
    }

    if(angle)
    {
        if(!CC_ARE_TYPES_EQ(angle, xmat))
            CC_ERROR_FROM_CODE(CC_StsUnmatchedFormats);
        
        if(!CC_ARE_SIZES_EQ(angle, xmat))
            CC_ERROR_FROM_CODE(CC_StsUnmatchedSizes);
        cont_flag &= CC_IS_MAT_CONT(angle);
    }
    ...
    size.width *= CC_MAT_CN(xmat);

    if(CC_IS_MAT_CONT(xmat) && CC_IS_MAT_CONT(ymat) && cont_flag)
    {
        size.width *= size.height;
        size.height = 1;
    }

    block_size = MIN(size.width, ICC_MATH_BLOCK_SIZE);
    if(type == CC_64F && angle)
    {
        x_buffer = (float*)cvStackAlloc(block_size*sizeof(float));
        y_buffer = (float*)cvStackAlloc(block_size*sizeof(float));
    }
    else if(type == CC_32F && mag)
    {
    ...
    if(type == CC_32F)
    {
        ...
        {
            float* x_data = (float*)(xmat->tt.data + xmat->step*y);
            float* y_data = (float*)(ymat->tt.data + ymat->step*y);
            float* mag_data = mag ? (float*)(mag->tt.data + mag->step*y) : 0;
            float* angle_data = angle ? (float*)(angle->tt.data + angle->step*y) : 0;
    ...
    else
    {
        for(y = 0; y < size.height; y++)
        {
            double* x_data = (double*)(xmat->tt.data + xmat->step*y);
            double* y_data = (double*)(ymat->tt.data + ymat->step*y);
            double* mag_data = mag ? (double*)(mag->tt.data + mag->step*y) : 0;
            double* angle_data = angle ? (double*)(angle->tt.data + angle->step*y) : 0;
            ...
            for(x = 0; x < size.width; x += block_size)
            {
                ...
                if(angle)
                {
                    ...
                    if(!angle_in_degrees)
                        icvScale_32f(x_buffer, x_buffer, len, (float)(CC_PI/180.), 0);
                ...
                int len, int angle_in_degrees)
{
    ...
    enum { N = 64 };
#define k2      (2*CC_PI/N)
#define sin_a0  (-0.166630293345647*k2*k2*k2)
#define sin_a2   k2
#define cos_a0  (-0.499818138450326*k2*k2)
...
CC_IMPL void
cvPolarToCart(const img_t* mag, const img_t* angle,
               img_t* xmat, img_t* ymat, int angle_in_degrees)
{
    CC_FUNCNAME("cvPolarToCart");
    ...
    if(mag)
    {
        if(!CC_ARE_TYPES_EQ(angle, mag))
            CC_ERROR_FROM_CODE(CC_StsUnmatchedFormats);

        if(!CC_ARE_SIZES_EQ(angle, mag))
            CC_ERROR_FROM_CODE(CC_StsUnmatchedSizes);
    ...
    if(xmat)
    {
        ...
        cont_flag &= CC_IS_MAT_CONT(xmat);
    }

    if(ymat)
    {
        if(!CC_ARE_TYPES_EQ(angle, ymat))
            CC_ERROR_FROM_CODE(CC_StsUnmatchedFormats);
        
        if(!CC_ARE_SIZES_EQ(angle, ymat))
            CC_ERROR_FROM_CODE(CC_StsUnmatchedSizes);
        
        cont_flag &= CC_IS_MAT_CONT(ymat);
    }
    ...
    if(type == CC_32F)
    {
        for(y = 0; y < size.height; y++)
        {
            float* x_data = (float*)(xmat ? xmat->tt.data + xmat->step*y : 0);
            float* y_data = (float*)(ymat ? ymat->tt.data + ymat->step*y : 0);
            float* mag_data = (float*)(mag ? mag->tt.data + mag->step*y : 0);
            float* angle_data = (float*)(angle->tt.data + angle->step*y);
    ...
    else
    {
        for(y = 0; y < size.height; y++)
        {
            double* x_data = (double*)(xmat ? xmat->tt.data + xmat->step*y : 0);
            double* y_data = (double*)(ymat ? ymat->tt.data + ymat->step*y : 0);
            double* mag_data = (double*)(mag ? mag->tt.data + mag->step*y : 0);
            double* angle_data = (double*)(angle->tt.data + angle->step*y);
            double C = angle_in_degrees ? CC_PI/180. : 1;
...
{
    struct {
#if (defined(WORDS_BIGENDIAN) && !defined(OPENCC_UNIVERSAL_BUILD)) || defined(__BIG_ENDIAN__)
        int hi;
        int lo;
#else
        int lo;
...
IPCVAPI_IMPL(CStatus, icvExp_32f, (const float *_x, float *y, int n), (_x, y, n))
{
    static const double
    ...
    int i = 0;
    DBLINT buf[4];
    const suf32_t* x = (const suf32_t*)_x;
...
IPCVAPI_IMPL(CStatus, icvExp_64f, (const double *_x, double *y, int n), (_x, y, n))
{
    static const double
    ...
    int i = 0;
    DBLINT buf[4];
    const suf64_t* x = (const suf64_t*)_x;
...
CC_IMPL void cvExp(const img_t* src, img_t* dst)
{
    CC_FUNCNAME("cvExp");
    ...
    if(!CC_ARE_CNS_EQ(src, dst) || src_type < CC_32F || dst_type < src_type)
        CC_ERROR_FROM_CODE(CC_StsUnmatchedFormats);
    ...
    for(y = 0; y < size.height; y++)
    {
        uchar* src_data = src->tt.data + src->step*y;
        uchar* dst_data = dst->tt.data + dst->step*y;
...
IPCVAPI_IMPL(CStatus, icvLog_32f, (const float *_x, float *y, int n), (_x, y, n))
{
...
IPCVAPI_IMPL(CStatus, icvLog_64f, (const double *x, double *y, int n), (x, y, n))
{
    static const double shift[] = { 0, -1./512 };
    static const double
...
CC_IMPL void cvLog(const img_t* src, img_t* dst)
{
    CC_FUNCNAME("cvLog");
    ...
    int coi1 = 0, coi2 = 0, src_type, dst_type;
    double* buffer = 0;
    CSize size;
    int x, y, dx = 0;
    ...
    if(!CC_ARE_CNS_EQ(src, dst) || dst_type < CC_32F || src_type < dst_type)
        CC_ERROR_FROM_CODE(CC_StsUnmatchedFormats);
...
#define ICC_DEF_IPOW_OP(flavor, arrtype, worktype, cast_macro)                    \
static CStatus C_STDCALL                                                          \
...
ICC_DEF_IPOW_OP(8u, uchar, int, CC_CAST_8U)
ICC_DEF_IPOW_OP(16u, ushort, int, CC_CAST_16U)
ICC_DEF_IPOW_OP(16s, short, int, CC_CAST_16S)
ICC_DEF_IPOW_OP(32s, int, int, CC_CAST_32S)
ICC_DEF_IPOW_OP(32f, float, double, CC_CAST_32F)
ICC_DEF_IPOW_OP(64f, double, double, CC_CAST_64F)

#define icvIPow_8s 0

CC_DEF_INIT_FUNC_TAB_1D(IPow)

typedef CStatus (C_STDCALL * CvIPowFunc)(const void* src, void* dst, int len, int power);
typedef CStatus (C_STDCALL * CvSqrtFunc)(const void* src, void* dst, int len);

CC_IMPL img_t* cvPow(const img_t* src, img_t* dst, double power)
{
    ...
    if(is_ipower)
    {
        CvIPowFunc pow_func = (CvIPowFunc)ipow_tab[type];
        if(!pow_func)
            CC_ERROR(CC_StsUnsupportedFormat, "The data type is not supported");
    ...
    else if(fabs(fabs(power) - 0.5) < DBL_EPSILON)
    {
        CvSqrtFunc sqrt_func = power < 0 ? 
            (type == CC_32F ? (CvSqrtFunc)icvInvSqrt_32f : (CvSqrtFunc)icvInvSqrt_64f) :
            (type == CC_32F ? (CvSqrtFunc)icvSqrt_32f : (CvSqrtFunc)icvSqrt_64f);
    ...
    {
        block_size = MIN(size.width, ICC_MATH_BLOCK_SIZE);
        temp_buffer = cvStackAlloc(block_size*CC_TYPE_SIZE(type));
...
CC_IMPL img_t* cvPow(const img_t* src, double power, img_t* dst) {
  return cvPow(src, dst, power);
}

//CC_IMPL img_t* cvPow(const img_t* src, double power, img_t* dst) {	return cvPow(src, dst, power); }

/************************** CheckArray for NaN's, Inf's *********************************/

IPCVAPI_IMPL(CStatus, icvCheckArray_32f_C1R,
    (const float* src, int srcstep, CSize size, int flags, double min_val, double max_val),
...
IPCVAPI_IMPL(CStatus,  icvCheckArray_64f_C1R,
    (const double* src, int srcstep, CSize size, int flags, double min_val, double max_val),
    (src, srcstep, size, flags, min_val, max_val))
{
    suf64_t a, b;
    int64 ia, ib;
...
CC_IMPL int
cvCheckArr(const img_t* mat, int flags CC_DEFAULT(0), double minVal CC_DEFAULT(0), double maxVal CC_DEFAULT(0))
{
    int result = 0;

    CC_FUNCNAME("cvCheckArr");
    ...
    if(mat)
    {
        ...
        if(type == CC_32F)
        {
            status = icvCheckArray_32f_C1R(mat->tt.fl, mat->step, size,
                                            flags, minVal, maxVal);
        }
        else if(type == CC_64F)
        {
            status = icvCheckArray_64f_C1R(mat->tt.db, mat->step, size,
                                            flags, minVal, maxVal);
        }
        else
        {
        ...
        if(status < 0)  
        {
            if(status != CC_BADRANGE_ERR || !(flags & CC_CHECK_QUIET))
                CC_ERROR(CC_StsOutOfRange, "CheckArray failed");
