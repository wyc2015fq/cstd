...
public:
    ...
    MSER() {}
    explicit MSER(const Params& _params) : params(_params) {}

    virtual ~MSER() {}

     static MSER* create(int _delta = 5, int _min_area = 60, int _max_area = 14400,
      double _max_variation = 0.25, double _min_diversity = .2,
      int _max_evolution = 200, double _area_threshold = 1.01,
      double _min_margin = 0.003, int _edge_blur_size = 5);
    ...
    {
        Params p;
        vector<vector<IPOINT> >* msers;
        vector<IRect>* bboxvec;
        Pixel* pix0;
        int step;
    };
    ...
    struct CompHistory
    {
        ...
        void checkAndCapture( WParams& wp )
        {
            ...
            wp.msers->push_back(vector<IPOINT>());
            vector<IPOINT>& region = wp.msers->back();
            region.resize(size);
            ...
            wp.bboxvec->push_back(iRect(xmin, ymin, xmax - xmin + 1, ymax - ymin + 1));
        }
    ...
    void detectRegions( const img_t* image,
                        std::vector<std::vector<IPOINT> >& msers,
                        std::vector<IRect>& bboxes );
    void detect( const img_t* _src, vector<CKeyPoint>& keypoints, const img_t* _mask );

    void preprocess1( const img_t* img, int* level_size )
    {
        ...
        for( i = 1; i < rows-1; i++ )
        {
            const uchar* imgptr = img_row(uchar, img, i);
    ...
    void preprocess2( const img_t* img, int* level_size )
    {
        int i;
    ...
    void pass( const img_t* img, vector<vector<IPOINT> >& msers, vector<IRect>& bboxvec,
              ISIZE size, const int* level_size, int mask )
    {
        CompHistory* histptr = &histbuf[0];
        int step = size.width;
        Pixel *ptr0 = &pixbuf[0], *ptr = &ptr0[step+1];
        const uchar* imgptr0 = img_ptr(uchar, img);
        Pixel** heap[256];
        ...
        for( ;; )
        {
            ...
            else
            {
                ...
                if (curr_gray < comptr[-1].gray_level)
                {
                    ...
                    CC_DbgAssert(comptr->size == comptr->history->size);
                }
                else
                {
                    ...
                    comptr--;
                    CC_DbgAssert(curr_gray == comptr->gray_level);
...
add support for color images into MSER::detectAndLabel.
*/
                               ...
                               int edgeBlurSize )
{
    ...
    const uchar* srcptr = img_ptr(uchar, src);
    const uchar* lastptr = srcptr+3;
    double* dxptr = img_ptr(double, dx);
    ...
    srcptr = img_ptr(uchar, src);
    lastptr = srcptr+src->step;
    double* dyptr = img_ptr(double, dy);
...
static bool MSCRStableCheck( MSCRNode* x, const MSER::Params& params )
{
...
static void
extractMSER_8uC3( const img_t* src,
                  vector<vector<IPOINT> >& msers,
                  vector<IRect>& bboxvec,
                  const MSER::Params& params )
{
  img_t tmp_[10];
  img_t* tmp = tmp_;
    bboxvec.clear();
    ...
    double emean = 0;
    img_t* dx = imcreate2(tmp++, src->rows, src->cols-1, CC_64FC1 );
    img_t* dy = imcreate2(tmp++, src->rows-1, src->cols, CC_64FC1 );
    ...
    for ( TempMSCR* ptr = mscr; ptr < mscrptr; ptr++ )
        ...
        if ( ptr->m > params.minMargin )
        {
            ...
            msers.push_back(vector<IPOINT>());
            vector<IPOINT>& mser = msers.back();
            ...
            bboxvec.push_back(iRect(xmin, ymin, xmax - xmin + 1, ymax - ymin + 1));
        }
    cvFree( &mscr );
    cvFree( &edge );
    cvFree( &map );
...
void MSER::detectRegions( const img_t* _src, vector<vector<IPOINT> >& msers, vector<IRect>& bboxes )
{
    ...
    if( src->rows < 3 || src->cols < 3 )
        CC_Error(CC_StsBadArg, "Input image is too small. Expected at least 3x3");
    ...
    {
        CC_Assert( src->type() == CC_8UC3 || src->type() == CC_8UC4 );
        extractMSER_8uC3( src, msers, bboxes, params );
    }
}

void MSER::detect( const img_t* _image, vector<CKeyPoint>& keypoints, const img_t* _mask )
{
    CC_INSTRUMENT_REGION()

    vector<IRect> bboxes;
    vector<vector<IPOINT> > msers;
    const img_t* mask = _mask;
    ...
    {
        IRect r = bboxes[i];
        // TODO check transformation from MSER region to CKeyPoint
        CRotatedRect rect = fitEllipse((msers[i]));
...
MSER* MSER::create( int _delta, int _min_area, int _max_area,
      double _max_variation, double _min_diversity,
      int _max_evolution, double _area_threshold,
      double _min_margin, int _edge_blur_size )
{
    return new MSER(
        MSER::Params(_delta, _min_area, _max_area,
