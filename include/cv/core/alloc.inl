...
{
    CC_Error_(CC_StsNoMem, ("Failed to allocate %lu bytes", (unsigned long)size));
    return 0;
}

#if CC_USE_SYSTEM_MALLOC
...
{
    uchar* udata = (uchar*)malloc(size + sizeof(void*) + CC_MALLOC_ALIGN);
    if(!udata)
        return OutOfMemoryError(size);
    uchar** adata = cAlignPtr((uchar**)udata + 1, CC_MALLOC_ALIGN);
    adata[-1] = udata;
    return adata;
}
...
void fastFree(void* ptr)
{
    if(ptr)
    {
        ...
        CC_DbgAssert(udata < (uchar*)ptr &&
               ((uchar*)ptr - udata) <= (ptrdiff_t)(sizeof(void*)+CC_MALLOC_ALIGN));
        free(udata);
    }
}

#else //CC_USE_SYSTEM_MALLOC

#if 0
#define SANITY_CHECK(block) \
    CC_Assert(((size_t)(block) & (MEM_BLOCK_SIZE-1)) == 0 && \
...
struct BigBlock
{
    ...
    {
        first = cAlignPtr((Block*)(this+1), MEM_BLOCK_SIZE);
        next = _next;
...
struct ThreadData
{
    ...
    void moveBlockToFreeList( Block* block )
    {
        ...
        CC_DbgAssert( block->next->prev == block && block->prev->next == block );
        if( block != freePtr )
        {
...
static void checkList(ThreadData* tls, int idx)
{
    ...
    if( !block )
    {
        CC_DbgAssert( tls->bins[idx][FREE] == 0 && tls->bins[idx][GC] == 0 );
    }
    else
    {
        bool gcInside = false;
        ...
        CC_DbgAssert( gcInside && freeInside );
    }
}
#else
...
void* fastMalloc( size_t size )
{
    if( size > MAX_BLOCK_SIZE )
    {
        ...
        uchar** adata = cAlignPtr((uchar**)udata + 2, MEM_BLOCK_SIZE);
        adata[-1] = udata;
        adata[-2] = (uchar*)size1;
        return adata;
    }
    ...
    if( block->threadData == tls )
    {
        ...
        if( !block->isFilled() && (block->allocated == 0 || prevFilled) )
        {
            if( block->allocated == 0 )
            {
                ...
                if( block == block->next )
                {
                    CC_DbgAssert( startPtr == block && freePtr == block && gcPtr == block );
