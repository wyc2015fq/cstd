...
namespace
{
    ...
    {
        CC_DbgAssert( ptr >= datastart && ptr < dataend );

    #if !defined(NDEBUG)
        const size_t allocSize = tip - ptr;
        CC_Assert( allocSize == allocations.back() );
...
namespace
{
    ...
    void MemoryPool::release()
    {
        if (mem_)
        {
            ...
            for (int i = 0; i < stackCount_; ++i)
            {
                CC_DbgAssert( stacks_[i].isFree );
                CC_DbgAssert( stacks_[i].tip == stacks_[i].datastart );
            }
#endif
    ...
    void MemoryPool::returnMemStack(MemoryStack* memStack)
    {
        ...
        CC_DbgAssert( memStack->tip == memStack->datastart );
...
cuda::Stream::Impl::Impl() : stream(0), ownStream(false)
{
...
cuda::Stream::Impl::Impl(cudaStream_t stream_) : stream(stream_), ownStream(false)
{
    stackAllocator = makePtr<StackAllocator>(stream);
}

cuda::Stream::Impl::~Impl()
{
...
namespace cv { namespace cuda
{
    ...
    Stream& DefaultDeviceInitializer::getNullStream(int deviceId)
    {
        ...
        CC_DbgAssert( deviceId >= 0 && deviceId < static_cast<int>(streams_.size()) );
    ...
    MemoryPool* DefaultDeviceInitializer::getMemoryPool(int deviceId)
    {
        ...
        CC_DbgAssert( deviceId >= 0 && deviceId < static_cast<int>(pools_.size()) );
...
bool cuda::Stream::queryIfComplete() const
{
#ifndef HAVE_CUDA
    throw_no_cuda();
    return false;
...
void cuda::Stream::waitForCompletion()
{
#ifndef HAVE_CUDA
    throw_no_cuda();
#else
...
void cuda::Stream::waitEvent(const Event& event)
{
#ifndef HAVE_CUDA
    (void) event;
    throw_no_cuda();
...
void cuda::Stream::enqueueHostCallback(StreamCallback callback, void* userData)
{
#ifndef HAVE_CUDA
    (void) callback;
    (void) userData;
...
cuda::Stream::operator bool_type() const
{
#ifndef HAVE_CUDA
    return 0;
#else
...
cudaStream_t cuda::StreamAccessor::getStream(const Stream& stream)
{
    return stream.impl_->stream;
}

Stream cuda::StreamAccessor::wrapStream(cudaStream_t stream)
{
...
void cuda::setBufferPoolUsage(bool on)
{
#ifndef HAVE_CUDA
    (void)on;
    throw_no_cuda();
...
void cuda::setBufferPoolConfig(int deviceId, size_t stackSize, int stackCount)
{
#ifndef HAVE_CUDA
    (void)deviceId;
    (void)stackSize;
...
cuda::BufferPool::BufferPool(Stream& stream) : allocator_(stream.impl_->stackAllocator.get())
{
}

GpuMat cuda::BufferPool::getBuffer(int rows, int cols, int type)
{
    GpuMat buf(allocator_);
    buf cvSetMat(rows, cols, type);
    return buf;
}
...
cuda::Event::Impl::Impl(unsigned int flags) : event(0), ownEvent(false)
{
    cudaSafeCall( cudaEventCreateWithFlags(&event, flags) );
    ownEvent = true;
}

cuda::Event::Impl::Impl(cudaEvent_t e) : event(e), ownEvent(false)
{
}

cuda::Event::Impl::~Impl()
{
    if (event && ownEvent)
    {
...
cudaEvent_t cuda::EventAccessor::getEvent(const Event& event)
{
    return event.impl_->event;
}

Event cuda::EventAccessor::wrapEvent(cudaEvent_t event)
{
...
cuda::Event::Event(CreateFlags flags)
{
#ifndef HAVE_CUDA
    (void) flags;
    throw_no_cuda();
...
void cuda::Event::record(Stream& stream)
{
#ifndef HAVE_CUDA
    (void) stream;
    throw_no_cuda();
...
bool cuda::Event::queryIfComplete() const
{
#ifndef HAVE_CUDA
    throw_no_cuda();
    return false;
...
void cuda::Event::waitForCompletion()
{
#ifndef HAVE_CUDA
    throw_no_cuda();
#else
...
float cuda::Event::elapsedTime(const Event& start, const Event& end)
{
#ifndef HAVE_CUDA
    (void) start;
    (void) end;
