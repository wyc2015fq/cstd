...
#if defined HAVE_TBB
#  define CC_PARALLEL_FRAMEWORK "tbb"
#elif defined HAVE_CSTRIPES
#  define CC_PARALLEL_FRAMEWORK "cstripes"
#elif defined HAVE_OPENMP
#  define CC_PARALLEL_FRAMEWORK "openmp"
#elif defined HAVE_GCD
#  define CC_PARALLEL_FRAMEWORK "gcd"
#elif defined WINRT
#  define CC_PARALLEL_FRAMEWORK "winrt-concurrency"
#elif defined HAVE_CONCURRENCY
#  define CC_PARALLEL_FRAMEWORK "ms-concurrency"
#elif defined HAVE_PTHREADS_PF
#  define CC_PARALLEL_FRAMEWORK "pthreads"
#endif
...
namespace cv
{
    ...
    void parallel_for_pthreads(const CRange& range, const ParallelLoopBody& body, double nstripes);
...
namespace
{
#ifdef CC_PARALLEL_FRAMEWORK
#ifdef ENABLE_INSTRUMENTATION
    static void SyncNodes(instr::InstrNode *pNode)
    {
        std::vector<instr::NodeDataTls*> data;
    ...
    {
    public:
        ParallelLoopBodyWrapper(const ParallelLoopBody& _body, const CRange& _r, double _nstripes)
        {

            body = &_body;
            wholeRange = _r;
            double len = wholeRange.end - wholeRange.start;
            nstripes = cRound(_nstripes <= 0 ? len : MIN(MAX(_nstripes, 1.), len));

#ifdef ENABLE_INSTRUMENTATION
            pThreadRoot = instr::getInstrumentTLSStruct().pCurrentNode;
#endif
        }
#ifdef ENABLE_INSTRUMENTATION
...
#endif
        void operator()(const CRange& sr) const
        {
#ifdef ENABLE_INSTRUMENTATION
            {
                instr::InstrTLSStruct *pInstrTLS = &instr::getInstrumentTLSStruct();
        ...
        CRange stripeRange() const { return CRange(0, nstripes); }
    ...
    {
    public:
        ProxyLoopBody(const ParallelLoopBody& _body, const CRange& _r, double _nstripes)
        : ParallelLoopBodyWrapper(_body, _r, _nstripes)
        {}

        void operator ()(const tbb::blocked_range<int>& range) const
        {
            this->ParallelLoopBodyWrapper::operator()(CRange(range.begin(), range.end()));
        }
    };
    ...
    static void block_function(void* context, size_t index)
    {
        ...
        (*ptr_body)(CRange((int)index, (int)index + 1));
    }
    ...
    public:
        ...
        {
            this->ParallelLoopBodyWrapper::operator()(CRange(i, i + 1));
        }
    };
#else
    typedef ParallelLoopBodyWrapper ProxyLoopBody;
...
#endif // CC_PARALLEL_FRAMEWORK

} //namespace

/* ================================   parallel_for_  ================================ */

void parallel_for_(const CRange& range, const ParallelLoopBody& body, double nstripes)
{
    CC_INSTRUMENT_REGION_MT_FORK()
    if (range.empty())
        return;
    ...
    if(numThreads != 0)
    {
        ...
        CRange stripeRange = pbody.stripeRange();
        ...
        parallel(MAX(0, numThreads))
        {
            ...
            CRange r(offset + CPX_RANGE_START(len), offset + CPX_RANGE_END(len));
            pbody(r);
            barrier();
        }
...
int getThreadNum(void)
{
    ...
    return 0;
#elif defined HAVE_CONCURRENCY
    return MAX(0, (int)Concurrency::Context::VirtualProcessorId()); // zero for master thread, unique number for others but not necessary 1,2,3,...
#elif defined HAVE_PTHREADS_PF
    return (int)(size_t)(void*)pthread_self(); // no zero-based indexing
#else
    return 0;
...
#ifdef CC_PARALLEL_FRAMEWORK
    return CC_PARALLEL_FRAMEWORK;
#else
    return NULL;
#endif
}

CC_IMPL void cvSetNumThreads(int nt)
{
    setNumThreads(nt);
}

CC_IMPL int cGetNumThreads()
{
    return getNumThreads();
}

CC_IMPL int cGetThreadNum()
{
    return getThreadNum();
}
