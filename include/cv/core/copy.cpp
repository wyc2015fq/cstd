...
copyMask_(const uchar* _src, size_t sstep, const uchar* mask, size_t mstep, uchar* _dst, size_t dstep, ISize size)
{
...
template<> void
copyMask_<uchar>(const uchar* _src, size_t sstep, const uchar* mask, size_t mstep, uchar* _dst, size_t dstep, ISize size)
{
...
template<> void
copyMask_<ushort>(const uchar* _src, size_t sstep, const uchar* mask, size_t mstep, uchar* _dst, size_t dstep, ISize size)
{
...
static void
copyMaskGeneric(const uchar* _src, size_t sstep, const uchar* mask, size_t mstep, uchar* _dst, size_t dstep, ISize size, void* _esz)
{
...
static void copyMask##suffix(const uchar* src, size_t sstep, const uchar* mask, size_t mstep, \
                             uchar* dst, size_t dstep, ISize size, void*) \
{ \
...
/* dst = src */
void copyTo( const img_t* src, img_t* _dst )
{
    ...
    {
        CC_Assert( channels() == CC_MAT_CN(dtype) );
    ...
    if( _dst->isUMat() )
    {
        ...
        CC_Assert(dst.u != NULL);
        size_t i, sz[CC_MAX_DIM] = {0}, dstofs[CC_MAX_DIM], esz = elemSize();
        CC_Assert(dims >= 0 && dims < CC_MAX_DIM);
    ...
    if( dims <= 2 )
    {
        ...
        if( rows > 0 && cols > 0 )
        {
            ...
            CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippiCopy_8u_C1R_L, sptr, (int)step, dptr, (int)dst.step, ippiSizeL((int)(cols*elemSize()), rows)) >= 0)
#endif

            ISize sz = getContinuousSize(*this, dst);
...
static bool ipp_copyTo(const Mat &src, Mat &dst, const Mat &mask)
{
    ...
    if(mask.channels() > 1 || mask.depth() != CC_8U)
        return false;

    if (src.dims <= 2)
    {
        IppiSize size = ippiSize(src.size());
        return CC_INSTRUMENT_FUN_IPP(llwiCopyMask, src.ptr(), (int)src.step, dst.ptr(), (int)dst.step, size, (int)src.elemSize1(), src.channels(), mask.ptr(), (int)mask.step) >= 0;
    }
    else
    {
        ...
        for (size_t i = 0; i < it.nplanes; i++, ++it)
        {
            if(CC_INSTRUMENT_FUN_IPP(llwiCopyMask, ptrs[0], 0, ptrs[1], 0, size, (int)src.elemSize1(), src.channels(), ptrs[2], 0) < 0)
                return false;
        }
        return true;
    }
#else
    CC_UNUSED(src); CC_UNUSED(dst); CC_UNUSED(mask);
    return false;
#endif
}
#endif

void Mat::copyTo( img_t* _dst, InputArray _mask ) const
{
    CC_INSTRUMENT_REGION()
    ...
    CC_Assert( mask.depth() == CC_8U && (mcn == 1 || mcn == cn) );
    ...
    CC_IPP_RUN_FAST(ipp_copyTo(*this, dst, mask))
    ...
    if( dims <= 2 )
    {
        ISize sz = getContinuousSize(*this, dst, mask, mcn);
...
static bool ipp_Mat_setTo_Mat(Mat &dst, Mat &_val, Mat &mask)
{
    ...
    if(mask.depth() != CC_8U || mask.channels() > 1)
        return false;
    ...
    if(dst.dims <= 2)
    {
        ...
        ::ipp::IwValueFloat s;
        convertAndUnrollScalar(_val, CC_MAKETYPE(CC_64F, dst.channels()), (uchar*)((Ipp64f*)s), 1);

        return CC_INSTRUMENT_FUN_IPP(llwiSetMask, s, dst.ptr(), (int)dst.step, size, dataType, dst.channels(), mask.ptr(), (int)mask.step) >= 0;
    }
    else
    {
        ...
        for( size_t i = 0; i < it.nplanes; i++, ++it)
        {
            if(CC_INSTRUMENT_FUN_IPP(llwiSetMask, s, ptrs[0], 0, size, dataType, dst.channels(), ptrs[1], 0) < 0)
                return false;
        }
        return true;
    }
#else
    CC_UNUSED(dst); CC_UNUSED(_val); CC_UNUSED(mask);
    return false;
#endif
}
#endif
...
Mat& Mat::setTo(InputArray _value, InputArray _mask)
{
    ...
    int cn = channels(), mcn = mask.channels();
    CC_Assert( mask.empty() || (mask.depth() == CC_8U && (mcn == 1 || mcn == cn) && size == mask.size) );

    CC_IPP_RUN_FAST(ipp_Mat_setTo_Mat(*this, value, mask), *this)
    ...
    for( size_t i = 0; i < it.nplanes; i++, ++it )
    {
        ...
        {
            ISize sz(std::min(blockSize0, totalsz - j), 1);
...
static void
flipHoriz( const uchar* src, size_t sstep, uchar* dst, size_t dstep, ISize size, size_t esz )
{
...
static void
flipVert( const uchar* src0, size_t sstep, uchar* dst0, size_t dstep, ISize size, size_t esz )
{
...
static bool ocl_flip(InputArray _src, img_t* _dst, int flipCode )
{
...
static bool ipp_flip(Mat &src, Mat &dst, int flip_mode)
{
    ...
    try
    {
        ...
        CC_INSTRUMENT_FUN_IPP(::ipp::iwiMirror, iwSrc, iwDst, ippMode);
    }
    ...
    return true;
#else
    CC_UNUSED(src); CC_UNUSED(dst); CC_UNUSED(flip_mode);
    return false;
#endif
}
#endif
...
void flip( InputArray _src, img_t* _dst, int flip_mode )
{
    CC_INSTRUMENT_REGION()
...
static bool ocl_rotate(InputArray _src, img_t* _dst, int rotateMode)
{
    switch (rotateMode)
    {
...
void rotate(InputArray _src, img_t* _dst, int rotateMode)
{
    CC_Assert(_src.dims() <= 2);
...
static bool ocl_repeat(InputArray _src, int ny, int nx, img_t* _dst)
{
    if (ny == 1 && nx == 1)
    {
        _src.copyTo(_dst);
...
void repeat(InputArray _src, int ny, int nx, img_t* _dst)
{
    CC_INSTRUMENT_REGION()

    CC_Assert(_src.getObj() != _dst->getObj());
    CC_Assert( _src.dims() <= 2 );
    CC_Assert( ny > 0 && nx > 0 );

    ISize ssize = _src.size();
...
void copyMakeBorder_8u( const uchar* src, size_t srcstep, cv::ISize srcroi,
                        uchar* dst, size_t dststep, cv::ISize dstroi,
...
void copyMakeConstBorder_8u( const uchar* src, size_t srcstep, cv::ISize srcroi,
                             uchar* dst, size_t dststep, cv::ISize dstroi,
...
static bool ocl_copyMakeBorder( InputArray _src, img_t* _dst, int top, int bottom,
                                int left, int right, int borderType, const Scalar& value )
{
    int type = _src.type(), cn = CC_MAT_CN(type), depth = CC_MAT_DEPTH(type),
            rowsPerWI = ocl::Device::getDefault().isIntel() ? 4 : 1;
                                ...
                                int left, int right, int _borderType, const Scalar& value )
{
    ...
    return CC_INSTRUMENT_FUN_IPP(llwiCopyMakeBorder, pSrc->ptr(), pSrc->step, subDst.ptr(), subDst.step, size, dataType, _src.channels(), borderSize, borderType, &value[0]) >= 0;
#else
    CC_UNUSED(_src); CC_UNUSED(_dst); CC_UNUSED(top); CC_UNUSED(bottom); CC_UNUSED(left); CC_UNUSED(right);
    CC_UNUSED(_borderType); CC_UNUSED(value);
    return false;
#endif
}
...
void cv::copyMakeBorder( InputArray _src, img_t* _dst, int top, int bottom,
                         int left, int right, int borderType, const Scalar& value )
{
    CC_INSTRUMENT_REGION()
    ...
    CC_IPP_RUN_FAST(ipp_copyMakeBorder(src, dst, top, bottom, left, right, borderType, value))
    ...
    else
    {
        ...
        scalarToRawData(value, buf, CC_MAKETYPE(src.depth(), cn1), cn);
...
cvCopy( const void* srcarr, void* dstarr, const void* maskarr )
{
    ...
    CC_Assert( src.depth() == dst.depth() && src.size == dst.size );
    ...
    if( coi1 || coi2 )
    {
        CC_Assert( (coi1 != 0 || src.channels() == 1) &&
        ...
        CC_Assert( src.channels() == dst.channels() );
...
cvFlip( const CvArr* srcarr, CvArr* dstarr, int flip_mode )
{
    ...
    CC_Assert( src.type() == dst.type() && src.size() == dst.size() );
