...
{
    CvTsSimpleSeq* seq = (CvTsSimpleSeq*)cAlloc( sizeof(*seq) + max_count * elem_size );
...
{
    CvTsSimpleSet* set_header = (CvTsSimpleSet*)cAlloc( sizeof(*set_header) + max_count *
                                               ...
                                               int edge_size, int oriented )
{
    ...
    graph = (CvTsSimpleGraph*)cAlloc( sizeof(*graph) +
...
#define CC_TS_SEQ_CHECK_CONDITION( expr, err_msg )          \
if( !(expr) )                                               \
{                                                           \
...
int Core_DynStructBaseTest::read_params( CvFileStorage* fs )
{
    ...
    generations = cRound(generations*sqrt_scale);
    iterations = cRound(iterations*sqrt_scale);
...
void Core_DynStructBaseTest::update_progressbar()
{
    ...
    {
        test_progress = 0;
        cpu_freq = getTickFrequency();
        start_time = cGetTickCount();
    }
...
int Core_DynStructBaseTest::test_seq_block_consistence( int _struct_idx, CvSeq* seq, int total )
{
    ...
    CC_TS_SEQ_CHECK_CONDITION( seq != 0, "Null sequence pointer" );
    ...
    if( seq->first )
    {
        ...
        for( ;; )
        {
            CC_TS_SEQ_CHECK_CONDITION( sum == block->start_index - delta_idx &&
        ...
        CC_TS_SEQ_CHECK_CONDITION( block->prev->count * seq->elem_size +
                                  block->prev->data <= seq->block_max,
                                  "block->data or block_max pointer are incorrect" );
    }

    CC_TS_SEQ_CHECK_CONDITION( seq->total == sum && sum == total,
...
int Core_SeqBaseTest::test_multi_create()
{
    ...
    for( int i = 0; i < struct_count; i++ )
    {
        ...
        elem_size = cRound( exp(t * CC_LOG2) );
        ...
        img_t m( 1, MAX(sseq->count,1)*elem_size, CC_8UC1, sseq->array );
    ...
    for( cur_count = struct_count; cur_count > 0; cur_count-- )
    {
        for(;;)
        {
...

            {
                ...
                CC_WRITE_SEQ_ELEM_VAR( el, writer[struct_idx] );
            }
            pos[struct_idx]++;
        }
    }
...
int  Core_SeqBaseTest::test_get_seq_elem( int _struct_idx, int iters )
{
    ...
    for( int i = 0; i < iters; i++ )
    {
        ...
        if( bad_range )
        {
            CC_TS_SEQ_CHECK_CONDITION( elem == 0,
        ...
        else
        {
            ...
            CC_TS_SEQ_CHECK_CONDITION( idx >= 0 && idx == idx0,
...
int  Core_SeqBaseTest::test_get_seq_reading( int _struct_idx, int iters )
{
    ...
    if( total == 0 )
    {
        CC_TS_SEQ_CHECK_CONDITION( reader.ptr == 0, "Empty sequence reader pointer is not NULL" );
        return 0;
    }

    pos = pos ? seq->total - 1 : 0;

    CC_TS_SEQ_CHECK_CONDITION( pos == cvGetSeqReaderPos(&reader),
    ...
    for( iter = 0; iter < iters; iter++ )
    {
        ...
        if( op >= max_val - 2 )
        {
            ...
            if( !bad_range )
            {
                CC_TS_SEQ_CHECK_CONDITION( new_pos0 == cvGetSeqReaderPos( &reader ),
        ...
        else
        {
            ...
            if( direction > 0 )
            {
                CC_NEXT_SEQ_ELEM( sseq->elem_size, reader );
            }
            else if( direction < 0 )
            {
                CC_PREV_SEQ_ELEM( sseq->elem_size, reader );
            }

            CC_TS_SEQ_CHECK_CONDITION( memcmp(elem, cvTsSimpleSeqElem(sseq, pos),
    ...
    for( iter = 0; iter < iters; iter++ )
    {
        ...
        switch( op )
        {
            ...
            case 2:  // push/pushfront/insert
                ...
                elem_mat = img_t(1, elem_size, CC_8U, elem);
                cvtest::randUni( rng, elem_mat, cScalarAll(0), cScalarAll(255) );
                ...
                CC_TS_SEQ_CHECK_CONDITION( elem2 != 0, "The inserted element could not be retrieved" );
                CC_TS_SEQ_CHECK_CONDITION( seq->total == sseq->count &&
            ...
            case 5: // pop/popfront/remove
                ...
                if( whence != 0 )
                    CC_TS_SEQ_CHECK_CONDITION( seq->total == sseq->count - 1 &&
                ...
                if( sseq->count > 0 )
                {
                    ...
                    CC_TS_SEQ_CHECK_CONDITION( elem2 != 0, "GetSeqElem fails after removing the element" );

                    CC_TS_SEQ_CHECK_CONDITION( memcmp( elem2,
                ...
                {
                    CC_TS_SEQ_CHECK_CONDITION( seq->first == 0,
            ...
            case 8: // push [front] multi/insert slice
                ...
                elem_mat = img_t(1, MAX(count,1) * elem_size, CC_8U, elem);
                cvtest::randUni( rng, elem_mat, cScalarAll(0), cScalarAll(255) );
                ...
                else
                {
                    CvSeq header;
                    CvSeqBlock block;
                    cvMakeSeqHeaderForArray( CC_SEQ_KIND_GENERIC, sizeof(CvSeq),
                                                     sseq->elem_size,
                                                     elem, count,
                                                     &header, &block );
                ...
                if( sseq->count > 0 )
                {
                    ...
                    elem2 = cvGetSeqElem( seq, pos );
                    CC_TS_SEQ_CHECK_CONDITION( elem2 != 0, "multi push operation doesn't add elements" );
                    CC_TS_SEQ_CHECK_CONDITION( seq->total == sseq->count &&
                                              memcmp( elem2, cvTsSimpleSeqElem(sseq,pos), elem_size) == 0,
                ...
                {
                    CC_TS_SEQ_CHECK_CONDITION( seq->total == 0 && seq->first == 0,
            ...
            case 11: // pop [front] multi
                ...
                if( whence != 0 )
                {
                    ...
                    if( count > 0 )
                    {
                        CC_TS_SEQ_CHECK_CONDITION( memcmp(elem,
                ...
                {
                     cvSeqRemoveSlice( seq, cSlice(pos, pos + count) );
                }

                CC_TS_SEQ_CHECK_CONDITION( seq->total == sseq->count - count,
                ...
                if( sseq->count > 0 )
                {
                    ...
                    elem2 = cvGetSeqElem( seq, pos );
                    CC_TS_SEQ_CHECK_CONDITION( elem2 &&
                                              memcmp( elem2, cvTsSimpleSeqElem(sseq,pos), elem_size) == 0,
            ...
            case 12: // seqslice
            {
                ...
                CvSeq* seq_slice = cvSeqSlice( seq, cSlice(pos, pos + count), storage, copy_data );

                CC_TS_SEQ_CHECK_CONDITION( seq_slice && seq_slice->total == count,
                ...
                if( count > 0 )
                {
                    ...
                    CC_TS_SEQ_CHECK_CONDITION( (elem2 == elem3) ^ copy_data,
    ...
    try
    {
        ...
        for( gen = 0; gen < generations; gen++ )
        {
            ...
            if( !storage )
            {
                ...
                + min_log_storage_block_size;
                storage.reset(cvCreateMemStorage( cRound( exp(t * CC_LOG2) ) ));
            }
        ...
        for( gen = 0; gen < generations; gen++ )
        {
            ...
            for( iter = 0; iter < iterations/10; iter++ )
            {
                ...
                for( i = 0; i < struct_count; i++ )
                {
                    ...
                    CC_TS_SEQ_CHECK_CONDITION( sseq->count == 0 || memcmp( &buffer[0],
                    ...
                    for( k = 0; k < (sseq->count > 0 ? 10 : 0); k++ )
                    {
                        ...
                        CC_TS_SEQ_CHECK_CONDITION( elem2 != 0 &&
...
int  Core_SetTest::test_set_ops( int iters )
{
    ...
    for( iter = 0; iter < iters; iter++ )
    {
        ...
        if( iter > iters/10 && cvtest::randInt(rng)%200 == 0 ) // clear set
        {
            ...
            CC_TS_SEQ_CHECK_CONDITION( cvset->active_count == 0 && cvset->total == 0 &&
        ...
        else if( op == 0 || op == 1 ) // add element
        {
            ...
            elem_mat = img_t(1, cvset->elem_size, CC_8U, &elem_buf[0]);
            cvtest::randUni( rng, elem_mat, cScalarAll(0), cScalarAll(255) );
            ...
            if( by_ptr )
            {
                ...
                CC_TS_SEQ_CHECK_CONDITION( elem2 != 0, "cvSetNew returned NULL pointer" );
            }
            else
            {
                pass_data = cvtest::randInt(rng) % 2;
                idx = cvSetAdd( cvset, pass_data ? elem : 0, &elem2 );
                CC_TS_SEQ_CHECK_CONDITION( elem2 != 0 && elem2->flags == idx,
        ...
        else if( op == 2 || op == 3 ) // remove element
        {
            ...
            CC_TS_SEQ_CHECK_CONDITION( CC_IS_SET_ELEM(elem) && elem->flags == idx &&
            ...
            CC_TS_SEQ_CHECK_CONDITION( !CC_IS_SET_ELEM(elem) && !cvGetSetElem(cvset, idx) &&
                                      (elem->flags & CC_SET_ELEM_IDX_MASK) == idx,
                                      "cvSetRemove[ByPtr] didn't release the element properly" );

            CC_TS_SEQ_CHECK_CONDITION( elem->next_free == first_free &&
        ...
        CC_TS_SEQ_CHECK_CONDITION( cvset->active_count == sset->max_count - sset->free_count &&
...
void Core_SetTest::run( int )
{
    try
    {
        ...
        for( gen = 0; gen < generations; gen++ )
        {
            ...
            {
                t = cvtest::randReal(rng)*(max_log_elem_size - min_log_elem_size) + min_log_elem_size;
                int pure_elem_size = cRound( exp(t * CC_LOG2) );
...
int  Core_GraphTest::test_graph_ops( int iters )
{
    ...
    for( iter = 0; iter < iters; iter++ )
    {
        ...
        if( cvtest::randInt(rng) % 200 == 0 ) // clear graph
        {
            ...
            CC_TS_SEQ_CHECK_CONDITION( graph->active_count == 0 && graph->total == 0 &&
                                      graph->first == 0 && graph->free_elems == 0 &&
                                      (graph->free_blocks != 0 || prev_vtx_count2 == 0),
                                      "The graph is not empty after clearing" );

            CC_TS_SEQ_CHECK_CONDITION( edges->active_count == 0 && edges->total == 0 &&
        ...
        else if( op == 0 ) // add vertex
        {
            ...
            if( pure_vtx_size )
            {
                elem_mat = img_t(1, graph->elem_size, CC_8U, &elem_buf[0]);
                cvtest::randUni( rng, elem_mat, cScalarAll(0), cScalarAll(255) );
            }
            ...
            CC_TS_SEQ_CHECK_CONDITION( (CC_IS_SET_ELEM(vtx3) && vtx3->flags == idx &&
                                        vtx3->first == 0) || (idx == idx0 && vtx3 == vtx2 &&
                                                              (!pass_data || pure_vtx_size == 0 ||
                                                               memcmp(vtx3 + 1, vtx + 1, pure_vtx_size) == 0)),
                                      "The added element is not correct" );

            CC_TS_SEQ_CHECK_CONDITION( (!first_free || first_free == (CvSetElem*)vtx3) &&
        ...
        else if( op == 1 ) // remove vertex
        {
            ...
            CC_TS_SEQ_CHECK_CONDITION( CC_IS_SET_ELEM(vtx) && vtx->flags == idx &&
            ...
            CC_TS_SEQ_CHECK_CONDITION( vtx_degree == vtx_degree0,
                                      "Number of incident edges is different in two graph representations" );

            CC_TS_SEQ_CHECK_CONDITION( !CC_IS_SET_ELEM(vtx) && !cvGetGraphVtx(graph, idx) &&
                                      (vtx->flags & CC_SET_ELEM_IDX_MASK) == idx,
                                      "cvGraphRemoveVtx[ByPtr] didn't release the vertex properly" );

            CC_TS_SEQ_CHECK_CONDITION( graph->edges->active_count == prev_edge_count - vtx_degree,
                                      "cvGraphRemoveVtx[ByPtr] didn't remove all the incident edges "
                                      "(or removed some extra)" );

            CC_TS_SEQ_CHECK_CONDITION( ((CvSetElem*)vtx)->next_free == first_free &&
        ...
        else if( op == 2 ) // add edge
        {
            ...
            CC_TS_SEQ_CHECK_CONDITION( edge == 0, "Extra edge appeared in the graph" );

            if( pure_edge_size > 0 )
            {
                elem_mat = img_t(1, graph->edges->elem_size, CC_8U, &elem_buf[0]);
                cvtest::randUni( rng, elem_mat, cScalarAll(0), cScalarAll(255) );
            }
            ...
            CC_TS_SEQ_CHECK_CONDITION( vtx != 0 && vtx2 != 0 && vtx->flags == v_idx[0] &&
            ...
            CC_TS_SEQ_CHECK_CONDITION( res == 1 && edge2 != 0 && CC_IS_SET_ELEM(edge2) &&
                                      ((edge2->vtx[0] == vtx && edge2->vtx[1] == vtx2) ||
                                       (!CC_IS_GRAPH_ORIENTED(graph) && edge2->vtx[0] == vtx2 && edge2->vtx[1] == vtx)) &&
            ...
            CC_TS_SEQ_CHECK_CONDITION( v_degree[0] == v_prev_degree[0] + 1 &&
            ...
            CC_TS_SEQ_CHECK_CONDITION( (!first_free || first_free == (CvSetElem*)edge2) &&
        ...
        else if( op == 3 ) // find & remove edge
        {
            ...
            CC_TS_SEQ_CHECK_CONDITION( edge != 0 && edge->weight == v_idx[0] + v_idx[1] &&
                                      ((edge->vtx[0] == vtx && edge->vtx[1] == vtx2) ||
                                       (!CC_IS_GRAPH_ORIENTED(graph) && edge->vtx[1] == vtx && edge->vtx[0] == vtx2)) &&
            ...
            CC_TS_SEQ_CHECK_CONDITION( !edge2 && !CC_IS_SET_ELEM(edge),
            ...
            CC_TS_SEQ_CHECK_CONDITION( graph->edges->free_elems == (CvSetElem*)edge &&
        ...
        CC_TS_SEQ_CHECK_CONDITION( graph->active_count == sgraph->vtx->max_count - sgraph->vtx->free_count &&
                                  graph->total >= graph->active_count &&
                                  (graph->total == 0 || graph->total >= prev_vtx_total),
                                  "The total number of graph vertices is not correct" );

        CC_TS_SEQ_CHECK_CONDITION( graph->edges->total >= graph->edges->active_count &&
...
void Core_GraphTest::run( int )
{
    try
    {
        ...
        for( gen = 0; gen < generations; gen++ )
        {
            struct_idx = iter = -1;
            t = cvtest::randReal(rng)*(max_log_storage_block_size - min_log_storage_block_size) + min_log_storage_block_size;
            int block_size = cRound( exp(t * CC_LOG2) );
            ...
            for( i = 0; i < struct_count; i++ )
            {
                ...
                {
                    t = cvtest::randReal(rng)*(max_log_elem_size - min_log_elem_size) + min_log_elem_size;
                    int pe = cRound( exp(t * CC_LOG2) ) - 1; // pure_elem_size==0 does also make sense
                ...
                cxcore_struct[i] = cvCreateGraph( is_oriented ? CC_ORIENTED_GRAPH : CC_GRAPH,
...
int Core_GraphScanTest::create_random_graph( int _struct_idx )
{
    ...
    cxcore_struct[_struct_idx] = graph =
        cvCreateGraph(is_oriented ? CC_ORIENTED_GRAPH : CC_GRAPH,
...
void Core_GraphScanTest::run( int )
{
    ...
    try
    {
        ...
        for( gen = 0; gen < generations; gen++ )
        {
            struct_idx = iter = -1;
            t = cvtest::randReal(rng)*(max_log_storage_block_size - min_log_storage_block_size) + min_log_storage_block_size;
            int storage_blocksize = cRound( exp(t * CC_LOG2) );
            ...
            if( gen == 0 )
            {
                ...
                CvGraph* graph = cvCreateGraph( CC_ORIENTED_GRAPH, sizeof(CvGraph),
                ...
                edge_count = i;
                scanner = cvCreateGraphScanner( graph, 0, CC_GRAPH_ALL_ITEMS );
                ...
                for(;;)
                {
                    ...
                    switch( code )
                    {
                        case CC_GRAPH_VERTEX:
                            event = "Vertex";
                            vtx_count--;
                            a = cvGraphVtxIdx( graph, scanner->vtx );
                            break;
                        case CC_GRAPH_TREE_EDGE:
                            event = "Tree Edge";
                            edge_count--;
                            CC_TS_SEQ_CHECK_CONDITION( scanner->edge->weight == (float)'t',
                                                      "Invalid edge type" );
                            a = cvGraphVtxIdx( graph, scanner->vtx );
                            b = cvGraphVtxIdx( graph, scanner->dst );
                            break;
                        case CC_GRAPH_BACK_EDGE:
                            event = "Back Edge";
                            edge_count--;
                            CC_TS_SEQ_CHECK_CONDITION( scanner->edge->weight == (float)'b',
                                                      "Invalid edge type" );
                            a = cvGraphVtxIdx( graph, scanner->vtx );
                            b = cvGraphVtxIdx( graph, scanner->dst );
                            break;
                        case CC_GRAPH_CROSS_EDGE:
                            event = "Cross Edge";
                            edge_count--;
                            CC_TS_SEQ_CHECK_CONDITION( scanner->edge->weight == (float)'c',
                                                      "Invalid edge type" );
                            a = cvGraphVtxIdx( graph, scanner->vtx );
                            b = cvGraphVtxIdx( graph, scanner->dst );
                            break;
                        case CC_GRAPH_FORWARD_EDGE:
                            event = "Forward Edge";
                            edge_count--;
                            CC_TS_SEQ_CHECK_CONDITION( scanner->edge->weight == (float)'f',
                                                      "Invalid edge type" );
                            a = cvGraphVtxIdx( graph, scanner->vtx );
                            b = cvGraphVtxIdx( graph, scanner->dst );
                            break;
                        case CC_GRAPH_BACKTRACKING:
                            event = "Backtracking";
                            a = cvGraphVtxIdx( graph, scanner->vtx );
                            break;
                        case CC_GRAPH_NEW_TREE:
                        ...
                        case CC_GRAPH_OVER:
                            ...
                            break;
                        default:
                            CC_TS_SEQ_CHECK_CONDITION( 0, "Invalid code appeared during graph scan" );
                    }
                ...
                CC_TS_SEQ_CHECK_CONDITION( vtx_count == 0 && edge_count == 0,
            ...
            for( iter = 0; iter < iterations; iter++ )
            {
                ...
                for( i = 0; i < 2; i++ )
                {
                    ...
                    scanner = cvCreateGraphScanner( graph, start_vtx, CC_GRAPH_ALL_ITEMS );
                    ...
                    for(;;)
                    {
                        ...
                        if( code == CC_GRAPH_OVER )
                            break;
                        else if( code & CC_GRAPH_ANY_EDGE )
                        {
                            int edge_idx = scanner->edge->flags & CC_SET_ELEM_IDX_MASK;

                            CC_TS_SEQ_CHECK_CONDITION( edge_idx < graph->edges->active_count &&
                                                      edge_mask[edge_idx] == 0,
                                                      "The edge is not found or visited for the second time" );
                            edge_mask[edge_idx] = 1;
                        }
                        else if( code & CC_GRAPH_VERTEX )
                        {
                            int vtx_idx = scanner->vtx->flags & CC_SET_ELEM_IDX_MASK;

                            CC_TS_SEQ_CHECK_CONDITION( vtx_idx < graph->active_count &&
                                                      vtx_mask[vtx_idx] == 0,
                    ...
                    CC_TS_SEQ_CHECK_CONDITION( cvtest::norm(img_t(vtx_mask),CC_L1) == graph->active_count &&
                                              cvtest::norm(img_t(edge_mask),CC_L1) == graph->edges->active_count,
