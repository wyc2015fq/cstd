...
protected:
    void run(int);
    bool check_pdf(const img_t& hist, double scale, int dist_type,
...
bool Core_RandTest::check_pdf(const img_t& hist, double scale,
                            int dist_type, double& refval, double& realval)
{
    img_t hist0(hist.size(), CC_32F);
    const int* H = hist img_ptr(int, );
    float* H0 = hist0 img_ptr(float, );
    int i, hsz = hist->cols;

    double sum = 0;
    for( i = 0; i < hsz; i++ )
        sum += H[i];
    CC_Assert( fabs(1./sum - scale) < FLT_EPSILON );

    if( dist_type == CC_RAND_UNI )
    {
    ...
    double chi2_pval = chi2_p95(hsz - 1 - (dist_type == CC_RAND_NORMAL ? 2 : 0));
...
void Core_RandTest::run( int )
{
    ...
    for( int idx = 0; idx < test_case_count; idx++ )
    {
        ...
        int dist_type = cvtest::randInt(rng) % (CC_RAND_NORMAL+1);
        int i, k, SZ = N/cn;
        Scalar A, B;
        ...
        bool do_sphere_test = dist_type == CC_RAND_UNI;
        img_t arr[2], hist[4];
        int W[] = {0,0,0,0};

        arr[0] cvSetMat(1, SZ, type);
        arr[1] cvSetMat(1, SZ, type);
        bool fast_algo = dist_type == CC_RAND_UNI && depth < CC_32F;
        ...
        for( c = 0; c < cn; c++ )
        {
            ...
            A[c] = a;
            B[c] = b;
            hist[c] cvSetMat(1, hsz, CC_32S);
        }

        RNG saved_rng = tested_rng;
        ...
        for( k = 0; k <= maxk; k++ )
        {
            ...
            for( slice = 0; slice < maxSlice; slice++, sz += dsz )
            {
                ...
                img_t aslice = arr[k].colRange(sz, sz + dsz);
        ...
        for( c = 0; c < cn; c++ )
        {
            const uchar* data = arr[0]->tt.data;
            int* H = hist[c] img_ptr(int, );
            int HSZ = hist[c]->cols;
            double minVal = dist_type == CC_RAND_UNI ? A[c] : A[c] - B[c]*4;
            double maxVal = dist_type == CC_RAND_UNI ? B[c] : A[c] + B[c]*4;
            ...
            {
                double val = depth == CC_8U ? ((const uchar*)data)[i] :
                depth == CC_8S ? ((const schar*)data)[i] :
                depth == CC_16U ? ((const ushort*)data)[i] :
                depth == CC_16S ? ((const short*)data)[i] :
                depth == CC_32S ? ((const int*)data)[i] :
                depth == CC_32F ? ((const float*)data)[i] :
                ((const double*)data)[i];
                int ival = cFloor(val*scale + delta);
                if( (unsigned)ival < (unsigned)HSZ )
                {
                    H[ival]++;
                    W[c]++;
                }
                else if( dist_type == CC_RAND_UNI )
                {
                    if( (minVal <= val && val < maxVal) || (depth >= CC_32F && val == maxVal) )
                    {
                        H[ival < 0 ? 0 : HSZ-1]++;
                        W[c]++;
                    }
            ...
            if( dist_type == CC_RAND_UNI && W[c] != SZ )
            {
            ...
            if( dist_type == CC_RAND_NORMAL && W[c] < SZ*.90)
            {
        ...
        if( do_sphere_test )
        {
            ...
            for( i = k = c = 0; i <= SZ*cn - SDIM; i++, k++, c++ )
            {
                ...
                depth == CC_16S ? ((const short*)data)[i] :
                depth == CC_32S ? ((const int*)data)[i] :
                depth == CC_32F ? ((const float*)data)[i] : ((const double*)data)[i];
                c &= c < cn ? -1 : 0;
...
protected:
    void run(int)
    {
        img_t a(Size(1280, 720), CC_8U, Scalar(20));
        img_t af(Size(1280, 720), CC_32F, Scalar(20));
        ...
        CC_Assert( n0 > nx*2 && n255 > nx*2 );
        CC_Assert( nfmin > nfx*2 && nfmax > nfx*2 );
    }
};
...
TEST(Core_Rand, Regression_Stack_Corruption)
{
    ...
    CAutoBuffer<uchar> buffer(bufsz);
    size_t offset = 0;
    Mat_<CPoint2d> x(2, 3, (CPoint2d*)(buffer+offset)); offset += x CC_MAT_TOTAL()*x CC_ELEM_SIZE();
    double& param1 = *(double*)(buffer+offset); offset += sizeof(double);
    double& param2 = *(double*)(buffer+offset); offset += sizeof(double);
    param1 = -9; param2 = 2;

    theRNG().fill(x, RNG::NORMAL, param1, param2);
