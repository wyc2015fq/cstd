...
static bool cvTsCheckSparse(const CvSparseMat* m1, const CvSparseMat* m2, double eps)
{
    ...
    CvSparseNode* node1;
    int depth = CC_MAT_DEPTH(m1->tid);

    if( m1->heap->active_count != m2->heap->active_count ||
       m1->dims != m2->dims || CC_MAT_TYPE(m1->tid) != CC_MAT_TYPE(m2->tid) )
        return false;

    for( node1 = cvInitSparseMatIterator( m1, &it1 );
        node1 != 0; node1 = cvGetNextSparseNode( &it1 ))
    {
        uchar* v1 = (uchar*)CC_NODE_VAL(m1,node1);
        uchar* v2 = cvPtrND( m2, CC_NODE_IDX(m1,node1), 0, 0, &node1->hashval );
        if( !v2 )
            return false;
        if( depth == CC_8U || depth == CC_8S )
        {
            if( *v1 != *v2 )
                return false;
        }
        else if( depth == CC_16U || depth == CC_16S )
        {
...
protected:
    void run(int)
    {
        ...
        for( int idx = 0; idx < test_case_count; idx++ )
        {
            ...
            img_t test_mat(cvtest::randInt(rng)%30+1, cvtest::randInt(rng)%30+1, CC_MAKETYPE(depth, cn));

            rng0.fill(test_mat, CC_RAND_UNI, Scalar::all(ranges[depth][0]), Scalar::all(ranges[depth][1]));
            if( depth >= CC_32F )
            {
                exp(test_mat, test_mat);
                img_t test_mat_scale(test_mat.size(), test_mat CC_MAT_TYPE());
                rng0.fill(test_mat_scale, CC_RAND_UNI, Scalar::all(-1), Scalar::all(1));
                multiply(test_mat, test_mat_scale, test_mat);
            }

            CvSeq* seq = cvCreateSeq(test_mat CC_MAT_TYPE(), (int)sizeof(CvSeq),
                                     (int)test_mat CC_ELEM_SIZE(), storage);
            cvSeqPushMulti(seq, test_mat->tt.data, test_mat->cols*test_mat->rows);

            CvGraph* graph = cvCreateGraph( CC_ORIENTED_GRAPH,
            ...
            depth = cvtest::randInt(rng) % (CC_64F+1);
            ...
            MatND test_mat_nd(3, sz, CC_MAKETYPE(depth, cn));

            rng0.fill(test_mat_nd, CC_RAND_UNI, Scalar::all(ranges[depth][0]), Scalar::all(ranges[depth][1]));
            if( depth >= CC_32F )
            {
                exp(test_mat_nd, test_mat_nd);
                MatND test_mat_scale(test_mat_nd.dims, test_mat_nd.size, test_mat_nd CC_MAT_TYPE());
                rng0.fill(test_mat_scale, CC_RAND_UNI, Scalar::all(-1), Scalar::all(1));
            ...
            SparseMat test_sparse_mat = cvTsGetRandomSparseMat(4, ssz, cvtest::randInt(rng)%(CC_64F+1),
            ...
            fs << "test_list" << "[" << 0.0000000000001 << 2 << CC_PI << -3435345 << "2-502 2-029 3egegeg" <<
            ...
            img_t* m = (img_t*)fs["test_mat"].readObj();
            img_t _test_mat = test_mat;
            double max_diff = 0;
            img_t stub1, _test_stub1;
            cvReshape(m, &stub1, 1, 0);
            cvReshape(&_test_mat, &_test_stub1, 1, 0);
            vector<int> pt;

            if( !m || !CC_IS_MAT(m) || m->rows != test_mat->rows || m->cols != test_mat->cols ||
               cvtest::cmpEps( cvarrToMat(&stub1), cvarrToMat(&_test_stub1), &max_diff, 0, &pt, true) < 0 )
            {
            ...
            if( tl CC_MAT_TYPE() != FileNode::SEQ || tl.size() != 6 ||
               fabs((double)tl[0] - 0.0000000000001) >= DBL_EPSILON ||
               (int)tl[1] != 2 ||
               fabs((double)tl[2] - CC_PI) >= DBL_EPSILON ||
               (int)tl[3] != -3435345 ||
               (String)tl[4] != "2-502 2-029 3egegeg" ||
               tl[5] CC_MAT_TYPE() != FileNode::MAP || tl[5].size() != 3 ||
               (int)tl[5]["month"] != 12 ||
               (int)tl[5]["day"] != 31 ||
               (int)tl[5]["year"] != 1969 )
            {
            ...
            if( tm CC_MAT_TYPE() != FileNode::MAP || tm.size() != 5 ||
               real_x != 1 ||
               real_y != 2 ||
               real_width != 100 ||
               real_height != 200 ||
               tm_lbp CC_MAT_TYPE() != FileNode::SEQ ||
               tm_lbp.size() != 8 ||
               real_lbp_val != 0xb6 )
            {
...
class CC_MiscIOTest : public cvtest::BaseTest
{
public:
    CC_MiscIOTest() {}
    ~CC_MiscIOTest() {}
protected:
    void run(int)
    {
        ...
        for ( size_t i = 0u; i < 3u; i++ )
        {
            try
            {
                string fname = tempfile(suffix[i]);
                vector<int> mi, mi2, mi3, mi4;
                vector<img_t> mv, mv2, mv3, mv4;
                ...
                CC_Assert( mi2.empty() );
                CC_Assert( mv2.empty() );
                CC_Assert( cvtest::norm(img_t(mi3), img_t(mi4), CC_C) == 0 );
                CC_Assert( mv4.size() == 1 );
                double n = cvtest::norm(mv3[0], mv4[0], CC_C);
                CC_Assert( vudt2.empty() );
                CC_Assert( vudt3 == vudt4 );
                CC_Assert( n == 0 );
                CC_Assert( op1 == p1 );
                CC_Assert( op2 == p2 );
                CC_Assert( os1 == s1 );
...
TEST(Core_InputOutput, misc) { CC_MiscIOTest test; test.safe_run(); }

/*class CC_BigMatrixIOTest : public cvtest::BaseTest
{
public:
    CC_BigMatrixIOTest() {}
    ~CC_BigMatrixIOTest() {}
protected:
    void run(int)
    {
        try
        {
            ...
            img_t mat(M, N, CC_32F);
            rng.fill(mat, RNG::UNIFORM, 0, 1);
            FileStorage fs(tempfile(".xml"), FileStorage::WRITE);
            fs << "mat" << mat;
            fs.release();
        }
        catch(...)
...
TEST(Core_InputOutput, huge) { CC_BigMatrixIOTest test; test.safe_run(); }
*/
...
TEST(Core_globbing, accuracy)
{
    ...
    glob(patternLena, lenas, true);
    glob(patternLenaPng, pngLenas, true);
...
{
    std::string file = tempfile(".xml");
    FileStorage f(file, FileStorage::WRITE);
...
{
    FileStorage f("dummy.yml", FileStorage::WRITE | FileStorage::MEMORY);
...
TEST(Core_InputOutput, FileStorageSpaces)
{
    ...
    for (size_t i = 0; i < valueCount; i++) {
        EXPECT_NO_THROW(f << format("key%d", i) << values[i]);
    }
    FileStorage f2(f.releaseAndGetString(), FileStorage::READ | FileStorage::MEMORY);
    std::string valuesRead[valueCount];
    for (size_t i = 0; i < valueCount; i++) {
        EXPECT_NO_THROW(f2[format("key%d", i)] >> valuesRead[i]);
...
TEST(Core_InputOutput, filestorage_base64_basic)
{
    ...
    for (char const ** ptr = filenames; *ptr; ptr++)
    {
        ...
        img_t _em_out, _em_in;
        img_t _2d_out, _2d_in;
        img_t _nd_out, _nd_in;
        img_t _rd_out(64, 64, CC_64FC1), _rd_in;
        ...
        {   /* init */
            ...
            /* a normal mat */
            _2d_out = img_t(100, 100, CC_8UC3, cScalar(1U, 2U, 127U));
            for (int i = 0; i < _2d_out->rows; ++i)
                for (int j = 0; j < _2d_out->cols; ++j)
                    _2d_out.at<Vec3b>(i, j)[1] = (i + j) % 256;

            /* a 4d mat */
            const int Size[] = {4, 4, 4, 4};
            img_t _4d(4, Size, CC_64FC4, cScalar(0.888, 0.111, 0.666, 0.444));
            const CRange ranges[] = {
                CRange(0, 2),
                CRange(0, 2),
                CRange(1, 2),
            ...
            /* a random mat */
            randu(_rd_out, Scalar(0.0), Scalar(1.0));
        ...
        {   /* write */
            FileStorage fs(name, FileStorage::WRITE_BASE64);
        ...
        EXPECT_EQ(_em_in CC_MAT_DEPTH(), _em_out CC_MAT_DEPTH());
        ...
        EXPECT_EQ(_2d_in CC_MAT_DEPTH(), _2d_out CC_MAT_DEPTH());
        for(int i = 0; i < _2d_out->rows; ++i)
            for (int j = 0; j < _2d_out->cols; ++j)
                EXPECT_EQ(_2d_in.at<Vec3b>(i, j), _2d_out.at<Vec3b>(i, j));

        EXPECT_EQ(_nd_in->rows   , _nd_out->rows);
        EXPECT_EQ(_nd_in->cols   , _nd_out->cols);
        EXPECT_EQ(_nd_in.dims   , _nd_out.dims);
        EXPECT_EQ(_nd_in CC_MAT_DEPTH(), _nd_out CC_MAT_DEPTH());
        EXPECT_EQ(countNonZero(mean(_nd_in != _nd_out)), 0);

        EXPECT_EQ(_rd_in->rows   , _rd_out->rows);
        EXPECT_EQ(_rd_in->cols   , _rd_out->cols);
        EXPECT_EQ(_rd_in.dims   , _rd_out.dims);
        EXPECT_EQ(_rd_in CC_MAT_DEPTH(), _rd_out CC_MAT_DEPTH());
        EXPECT_EQ(countNonZero(mean(_rd_in != _rd_out)), 0);
...
TEST(Core_InputOutput, filestorage_base64_valid_call)
{
    ...
    for (char const ** ptr = filenames; *ptr; ptr++)
    {
        ...
        EXPECT_NO_THROW(
        {
            ...
            cvWriteStructBegin(*fs, "manydata", CC_NODE_SEQ);
        ...
        EXPECT_NO_THROW(
        {
            ...
            cvWriteStructBegin(*fs, "manydata", CC_NODE_SEQ);
            cvWriteString(*fs, 0, str_out.c_str(), 1);
            cvWriteStructBegin(*fs, 0, CC_NODE_SEQ | CC_NODE_FLOW, "binary");
            for (int i = 0; i < 10; i++)
                cvWriteRawData(*fs, rawdata.data(), static_cast<int>(rawdata.size()), "i");
            cvWriteStructEnd(*fs);
            cvWriteStructEnd(*fs);
...
TEST(Core_InputOutput, filestorage_base64_invalid_call)
{
    ...
    for (char const ** ptr = filenames; *ptr; ptr++)
    {
        ...
        EXPECT_ANY_THROW({
            FileStorage fs(name, FileStorage::WRITE);
            cvWriteStructBegin(*fs, "rawdata", CC_NODE_SEQ, "binary");
            cvWriteStructBegin(*fs, 0, CC_NODE_SEQ | CC_NODE_FLOW);
        });

        EXPECT_ANY_THROW({
            FileStorage fs(name, FileStorage::WRITE);
            cvWriteStructBegin(*fs, "rawdata", CC_NODE_SEQ);
            cvWriteStructBegin(*fs, 0, CC_NODE_SEQ | CC_NODE_FLOW);
...
TEST(Core_InputOutput, filestorage_json_comment)
{
    ...
    EXPECT_NO_THROW(
    {
        FileStorage fs(mem_str, FileStorage::READ | FileStorage::MEMORY);
        fs["key"] >> str;
        fs.release();
    });
...
TEST(Core_InputOutput, filestorage_vec_vec_io)
{
    ...
    for(size_t i = 0; i < formats.size(); i++)
    {
        ...
        std::vector<std::vector<img_t> > testMats;
