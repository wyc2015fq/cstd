...
template<typename R> struct TheTest
{
    ...
    TheTest & test_sqrt_abs()
    {
        ...
        for (int i = 0; i < R::nlanes; ++i)
        {
            EXPECT_COMPARE_EQ((float)sqrt(dataA[i]), (float)resB[i]);
            EXPECT_COMPARE_EQ(1/(float)sqrt(dataA[i]), (float)resC[i]);
    ...
    TheTest & test_min_max()
    {
        ...
        for (int i = 0; i < R::nlanes; ++i)
        {
            EXPECT_EQ(MIN(dataA[i], dataB[i]), resC[i]);
            EXPECT_EQ(MAX(dataA[i], dataB[i]), resD[i]);
        }
    ...
    TheTest & test_float_math()
    {
        ...
        for (int i = 0; i < R::nlanes; ++i)
        {
            EXPECT_EQ(cRound(data1[i]), resB[i]);
            EXPECT_EQ((typename Ri::lane_type)data1[i], resC[i]);
            EXPECT_EQ(cFloor(data1[i]), resD[i]);
            EXPECT_EQ(cCeil(data1[i]), resE[i]);

            EXPECT_COMPARE_EQ(sqrt(data1[i]*data1[i] + data2[i]*data2[i]), resF[i]);
    ...
    TheTest & test_float_cvt32()
    {
        ...
        Rt b = v_cvt_f32(a);
        Data<Rt> resB = b;
        int n = MIN<int>(Rt::nlanes, R::nlanes);
        for (int i = 0; i < n; ++i)
        {
