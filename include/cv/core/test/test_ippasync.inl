...
PARAM_TEST_CASE(IPPAsyncShared, Channels, hppAccelType)
{
    ...
    virtual void SetUp()
    {
        ...
        type=CC_MAKE_TYPE(CC_8U, GET_PARAM(0));
    }
    ...
    void generateTestData()
    {
        ...
        if (pitch!=0 && size!=0)
        {
            ...
            matrix = img_t(matrix_Size.height, matrix_Size.width, type, pData, pitch);
        }
...
INSTANTIATE_TEST_CASE_P(IppATest, IPPAsync, Combine(Values(CC_8U, CC_16U, CC_16S, CC_32F),
                                                   Values(1, 2, 3, 4),
                                                   Values( HPP_ACCEL_TYPE_CPU, HPP_ACCEL_TYPE_GPU)));
