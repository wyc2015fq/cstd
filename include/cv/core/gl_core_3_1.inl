...
#include "gl_core_3_1.hpp"
    ...
    #if defined(_WIN32)
        #define CC_GL_GET_PROC_ADDRESS(name) WinGetProcAddress(name)
    #elif defined(__APPLE__)
        #define CC_GL_GET_PROC_ADDRESS(name) AppleGLGetProcAddress(name)
    #elif defined(__sgi) || defined(__sun)
        #define CC_GL_GET_PROC_ADDRESS(name) SunGetProcAddress(name)
    #else // GLX
        #include <GL/glx.h>

        #define CC_GL_GET_PROC_ADDRESS(name) glXGetProcAddressARB((const GLubyte*) name)
    #endif

    static void* IntGetProcAddress(const char* name)
    {
        void* func =  (void*) CC_GL_GET_PROC_ADDRESS(name);
        if (!func)
        {
            CC_Error(CC_OpenGlApiCallError, format("Can't load OpenGL extension [%s]", name) );
        }
        return func;
    }
#else
    ...
    {
        CC_Error(CC_OpenGlNotSupported, "The library is compiled without OpenGL support");
        return 0;
    }
#endif
