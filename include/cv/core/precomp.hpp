...
typedef void (*UnaryFunc)(const uchar* src1, size_t step1,
                       uchar* dst, size_t step, ISize sz,
                       void*);
...
inline ISize getContinuousSize_( int flags, int cols, int rows, int widthScale )
{
    int64 sz = (int64)cols * rows * widthScale;
    return (flags & Mat::CONTINUOUS_FLAG) != 0 &&
        (int)sz == sz ? ISize((int)sz, 1) : ISize(cols * widthScale, rows);
}

inline ISize getContinuousSize( const Mat& m1, int widthScale=1 )
{
    return getContinuousSize_(m1.flags,
                              m1.cols, m1.rows, widthScale);
}

inline ISize getContinuousSize( const Mat& m1, const Mat& m2, int widthScale=1 )
{
    return getContinuousSize_(m1.flags & m2.flags,
                              m1.cols, m1.rows, widthScale);
}

inline ISize getContinuousSize( const Mat& m1, const Mat& m2,
...
inline bool checkScalar(const Mat& sc, int atype, int sckind, int akind)
{
    ...
    if( akind == _InputArray::MATX && sckind != _InputArray::MATX )
        return false;
    return sz == ISize(1, 1) || sz == ISize(1, cn) || sz == ISize(cn, 1) ||
           (sz == ISize(1, 4) && sc.type() == CV_64F && cn <= 4);
}
