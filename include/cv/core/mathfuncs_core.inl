...
static const float atan2_p1 = 0.9997878412794807f*(float)(180/CC_PI);
static const float atan2_p3 = -0.3258083974640975f*(float)(180/CC_PI);
static const float atan2_p5 = 0.1555786518463281f*(float)(180/CC_PI);
static const float atan2_p7 = -0.04432655554792128f*(float)(180/CC_PI);
...
{
    int i = 0;
    T scale = angleInDegrees ? 1 : static_cast<T>(CC_PI/180);
...
void magnitude32f(const float* x, const float* y, float* mag, int len)
{
    ...
    CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippsMagnitude_32f, x, y, mag, len) >= 0);
...
void magnitude64f(const double* x, const double* y, double* mag, int len)
{
    ...
    CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippsMagnitude_64f, x, y, mag, len) >= 0);
...
void invSqrt32f(const float* src, float* dst, int len)
{
    ...
    CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippsInvSqrt_32f_A21, src, dst, len) >= 0);
...
void invSqrt64f(const double* src, double* dst, int len)
{
    ...
    CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippsInvSqrt_64f_A50, src, dst, len) >= 0);
...
void sqrt32f(const float* src, float* dst, int len)
{
    ...
    CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippsSqrt_32f_A21, src, dst, len) >= 0);
...
void sqrt64f(const double* src, double* dst, int len)
{
    ...
    CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippsSqrt_64f_A50, src, dst, len) >= 0);
...
// CC_AVX is not used here, because generated code is faster in non-AVX mode.
...
void exp32f(const float *src, float *dst, int n)
{
    ...
    CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippsExp_32f_A21, src, dst, n) >= 0);
...
void exp64f(const double *src, double *dst, int n)
{
    ...
    CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippsExp_64f_A50, src, dst, n) >= 0);
...
void log32f(const float *src, float *dst, int n)
{
    ...
    CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippsLn_32f_A21, src, dst, n) >= 0);
...
void log64f(const double *src, double *dst, int n)
{
    ...
    CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippsLn_64f_A50, src, dst, n) >= 0);
...
void exp32f( const float *_x, float *y, int n )
{
    ...
    CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippsExp_32f_A21, _x, y, n) >= 0);
...
(((((x) + A1)*(x) + A2)*(x) + A3)*(x) + A4)
    ...
    if( n >= 8 )
    {
        ...
        ushort CC_DECL_ALIGNED(16) tab_idx[8];
...
void exp64f( const double *_x, double *y, int n )
{
    ...
    CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippsExp_64f_A50, _x, y, n) >= 0);
...
#define EXPPOLY(x)  (((((A0*(x) + A1)*(x) + A2)*(x) + A3)*(x) + A4)*(x) + A5)
    ...
    int CC_DECL_ALIGNED(16) tab_idx[4];
...
static const double CC_DECL_ALIGNED(16) icvLogTab[] = {
...
void log32f( const float *_x, float *y, int n )
{
    ...
    CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippsLn_32f_A21, _x, y, n) >= 0);
...
void log64f( const double *x, double *y, int n )
{
    ...
    CC_IPP_RUN_FAST(CC_INSTRUMENT_FUN_IPP(ippsLn_64f_A50, x, y, n) >= 0);
