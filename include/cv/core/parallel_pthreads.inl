...
struct work_load
{
    ...
    work_load(const CRange& range, const ParallelLoopBody& body, int nstripes)
    {
        set(range, body, nstripes);
    }

    void set(const CRange& range, const ParallelLoopBody& body, unsigned int nstripes)
    {
        m_body = &body;
        m_range = &range;

        //ensure that nstripes not larger than range length
        m_nstripes = MIN( unsigned(m_range->end - m_range->start) , nstripes);
        ...
        m_nstripes = MIN(m_nstripes, unsigned(((m_range->end - m_range->start - 1)/m_block_size) + 1) );
    }

    const ParallelLoopBody* m_body;
    const CRange*            m_range;
...
public:
    ...
    {
        CC_SINGLETON_LAZY_INIT_REF(ThreadManager, new ThreadManager())
    }
    ...
    void run(const CRange& range, const ParallelLoopBody& body, double nstripes);
...
private:
    ...
    TLSData<work_thread_t> m_is_work_thread;

    ThreadManagerPoolState m_pool_state;
};

const char ThreadManager::m_env_name[] = "OPENCC_FOR_THREADS_NUM";
...
{
    unsigned int m_current_pos = CC_XADD(&m_parent->m_task_position, 1);
    ...
    while(m_current_pos < load.m_nstripes)
    {
        ...
        int end = MIN(start + load.m_block_size, load.m_range->end);

        load.m_body->operator()(CRange(start, end));

        m_current_pos = CC_XADD(&m_parent->m_task_position, 1);
    }
}
...
void ThreadManager::run(const CRange& range, const ParallelLoopBody& body, double nstripes)
{
        ...
        (range.end - range.start > 1) && (nstripes <= 0 || nstripes >= 1.5) )
    {
        ...
        if(!res)
        {
            if(initPool())
            {
                ...
                nstripes = MIN(nstripes, max_stripes);
                ...
                m_work_load.set(range, body, cCeil(nstripes));
...
void ThreadManager::notify_complete()
{
    ...
    unsigned int comp = CC_XADD(&m_num_of_completed_tasks, 1);
