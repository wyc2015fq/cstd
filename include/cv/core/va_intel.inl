...
#  include <va/va.h>
#else  // HAVE_VA
#  define NO_VA_SUPPORT_ERROR CC_ErrorNoReturn(CC_StsBadFunc, "OpenCV was build without VA support (libva)")
#endif // HAVE_VA
...
# if (defined(HAVE_VA_INTEL) && defined(HAVE_OPENCL))
    ...
    if (tryInterop)
    {
        ...
        if (status != CL_SUCCESS)
            CC_Error(CC_OpenCLInitError, "OpenCL: Can't get platform Id list");
...
static void copy_convert_nv12_to_bgr(const VAImage& image, const unsigned char* buffer, img_t& bgr)
{
    ...
    for (int y = 0; y < bgr->rows; y += 2)
    {
        ...
        for (int x = 0; x < bgr->cols; x += 2)
        {
            ...
            Y0 = MAX(0.0f, Y0 - d1) * coeffs[0];
            Y1 = MAX(0.0f, Y1 - d1) * coeffs[0];
            Y2 = MAX(0.0f, Y2 - d1) * coeffs[0];
            Y3 = MAX(0.0f, Y3 - d1) * coeffs[0];
...
static void copy_convert_bgr_to_nv12(const VAImage& image, const img_t& bgr, unsigned char* buffer)
{
    const float d1 = 16.0f;
    const float d2 = 128.0f;
...
void convertToVASurface(VADisplay display, const img_t* src, VASurfaceID surface, Size size)
{
    (void)display; (void)src; (void)surface; (void)size;
#if !defined(HAVE_VA)
    NO_VA_SUPPORT_ERROR;
#else  // !HAVE_VA
    const int stype = CC_8UC3;

    int srcType = src CC_MAT_TYPE();
    CC_Assert(srcType == stype);

    Size srcSize = cvGetSize(src);
    CC_Assert(srcSize.width == size.width && srcSize.height == size.height);
...
# if (defined(HAVE_VA_INTEL) && defined(HAVE_OPENCL))
    if (contextInitialized)
    {
        ...
        if (status != CL_SUCCESS)
            CC_Error(CC_OpenCLApiCallError, "OpenCL: clCreateFromVA_APIMediaSurfaceINTEL failed (Y plane)");
        cl_mem clImageUV = clCreateFromVA_APIMediaSurfaceINTEL(context, CL_MEM_WRITE_ONLY, &surface, 1, &status);
        if (status != CL_SUCCESS)
            CC_Error(CC_OpenCLApiCallError, "OpenCL: clCreateFromVA_APIMediaSurfaceINTEL failed (UV plane)");
        ...
        if (status != CL_SUCCESS)
            CC_Error(CC_OpenCLApiCallError, "OpenCL: clEnqueueAcquireVA_APIMediaSurfacesINTEL failed");
        ...
        if (status != CL_SUCCESS)
            CC_Error(CC_OpenCLApiCallError, "OpenCL: clEnqueueReleaseVA_APIMediaSurfacesINTEL failed");
        ...
        if (status != CL_SUCCESS)
            CC_Error(CC_OpenCLApiCallError, "OpenCL: clReleaseMem failed (Y plane)");
        status = clReleaseMemObject(clImageUV);
        if (status != CL_SUCCESS)
            CC_Error(CC_OpenCLApiCallError, "OpenCL: clReleaseMem failed (UV plane)");
    }
    else
# endif // HAVE_VA_INTEL && HAVE_OPENCL
    {
        ...
        if (status != VA_STATUS_SUCCESS)
            CC_Error(CC_StsError, "VA-API: vaSyncSurface failed");

        VAImage image;
        status = vaDeriveImage(display, surface, &image);
        if (status != VA_STATUS_SUCCESS)
            CC_Error(CC_StsError, "VA-API: vaDeriveImage failed");

        unsigned char* buffer = 0;
        status = vaMapBuffer(display, image.buf, (void **)&buffer);
        if (status != VA_STATUS_SUCCESS)
            CC_Error(CC_StsError, "VA-API: vaMapBuffer failed");

        CC_Assert(image.format.fourcc == VA_FOURCC_NV12);

        copy_convert_bgr_to_nv12(image, m, buffer);

        status = vaUnmapBuffer(display, image.buf);
        if (status != VA_STATUS_SUCCESS)
            CC_Error(CC_StsError, "VA-API: vaUnmapBuffer failed");

        status = vaDestroyImage(display, image.image_id);
        if (status != VA_STATUS_SUCCESS)
            CC_Error(CC_StsError, "VA-API: vaDestroyImage failed");
    }
#endif  // !HAVE_VA
}

void convertFromVASurface(VADisplay display, VASurfaceID surface, Size size, img_t* dst)
{
    (void)display; (void)surface; (void)dst; (void)size;
#if !defined(HAVE_VA)
    NO_VA_SUPPORT_ERROR;
#else  // !HAVE_VA
    const int dtype = CC_8UC3;

    // TODO Need to specify ACCESS_WRITE here somehow to prevent useless data copying!
    dst cvSetMat(size, dtype);
