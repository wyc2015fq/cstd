...
{                                                                                          \
    ...
    VSplit2()                                                                              \
    {                                                                                      \
        support = checkHardwareSupport(CC_CPU_SSE2);                                       \
    }                                                                                      \
                                                                                           \
...
hal_split_( const T* src, T** dst, int len, int cn )
{
    int k = cn % 4 ? cn % 4 : 4;
    int i, j;
    if( k == 1 )
...
static void hal_split8u(const uchar* src, uchar** dst, int len, int cn )
{
    CALL_HAL(split8u, cv_hal_split8u, (src, dst, len, cn))
    hal_split_(src, dst, len, cn);
}
static void hal_split16u(const ushort* src, ushort** dst, int len, int cn )
{
    CALL_HAL(split16u, cv_hal_split16u, (src, dst, len, cn))
    hal_split_(src, dst, len, cn);
}
static void hal_split32s(const int* src, int** dst, int len, int cn )
{
    CALL_HAL(split32s, cv_hal_split32s, (src, dst, len, cn))
    hal_split_(src, dst, len, cn);
}
static void hal_split64s(const int64* src, int64** dst, int len, int cn )
{
