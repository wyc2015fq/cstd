...
static bool _openvx_cvt(const T* src, size_t sstep,
                        DT* dst, size_t dstep, ISize continuousISize)
{
    using namespace ivx;

    if(!(continuousISize.width > 0 && continuousISize.height > 0))
    {
        return true;
    }
    ...
    ISize imgISize = continuousISize;
    if(continuousISize.height == 1)
    {
        if(sstep / sizeof(T) == dstep / sizeof(DT) && sstep / sizeof(T) > 0 &&
           continuousISize.width % (sstep / sizeof(T)) == 0)
        {
            //continuous n-lines image
            imgISize.width  = sstep / sizeof(T);
            imgISize.height = continuousISize.width / (sstep / sizeof(T));
        }
        else
        {
            //1-row image with possibly incorrect step
            sstep = continuousISize.width * sizeof(T);
            dstep = continuousISize.width * sizeof(DT);
        }
    }

    int srcType = DataType<T>::type, dstType = DataType<DT>::type;

    if (ovx::skipSmallImages<VX_KERNEL_CONVERTDEPTH>(imgISize.width, imgISize.height))
        return false;
    ...
    try
    {
        ...
        Image srcImage = Image::createFromHandle(context, Image::matTypeToFormat(srcType),
                                                 Image::createAddressing(imgISize.width, imgISize.height,
...
static bool openvx_cvt(const T* src, size_t sstep,
                       DT* dst, size_t dstep, ISize size)
{
...
template<>                                                                    \
bool openvx_cvt(const T *src, size_t sstep, DT *dst, size_t dstep, ISize size) \
{                                                                             \
...
cpy_( const T* src, size_t sstep, T* dst, size_t dstep, ISize size )
{
    sstep /= sizeof(src[0]);
    dstep /= sizeof(dst[0]);
...
static void cvt##suffix( const stype* src, size_t sstep, const uchar*, size_t, \
                         dtype* dst, size_t dstep, ISize size, double*) \
{ \
    CC_IPP_RUN(src && dst, CC_INSTRUMENT_FUN_IPP(ippiConvert_##ippFavor, src, (int)sstep, dst, (int)dstep, ippiISize(size.width, size.height)) >= 0) \
...
static void cvt##suffix( const stype* src, size_t sstep, const uchar*, size_t, \
                         dtype* dst, size_t dstep, ISize size, double*) \
{ \
    CC_IPP_RUN(src && dst, CC_INSTRUMENT_FUN_IPP(ippiConvert_##ippFavor, src, (int)sstep, dst, (int)dstep, ippiISize(size.width, size.height), ippRndFinancial, 0) >= 0) \
    cvt_(src, sstep, dst, dstep, size); \
}
#else
#define DEF_CVT_FUNC_F(suffix, stype, dtype, ippFavor) \
...
static void cvt##suffix( const stype* src, size_t sstep, const uchar*, size_t, \
                         stype* dst, size_t dstep, ISize size, double*) \
{ \
...
static bool ipp_convertTo(Mat &src, Mat &dst, double alpha, double beta)
{
    ...
    try
    {
        ...
        if(src.dims <= 2)
        {
            ISize sz = getContinuousISize(src, dst, channels);

            iwSrc.Init(ippiISize(sz), srcDepth, 1, NULL, (void*)src.ptr(), src.step);
            iwDst.Init(ippiISize(sz), dstDepth, 1, NULL, (void*)dst.ptr(), dst.step);

            CC_INSTRUMENT_FUN_IPP(::ipp::iwiScale, iwSrc, iwDst, alpha, beta, ::ipp::IwiScaleParams(mode));
        }
        else
        {
            ...
            iwSrc.Init(ippiISize(it.size, 1), srcDepth, channels);
            iwDst.Init(ippiISize(it.size, 1), dstDepth, channels);
    ...
    return true;
#else
    CC_UNUSED(src); CC_UNUSED(dst); CC_UNUSED(alpha); CC_UNUSED(beta);
    return false;
#endif
}
...
void convertTo(img_t* src, img_t* _dst, int _type, double alpha, double beta)
{
    ...
    if( _type < 0 )
        ...
        _type = CC_MAKETYPE(CC_MAT_DEPTH(_type), channels());

    int sdepth = src->depth(), ddepth = CC_MAT_DEPTH(_type);
    ...
    CC_IPP_RUN_FAST(ipp_convertTo(src, dst, alpha, beta ));
    ...
    if( dims <= 2 )
    {
        ISize sz = getContinuousISize(src, dst, cn);
...
template<typename T, typename DT> static void
cvtScaleHalf_( const T* src, size_t sstep, DT* dst, size_t dstep, ISize size);

template<> void
cvtScaleHalf_<float, short>( const float* src, size_t sstep, short* dst, size_t dstep, ISize size )
{
    CC_CPU_CALL_FP16_(cvtScaleHalf_SIMD32f16f, (src, sstep, dst, dstep, size));

#if !defined(CC_CPU_COMPILE_FP16)
    sstep /= sizeof(src[0]);
    dstep /= sizeof(dst[0]);
...
template<> void
cvtScaleHalf_<short, float>( const short* src, size_t sstep, float* dst, size_t dstep, ISize size )
{
    CC_CPU_CALL_FP16_(cvtScaleHalf_SIMD16f32f, (src, sstep, dst, dstep, size));
...
dtype* dst, size_t dstep, ISize size) \
{ \
...
static bool ocl_convertFp16( InputArray _src, img_t* _dst, int ddepth )
{
    int type = _src.type(), cn = CC_MAT_CN(type);

    _dst.createSameISize( _src, CC_MAKETYPE(ddepth, cn) );
    int kercn = 1;
    int rowsPerWI = 1;
    String build_opt = format("-D HALF_SUPPORT -D dstT=%s -D srcT=%s -D rowsPerWI=%d%s",
                           ddepth == CC_16S ? "half" : "float",
                           ddepth == CC_16S ? "float" : "half",
                           rowsPerWI,
                           ddepth == CC_16S ? " -D FLOAT_TO_HALF " : "");
    ...
    ocl::KernelArg srcarg = ocl::KernelArg::ReadOnlyNoISize(src),
...
void cv::convertFp16( InputArray _src, img_t* _dst)
{
    CC_INSTRUMENT_REGION()
    ...
    case CC_16S:
        ...
        break;
    default:
        CC_Error(Error::StsUnsupportedFormat, "Unsupported input depth");
        return;
    }
    ...
    int type = CC_MAKETYPE(ddepth, src.channels());
