...
static bool ocl_math_op(const img_t* _src1, const img_t* _src2, img_t* _dst, int oclop)
{
    ...
    if (!double_support && depth == CC_64F)
        return false;
    int rowsPerWI = d.isIntel() ? 4 : 1;

    ocl::Kernel k("KF", ocl::core::arithm_oclsrc,
                  format("-D %s -D %s -D dstT=%s -D rowsPerWI=%d%s", _src2.empty() ? "UNARY_OP" : "BINARY_OP",
                         oclop2str[oclop], ocl::typeToStr(CC_MAKE_TYPE(depth, kercn)), rowsPerWI,
                         double_support ? " -D DOUBLE_SUPPORT" : ""));
    if (k.empty())
        return false;

    UMat src1 = _src1.getUMat(), src2 = _src2.getUMat();
    _dst cvSetMat(src1.size(), type);
...
void magnitude( const img_t* src1, const img_t* src2, img_t* dst )
{
    CC_INSTRUMENT_REGION()

    int type = src1 CC_MAT_TYPE(), depth = src1 CC_MAT_DEPTH(), cn = src1 CC_MAT_CN();
    CC_Assert( src1.size() == src2.size() && type == src2 CC_MAT_TYPE() && (depth == CC_32F || depth == CC_64F));

    CC_OCL_RUN(dst.isUMat() && src1.dims() <= 2 && src2.dims() <= 2,
               ocl_math_op(src1, src2, dst, OCL_OP_MAG))

    img_t X = src1, Y = src2;
    dst cvSetMat(X.dims, X.size, X CC_MAT_TYPE());
    img_t Mag = dst;

    const img_t* arrays[] = {&X, &Y, &Mag, 0};
    uchar* ptrs[3];
    ...
    for( size_t i = 0; i < it.nplanes; i++, ++it )
    {
        if( depth == CC_32F )
        {
            ...
            hal_magnitude32f( x, y, mag, len );
        }
        else
        {
            const double *x = (const double*)ptrs[0], *y = (const double*)ptrs[1];
            double *mag = (double*)ptrs[2];
            hal_magnitude64f( x, y, mag, len );
        }
    }
}

void phase( const img_t* src1, const img_t* src2, img_t* dst, bool angleInDegrees )
{
    CC_INSTRUMENT_REGION()
    ...
    img_t X = src1, Y = src2;
    dst cvSetMat( X.dims, X.size, type );
    img_t Angle = dst;

    const img_t* arrays[] = {&X, &Y, &Angle, 0};
    uchar* ptrs[3];
    NAryMatIterator it(arrays, ptrs);
    int j, total = (int)(it.size*cn), blockSize = total;
    size_t esz1 = X CC_ELEM_SIZE1();
    for( size_t i = 0; i < it.nplanes; i++, ++it )
    {
        for( j = 0; j < total; j += blockSize )
        {
            ...
            if( depth == CC_32F )
            {
                ...
                hal_fastAtan32f( y, x, angle, len, angleInDegrees );
            }
            else
            {
                const double *x = (const double*)ptrs[0], *y = (const double*)ptrs[1];
                double *angle = (double*)ptrs[2];
                hal_fastAtan64f(y, x, angle, len, angleInDegrees);
            }
            ptrs[0] += len*esz1;
            ptrs[1] += len*esz1;
            ptrs[2] += len*esz1;
...
static bool ocl_cartToPolar( const img_t* _src1, const img_t* _src2,
                             img_t* _dst1, img_t* _dst2, bool angleInDegrees )
{
    const ocl::Device & d = ocl::Device::getDefault();
    int type = _src1 CC_MAT_TYPE(), depth = CC_MAT_DEPTH(type), cn = CC_MAT_CN(type),
    ...
    if ( !(_src1.dims() <= 2 && _src2.dims() <= 2 &&
           (depth == CC_32F || depth == CC_64F) && type == _src2 CC_MAT_TYPE()) ||
         (depth == CC_64F && !doubleSupport) )
        return false;

    ocl::Kernel k("KF", ocl::core::arithm_oclsrc,
                  format("-D BINARY_OP -D dstT=%s -D depth=%d -D rowsPerWI=%d -D OP_CTP_%s%s",
                         ocl::typeToStr(CC_MAKE_TYPE(depth, 1)),
    ...
    UMat src1 = _src1.getUMat(), src2 = _src2.getUMat();
    Size size = src1.size();
    CC_Assert( size == src2.size() );

    _dst1 cvSetMat(size, type);
    _dst2 cvSetMat(size, type);
...
void cartToPolar( const img_t* src1, const img_t* src2,
                  img_t* dst1, img_t* dst2, bool angleInDegrees )
{
    CC_INSTRUMENT_REGION()

    CC_OCL_RUN(dst1.isUMat() && dst2.isUMat(),
            ocl_cartToPolar(src1, src2, dst1, dst2, angleInDegrees))

    img_t X = src1, Y = src2;
    int type = X CC_MAT_TYPE(), depth = X CC_MAT_DEPTH(), cn = X CC_MAT_CN();
    CC_Assert( X.size == Y.size && type == Y CC_MAT_TYPE() && (depth == CC_32F || depth == CC_64F));
    dst1 cvSetMat( X.dims, X.size, type );
    dst2 cvSetMat( X.dims, X.size, type );
    img_t Mag = dst1, Angle = dst2;

    const img_t* arrays[] = {&X, &Y, &Mag, &Angle, 0};
    uchar* ptrs[4];
    NAryMatIterator it(arrays, ptrs);
    int j, total = (int)(it.size*cn), blockSize = MIN(total, ((BLOCK_SIZE+cn-1)/cn)*cn);
    size_t esz1 = X CC_ELEM_SIZE1();
    ...
    for( size_t i = 0; i < it.nplanes; i++, ++it )
    {
        for( j = 0; j < total; j += blockSize )
        {
            ...
            else
            {
                ...
                hal_magnitude64f(x, y, (double*)ptrs[2], len);
                hal_fastAtan64f(y, x, angle, len, angleInDegrees);
            }
            ptrs[0] += len*esz1;
            ptrs[1] += len*esz1;
                        ...
                        int len, int angle_in_degrees )
{
    ...
    static const double k2 = (2*CC_PI)/N;
...
static bool ocl_polarToCart( const img_t* _mag, const img_t* _angle,
                             img_t* _dst1, img_t* _dst2, bool angleInDegrees )
{
    const ocl::Device & d = ocl::Device::getDefault();
    int type = _angle CC_MAT_TYPE(), depth = CC_MAT_DEPTH(type), cn = CC_MAT_CN(type),
            rowsPerWI = d.isIntel() ? 4 : 1;
    bool doubleSupport = d.doubleFPConfig() > 0;
    ...
    ocl::Kernel k("KF", ocl::core::arithm_oclsrc,
                  format("-D dstT=%s -D rowsPerWI=%d -D depth=%d -D BINARY_OP -D OP_PTC_%s%s",
                         ocl::typeToStr(CC_MAKE_TYPE(depth, 1)), rowsPerWI,
...
void polarToCart( const img_t* src1, const img_t* src2,
                  img_t* dst1, img_t* dst2, bool angleInDegrees )
{
    CC_INSTRUMENT_REGION()

    int type = src2 CC_MAT_TYPE(), depth = CC_MAT_DEPTH(type), cn = CC_MAT_CN(type);
    CC_Assert((depth == CC_32F || depth == CC_64F) && (src1.empty() || src1 CC_MAT_TYPE() == type));

    CC_OCL_RUN(!src1.empty() && src2.dims() <= 2 && dst1.isUMat() && dst2.isUMat(),
               ocl_polarToCart(src1, src2, dst1, dst2, angleInDegrees))

    img_t Mag = src1, Angle = src2;
    CC_Assert( Mag.empty() || Angle.size == Mag.size);
    dst1 cvSetMat( Angle.dims, Angle.size, type );
    dst2 cvSetMat( Angle.dims, Angle.size, type );
    img_t X = dst1, Y = dst2;

#if defined(HAVE_IPP)
    CC_IPP_CHECK()
    {
        if (Mag CC_IS_CONT_MAT() && Angle CC_IS_CONT_MAT() && X CC_IS_CONT_MAT() && Y CC_IS_CONT_MAT() && !angleInDegrees)
        {
            typedef IppStatus (C_STDCALL * IppsPolarToCart)(const void * pSrcMagn, const void * pSrcPhase,
                                                             void * pDstRe, void * pDstIm, int len);
            IppsPolarToCart ippsPolarToCart =
            depth == CC_32F ? (IppsPolarToCart)ippsPolarToCart_32f :
            depth == CC_64F ? (IppsPolarToCart)ippsPolarToCart_64f : 0;
            CC_Assert(ippsPolarToCart != 0);

            IppStatus status = CC_INSTRUMENT_FUN_IPP(ippsPolarToCart, Mag->tt.data, Angle->tt.data, X->tt.data, Y->tt.data, static_cast<int>(cn * X CC_MAT_TOTAL()));
            if (status >= 0)
            {
    ...
    const img_t* arrays[] = {&Mag, &Angle, &X, &Y, 0};
    uchar* ptrs[4];
    NAryMatIterator it(arrays, ptrs);
    FREE(); float* = MALLOC(float, ) _buf;
    float* buf[2] = {0, 0};
    int j, k, total = (int)(it.size*cn), blockSize = MIN(total, ((BLOCK_SIZE+cn-1)/cn)*cn);
    size_t esz1 = Angle CC_ELEM_SIZE1();
...
void exp( const img_t* _src, img_t* _dst )
{
    CC_INSTRUMENT_REGION()

    int type = _src CC_MAT_TYPE(), depth = _src CC_MAT_DEPTH(), cn = _src CC_MAT_CN();
    CC_Assert( depth == CC_32F || depth == CC_64F );

    CC_OCL_RUN(_dst.isUMat() && _src.dims() <= 2,
               ocl_math_op(_src, NULL, _dst, OCL_OP_EXP))
    ...
    {
        if( depth == CC_32F )
            hal_exp32f((const float*)ptrs[0], (float*)ptrs[1], len);
        else
            hal_exp64f((const double*)ptrs[0], (double*)ptrs[1], len);
    }
}
...
void log( const img_t* _src, img_t* _dst )
{
    CC_INSTRUMENT_REGION()
    ...
    CC_OCL_RUN( _dst.isUMat() && _src.dims() <= 2,
                ocl_math_op(_src, NULL, _dst, OCL_OP_LOG))
    ...
    {
        if( depth == CC_32F )
            hal_log32f( (const float*)ptrs[0], (float*)ptrs[1], len );
        else
            hal_log64f( (const double*)ptrs[0], (double*)ptrs[1], len );
    }
}
...
static bool ocl_pow(const img_t* _src, double power, img_t* _dst,
...
static void InvSqrt_32f(const float* src, float* dst, int n) { hal_invSqrt32f(src, dst, n); }
static void InvSqrt_64f(const double* src, double* dst, int n) { hal_invSqrt64f(src, dst, n); }
static void Sqrt_32f(const float* src, float* dst, int n) { hal_sqrt32f(src, dst, n); }
static void Sqrt_64f(const double* src, double* dst, int n) { hal_sqrt64f(src, dst, n); }

void pow( const img_t* _src, double power, img_t* _dst )
{
    CC_INSTRUMENT_REGION()

    int type = _src CC_MAT_TYPE(), depth = CC_MAT_DEPTH(type),
            cn = CC_MAT_CN(type), ipower = cRound(power);
    ...
    if( is_ipower
#ifdef HAVE_OPENCL
            && !(useOpenCL && ocl::Device::getDefault().isIntel() && depth != CC_64F)
#endif
      )
    {
        switch( ipower )
    ...
    CC_OCL_RUN(useOpenCL, ocl_pow(_src, power, _dst, is_ipower, ipower))
    ...
    else if( fabs(fabs(power) - 0.5) < DBL_EPSILON )
    {
        MathFunc func = power < 0 ?
            (depth == CC_32F ? (MathFunc)InvSqrt_32f : (MathFunc)InvSqrt_64f) :
            (depth == CC_32F ? (MathFunc)Sqrt_32f : (MathFunc)Sqrt_64f);
    ...
    {
        int j, k, blockSize = MIN(len, ((BLOCK_SIZE + cn-1)/cn)*cn);
        size_t esz1 = src CC_ELEM_SIZE1();
        ...
        inf32.i = 0x7f800000;
        nan32.i = 0x7fffffff;
        inf64.i = CC_BIG_INT(0x7FF0000000000000);
        nan64.i = CC_BIG_INT(0x7FFFFFFFFFFFFFFF);
        ...
        for( size_t i = 0; i < it.nplanes; i++, ++it )
        {
            ...
            {
                int bsz = MIN(len - j, blockSize);

#if defined(HAVE_IPP)
                CC_IPP_CHECK()
                {
                    IppStatus status = depth == CC_32F ?
                    CC_INSTRUMENT_FUN_IPP(ippsPowx_32f_A21, (const float*)ptrs[0], (float)power, (float*)ptrs[1], bsz) :
                    CC_INSTRUMENT_FUN_IPP(ippsPowx_64f_A50, (const double*)ptrs[0], (double)power, (double*)ptrs[1], bsz);
...
void sqrt(const img_t* a, img_t* b)
{
    CC_INSTRUMENT_REGION()
...
template<> struct mat_type_assotiations<CC_8U>
{
...
template<> struct mat_type_assotiations<CC_8S>
{
...
template<> struct mat_type_assotiations<CC_16U>
{
    typedef unsigned short type;
    static const type min_allowable = 0x0;
    static const type max_allowable = USHRT_MAX;
};
template<> struct mat_type_assotiations<CC_16S>
{
...
template<> struct mat_type_assotiations<CC_32S>
{
    typedef int type;
...
template<int depth>
static bool checkIntegerRange(img_t src, Point& bad_pt, int minVal, int maxVal)
{
    ...
    img_t as_one_channel = src cvReshape(,1,0);
...
typedef bool (*check_range_function)(img_t src, Point& bad_pt, int minVal, int maxVal);

check_range_function check_range_functions[] =
{
    &checkIntegerRange<CC_8U>,
    &checkIntegerRange<CC_8S>,
    &checkIntegerRange<CC_16U>,
    &checkIntegerRange<CC_16S>,
    &checkIntegerRange<CC_32S>
};

bool checkRange(const img_t* _src, bool quiet, Point* pt, double minVal, double maxVal)
{
    CC_INSTRUMENT_REGION()
    ...
    if ( src.dims > 2 )
    {
        CC_Assert(pt == NULL); // no way to provide location info
    ...
    if (depth < CC_32F)
    {
        int minVali = minVal <= INT_MIN ? INT_MIN : cFloor(minVal);
        int maxVali = maxVal > INT_MAX ? INT_MAX : cCeil(maxVal) - 1;
    ...
    else
    {
        ...
        if( depth == CC_32F )
        {
            suf32_t a, b;
            int ia, ib;
            const int* isrc = src img_ptr(int, );
            size_t step = src->step/sizeof(isrc[0]);

            a.f = (float)MAX(minVal, (double)-FLT_MAX);
            b.f = (float)MIN(maxVal, (double)FLT_MAX);

            ia = CC_TOGGLE_FLT(a.i);
            ib = CC_TOGGLE_FLT(b.i);
        ...
        else
        {
            ...
            ia = CC_TOGGLE_DBL(a.i);
            ib = CC_TOGGLE_DBL(b.i);
    ...
    if( badPt.x >= 0 )
    {
        ...
        {
            String value_str;
            value_str << src(CRange(badPt.y, badPt.y + 1), CRange(badPt.x, badPt.x + 1));
            CC_Error_( CC_StsOutOfRange,
...
void patchNaNs( InputOutputArray _a, double _val )
{
    ...
    CC_Assert( _a CC_MAT_DEPTH() == CC_32F );

    CC_OCL_RUN(_a.isUMat() && _a.dims() <= 2,
    ...
    size_t len = it.size*a CC_MAT_CN();
    suf32_t val;
    val.f = (float)_val;
...
CC_IMPL float cCbrt(float value) { return cubeRoot(value); }
CC_IMPL float cFastArctan(float y, float x) { return fastAtan2(y, x); }

CC_IMPL void
cvCartToPolar( const img_t* xarr, const img_t* yarr,
               img_t* magarr, img_t* anglearr,
               int angle_in_degrees )
{
    img_t X = cvarrToMat(xarr), Y = cvarrToMat(yarr), Mag, Angle;
    if( magarr )
    {
        Mag = cvarrToMat(magarr);
        CC_Assert( Mag.size() == X.size() && Mag CC_MAT_TYPE() == X CC_MAT_TYPE() );
    }
    if( anglearr )
    {
        Angle = cvarrToMat(anglearr);
        CC_Assert( Angle.size() == X.size() && Angle CC_MAT_TYPE() == X CC_MAT_TYPE() );
    }
    if( magarr )
    {
        if( anglearr )
...
CC_IMPL void
cvPolarToCart( const img_t* magarr, const img_t* anglearr,
               img_t* xarr, img_t* yarr, int angle_in_degrees )
{
    img_t X, Y, Angle = cvarrToMat(anglearr), Mag;
    if( magarr )
    {
        Mag = cvarrToMat(magarr);
        CC_Assert( Mag.size() == Angle.size() && Mag CC_MAT_TYPE() == Angle CC_MAT_TYPE() );
    }
    if( xarr )
    {
        X = cvarrToMat(xarr);
        CC_Assert( X.size() == Angle.size() && X CC_MAT_TYPE() == Angle CC_MAT_TYPE() );
    }
    if( yarr )
    {
        Y = cvarrToMat(yarr);
        CC_Assert( Y.size() == Angle.size() && Y CC_MAT_TYPE() == Angle CC_MAT_TYPE() );
    }

    polarToCart( Mag, Angle, X, Y, angle_in_degrees != 0 );
}

CC_IMPL void cvExp( const img_t* srcarr, img_t* dstarr )
{
    img_t src = cvarrToMat(srcarr), dst = cvarrToMat(dstarr);
    CC_Assert( src CC_MAT_TYPE() == dst CC_MAT_TYPE() && src.size == dst.size );
    exp( src, dst );
}

CC_IMPL void cvLog( const img_t* srcarr, img_t* dstarr )
{
    img_t src = cvarrToMat(srcarr), dst = cvarrToMat(dstarr);
    CC_Assert( src CC_MAT_TYPE() == dst CC_MAT_TYPE() && src.size == dst.size );
    log( src, dst );
...
CC_IMPL void cvPow( const img_t* srcarr, img_t* dstarr, double power )
{
    img_t src = cvarrToMat(srcarr), dst = cvarrToMat(dstarr);
    CC_Assert( src CC_MAT_TYPE() == dst CC_MAT_TYPE() && src.size == dst.size );
    pow( src, power, dst );
...
CC_IMPL int cvCheckArr( const img_t* arr, int flags,
                        double minVal, double maxVal )
{
    if( (flags & CC_CHECK_RANGE) == 0 )
        minVal = -DBL_MAX, maxVal = DBL_MAX;
    return checkRange(cvarrToMat(arr), (flags & CC_CHECK_QUIET) != 0, 0, minVal, maxVal );
}
...
*/
#endif
static int solveCubic( const img_t* _coeffs, img_t* _roots )
{
    CC_INSTRUMENT_REGION()

    const int n0 = 3;
    const img_t *coeffs = _coeffs;
    TypeId ctype = CC_MAT_DEPTH(coeffs);

    CC_Assert( ctype == CC_32F || ctype == CC_64F );
    CC_Assert( (CC_ARE_SIZES_EQSZ1(coeffs, n0, 1) ||
                CC_ARE_SIZES_EQSZ1(coeffs, n0+1, 1) ||
                CC_ARE_SIZES_EQSZ1(coeffs, 1, n0) ||
                CC_ARE_SIZES_EQSZ1(coeffs, 1, n0+1)) );
    {

    img_t *roots = cvSetMat(_roots, n0, 1, ctype, 1, NULL, CC_AUTO_STEP);
    ...
    if( ctype == CC_32FC1 )
    {
        const float* coeffs_ = img_ptr(float, coeffs);
        if( ncoeffs == 4 )
            a0 = coeffs_[++i];
    ...
    {
        const double* coeffs_ = img_ptr(double, coeffs);
        if( ncoeffs == 4 )
            a0 = coeffs_[++i];
    ...
    {
        float* roots_ = img_ptr(float, roots);
        roots_[0] = (float)x0;
        roots_[1] = (float)x1;
        roots_[2] = (float)x2;
    }
    ...
    {
        double* roots_ = img_ptr(double, roots);
        roots_[0] = x0;
        roots_[1] = x1;
        roots_[2] = x2;
    }
...
double solvePoly( const img_t* _coeffs0, img_t* _roots0, int maxIters )
{
    CC_INSTRUMENT_REGION()
    ...
    double maxDiff = 0;
    int iter, i, j;
    img_t coeffs0 = _coeffs0;
    int ctype = _coeffs0 CC_MAT_TYPE();
    int cdepth = CC_MAT_DEPTH(ctype);

    CC_Assert( CC_MAT_DEPTH(ctype) >= CC_32F && CC_MAT_CN(ctype) <= 2 );
    CC_Assert( coeffs0->rows == 1 || coeffs0->cols == 1 );

    int n0 = coeffs0->cols + coeffs0->rows - 2, n = n0;

    _roots0 cvSetMat(n, 1, CC_MAKETYPE(cdepth, 2), -1, true, _OutputArray::DEPTH_MASK_FLT);
    img_t roots0 = _roots0;

    CAutoBuffer<C> buf(n*2+2);
    C *coeffs = buf, *roots = coeffs + n + 1;
    img_t coeffs1(coeffs0.size(), CC_MAKETYPE(CC_64F, coeffs0 CC_MAT_CN()), coeffs0 CC_MAT_CN() == 2 ? coeffs : roots);
    coeffs0.convertTo(coeffs1, coeffs1 CC_MAT_TYPE());
    if( coeffs0 CC_MAT_CN() == 1 )
    {
        ...
        for( i = 0; i < n; i++ )
        {
            ...
            if( num_same_root > 1)
            {
                ...
                if( num_same_root % 2 != 0){
                    img_t cube_coefs(4, 1, CC_64FC1);
                    img_t cube_roots(3, 1, CC_64FC2);
                    double* cube_coefs_ = img_ptr(double, cube_coefs);
                    cube_coefs_[3] = -(pow(old_num_re, 3));
                    cube_coefs_[2] = -(15*pow(old_num_re, 2) + 27*pow(old_num_im, 2));
                    cube_coefs_[1] = -48*old_num_re;
                    cube_coefs_[0] = 64;
                    solveCubic(cube_coefs, cube_roots);

                    if(cube_roots_[0] >= 0) num.re = pow(cube_roots_[0]), 1./3);
                    else num.re = -pow(-cube_roots_[0], 1./3);
    ...
    img_t(roots0.size(), CC_64FC2, roots).convertTo(roots0, roots0 CC_MAT_TYPE());
    return maxDiff;
}
...
{
    img_t _coeffs = cvarrToMat(coeffs), _roots = cvarrToMat(roots), _roots0 = _roots;
    int nroots = solveCubic(_coeffs, _roots);
    CC_Assert( _roots.data == _roots0.data ); // check that the array of roots was not reallocated
    return nroots;
}


void cvSolvePoly(const img_t* a, img_t *r, int maxiter, int)
{
    img_t _a = cvarrToMat(a);
    img_t _r = cvarrToMat(r);
    img_t _r0 = _r;
    solvePoly(_a, _r, maxiter);
    CC_Assert( _r.data == _r0.data ); // check that the array of roots was not reallocated
}
