...
class CC_GBTreesTest : public cvtest::BaseTest
{
public:
    CC_GBTreesTest();
    ~CC_GBTreesTest();
...
int _get_len(const img_t* mat)
{
    return (mat->cols > mat->rows) ? mat->cols : mat->rows;
}


CC_GBTreesTest::CC_GBTreesTest()
{
...
int CC_GBTreesTest::TestTrainPredict(int test_num)
{
    ...
    switch (test_num)
    {
        ...
        default  :
            {
            ts->printf(cvtest::TS::LOG, "Bad test_num value in CC_GBTreesTest::TestTrainPredict(..) function.");
    ...
    if (!data)
    {
        ...
        int train_sample_count = cFloor(_get_len(data->get_responses())*0.5f);
...
int CC_GBTreesTest::checkPredictError(int test_num)
{
    if (!gtb)
    ...
    float current_error = gtb->calc_error(data, CC_TEST_ERROR);
...
int CC_GBTreesTest::TestSaveLoad()
{
    if (!gtb)
        return cvtest::TS::FAIL_GENERIC;

    model_file_name1 = tempfile();
    model_file_name2 = tempfile();

    gtb->save(model_file_name1.c_str());
    gtb->calc_error(data, CC_TEST_ERROR, &test_resps1);
    gtb->load(model_file_name1.c_str());
    gtb->calc_error(data, CC_TEST_ERROR, &test_resps2);
...
int CC_GBTreesTest::checkLoadSave()
{
    int code = cvtest::TS::OK;
    ...
    string s1, s2;
    int lineIdx = 0;
    CC_Assert(f1.is_open() && f2.is_open());
    ...
    // 2. compare responses
    CC_Assert(test_resps1.size() == test_resps2.size());
...
TEST(ML_GBTrees, regression) { CC_GBTreesTest test; test.safe_run(); }
