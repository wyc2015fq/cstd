...
#define ICC_DEF_PYR_DOWN_FUNC(flavor, type, worktype, _pd_scale_)                     \
static CStatus C_STDCALL                                                              \
icvPyrDownG5x5_##flavor##_CnR(const type* src, int srcstep, type* dst,                 \
                               int dststep, CSize size, void *buf, int Cs)            \
{                                                                                       \
...
ICC_DEF_PYR_DOWN_FUNC(8u, uchar, int, PD_SCALE_INT)
ICC_DEF_PYR_DOWN_FUNC(16s, short, int, PD_SCALE_INT)
ICC_DEF_PYR_DOWN_FUNC(16u, ushort, int, PD_SCALE_INT)
ICC_DEF_PYR_DOWN_FUNC(32f, float, float, PD_SCALE_FLT)
ICC_DEF_PYR_DOWN_FUNC(64f, double, double, PD_SCALE_FLT)
...
#define ICC_DEF_PYR_UP_FUNC(flavor, type, worktype, _pu_scale_)                       \
static CStatus C_STDCALL                                                              \
...
ICC_DEF_PYR_UP_FUNC(8u, uchar, int, PU_SCALE_INT)
ICC_DEF_PYR_UP_FUNC(16s, short, int, PU_SCALE_INT)
ICC_DEF_PYR_UP_FUNC(16u, ushort, int, PU_SCALE_INT)
ICC_DEF_PYR_UP_FUNC(32f, float, float, PU_SCALE_FLT)
ICC_DEF_PYR_UP_FUNC(64f, double, double, PU_SCALE_FLT)
...
#define ICC_DEF_PYR_BORDER_FUNC(flavor, arrtype, worktype, _pd_scale_)                \
static CStatus C_STDCALL                                                              \
icvPyrDownBorder_##flavor##_CnR(const arrtype *src, int src_step, CSize src_size,     \
                          arrtype *dst, int dst_step, CSize dst_size, int channels)   \
{                                                                                       \
    int local_alloc = 0;                                                                \
...
#define ICC_DEF_INIT_PYR_TABLE(FUNCNAME)                          \
static void icvInit##FUNCNAME##Table(CvFuncTable  tab)            \
{                                                                   \
    tab[CC_8U] = (void*)icv##FUNCNAME##_8u_CnR;              \
    tab[CC_8S] = 0;                                          \
    tab[CC_16S] = (void*)icv##FUNCNAME##_16s_CnR;            \
    tab[CC_16U] = (void*)icv##FUNCNAME##_16u_CnR;            \
    tab[CC_32F] = (void*)icv##FUNCNAME##_32f_CnR;            \
    tab[CC_64F] = (void*)icv##FUNCNAME##_64f_CnR;            \
}

static void icvInitPyrDownBorderTable(CvFuncTable  tab);

ICC_DEF_INIT_PYR_TABLE(PyrUpG5x5)
ICC_DEF_INIT_PYR_TABLE(PyrDownG5x5)

typedef CStatus (C_STDCALL * CvPyrDownBorderFunc)(const void* src, int srcstep,
                                                     CSize srcsize, void* dst,
                                                     int dststep, CSize dstsize, int cn);

typedef CStatus (C_STDCALL * CvPyramidFunc)
(const void* src, int srcstep, void* dst,
  int dststep, CSize size, void* buffer, int cn);

typedef CStatus (C_STDCALL * CvPyramidIPPFunc)
(const void* src, int srcstep, void* dst, int dststep, CSize size, void* buffer);
...
CC_IMPL void
cvPyrUp(const img_t* src, img_t* dst, int _filter)
{
    ...
    if(filter != CC_GAUSSIAN_5x5)
        CC_ERROR(CC_StsBadArg, "this filter type not supported");
    ...
    if(cn != 1 && cn != 3)
        CC_ERROR(CC_StsUnsupportedFormat, "The images must have 1 or 3 channel");

    func = (CvPyramidFunc)pyrup_tab[type];
    ...
    {
        ipp_func = type == CC_8U && cn==1 ? icvPyrUp_Gauss5x5_8u_C1R_p :
                   type == CC_8U && cn==3 ? icvPyrUp_Gauss5x5_8u_C3R_p :
                   type == CC_32F && cn==1 ? icvPyrUp_Gauss5x5_32f_C1R_p :
                   type == CC_32F && cn==3 ? icvPyrUp_Gauss5x5_32f_C3R_p : 0;
    ...
    if(buffer_size <= CC_MAX_LOCAL_SIZE)
    {
        buffer = cvStackAlloc(buffer_size);
        local_alloc = 1;
    }
    else
        CC_CALL(buffer = cAlloc(buffer_size));

    if(!use_ipp)
        func(src->tt.data, src->step, dst->tt.data, dst->step, size, buffer, cn);
    else
        IPPI_CALL(ipp_func(src->tt.data, src->step ? src->step : CC_STUB_STEP,
            dst->tt.data, dst->step ? dst->step : CC_STUB_STEP, size, buffer));
    __END__;
...
CC_IMPL void
cvPyrDown(const img_t* src, img_t* dst, int _filter)
{
    ...
    CvPyramidIPPFunc ipp_func = 0;
    int use_ipp = 0;
    CSize src_size, src_size2, dst_size;
    ...
    /*if(src->tt.data == dst->tt.data)
        CC_ERROR(CC_StsInplaceNotSupported, "");*/
    ...
    func = (CvPyramidFunc)pyrdown_tab[type];
    ...
    if(!use_ipp)
        ...
        IPPI_CALL(ipp_func(src->tt.data, src->step ? src->step : CC_STUB_STEP,
            dst->tt.data, dst->step ? dst->step : CC_STUB_STEP, src_size2, buffer));

    if(src_size.width != dst_size.width*2 || src_size.height != dst_size.height*2)
    {
        CvPyrDownBorderFunc border_func = (CvPyrDownBorderFunc)
                            pyrdownborder_tab[type];

        if(!border_func)
            CC_ERROR(CC_StsUnsupportedFormat, "");

        IPPI_CALL(border_func(src->tt.data, src->step, src_size,
                                dst->tt.data, dst->step, dst_size, CC_MAT_CN(src)));
    }
...
#if defined _MSC_VER && _MSC_VER > 1300 && !defined CC_ICC
#pragma optimize("", off)
#endif

ICC_DEF_PYR_BORDER_FUNC(8u, uchar, int, PD_SCALE_INT)
ICC_DEF_PYR_BORDER_FUNC(16u, ushort, int, PD_SCALE_INT)
ICC_DEF_PYR_BORDER_FUNC(16s, short, int, PD_SCALE_INT)
ICC_DEF_PYR_BORDER_FUNC(32f, float, float, PD_SCALE_FLT)
ICC_DEF_PYR_BORDER_FUNC(64f, double, double, PD_SCALE_FLT)

#define ICC_DEF_INIT_PYR_BORDER_TABLE(FUNCNAME)                   \
static void icvInit##FUNCNAME##Table(CvFuncTable  tab)            \
{                                                                   \
    tab[CC_8U] = (void*)icv##FUNCNAME##_8u_CnR;              \
    tab[CC_8S] = 0;                                          \
...
ICC_DEF_INIT_PYR_BORDER_TABLE(PyrDownBorder)
