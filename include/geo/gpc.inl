...
Copyright: (C) 1997-2004, Advanced Interfaces Group,
 ...
 This software is pfree for non-commercial use. It may be copied,
...
*/
#undef FEQ
#define FEQ(a, b) (fabs((a) - (b)) <= GPC_EPSILON)
...
#define N_EDGE(d,e,p,i,j) {(d)= (e); \
    ...
    (i)= (d)->bot.x + (d)->dx * ((j)-(d)->bot.y);}
#define GPC_MALLOC(p, b, s, t) {if ((b) > 0) { \
      p= (t*)pmalloc(b); if (!(p)) { \
        fprintf(stderr, "gpc pmalloc failure: %s\n", s); \
        exit(0);}} else p= NULL;}
#define GPC_FREE(p) {if (p) {pfree(p); (p)= NULL;}}
/*
===========================================================================
 Private Data Types
===========================================================================
...
#define GPC_NUL 0 /* Empty non-intersection */
#define GPC_EMX 1 /* External maximum */
#define GPC_ELI 2 /* External left intermediate */
#define GPC_TED 3 /* Top edge */
#define GPC_ERI 4 /* External right intermediate */
#define GPC_RED 5 /* Right edge */
#define GPC_IMM 6 /* Internal maximum and minimum */
#define GPC_IMN 7 /* Internal minimum */
#define GPC_EMN 8 /* External minimum */
#define GPC_EMM 9 /* External maximum and minimum */
#define GPC_LED 10 /* Left edge */
#define GPC_ILI 11 /* Internal left intermediate */
#define GPC_BED 12 /* Bottom edge */
#define GPC_IRI 13 /* Internal right intermediate */
#define GPC_IMX 14 /* Internal maximum */
#define GPC_FUL 15 /* Full non-intersection */
typedef int vertex_type;
typedef enum /* Horizontal edge states */
{
  GPC_NH, /* No horizontal edge */
  GPC_BH, /* Bottom horizontal edge */
  GPC_TH /* Top horizontal edge */
} h_state;
...
const h_state next_h_state[3][6] = {
  /* ABOVE BELOW CROSS */
  /* L R L R L R */
  /* GPC_NH */ {GPC_BH, GPC_TH, GPC_TH, GPC_BH, GPC_NH, GPC_NH},
  /* GPC_BH */ {GPC_NH, GPC_NH, GPC_NH, GPC_NH, GPC_TH, GPC_TH},
  /* GPC_TH */ {GPC_NH, GPC_NH, GPC_NH, GPC_NH, GPC_BH, GPC_BH}
};
/*
===========================================================================
 Private Functions
...
static edge_node** bound_list(lmt_node** lmt, double y)
{
  ...
  if (!*lmt) {
    ...
    GPC_MALLOC(*lmt, sizeof(lmt_node), "LMT insertion", lmt_node);
    (*lmt)->y = y;
...
static void add_to_sbtree(int* entries, sb_tree** sbtree, double y)
{
  if (!*sbtree) {
    ...
    GPC_MALLOC(*sbtree, sizeof(sb_tree), "scanbeam tree insertion", sb_tree);
    (*sbtree)->y = y;
    (*sbtree)->less = NULL;
    (*sbtree)->more = NULL;
    (*entries)++;
...
static edge_node* build_lmt(lmt_node** lmt, sb_tree** sbtree,
    int* sbt_entries, gpc_polygon* p, int type, gpc_op op)
{
  ...
  GPC_MALLOC(edge_table, total_vertices * sizeof(edge_node),
...
static void add_intersection(it_node** it, edge_node* edge0, edge_node* edge1, double x, double y)
{
  ...
  if (!*it) {
    ...
    GPC_MALLOC(*it, sizeof(it_node), "IT insertion", it_node);
    (*it)->ie[0] = edge0;
    (*it)->ie[1] = edge1;
    (*it)->point.x = x;
    (*it)->point.y = y;
    ...
    double dy)
{
  ...
  if (!*st) {
    ...
    GPC_MALLOC(*st, sizeof(st_node), "ST insertion", st_node);
    (*st)->edge = edge;
    (*st)->xb = edge->xb;
    (*st)->xt = edge->xt;
    (*st)->dx = edge->dx;
  ...
  for (nc = 0; polygon; polygon = polygon->next)
    if (polygon->active) {
      ...
      if (nv > 2) {
        ...
        /* Invalid contour: just pfree the heap */
...
static void add_left(polygon_node* p, double x, double y)
{
  ...
  GPC_MALLOC(nv, sizeof(vertex_node), "vertex node creation", vertex_node);
  nv->x = x;
  nv->y = y;
    ...
    double x, double y)
{
  ...
  vertex_node* nv;
  existing_min = *p;
  GPC_MALLOC(*p, sizeof(polygon_node), "polygon node creation", polygon_node);
  /* Create a new vertex node and set its fields */
  GPC_MALLOC(nv, sizeof(vertex_node), "vertex node creation", vertex_node);
  nv->x = x;
  nv->y = y;
...
{
  if (!(*t)) {
    GPC_MALLOC(*t, sizeof(vertex_node), "tristrip vertex creation", vertex_node);
    (*t)->x = x;
    (*t)->y = y;
    (*t)->next = NULL;
  }
...
{
  if (!(*tn)) {
    GPC_MALLOC(*tn, sizeof(polygon_node), "tristrip node creation", polygon_node);
    (*tn)->next = NULL;
    (*tn)->v[LEFT] = NULL;
    (*tn)->v[RIGHT] = NULL;
    (*tn)->active = 1;
...
static bbox* create_contour_bboxes(gpc_polygon* p)
{
  bbox* box;
  int c, v;
  GPC_MALLOC(box, p->num_contours * sizeof(bbox), "Bounding box creation", bbox);
...
static void minimax_test(gpc_polygon* subj, gpc_polygon* clip, gpc_op op)
{
  ...
  GPC_MALLOC(o_table, subj->num_contours * clip->num_contours * sizeof(int),
...
===========================================================================
*/
static void gpc_free_polygon(gpc_polygon* p)
{
  int c;
  for (c = 0; c < p->num_contours; c++) {
...
static void gpc_read_polygon(FILE* fp, int read_hole_flags, gpc_polygon* p)
{
  int c, v;
  fscanf(fp, "%d", &(p->num_contours));
  GPC_MALLOC(p->hole, p->num_contours * sizeof(int),
      "hole flag array creation", int);
  GPC_MALLOC(p->contour, p->num_contours
...
static void gpc_write_polygon(FILE* fp, int write_hole_flags, gpc_polygon* p)
{
  int c, v;
  fprintf(fp, "%d\n", p->num_contours);
  for (c = 0; c < p->num_contours; c++) {
...
static void gpc_add_contour(gpc_polygon* p, gpc_vertex_list* new_contour, int hole)
{
  int* extended_hole, c, v;
  gpc_vertex_list* extended_contour;
  /* Create an extended hole array */
  GPC_MALLOC(extended_hole, (p->num_contours + 1)
      * sizeof(int), "contour hole addition", int);
  /* Create an extended contour array */
  GPC_MALLOC(extended_contour, (p->num_contours + 1)
...
static void gpc_polygon_clip(gpc_op op, gpc_polygon* subj, gpc_polygon* clip,
    gpc_polygon* result)
{
  sb_tree* sbtree = NULL;
  ...
  GPC_MALLOC(sbt, sbt_entries * sizeof(double), "sbt creation", double);
  ...
  while (scanbeam < sbt_entries) {
    ...
    for (next_edge = aet->next; next_edge; next_edge = next_edge->next) {
      ...
      if (next_edge->bundle[ABOVE][next_edge->type]) {
        if (FEQ(e0->xb, next_edge->xb) && FEQ(e0->dx, next_edge->dx)
            && (e0->top.y != yb)) {
    ...
    for (edge = aet; edge; edge = edge->next) {
      ...
      if (exists[CLIP] || exists[SUBJ]) {
        ...
        case GPC_INT:
          ...
          tr = (parity[CLIP] ^ (horiz[CLIP] != GPC_NH)) && (parity[SUBJ] ^ (horiz[SUBJ] != GPC_NH));
          tl = (parity[CLIP] ^ (horiz[CLIP] != GPC_NH) ^ edge->bundle[BELOW][CLIP])
              && (parity[SUBJ] ^ (horiz[SUBJ] != GPC_NH) ^ edge->bundle[BELOW][SUBJ]);
          break;
        case GPC_XOR:
          ...
          tr = (parity[CLIP] ^ (horiz[CLIP] != GPC_NH))
              ^ (parity[SUBJ] ^ (horiz[SUBJ] != GPC_NH));
          tl = (parity[CLIP] ^ (horiz[CLIP] != GPC_NH) ^ edge->bundle[BELOW][CLIP])
              ^ (parity[SUBJ] ^ (horiz[SUBJ] != GPC_NH) ^ edge->bundle[BELOW][SUBJ]);
          break;
        case GPC_UNION:
  ...
  if (result->num_contours > 0) {
    GPC_MALLOC(result->hole, result->num_contours
...
static void gpc_free_tristrip(gpc_tristrip* t)
{
  int s;
...
static void gpc_tristrip_clip(gpc_op op, gpc_polygon* subj, gpc_polygon* clip,
    gpc_tristrip* result)
{
  sb_tree* sbtree = NULL;
  it_node* it = NULL, *intersect;
  ...
  if (result->num_strips > 0) {
    GPC_MALLOC(result->strip, result->num_strips * sizeof(gpc_vertex_list),
        "tristrip list creation", gpc_vertex_list);
    s = 0;
    for (tn = tlist; tn; tn = tnn) {
      tnn = tn->next;
      if (tn->active > 2) {
        /* Valid tristrip: copy the vertices and pfree the heap */
        result->strip[s].num_vertices = tn->active;
        GPC_MALLOC(result->strip[s].vertex, tn->active * sizeof(gpc_vertex),
        ...
        /* Invalid tristrip: just pfree the heap */
...
static void gpc_polygon_to_tristrip(gpc_polygon* s, gpc_tristrip* t)
{
  gpc_polygon c;
  c.num_contours = 0;
  c.hole = NULL;
