...
double RegulaFalsi(double(*f)(double x), double left, double right)
{
...
double NewtonRaphson(double(*f)(double x), double(*df)(double x), double x)
{
  double d = (*df)(x);
  if (d != 0.0) {
    return (x - ((*f)(x) / d));
...
/* Based on Blinn */
double findroot(double left, double right, double tolerance, double(*f)(double x), double(*df)(double x))
{
  double newx = left;
