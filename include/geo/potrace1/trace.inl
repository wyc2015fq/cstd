...
CC_INLINE point_t dorth_infty(dpoint_t p0, dpoint_t p2) {
  point_t r;
...
CC_INLINE double dpara(dpoint_t p0, dpoint_t p1, dpoint_t p2) {
  double x1, y1, x2, y2;
...
CC_INLINE double ddenom(dpoint_t p0, dpoint_t p2) {
...
CC_INLINE int cyclic(int a, int b, int c) {
  if (a<=c) {
    return (a<=b && b<c);
  } else {
    return (a<=b || b<c);
...
CC_INLINE double quadform(quadform_t Q, dpoint_t w) {
  double v[3];
  int i, j;
  double sum;
...
/* calculate p1 x p2 */
CC_INLINE int xprod(point_t p1, point_t p2) {
  return p1.x*p2.y - p1.y*p2.x;
}

/* calculate (p1-p0)x(p3-p2) */
CC_INLINE double cprod(dpoint_t p0, dpoint_t p1, dpoint_t p2, dpoint_t p3) {
  double x1, y1, x2, y2;
...
CC_INLINE double iprod(dpoint_t p0, dpoint_t p1, dpoint_t p2) {
  double x1, y1, x2, y2;
...
CC_INLINE double iprod1(dpoint_t p0, dpoint_t p1, dpoint_t p2, dpoint_t p3) {
  double x1, y1, x2, y2;
...
CC_INLINE double ddist(dpoint_t p, dpoint_t q) {
  return sqrt(sq(p.x-q.x)+sq(p.y-q.y));
}

/* calculate point of a bezier curve */
CC_INLINE dpoint_t bezier(double t, dpoint_t p0, dpoint_t p1, dpoint_t p2, dpoint_t p3) {
  double s = 1-t;
  dpoint_t res;
