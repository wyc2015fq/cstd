...
/* $Id: backend_pdf.c,v 1.10 2005/02/22 21:31:23 selinger Exp $ */
...
CC_INLINE void intarray_init(intarray_t *ar) {
  ar->size = 0;
  ar->data = NULL;
}

CC_INLINE void intarray_term(intarray_t *ar) {
  pfree(ar->data);
  ar->size = 0;
  ar->data = NULL;
}
...
CC_INLINE int intarray_set(intarray_t *ar, int n, int val) {
  int *p;
  int s;
...
CC_INLINE point_t unit(dpoint_t p) {
  point_t q;
...
int init_pdf(FILE *fout)
{
	...
	shipcom("2 0 obj\n"
		"<</Creator"
		"("POTRACE" "VERSION", written by Peter Selinger 2001-2005)>>\n"
		"endobj\n");
...
int term_pdf(FILE *fout)
{
	...
	shipcom("xref\n0 %d\n", nxref + 1);
	shipcom("0000000000 65535 f \n");
	for (i = 0; i < nxref; i++)
		shipcom("%0.10d 00000 n \n", xref.data[i]);
...
static void pdf_pageinit(imginfo_t *imginfo)
{
	...
	intarray_set(&xref, nxref++, outcount);
	shipcom("%d 0 obj\n", nxref);
	shipcom("<</Type/Page/Parent 3 0 R/Resources<</ProcSet[/PDF]>>");
	shipcom("/MediaBox[0 0 %d %d]", pagew, pageh);
	shipcom("/Contents %d 0 R>>\n", nxref + 1);
	shipcom("endobj\n");
	...
	shipcom("%d 0 obj\n", nxref);
	if (info.compress)
		shipcom("<</Filter/FlateDecode/Length %d 0 R>>\n", nxref + 1);
	else
		shipcom("<</Length %d 0 R>>\n", nxref + 1);
	shipcom("stream\n");

	streamofs = outcount;

	/* TODO: info.angle rotate */
	ship("%f 0 0 %f %.0f %.0f cm\n", scalex, scaley, origx, origy);
}

static void pdf_pageterm()
{
	int streamlen;
