...
/* $Id: lzw.c,v 1.6 2005/02/22 21:31:23 selinger Exp $ */
...
/* the dictionary is implemented as a tree of strings under the prefix
   ...
   lzw_dict structures, with "children" pointing to a node's first
...
struct lzw_dict {
  ...
  struct lzw_dict *children;  /* list of sub-entries */
  struct lzw_dict *next;      /* for making a linked list */
};
typedef struct lzw_dict lzw_dict;

/* A state {s,d,n} is represented by the "dictionary state" part of
   the lzw_state structure. Here, s is a pointer directly to the node
   of the dictionary structure corresponding to the string s, or NULL
   if s=[]. Further, the lzw_state structure contains a buffer of
...
struct lzw_state {
  ...
  lzw_dict *d;     /* pointer to dictionary */
  lzw_dict *s;     /* pointer to current string, or NULL at beginning */
...
/* recursively pfree an lzw_dict object */
static void lzw_free_dict(lzw_dict *s) {
  lzw_dict *e;
...
/* re-initialize the lzw_state's dictionary state to "newdict",
   freeing any old dictionary. */
static void lzw_clear_table(lzw_state *st) {
...
CC_INLINE void lzw_emit(int code, lzw_state *st) {
  BITBUF_TYPE mask;
  int bits = hibit(st->n);
...
/* transfer one byte from bit buffer to output. Precondition:
   s->avail_out > 0. */
CC_INLINE void lzw_read_bitbuf(lzw_stream_t *s) {
  int ch;
  lzw_state *st = (lzw_state *)s->internal;
...
static int lzw_encode_char(lzw_state *st, char c) {
  lzw_dict *e;
  ...
  e = (lzw_dict *)pmalloc(sizeof(lzw_dict));
  if (!e) {
    return 1;
  }
  e->c = c;
...
static void lzw_encode_eod(lzw_state *st) {
...
lzw_stream_t *lzw_init(void) {
  ...
  s = (lzw_stream_t *)pmalloc(sizeof(lzw_stream_t));
  if (s==NULL) {
    goto fail;
  }
  st = (lzw_state *)pmalloc(sizeof(lzw_state));
  if (st==NULL) {
    goto fail;
  }
  st->buf = 0;
