...
/* $Id: main.c,v 1.41 2005/02/22 21:31:23 selinger Exp $ */
...
/* Initialize parameters for simple progress bar. The caller passes an
   allocated simple_progress_t structure to avoid having to pmalloc it
   here and pfree it later. */
CC_INLINE void init_progress(potrace_progress_t *prog, simple_progress_t *p, const char *filename, int count) {
  const char *q, *s;
  int len;
...
CC_INLINE double double_of_dim(dim_t d, double def) {
  if (d.d) {
    return d.x * d.d;
  } else {
    return d.x * def;
...
static int dopts(int ac, char *av[]) {
  ...
  while ((c = getopt_long(ac, av, shortopts, longopts, NULL)) != -1) {
    ...
    case 'h':
      ...
      break;
    case 'v':
      fprintf(stdout, ""POTRACE" "VERSION". Copyright (C) 2001-2005 Peter Selinger.\n");
