...
#define SAFE_MALLOC(var, n, typ) \
  if ((var = (typ *)pmalloc((n)*sizeof(typ))) == NULL) goto malloc_error 
...
typedef struct info_s1 info_t1;

static info_t1 info1;
...
static void process_file(FILE *fin, FILE *fout, char *infile, char *outfile) {
  ...
  for (count=0; ; count++) {
    ...
    if (info1.linear) {
      sm = interpolate_linear(gm, info1.scale, info1.bilevel, info1.level);
    } else {
      sm = interpolate_cubic(gm, info1.scale, info1.bilevel, info1.level);
    }      
    if (!sm) {
      fprintf(stderr, ""MKBITMAP": %s: %s\n", infile, strerror(errno));
      exit(2);
...
/* ---------------------------------------------------------------------- */
/* some info1 functions and option processing */
...
static int usage(FILE *f) {
  ...
  fprintf(f, " -v, --version        - print version info1 and exit\n");
  fprintf(f, " -l, --license        - print license info1 and exit\n");
...
static void dopts(int ac, char *av[]) {
  ...
  info1.outfile = NULL;    /* output file */
  info1.infiles = NULL;    /* input files */
  info1.infilecount = 0;   /* how many input files? */
  info1.invert = 0;        /* invert input? */
  info1.highpass = 1;      /* use highpass filter? */
  info1.lambda = 4;        /* highpass filter radius */
  info1.scale = 2;         /* scaling factor */
  info1.linear = 0;        /* linear scaling? */
  info1.bilevel = 1;       /* convert to bilevel? */
  info1.level = 0.45;      /* cutoff grey level */
  info1.outext = ".pbm";   /* output file extension */
  ...
  while ((c = getopt_long(ac, av, shortopts, longopts, NULL)) != -1) {
	...
	fprintf(stderr, ""MKBITMAP": invalid threshold -- %s\n", optarg);
      ...
      break;
    case '?':
      fprintf(stderr, "Try --help for more info1\n");
      exit(1);
      break;
    default:
  ...
  info1.infiles = &av[optind];
  info1.infilecount = ac-optind;
  return;
}
...
static char *make_outfilename(char *infile, char *ext) {
  ...
  outfile = (char *) pmalloc(strlen(infile)+strlen(ext)+5);
  if (!outfile) {
    return NULL;
  }
...
int main(int ac, char *av[]) {
  ...
  */
  ...
  if (info1.infilecount == 0 && info1.outfile == NULL) {  /* stdin to stdout */

    process_file(stdin, stdout, "stdin", "stdout");
    return 0;

  } else if (info1.infilecount == 0) {                  /* stdin to outfile */

    fout = my_fopen_write(info1.outfile);
    if (!fout) {
      fprintf(stderr, ""MKBITMAP": %s: %s\n", info1.outfile, strerror(errno));
      exit(2);
    }
    process_file(stdin, fout, "stdin", info1.outfile);
    my_fclose(fout, info1.outfile);
    return 0;

  } else if (info1.outfile == NULL) {       /* infiles -> multiple outfiles */

    for (i=0; i<info1.infilecount; i++) {
      outfile = make_outfilename(info1.infiles[i], info1.outext);
      if (!outfile) {
	fprintf(stderr, ""MKBITMAP": %s\n", strerror(errno));
        exit(2);
      }
      fin = my_fopen_read(info1.infiles[i]);
      if (!fin) {
	fprintf(stderr, ""MKBITMAP": %s: %s\n", info1.infiles[i], strerror(errno));
	exit(2);
      }
	...
	exit(2);
      ...
      process_file(fin, fout, info1.infiles[i], outfile);
      my_fclose(fin, info1.infiles[i]);
	...
	exit(2);
      ...
      process_file(fin, fout, info1.infiles[i], info1.outfile);
      my_fclose(fin, info1.infiles[i]);
    }
    my_fclose(fout, info1.outfile);
    return 0;
