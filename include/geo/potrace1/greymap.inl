...
greymap_t *gm_new(int w, int h) {
  ...
  gm = (greymap_t *) pmalloc(sizeof(greymap_t));
  if (!gm) {
    return NULL;
  }
  gm->w = w;
  gm->h = h;
  gm->map = (signed short int *) pmalloc(w*h*sizeof(signed short int));
  if (!gm->map) {
    errno_save = errno;
    pfree(gm);
    errno = errno_save;
...
int gm_readbody_bmp(FILE *f, greymap_t **gmp) {
  ...
  if (bmpinfo.bits <= 8) {
    coltable = (int *) pmalloc(bmpinfo.ncolors * sizeof(int));
    if (!coltable) {
      goto std_error;
    }
