...
CC_INLINE dpoint_t dpoint(point_t p) {
  dpoint_t res;
  res.x = p.x;
  res.y = p.y;
  return res;
...
CC_INLINE dpoint_t interval(double lambda, dpoint_t a, dpoint_t b) {
  dpoint_t res;
...
CC_INLINE int mod(int a, int n) {
  return a>=n ? a%n : a>=0 ? a : n-1-(-1-a)%n;
}

CC_INLINE int floordiv(int a, int n) {
