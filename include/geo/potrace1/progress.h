...
CC_INLINE void progress_update(double d, progress_t *prog) {
  double d_scaled;
...
CC_INLINE void progress_subrange_start(double a, double b, const progress_t *prog, progress_t *sub) {
  double min, max;
...
CC_INLINE void progress_subrange_end(progress_t *prog, progress_t *sub) {
  if (prog->callback != NULL) {
