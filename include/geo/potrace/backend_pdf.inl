...
static CC_INLINE void intarray_init(intarray_t *ar) {
  ar->size = 0;
  ar->data = NULL;
}

static CC_INLINE void intarray_term(intarray_t *ar) {
  free(ar->data);
  ar->size = 0;
  ar->data = NULL;
}
...
static CC_INLINE int intarray_set(intarray_t *ar, int n, int val) {
  int *p;
  int s;
...
static CC_INLINE point_t unit(dpoint_t p) {
  point_t q;
