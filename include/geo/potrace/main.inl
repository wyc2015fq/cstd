...
static pageformat_t pageformat_main[] = {
  { "a4",        595,  842 },
  { "a3",        842, 1191 },
  { "a5",        421,  595 },
  { "b5",        516,  729 },
...
static CC_INLINE double double_of_dim(dim_t d, double def) {
  if (d.d) {
    return d.x * d.d;
  } else {
    return d.x * def;
...
static void dopts(int ac, char *av[]) {
  ...
  while ((c = getopt_long(ac, av, shortopts, longopts, NULL)) != -1) {
    ...
    case 'P':
      matches = 0;
      bestmatch = 0;
      for (i=0; pageformat_main[i].name!=NULL; i++) {
        if (strcasecmp(pageformat_main[i].name, optarg)==0) {
          matches = 1;
          bestmatch = i;
          break;
	} else if (strncasecmp(pageformat_main[i].name, optarg, strlen(optarg))==0) {
      ...
      if (matches == 1) {
	info.paperwidth = pageformat_main[bestmatch].w;
	info.paperheight = pageformat_main[bestmatch].h;
	break;
      }
      parse_dimensions(optarg, &p, &dimx, &dimy);
      if (*p == 0) {
	...
	fprintf(stderr, ""POTRACE": ambiguous page format -- %s\n", optarg);
      ...
      for (i=0; pageformat_main[i].name!=NULL; i++) {
	if (j + strlen(pageformat_main[i].name) > 75) {
	  fprintf(stderr, "\n");
	  j = 0;
	}
	j += fprintf(stderr, "%s, ", pageformat_main[i].name);
      }
...
int test_potrace_main(int ac, char *av[]) {
