...
/* initialize the interval_box to [min, max] */
static void interval_box(interval_t *i, double min, double max) {
  i->min = min;
  i->max = max;
}

/* initialize the interval_box to [x, x] */
static CC_INLINE void singleton(interval_t *i, double x) {
  interval_box(i, x, x);
}

/* extend the interval_box to include the number x */
static CC_INLINE void extend(interval_t *i, double x) {
  if (x < i->min) {
    i->min = x;
  } else if (x > i->max) {
    i->max = x;
...
static CC_INLINE int in_interval(interval_t *i, double x) {
...
static double iprod_box(dpoint_t a, dpoint_t b) {
...
static CC_INLINE double bezier_box(double t, double x0, double x1, double x2, double x3) {
  double s = 1-t;
  return s*s*s*x0 + 3*(s*s*t)*x1 + 3*(t*t*s)*x2 + t*t*t*x3;
}

/* Extend the interval_box i to include the minimum and maximum of a
...
/* extend the interval_box i to include the inner product <v | dir> for
   all points v on the segment. Assume precondition a in i. */
static CC_INLINE void segment_limits(int tag, dpoint_t a, dpoint_t c[3], dpoint_t dir, interval_t *i) {
  switch (tag) {
  case POTRACE_CORNER:
    extend(i, iprod_box(c[1], dir));
    extend(i, iprod_box(c[2], dir));
    break;
  case POTRACE_CURVETO:
    bezier_limits(iprod_box(a, dir), iprod_box(c[0], dir), iprod_box(c[1], dir), iprod_box(c[2], dir), i);
    break;
  }
}

/* extend the interval_box i to include <v | dir> for all points v on the
   curve. */
...
/* compute the interval_box i to be the smallest interval_box including all <v
   | dir> for points in the pathlist. If the pathlist is empty, return
   the singleton interval_box [0,0]. */
void path_limits(potrace_path_t *path, dpoint_t dir, interval_t *i) {
  ...
  /* initialize interval_box to a point on the first curve */
  singleton(i, iprod_box(path->curve.c[0][2], dir));
