...
/* The present file defines some convenient macros and static CC_INLINE
   functions for accessing bitmaps. Since they only produce CC_INLINE
...
static CC_INLINE void bm_free(potrace_bitmap_t *bm) {
  if (bm) {
    free(bm->map);
  }
  free(bm);
...
/* return new un-initialized bitmap. NULL with errno on error.
   Assumes w, h >= 0. */
static CC_INLINE potrace_bitmap_t *bm_new(int w, int h) {
  potrace_bitmap_t *bm;
...
static CC_INLINE void bm_clear(potrace_bitmap_t *bm, int c) {
...
static CC_INLINE potrace_bitmap_t *bm_dup(const potrace_bitmap_t *bm) {
...
static CC_INLINE void bm_invert(potrace_bitmap_t *bm) {
  ptrdiff_t i;
