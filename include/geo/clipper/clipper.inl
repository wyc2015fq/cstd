#ifndef _CLIPPER_INL_
#define _CLIPPER_INL_
...
static const cInt hiRange = 0x3FFFFFFFFFFFFFFF;
...
typedef std::vector< IntPoint > IPath;
typedef std::vector< IPath > Paths;
inline IPath& operator <<(IPath& poly, const IntPoint& p)
{
  poly.push_back(p);
  return poly;
}
inline Paths& operator <<(Paths& polys, const IPath& p)
{
  polys.push_back(p);
  return polys;
}
std::ostream& operator <<(std::ostream& s, const IntPoint& p);
std::ostream& operator <<(std::ostream& s, const IPath& p);
...
bool Orientation(const IPath& poly);
double Area(const IPath& poly);
int PointInPolygon(const IntPoint& pt, const IPath& path);
void SimplifyPolygon(const IPath& in_poly, Paths& out_polys, PolyFillType fillType = pftEvenOdd);
void SimplifyPolygons(const Paths& in_polys, Paths& out_polys, PolyFillType fillType = pftEvenOdd);
void SimplifyPolygons(Paths& polys, PolyFillType fillType = pftEvenOdd);
void CleanPolygon(const IPath& in_poly, IPath& out_poly, double distance = 1.415);
void CleanPolygon(IPath& poly, double distance = 1.415);
void CleanPolygons(const Paths& in_polys, Paths& out_polys, double distance = 1.415);
void CleanPolygons(Paths& polys, double distance = 1.415);
void MinkowskiSum(const IPath& pattern, const IPath& path, Paths& solution, bool pathIsClosed);
void MinkowskiSum(const IPath& pattern, const Paths& paths, Paths& solution, bool pathIsClosed);
void MinkowskiDiff(const IPath& poly1, const IPath& poly2, Paths& solution);
void PolyTreeToPaths(const PolyTree& polytree, Paths& paths);
void ClosedPathsFromPolyTree(const PolyTree& polytree, Paths& paths);
void OpenPathsFromPolyTree(PolyTree& polytree, Paths& paths);
void ReversePath(IPath& p);
...
public:
  ClipperBase();
  virtual ~ClipperBase();
  bool AddPath(const IPath& pg, PolyType PolyTyp, bool Closed);
...
public:
  ...
  ~ClipperOffset();
  void AddPath(const IPath& path, JoinType joinType, EndType endType);
...
double Area(const IntPoint* poly, int size)
{
  if (size < 3) {
    return 0;
  }
...
bool Orientation(const IntPoint* poly, int size)
{
  return Area(poly, size) >= 0;
}
double Area(const IPath& poly)
{
...
bool ClipperBase::AddPath(const IPath& pg, PolyType PolyTyp, bool Closed)
{
#ifdef use_lines
...
void ClipperOffset::AddPath(const IPath& path, JoinType joinType, EndType endType)
{
...
void SimplifyPolygon(const IPath& in_poly, Paths& out_polys, PolyFillType fillType)
{
  Clipper c;
...
void CleanPolygon(const IPath& in_poly, IPath& out_poly, double distance)
{
  size_t i;
...
void CleanPolygon(IPath& poly, double distance)
{
...
void Minkowski(const IPath& poly, const IPath& path,
...
void TranslatePath(const IPath& input, IPath& output, IntPoint delta)
{
...
#define INTPOINTFMT  "(%d,%d)"
FILE* print(FILE* s, const IntPoint& p)
{
  fprintf(s, INTPOINTFMT, p.X, p.Y);
  return s;
}
FILE* print(FILE* s, const IPath& p)
{
  if (p.empty()) {
    return s;
  }
...
FILE* print(FILE* s, const Paths& p)
{
  int i;
  for (i = 0; i < p.size(); i++) {
    print(s, p[i]);
...
void DPOLYGON2Paths(const DPOLYGON* pl, Paths& ppg, double scale) {
  int i, j, pos=0;
  for (i=0; i<pl->n; ++i) {
    IPath pg;
    for (j=pos; j<pos+pl->len[i]; ++j) {
      double X = pl->pt[j].x;
      double Y = pl->pt[j].y;
...
void Paths2DPOLYGON(const Paths& ppg, DPOLYGON* pl, double scale) {
  int i, j, pos=0, npt = 0, n = ppg.size();
  for (i=0; i<n; ++i) {
    npt += ppg[i].size();
  }
  dPOLYGON_setsize(pl, n, npt);
  for (i=0; i<n; ++i) {
    pl->len[i] = ppg[i].size();
    const IPath& pg = ppg[i];
    for (j = pos; j<pos + pl->len[i]; ++j) {
      const IntPoint& pt = pg[j-pos];
      pl->pt[j].x = pt.X/scale;
      pl->pt[j].y = pt.Y/scale;
    }
    pos += pl->len[i];
  }
  return ;
...
void QPOLYGON2Paths(const QPOLYGON* pl, Paths& ppg) {
  int i, j, pos=0;
  for (i=0; i<pl->n; ++i) {
    IPath pg;
    for (j=pos; j<pos+pl->len[i]; ++j) {
      double X = pl->pt[j].x;
      double Y = pl->pt[j].y;
      pg.push_back(IntPoint((cInt)(X), (cInt)(Y)));
    }
    pos += pl->len[i];
    if (pg.size() > 0) {
      ppg.push_back(pg);
...
void Paths2QPOLYGON(const Paths& ppg, QPOLYGON* pl) {
  int i, j, pos=0, npt = 0, n = ppg.size();
  for (i=0; i<n; ++i) {
    npt += ppg[i].size();
  }
  qPOLYGON_setsize(pl, n, npt);
  for (i=0; i<n; ++i) {
    pl->len[i] = ppg[i].size();
    const IPath& pg = ppg[i];
    for (j = pos; j<pos + pl->len[i]; ++j) {
