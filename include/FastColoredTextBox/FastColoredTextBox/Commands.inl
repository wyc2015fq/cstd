...

{
    ...
    struct InsertCharCommand : UndoableCommand
    {
        char c;
        char deletedChar = '\x0';
        ...
        InsertCharCommand(TextSource ts, char c): base(ts)
        {
            this->c = c;
        }
        ...
        override UndoableCommand Clone()
        {
    ...
    /// Insert text
    /// </summary>
    struct InsertTextCommand : UndoableCommand
    {
        string InsertedText;
        ...
        InsertTextCommand(TextSource ts, string insertedText): base(ts)
        {
    ...
    /// </summary>
    struct ReplaceTextCommand : UndoableCommand
    {
        string insertedText;
        List<Range> ranges;
        ...
        /// <param name="insertedText">Text for inserting</param>
        ReplaceTextCommand(TextSource ts, List<Range> ranges, string insertedText)
            : base(ts)
        {
    ...
    /// Clear selected text
    /// </summary>
    struct ClearSelectedCommand : UndoableCommand
    {
        string deletedText;
        ...
        /// </summary>
        /// <param name="tb">Underlaying textbox</param>
        ClearSelectedCommand(TextSource ts): base(ts)
        {
        }
    ...
    /// Replaces text
    /// </summary>
    struct ReplaceMultipleTextCommand : UndoableCommand
    {
        ...
        /// <param name="ranges">List of ranges for replace</param>
        ReplaceMultipleTextCommand(TextSource ts, List<ReplaceRange> ranges)
            : base(ts)
        {
            //sort ranges by place
            ranges.Sort((r1, r2) =>
    ...
    /// Removes lines
    /// </summary>
    struct RemoveLinesCommand : UndoableCommand
    {
        List<int> iLines;
        List<string> prevText = new List<string>();
        ...
        /// <param name="ranges">List of ranges for replace</param>
        /// <param name="insertedText">Text for inserting</param>
        RemoveLinesCommand(TextSource ts, List<int> iLines)
            : base(ts)
        {
            //sort iLines
            iLines.Sort();
    ...
    /// </summary>
    struct MultiRangeCommand : UndoableCommand
    {
        UndoableCommand cmd;
        Range range;
        List<UndoableCommand> commandsByRanges = new List<UndoableCommand>();

        MultiRangeCommand(UndoableCommand command):base(command.ts)
        {
            this->cmd = command;
        ...
        void ExecuteInsertTextCommand(ref int iChar, string text)
        {
        ...
        void ExecuteCommand(ref int iChar)
        {
    ...
    /// </summary>
    struct SelectCommand : UndoableCommand
    {
        SelectCommand(TextSource ts):base(ts)
        {
        }
        ...
        override void OnTextChanged(bool invert)
        {
        }
