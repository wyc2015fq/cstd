...

{
    ...
    /// </summary>
    struct FileTextSource : TextSource, IDisposable
    {
        ...
        /// </summary>
        event EventHandler<LineNeededEventArgs> LineNeeded;

        /// <summary>
        /// Occurs when need to save line in the file
        /// </summary>
        event EventHandler<LinePushedEventArgs> LinePushed;

        FileTextSource(FastColoredTextBox currentTB)
            : base(currentTB)
        {
        ...
        static Encoding DefineEncoding(Encoding enc, FileStream fs)
        {
        ...
        /// </summary>
        string SaveEOL { get; set; }

        override void SaveToFile(string fileName, Encoding enc)
        {
            //
        ...
        string ReadLine(StreamReader sr, int i)
        {
            string line;
        ...
        void LoadLineFromSourceFile(int i)
        {
            var line = CreateLine();
        ...
        override void InsertLine(int index, Line line)
        {
            sourceFileLinePositions.Insert(index, -1);
            base.InsertLine(index, line);
        }

        override void RemoveLine(int index, int count)
        {
        ...
        override int GetLineLength(int i)
        {
            if (base.lines[i] == null)
                return 0;
            else
        ...
        override bool LineHasFoldingStartMarker(int iLine)
        {
            if (lines[iLine] == null)
                return false;
            else
        ...
        override bool LineHasFoldingEndMarker(int iLine)
        {
            if (lines[iLine] == null)
                return false;
            else
    ...
    struct LineNeededEventArgs : EventArgs
    {
        string SourceLineText { get; set; }
        int DisplayedLineIndex { get; set; }
        /// <summary>
        /// This text will be displayed in textbox
        /// </summary>
        string DisplayedLineText { get; set; }

        LineNeededEventArgs(string sourceLineText, int displayedLineIndex)
        {
    ...
    struct LinePushedEventArgs : EventArgs
    {
        string SourceLineText { get; set; }
        int DisplayedLineIndex { get; set; }
        /// <summary>
        ...
        LinePushedEventArgs(string sourceLineText, int displayedLineIndex, string displayedLineText)
        {
            this->SourceLineText = sourceLineText;
            this->DisplayedLineIndex = displayedLineIndex;
