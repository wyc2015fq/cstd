...
static int autosetup_capture_mode_v4l2(CvCaptureCAM_V4L* capture)
{
  if (try_palette_v4l2(capture, V4L2_PIX_FMT_BGR24) == 0) {
  ...
  else if (try_palette_v4l2(capture, V4L2_PIX_FMT_YVU420) == 0) {
    PALETTE_YVU420 = 1;
  }
  else if (try_palette_v4l2(capture, V4L2_PIX_FMT_YUV411P) == 0) {
    PALETTE_YUV411P = 1;
  }
  else
    ...
    */
    if (try_palette_v4l2(capture, V4L2_PIX_FMT_MJPEG) == 0 ||
      ...
      else if (try_palette_v4l2(capture, V4L2_PIX_FMT_SN9C10X) == 0) {
        PALETTE_SN9C10X = 1;
      }
      else if (try_palette_v4l2(capture, V4L2_PIX_FMT_SBGGR8) == 0) {
        PALETTE_SBGGR8 = 1;
      }
      else if (try_palette_v4l2(capture, V4L2_PIX_FMT_SGBRG) == 0) {
        PALETTE_SGBRG = 1;
      }
      else {
...
try_again:
  ...
  /* Set up Image data */
  cvInitImageHeader(capture,
      capture->captureWindow.width,
      capture->captureWindow.height ,
      IPL_DEPTH_8U, 3, IPL_ORIGIN_TL, 4);
  /* Allocate space for RGBA data */
  capture->data = (char*) cvAlloc(capture->imageSize);
...
static CvCaptureCAM_V4L* icvCaptureFromCAM_V4L(int index)
{
  ...
  capture->Close = (void*) icvCloseCAM_V4L;
  capture->GrabFrame = (void*) GrabFrameCAM_V4L;
  //capture->SetFramesPerSec = (void*)icvSetPropertyCAM_V4L;
  //capture->SetFramesPerSec = (void*)icvGetPropertyCAM_V4L;
...
static unsigned char* icvRetrieveFrameCAM_V4L(CvCaptureCAM_V4L* capture)
{
  ...
  if (V4L2_SUPPORT == 1) {
    ...
    if (((unsigned long) capture->width != capture->form.fmt.pix.width)
        || ((unsigned long) capture->height != capture->form.fmt.pix.height)) {
      cvFree(&capture->data);
      cvInitImageHeader(capture,
...
#endif /* HAVE_CAMV4L2 */
  {
    ...
    if ((capture->width != capture->mmaps[ capture->bufferIndex ].width)
        || (capture->height != capture->mmaps[ capture->bufferIndex ].height)) {
      cvFree(&capture->data);
      cvInitImageHeader(capture,
          capture->captureWindow.width,
          capture->captureWindow.height ,
...
static int GrabFrameCAM_V4L(CvCaptureCAM_V4L* capture)
{
  return icvGrabFrameCAM_V4L(capture) && icvRetrieveFrameCAM_V4L(capture);
}
