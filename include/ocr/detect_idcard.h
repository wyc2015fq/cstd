...
#include "../stdc/imgex.h"
#include "features2d/mser.hpp"
...
CRotatedRect face_rect_to_card_rect(const CRotatedRect& face, double y_off_scale, double scalex, double scaley) {
  double w = face.size.width;
  double len = -w * 2;
  double a = (face.angle) * CC_PI / 180;
  FPOINT center = ptMove(face.center, face.angle - 90, y_off_scale*len);
  FPOINT end = fPOINT(center.x + len*cos(a), center.y + len*sin(a));
  return cRotatedRect(end, fSIZE(w * 6* scalex, w * 4 * scaley), face.angle);
}

struct FaceDetector {
  vector<CRotatedRect> rects;
  int run(const img_t* src) {
    buf_t bf[1] = { 0 };
    bfsetsize(bf, 50 * _1M);
    XRECT out[100];
    rects.resize(0);
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      n = face_detect(bf, vis_nesting_face20110713, gray->rows, gray->cols, gray->data, gray->step,
        0, 0, ss, 1, 1000, 1.1, 1, 0.8, 5, out, countof(out), 0);
      for (int j = 0; j < n; j++) {
        const XRECT& r = out[j];
        CRotatedRect rr = cRotatedRect(fPOINT(r.x + r.w *0.5, r.y + r.h *0.5), fSIZE(r.w, r.h), 0);
        FPOINT center = fPOINT(gray->cols / 2., gray->rows / 2.);
        rr = rect_rotate90(rr, fSIZE(gray->cols, gray->rows), -i);
        rects.push_back(rr);
      }
    }
...
struct get_angle_t {
  ...
  int run(img_t src) {
    ...
    cv::img_t mserMapMat = cv::img_t::zeros(src.size(), CV_8UC1);
    vector<int> cnt;
    vector<int> cnt1;
    if (1) {
    ...
    for (int i = (int)regContours.size() - 1; i >= 0; i--)
    {
      ...
      CRotatedRect r(FPOINT(rc.x + rc.width / 2, rc.y + rc.height / 2), fSIZE(rc.width, rc.height), 0);
      r = minAreaRect(p);
      //int k = 10;
      ...
      {
        //CRotatedRect rectPoint = minAreaRect(p);
    ...
    if (0) {
      img_t element = getStructuringElement(MORPH_RECT, Size(3, 3));
    ...
    if (1) {
      ...
      HoughLinesP(mserMapMat, lines, 1, CC_PI / 180, 50, 30, 30);
...
struct get_rect_t {
  vector<CRotatedRect> lines_rect;
  vector<CRotatedRect> lines_ok;
  vector<vec4i> lines;
  ...
  int run(const img_t& src) {
    ...
    img_t gray;
    std::vector<cv::Rect> bboxes1;
    std::vector<cv::CRotatedRect> vrr;
    std::vector<std::vector<cv::Point> > regContours;
    cv::cvtColor(src, gray, CV_BGR2GRAY);
    ...
    if (0) {
      img_t element = getStructuringElement(MORPH_RECT, Size(1, 3));
    ...
    if (1) {
      ...
      for (int i = 0; i < (int)contours.size(); i++) {
        ...
        Scalar color = Scalar(rng.uniform(0, 255), rng.uniform(0, 255), rng.uniform(0, 255));
        //cv::rectangle(color_edge, r0, color, 1);
        cv::CRotatedRect r = minAreaRect(contours[i]);
        r = curr(r);
        double aa = r.size.width*1. / r.size.height;
    ...
    vector<vector<CRotatedRect>> rrects2;
    vector<int> labels;
    if (1) {
    vector<int> cnt;
      vector<Point > pts;
      vec4i top_line = vec4i(0, 1000, 200, 1000);
      double max_angle = 20; //最大允许倾斜角度
      vector<CRotatedRect> vec_rrect2 = vec_rrect;
      vec_rrect.clear();
      if (ret && linesxx.size() > 0) {
        ...
        for (int i = 0; i < vec_rrect2.size(); ++i) {
          const CRotatedRect& r = vec_rrect2[i];
    ...
    if (bot_rrect.size() > 5) {
      ...
      if (0) {
        vector<CRotatedRect> bot_rrect2 = bot_rrect;
        bot_rrect.clear();
        CRotatedRect r = minAreaRect(bot_rrect2);
        Vec4f l = linesxx[0];
    ...
    if (ret && linesxx.size() > 0) {
      //rrects.resize();
      int nclasses = cv::partition(vec_rrect, labels, [&meanh](const CRotatedRect& r1, const CRotatedRect& r2) {
      ...
      for (int i = 0; i < nclasses; ++i) {
        CRotatedRect r = myMinAreaRect(rrects2[i], meanh);
        r = curr(r);
        const vector<CRotatedRect>& rr = rrects2[i];
        double sumw = 0;
      ...
      sort(lines_rect.begin(), lines_rect.end(), [](const CRotatedRect& a, const CRotatedRect& b) {
