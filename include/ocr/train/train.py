...
# 数据集地址
path = 'E:/OCR_Line/chars1/fz-v4.0'
# 模型保存地址
model_path = 'E:/OCR_Line/model/charcnn/fc_model.ckpt'
    ...
    with tf.variable_scope('layer1-conv1'):
       # 初始化权重conv1_weights为可保存变量，大小为5x5,3个通道（RGB），数量为32个
        conv1_weights = tf.get_variable(
            "weight", [5, 5, 3, 32], initializer=tf.truncated_normal_initializer(stddev=0.1))
...
# -----------------------第三层----------------------------
    ...
    with tf.variable_scope("layer5-conv3"):
        conv3_weights = tf.get_variable(
            "weight", [3, 3, 64, 128], initializer=tf.truncated_normal_initializer(stddev=0.1))
...
# -----------------------第四层----------------------------
    ...
    with tf.variable_scope("layer7-conv4"):
        conv4_weights = tf.get_variable(
            "weight", [3, 3, 128, 128], initializer=tf.truncated_normal_initializer(stddev=0.1))
        conv4_biases = tf.get_variable(
            "bias", [128], initializer=tf.constant_initializer(0.0))
        conv4 = tf.nn.conv2d(pool3, conv4_weights, strides=[
                             1, 1, 1, 1], padding='SAME')
...
# -----------------------第五层---------------------------
    with tf.variable_scope('layer9-fc1'):
        ...
        if regularizer != None:
            tf.add_to_collection('losses', regularizer(fc1_weights))  # 正则化矩阵
