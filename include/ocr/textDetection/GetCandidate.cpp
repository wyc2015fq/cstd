...
std::vector<Candidate> GetCandidate::run(cv::Mat& Image)
{
	...
	CharacterClassifier = RTrees::load<RTrees>(FN_CHARACTER_CLASSIFIER);
	featureExtract();
	swtprocess();
	Filter();
...
void GetCandidate::featureExtract()
{
	...
	mser->detectRegions(this->gray_source_image,this->strVectorStore, bboxes);
}
...
void GetCandidate::Filter()
{
	...
	for(int i = 0; i< ccStore.size();i++)
	{
		...
		if( CharacterClassifier->predict(rtVector) > 0.15 ) candidateStore.push_back(cd);
	}
}
