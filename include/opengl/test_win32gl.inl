...
static void setupPixelFormat(HWND hwnd, HDC* hDC, HGLRC* hRC)
{
  ...
  *hDC = GetDC(hwnd);
  ...
  pfd.nVersion = 1;//版本
  pfd.dwFlags = PFD_DRAW_TO_WINDOW | PFD_SUPPORT_OPENGL | PFD_DOUBLEBUFFER;//支持双缓存
...
static void setupPalette(HDC hDC)
{
  ...
  //建一个简单的RGB调色板
  {
    ...
    for (i = 0; i < paletteSize; i++) {
      pPal->palPalEntry[i].peRed = (((i >> pfd.cRedShift) & redMask) * 255) / greemMask;
