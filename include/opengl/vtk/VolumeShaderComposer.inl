...
//--------------------------------------------------------------------------
std::string ComputeClipPositionImplementation(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
...
//--------------------------------------------------------------------------
std::string ComputeTextureCoordinates(OpenGLGPUVolumeRayCast* s)
{
  return std::string(
...
//--------------------------------------------------------------------------
std::string BaseDeclarationVertex(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
...
//--------------------------------------------------------------------------
std::string BaseDeclarationFragment(OpenGLGPUVolumeRayCast* s,
...
//--------------------------------------------------------------------------
std::string BaseInit(OpenGLGPUVolumeRayCast* s, int lightingComplexity)
{
  std::string shaderStr = std::string("\
\n bool l_adjustTextureExtents = !in_cellFlag;"
                                     );
  if (s->CurrentPass != DepthPass &&
      s->UseDepthPass && s->BlendMode ==
      COMPOSITE_BLEND) {
    shaderStr += std::string("\
\n //\
...
\n bool l_skip = false;");
  if (s->Shade && lightingComplexity == 1) {
    shaderStr += std::string("\
...
//--------------------------------------------------------------------------
std::string BaseImplementation(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
\n l_skip = false;"
                    );
...
//--------------------------------------------------------------------------
std::string BaseExit(OpenGLGPUVolumeRayCast* s)
{
  return std::string();
}
//--------------------------------------------------------------------------
std::string ComputeGradientDeclaration(OpenGLGPUVolumeRayCast* s,
    int noOfComponents,
    int independentComponents,
    const std::string* gradientTableMap)
{
  int i;
...
//--------------------------------------------------------------------------
std::string ComputeLightingDeclaration(OpenGLGPUVolumeRayCast* s,
    int noOfComponents,
    int independentComponents,
    int numberOfLights,
    int lightingComplexity)
...
\n vec4 finalColor = vec4(0.0);"
  ...
  int shadeReqd = s->Shade &&
      (s->BlendMode ==
          COMPOSITE_BLEND);
  if (shadeReqd || s->HasGradientOpacity) {
    shaderStr += std::string("\
...
//--------------------------------------------------------------------------
std::string ComputeRayDirectionDeclaration(OpenGLGPUVolumeRayCast* s)
{
  if (!s->ParallelProjection) {
    return std::string("\
...
//--------------------------------------------------------------------------
std::string ComputeColorDeclaration(OpenGLGPUVolumeRayCast* s,
    int noOfComponents,
    int independentComponents,
    const std::string* colorTableMap)
{
  int i;
  if (noOfComponents == 1) {
    return std::string("\
...
//--------------------------------------------------------------------------
std::string ComputeOpacityDeclaration(OpenGLGPUVolumeRayCast* s,
    int noOfComponents,
    int independentComponents,
    const std::string* opacityTableMap)
{
  int i;
  if (noOfComponents > 1 && independentComponents) {
    std::string shaderStr;
...
//--------------------------------------------------------------------------
std::string ShadingDeclarationVertex(OpenGLGPUVolumeRayCast* s)
{
  return std::string();
}
//--------------------------------------------------------------------------
std::string ShadingDeclarationFragment(OpenGLGPUVolumeRayCast* s)
{
  return std::string();
}
//--------------------------------------------------------------------------
std::string ShadingInit(OpenGLGPUVolumeRayCast* s)
{
  if (s->BlendMode == MAXIMUM_INTENSITY_BLEND) {
    return std::string("\
...
\n vec4 l_minValue = vec4(1.0);"
  ...
  else if (s->BlendMode == AVERAGE_INTENSITY_BLEND) {
    return std::string("\
\n //We get data between 0.0 - 1.0 range\
...
\n uvec4 l_numSamples = uvec4(0);"
  ...
  else if (s->BlendMode == ADDITIVE_BLEND) {
    return std::string("\
\n //We get data between 0.0 - 1.0 range\
...
//--------------------------------------------------------------------------
std::string ShadingImplementation(OpenGLGPUVolumeRayCast* s,
    ImageData* maskInput,
    Texture* mask, MaskTypeT maskType,
    int noOfComponents,
...
\n }"
  ...
  else if (s->BlendMode == MINIMUM_INTENSITY_BLEND) {
    if (noOfComponents > 1) {
      if (!independentComponents) {
        shaderStr += std::string("\
...
\n float intensity = in_scalarsRange[i][0] + (in_scalarsRange[i][1] - in_scalarsRange[i][0]) * scalar[i];\
\n if (in_averageIPRange.x <= intensity && intensity <= in_averageIPRange.y) {\
...
\n }"
      ...
      shaderStr += std::string("\
\n // Get the intensity in volume scalar range\
\n float intensity = in_scalarsRange[0][0] + (in_scalarsRange[0][1] - in_scalarsRange[0][0]) * scalar.x;\
\n if (in_averageIPRange.x <= intensity && intensity <= in_averageIPRange.y) {\
...
\n l_sumValue.x = l_sumValue.x + opacity * scalar.x;"
  ...
  else if (s->BlendMode == COMPOSITE_BLEND) {
    if (noOfComponents > 1 && independentComponents) {
      shaderStr += std::string("\
...
\n {\
");
      if (s->UseDepthPass && s->CurrentPass ==
          DepthPass) {
        shaderStr += std::string("\
...
\n g_fragColor = (1.0f - g_fragColor.a) * tmp + g_fragColor;"
    ...
    else if (s->UseDepthPass && s->CurrentPass ==
        DepthPass) {
      shaderStr += std::string("\
\n g_srcColor = vec4(0.0);\
...
//--------------------------------------------------------------------------
std::string PickingActorPassExit(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
...
//--------------------------------------------------------------------------
std::string PickingIdLow24PassExit(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
...
//--------------------------------------------------------------------------
std::string PickingIdMid24PassExit(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
\n // Special coloring mode which renders the voxel index in fragments that\
\n // have accumulated certain level of opacity. Used during the selection\
...
//--------------------------------------------------------------------------
std::string ShadingExit(OpenGLGPUVolumeRayCast* s, int noOfComponents, int independentComponents)
{
  if (s->UseDepthPass && s->CurrentPass ==
      DepthPass &&
      s->BlendMode == COMPOSITE_BLEND) {
    return std::string();
  }
  else if (s->BlendMode == MAXIMUM_INTENSITY_BLEND) {
    if (noOfComponents > 1 && independentComponents) {
      return std::string("\
\n g_srcColor = vec4(0);\
\n for (int i = 0; i < in_noOfComponents; ++i)\
...
//--------------------------------------------------------------------------
std::string TerminationDeclarationVertex(OpenGLGPUVolumeRayCast* s)
{
  return std::string();
}
//--------------------------------------------------------------------------
std::string TerminationDeclarationFragment(OpenGLGPUVolumeRayCast* s)
{
  return std::string();
}
//--------------------------------------------------------------------------
std::string PickingActorPassDeclaration(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
\n uniform vec3 in_propId;");
};
//--------------------------------------------------------------------------
std::string TerminationInit(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
...
//--------------------------------------------------------------------------
std::string TerminationImplementation(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
...
//--------------------------------------------------------------------------
std::string TerminationExit(OpenGLGPUVolumeRayCast* s)
{
  return std::string();
}
//--------------------------------------------------------------------------
std::string CroppingDeclarationVertex(OpenGLGPUVolumeRayCast* s)
{
  return std::string();
}
//--------------------------------------------------------------------------
std::string CroppingDeclarationFragment(OpenGLGPUVolumeRayCast* s)
{
  if (!s->Cropping) {
    return std::string();
  }
...
//--------------------------------------------------------------------------
std::string CroppingInit(OpenGLGPUVolumeRayCast* s)
{
  if (!s->Cropping) {
    return std::string();
  }
...
//--------------------------------------------------------------------------
std::string CroppingImplementation(OpenGLGPUVolumeRayCast* s)
{
  if (!s->Cropping) {
    return std::string();
  }
...
//--------------------------------------------------------------------------
std::string CroppingExit(OpenGLGPUVolumeRayCast* s)
{
  return std::string();
}
//--------------------------------------------------------------------------
std::string ClippingDeclarationVertex(OpenGLGPUVolumeRayCast* s)
{
  return std::string();
}
//--------------------------------------------------------------------------
std::string ClippingDeclarationFragment(OpenGLGPUVolumeRayCast* s)
{
  return std::string();
}
//--------------------------------------------------------------------------
std::string ClippingInit(OpenGLGPUVolumeRayCast* s)
{
  if (!s->ClippingPlanes) {
    return std::string();
  }
...
//--------------------------------------------------------------------------
std::string ClippingImplementation(OpenGLGPUVolumeRayCast* s)
{
  if (!s->ClippingPlanes) {
    return std::string();
  }
...
//--------------------------------------------------------------------------
std::string ClippingExit(OpenGLGPUVolumeRayCast* s)
{
  return std::string();
}
//--------------------------------------------------------------------------
std::string BinaryMaskDeclaration(OpenGLGPUVolumeRayCast* s, ImageData* maskInput, Texture* mask)
{
  if (!mask || !maskInput) {
    return std::string();
  }
...
//--------------------------------------------------------------------------
std::string BinaryMaskImplementation(OpenGLGPUVolumeRayCast* s, ImageData* maskInput, Texture* mask, int maskType)
{
  if (!mask || !maskInput ||
      maskType == LabelMapMaskType) {
    return std::string();
...
//--------------------------------------------------------------------------
std::string CompositeMaskDeclarationFragment(OpenGLGPUVolumeRayCast* s, ImageData* maskInput, Texture* mask, int maskType)
{
  if (!mask || !maskInput ||
      maskType != LabelMapMaskType) {
    return std::string();
...
//--------------------------------------------------------------------------
std::string CompositeMaskImplementation(OpenGLGPUVolumeRayCast* s, ImageData* maskInput, Texture* mask, int maskType, int noOfComponents)
{
  if (!mask || !maskInput ||
      maskType != LabelMapMaskType) {
    return std::string();
...
//--------------------------------------------------------------------------
std::string RenderToImageInit(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
...
//--------------------------------------------------------------------------
std::string RenderToImageImplementation(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
...
//--------------------------------------------------------------------------
std::string RenderToImageExit(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
...
//--------------------------------------------------------------------------
std::string DepthPassInit(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
...
//--------------------------------------------------------------------------
std::string DepthPassImplementation(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
...
//--------------------------------------------------------------------------
std::string DepthPassExit(OpenGLGPUVolumeRayCast* s)
{
  return std::string("\
\n vec4 depthValue = in_projectionMatrix * in_modelViewMatrix *\
\n in_volumeMatrix * in_textureDatasetMatrix *\
