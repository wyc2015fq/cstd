#include "math.inl"
#define vtkErrorMacro printf
#define vtkWarningMacro printf
#define vtkGenericWarningMacro printf
#define vtkDebugMacro printf
...
inline void vtkColorTransferFunctionLabToMsh(const float lab[3], float msh[3])
{
  const float& L = lab[0];
  const float& a = lab[1];
  const float& b = lab[2];
...
inline void vtkColorTransferFunctionMshToLab(const float msh[3], float lab[3])
{
  const float& M = msh[0];
  const float& s = msh[1];
  const float& h = msh[2];
...
inline double vtkColorTransferFunctionAdjustHue(const float msh[3],
    double unsatM)
{
  if (msh[0] >= unsatM - 0.1) {
...
static void vtkColorTransferFunctionInterpolateDiverging(double s,
    const float rgb1[3],
    const float rgb2[3],
    float result[3])
{
  ...
  if ((msh1[1] > 0.05) && (msh2[1] > 0.05)
      && (vtkColorTransferFunctionAngleDiff(msh1[2], msh2[2]) > 0.33 * M_PI)) {
    // Insert the white midpoint by setting one end to white and adjusting the
    // scalar value.
    double Mmid = MAX(msh1[0], msh2[0]);
    Mmid = MAX(88.0, Mmid);
    if (s < 0.5) {
      msh2[0] = Mmid;
      msh2[1] = 0.0;
  ...
  LabToRGB(labTmp, result);
}
//----------------------------------------------------------------------------
// Returns a table of RGB colors at regular intervals along the function
void GetRGBTable(const vtkCTFNode* Nodes, int Nodes_size, const int* pScale, const int* pClamping,
    const int* pColorSpace, const int* pHSVWrap, const float* pRange, const float* pNanColor,
    const float* AboveRangeColor, const float* BelowRangeColor,
    double xStart, double xEnd, int size, float* table)
{
  int i, j;
  int ColorSpace = pColorSpace ? *pColorSpace : VTK_CTF_RGB;
  int Scale = pScale ? *pScale : VTK_CTF_LINEAR;
  float NanColor[3] = {0.5, 0.0, 0.0};
  float Range[2] = {0, 0};
  int Clamping = pClamping ? *pClamping : 1;
  int HSVWrap = pHSVWrap ? *pHSVWrap : 1; //By default HSV will be wrap
  if (pNanColor) {
    COPY(3, pNanColor, NanColor);
  }
  if (pRange) {
    COPY(2, pRange, Range);
  }
  ...
  if (IsNan(xStart) || IsNan(xEnd)) {
    float* tableEntry = table;
    for (i = 0; i < size; i++) {
      tableEntry[0] = NanColor[0];
      tableEntry[1] = NanColor[1];
      tableEntry[2] = NanColor[2];
      tableEntry += 3;
    }
    return;
  }
  int idx = 0;
  int numNodes = (int)(Nodes_size);
  ...
  double lastB = 0.0;
  if (numNodes != 0) {
    lastR = Nodes[numNodes - 1].R;
    lastG = Nodes[numNodes - 1].G;
    lastB = Nodes[numNodes - 1].B;
  }
  float* tptr = NULL;
  double x = 0.0;
  double x1 = 0.0;
  ...
  bool usingLogScale = Scale == VTK_CTF_LOG10;
  if (usingLogScale) {
    // Note: This requires range[0] <= range[1].
    usingLogScale = Range[0] > 0.0;
  }
  double logStart = 0.0;
  double logEnd = 0.0;
  double logX = 0.0;
  ...
  for (i = 0; i < size; i++) {
    ...
    if (size > 1) {
      if (usingLogScale) {
        ...
        x = xStart + ((double)(i) / (double)(size - 1))
            * (xEnd - xStart);
      }
    }
    else {
    ...
    while (idx < numNodes && x > Nodes[idx].X) {
      idx++;
      ...
      if (idx < numNodes) {
        ...
        rgb1[0] = Nodes[idx - 1].R;
        rgb2[0] = Nodes[idx ].R;
        rgb1[1] = Nodes[idx - 1].G;
        rgb2[1] = Nodes[idx ].G;
        rgb1[2] = Nodes[idx - 1].B;
        rgb2[2] = Nodes[idx ].B;
        // We only need the previous midpoint and sharpness
        // since these control this region
        midpoint = Nodes[idx - 1].Midpoint;
        sharpness = Nodes[idx - 1].Sharpness;
    ...
    else if (x < Range[0] || (IsInf(x) && x < 0)) {
      tptr[0] = 0.0;
      tptr[1] = 0.0;
      tptr[2] = 0.0;
      if (Clamping) {
        if (BelowRangeColor) {
          COPY(3, BelowRangeColor, tptr);
        }
        else {
          if (numNodes > 0) {
            tptr[0] = Nodes[0].R;
    ...
    else if (idx == 0 && fabs(x - xStart) < 1e-6) {
      if (numNodes > 0) {
        tptr[0] = Nodes[0].R;
        tptr[1] = Nodes[0].G;
        tptr[2] = Nodes[0].B;
    ...
    // Otherwise, we are between two nodes - interpolate
      ...
      if (sharpness < 0.01) {
        ...
        if (ColorSpace == VTK_CTF_RGB) {
        ...
        else if (ColorSpace == VTK_CTF_HSV) {
          float hsv1[3], hsv2[3];
          RGBToHSV(rgb1, hsv1);
          RGBToHSV(rgb2, hsv2);
          if (HSVWrap &&
          ...
          HSVToRGB(hsvTmp, tptr);
        }
        else if (ColorSpace == VTK_CTF_LAB) {
          float lab1[3], lab2[3];
          RGBToLab(rgb1, lab1);
          RGBToLab(rgb2, lab2);
          float labTmp[3];
          ...
          // Now convert back to RGB
          LabToRGB(labTmp, tptr);
        }
        else if (ColorSpace == VTK_CTF_DIVERGING) {
...
struct vtkCTFFindNodeOutOfRange {
  double X1;
  double X2;
  bool operator()(const vtkCTFNode* node) {
    return (node->X < this->X1 ||
...
struct vtkColorTransferFunctionInternals {
...
struct vtkColorTransferFunction {
  ...
  */
  ...
  void vtkColorTransferFunction::GetTable(double xStart, double xEnd,
    ...
    for (i = 0; i < size; i++) {
      ...
      if (x > this->Range[1]) {
        ...
        tptr[2] = 0.0;
        if (this->Clamping) {
          if (this->UseAboveRangeColor) {
            MEMCPY(tptr, this->AboveRangeColor, 3);
          }
          else {
            tptr[0] = lastR;
            tptr[1] = lastG;
      ...
      // Are we before the first node? If so, duplicate this node's values.
      // We have to deal with -inf here
      else if (x < this->Range[0] || (IsInf(x) && x < 0)) {
        tptr[0] = 0.0;
        tptr[1] = 0.0;
        tptr[2] = 0.0;
        if (this->Clamping) {
          if (this->UseBelowRangeColor) {
            MEMCPY(tptr, this->BelowRangeColor, 3);
          }
          else {
            if (numNodes > 0) {
  ...
  //----------------------------------------------------------------------------
  template<class T>
  static void vtkColorTransferFunctionIndexedMapData(
