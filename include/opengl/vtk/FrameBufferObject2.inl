//----------------------------------------------------------------------------
void CreateFBO(GLuint* pfbo)
{
  *pfbo = 0;
  GLuint temp;
  glGenFramebuffers(1, &temp);
  OpenGLCheckError("failed at glGenFramebuffers");
  *pfbo = temp;
}
//----------------------------------------------------------------------------
void DestroyFBO(GLuint* pfbo)
{
  // because we don't hold a reference to the render
  // context we don't have any control on when it is
  // destroyed. In fact it may be destroyed before we are(eg smart pointers),
  // in which case we should do nothing.
  if (pfbo && *pfbo) {
    GLuint fbo = *pfbo;
    glDeleteFramebuffers(1, &fbo);
    OpenGLCheckError("failed at glDeleteFramebuffers");
    *pfbo = 0;
  }
}
#if 0
...
void vtkFrameBufferObject2::RestorePreviousBuffers(unsigned int mode)
{
  ...
  case GL_FRAMEBUFFER:
    ...
    OpenGLCheckError("failed at glDrawBuffer");
    glReadBuffer((GLenum)this->PreviousReadBuffer);
    OpenGLCheckError("failed at glReadBuffer");
    break;
...
void vtkFrameBufferObject2::Bind(unsigned int mode)
{
  ...
  OpenGLCheckError("failed at glBindFramebuffer");
}
//----------------------------------------------------------------------------
...
void vtkFrameBufferObject2::ActivateDrawBuffers(unsigned int* ids, int num)
{
  ...
  OpenGLCheckError("failed at glDrawBuffers");
}
//----------------------------------------------------------------------------
...
void vtkFrameBufferObject2::DeactivateDrawBuffers()
{
  GLenum att = GL_NONE;
  glDrawBuffers(1, &att);
  OpenGLCheckError("failed at glDrawBuffers(GL_NONE)");
}
//----------------------------------------------------------------------------
...
void vtkFrameBufferObject2::DeactivateReadBuffer()
{
  ...
  OpenGLCheckError("failed at glReadBuffer(GL_NONE)");
}
//----------------------------------------------------------------------------
    ...
    unsigned int handle)
{
  glFramebufferTexture2D(
      ...
      handle,
      0);
  OpenGLCheckError("failed at glFramebufferTexture2D");
}
//----------------------------------------------------------------------------
    ...
    unsigned int handle)
{
  glFramebufferRenderbuffer(
      ...
      GL_RENDERBUFFER,
      handle);
  OpenGLCheckError("failed at glFramebufferRenderbuffer");
}
//----------------------------------------------------------------------------
void vtkFrameBufferObject2::AddColorAttachment(
    unsigned int mode,
...
int vtkFrameBufferObject2::CheckFrameBufferStatus(unsigned int mode)
{
  ...
  GLenum status = glCheckFramebufferStatus((GLenum)mode);
  OpenGLCheckError("failed at glCheckFramebufferStatus");
  switch (status) {
  case GL_FRAMEBUFFER_COMPLETE:
    str = "FBO complete";
    ok = true;
