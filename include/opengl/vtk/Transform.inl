//----------------------------------------------------------------------------
void Translate(float9 s, double x, double y, double z)
{
...
//----------------------------------------------------------------------------
void Scale(float9 s, double x, double y, double z)
{
...
//----------------------------------------------------------------------------
void Rotate(float9 s, double angle, double x, double y, double z)
{
  ...
  double xz = x * z;
  double yz = y * z;
  double ss = ww - xx - yy - zz;
  matrix[0 * 4 + 0] = xx * 2 + ss;
  matrix[1 * 4 + 0] = (xy + wz) * 2;
  matrix[2 * 4 + 0] = (xz - wy) * 2;
  matrix[0 * 4 + 1] = (xy - wz) * 2;
  matrix[1 * 4 + 1] = yy * 2 + ss;
  matrix[2 * 4 + 1] = (yz + wx) * 2;
  matrix[0 * 4 + 2] = (xz + wy) * 2;
  matrix[1 * 4 + 2] = (yz - wx) * 2;
  matrix[2 * 4 + 2] = zz * 2 + ss;
  Concatenate16(s, matrix);
}
#if 0
//----------------------------------------------------------------------------
void TransformNormalAtPoint(const float point[3], const float in[3], float out[3])
{
  float matrix[9];
  float coord[3];
  InternalTransformDerivative(point, coord, matrix);
  Transpose9(matrix, matrix);
  LinearSolve9(matrix, in, out);
  Normalize(out);
}
void TransformNormalAtPoint(const float point[3],
    const float in[3],
    float out[3])
{
  float coord[3];
...
//----------------------------------------------------------------------------
void TransformVectorAtPoint(const float point[3],
    const float in[3],
    float out[3])
{
  this->Update();
    ...
    vtkDataArray* outVrs)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    if (inVrs) {
      ...
      Multiply9(matrix, coord, coord);
      outVrs->InsertNextTuple(coord);
    }
    if (inNms) {
      inNms->GetTuple(i, coord);
      Transpose9(matrix, matrix);
      LinearSolve9(matrix, coord, coord);
      Normalize(coord);
...
//----------------------------------------------------------------------------
void DeepCopy(vtkAbstractTransform* transform)
{
...
public:
  ...
  static vtkSimpleTransform* New() {
    VTK_STANDARD_NEW_BODY(vtkSimpleTransform)
  }
