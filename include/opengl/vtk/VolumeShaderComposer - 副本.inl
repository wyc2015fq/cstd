...
//--------------------------------------------------------------------------
std::string BaseDeclarationVertex()
{
  return std::string("\
...
//--------------------------------------------------------------------------
std::string BaseDeclarationFragment(int lightingComplexity,
    bool hasGradientOpacity,
    int noOfComponents,
    int independentComponents, Passes CurrentPass, bool UseDepthPass)
{
...
//--------------------------------------------------------------------------
std::string BaseInit(int lightingComplexity, bool CompositeBlend, bool hasGradientOpacity)
{
  std::string shaderStr = std::string("\
 \n bool l_adjustTextureExtents = !in_cellFlag;"
                                     );
  if (glMapper->CurrentPass() != vtkOpenGLGPUVolumeRayCastMapper::DepthPass &&
...
//--------------------------------------------------------------------------
std::string ComputeGradientDeclaration(int noOfComponents, int independentComponents, char** gradientTableMap, bool hasGradientOpacity)
{
  std::string shaderStr;
  if (hasGradientOpacity && (noOfComponents == 1 || !independentComponents)) {
    shaderStr += std::string("\
 \nuniform sampler2D in_gradientTransferFunc;\
 \nfloat computeGradientOpacity(vec4 grad)\
 \n {\
 \n return texture2D(" + std::string(gradientTableMap[0]) + ", vec2(grad.w, 0.0)).r;\
 \n }"
                            );
  }
  else if (noOfComponents > 1 && independentComponents && hasGradientOpacity) {
    int i;
 ...
 \n if (component == " + toString.str() + ")");
      shaderStr += std::string("\
 \n {\
 \n return texture2D(" + std::string(gradientTableMap[i]) + ", vec2(grad.w, 0.0)).r;\
 \n }"
                              );
      // Reset
      toString.str("");
...
//--------------------------------------------------------------------------
std::string ComputeLightingDeclaration(int noOfComponents, int independentComponents, int lightingComplexity, int shadeReqd, bool hasGradientOpacity)
{
  std::string shaderStr = std::string("\
 ...
 \n vec4 finalColor = vec4(0.0);"
  ...
  if (shadeReqd || hasGradientOpacity) {
    shaderStr += std::string("\
...
//--------------------------------------------------------------------------
std::string ComputeRayDirectionDeclaration(bool hasParallelProjection)
{
  if (!hasParallelProjection) {
    return std::string("\
...
//--------------------------------------------------------------------------
std::string ComputeColorDeclaration(int noOfComponents, int independentComponents, char** colorTableMap)
{
  if (noOfComponents == 1) {
    return std::string("\
...
//--------------------------------------------------------------------------
std::string ComputeOpacityDeclaration(int noOfComponents, int independentComponents, char** opacityTableMap)
{
  if (noOfComponents > 1 && independentComponents) {
    std::string shaderStr;
    std::ostringstream toString;
    int i;
    for (i = 0; i < noOfComponents; ++i) {
      shaderStr += std::string("\n uniform sampler2D ") + opacityTableMap[i] + std::string(";");
    }
    shaderStr += std::string("\
...
//--------------------------------------------------------------------------
std::string ShadingInit(int BlendMode)
{
  if (BlendMode = MAXIMUM_INTENSITY_BLEND) {
    return std::string("\
 ...
 \n vec4 l_minValue = vec4(1.0);"
  ...
  else if (BlendMode == AVERAGE_INTENSITY_BLEND) {
    return std::string("\
 \n //We get data between 0.0 - 1.0 range\
 ...
 \n uvec4 l_numSamples = uvec4(0);"
  ...
  else if (BlendMode == ADDITIVE_BLEND) {
    return std::string("\
 \n //We get data between 0.0 - 1.0 range\
...
//--------------------------------------------------------------------------
std::string ShadingImplementation(int maskType,
    int noOfComponents,
    int independentComponents,
    int BlendMode, bool DepthPass)
{
  std::string shaderStr = std::string("\
 \n if (!l_skip)\
 \n {\
 ...
 \n }"
  ...
  else if (BlendMode == MINIMUM_INTENSITY_BLEND) {
    if (noOfComponents > 1) {
      if (!independentComponents) {
        shaderStr += std::string("\
 ...
 \n l_sumValue.x = l_sumValue.x + opacity * scalar.x;"
  ...
  else if (BlendMode == COMPOSITE_BLEND) {
    if (noOfComponents > 1 && independentComponents) {
      shaderStr += std::string("\
 ...
 \n }"
      ...
      else if (maskType != LabelMapMaskType) {
        shaderStr += std::string("\
 \n // Data fetching from the red channel of volume texture\
...
//--------------------------------------------------------------------------
std::string PickingIdMid24PassExit(
)
{
  return std::string("\
 \n // Special coloring mode which renders the voxel index in fragments that\
...
//--------------------------------------------------------------------------
std::string ShadingExit(int noOfComponents,
    int independentComponents, int DepthPass, int BlendMode)
{
  if (DepthPass && BlendMode == COMPOSITE_BLEND) {
    return std::string();
  }
  else if (BlendMode == MAXIMUM_INTENSITY_BLEND) {
    if (noOfComponents > 1 && independentComponents) {
      return std::string("\
 \n g_srcColor = vec4(0);\
 \n for (int i = 0; i < in_noOfComponents; ++i)\
...
//--------------------------------------------------------------------------
std::string CroppingDeclarationFragment(int Cropping)
{
  if (!Cropping) {
    return std::string();
  }
...
//--------------------------------------------------------------------------
std::string CroppingInit(int Cropping)
{
  if (!Cropping) {
    return std::string();
  }
...
//--------------------------------------------------------------------------
std::string CroppingImplementation(int Cropping)
{
  if (!Cropping) {
    return std::string();
  }
...
//--------------------------------------------------------------------------
std::string ClippingInit(int ClippingPlanes, int ParallelProjection)
{
  if (!ClippingPlanes) {
    return std::string();
  }
...
//--------------------------------------------------------------------------
std::string ClippingImplementation(int ClippingPlanes)
{
  if (!ClippingPlanes) {
    return std::string();
  }
...
//--------------------------------------------------------------------------
std::string BinaryMaskDeclaration(int maskType)
{
  if (NoMask == maskType) {
    return std::string();
  }
...
//--------------------------------------------------------------------------
std::string BinaryMaskImplementation(int maskType)
{
  if (maskType == LabelMapMaskType) {
    return std::string();
  }
...
//--------------------------------------------------------------------------
std::string CompositeMaskDeclarationFragment(int maskType)
{
  if (maskType != LabelMapMaskType) {
    return std::string();
  }
...
//--------------------------------------------------------------------------
std::string CompositeMaskImplementation(int maskType, int noOfComponents)
{
  if (maskType != LabelMapMaskType) {
    return std::string();
  }
