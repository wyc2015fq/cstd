...
#undef MFC_DATA
#define MFC_DATA MFC_CORE_DATA


typedef struct { void* ptr; } * HGDIOBJ;

/*============================================================================*/
// CGdiObject abstract struct for CDC SelectObject

struct CGdiObject
{
	static CGdiObject* FromHandle(HGDIOBJ hObject);
	virtual HGDIOBJ GetSafeHandle() const = 0;
	virtual	BOOL Attach(HGDIOBJ hObject);
	virtual HGDIOBJ Detach();
	virtual BOOL Delete();

// Operations
	//int GetObject(int nCount, LPVOID lpObject) const;

	//virtual UINT GetObjectType() const = 0;
	virtual BOOL CreateStockObject(int nIndex);
	virtual BOOL UnrealizeObject();
	virtual BOOL Compare(const CGdiObject& obj) const = 0;
...
struct CPalette : public CGdiObject
{
...
#undef DrawText
#undef DrawTextEx
	virtual int _MFC_FUNCNAME(DrawText)(LPCTSTR lpszString, int nCount, LPRECT lpRect,
				UINT nFormat);
			int _MFC_FUNCNAME(DrawText)(const CString& str, LPRECT lpRect, UINT nFormat);

	virtual int _MFC_FUNCNAME(DrawTextEx)(LPTSTR lpszString, int nCount, LPRECT lpRect,
				UINT nFormat, LPDRAWTEXTPARAMS lpDTParams);
			int _MFC_FUNCNAME(DrawTextEx)(const CString& str, LPRECT lpRect, UINT nFormat, LPDRAWTEXTPARAMS lpDTParams);
...
#undef GetTextMetrics
	BOOL _MFC_FUNCNAME(GetTextMetrics)(LPTEXTMETRIC lpMetrics) const;
...
struct MFC_MSGMAP_ENTRY;       // declared below after CWnd

struct MFC_MSGMAP
{
	const MFC_MSGMAP* (PASCAL* pfnGetBaseMap)();
	const MFC_MSGMAP_ENTRY* lpEntries;
};


#define DECLARE_MESSAGE_MAP() \
protected: \
	static const MFC_MSGMAP* PASCAL GetThisMessageMap(); \
	virtual const MFC_MSGMAP* GetMessageMap() const; \

#define BEGIN_TEMPLATE_MESSAGE_MAP(theClass, type_name, baseClass)			\
	PTM_WARNING_DISABLE														\
	template < typename type_name >											\
	const MFC_MSGMAP* theClass< type_name >::GetMessageMap() const			\
		{ return GetThisMessageMap(); }										\
	template < typename type_name >											\
	const MFC_MSGMAP* PASCAL theClass< type_name >::GetThisMessageMap()		\
	{																		\
		typedef theClass< type_name > ThisClass;							\
		typedef baseClass TheBaseClass;										\
		static const MFC_MSGMAP_ENTRY _messageEntries[] =					\
		{

#define BEGIN_MESSAGE_MAP(theClass, baseClass) \
	PTM_WARNING_DISABLE \
	const MFC_MSGMAP* theClass::GetMessageMap() const \
		{ return GetThisMessageMap(); } \
	const MFC_MSGMAP* PASCAL theClass::GetThisMessageMap() \
	{ \
...
#define END_MESSAGE_MAP() \
		{0, 0, 0, 0, AfxSig_end, (MFC_PMSG)0 } \
	}; \
		static const MFC_MSGMAP messageMap = \
...
#endif 
// private structures
struct MFC_CMDHANDLERINFO;  // info about where the command is handled
struct MFC_EVENT;           // info about an event
struct CTypeLibCache;        // cache for OLE type libraries

/*============================================================================*/
// OLE interface map handling (more in MFCDISP.H)

struct MFC_INTERFACEMAP_ENTRY
{
	const void* piid;       // the interface id (IID) (NULL for aggregate)
	size_t nOffset;         // offset of the interface vtable from m_unknown
};

struct MFC_INTERFACEMAP
{
#ifdef _MFCDLL
	const MFC_INTERFACEMAP* (PASCAL* pfnGetBaseMap)(); // NULL is root class
#else
	const MFC_INTERFACEMAP* pBaseMap;
#endif
	const MFC_INTERFACEMAP_ENTRY* pEntry; // map for this class
};
...
private: \
	static const MFC_INTERFACEMAP_ENTRY _interfaceEntries[]; \
protected: \
	static const MFC_INTERFACEMAP interfaceMap; \
	static const MFC_INTERFACEMAP* PASCAL GetThisInterfaceMap(); \
	virtual const MFC_INTERFACEMAP* GetInterfaceMap() const; \
...
// (see CCmdTarget in MFCWIN.H for more information)

#define METHOD_PROLOGUE(theClass, localClass) \
	theClass* pThis = \
		((theClass*)((BYTE*)this - offsetof(theClass, m_x##localClass))); \
	MFC_MANAGE_STATE(pThis->m_pModuleState) \
...
#define BEGIN_INTERFACE_PART(localClass, baseClass) \
	struct X##localClass : public baseClass \
	{ \
	public: \
...
#define BEGIN_INTERFACE_MAP(theClass, theBase) \
	const MFC_INTERFACEMAP* PASCAL theClass::GetThisInterfaceMap() \
		{ return &theClass::interfaceMap; } \
	const MFC_INTERFACEMAP* theClass::GetInterfaceMap() const \
		{ return &theClass::interfaceMap; } \
	MFC_COMDAT const MFC_INTERFACEMAP theClass::interfaceMap = \
		{ &theBase::GetThisInterfaceMap, &theClass::_interfaceEntries[0], }; \
	MFC_COMDAT const MFC_INTERFACEMAP_ENTRY theClass::_interfaceEntries[] = \
	{ \
...
/*============================================================================*/
// OLE dispatch map handling (more in MFCDISP.H)

struct MFC_DISPMAP_ENTRY;

struct MFC_DISPMAP
{
#ifdef _MFCDLL
	const MFC_DISPMAP* (PASCAL* pfnGetBaseMap)();
#else
	const MFC_DISPMAP* pBaseMap;
#endif
	const MFC_DISPMAP_ENTRY* lpEntries;
	UINT* lpEntryCount;
...
private: \
	static const MFC_DISPMAP_ENTRY _dispatchEntries[]; \
	static UINT _dispatchEntryCount; \
	static DWORD _dwStockPropMask; \
protected: \
	static const MFC_DISPMAP dispatchMap; \
	static const MFC_DISPMAP* PASCAL GetThisDispatchMap(); \
	virtual const MFC_DISPMAP* GetDispatchMap() const; \
...
struct MFC_OLECMDMAP
{
#ifdef _MFCDLL
	const MFC_OLECMDMAP* (PASCAL* pfnGetBaseMap)();
#else
	const MFC_OLECMDMAP* pBaseMap;
#endif
	const MFC_OLECMDMAP_ENTRY* lpEntries;
};

#ifdef _MFCDLL
#define DECLARE_OLECMD_MAP() \
private: \
	static const MFC_OLECMDMAP_ENTRY _commandEntries[]; \
protected: \
	static const MFC_OLECMDMAP commandMap; \
	static const MFC_OLECMDMAP* PASCAL GetThisCommandMap(); \
	virtual const MFC_OLECMDMAP* GetCommandMap() const; \
...
#define BEGIN_OLECMD_MAP(theClass, baseClass) \
	const MFC_OLECMDMAP* PASCAL theClass::GetThisCommandMap() \
		{ return &theClass::commandMap; } \
	const MFC_OLECMDMAP* theClass::GetCommandMap() const \
		{ return &theClass::commandMap; } \
	MFC_COMDAT const MFC_OLECMDMAP theClass::commandMap = \
	{ &baseClass::GetThisCommandMap, &theClass::_commandEntries[0] }; \
	MFC_COMDAT const MFC_OLECMDMAP_ENTRY theClass::_commandEntries[] = \
	{ \
...
/*============================================================================*/
// OLE event sink map handling (more in MFCDISP.H)

struct MFC_EVENTSINKMAP_ENTRY;

struct MFC_EVENTSINKMAP
{
#ifdef _MFCDLL
	const MFC_EVENTSINKMAP* (PASCAL* pfnGetBaseMap)();
#else
	const MFC_EVENTSINKMAP* pBaseMap;
#endif
	const MFC_EVENTSINKMAP_ENTRY* lpEntries;
	UINT* lpEntryCount;
};
...
private: \
	static const MFC_EVENTSINKMAP_ENTRY _eventsinkEntries[]; \
	static UINT _eventsinkEntryCount; \
protected: \
	static const MFC_EVENTSINKMAP eventsinkMap; \
	static const MFC_EVENTSINKMAP* PASCAL GetThisEventSinkMap(); \
	virtual const MFC_EVENTSINKMAP* GetEventSinkMap() const; \
...
/*============================================================================*/
// OLE connection map handling (more in MFCDISP.H)

struct MFC_CONNECTIONMAP_ENTRY
{
	const void* piid;   // the interface id (IID)
	size_t nOffset;         // offset of the interface vtable from m_unknown
};

struct MFC_CONNECTIONMAP
{
#ifdef _MFCDLL
	const MFC_CONNECTIONMAP* (PASCAL* pfnGetBaseMap)(); // NULL is root class
#else
	const MFC_CONNECTIONMAP* pBaseMap;
#endif
	const MFC_CONNECTIONMAP_ENTRY* pEntry; // map for this class
};

#ifdef _MFCDLL
#define DECLARE_CONNECTION_MAP() \
private: \
	static const MFC_CONNECTIONMAP_ENTRY _connectionEntries[]; \
protected: \
	static const MFC_CONNECTIONMAP connectionMap; \
	static const MFC_CONNECTIONMAP* PASCAL GetThisConnectionMap(); \
	virtual const MFC_CONNECTIONMAP* GetConnectionMap() const; \
...
struct COccManager;      // forward reference (see ..\src\occimpl.h)

struct MFC_NOVTABLE CCmdTarget : public CObject
{
...
// Overridables
	...
	virtual BOOL OnCmdMsg(UINT nID, int nCode, void* pExtra,
		MFC_CMDHANDLERINFO* pHandlerInfo);
...
protected:
	...
	DECLARE_MESSAGE_MAP()       // base struct - no {{ }} macros

	DECLARE_OLECMD_MAP()
	friend struct COleCmdUI;
...
protected:
	...
	void GetStandardProp(const MFC_DISPMAP_ENTRY* pEntry,
		VARIANT* pvarResult, UINT* puArgErr);
	SCODE SetStandardProp(const MFC_DISPMAP_ENTRY* pEntry,
		DISPPARAMS* pDispParams, UINT* puArgErr);

	// DISPID to dispatch map lookup
	static UINT PASCAL GetEntryCount(const MFC_DISPMAP* pDispMap);
	const MFC_DISPMAP_ENTRY* PASCAL GetDispEntry(LONG memid);
	static LONG PASCAL MemberIDFromName(const MFC_DISPMAP* pDispMap, LPCTSTR lpszName);
...
#endif
	SCODE CallMemberFunc(const MFC_DISPMAP_ENTRY* pEntry, WORD wFlags,
		VARIANT* pvarResult, DISPPARAMS* pDispParams, UINT* puArgErr);

	friend struct COleDispatchImpl;

public:
	// OLE event sink implementation
	BOOL OnEvent(UINT idCtrl, MFC_EVENT* pEvent,
		MFC_CMDHANDLERINFO* pHandlerInfo);
protected:
	const MFC_EVENTSINKMAP_ENTRY* PASCAL GetEventSinkEntry(UINT idCtrl,
		MFC_EVENT* pEvent);
	...
	MFC_MODULE_STATE* m_pModuleState;
	friend struct CInnerUnknown;
	friend UINT APIENTRY _AfxThreadEntry(void* pParam);

	virtual BOOL GetExtraConnectionPoints(CPtrArray* pConnPoints);
	virtual LPCONNECTIONPOINT GetConnectionHook(const IID& iid);

	friend struct COleConnPtContainer;
};

struct CCmdUI        // simple helper class
{
public:
// Attributes
	UINT m_nID;
...
#define MFC_MSG_CALL
#endif
typedef void (MFC_MSG_CALL CCmdTarget::*MFC_PMSG)(void);

enum MFC_DISPMAP_FLAGS
{
	afxDispCustom = 0,
	afxDispStock = 1
};
...
struct MFC_DISPMAP_ENTRY
{
	...
	MFC_PMSG pfn;           // normal member On<membercall> or, OnGet<property>
	MFC_PMSG pfnSet;        // special member for OnSet<property>
	size_t nPropOffset;     // property offset
	MFC_DISPMAP_FLAGS flags;// flags (e.g. stock/custom)
};
#pragma warning(pop)

struct MFC_EVENTSINKMAP_ENTRY
{
	MFC_DISPMAP_ENTRY dispEntry;
	UINT nCtrlIDFirst;
	UINT nCtrlIDLast;
};
...
/// <summary>
/// CGestureConfig struct allows to customize Windows gesture features such as zoom, pan or rotate. This struct is used in CWnd::SetGestureConfig and CWnd::GetGestureConfig methods.</summary>
struct CGestureConfig : public CObject
{
	friend struct CWnd;
...
//---------------------------
extern MFC_IMPORT_DATA UINT MFC_WM_DRAW2D;
extern MFC_IMPORT_DATA UINT MFC_WM_RECREATED2DRESOURCES;
...
struct MFC_MSGMAP_ENTRY
{
	...
	MFC_PMSG pfn;    // routine to call (or special value)
};

/*============================================================================*/
// CWnd - a Microsoft Windows application window

struct COleDropTarget;   // for more information see MFCOLE.H
...
#undef SendMessage
	LRESULT _MFC_FUNCNAME(SendMessage)(UINT message, WPARAM wParam = 0, LPARAM lParam = 0) const;
...
// Window Size and Position Functions
	...
	static MFC_DATA const CWnd wndTop; // SetWindowPos's pWndInsertAfter
	static MFC_DATA const CWnd wndBottom; // SetWindowPos's pWndInsertAfter
	static MFC_DATA const CWnd wndTopMost; // SetWindowPos pWndInsertAfter
	static MFC_DATA const CWnd wndNoTopMost; // SetWindowPos pWndInsertAfter
...
// (NOTE: Dialog-Box Items/Controls are not necessarily in dialog boxes!)
	...
	int DlgDirListComboBox(_Inout_z_ LPTSTR lpPathSpec, _In_ int nIDComboBox,
					_In_ int nIDStaticPath, _In_ UINT nFileType);
	MFC_DEPRECATED("CWnd::DlgDirSelect(lpszOut, nControlId) is no longer supported. Instead, use CWnd::DlgDirSelect(lpszOut, nSize, nControlId)")
	...
	MFC_DEPRECATED("CWnd::DlgDirSelectComboBox(lpszOut, nControlId) is no longer supported. Instead, use CWnd::DlgDirSelectComboBox(lpszOut, nSize, nControlId)")
...
#undef MessageBox
	int _MFC_FUNCNAME(MessageBox)(LPCTSTR lpszText, LPCTSTR lpszCaption = NULL,
			UINT nType = MB_OK);
...
// OLE control wrapper functions
   COleControlSite* GetOleControlSite(UINT idControl) const;
	void MFC_CDECL InvokeHelper(DISPID dwDispID, WORD wFlags,
		VARTYPE vtRet, void* pvRet, const BYTE* pbParamInfo, ...);
	void MFC_CDECL SetProperty(DISPID dwDispID, VARTYPE vtProp, ...);
...
public :
	...
	/// <remarks>
	/// Override this method in a derived struct and draw on the specified device context in order to customize thumbnail and peek.
...
protected :
	...
	friend BOOL MFCAPI AfxWinInit(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance,
...
protected:
	...
	struct XAccessible //: public IAccessible
	{	
	public:
	...
	friend struct XAccessible; 

	struct XAccessibleServer //: public IAccessibleServer
	{	
	public:
		virtual ULONG __stdcall AddRef(); 
		virtual ULONG __stdcall Release(); 
	...
	friend struct XAccessibleServer;
...
protected:
	...
	friend struct COleDropTarget;
	friend struct CFrameWnd;
	...
	friend struct COccManager;
	friend struct COleControlSite;
	friend struct COleControlContainer;
	...
	virtual BOOL SetOccDialogInfo(struct _MFC_OCC_DIALOG_INFO* pOccDialogInfo);
	virtual _MFC_OCC_DIALOG_INFO* GetOccDialogInfo();
...
protected:
	...
	friend void MFCAPI _AfxStandardSubclass(HWND);
	friend LRESULT CALLBACK _AfxCbtFilterHook(int, WPARAM, LPARAM);
	friend LRESULT MFCAPI AfxCallWndProc(CWnd*, HWND, UINT, WPARAM, LPARAM);
	...
	template <struct TReturnType, struct TCchType >
...
LPCTSTR MFCAPI AfxRegisterWndClass(UINT nClassStyle,
	HCURSOR hCursor = 0, HBRUSH hbrBackground = 0, HICON hIcon = 0);

BOOL MFCAPI AfxRegisterClass(WNDCLASS* lpWndClass);

// helper to initialize rich edit 1.0 control
BOOL MFCAPI AfxInitRichEdit();
// helper to initialize rich edit 2.0 control
BOOL MFCAPI AfxInitRichEdit2();

#ifdef _UNICODE
// helper to initialize rich edit 4.1 control
BOOL MFCAPI AfxInitRichEdit5();
#endif
// Implementation
LRESULT CALLBACK AfxWndProc(HWND, UINT, WPARAM, LPARAM);

WNDPROC MFCAPI AfxGetAfxWndProc();
#define AfxWndProc (*AfxGetAfxWndProc())

typedef void (MFC_MSG_CALL CWnd::*MFC_PMSGW)(void);
	// like 'MFC_PMSG' but for CWnd derived classes only

typedef void (MFC_MSG_CALL CWinThread::*MFC_PMSGT)(void);
	// like 'MFC_PMSG' but for CWinThread-derived classes only
...
protected:
	...
	_MFC_OCC_DIALOG_INFO* m_pOccDialogInfo;
	virtual BOOL SetOccDialogInfo(_MFC_OCC_DIALOG_INFO* pOccDialogInfo);
	virtual _MFC_OCC_DIALOG_INFO* GetOccDialogInfo();
...
// Attributes
	...
	MFC_ANSI_DEPRECATED BOOL GetIdealSize(_Out_ LPSIZE psize) const;
	MFC_ANSI_DEPRECATED BOOL SetImageList(_In_ PBUTTON_IMAGELIST pbuttonImagelist);
	MFC_ANSI_DEPRECATED BOOL GetImageList(_In_ PBUTTON_IMAGELIST pbuttonImagelist) const;
	MFC_ANSI_DEPRECATED BOOL SetTextMargin(_In_ LPRECT pmargin);
	MFC_ANSI_DEPRECATED BOOL GetTextMargin(_Out_ LPRECT pmargin) const;
...
#if defined(UNICODE)
struct CSplitButton : public CButton
{
...
// NOTE: This struct must remain a binary-compatible subset
...
// Attributes
	...
	MFC_ANSI_DEPRECATED BOOL SetCueBanner(_In_z_ LPCWSTR lpszText, _In_ BOOL fDrawIfFocused = FALSE);
	MFC_ANSI_DEPRECATED BOOL GetCueBanner(_Out_writes_z_(cchText) LPWSTR lpszText, _In_ int cchText) const;
...
#define MFC_PREVIEW_VIEW_ID 1

/// <summary>
/// This struct implements a window that is placed on a host window provided by the Shell 
/// for Rich Preview. </summary>
struct CMFCPreviewCtrlImpl : public CWnd
{
...
/*============================================================================*/
// CFrameWnd - base struct for SDI and other frame windows
...
#define MFC_MBV_KEEPVISIBLE    0x01L // always visible
#define MFC_MBV_DISPLAYONFOCUS 0x02L // toggle state on ALT
#define MFC_MBV_DISPLAYONF10   0x04L // display on F10

// Frame window menu bar visibility states
#define MFC_MBS_VISIBLE 0x01L // visible
#define MFC_MBS_HIDDEN  0x02L // hidden

struct CPrintPreviewState;  // forward reference (see afxext.h)
struct CControlBar;          // forward reference (see afxext.h)
struct CReBar;               // forward reference (see afxext.h)

struct CDockBar;             // forward reference (see afxpriv.h)
struct CMiniDockFrameWnd;    // forward reference (see afxpriv.h)
struct CDockState;           // forward reference (see afxpriv.h)

struct COleFrameHook;        // forward reference (see ..\src\oleimpl2.h)
...
// Constructors
public:
	static MFC_DATA const CRect rectDefault;
	CFrameWnd();
	...
	CWnd* CreateView(CCreateContext* pContext, UINT nID = MFC_IDW_PANE_FIRST);
...
protected:
	...
	friend struct CWnd;  // for access to m_bModalDisable
	friend struct CReBar; // for access to m_bInRecalcLayout
	...
	friend struct CWinApp;
};

/*============================================================================*/
// MDI Support

struct CMDIFrameWnd : public CFrameWnd
{
...
struct CMDIChildWnd : public CFrameWnd
{
...
struct CMiniFrameWnd : public CFrameWnd
{
...
/*============================================================================*/
// struct CView is the client area UI for a document

struct CPrintDialog;     // forward reference (see afxdlgs.h)
struct CPreviewView;     // forward reference (see afxpriv.h)
struct CSplitterWnd;     // forward reference (see afxext.h)
struct COleServerDoc;    // forward reference (see afxole.h)

typedef DWORD DROPEFFECT;
struct COleDataObject;   // forward reference (see afxole.h)

struct MFC_NOVTABLE CView : public CWnd
{
	friend struct CWinAppEx;
...
protected:
	...
	friend struct CDocument;
	friend struct CDocTemplate;
	friend struct CPreviewView;
	friend struct CFrameWnd;
	friend struct CMDIFrameWnd;
	friend struct CMDIChildWnd;
	friend struct CSplitterWnd;
	friend struct COleServerDoc;
	friend struct CDocObjectServer;
...
/*============================================================================*/
// struct CCtrlView allows almost any control to be a view

struct MFC_NOVTABLE CCtrlView : public CView
{
...
/*============================================================================*/
// struct CScrollView supports simple scrolling and scaling

struct _MFC_MOUSEANCHORWND;

struct CScrollView : public CView
{
...
public:
	static MFC_DATA const SIZE sizeDefault;
...
// Implementation
protected:
	_MFC_MOUSEANCHORWND* m_pAnchorWindow; // window for wheel mouse anchor
	friend struct _MFC_MOUSEANCHORWND;
...
typedef UINT (MFC_CDECL *MFC_THREADPROC)(LPVOID);

struct COleMessageFilter;        // forward reference (see afxole.h)

BOOL MFCAPI AfxPumpMessage();
LRESULT MFCAPI AfxProcessWndProcException(CException*, const MSG* pMsg);
...
public:
	...
	CWinThread(MFC_THREADPROC pfnThreadProc, LPVOID pParam);

	// valid after construction
	LPVOID m_pThreadParams; // generic parameters passed to starting function
	MFC_THREADPROC m_pfnThreadProc;

	// set after OLE is initialized
	void (MFCAPI* m_lpfnOleTermOrFreeLib)(BOOL, BOOL);
...
CWinThread* MFCAPI AfxBeginThread(MFC_THREADPROC pfnThreadProc, LPVOID pParam,
	int nPriority = THREAD_PRIORITY_NORMAL, UINT nStackSize = 0,
	DWORD dwCreateFlags = 0, LPSECURITY_ATTRIBUTES lpSecurityAttrs = NULL);
CWinThread* MFCAPI AfxBeginThread(CRuntimeClass* pThreadClass,
	int nPriority = THREAD_PRIORITY_NORMAL, UINT nStackSize = 0,
	DWORD dwCreateFlags = 0, LPSECURITY_ATTRIBUTES lpSecurityAttrs = NULL);

CWinThread* MFCAPI AfxGetThread();
MSG* MFCAPI AfxGetCurrentMessage();

void MFCAPI AfxEndThread(UINT nExitCode, BOOL bDelete = TRUE);

void MFCAPI AfxInitThread();
void MFCAPI AfxTermThread(HINSTANCE hInstTerm = NULL);
...
BOOL MFCAPI AfxWinInit(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance,
	_In_z_ LPTSTR lpCmdLine, _In_ int nCmdShow);
void MFCAPI AfxWinTerm();

// Global Windows state data helper functions (inlines)
#ifdef _MFCDLL
ULONG MFCAPI AfxGetDllVersion();
#endif

CWinApp* MFCAPI AfxGetApp();
CWnd* MFCAPI AfxGetMainWnd();
HINSTANCE MFCAPI AfxGetInstanceHandle();
HINSTANCE MFCAPI AfxGetResourceHandle();
void MFCAPI AfxSetResourceHandle(HINSTANCE hInstResource);
LPCTSTR MFCAPI AfxGetAppName();
MFC_DEPRECATED("AfxLoadLangResourceDLL(LPCTSTR pszFormat) has been deprecated, use AfxLoadLangResourceDLL(LPCTSTR pszFormat, LPCTSTR pszPath) instead")
	HINSTANCE MFCAPI AfxLoadLangResourceDLL(LPCTSTR pszFormat);
HINSTANCE MFCAPI AfxLoadLangResourceDLL(LPCTSTR pszFormat, LPCTSTR pszPath);

// Use instead of PostQuitMessage in OLE server applications
void MFCAPI AfxPostQuitMessage(int nExitCode);
...
#else
HINSTANCE MFCAPI AfxFindResourceHandle(LPCTSTR lpszName, LPCTSTR lpszType);
#endif
...
LONG MFCAPI AfxDelRegTreeHelper(HKEY hParentKey, const CString& strKeyName, CAtlTransactionManager* pTM = NULL);

struct CRecentFileList;          // forward reference (see afxadv.h)
struct CDataRecoveryHandler;     // forward reference (see afxdatarecovery.h)

// access to message filter in CWinApp
COleMessageFilter* MFCAPI AfxOleGetMessageFilter();

/*============================================================================*/
// CCommandLineInfo

struct CCommandLineInfo : public CObject
{
public:
	// Sets default values
	CCommandLineInfo();
...
struct CDocManager : public CObject
{
...
#define _MFC_MRU_COUNT   4      // default support for 4 entries in file MRU
#define _MFC_MRU_MAX_COUNT 16   // currently allocated id range supports 16

#define _MFC_SYSPOLICY_NOTINITIALIZED			0
#define _MFC_SYSPOLICY_NORUN					1 
#define _MFC_SYSPOLICY_NODRIVES					2
#define _MFC_SYSPOLICY_RESTRICTRUN				4
#define _MFC_SYSPOLICY_NONETCONNECTDISCONNECTD	8
#define _MFC_SYSPOLICY_NOENTIRENETWORK			16
#define _MFC_SYSPOLICY_NORECENTDOCHISTORY		32
#define _MFC_SYSPOLICY_NOCLOSE					64
#define _MFC_SYSPOLICY_NOPLACESBAR				128
#define _MFC_SYSPOLICY_NOBACKBUTTON				256
#define _MFC_SYSPOLICY_NOFILEMRU				512
...
#define MFC_RESTART_MANAGER_SUPPORT_RESTART				0x01  // restart support, means application is registered via RegisterApplicationRestart
#define MFC_RESTART_MANAGER_SUPPORT_RECOVERY			0x02  // recovery support, means application is registered via RegisterApplicationRecoveryCallback
#define MFC_RESTART_MANAGER_AUTOSAVE_AT_RESTART			0x04  // auto-save support is enabled, documents will be autosaved at restart by restart manager
#define MFC_RESTART_MANAGER_AUTOSAVE_AT_INTERVAL		0x08  // auto-save support is enabled, documents will be autosaved periodically for crash recovery
#define MFC_RESTART_MANAGER_REOPEN_PREVIOUS_FILES		0x10  // reopen of previously opened documents is enabled, on restart all previous documents will be opened
#define MFC_RESTART_MANAGER_RESTORE_AUTOSAVED_FILES		0x20  // restoration of auto-saved documents is enabled, on restart user will be prompted to open auto-saved documents intead of last saved
#define MFC_RESTART_MANAGER_SUPPORT_NO_AUTOSAVE			MFC_RESTART_MANAGER_SUPPORT_RESTART | MFC_RESTART_MANAGER_SUPPORT_RECOVERY | MFC_RESTART_MANAGER_REOPEN_PREVIOUS_FILES
#define MFC_RESTART_MANAGER_SUPPORT_ALL_ASPECTS			MFC_RESTART_MANAGER_SUPPORT_NO_AUTOSAVE | MFC_RESTART_MANAGER_AUTOSAVE_AT_RESTART | MFC_RESTART_MANAGER_AUTOSAVE_AT_INTERVAL | MFC_RESTART_MANAGER_RESTORE_AUTOSAVED_FILES
#define MFC_RESTART_MANAGER_SUPPORT_RESTART_ASPECTS		MFC_RESTART_MANAGER_SUPPORT_RESTART | MFC_RESTART_MANAGER_AUTOSAVE_AT_RESTART | MFC_RESTART_MANAGER_REOPEN_PREVIOUS_FILES | MFC_RESTART_MANAGER_RESTORE_AUTOSAVED_FILES
#define MFC_RESTART_MANAGER_SUPPORT_RECOVERY_ASPECTS	MFC_RESTART_MANAGER_SUPPORT_RECOVERY | MFC_RESTART_MANAGER_AUTOSAVE_AT_INTERVAL | MFC_RESTART_MANAGER_REOPEN_PREVIOUS_FILES | MFC_RESTART_MANAGER_RESTORE_AUTOSAVED_FILES
...
// Attributes
	...
	// constructor or retreived from MFC_IDS_APP_TITLE.
...
public:
	...
	// Sets and initializes usage of HtmlHelp instead of WinHelp.
	void SetHelpMode( MFC_HELP_TYPE eHelpType );
	MFC_HELP_TYPE GetHelpMode();

	// help mode used by the app
	MFC_HELP_TYPE m_eHelpType;
...
protected:
	...
	void LoadStdProfileSettings(UINT nMaxMRU = _MFC_MRU_COUNT);

	void EnableShellOpen();

	// SetDialogBkColor is no longer supported.
	// To change dialog background and text color, handle WM_CTLCOLORDLG instead.
	MFC_DEPRECATED("CWinApp::SetDialogBkColor is no longer supported. Instead, handle WM_CTLCOLORDLG in your dialog")
	...
	MFC_DEPRECATED("CWinApp::Enable3dControls is no longer needed. You should remove this call.")
			BOOL Enable3dControls();
#ifndef _MFCDLL
	MFC_DEPRECATED("CWinApp::Enable3dControlsStatic is no longer needed. You should remove this call.")
...
// Overridables
	...
	virtual BOOL SupportsRestartManager() const { return m_dwRestartManagerSupportFlags & MFC_RESTART_MANAGER_SUPPORT_RESTART; }
	virtual BOOL SupportsApplicationRecovery() const { return m_dwRestartManagerSupportFlags & MFC_RESTART_MANAGER_SUPPORT_RECOVERY; }
	virtual BOOL SupportsAutosaveAtRestart() const { return m_dwRestartManagerSupportFlags & MFC_RESTART_MANAGER_AUTOSAVE_AT_RESTART; }
	virtual BOOL SupportsAutosaveAtInterval() const { return m_dwRestartManagerSupportFlags & MFC_RESTART_MANAGER_AUTOSAVE_AT_INTERVAL; }
	virtual BOOL ReopenPreviousFilesAtRestart() const { return m_dwRestartManagerSupportFlags & MFC_RESTART_MANAGER_REOPEN_PREVIOUS_FILES; }
	virtual BOOL RestoreAutosavedFilesAtRestart() const { return m_dwRestartManagerSupportFlags & MFC_RESTART_MANAGER_RESTORE_AUTOSAVED_FILES; }
...
/*============================================================================*/
// struct CDocTemplate creates documents

struct MFC_NOVTABLE CDocTemplate : public CCmdTarget
{
...
public:
	...
	/// <param name="pPreviewFrameClass">Specifies a pointer to a runtime struct information of preview frame.</param>
	/// <param name="pPreviewViewClass">Specifies a pointer to a runtime struct information of preview view.</param>
...
protected:
	...
	CRuntimeClass* m_pDocClass;				// struct for creating new documents
	CRuntimeClass* m_pFrameClass;			// struct for creating new frames
	CRuntimeClass* m_pViewClass;			// struct for creating new views
	CRuntimeClass* m_pOleFrameClass;		// struct for creating in-place frame
	CRuntimeClass* m_pOleViewClass;			// struct for creating in-place view
	CRuntimeClass* m_pPreviewFrameClass;	// struct for creating in-place preview frame
	CRuntimeClass* m_pPreviewViewClass;		// struct for creating in-place preview view
...
struct CSingleDocTemplate : public CDocTemplate
{
...
struct CMultiDocTemplate : public CDocTemplate
{
...
/*============================================================================*/
// struct CDocument is the main document data abstraction

struct MFC_NOVTABLE CDocument : public CCmdTarget
{
...
// Constructors
	...
	/// <summary>
	/// This struct implements IDocument interface required for Search and Organize handlers.</summary>
	/// <remarks>
	...
	struct CDocumentAdapter : public ATL::IDocument
	{
		friend struct CDocument;
	public:
		/// <summary>
	...
	friend struct CDocumentAdapter;
public:
	/// <summary>
...
// Overridables
	...
	/// <remarks>
	/// You can override this method in a derived struct to customize loading data from the stream. </remarks>
	/// <param name="pStream">A pointer to a stream. This stream is supplied by the Shell.</param>
	/// <param name="grfMode">Access mode to the stream.</param>
	...
	/// <remarks>
	/// You should override this method in a derived struct to initialize search content. 
	/// The content should be a string with parts delimited by ';'. For example, "point; rectangle; ole item". </remarks>
	virtual void InitializeSearchContent ()
	{
		TRACE0("Override this method in a derived struct to initialize chunk list for search filter.");
	}
...
public:
	...
	/// <summary>
	/// Override this method in a derived struct to draw content of thumbnail </summary>
...
/// <summary>
/// This is a struct which simplifies both chunk and property value pair logic. </summary>
/// <remarks>
/// To use, you simply create a CMFCFilterChunkValueImpl struct of the right kind
/// Example:
...
struct CMFCFilterChunkValueImpl : public ATL::IFilterChunkValue
{
public:
	/// <summary>
...
#ifdef _DEBUG
extern MFC_DATA UINT afxTraceFlags;
#endif // _DEBUG

#ifdef _DEBUG
#define DECLARE_MFC_TRACE_CATEGORY( name ) extern MFC_DATA ATL::CTraceCategory name;
#else
#define DECLARE_MFC_TRACE_CATEGORY( name ) const DWORD_PTR name = 0;
#endif

DECLARE_MFC_TRACE_CATEGORY( traceAppMsg )        // main message pump trace (includes DDE)
DECLARE_MFC_TRACE_CATEGORY( traceCmdRouting )    // Windows command routing trace
DECLARE_MFC_TRACE_CATEGORY( traceOle )          // special OLE callback trace
DECLARE_MFC_TRACE_CATEGORY( traceDatabase )     // special database trace
DECLARE_MFC_TRACE_CATEGORY( traceInternet )     // special Internet client trace
DECLARE_MFC_TRACE_CATEGORY( traceHtml )			// Html traces

/*============================================================================*/
// MessageBox helpers

void MFCAPI AfxFormatString1(CString& rString, UINT nIDS, LPCTSTR lpsz1);
void MFCAPI AfxFormatString2(CString& rString, UINT nIDS,
				LPCTSTR lpsz1, LPCTSTR lpsz2);
int MFCAPI AfxMessageBox(LPCTSTR lpszText, UINT nType = MB_OK,
				UINT nIDHelp = 0);
int MFCAPI AfxMessageBox(UINT nIDPrompt, UINT nType = MB_OK,
				UINT nIDHelp = (UINT)-1);

// Implementation string helpers
void MFCAPI AfxFormatStrings(CString& rString, UINT nIDS,
				LPCTSTR const* rglpsz, int nString);
void MFCAPI AfxFormatStrings(CString& rString, LPCTSTR lpszFormat,
				LPCTSTR const* rglpsz, int nString);
BOOL MFCAPI AfxExtractSubString(CString& rString, LPCTSTR lpszFullString,
...
void MFCAPI AfxLockTempMaps();
BOOL MFCAPI AfxUnlockTempMaps(BOOL bDeleteTemps = TRUE);

/*============================================================================*/
// Special OLE related functions (see OLELOCK.CPP)

void MFCAPI AfxOleOnReleaseAllObjects();
BOOL MFCAPI AfxOleCanExitApp();
void MFCAPI AfxOleLockApp();
void MFCAPI AfxOleUnlockApp();

void MFCAPI AfxOleSetUserCtrl(BOOL bUserCtrl);
BOOL MFCAPI AfxOleGetUserCtrl();

BOOL MFCAPI AfxOleLockControl(REFCLSID clsid);
BOOL MFCAPI AfxOleUnlockControl(REFCLSID clsid);
BOOL MFCAPI AfxOleLockControl(LPCTSTR lpszProgID);
BOOL MFCAPI AfxOleUnlockControl(LPCTSTR lpszProgID);
void MFCAPI AfxOleUnlockAllControls();
...
#ifdef _MFC_ENABLE_INLINES
#define _MFCWIN_INLINE MFC_INLINE
#include <afxwin1.inl>
#include <afxwin2.inl>
#include <afxwin3.inl>
#endif
