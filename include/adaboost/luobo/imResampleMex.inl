...
void resampleCoef( int ha, int hb, int *n, int **yas,
  int **ybs, T **wts, int bd[2], int pad DEFAULT(0) )
{
  int i, ya, yb;
...
void resample( T *A, T *B, int ha, int hb, int wa, int wb, int d, T r ) {
  ...
  for( z=0; z<d; z++ ) for( x=0; x<wb; x++ ) {
    ...
    #define FORs(X) if(sse) for(; y<ha-4; y+=4) STRm(Cf+y,X);
    #define FORr(X) for(; y<ha; y++) C[y] = X;
    if( wa==2*wb ) {
      FORs( ADD(LDu(Af0+y),LDu(Af1+y)) );
      FORr( A0[y]+A1[y] ); x1+=2;
    } else if( wa==3*wb ) {
      FORs( ADD3(LDu(Af0+y),LDu(Af1+y),LDu(Af2+y)) );
      FORr( A0[y]+A1[y]+A2[y] ); x1+=3;
    } else if( wa==4*wb ) {
      FORs( ADD4(LDu(Af0+y),LDu(Af1+y),LDu(Af2+y),LDu(Af3+y)) );
      FORr( A0[y]+A1[y]+A2[y]+A3[y] ); x1+=4;
    } else if( wa>wb ) {
      float wtsf[4]; int m=1; while( x1+m<wn && xb==xbs[x1+m] ) m++;
      for( x0=0; x0<(m<4?m:4); x0++ ) wtsf[x0]=(float)(xwts[x1+x0]);
      #define U(x) MUL( LDu(Af ## x + y), SET1f(wtsf[x]) )
      #define V(x) *(A ## x + y) * xwts[x1+x]
      if(m==1) { FORs(U(0));                     FORr(V(0)); }
      if(m==2) { FORs(ADD(U(0),U(1)));           FORr(V(0)+V(1)); }
      if(m==3) { FORs(ADD3(U(0),U(1),U(2)));      FORr(V(0)+V(1)+V(2)); }
      if(m>=4) { FORs(ADD4(U(0),U(1),U(2),U(3))); FORr(V(0)+V(1)+V(2)+V(3)); }
      #undef U
      #undef V
      for( x0=4; x0<m; x0++ ) {
        A1=A0+x0*ha; wt1=xwts[x1+x0]; Af1=(float*) A1; wt1f=(float)(wt1); y=0;
        FORs(ADD(_mm_load_ps(&Cf[y]),MUL(LDu(Af1+y),SET1f(wt1f)))); FORr(C[y]+A1[y]*wt1);
      }
      x1+=m;
    } else {
      bool xBd = x<xbd[0] || x>=wb-xbd[1]; x1++;
      if(xBd) memcpy(C,A0,ha*sizeof(T));
      if(!xBd) FORs(ADD(MUL(LDu(Af0+y),SET1f(wtf)),MUL(LDu(Af1+y),SET1f(wt1f))));
    ...
    if( ha==hb*2 ) {
      ...
      if(sse) for(; y<hb-4; y+=4) STRm(Bf0+y,MULf((float)r2,_mm_shuffle_ps(ADD(
...
void imResampleMex(int nlhs, mxArray *plhs, int nrhs, const mxArray *prhs) {
  ...
  nDims=mxGetNumberOfDimensions(prhs); id=mxGetClassID(prhs);
  ns = (int*) mxGetDimensions(prhs); nCh=(nDims==2) ? 1 : ns[2];
  if( (nDims!=2 && nDims!=3) ||
  ...
  // create output array
  mxSetNumericArray(plhs, 3, (const mwSize*) ms, id, mxREAL);
  n=ns[0]*ns[1]*nCh; m=ms[0]*ms[1]*nCh;

  // perform resampling (w appropriate type)
  A=mxGetData(prhs); B=mxGetData(plhs);
  if( id==mxSINGLE_CLASS ) {
    ...
    for(i=0; i<n; i++) A1[i]=(float) ((uchar*)A)[i];
    resample(A1, B1, ns[0], ms[0], ns[1], ms[1], nCh, float(nrm));
    for(i=0; i<m; i++) ((uchar*)B)[i]=(uchar) (B1[i]+.5);
	mxFree(A1), mxFree(B1);
#endif
  } else {
