...
void grad1( float *I, float *Gx, float *Gy, int h, int w, int x ) {
  ...
  if( h<4 || h%4>0 || ((size_t)(I)&15) || ((size_t)(Gx)&15) ) {
    ...
    _G=(__m128*) Gx; _Ip=(__m128*) Ip; _In=(__m128*) In; _r = SET1f(r);
...
void gradMag( float *I, float *M, float *O, int h, int w, int d, bool full ) {
  ...
  for( x=0; x<w; x++ ) {
    ...
    for( y=0; y<h4/4; y++ ) {
      _m = sseMIN( RCPSQRT(_M2[y]), SET1f(1e10f) );
      _M2[y] = RCP(_m);
      if(O) _Gx[y] = MUL( MUL(_Gx[y],_m), SET1f(acMult) );
      if(O) _Gx[y] = XOR( _Gx[y], AND(_Gy[y], SET1f(-0.f)) );
    };
    ...
    if( O!=0 && full ) {
      ...
      for( ; y<h-4; y+=4 ) STRu( O+y+x*h,
        ADD( LDu(O+y+x*h), AND(CMPLT(LDu(Gy+y),SET1f(0.f)),SET1f(PI)) ) );
...
void gradMagNorm( float *M, float *S, int h, int w, float norm ) {
  ...
  _S = (__m128*) S; _M = (__m128*) M; _norm = SET1f(norm);
  sse = !((size_t)(M)&15) && !((size_t)(S)&15);
  ...
  int nb, int n, float norm, int nOrients, bool full, bool interpolate )
{
  ...
  const __m128 _norm=SET1f(norm), _oMult=SET1f(oMult), _nbf=SET1f((float)nb);
  const __m128i _oMax=SET1i(oMax), _nb=SET1i(nb);
  ...
  if( interpolate ) for( i=0; i<=n-4; i+=4 ) {
    _o=MUL(LDu(O+i),_oMult); _o0=CVT(_o); _od=SUB(_o,CVTi(_o0));
    _o0=CVT(MUL(CVTi(_o0),_nbf)); _o0=ANDi(CMPGTi(_oMax,_o0),_o0); *_O0++=_o0;
    _o1=ADDi(_o0,_nb); _o1=ANDi(CMPGTi(_oMax,_o1),_o1); *_O1++=_o1;
    _m=MUL(LDu(M+i),_norm); *_M1=MUL(_od,_m); *_M0++=SUB(_m,*_M1); _M1++;
  } else for( i=0; i<=n-4; i+=4 ) {
    _o=MUL(LDu(O+i),_oMult); _o0=CVT(ADD(_o,SET1f(.5f)));
    _o0=CVT(MUL(CVTi(_o0),_nbf)); _o0=ANDi(CMPGTi(_oMax,_o0),_o0); *_O0++=_o0;
    *_M0++=MUL(LDu(M+i),_norm); *_M1++=SET1f(0.f); *_O1++=SET1i(0);
  }
  ...
  int bin, int nOrients, int softBin, bool full )
{
  ...
  for( x=0; x<w0; x++ ) {
    ...
    if( softBin<0 && softBin%2==0 ) {
      ...
      else for( y=0; y<h0;) { for( y1=0; y1<bin; y1++ ) { GH; } H1++; }
      #undef GH
      ...
      if( softBin<0 ) for( ; ; y++ ) {
        yb0 = (int) yb; if(yb0>=hb-1) break; GHinit; _m0=SET1f(M0[y]);
        if(hasLf) { _m=_mm_set_ps(0,0,ms[1],ms[0]); GH(H0+O0[y],_m,_m0); }
        if(hasRt) { _m=_mm_set_ps(0,0,ms[3],ms[2]); GH(H0+O0[y]+hb,_m,_m0); }
      } else for( ; ; y++ ) {
        yb0 = (int) yb; if(yb0>=hb-1) break; GHinit;
        _m0=SET1f(M0[y]); _m1=SET1f(M1[y]);
        if(hasLf) { _m=_mm_set_ps(0,0,ms[1],ms[0]);
          GH(H0+O0[y],_m,_m0); GH(H0+O1[y],_m,_m1); }
        if(hasRt) { _m=_mm_set_ps(0,0,ms[3],ms[2]);
  ...
  int nOrients, int softBin, bool full, float clip )
{
  ...
  hogChannels( H, R, N, hb, wb, nOrients, clip, 1 );
  wrFree(N); wrFree(R);
}
...
int mxSetMatrix3( mxArray* M, int h, int w, int d, mxClassID id, bool c, void **I ){
  ...
  *I = c ? mxCalloc(n,b) : mxMalloc(n*b);
  mxSetNumericMatrix(M, 0,0,id,mxREAL);
  mxSetData(M,*I); mxSetDimensions(M,dims,3); return 0;
}
  ...
  int nl1, int nr0, int nr1, int *h, int *w, int *d, mxClassID id, void **I )
{
  ...
  nDims = mxGetNumberOfDimensions(pr); dims = mxGetDimensions(pr);
  *h=dims[0]; *w=dims[1]; *d=(nDims==2) ? 1 : dims[2]; *I = mxGetPr(pr);
  if( nDims!=2 && nDims!=3 ) mexErrMsgTxt("I must be a 2D or 3D array.");
  if( mxGetClassID(pr)!=id ) mexErrMsgTxt("I has incorrect type.");
}
...
void mGrad2( int nl, mxArray *pl, int nr, const mxArray *pr ) {
  ...
  mxSetMatrix3(pl, h, w, d, mxSINGLE_CLASS, 0, (void**) &Gx );
  mxSetMatrix3(pl+1, h, w, d, mxSINGLE_CLASS, 0, (void**) &Gy );
  grad2( I, Gx, Gy, h, w, d );
}
...
void mGradMag( int nl, mxArray *pl, int nr, const mxArray *pr ) {
  ...
  mxSetMatrix3(pl,h,w,1,mxSINGLE_CLASS,0,(void**)&M);
  if(nl>=2) mxSetMatrix3(pl+1, h,w,1,mxSINGLE_CLASS,0,(void**)&O);
...
void mGradHist( int nl, mxArray *pl, int nr, const mxArray *pr ) {
  ...
  mxSetMatrix3(pl,hb,wb,nChns,mxSINGLE_CLASS,1,(void**)&H);
...
void gradientMex( int nl, mxArray *pl, int nr, const mxArray *pr ) {
  int f; char action[1024]; f=mxGetString(pr,action,1024); nr--; pr++;
