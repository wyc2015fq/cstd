...
static char* RCSid()
{
  return RCSid("$Id: show.c,v 1.326 2014/07/23 05:45:49 sfeam Exp $");
}
#endif
...
show_colorbox()
{
  ...
  if (color_box.where != SMCOLOR_BOX_NO) {
    if (color_box.layer == LAYER_FRONT) {
      fputs("drawn front\n\t", stderr);
    }
    else {
      fputs("drawn back\n\t", stderr);
...
show_pm3d()
{
  ...
  fputs("\tpm3d plotted at ", stderr);
  {
    ...
    for (; pm3d.where[i]; i++) {
      ...
      case PM3D_AT_SURFACE:
        fputs("SURFACE", stderr);
        break;
  ...
  case PM3D_WHICHCORNER_MEAN:
    fputs("averaged 4 corners\n", stderr);
    break;

  case PM3D_WHICHCORNER_GEOMEAN:
    fputs("geometrical mean of 4 corners\n", stderr);
    break;

  case PM3D_WHICHCORNER_HARMEAN:
    fputs("harmonic mean of 4 corners\n", stderr);
    break;

  case PM3D_WHICHCORNER_MEDIAN:
    fputs("median of 4 corners\n", stderr);
    break;

  case PM3D_WHICHCORNER_MIN:
    fputs("minimum of 4 corners\n", stderr);
    break;

  case PM3D_WHICHCORNER_MAX:
    fputs("maximum of 4 corners\n", stderr);
    break;

  case PM3D_WHICHCORNER_RMS:
    fputs("root mean square of 4 corners\n", stderr);
    break;

  default:
    fprintf(stderr, "corner %i\n", pm3d.which_corner_color - PM3D_WHICHCORNER_C1 + 1);
  }
}
