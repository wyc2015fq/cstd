...
#endif
        {
          ...
          if (color_from_rgbvar) {
            ...
            case PM3D_WHICHCORNER_GEOMEAN:
              avgC = geomean4(cb1, cb2, cb3, cb4);
              break;

            case PM3D_WHICHCORNER_HARMEAN:
              avgC = harmean4(cb1, cb2, cb3, cb4);
              break;

            case PM3D_WHICHCORNER_MEDIAN:
              avgC = median4(cb1, cb2, cb3, cb4);
              break;

            case PM3D_WHICHCORNER_MIN:
              avgC = minimum4(cb1, cb2, cb3, cb4);
              break;

            case PM3D_WHICHCORNER_MAX:
              avgC = maximum4(cb1, cb2, cb3, cb4);
              break;

            case PM3D_WHICHCORNER_RMS:
              avgC = rms4(cb1, cb2, cb3, cb4);
              break;
      ...
      if (interp_i > 1 || interp_j > 1) {
        ...
        for (i1 = 0; i1 < interp_i; i1++) {
          for (j1 = 0; j1 < interp_j; j1++) {
            ...
            default:
              int_error(NO_CARET, "cannot be here");
              avgC = 0;
            }
...
#define TC_USES_PALETTE(tctype) (tctype==TC_Z) || (tctype==TC_CB) || (tctype==TC_FRAC)
  ...
  if (TC_USES_PALETTE(title.textcolor.type)) {
    return;
  }

  if (TC_USES_PALETTE(axis_array[FIRST_X_AXIS].label.textcolor.type)) {
    return;
  }

  if (TC_USES_PALETTE(axis_array[FIRST_Y_AXIS].label.textcolor.type)) {
    return;
  }

  if (TC_USES_PALETTE(axis_array[SECOND_X_AXIS].label.textcolor.type)) {
    return;
  }

  if (TC_USES_PALETTE(axis_array[SECOND_Y_AXIS].label.textcolor.type)) {
    return;
  }

  if (plot_mode == MODE_SPLOT)
    if (TC_USES_PALETTE(axis_array[FIRST_Z_AXIS].label.textcolor.type)) {
      return;
    }

  if (TC_USES_PALETTE(axis_array[COLOR_AXIS].label.textcolor.type)) {
    return;
  }
