...
static char* RCSid()
{
  return RCSid("$Id: save.c,v 1.256 2014/08/16 00:12:30 sfeam Exp $");
}
#endif
...
#undef SAVE_AXISLABEL_OR_TITLE
  ...
  case PM3D_SCANS_AUTOMATIC:
    fputs("scansautomatic\n", fp);
    break;

  case PM3D_SCANS_FORWARD:
    fputs("scansforward\n", fp);
    break;

  case PM3D_SCANS_BACKWARD:
    fputs("scansbackward\n", fp);
    break;
  ...
  switch (pm3d.flush) {
  case PM3D_FLUSH_CENTER:
    fputs("center", fp);
    break;

  case PM3D_FLUSH_BEGIN:
    fputs("begin", fp);
    break;
  ...
  else if (color_box.border_lt_tag < 0) {
    fputs("bdefault", fp);
  }
  else {
...
save_histogram_opts(FILE* fp)
{
  ...
  default:
  case HT_CLUSTERED:
    fprintf(fp, "clustered gap %d ", histogram_opts.gap);
    break;

  case HT_ERRORBARS:
    fprintf(fp, "errorbars gap %d lw %g", histogram_opts.gap, histogram_opts.bar_lw);
    break;
