...
static char* RCSid()
{
  return RCSid("$Id: breaders.c,v 1.13 2014/05/09 22:14:11 broeker Exp $");
}
#endif
...
edf_filetype_function(void)
{
  ...
  if ((p = edf_findInHeader(header, "DataType"))) {
    ...
    if (k >= 0) { /* known EDF DataType */
      ...
      case 0:
        df_set_read_type(1, SIGNED_TEST(s));
        break;

      case 1:
        df_set_read_type(1, UNSIGNED_TEST(s));
        break;

      case 2:
        df_set_read_type(1, FLOAT_TEST(s));
        break;
      }
    }
  }
...
df_libgd_get_pixel(int i, int j, int component)
{
  ...
  switch (component) {
  case 0:
    pixel = gdImageGetTrueColorPixel(im, i, j);
    return gdTrueColorGetRed(pixel);

  case 1:
    return gdTrueColorGetGreen(pixel);

  case 2:
    return gdTrueColorGetBlue(pixel);
