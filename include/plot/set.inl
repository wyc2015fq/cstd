...
set_autoscale()
{
  ...
  if (set_autoscale_axis(FIRST_X_AXIS)) {
    return;
  }

  if (set_autoscale_axis(FIRST_Y_AXIS)) {
    return;
  }

  if (set_autoscale_axis(FIRST_Z_AXIS)) {
    return;
  }

  if (set_autoscale_axis(SECOND_X_AXIS)) {
    return;
  }

  if (set_autoscale_axis(SECOND_Y_AXIS)) {
    return;
  }

  if (set_autoscale_axis(COLOR_AXIS)) {
    return;
  }

  if (set_autoscale_axis(POLAR_AXIS)) {
    return;
  }

  /* FIXME: Do these commands make any sense? */
  if (set_autoscale_axis(T_AXIS)) {
    return;
  }

  if (set_autoscale_axis(U_AXIS)) {
    return;
  }

  if (set_autoscale_axis(V_AXIS)) {
    return;
  }
  ...
  if (END_OF_COMMAND) {
    ...
    if (cp_str) {
      ...
      */
      ...
      case 20866:
        encoding = S_ENC_KOI8_R;
        break;

      case 21866:
        encoding = S_ENC_KOI8_U;
        break;
      ...
      case 28592:
        encoding = S_ENC_ISO8859_2;
        break;

      case 28599:
        encoding = S_ENC_ISO8859_9;
        break;

      case 28605:
        encoding = S_ENC_ISO8859_15;
        break;
...
set_degreesign(char* locale)
{
  ...
  case S_ENC_SJIS:
    break;  /* should be 0x818B */

  case S_ENC_CP950:
    break;  /* should be 0xA258 */
...
set_palette_defined()
{
  ...
  while (!END_OF_COMMAND) {
    ...
    if (col_str) {
      ...
      if (r < 0 || r > 1) {
        int_error(c_token - 1, "Value out of range [0,1].");
      }
  ...
  if (END_OF_COMMAND) { /* reset to default settings */
    ...
    for (; !END_OF_COMMAND; c_token++) {
      ...
      case S_PALETTE_MAXCOLORS: { /* "maxc$olors" */
        ...
        if (i < 0) {
          int_error(c_token, "non-negative number required");
        }
