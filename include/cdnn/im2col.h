...
int FUN(im2col_nd_core)(const Dtype* data_input, const bool im2col,
  ...
  const DataShape dilation, Dtype* data_output)
{
  ...
  int* d_offset = (int*)malloc(num_spatial_axes * sizeof(int));
  int* d_iter = (int*)malloc(num_spatial_axes * sizeof(int));
...
int FUN(im2col_nd)(const Dtype* data_im, const int num_spatial_axes,
  const int num_kernels, const DataShape im_shape, const DataShape col_shape,
  const DataShape kernel_shape, const DataShape pad, const DataShape stride,
  const DataShape dilation, Dtype* data_col)
{
  const bool kIm2Col = true;
  return FUN(im2col_nd_core)(data_im, kIm2Col, num_spatial_axes, im_shape, col_shape,
    kernel_shape, pad, stride, dilation, data_col);
}

int FUN(col2im)(const Dtype* data_col, const int channels,
  const int height, const int width, const int kernel_h, const int kernel_w,
  const int pad_h, const int pad_w,
  const int stride_h, const int stride_w,
  const int dilation_h, const int dilation_w,
