...
static void sconv1x1s1(int h, int w, const float* src, int src_step, int srccn, float* dst, int dst_step, int dstcn, const float* _kernel, const float* _bias)
{
  int inch = srccn;
  int outh = h;
  int outw = w;
...
#pragma omp parallel for
  for (pp = 0; pp<nn_outch; pp++)
  {
    ...
    float* out0 = dst + (p + 0) * outh*dst_step;
    float* out1 = dst + (p + 1) * outh*dst_step;
    float* out2 = dst + (p + 2) * outh*dst_step;
    float* out3 = dst + (p + 3) * outh*dst_step;
    float* out4 = dst + (p + 4) * outh*dst_step;
    float* out5 = dst + (p + 5) * outh*dst_step;
    float* out6 = dst + (p + 6) * outh*dst_step;
    float* out7 = dst + (p + 7) * outh*dst_step;
    ...
    blas_sset(outh*outw, bias0, out0);
    blas_sset(outh*outw, bias1, out1);
    blas_sset(outh*outw, bias2, out2);
    blas_sset(outh*outw, bias3, out3);
    blas_sset(outh*outw, bias4, out4);
    blas_sset(outh*outw, bias5, out5);
    blas_sset(outh*outw, bias6, out6);
    blas_sset(outh*outw, bias7, out7);
    ...
    for (; q + 7<inch; q += 8)
    {
      ...
      const float* img4 = src + (q + 4)*h*src_step;
      const float* img5 = src + (q + 5)*h*src_step;
      const float* img6 = src + (q + 6)*h*src_step;
      const float* img7 = src + (q + 7)*h*src_step;

      const float* kernel0 = kernel + (p + 0)*inch + q;
...
#pragma omp parallel for
  ...
  {
    float* out = dst + p * outh*dst_step;

    const float bias0 = bias ? bias[p] : 0.f;

    blas_sset(outh*outw, bias0, out);
