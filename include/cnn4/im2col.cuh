...
__global__ void FUN(im2col_nd_kernel)(const int n, const Dtype* data_im,
  ...
  if (threadIdx.x < num_axes) {
    shared_dilation[threadIdx.x] = dilation.dim[threadIdx.x];
    shared_kernel_shape[threadIdx.x] = kernel_shape.dim[threadIdx.x];
    shared_pad[threadIdx.x] = pad.dim[threadIdx.x];
    shared_stride[threadIdx.x] = stride.dim[threadIdx.x];
  }
  if (threadIdx.x < num_axes + 1) {
    shared_col_shape[threadIdx.x] = col_shape.dim[threadIdx.x];
    shared_im_shape[threadIdx.x] = im_shape.dim[threadIdx.x];
  }
  __syncthreads();
