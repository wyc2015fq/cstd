...
namespace caffe
{
  ...
  protected:
    ...
    void TestDropoutForward(const float dropout_ratio) {
      ...
      DropoutLayer layer(layer_param);
  ...
  TYPED_TEST(NeuronLayerTest, TestSigmoid)
  {
    typedef typename TypeParam::Dtype Dtype;
    LayerParameter layer_param;
    SigmoidLayer layer(layer_param);
    layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
    layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
    // Now, check values
    const Dtype* bottom_data = this->blob_bottom_->cpu_data();
  ...
  TYPED_TEST(NeuronLayerTest, TestPReLUConsistencyReLU)
  {
    ...
    SHARED_PTR<Blob > blob_bottom_2(new Blob());
    ...
    // Check backward
    SHARED_PTR<Blob > tmp_blob(new Blob());
  ...
  TYPED_TEST(NeuronLayerTest, TestPReLUInPlace)
  {
    ...
    vector<Blob*> blob_top_vec_2;
    SHARED_PTR<Blob > blob_bottom_2(new Blob());
    SHARED_PTR<Blob > blob_middle_2(new Blob());
    SHARED_PTR<Blob > blob_top_2(new Blob());
    blob_bottom_vec_2.push_back(blob_bottom_2.get());
