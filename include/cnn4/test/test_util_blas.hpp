...
namespace caffe
{
  ...
  TYPED_TEST(GemmTest, TestGemmCPUGPU)
  {
    ...
    if (sizeof(TypeParam) == 4 || CAFFE_TEST_CUDA_PROP.major >= 2) {
      ...
      A.Reshape(1, 1, 3, 2);
      caffe_copy(6, A_reshape_data, A.cpu_mdata());
      ...
      B.Reshape(1, 1, 4, 3);
      caffe_copy(12, B_reshape_data, B.cpu_mdata());
  ...
  TYPED_TEST(GemmTest, TestGemvCPUGPU)
  {
    ...
    if (sizeof(TypeParam) == 4 || CAFFE_TEST_CUDA_PROP.major >= 2) {
      ...
      // Test transpose case
      caffe_copy(2, data, y.cpu_mdata());
      caffe_cpu_gemv<TypeParam>(CblasTrans, 2, 3, 1., A.cpu_data(),
                                y.cpu_data(), 0., x.cpu_mdata());
      for (int i = 0; i < 3; ++i) {
