...
namespace caffe
{
  ...
  protected:
    ...
    virtual void InitNetFromProtoFileWithState(const string & proto,
      ...
      net_.reset(new Net(param_file, phase, level, stages));
    }

    virtual void CopyNetBlobs(const bool copy_diff,
                              vector<SHARED_PTR<Blob > >* blobs_copy) {
      CHECK(net_);
    ...
    virtual void CopyNetParams(const bool copy_diff,
                               vector<SHARED_PTR<Blob > >* params_copy) {
      CHECK(net_);
  ...
  TYPED_TEST(NetTest, TestLossWeight)
  {
    ...
    vector<SHARED_PTR<Blob > > blob_grads;
    this->CopyNetBlobs(kCopyDiff, &blob_grads);
    vector<SHARED_PTR<Blob > > param_grads;
  ...
  TYPED_TEST(NetTest, TestComboLossWeight)
  {
    ...
    vector<SHARED_PTR<Blob > > blob_grads_loss_2;
    this->CopyNetBlobs(kCopyDiff, &blob_grads_loss_2);
    vector<SHARED_PTR<Blob > > param_grads_loss_2;
  ...
  TYPED_TEST(NetTest, TestUnsharedWeightsDiffNet)
  {
    ...
    net->Forward();
    net->Backward();
    Layer* ip1_layer = net->layer_by_name("innerproduct1").get();
    Layer* ip2_layer = net->layer_by_name("innerproduct2").get();
  ...
  TYPED_TEST(NetTest, TestParamPropagateDown)
  {
    ...
    vector param_asums(params.size());
  ...
  TYPED_TEST(NetTest, TestReshape)
  {
    ...
    Blob blob1(2, 3, 12, 10);
    Blob blob2(4, 3, 9, 11);
    ASSERT_LT(blob1.count(), blob2.count());
    filler.Fill(&blob1);
    filler.Fill(&blob2);
    this->InitReshapableNet();
    SHARED_PTR<Blob > input_blob = this->net_->blob_by_name("data");
