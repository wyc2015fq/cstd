...
namespace caffe
{
  ...
  protected:
    ...
    Blob* const data_blob_;
    Blob* const label_blob_;
        ...
        for (int h = 0; h < this->height_; ++h) {
          ...
          for (int w = 0; w < this->width_; ++w) {
            for (int c = 0; c < this->channels_; ++c) {
              ...
              Dtype pixel = static_cast(ptr_mat[index++]);
