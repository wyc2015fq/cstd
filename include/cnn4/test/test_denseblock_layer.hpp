...
protected:
  ...
  vector<Blob*> blob_top_vec_2_;
};

TYPED_TEST_CASE(DenseBlockLayerTest, TestDtypesAndDevices);

template <typename Dtype>
void convolution_Fwd_fast(Blob* input, Blob* output, Blob* filter, int N, int c_output, int c_input, int h_img, int w_img, int h_filter, int w_filter)
{
  //std::vector data_col;
