...
namespace caffe
{
  ...
  protected:
    ...
    void CheckBlobEqual(const Blob & b1, const Blob & b2);

    string output_file_name_;
    string input_file_name_;
    Blob* const blob_data_;
    Blob* const blob_label_;
  ...
  template<typename TypeParam>
  void HDF5OutputLayerTest<TypeParam>::CheckBlobEqual(const Blob & b1,
      const Blob & b2)
  {
  ...
  TYPED_TEST(HDF5OutputLayerTest, TestForward)
  {
    ...
    ASSERT_GE(
      file_id, 0) << "Failed to open HDF5 file" <<
                  this->input_file_name_;
    Blob* blob_data = new Blob();
    hdf5_load_nd_dataset(file_id, HDF5_DATA_DATASET_NAME, 0, 4,
                         blob_data);
    this->CheckBlobEqual(*(this->blob_data_), *blob_data);
    Blob* blob_label = new Blob();
    hdf5_load_nd_dataset(file_id, HDF5_DATA_LABEL_NAME, 0, 4,
                         blob_label);
