...
// #endif
    ...
    for (int transitionIdx = 0; transitionIdx < this->numTransition; ++transitionIdx) {
      ...
      for (int blobIdx = 0; blobIdx < inChannels; ++blobIdx) {
        ...
        localB->mutable_data<Context>()[localB->offset(0, blobIdx, 0, 0)] = 0;
      }
      //globalMean BC
      if (useBC) {
      ...
      for (int blobIdx = 0; blobIdx < inChannels; ++blobIdx) {
        ...
        localB->mutable_data<Context>()[localB->offset(0, blobIdx, 0, 0)] = 1;
      }
      //globalVar BC
      if (useBC) {
    ...
    this->blobs_[singletonIdx]->mutable_data<Context>()[0] = Dtype(0);
