...
#include <stdlib.h>
	...
	void DenseBlockLayer::logInternal_gpu(string dir, int TIdx, bool logDynamic, bool logDiff) {
		...
		if (logDynamic) {
			...
			for (int transitionIdx = 0; transitionIdx < this->numTransition; ++transitionIdx) {
				...
				log_gpuPtr(this->blobs_[3 * this->numTransition + transitionIdx]->gpu_mdata(), numChannel_moreWide, localDir + "globalMean_gpu_transition" + itos_cu(transitionIdx));
				log_gpuPtr(this->blobs_[4 * this->numTransition + transitionIdx]->gpu_mdata(), numChannel_moreWide, localDir + "globalVariance_gpu_transition" + itos_cu(transitionIdx));
				...
				if (useBC) {
					...
					log_gpuPtr(this->blobs_[8 * numTransition + transitionIdx]->gpu_mdata(), numChannel_quadG, localDir + "globalMean_BC_transition" + itos_cu(transitionIdx));
					log_gpuPtr(this->blobs_[9 * numTransition + transitionIdx]->gpu_mdata(), numChannel_quadG, localDir + "globalVar_BC_transition" + itos_cu(transitionIdx));
				...
				log_gpuPtr(this->blobs_[transitionIdx]->gpu_mdata(), filterSize, localDir + "Filter_data_gpu_" + itos_cu(transitionIdx));
				log_gpuPtr(this->blobs_[transitionIdx]->gpu_mdiff(), filterSize, localDir + "Filter_grad_gpu_" + itos_cu(transitionIdx));
				//Scaler_data/grad_gpu
				log_gpuPtr(this->blobs_[transitionIdx + this->numTransition]->gpu_mdiff(), numChannel_moreWide, localDir + "Scaler_grad_gpu_" + itos_cu(transitionIdx));
				log_gpuPtr(this->blobs_[transitionIdx + this->numTransition]->gpu_mdata(), numChannel_moreWide, localDir + "Scaler_data_gpu_" + itos_cu(transitionIdx));
				//Bias_data/grad_gpu
				log_gpuPtr(this->blobs_[transitionIdx + 2 * this->numTransition]->gpu_mdiff(), numChannel_moreWide, localDir + "Bias_grad_gpu_" + itos_cu(transitionIdx));
				log_gpuPtr(this->blobs_[transitionIdx + 2 * this->numTransition]->gpu_mdata(), numChannel_moreWide, localDir + "Bias_data_gpu_" + itos_cu(transitionIdx));
				if (useBC) {
					//BC Filter
					int filterBC_size = (initChannel + growthRate*transitionIdx) * 4 * growthRate * 1 * 1;
					log_gpuPtr(this->blobs_[5 * numTransition + transitionIdx]->gpu_mdata(), filterBC_size, localDir + "Filter_data_BC_" + itos_cu(transitionIdx));
					log_gpuPtr(this->blobs_[5 * numTransition + transitionIdx]->gpu_mdiff(), filterBC_size, localDir + "Filter_grad_BC_" + itos_cu(transitionIdx));
					//BC scaler
					log_gpuPtr(this->blobs_[6 * numTransition + transitionIdx]->gpu_mdiff(), numChannel_quadG, localDir + "Scaler_grad_BC_" + itos_cu(transitionIdx));
					log_gpuPtr(this->blobs_[6 * numTransition + transitionIdx]->gpu_mdata(), numChannel_quadG, localDir + "Scaler_data_BC_" + itos_cu(transitionIdx));
					//BC bias
					log_gpuPtr(this->blobs_[7 * numTransition + transitionIdx]->gpu_mdiff(), numChannel_quadG, localDir + "Bias_grad_BC_" + itos_cu(transitionIdx));
					log_gpuPtr(this->blobs_[7 * numTransition + transitionIdx]->gpu_mdata(), numChannel_quadG, localDir + "Bias_data_BC_" + itos_cu(transitionIdx));
				}
			}
		}
	}
    ...
    cudnnDataType_t type = dataType<Dtype>::type;
		...
		if (useBC) {
			...
			CUDNN_CHECK(cudnnSetConvolution2dDescriptor(*convBC_Descriptor, 0, 0, 1, 1, 1, 1, CUDNN_CONVOLUTION, type));
		}
		...
		CUDNN_CHECK(cudnnSetConvolution2dDescriptor(*this->conv_Descriptor, 1, 1, 1, 1, 1, 1, CUDNN_CONVOLUTION, type));
	...
	void DenseBlockLayer::Forward_gpu(const vector<Blob*>& bottom,
		...
		Dtype* top_data = top[0]->gpu_mdata();
		...
		//deploy top data
		if ((this->phase_cur_ == TRAIN) && useDropout) {
			cudaMemcpy(top[0]->gpu_mdata(), postDropout_data_gpu, work_n * sizeof(Dtype), cudaMemcpyDeviceToDevice);
		}
		else {
			cudaMemcpy(top[0]->gpu_mdata(), postConv_data_gpu, work_n * sizeof(Dtype), cudaMemcpyDeviceToDevice);
		}
	...
	void DenseBlockLayer::Backward_gpu(const vector<Blob*>& top, const vector<Blob*>& bottom) {
		...
		//deploy top diff
		if (useDropout) {
			cudaMemcpy(postDropout_grad_gpu, top[0]->gpu_mdiff(), work_n * sizeof(Dtype), cudaMemcpyDeviceToDevice);
		}
		else {
