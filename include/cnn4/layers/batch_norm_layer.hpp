...
public:
  ...
  void Reshape(const vector<Blob*> & bottom, const vector<Blob*> & top)
  {
    ...
    if (spatial_sum_multiplier_.num_axes() == 0 ||
      ...
      Dtype* multiplier_data = spatial_sum_multiplier_.mdata();
    ...
    const vector<Blob*> & top)
  {
    ...
    if (use_global_stats_) {
      ...
      caffe_scale(variance_.count(), scale_factor,
        this->blobs_[0]->data(), mean_.mdata());
      caffe_scale(variance_.count(), scale_factor,
        this->blobs_[1]->data(), variance_.mdata());
    }
    else {
      // compute mean
      caffe_gemv(CblasNoTrans, channels_ * num, spatial_dim,
    ...
    if (!use_global_stats_) {
      ...
      caffe_gemv(CblasTrans, num, channels_, 1.,
        num_by_chans_.data(), batch_sum_multiplier_.data(), 0.,
        variance_.mdata());  // E((X_EX)^2)
                                    // compute and save moving average
      this->blobs_[2]->mdata()[0] *= moving_average_fraction_;
      this->blobs_[2]->mdata()[0] += 1;
      caffe_axpby(mean_.count(), Dtype(1), mean_.data(),
        moving_average_fraction_, this->blobs_[0]->mdata());
    ...
    caffe_gemm(CblasNoTrans, CblasNoTrans, channels_ * num,
      spatial_dim, 1, 1., num_by_chans_.data(),
      spatial_sum_multiplier_.data(), 0., temp_.mdata());
