...
namespace
{
    ...
    const vector<Blob<Dtype>*> & bottom, const vector<Blob<Dtype>*> & top)
  {
    ...
    case EltwiseParameter_EltwiseOp_PROD:
      caffe_mul(count, bottom[0]->data<Context>(), bottom[1]->data<Context>(), top_data);
      for (int i = 2; i < bottom.size(); ++i) {
        caffe_mul(count, top_data, bottom[i]->data<Context>(), top_data);
      }
      break;
    case EltwiseParameter_EltwiseOp_SUM:
      ...
      for (int i = 0; i < bottom.size(); ++i) {
        caffe_axpy(count, coeffs_[i], bottom[i]->data<Context>(), top_data);
      }
      break;
    case EltwiseParameter_EltwiseOp_MAX:
      ...
      // bottom 0 & 1
      bottom_data_a = bottom[0]->data<Context>();
      bottom_data_b = bottom[1]->data<Context>();
      ...
      for (int blob_idx = 2; blob_idx < bottom.size(); ++blob_idx) {
        bottom_data_b = bottom[blob_idx]->data<Context>();
        for (int idx = 0; idx < count; ++idx) {
          ...
          if (stable_prod_grad_) {
            ...
            for (int j = 0; j < bottom.size(); ++j) {
              ...
              if (!initialized) {
                ...
                caffe_mul(count, bottom[j]->data<Context>(), bottom_diff,
                          bottom_diff);
              }
            }
          } else {
