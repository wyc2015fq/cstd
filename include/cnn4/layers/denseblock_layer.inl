...
#include "wstd/string.hpp"
  ...
  template <typename Dtype>
  void DenseBlockLayer::Forward(CPUContext* context, const vector<Blob*> & bottom,
      const vector<Blob*> & top)
  {
    ...
    for (int transitionIdx = 0; transitionIdx < this->numTransition; ++transitionIdx) {
      ...
      if (useBC) {
        //BC Conv 1*1
        Blob* BC_filterBlob = this->blobs_[5 * numTransition + transitionIdx].get();
  ...
  template <typename Dtype>
  void DenseBlockLayer::Backward(CPUContext* context, const vector<Blob*> & top,
      int*
      const vector<Blob*> & bottom)
  {
    ...
    for (int transitionIdx = this->numTransition - 1; transitionIdx >= 0; --transitionIdx) {
      ...
      Blob* filter = this->blobs_[transitionIdx].get();
      ...
      if (useBC) {
        ...
        Blob* BC_conv_x = postReLU_blobVec[transitionIdx];
        Blob* BC_conv_y = postConv_BCVec[transitionIdx];
        Blob* BC_filter = this->blobs_[5 * numTransition + transitionIdx].get();
      ...
      //BN Bwd
      Blob* BN_bottom = this->merged_conv[transitionIdx];
      Blob* scaler = this->blobs_[this->numTransition + transitionIdx].get();
      Blob* bias = this->blobs_[2 * this->numTransition + transitionIdx].get();
  ...
  {
    this->runForward(_CONTEXT,bottom, top);
  }
