...
namespace
{
  ...
  {
    if (pyramid_height_ == 1) {
      pooling_layers_[0]->runForward(bottom, top);
      return;
    }
    split_layer_->runForward(bottom, split_top_vec_);
    for (int i = 0; i < pyramid_height_; i++) {
      pooling_layers_[i]->runForward(
        *pooling_bottom_vecs_[i], *pooling_top_vecs_[i]);
      flatten_layers_[i]->runForward(
        *pooling_top_vecs_[i], *flatten_top_vecs_[i]);
    }
    concat_layer_->runForward(concat_bottom_vec_, top);
  }
                                     ...
                                     const vector<Blob*> & bottom)
  {
    ...
    if (pyramid_height_ == 1) {
      pooling_layers_[0]->runBackward(top, propagate_down, bottom);
      return;
    }
    vector<bool> concat_propagate_down(pyramid_height_, true);
    concat_layer_->runBackward(top, concat_propagate_down, concat_bottom_vec_);
    for (int i = 0; i < pyramid_height_; i++) {
      flatten_layers_[i]->runBackward(
        *flatten_top_vecs_[i], propagate_down, *pooling_top_vecs_[i]);
      pooling_layers_[i]->runBackward(
