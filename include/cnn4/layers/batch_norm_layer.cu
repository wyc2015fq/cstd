...
void BatchNormLayer::Forward(GPUContext* context, const vector<Blob*>& bottom,
  ...
  if (use_global_stats_) {
    ...
    caffe_gpu_gemv<Dtype>(CblasNoTrans, channels_ * num, spatial_dim,
      Dtype(1. / (num * spatial_dim)), bottom_data,
        spatial_sum_multiplier_.data(), Dtype(0.),
  ...
  if (!use_global_stats_) {
    ...
    caffe_gpu_gemv<Dtype>(CblasNoTrans, channels_ * num, spatial_dim,
      Dtype(1. / (num * spatial_dim)), temp_.data(),
        spatial_sum_multiplier_.data(), Dtype(0.),
        num_by_chans_.mutable_data());
    caffe_gpu_gemv<Dtype>(CblasTrans, num, channels_, 1.,
        num_by_chans_.data(), batch_sum_multiplier_.data(), 0.,
    ...
    caffe_gpu_axpby(mean_.count(), Dtype(1), mean_.data(),
...
template <typename Dtype>
void BatchNormLayer::Backward(GPUContext* context, const vector<Blob*>& top,
  ...
  if (use_global_stats_) {
    caffe_gpu_div(temp_.count(), top_diff, temp_.data(), bottom_diff);
    return;
  }
