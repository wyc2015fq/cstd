...
namespace
{
  ...
  template <typename Dtype>
  void SoftmaxLayer<Dtype>::Forward(CPUContext* context, const vector<Blob<Dtype>*> & bottom,
                                        const vector<Blob<Dtype>*> & top)
  {
    ...
    for (int i = 0; i < outer_num_; ++i) {
      ...
      caffe_gemm<Dtype>(CblasNoTrans, CblasNoTrans, channels, inner_num_,
                            1, -1., sum_multiplier_.data<Context>(), scale_data, 1., top_data);
      // exponentiation
      caffe_exp<Dtype>(dim, top_data, top_data);
      // sum after exp
      caffe_gemv<Dtype>(CblasTrans, channels, inner_num_, 1.,
                            top_data, sum_multiplier_.data<Context>(), 0., scale_data);
      // division
      for (int j = 0; j < channels; j++) {
                                         ...
                                         const vector<Blob<Dtype>*> & bottom)
  {
    ...
    for (int i = 0; i < outer_num_; ++i) {
      ...
      caffe_gemm<Dtype>(CblasNoTrans, CblasNoTrans, channels, inner_num_, 1,
                            -1., sum_multiplier_.data<Context>(), scale_data, 1., bottom_diff + i * dim);
    }
