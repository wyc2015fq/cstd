...
private:
    void init_connection(int pooling_size) {
      int x, y, c;
        for (c = 0; c < in_.depth_; c++) 
            for (y = 0; y < in_.height_; y += pooling_size)
                for (x = 0; x < in_.width_; x += pooling_size)
                    connect_kernel(pooling_size, x, y, c);


        for (c = 0; c < in_.depth_; c++) 
            for (y = 0; y < out_.height_; y++)
                for (x = 0; x < out_.width_; x++)
                    this->connect_bias(c, out_.get_index(x, y, c));
    }

    void connect_kernel(int pooling_size, int x, int y, int inc) {
      int dx, dy;
        for (dy = 0; dy < pooling_size; dy++)
            for (dx = 0; dx < pooling_size; dx++)
                this->connect_weight(
