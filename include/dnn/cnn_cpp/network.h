...
struct result {
    ...
    void print_detail(std::basic_ostream<Char, CharTraits>& os) {
        ...
        os << std::setw(5) << "*" << " ";
        std::set<label_t>::const_iterator c;
        for (c = all_labels.begin(); c!=all_labels.end(); ++c) 
            os << std::setw(5) << *c << " ";
        os << std::endl;

        std::set<label_t>::const_iterator r;
        for (r = all_labels.begin(); r!=all_labels.end(); ++r) {
            os << std::setw(5) << *r << " ";
            for (c = all_labels.begin(); c!=all_labels.end(); ++c) 
    ...
    std::set<label_t> labels() const {
        ...
        std::map<label_t, std::map<label_t, int> >::const_iterator r;
        for (r = confusion_matrix.begin(); r!=confusion_matrix.end(); ++r) {
            all_labels.insert(r->first);
            std::map<label_t, int>::const_iterator c; 
...
public:
    ...
    void add(layer *layer) { layers_.add(layer); }
    ...
    void train(const std::vector<vec_t>& in, const std::vector<T>& t, size_t batch_size, int epoch, OnBatchEnumerate on_batch_enumerate, OnEpochEnumerate on_epoch_enumerate) {
        ...
        for (iter = 0; iter < epoch; iter++) {
            if (optimizer_.requires_hessian())
                calc_hessian(in);
            for (i = 0; i < in.size(); i+=batch_size) {
                train_once(&in[i], &t[i], MIN(batch_size, in.size() - i));
                on_batch_enumerate(*this);
            }
            on_epoch_enumerate(*this);
        }
    }
...
private:
    ...
    void label2vector(const label_t* t, int num, std::vector<vec_t> *vec) const {
        ...
        for (int i = 0; i < num; i++) {
            assert(t[i] < outdim);
            vec_t v(outdim, target_value_min());
            v[t[i]] = target_value_max();
            vec->push_back(v);
        }
    }
    ...
    void calc_hessian(const std::vector<vec_t>& in, int size_initialize_hessian = 500) {
        int size = MIN((int)in.size(), size_initialize_hessian);
