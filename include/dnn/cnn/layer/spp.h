...
int SPP::forward(const Blob* prev, Blob* next) const
{
  // 1 + 4 + 16 + 64 + ... + (2*pyramid_height)^2
  int pyramid_num_bins = ((1 << (pyramid_height * 2)) - 1) / 3;
  next->create(pyramid_num_bins, 1, 2);

  if (next->empty()) {
    return -100;
  }

  float* pyramid_ptr = next;

  // all spatial pyramids
  for (p = 0; p < pyramid_height; p++) {
    int w = prevSize.w;
    int h = prevSize.h;
    ...
    if (pad_h > 0 || pad_w > 0) {
      copy_make_border(prev, bottom_blob_bordered, pad_h, pad_h, pad_w, pad_w, BORDER_CONSTANT, 0.f);
    ...
    if (pooling_type == PoolMethod_MAX) {
      ...
      for (q = 0; q < channels; q++) {
        const Blob m(w, h, bottom_blob_bordered.channel(q));
