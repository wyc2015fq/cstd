...
int LRN::forward(const Blob* prev, Blob* next) const
{
  int w = prevSize.w;
  int h = prevSize.h;
  ...
  if (region_type == NormRegion_ACROSS_CHANNELS) {
    ...
    for (q = 0; q < channels; q++) {
      ...
      for (p = q - local_size / 2; p <= q + local_size / 2; p++) {
        if (p < 0 || p >= channels) {
          continue;
        }
    ...
    Blob square_blob_bordered = square_blob;
    ...
    for (q = 0; q < channels; q++) {
      const float* ptr = prevData + q * Data_2DSize(prevSize);
      const Blob m = square_blob_bordered.channel(q);
      float* outptr = nextData + q * Data_2DSize(nextSize);
...
int LRN::forward_inplace(Blob* bottom_top_blob) const
{
  ...
  for (q = 0; q < channels; q++) {
    const float* ptr = Blob_channel_p(bottom_top_blob, q);
    float* outptr = square_blob.channel(q);
