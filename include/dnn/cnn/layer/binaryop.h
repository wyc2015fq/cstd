...
int BinaryOp::forward(const std::vector<Blob>& bottom_blobs, std::vector<Blob>& top_blobs) const
{
  ...
  if (op_type == Operation_ADD) {
    return binary_op< std::plus<float> >(prev, bottom_blob1, next);
  }

  if (op_type == Operation_SUB) {
    return binary_op< MINus<float> >(prev, bottom_blob1, next);
  }

  if (op_type == Operation_MUL) {
    return binary_op< std::multiplies<float> >(prev, bottom_blob1, next);
  }

  if (op_type == Operation_DIV) {
    return binary_op< std::divides<float> >(prev, bottom_blob1, next);
  }

  if (op_type == Operation_MAX) {
    return binary_op< binary_op_max<float> >(prev, bottom_blob1, next);
  }

  if (op_type == Operation_MIN) {
    return binary_op< binary_op_min<float> >(prev, bottom_blob1, next);
  }

  if (op_type == Operation_POW) {
    return binary_op< binary_op_pow<float> >(prev, bottom_blob1, next);
  }
