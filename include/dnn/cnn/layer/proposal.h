...
static Blob generate_anchors(int base_size, const Blob* ratios, const Blob* scales)
{
...
int Proposal::forward(const std::vector<Blob>& bottom_blobs, std::vector<Blob>& top_blobs) const
{
  const Blob* score_blob = bottom_blobs[0];
  const Blob* bbox_blob = bottom_blobs[1];
  const Blob* im_info_blob = bottom_blobs[2];
  ...
  if (top_blobs.size() > 1) {
    Blob* roi_score_blob = top_blobs[1];
