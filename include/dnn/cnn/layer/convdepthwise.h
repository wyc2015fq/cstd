...
int ConvolutionDepthWise::forward(const Blob* prev, Blob* next) const
{
  if (group == 1) {
    return Convolution::forward(prev, next);
  }
  ...
  if (channels == group && group == num_output) {
    ...
    for (g = 0; g < group; g++) {
      float* outptr = Blob_channel_p(next, g);
      const float* kptr = weight_data + maxk * g;
      const Blob m = bottom_blob_bordered.channel(g);
  ...
  for (g = 0; g < group; g++) {
    for (p = 0; p < num_output_g; p++) {
      float* outptr = Blob_channel_p(next, g * num_output_g + p);
      ...
      for (i = 0; i < outh; i++) {
        for (j = 0; j < outw; j++) {
          ...
          // channels_g
          for (q = 0; q < channels_g; q++) {
            const Blob m = bottom_blob_bordered.channel(channels_g * g + q);
            const float* sptr = m.data + m.w * i * stride + j * stride;
