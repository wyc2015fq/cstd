...
void WindowDataLayer::DataLayerSetUp(const vector<Blob*>& bottom,
  ...
  if (has_mean_values_) {
    ...
    for (c = 0; c < this->transform_param_.mean_value_size(); ++c) {
...
void WindowDataLayer::load_batch(Batch* batch) {
  ...
  // zero out batch
  blas_sset(batch->data_.count(), (Dtype)(0), nextData);

  const int num_fg = (int)(static_cast<float>(batch_size)
      * fg_fraction);
  ...
  for (is_fg = 0; is_fg < 2; ++is_fg) {
    for (dummy = 0; dummy < num_samples[is_fg]; ++dummy) {
      // sample a window
      timer.Start();
      ...
      if (context_pad > 0 || use_square) {
        ...
        Dtype context_scale = static_cast(crop_size) /
            static_cast(crop_size - 2*context_pad);

        // compute the expanded region
        Dtype half_height = static_cast(y2-y1+1)/2.0;
        Dtype half_width = static_cast(x2-x1+1)/2.0;
        Dtype center_x = static_cast(x1) + half_width;
        Dtype center_y = static_cast(y1) + half_height;
        if (use_square) {
        ...
        int pad_x1 = MAX(0, -x1);
        int pad_y1 = MAX(0, -y1);
        int pad_x2 = MAX(0, x2 - cv_img.cols + 1);
        int pad_y2 = MAX(0, y2 - cv_img.rows + 1);
        // clip bounds
        x1 = x1 + pad_x1;
        x2 = x2 - pad_x2;
        y1 = y1 + pad_y1;
        ...
        // expanded region
        Dtype scale_x =
            static_cast(crop_size)/static_cast(unclipped_width);
        Dtype scale_y =
            static_cast(crop_size)/static_cast(unclipped_height);

        // size to warp the clipped expanded region to
        cv_crop_size.width =
            (int)(round(static_cast(clipped_width)*scale_x));
        cv_crop_size.height =
            (int)(round(static_cast(clipped_height)*scale_y));
        pad_x1 = (int)(round(static_cast(pad_x1)*scale_x));
        pad_x2 = (int)(round(static_cast(pad_x2)*scale_x));
        pad_y1 = (int)(round(static_cast(pad_y1)*scale_y));
        pad_y2 = (int)(round(static_cast(pad_y2)*scale_y));
      ...
      // copy the warped window into nextData
      for (h = 0; h < cv_cropped_img.rows; ++h) {
        const uchar* ptr = cv_cropped_img.ptr<uchar>(h);
        int img_index = 0;
        for (w = 0; w < cv_cropped_img.cols; ++w) {
          for (c = 0; c < channels; ++c) {
            ...
            if (this->has_mean_file_) {
              ...
              nextData[top_index] = (pixel - mean[mean_index]) * scale;
            } else {
              if (this->has_mean_values_) {
                nextData[top_index] = (pixel - this->mean_values_[c]) * scale;
              } else {
                nextData[top_index] = pixel * scale;
              }
            }
          }
        }
      ...
      for (c = 0; c < channels; ++c) {
        for (h = 0; h < crop_size; ++h) {
          for (w = 0; w < crop_size; ++w) {
            top_data_file.write(reinterpret_cast<char*>(
                &nextData[((item_id * channels + c) * crop_size + h)
                          * crop_size + w]),
                sizeof(Dtype));
          }
        }
