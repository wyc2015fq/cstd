...
int DetectionOutput::forward(const std::vector<Blob>& bottom_blobs, std::vector<Blob>& top_blobs) const
{
  const Blob* location = bottom_blobs[0];
  const Blob* confidence = bottom_blobs[1];
  const Blob* priorbox = bottom_blobs[2];
  ...
  // start from 1 to ignore background struct
  #pragma omp parallel for
