...
void AccuracyLayer::Reshape(
  const vector<Blob*>& bottom, const vector<Blob*>& top) {
  CHECK_LE(top_k_, Data_count(prev->size) / bottom[1]->count())
...
void AccuracyLayer::Forward(const vector<Blob*>& bottom,
    const vector<Blob*>& top) {
  Dtype accuracy = 0;
  ...
  if (layer->o_size > 1) {
    blas_sset(nums_buffer_.count(), (Dtype)(0), nums_buffer_);
    blas_sset(top[1]->count(), (Dtype)(0), top[1]->mutable_data());
  }
  int count = 0;
  for (i = 0; i < outer_num_; ++i) {
    for (j = 0; j < inner_num_; ++j) {
      ...
      if (layer->i_size == 3 && getData(prev2)[i * inner_num_ + j] == 0) {
        continue;
      }
      if (has_ignore_label_ && label_value == ignore_label_) {
        continue;
      }
      if (layer->o_size > 1) ++nums_buffer_[label_value];
      ...
      for (k = 0; k < num_labels; ++k) {
        bottom_data_vector.push_back(std::make_pair(
            prevData[i * dim + k * inner_num_ + j], k));
      }
      if (min_is_better_) {
        std::partial_sort(
      ...
      for (k = 0; k < top_k_; k++) {
        if (bottom_data_vector[k].second == label_value) {
          ++accuracy;
          if (layer->o_size > 1) ++top[1]->mutable_data()[label_value];
          break;
        }
      }
      ++count;
  ...
  if (layer->o_size > 1) {
    for (i = 0; i < top[1]->count(); ++i) {
      top[1]->mutable_data()[i] =
