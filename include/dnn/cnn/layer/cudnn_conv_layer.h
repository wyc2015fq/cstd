...
void CuDNNConvolutionLayer::LayerSetUp(
  ...
  fwd_algo_       = new cudnnConvolutionFwdAlgo_t[layer->i_size];
  bwd_filter_algo_= new cudnnConvolutionBwdFilterAlgo_t[layer->i_size];
  bwd_data_algo_  = new cudnnConvolutionBwdDataAlgo_t[layer->i_size];

  // initialize size arrays
  workspace_fwd_sizes_ = new size_t[layer->i_size];
  workspace_bwd_filter_sizes_ = new size_t[layer->i_size];
  workspace_bwd_data_sizes_ = new size_t[layer->i_size];
  ...
  for (size_t i = 0; i < layer->i_size; ++i) {
  ...
  for (g = 0; g < this->group_ * CUDNN_STREAMS_PER_GROUP; g++) {
...
void CuDNNConvolutionLayer::Reshape(
  ...
  for (size_t i = 0; i < layer->i_size; i++) {
    total_workspace_fwd        = MAX(total_workspace_fwd,
                                     workspace_fwd_sizes_[i]);
    total_workspace_bwd_data   = MAX(total_workspace_bwd_data,
                                     workspace_bwd_data_sizes_[i]);
    total_workspace_bwd_filter = MAX(total_workspace_bwd_filter,
                                     workspace_bwd_filter_sizes_[i]);
  }
  // get max over all operations
  size_t max_workspace = MAX(total_workspace_fwd,
...
CuDNNConvolutionLayer::~CuDNNConvolutionLayer() {
  ...
  for (i = 0; i < bottom_descs_.size(); i++) {
