  ...
  */
  ...
  MultiLabelImageDataLayer::~MultiLabelImageDataLayer() {
  ...
  void MultiLabelImageDataLayer::DataLayerSetUp(const vector<Blob*>& bottom,
                                                       const vector<Blob*>& top) {
    ...
    while (!infile.eof()) {
      ...
      Dtype label;
      shared_ptr<vector > labels_ptr(new vector);
      while (!stream.eof()) {
        stream >> label;
    ...
    if (balance_) {
      ...
      filename_by_class_ = vector<vector<std::pair<std::string, shared_ptr<vector > > > >(max_label + 1);
      for (auto& l : lines_) {
  ...
  void MultiLabelImageDataLayer::ShuffleImages() {
  ...
  void MultiLabelImageDataLayer::load_batch(Batch* batch) {
      ...
      while (!valid_sample) {
        ...
        if (!cv_img.data) {
          ...
          if (item_id >= label_cut_start_ && item_id < batch_size - label_cut_end_) {
            for (label_id = 0; label_id < label_count; ++label_id) {
          ...
          valid_sample = true;
          if (image_data_param.face_transform()) {
            for (point_id = 0; point_id < image_data_param.face_point_num(); ++point_id) {
