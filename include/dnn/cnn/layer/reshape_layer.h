...
void ReshapeLayer::Reshape(const vector<Blob*>& bottom,
  ...
  for (i = 0; i < num_new_axes; ++i) {
    top_shape[top_shape_index++] = top_blob_shape.dim(i);
  }
  for (i = end_axis; i < bottom[0]->num_axes(); ++i) {
    top_shape[top_shape_index++] = bottom[0]->shape(i);
  }
  CHECK_EQ(top_shape_index, top_shape.size());
  for (i = 0; i < copy_axes_.size(); ++i) {
  ...
  if (inferred_axis_ >= 0) {
    ...
    CHECK_EQ(0, Data_count(prev->size) % explicit_count) << "bottom count ("
        << Data_count(prev->size) << ") must be divisible by the product of "
        << "the specified dimensions (" << explicit_count << ")";
    const int inferred_dim = Data_count(prev->size) / explicit_count;
