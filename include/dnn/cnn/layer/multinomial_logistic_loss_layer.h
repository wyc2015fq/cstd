...
void MultinomialLogisticLossLayer::Forward(
    const vector<Blob*>& bottom, const vector<Blob*>& top) {
  ...
  for (i = 0; i < num; ++i) {
    ...
    Dtype prob = MAX(
        prevData[i * dim + label], (Dtype)(kLOG_THRESHOLD));
    loss -= log(prob);
  }
  nextData[0] = loss / num;
}
...
void MultinomialLogisticLossLayer::Backward(
  ...
  if (propagate_down[0]) {
    ...
    for (i = 0; i < num; ++i) {
      ...
      Dtype prob = MAX(
          prevData[i * dim + label], (Dtype)(kLOG_THRESHOLD));
      prevDiff[i * dim + label] = scale / prob;
    }
  }
}
