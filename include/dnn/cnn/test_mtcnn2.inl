...
struct mtcnn {
    struct NetWork Pnet[1], Rnet[1], Onet[1];
    img_t img[1];

    Bbox firstBbox_[100];
    Bbox secondBbox_[100];
    Bbox thirdBbox_[100];
    orderScore firstOrderScore_[100];
    orderScore secondBboxScore_[100];
    orderScore thirdBboxScore_[100];
    int img_w, img_h;
};

void mtcnn_init(struct mtcnn* s) {
  net_load(s->Pnet, "det1.param", "det1.bin");
  net_load(s->Rnet, "det2.param", "det2.bin");
  net_load(s->Onet, "det3.param", "det3.bin");
}

#if 0
void generateBbox(struct mtcnn* s, const img_t* score, ncnn::Mat location, std::vector<Bbox>& boundingBox_, std::vector<orderScore>& bboxScore_, float scale){
    int stride = 2;
    int cellsize = 12;
    int count = 0;
    //score p
...
void mtcnn::nms(std::vector<Bbox> &boundingBox_, std::vector<orderScore> &bboxScore_, const float overlap_threshold, string modelname="Union"){
...
int test_mtcnn2(int argc, char** argv)
{
...
int test_mtcnn2() {
  if (1) {
    sys_chdir("E:/code/cstd/include/dnn/ncnn/mtcnn2");
    sys_chdir("D:/code/gender/gender_64x64");
    Net net[1] = {0};
    bfsetsize(net->bf, 20<<20);
    net_load_param_from_file(net, "ncnn.proto", "ncnn.bin");
    img_t im[1] = {0};
    imread("7481.jpg", 3, 1, im);
    imresize(im, 64, 64, im);
    net_input(net, "data", im);
    img_t* out = net_extract_name(net, "prob");
    float* prob = pBox_datap(out);
    int i;
    for (i=0; i<net->blobs_size; ++i) {
      img_t* mat = net->blob_mats+i;
      printf("%16s %d %d %d %f %f\n", net->blobs[i].name, mat->h, mat->w, pBox_channels(mat), mat->tt.f4[0], mat->tt.f4[1]);
    }
    net_free(net);
  }
  return 0;
