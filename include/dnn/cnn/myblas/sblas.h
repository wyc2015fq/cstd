...
#ifdef __ARM_NEON
static float32x4_t vinvsqrt_f32(float32x4_t x) {
...
static float32x4_t vinv_f32(const float32x4_t &x) {
...
static float32x4_t vtaylor_poly_f32(const float32x4_t &x, const float32x4_t* coeffs) {
...
static float32x4_t vexp_f32(const float32x4_t &x) {
  ...
  static const float32x4_t exp_tab[8] = {
    vdupq_n_f32(1.f),
...
static float32x4_t vlog_f32(const float32x4_t &x) {
  static const int32x4_t   CONST_127 = vdupq_n_s32(127);
  static const float32x4_t CONST_LN2 = vdupq_n_f32(0.6931471805f);
  static const float32x4_t log_tab[8] = {
...
static float32x4_t vtanh_f32(const float32x4_t &val) {
...
static float32x4_t vpowq_f32(const float32x4_t &val, const float32x4_t &n) {
 ...
 *   return NaN for x <= 0
 */
static float32x4_t log_ps(float32x4_t x)
{
 ...
 *   sincos_ps..
 */
static void sincos_ps(float32x4_t x, float32x4_t* ysin, float32x4_t* ycos)
{
  // any x
...
static float32x4_t sin_ps(float32x4_t x)
{
...
static float32x4_t cos_ps(float32x4_t x)
{
  float32x4_t ysin, ycos;
  sincos_ps(x, &ysin, &ycos);
  return ycos;
...
static float32x4_t div_ps(float32x4_t a, float32x4_t b)
{
...
static float32x4_t pow_ps(float32x4_t a, float32x4_t b)
{
  // pow(x, m) = exp(m * log(x))
...
static void v_sqr(const int n, const float *xs, float *ys) {
#ifdef ANDROID
  int i, m = n / 4;
  float32x4_t xv, yv;
  for (i = 0; i < m; i++) {
...
static void axpy(const int n, const float alpha, const float *xs, float *ys) {
#ifdef ANDROID
  int i, m = n / 4;
...
static void v_pow(const int n, const float *xs, const float p, float *ys) {
#ifdef ANDROID
  int i, m = n / 4;
...
static void v_mul(const int n, const float *xs, const float *ys, float *zs) {
#ifdef ANDROID
  int i, m = n / 4;
...
//     delete[] trans_data;
// }
static void v_scale(const int n, const float scale_factor, const float *data, float *dest) {
  int i = 0; for (; i < n; ++i) {
    dest[i] = data[i] * scale_factor;
  }
}
static void v_add(const int N, const float alpha, float *Y) {
  int i = 0; for (; i < N; ++i) {
    Y[i] += alpha;
  }
}
static void v_div(const int n, const float *a, const float *b, float *y) {
  int i = 0; for (; i < n; ++i) {
    if (b[i] != 0) {
      y[i] = a[i] / b[i];
    }
...
static void v_exp(const int n, const float *a, float *y) {
  int i = 0; for (; i < n; ++i) {
    y[i] = expf(a[i]);
  }
}
