...
static float* from_rgb(const unsigned char* rgb, int w, int h, float* m)
{
  int size = w * h;
  float* ptr0 = m + (0*size);
  float* ptr1 = m + (1*size);
...
static uchar* to_rgb(const float* m, int w, int h, unsigned char* rgb)
{
  int size = w * h;
  const float* ptr0 = m + (0*size);
  const float* ptr1 = m + (1*size);
  const float* ptr2 = m + (2*size);
  unsigned char* ret = rgb;
...
static float* from_gray(const unsigned char* gray, int w, int h, float* m)
{
  int size = w * h;
  float* ptr = m;
...
static uchar* to_gray(const float* m, int w, int h, unsigned char* gray)
{
  int size = w * h;
  const float* ptr = m;
  unsigned char* ret = gray;
...
static float* from_rgba(const unsigned char* rgba, int w, int h, float* m)
{
  int size = w * h;
  float* ptr0 = m + (0*size);
  float* ptr1 = m + (1*size);
...
static uchar* to_rgba(const float* m, int w, int h, unsigned char* rgba)
{
  int size = w * h;
  const float* ptr0 = m + (size*0);
  const float* ptr1 = m + (size*1);
  const float* ptr2 = m + (size*2);
  const float* ptr3 = m + (size*3);
  uchar* ret = rgba;
...
static float* from_rgb2bgr(const unsigned char* rgb, int w, int h, float* m)
{
  int size = w * h;
  float* ptr0 = m + (size*0);
  float* ptr1 = m + (size*1);
...
static uchar* to_bgr2rgb(const float* m, int w, int h, unsigned char* rgb)
{
  int size = w * h;
  const float* ptr0 = m + (size*0);
  const float* ptr1 = m + (size*1);
...
static float* from_rgb2gray(const unsigned char* rgb, int w, int h, float* m)
{
...
static float* from_bgr2gray(const unsigned char* bgr, int w, int h, float* m)
{
  // coeffs for r g b = 0.299f, 0.587f, 0.114f
  const unsigned char Y_shift = 8;//14
  const unsigned char R2Y = 77;
...
static float* from_gray2rgb(const unsigned char* gray, int w, int h, float* m)
{
  int size = w * h;
  float* ptr0 = m + (size*0);
  float* ptr1 = m + (size*1);
...
static float* from_rgba2rgb(const unsigned char* rgba, int w, int h, float* m)
{
  int size = w * h;
  float* ptr0 = m + (size*0);
  float* ptr1 = m + (size*1);
...
static float* from_rgba2bgr(const unsigned char* rgba, int w, int h, float* m)
{
  int size = w * h;
  float* ptr0 = m + (size*0);
  float* ptr1 = m + (size*1);
...
static float* from_rgba2gray(const unsigned char* rgba, int w, int h, float* m)
{
  // coeffs for r g b = 0.299f, 0.587f, 0.114f
  const unsigned char Y_shift = 8;//14
  const unsigned char R2Y = 77;
...
#undef SATURATE_CAST_SHORT
  ...
  // loop body
  short* rowsbuf0 = MALLOC(short, w * 3 + 6);
  short* rowsbuf1 = MALLOC(short, w * 3 + 6);
...
#undef SATURATE_CAST_SHORT
  ...
  // loop body
  short* rowsbuf0 = MALLOC(short, w + 2);
  short* rowsbuf1 = MALLOC(short, w + 2);
  short* rows0 = (short*)rowsbuf0;
  short* rows1 = (short*)rowsbuf1;
...
#undef SATURATE_CAST_SHORT
  ...
  // loop body
  short* rowsbuf0 = MALLOC(short, w * 4 + 8);
  short* rowsbuf1 = MALLOC(short, w * 4 + 8);
  short* rows0 = (short*)rowsbuf0;
  short* rows1 = (short*)rowsbuf1;
...
static float* from_pixels(const unsigned char* pixels, int type, int w, int h, float* m)
{
  if (type & PIXEL_CONVERT_MASK) {
    if (type == PIXEL_RGB2BGR || type == PIXEL_BGR2RGB) {
      //MYREALLOC(m, h*w*3);
      return from_rgb2bgr(pixels, w, h, m);
    }

    if (type == PIXEL_RGB2GRAY) {
      //MYREALLOC(m, h*w);
      return from_rgb2gray(pixels, w, h, m);
    }

    if (type == PIXEL_BGR2GRAY) {
      //MYREALLOC(m, h*w);
      return from_bgr2gray(pixels, w, h, m);
    }

    if (type == PIXEL_GRAY2RGB || type == PIXEL_GRAY2BGR) {
      //MYREALLOC(m, h*w*3);
      return from_gray2rgb(pixels, w, h, m);
    }

    if (type == PIXEL_RGBA2RGB) {
      //MYREALLOC(m, h*w*3);
      return from_rgba2rgb(pixels, w, h, m);
    }

    if (type == PIXEL_RGBA2BGR) {
      //MYREALLOC(m, h*w*3);
      return from_rgba2bgr(pixels, w, h, m);
    }

    if (type == PIXEL_RGBA2GRAY) {
      //MYREALLOC(m, h*w);
      return from_rgba2gray(pixels, w, h, m);
    }
  }
  else {
    if (type == PIXEL_RGB || type == PIXEL_BGR) {
      //MYREALLOC(m, h*w*3);
      return from_rgb(pixels, w, h, m);
    }

    if (type == PIXEL_GRAY) {
      //MYREALLOC(m, h*w);
      return from_gray(pixels, w, h, m);
    }

    if (type == PIXEL_RGBA) {
      //MYREALLOC(m, h*w*4);
      return from_rgba(pixels, w, h, m);
    }
  }
...
static float* from_pixels_resize(const unsigned char* pixels, int type, int w, int h, int target_width, int target_height, float* m)
{
  if (w == target_width && h == target_height) {
    return from_pixels(pixels, type, w, h, m);
  }

  int type_from = type & PIXEL_FORMAT_MASK;

  if (type_from == PIXEL_RGB || type_from == PIXEL_BGR) {
    unsigned char* dst = MALLOC(unsigned char, target_width * target_height * 3);
    resize_bilinear_c3(pixels, w, h, dst, target_width, target_height);
    m = from_pixels(dst, type, target_width, target_height, m);
    FREE(dst);
  }
  else if (type_from == PIXEL_GRAY) {
    unsigned char* dst = MALLOC(unsigned char, target_width * target_height);
    resize_bilinear_c1(pixels, w, h, dst, target_width, target_height);
    m = from_pixels(dst, type, target_width, target_height, m);
    FREE(dst);
  }
  else if (type_from == PIXEL_RGBA) {
    unsigned char* dst = MALLOC(unsigned char, target_width * target_height * 4);
...
static uchar* to_pixels(const float* m, int w, int h, unsigned char* pixels, int type)
{
  if (type & PIXEL_CONVERT_MASK) {
    if (type == PIXEL_RGB2BGR || type == PIXEL_BGR2RGB) {
      return to_bgr2rgb(m, w, h, pixels);
    }
  }
  else {
    if (type == PIXEL_RGB || type == PIXEL_BGR) {
      return to_rgb(m, w, h, pixels);
    }

    if (type == PIXEL_GRAY) {
      return to_gray(m, w, h, pixels);
    }

    if (type == PIXEL_RGBA) {
      return to_rgba(m, w, h, pixels);
    }
  }
  return NULL;
}

static uchar* to_pixels_resize(const float* m, int w, int h, unsigned char* pixels, int type, int target_width, int target_height)
{
  if (w == target_width && h == target_height) {
    return to_pixels(m, w, h, pixels, type);
  }
  ...
  if (type_to == PIXEL_RGB || type_to == PIXEL_BGR) {
    unsigned char* src = MALLOC(unsigned char, w * h * 3);
    to_pixels(m, w, h, src, type);
    ...
    unsigned char* src = MALLOC(unsigned char, w * h * 4);
    to_pixels(m, w, h, src, type);
