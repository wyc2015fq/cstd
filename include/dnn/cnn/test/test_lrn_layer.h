...
int LRNLayerTest(Net* net, int* pbottom, int* pbottom_2) {
...
void LRNLayerTest_ReferenceLRNForward() {
...
class CuDNNLRNLayerTest : public GPUDeviceTest {
 protected:
  CuDNNLRNLayerTest()
      : epsilon_(Dtype(1e-5)),
        blob_bottom_(new Blob()),
...
void CuDNNLRNLayerTest::ReferenceLRNForward(
...
TYPED_TEST(CuDNNLRNLayerTest, TestForwardAcrossChannelsLargeRegionCuDNN) {
  ...
  LayerParameter layer_param;
  layer_param.mutable_lrn_param()->set_local_size(15);
  CuDNNLRNLayer layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  Blob top_reference;
  this->ReferenceLRNForward(*(this->blob_bottom_), layer_param,
...
TYPED_TEST(CuDNNLRNLayerTest, TestForwardWithinChannel) {
  ...
  layer_param.mutable_lrn_param()->set_norm_region(
      LRNParameter_NormRegion_WITHIN_CHANNEL);
  layer_param.mutable_lrn_param()->set_local_size(3);
  CuDNNLCNLayer layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  Blob top_reference;
  this->ReferenceLRNForward(*(this->blob_bottom_), layer_param,
