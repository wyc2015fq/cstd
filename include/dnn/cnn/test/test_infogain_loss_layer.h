...
class InfogainLossLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  InfogainLossLayerTest()
      : blob_bottom_data_(new Blob(4, 2, 5, 2)),
        blob_bottom_label_(new Blob(4, 2, 1, 2)),
        blob_bottom_infogain_(new Blob(1, 1, 5, 5)),
        blob_top_loss_(new Blob()),
        blob_top_prob_(new Blob()),
...
TYPED_TEST(InfogainLossLayerTest, TestInfogainLoss) {
  ...
  vector est_prob(this->num_labels_, 0);
...
TYPED_TEST(InfogainLossLayerTest, TestGradient) {
  ...
  GradientChecker checker(1e-4, 2e-2, 1701);  // no "kink"
