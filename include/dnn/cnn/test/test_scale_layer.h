...
int ScaleLayerTest(Net* net) {
  const char* type = "bottom";
  int t = net_addBlob(net, dataSize(2, 3, 4, 5), "bottom", NULL );
  net_init_gaussian(net, t, 0, 1);
  return 1;
}
...
class ScaleLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  ScaleLayerTest()
      : blob_bottom(new Blob(2, 3, 4, 5)),
        blob_bottom_eltwise_(new Blob(2, 3, 4, 5)),
        blob_bottom_broadcast_0(new Blob()),
        blob_bottom_broadcast_1(new Blob()),
        blob_bottom_broadcast_2(new Blob()),
        blob_bottom_scale(new Blob(vector<int>())),
        blob_top_(new Blob()) {
    Caffe::set_random_seed(1701);
    vector<int> broadcast_shape(2);
    broadcast_shape[0] = 2; broadcast_shape[1] = 3;
    this->blob_bottom_broadcast_0->Reshape(broadcast_shape);
    broadcast_shape[0] = 3; broadcast_shape[1] = 4;
    this->blob_bottom_broadcast_1->Reshape(broadcast_shape);
    broadcast_shape[0] = 4; broadcast_shape[1] = 5;
    this->blob_bottom_broadcast_2->Reshape(broadcast_shape);
    ...
    filler.Fill(this->blob_bottom_broadcast_0);
    filler.Fill(this->blob_bottom_broadcast_1);
    filler.Fill(this->blob_bottom_broadcast_2);
    filler.Fill(this->blob_bottom_scale);
    blob_bottom_vec_.push_back(blob_bottom);
...
TYPED_TEST(ScaleLayerTest, TestForwardEltwise) {
  ...
  shared_ptr<ScaleLayer > layer(new ScaleLayer(layer_param));
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  ASSERT_EQ(this->blob_bottom->shape(), this->blob_top_->shape());
  layer->Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  const Dtype* data = this->blob_top_->cpu_data();
  const int count = this->blob_top_->count();
  const Dtype* in_data_a = this->blob_bottom->cpu_data();
...
TYPED_TEST(ScaleLayerTest, TestForwardEltwiseInPlace) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_top_vec_[0] = this->blob_bottom;  // in-place computation
  Blob orig_bottom(this->blob_bottom->shape());
  orig_bottom.CopyFrom(*this->blob_bottom);
  this->blob_bottom_vec_.push_back(this->blob_bottom_eltwise_);
  LayerParameter layer_param;
  layer_param.mutable_scale_param()->set_axis(0);
  shared_ptr<ScaleLayer > layer(new ScaleLayer(layer_param));
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  layer->Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  const Dtype* data = this->blob_bottom->cpu_data();
  const int count = this->blob_bottom->count();
...
TYPED_TEST(ScaleLayerTest, TestBackwardEltwiseInPlace) {
  ...
  layer_param.mutable_scale_param()->set_axis(0);
  shared_ptr<ScaleLayer > layer(new ScaleLayer(layer_param));
  Blob top_diff(this->blob_bottom->shape());
  FillerParameter filler_param;
  ...
  orig_bottom_diff.CopyFrom(*this->blob_bottom, kCopyDiff, kReshape);
  ...
  layer->Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  caffe_copy(top_diff.count(), top_diff.cpu_data(),
             this->blob_bottom->mutable_cpu_diff());
  layer->Backward(this->blob_top_vec_, propagate_down, this->blob_bottom_vec_);
  for (int i = 0; i < this->blob_bottom->count(); ++i) {
    EXPECT_NEAR(orig_bottom_diff.cpu_diff()[i],
                this->blob_bottom->cpu_diff()[i], 1e-5);
  }
...
TYPED_TEST(ScaleLayerTest, TestForwardBroadcastBegin) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.push_back(this->blob_bottom_broadcast_0);
  LayerParameter layer_param;
  layer_param.mutable_scale_param()->set_axis(0);
  shared_ptr<ScaleLayer > layer(new ScaleLayer(layer_param));
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  ASSERT_EQ(this->blob_bottom->shape(), this->blob_top_->shape());
  layer->Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  for (int n = 0; n < this->blob_bottom->num(); ++n) {
    for (int c = 0; c < this->blob_bottom->channels(); ++c) {
      for (int h = 0; h < this->blob_bottom->height(); ++h) {
        for (int w = 0; w < this->blob_bottom->width(); ++w) {
          EXPECT_NEAR(this->blob_top_->data_at(n, c, h, w),
                      this->blob_bottom->data_at(n, c, h, w) *
                      this->blob_bottom_broadcast_0->data_at(n, c, 0, 0),
                      1e-5);
        }
      }
    }
...
TYPED_TEST(ScaleLayerTest, TestForwardBroadcastMiddle) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.push_back(this->blob_bottom_broadcast_1);
  LayerParameter layer_param;
    ...
    for (int c = 0; c < this->blob_bottom->channels(); ++c) {
      for (int h = 0; h < this->blob_bottom->height(); ++h) {
        for (int w = 0; w < this->blob_bottom->width(); ++w) {
          EXPECT_NEAR(this->blob_top_->data_at(n, c, h, w),
                      this->blob_bottom->data_at(n, c, h, w) *
                      this->blob_bottom_broadcast_1->data_at(c, h, 0, 0),
                      1e-5);
        }
      }
    }
...
TYPED_TEST(ScaleLayerTest, TestForwardBroadcastMiddleInPlace) {
  ...
  for (int n = 0; n < this->blob_bottom->num(); ++n) {
    for (int c = 0; c < this->blob_bottom->channels(); ++c) {
      for (int h = 0; h < this->blob_bottom->height(); ++h) {
        for (int w = 0; w < this->blob_bottom->width(); ++w) {
          EXPECT_NEAR(this->blob_bottom->data_at(n, c, h, w),
...
TYPED_TEST(ScaleLayerTest, TestBackwardBroadcastMiddleInPlace) {
  ...
  orig_bottom_diff.CopyFrom(*this->blob_bottom, kCopyDiff, kReshape);
  Blob orig_scale_diff;
  orig_scale_diff.CopyFrom(*this->blob_bottom_broadcast_1,
                            kCopyDiff, kReshape);
  // Rerun forward + backward with in-place computation;
  // check that resulting bottom diffs are the same.
  this->blob_top_vec_[0] = this->blob_bottom;  // in-place computation
  ...
  for (int i = 0; i < this->blob_bottom_broadcast_1->count(); ++i) {
    EXPECT_NEAR(orig_scale_diff.cpu_diff()[i],
                this->blob_bottom_broadcast_1->cpu_diff()[i], 1e-5);
  }
}
...
TYPED_TEST(ScaleLayerTest, TestForwardBroadcastEnd) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.push_back(this->blob_bottom_broadcast_2);
  LayerParameter layer_param;
    ...
    for (int c = 0; c < this->blob_bottom->channels(); ++c) {
      for (int h = 0; h < this->blob_bottom->height(); ++h) {
        for (int w = 0; w < this->blob_bottom->width(); ++w) {
          EXPECT_NEAR(this->blob_top_->data_at(n, c, h, w),
                      this->blob_bottom->data_at(n, c, h, w) *
                      this->blob_bottom_broadcast_2->data_at(h, w, 0, 0),
                      1e-5);
        }
      }
    }
...
TYPED_TEST(ScaleLayerTest, TestForwardScale) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.push_back(this->blob_bottom_scale);
  LayerParameter layer_param;
  shared_ptr<ScaleLayer > layer(new ScaleLayer(layer_param));
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  ASSERT_EQ(this->blob_bottom->shape(), this->blob_top_->shape());
  ...
  const Dtype* data = this->blob_top_->cpu_data();
  const int count = this->blob_top_->count();
  const Dtype* in_data = this->blob_bottom->cpu_data();
  const Dtype scale = *this->blob_bottom_scale->cpu_data();
  for (int i = 0; i < count; ++i) {
