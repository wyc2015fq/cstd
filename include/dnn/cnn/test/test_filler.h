...
class ConstantFillerTest : public ::testing::Test {
 protected:
  ConstantFillerTest()
      : blob_(new Blob(2, 3, 4, 5)),
        filler_param_() {
    filler_param_.set_value(10.);
    filler_.reset(new ConstantFiller(filler_param_));
    filler_->Fill(blob_);
  }
  virtual ~ConstantFillerTest() { delete blob_; }
  Blob* const blob_;
  FillerParameter filler_param_;
  shared_ptr<ConstantFiller > filler_;
};
...
class UniformFillerTest : public ::testing::Test {
 protected:
  UniformFillerTest()
    ...
    filler_.reset(new UniformFiller(filler_param_));
    filler_->Fill(blob_);
  }
  virtual ~UniformFillerTest() { delete blob_; }
  Blob* const blob_;
  FillerParameter filler_param_;
  shared_ptr<UniformFiller > filler_;
};
...
class PositiveUnitballFillerTest : public ::testing::Test {
 protected:
  PositiveUnitballFillerTest()
      : blob_(new Blob(2, 3, 4, 5)),
        filler_param_() {
    filler_.reset(new PositiveUnitballFiller(filler_param_));
    filler_->Fill(blob_);
  }
  virtual ~PositiveUnitballFillerTest() { delete blob_; }
  Blob* const blob_;
  FillerParameter filler_param_;
  shared_ptr<PositiveUnitballFiller > filler_;
};
...
class GaussianFillerTest : public ::testing::Test {
 protected:
  GaussianFillerTest()
    ...
    filler_.reset(new GaussianFiller(filler_param_));
    filler_->Fill(blob_);
  }
  virtual ~GaussianFillerTest() { delete blob_; }
  Blob* const blob_;
  FillerParameter filler_param_;
  shared_ptr<GaussianFiller > filler_;
};
...
class XavierFillerTest : public ::testing::Test {
 protected:
  ...
  Blob* const blob_;
  FillerParameter filler_param_;
  shared_ptr<XavierFiller > filler_;
};
...
class MSRAFillerTest : public ::testing::Test {
 protected:
  ...
  virtual void test_params(FillerParameter_VarianceNorm variance_norm,
      Dtype n) {
    this->filler_param_.set_variance_norm(variance_norm);
    this->filler_.reset(new MSRAFiller(this->filler_param_));
    this->filler_->Fill(blob_);
    EXPECT_TRUE(this->blob_);
    const int count = this->blob_->count();
    const Dtype* data = this->blob_->cpu_data();
