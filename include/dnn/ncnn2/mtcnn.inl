...
static void pBoxShowE(const pBox* pbox, int channel, int row)
{
  ...
  printf("the data is :\n");
  mydataFmt* p = pbox->data->p + channel * pbox->width * pbox->height;
  //pbox->channel
  printf("the %d th channel data is :\n", channel);
  //pbox->height
...
#if 0
typedef struct orderScore { mydataFmt score; int oriOrder; } orderScore;
...
static int nms1(buf_t* bf, Bbox* boundingBox_, int boundingBox_size, const float overlap_threshold, RectOpt modelname CC_DEFAULT(RO_Union))
{
  int* heros = NULL;
  int heros_size = 0;
...
static int nms(buf_t* bf, Bbox* boundingBox_, int boundingBox_size, const float overlap_threshold, RectOpt modelname CC_DEFAULT(RO_Union) )
{
  int i, j;
  int count = 0;
  //sort the score
...
static int generateBbox(float Pthreshold, pBox* score, const pBox* location, mydataFmt scale, struct Bbox* boundingBox_, int boundingBox_size)
{
  //for pooling
  int stride = 2;
  int cellsize = 12;
  ...
  //score p
  mydataFmt* p = pBox_datap(score) + score->width * score->height;
  mydataFmt* plocal = pBox_datap(location);
  struct Bbox bbox;
...
static int Pnet_run(buf_t* bf, const Pnet* s, img_t* image, float scale, struct Bbox* boundingBox_, int boundingBox_size)
{
  int count;
  pBox w1[20] = {0};
  pBox* w = w1;
...
static int Rnet_run(buf_t* bf, const Rnet* s, img_t* image, struct Bbox* it)
{
  pBox w1[20] = {0};
  pBox* w = w1;
  pBox* w0 = w;
...

  {
    ...
    for (f = 0; f < n; ++f, ++it) {
      ...
      if (scorep[1] > s->Rthreshold) {
        ...
        memcpy(it->regreCoord, locationp, 4 * sizeof(mydataFmt));
        it->area = (it->x2 - it->x1) * (it->y2 - it->y1);
        it->score = *(scorep + 1);
      }
      else {
...
static int Onet_run(buf_t* bf, const Onet* s, img_t* image, struct Bbox* it)
{
  pBox w1[20] = {0};
  pBox* w = w1;
  pBox* score_ = w++;
  ...
  convolution(bf, s->conv3_wb, pooling2_out, conv3_out);
  prelu(conv3_out, s->conv3_wb->bias, s->prelu_gmma3->data);
  maxPooling(bf, conv3_out, pooling3_out, 2, 2);

  //conv4
  // feature2Matrix(pooling3_out, conv4_matrix, s->conv4_wb);
  convolution(bf, s->conv4_wb, pooling3_out, conv4_out);
  prelu(conv4_out, s->conv4_wb->bias, s->prelu_gmma4->data);

  fullconnect(bf, s->fc5_wb, conv4_out, fc5_out);
  prelu(fc5_out, s->fc5_wb->bias, s->prelu_gmma5->data);

  //conv6_1   score
  fullconnect(bf, s->score_wb, fc5_out, score_);
  addbias(score_, s->score_wb->bias);
  softmax(score_);
  // pBoxShow(s->score_);

  //conv6_2   location
  fullconnect(bf, s->location_wb, fc5_out, location_);
  addbias(location_, s->location_wb->bias);
  // pBoxShow(location_);

  //conv6_2   location
  fullconnect(bf, s->keyPoint_wb, fc5_out, keyPoint_);
  ...
  umem_getu(keyPoint_->data);
  {
    ...
    for (f = 0; f < n; ++f, ++it) {
      ...
      if (scorep[1] > s->Othreshold) {
        ...
        for (num = 0; num < 5; num++) {
          (it->ppoint)[num] = it->y1 + (it->y2 - it->y1) * (*(keyPoint_p + num));
        }
        
        for (num = 0; num < 5; num++) {
          (it->ppoint)[num + 5] = it->x1 + (it->x2 - it->x1) * (*(keyPoint_p + num + 5));
        }
      }
      else {
        it->exist = false;
...
CC_IMPL img_t*
bf_resize(buf_t* bf, const img_t* src, img_t* dst, CSize dsize, double fx CC_DEFAULT(0),
          double fy CC_DEFAULT(0), int method CC_DEFAULT(CC_INTER_LINEAR))
{
  bf_imresize(bf, src, dsize.h, dsize.w, dst);
  return dst;
}
...
static int mtcnn_findFace(buf_t* bf, const img_t* image, Bbox* out, int maxout, int minsize CC_DEFAULT(60))
{
  ...
  for (i = 0; i < scales_size && count < box_size; i++) {
    ...
    bf1 = *bf;
    tic;
    bf_resize(&bf1, image, reImage, cSize(changedW, changedH), 0, 0, CC_INTER_LINEAR);
    toc;
    tic;
    boundingBox_size = Pnet_run(&bf1, s->simpleFace_, reImage, scales_[i], box + box_pos, box_size - box_pos);
    toc;
    boundingBox_size = nms(bf, box + box_pos, boundingBox_size, s->simpleFace_->nms_threshold);
  ...
  for (i = 0; i < scales_size && count < box_size; i++) {
    ...
    boundingBox_size = Pnet_run(&bf1, s->simpleFace_, reImage, scales_[i], box + box_pos, box_size - box_pos);
    toc;
    //boundingBox_size = nms(bf, box + box_pos, boundingBox_size, s->simpleFace_->nms_threshold, RO_Union);
    box_pos += boundingBox_size;
    //imshow_(reImage);WaitKey(-1);
  }

  count = nms(bf, box, box_pos, s->simpleFace_->nms_threshold, RO_Union);
  ...
  if (count > 0) {
    count = nms(bf, box, count, s->nms_threshold[0], RO_Union);
  ...
  //printf("%d - %d\n", omp_get_num_threads(), omp_get_thread_num());
      ...
      bf1 = *bf;
      bf_resize(&bf1, temp, secImage, cSize(24, 24), 0, 0, CC_INTER_LINEAR);
      tic;
      Rnet_run(&bf1, s->refineNet, secImage, it);
      toc;
    }
    ...
    if (count > 0) {
      count = nms(bf, box, count, s->nms_threshold[1], RO_Union);
      refineAndSquareBbox(box, count, image->rows, image->cols);
      //third stage
...
//#pragma omp parallel for
      for (i = 0; i < count; ++i) {
        Bbox* it = box + i;
        img_t thirdImage[1] = {0};
        img_t temp[1] = {0};
        cvGetSubRect1(image, temp, it->y1, it->x1, it->y2 - it->y1, it->x2 - it->x1);
        bf1 = *bf;
        bf_resize(&bf1, temp, thirdImage, cSize(48, 48), 0, 0, CC_INTER_LINEAR);
        ASSERT(it->exist);
        tic;
        Onet_run(&bf1, s->outNet, thirdImage, it);
        toc;
      }
      ...
      if (count > 0) {
        refineAndSquareBbox(box, count, image->rows, image->cols);
        count = nms(bf, box, count, s->nms_threshold[2], RO_Min);
      }
    }
  }
