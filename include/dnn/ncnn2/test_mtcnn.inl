...
int test_mtcnn()
{
  ...
  if (0) {
    ...
    imread(filename, 3, 1, im);
    {
      ...
      for (i = 0; i < times; ++i) {
        nface = mtcnn_findFace(bf, im, out, countof(out), minsize);
      }
    ...
    if (capdshow_open(cap, 0, w, h) > 0) {
      ...
      for (; 'q' != ch && 'Q' != ch; ++fram) {
        ...
        
        {
          ...
          utime_start(_start_time);
          tictoc0;
          nface = mtcnn_findFace(bf, im1, out, countof(out), minsize);
          tictoc;
        ...
        if (1) {
          ...
          for (i = 0; i < nface; ++i) {
            ...
            cvRectangle(im1, cPoint((*it).y1, (*it).x1), cPoint((*it).y2, (*it).x2), cScalar(0, 0, 255, 0), 2, 8, 0);
