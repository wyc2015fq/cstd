...
int LSTM::load_model(const unsigned char*& mem)
{
  ...
  // raw weight data
  weight_hc_data = img_t(size * 4, num_output, (float*)mem);
  mem += size * 4 * num_output * sizeof(float);

  weight_xc_data = img_t(size * 4, num_output, (float*)mem);
  mem += size * 4 * num_output * sizeof(float);

  bias_c_data = img_t(4, num_output, (float*)mem);
  mem += 4 * num_output * sizeof(float);

  return 0;
}

int LSTM::forward(const std::vector<img_t>& bottom_blobs, std::vector<img_t>& top_blobs) const
{
  // size x 1 x T
