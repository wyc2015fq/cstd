...
{
  slope_data = img_t(channels, (float*)mem);
  mem += channels * sizeof(float);

  mean_data = img_t(channels, (float*)mem);
  mem += channels * sizeof(float);

  var_data = img_t(channels, (float*)mem);
  mem += channels * sizeof(float);

  bias_data = img_t(channels, (float*)mem);
  mem += channels * sizeof(float);
...
int BatchNorm::forward(const img_t* bottom_blob, img_t* top_blob) const
{
...
int BatchNorm::forward_inplace(img_t* bottom_top_blob) const
{
  // a = bias - slope * mean / sqrt(var)
  // b = slope / sqrt(var)
  // value = b * value + a
