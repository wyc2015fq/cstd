...
static int unary_op(const img_t* a, img_t* b)
{
  Op op;
...
template<typename Op>
static int unary_op_inplace(img_t* a)
{
  Op op;
...
int UnaryOp::forward(const img_t* bottom_blob, img_t* top_blob) const
{
...
int UnaryOp::forward_inplace(img_t* bottom_top_blob) const
{
  if (op_type == Operation_ABS) {
