...
static img_t generate_anchors(int base_size, const img_t* ratios, const img_t* scales)
{
...
int Proposal::forward(const std::vector<img_t>& bottom_blobs, std::vector<img_t>& top_blobs) const
{
  const img_t* score_blob = bottom_blobs[0];
  const img_t* bbox_blob = bottom_blobs[1];
  const img_t* im_info_blob = bottom_blobs[2];
  ...
  if (top_blobs.size() > 1) {
    img_t* roi_score_blob = top_blobs[1];
