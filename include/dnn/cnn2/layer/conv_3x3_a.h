...
#of 0
static void sum_neon(const Blob* bottom_blob, Blob* top_blob) {
  int i, inch = Blob_channels(bottom_blob);
  pBoxsetsize(top_blob, bottom_blob->h, bottom_blob->w, 1, );
  return ;
}
#endif
static void conv3x3s1_neon(const Blob* bottom_blob, Blob* top_blob, const float* _kernel, const float* _bias)
{
  int w = bottom_blob->w;
  int h = bottom_blob->h;
  int p, q, inch = Blob_channels(bottom_blob);
