...
int InnerProduct_arm_forward(const Blob* bottom_blob, Blob* top_blob, int num_output, const float* weight_data, const float* bias_data)
{
  int w = bottom_blob->w;
  int h = bottom_blob->h;
  ...
  for (int p = 0; p < num_output; p++) {
    float* outptr = Blob_channel(top_blob, p);
    float sum = 0.f;
    ...
    for (int q = 0; q < channels; q++) {
      const float* m = Blob_channel_p(bottom_blob, q);
