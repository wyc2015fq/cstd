...
int RNN::load_model(const unsigned char*& mem)
{
  ...
  // raw weight data
  weight_hh_data = Blob(size, num_output, (float*)mem);
  mem += size * num_output * sizeof(float);

  weight_xh_data = Blob(size, num_output, (float*)mem);
  mem += size * num_output * sizeof(float);

  weight_ho_data = Blob(num_output, num_output, (float*)mem);
  mem += num_output * num_output * sizeof(float);

  bias_h_data = Blob(num_output, (float*)mem);
  mem += num_output * sizeof(float);

  bias_o_data = Blob(num_output, (float*)mem);
  mem += num_output * sizeof(float);

  return 0;
}

int RNN::forward(const std::vector<Blob>& bottom_blobs, std::vector<Blob>& top_blobs) const
{
  // size x 1 x T
  ...
  for (int t = 0; t < T; t++) {
    ...
    Blob output = Blob_channel_p(top_blob, t);
