  ...
  for (int is_fg = 0; is_fg < 2; ++is_fg) {
    for (int dummy = 0; dummy < num_samples[is_fg]; ++dummy) {
      ...
      if (context_pad > 0 || use_square) {
        ...
        cv_crop_size.width =
            static_cast<int>(round(static_cast(clipped_width)*scale_x));
        cv_crop_size.height =
            static_cast<int>(round(static_cast(clipped_height)*scale_y));
        pad_x1 = static_cast<int>(round(static_cast(pad_x1)*scale_x));
        pad_x2 = static_cast<int>(round(static_cast(pad_x2)*scale_x));
        pad_y1 = static_cast<int>(round(static_cast(pad_y1)*scale_y));
        pad_y2 = static_cast<int>(round(static_cast(pad_y2)*scale_y));
