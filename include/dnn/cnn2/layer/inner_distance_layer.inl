...
void InnerDistanceLayer::Forward_cpu(const vector<Blob*>& bottom,
    const vector<Blob*>& top) {
  const Dtype* bottom_data = Blob_datap(bottom_blob->data);
  Dtype* top_data = top[0]->mutable_cpu_data();
  const Dtype* weight = bottom.size() >= 2 ? Blob_datap(top_blob->data) : this->blobs_[0]->cpu_data();
  ...
  if (normalize_ && bottom.size() == 1) {
    ...
    for (int n = 0; n < N_; n++) {
      ...
      caffe_scal(K_, Dtype(1) / sqrt(sum_sq), mutable_weight + n*K_);
    }
  }
...
void InnerDistanceLayer::Backward_cpu(const vector<Blob*>& top,
