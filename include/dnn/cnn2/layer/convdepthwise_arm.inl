...
int ConvolutionDepthWise_arm::forward(const Blob* bottom_blob, Blob* top_blob) const
{
  ...
  if (channels == group && group == num_output) {
    ...
    for (int g = 0; g < group; g++) {
      ...
      Blob top_blob_g = Blob_channel_p(top_blob, g);
  ...
  for (int g = 0; g < group; g++) {
    ...
    Blob top_blob_g(outw, outh, num_output_g, Blob_channel_p(top_blob, num_output_g * g));
