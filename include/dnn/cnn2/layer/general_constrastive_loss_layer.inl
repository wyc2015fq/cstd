...
void GeneralContrastiveLossLayer::Forward_cpu(const vector<Blob*>& bottom,
    const vector<Blob*>& top) {
  const Dtype* bottom_data = (bottom.size() == 3) ? bottom[2]->cpu_data() : Blob_datap(bottom_blob->data);
...
void GeneralContrastiveLossLayer::Backward_cpu(const vector<Blob*>& top,
