...
int ConvolutionDepthWise::forward(const Blob* bottom_blob, Blob* top_blob) const
{
  if (group == 1) {
  ...
  Blob bottom_blob_bordered = bottom_blob;
  ...
  if (channels == group && group == num_output) {
    ...
    for (int g = 0; g < group; g++) {
      float* outptr = Blob_channel_p(top_blob, g);
  ...
  for (int g = 0; g < group; g++) {
    for (int p = 0; p < num_output_g; p++) {
      float* outptr = Blob_channel_p(top_blob, g * num_output_g + p);
