...
void LargeMarginInnerProductLayer::Forward_cpu(const vector<Blob*>& bottom,
    const vector<Blob*>& top) {
  iter_ += (Dtype)1.;
  ...
  caffe_cpu_gemm(CblasTrans, CblasNoTrans, M_, N_, 1, (Dtype)1.,
  ...
  case LargeMarginInnerProductParameter_LargeMarginType_DOUBLE: {
    ...
    // + lambda * x'w
    caffe_cpu_gemm(CblasNoTrans, CblasTrans, M_, N_, K_, lambda_,
...
void LargeMarginInnerProductLayer::Backward_cpu(const vector<Blob*>& top,
  ...
  if ((bottom.size()==3 && propagate_down[2]) || this->param_propagate_down_[0]) {
    ...
    case LargeMarginInnerProductParameter_LargeMarginType_DOUBLE: {
      ...
      // + lambda/(1 + lambda) * x
      caffe_cpu_gemm(CblasTrans, CblasNoTrans, N_, K_, M_, lambda_/((Dtype)1. + lambda_),
        top_diff, bottom_data, (Dtype)1., this->blobs_[0]->mutable_cpu_diff());
      break;
    }
    case LargeMarginInnerProductParameter_LargeMarginType_TRIPLE: {
  ...
  if (propagate_down[0]) {
    ...
    case LargeMarginInnerProductParameter_LargeMarginType_DOUBLE: {
      ...
      // + lambda/(1 + lambda) * w
      caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, M_, K_, N_, lambda_/((Dtype)1. + lambda_),
