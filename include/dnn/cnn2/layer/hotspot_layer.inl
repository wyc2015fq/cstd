...
void HotspotLayer::Forward_cpu(const vector<Blob*>& bottom,
      const vector<Blob*>& top) {
  const Dtype* point_data = Blob_datap(bottom_blob->data);
...
void HotspotLayer::Backward_cpu(const vector<Blob*>& top,
