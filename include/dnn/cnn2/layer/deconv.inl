...
int Deconvolution::load_model(FILE* binfp)
{
  ...
  if (flag_struct.tag == 0x01306B47) {
    ...
    weight_data = Blob::from_float16(float16_weights.data(), weight_data_size);
...
int Deconvolution::load_model(const unsigned char*& mem)
{
  ...
  if (flag_struct.tag == 0x01306B47) {
    // half-precision weight data
    weight_data = Blob::from_float16((unsigned short*)mem, weight_data_size);
...
int Deconvolution::forward(const Blob* bottom_blob, Blob* top_blob) const
{
  ...
  for (int p = 0; p < num_output; p++) {
    Blob out = top_blob_bordered.channel(p);
    ...
    for (int i = 0; i < h; i++) {
      for (int j = 0; j < w; j++) {
        ...
        for (int q = 0; q < channels; q++) {
          const Blob m = Blob_channel_p(bottom_blob, q);
