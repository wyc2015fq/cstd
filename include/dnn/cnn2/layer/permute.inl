...
int Permute::forward(const Blob* bottom_blob, Blob* top_blob) const
{
  int w = bottom_blob->w;
  int h = bottom_blob->h;
  ...
  if (order_type == 0) {
    ...
    for (int q = 0; q < h; q++) {
      ...
      for (int i = 0; i < channels; i++) {
        const float* ptr = Blob_channel_p(bottom_blob, i).row(q);
    ...
    for (int q = 0; q < h; q++) {
      ...
      for (int i = 0; i < w; i++) {
        for (int j = 0; j < channels; j++) {
          const float* ptr = Blob_channel_p(bottom_blob, j).row(q);
    ...
    for (int q = 0; q < w; q++) {
      ...
      for (int i = 0; i < channels; i++) {
        const float* ptr = Blob_channel_p(bottom_blob, i);
    ...
    for (int q = 0; q < w; q++) {
      ...
      for (int i = 0; i < h; i++) {
        for (int j = 0; j < channels; j++) {
          const float* ptr = Blob_channel_p(bottom_blob, j);
