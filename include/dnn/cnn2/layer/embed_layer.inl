...
void EmbedLayer::Forward_cpu(const vector<Blob*>& bottom,
  ...
  for (int n = 0; n < M_; ++n) {
    ...
    DCHECK_GE(index, 0);
    DCHECK_LT(index, K_);
    DCHECK_EQ(static_cast(index), bottom_data[n]) << "non-integer input";
...
void EmbedLayer::Backward_cpu(const vector<Blob*>& top,
  ...
  if (this->param_propagate_down_[0]) {
    ...
    for (int n = 0; n < M_; ++n) {
      ...
      DCHECK_GE(index, 0);
      DCHECK_LT(index, K_);
      DCHECK_EQ(static_cast(index), bottom_data[n])
          << "non-integer input";
