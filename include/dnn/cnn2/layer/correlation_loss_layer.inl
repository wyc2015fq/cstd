...
void CorrelationLossLayer::Backward_cpu(const vector<Blob*>& top,
  ...
  if (propagate_down[0]) {
    caffe_cpu_scale(bottom[0]->count(), top[0]->cpu_diff()[0] / bottom[0]->count(), Blob_datap(top_blob->data), bottom[0]->mutable_cpu_data());
  }
  if (propagate_down[1]) {
    caffe_cpu_scale(bottom[1]->count(), top[0]->cpu_diff()[0] / bottom[0]->count(), Blob_datap(bottom_blob->data), bottom[1]->mutable_cpu_data());
  }
}
