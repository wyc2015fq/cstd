...
{
  scale_data = Blob(scale_data_size, (float*)mem);
  mem += scale_data_size * sizeof(float);

  if (bias_term) {
    bias_data = Blob(scale_data_size, (float*)mem);
    mem += scale_data_size * sizeof(float);
  }
...
int Scale::forward(const Blob* bottom_blob, Blob* top_blob) const
{
  int w = bottom_blob->w;
  int h = bottom_blob->h;
...
int Scale::forward_inplace(Blob* bottom_top_blob) const
{
