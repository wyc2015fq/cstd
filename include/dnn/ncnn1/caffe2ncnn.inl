    ...
    for (int i=0; i<layer_count; i++)
    {
        ...
        else if (layer.type() == "Convolution")
        {
			...
			for (int j = 0; j < binlayer.blobs_size(); j++) {
				...
				if (j == 1) {
					bias_size = blob.data_size();
				}
				data_size += blob.data_size();
			}
			fprintf(pp, " data_size=%d", data_size);
        ...
        else if (layer.type() == "InnerProduct")
        {
            ...
            for (int j=0; j<binlayer.blobs_size(); j++)
            {
				...
				{
                    // write original data
					const float* data = blob.data().data();
                    fwrite(data, sizeof(float), blob.data_size(), bp);
                }
            }
        }
        else if (layer.type() == "Input")
        {
            ...
            {
				if (bs.dim(j)>1)
					fprintf(pp, " dim%d=%ld", j, bs.dim(j));
            }
        }
        else if (layer.type() == "Interp")
        {
            ...
            fprintf(pp, " %d", 2);
            fprintf(pp, " %f", (float)interp_param.zoom_factor());
            fprintf(pp, " %f", (float)interp_param.zoom_factor());
        ...
        else if (layer.type() == "LRN")
        {
            ...
            fprintf(pp, " %f", lrn_param.alpha());
            fprintf(pp, " %f", lrn_param.beta());
        }
        ...
        else if (layer.type() == "Pooling")
        {
            ...
            fprintf(pp, " pool=%d", pooling_param.pool());
        ...
        else if (layer.type() == "Power")
        {
            ...
            fprintf(pp, " %f", power_param.power());
            fprintf(pp, " %f", power_param.scale());
            fprintf(pp, " %f", power_param.shift());
        }
        else if (layer.type() == "PReLU")
        {
            const caffe::LayerParameter& binlayer = net.layer(netidx);
            const caffe::BlobProto& slope_blob = binlayer.blobs(0);
            fprintf(pp, " data_size=%d", slope_blob.data_size());
...
//                 float scale;
        ...
        else if (layer.type() == "ReLU")
        {
			...
			fprintf(pp, " slope=%f", negative_slope);
        ...
        else if (layer.type() == "ROIPooling")
        {
            ...
            fprintf(pp, " %f", roi_pooling_param.spatial_scale());
        }
