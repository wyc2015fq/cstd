...
//		dst_data[dst_idx] = 0;
		...
		loc1[dst_idx] = static_cast(src_idx);
		...
		if (src_w + 1 < src_width)
		{
			loc2[dst_idx] = static_cast(src_idx + 1);
...
//			dst_data[dst_idx] += (w_h1 * w_w0 * src_data[src_idx + src_width]);
			...
			loc3[dst_idx] = static_cast(src_idx + src_width);
		}

		if (src_w + 1 < src_width && src_h + 1 < src_height)
		{
			loc4[dst_idx] = static_cast(src_idx + src_width + 1);
...
void CropBlobs_cpu( const Blob&src,
		const int start_h, const int start_w,
		const int end_h, const int end_w, Blob&dst)
{
