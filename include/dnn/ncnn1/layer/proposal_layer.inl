                                       ...
                                       const vector<Blob *> &top)
{
  ...
  const Dtype *p_bottom_item = pBox_datap(bottom_blob->data);
  const Dtype *p_d_anchor_item = pBox_datap(top_blob->data);
...
void ProposalLayer::Backward_cpu(const vector<Blob*>& top,
