...
int DetectionOutput::forward(const std::vector<img_t>& bottom_blobs, std::vector<img_t>& top_blobs) const
{
  const img_t* location = bottom_blobs[0];
  const img_t* confidence = bottom_blobs[1];
  const img_t* priorbox = bottom_blobs[2];
