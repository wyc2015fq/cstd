...
void MVNLayer::Backward_cpu(const vector<Blob*>& top,
  ...
  if (this->layer_param_.mvn_param().normalize_variance()) {
    ...
    caffe_cpu_gemv(CblasNoTrans, num, dim, 1., bottom_diff,
    ...
    caffe_cpu_gemv(CblasNoTrans, num, dim, 1., top_diff,
            sum_multiplier_.cpu_data(), 0., mean_.mutable_cpu_data());
    caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, num, dim, 1, 1.,
    ...
    caffe_cpu_gemv(CblasNoTrans, num, dim, 1. / dim, top_diff,
      sum_multiplier_.cpu_data(), 0., mean_.mutable_cpu_data());
    caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, num, dim, 1, -1.,
      mean_.cpu_data(), sum_multiplier_.cpu_data(), 0.,
      temp_.mutable_cpu_data());
