...
void HDF5OutputLayer::Forward_cpu(const vector<Blob*>& bottom,
      const vector<Blob*>& top) {
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    caffe_copy(data_datum_dim, &pBox_datap(bottom_blob->data)[i * data_datum_dim],
        &data_blob_.mutable_cpu_data()[i * data_datum_dim]);
    caffe_copy(label_datum_dim, &pBox_datap(top_blob->data)[i * label_datum_dim],
...
void HDF5OutputLayer::Backward_cpu(const vector<Blob*>& top,
