...
static void sconv5x5s1(int h, int w, const float* src, int src_step, int srccn, float* dst, int dst_step, int dstcn, const float* kernel, const float* bias)
{
  int outw = w-4;
  int outh = h-4;

  #pragma omp parallel for
  for (int p = 0; p < dstcn; p++) {
    float* outptr = dst + p * outh*dst_step;
    const float bias0 = bias ? bias[p] : 0.f;
    FILL(outh*outw, outptr, bias0);

    for (int q = 0; q < srccn; q++) {
      float* outptr = dst + p * outh*dst_step;
      float* outptr2 = outptr + dst_step;

      const float* img0 = src + (q+0)*h*src_step;

      const float* kernel0 = kernel + p * srccn * 25  + q * 25;

      const float* r0 = img0;
      const float* r1 = img0 + src_step;
      const float* r2 = img0 + src_step * 2;
      const float* r3 = img0 + src_step * 3;
      const float* r4 = img0 + src_step * 4;
      const float* r5 = img0 + src_step * 5;
...
static void conv5x5s2_neon(const img_t* bottom_blob, img_t* top_blob, const float* _kernel, const float* _bias)
{
  int w = bottom_blob->w;
  int h = bottom_blob->h;
  int srccn = pBox_channels(bottom_blob);

  int outw = top_blob->w;
  int outh = top_blob->h;
  int dstcn = pBox_channels(top_blob);
