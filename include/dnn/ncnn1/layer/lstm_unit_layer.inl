...
void LSTMUnitLayer::Forward_cpu(const vector<Blob*>& bottom,
    const vector<Blob*>& top) {
  const int num = bottom[0]->shape(1);
  const int x_dim = hidden_dim_ * 4;
  const Dtype* C_prev = pBox_datap(bottom_blob->data);
  const Dtype* X = pBox_datap(top_blob->data);
...
void LSTMUnitLayer::Backward_cpu(const vector<Blob*>& top,
