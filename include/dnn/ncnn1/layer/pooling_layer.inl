...
static void pooling2x2s2_max(const img_t* bottom_data, img_t* top_blob)
{
  int w = bottom_data->w;
  int h = bottom_data->h;
  int q, i, inch = pBox_channels(bottom_data);
  ...
  for (q = 0; q < inch; q++) {
    const float* img0 = pBox_channel_p(bottom_data, q);
...
static void pooling3x3s2_max(const img_t* bottom_data, img_t* top_blob)
{
  int w = bottom_data->w;
  int h = bottom_data->h;
  int q, i, inch = pBox_channels(bottom_data);
...
static int pooling_forward(const img_t* bottom_data, img_t* top_blob, Method pool,int kernel_h,int kernel_w,int stride_h,int stride_w,int pad,int global)
{
  ...
  const img_t* bottom_blob_bordered = bottom_data;
...
static int Pooling(enum LayerOp op, const struct Layer* layer, struct Net* net)
{
...
static int PoolingLayer(enum LayerOp op, const struct Layer* layer, struct Net* net) {
...
//  LOG(INFO)<<"this is the end in pool_setup";
  ...
  channels_ = pBox_channels(bottom_blob->data);
...
//          LOG(INFO) << "defp[" <<c1 <<"]: " << defp[c1];
  ...
  case LO_Forward:
    {
      pBox_create(top_blob->data, output_w, output_h, channels_, bottom_blob->data->f);
      const Dtype* bottom_data = pBox_datap(bottom_blob->data);
      Dtype* top_data = pBox_datap(top_blob->data);
      const int top_count = pBox_count(top_blob->data);
      int* mask = NULL;
      ...
      case Method_MAX:
        // Initialize
        pBox_ReshapeLike(top_blob->im, top_blob->data);
        mask = (int*)pBox_datap(top_blob->im);
  ...
  case LO_Backward:
    {
      ...
      blas_set(pBox_count(bottom_blob->diff), (0), bottom_diff);
