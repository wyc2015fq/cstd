...
void ScaleLayer::Forward_cpu(
  ...
  if (bottom[0] == top[0]) {
    ...
    caffe_copy(bottom[0]->count(), pBox_datap(bottom_blob->data),
  ...
  if (this->layer_param_.scale_param().has_min_value()) {
    for (int d = 0; d < scale_dim_; d++) {
      scale_data[d] = std::max(scale_data[d], this->layer_param_.scale_param().min_value());
    }
  }
  if (this->layer_param_.scale_param().has_max_value()) {
    for (int d = 0; d < scale_dim_; d++) {
      scale_data[d] = std::min(scale_data[d], this->layer_param_.scale_param().max_value());
    }
  }
...
void ScaleLayer::Backward_cpu(const vector<Blob*>& top,
