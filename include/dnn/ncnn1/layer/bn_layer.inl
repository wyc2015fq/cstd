...
#include "caffe/layer.hpp"
  ...
  void BNLayer::Forward_cpu(const vector<Blob*>& bottom,
                                   const vector<Blob*>& top) {
    const Dtype* const_bottom_data = pBox_datap(bottom_blob->data);
    ...
    if (frozen_ || this->phase_ == TEST) {
      ...
      caffe_cpu_gemv(CblasNoTrans, num_ * channels_, height_ * width_,
                            ...
                            spatial_sum_multiplier_.cpu_data(), Dtype(0),
                            spatial_statistic_.mutable_cpu_data());
      caffe_cpu_gemv(CblasTrans, num_, channels_, Dtype(1) / num_,
  ...
  void BNLayer::Backward_cpu(const vector<Blob*>& top,
    ...
    if (this->param_propagate_down_[0]) {
      ...
      caffe_cpu_gemv(CblasNoTrans, num_ * channels_, height_ * width_,
                            ...
                            spatial_sum_multiplier_.cpu_data(), Dtype(0),
                            spatial_statistic_.mutable_cpu_data());
      caffe_cpu_gemv(CblasTrans, num_, channels_, Dtype(1),
                            spatial_statistic_.cpu_data(), batch_sum_multiplier_.cpu_data(),
                            Dtype(1), scale_diff);
    }
