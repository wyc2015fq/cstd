...
#include "caffe/custom_layers.hpp"
  ...
  void TransformerLayer::Forward_cpu(const vector<Blob*>& bottom,
    const vector<Blob*>& top) {
    const Dtype* bottom_data = pBox_datap(bottom_blob->data);
    Dtype* top_data = top[0]->mutable_cpu_data();
    const Dtype* theta_data = pBox_datap(top_blob->data);
    ...
    caffe_set(top[0]->count(), 0, top_data);
    for (int n = 0; n < num; n++) {
      /*theta_data += 6 * n;
      ...
      CoordinateSource_data -= n * 3 * spatial_dim;*/
      caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, 2, CoordinateTarget.shape(2), 3,
  ...
  void TransformerLayer::Backward_cpu(const vector<Blob*>& top,
    ...
    caffe_set(bottom[0]->count(), 0, data_diff);
    caffe_set(CoordinateSource.count(), 0, CoordinateSource_diff);
    for (int n = 0; n < num; n++) {
      for (int i = 0; i < bottom[0]->shape(2); i++) {
        for (int j = 0; j < bottom[0]->shape(3); j++) {
          Dtype x = CoordinateSource_data[CoordinateSource.offset(n, 0, i, j)] * bottom[0]->shape(2) / 2 + (Dtype)bottom[0]->shape(2) / 2;
            ...
            for (int c = 0; c < bottom[0]->shape(1); c++) {
              for (int xx = floor(x); xx <= ceil(x); xx++) {
                for (int yy = floor(y); yy <= ceil(y); yy++) {
                  ...
                  CoordinateSource_diff[CoordinateSource.offset(n, 0, i, j)] += top_diff[top[0]->offset(n, c, i, j)] * bottom[0]->data_at(n, c, xx, yy) * caffe_sign(xx - x) * (1 - abs(y - yy)) * (Dtype)bottom[0]->shape(2) / 2;
                  CoordinateSource_diff[CoordinateSource.offset(n, 1, i, j)] += top_diff[top[0]->offset(n, c, i, j)] * bottom[0]->data_at(n, c, xx, yy) * (1 - abs(x - xx)) * caffe_sign(yy - y) * (Dtype)bottom[0]->shape(3) / 2;
                }
              }
            }
          }
      ...
      caffe_cpu_gemm(CblasNoTrans, CblasTrans, 2, 3, CoordinateTarget.shape(2),
