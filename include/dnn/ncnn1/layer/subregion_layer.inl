...
void SubRegionLayer::Forward_cpu(const vector<Blob*>& bottom,
      const vector<Blob*>& top) {
  const Dtype* bottom_data = pBox_datap(bottom_blob->data);
  const Dtype* point_data = pBox_datap(top_blob->data);
...
void SubRegionLayer::Backward_cpu(const vector<Blob*>& top,
