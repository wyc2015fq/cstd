...
{
  min_sizes = pd.get(0, img_t());
  max_sizes = pd.get(1, img_t());
  aspect_ratios = pd.get(2, img_t());
...
int PriorBox::forward(const std::vector<img_t>& bottom_blobs, std::vector<img_t>& top_blobs) const
{
