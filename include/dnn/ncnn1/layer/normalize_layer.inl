...
void NormalizeLayer::LayerSetUp(
...
void NormalizeLayer::Forward_cpu(const vector<Blob*>& bottom,
  ...
  if (normalize_type_ == "L2") {
    caffe_sqr(num*channels*spatial_dim, bottom_data, square_data);
    for (int n = 0; n < num; n++) {
    ...
    caffe_abs(num*channels*spatial_dim, bottom_data, square_data);
    for (int n = 0; n < num; n++) {
      for (int s = 0; s < spatial_dim; s++) {
        norm_data[n*spatial_dim +s] = Dtype(0);
        for (int c = 0; c < channels; c++) {
...
void NormalizeLayer::Backward_cpu(const vector<Blob*>& top,
