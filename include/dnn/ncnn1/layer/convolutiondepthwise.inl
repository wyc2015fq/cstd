...
int ConvolutionDepthWise::forward(const img_t* bottom_blob, img_t* top_blob) const
{
  if (group == 1) {
  ...
  if (channels == group && group == num_output) {
    ...
    for (int g = 0; g < group; g++) {
      float* outptr = pBox_channel_p(top_blob, g);
      const float* kptr = weight_data + maxk * g;
      const img_t m = bottom_blob_bordered.channel(g);
  ...
  for (int g = 0; g < group; g++) {
    for (int p = 0; p < num_output_g; p++) {
      float* outptr = pBox_channel_p(top_blob, g * num_output_g + p);
      ...
      for (int i = 0; i < outh; i++) {
        for (int j = 0; j < outw; j++) {
          ...
          for (int q = 0; q < channels_g; q++) {
            const img_t m = bottom_blob_bordered.channel(channels_g * g + q);
            const float* sptr = m.data + m.w * i * stride + j * stride;
