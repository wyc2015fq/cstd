...
void AdamSolver::AdamPreSolve() {
  ...
  for (int i = 0; i < net_params.size(); ++i) {
    ...
    this->history_.push_back(
            shared_ptr<Blob >(new Blob(shape)));
  }
}
...
void AdamSolver::ComputeUpdateValue(int param_id, Dtype rate) {
  const vector<Blob*>& net_params = this->net_->learnable_params();
  ...
  Blob* val_m = this->history_[param_id].get();
  Blob* val_v = this->history_[param_id + update_history_offset].get();
  Blob* val_t = this->temp_[param_id].get();
