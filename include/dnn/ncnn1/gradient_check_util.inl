...
class GradientChecker {
 public:
  ...
  void CheckGradientExhaustive(Layer* layer,
...
void GradientChecker::CheckGradientSingle(Layer* layer,
  ...
  for (int i = 0; i < layer->blobs().size(); ++i) {
    ...
    caffe_set(blob->count(), static_cast(0), blob->mutable_cpu_diff());
  ...
  for (int blob_id = 0; blob_id < blobs_to_check.size(); ++blob_id) {
    ...
    computed_gradient_blobs[blob_id].reset(new Blob());
...
void GradientChecker::CheckGradientNet(
    const Net& net, const vector<Blob*>& input) {
  const vector<shared_ptr<Layer > >& layers = net.layers();
...
Dtype GradientChecker::GetObjAndGradient(const Layer& layer,
    const vector<Blob*>& top, int top_id, int top_data_id) {
  Dtype loss = 0;
  if (top_id < 0) {
