...
static int detect_fasterrcnn(const cv::Mat& bgr, std::vector<Object>& objects)
{
  ...
  for (int i = 0; i < rois.c; i++) {
    ...
    // clip
    obj_x1 = MAX(MIN(obj_x1, (float)(bgr.cols - 1)), 0.f);
    obj_y1 = MAX(MIN(obj_y1, (float)(bgr.rows - 1)), 0.f);
    obj_x2 = MAX(MIN(obj_x2, (float)(bgr.cols - 1)), 0.f);
    obj_y2 = MAX(MIN(obj_y2, (float)(bgr.rows - 1)), 0.f);
