...
class GradientBasedSolverTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  ...
  shared_ptr<SGDSolver > solver_;
#ifdef USE_NCCL
  shared_ptr<NCCL > nccl_;
#endif
  int seed_;
...
#ifdef USE_NCCL
      this->nccl_.reset(new NCCL(this->solver_));
  ...
  void ComputeLeastSquaresUpdate(const Dtype learning_rate,
      const Dtype weight_decay, const Dtype momentum, const int num_iters,
      vector<shared_ptr<Blob > >* updated_params) {
    const int N = num_;
    ...
    for (int i = 0; i < num_param_blobs; ++i) {
      (*updated_params)[i].reset(new Blob());
    }
    Blob& updated_weights = *(*updated_params)[0];
    updated_weights.ReshapeLike(weights);
    Blob& updated_bias = *(*updated_params)[1];
    ...
    for (int i = 0; i <= D; ++i) {
      ...
      const vector<shared_ptr<Blob > >& history = solver_->history();
  ...
  void CheckLeastSquaresUpdate(
      const vector<shared_ptr<Blob > >& updated_params) {
    const int D = channels_ * height_ * width_;

    const Blob& updated_weights = *updated_params[0];
    const Blob& updated_bias = *updated_params[1];
  ...
  void TestSnapshot(const Dtype learning_rate = 1.0,
    ...
    vector<shared_ptr<Blob > > param_copies;
    const vector<Blob*>& orig_params =
    ...
    vector<shared_ptr<Blob > > history_copies;
    const vector<shared_ptr<Blob > >& orig_history = solver_->history();
    ...
    const vector<Blob*>& params = solver_->net()->learnable_params();
...
class SGDSolverTest : public GradientBasedSolverTest<TypeParam> {
 ...
 protected:
  virtual void InitSolver(const SolverParameter& param) {
    this->solver_.reset(new SGDSolver(param));
  }
};
...
class AdaGradSolverTest : public GradientBasedSolverTest<TypeParam> {
 ...
 protected:
  virtual void InitSolver(const SolverParameter& param) {
    this->solver_.reset(new AdaGradSolver(param));
  }
};
...
class NesterovSolverTest : public GradientBasedSolverTest<TypeParam> {
 ...
 protected:
  virtual void InitSolver(const SolverParameter& param) {
    this->solver_.reset(new NesterovSolver(param));
  }
};
...
class AdaDeltaSolverTest : public GradientBasedSolverTest<TypeParam> {
 ...
 protected:
  virtual void InitSolver(const SolverParameter& param) {
    this->solver_.reset(new AdaDeltaSolver(param));
  }
};
...
class AdamSolverTest : public GradientBasedSolverTest<TypeParam> {
 ...
 protected:
  virtual void InitSolver(const SolverParameter& param) {
    ...
    this->solver_.reset(new AdamSolver(new_param));
  }
};
...
class RMSPropSolverTest : public GradientBasedSolverTest<TypeParam> {
 ...
 protected:
  virtual void InitSolver(const SolverParameter& param) {
    ...
    this->solver_.reset(new RMSPropSolver(new_param));
  }
};
