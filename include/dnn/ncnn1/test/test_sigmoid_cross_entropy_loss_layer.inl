 ...
 protected:
  SigmoidCrossEntropyLossLayerTest()
      : blob_bottom_data_(new Blob(10, 5, 1, 1)),
        blob_bottom_targets_(new Blob(10, 5, 1, 1)),
        blob_top_loss_(new Blob()) {
    // Fill the data vector
    FillerParameter data_filler_param;
    data_filler_param.set_std(1);
    GaussianFiller data_filler(data_filler_param);
    ...
    UniformFiller targets_filler(targets_filler_param);
  ...
  void TestForward() {
    ...
    for (int i = 0; i < 100; ++i) {
      ...
      SigmoidCrossEntropyLossLayer layer(layer_param);
  ...
  Blob* const blob_bottom_targets_;
