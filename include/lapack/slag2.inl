...
/* Subroutine */ static int slag2_(real *a, integer *lda, real *b, integer *ldb, 
...
/* Computing MAX */
    r__5 = (r__1 = a[a_dim1 + 1], SABS(r__1)) + (r__2 = a[a_dim1 + 2], SABS(
            r__2)), r__6 = (r__3 = a[(a_dim1 << 1) + 1], SABS(r__3)) + (r__4 =
             a[(a_dim1 << 1) + 2], SABS(r__4)), r__5 = MAX(r__5,r__6);
...
/*     Perturb B if necessary to insure non-singularity */
    ...
    b22 = b[(b_dim1 << 1) + 2];
/* Computing MAX */
    r__1 = SABS(b11), r__2 = SABS(b12), r__1 = MAX(r__1,r__2), r__2 = SABS(
            b22), r__1 = MAX(r__1,r__2);
    bmin = rtmin * dmax(r__1,rtmin);
    if (SABS(b11) < bmin) {
        b11 = (real)r_sign(&bmin, &b11);
    }
    if (SABS(b22) < bmin) {
        b22 = (real)r_sign(&bmin, &b22);
    }

/*     Scale B */

/* Computing MAX */
    r__1 = SABS(b11), r__2 = SABS(b12) + SABS(b22), r__1 = MAX(r__1,r__2);
    bnorm = dmax(r__1,*safmin);
/* Computing MAX */
    r__1 = SABS(b11), r__2 = SABS(b22);
...
/*     ( AS is A shifted by -SHIFT*B ) */
    ...
    qq = ss * as12;
    if ((r__1 = pp * rtmin, SABS(r__1)) >= 1.f) {
/* Computing 2nd power */
        r__1 = rtmin * pp;
        discr = r__1 * r__1 + qq * *safmin;
        r__ = (real)sqrt((SABS(discr))) * rtmax;
    } else {
/* Computing 2nd power */
        r__1 = pp;
        if (r__1 * r__1 + SABS(qq) <= *safmin) {
/* Computing 2nd power */
            r__1 = rtmax * pp;
            discr = r__1 * r__1 + qq * safmax;
            r__ = (real)sqrt((SABS(discr))) * rtmin;
        } else {
/* Computing 2nd power */
            r__1 = pp;
            discr = r__1 * r__1 + qq;
            r__ = (real)sqrt((SABS(discr)));
        }
    }

/*     Note: the test of R in the following IF is to cover the case when */
/*           DISCR is small1 and negative and is flushed to zero during */
...
/* Computing MAX */
        r__1 = SABS(wsmall);
        if (SABS(wbig) * .5f > dmax(r__1,*safmin)) {
...
/* Computing MAX */
        ...
        r__1 = MAX(*safmin,c1), r__2 = (SABS(*wr2) * c2 + c3) * 
