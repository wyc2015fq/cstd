...
/* Subroutine */ static int csteqr_(char *compz, integer *n, real *d__, real *e, 
        scomplex *z__, integer *ldz, real *work, integer *info, ftnlen 
        compz_len)
{
...
static real c_b41 = 1.f;
    ...
    static integer lend, jtot;
     /* Subroutine */ int slae2_(real *, real *, real *, real *, real *)
            ;
    ...
    static real ssfmax;
     doublereal slanst_(char *, integer *, real *, real *, ftnlen);
     /* Subroutine */ int slasrt_(char *, integer *, real *, integer *, 
            ftnlen);
...
/* Computing 2nd power */
    ...
    safmax = 1.f / safmin;
    ssfmax = (real)sqrt(safmax) / 3.f;
    ssfmin = (real)sqrt(safmin) / eps2;
...
L10:
    ...
    if (l1 <= nm1) {
        i__1 = nm1;
        for (m = l1; m <= i__1; ++m) {
            tst = (r__1 = e[m], SABS(r__1));
            if (tst == 0.f) {
                goto L30;
            }
            if (tst <= (real)sqrt((r__1 = d__[m], SABS(r__1))) * sqrt((r__2 = d__[m 
                    + 1], SABS(r__2))) * eps) {
                e[m] = 0.f;
                goto L30;
            }
...
/*     Scale submatrix in rows and columns L to LEND */
    ...
    i__1 = lend - l + 1;
    anorm = (real)slanst_("I", &i__1, &d__[l], &e[l], (ftnlen)1);
    iscale = 0;
    if (anorm == 0.f) {
        goto L10;
    }
...
/*     Choose between QL and QR iteration */
    ...
    if ((r__1 = d__[lend], SABS(r__1)) < (r__2 = d__[l], SABS(r__2))) {
        lend = lsv;
        l = lendsv;
    }
...
/*        Look for small1 subdiagonal element. */
...
L40:
        if (l != lend) {
            ...
            for (m = l; m <= i__1; ++m) {
/* Computing 2nd power */
                r__2 = (r__1 = e[m], SABS(r__1));
                tst = r__2 * r__2;
                if (tst <= eps2 * (r__1 = d__[m], SABS(r__1)) * (r__2 = d__[m 
                        + 1], SABS(r__2)) + safmin) {
                    goto L60;
                }
/* L50: */
...
/*        Form shift. */
        ...
        g = (d__[l + 1] - p) / (e[l] * 2.f);
        r__ = (real)slapy2_(&g, &c_b41);
        g = (real)(d__[m] - p + e[l] / (g + r_sign(&r__, &g)));
...
/*        Look for small1 superdiagonal element. */
...
/*        Form shift. */
        ...
        r__ = (real)slapy2_(&g, &c_b41);
        g = (real)(d__[m] - p + e[l - 1] / (g + r_sign(&r__, &g)));
