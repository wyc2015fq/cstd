...
/* Subroutine */ static int slatrs_(char *uplo, char *trans, char *diag, char *
...
/*     Determine machine dependent parameters to control overflow. */
    ...
    smlnum = (real)(slamch_("Safe minimum", (ftnlen)12) / slamch_("Precision", (ftnlen)9));
    bignum = 1.f / smlnum;
    *scale = 1.f;
...
/*           A is upper triangular. */
            ...
            for (j = 1; j <= i__1; ++j) {
                i__2 = j - 1;
                cnorm[j] = (real)sasum_(&i__2, &a[j * a_dim1 + 1], &c__1);
/* L10: */
            }
        } else {
...
/*           A is lower triangular. */
            ...
            for (j = 1; j <= i__1; ++j) {
                i__2 = *n - j;
                cnorm[j] = (real)sasum_(&i__2, &a[j + 1 + j * a_dim1], &c__1);
/* L20: */
            }
            cnorm[*n] = 0.f;
        }
...
/*              M(j) = G(j-1) / ABS(A(j,j)) */
                ...
                tjj = (r__1 = a[j + j * a_dim1], SABS(r__1));
/* Computing MIN */
...
/*                 call SDOT to perform the dot product. */
                    ...
                    if (upper) {
                        i__3 = j - 1;
                        sumj = (real)sdot_(&i__3, &a[j * a_dim1 + 1], &c__1, &x[1], 
                                &c__1);
                    } else if (j < *n) {
                        i__3 = *n - j;
                        sumj = (real)sdot_(&i__3, &a[j + 1 + j * a_dim1], &c__1, &x[
                                j + 1], &c__1);
                    }
                } else {
...
/* Computing MAX */
                r__2 = xmax, r__3 = (r__1 = x[j], SABS(r__1));
                xmax = dmax(r__2,r__3);
/* L140: */
            }
        }
