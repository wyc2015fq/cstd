...
/* Subroutine */ static int shgeqz_(char *job, char *compq, char *compz, integer *n, 
        ...
        ftnlen job_len, ftnlen compq_len, ftnlen compz_len)
{
    ...
    static real ulp, sqr, szi, szr, ad11l, ad12l, ad21l, ad22l, ad32l, wabs, 
            atol, btol, temp;
     /* Subroutine */ int srot_(integer *, real *, integer *, real *, 
    ...
    static logical ilazr2;
     doublereal slapy2_(real *, real *), slapy3_(real *, real *, real *)
            ;
     /* Subroutine */ int slasv2_(real *, real *, real *, real *, real *
            , real *, real *, real *, real *);
    static real ascale, bscale;
     doublereal slamch_(char *, ftnlen);
    static real safmin;
     /* Subroutine */ int slarfg_(integer *, real *, real *, integer *, 
            real *);
    static real safmax;
    ...
    static integer icompq, ilastm;
     doublereal slanhs_(char *, integer *, real *, integer *, real *, 
            ftnlen);
     /* Subroutine */ int slartg_(real *, real *, real *, real *, real *
            );
    static integer ischur;
     /* Subroutine */ int slaset_(char *, integer *, integer *, real *, 
...
/*  scomplex generalized eigenvalues (see the description of the argument */
/*  JOB.) */
...
/*          Note that the (real or scomplex) values */
...
/*     Machine Constants */
    ...
    safmax = 1.f / safmin;
    ulp = (real)slamch_("E", (ftnlen)1) * slamch_("B", (ftnlen)1);
...
/*           Special case: j=ILAST */
            ...
            if ((r__1 = a[ilast + (ilast - 1) * a_dim1], SABS(r__1)) <= atol) 
                    {
        ...
        if ((r__1 = b[ilast + ilast * b_dim1], SABS(r__1)) <= btol) {
...
/*           Test 1: for A(j,j-1)=0 or j=ILO */
            ...
            if (j == *ilo) {
                ...
                if ((r__1 = a[j + (j - 1) * a_dim1], SABS(r__1)) <= atol) {
                    a[j + (j - 1) * a_dim1] = 0.f;
                    ilazro = TRUE_;
                } else {
                    ilazro = FALSE_;
...
/*           Test 2: for B(j,j)=0 */
            ...
            if ((r__1 = b[j + j * b_dim1], SABS(r__1)) < btol) {
                b[j + j * b_dim1] = 0.f;

/*              Test 1a: Check for 2 consecutive small1 subdiagonals in A */

                ilazr2 = FALSE_;
                if (! ilazro) {
                    temp = (r__1 = a[j + (j - 1) * a_dim1], SABS(r__1));
                    temp2 = (r__1 = a[j + j * a_dim1], SABS(r__1));
                    tempr = dmax(temp,temp2);
                    if (tempr < 1.f && tempr != 0.f) {
                        temp /= tempr;
                        temp2 /= tempr;
                    }
                    if (temp * (ascale * (r__1 = a[j + 1 + j * a_dim1], SABS(
...
/*              this may have to be done repeatedly. */
                ...
                if (ilazro || ilazr2) {
                    ...
                    for (jch = j; jch <= i__3; ++jch) {
                        ...
                        ilazr2 = FALSE_;
                        if ((r__1 = b[jch + 1 + (jch + 1) * b_dim1], SABS(
                                r__1)) >= btol) {
                            if (jch + 1 >= ilast) {
                                goto L80;
                            } else {
...
/*           (Single shift only.) */
            ...
            if ((real) maxit * safmin * (r__1 = a[ilast - 1 + ilast * a_dim1],
                     SABS(r__1)) < (r__2 = b[ilast - 1 + (ilast - 1) * b_dim1]
                    , SABS(r__2))) {
...
/* Computing MAX */
/* Computing MAX */
            r__3 = 1.f, r__4 = SABS(wr), r__3 = MAX(r__3,r__4), r__4 = SABS(
                    wi);
...
/*        Fiddle with shift to avoid overflow */
        ...
        if (SABS(wr) > temp) {
/* Computing MIN */
            r__1 = scale, r__2 = temp / SABS(wr);
            scale = dmin(r__1,r__2);
        }
        s1 = scale * s1;
        wr = scale * wr;

/*        Now check for two consecutive small1 subdiagonals. */

        i__2 = ifirst + 1;
        for (j = ilast - 1; j >= i__2; --j) {
            istart = j;
            temp = (r__1 = s1 * a[j + (j - 1) * a_dim1], SABS(r__1));
            ...
            if ((r__1 = ascale * a[j + 1 + j * a_dim1] * temp, SABS(r__1)) <= 
                    ascale * atol * temp2) {
                goto L130;
            }
/* L120: */
...
/*                     ( SZ    CZ ) */
            ...
            if (SABS(c11r) + SABS(c11i) + SABS(c12) > SABS(c21) + SABS(c22r) 
                    + SABS(c22i)) {
...
/*           ( -SQ   CQ ) */
            ...
            an = SABS(a11) + SABS(a12) + SABS(a21) + SABS(a22);
            bn = SABS(b11) + SABS(b22);
            wabs = SABS(wr) + SABS(wi);
            if (s1 * an > wabs * bn) {
                cq = cz * b11;
                sqr = szr * b22;
                sqi = -szi * b22;
...
/*           Compute diagonal elements of QBZ */
            ...
            b1i = tempi * b22;
            b1a = (real)slapy2_(&b1r, &b1i);
            b2r = cq * cz * b22 + tempr * b11;
            b2i = -tempi * b11;
            b2a = (real)slapy2_(&b2r, &b2i);
...
/*              Swap rows to pivot */
                ...
                ilpivt = FALSE_;
/* Computing MAX */
                r__3 = (r__1 = b[j + 1 + (j + 1) * b_dim1], SABS(r__1)), r__4 
                        = (r__2 = b[j + 1 + (j + 2) * b_dim1], SABS(r__2));
                temp = dmax(r__3,r__4);
/* Computing MAX */
                r__3 = (r__1 = b[j + 2 + (j + 1) * b_dim1], SABS(r__1)), r__4 
                        = (r__2 = b[j + 2 + (j + 2) * b_dim1], SABS(r__2));
...
/*              Compute SCALE */
                ...
                if (SABS(w22) < SABS(u2)) {
                    scale = (r__1 = w22 / u2, SABS(r__1));
                }
                if (SABS(w11) < SABS(u1)) {
/* Computing MIN */
                    r__2 = scale, r__3 = (r__1 = w11 / u1, SABS(r__1));
...
/* Computing 2nd power */
                r__3 = u2;
                t = (real)sqrt(r__1 * r__1 + r__2 * r__2 + r__3 * r__3);
                tau = scale / t + 1.f;
                vs = -1.f / (scale + t);
                v[0] = 1.f;
                v[1] = vs * u1;
