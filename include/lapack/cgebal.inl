...
/* Subroutine */ static int cgebal_(char *job, integer *n, scomplex *a, integer *lda, 
...
/*  CGEBAL balances a general scomplex matrix A.  This involves, first, */
...
/*     Iterative loop for norm reduction */
    ...
    sfmin1 = (real)(slamch_("S", (ftnlen)1) / slamch_("P", (ftnlen)1));
    sfmax1 = 1.f / sfmin1;
    sfmin2 = sfmin1 * 8.f;
    sfmax2 = 1.f / sfmin2;
L140:
    ...
    for (i__ = k; i__ <= i__1; ++i__) {
        ...
        for (j = k; j <= i__2; ++j) {
            ...
            i__3 = j + i__ * a_dim1;
            c__ += (r__1 = a[i__3].r, SABS(r__1)) + (r__2 = (real)r_imag(&a[j + i__ 
                    * a_dim1]), SABS(r__2));
            i__3 = i__ + j * a_dim1;
            r__ += (r__1 = a[i__3].r, SABS(r__1)) + (r__2 = (real)r_imag(&a[i__ + j 
                    * a_dim1]), SABS(r__2));
L150:
            ;
        }
        ica = icamax_(&l, &a[i__ * a_dim1 + 1], &c__1);
        ca = (real)c_abs(&a[ica + i__ * a_dim1]);
        i__2 = *n - k + 1;
        ira = icamax_(&i__2, &a[i__ + k * a_dim1], lda);
        ra = (real)c_abs(&a[i__ + (ira + k - 1) * a_dim1]);
