...
/* Subroutine */ static int clascl_(char *type__, integer *kl, integer *ku, real *
        cfrom, real *cto, integer *m, integer *n, scomplex *a, integer *lda, 
...
/*  CLASCL multiplies the M by N scomplex matrix A by the real scalar */
...
L10:
    ...
    cto1 = ctoc / bignum;
    if (SABS(cfrom1) > SABS(ctoc) && ctoc != 0.f) {
        mul = smlnum;
        done = FALSE_;
        cfromc = cfrom1;
    } else if (SABS(cto1) > SABS(cfromc)) {
        mul = bignum;
        done = FALSE_;
        ctoc = cto1;
    } else {
