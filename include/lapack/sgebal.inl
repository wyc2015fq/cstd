...
/* Subroutine */ static int sgebal_(char *job, integer *n, real *a, integer *lda, 
...
L140:
    ...
    for (i__ = k; i__ <= i__1; ++i__) {
        ...
        for (j = k; j <= i__2; ++j) {
            ...
            c__ += (r__1 = a[j + i__ * a_dim1], SABS(r__1));
            r__ += (r__1 = a[i__ + j * a_dim1], SABS(r__1));
L150:
            ;
        }
        ica = isamax_(&l, &a[i__ * a_dim1 + 1], &c__1);
        ca = (r__1 = a[ica + i__ * a_dim1], SABS(r__1));
        i__2 = *n - k + 1;
        ira = isamax_(&i__2, &a[i__ + k * a_dim1], lda);
        ra = (r__1 = a[i__ + (ira + k - 1) * a_dim1], SABS(r__1));
