...
/* Subroutine */ static int slaqps_(integer *m, integer *n, integer *offset, integer 
...
/*        Update partial column norms. */
        ...
        if (rk < lastrk) {
            ...
            for (j = k + 1; j <= i__1; ++j) {
                if (vn1[j] != 0.f) {
                    temp = (r__1 = a[rk + j * a_dim1], SABS(r__1)) / vn1[j];
/* Computing MAX */
...
L40:
    if (lsticc > 0) {
        ...
        i__1 = *m - rk;
        vn1[lsticc] = (real)snrm2_(&i__1, &a[rk + 1 + lsticc * a_dim1], &c__1);
