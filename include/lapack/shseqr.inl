...
/* Subroutine */ static int shseqr_(char *job, char *compz, integer *n, integer *ilo,
...
static integer c__1 = 1;
    ...
    static logical wantz;
     ...
     doublereal slanhs_(char *, integer *, real *, integer *, real *, 
            ftnlen);
     /* Subroutine */ int slahqr_(logical *, logical *, integer *, 
...
/*          2-by-2 diagonal blocks (corresponding to scomplex conjugate */
...
/*          eigenvalues. If two eigenvalues are computed as a scomplex */
/*          conjugate pair, they are stored in consecutive elements of */
/*          WR and WI, say the i-th and (i+1)th, with WI(i) > 0 and */
/*          WI(i+1) < 0. If JOB = 'S', the eigenvalues are stored in the */
/*          same order as on the diagonal of the Schur form returned in */
/*          H, with WR(i) = H(i,i) and, if H(i:i+1,i:i+1) is a 2-by-2 */
/*          diagonal block, WI(i) = (real)sqrt(H(i+1,i)*H(i,i+1)) and */
/*          WI(i+1) = -WI(i). */
...
/*        Look for a single small1 subdiagonal element. */
        ...
        i__2 = l + 1;
        for (k = i__; k >= i__2; --k) {
            tst1 = (r__1 = h__[k - 1 + (k - 1) * h_dim1], SABS(r__1)) + (r__2 
                    = h__[k + k * h_dim1], SABS(r__2));
            if (tst1 == 0.f) {
                i__4 = i__ - l + 1;
                tst1 = (real)slanhs_("1", &i__4, &h__[l + l * h_dim1], ldh, &work[1]
                        , (ftnlen)1);
            }
/* Computing MAX */
            r__2 = ulp * tst1;
            if ((r__1 = h__[k + (k - 1) * h_dim1], SABS(r__1)) <= dmax(r__2,
                    smlnum)) {
                goto L70;
            }
/* L60: */
...
/*           Exceptional shifts. */
            ...
            for (ii = i__ - ns + 1; ii <= i__2; ++ii) {
                wr[ii] = ((r__1 = h__[ii + (ii - 1) * h_dim1], SABS(r__1)) + (
                        r__2 = h__[ii + ii * h_dim1], SABS(r__2))) * 1.5f;
                wi[ii] = 0.f;
/* L80: */
            }
        } else {
...
/*                 scomplex conjugate pair of shifts */
...
/* Computing MAX */
                    r__2 = (r__1 = vv[itemp - 1], SABS(r__1));
                    temp = 1.f / dmax(r__2,smlnum);
                    i__4 = nv + 1;
                    sscal_(&i__4, &temp, vv, &c__1);
                    absw = (real)slapy2_(&wr[j], &wi[j]);
                    temp = temp * absw * absw;
                    i__4 = nv + 2;
                    i__5 = nv + 1;
