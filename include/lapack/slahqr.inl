...
/* Subroutine */ static int slahqr_(logical *wantt, logical *wantz, integer *n, 
...
/*          WI(i) = (real)sqrt(H(i+1,i)*H(i,i+1)) and WI(i+1) = -WI(i). */
...
/*        Look for a single small1 subdiagonal element. */
        ...
        for (k = i__; k >= i__2; --k) {
            ...
            if (tst1 == 0.f) {
                i__3 = i__ - l + 1;
                tst1 = (real)slanhs_("1", &i__3, &h__[l + l * h_dim1], ldh, work, (
                        ftnlen)1);
            }
/* Computing MAX */
            r__2 = ulp * tst1;
...
/*           Exceptional shift. */
            ...
            s = (r__1 = h__[i__ + (i__ - 1) * h_dim1], SABS(r__1)) + (r__2 = 
                    h__[i__ - 1 + (i__ - 2) * h_dim1], SABS(r__2));
...
/*              Real roots: use Wilkinson's shift twice */
                ...
                disc = (real)sqrt(disc);
                ave = (h33 + h44) * .5f;
                if (SABS(h33) - SABS(h44) > 0.f) {
                    h33 = h33 * h44 - h43h34;
                    h44 = (real)(h33 / (r_sign(&disc, &ave) + ave));
                } else {
                    h44 = (real)r_sign(&disc, &ave) + ave;
                }
                h33 = h44;
                h43h34 = 0.f;
            }
...
/*           negligible. */
            ...
            tst1 = SABS(v1) * (SABS(h00) + SABS(h11) + SABS(h22));
            if (SABS(h10) * (SABS(v2) + SABS(v3)) <= ulp * tst1) {
                goto L50;
            }
/* L40: */
        }
