...
/* Subroutine */ static int strsna_(char *job, char *howmny, logical *select, 
...
static logical c_false = FALSE_;
    ...
    static real rnrm, prod1, prod2;
     doublereal snrm2_(integer *, real *, integer *);
    ...
    static real dummy[1];
     ...
     /* Subroutine */ int xerbla_(char *, integer *, ftnlen), slacon_(
    ...
    static logical somcon;
     /* Subroutine */ int slaqtr_(logical *, logical *, integer *, real 
...
/*     Quick return if possible */
    ...
    if (*n == 1) {
        ...
        if (wantsp) {
            sep[1] = (r__1 = t[t_dim1 + 1], SABS(r__1));
        }
        return 0;
    }
...
/*              Real eigenvalue. */
                ...
                prod = (real)sdot_(n, &vr[ks * vr_dim1 + 1], &c__1, &vl[ks * 
                        vl_dim1 + 1], &c__1);
                rnrm = (real)snrm2_(n, &vr[ks * vr_dim1 + 1], &c__1);
                lnrm = (real)snrm2_(n, &vl[ks * vl_dim1 + 1], &c__1);
                s[ks] = SABS(prod) / (rnrm * lnrm);
            } else {

/*              Complex eigenvalue. */

                prod1 = (real)sdot_(n, &vr[ks * vr_dim1 + 1], &c__1, &vl[ks * 
                        vl_dim1 + 1], &c__1);
                prod1 += (real)sdot_(n, &vr[(ks + 1) * vr_dim1 + 1], &c__1, &vl[(ks 
                        + 1) * vl_dim1 + 1], &c__1);
                prod2 = (real)sdot_(n, &vl[ks * vl_dim1 + 1], &c__1, &vr[(ks + 1) * 
                        vr_dim1 + 1], &c__1);
                prod2 -= (real)sdot_(n, &vl[(ks + 1) * vl_dim1 + 1], &c__1, &vr[ks *
                         vr_dim1 + 1], &c__1);
                r__1 = (real)snrm2_(n, &vr[ks * vr_dim1 + 1], &c__1);
                r__2 = (real)snrm2_(n, &vr[(ks + 1) * vr_dim1 + 1], &c__1);
                rnrm = (real)slapy2_(&r__1, &r__2);
                r__1 = (real)snrm2_(n, &vl[ks * vl_dim1 + 1], &c__1);
                r__2 = (real)snrm2_(n, &vl[(ks + 1) * vl_dim1 + 1], &c__1);
                lnrm = (real)slapy2_(&r__1, &r__2);
                cond = (real)slapy2_(&prod1, &prod2) / (rnrm * lnrm);
                s[ks] = cond;
                s[ks + 1] = cond;
            }
        }
...
/*                 with negative imaginary  part. */
                    ...
                    mu = (real)(sqrt((r__1 = work[(work_dim1 << 1) + 1], SABS(r__1)))
                             * sqrt((r__2 = work[work_dim1 + 2], SABS(r__2))));
                    delta = (real)slapy2_(&mu, &work[work_dim1 + 2]);
                    cs = mu / delta;
