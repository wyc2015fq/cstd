...
/* Subroutine */ static int strevc_(char *side, char *howmny, logical *select, 
...
static logical c_true = TRUE_;
    ...
    static real scale;
     ...
     /* Subroutine */ int sscal_(integer *, real *, real *, integer *);
    static real remax;
    ...
    static logical somev;
     /* Subroutine */ int scopy_(integer *, real *, integer *, real *, 
            integer *);
    static real xnorm;
     /* Subroutine */ int saxpy_(integer *, real *, real *, integer *, 
...
/*  diagonal block is a scomplex conjugate pair of eigenvalues and */
...
/*                           scomplex-conjugate eigenvalues of T, then */
/*                           VL(i)+sqrt(-1)*VL(i+1) is the scomplex */
...
/*                           is a 2-by-2 block whose eigenvalues are */
/*                           scomplex-conjugate eigenvalues of T, then */
/*                           VR(i)+sqrt(-1)*VR(i+1) is the scomplex */
/*                           eigenvector corresponding to the eigenvalue */
/*                           with positive real part. */
...
/*  magnitude has magnitude 1; here the magnitude of a scomplex number */
...
/*     Set the constants to control overflow. */
    ...
    ovfl = 1.f / unfl;
    slabad_(&unfl, &ovfl);
    ulp = (real)slamch_("Precision", (ftnlen)9);
    smlnum = unfl * (*n / ulp);
...
/*     part of T to control overflow in triangular solver. */
    ...
    for (j = 2; j <= i__1; ++j) {
        ...
        for (i__ = 1; i__ <= i__2; ++i__) {
            work[j] += (r__1 = t[i__ + j * t_dim1], SABS(r__1));
/* L20: */
        }
/* L30: */
    }
...
/*           Compute the KI-th eigenvalue (WR,WI). */
            ...
            wi = 0.f;
            if (ip != 0) {
                wi = (real)(sqrt((r__1 = t[ki + (ki - 1) * t_dim1], SABS(r__1))) * 
                        sqrt((r__2 = t[ki - 1 + ki * t_dim1], SABS(r__2))));
            }
/* Computing MAX */
            r__1 = ulp * (SABS(wr) + SABS(wi));
...
/*              Copy the vector x or Q*x to VR and normalize. */
                ...
                if (! over) {
                    ...
                    remax = 1.f / (r__1 = vr[ii + is * vr_dim1], SABS(r__1));
...
/* L70: */
                    ...
                    remax = 1.f / (r__1 = vr[ii + ki * vr_dim1], SABS(r__1));
...
/*                [ (T(KI,KI-1)   T(KI,KI)   )               ] */
                ...
                if ((r__1 = t[ki - 1 + ki * t_dim1], SABS(r__1)) >= (r__2 = t[
                        ki + (ki - 1) * t_dim1], SABS(r__2))) {
                    work[ki - 1 + *n] = 1.f;
...
/* Computing MAX */
                        r__3 = emax, r__4 = (r__1 = vr[k + (is - 1) * vr_dim1]
                                , SABS(r__1)) + (r__2 = vr[k + is * vr_dim1], 
                                SABS(r__2));
                        emax = dmax(r__3,r__4);
/* L100: */
                    }
...
/* Computing MAX */
                        r__3 = emax, r__4 = (r__1 = vr[k + (ki - 1) * vr_dim1]
                                , SABS(r__1)) + (r__2 = vr[k + ki * vr_dim1], 
                                SABS(r__2));
                        emax = dmax(r__3,r__4);
/* L120: */
                    }
...
/*                    the right-hand side. */
                        ...
                        i__3 = j - ki - 1;
                        work[j + *n] -= (real)sdot_(&i__3, &t[ki + 1 + j * t_dim1], 
...
/* Computing MAX */
                        ...
                        i__3 = j - ki - 1;
                        work[j + 1 + *n] -= (real)sdot_(&i__3, &t[ki + 1 + (j + 1) *
...
/* Computing MAX */
                        r__3 = (r__1 = work[j + *n], SABS(r__1)), r__4 = (
                                r__2 = work[j + 1 + *n], SABS(r__2)), r__3 = 
                                MAX(r__3,r__4);
                        vmax = dmax(r__3,vmax);
                        vcrit = bignum / vmax;
...
/*              Copy the vector x or Q*x to VL and normalize. */
                ...
                if (! over) {
                    ...
                    ii = isamax_(&i__2, &vl[ki + is * vl_dim1], &c__1) + ki - 
                            1;
                    remax = 1.f / (r__1 = vl[ii + is * vl_dim1], SABS(r__1));
                    i__2 = *n - ki + 1;
...
/* L180: */
                    ...
                    remax = 1.f / (r__1 = vl[ii + ki * vl_dim1], SABS(r__1));
...
/*                 ((T(KI+1,KI) T(KI+1,KI+1))                ) */
                ...
                if ((r__1 = t[ki + (ki + 1) * t_dim1], SABS(r__1)) >= (r__2 = 
                        t[ki + 1 + ki * t_dim1], SABS(r__2))) {
...
/*                    forming the right-hand side elements. */
                        ...
                        i__3 = j - ki - 2;
                        work[j + *n] -= (real)sdot_(&i__3, &t[ki + 2 + j * t_dim1], 
                                &c__1, &work[ki + 2 + *n], &c__1);
                        i__3 = j - ki - 2;
                        work[j + n2] -= (real)sdot_(&i__3, &t[ki + 2 + j * t_dim1], 
...
/* Computing MAX */
                        r__3 = (r__1 = work[j + *n], SABS(r__1)), r__4 = (
                                r__2 = work[j + n2], SABS(r__2)), r__3 = MAX(
                                r__3,r__4);
                        vmax = dmax(r__3,vmax);
                        vcrit = bignum / vmax;
...
/* Computing MAX */
                        ...
                        i__3 = j - ki - 2;
                        work[j + 1 + *n] -= (real)sdot_(&i__3, &t[ki + 2 + (j + 1) *
                                 t_dim1], &c__1, &work[ki + 2 + *n], &c__1);

                        i__3 = j - ki - 2;
                        work[j + 1 + n2] -= (real)sdot_(&i__3, &t[ki + 2 + (j + 1) *
...
/*                    Scale if necessary */
                        ...
                        work[j + 1 + n2] = x[3];
/* Computing MAX */
                        r__1 = SABS(x[0]), r__2 = SABS(x[2]), r__1 = MAX(r__1,
                                r__2), r__2 = SABS(x[1]), r__1 = MAX(r__1,
                                r__2), r__2 = SABS(x[3]), r__1 = MAX(r__1,
                                r__2);
                        vmax = dmax(r__1,vmax);
                        vcrit = bignum / vmax;
