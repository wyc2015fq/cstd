...
/* Subroutine */ static int clacon_(integer *n, scomplex *v, scomplex *x, real *est, 
        integer *kase)
{
...
static integer c__1 = 1;
    ...
    static integer jlast;
     /* Subroutine */ int ccopy_(integer *, scomplex *, integer *, 
            scomplex *, integer *);
     integer icmax1_(integer *, scomplex *, integer *);
     doublereal scsum1_(integer *, scomplex *, integer *), slamch_(char *
            , ftnlen);
...
/*        ... QUIT */
    ...
    *est = (real)scsum1_(n, &x[1], &c__1);

    i__1 = *n;
    for (i__ = 1; i__ <= i__1; ++i__) {
        absxi = (real)c_abs(&x[i__]);
        if (absxi > safmin) {
            i__2 = i__;
            i__3 = i__;
            r__1 = x[i__3].r / absxi;
            r__2 = (real)r_imag(&x[i__]) / absxi;
...
L90:
    jlast = j;
    j = icmax1_(n, &x[1], &c__1);
    if (c_abs(&x[jlast]) != (real)c_abs(&x[j]) && iter < 5) {
        ++iter;
        goto L50;
    }
...
L120:
    temp = (real)scsum1_(n, &x[1], &c__1) / (real) (*n * 3) * 2.f;
    if (temp > *est) {
        ccopy_(n, &x[1], &c__1, &v[1], &c__1);
        *est = temp;
    }
