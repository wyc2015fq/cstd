...
/* Subroutine */ static int ztgevc_(char *side, char *howmny, logical *select, 
        integer *n, dcomplex *a, integer *lda, dcomplex *b, integer 
        *ldb, dcomplex *vl, integer *ldvl, dcomplex *vr, integer *
        ldvr, integer *mm, integer *m, dcomplex *work, doublereal *rwork,
...
/*              Scale to avoid underflow */
                ...
                lsa = ABS(sbeta) >= safmin && ABS(acoeff) < small1;
                lsb = (d__1 = salpha.r, ABS(d__1)) + (d__2 = d_imag(&salpha), 
                        ABS(d__2)) >= safmin && (d__3 = bcoeff.r, ABS(d__3)) 
                        + (d__4 = d_imag(&bcoeff), ABS(d__4)) < small1;

                scale = 1.;
                if (lsa) {
                    scale = small1 / ABS(sbeta) * MIN(anorm,big);
                }
                if (lsb) {
/* Computing MAX */
                    d__3 = scale, d__4 = small1 / ((d__1 = salpha.r, ABS(d__1))
