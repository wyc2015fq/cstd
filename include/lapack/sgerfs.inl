...
/* Subroutine */ static int sgerfs_(char *trans, integer *n, integer *nrhs, real *a, 
...
/*        numerator and denominator before dividing. */
        ...
        for (i__ = 1; i__ <= i__2; ++i__) {
            work[i__] = (r__1 = b[i__ + j * b_dim1], SABS(r__1));
/* L30: */
        }
...
/*        Compute ABS(op(A))*ABS(X) + ABS(B). */
        ...
        if (notran) {
            i__2 = *n;
            for (k = 1; k <= i__2; ++k) {
                xk = (r__1 = x[k + j * x_dim1], SABS(r__1));
                i__3 = *n;
                for (i__ = 1; i__ <= i__3; ++i__) {
                    work[i__] += (r__1 = a[i__ + k * a_dim1], SABS(r__1)) * 
                            xk;
/* L40: */
                }
/* L50: */
            ...
            for (k = 1; k <= i__2; ++k) {
                ...
                i__3 = *n;
                for (i__ = 1; i__ <= i__3; ++i__) {
                    s += (r__1 = a[i__ + k * a_dim1], SABS(r__1)) * (r__2 = x[
                            i__ + j * x_dim1], SABS(r__2));
/* L60: */
                }
                work[k] += s;
/* L70: */
        ...
        for (i__ = 1; i__ <= i__2; ++i__) {
            if (work[i__] > safe2) {
/* Computing MAX */
                r__2 = s, r__3 = (r__1 = work[*n + i__], SABS(r__1)) / work[i__];
                s = (real)dmax(r__2,r__3);
            } else {
/* Computing MAX */
                r__2 = s, r__3 = ((r__1 = work[*n + i__], SABS(r__1)) + safe1)
                         / (work[i__] + safe1);
                s = (real)dmax(r__2,r__3);
            }
/* L80: */
...
/*        where W = ABS(R) + NZ*EPS*( ABS(op(A))*ABS(X)+ABS(B) ))) */
        ...
        for (i__ = 1; i__ <= i__2; ++i__) {
            if (work[i__] > safe2) {
                work[i__] = (r__1 = work[*n + i__], SABS(r__1)) + nz * eps * 
                        work[i__];
            } else {
                work[i__] = (r__1 = work[*n + i__], SABS(r__1)) + nz * eps * 
                        work[i__] + safe1;
...
/*        Normalize error. */
        ...
        for (i__ = 1; i__ <= i__2; ++i__) {
/* Computing MAX */
            r__2 = lstres, r__3 = (r__1 = x[i__ + j * x_dim1], SABS(r__1));
            lstres = (real)dmax(r__2,r__3);
/* L130: */
        }
        if (lstres != 0.f) {
            ferr[j] /= lstres;
