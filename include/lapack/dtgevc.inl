...
/* Subroutine */ static int dtgevc_(char *side, char *howmny, logical *select, 
        ...
        ftnlen howmny_len)
{
    ...
    static doublereal dmin__, temp, suma[4]        /* was [2][2] */, sumb[4]        
            /* was [2][2] */, xmax;
     /* Subroutine */ int dlag2_(doublereal *, integer *, doublereal *, 
...
/*              Scale to avoid underflow */
                ...
                scale = 1.;
                lsa = ABS(sbeta) >= safmin && ABS(acoef) < small1;
                lsb = ABS(salfar) >= safmin && ABS(bcoefr) < small1;
                if (lsa) {
                    scale = small1 / ABS(sbeta) * MIN(anorm,big);
                }
                if (lsb) {
/* Computing MAX */
                    d__1 = scale, d__2 = small1 / ABS(salfar) * MIN(bnorm,big);
                    scale = MAX(d__1,d__2);
                }
                if (lsa || lsb) {
/* Computing MIN */
