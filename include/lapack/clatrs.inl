...
/* Subroutine */ static int clatrs_(char *uplo, char *trans, char *diag, char *
        normin, integer *n, scomplex *a, integer *lda, scomplex *x, real *scale,
...
static real c_b36 = .5f;
    ...
    static integer imax;
    static real tmax;
    static scomplex tjjs;
    static real xmax, grow;
     /* Complex */ VOID cdotc_(scomplex *, integer *, scomplex *, integer 
            *, scomplex *, integer *);
     logical lsame_(char *, char *, ftnlen, ftnlen);
     /* Subroutine */ int sscal_(integer *, real *, real *, integer *);
    static real tscal;
    static scomplex uscal;
    static integer jlast;
     /* Complex */ VOID cdotu_(scomplex *, integer *, scomplex *, integer 
            *, scomplex *, integer *);
    static scomplex csumj;
     /* Subroutine */ int caxpy_(integer *, scomplex *, scomplex *, 
            integer *, scomplex *, integer *);
    static logical upper;
     /* Subroutine */ int ctrsv_(char *, char *, char *, integer *, 
            scomplex *, integer *, scomplex *, integer *, ftnlen, ftnlen, 
    ...
    static real bignum;
     ...
     doublereal scasum_(integer *, scomplex *, integer *);
...
/*     Determine machine dependent parameters to control overflow. */
    ...
    smlnum /= (real)slamch_("Precision", (ftnlen)9);
    bignum = 1.f / smlnum;
    *scale = 1.f;
...
/*           A is upper triangular. */
            ...
            for (j = 1; j <= i__1; ++j) {
                i__2 = j - 1;
                cnorm[j] = (real)scasum_(&i__2, &a[j * a_dim1 + 1], &c__1);
/* L10: */
            }
        } else {
...
/*           A is lower triangular. */
            ...
            for (j = 1; j <= i__1; ++j) {
                i__2 = *n - j;
                cnorm[j] = (real)scasum_(&i__2, &a[j + 1 + j * a_dim1], &c__1);
/* L20: */
            }
            cnorm[*n] = 0.f;
        }
...
/* Computing MAX */
        i__2 = j;
        r__3 = xmax, r__4 = (r__1 = x[i__2].r / 2.f, SABS(r__1)) + (r__2 = 
                (real)r_imag(&x[j]) / 2.f, SABS(r__2));
        xmax = dmax(r__3,r__4);
/* L30: */
    }
    xbnd = xmax;
...
/*              Exit the loop if the growth factor is too small1. */
                ...
                tjj = (r__1 = tjjs.r, SABS(r__1)) + (r__2 = (real)r_imag(&tjjs), 
                        SABS(r__2));
...
/*        elements of X is not too small1. */
...
/*              Compute x(j) = b(j) / A(j,j), scaling x if necessary. */
                ...
                i__3 = j;
                xj = (r__1 = x[i__3].r, SABS(r__1)) + (r__2 = (real)r_imag(&x[j]), 
                        SABS(r__2));
                if (nounit) {
                    i__3 = j + j * a_dim1;
...
/*                       x(1:j-1) := x(1:j-1) - x(j) * A(1:j-1,j) */
                        ...
                        i__3 = i__;
                        xmax = (r__1 = x[i__3].r, SABS(r__1)) + (r__2 = 
                                (real)r_imag(&x[i__]), SABS(r__2));
                    }
                } else {
                    if (j < *n) {
...
/* Computing MAX */
                i__3 = j;
                r__3 = xmax, r__4 = (r__1 = x[i__3].r, SABS(r__1)) + (r__2 = 
                        (real)r_imag(&x[j]), SABS(r__2));
                xmax = dmax(r__3,r__4);
/* L150: */
            }
