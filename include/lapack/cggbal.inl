...
/* Subroutine */ static int cggbal_(char *job, integer *n, scomplex *a, integer *lda, 
        scomplex *b, integer *ldb, integer *ilo, integer *ihi, real *lscale, 
        real *rscale, real *work, integer *info, ftnlen job_len)
{
    ...
    /* Builtin functions */
    double r_lg10(real *), r_imag(scomplex *), c_abs(scomplex *), r_sign(real *,
...
/*     Compute right side vector in resulting linear equations */
    ...
    for (i__ = *ilo; i__ <= i__1; ++i__) {
        ...
        for (j = *ilo; j <= i__2; ++j) {
            ...
            i__3 = i__ + j * a_dim1;
            r__3 = (r__1 = a[i__3].r, SABS(r__1)) + (r__2 = (real)r_imag(&a[i__ + j 
                    * a_dim1]), SABS(r__2));
            ta = r_lg10(&r__3) / basl;
...
L210:
            ...
            i__3 = i__ + j * b_dim1;
            r__3 = (r__1 = b[i__3].r, SABS(r__1)) + (r__2 = (real)r_imag(&b[i__ + j 
                    * b_dim1]), SABS(r__2));
            tb = r_lg10(&r__3) / basl;
...
L250:
    ...
    gamma = (real)sdot_(&nr, &work[*ilo + (*n << 2)], &c__1, &work[*ilo + (*n << 2)]
...
/* L330: */
    ...
    sum = (real)sdot_(&nr, &work[*ilo + *n], &c__1, &work[*ilo + (*n << 1)], &c__1) 
...
L350:
    ...
    for (i__ = *ilo; i__ <= i__1; ++i__) {
        ...
        rab = (real)c_abs(&a[i__ + (irab + *ilo - 1) * a_dim1]);
        i__2 = *n - *ilo + 1;
        irab = icamax_(&i__2, &b[i__ + *ilo * b_dim1], lda);
/* Computing MAX */
        r__1 = rab, r__2 = (real)c_abs(&b[i__ + (irab + *ilo - 1) * b_dim1]);
        rab = dmax(r__1,r__2);
        r__1 = rab + sfmin;
...
/* Computing MIN */
        ...
        cab = (real)c_abs(&a[icab + i__ * a_dim1]);
        icab = icamax_(ihi, &b[i__ * b_dim1 + 1], &c__1);
/* Computing MAX */
        r__1 = cab, r__2 = (real)c_abs(&b[icab + i__ * b_dim1]);
        cab = dmax(r__1,r__2);
        r__1 = cab + sfmin;
