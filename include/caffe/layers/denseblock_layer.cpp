...
// #endif
	...
	//blobs_[4*numTransition + i] is its globalVar
		...
		for (int transitionIdx = 0; transitionIdx < this->numTransition; ++transitionIdx) {
			...
			if (!useBC) {
				...
				std::shared_ptr<Filler<Dtype> > filter_Filler(GetFiller<Dtype>(dbParam.filter_filler()));
				...
				std::shared_ptr<Filler<Dtype> > filter_Filler3(GetFiller<Dtype>(dbParam.filter_filler()));
				...
				std::shared_ptr<Filler<Dtype> > filter_Filler1(GetFiller<Dtype>(dbParam.filter_filler()));
			...
			std::shared_ptr<Filler<Dtype> > weight_filler0(GetFiller<Dtype>(dbParam.bn_scaler_filler()));
			...
			if (useBC) {
				...
				std::shared_ptr<Filler<Dtype> > weight_filler0_4G(GetFiller<Dtype>(dbParam.bn_scaler_filler()));
			...
			std::shared_ptr<Filler<Dtype> > weight_filler1(GetFiller<Dtype>(dbParam.bn_bias_filler()));
			...
			if (useBC) {
				...
				std::shared_ptr<Filler<Dtype> > weight_filler1_4G(GetFiller<Dtype>(dbParam.bn_bias_filler()));
			...
			for (int blobIdx = 0; blobIdx < inChannels; ++blobIdx) {
				std::shared_ptr<Blob<Dtype> > localB = this->blobs_[3 * numTransition + transitionIdx];
			...
			if (useBC) {
				...
				std::shared_ptr<Blob<Dtype> > localB = this->blobs_[8 * numTransition + transitionIdx];
			...
			for (int blobIdx = 0; blobIdx < inChannels; ++blobIdx) {
				std::shared_ptr<Blob<Dtype> > localB = this->blobs_[4 * numTransition + transitionIdx];
			...
			if (useBC) {
				...
				std::shared_ptr<Blob<Dtype> > localB = this->blobs_[9 * numTransition + transitionIdx];
				for (int blobIdx = 0; blobIdx < 4 * growthRate; ++blobIdx) {
					localB->mutable_cpu_data()[localB->offset(0, blobIdx, 0, 0)] = 1;
				}
			}
	...
	void DenseBlockLayer<Dtype>::syncBlobs(DenseBlockLayer<Dtype>* originLayer) {
		vector<std::shared_ptr<Blob<Dtype> > >& originBlobs = originLayer->blobs();
		for (int blobIdx = 0; blobIdx < originBlobs.size(); ++blobIdx) {
			std::shared_ptr<Blob<Dtype> > localBlob = originBlobs[blobIdx];
			Blob<Dtype> * newBlob = new Blob<Dtype>(localBlob->shape());
			newBlob->CopyFrom(*(localBlob.get()), false);
			std::shared_ptr<Blob<Dtype> > sharedPtrBlob(newBlob);
